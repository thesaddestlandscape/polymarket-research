...
| **P18** ⭐prioridad Javi 16-Jul | **Smart Exit — STOP-LOSS+TAKE-PROFIT** (actualizado 22-Jul). Ya **implementado** (`_check_early_exit` en `live_trade.py`/`shadow_resolve.py`, `activo=false`) y pasó **2 rondas** de `/code-review` (16-Jul: 10 hallazgos arreglados; 18-Jul: 9 pendientes arreglados + 2 bugs nuevos graves). Calibración aislada confirmada: stop-loss -0.30€ → mejora +12.68€ (n=28). 22-Jul: **simulación COMBINADA TP+SL ya hecha** (`simular_combinado()`, `analisis_smart_exit.py`, commit `70ef796a0a`) — el óptimo conjunto NO coincide con los aislados (SL=-0.70€, peor aislado, bate a SL=-0.30€ cuando se combina con TP). Recomendación con datos: general TP=0.70/SL=-0.70 delta+10.16€, longshot TP=0.45/SL=-0.70 delta+26.86€ (cambia de signo). Ver `idea_smart_exit_combinado_tp_sl_22jul`. Javi confirmó 19-Jul esperar a recarga+switch ON — cumplida 21-Jul. | Ya no falta simular — falta **`/code-review` + aprobación explícita de Javi** de los umbrales combinados antes de `activo=true` (toca ejecución de dinero real, no autónomo) |
