...
|---|---|---|
| ~~P23~~ | ✅ **RESUELTO 23-Jul (sesión siguiente)**: gate riguroso completo (cruce exacto market_id+timestamp≤30s a `libro_snapshots.csv`, Wilson 90%+shuffle 5000+bootstrap PnL) sobre `ballenas_executor_5min.py` ETH/SOL/XRP. **Fill-ability CONFIRMADA excelente** (ETH 95.2%, SOL 100%, XRP 100%, profundidad mediana en cientos de €) — la parte más prometedora del hallazgo del 23-Jul mañana se sostiene. **Pero NINGÚN activo pasa el gate**: ETH (n=20, el único con muestra decente) da PnL/trade=−0.159€ — payout asimétrico real (py medio=0.824, favorito ya caro, hit=70% por debajo del ~82% que ese precio exige), confirmado estable en split-half (ambas mitades −0.16€/trade, hit=70%). SOL n=5 y XRP n=4 (4/4 ganadoras, sin varianza) insuficientes para concluir nada. **No promocionado.** Ver memoria `idea_ballenas_5min_gate_riguroso_payout_asimetrico_23jul`. | Seguir acumulando — book-check solo lleva desde 21-Jul ~19:55 UTC (n post-deploy: ETH 26, SOL 6, XRP 5 señales totales). Repetir el gate cuando ETH llegue a n≥40 matched+fillable |
