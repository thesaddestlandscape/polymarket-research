...
| **P27** ⭐⭐⭐ pendiente construcción, 23-Jul | **3 hilos abiertos del barrido de hipótesis** (no cada uno merece su propio P todavía, agrupados aquí): (a) **`WEEKLY_PRICE`** tiene 3 hipótesis fuertes simultáneas sin ninguna tupla live (`H-CUSTOM-WEEKLY-PCTDIST-BUYNO` IC+0.381 n=166, `H-WEEKLY-BUYNO` IC+0.284 n=266, `H-WEEKLY` SOL IC+0.252 n=143) — candidata a revisión a fondo; (b) **HMM sobre secuencias de wallets** (`idea_hmm_wallets_secuencias_13jul`, bloqueo de datos del 13-Jul YA RESUELTO — `ballenas_timing_history.csv` tiene 393k filas con `ts_trade` por wallet — paso barato hecho, indicio débil no concluyente en `0xfcdc071df708`, método de terciles roto en wallets de hit-rate extremo, ver `idea_hmm_wallets_bloqueo_resuelto_23jul`); (c) **30 hipótesis en `hipotesis_auto.md` ya cruzando su gate** desde que se desactivó el auto-apply (15-Jul) sin revisión manual sistemática — el cuello de botella es revisión, no señal. | (a) revisión a fondo de WEEKLY_PRICE con el mismo rigor de siempre antes de proponer live; (b) redo con correlación continua/percentiles finos en vez de terciles, ampliar a las 15 wallets de arquetipo C; (c) sesión dedicada de triage de las 30 hipótesis listas, priorizando por IC×n |
