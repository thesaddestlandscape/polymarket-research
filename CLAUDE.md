...
| **P28** ⭐⭐⭐⭐⭐ prioridad Javi 23-Jul — **payout adverso/varianza, 4 hilos aprobados** | Tras el hallazgo repetido de payout asimétrico (`FAVORITO_CONFIRMADO#BUY_NO`, familia GBM_LATE), Javi decidió sobre 5 ideas (detalle completo en memoria `project_decisiones_payout_varianza_23jul`, guardado ANTES de empezar a trabajar, petición explícita): **(1) Smart Exit → ESPERAR** (sin cambio sobre P18). **(2) Log-growth por bucket grande Y micro-bucket, APROBADO** — no un corte único, la misma metodología de dos niveles usada hoy con `WEEKLY_PRICE`/`mercado_anchura_pct` pero aplicada a `analisis_log_growth.py` (crecimiento geométrico, no solo IC): encontrar dentro de una tupla que pasa el gate agregado si hay un micro-bucket de precio con crecimiento negativo (excluir) o especialmente fuerte (apostar más ahí). **(3) Spread YES/NO intra-ventana (cobertura parcial), APROBADO PARA MIRAR** — investigación/diseño, no construir código de ejecución todavía; base en `idea_racha_correlacion_ventana` (09-Jul, cálculo ya hecho: reduce pérdida máxima de racha correlacionada 23%→8%). **(4) Fórmula Kelly exacta (f*=(p-π)/(1-π)), APROBADO "HACERLO PERFECTO"** — desagregar por LAS 5 dimensiones a la vez (moneda, franja horaria, timing, bucket grande, micro-bucket), no agregado ni parcial; cuidado, la fórmula exacta AGRANDA el tamaño en zona de favoritos con el mismo edge (no lo reduce) si la calibración no es perfecta ahí — ver `idea_kelly_ic_vs_formula_exacta_precio_21jul`. **(5) Portfolio HRP — PROPUESTA ENTREGADA 23-Jul** (`idea_propuesta_hrp_portfolio_23jul`): HRP real (López de Prado, clustering jerárquico + bisección recursiva, no Markowitz) sobre PnL diario de las 8 tuplas live. **Hallazgo crítico**: sin puerta de madurez, `BALLENAS_TARDIAS#BTC#15min` (4 días de historia) se lleva 86.3% del peso por varianza artificialmente baja — propuesta de puerta ≥14 días/≥100 trades antes de incluir una tupla en el cálculo. Con 6 tuplas maduras: `FAV_BTC60_NO` 41.2% (menor volatilidad + correlación negativa con el resto, el diversificador real), resto 9.5-14.3%. Integración propuesta: HRP como techo de exposición AGREGADA por tupla, Kelly sigue decidiendo el tamaño por trade individual — las dos capas conviven. | Ninguno de los 4 activos (2,3,4,5) toca código de producción todavía — 2 y 4 son análisis puro (shadow/lectura), 3 es investigación de diseño, 5 ya tiene propuesta (falta diseño de integración con guardias existentes + `/code-review` antes de tocar `live_stake.py`) |
