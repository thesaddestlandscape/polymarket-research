...
| **P18** ⭐prioridad Javi 16-Jul | **Smart Exit — STOP-LOSS+TAKE-PROFIT** (actualizado 22-Jul, la entrada anterior estaba desactualizada). Ya **implementado** (`_check_early_exit` en `live_trade.py`/`shadow_resolve.py`, `activo=false`) y pasó **2 rondas** de `/code-review` (16-Jul: 10 hallazgos arreglados; 18-Jul: 9 pendientes arreglados + 2 bugs nuevos graves cazados por el propio code-review, uno habría bloqueado el 100% de compras live). Calibración con haircut real (spread+fee ~6%) confirmada: stop-loss -0.30€ → mejora +12.68€ (n=28, `idea_smart_exit.md`). 22-Jul: 3 mejoras longshot revisadas — frecuencia de chequeo subida (crontab 60s→~20s), take-profit longshot (`entry_price<0.53`) analizado con n=36 (cruza gate), umbral 0.45 → delta +27€ (`idea_smart_exit_3mejoras_resultado_22jul.md`). Javi confirmó 19-Jul esperar a recarga+switch ON — **esa condición ya se cumplió (21-Jul)**. | Falta simular TP+SL **combinados** (no aislados) antes de proponer umbrales finales. Activar (`activo=true`) toca ejecución de dinero real: `/code-review` + aprobación explícita de Javi, no autónomo |
