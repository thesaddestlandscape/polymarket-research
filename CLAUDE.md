...
1. **Leer la misión y los objetivos, LO PRIMERO de todo, antes de cualquier otra cosa** — vault de Obsidian `/root/second-brain/02_projects/polymarket-research.md` (sección "Misión y objetivo", la versión narrativa que Javi lee/edita) + memoria nativa, en este orden (`/root/.claude/projects/-root-polymarket-research/memory/`): `project_mision_sistema.md` (esencia/8 cualidades) → `project_plan_escalonado_200k_23jul.md` (plan escalonado vigente con gates falsificables — **supersede a `project_roadmap_150k.md`, ya no leer ese**) → `feedback_obsesion_busqueda_constante_23jul.md` + `feedback_todo_alineado_al_objetivo_23jul.md` (directivas de 23-Jul: buscar constantemente nuevas estrategias/hipótesis/wallets cruzando postmortem+datos propios+ballenas+Polymarket+trading mundial, y citar SIEMPRE qué Stage del plan empuja cada propuesta). Este proyecto es **el proyecto de vida del usuario**: toda decisión de la sesión, grande o pequeña, tiene que ir alineada sin excepción a cumplir esos objetivos (€200k/año año 1, €300k/año año 2..., las 8 cualidades del sistema). No es una tarea más entre otras — es el criterio de fondo de todo lo demás, y se relee entero cada sesión, no solo la primera vez.
...
12. **Gate por micro-bucket de PnL propio (`gate_bucket_propio.py`), revisar cada sesión — orden imperativa de Javi, 2026-07-28**: reportar el estado de `data/shadow/gate_bucket_propio.json` (regenerado a diario por el cron `55 6 * * *` → `vigia_gate_bucket_propio.py`, que además avisa por Telegram solo veredictos NUEVOS vía latch) — (a) qué buckets nuevos cruzaron rigor (n≥15, shuffle p<0.05, split-half consistente en ambas mitades) desde la última sesión, tanto `malo_confirmado` (empiezan a VETAR ejecución real automáticamente, sin aprobación adicional por bucket — el mecanismo ya pasó `/code-review` una vez, extender a nuevos buckets con el MISMO criterio no lo requiere de nuevo) como `bueno_confirmado` (solo informativo hoy, no boostea stake); (b) cobertura por tupla — cuántos buckets siguen `sin_concluir` por n insuficiente, especialmente en las tuplas de 60min/5min donde `results.csv` tiene mucho menos volumen que 15min. Origen: sesión 28-Jul, tras refutar con datos reales (retrospectivo Wilson+shuffle+split-half+fill-ability) que reusar el timing de ballenas (`ballenas_banda_fina_gate.py`, FASE 0 solo observacional, NO usar su `vetaria_fase1` para nada operativo) para gatear estas tuplas no transfiere — 3/5 tuplas con n suficiente vetaban el grupo que en realidad ganaba más. Detalle completo en memoria `project_gate_bucket_propio_activo_28jul`.
...
**Plan escalonado hacia €200k/año (año 1), €300k/año (año 2), y así sucesivamente** (Javi, 23-Jul, aclarado explícitamente): crecimiento **orgánico** (recargas de Javi solo mientras el sistema no se autosostenga, como las 2 ya hechas), operando **todos los días** dentro de ventanas horarias validadas + las que se descubran, mentalidad fría/calculadora/competitiva. Gate de año 1: **548€/día de media** (200k/365 — NO 2.500€/día, esa cifra solo cuadra como techo aspiracional de muy largo plazo, no como objetivo de año 1). Baseline real 23-Jul: 250 trades desde 29-Jun, PnL neto acumulado **-23.00€**, edge_neto modelado +7.1% pero PnL realizado **negativo** (-0.092€/trade) — ese gap modelo↔realidad es el Stage 0 a resolver antes que cualquier escalado. 6 stages con gates falsificables (Stage 0 cerrar gap modelo↔realidad → Stage 1 ampliar ventanas horarias → Stage 2 despinear stake con cuidado → Stage 3 mercados más profundos 60min/daily/nested-arb → Stage 4 portfolio de motores no correlacionados → Stage 5 compounding hacia run-rate). Detalle completo, tabla de sensibilidad trades×stake×edge y bankroll estimado por stage en memoria nativa `project_plan_escalonado_200k_23jul` (supersede a `project_roadmap_150k`, desactualizada). **Cada sesión: situar en qué Stage está el sistema y qué gate falta, con datos frescos — no repetir esta cifra de memoria.**
...
| `analisis_diario_ballenas_ejecutor.py` | (23-Jul, cron diario `40 6 * * *`) Mismo espíritu que el anterior pero centrado en `ballenas_executor_5min.py`: franja fina por activo (ETH/SOL/XRP/DOGE#5min) cruzando histórico + observer live-fino (con timing `rest_mediana_min`) + nuestro propio ejecutor, timing (restante_s) vs hit/pnl, top wallets validadas por (activo,5m) desde `wallet_edge_score_por_activo_marco.json`, snapshot de banda operativa con alerta en log si se mueve → `data/shadow/analisis_diario_ballenas_ejecutor.json`. Solo lectura. |
| `wallet_especialistas_observer.py` | (23-Jul, cron diario `45 6 * * *`, petición explícita Javi) Wallets con ≥80% de su actividad concentrada en UNA moneda (de las 5) + edge validado (shuffle+BH-FDR), cubriendo los 5 marcos (5m/15m/60m/240m/weekly) vía `wallet_edge_score_por_activo_marco.json`. Persiste estado (`wallet_especialistas_state.json`) marcando cuáles son NUEVAS cada corrida, y cruza cada una contra nuestra propia cobertura (`results.csv` agregado por activo/marco) para detectar huecos reales (wallet con edge validado, nosotros con n<15 ahí). Solo lectura. |
| `analisis_diario_franja_15min.py` | (23-Jul, cron diario `50 6 * * *`, petición explícita Javi) Mismo patrón que `analisis_diario_ballenas_ejecutor.py` pero para 15min, las 5 monedas, TODOS los buckets (no solo significativos): histórico + observer live-fino (timing) + nuestro propio agregado (`results.csv`, sin ejecutor dedicado a 15min salvo BTC15m) + top wallets validadas por (activo,15m) → `data/shadow/analisis_diario_franja_15min.json`. Solo lectura. |
| `gate_bucket_propio.py` / `vigia_gate_bucket_propio.py` | (28-Jul) Gate ACTIVO en `shadow_predict.py` (`s_favorito_confirmado`, `s_gbm_late_15min`) que veta ejecución real cuando el precio cae en un micro-bucket `malo_confirmado` de `data/shadow/gate_bucket_propio.json` (rigor: n≥15, shuffle p<0.05, split-half consistente, sobre `results.csv` — nuestro propio histórico, no ballenas). Cron diario `55 6 * * *` regenera el JSON y avisa por Telegram solo veredictos nuevos (latch). Ver protocolo pt.12 y memoria `project_gate_bucket_propio_activo_28jul`. |
...
|---|---|---|
| ~~P23~~ | ✅ **RESUELTO+CORREGIDO 23-Jul** (desagregado por moneda+franja+cronología a petición de Javi, tras un primer veredicto demasiado pesimista): fill-ability **CONFIRMADA excelente** (ETH 95.2%, SOL 100%, XRP 100%). Del gate riguroso inicial (solo book-matched, n pequeño y sesgado a lo más reciente) salía "ETH payout asimétrico" — al desagregar por tercios/mitades cronológicos con histórico completo (`ballenas_5min_dry_run.csv`, sin filtrar a book-matched) el cuadro cambia: **XRP SÍ tiene payout asimétrico estructural real** (mismo hit=70% en ambas mitades, pero PnL +0.066€→−0.080€ cuando py sube de 0.615→0.755 — precio ya "confirmado", paga poco). **ETH NO es estructural** — py constante (0.78-0.82) en los 3 tercios pero hit-rate cae solo en las últimas ~36h (90.9%→95.5%→69.6%), histórico completo (n=67) sigue positivo (PnL/trade=+0.056€, CI90% cruza 0, no probado ni refutado). **SOL es la más limpia**: n=21, PnL/trade=+0.159€, CI90%=[+0.035,+0.241] no cruza cero, sin degradación en ninguna mitad. Cruce fino (bucket 0.05) contra `ballenas_timing_history.csv`: nuestro hit-rate sigue la curva general del mercado, sin bucket aislado con edge extra. Ver memoria `idea_ballenas_5min_gate_riguroso_payout_asimetrico_23jul` (incluye la corrección completa). | Ninguno de los 3 llega a n≥40 todavía. Priorizar dejar madurar SOL (señal más limpia). Vigilar si el próximo tercio de ETH confirma degradación real o vuelve a línea base 85-90%. XRP: NO proponer filtro de precio sin más n en la banda `<0.70` (hoy n=9, data dredging) |
| **P-LIVE** | **Monitorear primer trade real** | **Ventana 15:00 Madrid (13:00 UTC)** |
| **P19** ⭐prioridad Javi 21-Jul, actualizado 23-Jul | **Gap sigma implícita (Polymarket) vs realizada — filtro candidato para familia GBM**. `gap = sigma_implicita - sigma_h` (VRP adaptada, paper SSRN 6712647). **22-Jul**: instrumentado como feature observacional en `shadow_predict.py` (`_gap_sigma_implicita`, commit `04c3dffea6`, solo logging, no toca `prob_yes`) — lleva acumulando forward desde entonces. **23-Jul**: aplicado el gate riguroso completo (Wilson+shuffle+bootstrap+split-half, `analisis_gap_sigma_gate_riguroso_23jul.py`) por familia y activo — el z-test simple del 22-Jul ("8/8 replican p=0.0000") NO se sostiene igual de limpio con más rigor: **robusto** (negativo en ambas mitades y 3 tercios) en ETH, SOL, `GBM_LATE_15M_TARDIO`, `GBM_LATE_5M`, `GBM_LATE_15M_PYCONFIRMADO`, `UPDOWN_GBM`, `UPDOWN_GBM_15M_TARDIO`; **BTC es NULL plano** (pnl +0.012/+0.055/+0.041 en los 3 tercios jun24→jul23, nunca negativo); `GBM_LATE_15M` base ruidoso sin tendencia; `GBM_LATE_15M_ESPACIO_ATR` con tendencia de mejora monótona (-0.029→-0.013→+0.125, posible pérdida de vigencia). Las 2 tuplas ya live de esta familia (`UPDOWN_GBM_15M_TARDIO#BTC#15min`, `GBM_LATE_15M#ETH#15min`) caen justo en los casos NO robustos (BTC=null, base15M=inestable) — sin urgencia de tocar nada ahí. Detalle completo en memoria `idea_gap_sigma_replicado_por_familia_activo_22jul` (incluye la corrección del 23-Jul). NO implementado ningún filtro todavía. | Seguir dejando acumular forward (el logueo ya corre solo). Si se retoma: filtro solo para las 7 combinaciones robustas (nunca BTC ni las 2 familias inestables), y aun así `/code-review` + aprobación explícita de Javi antes de tocar `prob_yes` de cualquier tupla live |
| **P20** ⭐prioridad Javi 21-Jul, extiende P19 — **PAUSADO 23-Jul (decisión explícita Javi)** | **Meta-modelo de confianza sobre estructura temporal de volatilidad** — inspirado en paper SSRN 4666899 (Sheikh Sadik, rotación SVOL/VIXY con modelo primario + meta-modelo que evalúa si el primario está funcionando y reduce exposición hacia neutral cuando la confianza baja). Ya tenemos 3 piezas de este patrón DISPERSAS: `vigia_degradacion_live.py`, `vigia_log_growth.py`, `cooldown_factor_streak` (`live_stake.py`, único que AJUSTA algo automático). Ninguno usa estructura temporal de volatilidad. La extensión natural: comparar el gap de P19 a través de los 5 marcos (5/15/60/240min/weekly) del mismo activo — análogo a la curva VIX term structure. **23-Jul, escaneo de alcance antes de escribir código**: `gap_sigma_implicita` (P19) hoy SOLO existe donde hay una estrategia GBM digital-option activa — familia `UPDOWN_GBM`/`GBM_LATE_*`, que cubre 5/15/60min. **240min y weekly no tienen ninguna estrategia GBM** (240min solo aparece en `MARCO_BALLENAS_MAP` para datos de ballenas; `s_weekly_price` usa un mecanismo de rango de precio, no opción digital — el gap_sigma no se traslada 1:1 sin adaptar la fórmula). Se preguntó a Javi cómo acotar (3 marcos ya disponibles vs construir 240min/weekly primero vs pausar) — **eligió PAUSAR**: dado que P19 salió con BTC null y 2 familias (`GBM_LATE_15M` base, `ESPACIO_ATR`) inestables en el gate riguroso del 23-Jul, construir una curva de volatilidad encima de una pieza todavía no limpia en todos los activos arriesga amplificar ruido. | **No retomar sin que P19 esté más maduro/limpio** (ver estado P19 arriba) o sin decisión explícita nueva de Javi sobre el alcance (3 marcos vs 5 marcos). No implementar sin generalizar primero `cooldown_factor_streak` o diseñar el mecanismo de ajuste con Javi — toca sizing de dinero real, `/code-review` + aprobación explícita antes de cualquier cambio en `live_stake.py` |
| **P18** ⭐prioridad Javi 16-Jul | **Smart Exit — STOP-LOSS+TAKE-PROFIT** (actualizado 22-Jul). Stop-loss **implementado** (`_check_salidas_tempranas` en `shadow_resolve.py`, `smart_exit_stop_loss.activo=false`), calibración aislada: -0.30€ → mejora +12.68€ (n=28). 22-Jul: take-profit **añadido al código** (commit `6f132df3d6`, gate independiente `take_profit_activo=false`) + `/code-review` 8 ángulos completo (commit `bf65c9515e` arregla 1 comentario engañoso). **Hallazgo del code-review que cambia la decisión**: el beneficio combinado agregado (TP=0.70/SL=-0.70, +10.16€) NO se reparte uniforme — 91% viene de `GBM_LATE_15M`, `FAVORITO_CONFIRMADO` dispara TP el 72% de las veces (entra ya cerca del umbral) pero solo aporta +0.71€. Ver `idea_codereview_smart_exit_tp_segmentacion_22jul`. **DECISIÓN JAVI 22-Jul (textual)**: "vamos a esperar para calibrar el umbral, tendremos que estudiar también cómo repartirlo en diferentes estrategias live. La clave es que beneficie a todas." | NO activar el umbral uniforme. Pendiente: dejar crecer n en `smart_exit_prices.csv` (hoy n=66 total, ningún grupo por estrategia llega a n≥40) y diseñar un TP/SL **por familia de estrategia**, no uno global — `simular_combinado()` ya soporta filtrar por población, falta la n |
| **P24** ⭐⭐⭐ prioridad Javi 23-Jul — **Stage 0/4 del plan escalonado** | **"Wallet Mirror" — arquetipo C de ballenas: wallets con edge REAL validado (BH-FDR + split-half) en zona de precio medio/bajo (0.12-0.55)**, justo donde nuestro propio modelo GBM tiene edge teórico pero fill-ability ~0% (ver P23 arriba). 15 wallets validadas encontradas, split-half robusto en las 7 más sólidas (ninguna se revierte de signo): candidatas SEGUIR (`0x6c003e4ca1a6` DOGE#15m n=216, `0x079fe5415de4` DOGE#15m n=55, `0x32d5d0c7db60`/`0xe6f45ef61a4c` XRP#5m) y candidatas FADE (`0xfcdc071df708` XRP#15m n=1626 — el mayor volumen de la lista, `0x5bb44d0d51c6` SOL#5m n=549). Detalle completo en memoria `idea_wallet_mirror_arquetipo_c_23jul`. NO construido nada todavía — solo investigación/hallazgo. | Ampliar el escaneo a TODAS las bandas de precio/marcos (hoy solo se miró precio<0.55), medir lag detección→ejecución real (misma metodología que `ballenas_executor_5min`), gate riguroso completo antes de plantear nada — infraestructura nueva (detección en tiempo real por wallet específica), no una tarea de una sesión, mismo alcance que P22 |
| **P25** ⭐⭐⭐⭐⭐ prioridad Javi 23-Jul — **Stage 0 del plan escalonado** | **`favorito_confirma_coincide` — cuando FAVORITO_CONFIRMADO discrepa de la familia GBM_LATE en el mismo mercado, GBM_LATE acierta solo 26.4% (n=768, peor que coinflip) vs 73.7% cuando coinciden (n=308)** — z=14.31 p=0.0000, split-half robusto (1ª mitad 80.3%/23.8%, 2ª mitad 69.6%/29.7%), consistente en las 3 sub-familias (base/TARDIO/ESPACIO_ATR). Feature instrumentada 13-Jul, nunca analizada hasta hoy. **Caveat de siempre**: cobertura solo 6.4% (FAVORITO_CONFIRMADO no dispara en la mayoría de mercados GBM_LATE) y fill-ability real ~0% en ambos grupos (mismo cuello de botella de selección adversa de toda la familia GBM_LATE, ver P23). Detalle completo en memoria `idea_favorito_confirma_coincide_hallazgo_grande_23jul`. NO implementado — toca la tupla live `GBM_LATE_15M#ETH#15min#BUY_YES`. | Repetir restringido a `GBM_LATE_15M#ETH#15min` específicamente (el agregado mezcla activos/marcos no todos en producción) + medir fill-ability de esa tupla exacta + gate riguroso completo antes de proponer veto/flip. `/code-review` + aprobación explícita de Javi antes de tocar `shadow_predict.py`/decisión de la tupla live |
| **P26** ⭐⭐⭐⭐⭐ prioridad Javi 23-Jul — **Stage 0, conecta P24+P25** | **Wallets FADE (arquetipo C) más presentes donde GBM_LATE falla al discrepar de FAVORITO_CONFIRMADO**: 27.4% de presencia FADE en mercados NO_COINCIDE vs 15.2% en COINCIDE, z=3.31 p=0.0009 (join aproximado por activo+marco+minuto, NO exacto — no existe mapping `market_id`↔`condition_id` cacheado hoy). Posible mecanismo unificado entre P24 y P25: el mercado es más ruidoso/con más dinero tonto precisamente cuando nuestro modelo falla. Detalle en memoria `idea_conexion_p24_p25_fade_explica_discrepancia_23jul`. | **Construir mapping `market_id`↔`condition_id` cacheado (infraestructura pequeña, reusable para futuros cruces)** + repetir el join con match exacto antes de confirmar. Si sobrevive, evaluar si "presencia FADE" cubre más mercados que `favorito_confirma_coincide` (hoy 6.4%) |
| **P27** ⭐⭐⭐⭐⭐ pendiente construcción, actualizado 23-Jul — **verificado con gate riguroso, 2 candidatas listas** | **Verificación completa (Wilson+shuffle+PnL bootstrap) de las 6 hipótesis más fuertes del barrido: 5/6 SÍ son concluyentes** (solo falla `FAVORITO_SOL_ALTACONVICCION`, payout asimétrico ya conocido). **Candidata 1 — ACTUALIZADA tras desagregado completo (por moneda/bucket/timing/ballenas, petición Javi 23-Jul)**: el agregado original (`pct_dist≥2%`, n=166) se rompe por moneda — BTC (n=31) y ETH (n=34) NO CONCLUYENTE (PnL CI90% cruza 0), **solo SOL pasa** (n=101 GATE OK). Dentro de SOL, el bucket grande revela que el umbral real es **`pct_dist≥10%`, no ≥2%** (`[2,5)` puro ruido hit=45.5%; `[10,+)` GATE OK n=80 hit=100%). Timing sin degradación (3 tercios 86.8-100%), split-half perfecto (40/40, 100% ambas mitades). **CORREGIDO 23-Jul (misma sesión)**: `WEEKLY_PRICE#SOL#BUY_NO` YA estaba en `candidatos_evaluacion_live` (nada que añadir) — fill-ability real del subconjunto `pct_dist≥10%` (el único con edge real) es **0/30 = 0%** (join `market_id` results.csv↔libro_snapshots.csv). El 39.3% agregado citado antes mezclaba las 3 monedas/2 direcciones, engañoso para este subconjunto específico. **Mismo patrón arquetipo A que GBM_LATE toda la sesión — NO promocionable a `pares_permitidos_live` hoy.** Dejar acumular en candidatos_evaluacion_live (ya activo), revisar cuando n>30. BTC/ETH mismo mecanismo esperado, n insuficiente (3/5). **Candidata 2 — filtro `mercado_anchura_pct≥0.056` para `GBM_LATE_15M#ETH#15min#BUY_YES` (YA LIVE)**: con filtro n=104 hit=77.9% PnL/trade=+0.540€ GATE OK; sin filtro (anchura<0.056) n=164 hit=49.4% NO CONCLUYENTE, puro ruido — separa limpiamente la mitad buena de la mala dentro de una tupla que ya opera con dinero real. Detalle en memoria `idea_dos_candidatas_gate_riguroso_confirmado_23jul`. Hilos menores sin resolver: HMM sobre secuencias de wallets (bloqueo de datos resuelto, indicio débil, ver `idea_hmm_wallets_bloqueo_resuelto_23jul`); resto de las 30 hipótesis del barrido sin triage individual todavía. **Candidata 1**: DESCARTADA por ahora (fill-ability 0%) — solo dejar acumular n en `candidatos_evaluacion_live`, no proponer nada hasta que crezca. **Candidata 2**: **diferida al lunes** (petición explícita Javi 23-Jul, sesión sin margen) — verificar frecuencia real del filtro en producción antes de proponer, `/code-review` + aprobación explícita antes de tocar `shadow_predict.py` de la tupla live |
| **P28** ⭐⭐⭐⭐⭐ prioridad Javi 23-Jul — **payout adverso/varianza, 4 hilos aprobados** | Tras el hallazgo repetido de payout asimétrico (`FAVORITO_CONFIRMADO#BUY_NO`, familia GBM_LATE), Javi decidió sobre 5 ideas (detalle completo en memoria `project_decisiones_payout_varianza_23jul`, guardado ANTES de empezar a trabajar, petición explícita): **(1) Smart Exit → ESPERAR** (sin cambio sobre P18). **(2) Log-growth por bucket grande Y micro-bucket — HECHO 23-Jul** (`analisis_log_growth_por_bucket_23jul.py`): el payout inverso de `FAVORITO_CONFIRMADO#{BTC,SOL,ETH}#15min#BUY_NO` NO es uniforme — vive concentrado en precio 0.40-0.75 (confianza moderada, TODO negativo), mientras 0.75-1.00 (favorito muy confirmado) SÍ pasa el gate de crecimiento en las 3 monedas. `ETH#60min#BUY_NO` (pausada 21-Jul) ha cambiado de signo con más n (+0.00107 vs -0.00237 original) — vigilar. **(3) Spread YES/NO intra-ventana (cobertura parcial), APROBADO PARA MIRAR** — investigación/diseño, no construir código de ejecución todavía; base en `idea_racha_correlacion_ventana` (09-Jul, cálculo ya hecho: reduce pérdida máxima de racha correlacionada 23%→8%). **(4) Fórmula Kelly exacta — CUANTIFICADO 23-Jul** (`analisis_kelly_exacto_desagregado_23jul.py` / `idea_kelly_exacto_cuantificado_23jul`): al contrario de lo que la advertencia teórica de `idea_kelly_ic_vs_formula_exacta_precio_21jul` sugería, con datos reales el efecto dominante es que el sizing actual **SOBREDIMENSIONA** `FAVORITO_CONFIRMADO` (ratio 0.53-0.66x agregado, hasta **0.07x** en SOL favorito muy confirmado `[0.80,1.00)`) — amplifica activamente el payout adverso en vez de corregirlo. `GBM_LATE_15M`/`UPDOWN_GBM_15M_TARDIO` van al revés, **infradimensionadas** (ratio 1.6-2.55x, hasta 7.73x en un micro-bucket). CORREGIDO mismo día: `FAVORITO_CONFIRMADO` SÍ loguea timing, con otro nombre (`restante_min`, no `T_h`) — desagregado en las 6 combinaciones `{BTC,ETH,SOL}#15min#{BUY_YES,BUY_NO}`: timing NO consistente entre monedas (BTC/SOL empeoran con más tiempo, ETH mejora), pero precio SÍ consistente — `[0.60,0.80)` es la zona sistemáticamente sobredimensionada en las 6 combinaciones (ratio 0.02x-0.19x). `GBM_LATE_15M` por moneda (BTC/SOL/XRP/ETH, #15min BUY_YES): patrón mucho más limpio, `[0.00,0.40)` correctamente en ratio≈0 (coincide con fill-ability=0% ya conocido), `[0.40,0.60)` sistemáticamente infradimensionada en las 4 monedas, extremo `[0.40,0.45)` el peor (2.35x-7.73x). XRP es el caso más uniforme (ratio~2.0-2.1x en TODOS los cortes). **(5) Portfolio HRP — PROPUESTA ENTREGADA 23-Jul** (`idea_propuesta_hrp_portfolio_23jul`): HRP real (López de Prado, clustering jerárquico + bisección recursiva, no Markowitz) sobre PnL diario de las 8 tuplas live. **Hallazgo crítico**: sin puerta de madurez, `BALLENAS_TARDIAS#BTC#15min` (4 días de historia) se lleva 86.3% del peso por varianza artificialmente baja — propuesta de puerta ≥14 días/≥100 trades antes de incluir una tupla en el cálculo. Con 6 tuplas maduras: `FAV_BTC60_NO` 41.2% (menor volatilidad + correlación negativa con el resto, el diversificador real), resto 9.5-14.3%. Integración propuesta: HRP como techo de exposición AGREGADA por tupla, Kelly sigue decidiendo el tamaño por trade individual — las dos capas conviven. | Ninguno de los 4 activos (2,3,4,5) toca código de producción todavía — 2 y 4 son análisis puro (shadow/lectura), 3 es investigación de diseño, 5 ya tiene propuesta (falta diseño de integración con guardias existentes + `/code-review` antes de tocar `live_stake.py`) |
| **P21** ⭐prioridad Javi 22-Jul | **LP farming — programa de rewards por liquidez de Polymarket** (petición explícita Javi: guardar como estrategia futura). Investigado 22-Jul: programa REAL, confirmado vía API propia de Polymarket contra un mercado BTC#15min real de hoy (`feeSchedule={'rate':0.07,'exponent':1,'takerOnly':True,'rebateRate':0.2}`, `rewardsMinSize=50`, `rewardsMaxSpread=4.5`) — `rate=0.07` coincide EXACTO con `FEE_RATE_TAKER_CRYPTO` ya usado en `live_trade.py`, re-verificado fresco hoy contra fee_eur real de 15 trades cerrados (`fee_eur=stake_eur×0.07×(1-precio)` match exacto, también en 60min, no solo 15min como decía una fuente externa incompleta). NO rentable con el bankroll actual (~25-26€): `rewardsMinSize=50` shares ≈ toda nuestra posición en UNA orden de UN lado de UN mercado, el pool se reparte por score relativo a TODOS los makers (competiríamos con MM profesionales ya detectados en el libro, hallazgo Kyle's lambda 10-Jul), y la selección adversa taker ya documentada (ejecutadas 19-47% hit vs vetadas 83-97%) implica el mismo riesgo desde el lado maker. Detalle completo en memoria `idea_lp_farming_rewards_polymarket_22jul`. | Bloqueado hasta bankroll sustancialmente mayor + bot de gestión activa de cola dedicado (infraestructura nueva completa, no extiende el stack taker actual) — no antes |
| **P22** ⭐prioridad Javi 23-Jul | **Bot de gestión activa de cola de posición (queue position) — maker inteligente, TODAS las monedas, no solo XRP**. Misma pieza de infraestructura que bloquea P21 (LP farming) y que ya se probó en su versión "tonta" (maker estático a precio fijo, `analisis_maker_xrp_tardio.py`, EV negativo en 6 precios — falla por selección adversa: te llenan justo cuando el mercado se mueve en tu contra). La versión "inteligente" (teoría de Moallemi & Yuan sobre queue position, la misma que sustenta P21): vigila la posición real en la cola FIFO del libro CLOB en tiempo real y cancela/reposiciona dinámicamente según señales en vivo (desequilibrio del libro, actividad de ballenas, precio acercándose desde el lado adverso) en vez de poner-y-esperar. Motivado 23-Jul al diagnosticar que XRP #15min sigue con selección adversa sistémica en TODAS sus estrategias (fillable hit-rate 18-47% con n≥10 en cada combinación estrategia#dirección, vs 60-70%+ en shadow sin filtrar) — pero el mecanismo no es específico de XRP, aplicaría a cualquier señal donde el taker choca con selección adversa por libro fino (mismo patrón ya visto en BTC#15min payout asimétrico). Infraestructura nueva completa (monitorización de libro en tiempo real + lógica cancel/replace), no extiende el stack taker actual (`live_trade.py`). **Ángulo nuevo 23-Jul**: `WEEKLY_PRICE#SOL#BUY_NO pct_dist≥10%` (P27, edge validado pero fill-ability taker=0%) podría ser el caso de prueba MÁS favorable para esta idea — mercados que resuelven en días, no minutos, dan mucho más margen para que una orden maker se rellene con paciencia, a diferencia de cripto 5/15min donde "esperar" no es viable. No investigado, solo anotado. | NO empezar a construir sin diseño explícito con Javi — toca ejecución de dinero real desde cero, `/code-review` antes de cualquier código que envíe/cancele órdenes reales. Candidato natural para las próximas sesiones (petición explícita 23-Jul), pero requiere su propio hilo de trabajo, no una tarea de una sesión |
| P6 | Cross-asset: GBM+OF BUY_NO mismo activo → ×1.5 | n≥20 ops OF BUY_NO post-filtro |
| **P7** ⭐⭐ **PRIORITARIO lunes, retomar primero** | **Kelly por hora, REDISEÑADO 23-Jul — pendiente `/code-review` antes de mergear**. El mecanismo agregado por hora (un IC pooling TODAS las estrategias/activos/direcciones) escondía celdas negativas: `GBM_LATE_60M` negativa en las 3 horas candidatas 13/15/17/19h UTC, `FAVORITO_CONFIRMADO` a las 19h con IC positivo pero PnL real **-15.20€** (payout asimétrico). Además un bug (`all_ready = all(...)`) bloqueaba el aviso de Telegram indefinidamente si UNA hora del set no cruzaba el umbral, aunque las otras llevaran miles de operaciones. Rediseño completo: `_eval_kelly_hora` ahora evalúa celdas `ESTRATEGIA#SUBTYPE#DIRECCION#HORA` con el MISMO gate riguroso que una promoción a live (Wilson+shuffle+PnL bootstrap, `analisis_gate_riguroso.gate`), persiste todo en `data/shadow/kelly_hora_segmentado.json`, avisa por Telegram solo celdas nuevas con latch propio (`kelly_hora_avisadas_latch.json`). `_hora_stake_factor` (`shadow_predict.py`) actualizado a buscar la clave compuesta en vez de solo la hora. **Verificado end-to-end** (no solo lectura): corrida real sobre 30714 filas en 0.52s, encontró 2 celdas reales pasando el gate completo (`GBM_LATE_15M#XRP#15min#BUY_YES#19` n=41 PnL/trade=+1.22€ CI90%=[+0.83,+1.59]; `#BUY_NO#13` n=40 PnL/trade=+0.54€ CI90%=[+0.07,+0.98]), Telegram real enviado y confirmado idempotente. **Trabajo completo en rama `wip/kelly-hora-segmentado-23jul` (commit `236715558f`), NO en main** — `main` está limpio/revertido a la última versión revisada (`git checkout main` hecho 23-Jul, el loop en vivo sigue ejecutando el código antiguo sin el fix hasta que esto se revise y mergee). `meta.hora_boost_factor` sigue vacío, nada de esto aplica ningún boost automático. | **Lunes: `/code-review` de la rama WIP antes de mergear a main.** Después de mergear, verificar que el fast loop recoge el código nuevo (restart si hace falta, `verify_deploy.py`) y vigilar los primeros avisos de Telegram de celdas nuevas. |
