...
|---|---|---|
| **P23** ⭐⭐ HACER PRIMERO próxima sesión (Javi, 23-Jul) | **Fill-ability + gate riguroso completo sobre `ballenas_executor_5min.py` (DRY_RUN) para ETH/SOL#5min** — el lead más prometedor encontrado en 5min hasta hoy. Contexto: barrido de TODAS las estrategias 5min por moneda (23-Jul) confirmó que la vía "modelo propio" (GBM_LATE_5M, FAVORITO_CONFIRMADO, LATE_WINDOW_5MIN, ORDER_FLOW_5M) sigue rota en las 5 monedas — fill-ability real BTC 16.1%, XRP 29.8%, SOL 40.4%, ETH 44.7% (n=47-180, `libro_snapshots.csv` ratio≥5x). `FAVORITO_CONFIRMADO#{ETH,SOL}#5min` es trampa doble: IC positivo en shadow pero PnL ya negativo (-27.92€/-40.70€, payout asimétrico) Y fill-ability cae a 40-51%. **Pero** `ballenas_executor_5min.py` (timing real de ballenas, no modelo de precio, arquetipo B) muestra SOL n=21 hit=95.2%, ETH n=65 hit=84.6%, XRP n=20 hit=70.0% — y un cruce aproximado contra `libro_snapshots.csv` (mismos market_id, no necesariamente al precio/momento exacto de la señal) muestra profundidad real en ETH 84% (27/32), SOL 75% (6/8), XRP solo 55% (6/11) — patrón EXACTAMENTE INVERTIDO al del modelo propio, coherente con arquetipo B (edge ballenas → mejor fillable). Nota colateral: la vieja nota de memoria que llamaba a `LATE_WINDOW_5MIN#BTC#5min` "el único punto positivo del 5min" está OBSOLETA — hoy IC=-0.045 PNL=-2.36€ shadow y fill-ability 10-16.3%, hay que corregirla. | **Pasos exactos para la próxima sesión** (no re-derivar desde cero, ver memoria `project_prioridad_5min_ballenas_executor_23jul`): (1) cruce fill-ability RIGUROSO (no aproximado) de `ballenas_5min_dry_run.csv` contra `libro_snapshots.csv` — necesita capturar `ratio_vs_stake` al precio/momento EXACTO de la señal ballena, no solo "algún snapshot del mismo market_id" como el chequeo aproximado de hoy; (2) `analisis_gate_riguroso.py` (Wilson+shuffle+bootstrap PnL) sobre el subconjunto fillable de ETH/SOL; (3) split-half + shuffle antes de proponer nada (feedback_shuffle_antes_de_bloquear); (4) SOLO si todo pasa limpio, plantear candidata — no promocionar en la misma sesión que se descubre, dejar madurar/revisar con Javi primero |
