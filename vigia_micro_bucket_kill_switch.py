...
STEP = 0.05
N_MIN_ALERTA = 5
FEE = 0.07  # mismo FEE_RATE_TAKER_CRYPTO del resto del proyecto
# 07-Sep (mismo hallazgo real que en vigia_micro_bucket_kill_switch_
# wallet_mirror.py, ver ese fichero para el caso concreto que lo motivó):
# "PnL agregado en €<0 con n>=3" no distingue ruido de payout asimétrico
# de una señal genuinamente rota -- un bucket de precio alto exige
# hit-rate alto para breakeven, y un solo fallo entre 3-4 trades tira el
# agregado a negativo aunque el hit-rate real esté en línea. Fix: test
# binomial de una cola contra el breakeven del precio real pagado.
P_MAX_ALERTA = 0.15


def _breakeven_hitrate(entry: float) -> float:
    """Hit-rate p* tal que EV neto=0 al precio pagado `entry` (misma
    fórmula pnl_neto=gross_win*(1-FEE) si acierta, -1.0 si no, que usa
    el resto del proyecto -- gate_bucket_propio.py, wallet_mirror_gate_
    bucket.py, analisis_bot_wallets_gate_bucket_25ago.py)."""
    gross_win = (1 - entry) / entry
...
def main() -> int:
    ...
    for strategy, subtype, direction in TUPLAS_VIGILADAS:
        ...
        for entry, pnl in trades:
            ...
            b = _bucket(py)
            por_bucket.setdefault(b, []).append((entry, pnl))
        ...
        for b, filas in por_bucket.items():
            ...
            # Test binomial de una cola contra el breakeven al precio REAL
            # pagado por cada trade (no el bucket agregado en €, ver
            # docstring arriba) -- evita el falso positivo por payout
            # asimétrico ya cazado en la variante WALLET_MIRROR.
            hits = sum(1 for _, pnl in filas if pnl > 0)
            p0 = sum(_breakeven_hitrate(entry) for entry, _ in filas) / n
            ...
            alertas.append((tupla_str, b, n, pnl_total, hits, p_valor))
            if not ya_override:
            ...
            vistos[clave_latch] = {"n": n, "pnl_total": round(pnl_total, 2), "hits": hits, "p_valor": round(p_valor, 4)}
    ...
    if alertas:
        detalle = "\n".join(
            f"  {tupla}: bucket [{b:.2f},{b+STEP:.2f}) n={n} hits={hits} PnL real={pnl:+.2f}€ p={p_valor:.3f}"
            for tupla, b, n, pnl, hits, p_valor in alertas
        )
        msg = (
