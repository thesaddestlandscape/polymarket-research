...
import candidata9_bot_consenso_executor as _m_candidata9
import dispersed_bot_executor_dryrun as _m_dispersedbot
...
# 08-Sep: candidata9_bot_consenso_executor.py añadido -- checklist de 6
# categorías completo para CANDIDATA9_BOT_CONSENSO#ETH#15min[0.50,0.55)
# (gate propio, fill-ability real 75%, cruce con ballenas incremental,
# Kelly g=+0.023 sin payout inverso; único hueco aceptado explícitamente
# por Javi: concentración de wallet top1+2=61.8%). DRY_RUN=True todavía
# a nivel de módulo -- whitelisteado en pares_permitidos_live para que el
# log de DRY_RUN muestre señales reales, pero SIN ejecutar dinero real
# hasta un flip explícito posterior (ver docstring del módulo).
# 08-Sep (mismo día, sesión de cierre Stage 0): dispersed_bot_executor_
# dryrun.py añadido -- DRY_RUN=False YA activo, pero SOLO para
# SNIPER#BTC#5min[0.25,0.30) (checklist 6 categorías + 2/2 días estables,
# ver docstring del módulo para los 6 guardianes verificados). Sacado de
# ejecutores_dryrun_fase0.py (exige DRY_RUN=True en todos sus módulos,
# se habría negado a arrancar). El resto del universo que este módulo
# trackea (17 candidatos más) sigue en observación pura -- ningún otro
# bucket está en BUCKETS_APROBADOS_REAL ni en pares_permitidos_live.
MODULOS = [
    ...
    ("CANDIDATA9_BOT_CONSENSO", _m_candidata9),
    ("DISPERSED_BOT", _m_dispersedbot),
]
