#!/usr/bin/env python3
"""Resuelve el tracker de ballenas_executor_15min.py (data/shadow/
ballenas_15min_dry_run.csv) contra el outcome OFICIAL de Polymarket
(gamma-api, mismo mecanismo que shadow_resolve.py -- resolutionSource real
de estos mercados, no un proxy de klines).

Origen (29-Jul): mismo patrón que resuelve_ballenas_5min.py (21-Jul) --
el ejecutor SIEMPRE apuesta BUY_YES (única dirección que dispara), así
que acierto = outcome_real == "YES".
...
TRACKER = REPO / "data/shadow/ballenas_15min_dry_run.csv"
# Mismo fichero de lock que ballenas_executor_15min.py (screen ballenas_15m,
# persistente, también reescribe TRACKER entero) -- ver comentario
# equivalente en resuelve_ballenas_5min.py, mismo motivo.
TRACKER_LOCK = REPO / "data/shadow/ballenas_15min_dry_run.csv.lock"
...
def main():
    if not TRACKER.exists():
        print("Sin tracker todavía -- ballenas_executor_15min.py no ha confirmado nada aún.")
        return 0

    # Mismo diseño que resuelve_ballenas_5min.py (/code-review 27-Jul): el
    # lock NO puede envolver las llamadas de red a gamma-api (estado_mercado,
    # con reintentos/backoff en 429 -- puede tardar varios segundos) --
    # ballenas_executor_15min.py llama a _registrar_tracker() de forma
    # SÍNCRONA en el hot path de una
