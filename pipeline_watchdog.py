    ...
    try:
        ...
        if en_v and not switch_on:
            if time.time() - _switch_alerta_ts > SWITCH_ALERTA_COOLDOWN:
                ...
                # Crítica con reintento (auditoría Telegram 14-Jul): sin
                # esto, un fallo puntual de envío deja el bot parado
                # durante la ventana hasta 30min (SWITCH_ALERTA_COOLDOWN)
                # sin que nadie se entere.
                enviar_telegram_critico(
        ...
        try:
            ...
            if nuevos:
                ...
                try:
                    ...
                    log(f"  [deploy-obsoleto] Telegram falló: {e}")
                # Auditoría Telegram 14-Jul: antes se marcaba como
                # alertado pase lo que pase (incluso si enviar_telegram
                # lanzaba excepción, tragada en silencio) — un fallo
                # puntual dejaba el deploy obsoleto sin re-avisar nunca
                # más, aunque nadie se hubiera enterado. Solo se marca lo
                # que sí se confirmó enviado; lo demás se reintenta en el
