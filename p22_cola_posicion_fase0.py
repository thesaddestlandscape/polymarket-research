...
def _iniciar_tail_libro() -> None:
    """Fija la posición de partida del tail al final del fichero (todo lo
    anterior ya lo cubrió `_marcar_backlog_como_visto()`) y captura el
    header -- evita releer libro_snapshots.csv ENTERO en cada ciclo de
    CICLO_TAIL_S=3s (bug real 11-Ago encontrado en barrido de salud: este
    fichero ya supera 11MB y crece 24/7, el full re-read cada 3s era una
    causa real de la sobresuscripción de CPU/RAM que degradaba live_trade;
    ver feedback_medico_proyecto_barrido_profundo_05ago)."""
...
def _leer_filas_nuevas() -> list:
    ...
    if tam < _libro_pos:
        # truncado/rotado -- no debería pasar (el fichero solo crece), pero
        # fail-safe: reinicia el tail desde el principio en vez de crashear
        # o perder datos en silencio.
        _libro_pos = 0
        _libro_header = None
        return []
    if tam == _libro_pos:
