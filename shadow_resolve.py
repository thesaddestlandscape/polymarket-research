...
YA_RESUELTAS_CACHE_TXT = DIR_SHADOW / "ya_resueltas_cache.txt"
...
def cargar_ya_resueltas() -> set:
    ...
    # 07-Sep (Javi, "no podemos permitirnos que el pipeline tarde nunca más
    # de 120s"): esta función releía results.csv COMPLETO (359MB/438k filas
    # y creciendo ~3-4MB/día) en CADA ciclo de resolve, solo para
    # reconstruir un set de claves ya vistas -- 11.7s medidos hoy, cada vez
    # más lento, mismo patrón exacto que el que ya se arregló en
    # shadow_postmortem.py (commit 5b7ee6f594, calibración Platt) esta
    # misma sesión. results.csv es estrictamente APPEND-ONLY (ningún script
    # del proyecto lo abre en modo escritura no-append -- verificado con
    # grep antes de este fix), así que solo hace falta parsear las filas
    # NUEVAS desde el último checkpoint, no el fichero entero cada vez.
    #
    # Checkpoint = (offset en bytes, header exacto en ese momento). Si el
    # header cambiara de orden/columnas entre corridas (razón teórica, no
    # observada), o el fichero encogiera por debajo del offset guardado
    # (imposible bajo append-only salvo intervención manual), el caché se
    # invalida y se hace una lectura completa -- fail-safe: en el peor caso
    # un ciclo más lento, NUNCA una clave perdida u obtenida de una columna
    # equivocada (eso duplicaría una resolución -- doble conteo de IC, o
    # peor, cerraría un trade real dos veces).
    header_actual = None
    with open(RESULTS_PATH, encoding="utf-8") as f:
        header_actual = f.readline().rstrip("\n")

    checkpoint = None
    if YA_RESUELTAS_CHECKPOINT.exists():
        try:
            checkpoint = json.loads(YA_RESUELTAS_CHECKPOINT.read_text(encoding="utf-8"))
        except Exception:
            checkpoint = None

    offset_previo = checkpoint.get("offset_bytes") if checkpoint else None
    header_previo = checkpoint.get("header") if checkpoint else None
    usar_cache = (
        isinstance(offset_previo, int)
        and 0 < offset_previo <= size_actual
        and header_previo == header_actual
        and YA_RESUELTAS_CACHE_TXT.exists()
    )

    ya = set()
    if usar_cache:
        with open(YA_RESUELTAS_CACHE_TXT, encoding="utf-8") as f:
            for linea in f:
                partes = linea.rstrip("\n").split("\t")
                if len(partes) == 3:
                    ya.add(tuple(partes))
        offset_inicio = offset_previo
    else:
        offset_inicio = 0  # caché ausente/inválido -- reconstruir desde cero

    cols = header_actual.split(",")
    try:
        idx_strategy, idx_market_id, idx_decision = (
            cols.index("strategy"), cols.index("market_id"), cols.index("decision"))
    except ValueError:
        # Header inesperado (no debería pasar nunca) -- fail-safe total:
        # cae al comportamiento ORIGINAL (lectura completa vía DictReader
        # por nombre de columna, inmune a orden) en vez de arriesgar índices.
        ya = set()
        with open(RESULTS_PATH, encoding="utf-8") as f:
            for row in csv.DictReader(f):
                ya.add((row.get("strategy", ""), row.get("market_id", ""), row.get("decision", "")))
        return ya
    ...
    if offset_inicio < size_actual:
        with open(RESULTS_PATH, "rb") as f:
            ...
            bloque = f.read(size_actual - offset_inicio)
        texto = bloque.decode("utf-8", errors="replace")
        filas = list(csv.reader(io.StringIO(texto)))
        if offset_inicio == 0 and filas:
            filas = filas[1:]  # saltar cabecera
        n_cols_min = max(idx_strategy, idx_market_id, idx_decision) + 1
        for row in filas:
            if len(row) < n_cols_min:
                continue
            clave = (row[idx_strategy], row[idx_market_id], row[idx_decision])
            ya.add(clave)
            nuevas_lineas.append("\t".join(clave) + "\n")

    # Persistir el caché ANTES de avanzar el checkpoint -- si algo falla a
    # mitad, el checkpoint se queda atrás (próximo ciclo relee un poco más,
    # inofensivo) en vez de avanzar sin haber guardado las claves nuevas
    # (eso sí perdería claves para siempre -- riesgo real de doble
    # resolución). Nunca reordenar estas dos escrituras.
    try:
        with open(YA_RESUELTAS_CACHE_TXT, "a" if usar_cache else "w", encoding="utf-8") as f:
            f.writelines(nuevas_lineas)
        tmp = YA_RESUELTAS_CHECKPOINT.with_suffix(".json.tmp")
        tmp.write_text(json.dumps({"offset_bytes": size_actual, "header": header_actual}), encoding="utf-8")
        tmp.replace(YA_RESUELTAS_CHECKPOINT)
    except Exception as e:
        print(f"  ⚠️ fallo al persistir caché de ya_resueltas ({type(e).__name__}: {e}) — "
              f"el próximo ciclo recalculará desde el último checkpoint bueno")
...
def _check_salidas_tempranas(ts: str):
    ...
    try:
        ...
        # 09-Sep (petición explícita Javi, WALLET_MIRROR#BTC#15min sangrando
        # -5,87€ reales: 29 wins avg+0,35€ vs 14 losses avg-1,14€, payout
        # asimétrico clásico -- ver idea_smart_exit_por_tupla_wallet_mirror_09sep):
        # umbral POR TUPLA (strategy#subtype), opt-in explícito -- si esta
        # clave no está vacía, SOLO las tuplas listadas aquí se evalúan para
        # salida temprana (TP incluido); cualquier otra tupla OPEN se salta
        # sin tocar, exactamente el comportamiento de hoy (activo=false a
        # nivel global quedaría igual de inerte para ellas). Evita repetir
        # el error de activar un único umbral global -- ya confirmado
        # NEGATIVO para FAVORITO_CONFIRMADO (P18, 28-Jul) y sin evidencia
        # para el resto de tuplas live -- con una sola tupla verificada.
        # Verificado con datos reales (smart_exit_prices.csv, fórmula EXACTA
        # de abajo, no aproximada): n=43, split-half consistente en las 4
        # franjas -0.4/-0.5/-0.6/-0.7, óptimo -0.50€ (+1,10€ retrospectivo
        # vs -5,87€ real). BTC#5min/ETH#5min/ETH#15min de la misma familia
        # NO incluidas -- n=11/3/5, insuficiente para concluir nada (una de
        # ellas incluso empeora con umbrales más anchos en la simulación).
        por_tupla_raw = sl_cfg.get("por_tupla", {}) or {}
        # 09-Sep (/code-review, hallazgo real): NO usar bool(por_tupla) para
        # decidir si el modo opt-in está activo -- si UNA entrada de
        # por_tupla_raw viene mal formada (typo futuro, valor no numérico),
        # el bucle de abajo la descarta y por_tupla podía quedar VACÍO pese
        # a que por_tupla_raw no lo estaba, cayendo por completo al modo
        # global (umbral=0.30€ aplicado a TODAS las posiciones OPEN de
        # TODAS las estrategias live -- justo el fallo fail-open que este
        # mecanismo existe para evitar). opt_in_activo se fija ANTES del
        # parseo, sobre el raw: si hay CUALQUIER entrada en config, el modo
        # opt-in queda activo pase lo que pase, y una entrada malformada
        # simplemente no protege a NADIE (fail-closed hacia "no actúa"),
        # nunca degrada a "protege/toca a todo el mundo".
        opt_in_activo = bool(por_tupla_raw)
        por_tupla = {}
        for clave, spec in por_tupla_raw.items():
            try:
                por_tupla[clave] = float(spec.get("umbral_perdida_eur"))
            except (TypeError, ValueError, AttributeError):
                print(f"  ⚠️  Smart Exit por_tupla: entrada '{clave}' mal formada, "
                      f"IGNORADA (esa tupla no queda protegida, el resto del "
                      f"opt-in sigue activo, nunca cae a modo global)")
                continue
...
def _check_salidas_tempranas_bajo_lock(LIVE_CSV: Path, ts: str, umbral: float, haircut: float,
    ...
    por_tupla (09-Sep): dict {"STRATEGY#SUBTYPE": umbral_eur} opt-in -- si
    NO está vacío, SOLO las tuplas ahí listadas se evalúan para salida
    temprana (con SU umbral propio, ignorando `umbral`/tp_activo globales
    por completo para ellas); cualquier otra posición OPEN se salta sin
    tocar. Si está vacío, se mantiene el comportamiento global de siempre
    (umbral/tp_activo aplican a TODAS las posiciones OPEN)."""
    import live_trade
    ...
    for t in abiertas:
        ...
        # 09-Sep: modo opt-in por tupla -- ver docstring de arriba. Se
        # decide por opt_in_activo (fijado sobre el config RAW, antes de
        # parsear -- ver _check_salidas_tempranas), NUNCA por bool(por_tupla)
        # -- una entrada malformada no debe poder degradar esto a modo
        # global (/code-review 09-Sep, hallazgo real). Con opt-in activo,
        # cualquier tupla que no esté en por_tupla (incluida una que
        # existía en el config pero no parseó) se salta sin protección,
        ...
        # simular_combinado(). tp_activo=False hoy (y el opt-in por_tupla de
        # 09-Sep fuerza tp_activo_efectivo=False para toda tupla listada) ->
        # esta rama nunca entra en producción hoy, comportamiento idéntico
