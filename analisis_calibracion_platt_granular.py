...
def main() -> int:
    ...
    for clave, triples in sorted(candidatas.items()):
        intentadas += 1
        # 07-Sep: _fit_calibracion_prob ahora devuelve (calib, cache_folds)
        # -- ver shadow_postmortem.py. Este script corre fuera del hot path
        # (cron aparte, sin límite de granularidad, ver comentario en
        # shadow_postmortem.py::calcular_params), no necesita el caché
        # incremental -- se descarta explícitamente, nunca se persiste.
        calib, _cache_folds_no_usado = sp._fit_calibracion_prob(triples)
