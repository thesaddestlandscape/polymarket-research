    ...
    async with websockets.connect(WS_URL, open_timeout=10, close_timeout=5) as ws:
        ...
        try:
            while True:
                ...
                #
                # 27-Ago noche, rediseñado 08-Sep (ver sports_live_guard.py
                # para el hallazgo completo): DOS guardianes independientes,
                # mismo patrón que WALLET_MIRROR cripto
                # (wallet_mirror_executor_dryrun.py) -- (1) whitelist
                # estática pares_permitidos_live (editada a mano por Javi,
                # SOLO categoria#tipo, sin precio -- decisión de negocio),
                # (2) gate VIVO (sports_wallet_mirror_gate_bucket.evaluar(),
                # regenerado a diario) que decide en exclusiva el
                # micro-bucket de precio y puede vetar automáticamente uno
                # ya aprobado si empieza a sangrar dinero real (kill
                # switch), sin que nadie tenga que tocar la whitelist a
                # mano. Antes (1) también codificaba una banda de precio
                # fija que quedaba obsoleta sin que (2) pudiera corregirla.
                ...
                if isinstance(mejor_ask, (int, float)):
                    ok_operar, motivo_guard = _guard.puede_operar_live(categoria, info["tipo"])
