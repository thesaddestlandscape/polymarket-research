...
def _ya_promocionada(tupla_str: str, config: dict) -> bool:
    """True si YA existe una entrada EXACTA para esta categoria#tipo en
    pares_permitidos_live -- 08-Sep: el formato pasó de
    "categoria#tipo#lo:hi" (banda de precio fija) a "categoria#tipo" a
    secas (el precio lo decide el gate dinámico, no esta whitelist -- ver
    sports_live_guard.py). Sin shim del formato viejo (config ya migrado
    por completo, 08-Sep) -- ver por qué en sports_live_guard.py::
    tupla_permitida() (/code-review: un prefijo habría sido fail-open)."""
    return tupla_str in config.get("pares_permitidos_live", [])
