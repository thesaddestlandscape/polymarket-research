#!/bin/bash
# backup_sports_wallet_mirror_sniper_dry_run.sh -- 25-Sep: copia de backup_wallet_mirror_sniper_dry_run.sh para data/sports/ (56MB, ~3,3MB/día, cruzó warning 50MB).
# data/sports/wallet_mirror_sniper_dry_run.csv fuera de git.
#
...
SRC=/root/polymarket-research/data/sports/wallet_mirror_sniper_dry_run.csv
DEST_DIR=/mnt/HC_Volume_106538179/backups_sports_wallet_mirror_sniper_dry_run
DIAS_MANTENER=14
...
mkdir -p "$DEST_DIR"
FECHA=$(date -u +%Y-%m-%d)
DEST="$DEST_DIR/sports_wallet_mirror_sniper_dry_run_${FECHA}.csv.gz"
...
for f in "$DEST_DIR"/sports_wallet_mirror_sniper_dry_run_*.csv.gz; do
  [ -f "$f" ] || continue
...
echo "$(date -u) fin. Backups actuales: $(ls "$DEST_DIR"/sports_wallet_mirror_sniper_dry_run_*.csv.gz 2>/dev/null | wc -l)"
