...
GATE_GRID_JSON = DIR_SPORTS / "wallet_mirror_gate_bucket.json"  # 09-Sep
GATE_FINO_JSON = DIR_SPORTS / "wallet_mirror_gate_bucket_fino.json"  # 09-Sep
CONCENTRACION_JSON = DIR_SPORTS / "wallet_mirror_concentracion.json"  # 09-Sep
COMBOS_JSON = REPO / "data" / "shadow" / "sports_wallet_combos_09sep.json"  # 09-Sep
...
def compute_gate_bucket():
    """09-Sep: tabla de micro-buckets de precio (grid fijo + ventana fina),
    mismo patrón que el gate de cripto -- responde a "en qué precio exacto
    da dinero cada categoria#tipo", no solo el agregado. Grid y fino se
    fusionan en una sola lista (columna 'origen') porque el fino solo
    promociona buckets que el grid tiene sin_concluir (evaluar_sin_
    override en sports_wallet_mirror_gate_bucket.py) -- no son
    contradictorios, son un mismo mapa de precio con dos resoluciones."""
    filas = []
    if GATE_GRID_JSON.exists():
        grid = json.loads(GATE_GRID_JSON.read_text(encoding="utf-8"))
        for tupla, buckets in grid.items():
            for b, det in buckets.items():
                if not isinstance(det, dict) or det.get("veredicto") in ("sin_concluir", "n_insuficiente"):
                    continue
                filas.append({
                    "tupla": tupla, "bucket": f"[{float(b):.2f},{float(b)+0.05:.2f})",
                    "origen": "grid", "n": det.get("n"), "hit_pct": round((det.get("hit") or 0) * 100, 1),
                    "pnl_medio": det.get("pnl_medio"), "wilson90lo": det.get("wilson90lo"),
                    "p_bh": det.get("p_bh"), "veredicto": det.get("veredicto"),
                })
    if GATE_FINO_JSON.exists():
        fino = json.loads(GATE_FINO_JSON.read_text(encoding="utf-8"))
        for tupla, det in fino.items():
            if det.get("veredicto") in ("sin_concluir", "n_insuficiente") or det.get("lo") is None:
                continue
            filas.append({
                "tupla": tupla, "bucket": f"[{det['lo']:.2f},{det['hi']:.2f})",
                "origen": "fino", "n": det.get("n"), "hit_pct": None,
                "pnl_medio": det.get("pnl_medio"), "wilson90lo": None,
                "p_bh": det.get("p_valor"), "veredicto": det.get("veredicto"),
            })
    filas.sort(key=lambda r: -(r.get("pnl_medio") or 0))
    return filas


def compute_concentracion_detalle():
    """09-Sep: concentración de wallets por bucket YA confirmado -- alerta
    si una sola wallet domina un bucket que hoy opera con dinero real
    (mismo umbral de alarma 30% usado en el resto del proyecto). Solo hay
    snapshot actual (sin histórico diario todavía -- pendiente si hace
    falta más adelante)."""
    if not CONCENTRACION_JSON.exists():
        return {"generado_utc": None, "buckets": []}
    d = json.loads(CONCENTRACION_JSON.read_text(encoding="utf-8"))
    filas = []
    for bucket, det in d.get("buckets", {}).items():
        top1 = (det.get("top5_wallets") or [{}])[0]
        filas.append({
            "bucket": bucket, "n_senales": det.get("n_senales"),
            "n_wallets": det.get("n_wallets_distintas"), "n_mercados": det.get("n_mercados_distintos"),
            "top1_pct": top1.get("pct", 0), "alarma": (top1.get("pct", 0) or 0) >= 30,
        })
    filas.sort(key=lambda r: -(r.get("top1_pct") or 0))
    return {"generado_utc": d.get("generado_utc"), "buckets": filas}


def compute_combos():
    """09-Sep: placeholder de la investigación de wallets que arman combos
    multi-mercado en el mismo evento (petición Javi). Sin resultados
    todavía -- el análisis (analisis_sports_wallet_combos_09sep.py) sigue
    resolviendo outcomes reales contra gamma-api. NO inventar cifras aquí:
    mientras no exista el JSON, mostrar 'en análisis' sin más."""
    if not COMBOS_JSON.exists():
        return {"listo": False}
    try:
        d = json.loads(COMBOS_JSON.read_text(encoding="utf-8"))
        d["listo"] = True
        return d
    except Exception:
        return {"listo": False}
...
def compute_data():
    ...
    # p_shuffle ya viene en cada wallet del JSON de origen -- solo faltaba
    # exponerlo en la tabla (estadística de confianza, 09-Sep).
    ...
    return {
        ...
        "live": compute_live(),
        "gate_bucket": compute_gate_bucket(),
        "concentracion": compute_concentracion_detalle(),
        "combos": compute_combos(),
        # 09-Sep: sports comparte wallet on-chain con cripto (ver
        # config_live_sports.json::depositos, capital ajeno descontado en
        # live_stake.py de cripto) pero NO tiene todavía un snapshot propio
        # de balance on-chain (equivalente a data/live/balance_history.csv
        # de cripto) -- el PnL de arriba es "de plan" (stake registrado),
        # no reconciliado contra el saldo real de la wallet. Pendiente,
        # no inventar una cifra de reconciliación que no existe.
        "reconciliacion_on_chain": {"disponible": False},
    }
...
<table><thead><tr><th>Wallet</th><th>Categoría</th><th>n</th><th>hit%</th><th>edge_pp</th><th>p_shuffle</th></tr></thead>
<tbody id="tbody-top"></tbody></table>
<h2>Peores wallets (candidatas a fade)</h2>
<table><thead><tr><th>Wallet</th><th>Categoría</th><th>n</th><th>hit%</th><th>edge_pp</th><th>p_shuffle</th></tr></thead>
<tbody id="tbody-worst"></tbody></table>
<h2>🎯 Gate por micro-bucket de precio (grid fijo + ventana fina)</h2>
<div class="aviso" style="background:#0d2818;border-color:#26a69a;color:#8fe3c8">Solo se conecta a dinero real la tupla completa
(categoria#tipo) en <code>pares_permitidos_live</code> -- el bucket exacto de precio lo decide siempre el gate vivo, no este texto.</div>
<table><thead><tr><th>Tupla</th><th>Bucket</th><th>Origen</th><th>n</th><th>hit%</th><th>pnl/tr</th><th>Wilson90lo</th><th>p_bh/p_valor</th><th>Veredicto</th></tr></thead>
<tbody id="tbody-gate"></tbody></table>
<h2>⚠️ Concentración de wallets por bucket confirmado</h2>
<div id="tbody-concentracion-aviso"></div>
<table><thead><tr><th>Bucket</th><th>Señales</th><th>Wallets distintas</th><th>Mercados distintos</th><th>Top1 wallet %</th></tr></thead>
<tbody id="tbody-concentracion"></tbody></table>
<h2>🔬 Investigación en curso — wallets combo (multi-mercado, mismo evento)</h2>
<div id="tbody-combos"></div>
...
async function refresh(){
  ...
  document.getElementById('tbody-top').innerHTML = d.top_wallets.map(w =>
    `<tr><td>${w.wallet.slice(0,14)}${w.especialista?'<span class="badge">ESP</span>':''}</td>
     ...
     <td class="pos">+${w.edge_pp.toFixed(2)}</td><td>${w.p_shuffle!=null?w.p_shuffle.toFixed(4):'—'}</td></tr>`).join('');
  document.getElementById('tbody-worst').innerHTML = d.peores_wallets.map(w =>
    `<tr><td>${w.wallet.slice(0,14)}${w.especialista?'<span class="badge">ESP</span>':''}</td>
     <td>${w.categoria}</td><td>${w.n}</td><td>${(w.hit*100).toFixed(1)}%</td>
     <td class="neg">${w.edge_pp.toFixed(2)}</td><td>${w.p_shuffle!=null?w.p_shuffle.toFixed(4):'—'}</td></tr>`).join('');
  const gate = d.gate_bucket || [];
  document.getElementById('tbody-gate').innerHTML = gate.length ? gate.map(g => {
    const vClass = g.veredicto === 'bueno_confirmado' ? 'pos' : (g.veredicto === 'malo_confirmado' ? 'neg' : '');
    const pnlClass = (g.pnl_medio||0) >= 0 ? 'pos' : 'neg';
    return `<tr><td>${g.tupla}</td><td>${g.bucket}</td><td>${g.origen}</td><td>${g.n}</td>
      <td>${g.hit_pct!=null?g.hit_pct+'%':'—'}</td>
      <td class="${pnlClass}">${(g.pnl_medio||0)>=0?'+':''}${(g.pnl_medio||0).toFixed(3)}€</td>
      <td>${g.wilson90lo!=null?(g.wilson90lo*100).toFixed(1)+'%':'—'}</td>
      <td>${g.p_bh!=null?g.p_bh.toFixed(4):'—'}</td>
      <td class="${vClass}">${g.veredicto}</td></tr>`;
  }).join('') : '<tr><td colspan="9" style="text-align:center;color:#8b949e">Sin buckets confirmados todavía</td></tr>';
  const concGate = d.concentracion || {buckets:[]};
  document.getElementById('tbody-concentracion-aviso').innerHTML = concGate.buckets.some(b=>b.alarma)
    ? `<div class="aviso">🚨 ${concGate.buckets.filter(b=>b.alarma).length} bucket(s) con &ge;30% de las señales concentradas en UNA wallet -- el edge podría depender de esa wallet, no de un patrón de mercado.</div>`
    : '';
  document.getElementById('tbody-concentracion').innerHTML = (concGate.buckets||[]).map(b =>
    `<tr><td>${b.bucket}</td><td>${b.n_senales}</td><td>${b.n_wallets}</td><td>${b.n_mercados}</td>
     <td class="${b.alarma?'neg':''}">${b.top1_pct}%${b.alarma?' ⚠️':''}</td></tr>`).join('')
    || '<tr><td colspan="5" style="text-align:center;color:#8b949e">Sin datos</td></tr>';
  const combos = d.combos || {listo:false};
  document.getElementById('tbody-combos').innerHTML = combos.listo
    ? `<div class="panel">Resultado disponible -- n=${combos.n||'?'}, ver detalle en <code>data/shadow/sports_wallet_combos_09sep.json</code></div>`
    : '<div class="aviso" style="background:#0d1b2a;border-color:#1f6feb;color:#79c0ff">🔬 En análisis -- wallets que compran en ≥2 mercados del mismo evento (combo). Sin resultados todavía, el script sigue resolviendo outcomes reales.</div>';
}
