...
from shadow_digest import enviar_telegram_critico  # noqa: E402
...
def main() -> int:
    ...
    for a in anomalias:
        ...
        if clave not in notificadas:
            nuevas.append((clave, a))

    if nuevas:
        cuerpo = "\n".join(f"• *{a['tipo']}* {a['market_id']}: {a['detalle']}" for _, a in nuevas)
        enviado = enviar_telegram_critico(
            "🔎 *RECONCILIACIÓN posiciones* (nuevo)\n" + cuerpo +
            "\n\nRevisar trades.csv vs data-api/positions manualmente antes de actuar."
        )
        # Solo se marcan como notificadas las que SÍ llegaron a avisarse.
        # Antes se marcaban ANTES de intentar el envío — si Telegram
        # fallaba (incluso hoy, con el reintento de
        # enviar_telegram_critico), la anomalía quedaba silenciada para
        # siempre: el próximo run horario ya no la volvía a intentar
        # porque "clave in notificadas" era True aunque nadie se hubiera
        # enterado nunca (bug encontrado en auditoría Telegram 14-Jul).
        if enviado:
            for clave, a in nuevas:
