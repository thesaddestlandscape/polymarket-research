...
def _registrar_prediccion(mercado: dict, py: float, edge: float, restante_s: float,
    ...
    # 09-Sep, petición explícita Javi: hueco real, BALLENAS_TARDIAS corría
    # fuera del bucle de shadow_predict.py y nunca logueaba bot_consenso
    # (0/4.793 señales recientes tenían el dato, verificado antes de
    # tocar código). Puro logging -- no toca py/edge/decisión.
    bots_c = _bots_consenso(mercado, ASSET)
    features = json.dumps({
