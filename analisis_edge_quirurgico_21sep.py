    ...
    for ancho in ANCHOS:
        ...
        for j in np.argsort(-diff)[:TOP_K]:
            ...
            out.append({"ancho": ancho, "lo": lo, "hi": hi, "n": int(b - a), "pnl_medio": round(media, 4),
                        ...
                        "wallets": len(cnt), "top1_wallet": round(conc, 3),
                        "pnl_sin_top_wallet": round(media_sin_top, 4)})
...
def main():
    ...
    for (arq, activo, marco), filas in grupos_bot.items():
        for w in _evaluar_tupla(filas, f"{arq}#{activo}#{marco}"):
            resultados.append({"familia": arq, "activo": activo, "marco": marco, "grande": None, **w})
    ...
    for (tipo, activo, marco, grande), filas in grupos_wm.items():
        for w in _evaluar_tupla(filas, f"WM#{tipo}#{activo}#{marco}#{grande}"):
            resultados.append({"familia": "WALLET_MIRROR", "activo": activo, "marco": marco, "grande": grande, **w})
    for r in resultados:
    ...
    print(f"tuplas con >=1 ventana calificada: {len({(r['familia'],r['activo'],r['marco'],r['grande']) for r in resultados})} | ventanas: {len(resultados)} | pasan BH-FDR: {sum(r['bh_ok'] for r in resultados)}")
    print(f"{'BH':2} {'estado':10} {'tupla':34} {'anch':5} {'ventana':12} {'n':>4} {'pnl':>6} {'dias':>4} {'sin2':>6} {'wal':>3} {'top1':>5} {'p':>6}")
    for r in resultados:
        tup = f"{r['familia']}#{r['activo']}#{r['marco']}" + (f"#g{r['grande']}" if r['grande'] is not None else "")
        print(f"{'OK' if r['bh_ok'] else '--':2} {r['estado']:10} {tup:34} {r['ancho']:<5} [{r['lo']:.2f},{r['hi']:.2f}) {r['n']:>4} {r['pnl_medio']:+.3f} {r['n_dias']:>4} {r['pnl_sin_2_mejores_dias']:+.3f} {r['wallets']:>3} {r['top1_wallet']:>5.2f} {r['p_max']:>6.3f}")
...
def forward():
    ...
    for (arq, activo, marco), filas in B.cargar_filas().items():
        fuentes.append((arq, activo, marco, None, filas))
    for (tipo, activo, marco, grande), filas in W.cargar_filas().items():
        fuentes.append(("WALLET_MIRROR", activo, marco, grande, filas))
    for fam, activo, marco, grande, filas in fuentes:
        ...
        for w in _evaluar_tupla(train, f"{fam}#{activo}#{marco}#{grande}#fwd"):
    ...
    print(f"{'estado':10} {'tupla':34} {'anch':5} {'ventana':12} {'n_tr':>4} {'pnl_tr':>7} {'n_te':>4} {'pnl_te':>7} {'hit_te':>6}")
    for r in sorted(filas_out, key=lambda r: (-(r["pnl_test"] if r["pnl_test"] is not None else -9))):
        ...
        print(f"{r['estado']:10} {tup:34} {r['ancho']:<5} [{r['lo']:.2f},{r['hi']:.2f}) {r['n_train']:>4} {r['pnl_train']:+.3f} {r['n_test']:>4} {pt:>7} {r['hit_test'] if r['hit_test'] is not None else '-':>6}")
