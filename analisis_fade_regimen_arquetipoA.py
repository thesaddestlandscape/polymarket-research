...
def binom_sf(k: int, n: int, p: float) -> float:
    ...
    07-Sep (barrido de salud): la versión anterior sumaba comb(n,i) *
    p**i * (1-p)**(n-i) término a término -- comb(n,i) es un int de
    Python SIN límite de tamaño, y para n grande (esta familia ya
    acumula miles de observaciones, results.csv sigue creciendo) supera
    el máximo representable en float (~1.8e308) mucho antes de que la
    multiplicación por p**i lo reduzca a un valor razonable, lanzando
    OverflowError -- crasheaba el vigía en cuanto cualquier bucket
    cruzaba ese umbral de n. scipy.stats.binom.sf calcula la cola en
    espacio logarítmico internamente (mismo resultado matemático, sin
    pasar por un entero intermedio gigante) -- sin límite práctico de n."""
