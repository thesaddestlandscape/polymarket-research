...
def cargar_csv(path, excluir_columnas=()):
    """08-Sep (incidente real: 27 OOM-kill del kernel en 24h, procesos
    python de 2,6-6,6GB -- Javi via Telegram, "pensaba que esto estaba
    solucionado ya"): esta función carga TODO el CSV como lista de dicts
    en memoria, sin excepción para results.csv (368MB, ~445k filas y
    creciendo ~13k filas/día). El culpable real no son las columnas
    numéricas -- es `features` (JSON por fila, 30-50 claves, 500-2000
    bytes de texto), que NINGÚN consumidor de generate_report.py necesita
    (stats_por_subtype solo lee pnl_neto/acierto/stake_real/strategy/
    subtype; hoja_shadow_ops solo lee columnas de resolución/predicción/
    mercado/precio/outcome/pnl). `excluir_columnas` permite soltar ese
    campo ANTES de materializar el dict completo en la lista -- reduce el
    footprint por fila sin cambiar ningún dato que sí se usa. Mismo
    principio que los fixes de shadow_postmortem.py esta sesión (no
    cargar/guardar lo que nadie necesita del histórico completo)."""
    if not path.exists():
        return []
...
def cargar_shadow_abiertas(resultados=None):
    """08-Sep (incidente OOM real -- ver docstring de cargar_csv): antes
    releía results.csv (368MB) ENTERO por segunda vez solo para construir
    `ya_resueltas`, duplicando el pico de memoria del ya caro
    `shadow_res = cargar_csv(SHADOW_RESULTS)` de main(). Ahora acepta ese
    mismo `resultados` ya cargado (con o sin `features`, da igual, aquí
    solo se leen 3 columnas) -- si no se pasa (uso suelto/tests), cae al
    comportamiento anterior."""
    if resultados is None:
        resultados = cargar_csv(SHADOW_RESULTS, excluir_columnas=("features",))
    ya_resueltas = set()
    for r in resultados:
