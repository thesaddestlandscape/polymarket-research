...
def main() -> int:
    ...
    # 07-Sep (barrido de salud, detectado por medir_vigias_rotos()): medido
    # en vivo en 179s ese día -- el timeout=600 aplicado entonces resultó
    # INSUFICIENTE, volvió a colgarse (root cause real no era solo el
    # timeout: shuffle_test() en analisis_bot_wallets_gate_bucket_25ago.py
    # construía una matriz (2000 iters, na+nb) y hacía argsort completo por
    # grupo -- con DISPERSO#BTC#5min ya en n=45.927 filas eso son ~92M
    # elementos por bucket, mismo patrón de crecimiento que ya rompió
    # calcular_params() en shadow_postmortem.py). Arreglado en el propio
    # analisis_*.py (MAX_N_SHUFFLE=3000, subsample determinista antes de
    # permutar, diff_real se sigue calculando sobre la población completa).
    # Medido tras el fix: 5m57s reales bajo carga alta del VPS (load 18+),
    # solo ~2min de CPU real -- timeout a 900s por el mismo criterio de
    # margen ~2x que el resto de vigías arreglados hoy.
...
if __name__ == "__main__":
    try:
        ...
        print(f"[vigia_bot_wallets_gate_bucket] ERROR {type(e).__name__}: {e}")
        try:
            from shadow_digest import enviar_telegram
            enviar_telegram(f"🔴 vigia_bot_wallets_gate_bucket.py falló: {type(e).__name__}: {e}")
        except Exception:
            pass
        raise
