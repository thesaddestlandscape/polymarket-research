...
def leer_activity_incremental(checkpoint_path: Path, dias: int = 2):
    """07-Sep (barrido de salud, petición explícita Javi tras encontrar el
    causante real de la sobrecarga crónica de CPU -- load 12-16 en 4
    núcleos, sostenido): los 4 consumidores de `_archivos_activity()`
    (candidata9/10_*_reactivo_fase0.py, bot_wallets_gate_bucket_fase0.py,
    dispersed_bot_executor_dryrun.py) releían los ficheros COMPLETOS de
    `polymarket_activity_YYYY-MM-DD.csv` (hoy+ayer, ~1,5GB/4M filas
    combinados hoy) con csv.DictReader desde cero en CADA ciclo (poll
    5-15s), solo para descartar en memoria las filas ya vistas -- mismo
    patrón exacto que ya rompió el pipeline de resolve/postmortem esta
    misma sesión (ver commits de shadow_resolve.py/shadow_postmortem.py),
    generalizado aquí porque son 4 procesos haciéndolo a la vez sobre el
    mismo fichero.

    Checkpoint por offset de bytes, MISMO diseño que shadow_resolve.py::
    cargar_ya_resueltas() -- estos ficheros son append-only (los escribe
    en exclusiva fetch_polymarket_activity_ws.py, un único proceso, sin
    reescritura). Devuelve (generador) solo las filas NUEVAS desde el
    último checkpoint de ESTE consumidor concreto (cada llamador pasa su
    propio `checkpoint_path`, nunca compartido entre consumidores -- cada
    uno avanza a su propio ritmo).

    Generador que lee en bloques de 8MB, NO un `list[dict]` materializado
    de golpe (fix real, /code-review 07-Sep tras desplegar esto para
    bot_wallets_gate_bucket_fase0.py/dispersed_bot_executor_dryrun.py sin
    checkpoint previo -- la primera lectura era el fichero ENTERO de hoy,
    ~1,5GB, y `f.read(...)` + `.decode()` + `list(csv.reader(...))` de una
    sola vez materializaba bytes+texto+filas simultáneamente en RAM -> 2
    OOM-kills reales en producción, ver dmesg 21:41/21:43 UTC). Con
    generador + bloques, la memoria pico es ~8MB sin importar cuánto
    backlog haya que absorber la primera vez.

    Solo confirma (avanza el checkpoint) hasta la última línea COMPLETA
    (terminada en '\\n') de cada bloque -- si el fichero (escrito en
    streaming por fetch_polymarket_activity_ws.py) está a mitad de
    escribir su última línea, esa línea se deja sin consumir y se relee
    completa en la siguiente llamada, igual que un `tail -f` real (fix
    real, /code-review 07-Sep: la versión anterior avanzaba siempre hasta
    el tamaño leído, perdiendo para siempre cualquier fila truncada a
    mitad de escritura)."""
    try:
        checkpoints = json.loads(checkpoint_path.read_text(encoding="utf-8")) if checkpoint_path.exists() else {}
    except Exception:
        checkpoints = {}
    checkpoints_nuevos = dict(checkpoints)
    TAM_BLOQUE = 8 * 1024 * 1024
    for path in _archivos_activity(dias=dias):
        clave = str(path)
        try:
        ...
        usar_checkpoint = (
            ...
            and entry.get("header") == header_actual
        )
        offset_inicio = offset_previo if usar_checkpoint else 0
        offset_final = offset_inicio
        fieldnames = header_actual.split(",")
        if offset_inicio < size_actual:
            primero = offset_inicio == 0  # saltar la cabecera solo si empezamos desde el principio
            sobrante = b""
            with open(path, "rb") as f:
                ...
                while True:
                    ...
                    if not bloque:
                        break
                    datos = sobrante + bloque
                    ultimo_nl = datos.rfind(b"\n")
                    if ultimo_nl == -1:
                        sobrante = datos  # ni una línea completa todavía en este bloque
                        continue
                    completo, sobrante = datos[: ultimo_nl + 1], datos[ultimo_nl + 1 :]
                    texto = completo.decode("utf-8", errors="replace")
                    for fila in csv.reader(io.StringIO(texto)):
                        if primero:
                            primero = False
                            continue
                        if len(fila) != len(fieldnames):
                            continue  # fila malformada -- fail-safe, se descarta
    ...
    try:
        ...
        pass  # el caché es solo optimización -- el próximo ciclo reintenta
