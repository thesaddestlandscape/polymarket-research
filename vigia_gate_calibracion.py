...
def main() -> int:
    ...
    # 07-Sep (barrido de salud): timeout 120s->400s -- medido en vivo hoy en
    # 269s (results.csv sigue creciendo, 356MB+), llevaba VARIOS DÍAS
    # crasheando cada mañana según logs/vigia_gate_calibracion.log
    # ("ERROR TimeoutExpired" repetido) sin que nadie lo notara -- el
    # try/except de main() ya evitaba el traceback crudo, pero gate_
    # calibracion.json llevaba días sin regenerarse. Mismo patrón de fondo
    # que vigia_gate_bucket_propio.py (fix el mismo día).
...
if __name__ == "__main__":
    try:
        ...
        # 07-Sep: antes solo se logueaba -- una racha de varios días de
        # TimeoutExpired pasó desapercibida hasta un barrido manual. Avisar
        # por Telegram para que una racha de fallos no dependa de que
        # alguien lea el log a mano.
        try:
            from shadow_digest import enviar_telegram
            enviar_telegram(f"🔴 vigia_gate_calibracion.py falló: {type(e).__name__}: {e} "
                             f"-- gate_calibracion.json puede quedar desactualizado")
        except Exception:
            pass
        sys.exit(1)
