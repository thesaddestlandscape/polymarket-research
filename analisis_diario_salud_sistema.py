...
def medir_ciclos_pipeline(horas: float = 24) -> dict:
    ...
    08-Sep (barrido de salud, hallazgo real -- dos bugs que se enmascaraban
    entre sí): la versión anterior emparejaba cada línea "=== Shadow
    resolve ===" con la SIGUIENTE línea "=== Fin postmortem ===" de
    shadow_postmortem.py. Dos problemas:
    (1) desde que POSTMORTEM_MIN_INTERVAL_S=600 (04-Sep) hace que
    shadow_postmortem.py se salte la mayoría de ciclos de resolve, varios
    "Shadow resolve" consecutivos quedaban emparejados con el MISMO "Fin
    postmortem" futuro y distante -- inflando el conteo de "ciclos lentos"
    hasta 4x (verificado 08-Sep: 21 resolves vs solo 5 postmortems reales
    en una hora con la alerta disparada).
    (2) el propio "=== Fin postmortem ===" reutiliza el `ts` capturado al
    INICIO de shadow_postmortem.py::main() (nunca se refresca antes del
    print final) -- el timestamp que dice "Fin" es en realidad el de
    inicio, así que ni siquiera un emparejamiento 1:1 mediría el final
    real.
    Fix: usar las líneas `⏱ shadow_resolve.py: Xms` / `⏱ shadow_postmortem.py:
    Xms` / `⏭ shadow_postmortem.py: saltado` que escribe el WRAPPER
    (run_fast_mantenimiento.sh, vía time.time() real alrededor del
    subproceso) -- no dependen de ningún timestamp interno de los scripts,
    y cada resolve se empareja SOLO con el resultado (ejecutado o saltado)
    de su propia iteración, nunca con uno de una iteración futura.

    `horas`: ventana hacia atrás (default 24h, el snapshot diario). El
    vigía horario (vigia_pipeline_latencia.py) llama a esta misma función
    con horas=1 en vez de mantener su propia copia -- una sola
    implementación, sin que las dos puedan volver a divergir."""
    if not FAST_LOG.exists():
        return {"error": "fast.log no existe"}
    corte = datetime.now(timezone.utc) - timedelta(hours=horas)
    re_resolve_dur = re.compile(
        r"^\[([\d\-T:]+)Z\] \[mantenimiento\]\s+⏱ shadow_resolve\.py: (\d+)ms")
    re_postmortem_dur = re.compile(
        r"^\[([\d\-T:]+)Z\] \[mantenimiento\]\s+⏱ shadow_postmortem\.py: (\d+)ms")
    re_postmortem_skip = re.compile(
        r"^\[([\d\-T:]+)Z\] \[mantenimiento\]\s+⏭ shadow_postmortem\.py: saltado")
    re_live = re.compile(r"^\[([\d\-T:+]+)\] === live_trade ciclo")

    duraciones = []
    live_trade_ts = []
    pendiente_resolve_ms = None
    try:
        with open(FAST_LOG, encoding="utf-8", errors="replace") as f:
            for linea in f:
                m = re_resolve_dur.match(linea)
                if m:
                    try:
                        ...
                        pendiente_resolve_ms = None
                        continue
                    pendiente_resolve_ms = int(m.group(2)) if ts >= corte else None
                    continue
                m = re_postmortem_dur.match(linea)
                if m and pendiente_resolve_ms is not None:
                    duraciones.append((pendiente_resolve_ms + int(m.group(2))) / 1000.0)
                    pendiente_resolve_ms = None
                    continue
                m = re_postmortem_skip.match(linea)
                if m and pendiente_resolve_ms is not None:
                    # postmortem saltado a propósito (min interval) -- no es
                    # un ciclo resolve+postmortem lento, resolve solo cerró
                    # trades reales y siguió, no cuenta para este umbral.
                    pendiente_resolve_ms = None
                    continue
                m = re_live.match(linea)
                if m:
                    try:
    ...
    return {
        ...
        "resolve_postmortem_duraciones_s": [round(d, 1) for d in duraciones],
...
UMBRAL_VIGIA_STALE_H = 48  # log de vigía sin tocar en más de esto: puede
# llevar tiempo muerto sin que nadie note que dejó de correr

_PATRON_FALLO_VIGIA = re.compile(
    r"TimeoutExpired|Traceback \(most recent call last\)|murió:|ERROR \w+Error"
)
...
def medir_vigias_rotos() -> dict:
    ...
    vigia_gate_bucket_wallet_mirror.py) fallando por timeout durante horas
    o días sin que nadie lo notara -- uno de ellos bloqueaba el gate que
    el ejecutor REAL de WALLET_MIRROR (dinero real) necesita para operar,
    parado desde las 01:28 UTC sin que ninguna alerta lo señalara ("no
    podemos permitirlo, el sistema tiene que vigilar esto", textual).
    Root cause común los 3 casos: subprocess.run(timeout=N) con N fijado
    cuando el análisis era rápido, sin margen para el crecimiento de
    results.csv/CSVs derivados -- mismo patrón que ya causó el incidente
    de shadow_postmortem.py del 04-Ago que motivó este script.

    Barato y sin red: escanea la COLA (últimas ~40 líneas) de cada
    logs/vigia_*.log tocado en las últimas UMBRAL_VIGIA_STALE_H horas,
    busca el patrón de fallo (TimeoutExpired/Traceback/"murió:"/errores
    con nombre de excepción) en esas últimas líneas -- si el fallo está
    ahí, la ÚLTIMA ejecución registrada de ese vigía fracasó. No es
    100% infalible (un vigía que loguea mucho tras el fallo podría
    empujarlo fuera de la cola), pero mismo criterio de "barato, no
    perfecto" que el resto de este script."""
    logs_dir = REPO / "logs"
    ahora = datetime.now(timezone.utc)
    rotos = []
    for path in sorted(logs_dir.glob("vigia_*.log")):
        try:
            mtime = datetime.fromtimestamp(path.stat().st_mtime, timezone.utc)
        except OSError:
            continue
        if (ahora - mtime).total_seconds() > UMBRAL_VIGIA_STALE_H * 3600:
            continue  # no tocado recientemente -- no hay ejecución reciente que auditar
        try:
            lineas = path.read_text(encoding="utf-8", errors="replace").splitlines()
        except Exception:
            continue
        cola = lineas[-40:]
        # 09-Sep (hallazgo real, barrido de salud): "cualquier fallo en las
        # últimas 40 líneas" daba falso positivo en vigías de cron diario
        # (ej. vigia_gate_calibracion.py, cron 06:59 UTC, log de 133 líneas
        # totales) -- un timeout viejo YA ARREGLADO (07-Sep, timeout 120s->
        # 400s) seguía dentro de la ventana de 40 líneas días después,
        # aunque la corrida MÁS RECIENTE hubiera terminado bien. Ahora solo
        # cuenta como "roto" si el ÚLTIMO fallo detectado no tiene ninguna
        # línea de contenido real detrás -- si el vigía siguió escribiendo
        # (señal de que terminó su ciclo, con o sin éxito) después del
        # último fallo, se considera recuperado.
        idx_fallos = [i for i, l in enumerate(cola) if _PATRON_FALLO_VIGIA.search(l)]
        if idx_fallos:
            ultimo_fallo_idx = idx_fallos[-1]
            hay_lineas_despues = any(l.strip() for l in cola[ultimo_fallo_idx + 1:])
            if not hay_lineas_despues:
                rotos.append({
                    "log": path.name,
                    "mtime_utc": mtime.isoformat(timespec="seconds"),
                    "ultima_linea_fallo": cola[ultimo_fallo_idx][:200],
                })
    return {"n_vigias_rotos": len(rotos), "vigias": rotos}
...
def main() -> int:
    ...
    if vr.get("n_vigias_rotos"):
        for v in vr["vigias"]:
            anomalias.append(f"🚨 vigía roto: {v['log']} (última ejecución falló, mtime "
                              f"{v['mtime_utc']}) -- {v['ultima_linea_fallo']}")
