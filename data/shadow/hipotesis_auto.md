# Hipótesis automáticas — 2026-07-19 02:44 UTC
_Generado por shadow_postmortem.py sobre 21741 resoluciones (PNL=+5223.76€)_
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.269 (n=115)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.248 (n=137)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.212)

- **PATRÓN** `py_entrada` < `0.395` → IC=+0.263 (n=302)
...
- **PATRÓN** `py_entrada` < `0.66` → IC=+0.207 (n=73)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.66 (IC base=+0.187)

- **PATRÓN** `py_entrada` > `0.575` → IC=+0.184 (n=74)

  - _Acción_: Kelly boost +0.92€ cuando `py_entrada` > 0.575 (IC base=+0.187)
...
- **PATRÓN** `dist_vwap_pct` > `0.5559` → IC=+0.134 (n=203)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` > 0.5559 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.004` → IC=+0.226 (n=166)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.004 (IC base=+0.114)

- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.131 (n=1436)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0088 (IC base=+0.112)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.133 (n=1542)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` < 12.0 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `0.1646` → IC=+0.126 (n=359)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` > 0.1646 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` < `0.9057` → IC=+0.122 (n=1073)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.9057 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.188` → IC=+0.145 (n=226)
...
- **PATRÓN** `dist_vwap_pct` > `0.136` → IC=+0.136 (n=64)
  ...
  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 0.136 (IC base=+0.073)

- **PATRÓN** `dist_vwap_pct` < `0.5188` → IC=+0.130 (n=217)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.5188 (IC base=+0.073)
...
- **PATRÓN** `dist_vwap_pct` > `0.6784` → IC=+0.161 (n=57)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.6784 (IC base=+0.077)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.618` → IC=+0.230 (n=72)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.326` → IC=+0.216 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.326 (IC base=+0.103)

- **PATRÓN** `sigma_h` < `0.013` → IC=+0.138 (n=468)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.013 (IC base=+0.123)

- **PATRÓN** `sigma_h` > `0.009` → IC=+0.127 (n=532)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_h` < `0.0284` → IC=+0.153 (n=545)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` < 0.0284 (IC base=+0.152)

- **PATRÓN** `sigma_h` > `0.0155` → IC=+0.177 (n=363)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0155 (IC base=+0.152)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.161 (n=487)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 8.0 (IC base=+0.152)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.156 (n=486)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` < `0.0098` → IC=+0.193 (n=190)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0098 (IC base=+0.170)
...
- **PATRÓN** `dist_vwap_pct` > `0.3099` → IC=+0.182 (n=86)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.3099 (IC base=+0.170)

- **PATRÓN** `dist_vwap_pct` < `0.587` → IC=+0.164 (n=313)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.587 (IC base=+0.170)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.696` → IC=+0.169 (n=282)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` < 7.696 (IC base=+0.170)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0116` → IC=+0.241 (n=350)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.1699` → IC=+0.208 (n=388)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1699 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.061` → IC=+0.291 (n=156)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.061 (IC base=+0.165)

- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.187 (n=1031)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.195 (n=346)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` < 5.0 (IC base=+0.170)

- **PATRÓN** `dist_vwap_pct` < `0.8826` → IC=+0.187 (n=941)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.8826 (IC base=+0.170)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.089` → IC=+0.227 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.089 (IC base=+0.170)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.048` → IC=+0.181 (n=647)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.136 (n=262)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0061 (IC base=+0.134)

- **PATRÓN** `sigma_h` > `0.0026` → IC=+0.167 (n=262)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` > 0.0026 (IC base=+0.134)
...
- **PATRÓN** `dist_vwap_pct` > `0.1165` → IC=+0.138 (n=67)
  ...
  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 0.1165 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` < `0.7143` → IC=+0.131 (n=231)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.7143 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.135` → IC=+0.227 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.135 (IC base=+0.134)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.247 (n=89)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.157 (n=281)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` < 18.0 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` > `0.5316` → IC=+0.192 (n=63)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.5316 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` < `0.886` → IC=+0.146 (n=213)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.886 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.066` → IC=+0.267 (n=41)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.066 (IC base=+0.148)

- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.140 (n=237)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0067 (IC base=+0.128)

- **PATRÓN** `sigma_h` > `0.0035` → IC=+0.135 (n=269)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` > 0.0035 (IC base=+0.128)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.156 (n=94)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 18.0 (IC base=+0.128)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.170 (n=101)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 6.0 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` > `0.6273` → IC=+0.189 (n=59)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.6273 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` < `1.1418` → IC=+0.143 (n=253)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 1.1418 (IC base=+0.128)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.618` → IC=+0.243 (n=68)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.618 (IC base=+0.128)
...
- **PATRÓN** `sigma_h` > `0.0119` → IC=+0.191 (n=134)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0119 (IC base=+0.134)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.159 (n=209)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 12.0 (IC base=+0.134)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.148 (n=103)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 6.0 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` > `0.6161` → IC=+0.195 (n=57)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.6161 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.625` → IC=+0.283 (n=113)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.625 (IC base=+0.134)

- **PATRÓN** `sigma_h` < `0.0122` → IC=+0.152 (n=248)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` < 0.0122 (IC base=+0.117)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.171 (n=83)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 5.0 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` < `0.185` → IC=+0.180 (n=170)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.185 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.556` → IC=+0.181 (n=164)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` < 2.556 (IC base=+0.117)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` < `0.0171` → IC=+0.265 (n=240)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0171 (IC base=+0.257)

- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.273 (n=240)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.266 (n=246)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.257)

- **PATRÓN** `dist_vwap_pct` > `0.2844` → IC=+0.323 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2844 (IC base=+0.257)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.797` → IC=+0.392 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.797 (IC base=+0.257)

- **PATRÓN** `sigma_h` > `0.0079` → IC=+0.328 (n=253)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0079 (IC base=+0.299)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.308 (n=264)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.299)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.303 (n=226)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.299)

- **PATRÓN** `dist_vwap_pct` > `0.3099` → IC=+0.325 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3099 (IC base=+0.299)

- **PATRÓN** `dist_vwap_pct` < `0.5637` → IC=+0.311 (n=252)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5637 (IC base=+0.299)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.458` → IC=+0.318 (n=223)
...
- **PATRÓN** `sigma_h` < `0.0053` → IC=+0.129 (n=324)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.64€ cuando `sigma_h` < 0.0053 (IC base=+0.128)

- **PATRÓN** `sigma_h` > `0.0136` → IC=+0.191 (n=322)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0136 (IC base=+0.128)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.134 (n=887)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 8.0 (IC base=+0.128)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.133 (n=889)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` < 16.0 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` > `0.5454` → IC=+0.172 (n=190)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.5454 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` < `0.1183` → IC=+0.153 (n=488)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.1183 (IC base=+0.128)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.253` → IC=+0.282 (n=131)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.253 (IC base=+0.128)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.129 (n=397)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` < 6.0 (IC base=+0.104)

- **PATRÓN** `dist_vwap_pct` < `0.7882` → IC=+0.127 (n=933)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` < 0.7882 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.527` → IC=+0.123 (n=242)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` > 3.527 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.097` → IC=+0.121 (n=830)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` < 11.097 (IC base=+0.104)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.888` → IC=+0.157 (n=97)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.179 (n=82)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0039 (IC base=+0.080)

- **PATRÓN** `dist_vwap_pct` < `0.2777` → IC=+0.141 (n=126)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.2777 (IC base=+0.080)

- **PATRÓN** `sigma_ewma_delta_pct` > `14.068` → IC=+0.259 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 14.068 (IC base=+0.080)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.997` → IC=+0.179 (n=51)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 7.997 (IC base=+0.044)
...
- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.122 (n=88)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` < 0.0083 (IC base=+0.040)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.121 (n=93)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` < 6.0 (IC base=+0.040)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` < `0.0284` → IC=+0.211 (n=303)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0284 (IC base=+0.209)

- **PATRÓN** `sigma_h` > `0.0149` → IC=+0.245 (n=202)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0149 (IC base=+0.209)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.234 (n=111)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.209)

- **PATRÓN** `dist_vwap_pct` > `0.2413` → IC=+0.260 (n=94)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2413 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.899` → IC=+0.289 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.899 (IC base=+0.209)

- **PATRÓN** `sigma_h` < `0.0095` → IC=+0.223 (n=146)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0095 (IC base=+0.216)

- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.224 (n=331)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0084 (IC base=+0.216)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.215 (n=300)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.216)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.234 (n=122)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.5425` → IC=+0.214 (n=302)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5425 (IC base=+0.216)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.154` → IC=+0.226 (n=283)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.588` → IC=-0.155 (n=27)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.128 (n=275)
...
- **PATRÓN** `ibs_15` < `0.0263` → IC=+0.121 (n=175)
  ...
  - _Acción_: Kelly boost +0.61€ cuando `ibs_15` < 0.0263 (IC base=+0.059)

- **PATRÓN** `dist_vwap_pct` > `0.4531` → IC=+0.123 (n=51)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` > 0.4531 (IC base=+0.059)

- **PATRÓN** `sigma_ewma_delta_pct` < `17.588` → IC=+0.128 (n=275)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` < 17.588 (IC base=+0.059)
...
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.130 (n=98)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` < 0.0058 (IC base=+0.051)

- **PATRÓN** `dist_vwap_pct` > `0.974` → IC=+0.214 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.974 (IC base=+0.051)

- **PATRÓN** `sigma_ewma_delta_pct` < `8.162` → IC=+0.130 (n=98)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` < 8.162 (IC base=+0.051)
...
- **LIVE-CANDIDATA**: `GBM_LATE_5M#XRP#5min` — IC=+0.125 n=38. Faltan ~2 resoluciones para umbral n≥40. ETA: ~1h.
- **LIVE-CANDIDATA**: `GBM_LATE_5M#XRP` — IC=+0.125 n=38. Faltan ~2 resoluciones para umbral n≥40. ETA: ~1h.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 261 | +0.139 | +13.28€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 261 | +0.139 | +13.28€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 111 | +0.155 | +6.08€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 111 | +0.155 | +6.08€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 93 | +0.153 | +5.44€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 93 | +0.153 | +5.44€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 57 | +0.076 | +1.75€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 57 | +0.076 | +1.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 3637 | +0.184 | +40.54€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 2361 | +0.217 | +84.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 154 | +0.083 | +7.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 501 | +0.098 | -75.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 621 | +0.150 | +23.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 1055 | +0.192 | +29.28€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 787 | +0.208 | -0.51€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min |