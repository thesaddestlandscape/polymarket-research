# Hipótesis automáticas — 2026-08-26 08:44 UTC
_Generado por shadow_postmortem.py sobre 159946 resoluciones (PNL=+11409.38€)_
...
- **FILTRO** `restante_s_al_confirmar` < `148.66` → IC=-0.279 (n=2223)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=6669)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.164 (n=1401)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.237 (n=1800)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.233)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.234 (n=1285)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.224 (n=1640)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.219)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.248 (n=1090)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.190 (n=1376)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` < 12.0 (IC base=+0.185)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.212 (n=1364)
...
- **PATRÓN** `restante_min` > `4.92` → IC=+0.157 (n=1471)

  - _Acción_: Kelly boost +0.78€ cuando `restante_min` > 4.92 (IC base=+0.132)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=4264)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `4.5` → IC=+0.157 (n=1362)
...
- **PATRÓN** `restante_min` > `4.9` → IC=+0.166 (n=714)
...
- **PATRÓN** `lag_apertura_s` < `5.74` → IC=+0.169 (n=677)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.126 (n=2137)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.147 (n=866)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3416` → IC=+0.164 (n=582)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4087` → IC=+0.157 (n=441)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` < 0.4087 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` > `0.1712` → IC=+0.164 (n=296)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.1712 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` < `0.8313` → IC=+0.139 (n=748)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.8313 (IC base=+0.141)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.054` → IC=+0.161 (n=299)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 4.054 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` < `1.2237` → IC=+0.154 (n=651)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 1.2237 (IC base=+0.141)

- **PATRÓN** `volumen_pendiente_norm` > `0.0698` → IC=+0.161 (n=317)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5136` → IC=+0.147 (n=585)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.5136 (IC base=+0.141)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.147 (n=866)

  - _Acción_: Kelly boost +0.74€ cuando `libro_spread` < 0.01 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `8260.3607` → IC=+0.146 (n=661)
...
- **PATRÓN** `sigma_h` < `0.0044` → IC=+0.182 (n=240)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0044 (IC base=+0.129)

- **PATRÓN** `drift_60min` |x|≤ `0.3206` → IC=+0.143 (n=359)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.3206 (IC base=+0.129)
...
- **PATRÓN** `hora_utc` < `4.0` → IC=+0.236 (n=123)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 4.0 (IC base=+0.129)

- **PATRÓN** `ibs_20min` < `0.144` → IC=+0.169 (n=158)

  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` < 0.144 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` > `0.6146` → IC=+0.183 (n=58)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.6146 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.104` → IC=+0.137 (n=362)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` < 6.104 (IC base=+0.129)

- **PATRÓN** `volumen_regimen` < `1.1847` → IC=+0.137 (n=359)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` < 1.1847 (IC base=+0.129)

- **PATRÓN** `volumen_regimen` > `0.6229` → IC=+0.134 (n=359)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` > 0.6229 (IC base=+0.129)

- **PATRÓN** `volumen_pendiente_norm` > `0.0692` → IC=+0.178 (n=169)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.0692 (IC base=+0.129)

- **PATRÓN** `volumen_spike_ratio` < `2.5254` → IC=+0.147 (n=358)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 2.5254 (IC base=+0.129)

- **PATRÓN** `libro_liquidez` > `11819.5436` → IC=+0.135 (n=321)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 11819.5436 (IC base=+0.129)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.141 (n=2175)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=5151)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.284 (n=1771)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=5555)

- **FILTRO** `ibs_7min` < `0.7368` → IC=-0.231 (n=1830)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7368
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=5496)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.175 (n=2490)
...
- **FILTRO** `py_entrada` > `0.69` → IC=-0.231 (n=2092)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=6914)

- **FILTRO** `ibs_7min` > `0.7239` → IC=-0.169 (n=2251)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7239
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=6755)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.235 (n=368)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=1160)
...
- **FILTRO** `ibs_7min` > `0.837` → IC=-0.202 (n=381)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.156 (n=318)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=1153)

- **FILTRO** `py_entrada` < `0.38` → IC=-0.243 (n=356)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.208 (n=293)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.088 (n=734)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.336 (n=242)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=785)

- **FILTRO** `ibs_7min` < `0.2127` → IC=-0.279 (n=256)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.265 (n=339)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=1238)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.150 (n=389)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=865)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.269 (n=292)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=962)

- **FILTRO** `ibs_7min` < `0.765` → IC=-0.186 (n=313)
...
- **FILTRO** `ballena_activa_n` > `43.0` → IC=-0.211 (n=310)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=944)

- **FILTRO** `hora_utc` > `15.0` → IC=-0.126 (n=410)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=828)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.286 (n=297)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=941)

- **FILTRO** `ibs_7min` > `0.1821` → IC=-0.154 (n=420)
...
- **FILTRO** `ballena_activa_n` > `38.0` → IC=-0.208 (n=306)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=932)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.189 (n=393)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.007 (n=1255)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.279 (n=306)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=1206)

- **FILTRO** `ibs_7min` > `0.8387` → IC=-0.160 (n=377)
...
- **FILTRO** `sigma_h` > `0.0071` → IC=-0.142 (n=93)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0071
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=283)

- **FILTRO** `drift_15min` |x|> `0.5623` → IC=-0.163 (n=93)
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 15079 | +0.192 | -1308.13€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 15079 | +0.192 | -1308.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 2771 | +0.125 | -510.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 2771 | +0.125 | -510.01€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 2603 | +0.162 | -338.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 2603 | +0.162 | -338.16€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 2390 | +0.233 | -59.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 2390 | +0.233 | -59.19€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 2434 | +0.219 | -109.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 2434 | +0.219 | -109.24€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 2544 | +0.185 | -249.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 2544 | +0.185 | -249.24€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 5447 | +0.132 | +174.02€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 5447 | +0.132 | +174.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 2706 | +0.140 | +126.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 2706 | +0.140 | +126.51€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 2741 | +0.124 | +47.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 2741 | +0.124 | +47.52€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0 | 202 | +0.098 | -0.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#240min | 62 | +0.094 | -0.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#60min | 140 | +0.099 | +0.19€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#ETH | 165 | +0.105 | +1.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#ETH#240min | 25 | +0.130 | +1.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#ETH#60min | 140 | +0.099 | +0.19€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 5440 | +0.097 | -196.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 560 | +0.053 | -33.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 4880 | +0.102 | -162.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC | 3757 | +0.096 | -94.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#15min | 560 | +0.053 | -33.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#5min | 3197 | +0.103 | -60.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL | 1683 | +0.098 | -102.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL#5min | 1683 | +0.098 | -102.10€ | 0 | 0 |
...
| ✅ GBM_LATE_5M | 1422 | +0.127 | +636.71€ | 1 | 23 |
| ✅ GBM_LATE_5M#5min | 1422 | +0.127 | +636.71€ | 0 | 0 |
| ✅ GBM_LATE_5M#BNB | 62 | +0.203 | +39.47€ | 0 | 0 |
| ✅ GBM_LATE_5M#BNB#5min | 62 | +0.203 | +39.47€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC | 544 | +0.121 | +270.09€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC#5min | 544 | +0.121 | +270.09€ | 1 | 19 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 16332 | -0.073 | +256.46€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 16332 | -0.073 | +256.46€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 2525 | -0.093 | +258.25€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 2525 | -0.093 | +258.25€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 2974 | -0.058 | +7.43€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 2974 | -0.058 | +7.43€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 2604 | -0.086 | +15.81€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 2604 | -0.086 | +15.81€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 2492 | -0.095 | -169.44€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 2492 | -0.095 | -169.44€ | 8 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 3023 | -0.048 | +26.20€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 3023 | -0.048 | +26.20€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 2714 | -0.069 | +118.20€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 2714 | -0.069 | +118.20€ | 5 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.086) — sin ventaja clara. oversold(IBS<0.3): IC=+0.005 n=1965 | neutral: IC=+0.003 n=2117 | overbought(IBS>0.7): IC=+0.088 n=2210
  - _Datos_: n=6593 IC=+0.033 PNL=+542.68€

**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  - _Umbral_: n≥40 por celda + gate riguroso completo (Wilson+shuffle+PnL bootstrap)
  - _Acción_: Añadir claves 'ESTRATEGIA#SUBTYPE#DIRECCION#HORA':1.2 a meta.hora_boost_factor, solo por celda confirmada
  - _Estado_: (cache 107s) 17 celda(s) GATE OK de 1979 trackeadas
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.076 n=89766 | tras_1loss IC=+0.045 n=69876 | tras_2loss IC=+0.009 n=31724/40 | gap=+0.067 (umbral 0.05)
...
**⏳ H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  ...
  - _Estado_: 5566 ops, 22 horas distintas. Sin hora con n≥15 y IC extremo aún.
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=-0.013 n=581 | contrario_BTC IC=-0.008 n=437/40 | gap=+0.005 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=5387 IC=+0.003 PNL=+95.44€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=5387 IC=+0.003 PNL=+95.44€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=2021 IC=-0.015 PNL=-44.42€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=2021 IC=-0.015 PNL=-44.42€
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=5947 IC=+0.094 PNL=+1526.35€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=5947 IC=+0.094 PNL=+1526.35€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=3883 IC=-0.134 PNL=+273.59€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=3883 IC=-0.134 PNL=+273.59€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=2771 IC=+0.125 PNL=-510.01€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=2771 IC=+0.125 PNL=-510.01€
