# Hipótesis automáticas — 2026-07-12 21:45 UTC
_Generado por shadow_postmortem.py sobre 11465 resoluciones (PNL=+2209.88€)_
...
- **PATRÓN** `libro_liquidez` > `5937.8835` → IC=+0.165 (n=183)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.241 (n=83)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.223)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.232 (n=39)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 10.0 (IC base=+0.223)

- **PATRÓN** `py_entrada` < `0.235` → IC=+0.348 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.235 (IC base=+0.223)

- **PATRÓN** `libro_liquidez` > `6364.5293` → IC=+0.241 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 6364.5293 (IC base=+0.223)
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.262 (n=40)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.245` → IC=+0.281 (n=30)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.245 (IC base=+0.218)

- **PATRÓN** `libro_liquidez` > `1672.5274` → IC=+0.250 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1672.5274 (IC base=+0.218)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.122 (n=1389)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5798` → IC=+0.123 (n=51)
  ...
  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` > 0.5798 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` < `0.1373` → IC=+0.199 (n=71)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.1373 (IC base=+0.116)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.569` → IC=+0.364 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.569 (IC base=+0.116)
...
- **PATRÓN** `dist_vwap_pct` > `0.5914` → IC=+0.146 (n=46)
  ...
  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.5914 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` < `0.1918` → IC=+0.190 (n=98)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.1918 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.06` → IC=+0.167 (n=64)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` < 4.06 (IC base=+0.111)
...
- **PATRÓN** `sigma_h` < `0.0044` → IC=+0.164 (n=114)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0044 (IC base=+0.092)

- **PATRÓN** `dist_vwap_pct` > `0.5857` → IC=+0.167 (n=25)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.5857 (IC base=+0.092)
...
- **PATRÓN** `sigma_h` < `0.017` → IC=+0.179 (n=238)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.017 (IC base=+0.176)

- **PATRÓN** `sigma_h` > `0.0291` → IC=+0.211 (n=119)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0291 (IC base=+0.176)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.197 (n=259)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0075 (IC base=+0.174)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.180 (n=270)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.1403` → IC=+0.217 (n=58)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1403 (IC base=+0.174)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.569` → IC=+0.350 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.569 (IC base=+0.174)

- **PATRÓN** `sigma_h` > `0.0042` → IC=+0.177 (n=401)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` > 0.0042 (IC base=+0.153)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.154 (n=134)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.1749` → IC=+0.257 (n=101)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1749 (IC base=+0.153)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.048` → IC=+0.236 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.048 (IC base=+0.153)
...
- **PATRÓN** `dist_vwap_pct` < `0.1589` → IC=+0.300 (n=33)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1589 (IC base=+0.104)
...
- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.319 (n=70)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0088 (IC base=+0.274)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.300 (n=53)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 12.0 (IC base=+0.274)

- **PATRÓN** `dist_vwap_pct` > `0.177` → IC=+0.278 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.177 (IC base=+0.274)

- **PATRÓN** `dist_vwap_pct` < `0.1067` → IC=+0.292 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1067 (IC base=+0.274)
...
- **PATRÓN** `dist_vwap_pct` > `0.4968` → IC=+0.175 (n=38)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.4968 (IC base=+0.086)
...
- **PATRÓN** `sigma_h` > `0.012` → IC=+0.139 (n=153)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.012 (IC base=+0.079)

- **PATRÓN** `hora_utc` < `9.0` → IC=+0.120 (n=214)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` < 9.0 (IC base=+0.079)

- **PATRÓN** `dist_vwap_pct` < `0.186` → IC=+0.167 (n=94)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.186 (IC base=+0.079)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.141 (n=51)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0039 (IC base=+0.026)

- **PATRÓN** `dist_vwap_pct` > `0.7076` → IC=+0.206 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7076 (IC base=+0.026)
...
- **PATRÓN** `sigma_h` > `0.0302` → IC=+0.273 (n=42)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0302 (IC base=+0.202)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.250 (n=62)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 10.0 (IC base=+0.202)

- **PATRÓN** `dist_vwap_pct` > `0.2925` → IC=+0.154 (n=24)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.2925 (IC base=+0.202)

- **PATRÓN** `dist_vwap_pct` < `0.1648` → IC=+0.179 (n=26)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.1648 (IC base=+0.202)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 1045 | +0.191 | +36.11€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 674 | +0.219 | +26.39€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 307 | +0.196 | +6.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 225 | +0.218 | -0.01€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 371 | +0.170 | +4.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 223 | +0.207 | +4.84€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 365 | +0.206 | +24.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 224 | +0.230 | +21.52€ | 0 | 8 |
...
| ✅ GBM_LATE_15M | 3702 | +0.113 | +1234.11€ | 0 | 10 |
| ✅ GBM_LATE_15M#15min | 3702 | +0.113 | +1234.11€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 904 | +0.074 | +131.37€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 904 | +0.074 | +131.37€ | 0 | 2 |
| ✅ GBM_LATE_15M#ETH | 937 | +0.073 | +159.42€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 937 | +0.073 | +159.42€ | 0 | 4 |
| ✅ GBM_LATE_15M#SOL | 967 | +0.136 | +473.86€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 967 | +0.136 | +473.86€ | 0 | 7 |
| ✅ GBM_LATE_15M#XRP | 894 | +0.170 | +469.46€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 894 | +0.170 | +469.46€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1051 | +0.164 | +608.78€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1051 | +0.164 | +608.78€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 293 | +0.120 | +108.24€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 293 | +0.120 | +108.24€ | 0 | 6 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 203 | +0.266 | +196.35€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 203 | +0.266 | +196.35€ | 0 | 7 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 1194 | +0.083 | +301.52€ | 0 | 8 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1194 | +0.083 | +301.52€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 274 | +0.087 | +46.58€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 274 | +0.087 | +46.58€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 299 | +0.008 | +13.31€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 299 | +0.008 | +13.31€ | 1 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 311 | +0.034 | +35.19€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 311 | +0.034 | +35.19€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 310 | +0.199 | +206.43€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 310 | +0.199 | +206.43€ | 0 | 8 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.094 n=6516 | tras_1loss IC=+0.049 n=4811 | tras_2loss IC=+0.016 n=2140/40 | gap=+0.078 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=9007 IC=+0.100 PNL=+2285.76€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=9007 IC=+0.100 PNL=+2285.76€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema, sin filtrar todavía
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.162 < -0.1 con n=320 PNL=-42.96€
  - _Datos_: n=320 IC=-0.162 PNL=-42.96€
...
**⏳ H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: 28/40 ops en el filtro definido (IC actual=+0.233 PNL=+26.53€)
  - _Datos_: n=28 IC=+0.233 PNL=+26.53€
