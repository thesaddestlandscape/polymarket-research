# Hipótesis automáticas — 2026-07-23 05:32 UTC
_Generado por shadow_postmortem.py sobre 30275 resoluciones (PNL=+6802.82€)_
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.202 (n=1665)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.205 (n=2145)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.215 (n=713)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.180 (n=2334)

  - _Acción_: Kelly boost +0.90€ cuando `libro_spread` < 0.01 (IC base=+0.176)

- **PATRÓN** `libro_liquidez` > `6066.0843` → IC=+0.176 (n=950)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 6066.0843 (IC base=+0.176)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.248 (n=157)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.230 (n=198)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.196 (n=90)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` < 15.0 (IC base=+0.162)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.190 (n=111)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.01 (IC base=+0.162)

- **PATRÓN** `libro_liquidez` > `5754.5746` → IC=+0.167 (n=34)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 5754.5746 (IC base=+0.162)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.223 (n=449)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `1620.0535` → IC=+0.211 (n=441)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1620.0535 (IC base=+0.211)

### FAVORITO_CONFIRMADO#SOL#60min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.226 (n=111)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.605` → IC=+0.216 (n=72)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.605 (IC base=+0.199)
...
- **PATRÓN** `dist_vwap_pct` > `0.6538` → IC=+0.131 (n=220)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` > 0.6538 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.644` → IC=+0.221 (n=432)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.644 (IC base=+0.114)

- **PATRÓN** `sigma_h` > `0.0133` → IC=+0.132 (n=905)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `11.193` → IC=+0.200 (n=68)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.193 (IC base=+0.094)
...
- **PATRÓN** `dist_vwap_pct` > `0.6593` → IC=+0.146 (n=80)
  ...
  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.6593 (IC base=+0.065)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.179` → IC=+0.170 (n=110)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 7.179 (IC base=+0.065)
...
- **PATRÓN** `sigma_h` > `0.0149` → IC=+0.155 (n=436)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.1078` → IC=+0.144 (n=282)
...
- **PATRÓN** `sigma_h` < `0.0096` → IC=+0.161 (n=246)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0096 (IC base=+0.153)

- **PATRÓN** `sigma_h` > `0.0267` → IC=+0.173 (n=246)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0267 (IC base=+0.153)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.167 (n=265)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 18.0 (IC base=+0.153)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.206 (n=270)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` > `0.3701` → IC=+0.211 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3701 (IC base=+0.153)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.81` → IC=+0.141 (n=405)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` < 3.81 (IC base=+0.153)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0116` → IC=+0.222 (n=462)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.2727` → IC=+0.203 (n=409)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2727 (IC base=+0.155)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.775` → IC=+0.259 (n=422)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.775 (IC base=+0.155)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.144 (n=582)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.7321` → IC=+0.136 (n=1540)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.149 (n=300)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 8.0 (IC base=+0.113)
...
- **PATRÓN** `dist_vwap_pct` > `0.619` → IC=+0.159 (n=80)
  ...
  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.619 (IC base=+0.091)
...
- **PATRÓN** `sigma_h` < `0.0173` → IC=+0.263 (n=336)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0173 (IC base=+0.241)

- **PATRÓN** `sigma_h` > `0.0085` → IC=+0.248 (n=335)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.241 (n=303)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.241)

- **PATRÓN** `dist_vwap_pct` > `0.5816` → IC=+0.333 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5816 (IC base=+0.241)
...
- **PATRÓN** `dist_vwap_pct` < `0.1071` → IC=+0.132 (n=791)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.1071 (IC base=+0.117)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.148 (n=271)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 7.0 (IC base=+0.095)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `11.358` → IC=+0.210 (n=60)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.358 (IC base=+0.095)
...
- **PATRÓN** `sigma_h` > `0.014` → IC=+0.203 (n=281)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.014 (IC base=+0.192)
...
- **PATRÓN** `sigma_h` < `0.0098` → IC=+0.179 (n=219)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0098 (IC base=+0.179)

- **PATRÓN** `sigma_h` > `0.0131` → IC=+0.185 (n=331)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.206 (n=178)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.179)

- **PATRÓN** `dist_vwap_pct` > `0.1407` → IC=+0.234 (n=141)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1407 (IC base=+0.179)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.27` → IC=+0.178 (n=483)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` < 10.27 (IC base=+0.179)
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 823 | +0.129 | +37.38€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 823 | +0.129 | +37.38€ | 0 | 0 |
...
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 316 | +0.151 | +14.58€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 316 | +0.151 | +14.58€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 187 | +0.082 | +12.53€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 187 | +0.082 | +12.53€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS | 21 | +0.326 | +0.90€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 21 | +0.326 | +0.90€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 21 | +0.326 | +0.90€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 21 | +0.326 | +0.90€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 5288 | +0.187 | +88.24€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 3466 | +0.216 | +93.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 229 | +0.054 | -14.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 675 | +0.117 | -68.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 918 | +0.164 | +77.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 1549 | +0.194 | +37.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1157 | +0.209 | -6.04€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 76 | +0.051 | -7.27€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 306 | +0.185 | +54.12€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH | 1868 | +0.179 | +17.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1141 | +0.215 | +41.15€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 76 | -0.026 | -17.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min |