# Hipótesis automáticas — 2026-09-25 12:11 UTC
_Generado por shadow_postmortem.py sobre 604261 resoluciones (PNL=+68419.11€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.12` → IC=-0.229 (n=7141)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.12
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=21427)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `137.45` → IC=-0.250 (n=937)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 137.45
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=2812)

### BALLENAS_TARDIAS#BTC#15min
- **FILTRO** `restante_s_al_confirmar` < `500.19` → IC=-0.147 (n=369)
...
- **FILTRO** `restante_s_al_confirmar` < `165.3` → IC=-0.216 (n=1726)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 165.3
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=5185)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `127.04` → IC=-0.352 (n=1406)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 127.04
  - _Potencial_: sin este filtro IC_bueno=-0.115 (n=4221)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.205 (n=13991)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.69 (IC base=+0.101)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.155 (n=3528)

  - _Acción_: Kelly boost +0.78€ cuando `libro_spread` < 0.01 (IC base=+0.101)

- **PATRÓN** `libro_liquidez` > `5649.2316` → IC=+0.178 (n=2246)
...
- **PATRÓN** `py_entrada` < `0.35` → IC=+0.231 (n=11008)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.35 (IC base=+0.126)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.174 (n=5713)

  - _Acción_: Kelly boost +0.87€ cuando `libro_spread` < 0.01 (IC base=+0.126)

- **PATRÓN** `libro_liquidez` > `7845.6927` → IC=+0.175 (n=2158)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 7845.6927 (IC base=+0.126)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.212 (n=1741)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.206)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.207 (n=1708)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.206 (n=2148)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.158 (n=463)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 11.0 (IC base=+0.138)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.140 (n=672)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `1311.0387` → IC=+0.151 (n=666)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.162 (n=1097)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.247 (n=868)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.243)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.246 (n=869)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.433 (n=207)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `lag_apertura_s` < `3.36` → IC=+0.142 (n=2412)

  - _Acción_: Kelly boost +0.71€ cuando `lag_apertura_s` < 3.36 (IC base=+0.122)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.188 (n=2609)

  - _Acción_: Kelly boost +0.94€ cuando `py_entrada` < 0.38 (IC base=+0.113)

- **PATRÓN** `restante_min` < `4.19` → IC=+0.126 (n=2451)

  - _Acción_: Kelly boost +0.63€ cuando `restante_min` < 4.19 (IC base=+0.113)

- **PATRÓN** `restante_min` > `4.96` → IC=+0.134 (n=2678)

  - _Acción_: Kelly boost +0.67€ cuando `restante_min` > 4.96 (IC base=+0.113)

- **PATRÓN** `lag_apertura_s` < `2.27` → IC=+0.139 (n=2445)
...
### GBM_LATE_15M
- **PATRÓN** `drift_60min` |x|≤ `0.4861` → IC=+0.122 (n=8189)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.61€ cuando `drift_60min` |x|≤ 0.4861 (IC base=+0.106)

- **PATRÓN** `ibs_20min` > `0.9836` → IC=+0.244 (n=2730)
...
- **PATRÓN** `dist_vwap_pct` < `0.6336` → IC=+0.250 (n=2322)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6336 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.342` → IC=+0.187 (n=2166)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 8.342 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` < `1.2089` → IC=+0.248 (n=2227)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2089 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` > `1.0457` → IC=+0.260 (n=1010)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4361` → IC=+0.164 (n=1726)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.4361 (IC base=+0.161)

- **PATRÓN** `sigma_h` > `0.005` → IC=+0.248 (n=1253)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.248 (n=951)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.235)

- **PATRÓN** `ibs_20min` < `0.0571` → IC=+0.294 (n=616)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0571 (IC base=+0.235)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.497` → IC=+0.242 (n=215)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.497 (IC base=+0.235)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.459` → IC=+0.246 (n=1453)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.459 (IC base=+0.235)

- **PATRÓN** `volumen_pendiente_norm` > `0.2782` → IC=+0.267 (n=187)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2782 (IC base=+0.235)

- **PATRÓN** `volumen_spike_ratio` < `1.429` → IC=+0.232 (n=427)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.429 (IC base=+0.235)

- **PATRÓN** `volumen_spike_ratio` > `2.6147` → IC=+0.243 (n=426)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6147 (IC base=+0.235)

- **PATRÓN** `libro_liquidez` > `1808.46` → IC=+0.237 (n=933)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1808.46 (IC base=+0.235)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.232 (n=1482)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.217)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.217 (n=1439)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` > `0.2074` → IC=+0.219 (n=755)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2074 (IC base=+0.217)

- **PATRÓN** `dist_vwap_pct` < `0.5738` → IC=+0.222 (n=1481)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5738 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.631` → IC=+0.262 (n=242)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.631 (IC base=+0.217)

- **PATRÓN** `volumen_regimen` < `1.2554` → IC=+0.220 (n=1414)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2554 (IC base=+0.217)

- **PATRÓN** `volumen_regimen` > `0.8736` → IC=+0.225 (n=942)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7497` → IC=+0.216 (n=924)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7497 (IC base=+0.217)

- **PATRÓN** `volumen_spike_ratio` > `2.3728` → IC=+0.233 (n=462)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3728 (IC base=+0.217)

- **PATRÓN** `libro_liquidez` > `12518.8764` → IC=+0.220 (n=1263)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12518.8764 (IC base=+0.217)

- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.167 (n=982)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0039 (IC base=+0.142)

- **PATRÓN** `drift_60min` |x|≤ `0.0769` → IC=+0.167 (n=491)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.0769 (IC base=+0.142)
...
- **PATRÓN** `ibs_20min` < `0.3275` → IC=+0.199 (n=982)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3275 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` < `0.1332` → IC=+0.159 (n=1308)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.1332 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.262` → IC=+0.164 (n=236)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 11.262 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.282` → IC=+0.145 (n=1342)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` < 4.282 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `1.2035` → IC=+0.153 (n=1472)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.2035 (IC base=+0.142)

- **PATRÓN** `volumen_pendiente_norm` > `0.1564` → IC=+0.181 (n=390)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.1564 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` < `2.4238` → IC=+0.155 (n=1362)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.4238 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` > `1.4164` → IC=+0.151 (n=1362)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.4164 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `14092.9821` → IC=+0.147 (n=981)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 14092.9821 (IC base=+0.142)

- **PATRÓN** `ballena_activa_n` < `410.0` → IC=+0.151 (n=1282)

  - _Acción_: Kelly boost +0.76€ cuando `ballena_activa_n` < 410.0 (IC base=+0.142)

### GBM_LATE_15M#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0117` → IC=+0.208 (n=608)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0117 (IC base=+0.186)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.191 (n=1911)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 5.0 (IC base=+0.186)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.191 (n=1634)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` < `0.0103` → IC=+0.222 (n=1377)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0103 (IC base=+0.211)

- **PATRÓN** `drift_60min` |x|≤ `0.6083` → IC=+0.212 (n=1566)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.6083 (IC base=+0.211)
...
- **PATRÓN** `ibs_20min` < `0.0645` → IC=+0.244 (n=690)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0645 (IC base=+0.211)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.627` → IC=+0.239 (n=534)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.627 (IC base=+0.211)

- **PATRÓN** `volumen_pendiente_norm` > `0.3531` → IC=+0.272 (n=226)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3531 (IC base=+0.211)

- **PATRÓN** `volumen_spike_ratio` > `2.1985` → IC=+0.220 (n=957)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1985 (IC base=+0.211)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.215 (n=1058)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.211)

- **PATRÓN** `libro_liquidez` > `1899.5579` → IC=+0.221 (n=710)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1899.5579 (IC base=+0.211)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.150 (n=101)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=2242)

- **PATRÓN** `ibs_20min` > `0.9456` → IC=+0.210 (n=364)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9456 (IC base=+0.024)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.752` → IC=+0.159 (n=726)
  ...
  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 4.752 (IC base=+0.024)
...
- **PATRÓN** `volumen_regimen` > `1.2039` → IC=+0.342 (n=112)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2039 (IC base=+0.024)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4099` → IC=+0.345 (n=108)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4099 (IC base=+0.024)
...
- **PATRÓN** `dist_vwap_pct` > `0.6848` → IC=+0.203 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6848 (IC base=+0.019)

- **PATRÓN** `volumen_regimen` < `0.8504` → IC=+0.166 (n=566)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.8504 (IC base=+0.019)

- **PATRÓN** `volumen_regimen` > `1.1607` → IC=+0.153 (n=283)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5204` → IC=+0.179 (n=711)
...
- **FILTRO** `ibs_20min` > `0.25` → IC=-0.125 (n=2217)

  - _Acción_: SKIP cuando `ibs_20min` > 0.25
  - _Potencial_: sin este filtro IC_bueno=+0.125 (n=1095)

- **FILTRO** `sigma_ewma_delta_pct` > `8.692` → IC=-0.208 (n=354)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.692
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=2958)
...
- **FILTRO** `ibs_20min` < `0.7191` → IC=-0.153 (n=1523)
...
- **FILTRO** `ibs_20min` > `0.77` → IC=-0.203 (n=844)

  - _Acción_: SKIP cuando `ibs_20min` > 0.77
  - _Potencial_: sin este filtro IC_bueno=+0.037 (n=2542)
...
- **PATRÓN** `dist_vwap_pct` > `0.9096` → IC=+0.274 (n=162)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4655` → IC=+0.240 (n=1986)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4655 (IC base=+0.098)

- **PATRÓN** `volumen_spike_ratio` > `2.6635` → IC=+0.243 (n=1985)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6635 (IC base=+0.098)

- **PATRÓN** `ballena_activa_n` < `96.0` → IC=+0.268 (n=5463)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 96.0 (IC base=+0.098)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.158 (n=3414)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0091 (IC base=+0.074)

- **PATRÓN** `ibs_20min` < `0.5491` → IC=+0.155 (n=9001)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.5491 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` > `0.7295` → IC=+0.243 (n=641)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7295 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` < `0.2525` → IC=+0.240 (n=2854)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2525 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` < `0.7099` → IC=+0.242 (n=1329)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7099 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` > `1.2057` → IC=+0.248 (n=1007)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2057 (IC base=+0.074)

- **PATRÓN** `volumen_pendiente_norm` > `0.2438` → IC=+0.304 (n=753)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2438 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` < `1.6017` → IC=+0.261 (n=1758)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6017 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` > `2.307` → IC=+0.258 (n=1811)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.307 (IC base=+0.074)

- **PATRÓN** `ballena_activa_n` < `84.0` → IC=+0.265 (n=3874)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 84.0 (IC base=+0.074)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `ibs_20min` < `0.2584` → IC=-0.146 (n=708)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2584
  - _Potencial_: sin este filtro IC_bueno=+0.108 (n=2126)

- **FILTRO** `sigma_ewma_delta_pct` > `4.535` → IC=-0.159 (n=540)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.535
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=1820)

- **PATRÓN** `ibs_20min` > `0.8935` → IC=+0.268 (n=710)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8935 (IC base=+0.044)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.823` → IC=+0.211 (n=375)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.823 (IC base=+0.044)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4339` → IC=+0.175 (n=300)
...
- **PATRÓN** `volumen_spike_ratio` > `2.0444` → IC=+0.441 (n=32)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.0444 (IC base=-0.016)
...
- **PATRÓN** `ibs_20min` > `0.8636` → IC=+0.158 (n=688)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` > 0.8636 (IC base=+0.025)

- **PATRÓN** `dist_vwap_pct` > `0.1282` → IC=+0.172 (n=525)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.1282 (IC base=+0.025)

- **PATRÓN** `volumen_regimen` > `0.6744` → IC=+0.169 (n=847)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4247` → IC=+0.196 (n=310)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 1.4247 (IC base=+0.025)

- **PATRÓN** `volumen_spike_ratio` > `2.3807` → IC=+0.153 (n=309)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 2.3807 (IC base=+0.025)

- **PATRÓN** `ballena_activa_n` < `243.0` → IC=+0.197 (n=404)

  - _Acción_: Kelly boost +0.99€ cuando `ballena_activa_n` < 243.0 (IC base=+0.025)

- **PATRÓN** `dist_vwap_pct` < `0.1614` → IC=+0.223 (n=590)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1614 (IC base=+0.006)

- **PATRÓN** `volumen_regimen` > `0.8539` → IC=+0.227 (n=386)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8539 (IC base=+0.006)

- **PATRÓN** `volumen_pendiente_norm` > `0.2713` → IC=+0.314 (n=68)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2713 (IC base=+0.006)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1523` → IC=+0.235 (n=243)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1523 (IC base=+0.006)

- **PATRÓN** `ballena_activa_n` < `464.0` → IC=+0.219 (n=532)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 464.0 (IC base=+0.006)

### GBM_LATE_15M_ESPACIO_ATR#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.287 (n=542)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0113 (IC base=+0.246)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.249 (n=1620)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.246)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.249 (n=1448)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.246)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.299 (n=859)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.104` → IC=+0.258 (n=1376)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.104 (IC base=+0.246)

- **PATRÓN** `volumen_spike_ratio` > `3.4171` → IC=+0.264 (n=511)
...
- **PATRÓN** `libro_liquidez` > `1802.242` → IC=+0.249 (n=1080)
...
- **PATRÓN** `drift_60min` |x|≤ `0.6123` → IC=+0.284 (n=1297)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.0909` → IC=+0.289 (n=870)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0909 (IC base=+0.282)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.812` → IC=+0.297 (n=511)
...
- **PATRÓN** `volumen_spike_ratio` > `2.7291` → IC=+0.287 (n=544)
...
- **PATRÓN** `libro_liquidez` > `1887.358` → IC=+0.297 (n=588)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1887.358 (IC base=+0.282)

- **PATRÓN** `ballena_activa_n` < `28.0` → IC=+0.287 (n=773)
...
- **FILTRO** `ibs_20min` < `0.294` → IC=-0.184 (n=504)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.081 (n=1512)

- **FILTRO** `ibs_20min` > `0.7758` → IC=-0.192 (n=605)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7758
  - _Potencial_: sin este filtro IC_bueno=+0.057 (n=1821)

- **PATRÓN** `ibs_20min` > `0.8224` → IC=+0.160 (n=686)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.8224 (IC base=+0.014)

- **PATRÓN** `dist_vwap_pct` > `0.4511` → IC=+0.229 (n=256)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4511 (IC base=+0.014)

- **PATRÓN** `dist_vwap_pct` < `0.6974` → IC=+0.216 (n=596)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6974 (IC base=+0.014)

- **PATRÓN** `volumen_regimen` < `0.9969` → IC=+0.239 (n=511)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9969 (IC base=+0.014)

- **PATRÓN** `volumen_regimen` > `0.5875` → IC=+0.215 (n=581)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.5875 (IC base=+0.014)

- **PATRÓN** `volumen_pendiente_norm` > `0.0813` → IC=+0.265 (n=211)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0813 (IC base=+0.014)

- **PATRÓN** `volumen_spike_ratio` < `1.5078` → IC=+0.266 (n=242)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5078 (IC base=+0.014)

- **PATRÓN** `ballena_activa_n` < `149.0` → IC=+0.248 (n=558)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 149.0 (IC base=+0.014)

- **PATRÓN** `dist_vwap_pct` > `0.1559` → IC=+0.216 (n=213)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1559 (IC base=-0.005)

- **PATRÓN** `dist_vwap_pct` < `0.6853` → IC=+0.203 (n=499)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6853 (IC base=-0.005)

- **PATRÓN** `volumen_regimen` < `1.1677` → IC=+0.215 (n=433)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.1677 (IC base=-0.005)

- **PATRÓN** `volumen_pendiente_norm` > `0.2852` → IC=+0.286 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2852 (IC base=-0.005)

- **PATRÓN** `volumen_spike_ratio` < `1.8269` → IC=+0.260 (n=260)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8269 (IC base=-0.005)

- **PATRÓN** `volumen_spike_ratio` > `1.4384` → IC=+0.237 (n=390)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4384 (IC base=-0.005)
...
- **FILTRO** `ibs_20min` < `0.7273` → IC=-0.194 (n=1090)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7273
  - _Potencial_: sin este filtro IC_bueno=+0.279 (n=1092)

- **FILTRO** `ibs_20min` > `0.6875` → IC=-0.237 (n=560)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6875
  - _Potencial_: sin este filtro IC_bueno=+0.097 (n=1697)

- **FILTRO** `sigma_ewma_delta_pct` > `4.71` → IC=-0.184 (n=492)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.71
  - _Potencial_: sin este filtro IC_bueno=+0.070 (n=1765)

- **PATRÓN** `ibs_20min` > `0.7273` → IC=+0.279 (n=1092)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7273 (IC base=+0.043)

- **PATRÓN** `dist_vwap_pct` > `0.8516` → IC=+0.335 (n=253)
...
- **PATRÓN** `volumen_regimen` < `0.8684` → IC=+0.305 (n=537)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8684 (IC base=+0.043)

- **PATRÓN** `volumen_regimen` > `0.7329` → IC=+0.296 (n=719)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7329 (IC base=+0.043)

- **PATRÓN** `volumen_pendiente_norm` < `0.1022` → IC=+0.294 (n=749)
...
- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.316 (n=684)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 56.0 (IC base=+0.043)

- **PATRÓN** `ibs_20min` < `0.5806` → IC=+0.121 (n=1490)
...
- **PATRÓN** `sigma_h` > `0.0168` → IC=+0.317 (n=890)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.3945` → IC=+0.305 (n=1432)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3945 (IC base=+0.273)

- **PATRÓN** `dist_vwap_pct` > `0.3069` → IC=+0.281 (n=547)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3069 (IC base=+0.273)

- **PATRÓN** `dist_vwap_pct` < `0.2265` → IC=+0.274 (n=1293)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2265 (IC base=+0.273)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.477` → IC=+0.291 (n=524)
...
- **PATRÓN** `volumen_regimen` > `1.2436` → IC=+0.308 (n=477)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2436 (IC base=+0.273)

- **PATRÓN** `volumen_pendiente_norm` > `0.2377` → IC=+0.335 (n=234)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2377 (IC base=+0.273)

- **PATRÓN** `volumen_spike_ratio` < `1.5347` → IC=+0.276 (n=557)
...
- **PATRÓN** `libro_liquidez` > `2626.5629` → IC=+0.278 (n=954)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2626.5629 (IC base=+0.273)
...
- **PATRÓN** `sigma_h` > `0.0112` → IC=+0.201 (n=2669)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.180 (n=8326)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 5.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` > `0.5753` → IC=+0.219 (n=7999)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5753 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` > `0.1778` → IC=+0.196 (n=3495)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1778 (IC base=+0.168)
...
- **PATRÓN** `volumen_regimen` < `1.2119` → IC=+0.161 (n=5297)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2119 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` > `0.6316` → IC=+0.161 (n=5297)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.6316 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.2956` → IC=+0.198 (n=1196)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.2956 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` < `1.559` → IC=+0.167 (n=3378)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.559 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.618` → IC=+0.177 (n=2558)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 2.618 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `2407.1003` → IC=+0.169 (n=5333)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 2407.1003 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `114.0` → IC=+0.181 (n=6907)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 114.0 (IC base=+0.168)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.182 (n=5088)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0066 (IC base=+0.168)

- **PATRÓN** `drift_60min` |x|≤ `0.0809` → IC=+0.208 (n=2542)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0809 (IC base=+0.168)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.204 (n=2934)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` < `0.48` → IC=+0.227 (n=7622)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.48 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` > `0.9207` → IC=+0.146 (n=741)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.9207 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` < `0.2351` → IC=+0.159 (n=5528)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.2351 (IC base=+0.168)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.348` → IC=+0.196 (n=1291)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 10.348 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` < `1.1736` → IC=+0.151 (n=5516)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5588` → IC=+0.166 (n=3051)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.5588 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.2519` → IC=+0.171 (n=3144)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.2519 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `115.0` → IC=+0.173 (n=6581)

  - _Acción_: Kelly boost +0.87€ cuando `ballena_activa_n` < 115.0 (IC base=+0.168)

### GBM_LATE_15M_MULTIHORIZONTE#BNB#15min
- **PATRÓN** `sigma_h` < `0.0052` → IC=+0.214 (n=456)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.3425` → IC=+0.207 (n=1360)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3425 (IC base=+0.183)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.183 (n=1430)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 5.0 (IC base=+0.183)
...
- **PATRÓN** `ibs_20min` < `0.3443` → IC=+0.265 (n=994)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3443 (IC base=+0.238)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.325` → IC=+0.250 (n=1074)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.325 (IC base=+0.238)

- **PATRÓN** `volumen_pendiente_norm` < `0.1603` → IC=+0.234 (n=938)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4211` → IC=+0.263 (n=306)
...
- **PATRÓN** `libro_liquidez` > `1809.2686` → IC=+0.244 (n=662)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1809.2686 (IC base=+0.238)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.158 (n=1296)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.5665` → IC=+0.188 (n=1293)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` < 0.5665 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.1346` → IC=+0.162 (n=1280)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.1346 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.889` → IC=+0.205 (n=259)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.889 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `1.2089` → IC=+0.160 (n=1293)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2089 (IC base=+0.138)

- **PATRÓN** `volumen_pendiente_norm` > `0.1561` → IC=+0.157 (n=395)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_pendiente_norm` > 0.1561 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` < `2.4411` → IC=+0.147 (n=1181)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 2.4411 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` > `1.4202` → IC=+0.139 (n=1181)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` > 1.4202 (IC base=+0.138)
...
- **PATRÓN** `sigma_h` > `0.0101` → IC=+0.222 (n=609)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0101 (IC base=+0.199)

- **PATRÓN** `drift_60min` |x|≤ `0.235` → IC=+0.216 (n=896)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.294 (n=711)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.199)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.877` → IC=+0.272 (n=414)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.877 (IC base=+0.199)

- **PATRÓN** `volumen_pendiente_norm` > `0.1325` → IC=+0.201 (n=523)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1325 (IC base=+0.199)

- **PATRÓN** `volumen_spike_ratio` < `1.6328` → IC=+0.199 (n=426)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 1.6328 (IC base=+0.199)

- **PATRÓN** `volumen_spike_ratio` > `2.8468` → IC=+0.214 (n=579)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8468 (IC base=+0.199)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.208 (n=947)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.199)

- **PATRÓN** `sigma_h` < `0.0114` → IC=+0.231 (n=1112)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.3478` → IC=+0.249 (n=1112)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3478 (IC base=+0.218)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.746` → IC=+0.265 (n=478)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7663` → IC=+0.213 (n=454)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7663 (IC base=+0.218)

- **PATRÓN** `volumen_spike_ratio` > `2.215` → IC=+0.229 (n=688)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.215 (IC base=+0.218)

- **PATRÓN** `libro_liquidez` > `1895.6572` → IC=+0.217 (n=504)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1895.6572 (IC base=+0.218)
...
- **PATRÓN** `dist_vwap_pct` > `0.1612` → IC=+0.181 (n=850)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.1612 (IC base=+0.144)
...
- **PATRÓN** `volumen_regimen` < `1.0405` → IC=+0.146 (n=1124)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4263` → IC=+0.154 (n=417)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 1.4263 (IC base=+0.144)
...
- **PATRÓN** `libro_liquidez` > `6488.9388` → IC=+0.185 (n=852)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 6488.9388 (IC base=+0.144)

- **PATRÓN** `ballena_activa_n` < `162.0` → IC=+0.149 (n=1213)

  - _Acción_: Kelly boost +0.75€ cuando `ballena_activa_n` < 162.0 (IC base=+0.144)

- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.153 (n=1341)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0072 (IC base=+0.123)

- **PATRÓN** `drift_60min` |x|≤ `0.3842` → IC=+0.143 (n=1341)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6457` → IC=+0.173 (n=1341)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.6457 (IC base=+0.123)

- **PATRÓN** `dist_vwap_pct` < `0.5823` → IC=+0.132 (n=1558)
...
- **PATRÓN** `volumen_regimen` < `0.8476` → IC=+0.148 (n=894)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.8476 (IC base=+0.123)

- **PATRÓN** `volumen_pendiente_norm` > `0.2919` → IC=+0.194 (n=194)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2919 (IC base=+0.123)

- **PATRÓN** `volumen_spike_ratio` < `1.789` → IC=+0.137 (n=813)
...
- **PATRÓN** `sigma_h` > `0.0101` → IC=+0.157 (n=656)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` > 0.0101 (IC base=+0.119)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.140 (n=1475)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 5.0 (IC base=+0.119)

- **PATRÓN** `ibs_20min` > `0.5156` → IC=+0.207 (n=1444)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5156 (IC base=+0.119)

- **PATRÓN** `dist_vwap_pct` > `0.8467` → IC=+0.215 (n=436)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8467 (IC base=+0.119)
...
- **PATRÓN** `volumen_regimen` < `1.2184` → IC=+0.132 (n=1445)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.0711` → IC=+0.126 (n=605)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_pendiente_norm` > 0.0711 (IC base=+0.119)

- **PATRÓN** `volumen_spike_ratio` < `2.4678` → IC=+0.129 (n=1395)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` < 2.4678 (IC base=+0.119)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.127 (n=1509)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.02 (IC base=+0.119)

- **PATRÓN** `libro_liquidez` > `2898.7698` → IC=+0.197 (n=655)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 2898.7698 (IC base=+0.119)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.138 (n=1099)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=1477)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` < `0.5667` → IC=+0.210 (n=1461)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5667 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `1.0109` → IC=+0.143 (n=197)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 1.0109 (IC base=+0.113)
...
- **PATRÓN** `libro_liquidez` > `2168.8568` → IC=+0.146 (n=974)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 2168.8568 (IC base=+0.113)

### GBM_LATE_15M_MULTIHORIZONTE#XRP#15min
- **PATRÓN** `sigma_h` > `0.0188` → IC=+0.213 (n=925)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0188 (IC base=+0.201)

- **PATRÓN** `drift_60min` |x|≤ `0.293` → IC=+0.201 (n=925)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.293 (IC base=+0.201)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.204 (n=1438)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.74` → IC=+0.258 (n=1240)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.74 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` > `0.5198` → IC=+0.221 (n=660)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5198 (IC base=+0.201)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.577` → IC=+0.241 (n=654)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.577 (IC base=+0.201)

- **PATRÓN** `volumen_regimen` < `1.2075` → IC=+0.205 (n=1387)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2075 (IC base=+0.201)

- **PATRÓN** `volumen_regimen` > `0.6251` → IC=+0.212 (n=1386)
...
- **PATRÓN** `ibs_20min` < `0.4384` → IC=+0.245 (n=1423)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4384 (IC base=+0.203)

- **PATRÓN** `dist_vwap_pct` > `1.2573` → IC=+0.229 (n=175)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2573 (IC base=+0.203)
...
- **PATRÓN** `volumen_regimen` > `0.6325` → IC=+0.214 (n=1423)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6325 (IC base=+0.203)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2006` → IC=+0.193 (n=1126)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 2.2006 (IC base=+0.203)

- **PATRÓN** `volumen_spike_ratio` > `1.4361` → IC=+0.198 (n=1280)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.4361 (IC base=+0.203)

- **PATRÓN** `libro_liquidez` > `2602.0936` → IC=+0.208 (n=949)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2602.0936 (IC base=+0.203)
...
- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.175 (n=644)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0088 (IC base=+0.147)

- **PATRÓN** `drift_60min` |x|≤ `0.1009` → IC=+0.155 (n=644)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.1009 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.184 (n=961)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 15.0 (IC base=+0.147)

- **PATRÓN** `ibs_20min` > `0.5386` → IC=+0.189 (n=1726)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.5386 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.8737` → IC=+0.180 (n=311)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.8737 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.703` → IC=+0.175 (n=873)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 3.703 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `0.8754` → IC=+0.165 (n=1134)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.8754 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` > `0.7015` → IC=+0.151 (n=1518)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 0.7015 (IC base=+0.147)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4385` → IC=+0.163 (n=621)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 1.4385 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` > `2.2107` → IC=+0.153 (n=845)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 2.2107 (IC base=+0.147)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.151 (n=2192)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.02 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `12421.138` → IC=+0.159 (n=644)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 12421.138 (IC base=+0.147)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.168 (n=1709)

  - _Acción_: Kelly boost +0.84€ cuando `ballena_activa_n` < 156.0 (IC base=+0.147)

- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.139 (n=1339)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0056 (IC base=+0.111)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.125 (n=2023)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 5.0 (IC base=+0.111)

- **PATRÓN** `ibs_20min` < `0.6607` → IC=+0.139 (n=2005)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` < 0.6607 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` < `0.2153` → IC=+0.123 (n=1777)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.2153 (IC base=+0.111)

- **PATRÓN** `volumen_regimen` < `0.6986` → IC=+0.125 (n=793)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` < 0.6986 (IC base=+0.111)

- **PATRÓN** `volumen_pendiente_norm` > `0.1653` → IC=+0.127 (n=499)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_pendiente_norm` > 0.1653 (IC base=+0.111)

- **PATRÓN** `volumen_spike_ratio` < `1.4526` → IC=+0.144 (n=644)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 1.4526 (IC base=+0.111)

- **PATRÓN** `libro_liquidez` > `2750.9096` → IC=+0.125 (n=1791)

  - _Acción_: Kelly boost +0.62€ cuando `libro_liquidez` > 2750.9096 (IC base=+0.111)

- **PATRÓN** `ballena_activa_n` < `29.0` → IC=+0.121 (n=817)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.209 (n=624)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1568` → IC=+0.209 (n=314)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1568 (IC base=+0.194)

- **PATRÓN** `dist_vwap_pct` < `0.2301` → IC=+0.195 (n=526)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` < 0.2301 (IC base=+0.194)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2622` → IC=+0.275 (n=87)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2622 (IC base=+0.194)
...
- **PATRÓN** `volumen_spike_ratio` > `1.7539` → IC=+0.218 (n=395)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7539 (IC base=+0.194)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.203 (n=668)
...
- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.120 (n=496)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.60€ cuando `sigma_h` < 0.0062 (IC base=+0.095)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.120 (n=377)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` > 11.0 (IC base=+0.095)

- **PATRÓN** `ibs_20min` < `0.0839` → IC=+0.163 (n=188)

  - _Acción_: Kelly boost +0.82€ cuando `ibs_20min` < 0.0839 (IC base=+0.095)

- **PATRÓN** `volumen_regimen` < `0.6847` → IC=+0.128 (n=248)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` < 0.6847 (IC base=+0.095)

- **PATRÓN** `volumen_pendiente_norm` > `0.1662` → IC=+0.132 (n=131)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_pendiente_norm` > 0.1662 (IC base=+0.095)

- **PATRÓN** `libro_liquidez` > `9139.2638` → IC=+0.122 (n=376)
...
- **PATRÓN** `ibs_20min` < `0.5669` → IC=+0.238 (n=9265)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5669 (IC base=+0.182)

- **PATRÓN** `dist_vwap_pct` < `0.2512` → IC=+0.162 (n=5736)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2512 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.044` → IC=+0.202 (n=1299)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.044 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.735` → IC=+0.183 (n=8965)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.735 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` < `0.7051` → IC=+0.162 (n=2798)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.7051 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` > `1.204` → IC=+0.155 (n=2119)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.204 (IC base=+0.182)

- **PATRÓN** `volumen_pendiente_norm` > `0.2878` → IC=+0.243 (n=1222)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2878 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` > `2.6218` → IC=+0.194 (n=2838)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 2.6218 (IC base=+0.182)

- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.194 (n=5503)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 48.0 (IC base=+0.182)

### GBM_LATE_15M_TARDIO#BNB#15min
- **PATRÓN** `sigma_h` < `0.0051` → IC=+0.206 (n=577)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0051 (IC base=+0.190)

- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.222 (n=578)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0083 (IC base=+0.190)

- **PATRÓN** `drift_60min` |x|≤ `0.3555` → IC=+0.191 (n=1729)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.242` → IC=+0.154 (n=1479)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 4.242 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` < `0.6287` → IC=+0.179 (n=547)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1137` → IC=+0.161 (n=1395)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.1137 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.4033` → IC=+0.155 (n=1585)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.4033 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `15747.2217` → IC=+0.161 (n=744)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 15747.2217 (IC base=+0.150)

- **PATRÓN** `ballena_activa_n` < `472.0` → IC=+0.158 (n=1517)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 472.0 (IC base=+0.150)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.166 (n=1419)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0057 (IC base=+0.154)

- **PATRÓN** `drift_60min` |x|≤ `0.2608` → IC=+0.168 (n=1249)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.2608 (IC base=+0.154)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.187 (n=554)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 17.0 (IC base=+0.154)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.157 (n=645)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 7.0 (IC base=+0.154)

- **PATRÓN** `ibs_20min` < `0.2703` → IC=+0.240 (n=946)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2703 (IC base=+0.154)

- **PATRÓN** `dist_vwap_pct` > `0.6888` → IC=+0.158 (n=232)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.6888 (IC base=+0.154)

- **PATRÓN** `dist_vwap_pct` < `0.1347` → IC=+0.168 (n=1276)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` < 0.1347 (IC base=+0.154)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.493` → IC=+0.163 (n=238)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 11.493 (IC base=+0.154)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.281` → IC=+0.154 (n=1286)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 4.281 (IC base=+0.154)

- **PATRÓN** `volumen_regimen` < `1.1888` → IC=+0.166 (n=1419)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.1888 (IC base=+0.154)

- **PATRÓN** `volumen_pendiente_norm` > `0.1512` → IC=+0.198 (n=379)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.1512 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` < `2.4038` → IC=+0.162 (n=1321)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.4038 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` > `2.0862` → IC=+0.169 (n=600)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.0862 (IC base=+0.154)

- **PATRÓN** `ballena_activa_n` < `420.0` → IC=+0.158 (n=1078)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 420.0 (IC base=+0.154)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `sigma_h` > `0.012` → IC=+0.246 (n=554)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.012 (IC base=+0.218)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.226 (n=1737)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.218)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.223 (n=1686)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` < `0.211` → IC=+0.221 (n=1645)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.211 (IC base=+0.218)

- **PATRÓN** `volumen_spike_ratio` > `2.8308` → IC=+0.240 (n=718)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8308 (IC base=+0.218)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.229 (n=1178)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.218)

- **PATRÓN** `libro_liquidez` > `1802.6286` → IC=+0.218 (n=1106)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1802.6286 (IC base=+0.218)

- **PATRÓN** `sigma_h` < `0.0117` → IC=+0.238 (n=1546)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.3611` → IC=+0.267 (n=1361)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3611 (IC base=+0.232)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.756` → IC=+0.274 (n=573)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.756 (IC base=+0.232)

- **PATRÓN** `volumen_pendiente_norm` > `0.3437` → IC=+0.309 (n=228)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3437 (IC base=+0.232)

- **PATRÓN** `volumen_spike_ratio` < `1.7536` → IC=+0.231 (n=626)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7536 (IC base=+0.232)

- **PATRÓN** `volumen_spike_ratio` > `2.1869` → IC=+0.235 (n=947)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1869 (IC base=+0.232)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.238 (n=1046)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.232)

- **PATRÓN** `libro_liquidez` > `1953.3163` → IC=+0.245 (n=516)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1953.3163 (IC base=+0.232)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.230 (n=1351)
...
- **PATRÓN** `volumen_regimen` < `0.8748` → IC=+0.157 (n=1161)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5221` → IC=+0.150 (n=742)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.5221 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `1.767` → IC=+0.150 (n=1123)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.767 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `8055.3183` → IC=+0.232 (n=789)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8055.3183 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `91.0` → IC=+0.152 (n=719)

  - _Acción_: Kelly boost +0.76€ cuando `ballena_activa_n` < 91.0 (IC base=+0.137)

- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.154 (n=1419)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0076 (IC base=+0.137)

- **PATRÓN** `drift_60min` |x|≤ `0.4484` → IC=+0.153 (n=1418)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.1425` → IC=+0.259 (n=624)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1425 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.5983` → IC=+0.142 (n=1569)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.5983 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.146` → IC=+0.179 (n=210)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 11.146 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `0.6962` → IC=+0.152 (n=624)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.6962 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` > `1.1938` → IC=+0.146 (n=473)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1326` → IC=+0.163 (n=559)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.82€ cuando `drift_60min` |x|≤ 0.1326 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.126 (n=1732)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 5.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` < `0.6364` → IC=+0.205 (n=1678)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6364 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` < `0.2204` → IC=+0.132 (n=1370)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.2204 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.486` → IC=+0.127 (n=1622)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 3.486 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` < `0.7176` → IC=+0.153 (n=738)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.7176 (IC base=+0.114)

- **PATRÓN** `volumen_pendiente_norm` > `0.2229` → IC=+0.167 (n=262)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.2229 (IC base=+0.114)

- **PATRÓN** `volumen_spike_ratio` < `1.4467` → IC=+0.145 (n=505)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 1.4467 (IC base=+0.114)

- **PATRÓN** `libro_liquidez` > `2839.872` → IC=+0.170 (n=559)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 2839.872 (IC base=+0.114)

- **PATRÓN** `ballena_activa_n` < `51.0` → IC=+0.121 (n=1311)

  - _Acción_: Kelly boost +0.60€ cuando `ballena_activa_n` < 51.0 (IC base=+0.114)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` > `0.0193` → IC=+0.218 (n=1155)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0193 (IC base=+0.208)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.214 (n=1805)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.208)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.210 (n=1553)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.208)

- **PATRÓN** `ibs_20min` > `0.5143` → IC=+0.248 (n=1734)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5143 (IC base=+0.208)

- **PATRÓN** `dist_vwap_pct` > `0.2063` → IC=+0.234 (n=1025)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2063 (IC base=+0.208)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.264` → IC=+0.272 (n=310)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.264 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` < `1.2447` → IC=+0.211 (n=1733)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2447 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` > `0.6398` → IC=+0.216 (n=1733)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6398 (IC base=+0.208)

- **PATRÓN** `volumen_pendiente_norm` > `0.2342` → IC=+0.247 (n=302)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2342 (IC base=+0.208)

- **PATRÓN** `volumen_spike_ratio` > `2.505` → IC=+0.236 (n=558)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.505 (IC base=+0.208)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.218 (n=1825)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.208)

- **PATRÓN** `libro_liquidez` > `2631.2954` → IC=+0.220 (n=1155)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2631.2954 (IC base=+0.208)

- **PATRÓN** `sigma_h` < `0.0091` → IC=+0.225 (n=617)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0091 (IC base=+0.201)

- **PATRÓN** `sigma_h` > `0.0256` → IC=+0.223 (n=615)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0256 (IC base=+0.201)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.209 (n=1292)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.201)

- **PATRÓN** `ibs_20min` < `0.4227` → IC=+0.266 (n=1624)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4227 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` > `1.2726` → IC=+0.205 (n=307)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2726 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` < `0.9274` → IC=+0.203 (n=2050)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.9274 (IC base=+0.201)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.826` → IC=+0.255 (n=255)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.826 (IC base=+0.201)

- **PATRÓN** `volumen_regimen` > `1.2357` → IC=+0.236 (n=615)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1829` → IC=+0.198 (n=1462)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.142 (n=3102)

- **PATRÓN** `sigma_h` < `0.0091` → IC=+0.167 (n=2619)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0091 (IC base=+0.157)

- **PATRÓN** `drift_60min` |x|≤ `0.5172` → IC=+0.166 (n=2975)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.5172 (IC base=+0.157)
...
- **PATRÓN** `ibs_20min` > `0.9415` → IC=+0.215 (n=992)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9415 (IC base=+0.157)

- **PATRÓN** `dist_vwap_pct` > `0.1891` → IC=+0.170 (n=1100)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.1891 (IC base=+0.157)

- **PATRÓN** `dist_vwap_pct` < `0.4851` → IC=+0.153 (n=1844)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.4851 (IC base=+0.157)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.192` → IC=+0.189 (n=487)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 10.192 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` > `0.8968` → IC=+0.166 (n=1309)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 0.8968 (IC base=+0.157)

- **PATRÓN** `volumen_pendiente_norm` > `0.171` → IC=+0.191 (n=817)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.171 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` < `1.4562` → IC=+0.165 (n=980)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.4562 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` > `1.8774` → IC=+0.165 (n=1959)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.8774 (IC base=+0.157)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.159 (n=2089)

  - _Acción_: Kelly boost +0.79€ cuando `libro_spread` < 0.01 (IC base=+0.157)

- **PATRÓN** `libro_liquidez` > `3861.7729` → IC=+0.166 (n=1983)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 3861.7729 (IC base=+0.157)

- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.198 (n=782)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0038 (IC base=+0.140)

- **PATRÓN** `drift_60min` |x|≤ `0.4871` → IC=+0.158 (n=2339)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.1809` → IC=+0.172 (n=1029)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.1809 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` > `0.6841` → IC=+0.162 (n=448)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.6841 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.281` → IC=+0.151 (n=2324)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` < 6.281 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `0.9003` → IC=+0.158 (n=1491)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.9003 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` < `0.0968` → IC=+0.144 (n=2117)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_pendiente_norm` < 0.0968 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` > `0.0726` → IC=+0.143 (n=1079)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_pendiente_norm` > 0.0726 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` < `1.4249` → IC=+0.154 (n=770)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 1.4249 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.8143` → IC=+0.143 (n=1539)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.8143 (IC base=+0.140)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.142 (n=3102)

  - _Acción_: Kelly boost +0.71€ cuando `libro_spread` < 0.01 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `7208.644` → IC=+0.153 (n=2089)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 7208.644 (IC base=+0.140)
...
- **PATRÓN** `ibs_20min` > `0.6084` → IC=+0.147 (n=432)
...
- **PATRÓN** `dist_vwap_pct` < `0.2226` → IC=+0.139 (n=968)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.2226 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.372` → IC=+0.163 (n=932)
...
- **PATRÓN** `libro_liquidez` > `11474.1256` → IC=+0.153 (n=953)
...
- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.170 (n=850)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0072 (IC base=+0.165)

- **PATRÓN** `sigma_h` > `0.0043` → IC=+0.172 (n=966)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0043 (IC base=+0.165)

- **PATRÓN** `drift_60min` |x|≤ `0.383` → IC=+0.171 (n=849)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.383 (IC base=+0.165)
...
- **PATRÓN** `ibs_20min` < `0.5481` → IC=+0.172 (n=644)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.5481 (IC base=+0.165)

- **PATRÓN** `ibs_20min` > `0.8884` → IC=+0.179 (n=322)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` > 0.8884 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` > `0.6979` → IC=+0.167 (n=277)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.6979 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` < `0.4249` → IC=+0.173 (n=897)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.4249 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.148` → IC=+0.175 (n=866)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` < 4.148 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` < `0.89` → IC=+0.167 (n=644)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 0.89 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` > `0.6413` → IC=+0.167 (n=965)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 0.6413 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` < `0.1056` → IC=+0.166 (n=888)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` < 0.1056 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` > `0.0781` → IC=+0.175 (n=419)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.0781 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` < `1.4349` → IC=+0.179 (n=316)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 1.4349 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `1.5168` → IC=+0.166 (n=845)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.5168 (IC base=+0.165)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.171 (n=958)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4977` → IC=+0.174 (n=801)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.4977 (IC base=+0.145)
...
- **PATRÓN** `ibs_20min` < `0.7466` → IC=+0.149 (n=801)

  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` < 0.7466 (IC base=+0.145)

- **PATRÓN** `ibs_20min` > `0.0954` → IC=+0.153 (n=801)

  - _Acción_: Kelly boost +0.76€ cuando `ibs_20min` > 0.0954 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.6251` → IC=+0.182 (n=174)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.6251 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.399` → IC=+0.155 (n=725)
...
- **PATRÓN** `volumen_regimen` > `0.7274` → IC=+0.148 (n=715)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` > 0.7274 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` > `0.0742` → IC=+0.163 (n=336)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` > 0.0742 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` < `2.2051` → IC=+0.158 (n=690)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.2051 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` > `1.783` → IC=+0.148 (n=523)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.783 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `7571.9816` → IC=+0.171 (n=801)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 7571.9816 (IC base=+0.145)
...
- **FILTRO** `sigma_h` > `0.0066` → IC=-0.212 (n=123)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.076 (n=375)

- **FILTRO** `dist_vwap_pct` > `0.1862` → IC=-0.179 (n=26)

  - _Acción_: SKIP cuando `dist_vwap_pct` > 0.1862
  - _Potencial_: sin este filtro IC_bueno=+0.115 (n=330)

- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.190 (n=414)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.0041 (IC base=+0.094)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.128 (n=865)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 8.0 (IC base=+0.094)

- **PATRÓN** `ibs_20min` > `0.6694` → IC=+0.204 (n=759)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6694 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` > `0.1554` → IC=+0.158 (n=460)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.1554 (IC base=+0.094)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.443` → IC=+0.195 (n=201)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 11.443 (IC base=+0.094)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0912` → IC=+0.131 (n=648)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` < 2.0912 (IC base=+0.094)

- **PATRÓN** `libro_liquidez` > `2435.8886` → IC=+0.138 (n=374)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 2435.8886 (IC base=+0.094)

- **PATRÓN** `ibs_20min` < `0.0617` → IC=+0.274 (n=131)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0617 (IC base=+0.004)

- **PATRÓN** `volumen_pendiente_norm` > `0.07` → IC=+0.189 (n=101)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.07 (IC base=+0.004)

- **PATRÓN** `volumen_spike_ratio` < `2.5298` → IC=+0.125 (n=238)
...
- **PATRÓN** `ibs_20min` < `0.0889` → IC=+0.283 (n=58)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0889 (IC base=+0.053)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.71` → IC=+0.146 (n=111)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` < 4.71 (IC base=+0.053)

- **PATRÓN** `volumen_regimen` < `0.6103` → IC=+0.152 (n=44)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.6103 (IC base=+0.053)

- **PATRÓN** `volumen_pendiente_norm` > `0.0712` → IC=+0.214 (n=47)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0712 (IC base=+0.053)

- **PATRÓN** `volumen_spike_ratio` < `2.0359` → IC=+0.173 (n=96)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.0359 (IC base=+0.053)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.137 (n=298)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 7.0 (IC base=+0.106)

- **PATRÓN** `ibs_20min` > `0.6741` → IC=+0.235 (n=258)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6741 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` > `0.348` → IC=+0.178 (n=116)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.348 (IC base=+0.106)
...
- **PATRÓN** `volumen_regimen` > `0.9419` → IC=+0.132 (n=131)
  ...
  - _Acción_: Kelly boost +0.66€ cuando `volumen_regimen` > 0.9419 (IC base=+0.106)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7281` → IC=+0.173 (n=157)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.7281 (IC base=+0.106)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.124 (n=192)

  - _Acción_: Kelly boost +0.62€ cuando `libro_spread` < 0.01 (IC base=+0.106)

- **PATRÓN** `libro_liquidez` > `1133.2011` → IC=+0.159 (n=253)
...
- **PATRÓN** `sigma_h` > `0.0058` → IC=+0.150 (n=118)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0058 (IC base=+0.089)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.132 (n=123)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 15.0 (IC base=+0.089)

- **PATRÓN** `ibs_20min` > `0.6417` → IC=+0.160 (n=260)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.6417 (IC base=+0.089)

- **PATRÓN** `dist_vwap_pct` > `0.4967` → IC=+0.188 (n=62)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.4967 (IC base=+0.089)
...
- **FILTRO** `volumen_regimen` < `0.7924` → IC=-0.167 (n=28)
...
- **PATRÓN** `sigma_h` < `0.0022` → IC=+0.281 (n=30)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0022 (IC base=+0.043)

- **PATRÓN** `drift_60min` |x|≤ `0.1365` → IC=+0.125 (n=46)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 0.1365 (IC base=+0.043)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.192 (n=24)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 17.0 (IC base=+0.043)

- **PATRÓN** `ibs_20min` > `0.5842` → IC=+0.157 (n=68)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` > 0.5842 (IC base=+0.043)

- **PATRÓN** `libro_liquidez` > `1532.5809` → IC=+0.135 (n=61)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 1532.5809 (IC base=+0.043)
...
- **PATRÓN** `libro_liquidez` > `2924.9554` → IC=+0.169 (n=240)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 2924.9554 (IC base=+0.108)
...
- **PATRÓN** `liq_usd_total` > `69212.62` → IC=+0.160 (n=98)
  ...
  - _Acción_: Kelly boost +0.80€ cuando `liq_usd_total` > 69212.62 (IC base=+0.023)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.125 (n=62)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=756)
...
- **PATRÓN** `py_entrada` < `0.495` → IC=+0.147 (n=66)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `py_entrada` < 0.495 (IC base=+0.013)

- **PATRÓN** `libro_liquidez` > `3951.8303` → IC=+0.167 (n=49)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 3951.8303 (IC base=+0.013)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.130 (n=417)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=217)

- **PATRÓN** `py_entrada` < `0.47` → IC=+0.183 (n=181)

  - _Acción_: Kelly boost +0.92€ cuando `py_entrada` < 0.47 (IC base=+0.019)
...
- **PATRÓN** `py_entrada` < `0.53` → IC=+0.173 (n=47)

  - _Acción_: Kelly boost +0.87€ cuando `py_entrada` < 0.53 (IC base=+0.047)

### LIQUIDACIONES_DEPTH_FASE0#BTC#5min
- **PATRÓN** `py_entrada` < `0.56` → IC=+0.179 (n=54)

  - _Acción_: Kelly boost +0.89€ cuando `py_entrada` < 0.56 (IC base=+0.074)

- **PATRÓN** `restante_min` > `3.98` → IC=+0.122 (n=43)

  - _Acción_: Kelly boost +0.61€ cuando `restante_min` > 3.98 (IC base=+0.074)

- **PATRÓN** `profundidad_ratio` > `91.8` → IC=+0.122 (n=72)

  - _Acción_: Kelly boost +0.61€ cuando `profundidad_ratio` > 91.8 (IC base=+0.074)
...
- **FILTRO** `py_entrada` > `0.519` → IC=-0.145 (n=29)
...
- **FILTRO** `py_entrada` > `0.49` → IC=-0.122 (n=43)

  - _Acción_: SKIP cuando `py_entrada` > 0.49
  - _Potencial_: sin este filtro IC_bueno=+0.088 (n=15)

- **FILTRO** `restante_min` < `10.69` → IC=-0.214 (n=19)

  - _Acción_: SKIP cuando `restante_min` < 10.69
  - _Potencial_: sin este filtro IC_bueno=+0.012 (n=39)

- **FILTRO** `lag_apertura_s` > `258.82` → IC=-0.214 (n=19)

  - _Acción_: SKIP cuando `lag_apertura_s` > 258.82
  - _Potencial_: sin este filtro IC_bueno=+0.012 (n=39)
...
- **FILTRO** `profundidad_ratio` < `48.0` → IC=-0.167 (n=34)

  - _Acción_: SKIP cuando `profundidad_ratio` < 48.0
  - _Potencial_: sin este filtro IC_bueno=+0.194 (n=34)

- **PATRÓN** `py_entrada` < `0.45` → IC=+0.250 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.45 (IC base=+0.014)

- **PATRÓN** `profundidad_ratio` > `48.0` → IC=+0.194 (n=34)

  - _Acción_: Kelly boost +0.97€ cuando `profundidad_ratio` > 48.0 (IC base=+0.014)

### LIQUIDACIONES_DEPTH_FASE0#SOL#15min
- **FILTRO** `restante_min` < `13.48` → IC=-0.157 (n=33)

  - _Acción_: SKIP cuando `restante_min` < 13.48
  - _Potencial_: sin este filtro IC_bueno=+0.167 (n=16)

- **FILTRO** `lag_apertura_s` > `90.91` → IC=-0.158 (n=36)

  - _Acción_: SKIP cuando `lag_apertura_s` > 90.91
  - _Potencial_: sin este filtro IC_bueno=+0.233 (n=13)

- **PATRÓN** `restante_min` > `13.48` → IC=+0.167 (n=16)

  - _Acción_: Kelly boost +0.83€ cuando `restante_min` > 13.48 (IC base=-0.049)

- **PATRÓN** `py_entrada` < `0.59` → IC=+0.159 (n=39)

  - _Acción_: Kelly boost +0.79€ cuando `py_entrada` < 0.59 (IC base=+0.033)

- **PATRÓN** `restante_min` > `13.49` → IC=+0.208 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `restante_min` > 13.49 (IC base=+0.033)

- **PATRÓN** `lag_apertura_s` < `90.69` → IC=+0.227 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `lag_apertura_s` < 90.69 (IC base=+0.033)

### LIQUIDACIONES_DEPTH_FASE0#SOL#5min
- **PATRÓN** `py_entrada` < `0.49` → IC=+0.136 (n=20)

  - _Acción_: Kelly boost +0.68€ cuando `py_entrada` < 0.49 (IC base=-0.025)
...
- **FILTRO** `restante_min` > `10.52` → IC=-0.167 (n=52)
...
- **PATRÓN** `restante_min` > `3.85` → IC=+0.132 (n=36)

  - _Acción_: Kelly boost +0.66€ cuando `restante_min` > 3.85 (IC base=+0.021)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=7562)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.166 (n=3579)

  - _Acción_: SKIP cuando `py_entrada` < 0.475
  - _Potencial_: sin este filtro IC_bueno=+0.058 (n=10811)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.168 (n=3700)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.036 (n=11186)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.46` → IC=-0.208 (n=614)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.100 (n=1886)

- **FILTRO** `py_entrada` > `0.64` → IC=-0.151 (n=652)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.192 (n=627)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=1922)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.202 (n=646)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.063 (n=2019)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.485` → IC=-0.174 (n=609)

  - _Acción_: SKIP cuando `py_entrada` < 0.485
  - _Potencial_: sin este filtro IC_bueno=+0.085 (n=1880)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.176 (n=649)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=2020)
...
- **FILTRO** `py_entrada` > `0.595` → IC=-0.212 (n=709)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=2270)

- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)

  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=2958)
...
- **FILTRO** `py_entrada` > `0.625` → IC=-0.279 (n=66)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.075 (n=245)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.129 (n=10193)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=22701)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.273 (n=8064)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=24830)

- **FILTRO** `ibs_7min` < `0.2821` → IC=-0.234 (n=8218)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2821
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=24676)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.156 (n=11165)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=21729)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.231 (n=10129)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=31188)

- **FILTRO** `ibs_7min` > `0.2928` → IC=-0.178 (n=10329)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2928
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=30988)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.136 (n=1664)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3782)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.310 (n=1296)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=4150)

- **FILTRO** `ibs_7min` < `0.7113` → IC=-0.249 (n=1797)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7113
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=3649)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.186 (n=1231)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4215)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.261 (n=1745)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=5361)

- **FILTRO** `drift_7min_pct` |x|> `0.1119` → IC=-0.126 (n=2415)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1119
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=4691)

- **FILTRO** `ibs_7min` > `0.7881` → IC=-0.207 (n=1776)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7881
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=5330)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.137 (n=1334)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=4342)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.251 (n=1373)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=4303)

- **FILTRO** `ibs_7min` < `0.7468` → IC=-0.192 (n=1417)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7468
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4259)

- **FILTRO** `ballena_activa_n` > `161.0` → IC=-0.177 (n=1415)

  - _Acción_: SKIP cuando `ballena_activa_n` > 161.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4261)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.255 (n=1440)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=4326)

- **FILTRO** `ibs_7min` > `0.2609` → IC=-0.179 (n=1439)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2609
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=4327)

- **FILTRO** `ballena_activa_n` > `154.0` → IC=-0.183 (n=1436)

  - _Acción_: SKIP cuando `ballena_activa_n` > 154.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=4330)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.165 (n=1470)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.082 (n=3702)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.304 (n=1282)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=3890)

- **FILTRO** `ibs_7min` < `0.7062` → IC=-0.243 (n=1705)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7062
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=3467)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.221 (n=1185)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3987)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.237 (n=1739)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=5859)

- **FILTRO** `ibs_7min` > `0.7467` → IC=-0.173 (n=1899)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7467
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=5699)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=1755)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=3677)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.241 (n=1337)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=4095)

- **FILTRO** `ibs_7min` < `0.7409` → IC=-0.179 (n=1358)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7409
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=4074)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.172 (n=1327)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=4105)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.261 (n=1384)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=4154)

- **FILTRO** `ibs_7min` > `0.2745` → IC=-0.179 (n=1383)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2745
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=4155)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.184 (n=1361)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4177)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.253 (n=1408)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=4324)

- **FILTRO** `ibs_7min` < `0.3` → IC=-0.233 (n=1423)

  - _Acción_: SKIP cuando `ibs_7min` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4309)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.178 (n=1867)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=5990)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.253 (n=1767)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3669)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.226 (n=1354)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=4082)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.214 (n=1315)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=4121)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.206 (n=1778)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=5674)
...
- **PATRÓN** `streak_estiramiento` < `0.5654` → IC=+0.162 (n=128)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `streak_estiramiento` < 0.5654 (IC base=+0.036)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.128 (n=41)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=1112)

### STREAK_MOM_5M#XRP#5min
- **FILTRO** `py_entrada` < `0.5` → IC=-0.121 (n=27)

  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.029 (n=771)
...
- **FILTRO** `py_entrada` < `0.495` → IC=-0.147 (n=32)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.008 (n=1505)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.154 (n=24)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.008 (n=1513)

### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.219 (n=746)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1263` → IC=+0.235 (n=580)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1263 (IC base=+0.187)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.197 (n=1524)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1187` → IC=+0.186 (n=843)
...
- **PATRÓN** `libro_liquidez` > `2966.2693` → IC=+0.196 (n=1096)
...
- **FILTRO** `sigma_ewma_delta_pct` > `24.513` → IC=-0.155 (n=27)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=387)

### UPDOWN_GBM#ETH#15min
- **PATRÓN** `sigma_h` < `0.0065` → IC=+0.141 (n=382)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0065 (IC base=+0.136)

- **PATRÓN** `sigma_h` > `0.0051` → IC=+0.158 (n=255)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0051 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.0674` → IC=+0.159 (n=168)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.0674 (IC base=+0.136)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2337` → IC=+0.169 (n=128)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.85€ cuando `delta_ratio_macro` |x|> 0.2337 (IC base=+0.136)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2579` → IC=+0.162 (n=273)

  - _Acción_: Kelly boost +0.81€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.2579 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.157 (n=281)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 11.0 (IC base=+0.136)

- **PATRÓN** `ibs_15` > `0.659` → IC=+0.256 (n=342)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.659 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `0.1631` → IC=+0.156 (n=300)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.1631 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.19` → IC=+0.207 (n=165)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.19 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `3410.3513` → IC=+0.145 (n=342)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 3410.3513 (IC base=+0.136)
...
- **FILTRO** `sigma_h` > `0.0129` → IC=-0.222 (n=667)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=2005)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.195 (n=907)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1199` → IC=+0.251 (n=1203)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1199 (IC base=-0.031)

- **PATRÓN** `ibs_15` < `0.3455` → IC=+0.280 (n=1804)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3455 (IC base=-0.031)

- **PATRÓN** `dist_vwap_pct` > `0.7029` → IC=+0.305 (n=295)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7029 (IC base=-0.031)
...
- **FILTRO** `sigma_h` < `0.0034` → IC=-0.234 (n=407)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.187 (n=1222)

- **FILTRO** `hora_utc` > `7.0` → IC=-0.210 (n=1027)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.179 (n=602)

- **FILTRO** `sigma_ewma_delta_pct` > `19.79` → IC=-0.252 (n=292)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 19.79
  - _Potencial_: sin este filtro IC_bueno=-0.187 (n=1337)
...
- **PATRÓN** `sigma_h` < `0.0044` → IC=+0.273 (n=236)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0044 (IC base=+0.233)

- **PATRÓN** `drift_60min` |x|≤ `0.4443` → IC=+0.233 (n=706)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4443 (IC base=+0.233)

- **PATRÓN** `drift_15min` |x|≤ `0.48` → IC=+0.238 (n=311)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.48 (IC base=+0.233)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2036` → IC=+0.264 (n=320)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.2036 (IC base=+0.233)
...
- **PATRÓN** `ibs_15` < `0.3452` → IC=+0.273 (n=706)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3452 (IC base=+0.233)

- **PATRÓN** `dist_vwap_pct` > `0.7773` → IC=+0.318 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7773 (IC base=+0.233)

- **PATRÓN** `sigma_ewma_delta_pct` > `16.718` → IC=+0.266 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 16.718 (IC base=+0.233)

- **PATRÓN** `sigma_ewma_delta_pct` < `12.182` → IC=+0.239 (n=749)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 12.182 (IC base=+0.233)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0764` → IC=+0.234 (n=280)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **PATRÓN** `dist_vwap_pct` > `0.4968` → IC=+0.213 (n=92)
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.659 sube el IC de +0.136 a +0.256 en UPDOWN_GBM#ETH#15min (n=342). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.6371 sube el IC de -0.064 a +0.278 en UPDOWN_GBM_15M_TARDIO (n=637). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3455 sube el IC de -0.031 a +0.280 en UPDOWN_GBM_15M_TARDIO (n=1804). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.3452 sube el IC de +0.233 a +0.273 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n