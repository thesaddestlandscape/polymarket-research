# Hipótesis automáticas — 2026-07-16 09:00 UTC
_Generado por shadow_postmortem.py sobre 16730 resoluciones (PNL=+3417.54€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.183 (n=809)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 8.0 (IC base=+0.183)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.192 (n=931)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.189 (n=1052)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.01 (IC base=+0.183)

- **PATRÓN** `libro_liquidez` > `4381.7761` → IC=+0.190 (n=601)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 4381.7761 (IC base=+0.183)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.177 (n=1110)

  - _Acción_: Kelly boost +0.89€ cuando `libro_spread` < 0.01 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `7360.9474` → IC=+0.187 (n=324)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 7360.9474 (IC base=+0.167)
...
- **PATRÓN** `py_entrada` < `0.425` → IC=+0.232 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.425 (IC base=+0.162)

- **PATRÓN** `libro_liquidez` > `7105.7782` → IC=+0.198 (n=51)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 7105.7782 (IC base=+0.162)
...
- **PATRÓN** `libro_liquidez` > `3759.4964` → IC=+0.215 (n=205)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3759.4964 (IC base=+0.205)
...
- **PATRÓN** `py_entrada` < `0.405` → IC=+0.204 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.405 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `4553.9223` → IC=+0.179 (n=54)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 4553.9223 (IC base=+0.144)

### FAVORITO_CONFIRMADO#SOL#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.227 (n=203)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.34` → IC=+0.283 (n=21)
...
- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.134 (n=1233)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` > 0.0091 (IC base=+0.107)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.127 (n=1239)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` < 11.0 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.2` → IC=+0.122 (n=141)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` > 6.2 (IC base=+0.107)
...
- **PATRÓN** `sigma_h` > `0.0275` → IC=+0.212 (n=158)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0275 (IC base=+0.164)
...
- **PATRÓN** `dist_vwap_pct` < `0.5138` → IC=+0.143 (n=183)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.5138 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.494` → IC=+0.141 (n=165)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.179 (n=751)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` > 0.0048 (IC base=+0.164)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.190 (n=259)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` < 5.0 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` < `0.4117` → IC=+0.198 (n=528)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.4117 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.188` → IC=+0.274 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.188 (IC base=+0.164)
...
- **PATRÓN** `sigma_h` < `0.0082` → IC=+0.186 (n=68)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.0082 (IC base=+0.103)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.145 (n=74)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` < 5.0 (IC base=+0.103)

- **PATRÓN** `dist_vwap_pct` < `0.2503` → IC=+0.172 (n=123)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.2503 (IC base=+0.103)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.13` → IC=+0.158 (n=115)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` < 3.13 (IC base=+0.103)
...
- **PATRÓN** `sigma_h` < `0.0095` → IC=+0.195 (n=80)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0095 (IC base=+0.195)

- **PATRÓN** `sigma_h` > `0.0269` → IC=+0.241 (n=79)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.4878` → IC=+0.183 (n=181)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` < 0.4878 (IC base=+0.195)
...
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#ETH#15min` — IC=+0.184 n=36. Faltan ~4 resoluciones para umbral n≥40. ETA: ~3h.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#ETH` — IC=+0.184 n=36. Faltan ~4 resoluciones para umbral n≥40. ETA: ~3h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2495 | +0.175 | -51.02€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1604 | +0.212 | +25.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 106 | +0.009 | -19.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 362 | +0.091 | -59.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 423 | +0.145 | +2.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 723 | +0.183 | -8.09€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 141 | +0.150 | +7.25€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#ETH | 887 | +0.174 | -3.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 531 | +0.215 | +16.90€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 35 | -0.095 | -12.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 180 | +0.110 | -21.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 141 | +0.164 | +12.82€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL | 877 | +0.170 | -35.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 528 | +0.215 | +17.91€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 36 | +0.079 | -1.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 172 | +0.092 | -34.37€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 141 | +0.115 | -18.04€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 4837 | +0.102 | +1506.93€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 4837 | +0.102 | +1506.93€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#SOL | 1294 | +0.110 | +523.61€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1294 | +0.110 | +523.61€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1224 | +0.157 | +622.64€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1224 | +0.157 | +622.64€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2066 | +0.150 | +1205.81€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2066 | +0.150 | +1205.81€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 557 | +0.112 | +243.46€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 557 | +0.112 | +243.46€ | 1 | 10 |
...
| ✅ GBM_LATE_15M_TARDIO | 2036 | +0.090 | +606.59€ | 0 | 6 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2036 | +0.090 | +606.59€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#SOL | 479 | +0.059 | +95.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 479 | +0.059 | +95.64€ | 3 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 582 | +0.199 | +413.56€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 582 | +0.199 | +413.56€ | 0 | 11 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 121 | +0.183 | +33.71€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 121 | +0.183 | +33.71€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 36 | +0.184 | +3.06€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 36 | +0.184 | +3.06€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.155: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.003 n=491 | neutral: IC=-0.020 n=373 | overbought(IBS>0.7): IC=+0.135 n=403
  - _Datos_: n=1341 IC=+0.043 PNL=+133.78€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9736 | tras_1loss IC=+0.068 n=6785 | tras_2loss IC=+0.036 n=2898/40 | gap=+0.066 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=14272 IC=+0.106 PNL=+3493.42€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=14272 IC=+0.106 PNL=+3493.42€
...
**🔶 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.167 n=199) pero sin cruzar ≥2 pares más — BTC: n=144 IC=-0.014; ETH: n=108 IC=+0.000; SOL: n=209 IC=+0.059 ✓
