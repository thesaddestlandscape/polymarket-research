# Hipótesis automáticas — 2026-08-31 04:59 UTC
_Generado por shadow_postmortem.py sobre 222958 resoluciones (PNL=+17384.55€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.12` → IC=-0.294 (n=3132)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.12
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=9396)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `146.35` → IC=-0.251 (n=412)
...
- **FILTRO** `restante_s_al_confirmar` < `109.78` → IC=-0.408 (n=401)
...
- **FILTRO** `restante_s_al_confirmar` < `123.2` → IC=-0.397 (n=550)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 123.2
  - _Potencial_: sin este filtro IC_bueno=-0.119 (n=1653)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.193 (n=6600)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.199 (n=774)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.191 (n=993)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.01 (IC base=+0.189)

- **PATRÓN** `libro_liquidez` > `13508.1892` → IC=+0.233 (n=256)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13508.1892 (IC base=+0.189)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.132 (n=1215)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.276 (n=575)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `1943.5814` → IC=+0.276 (n=551)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.193 (n=4552)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.174 (n=1318)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` < 17.0 (IC base=+0.165)

- **PATRÓN** `py_entrada` < `0.74` → IC=+0.178 (n=1410)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.176 (n=1146)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` < 15.0 (IC base=+0.168)

- **PATRÓN** `py_entrada` < `0.73` → IC=+0.170 (n=1292)
...
- **PATRÓN** `py_entrada` < `0.7` → IC=+0.196 (n=675)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.212 (n=14057)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.168 (n=1991)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.269 (n=1859)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.168 (n=1901)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 12.0 (IC base=+0.168)

- **PATRÓN** `py_entrada` > `0.74` → IC=+0.207 (n=1345)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.137 (n=1292)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` < 7.0 (IC base=+0.130)

- **PATRÓN** `lag_apertura_s` < `3.4` → IC=+0.168 (n=964)

  - _Acción_: Kelly boost +0.84€ cuando `lag_apertura_s` < 3.4 (IC base=+0.130)
...
- **PATRÓN** `dist_vwap_pct` > `0.2609` → IC=+0.227 (n=361)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2609 (IC base=+0.071)

- **PATRÓN** `dist_vwap_pct` < `0.4777` → IC=+0.212 (n=565)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4777 (IC base=+0.071)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.392` → IC=+0.155 (n=1122)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 5.392 (IC base=+0.071)

- **PATRÓN** `volumen_regimen` < `0.627` → IC=+0.218 (n=211)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.627 (IC base=+0.071)

- **PATRÓN** `volumen_regimen` > `1.0803` → IC=+0.243 (n=286)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0803 (IC base=+0.071)

- **PATRÓN** `volumen_pendiente_norm` > `0.2525` → IC=+0.163 (n=327)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` > 0.2525 (IC base=+0.071)

- **PATRÓN** `volumen_spike_ratio` < `2.4485` → IC=+0.165 (n=1447)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.4485 (IC base=+0.071)
...
- **PATRÓN** `volumen_spike_ratio` > `2.9552` → IC=+0.235 (n=429)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9552 (IC base=+0.029)

- **PATRÓN** `ballena_activa_n` < `42.0` → IC=+0.225 (n=459)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 42.0 (IC base=+0.029)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.267 (n=268)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3107` → IC=+0.354 (n=46)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3107 (IC base=+0.261)
...
- **PATRÓN** `libro_liquidez` > `1915.8922` → IC=+0.279 (n=134)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1915.8922 (IC base=+0.261)

- **PATRÓN** `ballena_activa_n` < `63.0` → IC=+0.257 (n=237)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.95` → IC=+0.246 (n=529)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6202` → IC=+0.240 (n=225)
...
- **PATRÓN** `ballena_activa_n` < `47.0` → IC=+0.232 (n=259)
...
- **FILTRO** `drift_60min` |x|> `0.5171` → IC=-0.174 (n=286)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=556)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.195 (n=198)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.081 (n=644)
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.136 (n=1662)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0076 (IC base=+0.055)

- **PATRÓN** `ibs_20min` > `0.2683` → IC=+0.121 (n=3666)
...
- **PATRÓN** `volumen_regimen` > `0.6774` → IC=+0.195 (n=1065)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` > 0.6774 (IC base=+0.055)

- **PATRÓN** `volumen_pendiente_norm` < `0.1162` → IC=+0.194 (n=1630)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` < 0.1162 (IC base=+0.055)

- **PATRÓN** `volumen_pendiente_norm` > `0.2555` → IC=+0.199 (n=314)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2555 (IC base=+0.055)

- **PATRÓN** `volumen_spike_ratio` < `1.4996` → IC=+0.215 (n=559)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4996 (IC base=+0.055)

- **PATRÓN** `volumen_spike_ratio` > `2.8814` → IC=+0.195 (n=558)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 2.8814 (IC base=+0.055)

- **PATRÓN** `ballena_activa_n` < `93.0` → IC=+0.284 (n=1125)
...
- **PATRÓN** `dist_vwap_pct` < `0.219` → IC=+0.214 (n=876)
...
- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.316 (n=324)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0083 (IC base=+0.313)

- **PATRÓN** `sigma_h` > `0.0058` → IC=+0.334 (n=329)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0058 (IC base=+0.313)

- **PATRÓN** `drift_60min` |x|≤ `0.2365` → IC=+0.315 (n=247)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2365 (IC base=+0.313)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.342 (n=245)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 12.0 (IC base=+0.313)

- **PATRÓN** `ibs_20min` < `0.3333` → IC=+0.332 (n=368)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3333 (IC base=+0.313)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.734` → IC=+0.320 (n=393)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.734 (IC base=+0.313)

- **PATRÓN** `volumen_pendiente_norm` > `0.3666` → IC=+0.351 (n=45)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3666 (IC base=+0.313)

- **PATRÓN** `volumen_spike_ratio` > `2.4592` → IC=+0.344 (n=184)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4592 (IC base=+0.313)
...
- **PATRÓN** `ballena_activa_n` < `27.0` → IC=+0.309 (n=82)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 27.0 (IC base=+0.313)
...
- **FILTRO** `ibs_20min` < `0.7593` → IC=-0.143 (n=528)
...
- **PATRÓN** `ibs_20min` > `0.8846` → IC=+0.293 (n=201)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8846 (IC base=-0.013)

- **PATRÓN** `dist_vwap_pct` > `1.2464` → IC=+0.328 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2464 (IC base=-0.013)
...
- **PATRÓN** `volumen_regimen` > `1.1437` → IC=+0.275 (n=78)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1437 (IC base=-0.013)

- **PATRÓN** `volumen_pendiente_norm` < `0.0898` → IC=+0.217 (n=217)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1847` → IC=+0.293 (n=235)
...
- **PATRÓN** `dist_vwap_pct` > `1.1138` → IC=+0.257 (n=520)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1138 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.532` → IC=+0.241 (n=1259)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.532 (IC base=+0.162)

- **PATRÓN** `volumen_regimen` > `0.6232` → IC=+0.172 (n=2121)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3394` → IC=+0.162 (n=2482)
...
- **PATRÓN** `libro_liquidez` > `3961.3086` → IC=+0.188 (n=1027)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3961.3086 (IC base=+0.162)

- **PATRÓN** `ballena_activa_n` < `158.0` → IC=+0.180 (n=1996)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 158.0 (IC base=+0.162)

- **PATRÓN** `sigma_h` < `0.006` → IC=+0.192 (n=1898)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.006 (IC base=+0.180)

- **PATRÓN** `drift_60min` |x|≤ `0.0783` → IC=+0.216 (n=949)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_regimen` < `1.1669` → IC=+0.169 (n=2200)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.1669 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` > `0.8458` → IC=+0.162 (n=1467)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.8458 (IC base=+0.180)
...
- **PATRÓN** `volumen_spike_ratio` < `1.9186` → IC=+0.171 (n=1485)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.9186 (IC base=+0.180)

- **PATRÓN** `volumen_spike_ratio` > `2.7141` → IC=+0.211 (n=743)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7141 (IC base=+0.180)

- **PATRÓN** `ballena_activa_n` < `105.0` → IC=+0.181 (n=1591)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 105.0 (IC base=+0.180)
...
- **PATRÓN** `sigma_h` > `0.0044` → IC=+0.273 (n=297)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0044 (IC base=+0.249)

- **PATRÓN** `drift_60min` |x|≤ `0.2619` → IC=+0.288 (n=262)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.268 (n=300)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.249)

- **PATRÓN** `ibs_20min` < `0.163` → IC=+0.291 (n=199)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.163 (IC base=+0.249)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.593` → IC=+0.267 (n=328)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.593 (IC base=+0.249)

- **PATRÓN** `volumen_pendiente_norm` < `0.0887` → IC=+0.238 (n=208)
...
- **PATRÓN** `volumen_spike_ratio` < `1.9498` → IC=+0.256 (n=154)
...
- **PATRÓN** `libro_liquidez` > `1914.09` → IC=+0.302 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1914.09 (IC base=+0.249)

- **PATRÓN** `ballena_activa_n` < `77.0` → IC=+0.245 (n=190)
...
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.165 (n=544)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0058 (IC base=+0.152)

- **PATRÓN** `drift_60min` |x|≤ `0.2236` → IC=+0.178 (n=479)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4432` → IC=+0.198 (n=544)

  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` < 0.4432 (IC base=+0.152)

- **PATRÓN** `dist_vwap_pct` > `0.5776` → IC=+0.160 (n=92)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.5776 (IC base=+0.152)

- **PATRÓN** `dist_vwap_pct` < `0.1515` → IC=+0.175 (n=540)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` < 0.1515 (IC base=+0.152)
...
- **PATRÓN** `volumen_regimen` < `0.6862` → IC=+0.223 (n=240)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6862 (IC base=+0.152)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5146` → IC=+0.164 (n=436)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.5146 (IC base=+0.152)

- **PATRÓN** `volumen_spike_ratio` > `1.3943` → IC=+0.153 (n=436)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.3943 (IC base=+0.152)

- **PATRÓN** `libro_liquidez` > `15322.8605` → IC=+0.179 (n=182)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 15322.8605 (IC base=+0.152)
...
- **PATRÓN** `sigma_h` < `0.01` → IC=+0.253 (n=350)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.3` → IC=+0.237 (n=386)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.3 (IC base=+0.237)
...
- **PATRÓN** `volumen_spike_ratio` > `3.139` → IC=+0.264 (n=125)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.139 (IC base=+0.237)

- **PATRÓN** `libro_liquidez` > `1866.5484` → IC=+0.256 (n=117)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1866.5484 (IC base=+0.237)

- **PATRÓN** `ballena_activa_n` < `59.0` → IC=+0.199 (n=234)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3745` → IC=+0.156 (n=530)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.158 (n=197)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` < 5.0 (IC base=+0.138)

- **PATRÓN** `ibs_20min` < `0.3871` → IC=+0.207 (n=466)
...
- **PATRÓN** `volumen_regimen` < `1.1598` → IC=+0.150 (n=530)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.1598 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `0.5948` → IC=+0.145 (n=530)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 0.5948 (IC base=+0.138)

- **PATRÓN** `volumen_pendiente_norm` > `0.1051` → IC=+0.172 (n=169)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.1051 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` < `1.9178` → IC=+0.166 (n=282)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.9178 (IC base=+0.138)
...
- **PATRÓN** `ballena_activa_n` < `143.0` → IC=+0.140 (n=231)

  - _Acción_: Kelly boost +0.70€ cuando `ballena_activa_n` < 143.0 (IC base=+0.138)
...
- **PATRÓN** `dist_vwap_pct` < `0.1783` → IC=+0.141 (n=466)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.1783 (IC base=+0.127)
...
- **PATRÓN** `volumen_regimen` > `0.6905` → IC=+0.239 (n=553)
...
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.161 (n=272)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` > 0.0078 (IC base=+0.126)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.201 (n=279)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 16.0 (IC base=+0.126)

- **PATRÓN** `ibs_20min` > `0.6191` → IC=+0.183 (n=535)

  - _Acción_: Kelly boost +0.92€ cuando `ibs_20min` > 0.6191 (IC base=+0.126)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.287` → IC=+0.185 (n=163)
  ...
  - _Acción_: Kelly boost +0.92€ cuando `sigma_ewma_delta_pct` > 8.287 (IC base=+0.126)

- **PATRÓN** `volumen_regimen` < `0.8887` → IC=+0.136 (n=322)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 0.8887 (IC base=+0.126)

- **PATRÓN** `volumen_regimen` > `0.6979` → IC=+0.142 (n=431)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 0.6979 (IC base=+0.126)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3726` → IC=+0.127 (n=480)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_spike_ratio` < 2.3726 (IC base=+0.126)

- **PATRÓN** `volumen_spike_ratio` > `1.9259` → IC=+0.139 (n=364)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` > 1.9259 (IC base=+0.126)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.133 (n=622)

  - _Acción_: Kelly boost +0.67€ cuando `libro_spread` < 0.02 (IC base=+0.126)

- **PATRÓN** `libro_liquidez` > `3054.5848` → IC=+0.146 (n=272)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 3054.5848 (IC base=+0.126)
...
- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.294 (n=100)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0064 (IC base=+0.296)

- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.308 (n=76)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0048 (IC base=+0.296)

- **PATRÓN** `drift_60min` |x|≤ `0.2347` → IC=+0.295 (n=76)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2347 (IC base=+0.296)
...
- **PATRÓN** `ibs_20min` > `0.8368` → IC=+0.344 (n=75)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8368 (IC base=+0.296)

- **PATRÓN** `dist_vwap_pct` > `0.1466` → IC=+0.375 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1466 (IC base=+0.296)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.648` → IC=+0.375 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.648 (IC base=+0.296)

- **PATRÓN** `volumen_regimen` < `0.8708` → IC=+0.323 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8708 (IC base=+0.296)

- **PATRÓN** `volumen_regimen` > `1.074` → IC=+0.311 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.074 (IC base=+0.296)

- **PATRÓN** `volumen_pendiente_norm` > `0.1796` → IC=+0.407 (n=41)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1796 (IC base=+0.296)

- **PATRÓN** `volumen_spike_ratio` > `1.8383` → IC=+0.338 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8383 (IC base=+0.296)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.197 (n=1642)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.398` → IC=+0.232 (n=1860)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.398 (IC base=+0.159)

- **PATRÓN** `volumen_regimen` < `0.699` → IC=+0.156 (n=1118)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.699 (IC base=+0.159)

- **PATRÓN** `volumen_regimen` > `1.0814` → IC=+0.157 (n=1152)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 1.0814 (IC base=+0.159)

- **PATRÓN** `volumen_pendiente_norm` > `0.1037` → IC=+0.183 (n=1265)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.1037 (IC base=+0.159)

- **PATRÓN** `volumen_spike_ratio` > `1.8815` → IC=+0.166 (n=2200)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.543` → IC=+0.190 (n=3304)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3461` → IC=+0.204 (n=1209)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3461 (IC base=+0.186)

- **PATRÓN** `ballena_activa_n` < `37.0` → IC=+0.180 (n=997)
...
- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.269 (n=417)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.290 (n=289)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 11.0 (IC base=+0.268)

- **PATRÓN** `ibs_20min` < `0.5212` → IC=+0.304 (n=417)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5212 (IC base=+0.268)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.231` → IC=+0.284 (n=448)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3034` → IC=+0.324 (n=49)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3034 (IC base=+0.268)

- **PATRÓN** `volumen_spike_ratio` > `1.624` → IC=+0.291 (n=281)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.624 (IC base=+0.268)
...
- **PATRÓN** `libro_liquidez` > `1924.5368` → IC=+0.280 (n=139)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1924.5368 (IC base=+0.268)

- **PATRÓN** `ballena_activa_n` < `69.0` → IC=+0.256 (n=223)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 69.0 (IC base=+0.268)
...
- **PATRÓN** `sigma_h` < `0.0059` → IC=+0.168 (n=546)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.0774` → IC=+0.190 (n=182)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.0774 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.168 (n=510)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 7.0 (IC base=+0.165)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.178 (n=563)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` < 18.0 (IC base=+0.165)

- **PATRÓN** `ibs_20min` < `0.5817` → IC=+0.203 (n=546)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5817 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` < `0.3254` → IC=+0.178 (n=539)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.3254 (IC base=+0.165)
...
- **PATRÓN** `volumen_spike_ratio` < `1.395` → IC=+0.204 (n=150)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.395 (IC base=+0.165)

- **PATRÓN** `libro_liquidez` > `14791.3144` → IC=+0.169 (n=182)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 14791.3144 (IC base=+0.165)

- **PATRÓN** `ballena_activa_n` < `320.0` → IC=+0.167 (n=154)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 320.0 (IC base=+0.165)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7408` → IC=+0.254 (n=128)
...
- **PATRÓN** `libro_liquidez` > `1874.7032` → IC=+0.243 (n=169)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1874.7032 (IC base=+0.236)

- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.203 (n=304)
...
- **PATRÓN** `sigma_h` > `0.0079` → IC=+0.143 (n=427)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` > 0.0079 (IC base=+0.091)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.138 (n=434)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 12.0 (IC base=+0.091)

- **PATRÓN** `ibs_20min` > `0.4762` → IC=+0.179 (n=640)
...
- **PATRÓN** `ballena_activa_n` < `65.0` → IC=+0.147 (n=414)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.223 (n=836)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.217)

- **PATRÓN** `ibs_20min` < `0.4921` → IC=+0.273 (n=792)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4921 (IC base=+0.217)

- **PATRÓN** `dist_vwap_pct` < `0.1729` → IC=+0.230 (n=713)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1729 (IC base=+0.217)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8631` → IC=+0.217 (n=411)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8631 (IC base=+0.217)

- **PATRÓN** `ballena_activa_n` < `36.0` → IC=+0.185 (n=509)
...
- **FILTRO** `py_entrada` < `0.45` → IC=-0.186 (n=1083)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.207 (n=1096)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=3605)

- **FILTRO** `ibs_20min` > `0.2727` → IC=-0.164 (n=1171)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2727
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=3530)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.123 (n=210)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=495)

- **FILTRO** `py_entrada` < `0.5` → IC=-0.172 (n=306)

  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.041 (n=399)

- **FILTRO** `ibs_20min` < `0.7222` → IC=-0.180 (n=176)
...
- **FILTRO** `py_entrada` < `0.495` → IC=-0.142 (n=213)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.043 (n=560)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.204 (n=184)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.044 (n=560)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.145 (n=3339)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=8023)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.285 (n=2710)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=8652)

- **FILTRO** `ibs_7min` < `0.72` → IC=-0.236 (n=2832)

  - _Acción_: SKIP cuando `ibs_7min` < 0.72
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=8530)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.170 (n=3842)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=7520)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.227 (n=3292)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=10896)

- **FILTRO** `ibs_7min` > `0.73` → IC=-0.167 (n=3545)

  - _Acción_: SKIP cuando `ibs_7min` > 0.73
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=10643)
...
- **FILTRO** `py_entrada` > `0.71` → IC=-0.234 (n=603)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=1870)

- **FILTRO** `drift_7min_pct` |x|> `0.1193` → IC=-0.138 (n=840)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1193
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=1633)

- **FILTRO** `ibs_7min` > `0.2963` → IC=-0.170 (n=840)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2963
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=1633)
...
- **FILTRO** `ballena_activa_n` > `100.0` → IC=-0.177 (n=737)

  - _Acción_: SKIP cuando `ballena_activa_n` > 100.0
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=1459)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.242 (n=557)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=1925)

- **FILTRO** `ibs_7min` > `0.8077` → IC=-0.172 (n=620)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8077
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=1862)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.150 (n=596)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.253 (n=464)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=1467)

- **FILTRO** `ibs_7min` < `0.7575` → IC=-0.182 (n=482)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7575
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=1449)
...
- **FILTRO** `py_entrada` > `0.66` → IC=-0.258 (n=485)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=1459)

- **FILTRO** `ibs_7min` > `0.192` → IC=-0.172 (n=660)
...
- **FILTRO** `ballena_activa_n` > `34.0` → IC=-0.193 (n=484)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=1460)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.190 (n=627)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=2003)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.136 (n=526)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=1313)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.306 (n=446)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=1393)

- **FILTRO** `ibs_7min` < `0.7391` → IC=-0.240 (n=459)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7391
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=1380)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.232 (n=449)

  - _Acción_: SKIP cuando `ballena_activa_n` > 13.0
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=1390)

- **FILTRO** `libro_liquidez` < `2709.2388` → IC=-0.140 (n=1213)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.261 (n=508)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=1955)

- **FILTRO** `ibs_7min` > `0.8` → IC=-0.159 (n=613)
...
- **PATRÓN** `delta_ratio` |x|> `0.4189` → IC=+0.140 (n=284)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.70€ cuando `delta_ratio` |x|> 0.4189 (IC base=+0.119)
...
- **PATRÓN** `total_vol_5m` < `456.268` → IC=+0.164 (n=135)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `total_vol_5m` < 456.268 (IC base=+0.119)

- **PATRÓN** `libro_liquidez` > `3219.7168` → IC=+0.148 (n=160)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 3219.7168 (IC base=+0.119)
...
- **PATRÓN** `delta_ratio` |x|> `0.4128` → IC=+0.196 (n=44)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.98€ cuando `delta_ratio` |x|> 0.4128 (IC base=+0.111)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.133 (n=47)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` < 10.0 (IC base=+0.111)

- **PATRÓN** `total_vol_5m` < `684.4028` → IC=+0.205 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `total_vol_5m` < 684.4028 (IC base=+0.111)

- **PATRÓN** `libro_liquidez` > `8047.0899` → IC=+0.139 (n=59)

  - _Acción_: Kelly boost +0.70€ cuando `libro_liquidez` > 8047.0899 (IC base=+0.111)

- **PATRÓN** `ballena_activa_n` < `152.0` → IC=+0.142 (n=65)

  - _Acción_: Kelly boost +0.71€ cuando `ballena_activa_n` < 152.0 (IC base=+0.111)
...
- **PATRÓN** `ibs_15` < `0.1379` → IC=+0.202 (n=122)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.1379 (IC base=+0.036)
...
- **FILTRO** `sigma_h` > `0.0104` → IC=-0.199 (n=287)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.135 (n=302)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=850)

- **FILTRO** `sigma_ewma_delta_pct` > `17.791` → IC=-0.172 (n=404)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 17.791
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=3108)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3537` → IC=+0.141 (n=154)

  - _Acción_: Kelly boost +0.71€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3537 (IC base=-0.073)

- **PATRÓN** `ibs_15` > `0.5433` → IC=+0.191 (n=283)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.96€ cuando `ibs_15` > 0.5433 (IC base=-0.073)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0945` → IC=+0.232 (n=252)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0945 (IC base=-0.077)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1056` → IC=+0.245 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1056 (IC base=-0.077)
...
- **PATRÓN** `dist_vwap_pct` > `1.0043` → IC=+0.241 (n=25)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0043 (IC base=-0.077)

- **PATRÓN** `dist_vwap_pct` < `0.1597` → IC=+0.236 (n=278)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1597 (IC base=-0.077)

- **PATRÓN** `ballena_activa_n` < `58.0` → IC=+0.231 (n=165)
...
- **FILTRO** `drift_15min` |x|> `0.7564` → IC=-0.218 (n=186)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.213 (n=562)

- **FILTRO** `sigma_ewma_delta_pct` > `19.454` → IC=-0.271 (n=138)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 19.454
  - _Potencial_: sin este filtro IC_bueno=-0.201 (n=610)

- **FILTRO** `libro_liquidez` < `15699.4792` → IC=-0.219 (n=493)
...
- **FILTRO** `sigma_ewma_delta_pct` > `16.013` → IC=-0.147 (n=137)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=1101)

- **PATRÓN** `ibs_15` > `0.8125` → IC=+0.206 (n=15)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8125 (IC base=-0.125)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0761` → IC=+0.161 (n=57)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.81€ cuando `delta_ratio_macro` |x|> 0.0761 (IC base=-0.057)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1634` → IC=+0.167 (n=40)

  - _Acción_: Kelly boost +0.83€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1634 (IC base=-0.057)

- **PATRÓN** `ibs_15` < `0.3659` → IC=+0.212 (n=64)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3659 (IC base=-0.057)

- **PATRÓN** `dist_vwap_pct` < `0.2814` → IC=+0.188 (n=62)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.2814 (IC base=-0.057)

- **PATRÓN** `ballena_activa_n` < `39.0` → IC=+0.167 (n=43)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 39.0 (IC base=-0.057)

### UPDOWN_GBM_15M_TARDIO#XRP#15min
- **FILTRO** `sigma_h` > `0.0216` → IC=-0.276 (n=105)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `sigma_ewma_delta_pct` > `15.666` → IC=-0.200 (n=38)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.162 (n=273)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.293 (n=27)

  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.154 (n=284)
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1379 sube el IC de +0.036 a +0.202 en UPDOWN_GBM#XRP#15min (n=122). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.5433 sube el IC de -0.073 a +0.191 en UPDOWN_GBM_15M_TARDIO (n=283). Ya aplicado como kelly_boost=+0.96€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3667 sube el IC de -0.077 a +0.293 en UPDOWN_GBM_15M_TARDIO (n=283). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#SOL#15min**: dentro de BUY_NO, IBS < 0.3659 sube el IC de -0.057 a +0.212 en UPDOWN_GBM_15M_TARDIO#SOL#15min (n=64). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 12528 | -0.106 | -2158.69€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 857 | -0.011 | -122.43€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 11671 | -0.113 | -2036.26€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1651 | -0.070 | -344.24€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1651 | -0.070 | -344.24€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 857 | -0.011 | -122.43€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 857 | -0.011 | -122.43€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 1606 | -0.154 | -502.38€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 1606 | -0.154 | -502.38€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 3352 | -0.062 | -299.10€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 3352 | -0.062 | -299.10€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 2859 | -0.113 | -305.53€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2859 | -0.113 | -305.53€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 2203 | -0.189 | -585.02€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 2203 | -0.189 | -585.02€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 2163 | -0.054 | +953.43€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 607 | -0.004 | +368.37€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 1556 | -0.074 | +585.05€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 2163 | -0.054 | +953.43€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 607 | -0.004 | +368.37€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 1556 | -0.074 | +585.05€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 40036 | +0.114 | -2507.70€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 7013 | +0.187 | -232.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 121 | -0.085 | -49.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 29445 | +0.097 | -2179.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3457 | +0.119 | -47.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 4934 | +0.075 | -723.37€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 22 | -0.083 | +0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 4907 | +0.076 | -717.90€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 8140 | +0.134 | -169.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 2013 | +0.201 | -76.94€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 4899 | +0.109 | -122.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1186 | +0.128 | +52.34€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 4948 | +0.083 | -591.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 24 | +0.077 | +2.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 4922 | +0.083 | -591.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 8735 | +0.127 | -133.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2566 | +0.170 | -12.25€ | 1 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 4908 | +0.112 | -84.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1249 | +0.099 | -28.38€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 8344 | +0.130 | -548.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2370 | +0.198 | -147.63€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 57 | +0.025 | -7.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 4895 | +0.097 | -322.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1022 | +0.134 | -71.16€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 4935 | +0.105 | -340.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 18 | +0.000 | +0.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 4914 | +0.105 | -339.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 6953 | +0.176 | -535.20€ | 2 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 6953 | +0.176 | -535.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 1755 | +0.165 | -192.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 1755 | +0.165 | -192.05€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 1728 | +0.168 | -183.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 