# Hipótesis automáticas — 2026-09-24 08:47 UTC
_Generado por shadow_postmortem.py sobre 586654 resoluciones (PNL=+65543.64€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `145.96` → IC=-0.234 (n=7001)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 145.96
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=21008)
...
- **FILTRO** `restante_s_al_confirmar` < `164.79` → IC=-0.226 (n=1673)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 164.79
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=5021)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.205 (n=2095)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.154 (n=2777)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 5.0 (IC base=+0.145)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.145 (n=2372)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.237 (n=1469)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.166 (n=1071)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.172 (n=720)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.202 (n=11298)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.197)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.200 (n=10844)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.250 (n=807)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.245)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.245 (n=814)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.188 (n=6115)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.208 (n=5549)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.203)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.255 (n=2246)
...
### GBM_LATE_15M
- **PATRÓN** `drift_60min` |x|≤ `0.4825` → IC=+0.121 (n=7936)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.60€ cuando `drift_60min` |x|≤ 0.4825 (IC base=+0.105)

- **PATRÓN** `ibs_20min` > `0.9836` → IC=+0.243 (n=2646)
...
- **PATRÓN** `dist_vwap_pct` < `0.2252` → IC=+0.250 (n=1688)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2252 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.966` → IC=+0.176 (n=3045)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 5.966 (IC base=+0.105)

- **PATRÓN** `volumen_regimen` < `1.2119` → IC=+0.247 (n=2136)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2119 (IC base=+0.105)

- **PATRÓN** `volumen_regimen` > `1.05` → IC=+0.256 (n=968)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.05 (IC base=+0.105)

- **PATRÓN** `volumen_pendiente_norm` > `0.3047` → IC=+0.218 (n=792)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3047 (IC base=+0.105)

- **PATRÓN** `volumen_spike_ratio` > `1.4645` → IC=+0.201 (n=5424)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4645 (IC base=+0.105)

- **PATRÓN** `ibs_20min` < `0.57` → IC=+0.134 (n=9578)

  - _Acción_: Kelly boost +0.67€ cuando `ibs_20min` < 0.57 (IC base=+0.065)

- **PATRÓN** `dist_vwap_pct` > `0.6067` → IC=+0.193 (n=702)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.6067 (IC base=+0.065)

- **PATRÓN** `dist_vwap_pct` < `0.1514` → IC=+0.174 (n=3032)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.1514 (IC base=+0.065)

- **PATRÓN** `volumen_regimen` < `0.6986` → IC=+0.181 (n=1473)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.6986 (IC base=+0.065)

- **PATRÓN** `volumen_regimen` > `1.0529` → IC=+0.176 (n=1518)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 1.0529 (IC base=+0.065)

- **PATRÓN** `volumen_pendiente_norm` > `0.1676` → IC=+0.222 (n=1600)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1676 (IC base=+0.065)

- **PATRÓN** `volumen_spike_ratio` > `1.5707` → IC=+0.201 (n=5020)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5707 (IC base=+0.065)

- **PATRÓN** `ballena_activa_n` < `136.0` → IC=+0.211 (n=5412)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3497` → IC=+0.165 (n=1796)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.3497 (IC base=+0.159)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.167 (n=866)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 15.0 (IC base=+0.159)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.168 (n=1211)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.138` → IC=+0.269 (n=769)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4352` → IC=+0.162 (n=1678)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.4352 (IC base=+0.159)

- **PATRÓN** `sigma_h` > `0.005` → IC=+0.250 (n=1206)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.005 (IC base=+0.235)

- **PATRÓN** `drift_60min` |x|≤ `0.0915` → IC=+0.283 (n=450)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0915 (IC base=+0.235)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.243 (n=1215)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.235)

- **PATRÓN** `ibs_20min` < `0.0538` → IC=+0.294 (n=594)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0538 (IC base=+0.235)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.424` → IC=+0.248 (n=1402)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.424 (IC base=+0.235)

- **PATRÓN** `volumen_pendiente_norm` < `0.0922` → IC=+0.231 (n=1156)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6417` → IC=+0.240 (n=410)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6417 (IC base=+0.235)

- **PATRÓN** `libro_liquidez` > `1800.35` → IC=+0.237 (n=899)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1800.35 (IC base=+0.235)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.232 (n=1426)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1998` → IC=+0.221 (n=728)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1998 (IC base=+0.216)

- **PATRÓN** `dist_vwap_pct` < `0.5843` → IC=+0.219 (n=1428)
...
- **PATRÓN** `libro_liquidez` > `16610.3398` → IC=+0.222 (n=455)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 16610.3398 (IC base=+0.216)

- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.153 (n=1423)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.6918` → IC=+0.172 (n=1423)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.6918 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` < `0.1323` → IC=+0.158 (n=1271)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.1323 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.209` → IC=+0.164 (n=227)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 11.209 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.274` → IC=+0.141 (n=1298)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` < 4.274 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `1.204` → IC=+0.151 (n=1423)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.204 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` > `0.0952` → IC=+0.172 (n=511)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.0952 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` < `2.4183` → IC=+0.153 (n=1313)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.4183 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.4188` → IC=+0.146 (n=1313)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.4188 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `14063.0346` → IC=+0.146 (n=949)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 14063.0346 (IC base=+0.140)

- **PATRÓN** `ballena_activa_n` < `411.0` → IC=+0.148 (n=1232)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.189 (n=1843)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 5.0 (IC base=+0.185)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.189 (n=1575)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2116` → IC=+0.188 (n=1752)
...
- **PATRÓN** `ibs_20min` < `0.0625` → IC=+0.248 (n=665)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0625 (IC base=+0.213)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.626` → IC=+0.246 (n=514)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3558` → IC=+0.265 (n=219)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3558 (IC base=+0.213)

- **PATRÓN** `volumen_spike_ratio` < `1.7743` → IC=+0.209 (n=607)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7743 (IC base=+0.213)

- **PATRÓN** `volumen_spike_ratio` > `2.2108` → IC=+0.220 (n=920)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2108 (IC base=+0.213)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.220 (n=1035)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.213)

- **PATRÓN** `libro_liquidez` > `1881.2284` → IC=+0.229 (n=685)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1881.2284 (IC base=+0.213)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.212 (n=1165)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.160 (n=98)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.024 (n=2191)
...
- **PATRÓN** `volumen_regimen` < `0.8497` → IC=+0.164 (n=548)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5154` → IC=+0.176 (n=687)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 1.5154 (IC base=+0.016)
...
- **FILTRO** `ibs_20min` > `0.2558` → IC=-0.124 (n=2160)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2558
  - _Potencial_: sin este filtro IC_bueno=+0.127 (n=1064)

- **FILTRO** `sigma_ewma_delta_pct` > `8.654` → IC=-0.210 (n=346)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.654
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=2878)
...
- **FILTRO** `drift_60min` |x|> `0.6585` → IC=-0.179 (n=558)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6585
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=1676)
...
- **FILTRO** `libro_spread` > `0.02` → IC=-0.199 (n=407)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=1827)

- **FILTRO** `ibs_20min` > `0.77` → IC=-0.200 (n=817)

  - _Acción_: SKIP cuando `ibs_20min` > 0.77
  - _Potencial_: sin este filtro IC_bueno=+0.035 (n=2467)
...
- **PATRÓN** `dist_vwap_pct` > `0.8742` → IC=+0.268 (n=153)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8742 (IC base=-0.023)

- **PATRÓN** `volumen_regimen` < `0.7387` → IC=+0.250 (n=330)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7387 (IC base=-0.023)

- **PATRÓN** `volumen_regimen` > `1.0842` → IC=+0.281 (n=340)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0842 (IC base=-0.023)
...
- **PATRÓN** `sigma_h` > `0.0097` → IC=+0.196 (n=3314)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0097 (IC base=+0.097)

- **PATRÓN** `ibs_20min` > `0.475` → IC=+0.187 (n=8882)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.475 (IC base=+0.097)

- **PATRÓN** `dist_vwap_pct` > `0.7623` → IC=+0.289 (n=1042)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7623 (IC base=+0.097)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.593` → IC=+0.155 (n=4700)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 3.593 (IC base=+0.097)

- **PATRÓN** `volumen_regimen` > `0.6895` → IC=+0.246 (n=3150)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4668` → IC=+0.239 (n=1910)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4668 (IC base=+0.097)

- **PATRÓN** `volumen_spike_ratio` > `2.6695` → IC=+0.237 (n=1910)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6695 (IC base=+0.097)

- **PATRÓN** `ballena_activa_n` < `97.0` → IC=+0.266 (n=5242)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 97.0 (IC base=+0.097)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.153 (n=3314)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0091 (IC base=+0.073)

- **PATRÓN** `ibs_20min` < `0.5472` → IC=+0.153 (n=8738)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.5472 (IC base=+0.073)

- **PATRÓN** `dist_vwap_pct` < `0.247` → IC=+0.240 (n=2767)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.247 (IC base=+0.073)

- **PATRÓN** `volumen_regimen` < `0.7094` → IC=+0.239 (n=1286)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7094 (IC base=+0.073)

- **PATRÓN** `volumen_regimen` > `1.2035` → IC=+0.247 (n=974)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2035 (IC base=+0.073)

- **PATRÓN** `volumen_pendiente_norm` > `0.2968` → IC=+0.303 (n=552)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2968 (IC base=+0.073)

- **PATRÓN** `volumen_spike_ratio` < `1.6059` → IC=+0.256 (n=1690)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6059 (IC base=+0.073)

- **PATRÓN** `volumen_spike_ratio` > `2.3201` → IC=+0.256 (n=1742)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3201 (IC base=+0.073)

- **PATRÓN** `ballena_activa_n` < `84.0` → IC=+0.262 (n=3727)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 84.0 (IC base=+0.073)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `sigma_ewma_delta_pct` > `4.528` → IC=-0.159 (n=523)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.528
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=1762)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4344` → IC=+0.179 (n=288)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 1.4344 (IC base=+0.046)

- **PATRÓN** `volumen_spike_ratio` > `2.1445` → IC=+0.192 (n=391)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 2.1445 (IC base=+0.046)

- **PATRÓN** `ballena_activa_n` < `14.0` → IC=+0.175 (n=373)
...
- **PATRÓN** `ibs_20min` > `0.862` → IC=+0.159 (n=664)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.862 (IC base=+0.025)

- **PATRÓN** `dist_vwap_pct` > `0.1247` → IC=+0.171 (n=506)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.1247 (IC base=+0.025)

- **PATRÓN** `volumen_regimen` > `0.6718` → IC=+0.163 (n=815)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` > 0.6718 (IC base=+0.025)

- **PATRÓN** `volumen_pendiente_norm` > `0.2738` → IC=+0.218 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2738 (IC base=+0.025)

- **PATRÓN** `volumen_spike_ratio` < `1.4226` → IC=+0.196 (n=297)
...
- **PATRÓN** `dist_vwap_pct` < `0.1603` → IC=+0.221 (n=567)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1603 (IC base=+0.005)

- **PATRÓN** `volumen_regimen` > `1.0371` → IC=+0.226 (n=250)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4405` → IC=+0.225 (n=169)
...
- **PATRÓN** `ballena_activa_n` < `473.0` → IC=+0.214 (n=505)
...
- **PATRÓN** `drift_60min` |x|≤ `0.6005` → IC=+0.284 (n=1256)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.2222` → IC=+0.289 (n=1104)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2222 (IC base=+0.281)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.816` → IC=+0.299 (n=495)
...
- **PATRÓN** `volumen_spike_ratio` > `2.7489` → IC=+0.286 (n=525)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7489 (IC base=+0.281)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.289 (n=853)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.281)

- **PATRÓN** `libro_liquidez` > `1874.6847` → IC=+0.297 (n=569)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1874.6847 (IC base=+0.281)
...
- **FILTRO** `ibs_20min` < `0.2842` → IC=-0.186 (n=485)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2842
  - _Potencial_: sin este filtro IC_bueno=+0.075 (n=1456)

- **FILTRO** `ibs_20min` > `0.7753` → IC=-0.185 (n=588)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7753
  - _Potencial_: sin este filtro IC_bueno=+0.054 (n=1768)
...
- **PATRÓN** `dist_vwap_pct` < `0.6847` → IC=+0.199 (n=483)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.6847 (IC base=-0.005)

- **PATRÓN** `volumen_regimen` < `1.1615` → IC=+0.208 (n=419)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8269` → IC=+0.259 (n=251)
...
- **FILTRO** `ibs_20min` < `0.7191` → IC=-0.201 (n=1053)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7191
  - _Potencial_: sin este filtro IC_bueno=+0.280 (n=1054)

- **FILTRO** `ibs_20min` > `0.6875` → IC=-0.229 (n=545)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6875
  - _Potencial_: sin este filtro IC_bueno=+0.096 (n=1657)

- **FILTRO** `sigma_ewma_delta_pct` > `4.685` → IC=-0.178 (n=480)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.685
  - _Potencial_: sin este filtro IC_bueno=+0.069 (n=1722)

- **PATRÓN** `ibs_20min` > `0.7191` → IC=+0.280 (n=1054)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7191 (IC base=+0.040)

- **PATRÓN** `dist_vwap_pct` > `0.8467` → IC=+0.335 (n=241)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8467 (IC base=+0.040)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.595` → IC=+0.161 (n=334)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 9.595 (IC base=+0.040)

- **PATRÓN** `volumen_regimen` < `0.8696` → IC=+0.301 (n=516)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8696 (IC base=+0.040)

- **PATRÓN** `volumen_regimen` > `0.7299` → IC=+0.292 (n=691)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7299 (IC base=+0.040)

- **PATRÓN** `volumen_pendiente_norm` < `0.1016` → IC=+0.292 (n=718)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1016 (IC base=+0.040)

- **PATRÓN** `volumen_pendiente_norm` > `0.2243` → IC=+0.299 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2243 (IC base=+0.040)

- **PATRÓN** `volumen_spike_ratio` < `1.4267` → IC=+0.325 (n=250)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4267 (IC base=+0.040)

- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.311 (n=653)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 56.0 (IC base=+0.040)

- **PATRÓN** `ibs_20min` < `0.1` → IC=+0.213 (n=559)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.294 (n=610)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.277)

- **PATRÓN** `ibs_20min` > `0.9111` → IC=+0.346 (n=860)
...
- **PATRÓN** `volumen_regimen` > `0.8621` → IC=+0.303 (n=860)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.178 (n=8056)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 5.0 (IC base=+0.166)

- **PATRÓN** `ibs_20min` > `0.5779` → IC=+0.216 (n=7752)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5779 (IC base=+0.166)

- **PATRÓN** `dist_vwap_pct` > `0.1752` → IC=+0.195 (n=3387)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1752 (IC base=+0.166)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.353` → IC=+0.253 (n=1590)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.353 (IC base=+0.166)

- **PATRÓN** `volumen_regimen` < `1.2119` → IC=+0.159 (n=5128)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.2119 (IC base=+0.166)

- **PATRÓN** `volumen_regimen` > `0.6296` → IC=+0.160 (n=5128)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.6296 (IC base=+0.166)

- **PATRÓN** `volumen_pendiente_norm` > `0.2958` → IC=+0.194 (n=1161)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2958 (IC base=+0.166)

- **PATRÓN** `volumen_spike_ratio` < `1.5598` → IC=+0.169 (n=3270)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` < 1.5598 (IC base=+0.166)

- **PATRÓN** `volumen_spike_ratio` > `2.6202` → IC=+0.174 (n=2481)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.6202 (IC base=+0.166)

- **PATRÓN** `libro_liquidez` > `2400.7826` → IC=+0.168 (n=5168)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2400.7826 (IC base=+0.166)

- **PATRÓN** `ballena_activa_n` < `116.0` → IC=+0.179 (n=6671)

  - _Acción_: Kelly boost +0.89€ cuando `ballena_activa_n` < 116.0 (IC base=+0.166)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.183 (n=4959)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0066 (IC base=+0.168)

- **PATRÓN** `drift_60min` |x|≤ `0.0805` → IC=+0.208 (n=2474)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4787` → IC=+0.227 (n=7422)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4787 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` < `0.2316` → IC=+0.159 (n=5398)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` < 0.2316 (IC base=+0.168)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.342` → IC=+0.197 (n=1260)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 10.342 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` < `1.1737` → IC=+0.153 (n=5383)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1737 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.2914` → IC=+0.220 (n=1070)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2914 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` < `1.5615` → IC=+0.168 (n=2968)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.5615 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.2548` → IC=+0.170 (n=3057)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.2548 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `117.0` → IC=+0.173 (n=6391)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.240 (n=847)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.1878` → IC=+0.292 (n=642)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1878 (IC base=+0.238)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.245 (n=864)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3437` → IC=+0.265 (n=963)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3437 (IC base=+0.238)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.234` → IC=+0.252 (n=1038)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.234 (IC base=+0.238)

- **PATRÓN** `volumen_pendiente_norm` < `0.1583` → IC=+0.234 (n=905)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5187` → IC=+0.260 (n=390)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5187 (IC base=+0.238)

- **PATRÓN** `libro_liquidez` > `1803.42` → IC=+0.245 (n=642)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1803.42 (IC base=+0.238)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2906` → IC=+0.161 (n=1262)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5649` → IC=+0.188 (n=1262)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` < 0.5649 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.1334` → IC=+0.163 (n=1250)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.1334 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.831` → IC=+0.205 (n=252)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.831 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `1.2116` → IC=+0.157 (n=1262)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4267` → IC=+0.148 (n=1150)
...
- **PATRÓN** `sigma_h` < `0.0113` → IC=+0.231 (n=1079)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0113 (IC base=+0.219)
...
- **PATRÓN** `ibs_20min` < `0.2417` → IC=+0.257 (n=950)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2417 (IC base=+0.219)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.746` → IC=+0.270 (n=458)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7913` → IC=+0.215 (n=440)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7913 (IC base=+0.219)

- **PATRÓN** `volumen_spike_ratio` > `3.406` → IC=+0.231 (n=333)
...
- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.154 (n=1312)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0072 (IC base=+0.123)

- **PATRÓN** `drift_60min` |x|≤ `0.383` → IC=+0.143 (n=1312)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6381` → IC=+0.171 (n=1312)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.6381 (IC base=+0.123)

- **PATRÓN** `dist_vwap_pct` < `0.5804` → IC=+0.133 (n=1522)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.5804 (IC base=+0.123)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.957` → IC=+0.170 (n=459)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 6.957 (IC base=+0.123)

- **PATRÓN** `volumen_regimen` < `0.8485` → IC=+0.148 (n=875)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.8485 (IC base=+0.123)

- **PATRÓN** `volumen_pendiente_norm` > `0.2909` → IC=+0.200 (n=191)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2909 (IC base=+0.123)

- **PATRÓN** `volumen_spike_ratio` < `1.7891` → IC=+0.135 (n=795)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 1.7891 (IC base=+0.123)

- **PATRÓN** `volumen_spike_ratio` > `2.489` → IC=+0.134 (n=397)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 2.489 (IC base=+0.123)

- **PATRÓN** `libro_liquidez` > `9988.1936` → IC=+0.160 (n=595)
...
- **PATRÓN** `ibs_20min` > `0.5172` → IC=+0.202 (n=1397)
...
- **PATRÓN** `volumen_regimen` < `1.2184` → IC=+0.129 (n=1398)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 1.2184 (IC base=+0.117)

- **PATRÓN** `volumen_regimen` > `0.7264` → IC=+0.122 (n=1248)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` > 0.7264 (IC base=+0.117)

- **PATRÓN** `volumen_pendiente_norm` > `0.0708` → IC=+0.122 (n=586)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_pendiente_norm` > 0.0708 (IC base=+0.117)

- **PATRÓN** `volumen_spike_ratio` < `1.5449` → IC=+0.141 (n=594)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 1.5449 (IC base=+0.117)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.127 (n=1463)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1033` → IC=+0.154 (n=475)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.1033 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=1439)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` < `0.5652` → IC=+0.210 (n=1424)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5652 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` > `0.9932` → IC=+0.144 (n=186)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.9932 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` < `0.1993` → IC=+0.140 (n=1313)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.1993 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.027` → IC=+0.142 (n=230)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 9.027 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` < `1.1714` → IC=+0.121 (n=1424)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` < 1.1714 (IC base=+0.114)

- **PATRÓN** `volumen_pendiente_norm` > `0.2757` → IC=+0.163 (n=173)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` > 0.2757 (IC base=+0.114)

- **PATRÓN** `volumen_spike_ratio` < `1.4527` → IC=+0.131 (n=426)
...
- **PATRÓN** `libro_liquidez` > `1465.1216` → IC=+0.134 (n=1272)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 1465.1216 (IC base=+0.114)
...
- **PATRÓN** `sigma_h` < `0.0113` → IC=+0.226 (n=611)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.438` → IC=+0.246 (n=1385)
...
- **PATRÓN** `dist_vwap_pct` < `0.9018` → IC=+0.203 (n=1616)
...
- **PATRÓN** `volumen_regimen` > `0.6291` → IC=+0.216 (n=1385)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2138` → IC=+0.193 (n=1094)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 2.2138 (IC base=+0.203)

- **PATRÓN** `volumen_spike_ratio` > `1.4374` → IC=+0.199 (n=1243)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.4374 (IC base=+0.203)

- **PATRÓN** `libro_liquidez` > `2588.022` → IC=+0.206 (n=923)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2588.022 (IC base=+0.203)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0996` → IC=+0.150 (n=612)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.0996 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.187 (n=922)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 15.0 (IC base=+0.145)

- **PATRÓN** `ibs_20min` > `0.3929` → IC=+0.178 (n=1835)

  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` > 0.3929 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.8719` → IC=+0.192 (n=287)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.8719 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.728` → IC=+0.175 (n=845)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 3.728 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` < `0.8739` → IC=+0.162 (n=1074)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.8739 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` > `1.2058` → IC=+0.151 (n=537)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.2058 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` > `0.1652` → IC=+0.176 (n=511)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.1652 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` < `1.4411` → IC=+0.166 (n=590)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.4411 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` > `2.539` → IC=+0.157 (n=589)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 2.539 (IC base=+0.145)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.149 (n=2082)

  - _Acción_: Kelly boost +0.74€ cuando `libro_spread` < 0.02 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `12366.3016` → IC=+0.155 (n=612)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 12366.3016 (IC base=+0.145)

- **PATRÓN** `ballena_activa_n` < `162.0` → IC=+0.165 (n=1613)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 162.0 (IC base=+0.145)

- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.139 (n=1289)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0056 (IC base=+0.108)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.122 (n=1945)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 5.0 (IC base=+0.108)

- **PATRÓN** `ibs_20min` < `0.5201` → IC=+0.146 (n=1699)

  - _Acción_: Kelly boost +0.73€ cuando `ibs_20min` < 0.5201 (IC base=+0.108)

- **PATRÓN** `volumen_spike_ratio` < `2.2268` → IC=+0.125 (n=1635)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_spike_ratio` < 2.2268 (IC base=+0.108)

- **PATRÓN** `libro_liquidez` > `3883.3874` → IC=+0.127 (n=1287)

  - _Acción_: Kelly boost +0.63€ cuando `libro_liquidez` > 3883.3874 (IC base=+0.108)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2746` → IC=+0.160 (n=542)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.2746 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.148 (n=552)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 7.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` < `0.5957` → IC=+0.180 (n=542)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` < 0.5957 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `0.3049` → IC=+0.155 (n=654)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.3049 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.384` → IC=+0.157 (n=240)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 4.384 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `1.208` → IC=+0.139 (n=616)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` < 1.208 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` > `0.7151` → IC=+0.149 (n=551)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1022` → IC=+0.157 (n=534)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.1022 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.42` → IC=+0.148 (n=606)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.42 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `386.0` → IC=+0.148 (n=584)

  - _Acción_: Kelly boost +0.74€ cuando `ballena_activa_n` < 386.0 (IC base=+0.136)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0954` → IC=+0.208 (n=190)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0954 (IC base=+0.191)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.207 (n=591)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.191)

- **PATRÓN** `ibs_20min` > `0.7028` → IC=+0.243 (n=379)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7028 (IC base=+0.191)
...
- **PATRÓN** `volumen_regimen` > `1.156` → IC=+0.208 (n=190)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.156 (IC base=+0.191)

- **PATRÓN** `volumen_pendiente_norm` > `0.2651` → IC=+0.267 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2651 (IC base=+0.191)

- **PATRÓN** `volumen_spike_ratio` < `1.3982` → IC=+0.237 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3982 (IC base=+0.191)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.200 (n=631)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.191)

- **PATRÓN** `libro_liquidez` > `12369.1399` → IC=+0.208 (n=190)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12369.1399 (IC base=+0.191)

- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.121 (n=470)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.60€ cuando `sigma_h` < 0.0061 (IC base=+0.090)

- **PATRÓN** `ibs_20min` < `0.0816` → IC=+0.158 (n=179)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` < 0.0816 (IC base=+0.090)

- **PATRÓN** `volumen_pendiente_norm` > `0.2279` → IC=+0.128 (n=84)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_pendiente_norm` > 0.2279 (IC base=+0.090)

### GBM_LATE_15M_PYCONFIRMADO#SOL#15min
- **FILTRO** `ibs_20min` > `0.5172` → IC=-0.154 (n=128)

  - _Acción_: SKIP cuando `ibs_20min` > 0.5172
  - _Potencial_: sin este filtro IC_bueno=+0.145 (n=387)

- **FILTRO** `dist_vwap_pct` > `0.3427` → IC=-0.149 (n=35)

  - _Acción_: SKIP cuando `dist_vwap_pct` > 0.3427
  - _Potencial_: sin este filtro IC_bueno=+0.087 (n=480)

- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.181 (n=183)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` > 0.0089 (IC base=+0.128)

- **PATRÓN** `drift_60min` |x|≤ `0.5281` → IC=+0.131 (n=404)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.65€ cuando `drift_60min` |x|≤ 0.5281 (IC base=+0.128)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.164 (n=373)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 8.0 (IC base=+0.128)

- **PATRÓN** `ibs_20min` > `0.7333` → IC=+0.203 (n=361)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.1` → IC=+0.213 (n=186)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.1 (IC base=+0.128)

- **PATRÓN** `volumen_regimen` < `1.0674` → IC=+0.145 (n=356)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 1.0674 (IC base=+0.128)

- **PATRÓN** `volumen_pendiente_norm` > `0.2872` → IC=+0.195 (n=57)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2872 (IC base=+0.128)

- **PATRÓN** `volumen_spike_ratio` > `2.2136` → IC=+0.178 (n=175)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 2.2136 (IC base=+0.128)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.130 (n=441)

  - _Acción_: Kelly boost +0.65€ cuando `libro_spread` < 0.02 (IC base=+0.128)

- **PATRÓN** `libro_liquidez` > `3065.6596` → IC=+0.186 (n=135)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 3065.6596 (IC base=+0.128)

- **PATRÓN** `ibs_20min` < `0.5172` → IC=+0.145 (n=387)

  - _Acción_: Kelly boost +0.73€ cuando `ibs_20min` < 0.5172 (IC base=+0.071)

- **PATRÓN** `volumen_spike_ratio` < `1.8498` → IC=+0.147 (n=242)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.8498 (IC base=+0.071)

- **PATRÓN** `libro_liquidez` > `2926.4608` → IC=+0.124 (n=176)

  - _Acción_: Kelly boost +0.62€ cuando `libro_liquidez` > 2926.4608 (IC base=+0.071)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0112` → IC=+0.206 (n=3303)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0112 (IC base=+0.168)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.179 (n=10309)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 5.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.215 (n=9913)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4706 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` > `0.9685` → IC=+0.207 (n=1385)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9685 (IC base=+0.168)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.628` → IC=+0.225 (n=4799)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2396` → IC=+0.199 (n=1852)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2396 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.5999` → IC=+0.186 (n=3172)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 2.5999 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `2342.629` → IC=+0.171 (n=6604)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 2342.629 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `87.0` → IC=+0.194 (n=7528)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 87.0 (IC base=+0.168)

- **PATRÓN** `sigma_h` < `0.007` → IC=+0.192 (n=5996)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.007 (IC base=+0.181)

- **PATRÓN** `drift_60min` |x|≤ `0.1474` → IC=+0.189 (n=3956)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.566` → IC=+0.238 (n=8991)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.566 (IC base=+0.181)

- **PATRÓN** `dist_vwap_pct` < `0.2469` → IC=+0.163 (n=5578)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2469 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.019` → IC=+0.204 (n=1258)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.019 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.731` → IC=+0.182 (n=8707)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.731 (IC base=+0.181)

- **PATRÓN** `volumen_regimen` < `0.7046` → IC=+0.162 (n=2721)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.7046 (IC base=+0.181)

- **PATRÓN** `volumen_pendiente_norm` > `0.2881` → IC=+0.240 (n=1181)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2881 (IC base=+0.181)

- **PATRÓN** `volumen_spike_ratio` > `2.631` → IC=+0.193 (n=2747)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 2.631 (IC base=+0.181)

- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.193 (n=5290)
...
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.223 (n=561)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0083 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.3541` → IC=+0.189 (n=1683)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.196 (n=1143)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_spike_ratio` < `1.5452` → IC=+0.256 (n=530)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5452 (IC base=+0.259)

- **PATRÓN** `volumen_spike_ratio` > `1.8682` → IC=+0.271 (n=803)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8682 (IC base=+0.259)

- **PATRÓN** `libro_liquidez` > `1799.71` → IC=+0.264 (n=876)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1799.71 (IC base=+0.259)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.163 (n=1654)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 5.0 (IC base=+0.149)

- **PATRÓN** `ibs_20min` > `0.3102` → IC=+0.203 (n=1586)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.757` → IC=+0.167 (n=364)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 9.757 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.255` → IC=+0.153 (n=1424)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` < 4.255 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` < `0.6306` → IC=+0.180 (n=530)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` < 0.6306 (IC base=+0.149)

- **PATRÓN** `volumen_pendiente_norm` > `0.2672` → IC=+0.200 (n=228)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2672 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` < `2.1158` → IC=+0.162 (n=1348)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.1158 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` > `1.7577` → IC=+0.155 (n=1021)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.7577 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `15727.0012` → IC=+0.157 (n=719)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 15727.0012 (IC base=+0.149)

- **PATRÓN** `ballena_activa_n` < `476.0` → IC=+0.158 (n=1462)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 476.0 (IC base=+0.149)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.166 (n=1374)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0057 (IC base=+0.152)

- **PATRÓN** `drift_60min` |x|≤ `0.3229` → IC=+0.162 (n=1373)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.3229 (IC base=+0.152)
...
- **PATRÓN** `ibs_20min` < `0.2689` → IC=+0.236 (n=916)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2689 (IC base=+0.152)

- **PATRÓN** `dist_vwap_pct` < `0.1332` → IC=+0.168 (n=1240)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` < 0.1332 (IC base=+0.152)
...
- **PATRÓN** `volumen_regimen` < `1.1913` → IC=+0.164 (n=1373)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.1913 (IC base=+0.152)

- **PATRÓN** `volumen_pendiente_norm` > `0.1514` → IC=+0.196 (n=370)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.1514 (IC base=+0.152)

- **PATRÓN** `volumen_spike_ratio` < `2.4038` → IC=+0.161 (n=1275)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.4038 (IC base=+0.152)

- **PATRÓN** `volumen_spike_ratio` > `1.7532` → IC=+0.161 (n=850)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 1.7532 (IC base=+0.152)

- **PATRÓN** `ballena_activa_n` < `422.0` → IC=+0.159 (n=1036)
...
- **PATRÓN** `sigma_h` < `0.0115` → IC=+0.239 (n=1495)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0115 (IC base=+0.233)

- **PATRÓN** `drift_60min` |x|≤ `0.1657` → IC=+0.239 (n=658)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.1935` → IC=+0.284 (n=998)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1935 (IC base=+0.233)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.76` → IC=+0.279 (n=550)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.76 (IC base=+0.233)

- **PATRÓN** `volumen_pendiente_norm` > `0.3446` → IC=+0.303 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3446 (IC base=+0.233)

- **PATRÓN** `volumen_spike_ratio` < `1.7544` → IC=+0.235 (n=603)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7544 (IC base=+0.233)

- **PATRÓN** `volumen_spike_ratio` > `2.1892` → IC=+0.236 (n=914)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1892 (IC base=+0.233)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.244 (n=1024)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.233)

- **PATRÓN** `libro_liquidez` > `1881.2284` → IC=+0.250 (n=678)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1881.2284 (IC base=+0.233)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.231 (n=1290)
...
- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.157 (n=1382)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0076 (IC base=+0.138)

- **PATRÓN** `drift_60min` |x|≤ `0.4454` → IC=+0.154 (n=1380)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.7027` → IC=+0.182 (n=1380)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` < 0.7027 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.5974` → IC=+0.144 (n=1527)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.5974 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.146` → IC=+0.176 (n=202)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 11.146 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `0.6962` → IC=+0.150 (n=607)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 0.6962 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `1.1877` → IC=+0.141 (n=460)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 1.1877 (IC base=+0.138)
...
- **PATRÓN** `volumen_regimen` < `0.8919` → IC=+0.134 (n=1115)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 0.8919 (IC base=+0.111)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.124 (n=1685)

  - _Acción_: Kelly boost +0.62€ cuando `libro_spread` < 0.02 (IC base=+0.111)

- **PATRÓN** `libro_liquidez` > `2898.7698` → IC=+0.248 (n=558)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2898.7698 (IC base=+0.111)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.129 (n=1289)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1316` → IC=+0.159 (n=546)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.1316 (IC base=+0.112)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.124 (n=1689)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.468` → IC=+0.123 (n=1581)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 3.468 (IC base=+0.112)

- **PATRÓN** `volumen_regimen` < `0.717` → IC=+0.154 (n=721)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.717 (IC base=+0.112)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4442` → IC=+0.140 (n=492)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 1.4442 (IC base=+0.112)

- **PATRÓN** `volumen_spike_ratio` > `2.5014` → IC=+0.128 (n=492)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 2.5014 (IC base=+0.112)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.215 (n=1742)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.208)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.209 (n=1499)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_regimen` < `1.2434` → IC=+0.210 (n=1676)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2434 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` > `0.6381` → IC=+0.215 (n=1676)
...
- **PATRÓN** `ibs_20min` < `0.5183` → IC=+0.255 (n=1795)
...
- **PATRÓN** `dist_vwap_pct` < `0.9133` → IC=+0.203 (n=2000)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.9133 (IC base=+0.200)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2826` → IC=+0.264 (n=235)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2826 (IC base=+0.200)

- **PATRÓN** `volumen_spike_ratio` < `2.1989` → IC=+0.196 (n=1418)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.1989 (IC base=+0.200)

- **PATRÓN** `volumen_spike_ratio` > `1.4288` → IC=+0.196 (n=1611)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.139 (n=2990)

- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.157 (n=2504)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0092 (IC base=+0.150)

- **PATRÓN** `sigma_h` > `0.0056` → IC=+0.150 (n=2547)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0056 (IC base=+0.150)

- **PATRÓN** `drift_60min` |x|≤ `0.5198` → IC=+0.159 (n=2845)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.5198 (IC base=+0.150)
...
- **PATRÓN** `ibs_20min` > `0.9429` → IC=+0.211 (n=949)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9429 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.1855` → IC=+0.156 (n=1041)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.1855 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` < `0.4875` → IC=+0.144 (n=1720)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.4875 (IC base=+0.150)
...
- **PATRÓN** `volumen_regimen` > `0.9005` → IC=+0.160 (n=1223)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.9005 (IC base=+0.150)

- **PATRÓN** `volumen_pendiente_norm` > `0.1721` → IC=+0.185 (n=778)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.1721 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` < `1.4564` → IC=+0.158 (n=937)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 1.4564 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.8832` → IC=+0.162 (n=1874)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.8832 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `3732.8792` → IC=+0.152 (n=1897)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 3732.8792 (IC base=+0.150)

- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.195 (n=753)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0038 (IC base=+0.137)

- **PATRÓN** `drift_60min` |x|≤ `0.4861` → IC=+0.156 (n=2254)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.0851` → IC=+0.172 (n=752)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.0851 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.6851` → IC=+0.151 (n=431)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.6851 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.4283` → IC=+0.131 (n=2232)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.4283 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.261` → IC=+0.146 (n=2246)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` < 6.261 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `1.2464` → IC=+0.142 (n=2152)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 1.2464 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` < `0.0972` → IC=+0.139 (n=2037)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_pendiente_norm` < 0.0972 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` > `0.0728` → IC=+0.145 (n=1045)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_pendiente_norm` > 0.0728 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` < `1.4255` → IC=+0.149 (n=742)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.4255 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `1.8123` → IC=+0.143 (n=1484)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 1.8123 (IC base=+0.137)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.139 (n=2990)

  - _Acción_: Kelly boost +0.70€ cuando `libro_spread` < 0.01 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `7032.9879` → IC=+0.151 (n=2014)
...
- **PATRÓN** `sigma_h` > `0.0043` → IC=+0.161 (n=887)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` > 0.0043 (IC base=+0.153)

- **PATRÓN** `drift_60min` |x|≤ `0.3872` → IC=+0.153 (n=781)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.3872 (IC base=+0.153)
...
- **PATRÓN** `ibs_20min` < `0.2981` → IC=+0.159 (n=391)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` < 0.2981 (IC base=+0.153)
...
- **PATRÓN** `dist_vwap_pct` < `0.4234` → IC=+0.165 (n=821)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.4234 (IC base=+0.153)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.762` → IC=+0.162 (n=889)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` < 6.762 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` < `0.8944` → IC=+0.155 (n=592)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.8944 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` > `0.7179` → IC=+0.157 (n=792)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 0.7179 (IC base=+0.153)

- **PATRÓN** `volumen_pendiente_norm` < `0.1099` → IC=+0.154 (n=819)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_pendiente_norm` < 0.1099 (IC base=+0.153)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.158 (n=873)
...
- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.153 (n=754)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0083 (IC base=+0.140)

- **PATRÓN** `drift_60min` |x|≤ `0.3844` → IC=+0.171 (n=664)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.3844 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.165 (n=264)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 17.0 (IC base=+0.140)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.155 (n=506)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 10.0 (IC base=+0.140)

- **PATRÓN** `ibs_20min` < `0.7466` → IC=+0.143 (n=754)

  - _Acción_: Kelly boost +0.71€ cuando `ibs_20min` < 0.7466 (IC base=+0.140)

- **PATRÓN** `ibs_20min` > `0.0998` → IC=+0.148 (n=754)
...
- **PATRÓN** `dist_vwap_pct` < `0.3802` → IC=+0.142 (n=764)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.3802 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.802` → IC=+0.159 (n=121)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 10.802 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.615` → IC=+0.143 (n=779)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` < 6.615 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `0.6472` → IC=+0.173 (n=252)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` < 0.6472 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` > `0.7288` → IC=+0.142 (n=674)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 0.7288 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` > `0.0738` → IC=+0.167 (n=319)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.0738 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` < `2.1956` → IC=+0.155 (n=651)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.1956 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.7791` → IC=+0.150 (n=493)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.7791 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `7580.4968` → IC=+0.167 (n=754)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 7580.4968 (IC base=+0.140)
...
### LEADLAG_BTC_XRP_15M
- **PATRÓN** `py_entrada` > `0.5` → IC=+0.121 (n=692)

  - _Acción_: Kelly boost +0.61€ cuando `py_entrada` > 0.5 (IC base=+0.105)

- **PATRÓN** `libro_liquidez` > `2918.2518` → IC=+0.165 (n=231)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 2918.2518 (IC base=+0.105)
...
- **PATRÓN** `liq_usd_total` > `80505.33` → IC=+0.167 (n=88)

  - _Acción_: Kelly boost +0.83€ cuando `liq_usd_total` > 80505.33 (IC base=+0.025)
...
- **FILTRO** `py_entrada` < `0.4` → IC=-0.189 (n=130)
...
- **PATRÓN** `py_entrada` < `0.57` → IC=+0.130 (n=228)

  - _Acción_: Kelly boost +0.65€ cuando `py_entrada` < 0.57 (IC base=+0.036)

- **PATRÓN** `profundidad_ratio` > `100.0` → IC=+0.188 (n=110)

  - _Acción_: Kelly boost +0.94€ cuando `profundidad_ratio` > 100.0 (IC base=+0.036)
...
- **FILTRO** `profundidad_ratio` < `60.7` → IC=-0.300 (n=33)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.194 (n=34)
...
- **FILTRO** `lag_apertura_s` > `139.44` → IC=-0.289 (n=17)

  - _Acción_: SKIP cuando `lag_apertura_s` > 139.44
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=54)

- **FILTRO** `profundidad_ratio` < `26.8` → IC=-0.146 (n=46)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=7470)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.167 (n=3471)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.058 (n=10446)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.167 (n=3605)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.035 (n=10820)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.46` → IC=-0.211 (n=597)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.100 (n=1817)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.47` → IC=-0.190 (n=611)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.104 (n=1846)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.202 (n=632)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.064 (n=1948)

- **FILTRO** `ibs_20min` > `0.2814` → IC=-0.161 (n=644)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2814
  - _Potencial_: sin este filtro IC_bueno=+0.052 (n=1936)
...
- **FILTRO** `py_entrada` > `0.57` → IC=-0.177 (n=633)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=1950)

### MOMENTUM_IBS_15M_FADE
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=2745)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=2929)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.129 (n=9940)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=21981)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.273 (n=7796)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=24125)

- **FILTRO** `ibs_7min` < `0.2857` → IC=-0.235 (n=7977)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2857
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=23944)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.157 (n=10845)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=21076)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.229 (n=9814)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=30202)

- **FILTRO** `ibs_7min` > `0.2927` → IC=-0.178 (n=10001)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2927
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=30015)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.135 (n=1628)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3644)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.307 (n=1259)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4013)

- **FILTRO** `ibs_7min` < `0.7099` → IC=-0.248 (n=1738)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7099
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=3534)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.184 (n=1214)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4058)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.260 (n=1696)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.004 (n=5180)

- **FILTRO** `drift_7min_pct` |x|> `0.1121` → IC=-0.123 (n=2337)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1121
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=4539)

- **FILTRO** `ibs_7min` > `0.7881` → IC=-0.206 (n=1718)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7881
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=5158)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=1299)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=4218)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.250 (n=1330)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=4187)

- **FILTRO** `ibs_7min` < `0.7482` → IC=-0.191 (n=1378)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7482
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=4139)

- **FILTRO** `ballena_activa_n` > `160.0` → IC=-0.173 (n=1379)

  - _Acción_: SKIP cuando `ballena_activa_n` > 160.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=4138)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.253 (n=1392)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=4201)

- **FILTRO** `ibs_7min` > `0.2601` → IC=-0.176 (n=1398)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2601
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=4195)

- **FILTRO** `ballena_activa_n` > `154.0` → IC=-0.182 (n=1392)

  - _Acción_: SKIP cuando `ballena_activa_n` > 154.0
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=4201)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.165 (n=1430)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.082 (n=3565)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.306 (n=1225)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=3770)

- **FILTRO** `ibs_7min` < `0.1935` → IC=-0.258 (n=1246)

  - _Acción_: SKIP cuando `ibs_7min` < 0.1935
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=3749)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.219 (n=1164)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=3831)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.236 (n=1681)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.019 (n=5673)

- **FILTRO** `ibs_7min` > `0.7458` → IC=-0.176 (n=1838)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7458
  - _Potencial_: sin este filtro IC_bueno=+0.006 (n=5516)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=1705)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=3559)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.231 (n=1556)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=3708)

- **FILTRO** `ibs_7min` < `0.7412` → IC=-0.182 (n=1316)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7412
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=3948)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.170 (n=1296)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.075 (n=3968)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.261 (n=1340)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=4033)

- **FILTRO** `ibs_7min` > `0.2747` → IC=-0.177 (n=1343)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2747
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4030)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.185 (n=1330)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=4043)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.258 (n=1355)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=4225)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.240 (n=1391)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=4189)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.177 (n=1812)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.023 (n=5797)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.255 (n=1702)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3591)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.228 (n=1308)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=3985)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.215 (n=1285)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=4008)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.204 (n=1728)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=5483)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=2877)
...
- **PATRÓN** `ibs_15` < `0.1159` → IC=+0.161 (n=467)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.80€ cuando `ibs_15` < 0.1159 (IC base=+0.047)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1201` → IC=+0.244 (n=1144)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1201 (IC base=-0.034)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.177` → IC=+0.234 (n=1108)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.177 (IC base=-0.034)

- **PATRÓN** `ibs_15` < `0.3457` → IC=+0.277 (n=1717)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **PATRÓN** `ibs_15` < `0.3453` → IC=+0.265 (n=674)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3453 (IC base=+0.224)

- **PATRÓN** `dist_vwap_pct` > `0.7773` → IC=+0.302 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7773 (IC base=+0.224)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `12.251` → IC=+0.230 (n=719)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 12.251 (IC base=+0.224)

- **PATRÓN** `libro_liquidez` > `3546.5572` → IC=+0.225 (n=674)
...
- **FILTRO** `sigma_ewma_delta_pct` > `18.012` → IC=-0.133 (n=322)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 18.012
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=2633)
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1159 sube el IC de +0.047 a +0.161 en UPDOWN_GBM#XRP#15min (n=467). Ya aplicado como kelly_boost=+0.80€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3457 sube el IC de -0.034 a +0.277 en UPDOWN_GBM_15M_TARDIO (n=1717). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.3453 sube el IC de +0.224 a +0.265 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=674). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 24925 | -0.092 | -3189.67€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1442 | -0.049 | -219.18€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 23483 | -0.095 | -2970.49€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#ETH | 7206 | -0.024 | -675.23€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 7206 | -0.024 | -675.23€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 6694 | -0.099 | -460.81€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 6694 | -0.099 | -460.81€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 5528 | -0.181 | -1079.46€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 5528 | -0.181 | -1079.46€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 17817 | -0.028 | +4066.35€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 4658 | +0.000 | +1828.11€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 13159 | -0.038 | +2238.24€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 17817 | -0.028 | +4066.35€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 4658 | +0.000 | +1828.11€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 13159 | -0.038 | +2238.24€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 91665 | +0.112 | -4717.52€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 13867 | +0.185 | -432.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 369 | -0.088 | -51.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 71451 | +0.099 | -4011.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 5978 | +0.107 | -221.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 11885 | +0.098 | -1013.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 45 | -0.160 | -0.32€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 11825 | +0.099 | -1001.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC |