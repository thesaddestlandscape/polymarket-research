# Hipótesis automáticas — 2026-07-23 04:39 UTC
_Generado por shadow_postmortem.py sobre 30218 resoluciones (PNL=+6781.90€)_
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.214 (n=704)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.179 (n=2325)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.219 (n=368)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.213)

- **PATRÓN** `py_entrada` > `0.605` → IC=+0.271 (n=417)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.250 (n=154)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.312 (n=46)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.228 (n=366)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.221)

- **PATRÓN** `py_entrada` > `0.715` → IC=+0.351 (n=199)
...
- **PATRÓN** `libro_liquidez` > `4253.2534` → IC=+0.228 (n=277)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4253.2534 (IC base=+0.221)
...
- **PATRÓN** `py_entrada` < `0.405` → IC=+0.170 (n=101)

  - _Acción_: Kelly boost +0.85€ cuando `py_entrada` < 0.405 (IC base=+0.144)

- **PATRÓN** `py_entrada` > `0.365` → IC=+0.149 (n=132)

  - _Acción_: Kelly boost +0.75€ cuando `py_entrada` > 0.365 (IC base=+0.144)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.142 (n=135)

  - _Acción_: Kelly boost +0.71€ cuando `libro_spread` < 0.01 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `4151.9377` → IC=+0.186 (n=116)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 4151.9377 (IC base=+0.144)
...
- **PATRÓN** `py_entrada` > `0.675` → IC=+0.331 (n=282)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.675 (IC base=+0.237)

- **PATRÓN** `libro_liquidez` > `1877.4208` → IC=+0.250 (n=278)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1877.4208 (IC base=+0.237)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.212 (n=220)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.210)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.222 (n=447)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `1629.147` → IC=+0.210 (n=439)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.646` → IC=+0.221 (n=431)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.646 (IC base=+0.114)

- **PATRÓN** `sigma_h` > `0.0132` → IC=+0.132 (n=908)
...
- **PATRÓN** `dist_vwap_pct` > `0.6593` → IC=+0.146 (n=80)
  ...
  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.6593 (IC base=+0.064)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.179` → IC=+0.170 (n=110)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 7.179 (IC base=+0.064)

### GBM_LATE_15M#SOL#15min
- **FILTRO** `sigma_ewma_delta_pct` > `9.271` → IC=-0.300 (n=18)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 9.271
  - _Potencial_: sin este filtro IC_bueno=+0.078 (n=415)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.144 (n=478)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` > `0.0149` → IC=+0.154 (n=435)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0149 (IC base=+0.149)
...
- **PATRÓN** `dist_vwap_pct` < `0.1078` → IC=+0.143 (n=281)
...
- **PATRÓN** `sigma_h` > `0.0267` → IC=+0.165 (n=246)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.204 (n=268)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.152)

- **PATRÓN** `dist_vwap_pct` > `0.3699` → IC=+0.214 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3699 (IC base=+0.152)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.771` → IC=+0.140 (n=401)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` < 3.771 (IC base=+0.152)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0116` → IC=+0.221 (n=461)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.2723` → IC=+0.203 (n=409)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2723 (IC base=+0.155)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.784` → IC=+0.259 (n=422)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.784 (IC base=+0.155)

- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.166 (n=1035)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` > 0.0072 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.143 (n=1080)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 12.0 (IC base=+0.133)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.143 (n=580)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` < 6.0 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` > `0.9789` → IC=+0.167 (n=118)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.9789 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `0.7321` → IC=+0.135 (n=1538)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.7321 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.196` → IC=+0.157 (n=304)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 6.196 (IC base=+0.133)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.895` → IC=+0.157 (n=100)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 6.895 (IC base=+0.098)
...
- **PATRÓN** `dist_vwap_pct` > `0.6192` → IC=+0.154 (n=79)
  ...
  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.6192 (IC base=+0.091)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.166` → IC=+0.189 (n=104)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 7.166 (IC base=+0.091)
...
- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.179 (n=163)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0083 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` < `0.7309` → IC=+0.122 (n=342)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.7309 (IC base=+0.102)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.493` → IC=+0.122 (n=334)
...
- **PATRÓN** `sigma_h` < `0.0172` → IC=+0.262 (n=334)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0172 (IC base=+0.240)

- **PATRÓN** `sigma_h` > `0.0085` → IC=+0.247 (n=334)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.240 (n=302)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.240)

- **PATRÓN** `dist_vwap_pct` > `0.5923` → IC=+0.333 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5923 (IC base=+0.240)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.089` → IC=+0.378 (n=88)
...
- **PATRÓN** `dist_vwap_pct` > `0.4452` → IC=+0.157 (n=284)
  ...
  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.4452 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` < `0.1065` → IC=+0.135 (n=786)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.1065 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.806` → IC=+0.279 (n=197)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `13.771` → IC=+0.221 (n=41)
...
- **FILTRO** `sigma_ewma_delta_pct` > `7.005` → IC=-0.152 (n=21)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.087 (n=337)
...
- **PATRÓN** `sigma_h` < `0.0109` → IC=+0.124 (n=325)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` < 0.0109 (IC base=+0.062)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.149 (n=132)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` < 6.0 (IC base=+0.062)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` < `0.02` → IC=+0.208 (n=371)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.02 (IC base=+0.191)

- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.195 (n=421)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0094 (IC base=+0.191)
...
- **PATRÓN** `sigma_h` < `0.0097` → IC=+0.182 (n=218)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0097 (IC base=+0.178)

- **PATRÓN** `sigma_h` > `0.0131` → IC=+0.184 (n=330)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0131 (IC base=+0.178)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.197 (n=176)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 18.0 (IC base=+0.178)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.202 (n=176)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.178)

- **PATRÓN** `dist_vwap_pct` > `0.1404` → IC=+0.236 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1404 (IC base=+0.178)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.347` → IC=+0.177 (n=481)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` < 10.347 (IC base=+0.178)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.7529` → IC=+0.227 (n=371)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `23.418` → IC=+0.196 (n=54)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 23.418 (IC base=+0.052)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.123 (n=242)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` < 0.0066 (IC base=+0.098)

- **PATRÓN** `drift_60min` |x|≤ `0.2247` → IC=+0.133 (n=265)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.66€ cuando `drift_60min` |x|≤ 0.2247 (IC base=+0.098)

- **PATRÓN** `delta_ratio_macro` |x|> `0.084` → IC=+0.129 (n=238)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.65€ cuando `delta_ratio_macro` |x|> 0.084 (IC base=+0.098)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.146 (n=162)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 15.0 (IC base=+0.098)

- **PATRÓN** `ibs_15` > `0.6323` → IC=+0.215 (n=226)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6323 (IC base=+0.098)

- **PATRÓN** `dist_vwap_pct` > `0.1484` → IC=+0.235 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1484 (IC base=+0.098)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.121` → IC=+0.157 (n=65)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 8.121 (IC base=+0.098)

- **PATRÓN** `sigma_ewma_delta_pct` < `14.299` → IC=+0.164 (n=105)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` < 14.299 (IC base=+0.098)
...
- **PATRÓN** `dist_vwap_pct` > `0.4531` → IC=+0.156 (n=62)
  ...
  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.4531 (IC base=+0.048)
...
- **FILTRO** `sigma_ewma_delta_pct` > `14.261` → IC=-0.154 (n=24)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.7529 sube el IC de +0.052 a +0.227 en UPDOWN_GBM#15min (n=371). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.6323 sube el IC de +0.098 a +0.215 en UPDOWN_GBM#BTC#15min (n=226). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 821 | +0.128 | +36.18€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 821 | +0.128 | +36.18€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 8 | +0.040 | +0.96€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 8 | +0.040 | +0.96€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 312 | +0.134 | +9.31€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 312 | +0.134 | +9.31€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 315 | +0.150 | +14.11€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 315 | +0.150 | +14.11€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 186 | +0.080 | +11.81€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 186 | +0.080 | +11.81€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS | 20 | +0.318 | +0.57€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 20 | +0.318 | +0.57€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 20 | +0.318 | +0.57€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 20 | +0.318 | +0.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 5277 | +0.187 | +89.77€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3458 | +0.216 | +92.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 229 | +0.054 | -14.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 675 | +0.117 | -68.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 915 | +0.165 | +80.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 1545 | +0.194 | +35.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1154 | +0.209 | -6.09€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 76 | +0.051 | -7.27€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 305 | +0.184 | +52.86€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH | 1864 | +0.179 | +19.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1138 | +0.215 | +41.68€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 76 | -0.026 | -17.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 345 | +0.125 | -27.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 305 | +0.155 | +22.98€ | 0 | 11 |
| ✅ FAVORITO_CONFIRMADO#SOL | 1839 | +0.190 | +29.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1141 | +0.224 | +56.32€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 77 | +0.133 | +9.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 316 | +0.116 | -40.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 305 | +0.155 | +4.66€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 27 | +0.155 | +2.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 23 | +0.100 | -2.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 280 | +0.316 | +14.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 280 | +0.316 | +14.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 280 | +0.316 | +14.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 280 | +0.316 | +14.03€ | 0 | 0 |
| ✅ GBM_LATE_15M | 7029 | +0.097 | +2204.89€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 7029 | +0.097 | +2204.89€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1695 | +0.067 | +281.40€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1695 | +0.067 | +281.40€ | 0 | 2 |
| ✅ GBM_LATE_15M#ETH | 1587 | +0.073 | +276.23€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1587 | +0.073 | +276.23€ | 0 | 6 |
| ✅ GBM_LATE_15M#SOL | 1894 | +0.090 | +687.10€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1894 | +0.090 | +687.10€ | 1 | 3 |
| ✅ GBM_LATE_15M#XRP | 1853 | +0.151 | +960.16€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1853 | +0.151 | +960.16€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 4226 | +0.128 | +2177.72€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 4226 | +0.128 | +2177.72€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 1084 | +0.087 | +401.01€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 1084 | +0.087 | +401.01€ | 0 | 4 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1102 | +0.092 | +415.50€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1102 | +0.092 | +415.50€ | 0 | 6 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 1068 | +0.104 | +469.37€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 1068 | +0.104 | +469.37€ | 0 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 972 | +0.239 | +891.83€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 972 | +0.239 | +891.83€ | 0 | 11 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 239 | +0.110 | +110.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 239 | +0.110 | +110.98€ | 0 | 0 |
| 🚫 GBM_LATE_15M_PYCONFIRMADO#BTC | 9 | -0.102 | -1.94€ | 0 | 0 |
| 🚫 GBM_LATE_15M_PYCONFIRMADO#BTC#15min | 9 | -0.102 | -1.94€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH | 78 | +0.000 | +14.40€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH#15min | 78 | +0.000 | +14.40€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO | 4227 | +0.082 | +1292.38€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 4227 | +0.082 | +1292.38€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 1017 | +0.043 | +147.61€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 1017 | +0.043 | +147.61€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 1019 | +0.030 | +86.46€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 1019 | +0.030 | +86.46€ | 0 | 1 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 971 | +0.049 | +219.39€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 971 | +0.049 | +219.39€ | 1 | 3 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 1220 | +0.184 | +838.92€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1220 | +0.184 | +838.92€ | 0 | 11 |
| ✅ GBM_LATE_5M | 310 | -0.013 | +11.34€ | 0 | 0 |
| ✅ GBM_LATE_5M#5min | 310 | -0.013 | +11.34€ | 0 | 0 |
...
| ✅ GBM_LATE_5M#SOL | 74 | -0.158 | -0.50€ | 0 | 0 |
| ✅ GBM_LATE_5M#SOL#5min | 74 | -0.158 | -0.50€ | 0 | 0 |
| ✅ GBM_LATE_5M#XRP | 141 | +0.101 | +20.76€ | 0 | 0 |
| ✅ GBM_LATE_5M#XRP#5min | 141 | +0.101 | +20.76€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 2281 | +0.029 | +188.47€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 2018 | +0.050 | +226.98€ | 0 | 3 |
...
| ✅ UPDOWN_GBM#BTC | 527 | +0.048 | +55.62€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 443 | +0.084 | +70.10€ | 1 | 10 |
| 🚫 UPDOWN_GBM#BTC#5min | 19 | -0.158 | -7.19€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#60min | 43 | -0.078 | -6.67€ | 1 | 0 |
| ✅ UPDOWN_GBM#BTC#daily | 18 | -0.135 | +1.83€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 169 | +0.044 | +16.73€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 166 | +0.048 | +17.63€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 939 | +0.030 | +53.86€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 845 | +0.045 | +65.42€ | 0 | 4 |
...
| ✅ UPDOWN_GBM#XRP | 225 | +0.024 | +27.45€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 214 | +0.037 | +30.74€ | 4 | 0 |
| 🚫 UPDOWN_GBM#XRP#5min | 11 | -0.106 | -3.29€ | 0 | 0 |
| 🚫 UPDOWN_GBM#daily | 45 | -0.202 | +1.33€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD | 127 | +0.291 | +24.00€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#15min | 127 | +0.291 | +24.00€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC | 59 | +0.271 | +7.30€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min | 59 | +0.271 | +7.30€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH | 68 | +0.300 | +16.70€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min | 68 | +0.300 | +16.70€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO | 1311 | +0.165 | +601.19€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1311 | +0.165 | +601.19€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 129 | +0.210 | +78.21€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 129 | +0.210 | +78.21€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 148 | +0.127 | +26.64€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 148 | +0.127 | +26.64€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 127 | +0.229 | +91.95€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 127 | +0.229 | +91.95€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 257 | +0.156 | +83.49€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 257 | +0.156 | +83.49€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 320 | +0.102 | +91.67€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 320 | +0.102 | +91.67€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 330 | +0.202 | +229.24€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 330 | +0.202 | +229.24€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.202: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.059 n=1142 | neutral: IC=+0.003 n=878 | overbought(IBS>0.7): IC=+0.206 n=1113
  - _Datos_: n=3340 IC=+0.101 PNL=+838.13€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.115 n=18095 | tras_1loss IC=+0.083 n=11921 | tras_2loss IC=+0.052 n=4941/40 | gap=+0.063 (umbral 0.05)
...
**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  ...
  - _Estado_: alineada_con_outcome_prev IC=+0.075 n=151/60 | contraria IC=+0.089 n=54 | gap=-0.014 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2149 IC=+0.035 PNL=+203.21€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2149 IC=+0.035 PNL=+203.21€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=914 IC=+0.052 PNL=+117.71€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=914 IC=+0.052 PNL=+117.71€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=443 IC=+0.084 PNL=+70.10€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=443 IC=+0.084 PNL=+70.10€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.089 > 0.08 con n=1074 PNL=+179.95€
  - _Datos_: n=1074 IC=+0.089 PNL=+179.95€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=1056 IC=+0.037 PNL=+78.89€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1056 IC=+0.037 PNL=+78.89€

**🟡 H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  - _Hipótesis_: Feature nueva 2026-06-27: drift del precio YES en Polymarket en últimas 5 obs (~5min). Si poly_drift<0 y decidimos BUY_NO (o poly_drift>0 y BUY_YES) → confluencia. Si diverge → reducción de stake. Hipótesis: confluencia Binance+Polymarket mejora IC; divergencia empeora.
  - _Umbral_: n≥40 en confluencia vs divergencia para validar el boost ×1.1
  - _Acción_: Si IC_confluencia>IC_divergencia con n≥40 → mantener el boost. Si no → retirar.
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.110 > 0.08 con n=221 PNL=+43.77€
  - _Datos_: n=221 IC=+0.110 PNL=+43.77€
...
**🟡 H-CUSTOM-CROSS-WINDOW-SPREAD-POS** — GBM 15min/60min: spread positivo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.250 > 0.08 con n=146 PNL=+24.63€
  - _Datos_: n=146 IC=+0.250 PNL=+24.63€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=290 IC=+0.021 PNL=+16.89€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=290 IC=+0.021 PNL=+16.89€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=1064 IC=+0.024 PNL=+87.16€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1064 IC=+0.024 PNL=+87.16€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.173 > 0.1 con n=711 PNL=+266.67€
  - _Datos_: n=711 IC=+0.173 PNL=+266.67€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=185 IC=+0.061 PNL=+34.10€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=185 IC=+0.061 PNL=+34.10€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=770 IC=-0.078 PNL=+173.77€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=770 IC=-0.078 PNL=+173.77€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=926 IC=+0.054 PNL=+111.68€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=926 IC=+0.054 PNL=+111.68€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.143 > 0.08 con n=510 PNL=+163.96€
  - _Datos_: n=510 IC=+0.143 PNL=+163.96€
...
**〰️ H-CUSTOM-GBM-04H-ASIA** — UPDOWN_GBM 04h-05h UTC — media sesión asiática, ¿mejor franja nocturna?
  ...
  - _Estado_: n=167 IC=+0.056 PNL=+29.78€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=167 IC=+0.056 PNL=+29.78€
...
**🟡 H-CUSTOM-LATE-04H-ASIA** — GBM_LATE_15M 04h-05h UTC — media sesión asiática (gemela de GBM-04H-ASIA)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.105 > 0.08 con n=580 PNL=+188.17€
  - _Datos_: n=580 IC=+0.105 PNL=+188.17€
...
**🟡 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.186 > 0.08 con n=234 PNL=+89.72€
  - _Datos_: n=234 IC=+0.186 PNL=+89.72€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.110 > 0.08 con n=413 PNL=+117.74€
  - _Datos_: n=413 IC=+0.110 PNL=+117.74€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.110 > 0.08 con n=441 PNL=+152.79€
  - _Datos_: n=441 IC=+0.110 PNL=+152.79€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.134 > 0.08 con n=528 PNL=+277.61€
  - _Datos_: n=528 IC=+0.134 PNL=+277.61€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.288 > 0.08 con n=395 PNL=+15.45€
  - _Datos_: n=395 IC=+0.288 PNL=+15.45€
