# Hipótesis automáticas — 2026-08-19 15:35 UTC
_Generado por shadow_postmortem.py sobre 77121 resoluciones (PNL=+8092.73€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `154.83` → IC=-0.251 (n=1030)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 154.83
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3092)
...
- **FILTRO** `n_ballenas` < `4.0` → IC=-0.138 (n=241)

  - _Acción_: SKIP cuando `n_ballenas` < 4.0
  - _Potencial_: sin este filtro IC_bueno=-0.097 (n=773)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.184 (n=2665)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.189 (n=2093)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.02 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `4072.1587` → IC=+0.177 (n=857)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.215 (n=328)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.196 (n=468)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.186 (n=2517)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` < 15.0 (IC base=+0.180)

- **PATRÓN** `py_entrada` < `0.7` → IC=+0.180 (n=1047)

  - _Acción_: Kelly boost +0.90€ cuando `py_entrada` < 0.7 (IC base=+0.180)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.233 (n=560)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.220 (n=3726)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.152 (n=421)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.231 (n=841)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.225)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.226 (n=864)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.251 (n=573)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.243)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.245 (n=759)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.150 (n=1920)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 5.0 (IC base=+0.146)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.152 (n=1895)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.159 (n=951)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 5.0 (IC base=+0.153)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.161 (n=833)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.197 (n=338)

  - _Acción_: Kelly boost +0.99€ cuando `py_entrada` < 0.37 (IC base=+0.139)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.141 (n=969)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 5.0 (IC base=+0.139)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.147 (n=960)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.132 (n=493)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` < 6.0 (IC base=+0.099)

- **PATRÓN** `ibs_20min` > `0.9834` → IC=+0.272 (n=455)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9834 (IC base=+0.099)

- **PATRÓN** `dist_vwap_pct` > `0.5942` → IC=+0.306 (n=34)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5942 (IC base=+0.099)
...
- **PATRÓN** `volumen_regimen` > `0.879` → IC=+0.250 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.879 (IC base=+0.099)

- **PATRÓN** `volumen_pendiente_norm` > `0.327` → IC=+0.143 (n=110)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_pendiente_norm` > 0.327 (IC base=+0.099)

- **PATRÓN** `volumen_spike_ratio` > `1.6526` → IC=+0.127 (n=700)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_spike_ratio` > 1.6526 (IC base=+0.099)
...
- **PATRÓN** `sigma_h` < `0.0018` → IC=+0.318 (n=42)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0018 (IC base=+0.265)

- **PATRÓN** `sigma_h` > `0.0036` → IC=+0.291 (n=41)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0036 (IC base=+0.265)

- **PATRÓN** `drift_60min` |x|≤ `0.0813` → IC=+0.272 (n=55)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0813 (IC base=+0.265)

- **PATRÓN** `hora_utc` > `9.0` → IC=+0.314 (n=111)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 9.0 (IC base=+0.265)

- **PATRÓN** `ibs_20min` > `0.6025` → IC=+0.292 (n=123)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6025 (IC base=+0.265)

- **PATRÓN** `dist_vwap_pct` > `0.171` → IC=+0.312 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.171 (IC base=+0.265)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.157` → IC=+0.333 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.157 (IC base=+0.265)

- **PATRÓN** `volumen_regimen` < `0.6694` → IC=+0.318 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6694 (IC base=+0.265)

- **PATRÓN** `volumen_regimen` > `0.7626` → IC=+0.268 (n=110)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7626 (IC base=+0.265)

- **PATRÓN** `volumen_pendiente_norm` < `0.1897` → IC=+0.304 (n=100)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1897 (IC base=+0.265)

- **PATRÓN** `volumen_pendiente_norm` > `0.2969` → IC=+0.273 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2969 (IC base=+0.265)

- **PATRÓN** `volumen_spike_ratio` < `1.4811` → IC=+0.361 (n=34)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4811 (IC base=+0.265)

- **PATRÓN** `libro_liquidez` > `13050.4574` → IC=+0.310 (n=56)
...
- **PATRÓN** `ibs_20min` > `0.772` → IC=+0.154 (n=53)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` > 0.772 (IC base=+0.042)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.896` → IC=+0.175 (n=38)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 9.896 (IC base=+0.042)
...
- **PATRÓN** `volumen_regimen` < `0.5591` → IC=+0.214 (n=19)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.5591 (IC base=-0.013)
...
- **PATRÓN** `sigma_h` > `0.0066` → IC=+0.201 (n=631)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0066 (IC base=+0.146)

- **PATRÓN** `drift_60min` |x|≤ `0.0919` → IC=+0.145 (n=610)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.0919 (IC base=+0.146)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.151 (n=1295)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` < 15.0 (IC base=+0.146)

- **PATRÓN** `ibs_20min` > `0.9197` → IC=+0.262 (n=923)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9197 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `0.2377` → IC=+0.184 (n=340)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.2377 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.948` → IC=+0.271 (n=678)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.948 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` < `1.2184` → IC=+0.145 (n=737)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 1.2184 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` > `0.6367` → IC=+0.151 (n=737)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 0.6367 (IC base=+0.146)

- **PATRÓN** `volumen_pendiente_norm` > `0.1106` → IC=+0.171 (n=481)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` > 0.1106 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` < `2.6402` → IC=+0.125 (n=1011)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_spike_ratio` < 2.6402 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` > `1.5436` → IC=+0.125 (n=1149)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_spike_ratio` > 1.5436 (IC base=+0.146)

- **PATRÓN** `libro_spread` < `0.05` → IC=+0.154 (n=1406)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.05 (IC base=+0.146)

- **PATRÓN** `libro_liquidez` > `2785.7274` → IC=+0.188 (n=462)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 2785.7274 (IC base=+0.146)
...
- **PATRÓN** `sigma_h` < `0.0018` → IC=+0.273 (n=42)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0018 (IC base=+0.216)

- **PATRÓN** `sigma_h` > `0.0035` → IC=+0.344 (n=43)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0035 (IC base=+0.216)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.265 (n=130)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.216)

- **PATRÓN** `ibs_20min` > `0.655` → IC=+0.289 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.655 (IC base=+0.216)

- **PATRÓN** `dist_vwap_pct` > `0.1487` → IC=+0.300 (n=73)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1487 (IC base=+0.216)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.832` → IC=+0.319 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.832 (IC base=+0.216)

- **PATRÓN** `volumen_regimen` < `1.429` → IC=+0.227 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.429 (IC base=+0.216)

- **PATRÓN** `volumen_regimen` > `0.9205` → IC=+0.244 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.9205 (IC base=+0.216)

- **PATRÓN** `volumen_pendiente_norm` > `0.3409` → IC=+0.333 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3409 (IC base=+0.216)

- **PATRÓN** `volumen_spike_ratio` < `1.4872` → IC=+0.300 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4872 (IC base=+0.216)

- **PATRÓN** `libro_liquidez` > `12448.5931` → IC=+0.318 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12448.5931 (IC base=+0.216)
...
- **PATRÓN** `sigma_h` < `0.0021` → IC=+0.333 (n=40)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0021 (IC base=+0.258)

- **PATRÓN** `sigma_h` > `0.003` → IC=+0.269 (n=76)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.003 (IC base=+0.258)

- **PATRÓN** `drift_60min` |x|≤ `0.1667` → IC=+0.308 (n=76)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1667 (IC base=+0.258)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.267 (n=114)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.614` → IC=+0.310 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.614 (IC base=+0.258)

- **PATRÓN** `dist_vwap_pct` < `0.364` → IC=+0.279 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.364 (IC base=+0.258)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.807` → IC=+0.419 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.807 (IC base=+0.258)

- **PATRÓN** `volumen_regimen` > `0.6636` → IC=+0.293 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6636 (IC base=+0.258)

- **PATRÓN** `volumen_pendiente_norm` > `0.1116` → IC=+0.300 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1116 (IC base=+0.258)

- **PATRÓN** `volumen_spike_ratio` < `1.6992` → IC=+0.255 (n=47)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6992 (IC base=+0.258)

- **PATRÓN** `volumen_spike_ratio` > `2.091` → IC=+0.281 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.091 (IC base=+0.258)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.269 (n=119)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.258)

- **PATRÓN** `libro_liquidez` > `10351.906` → IC=+0.300 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 10351.906 (IC base=+0.258)

- **PATRÓN** `ballena_activa_n` < `221.0` → IC=+0.265 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 221.0 (IC base=+0.258)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.208 (n=546)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0068 (IC base=+0.124)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.146 (n=806)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9655` → IC=+0.290 (n=740)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9655 (IC base=+0.124)

- **PATRÓN** `dist_vwap_pct` > `0.3609` → IC=+0.204 (n=204)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3609 (IC base=+0.124)
...
- **PATRÓN** `sigma_h` < `0.0027` → IC=+0.170 (n=116)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0027 (IC base=+0.143)

- **PATRÓN** `sigma_h` > `0.0032` → IC=+0.188 (n=78)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0032 (IC base=+0.143)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.180 (n=176)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 5.0 (IC base=+0.143)

- **PATRÓN** `ibs_20min` > `0.3547` → IC=+0.257 (n=171)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3547 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` > `0.2594` → IC=+0.297 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2594 (IC base=+0.143)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.391` → IC=+0.278 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.391 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` < `0.6819` → IC=+0.167 (n=58)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.6819 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` > `0.9291` → IC=+0.155 (n=114)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 0.9291 (IC base=+0.143)

- **PATRÓN** `volumen_pendiente_norm` < `0.1584` → IC=+0.198 (n=127)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` < 0.1584 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` < `2.8769` → IC=+0.238 (n=124)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.8769 (IC base=+0.143)

- **PATRÓN** `libro_liquidez` > `7424.0996` → IC=+0.215 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7424.0996 (IC base=+0.143)

- **PATRÓN** `ballena_activa_n` < `511.0` → IC=+0.255 (n=47)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 511.0 (IC base=+0.143)
...
- **FILTRO** `ibs_20min` < `0.3782` → IC=-0.183 (n=58)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.226 (n=177)

- **PATRÓN** `sigma_h` < `0.0019` → IC=+0.177 (n=60)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0019 (IC base=+0.124)

- **PATRÓN** `sigma_h` > `0.0043` → IC=+0.156 (n=59)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` > 0.0043 (IC base=+0.124)

- **PATRÓN** `drift_60min` |x|≤ `0.0548` → IC=+0.156 (n=59)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.0548 (IC base=+0.124)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.149 (n=166)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 8.0 (IC base=+0.124)

- **PATRÓN** `ibs_20min` > `0.3782` → IC=+0.226 (n=177)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3782 (IC base=+0.124)

- **PATRÓN** `dist_vwap_pct` > `0.4382` → IC=+0.235 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4382 (IC base=+0.124)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.344` → IC=+0.208 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.344 (IC base=+0.124)

- **PATRÓN** `volumen_regimen` < `0.7843` → IC=+0.163 (n=78)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.7843 (IC base=+0.124)

- **PATRÓN** `volumen_regimen` > `1.1345` → IC=+0.171 (n=80)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 1.1345 (IC base=+0.124)

- **PATRÓN** `volumen_pendiente_norm` > `0.29` → IC=+0.321 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.29 (IC base=+0.124)

- **PATRÓN** `volumen_spike_ratio` > `1.981` → IC=+0.258 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.981 (IC base=+0.124)

- **PATRÓN** `libro_liquidez` > `7711.5929` → IC=+0.293 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7711.5929 (IC base=+0.124)

- **PATRÓN** `ballena_activa_n` < `219.0` → IC=+0.304 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 219.0 (IC base=+0.124)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.144 (n=57)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 12.0 (IC base=+0.084)

- **PATRÓN** `ibs_20min` < `0.9851` → IC=+0.173 (n=53)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.9851 (IC base=+0.084)
...
- **FILTRO** `ibs_20min` > `0.9065` → IC=-0.214 (n=19)

  - _Acción_: SKIP cuando `ibs_20min` > 0.9065
  - _Potencial_: sin este filtro IC_bueno=+0.115 (n=37)

### MOMENTUM_IBS_15M#ETH#15min
- **PATRÓN** `hora_utc` > `14.0` → IC=+0.134 (n=39)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 14.0 (IC base=+0.070)

- **PATRÓN** `hora_utc` < `2.0` → IC=+0.188 (n=30)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` < 2.0 (IC base=+0.070)
...
- **FILTRO** `ibs_20min` < `0.7241` → IC=-0.193 (n=151)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=456)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.173 (n=151)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.185 (n=363)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=1270)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.273 (n=386)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=1247)

- **FILTRO** `ibs_7min` < `0.7222` → IC=-0.234 (n=408)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7222
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=1225)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.185 (n=553)

  - _Acción_: SKIP cuando `ballena_activa_n` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=1080)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.186 (n=479)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=+0.010 (n=1518)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.144 (n=130)
...
- **FILTRO** `ibs_7min` < `0.7638` → IC=-0.194 (n=83)
...
- **FILTRO** `py_entrada` < `0.31` → IC=-0.362 (n=56)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=168)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0