# Hipótesis automáticas — 2026-09-16 21:31 UTC
_Generado por shadow_postmortem.py sobre 472001 resoluciones (PNL=+50741.19€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `144.23` → IC=-0.252 (n=5828)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 144.23
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=17501)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `139.02` → IC=-0.276 (n=800)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 139.02
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=2403)
...
- **FILTRO** `restante_s_al_confirmar` < `158.06` → IC=-0.252 (n=1387)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.06
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=4163)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `118.93` → IC=-0.362 (n=1122)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 118.93
  - _Potencial_: sin este filtro IC_bueno=-0.103 (n=3369)

### CANDIDATA9_BOT_CONSENSO
- **FILTRO** `py_entrada` < `0.47` → IC=-0.244 (n=260)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=276)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.143 (n=124)
...
- **FILTRO** `py_entrada` < `0.48` → IC=-0.257 (n=134)

  - _Acción_: SKIP cuando `py_entrada` < 0.48
  - _Potencial_: sin este filtro IC_bueno=+0.006 (n=154)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.156 (n=59)
...
- **PATRÓN** `py_entrada` > `0.745` → IC=+0.347 (n=625)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.745 (IC base=+0.201)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.202 (n=1718)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.201)

- **PATRÓN** `libro_liquidez` > `15002.7092` → IC=+0.208 (n=444)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 15002.7092 (IC base=+0.201)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.208 (n=1267)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.375` → IC=+0.264 (n=1272)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.375 (IC base=+0.201)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.203 (n=1783)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.201)

- **PATRÓN** `libro_liquidez` > `13207.9317` → IC=+0.215 (n=626)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13207.9317 (IC base=+0.201)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.147 (n=2371)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.327 (n=789)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.223 (n=518)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.202 (n=9301)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.220 (n=3290)
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.178 (n=2298)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.314 (n=202)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.183 (n=2157)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.185 (n=1940)

  - _Acción_: Kelly boost +0.92€ cuando `py_entrada` > 0.71 (IC base=+0.178)

### FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.247 (n=2041)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.318 (n=681)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.200 (n=2208)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.195 (n=1590)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.187 (n=5072)

  - _Acción_: Kelly boost +0.94€ cuando `py_entrada` > 0.71 (IC base=+0.170)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.225 (n=4913)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `2.49` → IC=+0.150 (n=1930)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.4986` → IC=+0.155 (n=5602)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` > 0.4986 (IC base=+0.096)

- **PATRÓN** `dist_vwap_pct` < `0.2156` → IC=+0.245 (n=1333)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2156 (IC base=+0.096)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.866` → IC=+0.168 (n=2421)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` > 5.866 (IC base=+0.096)

- **PATRÓN** `volumen_regimen` < `0.6092` → IC=+0.251 (n=533)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1746` → IC=+0.191 (n=4299)
...
- **PATRÓN** `volumen_spike_ratio` > `1.9238` → IC=+0.197 (n=2758)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.9238 (IC base=+0.096)

- **PATRÓN** `ibs_20min` < `0.5687` → IC=+0.130 (n=7695)

  - _Acción_: Kelly boost +0.65€ cuando `ibs_20min` < 0.5687 (IC base=+0.059)

- **PATRÓN** `dist_vwap_pct` > `0.5754` → IC=+0.186 (n=456)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.5754 (IC base=+0.059)

- **PATRÓN** `dist_vwap_pct` < `0.3466` → IC=+0.168 (n=2678)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` < 0.3466 (IC base=+0.059)

- **PATRÓN** `volumen_regimen` < `0.6995` → IC=+0.169 (n=1107)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 0.6995 (IC base=+0.059)

- **PATRÓN** `volumen_regimen` > `0.8713` → IC=+0.176 (n=1676)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 0.8713 (IC base=+0.059)

- **PATRÓN** `volumen_pendiente_norm` > `0.2442` → IC=+0.223 (n=846)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2442 (IC base=+0.059)

- **PATRÓN** `volumen_spike_ratio` > `1.4676` → IC=+0.197 (n=4185)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.4676 (IC base=+0.059)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.209 (n=3957)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3242` → IC=+0.164 (n=1414)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.067` → IC=+0.277 (n=617)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.067 (IC base=+0.163)
...
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.154 (n=1011)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.164 (n=388)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 18.0 (IC base=+0.137)

- **PATRÓN** `ibs_20min` < `0.6723` → IC=+0.173 (n=1147)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.6723 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.1278` → IC=+0.150 (n=1045)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.1278 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.49` → IC=+0.168 (n=194)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` > 11.49 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `1.2085` → IC=+0.144 (n=1147)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.2085 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` > `0.8477` → IC=+0.140 (n=764)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 0.8477 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` > `0.1571` → IC=+0.183 (n=310)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.1571 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` < `2.4292` → IC=+0.153 (n=1038)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 2.4292 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `13499.0387` → IC=+0.150 (n=764)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 13499.0387 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `423.0` → IC=+0.145 (n=953)

  - _Acción_: Kelly boost +0.73€ cuando `ballena_activa_n` < 423.0 (IC base=+0.137)

### GBM_LATE_15M#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0058` → IC=+0.189 (n=1380)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0058 (IC base=+0.177)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.177 (n=1384)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.566` → IC=+0.212 (n=791)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.566 (IC base=+0.177)

- **PATRÓN** `volumen_pendiente_norm` < `0.1063` → IC=+0.182 (n=1175)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.152 (n=90)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=1744)
...
- **PATRÓN** `dist_vwap_pct` > `0.1669` → IC=+0.190 (n=195)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.1669 (IC base=+0.003)

- **PATRÓN** `volumen_regimen` < `0.8602` → IC=+0.149 (n=383)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 0.8602 (IC base=+0.003)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5087` → IC=+0.187 (n=467)
...
- **FILTRO** `ibs_20min` > `0.2727` → IC=-0.129 (n=1765)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2727
  - _Potencial_: sin este filtro IC_bueno=+0.119 (n=880)

- **FILTRO** `sigma_ewma_delta_pct` > `8.635` → IC=-0.204 (n=289)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.635
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=2356)
...
- **FILTRO** `drift_60min` |x|> `0.6585` → IC=-0.194 (n=439)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6585
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=1318)

- **FILTRO** `ibs_20min` < `0.79` → IC=-0.141 (n=1316)

  - _Acción_: SKIP cuando `ibs_20min` < 0.79
  - _Potencial_: sin este filtro IC_bueno=+0.082 (n=441)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.201 (n=379)

  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=1378)
...
- **PATRÓN** `dist_vwap_pct` > `1.0412` → IC=+0.271 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0412 (IC base=-0.027)

- **PATRÓN** `dist_vwap_pct` < `0.2677` → IC=+0.245 (n=563)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2677 (IC base=-0.027)

- **PATRÓN** `volumen_regimen` < `0.7337` → IC=+0.242 (n=242)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4768` → IC=+0.244 (n=1418)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4768 (IC base=+0.086)

- **PATRÓN** `volumen_spike_ratio` > `2.771` → IC=+0.234 (n=1417)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.771 (IC base=+0.086)

- **PATRÓN** `ballena_activa_n` < `102.0` → IC=+0.277 (n=3737)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 102.0 (IC base=+0.086)

- **PATRÓN** `sigma_h` > `0.0085` → IC=+0.138 (n=2663)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0085 (IC base=+0.068)

- **PATRÓN** `ibs_20min` < `0.5556` → IC=+0.150 (n=7016)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` < 0.5556 (IC base=+0.068)

- **PATRÓN** `dist_vwap_pct` > `0.686` → IC=+0.242 (n=389)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.686 (IC base=+0.068)

- **PATRÓN** `dist_vwap_pct` < `0.2319` → IC=+0.230 (n=2131)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2319 (IC base=+0.068)

- **PATRÓN** `volumen_regimen` < `0.7148` → IC=+0.230 (n=965)
...
- **FILTRO** `sigma_ewma_delta_pct` > `4.415` → IC=-0.166 (n=393)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=1317)

- **PATRÓN** `ibs_20min` > `0.8621` → IC=+0.246 (n=541)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4427` → IC=+0.197 (n=183)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 1.4427 (IC base=+0.036)
...
- **FILTRO** `ibs_20min` > `0.8266` → IC=-0.149 (n=588)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=1766)

- **PATRÓN** `dist_vwap_pct` > `0.2998` → IC=+0.136 (n=256)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 0.2998 (IC base=+0.012)

- **PATRÓN** `dist_vwap_pct` < `0.1641` → IC=+0.130 (n=592)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.1641 (IC base=+0.012)
...
- **PATRÓN** `volumen_regimen` > `0.6495` → IC=+0.139 (n=616)
  ...
  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 0.6495 (IC base=+0.012)

- **PATRÓN** `volumen_pendiente_norm` > `0.2196` → IC=+0.175 (n=124)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.2196 (IC base=+0.012)

- **PATRÓN** `volumen_spike_ratio` < `1.4208` → IC=+0.177 (n=224)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 1.4208 (IC base=+0.012)

- **PATRÓN** `ballena_activa_n` < `230.0` → IC=+0.186 (n=221)

  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 230.0 (IC base=+0.012)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.316 (n=340)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.278)

- **PATRÓN** `ibs_20min` < `0.3333` → IC=+0.283 (n=1000)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3333 (IC base=+0.278)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.774` → IC=+0.294 (n=382)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.774 (IC base=+0.278)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.611` → IC=+0.278 (n=1073)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.611 (IC base=+0.278)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2358` → IC=+0.279 (n=601)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2358 (IC base=+0.278)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.284 (n=531)
...
- **PATRÓN** `ballena_activa_n` < `21.0` → IC=+0.271 (n=383)
...
- **FILTRO** `ibs_20min` > `0.8086` → IC=-0.184 (n=469)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.036 (n=1411)
...
- **FILTRO** `ibs_20min` < `0.6712` → IC=-0.208 (n=850)
...
- **FILTRO** `ibs_20min` > `0.7115` → IC=-0.234 (n=449)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.082 (n=1351)

- **FILTRO** `sigma_ewma_delta_pct` > `4.689` → IC=-0.175 (n=420)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.689
  - _Potencial_: sin este filtro IC_bueno=+0.058 (n=1380)
...
- **PATRÓN** `dist_vwap_pct` > `0.5846` → IC=+0.204 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5846 (IC base=+0.003)

- **PATRÓN** `dist_vwap_pct` < `0.3939` → IC=+0.191 (n=393)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` < 0.3939 (IC base=+0.003)

- **PATRÓN** `volumen_regimen` < `0.7148` → IC=+0.246 (n=167)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7148 (IC base=+0.003)

- **PATRÓN** `volumen_pendiente_norm` < `0.1003` → IC=+0.187 (n=337)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` < 0.1003 (IC base=+0.003)

- **PATRÓN** `volumen_pendiente_norm` > `0.0701` → IC=+0.205 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0701 (IC base=+0.003)

- **PATRÓN** `volumen_spike_ratio` < `2.5839` → IC=+0.208 (n=347)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5839 (IC base=+0.003)

- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.226 (n=352)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5387` → IC=+0.264 (n=1004)
...
- **PATRÓN** `libro_liquidez` > `2575.228` → IC=+0.277 (n=778)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2575.228 (IC base=+0.269)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3355` → IC=+0.172 (n=5446)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.3355 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.177 (n=6468)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 5.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` > `0.5829` → IC=+0.214 (n=6188)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.226` → IC=+0.249 (n=1275)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0795` → IC=+0.203 (n=2007)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0795 (IC base=+0.171)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.199 (n=2894)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.171)

- **PATRÓN** `ibs_20min` < `0.4632` → IC=+0.228 (n=6015)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4632 (IC base=+0.171)

- **PATRÓN** `dist_vwap_pct` < `0.223` → IC=+0.161 (n=4493)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.223 (IC base=+0.171)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.239` → IC=+0.195 (n=1037)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 10.239 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` < `1.1891` → IC=+0.156 (n=4399)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.1891 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` > `0.6252` → IC=+0.150 (n=4399)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5745` → IC=+0.174 (n=2357)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.5745 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` > `2.6483` → IC=+0.175 (n=1786)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.6483 (IC base=+0.171)

- **PATRÓN** `ballena_activa_n` < `123.0` → IC=+0.173 (n=4974)
...
- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.203 (n=477)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0072 (IC base=+0.186)

- **PATRÓN** `drift_60min` |x|≤ `0.3176` → IC=+0.201 (n=1049)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3176 (IC base=+0.186)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.067` → IC=+0.307 (n=479)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.067 (IC base=+0.186)

- **PATRÓN** `volumen_pendiente_norm` > `0.2294` → IC=+0.239 (n=201)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2294 (IC base=+0.186)

- **PATRÓN** `volumen_spike_ratio` < `2.5472` → IC=+0.179 (n=955)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 2.5472 (IC base=+0.186)

- **PATRÓN** `volumen_spike_ratio` > `1.427` → IC=+0.179 (n=954)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 1.427 (IC base=+0.186)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.206 (n=983)
...
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.167 (n=1027)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.162 (n=950)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 7.0 (IC base=+0.146)

- **PATRÓN** `ibs_20min` < `0.535` → IC=+0.189 (n=1027)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` < 0.535 (IC base=+0.146)
...
- **PATRÓN** `volumen_regimen` < `1.2145` → IC=+0.160 (n=1027)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2145 (IC base=+0.146)

- **PATRÓN** `volumen_pendiente_norm` > `0.1584` → IC=+0.177 (n=317)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.1584 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` < `2.4267` → IC=+0.159 (n=916)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.4267 (IC base=+0.146)

- **PATRÓN** `ballena_activa_n` < `229.0` → IC=+0.157 (n=278)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 229.0 (IC base=+0.146)

### GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0059` → IC=+0.198 (n=1026)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0059 (IC base=+0.190)

- **PATRÓN** `drift_60min` |x|≤ `0.1968` → IC=+0.201 (n=683)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1968 (IC base=+0.190)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.220 (n=352)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_pendiente_norm` < `0.216` → IC=+0.186 (n=983)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` < 0.216 (IC base=+0.190)
...
- **PATRÓN** `volumen_spike_ratio` < `1.673` → IC=+0.189 (n=320)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` < 1.673 (IC base=+0.190)

- **PATRÓN** `volumen_spike_ratio` > `2.3431` → IC=+0.197 (n=638)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 2.3431 (IC base=+0.190)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.203 (n=1160)
...
- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.164 (n=1055)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0072 (IC base=+0.130)

- **PATRÓN** `drift_60min` |x|≤ `0.3801` → IC=+0.146 (n=1055)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.3801 (IC base=+0.130)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.177 (n=413)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 17.0 (IC base=+0.130)

- **PATRÓN** `ibs_20min` < `0.6027` → IC=+0.180 (n=1055)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` < 0.6027 (IC base=+0.130)
...
- **PATRÓN** `volumen_regimen` < `0.8585` → IC=+0.142 (n=704)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 0.8585 (IC base=+0.130)

- **PATRÓN** `volumen_regimen` > `0.6094` → IC=+0.132 (n=1055)
...
- **PATRÓN** `libro_liquidez` > `10007.3401` → IC=+0.159 (n=479)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 10007.3401 (IC base=+0.130)

- **PATRÓN** `ballena_activa_n` < `177.0` → IC=+0.127 (n=869)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.169 (n=532)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 15.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` < `0.55` → IC=+0.208 (n=1154)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.55 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.1871` → IC=+0.135 (n=1070)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.1871 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.383` → IC=+0.160 (n=245)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 7.383 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` < `1.0407` → IC=+0.127 (n=1016)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` < 1.0407 (IC base=+0.113)
...
- **PATRÓN** `volumen_spike_ratio` < `1.462` → IC=+0.137 (n=337)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` < 1.462 (IC base=+0.113)

- **PATRÓN** `volumen_spike_ratio` > `2.1766` → IC=+0.133 (n=458)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 2.1766 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `3082.2041` → IC=+0.164 (n=385)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 3082.2041 (IC base=+0.113)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.219 (n=575)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.029` → IC=+0.308 (n=513)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.029 (IC base=+0.205)

- **PATRÓN** `dist_vwap_pct` > `1.1744` → IC=+0.226 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1744 (IC base=+0.205)

- **PATRÓN** `dist_vwap_pct` < `0.2695` → IC=+0.205 (n=1215)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2695 (IC base=+0.205)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.179 (n=756)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_regimen` < `0.8626` → IC=+0.154 (n=862)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4312` → IC=+0.148 (n=478)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.4312 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` > `2.5597` → IC=+0.158 (n=478)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 2.5597 (IC base=+0.135)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.139 (n=1664)

  - _Acción_: Kelly boost +0.70€ cuando `libro_spread` < 0.02 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `8478.1762` → IC=+0.149 (n=679)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 8478.1762 (IC base=+0.135)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.158 (n=1277)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.128 (n=1611)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_regimen` < `1.2242` → IC=+0.121 (n=1409)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` < 1.2242 (IC base=+0.115)

- **PATRÓN** `volumen_pendiente_norm` > `0.1669` → IC=+0.143 (n=404)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_pendiente_norm` > 0.1669 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` < `2.2286` → IC=+0.136 (n=1338)
...
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.130 (n=406)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` < 0.0067 (IC base=+0.108)

- **PATRÓN** `drift_60min` |x|≤ `0.0973` → IC=+0.159 (n=136)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.0973 (IC base=+0.108)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.137 (n=276)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 11.0 (IC base=+0.108)

- **PATRÓN** `ibs_20min` < `0.3025` → IC=+0.163 (n=271)

  - _Acción_: Kelly boost +0.82€ cuando `ibs_20min` < 0.3025 (IC base=+0.108)
...
- **PATRÓN** `volumen_regimen` < `1.2294` → IC=+0.120 (n=406)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_regimen` < 1.2294 (IC base=+0.108)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8175` → IC=+0.133 (n=257)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 1.8175 (IC base=+0.108)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.699` → IC=+0.183 (n=6965)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.699 (IC base=+0.181)

- **PATRÓN** `volumen_regimen` < `0.7038` → IC=+0.159 (n=2193)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.7038 (IC base=+0.181)

- **PATRÓN** `volumen_regimen` > `1.2046` → IC=+0.158 (n=1661)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 1.2046 (IC base=+0.181)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2995` → IC=+0.189 (n=2934)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` > 2.2995 (IC base=+0.181)

- **PATRÓN** `ballena_activa_n` < `124.0` → IC=+0.178 (n=6077)
...
- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.232 (n=599)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0072 (IC base=+0.192)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.198 (n=643)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.891` → IC=+0.312 (n=599)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.891 (IC base=+0.192)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2625` → IC=+0.193 (n=559)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 2.2625 (IC base=+0.192)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.218 (n=1216)
...
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.169 (n=1116)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0058 (IC base=+0.153)

- **PATRÓN** `drift_60min` |x|≤ `0.2605` → IC=+0.167 (n=980)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.2605 (IC base=+0.153)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.172 (n=507)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 16.0 (IC base=+0.153)

- **PATRÓN** `ibs_20min` < `0.6389` → IC=+0.203 (n=1114)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6389 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` > `0.6991` → IC=+0.161 (n=169)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.6991 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` < `0.1309` → IC=+0.163 (n=1029)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.1309 (IC base=+0.153)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.226` → IC=+0.165 (n=553)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 3.226 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` < `1.2003` → IC=+0.161 (n=1114)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2176` → IC=+0.216 (n=1205)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2176 (IC base=+0.210)

- **PATRÓN** `volumen_spike_ratio` > `2.9939` → IC=+0.233 (n=530)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9939 (IC base=+0.210)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.224 (n=1424)
...
- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.233 (n=541)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0091 (IC base=+0.232)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.257 (n=451)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_spike_ratio` > `2.2488` → IC=+0.230 (n=712)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2488 (IC base=+0.232)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.243 (n=648)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.232)

- **PATRÓN** `libro_liquidez` > `1890.8984` → IC=+0.237 (n=398)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1890.8984 (IC base=+0.232)

- **PATRÓN** `ballena_activa_n` < `15.0` → IC=+0.255 (n=329)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4312` → IC=+0.140 (n=1327)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.70€ cuando `drift_60min` |x|≤ 0.4312 (IC base=+0.135)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.147 (n=1394)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.224` → IC=+0.162 (n=563)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 4.224 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` < `0.8789` → IC=+0.158 (n=885)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.8789 (IC base=+0.135)

- **PATRÓN** `volumen_pendiente_norm` > `0.2748` → IC=+0.226 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2748 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` < `1.5044` → IC=+0.149 (n=562)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.5044 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` > `2.4575` → IC=+0.157 (n=426)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 2.4575 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `8808.1502` → IC=+0.229 (n=602)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8808.1502 (IC base=+0.135)

- **PATRÓN** `ballena_activa_n` < `87.0` → IC=+0.168 (n=404)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4405` → IC=+0.158 (n=1074)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.4405 (IC base=+0.138)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.171 (n=408)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.6884` → IC=+0.192 (n=1073)
...
- **PATRÓN** `volumen_regimen` < `0.695` → IC=+0.146 (n=472)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.695 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `1.1921` → IC=+0.147 (n=358)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` > 1.1921 (IC base=+0.138)

- **PATRÓN** `volumen_pendiente_norm` > `0.2782` → IC=+0.266 (n=126)
...
- **PATRÓN** `ballena_activa_n` < `192.0` → IC=+0.144 (n=996)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1253` → IC=+0.154 (n=434)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.1253 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.146 (n=611)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 15.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` < `0.6364` → IC=+0.207 (n=1304)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6364 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.4691` → IC=+0.130 (n=1286)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.4691 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.446` → IC=+0.126 (n=1253)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 3.446 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` < `0.7204` → IC=+0.152 (n=573)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.7204 (IC base=+0.113)

- **PATRÓN** `volumen_pendiente_norm` > `0.2225` → IC=+0.167 (n=202)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.2225 (IC base=+0.113)

- **PATRÓN** `volumen_spike_ratio` < `1.4615` → IC=+0.148 (n=382)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.4615 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `2904.3214` → IC=+0.165 (n=434)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 2904.3214 (IC base=+0.113)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` > `0.0184` → IC=+0.212 (n=904)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0184 (IC base=+0.206)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.212 (n=1417)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.206)

- **PATRÓN** `ibs_20min` > `0.5116` → IC=+0.244 (n=1356)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5116 (IC base=+0.206)

- **PATRÓN** `dist_vwap_pct` > `0.1869` → IC=+0.233 (n=763)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1869 (IC base=+0.206)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.355` → IC=+0.248 (n=657)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.355 (IC base=+0.206)

- **PATRÓN** `volumen_regimen` < `1.2478` → IC=+0.209 (n=1356)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2478 (IC base=+0.206)

- **PATRÓN** `volumen_regimen` > `0.6268` → IC=+0.209 (n=1356)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.214 (n=1370)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.206)

- **PATRÓN** `libro_liquidez` > `2574.4443` → IC=+0.212 (n=904)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2324` → IC=+0.193 (n=1159)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 2.2324 (IC base=+0.201)

- **PATRÓN** `volumen_spike_ratio` > `1.4437` → IC=+0.198 (n=1317)
...
- **PATRÓN** `libro_liquidez` > `2549.3704` → IC=+0.202 (n=998)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2549.3704 (IC base=+0.201)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.175 (n=229)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 18.0 (IC base=+0.106)

- **PATRÓN** `ibs_20min` > `0.649` → IC=+0.221 (n=578)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.649 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` > `0.1318` → IC=+0.177 (n=298)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.1318 (IC base=+0.106)
...
- **PATRÓN** `volumen_regimen` < `1.0933` → IC=+0.129 (n=578)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 1.0933 (IC base=+0.106)
...
- **PATRÓN** `volumen_spike_ratio` < `2.456` → IC=+0.161 (n=470)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.456 (IC base=+0.106)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.140 (n=468)

  - _Acción_: Kelly boost +0.70€ cuando `libro_spread` < 0.02 (IC base=+0.106)

- **PATRÓN** `libro_liquidez` > `2445.5482` → IC=+0.160 (n=251)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 2445.5482 (IC base=+0.106)
...
- **PATRÓN** `sigma_h` < `0.006` → IC=+0.180 (n=229)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` > `0.592` → IC=+0.210 (n=198)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.592 (IC base=+0.109)

- **PATRÓN** `dist_vwap_pct` > `0.1301` → IC=+0.201 (n=95)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1301 (IC base=+0.109)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.535` → IC=+0.135 (n=50)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` > 13.535 (IC base=+0.109)

- **PATRÓN** `volumen_regimen` < `1.2106` → IC=+0.137 (n=199)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 1.2106 (IC base=+0.109)

- **PATRÓN** `volumen_pendiente_norm` < `0.0759` → IC=+0.164 (n=141)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` < 0.0759 (IC base=+0.109)

- **PATRÓN** `volumen_pendiente_norm` > `0.2541` → IC=+0.208 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2541 (IC base=+0.109)

- **PATRÓN** `volumen_spike_ratio` < `2.0118` → IC=+0.199 (n=141)
...
- **PATRÓN** `libro_liquidez` > `2859.3011` → IC=+0.131 (n=185)

  - _Acción_: Kelly boost +0.66€ cuando `libro_liquidez` > 2859.3011 (IC base=+0.109)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.152 (n=208)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 8.0 (IC base=+0.126)
...
- **PATRÓN** `dist_vwap_pct` > `0.1208` → IC=+0.195 (n=103)
  ...
  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1208 (IC base=+0.126)
...
- **PATRÓN** `volumen_regimen` < `0.791` → IC=+0.169 (n=134)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 0.791 (IC base=+0.126)

- **PATRÓN** `volumen_regimen` > `0.6236` → IC=+0.146 (n=179)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 0.6236 (IC base=+0.126)

- **PATRÓN** `volumen_pendiente_norm` > `0.3002` → IC=+0.250 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3002 (IC base=+0.126)

- **PATRÓN** `volumen_spike_ratio` < `1.7387` → IC=+0.189 (n=101)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` < 1.7387 (IC base=+0.126)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.156 (n=210)

  - _Acción_: Kelly boost +0.78€ cuando `libro_spread` < 0.02 (IC base=+0.126)

- **PATRÓN** `libro_liquidez` > `1103.6894` → IC=+0.195 (n=175)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.123 (n=1055)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=5511)

- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=6418)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.121 (n=27)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=1372)

### MOMENTUM_IBS_15M_BALLENA
- **FILTRO** `py_entrada` < `0.47` → IC=-0.179 (n=2699)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=8200)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.172 (n=2793)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.033 (n=8585)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.43` → IC=-0.220 (n=441)

  - _Acción_: SKIP cuando `py_entrada` < 0.43
  - _Potencial_: sin este filtro IC_bueno=+0.092 (n=1400)

- **FILTRO** `ibs_20min` < `0.7482` → IC=-0.180 (n=460)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7482
  - _Potencial_: sin este filtro IC_bueno=+0.083 (n=1381)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.166 (n=471)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=+0.054 (n=1559)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.43` → IC=-0.203 (n=462)

  - _Acción_: SKIP cuando `py_entrada` < 0.43
  - _Potencial_: sin este filtro IC_bueno=+0.084 (n=1427)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.217 (n=479)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.061 (n=1525)
...
- **FILTRO** `py_entrada` > `0.58` → IC=-0.193 (n=497)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=1507)

### MOMENTUM_IBS_15M_FADE
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=2428)

- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)

  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=2464)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=2470)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=7781)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.082 (n=17810)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.283 (n=6000)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=19591)

- **FILTRO** `ibs_7min` < `0.703` → IC=-0.242 (n=6395)

  - _Acción_: SKIP cuando `ibs_7min` < 0.703
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=19196)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.159 (n=8698)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=16893)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.224 (n=7984)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=24074)

- **FILTRO** `ibs_7min` > `0.2963` → IC=-0.177 (n=7999)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2963
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=24059)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.312 (n=972)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=3152)

- **FILTRO** `ibs_7min` < `0.7089` → IC=-0.256 (n=1360)
...
- **FILTRO** `ballena_activa_n` > `9.0` → IC=-0.195 (n=966)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=3158)

- **FILTRO** `py_entrada` > `0.505` → IC=-0.150 (n=3730)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.101 (n=1841)

- **FILTRO** `drift_7min_pct` |x|> `0.1369` → IC=-0.133 (n=1392)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1369
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=4179)

- **FILTRO** `ibs_7min` > `0.7992` → IC=-0.206 (n=1392)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7992
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=4179)
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.257 (n=1047)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=3424)

- **FILTRO** `ibs_7min` < `0.7555` → IC=-0.187 (n=1117)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7555
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=3354)

- **FILTRO** `ballena_activa_n` > `161.0` → IC=-0.173 (n=1115)

  - _Acción_: SKIP cuando `ballena_activa_n` > 161.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3356)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.255 (n=1068)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=3458)

- **FILTRO** `ibs_7min` > `0.2531` → IC=-0.168 (n=1131)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2531
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=3395)

- **FILTRO** `ballena_activa_n` > `152.0` → IC=-0.178 (n=1129)

  - _Acción_: SKIP cuando `ballena_activa_n` > 152.0
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=3397)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.182 (n=937)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.094 (n=2918)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.321 (n=929)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=2926)

- **FILTRO** `drift_7min_pct` |x|> `0.1812` → IC=-0.132 (n=1310)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1812
  - _Potencial_: sin este filtro IC_bueno=-0.107 (n=2545)

- **FILTRO** `ibs_7min` < `0.2031` → IC=-0.274 (n=963)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2031
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=2892)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.221 (n=907)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=2948)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.235 (n=1382)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.016 (n=4487)

- **FILTRO** `ibs_7min` > `0.2618` → IC=-0.155 (n=1995)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2618
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=3874)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=1324)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.089 (n=2887)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.252 (n=1026)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3185)

- **FILTRO** `ibs_7min` < `0.7433` → IC=-0.193 (n=1052)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7433
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3159)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.182 (n=1040)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.075 (n=3171)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.260 (n=1062)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=3245)

- **FILTRO** `ibs_7min` > `0.2739` → IC=-0.174 (n=1076)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2739
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=3231)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.182 (n=1066)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=3241)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.37` → IC=-0.255 (n=1102)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=3507)

- **FILTRO** `ibs_7min` < `0.7187` → IC=-0.221 (n=1149)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7187
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=3460)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.170 (n=1462)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=4610)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.35` → IC=-0.282 (n=1055)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=3266)

- **FILTRO** `ibs_7min` < `0.725` → IC=-0.232 (n=1080)

  - _Acción_: SKIP cuando `ibs_7min` < 0.725
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=3241)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.210 (n=1056)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3265)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.207 (n=1354)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=4359)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=2368)
...
- **PATRÓN** `drift_60min` |x|≤ `0.077` → IC=+0.178 (n=485)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.89€ cuando `drift_60min` |x|≤ 0.077 (IC base=+0.174)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0585` → IC=+0.176 (n=1102)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1344` → IC=+0.225 (n=344)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1344 (IC base=+0.174)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.178 (n=785)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` > `0.6216` → IC=+0.245 (n=1102)
...
- **PATRÓN** `dist_vwap_pct` > `0.2646` → IC=+0.170 (n=395)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.2646 (IC base=+0.174)

- **PATRÓN** `dist_vwap_pct` < `0.1047` → IC=+0.178 (n=733)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.1047 (IC base=+0.174)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.764` → IC=+0.241 (n=519)
...
- **PATRÓN** `ibs_15` < `0.1111` → IC=+0.179 (n=350)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1175` → IC=+0.236 (n=772)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1175 (IC base=-0.044)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1819` → IC=+0.239 (n=739)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1819 (IC base=-0.044)

- **PATRÓN** `ibs_15` < `0.3557` → IC=+0.280 (n=1158)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3557 (IC base=-0.044)

- **PATRÓN** `dist_vwap_pct` > `0.6614` → IC=+0.277 (n=173)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6614 (IC base=-0.044)
...
- **PATRÓN** `dist_vwap_pct` > `1.1135` → IC=+0.419 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1135 (IC base=-0.045)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.
63€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 6957 | +0.059 | +1114.30€ | 0 | 12 |
| ✅ UPDOWN_GBM#240min | 977 | +0.005 | +8.77€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 16342 | +0.023 | +318.97€ | 0 | 0 |
| ✅ UPDOWN_GBM#60min | 1573 | -0.002 | -4.05€ | 1 | 0 |
| ✅ UPDOWN_GBM#BNB | 2267 | +0.071 | +216.84€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 278 | +0.125 | +88.02€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#240min | 19 | -0.023 | -0.57€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#5min | 1970 | +0.064 | +129.39€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 4678 | +0.031 | +305.45€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 851 | +0.082 | +193.77€ | 0 | 12 |
| ✅ UPDOWN_GBM#BTC#240min | 277 | +0.023 | +7.27€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#5min | 2815 | +0.026 | +97.98€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#60min | 695 | -0.001 | +5.25€ | 0 | 1 |
| ✅ UPDOWN_GBM#BTC#daily | 40 | -0.119 | +1.18€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 3052 | +0.032 | +97.51€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 235 | +0.112 | +59.51€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#240min | 16 | +0.000 | -0.34€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 2801 | +0.025 | +38.34€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 5331 | +0.016 | +214.85€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 1913 | +0.046 | +213.72€ | 0 | 11 |
| ✅ UPDOWN_GBM#ETH#240min | 264 | +0.007 | +8.09€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 2557 | +0.002 | -4.14€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 562 | -0.005 | -6.70€ | 2 | 0 |
| ✅ UPDOWN_GBM#ETH#daily | 35 | -0.149 | +3.89€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 6710 | +0.017 | +159.99€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 1866 | +0.024 | +110.04€ | 1 | 11 |
| ✅ UPDOWN_GBM#SOL#240min | 258 | -0.008 | -2.47€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 4241 | +0.017 | +55.61€ | 1 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 316 | +0.003 | -2.61€ | 0 | 1 |
| ✅ UPDOWN_GBM#SOL#daily | 29 | -0.145 | -0.59€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 3915 | +0.042 | +447.82€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 1814 | +0.080 | +449.25€ | 0 | 11 |
| ✅ UPDOWN_GBM#XRP#240min | 143 | -0.010 | -3.21€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 1958 | +0.011 | +1.77€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 9149 | -0.048 | +1955.50€ | 3 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 9149 | -0.048 | +1955.50€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 2928 | -0.062 | +460.68€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 2928 | -0.062 | +460.68€ | 2 | 5 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 2908 | -0.075 | +492.37€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 2908 | -0.075 | +492.37€ | 2 | 3 |
...
**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  ...
  - _Estado_: 456 celda(s) pasan gate riguroso completo de 2051 evaluadas (n>=40) y 3020 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.024 < 0.08 — monitorear
  - _Datos_: n=1863 IC=+0.024 PNL=+109.63€
...
**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  ...
  - _Estado_: alineada_con_outcome_prev IC=+0.095 n=188/60 | contraria IC=+0.131 n=166 | gap=-0.036 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=561/40 IC=-0.006 PNL=-7.18€ | BTC#60min: n=693/40 IC=-0.001 PNL=+5.28€ | SOL#60min: n=315/40 IC=+0.002 PNL=-3.11€

**⏳ H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  - _Umbral_: n≥40 tras 2 losses y gap(IC_tras_win - IC_tras_2loss)≥0.05
  - _Acción_: Reducir stake (no desactivar) 1-2h tras 2 derrotas consecutivas en el mismo subtype
  - _Estado_: tras_win IC=+0.053 n=264495 | tras_1loss IC=+0.072 n=206774 | tras_2loss IC=+0.040 n=88500/40 | gap=+0.013 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-60MIN-BUYYES** — GBM 60min BUY_YES — ¿edge superior al BUY_NO?
  ...
  - _Estado_: n=1142 IC=-0.004 PNL=-13.19€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1142 IC=-0.004 PNL=-13.19€

**〰️ H-CUSTOM-GBM-60MIN-BUYNO** — GBM 60min BUY_NO — tracking por separado
  - _Hipótesis_: En 15min BUY_NO tiene IC=+0.119. ¿Se repite en 60min? Datos actuales: 8/14 (57%) IC=+0.044 — positivo pero débil. Puede ser que 60min requiera dirección alcista (BUY_YES) y no bajista.
  - _Umbral_: n≥30 para confirmar dirección
  - _Acción_: Si IC<0.05 con n≥30 → en 60min priorizar solo BUY_YES; si IC>0.08 → igualar al BUY_YES
  - _Estado_: n=427 IC=+0.001 PNL=+8.19€ — sin señal clara aún (umbral IC: min=0.05 max=None)
  - _Datos_: n=427 IC=+0.001 PNL=+8.19€
...
**🟡 H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.175 > 0.1 con n=1468 PNL=+841.55€
  - _Datos_: n=1468 IC=+0.175 PNL=+841.55€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=850 IC=+0.081 PNL=+193.29€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=850 IC=+0.081 PNL=+193.29€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=8435 IC=+0.049 PNL=+960.90€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=8435 IC=+0.049 PNL=+960.90€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=1396 IC=+0.049 PNL=+164.68€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1396 IC=+0.049 PNL=+164.68€
...
**🟡 H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.140 > 0.1 con n=270 PNL=+87.25€
  - _Datos_: n=270 IC=+0.140 PNL=+87.25€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=13794 IC=-0.144 PNL=+601.00€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=13794 IC=-0.144 PNL=+601.00€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.244 < -0.1 con n=1391 PNL=-204.94€
  - _Datos_: n=1391 IC=-0.244 PNL=-204.94€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=7115 IC=+0.170 PNL=-895.66€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=7115 IC=+0.170 PNL=-895.66€
