# Hipótesis automáticas — 2026-07-12 21:24 UTC
_Generado por shadow_postmortem.py sobre 11450 resoluciones (PNL=+2204.92€)_
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.123 (n=1387)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5779` → IC=+0.154 (n=50)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.5779 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` < `0.1365` → IC=+0.208 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1365 (IC base=+0.116)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.358` → IC=+0.385 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.358 (IC base=+0.116)
...
- **PATRÓN** `dist_vwap_pct` < `0.1391` → IC=+0.177 (n=29)
  ...
  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.1391 (IC base=+0.090)
...
- **PATRÓN** `sigma_h` < `0.0044` → IC=+0.164 (n=114)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0044 (IC base=+0.093)

- **PATRÓN** `dist_vwap_pct` > `0.3457` → IC=+0.157 (n=33)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.3457 (IC base=+0.093)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.190 (n=346)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0048 (IC base=+0.176)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.183 (n=269)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 13.0 (IC base=+0.176)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.184 (n=406)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 18.0 (IC base=+0.176)

- **PATRÓN** `dist_vwap_pct` > `0.5618` → IC=+0.256 (n=43)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.191 (n=108)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 6.0 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` < `0.2183` → IC=+0.175 (n=38)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` < 0.2183 (IC base=+0.169)
...
- **PATRÓN** `sigma_h` > `0.0037` → IC=+0.180 (n=101)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0037 (IC base=+0.153)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.179 (n=82)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 12.0 (IC base=+0.153)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.175 (n=118)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` < 18.0 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` > `0.3418` → IC=+0.227 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3418 (IC base=+0.153)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 1042 | +0.190 | +33.15€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 671 | +0.218 | +23.43€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 306 | +0.195 | +5.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 224 | +0.217 | -0.85€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 370 | +0.169 | +3.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 222 | +0.205 | +4.04€ | 0 | 9 |
...
| ✅ GBM_LATE_15M | 3699 | +0.113 | +1236.18€ | 0 | 10 |
| ✅ GBM_LATE_15M#15min | 3699 | +0.113 | +1236.18€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 904 | +0.074 | +131.37€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 904 | +0.074 | +131.37€ | 0 | 2 |
| ✅ GBM_LATE_15M#ETH | 936 | +0.074 | +161.41€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 936 | +0.074 | +161.41€ | 0 | 4 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1048 | +0.164 | +607.53€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1048 | +0.164 | +607.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 274 | +0.167 | +179.82€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 274 | +0.167 | +179.82€ | 0 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 280 | +0.131 | +126.40€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 280 | +0.131 | +126.40€ | 0 | 9 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.094 n=6505 | tras_1loss IC=+0.049 n=4807 | tras_2loss IC=+0.016 n=2140/40 | gap=+0.077 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8992 IC=+0.100 PNL=+2280.80€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8992 IC=+0.100 PNL=+2280.80€
