# Hipótesis automáticas — 2026-09-25 20:44 UTC
_Generado por shadow_postmortem.py sobre 609996 resoluciones (PNL=+68983.54€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.35` → IC=-0.228 (n=7195)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.35
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=21587)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `137.45` → IC=-0.251 (n=948)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 137.45
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=2846)

### BALLENAS_TARDIAS#BTC#15min
- **FILTRO** `restante_s_al_confirmar` < `500.19` → IC=-0.143 (n=373)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 500.19
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=1120)
...
- **FILTRO** `restante_s_al_confirmar` < `166.14` → IC=-0.213 (n=1746)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 166.14
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=5240)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `127.7` → IC=-0.350 (n=1414)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 127.7
  - _Potencial_: sin este filtro IC_bueno=-0.112 (n=4245)
...
- **PATRÓN** `py_entrada` < `0.35` → IC=+0.231 (n=11094)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.35 (IC base=+0.126)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.174 (n=5747)

  - _Acción_: Kelly boost +0.87€ cuando `libro_spread` < 0.01 (IC base=+0.126)

- **PATRÓN** `libro_liquidez` > `7871.4579` → IC=+0.175 (n=2170)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 7871.4579 (IC base=+0.126)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.211 (n=1675)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.155 (n=2862)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.344 (n=907)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.249 (n=536)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.235 (n=1508)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.232)

### FAVORITO_CONFIRMADO#ETH#60min
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.157 (n=470)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.150 (n=555)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.01 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `1313.0714` → IC=+0.150 (n=672)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.231 (n=716)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.207)

- **PATRÓN** `py_entrada` > `0.86` → IC=+0.434 (n=639)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.164 (n=557)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.146 (n=314)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.202 (n=11744)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.226 (n=3855)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.168 (n=2835)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.73` → IC=+0.176 (n=2685)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.245 (n=893)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.350 (n=419)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.73 (IC base=+0.241)

### FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.186 (n=2649)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.199 (n=2708)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.197 (n=2012)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.201 (n=34970)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.198)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.235 (n=15516)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.75 (IC base=+0.198)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.177 (n=6036)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.190 (n=6519)

  - _Acción_: Kelly boost +0.95€ cuando `py_entrada` > 0.71 (IC base=+0.176)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.226 (n=6272)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.231 (n=3147)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `4107.9466` → IC=+0.306 (n=375)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4107.9466 (IC base=+0.289)

### FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#BTC#60min
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.298 (n=350)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `4267.8515` → IC=+0.298 (n=334)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4267.8515 (IC base=+0.276)

### FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#ETH#60min
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.333 (n=380)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1451.8749` → IC=+0.308 (n=482)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4617` → IC=+0.203 (n=5686)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4617 (IC base=+0.106)

- **PATRÓN** `ibs_20min` < `0.5714` → IC=+0.133 (n=9980)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.5714 (IC base=+0.065)

- **PATRÓN** `dist_vwap_pct` > `0.6138` → IC=+0.202 (n=740)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6138 (IC base=+0.065)

- **PATRÓN** `volumen_regimen` < `0.697` → IC=+0.184 (n=1543)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 0.697 (IC base=+0.065)

- **PATRÓN** `volumen_regimen` > `1.0501` → IC=+0.176 (n=1589)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 1.0501 (IC base=+0.065)

- **PATRÓN** `volumen_pendiente_norm` > `0.167` → IC=+0.226 (n=1676)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.167 (IC base=+0.065)

- **PATRÓN** `volumen_spike_ratio` > `1.5694` → IC=+0.199 (n=5283)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5694 (IC base=+0.065)

- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.209 (n=5708)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 135.0 (IC base=+0.065)

### GBM_LATE_15M#BNB#15min
- **PATRÓN** `sigma_h` < `0.005` → IC=+0.175 (n=622)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.005 (IC base=+0.161)

- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.177 (n=623)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0082 (IC base=+0.161)

- **PATRÓN** `drift_60min` |x|≤ `0.3502` → IC=+0.166 (n=1861)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.479` → IC=+0.244 (n=1469)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4296` → IC=+0.233 (n=432)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4296 (IC base=+0.234)
...
- **PATRÓN** `ibs_20min` > `0.9861` → IC=+0.270 (n=477)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9861 (IC base=+0.217)

- **PATRÓN** `dist_vwap_pct` < `0.5705` → IC=+0.222 (n=1500)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5705 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.76` → IC=+0.261 (n=245)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.76 (IC base=+0.217)

- **PATRÓN** `volumen_regimen` < `1.2524` → IC=+0.220 (n=1429)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2524 (IC base=+0.217)

- **PATRÓN** `volumen_regimen` > `0.8707` → IC=+0.225 (n=952)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3728` → IC=+0.236 (n=467)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3728 (IC base=+0.217)

- **PATRÓN** `libro_liquidez` > `11189.3992` → IC=+0.227 (n=1428)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11189.3992 (IC base=+0.217)

- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.168 (n=997)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0039 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.077` → IC=+0.167 (n=497)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.077 (IC base=+0.141)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.171 (n=584)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.6975` → IC=+0.173 (n=1490)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.6975 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` < `0.1336` → IC=+0.157 (n=1332)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.1336 (IC base=+0.141)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.282` → IC=+0.145 (n=1358)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 4.282 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` < `1.1967` → IC=+0.151 (n=1490)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1967 (IC base=+0.141)

- **PATRÓN** `volumen_pendiente_norm` > `0.1558` → IC=+0.179 (n=394)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1558 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` < `2.4238` → IC=+0.154 (n=1380)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.4238 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` > `1.4164` → IC=+0.148 (n=1380)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.4164 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `14134.8887` → IC=+0.145 (n=993)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 14134.8887 (IC base=+0.141)

- **PATRÓN** `ballena_activa_n` < `235.0` → IC=+0.167 (n=577)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.191 (n=1935)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2091` → IC=+0.187 (n=1842)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` < 0.2091 (IC base=+0.185)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.193 (n=1302)

  - _Acción_: Kelly boost +0.96€ cuando `libro_spread` < 0.02 (IC base=+0.185)

- **PATRÓN** `sigma_h` < `0.0104` → IC=+0.219 (n=1395)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0104 (IC base=+0.209)

- **PATRÓN** `drift_60min` |x|≤ `0.6121` → IC=+0.209 (n=1583)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.6121 (IC base=+0.209)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.243 (n=598)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.721` → IC=+0.234 (n=603)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7653` → IC=+0.201 (n=640)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7653 (IC base=+0.209)

- **PATRÓN** `volumen_spike_ratio` > `2.1929` → IC=+0.216 (n=969)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1929 (IC base=+0.209)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.215 (n=1063)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.209)

- **PATRÓN** `libro_liquidez` > `1904.0224` → IC=+0.212 (n=718)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1904.0224 (IC base=+0.209)

- **PATRÓN** `ballena_activa_n` < `23.0` → IC=+0.213 (n=925)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.144 (n=102)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=2265)

- **PATRÓN** `ibs_20min` > `0.9449` → IC=+0.212 (n=366)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9449 (IC base=+0.026)
...
- **PATRÓN** `dist_vwap_pct` > `0.6787` → IC=+0.207 (n=145)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6787 (IC base=+0.018)

- **PATRÓN** `volumen_regimen` < `0.8439` → IC=+0.165 (n=574)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.8439 (IC base=+0.018)

- **PATRÓN** `volumen_regimen` > `1.1561` → IC=+0.150 (n=287)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 1.1561 (IC base=+0.018)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5223` → IC=+0.176 (n=721)
...
- **FILTRO** `ibs_20min` > `0.25` → IC=-0.125 (n=2236)

  - _Acción_: SKIP cuando `ibs_20min` > 0.25
  - _Potencial_: sin este filtro IC_bueno=+0.124 (n=1104)

- **FILTRO** `sigma_ewma_delta_pct` > `8.696` → IC=-0.208 (n=358)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.696
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=2982)
...
- **PATRÓN** `ibs_20min` < `0.25` → IC=+0.124 (n=1104)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` < 0.25 (IC base=-0.043)

- **PATRÓN** `dist_vwap_pct` > `0.7124` → IC=+0.253 (n=71)
...
- **PATRÓN** `volumen_regimen` < `1.0932` → IC=+0.229 (n=323)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.0932 (IC base=-0.043)

- **PATRÓN** `volumen_regimen` > `0.9027` → IC=+0.217 (n=245)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4205` → IC=+0.269 (n=305)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.4205 (IC base=-0.043)

### GBM_LATE_15M#XRP#15min
- **FILTRO** `drift_60min` |x|> `0.6559` → IC=-0.185 (n=582)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6559
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=1748)

- **FILTRO** `ibs_20min` < `0.7197` → IC=-0.152 (n=1537)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7197
  - _Potencial_: sin este filtro IC_bueno=+0.104 (n=793)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.199 (n=426)

  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=1904)

- **FILTRO** `ibs_20min` > `0.77` → IC=-0.202 (n=854)

  - _Acción_: SKIP cuando `ibs_20min` > 0.77
  - _Potencial_: sin este filtro IC_bueno=+0.037 (n=2566)
...
- **PATRÓN** `volumen_regimen` < `0.7337` → IC=+0.248 (n=347)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7337 (IC base=-0.022)

- **PATRÓN** `volumen_regimen` > `1.2596` → IC=+0.285 (n=263)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2596 (IC base=-0.022)

- **PATRÓN** `volumen_pendiente_norm` > `0.1026` → IC=+0.274 (n=277)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1026 (IC base=-0.022)

- **PATRÓN** `volumen_spike_ratio` < `2.141` → IC=+0.253 (n=596)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.141 (IC base=-0.022)

- **PATRÓN** `volumen_spike_ratio` > `1.5236` → IC=+0.246 (n=605)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5236 (IC base=-0.022)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0098` → IC=+0.198 (n=3466)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0098 (IC base=+0.097)

- **PATRÓN** `ibs_20min` > `0.4762` → IC=+0.187 (n=9292)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.4762 (IC base=+0.097)

- **PATRÓN** `dist_vwap_pct` > `0.7682` → IC=+0.284 (n=1111)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7682 (IC base=+0.097)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.61` → IC=+0.157 (n=4881)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 3.61 (IC base=+0.097)

- **PATRÓN** `volumen_regimen` > `0.6888` → IC=+0.250 (n=3315)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6527` → IC=+0.244 (n=2008)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6527 (IC base=+0.097)

- **PATRÓN** `ballena_activa_n` < `96.0` → IC=+0.267 (n=5534)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 96.0 (IC base=+0.097)

- **PATRÓN** `sigma_h` > `0.0092` → IC=+0.159 (n=3444)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0092 (IC base=+0.074)

- **PATRÓN** `ibs_20min` < `0.5495` → IC=+0.154 (n=9083)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.5495 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` > `0.7279` → IC=+0.242 (n=652)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7279 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` < `0.2541` → IC=+0.240 (n=2895)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2541 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` < `0.7075` → IC=+0.242 (n=1345)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7075 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` > `1.2035` → IC=+0.249 (n=1019)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2035 (IC base=+0.074)

- **PATRÓN** `volumen_pendiente_norm` > `0.2435` → IC=+0.304 (n=762)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2435 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` < `1.5993` → IC=+0.259 (n=1780)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5993 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` > `2.2992` → IC=+0.259 (n=1834)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2992 (IC base=+0.074)

- **PATRÓN** `ballena_activa_n` < `84.0` → IC=+0.265 (n=3921)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 84.0 (IC base=+0.074)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `ibs_20min` < `0.2584` → IC=-0.146 (n=715)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2584
  - _Potencial_: sin este filtro IC_bueno=+0.106 (n=2150)

- **FILTRO** `sigma_ewma_delta_pct` > `4.548` → IC=-0.158 (n=545)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.548
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=1836)

- **PATRÓN** `ibs_20min` > `0.8935` → IC=+0.265 (n=718)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1457` → IC=+0.201 (n=413)
...
- **PATRÓN** `volumen_regimen` > `0.6717` → IC=+0.169 (n=857)
...
- **PATRÓN** `dist_vwap_pct` < `0.1624` → IC=+0.224 (n=603)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1624 (IC base=+0.006)

- **PATRÓN** `volumen_regimen` > `0.8484` → IC=+0.228 (n=392)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8484 (IC base=+0.006)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4386` → IC=+0.221 (n=181)
...
- **PATRÓN** `ballena_activa_n` < `461.0` → IC=+0.222 (n=541)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.249 (n=1639)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1037` → IC=+0.259 (n=1386)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1037 (IC base=+0.246)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.253 (n=1149)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.246)

- **PATRÓN** `libro_liquidez` > `1803.7992` → IC=+0.249 (n=1089)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1803.7992 (IC base=+0.246)

- **PATRÓN** `sigma_h` > `0.01` → IC=+0.315 (n=594)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.01 (IC base=+0.280)

- **PATRÓN** `drift_60min` |x|≤ `0.1738` → IC=+0.286 (n=577)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1738 (IC base=+0.280)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.324 (n=441)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.280)

- **PATRÓN** `ibs_20min` < `0.0909` → IC=+0.288 (n=877)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.754` → IC=+0.280 (n=1403)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7408` → IC=+0.282 (n=534)
...
- **PATRÓN** `volumen_spike_ratio` > `2.7234` → IC=+0.288 (n=550)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7234 (IC base=+0.280)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.284 (n=874)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.280)

- **PATRÓN** `libro_liquidez` > `1893.9584` → IC=+0.292 (n=594)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1893.9584 (IC base=+0.280)

- **PATRÓN** `ballena_activa_n` < `28.0` → IC=+0.283 (n=788)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 28.0 (IC base=+0.280)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **FILTRO** `ibs_20min` < `0.294` → IC=-0.182 (n=510)

  - _Acción_: SKIP cuando `ibs_20min` < 0.294
  - _Potencial_: sin este filtro IC_bueno=+0.080 (n=1532)

- **FILTRO** `ibs_20min` > `0.7745` → IC=-0.187 (n=611)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7745
  - _Potencial_: sin este filtro IC_bueno=+0.055 (n=1836)
...
- **PATRÓN** `dist_vwap_pct` < `0.6852` → IC=+0.197 (n=507)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.6852 (IC base=-0.005)

- **PATRÓN** `volumen_regimen` < `1.1663` → IC=+0.208 (n=440)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8269` → IC=+0.257 (n=265)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8269 (IC base=-0.005)

- **PATRÓN** `volumen_spike_ratio` > `2.1439` → IC=+0.236 (n=180)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1439 (IC base=-0.005)

- **PATRÓN** `ballena_activa_n` < `139.0` → IC=+0.247 (n=402)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 139.0 (IC base=-0.005)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **FILTRO** `ibs_20min` < `0.7273` → IC=-0.194 (n=1095)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7273
  - _Potencial_: sin este filtro IC_bueno=+0.276 (n=1107)

- **FILTRO** `ibs_20min` > `0.6875` → IC=-0.235 (n=564)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6875
  - _Potencial_: sin este filtro IC_bueno=+0.096 (n=1709)

- **FILTRO** `sigma_ewma_delta_pct` > `4.714` → IC=-0.184 (n=495)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.714
  - _Potencial_: sin este filtro IC_bueno=+0.069 (n=1778)

- **PATRÓN** `ibs_20min` > `0.7273` → IC=+0.276 (n=1107)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7273 (IC base=+0.043)

- **PATRÓN** `dist_vwap_pct` > `0.8634` → IC=+0.324 (n=260)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8634 (IC base=+0.043)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.64` → IC=+0.171 (n=347)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 9.64 (IC base=+0.043)

- **PATRÓN** `volumen_regimen` < `0.8667` → IC=+0.304 (n=544)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8667 (IC base=+0.043)

- **PATRÓN** `volumen_regimen` > `0.7291` → IC=+0.293 (n=729)
...
- **PATRÓN** `ibs_20min` < `0.1923` → IC=+0.190 (n=753)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` < 0.1923 (IC base=+0.013)

- **PATRÓN** `dist_vwap_pct` > `0.7382` → IC=+0.201 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7382 (IC base=+0.013)

- **PATRÓN** `dist_vwap_pct` < `0.22` → IC=+0.221 (n=504)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.22 (IC base=+0.013)

- **PATRÓN** `volumen_regimen` < `0.7101` → IC=+0.253 (n=257)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7101 (IC base=+0.013)

- **PATRÓN** `volumen_pendiente_norm` < `0.0971` → IC=+0.212 (n=539)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4641` → IC=+0.222 (n=544)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.4641 (IC base=+0.013)

- **PATRÓN** `ballena_activa_n` < `58.0` → IC=+0.233 (n=552)
...
- **PATRÓN** `sigma_h` > `0.0154` → IC=+0.300 (n=965)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0154 (IC base=+0.273)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.284 (n=498)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3925` → IC=+0.305 (n=1447)
...
- **PATRÓN** `dist_vwap_pct` < `0.2281` → IC=+0.273 (n=1308)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2281 (IC base=+0.273)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.469` → IC=+0.289 (n=528)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.469 (IC base=+0.273)

- **PATRÓN** `volumen_regimen` < `0.6385` → IC=+0.279 (n=483)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6385 (IC base=+0.273)

- **PATRÓN** `volumen_regimen` > `1.2434` → IC=+0.308 (n=482)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2434 (IC base=+0.273)

- **PATRÓN** `volumen_pendiente_norm` > `0.2372` → IC=+0.334 (n=239)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2372 (IC base=+0.273)

- **PATRÓN** `volumen_spike_ratio` < `1.4234` → IC=+0.279 (n=427)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4234 (IC base=+0.273)

- **PATRÓN** `volumen_spike_ratio` > `2.1403` → IC=+0.273 (n=580)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1403 (IC base=+0.273)

- **PATRÓN** `libro_liquidez` > `2632.217` → IC=+0.279 (n=964)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2632.217 (IC base=+0.273)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.170 (n=2698)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0049 (IC base=+0.167)

- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.199 (n=2690)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0113 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0906` → IC=+0.186 (n=2693)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.0906 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.179 (n=8421)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 5.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` > `0.5745` → IC=+0.218 (n=8072)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5745 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.1784` → IC=+0.193 (n=3520)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.1784 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.333` → IC=+0.256 (n=1648)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.333 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.2103` → IC=+0.159 (n=5347)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2103 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` > `0.6297` → IC=+0.160 (n=5348)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.6297 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.2434` → IC=+0.194 (n=1628)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2434 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.5587` → IC=+0.167 (n=3408)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.5587 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.6134` → IC=+0.174 (n=2582)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.6134 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `2410.0397` → IC=+0.168 (n=5380)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2410.0397 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `114.0` → IC=+0.180 (n=6984)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 114.0 (IC base=+0.167)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.182 (n=5133)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0066 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0814` → IC=+0.208 (n=2568)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0814 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.205 (n=2966)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` < `0.4814` → IC=+0.225 (n=7696)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4814 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.9221` → IC=+0.145 (n=758)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.9221 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` < `0.2357` → IC=+0.158 (n=5579)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.2357 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.342` → IC=+0.192 (n=1301)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 10.342 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.1727` → IC=+0.151 (n=5571)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.1727 (IC base=+0.167)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3424` → IC=+0.206 (n=1369)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3424 (IC base=+0.182)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.183 (n=1442)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.225` → IC=+0.328 (n=423)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.225 (IC base=+0.182)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1882` → IC=+0.287 (n=670)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1882 (IC base=+0.237)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.242 (n=910)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3451` → IC=+0.264 (n=1005)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3451 (IC base=+0.237)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.34` → IC=+0.249 (n=1087)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.34 (IC base=+0.237)

- **PATRÓN** `volumen_pendiente_norm` < `0.1603` → IC=+0.234 (n=948)
...
- **PATRÓN** `volumen_regimen` > `1.0731` → IC=+0.170 (n=544)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 1.0731 (IC base=+0.161)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5028` → IC=+0.178 (n=513)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 1.5028 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` > `2.4541` → IC=+0.160 (n=389)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 2.4541 (IC base=+0.161)

- **PATRÓN** `libro_liquidez` > `11903.3418` → IC=+0.170 (n=1072)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 11903.3418 (IC base=+0.161)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.158 (n=1303)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0057 (IC base=+0.137)

- **PATRÓN** `drift_60min` |x|≤ `0.2935` → IC=+0.160 (n=1303)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.2935 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.185 (n=509)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 17.0 (IC base=+0.137)

- **PATRÓN** `ibs_20min` < `0.565` → IC=+0.187 (n=1303)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` < 0.565 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.1357` → IC=+0.160 (n=1294)
...
- **PATRÓN** `volumen_regimen` < `1.204` → IC=+0.159 (n=1303)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.205 (n=1407)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_spike_ratio` < `1.6263` → IC=+0.199 (n=430)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.206 (n=952)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.199)

- **PATRÓN** `sigma_h` < `0.0102` → IC=+0.234 (n=987)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0102 (IC base=+0.216)

- **PATRÓN** `drift_60min` |x|≤ `0.2467` → IC=+0.237 (n=748)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2467 (IC base=+0.216)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.272 (n=389)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.216)

- **PATRÓN** `ibs_20min` < `0.3486` → IC=+0.247 (n=1122)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3486 (IC base=+0.216)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3531` → IC=+0.265 (n=185)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3531 (IC base=+0.216)

- **PATRÓN** `volumen_spike_ratio` < `1.766` → IC=+0.209 (n=459)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.766 (IC base=+0.216)

- **PATRÓN** `volumen_spike_ratio` > `3.3711` → IC=+0.239 (n=347)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.3711 (IC base=+0.216)

- **PATRÓN** `ballena_activa_n` < `11.0` → IC=+0.206 (n=345)
...
- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.153 (n=1353)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0072 (IC base=+0.122)

- **PATRÓN** `drift_60min` |x|≤ `0.3848` → IC=+0.143 (n=1353)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.3848 (IC base=+0.122)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.180 (n=526)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 17.0 (IC base=+0.122)

- **PATRÓN** `ibs_20min` < `0.6457` → IC=+0.172 (n=1353)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.6457 (IC base=+0.122)

- **PATRÓN** `dist_vwap_pct` < `0.3678` → IC=+0.134 (n=1461)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.3678 (IC base=+0.122)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.961` → IC=+0.164 (n=477)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 6.961 (IC base=+0.122)

- **PATRÓN** `volumen_regimen` < `0.8475` → IC=+0.149 (n=903)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7906` → IC=+0.137 (n=821)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 1.7906 (IC base=+0.122)

- **PATRÓN** `libro_liquidez` > `10015.5876` → IC=+0.158 (n=614)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 10015.5876 (IC base=+0.122)

### GBM_LATE_15M_MULTIHORIZONTE#SOL#15min
- **PATRÓN** `sigma_h` > `0.0102` → IC=+0.152 (n=662)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0102 (IC base=+0.118)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.138 (n=1496)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 5.0 (IC base=+0.118)

- **PATRÓN** `ibs_20min` > `0.5156` → IC=+0.204 (n=1460)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5156 (IC base=+0.118)

- **PATRÓN** `dist_vwap_pct` > `0.5216` → IC=+0.197 (n=632)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.5216 (IC base=+0.118)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.731` → IC=+0.258 (n=328)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.731 (IC base=+0.118)

- **PATRÓN** `volumen_regimen` < `1.2161` → IC=+0.131 (n=1460)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_regimen` < 1.2161 (IC base=+0.118)

- **PATRÓN** `volumen_pendiente_norm` > `0.0711` → IC=+0.124 (n=612)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_pendiente_norm` > 0.0711 (IC base=+0.118)

- **PATRÓN** `volumen_spike_ratio` < `1.5431` → IC=+0.140 (n=621)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 1.5431 (IC base=+0.118)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.127 (n=1525)

  - _Acción_: Kelly boost +0.63€ cuando `libro_spread` < 0.02 (IC base=+0.118)

- **PATRÓN** `libro_liquidez` > `2898.8632` → IC=+0.193 (n=662)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 2898.8632 (IC base=+0.118)

- **PATRÓN** `ballena_activa_n` < `50.0` → IC=+0.135 (n=1133)
...
- **PATRÓN** `drift_60min` |x|≤ `0.104` → IC=+0.163 (n=493)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.104 (IC base=+0.112)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.130 (n=1500)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 5.0 (IC base=+0.112)

- **PATRÓN** `ibs_20min` < `0.5686` → IC=+0.209 (n=1478)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5686 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `1.0221` → IC=+0.137 (n=210)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 1.0221 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` < `0.2057` → IC=+0.136 (n=1353)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.2057 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.321` → IC=+0.128 (n=449)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 5.321 (IC base=+0.112)

- **PATRÓN** `volumen_regimen` < `0.6365` → IC=+0.142 (n=493)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 0.6365 (IC base=+0.112)

- **PATRÓN** `volumen_pendiente_norm` > `0.2729` → IC=+0.156 (n=181)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_pendiente_norm` > 0.2729 (IC base=+0.112)

- **PATRÓN** `volumen_spike_ratio` < `1.4527` → IC=+0.127 (n=443)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_spike_ratio` < 1.4527 (IC base=+0.112)

- **PATRÓN** `libro_liquidez` > `2761.3271` → IC=+0.156 (n=670)
...
- **PATRÓN** `ibs_20min` < `0.4384` → IC=+0.244 (n=1438)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4384 (IC base=+0.202)

- **PATRÓN** `dist_vwap_pct` > `1.2417` → IC=+0.225 (n=176)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2417 (IC base=+0.202)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.43` → IC=+0.236 (n=275)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.43 (IC base=+0.202)

- **PATRÓN** `volumen_regimen` > `0.6295` → IC=+0.214 (n=1438)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6295 (IC base=+0.202)

- **PATRÓN** `volumen_pendiente_norm` > `0.2814` → IC=+0.291 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2814 (IC base=+0.202)

- **PATRÓN** `volumen_spike_ratio` < `2.1933` → IC=+0.193 (n=1140)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` < 2.1933 (IC base=+0.202)

- **PATRÓN** `volumen_spike_ratio` > `1.434` → IC=+0.198 (n=1295)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.434 (IC base=+0.202)

- **PATRÓN** `libro_liquidez` > `2605.1552` → IC=+0.211 (n=959)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2605.1552 (IC base=+0.202)
...
- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.175 (n=653)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.186 (n=979)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 15.0 (IC base=+0.147)

- **PATRÓN** `ibs_20min` > `0.5385` → IC=+0.188 (n=1745)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.5385 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.8774` → IC=+0.179 (n=319)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.8774 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.707` → IC=+0.175 (n=880)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 3.707 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `0.8726` → IC=+0.166 (n=1146)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.8726 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` > `0.623` → IC=+0.151 (n=1718)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4417` → IC=+0.164 (n=628)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.4417 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` > `2.2086` → IC=+0.155 (n=854)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 2.2086 (IC base=+0.147)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.153 (n=2215)
...
- **PATRÓN** `ballena_activa_n` < `155.0` → IC=+0.168 (n=1729)

  - _Acción_: Kelly boost +0.84€ cuando `ballena_activa_n` < 155.0 (IC base=+0.147)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.136 (n=1350)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0057 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.123 (n=2044)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 5.0 (IC base=+0.110)

- **PATRÓN** `ibs_20min` < `0.6667` → IC=+0.137 (n=2025)

  - _Acción_: Kelly boost +0.69€ cuando `ibs_20min` < 0.6667 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.2153` → IC=+0.121 (n=1796)

  - _Acción_: Kelly boost +0.60€ cuando `dist_vwap_pct` < 0.2153 (IC base=+0.110)

- **PATRÓN** `volumen_regimen` < `0.6987` → IC=+0.127 (n=800)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4517` → IC=+0.142 (n=649)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.4517 (IC base=+0.110)

- **PATRÓN** `libro_liquidez` > `2761.0477` → IC=+0.122 (n=1805)
...
- **PATRÓN** `ibs_20min` < `0.084` → IC=+0.155 (n=192)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1188` → IC=+0.145 (n=150)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.1188 (IC base=+0.131)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.165 (n=422)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 8.0 (IC base=+0.131)

- **PATRÓN** `ibs_20min` > `0.8889` → IC=+0.239 (n=301)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8889 (IC base=+0.131)

- **PATRÓN** `dist_vwap_pct` > `0.9964` → IC=+0.227 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9964 (IC base=+0.131)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.234` → IC=+0.201 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.234 (IC base=+0.131)

- **PATRÓN** `volumen_regimen` < `1.0712` → IC=+0.150 (n=395)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.0712 (IC base=+0.131)

- **PATRÓN** `volumen_pendiente_norm` > `0.1825` → IC=+0.177 (n=128)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.1825 (IC base=+0.131)

- **PATRÓN** `volumen_spike_ratio` > `2.2078` → IC=+0.179 (n=194)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 2.2078 (IC base=+0.131)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.135 (n=488)

  - _Acción_: Kelly boost +0.67€ cuando `libro_spread` < 0.02 (IC base=+0.131)

- **PATRÓN** `libro_liquidez` > `2920.0962` → IC=+0.188 (n=203)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 2920.0962 (IC base=+0.131)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5923` → IC=+0.187 (n=3314)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 2.5923 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `1793.26` → IC=+0.171 (n=10334)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 1793.26 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `86.0` → IC=+0.195 (n=7903)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 86.0 (IC base=+0.169)

- **PATRÓN** `sigma_h` < `0.007` → IC=+0.192 (n=6246)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.007 (IC base=+0.180)

- **PATRÓN** `drift_60min` |x|≤ `0.1495` → IC=+0.188 (n=4121)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.1495 (IC base=+0.180)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.205 (n=3546)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5684` → IC=+0.236 (n=9362)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5684 (IC base=+0.180)

- **PATRÓN** `dist_vwap_pct` < `0.2524` → IC=+0.161 (n=5798)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` < 0.2524 (IC base=+0.180)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.034` → IC=+0.199 (n=1308)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.034 (IC base=+0.180)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.732` → IC=+0.182 (n=9050)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.732 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` < `0.6327` → IC=+0.163 (n=2143)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.6327 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` > `1.202` → IC=+0.155 (n=2142)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.202 (IC base=+0.180)

- **PATRÓN** `volumen_pendiente_norm` > `0.2876` → IC=+0.240 (n=1231)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2876 (IC base=+0.180)

- **PATRÓN** `volumen_spike_ratio` > `2.6133` → IC=+0.192 (n=2870)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 2.6133 (IC base=+0.180)

- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.193 (n=5572)

  - _Acción_: Kelly boost +0.96€ cuando `ballena_activa_n` < 48.0 (IC base=+0.180)

### GBM_LATE_15M_TARDIO#BNB#15min
- **PATRÓN** `sigma_h` < `0.0051` → IC=+0.203 (n=583)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.3555` → IC=+0.191 (n=1744)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.3555 (IC base=+0.190)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.202 (n=837)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.326 (n=629)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.707` → IC=+0.168 (n=377)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` > 9.707 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.21` → IC=+0.153 (n=1497)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2654` → IC=+0.202 (n=236)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2654 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` < `2.1124` → IC=+0.159 (n=1411)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.1124 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.7524` → IC=+0.156 (n=1069)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.7524 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `11359.3183` → IC=+0.162 (n=1481)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 11359.3183 (IC base=+0.150)

- **PATRÓN** `ballena_activa_n` < `472.0` → IC=+0.157 (n=1539)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 472.0 (IC base=+0.150)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.166 (n=1437)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.186 (n=562)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 17.0 (IC base=+0.153)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.157 (n=645)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 7.0 (IC base=+0.153)

- **PATRÓN** `ibs_20min` < `0.6603` → IC=+0.196 (n=1437)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` < 0.6603 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` > `0.6883` → IC=+0.155 (n=233)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.6883 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` < `0.1371` → IC=+0.166 (n=1301)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.1371 (IC base=+0.153)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.471` → IC=+0.161 (n=240)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 11.471 (IC base=+0.153)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.511` → IC=+0.154 (n=1455)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 6.511 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` < `1.186` → IC=+0.164 (n=1437)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.186 (IC base=+0.153)

- **PATRÓN** `volumen_pendiente_norm` > `0.1509` → IC=+0.197 (n=384)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.1509 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` < `2.4003` → IC=+0.161 (n=1339)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.4003 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` > `1.7535` → IC=+0.164 (n=892)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.7535 (IC base=+0.153)

- **PATRÓN** `ballena_activa_n` < `355.0` → IC=+0.162 (n=831)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.226 (n=1759)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.293` → IC=+0.298 (n=360)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.293 (IC base=+0.218)

- **PATRÓN** `volumen_pendiente_norm` < `0.2104` → IC=+0.221 (n=1663)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2104 (IC base=+0.218)

- **PATRÓN** `volumen_spike_ratio` > `2.8194` → IC=+0.241 (n=725)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8194 (IC base=+0.218)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.228 (n=1185)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.218)

- **PATRÓN** `sigma_h` < `0.0118` → IC=+0.236 (n=1565)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0118 (IC base=+0.229)

- **PATRÓN** `drift_60min` |x|≤ `0.1688` → IC=+0.238 (n=688)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1688 (IC base=+0.229)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.256 (n=592)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.0164` → IC=+0.307 (n=522)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0164 (IC base=+0.229)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.756` → IC=+0.272 (n=581)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.756 (IC base=+0.229)

- **PATRÓN** `volumen_pendiente_norm` > `0.3435` → IC=+0.302 (n=230)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3435 (IC base=+0.229)

- **PATRÓN** `volumen_spike_ratio` < `1.7521` → IC=+0.228 (n=633)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7521 (IC base=+0.229)

- **PATRÓN** `volumen_spike_ratio` > `2.184` → IC=+0.232 (n=959)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.184 (IC base=+0.229)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.238 (n=1050)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.229)

- **PATRÓN** `libro_liquidez` > `1902.0032` → IC=+0.233 (n=709)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1902.0032 (IC base=+0.229)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.227 (n=1371)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 54.0 (IC base=+0.229)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.179 (n=777)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.0041 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.4384` → IC=+0.145 (n=1761)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.4384 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.151 (n=1844)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 5.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` > `0.8757` → IC=+0.261 (n=798)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.181` → IC=+0.159 (n=728)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 4.181 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `0.873` → IC=+0.156 (n=1174)
...
- **PATRÓN** `libro_liquidez` > `8024.5486` → IC=+0.234 (n=798)
...
- **PATRÓN** `ibs_20min` < `0.5789` → IC=+0.196 (n=1260)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` < 0.5789 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` < `0.5967` → IC=+0.138 (n=1584)
...
- **PATRÓN** `volumen_regimen` < `0.8615` → IC=+0.145 (n=955)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 0.8615 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` > `1.1867` → IC=+0.142 (n=478)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 1.1867 (IC base=+0.135)

- **PATRÓN** `volumen_pendiente_norm` > `0.2891` → IC=+0.240 (n=171)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2891 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` > `1.4416` → IC=+0.149 (n=1359)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.4416 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `7246.0232` → IC=+0.196 (n=650)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 7246.0232 (IC base=+0.135)

- **PATRÓN** `ballena_activa_n` < `177.0` → IC=+0.141 (n=1356)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.133 (n=1803)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.192 (n=1752)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` > 0.4706 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` > `1.1004` → IC=+0.197 (n=367)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 1.1004 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.503` → IC=+0.237 (n=656)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.503 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` < `0.8928` → IC=+0.136 (n=1167)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 0.8928 (IC base=+0.114)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.128 (n=1761)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.02 (IC base=+0.114)

- **PATRÓN** `libro_liquidez` > `2898.7698` → IC=+0.249 (n=583)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2898.7698 (IC base=+0.114)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.133 (n=1357)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1331` → IC=+0.164 (n=564)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.82€ cuando `drift_60min` |x|≤ 0.1331 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.124 (n=1752)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 5.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` < `0.6364` → IC=+0.204 (n=1693)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6364 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.2216` → IC=+0.132 (n=1374)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.2216 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.488` → IC=+0.127 (n=1634)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 3.488 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` < `0.6488` → IC=+0.157 (n=564)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.6488 (IC base=+0.113)
...
- **PATRÓN** `volumen_spike_ratio` < `1.442` → IC=+0.139 (n=510)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` < 1.442 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `2831.1065` → IC=+0.168 (n=564)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2831.1065 (IC base=+0.113)

- **PATRÓN** `ballena_activa_n` < `51.0` → IC=+0.122 (n=1321)

  - _Acción_: Kelly boost +0.61€ cuando `ballena_activa_n` < 51.0 (IC base=+0.113)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` > `0.0193` → IC=+0.217 (n=1165)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0193 (IC base=+0.208)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.214 (n=1825)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.5116` → IC=+0.247 (n=1748)
...
- **PATRÓN** `volumen_regimen` < `1.2439` → IC=+0.210 (n=1748)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2439 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` > `0.6389` → IC=+0.216 (n=1748)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6389 (IC base=+0.208)

- **PATRÓN** `volumen_pendiente_norm` > `0.2333` → IC=+0.249 (n=305)
...
- **PATRÓN** `libro_liquidez` > `2635.1118` → IC=+0.220 (n=1165)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.209 (n=1316)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.200)

- **PATRÓN** `ibs_20min` < `0.4231` → IC=+0.265 (n=1641)
...
- **PATRÓN** `dist_vwap_pct` < `0.9244` → IC=+0.202 (n=2069)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.9244 (IC base=+0.200)
...
- **PATRÓN** `volumen_regimen` > `1.2347` → IC=+0.237 (n=621)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1818` → IC=+0.197 (n=1478)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.1818 (IC base=+0.200)

- **PATRÓN** `volumen_spike_ratio` > `1.4288` → IC=+0.196 (n=1679)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.4288 (IC base=+0.200)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.204 (n=1101)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.144 (n=3143)

- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.167 (n=2658)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0092 (IC base=+0.157)

- **PATRÓN** `drift_60min` |x|≤ `0.5194` → IC=+0.166 (n=3020)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.5194 (IC base=+0.157)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.172 (n=1198)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9414` → IC=+0.215 (n=1007)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9414 (IC base=+0.157)

- **PATRÓN** `dist_vwap_pct` > `0.1891` → IC=+0.171 (n=1122)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.1891 (IC base=+0.157)

- **PATRÓN** `dist_vwap_pct` < `0.4846` → IC=+0.154 (n=1881)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.4846 (IC base=+0.157)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.195` → IC=+0.193 (n=496)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 10.195 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` > `0.8955` → IC=+0.165 (n=1339)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 0.8955 (IC base=+0.157)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8741` → IC=+0.167 (n=1989)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.8741 (IC base=+0.157)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.159 (n=2141)

  - _Acción_: Kelly boost +0.80€ cuando `libro_spread` < 0.01 (IC base=+0.157)

- **PATRÓN** `libro_liquidez` > `3941.0508` → IC=+0.166 (n=2013)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 3941.0508 (IC base=+0.157)

- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.200 (n=792)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0039 (IC base=+0.142)

- **PATRÓN** `drift_60min` |x|≤ `0.4873` → IC=+0.160 (n=2370)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.4873 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.174 (n=860)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.1824` → IC=+0.171 (n=1043)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` < 0.1824 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.6795` → IC=+0.167 (n=457)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.6795 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.265` → IC=+0.153 (n=2359)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 6.265 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `1.0973` → IC=+0.153 (n=1995)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.0973 (IC base=+0.142)

- **PATRÓN** `volumen_pendiente_norm` < `0.0966` → IC=+0.146 (n=2149)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_pendiente_norm` < 0.0966 (IC base=+0.142)

- **PATRÓN** `volumen_pendiente_norm` > `0.0724` → IC=+0.144 (n=1091)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_pendiente_norm` > 0.0724 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` < `1.4237` → IC=+0.157 (n=780)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 1.4237 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` > `1.813` → IC=+0.144 (n=1559)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.813 (IC base=+0.142)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.144 (n=3143)

  - _Acción_: Kelly boost +0.72€ cuando `libro_spread` < 0.01 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `7275.7832` → IC=+0.155 (n=2116)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.167 (n=879)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0073 (IC base=+0.164)

- **PATRÓN** `sigma_h` > `0.0043` → IC=+0.171 (n=997)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0043 (IC base=+0.164)

- **PATRÓN** `drift_60min` |x|≤ `0.3863` → IC=+0.167 (n=878)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.3863 (IC base=+0.164)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.182 (n=382)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5389` → IC=+0.175 (n=665)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.5389 (IC base=+0.164)

- **PATRÓN** `ibs_20min` > `0.8859` → IC=+0.175 (n=333)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` > 0.8859 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` < `0.4193` → IC=+0.174 (n=928)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.4193 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.148` → IC=+0.176 (n=895)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` < 4.148 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` < `1.082` → IC=+0.165 (n=878)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.082 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` > `0.6393` → IC=+0.167 (n=997)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 0.6393 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.078` → IC=+0.173 (n=432)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.078 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` < `1.4349` → IC=+0.175 (n=327)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.4349 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` > `1.5194` → IC=+0.167 (n=874)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.5194 (IC base=+0.164)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.170 (n=995)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.01 (IC base=+0.164)

- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.188 (n=277)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0041 (IC base=+0.150)

- **PATRÓN** `drift_60min` |x|≤ `0.4952` → IC=+0.176 (n=826)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.4952 (IC base=+0.150)
...
- **PATRÓN** `ibs_20min` < `0.7466` → IC=+0.153 (n=826)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.7466 (IC base=+0.150)

- **PATRÓN** `ibs_20min` > `0.1001` → IC=+0.161 (n=826)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.1001 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.6132` → IC=+0.180 (n=179)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.6132 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.387` → IC=+0.161 (n=750)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` < 4.387 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` < `0.6452` → IC=+0.188 (n=277)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` < 0.6452 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` > `0.721` → IC=+0.153 (n=738)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.721 (IC base=+0.150)

- **PATRÓN** `volumen_pendiente_norm` < `0.1513` → IC=+0.153 (n=856)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_pendiente_norm` < 0.1513 (IC base=+0.150)

- **PATRÓN** `volumen_pendiente_norm` > `0.0737` → IC=+0.164 (n=346)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.0737 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` < `2.1956` → IC=+0.164 (n=713)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.1956 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.7767` → IC=+0.153 (n=540)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.7767 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `7573.3328` → IC=+0.175 (n=826)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 7573.3328 (IC base=+0.150)
...
- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.152 (n=225)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0069 (IC base=+0.119)

- **PATRÓN** `drift_60min` |x|≤ `0.3998` → IC=+0.160 (n=151)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.3998 (IC base=+0.119)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.154 (n=151)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` < 10.0 (IC base=+0.119)

- **PATRÓN** `ibs_20min` < `0.1538` → IC=+0.218 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1538 (IC base=+0.119)

- **PATRÓN** `dist_vwap_pct` > `0.6189` → IC=+0.212 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6189 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.684` → IC=+0.132 (n=123)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` > 2.684 (IC base=+0.119)

- **PATRÓN** `volumen_regimen` < `0.9473` → IC=+0.134 (n=151)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 0.9473 (IC base=+0.119)

- **PATRÓN** `volumen_pendiente_norm` < `0.0885` → IC=+0.186 (n=173)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` < 0.0885 (IC base=+0.119)

- **PATRÓN** `volumen_spike_ratio` < `1.867` → IC=+0.135 (n=146)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 1.867 (IC base=+0.119)

- **PATRÓN** `libro_liquidez` > `3331.2707` → IC=+0.152 (n=225)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 3331.2707 (IC base=+0.119)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.160 (n=189)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 49.0 (IC base=+0.119)
...
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.191 (n=418)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.0041 (IC base=+0.093)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.126 (n=877)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 8.0 (IC base=+0.093)

- **PATRÓN** `ibs_20min` > `0.6667` → IC=+0.199 (n=769)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6667 (IC base=+0.093)

- **PATRÓN** `dist_vwap_pct` > `0.157` → IC=+0.157 (n=464)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.157 (IC base=+0.093)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.443` → IC=+0.191 (n=202)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 11.443 (IC base=+0.093)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0834` → IC=+0.130 (n=655)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` < 2.0834 (IC base=+0.093)

- **PATRÓN** `libro_liquidez` > `2424.4386` → IC=+0.138 (n=379)
...
- **PATRÓN** `sigma_h` < `0.006` → IC=+0.161 (n=325)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.006 (IC base=+0.104)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.122 (n=337)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 6.0 (IC base=+0.104)

- **PATRÓN** `ibs_20min` > `0.4823` → IC=+0.182 (n=294)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` > 0.4823 (IC base=+0.104)

- **PATRÓN** `dist_vwap_pct` > `0.14` → IC=+0.171 (n=153)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.14 (IC base=+0.104)

- **PATRÓN** `volumen_regimen` < `1.067` → IC=+0.124 (n=259)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_regimen` < 1.067 (IC base=+0.104)

- **PATRÓN** `volumen_pendiente_norm` < `0.1573` → IC=+0.132 (n=270)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_pendiente_norm` < 0.1573 (IC base=+0.104)
...
- **PATRÓN** `sigma_h` < `0.004` → IC=+0.181 (n=142)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.004 (IC base=+0.106)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.136 (n=303)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 7.0 (IC base=+0.106)

- **PATRÓN** `ibs_20min` > `0.6741` → IC=+0.233 (n=260)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6741 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` > `0.3477` → IC=+0.181 (n=117)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.3477 (IC base=+0.106)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2706` → IC=+0.218 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2706 (IC base=+0.106)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.124 (n=195)

  - _Acción_: Kelly boost +0.62€ cuando `libro_spread` < 0.01 (IC base=+0.106)

- **PATRÓN** `libro_liquidez` > `1135.4629` → IC=+0.155 (n=256)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 1135.4629 (IC base=+0.106)
...
- **PATRÓN** `libro_liquidez` > `2925.8845` → IC=+0.169 (n=243)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 2925.8845 (IC base=+0.108)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.125 (n=62)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=766)
...
- **PATRÓN** `py_entrada` < `0.495` → IC=+0.148 (n=69)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `py_entrada` < 0.495 (IC base=+0.010)

- **PATRÓN** `libro_liquidez` > `3990.7434` → IC=+0.198 (n=51)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 3990.7434 (IC base=+0.010)

### LIQUIDACIONES_60M
- **FILTRO** `liq_imbalance` |x|≤ `1.0` → IC=-0.122 (n=80)

  - _Acción_: SKIP cuando `liq_imbalance` |x|≤ 1.0
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=636)
...
- **FILTRO** `py_entrada` < `0.44` → IC=-0.143 (n=208)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=508)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.120 (n=488)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.032 (n=263)

- **FILTRO** `restante_min` < `3.82` → IC=-0.123 (n=245)
...
- **PATRÓN** `py_entrada` < `0.54` → IC=+0.172 (n=56)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `py_entrada` < 0.54 (IC base=+0.037)
...
- **FILTRO** `hora_utc` > `14.0` → IC=-0.136 (n=20)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 14.0
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=39)

- **FILTRO** `profundidad_ratio` < `114.6` → IC=-0.150 (n=38)

  - _Acción_: SKIP cuando `profundidad_ratio` < 114.6
  - _Potencial_: sin este filtro IC_bueno=+0.065 (n=21)

- **FILTRO** `py_entrada` > `0.51` → IC=-0.123 (n=51)
...
- **FILTRO** `lag_apertura_s` > `139.54` → IC=-0.125 (n=30)

  - _Acción_: SKIP cuando `lag_apertura_s` > 139.54
  - _Potencial_: sin este filtro IC_bueno=+0.106 (n=31)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.204 (n=25)
...
- **PATRÓN** `restante_min` > `13.48` → IC=+0.182 (n=20)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `restante_min` > 13.48 (IC base=-0.008)

- **PATRÓN** `lag_apertura_s` < `90.92` → IC=+0.222 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `lag_apertura_s` < 90.92 (IC base=-0.008)

- **PATRÓN** `py_entrada` < `0.6` → IC=+0.167 (n=49)

  - _Acción_: Kelly boost +0.83€ cuando `py_entrada` < 0.6 (IC base=+0.040)

- **PATRÓN** `restante_min` > `13.48` → IC=+0.224 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `restante_min` > 13.48 (IC base=+0.040)

- **PATRÓN** `lag_apertura_s` < `90.9` → IC=+0.241 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `lag_apertura_s` < 90.9 (IC base=+0.040)

- **PATRÓN** `profundidad_ratio` > `4.8` → IC=+0.141 (n=37)

  - _Acción_: Kelly boost +0.71€ cuando `profundidad_ratio` > 4.8 (IC base=+0.040)

### LIQUIDACIONES_DEPTH_FASE0#SOL#5min
- **PATRÓN** `py_entrada` < `0.42` → IC=+0.167 (n=19)

  - _Acción_: Kelly boost +0.83€ cuando `py_entrada` < 0.42 (IC base=+0.007)

### LIQUIDACIONES_DEPTH_FASE0#XRP#15min
- **FILTRO** `py_entrada` < `0.49` → IC=-0.167 (n=61)

  - _Acción_: SKIP cuando `py_entrada` < 0.49
  - _Potencial_: sin este filtro IC_bueno=+0.083 (n=34)

- **FILTRO** `profundidad_ratio` < `24.4` → IC=-0.136 (n=31)
...
- **PATRÓN** `profundidad_ratio` > `13.0` → IC=+0.200 (n=18)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `profundidad_ratio` > 13.0 (IC base=+0.027)

### LIQUIDACIONES_DEPTH_FASE0#XRP#5min
- **FILTRO** `py_entrada` < `0.49` → IC=-0.186 (n=84)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=7593)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.166 (n=3613)

  - _Acción_: SKIP cuando `py_entrada` < 0.475
  - _Potencial_: sin este filtro IC_bueno=+0.058 (n=10927)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.165 (n=3743)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.035 (n=11309)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.46` → IC=-0.208 (n=622)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.100 (n=1906)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.47` → IC=-0.194 (n=631)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=1948)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.203 (n=651)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.061 (n=2045)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.485` → IC=-0.176 (n=612)

  - _Acción_: SKIP cuando `py_entrada` < 0.485
  - _Potencial_: sin este filtro IC_bueno=+0.085 (n=1905)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.176 (n=655)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=2047)

### MOMENTUM_IBS_15M_FADE
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=2810)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=2978)

### MOMENTUM_IBS_15M_FADE#BTC#15min
- **FILTRO** `py_entrada` < `0.465` → IC=-0.252 (n=151)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=788)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.129 (n=10193)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=23026)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.273 (n=8161)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=25058)

- **FILTRO** `ibs_7min` < `0.2807` → IC=-0.233 (n=8304)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2807
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=24915)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.156 (n=11282)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=21937)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.231 (n=10224)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=31488)

- **FILTRO** `ibs_7min` > `0.2927` → IC=-0.178 (n=10426)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2927
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=31286)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.136 (n=1664)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3845)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.309 (n=1310)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4199)

- **FILTRO** `ibs_7min` < `0.7111` → IC=-0.249 (n=1817)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7111
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=3692)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.186 (n=1236)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4273)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.259 (n=1768)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=5402)

- **FILTRO** `drift_7min_pct` |x|> `0.1119` → IC=-0.126 (n=2436)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1119
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=4734)

- **FILTRO** `ibs_7min` > `0.7881` → IC=-0.207 (n=1792)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7881
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=5378)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.137 (n=1334)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=4395)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.250 (n=1389)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=4340)

- **FILTRO** `ibs_7min` < `0.7468` → IC=-0.191 (n=1430)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7468
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4299)

- **FILTRO** `ballena_activa_n` > `161.0` → IC=-0.176 (n=1428)

  - _Acción_: SKIP cuando `ballena_activa_n` > 161.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4301)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.263 (n=1336)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=4478)

- **FILTRO** `ibs_7min` > `0.2609` → IC=-0.179 (n=1452)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2609
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4362)

- **FILTRO** `ballena_activa_n` > `154.0` → IC=-0.182 (n=1450)

  - _Acción_: SKIP cuando `ballena_activa_n` > 154.0
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=4364)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.166 (n=1278)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=3957)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.303 (n=1294)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=3941)

- **FILTRO** `ibs_7min` < `0.1935` → IC=-0.257 (n=1308)

  - _Acción_: SKIP cuando `ibs_7min` < 0.1935
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=3927)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.222 (n=1196)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=4039)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.237 (n=1754)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=5919)

- **FILTRO** `ibs_7min` > `0.7468` → IC=-0.172 (n=1918)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7468
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=5755)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=1755)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=3729)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.233 (n=1624)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=3860)

- **FILTRO** `ibs_7min` < `0.7406` → IC=-0.179 (n=1371)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7406
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4113)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.173 (n=1339)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=4145)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.260 (n=1395)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=4191)

- **FILTRO** `ibs_7min` > `0.2745` → IC=-0.179 (n=1396)
...
- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.184 (n=1373)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4213)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.255 (n=1426)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=4353)

- **FILTRO** `ibs_7min` < `0.3` → IC=-0.233 (n=1440)

  - _Acción_: SKIP cuando `ibs_7min` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=4339)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.180 (n=1882)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=6056)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.254 (n=1788)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3695)

- **FILTRO** `ibs_7min` < `0.299` → IC=-0.225 (n=1370)

  - _Acción_: SKIP cuando `ibs_7min` < 0.299
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=4113)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.215 (n=1337)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=4146)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.207 (n=1789)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=5742)
...
### ORDER_FLOW_5M
- **PATRÓN** `delta_ratio` |x|> `0.3979` → IC=+0.129 (n=756)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.65€ cuando `delta_ratio` |x|> 0.3979 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.122 (n=686)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.240 (n=344)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.207)

- **PATRÓN** `ibs_15` > `0.7036` → IC=+0.277 (n=370)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **PATRÓN** `libro_liquidez` > `16178.7073` → IC=+0.238 (n=124)
...
- **PATRÓN** `sigma_h` < `0.0065` → IC=+0.140 (n=387)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0065 (IC base=+0.133)

- **PATRÓN** `sigma_h` > `0.0051` → IC=+0.150 (n=258)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0051 (IC base=+0.133)

- **PATRÓN** `drift_60min` |x|≤ `0.0678` → IC=+0.151 (n=170)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.0678 (IC base=+0.133)

- **PATRÓN** `delta_ratio_macro` |x|> `0.232` → IC=+0.172 (n=129)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.86€ cuando `delta_ratio_macro` |x|> 0.232 (IC base=+0.133)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.258` → IC=+0.156 (n=277)

  - _Acción_: Kelly boost +0.78€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.258 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.150 (n=287)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` > `0.6586` → IC=+0.253 (n=346)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6586 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `0.1604` → IC=+0.150 (n=304)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.1604 (IC base=+0.133)
...
- **PATRÓN** `libro_liquidez` > `3410.3513` → IC=+0.141 (n=346)

  - _Acción_: Kelly boost +0.70€ cuando `libro_liquidez` > 3410.3513 (IC base=+0.133)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1078` → IC=+0.347 (n=280)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1078 (IC base=+0.341)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1435` → IC=+0.368 (n=279)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.361 (n=423)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.341)

- **PATRÓN** `ibs_15` > `0.7873` → IC=+0.384 (n=420)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7873 (IC base=+0.341)

- **PATRÓN** `dist_vwap_pct` > `0.4316` → IC=+0.391 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4316 (IC base=+0.341)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.231` → IC=+0.347 (n=246)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.346 (n=512)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.341)

- **PATRÓN** `libro_liquidez` > `3480.6224` → IC=+0.353 (n=420)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3480.6224 (IC base=+0.341)

- **PATRÓN** `ballena_activa_n` < `462.0` → IC=+0.362 (n=347)
...
- **FILTRO** `sigma_h` > `0.0129` → IC=-0.221 (n=672)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0129
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=2017)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.196 (n=916)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=1773)
...
- **FILTRO** `sigma_h` > `0.0068` → IC=-0.226 (n=410)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0068
  - _Potencial_: sin este filtro IC_bueno=-0.189 (n=1233)

- **FILTRO** `sigma_h` < `0.0034` → IC=-0.231 (n=410)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0034
  - _Potencial_: sin este filtro IC_bueno=-0.187 (n=1233)

- **FILTRO** `hora_utc` > `7.0` → IC=-0.209 (n=1041)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 7.0
...
- **PATRÓN** `dist_vwap_pct` < `0.3703` → IC=+0.274 (n=175)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.132 (n=17)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.163 (n=387)

- **PATRÓN** `sigma_h` < `0.006` → IC=+0.154 (n=267)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.006 (IC base=+0.150)

- **PATRÓN** `sigma_h` > `0.0051` → IC=+0.186 (n=202)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0051 (IC base=+0.150)

- **PATRÓN** `drift_60min` |x|≤ `0.0764` → IC=+0.221 (n=134)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0764 (IC base=+0.150)

- **PATRÓN** `drift_15min` |x|≤ `0.4223` → IC=+0.164 (n=102)

  - _Acción_: Kelly boost +0.82€ cuando `drift_15min` |x|≤ 0.4223 (IC base=+0.150)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0589` → IC=+0.156 (n=303)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.78€ cuando `delta_ratio_macro` |x|> 0.0589 (IC base=+0.150)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.208 (n=142)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.150)

- **PATRÓN** `ibs_15` > `0.6686` → IC=+0.264 (n=303)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6686 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.4741` → IC=+0.152 (n=87)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.4741 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` < `0.1175` → IC=+0.190 (n=217)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.1175 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.115` → IC=+0.158 (n=258)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` < 9.115 (IC base=+0.150)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.163 (n=387)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `11008.7835` → IC=+0.193 (n=138)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 11008.7835 (IC base=+0.150)

- **PATRÓN** `sigma_h` < `0.0077` → IC=+0.245 (n=716)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0077 (IC base=+0.231)

- **PATRÓN** `drift_60min` |x|≤ `0.4447` → IC=+0.234 (n=716)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4447 (IC base=+0.231)

- **PATRÓN** `drift_15min` |x|≤ `0.4794` → IC=+0.244 (n=315)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.4794 (IC base=+0.231)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2013` → IC=+0.265 (n=325)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.2013 (IC base=+0.231)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.239 (n=278)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` < `0.2691` → IC=+0.285 (n=630)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.2691 (IC base=+0.231)

- **PATRÓN** `dist_vwap_pct` > `0.764` → IC=+0.320 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.764 (IC base=+0.231)

- **PATRÓN** `sigma_ewma_delta_pct` > `20.297` → IC=+0.250 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 20.297 (IC base=+0.231)

- **PATRÓN** `sigma_ewma_delta_pct` < `12.221` → IC=+0.240 (n=759)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.215 (n=247)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.142 (n=389)
...
- **FILTRO** `pct_spot_vs_ref` |x|> `0.2012` → IC=-0.198 (n=266)
...
- **FILTRO** `drift_15min` |x|> `1.2727` → IC=-0.268 (n=196)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.173 (n=589)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1333` → IC=+0.292 (n=219)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **PATRÓN** `ibs_15` < `0.3443` → IC=+0.306 (n=483)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
### UPDOWN_GBM_IBS_ALTO
- **PATRÓN** `sigma_h` < `0.0045` → IC=+0.302 (n=448)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0045 (IC base=+0.294)

- **PATRÓN** `drift_60min` |x|≤ `0.0563` → IC=+0.332 (n=224)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `libro_liquidez` > `16196.8854` → IC=+0.325 (n=124)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 16196.8854 (IC base=+0.288)

### UPDOWN_GBM_IBS_ALTO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0068` → IC=+0.312 (n=302)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0068 (IC base=+0.299)

- **PATRÓN** `drift_60min` |x|≤ `0.1127` → IC=+0.300 (n=203)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1127 (IC base=+0.299)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.331 (n=270)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.299)

- **PATRÓN** `ibs_15` > `0.8527` → IC=+0.339 (n=302)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **PATRÓN** `dist_vwap_pct` < `0.1658` → IC=+0.302 (n=215)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.307 (n=340)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6053 sube el IC de +0.186 a +0.267 en UPDOWN_GBM#15min (n=1662). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.7036 sube el IC de +0.207 a +0.277 en UPDOWN_GBM#BTC#15min (n=370). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.6586 sube el IC de +0.133 a +0.253 en UPDOWN_GBM#ETH#15min (n=346). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1176 sube el IC de +0.050 a +0.158 en UPDOWN_GBM#XRP#15min (n=492). Ya aplicado como kelly_boost=+0.79€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.6395 sube el IC de -0.065 a +0.276 en UPDOWN_GBM_15M_TARDIO (n=640). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3482 sube el IC de -0.031 a +0.276 en UPDOWN_GBM_15M_TARDIO (n=1840). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_YES, IBS > 0.6686 sube el IC de +0.150 a +0.264 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=303). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.2691 sube el IC de +0.231 a +0.285 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=630). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#XRP#15min**: dentro de BUY_NO, IBS < 0.3443 sube el IC de -0.042 a +0.306 en UPDOWN_GBM_15M_TARDIO#XRP#15min (n=483). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.8411 sube el IC de +0.294 a +0.328 en UPDOWN_GBM_IBS_ALTO (n=672). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_IBS_ALTO#BTC#15min**: dentro de BUY_YES, IBS > 0.83 sube el IC de +0.288 a +0.317 en UPDOWN_GBM_IBS_ALTO#BTC#15min (n=370). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_IBS_ALTO#ETH#15min**: dentro de BUY_YES, IBS > 0.8527 sube el IC de +0.299 a +0.339 en UPDOWN_GBM_IBS_ALTO#ETH#15min (n=302). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD**: dentro de BUY_YES, IBS > 0.7873 sube el IC de +0.341 a +0.384 en UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD (n=420). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 28782 | -0.088 | -3853.47€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1493 | -0.044 | -221.37€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 27289 | -0.090 | -3632.10€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3794 | -0.099 | -626.48€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3794 | -0.099 | -626.48€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 1493 | -0.044 | -221.37€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 1493 | -0.044 | -221.37€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 3482 | -0.096 | -787.96€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 3482 | -0.096 | -787.96€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 7368 | -0.020 | -688.42€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 7368 | -0.020 | -688.42€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 6986 | -0.090 | -439.95€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 6986 | -0.090 | -439.95€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 5659 | -0.172 | -1089.29€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 5659 | -0.172 | -1089.29€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 18822 | -0.027 | +3980.19€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 4907 | -0.000 | +1809.70€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 13915 | -0.037 | +2170.50€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 18822 | -0.027 | +3980.19€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 4907 | -0.000 | +1809.70€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 13915 | -0.037 | +2170.50€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 94877 | +0.112 | -4800.36€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 14261 | +0.185 | -426.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 382 | -0.078 | -49.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 74095 | +0.100 | -4102.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 6139 | +0.107 | -221.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 12321 | +0.097 | -1046.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 45 | -0.160 | -0.32€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 12261 | +0.099 | -1034.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 19191 | +0.132 | -365.81€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 4507 | +0.204 | -128.15€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 12284 | +0.111 | -197.37€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 2358 | +0.103 | -18.06€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 12361 | +0.089 | -1132.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 52 | -0.093 | -6.98€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#DOGE#240min | 15 | -0.243 | -11.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 12294 | +0.091 | -1114.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 20167 | +0.123 | -401.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 5514 | +0.175 | -80.39€ | 1 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 12417 | +0.105 | -253.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 2224 | +0.098 | -58.69€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 18502 | +0.113 | -1113.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 4096 | +0.188 | -218.49€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 285 | -0.037 | +4.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 12564 | +0.090 | -754.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1557 | +0.125 | -145.10€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 12335 | +0.100 | -739.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 47 | -0.031 | +8.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 13 | -0.022 | -0.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 12275 | +0.101 | -747.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 15052 | +0.191 | -994.45€ | 1 | 5 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 15052 | +0.191 | -994.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 3576 | +0.167 | -377.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 3576 | +0.167 | -377.35€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC | 1285 | +0.195 | -14.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC#15min | 1285 | +0.195 | -14.24€ | 4 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 3529 | +0.179 | -304.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 3529 | +0.179 | -304.59€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 3124 | +0.239 | -100.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 3124 | +0.239 | -100.30€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 3459 | +0.192 | -211.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 3459 | +0.192 | -211.72€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 51920 | +0.197 | -4060.44€ | 3 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 51920 | +0.197 | -4060.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 8975 | +0.176 | -1040.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 8975 | +0.176 | -1040.55€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 8295 | +0.223 | -302.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 8295 | +0.223 | -302.23€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 8969 | +0.172 | -1074.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 8969 | +0.172 | -1074.12€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 8389 | +0.218 | -330.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 8389 | +0.218 | -330.64€ | 2 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 8583 | +0.204 | -559.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 8583 | +0.204 | -559.89€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 8709 | +0.193 | -753.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 8709 | +0.193 | -753.01€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 19580 | +0.117 | +156.81€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 19580 | +0.117 | +156.81€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 9724 | +0.121 | +138.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 9724 | +0.121 | +138.19€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 9856 | +0.113 | +18.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 9856 | +0.113 | +18.62€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION | 1498 | +0.289 | -18.62€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#60min | 1498 | +0.289 | -18.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#BTC | 668 | +0.276 | -20.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#BTC#60min | 668 | +0.276 | -20.01€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#ETH | 719 | +0.292 | -0.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#ETH#60min | 719 | +0.292 | -0.74€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 36464 | +0.097 | -1108.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 3010 | +0.090 | +27.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 33454 | +0.098 | -1135.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC | 20454 | +0.101 | -329.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#15min | 3010 | +0.090 | +27.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#5min | 17444 | +0.103 | -356.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH | 6886 | +0.106 | -44.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH#5min | 6886 | +0.106 | -44.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL | 9124 | +0.081 | -734.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL#5min | 9124 | +0.081 | -734.40€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 26130 | +0.083 | +12397.08€ | 0 | 15 |
| ✅ GBM_LATE_15M#15min | 26130 | +0.083 | +12397.08€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 4366 | +0.193 | +3165.50€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 4366 | +0.193 | +3165.50€ | 0 | 20 |
| ✅ GBM_LATE_15M#BTC | 3890 | +0.178 | +2729.89€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 3890 | +0.178 | +2729.89€ | 0 | 26 |
| ✅ GBM_LATE_15M#DOGE | 4559 | +0.196 | +3367.05€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 4559 | +0.196 | +3367.05€ | 0 | 21 |
| ✅ GBM_LATE_15M#ETH | 3830 | +0.021 | +836.03€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 3830 | +0.021 | +836.03€ | 1 | 15 |
| ✅ GBM_LATE_15M#SOL | 3735 | -0.033 | +839.33€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 3735 | -0.033 | +839.33€ | 4 | 14 |
| ✅ GBM_LATE_15M#XRP | 5750 | -0.040 | +1459.28€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 5750 | -0.040 | +1459.28€ | 4 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 27625 | +0.086 | +14412.65€ | 0 | 19 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 27625 | +0.086 | +14412.65€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 5246 | +0.017 | +2767.47€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 5246 | +0.017 | +2767.47€ | 2 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 5767 | +0.015 | +1197.65€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 5767 | +0.015 | +1197.65€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 3924 | +0.261 | +3937.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 3924 | +0.261 | +3937.25€ | 0 | 21 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 4489 | +0.004 | +864.36€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 4489 | +0.004 | +864.36€ | 2 | 15 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 4475 | +0.028 | +1685.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 4475 | +0.028 | +1685.84€ | 3 | 17 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 3724 | +0.275 | +3960.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 3724 | +0.275 | +3960.08€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 21021 | +0.167 | +15609.40€ | 0 | 26 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 21021 | +0.167 | +15609.40€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB | 3164 | +0.206 | +2506.44€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB#15min | 3164 | +0.206 | +2506.44€ | 0 | 21 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC | 3337 | +0.149 | +2437.23€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC#15min | 3337 | +0.149 | +2437.23€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE | 3302 | +0.207 | +2611.28€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min | 3302 | +0.207 | +2611.28€ | 0 | 18 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH | 3522 | +0.132 | +2440.59€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH#15min | 3522 | +0.132 | +2440.59€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL | 3916 | +0.115 | +2661.62€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL#15min | 3916 | +0.115 | +2661.62€ | 0 | 22 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP | 3780 | +0.202 | +2952.22€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP#15min | 3780 | +0.202 | +2952.22€ | 0 | 27 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 5296 | +0.128 | +2247.09€ | 0 | 24 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 5296 | +0.128 | +2247.09€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH | 1579 | +0.146 | +721.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH#15min | 1579 | +0.146 | +721.84€ | 0 | 18 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#SOL | 1132 | +0.106 | +381.97€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#SOL#15min | 1132 | +0.106 | +381.97€ | 1 | 16 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 506 | +0.134 | +215.28€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 506 | +0.134 | +215.28€ | 0 | 27 |
| ✅ GBM_LATE_15M_TARDIO | 26260 | +0.174 | +19491.54€ | 0 | 23 |
| ✅ GBM_LATE_15M_TARDIO#15min | 26260 | +0.174 | +19491.54€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 4160 | +0.219 | +3490.29€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 4160 | +0.219 | +3490.29€ | 0 | 21 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 4125 | +0.151 | +2726.39€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 4125 | +0.151 | +2726.39€ | 0 | 27 |
| ✅ GBM_LATE_15M_TARDIO#DOGE | 4318 | +0.223 | +3683.62€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#DOGE#15min | 4318 | +0.223 | +3683.62€ | 0 | 20 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 4256 | +0.136 | +2899.70€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 4256 | +0.136 | +2899.70€ | 0 | 25 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 4587 | +0.113 | +2924.32€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 4587 | +0.113 | +2924.32€ | 0 | 20 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 4814 | +0.204 | +3767.20€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 4814 | +0.204 | +3767.20€ | 0 | 24 |
| ✅ GBM_LATE_5M | 7184 | +0.151 | +4190.47€ | 1 | 28 |
| ✅ GBM_LATE_5M#5min | 7184 | +0.151 | +4190.47€ | 0 | 0 |
...
| ✅ GBM_LATE_5M#ETH | 2430 | +0.158 | +1444.32€ | 0 | 0 |
| ✅ GBM_LATE_5M#ETH#5min | 2430 | +0.158 | +1444.32€ | 0 | 30 |
| ✅ GBM_LATE_5M#SOL | 625 | +0.112 | +240.70€ | 0 | 0 |
| ✅ GBM_LATE_5M#SOL#5min | 625 | +0.112 | +240.70€ | 0 | 18 |
| ✅ GBM_LATE_5M#XRP | 837 | +0.116 | +326.92€ | 0 | 0 |
| ✅ GBM_LATE_5M#XRP#5min | 837 | +0.116 | +326.92€ | 0 | 0 |
| ✅ GBM_LATE_60M | 1768 | +0.068 | +750.69€ | 3 | 11 |
| ✅ GBM_LATE_60M#60min | 1768 | +0.068 | +750.69€ | 0 | 0 |
| ✅ GBM_LATE_60M#BTC | 642 | +0.087 | +267.71€ | 0 | 0 |
| ✅ GBM_LATE_60M#BTC#60min | 642 | +0.087 | +267.71€ | 0 | 12 |
| ✅ GBM_LATE_60M#ETH | 584 | +0.072 | +295.54€ | 0 | 0 |
| ✅ GBM_LATE_60M#ETH#60min | 584 | +0.072 | +295.54€ | 2 | 15 |
...
| ✅ LEADLAG_BTC_XRP_15M | 1999 | +0.104 | +555.80€ | 0 | 1 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 1999 | +0.104 | +555.80€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 1999 | +0.104 | +555.80€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 1999 | +0.104 | +555.80€ | 0 | 1 |
...
| ✅ LIQUIDACIONES_5M | 2051 | +0.007 | +18.97€ | 5 | 0 |
| ✅ LIQUIDACIONES_5M#5min | 2051 | +0.007 | +18.97€ | 0 | 0 |
...
| ✅ LIQUIDACIONES_5M#ETH | 859 | +0.026 | +23.85€ | 0 | 0 |
| ✅ LIQUIDACIONES_5M#ETH#5min | 859 | +0.026 | +23.85€ | 6 | 0 |
| ✅ LIQUIDACIONES_5M#SOL | 473 | -0.001 | -5.44€ | 0 | 0 |
| ✅ LIQUIDACIONES_5M#SOL#5min | 473 | -0.001 | -5.44€ | 4 | 0 |
| ✅ LIQUIDACIONES_5M#XRP | 217 | -0.002 | +0.55€ | 0 | 0 |
| ✅ LIQUIDACIONES_5M#XRP#5min | 217 | -0.002 | +0.55€ | 1 | 2 |
| ✅ LIQUIDACIONES_60M | 1101 | -0.043 | -26.16€ | 5 | 0 |
| ✅ LIQUIDACIONES_60M#60min | 1101 | -0.043 | -26.16€ | 0 | 0 |
| ✅ LIQUIDACIONES_60M#BTC | 313 | -0.043 | -13.27€ | 0 | 0 |
| ✅ LIQUIDACIONES_60M#BTC#60min | 313 | -0.043 | -13.27€ | 6 | 0 |
| ✅ LIQUIDACIONES_60M#ETH | 371 | -0.025 | -0.54€ | 0 | 0 |
| ✅ LIQUIDACIONES_60M#ETH#60min | 371 | -0.025 | -0.54€ | 3 | 0 |
| ✅ LIQUIDACIONES_60M#SOL | 417 | -0.059 | -12.36€ | 0 | 0 |
| ✅ LIQUIDACIONES_60M#SOL#60min | 417 | -0.059 | -12.36€ | 3 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0 | 1542 | -0.021 | +28.43€ | 2 | 1 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#15min | 726 | -0.019 | +9.35€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#5min | 816 | -0.022 | +19.08€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#BNB | 40 | +0.024 | +6.50€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#BNB#15min | 23 | +0.020 | +2.88€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#BNB#5min | 17 | +0.022 | +3.62€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#BTC | 379 | +0.038 | +53.79€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#BTC#15min | 174 | +0.017 | +11.30€ | 1 | 1 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#BTC#5min | 205 | +0.056 | +42.49€ | 0 | 1 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#DOGE | 208 | -0.057 | -10.77€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#DOGE#15min | 99 | -0.054 | -5.40€ | 1 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#DOGE#5min | 109 | -0.059 | -5.37€ | 3 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#ETH | 289 | -0.077 | -32.70€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#ETH#15min | 128 | -0.069 | -10.55€ | 4 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#ETH#5min | 161 | -0.083 | -22.15€ | 4 | 2 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#SOL | 266 | +0.007 | +22.25€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#SOL#15min | 135 | +0.018 | +13.80€ | 3 | 6 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#SOL#5min | 131 | -0.004 | +8.44€ | 0 | 1 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#XRP | 360 | -0.041 | -10.64€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#XRP#15min | 167 | -0.033 | -2.69€ | 2 | 1 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#XRP#5min | 193 | -0.049 | -7.95€ | 3 | 3 |
| ✅ MOMENTUM_IBS_15M | 14457 | -0.011 | -206.57€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M#15min | 14457 | -0.011 | -206.57€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB | 578 | -0.010 | -0.50€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB#15min | 578 | -0.010 | -0.50€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BTC | 3174 | -0.019 | -59.76€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BTC#15min | 3174 | -0.019 | -59.76€ | 2 | 0 |
...
| ✅ MOMENTUM_IBS_15M_BALLENA | 29592 | -0.007 | +1332.77€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#15min | 29592 | -0.007 | +1332.77€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB | 5225 | +0.018 | +638.84€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB#15min | 5225 | +0.018 | +638.84€ | 1 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC | 4569 | -0.028 | -49.55€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC#15min | 4569 | -0.028 | -49.55€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE | 5275 | +0.014 | +469.43€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE#15min | 5275 | +0.014 | +469.43€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH | 4347 | -0.051 | -127.62€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH#15min | 4347 | -0.051 | -127.62€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL | 4957 | -0.010 | +190.38€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL#15min | 4957 | -0.010 | +190.38€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP | 5219 | +0.008 | +211.29€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP#15min | 5219 | +0.008 | +211.29€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE | 5889 | -0.058 | -131.71€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#15min | 5889 | -0.058 | -131.71€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#BNB | 1212 | +0.001 | -14.22€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#BNB#15min | 1212 | +0.001 | -14.22€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#BTC | 1433 | -0.083 | -37.52€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#BTC#15min | 1433 | -0.083 | -37.52€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#DOGE | 44 | -0.130 | -5.93€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#DOGE#15min | 44 | -0.130 | -5.93€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#ETH | 644 | -0.110 | -17.58€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#ETH#15min | 644 | -0.110 | -17.58€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#SOL | 1709 | -0.077 | -30.75€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#SOL#15min | 1709 | -0.077 | -30.75€ | 1 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 74931 | -0.073 | +1746.84€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 74931 | -0.073 | +1746.84€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 12679 | -0.079 | +730.57€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 12679 | -0.079 | +730.57€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 11543 | -0.092 | -500.27€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 11543 | -0.092 | -500.27€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 12908 | -0.067 | +728.35€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 12908 | -0.067 | +728.35€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 11070 | -0.093 | -201.31€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 11070 | -0.093 | -201.31€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 13717 | -0.048 | +387.36€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 13717 | -0.048 | +387.36€ | 3 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 13014 | -0.063 | +602.14€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 13014 | -0.063 | +602.14€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE | 7633 | -0.025 | -124.19€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 7633 | -0.025 | -124.19€ | 0 | 0 |
...
| ✅ ORDER_FLOW_5M | 1144 | +0.106 | +379.76€ | 0 | 3 |
| ✅ ORDER_FLOW_5M#5min | 1008 | +0.113 | +367.16€ | 0 | 0 |
...
| ✅ ORDER_FLOW_5M_REACTIVO | 512 | -0.064 | -64.46€ | 0 | 0 |
| ✅ ORDER_FLOW_5M_REACTIVO#5min | 512 | -0.064 | -64.46€ | 0 | 0 |
| ✅ ORDER_FLOW_5M_REACTIVO#BNB | 111 | -0.004 | +2.77€ | 0 | 0 |
| ✅ ORDER_FLOW_5M_REACTIVO#BNB#5min | 111 | -0.004 | +2.77€ | 0 | 0 |
| ✅ ORDER_FLOW_5M_REACTIVO#DOGE | 61 | -0.151 | -20.44€ | 0 | 0 |
| ✅ ORDER_FLOW_5M_REACTIVO#DOGE#5min | 61 | -0.151 | -20.44€ | 0 | 0 |
...
| ✅ STREAK_FADE_5M | 2799 | -0.022 | -114.85€ | 0 | 0 |
| ✅ STREAK_FADE_5M#5min | 2799 | -0.022 | -114.85€ | 0 | 0 |
...
| ✅ STREAK_MOM_5M | 7904 | +0.024 | +124.19€ | 0 | 0 |
| ✅ STREAK_MOM_5M#5min | 7904 | +0.024 | +124.19€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE | 2144 | +0.022 | +23.43€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE#5min | 2144 | +0.022 | +23.43€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH | 1797 | +0.036 | +56.30€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH#5min | 1797 | +0.036 | +56.30€ | 1 | 0 |
| ✅ STREAK_MOM_5M#SOL | 2406 | +0.014 | +7.90€ | 0 | 0 |
| ✅ STREAK_MOM_5M#SOL#5min | 2406 | +0.014 | +7.90€ | 1 | 0 |
| ✅ STREAK_MOM_5M#XRP | 1557 | +0.028 | +36.56€ | 0 | 0 |
| ✅ STREAK_MOM_5M#XRP#5min | 1557 | +0.028 | +36.56€ | 2 | 0 |
| ✅ STRUCT_NO_15M | 7436 | +0.014 | -30.22€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 7436 | +0.014 | -30.22€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 2986 | +0.018 | -1.81€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 2986 | +0.018 | -1.81€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 2904 | +0.013 | -13.79€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 2904 | +0.013 | -13.79€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 1546 | +0.006 | -14.62€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 1546 | +0.006 | -14.62€ | 2 | 0 |
| ✅ UPDOWN_GBM | 38403 | +0.031 | +2307.95€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 10442 | +0.068 | +1890.04€ | 0 | 10 |
| ✅ UPDOWN_GBM#240min | 1418 | +0.004 | +6.44€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 24042 | +0.020 | +388.48€ | 0 | 0 |
| ✅ UPDOWN_GBM#60min | 2353 | +0.004 | +23.99€ | 1 | 0 |
| ✅ UPDOWN_GBM#BNB | 4000 | +0.068 | +450.93€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 730 | +0.154 | +298.45€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#240min | 32 | +0.000 | -0.19€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#5min | 3238 | +0.049 | +152.67€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 7137 | +0.037 | +504.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 1320 | +0.083 | +293.83€ | 0 | 10 |
| ✅ UPDOWN_GBM#BTC#240min | 381 | +0.020 | +7.79€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#5min | 4323 | +0.035 | +177.59€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#60min | 1056 | +0.003 | +24.06€ | 1 | 0 |
| ✅ UPDOWN_GBM#BTC#daily | 57 | -0.093 | +1.11€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 4540 | +0.040 | +282.12€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 677 | +0.138 | +236.91€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#240min | 28 | +0.000 | -1.43€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 3835 | +0.023 | +46.64€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 8136 | +0.018 | +286.54€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 2661 | +0.046 | +269.67€ | 0 | 11 |
| ✅ UPDOWN_GBM#ETH#240min | 372 | +0.005 | +6.91€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 4258 | +0.007 | +12.72€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 797 | +0.001 | -5.82€ | 2 | 0 |
| ✅ UPDOWN_GBM#ETH#daily | 48 | -0.140 | +3.06€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 8982 | +0.013 | +211.53€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 2510 | +0.026 | +171.89€ | 0 | 12 |
| ✅ UPDOWN_GBM#SOL#240min | 364 | -0.005 | -2.40€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 5567 | +0.010 | +39.63€ | 1 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 500 | +0.014 | +5.75€ | 0 | 1 |
| ✅ UPDOWN_GBM#SOL#daily | 41 | -0.174 | -3.33€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 5606 | +0.035 | +574.27€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 2544 | +0.082 | +619.28€ | 0 | 13 |
| ✅ UPDOWN_GBM#XRP#240min | 241 | -0.006 | -4.24€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 2821 | -0.005 | -40.77€ | 0 | 0 |
| ✅ UPDOWN_GBM#daily | 146 | -0.135 | +0.84€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD | 559 | +0.341 | +169.25€ | 0 | 13 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#15min | 559 | +0.341 | +169.25€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 12270 | -0.038 | +2571.01€ | 2 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 12270 | -0.038 | +2571.01€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 819 | -0.040 | +364.34€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 819 | -0.040 | +364.34€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 2254 | -0.123 | +6.20€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 2254 | -0.123 | +6.20€ | 4 | 8 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 426 | +0.175 | +276.85€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 426 | +0.175 | +276.85€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 1358 | +0.207 | +800.28€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 1358 | +0.207 | +800.28€ | 2 | 23 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 3694 | -0.064 | +545.74€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 3694 | -0.064 | +545.74€ | 3 | 5 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 3719 | -0.075 | +577.60€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 3719 | -0.075 | +577.60€ | 4 | 4 |
...
| ✅ UPDOWN_GBM_IBS_ALTO | 895 | +0.294 | +723.56€ | 0 | 10 |
| ✅ UPDOWN_GBM_IBS_ALTO#15min | 895 | +0.294 | +723.56€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#BTC | 493 | +0.288 | +378.32€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#BTC#15min | 493 | +0.288 | +378.32€ | 0 | 10 |
| ✅ UPDOWN_GBM_IBS_ALTO#ETH | 402 | +0.299 | +345.23€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#ETH#15min | 402 | +0.299 | +345.23€ | 0 | 10 |
...
**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  ...
  - _Estado_: 531 celda(s) pasan gate riguroso completo de 2259 evaluadas (n>=40) y 3230 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.026 < 0.08 — monitorear
  - _Datos_: n=2507 IC=+0.026 PNL=+172.42€
...
**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  ...
  - _Estado_: n_overlaps=295, boost estimado=+0.003. Necesita 0 más y boost>0.05
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=796/40 IC=+0.001 PNL=-5.31€ | BTC#60min: n=1054/40 IC=+0.003 PNL=+24.09€ | SOL#60min: n=499/40 IC=+0.015 PNL=+6.26€

**⏳ H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  - _Umbral_: n≥40 tras 2 losses y gap(IC_tras_win - IC_tras_2loss)≥0.05
  - _Acción_: Reducir stake (no desactivar) 1-2h tras 2 derrotas consecutivas en el mismo subtype
  - _Estado_: tras_win IC=+0.050 n=342666 | tras_1loss IC=+0.079 n=266449 | tras_2loss IC=+0.048 n=112207/40 | gap=+0.002 (umbral 0.05)
...
**🟡 H-24H-GBM-BUYYES-TARDE** — GBM BUY_YES en tarde europea (15-19h UTC) — señal alcista sostenida
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.213 > 0.08 con n=392 PNL=+284.47€
  - _Datos_: n=392 IC=+0.213 PNL=+284.47€
...
**〰️ H-CUSTOM-GBM-60MIN-BUYYES** — GBM 60min BUY_YES — ¿edge superior al BUY_NO?
  ...
  - _Estado_: n=1649 IC=+0.013 PNL=+12.67€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1649 IC=+0.013 PNL=+12.67€

**〰️ H-CUSTOM-GBM-60MIN-BUYNO** — GBM 60min BUY_NO — tracking por separado
  - _Hipótesis_: En 15min BUY_NO tiene IC=+0.119. ¿Se repite en 60min? Datos actuales: 8/14 (57%) IC=+0.044 — positivo pero débil. Puede ser que 60min requiera dirección alcista (BUY_YES) y no bajista.
  - _Umbral_: n≥30 para confirmar dirección
  - _Acción_: Si IC<0.05 con n≥30 → en 60min priorizar solo BUY_YES; si IC>0.08 → igualar al BUY_YES
  - _Estado_: n=700 IC=-0.014 PNL=+12.37€ — sin señal clara aún (umbral IC: min=0.05 max=None)
  - _Datos_: n=700 IC=-0.014 PNL=+12.37€
...
**🟡 H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.185 > 0.1 con n=2206 PNL=+1362.90€
  - _Datos_: n=2206 IC=+0.185 PNL=+1362.90€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=1318 IC=+0.083 PNL=+292.25€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=1318 IC=+0.083 PNL=+292.25€
...
**🟡 H-CUSTOM-OF-EDGE-ALTO** — ORDER_FLOW_5M: edge alto (>0.20) rinde mejor que edge cerca del suelo
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.118 > 0.02 con n=644 PNL=+236.76€
  - _Datos_: n=644 IC=+0.118 PNL=+236.76€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=13449 IC=+0.054 PNL=+1613.33€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=13449 IC=+0.054 PNL=+1613.33€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=1958 IC=+0.051 PNL=+209.81€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1958 IC=+0.051 PNL=+209.81€
...
**🟡 H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.130 > 0.1 con n=425 PNL=+127.53€
  - _Datos_: n=425 IC=+0.130 PNL=+127.53€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=18412 IC=-0.137 PNL=+1275.01€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=18412 IC=-0.137 PNL=+1275.01€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=1984 IC=+0.141 PNL=+1091.62€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=1984 IC=+0.141 PNL=+1091.62€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.237 < -0.1 con n=1795 PNL=-199.21€
  - _Datos_: n=1795 IC=-0.237 PNL=-199.21€
...
**🔶 H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.100 n=1027) pero sin cruzar ≥2 pares más — sin otros pares con datos
  - _Datos_: n=1027 IC=+0.100 PNL=+246.73€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=8969 IC=+0.176 PNL=-1039.30€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=8969 IC=+0.176 PNL=-1039.30€
