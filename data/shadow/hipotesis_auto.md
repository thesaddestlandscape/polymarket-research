# Hipótesis automáticas — 2026-07-19 04:09 UTC
_Generado por shadow_postmortem.py sobre 21856 resoluciones (PNL=+5289.00€)_
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.246 (n=140)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.212)

- **PATRÓN** `py_entrada` < `0.395` → IC=+0.262 (n=305)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.976` → IC=+0.224 (n=168)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.135 (n=1551)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` < 12.0 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `0.1605` → IC=+0.126 (n=362)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` > 0.1605 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.9035` → IC=+0.124 (n=1082)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.9035 (IC base=+0.113)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `11.016` → IC=+0.121 (n=959)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.263` → IC=-0.265 (n=15)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.096 (n=248)
...
- **PATRÓN** `sigma_h` < `0.013` → IC=+0.139 (n=471)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.170 (n=389)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` < `0.0121` → IC=+0.154 (n=241)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0121 (IC base=+0.153)

- **PATRÓN** `sigma_h` > `0.0154` → IC=+0.176 (n=365)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0154 (IC base=+0.153)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.156 (n=550)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 6.0 (IC base=+0.153)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.160 (n=366)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.1141` → IC=+0.152 (n=202)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.1141 (IC base=+0.153)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.472` → IC=+0.264 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.472 (IC base=+0.153)
...
- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.189 (n=1038)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.198 (n=356)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` < 5.0 (IC base=+0.171)

- **PATRÓN** `dist_vwap_pct` < `0.8699` → IC=+0.188 (n=950)
...
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.329 (n=255)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0078 (IC base=+0.298)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.308 (n=264)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.298)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.301 (n=229)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.298)

- **PATRÓN** `dist_vwap_pct` > `0.3075` → IC=+0.325 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3075 (IC base=+0.298)

- **PATRÓN** `dist_vwap_pct` < `0.5637` → IC=+0.309 (n=255)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5637 (IC base=+0.298)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.458` → IC=+0.315 (n=225)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.458 (IC base=+0.298)
...
- **PATRÓN** `sigma_h` > `0.0136` → IC=+0.188 (n=325)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0136 (IC base=+0.129)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.134 (n=887)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 8.0 (IC base=+0.129)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.134 (n=896)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` < 16.0 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` > `0.5395` → IC=+0.172 (n=193)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.5395 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` < `0.1178` → IC=+0.154 (n=490)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.1178 (IC base=+0.129)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.133 (n=404)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` < 6.0 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` < `0.7822` → IC=+0.128 (n=939)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.7822 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.536` → IC=+0.124 (n=243)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 3.536 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.972` → IC=+0.123 (n=837)
...
- **PATRÓN** `sigma_h` < `0.0282` → IC=+0.211 (n=306)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0282 (IC base=+0.209)

- **PATRÓN** `sigma_h` > `0.0148` → IC=+0.243 (n=204)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0148 (IC base=+0.209)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.235 (n=115)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.209)

- **PATRÓN** `dist_vwap_pct` > `0.2409` → IC=+0.263 (n=95)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2409 (IC base=+0.209)
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 269 | +0.142 | +15.82€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 269 | +0.142 | +15.82€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 114 | +0.164 | +8.54€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 114 | +0.164 | +8.54€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 96 | +0.143 | +4.20€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 96 | +0.143 | +4.20€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 59 | +0.090 | +3.08€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 59 | +0.090 | +3.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 3667 | +0.184 | +44.24€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 2378 | +0.218 | +88.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 157 | +0.079 | +5.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 505 | +0.098 | -78.15€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 1301 | +0.179 | +16.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 787 | +0.222 | +48.46€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 52 | -0.018 | -9.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 253 | +0.104 | -35.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 209 | +0.154 | +13.32€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#SOL | 1286 | +0.182 | -5.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 786 | +0.222 | +41.33€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 53 | +0.136 | +9.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 238 | +0.100 | -43.47€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 5713 | +0.106 | +1906.99€ | 0 | 9 |
| ✅ GBM_LATE_15M#15min | 5713 | +0.106 | +1906.99€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#SOL | 1547 | +0.100 | +601.75€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1547 | +0.100 | +601.75€ | 1 | 5 |
| ✅ GBM_LATE_15M#XRP | 1492 | +0.162 | +811.25€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1492 | +0.162 | +811.25€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2943 | +0.153 | +1763.67€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2943 | +0.153 | +1763.67€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 663 | +0.277 | +699.82€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 663 | +0.277 | +699.82€ | 0 | 12 |
...
| ✅ GBM_LATE_15M_TARDIO | 2887 | +0.101 | +1010.25€ | 0 | 11 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2887 | +0.101 | +1010.25€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#SOL | 649 | +0.050 | +129.00€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 649 | +0.050 | +129.00€ | 1 | 5 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 850 | +0.214 | +666.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 850 | +0.214 | +666.60€ | 0 | 10 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.117 n=13124 | tras_1loss IC=+0.087 n=8528 | tras_2loss IC=+0.054 n=3484/40 | gap=+0.063 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=19398 IC=+0.120 PNL=+5364.88€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=19398 IC=+0.120 PNL=+5364.88€
...
**🟡 H-CUSTOM-LATE-04H-ASIA** — GBM_LATE_15M 04h-05h UTC — media sesión asiática (gemela de GBM-04H-ASIA)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.132 > 0.08 con n=463 PNL=+186.54€
  - _Datos_: n=463 IC=+0.132 PNL=+186.54€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.172 > 0.08 con n=342 PNL=+222.50€
  - _Datos_: n=342 IC=+0.172 PNL=+222.50€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.194 > 0.08 con n=197 PNL=+3.71€
  - _Datos_: n=197 IC=+0.194 PNL=+3.71€
