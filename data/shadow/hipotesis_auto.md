# Hipótesis automáticas — 2026-07-15 21:26 UTC
_Generado por shadow_postmortem.py sobre 15948 resoluciones (PNL=+3214.52€)_
...
### GBM_LATE_15M
- **PATRÓN** `dist_vwap_pct` > `0.6483` → IC=+0.129 (n=122)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` > 0.6483 (IC base=+0.109)
...
- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.133 (n=1198)
...
- **PATRÓN** `sigma_h` < `0.0115` → IC=+0.140 (n=301)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0115 (IC base=+0.127)

- **PATRÓN** `sigma_h` > `0.0093` → IC=+0.132 (n=452)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0093 (IC base=+0.127)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.176 (n=331)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` < 12.0 (IC base=+0.127)

### GBM_LATE_15M#XRP#15min
- **PATRÓN** `sigma_h` > `0.0231` → IC=+0.190 (n=195)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0231 (IC base=+0.149)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0148` → IC=+0.182 (n=212)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` > 0.0148 (IC base=+0.111)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.121 (n=637)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` < 17.0 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` > `0.7311` → IC=+0.181 (n=111)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.7311 (IC base=+0.111)
...
- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.223 (n=182)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `1.2749` → IC=+0.204 (n=106)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 1.2749 (IC base=+0.205)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.51` → IC=+0.211 (n=43)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.51 (IC base=+0.205)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.684` → IC=+0.213 (n=92)
...
| ✅ GBM_LATE_15M | 4691 | +0.102 | +1446.60€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 4691 | +0.102 | +1446.60€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#SOL | 1250 | +0.109 | +498.15€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1250 | +0.109 | +498.15€ | 0 | 6 |
| ✅ GBM_LATE_15M#XRP | 1180 | +0.157 | +596.00€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1180 | +0.157 | +596.00€ | 0 | 10 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO | 48 | +0.080 | +12.51€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 48 | +0.080 | +12.51€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 23 | +0.340 | +15.45€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 23 | +0.340 | +15.45€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 1897 | +0.088 | +559.59€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1897 | +0.088 | +559.59€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#SOL | 455 | +0.051 | +80.28€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 455 | +0.051 | +80.28€ | 2 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 539 | +0.199 | +381.00€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 539 | +0.199 | +381.00€ | 0 | 12 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 56 | +0.224 | +17.83€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 56 | +0.224 | +17.83€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 22 | +0.125 | +6.60€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 22 | +0.125 | +6.60€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.135) — sin ventaja clara. oversold(IBS<0.3): IC=+0.002 n=452 | neutral: IC=-0.017 n=348 | overbought(IBS>0.7): IC=+0.118 n=370
  - _Datos_: n=1237 IC=+0.037 PNL=+114.62€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.101 n=9248 | tras_1loss IC=+0.065 n=6489 | tras_2loss IC=+0.032 n=2787/40 | gap=+0.069 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=13490 IC=+0.104 PNL=+3290.41€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=13490 IC=+0.104 PNL=+3290.41€
...
**🔶 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.146 n=179) pero sin cruzar ≥2 pares más — BTC: n=131 IC=-0.004; ETH: n=100 IC=+0.000; SOL: n=185 IC=+0.056 ✓
