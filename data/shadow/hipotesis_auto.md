# Hipótesis automáticas — 2026-07-01 00:36 UTC
_Generado por shadow_postmortem.py sobre 1490 resoluciones (PNL=-60.50€)_

## Patrones causales activos

### UPDOWN_GBM#BTC#15min
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0978` → IC=+0.140 (n=23)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.70€ cuando `pct_spot_vs_ref` |x|≤ 0.0978 (IC base=+0.038)

- **PATRÓN** `drift_60min` |x|≤ `0.0607` → IC=+0.136 (n=20)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.0607 (IC base=+0.038)
...
- **PATRÓN** `sigma_h` > `0.0129` → IC=+0.135 (n=50)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` > 0.0129 (IC base=+0.027)

- **PATRÓN** `ibs_15` < `0.1558` → IC=+0.152 (n=21)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.76€ cuando `ibs_15` < 0.1558 (IC base=+0.027)

### UPDOWN_GBM#ETH#60min
- **FILTRO** `sigma_h` < `0.0094` → IC=-0.125 (n=22)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0094
  - _Potencial_: sin este filtro IC_bueno=+0.140 (n=23)

- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0853` → IC=+0.147 (n=15)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.74€ cuando `pct_spot_vs_ref` |x|≤ 0.0853 (IC base=+0.011)

- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.140 (n=23)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` > 0.0094 (IC base=+0.011)

### UPDOWN_GBM#SOL#15min
- **FILTRO** `drift_60min` |x|> `0.2311` → IC=-0.184 (n=17)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.2311
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.1558 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.152 n=21). Confirma señal de reversión media → alinear con BUY_YES.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ ORDER_FLOW_5M | 791 | +0.009 | +5.47€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#5min | 655 | -0.001 | -7.12€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#BNB | 63 | +0.038 | +1.36€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#BNB#5min | 63 | +0.038 | +1.36€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#BTC | 133 | +0.004 | -0.52€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#BTC#5min | 133 | +0.004 | -0.52€ | 0 | 0 |
...
| ✅ PRICE_TARGET_GBM | 59 | -0.074 | -10.88€ | 0 | 0 |
| ✅ PRICE_TARGET_GBM#BTC | 30 | -0.125 | -6.71€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 494 | -0.022 | -13.97€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 305 | +0.018 | +3.26€ | 0 | 0 |
| 🚫 UPDOWN_GBM#240min | 12 | -0.171 | -4.82€ | 0 | 0 |
| 🚫 UPDOWN_GBM#5min | 56 | -0.155 | -16.64€ | 0 | 0 |
| ✅ UPDOWN_GBM#60min | 105 | -0.042 | -6.17€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 156 | -0.013 | -10.50€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 91 | +0.038 | -3.04€ | 0 | 2 |
...
| ✅ UPDOWN_GBM#ETH | 213 | +0.012 | +5.19€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 146 | +0.027 | +5.56€ | 1 | 2 |
| ✅ UPDOWN_GBM#ETH#240min | 5 | -0.018 | -0.44€ | 0 | 0 |
| 🚫 UPDOWN_GBM#ETH#5min | 12 | -0.086 | -3.67€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 45 | +0.011 | -0.11€ | 1 | 2 |
| ✅ UPDOWN_GBM#ETH#daily | 5 | +0.018 | +3.85€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 87 | -0.084 | -4.65€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 43 | -0.011 | +1.08€ | 1 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.066) — sin ventaja clara. oversold(IBS<0.3): IC=-0.035 n=56 | neutral: IC=-0.091 n=64 | overbought(IBS>0.7): IC=-0.024 n=80
  - _Datos_: n=205 IC=-0.051 PNL=-15.10€
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=43 IC=-0.011 PNL=+1.08€
...
**⏳ H-OF-PAR** — ORDER_FLOW per-pair delta_ratio ranges
  ...
  - _Estado_: BTC: 77 ops con delta_ratio | SOL: 88 ops con delta_ratio
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=45/40 IC=+0.011 PNL=-0.11€ | BTC#60min: n=39/40 IC=-0.037 PNL=-3.81€ | SOL#60min: n=21/40 IC=-0.152 PNL=-2.25€
...
**〰️ H-CUSTOM-OF-MADRUGADA** — ORDER_FLOW de madrugada (0h-6h UTC) BTC+SOL — ¿neutralizar?
  ...
  - _Estado_: n=39 IC=+0.012 PNL=+0.41€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=39 IC=+0.012 PNL=+0.41€

**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  - _Hipótesis_: Cuando la volatilidad horaria es muy alta el GBM puede sobreestimar el edge. Testear.
  - _Umbral_: n≥30 y IC<-0.05
  - _Acción_: Filtrar señales GBM cuando sigma_h > 0.002 si se confirma IC negativo
  - _Estado_: n=395 IC=-0.016 PNL=-8.33€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=395 IC=-0.016 PNL=-8.33€
...
**⏳ H-CUSTOM-GBM-60MIN-BUYNO** — GBM 60min BUY_NO — tracking por separado
  ...
  - _Estado_: 17/30 ops en el filtro definido (IC actual=+0.022 PNL=-0.06€)
  - _Datos_: n=17 IC=+0.022 PNL=-0.06€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=238 IC=-0.013 PNL=-16.99€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=238 IC=-0.013 PNL=-16.99€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=91 IC=+0.038 PNL=-3.04€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=91 IC=+0.038 PNL=-3.04€

**⏳ H-CUSTOM-DRIFT15-ZONA-MUERTA** — GBM#15min drift_15min ∈ [-0.3,+0.3] — zona muerta de señal
  - _Hipótesis_: Análisis n=127 GBM#15min: cuando drift_15min está entre -0.3 y +0.3 (mercado sin dirección clara) el IC es negativo (-0.043). Cuando drift>0.3 IC=+0.100 (n=28). Cuando drift<-1 IC=+0.048 (reversión). La señal requiere mercado con dirección clara.
  - _Umbral_: 50
  - _Acción_: Filtrar señales GBM#15min cuando drift_15min ∈ [-0.3, +0.3] — validar con n≥50 antes de implementar
  - _Estado_: 48/50 ops en el filtro definido (IC actual=-0.060 PNL=-2.97€)
  - _Datos_: n=48 IC=-0.060 PNL=-2.97€

**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  - _Hipótesis_: Cuando drift_15min > 0.3%/h el GBM captura bien la dirección: IC=+0.100 n=28 en todos GBM#15min; IC=+0.152 n=13 solo BTC. El mercado tiene dirección clara y el GBM la sigue. Hipótesis: este rango es donde la señal es real.
  - _Umbral_: n≥40 y IC>+0.08
  - _Acción_: Si se confirma IC>0.10 con n≥40 → boost ×1.2 en GBM#15min cuando drift_15min>0.3
  - _Estado_: n=135 IC=+0.040 PNL=-3.29€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=135 IC=+0.040 PNL=-3.29€

**⏳ H-CUSTOM-LONGSHOT-BIAS** — Longshot bias — ¿mejor IC cuando py_mkt < 0.20 o > 0.80?
  - _Hipótesis_: Jon-Becker repo documenta formalmente: contratos a 1-20 cents tienen win_rate < precio implícito (compradores pierden sistemáticamente en longshots). En nuestro sistema: cuando py_mkt<0.20 el GBM predice BUY_NO con edge estructural adicional al del modelo. ¿Se confirma en nuestros datos? Buscar en feature pct_spot_vs_ref si los mercados extremos tienen mejor IC en BUY_NO.
  - _Umbral_: 30
  - _Acción_: Si IC>0.10 con n≥30 en mercados extremos → boost ×1.2 en BUY_NO cuando py_mkt<0.20
  - _Estado_: 11/30 ops en el filtro definido (IC actual=-0.021 PNL=+1.70€)
  - _Datos_: n=11 IC=-0.021 PNL=+1.70€

**🟡 H-CUSTOM-ETH15-REVERSION** — ETH#15min con drift_15min < -1 — ¿mean reversion?
  - _Hipótesis_: ETH y BTC tienen patrones opuestos: BTC funciona con momentum (drift>0.3). ETH funciona con reversión (drift<-1): 9/14 (64%) IC=+0.087. La hipótesis es que ETH tiene más mean-reversion que BTC en 15min.
  - _Umbral_: n≥20 y IC>+0.08
  - _Acción_: Si ETH drift<-1 confirma IC>0.08 con n≥20 → boost ×1.1 en ETH#15min cuando drift_15min<-1
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.152 > 0.08 con n=21 PNL=+4.88€
  - _Datos_: n=21 IC=+0.152 PNL=+4.88€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=54 IC=+0.054 PNL=+9.12€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=54 IC=+0.054 PNL=+9.12€
