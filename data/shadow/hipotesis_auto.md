# Hipótesis automáticas — 2026-08-29 02:10 UTC
_Generado por shadow_postmortem.py sobre 195840 resoluciones (PNL=+14806.26€)_
...
- **FILTRO** `restante_s_al_confirmar` < `116.46` → IC=-0.387 (n=353)
...
- **FILTRO** `restante_s_al_confirmar` < `141.46` → IC=-0.310 (n=635)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=1906)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.191 (n=5912)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.189 (n=191)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `4142.7901` → IC=+0.148 (n=296)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 4142.7901 (IC base=+0.131)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.127 (n=1113)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.281 (n=514)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.277)

- **PATRÓN** `py_entrada` < `0.195` → IC=+0.409 (n=173)
...
- **PATRÓN** `libro_liquidez` > `4383.8031` → IC=+0.290 (n=165)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4383.8031 (IC base=+0.277)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.224 (n=320)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `1959.3595` → IC=+0.236 (n=305)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1959.3595 (IC base=+0.221)
...
- **FILTRO** `libro_liquidez` < `8656.6892` → IC=-0.262 (n=128)

  - _Acción_: SKIP cuando `libro_liquidez` < 8656.6892
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.191 (n=4147)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **FILTRO** `py_entrada` > `0.805` → IC=-0.417 (n=22)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.208 (n=70)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.178 (n=454)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` < 6.0 (IC base=+0.167)

- **PATRÓN** `py_entrada` < `0.7` → IC=+0.172 (n=453)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.182 (n=1023)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` < 15.0 (IC base=+0.182)

- **PATRÓN** `py_entrada` < `0.72` → IC=+0.192 (n=1014)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.219 (n=9985)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.133 (n=1779)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.280 (n=1227)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.151 (n=2463)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 8.0 (IC base=+0.133)

- **PATRÓN** `lag_apertura_s` < `4.22` → IC=+0.159 (n=1680)

  - _Acción_: Kelly boost +0.79€ cuando `lag_apertura_s` < 4.22 (IC base=+0.133)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.209 (n=919)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.38 (IC base=+0.139)

- **PATRÓN** `restante_min` < `3.94` → IC=+0.140 (n=842)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.165 (n=1220)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.139 (n=1243)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9498` → IC=+0.208 (n=848)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.262` → IC=+0.157 (n=1010)
...
- **PATRÓN** `ibs_20min` < `0.1163` → IC=+0.151 (n=1296)

  - _Acción_: Kelly boost +0.76€ cuando `ibs_20min` < 0.1163 (IC base=+0.031)
...
- **PATRÓN** `sigma_h` < `0.006` → IC=+0.261 (n=224)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0752` → IC=+0.316 (n=112)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0752 (IC base=+0.255)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.270 (n=233)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 11.0 (IC base=+0.255)

- **PATRÓN** `ibs_20min` < `0.4216` → IC=+0.291 (n=295)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4216 (IC base=+0.255)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.819` → IC=+0.278 (n=362)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.819 (IC base=+0.255)

- **PATRÓN** `volumen_pendiente_norm` < `0.0668` → IC=+0.246 (n=207)
...
- **PATRÓN** `drift_60min` |x|≤ `0.461` → IC=+0.258 (n=394)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.461 (IC base=+0.247)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.249 (n=273)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.247)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.262 (n=406)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.247)

- **PATRÓN** `ibs_20min` < `0.5208` → IC=+0.288 (n=394)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.873` → IC=+0.250 (n=458)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3846` → IC=+0.429 (n=40)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3846 (IC base=+0.247)

- **PATRÓN** `volumen_spike_ratio` > `2.5473` → IC=+0.243 (n=181)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5473 (IC base=+0.247)
...
- **FILTRO** `ibs_20min` > `0.7692` → IC=-0.172 (n=349)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=1050)

- **FILTRO** `sigma_ewma_delta_pct` > `5.027` → IC=-0.155 (n=297)
...
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.136 (n=1451)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0078 (IC base=+0.054)

- **PATRÓN** `ibs_20min` > `0.9441` → IC=+0.247 (n=1065)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1149` → IC=+0.181 (n=1438)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` < 0.1149 (IC base=+0.054)

- **PATRÓN** `volumen_pendiente_norm` > `0.2487` → IC=+0.200 (n=278)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2487 (IC base=+0.054)

- **PATRÓN** `volumen_spike_ratio` < `1.4786` → IC=+0.206 (n=492)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4786 (IC base=+0.054)

- **PATRÓN** `volumen_spike_ratio` > `2.8297` → IC=+0.180 (n=492)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` > 2.8297 (IC base=+0.054)

- **PATRÓN** `ballena_activa_n` < `96.0` → IC=+0.278 (n=923)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 96.0 (IC base=+0.054)
...
- **FILTRO** `ibs_20min` < `0.2448` → IC=-0.165 (n=225)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2448
  - _Potencial_: sin este filtro IC_bueno=+0.077 (n=459)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.349` → IC=+0.172 (n=129)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 5.349 (IC base=-0.003)
...
- **FILTRO** `ibs_20min` > `0.8844` → IC=-0.149 (n=294)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8844
  - _Potencial_: sin este filtro IC_bueno=-0.007 (n=884)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0788` → IC=+0.213 (n=169)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0788 (IC base=+0.195)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.264 (n=180)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.157` → IC=+0.301 (n=129)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.157 (IC base=+0.195)

- **PATRÓN** `volumen_pendiente_norm` < `0.1438` → IC=+0.205 (n=402)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1438 (IC base=+0.195)

- **PATRÓN** `volumen_spike_ratio` < `2.52` → IC=+0.204 (n=282)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.52 (IC base=+0.195)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.222 (n=469)
...
- **FILTRO** `ibs_20min` > `0.7367` → IC=-0.122 (n=350)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.171 (n=68)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=964)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.234 (n=287)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.227)

- **PATRÓN** `ibs_20min` > `0.8915` → IC=+0.308 (n=404)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.219` → IC=+0.278 (n=336)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1857` → IC=+0.240 (n=482)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1857 (IC base=+0.227)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3375` → IC=+0.161 (n=2402)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.3375 (IC base=+0.158)
...
- **PATRÓN** `dist_vwap_pct` > `1.1954` → IC=+0.256 (n=466)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1954 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.426` → IC=+0.244 (n=1133)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.426 (IC base=+0.158)

- **PATRÓN** `volumen_regimen` > `0.6215` → IC=+0.170 (n=1890)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 0.6215 (IC base=+0.158)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2896` → IC=+0.159 (n=2179)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.2896 (IC base=+0.158)

- **PATRÓN** `volumen_spike_ratio` > `1.8555` → IC=+0.153 (n=1651)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.8555 (IC base=+0.158)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.166 (n=2180)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.01 (IC base=+0.158)

- **PATRÓN** `libro_liquidez` > `3206.2208` → IC=+0.184 (n=1238)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 3206.2208 (IC base=+0.158)

- **PATRÓN** `ballena_activa_n` < `109.0` → IC=+0.180 (n=1442)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 109.0 (IC base=+0.158)

- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.199 (n=1668)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0061 (IC base=+0.184)

- **PATRÓN** `drift_60min` |x|≤ `0.0814` → IC=+0.206 (n=835)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.184 (n=936)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 5.0 (IC base=+0.184)

- **PATRÓN** `ibs_20min` < `0.4127` → IC=+0.239 (n=2500)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4127 (IC base=+0.184)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.502` → IC=+0.209 (n=1055)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.502 (IC base=+0.184)

- **PATRÓN** `volumen_regimen` < `1.1686` → IC=+0.168 (n=1971)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.1686 (IC base=+0.184)

- **PATRÓN** `volumen_regimen` > `0.618` → IC=+0.168 (n=1971)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 0.618 (IC base=+0.184)

- **PATRÓN** `volumen_pendiente_norm` > `0.2921` → IC=+0.254 (n=299)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2921 (IC base=+0.184)

- **PATRÓN** `volumen_spike_ratio` < `1.5829` → IC=+0.177 (n=830)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 1.5829 (IC base=+0.184)

- **PATRÓN** `volumen_spike_ratio` > `2.6614` → IC=+0.208 (n=629)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6614 (IC base=+0.184)

- **PATRÓN** `ballena_activa_n` < `118.0` → IC=+0.190 (n=1284)

  - _Acción_: Kelly boost +0.95€ cuando `ballena_activa_n` < 118.0 (IC base=+0.184)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.267 (n=243)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0048 (IC base=+0.252)

- **PATRÓN** `drift_60min` |x|≤ `0.2611` → IC=+0.287 (n=214)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2611 (IC base=+0.252)

- **PATRÓN** `hora_utc` < `14.0` → IC=+0.274 (n=219)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 14.0 (IC base=+0.252)

- **PATRÓN** `ibs_20min` < `0.4013` → IC=+0.281 (n=244)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4013 (IC base=+0.252)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.358` → IC=+0.269 (n=271)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.358 (IC base=+0.252)

- **PATRÓN** `volumen_pendiente_norm` > `0.311` → IC=+0.274 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.311 (IC base=+0.252)

- **PATRÓN** `volumen_spike_ratio` < `1.9491` → IC=+0.277 (n=119)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9491 (IC base=+0.252)

- **PATRÓN** `volumen_spike_ratio` > `2.8103` → IC=+0.274 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8103 (IC base=+0.252)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.304 (n=105)
...
- **PATRÓN** `ballena_activa_n` < `82.0` → IC=+0.237 (n=135)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 82.0 (IC base=+0.252)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.235 (n=153)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.958` → IC=+0.296 (n=101)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.958 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` < `0.2298` → IC=+0.161 (n=337)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` < 0.2298 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.9664` → IC=+0.207 (n=148)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9664 (IC base=+0.167)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.194 (n=354)

  - _Acción_: Kelly boost +0.97€ cuando `libro_spread` < 0.04 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `1945.061` → IC=+0.179 (n=132)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 1945.061 (IC base=+0.167)
...
- **PATRÓN** `volumen_spike_ratio` > `1.552` → IC=+0.128 (n=450)
...
- **PATRÓN** `drift_60min` |x|≤ `0.6757` → IC=+0.229 (n=558)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.6757 (IC base=+0.219)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.225 (n=274)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.219)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.253 (n=261)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.219)

- **PATRÓN** `ibs_20min` < `0.3824` → IC=+0.259 (n=555)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3824 (IC base=+0.219)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.223` → IC=+0.283 (n=113)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.223 (IC base=+0.219)

- **PATRÓN** `volumen_regimen` > `0.6911` → IC=+0.241 (n=496)
...
- **PATRÓN** `ibs_20min` < `0.3044` → IC=+0.132 (n=343)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.3044 (IC base=+0.053)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.789` → IC=+0.138 (n=114)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` > 7.789 (IC base=+0.053)

- **PATRÓN** `ballena_activa_n` < `13.0` → IC=+0.148 (n=123)

  - _Acción_: Kelly boost +0.74€ cuando `ballena_activa_n` < 13.0 (IC base=+0.053)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.185 (n=2128)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.9412` → IC=+0.290 (n=1446)
...
- **PATRÓN** `volumen_regimen` > `1.0776` → IC=+0.153 (n=1020)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.0776 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` > `0.1652` → IC=+0.177 (n=827)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.1652 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` < `2.3115` → IC=+0.153 (n=2527)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.3115 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` > `1.8684` → IC=+0.160 (n=1913)
...
- **PATRÓN** `libro_liquidez` > `2477.03` → IC=+0.177 (n=2126)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 2477.03 (IC base=+0.151)

- **PATRÓN** `ballena_activa_n` < `140.0` → IC=+0.191 (n=1594)

  - _Acción_: Kelly boost +0.95€ cuando `ballena_activa_n` < 140.0 (IC base=+0.151)

- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.215 (n=1967)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0067 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.4823` → IC=+0.199 (n=2948)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.4823 (IC base=+0.189)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.210 (n=1052)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.189)

- **PATRÓN** `ibs_20min` < `0.5624` → IC=+0.244 (n=2948)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.418` → IC=+0.207 (n=428)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.418 (IC base=+0.189)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.655` → IC=+0.193 (n=2768)
...
- **PATRÓN** `volumen_spike_ratio` > `2.304` → IC=+0.208 (n=1019)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.304 (IC base=+0.189)

- **PATRÓN** `ballena_activa_n` < `171.0` → IC=+0.169 (n=1820)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3349` → IC=+0.274 (n=343)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3349 (IC base=+0.265)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.265 (n=308)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.265)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.281 (n=240)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 11.0 (IC base=+0.265)

- **PATRÓN** `ibs_20min` < `0.56` → IC=+0.306 (n=343)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.56 (IC base=+0.265)
...
- **PATRÓN** `libro_liquidez` > `1955.5` → IC=+0.312 (n=115)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1955.5 (IC base=+0.265)

- **PATRÓN** `ballena_activa_n` < `74.0` → IC=+0.244 (n=162)
...
- **PATRÓN** `drift_60min` |x|≤ `0.208` → IC=+0.236 (n=286)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.208 (IC base=+0.236)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.280 (n=166)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.236)

- **PATRÓN** `ibs_20min` < `0.5536` → IC=+0.300 (n=429)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5536 (IC base=+0.236)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7651` → IC=+0.233 (n=103)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7651 (IC base=+0.236)

- **PATRÓN** `volumen_spike_ratio` > `1.9441` → IC=+0.203 (n=274)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9441 (IC base=+0.236)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.267 (n=165)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.236)

- **PATRÓN** `libro_liquidez` > `1857.661` → IC=+0.236 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1857.661 (IC base=+0.236)

- **PATRÓN** `ballena_activa_n` < `58.0` → IC=+0.183 (n=197)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 58.0 (IC base=+0.236)
...
- **PATRÓN** `ibs_20min` > `0.9609` → IC=+0.298 (n=300)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1708` → IC=+0.244 (n=154)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1708 (IC base=+0.179)

- **PATRÓN** `volumen_spike_ratio` > `1.8195` → IC=+0.193 (n=408)
...
- **PATRÓN** `libro_liquidez` > `2449.1286` → IC=+0.182 (n=661)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 2449.1286 (IC base=+0.179)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.145 (n=1267)
...
- **PATRÓN** `sigma_h` < `0.0088` → IC=+0.149 (n=277)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0088 (IC base=+0.136)

- **PATRÓN** `sigma_h` > `0.0062` → IC=+0.136 (n=185)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0062 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.5119` → IC=+0.181 (n=277)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.91€ cuando `drift_60min` |x|≤ 0.5119 (IC base=+0.136)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.165 (n=186)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 10.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` < `0.619` → IC=+0.138 (n=244)

  - _Acción_: Kelly boost +0.69€ cuando `ibs_20min` < 0.619 (IC base=+0.136)

- **PATRÓN** `ibs_20min` > `0.1184` → IC=+0.160 (n=277)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.1184 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.2026` → IC=+0.153 (n=93)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.2026 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.368` → IC=+0.186 (n=49)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 11.368 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `1.1886` → IC=+0.156 (n=277)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.1886 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` < `0.1338` → IC=+0.161 (n=281)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` < 0.1338 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` < `2.183` → IC=+0.178 (n=240)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 2.183 (IC base=+0.136)
...
- **PATRÓN** `libro_liquidez` > `8854.787` → IC=+0.136 (n=248)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 8854.787 (IC base=+0.136)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.171 (n=959)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.025 (n=2897)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.207 (n=975)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.008 (n=2963)

- **FILTRO** `ibs_20min` > `0.27` → IC=-0.172 (n=983)

  - _Acción_: SKIP cuando `ibs_20min` > 0.27
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=2955)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.126 (n=177)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
...
- **FILTRO** `ibs_20min` < `0.7248` → IC=-0.178 (n=147)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.141 (n=2792)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=6955)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.282 (n=2392)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=7355)

- **FILTRO** `ibs_7min` < `0.7288` → IC=-0.236 (n=2436)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7288
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=7311)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.172 (n=3279)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=6468)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.221 (n=2974)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=8932)

- **FILTRO** `ibs_7min` > `0.7179` → IC=-0.174 (n=2976)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7179
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=8930)
...
- **FILTRO** `ibs_7min` < `1.0` → IC=-0.189 (n=933)
...
- **FILTRO** `py_entrada` > `0.71` → IC=-0.227 (n=492)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=1559)

- **FILTRO** `drift_7min_pct` |x|> `0.1316` → IC=-0.138 (n=697)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1316
...
- **FILTRO** `ibs_7min` > `0.8511` → IC=-0.184 (n=511)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=1540)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.139 (n=405)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
...
- **FILTRO** `ibs_7min` < `0.7913` → IC=-0.188 (n=469)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7913
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=1410)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.199 (n=340)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
...
- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.182 (n=467)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=1415)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.124 (n=522)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
...
- **FILTRO** `py_entrada` < `0.37` → IC=-0.300 (n=388)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=1215)

- **FILTRO** `ibs_7min` < `0.75` → IC=-0.234 (n=397)
...
- **FILTRO** `ballena_activa_n` > `14.0` → IC=-0.233 (n=391)
  ...
  - _Acción_: SKIP cuando `ballena_activa_n` > 14.0
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=1212)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.271 (n=413)
...
- **FILTRO** `ibs_7min` > `0.8` → IC=-0.172 (n=508)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=1531)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.128 (n=661)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=1378)
...
- **PATRÓN** `streak_estiramiento` < `0.3153` → IC=+0.167 (n=31)

  - _Acción_: Kelly boost +0.83€ cuando `streak_estiramiento` < 0.3153 (IC base=+0.015)
...
- **FILTRO** `sigma_h` > `0.01` → IC=-0.175 (n=269)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.701` → IC=-0.175 (n=364)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=2780)
...
- **FILTRO** `sigma_h` > `0.0075` → IC=-0.248 (n=169)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.200 (n=511)

- **FILTRO** `sigma_h` < `0.0036` → IC=-0.217 (n=224)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0036
  - _Potencial_: sin este filtro IC_bueno=-0.210 (n=456)
...
- **FILTRO** `sigma_ewma_delta_pct` > `19.716` → IC=-0.267 (n=127)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.199 (n=553)
...
- **FILTRO** `drift_15min` |x|> `1.1577` → IC=-0.261 (n=69)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.123 (n=210)
...
- **FILTRO** `libro_liquidez` < `2495.6392` → IC=-0.218 (n=69)
...
| ✅ BALLENAS_TARDIAS | 10950 | -0.095 | -1814.74€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 769 | -0.052 | -131.69€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 10181 | -0.098 | -1683.05€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1387 | -0.040 | -278.52€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1387 | -0.040 | -278.52€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 769 | -0.052 | -131.69€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 769 | -0.052 | -131.69€ | 3 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 1412 | -0.143 | -427.08€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 1412 | -0.143 | -427.08€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 2948 | -0.063 | -293.74€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 2948 | -0.063 | -293.74€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 2541 | -0.097 | -227.11€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2541 | -0.097 | -227.11€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1893 | -0.163 | -456.60€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1893 | -0.163 | -456.60€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 813 | -0.061 | +423.92€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 234 | +0.000 | +97.21€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 579 | -0.085 | +326.71€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 813 | -0.061 | +423.92€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 234 | +0.000 | +97.21€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 579 | -0.085 | +326.71€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 35709 | +0.115 | -2211.32€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 6439 | +0.186 | -227.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 114 | -0.086 | -47.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 25841 | +0.098 | -1878.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3315 | +0.119 | -58.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 4334 | +0.069 | -688.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 22 | -0.083 | +0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 4307 | +0.070 | -682.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 7310 | +0.134 | -146.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1824 | +0.198 | -90.94€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 4300 | +0.111 | -85.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1144 | +0.127 | +51.82€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 4346 | +0.083 | -514.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 23 | +0.060 | +2.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 4322 | +0.084 | -515.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 7843 | +0.127 | -122.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2337 | +0.168 | -13.21€ | 0 | 8 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 4303 | +0.113 | -70.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1191 | +0.099 | -30.06€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 7543 | +0.133 | -467.91€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2216 | +0.200 | -125.06€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 52 | +0.018 | -8.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 4295 | +0.100 | -254.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 980 | +0.131 | -79.87€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#XRP | 4333 | +0.108 | -271.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 17 | -0.022 | -0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 4314 | +0.109 | -270.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 6341 | +0.175 | -485.40€ | 3 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 6341 | +0.175 | -485.40€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC | 126 | -0.117 | +1.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC#15min | 126 | -0.117 | +1.93€ | 3 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 1577 | +0.167 | -169.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 1577 | +0.167 | -169.89€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 1424 | +0.231 | -39.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 1424 | +0.231 | -39.87€ | 0 | 2 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 1536 | +0.182 | -122.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 1536 | +0.182 | -122.03€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 18384 | +0.191 | -1609.31€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 18384 | +0.191 | -1609.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 3334 | +0.132 | -588.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 3334 | +0.132 | -588.53€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 2876 | +0.234 | -64.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 2876 | +0.234 | -64.21€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 3150 | +0.168 | -383.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 3150 | +0.168 | -383.07€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 2924 | +0.227 | -97.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 2924 | +0.227 | -97.78€ | 0 | 2 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 3098 | +0.184 | -309.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 3098 | +0.184 | -309.30€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 6718 | +0.133 | +251.79€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 6718 | +0.133 | +251.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 3344 | +0.139 | +158.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 3344 | +0.139 | +158.12€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 3374 | +0.128 | +93.67€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 3374 | +0.128 | +93.67€ | 0 | 5 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 8097 | +0.096 | -273.56€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 787 | +0.072 | -18.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 