# Hipótesis automáticas — 2026-07-15 07:27 UTC
_Generado por shadow_postmortem.py sobre 14978 resoluciones (PNL=+2905.52€)_
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.195 (n=841)

  - _Acción_: Kelly boost +0.98€ cuando `libro_spread` < 0.01 (IC base=+0.192)

- **PATRÓN** `libro_liquidez` > `4381.7761` → IC=+0.195 (n=476)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.199 (n=314)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.172 (n=942)

  - _Acción_: Kelly boost +0.86€ cuando `libro_spread` < 0.01 (IC base=+0.162)

- **PATRÓN** `libro_liquidez` > `7344.6882` → IC=+0.177 (n=274)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 7344.6882 (IC base=+0.162)
...
- **PATRÓN** `libro_liquidez` > `8228.2866` → IC=+0.200 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8228.2866 (IC base=+0.193)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.215 (n=177)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `3727.5827` → IC=+0.210 (n=174)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.201 (n=175)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.194)

- **PATRÓN** `py_entrada` < `0.365` → IC=+0.245 (n=155)
...
- **PATRÓN** `dist_vwap_pct` > `0.2158` → IC=+0.155 (n=27)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.2158 (IC base=+0.059)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.004` → IC=+0.181 (n=45)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 3.004 (IC base=+0.059)
...
- **PATRÓN** `dist_vwap_pct` > `0.7422` → IC=+0.167 (n=31)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.7422 (IC base=+0.064)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.073` → IC=+0.125 (n=46)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 5.073 (IC base=+0.064)

- **PATRÓN** `sigma_ewma_delta_pct` < `12.131` → IC=+0.123 (n=104)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` < 12.131 (IC base=+0.064)

### GBM_LATE_15M#SOL#15min
- **FILTRO** `sigma_ewma_delta_pct` > `6.429` → IC=-0.125 (n=22)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 6.429
...
- **PATRÓN** `sigma_h` < `0.0135` → IC=+0.142 (n=383)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0135 (IC base=+0.132)

- **PATRÓN** `sigma_h` > `0.0093` → IC=+0.136 (n=435)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0093 (IC base=+0.132)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.170 (n=292)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` > `0.0261` → IC=+0.207 (n=145)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0261 (IC base=+0.164)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.242 (n=161)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` < `0.5914` → IC=+0.139 (n=142)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.5914 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.657` → IC=+0.139 (n=106)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` < 3.657 (IC base=+0.164)
...
- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.180 (n=660)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `1.0538` → IC=+0.186 (n=49)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 1.0538 (IC base=+0.160)

- **PATRÓN** `dist_vwap_pct` < `0.1912` → IC=+0.201 (n=386)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.165` → IC=+0.178 (n=321)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` < 3.165 (IC base=+0.160)
...
- **PATRÓN** `sigma_h` > `0.0027` → IC=+0.176 (n=168)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.1487` → IC=+0.131 (n=109)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.1487 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.18` → IC=+0.278 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.18 (IC base=+0.137)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.133 (n=148)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.0066 (IC base=+0.127)

- **PATRÓN** `sigma_h` > `0.0041` → IC=+0.151 (n=150)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0041 (IC base=+0.127)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.167 (n=85)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 15.0 (IC base=+0.127)
...
- **PATRÓN** `dist_vwap_pct` > `0.759` → IC=+0.188 (n=30)
  ...
  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.759 (IC base=+0.127)

- **PATRÓN** `dist_vwap_pct` < `1.1817` → IC=+0.153 (n=122)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 1.1817 (IC base=+0.127)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.568` → IC=+0.167 (n=37)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 5.568 (IC base=+0.127)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.121` → IC=+0.160 (n=92)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` < 10.121 (IC base=+0.127)
...
- **PATRÓN** `dist_vwap_pct` < `0.1754` → IC=+0.122 (n=321)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.1754 (IC base=+0.083)
...
- **PATRÓN** `dist_vwap_pct` < `0.5211` → IC=+0.185 (n=141)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` < 0.5211 (IC base=+0.195)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.549` → IC=+0.197 (n=107)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` < 2.549 (IC base=+0.195)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.235 (n=32)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.088)

- **PATRÓN** `py_entrada` < `0.485` → IC=+0.167 (n=46)

  - _Acción_: Kelly boost +0.83€ cuando `py_entrada` < 0.485 (IC base=+0.088)

- **PATRÓN** `regimen_ma_toques` > `4.0` → IC=+0.182 (n=20)

  - _Acción_: Kelly boost +0.91€ cuando `regimen_ma_toques` > 4.0 (IC base=+0.088)

- **PATRÓN** `volumen_racha` < `182999.9` → IC=+0.214 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_racha` < 182999.9 (IC base=+0.088)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2047 | +0.176 | -36.69€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1322 | +0.206 | -2.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 85 | -0.006 | -17.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 292 | +0.102 | -38.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 348 | +0.166 | +22.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 596 | +0.177 | -16.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 442 | +0.194 | -25.98€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 28 | +0.067 | -2.59€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 116 | +0.178 | +15.87€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH | 726 | +0.179 | +12.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 436 | +0.219 | +21.42€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 28 | -0.133 | -11.56€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 146 | +0.115 | -13.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 116 | +0.178 | +15.86€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL | 719 | +0.174 | -28.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 438 | +0.209 | +6.63€ | 0 | 6 |
...
| ✅ GBM_LATE_15M | 4503 | +0.102 | +1373.50€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 4503 | +0.102 | +1373.50€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1078 | +0.067 | +158.71€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1078 | +0.067 | +158.71€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1102 | +0.073 | +181.97€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1102 | +0.073 | +181.97€ | 1 | 6 |
| ✅ GBM_LATE_15M#SOL | 1198 | +0.112 | +492.47€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1198 | +0.112 | +492.47€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1125 | +0.151 | +540.36€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1125 | +0.151 | +540.36€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1770 | +0.151 | +1023.98€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1770 | +0.151 | +1023.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 452 | +0.123 | +226.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 452 | +0.123 | +226.84€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 468 | +0.121 | +206.32€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 468 | +0.121 | +206.32€ | 0 | 13 |
...
| ✅ GBM_LATE_15M_TARDIO | 1719 | +0.083 | +470.57€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1719 | +0.083 | +470.57€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 424 | +0.063 | +72.48€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 424 | +0.063 | +72.48€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 381 | +0.009 | +8.61€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 381 | +0.009 | +8.61€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 430 | +0.046 | +68.07€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 430 | +0.046 | +68.07€ | 2 | 5 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 484 | +0.189 | +321.41€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 484 | +0.189 | +321.41€ | 0 | 10 |
...
| ✅ GBM_LATE_60M_PYCONFIRMADO | 7 | -0.097 | -1.75€ | 0 | 0 |
| ✅ GBM_LATE_60M_PYCONFIRMADO#60min | 7 | -0.097 | -1.75€ | 0 | 0 |
...
| ✅ STREAK_FADE_15M | 191 | +0.096 | +20.86€ | 0 | 9 |
| ✅ STREAK_FADE_15M#15min | 191 | +0.096 | +20.86€ | 0 | 0 |
...
| ✅ STREAK_FADE_15M#XRP | 83 | +0.041 | -1.91€ | 0 | 0 |
| ✅ STREAK_FADE_15M#XRP#15min | 83 | +0.041 | -1.91€ | 1 | 0 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.098 n=8625 | tras_1loss IC=+0.060 n=6167 | tras_2loss IC=+0.027 n=2682/40 | gap=+0.070 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=12520 IC=+0.100 PNL=+2981.40€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=12520 IC=+0.100 PNL=+2981.40€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=575 IC=+0.058 PNL=+66.60€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=575 IC=+0.058 PNL=+66.60€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.125 > 0.08 con n=118 PNL=+35.53€
  - _Datos_: n=118 IC=+0.125 PNL=+35.53€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.120 > 0.08 con n=106 PNL=+46.35€
  - _Datos_: n=106 IC=+0.120 PNL=+46.35€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.119 > 0.08 con n=145 PNL=+65.78€
  - _Datos_: n=145 IC=+0.119 PNL=+65.78€
