# Hipótesis automáticas — 2026-08-08 08:32 UTC
_Generado por shadow_postmortem.py sobre 91326 resoluciones (PNL=+13785.73€)_
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.03` → IC=+0.245 (n=1298)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.03 (IC base=+0.146)

- **PATRÓN** `libro_liquidez` > `2041.0364` → IC=+0.152 (n=3059)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 2041.0364 (IC base=+0.146)

- **PATRÓN** `sigma_h` > `0.0039` → IC=+0.164 (n=3130)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.0039 (IC base=+0.148)

- **PATRÓN** `drift_60min` |x|≤ `0.0945` → IC=+0.205 (n=76)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0945 (IC base=+0.148)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.177` → IC=+0.172 (n=517)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 8.177 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.841` → IC=+0.148 (n=2588)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` < 2.841 (IC base=+0.148)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.309` → IC=+0.170 (n=289)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 4.309 (IC base=+0.108)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.489` → IC=+0.171 (n=168)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 9.489 (IC base=+0.074)
...
- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.246 (n=706)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0068 (IC base=+0.229)

- **PATRÓN** `drift_60min` |x|≤ `0.4576` → IC=+0.319 (n=70)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4576 (IC base=+0.229)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.257 (n=545)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 12.0 (IC base=+0.229)

- **PATRÓN** `dist_vwap_pct` > `0.8069` → IC=+0.353 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8069 (IC base=+0.229)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.29` → IC=+0.255 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.29 (IC base=+0.229)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.478` → IC=+0.225 (n=753)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.478 (IC base=+0.229)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.245 (n=708)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.229)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.331 (n=69)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0048 (IC base=+0.274)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.294 (n=71)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.274)

- **PATRÓN** `hora_utc` < `13.0` → IC=+0.288 (n=64)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 13.0 (IC base=+0.274)

- **PATRÓN** `dist_vwap_pct` < `0.0968` → IC=+0.222 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0968 (IC base=+0.274)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.018` → IC=+0.278 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.018 (IC base=+0.274)

- **PATRÓN** `sigma_ewma_delta_pct` < `12.905` → IC=+0.357 (n=68)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 12.905 (IC base=+0.274)

- **PATRÓN** `libro_liquidez` > `1225.8914` → IC=+0.278 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1225.8914 (IC base=+0.274)

- **PATRÓN** `sigma_h` < `0.0074` → IC=+0.199 (n=314)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0074 (IC base=+0.200)

- **PATRÓN** `sigma_h` > `0.0033` → IC=+0.210 (n=312)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0033 (IC base=+0.200)

- **PATRÓN** `drift_60min` |x|≤ `0.0962` → IC=+0.262 (n=103)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0962 (IC base=+0.200)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.291 (n=108)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.200)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.712` → IC=+0.225 (n=263)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.712 (IC base=+0.200)

- **PATRÓN** `libro_liquidez` > `1171.775` → IC=+0.210 (n=308)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1171.775 (IC base=+0.200)

### GBM_LATE_15M_MULTIHORIZONTE#BTC#15min
- **PATRÓN** `sigma_h` < `0.0031` → IC=+0.172 (n=59)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0031 (IC base=+0.150)

- **PATRÓN** `sigma_h` > `0.0021` → IC=+0.156 (n=59)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` > 0.0021 (IC base=+0.150)

- **PATRÓN** `drift_60min` |x|≤ `0.236` → IC=+0.167 (n=58)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.236 (IC base=+0.150)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.198 (n=41)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 12.0 (IC base=+0.150)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `7.518` → IC=+0.204 (n=52)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 7.518 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `3513.1214` → IC=+0.183 (n=58)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 3513.1214 (IC base=+0.150)
...
- **PATRÓN** `hora_utc` > `9.0` → IC=+0.194 (n=47)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` > 9.0 (IC base=+0.071)

- **PATRÓN** `libro_liquidez` > `1634.78` → IC=+0.292 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1634.78 (IC base=+0.071)

### GBM_LATE_15M_MULTIHORIZONTE#XRP#15min
- **PATRÓN** `sigma_h` < `0.0077` → IC=+0.287 (n=59)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0077 (IC base=+0.280)

- **PATRÓN** `sigma_h` > `0.0059` → IC=+0.297 (n=67)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0059 (IC base=+0.280)

- **PATRÓN** `drift_60min` |x|≤ `0.1306` → IC=+0.375 (n=22)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1306 (IC base=+0.280)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.558` → IC=+0.289 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.558 (IC base=+0.280)

- **PATRÓN** `libro_liquidez` > `2838.0713` → IC=+0.281 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2838.0713 (IC base=+0.280)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1081` → IC=+0.174 (n=44)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.1081 (IC base=+0.098)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.028` → IC=+0.214 (n=1247)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.028 (IC base=+0.098)

- **PATRÓN** `drift_60min` |x|≤ `0.093` → IC=+0.158 (n=112)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.093 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.599` → IC=+0.120 (n=2853)
...
- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.164 (n=865)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.0068 (IC base=+0.155)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.166 (n=801)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 7.0 (IC base=+0.155)

- **PATRÓN** `dist_vwap_pct` > `0.6667` → IC=+0.206 (n=141)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6667 (IC base=+0.155)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.161 (n=741)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1528` → IC=+0.225 (n=38)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.531` → IC=+0.176 (n=819)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` < 2.531 (IC base=+0.177)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.189 (n=833)
...
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_15MIN_EXTREMO#BTC#15min` — IC=+0.397 n=37. Faltan ~3 resoluciones para umbral n≥40. ETA: ~2h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_15MIN_EXTREMO#BTC` — IC=+0.397 n=37. Faltan ~3 resoluciones para umbral n≥40. ETA: ~2h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_15MIN_EXTREMO#ETH#15min` — IC=+0.397 n=37. Faltan ~3 resoluciones para umbral n≥40. ETA: ~2h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_15MIN_EXTREMO#ETH` — IC=+0.397 n=37. Faltan ~3 resoluciones para umbral n≥40. ETA: ~2h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 1762 | +0.046 | -36.80€ | 0 | 4 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 1762 | +0.046 | -36.80€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 803 | +0.018 | -39.66€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 803 | +0.018 | -39.66€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 628 | +0.068 | -16.76€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 628 | +0.068 | -16.76€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 312 | +0.073 | +19.97€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 312 | +0.073 | +19.97€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS | 1997 | -0.088 | -399.06€ | 0 | 1 |
| ✅ BALLENAS_TARDIAS#15min | 388 | +0.200 | -45.62€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 1609 | -0.158 | -353.44€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#BNB | 343 | -0.329 | -100.55€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#BNB#5min | 343 | -0.329 | -100.55€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 388 | +0.200 | -45.62€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 388 | +0.200 | -45.62€ | 0 | 1 |
| ✅ BALLENAS_TARDIAS#DOGE | 254 | -0.156 | -38.91€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 254 | -0.156 | -38.91€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 348 | -0.043 | -13.82€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 348 | -0.043 | -13.82€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#SOL | 337 | -0.205 | -70.88€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#SOL#5min | 337 | -0.205 | -70.88€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 327 | -0.050 | -129.28€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 327 | -0.050 | -129.28€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 24387 | +0.118 | -604.79€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 6047 | +0.195 | -96.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 441 | +0.051 | -39.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 16050 | +0.088 | -520.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 1849 | +0.139 | +51.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 2557 | +0.086 | -125.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 11 | +0.021 | -0.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | +0.018 | +2.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 2541 | +0.086 | -127.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 5287 | +0.128 | -97.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1899 | +0.182 | -94.96€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 141 | +0.052 | -15.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 2547 | +0.086 | -52.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 700 | +0.148 | +65.21€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 2556 | +0.077 | -148.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 15 | +0.243 | +10.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 2540 | +0.075 | -160.91€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 5720 | +0.135 | -33.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2023 | +0.197 | +5.95€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 146 | -0.013 | -31.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 2952 | +0.100 | -22.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 599 | +0.132 | +14.58€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#SOL | 5682 | +0.136 | -162.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2060 | +0.205 | -15.65€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 146 | +0.108 | +4.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 2926 | +0.088 | -123.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 550 | +0.134 | -28.32€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 2585 | +0.094 | -36.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 39 | +0.134 | -2.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 2544 | +0.093 | -33.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 2203 | +0.201 | -153.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 2203 | +0.201 | -153.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 335 | +0.147 | -45.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min |