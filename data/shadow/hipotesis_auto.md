# Hipótesis automáticas — 2026-07-20 03:59 UTC
_Generado por shadow_postmortem.py sobre 23774 resoluciones (PNL=+6090.02€)_
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.209 (n=1500)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.204 (n=1670)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.215 (n=595)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.173)

- **PATRÓN** `py_entrada` < `0.265` → IC=+0.340 (n=542)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.265 (IC base=+0.173)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.178 (n=1777)

  - _Acción_: Kelly boost +0.89€ cuando `libro_spread` < 0.01 (IC base=+0.173)

- **PATRÓN** `libro_liquidez` > `4191.5309` → IC=+0.175 (n=1064)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 4191.5309 (IC base=+0.173)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.227 (n=240)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.226 (n=71)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.201 (n=85)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.176)

- **PATRÓN** `libro_liquidez` > `5928.6237` → IC=+0.179 (n=26)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 5928.6237 (IC base=+0.176)
...
- **PATRÓN** `py_entrada` < `0.225` → IC=+0.342 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.225 (IC base=+0.210)

- **PATRÓN** `libro_liquidez` > `1576.3156` → IC=+0.210 (n=336)
...
- **PATRÓN** `py_entrada` < `0.585` → IC=+0.225 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.585 (IC base=+0.200)

- **PATRÓN** `py_entrada` > `0.64` → IC=+0.200 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.64 (IC base=+0.200)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.207 (n=80)
...
- **PATRÓN** `dist_vwap_pct` < `0.8085` → IC=+0.130 (n=1197)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.8085 (IC base=+0.116)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `11.074` → IC=+0.128 (n=1078)
...
- **PATRÓN** `sigma_h` < `0.0129` → IC=+0.138 (n=490)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.173 (n=249)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.237 (n=226)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` > `0.2769` → IC=+0.215 (n=335)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2769 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.331` → IC=+0.251 (n=499)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.331 (IC base=+0.165)

- **PATRÓN** `sigma_h` > `0.0044` → IC=+0.191 (n=1119)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.199 (n=377)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.176)

- **PATRÓN** `dist_vwap_pct` < `0.7773` → IC=+0.196 (n=1044)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.147 (n=310)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 18.0 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.414` → IC=+0.175 (n=75)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.414 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.722` → IC=+0.284 (n=35)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.141 (n=115)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` < 6.0 (IC base=+0.131)

- **PATRÓN** `dist_vwap_pct` > `0.7231` → IC=+0.180 (n=48)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.7231 (IC base=+0.131)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.325 (n=283)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.306 (n=251)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.301)

- **PATRÓN** `dist_vwap_pct` > `0.1637` → IC=+0.316 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1637 (IC base=+0.301)

- **PATRÓN** `dist_vwap_pct` < `0.5023` → IC=+0.309 (n=286)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5023 (IC base=+0.301)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.68` → IC=+0.314 (n=256)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.68 (IC base=+0.301)
...
- **PATRÓN** `dist_vwap_pct` < `0.1095` → IC=+0.158 (n=582)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.1095 (IC base=+0.131)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.125 (n=833)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` < 12.0 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.5163` → IC=+0.138 (n=1002)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.5163 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.951` → IC=+0.133 (n=213)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` > 5.951 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.187` → IC=+0.131 (n=956)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` < 11.187 (IC base=+0.110)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.145 (n=91)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` < 0.0039 (IC base=+0.077)

- **PATRÓN** `dist_vwap_pct` < `0.2213` → IC=+0.122 (n=149)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.2213 (IC base=+0.077)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.565` → IC=+0.262 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.565 (IC base=+0.077)
...
- **FILTRO** `sigma_ewma_delta_pct` > `4.825` → IC=-0.167 (n=16)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.075 (n=226)
...
- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.139 (n=95)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0083 (IC base=+0.049)
...
- **PATRÓN** `sigma_h` < `0.0116` → IC=+0.218 (n=246)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0116 (IC base=+0.217)
...
- **PATRÓN** `dist_vwap_pct` < `0.5022` → IC=+0.216 (n=347)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5022 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.223` → IC=+0.227 (n=331)
...
| ✅ FAVORITO_CONFIRMADO | 4068 | +0.186 | +61.26€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 2651 | +0.218 | +94.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 172 | +0.081 | +2.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 549 | +0.099 | -82.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 696 | +0.161 | +46.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 1182 | +0.198 | +47.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 883 | +0.211 | +2.35€ | 0 | 5 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 1448 | +0.180 | +11.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 880 | +0.221 | +44.38€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 57 | +0.009 | -7.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 279 | +0.105 | -39.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 232 | +0.154 | +14.53€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#SOL | 1420 | +0.183 | +0.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 874 | +0.224 | +51.00€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 58 | +0.117 | +5.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 256 | +0.101 | -43.81€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 232 | +0.137 | -12.31€ | 0 | 5 |
...
| ✅ GBM_LATE_15M | 6028 | +0.109 | +2139.18€ | 0 | 10 |
| ✅ GBM_LATE_15M#15min | 6028 | +0.109 | +2139.18€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#SOL | 1635 | +0.100 | +645.67€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1635 | +0.100 | +645.67€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1587 | +0.166 | +908.59€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1587 | +0.166 | +908.59€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 3247 | +0.156 | +2029.54€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 3247 | +0.156 | +2029.54€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 827 | +0.121 | +432.32€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 827 | +0.121 | +432.32€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 851 | +0.118 | +419.66€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 851 | +0.118 | +419.66€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 823 | +0.116 | +368.92€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 823 | +0.116 | +368.92€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 746 | +0.279 | +808.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 746 | +0.279 | +808.64€ | 0 | 12 |
...
| ✅ GBM_LATE_15M_TARDIO | 3210 | +0.104 | +1196.23€ | 0 | 10 |
| ✅ GBM_LATE_15M_TARDIO#15min | 3210 | +0.104 | +1196.23€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 764 | +0.074 | +178.68€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 764 | +0.074 | +178.68€ | 0 | 8 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 771 | +0.042 | +92.36€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 771 | +0.042 | +92.36€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 730 | +0.056 | +165.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 730 | +0.056 | +165.64€ | 1 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 945 | +0.214 | +759.55€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 945 | +0.214 | +759.55€ | 0 | 14 |
| ✅ GBM_LATE_5M | 140 | +0.056 | +7.40€ | 0 | 0 |
| ✅ GBM_LATE_5M#5min | 140 | +0.056 | +7.40€ | 0 | 0 |
...
| ✅ GBM_LATE_5M#XRP | 71 | +0.130 | +11.46€ | 0 | 0 |
| ✅ GBM_LATE_5M#XRP#5min | 71 | +0.130 | +11.46€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 737 | +0.175 | +332.37€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 737 | +0.175 | +332.37€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 62 | +0.172 | +24.05€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 62 | +0.172 | +24.05€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 89 | +0.093 | +7.33€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 89 | +0.093 | +7.33€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 186 | +0.223 | +137.42€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 186 | +0.223 | +137.42€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.204: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.057 n=815 | neutral: IC=-0.013 n=602 | overbought(IBS>0.7): IC=+0.191 n=771
  - _Datos_: n=2334 IC=+0.090 PNL=+509.40€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.120 n=14378 | tras_1loss IC=+0.093 n=9203 | tras_2loss IC=+0.060 n=3712/40 | gap=+0.060 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=21316 IC=+0.123 PNL=+6165.90€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=21316 IC=+0.123 PNL=+6165.90€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.167 > 0.08 con n=388 PNL=+259.35€
  - _Datos_: n=388 IC=+0.167 PNL=+259.35€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.193 > 0.08 con n=223 PNL=+4.42€
  - _Datos_: n=223 IC=+0.193 PNL=+4.42€
