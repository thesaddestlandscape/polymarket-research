# Hipótesis automáticas — 2026-07-18 22:17 UTC
_Generado por shadow_postmortem.py sobre 21389 resoluciones (PNL=+5144.48€)_
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.191 (n=535)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 17.0 (IC base=+0.175)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.181 (n=1578)

  - _Acción_: Kelly boost +0.91€ cuando `libro_spread` < 0.01 (IC base=+0.175)

- **PATRÓN** `libro_liquidez` > `7377.4888` → IC=+0.175 (n=463)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 7377.4888 (IC base=+0.175)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.229 (n=116)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `6814.239` → IC=+0.220 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 6814.239 (IC base=+0.215)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.224 (n=150)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.405` → IC=+0.259 (n=313)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.127 (n=1790)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.131 (n=1423)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` > 0.0089 (IC base=+0.113)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.133 (n=1533)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` < 12.0 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `0.1654` → IC=+0.133 (n=341)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.1654 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.9126` → IC=+0.124 (n=1053)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.9126 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.189` → IC=+0.146 (n=224)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` > 6.189 (IC base=+0.113)
...
- **PATRÓN** `dist_vwap_pct` > `0.6735` → IC=+0.179 (n=54)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.6735 (IC base=+0.077)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.146 (n=196)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `13.0` → IC=+0.186 (n=689)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 13.0 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` > `0.1745` → IC=+0.207 (n=374)
...
- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.189 (n=1011)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0046 (IC base=+0.170)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.186 (n=504)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.8826` → IC=+0.188 (n=920)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.136 (n=256)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0061 (IC base=+0.136)

- **PATRÓN** `sigma_h` > `0.0027` → IC=+0.163 (n=256)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.1203` → IC=+0.146 (n=63)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.1203 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `0.4662` → IC=+0.133 (n=224)
...
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.141 (n=232)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0067 (IC base=+0.128)

- **PATRÓN** `sigma_h` > `0.0036` → IC=+0.139 (n=264)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` > 0.0036 (IC base=+0.128)
...
- **PATRÓN** `dist_vwap_pct` > `0.6273` → IC=+0.202 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6273 (IC base=+0.128)
...
- **PATRÓN** `sigma_h` > `0.0136` → IC=+0.201 (n=95)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0136 (IC base=+0.130)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.154 (n=206)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 12.0 (IC base=+0.130)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.135 (n=261)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` < 16.0 (IC base=+0.130)

- **PATRÓN** `dist_vwap_pct` > `0.6196` → IC=+0.173 (n=53)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.6196 (IC base=+0.130)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.625` → IC=+0.275 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.625 (IC base=+0.130)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.135 (n=875)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5433` → IC=+0.179 (n=182)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.5433 (IC base=+0.129)
...
- **PATRÓN** `dist_vwap_pct` < `0.7882` → IC=+0.128 (n=915)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.7882 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.506` → IC=+0.128 (n=237)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 3.506 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.972` → IC=+0.123 (n=809)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.133 (n=88)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 17.0 (IC base=+0.079)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.888` → IC=+0.153 (n=96)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 2.888 (IC base=+0.079)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.89` → IC=+0.192 (n=50)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 7.89 (IC base=+0.045)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.184` → IC=+0.263 (n=74)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.184 (IC base=+0.081)
...
- **PATRÓN** `ibs_15` > `0.7393` → IC=+0.188 (n=299)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.94€ cuando `ibs_15` > 0.7393 (IC base=+0.033)

- **PATRÓN** `dist_vwap_pct` > `0.7405` → IC=+0.167 (n=46)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.7405 (IC base=+0.033)

- **PATRÓN** `dist_vwap_pct` < `0.5043` → IC=+0.138 (n=205)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.5043 (IC base=+0.033)

- **PATRÓN** `sigma_ewma_delta_pct` > `18.602` → IC=+0.143 (n=40)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 18.602 (IC base=+0.033)
...
- **PATRÓN** `drift_60min` |x|≤ `0.237` → IC=+0.121 (n=217)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.61€ cuando `drift_60min` |x|≤ 0.237 (IC base=+0.074)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.147 (n=165)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 12.0 (IC base=+0.074)

- **PATRÓN** `ibs_15` > `0.7019` → IC=+0.196 (n=159)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.98€ cuando `ibs_15` > 0.7019 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` > `0.7736` → IC=+0.259 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7736 (IC base=+0.074)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.517` → IC=+0.167 (n=52)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` < 4.517 (IC base=+0.074)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS > 0.7393 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.188 n=299). Confirma señal de reversión media → alinear con BUY_NO.
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS > 0.7019 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.196 n=159). Confirma señal de reversión media → alinear con BUY_NO.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 234 | +0.136 | +10.96€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 234 | +0.136 | +10.96€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 99 | +0.153 | +4.63€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 99 | +0.153 | +4.63€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 82 | +0.155 | +5.18€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 82 | +0.155 | +5.18€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 53 | +0.064 | +1.15€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 53 | +0.064 | +1.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 3562 | +0.183 | +31.16€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 2311 | +0.216 | +78.42€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 1034 | +0.191 | +25.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 771 | +0.208 | -4.08€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 1266 | +0.177 | +7.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 765 | +0.220 | +40.83€ | 0 | 9 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 1248 | +0.182 | -2.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 764 | +0.223 | +44.65€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 5644 | +0.106 | +1880.96€ | 0 | 8 |
| ✅ GBM_LATE_15M#15min | 5644 | +0.106 | +1880.96€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1350 | +0.075 | +239.04€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1350 | +0.075 | +239.04€ | 0 | 5 |
| ✅ GBM_LATE_15M#ETH | 1302 | +0.079 | +248.65€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1302 | +0.079 | +248.65€ | 0 | 5 |
| ✅ GBM_LATE_15M#SOL | 1523 | +0.101 | +594.54€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1523 | +0.101 | +594.54€ | 1 | 7 |
| ✅ GBM_LATE_15M#XRP | 1469 | +0.162 | +798.72€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1469 | +0.162 | +798.72€ | 0 | 14 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2869 | +0.152 | +1713.61€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2869 | +0.152 | +1713.61€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 733 | +0.116 | +350.59€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 733 | +0.116 | +350.59€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 754 | +0.119 | +361.69€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 754 | +0.119 | +361.69€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 741 | +0.111 | +319.92€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 741 | +0.111 | +319.92€ | 1 | 9 |
...
| ✅ GBM_LATE_15M_TARDIO | 2814 | +0.102 | +993.42€ | 0 | 10 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2814 | +0.102 | +993.42€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 683 | +0.071 | +143.04€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 683 | +0.071 | +143.04€ | 0 | 7 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 677 | +0.041 | +70.40€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 677 | +0.041 | +70.40€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 627 | +0.055 | +129.87€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 627 | +0.055 | +129.87€ | 2 | 6 |
...
| ✅ UPDOWN_GBM | 1787 | +0.021 | +133.37€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1527 | +0.047 | +171.42€ | 1 | 7 |
...
| ✅ UPDOWN_GBM#BTC | 453 | +0.025 | +28.77€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 370 | +0.062 | +42.74€ | 1 | 7 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 521 | +0.181 | +222.87€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 521 | +0.181 | +222.87€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 43 | +0.189 | +16.68€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 43 | +0.189 | +16.68€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 60 | +0.081 | +8.04€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 60 | +0.081 | +8.04€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 33 | +0.271 | +20.23€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 33 | +0.271 | +20.23€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 129 | +0.202 | +49.10€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 129 | +0.202 | +49.10€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 134 | +0.132 | +46.93€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 134 | +0.132 | +46.93€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.179: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.051 n=695 | neutral: IC=-0.007 n=507 | overbought(IBS>0.7): IC=+0.172 n=644
  - _Datos_: n=1976 IC=+0.083 PNL=+391.72€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=02h: IC=-0.112 n=47 PNL=-10.79€ → FILTRAR | H=07h: IC=+0.123 n=59 PNL=+26.81€ → BOOST | H=10h: IC=-0.154 n=15 PNL=-6.77€ → FILTRAR | H=14h: IC=+0.156 n=59 PNL=+26.91€ → BOOST | H=21h: IC=+0.129 n=60 PNL=+13.15€ → BOOST | H=22h: IC=+0.103 n=76 PNL=+13.62€ → BOOST
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.116 n=12820 | tras_1loss IC=+0.086 n=8370 | tras_2loss IC=+0.052 n=3431/40 | gap=+0.064 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1658 IC=+0.028 PNL=+150.75€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1658 IC=+0.028 PNL=+150.75€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=773 IC=+0.033 PNL=+61.71€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=773 IC=+0.033 PNL=+61.71€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=370 IC=+0.062 PNL=+42.74€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=370 IC=+0.062 PNL=+42.74€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.080 > 0.08 con n=808 PNL=+120.83€
  - _Datos_: n=808 IC=+0.080 PNL=+120.83€
...
**〰️ H-CUSTOM-CROSS-WINDOW-SPREAD-NEG** — GBM 15min/60min: spread negativo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: n=92 IC=+0.021 PNL=+28.82€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=92 IC=+0.021 PNL=+28.82€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=18931 IC=+0.119 PNL=+5220.36€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=18931 IC=+0.119 PNL=+5220.36€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=923 IC=+0.003 PNL=+31.16€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=923 IC=+0.003 PNL=+31.16€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.160 > 0.1 con n=507 PNL=+173.72€
  - _Datos_: n=507 IC=+0.160 PNL=+173.72€
...
**〰️ H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: n=180 IC=+0.060 PNL=+6.28€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=180 IC=+0.060 PNL=+6.28€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=605 IC=-0.090 PNL=+91.33€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=605 IC=-0.090 PNL=+91.33€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=710 IC=+0.066 PNL=+100.53€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=710 IC=+0.066 PNL=+100.53€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.136 > 0.08 con n=369 PNL=+107.95€
  - _Datos_: n=369 IC=+0.136 PNL=+107.95€
...
**🟡 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.178 > 0.08 con n=169 PNL=+63.45€
  - _Datos_: n=169 IC=+0.178 PNL=+63.45€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.135 > 0.08 con n=261 PNL=+97.69€
  - _Datos_: n=261 IC=+0.135 PNL=+97.69€
