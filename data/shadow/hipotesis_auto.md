# Hipótesis automáticas — 2026-09-20 22:59 UTC
_Generado por shadow_postmortem.py sobre 532780 resoluciones (PNL=+59144.40€)_
...
- **FILTRO** `py_entrada` > `0.495` → IC=-0.263 (n=112)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.117 (n=444)
...
- **PATRÓN** `ballena_activa_n` < `86.0` → IC=+0.167 (n=76)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 86.0 (IC base=+0.039)
...
- **FILTRO** `restante_s_al_confirmar` < `133.5` → IC=-0.276 (n=776)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 133.5
  - _Potencial_: sin este filtro IC_bueno=-0.004 (n=2330)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `160.52` → IC=-0.232 (n=1522)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 160.52
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=4567)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `121.38` → IC=-0.365 (n=1277)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 121.38
  - _Potencial_: sin este filtro IC_bueno=-0.117 (n=3831)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.198 (n=12956)

  - _Acción_: Kelly boost +0.99€ cuando `py_entrada` > 0.69 (IC base=+0.100)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.152 (n=3239)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.01 (IC base=+0.100)

- **PATRÓN** `libro_liquidez` > `5470.7315` → IC=+0.173 (n=2062)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 5470.7315 (IC base=+0.100)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.143 (n=10299)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.35` → IC=+0.236 (n=9898)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.35 (IC base=+0.130)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.176 (n=5302)

  - _Acción_: Kelly boost +0.88€ cuando `libro_spread` < 0.01 (IC base=+0.130)

- **PATRÓN** `libro_liquidez` > `7556.1139` → IC=+0.172 (n=1991)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 7556.1139 (IC base=+0.130)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.209 (n=1338)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.745` → IC=+0.349 (n=702)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.745 (IC base=+0.204)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.204 (n=1932)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.204 (n=1403)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.151 (n=2595)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.334 (n=854)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.71 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.249 (n=623)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.229 (n=567)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.199)

- **PATRÓN** `py_entrada` > `0.86` → IC=+0.430 (n=581)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.162 (n=288)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 17.0 (IC base=+0.118)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.202 (n=10409)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.224 (n=3590)
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.180 (n=2555)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.281 (n=304)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.330 (n=468)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.258)

### FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.185 (n=2375)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.197 (n=2432)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.192 (n=1761)

  - _Acción_: Kelly boost +0.96€ cuando `py_entrada` < 0.71 (IC base=+0.189)

### FAVORITO_CONFIRMADO_15MIN_EXTREMO
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.434 (n=424)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `2062.8229` → IC=+0.437 (n=472)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.400 (n=98)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.915` → IC=+0.404 (n=102)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.201 (n=30900)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.198)

- **PATRÓN** `py_entrada` > `0.76` → IC=+0.240 (n=11567)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.76 (IC base=+0.198)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.175 (n=6292)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.188 (n=5737)

  - _Acción_: Kelly boost +0.94€ cuando `py_entrada` > 0.71 (IC base=+0.174)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.226 (n=5522)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.232 (n=2782)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `3.56` → IC=+0.146 (n=2136)

  - _Acción_: Kelly boost +0.73€ cuando `lag_apertura_s` < 3.56 (IC base=+0.124)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.200 (n=2338)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.38 (IC base=+0.116)

- **PATRÓN** `restante_min` < `4.16` → IC=+0.126 (n=2170)

  - _Acción_: Kelly boost +0.63€ cuando `restante_min` < 4.16 (IC base=+0.116)

- **PATRÓN** `restante_min` > `4.96` → IC=+0.144 (n=2299)

  - _Acción_: Kelly boost +0.72€ cuando `restante_min` > 4.96 (IC base=+0.116)

- **PATRÓN** `lag_apertura_s` < `2.35` → IC=+0.146 (n=2174)
...
### GBM_LATE_15M
- **PATRÓN** `drift_60min` |x|≤ `0.3539` → IC=+0.126 (n=6325)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.63€ cuando `drift_60min` |x|≤ 0.3539 (IC base=+0.102)

- **PATRÓN** `ibs_20min` > `0.9804` → IC=+0.240 (n=2399)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9804 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` < `0.5964` → IC=+0.245 (n=2005)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5964 (IC base=+0.102)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.922` → IC=+0.174 (n=2773)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 5.922 (IC base=+0.102)

- **PATRÓN** `volumen_regimen` < `1.2107` → IC=+0.245 (n=1900)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2107 (IC base=+0.102)

- **PATRÓN** `volumen_regimen` > `1.0496` → IC=+0.245 (n=861)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0496 (IC base=+0.102)

- **PATRÓN** `volumen_pendiente_norm` > `0.3078` → IC=+0.212 (n=706)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3078 (IC base=+0.102)

- **PATRÓN** `volumen_spike_ratio` > `1.9163` → IC=+0.204 (n=3238)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9163 (IC base=+0.102)

- **PATRÓN** `ibs_20min` < `0.5706` → IC=+0.132 (n=8688)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.5706 (IC base=+0.063)

- **PATRÓN** `dist_vwap_pct` > `0.5712` → IC=+0.200 (n=601)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5712 (IC base=+0.063)

- **PATRÓN** `dist_vwap_pct` < `0.1425` → IC=+0.172 (n=2694)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.1425 (IC base=+0.063)

- **PATRÓN** `volumen_regimen` < `0.7025` → IC=+0.176 (n=1303)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 0.7025 (IC base=+0.063)

- **PATRÓN** `volumen_regimen` > `0.8706` → IC=+0.175 (n=1974)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 0.8706 (IC base=+0.063)

- **PATRÓN** `volumen_pendiente_norm` > `0.168` → IC=+0.225 (n=1436)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.168 (IC base=+0.063)

- **PATRÓN** `volumen_spike_ratio` > `1.5845` → IC=+0.202 (n=4400)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5845 (IC base=+0.063)

- **PATRÓN** `ballena_activa_n` < `143.0` → IC=+0.211 (n=4702)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 143.0 (IC base=+0.063)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3396` → IC=+0.168 (n=1631)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.3396 (IC base=+0.163)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.167 (n=800)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.063` → IC=+0.275 (n=708)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.063 (IC base=+0.163)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4398` → IC=+0.167 (n=1516)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.4398 (IC base=+0.163)

- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.248 (n=1068)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `drift_60min` |x|≤ `0.1134` → IC=+0.249 (n=543)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1134 (IC base=+0.212)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.226 (n=1234)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_regimen` < `1.2049` → IC=+0.147 (n=1294)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 1.2049 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` > `0.8526` → IC=+0.137 (n=862)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 0.8526 (IC base=+0.135)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4451` → IC=+0.147 (n=1184)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 2.4451 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` > `1.7707` → IC=+0.145 (n=789)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.7707 (IC base=+0.135)

- **PATRÓN** `ballena_activa_n` < `412.0` → IC=+0.144 (n=1100)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3914` → IC=+0.184 (n=1394)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.3914 (IC base=+0.184)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.185 (n=1589)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.577` → IC=+0.240 (n=455)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.577 (IC base=+0.184)

- **PATRÓN** `volumen_pendiente_norm` < `0.2142` → IC=+0.187 (n=1571)
...
- **PATRÓN** `volumen_spike_ratio` > `2.9235` → IC=+0.206 (n=679)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9235 (IC base=+0.184)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.192 (n=1052)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.258 (n=515)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.218)

- **PATRÓN** `ibs_20min` < `0.3824` → IC=+0.231 (n=1189)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.394` → IC=+0.219 (n=1477)
...
- **PATRÓN** `ballena_activa_n` < `25.0` → IC=+0.227 (n=794)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.149 (n=92)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=1998)

- **PATRÓN** `ibs_20min` > `0.94` → IC=+0.199 (n=324)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.94 (IC base=+0.019)

- **PATRÓN** `dist_vwap_pct` > `0.3482` → IC=+0.348 (n=123)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3482 (IC base=+0.019)

- **PATRÓN** `dist_vwap_pct` < `0.682` → IC=+0.328 (n=306)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.682 (IC base=+0.019)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.619` → IC=+0.149 (n=631)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.167` → IC=+0.325 (n=227)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.167 (IC base=+0.019)

- **PATRÓN** `volumen_pendiente_norm` > `0.2919` → IC=+0.346 (n=76)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1778` → IC=+0.332 (n=123)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1778 (IC base=+0.019)

- **PATRÓN** `ballena_activa_n` < `167.0` → IC=+0.337 (n=268)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 167.0 (IC base=+0.019)

- **PATRÓN** `dist_vwap_pct` > `0.314` → IC=+0.172 (n=227)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.314 (IC base=+0.010)

- **PATRÓN** `volumen_regimen` < `0.856` → IC=+0.152 (n=478)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5138` → IC=+0.178 (n=592)
...
- **FILTRO** `ibs_20min` > `0.2632` → IC=-0.127 (n=1981)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2632
  - _Potencial_: sin este filtro IC_bueno=+0.126 (n=980)

- **FILTRO** `sigma_ewma_delta_pct` > `8.637` → IC=-0.203 (n=321)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.637
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=2640)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1008` → IC=+0.231 (n=210)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1008 (IC base=-0.043)

- **PATRÓN** `volumen_pendiente_norm` > `0.1496` → IC=+0.263 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1496 (IC base=-0.043)

- **PATRÓN** `volumen_spike_ratio` < `2.4353` → IC=+0.269 (n=240)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.4353 (IC base=-0.043)

### GBM_LATE_15M#XRP#15min
- **FILTRO** `drift_60min` |x|> `0.638` → IC=-0.193 (n=503)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.638
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=1510)

- **FILTRO** `ibs_20min` < `0.6943` → IC=-0.159 (n=1328)

  - _Acción_: SKIP cuando `ibs_20min` < 0.6943
  - _Potencial_: sin este filtro IC_bueno=+0.079 (n=685)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.197 (n=394)

  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=1619)

- **FILTRO** `ibs_20min` > `0.7734` → IC=-0.201 (n=748)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7734
  - _Potencial_: sin este filtro IC_bueno=+0.037 (n=2247)

- **PATRÓN** `dist_vwap_pct` > `0.9637` → IC=+0.288 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9637 (IC base=-0.078)

- **PATRÓN** `dist_vwap_pct` < `0.2585` → IC=+0.316 (n=248)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2585 (IC base=-0.078)

- **PATRÓN** `volumen_regimen` > `0.6141` → IC=+0.299 (n=287)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6141 (IC base=-0.078)

- **PATRÓN** `volumen_pendiente_norm` > `0.0764` → IC=+0.300 (n=113)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8487` → IC=+0.296 (n=179)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8487 (IC base=-0.078)

- **PATRÓN** `dist_vwap_pct` > `0.9919` → IC=+0.316 (n=85)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9919 (IC base=-0.022)

- **PATRÓN** `volumen_regimen` < `0.7369` → IC=+0.254 (n=287)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7369 (IC base=-0.022)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1054` → IC=+0.283 (n=228)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1054 (IC base=-0.022)

- **PATRÓN** `volumen_spike_ratio` < `2.2099` → IC=+0.258 (n=478)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.2099 (IC base=-0.022)
...
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.190 (n=2986)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0094 (IC base=+0.094)

- **PATRÓN** `ibs_20min` > `0.4681` → IC=+0.185 (n=7998)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` > 0.4681 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` > `1.0121` → IC=+0.288 (n=690)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0121 (IC base=+0.094)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.553` → IC=+0.152 (n=4256)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 3.553 (IC base=+0.094)

- **PATRÓN** `volumen_regimen` < `1.1726` → IC=+0.230 (n=3128)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.1726 (IC base=+0.094)

- **PATRÓN** `volumen_regimen` > `0.682` → IC=+0.241 (n=2794)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4758` → IC=+0.243 (n=1693)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4758 (IC base=+0.094)

- **PATRÓN** `volumen_spike_ratio` > `2.7143` → IC=+0.242 (n=1693)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7143 (IC base=+0.094)

- **PATRÓN** `ballena_activa_n` < `98.0` → IC=+0.270 (n=4579)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 98.0 (IC base=+0.094)

- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.147 (n=3029)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` > 0.0088 (IC base=+0.070)

- **PATRÓN** `ibs_20min` < `0.5517` → IC=+0.150 (n=7983)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` < 0.5517 (IC base=+0.070)

- **PATRÓN** `dist_vwap_pct` > `0.6792` → IC=+0.241 (n=527)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6792 (IC base=+0.070)

- **PATRÓN** `dist_vwap_pct` < `0.1711` → IC=+0.235 (n=2348)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1711 (IC base=+0.070)

- **PATRÓN** `volumen_regimen` < `0.7171` → IC=+0.237 (n=1142)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7171 (IC base=+0.070)

- **PATRÓN** `volumen_regimen` > `1.2011` → IC=+0.245 (n=865)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2011 (IC base=+0.070)

- **PATRÓN** `volumen_pendiente_norm` > `0.2479` → IC=+0.309 (n=663)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2479 (IC base=+0.070)

- **PATRÓN** `volumen_spike_ratio` < `1.4914` → IC=+0.253 (n=1127)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4914 (IC base=+0.070)

- **PATRÓN** `volumen_spike_ratio` > `2.3584` → IC=+0.261 (n=1533)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3584 (IC base=+0.070)

- **PATRÓN** `ballena_activa_n` < `81.0` → IC=+0.264 (n=3249)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 81.0 (IC base=+0.070)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `sigma_ewma_delta_pct` > `2.68` → IC=-0.148 (n=645)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 2.68
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=1455)

- **PATRÓN** `ibs_20min` > `0.8933` → IC=+0.271 (n=613)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8933 (IC base=+0.048)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.29` → IC=+0.162 (n=852)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 3.29 (IC base=+0.048)

- **PATRÓN** `volumen_pendiente_norm` > `0.2236` → IC=+0.273 (n=152)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2236 (IC base=+0.048)

- **PATRÓN** `volumen_spike_ratio` < `1.4428` → IC=+0.186 (n=253)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 1.4428 (IC base=+0.048)

- **PATRÓN** `volumen_spike_ratio` > `2.1741` → IC=+0.199 (n=343)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.1741 (IC base=+0.048)

- **PATRÓN** `ballena_activa_n` < `15.0` → IC=+0.185 (n=322)
...
- **FILTRO** `ibs_20min` > `0.8168` → IC=-0.148 (n=654)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8168
  - _Potencial_: sin este filtro IC_bueno=+0.049 (n=1966)

- **PATRÓN** `ibs_20min` > `0.86` → IC=+0.154 (n=603)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` > 0.86 (IC base=+0.019)

- **PATRÓN** `dist_vwap_pct` > `0.295` → IC=+0.158 (n=317)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.295 (IC base=+0.019)

- **PATRÓN** `volumen_regimen` < `1.1952` → IC=+0.142 (n=812)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 1.1952 (IC base=+0.019)

- **PATRÓN** `volumen_regimen` > `0.6591` → IC=+0.149 (n=725)
...
- **PATRÓN** `ballena_activa_n` < `249.0` → IC=+0.189 (n=345)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1056` → IC=+0.246 (n=474)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1056 (IC base=+0.244)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.244 (n=713)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` > `0.0063` → IC=+0.307 (n=1008)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0063 (IC base=+0.285)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.328 (n=388)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.0909` → IC=+0.285 (n=756)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6254` → IC=+0.292 (n=344)
...
- **PATRÓN** `ballena_activa_n` < `30.0` → IC=+0.286 (n=666)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 30.0 (IC base=+0.285)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **FILTRO** `ibs_20min` < `0.2695` → IC=-0.199 (n=430)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2695
...
- **FILTRO** `ibs_20min` > `0.7866` → IC=-0.176 (n=535)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.047 (n=1608)

- **PATRÓN** `ibs_20min` > `0.8055` → IC=+0.155 (n=586)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` > 0.8055 (IC base=+0.004)

- **PATRÓN** `dist_vwap_pct` > `0.4461` → IC=+0.225 (n=205)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4461 (IC base=+0.004)

- **PATRÓN** `volumen_regimen` < `0.9756` → IC=+0.232 (n=404)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9756 (IC base=+0.004)

- **PATRÓN** `volumen_regimen` > `0.6434` → IC=+0.209 (n=411)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0961` → IC=+0.246 (n=380)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0961 (IC base=+0.004)

- **PATRÓN** `ballena_activa_n` < `103.0` → IC=+0.265 (n=291)
...
- **FILTRO** `ibs_20min` < `0.7037` → IC=-0.207 (n=952)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7037
  - _Potencial_: sin este filtro IC_bueno=+0.271 (n=955)

- **FILTRO** `ibs_20min` > `0.6923` → IC=-0.231 (n=500)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6923
  - _Potencial_: sin este filtro IC_bueno=+0.093 (n=1505)

- **FILTRO** `sigma_ewma_delta_pct` > `4.677` → IC=-0.175 (n=447)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.677
  - _Potencial_: sin este filtro IC_bueno=+0.065 (n=1558)

- **PATRÓN** `ibs_20min` > `0.7037` → IC=+0.271 (n=955)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7037 (IC base=+0.032)

- **PATRÓN** `dist_vwap_pct` > `0.2938` → IC=+0.327 (n=379)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2938 (IC base=+0.032)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.568` → IC=+0.154 (n=299)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 9.568 (IC base=+0.032)

- **PATRÓN** `volumen_regimen` < `0.8618` → IC=+0.296 (n=459)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8618 (IC base=+0.032)

- **PATRÓN** `volumen_regimen` > `0.6364` → IC=+0.287 (n=688)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6364 (IC base=+0.032)

- **PATRÓN** `volumen_pendiente_norm` < `0.1049` → IC=+0.288 (n=636)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7987` → IC=+0.305 (n=444)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7987 (IC base=+0.032)

- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.316 (n=578)
...
- **PATRÓN** `sigma_h` > `0.016` → IC=+0.318 (n=780)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.9091` → IC=+0.346 (n=783)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9091 (IC base=+0.274)

- **PATRÓN** `dist_vwap_pct` > `0.2766` → IC=+0.315 (n=610)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2766 (IC base=+0.274)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.468` → IC=+0.299 (n=625)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.468 (IC base=+0.274)

- **PATRÓN** `volumen_regimen` > `1.0357` → IC=+0.305 (n=531)
...
- **PATRÓN** `ibs_20min` < `0.3962` → IC=+0.304 (n=1291)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3962 (IC base=+0.271)

- **PATRÓN** `dist_vwap_pct` > `0.5397` → IC=+0.285 (n=365)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5397 (IC base=+0.271)

- **PATRÓN** `dist_vwap_pct` < `0.8963` → IC=+0.270 (n=1466)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.8963 (IC base=+0.271)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5387` → IC=+0.266 (n=1127)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5387 (IC base=+0.271)

- **PATRÓN** `volumen_spike_ratio` > `2.1751` → IC=+0.272 (n=510)
...
- **PATRÓN** `libro_liquidez` > `2582.6436` → IC=+0.278 (n=861)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2582.6436 (IC base=+0.271)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` < `0.0048` → IC=+0.171 (n=2358)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0048 (IC base=+0.167)

- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.201 (n=2354)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0108 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0884` → IC=+0.186 (n=2353)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.0884 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.177 (n=7383)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 5.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` > `0.5814` → IC=+0.217 (n=7052)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5814 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.1688` → IC=+0.194 (n=3068)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.1688 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.219` → IC=+0.255 (n=1449)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.219 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.2128` → IC=+0.160 (n=4662)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2128 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` > `0.6254` → IC=+0.157 (n=4662)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 0.6254 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.2463` → IC=+0.192 (n=1414)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.2463 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.5656` → IC=+0.171 (n=2965)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.5656 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.6486` → IC=+0.174 (n=2245)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.6486 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `2372.0487` → IC=+0.167 (n=4701)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 2372.0487 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `118.0` → IC=+0.180 (n=5964)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 118.0 (IC base=+0.167)

- **PATRÓN** `sigma_h` < `0.0065` → IC=+0.182 (n=4498)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.209 (n=2288)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` < `0.4753` → IC=+0.226 (n=6735)
...
- **PATRÓN** `volumen_spike_ratio` > `2.651` → IC=+0.176 (n=2023)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.651 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `119.0` → IC=+0.172 (n=5695)
...
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.190 (n=405)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0083 (IC base=+0.184)

- **PATRÓN** `drift_60min` |x|≤ `0.334` → IC=+0.206 (n=1214)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.067` → IC=+0.306 (n=551)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.067 (IC base=+0.184)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4343` → IC=+0.182 (n=1115)
...
- **PATRÓN** `sigma_h` > `0.0047` → IC=+0.253 (n=763)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.247 (n=778)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3451` → IC=+0.266 (n=853)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3451 (IC base=+0.239)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.049` → IC=+0.254 (n=928)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.049 (IC base=+0.239)

- **PATRÓN** `volumen_pendiente_norm` < `0.0953` → IC=+0.234 (n=701)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0744` → IC=+0.205 (n=347)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0744 (IC base=+0.162)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.185 (n=1041)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 6.0 (IC base=+0.162)

- **PATRÓN** `ibs_20min` > `0.4147` → IC=+0.228 (n=1041)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4147 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` > `0.1993` → IC=+0.214 (n=623)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1993 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.506` → IC=+0.232 (n=211)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4144` → IC=+0.204 (n=336)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4144 (IC base=+0.162)

- **PATRÓN** `libro_liquidez` > `15748.8418` → IC=+0.171 (n=472)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 15748.8418 (IC base=+0.162)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4318` → IC=+0.161 (n=1120)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.4318 (IC base=+0.148)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.162 (n=1123)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 6.0 (IC base=+0.148)

- **PATRÓN** `ibs_20min` > `0.3943` → IC=+0.202 (n=1119)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3943 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` > `0.1514` → IC=+0.182 (n=749)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.1514 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.953` → IC=+0.243 (n=200)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.953 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` < `1.048` → IC=+0.154 (n=985)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 1.048 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` > `1.1899` → IC=+0.153 (n=373)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.1899 (IC base=+0.148)

- **PATRÓN** `volumen_pendiente_norm` > `0.2911` → IC=+0.218 (n=172)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2911 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` < `1.5382` → IC=+0.159 (n=482)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 1.5382 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` > `2.5292` → IC=+0.173 (n=365)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.5292 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `6913.8034` → IC=+0.184 (n=746)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 6913.8034 (IC base=+0.148)

- **PATRÓN** `ballena_activa_n` < `168.0` → IC=+0.149 (n=1055)
...
- **PATRÓN** `volumen_regimen` < `1.2208` → IC=+0.126 (n=1282)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` < 1.2208 (IC base=+0.115)

- **PATRÓN** `volumen_pendiente_norm` < `0.1666` → IC=+0.129 (n=1278)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_pendiente_norm` < 0.1666 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` < `1.8138` → IC=+0.134 (n=823)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 1.8138 (IC base=+0.115)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.126 (n=1336)

  - _Acción_: Kelly boost +0.63€ cuando `libro_spread` < 0.02 (IC base=+0.115)

- **PATRÓN** `libro_liquidez` > `2881.368` → IC=+0.197 (n=582)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 2881.368 (IC base=+0.115)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.133 (n=955)
...
- **PATRÓN** `sigma_h` > `0.0184` → IC=+0.207 (n=814)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0184 (IC base=+0.196)

- **PATRÓN** `drift_60min` |x|≤ `0.1691` → IC=+0.199 (n=537)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.7321` → IC=+0.255 (n=1091)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7321 (IC base=+0.196)

- **PATRÓN** `dist_vwap_pct` > `1.2244` → IC=+0.220 (n=298)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2244 (IC base=+0.196)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.407` → IC=+0.241 (n=581)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.407 (IC base=+0.196)

- **PATRÓN** `volumen_regimen` < `1.2084` → IC=+0.198 (n=1221)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` < 1.2084 (IC base=+0.196)

- **PATRÓN** `volumen_regimen` > `0.8579` → IC=+0.214 (n=814)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1687` → IC=+0.206 (n=1035)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1687 (IC base=+0.196)

- **PATRÓN** `volumen_spike_ratio` > `1.8078` → IC=+0.201 (n=784)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8078 (IC base=+0.196)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.196 (n=1276)

  - _Acción_: Kelly boost +0.98€ cuando `libro_spread` < 0.02 (IC base=+0.196)

- **PATRÓN** `libro_liquidez` > `2587.189` → IC=+0.196 (n=814)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 2587.189 (IC base=+0.196)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1349` → IC=+0.154 (n=732)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.1349 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.189 (n=839)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 15.0 (IC base=+0.145)

- **PATRÓN** `ibs_20min` > `0.3984` → IC=+0.180 (n=1659)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` > 0.3984 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.367` → IC=+0.181 (n=490)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.367 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.726` → IC=+0.176 (n=772)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 3.726 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` < `0.8709` → IC=+0.163 (n=957)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1642` → IC=+0.174 (n=458)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.149 (n=1862)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.02 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `2477.0811` → IC=+0.149 (n=1482)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 2477.0811 (IC base=+0.145)

- **PATRÓN** `ballena_activa_n` < `165.0` → IC=+0.165 (n=1438)
...
- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.212 (n=161)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0068 (IC base=+0.202)

- **PATRÓN** `drift_60min` |x|≤ `0.0953` → IC=+0.218 (n=161)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0953 (IC base=+0.202)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.256 (n=240)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.202)

- **PATRÓN** `ibs_20min` > `0.6994` → IC=+0.265 (n=322)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6994 (IC base=+0.202)

- **PATRÓN** `dist_vwap_pct` > `0.3651` → IC=+0.250 (n=166)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3651 (IC base=+0.202)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.043` → IC=+0.241 (n=160)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.043 (IC base=+0.202)

- **PATRÓN** `volumen_regimen` < `0.8399` → IC=+0.213 (n=322)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8399 (IC base=+0.202)

- **PATRÓN** `volumen_regimen` > `0.6043` → IC=+0.205 (n=483)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6043 (IC base=+0.202)

- **PATRÓN** `volumen_pendiente_norm` > `0.2528` → IC=+0.317 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2528 (IC base=+0.202)
...
- **PATRÓN** `volumen_spike_ratio` > `2.391` → IC=+0.270 (n=159)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.391 (IC base=+0.202)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0109` → IC=+0.198 (n=2996)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0109 (IC base=+0.166)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.175 (n=9399)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 5.0 (IC base=+0.166)

- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.215 (n=8984)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4706 (IC base=+0.166)

- **PATRÓN** `dist_vwap_pct` > `0.9038` → IC=+0.194 (n=1227)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.9038 (IC base=+0.166)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.602` → IC=+0.225 (n=4380)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.602 (IC base=+0.166)

- **PATRÓN** `volumen_regimen` < `0.8812` → IC=+0.165 (n=4004)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.8812 (IC base=+0.166)

- **PATRÓN** `volumen_pendiente_norm` > `0.2405` → IC=+0.195 (n=1673)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2405 (IC base=+0.166)

- **PATRÓN** `volumen_spike_ratio` > `2.6243` → IC=+0.187 (n=2865)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 2.6243 (IC base=+0.166)

- **PATRÓN** `libro_liquidez` > `2323.1832` → IC=+0.168 (n=5983)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2323.1832 (IC base=+0.166)

- **PATRÓN** `ballena_activa_n` < `89.0` → IC=+0.193 (n=6695)

  - _Acción_: Kelly boost +0.96€ cuando `ballena_activa_n` < 89.0 (IC base=+0.166)

- **PATRÓN** `sigma_h` < `0.0069` → IC=+0.191 (n=5438)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0069 (IC base=+0.181)

- **PATRÓN** `drift_60min` |x|≤ `0.1441` → IC=+0.186 (n=3590)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.1441 (IC base=+0.181)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.207 (n=3135)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.181)

- **PATRÓN** `ibs_20min` < `0.5652` → IC=+0.238 (n=8157)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5652 (IC base=+0.181)

- **PATRÓN** `dist_vwap_pct` < `0.2371` → IC=+0.161 (n=5091)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` < 0.2371 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.897` → IC=+0.201 (n=1153)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.897 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.709` → IC=+0.182 (n=7907)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.709 (IC base=+0.181)

- **PATRÓN** `volumen_regimen` < `0.7053` → IC=+0.159 (n=2484)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.7053 (IC base=+0.181)

- **PATRÓN** `volumen_regimen` > `1.2035` → IC=+0.152 (n=1883)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2922` → IC=+0.191 (n=3361)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` > 2.2922 (IC base=+0.181)

- **PATRÓN** `ballena_activa_n` < `25.0` → IC=+0.197 (n=2468)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 25.0 (IC base=+0.181)

### GBM_LATE_15M_TARDIO#BNB#15min
- **PATRÓN** `sigma_h` < `0.0051` → IC=+0.209 (n=510)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0051 (IC base=+0.191)

- **PATRÓN** `sigma_h` > `0.0064` → IC=+0.212 (n=1019)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0064 (IC base=+0.191)

- **PATRÓN** `drift_60min` |x|≤ `0.3418` → IC=+0.193 (n=1530)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.97€ cuando `drift_60min` |x|≤ 0.3418 (IC base=+0.191)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.198 (n=746)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.322 (n=549)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.267 (n=1061)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3469` → IC=+0.290 (n=1026)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3469 (IC base=+0.257)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.424` → IC=+0.266 (n=1232)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2706` → IC=+0.188 (n=206)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.2706 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` < `2.1246` → IC=+0.158 (n=1212)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.1246 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` > `1.4084` → IC=+0.152 (n=1377)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.4084 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `13656.0542` → IC=+0.152 (n=954)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 13656.0542 (IC base=+0.147)

- **PATRÓN** `ballena_activa_n` < `482.0` → IC=+0.155 (n=1309)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.178 (n=424)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.6721` → IC=+0.255 (n=1289)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6721 (IC base=+0.216)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.736` → IC=+0.292 (n=431)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.736 (IC base=+0.216)

- **PATRÓN** `volumen_pendiente_norm` < `0.2156` → IC=+0.222 (n=1414)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2156 (IC base=+0.216)

- **PATRÓN** `volumen_spike_ratio` > `2.9133` → IC=+0.248 (n=620)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9133 (IC base=+0.216)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.227 (n=954)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.216)

- **PATRÓN** `libro_liquidez` > `1932.9398` → IC=+0.223 (n=481)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1932.9398 (IC base=+0.216)

- **PATRÓN** `sigma_h` < `0.0107` → IC=+0.241 (n=1343)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0107 (IC base=+0.237)

- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.241 (n=1201)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.267 (n=513)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.237)

- **PATRÓN** `ibs_20min` < `0.3653` → IC=+0.269 (n=1182)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7926` → IC=+0.232 (n=536)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7926 (IC base=+0.237)

- **PATRÓN** `volumen_spike_ratio` > `2.2342` → IC=+0.237 (n=812)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2342 (IC base=+0.237)
...
- **PATRÓN** `ballena_activa_n` < `32.0` → IC=+0.244 (n=769)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4329` → IC=+0.143 (n=1532)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.4329 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.148 (n=1609)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 5.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` > `0.8801` → IC=+0.262 (n=695)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8801 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.3602` → IC=+0.169 (n=605)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` > 0.3602 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.203` → IC=+0.159 (n=638)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 4.203 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `0.8772` → IC=+0.160 (n=1022)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.8772 (IC base=+0.136)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.173 (n=475)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.1579` → IC=+0.142 (n=1061)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.1579 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.012` → IC=+0.192 (n=186)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 11.012 (IC base=+0.136)
...
- **PATRÓN** `volumen_regimen` > `1.1836` → IC=+0.145 (n=415)
  ...
  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 1.1836 (IC base=+0.136)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4415` → IC=+0.153 (n=1173)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.4415 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `10997.1346` → IC=+0.210 (n=415)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 10997.1346 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `184.0` → IC=+0.143 (n=1168)

  - _Acción_: Kelly boost +0.71€ cuando `ballena_activa_n` < 184.0 (IC base=+0.136)

### GBM_LATE_15M_TARDIO#SOL#15min
- **PATRÓN** `sigma_h` > `0.0079` → IC=+0.132 (n=1015)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0079 (IC base=+0.105)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.167 (n=575)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 17.0 (IC base=+0.105)

- **PATRÓN** `ibs_20min` > `0.4695` → IC=+0.186 (n=1522)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` > 0.4695 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` > `1.047` → IC=+0.196 (n=287)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 1.047 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.408` → IC=+0.229 (n=578)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.408 (IC base=+0.105)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.120 (n=1525)
...
- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.127 (n=1160)

  - _Acción_: Kelly boost +0.64€ cuando `ballena_activa_n` < 54.0 (IC base=+0.105)

- **PATRÓN** `sigma_h` < `0.0109` → IC=+0.133 (n=1495)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` < 0.0109 (IC base=+0.111)

- **PATRÓN** `drift_60min` |x|≤ `0.1278` → IC=+0.151 (n=499)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.1278 (IC base=+0.111)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.124 (n=1551)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.439` → IC=+0.126 (n=1444)
...
- **PATRÓN** `sigma_h` > `0.0187` → IC=+0.213 (n=1014)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0187 (IC base=+0.202)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.207 (n=1591)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.5116` → IC=+0.243 (n=1521)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5116 (IC base=+0.202)

- **PATRÓN** `dist_vwap_pct` > `0.188` → IC=+0.224 (n=875)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.188 (IC base=+0.202)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.173` → IC=+0.265 (n=274)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.173 (IC base=+0.202)

- **PATRÓN** `volumen_regimen` < `1.0718` → IC=+0.203 (n=1338)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.0718 (IC base=+0.202)

- **PATRÓN** `volumen_regimen` > `0.6343` → IC=+0.209 (n=1521)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6343 (IC base=+0.202)

- **PATRÓN** `volumen_pendiente_norm` > `0.2361` → IC=+0.237 (n=268)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2361 (IC base=+0.202)

- **PATRÓN** `volumen_spike_ratio` > `2.5302` → IC=+0.233 (n=488)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5302 (IC base=+0.202)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.209 (n=1569)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.202)

- **PATRÓN** `libro_liquidez` > `2591.9144` → IC=+0.209 (n=1014)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2591.9144 (IC base=+0.202)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.211 (n=818)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5203` → IC=+0.257 (n=1661)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5203 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` < `0.8436` → IC=+0.204 (n=1855)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2836` → IC=+0.253 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2836 (IC base=+0.201)

- **PATRÓN** `volumen_spike_ratio` < `2.2213` → IC=+0.196 (n=1301)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.2213 (IC base=+0.201)

- **PATRÓN** `volumen_spike_ratio` > `1.4437` → IC=+0.201 (n=1479)
...
- **PATRÓN** `libro_liquidez` > `2568.8002` → IC=+0.202 (n=1107)
...
- **PATRÓN** `ibs_20min` > `0.9386` → IC=+0.210 (n=901)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9386 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.1884` → IC=+0.161 (n=949)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.1884 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` < `0.4998` → IC=+0.140 (n=1596)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.4998 (IC base=+0.150)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4594` → IC=+0.155 (n=891)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 1.4594 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.8934` → IC=+0.162 (n=1781)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.8934 (IC base=+0.150)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.150 (n=1785)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.01 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `3624.8897` → IC=+0.155 (n=1801)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.124 (n=735)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 8.0 (IC base=+0.102)

- **PATRÓN** `ibs_20min` > `0.5857` → IC=+0.197 (n=690)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` > 0.5857 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` > `0.3445` → IC=+0.167 (n=262)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.3445 (IC base=+0.102)
...
- **PATRÓN** `volumen_spike_ratio` < `2.116` → IC=+0.155 (n=511)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.116 (IC base=+0.102)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.133 (n=562)

  - _Acción_: Kelly boost +0.66€ cuando `libro_spread` < 0.02 (IC base=+0.102)

- **PATRÓN** `libro_liquidez` > `2435.8886` → IC=+0.151 (n=302)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 2435.8886 (IC base=+0.102)
...
- **PATRÓN** `sigma_h` < `0.005` → IC=+0.178 (n=181)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.005 (IC base=+0.119)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.149 (n=240)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 8.0 (IC base=+0.119)

- **PATRÓN** `ibs_20min` > `0.5578` → IC=+0.225 (n=238)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5578 (IC base=+0.119)

- **PATRÓN** `dist_vwap_pct` > `0.5279` → IC=+0.176 (n=66)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.5279 (IC base=+0.119)
...
- **PATRÓN** `volumen_regimen` < `0.8141` → IC=+0.146 (n=159)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.8141 (IC base=+0.119)

- **PATRÓN** `volumen_regimen` > `0.6396` → IC=+0.146 (n=213)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 0.6396 (IC base=+0.119)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7617` → IC=+0.172 (n=126)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.7617 (IC base=+0.119)

- **PATRÓN** `volumen_spike_ratio` > `1.4015` → IC=+0.153 (n=188)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.4015 (IC base=+0.119)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.142 (n=252)

  - _Acción_: Kelly boost +0.71€ cuando `libro_spread` < 0.02 (IC base=+0.119)

- **PATRÓN** `libro_liquidez` > `1103.6894` → IC=+0.173 (n=209)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 1103.6894 (IC base=+0.119)
...
- **PATRÓN** `ibs_20min` > `0.6923` → IC=+0.187 (n=193)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.6923 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` > `0.1948` → IC=+0.140 (n=123)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.1948 (IC base=+0.074)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.231` → IC=+0.222 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.231 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` > `1.0396` → IC=+0.162 (n=72)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 1.0396 (IC base=+0.074)

- **PATRÓN** `volumen_pendiente_norm` > `0.0865` → IC=+0.192 (n=89)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.0865 (IC base=+0.074)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=7214)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.178 (n=3030)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=9498)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.171 (n=3164)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=9823)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.44` → IC=-0.219 (n=517)

  - _Acción_: SKIP cuando `py_entrada` < 0.44
  - _Potencial_: sin este filtro IC_bueno=+0.101 (n=1639)
...
- **PATRÓN** `py_entrada` > `0.5` → IC=+0.122 (n=1300)
...
- **FILTRO** `py_entrada` < `0.45` → IC=-0.202 (n=544)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.101 (n=1646)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.200 (n=575)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.061 (n=1734)

- **FILTRO** `ibs_20min` > `0.2857` → IC=-0.164 (n=566)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2857
  - _Potencial_: sin este filtro IC_bueno=+0.048 (n=1743)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.475` → IC=-0.191 (n=528)

  - _Acción_: SKIP cuando `py_entrada` < 0.475
  - _Potencial_: sin este filtro IC_bueno=+0.079 (n=1601)

- **FILTRO** `py_entrada` > `0.58` → IC=-0.188 (n=553)

  - _Acción_: SKIP cuando `py_entrada` > 0.58
  - _Potencial_: sin este filtro IC_bueno=+0.055 (n=1758)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=2742)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=2748)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.129 (n=8892)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=20106)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.278 (n=6978)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=22020)

- **FILTRO** `ibs_7min` < `0.2941` → IC=-0.236 (n=7244)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2941
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=21754)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.159 (n=9801)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=19197)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.228 (n=9038)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=27262)

- **FILTRO** `ibs_7min` > `0.2941` → IC=-0.179 (n=9067)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2941
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=27233)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.309 (n=1131)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=3613)

- **FILTRO** `ibs_7min` < `0.7097` → IC=-0.256 (n=1565)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7097
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=3179)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.179 (n=1143)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=3601)

- **FILTRO** `py_entrada` > `0.505` → IC=-0.152 (n=4211)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.102 (n=2056)

- **FILTRO** `drift_7min_pct` |x|> `0.1115` → IC=-0.128 (n=2130)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1115
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=4137)

- **FILTRO** `ibs_7min` > `0.7945` → IC=-0.206 (n=1564)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7945
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=4703)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.137 (n=1167)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.087 (n=3865)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.254 (n=1215)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=3817)

- **FILTRO** `ibs_7min` < `0.7524` → IC=-0.189 (n=1258)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7524
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=3774)

- **FILTRO** `ballena_activa_n` > `161.0` → IC=-0.174 (n=1255)

  - _Acción_: SKIP cuando `ballena_activa_n` > 161.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=3777)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.257 (n=1250)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=3846)

- **FILTRO** `ibs_7min` > `0.2588` → IC=-0.172 (n=1271)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2588
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=3825)

- **FILTRO** `ballena_activa_n` > `154.0` → IC=-0.186 (n=1267)

  - _Acción_: SKIP cuando `ballena_activa_n` > 154.0
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=3829)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.173 (n=1097)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=3390)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.312 (n=1086)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=3401)

- **FILTRO** `ibs_7min` < `0.1972` → IC=-0.259 (n=1121)

  - _Acción_: SKIP cuando `ibs_7min` < 0.1972
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=3366)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.214 (n=1088)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=3399)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.237 (n=1548)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=5102)

- **FILTRO** `ibs_7min` > `0.759` → IC=-0.177 (n=1661)

  - _Acción_: SKIP cuando `ibs_7min` > 0.759
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=4989)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=1518)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.087 (n=3251)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.246 (n=1168)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3601)

- **FILTRO** `ibs_7min` < `0.7428` → IC=-0.181 (n=1192)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7428
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=3577)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.179 (n=1160)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.075 (n=3609)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.261 (n=1218)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=3668)

- **FILTRO** `ibs_7min` > `0.2747` → IC=-0.172 (n=1221)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2747
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=3665)

- **FILTRO** `ballena_activa_n` > `29.0` → IC=-0.187 (n=1220)

  - _Acción_: SKIP cuando `ballena_activa_n` > 29.0
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=3666)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.267 (n=1195)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=3920)

- **FILTRO** `ibs_7min` < `0.7037` → IC=-0.230 (n=1278)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7037
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=3837)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.177 (n=1664)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.019 (n=5221)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.34` → IC=-0.280 (n=1124)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=3727)

- **FILTRO** `ibs_7min` < `0.7097` → IC=-0.232 (n=1212)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7097
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=3639)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.212 (n=1169)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=3682)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.204 (n=1613)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=4903)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=2649)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.154 (n=24)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=1368)

### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` < `0.0042` → IC=+0.188 (n=459)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0042 (IC base=+0.186)

- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.219 (n=624)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0081 (IC base=+0.186)

- **PATRÓN** `drift_60min` |x|≤ `0.1622` → IC=+0.193 (n=1210)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.97€ cuando `drift_60min` |x|≤ 0.1622 (IC base=+0.186)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2175` → IC=+0.188 (n=460)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.94€ cuando `delta_ratio_macro` |x|> 0.2175 (IC base=+0.186)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1317` → IC=+0.232 (n=464)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1317 (IC base=+0.186)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.197 (n=977)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` > `0.6136` → IC=+0.260 (n=1375)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6136 (IC base=+0.186)

- **PATRÓN** `dist_vwap_pct` > `0.4454` → IC=+0.184 (n=324)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.4454 (IC base=+0.186)

- **PATRÓN** `dist_vwap_pct` < `0.5591` → IC=+0.175 (n=1354)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.5591 (IC base=+0.186)
...
- **PATRÓN** `libro_liquidez` > `2964.2504` → IC=+0.193 (n=917)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 2964.2504 (IC base=+0.186)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.943` → IC=+0.167 (n=31)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 8.943 (IC base=+0.021)

### UPDOWN_GBM#XRP#15min
- **PATRÓN** `sigma_h` > `0.0166` → IC=+0.225 (n=249)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0166 (IC base=+0.179)

- **PATRÓN** `drift_60min` |x|≤ `0.085` → IC=+0.195 (n=165)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.97€ cuando `drift_60min` |x|≤ 0.085 (IC base=+0.179)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0626` → IC=+0.188 (n=334)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.94€ cuando `delta_ratio_macro` |x|> 0.0626 (IC base=+0.179)
...
- **PATRÓN** `ibs_15` > `0.5455` → IC=+0.276 (n=373)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5455 (IC base=+0.179)

- **PATRÓN** `dist_vwap_pct` > `0.1228` → IC=+0.196 (n=225)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `16.303` → IC=+0.238 (n=82)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 16.303 (IC base=+0.179)

- **PATRÓN** `libro_liquidez` > `2842.2496` → IC=+0.264 (n=125)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2842.2496 (IC base=+0.179)
...
- **FILTRO** `sigma_h` > `0.013` → IC=-0.212 (n=595)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.013
  - _Potencial_: sin este filtro IC_bueno=-0.007 (n=1789)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.188 (n=770)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=1614)

- **FILTRO** `libro_liquidez` < `3905.3214` → IC=-0.133 (n=1573)
...
- **FILTRO** `sigma_h` > `0.0068` → IC=-0.217 (n=359)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.191 (n=1080)

- **FILTRO** `sigma_h` < `0.0037` → IC=-0.227 (n=474)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0037
  - _Potencial_: sin este filtro IC_bueno=-0.184 (n=965)

- **FILTRO** `hora_utc` > `7.0` → IC=-0.208 (n=913)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 7.0
...
- **FILTRO** `sigma_ewma_delta_pct` > `19.708` → IC=-0.243 (n=255)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.188 (n=1184)
...
- **PATRÓN** `ibs_15` < `0.273` → IC=+0.278 (n=516)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.273 (IC base=+0.224)

- **PATRÓN** `dist_vwap_pct` > `0.727` → IC=+0.276 (n=74)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `12.262` → IC=+0.228 (n=627)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 12.262 (IC base=+0.224)

- **PATRÓN** `libro_liquidez` > `3499.1712` → IC=+0.226 (n=586)
...
- **FILTRO** `sigma_ewma_delta_pct` > `18.047` → IC=-0.130 (n=298)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 18.047
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=2394)
...
- **FILTRO** `sigma_h` > `0.0196` → IC=-0.264 (n=358)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.120 (n=359)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.262 (n=179)

  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.169 (n=538)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1533` → IC=+0.278 (n=133)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1533 (IC base=-0.043)
...
- **PATRÓN** `dist_vwap_pct` > `1.0728` → IC=+0.400 (n=48)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0728 (IC base=-0.043)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6136 sube el IC de +0.186 a +0.260 en UPDOWN_GBM#15min (n=1375). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_YES, IBS > 0.5455 sube el IC de +0.179 a +0.276 en UPDOWN_GBM#XRP#15min (n=373). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.356 sube el IC de -0.039 a +0.280 en UPDOWN_GBM_15M_TARDIO (n=1444). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.273 sube el IC de +0.224 a +0.278 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=516). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#SOL#15min**: dentro de BUY_NO, IBS < 0.3636 sube el IC de -0.044 a +0.257 en UPDOWN_GBM_15M_TARDIO#SOL#15min (n=269). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO#XRP#15min**: dentro de BUY_NO, IBS < 0.35 sube el IC de -0.043 a +0.314 en UPDOWN_GBM_15M_TARDIO#XRP#15min (n=401). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 1229 | +0.090 | +153.74€ | 2 | 7 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 1229 | +0.090 | +153.74€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 26 | +0.036 | -1.50€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 26 | +0.036 | -1.50€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 901 | +0.099 | +130.39€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 901 | +0.099 | +130.39€ | 2 | 8 |
...
| ✅ BALLENAS_TARDIAS | 23075 | -0.093 | -3094.77€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1381 | -0.043 | -203.48€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 21694 | -0.096 | -2891.30€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3517 | -0.088 | -581.38€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3517 | -0.088 | -581.38€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 1381 | -0.043 | -203.48€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 1381 | -0.043 | -203.48€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 374 | -0.136 | -161.05€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 374 | -0.136 | -161.05€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 6606 | -0.034 | -603.30€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 6606 | -0.034 | -603.30€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 6089 | -0.095 | -444.45€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 6089 | -0.095 | -444.45€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 5108 | -0.179 | -1101.12€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 5108 | -0.179 | -1101.12€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 15424 | -0.033 | +4127.30€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 4066 | -0.002 | +1827.73€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 11358 | -0.044 | +2299.57€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 15424 | -0.033 | +4127.30€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 4066 | -0.002 | +1827.73€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min |
66€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 65298 | -0.074 | +1399.66€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 11011 | -0.080 | +615.50€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 11011 | -0.080 | +615.50€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 10128 | -0.093 | -448.99€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 10128 | -0.093 | -448.99€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 11137 | -0.069 | +562.75€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 11137 | -0.069 | +562.75€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 9655 | -0.094 | -169.62€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 9655 | -0.094 | -169.62€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 12000 | -0.049 | +338.84€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 12000 | -0.049 | +338.84€ | 3 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 11367 | -0.063 | +501.19€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 11367 | -0.063 | +501.19€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE | 6896 | -0.024 | -103.43€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 6896 | -0.024 | -103.43€ | 0 | 0 |
...
| ✅ STREAK_MOM_5M | 7178 | +0.024 | +112.51€ | 0 | 0 |
| ✅ STREAK_MOM_5M#5min | 7178 | +0.024 | +112.51€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE | 2121 | +0.021 | +20.90€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE#5min | 2121 | +0.021 | +20.90€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH | 1546 | +0.035 | +46.88€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH#5min | 1546 | +0.035 | +46.88€ | 1 | 0 |
...
| ✅ STRUCT_NO_15M | 6692 | +0.011 | -45.72€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 6692 | +0.011 | -45.72€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 2668 | +0.017 | -5.95€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 2668 | +0.017 | -5.95€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 2632 | +0.012 | -15.57€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 2632 | +0.012 | -15.57€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 1392 | -0.002 | -24.20€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 1392 | -0.002 | -24.20€ | 2 | 0 |
| ✅ UPDOWN_GBM | 31614 | +0.031 | +1875.47€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 8514 | +0.062 | +1476.19€ | 0 | 12 |
| ✅ UPDOWN_GBM#240min | 1166 | +0.004 | +7.79€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 19910 | +0.022 | +376.53€ | 0 | 0 |
| ✅ UPDOWN_GBM#60min | 1901 | +0.005 | +14.81€ | 1 | 0 |
| ✅ UPDOWN_GBM#BNB | 2970 | +0.073 | +333.71€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 462 | +0.153 | +187.47€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#240min | 22 | -0.042 | -1.10€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#5min | 2486 | +0.059 | +147.35€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 5866 | +0.033 | +391.72€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 1058 | +0.079 | +241.95€ | 0 | 11 |
| ✅ UPDOWN_GBM#BTC#240min | 324 | +0.021 | +7.54€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#5min | 3586 | +0.029 | +128.34€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#60min | 851 | +0.002 | +13.42€ | 1 | 0 |
| ✅ UPDOWN_GBM#BTC#daily | 47 | -0.112 | +0.47€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 3737 | +0.039 | +209.62€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 434 | +0.144 | +164.88€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#240min | 18 | +0.000 | -0.04€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 3285 | +0.025 | +44.78€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 6665 | +0.020 | +275.57€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 2237 | +0.044 | +244.73€ | 0 | 11 |
| ✅ UPDOWN_GBM#ETH#240min | 312 | +0.006 | +7.45€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 3419 | +0.010 | +21.77€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 657 | +0.004 | -2.27€ | 3 | 0 |
| ✅ UPDOWN_GBM#ETH#daily | 40 | -0.143 | +3.89€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 7742 | +0.016 | +190.51€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 2168 | +0.021 | +129.77€ | 0 | 12 |
| ✅ UPDOWN_GBM#SOL#240min | 306 | -0.006 | -2.90€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 4841 | +0.016 | +62.36€ | 1 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 393 | +0.014 | +3.66€ | 0 | 1 |
| ✅ UPDOWN_GBM#SOL#daily | 34 | -0.167 | -2.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 4632 | +0.035 | +476.16€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 2155 | +0.077 | +507.39€ | 0 | 11 |
| ✅ UPDOWN_GBM#XRP#240min | 184 | -0.005 | -3.17€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 2293 | -0.002 | -28.06€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 10545 | -0.043 | +2240.03€ | 3 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 10545 | -0.043 | +2240.03€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 626 | -0.045 | +333.01€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 626 | -0.045 | +333.01€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 1972 | -0.121 | +57.13€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 1972 | -0.121 | +57.13€ | 4 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 265 | +0.163 | +157.17€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 265 | +0.163 | +157.17€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 1155 | +0.198 | +650.89€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 1155 | +0.198 | +650.89€ | 2 | 20 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 3260 | -0.065 | +510.04€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 3260 | -0.065 | +510.04€ | 3 | 6 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 3267 | -0.076 | +531.79€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 3267 | -0.076 | +531.79€ | 2 | 4 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.060) — sin ventaja clara. oversold(IBS<0.3): IC=+0.048 n=11083 | neutral: IC=+0.027 n=11963 | overbought(IBS>0.7): IC=+0.087 n=11348
  - _Datos_: n=35671 IC=+0.054 PNL=+4297.14€
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=2168 IC=+0.021 PNL=+129.77€
...
**⏳ H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  ...
  - _Estado_: 31552 ops, 22 horas distintas. Sin hora con n≥15 y IC extremo aún.

**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  - _Umbral_: n≥60 alineadas y gap IC≥0.08 vs contrarias — y descartar que sea proxy de drift_15min/60min
  - _Acción_: Si confirma e independiente de drift → capturar prev_window_outcome como feature en shadow_predict y boost ×1.1-1.2 en señales alineadas
  - _Estado_: alineada_con_outcome_prev IC=+0.098 n=239/60 | contraria IC=+0.148 n=228 | gap=-0.050 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n