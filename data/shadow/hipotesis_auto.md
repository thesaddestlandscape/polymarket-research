# Hipótesis automáticas — 2026-07-21 14:04 UTC
_Generado por shadow_postmortem.py sobre 26871 resoluciones (PNL=+6242.40€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.206 (n=1493)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.203)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.209 (n=1716)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.203)

- **PATRÓN** `py_entrada` > `0.695` → IC=+0.316 (n=773)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.203)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.206 (n=1887)
...
- **PATRÓN** `py_entrada` < `0.265` → IC=+0.350 (n=178)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.239 (n=328)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.226)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.233 (n=376)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.230 (n=458)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.226)

- **PATRÓN** `libro_liquidez` > `4261.2218` → IC=+0.227 (n=240)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4261.2218 (IC base=+0.226)
...
- **PATRÓN** `py_entrada` < `0.235` → IC=+0.375 (n=134)
...
- **PATRÓN** `py_entrada` < `0.225` → IC=+0.347 (n=129)
...
- **PATRÓN** `sigma_h` > `0.0133` → IC=+0.130 (n=830)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` > 0.0133 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` > `0.9382` → IC=+0.129 (n=149)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` > 0.9382 (IC base=+0.102)
...
- **PATRÓN** `dist_vwap_pct` > `1.0964` → IC=+0.145 (n=29)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 1.0964 (IC base=+0.069)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.447` → IC=+0.173 (n=99)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 7.447 (IC base=+0.069)

### GBM_LATE_15M#SOL#15min
- **FILTRO** `sigma_ewma_delta_pct` > `7.945` → IC=-0.326 (n=21)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 7.945
  - _Potencial_: sin este filtro IC_bueno=+0.084 (n=337)
...
- **PATRÓN** `sigma_h` > `0.0263` → IC=+0.176 (n=223)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0263 (IC base=+0.162)
...
- **PATRÓN** `dist_vwap_pct` > `0.3458` → IC=+0.222 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3458 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.099` → IC=+0.151 (n=416)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` < 9.099 (IC base=+0.162)
...
- **PATRÓN** `sigma_h` > `0.0073` → IC=+0.174 (n=893)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0073 (IC base=+0.143)
...
- **PATRÓN** `dist_vwap_pct` > `0.1749` → IC=+0.152 (n=363)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.1749 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` < `0.4287` → IC=+0.153 (n=1192)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.4287 (IC base=+0.143)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.158` → IC=+0.160 (n=254)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 6.158 (IC base=+0.143)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.157 (n=164)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 15.0 (IC base=+0.102)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.461` → IC=+0.164 (n=123)
...
- **PATRÓN** `dist_vwap_pct` > `0.6406` → IC=+0.158 (n=74)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.6406 (IC base=+0.100)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.557` → IC=+0.182 (n=127)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 4.557 (IC base=+0.100)
...
- **PATRÓN** `sigma_h` < `0.0108` → IC=+0.140 (n=276)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0108 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` < `0.7822` → IC=+0.135 (n=275)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.7822 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.398` → IC=+0.154 (n=232)
...
- **PATRÓN** `sigma_h` < `0.0172` → IC=+0.277 (n=338)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0172 (IC base=+0.263)

- **PATRÓN** `sigma_h` > `0.0079` → IC=+0.277 (n=339)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0079 (IC base=+0.263)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.321 (n=115)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.263)

- **PATRÓN** `dist_vwap_pct` > `0.1917` → IC=+0.348 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1917 (IC base=+0.263)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.429` → IC=+0.266 (n=348)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 9.429 (IC base=+0.263)
...
- **PATRÓN** `sigma_h` > `0.0234` → IC=+0.208 (n=142)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.3099` → IC=+0.256 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3099 (IC base=+0.195)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.054` → IC=+0.203 (n=402)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 10.054 (IC base=+0.195)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.130 (n=71)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 12.0 (IC base=+0.086)

- **PATRÓN** `ibs_15` < `0.1818` → IC=+0.129 (n=87)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.65€ cuando `ibs_15` < 0.1818 (IC base=+0.086)
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 593 | +0.122 | +13.24€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 593 | +0.122 | +13.24€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 221 | +0.128 | +0.82€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 221 | +0.128 | +0.82€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 233 | +0.134 | +2.43€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 233 | +0.134 | +2.43€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 137 | +0.083 | +8.89€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 137 | +0.083 | +8.89€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 4636 | +0.185 | +67.77€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 3020 | +0.216 | +102.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 199 | +0.072 | -2.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 619 | +0.107 | -80.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 798 | +0.155 | +48.28€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 1348 | +0.197 | +54.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1006 | +0.210 | +3.66€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 1639 | +0.175 | -1.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 994 | +0.216 | +38.35€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 1625 | +0.185 | +13.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1000 | +0.225 | +62.52€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 67 | +0.123 | +6.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 292 | +0.109 | -43.39€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 6527 | +0.099 | +2114.07€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 6527 | +0.099 | +2114.07€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1560 | +0.070 | +280.32€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1560 | +0.070 | +280.32€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1482 | +0.072 | +260.53€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1482 | +0.072 | +260.53€ | 0 | 6 |
| ✅ GBM_LATE_15M#SOL | 1767 | +0.090 | +632.32€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1767 | +0.090 | +632.32€ | 1 | 3 |
| ✅ GBM_LATE_15M#XRP | 1718 | +0.159 | +940.91€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1718 | +0.159 | +940.91€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 3723 | +0.136 | +2040.97€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 3723 | +0.136 | +2040.97€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 948 | +0.097 | +394.29€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 948 | +0.097 | +394.29€ | 0 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 971 | +0.095 | +382.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 971 | +0.095 | +382.84€ | 0 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 945 | +0.106 | +411.62€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 945 | +0.106 | +411.62€ | 0 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 859 | +0.257 | +852.23€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 859 | +0.257 | +852.23€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 3697 | +0.088 | +1214.08€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 3697 | +0.088 | +1214.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 886 | +0.053 | +158.27€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 886 | +0.053 | +158.27€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 893 | +0.029 | +79.51€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 893 | +0.029 | +79.51€ | 0 | 2 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 842 | +0.047 | +177.04€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 842 | +0.047 | +177.04€ | 0 | 1 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 1076 | +0.199 | +799.26€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1076 | +0.199 | +799.26€ | 0 | 13 |
| ✅ GBM_LATE_5M | 251 | +0.010 | +18.83€ | 0 | 0 |
| ✅ GBM_LATE_5M#5min | 251 | +0.010 | +18.83€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC | 33 | -0.043 | -4.82€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC#5min | 33 | -0.043 | -4.82€ | 0 | 0 |
| ✅ GBM_LATE_5M#ETH | 29 | -0.048 | +0.85€ | 0 | 0 |
| ✅ GBM_LATE_5M#ETH#5min | 29 | -0.048 | +0.85€ | 0 | 0 |
| ✅ GBM_LATE_5M#SOL | 58 | -0.133 | +2.91€ | 0 | 0 |
| ✅ GBM_LATE_5M#SOL#5min | 58 | -0.133 | +2.91€ | 0 | 0 |
...
| ✅ LATE_WINDOW_5MIN | 207 | -0.002 | +5.73€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN#5min | 207 | -0.002 | +5.73€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN#BTC | 207 | -0.002 | +5.73€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN#BTC#5min | 207 | -0.002 | +5.73€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M | 250 | +0.016 | +11.79€ | 2 | 1 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 250 | +0.016 | +11.79€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 250 | +0.016 | +11.79€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 250 | +0.016 | +11.79€ | 2 | 1 |
...
| ✅ UPDOWN_GBM | 2056 | +0.027 | +169.49€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1794 | +0.050 | +208.56€ | 0 | 4 |
...
| ✅ UPDOWN_GBM#XRP | 183 | +0.035 | +30.56€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 172 | +0.052 | +33.85€ | 3 | 2 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 981 | +0.168 | +449.46€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 981 | +0.168 | +449.46€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 95 | +0.180 | +46.95€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 95 | +0.180 | +46.95€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 117 | +0.139 | +24.49€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 117 | +0.139 | +24.49€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 76 | +0.244 | +53.57€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 76 | +0.244 | +53.57€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 210 | +0.160 | +64.25€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 210 | +0.160 | +64.25€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 241 | +0.088 | +67.07€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 241 | +0.088 | +67.07€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 242 | +0.234 | +193.13€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 242 | +0.234 | +193.13€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.196: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.057 n=932 | neutral: IC=-0.001 n=715 | overbought(IBS>0.7): IC=+0.196 n=928
  - _Datos_: n=2757 IC=+0.097 PNL=+662.76€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.116 n=16078 | tras_1loss IC=+0.082 n=10586 | tras_2loss IC=+0.048 n=4390/40 | gap=+0.068 (umbral 0.05)
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=+0.088 n=1196/40 PNL=+255.63€ | H=15h UTC: IC=+0.156 n=1097/40 PNL=+453.48€ | H=17h UTC: IC=+0.101 n=1063/40 PNL=+233.09€ | H=19h UTC: IC=+0.122 n=993/40 PNL=+261.58€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1924 IC=+0.033 PNL=+184.23€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1924 IC=+0.033 PNL=+184.23€
...
**〰️ H-LATE-WINDOW-5MIN** — Late-window BTC 5min — arbitraje timing vs Polymarket
  ...
  - _Estado_: n=207 IC=-0.002 PNL=+5.73€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=207 IC=-0.002 PNL=+5.73€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=895 IC=+0.038 PNL=+74.31€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=895 IC=+0.038 PNL=+74.31€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=24406 IC=+0.114 PNL=+6317.08€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=24406 IC=+0.114 PNL=+6317.08€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=143 IC=+0.086 PNL=+37.21€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=143 IC=+0.086 PNL=+37.21€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=848 IC=+0.053 PNL=+107.49€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=848 IC=+0.053 PNL=+107.49€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.111 > 0.08 con n=366 PNL=+109.88€
  - _Datos_: n=366 IC=+0.111 PNL=+109.88€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.123 > 0.08 con n=383 PNL=+152.55€
  - _Datos_: n=383 IC=+0.123 PNL=+152.55€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.149 > 0.08 con n=460 PNL=+273.83€
  - _Datos_: n=460 IC=+0.149 PNL=+273.83€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.285 > 0.08 con n=352 PNL=+11.63€
  - _Datos_: n=352 IC=+0.285 PNL=+11.63€
