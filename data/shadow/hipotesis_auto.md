# Hipótesis automáticas — 2026-08-30 01:41 UTC
_Generado por shadow_postmortem.py sobre 208572 resoluciones (PNL=+16068.83€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `147.78` → IC=-0.292 (n=2917)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 147.78
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=8752)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `138.82` → IC=-0.265 (n=376)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 138.82
  - _Potencial_: sin este filtro IC_bueno=+0.014 (n=1131)
...
- **FILTRO** `restante_s_al_confirmar` < `121.42` → IC=-0.381 (n=377)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 121.42
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=1132)

### BALLENAS_TARDIAS#ETH#5min
- **FILTRO** `restante_s_al_confirmar` < `158.48` → IC=-0.166 (n=779)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.48
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=2340)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `139.97` → IC=-0.323 (n=674)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 139.97
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=2023)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `156.92` → IC=-0.358 (n=665)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 156.92
  - _Potencial_: sin este filtro IC_bueno=-0.082 (n=1353)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.192 (n=6238)

  - _Acción_: Kelly boost +0.96€ cuando `py_entrada` > 0.7 (IC base=+0.097)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.173 (n=1748)

  - _Acción_: Kelly boost +0.86€ cuando `libro_spread` < 0.01 (IC base=+0.097)

- **PATRÓN** `libro_liquidez` > `2375.6867` → IC=+0.174 (n=1680)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.207 (n=725)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.210 (n=912)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.201 (n=726)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.189 (n=934)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.01 (IC base=+0.189)

- **PATRÓN** `libro_liquidez` > `13171.6675` → IC=+0.224 (n=241)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.194 (n=194)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `4063.9233` → IC=+0.150 (n=301)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.128 (n=1158)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.279 (n=545)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `2172.4252` → IC=+0.282 (n=465)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2172.4252 (IC base=+0.277)
...
- **PATRÓN** `py_entrada` > `0.84` → IC=+0.439 (n=359)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.192 (n=4333)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 15.0 (IC base=+0.187)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.202 (n=2452)
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.177 (n=1344)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.308 (n=431)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.184 (n=1072)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.218 (n=10806)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.163 (n=1837)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.269 (n=1719)
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.216 (n=862)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.285 (n=832)
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.244 (n=1110)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.140 (n=1330)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.735` → IC=+0.343 (n=336)
...
- **PATRÓN** `libro_liquidez` > `2113.4428` → IC=+0.316 (n=123)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2113.4428 (IC base=+0.282)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9516` → IC=+0.211 (n=900)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9516 (IC base=+0.070)

- **PATRÓN** `dist_vwap_pct` > `0.186` → IC=+0.223 (n=363)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.186 (IC base=+0.070)

- **PATRÓN** `dist_vwap_pct` < `0.7242` → IC=+0.215 (n=601)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.7242 (IC base=+0.070)
...
- **PATRÓN** `volumen_regimen` > `1.067` → IC=+0.236 (n=267)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.067 (IC base=+0.070)

- **PATRÓN** `volumen_pendiente_norm` > `0.2464` → IC=+0.160 (n=310)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` > 0.2464 (IC base=+0.070)

- **PATRÓN** `volumen_spike_ratio` < `2.4264` → IC=+0.158 (n=1337)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.4264 (IC base=+0.070)

- **PATRÓN** `volumen_spike_ratio` > `1.9516` → IC=+0.153 (n=1013)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.9516 (IC base=+0.070)

- **PATRÓN** `ibs_20min` < `0.1159` → IC=+0.152 (n=1359)

  - _Acción_: Kelly boost +0.76€ cuando `ibs_20min` < 0.1159 (IC base=+0.029)

- **PATRÓN** `dist_vwap_pct` < `0.1781` → IC=+0.143 (n=883)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.1781 (IC base=+0.029)

- **PATRÓN** `volumen_regimen` < `0.618` → IC=+0.158 (n=299)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.618 (IC base=+0.029)

- **PATRÓN** `volumen_regimen` > `1.0409` → IC=+0.138 (n=407)
...
- **PATRÓN** `ballena_activa_n` < `72.0` → IC=+0.213 (n=590)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.183 (n=216)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 6.0 (IC base=+0.137)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.288 (n=220)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4339` → IC=+0.123 (n=507)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_spike_ratio` > 1.4339 (IC base=+0.137)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.174 (n=424)

  - _Acción_: Kelly boost +0.87€ cuando `libro_spread` < 0.04 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.165 (n=219)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 48.0 (IC base=+0.137)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.263 (n=243)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.268 (n=369)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.962` → IC=+0.280 (n=389)
...
- **PATRÓN** `volumen_spike_ratio` < `1.9291` → IC=+0.259 (n=172)
...
- **PATRÓN** `libro_liquidez` > `1933.4502` → IC=+0.290 (n=122)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1933.4502 (IC base=+0.258)
...
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.245 (n=155)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.0888` → IC=+0.233 (n=155)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0888 (IC base=+0.207)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.230 (n=476)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.207)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.210 (n=487)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.207)

- **PATRÓN** `ibs_20min` > `0.399` → IC=+0.224 (n=465)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.399 (IC base=+0.207)

- **PATRÓN** `dist_vwap_pct` > `0.2115` → IC=+0.225 (n=293)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2115 (IC base=+0.207)

- **PATRÓN** `dist_vwap_pct` < `0.5455` → IC=+0.207 (n=415)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5455 (IC base=+0.207)
...
- **PATRÓN** `volumen_regimen` < `0.6892` → IC=+0.220 (n=205)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6892 (IC base=+0.207)

- **PATRÓN** `volumen_regimen` > `1.0803` → IC=+0.232 (n=211)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0803 (IC base=+0.207)

- **PATRÓN** `volumen_pendiente_norm` < `0.0994` → IC=+0.213 (n=434)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4577` → IC=+0.255 (n=194)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4577 (IC base=+0.207)

- **PATRÓN** `libro_liquidez` > `14063.1806` → IC=+0.215 (n=310)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 14063.1806 (IC base=+0.207)

- **PATRÓN** `ballena_activa_n` < `398.0` → IC=+0.205 (n=391)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 398.0 (IC base=+0.207)
...
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.220 (n=184)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0083 (IC base=+0.154)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.222 (n=196)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1324` → IC=+0.147 (n=449)
...
- **PATRÓN** `volumen_spike_ratio` < `3.34` → IC=+0.144 (n=414)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 3.34 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` > `1.6927` → IC=+0.157 (n=470)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.6927 (IC base=+0.154)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.182 (n=524)

  - _Acción_: Kelly boost +0.91€ cuando `libro_spread` < 0.04 (IC base=+0.154)

- **PATRÓN** `ballena_activa_n` < `43.0` → IC=+0.189 (n=242)

  - _Acción_: Kelly boost +0.94€ cuando `ballena_activa_n` < 43.0 (IC base=+0.154)

- **PATRÓN** `sigma_h` < `0.0094` → IC=+0.261 (n=421)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0094 (IC base=+0.246)

- **PATRÓN** `drift_60min` |x|≤ `0.4476` → IC=+0.249 (n=421)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4476 (IC base=+0.246)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.247 (n=291)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.246)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.257 (n=435)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.246)

- **PATRÓN** `ibs_20min` < `0.5185` → IC=+0.282 (n=421)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5185 (IC base=+0.246)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.131` → IC=+0.265 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.131 (IC base=+0.246)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.907` → IC=+0.252 (n=489)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6358` → IC=+0.246 (n=199)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6358 (IC base=+0.246)
...
- **PATRÓN** `libro_liquidez` > `1876.5391` → IC=+0.255 (n=141)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1876.5391 (IC base=+0.246)

- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.228 (n=222)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5804` → IC=+0.139 (n=106)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 1.5804 (IC base=-0.048)
...
- **FILTRO** `sigma_h` > `0.0104` → IC=-0.136 (n=363)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=1090)

- **FILTRO** `sigma_ewma_delta_pct` > `5.019` → IC=-0.164 (n=313)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 5.019
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=1140)
...
- **FILTRO** `ibs_20min` > `0.7937` → IC=-0.183 (n=329)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=988)
...
- **PATRÓN** `dist_vwap_pct` < `0.1309` → IC=+0.220 (n=155)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1309 (IC base=-0.053)

- **PATRÓN** `volumen_regimen` > `1.3639` → IC=+0.260 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.3639 (IC base=-0.053)

- **PATRÓN** `volumen_pendiente_norm` > `0.088` → IC=+0.214 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.088 (IC base=-0.053)

- **PATRÓN** `volumen_spike_ratio` < `2.4934` → IC=+0.141 (n=37)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 2.4934 (IC base=-0.053)
...
- **PATRÓN** `ballena_activa_n` < `20.0` → IC=+0.167 (n=25)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 20.0 (IC base=-0.053)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.136 (n=1550)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0076 (IC base=+0.056)

- **PATRÓN** `ibs_20min` > `0.2759` → IC=+0.122 (n=3417)

  - _Acción_: Kelly boost +0.61€ cuando `ibs_20min` > 0.2759 (IC base=+0.056)

- **PATRÓN** `dist_vwap_pct` > `1.2567` → IC=+0.295 (n=256)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2567 (IC base=+0.056)

- **PATRÓN** `volumen_regimen` > `1.164` → IC=+0.218 (n=378)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.164 (IC base=+0.056)

- **PATRÓN** `volumen_pendiente_norm` < `0.1153` → IC=+0.185 (n=1535)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` < 0.1153 (IC base=+0.056)

- **PATRÓN** `volumen_pendiente_norm` > `0.2513` → IC=+0.203 (n=294)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2513 (IC base=+0.056)

- **PATRÓN** `volumen_spike_ratio` < `1.4956` → IC=+0.210 (n=526)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4956 (IC base=+0.056)

- **PATRÓN** `volumen_spike_ratio` > `2.8672` → IC=+0.187 (n=525)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 2.8672 (IC base=+0.056)

- **PATRÓN** `ballena_activa_n` < `92.0` → IC=+0.279 (n=1026)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 92.0 (IC base=+0.056)

- **PATRÓN** `ibs_20min` < `0.0909` → IC=+0.194 (n=1343)
...
- **PATRÓN** `dist_vwap_pct` < `0.223` → IC=+0.216 (n=830)
...
- **PATRÓN** `volumen_regimen` > `1.221` → IC=+0.232 (n=282)
...
- **PATRÓN** `volumen_spike_ratio` > `2.9156` → IC=+0.287 (n=271)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9156 (IC base=+0.039)

- **PATRÓN** `ballena_activa_n` < `59.0` → IC=+0.268 (n=654)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 59.0 (IC base=+0.039)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `ibs_20min` < `0.2584` → IC=-0.155 (n=256)
...
- **FILTRO** `ibs_20min` > `0.8809` → IC=-0.158 (n=308)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0788` → IC=+0.223 (n=182)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.260 (n=194)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.201)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.287 (n=266)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2343` → IC=+0.314 (n=234)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2343 (IC base=+0.322)

- **PATRÓN** `volumen_pendiente_norm` > `0.3655` → IC=+0.350 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3655 (IC base=+0.322)

- **PATRÓN** `volumen_spike_ratio` < `1.7028` → IC=+0.325 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7028 (IC base=+0.322)

- **PATRÓN** `volumen_spike_ratio` > `2.4717` → IC=+0.333 (n=154)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4717 (IC base=+0.322)
...
- **PATRÓN** `libro_liquidez` > `1860.9977` → IC=+0.354 (n=108)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1860.9977 (IC base=+0.322)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.171 (n=174)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
...
- **FILTRO** `ibs_20min` > `0.7391` → IC=-0.134 (n=364)
...
- **FILTRO** `ibs_20min` < `0.7561` → IC=-0.134 (n=498)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.235 (n=300)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.229)

- **PATRÓN** `ibs_20min` > `0.8902` → IC=+0.309 (n=418)
...
- **PATRÓN** `volumen_regimen` > `0.8361` → IC=+0.267 (n=418)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.238 (n=627)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.229)

- **PATRÓN** `libro_liquidez` > `2481.7809` → IC=+0.237 (n=560)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2481.7809 (IC base=+0.229)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.278 (n=219)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.2715` → IC=+0.286 (n=623)
...
- **PATRÓN** `sigma_h` > `0.0102` → IC=+0.197 (n=975)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0102 (IC base=+0.161)

- **PATRÓN** `drift_60min` |x|≤ `0.0852` → IC=+0.168 (n=976)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.283 (n=1402)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.46` → IC=+0.245 (n=1204)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.46 (IC base=+0.161)

- **PATRÓN** `volumen_regimen` > `0.6232` → IC=+0.172 (n=2020)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3185` → IC=+0.163 (n=2346)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.3185 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` > `1.8684` → IC=+0.155 (n=1777)
...
- **PATRÓN** `ballena_activa_n` < `162.0` → IC=+0.179 (n=1832)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0785` → IC=+0.214 (n=879)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0785 (IC base=+0.183)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.197 (n=1243)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 15.0 (IC base=+0.183)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.188 (n=984)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` < 5.0 (IC base=+0.183)

- **PATRÓN** `ibs_20min` < `0.4122` → IC=+0.237 (n=2635)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4122 (IC base=+0.183)

- **PATRÓN** `dist_vwap_pct` < `0.2217` → IC=+0.180 (n=2111)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.2217 (IC base=+0.183)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.93` → IC=+0.208 (n=491)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.93 (IC base=+0.183)

- **PATRÓN** `volumen_regimen` < `1.1669` → IC=+0.168 (n=2058)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.1669 (IC base=+0.183)

- **PATRÓN** `volumen_regimen` > `0.6178` → IC=+0.167 (n=2058)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 0.6178 (IC base=+0.183)

- **PATRÓN** `volumen_pendiente_norm` > `0.2966` → IC=+0.238 (n=326)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2966 (IC base=+0.183)

- **PATRÓN** `volumen_spike_ratio` < `1.9175` → IC=+0.177 (n=1346)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 1.9175 (IC base=+0.183)

- **PATRÓN** `volumen_spike_ratio` > `2.7079` → IC=+0.207 (n=673)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7079 (IC base=+0.183)

- **PATRÓN** `ballena_activa_n` < `110.0` → IC=+0.185 (n=1406)

  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 110.0 (IC base=+0.183)
...
- **PATRÓN** `libro_liquidez` > `1926.5102` → IC=+0.313 (n=89)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1926.5102 (IC base=+0.251)

- **PATRÓN** `ballena_activa_n` < `76.0` → IC=+0.237 (n=158)
...
- **PATRÓN** `sigma_h` < `0.0098` → IC=+0.264 (n=312)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0098 (IC base=+0.248)

- **PATRÓN** `drift_60min` |x|≤ `0.217` → IC=+0.286 (n=208)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.217 (IC base=+0.248)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.274 (n=215)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.248)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.248 (n=284)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 16.0 (IC base=+0.248)

- **PATRÓN** `ibs_20min` < `0.3889` → IC=+0.287 (n=312)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3889 (IC base=+0.248)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.973` → IC=+0.280 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.973 (IC base=+0.248)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.211` → IC=+0.251 (n=347)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.211 (IC base=+0.248)

- **PATRÓN** `volumen_pendiente_norm` > `0.3747` → IC=+0.318 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3747 (IC base=+0.248)

- **PATRÓN** `volumen_spike_ratio` > `3.1577` → IC=+0.264 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.1577 (IC base=+0.248)

- **PATRÓN** `libro_liquidez` > `1880.0598` → IC=+0.264 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1880.0598 (IC base=+0.248)

- **PATRÓN** `ballena_activa_n` < `59.0` → IC=+0.211 (n=195)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3741` → IC=+0.159 (n=494)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.3741 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.177 (n=190)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 17.0 (IC base=+0.142)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.156 (n=184)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 5.0 (IC base=+0.142)
...
- **PATRÓN** `volumen_regimen` < `1.1652` → IC=+0.155 (n=494)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.1652 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` > `0.604` → IC=+0.149 (n=494)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 0.604 (IC base=+0.142)

- **PATRÓN** `volumen_pendiente_norm` > `0.1044` → IC=+0.162 (n=155)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` > 0.1044 (IC base=+0.142)
...
- **PATRÓN** `volumen_spike_ratio` > `2.581` → IC=+0.149 (n=129)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 2.581 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `11127.6873` → IC=+0.147 (n=165)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 11127.6873 (IC base=+0.142)

- **PATRÓN** `ballena_activa_n` < `112.0` → IC=+0.150 (n=135)

  - _Acción_: Kelly boost +0.75€ cuando `ballena_activa_n` < 112.0 (IC base=+0.142)

### GBM_LATE_15M_MULTIHORIZONTE#SOL#15min
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.172 (n=382)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.5455` → IC=+0.188 (n=574)
...
- **PATRÓN** `volumen_regimen` > `0.6201` → IC=+0.137 (n=573)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 0.6201 (IC base=+0.119)

- **PATRÓN** `volumen_pendiente_norm` < `0.1612` → IC=+0.135 (n=565)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_pendiente_norm` < 0.1612 (IC base=+0.119)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5608` → IC=+0.126 (n=482)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_spike_ratio` > 1.5608 (IC base=+0.119)
...
- **PATRÓN** `dist_vwap_pct` > `0.9777` → IC=+0.151 (n=61)
  ...
  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.9777 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` < `0.2424` → IC=+0.143 (n=452)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.2424 (IC base=+0.128)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.296` → IC=+0.192 (n=183)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 3.296 (IC base=+0.128)

- **PATRÓN** `volumen_regimen` < `1.1545` → IC=+0.137 (n=475)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` < 1.1545 (IC base=+0.128)

- **PATRÓN** `volumen_regimen` > `0.8432` → IC=+0.164 (n=316)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` > 0.8432 (IC base=+0.128)

- **PATRÓN** `volumen_pendiente_norm` > `0.2176` → IC=+0.191 (n=66)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.2176 (IC base=+0.128)

- **PATRÓN** `volumen_spike_ratio` < `1.4538` → IC=+0.141 (n=115)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.4538 (IC base=+0.128)

- **PATRÓN** `volumen_spike_ratio` > `2.285` → IC=+0.198 (n=114)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.285 (IC base=+0.128)

- **PATRÓN** `libro_liquidez` > `2178.9306` → IC=+0.173 (n=316)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 2178.9306 (IC base=+0.128)

### GBM_LATE_15M_MULTIHORIZONTE#XRP#15min
- **PATRÓN** `sigma_h` > `0.0245` → IC=+0.203 (n=274)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0245 (IC base=+0.177)
...
- **PATRÓN** `ibs_20min` > `0.895` → IC=+0.265 (n=403)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.895 (IC base=+0.177)

- **PATRÓN** `dist_vwap_pct` > `0.9852` → IC=+0.240 (n=190)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9852 (IC base=+0.177)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.298` → IC=+0.247 (n=243)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.298 (IC base=+0.177)

- **PATRÓN** `volumen_regimen` > `0.8237` → IC=+0.201 (n=403)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5479` → IC=+0.197 (n=565)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.5479 (IC base=+0.177)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.181 (n=722)
...
- **PATRÓN** `drift_60min` |x|≤ `0.6622` → IC=+0.231 (n=577)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.6622 (IC base=+0.221)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.261 (n=270)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.221)

- **PATRÓN** `ibs_20min` < `0.383` → IC=+0.262 (n=577)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.383 (IC base=+0.221)

- **PATRÓN** `dist_vwap_pct` < `0.2494` → IC=+0.231 (n=611)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2494 (IC base=+0.221)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.402` → IC=+0.275 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.402 (IC base=+0.221)

- **PATRÓN** `volumen_regimen` > `0.6911` → IC=+0.243 (n=516)
...
- **PATRÓN** `volumen_spike_ratio` > `2.7671` → IC=+0.268 (n=149)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7671 (IC base=+0.221)

### GBM_LATE_15M_PYCONFIRMADO
- **PATRÓN** `sigma_h` > `0.006` → IC=+0.155 (n=384)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.6207` → IC=+0.177 (n=512)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.14` → IC=+0.190 (n=156)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 8.14 (IC base=+0.121)

- **PATRÓN** `volumen_regimen` < `0.8887` → IC=+0.135 (n=305)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 0.8887 (IC base=+0.121)

- **PATRÓN** `volumen_regimen` > `1.1727` → IC=+0.143 (n=152)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 1.1727 (IC base=+0.121)

- **PATRÓN** `volumen_pendiente_norm` > `0.2977` → IC=+0.202 (n=82)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2977 (IC base=+0.121)

- **PATRÓN** `volumen_spike_ratio` > `1.5952` → IC=+0.131 (n=464)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` > 1.5952 (IC base=+0.121)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.128 (n=589)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.02 (IC base=+0.121)

- **PATRÓN** `libro_liquidez` > `3047.4604` → IC=+0.137 (n=260)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 3047.4604 (IC base=+0.121)

- **PATRÓN** `ballena_activa_n` < `14.0` → IC=+0.175 (n=121)

  - _Acción_: Kelly boost +0.87€ cuando `ballena_activa_n` < 14.0 (IC base=+0.121)

- **PATRÓN** `ibs_20min` < `0.3158` → IC=+0.129 (n=375)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` < 0.3158 (IC base=+0.053)

- **PATRÓN** `ballena_activa_n` < `19.0` → IC=+0.158 (n=179)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 19.0 (IC base=+0.053)
...
- **FILTRO** `volumen_spike_ratio` > `2.4132` → IC=-0.194 (n=34)

  - _Acción_: SKIP cuando `volumen_spike_ratio` > 2.4132
...
- **PATRÓN** `sigma_h` > `0.0122` → IC=+0.211 (n=50)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0122 (IC base=+0.133)

- **PATRÓN** `drift_60min` |x|≤ `0.2864` → IC=+0.143 (n=96)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.2864 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.158 (n=109)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 7.0 (IC base=+0.133)

- **PATRÓN** `ibs_20min` > `0.7477` → IC=+0.167 (n=73)

  - _Acción_: Kelly boost +0.83€ cuando `ibs_20min` > 0.7477 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` > `0.2104` → IC=+0.203 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2104 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.718` → IC=+0.182 (n=20)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 9.718 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.385` → IC=+0.160 (n=95)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` < 3.385 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` > `0.6571` → IC=+0.170 (n=98)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 0.6571 (IC base=+0.133)

- **PATRÓN** `volumen_pendiente_norm` < `0.093` → IC=+0.146 (n=80)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_pendiente_norm` < 0.093 (IC base=+0.133)

- **PATRÓN** `volumen_pendiente_norm` > `0.2233` → IC=+0.180 (n=23)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.2233 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` < `1.935` → IC=+0.161 (n=57)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 1.935 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` > `1.6215` → IC=+0.167 (n=76)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.6215 (IC base=+0.133)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.153 (n=119)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.02 (IC base=+0.133)

- **PATRÓN** `ballena_activa_n` < `8.0` → IC=+0.250 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 8.0 (IC base=+0.133)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.197 (n=1544)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0082 (IC base=+0.154)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.164 (n=3461)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 6.0 (IC base=+0.154)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.295 (n=1245)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.154)

- **PATRÓN** `dist_vwap_pct` > `1.0684` → IC=+0.238 (n=514)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0684 (IC base=+0.154)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.544` → IC=+0.228 (n=1961)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.544 (IC base=+0.154)

- **PATRÓN** `volumen_regimen` < `0.6973` → IC=+0.158 (n=1055)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.6973 (IC base=+0.154)

- **PATRÓN** `volumen_regimen` > `1.0771` → IC=+0.150 (n=1085)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 1.0771 (IC base=+0.154)

- **PATRÓN** `volumen_pendiente_norm` > `0.1024` → IC=+0.177 (n=1180)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.1024 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` < `2.3229` → IC=+0.155 (n=2711)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.3229 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` > `1.8761` → IC=+0.163 (n=2054)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.8761 (IC base=+0.154)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.160 (n=2706)

  - _Acción_: Kelly boost +0.80€ cuando `libro_spread` < 0.01 (IC base=+0.154)

- **PATRÓN** `libro_liquidez` > `3124.4778` → IC=+0.185 (n=1542)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 3124.4778 (IC base=+0.154)

- **PATRÓN** `ballena_activa_n` < `65.0` → IC=+0.205 (n=1356)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 65.0 (IC base=+0.154)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.211 (n=2085)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0066 (IC base=+0.187)

- **PATRÓN** `drift_60min` |x|≤ `0.4726` → IC=+0.196 (n=3124)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.4726 (IC base=+0.187)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.206 (n=1117)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.187)

- **PATRÓN** `ibs_20min` < `0.5591` → IC=+0.241 (n=3124)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5591 (IC base=+0.187)

- **PATRÓN** `dist_vwap_pct` < `0.4295` → IC=+0.179 (n=2281)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.4295 (IC base=+0.187)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.543` → IC=+0.193 (n=3062)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3408` → IC=+0.199 (n=1097)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.3408 (IC base=+0.187)

- **PATRÓN** `ballena_activa_n` < `37.0` → IC=+0.174 (n=885)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8937` → IC=+0.165 (n=314)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.270 (n=376)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0073 (IC base=+0.263)

- **PATRÓN** `sigma_h` > `0.0047` → IC=+0.267 (n=375)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.272 (n=380)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.158` → IC=+0.283 (n=403)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.178` → IC=+0.314 (n=84)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.178 (IC base=+0.263)

- **PATRÓN** `volumen_spike_ratio` > `2.42` → IC=+0.309 (n=124)
...
- **PATRÓN** `libro_liquidez` > `1939.2468` → IC=+0.287 (n=125)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1939.2468 (IC base=+0.263)

- **PATRÓN** `ballena_activa_n` < `69.0` → IC=+0.247 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 69.0 (IC base=+0.263)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.192 (n=183)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0028 (IC base=+0.158)

- **PATRÓN** `sigma_h` > `0.0032` → IC=+0.164 (n=489)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.0032 (IC base=+0.158)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.179 (n=555)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 6.0 (IC base=+0.158)

- **PATRÓN** `ibs_20min` > `0.3282` → IC=+0.213 (n=545)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3282 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` > `0.2561` → IC=+0.228 (n=307)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2561 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.622` → IC=+0.207 (n=138)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.622 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.417` → IC=+0.158 (n=466)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` < 4.417 (IC base=+0.158)

- **PATRÓN** `volumen_regimen` < `1.2659` → IC=+0.169 (n=545)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 1.2659 (IC base=+0.158)

- **PATRÓN** `volumen_regimen` > `1.0855` → IC=+0.171 (n=247)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 1.0855 (IC base=+0.158)

- **PATRÓN** `volumen_pendiente_norm` < `0.0733` → IC=+0.177 (n=444)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2083` → IC=+0.176 (n=106)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.2083 (IC base=+0.158)

- **PATRÓN** `volumen_spike_ratio` < `2.4168` → IC=+0.191 (n=496)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` < 2.4168 (IC base=+0.158)

- **PATRÓN** `volumen_spike_ratio` > `1.3798` → IC=+0.173 (n=496)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.3798 (IC base=+0.158)

- **PATRÓN** `libro_liquidez` > `12688.6765` → IC=+0.188 (n=363)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 12688.6765 (IC base=+0.158)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2205` → IC=+0.195 (n=392)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` < 0.2205 (IC base=+0.191)

- **PATRÓN** `volumen_spike_ratio` < `1.9553` → IC=+0.196 (n=169)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 1.9553 (IC base=+0.191)

- **PATRÓN** `volumen_spike_ratio` > `4.2028` → IC=+0.192 (n=128)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 4.2028 (IC base=+0.191)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.222 (n=419)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.191)

- **PATRÓN** `libro_liquidez` > `1803.7992` → IC=+0.196 (n=304)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 1803.7992 (IC base=+0.191)

- **PATRÓN** `ballena_activa_n` < `51.0` → IC=+0.245 (n=147)
...
- **PATRÓN** `sigma_h` < `0.0086` → IC=+0.151 (n=545)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0086 (IC base=+0.140)

- **PATRÓN** `sigma_h` > `0.0039` → IC=+0.143 (n=488)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` > 0.0039 (IC base=+0.140)

- **PATRÓN** `drift_60min` |x|≤ `0.2375` → IC=+0.140 (n=365)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.70€ cuando `drift_60min` |x|≤ 0.2375 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.165 (n=496)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 8.0 (IC base=+0.140)

- **PATRÓN** `ibs_20min` > `0.3336` → IC=+0.195 (n=545)

  - _Acción_: Kelly boost +0.97€ cuando `ibs_20min` > 0.3336 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` > `0.798` → IC=+0.217 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.798 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.431` → IC=+0.191 (n=260)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 4.431 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `0.9006` → IC=+0.167 (n=364)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.9006 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` > `1.2022` → IC=+0.152 (n=182)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.2022 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` > `0.1005` → IC=+0.234 (n=201)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1005 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.4097` → IC=+0.183 (n=509)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 1.4097 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `4668.2669` → IC=+0.221 (n=363)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4668.2669 (IC base=+0.140)

- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.175 (n=416)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0073 (IC base=+0.147)

- **PATRÓN** `drift_60min` |x|≤ `0.4721` → IC=+0.162 (n=472)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.4721 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.155 (n=172)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 17.0 (IC base=+0.147)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.183 (n=219)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 7.0 (IC base=+0.147)

- **PATRÓN** `ibs_20min` < `0.1065` → IC=+0.238 (n=208)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1065 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.2233` → IC=+0.157 (n=199)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.2233 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.52` → IC=+0.247 (n=73)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.52 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `0.5844` → IC=+0.156 (n=158)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.5844 (IC base=+0.147)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5826` → IC=+0.163 (n=182)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.5826 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` > `2.5177` → IC=+0.186 (n=138)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 2.5177 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `9899.6333` → IC=+0.194 (n=214)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 9899.6333 (IC base=+0.147)

- **PATRÓN** `ballena_activa_n` < `192.0` → IC=+0.172 (n=346)

  - _Acción_: Kelly boost +0.86€ cuando `ballena_activa_n` < 192.0 (IC base=+0.147)

### GBM_LATE_15M_TARDIO#SOL#15min
- **PATRÓN** `sigma_h` > `0.0079` → IC=+0.148 (n=402)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0079 (IC base=+0.092)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.138 (n=415)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 12.0 (IC base=+0.092)

- **PATRÓN** `ibs_20min` > `0.4783` → IC=+0.178 (n=603)
...
- **PATRÓN** `libro_liquidez` > `3011.5404` → IC=+0.254 (n=201)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3011.5404 (IC base=+0.092)

- **PATRÓN** `ballena_activa_n` < `66.0` → IC=+0.151 (n=379)

  - _Acción_: Kelly boost +0.75€ cuando `ballena_activa_n` < 66.0 (IC base=+0.092)
...
- **PATRÓN** `sigma_h` > `0.0274` → IC=+0.225 (n=234)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0274 (IC base=+0.185)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.197 (n=705)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 6.0 (IC base=+0.185)

- **PATRÓN** `ibs_20min` > `0.9479` → IC=+0.291 (n=319)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9479 (IC base=+0.185)

- **PATRÓN** `dist_vwap_pct` > `1.4112` → IC=+0.287 (n=148)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.4112 (IC base=+0.185)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.118` → IC=+0.247 (n=365)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.118 (IC base=+0.185)

- **PATRÓN** `volumen_regimen` > `1.0357` → IC=+0.217 (n=320)
...
- **PATRÓN** `volumen_spike_ratio` < `2.6647` → IC=+0.185 (n=653)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 2.6647 (IC base=+0.185)

- **PATRÓN** `volumen_spike_ratio` > `1.4438` → IC=+0.189 (n=653)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 1.4438 (IC base=+0.185)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.193 (n=836)

  - _Acción_: Kelly boost +0.97€ cuando `libro_spread` < 0.02 (IC base=+0.185)

- **PATRÓN** `libro_liquidez` > `3033.2126` → IC=+0.191 (n=234)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 3033.2126 (IC base=+0.185)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4819` → IC=+0.226 (n=656)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_spike_ratio` > `1.465` → IC=+0.204 (n=570)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.465 (IC base=+0.218)

- **PATRÓN** `ballena_activa_n` < `36.0` → IC=+0.184 (n=457)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.145 (n=1329)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3655` → IC=+0.165 (n=888)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.3655 (IC base=+0.141)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.150 (n=387)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 17.0 (IC base=+0.141)

- **PATRÓN** `hora_utc` < `4.0` → IC=+0.192 (n=355)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_regimen` < `1.2164` → IC=+0.155 (n=981)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.2164 (IC base=+0.141)

- **PATRÓN** `volumen_pendiente_norm` < `0.1463` → IC=+0.147 (n=1004)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` < 0.1463 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` < `2.5268` → IC=+0.157 (n=1000)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.5268 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` > `1.4255` → IC=+0.148 (n=1000)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.4255 (IC base=+0.141)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.145 (n=1329)

  - _Acción_: Kelly boost +0.73€ cuando `libro_spread` < 0.01 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `7772.3359` → IC=+0.144 (n=1008)

  - _Acción_: Kelly boost +0.72€ cuando `libro_liquidez` > 7772.3359 (IC base=+0.141)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.185 (n=201)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 5.0 (IC base=+0.138)

- **PATRÓN** `ibs_20min` < `0.1703` → IC=+0.179 (n=241)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` < 0.1703 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` > `0.6648` → IC=+0.176 (n=66)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.6648 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.369` → IC=+0.164 (n=548)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` < 6.369 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `1.1906` → IC=+0.158 (n=548)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.1906 (IC base=+0.138)

- **PATRÓN** `volumen_pendiente_norm` > `0.0693` → IC=+0.168 (n=260)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_pendiente_norm` > 0.0693 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` < `2.5675` → IC=+0.153 (n=546)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.5675 (IC base=+0.138)
...
- **PATRÓN** `libro_liquidez` > `12033.4703` → IC=+0.140 (n=490)
  ...
  - _Acción_: Kelly boost +0.70€ cuando `libro_liquidez` > 12033.4703 (IC base=+0.138)
...
- **PATRÓN** `dist_vwap_pct` > `0.1234` → IC=+0.193 (n=86)
  ...
  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.1234 (IC base=+0.091)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.814` → IC=+0.252 (n=103)
...
- **PATRÓN** `volumen_regimen` > `1.1984` → IC=+0.202 (n=55)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1984 (IC base=+0.091)

- **PATRÓN** `volumen_pendiente_norm` < `0.0651` → IC=+0.265 (n=49)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2582` → IC=+0.350 (n=18)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2582 (IC base=+0.091)

- **PATRÓN** `volumen_spike_ratio` > `1.4015` → IC=+0.324 (n=66)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4015 (IC base=+0.091)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.184 (n=150)

  - _Acción_: Kelly boost +0.92€ cuando `libro_spread` < 0.02 (IC base=+0.091)

- **PATRÓN** `libro_liquidez` > `1593.6494` → IC=+0.169 (n=140)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 1593.6494 (IC base=+0.091)
...
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.218 (n=69)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0049 (IC base=+0.138)
...
- **PATRÓN** `dist_vwap_pct` < `0.1234` → IC=+0.238 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1234 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.134` → IC=+0.318 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.134 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `0.6023` → IC=+0.206 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6023 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `1.0919` → IC=+0.231 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0919 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `2204.6678` → IC=+0.245 (n=45)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2204.6678 (IC base=+0.138)
...
- **PATRÓN** `ibs_20min` < `0.4538` → IC=+0.124 (n=131)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` < 0.4538 (IC base=+0.040)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.944` → IC=+0.133 (n=58)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` > 5.944 (IC base=+0.040)
...
- **PATRÓN** `sigma_h` > `0.0035` → IC=+0.188 (n=30)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0035 (IC base=+0.111)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.250 (n=26)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 16.0 (IC base=+0.111)

- **PATRÓN** `ibs_20min` < `0.2878` → IC=+0.181 (n=67)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` < 0.2878 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.84` → IC=+0.160 (n=45)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 3.84 (IC base=+0.111)

- **PATRÓN** `volumen_regimen` < `1.1843` → IC=+0.152 (n=67)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1843 (IC base=+0.111)

- **PATRÓN** `libro_liquidez` > `3965.7979` → IC=+0.139 (n=59)

  - _Acción_: Kelly boost +0.70€ cuando `libro_liquidez` > 3965.7979 (IC base=+0.111)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.176 (n=1025)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.029 (n=3142)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.213 (n=1010)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=3295)

- **FILTRO** `ibs_20min` > `0.2734` → IC=-0.172 (n=1075)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2734
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=3230)
...
- **FILTRO** `py_entrada` > `0.67` → IC=-0.160 (n=186)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=567)

- **FILTRO** `ibs_20min` > `0.1818` → IC=-0.127 (n=376)
...
- **FILTRO** `ibs_20min` > `0.7336` → IC=-0.214 (n=180)
...
- **FILTRO** `py_entrada` > `0.54` → IC=-0.183 (n=181)
...
- **FILTRO** `ballena_activa_n` > `10.0` → IC=-0.169 (n=170)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=521)
...
- **FILTRO** `py_entrada` > `0.64` → IC=-0.255 (n=165)

  - _Acción_: SKIP cuando `py_entrada` > 0.64
  - _Potencial_: sin este filtro IC_bueno=+0.008 (n=535)

- **FILTRO** `ibs_20min` > `0.2733` → IC=-0.188 (n=174)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.145 (n=3022)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=7508)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.279 (n=2624)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=7906)

- **FILTRO** `ibs_7min` < `0.7268` → IC=-0.232 (n=2631)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7268
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=7899)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.173 (n=3562)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=6968)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.229 (n=2996)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=9974)

- **FILTRO** `ibs_7min` > `0.7272` → IC=-0.170 (n=3242)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7272
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=9728)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.3` → IC=-0.322 (n=346)

  - _Acción_: SKIP cuando `py_entrada` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=1107)

- **FILTRO** `ibs_7min` < `1.0` → IC=-0.186 (n=1014)
...
- **FILTRO** `ballena_activa_n` > `161.0` → IC=-0.189 (n=502)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=1522)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.225 (n=488)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=1528)

- **FILTRO** `ballena_activa_n` > `99.0` → IC=-0.176 (n=684)

  - _Acción_: SKIP cuando `ballena_activa_n` > 99.0
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=1332)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.201 (n=369)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.091 (n=1178)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.308 (n=487)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=1060)

- **FILTRO** `ibs_7min` < `0.2222` → IC=-0.282 (n=383)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2222
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=1164)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.241 (n=380)

  - _Acción_: SKIP cuando `ballena_activa_n` > 13.0
  - _Potencial_: sin este filtro IC_bueno=-0.077 (n=1167)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.243 (n=501)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=1770)

- **FILTRO** `ibs_7min` > `0.8113` → IC=-0.177 (n=565)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8113
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=1706)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.150 (n=535)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.081 (n=1241)

- **FILTRO** `py_entrada` < `0.44` → IC=-0.211 (n=848)

  - _Acción_: SKIP cuando `py_entrada` < 0.44
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=928)

- **FILTRO** `ibs_7min` < `0.7606` → IC=-0.182 (n=444)
...
- **FILTRO** `ballena_activa_n` > `39.0` → IC=-0.192 (n=440)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=1336)
...
- **FILTRO** `ibs_7min` > `0.1905` → IC=-0.173 (n=606)

  - _Acción_: SKIP cuando `ibs_7min` > 0.1905
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=1177)

- **FILTRO** `ballena_activa_n` > `35.0` → IC=-0.188 (n=437)

  - _Acción_: SKIP cuando `ballena_activa_n` > 35.0
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=1346)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.41` → IC=-0.235 (n=493)

  - _Acción_: SKIP cuando `py_entrada` < 0.41
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=1528)

- **FILTRO** `ibs_7min` < `0.7632` → IC=-0.200 (n=505)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7632
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=1516)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.185 (n=490)

  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=1531)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.181 (n=594)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=1804)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `hora_utc` < `9.0` → IC=-0.131 (n=554)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=1155)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.300 (n=418)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=1291)

- **FILTRO** `ibs_7min` < `0.75` → IC=-0.232 (n=423)

  - _Acción_: SKIP cuando `ibs_7min` < 0.75
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=1286)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.226 (n=422)

  - _Acción_: SKIP cuando `ballena_activa_n` > 13.0
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=1287)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.270 (n=463)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=1782)

- **FILTRO** `ibs_7min` > `0.8` → IC=-0.166 (n=557)
...
- **FILTRO** `ballena_activa_n` > `10.0` → IC=-0.132 (n=739)
  ...
  - _Acción_: SKIP cuando `ballena_activa_n` > 10.0
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=1506)
...
- **PATRÓN** `delta_ratio` |x|> `0.3984` → IC=+0.138 (n=387)
...
- **PATRÓN** `delta_ratio` |x|> `0.3997` → IC=+0.204 (n=52)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio` |x|> 0.3997 (IC base=+0.148)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.184 (n=36)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 12.0 (IC base=+0.148)

- **PATRÓN** `total_vol_5m` < `11104.393` → IC=+0.167 (n=52)

  - _Acción_: Kelly boost +0.83€ cuando `total_vol_5m` < 11104.393 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `3221.1629` → IC=+0.173 (n=47)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 3221.1629 (IC base=+0.148)

- **PATRÓN** `ballena_activa_n` < `68.0` → IC=+0.167 (n=28)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 68.0 (IC base=+0.148)

### ORDER_FLOW_5M#XRP#5min
- **PATRÓN** `delta_ratio` |x|> `0.3998` → IC=+0.156 (n=59)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.78€ cuando `delta_ratio` |x|> 0.3998 (IC base=+0.084)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.123 (n=67)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` < 18.0 (IC base=+0.084)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.192 (n=37)

  - _Acción_: Kelly boost +0.96€ cuando `libro_spread` < 0.01 (IC base=+0.084)

- **PATRÓN** `libro_liquidez` > `3338.8044` → IC=+0.208 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3338.8044 (IC base=+0.084)

- **PATRÓN** `ballena_activa_n` < `44.0` → IC=+0.183 (n=39)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 44.0 (IC base=+0.084)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.012 (n=1318)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.671` → IC=-0.176 (n=381)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=2925)
...
- **FILTRO** `sigma_h` < `0.0035` → IC=-0.215 (n=233)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0035
  - _Potencial_: sin este filtro IC_bueno=-0.210 (n=474)

- **FILTRO** `drift_15min` |x|> `0.7597` → IC=-0.219 (n=176)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 0.7597
  - _Potencial_: sin este filtro IC_bueno=-0.209 (n=531)
...
- **PATRÓN** `dist_vwap_pct` < `0.3035` → IC=+0.148 (n=52)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.3035 (IC base=-0.059)
...
- **FILTRO** `sigma_ewma_delta_pct` > `16.581` → IC=-0.152 (n=113)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=944)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.0628` → IC=+0.441 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.0628 (IC base=-0.070)

- **PATRÓN** `ibs_15` < `0.05` → IC=+0.342 (n=17)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.05 (IC base=-0.070)

- **PATRÓN** `ibs_15` > `0.119` → IC=+0.382 (n=32)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.119 (IC base=-0.070)
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3542 sube el IC de -0.077 a +0.314 en UPDOWN_GBM_15M_TARDIO (n=240). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#SOL#15min**: dentro de BUY_NO, IBS < 0.38 sube el IC de -0.059 a +0.167 en UPDOWN_GBM_15M_TARDIO#SOL#15min (n=52). Ya aplicado como kelly_boost=+0.83€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO#XRP#15min**: dentro de BUY_NO, IBS < 0.05 sube el IC de -0.070 a +0.342 en UPDOWN_GBM_15M_TARDIO#XRP#15min (n=17). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO#XRP#15min**: dentro de BUY_NO, IBS > 0.119 sube el IC de -0.070 a +0.382 en UPDOWN_GBM_15M_TARDIO#XRP#15min (n=32). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 11669 | -0.102 | -1972.87€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 819 | -0.032 | -130.91€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 10850 | -0.107 | -1841.96€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1507 | -0.056 | -304.35€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1507 | -0.056 | -304.35€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 819 | -0.032 | -130.91€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 819 | -0.032 | -130.91€ | 2 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 1509 | -0.141 | -454.89€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 1509 | -0.141 | -454.89€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 3119 | -0.070 | -317.20€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 3119 | -0.070 | -317.20€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 2697 | -0.109 | -271.50€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2697 | -0.109 | -271.50€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 2018 | -0.173 | -494.03€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 2018 | -0.173 | -494.03€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 1448 | -0.060 | +704.17€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 408 | -0.002 | +277.23€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 1040 | -0.083 | +426.94€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 1448 | -0.060 | +704.17€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 408 | -0.002 | +277.23€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 1040 | -0.083 | +426.94€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 37742 | +0.115 | -2344.20€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 6714 | +0.185 | -245.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 117 | -0.088 | -49.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 27531 | +0.098 | -2002.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3380 | +0.119 | -46.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 4616 | +0.071 | -712.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 22 | -0.083 | +0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 4589 | +0.072 | -706.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 7700 | +0.134 | -161.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1912 | +0.198 | -92.66€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 4582 | +0.110 | -101.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1164 | +0.128 | +54.56€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 4628 | +0.084 | -537.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 24 | +0.077 | +2.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 4603 | +0.084 | -538.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 8262 | +0.127 | -131.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2447 | +0.168 | -17.99€ | 1 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 4585 | +0.112 | -79.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1218 | +0.100 | -26.35€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 7922 | +0.131 | -505.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2292 | +0.199 | -138.17€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 55 | +0.009 | -9.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 4577 | +0.099 | -282.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 998 | +0.132 | -75.00€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#XRP | 4614 | +0.108 | -295.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 17 | -0.022 | -0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 4595 | +0.109 | -294.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 6648 | +0.175 | -519.21€ | 2 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 6648 | +0.175 | -519.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 1681 | +0.163 | -188.37€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 1681 | +0.163 | -188.37€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 1652 | +0.168 | -176.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 1652 | +0.168 | -176.72€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 1489 | +0.230 | -46.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 1489 | +0.230 | -46.86€ | 0 | 2 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 1615 | +0.184 | -123.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 1615 | +0.184 | -123.22€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 19623 | +0.191 | -1723.41€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 19623 | +0.191 | -1723.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 3549 | +0.134 | -615.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 3549 | +0.134 | -615.86€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 3077 | +0.233 | -76.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 3077 | +0.233 | -76.13€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 3352 | +0.169 | -400.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 3352 | +0.169 | -400.62€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 3121 | +0.227 | -105.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 3121 | +0.227 | -105.13€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 3216 | +0.209 | -191.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 3216 | +0.209 | -191.50€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 3308 | +0.184 | -334.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 3308 | +0.184 | -334.16€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 7160 | +0.133 | +259.40€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 7160 | +0.133 | +259.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 3567 | +0.138 | +159.90€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 3567 | +0.138 | +159.90€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 3593 | +0.128 | +99.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 3593 | +0.128 | +99.50€ | 0 | 5 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 9114 | +0.096 | -321.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 867 | +0.069 | -26.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 8247 | +0.099 | -294.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC | 5838 | +0.098 | -123.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#15min | 867 | +0.069 | -26.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#5min | 4971 | +0.103 | -96.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH | 653 | +0.118 | +10.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH#5min | 653 | +0.118 | +10.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL | 2623 | +0.086 | -208.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL#5min | 2623 | +0.086 | -208.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 489 | +0.282 | -35.15€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 489 | +0.282 | -35.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 489 | +0.282 | -35.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 489 | +0.282 | -35.15€ | 0 | 4 |
| ✅ GBM_LATE_15M | 9029 | +0.045 | +3030.59€ | 0 | 16 |
| ✅ GBM_LATE_15M#15min | 9029 | +0.045 | +3030.59€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 1285 | +0.183 | +860.49€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 1285 | +0.183 | +860.49€ | 0 | 21 |
| ✅ GBM_LATE_15M#BTC | 1308 | +0.174 | +786.41€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1308 | +0.174 | +786.41€ | 0 | 31 |
| ✅ GBM_LATE_15M#DOGE | 1296 | +0.194 | +923.22€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 1296 | +0.194 | +923.22€ | 0 | 22 |
| ✅ GBM_LATE_15M#ETH | 1452 | -0.047 | +47.21€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1452 | -0.047 | +47.21€ | 5 | 11 |
| ✅ GBM_LATE_15M#SOL | 1592 | -0.054 | +156.03€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1592 | -0.054 | +156.03€ | 5 | 3 |
| ✅ GBM_LATE_15M#XRP | 2096 | -0.072 | +257.23€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 2096 | -0.072 | +257.23€ | 4 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 9913 | +0.047 | +3920.94€ | 0 | 17 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 9913 | +0.047 | +3920.94€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 1590 | -0.015 | +661.97€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 1590 | -0.015 | +661.97€ | 2 | 4 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 2157 | -0.039 | +184.75€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 2157 | -0.039 | +184.75€ | 1 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 1157 | +0.246 | +1086.75€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 1157 | +0.246 | +1086.75€ | 0 | 24 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1611 | -0.057 | -31.76€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1611 | -0.057 | -31.76€ | 10 | 2 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 1705 | -0.029 | +376.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 1705 | -0.029 | +376.60€ | 7 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 1693 | +0.252 | +1642.63€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 1693 | +0.252 | +1642.63€ | 0 | 24 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 7411 | +0.172 | +5183.89€ | 0 | 27 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 7411 | +0.172 | +5183.89€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB | 987 | +0.193 | +722.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB#15min | 987 | +0.193 | +722.98€ | 0 | 22 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC | 1246 | +0.165 | +848.63€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC#15min | 1246 | +0.165 | +848.63€ | 0 | 22 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE | 988 | +0.201 | +760.87€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min | 988 | +0.201 | +760.87€ | 0 | 21 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH | 1220 | +0.156 | +776.18€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH#15min | 1220 | +0.156 | +776.18€ | 0 | 26 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL | 1396 | +0.123 | +853.59€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL#15min | 1396 | +0.123 | +853.59€ | 0 | 24 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP | 1574 | +0.199 | +1221.63€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP#15min | 1574 | +0.199 | +1221.63€ | 0 | 19 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 1512 | +0.087 | +387.23€ | 0 | 14 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 1512 | +0.087 | +387.23€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#SOL | 330 | -0.015 | +5.75€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#SOL#15min | 330 | -0.015 | +5.75€ | 4 | 5 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 266 | +0.112 | +76.36€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 266 | +0.112 | +76.36€ | 0 | 19 |
| ✅ GBM_LATE_15M_TARDIO | 8697 | +0.170 | +5916.21€ | 0 | 25 |
| ✅ GBM_LATE_15M_TARDIO#15min | 8697 | +0.170 | +5916.21€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 1235 | +0.203 | +945.12€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 1235 | +0.203 | +945.12€ | 0 | 20 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 1409 | +0.162 | +926.32€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 1409 | +0.162 | +926.32€ | 0 | 26 |
| ✅ GBM_LATE_15M_TARDIO#DOGE | 1222 | +0.214 | +992.37€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#DOGE#15min | 1222 | +0.214 | +992.37€ | 0 | 20 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 1355 | +0.143 | +795.57€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 1355 | +0.143 | +795.57€ | 0 | 26 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 1547 | +0.100 | +766.51€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 1547 | +0.100 | +766.51€ | 0 | 17 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 1929 | +0.202 | +1490.33€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1929 | +0.202 | +1490.33€ | 0 | 23 |
| ✅ GBM_LATE_5M | 2642 | +0.129 | +1210.43€ | 1 | 21 |
| ✅ GBM_LATE_5M#5min | 2642 | +0.129 | +1210.43€ | 0 | 0 |
| ✅ GBM_LATE_5M#BNB | 95 | +0.222 | +72.24€ | 0 | 0 |
| ✅ GBM_LATE_5M#BNB#5min | 95 | +0.222 | +72.24€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC | 909 | +0.127 | +450.35€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC#5min | 909 | +0.127 | +450.35€ | 1 | 17 |
| ✅ GBM_LATE_5M#DOGE | 333 | +0.169 | +199.02€ | 0 | 0 |
| ✅ GBM_LATE_5M#DOGE#5min | 333 | +0.169 | +199.02€ | 0 | 12 |
| ✅ GBM_LATE_5M#ETH | 826 | +0.133 | +361.95€ | 0 | 0 |
| ✅ GBM_LATE_5M#ETH#5min | 826 | +0.133 | +361.95€ | 0 | 24 |
...
| ✅ GBM_LATE_60M | 558 | -0.009 | +127.30€ | 3 | 12 |
| ✅ GBM_LATE_60M#60min | 558 | -0.009 | +127.30€ | 0 | 0 |
| ✅ GBM_LATE_60M#BTC | 193 | +0.028 | +18.37€ | 0 | 0 |
| ✅ GBM_LATE_60M#BTC#60min | 193 | +0.028 | +18.37€ | 2 | 6 |
| ✅ GBM_LATE_60M#ETH | 204 | +0.029 | +80.98€ | 0 | 0 |
| ✅ GBM_LATE_60M#ETH#60min | 204 | +0.029 | +80.98€ | 2 | 9 |
...
| ✅ GBM_LATE_60M_PYCONFIRMADO | 367 | +0.037 | +2.10€ | 2 | 3 |
| ✅ GBM_LATE_60M_PYCONFIRMADO#60min | 367 | +0.037 | +2.10€ | 0 | 0 |
| ✅ GBM_LATE_60M_PYCONFIRMADO#BTC | 154 | +0.032 | +9.47€ | 0 | 0 |
| ✅ GBM_LATE_60M_PYCONFIRMADO#BTC#60min | 154 | +0.032 | +9.47€ | 3 | 6 |
...
| ✅ MOMENTUM_IBS_15M | 5150 | -0.002 | -78.77€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#15min | 5150 | -0.002 | -78.77€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_15M#DOGE | 1014 | +0.002 | -20.30€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#DOGE#15min | 1014 | +0.002 | -20.30€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH | 1162 | +0.008 | +12.39€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH#15min | 1162 | +0.008 | +12.39€ | 1 | 0 |
...
| ✅ MOMENTUM_IBS_15M_BALLENA | 8472 | -0.035 | +193.54€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#15min | 8472 | -0.035 | +193.54€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB | 1353 | -0.035 | +140.91€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB#15min | 1353 | -0.035 | +140.91€ | 4 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC | 1480 | -0.028 | -20.87€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC#15min | 1480 | -0.028 | -20.87€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE | 1364 | -0.044 | +83.43€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE#15min | 1364 | -0.044 | +83.43€ | 5 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH | 1478 | -0.034 | -27.82€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH#15min | 1478 | -0.034 | -27.82€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL | 1410 | -0.040 | +27.49€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL#15min | 1410 | -0.040 | +27.49€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP | 1387 | -0.034 | -9.59€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP#15min | 1387 | -0.034 | -9.59€ | 4 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 23500 | -0.078 | +360.55€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 23500 | -0.078 | +360.55€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 3710 | -0.090 | +399.08€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 3710 | -0.090 | +399.08€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 4040 | -0.073 | -81.65€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 4040 | -0.073 | -81.65€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 3818 | -0.082 | +86.32€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 3818 | -0.082 | +86.32€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 3559 | -0.099 | -191.62€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 3559 | -0.099 | -191.62€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 4419 | -0.051 | +53.67€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 4419 | -0.051 | +53.67€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 3954 | -0.079 | +94.74€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 3954 | -0.079 | +94.74€ | 7 | 0 |
...
| ✅ ORDER_FLOW_5M | 553 | +0.100 | +148.15€ | 1 | 4 |
| ✅ ORDER_FLOW_5M#5min | 417 | +0.113 | +135.56€ | 0 | 0 |
...
| ✅ ORDER_FLOW_5M#SOL | 69 | +0.148 | +31.90€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#SOL#5min | 69 | +0.148 | +31.90€ | 0 | 5 |
| ✅ ORDER_FLOW_5M#XRP | 87 | +0.084 | +15.56€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#XRP#5min | 87 | +0.084 | +15.56€ | 0 | 5 |
...
| ✅ STREAK_FADE_5M#XRP#5min | 264 | -0.053 | -21.04€ | 6 | 0 |
...
| ✅ STRUCT_NO_15M | 3478 | +0.010 | -26.89€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 3478 | +0.010 | -26.89€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 1337 | +0.009 | -12.38€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 1337 | +0.009 | -12.38€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 1359 | +0.019 | +1.56€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 1359 | +0.019 | +1.56€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 782 | -0.005 | -16.07€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 782 | -0.005 | -16.07€ | 2 | 0 |
| ✅ UPDOWN_GBM | 7078 | +0.003 | +144.31€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 2524 | +0.041 | +234.78€ | 0 | 10 |
| ✅ UPDOWN_GBM#240min | 285 | +0.016 | +1.20€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 3797 | -0.018 | -84.42€ | 3 | 0 |
| ✅ UPDOWN_GBM#60min | 425 | -0.011 | -6.75€ | 3 | 0 |
...
| ✅ UPDOWN_GBM#ETH | 1699 | -0.002 | -9.90€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 760 | +0.021 | +15.16€ | 1 | 4 |
| ✅ UPDOWN_GBM#ETH#240min | 82 | +0.059 | +4.56€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 652 | -0.034 | -28.90€ | 3 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 190 | +0.005 | -0.34€ | 0 | 0 |
| 🚫 UPDOWN_GBM#ETH#daily | 15 | -0.154 | -0.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 1795 | -0.007 | -19.30€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 614 | -0.003 | -4.51€ | 1 | 2 |
| ✅ UPDOWN_GBM#SOL#240min | 69 | +0.007 | -1.88€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 1000 | -0.008 | -12.98€ | 2 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 100 | -0.010 | +0.19€ | 1 | 2 |
| 🚫 UPDOWN_GBM#SOL#daily | 12 | -0.129 | -0.13€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 976 | +0.009 | +71.98€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 597 | +0.058 | +106.43€ | 0 | 8 |
| ✅ UPDOWN_GBM#XRP#240min | 33 | -0.157 | -6.09€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 346 | -0.058 | -28.36€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 4414 | -0.074 | +910.37€ | 4 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 4414 | -0.074 | +910.37€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 303 | -0.051 | +341.24€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 303 | -0.051 | +341.24€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 897 | -0.164 | -94.08€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 897 | -0.164 | -94.08€ | 5 | 2 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 1430 | -0.069 | +278.77€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 1430 | -0.069 | +278.77€ | 3 | 4 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 1349 | -0.091 | +204.86€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 1349 | -0.091 | +204.86€ | 5 | 3 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.084) — sin ventaja clara. oversold(IBS<0.3): IC=+0.019 n=2521 | neutral: IC=-0.001 n=2689 | overbought(IBS>0.7): IC=+0.083 n=2825
  - _Datos_: n=8380 IC=+0.035 PNL=+749.81€
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=614 IC=-0.003 PNL=-4.51€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.073 n=116793 | tras_1loss IC=+0.045 n=91478 | tras_2loss IC=+0.008 n=41520/40 | gap=+0.065 (umbral 0.05)
...
**⏳ H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  ...
  - _Estado_: 7016 ops, 22 horas distintas. Sin hora con n≥15 y IC extremo aún.
...
**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  ...
  - _Estado_: n_overlaps=100, boost estimado=+0.014. Necesita 0 más y boost>0.05

**⏳ H-OF-PAR** — ORDER_FLOW per-pair delta_ratio ranges
  - _Umbral_: n≥200 por par con delta_ratio feature en shadow
  - _Acción_: Añadir DELTA_MIN/MAX por par dict en shadow_predict.py
  - _Estado_: BTC: 0/50 ops con delta_ratio feature | SOL: 69 ops con delta_ratio

**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40 en cualquier subtipo 60min
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=190/40 IC=+0.005 PNL=-0.34€ | BTC#60min: n=135/40 IC=-0.033 PNL=-6.60€ | SOL#60min: n=100/40 IC=-0.010 PNL=+0.19€
...
**⏳ H-CUSTOM-OF-MADRUGADA** — ORDER_FLOW de madrugada (0h-6h UTC) BTC+SOL — ¿neutralizar?
  ...
  - _Estado_: 19/30 ops en el filtro definido (IC actual=+0.158 PNL=+9.28€)
  - _Datos_: n=19 IC=+0.158 PNL=+9.28€

**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  - _Hipótesis_: Cuando la volatilidad horaria es muy alta el GBM puede sobreestimar el edge. Testear.
  - _Umbral_: n≥30 y IC<-0.05
  - _Acción_: Filtrar señales GBM cuando sigma_h > 0.002 si se confirma IC negativo
  - _Estado_: n=6809 IC=+0.000 PNL=+93.26€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=6809 IC=+0.000 PNL=+93.26€
...
**〰️ H-CUSTOM-GBM-60MIN-BUYYES** — GBM 60min BUY_YES — ¿edge superior al BUY_NO?
  ...
  - _Estado_: n=352 IC=+0.003 PNL=-0.57€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=352 IC=+0.003 PNL=-0.57€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=1469 IC=+0.025 PNL=+88.03€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1469 IC=+0.025 PNL=+88.03€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=1833 IC=+0.019 PNL=+93.45€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1833 IC=+0.019 PNL=+93.45€
...
**🟡 H-CUSTOM-OF-VOLUMEN-ALTO** — ORDER_FLOW_5M con total_vol_5m alto — ¿volumen extremo mejora el IC?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.103 > 0.08 con n=144 PNL=+32.66€
  - _Datos_: n=144 IC=+0.103 PNL=+32.66€
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=22888 IC=+0.103 PNL=+7066.91€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=22888 IC=+0.103 PNL=+7066.91€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=916 IC=+0.026 PNL=+47.66€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=916 IC=+0.026 PNL=+47.66€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=1669 IC=+0.021 PNL=+87.91€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1669 IC=+0.021 PNL=+87.91€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=5486 IC=-0.147 PNL=+172.25€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=5486 IC=-0.147 PNL=+172.25€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=1477 IC=+0.129 PNL=+764.13€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=1477 IC=+0.129 PNL=+764.13€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.183 > 0.08 con n=658 PNL=+408.49€
  - _Datos_: n=658 IC=+0.183 PNL=+408.49€
...
**〰️ H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: n=1100 IC=-0.064 PNL=+145.77€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1100 IC=-0.064 PNL=+145.77€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.117 > 0.08 con n=277 PNL=-37.71€
  - _Datos_: n=277 IC=+0.117 PNL=-37.71€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.235 > 0.08 con n=1633 PNL=-157.90€
  - _Datos_: n=1633 IC=+0.235 PNL=-157.90€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=3549 IC=+0.134 PNL=-615.86€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=3549 IC=+0.134 PNL=-615.86€
