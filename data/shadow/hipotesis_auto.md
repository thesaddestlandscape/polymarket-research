# Hipótesis automáticas — 2026-07-02 06:07 UTC
_Generado por shadow_postmortem.py sobre 2542 resoluciones (PNL=-84.77€)_
...
- **PATRÓN** `sigma_h` < `0.0101` → IC=+0.231 (n=65)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0101 (IC base=+0.066)
...
- **PATRÓN** `ibs_15` < `0.0714` → IC=+0.233 (n=43)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0714 (IC base=+0.066)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0813` → IC=+0.167 (n=37)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.83€ cuando `pct_spot_vs_ref` |x|≤ 0.0813 (IC base=+0.138)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.206 (n=15)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0058 (IC base=+0.138)

- **PATRÓN** `drift_60min` |x|≤ `1.0585` → IC=+0.167 (n=34)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 1.0585 (IC base=+0.138)

- **PATRÓN** `drift_15min` |x|≤ `0.688` → IC=+0.143 (n=26)

  - _Acción_: Kelly boost +0.71€ cuando `drift_15min` |x|≤ 0.688 (IC base=+0.138)

- **PATRÓN** `ibs_15` < `0.0787` → IC=+0.364 (n=20)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0787 (IC base=+0.138)
...
| ✅ UPDOWN_GBM | 784 | -0.034 | -35.90€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 561 | -0.010 | -9.50€ | 2 | 3 |
...
| ✅ UPDOWN_GBM#ETH | 304 | -0.006 | +7.65€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 221 | +0.011 | +13.60€ | 4 | 5 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.078) — sin ventaja clara. oversold(IBS<0.3): IC=+0.019 n=127 | neutral: IC=-0.058 n=118 | overbought(IBS>0.7): IC=-0.051 n=136
  - _Datos_: n=394 IC=-0.028 PNL=-10.66€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.003 n=1200 | tras_1loss IC=-0.036 n=1260 | tras_2loss IC=-0.050 n=658/40 | gap=+0.052 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=661 IC=-0.028 PNL=-22.34€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=661 IC=-0.028 PNL=-22.34€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=178 IC=+0.039 PNL=+18.36€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=178 IC=+0.039 PNL=+18.36€
...
**⏳ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: 125/200 ops en el filtro definido (IC actual=+0.043 PNL=+8.15€)
  - _Datos_: n=125 IC=+0.043 PNL=+8.15€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=125 IC=+0.043 PNL=+8.15€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=125 IC=+0.043 PNL=+8.15€
