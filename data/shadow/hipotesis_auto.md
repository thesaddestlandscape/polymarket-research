# Hipótesis automáticas — 2026-08-23 17:39 UTC
_Generado por shadow_postmortem.py sobre 128480 resoluciones (PNL=+9385.34€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `158.59` → IC=-0.252 (n=1792)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.59
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=5379)
...
- **FILTRO** `n_ballenas` < `5.0` → IC=-0.190 (n=140)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=469)

- **FILTRO** `restante_s_al_confirmar` > `641.6` → IC=-0.195 (n=152)
...
- **FILTRO** `restante_s_al_confirmar` < `170.12` → IC=-0.148 (n=473)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 170.12
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=1419)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `150.33` → IC=-0.259 (n=442)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 150.33
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=1326)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `174.57` → IC=-0.302 (n=438)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 174.57
  - _Potencial_: sin este filtro IC_bueno=-0.111 (n=892)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.186 (n=4298)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.143 (n=2694)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.187 (n=468)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.325` → IC=+0.284 (n=350)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.325 (IC base=+0.177)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.178 (n=666)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=1009)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.208 (n=293)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.251 (n=171)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.211)

- **PATRÓN** `py_entrada` < `0.305` → IC=+0.299 (n=332)
...
- **PATRÓN** `libro_liquidez` > `789.7295` → IC=+0.232 (n=495)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.197 (n=3749)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.205 (n=1892)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.170 (n=962)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.175 (n=503)

  - _Acción_: Kelly boost +0.88€ cuando `py_entrada` > 0.73 (IC base=+0.165)

### FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.239 (n=854)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.202 (n=905)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.202 (n=619)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.204 (n=3221)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.227 (n=6402)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.194)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.126 (n=1747)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.270 (n=1070)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.238)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.166 (n=762)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.241 (n=1445)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.235 (n=522)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.223 (n=544)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.196)
...
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.202 (n=1147)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.130)

- **PATRÓN** `restante_min` < `3.92` → IC=+0.143 (n=1075)
...
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.220 (n=559)
...
- **PATRÓN** `restante_min` > `4.86` → IC=+0.158 (n=738)
...
- **PATRÓN** `lag_apertura_s` < `6.36` → IC=+0.159 (n=538)
...
- **PATRÓN** `restante_min` < `3.98` → IC=+0.134 (n=545)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9517` → IC=+0.237 (n=588)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9517 (IC base=+0.077)

- **PATRÓN** `dist_vwap_pct` > `0.1697` → IC=+0.242 (n=223)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1697 (IC base=+0.077)

- **PATRÓN** `dist_vwap_pct` < `0.9817` → IC=+0.234 (n=352)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.9817 (IC base=+0.077)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.991` → IC=+0.199 (n=753)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.991 (IC base=+0.077)

- **PATRÓN** `volumen_regimen` < `1.2558` → IC=+0.228 (n=340)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5664` → IC=+0.136 (n=827)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 2.5664 (IC base=+0.077)

- **PATRÓN** `volumen_spike_ratio` > `1.4728` → IC=+0.134 (n=940)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4155` → IC=+0.227 (n=258)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4155 (IC base=+0.225)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.250 (n=266)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.225)

- **PATRÓN** `ibs_20min` > `0.6079` → IC=+0.237 (n=230)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6079 (IC base=+0.225)

- **PATRÓN** `dist_vwap_pct` > `0.1937` → IC=+0.242 (n=176)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1937 (IC base=+0.225)

- **PATRÓN** `dist_vwap_pct` < `0.7063` → IC=+0.235 (n=240)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.7063 (IC base=+0.225)
...
- **PATRÓN** `volumen_regimen` < `1.3035` → IC=+0.231 (n=258)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.3035 (IC base=+0.225)

- **PATRÓN** `volumen_regimen` > `1.1082` → IC=+0.248 (n=117)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1082 (IC base=+0.225)

- **PATRÓN** `volumen_pendiente_norm` < `0.0773` → IC=+0.233 (n=219)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0773 (IC base=+0.225)

- **PATRÓN** `volumen_pendiente_norm` > `0.2704` → IC=+0.295 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2704 (IC base=+0.225)

- **PATRÓN** `volumen_spike_ratio` < `1.349` → IC=+0.275 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.349 (IC base=+0.225)
...
- **PATRÓN** `libro_liquidez` > `11817.8916` → IC=+0.253 (n=172)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11817.8916 (IC base=+0.225)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.174 (n=41)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0066 (IC base=+0.066)

- **PATRÓN** `drift_60min` |x|≤ `0.4354` → IC=+0.135 (n=61)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.67€ cuando `drift_60min` |x|≤ 0.4354 (IC base=+0.066)
...
- **PATRÓN** `ibs_20min` > `0.3333` → IC=+0.149 (n=55)
  ...
  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` > 0.3333 (IC base=+0.066)

### GBM_LATE_15M#XRP#15min
- **FILTRO** `ibs_20min` < `0.4437` → IC=-0.137 (n=260)
...
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.157 (n=756)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` > 0.0078 (IC base=+0.057)

- **PATRÓN** `ibs_20min` > `0.9474` → IC=+0.251 (n=756)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9474 (IC base=+0.057)

- **PATRÓN** `dist_vwap_pct` > `0.4246` → IC=+0.266 (n=331)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4246 (IC base=+0.057)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.333` → IC=+0.131 (n=1295)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` > 2.333 (IC base=+0.057)

- **PATRÓN** `volumen_regimen` > `1.164` → IC=+0.189 (n=252)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` > 1.164 (IC base=+0.057)

- **PATRÓN** `volumen_pendiente_norm` < `0.1155` → IC=+0.159 (n=969)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` < 0.1155 (IC base=+0.057)

- **PATRÓN** `volumen_pendiente_norm` > `0.2516` → IC=+0.207 (n=189)
...
- **PATRÓN** `volumen_spike_ratio` > `2.9533` → IC=+0.169 (n=333)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 2.9533 (IC base=+0.057)

- **PATRÓN** `ballena_activa_n` < `97.0` → IC=+0.290 (n=445)
...
- **FILTRO** `ballena_activa_n` > `491.0` → IC=-0.190 (n=27)
...
- **PATRÓN** `dist_vwap_pct` > `0.6455` → IC=+0.318 (n=20)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6455 (IC base=-0.058)
...
- **PATRÓN** `ibs_20min` > `0.8333` → IC=+0.228 (n=123)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8333 (IC base=-0.002)

- **PATRÓN** `dist_vwap_pct` > `0.3033` → IC=+0.237 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3033 (IC base=-0.002)

- **PATRÓN** `volumen_regimen` > `0.6209` → IC=+0.128 (n=135)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` > 0.6209 (IC base=-0.002)

- **PATRÓN** `volumen_pendiente_norm` > `0.2621` → IC=+0.265 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2621 (IC base=-0.002)

- **PATRÓN** `volumen_spike_ratio` < `1.4567` → IC=+0.151 (n=41)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 1.4567 (IC base=-0.002)

- **PATRÓN** `volumen_spike_ratio` > `1.9854` → IC=+0.138 (n=56)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 1.9854 (IC base=-0.002)

- **PATRÓN** `ballena_activa_n` < `80.0` → IC=+0.189 (n=88)

  - _Acción_: Kelly boost +0.94€ cuando `ballena_activa_n` < 80.0 (IC base=-0.002)
...
- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.218 (n=647)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0088 (IC base=+0.159)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.177 (n=661)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9017` → IC=+0.259 (n=1294)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9017 (IC base=+0.159)

- **PATRÓN** `dist_vwap_pct` > `0.7333` → IC=+0.250 (n=458)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7333 (IC base=+0.159)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.4` → IC=+0.282 (n=599)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.4 (IC base=+0.159)

- **PATRÓN** `volumen_regimen` > `0.6919` → IC=+0.178 (n=1156)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` > 0.6919 (IC base=+0.159)
...
- **PATRÓN** `drift_60min` |x|≤ `0.34` → IC=+0.197 (n=1680)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.34 (IC base=+0.187)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.220 (n=765)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.187)

- **PATRÓN** `ibs_20min` < `0.4` → IC=+0.247 (n=1683)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4 (IC base=+0.187)

- **PATRÓN** `dist_vwap_pct` < `0.3174` → IC=+0.178 (n=1468)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.3174 (IC base=+0.187)
...
- **PATRÓN** `volumen_spike_ratio` < `1.9175` → IC=+0.187 (n=717)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` < 1.9175 (IC base=+0.187)
...
- **PATRÓN** `sigma_h` < `0.0025` → IC=+0.270 (n=85)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0025 (IC base=+0.183)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.218 (n=264)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.183)

- **PATRÓN** `ibs_20min` > `0.655` → IC=+0.246 (n=226)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.655 (IC base=+0.183)

- **PATRÓN** `dist_vwap_pct` > `0.1685` → IC=+0.250 (n=186)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1685 (IC base=+0.183)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.061` → IC=+0.320 (n=87)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.061 (IC base=+0.183)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2763` → IC=+0.325 (n=38)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2763 (IC base=+0.183)

- **PATRÓN** `volumen_spike_ratio` < `1.3995` → IC=+0.231 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3995 (IC base=+0.183)

- **PATRÓN** `volumen_spike_ratio` > `1.7062` → IC=+0.189 (n=149)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 1.7062 (IC base=+0.183)

- **PATRÓN** `libro_liquidez` > `11338.6336` → IC=+0.224 (n=168)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11338.6336 (IC base=+0.183)

- **PATRÓN** `ballena_activa_n` < `366.0` → IC=+0.178 (n=119)

  - _Acción_: Kelly boost +0.89€ cuando `ballena_activa_n` < 366.0 (IC base=+0.183)
...
- **PATRÓN** `ibs_20min` < `0.3093` → IC=+0.229 (n=293)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3093 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` < `0.5066` → IC=+0.170 (n=392)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` < 0.5066 (IC base=+0.149)
...
- **PATRÓN** `volumen_regimen` < `1.0485` → IC=+0.154 (n=293)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 1.0485 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` > `0.6089` → IC=+0.160 (n=333)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.6089 (IC base=+0.149)
...
- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.207 (n=172)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0089 (IC base=+0.112)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.133 (n=344)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 8.0 (IC base=+0.112)

- **PATRÓN** `ibs_20min` > `0.5714` → IC=+0.166 (n=381)

  - _Acción_: Kelly boost +0.83€ cuando `ibs_20min` > 0.5714 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `0.9958` → IC=+0.277 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9958 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.462` → IC=+0.261 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.462 (IC base=+0.112)

- **PATRÓN** `volumen_regimen` > `0.6098` → IC=+0.126 (n=378)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` > 0.6098 (IC base=+0.112)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5631` → IC=+0.125 (n=310)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_spike_ratio` > 1.5631 (IC base=+0.112)

- **PATRÓN** `libro_liquidez` > `2751.5045` → IC=+0.201 (n=172)
...
- **PATRÓN** `sigma_h` < `0.003` → IC=+0.258 (n=93)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.003 (IC base=+0.055)

- **PATRÓN** `ibs_20min` < `0.2623` → IC=+0.142 (n=185)

  - _Acción_: Kelly boost +0.71€ cuando `ibs_20min` < 0.2623 (IC base=+0.055)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.232` → IC=+0.245 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.232 (IC base=+0.055)

- **PATRÓN** `volumen_regimen` < `0.727` → IC=+0.121 (n=122)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_regimen` < 0.727 (IC base=+0.055)

- **PATRÓN** `volumen_spike_ratio` < `1.6718` → IC=+0.140 (n=98)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 1.6718 (IC base=+0.055)

- **PATRÓN** `libro_liquidez` > `3181.5493` → IC=+0.120 (n=185)

  - _Acción_: Kelly boost +0.60€ cuando `libro_liquidez` > 3181.5493 (IC base=+0.055)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3002` → IC=+0.216 (n=93)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3002 (IC base=+0.164)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.197 (n=31)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 17.0 (IC base=+0.164)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.217 (n=44)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.164)

- **PATRÓN** `ibs_20min` < `0.4957` → IC=+0.184 (n=93)

  - _Acción_: Kelly boost +0.92€ cuando `ibs_20min` < 0.4957 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.5584` → IC=+0.206 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5584 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` < `0.2247` → IC=+0.177 (n=97)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` < 0.2247 (IC base=+0.164)
...
- **PATRÓN** `volumen_regimen` < `1.218` → IC=+0.205 (n=93)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.218 (IC base=+0.164)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.227 (n=739)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0082 (IC base=+0.143)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.164 (n=1112)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.951` → IC=+0.288 (n=1004)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.951 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` > `0.9996` → IC=+0.253 (n=367)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9996 (IC base=+0.143)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.41` → IC=+0.236 (n=1358)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.41 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` < `0.7038` → IC=+0.155 (n=656)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.7038 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` > `1.0816` → IC=+0.145 (n=675)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 1.0816 (IC base=+0.143)

- **PATRÓN** `volumen_pendiente_norm` > `0.239` → IC=+0.181 (n=384)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.239 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` < `2.7899` → IC=+0.144 (n=1900)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 2.7899 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` > `1.9173` → IC=+0.154 (n=1266)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.9173 (IC base=+0.143)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.153 (n=1728)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.01 (IC base=+0.143)

- **PATRÓN** `libro_liquidez` > `3397.6253` → IC=+0.195 (n=738)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 3397.6253 (IC base=+0.143)

- **PATRÓN** `ballena_activa_n` < `164.0` → IC=+0.208 (n=734)
...
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.171 (n=141)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0028 (IC base=+0.158)

- **PATRÓN** `sigma_h` > `0.0051` → IC=+0.196 (n=146)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0051 (IC base=+0.158)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.186 (n=320)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 6.0 (IC base=+0.158)

- **PATRÓN** `ibs_20min` > `0.3404` → IC=+0.220 (n=319)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3404 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` > `0.2181` → IC=+0.242 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2181 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.065` → IC=+0.253 (n=95)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.065 (IC base=+0.158)

- **PATRÓN** `volumen_regimen` < `1.3057` → IC=+0.164 (n=319)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.3057 (IC base=+0.158)

- **PATRÓN** `volumen_regimen` > `1.1052` → IC=+0.173 (n=145)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 1.1052 (IC base=+0.158)

- **PATRÓN** `volumen_pendiente_norm` > `0.1369` → IC=+0.241 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1369 (IC base=+0.158)

- **PATRÓN** `volumen_spike_ratio` < `2.5664` → IC=+0.203 (n=271)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5664 (IC base=+0.158)

- **PATRÓN** `volumen_spike_ratio` > `1.3478` → IC=+0.200 (n=271)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.3478 (IC base=+0.158)

- **PATRÓN** `libro_liquidez` > `10984.4814` → IC=+0.207 (n=213)
...
- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.145 (n=280)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` < 0.0062 (IC base=+0.143)

- **PATRÓN** `sigma_h` > `0.008` → IC=+0.167 (n=106)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` > 0.008 (IC base=+0.143)

- **PATRÓN** `drift_60min` |x|≤ `0.1221` → IC=+0.148 (n=140)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.1221 (IC base=+0.143)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.164 (n=293)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_spike_ratio` > `1.4124` → IC=+0.228 (n=285)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4124 (IC base=+0.143)

- **PATRÓN** `libro_liquidez` > `4127.191` → IC=+0.224 (n=212)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4127.191 (IC base=+0.143)

- **PATRÓN** `ballena_activa_n` < `233.0` → IC=+0.211 (n=192)
...
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.178 (n=172)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` > 0.0083 (IC base=+0.082)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.132 (n=172)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 16.0 (IC base=+0.082)

- **PATRÓN** `ibs_20min` > `0.9231` → IC=+0.267 (n=174)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9231 (IC base=+0.082)

- **PATRÓN** `dist_vwap_pct` > `0.6572` → IC=+0.233 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6572 (IC base=+0.082)
...
- **PATRÓN** `ballena_activa_n` < `70.0` → IC=+0.197 (n=153)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 70.0 (IC base=+0.082)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.164 (n=444)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1907` → IC=+0.177 (n=230)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.1907 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.201 (n=115)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.6103` → IC=+0.182 (n=303)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` < 0.6103 (IC base=+0.151)
...
- **PATRÓN** `dist_vwap_pct` < `0.3056` → IC=+0.159 (n=335)
  ...
  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.3056 (IC base=+0.151)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.443` → IC=+0.178 (n=169)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 3.443 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` < `1.1834` → IC=+0.175 (n=303)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 1.1834 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` > `0.6545` → IC=+0.154 (n=345)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 0.6545 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` < `0.0971` → IC=+0.150 (n=298)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_pendiente_norm` < 0.0971 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` > `0.0763` → IC=+0.170 (n=177)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` > 0.0763 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` < `2.698` → IC=+0.183 (n=342)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` < 2.698 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` > `1.4358` → IC=+0.151 (n=342)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.4358 (IC base=+0.151)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.164 (n=444)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.151)

- **PATRÓN** `libro_liquidez` > `6124.0917` → IC=+0.177 (n=345)
...
- **PATRÓN** `sigma_h` < `0.0045` → IC=+0.191 (n=205)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.0045 (IC base=+0.146)

- **PATRÓN** `drift_60min` |x|≤ `0.065` → IC=+0.204 (n=69)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `dist_vwap_pct` < `0.2776` → IC=+0.156 (n=216)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.2776 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.584` → IC=+0.149 (n=95)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 3.584 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.051` → IC=+0.165 (n=201)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` < 6.051 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` < `1.3181` → IC=+0.167 (n=205)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.3181 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` > `0.6306` → IC=+0.157 (n=205)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 0.6306 (IC base=+0.146)

- **PATRÓN** `volumen_pendiente_norm` > `0.0834` → IC=+0.180 (n=98)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.0834 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` < `2.7472` → IC=+0.172 (n=205)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 2.7472 (IC base=+0.146)

- **PATRÓN** `libro_liquidez` > `9882.4678` → IC=+0.160 (n=204)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 9882.4678 (IC base=+0.146)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.163 (n=520)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.053 (n=1637)
...
- **FILTRO** `ibs_20min` < `0.7366` → IC=-0.191 (n=79)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.137 (n=1481)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=3743)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.276 (n=1267)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=3957)

- **FILTRO** `ibs_7min` < `0.74` → IC=-0.229 (n=1303)

  - _Acción_: SKIP cuando `ibs_7min` < 0.74
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=3921)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.177 (n=1750)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=3474)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.222 (n=1547)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=4730)

- **FILTRO** `ibs_7min` > `0.3` → IC=-0.168 (n=1564)

  - _Acción_: SKIP cuando `ibs_7min` > 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=4713)
...
- **FILTRO** `ibs_7min` < `1.0` → IC=-0.178 (n=514)
...
- **FILTRO** `ballena_activa_n` > `130.0` → IC=-0.161 (n=269)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.209 (n=201)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.079 (n=513)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.339 (n=178)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=536)

- **FILTRO** `ibs_7min` < `0.7184` → IC=-0.260 (n=235)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7184
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=479)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.275 (n=176)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=538)
...
- **FILTRO** `ibs_7min` > `0.812` → IC=-0.173 (n=270)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=812)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.158 (n=217)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=673)
...
- **FILTRO** `ibs_7min` < `0.7815` → IC=-0.192 (n=222)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=668)
...
- **FILTRO** `ibs_7min` > `0.1739` → IC=-0.151 (n=302)
...
- **FILTRO** `py_entrada` < `0.4` → IC=-0.215 (n=237)

  - _Acción_: SKIP cuando `py_entrada` < 0.4
  - _Potencial_: sin este filtro IC_bueno=+0.010 (n=719)

- **FILTRO** `ibs_7min` < `0.7727` → IC=-0.200 (n=238)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7727
  - _Potencial_: sin este filtro IC_bueno=+0.006 (n=718)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.214 (n=229)
...
- **FILTRO** `py_entrada` < `0.37` → IC=-0.279 (n=202)
...
- **FILTRO** `ibs_7min` < `0.75` → IC=-0.221 (n=206)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=645)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.291 (n=223)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=830)

- **FILTRO** `ibs_7min` > `0.8092` → IC=-0.153 (n=263)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8092
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=790)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.139 (n=344)

  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=709)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.008 (n=1010)
...
- **FILTRO** `sigma_h` > `0.0086` → IC=-0.157 (n=717)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `sigma_ewma_delta_pct` > `16.746` → IC=-0.199 (n=284)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 16.746
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=1827)
...
| ✅ BALLENAS_TARDIAS | 5564 | -0.098 | -606.17€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 609 | -0.079 | -102.87€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 4955 | -0.101 | -503.30€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 773 | -0.041 | -92.16€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 773 | -0.041 | -92.16€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 609 | -0.079 | -102.87€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 609 | -0.079 | -102.87€ | 3 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 1768 | -0.049 | -8.95€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 1768 | -0.049 | -8.95€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1330 | -0.174 | -208.72€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1330 | -0.174 | -208.72€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 24641 | +0.114 | -1670.53€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 4859 | +0.186 | -172.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 106 | -0.102 | -48.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 16727 | +0.092 | -1436.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2949 | +0.124 | -13.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 2814 | +0.059 | -509.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 18 | -0.045 | +2.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 