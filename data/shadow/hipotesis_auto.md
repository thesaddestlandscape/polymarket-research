# Hipótesis automáticas — 2026-07-23 22:32 UTC
_Generado por shadow_postmortem.py sobre 31750 resoluciones (PNL=+6949.10€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.204 (n=1825)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.196)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.200 (n=1760)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.202 (n=2263)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.233 (n=155)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.221 (n=406)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.213 (n=560)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.213)

- **PATRÓN** `libro_liquidez` > `4236.3867` → IC=+0.213 (n=294)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4236.3867 (IC base=+0.213)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.130 (n=2120)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.596` → IC=+0.222 (n=454)
...
- **PATRÓN** `dist_vwap_pct` > `0.2737` → IC=+0.132 (n=395)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.2737 (IC base=+0.097)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.34` → IC=+0.178 (n=116)
...
- **FILTRO** `sigma_ewma_delta_pct` > `9.123` → IC=-0.309 (n=19)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.076 (n=439)
...
- **PATRÓN** `sigma_h` < `0.0103` → IC=+0.162 (n=226)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0103 (IC base=+0.146)

- **PATRÓN** `sigma_h` > `0.0217` → IC=+0.160 (n=307)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.1132` → IC=+0.134 (n=315)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.1132 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.676` → IC=+0.289 (n=74)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.172 (n=1324)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 8.0 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` > `0.2782` → IC=+0.205 (n=401)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2782 (IC base=+0.151)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.258` → IC=+0.234 (n=648)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.258 (IC base=+0.151)

- **PATRÓN** `sigma_h` > `0.0043` → IC=+0.148 (n=1610)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0043 (IC base=+0.131)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.146 (n=1122)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.7514` → IC=+0.134 (n=1627)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.7514 (IC base=+0.131)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.153` → IC=+0.156 (n=321)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 6.153 (IC base=+0.131)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.896` → IC=+0.148 (n=106)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 6.896 (IC base=+0.091)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.125` → IC=+0.131 (n=312)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` < 3.125 (IC base=+0.095)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` < `0.018` → IC=+0.258 (n=354)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.253 (n=322)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.126 (n=1243)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 8.0 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `0.4705` → IC=+0.167 (n=274)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.4705 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.72` → IC=+0.280 (n=212)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.699` → IC=-0.167 (n=16)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.085 (n=362)
...
- **PATRÓN** `sigma_h` < `0.0108` → IC=+0.125 (n=339)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `sigma_h` < `0.0215` → IC=+0.189 (n=393)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0215 (IC base=+0.183)

- **PATRÓN** `sigma_h` > `0.0287` → IC=+0.195 (n=149)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0287 (IC base=+0.183)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.191 (n=399)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 8.0 (IC base=+0.183)

- **PATRÓN** `dist_vwap_pct` > `0.2426` → IC=+0.244 (n=135)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2426 (IC base=+0.183)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.065` → IC=+0.307 (n=81)
...
- **PATRÓN** `dist_vwap_pct` > `0.4577` → IC=+0.156 (n=62)
  ...
  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.4577 (IC base=+0.060)

- **PATRÓN** `sigma_ewma_delta_pct` < `8.385` → IC=+0.130 (n=163)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` < 8.385 (IC base=+0.060)
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 892 | +0.128 | +40.83€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 892 | +0.128 | +40.83€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 10 | +0.083 | +2.04€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 10 | +0.083 | +2.04€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 338 | +0.135 | +11.04€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 338 | +0.135 | +11.04€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 5563 | +0.187 | +91.92€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3649 | +0.215 | +91.60€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 1627 | +0.196 | +44.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1214 | +0.210 | -7.96€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 1966 | +0.176 | +1.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1203 | +0.212 | +36.16€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 1940 | +0.192 | +43.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1206 | +0.224 | +64.94€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 301 | +0.319 | +17.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 301 | +0.319 | +17.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 301 | +0.319 | +17.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 301 | +0.319 | +17.76€ | 0 | 0 |
| ✅ GBM_LATE_15M | 7244 | +0.096 | +2254.93€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 7244 | +0.096 | +2254.93€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1754 | +0.065 | +285.79€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1754 | +0.065 | +285.79€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1622 | +0.075 | +287.85€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1622 | +0.075 | +287.85€ | 0 | 6 |
| ✅ GBM_LATE_15M#SOL | 1952 | +0.089 | +697.91€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1952 | +0.089 | +697.91€ | 1 | 3 |
| ✅ GBM_LATE_15M#XRP | 1916 | +0.149 | +983.39€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1916 | +0.149 | +983.39€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 4439 | +0.125 | +2211.41€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 4439 | +0.125 | +2211.41€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 1147 | +0.082 | +396.00€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 1147 | +0.082 | +396.00€ | 0 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1159 | +0.092 | +423.31€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1159 | +0.092 | +423.31€ | 0 | 6 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 1122 | +0.099 | +465.07€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 1122 | +0.099 | +465.07€ | 0 | 6 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 1011 | +0.239 | +927.04€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 1011 | +0.239 | +927.04€ | 0 | 9 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 246 | +0.109 | +112.75€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 246 | +0.109 | +112.75€ | 0 | 0 |
| 🚫 GBM_LATE_15M_PYCONFIRMADO#BTC | 9 | -0.102 | -1.94€ | 0 | 0 |
| 🚫 GBM_LATE_15M_PYCONFIRMADO#BTC#15min | 9 | -0.102 | -1.94€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH | 85 | +0.006 | +16.17€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH#15min | 85 | +0.006 | +16.17€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO | 4465 | +0.077 | +1292.35€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 4465 | +0.077 | +1292.35€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 1076 | +0.034 | +135.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 1076 | +0.034 | +135.25€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 1075 | +0.028 | +80.57€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 1075 | +0.028 | +80.57€ | 0 | 1 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 1031 | +0.044 | +214.90€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 1031 | +0.044 | +214.90€ | 1 | 3 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 1283 | +0.180 | +861.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1283 | +0.180 | +861.64€ | 0 | 10 |
...
| ✅ UPDOWN_GBM | 2389 | +0.032 | +207.72€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 2126 | +0.052 | +246.23€ | 0 | 3 |
...
| ✅ UPDOWN_GBM#ETH | 968 | +0.036 | +62.04€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 874 | +0.051 | +73.61€ | 0 | 6 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1432 | +0.163 | +660.47€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1432 | +0.163 | +660.47€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 360 | +0.202 | +252.49€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 360 | +0.202 | +252.49€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.186: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.062 n=1215 | neutral: IC=+0.013 n=939 | overbought(IBS>0.7): IC=+0.200 n=1203
  - _Datos_: n=3579 IC=+0.103 PNL=+919.87€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=05h: IC=+0.128 n=84 PNL=+35.91€ → BOOST | H=10h: IC=-0.154 n=15 PNL=-6.77€ → FILTRAR | H=14h: IC=+0.167 n=94 PNL=+41.63€ → BOOST | H=16h: IC=+0.128 n=92 PNL=+18.23€ → BOOST | H=21h: IC=+0.113 n=91 PNL=+14.40€ → BOOST | H=22h: IC=+0.113 n=109 PNL=+18.74€ → BOOST
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.114 n=18982 | tras_1loss IC=+0.082 n=12572 | tras_2loss IC=+0.050 n=5225/40 | gap=+0.064 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2257 IC=+0.037 PNL=+222.46€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2257 IC=+0.037 PNL=+222.46€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.093 > 0.08 con n=1132 PNL=+196.80€
  - _Datos_: n=1132 IC=+0.093 PNL=+196.80€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=1134 IC=+0.039 PNL=+86.92€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1134 IC=+0.039 PNL=+86.92€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=959 IC=+0.051 PNL=+110.47€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=959 IC=+0.051 PNL=+110.47€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.115 > 0.08 con n=434 PNL=+127.03€
  - _Datos_: n=434 IC=+0.115 PNL=+127.03€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.109 > 0.08 con n=471 PNL=+161.80€
  - _Datos_: n=471 IC=+0.109 PNL=+161.80€
