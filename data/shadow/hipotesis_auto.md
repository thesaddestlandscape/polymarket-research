# Hipótesis automáticas — 2026-09-18 06:16 UTC
_Generado por shadow_postmortem.py sobre 492605 resoluciones (PNL=+53240.89€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `144.2` → IC=-0.248 (n=6050)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 144.2
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=18152)
...
- **FILTRO** `restante_s_al_confirmar` < `132.71` → IC=-0.294 (n=737)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 132.71
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=2213)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `158.81` → IC=-0.238 (n=1432)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.81
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=4296)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `118.48` → IC=-0.367 (n=1175)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 118.48
  - _Potencial_: sin este filtro IC_bueno=-0.109 (n=3528)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.198 (n=12187)

  - _Acción_: Kelly boost +0.99€ cuando `py_entrada` > 0.69 (IC base=+0.098)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.152 (n=3049)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.01 (IC base=+0.098)

- **PATRÓN** `libro_liquidez` > `5349.0852` → IC=+0.172 (n=1943)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.140 (n=11525)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` < 7.0 (IC base=+0.134)

- **PATRÓN** `py_entrada` < `0.345` → IC=+0.246 (n=8380)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.345 (IC base=+0.134)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.170 (n=5990)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.02 (IC base=+0.134)

- **PATRÓN** `libro_liquidez` > `1657.3138` → IC=+0.162 (n=5671)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 1657.3138 (IC base=+0.134)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.210 (n=1446)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.204)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.205 (n=1428)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.205 (n=1793)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.204)

- **PATRÓN** `libro_liquidez` > `15388.0586` → IC=+0.231 (n=463)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 15388.0586 (IC base=+0.204)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.208 (n=1310)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.200)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.207 (n=1447)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.200)

- **PATRÓN** `py_entrada` < `0.37` → IC=+0.271 (n=1264)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.200)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.202 (n=1857)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.200)

- **PATRÓN** `libro_liquidez` > `13508.1892` → IC=+0.214 (n=651)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.167 (n=304)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.146 (n=2435)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 5.0 (IC base=+0.138)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.138 (n=2080)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.86` → IC=+0.427 (n=536)
...
- **PATRÓN** `py_entrada` < `0.335` → IC=+0.223 (n=265)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.175 (n=2286)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` < 17.0 (IC base=+0.167)

- **PATRÓN** `py_entrada` < `0.74` → IC=+0.177 (n=2389)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.187 (n=5275)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.222 (n=1962)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.219)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.265 (n=1799)
...
- **PATRÓN** `libro_liquidez` > `1602.2161` → IC=+0.294 (n=968)
...
### GBM_LATE_15M
- **PATRÓN** `drift_60min` |x|≤ `0.3494` → IC=+0.121 (n=5787)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.61€ cuando `drift_60min` |x|≤ 0.3494 (IC base=+0.098)

- **PATRÓN** `ibs_20min` > `0.9779` → IC=+0.233 (n=2192)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9779 (IC base=+0.098)

- **PATRÓN** `dist_vwap_pct` < `0.2006` → IC=+0.247 (n=1403)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2006 (IC base=+0.098)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.835` → IC=+0.169 (n=2551)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 5.835 (IC base=+0.098)

- **PATRÓN** `volumen_regimen` < `1.2223` → IC=+0.244 (n=1692)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3078` → IC=+0.208 (n=635)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3078 (IC base=+0.098)

- **PATRÓN** `volumen_spike_ratio` > `1.4663` → IC=+0.197 (n=4373)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.4663 (IC base=+0.098)

- **PATRÓN** `ibs_20min` < `0.5706` → IC=+0.132 (n=8025)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.5706 (IC base=+0.060)

- **PATRÓN** `dist_vwap_pct` > `0.5485` → IC=+0.185 (n=487)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.5485 (IC base=+0.060)

- **PATRÓN** `dist_vwap_pct` < `0.1361` → IC=+0.170 (n=2490)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` < 0.1361 (IC base=+0.060)

- **PATRÓN** `volumen_regimen` < `0.7018` → IC=+0.175 (n=1172)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` < 0.7018 (IC base=+0.060)

- **PATRÓN** `volumen_regimen` > `0.8717` → IC=+0.173 (n=1775)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 0.8717 (IC base=+0.060)

- **PATRÓN** `volumen_pendiente_norm` > `0.167` → IC=+0.219 (n=1295)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.167 (IC base=+0.060)

- **PATRÓN** `volumen_spike_ratio` > `1.5864` → IC=+0.200 (n=3954)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5864 (IC base=+0.060)

- **PATRÓN** `ballena_activa_n` < `152.0` → IC=+0.210 (n=4199)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.362` → IC=+0.247 (n=1143)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.362 (IC base=+0.234)

- **PATRÓN** `volumen_pendiente_norm` < `0.0922` → IC=+0.231 (n=921)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.236 (n=1144)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.234)

- **PATRÓN** `libro_liquidez` > `1749.9034` → IC=+0.250 (n=729)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1749.9034 (IC base=+0.234)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `sigma_h` < `0.0052` → IC=+0.225 (n=990)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.228 (n=1170)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.213)

- **PATRÓN** `ibs_20min` > `0.916` → IC=+0.260 (n=510)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.916 (IC base=+0.213)

- **PATRÓN** `dist_vwap_pct` > `0.1847` → IC=+0.216 (n=575)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1847 (IC base=+0.213)

- **PATRÓN** `dist_vwap_pct` < `0.3532` → IC=+0.215 (n=1068)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3532 (IC base=+0.213)
...
- **PATRÓN** `volumen_regimen` < `1.2558` → IC=+0.224 (n=1125)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2558 (IC base=+0.213)

- **PATRÓN** `volumen_regimen` > `0.876` → IC=+0.215 (n=750)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.876 (IC base=+0.213)

- **PATRÓN** `volumen_pendiente_norm` > `0.0994` → IC=+0.215 (n=409)
...
- **PATRÓN** `volumen_spike_ratio` > `2.368` → IC=+0.226 (n=366)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.368 (IC base=+0.213)

- **PATRÓN** `libro_liquidez` > `11045.5521` → IC=+0.224 (n=1125)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11045.5521 (IC base=+0.213)

- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.148 (n=1200)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0056 (IC base=+0.136)
...
- **PATRÓN** `ibs_20min` < `0.6762` → IC=+0.173 (n=1200)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.6762 (IC base=+0.136)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.577` → IC=+0.160 (n=392)
  ...
  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 6.577 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `1.2116` → IC=+0.144 (n=1200)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.2116 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` > `0.6183` → IC=+0.137 (n=1200)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4311` → IC=+0.150 (n=1090)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 2.4311 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.4244` → IC=+0.140 (n=1091)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` > 1.4244 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `13590.0128` → IC=+0.146 (n=800)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 13590.0128 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `241.0` → IC=+0.155 (n=445)

  - _Acción_: Kelly boost +0.78€ cuando `ballena_activa_n` < 241.0 (IC base=+0.136)

### GBM_LATE_15M#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0059` → IC=+0.192 (n=1454)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0059 (IC base=+0.179)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.182 (n=1521)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 5.0 (IC base=+0.179)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.182 (n=968)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` < 11.0 (IC base=+0.179)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.257 (n=571)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.179)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.502` → IC=+0.229 (n=418)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.502 (IC base=+0.179)

- **PATRÓN** `volumen_pendiente_norm` < `0.1057` → IC=+0.186 (n=1236)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` < 0.1057 (IC base=+0.179)

- **PATRÓN** `volumen_pendiente_norm` > `0.374` → IC=+0.186 (n=189)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.374 (IC base=+0.179)

- **PATRÓN** `volumen_spike_ratio` > `2.9866` → IC=+0.199 (n=619)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.9866 (IC base=+0.179)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.192 (n=1678)

  - _Acción_: Kelly boost +0.96€ cuando `libro_spread` < 0.04 (IC base=+0.179)

- **PATRÓN** `sigma_h` < `0.0105` → IC=+0.224 (n=1242)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.3836` → IC=+0.232 (n=1093)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.323` → IC=+0.217 (n=1362)
...
- **PATRÓN** `volumen_spike_ratio` < `1.846` → IC=+0.206 (n=491)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.846 (IC base=+0.216)

- **PATRÓN** `volumen_spike_ratio` > `2.3036` → IC=+0.225 (n=744)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3036 (IC base=+0.216)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.227 (n=705)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.216)

- **PATRÓN** `libro_liquidez` > `1900.6539` → IC=+0.238 (n=414)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1900.6539 (IC base=+0.216)

- **PATRÓN** `ballena_activa_n` < `27.0` → IC=+0.222 (n=718)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.418` → IC=+0.137 (n=571)

  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` > 4.418 (IC base=+0.009)
...
- **PATRÓN** `volumen_regimen` > `1.1956` → IC=+0.366 (n=80)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1956 (IC base=+0.009)
...
- **PATRÓN** `dist_vwap_pct` > `0.1669` → IC=+0.183 (n=228)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.1669 (IC base=+0.007)

- **PATRÓN** `volumen_regimen` < `0.6123` → IC=+0.164 (n=209)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.6123 (IC base=+0.007)

- **PATRÓN** `volumen_pendiente_norm` > `0.2151` → IC=+0.213 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2151 (IC base=+0.007)

- **PATRÓN** `volumen_spike_ratio` > `1.5033` → IC=+0.185 (n=512)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` > 1.5033 (IC base=+0.007)
...
- **FILTRO** `ibs_20min` > `0.2727` → IC=-0.132 (n=1832)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2727
  - _Potencial_: sin este filtro IC_bueno=+0.122 (n=910)

- **FILTRO** `sigma_ewma_delta_pct` > `8.632` → IC=-0.210 (n=301)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.632
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=2441)
...
- **PATRÓN** `ibs_20min` < `0.2727` → IC=+0.122 (n=910)

  - _Acción_: Kelly boost +0.61€ cuando `ibs_20min` < 0.2727 (IC base=-0.048)

- **PATRÓN** `dist_vwap_pct` > `0.7352` → IC=+0.300 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7352 (IC base=-0.048)

- **PATRÓN** `volumen_regimen` < `1.1047` → IC=+0.212 (n=227)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.1047 (IC base=-0.048)

- **PATRÓN** `volumen_pendiente_norm` < `0.1972` → IC=+0.217 (n=217)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4353` → IC=+0.251 (n=199)
...
- **FILTRO** `ibs_20min` < `0.6689` → IC=-0.160 (n=1215)
...
- **FILTRO** `libro_spread` > `0.02` → IC=-0.197 (n=384)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=1458)

- **FILTRO** `ibs_20min` > `0.775` → IC=-0.200 (n=692)

  - _Acción_: SKIP cuando `ibs_20min` > 0.775
  - _Potencial_: sin este filtro IC_bueno=+0.033 (n=2083)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1692` → IC=+0.294 (n=66)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1692 (IC base=-0.082)

- **PATRÓN** `volumen_spike_ratio` < `2.5002` → IC=+0.276 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5002 (IC base=-0.082)
...
- **PATRÓN** `dist_vwap_pct` < `0.2574` → IC=+0.247 (n=600)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2574 (IC base=-0.025)
...
- **PATRÓN** `sigma_h` > `0.0093` → IC=+0.184 (n=2736)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0093 (IC base=+0.089)

- **PATRÓN** `ibs_20min` > `0.4593` → IC=+0.180 (n=7325)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` > 0.4593 (IC base=+0.089)

- **PATRÓN** `dist_vwap_pct` > `0.694` → IC=+0.280 (n=763)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.694 (IC base=+0.089)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.503` → IC=+0.148 (n=3881)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 3.503 (IC base=+0.089)

- **PATRÓN** `volumen_regimen` > `0.6792` → IC=+0.241 (n=2506)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6792 (IC base=+0.089)

- **PATRÓN** `volumen_pendiente_norm` > `0.2462` → IC=+0.262 (n=892)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2462 (IC base=+0.089)

- **PATRÓN** `volumen_spike_ratio` < `1.4769` → IC=+0.244 (n=1499)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4769 (IC base=+0.089)

- **PATRÓN** `volumen_spike_ratio` > `2.7492` → IC=+0.240 (n=1499)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7492 (IC base=+0.089)

- **PATRÓN** `ballena_activa_n` < `100.0` → IC=+0.277 (n=3988)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 100.0 (IC base=+0.089)

- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.140 (n=2773)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` > 0.0086 (IC base=+0.069)

- **PATRÓN** `ibs_20min` < `0.5543` → IC=+0.151 (n=7319)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` < 0.5543 (IC base=+0.069)

- **PATRÓN** `dist_vwap_pct` > `0.6674` → IC=+0.237 (n=424)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6674 (IC base=+0.069)

- **PATRÓN** `dist_vwap_pct` < `0.1628` → IC=+0.233 (n=2150)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1628 (IC base=+0.069)

- **PATRÓN** `volumen_regimen` < `0.7166` → IC=+0.237 (n=1021)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7166 (IC base=+0.069)

- **PATRÓN** `volumen_regimen` > `1.2011` → IC=+0.248 (n=773)
...
- **FILTRO** `ibs_20min` > `0.8214` → IC=-0.146 (n=609)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.049 (n=1833)

- **PATRÓN** `dist_vwap_pct` > `0.2885` → IC=+0.148 (n=268)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.2885 (IC base=+0.015)

- **PATRÓN** `dist_vwap_pct` < `0.1605` → IC=+0.137 (n=620)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.1605 (IC base=+0.015)

- **PATRÓN** `volumen_regimen` < `1.0507` → IC=+0.136 (n=638)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 1.0507 (IC base=+0.015)

- **PATRÓN** `volumen_regimen` > `0.85` → IC=+0.151 (n=482)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 0.85 (IC base=+0.015)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4247` → IC=+0.188 (n=235)
  ...
  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` < 1.4247 (IC base=+0.015)

- **PATRÓN** `ballena_activa_n` < `229.0` → IC=+0.191 (n=231)

  - _Acción_: Kelly boost +0.95€ cuando `ballena_activa_n` < 229.0 (IC base=+0.015)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.284 (n=870)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0075 (IC base=+0.240)

- **PATRÓN** `drift_60min` |x|≤ `0.0998` → IC=+0.241 (n=438)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0998 (IC base=+0.240)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.246 (n=656)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.240)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.255 (n=496)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.240)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.298 (n=676)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.240)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.826` → IC=+0.281 (n=317)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.826 (IC base=+0.240)

- **PATRÓN** `volumen_pendiente_norm` < `0.1386` → IC=+0.255 (n=1144)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1386 (IC base=+0.240)

- **PATRÓN** `volumen_spike_ratio` < `1.8724` → IC=+0.244 (n=537)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8724 (IC base=+0.240)

- **PATRÓN** `volumen_spike_ratio` > `3.5812` → IC=+0.256 (n=407)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.5812 (IC base=+0.240)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.257 (n=1494)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.240)

- **PATRÓN** `libro_liquidez` > `1912.1584` → IC=+0.246 (n=435)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1912.1584 (IC base=+0.240)

- **PATRÓN** `sigma_h` > `0.0056` → IC=+0.298 (n=1035)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.3357` → IC=+0.285 (n=1036)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6329` → IC=+0.288 (n=314)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6329 (IC base=+0.280)

- **PATRÓN** `volumen_spike_ratio` > `2.2184` → IC=+0.281 (n=627)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2184 (IC base=+0.280)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.288 (n=583)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.280)

- **PATRÓN** `libro_liquidez` > `1888.9584` → IC=+0.307 (n=345)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1888.9584 (IC base=+0.280)

- **PATRÓN** `ballena_activa_n` < `21.0` → IC=+0.278 (n=412)
...
- **FILTRO** `ibs_20min` > `0.7987` → IC=-0.181 (n=491)
...
- **PATRÓN** `ibs_20min` > `0.8048` → IC=+0.154 (n=521)
  ...
  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` > 0.8048 (IC base=-0.007)

- **PATRÓN** `dist_vwap_pct` > `0.4369` → IC=+0.201 (n=152)
...
- **PATRÓN** `volumen_regimen` < `0.9592` → IC=+0.216 (n=329)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9592 (IC base=-0.007)

- **PATRÓN** `volumen_regimen` > `0.6314` → IC=+0.199 (n=334)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6314 (IC base=-0.007)

- **PATRÓN** `volumen_pendiente_norm` > `0.2633` → IC=+0.286 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2633 (IC base=-0.007)

- **PATRÓN** `volumen_spike_ratio` < `2.0676` → IC=+0.239 (n=305)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0676 (IC base=-0.007)

- **PATRÓN** `volumen_spike_ratio` > `1.3727` → IC=+0.221 (n=346)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.3727 (IC base=-0.007)

- **PATRÓN** `ballena_activa_n` < `70.0` → IC=+0.275 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 70.0 (IC base=-0.007)
...
- **FILTRO** `ibs_20min` < `0.6818` → IC=-0.214 (n=883)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.265 (n=883)

- **FILTRO** `ibs_20min` > `0.7083` → IC=-0.232 (n=460)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7083
  - _Potencial_: sin este filtro IC_bueno=+0.081 (n=1391)

- **FILTRO** `sigma_ewma_delta_pct` > `4.635` → IC=-0.174 (n=427)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.635
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=1424)

- **PATRÓN** `ibs_20min` > `0.6818` → IC=+0.265 (n=883)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6818 (IC base=+0.025)

- **PATRÓN** `dist_vwap_pct` > `0.7895` → IC=+0.336 (n=181)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7895 (IC base=+0.025)
...
- **PATRÓN** `volumen_regimen` > `0.6366` → IC=+0.282 (n=618)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6366 (IC base=+0.025)
...
- **PATRÓN** `dist_vwap_pct` < `0.3963` → IC=+0.193 (n=409)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` < 0.3963 (IC base=+0.003)

- **PATRÓN** `volumen_regimen` < `0.7171` → IC=+0.240 (n=179)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7171 (IC base=+0.003)

- **PATRÓN** `volumen_pendiente_norm` < `0.1008` → IC=+0.178 (n=365)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` < 0.1008 (IC base=+0.003)
...
- **PATRÓN** `volumen_spike_ratio` < `2.629` → IC=+0.199 (n=373)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 2.629 (IC base=+0.003)

- **PATRÓN** `volumen_spike_ratio` > `1.5003` → IC=+0.176 (n=372)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 1.5003 (IC base=+0.003)

- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.216 (n=375)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 56.0 (IC base=+0.003)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` > `0.0158` → IC=+0.321 (n=726)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0158 (IC base=+0.273)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.280 (n=1141)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.273)

- **PATRÓN** `ibs_20min` > `0.9048` → IC=+0.347 (n=728)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9048 (IC base=+0.273)

- **PATRÓN** `dist_vwap_pct` > `0.1852` → IC=+0.316 (n=611)
...
- **PATRÓN** `volumen_regimen` > `0.8579` → IC=+0.298 (n=726)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5463` → IC=+0.279 (n=451)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.277 (n=1135)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.273)

- **PATRÓN** `libro_liquidez` > `2433.9906` → IC=+0.283 (n=973)
...
- **PATRÓN** `sigma_h` > `0.0146` → IC=+0.296 (n=803)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.3953` → IC=+0.306 (n=1205)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3953 (IC base=+0.270)

- **PATRÓN** `dist_vwap_pct` > `0.5326` → IC=+0.289 (n=321)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5474` → IC=+0.264 (n=1041)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5474 (IC base=+0.270)

- **PATRÓN** `volumen_spike_ratio` > `2.17` → IC=+0.274 (n=472)
...
- **PATRÓN** `libro_liquidez` > `2358.368` → IC=+0.276 (n=1077)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2358.368 (IC base=+0.270)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` < `0.0048` → IC=+0.176 (n=2166)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` < 0.0048 (IC base=+0.166)

- **PATRÓN** `sigma_h` > `0.0107` → IC=+0.199 (n=2160)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0107 (IC base=+0.166)

- **PATRÓN** `drift_60min` |x|≤ `0.3415` → IC=+0.174 (n=5702)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.3415 (IC base=+0.166)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.175 (n=6717)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 5.0 (IC base=+0.166)

- **PATRÓN** `ibs_20min` > `0.582` → IC=+0.215 (n=6479)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.582 (IC base=+0.166)

- **PATRÓN** `dist_vwap_pct` > `0.9299` → IC=+0.215 (n=967)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9299 (IC base=+0.166)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.201` → IC=+0.250 (n=1334)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.201 (IC base=+0.166)

- **PATRÓN** `volumen_regimen` < `1.2168` → IC=+0.162 (n=4295)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 1.2168 (IC base=+0.166)

- **PATRÓN** `volumen_regimen` > `0.6254` → IC=+0.158 (n=4295)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 0.6254 (IC base=+0.166)

- **PATRÓN** `volumen_pendiente_norm` > `0.2474` → IC=+0.190 (n=1300)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.2474 (IC base=+0.166)

- **PATRÓN** `volumen_spike_ratio` < `1.5658` → IC=+0.173 (n=2715)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.5658 (IC base=+0.166)

- **PATRÓN** `volumen_spike_ratio` > `2.6612` → IC=+0.172 (n=2056)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 2.6612 (IC base=+0.166)

- **PATRÓN** `libro_liquidez` > `3822.4537` → IC=+0.168 (n=2160)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 3822.4537 (IC base=+0.166)

- **PATRÓN** `ballena_activa_n` < `121.0` → IC=+0.180 (n=5402)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 121.0 (IC base=+0.166)

- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.186 (n=4180)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.0064 (IC base=+0.169)

- **PATRÓN** `drift_60min` |x|≤ `0.079` → IC=+0.204 (n=2087)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4737` → IC=+0.228 (n=6257)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4737 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` < `0.2201` → IC=+0.160 (n=4648)
...
- **PATRÓN** `volumen_regimen` < `1.1835` → IC=+0.153 (n=4577)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1835 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` > `0.6252` → IC=+0.147 (n=4578)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5745` → IC=+0.169 (n=2463)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.5745 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `2.6483` → IC=+0.174 (n=1866)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.6483 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `123.0` → IC=+0.171 (n=5225)

  - _Acción_: Kelly boost +0.86€ cuando `ballena_activa_n` < 123.0 (IC base=+0.169)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4266` → IC=+0.179 (n=1011)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` > 1.4266 (IC base=+0.184)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.203 (n=1059)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1787` → IC=+0.287 (n=524)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1787 (IC base=+0.237)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.243 (n=807)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.237)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.239 (n=791)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.016` → IC=+0.252 (n=852)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.237 (n=823)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.237)

- **PATRÓN** `libro_liquidez` > `1752.6996` → IC=+0.255 (n=524)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.186 (n=994)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 5.0 (IC base=+0.164)

- **PATRÓN** `ibs_20min` > `0.4244` → IC=+0.228 (n=950)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4244 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.1954` → IC=+0.209 (n=559)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1954 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.506` → IC=+0.222 (n=192)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.506 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` < `1.2628` → IC=+0.173 (n=950)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` < 1.2628 (IC base=+0.164)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4198` → IC=+0.198 (n=306)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 1.4198 (IC base=+0.164)

- **PATRÓN** `libro_liquidez` > `10191.6179` → IC=+0.180 (n=950)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 10191.6179 (IC base=+0.164)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.163 (n=1073)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0057 (IC base=+0.142)

- **PATRÓN** `drift_60min` |x|≤ `0.059` → IC=+0.195 (n=359)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.059 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.161 (n=992)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 7.0 (IC base=+0.142)

- **PATRÓN** `ibs_20min` < `0.5437` → IC=+0.188 (n=1073)
...
- **PATRÓN** `volumen_regimen` < `1.2116` → IC=+0.157 (n=1073)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4267` → IC=+0.153 (n=963)
...
- **PATRÓN** `sigma_h` > `0.0059` → IC=+0.200 (n=1078)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0059 (IC base=+0.191)

- **PATRÓN** `drift_60min` |x|≤ `0.1997` → IC=+0.207 (n=719)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.203 (n=1232)
...
- **PATRÓN** `ibs_20min` < `0.35` → IC=+0.254 (n=895)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6443` → IC=+0.216 (n=273)
...
- **PATRÓN** `volumen_regimen` < `1.0519` → IC=+0.155 (n=899)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1749` → IC=+0.154 (n=878)
...
- **PATRÓN** `ibs_20min` < `0.6067` → IC=+0.182 (n=1093)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.842` → IC=+0.183 (n=392)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 6.842 (IC base=+0.130)

- **PATRÓN** `volumen_regimen` < `0.856` → IC=+0.146 (n=729)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.856 (IC base=+0.130)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7931` → IC=+0.133 (n=652)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 1.7931 (IC base=+0.130)
...
- **PATRÓN** `ballena_activa_n` < `174.0` → IC=+0.128 (n=907)
...
- **PATRÓN** `sigma_h` > `0.0079` → IC=+0.145 (n=798)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.5156` → IC=+0.199 (n=1196)

  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` > 0.5156 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `0.7889` → IC=+0.204 (n=353)
...
- **PATRÓN** `volumen_regimen` < `1.2251` → IC=+0.121 (n=1196)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` < 1.2251 (IC base=+0.112)

- **PATRÓN** `volumen_spike_ratio` < `1.8208` → IC=+0.128 (n=766)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` < 1.8208 (IC base=+0.112)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.121 (n=1237)

  - _Acción_: Kelly boost +0.61€ cuando `libro_spread` < 0.02 (IC base=+0.112)

- **PATRÓN** `libro_liquidez` > `2903.4322` → IC=+0.189 (n=542)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 2903.4322 (IC base=+0.112)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1686` → IC=+0.209 (n=497)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.7255` → IC=+0.255 (n=1008)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.349` → IC=+0.242 (n=534)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.349 (IC base=+0.200)

- **PATRÓN** `volumen_regimen` < `1.2075` → IC=+0.206 (n=1129)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2075 (IC base=+0.200)

- **PATRÓN** `volumen_regimen` > `0.8588` → IC=+0.215 (n=752)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8588 (IC base=+0.200)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1881` → IC=+0.214 (n=955)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1881 (IC base=+0.200)

- **PATRÓN** `volumen_spike_ratio` > `1.4263` → IC=+0.206 (n=1085)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4263 (IC base=+0.200)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.202 (n=1162)
...
- **PATRÓN** `sigma_h` > `0.0169` → IC=+0.212 (n=803)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.208 (n=553)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.204)

- **PATRÓN** `ibs_20min` < `0.44` → IC=+0.243 (n=1205)
...
- **PATRÓN** `volumen_regimen` > `0.6261` → IC=+0.216 (n=1205)
...
- **PATRÓN** `ibs_20min` > `0.3929` → IC=+0.171 (n=1582)
...
- **PATRÓN** `volumen_regimen` < `0.8708` → IC=+0.155 (n=912)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.8708 (IC base=+0.139)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.143 (n=1768)

  - _Acción_: Kelly boost +0.72€ cuando `libro_spread` < 0.02 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `8668.3588` → IC=+0.151 (n=717)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 8668.3588 (IC base=+0.139)

- **PATRÓN** `ballena_activa_n` < `161.0` → IC=+0.158 (n=1360)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 161.0 (IC base=+0.139)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.135 (n=1114)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0057 (IC base=+0.114)

- **PATRÓN** `drift_60min` |x|≤ `0.3399` → IC=+0.128 (n=1470)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.64€ cuando `drift_60min` |x|≤ 0.3399 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.129 (n=1683)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 5.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` < `0.4893` → IC=+0.154 (n=1470)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.4893 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.856` → IC=+0.127 (n=649)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` > 3.856 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` < `1.2207` → IC=+0.121 (n=1480)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_regimen` < 1.2207 (IC base=+0.114)

- **PATRÓN** `volumen_pendiente_norm` > `0.1668` → IC=+0.139 (n=425)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_pendiente_norm` > 0.1668 (IC base=+0.114)

- **PATRÓN** `volumen_spike_ratio` < `2.2358` → IC=+0.133 (n=1409)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` < 2.2358 (IC base=+0.114)

### GBM_LATE_15M_PYCONFIRMADO#BTC#15min
- **PATRÓN** `drift_60min` |x|≤ `0.3479` → IC=+0.126 (n=375)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.63€ cuando `drift_60min` |x|≤ 0.3479 (IC base=+0.105)

- **PATRÓN** `hora_utc` > `9.0` → IC=+0.142 (n=342)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` > 9.0 (IC base=+0.105)

- **PATRÓN** `ibs_20min` > `0.2558` → IC=+0.139 (n=375)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` > 0.2558 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` > `0.2828` → IC=+0.127 (n=124)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` > 0.2828 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.328` → IC=+0.138 (n=172)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` > 3.328 (IC base=+0.105)

- **PATRÓN** `volumen_regimen` < `0.6135` → IC=+0.146 (n=125)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.6135 (IC base=+0.105)

- **PATRÓN** `libro_liquidez` > `10454.9515` → IC=+0.131 (n=375)

  - _Acción_: Kelly boost +0.66€ cuando `libro_liquidez` > 10454.9515 (IC base=+0.105)

- **PATRÓN** `ballena_activa_n` < `151.0` → IC=+0.152 (n=116)

  - _Acción_: Kelly boost +0.76€ cuando `ballena_activa_n` < 151.0 (IC base=+0.105)

- **PATRÓN** `sigma_h` < `0.0027` → IC=+0.209 (n=177)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0027 (IC base=+0.131)

- **PATRÓN** `drift_60min` |x|≤ `0.3374` → IC=+0.148 (n=523)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.3374 (IC base=+0.131)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.152 (n=467)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 7.0 (IC base=+0.131)

- **PATRÓN** `ibs_20min` < `0.3445` → IC=+0.201 (n=349)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3445 (IC base=+0.131)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `9.093` → IC=+0.133 (n=594)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` < 9.093 (IC base=+0.131)

- **PATRÓN** `volumen_regimen` < `1.2046` → IC=+0.134 (n=523)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 1.2046 (IC base=+0.131)

- **PATRÓN** `volumen_regimen` > `0.7193` → IC=+0.148 (n=467)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` > 0.7193 (IC base=+0.131)

- **PATRÓN** `volumen_pendiente_norm` > `0.1571` → IC=+0.213 (n=148)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1571 (IC base=+0.131)

- **PATRÓN** `volumen_spike_ratio` < `2.4411` → IC=+0.151 (n=514)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 2.4411 (IC base=+0.131)

- **PATRÓN** `volumen_spike_ratio` > `1.4106` → IC=+0.143 (n=513)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 1.4106 (IC base=+0.131)

- **PATRÓN** `ballena_activa_n` < `152.0` → IC=+0.171 (n=165)

  - _Acción_: Kelly boost +0.85€ cuando `ballena_activa_n` < 152.0 (IC base=+0.131)
...
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.124 (n=418)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` < 0.0067 (IC base=+0.100)

- **PATRÓN** `drift_60min` |x|≤ `0.0974` → IC=+0.148 (n=140)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.0974 (IC base=+0.100)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.128 (n=283)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 11.0 (IC base=+0.100)

- **PATRÓN** `ibs_20min` < `0.3071` → IC=+0.155 (n=279)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.3071 (IC base=+0.100)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.432` → IC=+0.149 (n=112)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 6.432 (IC base=+0.100)

- **PATRÓN** `volumen_pendiente_norm` > `0.1654` → IC=+0.163 (n=99)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.1654 (IC base=+0.100)

- **PATRÓN** `volumen_spike_ratio` > `1.8125` → IC=+0.134 (n=266)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 1.8125 (IC base=+0.100)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.193 (n=3728)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0088 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.173 (n=8545)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 5.0 (IC base=+0.165)

- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.214 (n=8228)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4706 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` > `0.8836` → IC=+0.195 (n=1072)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.8836 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.562` → IC=+0.224 (n=4023)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.562 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` < `0.8847` → IC=+0.165 (n=3679)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.8847 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` > `0.2392` → IC=+0.194 (n=1551)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2392 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `2.6304` → IC=+0.184 (n=2615)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` > 2.6304 (IC base=+0.165)

- **PATRÓN** `libro_liquidez` > `3789.8405` → IC=+0.170 (n=2741)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 3789.8405 (IC base=+0.165)

- **PATRÓN** `ballena_activa_n` < `92.0` → IC=+0.193 (n=6045)

  - _Acción_: Kelly boost +0.96€ cuando `ballena_activa_n` < 92.0 (IC base=+0.165)

- **PATRÓN** `sigma_h` < `0.0068` → IC=+0.194 (n=5028)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0068 (IC base=+0.180)

- **PATRÓN** `drift_60min` |x|≤ `0.4812` → IC=+0.183 (n=7526)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.91€ cuando `drift_60min` |x|≤ 0.4812 (IC base=+0.180)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.204 (n=2871)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.180)

- **PATRÓN** `ibs_20min` < `0.5633` → IC=+0.238 (n=7526)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5633 (IC base=+0.180)

- **PATRÓN** `dist_vwap_pct` < `0.2306` → IC=+0.162 (n=4760)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2306 (IC base=+0.180)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.683` → IC=+0.181 (n=7282)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.683 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` < `0.7055` → IC=+0.158 (n=2294)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.7055 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` > `1.2053` → IC=+0.156 (n=1738)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 1.2053 (IC base=+0.180)

- **PATRÓN** `volumen_pendiente_norm` > `0.2885` → IC=+0.243 (n=985)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2885 (IC base=+0.180)

- **PATRÓN** `volumen_spike_ratio` > `2.2944` → IC=+0.189 (n=3078)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 2.2944 (IC base=+0.180)

- **PATRÓN** `ballena_activa_n` < `25.0` → IC=+0.191 (n=2174)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3305` → IC=+0.191 (n=1392)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.3305 (IC base=+0.190)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.197 (n=680)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 15.0 (IC base=+0.190)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.203 (n=936)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 11.0 (IC base=+0.190)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.323 (n=495)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.190)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.821` → IC=+0.315 (n=630)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.821 (IC base=+0.190)
...
- **PATRÓN** `ibs_20min` < `0.3509` → IC=+0.286 (n=944)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3509 (IC base=+0.255)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.416` → IC=+0.262 (n=1129)
...
- **PATRÓN** `volumen_spike_ratio` > `1.88` → IC=+0.277 (n=645)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.88 (IC base=+0.255)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.255 (n=1124)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.255)

- **PATRÓN** `libro_liquidez` > `1750.37` → IC=+0.271 (n=715)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1750.37 (IC base=+0.255)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1853` → IC=+0.157 (n=873)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.1853 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.164 (n=1355)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 5.0 (IC base=+0.151)

- **PATRÓN** `ibs_20min` > `0.3156` → IC=+0.202 (n=1308)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3156 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` > `0.3236` → IC=+0.195 (n=500)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.3236 (IC base=+0.151)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.285` → IC=+0.155 (n=1163)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 4.285 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` < `0.6977` → IC=+0.182 (n=576)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.6977 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` < `0.0734` → IC=+0.155 (n=1136)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_pendiente_norm` < 0.0734 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` > `0.2652` → IC=+0.184 (n=188)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.2652 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` < `2.1166` → IC=+0.159 (n=1103)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.1166 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` > `1.7577` → IC=+0.161 (n=836)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.7577 (IC base=+0.151)

- **PATRÓN** `libro_liquidez` > `10989.1304` → IC=+0.168 (n=1169)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 10989.1304 (IC base=+0.151)

- **PATRÓN** `ballena_activa_n` < `486.0` → IC=+0.162 (n=1185)

  - _Acción_: Kelly boost +0.81€ cuando `ballena_activa_n` < 486.0 (IC base=+0.151)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.167 (n=1162)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0057 (IC base=+0.151)

- **PATRÓN** `drift_60min` |x|≤ `0.2618` → IC=+0.166 (n=1023)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.2618 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.175 (n=527)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 16.0 (IC base=+0.151)

- **PATRÓN** `ibs_20min` < `0.6376` → IC=+0.203 (n=1162)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6376 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` > `0.6839` → IC=+0.167 (n=172)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.6839 (IC base=+0.151)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `11.52` → IC=+0.170 (n=207)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 11.52 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` < `1.2004` → IC=+0.161 (n=1162)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2004 (IC base=+0.151)
...
- **PATRÓN** `volumen_spike_ratio` < `2.409` → IC=+0.164 (n=1065)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.409 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` > `2.1054` → IC=+0.158 (n=483)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 2.1054 (IC base=+0.151)

- **PATRÓN** `libro_liquidez` > `12624.5033` → IC=+0.151 (n=775)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 12624.5033 (IC base=+0.151)

- **PATRÓN** `ballena_activa_n` < `365.0` → IC=+0.163 (n=645)

  - _Acción_: Kelly boost +0.82€ cuando `ballena_activa_n` < 365.0 (IC base=+0.151)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0059` → IC=+0.229 (n=1315)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0059 (IC base=+0.212)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.217 (n=1370)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.212)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.215 (n=1330)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.212)

- **PATRÓN** `ibs_20min` > `0.6721` → IC=+0.252 (n=1175)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6721 (IC base=+0.212)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.548` → IC=+0.299 (n=392)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.548 (IC base=+0.212)

- **PATRÓN** `volumen_pendiente_norm` < `0.2147` → IC=+0.217 (n=1276)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2147 (IC base=+0.212)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.226 (n=1514)
...
- **PATRÓN** `sigma_h` < `0.0105` → IC=+0.239 (n=1243)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.3651` → IC=+0.266 (n=1093)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7946` → IC=+0.227 (n=492)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7946 (IC base=+0.233)

- **PATRÓN** `volumen_spike_ratio` > `2.2483` → IC=+0.231 (n=745)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2483 (IC base=+0.233)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.244 (n=713)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.233)

- **PATRÓN** `libro_liquidez` > `1902.2979` → IC=+0.243 (n=414)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1902.2979 (IC base=+0.233)

- **PATRÓN** `ballena_activa_n` < `15.0` → IC=+0.258 (n=365)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4353` → IC=+0.137 (n=1398)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.69€ cuando `drift_60min` |x|≤ 0.4353 (IC base=+0.134)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.144 (n=1454)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 5.0 (IC base=+0.134)

- **PATRÓN** `ibs_20min` > `0.8819` → IC=+0.261 (n=634)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8819 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` > `0.5525` → IC=+0.173 (n=374)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.5525 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.171` → IC=+0.161 (n=588)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 4.171 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` < `0.8782` → IC=+0.155 (n=932)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5119` → IC=+0.147 (n=593)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.5119 (IC base=+0.134)

- **PATRÓN** `volumen_spike_ratio` > `1.4087` → IC=+0.148 (n=1344)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.4087 (IC base=+0.134)

- **PATRÓN** `libro_liquidez` > `8514.6397` → IC=+0.226 (n=634)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8514.6397 (IC base=+0.134)

- **PATRÓN** `ballena_activa_n` < `85.0` → IC=+0.163 (n=428)

  - _Acción_: Kelly boost +0.81€ cuando `ballena_activa_n` < 85.0 (IC base=+0.134)

- **PATRÓN** `sigma_h` < `0.0075` → IC=+0.162 (n=1133)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0075 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.4443` → IC=+0.159 (n=1133)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.149 (n=516)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 7.0 (IC base=+0.141)

- **PATRÓN** `ibs_20min` < `0.6874` → IC=+0.196 (n=1133)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` < 0.6874 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` < `0.2045` → IC=+0.144 (n=1050)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.2045 (IC base=+0.141)
...
- **PATRÓN** `volumen_regimen` < `0.6969` → IC=+0.149 (n=499)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.6969 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` > `1.1774` → IC=+0.153 (n=378)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.1774 (IC base=+0.141)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4421` → IC=+0.157 (n=1062)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.4421 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `11048.3545` → IC=+0.203 (n=378)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11048.3545 (IC base=+0.141)

- **PATRÓN** `ballena_activa_n` < `189.0` → IC=+0.151 (n=1056)

  - _Acción_: Kelly boost +0.76€ cuando `ballena_activa_n` < 189.0 (IC base=+0.141)

### GBM_LATE_15M_TARDIO#SOL#15min
- **FILTRO** `ibs_20min` > `0.6458` → IC=-0.170 (n=452)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6458
  - _Potencial_: sin este filtro IC_bueno=+0.202 (n=1364)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1292` → IC=+0.157 (n=455)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.1292 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.148 (n=640)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 15.0 (IC base=+0.110)

- **PATRÓN** `ibs_20min` < `0.6458` → IC=+0.202 (n=1364)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6458 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.4735` → IC=+0.126 (n=1325)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` < 0.4735 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.451` → IC=+0.124 (n=1311)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 3.451 (IC base=+0.110)

- **PATRÓN** `volumen_regimen` < `0.7212` → IC=+0.149 (n=600)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 0.7212 (IC base=+0.110)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4685` → IC=+0.148 (n=402)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.4685 (IC base=+0.110)

- **PATRÓN** `libro_liquidez` > `2886.578` → IC=+0.158 (n=454)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 2886.578 (IC base=+0.110)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` > `0.0243` → IC=+0.219 (n=639)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0243 (IC base=+0.205)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.210 (n=1466)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.205)

- **PATRÓN** `ibs_20min` > `0.5111` → IC=+0.245 (n=1409)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5111 (IC base=+0.205)

- **PATRÓN** `dist_vwap_pct` > `0.1844` → IC=+0.229 (n=795)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1844 (IC base=+0.205)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.372` → IC=+0.249 (n=680)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.372 (IC base=+0.205)

- **PATRÓN** `volumen_regimen` < `1.2478` → IC=+0.208 (n=1409)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2478 (IC base=+0.205)

- **PATRÓN** `volumen_regimen` > `0.6312` → IC=+0.208 (n=1409)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6312 (IC base=+0.205)

- **PATRÓN** `volumen_pendiente_norm` > `0.0795` → IC=+0.234 (n=565)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.212 (n=1434)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.205)

- **PATRÓN** `libro_liquidez` > `2587.456` → IC=+0.208 (n=939)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2231` → IC=+0.193 (n=1210)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 2.2231 (IC base=+0.199)

- **PATRÓN** `volumen_spike_ratio` > `1.4438` → IC=+0.199 (n=1375)
...
- **PATRÓN** `libro_liquidez` > `2566.217` → IC=+0.200 (n=1037)
...
- **PATRÓN** `ibs_20min` > `0.625` → IC=+0.207 (n=613)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.625 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` > `0.1307` → IC=+0.171 (n=323)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.1307 (IC base=+0.105)
...
- **PATRÓN** `volumen_regimen` < `1.0909` → IC=+0.128 (n=613)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4899` → IC=+0.155 (n=505)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.4899 (IC base=+0.105)

- **PATRÓN** `volumen_spike_ratio` > `1.4022` → IC=+0.137 (n=505)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 1.4022 (IC base=+0.105)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.135 (n=497)

  - _Acción_: Kelly boost +0.68€ cuando `libro_spread` < 0.02 (IC base=+0.105)

- **PATRÓN** `libro_liquidez` > `2468.387` → IC=+0.151 (n=267)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 2468.387 (IC base=+0.105)

- **PATRÓN** `ibs_20min` < `0.0714` → IC=+0.282 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0714 (IC base=-0.026)

- **PATRÓN** `volumen_pendiente_norm` > `0.0631` → IC=+0.179 (n=76)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.0631 (IC base=-0.026)

- **PATRÓN** `volumen_spike_ratio` < `2.6098` → IC=+0.135 (n=165)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 2.6098 (IC base=-0.026)
...
- **PATRÓN** `ibs_20min` < `0.7371` → IC=+0.184 (n=93)

  - _Acción_: Kelly boost +0.92€ cuando `ibs_20min` < 0.7371 (IC base=+0.048)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.698` → IC=+0.179 (n=76)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` < 4.698 (IC base=+0.048)

- **PATRÓN** `volumen_regimen` < `0.9536` → IC=+0.143 (n=82)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 0.9536 (IC base=+0.048)

- **PATRÓN** `volumen_pendiente_norm` > `0.0826` → IC=+0.214 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0826 (IC base=+0.048)

- **PATRÓN** `volumen_spike_ratio` < `2.3987` → IC=+0.194 (n=70)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 2.3987 (IC base=+0.048)

- **PATRÓN** `libro_liquidez` > `3541.649` → IC=+0.175 (n=38)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 3541.649 (IC base=+0.048)
...
- **PATRÓN** `sigma_h` < `0.005` → IC=+0.175 (n=164)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` > `0.6355` → IC=+0.235 (n=213)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6355 (IC base=+0.123)

- **PATRÓN** `dist_vwap_pct` > `0.1205` → IC=+0.187 (n=113)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.1205 (IC base=+0.123)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.087` → IC=+0.311 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.087 (IC base=+0.123)

- **PATRÓN** `volumen_regimen` < `1.0679` → IC=+0.151 (n=213)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.0679 (IC base=+0.123)

- **PATRÓN** `volumen_regimen` > `0.6313` → IC=+0.151 (n=190)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7434` → IC=+0.176 (n=109)
...
- **PATRÓN** `volumen_spike_ratio` > `1.3908` → IC=+0.165 (n=162)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.3908 (IC base=+0.123)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.156 (n=222)

  - _Acción_: Kelly boost +0.78€ cuando `libro_spread` < 0.02 (IC base=+0.123)

- **PATRÓN** `libro_liquidez` > `1101.9191` → IC=+0.192 (n=186)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 1101.9191 (IC base=+0.123)

- **PATRÓN** `ibs_20min` < `0.0943` → IC=+0.231 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0943 (IC base=-0.068)

- **PATRÓN** `volumen_pendiente_norm` > `0.0767` → IC=+0.136 (n=20)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_pendiente_norm` > 0.0767 (IC base=-0.068)

- **PATRÓN** `volumen_spike_ratio` > `2.7298` → IC=+0.200 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7298 (IC base=-0.068)
...
- **PATRÓN** `py_entrada` > `0.5` → IC=+0.132 (n=519)

  - _Acción_: Kelly boost +0.66€ cuando `py_entrada` > 0.5 (IC base=+0.116)

- **PATRÓN** `libro_liquidez` > `2846.2392` → IC=+0.185 (n=179)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 2846.2392 (IC base=+0.116)

- **PATRÓN** `libro_liquidez` > `2326.7299` → IC=+0.123 (n=593)

  - _Acción_: Kelly boost +0.62€ cuando `libro_liquidez` > 2326.7299 (IC base=+0.099)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=1385)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=6794)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.179 (n=2825)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.053 (n=8654)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.172 (n=2914)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=9005)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.44` → IC=-0.213 (n=483)

  - _Acción_: SKIP cuando `py_entrada` < 0.44
  - _Potencial_: sin este filtro IC_bueno=+0.096 (n=1470)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.171 (n=496)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=1631)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.43` → IC=-0.205 (n=480)

  - _Acción_: SKIP cuando `py_entrada` < 0.43
  - _Potencial_: sin este filtro IC_bueno=+0.089 (n=1515)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.221 (n=496)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.061 (n=1609)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.199 (n=466)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.076 (n=1460)

- **FILTRO** `py_entrada` > `0.58` → IC=-0.191 (n=513)

  - _Acción_: SKIP cuando `py_entrada` > 0.58
  - _Potencial_: sin este filtro IC_bueno=+0.052 (n=1595)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=2577)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.127 (n=8268)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.082 (n=18421)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.281 (n=6344)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=20345)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.242 (n=6635)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=20054)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.160 (n=9063)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=17626)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.230 (n=7879)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=25662)

- **FILTRO** `ibs_7min` > `0.2962` → IC=-0.177 (n=8384)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2962
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=25157)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.308 (n=1029)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=3292)

- **FILTRO** `ibs_7min` < `0.7071` → IC=-0.260 (n=1425)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7071
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=2896)

- **FILTRO** `ballena_activa_n` > `9.0` → IC=-0.198 (n=982)

  - _Acción_: SKIP cuando `ballena_activa_n` > 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=3339)

- **FILTRO** `py_entrada` > `0.505` → IC=-0.150 (n=3912)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=1910)

- **FILTRO** `drift_7min_pct` |x|> `0.1111` → IC=-0.124 (n=1978)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1111
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=3844)

- **FILTRO** `ibs_7min` > `0.8` → IC=-0.204 (n=1454)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=4368)
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.259 (n=1106)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=3548)

- **FILTRO** `ibs_7min` < `0.7547` → IC=-0.188 (n=1163)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7547
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=3491)

- **FILTRO** `ballena_activa_n` > `161.0` → IC=-0.177 (n=1162)

  - _Acción_: SKIP cuando `ballena_activa_n` > 161.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3492)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.258 (n=1135)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=3597)

- **FILTRO** `ibs_7min` > `0.2551` → IC=-0.171 (n=1182)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2551
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=3550)

- **FILTRO** `ballena_activa_n` > `153.0` → IC=-0.184 (n=1177)

  - _Acción_: SKIP cuando `ballena_activa_n` > 153.0
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=3555)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.171 (n=1159)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.089 (n=2880)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.315 (n=975)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=3064)

- **FILTRO** `ibs_7min` < `0.2` → IC=-0.266 (n=1002)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=3037)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.220 (n=927)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=3112)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.235 (n=1442)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=4701)

- **FILTRO** `ibs_7min` > `0.2602` → IC=-0.155 (n=2088)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2602
  - _Potencial_: sin este filtro IC_bueno=+0.016 (n=4055)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `py_entrada` < `0.35` → IC=-0.251 (n=1083)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=3322)

- **FILTRO** `ibs_7min` < `0.7425` → IC=-0.189 (n=1100)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7425
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3305)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.185 (n=1089)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.075 (n=3316)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.260 (n=1115)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=3391)

- **FILTRO** `ibs_7min` > `0.2739` → IC=-0.174 (n=1126)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2739
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=3380)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.181 (n=1100)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=3406)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.37` → IC=-0.259 (n=1163)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=3603)

- **FILTRO** `ibs_7min` < `0.7143` → IC=-0.231 (n=1176)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=3590)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.172 (n=1558)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=4792)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.35` → IC=-0.278 (n=1119)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=3385)

- **FILTRO** `ibs_7min` < `0.7196` → IC=-0.232 (n=1126)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7196
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=3378)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.213 (n=1091)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3413)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.206 (n=1421)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=+0.010 (n=4567)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.324 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.007 (n=989)
...
### ORDER_FLOW_5M
- **PATRÓN** `delta_ratio` |x|> `0.3982` → IC=+0.138 (n=661)
...
- **PATRÓN** `total_vol_5m` < `453.526` → IC=+0.164 (n=221)

  - _Acción_: Kelly boost +0.82€ cuando `total_vol_5m` < 453.526 (IC base=+0.122)

- **PATRÓN** `libro_liquidez` > `3728.9067` → IC=+0.129 (n=300)

  - _Acción_: Kelly boost +0.65€ cuando `libro_liquidez` > 3728.9067 (IC base=+0.122)

- **PATRÓN** `ballena_activa_n` < `60.0` → IC=+0.132 (n=552)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.177 (n=156)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 5.0 (IC base=+0.136)

- **PATRÓN** `total_vol_5m` < `417.524` → IC=+0.142 (n=135)

  - _Acción_: Kelly boost +0.71€ cuando `total_vol_5m` < 417.524 (IC base=+0.136)
...
- **PATRÓN** `delta_ratio` |x|> `0.4` → IC=+0.170 (n=113)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.85€ cuando `delta_ratio` |x|> 0.4 (IC base=+0.123)

- **PATRÓN** `hora_utc` < `13.0` → IC=+0.155 (n=114)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 13.0 (IC base=+0.123)

- **PATRÓN** `total_vol_5m` < `356326.0` → IC=+0.146 (n=111)

  - _Acción_: Kelly boost +0.73€ cuando `total_vol_5m` < 356326.0 (IC base=+0.123)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.240 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.123)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.128 (n=41)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.012 (n=989)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=2470)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.154 (n=24)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=1282)

### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.193 (n=395)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0041 (IC base=+0.180)

- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.218 (n=537)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0078 (IC base=+0.180)

- **PATRÓN** `drift_60min` |x|≤ `0.0527` → IC=+0.188 (n=395)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.0527 (IC base=+0.180)

- **PATRÓN** `delta_ratio_macro` |x|> `0.058` → IC=+0.182 (n=1184)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.91€ cuando `delta_ratio_macro` |x|> 0.058 (IC base=+0.180)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1373` → IC=+0.236 (n=381)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1373 (IC base=+0.180)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.188 (n=835)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 11.0 (IC base=+0.180)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.193 (n=574)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 6.0 (IC base=+0.180)

- **PATRÓN** `ibs_15` > `0.6179` → IC=+0.254 (n=1183)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6179 (IC base=+0.180)

- **PATRÓN** `dist_vwap_pct` < `0.5472` → IC=+0.175 (n=1224)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.5472 (IC base=+0.180)

- **PATRÓN** `sigma_ewma_delta_pct` > `17.106` → IC=+0.262 (n=313)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 17.106 (IC base=+0.180)

- **PATRÓN** `libro_liquidez` > `4681.5356` → IC=+0.183 (n=537)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 4681.5356 (IC base=+0.180)
...
- **PATRÓN** `sigma_h` > `0.005` → IC=+0.197 (n=97)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.005 (IC base=+0.197)

- **PATRÓN** `drift_60min` |x|≤ `0.0587` → IC=+0.278 (n=97)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0587 (IC base=+0.197)

- **PATRÓN** `drift_15min` |x|≤ `0.3775` → IC=+0.217 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.3775 (IC base=+0.197)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2237` → IC=+0.248 (n=145)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.2237 (IC base=+0.197)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.216 (n=301)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.197)

- **PATRÓN** `ibs_15` > `0.7061` → IC=+0.254 (n=291)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7061 (IC base=+0.197)

- **PATRÓN** `dist_vwap_pct` > `0.3722` → IC=+0.253 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3722 (IC base=+0.197)

- **PATRÓN** `dist_vwap_pct` < `0.0989` → IC=+0.200 (n=201)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0989 (IC base=+0.197)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.624` → IC=+0.248 (n=169)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.624 (IC base=+0.197)

- **PATRÓN** `libro_liquidez` > `15571.5606` → IC=+0.247 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 15571.5606 (IC base=+0.197)
...
- **PATRÓN** `sigma_h` > `0.0159` → IC=+0.244 (n=221)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0159 (IC base=+0.187)

- **PATRÓN** `drift_60min` |x|≤ `0.0851` → IC=+0.209 (n=146)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0851 (IC base=+0.187)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0439` → IC=+0.201 (n=332)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0439 (IC base=+0.187)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1209` → IC=+0.274 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1209 (IC base=+0.187)

- **PATRÓN** `hora_utc` < `4.0` → IC=+0.241 (n=114)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 4.0 (IC base=+0.187)

- **PATRÓN** `ibs_15` > `0.5455` → IC=+0.281 (n=332)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5455 (IC base=+0.187)

- **PATRÓN** `dist_vwap_pct` > `0.1142` → IC=+0.197 (n=196)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1142 (IC base=+0.187)

- **PATRÓN** `dist_vwap_pct` < `0.3071` → IC=+0.191 (n=318)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.3071 (IC base=+0.187)
...
- **PATRÓN** `libro_spread` < `0.03` → IC=+0.188 (n=366)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.03 (IC base=+0.187)

- **PATRÓN** `libro_liquidez` > `2846.2656` → IC=+0.270 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2846.2656 (IC base=+0.187)

- **PATRÓN** `ibs_15` < `0.1143` → IC=+0.168 (n=378)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.84€ cuando `ibs_15` < 0.1143 (IC base=+0.045)
...
- **FILTRO** `sigma_h` > `0.0129` → IC=-0.209 (n=558)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0129
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=1678)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.179 (n=718)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=1518)

- **FILTRO** `libro_liquidez` < `3802.1817` → IC=-0.124 (n=1475)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1167` → IC=+0.236 (n=843)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1167 (IC base=-0.042)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1825` → IC=+0.240 (n=810)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1825 (IC base=-0.042)

- **PATRÓN** `ibs_15` < `0.361` → IC=+0.280 (n=1265)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.361 (IC base=-0.042)

- **PATRÓN** `dist_vwap_pct` > `0.6428` → IC=+0.263 (n=196)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6428 (IC base=-0.042)

### UPDOWN_GBM_15M_TARDIO#BTC#15min
- **FILTRO** `sigma_h` > `0.007` → IC=-0.208 (n=337)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.007
  - _Potencial_: sin este filtro IC_bueno=-0.194 (n=1015)

- **FILTRO** `sigma_h` < `0.0037` → IC=-0.230 (n=446)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0037
  - _Potencial_: sin este filtro IC_bueno=-0.182 (n=906)
...
- **FILTRO** `sigma_ewma_delta_pct` > `19.957` → IC=-0.241 (n=241)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 19.957
  - _Potencial_: sin este filtro IC_bueno=-0.188 (n=1111)

- **PATRÓN** `sigma_h` < `0.0027` → IC=+0.172 (n=120)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0027 (IC base=+0.073)
...
- **FILTRO** `sigma_ewma_delta_pct` > `18.123` → IC=-0.132 (n=278)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=2231)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1964` → IC=+0.195 (n=162)

  - _Acción_: Kelly boost +0.98€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1964 (IC base=-0.044)

- **PATRÓN** `ibs_15` < `0.3667` → IC=+0.252 (n=252)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **PATRÓN** `drift_60min` |x|≤ `0.0567` → IC=+0.313 (n=185)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0567 (IC base=+0.287)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1417` → IC=+0.289 (n=368)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1417 (IC base=+0.287)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.289 (n=677)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.287)

- **PATRÓN** `libro_liquidez` > `12762.7474` → IC=+0.298 (n=251)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6179 sube el IC de +0.180 a +0.254 en UPDOWN_GBM#15min (n=1183). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.7061 sube el IC de +0.197 a +0.254 en UPDOWN_GBM#BTC#15min (n=291). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_YES, IBS > 0.5455 sube el IC de +0.187 a +0.281 en UPDOWN_GBM#XRP#15min (n=332). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1143 sube el IC de +0.045 a +0.168 en UPDOWN_GBM#XRP#15min (n=378). Ya aplicado como kelly_boost=+0.84€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.361 sube el IC de -0.042 a +0.280 en UPDOWN_GBM_15M_TARDIO (n=1265). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.2757 sube el IC de +0.228 a +0.288 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=464). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#SOL#15min**: dentro de BUY_NO, IBS < 0.3667 sube el IC de -0.044 a +0.252 en UPDOWN_GBM_15M_TARDIO#SOL#15min (n=252). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.8374 sube el IC de +0.287 a +0.325 en UPDOWN_GBM_IBS_ALTO (n=552). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 21626 | -0.096 | -3113.41€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1282 | -0.050 | -190.91€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 20344 | -0.099 | -2922.49€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3334 | -0.100 | -570.32€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3334 | -0.100 | -570.32€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 1282 | -0.050 | -190.91€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 1282 | -0.050 | -190.91€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 374 | -0.136 | -161.05€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 374 | -0.136 | -161.05€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 6205 | -0.043 | -605.33€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 6205 | -0.043 | -605.33€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 5728 | -0.096 | -475.64€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 5728 | -0.096 | -475.64€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 4703 | -0.174 | -1110.16€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 4703 | -0.174 | -1110.16€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 13620 | -0.038 | +4190.98€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 3622 | -0.005 | +1832.61€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 9998 | -0.050 | +2358.37€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 13620 | -0.038 | +4190.98€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 3622 | -0.005 | +1832.61€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 9998 | -0.050 | +2358.37€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO |
44€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 2026 | +0.048 | +233.07€ | 0 | 10 |
| ✅ UPDOWN_GBM#ETH#240min | 280 | +0.007 | +7.92€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 2857 | +0.003 | -0.88€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 596 | +0.000 | -4.04€ | 2 | 0 |
| ✅ UPDOWN_GBM#ETH#daily | 36 | -0.158 | +3.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 6964 | +0.017 | +165.96€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 1974 | +0.021 | +108.16€ | 0 | 11 |
| ✅ UPDOWN_GBM#SOL#240min | 274 | -0.007 | -2.70€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 4342 | +0.018 | +61.29€ | 1 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 344 | +0.012 | +0.30€ | 0 | 1 |
| ✅ UPDOWN_GBM#SOL#daily | 30 | -0.156 | -1.10€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 4116 | +0.041 | +466.18€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 1942 | +0.077 | +471.82€ | 0 | 12 |
| ✅ UPDOWN_GBM#XRP#240min | 154 | -0.006 | -2.82€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 2020 | +0.009 | -2.83€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 9637 | -0.046 | +2005.08€ | 3 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 9637 | -0.046 | +2005.08€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 484 | -0.051 | +335.03€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 484 | -0.051 | +335.03€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 1828 | -0.127 | +24.93€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 1828 | -0.127 | +24.93€ | 4 | 5 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 179 | +0.135 | +84.77€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 179 | +0.135 | +84.77€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 1061 | +0.198 | +607.84€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 1061 | +0.198 | +607.84€ | 2 | 23 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 3047 | -0.064 | +451.87€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 3047 | -0.064 | +451.87€ | 4 | 6 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 3038 | -0.075 | +500.64€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 3038 | -0.075 | +500.64€ | 2 | 3 |
...
| ✅ UPDOWN_GBM_IBS_ALTO | 736 | +0.287 | +597.51€ | 0 | 11 |
| ✅ UPDOWN_GBM_IBS_ALTO#15min | 736 | +0.287 | +597.51€ | 0 | 0 |
...
**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  ...
  - _Estado_: 470 celda(s) pasan gate riguroso completo de 2072 evaluadas (n>=40) y 3051 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.021 < 0.08 — monitorear
  - _Datos_: n=1972 IC=+0.021 PNL=+107.14€
...
**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  ...
  - _Estado_: alineada_con_outcome_prev IC=+0.097 n=204/60 | contraria IC=+0.153 n=191 | gap=-0.056 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=595/40 IC=+0.001 PNL=-3.53€ | BTC#60min: n=746/40 IC=+0.000 PNL=+7.68€ | SOL#60min: n=343/40 IC=+0.013 PNL=+0.81€

**⏳ H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  - _Umbral_: n≥40 tras 2 losses y gap(IC_tras_win - IC_tras_2loss)≥0.05
  - _Acción_: Reducir stake (no desactivar) 1-2h tras 2 derrotas consecutivas en el mismo subtype
  - _Estado_: tras_win IC=+0.053 n=276267 | tras_1loss IC=+0.073 n=215741 | tras_2loss IC=+0.041 n=92144/40 | gap=+0.012 (umbral 0.05)
...
**🟡 H-24H-GBM-BUYYES-MADRUGADA** — GBM BUY_YES en madrugada europea (05-07h UTC) — señal alcista
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.190 > 0.08 con n=227 PNL=+160.68€
  - _Datos_: n=227 IC=+0.190 PNL=+160.68€
...
**〰️ H-CUSTOM-GBM-60MIN-BUYYES** — GBM 60min BUY_YES — ¿edge superior al BUY_NO?
  ...
  - _Estado_: n=1231 IC=+0.007 PNL=-2.10€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1231 IC=+0.007 PNL=-2.10€
...
**🟡 H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.180 > 0.1 con n=1577 PNL=+944.33€
  - _Datos_: n=1577 IC=+0.180 PNL=+944.33€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=912 IC=+0.079 PNL=+205.17€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=912 IC=+0.079 PNL=+205.17€
...
**🟡 H-CUSTOM-OF-EDGE-ALTO** — ORDER_FLOW_5M: edge alto (>0.20) rinde mejor que edge cerca del suelo
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.131 > 0.02 con n=578 PNL=+239.98€
  - _Datos_: n=578 IC=+0.131 PNL=+239.98€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=9257 IC=+0.051 PNL=+1088.53€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=9257 IC=+0.051 PNL=+1088.53€
...
**🔴 H-CUSTOM-BUYNO-LONGSHOT-15MIN** — BUY_NO longshot en 15min (py_mkt≥0.55) — comprar NO barato pierde
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.171 < -0.1 con n=174 PNL=+11.06€
  - _Datos_: n=174 IC=-0.171 PNL=+11.06€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=1498 IC=+0.044 PNL=+161.69€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1498 IC=+0.044 PNL=+161.69€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=14436 IC=-0.141 PNL=+789.79€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=14436 IC=-0.141 PNL=+789.79€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=1598 IC=+0.136 PNL=+830.96€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=1598 IC=+0.136 PNL=+830.96€
...
**〰️ H-CUSTOM-GBM-04H-ASIA** — UPDOWN_GBM 04h-05h UTC — media sesión asiática, ¿mejor franja nocturna?
  ...
  - _Estado_: n=2906 IC=+0.015 PNL=+82.20€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=2906 IC=+0.015 PNL=+82.20€
...
**〰️ H-CUSTOM-LATE-04H-ASIA** — GBM_LATE_15M 04h-05h UTC — media sesión asiática (gemela de GBM-04H-ASIA)
  ...
  - _Estado_: n=1704 IC=+0.080 PNL=+861.94€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1704 IC=+0.080 PNL=+861.94€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.241 < -0.1 con n=1447 PNL=-192.04€
  - _Datos_: n=1447 IC=-0.241 PNL=-192.04€
...
**🔶 H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.099 n=790) pero sin cruzar ≥2 pares más — sin otros pares con datos
  - _Datos_: n=790 IC=+0.099 PNL=+206.92€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=7402 IC=+0.172 PNL=-918.29€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=7402 IC=+0.172 PNL=-918.29€
