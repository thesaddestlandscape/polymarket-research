# Hipótesis automáticas — 2026-07-18 20:46 UTC
_Generado por shadow_postmortem.py sobre 21240 resoluciones (PNL=+5070.45€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.191 (n=1186)
...
- **PATRÓN** `py_entrada` > `0.695` → IC=+0.298 (n=582)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.195 (n=512)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.265` → IC=+0.346 (n=473)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.265 (IC base=+0.176)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.182 (n=1561)

  - _Acción_: Kelly boost +0.91€ cuando `libro_spread` < 0.01 (IC base=+0.176)

- **PATRÓN** `libro_liquidez` > `7387.9559` → IC=+0.178 (n=458)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 7387.9559 (IC base=+0.176)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.223 (n=99)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.235` → IC=+0.389 (n=106)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.127 (n=1780)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 8.0 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` > `0.7393` → IC=+0.140 (n=134)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.7393 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.024` → IC=+0.216 (n=160)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.024 (IC base=+0.114)

- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.130 (n=1417)
...
- **PATRÓN** `dist_vwap_pct` > `0.1641` → IC=+0.128 (n=329)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` > 0.1641 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` < `0.9162` → IC=+0.122 (n=1041)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.9162 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.189` → IC=+0.144 (n=223)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` > 6.189 (IC base=+0.112)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `dist_vwap_pct` > `0.501` → IC=+0.230 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.501 (IC base=+0.096)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.388` → IC=+0.217 (n=44)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.388 (IC base=+0.096)

- **PATRÓN** `dist_vwap_pct` > `0.1355` → IC=+0.150 (n=58)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.1355 (IC base=+0.073)

- **PATRÓN** `dist_vwap_pct` < `0.49` → IC=+0.128 (n=216)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.49 (IC base=+0.073)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.887` → IC=+0.188 (n=94)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 2.887 (IC base=+0.073)
...
- **PATRÓN** `sigma_h` > `0.0118` → IC=+0.235 (n=342)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0118 (IC base=+0.164)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.185 (n=683)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1732` → IC=+0.205 (n=368)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1732 (IC base=+0.164)
...
- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.133 (n=246)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.0064 (IC base=+0.129)

- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.149 (n=220)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0029 (IC base=+0.129)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.179 (n=166)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 13.0 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` > `0.5737` → IC=+0.186 (n=33)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.5737 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.924` → IC=+0.244 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.924 (IC base=+0.129)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0139` → IC=+0.205 (n=313)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0139 (IC base=+0.129)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.136 (n=863)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5282` → IC=+0.186 (n=173)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.5282 (IC base=+0.129)
...
- **PATRÓN** `dist_vwap_pct` < `0.7895` → IC=+0.126 (n=905)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` < 0.7895 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.506` → IC=+0.126 (n=236)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` > 3.506 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.972` → IC=+0.121 (n=797)

  - _Acción_: Kelly boost +0.60€ cuando `sigma_ewma_delta_pct` < 10.972 (IC base=+0.104)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0044` → IC=+0.148 (n=143)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0044 (IC base=+0.113)

- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.120 (n=214)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.60€ cuando `sigma_h` > 0.0029 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.157 (n=196)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 8.0 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `0.5186` → IC=+0.230 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5186 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.72` → IC=+0.250 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.72 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.136 (n=86)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 17.0 (IC base=+0.080)
...
- **PATRÓN** `ibs_15` > `0.7353` → IC=+0.189 (n=297)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.94€ cuando `ibs_15` > 0.7353 (IC base=+0.033)

- **PATRÓN** `dist_vwap_pct` > `0.7162` → IC=+0.174 (n=44)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.7162 (IC base=+0.033)

- **PATRÓN** `dist_vwap_pct` < `0.2815` → IC=+0.140 (n=170)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.2815 (IC base=+0.033)

- **PATRÓN** `sigma_ewma_delta_pct` > `17.819` → IC=+0.134 (n=39)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` > 17.819 (IC base=+0.033)
...
- **PATRÓN** `drift_60min` |x|≤ `0.237` → IC=+0.120 (n=214)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.60€ cuando `drift_60min` |x|≤ 0.237 (IC base=+0.073)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.146 (n=162)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 12.0 (IC base=+0.073)

- **PATRÓN** `ibs_15` > `0.6428` → IC=+0.178 (n=175)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.89€ cuando `ibs_15` > 0.6428 (IC base=+0.073)

- **PATRÓN** `dist_vwap_pct` > `0.7268` → IC=+0.278 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7268 (IC base=+0.073)

- **PATRÓN** `sigma_ewma_delta_pct` < `13.739` → IC=+0.156 (n=62)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` < 13.739 (IC base=+0.073)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS > 0.7353 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.189 n=297). Confirma señal de reversión media → alinear con BUY_NO.
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS > 0.6428 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.178 n=175). Confirma señal de reversión media → alinear con BUY_NO.
...
| ✅ FAVORITO_CONFIRMADO | 3531 | +0.183 | +26.98€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 2295 | +0.217 | +76.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 151 | +0.075 | +3.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 482 | +0.093 | -80.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 603 | +0.153 | +26.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 1027 | +0.194 | +32.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 766 | +0.210 | -0.24€ | 0 | 6 |
...
| ✅ GBM_LATE_15M | 5621 | +0.105 | +1865.70€ | 0 | 8 |
| ✅ GBM_LATE_15M#15min | 5621 | +0.105 | +1865.70€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1345 | +0.075 | +238.49€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1345 | +0.075 | +238.49€ | 0 | 5 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2848 | +0.150 | +1678.01€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2848 | +0.150 | +1678.01€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 728 | +0.115 | +340.17€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 728 | +0.115 | +340.17€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 2790 | +0.102 | +982.15€ | 0 | 10 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2790 | +0.102 | +982.15€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 678 | +0.071 | +142.45€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 678 | +0.071 | +142.45€ | 0 | 7 |
...
| ✅ UPDOWN_GBM | 1781 | +0.021 | +131.07€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1521 | +0.046 | +169.12€ | 1 | 7 |
...
| ✅ UPDOWN_GBM#BTC | 450 | +0.024 | +27.64€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 367 | +0.061 | +41.61€ | 1 | 7 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 501 | +0.182 | +216.35€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 501 | +0.182 | +216.35€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 40 | +0.191 | +15.70€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 40 | +0.191 | +15.70€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 57 | +0.076 | +7.51€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 57 | +0.076 | +7.51€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.178: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.051 n=689 | neutral: IC=-0.007 n=505 | overbought(IBS>0.7): IC=+0.171 n=628
  - _Datos_: n=1949 IC=+0.082 PNL=+382.14€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.116 n=12726 | tras_1loss IC=+0.085 n=8318 | tras_2loss IC=+0.052 n=3417/40 | gap=+0.064 (umbral 0.05)
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=-0.004 n=135 | contrario_BTC IC=-0.062 n=62/40 | gap=-0.059 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1652 IC=+0.028 PNL=+148.45€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1652 IC=+0.028 PNL=+148.45€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=768 IC=+0.033 PNL=+60.52€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=768 IC=+0.033 PNL=+60.52€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=367 IC=+0.061 PNL=+41.61€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=367 IC=+0.061 PNL=+41.61€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=802 IC=+0.080 PNL=+118.53€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=802 IC=+0.080 PNL=+118.53€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=18782 IC=+0.119 PNL=+5146.33€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=18782 IC=+0.119 PNL=+5146.33€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=918 IC=+0.002 PNL=+29.97€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=918 IC=+0.002 PNL=+29.97€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.161 > 0.1 con n=502 PNL=+172.53€
  - _Datos_: n=502 IC=+0.161 PNL=+172.53€
...
**〰️ H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: n=177 IC=+0.059 PNL=+5.15€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=177 IC=+0.059 PNL=+5.15€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=708 IC=+0.066 PNL=+101.11€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=708 IC=+0.066 PNL=+101.11€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.137 > 0.08 con n=364 PNL=+106.77€
  - _Datos_: n=364 IC=+0.137 PNL=+106.77€
...
**🟡 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.179 > 0.08 con n=166 PNL=+62.32€
  - _Datos_: n=166 IC=+0.179 PNL=+62.32€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.229 > 0.2 con n=466 PNL=+292.52€
  - _Datos_: n=466 IC=+0.229 PNL=+292.52€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.154 > 0.08 con n=261 PNL=+120.55€
  - _Datos_: n=261 IC=+0.154 PNL=+120.55€
