# Hipótesis automáticas — 2026-07-16 05:24 UTC
_Generado por shadow_postmortem.py sobre 16504 resoluciones (PNL=+3346.70€)_
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.232 (n=196)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.62` → IC=+0.284 (n=169)
...
### GBM_LATE_15M
- **PATRÓN** `dist_vwap_pct` > `0.9339` → IC=+0.133 (n=96)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.9339 (IC base=+0.109)
...
- **PATRÓN** `sigma_h` > `0.0238` → IC=+0.175 (n=201)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0238 (IC base=+0.148)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.157 (n=467)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 5.0 (IC base=+0.148)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.154 (n=299)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.221 (n=328)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.166 (n=744)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 18.0 (IC base=+0.159)

- **PATRÓN** `dist_vwap_pct` > `0.1497` → IC=+0.195 (n=280)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1497 (IC base=+0.159)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.007` → IC=+0.311 (n=88)
...
- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.244 (n=88)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0068 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.159 (n=136)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 12.0 (IC base=+0.142)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.163 (n=203)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 18.0 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.2519` → IC=+0.171 (n=74)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.2519 (IC base=+0.142)
...
- **PATRÓN** `sigma_h` > `0.0127` → IC=+0.214 (n=89)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0127 (IC base=+0.141)
...
- **PATRÓN** `dist_vwap_pct` > `0.3907` → IC=+0.192 (n=50)
  ...
  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.3907 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` < `1.0527` → IC=+0.149 (n=146)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 1.0527 (IC base=+0.141)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.082` → IC=+0.285 (n=63)
...
- **PATRÓN** `sigma_h` < `0.0214` → IC=+0.279 (n=156)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0214 (IC base=+0.251)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.252 (n=143)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.251)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.294 (n=139)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.251)

- **PATRÓN** `dist_vwap_pct` > `0.7553` → IC=+0.326 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7553 (IC base=+0.251)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.511` → IC=+0.361 (n=34)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.511 (IC base=+0.251)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.015` → IC=+0.190 (n=224)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.015 (IC base=+0.114)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.124 (n=698)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` < 18.0 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` > `0.3431` → IC=+0.174 (n=173)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.3431 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` < `0.1365` → IC=+0.153 (n=237)
...
- **PATRÓN** `sigma_h` < `0.0045` → IC=+0.141 (n=76)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0045 (IC base=+0.058)

- **PATRÓN** `dist_vwap_pct` < `0.2621` → IC=+0.133 (n=47)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.2621 (IC base=+0.058)

- **PATRÓN** `sigma_ewma_delta_pct` > `14.551` → IC=+0.382 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 14.551 (IC base=+0.058)
...
- **PATRÓN** `sigma_h` > `0.0115` → IC=+0.216 (n=195)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0115 (IC base=+0.202)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.202 (n=206)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.3134` → IC=+0.272 (n=55)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3134 (IC base=+0.202)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.107` → IC=+0.229 (n=68)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.107 (IC base=+0.202)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2438 | +0.171 | -72.93€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1568 | +0.208 | +4.23€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 867 | +0.172 | -6.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 519 | +0.214 | +15.65€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 856 | +0.166 | -45.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 516 | +0.210 | +9.64€ | 0 | 6 |
...
| ✅ GBM_LATE_15M | 4791 | +0.102 | +1496.50€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 4791 | +0.102 | +1496.50€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#SOL | 1281 | +0.110 | +519.42€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1281 | +0.110 | +519.42€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1210 | +0.155 | +607.43€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1210 | +0.155 | +607.43€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2028 | +0.149 | +1179.07€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2028 | +0.149 | +1179.07€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 518 | +0.108 | +245.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 518 | +0.108 | +245.98€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 538 | +0.130 | +257.82€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 538 | +0.130 | +257.82€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 548 | +0.111 | +240.73€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 548 | +0.111 | +240.73€ | 1 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 424 | +0.270 | +434.54€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 424 | +0.270 | +434.54€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 1996 | +0.091 | +603.22€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1996 | +0.091 | +603.22€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 492 | +0.059 | +90.77€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 492 | +0.059 | +90.77€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 459 | +0.025 | +18.11€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 459 | +0.025 | +18.11€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 477 | +0.059 | +95.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 477 | +0.059 | +95.98€ | 3 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 568 | +0.197 | +398.36€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 568 | +0.197 | +398.36€ | 0 | 12 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.101 n=9577 | tras_1loss IC=+0.068 n=6710 | tras_2loss IC=+0.036 n=2869/40 | gap=+0.065 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=14046 IC=+0.105 PNL=+3422.59€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=14046 IC=+0.105 PNL=+3422.59€
...
**🟡 H-CUSTOM-LATE-04H-ASIA** — GBM_LATE_15M 04h-05h UTC — media sesión asiática (gemela de GBM-04H-ASIA)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.131 > 0.08 con n=405 PNL=+160.24€
  - _Datos_: n=405 IC=+0.131 PNL=+160.24€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.248 > 0.2 con n=343 PNL=+236.82€
  - _Datos_: n=343 IC=+0.248 PNL=+236.82€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.197 > 0.08 con n=130 PNL=+0.80€
  - _Datos_: n=130 IC=+0.197 PNL=+0.80€
