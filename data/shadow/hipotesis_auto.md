# Hipótesis automáticas — 2026-08-28 00:21 UTC
_Generado por shadow_postmortem.py sobre 180898 resoluciones (PNL=+13105.36€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `147.51` → IC=-0.285 (n=2522)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 147.51
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=7569)
...
- **FILTRO** `restante_s_al_confirmar` < `161.14` → IC=-0.172 (n=682)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=2048)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.158 (n=4369)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` < 7.0 (IC base=+0.142)

- **PATRÓN** `py_entrada` < `0.34` → IC=+0.258 (n=3489)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.34 (IC base=+0.142)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.187 (n=3018)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.02 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `1888.2922` → IC=+0.179 (n=2528)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 1888.2922 (IC base=+0.142)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.191 (n=3942)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 15.0 (IC base=+0.187)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.200 (n=2227)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.191 (n=9007)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` < 12.0 (IC base=+0.191)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.220 (n=9205)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.129 (n=1660)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` < 12.0 (IC base=+0.126)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.154 (n=1813)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.166 (n=1540)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.229 (n=972)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.244 (n=1249)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.191 (n=1517)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 12.0 (IC base=+0.185)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.212 (n=1590)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.151 (n=2248)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.139 (n=1135)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `ibs_20min` < `0.2156` → IC=+0.132 (n=1597)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.2156 (IC base=+0.029)

- **PATRÓN** `dist_vwap_pct` < `0.3014` → IC=+0.142 (n=805)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.3014 (IC base=+0.029)

- **PATRÓN** `volumen_regimen` < `0.6267` → IC=+0.157 (n=260)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.6267 (IC base=+0.029)

- **PATRÓN** `volumen_regimen` > `1.0483` → IC=+0.140 (n=354)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 1.0483 (IC base=+0.029)

- **PATRÓN** `volumen_pendiente_norm` > `0.3027` → IC=+0.274 (n=135)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3027 (IC base=+0.029)

- **PATRÓN** `volumen_spike_ratio` < `1.9478` → IC=+0.183 (n=598)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` < 1.9478 (IC base=+0.029)

- **PATRÓN** `volumen_spike_ratio` > `2.9099` → IC=+0.231 (n=299)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9099 (IC base=+0.029)

- **PATRÓN** `ballena_activa_n` < `86.0` → IC=+0.218 (n=416)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 86.0 (IC base=+0.029)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0744` → IC=+0.162 (n=152)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.147 (n=482)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 18.0 (IC base=+0.138)

- **PATRÓN** `ibs_20min` < `0.4406` → IC=+0.172 (n=400)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.4406 (IC base=+0.138)
...
- **PATRÓN** `volumen_regimen` < `0.6267` → IC=+0.188 (n=152)
  ...
  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` < 0.6267 (IC base=+0.138)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7479` → IC=+0.175 (n=235)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 1.7479 (IC base=+0.138)
...
- **PATRÓN** `libro_liquidez` > `13573.7225` → IC=+0.175 (n=207)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 13573.7225 (IC base=+0.138)

- **PATRÓN** `ballena_activa_n` < `220.0` → IC=+0.189 (n=88)

  - _Acción_: Kelly boost +0.94€ cuando `ballena_activa_n` < 220.0 (IC base=+0.138)
...
- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.154 (n=53)

  - _Acción_: Kelly boost +0.77€ cuando `ballena_activa_n` < 135.0 (IC base=-0.049)
...
- **FILTRO** `ibs_20min` < `0.4355` → IC=-0.171 (n=347)
...
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.138 (n=1347)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `drift_60min` |x|≤ `0.3373` → IC=+0.158 (n=2265)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_spike_ratio` < `2.2796` → IC=+0.155 (n=2044)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.2796 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` > `1.8485` → IC=+0.152 (n=1548)
...
- **PATRÓN** `ibs_20min` < `0.4098` → IC=+0.237 (n=2288)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4098 (IC base=+0.183)

- **PATRÓN** `dist_vwap_pct` < `0.2145` → IC=+0.177 (n=1839)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2236` → IC=+0.175 (n=404)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4241` → IC=+0.188 (n=459)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` < 0.4241 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `0.7019` → IC=+0.145 (n=74)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.7019 (IC base=+0.144)
...
- **PATRÓN** `volumen_regimen` < `0.6392` → IC=+0.229 (n=153)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6392 (IC base=+0.144)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5404` → IC=+0.157 (n=351)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.5404 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `14483.9491` → IC=+0.158 (n=153)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 14483.9491 (IC base=+0.144)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3806` → IC=+0.162 (n=439)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.3411` → IC=+0.211 (n=386)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3411 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` < `0.1505` → IC=+0.161 (n=446)
...
- **PATRÓN** `volumen_regimen` < `1.1582` → IC=+0.158 (n=439)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.0996` → IC=+0.177 (n=131)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.0996 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` < `1.8946` → IC=+0.177 (n=221)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 1.8946 (IC base=+0.143)

- **PATRÓN** `libro_liquidez` > `10334.4562` → IC=+0.155 (n=146)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 10334.4562 (IC base=+0.143)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.195 (n=1358)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_spike_ratio` < `2.3027` → IC=+0.149 (n=2355)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.155 (n=2427)

  - _Acción_: Kelly boost +0.78€ cuando `libro_spread` < 0.01 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `2502.6824` → IC=+0.175 (n=1997)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 2502.6824 (IC base=+0.149)

- **PATRÓN** `ballena_activa_n` < `71.0` → IC=+0.196 (n=1085)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 71.0 (IC base=+0.149)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.219 (n=1797)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0066 (IC base=+0.190)

- **PATRÓN** `drift_60min` |x|≤ `0.4735` → IC=+0.202 (n=2695)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.208 (n=960)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.7287` → IC=+0.181 (n=2146)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.7287 (IC base=+0.190)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.366` → IC=+0.208 (n=392)
...
- **PATRÓN** `volumen_regimen` > `1.197` → IC=+0.179 (n=668)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` > 1.197 (IC base=+0.190)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2922` → IC=+0.208 (n=905)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1713` → IC=+0.193 (n=298)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.97€ cuando `drift_60min` |x|≤ 0.1713 (IC base=+0.165)
...
- **PATRÓN** `ibs_20min` < `0.3841` → IC=+0.211 (n=393)
...
- **PATRÓN** `volumen_regimen` < `0.6237` → IC=+0.242 (n=149)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4777` → IC=+0.191 (n=351)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` < 2.4777 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `1.5054` → IC=+0.182 (n=313)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 1.5054 (IC base=+0.165)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.165 (n=577)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.165)

- **PATRÓN** `libro_liquidez` > `12242.4641` → IC=+0.173 (n=203)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 12242.4641 (IC base=+0.165)
...
- **PATRÓN** `sigma_h` < `0.0106` → IC=+0.138 (n=545)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` > `0.9295` → IC=+0.175 (n=207)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` > 0.9295 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.496` → IC=+0.125 (n=564)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 3.496 (IC base=+0.119)

- **PATRÓN** `volumen_regimen` > `1.0516` → IC=+0.136 (n=218)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 1.0516 (IC base=+0.119)

- **PATRÓN** `volumen_pendiente_norm` > `0.2806` → IC=+0.204 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2806 (IC base=+0.119)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2014` → IC=+0.133 (n=276)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 2.2014 (IC base=+0.119)

- **PATRÓN** `libro_liquidez` > `9436.689` → IC=+0.133 (n=281)

  - _Acción_: Kelly boost +0.66€ cuando `libro_liquidez` > 9436.689 (IC base=+0.119)
...
- **FILTRO** `libro_liquidez` < `12303.8073` → IC=-0.210 (n=29)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.152 (n=87)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.130 (n=79)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 14.0 (IC base=+0.059)

- **PATRÓN** `volumen_regimen` > `0.9152` → IC=+0.214 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.9152 (IC base=+0.059)

- **PATRÓN** `libro_liquidez` > `12303.8073` → IC=+0.152 (n=87)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 12303.8073 (IC base=+0.059)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.215 (n=838)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=2694)

- **FILTRO** `ibs_20min` > `0.2727` → IC=-0.179 (n=882)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2727
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=2650)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.139 (n=2135)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **FILTRO** `py_entrada` < `0.36` → IC=-0.284 (n=2137)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=6711)

- **FILTRO** `ibs_7min` < `0.7333` → IC=-0.232 (n=2206)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7333
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=6642)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.174 (n=3002)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=5846)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.231 (n=2483)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=8248)

- **FILTRO** `ibs_7min` > `0.7241` → IC=-0.173 (n=2681)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7241
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=8050)
...
- **FILTRO** `py_entrada` > `0.61` → IC=-0.225 (n=409)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=-0.007 (n=1319)

- **FILTRO** `ballena_activa_n` > `94.0` → IC=-0.159 (n=585)
...
- **FILTRO** `ibs_7min` > `0.8192` → IC=-0.196 (n=468)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=1407)
...
- **FILTRO** `hora_utc` > `18.0` → IC=-0.141 (n=318)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=1160)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.276 (n=350)
...
- **FILTRO** `ballena_activa_n` > `26.0` → IC=-0.180 (n=492)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=986)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.43` → IC=-0.228 (n=420)

  - _Acción_: SKIP cuando `py_entrada` < 0.43
  - _Potencial_: sin este filtro IC_bueno=+0.014 (n=1278)
...
- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.189 (n=413)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=1285)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.266 (n=378)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=1442)
...
- **FILTRO** `sigma_ewma_delta_pct` > `14.492` → IC=-0.184 (n=36)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.138 (n=222)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.177 (n=60)
...
- **FILTRO** `divergencia_cvd_spot_perp` |x|> `0.1682` → IC=-0.157 (n=33)
...
| ✅ BALLENAS_TARDIAS | 10091 | -0.095 | -1603.21€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 721 | -0.071 | -127.98€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 9370 | -0.096 | -1475.22€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#ETH | 2730 | -0.068 | -263.99€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 2730 | -0.068 | -263.99€ | 1 | 0 |
...
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 126 | -0.094 | +37.53€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 35 | -0.013 | +15.13€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 91 | -0.124 | +22.40€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 126 | -0.094 | +37.53€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min |