# Hipótesis automáticas — 2026-07-21 19:54 UTC
_Generado por shadow_postmortem.py sobre 27400 resoluciones (PNL=+6246.18€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.204 (n=1533)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.204 (n=1924)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.201)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.172 (n=955)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 15.0 (IC base=+0.169)
...
- **PATRÓN** `py_entrada` > `0.625` → IC=+0.284 (n=332)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.231 (n=340)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.173 (n=96)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.595` → IC=+0.175 (n=75)

  - _Acción_: Kelly boost +0.88€ cuando `py_entrada` > 0.595 (IC base=+0.167)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.189 (n=101)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.252 (n=131)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1865.1754` → IC=+0.241 (n=249)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1865.1754 (IC base=+0.239)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.212 (n=196)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.23` → IC=+0.335 (n=131)
...
- **PATRÓN** `sigma_h` > `0.0133` → IC=+0.130 (n=846)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` > 0.0133 (IC base=+0.100)

- **PATRÓN** `dist_vwap_pct` > `0.1827` → IC=+0.124 (n=429)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` > 0.1827 (IC base=+0.100)
...
- **PATRÓN** `dist_vwap_pct` > `1.1709` → IC=+0.155 (n=27)
  ...
  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 1.1709 (IC base=+0.069)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.471` → IC=+0.183 (n=99)

  - _Acción_: Kelly boost +0.92€ cuando `sigma_ewma_delta_pct` > 7.471 (IC base=+0.069)
...
- **PATRÓN** `sigma_h` < `0.0107` → IC=+0.122 (n=411)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `sigma_h` < `0.0095` → IC=+0.171 (n=229)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0095 (IC base=+0.157)

- **PATRÓN** `sigma_h` > `0.0264` → IC=+0.171 (n=229)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0264 (IC base=+0.157)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.164 (n=266)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 17.0 (IC base=+0.157)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `9.332` → IC=+0.139 (n=436)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` < 9.332 (IC base=+0.157)
...
- **PATRÓN** `sigma_h` > `0.0073` → IC=+0.169 (n=930)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` > 0.0073 (IC base=+0.139)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.172 (n=514)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.9879` → IC=+0.164 (n=117)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.9879 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` < `0.7664` → IC=+0.146 (n=1340)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.7664 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.158` → IC=+0.159 (n=265)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 6.158 (IC base=+0.139)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.146 (n=176)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 15.0 (IC base=+0.100)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.527` → IC=+0.167 (n=124)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 4.527 (IC base=+0.100)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.143 (n=180)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` > 15.0 (IC base=+0.099)

- **PATRÓN** `dist_vwap_pct` > `0.7083` → IC=+0.143 (n=68)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.7083 (IC base=+0.099)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.58` → IC=+0.182 (n=127)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 4.58 (IC base=+0.099)
...
- **PATRÓN** `sigma_h` < `0.0107` → IC=+0.135 (n=286)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0107 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` < `0.7524` → IC=+0.132 (n=289)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.7524 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.311` → IC=+0.149 (n=243)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `9.664` → IC=+0.246 (n=368)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 9.664 (IC base=+0.248)
...
- **PATRÓN** `sigma_h` > `0.0238` → IC=+0.200 (n=148)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0238 (IC base=+0.185)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.213 (n=165)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.3534` → IC=+0.247 (n=81)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3534 (IC base=+0.185)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.216` → IC=+0.189 (n=423)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` < 10.216 (IC base=+0.185)
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 632 | +0.121 | +11.38€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 632 | +0.121 | +11.38€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 233 | +0.121 | -4.32€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 233 | +0.121 | -4.32€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 245 | +0.140 | +4.03€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 245 | +0.140 | +4.03€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 4727 | +0.184 | +58.38€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 3084 | +0.215 | +99.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 202 | +0.069 | -4.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 625 | +0.104 | -84.91€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 816 | +0.154 | +47.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 1377 | +0.195 | +50.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1028 | +0.211 | +5.03€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 67 | +0.094 | +3.50€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 272 | +0.179 | +45.39€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#ETH | 1671 | +0.174 | -8.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1016 | +0.212 | +30.46€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 67 | -0.022 | -14.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 316 | +0.113 | -36.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 272 | +0.146 | +12.98€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#SOL | 1655 | +0.185 | +14.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1020 | +0.225 | +66.12€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 231 | +0.311 | +9.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 231 | +0.311 | +9.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 231 | +0.311 | +9.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 231 | +0.311 | +9.32€ | 0 | 0 |
| ✅ GBM_LATE_15M | 6610 | +0.098 | +2114.54€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 6610 | +0.098 | +2114.54€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1580 | +0.070 | +281.03€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1580 | +0.070 | +281.03€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1502 | +0.072 | +260.95€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1502 | +0.072 | +260.95€ | 0 | 6 |
| ✅ GBM_LATE_15M#SOL | 1786 | +0.089 | +637.69€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1786 | +0.089 | +637.69€ | 1 | 4 |
| ✅ GBM_LATE_15M#XRP | 1742 | +0.156 | +934.87€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1742 | +0.156 | +934.87€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 3806 | +0.133 | +2041.83€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 3806 | +0.133 | +2041.83€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 968 | +0.096 | +396.41€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 968 | +0.096 | +396.41€ | 0 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 991 | +0.094 | +386.40€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 991 | +0.094 | +386.40€ | 0 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 964 | +0.105 | +412.82€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 964 | +0.105 | +412.82€ | 0 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 883 | +0.249 | +846.19€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 883 | +0.249 | +846.19€ | 0 | 12 |
...
| ✅ GBM_LATE_15M_TARDIO | 3777 | +0.086 | +1213.21€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 3777 | +0.086 | +1213.21€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 904 | +0.052 | +157.86€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 904 | +0.052 | +157.86€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 911 | +0.028 | +80.22€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 911 | +0.028 | +80.22€ | 0 | 2 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 862 | +0.046 | +181.78€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 862 | +0.046 | +181.78€ | 0 | 1 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 1100 | +0.193 | +793.35€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1100 | +0.193 | +793.35€ | 0 | 13 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1031 | +0.167 | +466.53€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1031 | +0.167 | +466.53€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 216 | +0.161 | +67.06€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 216 | +0.161 | +67.06€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 252 | +0.095 | +70.52€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 252 | +0.095 | +70.52€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.186: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.055 n=950 | neutral: IC=+0.005 n=745 | overbought(IBS>0.7): IC=+0.191 n=956
  - _Datos_: n=2835 IC=+0.096 PNL=+674.76€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.114 n=16376 | tras_1loss IC=+0.083 n=10821 | tras_2loss IC=+0.049 n=4475/40 | gap=+0.064 (umbral 0.05)
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=+0.088 n=1196/40 PNL=+255.63€ | H=15h UTC: IC=+0.148 n=1181/40 PNL=+443.93€ | H=17h UTC: IC=+0.092 n=1140/40 PNL=+216.18€ | H=19h UTC: IC=+0.116 n=1072/40 PNL=+266.60€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=24932 IC=+0.113 PNL=+6324.07€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=24932 IC=+0.113 PNL=+6324.07€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=868 IC=+0.053 PNL=+108.20€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=868 IC=+0.053 PNL=+108.20€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.149 > 0.08 con n=471 PNL=+278.80€
  - _Datos_: n=471 IC=+0.149 PNL=+278.80€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.203 > 0.08 con n=267 PNL=+13.01€
  - _Datos_: n=267 IC=+0.203 PNL=+13.01€
