# Hipótesis automáticas — 2026-09-03 19:32 UTC
_Generado por shadow_postmortem.py sobre 270534 resoluciones (PNL=+25746.76€)_
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.190 (n=7876)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.197 (n=6087)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.203 (n=2898)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.176 (n=2513)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.204 (n=2872)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3571` → IC=+0.129 (n=3614)

  - _Acción_: Kelly boost +0.65€ cuando `ibs_20min` > 0.3571 (IC base=+0.081)

- **PATRÓN** `dist_vwap_pct` > `0.2656` → IC=+0.224 (n=462)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2656 (IC base=+0.081)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.566` → IC=+0.153 (n=1383)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 5.566 (IC base=+0.081)

- **PATRÓN** `volumen_regimen` < `0.626` → IC=+0.225 (n=274)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.626 (IC base=+0.081)

- **PATRÓN** `volumen_regimen` > `1.0855` → IC=+0.244 (n=373)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0855 (IC base=+0.081)

- **PATRÓN** `volumen_pendiente_norm` > `0.1083` → IC=+0.178 (n=796)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.1083 (IC base=+0.081)

- **PATRÓN** `volumen_spike_ratio` < `2.88` → IC=+0.177 (n=2151)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 2.88 (IC base=+0.081)

- **PATRÓN** `volumen_spike_ratio` > `1.4745` → IC=+0.172 (n=2152)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.4745 (IC base=+0.081)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.203 (n=1069)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2773` → IC=+0.156 (n=803)
...
- **PATRÓN** `volumen_spike_ratio` < `2.6507` → IC=+0.140 (n=706)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.190 (n=640)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.04 (IC base=+0.155)

- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.189 (n=455)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1606` → IC=+0.218 (n=640)
...
- **PATRÓN** `libro_liquidez` > `11871.8913` → IC=+0.240 (n=564)
...
- **PATRÓN** `dist_vwap_pct` > `0.3632` → IC=+0.281 (n=30)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3632 (IC base=-0.037)

- **PATRÓN** `dist_vwap_pct` < `0.1735` → IC=+0.278 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1735 (IC base=-0.037)
...
- **PATRÓN** `volumen_regimen` > `1.1445` → IC=+0.278 (n=34)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1445 (IC base=-0.037)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4376` → IC=+0.346 (n=24)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4376 (IC base=-0.037)
...
- **FILTRO** `ibs_20min` < `0.28` → IC=-0.156 (n=59)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.142 (n=121)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.177 (n=60)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0066 (IC base=+0.044)

- **PATRÓN** `ibs_20min` > `0.28` → IC=+0.142 (n=121)

  - _Acción_: Kelly boost +0.71€ cuando `ibs_20min` > 0.28 (IC base=+0.044)

- **PATRÓN** `dist_vwap_pct` < `0.5682` → IC=+0.278 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5682 (IC base=+0.044)
...
- **PATRÓN** `volumen_spike_ratio` < `1.9974` → IC=+0.281 (n=30)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9974 (IC base=+0.044)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.174 (n=253)
...
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.174 (n=1492)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0094 (IC base=+0.071)

- **PATRÓN** `ibs_20min` > `0.2889` → IC=+0.140 (n=4471)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` > 0.2889 (IC base=+0.071)

- **PATRÓN** `dist_vwap_pct` > `1.188` → IC=+0.296 (n=327)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.188 (IC base=+0.071)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.472` → IC=+0.121 (n=2362)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` > 2.472 (IC base=+0.071)

- **PATRÓN** `volumen_regimen` > `0.68` → IC=+0.218 (n=1295)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.68 (IC base=+0.071)

- **PATRÓN** `volumen_pendiente_norm` < `0.115` → IC=+0.216 (n=2086)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.115 (IC base=+0.071)

- **PATRÓN** `volumen_pendiente_norm` > `0.2528` → IC=+0.232 (n=423)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8344` → IC=+0.227 (n=719)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8344 (IC base=+0.071)

- **PATRÓN** `ballena_activa_n` < `94.0` → IC=+0.303 (n=1611)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 94.0 (IC base=+0.071)

- **PATRÓN** `ibs_20min` < `0.5834` → IC=+0.131 (n=4329)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.5834 (IC base=+0.052)
...
- **FILTRO** `sigma_ewma_delta_pct` > `2.404` → IC=-0.164 (n=319)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 2.404
  - _Potencial_: sin este filtro IC_bueno=+0.030 (n=700)
...
- **PATRÓN** `ballena_activa_n` < `254.0` → IC=+0.160 (n=101)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 254.0 (IC base=-0.016)
...
- **FILTRO** `ibs_20min` < `0.8043` → IC=-0.141 (n=639)
...
- **PATRÓN** `sigma_h` > `0.0132` → IC=+0.334 (n=487)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0132 (IC base=+0.252)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.252 (n=280)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2409` → IC=+0.290 (n=141)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2409 (IC base=+0.252)

- **PATRÓN** `volumen_spike_ratio` < `2.5739` → IC=+0.260 (n=670)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5739 (IC base=+0.252)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.255 (n=858)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.252)

- **PATRÓN** `libro_liquidez` > `2469.502` → IC=+0.254 (n=653)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2469.502 (IC base=+0.252)
...
- **PATRÓN** `sigma_h` > `0.0106` → IC=+0.209 (n=1261)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0106 (IC base=+0.164)

- **PATRÓN** `drift_60min` |x|≤ `0.3428` → IC=+0.170 (n=3324)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.3428 (IC base=+0.164)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.185 (n=1351)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 17.0 (IC base=+0.164)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.289 (n=1726)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.779` → IC=+0.243 (n=823)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.779 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.689` → IC=+0.237 (n=1536)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.689 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` > `0.6274` → IC=+0.166 (n=2555)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 0.6274 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.1055` → IC=+0.186 (n=1413)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1055 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` < `2.3226` → IC=+0.164 (n=3088)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.3226 (IC base=+0.164)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.165 (n=3872)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.03 (IC base=+0.164)

- **PATRÓN** `libro_liquidez` > `3937.758` → IC=+0.178 (n=1259)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 3937.758 (IC base=+0.164)

- **PATRÓN** `ballena_activa_n` < `143.0` → IC=+0.182 (n=2691)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.186 (n=278)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.0057 (IC base=+0.174)

- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.201 (n=292)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0071 (IC base=+0.174)

- **PATRÓN** `drift_60min` |x|≤ `0.2822` → IC=+0.196 (n=632)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.2822 (IC base=+0.174)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.225 (n=278)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.174)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.308 (n=289)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.174)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.714` → IC=+0.311 (n=204)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.714 (IC base=+0.174)
...
- **PATRÓN** `volumen_regimen` < `0.6381` → IC=+0.192 (n=186)
  ...
  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` < 0.6381 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` > `1.0678` → IC=+0.178 (n=253)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` > 1.0678 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.2311` → IC=+0.198 (n=117)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.2311 (IC base=+0.168)
...
- **PATRÓN** `libro_liquidez` > `11341.8291` → IC=+0.190 (n=498)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 11341.8291 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `422.0` → IC=+0.164 (n=421)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.174 (n=575)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 6.0 (IC base=+0.159)

- **PATRÓN** `ibs_20min` > `0.4502` → IC=+0.202 (n=562)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4502 (IC base=+0.159)

- **PATRÓN** `dist_vwap_pct` > `0.1502` → IC=+0.192 (n=391)
...
- **PATRÓN** `volumen_regimen` > `1.212` → IC=+0.221 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.212 (IC base=+0.159)

- **PATRÓN** `volumen_pendiente_norm` > `0.3003` → IC=+0.214 (n=96)
...
- **PATRÓN** `libro_liquidez` > `8738.117` → IC=+0.195 (n=375)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 8738.117 (IC base=+0.159)

- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.147 (n=335)
...
- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.155 (n=635)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0065 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=747)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` > `0.5417` → IC=+0.194 (n=711)

  - _Acción_: Kelly boost +0.97€ cuando `ibs_20min` > 0.5417 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `1.1462` → IC=+0.258 (n=163)
...
- **PATRÓN** `volumen_regimen` > `0.6276` → IC=+0.126 (n=711)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` > 0.6276 (IC base=+0.113)
...
- **PATRÓN** `sigma_h` > `0.0097` → IC=+0.173 (n=261)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0097 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.166 (n=747)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 8.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` > `0.476` → IC=+0.169 (n=784)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` > 0.476 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.9055` → IC=+0.230 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9055 (IC base=+0.136)
...
- **PATRÓN** `volumen_regimen` < `0.8709` → IC=+0.156 (n=422)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.8709 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` > `1.1727` → IC=+0.148 (n=211)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` > 1.1727 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.1785` → IC=+0.177 (n=215)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.1785 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` < `2.2754` → IC=+0.147 (n=641)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 2.2754 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.8649` → IC=+0.143 (n=485)
...
- **PATRÓN** `libro_liquidez` > `6740.9632` → IC=+0.173 (n=261)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 6740.9632 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `12.0` → IC=+0.189 (n=191)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.318 (n=64)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0075 (IC base=+0.264)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.294 (n=192)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.264)

- **PATRÓN** `ibs_20min` > `0.7776` → IC=+0.315 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7776 (IC base=+0.264)

- **PATRÓN** `dist_vwap_pct` > `0.1787` → IC=+0.291 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1787 (IC base=+0.264)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.256` → IC=+0.324 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.256 (IC base=+0.264)

- **PATRÓN** `volumen_regimen` < `0.8463` → IC=+0.271 (n=129)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8463 (IC base=+0.264)

- **PATRÓN** `volumen_regimen` > `1.1833` → IC=+0.303 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1833 (IC base=+0.264)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.204 (n=2051)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0087 (IC base=+0.166)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.173 (n=4534)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 6.0 (IC base=+0.166)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.306 (n=1559)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.166)

- **PATRÓN** `dist_vwap_pct` > `1.0488` → IC=+0.228 (n=655)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0488 (IC base=+0.166)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.632` → IC=+0.226 (n=2549)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.632 (IC base=+0.166)

- **PATRÓN** `volumen_regimen` < `0.886` → IC=+0.159 (n=2088)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.886 (IC base=+0.166)

- **PATRÓN** `volumen_regimen` > `1.0875` → IC=+0.154 (n=1420)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.0875 (IC base=+0.166)

- **PATRÓN** `volumen_pendiente_norm` > `0.1673` → IC=+0.193 (n=1226)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.1673 (IC base=+0.166)

- **PATRÓN** `volumen_spike_ratio` > `1.8738` → IC=+0.174 (n=2794)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 1.8738 (IC base=+0.166)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.167 (n=4289)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.02 (IC base=+0.166)

- **PATRÓN** `libro_liquidez` > `3038.6268` → IC=+0.185 (n=2048)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 3038.6268 (IC base=+0.166)

- **PATRÓN** `ballena_activa_n` < `60.0` → IC=+0.215 (n=2093)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 60.0 (IC base=+0.166)
...
- **PATRÓN** `sigma_h` < `0.0053` → IC=+0.191 (n=247)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.0053 (IC base=+0.187)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.338 (n=257)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.187)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.052` → IC=+0.299 (n=476)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.052 (IC base=+0.187)
...
- **PATRÓN** `volumen_spike_ratio` > `1.886` → IC=+0.180 (n=439)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` > 1.886 (IC base=+0.187)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.235 (n=572)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.187)

- **PATRÓN** `ballena_activa_n` < `50.0` → IC=+0.238 (n=273)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0966` → IC=+0.171 (n=247)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.0966 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.176 (n=739)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 6.0 (IC base=+0.161)

- **PATRÓN** `ibs_20min` > `0.3282` → IC=+0.209 (n=738)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3282 (IC base=+0.161)

- **PATRÓN** `dist_vwap_pct` > `0.2351` → IC=+0.221 (n=424)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2351 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.721` → IC=+0.189 (n=181)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 9.721 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.439` → IC=+0.171 (n=642)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` < 4.439 (IC base=+0.161)

- **PATRÓN** `volumen_regimen` < `1.2673` → IC=+0.168 (n=739)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2088` → IC=+0.191 (n=150)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.2088 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` < `2.413` → IC=+0.180 (n=689)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 2.413 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` > `1.7246` → IC=+0.181 (n=459)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 1.7246 (IC base=+0.161)

- **PATRÓN** `libro_liquidez` > `10640.5814` → IC=+0.187 (n=660)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 10640.5814 (IC base=+0.161)

- **PATRÓN** `ballena_activa_n` < `456.0` → IC=+0.173 (n=540)

  - _Acción_: Kelly boost +0.87€ cuando `ballena_activa_n` < 456.0 (IC base=+0.161)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.157 (n=675)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 8.0 (IC base=+0.143)

- **PATRÓN** `ibs_20min` > `0.7425` → IC=+0.238 (n=498)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7425 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` > `0.3732` → IC=+0.187 (n=298)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.3732 (IC base=+0.143)
...
- **PATRÓN** `volumen_regimen` > `1.2091` → IC=+0.161 (n=249)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1579` → IC=+0.206 (n=321)
...
- **PATRÓN** `libro_liquidez` > `9479.813` → IC=+0.242 (n=339)
...
- **PATRÓN** `sigma_h` > `0.0085` → IC=+0.146 (n=523)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` > 0.0085 (IC base=+0.094)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.137 (n=530)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 12.0 (IC base=+0.094)

- **PATRÓN** `ibs_20min` > `0.4805` → IC=+0.182 (n=784)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` > 0.4805 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` > `1.1174` → IC=+0.200 (n=148)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1174 (IC base=+0.094)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.403` → IC=+0.223 (n=298)
...
- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.138 (n=564)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.213 (n=907)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.201)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.302 (n=327)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` > `0.166` → IC=+0.256 (n=519)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.166 (IC base=+0.201)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.172` → IC=+0.250 (n=430)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1006` → IC=+0.244 (n=295)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.207 (n=998)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.204 (n=218)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0057 (IC base=+0.109)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.178 (n=116)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 18.0 (IC base=+0.109)

- **PATRÓN** `ibs_20min` > `0.6842` → IC=+0.251 (n=235)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6842 (IC base=+0.109)

- **PATRÓN** `dist_vwap_pct` > `0.3468` → IC=+0.220 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3468 (IC base=+0.109)

- **PATRÓN** `dist_vwap_pct` < `0.2094` → IC=+0.161 (n=184)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2094 (IC base=+0.109)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.907` → IC=+0.284 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.907 (IC base=+0.109)

- **PATRÓN** `volumen_regimen` < `0.6313` → IC=+0.173 (n=105)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 0.6313 (IC base=+0.109)

- **PATRÓN** `volumen_regimen` > `1.1022` → IC=+0.179 (n=79)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` > 1.1022 (IC base=+0.109)

- **PATRÓN** `volumen_pendiente_norm` < `0.1482` → IC=+0.216 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1482 (IC base=+0.109)

- **PATRÓN** `volumen_pendiente_norm` > `0.2799` → IC=+0.333 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2799 (IC base=+0.109)

- **PATRÓN** `volumen_spike_ratio` > `1.7631` → IC=+0.263 (n=91)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7631 (IC base=+0.109)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.195 (n=201)

  - _Acción_: Kelly boost +0.97€ cuando `libro_spread` < 0.02 (IC base=+0.109)

- **PATRÓN** `libro_liquidez` > `1205.3517` → IC=+0.190 (n=211)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 1205.3517 (IC base=+0.109)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.204 (n=42)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.047)

- **PATRÓN** `ibs_20min` > `0.6842` → IC=+0.183 (n=58)

  - _Acción_: Kelly boost +0.92€ cuando `ibs_20min` > 0.6842 (IC base=+0.047)

- **PATRÓN** `dist_vwap_pct` > `0.3068` → IC=+0.188 (n=30)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.3068 (IC base=+0.047)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.36` → IC=+0.250 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.36 (IC base=+0.047)

- **PATRÓN** `volumen_regimen` > `0.9891` → IC=+0.179 (n=26)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` > 0.9891 (IC base=+0.047)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.196 (n=21)

  - _Acción_: Kelly boost +0.98€ cuando `libro_spread` < 0.04 (IC base=+0.047)

- **PATRÓN** `libro_liquidez` > `667.907` → IC=+0.139 (n=34)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 667.907 (IC base=+0.047)
...
| ✅ FAVORITO_CONFIRMADO#DOGE | 5947 | +0.082 | -753.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 28 | +0.033 | +3.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#240min | 8 | -0.120 | -5.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 5911 | +0.082 | -751.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 10189 | +0.126 | -154.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2937 | +0.172 | -12.37€ | 1 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 5869 | +0.111 | -94.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1371 | +0.096 | -38.68€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 9645 | +0.125 | -623.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2566 | +0.192 | -160.91€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 78 | -0.013 | -3.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 5901 | +0.097 | -374.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1100 | +0.131 | -84.01€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 5930 | +0.105 | -411.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 20 | +0.000 | +1.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 5 | -0.018 | -1.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 5905 | +0.105 | -411.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 7843 | +0.178 | -596.64€ | 2 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 7843 | +0.178 | -596.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 1999 | +0.166 | -221.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 1999 | +0.166 | -221.39€ | 0 | 2 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 25065 | +0.188 | -2358.02€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 25065 | +0.188 | -2358.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 4464 | +0.146 | -717.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 4464 | +0.146 | -717.94€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 4342 | +0.162 | -594.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 4342 | +0.162 | -594.48€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 3978 | +0.220 | -155.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 3978 | +0.220 | -155.89€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 4141 | +0.200 | -307.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 4141 | +0.200 | -307.84€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 4226 | +0.183 | -448.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 4226 | +0.183 | -448.15€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 13611 | +0.094 | -529.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 1228 | +0.071 | -31.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 12383 | +0.096 | -497.32€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL | 3729 | +0.077 | -368.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL#5min | 3729 | +0.077 | -368.50€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 11767 | +0.057 | +4867.61€ | 0 | 15 |
| ✅ GBM_LATE_15M#15min | 11767 | +0.057 | +4867.61€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 1780 | +0.198 | +1318.99€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 1780 | +0.198 | +1318.99€ | 0 | 24 |
| ✅ GBM_LATE_15M#BTC | 1761 | +0.174 | +1160.59€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1761 | +0.174 | +1160.59€ | 0 | 29 |
| ✅ GBM_LATE_15M#DOGE | 1823 | +0.198 | +1340.28€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 1823 | +0.198 | +1340.28€ | 0 | 20 |
| ✅ GBM_LATE_15M#ETH | 1842 | -0.039 | +89.10€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1842 | -0.039 | +89.10€ | 3 | 11 |
| ✅ GBM_LATE_15M#SOL | 1926 | -0.049 | +422.43€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1926 | -0.049 | +422.43€ | 4 | 7 |
| ✅ GBM_LATE_15M#XRP | 2635 | -0.070 | +536.22€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 2635 | -0.070 | +536.22€ | 5 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 12519 | +0.061 | +6238.08€ | 0 | 18 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 12519 | +0.061 | +6238.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 2139 | -0.004 | +1478.10€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 2139 | -0.004 | +1478.10€ | 2 | 6 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 2729 | -0.023 | +354.92€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 2729 | -0.023 | +354.92€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 1623 | +0.259 | +1615.37€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 1623 | +0.259 | +1615.37€ | 0 | 20 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1927 | -0.050 | +1.27€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1927 | -0.050 | +1.27€ | 8 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 2080 | -0.014 | +718.02€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 2080 | -0.014 | +718.02€ | 3 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 2021 | +0.266 | +2070.41€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 2021 | +0.266 | +2070.41€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 9702 | +0.171 | +6908.08€ | 0 | 25 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 9702 | +0.171 | +6908.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB | 1366 | +0.202 | +1059.10€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB#15min | 1366 | +0.202 | +1059.10€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC | 1605 | +0.160 | +1127.90€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC#15min | 1605 | +0.160 | +1127.90€ | 0 | 24 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE | 1393 | +0.200 | +1068.21€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min | 1393 | +0.200 | +1068.21€ | 0 | 21 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH | 1591 | +0.145 | +980.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH#15min | 1591 | +0.145 | +980.25€ | 0 | 26 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL | 1801 | +0.123 | +1122.80€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL#15min | 1801 | +0.123 | +1122.80€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP | 1946 | +0.204 | +1549.81€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP#15min | 1946 | +0.204 | +1549.81€ | 0 | 19 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 2104 | +0.103 | +680.76€ | 0 | 16 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 2104 | +0.103 | +680.76€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH | 441 | +0.175 | +193.54€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH#15min | 441 | +0.175 | +193.54€ | 0 | 16 |
...
| ✅ GBM_LATE_15M_TARDIO | 11579 | +0.177 | +8400.26€ | 0 | 23 |
| ✅ GBM_LATE_15M_TARDIO#15min | 11579 | +0.177 | +8400.26€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 1711 | +0.225 | +1470.38€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 1711 | +0.225 | +1470.38€ | 0 | 22 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 1887 | +0.161 | +1330.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 1887 | +0.161 | +1330.98€ | 0 | 29 |
| ✅ GBM_LATE_15M_TARDIO#DOGE | 1730 | +0.225 | +1487.80€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#DOGE#15min | 1730 | +0.225 | +1487.80€ | 0 | 20 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 1789 | +0.140 | +1084.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 1789 | +0.140 | +1084.60€ | 0 | 26 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 2047 | +0.104 | +1106.90€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 2047 | +0.104 | +1106.90€ | 0 | 19 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 2415 | +0.208 | +1919.59€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 2415 | +0.208 | +1919.59€ | 0 | 22 |
...
| ✅ GBM_LATE_60M | 676 | +0.024 | +215.35€ | 3 | 14 |
| ✅ GBM_LATE_60M#60min | 676 | +0.024 | +215.35€ | 0 | 0 |
...
| ✅ GBM_LATE_60M#SOL | 200 | -0.054 | +46.05€ | 0 | 0 |
| ✅ GBM_LATE_60M#SOL#60min | 200 | -0.054 | +46.05€ | 3 | 7 |
...
| ✅ LIQUIDACIONES_5M | 1238 | -0.014 | -23.14€ | 5 | 0 |
| ✅ LIQUIDACIONES_5M#5min | 1238 | -0.014 | -23.14€ | 0 | 0 |
...
| ✅ LIQUIDACIONES_5M#ETH | 390 | +0.008 | +5.90€ | 0 | 0 |
| ✅ LIQUIDACIONES_5M#ETH#5min | 390 | +0.008 | +5.90€ | 3 | 0 |
...
| ✅ MOMENTUM_IBS_15M | 7236 | -0.003 | -92.04€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#15min | 7236 | -0.003 | -92.04€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_15M#SOL | 1278 | -0.013 | -37.94€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#SOL#15min | 1278 | -0.013 | -37.94€ | 0 | 0 |
...
| ✅ STRUCT_NO_15M | 4155 | +0.010 | -31.62€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 4155 | +0.010 | -31.62€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 1589 | +0.010 | -13.16€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 1589 | +0.010 | -13.16€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 1649 | +0.017 | -0.87€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 1649 | +0.017 | -0.87€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 917 | -0.004 | -17.59€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 917 | -0.004 | -17.59€ | 2 | 0 |
| ✅ UPDOWN_GBM | 9431 | +0.009 | +272.86€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 3336 | +0.037 | +320.43€ | 0 | 11 |
...
| ✅ UPDOWN_GBM#SOL | 2734 | +0.005 | +11.11€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 868 | +0.001 | +4.77€ | 1 | 4 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 5524 | -0.066 | +1209.55€ | 3 | 8 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 5524 | -0.066 | +1209.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 307 | -0.050 | +341.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 307 | -0.050 | +341.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 1137 | -0.157 | -82.52€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 1137 | -0.157 | -82.52€ | 5 | 7 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 1797 | -0.063 | +346.86€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 1797 | -0.063 | +346.86€ | 4 | 6 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.089) — sin ventaja clara. oversold(IBS<0.3): IC=+0.028 n=3356 | neutral: IC=+0.001 n=3676 | overbought(IBS>0.7): IC=+0.090 n=3687
  - _Datos_: n=11153 IC=+0.041 PNL=+1155.31€

**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  - _Umbral_: n≥40 por celda + gate riguroso completo (Wilson+shuffle+PnL bootstrap)
  - _Acción_: Añadir claves 'ESTRATEGIA#SUBTYPE#DIRECCION#HORA':1.2 a meta.hora_boost_factor, solo por celda confirmada
  - _Estado_: (cache 663s) 168 celda(s) GATE OK de 2457 trackeadas

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.001 < 0.08 — monitorear
  - _Datos_: n=868 IC=+0.001 PNL=+4.77€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.067 n=151356 | tras_1loss IC=+0.051 n=118890 | tras_2loss IC=+0.015 n=53362/40 | gap=+0.053 (umbral 0.05)
...
**⏳ H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  ...
  - _Estado_: 9369 ops, 22 horas distintas. Sin hora con n≥15 y IC extremo aún.
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=9154 IC=+0.006 PNL=+219.24€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=9154 IC=+0.006 PNL=+219.24€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=1945 IC=+0.029 PNL=+164.59€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1945 IC=+0.029 PNL=+164.59€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=2273 IC=+0.029 PNL=+182.15€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2273 IC=+0.029 PNL=+182.15€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=2150 IC=+0.139 PNL=+1254.09€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=2150 IC=+0.139 PNL=+1254.09€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.184 > 0.08 con n=834 PNL=+544.08€
  - _Datos_: n=834 IC=+0.184 PNL=+544.08€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.229 > 0.08 con n=1995 PNL=-206.92€
  - _Datos_: n=1995 IC=+0.229 PNL=-206.92€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=4464 IC=+0.146 PNL=-717.94€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=4464 IC=+0.146 PNL=-717.94€
...
**🟡 H-CUSTOM-GBMLATE15M-SOL-RESCATE-PRECIO** — GBM_LATE_15M#SOL#15min#BUY_YES (pausada 05-Ago) -- posible rescate con filtro py en [0.45,0.55)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.246 > 0.1 con n=61 PNL=+42.77€
  - _Datos_: n=61 IC=+0.246 PNL=+42.77€
