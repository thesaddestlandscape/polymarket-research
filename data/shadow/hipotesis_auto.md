# Hipótesis automáticas — 2026-08-18 19:52 UTC
_Generado por shadow_postmortem.py sobre 66918 resoluciones (PNL=+6645.60€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `150.46` → IC=-0.253 (n=886)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 150.46
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=2661)
...
- **FILTRO** `restante_s_al_confirmar` < `13.65` → IC=-0.492 (n=118)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.060 (n=241)

### BALLENAS_TARDIAS#ETH#5min
- **FILTRO** `concentracion_yes` < `1.0` → IC=-0.135 (n=61)

  - _Acción_: SKIP cuando `concentracion_yes` < 1.0
  - _Potencial_: sin este filtro IC_bueno=-0.129 (n=787)

- **FILTRO** `restante_s_al_confirmar` < `247.64` → IC=-0.155 (n=636)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.194 (n=2292)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.156 (n=3900)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.189 (n=2009)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.02 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `4043.0621` → IC=+0.179 (n=814)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.229 (n=308)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.178 (n=578)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.201 (n=909)
...
- **PATRÓN** `py_entrada` < `0.73` → IC=+0.173 (n=603)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.159 (n=681)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.217 (n=603)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.198 (n=220)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.72` → IC=+0.187 (n=503)
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.251 (n=1546)
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.296 (n=273)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.244 (n=346)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.258 (n=242)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.291 (n=352)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.74 (IC base=+0.247)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.239 (n=259)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.257 (n=282)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.76 (IC base=+0.189)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.226 (n=597)
...
- **PATRÓN** `restante_min` > `4.91` → IC=+0.200 (n=545)

  - _Acción_: Kelly boost +1.00€ cuando `restante_min` > 4.91 (IC base=+0.148)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.154 (n=1501)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `5.54` → IC=+0.205 (n=537)

  - _Acción_: Kelly boost +1.00€ cuando `lag_apertura_s` < 5.54 (IC base=+0.148)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.241 (n=299)
...
- **PATRÓN** `restante_min` > `4.88` → IC=+0.186 (n=285)

  - _Acción_: Kelly boost +0.93€ cuando `restante_min` > 4.88 (IC base=+0.155)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.161 (n=836)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `7.33` → IC=+0.190 (n=266)

  - _Acción_: Kelly boost +0.95€ cuando `lag_apertura_s` < 7.33 (IC base=+0.155)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.146 (n=758)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.6598` → IC=+0.122 (n=2158)

  - _Acción_: Kelly boost +0.61€ cuando `ibs_20min` < 0.6598 (IC base=+0.078)

- **PATRÓN** `dist_vwap_pct` < `0.1164` → IC=+0.143 (n=538)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.1164 (IC base=+0.078)

- **PATRÓN** `volumen_regimen` < `1.3118` → IC=+0.140 (n=487)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` < 1.3118 (IC base=+0.078)

- **PATRÓN** `volumen_regimen` > `0.6933` → IC=+0.145 (n=435)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 0.6933 (IC base=+0.078)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7042` → IC=+0.250 (n=170)
...
- **PATRÓN** `ballena_activa_n` < `173.0` → IC=+0.280 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 173.0 (IC base=+0.078)
...
- **PATRÓN** `sigma_h` < `0.0018` → IC=+0.163 (n=81)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0018 (IC base=+0.143)

- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.154 (n=108)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0029 (IC base=+0.143)

- **PATRÓN** `drift_60min` |x|≤ `0.1881` → IC=+0.167 (n=208)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.1881 (IC base=+0.143)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.184 (n=172)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 12.0 (IC base=+0.143)

- **PATRÓN** `ibs_20min` < `0.4787` → IC=+0.203 (n=237)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4787 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` < `0.1349` → IC=+0.164 (n=257)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.1349 (IC base=+0.143)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.439` → IC=+0.242 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.439 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` < `1.2895` → IC=+0.161 (n=237)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 1.2895 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` > `0.6821` → IC=+0.164 (n=212)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` > 0.6821 (IC base=+0.143)
...
- **PATRÓN** `libro_liquidez` > `12094.187` → IC=+0.179 (n=79)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 12094.187 (IC base=+0.143)

- **PATRÓN** `ballena_activa_n` < `235.0` → IC=+0.214 (n=19)
...
- **PATRÓN** `libro_spread` < `0.06` → IC=+0.150 (n=404)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.06 (IC base=+0.130)

- **PATRÓN** `libro_liquidez` > `1919.501` → IC=+0.131 (n=166)

  - _Acción_: Kelly boost +0.65€ cuando `libro_liquidez` > 1919.501 (IC base=+0.130)

- **PATRÓN** `ballena_activa_n` < `27.0` → IC=+0.160 (n=51)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 27.0 (IC base=+0.130)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.2` → IC=+0.128 (n=999)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8454` → IC=+0.169 (n=297)
  ...
  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 2.8454 (IC base=+0.052)

- **PATRÓN** `ballena_activa_n` < `40.0` → IC=+0.291 (n=65)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0626` → IC=+0.206 (n=117)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0626 (IC base=+0.178)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1388` → IC=+0.188 (n=251)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` < 0.1388 (IC base=+0.178)
...
- **PATRÓN** `volumen_spike_ratio` < `4.5876` → IC=+0.163 (n=268)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 4.5876 (IC base=+0.178)

- **PATRÓN** `volumen_spike_ratio` > `3.7364` → IC=+0.185 (n=122)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 3.7364 (IC base=+0.178)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.195 (n=378)
...
- **PATRÓN** `ballena_activa_n` < `31.0` → IC=+0.257 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 31.0 (IC base=+0.178)
...
- **PATRÓN** `sigma_h` > `0.0066` → IC=+0.193 (n=574)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0066 (IC base=+0.139)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.149 (n=422)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1119` → IC=+0.156 (n=419)
  ...
  - _Acción_: Kelly boost +0.78€ cuando `volumen_pendiente_norm` > 0.1119 (IC base=+0.139)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.145 (n=1426)

  - _Acción_: Kelly boost +0.72€ cuando `libro_spread` < 0.06 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `2684.1264` → IC=+0.174 (n=422)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 2684.1264 (IC base=+0.139)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.220 (n=1212)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.213)

- **PATRÓN** `drift_60min` |x|≤ `0.2382` → IC=+0.221 (n=1066)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2382 (IC base=+0.213)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.256 (n=551)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.213)

- **PATRÓN** `ibs_20min` < `0.3738` → IC=+0.280 (n=1213)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3738 (IC base=+0.213)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.625` → IC=+0.236 (n=259)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.625 (IC base=+0.213)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.278` → IC=+0.213 (n=1195)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.278 (IC base=+0.213)

- **PATRÓN** `volumen_regimen` < `1.2535` → IC=+0.189 (n=941)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_regimen` < 1.2535 (IC base=+0.213)

- **PATRÓN** `volumen_regimen` > `0.8726` → IC=+0.201 (n=627)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8726 (IC base=+0.213)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0479` → IC=+0.231 (n=407)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0479 (IC base=+0.213)

- **PATRÓN** `volumen_spike_ratio` > `3.1261` → IC=+0.237 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.1261 (IC base=+0.213)

- **PATRÓN** `ballena_activa_n` < `97.0` → IC=+0.257 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 97.0 (IC base=+0.213)
...
- **PATRÓN** `sigma_h` > `0.002` → IC=+0.186 (n=202)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.002 (IC base=+0.182)

- **PATRÓN** `drift_60min` |x|≤ `0.1928` → IC=+0.202 (n=199)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1928 (IC base=+0.182)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.216 (n=213)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.182)

- **PATRÓN** `ibs_20min` < `0.2977` → IC=+0.237 (n=226)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2977 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.413` → IC=+0.271 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.413 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` < `0.637` → IC=+0.218 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.637 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` > `0.6986` → IC=+0.185 (n=201)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` > 0.6986 (IC base=+0.182)

- **PATRÓN** `volumen_pendiente_norm` < `0.1932` → IC=+0.205 (n=110)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1932 (IC base=+0.182)

- **PATRÓN** `volumen_pendiente_norm` > `0.136` → IC=+0.258 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.136 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` < `2.5474` → IC=+0.248 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5474 (IC base=+0.182)

- **PATRÓN** `libro_liquidez` > `11393.2913` → IC=+0.214 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11393.2913 (IC base=+0.182)
...
- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.173 (n=197)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0065 (IC base=+0.146)

- **PATRÓN** `drift_60min` |x|≤ `0.1483` → IC=+0.157 (n=199)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.1483 (IC base=+0.146)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.164 (n=114)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2301` → IC=+0.126 (n=236)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_pendiente_norm` < 0.2301 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` < `2.0094` → IC=+0.207 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0094 (IC base=+0.146)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.175 (n=226)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1417` → IC=+0.196 (n=146)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.1417 (IC base=+0.189)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.213 (n=221)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.189)

- **PATRÓN** `ibs_20min` < `0.314` → IC=+0.260 (n=219)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.314 (IC base=+0.189)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.358` → IC=+0.259 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.358 (IC base=+0.189)

- **PATRÓN** `volumen_regimen` < `1.2412` → IC=+0.220 (n=219)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2412 (IC base=+0.189)

- **PATRÓN** `volumen_pendiente_norm` < `0.0858` → IC=+0.226 (n=93)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0858 (IC base=+0.189)

- **PATRÓN** `volumen_spike_ratio` < `1.9228` → IC=+0.285 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9228 (IC base=+0.189)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.190 (n=253)
...
- **FILTRO** `ibs_20min` > `0.381` → IC=-0.194 (n=109)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.262 (n=212)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.226 (n=82)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.107)

- **PATRÓN** `ibs_20min` < `0.381` → IC=+0.262 (n=212)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.381 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.906` → IC=+0.250 (n=34)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.906 (IC base=+0.107)

- **PATRÓN** `volumen_regimen` > `0.8622` → IC=+0.144 (n=161)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 0.8622 (IC base=+0.107)

- **PATRÓN** `volumen_pendiente_norm` > `0.122` → IC=+0.206 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.122 (IC base=+0.107)

- **PATRÓN** `volumen_spike_ratio` > `1.5471` → IC=+0.143 (n=113)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.5471 (IC base=+0.107)

- **PATRÓN** `libro_liquidez` > `2385.1288` → IC=+0.223 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2385.1288 (IC base=+0.107)
...
- **PATRÓN** `sigma_h` < `0.007` → IC=+0.276 (n=257)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.007 (IC base=+0.265)

- **PATRÓN** `sigma_h` > `0.005` → IC=+0.272 (n=230)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.005 (IC base=+0.265)

- **PATRÓN** `drift_60min` |x|≤ `0.0808` → IC=+0.284 (n=86)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0808 (IC base=+0.265)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.277 (n=119)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3141` → IC=+0.307 (n=257)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3141 (IC base=+0.265)

- **PATRÓN** `dist_vwap_pct` > `0.1297` → IC=+0.320 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1297 (IC base=+0.265)
...
- **PATRÓN** `volumen_regimen` > `0.8965` → IC=+0.309 (n=171)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8965 (IC base=+0.265)
...
- **PATRÓN** `volumen_spike_ratio` > `1.7731` → IC=+0.263 (n=116)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7731 (IC base=+0.265)

- **PATRÓN** `libro_liquidez` > `2623.4578` → IC=+0.265 (n=117)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2623.4578 (IC base=+0.265)
...
- **PATRÓN** `ibs_20min` < `0.1481` → IC=+0.142 (n=118)

  - _Acción_: Kelly boost +0.71€ cuando `ibs_20min` < 0.1481 (IC base=+0.049)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.859` → IC=+0.239 (n=44)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.859 (IC base=+0.049)

- **PATRÓN** `volumen_spike_ratio` < `2.7248` → IC=+0.124 (n=107)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_spike_ratio` < 2.7248 (IC base=+0.049)
...
- **PATRÓN** `sigma_h` < `0.0026` → IC=+0.244 (n=37)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0026 (IC base=+0.155)

- **PATRÓN** `drift_60min` |x|≤ `0.2654` → IC=+0.192 (n=37)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.96€ cuando `drift_60min` |x|≤ 0.2654 (IC base=+0.155)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.167 (n=16)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 17.0 (IC base=+0.155)

- **PATRÓN** `hora_utc` < `13.0` → IC=+0.227 (n=31)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 13.0 (IC base=+0.155)

- **PATRÓN** `ibs_20min` < `0.1986` → IC=+0.269 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1986 (IC base=+0.155)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.27` → IC=+0.370 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.27 (IC base=+0.155)

- **PATRÓN** `volumen_regimen` < `1.178` → IC=+0.167 (n=37)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.178 (IC base=+0.155)

- **PATRÓN** `volumen_regimen` > `0.7034` → IC=+0.200 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7034 (IC base=+0.155)

- **PATRÓN** `volumen_pendiente_norm` > `0.0902` → IC=+0.395 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0902 (IC base=+0.155)

- **PATRÓN** `volumen_spike_ratio` < `1.852` → IC=+0.340 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.852 (IC base=+0.155)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.200 (n=514)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0068 (IC base=+0.118)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.149 (n=562)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.259` → IC=+0.230 (n=1032)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.259 (IC base=+0.118)

- **PATRÓN** `volumen_pendiente_norm` > `0.1816` → IC=+0.137 (n=359)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_pendiente_norm` > 0.1816 (IC base=+0.118)
...
- **PATRÓN** `libro_spread` < `0.06` → IC=+0.124 (n=1718)

  - _Acción_: Kelly boost +0.62€ cuando `libro_spread` < 0.06 (IC base=+0.118)

- **PATRÓN** `libro_liquidez` > `2726.373` → IC=+0.152 (n=513)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 2726.373 (IC base=+0.118)

- **PATRÓN** `ballena_activa_n` < `150.0` → IC=+0.198 (n=160)

  - _Acción_: Kelly boost +0.99€ cuando `ballena_activa_n` < 150.0 (IC base=+0.118)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.233 (n=1191)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.225)

- **PATRÓN** `sigma_h` > `0.0038` → IC=+0.231 (n=1352)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0038 (IC base=+0.225)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.236 (n=1231)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.225)

- **PATRÓN** `ibs_20min` < `0.5` → IC=+0.287 (n=1357)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5 (IC base=+0.225)

- **PATRÓN** `dist_vwap_pct` < `0.1522` → IC=+0.198 (n=899)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.1522 (IC base=+0.225)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.046` → IC=+0.257 (n=269)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.046 (IC base=+0.225)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.483` → IC=+0.225 (n=1265)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.483 (IC base=+0.225)

- **PATRÓN** `volumen_regimen` < `0.6181` → IC=+0.213 (n=305)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6181 (IC base=+0.225)

- **PATRÓN** `volumen_regimen` > `1.0614` → IC=+0.219 (n=414)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0614 (IC base=+0.225)

- **PATRÓN** `volumen_pendiente_norm` > `0.1822` → IC=+0.266 (n=229)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1822 (IC base=+0.225)

- **PATRÓN** `volumen_spike_ratio` < `2.0101` → IC=+0.275 (n=443)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0101 (IC base=+0.225)
...
- **PATRÓN** `sigma_h` < `0.0018` → IC=+0.200 (n=78)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.1874` → IC=+0.181 (n=205)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.91€ cuando `drift_60min` |x|≤ 0.1874 (IC base=+0.173)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.192 (n=238)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 6.0 (IC base=+0.173)

- **PATRÓN** `ibs_20min` < `0.4222` → IC=+0.228 (n=233)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4222 (IC base=+0.173)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.22` → IC=+0.253 (n=148)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.22 (IC base=+0.173)

- **PATRÓN** `volumen_regimen` < `0.6183` → IC=+0.200 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6183 (IC base=+0.173)

- **PATRÓN** `volumen_regimen` > `0.856` → IC=+0.194 (n=155)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 0.856 (IC base=+0.173)
...
- **PATRÓN** `libro_liquidez` > `5039.1995` → IC=+0.186 (n=208)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 5039.1995 (IC base=+0.173)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1461` → IC=+0.173 (n=221)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` < 0.1461 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` < `2.1733` → IC=+0.157 (n=106)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.1733 (IC base=+0.172)
...
- **PATRÓN** `libro_spread` < `0.06` → IC=+0.191 (n=338)
...
- **PATRÓN** `sigma_h` < `0.0033` → IC=+0.221 (n=109)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0033 (IC base=+0.144)

- **PATRÓN** `drift_60min` |x|≤ `0.2641` → IC=+0.149 (n=109)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.2641 (IC base=+0.144)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.155 (n=114)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 4.0 (IC base=+0.144)

- **PATRÓN** `ibs_20min` < `0.3` → IC=+0.245 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `0.1931` → IC=+0.250 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1931 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.7` → IC=+0.255 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.7 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` < `0.8857` → IC=+0.204 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8857 (IC base=+0.144)

- **PATRÓN** `volumen_pendiente_norm` < `0.141` → IC=+0.331 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.141 (IC base=+0.144)

- **PATRÓN** `volumen_spike_ratio` < `2.2709` → IC=+0.289 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.2709 (IC base=+0.144)

- **PATRÓN** `volumen_spike_ratio` > `1.7529` → IC=+0.289 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7529 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `6728.8499` → IC=+0.237 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 6728.8499 (IC base=+0.144)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.299 (n=301)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `volumen_regimen` > `0.8813` → IC=+0.300 (n=228)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8813 (IC base=+0.259)

- **PATRÓN** `volumen_pendiente_norm` > `0.2728` → IC=+0.357 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2728 (IC base=+0.259)

- **PATRÓN** `volumen_spike_ratio` < `1.5017` → IC=+0.280 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5017 (IC base=+0.259)

- **PATRÓN** `volumen_spike_ratio` > `2.6341` → IC=+0.260 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6341 (IC base=+0.259)

- **PATRÓN** `libro_liquidez` > `2830.7244` → IC=+0.267 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2830.7244 (IC base=+0.259)

- **PATRÓN** `ballena_activa_n` < `55.0` → IC=+0.196 (n=54)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 55.0 (IC base=+0.259)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.208 (n=22)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.103)

- **PATRÓN** `drift_20min_pct` |x|≤ `0.0569` → IC=+0.237 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `drift_20min_pct` |x|≤ 0.0569 (IC base=+0.103)

- **PATRÓN** `libro_liquidez` > `15297.4391` → IC=+0.184 (n=17)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 15297.4391 (IC base=+0.103)

### MOMENTUM_IBS_15M_BALLENA
- **FILTRO** `py_entrada` < `0.5` → IC=-0.190 (n=169)

  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=208)

- **FILTRO** `ibs_20min` < `0.7405` → IC=-0.177 (n=94)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7405
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=283)

- **FILTRO** `ballena_activa_n` > `14.0` → IC=-0.174 (n=90)
...
- **FILTRO** `ibs_20min` < `0.7588` → IC=-0.214 (n=19)
...
- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.214 (n=19)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.206 (n=32)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.5 (IC base=+0.061)

### MOMENTUM_IBS_15M_BALLENA#SOL#15min
- **FILTRO** `hora_utc` > `15.0` → IC=-0.154 (n=24)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 15.0
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.289 (n=17)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.009 (n=51)

- **FILTRO** `ibs_20min` < `0.743` → IC=-0.237 (n=17)

  - _Acción_: SKIP cuando `ibs_20min` < 0.743
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=51)

- **FILTRO** `ballena_activa_n` > `2.0` → IC=-0.133 (n=28)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.174 (n=225)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.081 (n=827)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.304 (n=263)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=789)

- **FILTRO** `ibs_7min` < `0.7399` → IC=-0.232 (n=263)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7399
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=789)

- **FILTRO** `ballena_activa_n` > `23.0` → IC=-0.209 (n=259)

  - _Acción_: SKIP cuando `ballena_activa_n` > 23.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=793)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.218 (n=260)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=+0.012 (n=842)
...
- **FILTRO** `drift_7min_pct` |x|> `0.1157` → IC=-0.207 (n=39)
...
- **FILTRO** `py_entrada` < `0.38` → IC=-0.292 (n=51)
...
- **FILTRO** `py_entrada` > `0.6` → IC=-0.217 (n=90)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.082 (n=96)

- **FILTRO** `ibs_7min` > `0.7368` → IC=-0.188 (n=46)
...
- **FILTRO** `ibs_7min` < `0.8376` → IC=-0.256 (n=43)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=131)

- **FILTRO** `ballena_activa_n` > `25.0` → IC=-0.256 (n=43)

  - _Acción_: SKIP cuando `ballena_activa_n` > 25.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=131)

- **FILTRO** `libro_liquidez` < `9178.4058` → IC=-0.147 (n=114)

  - _Acción_: SKIP cuando `libro_liquidez` < 9178.4058
  - _Potencial_: sin este filtro IC_bueno=-0.097 (n=60)

- **FILTRO** `py_entrada` > `0.58` → IC=-0.152 (n=44)
...
- **FILTRO** `py_entrada` > `0.5` → IC=-0.151 (n=130)

  - _Acción_: SKIP cuando `py_entrada` > 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.181 (n=70)

- **PATRÓN** `py_entrada` < `0.5` → IC=+0.181 (n=70)

  - _Acción_: Kelly boost +0.90€ cuando `py_entrada` < 0.5 (IC base=-0.035)
...
- **PATRÓN** `delta_ratio` |x|> `0.4012` → IC=+0.162 (n=140)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.81€ cuando `delta_ratio` |x|> 0.4012 (IC base=+0.133)
...
- **PATRÓN** `total_vol_5m` < `357.983` → IC=+0.232 (n=39)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `total_vol_5m` < 357.983 (IC base=+0.133)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.149 (n=55)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.02 (IC base=+0.133)
...
- **PATRÓN** `ibs_15` < `0.45` → IC=+0.126 (n=362)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.63€ cuando `ibs_15` < 0.45 (IC base=+0.076)

- **PATRÓN** `dist_vwap_pct` > `0.566` → IC=+0.175 (n=38)
...
- **PATRÓN** `sigma_h` < `0.0033` → IC=+0.174 (n=44)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1155` → IC=+0.152 (n=90)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.76€ cuando `delta_ratio_macro` |x|> 0.1155 (IC base=+0.142)
...
- **PATRÓN** `hora_utc` < `14.0` → IC=+0.153 (n=93)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 14.0 (IC base=+0.142)

- **PATRÓN** `ibs_15` < `0.3247` → IC=+0.167 (n=100)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.83€ cuando `ibs_15` < 0.3247 (IC base=+0.142)

- **PATRÓN** `ibs_15` > `0.0304` → IC=+0.183 (n=99)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.92€ cuando `ibs_15` > 0.0304 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.1596` → IC=+0.224 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1596 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` < `23.352` → IC=+0.200 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 23.352 (IC base=+0.142)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1198` → IC=+0.167 (n=43)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.1198 (IC base=+0.100)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0789` → IC=+0.141 (n=126)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.70€ cuando `delta_ratio_macro` |x|> 0.0789 (IC base=+0.100)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.146 (n=111)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 15.0 (IC base=+0.100)

- **PATRÓN** `ibs_15` < `0.1154` → IC=+0.224 (n=56)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.1154 (IC base=+0.100)

- **PATRÓN** `dist_vwap_pct` > `0.1269` → IC=+0.218 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1269 (IC base=+0.100)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.421` → IC=+0.154 (n=105)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 5.421 (IC base=+0.100)

- **PATRÓN** `libro_liquidez` > `2603.9534` → IC=+0.143 (n=113)

  - _Acción_: Kelly boost +0.72€ cuando `libro_liquidez` > 2603.9534 (IC base=+0.100)
...
- **FILTRO** `ibs_15` < `0.3664` → IC=-0.331 (n=75)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.152 (n=153)

- **FILTRO** `sigma_ewma_delta_pct` > `12.903` → IC=-0.161 (n=225)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 12.903
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=916)

- **PATRÓN** `ibs_15` > `0.3664` → IC=+0.152 (n=153)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.76€ cuando `ibs_15` > 0.3664 (IC base=-0.039)
...
- **FILTRO** `sigma_ewma_delta_pct` > `7.956` → IC=-0.144 (n=88)
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_NO, IBS < 0.3247 sube el IC de +0.142 a +0.167 en UPDOWN_GBM#ETH#15min (n=100). Ya aplicado como kelly_boost=+0.83€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_NO, IBS > 0.0304 sube el IC de +0.142 a +0.183 en UPDOWN_GBM#ETH#15min (n=99). Ya aplicado como kelly_boost=+0.92€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1154 sube el IC de +0.100 a +0.224 en UPDOWN_GBM#XRP#15min (n=56). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.3664 sube el IC de -0.039 a +0.152 en UPDOWN_GBM_15M_TARDIO (n=153). Ya aplicado como kelly_boost=+0.76€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 3375 | -0.112 | -527.70€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 460 | -0.024 | -17.86€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 2915 | -0.126 | -509.84€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 369 | -0.185 | -101.90€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 369 | -0.185 | -101.90€ | 1 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 822 | +0.004 | -96.98€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 822 | +0.004 | -96.98€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP | 689 | -0.219 | -120.71€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP#5min | 689 | -0.219 | -120.71€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 14536 | +0.116 | -833.40€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3404 | +0.182 | -103.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 99 | -0.104 | -47.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 8434 | +0.086 | -707.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2599 | +0.132 | +26.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 1423 | +0.033 | -307.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 13 | -0.065 | -2.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 1406 | +0.036 | -299.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 3301 | +0.141 | +2.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 930 | +0.197 | -28.88€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 41 | -0.105 | -20.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 1405 | +0.111 | -12.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 925 | +0.140 | +64.97€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 1421 | +0.054 | -250.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 9 | -0.021 | -3.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 1411 | +0.055 | -244.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 3570 | +0.127 | -40.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1257 | +0.161 | -20.67€ | 0 | 6 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 1403 | +0.106 | -15.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 898 | +0.117 | +3.62€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 3403 | +0.134 | -209.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1183 | +0.197 | -50.19€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 40 | +0.000 | -9.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 1404 | +0.082 | -107.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 776 | +0.140 | -42.31€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 1418 | +0.128 | -28.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 12 | +0.043 | +1.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 1405 | +0.128 | -29.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 3482 | +0.160 | -336.82€ | 2 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 3482 | +0.160 | -336.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 872 | +0.154 | -114.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 872 | +0.154 | -114.04€ | 0 | 2 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 863 | +0.159 | -107.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 863 | +0.159 | -107.88€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 773 | +0.217 | -42.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 773 | +0.217 | -42.66€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 817 | +0.173 | -80.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 817 | +0.173 | -80.26€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 6008 | +0.190 | -553.37€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 6008 | +0.190 | -553.37€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 1126 | +0.098 | -253.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 1126 | +0.098 | -253.80€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 925 | +0.250 | -6.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 925 | +0.250 | -6.10€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 1059 | +0.153 | -152.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 1059 | +0.153 | -152.76€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 964 | +0.225 | -35.67€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 964 | +0.225 | -35.67€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 935 | +0.247 | -10.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 935 | +0.247 | -10.23€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 999 | +0.189 | -94.81€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 999 | +0.189 | -94.81€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 2147 | +0.148 | +116.09€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 2147 | +0.148 | +116.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 1061 | +0.155 | +69.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 1061 | +0.155 | +69.17€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 1086 | +0.141 | +46.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 1086 | +0.141 | +46.92€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 4551 | +0.087 | +1610.27€ | 0 | 14 |
| ✅ GBM_LATE_15M#15min | 4551 | +0.087 | +1610.27€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 792 | +0.176 | +498.66€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 792 | +0.176 | +498.66€ | 0 | 17 |
| ✅ GBM_LATE_15M#BTC | 448 | +0.180 | +227.69€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 448 | +0.180 | +227.69€ | 0 | 24 |
| ✅ GBM_LATE_15M#DOGE | 797 | +0.191 | +544.42€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 797 | +0.191 | +544.42€ | 0 | 18 |
| ✅ GBM_LATE_15M#ETH | 609 | -0.003 | +32.41€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 609 | -0.003 | +32.41€ | 0 | 2 |
| ✅ GBM_LATE_15M#SOL | 853 | +0.004 | +75.89€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 853 | +0.004 | +75.89€ | 3 | 4 |
| ✅ GBM_LATE_15M#XRP | 1052 | +0.020 | +231.19€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1052 | +0.020 | +231.19€ | 0 | 4 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 5568 | +0.049 | +1651.91€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 5568 | +0.049 | +1651.91€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 1068 | -0.028 | +192.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 1068 | -0.028 | +192.53€ | 1 | 2 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 1055 | -0.009 | +103.01€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 1055 | -0.009 | +103.01€ | 0 | 3 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 675 | +0.239 | +612.26€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 675 | +0.239 | +612.26€ | 0 | 21 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 976 | -0.018 | +12.31€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 976 | -0.018 | +12.31€ | 6 | 0 |
...
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 3303 | +0.175 | +2229.92€ | 0 | 24 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 3303 | +0.175 | +2229.92€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB | 588 | +0.192 | +425.31€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB#15min | 588 | +0.192 | +425.31€ | 0 | 17 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC | 435 | +0.193 | +285.86€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC#15min | 435 | +0.193 | +285.86€ | 0 | 24 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE | 585 | +0.200 | +450.05€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min | 585 | +0.200 | +450.05€ | 0 | 22 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH | 409 | +0.208 | +298.86€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH#15min | 409 | +0.208 | +298.86€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL | 621 | +0.076 | +256.46€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL#15min | 621 | +0.076 | +256.46€ | 1 | 10 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP | 665 | +0.196 | +513.39€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP#15min | 665 | +0.196 | +513.39€ | 0 | 25 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 542 | +0.044 | +41.70€ | 0 | 7 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 542 | +0.044 | +41.70€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BTC | 98 | +0.040 | -6.35€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BTC#15min | 98 | +0.040 | -6.35€ | 2 | 10 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 105 | +0.033 | -0.63€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 105 | +0.033 | -0.63€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 3851 | +0.168 | +2480.51€ | 0 | 21 |
| ✅ GBM_LATE_15M_TARDIO#15min | 3851 | +0.168 | +2480.51€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 762 | +0.187 | +534.99€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 762 | +0.187 | +534.99€ | 0 | 18 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 508 | +0.155 | +269.79€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 508 | +0.155 | +269.79€ | 1 | 22 |
| ✅ GBM_LATE_15M_TARDIO#DOGE | 751 | +0.218 | +622.68€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#DOGE#15min | 751 | +0.218 | +622.68€ | 0 | 16 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 349 | +0.115 | +132.86€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 349 | +0.115 | +132.86€ | 1 | 19 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 623 | +0.079 | +267.80€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 623 | +0.079 | +267.80€ | 0 | 15 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 858 | +0.199 | +652.39€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 858 | +0.199 | +652.39€ | 0 | 24 |
...
| ✅ MOMENTUM_IBS_15M | 567 | +0.034 | -0.09€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#15min | 567 | +0.034 | -0.09€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB | 92 | +0.064 | +9.77€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB#15min | 92 | +0.064 | +9.77€ | 0 | 4 |
| ✅ MOMENTUM_IBS_15M#BTC | 97 | +0.056 | +7.84€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BTC#15min | 97 | +0.056 | +7.84€ | 0 | 4 |
| ✅ MOMENTUM_IBS_15M#DOGE | 90 | +0.000 | -11.86€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#DOGE#15min | 90 | +0.000 | -11.86€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH | 99 | +0.064 | +14.31€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH#15min | 99 | +0.064 | +14.31€ | 0 | 3 |
| ✅ MOMENTUM_IBS_15M#SOL | 94 | -0.031 | -18.00€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#SOL#15min | 94 | -0.031 | -18.00€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#XRP | 95 | +0.046 | -2.15€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#XRP#15min | 95 | +0.046 | -2.15€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA | 802 | -0.036 | -3.47€ | 4 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#15min | 802 | -0.036 | -3.47€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB | 128 | -0.046 | +18.13€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB#15min | 128 | -0.046 | +18.13€ | 4 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC | 140 | -0.063 | -14.18€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC#15min | 140 | -0.063 | -14.18€ | 6 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE | 129 | -0.019 | +7.88€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE#15min | 129 | -0.019 | +7.88€ | 1 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH | 140 | +0.007 | -1.11€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH#15min | 140 | +0.007 | -1.11€ | 2 | 1 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL | 129 | -0.065 | -6.67€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL#15min | 129 | -0.065 | -6.67€ | 4 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP | 136 | -0.029 | -7.50€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP#15min | 136 | -0.029 | -7.50€ | 4 | 2 |
| ✅ MOMENTUM_IBS_15M_FADE | 283 | -0.086 | -27.57€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#15min | 283 | -0.086 | -27.57€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_15M_FADE#SOL | 64 | -0.045 | -3.54€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#SOL#15min | 64 | -0.045 | -3.54€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#XRP | 55 | -0.044 | -2.85€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#XRP#15min | 55 | -0.044 | -2.85€ | 1 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 2154 | -0.071 | +23.08€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 2154 | -0.071 | +23.08€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 322 | -0.096 | +3.57€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 322 | -0.096 | +3.57€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 407 | -0.065 | +77.18€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 407 | -0.065 | +77.18€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 334 | -0.080 | -21.75€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 334 | -0.080 | -21.75€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 355 | -0.091 | -36.96€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 355 | -0.091 | -36.96€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 395 | -0.034 | +7.18€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 395 | -0.034 | +7.18€ | 2 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 341 | -0.069 | -6.13€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 341 | -0.069 | -6.13€ | 5 | 1 |
| ✅ MOMENTUM_IBS_5M_FADE | 1983 | +0.017 | +16.56€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 1983 | +0.017 | +16.56€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB | 339 | +0.002 | +2.16€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB#5min | 339 | +0.002 | +2.16€ | 1 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC | 284 | +0.045 | +9.24€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC#5min | 284 | +0.045 | +9.24€ | 1 | 1 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE | 343 | +0.022 | +3.68€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE#5min | 343 | +0.022 | +3.68€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH | 342 | +0.015 | +1.91€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH#5min | 342 | +0.015 | +1.91€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL | 346 | +0.020 | +3.96€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL#5min | 346 | +0.020 | +3.96€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP | 329 | +0.004 | -4.38€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP#5min | 329 | +0.004 | -4.38€ | 0 | 0 |
| ✅ ORDER_FLOW_5M | 224 | +0.080 | +39.16€ | 1 | 4 |
| ✅ ORDER_FLOW_5M#5min | 88 | +0.111 | +26.56€ | 0 | 0 |
...
| ✅ STRUCT_NO_15M | 1869 | +0.016 | -3.72€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 1869 | +0.016 | -3.72€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 712 | +0.007 | -8.15€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 712 | +0.007 | -8.15€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 734 | +0.020 | +1.76€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 734 | +0.020 | +1.76€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 423 | +0.022 | +2.66€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 423 | +0.022 | +2.66€ | 2 | 0 |
| ✅ UPDOWN_GBM | 2213 | +0.024 | +138.42€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 965 | +0.085 | +165.18€ | 1 | 6 |
| ✅ UPDOWN_GBM#240min | 119 | +0.021 | +0.93€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 831 | -0.020 | -17.37€ | 4 | 0 |
| ✅ UPDOWN_GBM#60min | 251 | -0.022 | -9.81€ | 3 | 1 |
| ✅ UPDOWN_GBM#BNB | 98 | +0.100 | +19.58€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 91 | +0.124 | +21.20€ | 0 | 0 |
...
| ✅ UPDOWN_GBM#ETH | 511 | +0.058 | +54.52€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 224 | +0.124 | +51.25€ | 1 | 13 |
...
| ✅ UPDOWN_GBM#SOL | 397 | -0.009 | +0.30€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 169 | +0.015 | +1.51€ | 1 | 2 |
...
| ✅ UPDOWN_GBM#XRP | 491 | +0.025 | +60.28€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 296 | +0.094 | +82.66€ | 0 | 14 |
| ✅ UPDOWN_GBM#XRP#240min | 17 | -0.112 | -2.90€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 178 | -0.072 | -19.47€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1670 | -0.047 | +176.31€ | 2 | 3 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1670 | -0.047 | +176.31€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 537 | -0.010 | +107.75€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 537 | -0.010 | +107.75€ | 1 | 1 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 518 | -0.033 | +70.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 518 | -0.033 | +70.55€ | 3 | 0 |
...
| ✅ UPDOWN_OU_5M | 351 | -0.061 | -26.89€ | 3 | 0 |
| ✅ UPDOWN_OU_5M#5min | 351 | -0.061 | -26.89€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#BNB | 226 | -0.004 | -10.59€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#BNB#5min | 226 | -0.004 | -10.59€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.138) — sin ventaja clara. oversold(IBS<0.3): IC=-0.019 n=772 | neutral: IC=+0.010 n=745 | overbought(IBS>0.7): IC=+0.119 n=997
  - _Datos_: n=2698 IC=+0.043 PNL=+223.80€
...
**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  ...
  - _Estado_: 1 celda(s) pasan gate riguroso completo de 52 evaluadas (n>=40) y 287 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.015 < 0.08 — monitorear
  - _Datos_: n=169 IC=+0.015 PNL=+1.51€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.104 n=39051 | tras_1loss IC=+0.061 n=27566 | tras_2loss IC=+0.021 n=12063/40 | gap=+0.084 (umbral 0.05)
...
**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  ...
  - _Estado_: n_overlaps=15, boost estimado=+0.019. Necesita 5 más y boost>0.05
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=+0.081 n=172 | contrario_BTC IC=-0.036 n=95/40 | gap=-0.117 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2031 IC=+0.018 PNL=+100.02€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2031 IC=+0.018 PNL=+100.02€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=602 IC=+0.078 PNL=+72.92€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=602 IC=+0.078 PNL=+72.92€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=89 IC=+0.017 PNL=+8.20€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=89 IC=+0.017 PNL=+8.20€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=462 IC=+0.090 PNL=+108.36€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=462 IC=+0.090 PNL=+108.36€

**〰️ H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  - _Hipótesis_: Feature nueva 2026-06-27: drift del precio YES en Polymarket en últimas 5 obs (~5min). Si poly_drift<0 y decidimos BUY_NO (o poly_drift>0 y BUY_YES) → confluencia. Si diverge → reducción de stake. Hipótesis: confluencia Binance+Polymarket mejora IC; divergencia empeora.
  - _Umbral_: n≥40 en confluencia vs divergencia para validar el boost ×1.1
  - _Acción_: Si IC_confluencia>IC_divergencia con n≥40 → mantener el boost. Si no → retirar.
  - _Estado_: n=297 IC=+0.032 PNL=-8.06€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=297 IC=+0.032 PNL=-8.06€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=323 IC=+0.020 PNL=+1.60€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=323 IC=+0.020 PNL=+1.60€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.149 > 0.1 con n=560 PNL=+166.88€
  - _Datos_: n=560 IC=+0.149 PNL=+166.88€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=168 IC=+0.100 PNL=+48.64€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=168 IC=+0.100 PNL=+48.64€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1355 IC=-0.147 PNL=+61.84€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1355 IC=-0.147 PNL=+61.84€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=315 IC=+0.143 PNL=+105.09€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=315 IC=+0.143 PNL=+105.09€
...
**〰️ H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: n=338 IC=-0.003 PNL=+21.79€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=338 IC=-0.003 PNL=+21.79€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.205 > 0.08 con n=279 PNL=+158.10€
  - _Datos_: n=279 IC=+0.205 PNL=+158.10€
...
**〰️ H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: n=558 IC=+0.029 PNL=+151.02€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=558 IC=+0.029 PNL=+151.02€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.245 > 0.08 con n=654 PNL=-70.75€
  - _Datos_: n=654 IC=+0.245 PNL=-70.75€
...
**🟡 H-CUSTOM-ETH15-BUYNO-TARDIO** — UPDOWN_GBM ETH#15min BUY_NO tardío (T_h<0.2) -- edge fuerte no capturado por el aprendizaje causal automático
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.370 > 0.08 con n=44 PNL=+36.69€
  - _Datos_: n=44 IC=+0.370 PNL=+36.69€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=1126 IC=+0.098 PNL=-253.80€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=1126 IC=+0.098 PNL=-253.80€
