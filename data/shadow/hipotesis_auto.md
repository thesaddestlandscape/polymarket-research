# Hipótesis automáticas — 2026-07-12 10:19 UTC
_Generado por shadow_postmortem.py sobre 10774 resoluciones (PNL=+1878.07€)_
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.188 (n=232)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` < 12.0 (IC base=+0.151)
...
- **PATRÓN** `libro_liquidez` > `5910.4373` → IC=+0.171 (n=147)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 5910.4373 (IC base=+0.151)
...
- **PATRÓN** `py_entrada` < `0.275` → IC=+0.294 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.275 (IC base=+0.199)

- **PATRÓN** `libro_liquidez` > `1648.9469` → IC=+0.230 (n=61)
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.143 (n=928)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` < 11.0 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` < `0.2518` → IC=+0.220 (n=91)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2518 (IC base=+0.111)
...
- **PATRÓN** `sigma_h` > `0.0149` → IC=+0.190 (n=114)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0149 (IC base=+0.154)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.215 (n=233)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.181 (n=302)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0049 (IC base=+0.166)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.207 (n=162)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.166)

- **PATRÓN** `dist_vwap_pct` > `0.4215` → IC=+0.154 (n=24)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.4215 (IC base=+0.166)

- **PATRÓN** `dist_vwap_pct` < `0.1427` → IC=+0.220 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1427 (IC base=+0.166)
...
- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.184 (n=55)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0113 (IC base=+0.122)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.184 (n=36)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 8.0 (IC base=+0.122)
...
### GBM_LATE_15M_TARDIO
- **FILTRO** `sigma_ewma_delta_pct` < `8.556` → IC=-0.147 (n=15)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` < 8.556
  - _Potencial_: sin este filtro IC_bueno=+0.278 (n=7)

- **PATRÓN** `sigma_h` > `0.0136` → IC=+0.121 (n=138)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` > 0.0136 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` < `0.1146` → IC=+0.146 (n=63)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.1146 (IC base=+0.074)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 839 | +0.185 | +8.97€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 541 | +0.213 | +4.36€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 292 | +0.191 | +5.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 179 | +0.213 | +6.48€ | 0 | 8 |
...
| ✅ GBM_LATE_15M | 3547 | +0.113 | +1155.77€ | 0 | 7 |
| ✅ GBM_LATE_15M#15min | 3547 | +0.113 | +1155.77€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#SOL | 921 | +0.139 | +456.96€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 921 | +0.139 | +456.96€ | 0 | 8 |
| ✅ GBM_LATE_15M#XRP | 849 | +0.173 | +453.41€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 849 | +0.173 | +453.41€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 909 | +0.160 | +454.64€ | 0 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 909 | +0.160 | +454.64€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 254 | +0.109 | +59.27€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 254 | +0.109 | +59.27€ | 0 | 5 |
...
| ✅ GBM_LATE_15M_TARDIO | 1089 | +0.080 | +249.64€ | 1 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1089 | +0.080 | +249.64€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#SOL | 283 | +0.033 | +24.45€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 283 | +0.033 | +24.45€ | 0 | 3 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.089 n=6064 | tras_1loss IC=+0.043 n=4573 | tras_2loss IC=+0.009 n=2060/40 | gap=+0.081 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8316 IC=+0.096 PNL=+1953.95€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8316 IC=+0.096 PNL=+1953.95€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=361 IC=-0.062 PNL=+66.18€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=361 IC=-0.062 PNL=+66.18€
