# Hipótesis automáticas — 2026-07-12 11:00 UTC
_Generado por shadow_postmortem.py sobre 10808 resoluciones (PNL=+1892.54€)_
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.193 (n=242)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 12.0 (IC base=+0.155)

- **PATRÓN** `py_entrada` < `0.275` → IC=+0.328 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.275 (IC base=+0.155)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.161 (n=376)

  - _Acción_: Kelly boost +0.81€ cuando `libro_spread` < 0.01 (IC base=+0.155)

- **PATRÓN** `libro_liquidez` > `5937.8835` → IC=+0.184 (n=150)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 5937.8835 (IC base=+0.155)
...
- **PATRÓN** `libro_liquidez` > `1618.5911` → IC=+0.238 (n=63)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.122 (n=1312)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 6.0 (IC base=+0.115)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.121 (n=1308)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.143 (n=933)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` < 11.0 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` < `0.5211` → IC=+0.192 (n=115)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` < 0.5211 (IC base=+0.111)
...
- **PATRÓN** `sigma_h` < `0.0162` → IC=+0.140 (n=351)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0162 (IC base=+0.123)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.163 (n=161)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 16.0 (IC base=+0.123)
...
- **PATRÓN** `sigma_h` < `0.0173` → IC=+0.186 (n=227)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.0173 (IC base=+0.179)

- **PATRÓN** `sigma_h` > `0.0118` → IC=+0.190 (n=340)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0118 (IC base=+0.179)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.258 (n=130)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.179)

- **PATRÓN** `dist_vwap_pct` < `0.098` → IC=+0.231 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.098 (IC base=+0.179)
...
- **PATRÓN** `sigma_h` > `0.0045` → IC=+0.174 (n=348)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.2169` → IC=+0.285 (n=91)
...
- **PATRÓN** `sigma_h` < `0.0081` → IC=+0.244 (n=37)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0081 (IC base=+0.097)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.174 (n=41)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` < 5.0 (IC base=+0.097)

- **PATRÓN** `dist_vwap_pct` < `0.0914` → IC=+0.241 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0914 (IC base=+0.097)
...
- **PATRÓN** `sigma_h` > `0.009` → IC=+0.306 (n=60)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.009 (IC base=+0.280)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.283 (n=67)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.280)

- **PATRÓN** `hora_utc` < `14.0` → IC=+0.287 (n=59)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` > `0.0127` → IC=+0.150 (n=135)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0127 (IC base=+0.088)

- **PATRÓN** `dist_vwap_pct` < `0.1589` → IC=+0.230 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1589 (IC base=+0.088)
...
- **PATRÓN** `sigma_h` < `0.0112` → IC=+0.226 (n=49)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `dist_vwap_pct` < `0.1752` → IC=+0.269 (n=24)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 852 | +0.185 | +11.07€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 548 | +0.214 | +9.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 34 | +0.028 | -2.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 127 | +0.074 | -12.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 143 | +0.203 | +16.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 252 | +0.193 | +0.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 183 | +0.213 | -4.42€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 301 | +0.170 | +2.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 181 | +0.210 | +4.14€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 297 | +0.192 | +7.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 182 | +0.217 | +9.82€ | 0 | 8 |
...
| ✅ GBM_LATE_15M | 3553 | +0.113 | +1154.40€ | 0 | 7 |
| ✅ GBM_LATE_15M#15min | 3553 | +0.113 | +1154.40€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#SOL | 924 | +0.138 | +454.86€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 924 | +0.138 | +454.86€ | 0 | 8 |
| ✅ GBM_LATE_15M#XRP | 851 | +0.173 | +453.29€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 851 | +0.173 | +453.29€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 916 | +0.160 | +461.68€ | 0 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 916 | +0.160 | +461.68€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 257 | +0.106 | +56.73€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 257 | +0.106 | +56.73€ | 0 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 170 | +0.273 | +167.86€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 170 | +0.273 | +167.86€ | 0 | 5 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 1094 | +0.081 | +254.42€ | 1 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1094 | +0.081 | +254.42€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 283 | +0.205 | +193.69€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 283 | +0.205 | +193.69€ | 0 | 8 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.090 n=6087 | tras_1loss IC=+0.044 n=4584 | tras_2loss IC=+0.009 n=2062/40 | gap=+0.081 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8350 IC=+0.096 PNL=+1968.42€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8350 IC=+0.096 PNL=+1968.42€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema, sin filtrar todavía
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.158 < -0.1 con n=308 PNL=-44.60€
  - _Datos_: n=308 IC=-0.158 PNL=-44.60€
