# Hipótesis automáticas — 2026-07-28 12:26 UTC
_Generado por shadow_postmortem.py sobre 40069 resoluciones (PNL=+8210.60€)_
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.191 (n=1242)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0075 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.162 (n=1738)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 7.0 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.6749` → IC=+0.197 (n=229)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.6749 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.398` → IC=+0.233 (n=597)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.398 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `2417.1915` → IC=+0.136 (n=1510)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 2417.1915 (IC base=+0.142)

- **PATRÓN** `sigma_h` > `0.0067` → IC=+0.157 (n=1433)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.392` → IC=+0.129 (n=340)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` > 0.392 (IC base=+0.121)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.137` → IC=+0.149 (n=317)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 8.137 (IC base=+0.121)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.140 (n=428)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 7.0 (IC base=+0.108)

- **PATRÓN** `dist_vwap_pct` > `0.4428` → IC=+0.205 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4428 (IC base=+0.108)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.697` → IC=+0.163 (n=84)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 11.697 (IC base=+0.108)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.404` → IC=+0.148 (n=197)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 4.404 (IC base=+0.093)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.209 (n=156)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0075 (IC base=+0.119)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.143 (n=334)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` > 11.0 (IC base=+0.119)

- **PATRÓN** `dist_vwap_pct` > `0.6615` → IC=+0.171 (n=83)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.6615 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.25` → IC=+0.185 (n=125)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 7.25 (IC base=+0.119)

- **PATRÓN** `libro_liquidez` > `4451.6072` → IC=+0.125 (n=326)

  - _Acción_: Kelly boost +0.62€ cuando `libro_liquidez` > 4451.6072 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.399` → IC=+0.183 (n=102)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 9.399 (IC base=+0.088)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.142 (n=465)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` > 0.0078 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.146 (n=337)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 11.0 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `0.3025` → IC=+0.162 (n=128)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.3025 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.566` → IC=+0.267 (n=200)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.566 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `3211.0609` → IC=+0.209 (n=173)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3211.0609 (IC base=+0.113)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` < `0.0173` → IC=+0.248 (n=430)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0173 (IC base=+0.228)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.247 (n=390)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.563` → IC=+0.344 (n=120)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.563 (IC base=+0.228)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.223 (n=463)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.228)

- **PATRÓN** `sigma_h` < `0.0172` → IC=+0.209 (n=552)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0172 (IC base=+0.206)

- **PATRÓN** `sigma_h` > `0.007` → IC=+0.233 (n=552)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.487` → IC=+0.223 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.487 (IC base=+0.206)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.119` → IC=+0.197 (n=606)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` < 9.119 (IC base=+0.206)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.220 (n=455)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.206)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0103` → IC=+0.125 (n=833)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.63€ cuando `sigma_h` > 0.0103 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` > `0.4559` → IC=+0.155 (n=334)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.4559 (IC base=+0.094)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.031` → IC=+0.232 (n=297)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.031 (IC base=+0.094)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `dist_vwap_pct` > `0.4395` → IC=+0.178 (n=57)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.4395 (IC base=+0.072)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.655` → IC=+0.186 (n=68)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 12.655 (IC base=+0.072)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_ewma_delta_pct` > `12.266` → IC=+0.167 (n=64)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 12.266 (IC base=+0.056)

- **PATRÓN** `libro_liquidez` > `6410.4087` → IC=+0.122 (n=223)

  - _Acción_: Kelly boost +0.61€ cuando `libro_liquidez` > 6410.4087 (IC base=+0.056)

### GBM_LATE_15M_TARDIO#SOL#15min
- **PATRÓN** `sigma_ewma_delta_pct` > `9.068` → IC=+0.283 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.068 (IC base=+0.048)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` < `0.0191` → IC=+0.173 (n=478)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0191 (IC base=+0.168)

- **PATRÓN** `sigma_h` > `0.0103` → IC=+0.167 (n=485)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` > 0.0103 (IC base=+0.168)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.173 (n=548)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 6.0 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` > `0.2414` → IC=+0.230 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2414 (IC base=+0.168)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.932` → IC=+0.284 (n=100)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.932 (IC base=+0.168)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.167 (n=409)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.01 (IC base=+0.168)

- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.169 (n=666)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.1404` → IC=+0.219 (n=165)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1404 (IC base=+0.155)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.452` → IC=+0.152 (n=688)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` < 9.452 (IC base=+0.155)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.163 (n=532)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.6045` → IC=+0.173 (n=644)
...
- **PATRÓN** `dist_vwap_pct` > `0.1666` → IC=+0.164 (n=236)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.1666 (IC base=+0.055)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.225` → IC=+0.157 (n=211)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 7.225 (IC base=+0.055)

- **PATRÓN** `libro_liquidez` > `10166.4857` → IC=+0.139 (n=181)

  - _Acción_: Kelly boost +0.70€ cuando `libro_liquidez` > 10166.4857 (IC base=+0.055)
...
- **PATRÓN** `sigma_h` < `0.0071` → IC=+0.121 (n=323)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` < 0.0071 (IC base=+0.100)

- **PATRÓN** `drift_60min` |x|≤ `0.2197` → IC=+0.138 (n=313)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.69€ cuando `drift_60min` |x|≤ 0.2197 (IC base=+0.100)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0631` → IC=+0.127 (n=314)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.63€ cuando `delta_ratio_macro` |x|> 0.0631 (IC base=+0.100)

- **PATRÓN** `hora_utc` < `14.0` → IC=+0.161 (n=184)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 14.0 (IC base=+0.100)

- **PATRÓN** `ibs_15` > `0.6428` → IC=+0.214 (n=274)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6428 (IC base=+0.100)

- **PATRÓN** `dist_vwap_pct` > `0.257` → IC=+0.242 (n=91)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.257 (IC base=+0.100)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.121` → IC=+0.173 (n=96)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 8.121 (IC base=+0.100)

- **PATRÓN** `libro_liquidez` > `11672.79` → IC=+0.161 (n=107)
...
- **PATRÓN** `ibs_15` > `0.6676` → IC=+0.191 (n=208)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.95€ cuando `ibs_15` > 0.6676 (IC base=+0.043)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.038` → IC=+0.151 (n=61)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 7.038 (IC base=+0.043)

- **PATRÓN** `sigma_h` < `0.0059` → IC=+0.130 (n=182)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` < 0.0059 (IC base=+0.055)

- **PATRÓN** `dist_vwap_pct` > `0.9263` → IC=+0.186 (n=33)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.9263 (IC base=+0.055)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6045 sube el IC de +0.055 a +0.173 en UPDOWN_GBM#15min (n=644). Ya aplicado como kelly_boost=+0.87€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.6428 sube el IC de +0.100 a +0.214 en UPDOWN_GBM#BTC#15min (n=274). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.6676 sube el IC de +0.043 a +0.191 en UPDOWN_GBM#ETH#15min (n=208). Ya aplicado como kelly_boost=+0.95€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO#XRP` — IC=+0.149 n=35. Faltan ~5 resoluciones para umbral n≥40. ETA: ~4h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M_ESPACIO_ATR#BNB#15min` — IC=+0.300 n=38. Faltan ~2 resoluciones para umbral n≥40. ETA: ~1h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M_ESPACIO_ATR#BNB` — IC=+0.300 n=38. Faltan ~2 resoluciones para umbral n≥40. ETA: ~1h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 1419 | +0.098 | +30.05€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 1419 | +0.098 | +30.05€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 564 | +0.092 | +0.40€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 564 | +0.092 | +0.40€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 6901 | +0.177 | -57.66€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 4543 | +0.201 | -47.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 314 | +0.054 | -25.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 821 | +0.128 | -56.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 1223 | +0.152 | +72.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 2041 | +0.182 | -11.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1517 | +0.197 | -53.25€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 104 | +0.038 | -15.67€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 410 | +0.175 | +61.14€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 8 | +0.120 | +5.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min |
42€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 33 | -0.100 | -1.97€ | 0 | 0 |
| 🚫 UPDOWN_GBM#SOL#daily | 12 | -0.129 | -0.13€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 346 | +0.049 | +46.39€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 335 | +0.058 | +49.68€ | 4 | 1 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 2252 | +0.149 | +976.61€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 2252 | +0.149 | +976.61€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 219 | +0.210 | +144.76€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 219 | +0.210 | +144.76€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 232 | +0.111 | +39.29€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 232 | +0.111 | +39.29€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 208 | +0.200 | +135.37€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 208 | +0.200 | +135.37€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 428 | +0.158 | +151.13€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 428 | +0.158 | +151.13€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 530 | +0.086 | +131.32€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 530 | +0.086 | +131.32€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 635 | +0.169 | +374.74€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 635 | +0.169 | +374.74€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.166: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.070 n=1718 | neutral: IC=+0.020 n=1359 | overbought(IBS>0.7): IC=+0.186 n=1663
  - _Datos_: n=5053 IC=+0.102 PNL=+1305.87€
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=254 IC=+0.004 PNL=+12.01€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.109 n=23785 | tras_1loss IC=+0.078 n=16051 | tras_2loss IC=+0.053 n=6742/40 | gap=+0.056 (umbral 0.05)
...
**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  ...
  - _Estado_: alineada_con_outcome_prev IC=+0.049 n=173/60 | contraria IC=+0.106 n=64 | gap=-0.058 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2833 IC=+0.039 PNL=+278.98€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2833 IC=+0.039 PNL=+278.98€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=1134 IC=+0.055 PNL=+153.24€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1134 IC=+0.055 PNL=+153.24€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=510 IC=+0.088 PNL=+89.22€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=510 IC=+0.088 PNL=+89.22€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.089 > 0.08 con n=1425 PNL=+240.92€
  - _Datos_: n=1425 IC=+0.089 PNL=+240.92€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=1503 IC=+0.039 PNL=+115.74€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1503 IC=+0.039 PNL=+115.74€

**〰️ H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  - _Hipótesis_: Feature nueva 2026-06-27: drift del precio YES en Polymarket en últimas 5 obs (~5min). Si poly_drift<0 y decidimos BUY_NO (o poly_drift>0 y BUY_YES) → confluencia. Si diverge → reducción de stake. Hipótesis: confluencia Binance+Polymarket mejora IC; divergencia empeora.
  - _Umbral_: n≥40 en confluencia vs divergencia para validar el boost ×1.1
  - _Acción_: Si IC_confluencia>IC_divergencia con n≥40 → mantener el boost. Si no → retirar.
  - _Estado_: n=432 IC=+0.055 PNL=+50.78€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=432 IC=+0.055 PNL=+50.78€
...
**〰️ H-CUSTOM-CROSS-WINDOW-SPREAD-NEG** — GBM 15min/60min: spread negativo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: n=191 IC=+0.054 PNL=+61.06€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=191 IC=+0.054 PNL=+61.06€
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=3787 IC=+0.063 PNL=+555.16€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=3787 IC=+0.063 PNL=+555.16€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=442 IC=+0.020 PNL=+16.20€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=442 IC=+0.020 PNL=+16.20€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=1284 IC=+0.030 PNL=+122.68€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1284 IC=+0.030 PNL=+122.68€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.159 > 0.1 con n=1036 PNL=+346.30€
  - _Datos_: n=1036 IC=+0.159 PNL=+346.30€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=298 IC=+0.077 PNL=+53.00€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=298 IC=+0.077 PNL=+53.00€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1030 IC=-0.077 PNL=+243.63€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1030 IC=-0.077 PNL=+243.63€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=1129 IC=+0.055 PNL=+121.44€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=1129 IC=+0.055 PNL=+121.44€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.119 > 0.08 con n=730 PNL=+199.48€
  - _Datos_: n=730 IC=+0.119 PNL=+199.48€
...
**🟡 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.170 > 0.08 con n=298 PNL=+108.24€
  - _Datos_: n=298 IC=+0.170 PNL=+108.24€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.203 > 0.2 con n=725 PNL=+417.91€
  - _Datos_: n=725 IC=+0.203 PNL=+417.91€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.103 > 0.08 con n=567 PNL=+146.37€
  - _Datos_: n=567 IC=+0.103 PNL=+146.37€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.105 > 0.08 con n=614 PNL=+181.68€
  - _Datos_: n=614 IC=+0.105 PNL=+181.68€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.125 > 0.08 con n=768 PNL=+367.98€
  - _Datos_: n=768 IC=+0.125 PNL=+367.98€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.205 > 0.08 con n=439 PNL=+26.21€
  - _Datos_: n=439 IC=+0.205 PNL=+26.21€
...
**🟡 H-CUSTOM-ETH15-BUYNO-TARDIO** — UPDOWN_GBM ETH#15min BUY_NO tardío (T_h<0.2) -- edge fuerte no capturado por el aprendizaje causal automático
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.302 > 0.08 con n=114 PNL=+61.51€
  - _Datos_: n=114 IC=+0.302 PNL=+61.51€
