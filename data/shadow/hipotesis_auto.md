# Hipótesis automáticas — 2026-08-26 17:12 UTC
_Generado por shadow_postmortem.py sobre 164250 resoluciones (PNL=+11610.81€)_
...
- **FILTRO** `restante_s_al_confirmar` < `148.66` → IC=-0.280 (n=2271)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=6817)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.222 (n=616)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `12563.6486` → IC=+0.211 (n=199)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12563.6486 (IC base=+0.208)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.134 (n=1177)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.310 (n=393)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.190 (n=4370)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.202 (n=2106)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.166 (n=753)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 11.0 (IC base=+0.165)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.176 (n=1083)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.160 (n=1117)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.242 (n=994)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.191 (n=1069)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.194 (n=10476)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.220 (n=8213)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.242 (n=1817)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.235)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.272 (n=1344)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.236 (n=1666)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.247 (n=1117)
...
- **PATRÓN** `restante_min` < `3.93` → IC=+0.141 (n=1406)
...
- **PATRÓN** `restante_min` < `3.98` → IC=+0.135 (n=713)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.129 (n=2222)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9474` → IC=+0.206 (n=716)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9474 (IC base=+0.062)

- **PATRÓN** `dist_vwap_pct` > `0.211` → IC=+0.234 (n=288)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.211 (IC base=+0.062)

- **PATRÓN** `dist_vwap_pct` < `0.5051` → IC=+0.212 (n=408)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5051 (IC base=+0.062)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.229` → IC=+0.170 (n=876)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 5.229 (IC base=+0.062)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1716` → IC=+0.138 (n=329)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_pendiente_norm` > 0.1716 (IC base=+0.062)

- **PATRÓN** `volumen_spike_ratio` < `1.5713` → IC=+0.136 (n=503)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 1.5713 (IC base=+0.062)

- **PATRÓN** `volumen_spike_ratio` > `1.4564` → IC=+0.129 (n=1142)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.4564 (IC base=+0.062)

- **PATRÓN** `ibs_20min` < `0.2129` → IC=+0.130 (n=1470)

  - _Acción_: Kelly boost +0.65€ cuando `ibs_20min` < 0.2129 (IC base=+0.030)

- **PATRÓN** `dist_vwap_pct` < `0.3023` → IC=+0.145 (n=742)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.3023 (IC base=+0.030)
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.193 (n=161)
  ...
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0076 (IC base=+0.114)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.733` → IC=+0.331 (n=170)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.733 (IC base=+0.114)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3368` → IC=+0.214 (n=313)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3368 (IC base=+0.207)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.233 (n=357)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.4287` → IC=+0.220 (n=355)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4287 (IC base=+0.207)

- **PATRÓN** `dist_vwap_pct` > `0.311` → IC=+0.233 (n=208)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.311 (IC base=+0.207)
...
- **PATRÓN** `volumen_regimen` < `1.2653` → IC=+0.209 (n=355)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.264` → IC=+0.266 (n=45)
...
- **PATRÓN** `libro_liquidez` > `13116.9689` → IC=+0.220 (n=237)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13116.9689 (IC base=+0.207)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0691` → IC=+0.132 (n=142)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.66€ cuando `drift_60min` |x|≤ 0.0691 (IC base=+0.126)
...
- **PATRÓN** `libro_liquidez` > `1918.9386` → IC=+0.136 (n=193)
  ...
  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 1918.9386 (IC base=+0.126)

- **PATRÓN** `ballena_activa_n` < `16.0` → IC=+0.140 (n=48)

  - _Acción_: Kelly boost +0.70€ cuando `ballena_activa_n` < 16.0 (IC base=+0.126)
...
- **FILTRO** `ibs_20min` > `0.4865` → IC=-0.121 (n=629)

  - _Acción_: SKIP cuando `ibs_20min` > 0.4865
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=631)

- **FILTRO** `sigma_ewma_delta_pct` > `4.921` → IC=-0.154 (n=267)
...
- **FILTRO** `ibs_20min` > `0.775` → IC=-0.167 (n=283)

  - _Acción_: SKIP cuando `ibs_20min` > 0.775
  - _Potencial_: sin este filtro IC_bueno=+0.012 (n=850)
...
- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.144 (n=907)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` > 0.0088 (IC base=+0.051)

- **PATRÓN** `ibs_20min` > `0.9441` → IC=+0.246 (n=907)
...
- **PATRÓN** `volumen_regimen` > `1.1596` → IC=+0.194 (n=312)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 1.1596 (IC base=+0.051)

- **PATRÓN** `volumen_pendiente_norm` < `0.1147` → IC=+0.164 (n=1202)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` < 0.1147 (IC base=+0.051)

- **PATRÓN** `volumen_pendiente_norm` > `0.2437` → IC=+0.204 (n=241)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2437 (IC base=+0.051)

- **PATRÓN** `volumen_spike_ratio` < `1.4768` → IC=+0.187 (n=413)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 1.4768 (IC base=+0.051)

- **PATRÓN** `volumen_spike_ratio` > `2.8454` → IC=+0.170 (n=413)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.8454 (IC base=+0.051)

- **PATRÓN** `ballena_activa_n` < `100.0` → IC=+0.263 (n=685)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.08` → IC=+0.189 (n=104)
  ...
  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 5.08 (IC base=-0.016)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0698` → IC=+0.209 (n=139)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0698 (IC base=+0.175)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.027` → IC=+0.290 (n=122)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.027 (IC base=+0.175)

- **PATRÓN** `volumen_pendiente_norm` < `0.1458` → IC=+0.183 (n=314)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` < 0.1458 (IC base=+0.175)

- **PATRÓN** `volumen_pendiente_norm` > `0.4252` → IC=+0.180 (n=48)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.4252 (IC base=+0.175)

- **PATRÓN** `volumen_spike_ratio` < `2.0211` → IC=+0.167 (n=148)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.0211 (IC base=+0.175)

- **PATRÓN** `volumen_spike_ratio` > `3.6756` → IC=+0.182 (n=152)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 3.6756 (IC base=+0.175)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.218 (n=193)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.175)

- **PATRÓN** `libro_liquidez` > `1917.5696` → IC=+0.207 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1917.5696 (IC base=+0.175)

- **PATRÓN** `ballena_activa_n` < `34.0` → IC=+0.193 (n=73)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 34.0 (IC base=+0.175)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.157 (n=65)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=889)
...
- **PATRÓN** `sigma_h` > `0.0242` → IC=+0.320 (n=181)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0242 (IC base=+0.213)
...
- **PATRÓN** `dist_vwap_pct` > `1.3482` → IC=+0.336 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.3482 (IC base=+0.213)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4046` → IC=+0.256 (n=162)
...
- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.187 (n=1063)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0082 (IC base=+0.152)

- **PATRÓN** `drift_60min` |x|≤ `0.3256` → IC=+0.152 (n=2063)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.3256 (IC base=+0.152)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.171 (n=803)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 17.0 (IC base=+0.152)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.159 (n=900)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` < 6.0 (IC base=+0.152)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.282 (n=1161)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.152)

- **PATRÓN** `dist_vwap_pct` > `0.7889` → IC=+0.252 (n=542)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7889 (IC base=+0.152)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.334` → IC=+0.249 (n=998)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.334 (IC base=+0.152)

- **PATRÓN** `volumen_regimen` < `1.2168` → IC=+0.155 (n=1630)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.2168 (IC base=+0.152)

- **PATRÓN** `volumen_regimen` > `0.6824` → IC=+0.170 (n=1457)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 0.6824 (IC base=+0.152)

- **PATRÓN** `volumen_pendiente_norm` > `0.1017` → IC=+0.178 (n=831)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.1017 (IC base=+0.152)

- **PATRÓN** `volumen_spike_ratio` < `2.2702` → IC=+0.150 (n=1845)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 2.2702 (IC base=+0.152)

- **PATRÓN** `volumen_spike_ratio` > `1.8521` → IC=+0.145 (n=1398)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.8521 (IC base=+0.152)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.165 (n=1920)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.152)

- **PATRÓN** `libro_liquidez` > `3900.1145` → IC=+0.188 (n=782)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3900.1145 (IC base=+0.152)

- **PATRÓN** `ballena_activa_n` < `125.0` → IC=+0.169 (n=1102)

  - _Acción_: Kelly boost +0.85€ cuando `ballena_activa_n` < 125.0 (IC base=+0.152)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2461` → IC=+0.146 (n=377)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.2461 (IC base=+0.143)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.190 (n=259)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` < 11.0 (IC base=+0.143)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.310 (n=177)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.143)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.02` → IC=+0.352 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.02 (IC base=+0.143)

- **PATRÓN** `volumen_pendiente_norm` > `0.1402` → IC=+0.163 (n=87)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` > 0.1402 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` > `1.4214` → IC=+0.131 (n=299)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 1.4214 (IC base=+0.143)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.167 (n=217)
...
- **PATRÓN** `dist_vwap_pct` > `0.2143` → IC=+0.249 (n=241)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2143 (IC base=+0.176)
...
- **PATRÓN** `volumen_regimen` < `0.6321` → IC=+0.199 (n=111)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6321 (IC base=+0.176)

- **PATRÓN** `volumen_regimen` > `0.897` → IC=+0.198 (n=220)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` > 0.897 (IC base=+0.176)

- **PATRÓN** `volumen_pendiente_norm` > `0.211` → IC=+0.258 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.211 (IC base=+0.176)
...
- **PATRÓN** `libro_liquidez` > `12042.9066` → IC=+0.198 (n=220)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 12042.9066 (IC base=+0.176)
...
- **PATRÓN** `drift_60min` |x|≤ `0.493` → IC=+0.197 (n=331)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.493 (IC base=+0.182)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.201 (n=336)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.182)

- **PATRÓN** `ibs_20min` > `0.4615` → IC=+0.239 (n=331)
...
- **PATRÓN** `volumen_regimen` > `0.6356` → IC=+0.200 (n=331)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6356 (IC base=+0.182)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.184 (n=375)

  - _Acción_: Kelly boost +0.92€ cuando `libro_spread` < 0.01 (IC base=+0.182)

- **PATRÓN** `libro_liquidez` > `8624.0494` → IC=+0.204 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8624.0494 (IC base=+0.182)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.178 (n=178)
...
- **PATRÓN** `sigma_h` > `0.0225` → IC=+0.184 (n=229)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0225 (IC base=+0.160)
...
- **PATRÓN** `ibs_20min` > `0.9253` → IC=+0.257 (n=336)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9253 (IC base=+0.160)

- **PATRÓN** `dist_vwap_pct` > `1.0022` → IC=+0.247 (n=164)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0022 (IC base=+0.160)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.057` → IC=+0.241 (n=272)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.057 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` < `1.1907` → IC=+0.160 (n=504)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.1907 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` > `0.8248` → IC=+0.189 (n=336)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_regimen` > 0.8248 (IC base=+0.160)

- **PATRÓN** `volumen_pendiente_norm` > `0.0805` → IC=+0.216 (n=192)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0805 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` < `2.1508` → IC=+0.175 (n=410)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.1508 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` > `1.8029` → IC=+0.161 (n=311)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.8029 (IC base=+0.160)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.188 (n=152)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 17.0 (IC base=+0.078)

- **PATRÓN** `ibs_20min` > `0.6242` → IC=+0.144 (n=355)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` > 0.6242 (IC base=+0.078)

- **PATRÓN** `dist_vwap_pct` > `0.7807` → IC=+0.240 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7807 (IC base=+0.078)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.235` → IC=+0.209 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.235 (IC base=+0.078)

- **PATRÓN** `volumen_pendiente_norm` > `0.1717` → IC=+0.186 (n=100)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1717 (IC base=+0.078)

- **PATRÓN** `libro_liquidez` > `2940.9508` → IC=+0.122 (n=265)

  - _Acción_: Kelly boost +0.61€ cuando `libro_liquidez` > 2940.9508 (IC base=+0.078)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.363` → IC=+0.245 (n=1153)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.363 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` < `0.6307` → IC=+0.146 (n=643)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.6307 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` > `1.0819` → IC=+0.157 (n=872)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 1.0819 (IC base=+0.144)

- **PATRÓN** `volumen_pendiente_norm` > `0.1649` → IC=+0.181 (n=687)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1649 (IC base=+0.144)

- **PATRÓN** `volumen_spike_ratio` < `2.3205` → IC=+0.141 (n=2114)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 2.3205 (IC base=+0.144)

- **PATRÓN** `volumen_spike_ratio` > `1.8767` → IC=+0.157 (n=1602)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.8767 (IC base=+0.144)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.152 (n=2221)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.01 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `3828.4706` → IC=+0.192 (n=907)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 3828.4706 (IC base=+0.144)

- **PATRÓN** `ballena_activa_n` < `162.0` → IC=+0.186 (n=1182)

  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 162.0 (IC base=+0.144)

- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.227 (n=1634)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0064 (IC base=+0.193)

- **PATRÓN** `drift_60min` |x|≤ `0.4548` → IC=+0.201 (n=2446)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4548 (IC base=+0.193)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.195 (n=2240)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.7203` → IC=+0.180 (n=2007)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.7203 (IC base=+0.193)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.418` → IC=+0.224 (n=357)
...
- **PATRÓN** `volumen_regimen` < `0.6245` → IC=+0.186 (n=619)
...
- **PATRÓN** `ballena_activa_n` < `181.0` → IC=+0.168 (n=1319)
...
- **PATRÓN** `ibs_20min` > `0.9489` → IC=+0.281 (n=199)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8937` → IC=+0.133 (n=238)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 1.8937 (IC base=+0.133)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.170 (n=222)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.04 (IC base=+0.133)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.184 (n=384)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 8.0 (IC base=+0.154)

- **PATRÓN** `ibs_20min` > `0.3282` → IC=+0.206 (n=430)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.721` → IC=+0.206 (n=117)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.721 (IC base=+0.154)

- **PATRÓN** `volumen_regimen` < `0.6489` → IC=+0.164 (n=144)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.6489 (IC base=+0.154)

- **PATRÓN** `volumen_regimen` > `1.099` → IC=+0.185 (n=195)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` > 1.099 (IC base=+0.154)

- **PATRÓN** `volumen_pendiente_norm` > `0.1444` → IC=+0.229 (n=105)
...
- **PATRÓN** `volumen_spike_ratio` > `1.7052` → IC=+0.195 (n=254)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.7052 (IC base=+0.154)

- **PATRÓN** `libro_liquidez` > `11715.0209` → IC=+0.185 (n=287)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 11715.0209 (IC base=+0.154)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0753` → IC=+0.180 (n=120)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.90€ cuando `drift_60min` |x|≤ 0.0753 (IC base=+0.173)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.241 (n=164)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.173)

- **PATRÓN** `ibs_20min` > `0.7122` → IC=+0.255 (n=321)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7122 (IC base=+0.173)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.113` → IC=+0.340 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.113 (IC base=+0.173)

- **PATRÓN** `volumen_pendiente_norm` < `0.1458` → IC=+0.177 (n=264)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0929` → IC=+0.172 (n=126)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 2.0929 (IC base=+0.173)

- **PATRÓN** `volumen_spike_ratio` > `2.8216` → IC=+0.163 (n=191)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 2.8216 (IC base=+0.173)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.187 (n=244)

  - _Acción_: Kelly boost +0.93€ cuando `libro_spread` < 0.03 (IC base=+0.173)

- **PATRÓN** `libro_liquidez` > `1918.9386` → IC=+0.191 (n=163)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 1918.9386 (IC base=+0.173)

- **PATRÓN** `ballena_activa_n` < `58.0` → IC=+0.219 (n=62)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.272 (n=335)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `ibs_20min` > `0.3557` → IC=+0.198 (n=432)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.539` → IC=+0.195 (n=208)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 4.539 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `0.6318` → IC=+0.187 (n=145)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` < 0.6318 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` > `1.2073` → IC=+0.164 (n=144)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` > 1.2073 (IC base=+0.142)
...
- **PATRÓN** `libro_liquidez` > `4999.3593` → IC=+0.234 (n=288)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4999.3593 (IC base=+0.142)

- **PATRÓN** `ballena_activa_n` < `167.0` → IC=+0.189 (n=204)
...
- **PATRÓN** `sigma_h` > `0.0259` → IC=+0.213 (n=193)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0259 (IC base=+0.170)
...
- **PATRÓN** `dist_vwap_pct` > `1.4364` → IC=+0.291 (n=132)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.4364 (IC base=+0.170)
...
- **PATRÓN** `volumen_regimen` > `1.041` → IC=+0.193 (n=262)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 1.041 (IC base=+0.170)

- **PATRÓN** `volumen_pendiente_norm` > `0.1717` → IC=+0.254 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1717 (IC base=+0.170)

- **PATRÓN** `volumen_spike_ratio` < `1.5675` → IC=+0.168 (n=233)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.5675 (IC base=+0.170)

- **PATRÓN** `volumen_spike_ratio` > `1.829` → IC=+0.181 (n=352)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` > 1.829 (IC base=+0.170)
...
- **PATRÓN** `libro_liquidez` > `3108.1723` → IC=+0.182 (n=193)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 3108.1723 (IC base=+0.170)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.214 (n=306)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.154 (n=908)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3523` → IC=+0.168 (n=610)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.3523 (IC base=+0.148)
...
- **PATRÓN** `ibs_20min` < `0.6296` → IC=+0.159 (n=610)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` < 0.6296 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` > `0.1773` → IC=+0.163 (n=298)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.1773 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` < `0.8091` → IC=+0.146 (n=788)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.8091 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.057` → IC=+0.164 (n=313)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 4.057 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.039` → IC=+0.147 (n=693)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` < 6.039 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` < `1.2237` → IC=+0.160 (n=681)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2237 (IC base=+0.148)

- **PATRÓN** `volumen_pendiente_norm` > `0.0707` → IC=+0.166 (n=333)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.0707 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` < `2.4847` → IC=+0.165 (n=685)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.4847 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` > `1.513` → IC=+0.153 (n=612)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.513 (IC base=+0.148)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.154 (n=908)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `8448.0568` → IC=+0.153 (n=693)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 8448.0568 (IC base=+0.148)
...
- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.163 (n=387)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0064 (IC base=+0.142)

- **PATRÓN** `drift_60min` |x|≤ `0.3481` → IC=+0.150 (n=387)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.3481 (IC base=+0.142)
...
- **PATRÓN** `ibs_20min` < `0.1485` → IC=+0.169 (n=170)
  ...
  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` < 0.1485 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.6272` → IC=+0.190 (n=56)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.6272 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.34` → IC=+0.152 (n=389)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` < 6.34 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `1.1847` → IC=+0.148 (n=387)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 1.1847 (IC base=+0.142)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5555` → IC=+0.156 (n=385)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.5555 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` > `1.794` → IC=+0.145 (n=257)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.794 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `11896.0265` → IC=+0.147 (n=346)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 11896.0265 (IC base=+0.142)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.211 (n=734)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=2372)

- **FILTRO** `ibs_20min` > `0.2725` → IC=-0.175 (n=776)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2725
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=2330)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.180 (n=148)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.014 (n=401)

- **FILTRO** `ballena_activa_n` > `5.0` → IC=-0.146 (n=173)

  - _Acción_: SKIP cuando `ballena_activa_n` > 5.0
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=376)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.141 (n=2175)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=5441)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.288 (n=1836)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=5780)

- **FILTRO** `ibs_7min` < `0.7365` → IC=-0.237 (n=1904)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7365
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=5712)

- **FILTRO** `ballena_activa_n` > `20.0` → IC=-0.176 (n=2534)

  - _Acción_: SKIP cuando `ballena_activa_n` > 20.0
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=5082)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.226 (n=2157)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=7231)

- **FILTRO** `ibs_7min` > `0.7168` → IC=-0.167 (n=2346)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7168
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=7042)
...
- **FILTRO** `drift_7min_pct` |x|> `0.1413` → IC=-0.148 (n=541)
...
- **FILTRO** `ballena_activa_n` > `156.0` → IC=-0.184 (n=378)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=1140)
...
- **FILTRO** `ibs_7min` > `0.8128` → IC=-0.182 (n=410)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.150 (n=389)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=911)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.255 (n=325)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=975)
...
- **FILTRO** `ibs_7min` < `0.7714` → IC=-0.205 (n=357)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=1077)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.206 (n=352)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.171 (n=427)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=1303)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.267 (n=311)
...
- **PATRÓN** `sigma_h` < `0.0029` → IC=+0.143 (n=127)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` < 0.0029 (IC base=+0.112)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0676` → IC=+0.124 (n=381)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.62€ cuando `delta_ratio_macro` |x|> 0.0676 (IC base=+0.112)

- **PATRÓN** `ibs_15` > `0.5217` → IC=+0.202 (n=380)
...
- **PATRÓN** `dist_vwap_pct` > `0.3848` → IC=+0.173 (n=102)
  ...
  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.3848 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.849` → IC=+0.248 (n=145)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.849 (IC base=+0.112)

- **PATRÓN** `libro_liquidez` > `7219.8643` → IC=+0.159 (n=127)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 7219.8643 (IC base=+0.112)

- **PATRÓN** `ballena_activa_n` < `88.0` → IC=+0.182 (n=42)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 88.0 (IC base=+0.112)

- **PATRÓN** `ibs_15` < `0.1143` → IC=+0.137 (n=419)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.68€ cuando `ibs_15` < 0.1143 (IC base=+0.026)
...
- **PATRÓN** `sigma_h` < `0.0053` → IC=+0.131 (n=166)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` < 0.0053 (IC base=+0.014)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0582` → IC=+0.354 (n=46)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0582 (IC base=+0.305)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1529` → IC=+0.319 (n=92)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1529 (IC base=+0.305)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3943` → IC=+0.383 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3943 (IC base=+0.305)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.320 (n=148)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.305)

- **PATRÓN** `ibs_15` > `0.8044` → IC=+0.364 (n=123)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8044 (IC base=+0.305)

- **PATRÓN** `dist_vwap_pct` > `0.447` → IC=+0.378 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.447 (IC base=+0.305)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.489` → IC=+0.335 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.489 (IC base=+0.305)

- **PATRÓN** `libro_liquidez` > `7868.9642` → IC=+0.354 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7868.9642 (IC base=+0.305)

- **PATRÓN** `ballena_activa_n` < `540.0` → IC=+0.381 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 540.0 (IC base=+0.305)

### UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1986` → IC=+0.292 (n=70)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1567` → IC=+0.306 (n=70)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1567 (IC base=+0.276)

- **PATRÓN** `drift_15min` |x|≤ `0.4366` → IC=+0.311 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.4366 (IC base=+0.276)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.305 (n=85)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` > `0.8246` → IC=+0.322 (n=71)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8246 (IC base=+0.276)

- **PATRÓN** `dist_vwap_pct` > `0.447` → IC=+0.375 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.447 (IC base=+0.276)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.871` → IC=+0.292 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.871 (IC base=+0.276)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0504` → IC=+0.409 (n=20)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0504 (IC base=+0.338)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1506` → IC=+0.402 (n=39)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1506 (IC base=+0.338)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.348 (n=44)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` > `0.7053` → IC=+0.418 (n=59)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7053 (IC base=+0.338)

- **PATRÓN** `dist_vwap_pct` > `0.1297` → IC=+0.342 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1297 (IC base=+0.338)

- **PATRÓN** `dist_vwap_pct` < `0.3445` → IC=+0.336 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3445 (IC base=+0.338)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.86` → IC=+0.400 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.86 (IC base=+0.338)

- **PATRÓN** `libro_liquidez` > `3141.8886` → IC=+0.354 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3141.8886 (IC base=+0.338)

- **PATRÓN** `ballena_activa_n` < `198.0` → IC=+0.346 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 198.0 (IC base=+0.338)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.235` → IC=-0.181 (n=330)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 17.235
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=2372)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3346` → IC=+0.141 (n=129)

  - _Acción_: Kelly boost +0.71€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3346 (IC base=-0.057)

- **PATRÓN** `ibs_15` > `0.4526` → IC=+0.162 (n=291)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **FILTRO** `sigma_h` < `0.0027` → IC=-0.215 (n=149)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.203 (n=449)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0603` → IC=+0.182 (n=42)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.91€ cuando `drift_60min` |x|≤ 0.0603 (IC base=+0.042)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3217` → IC=+0.214 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3217 (IC base=+0.042)

- **PATRÓN** `ibs_15` > `0.4476` → IC=+0.177 (n=125)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.89€ cuando `ibs_15` > 0.4476 (IC base=+0.042)

- **PATRÓN** `libro_liquidez` > `9700.4747` → IC=+0.212 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 9700.4747 (IC base=+0.042)
...
### UPDOWN_OU_5M
- **FILTRO** `drift_60min` |x|> `0.1471` → IC=-0.204 (n=25)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.5217 sube el IC de +0.112 a +0.202 en UPDOWN_GBM#15min (n=380). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.4526 sube el IC de -0.057 a +0.162 en UPDOWN_GBM_15M_TARDIO (n=291). Ya aplicado como kelly_boost=+0.81€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_YES, IBS > 0.4476 sube el IC de +0.042 a +0.177 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=125). Ya aplicado como kelly_boost=+0.89€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD**: dentro de BUY_YES, IBS > 0.8044 sube el IC de +0.305 a +0.364 en UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD (n=123). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min**: dentro de BUY_YES, IBS > 0.8246 sube el IC de +0.276 a +0.322 en UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min (n=71). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min**: dentro de BUY_YES, IBS > 0.7053 sube el IC de +0.338 a +0.418 en UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min (n=59). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 6575 | -0.101 | -938.58€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 670 | -0.089 | -119.37€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 5905 | -0.103 | -819.21€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 30645 | +0.113 | -2018.44€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 5713 | +0.186 | -191.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 107 | -0.105 | -50.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 21680 | +0.094 | -1734.91€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3145 | +0.121 | -42.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 3639 | +0.062 | -626.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 20 | -0.045 | +2.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 3614 | +0.063 | -623.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 6353 | +0.133 | -141.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1609 | +0.198 | -82.94€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 3608 | +0.109 | -89.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1094 | +0.129 | +53.05€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 3647 | +0.078 | -474.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 20 | +0.045 | +2.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 3626 | +0.079 | -475.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 6824 | +0.127 | -98.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2081 | +0.168 | -12.34€ | 0 | 8 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 3610 | +0.112 | -55.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1121 | +0.103 | -21.72€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#SOL | 6545 | +0.132 | -421.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1966 | +0.202 | -100.35€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 46 | +0.000 | -9.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 3603 | +0.096 | -238.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 930 | +0.132 | -73.35€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 3637 | +0.105 | -256.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 17 | -0.022 | -0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 3619 | +0.105 | -253.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 5676 | +0.172 | -456.58€ | 3 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 5676 | +0.172 | -456.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 1428 | +0.165 | -155.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 1428 | +0.165 | -155.85€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 1417 | +0.157 | -175.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 1417 | +0.157 | -175.17€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 1271 | +0.231 | -33.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 1271 | +0.231 | -33.38€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 1370 | +0.183 | -108.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 1370 | +0.183 | -108.52€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 15497 | +0.189 | -1398.99€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 15497 | +0.189 | -1398.99€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 2409 | +0.235 | -54.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 2409 | +0.235 | -54.01€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 2669 | +0.160 | -351.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 2669 | +0.160 | -351.46€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 2455 | +0.230 | -71.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 2455 | +0.230 | -71.25€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 2508 | +0.216 | -123.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 2508 | +0.216 | -123.52€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 2610 | +0.183 | -262.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 2610 | +0.183 | -262.73€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 5616 | +0.134 | +205.14€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 5616 | +0.134 | +205.14€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 2826 | +0.126 | +62.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 2826 | +0.126 | +62.83€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 5735 | +0.095 | -219.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 591 | +0.060 | -27.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 5144 | +0.099 | -192.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC | 3964 | +0.095 | -100.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#15min | 591 | +0.060 | -27.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#5min | 3373 | +0.101 | -73.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL | 1771 | +0.094 | -118.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL#5min | 1771 | +0.094 | -118.75€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 7317 | +0.043 | +2301.38€ | 0 | 17 |
| ✅ GBM_LATE_15M#15min | 7317 | +0.043 | +2301.38€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 973 | +0.170 | +599.45€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 973 | +0.170 | +599.45€ | 0 | 18 |
| ✅ GBM_LATE_15M#BTC | 1021 | +0.172 | +586.67€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1021 | +0.172 | +586.67€ | 0 | 29 |
| ✅ GBM_LATE_15M#DOGE | 976 | +0.184 | +649.64€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 976 | +0.184 | +649.64€ | 0 | 18 |
| ✅ GBM_LATE_15M#ETH | 1194 | -0.046 | +32.47€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1194 | -0.046 | +32.47€ | 2 | 10 |
| ✅ GBM_LATE_15M#SOL | 1374 | -0.044 | +139.13€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1374 | -0.044 | +139.13€ | 6 | 3 |
| ✅ GBM_LATE_15M#XRP | 1779 | -0.052 | +294.03€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1779 | -0.052 | +294.03€ | 4 | 6 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 8280 | +0.045 | +3206.87€ | 0 | 16 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 8280 | +0.045 | +3206.87€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 1261 | -0.023 | +628.52€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 1261 | -0.023 | +628.52€ | 2 | 3 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 1813 | -0.035 | +167.29€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 1813 | -0.035 | +167.29€ | 0 | 1 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 846 | +0.236 | +759.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 846 | +0.236 | +759.84€ | 0 | 22 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1401 | -0.042 | +9.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1401 | -0.042 | +9.34€ | 8 | 3 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 1491 | -0.020 | +283.77€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 1491 | -0.020 | +283.77€ | 5 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 1468 | +0.241 | +1358.11€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 1468 | +0.241 | +1358.11€ | 0 | 22 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 5928 | +0.168 | +4017.55€ | 0 | 27 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 5928 | +0.168 | +4017.55€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB | 746 | +0.184 | +523.48€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB#15min | 746 | +0.184 | +523.48€ | 0 | 20 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC | 1008 | +0.163 | +676.71€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC#15min | 1008 | +0.163 | +676.71€ | 0 | 22 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE | 735 | +0.200 | +565.52€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min | 735 | +0.200 | +565.52€ | 0 | 16 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH | 985 | +0.161 | +629.56€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH#15min | 985 | +0.161 | +629.56€ | 0 | 25 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL | 1142 | +0.119 | +648.18€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL#15min | 1142 | +0.119 | +648.18€ | 1 | 19 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP | 1312 | +0.190 | +974.10€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP#15min | 1312 | +0.190 | +974.10€ | 0 | 24 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 1009 | +0.076 | +185.48€ | 0 | 13 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 1009 | +0.076 | +185.48€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#DOGE | 11 | -0.021 | -0.37€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#DOGE#15min | 11 | -0.021 | -0.37€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO | 6886 | +0.167 | +4507.17€ | 0 | 26 |
| ✅ GBM_LATE_15M_TARDIO#15min | 6886 | +0.167 | +4507.17€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 930 | +0.187 | +655.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 930 | +0.187 | +655.53€ | 0 | 19 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 1123 | +0.160 | +694.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 1123 | +0.160 | +694.60€ | 0 | 28 |
| ✅ GBM_LATE_15M_TARDIO#DOGE | 920 | +0.218 | +763.26€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#DOGE#15min | 920 | +0.218 | +763.26€ | 0 | 19 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 1051 | +0.146 | +600.23€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 1051 | +0.146 | +600.23€ | 0 | 25 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 1249 | +0.105 | +609.55€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 1249 | +0.105 | +609.55€ | 0 | 19 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 1613 | +0.192 | +1184.00€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1613 | +0.192 | +1184.00€ | 0 | 23 |
| ✅ GBM_LATE_5M | 1484 | +0.132 | +697.02€ | 1 | 25 |
| ✅ GBM_LATE_5M#5min | 1484 | +0.132 | +697.02€ | 0 | 0 |
| ✅ GBM_LATE_5M#BNB | 76 | +0.192 | +46.99€ | 0 | 0 |
| ✅ GBM_LATE_5M#BNB#5min | 76 | +0.192 | +46.99€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC | 588 | +0.134 | +319.86€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC#5min | 588 | +0.134 | +319.86€ | 3 | 20 |
...
| ✅ MOMENTUM_IBS_15M | 4246 | -0.003 | -72.24€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#15min | 4246 | -0.003 | -72.24€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB | 479 | -0.007 | +1.35€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB#15min | 479 | -0.007 | +1.35€ | 1 | 0 |
| ✅ MOMENTUM_IBS_15M#BTC | 598 | +0.005 | -8.60€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BTC#15min | 598 | +0.005 | -8.60€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#DOGE | 723 | -0.003 | -22.26€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#DOGE#15min | 723 | -0.003 | -22.26€ | 1 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH | 841 | +0.014 | +17.85€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH#15min | 841 | +0.014 | +17.85€ | 1 | 0 |
| ✅ MOMENTUM_IBS_15M#SOL | 753 | -0.014 | -32.91€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#SOL#15min | 753 | -0.014 | -32.91€ | 1 | 0 |
| ✅ MOMENTUM_IBS_15M#XRP | 852 | -0.012 | -27.67€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#XRP#15min | 852 | -0.012 | -27.67€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA | 6195 | -0.035 | +208.28€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#15min | 6195 | -0.035 | +208.28€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB | 983 | -0.022 | +142.75€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB#15min | 983 | -0.022 | +142.75€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC | 1112 | -0.035 | -28.33€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC#15min | 1112 | -0.035 | -28.33€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE | 988 | -0.042 | +99.05€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE#15min | 988 | -0.042 | +99.05€ | 4 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH | 1110 | -0.034 | -31.96€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH#15min | 1110 | -0.034 | -31.96€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL | 1001 | -0.042 | +33.16€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL#15min | 1001 | -0.042 | +33.16€ | 4 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP | 1001 | -0.030 | -6.37€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP#15min | 1001 | -0.030 | -6.37€ | 4 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 17004 | -0.073 | +280.07€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 17004 | -0.073 | +280.07€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 2629 | -0.091 | +294.44€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 2629 | -0.091 | +294.44€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 3073 | -0.060 | -6.55€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 3073 | -0.060 | -6.55€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 2713 | -0.085 | +20.26€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 2713 | -0.085 | +20.26€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 2592 | -0.096 | -180.95€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 2592 | -0.096 | -180.95€ | 8 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 3164 | -0.047 | +20.38€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 3164 | -0.047 | +20.38€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 2833 | -0.068 | +132.50€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 2833 | -0.068 | +132.50€ | 5 | 0 |
...
| ✅ UPDOWN_GBM | 5780 | +0.007 | +148.41€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 2178 | +0.046 | +216.16€ | 0 | 8 |
| ✅ UPDOWN_GBM#240min | 246 | +0.020 | +3.34€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 2947 | -0.017 | -61.33€ | 2 | 0 |
...
| ✅ UPDOWN_GBM#ETH | 1330 | +0.007 | +10.71€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 612 | +0.026 | +21.55€ | 1 | 6 |
...
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD | 183 | +0.305 | +29.10€ | 0 | 9 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#15min | 183 | +0.305 | +29.10€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC | 105 | +0.276 | +4.93€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min | 105 | +0.276 | +4.93€ | 0 | 9 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH | 78 | +0.338 | +24.18€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min | 78 | +0.338 | +24.18€ | 0 | 11 |
| ✅ UPDOWN_GBM_15M_TARDIO | 3661 | -0.073 | +836.94€ | 2 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 3661 | -0.073 | +836.94€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 301 | -0.051 | +340.81€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 301 | -0.051 | +340.81€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 752 | -0.162 | -78.07€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 752 | -0.162 | -78.07€ | 4 | 1 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 77 | +0.070 | +14.56€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 77 | +0.070 | +14.56€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 269 | +0.116 | +114.20€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 269 | +0.116 | +114.20€ | 2 | 16 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 1161 | -0.066 | +231.37€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 1161 | -0.066 | +231.37€ | 3 | 1 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 1101 | -0.083 | +214.07€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 1101 | -0.083 | +214.07€ | 5 | 0 |
...
| ✅ UPDOWN_OU_5M | 421 | -0.077 | -39.22€ | 3 | 0 |
| ✅ UPDOWN_OU_5M#5min | 421 | -0.077 | -39.22€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.087) — sin ventaja clara. oversold(IBS<0.3): IC=+0.009 n=2029 | neutral: IC=+0.002 n=2183 | overbought(IBS>0.7): IC=+0.089 n=2254
  - _Datos_: n=6772 IC=+0.034 PNL=+567.28€

**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  - _Umbral_: n≥40 por celda + gate riguroso completo (Wilson+shuffle+PnL bootstrap)
  - _Acción_: Añadir claves 'ESTRATEGIA#SUBTYPE#DIRECCION#HORA':1.2 a meta.hora_boost_factor, solo por celda confirmada
  - _Estado_: 17 celda(s) pasan gate riguroso completo de 708 evaluadas (n>=40) y 2021 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.075 n=92094 | tras_1loss IC=+0.044 n=71851 | tras_2loss IC=+0.008 n=32687/40 | gap=+0.068 (umbral 0.05)
...
**⏳ H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  ...
  - _Estado_: 5718 ops, 22 horas distintas. Sin hora con n≥15 y IC extremo aún.
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=-0.011 n=609 | contrario_BTC IC=-0.002 n=448/40 | gap=+0.008 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**🟡 H-24H-GBM-BUYYES-TARDE** — GBM BUY_YES en tarde europea (15-19h UTC) — señal alcista sostenida
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.132 > 0.08 con n=93 PNL=+23.23€
  - _Datos_: n=93 IC=+0.132 PNL=+23.23€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=5539 IC=+0.003 PNL=+97.70€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=5539 IC=+0.003 PNL=+97.70€
...
**🟡 H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.112 > 0.1 con n=506 PNL=+108.81€
  - _Datos_: n=506 IC=+0.112 PNL=+108.81€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=1257 IC=+0.034 PNL=+83.06€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1257 IC=+0.034 PNL=+83.06€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=1553 IC=+0.027 PNL=+101.74€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1553 IC=+0.027 PNL=+101.74€
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=7514 IC=+0.092 PNL=+1936.28€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=7514 IC=+0.092 PNL=+1936.28€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=1420 IC=+0.021 PNL=+68.52€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1420 IC=+0.021 PNL=+68.52€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.165 > 0.1 con n=756 PNL=+270.50€
  - _Datos_: n=756 IC=+0.165 PNL=+270.50€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=4047 IC=-0.139 PNL=+219.32€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=4047 IC=-0.139 PNL=+219.32€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.112 > 0.08 con n=506 PNL=+108.81€
  - _Datos_: n=506 IC=+0.112 PNL=+108.81€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=1067 IC=+0.131 PNL=+522.85€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=1067 IC=+0.131 PNL=+522.85€
...
**〰️ H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: n=617 IC=-0.027 PNL=+37.86€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=617 IC=-0.027 PNL=+37.86€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.182 > 0.08 con n=524 PNL=+315.43€
  - _Datos_: n=524 IC=+0.182 PNL=+315.43€
...
**〰️ H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: n=943 IC=-0.043 PNL=+170.16€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=943 IC=-0.043 PNL=+170.16€
