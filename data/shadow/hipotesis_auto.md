# Hipótesis automáticas — 2026-07-21 06:17 UTC
_Generado por shadow_postmortem.py sobre 26131 resoluciones (PNL=+6288.90€)_
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.207 (n=1656)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.205 (n=1838)
...
- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.125 (n=1609)
...
- **PATRÓN** `sigma_h` > `0.0255` → IC=+0.191 (n=215)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0255 (IC base=+0.167)
...
- **PATRÓN** `dist_vwap_pct` > `0.3076` → IC=+0.203 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3076 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.7` → IC=+0.165 (n=320)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` < 3.7 (IC base=+0.167)
...
- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.187 (n=845)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.3811` → IC=+0.171 (n=1082)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` < 0.3811 (IC base=+0.156)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.097` → IC=+0.186 (n=234)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 6.097 (IC base=+0.156)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.004` → IC=+0.159 (n=863)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` < 3.004 (IC base=+0.156)
...
- **PATRÓN** `sigma_h` < `0.0157` → IC=+0.285 (n=315)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0157 (IC base=+0.281)

- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.304 (n=315)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0077 (IC base=+0.281)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.300 (n=318)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.281)

- **PATRÓN** `dist_vwap_pct` > `0.3142` → IC=+0.359 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3142 (IC base=+0.281)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.538` → IC=+0.292 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.538 (IC base=+0.281)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.004` → IC=+0.289 (n=264)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.004 (IC base=+0.281)
...
- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.173 (n=102)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0083 (IC base=+0.051)
...
- **PATRÓN** `sigma_h` > `0.0208` → IC=+0.215 (n=135)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0208 (IC base=+0.205)
...
- **PATRÓN** `dist_vwap_pct` > `0.4269` → IC=+0.254 (n=59)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4269 (IC base=+0.205)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.122` → IC=+0.213 (n=374)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 10.122 (IC base=+0.205)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.7529` → IC=+0.216 (n=336)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7529 (IC base=+0.046)

- **PATRÓN** `dist_vwap_pct` > `0.133` → IC=+0.171 (n=165)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.133 (IC base=+0.046)

- **PATRÓN** `dist_vwap_pct` < `0.473` → IC=+0.146 (n=244)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.473 (IC base=+0.046)

- **PATRÓN** `sigma_ewma_delta_pct` > `23.684` → IC=+0.181 (n=45)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 23.684 (IC base=+0.046)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS > 0.7529 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.216 n=336). Confirma señal de reversión media → alinear con BUY_NO.
...
| ✅ FAVORITO_CONFIRMADO | 4514 | +0.186 | +59.31€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 2936 | +0.217 | +97.49€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 1581 | +0.183 | +1.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 971 | +0.224 | +54.87€ | 0 | 6 |
...
| ✅ GBM_LATE_15M | 6405 | +0.102 | +2124.32€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 6405 | +0.102 | +2124.32€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#SOL | 1736 | +0.091 | +629.97€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1736 | +0.091 | +629.97€ | 0 | 4 |
| ✅ GBM_LATE_15M#XRP | 1687 | +0.161 | +940.17€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1687 | +0.161 | +940.17€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 3601 | +0.142 | +2065.28€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 3601 | +0.142 | +2065.28€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 914 | +0.109 | +407.37€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 914 | +0.109 | +407.37€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 828 | +0.266 | +851.49€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 828 | +0.266 | +851.49€ | 0 | 12 |
...
| ✅ GBM_LATE_15M_TARDIO | 3580 | +0.093 | +1220.82€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 3580 | +0.093 | +1220.82€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#SOL | 814 | +0.048 | +172.51€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 814 | +0.048 | +172.51€ | 1 | 2 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 1045 | +0.204 | +798.48€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1045 | +0.204 | +798.48€ | 0 | 13 |
...
| ✅ UPDOWN_GBM | 2034 | +0.026 | +159.41€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1772 | +0.050 | +198.49€ | 0 | 4 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 932 | +0.173 | +435.34€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 932 | +0.173 | +435.34€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 87 | +0.197 | +46.82€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 87 | +0.197 | +46.82€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 228 | +0.100 | +69.40€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 228 | +0.100 | +69.40€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 230 | +0.237 | +185.82€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 230 | +0.237 | +185.82€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.194: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.059 n=914 | neutral: IC=+0.001 n=689 | overbought(IBS>0.7): IC=+0.195 n=900
  - _Datos_: n=2680 IC=+0.097 PNL=+638.44€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.117 n=15728 | tras_1loss IC=+0.088 n=10201 | tras_2loss IC=+0.053 n=4164/40 | gap=+0.064 (umbral 0.05)
...
**🟡 H-24H-GBM-BUYYES-MADRUGADA** — GBM BUY_YES en madrugada europea (05-07h UTC) — señal alcista
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.096 > 0.08 con n=92 PNL=+30.36€
  - _Datos_: n=92 IC=+0.096 PNL=+30.36€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1902 IC=+0.032 PNL=+174.16€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1902 IC=+0.032 PNL=+174.16€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=847 IC=+0.046 PNL=+93.60€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=847 IC=+0.046 PNL=+93.60€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.082 > 0.08 con n=947 PNL=+140.39€
  - _Datos_: n=947 IC=+0.082 PNL=+140.39€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=23667 IC=+0.118 PNL=+6363.75€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=23667 IC=+0.118 PNL=+6363.75€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=997 IC=+0.017 PNL=+63.05€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=997 IC=+0.017 PNL=+63.05€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.170 > 0.1 con n=616 PNL=+226.69€
  - _Datos_: n=616 IC=+0.170 PNL=+226.69€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.145 > 0.08 con n=443 PNL=+139.85€
  - _Datos_: n=443 IC=+0.145 PNL=+139.85€
