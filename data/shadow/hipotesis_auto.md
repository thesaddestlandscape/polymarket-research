# Hipótesis automáticas — 2026-08-19 00:11 UTC
_Generado por shadow_postmortem.py sobre 69411 resoluciones (PNL=+6917.43€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `n_ballenas` < `5.0` → IC=-0.126 (n=2203)

  - _Acción_: SKIP cuando `n_ballenas` < 5.0
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=1492)

- **FILTRO** `restante_s_al_confirmar` < `151.49` → IC=-0.250 (n=923)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 151.49
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=2772)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `118.97` → IC=-0.400 (n=98)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 118.97
  - _Potencial_: sin este filtro IC_bueno=-0.119 (n=295)
...
- **PATRÓN** `libro_liquidez` > `2373.5135` → IC=+0.168 (n=1135)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.157 (n=3129)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 12.0 (IC base=+0.152)

- **PATRÓN** `py_entrada` < `0.34` → IC=+0.286 (n=1518)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.34 (IC base=+0.152)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.191 (n=2030)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.02 (IC base=+0.152)

- **PATRÓN** `libro_liquidez` > `4052.555` → IC=+0.181 (n=823)
...
- **PATRÓN** `py_entrada` < `0.335` → IC=+0.301 (n=244)
...
- **PATRÓN** `py_entrada` < `0.31` → IC=+0.353 (n=230)
...
- **PATRÓN** `libro_liquidez` > `913.9984` → IC=+0.246 (n=305)
...
- **PATRÓN** `py_entrada` < `0.72` → IC=+0.188 (n=508)
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.216 (n=297)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.220 (n=797)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.220)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.293 (n=268)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.257 (n=274)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.197 (n=305)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `ibs_20min` > `0.9882` → IC=+0.266 (n=429)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9882 (IC base=+0.100)

- **PATRÓN** `dist_vwap_pct` > `0.5859` → IC=+0.321 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5859 (IC base=+0.100)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.715` → IC=+0.236 (n=612)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.715 (IC base=+0.100)

- **PATRÓN** `volumen_regimen` < `1.294` → IC=+0.258 (n=163)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.294 (IC base=+0.100)

- **PATRÓN** `volumen_regimen` > `0.6468` → IC=+0.250 (n=162)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6468 (IC base=+0.100)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.471` → IC=+0.345 (n=153)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.471 (IC base=+0.121)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.411` → IC=+0.144 (n=57)
  ...
  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` > 8.411 (IC base=-0.030)

- **PATRÓN** `volumen_regimen` < `0.7863` → IC=+0.167 (n=22)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.7863 (IC base=-0.030)
...
- **PATRÓN** `ibs_20min` > `0.9441` → IC=+0.234 (n=551)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.21` → IC=+0.127 (n=1021)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` > 2.21 (IC base=+0.050)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2038` → IC=+0.156 (n=448)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 2.2038 (IC base=+0.050)

- **PATRÓN** `ballena_activa_n` < `38.0` → IC=+0.269 (n=76)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3516` → IC=+0.357 (n=47)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3516 (IC base=+0.048)

- **PATRÓN** `volumen_spike_ratio` > `3.7273` → IC=+0.316 (n=85)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.7273 (IC base=+0.048)

- **PATRÓN** `ballena_activa_n` < `71.0` → IC=+0.322 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 71.0 (IC base=+0.048)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.363` → IC=+0.172 (n=126)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 3.363 (IC base=-0.015)
...
- **FILTRO** `ibs_20min` < `0.0555` → IC=-0.148 (n=86)

  - _Acción_: SKIP cuando `ibs_20min` < 0.0555
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=260)
...
- **PATRÓN** `sigma_h` > `0.005` → IC=+0.300 (n=278)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.3173` → IC=+0.337 (n=310)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3173 (IC base=+0.285)

- **PATRÓN** `dist_vwap_pct` > `0.5441` → IC=+0.375 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5441 (IC base=+0.285)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.02` → IC=+0.289 (n=311)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.02 (IC base=+0.285)

- **PATRÓN** `volumen_regimen` > `0.8965` → IC=+0.318 (n=207)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8965 (IC base=+0.285)

- **PATRÓN** `volumen_pendiente_norm` > `0.2896` → IC=+0.379 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2896 (IC base=+0.285)

- **PATRÓN** `volumen_spike_ratio` > `3.5418` → IC=+0.318 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.5418 (IC base=+0.285)

- **PATRÓN** `libro_liquidez` > `2852.5264` → IC=+0.292 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2852.5264 (IC base=+0.285)
...
- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.193 (n=588)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0065 (IC base=+0.139)
...
- **PATRÓN** `ibs_20min` > `0.9206` → IC=+0.251 (n=863)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9206 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` > `0.2288` → IC=+0.157 (n=295)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.2288 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.935` → IC=+0.268 (n=635)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.935 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` < `1.1809` → IC=+0.145 (n=671)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.1809 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` > `0.6258` → IC=+0.143 (n=671)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 0.6258 (IC base=+0.139)

- **PATRÓN** `volumen_pendiente_norm` > `0.1102` → IC=+0.154 (n=435)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_pendiente_norm` > 0.1102 (IC base=+0.139)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.146 (n=1459)

  - _Acción_: Kelly boost +0.73€ cuando `libro_spread` < 0.06 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `2699.9007` → IC=+0.179 (n=431)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 2699.9007 (IC base=+0.139)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1492` → IC=+0.140 (n=73)
  ...
  - _Acción_: Kelly boost +0.70€ cuando `volumen_pendiente_norm` > 0.1492 (IC base=+0.148)

- **PATRÓN** `libro_spread` < `0.08` → IC=+0.165 (n=350)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1726` → IC=+0.297 (n=62)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1726 (IC base=+0.250)

- **PATRÓN** `hora_utc` > `10.0` → IC=+0.250 (n=82)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.7312` → IC=+0.308 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7312 (IC base=+0.250)

- **PATRÓN** `dist_vwap_pct` < `0.364` → IC=+0.287 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.364 (IC base=+0.250)
...
- **PATRÓN** `volumen_regimen` > `0.6636` → IC=+0.277 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6636 (IC base=+0.250)

- **PATRÓN** `volumen_pendiente_norm` > `0.1116` → IC=+0.276 (n=47)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1116 (IC base=+0.250)

- **PATRÓN** `volumen_spike_ratio` < `1.688` → IC=+0.275 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.688 (IC base=+0.250)

- **PATRÓN** `volumen_spike_ratio` > `2.5007` → IC=+0.256 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5007 (IC base=+0.250)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.250 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.250)

- **PATRÓN** `libro_liquidez` > `2543.553` → IC=+0.255 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2543.553 (IC base=+0.250)
...
- **PATRÓN** `ibs_20min` > `0.9601` → IC=+0.173 (n=108)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` > 0.9601 (IC base=+0.047)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.961` → IC=+0.250 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.961 (IC base=+0.047)

- **PATRÓN** `volumen_pendiente_norm` > `0.1778` → IC=+0.148 (n=52)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` > 0.1778 (IC base=+0.047)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.194 (n=524)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.9631` → IC=+0.281 (n=710)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9631 (IC base=+0.119)

- **PATRÓN** `dist_vwap_pct` > `0.3604` → IC=+0.185 (n=179)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.3604 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.252` → IC=+0.232 (n=1052)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.252 (IC base=+0.119)

- **PATRÓN** `volumen_pendiente_norm` > `0.1809` → IC=+0.134 (n=372)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_pendiente_norm` > 0.1809 (IC base=+0.119)

- **PATRÓN** `volumen_spike_ratio` > `1.6912` → IC=+0.121 (n=1125)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_spike_ratio` > 1.6912 (IC base=+0.119)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.125 (n=1754)

  - _Acción_: Kelly boost +0.62€ cuando `libro_spread` < 0.06 (IC base=+0.119)

- **PATRÓN** `libro_liquidez` > `2741.8936` → IC=+0.155 (n=522)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 2741.8936 (IC base=+0.119)

- **PATRÓN** `ballena_activa_n` < `147.0` → IC=+0.186 (n=189)

  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 147.0 (IC base=+0.119)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.232 (n=1221)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.225)

- **PATRÓN** `sigma_h` > `0.0038` → IC=+0.231 (n=1385)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.5` → IC=+0.287 (n=1388)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0228` → IC=+0.273 (n=465)
...
- **PATRÓN** `ibs_20min` > `0.9489` → IC=+0.284 (n=179)
...
- **FILTRO** `ibs_20min` < `0.3033` → IC=-0.217 (n=51)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.250 (n=154)

- **PATRÓN** `sigma_h` < `0.0026` → IC=+0.157 (n=103)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0026 (IC base=+0.133)

- **PATRÓN** `sigma_h` > `0.0034` → IC=+0.185 (n=52)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0034 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.286 (n=54)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.133)

- **PATRÓN** `ibs_20min` > `0.3033` → IC=+0.250 (n=154)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3033 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` > `0.2647` → IC=+0.278 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2647 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.779` → IC=+0.268 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.779 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` < `0.6694` → IC=+0.167 (n=52)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.6694 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` > `1.1082` → IC=+0.167 (n=70)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 1.1082 (IC base=+0.133)

- **PATRÓN** `volumen_pendiente_norm` < `0.1584` → IC=+0.193 (n=112)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` < 0.1584 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` < `2.8608` → IC=+0.230 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.8608 (IC base=+0.133)

- **PATRÓN** `libro_liquidez` > `9302.815` → IC=+0.250 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 9302.815 (IC base=+0.133)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.287 (n=351)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `dist_vwap_pct` > `0.3858` → IC=+0.354 (n=46)
...
- **PATRÓN** `volumen_regimen` > `0.8813` → IC=+0.301 (n=234)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8813 (IC base=+0.261)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5095` → IC=+0.258 (n=60)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5095 (IC base=+0.261)

- **PATRÓN** `volumen_spike_ratio` > `2.6928` → IC=+0.271 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6928 (IC base=+0.261)

- **PATRÓN** `libro_liquidez` > `2832.6472` → IC=+0.265 (n=117)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2832.6472 (IC base=+0.261)

- **PATRÓN** `ballena_activa_n` < `53.0` → IC=+0.238 (n=63)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.192 (n=157)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0039 (IC base=+0.132)

- **PATRÓN** `drift_60min` |x|≤ `0.0725` → IC=+0.241 (n=52)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0725 (IC base=+0.132)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.138 (n=56)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 15.0 (IC base=+0.132)

- **PATRÓN** `hora_utc` < `3.0` → IC=+0.209 (n=53)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 3.0 (IC base=+0.132)

- **PATRÓN** `ibs_20min` < `0.5313` → IC=+0.152 (n=156)

  - _Acción_: Kelly boost +0.76€ cuando `ibs_20min` < 0.5313 (IC base=+0.132)

- **PATRÓN** `dist_vwap_pct` > `0.2485` → IC=+0.143 (n=40)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.2485 (IC base=+0.132)

- **PATRÓN** `dist_vwap_pct` < `0.1779` → IC=+0.138 (n=161)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.1779 (IC base=+0.132)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.332` → IC=+0.141 (n=37)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 11.332 (IC base=+0.132)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.997` → IC=+0.133 (n=137)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` < 5.997 (IC base=+0.132)

- **PATRÓN** `volumen_regimen` < `1.3953` → IC=+0.165 (n=156)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.3953 (IC base=+0.132)

- **PATRÓN** `volumen_regimen` > `0.6531` → IC=+0.139 (n=156)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 0.6531 (IC base=+0.132)

- **PATRÓN** `volumen_pendiente_norm` < `0.1409` → IC=+0.146 (n=145)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_pendiente_norm` < 0.1409 (IC base=+0.132)

- **PATRÓN** `volumen_spike_ratio` < `2.8806` → IC=+0.173 (n=154)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.8806 (IC base=+0.132)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.160 (n=192)

  - _Acción_: Kelly boost +0.80€ cuando `libro_spread` < 0.01 (IC base=+0.132)

- **PATRÓN** `libro_liquidez` > `8454.4508` → IC=+0.160 (n=139)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 8454.4508 (IC base=+0.132)

### GBM_LATE_5M#BTC#5min
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.174 (n=84)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0028 (IC base=+0.125)
...
- **PATRÓN** `hora_utc` < `9.0` → IC=+0.129 (n=68)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 9.0 (IC base=+0.125)

- **PATRÓN** `ibs_20min` < `0.1217` → IC=+0.182 (n=64)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` < 0.1217 (IC base=+0.125)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.189` → IC=+0.146 (n=97)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` < 10.189 (IC base=+0.125)

- **PATRÓN** `volumen_regimen` < `1.1967` → IC=+0.139 (n=95)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` < 1.1967 (IC base=+0.125)

- **PATRÓN** `volumen_regimen` > `0.6265` → IC=+0.139 (n=95)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 0.6265 (IC base=+0.125)

- **PATRÓN** `volumen_pendiente_norm` > `0.1563` → IC=+0.218 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1563 (IC base=+0.125)

- **PATRÓN** `volumen_spike_ratio` < `2.7111` → IC=+0.160 (n=95)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.7111 (IC base=+0.125)

- **PATRÓN** `volumen_spike_ratio` > `1.4927` → IC=+0.149 (n=95)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.4927 (IC base=+0.125)

### GBM_LATE_5M#ETH#5min
- **PATRÓN** `sigma_h` < `0.0025` → IC=+0.300 (n=28)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0025 (IC base=+0.237)

- **PATRÓN** `drift_60min` |x|≤ `0.1878` → IC=+0.300 (n=28)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1878 (IC base=+0.237)

- **PATRÓN** `hora_utc` < `9.0` → IC=+0.348 (n=31)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 9.0 (IC base=+0.237)

- **PATRÓN** `ibs_20min` > `0.2032` → IC=+0.267 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.2032 (IC base=+0.237)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.54` → IC=+0.262 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.54 (IC base=+0.237)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.369` → IC=+0.274 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.369 (IC base=+0.237)

- **PATRÓN** `volumen_regimen` < `1.3884` → IC=+0.295 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.3884 (IC base=+0.237)

- **PATRÓN** `volumen_pendiente_norm` < `0.1037` → IC=+0.305 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1037 (IC base=+0.237)

- **PATRÓN** `volumen_spike_ratio` < `1.8733` → IC=+0.333 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8733 (IC base=+0.237)

- **PATRÓN** `libro_liquidez` > `8304.7792` → IC=+0.267 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8304.7792 (IC base=+0.237)
...
- **PATRÓN** `hora_utc` < `9.0` → IC=+0.283 (n=21)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 9.0 (IC base=+0.221)

- **PATRÓN** `py_entrada` < `0.505` → IC=+0.308 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.505 (IC base=+0.221)
...
- **PATRÓN** `ibs_20min` < `0.9585` → IC=+0.154 (n=24)
  ...
  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.9585 (IC base=+0.092)
...
- **FILTRO** `ibs_20min` < `0.7235` → IC=-0.248 (n=109)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7235
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=330)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.209 (n=108)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=331)

- **FILTRO** `libro_liquidez` < `2618.4646` → IC=-0.128 (n=329)

  - _Acción_: SKIP cuando `libro_liquidez` < 2618.4646
...
- **FILTRO** `py_entrada` < `0.38` → IC=-0.300 (n=18)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.177 (n=29)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 17.0 (IC base=+0.057)

- **PATRÓN** `py_entrada` < `0.69` → IC=+0.125 (n=62)

  - _Acción_: Kelly boost +0.62€ cuando `py_entrada` < 0.69 (IC base=+0.057)
...
- **PATRÓN** `libro_liquidez` > `2675.9522` → IC=+0.190 (n=27)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 2675.9522 (IC base=+0.013)
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.279 (n=301)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=916)

- **FILTRO** `ibs_7min` < `0.7241` → IC=-0.235 (n=304)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7241
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=913)

- **FILTRO** `ballena_activa_n` > `25.0` → IC=-0.191 (n=302)

  - _Acción_: SKIP cuando `ballena_activa_n` > 25.0
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=915)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.194 (n=321)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=1008)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.240 (n=48)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.105 (n=145)

- **FILTRO** `ibs_7min` < `0.8485` → IC=-0.204 (n=96)

  - _Acción_: SKIP cuando `ibs_7min` < 0.8485
  - _Potencial_: sin este filtro IC_bueno=-0.076 (n=97)

- **FILTRO** `ballena_activa_n` > `4.0` → IC=-0.228 (n=90)
...
- **FILTRO** `py_entrada` < `0.38` → IC=-0.288 (n=64)
...
- **FILTRO** `py_entrada` < `0.495` → IC=-0.214 (n=96)
...
- **FILTRO** `ballena_activa_n` > `29.0` → IC=-0.192 (n=50)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.159 (n=83)

  - _Acción_: Kelly boost +0.79€ cuando `py_entrada` < 0.5 (IC base=-0.015)
...
- **PATRÓN** `delta_ratio` |x|> `0.4021` → IC=+0.164 (n=144)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.82€ cuando `delta_ratio` |x|> 0.4021 (IC base=+0.139)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.144 (n=57)
...
- **PATRÓN** `hora_utc` < `14.0` → IC=+0.125 (n=62)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **FILTRO** `py_entrada` < `0.495` → IC=-0.147 (n=32)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.040 (n=398)
...
- **PATRÓN** `ibs_15` < `0.4645` → IC=+0.121 (n=391)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.60€ cuando `ibs_15` < 0.4645 (IC base=+0.081)

- **PATRÓN** `dist_vwap_pct` > `0.4927` → IC=+0.139 (n=70)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 0.4927 (IC base=+0.081)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0172` → IC=+0.130 (n=71)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.65€ cuando `pct_spot_vs_ref` |x|≤ 0.0172 (IC base=+0.085)

- **PATRÓN** `sigma_h` < `0.0019` → IC=+0.214 (n=47)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0019 (IC base=+0.085)

- **PATRÓN** `dist_vwap_pct` > `0.3059` → IC=+0.125 (n=46)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` > 0.3059 (IC base=+0.085)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.44` → IC=+0.242 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.44 (IC base=+0.085)

- **PATRÓN** `libro_liquidez` > `11029.6268` → IC=+0.135 (n=94)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 11029.6268 (IC base=+0.085)
...
- **PATRÓN** `drift_15min` |x|≤ `0.2771` → IC=+0.135 (n=50)

  - _Acción_: Kelly boost +0.67€ cuando `drift_15min` |x|≤ 0.2771 (IC base=+0.064)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.3058` → IC=+0.167 (n=31)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.83€ cuando `delta_ratio_macro` |x|> 0.3058 (IC base=+0.043)

- **PATRÓN** `hora_utc` < `4.0` → IC=+0.180 (n=23)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` < 4.0 (IC base=+0.043)

- **PATRÓN** `ibs_15` < `0.1` → IC=+0.281 (n=30)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.1 (IC base=+0.043)
...
- **PATRÓN** `sigma_h` > `0.0063` → IC=+0.149 (n=92)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0063 (IC base=+0.105)

- **PATRÓN** `drift_60min` |x|≤ `0.1583` → IC=+0.198 (n=61)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.1583 (IC base=+0.105)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0979` → IC=+0.156 (n=123)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.78€ cuando `delta_ratio_macro` |x|> 0.0979 (IC base=+0.105)

- **PATRÓN** `hora_utc` < `20.0` → IC=+0.151 (n=144)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` < 20.0 (IC base=+0.105)

- **PATRÓN** `ibs_15` < `0.2353` → IC=+0.181 (n=92)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.90€ cuando `ibs_15` < 0.2353 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` > `0.1294` → IC=+0.204 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1294 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.148` → IC=+0.144 (n=116)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 5.148 (IC base=+0.105)

- **PATRÓN** `libro_liquidez` > `2541.6579` → IC=+0.143 (n=138)

  - _Acción_: Kelly boost +0.71€ cuando `libro_liquidez` > 2541.6579 (IC base=+0.105)
...
- **FILTRO** `sigma_h` < `0.0042` → IC=-0.163 (n=292)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0042
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=876)

- **FILTRO** `sigma_ewma_delta_pct` > `12.832` → IC=-0.155 (n=230)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 12
 | 1 | 4 |
| ✅ ORDER_FLOW_5M#5min | 94 | +0.125 | +33.33€ | 0 | 0 |
...
| ✅ ORDER_FLOW_5M#ETH | 7 | +0.058 | +4.44€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#ETH#5min | 7 | +0.058 | +4.44€ | 0 | 0 |
...
| ✅ STREAK_FADE_5M | 366 | -0.025 | -20.72€ | 2 | 1 |
| ✅ STREAK_FADE_5M#5min | 366 | -0.025 | -20.72€ | 0 | 0 |
...
| ✅ STREAK_FADE_5M#XRP | 78 | -0.062 | -7.45€ | 0 | 0 |
| ✅ STREAK_FADE_5M#XRP#5min | 78 | -0.062 | -7.45€ | 2 | 0 |
...
| ✅ STREAK_MOM_5M | 621 | +0.020 | -0.21€ | 1 | 0 |
| ✅ STREAK_MOM_5M#5min | 621 | +0.020 | -0.21€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE | 199 | +0.012 | -1.93€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE#5min | 199 | +0.012 | -1.93€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH | 116 | -0.009 | -3.26€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH#5min | 116 | -0.009 | -3.26€ | 2 | 2 |
| ✅ STREAK_MOM_5M#SOL | 162 | +0.030 | +1.45€ | 0 | 0 |
| ✅ STREAK_MOM_5M#SOL#5min | 162 | +0.030 | +1.45€ | 0 | 0 |
| ✅ STREAK_MOM_5M#XRP | 144 | +0.041 | +3.53€ | 0 | 0 |
| ✅ STREAK_MOM_5M#XRP#5min | 144 | +0.041 | +3.53€ | 2 | 2 |
| ✅ STRUCT_NO_15M | 1896 | +0.017 | -1.76€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 1896 | +0.017 | -1.76€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 723 | +0.008 | -7.91€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 723 | +0.008 | -7.91€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 743 | +0.021 | +2.10€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 743 | +0.021 | +2.10€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 430 | +0.025 | +4.04€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 430 | +0.025 | +4.04€ | 2 | 0 |
| ✅ UPDOWN_GBM | 2340 | +0.026 | +166.29€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1015 | +0.090 | +191.48€ | 1 | 8 |
| ✅ UPDOWN_GBM#240min | 124 | +0.016 | -0.04€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 899 | -0.018 | -15.48€ | 4 | 0 |
...
| ✅ UPDOWN_GBM#ETH | 542 | +0.055 | +56.22€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 232 | +0.124 | +54.71€ | 1 | 13 |
| ✅ UPDOWN_GBM#ETH#240min | 38 | +0.050 | +1.01€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 154 | +0.006 | +0.61€ | 4 | 2 |
| ✅ UPDOWN_GBM#ETH#60min | 103 | +0.014 | +0.27€ | 0 | 2 |
| 🚫 UPDOWN_GBM#ETH#daily | 15 | -0.154 | -0.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 428 | -0.007 | +0.78€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 186 | +0.021 | +5.43€ | 1 | 2 |
| ✅ UPDOWN_GBM#SOL#240min | 30 | -0.031 | -2.06€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 134 | -0.007 | -1.58€ | 2 | 3 |
| ✅ UPDOWN_GBM#SOL#60min | 66 | -0.029 | -0.87€ | 0 | 0 |
| 🚫 UPDOWN_GBM#SOL#daily | 12 | -0.129 | -0.13€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 520 | +0.025 | +67.92€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 311 | +0.097 | +92.52€ | 0 | 15 |
| ✅ UPDOWN_GBM#XRP#240min | 17 | -0.112 | -2.90€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 192 | -0.077 | -21.70€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1720 | -0.051 | +214.32€ | 3 | 3 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1720 | -0.051 | +214.32€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 556 | -0.014 | +142.04€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 556 | -0.014 | +142.04€ | 1 | 1 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 535 | -0.040 | +70.99€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 535 | -0.040 | +70.99€ | 3 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.136) — sin ventaja clara. oversold(IBS<0.3): IC=-0.014 n=817 | neutral: IC=+0.009 n=787 | overbought(IBS>0.7): IC=+0.122 n=1044
  - _Datos_: n=2841 IC=+0.046 PNL=+261.84€
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=186 IC=+0.021 PNL=+5.43€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=40411 | tras_1loss IC=+0.060 n=28696 | tras_2loss IC=+0.018 n=12583/40 | gap=+0.084 (umbral 0.05)
...
**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  ...
  - _Estado_: n_overlaps=18, boost estimado=+0.018. Necesita 2 más y boost>0.05
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=+0.070 n=177 | contrario_BTC IC=-0.013 n=109/40 | gap=-0.083 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2115 IC=+0.018 PNL=+111.39€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2115 IC=+0.018 PNL=+111.39€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.090 > 0.08 con n=635 PNL=+105.25€
  - _Datos_: n=635 IC=+0.090 PNL=+105.25€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=118 IC=+0.025 PNL=+11.00€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=118 IC=+0.025 PNL=+11.00€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=500 IC=+0.096 PNL=+126.82€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=500 IC=+0.096 PNL=+126.82€
...
**🟡 H-CUSTOM-OF-EDGE-ALTO** — ORDER_FLOW_5M: edge alto (>0.20) rinde mejor que edge cerca del suelo
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.167 > 0.02 con n=115 PNL=+46.07€
  - _Datos_: n=115 IC=+0.167 PNL=+46.07€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=982 IC=+0.023 PNL=+30.97€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=982 IC=+0.023 PNL=+30.97€
...
**🟡 H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.105 > 0.1 con n=183 PNL=+58.50€
  - _Datos_: n=183 IC=+0.105 PNL=+58.50€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1434 IC=-0.148 PNL=+61.24€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1434 IC=-0.148 PNL=+61.24€
...
**⏳ H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: 61/289 ops en el filtro definido (IC actual=-0.341 PNL=-27.82€)
  - _Datos_: n=61 IC=-0.341 PNL=-27.82€
...
**〰️ H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: n=566 IC=+0.021 PNL=+142.45€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=566 IC=+0.021 PNL=+142.45€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.245 > 0.08 con n=668 PNL=-71.76€
  - _Datos_: n=668 IC=+0.245 PNL=-71.76€
...
**🔶 H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.221 n=41) pero sin cruzar ≥2 pares más — sin otros pares con datos
  - _Datos_: n=41 IC=+0.221 PNL=+26.46€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=1166 IC=+0.096 PNL=-265.31€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=1166 IC=+0.096 PNL=-265.31€
