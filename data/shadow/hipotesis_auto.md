# Hipótesis automáticas — 2026-08-19 19:18 UTC
_Generado por shadow_postmortem.py sobre 79308 resoluciones (PNL=+8226.88€)_
...
- **PATRÓN** `py_entrada` > `0.715` → IC=+0.244 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.715 (IC base=+0.084)

- **PATRÓN** `n_total_lado` > `74.0` → IC=+0.202 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `n_total_lado` > 74.0 (IC base=+0.084)

- **PATRÓN** `banda_hit_calibrado` > `0.6142` → IC=+0.184 (n=166)

  - _Acción_: Kelly boost +0.92€ cuando `banda_hit_calibrado` > 0.6142 (IC base=+0.084)
...
- **PATRÓN** `py_entrada` > `0.705` → IC=+0.246 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.705 (IC base=+0.095)

- **PATRÓN** `n_total_lado` > `53.0` → IC=+0.158 (n=74)

  - _Acción_: Kelly boost +0.79€ cuando `n_total_lado` > 53.0 (IC base=+0.095)

- **PATRÓN** `banda_hit_calibrado` > `0.6142` → IC=+0.194 (n=83)

  - _Acción_: Kelly boost +0.97€ cuando `banda_hit_calibrado` > 0.6142 (IC base=+0.095)
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `155.05` → IC=-0.256 (n=1063)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 155.05
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=3189)
...
- **FILTRO** `restante_s_al_confirmar` > `643.08` → IC=-0.258 (n=118)
...
- **FILTRO** `n_ballenas` < `4.0` → IC=-0.133 (n=246)

  - _Acción_: SKIP cuando `n_ballenas` < 4.0
  - _Potencial_: sin este filtro IC_bueno=-0.091 (n=807)
...
- **PATRÓN** `py_entrada` < `0.335` → IC=+0.276 (n=1585)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.209 (n=390)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.200 (n=485)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.132 (n=663)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` < 15.0 (IC base=+0.116)

- **PATRÓN** `py_entrada` > `0.7` → IC=+0.302 (n=261)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.7 (IC base=+0.116)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.183 (n=610)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.181 (n=2879)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.159 (n=487)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.75` → IC=+0.174 (n=740)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.157 (n=736)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.225 (n=649)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.203 (n=234)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.72` → IC=+0.196 (n=544)

  - _Acción_: Kelly boost +0.98€ cuando `py_entrada` < 0.72 (IC base=+0.180)

### FAVORITO_CONFIRMADO_15MIN_EXTREMO
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.438 (n=126)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `3363.9486` → IC=+0.423 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3363.9486 (IC base=+0.414)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.193 (n=1904)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.211 (n=361)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.76 (IC base=+0.150)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.231 (n=872)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.283 (n=417)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.74 (IC base=+0.225)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.242 (n=607)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.261 (n=558)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.73 (IC base=+0.239)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.221 (n=321)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.180)
...
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.218 (n=697)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.144)

- **PATRÓN** `restante_min` < `3.81` → IC=+0.165 (n=634)

  - _Acción_: Kelly boost +0.83€ cuando `restante_min` < 3.81 (IC base=+0.144)

- **PATRÓN** `restante_min` > `4.91` → IC=+0.204 (n=639)

  - _Acción_: Kelly boost +1.00€ cuando `restante_min` > 4.91 (IC base=+0.144)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.148 (n=1990)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `5.61` → IC=+0.204 (n=634)

  - _Acción_: Kelly boost +1.00€ cuando `lag_apertura_s` < 5.61 (IC base=+0.144)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.236 (n=351)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.157 (n=987)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `3.3` → IC=+0.203 (n=321)

  - _Acción_: Kelly boost +1.00€ cuando `lag_apertura_s` < 3.3 (IC base=+0.137)
...
- **PATRÓN** `dist_vwap_pct` > `0.4392` → IC=+0.292 (n=46)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4392 (IC base=+0.098)
...
- **PATRÓN** `volumen_spike_ratio` > `1.6507` → IC=+0.126 (n=703)
  ...
  - _Acción_: Kelly boost +0.63€ cuando `volumen_spike_ratio` > 1.6507 (IC base=+0.098)

- **PATRÓN** `ibs_20min` < `0.5556` → IC=+0.130 (n=2034)

  - _Acción_: Kelly boost +0.65€ cuando `ibs_20min` < 0.5556 (IC base=+0.069)

- **PATRÓN** `dist_vwap_pct` < `0.148` → IC=+0.143 (n=575)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.148 (IC base=+0.069)

- **PATRÓN** `volumen_regimen` < `1.295` → IC=+0.138 (n=520)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` < 1.295 (IC base=+0.069)

- **PATRÓN** `volumen_regimen` > `0.6252` → IC=+0.135 (n=519)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 0.6252 (IC base=+0.069)

- **PATRÓN** `volumen_pendiente_norm` > `0.3296` → IC=+0.279 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3296 (IC base=+0.069)

- **PATRÓN** `volumen_spike_ratio` < `1.7042` → IC=+0.250 (n=198)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7042 (IC base=+0.069)

- **PATRÓN** `volumen_spike_ratio` > `2.8367` → IC=+0.223 (n=204)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8367 (IC base=+0.069)

- **PATRÓN** `ballena_activa_n` < `151.0` → IC=+0.269 (n=154)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 151.0 (IC base=+0.069)
...
- **PATRÓN** `sigma_h` < `0.0034` → IC=+0.151 (n=259)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` < 0.0034 (IC base=+0.144)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1869` → IC=+0.178 (n=228)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.89€ cuando `drift_60min` |x|≤ 0.1869 (IC base=+0.144)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.179 (n=182)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 12.0 (IC base=+0.144)

- **PATRÓN** `ibs_20min` < `0.4773` → IC=+0.201 (n=259)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4773 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` < `0.1429` → IC=+0.166 (n=276)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.1429 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.3` → IC=+0.242 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.3 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` < `1.2726` → IC=+0.159 (n=259)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2726 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` > `0.6746` → IC=+0.157 (n=231)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 0.6746 (IC base=+0.144)

- **PATRÓN** `volumen_pendiente_norm` < `0.1879` → IC=+0.185 (n=157)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` < 0.1879 (IC base=+0.144)

- **PATRÓN** `volumen_pendiente_norm` > `0.0934` → IC=+0.231 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0934 (IC base=+0.144)

- **PATRÓN** `volumen_spike_ratio` < `1.6134` → IC=+0.257 (n=68)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6134 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `13029.6077` → IC=+0.148 (n=86)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 13029.6077 (IC base=+0.144)

- **PATRÓN** `ballena_activa_n` < `231.0` → IC=+0.210 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 231.0 (IC base=+0.144)
...
- **PATRÓN** `ibs_20min` < `0.1014` → IC=+0.167 (n=901)

  - _Acción_: Kelly boost +0.83€ cuando `ibs_20min` < 0.1014 (IC base=+0.046)

- **PATRÓN** `dist_vwap_pct` > `0.1015` → IC=+0.201 (n=155)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1015 (IC base=+0.046)

- **PATRÓN** `dist_vwap_pct` < `0.2416` → IC=+0.187 (n=534)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` < 0.2416 (IC base=+0.046)

- **PATRÓN** `volumen_regimen` < `0.715` → IC=+0.190 (n=214)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_regimen` < 0.715 (IC base=+0.046)

- **PATRÓN** `volumen_regimen` > `1.064` → IC=+0.200 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.064 (IC base=+0.046)

- **PATRÓN** `volumen_pendiente_norm` > `0.2082` → IC=+0.321 (n=104)
...
- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.261 (n=136)
...
- **PATRÓN** `volumen_regimen` < `0.5591` → IC=+0.182 (n=20)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.5591 (IC base=-0.016)
...
- **PATRÓN** `sigma_h` > `0.0051` → IC=+0.270 (n=306)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `14.0` → IC=+0.308 (n=175)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1604` → IC=+0.271 (n=107)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1604 (IC base=+0.266)

- **PATRÓN** `dist_vwap_pct` < `0.2733` → IC=+0.269 (n=370)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2733 (IC base=+0.266)

- **PATRÓN** `volumen_regimen` > `1.0789` → IC=+0.304 (n=156)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0789 (IC base=+0.266)
...
- **PATRÓN** `volumen_spike_ratio` > `3.3154` → IC=+0.333 (n=64)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.3154 (IC base=+0.266)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.204 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 49.0 (IC base=+0.266)
...
- **PATRÓN** `sigma_h` > `0.0067` → IC=+0.210 (n=636)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0067 (IC base=+0.148)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.157 (n=470)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9167` → IC=+0.262 (n=936)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9167 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` > `0.5013` → IC=+0.221 (n=177)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5013 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.97` → IC=+0.270 (n=681)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.97 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` < `1.2173` → IC=+0.151 (n=754)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.2173 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` > `0.6287` → IC=+0.156 (n=753)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 0.6287 (IC base=+0.148)

- **PATRÓN** `volumen_pendiente_norm` > `0.1112` → IC=+0.173 (n=484)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.1112 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` < `2.6298` → IC=+0.130 (n=1025)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` < 2.6298 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` > `1.5368` → IC=+0.128 (n=1165)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.5368 (IC base=+0.148)

- **PATRÓN** `libro_spread` < `0.05` → IC=+0.157 (n=1427)

  - _Acción_: Kelly boost +0.79€ cuando `libro_spread` < 0.05 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `2507.1752` → IC=+0.186 (n=636)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 2507.1752 (IC base=+0.148)
...
- **PATRÓN** `sigma_h` > `0.003` → IC=+0.265 (n=79)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.003 (IC base=+0.261)

- **PATRÓN** `drift_60min` |x|≤ `0.1718` → IC=+0.315 (n=79)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1718 (IC base=+0.261)

- **PATRÓN** `hora_utc` > `9.0` → IC=+0.280 (n=107)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.5988` → IC=+0.317 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5988 (IC base=+0.261)

- **PATRÓN** `dist_vwap_pct` > `0.1224` → IC=+0.266 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1224 (IC base=+0.261)

- **PATRÓN** `dist_vwap_pct` < `0.3995` → IC=+0.274 (n=113)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3995 (IC base=+0.261)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.307` → IC=+0.431 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.307 (IC base=+0.261)

- **PATRÓN** `volumen_regimen` > `0.6447` → IC=+0.292 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6447 (IC base=+0.261)

- **PATRÓN** `volumen_pendiente_norm` > `0.1125` → IC=+0.320 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1125 (IC base=+0.261)

- **PATRÓN** `volumen_spike_ratio` > `2.0665` → IC=+0.289 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.0665 (IC base=+0.261)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.272 (n=125)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.261)

- **PATRÓN** `libro_liquidez` > `10544.7398` → IC=+0.309 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 10544.7398 (IC base=+0.261)

- **PATRÓN** `ballena_activa_n` < `149.0` → IC=+0.316 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 149.0 (IC base=+0.261)
...
- **PATRÓN** `sigma_h` > `0.0067` → IC=+0.175 (n=81)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0067 (IC base=+0.057)

- **PATRÓN** `ibs_20min` > `0.8667` → IC=+0.195 (n=162)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` > 0.8667 (IC base=+0.057)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.386` → IC=+0.210 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.386 (IC base=+0.057)
...
- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.216 (n=551)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0069 (IC base=+0.127)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.153 (n=828)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 15.0 (IC base=+0.127)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.142 (n=602)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` < 6.0 (IC base=+0.127)

- **PATRÓN** `ibs_20min` > `0.9655` → IC=+0.290 (n=747)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9655 (IC base=+0.127)

- **PATRÓN** `dist_vwap_pct` > `0.48` → IC=+0.235 (n=164)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.48 (IC base=+0.127)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.284` → IC=+0.234 (n=1097)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.284 (IC base=+0.127)

- **PATRÓN** `volumen_regimen` < `0.7058` → IC=+0.123 (n=407)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_regimen` < 0.7058 (IC base=+0.127)

- **PATRÓN** `volumen_regimen` > `1.0803` → IC=+0.125 (n=419)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_regimen` > 1.0803 (IC base=+0.127)

- **PATRÓN** `volumen_pendiente_norm` > `0.1831` → IC=+0.154 (n=394)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_pendiente_norm` > 0.1831 (IC base=+0.127)

- **PATRÓN** `volumen_spike_ratio` > `1.6776` → IC=+0.131 (n=1195)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` > 1.6776 (IC base=+0.127)

- **PATRÓN** `libro_spread` < `0.05` → IC=+0.134 (n=1657)

  - _Acción_: Kelly boost +0.67€ cuando `libro_spread` < 0.05 (IC base=+0.127)

- **PATRÓN** `libro_liquidez` > `2876.6309` → IC=+0.168 (n=549)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2876.6309 (IC base=+0.127)

- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.231 (n=1335)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0064 (IC base=+0.211)

- **PATRÓN** `sigma_h` > `0.0039` → IC=+0.214 (n=1517)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0039 (IC base=+0.211)

- **PATRÓN** `drift_60min` |x|≤ `0.2935` → IC=+0.213 (n=1517)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2935 (IC base=+0.211)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.244 (n=546)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.211)

- **PATRÓN** `ibs_20min` < `0.519` → IC=+0.278 (n=1517)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.519 (IC base=+0.211)

- **PATRÓN** `dist_vwap_pct` < `0.1577` → IC=+0.194 (n=972)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` < 0.1577 (IC base=+0.211)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.978` → IC=+0.242 (n=296)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.978 (IC base=+0.211)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.485` → IC=+0.220 (n=1424)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.485 (IC base=+0.211)

- **PATRÓN** `volumen_regimen` < `0.6112` → IC=+0.204 (n=346)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6112 (IC base=+0.211)

- **PATRÓN** `volumen_regimen` > `1.2256` → IC=+0.207 (n=346)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2256 (IC base=+0.211)

- **PATRÓN** `volumen_pendiente_norm` < `0.114` → IC=+0.221 (n=747)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.114 (IC base=+0.211)

- **PATRÓN** `volumen_pendiente_norm` > `0.2486` → IC=+0.247 (n=192)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2486 (IC base=+0.211)

- **PATRÓN** `volumen_spike_ratio` < `1.6449` → IC=+0.243 (n=364)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6449 (IC base=+0.211)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1874` → IC=+0.192 (n=222)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.96€ cuando `drift_60min` |x|≤ 0.1874 (IC base=+0.175)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.186 (n=253)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 6.0 (IC base=+0.175)

- **PATRÓN** `ibs_20min` < `0.4222` → IC=+0.226 (n=253)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4222 (IC base=+0.175)

- **PATRÓN** `dist_vwap_pct` < `0.1458` → IC=+0.192 (n=271)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` < 0.1458 (IC base=+0.175)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.388` → IC=+0.245 (n=155)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.388 (IC base=+0.175)

- **PATRÓN** `volumen_regimen` < `0.6097` → IC=+0.224 (n=85)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6097 (IC base=+0.175)

- **PATRÓN** `volumen_regimen` > `0.8413` → IC=+0.194 (n=168)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 0.8413 (IC base=+0.175)

- **PATRÓN** `volumen_pendiente_norm` > `0.1112` → IC=+0.306 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1112 (IC base=+0.175)

- **PATRÓN** `volumen_spike_ratio` < `1.5708` → IC=+0.319 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5708 (IC base=+0.175)

- **PATRÓN** `libro_liquidez` > `5258.5018` → IC=+0.184 (n=226)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 5258.5018 (IC base=+0.175)
...
- **PATRÓN** `sigma_h` < `0.0019` → IC=+0.167 (n=61)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0019 (IC base=+0.134)

- **PATRÓN** `sigma_h` > `0.0044` → IC=+0.172 (n=62)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0044 (IC base=+0.134)

- **PATRÓN** `drift_60min` |x|≤ `0.0555` → IC=+0.167 (n=61)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.0555 (IC base=+0.134)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.191 (n=66)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 17.0 (IC base=+0.134)

- **PATRÓN** `ibs_20min` > `0.3792` → IC=+0.243 (n=181)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3792 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` > `0.4492` → IC=+0.275 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4492 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.602` → IC=+0.235 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.602 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` < `0.7651` → IC=+0.195 (n=80)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` < 0.7651 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` > `1.1314` → IC=+0.179 (n=82)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` > 1.1314 (IC base=+0.134)
...
- **PATRÓN** `volumen_spike_ratio` > `1.9425` → IC=+0.265 (n=100)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9425 (IC base=+0.134)

- **PATRÓN** `libro_liquidez` > `7829.088` → IC=+0.298 (n=82)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7829.088 (IC base=+0.134)

- **PATRÓN** `ballena_activa_n` < `190.0` → IC=+0.296 (n=47)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 190.0 (IC base=+0.134)
...
- **FILTRO** `ibs_20min` > `0.619` → IC=-0.163 (n=84)

  - _Acción_: SKIP cuando `ibs_20min` > 0.619
  - _Potencial_: sin este filtro IC_bueno=+0.208 (n=255)
...
- **PATRÓN** `sigma_h` < `0.0071` → IC=+0.158 (n=255)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0071 (IC base=+0.116)

- **PATRÓN** `drift_60min` |x|≤ `0.1846` → IC=+0.151 (n=170)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.1846 (IC base=+0.116)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.185 (n=122)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 15.0 (IC base=+0.116)

- **PATRÓN** `ibs_20min` < `0.2353` → IC=+0.308 (n=170)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2353 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` < `0.4779` → IC=+0.142 (n=255)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.4779 (IC base=+0.116)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.824` → IC=+0.136 (n=234)
  ...
  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` < 2.824 (IC base=+0.116)

- **PATRÓN** `volumen_regimen` < `0.699` → IC=+0.175 (n=112)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 0.699 (IC base=+0.116)

- **PATRÓN** `volumen_regimen` > `1.0749` → IC=+0.152 (n=116)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.0749 (IC base=+0.116)

- **PATRÓN** `volumen_pendiente_norm` < `0.1693` → IC=+0.223 (n=110)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1693 (IC base=+0.116)

- **PATRÓN** `volumen_spike_ratio` < `1.9031` → IC=+0.193 (n=73)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 1.9031 (IC base=+0.116)

- **PATRÓN** `volumen_spike_ratio` > `1.6476` → IC=+0.220 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.6476 (IC base=+0.116)

- **PATRÓN** `libro_liquidez` > `1438.3006` → IC=+0.178 (n=116)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 1438.3006 (IC base=+0.116)
...
- **PATRÓN** `hora_utc` > `14.0` → IC=+0.288 (n=196)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 14.0 (IC base=+0.232)

- **PATRÓN** `ibs_20min` < `0.4346` → IC=+0.294 (n=391)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4346 (IC base=+0.232)

- **PATRÓN** `dist_vwap_pct` > `0.3856` → IC=+0.266 (n=62)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.782` → IC=+0.240 (n=405)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.782 (IC base=+0.232)

- **PATRÓN** `volumen_regimen` > `0.8819` → IC=+0.263 (n=260)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8819 (IC base=+0.232)

- **PATRÓN** `volumen_pendiente_norm` > `0.2659` → IC=+0.315 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2659 (IC base=+0.232)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.166 (n=282)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.199 (n=197)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0039 (IC base=+0.145)

- **PATRÓN** `drift_60min` |x|≤ `0.2194` → IC=+0.183 (n=197)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.2194 (IC base=+0.145)
...
- **PATRÓN** `ibs_20min` < `0.6378` → IC=+0.167 (n=223)

  - _Acción_: Kelly boost +0.83€ cuando `ibs_20min` < 0.6378 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.4828` → IC=+0.146 (n=46)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.4828 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` < `0.3536` → IC=+0.156 (n=239)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.3536 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.477` → IC=+0.154 (n=212)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 5.477 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` < `1.1717` → IC=+0.183 (n=197)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 1.1717 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` < `0.0968` → IC=+0.160 (n=186)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` < 0.0968 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` < `1.9176` → IC=+0.220 (n=148)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9176 (IC base=+0.145)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.166 (n=282)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.01 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `7478.3992` → IC=+0.198 (n=223)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 7478.3992 (IC base=+0.145)
...
- **PATRÓN** `ibs_20min` < `0.9851` → IC=+0.156 (n=59)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` < 0.9851 (IC base=+0.063)
...
- **PATRÓN** `hora_utc` < `2.0` → IC=+0.188 (n=30)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` < 2.0 (IC base=+0.060)
...
- **FILTRO** `ibs_20min` < `0.7265` → IC=-0.165 (n=165)
...
- **FILTRO** `py_entrada` > `0.69` → IC=-0.231 (n=24)
...
- **PATRÓN** `py_entrada` < `0.65` → IC=+0.133 (n=77)

  - _Acción_: Kelly boost +0.66€ cuando `py_entrada` < 0.65 (IC base=+0.042)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.263 (n=36)
...
- **FILTRO** `ibs_20min` > `0.7895` → IC=-0.125 (n=38)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.160 (n=565)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=1215)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.271 (n=439)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=1341)

- **FILTRO** `ibs_7min` < `0.7308` → IC=-0.214 (n=445)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7308
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=1335)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.182 (n=602)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=1178)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.193 (n=535)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=+0.013 (n=1650)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` > `17.0` → IC=-0.125 (n=86)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 17.0
  - _Potencial_: sin este filtro IC_bueno=-0.116 (n=188)

- **FILTRO** `py_entrada` < `0.41` → IC=-0.210 (n=136)

  - _Acción_: SKIP cuando `py_entrada` < 0.41
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=138)

- **FILTRO** `ibs_7min` < `0.9265` → IC=-0.176 (n=180)

  - _Acción_: SKIP cuando `ibs_7min` < 0.9265
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=94)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.185 (n=87)
...
- **FILTRO** `ibs_7min` < `0.7041` → IC=-0.222 (n=77)
...
- **FILTRO** `py_entrada` < `0.42` → IC=-0.226 (n=100)
...
- **FILTRO** `ballena_activa_n` > `28.0` → IC=-0.183 (n=80)
...
- **FILTRO** `py_entrada` < `0.41` → IC=-0.262 (n=82)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.41
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=247)

- **FILTRO** `ibs_7min` < `1.0` → IC=-0.156 (n=155)
...
- **FILTRO** `ibs_7min` > `0.8039` → IC=-0.170 (n=92)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.009 (n=745)
...
- **PATRÓN** `ibs_15` < `0.2895` → IC=+0.122 (n=376)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.61€ cuando `ibs_15` < 0.2895 (IC base=+0.063)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0962` → IC=+0.137 (n=155)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.68€ cuando `delta_ratio_macro` |x|> 0.0962 (IC base=+0.060)

- **PATRÓN** `ibs_15` < `0.1333` → IC=+0.167 (n=76)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.83€ cuando `ibs_15` < 0.1333 (IC base=+0.060)
...
- **PATRÓN** `sigma_h` > `0.0032` → IC=+0.291 (n=41)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0032 (IC base=+0.267)

- **PATRÓN** `drift_60min` |x|≤ `0.1567` → IC=+0.287 (n=78)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1567 (IC base=+0.267)

- **PATRÓN** `drift_15min` |x|≤ `0.4089` → IC=+0.281 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.4089 (IC base=+0.267)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0706` → IC=+0.272 (n=90)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0706 (IC base=+0.267)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.344 (n=30)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.267)

- **PATRÓN** `ibs_15` > `0.7117` → IC=+0.324 (n=89)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7117 (IC base=+0.267)

- **PATRÓN** `dist_vwap_pct` > `0.3665` → IC=+0.382 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3665 (IC base=+0.267)

- **PATRÓN** `dist_vwap_pct` < `0.0842` → IC=+0.276 (n=47)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0842 (IC base=+0.267)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.414` → IC=+0.300 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.414 (IC base=+0.267)

- **PATRÓN** `libro_liquidez` > `3208.0074` → IC=+0.281 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3208.0074 (IC base=+0.267)
...
- **PATRÓN** `sigma_h` > `0.0035` → IC=+0.333 (n=22)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0035 (IC base=+0.322)

- **PATRÓN** `drift_60min` |x|≤ `0.1163` → IC=+0.375 (n=22)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1163 (IC base=+0.322)

- **PATRÓN** `delta_ratio_macro` |x|> `0.13` → IC=+0.417 (n=22)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.13 (IC base=+0.322)
...
- **PATRÓN** `ibs_15` > `0.854` → IC=+0.417 (n=22)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.854 (IC base=+0.322)

- **PATRÓN** `dist_vwap_pct` < `0.2769` → IC=+0.367 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2769 (IC base=+0.322)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.588` → IC=+0.395 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.588 (IC base=+0.322)

- **PATRÓN** `libro_liquidez` > `2812.5928` → IC=+0.333 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2812.5928 (IC base=+0.322)
...
- **FILTRO** `sigma_ewma_delta_pct` > `12.785` → IC=-0.173 (n=258)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=1059)
...
- **PATRÓN** `ibs_15` < `0.531` → IC=+0.284 (n=49)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.531 (IC base=-0.066)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.244 (n=88)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.189 (n=194)

- **FILTRO** `hora_utc` > `16.0` → IC=-0.233 (n=58)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.199 (n=224)

- **FILTRO** `libro_liquidez` < `13827.5224` → IC=-0.223 (n=211)

  - _Acción_: SKIP cuando `libro_liquidez` < 13827.5224
...
### UPDOWN_GBM_IBS_ALTO
- **PATRÓN** `sigma_h` > `0.0027` → IC=+0.302 (n=84)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0027 (IC base=+0.269)

- **PATRÓN** `drift_60min` |x|≤ `0.1815` → IC=+0.281 (n=126)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1815 (IC base=+0.269)

- **PATRÓN** `drift_15min` |x|≤ `0.5909` → IC=+0.279 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.5909 (IC base=+0.269)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1879` → IC=+0.331 (n=57)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1879 (IC base=+0.269)

- **PATRÓN** `hora_utc` > `3.0` → IC=+0.301 (n=134)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 3.0 (IC base=+0.269)

- **PATRÓN** `ibs_15` > `0.9375` → IC=+0.360 (n=84)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.9375 (IC base=+0.269)

- **PATRÓN** `dist_vwap_pct` > `0.3763` → IC=+0.341 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3763 (IC base=+0.269)

- **PATRÓN** `dist_vwap_pct` < `0.0769` → IC=+0.319 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0769 (IC base=+0.269)

- **PATRÓN** `sigma_ewma_delta_pct` < `20.727` → IC=+0.281 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 20.727 (IC base=+0.269)

- **PATRÓN** `libro_liquidez` > `8878.5012` → IC=+0.280 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8878.5012 (IC base=+0.269)

- **PATRÓN** `ballena_activa_n` < `304.0` → IC=+0.324 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 304.0 (IC base=+0.269)
...
- **PATRÓN** `sigma_h` > `0.0028` → IC=+0.330 (n=45)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0028 (IC base=+0.297)

- **PATRÓN** `drift_15min` |x|≤ `0.3977` → IC=+0.289 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.3977 (IC base=+0.297)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1843` → IC=+0.420 (n=23)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1843 (IC base=+0.297)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.330 (n=45)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.297)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.311 (n=51)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.297)

- **PATRÓN** `ibs_15` > `0.9416` → IC=+0.389 (n=34)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.9416 (IC base=+0.297)

- **PATRÓN** `dist_vwap_pct` > `0.2769` → IC=+0.321 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2769 (IC base=+0.297)

- **PATRÓN** `dist_vwap_pct` < `0.0758` → IC=+0.362 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0758 (IC base=+0.297)

- **PATRÓN** `sigma_ewma_delta_pct` < `19.88` → IC=+0.333 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 19.88 (IC base=+0.297)

- **PATRÓN** `libro_liquidez` > `10832.5702` → IC=+0.342 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 10832.5702 (IC base=+0.297)
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1333 sube el IC de +0.060 a +0.167 en UPDOWN_GBM#XRP#15min (n=76). Ya aplicado como kelly_boost=+0.83€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.531 sube el IC de -0.066 a +0.284 en UPDOWN_GBM_15M_TARDIO (n=49). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.9375 sube el IC de +0.269 a +0.360 en UPDOWN_GBM_IBS_ALTO (n=84). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO#ETH#15min**: dentro de BUY_YES, IBS > 0.9416 sube el IC de +0.297 a +0.389 en UPDOWN_GBM_IBS_ALTO#ETH#15min (n=34). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD**: dentro de BUY_YES, IBS > 0.7117 sube el IC de +0.267 a +0.324 en UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD (n=89). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min**: dentro de BUY_YES, IBS > 0.854 sube el IC de +0.322 a +0.417 en UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min (n=22). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 497 | +0.047 | +39.08€ | 3 | 4 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 497 | +0.047 | +39.08€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 257 | +0.048 | +27.79€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 257 | +0.048 | +27.79€ | 2 | 4 |
...
| ✅ BALLENAS_TARDIAS | 3790 | -0.113 | -571.75€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 476 | -0.021 | -19.20€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#BTC | 476 | -0.021 | -19.20€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 476 | -0.021 | -19.20€ | 1 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 16431 | +0.113 | -1080.54€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3663 | +0.182 | -127.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 104 | -0.094 | -47.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 9994 | +0.084 | -917.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2670 | +0.130 | +12.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 1688 | +0.030 | -379.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 14 | -0.044 | +0.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 1670 | +0.032 | -373.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 3650 | +0.137 | -42.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1002 | +0.193 | -50.33€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 1659 | +0.107 | -33.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 947 | +0.139 | +64.05€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 1688 | +0.057 | -292.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 10 | +0.000 | -3.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 1677 | +0.058 | -287.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 3950 | +0.128 | -43.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1353 | +0.166 | -12.24€ | 0 | 6 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 1659 | +0.106 | -23.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 926 | +0.115 | +1.26€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 3773 | +0.132 | -251.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1272 | +0.196 | -63.41€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 44 | +0.022 | -7.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 1660 | +0.082 | -127.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 797 | +0.137 | -52.92€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 1682 | +0.118 | -71.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 12 | +0.043 | +1.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 1669 | +0.119 | -72.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 3747 | +0.163 | -350.32€ | 2 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 3747 | +0.163 | -350.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 943 | +0.154 | -125.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 943 | +0.154 | -125.04€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 933 | +0.156 | -122.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 933 | +0.156 | -122.13€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 829 | +0.223 | -34.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 829 | +0.223 | -34.14€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 2 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 883 | +0.180 | -78.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 883 | +0.180 | -78.45€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_EXTREMO | 185 | +0.414 | -10.12€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_EXTREMO#15min | 185 | +0.414 | -10.12€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 7156 | +0.184 | -724.81€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 7156 | +0.184 | -724.81€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 1340 | +0.092 | -311.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 1340 | +0.092 | -311.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 1092 | +0.242 | -18.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 1092 | +0.242 | -18.77€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 1261 | +0.150 | -190.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 1261 | +0.150 | -190.29€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 1147 | +0.225 | -44.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 1147 | +0.225 | -44.96€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 1112 | +0.239 | -25.28€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 1112 | +0.239 | -25.28€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 1204 | +0.180 | -133.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 