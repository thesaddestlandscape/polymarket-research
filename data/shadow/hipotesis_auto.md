# Hipótesis automáticas — 2026-09-25 14:17 UTC
_Generado por shadow_postmortem.py sobre 605675 resoluciones (PNL=+68496.98€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.26` → IC=-0.229 (n=7158)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.26
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=21474)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `137.87` → IC=-0.248 (n=940)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 137.87
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=2822)
...
- **FILTRO** `restante_s_al_confirmar` < `165.6` → IC=-0.215 (n=1733)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 165.6
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=5199)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `127.48` → IC=-0.351 (n=1410)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 127.48
  - _Potencial_: sin este filtro IC_bueno=-0.114 (n=4230)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.204 (n=14002)
...
- **PATRÓN** `libro_liquidez` > `5649.2316` → IC=+0.178 (n=2248)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.155 (n=2846)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 5.0 (IC base=+0.145)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.146 (n=2432)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `1311.0387` → IC=+0.151 (n=668)
...
- **PATRÓN** `py_entrada` < `0.335` → IC=+0.209 (n=304)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.167 (n=2817)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 5.0 (IC base=+0.166)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.172 (n=2682)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` < 17.0 (IC base=+0.166)

- **PATRÓN** `py_entrada` < `0.73` → IC=+0.176 (n=2672)

  - _Acción_: Kelly boost +0.88€ cuando `py_entrada` < 0.73 (IC base=+0.166)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.245 (n=876)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.241)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.244 (n=877)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.267 (n=2135)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.75 (IC base=+0.221)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.209 (n=5737)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.204)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.256 (n=2304)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.194 (n=5805)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `lag_apertura_s` < `2.76` → IC=+0.140 (n=4864)

  - _Acción_: Kelly boost +0.70€ cuando `lag_apertura_s` < 2.76 (IC base=+0.117)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.197 (n=2653)

  - _Acción_: Kelly boost +0.99€ cuando `py_entrada` < 0.38 (IC base=+0.122)

- **PATRÓN** `restante_min` < `4.12` → IC=+0.129 (n=2437)

  - _Acción_: Kelly boost +0.64€ cuando `restante_min` < 4.12 (IC base=+0.122)

- **PATRÓN** `restante_min` > `4.94` → IC=+0.140 (n=2642)
...
- **PATRÓN** `libro_spread` < `0.03` → IC=+0.268 (n=562)
...
### GBM_LATE_15M
- **PATRÓN** `drift_60min` |x|≤ `0.4862` → IC=+0.122 (n=8206)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.61€ cuando `drift_60min` |x|≤ 0.4862 (IC base=+0.106)

- **PATRÓN** `ibs_20min` > `0.9829` → IC=+0.244 (n=2736)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4628` → IC=+0.203 (n=5639)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4628 (IC base=+0.106)

- **PATRÓN** `ibs_20min` < `0.5714` → IC=+0.134 (n=9909)

  - _Acción_: Kelly boost +0.67€ cuando `ibs_20min` < 0.5714 (IC base=+0.066)

- **PATRÓN** `dist_vwap_pct` > `0.6159` → IC=+0.202 (n=737)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6159 (IC base=+0.066)

- **PATRÓN** `volumen_regimen` < `0.6983` → IC=+0.184 (n=1528)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 0.6983 (IC base=+0.066)

- **PATRÓN** `volumen_regimen` > `1.0529` → IC=+0.177 (n=1574)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` > 1.0529 (IC base=+0.066)

- **PATRÓN** `volumen_pendiente_norm` > `0.167` → IC=+0.227 (n=1665)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.167 (IC base=+0.066)

- **PATRÓN** `volumen_spike_ratio` > `1.568` → IC=+0.201 (n=5232)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.568 (IC base=+0.066)

- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.211 (n=5647)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.272 (n=717)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.160)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.136` → IC=+0.266 (n=793)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.464` → IC=+0.245 (n=1456)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.464 (IC base=+0.235)

- **PATRÓN** `volumen_pendiente_norm` > `0.2774` → IC=+0.259 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2774 (IC base=+0.235)

- **PATRÓN** `volumen_spike_ratio` < `1.4285` → IC=+0.235 (n=428)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4285 (IC base=+0.235)

- **PATRÓN** `volumen_spike_ratio` > `2.6131` → IC=+0.239 (n=428)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6131 (IC base=+0.235)

- **PATRÓN** `libro_liquidez` > `1809.2686` → IC=+0.236 (n=936)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1809.2686 (IC base=+0.235)
...
- **PATRÓN** `dist_vwap_pct` < `0.7288` → IC=+0.220 (n=1573)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.7288 (IC base=+0.217)
...
- **PATRÓN** `volumen_regimen` < `1.2554` → IC=+0.219 (n=1417)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2554 (IC base=+0.217)

- **PATRÓN** `volumen_regimen` > `1.0803` → IC=+0.227 (n=643)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3766` → IC=+0.233 (n=463)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3766 (IC base=+0.217)

- **PATRÓN** `libro_liquidez` > `12537.0225` → IC=+0.221 (n=1266)
...
- **PATRÓN** `ibs_20min` < `0.3289` → IC=+0.198 (n=985)

  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` < 0.3289 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` < `0.1332` → IC=+0.157 (n=1314)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.282` → IC=+0.144 (n=1347)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 4.282 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` < `1.204` → IC=+0.152 (n=1477)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.204 (IC base=+0.141)

- **PATRÓN** `volumen_pendiente_norm` > `0.1561` → IC=+0.180 (n=392)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1561 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` < `2.4183` → IC=+0.153 (n=1366)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.4183 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` > `1.4185` → IC=+0.150 (n=1366)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.4185 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `14101.4554` → IC=+0.144 (n=984)

  - _Acción_: Kelly boost +0.72€ cuando `libro_liquidez` > 14101.4554 (IC base=+0.141)

- **PATRÓN** `ballena_activa_n` < `235.0` → IC=+0.164 (n=566)

  - _Acción_: Kelly boost +0.82€ cuando `ballena_activa_n` < 235.0 (IC base=+0.141)

### GBM_LATE_15M#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0117` → IC=+0.209 (n=609)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0117 (IC base=+0.185)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.191 (n=1918)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 5.0 (IC base=+0.185)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.190 (n=1641)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` < `0.21` → IC=+0.186 (n=1829)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` < 0.21 (IC base=+0.185)

- **PATRÓN** `volumen_pendiente_norm` > `0.3585` → IC=+0.202 (n=243)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3585 (IC base=+0.185)
...
- **PATRÓN** `ibs_20min` < `0.0643` → IC=+0.246 (n=691)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0643 (IC base=+0.211)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.615` → IC=+0.236 (n=537)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.615 (IC base=+0.211)

- **PATRÓN** `volumen_pendiente_norm` > `0.353` → IC=+0.273 (n=227)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.353 (IC base=+0.211)

- **PATRÓN** `volumen_spike_ratio` < `1.7621` → IC=+0.204 (n=634)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7621 (IC base=+0.211)

- **PATRÓN** `volumen_spike_ratio` > `2.1981` → IC=+0.217 (n=960)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1981 (IC base=+0.211)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.216 (n=1060)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.211)

- **PATRÓN** `libro_liquidez` > `1900.976` → IC=+0.218 (n=712)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1900.976 (IC base=+0.211)

- **PATRÓN** `ballena_activa_n` < `24.0` → IC=+0.216 (n=941)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.150 (n=101)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=2249)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.741` → IC=+0.159 (n=726)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 4.741 (IC base=+0.024)
...
- **PATRÓN** `volumen_regimen` < `0.8486` → IC=+0.165 (n=568)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.8486 (IC base=+0.019)

- **PATRÓN** `volumen_regimen` > `1.1597` → IC=+0.154 (n=284)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.1597 (IC base=+0.019)

- **PATRÓN** `volumen_pendiente_norm` > `0.2266` → IC=+0.219 (n=137)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2266 (IC base=+0.019)

- **PATRÓN** `volumen_spike_ratio` > `1.5204` → IC=+0.178 (n=713)
...
- **FILTRO** `ibs_20min` > `0.25` → IC=-0.125 (n=2222)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.692` → IC=-0.208 (n=354)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=2963)
...
- **FILTRO** `ibs_20min` > `0.77` → IC=-0.203 (n=847)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.037 (n=2547)
...
- **PATRÓN** `dist_vwap_pct` > `0.9121` → IC=+0.271 (n=164)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9121 (IC base=-0.023)

- **PATRÓN** `volumen_regimen` < `0.7364` → IC=+0.254 (n=343)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7364 (IC base=-0.023)

- **PATRÓN** `volumen_regimen` > `1.2645` → IC=+0.286 (n=260)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2645 (IC base=-0.023)

- **PATRÓN** `volumen_pendiente_norm` > `0.1027` → IC=+0.275 (n=274)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1027 (IC base=-0.023)

- **PATRÓN** `volumen_spike_ratio` < `2.141` → IC=+0.256 (n=589)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.141 (IC base=-0.023)

- **PATRÓN** `volumen_spike_ratio` > `1.5236` → IC=+0.248 (n=597)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5236 (IC base=-0.023)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0098` → IC=+0.197 (n=3443)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0098 (IC base=+0.097)

- **PATRÓN** `ibs_20min` > `0.4762` → IC=+0.188 (n=9220)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.4762 (IC base=+0.097)

- **PATRÓN** `dist_vwap_pct` > `0.7692` → IC=+0.285 (n=1100)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7692 (IC base=+0.097)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.61` → IC=+0.156 (n=4849)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 3.61 (IC base=+0.097)

- **PATRÓN** `volumen_regimen` > `0.6905` → IC=+0.249 (n=3288)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6905 (IC base=+0.097)

- **PATRÓN** `volumen_pendiente_norm` > `0.2945` → IC=+0.270 (n=873)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2945 (IC base=+0.097)

- **PATRÓN** `volumen_spike_ratio` < `1.4651` → IC=+0.237 (n=1991)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4651 (IC base=+0.097)

- **PATRÓN** `volumen_spike_ratio` > `2.6583` → IC=+0.244 (n=1991)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6583 (IC base=+0.097)

- **PATRÓN** `ballena_activa_n` < `97.0` → IC=+0.267 (n=5494)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 97.0 (IC base=+0.097)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.157 (n=3419)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0091 (IC base=+0.074)

- **PATRÓN** `ibs_20min` < `0.5488` → IC=+0.155 (n=9019)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` < 0.5488 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` > `0.7307` → IC=+0.243 (n=645)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7307 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` < `0.2525` → IC=+0.240 (n=2857)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2525 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` < `0.7096` → IC=+0.242 (n=1332)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7096 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` > `1.2058` → IC=+0.248 (n=1009)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2058 (IC base=+0.074)

- **PATRÓN** `volumen_pendiente_norm` > `0.2438` → IC=+0.304 (n=757)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2438 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` < `1.6005` → IC=+0.261 (n=1763)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6005 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` > `2.3041` → IC=+0.258 (n=1816)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3041 (IC base=+0.074)

- **PATRÓN** `ballena_activa_n` < `84.0` → IC=+0.265 (n=3883)
...
- **PATRÓN** `ibs_20min` > `0.8935` → IC=+0.266 (n=712)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8935 (IC base=+0.043)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.845` → IC=+0.206 (n=375)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4339` → IC=+0.173 (n=301)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.4339 (IC base=+0.043)

- **PATRÓN** `volumen_spike_ratio` > `2.1457` → IC=+0.198 (n=409)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.1457 (IC base=+0.043)

- **PATRÓN** `ballena_activa_n` < `14.0` → IC=+0.183 (n=405)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 14.0 (IC base=+0.043)
...
- **PATRÓN** `ibs_20min` > `0.8636` → IC=+0.159 (n=690)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` > 0.8636 (IC base=+0.025)

- **PATRÓN** `dist_vwap_pct` > `0.3135` → IC=+0.175 (n=380)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.3135 (IC base=+0.025)

- **PATRÓN** `volumen_regimen` > `0.6744` → IC=+0.169 (n=850)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4243` → IC=+0.193 (n=311)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 1.4243 (IC base=+0.025)

- **PATRÓN** `volumen_spike_ratio` > `2.3807` → IC=+0.154 (n=310)
...
- **PATRÓN** `dist_vwap_pct` < `0.1614` → IC=+0.222 (n=592)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1614 (IC base=+0.005)

- **PATRÓN** `volumen_regimen` > `0.8539` → IC=+0.228 (n=387)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.104` → IC=+0.258 (n=1380)
...
- **PATRÓN** `volumen_spike_ratio` > `3.4136` → IC=+0.263 (n=513)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.4136 (IC base=+0.246)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.254 (n=1144)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.246)

- **PATRÓN** `libro_liquidez` > `1802.6286` → IC=+0.248 (n=1083)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1802.6286 (IC base=+0.246)

- **PATRÓN** `sigma_h` > `0.0099` → IC=+0.318 (n=591)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0099 (IC base=+0.282)

- **PATRÓN** `drift_60min` |x|≤ `0.616` → IC=+0.284 (n=1300)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.616 (IC base=+0.282)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.330 (n=438)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.282)

- **PATRÓN** `ibs_20min` < `0.0909` → IC=+0.289 (n=873)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0909 (IC base=+0.282)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.808` → IC=+0.296 (n=513)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7408` → IC=+0.284 (n=530)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7408 (IC base=+0.282)

- **PATRÓN** `volumen_spike_ratio` > `2.7264` → IC=+0.288 (n=546)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7264 (IC base=+0.282)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.286 (n=871)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.282)

- **PATRÓN** `libro_liquidez` > `1888.9584` → IC=+0.297 (n=590)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1888.9584 (IC base=+0.282)

- **PATRÓN** `ballena_activa_n` < `28.0` → IC=+0.287 (n=776)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 28.0 (IC base=+0.282)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **FILTRO** `ibs_20min` < `0.294` → IC=-0.184 (n=505)

  - _Acción_: SKIP cuando `ibs_20min` < 0.294
  - _Potencial_: sin este filtro IC_bueno=+0.080 (n=1517)

- **FILTRO** `ibs_20min` > `0.7753` → IC=-0.190 (n=607)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7753
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=1824)

- **PATRÓN** `ibs_20min` > `0.8224` → IC=+0.161 (n=688)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.8224 (IC base=+0.014)

- **PATRÓN** `dist_vwap_pct` > `0.4514` → IC=+0.223 (n=258)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4514 (IC base=+0.014)

- **PATRÓN** `dist_vwap_pct` < `0.7002` → IC=+0.216 (n=597)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.7002 (IC base=+0.014)

- **PATRÓN** `volumen_regimen` < `0.9969` → IC=+0.236 (n=513)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9969 (IC base=+0.014)

- **PATRÓN** `volumen_regimen` > `0.5875` → IC=+0.214 (n=583)
...
- **FILTRO** `ibs_20min` < `0.7273` → IC=-0.193 (n=1093)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.278 (n=1095)
...
- **PATRÓN** `ibs_20min` > `0.7273` → IC=+0.278 (n=1095)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7273 (IC base=+0.043)

- **PATRÓN** `dist_vwap_pct` > `0.8548` → IC=+0.329 (n=255)
...
- **PATRÓN** `volumen_regimen` < `0.8684` → IC=+0.302 (n=539)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1022` → IC=+0.292 (n=752)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4185` → IC=+0.325 (n=261)
...
- **PATRÓN** `sigma_h` > `0.0168` → IC=+0.316 (n=893)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `libro_liquidez` > `2636.2554` → IC=+0.293 (n=893)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2636.2554 (IC base=+0.276)

- **PATRÓN** `sigma_h` > `0.0153` → IC=+0.301 (n=958)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.3925` → IC=+0.306 (n=1435)
...
- **PATRÓN** `dist_vwap_pct` < `0.2275` → IC=+0.274 (n=1293)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2275 (IC base=+0.273)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.471` → IC=+0.291 (n=525)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.471 (IC base=+0.273)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4255` → IC=+0.279 (n=423)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4255 (IC base=+0.273)

- **PATRÓN** `volumen_spike_ratio` > `2.1415` → IC=+0.273 (n=575)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1415 (IC base=+0.273)

- **PATRÓN** `libro_liquidez` > `2627.6354` → IC=+0.278 (n=957)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2627.6354 (IC base=+0.273)
...
- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.200 (n=2674)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0113 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0908` → IC=+0.188 (n=2673)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.0908 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.179 (n=8350)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 5.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` > `0.575` → IC=+0.218 (n=8021)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.575 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.1782` → IC=+0.194 (n=3506)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.1782 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.337` → IC=+0.254 (n=1639)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.337 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.2127` → IC=+0.159 (n=5310)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2127 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` > `0.6308` → IC=+0.161 (n=5310)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.6308 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.2953` → IC=+0.196 (n=1200)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.2953 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.5586` → IC=+0.167 (n=3384)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.5586 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.6169` → IC=+0.176 (n=2564)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.6169 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `2407.6479` → IC=+0.169 (n=5345)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2407.6479 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `114.0` → IC=+0.180 (n=6926)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0812` → IC=+0.208 (n=2549)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.48` → IC=+0.226 (n=7646)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.48 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.9212` → IC=+0.146 (n=747)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.9212 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` < `0.2353` → IC=+0.158 (n=5538)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.2353 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.347` → IC=+0.193 (n=1297)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 10.347 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.1737` → IC=+0.152 (n=5532)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1737 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.2909` → IC=+0.216 (n=1102)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2909 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.5586` → IC=+0.165 (n=3061)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.5586 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.251` → IC=+0.169 (n=3155)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.251 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `115.0` → IC=+0.173 (n=6605)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.243 (n=901)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3437` → IC=+0.264 (n=998)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3437 (IC base=+0.238)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.34` → IC=+0.250 (n=1078)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.34 (IC base=+0.238)

- **PATRÓN** `volumen_pendiente_norm` < `0.1599` → IC=+0.234 (n=942)
...
- **PATRÓN** `libro_spread` < `0.03` → IC=+0.238 (n=707)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.03 (IC base=+0.238)

- **PATRÓN** `libro_liquidez` > `1809.7` → IC=+0.244 (n=665)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0744` → IC=+0.195 (n=398)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.97€ cuando `drift_60min` |x|≤ 0.0744 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.185 (n=1257)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 5.0 (IC base=+0.161)

- **PATRÓN** `ibs_20min` > `0.4039` → IC=+0.228 (n=1192)
...
- **PATRÓN** `volumen_regimen` > `1.0742` → IC=+0.169 (n=541)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 1.0742 (IC base=+0.161)
...
- **PATRÓN** `libro_liquidez` > `11874.9267` → IC=+0.169 (n=1065)
...
- **PATRÓN** `ibs_20min` < `0.565` → IC=+0.187 (n=1296)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` < 0.565 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.1345` → IC=+0.161 (n=1283)
...
- **PATRÓN** `volumen_regimen` < `1.2096` → IC=+0.159 (n=1296)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.2096 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` > `0.1558` → IC=+0.156 (n=396)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_pendiente_norm` > 0.1558 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` < `2.4411` → IC=+0.146 (n=1184)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 2.4411 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `1.4211` → IC=+0.137 (n=1184)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 1.4211 (IC base=+0.137)
...
- **PATRÓN** `sigma_h` > `0.0101` → IC=+0.218 (n=612)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2071` → IC=+0.203 (n=398)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2071 (IC base=+0.199)

- **PATRÓN** `volumen_spike_ratio` < `1.6285` → IC=+0.198 (n=428)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 1.6285 (IC base=+0.199)

- **PATRÓN** `volumen_spike_ratio` > `2.8411` → IC=+0.215 (n=581)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8411 (IC base=+0.199)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.207 (n=949)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.199)

- **PATRÓN** `sigma_h` < `0.0115` → IC=+0.231 (n=1115)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0115 (IC base=+0.217)

- **PATRÓN** `drift_60min` |x|≤ `0.0967` → IC=+0.249 (n=372)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.3456` → IC=+0.247 (n=1115)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3456 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.735` → IC=+0.261 (n=480)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.735 (IC base=+0.217)

- **PATRÓN** `volumen_pendiente_norm` > `0.3547` → IC=+0.269 (n=184)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3547 (IC base=+0.217)

- **PATRÓN** `volumen_spike_ratio` < `1.766` → IC=+0.212 (n=456)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.766 (IC base=+0.217)

- **PATRÓN** `volumen_spike_ratio` > `2.2146` → IC=+0.228 (n=690)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2146 (IC base=+0.217)

- **PATRÓN** `ballena_activa_n` < `11.0` → IC=+0.205 (n=341)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4364` → IC=+0.157 (n=1281)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.4364 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=1343)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.142)

- **PATRÓN** `ibs_20min` > `0.3749` → IC=+0.198 (n=1281)

  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` > 0.3749 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.1614` → IC=+0.178 (n=853)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.1614 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.995` → IC=+0.228 (n=237)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.995 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `1.0403` → IC=+0.144 (n=1127)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.0403 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` > `0.6254` → IC=+0.148 (n=1281)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` > 0.6254 (IC base=+0.142)

- **PATRÓN** `volumen_pendiente_norm` > `0.2916` → IC=+0.198 (n=200)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.2916 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` < `1.4263` → IC=+0.152 (n=418)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 1.4263 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` > `2.5072` → IC=+0.169 (n=418)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.5072 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `6488.9388` → IC=+0.183 (n=854)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 6488.9388 (IC base=+0.142)

- **PATRÓN** `ballena_activa_n` < `162.0` → IC=+0.148 (n=1216)

  - _Acción_: Kelly boost +0.74€ cuando `ballena_activa_n` < 162.0 (IC base=+0.142)

- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.153 (n=1345)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0072 (IC base=+0.123)

- **PATRÓN** `drift_60min` |x|≤ `0.3848` → IC=+0.143 (n=1345)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.644` → IC=+0.172 (n=1345)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.644 (IC base=+0.123)

- **PATRÓN** `dist_vwap_pct` < `0.2146` → IC=+0.138 (n=1358)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.2146 (IC base=+0.123)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.961` → IC=+0.166 (n=474)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 6.961 (IC base=+0.123)

- **PATRÓN** `volumen_regimen` < `0.8476` → IC=+0.148 (n=897)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.8476 (IC base=+0.123)

- **PATRÓN** `volumen_pendiente_norm` > `0.2919` → IC=+0.187 (n=196)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.2919 (IC base=+0.123)

- **PATRÓN** `volumen_spike_ratio` < `1.7891` → IC=+0.138 (n=815)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` < 1.7891 (IC base=+0.123)

- **PATRÓN** `libro_liquidez` > `10007.3401` → IC=+0.159 (n=610)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 10007.3401 (IC base=+0.123)

### GBM_LATE_15M_MULTIHORIZONTE#SOL#15min
- **PATRÓN** `sigma_h` > `0.0101` → IC=+0.154 (n=659)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.5152` → IC=+0.205 (n=1448)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5152 (IC base=+0.118)

- **PATRÓN** `dist_vwap_pct` > `0.5187` → IC=+0.198 (n=624)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.5187 (IC base=+0.118)
...
- **PATRÓN** `volumen_regimen` < `1.2173` → IC=+0.130 (n=1448)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 1.2173 (IC base=+0.118)

- **PATRÓN** `volumen_regimen` > `0.6463` → IC=+0.123 (n=1448)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1034` → IC=+0.160 (n=489)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.1034 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.131 (n=1483)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 5.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` < `0.5652` → IC=+0.209 (n=1465)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5652 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `1.012` → IC=+0.144 (n=200)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 1.012 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.2019` → IC=+0.137 (n=1350)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.2019 (IC base=+0.113)
...
- **PATRÓN** `volumen_regimen` < `0.6365` → IC=+0.141 (n=489)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 0.6365 (IC base=+0.113)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4556` → IC=+0.130 (n=439)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` < 1.4556 (IC base=+0.113)

- **PATRÓN** `volumen_spike_ratio` > `2.4248` → IC=+0.130 (n=438)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` > 2.4248 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `2763.2904` → IC=+0.155 (n=665)
...
- **PATRÓN** `sigma_h` > `0.0173` → IC=+0.205 (n=952)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0173 (IC base=+0.203)

- **PATRÓN** `drift_60min` |x|≤ `0.0901` → IC=+0.218 (n=476)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `drift_60min` |x|≤ `0.3556` → IC=+0.150 (n=1704)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.5386` → IC=+0.188 (n=1730)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.5386 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.8726` → IC=+0.178 (n=312)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.8726 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.707` → IC=+0.174 (n=875)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 3.707 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `0.8753` → IC=+0.163 (n=1135)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.8753 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` > `0.7015` → IC=+0.151 (n=1521)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.7015 (IC base=+0.147)

- **PATRÓN** `volumen_pendiente_norm` > `0.1644` → IC=+0.176 (n=535)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.1644 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` < `1.4411` → IC=+0.162 (n=623)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 1.4411 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` > `1.8245` → IC=+0.151 (n=1245)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.8245 (IC base=+0.147)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.151 (n=2196)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.02 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `12428.4336` → IC=+0.159 (n=646)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 12428.4336 (IC base=+0.147)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.167 (n=1715)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4526` → IC=+0.142 (n=646)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.4526 (IC base=+0.111)

- **PATRÓN** `libro_liquidez` > `2753.0147` → IC=+0.124 (n=1795)
...
- **PATRÓN** `ibs_20min` < `0.084` → IC=+0.162 (n=190)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `ibs_20min` < 0.084 (IC base=+0.094)

- **PATRÓN** `volumen_regimen` < `0.6858` → IC=+0.131 (n=250)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 0.6858 (IC base=+0.094)

- **PATRÓN** `volumen_pendiente_norm` > `0.1663` → IC=+0.130 (n=133)
...
- **PATRÓN** `sigma_h` > `0.0073` → IC=+0.162 (n=294)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` > 0.0073 (IC base=+0.132)

- **PATRÓN** `drift_60min` |x|≤ `0.1179` → IC=+0.144 (n=147)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.1179 (IC base=+0.132)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.167 (n=412)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.9532` → IC=+0.235 (n=81)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9532 (IC base=+0.132)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.134` → IC=+0.204 (n=194)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.134 (IC base=+0.132)

- **PATRÓN** `volumen_regimen` < `1.0738` → IC=+0.149 (n=388)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 1.0738 (IC base=+0.132)

- **PATRÓN** `volumen_pendiente_norm` > `0.1842` → IC=+0.172 (n=126)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.1842 (IC base=+0.132)

- **PATRÓN** `volumen_spike_ratio` > `2.2078` → IC=+0.174 (n=191)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.2078 (IC base=+0.132)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.133 (n=480)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.205 (n=3421)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0113 (IC base=+0.169)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.179 (n=10704)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 5.0 (IC base=+0.169)

- **PATRÓN** `ibs_20min` > `0.4694` → IC=+0.216 (n=10259)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4694 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` > `0.9775` → IC=+0.202 (n=1459)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9775 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.861` → IC=+0.233 (n=3772)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.861 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` < `0.8814` → IC=+0.165 (n=4575)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.8814 (IC base=+0.169)

- **PATRÓN** `volumen_pendiente_norm` > `0.2391` → IC=+0.200 (n=1912)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2391 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `2.5948` → IC=+0.186 (n=3290)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 2.5948 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `2350.8` → IC=+0.172 (n=6839)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 2350.8 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `86.0` → IC=+0.194 (n=7831)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 86.0 (IC base=+0.169)

- **PATRÓN** `sigma_h` < `0.007` → IC=+0.192 (n=6195)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.007 (IC base=+0.182)

- **PATRÓN** `drift_60min` |x|≤ `0.149` → IC=+0.189 (n=4089)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5667` → IC=+0.237 (n=9289)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5667 (IC base=+0.182)

- **PATRÓN** `dist_vwap_pct` < `0.2512` → IC=+0.162 (n=5746)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2512 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.037` → IC=+0.200 (n=1303)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.037 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.732` → IC=+0.183 (n=8983)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.732 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` < `0.705` → IC=+0.163 (n=2804)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.705 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` > `1.2044` → IC=+0.153 (n=2124)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.2044 (IC base=+0.182)

- **PATRÓN** `volumen_pendiente_norm` > `0.2876` → IC=+0.240 (n=1227)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2876 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` > `2.621` → IC=+0.193 (n=2845)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 2.621 (IC base=+0.182)

- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.194 (n=5516)
...
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.224 (n=578)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0083 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.356` → IC=+0.190 (n=1734)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.326 (n=626)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.189)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.069` → IC=+0.311 (n=772)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.069 (IC base=+0.189)
...
- **PATRÓN** `libro_liquidez` > `1807.6` → IC=+0.264 (n=910)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1807.6 (IC base=+0.258)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0853` → IC=+0.159 (n=549)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.0853 (IC base=+0.149)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.164 (n=1719)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 5.0 (IC base=+0.149)

- **PATRÓN** `ibs_20min` > `0.3085` → IC=+0.203 (n=1644)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3085 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` > `0.3455` → IC=+0.192 (n=666)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.3455 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.73` → IC=+0.168 (n=375)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` > 9.73 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.234` → IC=+0.154 (n=1484)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2665` → IC=+0.200 (n=235)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2665 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` < `2.1148` → IC=+0.160 (n=1399)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.1148 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` > `1.403` → IC=+0.154 (n=1590)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.403 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `15748.4784` → IC=+0.159 (n=746)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 15748.4784 (IC base=+0.149)

- **PATRÓN** `ballena_activa_n` < `472.0` → IC=+0.158 (n=1522)
...
- **PATRÓN** `drift_60min` |x|≤ `0.263` → IC=+0.168 (n=1253)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.2708` → IC=+0.239 (n=949)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2104` → IC=+0.221 (n=1651)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2104 (IC base=+0.218)

- **PATRÓN** `volumen_spike_ratio` > `2.8244` → IC=+0.241 (n=720)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8244 (IC base=+0.218)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.228 (n=1179)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1684` → IC=+0.238 (n=682)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1684 (IC base=+0.231)
...
- **PATRÓN** `ibs_20min` < `0.3611` → IC=+0.266 (n=1364)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3611 (IC base=+0.231)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.748` → IC=+0.273 (n=575)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.748 (IC base=+0.231)

- **PATRÓN** `volumen_pendiente_norm` > `0.3437` → IC=+0.305 (n=229)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3437 (IC base=+0.231)

- **PATRÓN** `volumen_spike_ratio` < `1.7484` → IC=+0.228 (n=627)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7484 (IC base=+0.231)

- **PATRÓN** `volumen_spike_ratio` > `2.1845` → IC=+0.234 (n=950)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1845 (IC base=+0.231)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.239 (n=1047)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.231)

- **PATRÓN** `libro_liquidez` > `1896.6686` → IC=+0.239 (n=703)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1896.6686 (IC base=+0.231)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.229 (n=1355)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4388` → IC=+0.146 (n=1747)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.4388 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.152 (n=1824)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 5.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` > `0.8762` → IC=+0.261 (n=792)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8762 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.375` → IC=+0.163 (n=695)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.375 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.19` → IC=+0.158 (n=721)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 4.19 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `0.8743` → IC=+0.155 (n=1164)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5208` → IC=+0.149 (n=744)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.5208 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.7649` → IC=+0.150 (n=1127)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.7649 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `8024.5486` → IC=+0.233 (n=792)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8024.5486 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `91.0` → IC=+0.149 (n=722)

  - _Acción_: Kelly boost +0.75€ cuando `ballena_activa_n` < 91.0 (IC base=+0.136)

- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.154 (n=1424)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0076 (IC base=+0.137)

- **PATRÓN** `drift_60min` |x|≤ `0.4486` → IC=+0.153 (n=1423)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.4486 (IC base=+0.137)
...
- **PATRÓN** `dist_vwap_pct` < `0.5983` → IC=+0.141 (n=1573)
...
- **PATRÓN** `volumen_regimen` < `0.695` → IC=+0.151 (n=626)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.695 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` > `1.1945` → IC=+0.143 (n=474)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 1.1945 (IC base=+0.137)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4416` → IC=+0.151 (n=1350)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.4416 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `7268.5561` → IC=+0.199 (n=645)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 7268.5561 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `178.0` → IC=+0.144 (n=1351)

  - _Acción_: Kelly boost +0.72€ cuando `ballena_activa_n` < 178.0 (IC base=+0.137)

### GBM_LATE_15M_TARDIO#SOL#15min
- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.139 (n=1157)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` > 0.0082 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=1784)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.192 (n=1737)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` > 0.4706 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `1.0902` → IC=+0.198 (n=359)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 1.0902 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.508` → IC=+0.236 (n=650)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.508 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` < `0.8946` → IC=+0.135 (n=1157)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 0.8946 (IC base=+0.113)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.128 (n=1743)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.02 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `2897.8254` → IC=+0.249 (n=579)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2897.8254 (IC base=+0.113)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.132 (n=1347)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.126 (n=1735)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 5.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` < `0.6364` → IC=+0.205 (n=1681)
...
- **PATRÓN** `volumen_regimen` < `0.7169` → IC=+0.153 (n=739)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.7169 (IC base=+0.114)
...
- **PATRÓN** `volumen_regimen` < `1.2484` → IC=+0.211 (n=1738)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2484 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` > `0.64` → IC=+0.215 (n=1737)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.64 (IC base=+0.208)

- **PATRÓN** `volumen_pendiente_norm` > `0.2333` → IC=+0.247 (n=303)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2333 (IC base=+0.208)

- **PATRÓN** `volumen_spike_ratio` > `2.5001` → IC=+0.235 (n=560)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5001 (IC base=+0.208)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.216 (n=1829)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.208)

- **PATRÓN** `libro_liquidez` > `2632.6162` → IC=+0.219 (n=1158)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2632.6162 (IC base=+0.208)

- **PATRÓN** `sigma_h` < `0.0091` → IC=+0.225 (n=617)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0091 (IC base=+0.201)

- **PATRÓN** `sigma_h` > `0.0256` → IC=+0.223 (n=619)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0256 (IC base=+0.201)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.210 (n=1297)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.201)

- **PATRÓN** `ibs_20min` < `0.4215` → IC=+0.267 (n=1627)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4215 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` > `1.2792` → IC=+0.205 (n=310)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2792 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` < `0.9289` → IC=+0.203 (n=2052)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.9289 (IC base=+0.201)
...
- **PATRÓN** `volumen_regimen` > `1.236` → IC=+0.237 (n=617)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1818` → IC=+0.198 (n=1465)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 2.1818 (IC base=+0.201)

- **PATRÓN** `volumen_spike_ratio` > `1.4276` → IC=+0.197 (n=1665)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.4276 (IC base=+0.201)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.143 (n=3115)

- **PATRÓN** `sigma_h` < `0.0091` → IC=+0.167 (n=2630)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0091 (IC base=+0.157)

- **PATRÓN** `drift_60min` |x|≤ `0.5186` → IC=+0.166 (n=2984)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.5186 (IC base=+0.157)
...
- **PATRÓN** `volumen_regimen` > `0.8969` → IC=+0.168 (n=1315)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 0.8969 (IC base=+0.157)

- **PATRÓN** `volumen_pendiente_norm` > `0.1707` → IC=+0.191 (n=820)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.1707 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` < `1.4563` → IC=+0.164 (n=984)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.4563 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` > `1.8774` → IC=+0.166 (n=1965)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.8774 (IC base=+0.157)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.157 (n=3313)

  - _Acción_: Kelly boost +0.79€ cuando `libro_spread` < 0.02 (IC base=+0.157)

- **PATRÓN** `libro_liquidez` > `3873.9992` → IC=+0.166 (n=1989)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 3873.9992 (IC base=+0.157)

- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.199 (n=784)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0039 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.4873` → IC=+0.159 (n=2349)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.4873 (IC base=+0.141)
...
- **PATRÓN** `ibs_20min` < `0.1814` → IC=+0.172 (n=1033)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.1814 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` > `0.6833` → IC=+0.162 (n=451)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.6833 (IC base=+0.141)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.273` → IC=+0.152 (n=2335)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` < 6.273 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` < `0.9002` → IC=+0.159 (n=1498)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.9002 (IC base=+0.141)

- **PATRÓN** `volumen_pendiente_norm` < `0.0968` → IC=+0.144 (n=2126)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_pendiente_norm` < 0.0968 (IC base=+0.141)

- **PATRÓN** `volumen_pendiente_norm` > `0.2204` → IC=+0.145 (n=502)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_pendiente_norm` > 0.2204 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` < `1.4242` → IC=+0.154 (n=773)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 1.4242 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` > `1.8143` → IC=+0.144 (n=1545)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.8143 (IC base=+0.141)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.143 (n=3115)

  - _Acción_: Kelly boost +0.71€ cuando `libro_spread` < 0.01 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `7245.8234` → IC=+0.154 (n=2098)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 7245.8234 (IC base=+0.141)
...
- **PATRÓN** `ibs_20min` < `0.5456` → IC=+0.173 (n=650)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.5456 (IC base=+0.165)

- **PATRÓN** `ibs_20min` > `0.8863` → IC=+0.179 (n=325)

  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` > 0.8863 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` < `0.4255` → IC=+0.173 (n=904)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.4255 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.134` → IC=+0.176 (n=874)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` < 4.134 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` < `1.0865` → IC=+0.166 (n=857)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.0865 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` > `0.6416` → IC=+0.168 (n=974)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 0.6416 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` < `0.1056` → IC=+0.166 (n=896)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` < 0.1056 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` > `0.0781` → IC=+0.174 (n=425)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.0781 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` < `1.435` → IC=+0.176 (n=319)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 1.435 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `1.5228` → IC=+0.167 (n=853)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.5228 (IC base=+0.165)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.170 (n=968)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.01 (IC base=+0.165)

- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.190 (n=272)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.0041 (IC base=+0.148)

- **PATRÓN** `drift_60min` |x|≤ `0.4985` → IC=+0.175 (n=810)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.4985 (IC base=+0.148)
...
- **PATRÓN** `hora_utc` < `10.0` → IC=+0.161 (n=546)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 10.0 (IC base=+0.148)

- **PATRÓN** `ibs_20min` < `0.7466` → IC=+0.151 (n=810)

  - _Acción_: Kelly boost +0.76€ cuando `ibs_20min` < 0.7466 (IC base=+0.148)

- **PATRÓN** `ibs_20min` > `0.0956` → IC=+0.156 (n=810)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` > 0.0956 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` > `0.6243` → IC=+0.182 (n=177)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.6243 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.388` → IC=+0.157 (n=732)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` < 4.388 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` < `0.6472` → IC=+0.184 (n=270)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 0.6472 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` > `0.7262` → IC=+0.151 (n=723)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.7262 (IC base=+0.148)

- **PATRÓN** `volumen_pendiente_norm` > `0.0741` → IC=+0.167 (n=340)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.0741 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` < `2.2115` → IC=+0.160 (n=698)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.2115 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` > `1.7829` → IC=+0.152 (n=529)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.7829 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `7572.3913` → IC=+0.174 (n=810)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 7572.3913 (IC base=+0.148)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.126 (n=868)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 8.0 (IC base=+0.093)

- **PATRÓN** `ibs_20min` > `0.6694` → IC=+0.204 (n=761)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6694 (IC base=+0.093)

- **PATRÓN** `dist_vwap_pct` > `0.1562` → IC=+0.157 (n=462)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.1562 (IC base=+0.093)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2792` → IC=+0.202 (n=112)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2792 (IC base=+0.093)

- **PATRÓN** `volumen_spike_ratio` < `2.0892` → IC=+0.129 (n=650)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` < 2.0892 (IC base=+0.093)

- **PATRÓN** `libro_liquidez` > `2424.4386` → IC=+0.138 (n=376)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 2424.4386 (IC base=+0.093)
...
- **FILTRO** `py_entrada` < `0.4` → IC=-0.170 (n=207)
...
- **FILTRO** `profundidad_ratio` < `235.5` → IC=-0.265 (n=15)

  - _Acción_: SKIP cuando `profundidad_ratio` < 235.5
  - _Potencial_: sin este filtro IC_bueno=+0.040 (n=48)

- **PATRÓN** `py_entrada` < `0.53` → IC=+0.173 (n=50)

  - _Acción_: Kelly boost +0.87€ cuando `py_entrada` < 0.53 (IC base=+0.035)

### LIQUIDACIONES_DEPTH_FASE0#BTC#5min
- **PATRÓN** `py_entrada` < `0.59` → IC=+0.133 (n=77)

  - _Acción_: Kelly boost +0.66€ cuando `py_entrada` < 0.59 (IC base=+0.080)

- **PATRÓN** `restante_min` > `3.99` → IC=+0.139 (n=34)

  - _Acción_: Kelly boost +0.69€ cuando `restante_min` > 3.99 (IC base=+0.080)

- **PATRÓN** `profundidad_ratio` > `79.6` → IC=+0.132 (n=74)

  - _Acción_: Kelly boost +0.66€ cuando `profundidad_ratio` > 79.6 (IC base=+0.080)
...
- **FILTRO** `py_entrada` > `0.519` → IC=-0.125 (n=30)
...
- **FILTRO** `py_entrada` < `0.41` → IC=-0.239 (n=21)

  - _Acción_: SKIP cuando `py_entrada` < 0.41
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=28)

- **FILTRO** `profundidad_ratio` < `61.1` → IC=-0.220 (n=23)
...
- **PATRÓN** `profundidad_ratio` > `49.7` → IC=+0.158 (n=36)
  ...
  - _Acción_: Kelly boost +0.79€ cuando `profundidad_ratio` > 49.7 (IC base=+0.013)

### LIQUIDACIONES_DEPTH_FASE0#SOL#15min
- **FILTRO** `lag_apertura_s` > `90.96` → IC=-0.125 (n=38)

  - _Acción_: SKIP cuando `lag_apertura_s` > 90.96
  - _Potencial_: sin este filtro IC_bueno=+0.206 (n=15)

- **PATRÓN** `lag_apertura_s` < `90.96` → IC=+0.206 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `lag_apertura_s` < 90.96 (IC base=-0.027)

- **PATRÓN** `py_entrada` < `0.6` → IC=+0.144 (n=43)

  - _Acción_: Kelly boost +0.72€ cuando `py_entrada` < 0.6 (IC base=+0.038)

- **PATRÓN** `restante_min` > `13.49` → IC=+0.208 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `restante_min` > 13.49 (IC base=+0.038)

- **PATRÓN** `lag_apertura_s` < `90.79` → IC=+0.196 (n=21)

  - _Acción_: Kelly boost +0.98€ cuando `lag_apertura_s` < 90.79 (IC base=+0.038)

### LIQUIDACIONES_DEPTH_FASE0#SOL#5min
- **FILTRO** `restante_min` < `3.75` → IC=-0.167 (n=25)

  - _Acción_: SKIP cuando `restante_min` < 3.75
  - _Potencial_: sin este filtro IC_bueno=+0.071 (n=26)

- **FILTRO** `lag_apertura_s` > `75.01` → IC=-0.167 (n=25)

  - _Acción_: SKIP cuando `lag_apertura_s` > 75.01
  - _Potencial_: sin este filtro IC_bueno=+0.071 (n=26)

### LIQUIDACIONES_DEPTH_FASE0#XRP#15min
- **FILTRO** `py_entrada` < `0.44` → IC=-0.225 (n=38)

  - _Acción_: SKIP cuando `py_entrada` < 0.44
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=45)

- **FILTRO** `profundidad_ratio` < `49.6` → IC=-0.125 (n=54)

  - _Acción_: SKIP cuando `profundidad_ratio` < 49.6
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=29)

### LIQUIDACIONES_DEPTH_FASE0#XRP#5min
- **FILTRO** `py_entrada` < `0.49` → IC=-0.179 (n=76)

  - _Acción_: SKIP cuando `py_entrada` < 0.49
  - _Potencial_: sin este filtro IC_bueno=+0.143 (n=26)

- **FILTRO** `restante_min` < `2.8` → IC=-0.204 (n=25)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=7570)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.166 (n=3591)

  - _Acción_: SKIP cuando `py_entrada` < 0.475
  - _Potencial_: sin este filtro IC_bueno=+0.058 (n=10832)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.167 (n=3710)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.036 (n=11218)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.46` → IC=-0.209 (n=616)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.100 (n=1890)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.47` → IC=-0.192 (n=628)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.203 (n=647)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.062 (n=2026)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.485` → IC=-0.174 (n=609)

  - _Acción_: SKIP cuando `py_entrada` < 0.485
  - _Potencial_: sin este filtro IC_bueno=+0.085 (n=1886)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.176 (n=652)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=2025)

### MOMENTUM_IBS_15M_FADE
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=2792)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=2964)
...
- **FILTRO** `ibs_20min` < `0.1205` → IC=-0.232 (n=80)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=243)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.129 (n=10193)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=22774)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.273 (n=8087)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=24880)

- **FILTRO** `ibs_7min` < `0.2814` → IC=-0.234 (n=8241)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2814
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=24726)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.156 (n=11195)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=21772)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.230 (n=10146)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=31271)

- **FILTRO** `ibs_7min` > `0.2927` → IC=-0.178 (n=10354)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2927
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=31063)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.136 (n=1664)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.075 (n=3797)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.310 (n=1298)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4163)

- **FILTRO** `ibs_7min` < `0.7111` → IC=-0.250 (n=1801)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7111
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=3660)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.186 (n=1232)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=4229)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.260 (n=1750)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=5371)

- **FILTRO** `drift_7min_pct` |x|> `0.1121` → IC=-0.126 (n=2418)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1121
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=4703)

- **FILTRO** `ibs_7min` > `0.7882` → IC=-0.206 (n=1780)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7882
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=5341)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.137 (n=1334)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=4355)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.250 (n=1379)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=4310)

- **FILTRO** `ibs_7min` < `0.7468` → IC=-0.192 (n=1422)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7468
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4267)

- **FILTRO** `ballena_activa_n` > `161.0` → IC=-0.176 (n=1417)

  - _Acción_: SKIP cuando `ballena_activa_n` > 161.0
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=4272)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.255 (n=1442)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=4336)

- **FILTRO** `ibs_7min` > `0.2609` → IC=-0.178 (n=1441)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2609
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=4337)

- **FILTRO** `ballena_activa_n` > `154.0` → IC=-0.183 (n=1441)

  - _Acción_: SKIP cuando `ballena_activa_n` > 154.0
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=4337)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.165 (n=1470)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.082 (n=3716)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.305 (n=1284)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=3902)

- **FILTRO** `ibs_7min` < `0.7063` → IC=-0.243 (n=1711)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7063
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=3475)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.222 (n=1187)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3999)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.237 (n=1742)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=5877)

- **FILTRO** `ibs_7min` > `0.7468` → IC=-0.173 (n=1904)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7468
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=5715)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=1755)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=3690)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.233 (n=1607)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=3838)

- **FILTRO** `ibs_7min` < `0.7407` → IC=-0.179 (n=1361)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7407
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=4084)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.172 (n=1331)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=4114)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.260 (n=1386)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=4162)

- **FILTRO** `ibs_7min` > `0.2745` → IC=-0.178 (n=1385)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2745
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=4163)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.183 (n=1365)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4183)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.254 (n=1412)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=4329)

- **FILTRO** `ibs_7min` < `0.3` → IC=-0.234 (n=1428)

  - _Acción_: SKIP cuando `ibs_7min` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4313)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.179 (n=1869)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=6010)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.254 (n=1771)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3674)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.227 (n=1359)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=4086)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.215 (n=1319)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=4126)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.206 (n=1779)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=5693)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=2954)
...
- **PATRÓN** `ibs_15` < `0.1176` → IC=+0.159 (n=488)
...
- **FILTRO** `sigma_h` > `0.0129` → IC=-0.221 (n=668)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=2007)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1195` → IC=+0.249 (n=1211)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1195 (IC base=-0.031)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1779` → IC=+0.235 (n=1173)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1779 (IC base=-0.031)

- **PATRÓN** `ibs_15` < `0.3453` → IC=+0.278 (n=1815)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3453 (IC base=-0.031)

- **PATRÓN** `dist_vwap_pct` > `0.7074` → IC=+0.303 (n=297)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7074 (IC base=-0.031)

### UPDOWN_GBM_15M_TARDIO#BTC#15min
- **FILTRO** `sigma_h` > `0.0068` → IC=-0.222 (n=408)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0068
  - _Potencial_: sin este filtro IC_bueno=-0.191 (n=1226)

- **FILTRO** `sigma_h` < `0.0034` → IC=-0.234 (n=407)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0034
  - _Potencial_: sin este filtro IC_bueno=-0.187 (n=1227)

- **FILTRO** `hora_utc` > `7.0` → IC=-0.210 (n=1032)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.179 (n=602)

- **FILTRO** `sigma_ewma_delta_pct` > `19.708` → IC=-0.250 (n=294)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 19.708
  - _Potencial_: sin este filtro IC_bueno=-0.187 (n=1340)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4447` → IC=+0.234 (n=709)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4447 (IC base=+0.232)

- **PATRÓN** `drift_15min` |x|≤ `0.48` → IC=+0.239 (n=312)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.48 (IC base=+0.232)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2036` → IC=+0.262 (n=321)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **PATRÓN** `ibs_15` < `0.3452` → IC=+0.271 (n=709)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3452 (IC base=+0.232)

- **PATRÓN** `dist_vwap_pct` > `0.7744` → IC=+0.320 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7744 (IC base=+0.232)

- **PATRÓN** `sigma_ewma_delta_pct` > `20.251` → IC=+0.258 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 20.251 (IC base=+0.232)

- **PATRÓN** `sigma_ewma_delta_pct` < `12.189` → IC=+0.239 (n=752)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 12.189 (IC base=+0.232)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1033` → IC=+0.342 (n=207)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1033 (IC base=-0.042)

- **PATRÓN** `ibs_15` < `0.34` → IC=+0.304 (n=477)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **FILTRO** `sigma_h` < `0.0051` → IC=-0.176 (n=109)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.077 (n=334)
...
- **FILTRO** `sigma_h` < `0.0056` → IC=-0.157 (n=33)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1176 sube el IC de +0.051 a +0.159 en UPDOWN_GBM#XRP#15min (n=488). Ya aplicado como kelly_boost=+0.80€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3453 sube el IC de -0.031 a +0.278 en UPDOWN_GBM_15M_TARDIO (n=1815). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.3452 sube el IC de +0.232 a +0.271 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=709). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#XRP#15min**: dentro de BUY_NO, IBS < 0.34 sube el IC de -0.042 a +0.304 en UPDOWN_GBM_15M_TARDIO#XRP#15min (n=477). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 28632 | -0.089 | -3836.23€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1479 | -0.048 | -222.91€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 27153 | -0.091 | -3613.33€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3762 | -0.097 | -610.91€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3762 | -0.097 | -610.91€ | 1 | 0 |
...
| ✅ BALLENAS_TARDIAS#ETH | 7337 | -0.021 | -682.14€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 7337 | -0.021 | -682.14€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 6932 | -0.092 | -443.22€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 6932 | -0.092 | -443.22€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 5640 | -0.173 | -1089.10€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 5640 | -0.173 | -1089.10€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 18633 | -0.027 | +4004.72€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 4857 | -0.000 | +1816.22€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 13776 | -0.037 | +2188.50€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 18633 | -0.027 | +4004.72€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 4857 | -0.000 | +1816.22€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 13776 | -0.037 | +2188.50€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 94306 | +0.112 | -4777.82€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 14187 | +0.185 | -430.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 379 | -0.080 | -49.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 73626 | +0.100 | -4079.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 6114 | +0.107 | -217.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 12244 | +0.097 | -1042.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 45 | -0.160 | -0.32€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 12184 | +0.099 | -1029.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 19076 | +0.131 | -377.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 4480 | +0.204 | -133.54€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 12207 | +0.111 | -205.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 2347 | +0.103 | -16.53€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 12283 | +0.089 | -1126.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 52 | -0.093 | -6.98€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#DOGE#240min | 15 | -0.243 | -11.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 12216 | +0.091 | -1108.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 20052 | +0.123 | -400.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 5488 | +0.175 | -77.28€ | 1 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 12338 | +0.105 | -256.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 2214 | +0.098 | -58.12€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 18393 | +0.113 | -1098.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 4075 | +0.188 | -220.82€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 282 | -0.039 | +4.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 12483 | +0.091 | -739.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1553 | +0.125 | -142.65€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 12258 | +0.100 | -731.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 47 | -0.031 | +8.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 13 | -0.022 | -0.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 12198 | +0.101 | -739.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 14966 | +0.191 | -999.89€ | 1 | 5 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 14966 | +0.191 | -999.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 3558 | +0.166 | -379.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 3558 | +0.166 | -379.82€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC | 1268 | +0.195 | -15.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC#15min | 1268 | +0.195 | -15.35€ | 4 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 3509 | +0.179 | -303.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 3509 | +0.179 | -303.76€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 3109 | +0.239 | -100.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 3109 | +0.239 | -100.98€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 3443 | +0.191 | -213.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 3443 | +0.191 | -213.73€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 51583 | +0.197 | -4048.79€ | 3 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 51583 | +0.197 | -4048.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 8922 | +0.176 | -1036.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 8922 | +0.176 | -1036.10€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 8238 | +0.222 | -309.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 8238 | +0.222 | -309.83€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 8909 | +0.172 | -1068.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 8909 | +0.172 | -1068.36€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 8333 | +0.219 | -322.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 8333 | +0.219 | -322.43€ | 2 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 8526 | +0.203 | -559.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 8526 | +0.203 | -559.44€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 8655 | +0.193 | -752.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 8655 | +0.193 | -752.62€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 19453 | +0.117 | +162.62€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 19453 | +0.117 | +162.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 9658 | +0.122 | +143.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 9658 | +0.122 | +143.11€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 