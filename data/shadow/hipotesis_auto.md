# Hipótesis automáticas — 2026-09-12 19:27 UTC
_Generado por shadow_postmortem.py sobre 408675 resoluciones (PNL=+42945.46€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.92` → IC=-0.265 (n=5247)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.92
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=15742)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `142.27` → IC=-0.277 (n=729)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 142.27
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=2188)

### BALLENAS_TARDIAS#DOGE#5min
- **FILTRO** `restante_s_al_confirmar` < `133.27` → IC=-0.296 (n=670)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 133.27
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=2010)

### BALLENAS_TARDIAS#ETH#5min
- **FILTRO** `restante_s_al_confirmar` < `145.65` → IC=-0.172 (n=1359)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 145.65
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=4080)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `157.01` → IC=-0.254 (n=1205)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 157.01
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=3617)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `156.95` → IC=-0.343 (n=1320)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 156.95
  - _Potencial_: sin este filtro IC_bueno=-0.100 (n=2685)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.202 (n=1516)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.201)

- **PATRÓN** `libro_liquidez` > `14522.6972` → IC=+0.216 (n=393)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 14522.6972 (IC base=+0.201)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.209 (n=1127)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.365` → IC=+0.268 (n=1081)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.365 (IC base=+0.202)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.203 (n=1582)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.202)

- **PATRÓN** `libro_liquidez` > `12694.6625` → IC=+0.204 (n=556)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12694.6625 (IC base=+0.202)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.146 (n=2144)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.188 (n=462)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.03` → IC=+0.186 (n=1046)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.138 (n=644)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.201 (n=8228)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.217 (n=2933)
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.181 (n=2083)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.182 (n=1954)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.73` → IC=+0.177 (n=1985)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.248 (n=1839)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.329 (n=584)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.198 (n=1990)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.192 (n=840)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.266 (n=1541)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.75 (IC base=+0.219)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.201 (n=4029)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.249 (n=2031)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.74 (IC base=+0.199)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.192 (n=1746)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9725` → IC=+0.230 (n=1818)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9725 (IC base=+0.096)

- **PATRÓN** `dist_vwap_pct` < `0.2184` → IC=+0.243 (n=1113)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2184 (IC base=+0.096)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.838` → IC=+0.165 (n=2097)
...
- **PATRÓN** `volumen_regimen` > `1.0676` → IC=+0.236 (n=612)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.11` → IC=+0.194 (n=1358)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.11 (IC base=+0.096)

- **PATRÓN** `volumen_spike_ratio` > `1.4706` → IC=+0.192 (n=3516)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 1.4706 (IC base=+0.096)

- **PATRÓN** `ibs_20min` < `0.5776` → IC=+0.123 (n=6711)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` < 0.5776 (IC base=+0.055)

- **PATRÓN** `dist_vwap_pct` < `0.1316` → IC=+0.163 (n=1962)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.1316 (IC base=+0.055)

- **PATRÓN** `volumen_regimen` > `0.6202` → IC=+0.162 (n=2065)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.6202 (IC base=+0.055)

- **PATRÓN** `volumen_pendiente_norm` > `0.169` → IC=+0.225 (n=1000)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.169 (IC base=+0.055)

- **PATRÓN** `volumen_spike_ratio` > `1.4611` → IC=+0.193 (n=3435)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 1.4611 (IC base=+0.055)

- **PATRÓN** `ballena_activa_n` < `167.0` → IC=+0.202 (n=3198)
...
- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.258 (n=811)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0049 (IC base=+0.237)

- **PATRÓN** `drift_60min` |x|≤ `0.195` → IC=+0.278 (n=605)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.195 (IC base=+0.237)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.252 (n=622)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.237)

- **PATRÓN** `ibs_20min` < `0.0621` → IC=+0.283 (n=399)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0621 (IC base=+0.237)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.353` → IC=+0.249 (n=942)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.353 (IC base=+0.237)

- **PATRÓN** `volumen_pendiente_norm` < `0.0686` → IC=+0.234 (n=713)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0686 (IC base=+0.237)

- **PATRÓN** `volumen_pendiente_norm` > `0.2841` → IC=+0.288 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2841 (IC base=+0.237)

- **PATRÓN** `volumen_spike_ratio` > `2.6474` → IC=+0.271 (n=264)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6474 (IC base=+0.237)

- **PATRÓN** `libro_spread` < `0.05` → IC=+0.240 (n=956)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.05 (IC base=+0.237)

- **PATRÓN** `libro_liquidez` > `1570.482` → IC=+0.247 (n=810)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1570.482 (IC base=+0.237)

- **PATRÓN** `ballena_activa_n` < `66.0` → IC=+0.231 (n=742)
...
- **PATRÓN** `volumen_regimen` < `1.1954` → IC=+0.151 (n=994)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.1954 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` > `0.6829` → IC=+0.142 (n=888)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3977` → IC=+0.156 (n=885)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.3977 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` > `1.4103` → IC=+0.153 (n=885)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.4103 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `12882.2757` → IC=+0.154 (n=662)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 12882.2757 (IC base=+0.141)

- **PATRÓN** `ballena_activa_n` < `219.0` → IC=+0.169 (n=267)

  - _Acción_: Kelly boost +0.85€ cuando `ballena_activa_n` < 219.0 (IC base=+0.141)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.178 (n=1195)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3929` → IC=+0.229 (n=903)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3929 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.688` → IC=+0.234 (n=366)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.688 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.401` → IC=+0.209 (n=1127)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.401 (IC base=+0.209)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8446` → IC=+0.197 (n=397)
  ...
  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 1.8446 (IC base=+0.209)
...
- **PATRÓN** `libro_liquidez` > `1887.161` → IC=+0.221 (n=342)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1887.161 (IC base=+0.209)

- **PATRÓN** `ballena_activa_n` < `32.0` → IC=+0.200 (n=551)
...
- **PATRÓN** `dist_vwap_pct` > `0.1559` → IC=+0.185 (n=147)
  ...
  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.1559 (IC base=-0.011)

- **PATRÓN** `volumen_regimen` < `1.1607` → IC=+0.137 (n=444)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 1.1607 (IC base=-0.011)

- **PATRÓN** `volumen_regimen` > `0.6141` → IC=+0.136 (n=443)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 0.6141 (IC base=-0.011)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5064` → IC=+0.170 (n=352)
...
- **FILTRO** `ibs_20min` > `0.2857` → IC=-0.125 (n=1567)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.111 (n=777)
...
- **PATRÓN** `dist_vwap_pct` > `0.6475` → IC=+0.289 (n=17)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6475 (IC base=-0.047)

- **PATRÓN** `volumen_regimen` < `1.1257` → IC=+0.173 (n=163)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 1.1257 (IC base=-0.047)

- **PATRÓN** `volumen_regimen` > `0.9321` → IC=+0.180 (n=123)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` > 0.9321 (IC base=-0.047)

- **PATRÓN** `volumen_pendiente_norm` > `0.1483` → IC=+0.226 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1483 (IC base=-0.047)

- **PATRÓN** `volumen_spike_ratio` < `2.43` → IC=+0.213 (n=127)
...
- **FILTRO** `drift_60min` |x|> `0.6434` → IC=-0.188 (n=382)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6434
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=1149)

- **FILTRO** `ibs_20min` < `0.6396` → IC=-0.156 (n=1010)

  - _Acción_: SKIP cuando `ibs_20min` < 0.6396
  - _Potencial_: sin este filtro IC_bueno=+0.049 (n=521)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.183 (n=298)

  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=1233)
...
- **PATRÓN** `dist_vwap_pct` > `0.4569` → IC=+0.258 (n=89)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4569 (IC base=-0.030)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1728` → IC=+0.221 (n=345)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1728 (IC base=-0.030)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5021` → IC=+0.243 (n=340)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5021 (IC base=-0.030)
...
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.158 (n=3078)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0077 (IC base=+0.083)

- **PATRÓN** `ibs_20min` > `0.4545` → IC=+0.170 (n=6062)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` > 0.4545 (IC base=+0.083)

- **PATRÓN** `dist_vwap_pct` > `0.7049` → IC=+0.272 (n=620)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7049 (IC base=+0.083)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.472` → IC=+0.136 (n=3206)

  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` > 3.472 (IC base=+0.083)

- **PATRÓN** `volumen_regimen` > `0.6736` → IC=+0.226 (n=2032)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6736 (IC base=+0.083)

- **PATRÓN** `volumen_pendiente_norm` > `0.248` → IC=+0.254 (n=713)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.248 (IC base=+0.083)

- **PATRÓN** `volumen_spike_ratio` < `1.4802` → IC=+0.232 (n=1196)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4802 (IC base=+0.083)

- **PATRÓN** `volumen_spike_ratio` > `2.7873` → IC=+0.234 (n=1195)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7873 (IC base=+0.083)

- **PATRÓN** `ballena_activa_n` < `105.0` → IC=+0.276 (n=3064)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 105.0 (IC base=+0.083)

- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.138 (n=2337)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0084 (IC base=+0.064)

- **PATRÓN** `ibs_20min` < `0.5625` → IC=+0.145 (n=6158)

  - _Acción_: Kelly boost +0.73€ cuando `ibs_20min` < 0.5625 (IC base=+0.064)

- **PATRÓN** `dist_vwap_pct` > `0.6642` → IC=+0.242 (n=327)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6642 (IC base=+0.064)

- **PATRÓN** `dist_vwap_pct` < `0.1591` → IC=+0.233 (n=1684)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1591 (IC base=+0.064)

- **PATRÓN** `volumen_regimen` > `1.1945` → IC=+0.262 (n=603)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1945 (IC base=+0.064)

- **PATRÓN** `volumen_pendiente_norm` > `0.2488` → IC=+0.321 (n=473)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2488 (IC base=+0.064)

- **PATRÓN** `volumen_spike_ratio` < `1.6164` → IC=+0.251 (n=1027)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6164 (IC base=+0.064)

- **PATRÓN** `volumen_spike_ratio` > `2.3699` → IC=+0.258 (n=1058)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3699 (IC base=+0.064)

- **PATRÓN** `ballena_activa_n` < `78.0` → IC=+0.256 (n=2188)
...
- **FILTRO** `sigma_ewma_delta_pct` > `4.316` → IC=-0.157 (n=351)
...
- **FILTRO** `ibs_20min` > `0.8349` → IC=-0.152 (n=521)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8349
  - _Potencial_: sin este filtro IC_bueno=+0.045 (n=1566)

- **PATRÓN** `dist_vwap_pct` > `0.3015` → IC=+0.127 (n=215)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` > 0.3015 (IC base=+0.005)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4165` → IC=+0.153 (n=188)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 1.4165 (IC base=+0.005)

- **PATRÓN** `ballena_activa_n` < `267.0` → IC=+0.160 (n=242)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 267.0 (IC base=+0.005)

- **PATRÓN** `volumen_regimen` > `1.1253` → IC=+0.239 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1253 (IC base=-0.004)

- **PATRÓN** `volumen_pendiente_norm` > `0.275` → IC=+0.350 (n=38)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7529` → IC=+0.217 (n=189)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7529 (IC base=-0.004)

- **PATRÓN** `volumen_spike_ratio` > `2.1198` → IC=+0.231 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1198 (IC base=-0.004)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0966` → IC=+0.246 (n=360)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0966 (IC base=+0.237)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.241 (n=538)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.237)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.313 (n=287)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.277)

- **PATRÓN** `ibs_20min` < `0.3415` → IC=+0.285 (n=856)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3415 (IC base=+0.277)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3446` → IC=+0.317 (n=129)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3446 (IC base=+0.277)

- **PATRÓN** `volumen_spike_ratio` < `3.3673` → IC=+0.272 (n=762)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.3673 (IC base=+0.277)

- **PATRÓN** `volumen_spike_ratio` > `2.2254` → IC=+0.282 (n=508)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2254 (IC base=+0.277)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.286 (n=433)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.277)

- **PATRÓN** `libro_liquidez` > `1875.8832` → IC=+0.298 (n=285)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1875.8832 (IC base=+0.277)

- **PATRÓN** `ballena_activa_n` < `28.0` → IC=+0.274 (n=330)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 28.0 (IC base=+0.277)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **FILTRO** `ibs_20min` < `0.2183` → IC=-0.213 (n=294)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2183
  - _Potencial_: sin este filtro IC_bueno=+0.035 (n=885)

- **FILTRO** `ibs_20min` > `0.8331` → IC=-0.181 (n=403)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8331
  - _Potencial_: sin este filtro IC_bueno=+0.016 (n=1211)
...
- **PATRÓN** `dist_vwap_pct` > `0.3127` → IC=+0.218 (n=122)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3127 (IC base=-0.027)

- **PATRÓN** `volumen_regimen` < `0.9231` → IC=+0.204 (n=204)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9231 (IC base=-0.027)

- **PATRÓN** `volumen_regimen` > `0.6044` → IC=+0.171 (n=208)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4` → IC=+0.217 (n=210)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4 (IC base=-0.027)

- **PATRÓN** `ballena_activa_n` < `160.0` → IC=+0.231 (n=184)
...
- **PATRÓN** `volumen_regimen` < `1.0685` → IC=+0.134 (n=151)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 1.0685 (IC base=-0.033)

- **PATRÓN** `volumen_regimen` > `0.6889` → IC=+0.138 (n=150)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` > 0.6889 (IC base=-0.033)

- **PATRÓN** `volumen_pendiente_norm` > `0.0595` → IC=+0.308 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0595 (IC base=-0.033)

- **PATRÓN** `volumen_spike_ratio` < `1.7063` → IC=+0.210 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7063 (IC base=-0.033)

- **PATRÓN** `volumen_spike_ratio` > `1.397` → IC=+0.223 (n=110)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.397 (IC base=-0.033)

- **PATRÓN** `ballena_activa_n` < `158.0` → IC=+0.221 (n=109)
...
- **FILTRO** `sigma_ewma_delta_pct` > `4.743` → IC=-0.156 (n=382)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.743
  - _Potencial_: sin este filtro IC_bueno=+0.049 (n=1202)
...
- **PATRÓN** `ibs_20min` < `0.2105` → IC=+0.164 (n=527)
...
- **PATRÓN** `dist_vwap_pct` < `0.156` → IC=+0.190 (n=256)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.156 (IC base=-0.001)

- **PATRÓN** `volumen_regimen` < `0.7171` → IC=+0.229 (n=127)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7171 (IC base=-0.001)

- **PATRÓN** `volumen_pendiente_norm` < `0.1007` → IC=+0.191 (n=247)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5701` → IC=+0.209 (n=259)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5701 (IC base=-0.001)

- **PATRÓN** `volumen_spike_ratio` > `1.5131` → IC=+0.185 (n=258)
...
- **PATRÓN** `ibs_20min` > `0.9` → IC=+0.330 (n=640)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.341` → IC=+0.289 (n=514)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.341 (IC base=+0.262)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.79` → IC=+0.263 (n=1052)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1112` → IC=+0.266 (n=836)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2141` → IC=+0.265 (n=406)
...
- **PATRÓN** `libro_liquidez` > `2431.0035` → IC=+0.270 (n=858)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2431.0035 (IC base=+0.262)
...
- **PATRÓN** `sigma_h` < `0.0048` → IC=+0.173 (n=1821)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.3367` → IC=+0.174 (n=4803)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.3367 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.177 (n=5718)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 5.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` > `0.6946` → IC=+0.229 (n=4875)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6946 (IC base=+0.167)
...
- **PATRÓN** `ballena_activa_n` < `127.0` → IC=+0.184 (n=4367)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 127.0 (IC base=+0.167)

- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.184 (n=3497)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0064 (IC base=+0.169)

- **PATRÓN** `drift_60min` |x|≤ `0.0789` → IC=+0.204 (n=1748)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0789 (IC base=+0.169)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.200 (n=2549)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.169)

- **PATRÓN** `ibs_20min` < `0.4551` → IC=+0.224 (n=5240)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4551 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.272` → IC=+0.195 (n=907)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 10.272 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` < `1.1823` → IC=+0.154 (n=3842)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 1.1823 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` > `0.6248` → IC=+0.152 (n=3842)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.6248 (IC base=+0.169)

- **PATRÓN** `volumen_pendiente_norm` > `0.2909` → IC=+0.225 (n=729)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2909 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` < `1.8716` → IC=+0.168 (n=3065)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.8716 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `2.6218` → IC=+0.171 (n=1532)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 2.6218 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `132.0` → IC=+0.168 (n=4200)
...
- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.237 (n=575)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0064 (IC base=+0.234)

- **PATRÓN** `sigma_h` > `0.0047` → IC=+0.252 (n=583)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0047 (IC base=+0.234)

- **PATRÓN** `drift_60min` |x|≤ `0.1819` → IC=+0.294 (n=434)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1819 (IC base=+0.234)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.242 (n=595)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.1095` → IC=+0.264 (n=434)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1095 (IC base=+0.234)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.964` → IC=+0.247 (n=705)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.964 (IC base=+0.234)

- **PATRÓN** `volumen_pendiente_norm` < `0.0686` → IC=+0.230 (n=506)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0686 (IC base=+0.234)

- **PATRÓN** `volumen_pendiente_norm` > `0.2332` → IC=+0.252 (n=123)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2332 (IC base=+0.234)

- **PATRÓN** `volumen_spike_ratio` < `1.8653` → IC=+0.250 (n=386)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8653 (IC base=+0.234)

- **PATRÓN** `volumen_spike_ratio` > `2.621` → IC=+0.228 (n=193)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.621 (IC base=+0.234)

- **PATRÓN** `libro_liquidez` > `1576.48` → IC=+0.250 (n=581)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1576.48 (IC base=+0.234)

- **PATRÓN** `ballena_activa_n` < `50.0` → IC=+0.228 (n=373)
...
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.180 (n=794)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.0049 (IC base=+0.151)

- **PATRÓN** `drift_60min` |x|≤ `0.2871` → IC=+0.170 (n=903)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.2871 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.168 (n=830)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 7.0 (IC base=+0.151)

- **PATRÓN** `ibs_20min` < `0.5107` → IC=+0.193 (n=903)
...
- **PATRÓN** `volumen_regimen` < `1.1987` → IC=+0.167 (n=903)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.1987 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` > `0.1584` → IC=+0.200 (n=271)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1584 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` < `2.4003` → IC=+0.163 (n=794)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.4003 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` > `1.4105` → IC=+0.154 (n=794)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.4105 (IC base=+0.151)
...
- **PATRÓN** `drift_60min` |x|≤ `0.198` → IC=+0.196 (n=594)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.198 (IC base=+0.188)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.216 (n=301)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1346` → IC=+0.187 (n=343)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1346 (IC base=+0.188)

- **PATRÓN** `volumen_spike_ratio` < `1.6698` → IC=+0.182 (n=275)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 1.6698 (IC base=+0.188)

- **PATRÓN** `volumen_spike_ratio` > `2.3483` → IC=+0.195 (n=549)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 2.3483 (IC base=+0.188)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.204 (n=981)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.188)

- **PATRÓN** `sigma_h` < `0.0105` → IC=+0.228 (n=749)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `libro_liquidez` > `1884.0848` → IC=+0.226 (n=250)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1884.0848 (IC base=+0.217)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.156 (n=806)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0061 (IC base=+0.124)

- **PATRÓN** `drift_60min` |x|≤ `0.3745` → IC=+0.144 (n=914)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.3745 (IC base=+0.124)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.181 (n=305)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 18.0 (IC base=+0.124)

- **PATRÓN** `ibs_20min` < `0.5954` → IC=+0.175 (n=914)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.5954 (IC base=+0.124)

- **PATRÓN** `dist_vwap_pct` < `0.1486` → IC=+0.137 (n=910)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.1486 (IC base=+0.124)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.174` → IC=+0.191 (n=179)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 12.174 (IC base=+0.124)

- **PATRÓN** `volumen_regimen` < `0.8624` → IC=+0.136 (n=611)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 0.8624 (IC base=+0.124)

- **PATRÓN** `volumen_regimen` > `0.613` → IC=+0.131 (n=914)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_regimen` > 0.613 (IC base=+0.124)

- **PATRÓN** `volumen_pendiente_norm` > `0.0738` → IC=+0.164 (n=358)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.0738 (IC base=+0.124)

- **PATRÓN** `volumen_spike_ratio` < `1.7867` → IC=+0.129 (n=534)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` < 1.7867 (IC base=+0.124)

- **PATRÓN** `libro_liquidez` > `11643.1172` → IC=+0.151 (n=305)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 11643.1172 (IC base=+0.124)
...
- **PATRÓN** `sigma_h` < `0.0053` → IC=+0.149 (n=331)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0053 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.178 (n=457)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 15.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` < `0.525` → IC=+0.205 (n=988)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.525 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` < `0.2457` → IC=+0.130 (n=955)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.2457 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.316` → IC=+0.154 (n=206)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 7.316 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` < `1.0388` → IC=+0.126 (n=870)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` < 1.0388 (IC base=+0.114)

- **PATRÓN** `volumen_pendiente_norm` > `0.2732` → IC=+0.167 (n=115)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.2732 (IC base=+0.114)

- **PATRÓN** `volumen_spike_ratio` > `2.133` → IC=+0.133 (n=385)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 2.133 (IC base=+0.114)

- **PATRÓN** `libro_liquidez` > `3113.5147` → IC=+0.157 (n=330)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 3113.5147 (IC base=+0.114)

### GBM_LATE_15M_MULTIHORIZONTE#XRP#15min
- **PATRÓN** `sigma_h` > `0.0173` → IC=+0.211 (n=653)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_h` < `0.0037` → IC=+0.142 (n=434)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0037 (IC base=+0.104)

- **PATRÓN** `drift_60min` |x|≤ `0.0976` → IC=+0.125 (n=435)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 0.0976 (IC base=+0.104)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.124 (n=883)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 11.0 (IC base=+0.104)

- **PATRÓN** `ibs_20min` < `0.305` → IC=+0.160 (n=868)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` < 0.305 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.834` → IC=+0.134 (n=517)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` > 3.834 (IC base=+0.104)

- **PATRÓN** `volumen_pendiente_norm` > `0.1661` → IC=+0.148 (n=330)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` > 0.1661 (IC base=+0.104)

- **PATRÓN** `volumen_spike_ratio` < `2.2216` → IC=+0.127 (n=1087)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` < 2.2216 (IC base=+0.104)

- **PATRÓN** `ballena_activa_n` < `20.0` → IC=+0.141 (n=388)

  - _Acción_: Kelly boost +0.71€ cuando `ballena_activa_n` < 20.0 (IC base=+0.104)
...
- **PATRÓN** `drift_60min` |x|≤ `0.098` → IC=+0.164 (n=108)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.82€ cuando `drift_60min` |x|≤ 0.098 (IC base=+0.086)

- **PATRÓN** `ibs_20min` < `0.3132` → IC=+0.142 (n=216)

  - _Acción_: Kelly boost +0.71€ cuando `ibs_20min` < 0.3132 (IC base=+0.086)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.016` → IC=+0.136 (n=116)

  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` > 4.016 (IC base=+0.086)

- **PATRÓN** `volumen_pendiente_norm` > `0.2153` → IC=+0.130 (n=52)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_pendiente_norm` > 0.2153 (IC base=+0.086)
...
- **FILTRO** `dist_vwap_pct` > `0.3414` → IC=-0.204 (n=25)

  - _Acción_: SKIP cuando `dist_vwap_pct` > 0.3414
  - _Potencial_: sin este filtro IC_bueno=+0.103 (n=280)
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.159 (n=89)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 16.0 (IC base=+0.076)

- **PATRÓN** `ibs_20min` < `0.4524` → IC=+0.157 (n=231)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` < 0.4524 (IC base=+0.076)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.854` → IC=+0.159 (n=42)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 8.854 (IC base=+0.076)

- **PATRÓN** `volumen_regimen` < `0.7263` → IC=+0.160 (n=101)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.7263 (IC base=+0.076)

- **PATRÓN** `volumen_spike_ratio` < `2.4885` → IC=+0.132 (n=207)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` < 2.4885 (IC base=+0.076)

- **PATRÓN** `ballena_activa_n` < `47.0` → IC=+0.146 (n=173)

  - _Acción_: Kelly boost +0.73€ cuando `ballena_activa_n` < 47.0 (IC base=+0.076)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.191 (n=3105)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0086 (IC base=+0.164)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.174 (n=7169)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 5.0 (IC base=+0.164)

- **PATRÓN** `ibs_20min` > `0.4749` → IC=+0.211 (n=6837)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4749 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.8962` → IC=+0.199 (n=908)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.8962 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.568` → IC=+0.221 (n=3343)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.568 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` < `0.8827` → IC=+0.162 (n=3094)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.8827 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.1696` → IC=+0.185 (n=1870)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.1696 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` > `2.6421` → IC=+0.178 (n=2160)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 2.6421 (IC base=+0.164)

- **PATRÓN** `libro_liquidez` > `3810.063` → IC=+0.171 (n=2279)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 3810.063 (IC base=+0.164)

- **PATRÓN** `ballena_activa_n` < `99.0` → IC=+0.192 (n=4808)

  - _Acción_: Kelly boost +0.96€ cuando `ballena_activa_n` < 99.0 (IC base=+0.164)

- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.196 (n=4170)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0067 (IC base=+0.182)

- **PATRÓN** `drift_60min` |x|≤ `0.477` → IC=+0.185 (n=6248)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.477 (IC base=+0.182)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.200 (n=2381)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5588` → IC=+0.237 (n=6248)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5588 (IC base=+0.182)

- **PATRÓN** `dist_vwap_pct` < `0.2291` → IC=+0.167 (n=3944)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` < 0.2291 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.78` → IC=+0.201 (n=912)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.78 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.691` → IC=+0.182 (n=6036)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.691 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` < `1.0559` → IC=+0.157 (n=3797)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.0559 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` > `1.1975` → IC=+0.162 (n=1440)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 1.1975 (IC base=+0.182)

- **PATRÓN** `volumen_pendiente_norm` > `0.2357` → IC=+0.245 (n=1077)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2357 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` > `2.284` → IC=+0.189 (n=2505)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` > 2.284 (IC base=+0.182)

- **PATRÓN** `ballena_activa_n` < `131.0` → IC=+0.177 (n=5123)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.264 (n=599)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0057 (IC base=+0.258)

- **PATRÓN** `sigma_h` > `0.0044` → IC=+0.271 (n=897)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.270 (n=814)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.0692` → IC=+0.303 (n=395)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0692 (IC base=+0.258)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.611` → IC=+0.268 (n=894)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.611 (IC base=+0.258)

- **PATRÓN** `volumen_pendiente_norm` > `0.226` → IC=+0.316 (n=161)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.226 (IC base=+0.258)

- **PATRÓN** `volumen_spike_ratio` > `1.9051` → IC=+0.279 (n=528)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9051 (IC base=+0.258)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.257 (n=900)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.258)

- **PATRÓN** `libro_liquidez` > `1576.48` → IC=+0.267 (n=800)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1576.48 (IC base=+0.258)

- **PATRÓN** `ballena_activa_n` < `70.0` → IC=+0.258 (n=651)
...
- **PATRÓN** `volumen_regimen` < `1.1847` → IC=+0.170 (n=968)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3805` → IC=+0.173 (n=871)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.3805 (IC base=+0.159)

- **PATRÓN** `volumen_spike_ratio` > `2.0851` → IC=+0.172 (n=395)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 2.0851 (IC base=+0.159)

- **PATRÓN** `ballena_activa_n` < `322.0` → IC=+0.176 (n=341)

  - _Acción_: Kelly boost +0.88€ cuando `ballena_activa_n` < 322.0 (IC base=+0.159)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.215 (n=1064)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.6739` → IC=+0.251 (n=946)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.245 (n=391)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3878` → IC=+0.265 (n=913)
...
- **PATRÓN** `volumen_spike_ratio` < `2.9564` → IC=+0.216 (n=804)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.9564 (IC base=+0.226)

- **PATRÓN** `volumen_spike_ratio` > `2.2488` → IC=+0.222 (n=609)
...
- **PATRÓN** `libro_liquidez` > `1890.8984` → IC=+0.227 (n=346)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1890.8984 (IC base=+0.226)

- **PATRÓN** `ballena_activa_n` < `24.0` → IC=+0.225 (n=282)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 24.0 (IC base=+0.226)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.165 (n=509)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0039 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.0947` → IC=+0.139 (n=386)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.70€ cuando `drift_60min` |x|≤ 0.0947 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.148 (n=1210)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 5.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` > `0.7132` → IC=+0.235 (n=769)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.369` → IC=+0.171 (n=503)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 4.369 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `0.8819` → IC=+0.160 (n=769)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.8819 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.273` → IC=+0.229 (n=168)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.273 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.743` → IC=+0.160 (n=737)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 1.743 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `9040.9286` → IC=+0.237 (n=523)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 9040.9286 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `170.0` → IC=+0.145 (n=904)

  - _Acción_: Kelly boost +0.72€ cuando `ballena_activa_n` < 170.0 (IC base=+0.136)

- **PATRÓN** `sigma_h` < `0.0051` → IC=+0.162 (n=611)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0051 (IC base=+0.130)

- **PATRÓN** `drift_60min` |x|≤ `0.4299` → IC=+0.147 (n=914)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.4299 (IC base=+0.130)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.161 (n=349)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 17.0 (IC base=+0.130)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.156 (n=405)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 7.0 (IC base=+0.130)

- **PATRÓN** `ibs_20min` < `0.6933` → IC=+0.180 (n=915)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` < 0.6933 (IC base=+0.130)

- **PATRÓN** `dist_vwap_pct` < `0.2016` → IC=+0.134 (n=840)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.2016 (IC base=+0.130)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.14` → IC=+0.179 (n=135)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 11.14 (IC base=+0.130)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.352` → IC=+0.130 (n=855)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` < 4.352 (IC base=+0.130)

- **PATRÓN** `volumen_regimen` < `0.8529` → IC=+0.134 (n=610)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 0.8529 (IC base=+0.130)

- **PATRÓN** `volumen_regimen` > `1.1684` → IC=+0.145 (n=305)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 1.1684 (IC base=+0.130)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1354` → IC=+0.156 (n=385)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 2.1354 (IC base=+0.130)

- **PATRÓN** `libro_liquidez` > `11135.5359` → IC=+0.171 (n=305)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 11135.5359 (IC base=+0.130)

- **PATRÓN** `ballena_activa_n` < `178.0` → IC=+0.131 (n=740)
...
- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.183 (n=373)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0056 (IC base=+0.120)

- **PATRÓN** `drift_60min` |x|≤ `0.1233` → IC=+0.157 (n=371)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.1233 (IC base=+0.120)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.157 (n=523)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 15.0 (IC base=+0.120)

- **PATRÓN** `ibs_20min` < `0.6136` → IC=+0.213 (n=1111)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6136 (IC base=+0.120)

- **PATRÓN** `dist_vwap_pct` < `0.19` → IC=+0.141 (n=901)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.19 (IC base=+0.120)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.344` → IC=+0.129 (n=1071)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` < 3.344 (IC base=+0.120)

- **PATRÓN** `volumen_regimen` < `0.7144` → IC=+0.156 (n=489)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.7144 (IC base=+0.120)

- **PATRÓN** `volumen_pendiente_norm` > `0.2169` → IC=+0.178 (n=169)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.2169 (IC base=+0.120)

- **PATRÓN** `volumen_spike_ratio` < `1.5802` → IC=+0.155 (n=421)
...
- **PATRÓN** `ibs_20min` > `0.5` → IC=+0.238 (n=1213)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6094` → IC=+0.228 (n=384)
...
- **PATRÓN** `libro_liquidez` > `2566.5304` → IC=+0.206 (n=803)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2566.5304 (IC base=+0.203)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.142 (n=2319)

- **PATRÓN** `sigma_h` < `0.0095` → IC=+0.146 (n=1844)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` < 0.0095 (IC base=+0.130)

- **PATRÓN** `drift_60min` |x|≤ `0.5361` → IC=+0.139 (n=2095)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.69€ cuando `drift_60min` |x|≤ 0.5361 (IC base=+0.130)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.159 (n=722)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9291` → IC=+0.199 (n=700)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9291 (IC base=+0.130)

- **PATRÓN** `dist_vwap_pct` > `0.1852` → IC=+0.127 (n=721)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` > 0.1852 (IC base=+0.130)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.215` → IC=+0.152 (n=337)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 10.215 (IC base=+0.130)

- **PATRÓN** `volumen_regimen` > `0.8967` → IC=+0.124 (n=903)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_regimen` > 0.8967 (IC base=+0.130)

- **PATRÓN** `volumen_pendiente_norm` > `0.1732` → IC=+0.153 (n=578)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_pendiente_norm` > 0.1732 (IC base=+0.130)

- **PATRÓN** `volumen_spike_ratio` < `1.453` → IC=+0.150 (n=692)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.453 (IC base=+0.130)

- **PATRÓN** `volumen_spike_ratio` > `1.8824` → IC=+0.143 (n=1382)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.8824 (IC base=+0.130)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.131 (n=1403)

  - _Acción_: Kelly boost +0.65€ cuando `libro_spread` < 0.01 (IC base=+0.130)

- **PATRÓN** `libro_liquidez` > `8892.7152` → IC=+0.152 (n=950)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 8892.7152 (IC base=+0.130)

- **PATRÓN** `sigma_h` < `0.0037` → IC=+0.200 (n=588)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0037 (IC base=+0.140)

- **PATRÓN** `drift_60min` |x|≤ `0.4792` → IC=+0.162 (n=1751)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.4792 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.171 (n=654)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.1851` → IC=+0.155 (n=771)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.1851 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` > `0.6994` → IC=+0.150 (n=287)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.6994 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.265` → IC=+0.148 (n=1730)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` < 6.265 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `0.8989` → IC=+0.155 (n=1107)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.8989 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` > `0.0718` → IC=+0.154 (n=823)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_pendiente_norm` > 0.0718 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` < `2.5651` → IC=+0.145 (n=1733)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 2.5651 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.8123` → IC=+0.152 (n=1155)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.8123 (IC base=+0.140)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.142 (n=2319)

  - _Acción_: Kelly boost +0.71€ cuando `libro_spread` < 0.01 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `12050.2084` → IC=+0.160 (n=794)
...
- **PATRÓN** `sigma_h` < `0.0034` → IC=+0.207 (n=360)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0034 (IC base=+0.134)

- **PATRÓN** `drift_60min` |x|≤ `0.0856` → IC=+0.174 (n=274)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.0856 (IC base=+0.134)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.172 (n=312)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.1571` → IC=+0.158 (n=360)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` < 0.1571 (IC base=+0.134)
...
- **PATRÓN** `dist_vwap_pct` > `0.6099` → IC=+0.173 (n=105)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.6099 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.318` → IC=+0.158 (n=798)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` < 6.318 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` < `0.8794` → IC=+0.178 (n=545)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` < 0.8794 (IC base=+0.134)

- **PATRÓN** `volumen_pendiente_norm` > `0.0686` → IC=+0.159 (n=382)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_pendiente_norm` > 0.0686 (IC base=+0.134)

- **PATRÓN** `volumen_spike_ratio` < `2.5479` → IC=+0.141 (n=814)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 2.5479 (IC base=+0.134)

- **PATRÓN** `volumen_spike_ratio` > `1.8008` → IC=+0.147 (n=542)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.8008 (IC base=+0.134)
...
- **FILTRO** `ibs_20min` > `0.2527` → IC=-0.125 (n=86)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.135 (n=168)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.145 (n=91)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 15.0 (IC base=+0.047)

- **PATRÓN** `ibs_20min` < `0.2527` → IC=+0.135 (n=168)

  - _Acción_: Kelly boost +0.68€ cuando `ibs_20min` < 0.2527 (IC base=+0.047)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.866` → IC=+0.143 (n=82)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 5.866 (IC base=+0.047)

- **PATRÓN** `volumen_pendiente_norm` > `0.0649` → IC=+0.144 (n=57)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_pendiente_norm` > 0.0649 (IC base=+0.047)

- **PATRÓN** `libro_liquidez` > `3703.4476` → IC=+0.152 (n=87)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 3703.4476 (IC base=+0.047)
...
- **FILTRO** `ibs_20min` > `0.2778` → IC=-0.155 (n=27)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.321` → IC=+0.289 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.321 (IC base=+0.012)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=5548)

### MOMENTUM_IBS_15M#BTC#15min
- **FILTRO** `py_entrada` > `0.505` → IC=-0.147 (n=15)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=955)

### MOMENTUM_IBS_15M#ETH#15min
- **FILTRO** `libro_spread` > `0.01` → IC=-0.121 (n=27)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=1080)

### MOMENTUM_IBS_15M_BALLENA
- **FILTRO** `py_entrada` < `0.465` → IC=-0.175 (n=2302)

  - _Acción_: SKIP cuando `py_entrada` < 0.465
  - _Potencial_: sin este filtro IC_bueno=+0.045 (n=6943)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.177 (n=2198)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.024 (n=7400)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.43` → IC=-0.210 (n=370)

  - _Acción_: SKIP cuando `py_entrada` < 0.43
  - _Potencial_: sin este filtro IC_bueno=+0.085 (n=1161)

- **FILTRO** `ibs_20min` < `0.748` → IC=-0.175 (n=380)

  - _Acción_: SKIP cuando `ibs_20min` < 0.748
  - _Potencial_: sin este filtro IC_bueno=+0.076 (n=1151)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.42` → IC=-0.200 (n=391)

  - _Acción_: SKIP cuando `py_entrada` < 0.42
  - _Potencial_: sin este filtro IC_bueno=+0.073 (n=1211)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.214 (n=400)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=1275)

- **FILTRO** `ibs_20min` > `0.2883` → IC=-0.179 (n=418)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2883
  - _Potencial_: sin este filtro IC_bueno=+0.043 (n=1257)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.47` → IC=-0.193 (n=372)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.062 (n=1124)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.187 (n=420)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.038 (n=1272)

### MOMENTUM_IBS_15M_FADE
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=1930)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=1814)
...
- **FILTRO** `ibs_20min` > `0.1636` → IC=-0.133 (n=88)

  - _Acción_: SKIP cuando `ibs_20min` > 0.1636
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.273 (n=5416)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=16632)

- **FILTRO** `ibs_7min` < `0.7124` → IC=-0.232 (n=5512)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7124
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=16536)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.163 (n=7496)

  - _Acción_: SKIP cuando `ballena_activa_n` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=14552)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.220 (n=6820)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=20704)

- **FILTRO** `ibs_7min` > `0.2981` → IC=-0.175 (n=6880)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2981
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=20644)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.312 (n=808)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=2632)

- **FILTRO** `ibs_7min` < `0.7099` → IC=-0.256 (n=1135)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7099
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=2305)

- **FILTRO** `ballena_activa_n` > `10.0` → IC=-0.203 (n=849)

  - _Acción_: SKIP cuando `ballena_activa_n` > 10.0
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=2591)

- **FILTRO** `py_entrada` > `0.51` → IC=-0.150 (n=3140)

  - _Acción_: SKIP cuando `py_entrada` > 0.51
  - _Potencial_: sin este filtro IC_bueno=+0.087 (n=1642)

- **FILTRO** `drift_7min_pct` |x|> `0.1108` → IC=-0.129 (n=1621)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1108
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=3161)

- **FILTRO** `ibs_7min` > `0.8002` → IC=-0.203 (n=1195)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8002
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=3587)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.146 (n=889)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=3010)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.252 (n=967)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=2932)

- **FILTRO** `ibs_7min` < `0.7628` → IC=-0.182 (n=974)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7628
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=2925)

- **FILTRO** `ballena_activa_n` > `162.0` → IC=-0.172 (n=972)

  - _Acción_: SKIP cuando `ballena_activa_n` > 162.0
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=2927)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.259 (n=901)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=3021)

- **FILTRO** `ibs_7min` > `0.2498` → IC=-0.168 (n=980)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2498
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=2942)

- **FILTRO** `ballena_activa_n` > `153.0` → IC=-0.174 (n=971)

  - _Acción_: SKIP cuando `ballena_activa_n` > 153.0
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=2951)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.177 (n=785)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.095 (n=2471)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.321 (n=762)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=2494)

- **FILTRO** `ibs_7min` < `0.2125` → IC=-0.272 (n=813)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2125
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=2443)

- **FILTRO** `ballena_activa_n` > `9.0` → IC=-0.214 (n=771)

  - _Acción_: SKIP cuando `ballena_activa_n` > 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=2485)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.229 (n=1161)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.012 (n=3865)

- **FILTRO** `ibs_7min` > `0.2667` → IC=-0.153 (n=1704)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2667
  - _Potencial_: sin este filtro IC_bueno=+0.012 (n=3322)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.130 (n=1129)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.091 (n=2502)

- **FILTRO** `ibs_7min` < `0.7511` → IC=-0.185 (n=907)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7511
  - _Potencial_: sin este filtro IC_bueno=-0.076 (n=2724)

- **FILTRO** `ballena_activa_n` > `35.0` → IC=-0.182 (n=888)

  - _Acción_: SKIP cuando `ballena_activa_n` > 35.0
  - _Potencial_: sin este filtro IC_bueno=-0.077 (n=2743)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.261 (n=905)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=2798)

- **FILTRO** `ibs_7min` > `0.2751` → IC=-0.173 (n=925)
...
- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.175 (n=901)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=2802)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.39` → IC=-0.233 (n=1016)

  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=3090)

- **FILTRO** `ibs_7min` < `0.7368` → IC=-0.202 (n=1021)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7368
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=3085)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.168 (n=1246)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=3956)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.35` → IC=-0.281 (n=882)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=2834)

- **FILTRO** `ibs_7min` < `0.7353` → IC=-0.225 (n=929)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7353
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=2787)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.209 (n=877)

  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=2839)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.195 (n=1179)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=3710)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.128 (n=41)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=802)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=2113)
...
- **FILTRO** `ibs_15` < `0.5263` → IC=-0.127 (n=57)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
| ✅ BALLENAS_TARDIAS | 20989 | -0.105 | -3119.50€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1126 | -0.029 | -191.75€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 19863 | -0.109 | -2927.75€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 2917 | -0.111 | -524.02€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 2917 | -0.111 | -524.02€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 1126 | -0.029 | -191.75€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 1126 | -0.029 | -191.75€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 2680 | -0.091 | -606.71€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 2680 | -0.091 | -606.71€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 5439 | -0.064 | -503.91€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 5439 | -0.064 | -503.91€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 4822 | -0.110 | -374.41€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 4822 | -0.110 | -374.41€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 4005 | -0.180 | -918.70€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 4005 | -0.180 | -918.70€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 9776 | -0.051 | +4121.16€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 2684 | -0.010 | +1746.36€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 7092 | -0.066 | +2374.80€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 9776 | -0.051 | +4121.16€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 2684 | -0.010 | +1746.36€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 7092 | -0.066 | +2374.80€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 66504 | +0.113 | -3695.27€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 10634 | +0.182 | -319.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 254 | -0.113 | -46.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 51182 | +0.099 | -3236.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 4434 | +0.115 | -92.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 8540 | +0.093 | -885.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 35 | -0.176 | -2.14€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 8490 | +0.095 | -871.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 13169 | +0.131 | -266.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 3205 | +0.202 | -95.04€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 8484 | +0.108 | -182.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1438 | +0.117 | +33.74€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 