# Hipótesis automáticas — 2026-09-02 04:36 UTC
_Generado por shadow_postmortem.py sobre 247421 resoluciones (PNL=+21851.88€)_
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.189 (n=7356)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.274 (n=623)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `2215.2553` → IC=+0.277 (n=536)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.193 (n=4855)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 15.0 (IC base=+0.188)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.201 (n=2757)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.173 (n=1414)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.185 (n=1198)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.207 (n=15772)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.170 (n=2272)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.181 (n=2695)
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.240 (n=1350)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.189 (n=2063)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` < 12.0 (IC base=+0.182)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.203 (n=2630)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3493` → IC=+0.123 (n=3267)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` > 0.3493 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` > `0.2623` → IC=+0.225 (n=402)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2623 (IC base=+0.074)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.44` → IC=+0.147 (n=1250)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` > 5.44 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` > `1.0855` → IC=+0.237 (n=333)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0855 (IC base=+0.074)

- **PATRÓN** `volumen_pendiente_norm` > `0.1085` → IC=+0.171 (n=701)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.1085 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` < `2.4282` → IC=+0.166 (n=1678)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.4282 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` > `1.4796` → IC=+0.165 (n=1907)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.4796 (IC base=+0.074)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.196 (n=898)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 54.0 (IC base=+0.074)

- **PATRÓN** `ibs_20min` < `0.4059` → IC=+0.122 (n=3201)

  - _Acción_: Kelly boost +0.61€ cuando `ibs_20min` < 0.4059 (IC base=+0.037)

- **PATRÓN** `volumen_regimen` < `0.6769` → IC=+0.161 (n=482)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.6769 (IC base=+0.037)

- **PATRÓN** `volumen_pendiente_norm` > `0.3055` → IC=+0.265 (n=236)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3055 (IC base=+0.037)

- **PATRÓN** `volumen_spike_ratio` > `2.8846` → IC=+0.214 (n=523)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8846 (IC base=+0.037)

- **PATRÓN** `ballena_activa_n` < `37.0` → IC=+0.227 (n=588)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 37.0 (IC base=+0.037)

### GBM_LATE_15M#BNB#15min
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.176 (n=328)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.007 (IC base=+0.148)

- **PATRÓN** `drift_60min` |x|≤ `0.2693` → IC=+0.149 (n=724)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.2693 (IC base=+0.148)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.181 (n=352)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` < 8.0 (IC base=+0.148)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.287 (n=256)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.453` → IC=+0.300 (n=228)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.453 (IC base=+0.148)

- **PATRÓN** `volumen_pendiente_norm` > `0.2285` → IC=+0.214 (n=124)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2285 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` < `2.3307` → IC=+0.124 (n=552)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_spike_ratio` < 2.3307 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` > `1.4564` → IC=+0.144 (n=627)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.4564 (IC base=+0.148)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.190 (n=560)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.04 (IC base=+0.148)

- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.181 (n=377)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 61.0 (IC base=+0.148)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.265 (n=322)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.266 (n=477)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.259)

- **PATRÓN** `ibs_20min` < `0.4014` → IC=+0.284 (n=419)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0686` → IC=+0.266 (n=327)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8031` → IC=+0.323 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8031 (IC base=+0.259)
...
- **PATRÓN** `sigma_h` > `0.008` → IC=+0.200 (n=308)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.008 (IC base=+0.161)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.200 (n=255)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.161)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.264 (n=261)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.171` → IC=+0.253 (n=156)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.171 (IC base=+0.161)

- **PATRÓN** `volumen_pendiente_norm` < `0.1324` → IC=+0.160 (n=572)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` < 0.1324 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` < `3.2728` → IC=+0.151 (n=523)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 3.2728 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` > `1.6927` → IC=+0.173 (n=594)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.6927 (IC base=+0.161)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.188 (n=677)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.04 (IC base=+0.161)

- **PATRÓN** `ballena_activa_n` < `16.0` → IC=+0.231 (n=132)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4724` → IC=+0.243 (n=540)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.245 (n=558)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.236)

- **PATRÓN** `ibs_20min` < `0.3966` → IC=+0.267 (n=475)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3966 (IC base=+0.236)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.205` → IC=-0.181 (n=186)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.203 (n=207)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=739)

- **FILTRO** `sigma_h` > `0.0245` → IC=-0.140 (n=373)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0245
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=1120)

- **FILTRO** `ibs_20min` > `0.797` → IC=-0.180 (n=373)

  - _Acción_: SKIP cuando `ibs_20min` > 0.797
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=1120)

- **FILTRO** `sigma_ewma_delta_pct` > `8.646` → IC=-0.147 (n=182)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.646
...
- **PATRÓN** `volumen_regimen` > `0.6895` → IC=+0.231 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6895 (IC base=-0.105)

- **PATRÓN** `volumen_pendiente_norm` > `0.0729` → IC=+0.274 (n=29)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3039` → IC=+0.167 (n=19)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 2.3039 (IC base=-0.105)

- **PATRÓN** `ballena_activa_n` < `24.0` → IC=+0.210 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 24.0 (IC base=-0.105)

- **PATRÓN** `volumen_regimen` < `0.6886` → IC=+0.197 (n=74)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` < 0.6886 (IC base=-0.052)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3188` → IC=+0.161 (n=57)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.3188 (IC base=-0.052)

- **PATRÓN** `volumen_spike_ratio` > `1.6969` → IC=+0.161 (n=57)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.6969 (IC base=-0.052)

- **PATRÓN** `ballena_activa_n` < `18.0` → IC=+0.151 (n=41)

  - _Acción_: Kelly boost +0.76€ cuando `ballena_activa_n` < 18.0 (IC base=-0.052)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.147 (n=1846)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0077 (IC base=+0.060)

- **PATRÓN** `ibs_20min` > `0.2718` → IC=+0.129 (n=4072)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` > 0.2718 (IC base=+0.060)

- **PATRÓN** `dist_vwap_pct` > `1.2127` → IC=+0.288 (n=286)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2127 (IC base=+0.060)

- **PATRÓN** `volumen_regimen` > `0.6802` → IC=+0.207 (n=1169)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6802 (IC base=+0.060)

- **PATRÓN** `volumen_pendiente_norm` < `0.115` → IC=+0.203 (n=1842)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.115 (IC base=+0.060)

- **PATRÓN** `volumen_pendiente_norm` > `0.2534` → IC=+0.217 (n=359)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2534 (IC base=+0.060)

- **PATRÓN** `volumen_spike_ratio` < `1.497` → IC=+0.219 (n=631)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.497 (IC base=+0.060)

- **PATRÓN** `volumen_spike_ratio` > `2.8672` → IC=+0.214 (n=631)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8672 (IC base=+0.060)

- **PATRÓN** `ballena_activa_n` < `90.0` → IC=+0.292 (n=1346)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 90.0 (IC base=+0.060)

- **PATRÓN** `ibs_20min` < `0.5868` → IC=+0.127 (n=3997)
...
- **FILTRO** `ibs_20min` < `0.2584` → IC=-0.152 (n=334)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.081 (n=680)

- **FILTRO** `sigma_ewma_delta_pct` > `2.223` → IC=-0.166 (n=282)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 2.223
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=618)

- **PATRÓN** `ibs_20min` > `0.7864` → IC=+0.188 (n=254)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.7864 (IC base=+0.004)
...
- **PATRÓN** `ballena_activa_n` < `40.0` → IC=+0.406 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 40.0 (IC base=+0.004)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **FILTRO** `ibs_20min` > `0.7642` → IC=-0.140 (n=478)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7642
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=929)
...
- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.278 (n=219)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0091 (IC base=+0.213)

- **PATRÓN** `drift_60min` |x|≤ `0.0827` → IC=+0.220 (n=219)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0827 (IC base=+0.213)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.245 (n=245)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.213)

- **PATRÓN** `ibs_20min` > `0.7083` → IC=+0.250 (n=587)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7083 (IC base=+0.213)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.142` → IC=+0.276 (n=159)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.142 (IC base=+0.213)

- **PATRÓN** `volumen_pendiente_norm` < `0.1413` → IC=+0.226 (n=550)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1413 (IC base=+0.213)

- **PATRÓN** `volumen_spike_ratio` < `2.5143` → IC=+0.217 (n=383)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5143 (IC base=+0.213)

- **PATRÓN** `volumen_spike_ratio` > `3.2944` → IC=+0.225 (n=260)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.2944 (IC base=+0.213)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.242 (n=656)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.213)

- **PATRÓN** `ballena_activa_n` < `20.0` → IC=+0.250 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 20.0 (IC base=+0.213)
...
- **FILTRO** `ibs_20min` > `0.8712` → IC=-0.158 (n=290)
...
- **FILTRO** `ibs_20min` < `0.7797` → IC=-0.146 (n=580)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.261 (n=303)
...
- **PATRÓN** `ballena_activa_n` < `60.0` → IC=+0.281 (n=194)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 60.0 (IC base=-0.006)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` > `0.0213` → IC=+0.322 (n=313)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.253 (n=606)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.245)

- **PATRÓN** `ibs_20min` > `0.8983` → IC=+0.316 (n=459)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8983 (IC base=+0.245)
...
- **PATRÓN** `volumen_regimen` > `0.8448` → IC=+0.283 (n=459)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8448 (IC base=+0.245)

- **PATRÓN** `volumen_pendiente_norm` > `0.2406` → IC=+0.286 (n=129)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2406 (IC base=+0.245)

- **PATRÓN** `volumen_spike_ratio` < `2.5805` → IC=+0.251 (n=629)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5805 (IC base=+0.245)

- **PATRÓN** `volumen_spike_ratio` > `2.2293` → IC=+0.249 (n=285)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2293 (IC base=+0.245)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.255 (n=695)
...
- **PATRÓN** `sigma_h` > `0.0103` → IC=+0.206 (n=1144)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0103 (IC base=+0.163)

- **PATRÓN** `drift_60min` |x|≤ `0.3322` → IC=+0.169 (n=3014)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.286 (n=1569)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.526` → IC=+0.235 (n=1396)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.526 (IC base=+0.163)

- **PATRÓN** `volumen_regimen` > `0.6276` → IC=+0.170 (n=2334)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 0.6276 (IC base=+0.163)

- **PATRÓN** `volumen_pendiente_norm` > `0.1054` → IC=+0.186 (n=1256)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1054 (IC base=+0.163)

- **PATRÓN** `volumen_spike_ratio` < `2.324` → IC=+0.166 (n=2782)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.324 (IC base=+0.163)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.171 (n=2655)

  - _Acción_: Kelly boost +0.86€ cuando `libro_spread` < 0.01 (IC base=+0.163)

- **PATRÓN** `libro_liquidez` > `3951.6306` → IC=+0.180 (n=1142)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 3951.6306 (IC base=+0.163)

- **PATRÓN** `ballena_activa_n` < `89.0` → IC=+0.183 (n=2052)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 89.0 (IC base=+0.163)

- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.194 (n=2156)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0063 (IC base=+0.179)

- **PATRÓN** `drift_60min` |x|≤ `0.0795` → IC=+0.214 (n=1077)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.183 (n=1486)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` < 7.0 (IC base=+0.179)

- **PATRÓN** `ibs_20min` < `0.4217` → IC=+0.231 (n=3227)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4217 (IC base=+0.179)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.914` → IC=+0.223 (n=591)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.914 (IC base=+0.179)

- **PATRÓN** `volumen_regimen` < `1.1714` → IC=+0.170 (n=2457)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 1.1714 (IC base=+0.179)

- **PATRÓN** `volumen_pendiente_norm` > `0.2948` → IC=+0.255 (n=427)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2948 (IC base=+0.179)

- **PATRÓN** `volumen_spike_ratio` < `1.5764` → IC=+0.172 (n=1146)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.5764 (IC base=+0.179)

- **PATRÓN** `volumen_spike_ratio` > `2.6719` → IC=+0.195 (n=868)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 2.6719 (IC base=+0.179)

- **PATRÓN** `ballena_activa_n` < `160.0` → IC=+0.177 (n=2189)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2758` → IC=+0.191 (n=567)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.2758 (IC base=+0.171)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.215 (n=254)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.54` → IC=+0.322 (n=183)
...
- **PATRÓN** `volumen_spike_ratio` < `2.7061` → IC=+0.155 (n=485)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.7061 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` > `1.4596` → IC=+0.167 (n=485)
...
- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.187 (n=257)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2562` → IC=+0.286 (n=316)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2562 (IC base=+0.244)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.245 (n=387)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.244)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.262 (n=381)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.244)

- **PATRÓN** `ibs_20min` < `0.2708` → IC=+0.267 (n=316)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2708 (IC base=+0.244)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.691` → IC=+0.257 (n=397)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.691 (IC base=+0.244)

- **PATRÓN** `volumen_pendiente_norm` < `0.0896` → IC=+0.239 (n=259)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0896 (IC base=+0.244)

- **PATRÓN** `volumen_pendiente_norm` > `0.2488` → IC=+0.285 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2488 (IC base=+0.244)

- **PATRÓN** `volumen_spike_ratio` < `1.9129` → IC=+0.255 (n=194)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9129 (IC base=+0.244)

- **PATRÓN** `volumen_spike_ratio` > `2.791` → IC=+0.258 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.791 (IC base=+0.244)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.285 (n=161)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.244)

- **PATRÓN** `libro_liquidez` > `1698.2448` → IC=+0.276 (n=239)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1698.2448 (IC base=+0.244)

- **PATRÓN** `ballena_activa_n` < `50.0` → IC=+0.256 (n=170)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4064` → IC=+0.179 (n=499)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.4738` → IC=+0.220 (n=498)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4738 (IC base=+0.171)

- **PATRÓN** `dist_vwap_pct` > `0.1867` → IC=+0.232 (n=349)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1867 (IC base=+0.171)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.987` → IC=+0.238 (n=139)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.987 (IC base=+0.171)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.52` → IC=+0.174 (n=476)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` < 7.52 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` < `0.6988` → IC=+0.176 (n=220)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 0.6988 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` > `1.0648` → IC=+0.189 (n=226)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` > 1.0648 (IC base=+0.171)

- **PATRÓN** `volumen_pendiente_norm` > `0.165` → IC=+0.193 (n=148)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.165 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` < `2.1261` → IC=+0.192 (n=413)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` < 2.1261 (IC base=+0.171)

- **PATRÓN** `libro_liquidez` > `11364.2884` → IC=+0.191 (n=445)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 11364.2884 (IC base=+0.171)
...
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.208 (n=238)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0081 (IC base=+0.169)

- **PATRÓN** `drift_60min` |x|≤ `0.2701` → IC=+0.187 (n=461)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.2701 (IC base=+0.169)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.194 (n=178)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` > 17.0 (IC base=+0.169)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.189 (n=181)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` < 5.0 (IC base=+0.169)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.308 (n=274)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.32` → IC=+0.284 (n=123)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.32 (IC base=+0.169)

- **PATRÓN** `volumen_pendiente_norm` > `0.1328` → IC=+0.163 (n=191)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.1328 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` < `1.9477` → IC=+0.170 (n=204)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` < 1.9477 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `3.2879` → IC=+0.160 (n=210)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 3.2879 (IC base=+0.169)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.195 (n=512)

  - _Acción_: Kelly boost +0.97€ cuando `libro_spread` < 0.04 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `16.0` → IC=+0.214 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 16.0 (IC base=+0.169)

- **PATRÓN** `sigma_h` < `0.0103` → IC=+0.249 (n=412)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0103 (IC base=+0.230)

- **PATRÓN** `drift_60min` |x|≤ `0.2226` → IC=+0.265 (n=275)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `16.0` → IC=+0.231 (n=373)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 16.0 (IC base=+0.230)

- **PATRÓN** `ibs_20min` < `0.3724` → IC=+0.273 (n=412)
...
- **PATRÓN** `ballena_activa_n` < `60.0` → IC=+0.193 (n=298)
...
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.173 (n=264)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0094 (IC base=+0.133)

- **PATRÓN** `drift_60min` |x|≤ `0.5207` → IC=+0.133 (n=581)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.66€ cuando `drift_60min` |x|≤ 0.5207 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.191 (n=270)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 15.0 (IC base=+0.133)

- **PATRÓN** `ibs_20min` < `0.4375` → IC=+0.232 (n=581)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4375 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` > `1.013` → IC=+0.151 (n=64)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 1.013 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.873` → IC=+0.207 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.873 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` < `1.1545` → IC=+0.148 (n=581)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 1.1545 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` > `0.8432` → IC=+0.148 (n=387)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` > 0.8432 (IC base=+0.133)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5492` → IC=+0.145 (n=198)
...
- **PATRÓN** `libro_liquidez` > `1427.0271` → IC=+0.158 (n=519)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.257 (n=315)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.215)

- **PATRÓN** `ibs_20min` < `0.3987` → IC=+0.258 (n=685)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3987 (IC base=+0.215)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.47` → IC=+0.268 (n=257)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.47 (IC base=+0.215)

- **PATRÓN** `volumen_regimen` > `0.6917` → IC=+0.234 (n=612)
...
- **PATRÓN** `sigma_h` > `0.0064` → IC=+0.160 (n=474)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` > 0.0064 (IC base=+0.132)

- **PATRÓN** `drift_60min` |x|≤ `0.3319` → IC=+0.132 (n=626)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.66€ cuando `drift_60min` |x|≤ 0.3319 (IC base=+0.132)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.160 (n=681)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 8.0 (IC base=+0.132)

- **PATRÓN** `ibs_20min` > `0.6` → IC=+0.180 (n=639)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` > 0.6 (IC base=+0.132)

- **PATRÓN** `dist_vwap_pct` > `0.1633` → IC=+0.195 (n=244)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1633 (IC base=+0.132)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.287` → IC=+0.190 (n=188)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 8.287 (IC base=+0.132)

- **PATRÓN** `volumen_regimen` < `0.8827` → IC=+0.147 (n=378)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.8827 (IC base=+0.132)

- **PATRÓN** `volumen_regimen` > `1.1694` → IC=+0.154 (n=189)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.1694 (IC base=+0.132)

- **PATRÓN** `volumen_pendiente_norm` > `0.1816` → IC=+0.199 (n=194)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.1816 (IC base=+0.132)

- **PATRÓN** `volumen_spike_ratio` < `2.3099` → IC=+0.141 (n=578)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 2.3099 (IC base=+0.132)

- **PATRÓN** `volumen_spike_ratio` > `1.899` → IC=+0.139 (n=438)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 1.899 (IC base=+0.132)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.144 (n=529)

  - _Acción_: Kelly boost +0.72€ cuando `libro_spread` < 0.01 (IC base=+0.132)

- **PATRÓN** `libro_liquidez` > `4704.18` → IC=+0.148 (n=237)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 4704.18 (IC base=+0.132)

- **PATRÓN** `ballena_activa_n` < `12.0` → IC=+0.180 (n=170)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 12.0 (IC base=+0.132)

- **PATRÓN** `ibs_20min` < `0.3111` → IC=+0.131 (n=478)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.3111 (IC base=+0.060)

- **PATRÓN** `ballena_activa_n` < `17.0` → IC=+0.169 (n=246)

  - _Acción_: Kelly boost +0.85€ cuando `ballena_activa_n` < 17.0 (IC base=+0.060)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0994` → IC=+0.268 (n=54)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.7838` → IC=+0.327 (n=108)
...
- **PATRÓN** `volumen_regimen` < `0.6873` → IC=+0.281 (n=71)
...
- **PATRÓN** `volumen_spike_ratio` < `1.368` → IC=+0.300 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.368 (IC base=+0.260)

- **PATRÓN** `volumen_spike_ratio` > `2.5338` → IC=+0.318 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5338 (IC base=+0.260)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0085` → IC=+0.201 (n=1846)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.301 (n=1416)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` > `0.2294` → IC=+0.203 (n=1474)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2294 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.392` → IC=+0.229 (n=2074)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.392 (IC base=+0.162)

- **PATRÓN** `volumen_regimen` < `0.8824` → IC=+0.156 (n=1891)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.8824 (IC base=+0.162)

- **PATRÓN** `volumen_regimen` > `1.0852` → IC=+0.153 (n=1286)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.0852 (IC base=+0.162)

- **PATRÓN** `volumen_pendiente_norm` > `0.1666` → IC=+0.192 (n=1092)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.1666 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` > `1.8815` → IC=+0.171 (n=2499)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 1.8815 (IC base=+0.162)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.164 (n=3939)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.02 (IC base=+0.162)

- **PATRÓN** `libro_liquidez` > `3900.1145` → IC=+0.187 (n=1357)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3900.1145 (IC base=+0.162)

- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.212 (n=1803)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4749` → IC=+0.195 (n=3807)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.97€ cuando `drift_60min` |x|≤ 0.4749 (IC base=+0.187)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.204 (n=1725)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.187)

- **PATRÓN** `ibs_20min` < `0.5597` → IC=+0.241 (n=3807)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5597 (IC base=+0.187)

- **PATRÓN** `dist_vwap_pct` < `0.2435` → IC=+0.173 (n=2434)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.2435 (IC base=+0.187)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.576` → IC=+0.215 (n=546)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.576 (IC base=+0.187)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.679` → IC=+0.190 (n=3570)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` < 2.679 (IC base=+0.187)

- **PATRÓN** `volumen_regimen` < `0.6208` → IC=+0.178 (n=912)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` < 0.6208 (IC base=+0.187)

- **PATRÓN** `volumen_pendiente_norm` > `0.2378` → IC=+0.252 (n=610)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2378 (IC base=+0.187)

- **PATRÓN** `volumen_spike_ratio` > `2.3241` → IC=+0.197 (n=1406)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 2.3241 (IC base=+0.187)

- **PATRÓN** `ballena_activa_n` < `142.0` → IC=+0.174 (n=2680)
...
- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.220 (n=301)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_spike_ratio` < `1.5939` → IC=+0.168 (n=257)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.5939 (IC base=+0.180)

- **PATRÓN** `volumen_spike_ratio` > `1.9019` → IC=+0.177 (n=388)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 1.9019 (IC base=+0.180)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.237 (n=496)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.180)

- **PATRÓN** `ballena_activa_n` < `64.0` → IC=+0.237 (n=291)
...
- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.176 (n=220)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0069 (IC base=+0.159)

- **PATRÓN** `drift_60min` |x|≤ `0.0932` → IC=+0.168 (n=221)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.0932 (IC base=+0.159)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.177 (n=691)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 5.0 (IC base=+0.159)

- **PATRÓN** `ibs_20min` > `0.3271` → IC=+0.208 (n=660)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3271 (IC base=+0.159)

- **PATRÓN** `dist_vwap_pct` > `0.2359` → IC=+0.217 (n=365)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.384` → IC=+0.168 (n=574)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` < 4.384 (IC base=+0.159)

- **PATRÓN** `volumen_regimen` < `1.2673` → IC=+0.165 (n=661)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.2673 (IC base=+0.159)

- **PATRÓN** `volumen_regimen` > `1.1002` → IC=+0.169 (n=300)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 1.1002 (IC base=+0.159)

- **PATRÓN** `volumen_pendiente_norm` < `0.074` → IC=+0.177 (n=550)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` < 0.074 (IC base=+0.159)

- **PATRÓN** `volumen_pendiente_norm` > `0.2078` → IC=+0.187 (n=129)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.2078 (IC base=+0.159)

- **PATRÓN** `volumen_spike_ratio` < `2.1056` → IC=+0.180 (n=538)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 2.1056 (IC base=+0.159)

- **PATRÓN** `volumen_spike_ratio` > `1.3839` → IC=+0.174 (n=612)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 1.3839 (IC base=+0.159)

- **PATRÓN** `libro_liquidez` > `10760.7327` → IC=+0.188 (n=590)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 10760.7327 (IC base=+0.159)

- **PATRÓN** `ballena_activa_n` < `500.0` → IC=+0.173 (n=536)

  - _Acción_: Kelly boost +0.86€ cuando `ballena_activa_n` < 500.0 (IC base=+0.159)
...
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.267 (n=191)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0094 (IC base=+0.204)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.241 (n=272)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.204)

- **PATRÓN** `ibs_20min` > `0.6907` → IC=+0.264 (n=511)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6907 (IC base=+0.204)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.357` → IC=+0.333 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.357 (IC base=+0.204)

- **PATRÓN** `volumen_pendiente_norm` < `0.2199` → IC=+0.212 (n=509)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2199 (IC base=+0.204)

- **PATRÓN** `volumen_spike_ratio` < `1.9302` → IC=+0.216 (n=220)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9302 (IC base=+0.204)

- **PATRÓN** `volumen_spike_ratio` > `4.1507` → IC=+0.196 (n=166)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 4.1507 (IC base=+0.204)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.236 (n=559)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.204)

- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.261 (n=287)
...
- **PATRÓN** `ibs_20min` > `0.7469` → IC=+0.237 (n=443)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7469 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `0.3783` → IC=+0.183 (n=266)
...
- **PATRÓN** `volumen_regimen` < `0.9051` → IC=+0.170 (n=443)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1035` → IC=+0.232 (n=244)
...
- **PATRÓN** `libro_liquidez` > `9624.3354` → IC=+0.236 (n=301)
...
- **FILTRO** `ibs_20min` < `0.4841` → IC=-0.181 (n=236)

  - _Acción_: SKIP cuando `ibs_20min` < 0.4841
  - _Potencial_: sin este filtro IC_bueno=+0.185 (n=710)

- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.144 (n=473)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.4841` → IC=+0.185 (n=710)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` > 0.4841 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` > `0.8836` → IC=+0.203 (n=183)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8836 (IC base=+0.094)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.436` → IC=+0.193 (n=353)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 3.436 (IC base=+0.094)

- **PATRÓN** `libro_liquidez` > `2971.573` → IC=+0.253 (n=237)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2971.573 (IC base=+0.094)

- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.142 (n=431)

  - _Acción_: Kelly boost +0.71€ cuando `ballena_activa_n` < 56.0 (IC base=+0.094)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.177 (n=227)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` < 0.0058 (IC base=+0.107)

- **PATRÓN** `drift_60min` |x|≤ `0.1302` → IC=+0.159 (n=227)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.1302 (IC base=+0.107)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.135 (n=332)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 15.0 (IC base=+0.107)

- **PATRÓN** `ibs_20min` < `0.6` → IC=+0.192 (n=682)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` < 0.6 (IC base=+0.107)

- **PATRÓN** `dist_vwap_pct` < `0.2914` → IC=+0.130 (n=563)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.2914 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.187` → IC=+0.130 (n=665)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` < 3.187 (IC base=+0.107)

- **PATRÓN** `volumen_regimen` < `1.0568` → IC=+0.121 (n=597)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` < 1.0568 (IC base=+0.107)

- **PATRÓN** `volumen_pendiente_norm` > `0.0719` → IC=+0.145 (n=218)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_pendiente_norm` > 0.0719 (IC base=+0.107)

- **PATRÓN** `volumen_spike_ratio` < `1.443` → IC=+0.137 (n=177)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 1.443 (IC base=+0.107)

- **PATRÓN** `libro_liquidez` > `2648.2907` → IC=+0.165 (n=308)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 2648.2907 (IC base=+0.107)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` > `0.0165` → IC=+0.230 (n=535)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.12` → IC=+0.249 (n=401)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.12 (IC base=+0.195)

- **PATRÓN** `volumen_regimen` > `0.6863` → IC=+0.208 (n=717)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6863 (IC base=+0.195)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.223 (n=923)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.217)

- **PATRÓN** `ibs_20min` < `0.4944` → IC=+0.269 (n=878)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4944 (IC base=+0.217)

- **PATRÓN** `dist_vwap_pct` < `0.1806` → IC=+0.225 (n=781)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1806 (IC base=+0.217)
...
- **PATRÓN** `volumen_spike_ratio` > `1.465` → IC=+0.209 (n=703)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.465 (IC base=+0.217)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.195 (n=602)
...
- **PATRÓN** `sigma_h` < `0.0103` → IC=+0.134 (n=1138)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.0103 (IC base=+0.125)

- **PATRÓN** `drift_60min` |x|≤ `0.4433` → IC=+0.137 (n=1138)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.4433 (IC base=+0.125)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.163 (n=455)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 18.0 (IC base=+0.125)

- **PATRÓN** `hora_utc` < `4.0` → IC=+0.137 (n=444)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` < 4.0 (IC base=+0.125)

- **PATRÓN** `ibs_20min` > `0.9213` → IC=+0.198 (n=431)

  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` > 0.9213 (IC base=+0.125)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.542` → IC=+0.168 (n=197)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` > 10.542 (IC base=+0.125)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.078` → IC=+0.129 (n=1318)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` < 6.078 (IC base=+0.125)

- **PATRÓN** `volumen_regimen` > `0.8915` → IC=+0.131 (n=607)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` > 0.8915 (IC base=+0.125)

- **PATRÓN** `volumen_pendiente_norm` > `0.175` → IC=+0.163 (n=351)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` > 0.175 (IC base=+0.125)

- **PATRÓN** `volumen_spike_ratio` < `1.4634` → IC=+0.153 (n=427)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 1.4634 (IC base=+0.125)

- **PATRÓN** `volumen_spike_ratio` > `2.3911` → IC=+0.155 (n=580)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 2.3911 (IC base=+0.125)

- **PATRÓN** `libro_liquidez` > `9093.4424` → IC=+0.129 (n=586)

  - _Acción_: Kelly boost +0.65€ cuando `libro_liquidez` > 9093.4424 (IC base=+0.125)
...
- **PATRÓN** `sigma_h` < `0.0052` → IC=+0.162 (n=211)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0052 (IC base=+0.131)

- **PATRÓN** `drift_60min` |x|≤ `0.4858` → IC=+0.145 (n=477)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.4858 (IC base=+0.131)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.176 (n=171)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 18.0 (IC base=+0.131)

- **PATRÓN** `hora_utc` < `4.0` → IC=+0.141 (n=165)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` < 4.0 (IC base=+0.131)

- **PATRÓN** `ibs_20min` > `0.7912` → IC=+0.170 (n=216)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` > 0.7912 (IC base=+0.131)

- **PATRÓN** `dist_vwap_pct` > `0.9899` → IC=+0.173 (n=102)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.9899 (IC base=+0.131)

- **PATRÓN** `dist_vwap_pct` < `0.2409` → IC=+0.137 (n=406)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.2409 (IC base=+0.131)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.012` → IC=+0.142 (n=107)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 10.012 (IC base=+0.131)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.07` → IC=+0.141 (n=480)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` < 7.07 (IC base=+0.131)

- **PATRÓN** `volumen_regimen` < `0.7283` → IC=+0.156 (n=210)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.7283 (IC base=+0.131)

- **PATRÓN** `volumen_pendiente_norm` > `0.1758` → IC=+0.164 (n=144)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.1758 (IC base=+0.131)

- **PATRÓN** `volumen_spike_ratio` < `1.4338` → IC=+0.160 (n=157)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 1.4338 (IC base=+0.131)

- **PATRÓN** `volumen_spike_ratio` > `1.8484` → IC=+0.153 (n=312)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.8484 (IC base=+0.131)

- **PATRÓN** `libro_liquidez` > `8379.1781` → IC=+0.141 (n=477)

  - _Acción_: Kelly boost +0.70€ cuando `libro_liquidez` > 8379.1781 (IC base=+0.131)
...
- **PATRÓN** `dist_vwap_pct` < `0.1959` → IC=+0.167 (n=157)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.1959 (IC base=+0.105)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.153 (n=119)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.1092` → IC=+0.210 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1092 (IC base=+0.141)
...
- **FILTRO** `libro_liquidez` < `2167.8726` → IC=-0.304 (n=49)
...
### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.160 (n=198)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0041 (IC base=+0.122)

- **PATRÓN** `sigma_h` > `0.0064` → IC=+0.158 (n=150)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0064 (IC base=+0.122)

- **PATRÓN** `drift_60min` |x|≤ `0.1656` → IC=+0.126 (n=396)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.63€ cuando `drift_60min` |x|≤ 0.1656 (IC base=+0.122)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0649` → IC=+0.133 (n=450)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.66€ cuando `delta_ratio_macro` |x|> 0.0649 (IC base=+0.122)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.128 (n=479)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 4.0 (IC base=+0.122)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.157 (n=199)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 6.0 (IC base=+0.122)

- **PATRÓN** `ibs_15` > `0.5659` → IC=+0.210 (n=450)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5659 (IC base=+0.122)

- **PATRÓN** `dist_vwap_pct` > `0.4138` → IC=+0.185 (n=106)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.4138 (IC base=+0.122)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.353` → IC=+0.225 (n=176)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.353 (IC base=+0.122)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.130 (n=479)

  - _Acción_: Kelly boost +0.65€ cuando `libro_spread` < 0.01 (IC base=+0.122)

- **PATRÓN** `libro_liquidez` > `8988.4246` → IC=+0.158 (n=150)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 8988.4246 (IC base=+0.122)

- **PATRÓN** `ballena_activa_n` < `33.0` → IC=+0.184 (n=55)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 33.0 (IC base=+0.122)
...
- **FILTRO** `ibs_15` < `0.5728` → IC=-0.257 (n=35)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.179 (n=107)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2488` → IC=+0.158 (n=36)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.79€ cuando `delta_ratio_macro` |x|> 0.2488 (IC base=+0.069)

- **PATRÓN** `ibs_15` > `0.5728` → IC=+0.179 (n=107)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.89€ cuando `ibs_15` > 0.5728 (IC base=+0.069)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.912` → IC=+0.125 (n=46)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 8.912 (IC base=+0.069)
...
- **PATRÓN** `ibs_15` < `0.131` → IC=+0.169 (n=140)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.85€ cuando `ibs_15` < 0.131 (IC base=+0.031)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.973` → IC=-0.156 (n=443)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0846` → IC=+0.234 (n=321)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0846 (IC base=-0.067)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.0943` → IC=+0.250 (n=138)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.0943 (IC base=-0.067)
...
- **PATRÓN** `dist_vwap_pct` < `0.2668` → IC=+0.224 (n=360)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2668 (IC base=-0.067)
...
- **FILTRO** `sigma_h` > `0.0074` → IC=-0.239 (n=205)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.188 (n=617)

- **FILTRO** `sigma_h` < `0.005` → IC=-0.219 (n=411)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.005
  - _Potencial_: sin este filtro IC_bueno=-0.183 (n=411)

- **FILTRO** `hora_utc` > `17.0` → IC=-0.225 (n=176)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 17.0
  - _Potencial_: sin este filtro IC_bueno=-0.194 (n=646)

- **FILTRO** `sigma_ewma_delta_pct` > `19.475` → IC=-0.250 (n=154)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 19.475
  - _Potencial_: sin este filtro IC_bueno=-0.190 (n=668)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.5659 sube el IC de +0.122 a +0.210 en UPDOWN_GBM#15min (n=450). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.5728 sube el IC de +0.069 a +0.179 en UPDOWN_GBM#ETH#15min (n=107). Ya aplicado como kelly_boost=+0.89€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.131 sube el IC de +0.031 a +0.169 en UPDOWN_GBM#XRP#15min (n=140). Ya aplicado como kelly_boost=+0.85€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3519 sube el IC de -0.067 a +0.287 en UPDOWN_GBM_15M_TARDIO (n=359). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ FAVORITO_CONFIRMADO | 43656 | +0.113 | -2824.83€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 7456 | +0.186 | -235.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 136 | -0.101 | -56.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 32478 | +0.096 | -2480.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3586 | +0.118 | -52.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 5459 | +0.076 | -795.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 24 | -0.115 | -1.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 9 | -0.143 | -7.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 5426 | +0.078 | -785.96€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#DOGE | 5469 | +0.081 | -702.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 26 | +0.036 | +0.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#240min | 6 | -0.075 | -4.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 5437 | +0.082 | -698.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 9466 | +0.127 | -128.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2745 | +0.172 | -4.36€ | 1 | 7 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 5404 | +0.111 | -86.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1305 | +0.098 | -28.94€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 9015 | +0.127 | -610.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2479 | +0.195 | -153.79€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 64 | +0.000 | -11.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 5409 | +0.097 | -365.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1063 | +0.132 | -79.41€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 5448 | +0.104 | -386.67€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 19 | -0.023 | -0.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 5426 | +0.105 | -384.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 7414 | +0.176 | -581.35€ | 2 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 7414 | +0.176 | -581.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 1881 | +0.164 | -210.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 1881 | +0.164 | -210.71€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 1802 | +0.182 | -143.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 1802 | +0.182 | -143.73€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 23121 | +0.188 | -2171.93€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 23121 | +0.188 | -2171.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 4137 | +0.140 | -695.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 4137 | +0.140 | -695.47€ | 0 | 2 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 3998 | +0.164 | -532.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 