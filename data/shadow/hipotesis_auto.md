# Hipótesis automáticas — 2026-07-16 10:36 UTC
_Generado por shadow_postmortem.py sobre 16822 resoluciones (PNL=+3438.64€)_
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.185 (n=1069)

  - _Acción_: Kelly boost +0.93€ cuando `libro_spread` < 0.01 (IC base=+0.179)

- **PATRÓN** `libro_liquidez` > `4364.0743` → IC=+0.185 (n=611)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 4364.0743 (IC base=+0.179)
...
- **PATRÓN** `py_entrada` > `0.62` → IC=+0.285 (n=175)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.62 (IC base=+0.219)

- **PATRÓN** `libro_liquidez` > `1606.4463` → IC=+0.217 (n=196)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1606.4463 (IC base=+0.219)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.165 (n=764)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 18.0 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` > `0.1497` → IC=+0.197 (n=285)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1497 (IC base=+0.158)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.179 (n=758)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.4184` → IC=+0.197 (n=539)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` < 0.4184 (IC base=+0.163)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.101` → IC=+0.257 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.101 (IC base=+0.163)
...
- **PATRÓN** `sigma_h` > `0.0127` → IC=+0.223 (n=92)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0127 (IC base=+0.138)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.151 (n=190)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.2328` → IC=+0.181 (n=67)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.2328 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `1.0007` → IC=+0.152 (n=153)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 1.0007 (IC base=+0.138)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.131 (n=600)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` < 15.0 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `0.3431` → IC=+0.171 (n=174)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.3431 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.1391` → IC=+0.142 (n=252)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.1391 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.298` → IC=+0.292 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.298 (IC base=+0.113)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2524 | +0.173 | -67.91€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 1624 | +0.210 | +14.41€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 732 | +0.180 | -15.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 544 | +0.205 | -12.58€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 885 | +0.170 | -37.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 534 | +0.215 | +16.48€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 4857 | +0.102 | +1515.10€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 4857 | +0.102 | +1515.10€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#SOL | 1301 | +0.109 | +524.86€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1301 | +0.109 | +524.86€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1229 | +0.157 | +628.44€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1229 | +0.157 | +628.44€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2081 | +0.149 | +1214.77€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2081 | +0.149 | +1214.77€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 562 | +0.110 | +242.92€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 562 | +0.110 | +242.92€ | 1 | 10 |
...
| ✅ GBM_LATE_15M_TARDIO | 2055 | +0.091 | +618.24€ | 0 | 7 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2055 | +0.091 | +618.24€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#SOL | 484 | +0.062 | +97.97€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 484 | +0.062 | +97.97€ | 1 | 6 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9790 | tras_1loss IC=+0.069 n=6823 | tras_2loss IC=+0.037 n=2913/40 | gap=+0.065 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=14364 IC=+0.106 PNL=+3514.52€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=14364 IC=+0.106 PNL=+3514.52€
