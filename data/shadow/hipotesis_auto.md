# Hipótesis automáticas — 2026-07-02 00:47 UTC
_Generado por shadow_postmortem.py sobre 2498 resoluciones (PNL=-64.64€)_
...
- **PATRÓN** `sigma_h` < `0.0096` → IC=+0.272 (n=55)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0096 (IC base=+0.120)

- **PATRÓN** `drift_60min` |x|≤ `0.7563` → IC=+0.156 (n=94)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.7563 (IC base=+0.120)

- **PATRÓN** `drift_15min` |x|≤ `0.7516` → IC=+0.144 (n=71)

  - _Acción_: Kelly boost +0.72€ cuando `drift_15min` |x|≤ 0.7516 (IC base=+0.120)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2386` → IC=+0.132 (n=36)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.66€ cuando `delta_ratio_macro` |x|> 0.2386 (IC base=+0.120)
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.194 (n=34)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` < 11.0 (IC base=+0.120)

- **PATRÓN** `ibs_15` < `0.0634` → IC=+0.271 (n=33)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0634 (IC base=+0.120)
...
| ✅ UPDOWN_GBM | 747 | -0.026 | -14.11€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 529 | +0.003 | +11.84€ | 1 | 7 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Datos_: n=357 IC=-0.010 PNL=+11.12€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.004 n=1180 | tras_1loss IC=-0.033 n=1236 | tras_2loss IC=-0.047 n=643/40 | gap=+0.051 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=624 IC=-0.018 PNL=-0.56€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=624 IC=-0.018 PNL=-0.56€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=146 IC=+0.095 PNL=+39.70€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=146 IC=+0.095 PNL=+39.70€
...
**⏳ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: 81/200 ops en el filtro definido (IC actual=+0.139 PNL=+28.28€)
  - _Datos_: n=81 IC=+0.139 PNL=+28.28€
...
**⏳ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: 81/100 ops en el filtro definido (IC actual=+0.139 PNL=+28.28€)
  - _Datos_: n=81 IC=+0.139 PNL=+28.28€
