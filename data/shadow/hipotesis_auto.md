# Hipótesis automáticas — 2026-07-15 08:47 UTC
_Generado por shadow_postmortem.py sobre 15050 resoluciones (PNL=+2904.82€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.200 (n=668)
...
- **PATRÓN** `py_entrada` > `0.695` → IC=+0.313 (n=340)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.193)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.198 (n=851)

  - _Acción_: Kelly boost +0.99€ cuando `libro_spread` < 0.01 (IC base=+0.193)

- **PATRÓN** `libro_liquidez` > `4385.46` → IC=+0.198 (n=481)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 4385.46 (IC base=+0.193)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.207 (n=138)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.250 (n=150)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.235)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.241 (n=164)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.235)

- **PATRÓN** `py_entrada` > `0.715` → IC=+0.348 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.715 (IC base=+0.235)

- **PATRÓN** `libro_liquidez` > `5171.5093` → IC=+0.257 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 5171.5093 (IC base=+0.235)
...
- **PATRÓN** `py_entrada` > `0.625` → IC=+0.290 (n=141)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.625 (IC base=+0.224)

- **PATRÓN** `libro_liquidez` > `1644.8282` → IC=+0.228 (n=156)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1644.8282 (IC base=+0.224)
...
- **PATRÓN** `dist_vwap_pct` > `0.7422` → IC=+0.167 (n=31)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.7422 (IC base=+0.063)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.073` → IC=+0.125 (n=46)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 5.073 (IC base=+0.063)
...
- **PATRÓN** `dist_vwap_pct` < `0.5914` → IC=+0.137 (n=144)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.5914 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.147` → IC=+0.127 (n=116)
...
- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.178 (n=663)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.189 (n=233)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` < 5.0 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` > `1.0508` → IC=+0.186 (n=49)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 1.0508 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` < `0.1912` → IC=+0.198 (n=389)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.1912 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.996` → IC=+0.217 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.996 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.13` → IC=+0.174 (n=323)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` < 3.13 (IC base=+0.158)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.133 (n=148)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.0066 (IC base=+0.124)

- **PATRÓN** `sigma_h` > `0.0041` → IC=+0.147 (n=151)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0041 (IC base=+0.124)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.167 (n=85)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 15.0 (IC base=+0.124)
...
- **PATRÓN** `dist_vwap_pct` > `0.7422` → IC=+0.197 (n=31)
  ...
  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.7422 (IC base=+0.124)

- **PATRÓN** `dist_vwap_pct` < `0.4154` → IC=+0.153 (n=93)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.4154 (IC base=+0.124)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.568` → IC=+0.167 (n=37)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 5.568 (IC base=+0.124)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.175` → IC=+0.156 (n=94)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` < 10.175 (IC base=+0.124)
...
- **PATRÓN** `sigma_h` > `0.0127` → IC=+0.216 (n=79)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0127 (IC base=+0.129)
...
- **PATRÓN** `dist_vwap_pct` > `0.4922` → IC=+0.188 (n=46)
  ...
  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.4922 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` < `1.1668` → IC=+0.129 (n=122)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 1.1668 (IC base=+0.129)
...
- **PATRÓN** `sigma_h` > `0.0173` → IC=+0.226 (n=93)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.515` → IC=+0.183 (n=143)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.515 (IC base=+0.193)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.652` → IC=+0.194 (n=109)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2068 | +0.178 | -23.85€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1337 | +0.209 | +7.20€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 603 | +0.181 | -10.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 447 | +0.197 | -22.22€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 733 | +0.180 | +15.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 441 | +0.222 | +25.01€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 726 | +0.176 | -24.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 443 | +0.210 | +8.58€ | 0 | 6 |
...
| ✅ GBM_LATE_15M | 4518 | +0.101 | +1365.63€ | 0 | 3 |
| ✅ GBM_LATE_15M#15min | 4518 | +0.101 | +1365.63€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1083 | +0.066 | +156.97€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1083 | +0.066 | +156.97€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1104 | +0.072 | +179.36€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1104 | +0.072 | +179.36€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1202 | +0.111 | +489.10€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1202 | +0.111 | +489.10€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1129 | +0.151 | +540.20€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1129 | +0.151 | +540.20€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1783 | +0.149 | +1017.39€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1783 | +0.149 | +1017.39€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 455 | +0.121 | +224.81€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 455 | +0.121 | +224.81€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 471 | +0.119 | +204.01€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 471 | +0.119 | +204.01€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 483 | +0.106 | +204.22€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 483 | +0.106 | +204.22€ | 1 | 10 |
...
| ✅ GBM_LATE_15M_TARDIO | 1737 | +0.082 | +471.12€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1737 | +0.082 | +471.12€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 429 | +0.061 | +71.27€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 429 | +0.061 | +71.27€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 386 | +0.008 | +6.74€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 386 | +0.008 | +6.74€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 434 | +0.048 | +71.86€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 434 | +0.048 | +71.86€ | 2 | 5 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 488 | +0.188 | +321.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 488 | +0.188 | +321.25€ | 0 | 10 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.098 n=8665 | tras_1loss IC=+0.059 n=6198 | tras_2loss IC=+0.027 n=2696/40 | gap=+0.070 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=12592 IC=+0.100 PNL=+2980.70€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=12592 IC=+0.100 PNL=+2980.70€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=578 IC=+0.057 PNL=+65.04€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=578 IC=+0.057 PNL=+65.04€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.272 > 0.08 con n=160 PNL=+0.14€
  - _Datos_: n=160 IC=+0.272 PNL=+0.14€
