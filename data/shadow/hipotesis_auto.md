# Hipótesis automáticas — 2026-08-28 20:06 UTC
_Generado por shadow_postmortem.py sobre 192326 resoluciones (PNL=+14319.81€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `147.46` → IC=-0.287 (n=2688)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 147.46
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=8065)
...
- **FILTRO** `restante_s_al_confirmar` < `161.45` → IC=-0.158 (n=723)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.189 (n=5834)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.148 (n=4027)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.198 (n=624)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.365` → IC=+0.260 (n=606)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.365 (IC base=+0.189)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.190 (n=884)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.01 (IC base=+0.189)

- **PATRÓN** `libro_liquidez` > `12879.5491` → IC=+0.230 (n=228)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12879.5491 (IC base=+0.189)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.278 (n=508)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.276 (n=520)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.196 (n=981)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.259 (n=234)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.232 (n=603)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.225)

- **PATRÓN** `libro_liquidez` > `1938.0591` → IC=+0.252 (n=300)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1938.0591 (IC base=+0.225)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.194 (n=4853)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.201 (n=2300)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.194 (n=1189)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.73` → IC=+0.188 (n=1179)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.195 (n=12271)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.218 (n=9791)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.190)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.136 (n=2473)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.240 (n=2133)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.182 (n=893)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.232 (n=1959)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.224 (n=847)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.262` → IC=+0.155 (n=998)
...
- **PATRÓN** `ibs_20min` < `0.1159` → IC=+0.150 (n=1276)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` < 0.1159 (IC base=+0.032)

- **PATRÓN** `dist_vwap_pct` < `0.1358` → IC=+0.148 (n=797)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.1358 (IC base=+0.032)

- **PATRÓN** `volumen_regimen` < `0.6207` → IC=+0.157 (n=278)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.6207 (IC base=+0.032)

- **PATRÓN** `volumen_regimen` > `1.06` → IC=+0.138 (n=376)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` > 1.06 (IC base=+0.032)
...
- **PATRÓN** `volumen_spike_ratio` > `2.9153` → IC=+0.245 (n=335)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.154 (n=457)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.44` → IC=+0.180 (n=426)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` < 0.44 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` < `0.1424` → IC=+0.171 (n=417)
...
- **PATRÓN** `volumen_regimen` < `0.6207` → IC=+0.183 (n=162)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.6207 (IC base=+0.144)

- **PATRÓN** `volumen_pendiente_norm` > `0.0721` → IC=+0.204 (n=160)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0721 (IC base=+0.144)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4117` → IC=+0.170 (n=380)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 1.4117 (IC base=+0.144)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.145 (n=626)

  - _Acción_: Kelly boost +0.72€ cuando `libro_spread` < 0.01 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `13845.8953` → IC=+0.194 (n=220)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 13845.8953 (IC base=+0.144)
...
- **PATRÓN** `volumen_spike_ratio` > `2.4256` → IC=+0.186 (n=33)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 2.4256 (IC base=-0.050)

- **PATRÓN** `ballena_activa_n` < `143.0` → IC=+0.142 (n=79)
...
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.137 (n=1420)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0078 (IC base=+0.054)

- **PATRÓN** `ibs_20min` > `0.9448` → IC=+0.246 (n=1045)
...
- **PATRÓN** `ibs_20min` < `0.0909` → IC=+0.195 (n=1286)
...
- **FILTRO** `ibs_20min` < `0.7451` → IC=-0.136 (n=460)
...
- **PATRÓN** `volumen_regimen` < `0.8566` → IC=+0.189 (n=133)
  ...
  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` < 0.8566 (IC base=-0.016)

- **PATRÓN** `volumen_regimen` > `1.1463` → IC=+0.250 (n=66)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1463 (IC base=-0.016)

- **PATRÓN** `volumen_pendiente_norm` < `0.1202` → IC=+0.174 (n=185)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` < 0.1202 (IC base=-0.016)

- **PATRÓN** `volumen_pendiente_norm` > `0.2649` → IC=+0.286 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2649 (IC base=-0.016)

- **PATRÓN** `volumen_spike_ratio` < `1.4599` → IC=+0.208 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4599 (IC base=-0.016)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.423` → IC=+0.244 (n=1117)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.423 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` > `0.6875` → IC=+0.171 (n=1664)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` > 0.6875 (IC base=+0.157)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2882` → IC=+0.159 (n=2142)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.2882 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` > `1.8523` → IC=+0.151 (n=1622)
...
- **PATRÓN** `volumen_regimen` < `1.1737` → IC=+0.172 (n=1943)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 1.1737 (IC base=+0.187)

- **PATRÓN** `volumen_regimen` > `0.6207` → IC=+0.170 (n=1944)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5799` → IC=+0.179 (n=811)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6682` → IC=+0.211 (n=614)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6682 (IC base=+0.187)
...
- **PATRÓN** `sigma_h` < `0.0059` → IC=+0.173 (n=487)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0059 (IC base=+0.151)

- **PATRÓN** `drift_60min` |x|≤ `0.2337` → IC=+0.175 (n=429)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.2337 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.173 (n=448)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 7.0 (IC base=+0.151)

- **PATRÓN** `ibs_20min` < `0.4268` → IC=+0.195 (n=487)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` < 0.4268 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` < `0.1623` → IC=+0.177 (n=484)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1673` → IC=+0.228 (n=134)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1673 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` < `2.5474` → IC=+0.164 (n=379)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.5474 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` > `1.4105` → IC=+0.151 (n=379)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.4105 (IC base=+0.151)

- **PATRÓN** `libro_liquidez` > `14744.132` → IC=+0.191 (n=163)
...
- **PATRÓN** `ibs_20min` < `0.3128` → IC=+0.145 (n=328)

  - _Acción_: Kelly boost +0.73€ cuando `ibs_20min` < 0.3128 (IC base=+0.058)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.783` → IC=+0.155 (n=111)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 7.783 (IC base=+0.058)
...
- **PATRÓN** `ibs_20min` < `0.1125` → IC=+0.219 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1125 (IC base=+0.054)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0066` → IC=+0.185 (n=2106)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0066 (IC base=+0.150)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.158 (n=3203)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.7851` → IC=+0.231 (n=638)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7851 (IC base=+0.150)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3083` → IC=+0.151 (n=2491)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 2.3083 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.8618` → IC=+0.158 (n=1887)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.8618 (IC base=+0.150)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.155 (n=2520)

  - _Acción_: Kelly boost +0.78€ cuando `libro_spread` < 0.01 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `2484.1984` → IC=+0.174 (n=2100)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 2484.1984 (IC base=+0.150)

- **PATRÓN** `ballena_activa_n` < `142.0` → IC=+0.190 (n=1560)

  - _Acción_: Kelly boost +0.95€ cuando `ballena_activa_n` < 142.0 (IC base=+0.150)

- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.218 (n=1927)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0067 (IC base=+0.191)

- **PATRÓN** `drift_60min` |x|≤ `0.4859` → IC=+0.202 (n=2884)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4859 (IC base=+0.191)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.196 (n=1065)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5625` → IC=+0.247 (n=2884)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5625 (IC base=+0.191)

- **PATRÓN** `dist_vwap_pct` < `0.4325` → IC=+0.182 (n=2136)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.4325 (IC base=+0.191)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.426` → IC=+0.207 (n=421)
...
- **PATRÓN** `volumen_regimen` < `0.6199` → IC=+0.185 (n=713)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5117` → IC=+0.165 (n=434)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.5117 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.154 (n=151)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.295` → IC=+0.218 (n=289)
...
- **PATRÓN** `dist_vwap_pct` < `0.3958` → IC=+0.151 (n=433)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.3958 (IC base=+0.147)
...
- **PATRÓN** `volumen_regimen` < `0.5836` → IC=+0.160 (n=145)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.5836 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` > `0.6471` → IC=+0.157 (n=386)
...
- **PATRÓN** `dist_vwap_pct` > `0.904` → IC=+0.232 (n=151)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.904 (IC base=+0.091)
...
- **PATRÓN** `ballena_activa_n` < `66.0` → IC=+0.154 (n=333)
...
- **PATRÓN** `drift_60min` |x|≤ `0.496` → IC=+0.227 (n=624)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.496 (IC base=+0.218)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.223 (n=663)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.145 (n=1236)

- **PATRÓN** `ibs_20min` > `0.2455` → IC=+0.123 (n=823)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` > 0.2455 (IC base=+0.102)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3873` → IC=+0.166 (n=826)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.3873 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.154 (n=351)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.1923` → IC=+0.164 (n=414)

  - _Acción_: Kelly boost +0.82€ cuando `ibs_20min` < 0.1923 (IC base=+0.140)

- **PATRÓN** `ibs_20min` > `0.0889` → IC=+0.142 (n=939)
...
- **PATRÓN** `volumen_regimen` < `1.2164` → IC=+0.151 (n=915)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.2164 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` < `0.0933` → IC=+0.150 (n=842)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_pendiente_norm` < 0.0933 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` < `2.5395` → IC=+0.158 (n=931)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.5395 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.423` → IC=+0.147 (n=930)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.423 (IC base=+0.140)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.145 (n=1236)

  - _Acción_: Kelly boost +0.72€ cuando `libro_spread` < 0.01 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `10638.1798` → IC=+0.150 (n=626)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4002` → IC=+0.152 (n=515)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.4002 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.190 (n=195)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.1703` → IC=+0.172 (n=227)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.1703 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.6734` → IC=+0.194 (n=60)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.6734 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.265` → IC=+0.163 (n=514)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` < 6.265 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `1.1847` → IC=+0.158 (n=515)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.1847 (IC base=+0.142)

- **PATRÓN** `volumen_pendiente_norm` > `0.0692` → IC=+0.167 (n=244)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.0692 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` < `2.5395` → IC=+0.156 (n=513)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.5395 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` > `1.8045` → IC=+0.151 (n=342)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.8045 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `11991.4883` → IC=+0.151 (n=460)
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.208 (n=63)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.267` → IC=+0.260 (n=23)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.267 (IC base=+0.118)
...
- **PATRÓN** `volumen_spike_ratio` > `2.4093` → IC=+0.145 (n=105)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 2.4093 (IC base=+0.118)

- **PATRÓN** `libro_liquidez` > `1867.3696` → IC=+0.150 (n=158)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 1867.3696 (IC base=+0.118)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.175 (n=933)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=2834)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.209 (n=954)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.010 (n=2905)

- **FILTRO** `ibs_20min` > `0.2692` → IC=-0.175 (n=962)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2692
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=2897)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.143 (n=2723)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=6805)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.283 (n=2336)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=7192)

- **FILTRO** `ibs_7min` < `0.7294` → IC=-0.236 (n=2382)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7294
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=7146)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.174 (n=3222)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=6306)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.222 (n=2886)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=8745)

- **FILTRO** `ibs_7min` > `0.7152` → IC=-0.173 (n=2907)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7152
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=8724)
...
- **FILTRO** `py_entrada` > `0.71` → IC=-0.229 (n=477)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=1526)

- **FILTRO** `drift_7min_pct` |x|> `0.1335` → IC=-0.138 (n=681)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1335
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=1322)

- **FILTRO** `ibs_7min` > `0.7021` → IC=-0.171 (n=681)
...
- **FILTRO** `py_entrada` > `0.61` → IC=-0.214 (n=445)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=1396)

- **FILTRO** `ballena_activa_n` > `98.0` → IC=-0.162 (n=622)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.202 (n=333)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.099 (n=1045)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.312 (n=445)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=933)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.252 (n=438)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=1605)

- **FILTRO** `ibs_7min` > `0.812` → IC=-0.190 (n=510)

  - _Acción_: SKIP cuando `ibs_7min` > 0.812
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=1533)
...
- **FILTRO** `hora_utc` > `15.0` → IC=-0.120 (n=525)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=1078)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.267 (n=384)
...
- **FILTRO** `ballena_activa_n` > `25.0` → IC=-0.172 (n=544)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=1059)
...
- **PATRÓN** `delta_ratio` |x|> `0.3987` → IC=+0.137 (n=345)
  ...
  - _Acción_: Kelly boost +0.68€ cuando `delta_ratio` |x|> 0.3987 (IC base=+0.119)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.133 (n=186)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 11.0 (IC base=+0.119)

- **PATRÓN** `total_vol_5m` < `459.6089` → IC=+0.173 (n=108)

  - _Acción_: Kelly boost +0.86€ cuando `total_vol_5m` < 459.6089 (IC base=+0.119)

- **PATRÓN** `libro_liquidez` > `3204.915` → IC=+0.135 (n=124)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 3204.915 (IC base=+0.119)
...
- **PATRÓN** `delta_ratio` |x|> `0.42` → IC=+0.139 (n=34)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.69€ cuando `delta_ratio` |x|> 0.42 (IC base=+0.114)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.133 (n=47)

  - _Acción_: Kelly boost +0.66€ cuando `libro_spread` < 0.03 (IC base=+0.114)

- **PATRÓN** `libro_liquidez` > `2025.6726` → IC=+0.160 (n=51)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 2025.6726 (IC base=+0.114)
...
- **PATRÓN** `streak_estiramiento` < `0.2837` → IC=+0.167 (n=22)

  - _Acción_: Kelly boost +0.83€ cuando `streak_estiramiento` < 0.2837 (IC base=+0.010)
...
- **FILTRO** `sigma_ewma_delta_pct` > `6.578` → IC=-0.209 (n=53)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=488)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1043` → IC=+0.260 (n=77)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +1.00€ cuando `pct_spot_vs_ref` |x|≤ 0.1043 (IC base=+0.240)

- **PATRÓN** `sigma_h` < `0.0077` → IC=+0.261 (n=115)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0077 (IC base=+0.240)

- **PATRÓN** `sigma_h` > `0.0063` → IC=+0.256 (n=76)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0063 (IC base=+0.240)

- **PATRÓN** `drift_15min` |x|≤ `0.4277` → IC=+0.281 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.4277 (IC base=+0.240)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1016` → IC=+0.260 (n=102)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1016 (IC base=+0.240)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3069` → IC=+0.256 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3069 (IC base=+0.240)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.250 (n=86)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` < `0.3764` → IC=+0.329 (n=115)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3764 (IC base=+0.240)

- **PATRÓN** `dist_vwap_pct` < `0.1254` → IC=+0.252 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1254 (IC base=+0.240)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.326` → IC=+0.270 (n=124)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 11.326 (IC base=+0.240)

- **PATRÓN** `libro_liquidez` > `13838.1297` → IC=+0.275 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13838.1297 (IC base=+0.240)

- **PATRÓN** `ballena_activa_n` < `206.0` → IC=+0.247 (n=85)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 206.0 (IC base=+0.240)
...
- **FILTRO** `sigma_ewma_delta_pct` > `16.036` → IC=-0.145 (n=119)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=958)
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3676 sube el IC de -0.075 a +0.332 en UPDOWN_GBM_15M_TARDIO (n=200). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.3764 sube el IC de +0.240 a +0.329 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=115). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 10753 | -0.096 | -1778.42€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 761 | -0.058 | -134.21€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 9992 | -0.099 | -1644.21€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#DOGE | 1384 | -0.150 | -430.83€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 1384 | -0.150 | -430.83€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 2892 | -0.065 | -283.01€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 2892 | -0.065 | -283.01€ | 1 | 0 |
...
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 649 | -0.059 | +293.02€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 186 | -0.005 | +59.13€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 463 | -0.081 | +233.89€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 649 | -0.059 | +293.02€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 186 | -0.005 | +59.13€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 463 | -0.081 | +233.89€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 35167 | +0.114 | -2218.15€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 6356 | +0.186 | -226.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 113 | -0.083 | -46.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 25403 | +0.097 | -1894.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3295 | +0.119 | -50.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 4261 | +0.067 | -689.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 21 | -0.065 | +2.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 4235 | +0.069 | -685.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 7208 | +0.133 | -161.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1802 | +0.197 | -94.02€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 4227 | +0.109 | -98.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1137 | +0.128 | +52.88€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 4272 | +0.082 | -513.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 22 | +0.083 | +3.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 4249 | +0.083 | -514.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 7735 | +0.126 | -129.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2309 | +0.166 | -20.42€ | 0 | 8 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 4230 | +0.112 | -73.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1184 | +0.100 | -27.29€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 7432 | +0.133 | -451.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2185 | +0.201 | -117.37€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 51 | +0.028 | -6.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 4222 | +0.099 | -251.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 974 | +0.132 | -76.32€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#XRP | 