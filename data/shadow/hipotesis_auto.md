# Hipótesis automáticas — 2026-07-17 03:21 UTC
_Generado por shadow_postmortem.py sobre 18025 resoluciones (PNL=+3803.36€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.192 (n=1034)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 18.0 (IC base=+0.177)

- **PATRÓN** `py_entrada` > `0.695` → IC=+0.293 (n=463)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.177)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.182 (n=1184)

  - _Acción_: Kelly boost +0.91€ cuando `libro_spread` < 0.01 (IC base=+0.177)

- **PATRÓN** `libro_liquidez` > `4385.46` → IC=+0.184 (n=676)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 4385.46 (IC base=+0.177)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.245 (n=104)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.194)

- **PATRÓN** `py_entrada` > `0.615` → IC=+0.247 (n=200)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.228 (n=226)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.615` → IC=+0.271 (n=208)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.615 (IC base=+0.225)

- **PATRÓN** `libro_liquidez` > `1592.1422` → IC=+0.229 (n=219)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1592.1422 (IC base=+0.225)
...
- **PATRÓN** `dist_vwap_pct` > `0.8734` → IC=+0.131 (n=101)
  ...
  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.8734 (IC base=+0.108)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.274` → IC=+0.207 (n=145)
...
- **PATRÓN** `sigma_h` > `0.0239` → IC=+0.176 (n=214)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0239 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.150 (n=178)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 17.0 (IC base=+0.147)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.155 (n=424)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` < 15.0 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.6767` → IC=+0.227 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6767 (IC base=+0.147)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.169 (n=360)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 8.0 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` > `0.3769` → IC=+0.217 (n=178)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3769 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.007` → IC=+0.309 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.007 (IC base=+0.158)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.147 (n=222)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 18.0 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` > `0.3457` → IC=+0.191 (n=66)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.3457 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `1.1151` → IC=+0.126 (n=161)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` < 1.1151 (IC base=+0.133)
...
- **PATRÓN** `sigma_h` < `0.0093` → IC=+0.154 (n=76)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0093 (IC base=+0.140)

- **PATRÓN** `sigma_h` > `0.0125` → IC=+0.214 (n=103)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `16.0` → IC=+0.141 (n=204)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.9819` → IC=+0.150 (n=184)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.9819 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.057` → IC=+0.290 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.057 (IC base=+0.140)
...
- **PATRÓN** `sigma_h` > `0.0151` → IC=+0.194 (n=253)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0151 (IC base=+0.114)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.128 (n=364)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 8.0 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` > `0.6294` → IC=+0.169 (n=122)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.6294 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` < `0.1427` → IC=+0.141 (n=338)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.1427 (IC base=+0.114)
...
- **PATRÓN** `sigma_h` < `0.0047` → IC=+0.136 (n=86)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0047 (IC base=+0.053)
...
- **PATRÓN** `sigma_h` < `0.033` → IC=+0.207 (n=227)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.033 (IC base=+0.204)

- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.222 (n=228)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_15` > `0.7262` → IC=+0.176 (n=270)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.88€ cuando `ibs_15` > 0.7262 (IC base=+0.026)

- **PATRÓN** `dist_vwap_pct` > `0.7268` → IC=+0.210 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7268 (IC base=+0.026)

- **PATRÓN** `dist_vwap_pct` < `0.5084` → IC=+0.137 (n=177)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2407` → IC=+0.121 (n=196)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.61€ cuando `drift_60min` |x|≤ 0.2407 (IC base=+0.069)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.137 (n=166)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 11.0 (IC base=+0.069)

- **PATRÓN** `ibs_15` > `0.6354` → IC=+0.185 (n=157)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.93€ cuando `ibs_15` > 0.6354 (IC base=+0.069)

- **PATRÓN** `dist_vwap_pct` > `0.7268` → IC=+0.309 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7268 (IC base=+0.069)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.945` → IC=+0.143 (n=26)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 3.945 (IC base=+0.069)

- **PATRÓN** `sigma_ewma_delta_pct` < `12.026` → IC=+0.125 (n=46)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 12.026 (IC base=+0.069)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS > 0.7262 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.176 n=270). Confirma señal de reversión media → alinear con BUY_NO.
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS > 0.6354 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.185 n=157). Confirma señal de reversión media → alinear con BUY_NO.
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2812 | +0.174 | -58.14€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 1815 | +0.208 | +14.73€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 814 | +0.181 | -7.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 605 | +0.203 | -13.63€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 1002 | +0.170 | -15.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 601 | +0.205 | +6.13€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 986 | +0.173 | -33.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 599 | +0.217 | +23.67€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 62 | +0.312 | +2.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 62 | +0.312 | +2.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 62 | +0.312 | +2.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 62 | +0.312 | +2.26€ | 0 | 0 |
| ✅ GBM_LATE_15M | 5094 | +0.102 | +1587.18€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 5094 | +0.102 | +1587.18€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1227 | +0.067 | +184.38€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1227 | +0.067 | +184.38€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1206 | +0.074 | +200.49€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1206 | +0.074 | +200.49€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1364 | +0.107 | +544.90€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1364 | +0.107 | +544.90€ | 1 | 7 |
| ✅ GBM_LATE_15M#XRP | 1297 | +0.155 | +657.42€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1297 | +0.155 | +657.42€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2301 | +0.150 | +1329.29€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2301 | +0.150 | +1329.29€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 592 | +0.109 | +284.77€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 592 | +0.109 | +284.77€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 608 | +0.118 | +260.02€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 608 | +0.118 | +260.02€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 611 | +0.118 | +270.95€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 611 | +0.118 | +270.95€ | 1 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 490 | +0.276 | +513.54€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 490 | +0.276 | +513.54€ | 0 | 11 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 119 | +0.145 | +59.15€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 119 | +0.145 | +59.15€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 89 | +0.236 | +61.46€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 89 | +0.236 | +61.46€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 2279 | +0.095 | +714.67€ | 0 | 7 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2279 | +0.095 | +714.67€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 570 | +0.063 | +106.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 570 | +0.063 | +106.84€ | 0 | 6 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 536 | +0.022 | +19.12€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 536 | +0.022 | +19.12€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 518 | +0.065 | +108.02€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 518 | +0.065 | +108.02€ | 2 | 5 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 655 | +0.203 | +480.69€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 655 | +0.203 | +480.69€ | 0 | 11 |
...
| ✅ UPDOWN_GBM | 1633 | +0.009 | +72.13€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1374 | +0.036 | +109.67€ | 1 | 6 |
...
| ✅ UPDOWN_GBM#BTC | 423 | +0.022 | +25.79€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 341 | +0.060 | +39.25€ | 1 | 8 |
...
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD | 24 | +0.269 | +2.63€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#15min | 24 | +0.269 | +2.63€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC | 9 | +0.061 | -0.37€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min | 9 | +0.061 | -0.37€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH | 15 | +0.243 | +3.00€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min | 15 | +0.243 | +3.00€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO | 200 | +0.163 | +58.66€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 200 | +0.163 | +58.66€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 7 | -0.019 | +2.23€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 7 | -0.019 | +2.23€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 28 | +0.033 | +2.93€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 28 | +0.033 | +2.93€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 66 | +0.176 | +11.95€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 66 | +0.176 | +11.95€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.156: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.008 n=543 | neutral: IC=-0.014 n=401 | overbought(IBS>0.7): IC=+0.142 n=445
  - _Datos_: n=1467 IC=+0.049 PNL=+158.65€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.106 n=10560 | tras_1loss IC=+0.072 n=7260 | tras_2loss IC=+0.042 n=3074/40 | gap=+0.064 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1506 IC=+0.016 PNL=+86.13€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1506 IC=+0.016 PNL=+86.13€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=715 IC=+0.026 PNL=+44.67€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=715 IC=+0.026 PNL=+44.67€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=341 IC=+0.060 PNL=+39.25€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=341 IC=+0.060 PNL=+39.25€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=728 IC=+0.073 PNL=+91.63€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=728 IC=+0.073 PNL=+91.63€
...
**⏳ H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  ...
  - _Estado_: 9/40 ops en el filtro definido (IC actual=+0.061 PNL=+0.30€)
  - _Datos_: n=9 IC=+0.061 PNL=+0.30€
...
**🟡 H-CUSTOM-CROSS-WINDOW-SPREAD-POS** — GBM 15min/60min: spread positivo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.201 > 0.08 con n=85 PNL=+8.03€
  - _Datos_: n=85 IC=+0.201 PNL=+8.03€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=15567 IC=+0.109 PNL=+3879.24€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=15567 IC=+0.109 PNL=+3879.24€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=864 IC=-0.005 PNL=+14.63€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=864 IC=-0.005 PNL=+14.63€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.154 > 0.1 con n=426 PNL=+135.44€
  - _Datos_: n=426 IC=+0.154 PNL=+135.44€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.139 > 0.08 con n=311 PNL=+90.91€
  - _Datos_: n=311 IC=+0.139 PNL=+90.91€
...
**🟡 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.188 > 0.08 con n=142 PNL=+57.13€
  - _Datos_: n=142 IC=+0.188 PNL=+57.13€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.241 > 0.2 con n=395 PNL=+267.49€
  - _Datos_: n=395 IC=+0.241 PNL=+267.49€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.130 > 0.08 con n=187 PNL=+72.16€
  - _Datos_: n=187 IC=+0.130 PNL=+72.16€
