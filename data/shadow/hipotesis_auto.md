# Hipótesis automáticas — 2026-07-16 03:33 UTC
_Generado por shadow_postmortem.py sobre 16381 resoluciones (PNL=+3350.32€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.219 (n=421)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.185)

- **PATRÓN** `py_entrada` > `0.695` → IC=+0.292 (n=411)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.185)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.189 (n=1022)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.01 (IC base=+0.185)

- **PATRÓN** `libro_liquidez` > `4397.2353` → IC=+0.188 (n=581)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 4397.2353 (IC base=+0.185)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.197 (n=368)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 17.0 (IC base=+0.161)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.190 (n=343)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` < 6.0 (IC base=+0.161)

- **PATRÓN** `py_entrada` < `0.265` → IC=+0.324 (n=321)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.265 (IC base=+0.161)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.170 (n=1068)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.225 (n=89)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.201)

- **PATRÓN** `py_entrada` > `0.615` → IC=+0.256 (n=174)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.235 (n=194)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.222 (n=88)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
### GBM_LATE_15M
- **PATRÓN** `dist_vwap_pct` > `0.6132` → IC=+0.125 (n=126)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` > 0.6132 (IC base=+0.109)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.225` → IC=+0.226 (n=93)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.225 (IC base=+0.109)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.134 (n=1216)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` > 0.0091 (IC base=+0.105)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.129 (n=1317)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 12.0 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.23` → IC=+0.124 (n=131)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 6.23 (IC base=+0.105)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `dist_vwap_pct` > `0.2776` → IC=+0.155 (n=27)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.2776 (IC base=+0.077)

- **PATRÓN** `sigma_ewma_delta_pct` > `15.331` → IC=+0.222 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 15.331 (IC base=+0.077)
...
- **PATRÓN** `sigma_h` < `0.0108` → IC=+0.151 (n=207)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0108 (IC base=+0.112)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.515` → IC=+0.235 (n=47)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.515 (IC base=+0.112)
...
- **PATRÓN** `sigma_h` < `0.0163` → IC=+0.162 (n=309)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0163 (IC base=+0.162)

- **PATRÓN** `sigma_h` > `0.0269` → IC=+0.199 (n=154)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0269 (IC base=+0.162)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.237 (n=169)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` > `0.3336` → IC=+0.123 (n=51)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` > 0.3336 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` < `0.5277` → IC=+0.134 (n=170)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.5277 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.722` → IC=+0.126 (n=153)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 7.722 (IC base=+0.162)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.220 (n=323)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.169 (n=736)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 18.0 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` > `0.1528` → IC=+0.197 (n=275)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1528 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.023` → IC=+0.307 (n=86)
...
- **PATRÓN** `sigma_h` > `0.0027` → IC=+0.148 (n=174)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0027 (IC base=+0.118)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.161 (n=125)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 12.0 (IC base=+0.118)

- **PATRÓN** `dist_vwap_pct` > `0.5034` → IC=+0.206 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5034 (IC base=+0.118)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.074` → IC=+0.224 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.074 (IC base=+0.118)
...
- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.242 (n=87)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0069 (IC base=+0.146)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.159 (n=136)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 12.0 (IC base=+0.146)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.168 (n=200)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 18.0 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `0.3418` → IC=+0.181 (n=67)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.3418 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` < `1.1435` → IC=+0.140 (n=134)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 1.1435 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.551` → IC=+0.400 (n=18)
...
- **PATRÓN** `sigma_h` > `0.013` → IC=+0.211 (n=88)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.013 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.158 (n=71)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 18.0 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.4285` → IC=+0.186 (n=49)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.4285 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` < `1.0625` → IC=+0.153 (n=145)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 1.0625 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.236` → IC=+0.281 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.236 (IC base=+0.142)
...
- **PATRÓN** `sigma_h` > `0.0242` → IC=+0.231 (n=76)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0242 (IC base=+0.192)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.250 (n=78)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.192)

- **PATRÓN** `dist_vwap_pct` > `0.3121` → IC=+0.180 (n=48)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.3121 (IC base=+0.192)

- **PATRÓN** `dist_vwap_pct` < `0.5017` → IC=+0.178 (n=169)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.5017 (IC base=+0.192)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.549` → IC=+0.192 (n=131)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` < 2.549 (IC base=+0.192)
...
- **LIVE-CANDIDATA**: `GBM_LATE_15M_PYCONFIRMADO#XRP#15min` — IC=+0.300 n=33. Faltan ~7 resoluciones para umbral n≥40. ETA: ~5h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M_PYCONFIRMADO#XRP` — IC=+0.300 n=33. Faltan ~7 resoluciones para umbral n≥40. ETA: ~5h.
...
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#SOL#15min` — IC=+0.186 n=33. Faltan ~7 resoluciones para umbral n≥40. ETA: ~5h.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#SOL` — IC=+0.186 n=33. Faltan ~7 resoluciones para umbral n≥40. ETA: ~5h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2408 | +0.173 | -63.19€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 1550 | +0.209 | +8.01€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 697 | +0.181 | -12.91€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 518 | +0.204 | -14.40€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 857 | +0.172 | -6.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 514 | +0.215 | +17.34€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 846 | +0.168 | -40.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 510 | +0.209 | +8.44€ | 0 | 5 |
...
| ✅ GBM_LATE_15M | 4770 | +0.103 | +1494.09€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 4770 | +0.103 | +1494.09€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1143 | +0.065 | +163.53€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1143 | +0.065 | +163.53€ | 0 | 3 |
| ✅ GBM_LATE_15M#ETH | 1150 | +0.077 | +204.52€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1150 | +0.077 | +204.52€ | 1 | 6 |
| ✅ GBM_LATE_15M#SOL | 1274 | +0.111 | +524.40€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1274 | +0.111 | +524.40€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1203 | +0.155 | +601.63€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1203 | +0.155 | +601.63€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2009 | +0.151 | +1182.08€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2009 | +0.151 | +1182.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 512 | +0.111 | +246.82€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 512 | +0.111 | +246.82€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 533 | +0.132 | +260.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 533 | +0.132 | +260.08€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 542 | +0.112 | +240.59€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 542 | +0.112 | +240.59€ | 1 | 9 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO | 61 | +0.087 | +19.26€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 61 | +0.087 | +19.26€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 33 | +0.300 | +21.91€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 33 | +0.300 | +21.91€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 1972 | +0.091 | +595.31€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1972 | +0.091 | +595.31€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 487 | +0.058 | +89.01€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 487 | +0.058 | +89.01€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 453 | +0.025 | +18.20€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 453 | +0.025 | +18.20€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 471 | +0.060 | +95.54€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 471 | +0.060 | +95.54€ | 1 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 561 | +0.196 | +392.56€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 561 | +0.196 | +392.56€ | 0 | 11 |
...
| ✅ ORDER_FLOW_5M | 1610 | +0.011 | +11.25€ | 1 | 0 |
| ✅ ORDER_FLOW_5M#5min | 1474 | +0.007 | -1.35€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 1584 | +0.009 | +72.91€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1326 | +0.036 | +109.95€ | 1 | 7 |
...
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD | 12 | +0.171 | +0.79€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#15min | 12 | +0.171 | +0.79€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH | 8 | +0.120 | +0.77€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min | 8 | +0.120 | +0.77€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO | 98 | +0.220 | +33.17€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 98 | +0.220 | +33.17€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 5 | -0.018 | +2.20€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 5 | -0.018 | +2.20€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 13 | +0.108 | +3.90€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 13 | +0.108 | +3.90€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 27 | +0.259 | +7.78€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 27 | +0.259 | +7.78€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 33 | +0.186 | +12.20€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 33 | +0.186 | +12.20€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.150) — sin ventaja clara. oversold(IBS<0.3): IC=+0.009 n=477 | neutral: IC=-0.019 n=370 | overbought(IBS>0.7): IC=+0.131 n=388
  - _Datos_: n=1304 IC=+0.044 PNL=+132.10€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9513 | tras_1loss IC=+0.067 n=6656 | tras_2loss IC=+0.035 n=2849/40 | gap=+0.068 (umbral 0.05)
...
**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  ...
  - _Estado_: n_overlaps=34, boost estimado=-0.041. Necesita 0 más y boost>0.05
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1457 IC=+0.016 PNL=+86.91€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1457 IC=+0.016 PNL=+86.91€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=699 IC=+0.026 PNL=+44.55€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=699 IC=+0.026 PNL=+44.55€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=327 IC=+0.065 PNL=+40.43€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=327 IC=+0.065 PNL=+40.43€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=701 IC=+0.075 PNL=+92.50€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=701 IC=+0.075 PNL=+92.50€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=13923 IC=+0.105 PNL=+3426.21€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=13923 IC=+0.105 PNL=+3426.21€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=847 IC=-0.004 PNL=+15.02€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=847 IC=-0.004 PNL=+15.02€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.162 > 0.1 con n=403 PNL=+137.07€
  - _Datos_: n=403 IC=+0.162 PNL=+137.07€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.146 > 0.08 con n=295 PNL=+90.80€
  - _Datos_: n=295 IC=+0.146 PNL=+90.80€
...
**🔶 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA en BTC (IC=+0.212 n=130) pero sin cruzar ≥2 pares más — BNB: n=18 IC=+0.000; DOGE: n=4 IC=+0.067; ETH: n=129 IC=+0.103 ✓; SOL: n=7 IC=-0.019; XRP: n=7 IC=+0.019
  - _Datos_: n=130 IC=+0.212 PNL=+58.61€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.251 > 0.2 con n=339 PNL=+232.57€
  - _Datos_: n=339 IC=+0.251 PNL=+232.57€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.119 > 0.08 con n=137 PNL=+55.33€
  - _Datos_: n=137 IC=+0.119 PNL=+55.33€
...
**🔶 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.153 n=188) pero sin cruzar ≥2 pares más — BTC: n=139 IC=-0.004; ETH: n=106 IC=+0.000; SOL: n=200 IC=+0.059 ✓
  - _Datos_: n=188 IC=+0.153 PNL=+109.55€
