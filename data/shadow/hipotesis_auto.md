# Hipótesis automáticas — 2026-09-25 01:33 UTC
_Generado por shadow_postmortem.py sobre 597323 resoluciones (PNL=+67296.68€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.02` → IC=-0.232 (n=7076)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.02
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=21241)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `138.32` → IC=-0.249 (n=927)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 138.32
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=2782)

### BALLENAS_TARDIAS#BTC#15min
- **FILTRO** `restante_s_al_confirmar` < `500.19` → IC=-0.148 (n=365)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 500.19
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=1096)
...
- **FILTRO** `restante_s_al_confirmar` < `165.09` → IC=-0.221 (n=1708)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 165.09
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=5125)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `126.29` → IC=-0.355 (n=1392)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 126.29
  - _Potencial_: sin este filtro IC_bueno=-0.119 (n=4177)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.206 (n=1688)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.205)

- **PATRÓN** `py_entrada` > `0.745` → IC=+0.352 (n=782)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.745 (IC base=+0.205)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.206 (n=2128)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.205)

- **PATRÓN** `libro_liquidez` > `16019.7856` → IC=+0.240 (n=549)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.146 (n=2398)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 15.0 (IC base=+0.145)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.337 (n=963)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.235 (n=1487)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.141 (n=661)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.153 (n=542)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.01 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `1311.0387` → IC=+0.151 (n=658)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.162 (n=583)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 7.0 (IC base=+0.159)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.200 (n=10994)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.197)

- **PATRÓN** `py_entrada` > `0.74` → IC=+0.226 (n=3805)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.174 (n=2644)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` < 17.0 (IC base=+0.167)

- **PATRÓN** `py_entrada` < `0.71` → IC=+0.182 (n=1946)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.246 (n=844)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.321 (n=863)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.194 (n=2559)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` < 17.0 (IC base=+0.192)

- **PATRÓN** `py_entrada` < `0.71` → IC=+0.196 (n=1972)
...
- **PATRÓN** `py_entrada` > `0.915` → IC=+0.450 (n=219)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.236 (n=15277)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.180 (n=4729)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` < 12.0 (IC base=+0.176)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.190 (n=6385)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.224 (n=6112)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.222)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.264 (n=3496)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.189 (n=6219)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.266 (n=2111)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.256 (n=2282)
...
- **PATRÓN** `libro_liquidez` > `4103.1582` → IC=+0.309 (n=369)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4103.1582 (IC base=+0.291)
...
- **PATRÓN** `libro_liquidez` > `4262.2927` → IC=+0.298 (n=329)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.301 (n=555)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `1462.5909` → IC=+0.311 (n=475)
...
- **PATRÓN** `dist_vwap_pct` < `0.6369` → IC=+0.249 (n=2281)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6369 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.343` → IC=+0.185 (n=2141)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 8.343 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` < `1.2089` → IC=+0.247 (n=2191)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2089 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` > `1.045` → IC=+0.257 (n=994)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4631` → IC=+0.202 (n=5548)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4631 (IC base=+0.106)

- **PATRÓN** `ibs_20min` < `0.57` → IC=+0.135 (n=9756)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3505` → IC=+0.165 (n=1827)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.0549` → IC=+0.295 (n=608)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.448` → IC=+0.248 (n=1433)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.448 (IC base=+0.236)

- **PATRÓN** `volumen_pendiente_norm` < `0.0922` → IC=+0.232 (n=1186)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0922 (IC base=+0.236)

- **PATRÓN** `volumen_pendiente_norm` > `0.2774` → IC=+0.262 (n=183)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2774 (IC base=+0.236)

- **PATRÓN** `volumen_spike_ratio` > `2.6178` → IC=+0.244 (n=420)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6178 (IC base=+0.236)

- **PATRÓN** `libro_liquidez` > `1806.1161` → IC=+0.236 (n=920)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1806.1161 (IC base=+0.236)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `sigma_h` < `0.004` → IC=+0.229 (n=934)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.232 (n=1395)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.216)

- **PATRÓN** `ibs_20min` > `0.9042` → IC=+0.263 (n=634)
...
- **PATRÓN** `dist_vwap_pct` < `0.5797` → IC=+0.220 (n=1462)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5797 (IC base=+0.216)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.762` → IC=+0.259 (n=239)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.762 (IC base=+0.216)

- **PATRÓN** `volumen_regimen` < `1.2525` → IC=+0.219 (n=1396)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2525 (IC base=+0.216)

- **PATRÓN** `volumen_regimen` > `1.0803` → IC=+0.224 (n=633)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0803 (IC base=+0.216)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7485` → IC=+0.217 (n=912)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7485 (IC base=+0.216)

- **PATRÓN** `volumen_spike_ratio` > `2.368` → IC=+0.231 (n=456)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.368 (IC base=+0.216)

- **PATRÓN** `libro_liquidez` > `16049.3542` → IC=+0.223 (n=633)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.140 (n=648)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` < 7.0 (IC base=+0.139)

- **PATRÓN** `ibs_20min` < `0.3271` → IC=+0.197 (n=969)

  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` < 0.3271 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` < `0.1331` → IC=+0.156 (n=1290)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.1331 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.254` → IC=+0.160 (n=233)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 11.254 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.274` → IC=+0.143 (n=1322)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` < 4.274 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` < `1.2044` → IC=+0.150 (n=1453)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.2044 (IC base=+0.139)

- **PATRÓN** `volumen_pendiente_norm` > `0.1566` → IC=+0.177 (n=385)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.1566 (IC base=+0.139)

- **PATRÓN** `volumen_spike_ratio` < `2.4267` → IC=+0.152 (n=1342)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 2.4267 (IC base=+0.139)

- **PATRÓN** `volumen_spike_ratio` > `1.4164` → IC=+0.148 (n=1342)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.4164 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `14091.9548` → IC=+0.144 (n=968)

  - _Acción_: Kelly boost +0.72€ cuando `libro_liquidez` > 14091.9548 (IC base=+0.139)

- **PATRÓN** `ballena_activa_n` < `410.0` → IC=+0.148 (n=1260)

  - _Acción_: Kelly boost +0.74€ cuando `ballena_activa_n` < 410.0 (IC base=+0.139)

### GBM_LATE_15M#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0116` → IC=+0.213 (n=598)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.192 (n=1601)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 15.0 (IC base=+0.186)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.265 (n=710)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.186)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.193` → IC=+0.251 (n=379)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.193 (IC base=+0.186)

- **PATRÓN** `volumen_pendiente_norm` < `0.2104` → IC=+0.188 (n=1798)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` < 0.2104 (IC base=+0.186)

- **PATRÓN** `volumen_pendiente_norm` > `0.3585` → IC=+0.199 (n=237)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.3585 (IC base=+0.186)

- **PATRÓN** `volumen_spike_ratio` > `2.833` → IC=+0.205 (n=774)
...
- **PATRÓN** `sigma_h` < `0.0116` → IC=+0.220 (n=1543)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0116 (IC base=+0.211)

- **PATRÓN** `drift_60min` |x|≤ `0.6053` → IC=+0.214 (n=1543)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.213 (n=717)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.211)

- **PATRÓN** `ibs_20min` < `0.0643` → IC=+0.246 (n=680)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0643 (IC base=+0.211)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.635` → IC=+0.240 (n=528)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.635 (IC base=+0.211)

- **PATRÓN** `volumen_pendiente_norm` > `0.3556` → IC=+0.268 (n=222)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3556 (IC base=+0.211)

- **PATRÓN** `volumen_spike_ratio` > `2.2024` → IC=+0.220 (n=942)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2024 (IC base=+0.211)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.217 (n=1053)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.211)

- **PATRÓN** `libro_liquidez` > `1890.8984` → IC=+0.222 (n=700)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1890.8984 (IC base=+0.211)

- **PATRÓN** `ballena_activa_n` < `24.0` → IC=+0.215 (n=916)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.157 (n=100)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=2223)
...
- **FILTRO** `ibs_20min` > `0.2549` → IC=-0.126 (n=2196)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2549
  - _Potencial_: sin este filtro IC_bueno=+0.127 (n=1082)

- **FILTRO** `sigma_ewma_delta_pct` > `8.667` → IC=-0.209 (n=352)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.667
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=2926)
...
- **PATRÓN** `ibs_20min` < `0.2549` → IC=+0.127 (n=1082)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` < 0.2549 (IC base=-0.042)
...
- **FILTRO** `drift_60min` |x|> `0.6585` → IC=-0.183 (n=569)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6585
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=1710)

- **FILTRO** `ibs_20min` < `0.7149` → IC=-0.155 (n=1504)
...
- **FILTRO** `ibs_20min` > `0.77` → IC=-0.203 (n=832)

  - _Acción_: SKIP cuando `ibs_20min` > 0.77
  - _Potencial_: sin este filtro IC_bueno=+0.037 (n=2512)

- **PATRÓN** `dist_vwap_pct` > `0.8527` → IC=+0.314 (n=95)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8527 (IC base=-0.066)

- **PATRÓN** `dist_vwap_pct` < `0.2747` → IC=+0.316 (n=286)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2747 (IC base=-0.066)

- **PATRÓN** `volumen_regimen` < `0.9817` → IC=+0.289 (n=311)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9817 (IC base=-0.066)

- **PATRÓN** `volumen_regimen` > `0.6251` → IC=+0.300 (n=353)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6251 (IC base=-0.066)

- **PATRÓN** `volumen_pendiente_norm` < `0.1006` → IC=+0.296 (n=321)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8015` → IC=+0.291 (n=223)
...
- **PATRÓN** `volumen_regimen` < `0.7351` → IC=+0.252 (n=337)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1041` → IC=+0.275 (n=269)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1041 (IC base=-0.023)

- **PATRÓN** `volumen_spike_ratio` < `2.1485` → IC=+0.258 (n=577)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1485 (IC base=-0.023)

- **PATRÓN** `volumen_spike_ratio` > `1.4315` → IC=+0.246 (n=655)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4315 (IC base=-0.023)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.244 (n=671)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 35.0 (IC base=-0.023)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0098` → IC=+0.197 (n=3388)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0098 (IC base=+0.098)

- **PATRÓN** `ibs_20min` > `0.4762` → IC=+0.189 (n=9072)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.4762 (IC base=+0.098)

- **PATRÓN** `dist_vwap_pct` > `0.7752` → IC=+0.290 (n=1078)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7752 (IC base=+0.098)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.593` → IC=+0.157 (n=4772)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 3.593 (IC base=+0.098)

- **PATRÓN** `volumen_regimen` > `0.6889` → IC=+0.247 (n=3229)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6889 (IC base=+0.098)

- **PATRÓN** `volumen_pendiente_norm` > `0.2951` → IC=+0.267 (n=858)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2951 (IC base=+0.098)

- **PATRÓN** `volumen_spike_ratio` < `1.4663` → IC=+0.240 (n=1957)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4663 (IC base=+0.098)

- **PATRÓN** `volumen_spike_ratio` > `2.6662` → IC=+0.242 (n=1956)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6662 (IC base=+0.098)

- **PATRÓN** `ballena_activa_n` < `97.0` → IC=+0.268 (n=5384)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 97.0 (IC base=+0.098)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.154 (n=3376)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0091 (IC base=+0.073)

- **PATRÓN** `ibs_20min` < `0.5484` → IC=+0.155 (n=8897)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.5484 (IC base=+0.073)

- **PATRÓN** `dist_vwap_pct` > `1.041` → IC=+0.241 (n=485)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.041 (IC base=+0.073)

- **PATRÓN** `dist_vwap_pct` < `0.25` → IC=+0.239 (n=2809)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.25 (IC base=+0.073)

- **PATRÓN** `volumen_regimen` < `0.7092` → IC=+0.240 (n=1311)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7092 (IC base=+0.073)

- **PATRÓN** `volumen_regimen` > `1.205` → IC=+0.247 (n=993)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.205 (IC base=+0.073)

- **PATRÓN** `volumen_pendiente_norm` > `0.2444` → IC=+0.304 (n=743)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2444 (IC base=+0.073)

- **PATRÓN** `volumen_spike_ratio` < `1.604` → IC=+0.258 (n=1728)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.604 (IC base=+0.073)

- **PATRÓN** `volumen_spike_ratio` > `2.3173` → IC=+0.257 (n=1781)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3173 (IC base=+0.073)

- **PATRÓN** `ballena_activa_n` < `84.0` → IC=+0.263 (n=3811)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 84.0 (IC base=+0.073)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `sigma_ewma_delta_pct` > `4.523` → IC=-0.159 (n=534)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.523
  - _Potencial_: sin este filtro IC_bueno=+0.025 (n=1796)

- **PATRÓN** `ibs_20min` > `0.8935` → IC=+0.268 (n=701)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8935 (IC base=+0.045)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.796` → IC=+0.209 (n=372)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.796 (IC base=+0.045)

- **PATRÓN** `volumen_pendiente_norm` > `0.2241` → IC=+0.271 (n=173)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2241 (IC base=+0.045)

- **PATRÓN** `volumen_spike_ratio` < `1.433` → IC=+0.173 (n=295)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.433 (IC base=+0.045)

- **PATRÓN** `volumen_spike_ratio` > `2.1547` → IC=+0.200 (n=401)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1547 (IC base=+0.045)

- **PATRÓN** `ballena_activa_n` < `14.0` → IC=+0.180 (n=392)
...
- **PATRÓN** `ibs_20min` > `0.862` → IC=+0.160 (n=678)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.862 (IC base=+0.026)

- **PATRÓN** `dist_vwap_pct` > `0.1279` → IC=+0.173 (n=524)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.1279 (IC base=+0.026)

- **PATRÓN** `volumen_regimen` > `0.6718` → IC=+0.165 (n=836)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4247` → IC=+0.197 (n=305)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 1.4247 (IC base=+0.026)

- **PATRÓN** `ballena_activa_n` < `245.0` → IC=+0.193 (n=399)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 245.0 (IC base=+0.026)

- **PATRÓN** `dist_vwap_pct` < `0.1614` → IC=+0.218 (n=576)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1614 (IC base=+0.005)

- **PATRÓN** `volumen_regimen` > `0.8539` → IC=+0.223 (n=377)
...
- **PATRÓN** `sigma_h` > `0.0098` → IC=+0.316 (n=580)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0098 (IC base=+0.282)

- **PATRÓN** `drift_60min` |x|≤ `0.6107` → IC=+0.284 (n=1278)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.2222` → IC=+0.290 (n=1125)
...
- **PATRÓN** `volumen_spike_ratio` > `2.7347` → IC=+0.288 (n=536)
...
- **PATRÓN** `libro_liquidez` > `1878.6984` → IC=+0.299 (n=580)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1878.6984 (IC base=+0.282)
...
- **FILTRO** `ibs_20min` < `0.2871` → IC=-0.183 (n=496)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2871
  - _Potencial_: sin este filtro IC_bueno=+0.078 (n=1490)

- **FILTRO** `ibs_20min` > `0.7753` → IC=-0.187 (n=599)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7753
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=1801)

- **PATRÓN** `ibs_20min` > `0.822` → IC=+0.156 (n=676)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` > 0.822 (IC base=+0.013)

- **PATRÓN** `dist_vwap_pct` > `0.4586` → IC=+0.225 (n=249)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4586 (IC base=+0.013)

- **PATRÓN** `dist_vwap_pct` < `0.709` → IC=+0.210 (n=584)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.709 (IC base=+0.013)

- **PATRÓN** `volumen_regimen` < `0.9865` → IC=+0.236 (n=501)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9865 (IC base=+0.013)

- **PATRÓN** `volumen_regimen` > `0.5863` → IC=+0.209 (n=569)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.5863 (IC base=+0.013)

- **PATRÓN** `volumen_pendiente_norm` > `0.0807` → IC=+0.260 (n=206)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0807 (IC base=+0.013)

- **PATRÓN** `volumen_spike_ratio` < `1.5113` → IC=+0.266 (n=237)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5113 (IC base=+0.013)

- **PATRÓN** `ballena_activa_n` < `151.0` → IC=+0.244 (n=546)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 151.0 (IC base=+0.013)

- **PATRÓN** `dist_vwap_pct` > `0.1552` → IC=+0.211 (n=209)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1552 (IC base=-0.005)

- **PATRÓN** `dist_vwap_pct` < `0.6853` → IC=+0.203 (n=493)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8269` → IC=+0.264 (n=256)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8269 (IC base=-0.005)

- **PATRÓN** `ballena_activa_n` < `138.0` → IC=+0.255 (n=385)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 138.0 (IC base=-0.005)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **FILTRO** `ibs_20min` < `0.725` → IC=-0.197 (n=1075)

  - _Acción_: SKIP cuando `ibs_20min` < 0.725
  - _Potencial_: sin este filtro IC_bueno=+0.281 (n=1077)

- **FILTRO** `ibs_20min` > `0.6875` → IC=-0.234 (n=554)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6875
  - _Potencial_: sin este filtro IC_bueno=+0.096 (n=1683)

- **FILTRO** `sigma_ewma_delta_pct` > `4.705` → IC=-0.181 (n=487)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.705
  - _Potencial_: sin este filtro IC_bueno=+0.069 (n=1750)

- **PATRÓN** `ibs_20min` > `0.725` → IC=+0.281 (n=1077)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.725 (IC base=+0.042)

- **PATRÓN** `dist_vwap_pct` > `0.8548` → IC=+0.337 (n=249)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8548 (IC base=+0.042)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.595` → IC=+0.164 (n=340)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 9.595 (IC base=+0.042)

- **PATRÓN** `volumen_regimen` < `0.8679` → IC=+0.304 (n=530)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8679 (IC base=+0.042)

- **PATRÓN** `volumen_regimen` > `0.7306` → IC=+0.295 (n=709)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7306 (IC base=+0.042)

- **PATRÓN** `volumen_pendiente_norm` < `0.1042` → IC=+0.292 (n=738)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1042 (IC base=+0.042)

- **PATRÓN** `volumen_pendiente_norm` > `0.2247` → IC=+0.301 (n=149)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2247 (IC base=+0.042)

- **PATRÓN** `volumen_spike_ratio` < `1.422` → IC=+0.326 (n=257)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.422 (IC base=+0.042)

- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.314 (n=671)
...
- **PATRÓN** `dist_vwap_pct` > `0.7308` → IC=+0.201 (n=135)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7308 (IC base=+0.015)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.285 (n=1378)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.277)

- **PATRÓN** `ibs_20min` > `0.9118` → IC=+0.345 (n=878)
...
- **PATRÓN** `volumen_regimen` > `0.8611` → IC=+0.302 (n=878)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8611 (IC base=+0.277)

- **PATRÓN** `volumen_pendiente_norm` < `0.0798` → IC=+0.280 (n=1119)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0798 (IC base=+0.277)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.283 (n=1410)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.277)

- **PATRÓN** `libro_liquidez` > `2628.8988` → IC=+0.292 (n=878)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2628.8988 (IC base=+0.277)

- **PATRÓN** `sigma_h` > `0.0152` → IC=+0.298 (n=945)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.273 (n=699)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.272)

- **PATRÓN** `ibs_20min` < `0.3925` → IC=+0.305 (n=1417)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3925 (IC base=+0.272)

- **PATRÓN** `dist_vwap_pct` > `0.3051` → IC=+0.282 (n=544)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3051 (IC base=+0.272)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.494` → IC=+0.290 (n=521)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.494 (IC base=+0.272)

- **PATRÓN** `volumen_regimen` < `0.6405` → IC=+0.277 (n=473)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6405 (IC base=+0.272)

- **PATRÓN** `volumen_regimen` > `1.2436` → IC=+0.306 (n=473)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2436 (IC base=+0.272)

- **PATRÓN** `volumen_pendiente_norm` > `0.2392` → IC=+0.341 (n=230)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2392 (IC base=+0.272)

- **PATRÓN** `volumen_spike_ratio` < `1.4288` → IC=+0.273 (n=417)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4288 (IC base=+0.272)

- **PATRÓN** `volumen_spike_ratio` > `2.1485` → IC=+0.277 (n=567)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1485 (IC base=+0.272)

- **PATRÓN** `libro_liquidez` > `2622.2916` → IC=+0.275 (n=945)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2622.2916 (IC base=+0.272)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.171 (n=2636)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0049 (IC base=+0.168)

- **PATRÓN** `sigma_h` > `0.0112` → IC=+0.201 (n=2636)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0112 (IC base=+0.168)

- **PATRÓN** `drift_60min` |x|≤ `0.0907` → IC=+0.188 (n=2636)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.0907 (IC base=+0.168)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.179 (n=8238)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 5.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` > `0.5765` → IC=+0.218 (n=7906)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5765 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` > `0.1772` → IC=+0.196 (n=3463)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1772 (IC base=+0.168)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.335` → IC=+0.256 (n=1620)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.335 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` < `1.2103` → IC=+0.160 (n=5233)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2103 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` > `0.6298` → IC=+0.161 (n=5233)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.6298 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.2957` → IC=+0.196 (n=1184)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.2957 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` < `1.559` → IC=+0.169 (n=3336)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.559 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.6202` → IC=+0.177 (n=2527)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.6202 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `2402.5634` → IC=+0.169 (n=5271)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 2402.5634 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `115.0` → IC=+0.181 (n=6815)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 115.0 (IC base=+0.168)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.182 (n=5040)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0066 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0814` → IC=+0.207 (n=2518)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4792` → IC=+0.227 (n=7546)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4792 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.9254` → IC=+0.148 (n=732)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.9254 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` < `0.2337` → IC=+0.158 (n=5464)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.2337 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.343` → IC=+0.195 (n=1282)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 10.343 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.1737` → IC=+0.151 (n=5467)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1737 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.2911` → IC=+0.220 (n=1084)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2911 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.5609` → IC=+0.166 (n=3019)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.5609 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.2526` → IC=+0.170 (n=3111)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.2526 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `116.0` → IC=+0.172 (n=6508)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3439` → IC=+0.205 (n=1346)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_spike_ratio` > `1.4355` → IC=+0.180 (n=1245)
...
- **PATRÓN** `ballena_activa_n` < `47.0` → IC=+0.239 (n=776)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0749` → IC=+0.201 (n=393)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0749 (IC base=+0.162)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.192 (n=1056)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 8.0 (IC base=+0.162)

- **PATRÓN** `ibs_20min` > `0.4068` → IC=+0.228 (n=1176)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4068 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` > `0.212` → IC=+0.213 (n=701)
...
- **PATRÓN** `volumen_regimen` < `1.2628` → IC=+0.164 (n=1177)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.2628 (IC base=+0.162)

- **PATRÓN** `volumen_regimen` > `1.0731` → IC=+0.170 (n=534)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 1.0731 (IC base=+0.162)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5024` → IC=+0.183 (n=503)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` < 1.5024 (IC base=+0.162)
...
- **PATRÓN** `libro_liquidez` > `15975.8246` → IC=+0.164 (n=534)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 15975.8246 (IC base=+0.162)

- **PATRÓN** `ballena_activa_n` < `390.0` → IC=+0.160 (n=966)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 390.0 (IC base=+0.162)
...
- **PATRÓN** `sigma_h` < `0.0114` → IC=+0.230 (n=1098)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0114 (IC base=+0.217)

- **PATRÓN** `drift_60min` |x|≤ `0.2463` → IC=+0.236 (n=732)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.2407` → IC=+0.254 (n=966)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2407 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.746` → IC=+0.263 (n=471)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7776` → IC=+0.209 (n=448)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7776 (IC base=+0.217)
...
- **PATRÓN** `libro_liquidez` > `1889.461` → IC=+0.220 (n=498)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1889.461 (IC base=+0.217)

- **PATRÓN** `ballena_activa_n` < `12.0` → IC=+0.211 (n=354)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 12.0 (IC base=+0.217)

### GBM_LATE_15M_MULTIHORIZONTE#ETH#15min
- **PATRÓN** `sigma_h` < `0.0036` → IC=+0.210 (n=422)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0036 (IC base=+0.144)

- **PATRÓN** `drift_60min` |x|≤ `0.4335` → IC=+0.159 (n=1264)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.4335 (IC base=+0.144)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=1327)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.144)

- **PATRÓN** `ibs_20min` > `0.3782` → IC=+0.198 (n=1264)

  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` > 0.3782 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `0.1614` → IC=+0.180 (n=842)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.1614 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.979` → IC=+0.233 (n=234)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.979 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` < `1.0385` → IC=+0.145 (n=1113)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.0385 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` > `0.6254` → IC=+0.148 (n=1264)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` > 0.6254 (IC base=+0.144)

- **PATRÓN** `volumen_pendiente_norm` > `0.2917` → IC=+0.200 (n=198)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2917 (IC base=+0.144)

- **PATRÓN** `volumen_spike_ratio` < `1.4219` → IC=+0.153 (n=413)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 1.4219 (IC base=+0.144)

- **PATRÓN** `volumen_spike_ratio` > `2.5113` → IC=+0.169 (n=412)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.5113 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `6488.9388` → IC=+0.185 (n=843)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 6488.9388 (IC base=+0.144)

- **PATRÓN** `ballena_activa_n` < `163.0` → IC=+0.148 (n=1201)
...
- **PATRÓN** `sigma_h` > `0.0101` → IC=+0.159 (n=648)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `drift_60min` |x|≤ `0.104` → IC=+0.158 (n=483)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.104 (IC base=+0.112)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.162 (n=658)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 15.0 (IC base=+0.112)

- **PATRÓN** `ibs_20min` < `0.5652` → IC=+0.210 (n=1447)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5652 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `1.0165` → IC=+0.143 (n=194)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 1.0165 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` < `0.2006` → IC=+0.138 (n=1331)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.205 (n=619)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.201)

- **PATRÓN** `ibs_20min` > `0.74` → IC=+0.259 (n=1222)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.211 (n=638)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.203)

- **PATRÓN** `ibs_20min` < `0.438` → IC=+0.246 (n=1407)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.438 (IC base=+0.203)

- **PATRÓN** `dist_vwap_pct` > `1.273` → IC=+0.236 (n=172)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.273 (IC base=+0.203)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.43` → IC=+0.241 (n=272)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.43 (IC base=+0.203)

- **PATRÓN** `volumen_regimen` > `0.6302` → IC=+0.215 (n=1407)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2093` → IC=+0.194 (n=1113)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 2.2093 (IC base=+0.203)

- **PATRÓN** `volumen_spike_ratio` > `1.4374` → IC=+0.198 (n=1264)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.4374 (IC base=+0.203)

- **PATRÓN** `libro_liquidez` > `2592.8602` → IC=+0.208 (n=938)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2592.8602 (IC base=+0.203)

### GBM_LATE_15M_PYCONFIRMADO
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.161 (n=635)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0039 (IC base=+0.147)

- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.181 (n=634)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0088 (IC base=+0.147)

- **PATRÓN** `drift_60min` |x|≤ `0.1006` → IC=+0.153 (n=632)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.1006 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.184 (n=961)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 15.0 (IC base=+0.147)

- **PATRÓN** `ibs_20min` > `0.5446` → IC=+0.188 (n=1692)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.5446 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.8823` → IC=+0.185 (n=303)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.8823 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.703` → IC=+0.172 (n=859)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 3.703 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `0.8709` → IC=+0.163 (n=1112)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.8709 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` > `0.698` → IC=+0.150 (n=1490)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 0.698 (IC base=+0.147)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4417` → IC=+0.163 (n=609)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 1.4417 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` > `2.5338` → IC=+0.153 (n=609)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 2.5338 (IC base=+0.147)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.150 (n=2153)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.02 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `12439.6953` → IC=+0.159 (n=632)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 12439.6953 (IC base=+0.147)

- **PATRÓN** `ballena_activa_n` < `161.0` → IC=+0.167 (n=1674)

  - _Acción_: Kelly boost +0.84€ cuando `ballena_activa_n` < 161.0 (IC base=+0.147)

- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.141 (n=1320)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.6598` → IC=+0.139 (n=1975)

  - _Acción_: Kelly boost +0.69€ cuando `ibs_20min` < 0.6598 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` < `0.2134` → IC=+0.122 (n=1750)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.2134 (IC base=+0.111)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4556` → IC=+0.140 (n=634)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 1.4556 (IC base=+0.111)

- **PATRÓN** `libro_liquidez` > `2753.0147` → IC=+0.124 (n=1764)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3447` → IC=+0.160 (n=634)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6039` → IC=+0.185 (n=557)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` < 0.6039 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.487` → IC=+0.153 (n=727)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.487 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.406` → IC=+0.157 (n=246)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 4.406 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `1.2142` → IC=+0.142 (n=633)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 1.2142 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `1.0609` → IC=+0.168 (n=287)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 1.0609 (IC base=+0.138)

- **PATRÓN** `volumen_pendiente_norm` > `0.1583` → IC=+0.198 (n=170)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.1583 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` < `2.1106` → IC=+0.156 (n=548)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.1106 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` > `1.42` → IC=+0.151 (n=623)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.42 (IC base=+0.138)

- **PATRÓN** `ballena_activa_n` < `325.0` → IC=+0.155 (n=529)
...
- **PATRÓN** `dist_vwap_pct` > `0.1561` → IC=+0.208 (n=313)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1561 (IC base=+0.190)

- **PATRÓN** `dist_vwap_pct` < `0.2301` → IC=+0.190 (n=511)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.2301 (IC base=+0.190)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.015` → IC=+0.223 (n=193)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.015 (IC base=+0.190)

- **PATRÓN** `volumen_regimen` < `0.8331` → IC=+0.195 (n=392)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` < 0.8331 (IC base=+0.190)
...
- **PATRÓN** `volumen_spike_ratio` < `1.3993` → IC=+0.219 (n=194)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3993 (IC base=+0.190)
...
- **FILTRO** `dist_vwap_pct` > `0.3434` → IC=-0.139 (n=34)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.090 (n=491)

- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.184 (n=194)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0089 (IC base=+0.133)

- **PATRÓN** `drift_60min` |x|≤ `0.5317` → IC=+0.136 (n=426)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.9016` → IC=+0.238 (n=284)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9016 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` > `0.947` → IC=+0.244 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.947 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.1` → IC=+0.203 (n=190)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.1 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` < `1.0674` → IC=+0.150 (n=375)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.0674 (IC base=+0.133)

- **PATRÓN** `volumen_pendiente_norm` > `0.2872` → IC=+0.182 (n=61)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.2872 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` > `2.2097` → IC=+0.177 (n=184)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 2.2097 (IC base=+0.133)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.134 (n=463)

  - _Acción_: Kelly boost +0.67€ cuando `libro_spread` < 0.02 (IC base=+0.133)

- **PATRÓN** `libro_liquidez` > `3089.4567` → IC=+0.201 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3089.4567 (IC base=+0.133)

- **PATRÓN** `ibs_20min` < `0.5` → IC=+0.151 (n=394)

  - _Acción_: Kelly boost +0.76€ cuando `ibs_20min` < 0.5 (IC base=+0.075)

- **PATRÓN** `volumen_spike_ratio` < `1.8405` → IC=+0.153 (n=246)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 1.8405 (IC base=+0.075)

- **PATRÓN** `libro_liquidez` > `2552.1243` → IC=+0.123 (n=263)

  - _Acción_: Kelly boost +0.61€ cuando `libro_liquidez` > 2552.1243 (IC base=+0.075)

- **PATRÓN** `ballena_activa_n` < `43.0` → IC=+0.125 (n=337)

  - _Acción_: Kelly boost +0.63€ cuando `ballena_activa_n` < 43.0 (IC base=+0.075)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.206 (n=3371)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.216 (n=10111)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4706 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` > `0.984` → IC=+0.204 (n=1428)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.984 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.842` → IC=+0.234 (n=3720)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.842 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` < `0.8803` → IC=+0.166 (n=4506)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.8803 (IC base=+0.169)

- **PATRÓN** `volumen_pendiente_norm` > `0.2899` → IC=+0.201 (n=1393)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2899 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `2.5995` → IC=+0.186 (n=3239)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 2.5995 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `2347.9054` → IC=+0.172 (n=6737)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 2347.9054 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `86.0` → IC=+0.195 (n=7683)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 86.0 (IC base=+0.169)

- **PATRÓN** `sigma_h` < `0.0054` → IC=+0.195 (n=4030)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0054 (IC base=+0.182)

- **PATRÓN** `drift_60min` |x|≤ `0.149` → IC=+0.189 (n=4031)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `drift_60min` |x|≤ `0.3586` → IC=+0.190 (n=1712)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_spike_ratio` > `2.2444` → IC=+0.198 (n=733)
...
- **PATRÓN** `sigma_h` > `0.0045` → IC=+0.264 (n=1345)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0045 (IC base=+0.259)

- **PATRÓN** `drift_60min` |x|≤ `0.1282` → IC=+0.284 (n=591)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `drift_60min` |x|≤ `0.085` → IC=+0.162 (n=540)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.3086` → IC=+0.204 (n=1620)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3086 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.3459` → IC=+0.193 (n=660)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.3459 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.721` → IC=+0.169 (n=370)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 9.721 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.242` → IC=+0.153 (n=1460)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 4.242 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` < `0.628` → IC=+0.179 (n=540)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` < 0.628 (IC base=+0.150)

- **PATRÓN** `volumen_pendiente_norm` > `0.2676` → IC=+0.201 (n=232)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2676 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` < `2.1149` → IC=+0.162 (n=1377)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.1149 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.4047` → IC=+0.155 (n=1565)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.4047 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `15747.0266` → IC=+0.158 (n=735)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 15747.0266 (IC base=+0.150)

- **PATRÓN** `ballena_activa_n` < `473.0` → IC=+0.158 (n=1496)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 473.0 (IC base=+0.150)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.165 (n=1402)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0057 (IC base=+0.151)

- **PATRÓN** `drift_60min` |x|≤ `0.2608` → IC=+0.165 (n=1232)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.2608 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.179 (n=471)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 18.0 (IC base=+0.151)

- **PATRÓN** `ibs_20min` < `0.6574` → IC=+0.196 (n=1400)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` < 0.6574 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` > `0.6926` → IC=+0.155 (n=230)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.6926 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` < `0.1343` → IC=+0.165 (n=1258)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.274` → IC=+0.152 (n=1266)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` < 4.274 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` < `1.1894` → IC=+0.163 (n=1400)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.1894 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` > `0.1513` → IC=+0.198 (n=375)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.1513 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` < `2.4071` → IC=+0.160 (n=1302)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.4071 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` > `1.5107` → IC=+0.160 (n=1163)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 1.5107 (IC base=+0.151)

- **PATRÓN** `ballena_activa_n` < `421.0` → IC=+0.157 (n=1063)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 421.0 (IC base=+0.151)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `sigma_h` > `0.012` → IC=+0.247 (n=548)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.223 (n=1657)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.218)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.297 (n=645)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.218)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.809` → IC=+0.293 (n=481)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.809 (IC base=+0.218)

- **PATRÓN** `volumen_pendiente_norm` < `0.2112` → IC=+0.222 (n=1620)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2112 (IC base=+0.218)

- **PATRÓN** `volumen_spike_ratio` > `2.8331` → IC=+0.241 (n=708)
...
- **PATRÓN** `sigma_h` < `0.0117` → IC=+0.239 (n=1526)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0117 (IC base=+0.232)

- **PATRÓN** `drift_60min` |x|≤ `0.1677` → IC=+0.236 (n=672)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.235 (n=716)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.232)

- **PATRÓN** `ibs_20min` < `0.36` → IC=+0.268 (n=1343)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7564` → IC=+0.231 (n=618)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7564 (IC base=+0.232)

- **PATRÓN** `volumen_spike_ratio` > `2.1905` → IC=+0.238 (n=934)
...
- **PATRÓN** `libro_liquidez` > `1947.66` → IC=+0.244 (n=509)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1947.66 (IC base=+0.232)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.229 (n=1328)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4384` → IC=+0.145 (n=1719)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.4384 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.151 (n=1799)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 5.0 (IC base=+0.137)

- **PATRÓN** `ibs_20min` > `0.877` → IC=+0.262 (n=779)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.877 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.3757` → IC=+0.163 (n=681)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.3757 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.419` → IC=+0.169 (n=282)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 11.419 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `0.8735` → IC=+0.156 (n=1146)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.8735 (IC base=+0.137)
...
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.139 (n=1141)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0081 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=1759)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.191 (n=1712)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` > 0.4706 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `1.0905` → IC=+0.202 (n=350)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0905 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.447` → IC=+0.232 (n=644)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.447 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` < `0.8911` → IC=+0.136 (n=1140)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 0.8911 (IC base=+0.113)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.126 (n=1717)

  - _Acción_: Kelly boost +0.63€ cuando `libro_spread` < 0.02 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `2897.8254` → IC=+0.246 (n=570)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2897.8254 (IC base=+0.113)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.132 (n=1322)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1331` → IC=+0.161 (n=555)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6364` → IC=+0.204 (n=1665)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6364 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.22` → IC=+0.130 (n=1356)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.22 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.474` → IC=+0.124 (n=1609)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 3.474 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` < `0.7169` → IC=+0.153 (n=732)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.7169 (IC base=+0.113)

- **PATRÓN** `volumen_pendiente_norm` > `0.2234` → IC=+0.168 (n=260)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_pendiente_norm` > 0.2234 (IC base=+0.113)

- **PATRÓN** `volumen_spike_ratio` < `1.4467` → IC=+0.144 (n=501)
...
- **PATRÓN** `sigma_h` > `0.0193` → IC=+0.218 (n=1140)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.209 (n=1523)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.208)

- **PATRÓN** `ibs_20min` > `0.5143` → IC=+0.248 (n=1712)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5143 (IC base=+0.208)

- **PATRÓN** `dist_vwap_pct` > `0.2045` → IC=+0.234 (n=1016)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2045 (IC base=+0.208)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.264` → IC=+0.269 (n=305)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.264 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` < `1.2434` → IC=+0.211 (n=1711)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2434 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` > `0.6408` → IC=+0.216 (n=1710)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6408 (IC base=+0.208)

- **PATRÓN** `volumen_pendiente_norm` > `0.2358` → IC=+0.244 (n=299)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.217 (n=1800)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.208)

- **PATRÓN** `libro_liquidez` > `2625.687` → IC=+0.220 (n=1140)
...
- **PATRÓN** `sigma_h` > `0.0256` → IC=+0.221 (n=608)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.5172` → IC=+0.255 (n=1825)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5172 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` > `1.2878` → IC=+0.204 (n=305)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2878 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` < `0.9321` → IC=+0.203 (n=2025)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.9321 (IC base=+0.201)
...
- **PATRÓN** `volumen_regimen` > `1.236` → IC=+0.234 (n=608)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.236 (IC base=+0.201)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1989` → IC=+0.197 (n=1443)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.1989 (IC base=+0.201)

- **PATRÓN** `volumen_spike_ratio` > `1.432` → IC=+0.197 (n=1640)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.432 (IC base=+0.201)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.204 (n=1081)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.141 (n=3059)

- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.161 (n=2565)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0092 (IC base=+0.153)

- **PATRÓN** `drift_60min` |x|≤ `0.5214` → IC=+0.162 (n=2915)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.164 (n=1288)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 6.0 (IC base=+0.153)

- **PATRÓN** `ibs_20min` > `0.9414` → IC=+0.212 (n=971)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9414 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` > `0.1884` → IC=+0.167 (n=1087)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.1884 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` < `0.4996` → IC=+0.146 (n=1783)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.4996 (IC base=+0.153)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.152` → IC=+0.177 (n=478)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 10.152 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` > `0.8967` → IC=+0.160 (n=1268)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.8967 (IC base=+0.153)

- **PATRÓN** `volumen_pendiente_norm` > `0.1712` → IC=+0.185 (n=798)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1712 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` < `1.4564` → IC=+0.164 (n=960)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.4564 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` > `1.8826` → IC=+0.163 (n=1918)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.8826 (IC base=+0.153)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.153 (n=2023)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.153)

- **PATRÓN** `libro_liquidez` > `8270.0397` → IC=+0.158 (n=1321)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 8270.0397 (IC base=+0.153)

- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.197 (n=769)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0038 (IC base=+0.139)

- **PATRÓN** `drift_60min` |x|≤ `0.4897` → IC=+0.157 (n=2307)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `4.0` → IC=+0.164 (n=774)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 4.0 (IC base=+0.139)

- **PATRÓN** `ibs_20min` < `0.1827` → IC=+0.171 (n=1016)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` < 0.1827 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` > `0.6904` → IC=+0.157 (n=441)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.6904 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.306` → IC=+0.149 (n=2294)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` < 6.306 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` < `1.1012` → IC=+0.149 (n=1940)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 1.1012 (IC base=+0.139)

- **PATRÓN** `volumen_pendiente_norm` < `0.097` → IC=+0.142 (n=2087)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_pendiente_norm` < 0.097 (IC base=+0.139)

- **PATRÓN** `volumen_pendiente_norm` > `0.2207` → IC=+0.144 (n=492)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_pendiente_norm` > 0.2207 (IC base=+0.139)

- **PATRÓN** `volumen_spike_ratio` < `1.4242` → IC=+0.152 (n=759)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 1.4242 (IC base=+0.139)

- **PATRÓN** `volumen_spike_ratio` > `1.8123` → IC=+0.143 (n=1518)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 1.8123 (IC base=+0.139)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.141 (n=3059)

  - _Acción_: Kelly boost +0.70€ cuando `libro_spread` < 0.01 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `7207.0296` → IC=+0.152 (n=2060)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 7207.0296 (IC base=+0.139)

### GBM_LATE_5M#BTC#5min
- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.179 (n=344)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.0056 (IC base=+0.162)

- **PATRÓN** `sigma_h` > `0.0064` → IC=+0.177 (n=131)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0064 (IC base=+0.162)

- **PATRÓN** `drift_60min` |x|≤ `0.0902` → IC=+0.197 (n=130)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.0902 (IC base=+0.162)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.172 (n=394)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 5.0 (IC base=+0.162)

- **PATRÓN** `ibs_20min` < `0.5549` → IC=+0.187 (n=260)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` < 0.5549 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` > `0.2159` → IC=+0.175 (n=189)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.2159 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` < `0.3956` → IC=+0.164 (n=376)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.3956 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.783` → IC=+0.167 (n=61)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 5.783 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.403` → IC=+0.172 (n=412)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` < 2.403 (IC base=+0.162)

- **PATRÓN** `volumen_regimen` < `1.2372` → IC=+0.168 (n=389)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.2372 (IC base=+0.162)

- **PATRÓN** `volumen_regimen` > `0.8402` → IC=+0.194 (n=259)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 0.8402 (IC base=+0.162)

- **PATRÓN** `volumen_pendiente_norm` > `0.3053` → IC=+0.295 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3053 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` < `1.4394` → IC=+0.197 (n=130)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 1.4394 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` > `2.6647` → IC=+0.212 (n=130)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6647 (IC base=+0.162)

- **PATRÓN** `libro_liquidez` > `12550.0528` → IC=+0.206 (n=348)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12550.0528 (IC base=+0.162)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.173 (n=353)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` < 5.0 (IC base=+0.139)

- **PATRÓN** `ibs_20min` < `0.1382` → IC=+0.179 (n=415)

  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` < 0.1382 (IC base=+0.139)

- **PATRÓN** `ibs_20min` > `0.6084` → IC=+0.146 (n=428)

  - _Acción_: Kelly boost +0.73€ cuando `ibs_20min` > 0.6084 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` > `0.6972` → IC=+0.163 (n=87)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.6972 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` < `0.2195` → IC=+0.139 (n=955)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.2195 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.114` → IC=+0.154 (n=1031)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 9.114 (IC base=+0.139)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.0692` → IC=+0.164 (n=441)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.0692 (IC base=+0.139)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8156` → IC=+0.149 (n=627)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.8156 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `11419.5725` → IC=+0.153 (n=943)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 11419.5725 (IC base=+0.139)

- **PATRÓN** `ballena_activa_n` < `705.0` → IC=+0.146 (n=898)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.161 (n=820)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0073 (IC base=+0.157)

- **PATRÓN** `sigma_h` > `0.0043` → IC=+0.165 (n=929)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.0043 (IC base=+0.157)

- **PATRÓN** `drift_60min` |x|≤ `0.3873` → IC=+0.160 (n=818)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.3873 (IC base=+0.157)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.170 (n=656)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 11.0 (IC base=+0.157)

- **PATRÓN** `hora_utc` < `4.0` → IC=+0.164 (n=319)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 4.0 (IC base=+0.157)

- **PATRÓN** `ibs_20min` < `0.5617` → IC=+0.161 (n=620)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` < 0.5617 (IC base=+0.157)

- **PATRÓN** `ibs_20min` > `0.8884` → IC=+0.170 (n=310)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` > 0.8884 (IC base=+0.157)

- **PATRÓN** `dist_vwap_pct` > `0.7184` → IC=+0.163 (n=268)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.7184 (IC base=+0.157)

- **PATRÓN** `dist_vwap_pct` < `0.4283` → IC=+0.167 (n=865)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.4283 (IC base=+0.157)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.12` → IC=+0.169 (n=834)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` < 4.12 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` < `1.0845` → IC=+0.159 (n=818)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.0845 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` > `0.7159` → IC=+0.160 (n=830)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.7159 (IC base=+0.157)

- **PATRÓN** `volumen_pendiente_norm` < `0.107` → IC=+0.159 (n=855)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` < 0.107 (IC base=+0.157)

- **PATRÓN** `volumen_pendiente_norm` > `0.0781` → IC=+0.163 (n=402)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.0781 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` < `1.4364` → IC=+0.176 (n=304)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 1.4364 (IC base=+0.157)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.163 (n=918)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.157)

- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.155 (n=783)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0083 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.395` → IC=+0.172 (n=689)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.395 (IC base=+0.141)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.170 (n=274)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 17.0 (IC base=+0.141)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.154 (n=524)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` < 10.0 (IC base=+0.141)

- **PATRÓN** `ibs_20min` < `0.748` → IC=+0.146 (n=783)

  - _Acción_: Kelly boost +0.73€ cuando `ibs_20min` < 0.748 (IC base=+0.141)

- **PATRÓN** `ibs_20min` > `0.0993` → IC=+0.147 (n=783)

  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` > 0.0993 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` > `0.6358` → IC=+0.174 (n=170)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.6358 (IC base=+0.141)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.388` → IC=+0.148 (n=709)
...
- **PATRÓN** `volumen_regimen` > `0.7236` → IC=+0.142 (n=699)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2019` → IC=+0.155 (n=676)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.2019 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` > `1.7829` → IC=+0.145 (n=511)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.7829 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `7573.3328` → IC=+0.168 (n=783)
...
- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.152 (n=219)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0069 (IC base=+0.116)

- **PATRÓN** `drift_60min` |x|≤ `0.5355` → IC=+0.146 (n=193)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.5355 (IC base=+0.116)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.162 (n=149)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 10.0 (IC base=+0.116)

- **PATRÓN** `ibs_20min` < `0.1538` → IC=+0.210 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1538 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` > `0.6027` → IC=+0.198 (n=104)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.6027 (IC base=+0.116)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.699` → IC=+0.134 (n=121)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` > 2.699 (IC base=+0.116)

- **PATRÓN** `volumen_regimen` < `0.9493` → IC=+0.131 (n=147)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 0.9493 (IC base=+0.116)

- **PATRÓN** `volumen_pendiente_norm` < `0.0898` → IC=+0.176 (n=168)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` < 0.0898 (IC base=+0.116)

- **PATRÓN** `volumen_spike_ratio` < `1.8448` → IC=+0.139 (n=142)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` < 1.8448 (IC base=+0.116)

- **PATRÓN** `libro_liquidez` > `3331.2707` → IC=+0.142 (n=219)

  - _Acción_: Kelly boost +0.71€ cuando `libro_liquidez` > 3331.2707 (IC base=+0.116)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.161 (n=184)

  - _Acción_: Kelly boost +0.81€ cuando `ballena_activa_n` < 49.0 (IC base=+0.116)

### GBM_LATE_60M
- **FILTRO** `sigma_h` > `0.0066` → IC=-0.210 (n=122)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0066
  - _Potencial_: sin este filtro IC_bueno=+0.073 (n=368)

- **FILTRO** `dist_vwap_pct` > `0.1829` → IC=-0.179 (n=26)

  - _Acción_: SKIP cuando `dist_vwap_pct` > 0.1829
  - _Potencial_: sin este filtro IC_bueno=+0.114 (n=322)
...
- **PATRÓN** `ibs_20min` > `0.6731` → IC=+0.207 (n=745)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6731 (IC base=+0.096)

- **PATRÓN** `dist_vwap_pct` > `0.1549` → IC=+0.156 (n=457)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.1549 (IC base=+0.096)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2818` → IC=+0.197 (n=107)
  ...
  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.2818 (IC base=+0.096)

- **PATRÓN** `volumen_spike_ratio` < `2.0922` → IC=+0.136 (n=633)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 2.0922 (IC base=+0.096)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.121 (n=671)

  - _Acción_: Kelly boost +0.61€ cuando `libro_spread` < 0.02 (IC base=+0.096)

- **PATRÓN** `libro_liquidez` > `2432.8034` → IC=+0.142 (n=367)

  - _Acción_: Kelly boost +0.71€ cuando `libro_liquidez` > 2432.8034 (IC base=+0.096)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.0685` → IC=+0.203 (n=99)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0685 (IC base=+0.002)

- **PATRÓN** `volumen_spike_ratio` < `2.5091` → IC=+0.124 (n=232)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_spike_ratio` < 2.5091 (IC base=+0.002)

### GBM_LATE_60M#BTC#60min
- **PATRÓN** `sigma_h` < `0.006` → IC=+0.160 (n=316)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.006 (IC base=+0.105)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.122 (n=331)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 6.0 (IC base=+0.105)

- **PATRÓN** `ibs_20min` > `0.52` → IC=+0.186 (n=285)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` > 0.52 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` > `0.1379` → IC=+0.173 (n=151)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.1379 (IC base=+0.105)

- **PATRÓN** `volumen_regimen` < `1.067` → IC=+0.129 (n=251)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` < 1.067 (IC base=+0.105)

- **PATRÓN** `volumen_pendiente_norm` < `0.1568` → IC=+0.139 (n=261)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_pendiente_norm` < 0.1568 (IC base=+0.105)

- **PATRÓN** `volumen_spike_ratio` < `2.0118` → IC=+0.171 (n=217)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 2.0118 (IC base=+0.105)
...
- **PATRÓN** `volumen_regimen` < `0.6023` → IC=+0.167 (n=43)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.6023 (IC base=+0.050)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3732` → IC=+0.160 (n=104)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.3732 (IC base=+0.050)
...
- **PATRÓN** `sigma_h` < `0.004` → IC=+0.186 (n=138)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.004 (IC base=+0.108)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.144 (n=279)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 8.0 (IC base=+0.108)

- **PATRÓN** `ibs_20min` > `0.6789` → IC=+0.236 (n=252)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6789 (IC base=+0.108)

- **PATRÓN** `dist_vwap_pct` > `0.348` → IC=+0.172 (n=114)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.348 (IC base=+0.108)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.853` → IC=+0.306 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.853 (IC base=+0.108)

- **PATRÓN** `volumen_regimen` > `0.9419` → IC=+0.146 (n=128)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 0.9419 (IC base=+0.108)

- **PATRÓN** `volumen_pendiente_norm` > `0.2799` → IC=+0.237 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2799 (IC base=+0.108)

- **PATRÓN** `volumen_spike_ratio` < `1.7281` → IC=+0.177 (n=153)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 1.7281 (IC base=+0.108)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.124 (n=296)

  - _Acción_: Kelly boost +0.62€ cuando `libro_spread` < 0.02 (IC base=+0.108)

- **PATRÓN** `libro_liquidez` > `1133.0595` → IC=+0.160 (n=248)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 1133.0595 (IC base=+0.108)
...
- **PATRÓN** `ibs_20min` > `0.6667` → IC=+0.175 (n=241)
  ...
  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` > 0.6667 (IC base=+0.073)

- **PATRÓN** `dist_vwap_pct` > `0.2066` → IC=+0.147 (n=151)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.2066 (IC base=+0.073)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.497` → IC=+0.176 (n=106)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 5.497 (IC base=+0.073)

- **PATRÓN** `volumen_regimen` > `1.0643` → IC=+0.181 (n=89)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` > 1.0643 (IC base=+0.073)

- **PATRÓN** `volumen_pendiente_norm` > `0.2825` → IC=+0.194 (n=34)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2825 (IC base=+0.073)
...
- **FILTRO** `hora_utc` < `4.0` → IC=-0.305 (n=39)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.282 (n=131)
...
- **FILTRO** `volumen_spike_ratio` > `1.9204` → IC=-0.413 (n=21)

  - _Acción_: SKIP cuando `volumen_spike_ratio` > 1.9204
...
- **FILTRO** `sigma_h` > `0.0034` → IC=-0.362 (n=27)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
### LEADLAG_BTC_XRP_15M
- **PATRÓN** `py_entrada` > `0.5` → IC=+0.121 (n=702)

  - _Acción_: Kelly boost +0.60€ cuando `py_entrada` > 0.5 (IC base=+0.105)

- **PATRÓN** `libro_liquidez` > `2919.5711` → IC=+0.176 (n=236)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 2919.5711 (IC base=+0.105)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.028 (n=1786)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.167 (n=16)
  ...
  - _Acción_: SKIP cuando `liq_imbalance_15min` |x|≤ 1.0
  - _Potencial_: sin este filtro IC_bueno=+0.043 (n=787)

- **FILTRO** `py_entrada` > `0.505` → IC=-0.125 (n=62)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.052 (n=741)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.136 (n=377)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.005 (n=198)

- **PATRÓN** `py_entrada` < `0.47` → IC=+0.173 (n=154)

  - _Acción_: Kelly boost +0.87€ cuando `py_entrada` < 0.47 (IC base=+0.009)
...
- **PATRÓN** `py_entrada` < `0.59` → IC=+0.182 (n=64)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `py_entrada` < 0.59 (IC base=+0.094)

- **PATRÓN** `restante_min` > `3.82` → IC=+0.128 (n=49)

  - _Acción_: Kelly boost +0.64€ cuando `restante_min` > 3.82 (IC base=+0.094)

- **PATRÓN** `profundidad_ratio` > `46.0` → IC=+0.158 (n=71)

  - _Acción_: Kelly boost +0.79€ cuando `profundidad_ratio` > 46.0 (IC base=+0.094)
...
- **FILTRO** `profundidad_ratio` < `44.1` → IC=-0.167 (n=28)

  - _Acción_: SKIP cuando `profundidad_ratio` < 44.1
  - _Potencial_: sin este filtro IC_bueno=+0.177 (n=29)

- **PATRÓN** `py_entrada` < `0.45` → IC=+0.278 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.45 (IC base=+0.009)

- **PATRÓN** `profundidad_ratio` > `44.1` → IC=+0.177 (n=29)

  - _Acción_: Kelly boost +0.89€ cuando `profundidad_ratio` > 44.1 (IC base=+0.009)
...
- **FILTRO** `restante_min` < `13.42` → IC=-0.129 (n=33)

  - _Acción_: SKIP cuando `restante_min` < 13.42
...
- **PATRÓN** `restante_min` > `13.42` → IC=+0.200 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `restante_min` > 13.42 (IC base=-0.009)

- **PATRÓN** `lag_apertura_s` < `126.28` → IC=+0.143 (n=26)

  - _Acción_: Kelly boost +0.71€ cuando `lag_apertura_s` < 126.28 (IC base=-0.009)

### LIQUIDACIONES_DEPTH_FASE0#XRP#15min
- **FILTRO** `py_entrada` < `0.5` → IC=-0.208 (n=46)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.239 (n=21)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=67)
...
- **FILTRO** `py_entrada` > `0.59` → IC=-0.182 (n=20)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.122 (n=43)

- **PATRÓN** `py_entrada` < `0.59` → IC=+0.122 (n=43)

  - _Acción_: Kelly boost +0.61€ cuando `py_entrada` < 0.59 (IC base=+0.023)

- **PATRÓN** `restante_min` > `3.85` → IC=+0.147 (n=32)

  - _Acción_: Kelly boost +0.74€ cuando `restante_min` > 3.85 (IC base=+0.023)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=7530)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.166 (n=3537)

  - _Acción_: SKIP cuando `py_entrada` < 0.475
  - _Potencial_: sin este filtro IC_bueno=+0.058 (n=10673)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.168 (n=3657)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.036 (n=11042)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.46` → IC=-0.210 (n=606)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.100 (n=1863)

- **FILTRO** `py_entrada` > `0.64` → IC=-0.152 (n=645)

  - _Acción_: SKIP cuando `py_entrada` > 0.64
  - _Potencial_: sin este filtro IC_bueno=+0.064 (n=1990)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.47` → IC=-0.192 (n=618)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=1894)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.203 (n=641)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.063 (n=1991)

- **FILTRO** `ibs_20min` > `0.2814` → IC=-0.162 (n=657)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2814
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=1975)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.485` → IC=-0.173 (n=604)

  - _Acción_: SKIP cuando `py_entrada` < 0.485
  - _Potencial_: sin este filtro IC_bueno=+0.084 (n=1848)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.176 (n=641)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=1993)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=2945)
...
- **FILTRO** `hora_utc` < `15.0` → IC=-0.169 (n=119)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 15.0
...
- **FILTRO** `py_entrada` < `0.415` → IC=-0.214 (n=197)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=599)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.129 (n=9997)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=22544)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.273 (n=7969)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=24572)

- **FILTRO** `ibs_7min` < `0.2838` → IC=-0.234 (n=8135)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2838
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=24406)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.156 (n=11050)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=21491)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.230 (n=9990)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=30801)

- **FILTRO** `ibs_7min` > `0.2922` → IC=-0.177 (n=10196)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2922
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=30595)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.136 (n=1638)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3752)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.310 (n=1286)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=4104)

- **FILTRO** `ibs_7min` < `0.7115` → IC=-0.250 (n=1776)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7115
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3614)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.186 (n=1226)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4164)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.261 (n=1727)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=5282)

- **FILTRO** `drift_7min_pct` |x|> `0.1121` → IC=-0.125 (n=2381)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1121
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=4628)

- **FILTRO** `ibs_7min` > `0.7872` → IC=-0.206 (n=1752)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7872
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=5257)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.137 (n=1311)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.250 (n=1360)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=4256)

- **FILTRO** `ibs_7min` < `0.7471` → IC=-0.191 (n=1404)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7471
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=4212)

- **FILTRO** `ballena_activa_n` > `162.0` → IC=-0.178 (n=1396)

  - _Acción_: SKIP cuando `ballena_activa_n` > 162.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4220)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.253 (n=1421)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=4276)

- **FILTRO** `ibs_7min` > `0.2604` → IC=-0.177 (n=1424)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2604
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=4273)

- **FILTRO** `ballena_activa_n` > `154.0` → IC=-0.182 (n=1421)

  - _Acción_: SKIP cuando `ballena_activa_n` > 154.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=4276)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.164 (n=1248)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.087 (n=3862)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.306 (n=1260)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=3850)

- **FILTRO** `ibs_7min` < `0.7077` → IC=-0.243 (n=1685)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7077
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=3425)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.221 (n=1181)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3929)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.237 (n=1712)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=5785)

- **FILTRO** `ibs_7min` > `0.7442` → IC=-0.174 (n=1873)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7442
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=5624)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `py_entrada` < `0.37` → IC=-0.232 (n=1588)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=3784)

- **FILTRO** `ibs_7min` < `0.7406` → IC=-0.180 (n=1343)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7406
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=4029)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.170 (n=1316)
...
- **FILTRO** `py_entrada` > `0.66` → IC=-0.259 (n=1364)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=4105)

- **FILTRO** `ibs_7min` > `0.2745` → IC=-0.177 (n=1366)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2745
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=4103)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.184 (n=1348)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4121)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.253 (n=1388)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=4282)

- **FILTRO** `ibs_7min` < `0.3` → IC=-0.233 (n=1404)

  - _Acción_: SKIP cuando `ibs_7min` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4266)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.179 (n=1843)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=5923)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.254 (n=1740)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3643)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.228 (n=1335)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=4048)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.214 (n=1309)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=4074)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.206 (n=1752)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.010 (n=5601)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.324 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=1086)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.190 (n=27)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=787)
...
- **FILTRO** `py_entrada` > `0.495` → IC=-0.129 (n=33)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.032 (n=400)
...
- **FILTRO** `streak_len` > `3.0` → IC=-0.147 (n=15)
  ...
  - _Acción_: SKIP cuando `streak_len` > 3.0
  - _Potencial_: sin este filtro IC_bueno=+0.038 (n=725)

### STRUCT_NO_15M#BTC#15min
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=2923)

### STRUCT_NO_15M#SOL#15min
- **FILTRO** `py_entrada` < `0.495` → IC=-0.147 (n=32)

  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=1485)
...
### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.218 (n=733)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0086 (IC base=+0.187)

- **PATRÓN** `drift_60min` |x|≤ `0.0509` → IC=+0.202 (n=539)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0509 (IC base=+0.187)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2145` → IC=+0.193 (n=539)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.97€ cuando `delta_ratio_macro` |x|> 0.2145 (IC base=+0.187)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1251` → IC=+0.233 (n=567)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.187 (n=1689)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` < 17.0 (IC base=+0.187)

- **PATRÓN** `ibs_15` > `0.6061` → IC=+0.267 (n=1617)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6061 (IC base=+0.187)

- **PATRÓN** `dist_vwap_pct` > `0.118` → IC=+0.187 (n=841)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.118 (IC base=+0.187)

- **PATRÓN** `dist_vwap_pct` < `0.6263` → IC=+0.178 (n=1541)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.6263 (IC base=+0.187)

- **PATRÓN** `sigma_ewma_delta_pct` > `16.756` → IC=+0.281 (n=418)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 16.756 (IC base=+0.187)

- **PATRÓN** `libro_liquidez` > `2971.9929` → IC=+0.196 (n=1078)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 2971.9929 (IC base=+0.187)

### UPDOWN_GBM#60min
- **FILTRO** `libro_spread` > `0.01` → IC=-0.222 (n=16)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=616)
...
- **FILTRO** `dist_vwap_pct` > `0.5688` → IC=-0.135 (n=124)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=903)
...
- **PATRÓN** `sigma_h` > `0.0217` → IC=+0.249 (n=193)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0217 (IC base=+0.187)

- **PATRÓN** `drift_60min` |x|≤ `0.0864` → IC=+0.210 (n=188)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0864 (IC base=+0.187)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0393` → IC=+0.191 (n=425)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.95€ cuando `delta_ratio_macro` |x|> 0.0393 (IC base=+0.187)
...
- **PATRÓN** `hora_utc` < `4.0` → IC=+0.231 (n=143)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 4.0 (IC base=+0.187)

- **PATRÓN** `ibs_15` > `0.5455` → IC=+0.282 (n=425)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5455 (IC base=+0.187)

- **PATRÓN** `dist_vwap_pct` > `0.1318` → IC=+0.210 (n=257)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1318 (IC base=+0.187)

- **PATRÓN** `dist_vwap_pct` < `0.8758` → IC=+0.188 (n=489)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.8758 (IC base=+0.187)
...
- **FILTRO** `sigma_h` > `0.013` → IC=-0.220 (n=661)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.013
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=1985)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.193 (n=900)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=1746)

- **FILTRO** `libro_liquidez` < `3905.4186` → IC=-0.138 (n=1746)

  - _Acción_: SKIP cuando `libro_liquidez` < 3905.4186
  - _Potencial_: sin este filtro IC_bueno=+0.090 (n=900)
...
- **PATRÓN** `ibs_15` > `0.6371` → IC=+0.276 (n=633)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6371 (IC base=-0.061)

- **PATRÓN** `dist_vwap_pct` < `0.2815` → IC=+0.188 (n=501)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.2815 (IC base=-0.061)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1204` → IC=+0.249 (n=1177)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1204 (IC base=-0.033)

- **PATRÓN** `ibs_15` < `0.3469` → IC=+0.278 (n=1765)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3469 (IC base=-0.033)

- **PATRÓN** `dist_vwap_pct` > `0.6992` → IC=+0.303 (n=292)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6992 (IC base=-0.033)

### UPDOWN_GBM_15M_TARDIO#BTC#15min
- **FILTRO** `sigma_h` > `0.0068` → IC=-0.221 (n=403)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0068
  - _Potencial_: sin este filtro IC_bueno=-0.190 (n=1210)

- **FILTRO** `sigma_h` < `0.0034` → IC=-0.231 (n=403)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0034
  - _Potencial_: sin este filtro IC_bueno=-0.186 (n=1210)

- **FILTRO** `hora_utc` > `7.0` → IC=-0.209 (n=1023)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.177 (n=590)

- **FILTRO** `sigma_ewma_delta_pct` > `19.716` → IC=-0.247 (n=290)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 19.716
  - _Potencial_: sin este filtro IC_bueno=-0.187 (n=1323)

- **PATRÓN** `sigma_h` < `0.0029` → IC=+0.167 (n=151)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0029 (IC base=+0.086)

- **PATRÓN** `delta_ratio_macro` |x|> `0.252` → IC=+0.300 (n=58)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.252 (IC base=+0.086)
...
- **PATRÓN** `ibs_15` > `0.7496` → IC=+0.330 (n=174)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7496 (IC base=+0.086)

- **PATRÓN** `dist_vwap_pct` > `0.102` → IC=+0.287 (n=120)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.102 (IC base=+0.086)

- **PATRÓN** `dist_vwap_pct` < `0.5662` → IC=+0.285 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5662 (IC base=+0.086)

### UPDOWN_GBM_15M_TARDIO#ETH#15min
- **FILTRO** `libro_spread` > `0.01` → IC=-0.132 (n=17)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.168 (n=383)

- **PATRÓN** `sigma_h` > `0.0051` → IC=+0.193 (n=200)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0051 (IC base=+0.154)

- **PATRÓN** `drift_60min` |x|≤ `0.0757` → IC=+0.218 (n=133)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0757 (IC base=+0.154)

- **PATRÓN** `drift_15min` |x|≤ `0.4223` → IC=+0.170 (n=101)

  - _Acción_: Kelly boost +0.85€ cuando `drift_15min` |x|≤ 0.4223 (IC base=+0.154)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0589` → IC=+0.159 (n=300)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.79€ cuando `delta_ratio_macro` |x|> 0.0589 (IC base=+0.154)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2902` → IC=+0.240 (n=206)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.2902 (IC base=+0.154)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.206 (n=141)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.154)

- **PATRÓN** `ibs_15` > `0.6686` → IC=+0.268 (n=300)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6686 (IC base=+0.154)

- **PATRÓN** `dist_vwap_pct` > `0.4805` → IC=+0.167 (n=85)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.4805 (IC base=+0.154)

- **PATRÓN** `dist_vwap_pct` < `0.1187` → IC=+0.188 (n=216)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.1187 (IC base=+0.154)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.115` → IC=+0.165 (n=255)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` < 9.115 (IC base=+0.154)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.168 (n=383)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.01 (IC base=+0.154)

- **PATRÓN** `libro_liquidez` > `11008.7835` → IC=+0.203 (n=136)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11008.7835 (IC base=+0.154)

- **PATRÓN** `sigma_h` < `0.0044` → IC=+0.273 (n=231)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0044 (IC base=+0.230)

- **PATRÓN** `drift_60min` |x|≤ `0.4443` → IC=+0.231 (n=693)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4443 (IC base=+0.230)

- **PATRÓN** `drift_15min` |x|≤ `0.48` → IC=+0.236 (n=305)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.48 (IC base=+0.230)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2028` → IC=+0.263 (n=314)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.2028 (IC base=+0.230)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.240 (n=275)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.230)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.239 (n=305)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.230)

- **PATRÓN** `ibs_15` < `0.3453` → IC=+0.268 (n=693)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **PATRÓN** `sigma_ewma_delta_pct` > `16.687` → IC=+0.256 (n=121)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 16.687 (IC base=+0.230)

- **PATRÓN** `sigma_ewma_delta_pct` < `12.182` → IC=+0.237 (n=737)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 12.182 (IC base=+0.230)

- **PATRÓN** `libro_liquidez` > `3544.1821` → IC=+0.230 (n=693)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3544.1821 (IC base=+0.230)

### UPDOWN_GBM_15M_TARDIO#SOL#15min
- **FILTRO** `sigma_h` > `0.0102` → IC=-0.247 (n=156)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0102
  - _Potencial_: sin este filtro IC_bueno=-0.141 (n=471)

- **FILTRO** `drift_60min` |x|> `0.1671` → IC=-0.216 (n=213)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.1671
  - _Potencial_: sin este filtro IC_bueno=-0.142 (n=414)

- **FILTRO** `drift_15min` |x|> `0.888` → IC=-0.266 (n=156)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 0.888
  - _Potencial_: sin este filtro IC_bueno=-0.134 (n=471)
...
- **PATRÓN** `ibs_15` < `0.3437` → IC=+0.307 (n=469)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
### UPDOWN_GBM_IBS_ALTO
- **PATRÓN** `sigma_h` < `0.0045` → IC=+0.304 (n=443)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0045 (IC base=+0.296)

- **PATRÓN** `sigma_h` > `0.0054` → IC=+0.295 (n=301)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0054 (IC base=+0.296)

- **PATRÓN** `drift_60min` |x|≤ `0.0563` → IC=+0.330 (n=222)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0563 (IC base=+0.296)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2402` → IC=+0.316 (n=221)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.2402 (IC base=+0.296)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1077` → IC=+0.349 (n=184)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1077 (IC base=+0.296)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.323 (n=604)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.296)

- **PATRÓN** `ibs_15` > `0.8404` → IC=+0.330 (n=663)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8404 (IC base=+0.296)

- **PATRÓN** `dist_vwap_pct` > `0.1578` → IC=+0.324 (n=390)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1578 (IC base=+0.296)

- **PATRÓN** `sigma_ewma_delta_pct` > `23.12` → IC=+0.330 (n=139)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 23.12 (IC base=+0.296)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.298 (n=811)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.296)

- **PATRÓN** `libro_liquidez` > `14611.6311` → IC=+0.312 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 14611.6311 (IC base=+0.296)
...
- **PATRÓN** `sigma_h` > `0.0047` → IC=+0.293 (n=167)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_15` > `0.83` → IC=+0.316 (n=367)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.83 (IC base=+0.288)

- **PATRÓN** `dist_vwap_pct` > `0.4454` → IC=+0.357 (n=103)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4454 (IC base=+0.288)
...
- **PATRÓN** `libro_liquidez` > `16193.642` → IC=+0.332 (n=123)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 16193.642 (IC base=+0.288)

### UPDOWN_GBM_IBS_ALTO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0068` → IC=+0.313 (n=297)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0068 (IC base=+0.303)

- **PATRÓN** `drift_60min` |x|≤ `0.1127` → IC=+0.306 (n=199)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1127 (IC base=+0.303)

- **PATRÓN** `delta_ratio_macro` |x|> `0.226` → IC=+0.322 (n=99)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.226 (IC base=+0.303)
...
- **PATRÓN** `ibs_15` > `0.8489` → IC=+0.343 (n=297)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8489 (IC base=+0.303)

- **PATRÓN** `dist_vwap_pct` > `0.6422` → IC=+0.312 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6422 (IC base=+0.303)

- **PATRÓN** `dist_vwap_pct` < `0.1676` → IC=+0.304 (n=212)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1676 (IC base=+0.303)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.314 (n=336)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6061 sube el IC de +0.187 a +0.267 en UPDOWN_GBM#15min (n=1617). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_YES, IBS > 0.5455 sube el IC de +0.187 a +0.282 en UPDOWN_GBM#XRP#15min (n=425). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1176 sube el IC de +0.049 a +0.157 en UPDOWN_GBM#XRP#15min (n=479). Ya aplicado como kelly_boost=+0.78€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.6371 sube el IC de -0.061 a +0.276 en UPDOWN_GBM_15M_TARDIO (n=633). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3469 sube el IC de -0.033 a +0.278 en UPDOWN_GBM_15M_TARDIO (n=1765). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO#BTC#15min**: dentro de BUY_YES, IBS > 0.7496 sube el IC de +0.086 a +0.330 en UPDOWN_GBM_15M_TARDIO#BTC#15min (n=174). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_YES, IBS > 0.6686 sube el IC de +0.154 a +0.268 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=300). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.3453 sube el IC de +0.230 a +0.268 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=693). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#XRP#15min**: dentro de BUY_NO, IBS < 0.3437 sube el IC de -0.043 a +0.307 en UPDOWN_GBM_15M_TARDIO#XRP#15min (n=469). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.8404 sube el IC de +0.296 a +0.330 en UPDOWN_GBM_IBS_ALTO (n=663). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_IBS_ALTO#BTC#15min**: dentro de BUY_YES, IBS > 0.83 sube el IC de +0.288 a +0.316 en UPDOWN_GBM_IBS_ALTO#BTC#15min (n=367). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_IBS_ALTO#ETH#15min**: dentro de BUY_YES, IBS > 0.8489 sube el IC de +0.303 a +0.343 en UPDOWN_GBM_IBS_ALTO#ETH#15min (n=297). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 28317 | -0.091 | -3810.91€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1461 | -0.050 | -218.43€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 26856 | -0.093 | -3592.49€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3709 | -0.094 | -601.27€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3709 | -0.094 | -601.27€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 1461 | -0.050 | -218.43€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 1461 | -0.050 | -218.43€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 3479 | -0.096 | -791.31€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 3479 | -0.096 | -791.31€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 7266 | -0.023 | -678.00€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 7266 | -0.023 | -678.00€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 6833 | -0.095 | -439.85€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 6833 | -0.095 | -439.85€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 5569 | -0.178 | -1082.05€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 5569 | -0.178 | -1082.05€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 18279 | -0.028 | +4030.00€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 4774 | +0.000 | +1822.08€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 13505 | -0.037 | +2207.91€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 18279 | -0.028 | +4030.00€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 4774 | +0.000 | +1822.08€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 13505 | -0.037 | +2207.91€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 93176 | +0.112 | -4745.92€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 14055 | +0.184 | -448.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 374 | -0.082 | -50.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 72693 | +0.100 | -4028.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 6054 | +0.107 | -218.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 12089 | +0.098 | -1016.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 45 | -0.160 | -0.32€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 12029 | +0.100 | -1004.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 18846 | +0.131 | -380.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 4429 | +0.203 | -141.16€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 12053 | +0.111 | -193.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 2322 | +0.104 | -23.83€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 12130 | +0.089 | -1129.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 52 | -0.093 | -6.98€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#DOGE#240min | 15 | -0.243 | -11.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 12063 | +0.090 | -1111.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 19824 | +0.123 | -405.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 5441 | +0.175 | -86.72€ | 1 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 12181 | +0.105 | -257.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 2190 | +0.098 | -53.47€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 18182 | +0.113 | -1091.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 4041 | +0.188 | -221.82€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 277 | -0.041 | +3.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 12322 | +0.091 | -731.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1542 | +0.125 | -140.91€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 12105 | +0.100 | -722.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 47 | -0.031 | +8.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 13 | -0.022 | -0.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 12045 | +0.101 | -730.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 14783 | +0.191 | -977.09€ | 1 | 5 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 14783 | +0.191 | -977.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 3520 | +0.167 | -371.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 3520 | +0.167 | -371.40€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 50915 | +0.197 | -4000.56€ | 3 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 50915 | +0.197 | -4000.56€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 8809 | +0.176 | -1020.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 8809 | +0.176 | -1020.95€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 8132 | +0.222 | -304.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 8132 | +0.222 | -304.98€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 8791 | +0.172 | -1058.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 8791 | +0.172 | -1058.69€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 8229 | +0.219 | -318.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 8229 | +0.219 | -318.78€ | 1 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 8413 | +0.203 | -553.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 8413 | +0.203 | -553.93€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 8541 | +0.193 | -743.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 8541 | +0.193 | -743.23€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 19202 | +0.118 | +173.52€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 19202 | +0.118 | +173.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 9531 | +0.122 | +152.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 9531 | +0.122 | +152.66€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 9671 | +0.113 | +20.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 9671 | +0.113 | +20.85€ | 0 | 4 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 35637 | +0.097 | -1123.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 2945 | +0.089 | +21.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 32692 | +0.097 | -1144.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC | 20011 | +0.101 | -334.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#15min | 2945 | +0.089 | +21.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#5min | 17066 | +0.103 | -356.28€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH | 6699 | +0.106 | -55.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH#5min | 6699 | +0.106 | -55.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL | 8927 | +0.080 | -733.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL#5min | 8927 | +0.080 | -733.33€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 25567 | +0.083 | +12121.18€ | 0 | 16 |
| ✅ GBM_LATE_15M#15min | 25567 | +0.083 | +12121.18€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 4274 | +0.193 | +3109.19€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 4274 | +0.193 | +3109.19€ | 0 | 19 |
| ✅ GBM_LATE_15M#BTC | 3796 | +0.177 | +2635.96€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 3796 | +0.177 | +2635.96€ | 0 | 27 |
| ✅ GBM_LATE_15M#DOGE | 4448 | +0.198 | +3313.20€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 4448 | +0.198 | +3313.20€ | 0 | 20 |
| ✅ GBM_LATE_15M#ETH | 3760 | +0.021 | +806.83€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 3760 | +0.021 | +806.83€ | 1 | 14 |
| ✅ GBM_LATE_15M#SOL | 3666 | -0.033 | +828.00€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 3666 | -0.033 | +828.00€ | 4 | 15 |
| ✅ GBM_LATE_15M#XRP | 5623 | -0.041 | +1427.99€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 5623 | -0.041 | +1427.99€ | 4 | 14 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 27010 | +0.086 | +14088.10€ | 0 | 19 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 27010 | +0.086 | +14088.10€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 5123 | +0.017 | +2692.93€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 5123 | +0.017 | +2692.93€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 5630 | +0.015 | +1171.31€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 5630 | +0.015 | +1171.31€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 3838 | +0.262 | +3860.69€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 3838 | +0.262 | +3860.69€ | 0 | 20 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 4386 | +0.003 | +837.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 4386 | +0.003 | +837.98€ | 2 | 14 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 4389 | +0.028 | +1654.90€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 4389 | +0.028 | +1654.90€ | 3 | 17 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 3644 | +0.275 | +3870.30€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 3644 | +0.275 | +3870.30€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 20599 | +0.168 | +15340.56€ | 0 | 26 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 20599 | +0.168 | +15340.56€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB | 3102 | +0.206 | +2460.62€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB#15min | 3102 | +0.206 | +2460.62€ | 0 | 21 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC | 3276 | +0.149 | +2402.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC#15min | 3276 | +0.149 | +2402.34€ | 0 | 24 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE | 3233 | +0.207 | +2555.13€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min | 3233 | +0.207 | +2555.13€ | 0 | 20 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH | 3461 | +0.133 | +2415.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH#15min | 3461 | +0.133 | +2415.08€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL | 3831 | +0.116 | +2609.50€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL#15min | 3831 | +0.116 | +2609.50€ | 0 | 24 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP | 3696 | +0.202 | +2897.90€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP#15min | 3696 | +0.202 | +2897.90€ | 0 | 27 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 5157 | +0.128 | +2187.39€ | 0 | 23 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 5157 | +0.128 | +2187.39€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BNB | 189 | +0.123 | +84.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BNB#15min | 189 | +0.123 | +84.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BTC | 1480 | +0.123 | +671.16€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BTC#15min | 1480 | +0.123 | +671.16€ | 0 | 19 |
...
| ✅ GBM_LATE_15M_TARDIO | 25682 | +0.175 | +19106.65€ | 0 | 22 |
| ✅ GBM_LATE_15M_TARDIO#15min | 25682 | +0.175 | +19106.65€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 4072 | +0.220 | +3429.91€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 4072 | +0.220 | +3429.91€ | 0 | 20 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 4025 | +0.151 | +2667.80€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 4025 | +0.151 | +2667.80€ | 0 | 26 |
| ✅ GBM_LATE_15M_TARDIO#DOGE | 4217 | +0.225 | +3621.50€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#DOGE#15min | 4217 | +0.225 | +3621.50€ | 0 | 20 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 4160 | +0.138 | +2853.05€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 4160 | +0.138 | +2853.05€ | 0 | 25 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 4497 | +0.113 | +2841.87€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 4497 | +0.113 | +2841.87€ | 0 | 19 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 4711 | +0.204 | +3692.52€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 4711 | +0.204 | +3692.52€ | 0 | 24 |
| ✅ GBM_LATE_5M | 6958 | +0.147 | +3958.85€ | 1 | 28 |
| ✅ GBM_LATE_5M#5min | 6958 | +0.147 | +3958.85€ | 0 | 0 |
| ✅ GBM_LATE_5M#BNB | 594 | +0.186 | +416.56€ | 0 | 0 |
| ✅ GBM_LATE_5M#BNB#5min | 594 | +0.186 | +416.56€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC | 1775 | +0.145 | +1157.72€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC#5min | 1775 | +0.145 | +1157.72€ | 0 | 30 |
| ✅ GBM_LATE_5M#DOGE | 889 | +0.171 | +564.50€ | 0 | 0 |
| ✅ GBM_LATE_5M#DOGE#5min | 889 | +0.171 | +564.50€ | 0 | 22 |
| ✅ GBM_LATE_5M#ETH | 2281 | +0.150 | +1285.93€ | 0 | 0 |
| ✅ GBM_LATE_5M#ETH#5min | 2281 | +0.150 | +1285.93€ | 0 | 30 |
...
| ✅ LEADLAG_BTC_XRP_15M | 1943 | +0.099 | +523.29€ | 0 | 2 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 1943 | +0.099 | +523.29€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 1943 | +0.099 | +523.29€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 1943 | +0.099 | +523.29€ | 0 | 2 |
...
| ✅ LIQUIDACIONES_5M | 1984 | +0.008 | +17.14€ | 4 | 0 |
| ✅ LIQUIDACIONES_5M#5min | 1984 | +0.008 | +17.14€ | 0 | 0 |
...
| ✅ LIQUIDACIONES_5M#ETH | 834 | +0.028 | +24.41€ | 0 | 0 |
| ✅ LIQUIDACIONES_5M#ETH#5min | 834 | +0.028 | +24.41€ | 6 | 0 |
| ✅ LIQUIDACIONES_5M#SOL | 470 | +0.000 | -4.93€ | 0 | 0 |
| ✅ LIQUIDACIONES_5M#SOL#5min | 470 | +0.000 | -4.93€ | 4 | 0 |
| ✅ LIQUIDACIONES_5M#XRP | 190 | -0.010 | -3.55€ | 0 | 0 |
| ✅ LIQUIDACIONES_5M#XRP#5min | 190 | -0.010 | -3.55€ | 1 | 0 |
...
| ✅ LIQUIDACIONES_DEPTH_FASE0 | 1168 | -0.038 | -26.70€ | 1 | 1 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#15min | 545 | -0.052 | -32.25€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#5min | 623 | -0.026 | +5.55€ | 0 | 0 |
...
| ✅ LIQUIDACIONES_DEPTH_FASE0#BTC | 295 | +0.029 | +36.10€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#BTC#15min | 134 | -0.007 | +4.00€ | 1 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#BTC#5min | 161 | +0.058 | +32.10€ | 0 | 4 |
...
| ✅ LIQUIDACIONES_DEPTH_FASE0#XRP | 275 | -0.056 | -18.57€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#XRP#15min | 124 | -0.064 | -11.50€ | 1 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#XRP#5min | 151 | -0.049 | -7.08€ | 5 | 2 |
| ✅ MOMENTUM_IBS_15M | 14394 | -0.011 | -205.76€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M#15min | 14394 | -0.011 | -205.76€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB | 578 | -0.010 | -0.50€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB#15min | 578 | -0.010 | -0.50€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BTC | 3111 | -0.020 | -58.94€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BTC#15min | 3111 | -0.020 | -58.94€ | 2 | 0 |
...
| ✅ MOMENTUM_IBS_15M_BALLENA | 28909 | -0.006 | +1309.69€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#15min | 28909 | -0.006 | +1309.69€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB | 5104 | +0.017 | +619.45€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB#15min | 5104 | +0.017 | +619.45€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC | 4480 | -0.027 | -46.89€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC#15min | 4480 | -0.027 | -46.89€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE | 5144 | +0.015 | +464.00€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE#15min | 5144 | +0.015 | +464.00€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH | 4257 | -0.050 | -128.20€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH#15min | 4257 | -0.050 | -128.20€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL | 4838 | -0.010 | +185.93€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL#15min | 4838 | -0.010 | +185.93€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP | 5086 | +0.007 | +215.40€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP#15min | 5086 | +0.007 | +215.40€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE | 5825 | -0.057 | -132.03€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#15min | 5825 | -0.057 | -132.03€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#BNB | 1207 | +0.000 | -14.75€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#BNB#15min | 1207 | +0.000 | -14.75€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#BTC | 1415 | -0.082 | -35.79€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#BTC#15min | 1415 | -0.082 | -35.79€ | 2 | 0 |
...
| ✅ MOMENTUM_IBS_15M_FADE#SOL | 1684 | -0.078 | -33.77€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#SOL#15min | 1684 | -0.078 | -33.77€ | 2 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 73332 | -0.072 | +1725.59€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 73332 | -0.072 | +1725.59€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 12399 | -0.079 | +713.85€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 12399 | -0.079 | +713.85€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 11313 | -0.092 | -489.12€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 11313 | -0.092 | -489.12€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 12607 | -0.067 | +699.44€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 12607 | -0.067 | +699.44€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 10841 | -0.093 | -171.39€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 10841 | -0.093 | -171.39€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 13436 | -0.048 | +408.55€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 13436 | -0.048 | +408.55€ | 3 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 12736 | -0.063 | +564.26€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 12736 | -0.063 | +564.26€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE | 7602 | -0.024 | -119.44€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 7602 | -0.024 | -119.44€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB | 996 | -0.017 | -19.84€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB#5min | 996 | -0.017 | -19.84€ | 1 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC | 1680 | -0.027 | -3.45€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC#5min | 1680 | -0.027 | -3.45€ | 2 | 0 |
...
| ✅ ORDER_FLOW_5M_REACTIVO | 486 | -0.066 | -62.12€ | 0 | 0 |
| ✅ ORDER_FLOW_5M_REACTIVO#5min | 486 | -0.066 | -62.12€ | 0 | 0 |
...
| ✅ ORDER_FLOW_5M_REACTIVO#ETH | 143 | -0.093 | -30.98€ | 0 | 0 |
| ✅ ORDER_FLOW_5M_REACTIVO#ETH#5min | 143 | -0.093 | -30.98€ | 0 | 0 |
...
| ✅ STREAK_FADE_5M | 2776 | -0.022 | -113.14€ | 0 | 0 |
| ✅ STREAK_FADE_5M#5min | 2776 | -0.022 | -113.14€ | 0 | 0 |
...
| ✅ STREAK_FADE_5M#XRP | 1247 | -0.020 | -47.99€ | 0 | 0 |
| ✅ STREAK_FADE_5M#XRP#5min | 1247 | -0.020 | -47.99€ | 3 | 0 |
...
| ✅ STREAK_MOM_5M | 7785 | +0.024 | +119.67€ | 0 | 0 |
| ✅ STREAK_MOM_5M#5min | 7785 | +0.024 | +119.67€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE | 2141 | +0.021 | +21.89€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE#5min | 2141 | +0.021 | +21.89€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH | 1759 | +0.035 | +53.63€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH#5min | 1759 | +0.035 | +53.63€ | 1 | 0 |
| ✅ STREAK_MOM_5M#SOL | 2359 | +0.013 | +5.73€ | 0 | 0 |
| ✅ STREAK_MOM_5M#SOL#5min | 2359 | +0.013 | +5.73€ | 1 | 0 |
| ✅ STREAK_MOM_5M#XRP | 1526 | +0.029 | +38.42€ | 0 | 0 |
| ✅ STREAK_MOM_5M#XRP#5min | 1526 | +0.029 | +38.42€ | 2 | 0 |
| ✅ STRUCT_NO_15M | 7322 | +0.012 | -41.07€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 7322 | +0.012 | -41.07€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 2942 | +0.016 | -6.90€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 2942 | +0.016 | -6.90€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 2863 | +0.012 | -16.52€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 2863 | +0.012 | -16.52€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 1517 | +0.004 | -17.65€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 1517 | +0.004 | -17.65€ | 2 | 0 |
| ✅ UPDOWN_GBM | 37310 | +0.030 | +2204.41€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 10114 | +0.068 | +1825.03€ | 0 | 11 |
| ✅ UPDOWN_GBM#240min | 1383 | +0.005 | +7.53€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 23393 | +0.019 | +352.40€ | 0 | 0 |
| ✅ UPDOWN_GBM#60min | 2277 | +0.005 | +20.36€ | 1 | 0 |
| ✅ UPDOWN_GBM#BNB | 3854 | +0.067 | +419.84€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 681 | +0.151 | +272.13€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#240min | 32 | +0.000 | -0.19€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#5min | 3141 | +0.049 | +147.90€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 6920 | +0.036 | +482.17€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 1284 | +0.083 | +293.33€ | 0 | 10 |
| ✅ UPDOWN_GBM#BTC#240min | 374 | +0.019 | +7.65€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#5min | 4185 | +0.033 | +161.16€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#60min | 1022 | +0.003 | +19.48€ | 1 | 0 |
| ✅ UPDOWN_GBM#BTC#daily | 55 | -0.097 | +0.55€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 4388 | +0.038 | +259.94€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 635 | +0.137 | +221.81€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#240min | 28 | +0.000 | -1.43€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 3725 | +0.022 | +39.56€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 7894 | +0.017 | +277.22€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 2590 | +0.046 | +272.59€ | 0 | 11 |
| ✅ UPDOWN_GBM#ETH#240min | 362 | +0.005 | +6.97€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 4123 | +0.004 | +0.15€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 773 | +0.001 | -5.69€ | 2 | 0 |
| ✅ UPDOWN_GBM#ETH#daily | 46 | -0.146 | +3.20€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 8822 | +0.014 | +211.48€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 2452 | +0.027 | +169.21€ | 0 | 12 |
| ✅ UPDOWN_GBM#SOL#240min | 354 | -0.006 | -2.31€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 5494 | +0.011 | +40.83€ | 1 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 482 | +0.015 | +6.57€ | 0 | 1 |
| ✅ UPDOWN_GBM#SOL#daily | 40 | -0.167 | -2.82€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 5430 | +0.035 | +555.59€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 2472 | +0.081 | +595.95€ | 0 | 12 |
| ✅ UPDOWN_GBM#XRP#240min | 233 | -0.002 | -3.16€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 2725 | -0.004 | -37.20€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 11980 | -0.039 | +2520.56€ | 3 | 6 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 11980 | -0.039 | +2520.56€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 778 | -0.042 | +346.45€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 778 | -0.042 | +346.45€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 2215 | -0.121 | +25.68€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 2215 | -0.121 | +25.68€ | 4 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 392 | +0.178 | +258.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 392 | +0.178 | +258.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 1323 | +0.207 | +781.30€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 1323 | +0.207 | +781.30€ | 1 | 23 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 3628 | -0.064 | +547.59€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 3628 | -0.064 | +547.59€ | 3 | 5 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 3644 | -0.076 | +561.00€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 3644 | -0.076 | +561.00€ | 3 | 4 |
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=2451 IC=+0.028 PNL=+169.72€
...
**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  ...
  - _Estado_: alineada_con_outcome_prev IC=+0.110 n=321/60 | contraria IC=+0.144 n=307 | gap=-0.034 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar

**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  - _Umbral_: n_overlaps≥20 y IC_overlap > IC_base + 0.05
  - _Acción_: Cambiar _aplicar_kelly_compuesto: match por activo, no market_id
  - _Estado_: n_overlaps=290, boost estimado=+0.001. Necesita 0 más y boost>0.05
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=772/40 IC=+0.000 PNL=-6.19€ | BTC#60min: n=1020/40 IC=+0.003 PNL=+19.53€ | SOL#60min: n=481/40 IC=+0.013 PNL=+6.06€

**⏳ H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  - _Umbral_: n≥40 tras 2 losses y gap(IC_tras_win - IC_tras_2loss)≥0.05
  - _Acción_: Reducir stake (no desactivar) 1-2h tras 2 derrotas consecutivas en el mismo subtype
  - _Estado_: tras_win IC=+0.050 n=335507 | tras_1loss IC=+0.078 n=261097 | tras_2loss IC=+0.047 n=110142/40 | gap=+0.003 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-60MIN-BUYYES** — GBM 60min BUY_YES — ¿edge superior al BUY_NO?
  ...
  - _Estado_: n=1604 IC=+0.013 PNL=+9.94€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1604 IC=+0.013 PNL=+9.94€

**〰️ H-CUSTOM-GBM-60MIN-BUYNO** — GBM 60min BUY_NO — tracking por separado
  - _Hipótesis_: En 15min BUY_NO tiene IC=+0.119. ¿Se repite en 60min? Datos actuales: 8/14 (57%) IC=+0.044 — positivo pero débil. Puede ser que 60min requiera dirección alcista (BUY_YES) y no bajista.
  - _Umbral_: n≥30 para confirmar dirección
  - _Acción_: Si IC<0.05 con n≥30 → en 60min priorizar solo BUY_YES; si IC>0.08 → igualar al BUY_YES
  - _Estado_: n=669 IC=-0.016 PNL=+9.46€ — sin señal clara aún (umbral IC: min=0.05 max=None)
  - _Datos_: n=669 IC=-0.016 PNL=+9.46€
...
**🟡 H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.186 > 0.1 con n=2151 PNL=+1340.45€
  - _Datos_: n=2151 IC=+0.186 PNL=+1340.45€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=1282 IC=+0.083 PNL=+293.38€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=1282 IC=+0.083 PNL=+293.38€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=13031 IC=+0.053 PNL=+1570.89€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=13031 IC=+0.053 PNL=+1570.89€
...
**🔴 H-CUSTOM-BUYNO-LONGSHOT-15MIN** — BUY_NO longshot en 15min (py_mkt≥0.55) — comprar NO barato pierde
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.157 < -0.1 con n=234 PNL=+22.90€
  - _Datos_: n=234 IC=-0.157 PNL=+22.90€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=1904 IC=+0.049 PNL=+201.21€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1904 IC=+0.049 PNL=+201.21€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=17954 IC=-0.136 PNL=+1244.18€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=17954 IC=-0.136 PNL=+1244.18€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=1933 IC=+0.139 PNL=+1052.74€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=1933 IC=+0.139 PNL=+1052.74€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.238 < -0.1 con n=1749 PNL=-195.70€
  - _Datos_: n=1749 IC=-0.238 PNL=-195.70€
...
**🔶 H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.093 n=1000) pero sin cruzar ≥2 pares más — sin otros pares con datos
  - _Datos_: n=1000 IC=+0.093 PNL=+229.00€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=8804 IC=+0.176 PNL=-1021.36€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=8804 IC=+0.176 PNL=-1021.36€
