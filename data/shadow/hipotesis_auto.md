# Hipótesis automáticas — 2026-07-15 07:02 UTC
_Generado por shadow_postmortem.py sobre 14959 resoluciones (PNL=+2916.94€)_
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.219 (n=176)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `3727.5827` → IC=+0.214 (n=173)
...
- **PATRÓN** `py_entrada` < `0.575` → IC=+0.309 (n=19)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.575 (IC base=+0.232)

- **PATRÓN** `py_entrada` > `0.635` → IC=+0.237 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.635 (IC base=+0.232)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.201 (n=182)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.197)

- **PATRÓN** `py_entrada` < `0.365` → IC=+0.250 (n=154)
...
- **PATRÓN** `py_entrada` < `0.65` → IC=+0.206 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.65 (IC base=+0.200)
...
- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.136 (n=1153)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.004` → IC=+0.196 (n=44)
  ...
  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 3.004 (IC base=+0.060)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.172 (n=291)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` < 8.0 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` > `0.7916` → IC=+0.242 (n=87)
...
- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.182 (n=658)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `1.0626` → IC=+0.186 (n=49)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 1.0626 (IC base=+0.161)

- **PATRÓN** `dist_vwap_pct` < `0.1925` → IC=+0.205 (n=384)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1925 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.956` → IC=+0.245 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.956 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.13` → IC=+0.178 (n=321)
...
- **PATRÓN** `sigma_h` > `0.0027` → IC=+0.182 (n=168)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.1537` → IC=+0.140 (n=109)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.1537 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.703` → IC=+0.278 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.703 (IC base=+0.140)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.138 (n=147)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0066 (IC base=+0.130)

- **PATRÓN** `sigma_h` > `0.0041` → IC=+0.156 (n=149)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` > 0.0041 (IC base=+0.130)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.172 (n=59)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` < 5.0 (IC base=+0.130)

- **PATRÓN** `dist_vwap_pct` > `0.759` → IC=+0.188 (n=30)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.759 (IC base=+0.130)

- **PATRÓN** `dist_vwap_pct` < `1.1817` → IC=+0.159 (n=121)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 1.1817 (IC base=+0.130)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.42` → IC=+0.192 (n=37)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 5.42 (IC base=+0.130)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.079` → IC=+0.160 (n=92)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` < 10.079 (IC base=+0.130)
...
- **PATRÓN** `sigma_h` > `0.013` → IC=+0.212 (n=78)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.4922` → IC=+0.188 (n=46)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.4922 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `0.9751` → IC=+0.137 (n=111)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.9751 (IC base=+0.133)
...
- **PATRÓN** `dist_vwap_pct` > `0.7118` → IC=+0.174 (n=84)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.7118 (IC base=+0.095)
...
- **PATRÓN** `dist_vwap_pct` < `0.1754` → IC=+0.124 (n=320)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.1754 (IC base=+0.083)
...
- **PATRÓN** `sigma_h` < `0.0037` → IC=+0.140 (n=48)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0037 (IC base=+0.036)

- **PATRÓN** `dist_vwap_pct` > `0.6294` → IC=+0.167 (n=31)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.6294 (IC base=+0.036)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.728` → IC=+0.324 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.728 (IC base=+0.036)
...
- **PATRÓN** `ibs_15` < `0.0314` → IC=+0.137 (n=133)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.69€ cuando `ibs_15` < 0.0314 (IC base=+0.049)

- **PATRÓN** `dist_vwap_pct` > `0.431` → IC=+0.125 (n=30)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` > 0.431 (IC base=+0.049)

- **PATRÓN** `sigma_ewma_delta_pct` < `15.796` → IC=+0.139 (n=117)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` < 15.796 (IC base=+0.049)
...
- **PATRÓN** `dist_vwap_pct` > `0.6579` → IC=+0.152 (n=21)
  ...
  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.6579 (IC base=+0.049)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2042 | +0.177 | -30.54€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1319 | +0.208 | +4.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 85 | -0.006 | -17.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 291 | +0.104 | -37.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 347 | +0.165 | +21.02€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 724 | +0.180 | +15.28€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 435 | +0.221 | +23.46€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 28 | -0.133 | -11.56€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 145 | +0.119 | -12.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 116 | +0.178 | +15.86€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL | 718 | +0.175 | -26.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 437 | +0.211 | +8.67€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 29 | +0.048 | -3.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 136 | +0.116 | -21.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 116 | +0.136 | -9.65€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO#XRP | 6 | -0.037 | -4.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 6 | -0.037 | -4.17€ | 0 | 0 |
| ✅ GBM_LATE_15M | 4498 | +0.102 | +1378.61€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 4498 | +0.102 | +1378.61€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1077 | +0.067 | +160.31€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1077 | +0.067 | +160.31€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1101 | +0.074 | +183.32€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1101 | +0.074 | +183.32€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1197 | +0.112 | +494.51€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1197 | +0.112 | +494.51€ | 0 | 6 |
| ✅ GBM_LATE_15M#XRP | 1123 | +0.152 | +540.48€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1123 | +0.152 | +540.48€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1768 | +0.151 | +1028.06€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1768 | +0.151 | +1028.06€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 451 | +0.125 | +228.88€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 451 | +0.125 | +228.88€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 467 | +0.123 | +208.36€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 467 | +0.123 | +208.36€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 478 | +0.108 | +206.44€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 478 | +0.108 | +206.44€ | 1 | 10 |
...
| ✅ GBM_LATE_15M_TARDIO | 1715 | +0.083 | +469.89€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1715 | +0.083 | +469.89€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 423 | +0.062 | +70.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 423 | +0.062 | +70.53€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 380 | +0.011 | +9.76€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 380 | +0.011 | +9.76€ | 0 | 3 |
...
| ✅ GBM_LATE_5M | 9 | -0.021 | -0.61€ | 0 | 0 |
| ✅ GBM_LATE_5M#5min | 9 | -0.021 | -0.61€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 1538 | +0.009 | +70.92€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1281 | +0.036 | +107.45€ | 1 | 7 |
| 🚫 UPDOWN_GBM#240min | 11 | -0.148 | -4.31€ | 0 | 0 |
| 🚫 UPDOWN_GBM#5min | 75 | -0.162 | -22.48€ | 4 | 0 |
| ✅ UPDOWN_GBM#60min | 127 | -0.058 | -10.76€ | 7 | 0 |
| ✅ UPDOWN_GBM#BNB | 75 | +0.071 | +15.46€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 75 | +0.071 | +15.46€ | 0 | 0 |
...
| ✅ UPDOWN_GBM#ETH | 740 | +0.019 | +38.42€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 647 | +0.036 | +49.47€ | 1 | 3 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.121) — sin ventaja clara. oversold(IBS<0.3): IC=-0.008 n=425 | neutral: IC=-0.022 n=322 | overbought(IBS>0.7): IC=+0.099 n=340
  - _Datos_: n=1148 IC=+0.026 PNL=+96.16€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.098 n=8612 | tras_1loss IC=+0.060 n=6163 | tras_2loss IC=+0.027 n=2681/40 | gap=+0.071 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1411 IC=+0.016 PNL=+84.92€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1411 IC=+0.016 PNL=+84.92€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=672 IC=+0.074 PNL=+87.97€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=672 IC=+0.074 PNL=+87.97€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=560 IC=+0.034 PNL=+41.23€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=560 IC=+0.034 PNL=+41.23€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=12501 IC=+0.101 PNL=+2992.82€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=12501 IC=+0.101 PNL=+2992.82€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.165 > 0.1 con n=383 PNL=+134.96€
  - _Datos_: n=383 IC=+0.165 PNL=+134.96€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=460 IC=-0.084 PNL=+63.65€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=460 IC=-0.084 PNL=+63.65€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=574 IC=+0.059 PNL=+68.21€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=574 IC=+0.059 PNL=+68.21€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema, sin filtrar todavía
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.186 < -0.1 con n=384 PNL=-86.75€
  - _Datos_: n=384 IC=-0.186 PNL=-86.75€
...
**🔶 H-CUSTOM-ETH15-BUYNO-TARDIO** — UPDOWN_GBM ETH#15min BUY_NO tardío (T_h<0.2) -- edge fuerte no capturado por el aprendizaje causal automático
  ...
  - _Estado_: SEÑAL POSITIVA en ETH (IC=+0.272 n=55) pero sin cruzar ≥2 pares más — BNB: n=9 IC=+0.061; BTC: n=12 IC=+0.043; DOGE: n=8 IC=+0.080; SOL: n=8 IC=+0.000; XRP: n=9 IC=+0.061
