# Hipótesis automáticas — 2026-07-03 15:05 UTC
_Generado por shadow_postmortem.py sobre 3101 resoluciones (PNL=-38.69€)_

## Patrones causales activos

### UPDOWN_GBM#15min
- **FILTRO** `ibs_15` > `0.3451` → IC=-0.122 (n=43)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: SKIP cuando `ibs_15` > 0.3451
  - _Potencial_: sin este filtro IC_bueno=+0.041 (n=131)

- **PATRÓN** `sigma_h` < `0.0097` → IC=+0.195 (n=80)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0097 (IC base=+0.035)

- **PATRÓN** `ibs_15` < `0.0634` → IC=+0.167 (n=58)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.83€ cuando `ibs_15` < 0.0634 (IC base=+0.035)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1297` → IC=+0.141 (n=62)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.70€ cuando `pct_spot_vs_ref` |x|≤ 0.1297 (IC base=+0.107)

- **PATRÓN** `sigma_h` < `0.0089` → IC=+0.182 (n=42)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0089 (IC base=+0.107)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.192 (n=24)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 7.0 (IC base=+0.107)

- **PATRÓN** `ibs_15` < `0.08` → IC=+0.243 (n=33)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.08 (IC base=+0.107)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS < 0.0634 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.167 n=58). Confirma señal de reversión media → alinear con BUY_YES.
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.08 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.243 n=33). Confirma señal de reversión media → alinear con BUY_YES.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 438 | +0.052 | +57.16€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 438 | +0.052 | +57.16€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 113 | +0.013 | +3.35€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 113 | +0.013 | +3.35€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 112 | -0.018 | -0.17€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 112 | -0.018 | -0.17€ | 0 | 0 |
...
| ✅ ORDER_FLOW_5M | 1525 | +0.013 | +11.21€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#5min | 1389 | +0.008 | -1.38€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#BNB | 191 | +0.049 | +7.28€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#BNB#5min | 191 | +0.049 | +7.28€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#BTC | 261 | -0.025 | -8.66€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#BTC#5min | 261 | -0.025 | -8.66€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 863 | -0.041 | -50.49€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 633 | -0.020 | -22.65€ | 1 | 2 |
...
| ✅ UPDOWN_GBM#BTC | 250 | -0.044 | -35.20€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 179 | -0.019 | -24.49€ | 3 | 0 |
...
| ✅ UPDOWN_GBM#ETH | 341 | -0.010 | +8.19€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 257 | +0.006 | +14.66€ | 3 | 4 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.063) — sin ventaja clara. oversold(IBS<0.3): IC=-0.006 n=164 | neutral: IC=-0.069 n=142 | overbought(IBS>0.7): IC=-0.049 n=151
  - _Datos_: n=473 IC=-0.041 PNL=-25.26€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=01h: IC=-0.200 n=28 PNL=-13.74€ → FILTRAR | H=02h: IC=-0.180 n=23 PNL=-6.09€ → FILTRAR | H=03h: IC=-0.110 n=15 PNL=-5.87€ → FILTRAR | H=10h: IC=-0.154 n=15 PNL=-6.77€ → FILTRAR | H=15h: IC=-0.109 n=21 PNL=-6.63€ → FILTRAR | H=16h: IC=+0.140 n=23 PNL=+3.65€ → BOOST | H=20h: IC=-0.143 n=26 PNL=-5.82€ → FILTRAR | H=23h: IC=+0.180 n=18 PNL=+12.32€ → BOOST

**🟡 H-OF-PAR** — ORDER_FLOW per-pair delta_ratio ranges
  - _Umbral_: n≥200 por par con delta_ratio feature en shadow
  - _Acción_: Añadir DELTA_MIN/MAX por par dict en shadow_predict.py
  - _Estado_: BTC: 157 ops con delta_ratio | SOL: 198 ops con delta_ratio
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.009 n=1488 | tras_1loss IC=-0.027 n=1521 | tras_2loss IC=-0.043 n=783/40 | gap=+0.051 (umbral 0.05)
...
**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  ...
  - _Estado_: alineada_con_outcome_prev IC=+0.067 n=17/60 | contraria IC=+0.040 n=8 | gap=+0.027 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar

**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  - _Umbral_: n_overlaps≥20 y IC_overlap > IC_base + 0.05
  - _Acción_: Cambiar _aplicar_kelly_compuesto: match por activo, no market_id
  - _Estado_: n_overlaps=25, boost estimado=+0.014. Necesita 0 más y boost>0.05
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=-0.052 n=132/40 PNL=-4.05€ | H=15h UTC: IC=+0.016 n=120/40 PNL=+5.37€ | H=17h UTC: IC=+0.200 n=58/40 PNL=+21.47€ | H=19h UTC: IC=-0.013 n=72/40 PNL=-9.58€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=740 IC=-0.036 PNL=-36.94€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=740 IC=-0.036 PNL=-36.94€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=392 IC=-0.053 PNL=-46.97€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=392 IC=-0.053 PNL=-46.97€
...
**🔴 H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.019 < 0.02 con n=179 PNL=-24.49€
  - _Datos_: n=179 IC=-0.019 PNL=-24.49€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=272 IC=-0.018 PNL=-32.94€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=272 IC=-0.018 PNL=-32.94€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=223 IC=+0.011 PNL=+12.66€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=223 IC=+0.011 PNL=+12.66€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=675 IC=+0.033 PNL=+51.47€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=675 IC=+0.033 PNL=+51.47€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=47 IC=-0.010 PNL=-6.97€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=47 IC=-0.010 PNL=-6.97€
...
**🔴 H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.076 < -0.05 con n=534 PNL=-74.89€
  - _Datos_: n=534 IC=-0.076 PNL=-74.89€
