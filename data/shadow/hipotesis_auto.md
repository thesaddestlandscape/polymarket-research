# Hipótesis automáticas — 2026-08-26 21:32 UTC
_Generado por shadow_postmortem.py sobre 166515 resoluciones (PNL=+11954.66€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `148.37` → IC=-0.282 (n=2300)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 148.37
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=6901)
...
- **FILTRO** `restante_s_al_confirmar` < `158.28` → IC=-0.192 (n=627)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.28
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=1884)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `152.74` → IC=-0.268 (n=542)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 152.74
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=1626)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.224 (n=624)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.213 (n=766)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.210)

- **PATRÓN** `libro_liquidez` > `12720.9358` → IC=+0.224 (n=201)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12720.9358 (IC base=+0.210)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.133 (n=1191)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.195 (n=939)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.190 (n=4421)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.201 (n=2120)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.168 (n=765)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.72` → IC=+0.173 (n=959)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=1130)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.240 (n=1008)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.191 (n=1081)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.200 (n=4548)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.220 (n=8363)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.190)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.128 (n=2180)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.237 (n=1706)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `4.46` → IC=+0.160 (n=1425)
...
- **PATRÓN** `restante_min` < `4.27` → IC=+0.141 (n=943)

  - _Acción_: Kelly boost +0.71€ cuando `restante_min` < 4.27 (IC base=+0.141)

- **PATRÓN** `restante_min` > `4.91` → IC=+0.173 (n=726)

  - _Acción_: Kelly boost +0.87€ cuando `restante_min` > 4.91 (IC base=+0.141)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.160 (n=1026)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 8.0 (IC base=+0.141)

- **PATRÓN** `lag_apertura_s` < `5.59` → IC=+0.176 (n=710)

  - _Acción_: Kelly boost +0.88€ cuando `lag_apertura_s` < 5.59 (IC base=+0.141)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.195 (n=805)

  - _Acción_: Kelly boost +0.98€ cuando `py_entrada` < 0.38 (IC base=+0.126)

- **PATRÓN** `restante_min` < `4.41` → IC=+0.130 (n=952)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.129 (n=2155)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `3.47` → IC=+0.163 (n=717)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `lag_apertura_s` < 3.47 (IC base=+0.126)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.303 (n=283)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.03` → IC=+0.301 (n=340)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.03 (IC base=+0.286)

- **PATRÓN** `libro_liquidez` > `1563.731` → IC=+0.341 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1563.731 (IC base=+0.286)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9487` → IC=+0.208 (n=727)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9487 (IC base=+0.064)

- **PATRÓN** `dist_vwap_pct` > `0.2093` → IC=+0.234 (n=291)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2093 (IC base=+0.064)

- **PATRÓN** `dist_vwap_pct` < `0.7404` → IC=+0.214 (n=467)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.7404 (IC base=+0.064)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.223` → IC=+0.170 (n=892)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 5.223 (IC base=+0.064)

- **PATRÓN** `volumen_regimen` < `0.6325` → IC=+0.220 (n=155)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6325 (IC base=+0.064)

- **PATRÓN** `volumen_regimen` > `1.0569` → IC=+0.246 (n=211)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0569 (IC base=+0.064)

- **PATRÓN** `volumen_pendiente_norm` > `0.3074` → IC=+0.141 (n=165)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_pendiente_norm` > 0.3074 (IC base=+0.064)

- **PATRÓN** `volumen_spike_ratio` < `2.4495` → IC=+0.139 (n=1026)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 2.4495 (IC base=+0.064)

- **PATRÓN** `ibs_20min` < `0.2133` → IC=+0.131 (n=1486)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.2133 (IC base=+0.031)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.766` → IC=+0.324 (n=174)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.766 (IC base=+0.115)
...
- **PATRÓN** `sigma_h` < `0.0034` → IC=+0.247 (n=160)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.0953` → IC=+0.224 (n=121)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0953 (IC base=+0.210)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.236 (n=365)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.4244` → IC=+0.225 (n=361)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4244 (IC base=+0.210)

- **PATRÓN** `dist_vwap_pct` > `0.2276` → IC=+0.234 (n=235)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2276 (IC base=+0.210)

- **PATRÓN** `dist_vwap_pct` < `0.9628` → IC=+0.210 (n=384)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.9628 (IC base=+0.210)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.271` → IC=+0.253 (n=95)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2657` → IC=+0.276 (n=47)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2657 (IC base=+0.210)

- **PATRÓN** `volumen_spike_ratio` < `1.3527` → IC=+0.265 (n=113)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3527 (IC base=+0.210)

- **PATRÓN** `libro_liquidez` > `11253.359` → IC=+0.220 (n=323)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11253.359 (IC base=+0.210)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0713` → IC=+0.135 (n=146)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.0713 (IC base=+0.128)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.931` → IC=+0.269 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.931 (IC base=+0.128)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.167 (n=205)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.01 (IC base=+0.128)

- **PATRÓN** `libro_liquidez` > `1919.501` → IC=+0.136 (n=196)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 1919.501 (IC base=+0.128)
...
- **FILTRO** `ibs_20min` > `0.4872` → IC=-0.121 (n=634)

  - _Acción_: SKIP cuando `ibs_20min` > 0.4872
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=635)

- **FILTRO** `sigma_ewma_delta_pct` > `4.919` → IC=-0.156 (n=271)
...
- **FILTRO** `ibs_20min` > `0.7761` → IC=-0.169 (n=285)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7761
  - _Potencial_: sin este filtro IC_bueno=+0.012 (n=859)

- **FILTRO** `sigma_ewma_delta_pct` > `6.601` → IC=-0.162 (n=190)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 6.601
  - _Potencial_: sin este filtro IC_bueno=-0.007 (n=954)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.132 (n=1251)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0075 (IC base=+0.052)

- **PATRÓN** `ibs_20min` > `0.9429` → IC=+0.246 (n=919)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.115` → IC=+0.165 (n=1222)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` < 0.115 (IC base=+0.052)

- **PATRÓN** `volumen_pendiente_norm` > `0.2472` → IC=+0.209 (n=242)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2472 (IC base=+0.052)

- **PATRÓN** `volumen_spike_ratio` < `1.4786` → IC=+0.189 (n=419)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` < 1.4786 (IC base=+0.052)

- **PATRÓN** `volumen_spike_ratio` > `2.85` → IC=+0.172 (n=419)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 2.85 (IC base=+0.052)

- **PATRÓN** `ballena_activa_n` < `100.0` → IC=+0.265 (n=703)
...
- **PATRÓN** `dist_vwap_pct` < `0.2079` → IC=+0.204 (n=727)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2079 (IC base=+0.040)
...
- **FILTRO** `ibs_20min` > `0.8811` → IC=-0.141 (n=268)
...
- **PATRÓN** `drift_60min` |x|≤ `0.099` → IC=+0.209 (n=187)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.099 (IC base=+0.179)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0195` → IC=+0.173 (n=151)
  ...
  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.0195 (IC base=+0.179)

- **PATRÓN** `volumen_spike_ratio` > `3.6591` → IC=+0.177 (n=156)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 3.6591 (IC base=+0.179)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.225 (n=198)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.179)

- **PATRÓN** `libro_liquidez` > `1916.3861` → IC=+0.213 (n=193)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1916.3861 (IC base=+0.179)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.162 (n=66)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=896)
...
- **FILTRO** `sigma_ewma_delta_pct` > `4.583` → IC=-0.123 (n=213)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.583
  - _Potencial_: sin este filtro IC_bueno=+0.005 (n=669)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.224 (n=208)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.8983` → IC=+0.293 (n=366)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8983 (IC base=+0.212)

- **PATRÓN** `dist_vwap_pct` > `1.4169` → IC=+0.335 (n=125)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.4169 (IC base=+0.212)
...
- **PATRÓN** `sigma_h` > `0.0097` → IC=+0.193 (n=793)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0097 (IC base=+0.154)

- **PATRÓN** `drift_60min` |x|≤ `0.325` → IC=+0.156 (n=2093)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.325 (IC base=+0.154)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.283 (n=1179)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.154)

- **PATRÓN** `dist_vwap_pct` > `0.7834` → IC=+0.251 (n=545)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7834 (IC base=+0.154)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.334` → IC=+0.249 (n=1012)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.334 (IC base=+0.154)

- **PATRÓN** `volumen_regimen` > `0.6201` → IC=+0.167 (n=1653)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 0.6201 (IC base=+0.154)

- **PATRÓN** `volumen_pendiente_norm` > `0.1017` → IC=+0.180 (n=847)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1017 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` < `2.2828` → IC=+0.152 (n=1876)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 2.2828 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` > `1.8529` → IC=+0.147 (n=1421)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.8529 (IC base=+0.154)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.166 (n=1948)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.01 (IC base=+0.154)

- **PATRÓN** `libro_liquidez` > `3939.7883` → IC=+0.188 (n=793)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3939.7883 (IC base=+0.154)

- **PATRÓN** `ballena_activa_n` < `125.0` → IC=+0.173 (n=1135)

  - _Acción_: Kelly boost +0.86€ cuando `ballena_activa_n` < 125.0 (IC base=+0.154)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2465` → IC=+0.150 (n=384)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.02` → IC=+0.348 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.02 (IC base=+0.143)

- **PATRÓN** `volumen_pendiente_norm` > `0.1422` → IC=+0.167 (n=91)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.1422 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` > `1.4227` → IC=+0.130 (n=306)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` > 1.4227 (IC base=+0.143)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.167 (n=226)
...
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.235 (n=115)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.211 (n=337)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.179)

- **PATRÓN** `ibs_20min` > `0.9974` → IC=+0.286 (n=152)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9974 (IC base=+0.179)

- **PATRÓN** `dist_vwap_pct` > `0.2074` → IC=+0.251 (n=243)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2074 (IC base=+0.179)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.777` → IC=+0.248 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.777 (IC base=+0.179)

- **PATRÓN** `volumen_regimen` < `1.2894` → IC=+0.183 (n=336)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 1.2894 (IC base=+0.179)

- **PATRÓN** `volumen_regimen` > `0.8896` → IC=+0.203 (n=224)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8896 (IC base=+0.179)

- **PATRÓN** `volumen_pendiente_norm` > `0.2159` → IC=+0.261 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2159 (IC base=+0.179)

- **PATRÓN** `volumen_spike_ratio` < `1.3851` → IC=+0.233 (n=103)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3851 (IC base=+0.179)

- **PATRÓN** `libro_liquidez` > `12067.4093` → IC=+0.208 (n=224)
...
- **PATRÓN** `drift_60min` |x|≤ `0.144` → IC=+0.158 (n=229)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.144 (IC base=+0.149)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.159 (n=136)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 16.0 (IC base=+0.149)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.904` → IC=+0.298 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.904 (IC base=+0.149)

- **PATRÓN** `volumen_pendiente_norm` < `0.2288` → IC=+0.144 (n=282)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_pendiente_norm` < 0.2288 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` < `1.9992` → IC=+0.201 (n=125)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9992 (IC base=+0.149)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.194 (n=158)

  - _Acción_: Kelly boost +0.97€ cuando `libro_spread` < 0.01 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `1961.8084` → IC=+0.175 (n=115)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 1961.8084 (IC base=+0.149)
...
- **PATRÓN** `volumen_regimen` > `0.6356` → IC=+0.198 (n=339)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` > 0.6356 (IC base=+0.182)

- **PATRÓN** `volumen_pendiente_norm` > `0.0999` → IC=+0.236 (n=146)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0999 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` < `1.4303` → IC=+0.214 (n=110)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4303 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` > `2.4523` → IC=+0.230 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4523 (IC base=+0.182)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.185 (n=385)

  - _Acción_: Kelly boost +0.92€ cuando `libro_spread` < 0.01 (IC base=+0.182)

- **PATRÓN** `libro_liquidez` > `8687.9043` → IC=+0.210 (n=226)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8687.9043 (IC base=+0.182)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.177 (n=184)

  - _Acción_: Kelly boost +0.89€ cuando `ballena_activa_n` < 156.0 (IC base=+0.182)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.130 (n=493)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.7814` → IC=+0.275 (n=149)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7814 (IC base=+0.116)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.202` → IC=+0.289 (n=107)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.202 (IC base=+0.116)

- **PATRÓN** `volumen_regimen` > `0.6783` → IC=+0.136 (n=421)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 0.6783 (IC base=+0.116)

- **PATRÓN** `volumen_pendiente_norm` > `0.2806` → IC=+0.134 (n=69)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_pendiente_norm` > 0.2806 (IC base=+0.116)

- **PATRÓN** `volumen_spike_ratio` > `1.5631` → IC=+0.130 (n=393)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` > 1.5631 (IC base=+0.116)

- **PATRÓN** `libro_liquidez` > `2917.9058` → IC=+0.190 (n=214)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 2917.9058 (IC base=+0.116)
...
- **PATRÓN** `sigma_h` > `0.0227` → IC=+0.187 (n=231)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0227 (IC base=+0.161)

- **PATRÓN** `drift_60min` |x|≤ `0.1619` → IC=+0.190 (n=224)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.1619 (IC base=+0.161)
...
- **PATRÓN** `ibs_20min` > `0.9245` → IC=+0.260 (n=339)
...
- **PATRÓN** `volumen_regimen` > `0.8236` → IC=+0.183 (n=339)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` > 0.8236 (IC base=+0.161)

- **PATRÓN** `volumen_pendiente_norm` > `0.0804` → IC=+0.219 (n=194)
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.183 (n=200)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 16.0 (IC base=+0.084)

- **PATRÓN** `ibs_20min` > `0.6267` → IC=+0.151 (n=365)

  - _Acción_: Kelly boost +0.76€ cuando `ibs_20min` > 0.6267 (IC base=+0.084)

- **PATRÓN** `dist_vwap_pct` > `0.7767` → IC=+0.245 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7767 (IC base=+0.084)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.306` → IC=+0.200 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.306 (IC base=+0.084)

- **PATRÓN** `volumen_pendiente_norm` > `0.1717` → IC=+0.186 (n=103)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1717 (IC base=+0.084)

- **PATRÓN** `libro_liquidez` > `2920.0962` → IC=+0.128 (n=272)

  - _Acción_: Kelly boost +0.64€ cuando `libro_liquidez` > 2920.0962 (IC base=+0.084)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.274 (n=29)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.032)

- **PATRÓN** `ibs_20min` > `0.9291` → IC=+0.244 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9291 (IC base=+0.032)

- **PATRÓN** `dist_vwap_pct` > `0.2015` → IC=+0.151 (n=41)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.2015 (IC base=+0.032)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.351` → IC=+0.167 (n=37)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 4.351 (IC base=+0.032)

- **PATRÓN** `libro_liquidez` > `11295.2956` → IC=+0.143 (n=54)

  - _Acción_: Kelly boost +0.71€ cuando `libro_liquidez` > 11295.2956 (IC base=+0.032)
...
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.278 (n=79)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0058 (IC base=+0.277)

- **PATRÓN** `sigma_h` > `0.0045` → IC=+0.306 (n=60)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0045 (IC base=+0.277)

- **PATRÓN** `drift_60min` |x|≤ `0.224` → IC=+0.290 (n=60)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.224 (IC base=+0.277)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.340 (n=48)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 16.0 (IC base=+0.277)

- **PATRÓN** `ibs_20min` > `0.999` → IC=+0.314 (n=41)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.999 (IC base=+0.277)

- **PATRÓN** `dist_vwap_pct` > `0.1421` → IC=+0.325 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1421 (IC base=+0.277)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.648` → IC=+0.378 (n=47)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.648 (IC base=+0.277)

- **PATRÓN** `volumen_regimen` < `0.6873` → IC=+0.381 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6873 (IC base=+0.277)

- **PATRÓN** `volumen_pendiente_norm` > `0.1824` → IC=+0.386 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1824 (IC base=+0.277)

- **PATRÓN** `volumen_spike_ratio` < `2.2928` → IC=+0.292 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.2928 (IC base=+0.277)

- **PATRÓN** `volumen_spike_ratio` > `1.5464` → IC=+0.295 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5464 (IC base=+0.277)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0079` → IC=+0.206 (n=1251)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0079 (IC base=+0.146)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.155 (n=2508)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 8.0 (IC base=+0.146)

- **PATRÓN** `ibs_20min` > `0.9432` → IC=+0.290 (n=1250)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9432 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `1.0689` → IC=+0.251 (n=432)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0689 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.355` → IC=+0.247 (n=1168)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.355 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` < `0.7021` → IC=+0.150 (n=859)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 0.7021 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` > `1.0788` → IC=+0.151 (n=884)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.0788 (IC base=+0.146)

- **PATRÓN** `volumen_pendiente_norm` > `0.1651` → IC=+0.184 (n=698)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.1651 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` > `1.8767` → IC=+0.159 (n=1627)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 1.8767 (IC base=+0.146)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.154 (n=2252)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.146)

- **PATRÓN** `libro_liquidez` > `3897.8392` → IC=+0.194 (n=919)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 3897.8392 (IC base=+0.146)

- **PATRÓN** `ballena_activa_n` < `160.0` → IC=+0.190 (n=1215)
...
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.219 (n=201)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.007 (IC base=+0.136)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.167 (n=298)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 11.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` > `0.8232` → IC=+0.248 (n=296)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8232 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.565` → IC=+0.354 (n=169)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.565 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.2139` → IC=+0.190 (n=69)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.2139 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.9026` → IC=+0.135 (n=242)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` > 1.9026 (IC base=+0.136)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.177 (n=230)

  - _Acción_: Kelly boost +0.88€ cuando `libro_spread` < 0.04 (IC base=+0.136)
...
- **PATRÓN** `volumen_regimen` < `1.2659` → IC=+0.160 (n=436)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2659 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` > `1.0896` → IC=+0.190 (n=198)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_regimen` > 1.0896 (IC base=+0.157)

- **PATRÓN** `volumen_pendiente_norm` > `0.1452` → IC=+0.234 (n=107)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1452 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` < `2.1158` → IC=+0.185 (n=341)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 2.1158 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` > `1.7052` → IC=+0.196 (n=258)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.7052 (IC base=+0.157)

- **PATRÓN** `libro_liquidez` > `11754.5395` → IC=+0.193 (n=291)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 11754.5395 (IC base=+0.157)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0753` → IC=+0.185 (n=122)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.0753 (IC base=+0.178)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.241 (n=164)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.178)

- **PATRÓN** `ibs_20min` > `0.717` → IC=+0.258 (n=325)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.717 (IC base=+0.178)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.106` → IC=+0.343 (n=100)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.106 (IC base=+0.178)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0881` → IC=+0.187 (n=129)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` < 2.0881 (IC base=+0.178)

- **PATRÓN** `volumen_spike_ratio` > `2.8216` → IC=+0.168 (n=194)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 2.8216 (IC base=+0.178)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.198 (n=177)

  - _Acción_: Kelly boost +0.99€ cuando `libro_spread` < 0.01 (IC base=+0.178)

- **PATRÓN** `libro_liquidez` > `1919.501` → IC=+0.195 (n=165)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 1919.501 (IC base=+0.178)

- **PATRÓN** `ballena_activa_n` < `59.0` → IC=+0.243 (n=68)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.492` → IC=+0.196 (n=212)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 4.492 (IC base=+0.142)
...
- **PATRÓN** `volumen_regimen` > `1.1936` → IC=+0.164 (n=147)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` > 1.1936 (IC base=+0.142)
...
- **PATRÓN** `volumen_spike_ratio` > `1.3991` → IC=+0.196 (n=406)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.3991 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `5254.2901` → IC=+0.233 (n=294)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 5254.2901 (IC base=+0.142)

- **PATRÓN** `ballena_activa_n` < `167.0` → IC=+0.185 (n=211)

  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 167.0 (IC base=+0.142)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.133 (n=336)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 12.0 (IC base=+0.096)

- **PATRÓN** `ibs_20min` > `0.9062` → IC=+0.304 (n=222)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9062 (IC base=+0.096)

- **PATRÓN** `dist_vwap_pct` > `0.7678` → IC=+0.254 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7678 (IC base=+0.096)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.432` → IC=+0.224 (n=190)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.432 (IC base=+0.096)

- **PATRÓN** `libro_liquidez` > `2911.4143` → IC=+0.288 (n=163)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2911.4143 (IC base=+0.096)

- **PATRÓN** `ballena_activa_n` < `68.0` → IC=+0.174 (n=265)

  - _Acción_: Kelly boost +0.87€ cuando `ballena_activa_n` < 68.0 (IC base=+0.096)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.190 (n=224)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 17.0 (IC base=+0.170)

- **PATRÓN** `ibs_20min` > `0.9643` → IC=+0.295 (n=266)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9643 (IC base=+0.170)

- **PATRÓN** `dist_vwap_pct` > `0.993` → IC=+0.279 (n=161)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.993 (IC base=+0.170)
...
- **PATRÓN** `volumen_regimen` < `0.611` → IC=+0.182 (n=196)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.611 (IC base=+0.170)

- **PATRÓN** `volumen_regimen` > `1.0357` → IC=+0.195 (n=267)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` > 1.0357 (IC base=+0.170)

- **PATRÓN** `volumen_pendiente_norm` > `0.1713` → IC=+0.250 (n=134)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8278` → IC=+0.182 (n=357)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.124 (n=131)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=356)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.167 (n=16)
  ...
  - _Acción_: SKIP cuando `liq_imbalance_15min` |x|≤ 1.0
  - _Potencial_: sin este filtro IC_bueno=+0.062 (n=119)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.170 (n=770)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=2384)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.211 (n=746)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=2415)

- **FILTRO** `ibs_20min` > `0.2724` → IC=-0.175 (n=790)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2724
  - _Potencial_: sin este filtro IC_bueno=-0.007 (n=2371)
...
- **FILTRO** `ibs_20min` > `0.2712` → IC=-0.124 (n=184)
...
- **FILTRO** `py_entrada` > `0.6` → IC=-0.257 (n=167)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.052 (n=333)

- **FILTRO** `ibs_20min` > `0.2857` → IC=-0.234 (n=122)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.136 (n=1901)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=5917)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.286 (n=1866)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=5952)

- **FILTRO** `ibs_7min` < `0.7368` → IC=-0.235 (n=1947)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7368
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=5871)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.175 (n=2656)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=5162)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.228 (n=2198)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=7367)

- **FILTRO** `ibs_7min` > `0.72` → IC=-0.170 (n=2390)

  - _Acción_: SKIP cuando `ibs_7min` > 0.72
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=7175)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.161 (n=237)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.098 (n=816)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.331 (n=253)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=800)

- **FILTRO** `ibs_7min` < `0.9818` → IC=-0.201 (n=694)

  - _Acción_: SKIP cuando `ibs_7min` < 0.9818
  - _Potencial_: sin este filtro IC_bueno=+0.060 (n=359)
...
- **FILTRO** `py_entrada` < `0.38` → IC=-0.238 (n=380)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=1165)

- **FILTRO** `ibs_7min` < `0.8038` → IC=-0.165 (n=386)

  - _Acción_: SKIP cuando `ibs_7min` < 0.8038
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=1159)

- **FILTRO** `ballena_activa_n` > `155.0` → IC=-0.180 (n=385)

  - _Acción_: SKIP cuando `ballena_activa_n` > 155.0
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=1160)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.193 (n=262)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.100 (n=848)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.314 (n=359)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=751)

- **FILTRO** `ibs_7min` < `0.2125` → IC=-0.285 (n=277)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2125
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=833)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.270 (n=276)

  - _Acción_: SKIP cuando `ballena_activa_n` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=834)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.153 (n=286)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=1042)
...
- **FILTRO** `ibs_7min` < `0.7655` → IC=-0.186 (n=332)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7655
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=996)

- **FILTRO** `ballena_activa_n` > `43.0` → IC=-0.217 (n=327)

  - _Acción_: SKIP cuando `ballena_activa_n` > 43.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=1001)

- **FILTRO** `hora_utc` > `15.0` → IC=-0.130 (n=441)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.081 (n=876)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.287 (n=318)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=999)

- **FILTRO** `ibs_7min` > `0.2707` → IC=-0.171 (n=329)
...
- **FILTRO** `ballena_activa_n` > `38.0` → IC=-0.208 (n=323)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=994)
...
- **FILTRO** `ibs_7min` < `0.7719` → IC=-0.204 (n=370)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7719
  - _Potencial_: sin este filtro IC_bueno=+0.006 (n=1112)

- **FILTRO** `ballena_activa_n` > `14.0` → IC=-0.195 (n=369)

  - _Acción_: SKIP cuando `ballena_activa_n` > 14.0
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=1113)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.179 (n=438)
...
- **FILTRO** `py_entrada` < `0.39` → IC=-0.276 (n=319)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=+0.005 (n=981)

- **FILTRO** `ibs_7min` < `0.7534` → IC=-0.215 (n=324)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7534
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=976)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.238 (n=311)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=989)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.266 (n=318)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=1287)

- **FILTRO** `ibs_7min` > `0.83` → IC=-0.158 (n=401)
...
### STREAK_FADE_5M
- **FILTRO** `py_entrada` > `0.495` → IC=-0.188 (n=30)
...
- **PATRÓN** `streak_len` < `3.0` → IC=+0.135 (n=143)
  ...
  - _Acción_: Kelly boost +0.67€ cuando `streak_len` < 3.0 (IC base=+0.070)
...
- **PATRÓN** `ibs_15` < `0.1143` → IC=+0.139 (n=424)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.69€ cuando `ibs_15` < 0.1143 (IC base=+0.027)

### UPDOWN_GBM#5min
- **FILTRO** `ibs_15` < `0.2069` → IC=-0.223 (n=110)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: SKIP cuando `ibs_15` < 0.2069
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=331)

- **FILTRO** `sigma_ewma_delta_pct` > `6.666` → IC=-0.209 (n=53)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 6.666
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=388)
...
### UPDOWN_GBM#SOL#5min
- **FILTRO** `pct_spot_vs_ref` |x|> `0.0134` → IC=-0.210 (n=29)
  - _Por qué funciona_: precio spot lejos de la referencia → señal GBM sobreextiende; riesgo de reversión
  - _Acción_: SKIP cuando `pct_spot_vs_ref` |x|> 0.0134
...
- **PATRÓN** `sigma_h` > `0.005` → IC=+0.396 (n=65)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.005 (IC base=+0.308)

- **PATRÓN** `drift_60min` |x|≤ `0.0571` → IC=+0.340 (n=48)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0571 (IC base=+0.308)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1528` → IC=+0.327 (n=96)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1528 (IC base=+0.308)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3802` → IC=+0.379 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3802 (IC base=+0.308)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.323 (n=156)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.308)

- **PATRÓN** `ibs_15` > `0.8055` → IC=+0.369 (n=128)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8055 (IC base=+0.308)

- **PATRÓN** `dist_vwap_pct` > `0.4534` → IC=+0.364 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4534 (IC base=+0.308)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.489` → IC=+0.338 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.489 (IC base=+0.308)

- **PATRÓN** `libro_liquidez` > `7959.8654` → IC=+0.380 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7959.8654 (IC base=+0.308)

- **PATRÓN** `ballena_activa_n` < `540.0` → IC=+0.377 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 540.0 (IC base=+0.308)

### UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1971` → IC=+0.297 (n=72)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +1.00€ cuando `pct_spot_vs_ref` |x|≤ 0.1971 (IC base=+0.282)

- **PATRÓN** `sigma_h` < `0.0022` → IC=+0.289 (n=36)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0022 (IC base=+0.282)

- **PATRÓN** `drift_60min` |x|≤ `0.1567` → IC=+0.311 (n=72)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1567 (IC base=+0.282)

- **PATRÓN** `drift_15min` |x|≤ `0.4364` → IC=+0.316 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.4364 (IC base=+0.282)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.311 (n=88)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.282)

- **PATRÓN** `ibs_15` > `0.8246` → IC=+0.327 (n=73)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8246 (IC base=+0.282)

- **PATRÓN** `dist_vwap_pct` > `0.4311` → IC=+0.380 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4311 (IC base=+0.282)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.145` → IC=+0.287 (n=45)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.145 (IC base=+0.282)

- **PATRÓN** `ballena_activa_n` < `480.0` → IC=+0.441 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 480.0 (IC base=+0.282)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.219` → IC=-0.176 (n=334)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 17.219
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=2399)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1036` → IC=+0.397 (n=37)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1036 (IC base=-0.078)
...
- **PATRÓN** `dist_vwap_pct` < `0.1907` → IC=+0.273 (n=126)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1907 (IC base=-0.078)

- **PATRÓN** `ballena_activa_n` < `142.0` → IC=+0.307 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 142.0 (IC base=-0.078)
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3315 sube el IC de -0.078 a +0.355 en UPDOWN_GBM_15M_TARDIO (n=129). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD**: dentro de BUY_YES, IBS > 0.8055 sube el IC de +0.308 a +0.369 en UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD (n=128). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min**: dentro de BUY_YES, IBS > 0.8246 sube el IC de +0.282 a +0.327 en UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min (n=73). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 6644 | -0.102 | -964.56€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 672 | -0.090 | -121.51€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 5972 | -0.104 | -843.05€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 2168 | -0.074 | -108.79€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2168 | -0.074 | -108.79€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1595 | -0.198 | -389.57€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1595 | -0.198 | -389.57€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 31022 | +0.114 | -1991.03€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 5764 | +0.187 | -181.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 107 | -0.105 | -50.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 21995 | +0.095 | -1718.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3156 | +0.121 | -40.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 3691 | +0.062 | -636.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 20 | -0.045 | +2.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 3666 | +0.064 | -632.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 6425 | +0.134 | -128.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1625 | +0.199 | -77.50€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 3660 | +0.110 | -83.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1098 | +0.129 | +54.33€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 3699 | +0.079 | -472.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 20 | +0.045 | +2.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 3678 | +0.080 | -472.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 6900 | +0.128 | -94.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2100 | +0.168 | -11.92€ | 0 | 8 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 3663 | +0.113 | -52.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1125 | +0.102 | -21.70€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#SOL | 6617 | +0.134 | -404.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1982 | +0.203 | -96.85€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 46 | +0.000 | -9.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 3656 | +0.098 | -225.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 933 | +0.132 | -73.08€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 3690 | +0.105 | -254.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 17 | -0.022 | -0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 3672 | +0.106 | -252.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 5727 | +0.172 | -460.26€ | 3 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 5727 | +0.172 | -460.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 1440 | +0.166 | -155.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 1440 | +0.166 | -155.51€ | 0 | 4 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 1430 | +0.158 | -174.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 1430 | +0.158 | -174.32€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 1285 | +0.230 | -37.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 1285 | +0.230 | -37.16€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 1382 | +0.183 | -109.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 1382 | +0.183 | -109.59€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 15740 | +0.190 | -1411.85€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 15740 | +0.190 | -1411.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 2888 | +0.124 | -539.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 2888 | +0.124 | -539.26€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 2446 | +0.236 | -52.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 2446 | +0.236 | -52.10€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 2709 | +0.161 | -353.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 2709 | +0.161 | -353.17€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 2495 | +0.231 | -70.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 2495 | +0.231 | -70.29€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 2551 | +0.215 | -129.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 2551 | +0.215 | -129.64€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 2651 | +0.183 | -267.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 2651 | +0.183 | -267.39€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 5697 | +0.134 | +202.44€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 5697 | +0.134 | +202.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 2832 | +0.141 | +137.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 2832 | +0.141 | +137.36€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 2865 | +0.126 | +65.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 2865 | +0.126 | +65.08€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 5883 | +0.095 | -222.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 603 | +0.060 | -27.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 5280 | +0.099 | -195.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC | 4061 | +0.096 | -97.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#15min | 603 | +0.060 | -27.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#5min | 3458 | +0.102 | -70.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL | 1822 | +0.093 | -125.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL#5min | 1822 | +0.093 | -125.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 418 | +0.286 | -25.92€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 418 | +0.286 | -25.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 418 | +0.286 | -25.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 418 | +0.286 | -25.92€ | 0 | 4 |
| ✅ GBM_LATE_15M | 7407 | +0.044 | +2350.91€ | 0 | 16 |
| ✅ GBM_LATE_15M#15min | 