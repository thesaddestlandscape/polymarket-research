# Hipótesis automáticas — 2026-08-25 16:17 UTC
_Generado por shadow_postmortem.py sobre 152468 resoluciones (PNL=+10187.53€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `148.73` → IC=-0.279 (n=2137)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 148.73
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=6418)
...
- **PATRÓN** `py_entrada` < `0.34` → IC=+0.258 (n=3001)
...
- **PATRÓN** `libro_liquidez` > `1813.8317` → IC=+0.177 (n=2309)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 1813.8317 (IC base=+0.138)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.138 (n=1121)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.198 (n=890)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 5.0 (IC base=+0.192)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.197 (n=759)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `840.3405` → IC=+0.230 (n=561)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 840.3405 (IC base=+0.216)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.196 (n=4173)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.199 (n=1017)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.198 (n=707)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.196 (n=10844)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.222 (n=7641)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.248 (n=1769)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.236)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.270 (n=1269)
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.213 (n=654)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.76 (IC base=+0.163)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.243 (n=1529)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.196 (n=1502)
...
- **PATRÓN** `restante_min` > `4.92` → IC=+0.157 (n=1396)
...
- **PATRÓN** `lag_apertura_s` < `4.54` → IC=+0.157 (n=1303)

  - _Acción_: Kelly boost +0.78€ cuando `lag_apertura_s` < 4.54 (IC base=+0.131)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.204 (n=747)
...
- **PATRÓN** `restante_min` > `4.9` → IC=+0.173 (n=673)
...
- **PATRÓN** `lag_apertura_s` < `5.77` → IC=+0.175 (n=645)

  - _Acción_: Kelly boost +0.88€ cuando `lag_apertura_s` < 5.77 (IC base=+0.139)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.190 (n=655)
...
- **PATRÓN** `restante_min` > `4.94` → IC=+0.153 (n=705)

  - _Acción_: Kelly boost +0.77€ cuando `restante_min` > 4.94 (IC base=+0.123)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.124 (n=2052)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9448` → IC=+0.221 (n=660)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9448 (IC base=+0.070)

- **PATRÓN** `dist_vwap_pct` > `0.1985` → IC=+0.238 (n=280)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1985 (IC base=+0.070)

- **PATRÓN** `dist_vwap_pct` < `0.741` → IC=+0.227 (n=401)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.741 (IC base=+0.070)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.06` → IC=+0.185 (n=817)

  - _Acción_: Kelly boost +0.92€ cuando `sigma_ewma_delta_pct` > 5.06 (IC base=+0.070)

- **PATRÓN** `volumen_regimen` < `0.7094` → IC=+0.230 (n=183)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7094 (IC base=+0.070)

- **PATRÓN** `volumen_regimen` > `1.0826` → IC=+0.263 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0826 (IC base=+0.070)

- **PATRÓN** `volumen_pendiente_norm` < `0.1063` → IC=+0.136 (n=989)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_pendiente_norm` < 0.1063 (IC base=+0.070)

- **PATRÓN** `volumen_pendiente_norm` > `0.3056` → IC=+0.148 (n=143)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` > 0.3056 (IC base=+0.070)

- **PATRÓN** `volumen_spike_ratio` < `1.5794` → IC=+0.148 (n=455)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.5794 (IC base=+0.070)

- **PATRÓN** `volumen_spike_ratio` > `1.9534` → IC=+0.137 (n=689)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` > 1.9534 (IC base=+0.070)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.193 (n=148)
  ...
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0075 (IC base=+0.122)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.181 (n=161)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` < 6.0 (IC base=+0.122)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.300 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.122)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.377` → IC=+0.340 (n=160)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.377 (IC base=+0.122)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.161 (n=216)

  - _Acción_: Kelly boost +0.80€ cuando `libro_spread` < 0.04 (IC base=+0.122)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.250 (n=322)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9585` → IC=+0.243 (n=146)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9585 (IC base=+0.221)

- **PATRÓN** `dist_vwap_pct` > `0.2345` → IC=+0.237 (n=226)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2345 (IC base=+0.221)

- **PATRÓN** `dist_vwap_pct` < `1.0239` → IC=+0.225 (n=332)
...
- **PATRÓN** `volumen_regimen` < `1.294` → IC=+0.222 (n=322)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0769` → IC=+0.229 (n=286)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0769 (IC base=+0.221)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4485` → IC=+0.259 (n=131)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4485 (IC base=+0.221)

- **PATRÓN** `libro_liquidez` > `11109.7462` → IC=+0.237 (n=287)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11109.7462 (IC base=+0.221)

- **PATRÓN** `ballena_activa_n` < `369.0` → IC=+0.220 (n=216)
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.192 (n=131)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0076 (IC base=+0.129)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.924` → IC=+0.289 (n=107)
...
- **PATRÓN** `ballena_activa_n` < `27.0` → IC=+0.138 (n=78)
  ...
  - _Acción_: Kelly boost +0.69€ cuando `ballena_activa_n` < 27.0 (IC base=+0.129)
...
- **PATRÓN** `dist_vwap_pct` < `0.0784` → IC=+0.346 (n=24)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0784 (IC base=-0.027)

- **PATRÓN** `volumen_regimen` > `0.8071` → IC=+0.326 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8071 (IC base=-0.027)
...
- **PATRÓN** `sigma_h` > `0.0073` → IC=+0.136 (n=1149)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0073 (IC base=+0.056)

- **PATRÓN** `ibs_20min` > `0.9459` → IC=+0.249 (n=843)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9459 (IC base=+0.056)

- **PATRÓN** `dist_vwap_pct` > `1.245` → IC=+0.288 (n=225)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.245 (IC base=+0.056)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.084` → IC=+0.124 (n=1006)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 5.084 (IC base=+0.056)

- **PATRÓN** `volumen_regimen` > `1.164` → IC=+0.196 (n=294)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` > 1.164 (IC base=+0.056)

- **PATRÓN** `volumen_pendiente_norm` < `0.1151` → IC=+0.166 (n=1107)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` < 0.1151 (IC base=+0.056)

- **PATRÓN** `volumen_pendiente_norm` > `0.2423` → IC=+0.224 (n=219)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2423 (IC base=+0.056)

- **PATRÓN** `volumen_spike_ratio` < `1.4831` → IC=+0.188 (n=379)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` < 1.4831 (IC base=+0.056)

- **PATRÓN** `volumen_spike_ratio` > `2.8753` → IC=+0.174 (n=379)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.8753 (IC base=+0.056)

- **PATRÓN** `ballena_activa_n` < `103.0` → IC=+0.271 (n=584)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.422` → IC=+0.159 (n=136)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 3.422 (IC base=-0.013)
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.291 (n=127)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.94` → IC=+0.303 (n=115)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0881` → IC=+0.174 (n=133)
  ...
  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.0881 (IC base=+0.180)
...
- **PATRÓN** `libro_liquidez` > `1914.9184` → IC=+0.203 (n=173)
...
- **FILTRO** `ibs_20min` < `0.5294` → IC=-0.144 (n=282)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.138 (n=288)
...
- **PATRÓN** `ibs_20min` > `0.8462` → IC=+0.247 (n=144)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8462 (IC base=-0.002)

- **PATRÓN** `dist_vwap_pct` > `0.3189` → IC=+0.263 (n=95)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3189 (IC base=-0.002)

- **PATRÓN** `volumen_regimen` < `0.8696` → IC=+0.173 (n=108)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 0.8696 (IC base=-0.002)

- **PATRÓN** `volumen_regimen` > `0.639` → IC=+0.152 (n=162)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.639 (IC base=-0.002)

- **PATRÓN** `volumen_pendiente_norm` > `0.2631` → IC=+0.326 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2631 (IC base=-0.002)

- **PATRÓN** `volumen_spike_ratio` < `1.4966` → IC=+0.173 (n=50)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.4966 (IC base=-0.002)

- **PATRÓN** `volumen_spike_ratio` > `1.5794` → IC=+0.162 (n=134)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.5794 (IC base=-0.002)

- **PATRÓN** `ballena_activa_n` < `77.0` → IC=+0.209 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 77.0 (IC base=-0.002)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.214 (n=260)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9048` → IC=+0.289 (n=344)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9048 (IC base=+0.210)

- **PATRÓN** `dist_vwap_pct` > `1.3033` → IC=+0.332 (n=129)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.3033 (IC base=+0.210)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.118` → IC=+0.270 (n=306)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.118 (IC base=+0.210)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0805` → IC=+0.212 (n=435)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0805 (IC base=+0.210)

- **PATRÓN** `volumen_pendiente_norm` > `0.2797` → IC=+0.276 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2797 (IC base=+0.210)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.227 (n=574)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.210)

- **PATRÓN** `libro_liquidez` > `3094.439` → IC=+0.247 (n=172)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3094.439 (IC base=+0.210)
...
- **PATRÓN** `sigma_h` > `0.0066` → IC=+0.186 (n=1455)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0066 (IC base=+0.159)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.165 (n=2280)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 5.0 (IC base=+0.159)

- **PATRÓN** `ibs_20min` > `0.893` → IC=+0.261 (n=1453)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.893 (IC base=+0.159)

- **PATRÓN** `dist_vwap_pct` > `0.7901` → IC=+0.251 (n=540)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7901 (IC base=+0.159)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.233` → IC=+0.260 (n=934)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.233 (IC base=+0.159)

- **PATRÓN** `volumen_regimen` < `1.2315` → IC=+0.165 (n=1519)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.2315 (IC base=+0.159)

- **PATRÓN** `volumen_regimen` > `0.6917` → IC=+0.173 (n=1357)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 0.6917 (IC base=+0.159)

- **PATRÓN** `volumen_pendiente_norm` > `0.1034` → IC=+0.189 (n=774)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.1034 (IC base=+0.159)

- **PATRÓN** `volumen_spike_ratio` < `2.2901` → IC=+0.159 (n=1701)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.2901 (IC base=+0.159)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.170 (n=1778)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.01 (IC base=+0.159)

- **PATRÓN** `libro_liquidez` > `3727.1826` → IC=+0.201 (n=727)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3727.1826 (IC base=+0.159)

- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.185 (n=960)

  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 135.0 (IC base=+0.159)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.556` → IC=+0.361 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.556 (IC base=+0.151)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4192` → IC=+0.137 (n=268)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 1.4192 (IC base=+0.151)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.195 (n=175)

  - _Acción_: Kelly boost +0.97€ cuando `libro_spread` < 0.04 (IC base=+0.151)
...
- **PATRÓN** `volumen_regimen` < `0.6383` → IC=+0.209 (n=101)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6383 (IC base=+0.189)

- **PATRÓN** `volumen_regimen` > `0.9006` → IC=+0.211 (n=202)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.9006 (IC base=+0.189)
...
- **PATRÓN** `libro_liquidez` > `11835.8417` → IC=+0.226 (n=202)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11835.8417 (IC base=+0.189)

- **PATRÓN** `ballena_activa_n` < `372.0` → IC=+0.191 (n=150)

  - _Acción_: Kelly boost +0.95€ cuando `ballena_activa_n` < 372.0 (IC base=+0.189)
...
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.194 (n=106)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0077 (IC base=+0.146)

- **PATRÓN** `drift_60min` |x|≤ `0.144` → IC=+0.154 (n=212)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.144 (IC base=+0.146)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.877` → IC=+0.316 (n=85)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2301` → IC=+0.130 (n=255)
  ...
  - _Acción_: Kelly boost +0.65€ cuando `volumen_pendiente_norm` < 0.2301 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` < `2.0146` → IC=+0.196 (n=113)
...
- **PATRÓN** `volumen_spike_ratio` > `3.8668` → IC=+0.147 (n=117)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 3.8668 (IC base=+0.146)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.172 (n=251)

  - _Acción_: Kelly boost +0.86€ cuando `libro_spread` < 0.04 (IC base=+0.146)

- **PATRÓN** `libro_liquidez` > `1961.8084` → IC=+0.167 (n=106)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 1961.8084 (IC base=+0.146)
...
- **PATRÓN** `sigma_h` < `0.0077` → IC=+0.239 (n=266)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0077 (IC base=+0.191)

- **PATRÓN** `drift_60min` |x|≤ `0.5012` → IC=+0.208 (n=303)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.5012 (IC base=+0.191)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.215 (n=307)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.191)

- **PATRÓN** `ibs_20min` > `0.4502` → IC=+0.251 (n=303)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4502 (IC base=+0.191)

- **PATRÓN** `dist_vwap_pct` > `1.0605` → IC=+0.247 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0605 (IC base=+0.191)
...
- **PATRÓN** `volumen_regimen` > `0.6366` → IC=+0.211 (n=303)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6366 (IC base=+0.191)

- **PATRÓN** `volumen_pendiente_norm` > `0.101` → IC=+0.263 (n=133)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4394` → IC=+0.210 (n=98)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4394 (IC base=+0.191)

- **PATRÓN** `volumen_spike_ratio` > `2.4658` → IC=+0.227 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4658 (IC base=+0.191)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.193 (n=340)

  - _Acción_: Kelly boost +0.96€ cuando `libro_spread` < 0.01 (IC base=+0.191)

- **PATRÓN** `libro_liquidez` > `9735.6545` → IC=+0.214 (n=138)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 9735.6545 (IC base=+0.191)

- **PATRÓN** `ballena_activa_n` < `126.0` → IC=+0.204 (n=106)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 126.0 (IC base=+0.191)
...
- **PATRÓN** `sigma_h` > `0.01` → IC=+0.192 (n=199)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.01 (IC base=+0.119)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.136 (n=454)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 5.0 (IC base=+0.119)

- **PATRÓN** `ibs_20min` > `0.8511` → IC=+0.231 (n=292)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8511 (IC base=+0.119)

- **PATRÓN** `dist_vwap_pct` > `0.7834` → IC=+0.272 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7834 (IC base=+0.119)
...
- **PATRÓN** `volumen_regimen` > `0.6804` → IC=+0.136 (n=391)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 0.6804 (IC base=+0.119)

- **PATRÓN** `volumen_pendiente_norm` > `0.0998` → IC=+0.145 (n=153)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_pendiente_norm` > 0.0998 (IC base=+0.119)

- **PATRÓN** `volumen_spike_ratio` > `1.4421` → IC=+0.122 (n=406)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_spike_ratio` > 1.4421 (IC base=+0.119)

- **PATRÓN** `libro_liquidez` > `2846.759` → IC=+0.192 (n=199)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 2846.759 (IC base=+0.119)
...
- **PATRÓN** `sigma_h` > `0.0275` → IC=+0.202 (n=159)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0275 (IC base=+0.166)
...
- **PATRÓN** `ibs_20min` > `0.7013` → IC=+0.210 (n=477)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7013 (IC base=+0.166)

- **PATRÓN** `dist_vwap_pct` > `0.9631` → IC=+0.241 (n=168)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9631 (IC base=+0.166)
...
- **PATRÓN** `volumen_regimen` < `1.2221` → IC=+0.170 (n=477)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 1.2221 (IC base=+0.166)

- **PATRÓN** `volumen_regimen` > `0.8345` → IC=+0.188 (n=318)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` > 0.8345 (IC base=+0.166)

- **PATRÓN** `volumen_pendiente_norm` > `0.0804` → IC=+0.224 (n=183)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0804 (IC base=+0.166)

- **PATRÓN** `volumen_spike_ratio` < `2.1643` → IC=+0.179 (n=387)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 2.1643 (IC base=+0.166)

- **PATRÓN** `volumen_spike_ratio` > `1.8094` → IC=+0.171 (n=293)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.8094 (IC base=+0.166)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.170 (n=534)
...
- **PATRÓN** `sigma_h` > `0.0063` → IC=+0.130 (n=163)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` > 0.0063 (IC base=+0.088)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.214 (n=173)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.088)

- **PATRÓN** `ibs_20min` > `0.6176` → IC=+0.159 (n=321)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.6176 (IC base=+0.088)

- **PATRÓN** `dist_vwap_pct` > `0.7744` → IC=+0.240 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7744 (IC base=+0.088)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.14` → IC=+0.221 (n=109)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1824` → IC=+0.225 (n=89)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1824 (IC base=+0.088)

- **PATRÓN** `libro_liquidez` > `2964.2504` → IC=+0.128 (n=240)

  - _Acción_: Kelly boost +0.64€ cuando `libro_liquidez` > 2964.2504 (IC base=+0.088)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.206 (n=1145)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0076 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.170 (n=1257)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 15.0 (IC base=+0.145)

- **PATRÓN** `ibs_20min` > `0.9437` → IC=+0.288 (n=1145)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9437 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `1.053` → IC=+0.255 (n=442)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.053 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.481` → IC=+0.230 (n=1519)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.481 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` < `0.7058` → IC=+0.157 (n=786)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.7058 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` > `1.0858` → IC=+0.152 (n=809)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.0858 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` > `0.1654` → IC=+0.180 (n=633)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1654 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` > `1.8963` → IC=+0.154 (n=1474)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.8963 (IC base=+0.145)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.155 (n=2050)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `3101.1726` → IC=+0.194 (n=1145)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 3101.1726 (IC base=+0.145)

- **PATRÓN** `ballena_activa_n` < `172.0` → IC=+0.189 (n=1010)

  - _Acción_: Kelly boost +0.94€ cuando `ballena_activa_n` < 172.0 (IC base=+0.145)

- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.223 (n=1494)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0063 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.4333` → IC=+0.199 (n=2237)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.4333 (IC base=+0.189)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.195 (n=1020)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5435` → IC=+0.244 (n=2237)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5435 (IC base=+0.189)

- **PATRÓN** `dist_vwap_pct` < `0.7107` → IC=+0.177 (n=1843)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` < 0.7107 (IC base=+0.189)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.472` → IC=+0.201 (n=406)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2558` → IC=+0.198 (n=700)
...
- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.314 (n=143)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.5765` → IC=+0.338 (n=214)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0647` → IC=+0.312 (n=94)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8682` → IC=+0.285 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8682 (IC base=+0.287)

- **PATRÓN** `volumen_spike_ratio` > `1.5851` → IC=+0.300 (n=103)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5851 (IC base=+0.287)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.326 (n=107)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.03 (IC base=+0.287)

- **PATRÓN** `libro_liquidez` > `1989.8695` → IC=+0.338 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1989.8695 (IC base=+0.287)

- **PATRÓN** `ballena_activa_n` < `69.0` → IC=+0.292 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 69.0 (IC base=+0.287)
...
- **PATRÓN** `volumen_regimen` < `0.6563` → IC=+0.187 (n=132)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` < 0.6563 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` > `1.1052` → IC=+0.191 (n=179)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_regimen` > 1.1052 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` > `0.1441` → IC=+0.235 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1441 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` < `2.53` → IC=+0.198 (n=346)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 2.53 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `1.4672` → IC=+0.204 (n=309)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4672 (IC base=+0.165)

- **PATRÓN** `libro_liquidez` > `11526.545` → IC=+0.206 (n=263)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11526.545 (IC base=+0.165)
...
- **PATRÓN** `volumen_spike_ratio` < `3.6434` → IC=+0.262 (n=149)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5036` → IC=+0.154 (n=322)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `dist_vwap_pct` < `0.727` → IC=+0.166 (n=339)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.727 (IC base=+0.142)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.088` → IC=+0.142 (n=294)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` < 4.088 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `0.5857` → IC=+0.164 (n=108)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.5857 (IC base=+0.142)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8037` → IC=+0.182 (n=177)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 1.8037 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` > `1.443` → IC=+0.170 (n=265)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 1.443 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `9053.953` → IC=+0.176 (n=146)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 9053.953 (IC base=+0.142)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.196 (n=271)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 15.0 (IC base=+0.172)
...
- **PATRÓN** `dist_vwap_pct` > `1.3375` → IC=+0.280 (n=139)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.3375 (IC base=+0.172)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.048` → IC=+0.253 (n=298)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.048 (IC base=+0.172)

- **PATRÓN** `volumen_regimen` < `0.6162` → IC=+0.196 (n=182)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` < 0.6162 (IC base=+0.172)

- **PATRÓN** `volumen_regimen` > `1.0463` → IC=+0.192 (n=248)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` > 1.0463 (IC base=+0.172)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5621` → IC=+0.179 (n=219)
  ...
  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 1.5621 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` > `1.8439` → IC=+0.179 (n=331)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 1.8439 (IC base=+0.172)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.186 (n=609)

  - _Acción_: Kelly boost +0.93€ cuando `libro_spread` < 0.01 (IC base=+0.172)

- **PATRÓN** `libro_liquidez` > `3111.1498` → IC=+0.190 (n=182)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 3111.1498 (IC base=+0.172)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.156 (n=718)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.138 (n=136)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `8352.5828` → IC=+0.131 (n=215)

  - _Acción_: Kelly boost +0.66€ cuando `libro_liquidez` > 8352.5828 (IC base=+0.078)

- **PATRÓN** `sigma_h` < `0.0037` → IC=+0.209 (n=242)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0037 (IC base=+0.148)

- **PATRÓN** `drift_60min` |x|≤ `0.3378` → IC=+0.177 (n=484)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.3378 (IC base=+0.148)
...
- **PATRÓN** `ibs_20min` < `0.6273` → IC=+0.163 (n=484)

  - _Acción_: Kelly boost +0.81€ cuando `ibs_20min` < 0.6273 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` > `0.6097` → IC=+0.154 (n=160)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.6097 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` < `0.7556` → IC=+0.150 (n=607)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.7556 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.926` → IC=+0.167 (n=277)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 2.926 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` < `1.2467` → IC=+0.167 (n=550)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.2467 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` > `0.7234` → IC=+0.148 (n=492)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` > 0.7234 (IC base=+0.148)

- **PATRÓN** `volumen_pendiente_norm` > `0.0733` → IC=+0.170 (n=268)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` > 0.0733 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` < `2.5268` → IC=+0.172 (n=544)
...
- **PATRÓN** `libro_liquidez` > `8247.5526` → IC=+0.161 (n=550)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.171` → IC=+0.154 (n=267)
  ...
  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 4.171 (IC base=+0.150)
...
- **PATRÓN** `sigma_h` < `0.0105` → IC=+0.132 (n=161)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` < 0.0105 (IC base=+0.102)
...
- **PATRÓN** `ibs_20min` < `0.1965` → IC=+0.125 (n=54)
  ...
  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` < 0.1965 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` > `1.1654` → IC=+0.222 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1654 (IC base=+0.102)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.483` → IC=+0.175 (n=38)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 10.483 (IC base=+0.102)

- **PATRÓN** `volumen_spike_ratio` < `1.3905` → IC=+0.204 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3905 (IC base=+0.102)

- **PATRÓN** `libro_liquidez` > `7710.7968` → IC=+0.150 (n=161)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 7710.7968 (IC base=+0.102)
...
- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.181 (n=117)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0065 (IC base=+0.172)

- **PATRÓN** `drift_60min` |x|≤ `0.2917` → IC=+0.223 (n=117)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2917 (IC base=+0.172)
...
- **PATRÓN** `ibs_20min` < `0.5833` → IC=+0.181 (n=155)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` < 0.5833 (IC base=+0.172)

- **PATRÓN** `ibs_20min` > `0.1096` → IC=+0.206 (n=175)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.1096 (IC base=+0.172)

- **PATRÓN** `dist_vwap_pct` < `0.6774` → IC=+0.183 (n=184)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.6774 (IC base=+0.172)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.561` → IC=+0.268 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.561 (IC base=+0.172)

- **PATRÓN** `volumen_regimen` < `1.1687` → IC=+0.217 (n=175)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1543` → IC=+0.232 (n=151)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4319` → IC=+0.194 (n=171)
  ...
  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 1.4319 (IC base=+0.172)

- **PATRÓN** `libro_liquidez` > `9414.3389` → IC=+0.172 (n=117)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 9414.3389 (IC base=+0.172)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.147 (n=15)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.032 (n=342)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.165 (n=687)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.037 (n=2136)
...
- **FILTRO** `py_entrada` < `0.39` → IC=-0.224 (n=103)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.062 (n=311)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.183 (n=181)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.142 (n=2031)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=4866)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.285 (n=1657)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=5240)

- **FILTRO** `ibs_7min` < `0.7368` → IC=-0.233 (n=1721)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7368
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=5176)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.175 (n=2331)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=4566)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.228 (n=1985)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=6454)

- **FILTRO** `ibs_7min` > `0.7232` → IC=-0.169 (n=2109)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7232
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=6330)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.154 (n=218)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.091 (n=721)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.325 (n=227)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=712)

- **FILTRO** `ibs_7min` < `0.9839` → IC=-0.199 (n=619)
...
- **FILTRO** `ibs_7min` > `0.8378` → IC=-0.200 (n=355)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=1066)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.161 (n=290)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=1106)

- **FILTRO** `py_entrada` < `0.39` → IC=-0.249 (n=348)

  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=1048)

- **FILTRO** `ballena_activa_n` > `148.0` → IC=-0.171 (n=348)
...
- **FILTRO** `py_entrada` > `0.58` → IC=-0.221 (n=346)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=1079)

- **FILTRO** `ballena_activa_n` > `118.0` → IC=-0.173 (n=356)

  - _Acción_: SKIP cuando `ballena_activa_n` > 118.0
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=1069)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.213 (n=273)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.077 (n=690)

- **FILTRO** `py_entrada` < `0.33` → IC=-0.326 (n=240)

  - _Acción_: SKIP cuando `py_entrada` < 0.33
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=723)

- **FILTRO** `ibs_7min` < `0.2143` → IC=-0.276 (n=239)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2143
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=724)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.274 (n=237)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.157 (n=359)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=817)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.266 (n=289)
...
- **FILTRO** `ballena_activa_n` > `43.0` → IC=-0.208 (n=293)

  - _Acción_: SKIP cuando `ballena_activa_n` > 43.0
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=883)
...
- **FILTRO** `ibs_7min` > `0.1818` → IC=-0.155 (n=395)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=768)

- **FILTRO** `ballena_activa_n` > `39.0` → IC=-0.213 (n=287)

  - _Acción_: SKIP cuando `ballena_activa_n` > 39.0
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=876)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.4` → IC=-0.225 (n=311)

  - _Acción_: SKIP cuando `py_entrada` < 0.4
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=979)
...
- **FILTRO** `py_entrada` < `0.38` → IC=-0.290 (n=274)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=859)

- **FILTRO** `ibs_7min` < `0.75` → IC=-0.239 (n=274)

  - _Acción_: SKIP cuando `ibs_7min` < 0.75
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=859)
...
- **FILTRO** `ibs_7min` > `0.8346` → IC=-0.147 (n=355)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8346
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=1066)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.123 (n=478)

  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=943)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.036` → IC=-0.190 (n=311)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 17.036
  - _Potencial_: sin este filtro IC_bueno=-0.081 (n=2153)
...
- **PATRÓN** `ibs_15` < `0.3357` → IC=+0.339 (n=85)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3357 (IC base=-0.095)

- **PATRÓN** `dist_vwap_pct` < `0.1943` → IC=+0.233 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1943 (IC base=-0.095)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.278 (n=43)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 156.0 (IC base=-0.095)
...
- **FILTRO** `drift_15min` |x|> `1.5229` → IC=-0.148 (n=197)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 1.5229
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=594)

- **FILTRO** `sigma_ewma_delta_pct` > `15.454` → IC=-0.177 (n=97)
...
### UPDOWN_OU_5M
- **FILTRO** `drift_60min` |x|> `0.0932` → IC=-0.155 (n=27)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.0932
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3357 sube el IC de -0.095 a +0.339 en UPDOWN_GBM_15M_TARDIO (n=85). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 6312 | -0.098 | -817.70€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 656 | -0.085 | -111.46€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 5656 | -0.100 | -706.24€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 994 | +0.019 | -106.61€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 994 | +0.019 | -106.61€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 656 | -0.085 | -111.46€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 656 | -0.085 | -111.46€ | 2 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 2090 | -0.075 | -94.08€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2090 | -0.075 | -94.08€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1488 | -0.186 | -312.09€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1488 | -0.186 | -312.09€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 28753 | +0.114 | -1879.37€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 5449 | +0.187 | -179.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 107 | -0.105 | -50.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 20119 | +0.094 | -1620.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3078 | +0.122 | -29.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 3378 | +0.063 | -580.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 19 | -0.068 | +0.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 3354 | +0.065 | -574.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 5997 | +0.135 | -118.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1530 | +0.197 | -83.30€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 3351 | +0.111 | -73.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1074 | +0.132 | +59.80€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 3384 | +0.076 | -463.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 19 | +0.023 | -2.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 3364 | +0.077 | -458.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 6433 | +0.131 | -47.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1981 | +0.172 | +3.51€ | 0 | 8 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 