# Hipótesis automáticas — 2026-09-03 19:19 UTC
_Generado por shadow_postmortem.py sobre 270470 resoluciones (PNL=+25702.68€)_
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.191 (n=7864)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.197 (n=6086)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.203 (n=2897)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.197 (n=7140)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 17.0 (IC base=+0.189)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.208 (n=17151)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.71 (IC base=+0.189)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.151 (n=3523)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.176 (n=2510)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.274 (n=1068)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.192 (n=1211)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.227 (n=1466)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3571` → IC=+0.130 (n=3609)

  - _Acción_: Kelly boost +0.65€ cuando `ibs_20min` > 0.3571 (IC base=+0.082)

- **PATRÓN** `dist_vwap_pct` > `0.2623` → IC=+0.228 (n=461)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2623 (IC base=+0.082)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.572` → IC=+0.153 (n=1382)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 5.572 (IC base=+0.082)

- **PATRÓN** `volumen_regimen` < `0.6962` → IC=+0.224 (n=360)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6962 (IC base=+0.082)

- **PATRÓN** `volumen_regimen` > `1.0855` → IC=+0.246 (n=372)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0855 (IC base=+0.082)

- **PATRÓN** `volumen_pendiente_norm` > `0.1727` → IC=+0.180 (n=614)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1727 (IC base=+0.082)

- **PATRÓN** `volumen_spike_ratio` < `2.8795` → IC=+0.177 (n=2149)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 2.8795 (IC base=+0.082)

- **PATRÓN** `volumen_spike_ratio` > `1.4754` → IC=+0.173 (n=2148)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.4754 (IC base=+0.082)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.205 (n=1066)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 54.0 (IC base=+0.082)

- **PATRÓN** `ibs_20min` < `0.4065` → IC=+0.128 (n=3476)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2773` → IC=+0.157 (n=802)
...
- **PATRÓN** `sigma_h` < `0.003` → IC=+0.256 (n=211)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.0966` → IC=+0.237 (n=211)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0966 (IC base=+0.214)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.236 (n=630)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.214)

- **PATRÓN** `ibs_20min` > `0.929` → IC=+0.246 (n=286)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.929 (IC base=+0.214)

- **PATRÓN** `dist_vwap_pct` > `0.1833` → IC=+0.225 (n=402)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1833 (IC base=+0.214)

- **PATRÓN** `dist_vwap_pct` < `0.4777` → IC=+0.215 (n=571)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4777 (IC base=+0.214)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.309` → IC=+0.229 (n=304)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.309 (IC base=+0.214)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.05` → IC=+0.216 (n=633)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 7.05 (IC base=+0.214)

- **PATRÓN** `volumen_regimen` < `1.2593` → IC=+0.219 (n=631)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2593 (IC base=+0.214)

- **PATRÓN** `volumen_regimen` > `1.099` → IC=+0.229 (n=286)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.099 (IC base=+0.214)

- **PATRÓN** `volumen_pendiente_norm` < `0.1007` → IC=+0.219 (n=600)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1007 (IC base=+0.214)

- **PATRÓN** `volumen_spike_ratio` < `1.4867` → IC=+0.247 (n=267)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4867 (IC base=+0.214)

- **PATRÓN** `libro_liquidez` > `11871.8913` → IC=+0.242 (n=563)
...
- **PATRÓN** `sigma_h` > `0.0095` → IC=+0.218 (n=253)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0095 (IC base=+0.165)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1321` → IC=+0.166 (n=647)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` < 0.1321 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` < `3.8997` → IC=+0.158 (n=673)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 3.8997 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `1.8973` → IC=+0.178 (n=601)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 1.8973 (IC base=+0.165)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.190 (n=776)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.04 (IC base=+0.165)

- **PATRÓN** `ballena_activa_n` < `16.0` → IC=+0.233 (n=148)
...
- **PATRÓN** `dist_vwap_pct` > `0.3604` → IC=+0.312 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3604 (IC base=-0.036)

- **PATRÓN** `dist_vwap_pct` < `0.1549` → IC=+0.278 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1549 (IC base=-0.036)

- **PATRÓN** `volumen_regimen` < `0.5993` → IC=+0.352 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.5993 (IC base=-0.036)
...
- **PATRÓN** `ballena_activa_n` < `175.0` → IC=+0.285 (n=63)
...
- **PATRÓN** `dist_vwap_pct` < `0.5682` → IC=+0.278 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5682 (IC base=+0.047)
...
- **FILTRO** `sigma_h` > `0.0248` → IC=-0.144 (n=405)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=1217)

- **FILTRO** `ibs_20min` > `0.7937` → IC=-0.190 (n=405)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7937
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=1217)
...
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.174 (n=1491)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0094 (IC base=+0.071)

- **PATRÓN** `ibs_20min` > `0.2883` → IC=+0.140 (n=4468)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` > 0.2883 (IC base=+0.071)

- **PATRÓN** `dist_vwap_pct` > `0.8858` → IC=+0.293 (n=432)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8858 (IC base=+0.071)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.472` → IC=+0.122 (n=2360)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0868` → IC=+0.216 (n=2002)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0868 (IC base=+0.071)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4938` → IC=+0.239 (n=719)
...
- **PATRÓN** `ballena_activa_n` < `94.0` → IC=+0.304 (n=1609)
...
- **FILTRO** `ibs_20min` < `0.2878` → IC=-0.155 (n=369)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.107 (n=751)

- **FILTRO** `sigma_ewma_delta_pct` > `2.395` → IC=-0.164 (n=319)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 2.395
  - _Potencial_: sin este filtro IC_bueno=+0.029 (n=699)

- **PATRÓN** `ibs_20min` > `0.8182` → IC=+0.227 (n=280)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8182 (IC base=+0.021)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.112` → IC=+0.128 (n=420)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 2.112 (IC base=+0.021)
...
- **PATRÓN** `volumen_spike_ratio` < `1.9482` → IC=+0.242 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9482 (IC base=+0.021)

- **PATRÓN** `volumen_spike_ratio` > `2.6391` → IC=+0.294 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6391 (IC base=+0.021)

- **PATRÓN** `ballena_activa_n` < `63.0` → IC=+0.362 (n=121)
...
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.290 (n=326)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0083 (IC base=+0.224)

- **PATRÓN** `drift_60min` |x|≤ `0.1246` → IC=+0.224 (n=317)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1246 (IC base=+0.224)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1395` → IC=+0.240 (n=606)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1395 (IC base=+0.224)

- **PATRÓN** `volumen_spike_ratio` < `2.4905` → IC=+0.223 (n=424)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.254 (n=733)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.224)

- **PATRÓN** `ballena_activa_n` < `21.0` → IC=+0.261 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 21.0 (IC base=+0.224)
...
- **FILTRO** `ibs_20min` < `0.7707` → IC=-0.142 (n=476)
...
- **PATRÓN** `sigma_h` > `0.0131` → IC=+0.336 (n=487)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0131 (IC base=+0.253)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.254 (n=279)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.898` → IC=+0.326 (n=487)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.898 (IC base=+0.253)

- **PATRÓN** `dist_vwap_pct` > `0.1617` → IC=+0.321 (n=427)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2406` → IC=+0.296 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2406 (IC base=+0.253)

- **PATRÓN** `volumen_spike_ratio` < `2.5754` → IC=+0.262 (n=670)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5754 (IC base=+0.253)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.256 (n=857)
...
- **PATRÓN** `sigma_h` > `0.0106` → IC=+0.209 (n=1258)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0106 (IC base=+0.164)

- **PATRÓN** `drift_60min` |x|≤ `0.3428` → IC=+0.171 (n=3321)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.3428 (IC base=+0.164)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.187 (n=1347)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 17.0 (IC base=+0.164)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.290 (n=1724)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.7781` → IC=+0.246 (n=822)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7781 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.699` → IC=+0.237 (n=1535)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.699 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` > `0.6274` → IC=+0.167 (n=2553)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 0.6274 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.1055` → IC=+0.188 (n=1409)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.1055 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` < `2.3226` → IC=+0.165 (n=3085)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.3226 (IC base=+0.164)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.166 (n=3868)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.03 (IC base=+0.164)

- **PATRÓN** `libro_liquidez` > `3936.5622` → IC=+0.179 (n=1258)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 3936.5622 (IC base=+0.164)

- **PATRÓN** `ballena_activa_n` < `143.0` → IC=+0.183 (n=2687)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 143.0 (IC base=+0.164)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.199 (n=1690)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.4388` → IC=+0.232 (n=3502)
...
- **PATRÓN** `volumen_regimen` < `1.1777` → IC=+0.169 (n=2653)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 1.1777 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` > `0.6226` → IC=+0.162 (n=2653)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6561` → IC=+0.201 (n=959)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6561 (IC base=+0.180)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.181 (n=2461)
...
- **PATRÓN** `sigma_h` < `0.006` → IC=+0.193 (n=490)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.006 (IC base=+0.169)

- **PATRÓN** `drift_60min` |x|≤ `0.4155` → IC=+0.176 (n=556)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.4155 (IC base=+0.169)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.197 (n=568)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 6.0 (IC base=+0.169)

- **PATRÓN** `ibs_20min` > `0.4793` → IC=+0.213 (n=556)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4793 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` > `0.2039` → IC=+0.228 (n=402)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2039 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.987` → IC=+0.239 (n=155)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.987 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` < `0.6381` → IC=+0.192 (n=186)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` < 0.6381 (IC base=+0.169)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4857` → IC=+0.209 (n=232)
...
- **PATRÓN** `libro_liquidez` > `11341.8291` → IC=+0.191 (n=497)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 11341.8291 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `422.0` → IC=+0.166 (n=420)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 422.0 (IC base=+0.169)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.312 (n=301)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.173)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.569` → IC=+0.320 (n=137)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.198 (n=594)

  - _Acción_: Kelly boost +0.99€ cuando `libro_spread` < 0.04 (IC base=+0.173)

- **PATRÓN** `ballena_activa_n` < `17.0` → IC=+0.213 (n=113)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.208 (n=375)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0057 (IC base=+0.160)

- **PATRÓN** `drift_60min` |x|≤ `0.1283` → IC=+0.179 (n=247)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.89€ cuando `drift_60min` |x|≤ 0.1283 (IC base=+0.160)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.175 (n=574)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 6.0 (IC base=+0.160)

- **PATRÓN** `ibs_20min` > `0.4502` → IC=+0.203 (n=561)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4502 (IC base=+0.160)

- **PATRÓN** `dist_vwap_pct` > `0.1502` → IC=+0.194 (n=390)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.1502 (IC base=+0.160)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.391` → IC=+0.269 (n=106)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2967` → IC=+0.225 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2967 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` > `2.6255` → IC=+0.206 (n=182)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6255 (IC base=+0.160)
...
- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.149 (n=334)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.204 (n=761)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.194)

- **PATRÓN** `ibs_20min` > `0.89` → IC=+0.274 (n=484)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.89 (IC base=+0.194)

- **PATRÓN** `dist_vwap_pct` > `0.167` → IC=+0.229 (n=519)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.167 (IC base=+0.194)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.219` → IC=+0.246 (n=360)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.219 (IC base=+0.194)

- **PATRÓN** `volumen_regimen` > `0.6793` → IC=+0.213 (n=649)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6793 (IC base=+0.194)

- **PATRÓN** `volumen_pendiente_norm` > `0.2392` → IC=+0.267 (n=127)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2392 (IC base=+0.194)

- **PATRÓN** `volumen_spike_ratio` < `2.5223` → IC=+0.213 (n=687)
...
- **PATRÓN** `sigma_h` > `0.0097` → IC=+0.173 (n=261)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0097 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.167 (n=746)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 8.0 (IC base=+0.137)

- **PATRÓN** `ibs_20min` > `0.476` → IC=+0.170 (n=783)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` > 0.476 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.897` → IC=+0.237 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.897 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.176` → IC=+0.185 (n=198)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 8.176 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `0.8709` → IC=+0.157 (n=421)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.8709 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` > `1.1727` → IC=+0.148 (n=211)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` > 1.1727 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` > `0.1782` → IC=+0.182 (n=215)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.1782 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` < `2.2754` → IC=+0.148 (n=640)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 2.2754 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `1.8612` → IC=+0.145 (n=485)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.8612 (IC base=+0.137)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.153 (n=586)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `6676.3421` → IC=+0.177 (n=261)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 6676.3421 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `12.0` → IC=+0.193 (n=190)

  - _Acción_: Kelly boost +0.96€ cuando `ballena_activa_n` < 12.0 (IC base=+0.137)
...
- **PATRÓN** `sigma_h` > `0.0035` → IC=+0.128 (n=111)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.64€ cuando `sigma_h` > 0.0035 (IC base=+0.093)

- **PATRÓN** `hora_utc` > `10.0` → IC=+0.164 (n=132)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 10.0 (IC base=+0.093)

- **PATRÓN** `ibs_20min` > `0.6262` → IC=+0.164 (n=111)

  - _Acción_: Kelly boost +0.82€ cuando `ibs_20min` > 0.6262 (IC base=+0.093)

- **PATRÓN** `dist_vwap_pct` > `0.2504` → IC=+0.172 (n=65)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.2504 (IC base=+0.093)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.673` → IC=+0.189 (n=59)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 3.673 (IC base=+0.093)

- **PATRÓN** `volumen_regimen` < `0.5736` → IC=+0.159 (n=42)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.5736 (IC base=+0.093)

- **PATRÓN** `libro_liquidez` > `11005.6828` → IC=+0.146 (n=111)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 11005.6828 (IC base=+0.093)

- **PATRÓN** `ballena_activa_n` < `160.0` → IC=+0.176 (n=32)

  - _Acción_: Kelly boost +0.88€ cuando `ballena_activa_n` < 160.0 (IC base=+0.093)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.318 (n=64)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0075 (IC base=+0.267)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.304 (n=202)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.267)

- **PATRÓN** `ibs_20min` > `0.7759` → IC=+0.315 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7759 (IC base=+0.267)

- **PATRÓN** `dist_vwap_pct` > `0.1675` → IC=+0.302 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1675 (IC base=+0.267)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.316` → IC=+0.322 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.316 (IC base=+0.267)

- **PATRÓN** `volumen_regimen` < `0.8389` → IC=+0.277 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8389 (IC base=+0.267)

- **PATRÓN** `volumen_regimen` > `1.1833` → IC=+0.303 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1833 (IC base=+0.267)

- **PATRÓN** `volumen_pendiente_norm` > `0.0968` → IC=+0.364 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0968 (IC base=+0.267)
...
- **PATRÓN** `volumen_spike_ratio` > `2.0848` → IC=+0.328 (n=85)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.0848 (IC base=+0.267)

- **PATRÓN** `libro_liquidez` > `2746.5037` → IC=+0.268 (n=192)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2746.5037 (IC base=+0.267)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.205 (n=2048)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0087 (IC base=+0.166)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.174 (n=4529)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 6.0 (IC base=+0.166)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.308 (n=1554)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.166)

- **PATRÓN** `dist_vwap_pct` > `1.0461` → IC=+0.233 (n=653)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0461 (IC base=+0.166)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.632` → IC=+0.227 (n=2545)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.632 (IC base=+0.166)

- **PATRÓN** `volumen_regimen` < `0.8867` → IC=+0.160 (n=2086)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.8867 (IC base=+0.166)

- **PATRÓN** `volumen_regimen` > `1.0878` → IC=+0.155 (n=1418)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.0878 (IC base=+0.166)
...
- **PATRÓN** `volumen_spike_ratio` > `1.873` → IC=+0.175 (n=2792)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 1.873 (IC base=+0.166)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.168 (n=4284)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.02 (IC base=+0.166)

- **PATRÓN** `libro_liquidez` > `3848.1598` → IC=+0.189 (n=1505)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3848.1598 (IC base=+0.166)

- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.217 (n=2108)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.341 (n=256)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4696` → IC=+0.189 (n=220)
  ...
  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` < 1.4696 (IC base=+0.188)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.236 (n=571)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.188)

- **PATRÓN** `ballena_activa_n` < `50.0` → IC=+0.241 (n=272)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.177 (n=738)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 6.0 (IC base=+0.162)

- **PATRÓN** `ibs_20min` > `0.3271` → IC=+0.209 (n=738)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3271 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` > `0.2351` → IC=+0.222 (n=423)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2351 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.721` → IC=+0.189 (n=181)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 9.721 (IC base=+0.162)
...
- **PATRÓN** `volumen_regimen` < `1.2673` → IC=+0.169 (n=738)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.2673 (IC base=+0.162)

- **PATRÓN** `volumen_regimen` > `1.1034` → IC=+0.171 (n=335)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 1.1034 (IC base=+0.162)

- **PATRÓN** `volumen_pendiente_norm` < `0.0723` → IC=+0.176 (n=616)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2088` → IC=+0.195 (n=149)
  ...
  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.2088 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` < `2.3977` → IC=+0.180 (n=688)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 2.3977 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` > `1.7228` → IC=+0.181 (n=459)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 1.7228 (IC base=+0.162)

- **PATRÓN** `libro_liquidez` > `10640.5814` → IC=+0.188 (n=659)
...
- **PATRÓN** `sigma_h` > `0.01` → IC=+0.252 (n=216)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.6825` → IC=+0.259 (n=578)
...
- **PATRÓN** `sigma_h` < `0.0071` → IC=+0.157 (n=657)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0071 (IC base=+0.144)

- **PATRÓN** `drift_60min` |x|≤ `0.1419` → IC=+0.146 (n=329)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.1419 (IC base=+0.144)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.158 (n=674)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 8.0 (IC base=+0.144)

- **PATRÓN** `ibs_20min` > `0.7425` → IC=+0.239 (n=497)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7425 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `0.3732` → IC=+0.189 (n=297)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.3732 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.376` → IC=+0.175 (n=343)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 4.376 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` < `0.905` → IC=+0.166 (n=498)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.905 (IC base=+0.144)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1486` → IC=+0.206 (n=321)
...
- **PATRÓN** `libro_liquidez` > `9479.813` → IC=+0.244 (n=338)
...
- **PATRÓN** `volumen_regimen` > `1.2349` → IC=+0.231 (n=288)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2349 (IC base=+0.202)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8201` → IC=+0.208 (n=542)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8201 (IC base=+0.202)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.208 (n=997)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4612` → IC=+0.149 (n=488)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.4612 (IC base=+0.121)

- **PATRÓN** `volumen_spike_ratio` > `1.9004` → IC=+0.139 (n=976)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` > 1.9004 (IC base=+0.121)

- **PATRÓN** `libro_liquidez` > `8951.8366` → IC=+0.135 (n=671)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 8951.8366 (IC base=+0.121)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4007` → IC=+0.146 (n=473)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.4007 (IC base=+0.135)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.194 (n=181)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.186` → IC=+0.147 (n=537)

  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` > 0.186 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` > `1.0249` → IC=+0.172 (n=117)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 1.0249 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` < `0.4381` → IC=+0.147 (n=513)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.4381 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.061` → IC=+0.146 (n=541)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` < 7.061 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` < `1.2823` → IC=+0.144 (n=538)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.2823 (IC base=+0.135)

- **PATRÓN** `volumen_pendiente_norm` < `0.1175` → IC=+0.138 (n=495)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_pendiente_norm` < 0.1175 (IC base=+0.135)

- **PATRÓN** `volumen_pendiente_norm` > `0.1782` → IC=+0.161 (n=160)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` > 0.1782 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` < `1.4471` → IC=+0.182 (n=177)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 1.4471 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` > `1.8309` → IC=+0.138 (n=352)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 1.8309 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `8938.0512` → IC=+0.147 (n=480)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 8938.0512 (IC base=+0.135)
...
- **PATRÓN** `libro_liquidez` > `2469.6086` → IC=+0.140 (n=195)
  ...
  - _Acción_: Kelly boost +0.70€ cuando `libro_liquidez` > 2469.6086 (IC base=+0.104)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=1041)
...
| ✅ FAVORITO_CONFIRMADO#DOGE | 5945 | +0.082 | -751.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 28 | +0.033 | +3.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#240min | 8 | -0.120 | -5.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 5909 | +0.083 | -749.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 10187 | +0.127 | -152.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2937 | +0.172 | -12.37€ | 1 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 5867 | +0.112 | -92.44€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#XRP | 5928 | +0.105 | -408.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 20 | +0.000 | +1.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 5 | -0.018 | -1.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 5903 | +0.105 | -409.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 7842 | +0.178 | -595.57€ | 2 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 7842 | +0.178 | -595.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 1998 | +0.166 | -220.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 1998 | +0.166 | -220.32€ | 0 | 2 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 25053 | +0.188 | -2347.42€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 25053 | +0.188 | -2347.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 4461 | +0.146 | -715.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 4461 | +0.146 | -715.82€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 4340 | +0.162 | -592.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 4340 | +0.162 | -592.34€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 3976 | +0.221 | -153.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 3976 | +0.221 | -153.74€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 4224 | +0.183 | -446.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 4224 | +0.183 | -446.01€ | 0 | 3 |
...
| ✅ GBM_LATE_15M | 11762 | +0.057 | +4875.52€ | 0 | 15 |
| ✅ GBM_LATE_15M#15min | 11762 | +0.057 | +4875.52€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 1779 | +0.199 | +1321.03€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 1779 | +0.199 | +1321.03€ | 0 | 24 |
| ✅ GBM_LATE_15M#BTC | 1760 | +0.175 | +1162.63€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1760 | +0.175 | +1162.63€ | 0 | 29 |
| ✅ GBM_LATE_15M#DOGE | 1823 | +0.198 | +1340.28€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 1823 | +0.198 | +1340.28€ | 0 | 20 |
| ✅ GBM_LATE_15M#ETH | 1841 | -0.039 | +90.78€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1841 | -0.039 | +90.78€ | 3 | 11 |
| ✅ GBM_LATE_15M#SOL | 1925 | -0.049 | +423.50€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1925 | -0.049 | +423.50€ | 4 | 7 |
| ✅ GBM_LATE_15M#XRP | 2634 | -0.070 | +537.29€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 2634 | -0.070 | +537.29€ | 5 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 12514 | +0.061 | +6140.02€ | 0 | 18 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 12514 | +0.061 | +6140.02€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 2138 | -0.004 | +1374.17€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 2138 | -0.004 | +1374.17€ | 2 | 6 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 2728 | -0.023 | +356.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 2728 | -0.023 | +356.60€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 1623 | +0.259 | +1615.37€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 1623 | +0.259 | +1615.37€ | 0 | 20 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1926 | -0.049 | +2.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1926 | -0.049 | +2.34€ | 8 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 2079 | -0.014 | +719.09€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 2079 | -0.014 | +719.09€ | 3 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 2020 | +0.266 | +2072.45€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 2020 | +0.266 | +2072.45€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 9698 | +0.172 | +6916.24€ | 0 | 25 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 9698 | +0.172 | +6916.24€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB | 1365 | +0.202 | +1061.14€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB#15min | 1365 | +0.202 | +1061.14€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC | 1604 | +0.161 | +1129.94€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC#15min | 1604 | +0.161 | +1129.94€ | 0 | 25 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE | 1393 | +0.200 | +1068.21€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min | 1393 | +0.200 | +1068.21€ | 0 | 21 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH | 1590 | +0.146 | +982.29€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH#15min | 1590 | +0.146 | +982.29€ | 0 | 26 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL | 1800 | +0.124 | +1124.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL#15min | 1800 | +0.124 | +1124.84€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP | 1946 | +0.204 | +1549.81€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP#15min | 1946 | +0.204 | +1549.81€ | 0 | 19 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 2103 | +0.103 | +682.80€ | 0 | 16 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 