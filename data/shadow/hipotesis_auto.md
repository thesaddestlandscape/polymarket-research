# Hipótesis automáticas — 2026-07-20 05:20 UTC
_Generado por shadow_postmortem.py sobre 23916 resoluciones (PNL=+6136.64€)_
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.223 (n=283)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.212)

- **PATRÓN** `py_entrada` > `0.605` → IC=+0.260 (n=319)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.235 (n=334)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.233 (n=410)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.229)

- **PATRÓN** `libro_liquidez` > `4287.3371` → IC=+0.233 (n=215)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4287.3371 (IC base=+0.229)
...
- **PATRÓN** `py_entrada` > `0.665` → IC=+0.317 (n=227)
...
- **PATRÓN** `dist_vwap_pct` < `0.1203` → IC=+0.128 (n=687)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.1203 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.041` → IC=+0.246 (n=191)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.041 (IC base=+0.117)

- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.131 (n=1514)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0086 (IC base=+0.117)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.137 (n=1640)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` < 12.0 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` > `0.1365` → IC=+0.139 (n=386)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.1365 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` < `0.8045` → IC=+0.131 (n=1208)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.8045 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.174` → IC=+0.149 (n=260)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 6.174 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.097` → IC=+0.130 (n=1087)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` < 11.097 (IC base=+0.117)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.123 (n=422)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` < 0.0063 (IC base=+0.102)
...
- **PATRÓN** `dist_vwap_pct` > `0.4456` → IC=+0.198 (n=41)
  ...
  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.4456 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` < `0.1011` → IC=+0.149 (n=146)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.1011 (IC base=+0.102)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.893` → IC=+0.244 (n=41)
...
- **PATRÓN** `sigma_h` < `0.0044` → IC=+0.160 (n=139)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0044 (IC base=+0.100)

- **PATRÓN** `dist_vwap_pct` > `0.7664` → IC=+0.136 (n=20)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 0.7664 (IC base=+0.100)

- **PATRÓN** `dist_vwap_pct` < `0.283` → IC=+0.148 (n=89)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.283 (IC base=+0.100)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.14` → IC=+0.220 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.14 (IC base=+0.100)
...
- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.188 (n=203)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0092 (IC base=+0.175)

- **PATRÓN** `sigma_h` > `0.0247` → IC=+0.193 (n=203)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0247 (IC base=+0.175)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.237 (n=230)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.175)

- **PATRÓN** `dist_vwap_pct` > `0.4129` → IC=+0.183 (n=58)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.4129 (IC base=+0.175)

- **PATRÓN** `dist_vwap_pct` < `0.5244` → IC=+0.177 (n=360)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` < 0.5244 (IC base=+0.175)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.375` → IC=+0.174 (n=339)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` < 9.375 (IC base=+0.175)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.206 (n=788)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.2749` → IC=+0.216 (n=336)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2749 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.083` → IC=+0.293 (n=182)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.083 (IC base=+0.164)
...
- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.155 (n=247)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.5034` → IC=+0.207 (n=39)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5034 (IC base=+0.132)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.233` → IC=+0.208 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.233 (IC base=+0.132)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.143 (n=312)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` < 18.0 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.1876` → IC=+0.176 (n=109)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.1876 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.684` → IC=+0.284 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.684 (IC base=+0.142)
...
- **PATRÓN** `sigma_h` < `0.0164` → IC=+0.258 (n=279)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0164 (IC base=+0.256)

- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.272 (n=279)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.265 (n=245)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.256)

- **PATRÓN** `dist_vwap_pct` > `0.5545` → IC=+0.350 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5545 (IC base=+0.256)
...
- **PATRÓN** `sigma_h` > `0.0131` → IC=+0.185 (n=363)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.4709` → IC=+0.177 (n=221)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.4709 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` < `0.1043` → IC=+0.154 (n=588)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.1043 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.147` → IC=+0.296 (n=155)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.145 (n=91)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` < 0.0039 (IC base=+0.075)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.492` → IC=+0.262 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.492 (IC base=+0.075)
...
- **PATRÓN** `sigma_h` < `0.0254` → IC=+0.207 (n=343)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0254 (IC base=+0.208)

- **PATRÓN** `sigma_h` > `0.0092` → IC=+0.219 (n=343)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.5354` → IC=+0.268 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5354 (IC base=+0.208)

- **PATRÓN** `dist_vwap_pct` < `0.0981` → IC=+0.211 (n=199)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0981 (IC base=+0.208)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.26` → IC=-0.167 (n=28)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.098 (n=341)
...
- **PATRÓN** `ibs_15` < `0.0315` → IC=+0.132 (n=191)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.66€ cuando `ibs_15` < 0.0315 (IC base=+0.051)
...
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.121 (n=101)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` < 0.0058 (IC base=+0.046)

- **PATRÓN** `dist_vwap_pct` > `0.9401` → IC=+0.182 (n=20)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.9401 (IC base=+0.046)
...
- **PATRÓN** `sigma_h` < `0.0145` → IC=+0.122 (n=88)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` < 0.0145 (IC base=+0.110)

- **PATRÓN** `drift_60min` |x|≤ `0.1792` → IC=+0.141 (n=37)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.1792 (IC base=+0.110)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1341` → IC=+0.149 (n=55)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.75€ cuando `delta_ratio_macro` |x|> 0.1341 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.144 (n=85)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 4.0 (IC base=+0.110)

- **PATRÓN** `ibs_15` < `0.1613` → IC=+0.142 (n=79)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.71€ cuando `ibs_15` < 0.1613 (IC base=+0.110)
...
| ✅ FAVORITO_CONFIRMADO | 4103 | +0.188 | +77.02€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 2669 | +0.219 | +104.60€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 1191 | +0.198 | +52.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 889 | +0.212 | +6.72€ | 0 | 5 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 1460 | +0.181 | +18.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 886 | +0.222 | +47.15€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 1434 | +0.184 | +5.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 880 | +0.225 | +53.85€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 171 | +0.321 | +10.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 171 | +0.321 | +10.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 171 | +0.321 | +10.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 171 | +0.321 | +10.45€ | 0 | 0 |
| ✅ GBM_LATE_15M | 6048 | +0.109 | +2143.78€ | 0 | 10 |
| ✅ GBM_LATE_15M#15min | 6048 | +0.109 | +2143.78€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1439 | +0.082 | +308.24€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1439 | +0.082 | +308.24€ | 0 | 8 |
| ✅ GBM_LATE_15M#ETH | 1376 | +0.080 | +275.72€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1376 | +0.080 | +275.72€ | 0 | 6 |
| ✅ GBM_LATE_15M#SOL | 1640 | +0.100 | +647.45€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1640 | +0.100 | +647.45€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1593 | +0.166 | +912.37€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1593 | +0.166 | +912.37€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 3268 | +0.155 | +2031.65€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 3268 | +0.155 | +2031.65€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 831 | +0.122 | +435.28€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 831 | +0.122 | +435.28€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 857 | +0.116 | +413.73€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 857 | +0.116 | +413.73€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 828 | +0.116 | +370.22€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 828 | +0.116 | +370.22€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 752 | +0.279 | +812.42€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 752 | +0.279 | +812.42€ | 0 | 12 |
...
| ✅ GBM_LATE_15M_TARDIO | 3231 | +0.104 | +1209.16€ | 0 | 10 |
| ✅ GBM_LATE_15M_TARDIO#15min | 3231 | +0.104 | +1209.16€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 768 | +0.075 | +180.33€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 768 | +0.075 | +180.33€ | 0 | 8 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 777 | +0.042 | +94.67€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 777 | +0.042 | +94.67€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 735 | +0.056 | +170.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 735 | +0.056 | +170.84€ | 1 | 7 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 951 | +0.213 | +763.33€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 951 | +0.213 | +763.33€ | 0 | 13 |
| ✅ GBM_LATE_5M | 154 | +0.070 | +16.94€ | 0 | 0 |
| ✅ GBM_LATE_5M#5min | 154 | +0.070 | +16.94€ | 0 | 0 |
...
| ✅ GBM_LATE_5M#XRP | 84 | +0.140 | +19.62€ | 0 | 0 |
| ✅ GBM_LATE_5M#XRP#5min | 84 | +0.140 | +19.62€ | 0 | 0 |
...
| ✅ LEADLAG_BTC_XRP_15M | 212 | +0.023 | +11.76€ | 1 | 2 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 212 | +0.023 | +11.76€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 212 | +0.023 | +11.76€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 212 | +0.023 | +11.76€ | 1 | 2 |
...
| ✅ UPDOWN_GBM | 1904 | +0.021 | +134.01€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1644 | +0.045 | +172.07€ | 1 | 5 |
...
| ✅ UPDOWN_GBM#ETH | 850 | +0.025 | +43.49€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 757 | +0.040 | +54.55€ | 2 | 5 |
...
| ✅ UPDOWN_GBM#XRP | 156 | +0.044 | +32.44€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 145 | +0.065 | +35.73€ | 4 | 5 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.203: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.057 n=824 | neutral: IC=-0.014 n=605 | overbought(IBS>0.7): IC=+0.189 n=776
  - _Datos_: n=2354 IC=+0.090 PNL=+508.63€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.120 n=14469 | tras_1loss IC=+0.093 n=9252 | tras_2loss IC=+0.060 n=3730/40 | gap=+0.060 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1772 IC=+0.027 PNL=+148.76€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1772 IC=+0.027 PNL=+148.76€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=785 IC=+0.036 PNL=+66.16€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=785 IC=+0.036 PNL=+66.16€

**🟡 H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  - _Hipótesis_: Feature nueva 2026-06-27: drift del precio YES en Polymarket en últimas 5 obs (~5min). Si poly_drift<0 y decidimos BUY_NO (o poly_drift>0 y BUY_YES) → confluencia. Si diverge → reducción de stake. Hipótesis: confluencia Binance+Polymarket mejora IC; divergencia empeora.
  - _Umbral_: n≥40 en confluencia vs divergencia para validar el boost ×1.1
  - _Acción_: Si IC_confluencia>IC_divergencia con n≥40 → mantener el boost. Si no → retirar.
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.120 > 0.08 con n=90 PNL=+21.52€
  - _Datos_: n=90 IC=+0.120 PNL=+21.52€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=21458 IC=+0.123 PNL=+6212.52€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=21458 IC=+0.123 PNL=+6212.52€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=210 IC=+0.005 PNL=+2.02€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=210 IC=+0.005 PNL=+2.02€
...
**🟡 H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.110 > 0.1 con n=116 PNL=+39.09€
  - _Datos_: n=116 IC=+0.110 PNL=+39.09€
...
**〰️ H-CUSTOM-GBM-04H-ASIA** — UPDOWN_GBM 04h-05h UTC — media sesión asiática, ¿mejor franja nocturna?
  ...
  - _Estado_: n=132 IC=+0.015 PNL=+13.99€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=132 IC=+0.015 PNL=+13.99€
...
**🟡 H-CUSTOM-LATE-04H-ASIA** — GBM_LATE_15M 04h-05h UTC — media sesión asiática (gemela de GBM-04H-ASIA)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.132 > 0.08 con n=504 PNL=+199.46€
  - _Datos_: n=504 IC=+0.132 PNL=+199.46€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.235 > 0.2 con n=534 PNL=+347.40€
  - _Datos_: n=534 IC=+0.235 PNL=+347.40€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.138 > 0.08 con n=307 PNL=+119.69€
  - _Datos_: n=307 IC=+0.138 PNL=+119.69€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.163 > 0.08 con n=318 PNL=+167.19€
  - _Datos_: n=318 IC=+0.163 PNL=+167.19€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.276 > 0.08 con n=311 PNL=+4.00€
  - _Datos_: n=311 IC=+0.276 PNL=+4.00€
