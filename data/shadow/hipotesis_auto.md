# Hipótesis automáticas — 2026-07-19 07:49 UTC
_Generado por shadow_postmortem.py sobre 22143 resoluciones (PNL=+5380.90€)_
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.135 (n=1572)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.8954` → IC=+0.123 (n=1100)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.8954 (IC base=+0.113)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `11.031` → IC=+0.120 (n=977)

  - _Acción_: Kelly boost +0.60€ cuando `sigma_ewma_delta_pct` < 11.031 (IC base=+0.113)
...
- **PATRÓN** `sigma_h` < `0.0097` → IC=+0.196 (n=192)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0097 (IC base=+0.172)
...
- **PATRÓN** `dist_vwap_pct` < `0.5513` → IC=+0.171 (n=320)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` < 0.5513 (IC base=+0.172)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.536` → IC=+0.173 (n=289)
...
- **PATRÓN** `sigma_h` > `0.0045` → IC=+0.190 (n=1052)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.8516` → IC=+0.186 (n=964)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` < 0.8516 (IC base=+0.170)
...
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.331 (n=258)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0077 (IC base=+0.298)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.306 (n=266)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.3064` → IC=+0.325 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3064 (IC base=+0.298)

- **PATRÓN** `dist_vwap_pct` < `0.5452` → IC=+0.308 (n=259)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5452 (IC base=+0.298)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.458` → IC=+0.314 (n=229)
...
- **PATRÓN** `dist_vwap_pct` < `0.7685` → IC=+0.127 (n=953)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.7685 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.527` → IC=+0.132 (n=248)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` > 3.527 (IC base=+0.104)
...
- **PATRÓN** `sigma_h` < `0.0123` → IC=+0.217 (n=228)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0123 (IC base=+0.215)

- **PATRÓN** `sigma_h` > `0.0167` → IC=+0.235 (n=153)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0167 (IC base=+0.215)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.248 (n=113)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.215)

- **PATRÓN** `dist_vwap_pct` < `0.5335` → IC=+0.214 (n=309)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5335 (IC base=+0.215)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.003` → IC=+0.224 (n=291)
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 284 | +0.143 | +16.80€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 284 | +0.143 | +16.80€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 5758 | +0.107 | +1939.43€ | 0 | 9 |
| ✅ GBM_LATE_15M#15min | 5758 | +0.107 | +1939.43€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#XRP | 1506 | +0.164 | +830.61€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1506 | +0.164 | +830.61€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2989 | +0.153 | +1777.06€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2989 | +0.153 | +1777.06€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 673 | +0.278 | +711.14€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 673 | +0.278 | +711.14€ | 0 | 12 |
...
| ✅ GBM_LATE_15M_TARDIO | 2933 | +0.101 | +1022.33€ | 0 | 10 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2933 | +0.101 | +1022.33€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 864 | +0.212 | +673.87€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 864 | +0.212 | +673.87€ | 0 | 11 |
...
| ✅ LEADLAG_BTC_XRP_15M | 189 | +0.039 | +14.45€ | 1 | 2 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 189 | +0.039 | +14.45€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 189 | +0.039 | +14.45€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 189 | +0.039 | +14.45€ | 1 | 2 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 596 | +0.186 | +276.45€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 596 | +0.186 | +276.45€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 146 | +0.230 | +107.05€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 146 | +0.230 | +107.05€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.179: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.055 n=726 | neutral: IC=-0.003 n=541 | overbought(IBS>0.7): IC=+0.176 n=683
  - _Datos_: n=2087 IC=+0.087 PNL=+442.24€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.118 n=13318 | tras_1loss IC=+0.088 n=8626 | tras_2loss IC=+0.054 n=3519/40 | gap=+0.064 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=19685 IC=+0.121 PNL=+5456.78€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=19685 IC=+0.121 PNL=+5456.78€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.174 > 0.08 con n=348 PNL=+230.15€
  - _Datos_: n=348 IC=+0.174 PNL=+230.15€
