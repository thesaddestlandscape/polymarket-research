# Hipótesis automáticas — 2026-07-28 22:22 UTC
_Generado por shadow_postmortem.py sobre 40853 resoluciones (PNL=+8551.38€)_
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.194 (n=1298)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0076 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.169 (n=1757)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 8.0 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.2777` → IC=+0.180 (n=541)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.2777 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.341` → IC=+0.238 (n=640)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.341 (IC base=+0.145)

- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.155 (n=1456)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` > 0.0068 (IC base=+0.121)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.133 (n=1501)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 12.0 (IC base=+0.121)

- **PATRÓN** `dist_vwap_pct` > `0.6713` → IC=+0.123 (n=237)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` > 0.6713 (IC base=+0.121)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.139` → IC=+0.150 (n=324)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 8.139 (IC base=+0.121)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.136 (n=435)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 7.0 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` > `0.4476` → IC=+0.188 (n=62)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.4476 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.79` → IC=+0.167 (n=85)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 11.79 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.573` → IC=+0.167 (n=85)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 11.573 (IC base=+0.092)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.389` → IC=+0.182 (n=105)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 9.389 (IC base=+0.085)
...
- **PATRÓN** `sigma_h` < `0.0171` → IC=+0.243 (n=450)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0171 (IC base=+0.229)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.247 (n=417)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.229)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.229 (n=396)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.229)

- **PATRÓN** `dist_vwap_pct` > `0.6583` → IC=+0.283 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6583 (IC base=+0.229)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.537` → IC=+0.353 (n=127)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.537 (IC base=+0.229)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.224 (n=490)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.229)

- **PATRÓN** `sigma_h` < `0.0171` → IC=+0.208 (n=556)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.242 (n=378)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 12.0 (IC base=+0.204)

- **PATRÓN** `dist_vwap_pct` > `0.189` → IC=+0.311 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.189 (IC base=+0.204)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.391` → IC=+0.212 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.391 (IC base=+0.204)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.119` → IC=+0.197 (n=611)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` < 9.119 (IC base=+0.204)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.217 (n=458)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.204)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.124 (n=1286)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` > 0.0083 (IC base=+0.098)

- **PATRÓN** `dist_vwap_pct` > `0.4741` → IC=+0.135 (n=357)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 0.4741 (IC base=+0.098)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.941` → IC=+0.237 (n=314)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.941 (IC base=+0.098)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_ewma_delta_pct` > `12.471` → IC=+0.181 (n=70)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 12.471 (IC base=+0.072)

- **PATRÓN** `libro_liquidez` > `13158.3924` → IC=+0.122 (n=141)

  - _Acción_: Kelly boost +0.61€ cuando `libro_liquidez` > 13158.3924 (IC base=+0.072)
...
- **PATRÓN** `sigma_h` < `0.0091` → IC=+0.195 (n=188)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0091 (IC base=+0.171)

- **PATRÓN** `sigma_h` > `0.0102` → IC=+0.173 (n=502)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0102 (IC base=+0.171)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.177 (n=574)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 6.0 (IC base=+0.171)

- **PATRÓN** `dist_vwap_pct` > `0.461` → IC=+0.233 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.461 (IC base=+0.171)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.863` → IC=+0.294 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.863 (IC base=+0.171)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.169 (n=430)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.01 (IC base=+0.171)

- **PATRÓN** `sigma_h` < `0.0219` → IC=+0.156 (n=670)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0219 (IC base=+0.155)

- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.168 (n=670)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` > 0.0077 (IC base=+0.155)
...
- **PATRÓN** `dist_vwap_pct` > `0.1378` → IC=+0.220 (n=166)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1378 (IC base=+0.155)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.405` → IC=+0.153 (n=692)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` < 9.405 (IC base=+0.155)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.162 (n=534)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.604` → IC=+0.174 (n=654)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.87€ cuando `ibs_15` > 0.604 (IC base=+0.056)

- **PATRÓN** `dist_vwap_pct` > `0.1672` → IC=+0.165 (n=243)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.1672 (IC base=+0.056)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.951` → IC=+0.147 (n=242)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 4.951 (IC base=+0.056)

- **PATRÓN** `libro_liquidez` > `10361.4771` → IC=+0.138 (n=186)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 10361.4771 (IC base=+0.056)

- **PATRÓN** `dist_vwap_pct` > `0.9736` → IC=+0.172 (n=59)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.9736 (IC base=+0.047)
...
- **PATRÓN** `ibs_15` > `0.6674` → IC=+0.189 (n=210)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.94€ cuando `ibs_15` > 0.6674 (IC base=+0.044)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.038` → IC=+0.151 (n=64)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 7.038 (IC base=+0.044)

- **PATRÓN** `libro_liquidez` > `10723.3841` → IC=+0.130 (n=44)

  - _Acción_: Kelly boost +0.65€ cuando `libro_liquidez` > 10723.3841 (IC base=+0.044)

- **PATRÓN** `sigma_h` < `0.0055` → IC=+0.138 (n=139)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0055 (IC base=+0.057)

- **PATRÓN** `dist_vwap_pct` > `0.9263` → IC=+0.203 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9263 (IC base=+0.057)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.604 sube el IC de +0.056 a +0.174 en UPDOWN_GBM#15min (n=654). Ya aplicado como kelly_boost=+0.87€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.6674 sube el IC de +0.044 a +0.189 en UPDOWN_GBM#ETH#15min (n=210). Ya aplicado como kelly_boost=+0.94€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 1430 | +0.098 | +34.12€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 1430 | +0.098 | +34.12€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 574 | +0.094 | +6.52€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 574 | +0.094 | +6.52€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 530 | +0.117 | +9.27€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 530 | +0.117 | +9.27€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 307 | +0.073 | +18.69€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 307 | +0.073 | +18.69€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS | 50 | +0.346 | +2.63€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 50 | +0.346 | +2.63€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 50 | +0.346 | +2.63€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 50 | +0.346 | +2.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 6947 | +0.177 | -52.14€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 4571 | +0.200 | -47.37€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 317 | +0.058 | -21.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 821 | +0.128 | -56.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 1238 | +0.152 | +73.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 2063 | +0.183 | +1.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1530 | +0.198 | -43.61€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 105 | +0.042 | -14.32€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 2433 | +0.167 | -49.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1498 | +0.199 | -4.14€ | 0 | 5 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 28 | +0.367 | +1.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 28 | +0.367 | +1.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC | 10 | +0.125 | -0.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC#15min | 10 | +0.125 | -0.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 12 | +0.257 | +2.91€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 12 | +0.257 | +2.91€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 6 | +0.075 | -0.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 6 | +0.075 | -0.26€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 447 | +0.302 | +6.48€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 8810 | +0.093 | +2630.14€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 8810 | +0.093 | +2630.14€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 90 | +0.196 | +51.86€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min |
79€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2876 IC=+0.039 PNL=+284.79€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=1151 IC=+0.056 PNL=+158.43€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1151 IC=+0.056 PNL=+158.43€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.087 > 0.08 con n=1450 PNL=+244.08€
  - _Datos_: n=1450 IC=+0.087 PNL=+244.08€

**⏳ H-CUSTOM-LONGSHOT-BIAS** — Longshot bias — ¿mejor IC cuando py_mkt < 0.20 o > 0.80?
  - _Hipótesis_: Jon-Becker repo documenta formalmente: contratos a 1-20 cents tienen win_rate < precio implícito (compradores pierden sistemáticamente en longshots). En nuestro sistema: cuando py_mkt<0.20 el GBM predice BUY_NO con edge estructural adicional al del modelo. ¿Se confirma en nuestros datos? Buscar en feature pct_spot_vs_ref si los mercados extremos tienen mejor IC en BUY_NO.
  - _Umbral_: 30
  - _Acción_: Si IC>0.10 con n≥30 en mercados extremos → boost ×1.2 en BUY_NO cuando py_mkt<0.20
  - _Estado_: 23/30 ops en el filtro definido (IC actual=-0.100 PNL=+1.31€)
  - _Datos_: n=23 IC=-0.100 PNL=+1.31€

**〰️ H-CUSTOM-ETH15-REVERSION** — ETH#15min con drift_15min < -1 — ¿mean reversion?
  - _Hipótesis_: ETH y BTC tienen patrones opuestos: BTC funciona con momentum (drift>0.3). ETH funciona con reversión (drift<-1): 9/14 (64%) IC=+0.087. La hipótesis es que ETH tiene más mean-reversion que BTC en 15min.
  - _Umbral_: n≥20 y IC>+0.08
  - _Acción_: Si ETH drift<-1 confirma IC>0.08 con n≥20 → boost ×1.1 en ETH#15min cuando drift_15min<-1
  - _Estado_: n=115 IC=+0.013 PNL=+4.50€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=115 IC=+0.013 PNL=+4.50€
...
**〰️ H-LATE-WINDOW-5MIN** — Late-window BTC 5min — arbitraje timing vs Polymarket
  ...
  - _Estado_: n=376 IC=-0.064 PNL=-7.46€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=376 IC=-0.064 PNL=-7.46€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=1529 IC=+0.038 PNL=+116.36€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1529 IC=+0.038 PNL=+116.36€

**〰️ H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  - _Hipótesis_: Feature nueva 2026-06-27: drift del precio YES en Polymarket en últimas 5 obs (~5min). Si poly_drift<0 y decidimos BUY_NO (o poly_drift>0 y BUY_YES) → confluencia. Si diverge → reducción de stake. Hipótesis: confluencia Binance+Polymarket mejora IC; divergencia empeora.
  - _Umbral_: n≥40 en confluencia vs divergencia para validar el boost ×1.1
  - _Acción_: Si IC_confluencia>IC_divergencia con n≥40 → mantener el boost. Si no → retirar.
  - _Estado_: n=442 IC=+0.056 PNL=+54.96€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=442 IC=+0.056 PNL=+54.96€
...
**🟡 H-CUSTOM-CROSS-WINDOW-SPREAD-POS** — GBM 15min/60min: spread positivo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.239 > 0.08 con n=205 PNL=+44.54€
  - _Datos_: n=205 IC=+0.239 PNL=+44.54€
...
**〰️ H-CUSTOM-CROSS-WINDOW-SPREAD-NEG** — GBM 15min/60min: spread negativo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: n=193 IC=+0.049 PNL=+58.20€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=193 IC=+0.049 PNL=+58.20€
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=4552 IC=+0.072 PNL=+885.60€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=4552 IC=+0.072 PNL=+885.60€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=449 IC=+0.019 PNL=+14.35€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=449 IC=+0.019 PNL=+14.35€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=1301 IC=+0.032 PNL=+127.87€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1301 IC=+0.032 PNL=+127.87€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.159 > 0.1 con n=1059 PNL=+352.86€
  - _Datos_: n=1059 IC=+0.159 PNL=+352.86€
...
**〰️ H-CUSTOM-BUYNO-LONGSHOT-15MIN** — BUY_NO longshot en 15min (py_mkt≥0.55) — comprar NO barato pierde
  ...
  - _Estado_: n=63 IC=-0.023 PNL=+11.80€ — sin señal clara aún (umbral IC: min=None max=-0.1)
  - _Datos_: n=63 IC=-0.023 PNL=+11.80€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=304 IC=+0.072 PNL=+50.42€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=304 IC=+0.072 PNL=+50.42€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1050 IC=-0.074 PNL=+259.93€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1050 IC=-0.074 PNL=+259.93€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=1148 IC=+0.050 PNL=+114.23€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=1148 IC=+0.050 PNL=+114.23€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.119 > 0.08 con n=747 PNL=+204.67€
  - _Datos_: n=747 IC=+0.119 PNL=+204.67€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.210 > 0.2 con n=787 PNL=+477.65€
  - _Datos_: n=787 IC=+0.210 PNL=+477.65€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.095 > 0.08 con n=580 PNL=+138.13€
  - _Datos_: n=580 IC=+0.095 PNL=+138.13€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.101 > 0.08 con n=627 PNL=+179.35€
  - _Datos_: n=627 IC=+0.101 PNL=+179.35€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.126 > 0.08 con n=787 PNL=+381.23€
  - _Datos_: n=787 IC=+0.126 PNL=+381.23€
