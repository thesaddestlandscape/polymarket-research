# Hipótesis automáticas — 2026-08-23 20:12 UTC
_Generado por shadow_postmortem.py sobre 129726 resoluciones (PNL=+9276.49€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `158.64` → IC=-0.251 (n=1810)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.64
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=5432)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `148.64` → IC=-0.231 (n=195)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 148.64
  - _Potencial_: sin este filtro IC_bueno=+0.030 (n=587)
...
- **FILTRO** `restante_s_al_confirmar` > `643.08` → IC=-0.182 (n=152)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.212 (n=640)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.205)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.183 (n=474)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=1019)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.314 (n=325)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.7 (IC base=+0.127)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.290 (n=350)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.281 (n=400)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.282)

- **PATRÓN** `libro_liquidez` > `3447.6628` → IC=+0.305 (n=167)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3447.6628 (IC base=+0.282)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.203 (n=301)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.243 (n=177)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.222 (n=491)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.209)

- **PATRÓN** `libro_liquidez` > `793.7358` → IC=+0.229 (n=499)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 793.7358 (IC base=+0.209)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.196 (n=3782)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.204 (n=1906)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.73 (IC base=+0.193)

### FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.174 (n=646)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.193 (n=967)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.168 (n=970)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.201 (n=913)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.199 (n=625)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.204 (n=3349)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.194)
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.271 (n=500)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.76 (IC base=+0.227)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.217 (n=568)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `4.54` → IC=+0.149 (n=1088)

  - _Acción_: Kelly boost +0.75€ cuando `lag_apertura_s` < 4.54 (IC base=+0.130)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.219 (n=561)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.142)

- **PATRÓN** `restante_min` < `3.86` → IC=+0.157 (n=543)

  - _Acción_: Kelly boost +0.78€ cuando `restante_min` < 3.86 (IC base=+0.142)

- **PATRÓN** `restante_min` > `4.86` → IC=+0.160 (n=744)
...
- **PATRÓN** `lag_apertura_s` < `3.37` → IC=+0.156 (n=548)

  - _Acción_: Kelly boost +0.78€ cuando `lag_apertura_s` < 3.37 (IC base=+0.118)

### FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.311 (n=511)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.286 (n=218)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.276 (n=270)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.337 (n=170)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `2542.757` → IC=+0.329 (n=80)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2542.757 (IC base=+0.302)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9517` → IC=+0.232 (n=594)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9517 (IC base=+0.075)

- **PATRÓN** `dist_vwap_pct` > `0.171` → IC=+0.233 (n=226)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.171 (IC base=+0.075)

- **PATRÓN** `dist_vwap_pct` < `0.6914` → IC=+0.231 (n=329)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6914 (IC base=+0.075)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.989` → IC=+0.197 (n=759)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 4.989 (IC base=+0.075)

- **PATRÓN** `volumen_regimen` > `1.0803` → IC=+0.253 (n=156)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0803 (IC base=+0.075)

- **PATRÓN** `volumen_pendiente_norm` < `0.0782` → IC=+0.129 (n=856)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_pendiente_norm` < 0.0782 (IC base=+0.075)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5664` → IC=+0.133 (n=830)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 2.5664 (IC base=+0.075)

- **PATRÓN** `volumen_spike_ratio` > `1.4731` → IC=+0.132 (n=943)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 1.4731 (IC base=+0.075)

- **PATRÓN** `ibs_20min` < `0.2091` → IC=+0.130 (n=1275)

  - _Acción_: Kelly boost +0.65€ cuando `ibs_20min` < 0.2091 (IC base=+0.033)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4094` → IC=+0.219 (n=261)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4094 (IC base=+0.219)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.243 (n=270)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.219)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.219 (n=261)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 16.0 (IC base=+0.219)

- **PATRÓN** `ibs_20min` > `0.6079` → IC=+0.232 (n=233)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6079 (IC base=+0.219)

- **PATRÓN** `dist_vwap_pct` > `0.1985` → IC=+0.235 (n=179)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.802` → IC=+0.293 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.802 (IC base=+0.219)

- **PATRÓN** `volumen_regimen` < `1.2957` → IC=+0.226 (n=261)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2957 (IC base=+0.219)

- **PATRÓN** `volumen_regimen` > `0.8798` → IC=+0.239 (n=174)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8798 (IC base=+0.219)

- **PATRÓN** `volumen_pendiente_norm` < `0.0773` → IC=+0.231 (n=221)
...
- **PATRÓN** `volumen_spike_ratio` < `1.3527` → IC=+0.278 (n=79)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3527 (IC base=+0.219)
...
- **PATRÓN** `libro_liquidez` > `13331.7729` → IC=+0.258 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13331.7729 (IC base=+0.219)
...
- **FILTRO** `sigma_h` > `0.0048` → IC=-0.140 (n=220)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `ibs_20min` < `0.4437` → IC=-0.135 (n=264)
...
- **FILTRO** `sigma_h` > `0.0109` → IC=-0.149 (n=249)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=751)

- **FILTRO** `ibs_20min` > `0.7692` → IC=-0.165 (n=240)
...
- **FILTRO** `sigma_ewma_delta_pct` > `6.517` → IC=-0.163 (n=164)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.006 (n=836)
...
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.158 (n=759)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0078 (IC base=+0.056)

- **PATRÓN** `ibs_20min` > `0.9474` → IC=+0.247 (n=762)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9474 (IC base=+0.056)

- **PATRÓN** `dist_vwap_pct` > `0.4258` → IC=+0.259 (n=334)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4258 (IC base=+0.056)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.333` → IC=+0.128 (n=1302)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 2.333 (IC base=+0.056)

- **PATRÓN** `volumen_regimen` > `1.164` → IC=+0.190 (n=253)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_regimen` > 1.164 (IC base=+0.056)

- **PATRÓN** `volumen_pendiente_norm` < `0.1157` → IC=+0.159 (n=971)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_pendiente_norm` < 0.1157 (IC base=+0.056)

- **PATRÓN** `volumen_pendiente_norm` > `0.2513` → IC=+0.208 (n=190)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2513 (IC base=+0.056)

- **PATRÓN** `volumen_spike_ratio` < `1.5046` → IC=+0.168 (n=335)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.5046 (IC base=+0.056)

- **PATRÓN** `volumen_spike_ratio` > `2.9475` → IC=+0.168 (n=335)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 2.9475 (IC base=+0.056)

- **PATRÓN** `ballena_activa_n` < `97.0` → IC=+0.287 (n=448)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 97.0 (IC base=+0.056)

- **PATRÓN** `ibs_20min` < `0.1` → IC=+0.165 (n=1038)

  - _Acción_: Kelly boost +0.83€ cuando `ibs_20min` < 0.1 (IC base=+0.035)

- **PATRÓN** `dist_vwap_pct` > `0.5906` → IC=+0.219 (n=137)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5906 (IC base=+0.035)

- **PATRÓN** `dist_vwap_pct` < `0.1295` → IC=+0.189 (n=580)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.1295 (IC base=+0.035)

- **PATRÓN** `volumen_regimen` > `0.6268` → IC=+0.199 (n=630)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6268 (IC base=+0.035)

- **PATRÓN** `volumen_pendiente_norm` > `0.2539` → IC=+0.345 (n=95)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2539 (IC base=+0.035)

- **PATRÓN** `volumen_spike_ratio` > `1.6198` → IC=+0.250 (n=398)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.6198 (IC base=+0.035)

- **PATRÓN** `ballena_activa_n` < `58.0` → IC=+0.246 (n=282)
...
- **FILTRO** `ibs_20min` < `0.1074` → IC=-0.163 (n=93)
...
- **FILTRO** `libro_liquidez` < `8719.9362` → IC=-0.184 (n=93)
...
- **FILTRO** `ibs_20min` < `0.5094` → IC=-0.136 (n=248)

  - _Acción_: SKIP cuando `ibs_20min` < 0.5094
  - _Potencial_: sin este filtro IC_bueno=+0.132 (n=248)
...
- **PATRÓN** `ibs_20min` > `0.8462` → IC=+0.230 (n=124)
...
- **PATRÓN** `volumen_regimen` > `0.6209` → IC=+0.123 (n=136)
  ...
  - _Acción_: Kelly boost +0.62€ cuando `volumen_regimen` > 0.6209 (IC base=-0.002)
...
- **PATRÓN** `volumen_spike_ratio` > `1.9854` → IC=+0.127 (n=57)
  ...
  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.9854 (IC base=-0.002)

- **PATRÓN** `ballena_activa_n` < `80.0` → IC=+0.185 (n=90)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 80.0 (IC base=-0.002)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3766` → IC=+0.272 (n=406)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3766 (IC base=+0.263)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.304 (n=212)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.263)

- **PATRÓN** `ibs_20min` < `0.3636` → IC=+0.307 (n=460)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3636 (IC base=+0.263)

- **PATRÓN** `dist_vwap_pct` > `0.3841` → IC=+0.269 (n=145)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3841 (IC base=+0.263)

- **PATRÓN** `dist_vwap_pct` < `0.223` → IC=+0.270 (n=420)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.223 (IC base=+0.263)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.107` → IC=+0.262 (n=464)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.107 (IC base=+0.263)

- **PATRÓN** `volumen_regimen` > `0.7142` → IC=+0.284 (n=411)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7142 (IC base=+0.263)

- **PATRÓN** `volumen_pendiente_norm` > `0.2431` → IC=+0.344 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2431 (IC base=+0.263)

- **PATRÓN** `volumen_spike_ratio` > `1.5839` → IC=+0.255 (n=275)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5839 (IC base=+0.263)

- **PATRÓN** `ballena_activa_n` < `40.0` → IC=+0.234 (n=205)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 40.0 (IC base=+0.263)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.214 (n=652)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0087 (IC base=+0.158)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.173 (n=671)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9014` → IC=+0.258 (n=1299)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9014 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` > `1.0943` → IC=+0.251 (n=363)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0943 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.4` → IC=+0.278 (n=602)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.4 (IC base=+0.158)

- **PATRÓN** `volumen_regimen` > `0.6919` → IC=+0.176 (n=1162)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 0.6919 (IC base=+0.158)

- **PATRÓN** `volumen_pendiente_norm` > `0.103` → IC=+0.189 (n=686)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.103 (IC base=+0.158)

- **PATRÓN** `volumen_spike_ratio` < `2.3454` → IC=+0.153 (n=1501)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 2.3454 (IC base=+0.158)

- **PATRÓN** `volumen_spike_ratio` > `1.4539` → IC=+0.147 (n=1705)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.4539 (IC base=+0.158)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.169 (n=1530)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.01 (IC base=+0.158)

- **PATRÓN** `libro_liquidez` > `3501.8442` → IC=+0.195 (n=650)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 3501.8442 (IC base=+0.158)

- **PATRÓN** `ballena_activa_n` < `132.0` → IC=+0.191 (n=755)

  - _Acción_: Kelly boost +0.95€ cuando `ballena_activa_n` < 132.0 (IC base=+0.158)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.208 (n=1491)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0066 (IC base=+0.185)

- **PATRÓN** `drift_60min` |x|≤ `0.3397` → IC=+0.194 (n=1693)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.97€ cuando `drift_60min` |x|≤ 0.3397 (IC base=+0.185)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.212 (n=783)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.185)

- **PATRÓN** `ibs_20min` < `0.4` → IC=+0.244 (n=1696)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4 (IC base=+0.185)

- **PATRÓN** `dist_vwap_pct` < `0.3207` → IC=+0.178 (n=1471)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.3207 (IC base=+0.185)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.429` → IC=+0.194 (n=720)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 3.429 (IC base=+0.185)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.451` → IC=+0.185 (n=1712)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` < 5.451 (IC base=+0.185)

- **PATRÓN** `volumen_regimen` > `0.8511` → IC=+0.174 (n=929)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 0.8511 (IC base=+0.185)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5932` → IC=+0.184 (n=479)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` < 1.5932 (IC base=+0.185)

- **PATRÓN** `volumen_spike_ratio` > `2.7602` → IC=+0.204 (n=363)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7602 (IC base=+0.185)

- **PATRÓN** `ballena_activa_n` < `60.0` → IC=+0.194 (n=436)
...
- **PATRÓN** `volumen_regimen` > `0.897` → IC=+0.215 (n=170)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.897 (IC base=+0.181)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1727` → IC=+0.186 (n=103)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 2.1727 (IC base=+0.181)

- **PATRÓN** `libro_liquidez` > `11364.2884` → IC=+0.221 (n=170)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11364.2884 (IC base=+0.181)

- **PATRÓN** `ballena_activa_n` < `366.0` → IC=+0.175 (n=121)

  - _Acción_: Kelly boost +0.87€ cuando `ballena_activa_n` < 366.0 (IC base=+0.181)
...
- **PATRÓN** `sigma_h` < `0.0074` → IC=+0.247 (n=219)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0074 (IC base=+0.206)

- **PATRÓN** `drift_60min` |x|≤ `0.4913` → IC=+0.213 (n=249)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4913 (IC base=+0.206)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.228 (n=226)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.206)

- **PATRÓN** `ibs_20min` > `0.4362` → IC=+0.269 (n=249)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4362 (IC base=+0.206)

- **PATRÓN** `dist_vwap_pct` > `0.1643` → IC=+0.232 (n=181)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1643 (IC base=+0.206)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.788` → IC=+0.387 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.788 (IC base=+0.206)

- **PATRÓN** `volumen_regimen` > `0.6331` → IC=+0.237 (n=249)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6331 (IC base=+0.206)

- **PATRÓN** `volumen_pendiente_norm` > `0.1648` → IC=+0.305 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1648 (IC base=+0.206)

- **PATRÓN** `volumen_spike_ratio` < `1.4523` → IC=+0.207 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4523 (IC base=+0.206)

- **PATRÓN** `volumen_spike_ratio` > `2.4571` → IC=+0.244 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4571 (IC base=+0.206)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.207 (n=278)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.206)

- **PATRÓN** `libro_liquidez` > `9488.8794` → IC=+0.230 (n=113)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 9488.8794 (IC base=+0.206)

- **PATRÓN** `ballena_activa_n` < `198.0` → IC=+0.189 (n=162)

  - _Acción_: Kelly boost +0.95€ cuando `ballena_activa_n` < 198.0 (IC base=+0.206)

- **PATRÓN** `sigma_h` < `0.005` → IC=+0.203 (n=294)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.005 (IC base=+0.146)

- **PATRÓN** `drift_60min` |x|≤ `0.2729` → IC=+0.172 (n=294)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.2729 (IC base=+0.146)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.165 (n=228)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3077` → IC=+0.223 (n=294)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3077 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` < `0.5072` → IC=+0.170 (n=392)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` < 0.5072 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.607` → IC=+0.265 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.607 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` < `0.849` → IC=+0.153 (n=223)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.849 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` > `0.6089` → IC=+0.158 (n=334)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 0.6089 (IC base=+0.146)

- **PATRÓN** `volumen_pendiente_norm` > `0.1001` → IC=+0.174 (n=93)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.1001 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` < `1.9099` → IC=+0.210 (n=153)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9099 (IC base=+0.146)

### GBM_LATE_15M_MULTIHORIZONTE#SOL#15min
- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.203 (n=173)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0089 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.130 (n=347)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 8.0 (IC base=+0.110)

- **PATRÓN** `ibs_20min` > `0.8621` → IC=+0.222 (n=253)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8621 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` > `0.9936` → IC=+0.277 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9936 (IC base=+0.110)
...
- **PATRÓN** `volumen_regimen` > `0.6752` → IC=+0.129 (n=340)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` > 0.6752 (IC base=+0.110)

- **PATRÓN** `volumen_pendiente_norm` > `0.0991` → IC=+0.144 (n=133)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_pendiente_norm` > 0.0991 (IC base=+0.110)
...
- **PATRÓN** `sigma_h` < `0.0047` → IC=+0.213 (n=113)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0047 (IC base=+0.104)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.198 (n=160)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 14.0 (IC base=+0.104)

- **PATRÓN** `ibs_20min` < `0.4444` → IC=+0.210 (n=332)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4444 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.307` → IC=+0.133 (n=47)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` > 7.307 (IC base=+0.104)

- **PATRÓN** `volumen_regimen` > `0.8507` → IC=+0.146 (n=221)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 0.8507 (IC base=+0.104)

- **PATRÓN** `volumen_pendiente_norm` > `0.2708` → IC=+0.204 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2708 (IC base=+0.104)

- **PATRÓN** `volumen_spike_ratio` > `2.108` → IC=+0.160 (n=92)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 2.108 (IC base=+0.104)

- **PATRÓN** `libro_liquidez` > `2575.6322` → IC=+0.173 (n=151)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 2575.6322 (IC base=+0.104)
...
- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.272 (n=169)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0056 (IC base=+0.230)

- **PATRÓN** `drift_60min` |x|≤ `0.439` → IC=+0.232 (n=382)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.439 (IC base=+0.230)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.250 (n=186)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.1224` → IC=+0.329 (n=255)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1224 (IC base=+0.230)

- **PATRÓN** `dist_vwap_pct` < `0.6504` → IC=+0.240 (n=436)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6504 (IC base=+0.230)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.724` → IC=+0.263 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.724 (IC base=+0.230)

- **PATRÓN** `volumen_regimen` > `0.6961` → IC=+0.262 (n=342)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6961 (IC base=+0.230)

- **PATRÓN** `volumen_pendiente_norm` > `0.2945` → IC=+0.337 (n=41)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2945 (IC base=+0.230)

- **PATRÓN** `volumen_spike_ratio` > `2.8936` → IC=+0.247 (n=85)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8936 (IC base=+0.230)

- **PATRÓN** `ballena_activa_n` < `31.0` → IC=+0.186 (n=167)

  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 31.0 (IC base=+0.230)

### GBM_LATE_15M_PYCONFIRMADO
- **PATRÓN** `sigma_h` > `0.0058` → IC=+0.153 (n=142)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0058 (IC base=+0.086)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.211 (n=157)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.086)

- **PATRÓN** `ibs_20min` > `0.6` → IC=+0.155 (n=279)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` > 0.6 (IC base=+0.086)

- **PATRÓN** `dist_vwap_pct` > `0.5225` → IC=+0.220 (n=48)
...
- **PATRÓN** `ballena_activa_n` < `278.0` → IC=+0.220 (n=91)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.224 (n=743)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0082 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.159 (n=1130)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9497` → IC=+0.285 (n=1009)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9497 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.9996` → IC=+0.245 (n=371)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9996 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.406` → IC=+0.234 (n=1368)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.406 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `0.7026` → IC=+0.153 (n=661)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.7026 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` > `1.2394` → IC=+0.144 (n=501)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 1.2394 (IC base=+0.142)

- **PATRÓN** `volumen_pendiente_norm` > `0.2389` → IC=+0.181 (n=387)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.2389 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` < `2.7845` → IC=+0.141 (n=1913)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 2.7845 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` > `1.9176` → IC=+0.152 (n=1275)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.9176 (IC base=+0.142)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.150 (n=1742)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.01 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `3400.5924` → IC=+0.191 (n=742)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 3400.5924 (IC base=+0.142)

- **PATRÓN** `ballena_activa_n` < `166.0` → IC=+0.200 (n=750)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 166.0 (IC base=+0.142)

- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.214 (n=1772)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0073 (IC base=+0.184)

- **PATRÓN** `drift_60min` |x|≤ `0.3891` → IC=+0.197 (n=2013)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.3891 (IC base=+0.184)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.188 (n=1841)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5398` → IC=+0.241 (n=2013)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5398 (IC base=+0.184)

- **PATRÓN** `dist_vwap_pct` < `0.6691` → IC=+0.174 (n=1617)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.6691 (IC base=+0.184)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.586` → IC=+0.190 (n=1890)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` < 2.586 (IC base=+0.184)

- **PATRÓN** `volumen_regimen` < `0.6151` → IC=+0.175 (n=512)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 0.6151 (IC base=+0.184)

- **PATRÓN** `volumen_regimen` > `1.1898` → IC=+0.180 (n=511)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` > 1.1898 (IC base=+0.184)

- **PATRÓN** `volumen_pendiente_norm` > `0.1676` → IC=+0.235 (n=391)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1676 (IC base=+0.184)

- **PATRÓN** `volumen_spike_ratio` > `2.2777` → IC=+0.194 (n=599)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 2.2777 (IC base=+0.184)

- **PATRÓN** `ballena_activa_n` < `57.0` → IC=+0.134 (n=593)
...
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.171 (n=144)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0028 (IC base=+0.156)

- **PATRÓN** `sigma_h` > `0.0051` → IC=+0.198 (n=147)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0051 (IC base=+0.156)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.183 (n=326)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 6.0 (IC base=+0.156)

- **PATRÓN** `ibs_20min` > `0.3415` → IC=+0.218 (n=324)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3415 (IC base=+0.156)

- **PATRÓN** `dist_vwap_pct` > `0.2301` → IC=+0.235 (n=194)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2301 (IC base=+0.156)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.065` → IC=+0.247 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.065 (IC base=+0.156)

- **PATRÓN** `volumen_regimen` < `1.3007` → IC=+0.163 (n=324)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 1.3007 (IC base=+0.156)

- **PATRÓN** `volumen_regimen` > `0.8877` → IC=+0.170 (n=216)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 0.8877 (IC base=+0.156)

- **PATRÓN** `volumen_pendiente_norm` > `0.1369` → IC=+0.223 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1369 (IC base=+0.156)

- **PATRÓN** `volumen_spike_ratio` < `2.5982` → IC=+0.194 (n=276)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 2.5982 (IC base=+0.156)

- **PATRÓN** `volumen_spike_ratio` > `1.6756` → IC=+0.215 (n=184)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.6756 (IC base=+0.156)

- **PATRÓN** `libro_liquidez` > `10994.9669` → IC=+0.197 (n=216)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 10994.9669 (IC base=+0.156)
...
- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.142 (n=283)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0062 (IC base=+0.140)

- **PATRÓN** `sigma_h` > `0.008` → IC=+0.161 (n=107)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` > 0.008 (IC base=+0.140)

- **PATRÓN** `drift_60min` |x|≤ `0.126` → IC=+0.146 (n=142)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.126 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.159 (n=297)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 8.0 (IC base=+0.140)

- **PATRÓN** `ibs_20min` > `0.3665` → IC=+0.206 (n=321)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3665 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` > `0.201` → IC=+0.185 (n=179)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.201 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.544` → IC=+0.199 (n=171)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.544 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `0.7028` → IC=+0.174 (n=142)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` < 0.7028 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` > `1.2022` → IC=+0.170 (n=107)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 1.2022 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` > `0.2719` → IC=+0.327 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2719 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.4166` → IC=+0.221 (n=288)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4166 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `4127.191` → IC=+0.218 (n=214)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4127.191 (IC base=+0.140)

- **PATRÓN** `ballena_activa_n` < `233.0` → IC=+0.201 (n=195)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4329` → IC=+0.140 (n=265)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.70€ cuando `drift_60min` |x|≤ 0.4329 (IC base=+0.124)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.161 (n=184)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 11.0 (IC base=+0.124)

- **PATRÓN** `ibs_20min` < `0.0606` → IC=+0.231 (n=117)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0606 (IC base=+0.124)

- **PATRÓN** `dist_vwap_pct` < `0.7345` → IC=+0.144 (n=279)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.7345 (IC base=+0.124)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.73` → IC=+0.269 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.73 (IC base=+0.124)

- **PATRÓN** `volumen_regimen` < `0.5724` → IC=+0.148 (n=89)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.5724 (IC base=+0.124)

- **PATRÓN** `volumen_regimen` > `0.6234` → IC=+0.126 (n=236)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` > 0.6234 (IC base=+0.124)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1725` → IC=+0.177 (n=184)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 2.1725 (IC base=+0.124)

- **PATRÓN** `libro_liquidez` > `4118.5114` → IC=+0.140 (n=176)

  - _Acción_: Kelly boost +0.70€ cuando `libro_liquidez` > 4118.5114 (IC base=+0.124)

- **PATRÓN** `ballena_activa_n` < `171.0` → IC=+0.169 (n=125)

  - _Acción_: Kelly boost +0.85€ cuando `ballena_activa_n` < 171.0 (IC base=+0.124)

### GBM_LATE_15M_TARDIO#SOL#15min
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.171 (n=174)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0083 (IC base=+0.079)

- **PATRÓN** `ibs_20min` > `0.9231` → IC=+0.264 (n=176)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9231 (IC base=+0.079)

- **PATRÓN** `dist_vwap_pct` > `0.6741` → IC=+0.228 (n=101)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6741 (IC base=+0.079)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.074` → IC=+0.262 (n=103)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.074 (IC base=+0.079)

- **PATRÓN** `libro_liquidez` > `2724.2301` → IC=+0.269 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2724.2301 (IC base=+0.079)

- **PATRÓN** `ballena_activa_n` < `70.0` → IC=+0.183 (n=159)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 70.0 (IC base=+0.079)

- **PATRÓN** `sigma_h` < `0.005` → IC=+0.188 (n=126)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.005 (IC base=+0.098)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.154 (n=177)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 15.0 (IC base=+0.098)

- **PATRÓN** `ibs_20min` < `0.5758` → IC=+0.180 (n=376)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` < 0.5758 (IC base=+0.098)

- **PATRÓN** `dist_vwap_pct` < `0.4479` → IC=+0.125 (n=337)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` < 0.4479 (IC base=+0.098)

- **PATRÓN** `volumen_regimen` > `1.0525` → IC=+0.128 (n=170)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` > 1.0525 (IC base=+0.098)

- **PATRÓN** `volumen_pendiente_norm` > `0.0719` → IC=+0.160 (n=98)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` > 0.0719 (IC base=+0.098)

- **PATRÓN** `volumen_spike_ratio` > `1.4277` → IC=+0.151 (n=230)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.4277 (IC base=+0.098)

- **PATRÓN** `libro_liquidez` > `1400.9993` → IC=+0.135 (n=250)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 1400.9993 (IC base=+0.098)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` > `0.0107` → IC=+0.248 (n=216)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0107 (IC base=+0.172)

- **PATRÓN** `drift_60min` |x|≤ `0.0992` → IC=+0.177 (n=159)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.89€ cuando `drift_60min` |x|≤ 0.0992 (IC base=+0.172)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.198 (n=180)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 17.0 (IC base=+0.172)

- **PATRÓN** `ibs_20min` > `0.9795` → IC=+0.298 (n=216)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9795 (IC base=+0.172)

- **PATRÓN** `dist_vwap_pct` > `0.3864` → IC=+0.258 (n=213)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3864 (IC base=+0.172)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.924` → IC=+0.249 (n=265)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.924 (IC base=+0.172)

- **PATRÓN** `volumen_regimen` < `0.6105` → IC=+0.208 (n=159)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6105 (IC base=+0.172)

- **PATRÓN** `volumen_regimen` > `1.2439` → IC=+0.189 (n=159)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_regimen` > 1.2439 (IC base=+0.172)

- **PATRÓN** `volumen_pendiente_norm` > `0.1732` → IC=+0.243 (n=107)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1732 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` < `2.7393` → IC=+0.183 (n=427)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 2.7393 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` > `1.9023` → IC=+0.169 (n=285)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 1.9023 (IC base=+0.172)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.186 (n=523)

  - _Acción_: Kelly boost +0.93€ cuando `libro_spread` < 0.01 (IC base=+0.172)

- **PATRÓN** `libro_liquidez` > `2636.9304` → IC=+0.181 (n=318)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 2636.9304 (IC base=+0.172)

- **PATRÓN** `ballena_activa_n` < `36.0` → IC=+0.240 (n=206)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 36.0 (IC base=+0.172)

- **PATRÓN** `sigma_h` < `0.0054` → IC=+0.306 (n=178)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0054 (IC base=+0.209)

- **PATRÓN** `drift_60min` |x|≤ `0.3797` → IC=+0.228 (n=464)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3797 (IC base=+0.209)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.226 (n=491)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.209)

- **PATRÓN** `ibs_20min` < `0.4875` → IC=+0.283 (n=527)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4875 (IC base=+0.209)

- **PATRÓN** `dist_vwap_pct` < `0.676` → IC=+0.224 (n=577)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.676 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.01` → IC=+0.267 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.01 (IC base=+0.209)
...
- **PATRÓN** `volumen_regimen` > `0.8813` → IC=+0.236 (n=351)
...
- **PATRÓN** `ballena_activa_n` < `37.0` → IC=+0.147 (n=239)

  - _Acción_: Kelly boost +0.74€ cuando `ballena_activa_n` < 37.0 (IC base=+0.209)

### GBM_LATE_5M
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.164 (n=450)

- **PATRÓN** `dist_vwap_pct` > `0.2771` → IC=+0.159 (n=127)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.2771 (IC base=+0.048)

- **PATRÓN** `sigma_h` < `0.0037` → IC=+0.211 (n=233)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0037 (IC base=+0.151)

- **PATRÓN** `drift_60min` |x|≤ `0.1901` → IC=+0.177 (n=233)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.1901 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.199 (n=121)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.6029` → IC=+0.180 (n=307)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` < 0.6029 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` > `0.6733` → IC=+0.186 (n=84)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.6733 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` < `0.3086` → IC=+0.159 (n=335)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.3086 (IC base=+0.151)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.443` → IC=+0.180 (n=170)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 3.443 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` < `1.1826` → IC=+0.176 (n=307)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 1.1826 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` > `0.6526` → IC=+0.152 (n=349)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.6526 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` < `0.0968` → IC=+0.151 (n=302)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_pendiente_norm` < 0.0968 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` > `0.0763` → IC=+0.170 (n=180)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` > 0.0763 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` < `2.6912` → IC=+0.187 (n=346)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 2.6912 (IC base=+0.151)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.164 (n=450)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.151)

- **PATRÓN** `libro_liquidez` > `6262.9011` → IC=+0.175 (n=349)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 6262.9011 (IC base=+0.151)
...
- **PATRÓN** `sigma_h` > `0.0037` → IC=+0.239 (n=21)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0037 (IC base=+0.045)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.239 (n=21)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 16.0 (IC base=+0.045)

- **PATRÓN** `dist_vwap_pct` > `0.5259` → IC=+0.241 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5259 (IC base=+0.045)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.401` → IC=+0.190 (n=27)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` < 5.401 (IC base=+0.045)

- **PATRÓN** `libro_liquidez` > `12797.7595` → IC=+0.283 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12797.7595 (IC base=+0.045)
...
- **PATRÓN** `libro_liquidez` > `2553.7488` → IC=+0.181 (n=67)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 2553.7488 (IC base=+0.098)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.156 (n=536)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=1662)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.257 (n=512)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=1599)

- **FILTRO** `ibs_20min` > `0.2762` → IC=-0.226 (n=527)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2762
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=1584)

- **FILTRO** `ballena_activa_n` > `17.0` → IC=-0.141 (n=522)

  - _Acción_: SKIP cuando `ballena_activa_n` > 17.0
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=1589)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.5` → IC=-0.167 (n=139)

  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.109 (n=182)

- **FILTRO** `ibs_20min` < `0.7767` → IC=-0.173 (n=105)
...
- **PATRÓN** `py_entrada` > `0.505` → IC=+0.121 (n=143)

  - _Acción_: Kelly boost +0.60€ cuando `py_entrada` > 0.505 (IC base=-0.011)

### MOMENTUM_IBS_15M_BALLENA#BTC#15min
- **FILTRO** `hora_utc` > `11.0` → IC=-0.135 (n=168)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=199)

- **FILTRO** `py_entrada` > `0.55` → IC=-0.272 (n=90)

  - _Acción_: SKIP cuando `py_entrada` > 0.55
  - _Potencial_: sin este filtro IC_bueno=+0.005 (n=277)

- **FILTRO** `ibs_20min` > `0.2136` → IC=-0.217 (n=90)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2136
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=277)

- **FILTRO** `ballena_activa_n` > `82.0` → IC=-0.206 (n=90)
...
- **FILTRO** `py_entrada` > `0.64` → IC=-0.211 (n=119)

  - _Acción_: SKIP cuando `py_entrada` > 0.64
  - _Potencial_: sin este filtro IC_bueno=+0.044 (n=239)

- **FILTRO** `ibs_20min` > `0.2857` → IC=-0.215 (n=121)
...
- **FILTRO** `hora_utc` > `17.0` → IC=-0.158 (n=77)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 17.0
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.295 (n=76)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=252)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.208 (n=94)
...
- **FILTRO** `drift_20min_pct` |x|> `0.1538` → IC=-0.142 (n=216)
...
- **FILTRO** `py_entrada` < `0.36` → IC=-0.279 (n=1291)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=4029)

- **FILTRO** `ibs_7min` < `0.7405` → IC=-0.226 (n=1330)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7405
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=3990)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.179 (n=1781)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=3539)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.223 (n=1573)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=4829)

- **FILTRO** `ibs_7min` > `0.3` → IC=-0.168 (n=1593)

  - _Acción_: SKIP cuando `ibs_7min` > 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=4809)
...
- **FILTRO** `ibs_7min` < `1.0` → IC=-0.174 (n=520)
...
- **FILTRO** `ballena_activa_n` > `14.0` → IC=-0.219 (n=183)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=563)

- **FILTRO** `hora_utc` > `15.0` → IC=-0.121 (n=320)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.079 (n=705)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.250 (n=246)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=779)

- **FILTRO** `drift_7min_pct` |x|> `0.1375` → IC=-0.169 (n=348)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1375
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=677)

- **FILTRO** `ibs_7min` > `0.2899` → IC=-0.189 (n=348)
...
- **FILTRO** `ballena_activa_n` > `131.0` → IC=-0.169 (n=273)

  - _Acción_: SKIP cuando `ballena_activa_n` > 131.0
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=829)

- **FILTRO** `py_entrada` > `0.53` → IC=-0.188 (n=277)

  - _Acción_: SKIP cuando `py_entrada` > 0.53
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=849)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.198 (n=180)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=548)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.341 (n=180)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=548)

- **FILTRO** `ibs_7min` < `0.7191` → IC=-0.256 (n=240)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7191
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=488)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.279 (n=179)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=549)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.267 (n=243)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=860)

- **FILTRO** `ibs_7min` > `0.812` → IC=-0.175 (n=275)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.158 (n=217)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=689)
...
- **FILTRO** `ibs_7min` < `0.7803` → IC=-0.197 (n=226)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7803
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=680)

- **FILTRO** `ballena_activa_n` > `43.0` → IC=-0.223 (n=222)

  - _Acción_: SKIP cuando `ballena_activa_n` > 43.0
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=684)

- **FILTRO** `py_entrada` > `0.64` → IC=-0.303 (n=221)

  - _Acción_: SKIP cuando `py_entrada` > 0.64
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=682)

- **FILTRO** `ibs_7min` > `0.1719` → IC=-0.154 (n=307)

  - _Acción_: SKIP cuando `ibs_7min` > 0.1719
  - _Potencial_: sin este filtro IC_bueno=-0.077 (n=596)

- **FILTRO** `ballena_activa_n` > `35.0` → IC=-0.203 (n=224)

  - _Acción_: SKIP cuando `ballena_activa_n` > 35.0
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=679)
...
- **FILTRO** `ibs_7min` < `0.7727` → IC=-0.201 (n=242)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7727
  - _Potencial_: sin este filtro IC_bueno=+0.005 (n=731)

- **FILTRO** `ballena_activa_n` > `17.0` → IC=-0.213 (n=242)

  - _Acción_: SKIP cuando `ballena_activa_n` > 17.0
  - _Potencial_: sin este filtro IC_bueno=+0.009 (n=731)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.172 (n=288)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=880)
...
- **FILTRO** `ibs_7min` < `0.75` → IC=-0.224 (n=208)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=657)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.215 (n=212)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=653)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.293 (n=225)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=852)

- **FILTRO** `ibs_7min` > `0.2716` → IC=-0.130 (n=366)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2716
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=711)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.140 (n=348)
...
- **PATRÓN** `hora_utc` < `16.0` → IC=+0.124 (n=123)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` < 16.0 (IC base=+0.082)

- **PATRÓN** `ballena_activa_n` < `22.0` → IC=+0.167 (n=52)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 22.0 (IC base=+0.082)

### STRUCT_NO_15M#BTC#15min
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.008 (n=1013)
...
- **PATRÓN** `sigma_h` < `0.003` → IC=+0.131 (n=177)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.294 (n=105)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0041 (IC base=+0.283)

- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.357 (n=40)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0048 (IC base=+0.283)

- **PATRÓN** `drift_60min` |x|≤ `0.1981` → IC=+0.295 (n=120)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1981 (IC base=+0.283)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1559` → IC=+0.293 (n=80)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1559 (IC base=+0.283)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.307 (n=107)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.283)

- **PATRÓN** `ibs_15` > `0.7117` → IC=+0.352 (n=120)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7117 (IC base=+0.283)

- **PATRÓN** `dist_vwap_pct` > `0.5302` → IC=+0.357 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5302 (IC base=+0.283)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.145` → IC=+0.309 (n=66)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.145 (IC base=+0.283)

- **PATRÓN** `libro_liquidez` > `7219.8643` → IC=+0.333 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7219.8643 (IC base=+0.283)

- **PATRÓN** `ballena_activa_n` < `550.0` → IC=+0.357 (n=47)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 550.0 (IC base=+0.283)
...
- **PATRÓN** `sigma_h` < `0.0045` → IC=+0.333 (n=34)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0045 (IC base=+0.309)

- **PATRÓN** `sigma_h` > `0.0054` → IC=+0.340 (n=23)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0054 (IC base=+0.309)

- **PATRÓN** `drift_60min` |x|≤ `0.0603` → IC=+0.395 (n=17)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0603 (IC base=+0.309)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1559` → IC=+0.386 (n=33)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1559 (IC base=+0.309)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.316 (n=36)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` > `0.7045` → IC=+0.404 (n=50)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7045 (IC base=+0.309)

- **PATRÓN** `dist_vwap_pct` > `0.1511` → IC=+0.318 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1511 (IC base=+0.309)

- **PATRÓN** `dist_vwap_pct` < `0.3445` → IC=+0.304 (n=44)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3445 (IC base=+0.309)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.937` → IC=+0.380 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.937 (IC base=+0.309)

- **PATRÓN** `libro_liquidez` > `4118.4585` → IC=+0.342 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4118.4585 (IC base=+0.309)

### UPDOWN_GBM_15M_TARDIO
- **FILTRO** `ibs_15` < `0.4043` → IC=-0.326 (n=84)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: SKIP cuando `ibs_15` < 0.4043
  - _Potencial_: sin este filtro IC_bueno=+0.144 (n=254)

- **FILTRO** `sigma_h` > `0.0086` → IC=-0.157 (n=720)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0086
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=1399)
...
- **FILTRO** `sigma_ewma_delta_pct` > `16.723` → IC=-0.194 (n=286)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 16.723
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=1833)

- **PATRÓN** `ibs_15` > `0.4043` → IC=+0.144 (n=254)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.72€ cuando `ibs_15` > 0.4043 (IC base=-0.049)
...
- **FILTRO** `sigma_h` < `0.0039` → IC=-0.223 (n=236)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0039
  - _Potencial_: sin este filtro IC_bueno=-0.182 (n=237)

- **FILTRO** `sigma_ewma_delta_pct` > `19.22` → IC=-0.245 (n=96)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 19.22
  - _Potencial_: sin este filtro IC_bueno=-0.191 (n=377)

- **FILTRO** `libro_liquidez` < `11395.9187` → IC=-0.203 (n=156)

  - _Acción_: SKIP cuando `libro_liquidez` < 11395.9187
  - _Potencial_: sin este filtro IC_bueno=-0.202 (n=317)

### UPDOWN_GBM_15M_TARDIO#ETH#15min
- **FILTRO** `ibs_15` < `0.4827` → IC=-0.370 (n=44)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: SKIP cuando `ibs_15` < 0.4827
  - _Potencial_: sin este filtro IC_bueno=+0.167 (n=91)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3048` → IC=+0.194 (n=34)

  - _Acción_: Kelly boost +0.97€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3048 (IC base=-0.011)

- **PATRÓN** `ibs_15` > `0.4827` → IC=+0.167 (n=91)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.83€ cuando `ibs_15` > 0.4827 (IC base=-0.011)

- **PATRÓN** `libro_liquidez` > `10015.5876` → IC=+0.222 (n=34)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 