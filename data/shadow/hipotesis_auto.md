# Hipótesis automáticas — 2026-07-04 00:14 UTC
_Generado por shadow_postmortem.py sobre 3274 resoluciones (PNL=-17.05€)_
...
### UPDOWN_GBM#15min
- **FILTRO** `ibs_15` > `0.3451` → IC=-0.140 (n=48)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: SKIP cuando `ibs_15` > 0.3451
  - _Potencial_: sin este filtro IC_bueno=+0.041 (n=146)

- **PATRÓN** `sigma_h` < `0.0098` → IC=+0.197 (n=87)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0098 (IC base=+0.029)

- **PATRÓN** `ibs_15` < `0.0634` → IC=+0.142 (n=65)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.71€ cuando `ibs_15` < 0.0634 (IC base=+0.029)
...
- **FILTRO** `ibs_15` < `0.7576` → IC=-0.167 (n=16)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1289` → IC=+0.122 (n=72)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.61€ cuando `pct_spot_vs_ref` |x|≤ 0.1289 (IC base=+0.088)

- **PATRÓN** `sigma_h` < `0.0091` → IC=+0.180 (n=48)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.0091 (IC base=+0.088)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.152 (n=21)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 6.0 (IC base=+0.088)

- **PATRÓN** `ibs_15` < `0.2241` → IC=+0.150 (n=58)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.75€ cuando `ibs_15` < 0.2241 (IC base=+0.088)
...
- **FILTRO** `drift_60min` |x|> `0.3271` → IC=-0.132 (n=17)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.3271
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=19)

- **FILTRO** `drift_15min` |x|> `0.7778` → IC=-0.132 (n=17)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
...
- **FILTRO** `ibs_15` > `0.0244` → IC=-0.125 (n=22)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: SKIP cuando `ibs_15` > 0.0244
  - _Potencial_: sin este filtro IC_bueno=+0.300 (n=8)

- **PATRÓN** `sigma_h` < `0.0137` → IC=+0.167 (n=22)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0137 (IC base=+0.091)
...
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 557 | +0.069 | +83.50€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 557 | +0.069 | +83.50€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 145 | +0.031 | +7.09€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 145 | +0.031 | +7.09€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 146 | +0.013 | +3.81€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 146 | +0.013 | +3.81€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL | 139 | +0.110 | +36.41€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 139 | +0.110 | +36.41€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP | 127 | +0.128 | +36.20€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 127 | +0.128 | +36.20€ | 0 | 0 |
| 🚫 GBM_LATE_60M | 8 | -0.120 | -3.18€ | 0 | 0 |
| 🚫 GBM_LATE_60M#60min | 8 | -0.120 | -3.18€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN | 8 | +0.040 | +1.00€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN#5min | 8 | +0.040 | +1.00€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN#BTC | 8 | +0.040 | +1.00€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN#BTC#5min | 8 | +0.040 | +1.00€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 887 | -0.042 | -56.02€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 655 | -0.022 | -28.23€ | 1 | 2 |
| 🚫 UPDOWN_GBM#240min | 11 | -0.148 | -4.31€ | 0 | 0 |
| 🚫 UPDOWN_GBM#5min | 75 | -0.162 | -22.48€ | 4 | 0 |
| ✅ UPDOWN_GBM#60min | 125 | -0.051 | -9.74€ | 7 | 0 |
| ✅ UPDOWN_GBM#BNB | 24 | -0.154 | -7.27€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 24 | -0.154 | -7.27€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 252 | -0.047 | -37.29€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 181 | -0.025 | -26.58€ | 2 | 0 |
...
| ✅ UPDOWN_GBM#ETH | 356 | -0.014 | +4.55€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 271 | +0.002 | +11.52€ | 3 | 4 |
...
| ✅ UPDOWN_GBM#SOL | 150 | -0.079 | -11.46€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 90 | -0.065 | -7.37€ | 7 | 0 |
| 🚫 UPDOWN_GBM#SOL#5min | 23 | -0.060 | -3.42€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 30 | -0.094 | -1.52€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#daily | 5 | -0.018 | +2.28€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 76 | -0.013 | +1.94€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 65 | +0.022 | +5.23€ | 4 | 1 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.063) — sin ventaja clara. oversold(IBS<0.3): IC=-0.014 n=177 | neutral: IC=-0.077 n=147 | overbought(IBS>0.7): IC=-0.048 n=155
  - _Datos_: n=497 IC=-0.043 PNL=-30.79€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=01h: IC=-0.200 n=28 PNL=-13.74€ → FILTRAR | H=02h: IC=-0.180 n=23 PNL=-6.09€ → FILTRAR | H=03h: IC=-0.110 n=15 PNL=-5.87€ → FILTRAR | H=10h: IC=-0.154 n=15 PNL=-6.77€ → FILTRAR | H=16h: IC=+0.167 n=25 PNL=+7.36€ → BOOST | H=20h: IC=-0.167 n=28 PNL=-9.59€ → FILTRAR | H=23h: IC=+0.115 n=24 PNL=+9.49€ → BOOST
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=90 IC=-0.065 PNL=-7.37€

**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  - _Umbral_: n≥40 tras 2 losses y gap(IC_tras_win - IC_tras_2loss)≥0.05
  - _Acción_: Reducir stake (no desactivar) 1-2h tras 2 derrotas consecutivas en el mismo subtype
  - _Estado_: tras_win IC=+0.013 n=1578 | tras_1loss IC=-0.024 n=1592 | tras_2loss IC=-0.042 n=816/40 | gap=+0.055 (umbral 0.05)
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=53/40 IC=-0.009 PNL=-2.05€ | BTC#60min: n=42/40 IC=-0.068 PNL=-6.16€ | SOL#60min: n=30/40 IC=-0.094 PNL=-1.52€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=764 IC=-0.038 PNL=-42.47€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=764 IC=-0.038 PNL=-42.47€
...
**〰️ H-CUSTOM-GBM-60MIN-BUYNO** — GBM 60min BUY_NO — tracking por separado
  ...
  - _Estado_: n=36 IC=-0.026 PNL=-2.14€ — sin señal clara aún (umbral IC: min=0.05 max=None)
  - _Datos_: n=36 IC=-0.026 PNL=-2.14€
...
**🔴 H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.025 < 0.02 con n=181 PNL=-26.58€
  - _Datos_: n=181 IC=-0.025 PNL=-26.58€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=282 IC=-0.025 PNL=-37.97€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=282 IC=-0.025 PNL=-37.97€
...
**〰️ H-CUSTOM-ETH15-REVERSION** — ETH#15min con drift_15min < -1 — ¿mean reversion?
  ...
  - _Estado_: n=48 IC=+0.060 PNL=+10.55€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=48 IC=+0.060 PNL=+10.55€
...
**⏳ H-LATE-WINDOW-5MIN** — Late-window BTC 5min — arbitraje timing vs Polymarket
  ...
  - _Estado_: 8/30 ops en el filtro definido (IC actual=+0.040 PNL=+1.00€)
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=243 IC=+0.006 PNL=+8.20€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=243 IC=+0.006 PNL=+8.20€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=841 IC=+0.040 PNL=+68.52€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=841 IC=+0.040 PNL=+68.52€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=50 IC=-0.038 PNL=-11.10€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=50 IC=-0.038 PNL=-11.10€
...
**⏳ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: 42/50 ops en el filtro definido (IC actual=+0.091 PNL=+7.32€)
  - _Datos_: n=42 IC=+0.091 PNL=+7.32€
...
**〰️ H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: n=93 IC=-0.005 PNL=-8.66€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=93 IC=-0.005 PNL=-8.66€
