# Hipótesis automáticas — 2026-07-17 03:31 UTC
_Generado por shadow_postmortem.py sobre 18031 resoluciones (PNL=+3808.37€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.192 (n=1035)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 18.0 (IC base=+0.177)

- **PATRÓN** `py_entrada` > `0.695` → IC=+0.294 (n=464)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.177)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.182 (n=1185)
...
- **PATRÓN** `dist_vwap_pct` > `0.8712` → IC=+0.131 (n=101)
  ...
  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.8712 (IC base=+0.108)
...
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.229 (n=367)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.169 (n=361)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 8.0 (IC base=+0.158)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.284 (n=179)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.256)

- **PATRÓN** `dist_vwap_pct` > `0.9553` → IC=+0.364 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9553 (IC base=+0.256)
...
- **PATRÓN** `sigma_h` > `0.0151` → IC=+0.195 (n=254)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0151 (IC base=+0.114)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.139 (n=253)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` < 5.0 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` > `0.3606` → IC=+0.169 (n=176)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` > 0.3606 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` < `0.1434` → IC=+0.141 (n=338)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.1434 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.728` → IC=+0.296 (n=91)
...
- **PATRÓN** `sigma_h` < `0.033` → IC=+0.209 (n=228)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.033 (IC base=+0.205)

- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.223 (n=229)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0113 (IC base=+0.205)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.256 (n=76)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.205)

- **PATRÓN** `dist_vwap_pct` > `0.3403` → IC=+0.283 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3403 (IC base=+0.205)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.35` → IC=+0.266 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.35 (IC base=+0.205)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2813 | +0.175 | -57.64€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 1816 | +0.208 | +15.24€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 987 | +0.173 | -33.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 600 | +0.218 | +24.18€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 63 | +0.315 | +2.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 63 | +0.315 | +2.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 63 | +0.315 | +2.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 63 | +0.315 | +2.77€ | 0 | 0 |
| ✅ GBM_LATE_15M | 5096 | +0.102 | +1587.18€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 5096 | +0.102 | +1587.18€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#SOL | 1365 | +0.106 | +542.89€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1365 | +0.106 | +542.89€ | 1 | 7 |
| ✅ GBM_LATE_15M#XRP | 1298 | +0.155 | +659.42€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1298 | +0.155 | +659.42€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2302 | +0.150 | +1331.29€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2302 | +0.150 | +1331.29€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 491 | +0.277 | +515.54€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 491 | +0.277 | +515.54€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 2280 | +0.095 | +716.67€ | 0 | 7 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2280 | +0.095 | +716.67€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 656 | +0.204 | +482.69€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 656 | +0.204 | +482.69€ | 0 | 11 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.106 n=10566 | tras_1loss IC=+0.072 n=7260 | tras_2loss IC=+0.042 n=3074/40 | gap=+0.065 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=15573 IC=+0.109 PNL=+3884.25€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=15573 IC=+0.109 PNL=+3884.25€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.241 > 0.2 con n=396 PNL=+269.49€
  - _Datos_: n=396 IC=+0.241 PNL=+269.49€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.255 > 0.08 con n=214 PNL=-8.82€
  - _Datos_: n=214 IC=+0.255 PNL=-8.82€
