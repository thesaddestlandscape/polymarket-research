# Hipótesis automáticas — 2026-08-22 01:35 UTC
_Generado por shadow_postmortem.py sobre 107917 resoluciones (PNL=+8747.68€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `156.97` → IC=-0.262 (n=1524)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.144 (n=2766)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.183 (n=2355)

  - _Acción_: Kelly boost +0.91€ cuando `libro_spread` < 0.02 (IC base=+0.131)

- **PATRÓN** `libro_liquidez` > `4284.3439` → IC=+0.170 (n=980)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.135 (n=775)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.201 (n=476)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.192 (n=2947)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.174 (n=846)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` < 17.0 (IC base=+0.167)

- **PATRÓN** `py_entrada` < `0.74` → IC=+0.188 (n=858)
...
- **PATRÓN** `py_entrada` < `0.7` → IC=+0.184 (n=299)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.198 (n=355)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` < 7.0 (IC base=+0.191)

- **PATRÓN** `py_entrada` < `0.71` → IC=+0.197 (n=546)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.228 (n=5450)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.196)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.132 (n=986)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.162 (n=501)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 6.0 (IC base=+0.158)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.189 (n=913)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.236 (n=1046)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.234)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.253 (n=1081)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.203 (n=888)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 12.0 (IC base=+0.201)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.242 (n=592)
...
- **PATRÓN** `restante_min` < `3.78` → IC=+0.178 (n=442)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.162 (n=930)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.123 (n=1411)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `ibs_20min` < `0.4013` → IC=+0.129 (n=1778)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` < 0.4013 (IC base=+0.044)

- **PATRÓN** `dist_vwap_pct` < `0.1302` → IC=+0.144 (n=583)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.1302 (IC base=+0.044)

- **PATRÓN** `volumen_regimen` < `0.8548` → IC=+0.134 (n=402)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6573` → IC=+0.203 (n=234)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2459` → IC=+0.145 (n=294)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.2459 (IC base=+0.130)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.138 (n=313)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 8.0 (IC base=+0.130)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.139 (n=339)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` < 18.0 (IC base=+0.130)

- **PATRÓN** `ibs_20min` < `0.5583` → IC=+0.158 (n=334)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` < 0.5583 (IC base=+0.130)

- **PATRÓN** `dist_vwap_pct` < `0.1781` → IC=+0.167 (n=292)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.1781 (IC base=+0.130)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.0771` → IC=+0.226 (n=100)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0771 (IC base=+0.130)

- **PATRÓN** `volumen_spike_ratio` < `1.7866` → IC=+0.167 (n=154)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.7866 (IC base=+0.130)

- **PATRÓN** `volumen_spike_ratio` > `2.217` → IC=+0.154 (n=105)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 2.217 (IC base=+0.130)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `11.873` → IC=+0.188 (n=46)
  ...
  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 11.873 (IC base=+0.002)

### GBM_LATE_15M#SOL#15min
- **FILTRO** `sigma_ewma_delta_pct` > `2.933` → IC=-0.131 (n=299)
...
- **FILTRO** `ibs_20min` > `0.7479` → IC=-0.145 (n=226)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.124` → IC=-0.167 (n=109)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.016 (n=797)
...
- **PATRÓN** `ibs_20min` > `0.9479` → IC=+0.255 (n=679)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9479 (IC base=+0.056)

- **PATRÓN** `dist_vwap_pct` > `0.3429` → IC=+0.251 (n=291)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3429 (IC base=+0.056)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.32` → IC=+0.133 (n=1201)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` > 2.32 (IC base=+0.056)

- **PATRÓN** `volumen_regimen` > `1.1809` → IC=+0.182 (n=209)
...
- **PATRÓN** `volumen_spike_ratio` > `1.6944` → IC=+0.162 (n=781)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.6944 (IC base=+0.056)

- **PATRÓN** `ballena_activa_n` < `89.0` → IC=+0.297 (n=319)
...
- **PATRÓN** `sigma_h` > `0.0098` → IC=+0.373 (n=132)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.277 (n=222)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.191)

- **PATRÓN** `dist_vwap_pct` > `0.2838` → IC=+0.308 (n=175)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2838 (IC base=+0.191)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.12` → IC=+0.265 (n=249)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.12 (IC base=+0.191)

- **PATRÓN** `volumen_regimen` > `0.8361` → IC=+0.230 (n=265)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8361 (IC base=+0.191)

- **PATRÓN** `volumen_pendiente_norm` > `0.1829` → IC=+0.269 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1829 (IC base=+0.191)

- **PATRÓN** `volumen_spike_ratio` < `1.9919` → IC=+0.216 (n=227)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9919 (IC base=+0.191)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.207 (n=421)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.191)

- **PATRÓN** `libro_liquidez` > `2682.2064` → IC=+0.222 (n=264)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2682.2064 (IC base=+0.191)

- **PATRÓN** `ballena_activa_n` < `41.0` → IC=+0.362 (n=121)
...
- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.220 (n=576)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.161 (n=857)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 8.0 (IC base=+0.158)

- **PATRÓN** `ibs_20min` > `0.9107` → IC=+0.260 (n=1149)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9107 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` > `0.9068` → IC=+0.260 (n=336)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9068 (IC base=+0.158)
...
- **PATRÓN** `volumen_regimen` > `0.7174` → IC=+0.177 (n=961)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4324` → IC=+0.148 (n=1304)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 2.4324 (IC base=+0.158)

- **PATRÓN** `volumen_spike_ratio` > `1.4785` → IC=+0.146 (n=1482)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.4785 (IC base=+0.158)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.170 (n=1796)
...
- **PATRÓN** `sigma_h` < `0.007` → IC=+0.208 (n=1514)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.007 (IC base=+0.190)

- **PATRÓN** `drift_60min` |x|≤ `0.3179` → IC=+0.200 (n=1513)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3179 (IC base=+0.190)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.230 (n=709)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.190)

- **PATRÓN** `ibs_20min` < `0.3916` → IC=+0.256 (n=1513)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3916 (IC base=+0.190)

- **PATRÓN** `dist_vwap_pct` < `0.1759` → IC=+0.183 (n=1176)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.328` → IC=+0.193 (n=1521)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` < 5.328 (IC base=+0.190)

- **PATRÓN** `volumen_regimen` < `1.1777` → IC=+0.164 (n=1213)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.1777 (IC base=+0.190)

- **PATRÓN** `volumen_regimen` > `0.6777` → IC=+0.171 (n=1083)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 0.6777 (IC base=+0.190)
...
- **PATRÓN** `sigma_h` < `0.0043` → IC=+0.183 (n=298)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0043 (IC base=+0.160)

- **PATRÓN** `drift_60min` |x|≤ `0.2065` → IC=+0.200 (n=261)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2065 (IC base=+0.160)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.182 (n=278)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 7.0 (IC base=+0.160)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.164 (n=313)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 18.0 (IC base=+0.160)

- **PATRÓN** `ibs_20min` < `0.3397` → IC=+0.216 (n=297)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3397 (IC base=+0.160)

- **PATRÓN** `dist_vwap_pct` < `0.1366` → IC=+0.196 (n=294)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` < 0.1366 (IC base=+0.160)
...
- **PATRÓN** `volumen_spike_ratio` < `1.773` → IC=+0.198 (n=127)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 1.773 (IC base=+0.160)
...
- **PATRÓN** `dist_vwap_pct` > `0.7694` → IC=+0.281 (n=94)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7694 (IC base=+0.104)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.0971` → IC=+0.135 (n=113)
  ...
  - _Acción_: Kelly boost +0.67€ cuando `volumen_pendiente_norm` > 0.0971 (IC base=+0.104)

- **PATRÓN** `libro_liquidez` > `2711.1262` → IC=+0.189 (n=146)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 2711.1262 (IC base=+0.104)

- **PATRÓN** `ballena_activa_n` < `70.0` → IC=+0.220 (n=123)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 70.0 (IC base=+0.104)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.161 (n=175)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 8.0 (IC base=+0.158)
...
- **PATRÓN** `dist_vwap_pct` > `0.3213` → IC=+0.224 (n=197)
...
- **PATRÓN** `volumen_regimen` > `0.8509` → IC=+0.182 (n=237)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` > 0.8509 (IC base=+0.158)
...
- **PATRÓN** `volumen_spike_ratio` < `2.6798` → IC=+0.174 (n=320)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.6798 (IC base=+0.158)

- **PATRÓN** `volumen_spike_ratio` > `1.9317` → IC=+0.161 (n=213)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 1.9317 (IC base=+0.158)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.158 (n=381)

  - _Acción_: Kelly boost +0.79€ cuando `libro_spread` < 0.01 (IC base=+0.158)

- **PATRÓN** `libro_liquidez` > `2452.7664` → IC=+0.159 (n=356)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.149 (n=731)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` > `0.322` → IC=+0.222 (n=596)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.322 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.06` → IC=+0.266 (n=888)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.06 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `0.719` → IC=+0.146 (n=551)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.719 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` > `1.0973` → IC=+0.155 (n=567)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 1.0973 (IC base=+0.140)
...
- **PATRÓN** `volumen_spike_ratio` < `2.8769` → IC=+0.137 (n=1665)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` < 2.8769 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.9582` → IC=+0.149 (n=1109)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.9582 (IC base=+0.140)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.151 (n=2021)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.04 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `2788.952` → IC=+0.187 (n=895)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 2788.952 (IC base=+0.140)

- **PATRÓN** `ballena_activa_n` < `166.0` → IC=+0.218 (n=527)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 166.0 (IC base=+0.140)

- **PATRÓN** `sigma_h` < `0.0078` → IC=+0.211 (n=1795)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.5371` → IC=+0.254 (n=1795)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5371 (IC base=+0.191)

- **PATRÓN** `dist_vwap_pct` < `0.5302` → IC=+0.178 (n=1341)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.533` → IC=+0.193 (n=1684)
...
- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.167 (n=109)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` > 0.0049 (IC base=+0.159)

- **PATRÓN** `drift_60min` |x|≤ `0.3105` → IC=+0.172 (n=327)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.169 (n=333)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 18.0 (IC base=+0.159)

- **PATRÓN** `ibs_20min` < `0.3322` → IC=+0.210 (n=288)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3322 (IC base=+0.159)

- **PATRÓN** `dist_vwap_pct` < `0.2792` → IC=+0.190 (n=311)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.2792 (IC base=+0.159)
...
- **PATRÓN** `volumen_regimen` > `1.2202` → IC=+0.185 (n=109)
  ...
  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` > 1.2202 (IC base=+0.159)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5125` → IC=+0.184 (n=207)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` > 1.5125 (IC base=+0.159)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.161 (n=420)

  - _Acción_: Kelly boost +0.81€ cuando `libro_spread` < 0.01 (IC base=+0.159)

- **PATRÓN** `libro_liquidez` > `12510.8748` → IC=+0.176 (n=109)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 12510.8748 (IC base=+0.159)

- **PATRÓN** `ballena_activa_n` < `329.0` → IC=+0.160 (n=45)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 329.0 (IC base=+0.159)
...
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.238 (n=105)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0078 (IC base=+0.073)

- **PATRÓN** `ibs_20min` > `0.9429` → IC=+0.264 (n=142)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.935` → IC=+0.267 (n=84)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.935 (IC base=+0.073)

- **PATRÓN** `libro_liquidez` > `2638.9696` → IC=+0.229 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2638.9696 (IC base=+0.073)

- **PATRÓN** `ballena_activa_n` < `63.0` → IC=+0.269 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 63.0 (IC base=+0.073)
...
- **PATRÓN** `dist_vwap_pct` > `0.2184` → IC=+0.130 (n=90)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` > 0.2184 (IC base=+0.046)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.41` → IC=+0.167 (n=16)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 3.41 (IC base=+0.050)
...
- **FILTRO** `py_entrada` < `0.45` → IC=-0.169 (n=373)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.068 (n=1142)
...
- **FILTRO** `ibs_20min` > `0.2762` → IC=-0.234 (n=381)
...
- **PATRÓN** `py_entrada` > `0.505` → IC=+0.129 (n=87)

  - _Acción_: Kelly boost +0.65€ cuando `py_entrada` > 0.505 (IC base=-0.002)

- **PATRÓN** `libro_liquidez` > `1523.5048` → IC=+0.128 (n=76)

  - _Acción_: Kelly boost +0.64€ cuando `libro_liquidez` > 1523.5048 (IC base=-0.002)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.178 (n=57)
  ...
  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` > 1.0 (IC base=-0.018)
...
- **FILTRO** `drift_20min_pct` |x|> `0.3804` → IC=-0.191 (n=79)
...
- **PATRÓN** `py_entrada` > `0.505` → IC=+0.129 (n=122)

  - _Acción_: Kelly boost +0.65€ cuando `py_entrada` > 0.505 (IC base=+0.032)

- **PATRÓN** `libro_liquidez` > `2476.5327` → IC=+0.137 (n=89)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 2476.5327 (IC base=+0.032)
...
- **PATRÓN** `libro_liquidez` > `9409.0356` → IC=+0.167 (n=46)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 9409.0356 (IC base=+0.051)

### MOMENTUM_IBS_5M_BALLENA
- **FILTRO** `py_entrada` < `0.35` → IC=-0.279 (n=899)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=2852)

- **FILTRO** `ibs_7min` < `0.7309` → IC=-0.223 (n=936)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7309
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=2815)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.167 (n=1257)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=2494)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.222 (n=1111)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=3341)

- **FILTRO** `ibs_7min` > `0.7198` → IC=-0.161 (n=1112)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7198
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=3340)
...
- **FILTRO** `ibs_7min` > `0.8333` → IC=-0.191 (n=173)
...
- **FILTRO** `py_entrada` > `0.53` → IC=-0.198 (n=187)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.266 (n=173)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.006 (n=599)

- **FILTRO** `ibs_7min` > `0.84` → IC=-0.180 (n=192)

  - _Acción_: SKIP cuando `ibs_7min` > 0.84
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=580)
...
- **FILTRO** `ibs_7min` < `0.7815` → IC=-0.173 (n=166)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.256 (n=154)
...
- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.204 (n=160)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=496)

- **FILTRO** `libro_liquidez` < `8458.6386` → IC=-0.136 (n=432)

  - _Acción_: SKIP cuando `libro_liquidez` < 8458.6386
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.754` → IC=+0.179 (n=51)
  ...
  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 10.754 (IC base=+0.043)
...
- **FILTRO** `sigma_h` > `0.009` → IC=-0.184 (n=448)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.009
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=1346)

- **FILTRO** `drift_15min` |x|> `0.7984` → IC=-0.131 (n=448)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 0.7984
...
- **FILTRO** `sigma_h` > `0.0077` → IC=-0.221 (n=41)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.373` → IC=+0.180 (n=23)
  ...
  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 10.373 (IC base=-0.047)
...
- **FILTRO** `sigma_ewma_delta_pct` > `14.673` → IC=-0.236 (n=70)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=512)
...
| ✅ BALLENAS_TARDIAS | 4920 | -0.104 | -544.01€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 567 | -0.082 | -104.16€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 4353 | -0.106 | -439.85€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 1465 | -0.010 | +37.93€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 1465 | -0.010 | +37.93€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1196 | -0.193 | -203.73€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1196 | -0.193 | -203.73€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 21146 | +0.112 | -1507.94€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 4339 | +0.184 | -167.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 106 | -0.102 | -48.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 13871 | +0.088 | -1280.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2830 | +0.126 | -11.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 2336 | +0.051 | -456.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 17 | -0.067 | -0.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 2314 | +0.053 | -450.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 4547 | +0.134 | -101.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1201 | +0.191 | -81.92€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 2307 | +0.107 | -65.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 997 | +0.139 | +68.74€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 2340 | +0.065 | -373.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 16 | +0.044 | -1.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 2323 | +0.065 | -370.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 4902 | +0.127 | -75.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1595 | +0.169 | -4.59€ | 0 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 