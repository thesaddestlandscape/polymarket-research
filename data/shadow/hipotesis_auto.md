# Hipótesis automáticas — 2026-07-15 20:00 UTC
_Generado por shadow_postmortem.py sobre 15842 resoluciones (PNL=+3208.10€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.207 (n=271)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.262 (n=61)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.223)

- **PATRÓN** `py_entrada` > `0.625` → IC=+0.280 (n=157)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.625 (IC base=+0.223)

- **PATRÓN** `libro_liquidez` > `1656.2958` → IC=+0.223 (n=175)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1656.2958 (IC base=+0.223)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.120 (n=1483)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` > 8.0 (IC base=+0.109)

- **PATRÓN** `dist_vwap_pct` > `0.6506` → IC=+0.134 (n=121)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` > 0.6506 (IC base=+0.109)
...
- **PATRÓN** `sigma_h` > `0.0231` → IC=+0.194 (n=194)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0231 (IC base=+0.148)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.160 (n=151)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.8049` → IC=+0.250 (n=30)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8049 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.281` → IC=+0.180 (n=23)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 9.281 (IC base=+0.148)
...
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.224 (n=310)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0108 (IC base=+0.163)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.179 (n=494)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.142 (n=65)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` > 18.0 (IC base=+0.136)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.138 (n=172)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` < 16.0 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.3907` → IC=+0.192 (n=50)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.3907 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `1.0657` → IC=+0.147 (n=137)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 1.0657 (IC base=+0.136)
...
- **PATRÓN** `sigma_h` > `0.0101` → IC=+0.262 (n=145)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0101 (IC base=+0.259)
...
- **PATRÓN** `dist_vwap_pct` > `0.917` → IC=+0.364 (n=20)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.917 (IC base=+0.259)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.136` → IC=+0.331 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.136 (IC base=+0.259)
...
- **PATRÓN** `sigma_h` > `0.0148` → IC=+0.182 (n=209)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` > 0.0148 (IC base=+0.112)
...
- **PATRÓN** `dist_vwap_pct` > `0.7123` → IC=+0.200 (n=108)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7123 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.861` → IC=+0.312 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.861 (IC base=+0.112)
...
- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.224 (n=179)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.1836` → IC=+0.254 (n=59)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1836 (IC base=+0.204)

- **PATRÓN** `dist_vwap_pct` < `1.3485` → IC=+0.205 (n=103)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 1.3485 (IC base=+0.204)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.513` → IC=+0.219 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.513 (IC base=+0.204)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2270 | +0.176 | -31.85€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1465 | +0.209 | +13.71€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 798 | +0.170 | -29.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 484 | +0.210 | +11.79€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 17 | +0.246 | +0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 17 | +0.246 | +0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 17 | +0.246 | +0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 17 | +0.246 | +0.79€ | 0 | 0 |
| ✅ GBM_LATE_15M | 4674 | +0.103 | +1445.74€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 4674 | +0.103 | +1445.74€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#SOL | 1244 | +0.110 | +498.98€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1244 | +0.110 | +498.98€ | 0 | 6 |
| ✅ GBM_LATE_15M#XRP | 1174 | +0.157 | +592.19€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1174 | +0.157 | +592.19€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1922 | +0.152 | +1125.58€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1922 | +0.152 | +1125.58€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 518 | +0.110 | +223.37€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 518 | +0.110 | +223.37€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 403 | +0.275 | +421.04€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 403 | +0.275 | +421.04€ | 0 | 11 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 45 | +0.074 | +10.88€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 45 | +0.074 | +10.88€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 20 | +0.364 | +13.82€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 20 | +0.364 | +13.82€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 1877 | +0.089 | +559.33€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1877 | +0.089 | +559.33€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#SOL | 450 | +0.051 | +77.46€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 450 | +0.051 | +77.46€ | 3 | 5 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 533 | +0.199 | +377.20€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 533 | +0.199 | +377.20€ | 0 | 12 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 47 | +0.255 | +17.84€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 47 | +0.255 | +17.84€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 19 | +0.158 | +7.17€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 19 | +0.158 | +7.17€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.133) — sin ventaja clara. oversold(IBS<0.3): IC=+0.000 n=450 | neutral: IC=-0.015 n=344 | overbought(IBS>0.7): IC=+0.118 n=365
  - _Datos_: n=1224 IC=+0.037 PNL=+113.51€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9184 | tras_1loss IC=+0.064 n=6449 | tras_2loss IC=+0.032 n=2779/40 | gap=+0.070 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=13384 IC=+0.104 PNL=+3283.98€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=13384 IC=+0.104 PNL=+3283.98€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.260 > 0.2 con n=323 PNL=+228.46€
  - _Datos_: n=323 IC=+0.260 PNL=+228.46€
...
**🔶 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.148 n=177) pero sin cruzar ≥2 pares más — BTC: n=129 IC=-0.004; ETH: n=100 IC=+0.000; SOL: n=181 IC=+0.063 ✓
  - _Datos_: n=177 IC=+0.148 PNL=+99.90€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.251 > 0.08 con n=175 PNL=-6.69€
  - _Datos_: n=175 IC=+0.251 PNL=-6.69€
