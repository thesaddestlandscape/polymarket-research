# Hipótesis automáticas — 2026-07-18 20:33 UTC
_Generado por shadow_postmortem.py sobre 21228 resoluciones (PNL=+5060.07€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.191 (n=1185)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.194 (n=1483)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.234 (n=261)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.224 (n=357)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.127 (n=1779)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 8.0 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` > `0.7393` → IC=+0.137 (n=133)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 0.7393 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.033` → IC=+0.216 (n=160)
...
- **PATRÓN** `sigma_h` < `0.0045` → IC=+0.172 (n=135)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0045 (IC base=+0.095)

- **PATRÓN** `dist_vwap_pct` > `0.8499` → IC=+0.132 (n=17)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.8499 (IC base=+0.095)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.301` → IC=+0.204 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.301 (IC base=+0.095)
...
- **PATRÓN** `sigma_h` < `0.0101` → IC=+0.123 (n=250)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` < 0.0101 (IC base=+0.104)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.148 (n=194)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.161 (n=482)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5822` → IC=+0.250 (n=42)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.547` → IC=+0.269 (n=50)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.183 (n=743)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5919` → IC=+0.220 (n=173)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5919 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.023` → IC=+0.295 (n=149)
...
- **PATRÓN** `dist_vwap_pct` > `0.5316` → IC=+0.189 (n=59)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.5316 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` < `0.8807` → IC=+0.144 (n=209)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.8807 (IC base=+0.147)
...
- **PATRÓN** `sigma_h` < `0.0173` → IC=+0.265 (n=236)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0173 (IC base=+0.255)

- **PATRÓN** `sigma_h` > `0.0092` → IC=+0.267 (n=234)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0092 (IC base=+0.255)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.259 (n=222)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.2844` → IC=+0.316 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2844 (IC base=+0.255)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.797` → IC=+0.389 (n=61)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.135 (n=862)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5282` → IC=+0.184 (n=172)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.5282 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` < `0.1183` → IC=+0.154 (n=478)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.1183 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.366` → IC=+0.287 (n=125)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.171 (n=80)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0039 (IC base=+0.078)

- **PATRÓN** `dist_vwap_pct` < `0.2773` → IC=+0.135 (n=124)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.2773 (IC base=+0.078)

- **PATRÓN** `sigma_ewma_delta_pct` > `14.068` → IC=+0.250 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 14.068 (IC base=+0.078)
...
- **PATRÓN** `sigma_h` > `0.0124` → IC=+0.122 (n=88)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` > 0.0124 (IC base=+0.084)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.148 (n=86)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 8.0 (IC base=+0.084)

- **PATRÓN** `dist_vwap_pct` > `0.3389` → IC=+0.134 (n=39)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` > 0.3389 (IC base=+0.084)

- **PATRÓN** `dist_vwap_pct` < `0.1438` → IC=+0.136 (n=86)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.1438 (IC base=+0.084)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.573` → IC=+0.273 (n=73)
...
- **PATRÓN** `sigma_h` < `0.0287` → IC=+0.210 (n=295)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `dist_vwap_pct` > `0.2409` → IC=+0.267 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2409 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.899` → IC=+0.279 (n=66)
...
- **PATRÓN** `ibs_15` > `0.7353` → IC=+0.188 (n=296)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.94€ cuando `ibs_15` > 0.7353 (IC base=+0.032)

- **PATRÓN** `dist_vwap_pct` > `0.133` → IC=+0.150 (n=135)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.133 (IC base=+0.032)

- **PATRÓN** `dist_vwap_pct` < `0.2752` → IC=+0.143 (n=169)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.2752 (IC base=+0.032)

- **PATRÓN** `sigma_ewma_delta_pct` > `18.602` → IC=+0.150 (n=38)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 18.602 (IC base=+0.032)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS > 0.7353 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.188 n=296). Confirma señal de reversión media → alinear con BUY_NO.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 221 | +0.137 | +10.68€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 221 | +0.137 | +10.68€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 96 | +0.153 | +4.20€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 96 | +0.153 | +4.20€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 78 | +0.163 | +6.13€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 78 | +0.163 | +6.13€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 47 | +0.051 | +0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 47 | +0.051 | +0.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 3527 | +0.183 | +24.19€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 2293 | +0.216 | +76.38€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 1255 | +0.175 | +1.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 760 | +0.218 | +35.31€ | 0 | 9 |
...
| ✅ GBM_LATE_15M | 5619 | +0.105 | +1862.84€ | 0 | 8 |
| ✅ GBM_LATE_15M#15min | 5619 | +0.105 | +1862.84€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1343 | +0.075 | +235.63€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1343 | +0.075 | +235.63€ | 0 | 5 |
| ✅ GBM_LATE_15M#ETH | 1297 | +0.077 | +243.26€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1297 | +0.077 | +243.26€ | 0 | 5 |
| ✅ GBM_LATE_15M#SOL | 1517 | +0.101 | +594.97€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1517 | +0.101 | +594.97€ | 1 | 7 |
| ✅ GBM_LATE_15M#XRP | 1462 | +0.161 | +788.98€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1462 | +0.161 | +788.98€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2846 | +0.151 | +1678.21€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2846 | +0.151 | +1678.21€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 726 | +0.115 | +340.37€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 726 | +0.115 | +340.37€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 748 | +0.116 | +351.80€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 748 | +0.116 | +351.80€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 737 | +0.110 | +316.17€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 737 | +0.110 | +316.17€ | 1 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 635 | +0.277 | +669.85€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 635 | +0.277 | +669.85€ | 0 | 12 |
...
| ✅ GBM_LATE_15M_TARDIO | 2788 | +0.101 | +979.51€ | 0 | 10 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2788 | +0.101 | +979.51€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 676 | +0.069 | +139.81€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 676 | +0.069 | +139.81€ | 0 | 7 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 671 | +0.038 | +66.29€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 671 | +0.038 | +66.29€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 621 | +0.057 | +133.04€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 621 | +0.057 | +133.04€ | 2 | 7 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 820 | +0.213 | +640.37€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 820 | +0.213 | +640.37€ | 0 | 10 |
...
| ✅ UPDOWN_GBM | 1780 | +0.020 | +129.54€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1520 | +0.046 | +167.60€ | 1 | 7 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 500 | +0.181 | +215.58€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 500 | +0.181 | +215.58€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 125 | +0.209 | +49.48€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 125 | +0.209 | +49.48€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 130 | +0.129 | +45.44€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 130 | +0.129 | +45.44€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.177: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.051 n=689 | neutral: IC=-0.007 n=505 | overbought(IBS>0.7): IC=+0.170 n=626
  - _Datos_: n=1947 IC=+0.081 PNL=+379.85€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.116 n=12717 | tras_1loss IC=+0.085 n=8315 | tras_2loss IC=+0.052 n=3417/40 | gap=+0.064 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1651 IC=+0.028 PNL=+146.93€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1651 IC=+0.028 PNL=+146.93€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=767 IC=+0.032 PNL=+59.00€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=767 IC=+0.032 PNL=+59.00€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=801 IC=+0.079 PNL=+117.00€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=801 IC=+0.079 PNL=+117.00€
...
**🟡 H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.202 > 0.08 con n=45 PNL=+23.16€
  - _Datos_: n=45 IC=+0.202 PNL=+23.16€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=18770 IC=+0.119 PNL=+5135.95€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=18770 IC=+0.119 PNL=+5135.95€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=917 IC=+0.002 PNL=+28.45€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=917 IC=+0.002 PNL=+28.45€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.160 > 0.1 con n=501 PNL=+171.01€
  - _Datos_: n=501 IC=+0.160 PNL=+171.01€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.136 > 0.08 con n=363 PNL=+105.24€
  - _Datos_: n=363 IC=+0.136 PNL=+105.24€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.228 > 0.2 con n=465 PNL=+290.95€
  - _Datos_: n=465 IC=+0.228 PNL=+290.95€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.129 > 0.08 con n=257 PNL=+93.80€
  - _Datos_: n=257 IC=+0.129 PNL=+93.80€
