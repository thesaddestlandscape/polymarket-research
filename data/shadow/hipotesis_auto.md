# Hipótesis automáticas — 2026-08-18 09:24 UTC
_Generado por shadow_postmortem.py sobre 61278 resoluciones (PNL=+6420.39€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `144.37` → IC=-0.266 (n=804)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 144.37
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=2413)
...
- **FILTRO** `restante_s_al_confirmar` > `639.7` → IC=-0.279 (n=111)
...
- **FILTRO** `restante_s_al_confirmar` < `244.14` → IC=-0.159 (n=550)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.206 (n=2122)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.303 (n=257)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `3301.4437` → IC=+0.347 (n=83)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3301.4437 (IC base=+0.285)
...
- **PATRÓN** `py_entrada` < `0.315` → IC=+0.338 (n=227)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.163 (n=653)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.231 (n=2897)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.226 (n=684)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.157 (n=1481)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 5.0 (IC base=+0.151)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.159 (n=1494)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.164 (n=738)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 5.0 (IC base=+0.156)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.168 (n=658)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.153 (n=696)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 6.0 (IC base=+0.147)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.157 (n=754)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `ibs_20min` < `0.6594` → IC=+0.123 (n=2083)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7094` → IC=+0.233 (n=155)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7094 (IC base=+0.079)

- **PATRÓN** `volumen_spike_ratio` > `2.934` → IC=+0.233 (n=159)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.934 (IC base=+0.079)

- **PATRÓN** `ballena_activa_n` < `254.0` → IC=+0.266 (n=75)
...
- **PATRÓN** `sigma_h` > `0.0024` → IC=+0.151 (n=150)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0024 (IC base=+0.143)

- **PATRÓN** `drift_60min` |x|≤ `0.1825` → IC=+0.160 (n=195)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.1825 (IC base=+0.143)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.190 (n=156)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 12.0 (IC base=+0.143)

- **PATRÓN** `ibs_20min` < `0.4755` → IC=+0.210 (n=222)
...
- **PATRÓN** `volumen_regimen` < `1.2895` → IC=+0.156 (n=222)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.2895 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` > `0.8413` → IC=+0.167 (n=148)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 0.8413 (IC base=+0.143)
...
- **PATRÓN** `libro_liquidez` > `5315.7563` → IC=+0.145 (n=198)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 5315.7563 (IC base=+0.143)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0858` → IC=+0.308 (n=76)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0858 (IC base=+0.281)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.290 (n=231)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.281)

- **PATRÓN** `ibs_20min` < `0.5` → IC=+0.307 (n=226)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5 (IC base=+0.281)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.4178` → IC=+0.405 (n=19)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.4178 (IC base=+0.281)

- **PATRÓN** `volumen_spike_ratio` < `5.2303` → IC=+0.252 (n=103)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 5.2303 (IC base=+0.281)

- **PATRÓN** `volumen_spike_ratio` > `3.1342` → IC=+0.246 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.1342 (IC base=+0.281)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.286 (n=264)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.97` → IC=+0.125 (n=94)
  ...
  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 4.97 (IC base=-0.003)
...
- **PATRÓN** `dist_vwap_pct` > `0.3235` → IC=+0.187 (n=65)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.3235 (IC base=+0.057)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8797` → IC=+0.177 (n=283)
  ...
  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 2.8797 (IC base=+0.057)
...
- **FILTRO** `libro_liquidez` < `8587.8262` → IC=-0.202 (n=55)
...
- **PATRÓN** `ibs_20min` > `0.4444` → IC=+0.136 (n=138)
  ...
  - _Acción_: Kelly boost +0.68€ cuando `ibs_20min` > 0.4444 (IC base=+0.000)

- **PATRÓN** `dist_vwap_pct` > `0.1723` → IC=+0.167 (n=16)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.1723 (IC base=+0.000)
...
- **PATRÓN** `sigma_h` > `0.0038` → IC=+0.234 (n=1275)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0038 (IC base=+0.227)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.238 (n=1194)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.227)

- **PATRÓN** `ibs_20min` < `0.4851` → IC=+0.289 (n=1275)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4851 (IC base=+0.227)

- **PATRÓN** `dist_vwap_pct` < `0.151` → IC=+0.203 (n=866)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.151 (IC base=+0.227)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.11` → IC=+0.261 (n=253)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.11 (IC base=+0.227)

- **PATRÓN** `volumen_regimen` < `0.6178` → IC=+0.214 (n=285)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6178 (IC base=+0.227)

- **PATRÓN** `volumen_regimen` > `1.0773` → IC=+0.228 (n=387)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0773 (IC base=+0.227)

- **PATRÓN** `volumen_pendiente_norm` > `0.248` → IC=+0.297 (n=146)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.248 (IC base=+0.227)

- **PATRÓN** `volumen_spike_ratio` < `1.6449` → IC=+0.285 (n=259)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6449 (IC base=+0.227)
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.283 (n=81)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0076 (IC base=+0.281)
...
- **PATRÓN** `ibs_20min` < `0.5337` → IC=+0.337 (n=243)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5337 (IC base=+0.281)
...
- **PATRÓN** `volumen_spike_ratio` < `3.6802` → IC=+0.271 (n=107)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.6802 (IC base=+0.281)
...
- **PATRÓN** `volumen_regimen` > `0.8842` → IC=+0.306 (n=215)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8842 (IC base=+0.262)

- **PATRÓN** `volumen_pendiente_norm` > `0.2728` → IC=+0.392 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2728 (IC base=+0.262)
...
- **PATRÓN** `libro_liquidez` > `2743.0687` → IC=+0.264 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2743.0687 (IC base=+0.262)
...
- **PATRÓN** `drift_20min_pct` |x|≤ `0.0526` → IC=+0.207 (n=56)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `drift_20min_pct` |x|≤ 0.0526 (IC base=+0.091)
...
- **PATRÓN** `drift_20min_pct` |x|≤ `0.0569` → IC=+0.265 (n=15)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `drift_20min_pct` |x|≤ 0.0569 (IC base=+0.217)
...
- **FILTRO** `py_entrada` < `0.49` → IC=-0.204 (n=79)
...
- **PATRÓN** `py_entrada` > `0.5` → IC=+0.143 (n=26)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `py_entrada` > 0.5 (IC base=+0.022)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.186 (n=33)
...
- **FILTRO** `hora_utc` < `4.0` → IC=-0.187 (n=148)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=524)
...
- **FILTRO** `ibs_7min` < `0.7655` → IC=-0.241 (n=168)
...
- **FILTRO** `libro_liquidez` < `1724.093` → IC=-0.189 (n=336)
...
- **FILTRO** `py_entrada` < `0.41` → IC=-0.210 (n=36)
...
- **FILTRO** `drift_7min_pct` |x|> `0.0854` → IC=-0.180 (n=23)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.0854
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=70)

- **FILTRO** `ballena_activa_n` > `69.0` → IC=-0.220 (n=23)
...
- **FILTRO** `ibs_7min` < `1.0` → IC=-0.200 (n=58)

  - _Acción_: SKIP cuando `ibs_7min` < 1.0
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=59)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.242 (n=29)
...
- **PATRÓN** `libro_liquidez` > `4172.3896` → IC=+0.190 (n=27)
  ...
  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 4172.3896 (IC base=+0.009)
...
- **PATRÓN** `py_entrada` < `0.51` → IC=+0.150 (n=58)
  ...
  - _Acción_: Kelly boost +0.75€ cuando `py_entrada` < 0.51 (IC base=-0.043)

- **PATRÓN** `libro_liquidez` > `3490.026` → IC=+0.183 (n=39)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 3490.026 (IC base=-0.043)
...
| ✅ BALLENAS_TARDIAS | 3205 | -0.116 | -492.42€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 445 | -0.010 | -5.42€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 2760 | -0.133 | -487.00€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 353 | -0.199 | -105.65€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 353 | -0.199 | -105.65€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 445 | -0.010 | -5.42€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 445 | -0.010 | -5.42€ | 1 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 759 | -0.015 | -91.38€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 759 | -0.015 | -91.38€ | 1 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP | 623 | -0.220 | -105.10€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP#5min | 623 | -0.220 | -105.10€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 13630 | +0.116 | -783.22€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3285 | +0.179 | -119.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 98 | -0.100 | -46.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 7674 | +0.087 | -632.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2573 | +0.131 | +15.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 1297 | +0.032 | -286.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 13 | -0.065 | -2.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 1280 | +0.035 | -278.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 3134 | +0.140 | -3.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 895 | +0.192 | -40.13€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 41 | -0.105 | -20.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 1280 | +0.112 | -6.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 918 | +0.139 | +63.96€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 1293 | +0.055 | -224.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 9 | -0.021 | -3.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 1283 | +0.056 | -218.91€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 3386 | +0.126 | -48.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1212 | +0.159 | -27.82€ | 0 | 6 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 1275 | +0.106 | -11.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 887 | +0.115 | -0.23€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 3229 | +0.137 | -193.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1144 | +0.196 | -46.97€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 39 | +0.012 | -7.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 1278 | +0.086 | -89.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 768 | +0.138 | -48.49€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 1291 | +0.127 | -27.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 12 | +0.043 | +1.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 1278 | +0.127 | -27.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 3356 | +0.163 | -308.06€ | 2 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 3356 | +0.163 | -308.06€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 835 | +0.162 | -101.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 835 | +0.162 | -101.14€ | 0 | 4 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 5466 | +0.194 | -473.91€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 5466 | +0.194 | -473.91€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 845 | +0.254 | -0.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 845 | +0.254 | -0.15€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 966 | +0.152 | -141.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 966 | +0.152 | -141.47€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 885 | +0.226 | -31.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 885 | +0.226 | -31.76€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 845 | +0.251 | -4.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 845 | +0.251 | -4.78€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 896 | +0.203 | -68.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 896 | +0.203 | -68.69€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 1940 | +0.151 | +112.24€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 1940 | +0.151 | +112.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 963 | +0.156 | +64.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 963 | +0.156 | +64.71€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 977 | +0.147 | +47.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 977 | +0.147 | +47.53€ | 0 | 6 |
...
| ✅ GBM_LATE_15M | 4360 | +0.090 | +1560.09€ | 0 | 15 |
| ✅ GBM_LATE_15M#15min | 4360 | +0.090 | +1560.09€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 755 | +0.176 | +475.39€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 755 | +0.176 | +475.39€ | 0 | 18 |
| ✅ GBM_LATE_15M#BTC |