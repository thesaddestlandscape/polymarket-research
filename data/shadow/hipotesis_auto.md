# Hipótesis automáticas — 2026-07-03 11:56 UTC
_Generado por shadow_postmortem.py sobre 3046 resoluciones (PNL=-44.89€)_
...
- **PATRÓN** `sigma_h` < `0.0098` → IC=+0.212 (n=78)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0098 (IC base=+0.036)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.122 (n=43)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 15.0 (IC base=+0.036)

- **PATRÓN** `ibs_15` < `0.0563` → IC=+0.155 (n=56)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.78€ cuando `ibs_15` < 0.0563 (IC base=+0.036)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1297` → IC=+0.150 (n=58)
  ...
  - _Acción_: Kelly boost +0.75€ cuando `pct_spot_vs_ref` |x|≤ 0.1297 (IC base=+0.108)

- **PATRÓN** `sigma_h` < `0.0089` → IC=+0.183 (n=39)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0089 (IC base=+0.108)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.156 (n=30)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 11.0 (IC base=+0.108)

- **PATRÓN** `ibs_15` < `0.0758` → IC=+0.250 (n=30)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0758 (IC base=+0.108)
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.0758 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.250 n=30). Confirma señal de reversión media → alinear con BUY_YES.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 392 | +0.053 | +51.59€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 392 | +0.053 | +51.59€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 103 | +0.024 | +5.08€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 103 | +0.024 | +5.08€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 101 | -0.034 | -2.16€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 101 | -0.034 | -2.16€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL | 97 | +0.116 | +25.04€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 97 | +0.116 | +25.04€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP | 91 | +0.113 | +23.63€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 91 | +0.113 | +23.63€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 857 | -0.041 | -51.68€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 627 | -0.020 | -23.83€ | 1 | 3 |
...
| ✅ UPDOWN_GBM#ETH | 336 | -0.012 | +6.50€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 252 | +0.004 | +12.96€ | 3 | 4 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.061) — sin ventaja clara. oversold(IBS<0.3): IC=-0.009 n=161 | neutral: IC=-0.070 n=140 | overbought(IBS>0.7): IC=-0.046 n=150
  - _Datos_: n=467 IC=-0.042 PNL=-26.44€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.008 n=1460 | tras_1loss IC=-0.028 n=1495 | tras_2loss IC=-0.045 n=770/40 | gap=+0.053 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=734 IC=-0.037 PNL=-38.12€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=734 IC=-0.037 PNL=-38.12€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=269 IC=-0.020 PNL=-36.27€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=269 IC=-0.020 PNL=-36.27€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=217 IC=+0.011 PNL=+11.47€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=217 IC=+0.011 PNL=+11.47€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=620 IC=+0.034 PNL=+45.28€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=620 IC=+0.034 PNL=+45.28€
