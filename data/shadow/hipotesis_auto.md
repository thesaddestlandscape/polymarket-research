# Hipótesis automáticas — 2026-08-25 13:40 UTC
_Generado por shadow_postmortem.py sobre 151104 resoluciones (PNL=+10029.37€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `148.86` → IC=-0.277 (n=2118)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 148.86
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=6354)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.222 (n=7575)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.241 (n=1700)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `restante_min` > `4.92` → IC=+0.159 (n=1375)
...
- **PATRÓN** `lag_apertura_s` < `4.58` → IC=+0.158 (n=1291)
...
- **PATRÓN** `restante_min` > `4.9` → IC=+0.172 (n=662)
...
- **PATRÓN** `lag_apertura_s` < `5.92` → IC=+0.172 (n=642)
...
- **PATRÓN** `restante_min` > `4.94` → IC=+0.157 (n=695)

  - _Acción_: Kelly boost +0.79€ cuando `restante_min` > 4.94 (IC base=+0.123)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.124 (n=2030)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `3.48` → IC=+0.158 (n=649)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.065` → IC=+0.186 (n=813)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 5.065 (IC base=+0.068)
...
- **PATRÓN** `ibs_20min` < `0.2129` → IC=+0.124 (n=1367)
...
- **PATRÓN** `hora_utc` < `16.0` → IC=+0.297 (n=215)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 16.0 (IC base=+0.288)

- **PATRÓN** `ibs_20min` < `0.4739` → IC=+0.335 (n=186)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4739 (IC base=+0.288)
...
- **PATRÓN** `sigma_h` > `0.0073` → IC=+0.133 (n=1134)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0073 (IC base=+0.053)

- **PATRÓN** `ibs_20min` > `0.9465` → IC=+0.252 (n=834)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9465 (IC base=+0.053)

- **PATRÓN** `dist_vwap_pct` > `0.4517` → IC=+0.262 (n=393)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4517 (IC base=+0.053)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.331` → IC=+0.122 (n=1417)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` > 2.331 (IC base=+0.053)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1151` → IC=+0.162 (n=1094)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` < 0.1151 (IC base=+0.053)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8774` → IC=+0.176 (n=375)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.8774 (IC base=+0.053)

- **PATRÓN** `ballena_activa_n` < `104.0` → IC=+0.270 (n=571)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 104.0 (IC base=+0.053)

- **PATRÓN** `ibs_20min` < `0.0968` → IC=+0.173 (n=1094)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.0968 (IC base=+0.035)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.413` → IC=+0.169 (n=134)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 3.413 (IC base=-0.018)
...
- **PATRÓN** `ibs_20min` < `0.2398` → IC=+0.386 (n=156)
...
- **PATRÓN** `sigma_h` > `0.0066` → IC=+0.184 (n=1438)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0066 (IC base=+0.158)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.164 (n=2245)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 5.0 (IC base=+0.158)

- **PATRÓN** `ibs_20min` > `0.8947` → IC=+0.261 (n=1439)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8947 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` > `0.7812` → IC=+0.250 (n=546)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7812 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.22` → IC=+0.261 (n=929)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.22 (IC base=+0.158)

- **PATRÓN** `volumen_regimen` < `1.238` → IC=+0.165 (n=1501)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.238 (IC base=+0.158)

- **PATRÓN** `volumen_regimen` > `0.694` → IC=+0.173 (n=1340)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` > 0.694 (IC base=+0.158)

- **PATRÓN** `volumen_pendiente_norm` > `0.1037` → IC=+0.190 (n=765)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.1037 (IC base=+0.158)

- **PATRÓN** `volumen_spike_ratio` < `2.3042` → IC=+0.159 (n=1679)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.3042 (IC base=+0.158)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.169 (n=1758)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.01 (IC base=+0.158)

- **PATRÓN** `libro_liquidez` > `3716.9481` → IC=+0.199 (n=718)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 3716.9481 (IC base=+0.158)
...
- **PATRÓN** `sigma_h` < `0.0077` → IC=+0.239 (n=262)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0077 (IC base=+0.188)

- **PATRÓN** `drift_60min` |x|≤ `0.1373` → IC=+0.222 (n=131)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1373 (IC base=+0.188)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.212 (n=300)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `1.0628` → IC=+0.244 (n=80)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0628 (IC base=+0.188)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.788` → IC=+0.341 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.788 (IC base=+0.188)

- **PATRÓN** `volumen_regimen` > `0.7015` → IC=+0.216 (n=266)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7015 (IC base=+0.188)

- **PATRÓN** `volumen_pendiente_norm` > `0.1005` → IC=+0.269 (n=132)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4523` → IC=+0.204 (n=96)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4523 (IC base=+0.188)

- **PATRÓN** `volumen_spike_ratio` > `2.4658` → IC=+0.225 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4658 (IC base=+0.188)
...
- **PATRÓN** `libro_liquidez` > `9653.043` → IC=+0.208 (n=135)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 9653.043 (IC base=+0.188)

- **PATRÓN** `ballena_activa_n` < `126.0` → IC=+0.198 (n=104)

  - _Acción_: Kelly boost +0.99€ cuando `ballena_activa_n` < 126.0 (IC base=+0.188)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.200 (n=1135)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0076 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.166 (n=1242)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 15.0 (IC base=+0.142)

- **PATRÓN** `ibs_20min` > `0.9445` → IC=+0.288 (n=1132)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9445 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `1.0378` → IC=+0.256 (n=448)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0378 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.479` → IC=+0.231 (n=1505)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.479 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `0.706` → IC=+0.153 (n=776)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.706 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` > `1.0855` → IC=+0.150 (n=799)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 1.0855 (IC base=+0.142)

- **PATRÓN** `volumen_pendiente_norm` > `0.1654` → IC=+0.184 (n=628)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.1654 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` > `1.8994` → IC=+0.153 (n=1454)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.8994 (IC base=+0.142)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.152 (n=2025)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.01 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `3085.6316` → IC=+0.190 (n=1132)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 3085.6316 (IC base=+0.142)

- **PATRÓN** `ballena_activa_n` < `75.0` → IC=+0.184 (n=751)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 75.0 (IC base=+0.142)
...
- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.197 (n=130)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0069 (IC base=+0.162)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.185 (n=389)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 6.0 (IC base=+0.162)

- **PATRÓN** `ibs_20min` > `0.3404` → IC=+0.216 (n=389)
...
- **PATRÓN** `volumen_regimen` > `1.1006` → IC=+0.193 (n=177)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` > 1.1006 (IC base=+0.162)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5432` → IC=+0.196 (n=340)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.5432 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` > `1.4727` → IC=+0.199 (n=304)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4727 (IC base=+0.162)

- **PATRÓN** `libro_liquidez` > `11364.2884` → IC=+0.201 (n=259)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.167 (n=349)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 8.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` > `0.3491` → IC=+0.192 (n=388)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` > 0.3491 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.9062` → IC=+0.214 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9062 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.539` → IC=+0.196 (n=189)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 4.539 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `0.6307` → IC=+0.189 (n=130)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_regimen` < 0.6307 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` > `1.2094` → IC=+0.159 (n=130)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 1.2094 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.2701` → IC=+0.318 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2701 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.4212` → IC=+0.202 (n=354)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4212 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `3071.3952` → IC=+0.188 (n=347)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3071.3952 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `170.0` → IC=+0.182 (n=174)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 170.0 (IC base=+0.136)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.158 (n=706)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3361` → IC=+0.180 (n=476)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.90€ cuando `drift_60min` |x|≤ 0.3361 (IC base=+0.150)
...
- **PATRÓN** `ibs_20min` < `0.6264` → IC=+0.163 (n=476)

  - _Acción_: Kelly boost +0.82€ cuando `ibs_20min` < 0.6264 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.7292` → IC=+0.152 (n=133)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.7292 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` < `0.3668` → IC=+0.154 (n=504)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.3668 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.942` → IC=+0.172 (n=272)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 2.942 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` < `1.2459` → IC=+0.170 (n=541)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 1.2459 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` > `0.7271` → IC=+0.150 (n=484)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 0.7271 (IC base=+0.150)

- **PATRÓN** `volumen_pendiente_norm` > `0.0749` → IC=+0.170 (n=265)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` > 0.0749 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` < `2.5268` → IC=+0.178 (n=535)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 2.5268 (IC base=+0.150)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.158 (n=706)

  - _Acción_: Kelly boost +0.79€ cuando `libro_spread` < 0.01 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `8233.2138` → IC=+0.165 (n=541)
...
- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.195 (n=195)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0038 (IC base=+0.151)

- **PATRÓN** `drift_60min` |x|≤ `0.3026` → IC=+0.173 (n=292)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.3026 (IC base=+0.151)
...
- **PATRÓN** `ibs_20min` < `0.1373` → IC=+0.225 (n=129)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1373 (IC base=+0.151)

- **PATRÓN** `ibs_20min` > `0.7389` → IC=+0.157 (n=97)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` > 0.7389 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` > `0.6569` → IC=+0.192 (n=50)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.6569 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` < `0.3474` → IC=+0.157 (n=287)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.3474 (IC base=+0.151)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.168` → IC=+0.153 (n=145)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 3.168 (IC base=+0.151)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.372` → IC=+0.162 (n=294)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` < 6.372 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` < `1.2377` → IC=+0.167 (n=292)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.2377 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` > `0.6971` → IC=+0.160 (n=260)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.6971 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` > `0.1537` → IC=+0.206 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1537 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` < `2.6153` → IC=+0.183 (n=291)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 2.6153 (IC base=+0.151)

- **PATRÓN** `libro_liquidez` > `11518.9437` → IC=+0.172 (n=260)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 11518.9437 (IC base=+0.151)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.223 (n=665)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=2059)

- **FILTRO** `ibs_20min` > `0.2765` → IC=-0.197 (n=680)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2765
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=2044)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.142 (n=2031)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=4767)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.288 (n=1644)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=5154)

- **FILTRO** `ibs_7min` < `0.7352` → IC=-0.236 (n=1697)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7352
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=5101)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.177 (n=2300)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=4498)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.226 (n=1965)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=6360)

- **FILTRO** `ibs_7min` > `0.72` → IC=-0.166 (n=2081)

  - _Acción_: SKIP cuando `ibs_7min` > 0.72
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=6244)
...
- **FILTRO** `drift_7min_pct` |x|> `0.1444` → IC=-0.149 (n=474)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=923)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.161 (n=290)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=1089)

- **FILTRO** `py_entrada` < `0.38` → IC=-0.260 (n=323)
...
- **FILTRO** `ibs_7min` < `0.8125` → IC=-0.165 (n=344)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=1035)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.213 (n=273)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=680)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.262 (n=317)
...
- **FILTRO** `ibs_7min` < `0.7662` → IC=-0.195 (n=290)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=872)

- **FILTRO** `ballena_activa_n` > `43.0` → IC=-0.215 (n=289)

  - _Acción_: SKIP cuando `ballena_activa_n` > 43.0
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=873)

- **FILTRO** `hora_utc` > `15.0` → IC=-0.135 (n=360)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=788)
...
- **FILTRO** `ibs_7min` > `0.1818` → IC=-0.153 (n=387)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.181 (n=359)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=1150)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.272 (n=287)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=1117)

- **FILTRO** `ibs_7min` > `0.8272` → IC=-0.142 (n=350)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8272
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=1054)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.170 (n=92)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `14.018` → IC=+0.265 (n=66)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 14.018 (IC base=+0.265)
...
- **FILTRO** `sigma_ewma_delta_pct` > `16.925` → IC=-0.188 (n=312)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=2128)

- **PATRÓN** `ibs_15` > `0.4444` → IC=+0.149 (n=280)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.74€ cuando `ibs_15` > 0.4444 (IC base=-0.058)
...
- **PATRÓN** `ballena_activa_n` < `67.0` → IC=+0.326 (n=21)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 67.0 (IC base=-0.094)
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3542 sube el IC de -0.094 a +0.335 en UPDOWN_GBM_15M_TARDIO (n=83). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 6269 | -0.099 | -821.71€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 654 | -0.084 | -109.32€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 5615 | -0.101 | -712.38€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 2076 | -0.075 | -106.75€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2076 | -0.075 | -106.75€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1479 | -0.184 | -302.45€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1479 | -0.184 | -302.45€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 28525 | +0.114 | -1871.92€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 5416 | +0.187 | -180.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 107 | -0.105 | -50.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 