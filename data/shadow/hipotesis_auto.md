# Hipótesis automáticas — 2026-07-02 07:05 UTC
_Generado por shadow_postmortem.py sobre 2559 resoluciones (PNL=-82.70€)_
...
- **FILTRO** `ibs_15` > `0.3824` → IC=-0.157 (n=33)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: SKIP cuando `ibs_15` > 0.3824
  - _Potencial_: sin este filtro IC_bueno=+0.090 (n=103)

- **PATRÓN** `sigma_h` < `0.0098` → IC=+0.210 (n=67)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0098 (IC base=+0.061)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.167 (n=34)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 15.0 (IC base=+0.061)

- **PATRÓN** `ibs_15` < `0.0669` → IC=+0.223 (n=45)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0669 (IC base=+0.061)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0745` → IC=+0.134 (n=39)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.67€ cuando `pct_spot_vs_ref` |x|≤ 0.0745 (IC base=+0.123)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.206 (n=15)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0058 (IC base=+0.123)

- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.125 (n=30)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` > 0.0082 (IC base=+0.123)

- **PATRÓN** `drift_60min` |x|≤ `0.9082` → IC=+0.132 (n=36)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.66€ cuando `drift_60min` |x|≤ 0.9082 (IC base=+0.123)

- **PATRÓN** `drift_15min` |x|≤ `0.688` → IC=+0.133 (n=28)

  - _Acción_: Kelly boost +0.67€ cuando `drift_15min` |x|≤ 0.688 (IC base=+0.123)

- **PATRÓN** `ibs_15` < `0.0787` → IC=+0.326 (n=21)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0787 (IC base=+0.123)
...
- **FILTRO** `ibs_15` > `0.0669` → IC=-0.132 (n=17)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: SKIP cuando `ibs_15` > 0.0669
  - _Potencial_: sin este filtro IC_bueno=+0.318 (n=9)

- **PATRÓN** `sigma_h` < `0.0136` → IC=+0.182 (n=20)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0136 (IC base=+0.125)

- **PATRÓN** `drift_15min` |x|≤ `0.7214` → IC=+0.132 (n=17)

  - _Acción_: Kelly boost +0.66€ cuando `drift_15min` |x|≤ 0.7214 (IC base=+0.125)

## Estrategias nuevas sugeridas
_Derivadas de los patrones aprendidos:_

- **H-IBS-UPDOWN_GBM#15min**: IBS < 0.0669 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.223 n=45). Confirma señal de reversión media → alinear con BUY_YES.
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.0787 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.326 n=21). Confirma señal de reversión media → alinear con BUY_YES.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 9 | +0.102 | +2.45€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 9 | +0.102 | +2.45€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 792 | -0.034 | -36.27€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 568 | -0.011 | -10.44€ | 2 | 3 |
...
| ✅ UPDOWN_GBM#ETH | 307 | -0.008 | +5.65€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 224 | +0.009 | +11.61€ | 4 | 6 |
...
| ✅ UPDOWN_GBM#XRP | 70 | +0.014 | +6.36€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 59 | +0.057 | +9.65€ | 2 | 2 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.092) — sin ventaja clara. oversold(IBS<0.3): IC=+0.026 n=131 | neutral: IC=-0.066 n=120 | overbought(IBS>0.7): IC=-0.047 n=137
  - _Datos_: n=402 IC=-0.027 PNL=-11.04€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.003 n=1204 | tras_1loss IC=-0.035 n=1264 | tras_2loss IC=-0.050 n=658/40 | gap=+0.052 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=669 IC=-0.028 PNL=-22.72€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=669 IC=-0.028 PNL=-22.72€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=185 IC=+0.035 PNL=+17.42€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=185 IC=+0.035 PNL=+17.42€
...
**⏳ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: 142/200 ops en el filtro definido (IC actual=+0.056 PNL=+10.22€)
  - _Datos_: n=142 IC=+0.056 PNL=+10.22€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=142 IC=+0.056 PNL=+10.22€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=142 IC=+0.056 PNL=+10.22€
...
**⏳ H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: 62/120 ops en el filtro definido (IC actual=+0.141 PNL=+8.47€)
  - _Datos_: n=62 IC=+0.141 PNL=+8.47€
...
**⏳ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: 38/50 ops en el filtro definido (IC actual=+0.125 PNL=+10.66€)
  - _Datos_: n=38 IC=+0.125 PNL=+10.66€
