# Hipótesis automáticas — 2026-09-25 03:38 UTC
_Generado por shadow_postmortem.py sobre 598738 resoluciones (PNL=+67572.11€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.02` → IC=-0.231 (n=7087)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.02
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=21274)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `138.37` → IC=-0.249 (n=929)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 138.37
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=2787)
...
- **FILTRO** `restante_s_al_confirmar` < `165.07` → IC=-0.219 (n=1711)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 165.07
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=5135)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `126.3` → IC=-0.355 (n=1395)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 126.3
  - _Potencial_: sin este filtro IC_bueno=-0.119 (n=4185)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.206 (n=2132)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.205)

- **PATRÓN** `libro_liquidez` > `16027.7188` → IC=+0.241 (n=550)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.206 (n=1691)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.201)

- **PATRÓN** `py_entrada` < `0.375` → IC=+0.263 (n=1543)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.375 (IC base=+0.201)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.203 (n=2168)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.201)

- **PATRÓN** `libro_liquidez` > `15863.0179` → IC=+0.213 (n=560)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.143 (n=354)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.145 (n=2403)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.199 (n=11018)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.197)

- **PATRÓN** `py_entrada` > `0.74` → IC=+0.226 (n=3811)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.173 (n=2650)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` < 17.0 (IC base=+0.167)

- **PATRÓN** `py_entrada` < `0.73` → IC=+0.177 (n=2644)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.246 (n=848)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.184 (n=2622)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.194 (n=2564)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` < 17.0 (IC base=+0.192)

- **PATRÓN** `py_entrada` < `0.71` → IC=+0.195 (n=1976)
...
- **PATRÓN** `hora_utc` < `16.0` → IC=+0.438 (n=206)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 16.0 (IC base=+0.437)

- **PATRÓN** `py_entrada` > `0.915` → IC=+0.451 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.915 (IC base=+0.437)

- **PATRÓN** `libro_liquidez` > `19064.2752` → IC=+0.442 (n=67)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.236 (n=15298)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.180 (n=4748)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` < 12.0 (IC base=+0.176)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.190 (n=6399)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.224 (n=6130)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.222)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.264 (n=3503)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.189 (n=6231)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.266 (n=2115)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.256 (n=2285)
...
- **PATRÓN** `libro_liquidez` > `1549.9598` → IC=+0.295 (n=1110)
...
- **PATRÓN** `libro_liquidez` > `4262.2927` → IC=+0.298 (n=330)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.298 (n=557)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `1462.5909` → IC=+0.310 (n=476)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1462.5909 (IC base=+0.292)
...
### GBM_LATE_15M
- **PATRÓN** `drift_60min` |x|≤ `0.4861` → IC=+0.122 (n=8111)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.61€ cuando `drift_60min` |x|≤ 0.4861 (IC base=+0.105)

- **PATRÓN** `ibs_20min` > `0.9831` → IC=+0.244 (n=2705)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9831 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` > `0.8494` → IC=+0.252 (n=489)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8494 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` < `0.6362` → IC=+0.249 (n=2287)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6362 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.344` → IC=+0.186 (n=2143)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 8.344 (IC base=+0.105)

- **PATRÓN** `volumen_regimen` < `1.2089` → IC=+0.248 (n=2197)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2089 (IC base=+0.105)

- **PATRÓN** `volumen_regimen` > `1.0448` → IC=+0.257 (n=996)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0448 (IC base=+0.105)

- **PATRÓN** `volumen_pendiente_norm` > `0.3043` → IC=+0.219 (n=810)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3043 (IC base=+0.105)

- **PATRÓN** `volumen_spike_ratio` > `1.4628` → IC=+0.202 (n=5560)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4628 (IC base=+0.105)

- **PATRÓN** `ibs_20min` < `0.5702` → IC=+0.135 (n=9778)

  - _Acción_: Kelly boost +0.67€ cuando `ibs_20min` < 0.5702 (IC base=+0.066)

- **PATRÓN** `dist_vwap_pct` > `0.6169` → IC=+0.199 (n=725)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.6169 (IC base=+0.066)

- **PATRÓN** `volumen_regimen` < `0.6973` → IC=+0.184 (n=1508)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 0.6973 (IC base=+0.066)

- **PATRÓN** `volumen_regimen` > `1.0529` → IC=+0.177 (n=1554)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` > 1.0529 (IC base=+0.066)

- **PATRÓN** `volumen_pendiente_norm` > `0.1673` → IC=+0.226 (n=1634)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1673 (IC base=+0.066)

- **PATRÓN** `volumen_spike_ratio` > `1.5701` → IC=+0.203 (n=5156)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5701 (IC base=+0.066)

- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.213 (n=5561)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 135.0 (IC base=+0.066)

### GBM_LATE_15M#BNB#15min
- **PATRÓN** `sigma_h` < `0.005` → IC=+0.177 (n=612)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.005 (IC base=+0.160)

- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.176 (n=610)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0082 (IC base=+0.160)

- **PATRÓN** `drift_60min` |x|≤ `0.3505` → IC=+0.165 (n=1830)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.3505 (IC base=+0.160)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.168 (n=886)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 15.0 (IC base=+0.160)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.167 (n=1228)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.122` → IC=+0.269 (n=786)
...
- **PATRÓN** `ibs_20min` < `0.0542` → IC=+0.294 (n=609)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.458` → IC=+0.248 (n=1437)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.458 (IC base=+0.236)

- **PATRÓN** `volumen_pendiente_norm` < `0.0923` → IC=+0.233 (n=1188)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0923 (IC base=+0.236)

- **PATRÓN** `volumen_pendiente_norm` > `0.2774` → IC=+0.259 (n=185)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2774 (IC base=+0.236)

- **PATRÓN** `volumen_spike_ratio` > `2.6178` → IC=+0.243 (n=422)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6178 (IC base=+0.236)

- **PATRÓN** `libro_liquidez` > `1806.4548` → IC=+0.236 (n=924)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1806.4548 (IC base=+0.236)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0863` → IC=+0.251 (n=467)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0863 (IC base=+0.216)
...
- **PATRÓN** `dist_vwap_pct` < `0.5794` → IC=+0.221 (n=1465)
...
- **PATRÓN** `volumen_regimen` < `1.2525` → IC=+0.220 (n=1399)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2525 (IC base=+0.216)

- **PATRÓN** `volumen_regimen` > `0.8719` → IC=+0.222 (n=932)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8719 (IC base=+0.216)

- **PATRÓN** `volumen_pendiente_norm` > `0.2785` → IC=+0.244 (n=197)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2785 (IC base=+0.216)

- **PATRÓN** `volumen_spike_ratio` < `1.7485` → IC=+0.217 (n=914)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7485 (IC base=+0.216)

- **PATRÓN** `volumen_spike_ratio` > `2.368` → IC=+0.230 (n=457)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.368 (IC base=+0.216)

- **PATRÓN** `libro_liquidez` > `16055.5853` → IC=+0.222 (n=634)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 16055.5853 (IC base=+0.216)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.143 (n=653)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` < 7.0 (IC base=+0.140)

- **PATRÓN** `ibs_20min` < `0.3271` → IC=+0.198 (n=971)

  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` < 0.3271 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` < `0.1327` → IC=+0.157 (n=1294)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.1327 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.262` → IC=+0.161 (n=234)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 11.262 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.282` → IC=+0.144 (n=1325)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 4.282 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `1.2044` → IC=+0.151 (n=1456)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4292` → IC=+0.153 (n=1346)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 2.4292 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.4164` → IC=+0.149 (n=1346)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.4164 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `14091.9548` → IC=+0.145 (n=971)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 14091.9548 (IC base=+0.140)

- **PATRÓN** `ballena_activa_n` < `410.0` → IC=+0.150 (n=1265)

  - _Acción_: Kelly boost +0.75€ cuando `ballena_activa_n` < 410.0 (IC base=+0.140)

### GBM_LATE_15M#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0117` → IC=+0.208 (n=601)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.190 (n=1607)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.193` → IC=+0.251 (n=380)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.193 (IC base=+0.185)

- **PATRÓN** `volumen_pendiente_norm` < `0.2101` → IC=+0.187 (n=1801)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` < 0.2101 (IC base=+0.185)

- **PATRÓN** `volumen_pendiente_norm` > `0.3578` → IC=+0.197 (n=239)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.3578 (IC base=+0.185)

- **PATRÓN** `volumen_spike_ratio` > `2.8308` → IC=+0.203 (n=776)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8308 (IC base=+0.185)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.193 (n=1288)

  - _Acción_: Kelly boost +0.97€ cuando `libro_spread` < 0.02 (IC base=+0.185)

- **PATRÓN** `sigma_h` < `0.0103` → IC=+0.223 (n=1363)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0103 (IC base=+0.212)

- **PATRÓN** `drift_60min` |x|≤ `0.6053` → IC=+0.215 (n=1548)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_spike_ratio` < `1.7663` → IC=+0.204 (n=624)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7663 (IC base=+0.212)

- **PATRÓN** `volumen_spike_ratio` > `2.2024` → IC=+0.221 (n=945)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2024 (IC base=+0.212)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.217 (n=1054)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.212)

- **PATRÓN** `libro_liquidez` > `1893.9584` → IC=+0.223 (n=702)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1893.9584 (IC base=+0.212)

- **PATRÓN** `ballena_activa_n` < `24.0` → IC=+0.216 (n=922)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.150 (n=101)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=2226)

- **PATRÓN** `ibs_20min` > `0.9436` → IC=+0.213 (n=361)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9436 (IC base=+0.023)

- **PATRÓN** `dist_vwap_pct` > `0.1604` → IC=+0.318 (n=212)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1604 (IC base=+0.023)

- **PATRÓN** `dist_vwap_pct` < `0.7982` → IC=+0.331 (n=359)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.7982 (IC base=+0.023)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.739` → IC=+0.158 (n=714)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 4.739 (IC base=+0.023)

- **PATRÓN** `volumen_regimen` < `0.8561` → IC=+0.324 (n=219)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8561 (IC base=+0.023)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3014` → IC=+0.346 (n=89)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3014 (IC base=+0.023)
...
- **PATRÓN** `ballena_activa_n` < `161.0` → IC=+0.327 (n=316)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 161.0 (IC base=+0.023)

- **PATRÓN** `dist_vwap_pct` > `0.3372` → IC=+0.194 (n=266)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.3372 (IC base=+0.019)

- **PATRÓN** `volumen_regimen` < `0.8475` → IC=+0.166 (n=561)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.8475 (IC base=+0.019)

- **PATRÓN** `volumen_pendiente_norm` > `0.2255` → IC=+0.224 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2255 (IC base=+0.019)

- **PATRÓN** `volumen_spike_ratio` > `1.5154` → IC=+0.179 (n=703)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` > 1.5154 (IC base=+0.019)
...
- **FILTRO** `ibs_20min` > `0.2549` → IC=-0.125 (n=2201)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2549
  - _Potencial_: sin este filtro IC_bueno=+0.127 (n=1085)

- **FILTRO** `sigma_ewma_delta_pct` > `8.672` → IC=-0.209 (n=352)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.672
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=2934)
...
- **PATRÓN** `ibs_20min` < `0.2549` → IC=+0.127 (n=1085)
...
- **PATRÓN** `volumen_spike_ratio` < `1.3932` → IC=+0.300 (n=113)
...
- **PATRÓN** `dist_vwap_pct` > `0.9121` → IC=+0.272 (n=160)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9121 (IC base=-0.022)

- **PATRÓN** `volumen_regimen` < `0.7359` → IC=+0.254 (n=339)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7359 (IC base=-0.022)

- **PATRÓN** `volumen_regimen` > `1.2596` → IC=+0.284 (n=257)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2596 (IC base=-0.022)

- **PATRÓN** `volumen_pendiente_norm` > `0.1035` → IC=+0.276 (n=270)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1035 (IC base=-0.022)

- **PATRÓN** `volumen_spike_ratio` < `2.1457` → IC=+0.257 (n=579)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1457 (IC base=-0.022)

- **PATRÓN** `volumen_spike_ratio` > `1.5286` → IC=+0.247 (n=588)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5286 (IC base=-0.022)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.244 (n=675)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 35.0 (IC base=-0.022)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0098` → IC=+0.196 (n=3397)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0098 (IC base=+0.098)

- **PATRÓN** `ibs_20min` > `0.4762` → IC=+0.189 (n=9091)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.604` → IC=+0.156 (n=4785)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 3.604 (IC base=+0.098)

- **PATRÓN** `volumen_regimen` > `0.6895` → IC=+0.247 (n=3237)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6895 (IC base=+0.098)

- **PATRÓN** `volumen_pendiente_norm` > `0.2947` → IC=+0.268 (n=861)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2947 (IC base=+0.098)

- **PATRÓN** `volumen_spike_ratio` < `1.4655` → IC=+0.240 (n=1961)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4655 (IC base=+0.098)

- **PATRÓN** `volumen_spike_ratio` > `2.6649` → IC=+0.241 (n=1961)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6649 (IC base=+0.098)

- **PATRÓN** `ballena_activa_n` < `97.0` → IC=+0.267 (n=5400)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 97.0 (IC base=+0.098)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.156 (n=3384)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` > 0.0091 (IC base=+0.074)

- **PATRÓN** `ibs_20min` < `0.5484` → IC=+0.155 (n=8921)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` < 0.5484 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` > `0.7309` → IC=+0.241 (n=634)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7309 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` < `0.25` → IC=+0.240 (n=2822)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.25 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` < `0.7096` → IC=+0.240 (n=1316)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7096 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` > `1.205` → IC=+0.248 (n=997)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.205 (IC base=+0.074)

- **PATRÓN** `volumen_pendiente_norm` > `0.2441` → IC=+0.302 (n=747)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2441 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` < `1.6035` → IC=+0.261 (n=1736)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6035 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` > `2.3137` → IC=+0.258 (n=1789)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3137 (IC base=+0.074)

- **PATRÓN** `ballena_activa_n` < `84.0` → IC=+0.264 (n=3828)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 84.0 (IC base=+0.074)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `sigma_ewma_delta_pct` > `4.525` → IC=-0.158 (n=536)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.525
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=1801)

- **PATRÓN** `ibs_20min` > `0.8935` → IC=+0.269 (n=703)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8935 (IC base=+0.045)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.799` → IC=+0.209 (n=372)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.799 (IC base=+0.045)

- **PATRÓN** `volumen_pendiente_norm` > `0.2236` → IC=+0.273 (n=174)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1457` → IC=+0.199 (n=403)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.1457 (IC base=+0.045)

- **PATRÓN** `ballena_activa_n` < `14.0` → IC=+0.178 (n=396)

  - _Acción_: Kelly boost +0.89€ cuando `ballena_activa_n` < 14.0 (IC base=+0.045)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5496` → IC=+0.440 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5496 (IC base=-0.017)
...
- **PATRÓN** `dist_vwap_pct` > `0.1264` → IC=+0.173 (n=524)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.1264 (IC base=+0.026)

- **PATRÓN** `volumen_regimen` > `0.6718` → IC=+0.166 (n=838)
...
- **PATRÓN** `dist_vwap_pct` < `0.1608` → IC=+0.220 (n=580)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1608 (IC base=+0.005)

- **PATRÓN** `volumen_regimen` > `0.854` → IC=+0.227 (n=379)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4434` → IC=+0.223 (n=175)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4434 (IC base=+0.005)

- **PATRÓN** `volumen_spike_ratio` > `2.1554` → IC=+0.237 (n=238)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1554 (IC base=+0.005)

- **PATRÓN** `ballena_activa_n` < `469.0` → IC=+0.214 (n=522)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 469.0 (IC base=+0.005)

### GBM_LATE_15M_ESPACIO_ATR#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.287 (n=537)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.248 (n=1427)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `1961.66` → IC=+0.251 (n=535)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1961.66 (IC base=+0.245)

- **PATRÓN** `sigma_h` > `0.0099` → IC=+0.321 (n=583)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0099 (IC base=+0.283)

- **PATRÓN** `drift_60min` |x|≤ `0.6107` → IC=+0.284 (n=1283)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.2222` → IC=+0.291 (n=1130)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2222 (IC base=+0.283)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.808` → IC=+0.296 (n=507)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5928` → IC=+0.284 (n=396)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5928 (IC base=+0.283)

- **PATRÓN** `volumen_spike_ratio` > `2.7347` → IC=+0.289 (n=538)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7347 (IC base=+0.283)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.287 (n=866)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.283)

- **PATRÓN** `libro_liquidez` > `1881.0184` → IC=+0.300 (n=582)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1881.0184 (IC base=+0.283)

- **PATRÓN** `ballena_activa_n` < `29.0` → IC=+0.288 (n=772)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 29.0 (IC base=+0.283)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **FILTRO** `ibs_20min` < `0.2866` → IC=-0.185 (n=497)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2866
  - _Potencial_: sin este filtro IC_bueno=+0.079 (n=1494)
...
- **PATRÓN** `dist_vwap_pct` > `0.4536` → IC=+0.226 (n=250)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4536 (IC base=+0.013)

- **PATRÓN** `dist_vwap_pct` < `0.7062` → IC=+0.210 (n=587)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.7062 (IC base=+0.013)

- **PATRÓN** `volumen_regimen` < `0.9865` → IC=+0.237 (n=503)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9865 (IC base=+0.013)

- **PATRÓN** `volumen_regimen` > `0.5867` → IC=+0.210 (n=571)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5105` → IC=+0.263 (n=238)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5105 (IC base=+0.013)

- **PATRÓN** `ballena_activa_n` < `150.0` → IC=+0.243 (n=546)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 150.0 (IC base=+0.013)

- **PATRÓN** `dist_vwap_pct` > `0.1547` → IC=+0.211 (n=209)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1547 (IC base=-0.004)

- **PATRÓN** `dist_vwap_pct` < `0.6852` → IC=+0.204 (n=494)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6852 (IC base=-0.004)

- **PATRÓN** `volumen_regimen` < `1.1615` → IC=+0.214 (n=428)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.1615 (IC base=-0.004)

- **PATRÓN** `volumen_pendiente_norm` > `0.282` → IC=+0.286 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.282 (IC base=-0.004)

- **PATRÓN** `volumen_spike_ratio` < `1.8268` → IC=+0.268 (n=257)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8268 (IC base=-0.004)

- **PATRÓN** `ballena_activa_n` < `139.0` → IC=+0.251 (n=392)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 139.0 (IC base=-0.004)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **FILTRO** `ibs_20min` < `0.725` → IC=-0.198 (n=1077)

  - _Acción_: SKIP cuando `ibs_20min` < 0.725
  - _Potencial_: sin este filtro IC_bueno=+0.282 (n=1080)

- **FILTRO** `ibs_20min` > `0.6875` → IC=-0.234 (n=555)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6875
  - _Potencial_: sin este filtro IC_bueno=+0.097 (n=1685)

- **FILTRO** `sigma_ewma_delta_pct` > `4.705` → IC=-0.182 (n=488)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.705
  - _Potencial_: sin este filtro IC_bueno=+0.070 (n=1752)

- **PATRÓN** `ibs_20min` > `0.725` → IC=+0.282 (n=1080)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.58` → IC=+0.165 (n=341)
...
- **PATRÓN** `volumen_regimen` > `0.7306` → IC=+0.295 (n=711)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7306 (IC base=+0.042)

- **PATRÓN** `volumen_pendiente_norm` < `0.1034` → IC=+0.294 (n=740)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0972` → IC=+0.210 (n=526)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.565` → IC=+0.300 (n=694)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.565 (IC base=+0.277)

- **PATRÓN** `volumen_regimen` > `0.8606` → IC=+0.301 (n=881)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8606 (IC base=+0.277)

- **PATRÓN** `volumen_pendiente_norm` > `0.2811` → IC=+0.323 (n=196)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2811 (IC base=+0.277)

- **PATRÓN** `volumen_spike_ratio` > `2.1484` → IC=+0.295 (n=568)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1484 (IC base=+0.277)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.283 (n=1414)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.277)

- **PATRÓN** `libro_liquidez` > `2629.6954` → IC=+0.291 (n=880)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2629.6954 (IC base=+0.277)

- **PATRÓN** `sigma_h` > `0.0152` → IC=+0.300 (n=948)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.274 (n=705)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.272)

- **PATRÓN** `ibs_20min` < `0.3939` → IC=+0.305 (n=1422)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3939 (IC base=+0.272)

- **PATRÓN** `dist_vwap_pct` > `0.3053` → IC=+0.282 (n=544)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3053 (IC base=+0.272)

- **PATRÓN** `dist_vwap_pct` < `0.9843` → IC=+0.272 (n=1599)
...
- **PATRÓN** `volumen_regimen` > `1.2436` → IC=+0.307 (n=474)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2436 (IC base=+0.272)

- **PATRÓN** `volumen_pendiente_norm` > `0.2384` → IC=+0.337 (n=231)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2384 (IC base=+0.272)

- **PATRÓN** `volumen_spike_ratio` < `1.4288` → IC=+0.274 (n=419)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4288 (IC base=+0.272)

- **PATRÓN** `volumen_spike_ratio` > `2.1485` → IC=+0.276 (n=569)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1485 (IC base=+0.272)

- **PATRÓN** `libro_liquidez` > `2623.4578` → IC=+0.277 (n=948)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2623.4578 (IC base=+0.272)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.170 (n=2650)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0049 (IC base=+0.167)

- **PATRÓN** `sigma_h` > `0.0112` → IC=+0.201 (n=2642)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0112 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0906` → IC=+0.187 (n=2641)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.5759` → IC=+0.218 (n=7921)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5759 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.1772` → IC=+0.196 (n=3469)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1772 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.337` → IC=+0.255 (n=1624)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.337 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.211` → IC=+0.160 (n=5246)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.211 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` > `0.6307` → IC=+0.160 (n=5247)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2956` → IC=+0.196 (n=1186)
  ...
  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.2956 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.559` → IC=+0.168 (n=3345)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.559 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.6194` → IC=+0.177 (n=2532)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.6194 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `2404.2065` → IC=+0.168 (n=5281)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2404.2065 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `115.0` → IC=+0.180 (n=6831)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 115.0 (IC base=+0.167)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.182 (n=5051)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0066 (IC base=+0.168)

- **PATRÓN** `drift_60min` |x|≤ `0.0814` → IC=+0.208 (n=2526)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4792` → IC=+0.227 (n=7569)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4792 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` > `0.9248` → IC=+0.150 (n=735)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.9248 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` < `0.2338` → IC=+0.158 (n=5484)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.2338 (IC base=+0.168)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.348` → IC=+0.197 (n=1284)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 10.348 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` < `1.1737` → IC=+0.152 (n=5480)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1737 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.2911` → IC=+0.220 (n=1088)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2911 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` < `1.5601` → IC=+0.166 (n=3030)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.5601 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.2526` → IC=+0.171 (n=3122)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.2526 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `116.0` → IC=+0.173 (n=6536)

  - _Acción_: Kelly boost +0.86€ cuando `ballena_activa_n` < 116.0 (IC base=+0.168)

### GBM_LATE_15M_MULTIHORIZONTE#BNB#15min
- **PATRÓN** `sigma_h` < `0.0052` → IC=+0.217 (n=450)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.3432` → IC=+0.206 (n=1348)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3432 (IC base=+0.182)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.183 (n=1416)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 5.0 (IC base=+0.182)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.191 (n=901)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 11.0 (IC base=+0.182)

- **PATRÓN** `ibs_20min` > `0.8961` → IC=+0.278 (n=899)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8961 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.122` → IC=+0.311 (n=606)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.249 (n=484)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.239)

- **PATRÓN** `ibs_20min` < `0.342` → IC=+0.267 (n=985)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.342 (IC base=+0.239)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.305` → IC=+0.251 (n=1063)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.305 (IC base=+0.239)

- **PATRÓN** `volumen_pendiente_norm` < `0.0983` → IC=+0.235 (n=822)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0983 (IC base=+0.239)

- **PATRÓN** `volumen_pendiente_norm` > `0.2864` → IC=+0.252 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2864 (IC base=+0.239)

- **PATRÓN** `volumen_spike_ratio` < `1.4211` → IC=+0.261 (n=303)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4211 (IC base=+0.239)

- **PATRÓN** `libro_liquidez` > `1807.6` → IC=+0.247 (n=657)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1807.6 (IC base=+0.239)

### GBM_LATE_15M_MULTIHORIZONTE#BTC#15min
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.227 (n=397)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0028 (IC base=+0.161)

- **PATRÓN** `drift_60min` |x|≤ `0.0745` → IC=+0.197 (n=394)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.0745 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.185 (n=1242)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 5.0 (IC base=+0.161)

- **PATRÓN** `ibs_20min` > `0.4056` → IC=+0.227 (n=1179)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4056 (IC base=+0.161)

- **PATRÓN** `dist_vwap_pct` > `0.212` → IC=+0.213 (n=702)
...
- **PATRÓN** `volumen_regimen` < `1.2628` → IC=+0.163 (n=1180)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.2628 (IC base=+0.161)

- **PATRÓN** `volumen_regimen` > `1.0736` → IC=+0.167 (n=535)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 1.0736 (IC base=+0.161)

- **PATRÓN** `volumen_pendiente_norm` > `0.2815` → IC=+0.208 (n=193)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2815 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` < `1.5024` → IC=+0.182 (n=504)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 1.5024 (IC base=+0.161)

- **PATRÓN** `libro_liquidez` > `11871.8913` → IC=+0.170 (n=1054)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 11871.8913 (IC base=+0.161)

- **PATRÓN** `ballena_activa_n` < `389.0` → IC=+0.159 (n=969)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 389.0 (IC base=+0.161)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.157 (n=1287)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0057 (IC base=+0.137)

- **PATRÓN** `drift_60min` |x|≤ `0.0596` → IC=+0.202 (n=428)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5665` → IC=+0.187 (n=1284)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` < 0.5665 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.1339` → IC=+0.160 (n=1269)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` < 0.1339 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.889` → IC=+0.202 (n=256)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.889 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `1.2096` → IC=+0.157 (n=1284)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.2096 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` > `0.1561` → IC=+0.155 (n=392)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_pendiente_norm` > 0.1561 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` < `2.4356` → IC=+0.146 (n=1172)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 2.4356 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `1.42` → IC=+0.137 (n=1172)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 1.42 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `215.0` → IC=+0.166 (n=366)
...
- **PATRÓN** `sigma_h` < `0.0114` → IC=+0.231 (n=1103)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0114 (IC base=+0.218)

- **PATRÓN** `drift_60min` |x|≤ `0.2465` → IC=+0.237 (n=736)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2465 (IC base=+0.218)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.276 (n=387)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.218)

- **PATRÓN** `ibs_20min` < `0.2407` → IC=+0.255 (n=971)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2407 (IC base=+0.218)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.746` → IC=+0.264 (n=473)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7747` → IC=+0.212 (n=450)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7747 (IC base=+0.218)

- **PATRÓN** `volumen_spike_ratio` > `2.2159` → IC=+0.228 (n=682)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2159 (IC base=+0.218)

- **PATRÓN** `libro_liquidez` > `1892.3107` → IC=+0.221 (n=500)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1892.3107 (IC base=+0.218)

- **PATRÓN** `ballena_activa_n` < `11.0` → IC=+0.208 (n=330)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.172 (n=1117)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0066 (IC base=+0.142)

- **PATRÓN** `drift_60min` |x|≤ `0.4327` → IC=+0.157 (n=1267)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.375` → IC=+0.197 (n=1267)
...
- **PATRÓN** `volumen_regimen` > `0.6274` → IC=+0.146 (n=1267)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 0.6274 (IC base=+0.142)

- **PATRÓN** `volumen_pendiente_norm` > `0.2429` → IC=+0.194 (n=269)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2429 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` < `1.4237` → IC=+0.154 (n=414)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 1.4237 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` > `2.5111` → IC=+0.170 (n=413)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.5111 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `6496.8974` → IC=+0.184 (n=845)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 6496.8974 (IC base=+0.142)

- **PATRÓN** `ballena_activa_n` < `163.0` → IC=+0.146 (n=1205)

  - _Acción_: Kelly boost +0.73€ cuando `ballena_activa_n` < 163.0 (IC base=+0.142)

- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.153 (n=1334)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0072 (IC base=+0.124)

- **PATRÓN** `drift_60min` |x|≤ `0.3848` → IC=+0.143 (n=1334)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.3848 (IC base=+0.124)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.179 (n=521)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 17.0 (IC base=+0.124)

- **PATRÓN** `ibs_20min` < `0.6381` → IC=+0.173 (n=1334)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.6381 (IC base=+0.124)

- **PATRÓN** `dist_vwap_pct` < `0.5895` → IC=+0.133 (n=1549)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.5895 (IC base=+0.124)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.963` → IC=+0.170 (n=470)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 6.963 (IC base=+0.124)

- **PATRÓN** `volumen_regimen` < `0.8476` → IC=+0.148 (n=890)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.8476 (IC base=+0.124)

- **PATRÓN** `volumen_pendiente_norm` > `0.291` → IC=+0.197 (n=193)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.291 (IC base=+0.124)

- **PATRÓN** `volumen_spike_ratio` < `1.789` → IC=+0.137 (n=808)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` < 1.789 (IC base=+0.124)

- **PATRÓN** `libro_liquidez` > `10039.916` → IC=+0.161 (n=605)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 10039.916 (IC base=+0.124)

### GBM_LATE_15M_MULTIHORIZONTE#SOL#15min
- **PATRÓN** `sigma_h` > `0.0101` → IC=+0.157 (n=651)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.5167` → IC=+0.205 (n=1432)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5167 (IC base=+0.118)

- **PATRÓN** `dist_vwap_pct` > `0.8477` → IC=+0.216 (n=431)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8477 (IC base=+0.118)
...
- **PATRÓN** `volumen_regimen` < `1.2184` → IC=+0.131 (n=1433)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 1.2184 (IC base=+0.118)

- **PATRÓN** `volumen_regimen` > `0.6461` → IC=+0.123 (n=1432)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4678` → IC=+0.128 (n=1383)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` < 2.4678 (IC base=+0.118)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.127 (n=1497)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.02 (IC base=+0.118)

- **PATRÓN** `libro_liquidez` > `2896.4901` → IC=+0.196 (n=650)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 2896.4901 (IC base=+0.118)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.136 (n=1087)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1034` → IC=+0.156 (n=484)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.1034 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.162 (n=658)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 15.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` < `0.5652` → IC=+0.210 (n=1451)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5652 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `1.013` → IC=+0.145 (n=195)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 1.013 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.2006` → IC=+0.138 (n=1336)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.2006 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.006` → IC=+0.136 (n=234)

  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` > 9.006 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` < `0.6362` → IC=+0.138 (n=484)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` < 0.6362 (IC base=+0.113)

- **PATRÓN** `volumen_pendiente_norm` > `0.2757` → IC=+0.165 (n=177)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.2757 (IC base=+0.113)

- **PATRÓN** `volumen_spike_ratio` > `2.4248` → IC=+0.131 (n=434)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` > 2.4248 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `2170.6408` → IC=+0.146 (n=967)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 2170.6408 (IC base=+0.113)

### GBM_LATE_15M_MULTIHORIZONTE#XRP#15min
- **PATRÓN** `sigma_h` > `0.0188` → IC=+0.213 (n=913)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0188 (IC base=+0.200)

- **PATRÓN** `drift_60min` |x|≤ `0.1355` → IC=+0.202 (n=457)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1355 (IC base=+0.200)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.204 (n=1419)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.200)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.203 (n=622)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.200)

- **PATRÓN** `ibs_20min` > `0.74` → IC=+0.259 (n=1224)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.74 (IC base=+0.200)

- **PATRÓN** `dist_vwap_pct` > `0.5119` → IC=+0.221 (n=654)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5119 (IC base=+0.200)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.576` → IC=+0.241 (n=643)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.576 (IC base=+0.200)

- **PATRÓN** `volumen_regimen` < `1.2068` → IC=+0.203 (n=1369)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2068 (IC base=+0.200)

- **PATRÓN** `volumen_regimen` > `0.6279` → IC=+0.210 (n=1368)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6279 (IC base=+0.200)

- **PATRÓN** `volumen_pendiente_norm` > `0.2333` → IC=+0.264 (n=261)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2333 (IC base=+0.200)

- **PATRÓN** `volumen_spike_ratio` < `2.1479` → IC=+0.210 (n=1163)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1479 (IC base=+0.200)

- **PATRÓN** `volumen_spike_ratio` > `1.409` → IC=+0.207 (n=1322)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.409 (IC base=+0.200)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.204 (n=1457)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.200)

- **PATRÓN** `libro_liquidez` > `2825.677` → IC=+0.206 (n=621)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2825.677 (IC base=+0.200)

- **PATRÓN** `sigma_h` < `0.0115` → IC=+0.228 (n=622)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0115 (IC base=+0.203)

- **PATRÓN** `sigma_h` > `0.0172` → IC=+0.207 (n=942)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0172 (IC base=+0.203)

- **PATRÓN** `drift_60min` |x|≤ `0.0902` → IC=+0.223 (n=471)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.211 (n=646)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.203)

- **PATRÓN** `ibs_20min` < `0.4385` → IC=+0.245 (n=1413)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4385 (IC base=+0.203)

- **PATRÓN** `dist_vwap_pct` > `1.265` → IC=+0.231 (n=173)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.265 (IC base=+0.203)
...
- **PATRÓN** `volumen_regimen` > `0.6303` → IC=+0.215 (n=1413)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2088` → IC=+0.195 (n=1118)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 2.2088 (IC base=+0.203)

- **PATRÓN** `volumen_spike_ratio` > `1.4369` → IC=+0.199 (n=1270)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.4369 (IC base=+0.203)

- **PATRÓN** `libro_liquidez` > `2595.656` → IC=+0.211 (n=942)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2595.656 (IC base=+0.203)

### GBM_LATE_15M_PYCONFIRMADO
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.160 (n=637)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0039 (IC base=+0.146)

- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.180 (n=635)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0088 (IC base=+0.146)

- **PATRÓN** `drift_60min` |x|≤ `0.1007` → IC=+0.153 (n=635)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.1007 (IC base=+0.146)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.184 (n=961)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 15.0 (IC base=+0.146)

- **PATRÓN** `ibs_20min` > `0.3875` → IC=+0.179 (n=1903)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` > 0.3875 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `0.8753` → IC=+0.183 (n=304)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.8753 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.703` → IC=+0.172 (n=861)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 3.703 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` < `0.871` → IC=+0.165 (n=1117)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.871 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` > `0.698` → IC=+0.148 (n=1496)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` > 0.698 (IC base=+0.146)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4411` → IC=+0.162 (n=611)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 1.4411 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` > `1.8262` → IC=+0.149 (n=1221)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.8262 (IC base=+0.146)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.149 (n=2162)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.02 (IC base=+0.146)

- **PATRÓN** `libro_liquidez` > `12451.6072` → IC=+0.157 (n=634)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 12451.6072 (IC base=+0.146)

- **PATRÓN** `ballena_activa_n` < `160.0` → IC=+0.167 (n=1680)

  - _Acción_: Kelly boost +0.84€ cuando `ballena_activa_n` < 160.0 (IC base=+0.146)

- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.142 (n=1324)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.66` → IC=+0.139 (n=1981)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` < 0.66 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` < `0.2132` → IC=+0.123 (n=1757)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.2132 (IC base=+0.111)

- **PATRÓN** `volumen_regimen` < `0.6256` → IC=+0.127 (n=596)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` < 0.6256 (IC base=+0.111)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4535` → IC=+0.141 (n=636)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.4535 (IC base=+0.111)

- **PATRÓN** `libro_liquidez` > `2760.1991` → IC=+0.125 (n=1769)

  - _Acción_: Kelly boost +0.63€ cuando `libro_liquidez` > 2760.1991 (IC base=+0.111)

### GBM_LATE_15M_PYCONFIRMADO#BTC#15min
- **PATRÓN** `drift_60min` |x|≤ `0.3432` → IC=+0.122 (n=479)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.2515` → IC=+0.142 (n=479)

  - _Acción_: Kelly boost +0.71€ cuando `ibs_20min` > 0.2515 (IC base=+0.103)

- **PATRÓN** `dist_vwap_pct` > `0.3087` → IC=+0.155 (n=169)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.3087 (IC base=+0.103)

- **PATRÓN** `volumen_regimen` < `0.6182` → IC=+0.148 (n=160)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.6182 (IC base=+0.103)

- **PATRÓN** `libro_liquidez` > `15024.4373` → IC=+0.145 (n=319)

  - _Acción_: Kelly boost +0.72€ cuando `libro_liquidez` > 15024.4373 (IC base=+0.103)

- **PATRÓN** `ballena_activa_n` < `146.0` → IC=+0.149 (n=152)

  - _Acción_: Kelly boost +0.75€ cuando `ballena_activa_n` < 146.0 (IC base=+0.103)

- **PATRÓN** `sigma_h` < `0.0027` → IC=+0.216 (n=216)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0027 (IC base=+0.139)

- **PATRÓN** `drift_60min` |x|≤ `0.3402` → IC=+0.161 (n=635)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6039` → IC=+0.186 (n=559)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` < 0.6039 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` < `0.4869` → IC=+0.154 (n=730)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.4869 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.41` → IC=+0.159 (n=247)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 4.41 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` < `1.218` → IC=+0.144 (n=635)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1023` → IC=+0.158 (n=550)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.1023 (IC base=+0.139)

- **PATRÓN** `volumen_spike_ratio` > `1.42` → IC=+0.152 (n=625)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.42 (IC base=+0.139)

- **PATRÓN** `ballena_activa_n` < `325.0` → IC=+0.157 (n=532)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 325.0 (IC base=+0.139)

### GBM_LATE_15M_PYCONFIRMADO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.252 (n=260)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0038 (IC base=+0.189)

- **PATRÓN** `sigma_h` > `0.007` → IC=+0.195 (n=198)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.007 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.0977` → IC=+0.208 (n=197)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0977 (IC base=+0.189)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.204 (n=614)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.189)

- **PATRÓN** `ibs_20min` > `0.9737` → IC=+0.269 (n=197)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9737 (IC base=+0.189)

- **PATRÓN** `dist_vwap_pct` > `0.1561` → IC=+0.208 (n=313)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1561 (IC base=+0.189)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.026` → IC=+0.223 (n=193)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.026 (IC base=+0.189)

- **PATRÓN** `volumen_regimen` < `0.8342` → IC=+0.197 (n=394)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` < 0.8342 (IC base=+0.189)

- **PATRÓN** `volumen_regimen` > `1.1573` → IC=+0.203 (n=197)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1573 (IC base=+0.189)

- **PATRÓN** `volumen_pendiente_norm` > `0.2622` → IC=+0.267 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2622 (IC base=+0.189)

- **PATRÓN** `volumen_spike_ratio` < `1.3991` → IC=+0.221 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3991 (IC base=+0.189)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.198 (n=656)

  - _Acción_: Kelly boost +0.99€ cuando `libro_spread` < 0.01 (IC base=+0.189)

- **PATRÓN** `libro_liquidez` > `12428.4336` → IC=+0.214 (n=197)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12428.4336 (IC base=+0.189)

- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.124 (n=488)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` < 0.0061 (IC base=+0.097)

- **PATRÓN** `ibs_20min` < `0.1458` → IC=+0.159 (n=244)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` < 0.1458 (IC base=+0.097)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.306` → IC=+0.131 (n=147)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` > 6.306 (IC base=+0.097)

- **PATRÓN** `volumen_regimen` < `0.6847` → IC=+0.130 (n=244)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 0.6847 (IC base=+0.097)

- **PATRÓN** `volumen_pendiente_norm` > `0.1662` → IC=+0.131 (n=128)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_pendiente_norm` > 0.1662 (IC base=+0.097)

- **PATRÓN** `libro_liquidez` > `3821.4035` → IC=+0.122 (n=496)

  - _Acción_: Kelly boost +0.61€ cuando `libro_liquidez` > 3821.4035 (IC base=+0.097)
...
- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.180 (n=195)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0089 (IC base=+0.132)

- **PATRÓN** `drift_60min` |x|≤ `0.5317` → IC=+0.136 (n=429)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.5317 (IC base=+0.132)
...
- **PATRÓN** `dist_vwap_pct` > `0.9426` → IC=+0.234 (n=77)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9426 (IC base=+0.132)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.092` → IC=+0.205 (n=191)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.092 (IC base=+0.132)

- **PATRÓN** `volumen_regimen` < `1.0674` → IC=+0.152 (n=377)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.0674 (IC base=+0.132)

- **PATRÓN** `volumen_pendiente_norm` > `0.2873` → IC=+0.182 (n=61)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.2873 (IC base=+0.132)

- **PATRÓN** `volumen_spike_ratio` < `1.4824` → IC=+0.155 (n=137)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 1.4824 (IC base=+0.132)

- **PATRÓN** `volumen_spike_ratio` > `2.2097` → IC=+0.174 (n=185)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.2097 (IC base=+0.132)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.133 (n=467)

  - _Acción_: Kelly boost +0.67€ cuando `libro_spread` < 0.02 (IC base=+0.132)

- **PATRÓN** `libro_liquidez` > `3103.1049` → IC=+0.190 (n=143)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 3103.1049 (IC base=+0.132)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.206 (n=3379)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.216 (n=10126)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4706 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` > `0.9813` → IC=+0.204 (n=1431)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9813 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.857` → IC=+0.234 (n=3728)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.857 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` < `0.8804` → IC=+0.166 (n=4514)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.8804 (IC base=+0.169)

- **PATRÓN** `volumen_pendiente_norm` > `0.2895` → IC=+0.201 (n=1394)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2895 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `2.5993` → IC=+0.186 (n=3246)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 2.5993 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `1789.8784` → IC=+0.170 (n=10125)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 1789.8784 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `86.0` → IC=+0.195 (n=7701)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 86.0 (IC base=+0.169)

- **PATRÓN** `sigma_h` < `0.007` → IC=+0.192 (n=6135)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.007 (IC base=+0.182)

- **PATRÓN** `drift_60min` |x|≤ `0.149` → IC=+0.189 (n=4039)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.149 (IC base=+0.182)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.207 (n=3501)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.182)

- **PATRÓN** `ibs_20min` < `0.566` → IC=+0.239 (n=9178)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.566 (IC base=+0.182)

- **PATRÓN** `dist_vwap_pct` < `0.2497` → IC=+0.162 (n=5674)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2497 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.037` → IC=+0.201 (n=1292)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.037 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.732` → IC=+0.183 (n=8874)

  - _Acción_: Kelly boost +0.92€ cuando `sigma_ewma_delta_pct` < 3.732 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` < `0.7046` → IC=+0.162 (n=2773)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.7046 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` > `1.204` → IC=+0.154 (n=2101)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.204 (IC base=+0.182)

- **PATRÓN** `volumen_pendiente_norm` > `0.2881` → IC=+0.243 (n=1210)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2881 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` > `2.6295` → IC=+0.193 (n=2809)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 2.6295 (IC base=+0.182)

- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.194 (n=5436)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 48.0 (IC base=+0.182)

### GBM_LATE_15M_TARDIO#BNB#15min
- **PATRÓN** `sigma_h` < `0.0051` → IC=+0.207 (n=574)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0051 (IC base=+0.190)

- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.227 (n=572)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.195 (n=1158)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.541` → IC=+0.344 (n=390)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5691` → IC=+0.199 (n=540)
...
- **PATRÓN** `sigma_h` > `0.0045` → IC=+0.265 (n=1350)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0045 (IC base=+0.259)

- **PATRÓN** `drift_60min` |x|≤ `0.1294` → IC=+0.285 (n=593)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.3509` → IC=+0.290 (n=1186)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3509 (IC base=+0.259)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.569` → IC=+0.264 (n=1350)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.569 (IC base=+0.259)

- **PATRÓN** `volumen_pendiente_norm` > `0.2269` → IC=+0.278 (n=250)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2269 (IC base=+0.259)

- **PATRÓN** `volumen_spike_ratio` < `1.5478` → IC=+0.259 (n=545)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5478 (IC base=+0.259)

- **PATRÓN** `volumen_spike_ratio` > `2.635` → IC=+0.278 (n=413)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.635 (IC base=+0.259)

- **PATRÓN** `libro_liquidez` > `1804.9372` → IC=+0.263 (n=898)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1804.9372 (IC base=+0.259)
...
- **PATRÓN** `drift_60min` |x|≤ `0.085` → IC=+0.162 (n=542)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.3086` → IC=+0.204 (n=1623)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.755` → IC=+0.169 (n=370)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 9.755 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.251` → IC=+0.154 (n=1463)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1148` → IC=+0.162 (n=1381)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.1148 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.4047` → IC=+0.155 (n=1569)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.4047 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `15748.4784` → IC=+0.159 (n=736)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 15748.4784 (IC base=+0.150)

- **PATRÓN** `ballena_activa_n` < `473.0` → IC=+0.158 (n=1501)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 473.0 (IC base=+0.150)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.166 (n=1404)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0057 (IC base=+0.152)

- **PATRÓN** `drift_60min` |x|≤ `0.2608` → IC=+0.166 (n=1235)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.2608 (IC base=+0.152)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.179 (n=471)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 18.0 (IC base=+0.152)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.152 (n=634)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 7.0 (IC base=+0.152)

- **PATRÓN** `ibs_20min` < `0.6583` → IC=+0.197 (n=1403)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` < 0.6583 (IC base=+0.152)

- **PATRÓN** `dist_vwap_pct` > `0.6925` → IC=+0.157 (n=231)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.6925 (IC base=+0.152)

- **PATRÓN** `dist_vwap_pct` < `0.1332` → IC=+0.167 (n=1261)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.1332 (IC base=+0.152)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.279` → IC=+0.153 (n=1270)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 4.279 (IC base=+0.152)

- **PATRÓN** `volumen_regimen` < `1.1913` → IC=+0.163 (n=1403)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.1913 (IC base=+0.152)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4071` → IC=+0.160 (n=1305)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.4071 (IC base=+0.152)

- **PATRÓN** `volumen_spike_ratio` > `1.5107` → IC=+0.161 (n=1166)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 1.5107 (IC base=+0.152)

- **PATRÓN** `ballena_activa_n` < `421.0` → IC=+0.159 (n=1067)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 421.0 (IC base=+0.152)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `sigma_h` > `0.012` → IC=+0.245 (n=547)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.222 (n=1661)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.809` → IC=+0.293 (n=482)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.809 (IC base=+0.217)

- **PATRÓN** `volumen_pendiente_norm` < `0.2107` → IC=+0.221 (n=1624)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2107 (IC base=+0.217)

- **PATRÓN** `volumen_spike_ratio` > `2.8331` → IC=+0.240 (n=709)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8331 (IC base=+0.217)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.228 (n=1173)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.217)

- **PATRÓN** `sigma_h` < `0.0117` → IC=+0.239 (n=1531)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0117 (IC base=+0.233)

- **PATRÓN** `drift_60min` |x|≤ `0.1678` → IC=+0.237 (n=674)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1678 (IC base=+0.233)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.261 (n=584)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.233)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.238 (n=722)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.233)

- **PATRÓN** `ibs_20min` < `0.36` → IC=+0.269 (n=1347)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.36 (IC base=+0.233)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.766` → IC=+0.274 (n=569)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7544` → IC=+0.231 (n=619)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7544 (IC base=+0.233)

- **PATRÓN** `volumen_spike_ratio` > `2.1892` → IC=+0.237 (n=938)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1892 (IC base=+0.233)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.240 (n=1043)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.233)

- **PATRÓN** `libro_liquidez` > `1949.24` → IC=+0.246 (n=510)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1949.24 (IC base=+0.233)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.231 (n=1334)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 54.0 (IC base=+0.233)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0035` → IC=+0.182 (n=577)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0035 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.4376` → IC=+0.145 (n=1723)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.4376 (IC base=+0.136)
...
- **PATRÓN** `dist_vwap_pct` > `0.3755` → IC=+0.164 (n=682)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.3755 (IC base=+0.136)
...
- **PATRÓN** `volumen_regimen` < `0.8743` → IC=+0.156 (n=1149)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.8743 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.2368` → IC=+0.215 (n=321)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2368 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` < `1.5221` → IC=+0.149 (n=734)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.5221 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.768` → IC=+0.150 (n=1112)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.768 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `8055.3183` → IC=+0.229 (n=781)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8055.3183 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `91.0` → IC=+0.148 (n=706)

  - _Acción_: Kelly boost +0.74€ cuando `ballena_activa_n` < 91.0 (IC base=+0.136)

- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.158 (n=1406)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0076 (IC base=+0.139)

- **PATRÓN** `drift_60min` |x|≤ `0.4471` → IC=+0.155 (n=1405)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.4471 (IC base=+0.139)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.175 (n=528)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 17.0 (IC base=+0.139)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.145 (n=643)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` < 7.0 (IC base=+0.139)

- **PATRÓN** `ibs_20min` < `0.7022` → IC=+0.186 (n=1405)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` < 0.7022 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` < `0.6022` → IC=+0.144 (n=1556)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.6022 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.146` → IC=+0.178 (n=209)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 11.146 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` < `0.6938` → IC=+0.152 (n=618)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.6938 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` > `1.1921` → IC=+0.145 (n=468)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 1.1921 (IC base=+0.139)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4421` → IC=+0.152 (n=1333)
...
- **PATRÓN** `ballena_activa_n` < `178.0` → IC=+0.147 (n=1328)

  - _Acción_: Kelly boost +0.74€ cuando `ballena_activa_n` < 178.0 (IC base=+0.139)

### GBM_LATE_15M_TARDIO#SOL#15min
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.140 (n=1141)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.453` → IC=+0.234 (n=644)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.453 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` < `0.8919` → IC=+0.136 (n=1142)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 0.8919 (IC base=+0.113)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.127 (n=1720)

  - _Acción_: Kelly boost +0.63€ cuando `libro_spread` < 0.02 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `2898.7698` → IC=+0.247 (n=571)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2898.7698 (IC base=+0.113)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.133 (n=1325)

  - _Acción_: Kelly boost +0.67€ cuando `ballena_activa_n` < 54.0 (IC base=+0.113)
...
- **PATRÓN** `ibs_20min` < `0.6364` → IC=+0.205 (n=1667)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6364 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.22` → IC=+0.130 (n=1359)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.22 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.469` → IC=+0.125 (n=1610)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.210 (n=1527)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `ibs_20min` < `0.5172` → IC=+0.256 (n=1830)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5172 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` > `1.2849` → IC=+0.204 (n=305)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2849 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` < `0.9321` → IC=+0.204 (n=2032)
...
- **PATRÓN** `volumen_regimen` > `1.2357` → IC=+0.234 (n=610)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1989` → IC=+0.197 (n=1448)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 2.1989 (IC base=+0.201)

- **PATRÓN** `volumen_spike_ratio` > `1.4316` → IC=+0.198 (n=1645)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.4316 (IC base=+0.201)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.206 (n=1085)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.141 (n=3071)

- **PATRÓN** `sigma_h` < `0.0091` → IC=+0.163 (n=2576)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0091 (IC base=+0.154)

- **PATRÓN** `drift_60min` |x|≤ `0.5212` → IC=+0.163 (n=2927)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.82€ cuando `drift_60min` |x|≤ 0.5212 (IC base=+0.154)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.165 (n=983)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 18.0 (IC base=+0.154)

- **PATRÓN** `hora_utc` < `4.0` → IC=+0.168 (n=1026)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 4.0 (IC base=+0.154)

- **PATRÓN** `ibs_20min` > `0.942` → IC=+0.213 (n=976)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.942 (IC base=+0.154)

- **PATRÓN** `dist_vwap_pct` > `0.1866` → IC=+0.166 (n=1089)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.1866 (IC base=+0.154)

- **PATRÓN** `dist_vwap_pct` < `0.4908` → IC=+0.148 (n=1795)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.4908 (IC base=+0.154)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.188` → IC=+0.183 (n=480)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 10.188 (IC base=+0.154)

- **PATRÓN** `volumen_regimen` > `0.8969` → IC=+0.163 (n=1277)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` > 0.8969 (IC base=+0.154)

- **PATRÓN** `volumen_pendiente_norm` > `0.1711` → IC=+0.185 (n=799)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1711 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` < `1.4564` → IC=+0.165 (n=964)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.4564 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` > `1.8808` → IC=+0.163 (n=1927)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.8808 (IC base=+0.154)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.155 (n=2039)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.154)

- **PATRÓN** `libro_liquidez` > `8307.441` → IC=+0.159 (n=1327)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 8307.441 (IC base=+0.154)

- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.197 (n=773)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0038 (IC base=+0.139)

- **PATRÓN** `drift_60min` |x|≤ `0.4875` → IC=+0.157 (n=2315)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.4875 (IC base=+0.139)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.174 (n=847)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 17.0 (IC base=+0.139)

- **PATRÓN** `hora_utc` < `4.0` → IC=+0.165 (n=786)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 4.0 (IC base=+0.139)

- **PATRÓN** `ibs_20min` < `0.1802` → IC=+0.174 (n=1019)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.1802 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` > `0.6865` → IC=+0.158 (n=443)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.6865 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.274` → IC=+0.150 (n=2303)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` < 6.274 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` < `0.9009` → IC=+0.158 (n=1476)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.9009 (IC base=+0.139)

- **PATRÓN** `volumen_pendiente_norm` < `0.0969` → IC=+0.142 (n=2096)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_pendiente_norm` < 0.0969 (IC base=+0.139)

- **PATRÓN** `volumen_pendiente_norm` > `0.0728` → IC=+0.143 (n=1070)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_pendiente_norm` > 0.0728 (IC base=+0.139)

- **PATRÓN** `volumen_spike_ratio` < `1.4237` → IC=+0.152 (n=762)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 1.4237 (IC base=+0.139)

- **PATRÓN** `volumen_spike_ratio` > `1.8123` → IC=+0.143 (n=1523)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.8123 (IC base=+0.139)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.141 (n=3071)

  - _Acción_: Kelly boost +0.71€ cuando `libro_spread` < 0.01 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `7207.0296` → IC=+0.153 (n=2068)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 7207.0296 (IC base=+0.139)

### GBM_LATE_5M#BTC#5min
- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.175 (n=346)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` < 0.0056 (IC base=+0.164)

- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.187 (n=132)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0065 (IC base=+0.164)

- **PATRÓN** `drift_60min` |x|≤ `0.0919` → IC=+0.202 (n=132)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0919 (IC base=+0.164)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.172 (n=394)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 5.0 (IC base=+0.164)

- **PATRÓN** `ibs_20min` < `0.5544` → IC=+0.187 (n=263)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` < 0.5544 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.212` → IC=+0.174 (n=191)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.212 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` < `0.3825` → IC=+0.165 (n=380)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.3825 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.489` → IC=+0.167 (n=40)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 8.489 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.415` → IC=+0.173 (n=417)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` < 2.415 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` < `1.2372` → IC=+0.172 (n=394)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 1.2372 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` > `0.8476` → IC=+0.193 (n=262)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 0.8476 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.3053` → IC=+0.295 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3053 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` < `1.4485` → IC=+0.202 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4485 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` > `2.6647` → IC=+0.207 (n=131)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6647 (IC base=+0.164)

- **PATRÓN** `libro_liquidez` > `12565.4829` → IC=+0.212 (n=352)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12565.4829 (IC base=+0.164)

- **PATRÓN** `sigma_h` < `0.0034` → IC=+0.216 (n=420)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0034 (IC base=+0.139)

- **PATRÓN** `drift_60min` |x|≤ `0.085` → IC=+0.179 (n=316)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.174 (n=357)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` < 5.0 (IC base=+0.139)

- **PATRÓN** `ibs_20min` < `0.138` → IC=+0.178 (n=417)

  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` < 0.138 (IC base=+0.139)

- **PATRÓN** `ibs_20min` > `0.6078` → IC=+0.147 (n=429)

  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` > 0.6078 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` > `0.6965` → IC=+0.167 (n=88)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.6965 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` < `0.2198` → IC=+0.139 (n=959)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.2198 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.111` → IC=+0.155 (n=1035)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 9.111 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` < `0.8795` → IC=+0.187 (n=631)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` < 0.8795 (IC base=+0.139)

- **PATRÓN** `volumen_pendiente_norm` > `0.0691` → IC=+0.165 (n=443)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.0691 (IC base=+0.139)

- **PATRÓN** `volumen_spike_ratio` < `1.418` → IC=+0.143 (n=315)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 1.418 (IC base=+0.139)

- **PATRÓN** `volumen_spike_ratio` > `1.8153` → IC=+0.150 (n=629)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.8153 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `11440.1761` → IC=+0.153 (n=946)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 11440.1761 (IC base=+0.139)

- **PATRÓN** `ballena_activa_n` < `705.0` → IC=+0.146 (n=902)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.163 (n=826)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0073 (IC base=+0.159)

- **PATRÓN** `sigma_h` > `0.0043` → IC=+0.166 (n=938)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` > 0.0043 (IC base=+0.159)

- **PATRÓN** `drift_60min` |x|≤ `0.3873` → IC=+0.162 (n=826)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.3873 (IC base=+0.159)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.177 (n=367)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 17.0 (IC base=+0.159)

- **PATRÓN** `hora_utc` < `4.0` → IC=+0.170 (n=331)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 4.0 (IC base=+0.159)

- **PATRÓN** `ibs_20min` < `0.5623` → IC=+0.164 (n=626)

  - _Acción_: Kelly boost +0.82€ cuando `ibs_20min` < 0.5623 (IC base=+0.159)

- **PATRÓN** `ibs_20min` > `0.7889` → IC=+0.170 (n=425)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` > 0.7889 (IC base=+0.159)

- **PATRÓN** `dist_vwap_pct` > `0.7142` → IC=+0.164 (n=269)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.7142 (IC base=+0.159)

- **PATRÓN** `dist_vwap_pct` < `0.427` → IC=+0.168 (n=875)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` < 0.427 (IC base=+0.159)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.12` → IC=+0.169 (n=842)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` < 4.12 (IC base=+0.159)

- **PATRÓN** `volumen_regimen` < `1.0854` → IC=+0.159 (n=826)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.0854 (IC base=+0.159)

- **PATRÓN** `volumen_regimen` > `0.7178` → IC=+0.162 (n=838)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.7178 (IC base=+0.159)

- **PATRÓN** `volumen_pendiente_norm` < `0.1056` → IC=+0.162 (n=863)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` < 0.1056 (IC base=+0.159)

- **PATRÓN** `volumen_pendiente_norm` > `0.078` → IC=+0.165 (n=404)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.078 (IC base=+0.159)

- **PATRÓN** `volumen_spike_ratio` < `1.4358` → IC=+0.176 (n=307)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 1.4358 (IC base=+0.159)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.165 (n=928)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.159)

- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.183 (n=263)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0041 (IC base=+0.143)

- **PATRÓN** `drift_60min` |x|≤ `0.4985` → IC=+0.170 (n=788)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `10.0` → IC=+0.157 (n=531)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 10.0 (IC base=+0.143)

- **PATRÓN** `ibs_20min` < `0.7466` → IC=+0.147 (n=789)

  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` < 0.7466 (IC base=+0.143)

- **PATRÓN** `ibs_20min` > `0.0954` → IC=+0.148 (n=788)

  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` > 0.0954 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` > `0.6317` → IC=+0.174 (n=170)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.6317 (IC base=+0.143)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.387` → IC=+0.151 (n=714)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` < 4.387 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` < `0.6464` → IC=+0.179 (n=263)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` < 0.6464 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` > `0.7257` → IC=+0.143 (n=704)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 0.7257 (IC base=+0.143)

- **PATRÓN** `volumen_pendiente_norm` > `0.0741` → IC=+0.164 (n=331)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.0741 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` < `2.2019` → IC=+0.155 (n=680)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.2019 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` > `1.7829` → IC=+0.147 (n=514)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.7829 (IC base=+0.143)

- **PATRÓN** `libro_liquidez` > `7572.3913` → IC=+0.170 (n=788)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 7572.3913 (IC base=+0.143)
...
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.194 (n=410)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0041 (IC base=+0.095)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.127 (n=861)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 8.0 (IC base=+0.095)

- **PATRÓN** `ibs_20min` > `0.67` → IC=+0.207 (n=748)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.67 (IC base=+0.095)

- **PATRÓN** `dist_vwap_pct` > `0.1549` → IC=+0.156 (n=457)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.1549 (IC base=+0.095)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.429` → IC=+0.202 (n=196)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.429 (IC base=+0.095)

- **PATRÓN** `volumen_pendiente_norm` > `0.2807` → IC=+0.200 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2807 (IC base=+0.095)

- **PATRÓN** `volumen_spike_ratio` < `2.0912` → IC=+0.134 (n=637)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 2.0912 (IC base=+0.095)

- **PATRÓN** `libro_liquidez` > `2432.8034` → IC=+0.141 (n=369)

  - _Acción_: Kelly boost +0.71€ cuando `libro_liquidez` > 2432.8034 (IC base=+0.095)
...
- **PATRÓN** `sigma_h` < `0.006` → IC=+0.159 (n=318)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` > `0.516` → IC=+0.185 (n=287)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` > 0.516 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` > `0.1379` → IC=+0.173 (n=151)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.1379 (IC base=+0.105)

- **PATRÓN** `volumen_regimen` < `1.0688` → IC=+0.123 (n=253)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_regimen` < 1.0688 (IC base=+0.105)

- **PATRÓN** `volumen_pendiente_norm` < `0.1568` → IC=+0.138 (n=263)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_pendiente_norm` < 0.1568 (IC base=+0.105)

- **PATRÓN** `volumen_spike_ratio` < `2.0105` → IC=+0.173 (n=218)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 2.0105 (IC base=+0.105)
...
- **PATRÓN** `sigma_h` < `0.004` → IC=+0.186 (n=138)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.004 (IC base=+0.105)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.136 (n=295)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 7.0 (IC base=+0.105)

- **PATRÓN** `ibs_20min` > `0.6741` → IC=+0.234 (n=254)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6741 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` > `0.348` → IC=+0.172 (n=114)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.348 (IC base=+0.105)
...
- **PATRÓN** `volumen_regimen` > `0.9419` → IC=+0.141 (n=129)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 0.9419 (IC base=+0.105)

- **PATRÓN** `volumen_pendiente_norm` > `0.2799` → IC=+0.237 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2799 (IC base=+0.105)

- **PATRÓN** `volumen_spike_ratio` < `1.7281` → IC=+0.173 (n=154)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.7281 (IC base=+0.105)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.123 (n=189)

  - _Acción_: Kelly boost +0.62€ cuando `libro_spread` < 0.01 (IC base=+0.105)

- **PATRÓN** `libro_liquidez` > `1133.2011` → IC=+0.157 (n=249)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 1133.2011 (IC base=+0.105)
...
- **PATRÓN** `dist_vwap_pct` > `0.1982` → IC=+0.149 (n=152)
  ...
  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.1982 (IC base=+0.073)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.438` → IC=+0.176 (n=106)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 5.438 (IC base=+0.073)
...
- **FILTRO** `dist_vwap_pct` > `0.2348` → IC=-0.333 (n=16)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.210 (n=188)

- **FILTRO** `volumen_regimen` < `0.7597` → IC=-0.341 (n=67)

  - _Acción_: SKIP cuando `volumen_regimen` < 0.7597
  - _Potencial_: sin este filtro IC_bueno=-0.162 (n=137)
...
### LEADLAG_BTC_XRP_15M
- **PATRÓN** `py_entrada` > `0.5` → IC=+0.120 (n=704)

  - _Acción_: Kelly boost +0.60€ cuando `py_entrada` > 0.5 (IC base=+0.104)

- **PATRÓN** `libro_liquidez` > `2920.5936` → IC=+0.174 (n=237)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 2920.5936 (IC base=+0.104)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=1798)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.222 (n=16)
  ...
  - _Acción_: SKIP cuando `liq_imbalance_15min` |x|≤ 1.0
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=165)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.133 (n=393)
...
- **PATRÓN** `py_entrada` < `0.47` → IC=+0.169 (n=161)
  ...
  - _Acción_: Kelly boost +0.84€ cuando `py_entrada` < 0.47 (IC base=+0.013)
...
- **FILTRO** `lag_apertura_s` > `90.91` → IC=-0.176 (n=32)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.180 (n=48)

  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=25)

### LIQUIDACIONES_DEPTH_FASE0#XRP#5min
- **FILTRO** `py_entrada` < `0.41` → IC=-0.233 (n=43)
...
- **FILTRO** `lag_apertura_s` > `135.61` → IC=-0.180 (n=23)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=7537)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.167 (n=3543)

  - _Acción_: SKIP cuando `py_entrada` < 0.475
  - _Potencial_: sin este filtro IC_bueno=+0.058 (n=10694)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.167 (n=3668)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.036 (n=11076)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.193 (n=620)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=1898)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.202 (n=642)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.064 (n=1998)

- **FILTRO** `ibs_20min` > `0.281` → IC=-0.163 (n=659)
...
- **FILTRO** `py_entrada` < `0.485` → IC=-0.174 (n=605)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.084 (n=1853)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.176 (n=643)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=2000)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=2949)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=2955)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.129 (n=10065)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=22544)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.273 (n=7986)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=24623)

- **FILTRO** `ibs_7min` < `0.283` → IC=-0.235 (n=8149)

  - _Acción_: SKIP cuando `ibs_7min` < 0.283
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=24460)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.156 (n=11071)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=21538)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.230 (n=10010)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=30887)

- **FILTRO** `ibs_7min` > `0.2917` → IC=-0.177 (n=10220)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2917
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=30677)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.136 (n=1647)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3752)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.309 (n=1289)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=4110)

- **FILTRO** `ibs_7min` < `0.7113` → IC=-0.250 (n=1781)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7113
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=3618)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.186 (n=1226)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4173)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.261 (n=1731)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=5299)

- **FILTRO** `drift_7min_pct` |x|> `0.1122` → IC=-0.126 (n=2388)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1122
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=4642)

- **FILTRO** `ibs_7min` > `0.7875` → IC=-0.207 (n=1757)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7875
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=5273)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.135 (n=1322)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=4305)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.251 (n=1361)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=4266)

- **FILTRO** `ibs_7min` < `0.7468` → IC=-0.191 (n=1404)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7468
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4223)

- **FILTRO** `ballena_activa_n` > `161.0` → IC=-0.177 (n=1405)

  - _Acción_: SKIP cuando `ballena_activa_n` > 161.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4222)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.253 (n=1425)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=4286)

- **FILTRO** `ibs_7min` > `0.2602` → IC=-0.177 (n=1427)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2602
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=4284)

- **FILTRO** `ballena_activa_n` > `154.0` → IC=-0.182 (n=1426)

  - _Acción_: SKIP cuando `ballena_activa_n` > 154.0
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=4285)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.166 (n=1258)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.087 (n=3862)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.306 (n=1263)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=3857)

- **FILTRO** `ibs_7min` < `0.7073` → IC=-0.244 (n=1688)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7073
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=3432)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.221 (n=1181)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=3939)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.237 (n=1715)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=5803)

- **FILTRO** `ibs_7min` > `0.7436` → IC=-0.172 (n=1879)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7436
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=5639)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `py_entrada` < `0.35` → IC=-0.240 (n=1327)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=4059)

- **FILTRO** `ibs_7min` < `0.7398` → IC=-0.182 (n=1346)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7398
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=4040)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.170 (n=1317)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=4069)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.259 (n=1368)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=4113)

- **FILTRO** `ibs_7min` > `0.2744` → IC=-0.176 (n=1369)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2744
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=4112)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.184 (n=1351)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4130)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.253 (n=1391)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=4291)

- **FILTRO** `ibs_7min` < `0.3` → IC=-0.233 (n=1407)

  - _Acción_: SKIP cuando `ibs_7min` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4275)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.178 (n=1844)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=5941)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.254 (n=1747)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3648)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.228 (n=1341)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=4054)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.215 (n=1312)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=4083)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.205 (n=1755)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=5617)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.324 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=1090)
...
### ORDER_FLOW_5M
- **PATRÓN** `delta_ratio` |x|> `0.4161` → IC=+0.141 (n=497)
...
- **PATRÓN** `total_vol_5m` < `469.512` → IC=+0.141 (n=249)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `total_vol_5m` < 469.512 (IC base=+0.110)
...
- **FILTRO** `streak_len` > `5.0` → IC=-0.147 (n=15)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.044 (n=178)
...
- **PATRÓN** `streak_estiramiento` < `0.4117` → IC=+0.167 (n=46)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `streak_estiramiento` < 0.4117 (IC base=+0.028)
...
- **FILTRO** `streak_len` > `3.0` → IC=-0.155 (n=27)
  ...
  - _Acción_: SKIP cuando `streak_len` > 3.0
  - _Potencial_: sin este filtro IC_bueno=+0.040 (n=605)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=2930)

### STRUCT_NO_15M#SOL#15min
- **FILTRO** `py_entrada` < `0.495` → IC=-0.147 (n=32)

  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.008 (n=1487)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.154 (n=24)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=1495)

### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.220 (n=736)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0086 (IC base=+0.186)

- **PATRÓN** `drift_60min` |x|≤ `0.0512` → IC=+0.204 (n=542)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0512 (IC base=+0.186)

- **PATRÓN** `delta_ratio_macro` |x|> `0.217` → IC=+0.192 (n=541)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.96€ cuando `delta_ratio_macro` |x|> 0.217 (IC base=+0.186)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1262` → IC=+0.234 (n=570)
...
- **PATRÓN** `ibs_15` > `0.6068` → IC=+0.267 (n=1623)
...
- **PATRÓN** `dist_vwap_pct` < `0.6245` → IC=+0.178 (n=1546)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.6245 (IC base=+0.186)

- **PATRÓN** `sigma_ewma_delta_pct` > `16.749` → IC=+0.282 (n=420)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 16.749 (IC base=+0.186)

- **PATRÓN** `libro_liquidez` > `2972.1985` → IC=+0.196 (n=1082)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.222 (n=16)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=618)
...
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.149 (n=166)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0041 (IC base=+0.136)

- **PATRÓN** `sigma_h` > `0.0051` → IC=+0.158 (n=252)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0051 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.0674` → IC=+0.155 (n=166)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.0674 (IC base=+0.136)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2346` → IC=+0.172 (n=126)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.86€ cuando `delta_ratio_macro` |x|> 0.2346 (IC base=+0.136)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.140 (n=395)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` < 17.0 (IC base=+0.136)

- **PATRÓN** `ibs_15` > `0.659` → IC=+0.253 (n=338)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.659 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `0.1651` → IC=+0.153 (n=295)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.1651 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.918` → IC=+0.205 (n=164)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.918 (IC base=+0.136)
...
- **PATRÓN** `sigma_h` > `0.0236` → IC=+0.257 (n=142)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0236 (IC base=+0.186)

- **PATRÓN** `drift_60min` |x|≤ `0.0864` → IC=+0.212 (n=189)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0864 (IC base=+0.186)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0394` → IC=+0.189 (n=426)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.95€ cuando `delta_ratio_macro` |x|> 0.0394 (IC base=+0.186)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.0883` → IC=+0.250 (n=110)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.0883 (IC base=+0.186)

- **PATRÓN** `hora_utc` < `4.0` → IC=+0.228 (n=145)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 4.0 (IC base=+0.186)

- **PATRÓN** `ibs_15` > `0.5488` → IC=+0.280 (n=426)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5488 (IC base=+0.186)

- **PATRÓN** `dist_vwap_pct` > `0.1329` → IC=+0.208 (n=258)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1329 (IC base=+0.186)

- **PATRÓN** `dist_vwap_pct` < `0.8758` → IC=+0.188 (n=491)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.8758 (IC base=+0.186)

- **PATRÓN** `sigma_ewma_delta_pct` > `19.796` → IC=+0.221 (n=66)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 19.796 (IC base=+0.186)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.743` → IC=+0.187 (n=426)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` < 10.743 (IC base=+0.186)

- **PATRÓN** `libro_liquidez` > `2916.4526` → IC=+0.285 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2916.4526 (IC base=+0.186)

- **PATRÓN** `ibs_15` < `0.1176` → IC=+0.158 (n=481)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.79€ cuando `ibs_15` < 0.1176 (IC base=+0.050)
...
- **FILTRO** `sigma_h` > `0.0129` → IC=-0.220 (n=663)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0129
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=1990)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.194 (n=902)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.006 (n=1751)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1204` → IC=+0.251 (n=1185)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1204 (IC base=-0.032)

- **PATRÓN** `ibs_15` < `0.3455` → IC=+0.280 (n=1778)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3455 (IC base=-0.032)

- **PATRÓN** `dist_vwap_pct` > `0.6985` → IC=+0.303 (n=292)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6985 (IC base=-0.032)

### UPDOWN_GBM_15M_TARDIO#BTC#15min
- **FILTRO** `sigma_h` > `0.0068` → IC=-0.222 (n=404)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0068
  - _Potencial_: sin este filtro IC_bueno=-0.190 (n=1213)

- **FILTRO** `sigma_h` < `0.0034` → IC=-0.232 (n=404)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0034
  - _Potencial_: sin este filtro IC_bueno=-0.186 (n=1213)

- **FILTRO** `hora_utc` > `7.0` → IC=-0.209 (n=1023)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.178 (n=594)

- **FILTRO** `sigma_ewma_delta_pct` > `23.623` → IC=-0.250 (n=226)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 23.623
  - _Potencial_: sin este filtro IC_bueno=-0.189 (n=1391)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4443` → IC=+0.233 (n=697)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4443 (IC base=+0.231)

- **PATRÓN** `drift_15min` |x|≤ `0.48` → IC=+0.238 (n=307)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.48 (IC base=+0.231)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2029` → IC=+0.264 (n=316)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.244 (n=310)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.231)

- **PATRÓN** `ibs_15` < `0.3452` → IC=+0.271 (n=697)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3452 (IC base=+0.231)

- **PATRÓN** `dist_vwap_pct` > `0.7773` → IC=+0.312 (n=94)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7773 (IC base=+0.231)

- **PATRÓN** `sigma_ewma_delta_pct` > `16.687` → IC=+0.260 (n=123)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 16.687 (IC base=+0.231)

- **PATRÓN** `sigma_ewma_delta_pct` < `12.182` → IC=+0.238 (n=740)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 12.182 (IC base=+0.231)

### UPDOWN_GBM_15M_TARDIO#SOL#15min
- **FILTRO** `sigma_h` > `0.0102` → IC=-0.248 (n=157)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0102
  - _Potencial_: sin este filtro IC_bueno=-0.141 (n=472)

- **FILTRO** `drift_60min` |x|> `0.1682` → IC=-0.216 (n=213)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
...
- **FILTRO** `ibs_15` < `0.5625` → IC=-0.140 (n=23)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: SKIP cuando `ibs_15` < 0.5625
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0761` → IC=+0.232 (n=278)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0761 (IC base=-0.041)

- **PATRÓN** `ibs_15` < `0.35` → IC=+0.270 (n=311)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.35 (IC base=-0.041)

- **PATRÓN** `dist_vwap_pct` > `0.4961` → IC=+0.210 (n=91)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4961 (IC base=-0.041)

- **PATRÓN** `dist_vwap_pct` < `0.2005` → IC=+0.228 (n=277)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1333` → IC=+0.287 (n=214)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **PATRÓN** `ibs_15` < `0.3429` → IC=+0.305 (n=471)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3429 (IC base=-0.043)

- **PATRÓN** `dist_vwap_pct` > `0.9431` → IC=+0.362 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9431 (IC base=-0.043)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6068 sube el IC de +0.186 a +0.267 en UPDOWN_GBM#15min (n=1623). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.659 sube el IC de +0.136 a +0.253 en UPDOWN_GBM#ETH#15min (n=338). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_YES, IBS > 0.5488 sube el IC de +0.186 a +0.280 en UPDOWN_GBM#XRP#15min (n=426). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1176 sube el IC de +0.050 a +0.158 en UPDOWN_GBM#XRP#15min (n=481). Ya aplicado como kelly_boost=+0.79€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3455 sube el IC de -0.032 a +0.280 en UPDOWN_GBM_15M_TARDIO (n=1778). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.3452 sube el IC de +0.231 a +0.271 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=697). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#SOL#15min**: dentro de BUY_NO, IBS < 0.35 sube el IC de -0.041 a +0.270 en UPDOWN_GBM_15M_TARDIO#SOL#15min (n=311). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO#XRP#15min**: dentro de BUY_NO, IBS < 0.3429 sube el IC de -0.043 a +0.305 en UPDOWN_GBM_15M_TARDIO#XRP#15min (n=471). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 28361 | -0.090 | -3816.67€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1463 | -0.050 | -220.57€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 26898 | -0.093 | -3596.11€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3716 | -0.094 | -605.49€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3716 | -0.094 | -605.49€ | 1 | 0 |
...
| ✅ BALLENAS_TARDIAS#ETH | 7277 | -0.023 | -677.73€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 7277 | -0.023 | -677.73€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 6846 | -0.095 | -435.37€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 6846 | -0.095 | -435.37€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 5580 | -0.178 | -1086.21€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 5580 | -0.178 | -1086.21€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 18336 | -0.028 | +4029.46€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 4787 | -0.000 | +1819.40€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 13549 | -0.037 | +2210.07€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 18336 | -0.028 | +4029.46€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 4787 | -0.000 | +1819.40€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 13549 | -0.037 | +2210.07€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 93355 | +0.112 | -4756.81€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 14071 | +0.185 | -444.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 374 | -0.082 | -50.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 72846 | +0.100 | -4047.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 6064 | +0.107 | -214.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 12115 | +0.098 | -1025.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min |