# Hipótesis automáticas — 2026-09-24 00:13 UTC
_Generado por shadow_postmortem.py sobre 580857 resoluciones (PNL=+64892.70€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `145.96` → IC=-0.234 (n=6952)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 145.96
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=20864)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `138.82` → IC=-0.241 (n=911)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 138.82
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=2734)
...
- **FILTRO** `restante_s_al_confirmar` < `164.55` → IC=-0.228 (n=1657)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 164.55
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=4972)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.203 (n=13631)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.69 (IC base=+0.101)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.155 (n=3424)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.101)

- **PATRÓN** `libro_liquidez` > `5620.2261` → IC=+0.176 (n=2183)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.136 (n=13476)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` < 7.0 (IC base=+0.127)

- **PATRÓN** `py_entrada` < `0.35` → IC=+0.231 (n=10687)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.35 (IC base=+0.127)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.174 (n=5570)

  - _Acción_: Kelly boost +0.87€ cuando `libro_spread` < 0.01 (IC base=+0.127)

- **PATRÓN** `libro_liquidez` > `7777.02` → IC=+0.173 (n=2098)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 7777.02 (IC base=+0.127)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.205 (n=2081)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.204)

- **PATRÓN** `libro_liquidez` > `15909.0735` → IC=+0.238 (n=537)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 15909.0735 (IC base=+0.204)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.202 (n=2113)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.200)

- **PATRÓN** `libro_liquidez` > `14103.7518` → IC=+0.212 (n=742)
...
- **PATRÓN** `py_entrada` < `0.44` → IC=+0.145 (n=810)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.335 (n=939)
...
- **PATRÓN** `py_entrada` > `0.86` → IC=+0.433 (n=622)
...
- **PATRÓN** `py_entrada` < `0.335` → IC=+0.204 (n=292)
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.184 (n=1901)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.186 (n=2212)
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.198 (n=1919)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.235 (n=15008)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.180 (n=4611)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.225 (n=5951)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.222)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.274 (n=2131)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.75 (IC base=+0.222)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.186 (n=2143)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 18.0 (IC base=+0.172)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.188 (n=6089)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.220 (n=2256)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.220)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.264 (n=2079)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.256 (n=2231)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.195 (n=5540)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.03` → IC=+0.269 (n=548)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.03 (IC base=+0.258)

- **PATRÓN** `libro_liquidez` > `1376.3842` → IC=+0.286 (n=362)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1376.3842 (IC base=+0.258)
...
### GBM_LATE_15M
- **PATRÓN** `drift_60min` |x|≤ `0.4813` → IC=+0.121 (n=7855)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.61€ cuando `drift_60min` |x|≤ 0.4813 (IC base=+0.106)

- **PATRÓN** `ibs_20min` > `0.9823` → IC=+0.245 (n=2618)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9823 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` > `0.8527` → IC=+0.252 (n=465)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8527 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` < `0.3995` → IC=+0.251 (n=1947)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3995 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.965` → IC=+0.177 (n=3016)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 5.965 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` < `1.2107` → IC=+0.248 (n=2109)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3051` → IC=+0.219 (n=781)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3051 (IC base=+0.106)

- **PATRÓN** `volumen_spike_ratio` > `1.9075` → IC=+0.207 (n=3573)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9075 (IC base=+0.106)

- **PATRÓN** `ibs_20min` < `0.57` → IC=+0.133 (n=9483)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.57 (IC base=+0.064)

- **PATRÓN** `dist_vwap_pct` > `0.6175` → IC=+0.192 (n=696)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.6175 (IC base=+0.064)

- **PATRÓN** `dist_vwap_pct` < `0.1526` → IC=+0.173 (n=2986)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.1526 (IC base=+0.064)

- **PATRÓN** `volumen_regimen` < `0.6976` → IC=+0.181 (n=1453)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.6976 (IC base=+0.064)

- **PATRÓN** `volumen_regimen` > `1.0514` → IC=+0.173 (n=1498)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 1.0514 (IC base=+0.064)

- **PATRÓN** `volumen_pendiente_norm` > `0.168` → IC=+0.222 (n=1578)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.168 (IC base=+0.064)

- **PATRÓN** `volumen_spike_ratio` > `1.5718` → IC=+0.201 (n=4949)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5718 (IC base=+0.064)

- **PATRÓN** `ballena_activa_n` < `136.0` → IC=+0.211 (n=5328)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 136.0 (IC base=+0.064)

### GBM_LATE_15M#BNB#15min
- **PATRÓN** `sigma_h` < `0.005` → IC=+0.184 (n=593)
...
- **PATRÓN** `drift_60min` |x|≤ `0.348` → IC=+0.166 (n=1777)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.348 (IC base=+0.162)
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.172 (n=1185)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` < 11.0 (IC base=+0.162)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.270 (n=689)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.122` → IC=+0.272 (n=764)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4363` → IC=+0.163 (n=1660)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.4363 (IC base=+0.162)

- **PATRÓN** `sigma_h` > `0.005` → IC=+0.249 (n=1191)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.0571` → IC=+0.291 (n=587)
...
- **PATRÓN** `volumen_regimen` < `1.2524` → IC=+0.221 (n=1350)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2524 (IC base=+0.217)

- **PATRÓN** `volumen_regimen` > `1.0807` → IC=+0.228 (n=612)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7478` → IC=+0.220 (n=882)
...
- **PATRÓN** `libro_liquidez` > `16596.3663` → IC=+0.224 (n=450)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 16596.3663 (IC base=+0.217)

- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.150 (n=1411)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0056 (IC base=+0.138)

- **PATRÓN** `drift_60min` |x|≤ `0.0755` → IC=+0.161 (n=470)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6918` → IC=+0.170 (n=1408)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` < 0.6918 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.1332` → IC=+0.156 (n=1254)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.243` → IC=+0.140 (n=1282)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` < 4.243 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `1.204` → IC=+0.150 (n=1408)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4221` → IC=+0.144 (n=1297)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.4221 (IC base=+0.138)

- **PATRÓN** `ballena_activa_n` < `411.0` → IC=+0.147 (n=1215)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.263 (n=689)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2114` → IC=+0.189 (n=1733)
...
- **PATRÓN** `volumen_spike_ratio` > `2.847` → IC=+0.207 (n=748)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.847 (IC base=+0.187)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.194 (n=1241)

  - _Acción_: Kelly boost +0.97€ cuando `libro_spread` < 0.02 (IC base=+0.187)

- **PATRÓN** `sigma_h` < `0.0115` → IC=+0.223 (n=1491)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0115 (IC base=+0.214)

- **PATRÓN** `drift_60min` |x|≤ `0.5884` → IC=+0.216 (n=1491)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.0625` → IC=+0.245 (n=656)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0625 (IC base=+0.214)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.615` → IC=+0.248 (n=509)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.160 (n=98)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.024 (n=2175)
...
- **PATRÓN** `volumen_regimen` < `0.8475` → IC=+0.165 (n=541)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.8475 (IC base=+0.016)

- **PATRÓN** `volumen_pendiente_norm` > `0.2269` → IC=+0.218 (n=129)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2269 (IC base=+0.016)

- **PATRÓN** `volumen_spike_ratio` > `1.5223` → IC=+0.177 (n=677)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 1.5223 (IC base=+0.016)
...
- **FILTRO** `ibs_20min` > `0.2571` → IC=-0.124 (n=2139)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2571
  - _Potencial_: sin este filtro IC_bueno=+0.125 (n=1059)

- **FILTRO** `sigma_ewma_delta_pct` > `8.651` → IC=-0.205 (n=344)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.651
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=2854)
...
- **PATRÓN** `ibs_20min` < `0.2571` → IC=+0.125 (n=1059)
...
- **FILTRO** `drift_60min` |x|> `0.6615` → IC=-0.180 (n=551)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6615
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=1656)

- **FILTRO** `ibs_20min` < `0.7098` → IC=-0.156 (n=1456)
...
- **FILTRO** `libro_spread` > `0.02` → IC=-0.199 (n=406)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=1801)

- **FILTRO** `ibs_20min` > `0.77` → IC=-0.202 (n=812)

  - _Acción_: SKIP cuando `ibs_20min` > 0.77
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=2442)

- **PATRÓN** `dist_vwap_pct` > `0.7963` → IC=+0.320 (n=87)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7963 (IC base=-0.066)

- **PATRÓN** `dist_vwap_pct` < `0.2704` → IC=+0.323 (n=275)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2704 (IC base=-0.066)

- **PATRÓN** `volumen_regimen` < `0.9865` → IC=+0.297 (n=293)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9865 (IC base=-0.066)

- **PATRÓN** `volumen_regimen` > `0.616` → IC=+0.312 (n=333)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.616 (IC base=-0.066)

- **PATRÓN** `volumen_pendiente_norm` < `0.0995` → IC=+0.299 (n=302)
...
- **PATRÓN** `volumen_spike_ratio` > `1.7999` → IC=+0.301 (n=209)
...
- **PATRÓN** `volumen_regimen` > `1.0826` → IC=+0.277 (n=335)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1667` → IC=+0.253 (n=552)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1667 (IC base=-0.025)
...
- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.244 (n=643)
...
- **PATRÓN** `ibs_20min` > `0.4738` → IC=+0.188 (n=8786)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.4738 (IC base=+0.098)

- **PATRÓN** `dist_vwap_pct` > `0.7795` → IC=+0.289 (n=1034)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7795 (IC base=+0.098)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.588` → IC=+0.156 (n=4649)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 3.588 (IC base=+0.098)

- **PATRÓN** `volumen_regimen` > `0.688` → IC=+0.249 (n=3112)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.688 (IC base=+0.098)

- **PATRÓN** `volumen_pendiente_norm` > `0.297` → IC=+0.266 (n=827)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.297 (IC base=+0.098)

- **PATRÓN** `volumen_spike_ratio` < `1.4704` → IC=+0.242 (n=1885)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4704 (IC base=+0.098)

- **PATRÓN** `volumen_spike_ratio` > `2.68` → IC=+0.238 (n=1886)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.68 (IC base=+0.098)

- **PATRÓN** `ballena_activa_n` < `98.0` → IC=+0.268 (n=5175)
...
- **PATRÓN** `ibs_20min` < `0.5474` → IC=+0.152 (n=8655)

  - _Acción_: Kelly boost +0.76€ cuando `ibs_20min` < 0.5474 (IC base=+0.073)

- **PATRÓN** `dist_vwap_pct` < `0.2498` → IC=+0.238 (n=2734)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2498 (IC base=+0.073)

- **PATRÓN** `volumen_regimen` < `0.7084` → IC=+0.239 (n=1272)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7084 (IC base=+0.073)

- **PATRÓN** `volumen_regimen` > `1.2035` → IC=+0.245 (n=963)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2035 (IC base=+0.073)

- **PATRÓN** `volumen_pendiente_norm` > `0.2454` → IC=+0.300 (n=723)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2454 (IC base=+0.073)

- **PATRÓN** `volumen_spike_ratio` < `1.6086` → IC=+0.254 (n=1669)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6086 (IC base=+0.073)

- **PATRÓN** `volumen_spike_ratio` > `2.3262` → IC=+0.256 (n=1721)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3262 (IC base=+0.073)

- **PATRÓN** `ballena_activa_n` < `83.0` → IC=+0.262 (n=3669)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 83.0 (IC base=+0.073)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `sigma_ewma_delta_pct` > `4.511` → IC=-0.164 (n=516)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.511
  - _Potencial_: sin este filtro IC_bueno=+0.025 (n=1736)

- **PATRÓN** `ibs_20min` > `0.8893` → IC=+0.268 (n=679)
...
- **PATRÓN** `volumen_spike_ratio` < `1.44` → IC=+0.181 (n=283)
...
- **PATRÓN** `dist_vwap_pct` > `0.3125` → IC=+0.178 (n=364)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.3125 (IC base=+0.026)

- **PATRÓN** `volumen_regimen` > `0.6717` → IC=+0.165 (n=804)
...
- **PATRÓN** `ballena_activa_n` < `247.0` → IC=+0.194 (n=384)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.300 (n=827)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1373` → IC=+0.258 (n=1380)
...
- **PATRÓN** `ibs_20min` < `0.2248` → IC=+0.288 (n=1091)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2248 (IC base=+0.282)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.841` → IC=+0.300 (n=489)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.841 (IC base=+0.282)

- **PATRÓN** `volumen_pendiente_norm` > `0.3431` → IC=+0.308 (n=191)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3431 (IC base=+0.282)

- **PATRÓN** `volumen_spike_ratio` < `1.6001` → IC=+0.286 (n=382)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6001 (IC base=+0.282)

- **PATRÓN** `volumen_spike_ratio` > `2.7699` → IC=+0.285 (n=519)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7699 (IC base=+0.282)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.290 (n=836)
...
- **FILTRO** `ibs_20min` < `0.2829` → IC=-0.190 (n=478)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2829
  - _Potencial_: sin este filtro IC_bueno=+0.075 (n=1439)

- **FILTRO** `ibs_20min` > `0.7739` → IC=-0.180 (n=583)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7739
  - _Potencial_: sin este filtro IC_bueno=+0.052 (n=1750)

- **PATRÓN** `ibs_20min` > `0.9101` → IC=+0.178 (n=480)
...
- **PATRÓN** `volumen_regimen` < `0.9869` → IC=+0.234 (n=476)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9869 (IC base=+0.009)

- **PATRÓN** `volumen_regimen` > `0.5819` → IC=+0.209 (n=541)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0972` → IC=+0.245 (n=449)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0972 (IC base=+0.009)

- **PATRÓN** `ballena_activa_n` < `101.0` → IC=+0.251 (n=348)
...
- **PATRÓN** `dist_vwap_pct` < `0.6907` → IC=+0.196 (n=475)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` < 0.6907 (IC base=-0.006)

- **PATRÓN** `volumen_regimen` < `1.1643` → IC=+0.207 (n=411)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8289` → IC=+0.257 (n=245)
...
- **PATRÓN** `ballena_activa_n` < `139.0` → IC=+0.244 (n=373)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 139.0 (IC base=-0.006)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **FILTRO** `ibs_20min` < `0.7188` → IC=-0.200 (n=1041)
...
- **FILTRO** `ibs_20min` > `0.6875` → IC=-0.230 (n=542)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.095 (n=1646)

- **FILTRO** `sigma_ewma_delta_pct` > `4.678` → IC=-0.176 (n=477)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.678
  - _Potencial_: sin este filtro IC_bueno=+0.068 (n=1711)
...
- **PATRÓN** `dist_vwap_pct` < `0.2141` → IC=+0.216 (n=481)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0994` → IC=+0.207 (n=506)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4893` → IC=+0.216 (n=513)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.4893 (IC base=+0.015)

- **PATRÓN** `ballena_activa_n` < `51.0` → IC=+0.234 (n=460)
...
- **PATRÓN** `ibs_20min` > `0.9111` → IC=+0.347 (n=853)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9111 (IC base=+0.279)

- **PATRÓN** `dist_vwap_pct` > `0.2079` → IC=+0.319 (n=759)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2079 (IC base=+0.279)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.536` → IC=+0.304 (n=676)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.536 (IC base=+0.279)

- **PATRÓN** `volumen_regimen` > `0.8622` → IC=+0.303 (n=853)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5119` → IC=+0.293 (n=404)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5119 (IC base=+0.279)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.285 (n=1366)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.279)

- **PATRÓN** `libro_liquidez` > `2621.5822` → IC=+0.295 (n=853)
...
- **PATRÓN** `ibs_20min` > `0.5781` → IC=+0.217 (n=7671)
...
- **PATRÓN** `volumen_regimen` < `1.2134` → IC=+0.160 (n=5075)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2134 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` > `0.6282` → IC=+0.161 (n=5075)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.6282 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.2963` → IC=+0.195 (n=1146)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.2963 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` < `1.5622` → IC=+0.172 (n=3236)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.5622 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.6248` → IC=+0.175 (n=2451)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.6248 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `2400.6554` → IC=+0.169 (n=5114)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2400.6554 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `116.0` → IC=+0.181 (n=6579)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0805` → IC=+0.209 (n=2451)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4773` → IC=+0.226 (n=7347)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4773 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` > `0.9207` → IC=+0.145 (n=710)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.9207 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` < `0.2329` → IC=+0.159 (n=5338)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.2329 (IC base=+0.168)
...
- **PATRÓN** `volumen_regimen` < `1.1735` → IC=+0.153 (n=5329)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1735 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.2916` → IC=+0.223 (n=1054)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2916 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` < `1.5637` → IC=+0.168 (n=2934)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.5637 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.2571` → IC=+0.170 (n=3023)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.2571 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `117.0` → IC=+0.173 (n=6310)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1882` → IC=+0.293 (n=635)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.3443` → IC=+0.263 (n=952)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3443 (IC base=+0.238)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.234` → IC=+0.251 (n=1026)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.234 (IC base=+0.238)

- **PATRÓN** `volumen_pendiente_norm` < `0.0963` → IC=+0.233 (n=789)
...
- **PATRÓN** `libro_liquidez` > `1801.66` → IC=+0.245 (n=634)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1801.66 (IC base=+0.238)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.158 (n=1249)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0057 (IC base=+0.137)

- **PATRÓN** `drift_60min` |x|≤ `0.291` → IC=+0.160 (n=1249)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5646` → IC=+0.185 (n=1249)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` < 0.5646 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.1347` → IC=+0.161 (n=1235)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.1347 (IC base=+0.137)
...
- **PATRÓN** `volumen_regimen` < `1.2096` → IC=+0.157 (n=1249)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.2096 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` > `0.1575` → IC=+0.154 (n=380)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_pendiente_norm` > 0.1575 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` < `2.4266` → IC=+0.146 (n=1138)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 2.4266 (IC base=+0.137)
...
- **PATRÓN** `sigma_h` > `0.0099` → IC=+0.217 (n=584)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0099 (IC base=+0.198)

- **PATRÓN** `drift_60min` |x|≤ `0.2255` → IC=+0.215 (n=859)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.292 (n=685)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.198)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.851` → IC=+0.276 (n=400)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.851 (IC base=+0.198)

- **PATRÓN** `volumen_pendiente_norm` > `0.1328` → IC=+0.199 (n=503)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1328 (IC base=+0.198)

- **PATRÓN** `volumen_spike_ratio` < `1.6421` → IC=+0.199 (n=407)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6421 (IC base=+0.198)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.208 (n=912)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.198)

- **PATRÓN** `libro_liquidez` > `1953.345` → IC=+0.201 (n=429)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1953.345 (IC base=+0.198)

- **PATRÓN** `sigma_h` < `0.0113` → IC=+0.232 (n=1068)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0113 (IC base=+0.220)

- **PATRÓN** `drift_60min` |x|≤ `0.0967` → IC=+0.255 (n=357)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.2417` → IC=+0.257 (n=939)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7801` → IC=+0.220 (n=434)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7801 (IC base=+0.220)

- **PATRÓN** `volumen_spike_ratio` > `3.417` → IC=+0.231 (n=329)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.417 (IC base=+0.220)

- **PATRÓN** `libro_liquidez` > `1881.693` → IC=+0.224 (n=484)
...
- **PATRÓN** `sigma_h` < `0.0036` → IC=+0.208 (n=409)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0036 (IC base=+0.143)

- **PATRÓN** `drift_60min` |x|≤ `0.4324` → IC=+0.158 (n=1225)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.379` → IC=+0.196 (n=1225)
...
- **PATRÓN** `volumen_regimen` < `1.0405` → IC=+0.144 (n=1078)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.0405 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` > `0.6274` → IC=+0.148 (n=1225)
...
- **PATRÓN** `ibs_20min` < `0.635` → IC=+0.171 (n=1297)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` < 0.635 (IC base=+0.123)

- **PATRÓN** `dist_vwap_pct` < `0.5895` → IC=+0.133 (n=1505)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.5895 (IC base=+0.123)
...
- **PATRÓN** `volumen_regimen` < `0.8475` → IC=+0.148 (n=865)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7906` → IC=+0.134 (n=785)
...
- **PATRÓN** `libro_liquidez` > `10007.3401` → IC=+0.159 (n=588)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1031` → IC=+0.155 (n=470)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5652` → IC=+0.210 (n=1410)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5652 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `1.0043` → IC=+0.142 (n=185)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 1.0043 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.1993` → IC=+0.138 (n=1298)
...
- **PATRÓN** `volumen_regimen` < `1.1736` → IC=+0.122 (n=1410)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4572` → IC=+0.129 (n=421)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` < 1.4572 (IC base=+0.113)
...
- **PATRÓN** `sigma_h` > `0.0188` → IC=+0.214 (n=883)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0188 (IC base=+0.203)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.210 (n=487)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.7381` → IC=+0.262 (n=1184)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7381 (IC base=+0.203)

- **PATRÓN** `dist_vwap_pct` > `0.506` → IC=+0.222 (n=632)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.506 (IC base=+0.203)
...
- **PATRÓN** `volumen_regimen` < `1.2084` → IC=+0.205 (n=1325)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2084 (IC base=+0.203)

- **PATRÓN** `volumen_regimen` > `0.8589` → IC=+0.222 (n=883)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8589 (IC base=+0.203)

- **PATRÓN** `volumen_pendiente_norm` > `0.2335` → IC=+0.269 (n=253)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2335 (IC base=+0.203)

- **PATRÓN** `volumen_spike_ratio` < `2.1563` → IC=+0.216 (n=1126)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1563 (IC base=+0.203)

- **PATRÓN** `volumen_spike_ratio` > `1.4095` → IC=+0.208 (n=1279)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4095 (IC base=+0.203)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.206 (n=1407)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.203)

- **PATRÓN** `libro_liquidez` > `2612.836` → IC=+0.205 (n=883)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0896` → IC=+0.222 (n=459)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.44` → IC=+0.245 (n=1381)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.44 (IC base=+0.203)

- **PATRÓN** `dist_vwap_pct` > `1.2367` → IC=+0.221 (n=163)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2367 (IC base=+0.203)

- **PATRÓN** `dist_vwap_pct` < `0.9121` → IC=+0.203 (n=1605)
...
- **PATRÓN** `volumen_regimen` > `0.6289` → IC=+0.215 (n=1374)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2184` → IC=+0.194 (n=1085)
...
- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.166 (n=605)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.0997` → IC=+0.153 (n=604)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.5494` → IC=+0.188 (n=1615)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.5494 (IC base=+0.148)
...
- **PATRÓN** `volumen_regimen` < `0.8735` → IC=+0.165 (n=1055)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.8735 (IC base=+0.148)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4378` → IC=+0.169 (n=581)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.4378 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` > `2.5493` → IC=+0.158 (n=580)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 2.5493 (IC base=+0.148)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.152 (n=2048)
...
- **PATRÓN** `ballena_activa_n` < `163.0` → IC=+0.168 (n=1586)

  - _Acción_: Kelly boost +0.84€ cuando `ballena_activa_n` < 163.0 (IC base=+0.148)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.135 (n=1275)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.5217` → IC=+0.143 (n=1677)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` < 0.5217 (IC base=+0.106)

- **PATRÓN** `volumen_spike_ratio` < `2.2282` → IC=+0.123 (n=1613)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_spike_ratio` < 2.2282 (IC base=+0.106)

- **PATRÓN** `libro_liquidez` > `3866.7412` → IC=+0.124 (n=1270)

  - _Acción_: Kelly boost +0.62€ cuando `libro_liquidez` > 3866.7412 (IC base=+0.106)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2746` → IC=+0.157 (n=535)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6039` → IC=+0.178 (n=535)

  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` < 0.6039 (IC base=+0.132)

- **PATRÓN** `dist_vwap_pct` < `0.31` → IC=+0.150 (n=646)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.31 (IC base=+0.132)
...
- **PATRÓN** `volumen_regimen` > `0.7161` → IC=+0.148 (n=543)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` > 0.7161 (IC base=+0.132)
...
- **PATRÓN** `volumen_spike_ratio` > `1.415` → IC=+0.143 (n=598)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.415 (IC base=+0.132)

- **PATRÓN** `ballena_activa_n` < `387.0` → IC=+0.144 (n=576)

  - _Acción_: Kelly boost +0.72€ cuando `ballena_activa_n` < 387.0 (IC base=+0.132)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0967` → IC=+0.213 (n=186)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.948` → IC=+0.220 (n=244)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.948 (IC base=+0.193)

- **PATRÓN** `volumen_regimen` < `0.8358` → IC=+0.201 (n=372)
...
- **PATRÓN** `ibs_20min` > `0.7333` → IC=+0.205 (n=354)
...
- **PATRÓN** `volumen_regimen` < `1.0699` → IC=+0.149 (n=348)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 1.0699 (IC base=+0.133)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4844` → IC=+0.141 (n=126)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 1.4844 (IC base=+0.133)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.135 (n=431)
...
- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.216 (n=9807)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4706 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` > `0.9838` → IC=+0.206 (n=1372)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9838 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.613` → IC=+0.227 (n=4745)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.613 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` < `0.8808` → IC=+0.165 (n=4367)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.8808 (IC base=+0.169)

- **PATRÓN** `volumen_pendiente_norm` > `0.2398` → IC=+0.201 (n=1834)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2398 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `2.6028` → IC=+0.188 (n=3138)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 2.6028 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `2342.7057` → IC=+0.172 (n=6533)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 2342.7057 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `87.0` → IC=+0.195 (n=7422)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 87.0 (IC base=+0.169)

- **PATRÓN** `sigma_h` < `0.007` → IC=+0.191 (n=5942)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.007 (IC base=+0.180)

- **PATRÓN** `drift_60min` |x|≤ `0.1472` → IC=+0.189 (n=3920)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5667` → IC=+0.237 (n=8908)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5667 (IC base=+0.180)

- **PATRÓN** `dist_vwap_pct` < `0.2492` → IC=+0.161 (n=5527)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2492 (IC base=+0.180)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.007` → IC=+0.203 (n=1250)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.007 (IC base=+0.180)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.72` → IC=+0.181 (n=8625)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.72 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` < `0.7035` → IC=+0.161 (n=2697)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.7035 (IC base=+0.180)

- **PATRÓN** `volumen_pendiente_norm` > `0.2881` → IC=+0.241 (n=1166)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2881 (IC base=+0.180)

- **PATRÓN** `volumen_spike_ratio` > `2.6332` → IC=+0.192 (n=2718)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 2.6332 (IC base=+0.180)

- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.192 (n=5214)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.05` → IC=+0.314 (n=741)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.05 (IC base=+0.190)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5518` → IC=+0.183 (n=692)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 1.5518 (IC base=+0.190)
...
- **PATRÓN** `ibs_20min` < `0.3485` → IC=+0.289 (n=1142)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3485 (IC base=+0.258)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.466` → IC=+0.262 (n=1357)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8682` → IC=+0.272 (n=792)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8682 (IC base=+0.258)

- **PATRÓN** `libro_liquidez` > `1799.51` → IC=+0.262 (n=865)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1799.51 (IC base=+0.258)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0841` → IC=+0.161 (n=523)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.3107` → IC=+0.203 (n=1569)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.756` → IC=+0.173 (n=359)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 9.756 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.234` → IC=+0.154 (n=1410)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 4.234 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` < `0.629` → IC=+0.178 (n=523)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1137` → IC=+0.163 (n=1332)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.1137 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.7583` → IC=+0.156 (n=1009)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.7583 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `15679.6604` → IC=+0.156 (n=711)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 15679.6604 (IC base=+0.150)

- **PATRÓN** `ballena_activa_n` < `477.0` → IC=+0.160 (n=1444)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 477.0 (IC base=+0.150)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.165 (n=1358)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0057 (IC base=+0.150)

- **PATRÓN** `drift_60min` |x|≤ `0.3224` → IC=+0.162 (n=1358)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.269` → IC=+0.235 (n=906)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.269 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` < `0.1371` → IC=+0.166 (n=1225)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.1371 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.186` → IC=+0.157 (n=648)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 3.186 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` < `1.1862` → IC=+0.163 (n=1358)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4003` → IC=+0.159 (n=1260)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.4003 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `2.0873` → IC=+0.161 (n=571)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 2.0873 (IC base=+0.150)

- **PATRÓN** `ballena_activa_n` < `424.0` → IC=+0.158 (n=1023)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 424.0 (IC base=+0.150)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0118` → IC=+0.244 (n=530)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.6694` → IC=+0.256 (n=1417)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6694 (IC base=+0.218)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.212` → IC=+0.222 (n=1564)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.229 (n=1131)
...
- **PATRÓN** `ibs_20min` < `0.3611` → IC=+0.270 (n=1301)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3611 (IC base=+0.234)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.751` → IC=+0.280 (n=540)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1905` → IC=+0.237 (n=902)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1905 (IC base=+0.234)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.244 (n=1003)
...
- **PATRÓN** `sigma_h` < `0.0035` → IC=+0.186 (n=558)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.0035 (IC base=+0.135)

- **PATRÓN** `drift_60min` |x|≤ `0.4373` → IC=+0.143 (n=1671)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `dist_vwap_pct` > `0.3757` → IC=+0.163 (n=659)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.3757 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.171` → IC=+0.159 (n=693)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 4.171 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` < `0.8743` → IC=+0.153 (n=1114)
...
- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.156 (n=1367)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0076 (IC base=+0.137)

- **PATRÓN** `drift_60min` |x|≤ `0.4461` → IC=+0.154 (n=1366)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.7012` → IC=+0.181 (n=1366)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` < 0.7012 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.6035` → IC=+0.143 (n=1512)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.6035 (IC base=+0.137)
...
- **PATRÓN** `volumen_regimen` < `0.8615` → IC=+0.147 (n=912)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4471` → IC=+0.151 (n=1294)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.4471 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `10917.9028` → IC=+0.207 (n=456)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 10917.9028 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `179.0` → IC=+0.145 (n=1290)

  - _Acción_: Kelly boost +0.72€ cuando `ballena_activa_n` < 179.0 (IC base=+0.137)

### GBM_LATE_15M_TARDIO#SOL#15min
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.136 (n=1104)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.190 (n=1657)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` > 0.4706 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` > `1.0829` → IC=+0.204 (n=329)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0829 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.442` → IC=+0.234 (n=621)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.442 (IC base=+0.111)

- **PATRÓN** `volumen_regimen` < `0.8906` → IC=+0.134 (n=1103)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 0.8906 (IC base=+0.111)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.125 (n=1668)
...
- **PATRÓN** `ibs_20min` < `0.6364` → IC=+0.203 (n=1628)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6364 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` < `0.219` → IC=+0.128 (n=1326)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.219 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.449` → IC=+0.123 (n=1573)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 3.449 (IC base=+0.111)

- **PATRÓN** `volumen_regimen` < `0.7164` → IC=+0.151 (n=715)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.7164 (IC base=+0.111)

- **PATRÓN** `volumen_pendiente_norm` > `0.2248` → IC=+0.169 (n=252)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` > 0.2248 (IC base=+0.111)

- **PATRÓN** `volumen_spike_ratio` < `1.4488` → IC=+0.137 (n=488)
...
- **PATRÓN** `sigma_h` > `0.0193` → IC=+0.221 (n=1106)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.5143` → IC=+0.251 (n=1661)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5143 (IC base=+0.211)

- **PATRÓN** `dist_vwap_pct` > `0.2021` → IC=+0.235 (n=978)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2021 (IC base=+0.211)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.285` → IC=+0.271 (n=295)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.285 (IC base=+0.211)

- **PATRÓN** `volumen_regimen` < `1.0683` → IC=+0.212 (n=1460)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.0683 (IC base=+0.211)

- **PATRÓN** `volumen_regimen` > `0.6352` → IC=+0.218 (n=1659)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6352 (IC base=+0.211)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5103` → IC=+0.237 (n=534)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5103 (IC base=+0.211)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.220 (n=1740)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.211)

- **PATRÓN** `libro_liquidez` > `2617.2954` → IC=+0.225 (n=1106)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2617.2954 (IC base=+0.211)
...
- **PATRÓN** `ibs_20min` < `0.5188` → IC=+0.255 (n=1781)
...
- **PATRÓN** `dist_vwap_pct` < `0.9192` → IC=+0.203 (n=1982)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.9192 (IC base=+0.200)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.808` → IC=+0.262 (n=250)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.808 (IC base=+0.200)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2045` → IC=+0.195 (n=1406)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.2045 (IC base=+0.200)

- **PATRÓN** `volumen_spike_ratio` > `1.4328` → IC=+0.196 (n=1597)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.4328 (IC base=+0.200)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.138 (n=2953)

- **PATRÓN** `sigma_h` < `0.0093` → IC=+0.159 (n=2481)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.5227` → IC=+0.160 (n=2820)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.9429` → IC=+0.211 (n=940)
...
- **PATRÓN** `dist_vwap_pct` < `0.5007` → IC=+0.143 (n=1697)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.5007 (IC base=+0.150)
...
- **PATRÓN** `volumen_regimen` > `0.9007` → IC=+0.161 (n=1206)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.9007 (IC base=+0.150)
...
- **PATRÓN** `ibs_20min` < `0.0862` → IC=+0.170 (n=743)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` < 0.0862 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.9229` → IC=+0.149 (n=337)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.9229 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.224` → IC=+0.145 (n=2217)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 6.224 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `1.2455` → IC=+0.141 (n=2125)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 1.2455 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.0724` → IC=+0.144 (n=1032)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8153` → IC=+0.142 (n=1465)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 1.8153 (IC base=+0.136)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.138 (n=2953)

  - _Acción_: Kelly boost +0.69€ cuando `libro_spread` < 0.01 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `6989.8968` → IC=+0.150 (n=1989)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 6989.8968 (IC base=+0.136)
...
- **PATRÓN** `sigma_h` > `0.0044` → IC=+0.161 (n=868)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` > 0.0044 (IC base=+0.153)

- **PATRÓN** `drift_60min` |x|≤ `0.1253` → IC=+0.158 (n=290)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.1253 (IC base=+0.153)
...
- **PATRÓN** `ibs_20min` < `0.5623` → IC=+0.159 (n=579)
  ...
  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` < 0.5623 (IC base=+0.153)

- **PATRÓN** `ibs_20min` > `0.8892` → IC=+0.168 (n=290)
...
- **PATRÓN** `dist_vwap_pct` < `0.4271` → IC=+0.167 (n=805)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.4271 (IC base=+0.153)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.678` → IC=+0.161 (n=871)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` < 6.678 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` < `0.6357` → IC=+0.154 (n=290)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.6357 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` > `0.7179` → IC=+0.157 (n=776)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 0.7179 (IC base=+0.153)

- **PATRÓN** `volumen_pendiente_norm` < `0.1108` → IC=+0.156 (n=801)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1721` → IC=+0.160 (n=254)
  ...
  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` > 0.1721 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` < `1.4349` → IC=+0.164 (n=284)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.4349 (IC base=+0.153)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.159 (n=849)

  - _Acción_: Kelly boost +0.80€ cuando `libro_spread` < 0.01 (IC base=+0.153)

- **PATRÓN** `libro_liquidez` > `9293.3689` → IC=+0.154 (n=579)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 9293.3689 (IC base=+0.153)

- **PATRÓN** `sigma_h` < `0.0084` → IC=+0.154 (n=735)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0084 (IC base=+0.138)

- **PATRÓN** `drift_60min` |x|≤ `0.3902` → IC=+0.171 (n=645)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.3902 (IC base=+0.138)
...
- **PATRÓN** `ibs_20min` < `0.7466` → IC=+0.141 (n=734)

  - _Acción_: Kelly boost +0.71€ cuando `ibs_20min` < 0.7466 (IC base=+0.138)

- **PATRÓN** `ibs_20min` > `0.103` → IC=+0.150 (n=733)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` > 0.103 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` > `0.6524` → IC=+0.169 (n=164)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` > 0.6524 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.3876` → IC=+0.139 (n=743)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.3876 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.738` → IC=+0.156 (n=120)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 10.738 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.34` → IC=+0.141 (n=666)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` < 4.34 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `0.6434` → IC=+0.172 (n=245)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 0.6434 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `0.7257` → IC=+0.139 (n=655)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 0.7257 (IC base=+0.138)

- **PATRÓN** `volumen_pendiente_norm` > `0.0735` → IC=+0.166 (n=309)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.0735 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` < `2.2019` → IC=+0.149 (n=633)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 2.2019 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` > `1.783` → IC=+0.149 (n=479)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.783 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `7592.1772` → IC=+0.163 (n=733)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 7592.1772 (IC base=+0.138)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.129 (n=833)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 8.0 (IC base=+0.098)

- **PATRÓN** `ibs_20min` > `0.6778` → IC=+0.205 (n=713)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6778 (IC base=+0.098)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2822` → IC=+0.202 (n=102)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2822 (IC base=+0.098)
...
- **PATRÓN** `py_entrada` < `0.48` → IC=+0.170 (n=110)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `py_entrada` < 0.48 (IC base=+0.029)

- **PATRÓN** `profundidad_ratio` > `103.8` → IC=+0.173 (n=102)

  - _Acción_: Kelly boost +0.87€ cuando `profundidad_ratio` > 103.8 (IC base=+0.029)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=7442)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.173 (n=3256)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=10524)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.169 (n=3566)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=10706)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.199 (n=590)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=1841)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.203 (n=625)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.062 (n=1928)

- **FILTRO** `ibs_20min` > `0.2812` → IC=-0.163 (n=638)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2812
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=1915)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.485` → IC=-0.170 (n=589)

  - _Acción_: SKIP cuando `py_entrada` < 0.485
  - _Potencial_: sin este filtro IC_bueno=+0.085 (n=1783)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.182 (n=624)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=1929)

### MOMENTUM_IBS_15M_FADE
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=2736)

- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)

  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=2912)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=2918)
...
- **FILTRO** `libro_liquidez` < `17176.9722` → IC=-0.124 (n=163)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=9690)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=21964)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.273 (n=7724)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=23930)

- **FILTRO** `ibs_7min` < `0.2857` → IC=-0.235 (n=7900)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2857
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=23754)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.158 (n=10467)

  - _Acción_: SKIP cuando `ballena_activa_n` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=21187)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.229 (n=9733)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=29867)

- **FILTRO** `ibs_7min` > `0.2934` → IC=-0.179 (n=9897)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2934
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=29703)
...
- **FILTRO** `ibs_7min` < `0.7104` → IC=-0.250 (n=1725)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7104
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3505)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.185 (n=1205)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=4025)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.259 (n=1681)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=5121)

- **FILTRO** `drift_7min_pct` |x|> `0.1117` → IC=-0.124 (n=2312)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1117
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=4490)

- **FILTRO** `ibs_7min` > `0.7895` → IC=-0.204 (n=1700)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7895
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=5102)
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.249 (n=1313)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=4154)

- **FILTRO** `ibs_7min` < `0.7482` → IC=-0.191 (n=1363)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7482
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=4104)

- **FILTRO** `ballena_activa_n` > `160.0` → IC=-0.171 (n=1366)

  - _Acción_: SKIP cuando `ballena_activa_n` > 160.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=4101)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.254 (n=1379)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=4161)

- **FILTRO** `ibs_7min` > `0.2604` → IC=-0.176 (n=1384)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2604
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=4156)

- **FILTRO** `ballena_activa_n` > `154.0` → IC=-0.183 (n=1380)

  - _Acción_: SKIP cuando `ballena_activa_n` > 154.0
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=4160)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.166 (n=1212)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=3745)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.307 (n=1212)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=3745)

- **FILTRO** `ibs_7min` < `0.1944` → IC=-0.257 (n=1239)
...
- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.219 (n=1160)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3797)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.238 (n=1667)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=5608)

- **FILTRO** `ibs_7min` > `0.7474` → IC=-0.177 (n=1818)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7474
  - _Potencial_: sin este filtro IC_bueno=+0.005 (n=5457)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `py_entrada` < `0.37` → IC=-0.231 (n=1542)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=3678)

- **FILTRO** `ibs_7min` < `0.7412` → IC=-0.181 (n=1305)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7412
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=3915)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.169 (n=1289)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3931)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.267 (n=1176)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=4138)

- **FILTRO** `ibs_7min` > `0.2747` → IC=-0.176 (n=1328)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2747
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=3986)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.185 (n=1323)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=3991)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.260 (n=1340)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=4187)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.240 (n=1376)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=4151)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.178 (n=1802)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=5731)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.254 (n=1694)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=3559)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.227 (n=1300)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=3953)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.213 (n=1279)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=3974)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.204 (n=1713)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.010 (n=5423)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.129 (n=33)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=1061)
...
- **FILTRO** `pct_vs_K` |x|> `2.8565` → IC=-0.427 (n=39)

  - _Acción_: SKIP cuando `pct_vs_K` |x|> 2.8565
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.190 (n=27)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=755)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.152 (n=21)

  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=761)
...
- **FILTRO** `py_entrada` < `0.495` → IC=-0.147 (n=32)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.008 (n=1452)
...
- **PATRÓN** `ibs_15` > `0.6071` → IC=+0.264 (n=1547)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6071 (IC base=+0.186)

- **PATRÓN** `dist_vwap_pct` > `0.1187` → IC=+0.188 (n=811)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.1187 (IC base=+0.186)

- **PATRÓN** `dist_vwap_pct` < `0.6143` → IC=+0.177 (n=1493)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` < 0.6143 (IC base=+0.186)

- **PATRÓN** `sigma_ewma_delta_pct` > `16.881` → IC=+0.274 (n=400)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 16.881 (IC base=+0.186)

- **PATRÓN** `libro_liquidez` > `2978.7992` → IC=+0.192 (n=1031)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 2978.7992 (IC base=+0.186)
...
### UPDOWN_GBM#SOL#5min
- **FILTRO** `dist_vwap_pct` > `0.5798` → IC=-0.131 (n=120)

  - _Acción_: SKIP cuando `dist_vwap_pct` > 0.5798
  - _Potencial_: sin este filtro IC_bueno=+0.053 (n=869)
...
- **PATRÓN** `sigma_h` > `0.0177` → IC=+0.235 (n=274)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0177 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.0862` → IC=+0.210 (n=181)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0862 (IC base=+0.189)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0596` → IC=+0.195 (n=368)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.97€ cuando `delta_ratio_macro` |x|> 0.0596 (IC base=+0.189)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.0893` → IC=+0.248 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.0893 (IC base=+0.189)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.226 (n=202)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.189)

- **PATRÓN** `ibs_15` > `0.5488` → IC=+0.284 (n=411)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5488 (IC base=+0.189)

- **PATRÓN** `dist_vwap_pct` > `0.1332` → IC=+0.208 (n=248)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1332 (IC base=+0.189)

- **PATRÓN** `dist_vwap_pct` < `0.8342` → IC=+0.190 (n=475)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `10.785` → IC=+0.192 (n=410)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` < 10.785 (IC base=+0.189)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.189 (n=422)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.02 (IC base=+0.189)

- **PATRÓN** `libro_liquidez` > `2910.1242` → IC=+0.277 (n=137)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2910.1242 (IC base=+0.189)
...
- **FILTRO** `sigma_h` > `0.0131` → IC=-0.219 (n=646)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0131
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=1942)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.190 (n=874)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1198` → IC=+0.243 (n=1121)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1198 (IC base=-0.035)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1778` → IC=+0.236 (n=1085)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1778 (IC base=-0.035)

- **PATRÓN** `ibs_15` < `0.35` → IC=+0.276 (n=1685)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.35 (IC base=-0.035)

- **PATRÓN** `dist_vwap_pct` > `0.9756` → IC=+0.300 (n=198)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9756 (IC base=-0.035)
...
- **PATRÓN** `ibs_15` < `0.3482` → IC=+0.263 (n=665)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **PATRÓN** `sigma_ewma_delta_pct` < `12.221` → IC=+0.229 (n=711)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 12.221 (IC base=+0.224)

- **PATRÓN** `libro_liquidez` > `3546.5572` → IC=+0.224 (n=665)
...
- **FILTRO** `drift_60min` |x|> `0.1657` → IC=-0.223 (n=207)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.140 (n=403)

- **FILTRO** `drift_15min` |x|> `0.888` → IC=-0.266 (n=152)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 0.888
  - _Potencial_: sin este filtro IC_bueno=-0.135 (n=458)
...
- **FILTRO** `sigma_h` > `0.0198` → IC=-0.260 (n=381)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.133 (n=382)

- **FILTRO** `drift_15min` |x|> `1.2462` → IC=-0.255 (n=190)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 1.2462
  - _Potencial_: sin este filtro IC_bueno=-0.176 (n=573)
...
- **PATRÓN** `ibs_15` < `0.3448` → IC=+0.310 (n=451)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6071 sube el IC de +0.186 a +0.264 en UPDOWN_GBM#15min (n=1547). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_YES, IBS > 0.5488 sube el IC de +0.189 a +0.284 en UPDOWN_GBM#XRP#15min (n=411). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.35 sube el IC de -0.035 a +0.276 en UPDOWN_GBM_15M_TARDIO (n=1685). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.3482 sube el IC de +0.224 a +0.263 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=665). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#XRP#15min**: dentro de BUY_NO, IBS < 0.3448 sube el IC de -0.046 a +0.310 en UPDOWN_GBM_15M_TARDIO#XRP#15min (n=451). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 24743 | -0.091 | -3143.80€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1436 | -0.048 | -223.38€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 23307 | -0.094 | -2920.43€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3645 | -0.089 | -577.41€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3645 | -0.089 | -577.41€ | 1 | 0 |
...
| ✅ BALLENAS_TARDIAS#ETH | 7150 | -0.023 | -647.86€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 7150 | -0.023 | -647.86€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 6629 | -0.098 | -451.68€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 6629 | -0.098 | -451.68€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 5509 | -0.183 | -1082.43€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 5509 | -0.183 | -1082.43€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 17565 | -0.029 | +4073.34€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 4593 | +0.000 | +1830.02€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 12972 | -0.039 | +2243.32€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 17565 | -0.029 | +4073.34€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 4593 | +0.000 | +1830.02€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 12972 | -0.039 | +2243.32€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 90908 | +0.112 | -4649.14€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 13772 | +0.185 | -444.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 365 | -0.089 | -51.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 70834 | +0.100 | -3946.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 5937 | +0.108 | -206.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 11782 | +0.098 | -1007.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 44 | -0.174 | -3.33€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 11723 | +0.100 | -992.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 18387 | +0.132 | -377.67€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 4328 | +0.202 | -146.45€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 11742 | +0.112 | -182.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 2275 | +0.106 | -26.83€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 11822 | +0.088 | -1110.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 51 | -0.085 | -5.77€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#DOGE#240min | 15 | -0.243 | -11.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 11756 | +0.090 | -1093.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 19351 | +0.123 | -407.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 5325 | +0.175 | -82.07€ | 1 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 11866 | +0.105 | -265.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 2148 | +0.098 | -51.06€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 17768 | +0.114 | -1040.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 3979 | +0.188 | -214.91€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 268 | -0.048 | +2.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 12007 | +0.091 | -699.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1514 | +0.127 | -128.73€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 11798 | +0.100 | -705.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 45 | -0.032 | +7.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 13 | -0.022 | -0.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 11740 | +0.101 | -712.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 