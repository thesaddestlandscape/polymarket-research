# Hipótesis automáticas — 2026-09-02 13:27 UTC
_Generado por shadow_postmortem.py sobre 252654 resoluciones (PNL=+22665.57€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `147.07` → IC=-0.292 (n=3448)
...
- **FILTRO** `restante_s_al_confirmar` < `137.96` → IC=-0.323 (n=764)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=2294)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.189 (n=7500)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.203 (n=773)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.194)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.204 (n=855)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.196 (n=1094)

  - _Acción_: Kelly boost +0.98€ cuando `libro_spread` < 0.01 (IC base=+0.194)

- **PATRÓN** `libro_liquidez` > `12455.0907` → IC=+0.212 (n=384)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.276 (n=418)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `2217.7203` → IC=+0.278 (n=552)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2217.7203 (IC base=+0.269)
...
- **PATRÓN** `libro_liquidez` > `921.8064` → IC=+0.221 (n=746)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.206 (n=16074)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.71 (IC base=+0.188)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.147 (n=3305)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.171 (n=2322)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `py_entrada` > 0.72 (IC base=+0.142)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.228 (n=2790)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.259 (n=2073)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.180 (n=2756)
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.240 (n=1376)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.222 (n=1367)
...
- **PATRÓN** `restante_min` > `4.93` → IC=+0.155 (n=2289)
...
- **PATRÓN** `lag_apertura_s` < `4.23` → IC=+0.157 (n=2146)
...
- **PATRÓN** `restante_min` > `4.94` → IC=+0.163 (n=1201)

  - _Acción_: Kelly boost +0.81€ cuando `restante_min` > 4.94 (IC base=+0.131)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.133 (n=3406)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.289 (n=264)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.272 (n=353)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.333 (n=225)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.297 (n=397)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.298)

- **PATRÓN** `libro_liquidez` > `1686.2353` → IC=+0.320 (n=321)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1686.2353 (IC base=+0.298)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3493` → IC=+0.125 (n=3333)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.482` → IC=+0.146 (n=1280)
...
- **PATRÓN** `ibs_20min` < `0.405` → IC=+0.124 (n=3268)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` < 0.405 (IC base=+0.038)

- **PATRÓN** `volumen_regimen` < `0.6788` → IC=+0.161 (n=496)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.6788 (IC base=+0.038)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8751` → IC=+0.214 (n=544)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8751 (IC base=+0.038)

- **PATRÓN** `ballena_activa_n` < `60.0` → IC=+0.214 (n=917)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.139` → IC=+0.276 (n=524)
...
- **PATRÓN** `libro_liquidez` > `1701.1448` → IC=+0.276 (n=329)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1701.1448 (IC base=+0.259)
...
- **PATRÓN** `sigma_h` < `0.006` → IC=+0.150 (n=643)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.006 (IC base=+0.147)

- **PATRÓN** `drift_60min` |x|≤ `0.1746` → IC=+0.154 (n=429)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.1746 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.151 (n=577)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.6389` → IC=+0.171 (n=643)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.6389 (IC base=+0.147)
...
- **PATRÓN** `volumen_regimen` < `0.6181` → IC=+0.196 (n=215)
  ...
  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` < 0.6181 (IC base=+0.147)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4168` → IC=+0.165 (n=535)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.4168 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` > `1.3957` → IC=+0.163 (n=535)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.3957 (IC base=+0.147)
...
- **PATRÓN** `libro_liquidez` > `12567.3384` → IC=+0.170 (n=428)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 12567.3384 (IC base=+0.147)
...
- **FILTRO** `ibs_20min` < `0.2901` → IC=-0.144 (n=209)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2901
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=426)
...
- **FILTRO** `drift_60min` |x|> `0.5207` → IC=-0.168 (n=329)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=639)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.198 (n=213)
...
- **PATRÓN** `ibs_20min` < `0.5862` → IC=+0.129 (n=4087)
...
- **PATRÓN** `volumen_regimen` < `0.8624` → IC=+0.225 (n=679)
...
- **FILTRO** `sigma_ewma_delta_pct` > `2.241` → IC=-0.164 (n=296)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1486` → IC=+0.184 (n=115)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` < 0.1486 (IC base=-0.028)

- **PATRÓN** `volumen_spike_ratio` < `1.7866` → IC=+0.264 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7866 (IC base=-0.028)

- **PATRÓN** `volumen_spike_ratio` > `1.3539` → IC=+0.182 (n=105)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 1.3539 (IC base=-0.028)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.331 (n=323)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.308)

- **PATRÓN** `ibs_20min` < `0.3296` → IC=+0.323 (n=456)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.285 (n=706)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.278)

- **PATRÓN** `ibs_20min` < `0.3656` → IC=+0.320 (n=747)
...
- **PATRÓN** `volumen_regimen` < `0.6376` → IC=+0.281 (n=249)
...
- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.195 (n=2210)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0063 (IC base=+0.179)

- **PATRÓN** `drift_60min` |x|≤ `0.0796` → IC=+0.212 (n=1105)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4241` → IC=+0.231 (n=3313)
...
- **PATRÓN** `volumen_regimen` < `1.1748` → IC=+0.169 (n=2515)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.1748 (IC base=+0.179)

- **PATRÓN** `volumen_regimen` > `0.6212` → IC=+0.162 (n=2515)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.6212 (IC base=+0.179)

- **PATRÓN** `volumen_pendiente_norm` > `0.2935` → IC=+0.262 (n=440)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2935 (IC base=+0.179)

- **PATRÓN** `volumen_spike_ratio` < `1.5767` → IC=+0.172 (n=1184)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.5767 (IC base=+0.179)

- **PATRÓN** `volumen_spike_ratio` > `2.661` → IC=+0.196 (n=897)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 2.661 (IC base=+0.179)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.176 (n=2273)

  - _Acción_: Kelly boost +0.88€ cuando `ballena_activa_n` < 156.0 (IC base=+0.179)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.261 (n=333)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0048 (IC base=+0.243)

- **PATRÓN** `drift_60min` |x|≤ `0.1635` → IC=+0.309 (n=249)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.257 (n=377)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.691` → IC=+0.255 (n=415)
...
- **PATRÓN** `volumen_spike_ratio` < `1.9129` → IC=+0.252 (n=204)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9129 (IC base=+0.243)
...
- **PATRÓN** `libro_liquidez` > `1699.8576` → IC=+0.277 (n=249)
...
- **PATRÓN** `volumen_spike_ratio` > `3.139` → IC=+0.235 (n=160)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.139 (IC base=+0.229)

- **PATRÓN** `libro_liquidez` > `1857.661` → IC=+0.243 (n=142)
...
- **PATRÓN** `drift_60min` |x|≤ `0.381` → IC=+0.152 (n=601)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.3886` → IC=+0.201 (n=529)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3886 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.086` → IC=+0.191 (n=173)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 10.086 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` < `1.1618` → IC=+0.140 (n=601)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` < 1.1618 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` > `0.6788` → IC=+0.133 (n=537)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_regimen` > 0.6788 (IC base=+0.133)

- **PATRÓN** `volumen_pendiente_norm` > `0.1059` → IC=+0.185 (n=198)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1059 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` < `1.8661` → IC=+0.153 (n=329)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5362` → IC=+0.165 (n=165)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 2.5362 (IC base=+0.133)

- **PATRÓN** `libro_liquidez` > `5154.5272` → IC=+0.148 (n=401)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 5154.5272 (IC base=+0.133)
...
- **PATRÓN** `sigma_h` < `0.0107` → IC=+0.197 (n=3903)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0107 (IC base=+0.188)

- **PATRÓN** `drift_60min` |x|≤ `0.4781` → IC=+0.194 (n=3902)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5593` → IC=+0.241 (n=3902)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5593 (IC base=+0.188)

- **PATRÓN** `dist_vwap_pct` < `0.2444` → IC=+0.173 (n=2507)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.2444 (IC base=+0.188)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.588` → IC=+0.214 (n=555)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.588 (IC base=+0.188)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.552` → IC=+0.190 (n=3829)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` < 3.552 (IC base=+0.188)

- **PATRÓN** `volumen_regimen` < `0.6218` → IC=+0.175 (n=933)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` < 0.6218 (IC base=+0.188)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3234` → IC=+0.198 (n=1449)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.3234 (IC base=+0.188)

- **PATRÓN** `ballena_activa_n` < `141.0` → IC=+0.176 (n=2776)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.277 (n=509)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.1115` → IC=+0.309 (n=223)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1115 (IC base=+0.271)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.281 (n=459)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.271)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.276 (n=516)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.271)

- **PATRÓN** `ibs_20min` < `0.4087` → IC=+0.308 (n=446)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4087 (IC base=+0.271)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.231` → IC=+0.285 (n=547)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.231 (IC base=+0.271)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5009` → IC=+0.288 (n=403)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.172 (n=633)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0061 (IC base=+0.169)

- **PATRÓN** `drift_60min` |x|≤ `0.2716` → IC=+0.183 (n=557)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.2716 (IC base=+0.169)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.179 (n=456)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` < 12.0 (IC base=+0.169)

- **PATRÓN** `ibs_20min` < `0.4364` → IC=+0.217 (n=557)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4364 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.993` → IC=+0.218 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.993 (IC base=+0.169)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4071` → IC=+0.190 (n=537)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` < 2.4071 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `2.084` → IC=+0.183 (n=244)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 2.084 (IC base=+0.169)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.169 (n=820)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.01 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `11565.2208` → IC=+0.170 (n=422)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.238 (n=410)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.238)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.247 (n=630)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_spike_ratio` > `1.9102` → IC=+0.217 (n=425)
...
- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.157 (n=502)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0072 (IC base=+0.132)

- **PATRÓN** `drift_60min` |x|≤ `0.4509` → IC=+0.152 (n=570)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6715` → IC=+0.175 (n=570)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.299` → IC=+0.135 (n=535)

  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` < 4.299 (IC base=+0.132)

- **PATRÓN** `volumen_regimen` < `0.5888` → IC=+0.141 (n=190)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` < 0.5888 (IC base=+0.132)
...
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.181 (n=233)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.0058 (IC base=+0.110)

- **PATRÓN** `drift_60min` |x|≤ `0.1315` → IC=+0.155 (n=233)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.1315 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.135 (n=332)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 15.0 (IC base=+0.110)

- **PATRÓN** `ibs_20min` < `0.5918` → IC=+0.196 (n=699)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` < 0.5918 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.2907` → IC=+0.133 (n=587)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.2907 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.138` → IC=+0.133 (n=685)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` < 3.138 (IC base=+0.110)

- **PATRÓN** `volumen_regimen` < `1.1985` → IC=+0.120 (n=699)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_regimen` < 1.1985 (IC base=+0.110)

- **PATRÓN** `volumen_pendiente_norm` > `0.0724` → IC=+0.150 (n=224)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_pendiente_norm` > 0.0724 (IC base=+0.110)

- **PATRÓN** `volumen_spike_ratio` < `1.4497` → IC=+0.140 (n=184)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 1.4497 (IC base=+0.110)

- **PATRÓN** `volumen_spike_ratio` > `2.1909` → IC=+0.138 (n=249)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 2.1909 (IC base=+0.110)
...
- **PATRÓN** `sigma_h` > `0.0253` → IC=+0.226 (n=301)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0253 (IC base=+0.215)

- **PATRÓN** `drift_60min` |x|≤ `0.5228` → IC=+0.216 (n=791)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ballena_activa_n` < `34.0` → IC=+0.189 (n=612)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.131 (n=1668)

- **PATRÓN** `sigma_h` < `0.0101` → IC=+0.129 (n=1201)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` < 0.0101 (IC base=+0.120)

- **PATRÓN** `drift_60min` |x|≤ `0.4383` → IC=+0.133 (n=1203)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.581` → IC=+0.161 (n=213)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 10.581 (IC base=+0.120)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.037` → IC=+0.122 (n=1387)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1749` → IC=+0.155 (n=369)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_pendiente_norm` > 0.1749 (IC base=+0.120)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3911` → IC=+0.148 (n=612)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 2.3911 (IC base=+0.120)

- **PATRÓN** `libro_liquidez` > `9008.5379` → IC=+0.135 (n=619)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 9008.5379 (IC base=+0.120)

- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.183 (n=421)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0038 (IC base=+0.127)

- **PATRÓN** `drift_60min` |x|≤ `0.5081` → IC=+0.147 (n=1263)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.5081 (IC base=+0.127)
...
- **PATRÓN** `ibs_20min` < `0.1983` → IC=+0.154 (n=556)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.1983 (IC base=+0.127)

- **PATRÓN** `dist_vwap_pct` > `0.4829` → IC=+0.133 (n=336)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` > 0.4829 (IC base=+0.127)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.363` → IC=+0.140 (n=1260)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` < 6.363 (IC base=+0.127)

- **PATRÓN** `volumen_regimen` < `1.232` → IC=+0.134 (n=1231)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 1.232 (IC base=+0.127)

- **PATRÓN** `volumen_pendiente_norm` < `0.1481` → IC=+0.129 (n=1258)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_pendiente_norm` < 0.1481 (IC base=+0.127)

- **PATRÓN** `volumen_pendiente_norm` > `0.0692` → IC=+0.142 (n=605)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_pendiente_norm` > 0.0692 (IC base=+0.127)

- **PATRÓN** `volumen_spike_ratio` < `2.4757` → IC=+0.137 (n=1251)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 2.4757 (IC base=+0.127)

- **PATRÓN** `volumen_spike_ratio` > `1.7939` → IC=+0.133 (n=834)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 1.7939 (IC base=+0.127)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.131 (n=1668)

  - _Acción_: Kelly boost +0.65€ cuando `libro_spread` < 0.01 (IC base=+0.127)

- **PATRÓN** `libro_liquidez` > `8304.7792` → IC=+0.137 (n=1128)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 8304.7792 (IC base=+0.127)
...
- **PATRÓN** `sigma_h` < `0.0091` → IC=+0.146 (n=493)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` < 0.0091 (IC base=+0.131)

- **PATRÓN** `drift_60min` |x|≤ `0.4826` → IC=+0.146 (n=493)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.4826 (IC base=+0.131)
...
- **PATRÓN** `dist_vwap_pct` > `1.006` → IC=+0.170 (n=101)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 1.006 (IC base=+0.131)

- **PATRÓN** `dist_vwap_pct` < `0.2469` → IC=+0.140 (n=423)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.2469 (IC base=+0.131)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.972` → IC=+0.137 (n=111)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` > 9.972 (IC base=+0.131)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.061` → IC=+0.142 (n=495)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` < 7.061 (IC base=+0.131)

- **PATRÓN** `volumen_regimen` < `0.728` → IC=+0.158 (n=217)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.728 (IC base=+0.131)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8388` → IC=+0.143 (n=323)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.8388 (IC base=+0.131)

- **PATRÓN** `libro_liquidez` > `8943.6658` → IC=+0.146 (n=441)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 8943.6658 (IC base=+0.131)
...
- **PATRÓN** `sigma_h` < `0.0054` → IC=+0.210 (n=198)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` > `0.6845` → IC=+0.246 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6845 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` > `0.3638` → IC=+0.198 (n=61)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.3638 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` < `0.2012` → IC=+0.169 (n=164)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` < 0.2012 (IC base=+0.102)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.549` → IC=+0.254 (n=124)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.549 (IC base=+0.102)

- **PATRÓN** `volumen_regimen` < `0.6313` → IC=+0.163 (n=90)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.6313 (IC base=+0.102)

- **PATRÓN** `volumen_regimen` > `0.5769` → IC=+0.159 (n=203)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 0.5769 (IC base=+0.102)

- **PATRÓN** `volumen_pendiente_norm` < `0.0856` → IC=+0.236 (n=85)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4589` → IC=+0.300 (n=93)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4589 (IC base=+0.102)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.192 (n=180)

  - _Acción_: Kelly boost +0.96€ cuando `libro_spread` < 0.02 (IC base=+0.102)

- **PATRÓN** `libro_liquidez` > `2311.7445` → IC=+0.186 (n=119)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 2311.7445 (IC base=+0.102)
...
- **PATRÓN** `ibs_20min` > `0.75` → IC=+0.150 (n=38)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` > 0.75 (IC base=+0.020)

- **PATRÓN** `dist_vwap_pct` > `0.492` → IC=+0.147 (n=15)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.492 (IC base=+0.020)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.36` → IC=+0.250 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.36 (IC base=+0.020)

- **PATRÓN** `volumen_regimen` > `0.7974` → IC=+0.167 (n=28)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 0.7974 (IC base=+0.020)

- **PATRÓN** `libro_liquidez` > `635.4762` → IC=+0.129 (n=33)

  - _Acción_: Kelly boost +0.64€ cuando `libro_liquidez` > 635.4762 (IC base=+0.020)
...
- **PATRÓN** `py_entrada` < `0.495` → IC=+0.136 (n=64)

  - _Acción_: Kelly boost +0.68€ cuando `py_entrada` < 0.495 (IC base=+0.111)

- **PATRÓN** `libro_liquidez` > `2471.3992` → IC=+0.148 (n=163)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 2471.3992 (IC base=+0.111)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.184 (n=1303)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.186 (n=1368)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.009 (n=4148)
...
- **FILTRO** `py_entrada` > `0.56` → IC=-0.186 (n=218)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=686)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=35)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=869)
...
- **FILTRO** `py_entrada` > `0.65` → IC=-0.229 (n=223)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.025 (n=699)

- **FILTRO** `ibs_20min` > `0.7279` → IC=-0.198 (n=230)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7279
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=692)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.187 (n=212)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=700)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=836)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.147 (n=3747)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.079 (n=9192)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.284 (n=3211)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=9728)

- **FILTRO** `ibs_7min` < `0.7097` → IC=-0.247 (n=3234)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7097
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=9705)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.173 (n=4338)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=8601)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.223 (n=3792)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=12569)

- **FILTRO** `ibs_7min` > `0.72` → IC=-0.171 (n=4089)

  - _Acción_: SKIP cuando `ibs_7min` > 0.72
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=12272)
...
- **FILTRO** `py_entrada` > `0.52` → IC=-0.138 (n=1911)

  - _Acción_: SKIP cuando `py_entrada` > 0.52
  - _Potencial_: sin este filtro IC_bueno=+0.079 (n=943)

- **FILTRO** `drift_7min_pct` |x|> `0.1143` → IC=-0.128 (n=970)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1143
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=1884)

- **FILTRO** `ibs_7min` > `0.8362` → IC=-0.185 (n=713)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8362
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=2141)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.147 (n=533)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=1904)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.266 (n=583)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=1854)

- **FILTRO** `ibs_7min` < `0.7699` → IC=-0.192 (n=609)
...
- **FILTRO** `ballena_activa_n` > `164.0` → IC=-0.193 (n=608)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=1829)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.227 (n=617)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=1860)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.199 (n=466)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.102 (n=1438)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.312 (n=616)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=1288)

- **FILTRO** `ibs_7min` < `0.2052` → IC=-0.284 (n=476)
...
- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.244 (n=466)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.087 (n=1438)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.230 (n=683)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.000 (n=2222)

- **FILTRO** `ibs_7min` > `0.8061` → IC=-0.172 (n=726)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8061
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.236 (n=726)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=1493)

- **FILTRO** `ibs_7min` > `0.2764` → IC=-0.183 (n=554)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2764
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=1665)

- **FILTRO** `ballena_activa_n` > `34.0` → IC=-0.183 (n=554)

  - _Acción_: SKIP cuando `ballena_activa_n` > 34.0
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=1665)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.39` → IC=-0.244 (n=585)

  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=1826)

- **FILTRO** `ibs_7min` < `0.7317` → IC=-0.212 (n=602)
...
- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.195 (n=584)
  ...
  - _Acción_: SKIP cuando `ballena_activa_n` > 13.0
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=1827)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.178 (n=752)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=2301)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.247 (n=572)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=2281)

- **FILTRO** `ibs_7min` > `0.7838` → IC=-0.159 (n=713)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7838
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=2140)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=1479)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1662` → IC=+0.131 (n=404)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.65€ cuando `drift_60min` |x|≤ 0.1662 (IC base=+0.125)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0646` → IC=+0.133 (n=459)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.67€ cuando `delta_ratio_macro` |x|> 0.0646 (IC base=+0.125)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.131 (n=491)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 4.0 (IC base=+0.125)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.159 (n=203)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` < 6.0 (IC base=+0.125)

- **PATRÓN** `ibs_15` > `0.5664` → IC=+0.216 (n=459)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5664 (IC base=+0.125)

- **PATRÓN** `dist_vwap_pct` > `0.4229` → IC=+0.170 (n=107)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.4229 (IC base=+0.125)
...
- **PATRÓN** `libro_liquidez` > `3042.6038` → IC=+0.153 (n=306)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 3042.6038 (IC base=+0.125)

- **PATRÓN** `ballena_activa_n` < `50.0` → IC=+0.188 (n=78)

  - _Acción_: Kelly boost +0.94€ cuando `ballena_activa_n` < 50.0 (IC base=+0.125)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0571` → IC=+0.155 (n=117)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.78€ cuando `delta_ratio_macro` |x|> 0.0571 (IC base=+0.108)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.173 (n=53)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` < 6.0 (IC base=+0.108)

- **PATRÓN** `ibs_15` > `0.5` → IC=+0.172 (n=117)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.86€ cuando `ibs_15` > 0.5 (IC base=+0.108)

- **PATRÓN** `dist_vwap_pct` > `0.384` → IC=+0.250 (n=34)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.384 (IC base=+0.108)
...
- **PATRÓN** `libro_liquidez` > `2489.2422` → IC=+0.154 (n=105)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 2489.2422 (IC base=+0.108)
...
- **FILTRO** `sigma_h` > `0.0106` → IC=-0.207 (n=315)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0106
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=947)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.135 (n=321)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=941)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0833` → IC=+0.238 (n=338)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0833 (IC base=-0.066)
...
- **PATRÓN** `ibs_15` < `0.3659` → IC=+0.287 (n=378)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.5664 sube el IC de +0.125 a +0.216 en UPDOWN_GBM#15min (n=459). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_YES, IBS > 0.5 sube el IC de +0.108 a +0.172 en UPDOWN_GBM#XRP#15min (n=117). Ya aplicado como kelly_boost=+0.86€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3659 sube el IC de -0.066 a +0.287 en UPDOWN_GBM_15M_TARDIO (n=378). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 13792 | -0.107 | -2340.23€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 914 | -0.005 | -134.65€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 12878 | -0.114 | -2205.58€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 3058 | -0.122 | -301.15€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 3058 | -0.122 | -301.15€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 2491 | -0.186 | -670.04€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 2491 | -0.186 | -670.04€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 3305 | -0.064 | +1547.70€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 938 | -0.005 | +665.41€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 2367 | -0.087 | +882.29€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 3305 | -0.064 | +1547.70€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 938 | -0.005 | +665.41€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 2367 | -0.087 | +882.29€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 44450 | +0.113 | -2904.30€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 7554 | +0.186 | -247.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 140 | -0.099 | -57.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 33137 | +0.096 | -2539.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3619 | +0.118 | -60.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 5567 | +0.078 | -788.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 24 | -0.115 | -1.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 9 | -0.143 | -7.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 5534 | +0.080 | -779.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 8952 | +0.130 | -234.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 2197 | +0.199 | -87.85€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 5486 | +0.105 | -172.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1227 | +0.126 | +48.74€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 5576 | +0.081 | -715.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 26 | +0.036 | +0.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#240min | 6 | -0.075 | -4.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 5544 | +0.082 | -711.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 9636 | +0.126 | -151.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2788 | +0.172 | -4.37€ | 1 | 7 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 5515 | +0.110 | -105.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1321 | +0.097 | -33.30€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 9161 | +0.127 | -617.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2500 | +0.195 | -152.81€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 68 | +0.000 | -12.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 5522 | +0.096 | -376.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1071 | +0.134 | -75.51€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 5558 | +0.104 | -396.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 19 | -0.023 | -0.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 5536 | +0.105 | -394.40€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 23556 | +0.188 | -2234.17€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 23556 | +0.188 | -2234.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 4203 | +0.142 | -700.91€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 4203 | +0.142 | -700.91€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 3685 | +0.225 | -132.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 3685 | +0.225 | -132.98€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 4077 | +0.163 | -549.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 4077 | +0.163 | -549.82€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 3741 | +0.223 | -138.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 3741 | +0.223 | -138.06€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 3889 | +0.200 | -289.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 3889 | +0.200 | -289.78€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 3961 | +0.182 | -422.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 3961 | +0.182 | -422.62€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 8580 | +0.135 | +343.05€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 
77€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 3055 | +0.037 | +278.73€ | 0 | 12 |
| ✅ UPDOWN_GBM#240min | 355 | +0.007 | -0.51€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 4630 | -0.009 | -59.61€ | 2 | 0 |
| ✅ UPDOWN_GBM#60min | 530 | -0.002 | -3.33€ | 1 | 0 |
...
| ✅ UPDOWN_GBM#BTC | 1748 | +0.017 | +90.62€ | 0 | 0 |
...
| ✅ UPDOWN_GBM#BTC#60min | 182 | -0.011 | -4.44€ | 2 | 1 |
| ✅ UPDOWN_GBM#BTC#daily | 18 | -0.135 | +1.83€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 938 | -0.005 | -5.35€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 126 | +0.086 | +24.76€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#240min | 9 | +0.021 | +0.39€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 803 | -0.021 | -30.50€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 1906 | -0.003 | -3.94€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 897 | +0.018 | +24.02€ | 1 | 3 |
...
| ✅ UPDOWN_GBM#SOL | 2479 | +0.003 | +7.22€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 797 | +0.002 | +5.58€ | 1 | 4 |
...
| ✅ UPDOWN_GBM#XRP | 1348 | +0.014 | +89.59€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 734 | +0.048 | +115.65€ | 0 | 8 |
| ✅ UPDOWN_GBM#XRP#240min | 39 | -0.159 | -7.14€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 575 | -0.017 | -18.92€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 5217 | -0.067 | +1157.31€ | 3 | 6 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 5217 | -0.067 | +1157.31€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 466 | +0.158 | +236.57€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 466 | +0.158 | +236.57€ | 2 | 15 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 1704 | -0.063 | +332.45€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 1704 | -0.063 | +332.45€ | 3 | 6 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 1604 | -0.088 | +325.17€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 1604 | -0.088 | +325.17€ | 4 | 5 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.089) — sin ventaja clara. oversold(IBS<0.3): IC=+0.025 n=3101 | neutral: IC=+0.001 n=3357 | overbought(IBS>0.7): IC=+0.090 n=3341
  - _Datos_: n=10203 IC=+0.040 PNL=+1014.68€

**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  - _Umbral_: n≥40 por celda + gate riguroso completo (Wilson+shuffle+PnL bootstrap)
  - _Acción_: Añadir claves 'ESTRATEGIA#SUBTYPE#DIRECCION#HORA':1.2 a meta.hora_boost_factor, solo por celda confirmada
  - _Estado_: (cache 642s) 109 celda(s) GATE OK de 2400 trackeadas

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.002 < 0.08 — monitorear
  - _Datos_: n=797 IC=+0.002 PNL=+5.58€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.069 n=141262 | tras_1loss IC=+0.048 n=111092 | tras_2loss IC=+0.012 n=50158/40 | gap=+0.058 (umbral 0.05)
...
**⏳ H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  ...
  - _Estado_: 8555 ops, 22 horas distintas. Sin hora con n≥15 y IC extremo aún.
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=232/40 IC=+0.000 PNL=-2.57€ | BTC#60min: n=182/40 IC=-0.011 PNL=-4.44€ | SOL#60min: n=116/40 IC=+0.009 PNL=+3.68€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=8340 IC=+0.005 PNL=+161.16€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=8340 IC=+0.005 PNL=+161.16€
...
**〰️ H-CUSTOM-GBM-60MIN-BUYYES** — GBM 60min BUY_YES — ¿edge superior al BUY_NO?
  ...
  - _Estado_: n=437 IC=+0.006 PNL=-0.60€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=437 IC=+0.006 PNL=-0.60€
...
**🟡 H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.125 > 0.1 con n=611 PNL=+176.83€
  - _Datos_: n=611 IC=+0.125 PNL=+176.83€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=2269 IC=+0.015 PNL=+96.04€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=2269 IC=+0.015 PNL=+96.04€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=1169 IC=+0.029 PNL=+64.54€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1169 IC=+0.029 PNL=+64.54€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=1958 IC=+0.021 PNL=+123.51€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1958 IC=+0.021 PNL=+123.51€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.166 > 0.1 con n=918 PNL=+349.65€
  - _Datos_: n=918 IC=+0.166 PNL=+349.65€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=6973 IC=-0.148 PNL=+201.89€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=6973 IC=-0.148 PNL=+201.89€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=856 IC=+0.147 PNL=+446.47€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=856 IC=+0.147 PNL=+446.47€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.125 > 0.08 con n=590 PNL=+168.54€
  - _Datos_: n=590 IC=+0.125 PNL=+168.54€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.247 < -0.1 con n=778 PNL=-121.99€
  - _Datos_: n=778 IC=-0.247 PNL=-121.99€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=1956 IC=+0.125 PNL=+1037.80€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=1956 IC=+0.125 PNL=+1037.80€
...
**〰️ H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: n=882 IC=-0.018 PNL=+71.20€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=882 IC=-0.018 PNL=+71.20€
...
**🔶 H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.111 n=242) pero sin cruzar ≥2 pares más — sin otros pares con datos
  - _Datos_: n=242 IC=+0.111 PNL=+66.89€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=4203 IC=+0.142 PNL=-700.91€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=4203 IC=+0.142 PNL=-700.91€
