# Hipótesis automáticas — 2026-07-02 07:26 UTC
_Generado por shadow_postmortem.py sobre 2565 resoluciones (PNL=-81.43€)_
...
- **PATRÓN** `sigma_h` < `0.0098` → IC=+0.214 (n=68)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0098 (IC base=+0.058)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.150 (n=38)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 14.0 (IC base=+0.058)

- **PATRÓN** `ibs_15` < `0.0714` → IC=+0.229 (n=46)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0745` → IC=+0.143 (n=40)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.71€ cuando `pct_spot_vs_ref` |x|≤ 0.0745 (IC base=+0.129)

- **PATRÓN** `sigma_h` < `0.0059` → IC=+0.167 (n=16)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0059 (IC base=+0.129)

- **PATRÓN** `sigma_h` > `0.0063` → IC=+0.128 (n=41)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.64€ cuando `sigma_h` > 0.0063 (IC base=+0.129)

- **PATRÓN** `drift_60min` |x|≤ `0.9082` → IC=+0.141 (n=37)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.9082 (IC base=+0.129)

- **PATRÓN** `drift_15min` |x|≤ `0.688` → IC=+0.133 (n=28)

  - _Acción_: Kelly boost +0.67€ cuando `drift_15min` |x|≤ 0.688 (IC base=+0.129)

- **PATRÓN** `ibs_15` < `0.0792` → IC=+0.333 (n=22)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0792 (IC base=+0.129)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS < 0.0714 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.229 n=46). Confirma señal de reversión media → alinear con BUY_YES.
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.0792 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.333 n=22). Confirma señal de reversión media → alinear con BUY_YES.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 12 | +0.129 | +3.05€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 12 | +0.129 | +3.05€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 795 | -0.035 | -35.60€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 571 | -0.011 | -9.77€ | 2 | 3 |
...
| ✅ UPDOWN_GBM#ETH | 308 | -0.006 | +8.01€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 225 | +0.011 | +13.96€ | 4 | 6 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.095) — sin ventaja clara. oversold(IBS<0.3): IC=+0.026 n=133 | neutral: IC=-0.069 n=121 | overbought(IBS>0.7): IC=-0.047 n=137
  - _Datos_: n=405 IC=-0.028 PNL=-10.37€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.002 n=1205 | tras_1loss IC=-0.035 n=1266 | tras_2loss IC=-0.050 n=660/40 | gap=+0.052 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=672 IC=-0.028 PNL=-22.05€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=672 IC=-0.028 PNL=-22.05€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=188 IC=+0.032 PNL=+18.09€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=188 IC=+0.032 PNL=+18.09€
...
**⏳ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: 148/200 ops en el filtro definido (IC actual=+0.053 PNL=+11.49€)
  - _Datos_: n=148 IC=+0.053 PNL=+11.49€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=148 IC=+0.053 PNL=+11.49€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=148 IC=+0.053 PNL=+11.49€
