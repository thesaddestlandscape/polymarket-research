# Hipótesis automáticas — 2026-08-18 04:19 UTC
_Generado por shadow_postmortem.py sobre 58427 resoluciones (PNL=+6235.67€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `142.41` → IC=-0.272 (n=764)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 142.41
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=2293)
...
- **FILTRO** `restante_s_al_confirmar` > `638.19` → IC=-0.271 (n=107)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` > 638.19
...
- **FILTRO** `restante_s_al_confirmar` < `242.75` → IC=-0.174 (n=511)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.203 (n=2043)
...
- **PATRÓN** `py_entrada` < `0.275` → IC=+0.332 (n=153)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.275 (IC base=+0.191)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.191 (n=409)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.188 (n=366)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.195 (n=1124)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.228 (n=491)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.156 (n=335)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 7.0 (IC base=+0.145)

- **PATRÓN** `py_entrada` > `0.76` → IC=+0.215 (n=251)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.76 (IC base=+0.145)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.298 (n=226)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.256 (n=215)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.249)

- **PATRÓN** `py_entrada` > `0.74` → IC=+0.284 (n=299)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.160 (n=1409)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 17.0 (IC base=+0.152)

- **PATRÓN** `lag_apertura_s` < `5.41` → IC=+0.208 (n=464)
...
- **PATRÓN** `restante_min` < `3.71` → IC=+0.182 (n=231)

  - _Acción_: Kelly boost +0.91€ cuando `restante_min` < 3.71 (IC base=+0.156)

- **PATRÓN** `restante_min` > `4.88` → IC=+0.192 (n=245)

  - _Acción_: Kelly boost +0.96€ cuando `restante_min` > 4.88 (IC base=+0.156)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=703)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.156)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.169 (n=617)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 15.0 (IC base=+0.156)

- **PATRÓN** `lag_apertura_s` < `7.37` → IC=+0.192 (n=232)

  - _Acción_: Kelly boost +0.96€ cuando `lag_apertura_s` < 7.37 (IC base=+0.156)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.142 (n=411)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `ibs_20min` < `0.6571` → IC=+0.124 (n=2049)
...
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.323 (n=60)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.007 (IC base=+0.279)

- **PATRÓN** `drift_60min` |x|≤ `0.2401` → IC=+0.308 (n=180)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2401 (IC base=+0.279)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.282 (n=186)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.279)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.285 (n=184)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.279)

- **PATRÓN** `ibs_20min` < `0.5765` → IC=+0.323 (n=179)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5765 (IC base=+0.279)

- **PATRÓN** `volumen_pendiente_norm` < `0.0707` → IC=+0.317 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0707 (IC base=+0.279)

- **PATRÓN** `volumen_pendiente_norm` > `0.24` → IC=+0.308 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.24 (IC base=+0.279)

- **PATRÓN** `volumen_spike_ratio` < `1.8119` → IC=+0.330 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8119 (IC base=+0.279)

- **PATRÓN** `volumen_spike_ratio` > `1.5873` → IC=+0.300 (n=68)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5873 (IC base=+0.279)

- **PATRÓN** `libro_spread` < `0.05` → IC=+0.316 (n=123)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.05 (IC base=+0.279)

- **PATRÓN** `libro_liquidez` > `1903.96` → IC=+0.283 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1903.96 (IC base=+0.279)
...
- **PATRÓN** `sigma_h` < `0.0018` → IC=+0.167 (n=73)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0018 (IC base=+0.140)

- **PATRÓN** `sigma_h` > `0.0024` → IC=+0.150 (n=141)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0024 (IC base=+0.140)

- **PATRÓN** `drift_60min` |x|≤ `0.1916` → IC=+0.160 (n=186)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.1916 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.190 (n=156)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 12.0 (IC base=+0.140)

- **PATRÓN** `ibs_20min` < `0.4388` → IC=+0.214 (n=211)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4388 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.599` → IC=+0.253 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.599 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `1.2726` → IC=+0.148 (n=211)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 1.2726 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` > `1.1074` → IC=+0.173 (n=96)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 1.1074 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` > `0.1294` → IC=+0.265 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1294 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` < `1.5117` → IC=+0.263 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5117 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `5211.9825` → IC=+0.139 (n=189)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 5211.9825 (IC base=+0.140)
...
- **PATRÓN** `ibs_20min` > `0.5406` → IC=+0.172 (n=59)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` > 0.5406 (IC base=+0.055)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.581` → IC=+0.192 (n=24)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 11.581 (IC base=+0.055)
...
- **PATRÓN** `ibs_20min` > `0.9497` → IC=+0.239 (n=497)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9497 (IC base=+0.059)

- **PATRÓN** `dist_vwap_pct` > `0.1186` → IC=+0.164 (n=144)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.1186 (IC base=+0.059)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.304` → IC=+0.130 (n=927)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` > 2.304 (IC base=+0.059)

- **PATRÓN** `volumen_pendiente_norm` > `0.3377` → IC=+0.184 (n=96)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.3377 (IC base=+0.059)

- **PATRÓN** `volumen_spike_ratio` > `2.8942` → IC=+0.174 (n=277)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.8942 (IC base=+0.059)

- **PATRÓN** `ballena_activa_n` < `30.0` → IC=+0.357 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 30.0 (IC base=+0.059)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.107` → IC=+0.187 (n=81)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 5.107 (IC base=-0.008)

- **PATRÓN** `volumen_pendiente_norm` > `0.0487` → IC=+0.167 (n=22)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.0487 (IC base=-0.008)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.147 (n=49)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=155)

- **FILTRO** `ibs_20min` < `0.5879` → IC=-0.140 (n=134)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.226 (n=1128)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0061 (IC base=+0.219)

- **PATRÓN** `drift_60min` |x|≤ `0.2474` → IC=+0.228 (n=992)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2474 (IC base=+0.219)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.262 (n=527)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.219)

- **PATRÓN** `ibs_20min` < `0.3611` → IC=+0.288 (n=1128)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3611 (IC base=+0.219)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.805` → IC=+0.245 (n=245)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.805 (IC base=+0.219)

- **PATRÓN** `volumen_regimen` < `1.2637` → IC=+0.198 (n=874)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` < 1.2637 (IC base=+0.219)

- **PATRÓN** `volumen_regimen` > `0.7176` → IC=+0.205 (n=781)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7176 (IC base=+0.219)

- **PATRÓN** `volumen_pendiente_norm` > `0.1884` → IC=+0.292 (n=205)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1884 (IC base=+0.219)

- **PATRÓN** `volumen_spike_ratio` < `2.0479` → IC=+0.239 (n=351)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0479 (IC base=+0.219)

- **PATRÓN** `volumen_spike_ratio` > `3.1581` → IC=+0.253 (n=176)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.1581 (IC base=+0.219)
...
- **PATRÓN** `sigma_h` > `0.0067` → IC=+0.309 (n=40)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0067 (IC base=+0.283)

- **PATRÓN** `drift_60min` |x|≤ `0.2252` → IC=+0.319 (n=103)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2252 (IC base=+0.283)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.308 (n=118)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` < `0.081` → IC=+0.329 (n=39)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8801` → IC=+0.372 (n=37)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8801 (IC base=+0.283)

- **PATRÓN** `libro_spread` < `0.08` → IC=+0.300 (n=138)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.08 (IC base=+0.283)

- **PATRÓN** `libro_liquidez` > `1926.44` → IC=+0.329 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1926.44 (IC base=+0.283)
...
- **PATRÓN** `sigma_h` < `0.0018` → IC=+0.204 (n=69)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0018 (IC base=+0.193)

- **PATRÓN** `sigma_h` > `0.0024` → IC=+0.200 (n=138)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0024 (IC base=+0.193)

- **PATRÓN** `drift_60min` |x|≤ `0.1983` → IC=+0.206 (n=182)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1983 (IC base=+0.193)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.259 (n=139)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 12.0 (IC base=+0.193)

- **PATRÓN** `ibs_20min` < `0.2725` → IC=+0.237 (n=207)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2725 (IC base=+0.193)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.586` → IC=+0.295 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.586 (IC base=+0.193)

- **PATRÓN** `volumen_regimen` < `0.6392` → IC=+0.246 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6392 (IC base=+0.193)

- **PATRÓN** `volumen_regimen` > `0.6986` → IC=+0.195 (n=185)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` > 0.6986 (IC base=+0.193)

- **PATRÓN** `volumen_pendiente_norm` < `0.2719` → IC=+0.229 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2719 (IC base=+0.193)

- **PATRÓN** `volumen_pendiente_norm` > `0.1345` → IC=+0.293 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1345 (IC base=+0.193)

- **PATRÓN** `volumen_spike_ratio` < `2.5474` → IC=+0.256 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5474 (IC base=+0.193)
...
- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.241 (n=203)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0038 (IC base=+0.206)

- **PATRÓN** `drift_60min` |x|≤ `0.1452` → IC=+0.225 (n=136)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1452 (IC base=+0.206)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.226 (n=210)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.206)

- **PATRÓN** `ibs_20min` < `0.2489` → IC=+0.266 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2489 (IC base=+0.206)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.362` → IC=+0.269 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.362 (IC base=+0.206)

- **PATRÓN** `volumen_regimen` < `1.2412` → IC=+0.237 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2412 (IC base=+0.206)

- **PATRÓN** `volumen_pendiente_norm` < `0.0873` → IC=+0.253 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0873 (IC base=+0.206)

- **PATRÓN** `volumen_pendiente_norm` > `0.2357` → IC=+0.315 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2357 (IC base=+0.206)

- **PATRÓN** `volumen_spike_ratio` < `2.2305` → IC=+0.267 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.2305 (IC base=+0.206)

- **PATRÓN** `libro_liquidez` > `5079.8508` → IC=+0.213 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 5079.8508 (IC base=+0.206)
...
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.333 (n=58)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.007 (IC base=+0.294)

- **PATRÓN** `drift_60min` |x|≤ `0.2132` → IC=+0.345 (n=153)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2132 (IC base=+0.294)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.298 (n=181)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.294)

- **PATRÓN** `hora_utc` < `14.0` → IC=+0.305 (n=157)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 14.0 (IC base=+0.294)

- **PATRÓN** `ibs_20min` < `0.5657` → IC=+0.335 (n=174)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5657 (IC base=+0.294)

- **PATRÓN** `volumen_pendiente_norm` < `0.0708` → IC=+0.333 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0708 (IC base=+0.294)

- **PATRÓN** `volumen_spike_ratio` < `1.9246` → IC=+0.327 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9246 (IC base=+0.294)

- **PATRÓN** `volumen_spike_ratio` > `1.5394` → IC=+0.305 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5394 (IC base=+0.294)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.323 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.03 (IC base=+0.294)

- **PATRÓN** `libro_liquidez` > `1888.7109` → IC=+0.315 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1888.7109 (IC base=+0.294)
...
- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.160 (n=92)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0038 (IC base=+0.100)

- **PATRÓN** `drift_60min` |x|≤ `0.0795` → IC=+0.149 (n=35)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.0795 (IC base=+0.100)

- **PATRÓN** `hora_utc` < `3.0` → IC=+0.204 (n=52)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 3.0 (IC base=+0.100)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.363` → IC=+0.129 (n=33)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 9.363 (IC base=+0.100)

- **PATRÓN** `volumen_regimen` < `1.5227` → IC=+0.132 (n=104)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_regimen` < 1.5227 (IC base=+0.100)

- **PATRÓN** `volumen_spike_ratio` < `2.8163` → IC=+0.148 (n=103)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 2.8163 (IC base=+0.100)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.140 (n=123)

  - _Acción_: Kelly boost +0.70€ cuando `libro_spread` < 0.01 (IC base=+0.100)

- **PATRÓN** `libro_liquidez` > `6124.0917` → IC=+0.151 (n=104)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 6124.0917 (IC base=+0.100)
...
- **PATRÓN** `hora_utc` > `20.0` → IC=+0.196 (n=44)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 20.0 (IC base=+0.121)

- **PATRÓN** `hora_utc` < `2.0` → IC=+0.155 (n=56)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 2.0 (IC base=+0.121)

- **PATRÓN** `py_entrada` > `0.505` → IC=+0.141 (n=126)

  - _Acción_: Kelly boost +0.70€ cuando `py_entrada` > 0.505 (IC base=+0.121)

- **PATRÓN** `drift_20min_pct` |x|≤ `0.0526` → IC=+0.278 (n=43)

  - _Acción_: Kelly boost +1.00€ cuando `drift_20min_pct` |x|≤ 0.0526 (IC base=+0.121)

- **PATRÓN** `ibs_20min` < `0.2136` → IC=+0.141 (n=126)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` < 0.2136 (IC base=+0.121)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.124 (n=131)

  - _Acción_: Kelly boost +0.62€ cuando `libro_spread` < 0.01 (IC base=+0.121)

- **PATRÓN** `libro_liquidez` > `14201.4527` → IC=+0.178 (n=57)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 14201.4527 (IC base=+0.121)
...
- **FILTRO** `py_entrada` < `0.485` → IC=-0.183 (n=39)

  - _Acción_: SKIP cuando `py_entrada` < 0.485
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=120)

- **FILTRO** `ibs_20min` < `0.7511` → IC=-0.183 (n=39)
...
- **FILTRO** `libro_liquidez` < `2278.3444` → IC=-0.136 (n=20)
...
- **PATRÓN** `drift_7min_pct` |x|≤ `0.0853` → IC=+0.156 (n=30)

  - _Acción_: Kelly boost +0.78€ cuando `drift_7min_pct` |x|≤ 0.0853 (IC base=+0.057)
...
- **PATRÓN** `drift_7min_pct` |x|≤ `0.0379` → IC=+0.250 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `drift_7min_pct` |x|≤ 0.0379 (IC base=+0.053)
...
- **FILTRO** `hora_utc` > `20.0` → IC=-0.127 (n=124)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=350)
...
- **FILTRO** `ibs_7min` < `0.7732` → IC=-0.217 (n=118)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7732
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=356)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.217 (n=118)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=356)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.242 (n=87)
...
- **FILTRO** `hora_utc` < `15.0` → IC=-0.227 (n=31)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 15.0
...
- **FILTRO** `ballena_activa_n` > `68.0` → IC=-0.204 (n=25)
...
- **FILTRO** `libro_liquidez` < `2488.1182` → IC=-0.250 (n=38)

  - _Acción_: SKIP cuando `libro_liquidez` < 2488.1182
  - _Potencial_: sin este filtro IC_bueno=+0.110 (n=39)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.278 (n=16)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.077 (n=50)

- **PATRÓN** `libro_liquidez` > `2706.4021` → IC=+0.155 (n=27)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 2706.4021 (IC base=-0.070)
...
- **FILTRO** `libro_liquidez` < `8699.3721` → IC=-0.167 (n=40)
...
- **FILTRO** `hora_utc` < `15.0` → IC=-0.214 (n=26)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 15.0
...
- **FILTRO** `ibs_7min` < `0.875` → IC=-0.250 (n=26)

  - _Acción_: SKIP cuando `ibs_7min` < 0.875
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=54)

- **FILTRO** `ballena_activa_n` > `17.0` → IC=-0.262 (n=19)
...
- **PATRÓN** `hora_utc` > `19.0` → IC=+0.208 (n=22)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 19.0 (IC base=+0.053)
...
- **PATRÓN** `sigma_h` > `0.0059` → IC=+0.137 (n=202)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0059 (IC base=+0.100)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0809` → IC=+0.135 (n=302)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.67€ cuando `delta_ratio_macro` |x|> 0.0809 (IC base=+0.100)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.122 (n=310)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` < 17.0 (IC base=+0.100)

- **PATRÓN** `ibs_15` < `0.5` → IC=+0.146 (n=303)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.73€ cuando `ibs_15` < 0.5 (IC base=+0.100)

- **PATRÓN** `dist_vwap_pct` > `0.4968` → IC=+0.208 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4968 (IC base=+0.100)

- **PATRÓN** `libro_liquidez` > `3517.7422` → IC=+0.150 (n=101)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 3517.7422 (IC base=+0.100)
...
- **PATRÓN** `ibs_15` > `0.5408` → IC=+0.236 (n=104)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5408 (IC base=-0.031)
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.5408 sube el IC de -0.031 a +0.236 en UPDOWN_GBM_15M_TARDIO (n=104). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 3057 | -0.122 | -496.85€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 430 | -0.002 | -6.76€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 2627 | -0.142 | -490.09€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 345 | -0.195 | -105.16€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 345 | -0.195 | -105.16€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 430 | -0.002 | -6.76€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 430 | -0.002 | -6.76€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 282 | -0.197 | -153.98€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 282 | -0.197 | -153.98€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 682 | -0.145 | -43.26€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 682 | -0.145 | -43.26€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 728 | -0.030 | -93.09€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 728 | -0.030 | -93.09€ | 1 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP | 590 | -0.216 | -94.59€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP#5min | 590 | -0.216 | -94.59€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 13179 | +0.114 | -807.12€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3217 | +0.178 | -121.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 98 | -0.100 | -46.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 7306 | +0.083 | -655.28€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2558 | +0.131 | +16.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 1236 | +0.030 | -277.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 13 | -0.065 | -2.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 1219 | +0.033 | -269.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 3048 | +0.140 | -5.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 877 | +0.191 | -42.71€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 41 | -0.105 | -20.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 1217 | +0.113 | -3.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 913 | +0.138 | +61.96€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 1232 | +0.048 | -229.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 9 | -0.021 | -3.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 1222 | +0.049 | -224.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 3294 | +0.125 | -54.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1186 | +0.158 | -26.88€ | 0 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 1214 | +0.101 | -20.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 882 | +0.117 | +1.31€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 3138 | +0.135 | -198.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1120 | +0.195 | -48.03€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 39 | +0.012 | -7.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 1216 | +0.081 | -95.71€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 3295 | +0.162 | -304.61€ | 1 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 3295 | +0.162 | -304.61€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 728 | +0.216 | -41.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 728 | +0.216 | -41.11€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 5216 | +0.191 | -472.65€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 5216 | +0.191 | -472.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 988 | +0.098 | -219.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 988 | +0.098 | -219.47€ | 0 | 1 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 805 | +0.252 | -2.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 805 | +0.252 | -2.62€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 923 | +0.145 | -143.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 923 | +0.145 | -143.40€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 846 | +0.223 | -34.28€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 846 | +0.223 | -34.28€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 802 | +0.249 | -6.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 802 | +0.249 | -6.29€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 1855 | +0.152 | +107.06€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 1855 | +0.152 | +107.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 922 | +0.156 | +61.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 922 | +0.156 | +61.49€ | 0 | 6 |
...
| ✅ GBM_LATE_15M | 4260 | +0.089 | +1501.03€ | 0 | 15 |
| ✅ GBM_LATE_15M#15min | 4260 | +0.089 | +1501.03€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 738 | +0.176 | +461.91€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 738 | +0.176 | +461.91€ | 0 | 18 |
| ✅ GBM_LATE_15M#BTC | 399 | +0.171 | +180.04€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 399 | +0.171 | +180.04€ | 0 | 22 |
| ✅ GBM_LATE_15M#DOGE | 740 | +0.193 | +509.21€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 740 | +0.193 | +509.21€ | 0 | 20 |
| ✅ GBM_LATE_15M#ETH | 575 | +0.001 | +28.23€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 575 | +0.001 | +28.23€ | 0 | 2 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR | 5267 | +0.052 | +1596.30€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 5267 | +0.052 | +1596.30€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 1011 | -0.024 | +197.30€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 1011 | -0.024 | +197.30€ | 1 | 2 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 996 | +0.000 | +125.04€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 996 | +0.000 | +125.04€ | 0 | 2 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 632 | +0.240 | +576.70€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 632 | +0.240 | +576.70€ | 0 | 18 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 917 | -0.010 | +26.61€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 917 | -0.010 | +26.61€ | 6 | 0 |
...
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 3065 | +0.176 | +2077.51€ | 0 | 22 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 3065 | +0.176 | +2077.51€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB | 546 | +0.192 | +394.76€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB#15min | 546 | +0.192 | +394.76€ | 0 | 16 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC | 396 | +0.196 | +264.05€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC#15min | 396 | +0.196 | +264.05€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE | 542 | +0.206 | +429.86€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min | 542 | +0.206 | +429.86€ | 0 | 21 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH | 369 | +0.222 | +281.19€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH#15min | 369 | +0.222 | +281.19€ | 0 | 23 |
...
| ✅ GBM_LATE_15M_TARDIO | 3579 | +0.169 | +2320.34€ | 0 | 16 |
| ✅ GBM_LATE_15M_TARDIO#15min | 3579 | +0.169 | +2320.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 709 | +0.186 | +494.87€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 709 | +0.186 | +494.87€ | 0 | 18 |
...
| ✅ GBM_LATE_5M | 194 | +0.071 | +30.94€ | 1 | 9 |
| ✅ GBM_LATE_5M#5min | 194 | +0.071 | +30.94€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_15M | 335 | +0.052 | +18.87€ | 1 | 7 |
| ✅ MOMENTUM_IBS_15M#15min | 335 | +0.052 | +18.87€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_15M#ETH | 56 | +0.155 | +24.36€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH#15min | 56 | +0.155 | +24.36€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_15M_BALLENA | 361 | +0.015 | +30.08€ | 3 | 1 |
| ✅ MOMENTUM_IBS_15M_BALLENA#15min | 361 | +0.015 | +30.08€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB | 57 | +0.009 | +19.67€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB#15min | 57 | +0.009 | +19.67€ | 1 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC | 61 | -0.040 | -4.65€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC#15min | 61 | -0.040 | -4.65€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE | 61 | +0.040 | +9.56€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE#15min | 61 | +0.040 | +9.56€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH | 59 | +0.074 | +8.69€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH#15min | 59 | +0.074 | +8.69€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_5M | 866 | +0.006 | +9.53€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#5min | 866 | +0.006 | +9.53€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#BNB | 117 | -0.029 | -0.72€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#BNB#5min | 117 | -0.029 | -0.72€ | 2 | 1 |
| ✅ MOMENTUM_IBS_5M#BTC | 154 | +0.013 | -2.36€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#BTC#5min | 154 | +0.013 | -2.36€ | 1 | 1 |
| ✅ MOMENTUM_IBS_5M#DOGE | 114 | +0.026 | +3.16€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#DOGE#5min | 114 | +0.026 | +3.16€ | 1 | 1 |
| ✅ MOMENTUM_IBS_5M#ETH | 164 | +0.006 | +4.11€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#ETH#5min | 164 | +0.006 | +4.11€ | 1 | 1 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 850 | -0.054 | -50.31€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 850 | -0.054 | -50.31€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 129 | -0.042 | +12.37€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 129 | -0.042 | +12.37€ | 0 | 1 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 156 | -0.076 | -18.18€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 156 | -0.076 | -18.18€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 143 | -0.045 | -18.09€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 143 | -0.045 | -18.09€ | 5 | 1 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 126 | -0.102 | -15.41€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 126 | -0.102 | -15.41€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 150 | -0.040 | -4.18€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 150 | -0.040 | -4.18€ | 4 | 2 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 146 | -0.020 | -6.81€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 146 | -0.020 | -6.81€ | 4 | 2 |
| ✅ MOMENTUM_IBS_5M_FADE | 939 | +0.017 | +3.53€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 939 | +0.017 | +3.53€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB | 154 | +0.051 | +10.55€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB#5min | 154 | +0.051 | +10.55€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC | 145 | +0.037 | -1.48€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC#5min | 145 | +0.037 | -1.48€ | 2 | 2 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE | 157 | +0.003 | -1.21€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE#5min | 157 | +0.003 | -1.21€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH | 168 | +0.012 | +1.78€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH#5min | 168 | +0.012 | +1.78€ | 0 | 1 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL | 162 | -0.012 | -3.35€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL#5min | 162 | -0.012 | -3.35€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 1849 | +0.024 | +122.39€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 813 | +0.098 | +166.44€ | 1 | 10 |
| ✅ UPDOWN_GBM#240min | 112 | +0.026 | +0.60€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 631 | -0.034 | -33.51€ | 5 | 0 |
| ✅ UPDOWN_GBM#60min | 246 | -0.024 | -10.64€ | 2 | 1 |
| ✅ UPDOWN_GBM#BNB | 75 | +0.045 | +4.92€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 68 | +0.071 | +6.54€ | 0 | 0 |
...
| ✅ UPDOWN_GBM#DOGE | 259 | +0.013 | +6.79€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 51 | +0.160 | +20.17€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 208 | -0.024 | -13.38€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1524 | -0.041 | +180.81€ | 2 | 2 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1524 | -0.041 | +180.81€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 97 | -0.066 | -0.50€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 97 | -0.066 | -0.50€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.158: overbought→boost, oversold→filtro | oversold(IBS<0.3): IC=-0.030 n=634 | neutral: IC=+0.003 n=646 | overbought(IBS>0.7): IC=+0.128 n=861
  - _Datos_: n=2290 IC=+0.043 PNL=+184.70€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.110 n=34423 | tras_1loss IC=+0.066 n=23709 | tras_2loss IC=+0.024 n=10242/40 | gap=+0.086 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1715 IC=+0.024 PNL=+122.46€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1715 IC=+0.024 PNL=+122.46€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.093 > 0.08 con n=509 PNL=+78.57€
  - _Datos_: n=509 IC=+0.093 PNL=+78.57€
...
**🟡 H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.111 > 0.1 con n=342 PNL=+105.03€
  - _Datos_: n=342 IC=+0.111 PNL=+105.03€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=281 IC=+0.140 PNL=+81.54€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=281 IC=+0.140 PNL=+81.54€
...
**〰️ H-CUSTOM-GBM-04H-ASIA** — UPDOWN_GBM 04h-05h UTC — media sesión asiática, ¿mejor franja nocturna?
  ...
  - _Estado_: n=159 IC=-0.009 PNL=+2.57€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=159 IC=-0.009 PNL=+2.57€
...
**🟡 H-CUSTOM-LATE-04H-ASIA** — GBM_LATE_15M 04h-05h UTC — media sesión asiática (gemela de GBM-04H-ASIA)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.121 > 0.08 con n=346 PNL=+164.05€
  - _Datos_: n=346 IC=+0.121 PNL=+164.05€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=727 IC=+0.154 PNL=+365.97€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=727 IC=+0.154 PNL=+365.97€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.207 > 0.08 con n=254 PNL=+140.34€
  - _Datos_: n=254 IC=+0.207 PNL=+140.34€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.248 > 0.08 con n=602 PNL=-65.13€
  - _Datos_: n=602 IC=+0.248 PNL=-65.13€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=988 IC=+0.098 PNL=-219.47€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=988 IC=+0.098 PNL=-219.47€
