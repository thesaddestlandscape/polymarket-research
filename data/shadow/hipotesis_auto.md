# Hipótesis automáticas — 2026-08-21 14:49 UTC
_Generado por shadow_postmortem.py sobre 102357 resoluciones (PNL=+8532.14€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `156.21` → IC=-0.263 (n=1439)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 156.21
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=4318)
...
- **FILTRO** `restante_s_al_confirmar` < `130.44` → IC=-0.325 (n=141)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=425)
...
- **PATRÓN** `py_entrada` < `0.335` → IC=+0.267 (n=2017)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.335 (IC base=+0.135)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.185 (n=2289)

  - _Acción_: Kelly boost +0.92€ cuando `libro_spread` < 0.02 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `4243.5404` → IC=+0.172 (n=953)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 4243.5404 (IC base=+0.135)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.214 (n=449)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.200)

- **PATRÓN** `py_entrada` > `0.785` → IC=+0.377 (n=153)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.187 (n=3296)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 5.0 (IC base=+0.186)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.193 (n=2912)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.165 (n=553)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.186 (n=829)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=842)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.7` → IC=+0.182 (n=284)

  - _Acción_: Kelly boost +0.91€ cuando `py_entrada` < 0.7 (IC base=+0.160)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.165 (n=466)

  - _Acción_: Kelly boost +0.82€ cuando `py_entrada` > 0.73 (IC base=+0.160)

### FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.224 (n=749)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.223)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.230 (n=658)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.192 (n=818)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.198 (n=7034)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.227 (n=5158)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.160 (n=938)

  - _Acción_: Kelly boost +0.80€ cuando `py_entrada` > 0.71 (IC base=+0.116)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.250 (n=1141)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.246 (n=1121)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.240 (n=1016)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.206 (n=918)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.133)

- **PATRÓN** `restante_min` < `3.85` → IC=+0.162 (n=836)

  - _Acción_: Kelly boost +0.81€ cuando `restante_min` < 3.85 (IC base=+0.133)

- **PATRÓN** `restante_min` > `4.91` → IC=+0.155 (n=895)

  - _Acción_: Kelly boost +0.78€ cuando `restante_min` > 4.91 (IC base=+0.133)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.149 (n=1144)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` < 7.0 (IC base=+0.133)

- **PATRÓN** `lag_apertura_s` < `5.27` → IC=+0.158 (n=838)

  - _Acción_: Kelly boost +0.79€ cuando `lag_apertura_s` < 5.27 (IC base=+0.133)
...
- **PATRÓN** `restante_min` < `3.75` → IC=+0.184 (n=416)

  - _Acción_: Kelly boost +0.92€ cuando `restante_min` < 3.75 (IC base=+0.147)

- **PATRÓN** `restante_min` > `4.89` → IC=+0.166 (n=423)

  - _Acción_: Kelly boost +0.83€ cuando `restante_min` > 4.89 (IC base=+0.147)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.174 (n=421)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` < 5.0 (IC base=+0.147)

- **PATRÓN** `lag_apertura_s` < `6.79` → IC=+0.167 (n=415)

  - _Acción_: Kelly boost +0.83€ cuando `lag_apertura_s` < 6.79 (IC base=+0.147)
...
- **PATRÓN** `restante_min` > `4.95` → IC=+0.164 (n=426)

  - _Acción_: Kelly boost +0.82€ cuando `restante_min` > 4.95 (IC base=+0.118)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.125 (n=1286)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` < 17.0 (IC base=+0.118)

- **PATRÓN** `lag_apertura_s` < `3.28` → IC=+0.164 (n=421)

  - _Acción_: Kelly boost +0.82€ cuando `lag_apertura_s` < 3.28 (IC base=+0.118)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.289 (n=230)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.273)

- **PATRÓN** `libro_liquidez` > `749.4432` → IC=+0.284 (n=230)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 749.4432 (IC base=+0.273)
...
- **PATRÓN** `ibs_20min` > `0.9743` → IC=+0.262 (n=515)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9743 (IC base=+0.090)

- **PATRÓN** `dist_vwap_pct` > `0.2276` → IC=+0.238 (n=170)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2276 (IC base=+0.090)

- **PATRÓN** `dist_vwap_pct` < `0.9606` → IC=+0.236 (n=271)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.9606 (IC base=+0.090)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.918` → IC=+0.216 (n=693)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.918 (IC base=+0.090)

- **PATRÓN** `volumen_regimen` < `0.7207` → IC=+0.227 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7207 (IC base=+0.090)
...
- **PATRÓN** `volumen_spike_ratio` < `3.2031` → IC=+0.121 (n=862)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_spike_ratio` < 3.2031 (IC base=+0.090)

- **PATRÓN** `volumen_spike_ratio` > `1.502` → IC=+0.132 (n=861)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 1.502 (IC base=+0.090)

- **PATRÓN** `ibs_20min` < `0.4` → IC=+0.134 (n=1743)

  - _Acción_: Kelly boost +0.67€ cuando `ibs_20min` < 0.4 (IC base=+0.049)

- **PATRÓN** `dist_vwap_pct` < `0.1267` → IC=+0.145 (n=581)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.1267 (IC base=+0.049)

- **PATRÓN** `volumen_regimen` < `0.6264` → IC=+0.135 (n=195)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 0.6264 (IC base=+0.049)

- **PATRÓN** `volumen_regimen` > `1.2618` → IC=+0.140 (n=195)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 1.2618 (IC base=+0.049)
...
- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.253 (n=160)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 135.0 (IC base=+0.049)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4191` → IC=+0.245 (n=190)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4191 (IC base=+0.232)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.264 (n=193)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.232)

- **PATRÓN** `ibs_20min` > `0.4893` → IC=+0.249 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4893 (IC base=+0.232)

- **PATRÓN** `dist_vwap_pct` > `0.1697` → IC=+0.245 (n=139)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1697 (IC base=+0.232)

- **PATRÓN** `dist_vwap_pct` < `1.0969` → IC=+0.246 (n=191)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 1.0969 (IC base=+0.232)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.507` → IC=+0.315 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.507 (IC base=+0.232)

- **PATRÓN** `volumen_regimen` > `0.9107` → IC=+0.250 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.9107 (IC base=+0.232)

- **PATRÓN** `volumen_pendiente_norm` < `0.0845` → IC=+0.255 (n=153)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0845 (IC base=+0.232)

- **PATRÓN** `volumen_pendiente_norm` > `0.2969` → IC=+0.308 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2969 (IC base=+0.232)

- **PATRÓN** `volumen_spike_ratio` < `1.4811` → IC=+0.273 (n=73)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4811 (IC base=+0.232)

- **PATRÓN** `volumen_spike_ratio` > `1.6989` → IC=+0.241 (n=110)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.6989 (IC base=+0.232)

- **PATRÓN** `libro_liquidez` > `11790.5042` → IC=+0.273 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11790.5042 (IC base=+0.232)

- **PATRÓN** `ballena_activa_n` < `288.0` → IC=+0.250 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 288.0 (IC base=+0.232)

- **PATRÓN** `sigma_h` < `0.0022` → IC=+0.176 (n=140)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` < 0.0022 (IC base=+0.134)

- **PATRÓN** `drift_60min` |x|≤ `0.0826` → IC=+0.169 (n=140)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.0826 (IC base=+0.134)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.144 (n=293)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 8.0 (IC base=+0.134)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.139 (n=328)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` < 18.0 (IC base=+0.134)

- **PATRÓN** `ibs_20min` < `0.3841` → IC=+0.176 (n=279)

  - _Acción_: Kelly boost +0.88€ cuando `ibs_20min` < 0.3841 (IC base=+0.134)
...
- **PATRÓN** `volumen_regimen` < `0.6237` → IC=+0.148 (n=106)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.6237 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` > `1.256` → IC=+0.157 (n=106)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 1.256 (IC base=+0.134)

- **PATRÓN** `volumen_pendiente_norm` > `0.1151` → IC=+0.247 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1151 (IC base=+0.134)

- **PATRÓN** `volumen_spike_ratio` < `1.576` → IC=+0.198 (n=94)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 1.576 (IC base=+0.134)

- **PATRÓN** `libro_liquidez` > `12133.9239` → IC=+0.164 (n=144)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 12133.9239 (IC base=+0.134)
...
- **FILTRO** `ibs_20min` > `0.8879` → IC=-0.192 (n=144)
...
- **FILTRO** `sigma_ewma_delta_pct` > `4.656` → IC=-0.141 (n=218)
...
- **FILTRO** `ibs_20min` > `0.6061` → IC=-0.127 (n=298)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6061
  - _Potencial_: sin este filtro IC_bueno=+0.061 (n=581)

- **FILTRO** `sigma_ewma_delta_pct` > `8.034` → IC=-0.179 (n=104)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.729` → IC=+0.131 (n=82)
  ...
  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` > 6.729 (IC base=-0.038)
...
- **PATRÓN** `ibs_20min` > `0.9474` → IC=+0.248 (n=658)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9474 (IC base=+0.051)

- **PATRÓN** `dist_vwap_pct` > `0.3119` → IC=+0.246 (n=258)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3119 (IC base=+0.051)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.304` → IC=+0.128 (n=1177)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 2.304 (IC base=+0.051)

- **PATRÓN** `volumen_regimen` > `1.1814` → IC=+0.173 (n=197)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 1.1814 (IC base=+0.051)
...
- **PATRÓN** `ballena_activa_n` < `86.0` → IC=+0.285 (n=282)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 86.0 (IC base=+0.051)

- **PATRÓN** `ibs_20min` < `0.1071` → IC=+0.165 (n=964)

  - _Acción_: Kelly boost +0.82€ cuando `ibs_20min` < 0.1071 (IC base=+0.039)

- **PATRÓN** `dist_vwap_pct` > `0.6319` → IC=+0.207 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6319 (IC base=+0.039)

- **PATRÓN** `dist_vwap_pct` < `0.2752` → IC=+0.179 (n=565)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.2752 (IC base=+0.039)

- **PATRÓN** `volumen_regimen` > `0.6268` → IC=+0.188 (n=553)
...
- **PATRÓN** `ballena_activa_n` < `68.0` → IC=+0.235 (n=202)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 68.0 (IC base=+0.039)
...
- **PATRÓN** `volumen_regimen` < `0.5607` → IC=+0.140 (n=23)
  ...
  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` < 0.5607 (IC base=-0.029)

- **PATRÓN** `volumen_regimen` > `1.1074` → IC=+0.167 (n=22)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 1.1074 (IC base=-0.029)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.141 (n=51)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=785)
...
- **PATRÓN** `ibs_20min` > `0.6111` → IC=+0.209 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6111 (IC base=-0.008)

- **PATRÓN** `dist_vwap_pct` > `0.1723` → IC=+0.214 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1723 (IC base=-0.008)

- **PATRÓN** `volumen_regimen` > `1.1259` → IC=+0.176 (n=32)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 1.1259 (IC base=-0.008)

- **PATRÓN** `volumen_pendiente_norm` > `0.2269` → IC=+0.214 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2269 (IC base=-0.008)

- **PATRÓN** `volumen_spike_ratio` > `2.0579` → IC=+0.134 (n=39)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 2.0579 (IC base=-0.008)

- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.214 (n=33)
...
- **PATRÓN** `ibs_20min` > `0.913` → IC=+0.261 (n=253)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.913 (IC base=+0.183)

- **PATRÓN** `dist_vwap_pct` > `0.6283` → IC=+0.330 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6283 (IC base=+0.183)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.12` → IC=+0.259 (n=243)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.12 (IC base=+0.183)

- **PATRÓN** `volumen_regimen` > `0.8361` → IC=+0.222 (n=253)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8361 (IC base=+0.183)

- **PATRÓN** `volumen_pendiente_norm` > `0.1846` → IC=+0.273 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1846 (IC base=+0.183)

- **PATRÓN** `volumen_spike_ratio` < `1.9866` → IC=+0.206 (n=216)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9866 (IC base=+0.183)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.198 (n=399)

  - _Acción_: Kelly boost +0.99€ cuando `libro_spread` < 0.01 (IC base=+0.183)

- **PATRÓN** `libro_liquidez` > `2694.456` → IC=+0.214 (n=253)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2694.456 (IC base=+0.183)

- **PATRÓN** `ballena_activa_n` < `41.0` → IC=+0.363 (n=100)
...
- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.260 (n=390)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0049 (IC base=+0.259)

- **PATRÓN** `drift_60min` |x|≤ `0.3804` → IC=+0.271 (n=391)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3804 (IC base=+0.259)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.318 (n=196)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.275` → IC=+0.338 (n=344)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.275 (IC base=+0.259)

- **PATRÓN** `dist_vwap_pct` > `0.5126` → IC=+0.261 (n=90)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.693` → IC=+0.273 (n=64)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.693 (IC base=+0.259)

- **PATRÓN** `volumen_regimen` > `1.0826` → IC=+0.299 (n=177)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0826 (IC base=+0.259)
...
- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.202 (n=756)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.9091` → IC=+0.257 (n=1111)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9091 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` > `0.7681` → IC=+0.243 (n=306)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7681 (IC base=+0.153)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.335` → IC=+0.289 (n=539)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.335 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` > `0.7094` → IC=+0.169 (n=909)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 0.7094 (IC base=+0.153)

- **PATRÓN** `volumen_pendiente_norm` > `0.1053` → IC=+0.182 (n=583)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.1053 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` < `1.981` → IC=+0.140 (n=950)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 1.981 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` > `1.4889` → IC=+0.143 (n=1425)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 1.4889 (IC base=+0.153)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.165 (n=1720)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.04 (IC base=+0.153)

- **PATRÓN** `libro_liquidez` > `3246.091` → IC=+0.200 (n=555)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3246.091 (IC base=+0.153)
...
- **PATRÓN** `sigma_h` < `0.0065` → IC=+0.213 (n=193)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0065 (IC base=+0.198)

- **PATRÓN** `sigma_h` > `0.0034` → IC=+0.208 (n=128)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0034 (IC base=+0.198)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.239 (n=197)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.198)

- **PATRÓN** `ibs_20min` > `0.5283` → IC=+0.242 (n=192)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5283 (IC base=+0.198)

- **PATRÓN** `dist_vwap_pct` > `0.2276` → IC=+0.257 (n=146)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2276 (IC base=+0.198)
...
- **PATRÓN** `volumen_regimen` > `0.9306` → IC=+0.246 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.9306 (IC base=+0.198)

- **PATRÓN** `volumen_pendiente_norm` > `0.2969` → IC=+0.300 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2969 (IC base=+0.198)

- **PATRÓN** `volumen_spike_ratio` < `1.4452` → IC=+0.254 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4452 (IC base=+0.198)

- **PATRÓN** `volumen_spike_ratio` > `2.6612` → IC=+0.219 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6612 (IC base=+0.198)

- **PATRÓN** `libro_liquidez` > `11088.6716` → IC=+0.246 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11088.6716 (IC base=+0.198)
...
- **PATRÓN** `sigma_h` < `0.0084` → IC=+0.250 (n=178)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0084 (IC base=+0.220)

- **PATRÓN** `drift_60min` |x|≤ `0.0871` → IC=+0.306 (n=60)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0871 (IC base=+0.220)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.231 (n=184)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.4752` → IC=+0.284 (n=179)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4752 (IC base=+0.220)

- **PATRÓN** `dist_vwap_pct` > `0.1741` → IC=+0.237 (n=131)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1741 (IC base=+0.220)

- **PATRÓN** `dist_vwap_pct` < `0.3391` → IC=+0.243 (n=134)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3391 (IC base=+0.220)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.691` → IC=+0.405 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.691 (IC base=+0.220)

- **PATRÓN** `volumen_regimen` > `0.6447` → IC=+0.250 (n=178)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6447 (IC base=+0.220)

- **PATRÓN** `volumen_pendiente_norm` > `0.1034` → IC=+0.295 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1034 (IC base=+0.220)

- **PATRÓN** `volumen_spike_ratio` < `1.467` → IC=+0.229 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.467 (IC base=+0.220)

- **PATRÓN** `volumen_spike_ratio` > `2.6068` → IC=+0.263 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6068 (IC base=+0.220)

- **PATRÓN** `ballena_activa_n` < `221.0` → IC=+0.206 (n=100)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 221.0 (IC base=+0.220)
...
- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.197 (n=140)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0072 (IC base=+0.090)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.139 (n=117)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 17.0 (IC base=+0.090)

- **PATRÓN** `ibs_20min` > `0.8621` → IC=+0.214 (n=204)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8621 (IC base=+0.090)

- **PATRÓN** `dist_vwap_pct` > `0.6671` → IC=+0.256 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6671 (IC base=+0.090)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.581` → IC=+0.237 (n=93)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.581 (IC base=+0.090)

- **PATRÓN** `libro_liquidez` > `2667.202` → IC=+0.167 (n=139)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 2667.202 (IC base=+0.090)

- **PATRÓN** `ballena_activa_n` < `36.0` → IC=+0.200 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 36.0 (IC base=+0.090)
...
- **PATRÓN** `sigma_h` > `0.0066` → IC=+0.248 (n=228)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.246 (n=215)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.154)

- **PATRÓN** `dist_vwap_pct` > `0.2968` → IC=+0.220 (n=187)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2968 (IC base=+0.154)
...
- **PATRÓN** `volumen_regimen` < `0.6051` → IC=+0.164 (n=114)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.6051 (IC base=+0.154)

- **PATRÓN** `volumen_regimen` > `0.8449` → IC=+0.165 (n=228)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 0.8449 (IC base=+0.154)
...
- **PATRÓN** `volumen_spike_ratio` < `2.6857` → IC=+0.172 (n=306)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 2.6857 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` > `1.4633` → IC=+0.156 (n=306)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.213 (n=639)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0075 (IC base=+0.135)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.159 (n=936)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 15.0 (IC base=+0.135)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.147 (n=718)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 6.0 (IC base=+0.135)

- **PATRÓN** `ibs_20min` > `0.96` → IC=+0.286 (n=863)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.96 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` > `0.704` → IC=+0.226 (n=308)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.704 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.046` → IC=+0.264 (n=865)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.046 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` < `0.7112` → IC=+0.146 (n=520)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.7112 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` > `1.0983` → IC=+0.143 (n=536)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 1.0983 (IC base=+0.135)

- **PATRÓN** `volumen_pendiente_norm` > `0.2446` → IC=+0.168 (n=329)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_pendiente_norm` > 0.2446 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` > `1.632` → IC=+0.141 (n=1424)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` > 1.632 (IC base=+0.135)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.146 (n=1928)

  - _Acción_: Kelly boost +0.73€ cuando `libro_spread` < 0.04 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `2738.706` → IC=+0.181 (n=863)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 2738.706 (IC base=+0.135)

- **PATRÓN** `ballena_activa_n` < `162.0` → IC=+0.203 (n=463)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 162.0 (IC base=+0.135)

- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.214 (n=1744)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0076 (IC base=+0.192)

- **PATRÓN** `drift_60min` |x|≤ `0.3399` → IC=+0.202 (n=1742)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3399 (IC base=+0.192)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.215 (n=636)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.192)

- **PATRÓN** `ibs_20min` < `0.5333` → IC=+0.256 (n=1742)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5333 (IC base=+0.192)

- **PATRÓN** `dist_vwap_pct` < `0.6974` → IC=+0.178 (n=1386)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.6974 (IC base=+0.192)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.513` → IC=+0.196 (n=1633)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` < 2.513 (IC base=+0.192)
...
- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.127 (n=414)
...
- **PATRÓN** `sigma_h` < `0.0022` → IC=+0.171 (n=83)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0022 (IC base=+0.152)

- **PATRÓN** `sigma_h` > `0.0033` → IC=+0.173 (n=163)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0033 (IC base=+0.152)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.180 (n=254)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 5.0 (IC base=+0.152)

- **PATRÓN** `ibs_20min` > `0.3307` → IC=+0.229 (n=245)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3307 (IC base=+0.152)

- **PATRÓN** `dist_vwap_pct` > `0.2039` → IC=+0.240 (n=152)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2039 (IC base=+0.152)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.777` → IC=+0.265 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.777 (IC base=+0.152)

- **PATRÓN** `volumen_regimen` < `0.6746` → IC=+0.167 (n=82)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.6746 (IC base=+0.152)

- **PATRÓN** `volumen_regimen` > `0.9228` → IC=+0.173 (n=163)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` > 0.9228 (IC base=+0.152)

- **PATRÓN** `volumen_pendiente_norm` < `0.1037` → IC=+0.186 (n=186)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` < 0.1037 (IC base=+0.152)

- **PATRÓN** `volumen_pendiente_norm` > `0.151` → IC=+0.183 (n=58)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.151 (IC base=+0.152)

- **PATRÓN** `volumen_spike_ratio` < `2.6687` → IC=+0.210 (n=198)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.6687 (IC base=+0.152)

- **PATRÓN** `volumen_spike_ratio` > `1.3711` → IC=+0.200 (n=198)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.3711 (IC base=+0.152)

- **PATRÓN** `libro_liquidez` > `9614.35` → IC=+0.221 (n=163)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 9614.35 (IC base=+0.152)

- **PATRÓN** `sigma_h` < `0.0022` → IC=+0.200 (n=138)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0022 (IC base=+0.162)

- **PATRÓN** `drift_60min` |x|≤ `0.307` → IC=+0.172 (n=312)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.307 (IC base=+0.162)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.167 (n=280)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 8.0 (IC base=+0.162)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.168 (n=323)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 18.0 (IC base=+0.162)

- **PATRÓN** `ibs_20min` < `0.4901` → IC=+0.207 (n=312)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4901 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` < `0.2672` → IC=+0.189 (n=307)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.2672 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.57` → IC=+0.244 (n=178)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.57 (IC base=+0.162)

- **PATRÓN** `volumen_regimen` < `0.6183` → IC=+0.217 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6183 (IC base=+0.162)

- **PATRÓN** `volumen_regimen` > `0.8408` → IC=+0.181 (n=208)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` > 0.8408 (IC base=+0.162)

- **PATRÓN** `volumen_pendiente_norm` > `0.166` → IC=+0.300 (n=68)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.166 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` < `1.4377` → IC=+0.233 (n=73)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4377 (IC base=+0.162)

- **PATRÓN** `libro_liquidez` > `12236.1369` → IC=+0.189 (n=104)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 12236.1369 (IC base=+0.162)

- **PATRÓN** `ballena_activa_n` < `326.0` → IC=+0.183 (n=39)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 326.0 (IC base=+0.162)
...
- **PATRÓN** `sigma_h` < `0.0022` → IC=+0.151 (n=81)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0022 (IC base=+0.139)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0727` → IC=+0.163 (n=81)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.0727 (IC base=+0.139)
...
- **PATRÓN** `ibs_20min` > `0.3749` → IC=+0.217 (n=242)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3749 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` > `0.2593` → IC=+0.198 (n=114)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.2593 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.513` → IC=+0.202 (n=139)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.513 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` < `0.7478` → IC=+0.197 (n=107)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` < 0.7478 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` > `1.1345` → IC=+0.143 (n=110)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 1.1345 (IC base=+0.139)
...
- **PATRÓN** `volumen_spike_ratio` > `1.435` → IC=+0.226 (n=210)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.435 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `3655.4485` → IC=+0.212 (n=161)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3655.4485 (IC base=+0.139)

- **PATRÓN** `ballena_activa_n` < `235.0` → IC=+0.241 (n=114)
...
- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.183 (n=99)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0072 (IC base=+0.057)

- **PATRÓN** `ibs_20min` > `0.931` → IC=+0.244 (n=135)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.931 (IC base=+0.057)

- **PATRÓN** `dist_vwap_pct` > `0.6367` → IC=+0.189 (n=72)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.6367 (IC base=+0.057)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.234` → IC=+0.237 (n=116)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.234 (IC base=+0.057)

- **PATRÓN** `libro_liquidez` > `2545.7704` → IC=+0.193 (n=99)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 2545.7704 (IC base=+0.057)

- **PATRÓN** `ballena_activa_n` < `64.0` → IC=+0.242 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 64.0 (IC base=+0.057)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0881` → IC=+0.189 (n=133)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.0881 (IC base=+0.157)
...
- **PATRÓN** `dist_vwap_pct` > `0.2833` → IC=+0.250 (n=178)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2833 (IC base=+0.157)
...
- **PATRÓN** `volumen_regimen` < `0.5999` → IC=+0.181 (n=133)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.5999 (IC base=+0.157)
...
- **PATRÓN** `volumen_spike_ratio` > `1.6588` → IC=+0.167 (n=313)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.6588 (IC base=+0.157)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.169 (n=421)
...
- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.233 (n=114)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3884` → IC=+0.232 (n=445)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3884 (IC base=+0.213)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.272 (n=222)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 14.0 (IC base=+0.213)

- **PATRÓN** `ibs_20min` < `0.4611` → IC=+0.285 (n=445)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4611 (IC base=+0.213)

- **PATRÓN** `dist_vwap_pct` < `0.6738` → IC=+0.224 (n=509)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6738 (IC base=+0.213)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.929` → IC=+0.250 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.929 (IC base=+0.213)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.873` → IC=+0.223 (n=452)
...
- **PATRÓN** `py_entrada` < `0.485` → IC=+0.192 (n=24)
  ...
  - _Acción_: Kelly boost +0.96€ cuando `py_entrada` < 0.485 (IC base=+0.171)

- **PATRÓN** `py_entrada` > `0.5` → IC=+0.214 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.5 (IC base=+0.171)

- **PATRÓN** `btc_momentum` |x|≤ `0.01` → IC=+0.180 (n=23)

  - _Acción_: Kelly boost +0.90€ cuando `btc_momentum` |x|≤ 0.01 (IC base=+0.171)

- **PATRÓN** `libro_liquidez` > `2724.1553` → IC=+0.207 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2724.1553 (IC base=+0.171)
...
- **FILTRO** `libro_liquidez` < `3812.0159` → IC=-0.176 (n=32)
...
- **FILTRO** `py_entrada` < `0.44` → IC=-0.174 (n=314)

  - _Acción_: SKIP cuando `py_entrada` < 0.44
  - _Potencial_: sin este filtro IC_bueno=+0.058 (n=1011)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.239 (n=339)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=1044)

- **FILTRO** `ibs_20min` > `0.2724` → IC=-0.223 (n=345)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2724
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=1038)
...
- **FILTRO** `ibs_20min` < `0.8288` → IC=-0.136 (n=97)

  - _Acción_: SKIP cuando `ibs_20min` < 0.8288
  - _Potencial_: sin este filtro IC_bueno=+0.140 (n=98)

- **FILTRO** `ibs_20min` > `0.1842` → IC=-0.172 (n=114)

  - _Acción_: SKIP cuando `ibs_20min` > 0.1842
  - _Potencial_: sin este filtro IC_bueno=+0.076 (n=116)

- **PATRÓN** `ibs_20min` > `0.8288` → IC=+0.140 (n=98)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` > 0.8288 (IC base=+0.003)
...
- **FILTRO** `ibs_20min` < `0.9522` → IC=-0.121 (n=130)

  - _Acción_: SKIP cuando `ibs_20min` < 0.9522
...
- **PATRÓN** `ibs_20min` > `0.9522` → IC=+0.152 (n=67)

  - _Acción_: Kelly boost +0.76€ cuando `ibs_20min` > 0.9522 (IC base=-0.028)

### MOMENTUM_IBS_15M_BALLENA#ETH#15min
- **FILTRO** `py_entrada` > `0.505` → IC=-0.191 (n=53)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.013 (n=195)

- **FILTRO** `ballena_activa_n` > `3.0` → IC=-0.130 (n=79)
...
- **FILTRO** `py_entrada` < `0.36` → IC=-0.269 (n=834)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=2505)

- **FILTRO** `ibs_7min` < `0.7333` → IC=-0.214 (n=830)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7333
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=2509)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.173 (n=1108)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=2231)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.221 (n=979)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=3056)

- **FILTRO** `ibs_7min` > `0.7027` → IC=-0.157 (n=1008)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7027
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=3027)
...
- **FILTRO** `ibs_7min` < `0.9405` → IC=-0.181 (n=312)
...
- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.213 (n=113)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=361)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.220 (n=180)
...
- **FILTRO** `drift_7min_pct` |x|> `0.1303` → IC=-0.185 (n=211)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=410)

- **FILTRO** `ibs_7min` > `0.8182` → IC=-0.188 (n=155)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8182
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=466)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.143 (n=152)
...
- **FILTRO** `ibs_7min` < `0.1975` → IC=-0.257 (n=113)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.258 (n=151)
...
- **FILTRO** `ibs_7min` < `0.7958` → IC=-0.189 (n=146)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7958
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=441)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.253 (n=148)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=464)

- **FILTRO** `ibs_7min` > `0.166` → IC=-0.124 (n=208)

  - _Acción_: SKIP cuando `ibs_7min` > 0.166
...
- **FILTRO** `ballena_activa_n` > `29.0` → IC=-0.193 (n=151)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=461)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.4` → IC=-0.203 (n=153)

  - _Acción_: SKIP cuando `py_entrada` < 0.4
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=470)
...
- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.213 (n=148)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=+0.016 (n=475)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.168 (n=185)
...
- **FILTRO** `drift_7min_pct` |x|> `0.1355` → IC=-0.151 (n=328)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1355
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=330)
...
- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.144 (n=220)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.129 (n=33)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=471)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.154 (n=24)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=518)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.676` → IC=+0.123 (n=67)
  ...
  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 8.676 (IC base=+0.046)
...
- **FILTRO** `ibs_15` < `0.3587` → IC=-0.327 (n=73)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.147 (n=219)

- **FILTRO** `sigma_h` > `0.0086` → IC=-0.177 (n=428)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0086
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=1285)
...
- **PATRÓN** `ibs_15` > `0.3587` → IC=+0.147 (n=219)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **FILTRO** `sigma_h` > `0.0058` → IC=-0.232 (n=95)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0058
  - _Potencial_: sin este filtro IC_bueno=-0.196 (n=287)

- **FILTRO** `drift_15min` |x|> `0.7433` → IC=-0.211 (n=95)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 0.7433
  - _Potencial_: sin este filtro IC_bueno=-0.202 (n=287)
...
- **FILTRO** `sigma_ewma_delta_pct` > `20.745` → IC=-0.238 (n=82)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.195 (n=300)
...
- **FILTRO** `sigma_h` > `0.0077` → IC=-0.142 (n=205)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `sigma_ewma_delta_pct` > `12.12` → IC=-0.203 (n=99)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=506)
...
### UPDOWN_GBM_IBS_ALTO
- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.363 (n=49)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0046 (IC base=+0.292)

- **PATRÓN** `drift_60min` |x|≤ `0.1853` → IC=+0.299 (n=147)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1853 (IC base=+0.292)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1832` → IC=+0.341 (n=67)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1832 (IC base=+0.292)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2105` → IC=+0.403 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.2105 (IC base=+0.292)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.328 (n=132)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.292)

- **PATRÓN** `ibs_15` > `0.9353` → IC=+0.360 (n=98)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.9353 (IC base=+0.292)

- **PATRÓN** `dist_vwap_pct` > `0.5182` → IC=+0.400 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5182 (IC base=+0.292)

- **PATRÓN** `dist_vwap_pct` < `0.0834` → IC=+0.317 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0834 (IC base=+0.292)

- **PATRÓN** `sigma_ewma_delta_pct` < `20.416` → IC=+0.301 (n=149)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 20.416 (IC base=+0.292)

- **PATRÓN** `libro_liquidez` > `6210.7857` → IC=+0.300 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 6210.7857 (IC base=+0.292)

- **PATRÓN** `ballena_activa_n` < `190.0` → IC=+0.352 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 190.0 (IC base=+0.292)

### UPDOWN_GBM_IBS_ALTO#BTC#15min
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.2` → IC=+0.284 (n=86)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +1.00€ cuando `pct_spot_vs_ref` |x|≤ 0.2 (IC base=+0.267)

- **PATRÓN** `sigma_h` < `0.0026` → IC=+0.267 (n=58)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0026 (IC base=+0.267)

- **PATRÓN** `sigma_h` > `0.0036` → IC=+0.274 (n=29)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0036 (IC base=+0.267)

- **PATRÓN** `drift_60min` |x|≤ `0.1837` → IC=+0.307 (n=86)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1837 (IC base=+0.267)

- **PATRÓN** `drift_15min` |x|≤ `0.5909` → IC=+0.295 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.5909 (IC base=+0.267)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0679` → IC=+0.273 (n=86)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0679 (IC base=+0.267)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.312 (n=83)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.267)

- **PATRÓN** `ibs_15` > `0.8845` → IC=+0.310 (n=77)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8845 (IC base=+0.267)

- **PATRÓN** `dist_vwap_pct` > `0.3842` → IC=+0.375 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3842 (IC base=+0.267)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `27.622` → IC=+0.292 (n=22)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 27.622 (IC base=+0.267)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.699` → IC=+0.269 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 7.699 (IC base=+0.267)

- **PATRÓN** `libro_liquidez` > `9958.5281` → IC=+0.281 (n=39)
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.9353 sube el IC de +0.292 a +0.360 en UPDOWN_GBM_IBS_ALTO (n=98). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_IBS_ALTO#BTC#15min**: dentro de BUY_YES, IBS > 0.8845 sube el IC de +0.267 a +0.310 en UPDOWN_GBM_IBS_ALTO#BTC#15min (n=77). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 4694 | -0.103 | -544.12€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 549 | -0.070 | -86.36€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 4145 | -0.107 | -457.76€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 566 | -0.129 | -79.49€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 566 | -0.129 | -79.49€ | 2 | 0 |
...
| ✅ BALLENAS_TARDIAS#XRP | 1135 | -0.189 | -209.14€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1135 | -0.189 | -209.14€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 20189 | +0.113 | -1430.39€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 4191 | +0.184 | -161.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 106 | -0.102 | -48.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 13098 | +0.088 | -1222.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2794 | +0.129 | +3.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 2206 | +0.047 | -452.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 16 | -0.089 | -1.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 2185 | +0.050 | -444.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 4365 | +0.137 | -75.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1159 | +0.193 | -68.86€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 2178 | +0.109 | -57.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 986 | +0.142 | +73.05€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 2210 | +0.064 | -360.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 15 | +0.022 | -2.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 2194 | +0.064 | -356.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 4710 | +0.127 | -77.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1545 | +0.168 | -8.43€ | 0 | 6 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 2178 | +0.106 | -54.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 975 | +0.111 | -6.49€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 4493 | +0.127 | -352.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1442 | +0.199 | -78.87€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 45 | +0.011 | -8.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 2173 | +0.078 | -202.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 833 | +0.135 | -63.52€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 2205 | +0.118 | -110.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 14 | +0.000 | -0.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 2190 | +0.119 | -108.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 4309 | +0.170 | -356.36€ | 2 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 4309 | +0.170 | -356.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 1082 | +0.164 | -124.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 1082 | +0.164 | -124.14€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 1074 | +0.160 | -132.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 1074 | +0.160 | -132.15€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 963 | +0.223 | -38.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 963 | +0.223 | -38.30€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 1022 | +0.190 | -75.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 1022 | +0.190 | -75.18€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 9341 | +0.196 | -815.06€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 9341 | +0.196 | -815.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 1747 | +0.116 | -355.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 1747 | +0.116 | -355.59€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 1446 | +0.240 | -31.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 1446 | +0.240 | -31.59€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 1631 | +0.156 | -234.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 1631 | +0.156 | -234.60€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 1478 | +0.242 | -27.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 1478 | +0.242 | -27.06€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 1472 | +0.238 | -36.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 1472 | +0.238 | -36.45€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 1567 | +0.200 | -129.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 1567 | +0.200 | -129.76€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 3342 | +0.133 | +77.87€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 3342 | +0.133 | +77.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 1659 | +0.147 | +81.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 1659 | +0.147 | +81.01€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 