# Hipótesis automáticas — 2026-07-26 03:25 UTC
_Generado por shadow_postmortem.py sobre 35936 resoluciones (PNL=+7639.02€)_
...
- **PATRÓN** `dist_vwap_pct` > `0.4739` → IC=+0.132 (n=308)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.4739 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` < `0.1211` → IC=+0.124 (n=1031)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.1211 (IC base=+0.116)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.441` → IC=+0.208 (n=501)
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.192 (n=1063)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.278` → IC=+0.196 (n=442)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.278 (IC base=+0.149)
...
- **PATRÓN** `sigma_h` > `0.0079` → IC=+0.199 (n=134)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0079 (IC base=+0.122)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.148 (n=268)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 12.0 (IC base=+0.122)

- **PATRÓN** `dist_vwap_pct` > `0.1126` → IC=+0.159 (n=171)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.1126 (IC base=+0.122)
...
- **PATRÓN** `sigma_h` < `0.0179` → IC=+0.264 (n=375)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0129` → IC=+0.146 (n=510)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` > 0.0129 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.122 (n=1432)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 7.0 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` > `0.4746` → IC=+0.161 (n=293)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.4746 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.474` → IC=+0.270 (n=241)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.474 (IC base=+0.110)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.62` → IC=+0.243 (n=138)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.62 (IC base=+0.067)
...
- **PATRÓN** `sigma_h` < `0.0209` → IC=+0.187 (n=417)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0209 (IC base=+0.181)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.181 (n=425)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` < 15.0 (IC base=+0.181)

- **PATRÓN** `dist_vwap_pct` > `0.2608` → IC=+0.227 (n=141)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2608 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.005` → IC=+0.302 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.005 (IC base=+0.181)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.761` → IC=+0.229 (n=429)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.761 (IC base=+0.058)

- **PATRÓN** `dist_vwap_pct` > `0.168` → IC=+0.183 (n=197)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.168 (IC base=+0.058)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.108` → IC=+0.165 (n=180)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 7.108 (IC base=+0.058)
...
- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.126 (n=265)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.63€ cuando `sigma_h` < 0.0063 (IC base=+0.102)

- **PATRÓN** `drift_60min` |x|≤ `0.2217` → IC=+0.140 (n=292)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.70€ cuando `drift_60min` |x|≤ 0.2217 (IC base=+0.102)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0862` → IC=+0.126 (n=260)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.63€ cuando `delta_ratio_macro` |x|> 0.0862 (IC base=+0.102)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.159 (n=89)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 8.0 (IC base=+0.102)

- **PATRÓN** `ibs_15` > `0.7248` → IC=+0.249 (n=225)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7248 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` > `0.2554` → IC=+0.232 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2554 (IC base=+0.102)

- **PATRÓN** `sigma_ewma_delta_pct` > `24.96` → IC=+0.167 (n=34)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 24.96 (IC base=+0.102)

- **PATRÓN** `sigma_ewma_delta_pct` < `14.681` → IC=+0.156 (n=126)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` < 14.681 (IC base=+0.102)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1851` → IC=+0.122 (n=146)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.61€ cuando `delta_ratio_macro` |x|> 0.1851 (IC base=+0.048)

- **PATRÓN** `ibs_15` > `0.6814` → IC=+0.188 (n=187)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.94€ cuando `ibs_15` > 0.6814 (IC base=+0.048)

- **PATRÓN** `dist_vwap_pct` > `0.1362` → IC=+0.140 (n=98)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.1362 (IC base=+0.048)

- **PATRÓN** `dist_vwap_pct` < `0.658` → IC=+0.124 (n=176)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.761 sube el IC de +0.058 a +0.229 en UPDOWN_GBM#15min (n=429). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.7248 sube el IC de +0.102 a +0.249 en UPDOWN_GBM#BTC#15min (n=225). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.6814 sube el IC de +0.048 a +0.188 en UPDOWN_GBM#ETH#15min (n=187). Ya aplicado como kelly_boost=+0.94€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 1201 | +0.122 | +53.95€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 1201 | +0.122 | +53.95€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 18 | +0.090 | +0.57€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 18 | +0.090 | +0.57€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 466 | +0.126 | +15.23€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 466 | +0.126 | +15.23€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 448 | +0.140 | +17.91€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 448 | +0.140 | +17.91€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 7892 | +0.095 | +2424.96€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 7892 | +0.095 | +2424.96€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1909 | +0.071 | +338.50€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1909 | +0.071 | +338.50€ | 0 | 5 |
| ✅ GBM_LATE_15M#ETH | 1750 | +0.074 | +300.27€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1750 | +0.074 | +300.27€ | 0 | 6 |
| ✅ GBM_LATE_15M#SOL | 2125 | +0.087 | +748.36€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 2125 | +0.087 | +748.36€ | 1 | 3 |
| ✅ GBM_LATE_15M#XRP | 2108 | +0.143 | +1037.83€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 2108 | +0.143 | +1037.83€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 5110 | +0.119 | +2463.28€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 5110 | +0.119 | +2463.28€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 1336 | +0.086 | +482.68€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 1336 | +0.086 | +482.68€ | 0 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1333 | +0.086 | +466.51€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1333 | +0.086 | +466.51€ | 0 | 6 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 1284 | +0.091 | +515.70€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 1284 | +0.091 | +515.70€ | 0 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 1157 | +0.226 | +998.40€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 1157 | +0.226 | +998.40€ | 0 | 8 |
...
| ✅ GBM_LATE_15M_TARDIO | 5110 | +0.072 | +1427.50€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 5110 | +0.072 | +1427.50€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 1227 | +0.040 | +165.96€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 1227 | +0.040 | +165.96€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 1239 | +0.026 | +98.20€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 1239 | +0.026 | +98.20€ | 0 | 1 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 1199 | +0.035 | +237.78€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 1199 | +0.035 | +237.78€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 1445 | +0.171 | +925.56€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1445 | +0.171 | +925.56€ | 0 | 10 |
...
| ✅ UPDOWN_GBM | 2632 | +0.036 | +255.33€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 2369 | +0.055 | +293.84€ | 0 | 3 |
...
| ✅ UPDOWN_GBM#BTC | 562 | +0.053 | +71.45€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 478 | +0.087 | +85.93€ | 1 | 9 |
...
| ✅ UPDOWN_GBM#ETH | 1040 | +0.040 | +77.22€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 946 | +0.055 | +88.78€ | 0 | 7 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1810 | +0.156 | +818.97€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1810 | +0.156 | +818.97€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 172 | +0.201 | +103.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 172 | +0.201 | +103.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 193 | +0.115 | +38.24€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 193 | +0.115 | +38.24€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 158 | +0.212 | +107.33€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 158 | +0.212 | +107.33€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 354 | +0.174 | +148.81€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 354 | +0.174 | +148.81€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 436 | +0.094 | +117.77€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 436 | +0.094 | +117.77€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 497 | +0.175 | +303.27€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 497 | +0.175 | +303.27€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.176: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.075 n=1410 | neutral: IC=+0.015 n=1127 | overbought(IBS>0.7): IC=+0.192 n=1433
  - _Datos_: n=4234 IC=+0.105 PNL=+1132.74€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.113 n=21469 | tras_1loss IC=+0.081 n=14260 | tras_2loss IC=+0.053 n=5930/40 | gap=+0.059 (umbral 0.05)
...
**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  ...
  - _Estado_: alineada_con_outcome_prev IC=+0.051 n=165/60 | contraria IC=+0.110 n=57 | gap=-0.059 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=-0.016 n=157 | contrario_BTC IC=-0.029 n=68/40 | gap=-0.013 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2496 IC=+0.042 PNL=+271.04€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2496 IC=+0.042 PNL=+271.04€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=1032 IC=+0.058 PNL=+153.86€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1032 IC=+0.058 PNL=+153.86€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=478 IC=+0.087 PNL=+85.93€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=478 IC=+0.087 PNL=+85.93€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.091 > 0.08 con n=1259 PNL=+226.05€
  - _Datos_: n=1259 IC=+0.091 PNL=+226.05€
...
**〰️ H-CUSTOM-CROSS-WINDOW-SPREAD-NEG** — GBM 15min/60min: spread negativo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: n=157 IC=+0.060 PNL=+65.51€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=157 IC=+0.060 PNL=+65.51€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=1182 IC=+0.031 PNL=+123.31€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1182 IC=+0.031 PNL=+123.31€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.168 > 0.1 con n=877 PNL=+325.07€
  - _Datos_: n=877 IC=+0.168 PNL=+325.07€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.135 > 0.08 con n=628 PNL=+200.11€
  - _Datos_: n=628 IC=+0.135 PNL=+200.11€
...
**🟡 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.181 > 0.08 con n=268 PNL=+107.08€
  - _Datos_: n=268 IC=+0.181 PNL=+107.08€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.104 > 0.08 con n=501 PNL=+132.60€
  - _Datos_: n=501 IC=+0.104 PNL=+132.60€
