# Hipótesis automáticas — 2026-07-27 17:52 UTC
_Generado por shadow_postmortem.py sobre 38723 resoluciones (PNL=+7918.12€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.199 (n=2249)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.193 (n=2748)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.208 (n=570)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.205)

- **PATRÓN** `py_entrada` > `0.625` → IC=+0.271 (n=491)
...
- **PATRÓN** `py_entrada` < `0.565` → IC=+0.208 (n=63)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.565 (IC base=+0.182)

- **PATRÓN** `py_entrada` > `0.635` → IC=+0.222 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.635 (IC base=+0.182)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.221 (n=503)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.221 (n=593)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.220)

- **PATRÓN** `libro_liquidez` > `2374.7716` → IC=+0.236 (n=180)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.131 (n=2452)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 7.0 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` > `0.4525` → IC=+0.129 (n=332)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` > 0.4525 (IC base=+0.116)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.034` → IC=+0.210 (n=384)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.034 (IC base=+0.116)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.132 (n=595)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.1002` → IC=+0.143 (n=281)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.1002 (IC base=+0.109)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.155 (n=276)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` > `0.0149` → IC=+0.148 (n=495)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0149 (IC base=+0.143)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.150 (n=753)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 6.0 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` > `0.6409` → IC=+0.189 (n=72)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.6409 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` < `0.1078` → IC=+0.130 (n=371)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.1078 (IC base=+0.143)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.54` → IC=+0.247 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.54 (IC base=+0.143)

- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.145 (n=940)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.1262` → IC=+0.183 (n=184)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.1262 (IC base=+0.136)
...
- **PATRÓN** `sigma_h` > `0.0074` → IC=+0.191 (n=1156)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0074 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.170 (n=1548)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 8.0 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.2749` → IC=+0.189 (n=500)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.2749 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.527` → IC=+0.235 (n=541)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.527 (IC base=+0.145)

- **PATRÓN** `sigma_h` > `0.0067` → IC=+0.151 (n=1382)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0067 (IC base=+0.117)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.132 (n=1426)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` < `0.0047` → IC=+0.133 (n=388)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.0047 (IC base=+0.115)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.152 (n=412)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 7.0 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` > `0.448` → IC=+0.212 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.448 (IC base=+0.115)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.798` → IC=+0.195 (n=80)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 11.798 (IC base=+0.115)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.197 (n=150)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0075 (IC base=+0.123)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.151 (n=299)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 12.0 (IC base=+0.123)

- **PATRÓN** `dist_vwap_pct` > `0.6749` → IC=+0.171 (n=77)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.6749 (IC base=+0.123)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.301` → IC=+0.192 (n=118)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 7.301 (IC base=+0.123)
...
- **PATRÓN** `sigma_h` < `0.0179` → IC=+0.258 (n=403)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0179 (IC base=+0.235)

- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.236 (n=403)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.4483` → IC=+0.293 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4483 (IC base=+0.235)
...
- **PATRÓN** `sigma_h` < `0.0174` → IC=+0.208 (n=535)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0174 (IC base=+0.203)

- **PATRÓN** `sigma_h` > `0.007` → IC=+0.232 (n=535)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.1883` → IC=+0.319 (n=103)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1883 (IC base=+0.203)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.495` → IC=+0.214 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.495 (IC base=+0.203)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.595` → IC=+0.196 (n=491)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` < 3.595 (IC base=+0.203)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0124` → IC=+0.138 (n=565)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0124 (IC base=+0.100)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.124 (n=802)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 15.0 (IC base=+0.100)

- **PATRÓN** `dist_vwap_pct` > `0.4579` → IC=+0.156 (n=332)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.4579 (IC base=+0.100)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.281` → IC=+0.245 (n=269)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.281 (IC base=+0.100)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.004` → IC=+0.138 (n=255)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.004 (IC base=+0.086)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.125 (n=355)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 7.0 (IC base=+0.086)

- **PATRÓN** `dist_vwap_pct` > `0.4456` → IC=+0.190 (n=56)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.4456 (IC base=+0.086)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.89` → IC=+0.209 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.89 (IC base=+0.086)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_ewma_delta_pct` > `12.366` → IC=+0.198 (n=61)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` > 12.366 (IC base=+0.061)
...
- **PATRÓN** `sigma_h` < `0.0202` → IC=+0.183 (n=453)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0202 (IC base=+0.174)

- **PATRÓN** `sigma_h` > `0.0142` → IC=+0.175 (n=343)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0142 (IC base=+0.174)
...
- **PATRÓN** `dist_vwap_pct` > `0.2413` → IC=+0.230 (n=150)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2413 (IC base=+0.174)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `9.452` → IC=+0.151 (n=669)
...
- **PATRÓN** `dist_vwap_pct` > `0.178` → IC=+0.121 (n=180)

  - _Acción_: Kelly boost +0.60€ cuando `dist_vwap_pct` > 0.178 (IC base=+0.048)
...
- **PATRÓN** `ibs_15` < `0.0286` → IC=+0.157 (n=65)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.78€ cuando `ibs_15` < 0.0286 (IC base=+0.074)
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.0286 sube el IC de +0.074 a +0.157 en UPDOWN_GBM#XRP#15min (n=65). Ya aplicado como kelly_boost=+0.78€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ FAVORITO_CONFIRMADO | 6815 | +0.179 | -18.42€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 4491 | +0.204 | -12.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 306 | +0.052 | -23.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 813 | +0.127 | -60.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 1205 | +0.154 | +77.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 2007 | +0.184 | +0.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1494 | +0.200 | -40.32€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 101 | +0.034 | -15.84€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 402 | +0.176 | +59.97€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 8 | +0.120 | +5.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 7 | +0.097 | +4.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 2394 | +0.169 | -37.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1479 | +0.200 | -0.25€ | 0 | 5 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 2367 | +0.185 | +11.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1478 | +0.211 | +25.18€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 431 | +0.306 | +12.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 431 | +0.306 | +12.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 431 | +0.306 | +12.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 431 | +0.306 | +12.44€ | 0 | 0 |
| ✅ GBM_LATE_15M | 8335 | +0.094 | +2501.66€ | 0 | 3 |
| ✅ GBM_LATE_15M#15min | 8335 | +0.094 | +2501.66€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 2021 | +0.073 | +355.43€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 2021 | +0.073 | +355.43€ | 0 | 5 |
| ✅ GBM_LATE_15M#ETH | 1829 | +0.073 | +304.63€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1829 | +0.073 | +304.63€ | 0 | 6 |
| ✅ GBM_LATE_15M#SOL | 2242 | +0.083 | +765.60€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 2242 | +0.083 | +765.60€ | 1 | 3 |
| ✅ GBM_LATE_15M#XRP | 2243 | +0.139 | +1076.00€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 2243 | +0.139 | +1076.00€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 5567 | +0.115 | +2556.53€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 5567 | +0.115 | +2556.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 1462 | +0.085 | +504.62€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 1462 | +0.085 | +504.62€ | 0 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1462 | +0.089 | +502.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1462 | +0.089 | +502.25€ | 0 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 1392 | +0.081 | +515.95€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 1392 | +0.081 | +515.95€ | 0 | 4 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 1251 | +0.217 | +1033.71€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 1251 | +0.217 | +1033.71€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 5556 | +0.065 | +1451.90€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 5556 | +0.065 | +1451.90€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 1330 | +0.032 | +160.70€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 1330 | +0.032 | +160.70€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 1361 | +0.024 | +108.78€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 1361 | +0.024 | +108.78€ | 0 | 1 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 1310 | +0.024 | +237.30€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 1310 | +0.024 | +237.30€ | 1 | 1 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 1555 | +0.163 | +945.12€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1555 | +0.163 | +945.12€ | 0 | 11 |
| ✅ GBM_LATE_5M | 837 | -0.036 | -6.36€ | 0 | 0 |
| ✅ GBM_LATE_5M#5min | 837 | -0.036 | -6.36€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 2848 | +0.035 | +256.90€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 2585 | +0.052 | +295.42€ | 0 | 4 |
...
| ✅ UPDOWN_GBM#ETH | 1092 | +0.039 | +74.62€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 998 | +0.053 | +86.18€ | 0 | 4 |
...
| ✅ UPDOWN_GBM#XRP | 316 | +0.044 | +38.20€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 305 | +0.054 | +41.49€ | 4 | 1 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 2105 | +0.151 | +926.97€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 2105 | +0.151 | +926.97€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 591 | +0.168 | +347.62€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 591 | +0.168 | +347.62€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.167: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.074 n=1595 | neutral: IC=+0.020 n=1287 | overbought(IBS>0.7): IC=+0.187 n=1597
  - _Datos_: n=4774 IC=+0.103 PNL=+1249.19€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.111 n=23018 | tras_1loss IC=+0.077 n=15496 | tras_2loss IC=+0.050 n=6519/40 | gap=+0.061 (umbral 0.05)
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=+0.090 n=1711/40 PNL=+332.83€ | H=15h UTC: IC=+0.116 n=1664/40 PNL=+517.02€ | H=17h UTC: IC=+0.088 n=1595/40 PNL=+283.94€ | H=19h UTC: IC=+0.140 n=1475/40 PNL=+423.27€
...
**🔶 H-CUSTOM-GBM-17H-BTC** — GBM BTC a las 17h UTC — ¿edge real?
  ...
  - _Estado_: SEÑAL POSITIVA en BTC (IC=+0.156 n=30) pero sin cruzar ≥2 pares más — BNB: n=8 IC=+0.000; DOGE: n=9 IC=+0.061; ETH: n=59 IC=+0.123 ✓; SOL: n=10 IC=+0.000; XRP: n=14 IC=+0.131
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2711 IC=+0.039 PNL=+270.70€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2711 IC=+0.039 PNL=+270.70€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.090 > 0.08 con n=1375 PNL=+236.44€
  - _Datos_: n=1375 IC=+0.090 PNL=+236.44€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=1407 IC=+0.038 PNL=+108.67€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1407 IC=+0.038 PNL=+108.67€
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=2443 IC=+0.053 PNL=+260.54€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=2443 IC=+0.053 PNL=+260.54€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.162 > 0.1 con n=992 PNL=+338.63€
  - _Datos_: n=992 IC=+0.162 PNL=+338.63€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=268 IC=+0.074 PNL=+44.81€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=268 IC=+0.074 PNL=+44.81€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=991 IC=-0.077 PNL=+233.11€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=991 IC=-0.077 PNL=+233.11€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=1109 IC=+0.056 PNL=+121.51€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=1109 IC=+0.056 PNL=+121.51€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.128 > 0.08 con n=735 PNL=+363.53€
  - _Datos_: n=735 IC=+0.128 PNL=+363.53€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.275 > 0.08 con n=508 PNL=+2.85€
  - _Datos_: n=508 IC=+0.275 PNL=+2.85€
