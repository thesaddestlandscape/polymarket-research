# Hipótesis automáticas — 2026-08-26 03:22 UTC
_Generado por shadow_postmortem.py sobre 158391 resoluciones (PNL=+10630.55€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `148.49` → IC=-0.279 (n=2216)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 148.49
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=6649)
...
- **FILTRO** `restante_s_al_confirmar` < `155.51` → IC=-0.344 (n=503)
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.193 (n=412)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.186 (n=769)

  - _Acción_: Kelly boost +0.93€ cuando `libro_spread` < 0.01 (IC base=+0.185)

- **PATRÓN** `libro_liquidez` > `11931.9544` → IC=+0.202 (n=199)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.294 (n=439)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.288)

- **PATRÓN** `py_entrada` < `0.375` → IC=+0.332 (n=421)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.375 (IC base=+0.288)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.288 (n=451)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.288)

- **PATRÓN** `libro_liquidez` > `3623.3782` → IC=+0.303 (n=191)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3623.3782 (IC base=+0.288)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.194 (n=3653)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.180 (n=1053)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` < 17.0 (IC base=+0.168)

- **PATRÓN** `py_entrada` < `0.72` → IC=+0.174 (n=929)

  - _Acción_: Kelly boost +0.87€ cuando `py_entrada` < 0.72 (IC base=+0.168)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.190 (n=898)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` < 15.0 (IC base=+0.186)

- **PATRÓN** `py_entrada` < `0.7` → IC=+0.197 (n=523)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.193 (n=8035)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` < 12.0 (IC base=+0.193)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.223 (n=7907)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.131 (n=1487)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` < 12.0 (IC base=+0.125)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.159 (n=1497)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.273 (n=1306)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.167 (n=1381)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.235 (n=1271)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.193 (n=1356)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `restante_min` < `3.93` → IC=+0.138 (n=1367)

  - _Acción_: Kelly boost +0.69€ cuando `restante_min` < 3.93 (IC base=+0.131)

- **PATRÓN** `restante_min` > `4.92` → IC=+0.156 (n=1462)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.148 (n=1819)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 7.0 (IC base=+0.131)

- **PATRÓN** `lag_apertura_s` < `4.52` → IC=+0.157 (n=1355)
...
- **PATRÓN** `restante_min` < `3.89` → IC=+0.142 (n=674)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.161 (n=897)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 7.0 (IC base=+0.139)

- **PATRÓN** `lag_apertura_s` < `5.74` → IC=+0.169 (n=674)
...
- **PATRÓN** `restante_min` < `4.4` → IC=+0.136 (n=901)

  - _Acción_: Kelly boost +0.68€ cuando `restante_min` < 4.4 (IC base=+0.123)

- **PATRÓN** `restante_min` > `4.94` → IC=+0.154 (n=735)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.136 (n=922)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` < 7.0 (IC base=+0.123)

- **PATRÓN** `lag_apertura_s` < `3.47` → IC=+0.158 (n=682)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9495` → IC=+0.209 (n=689)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.138` → IC=+0.176 (n=843)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 5.138 (IC base=+0.064)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4282` → IC=+0.134 (n=963)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 2.4282 (IC base=+0.064)
...
- **FILTRO** `ibs_20min` > `0.8795` → IC=-0.181 (n=183)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.151 (n=61)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=675)
...
- **FILTRO** `ibs_20min` > `0.7778` → IC=-0.166 (n=276)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.008 (n=830)

- **FILTRO** `sigma_ewma_delta_pct` > `6.558` → IC=-0.181 (n=186)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 6.558
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=920)
...
- **PATRÓN** `sigma_h` > `0.0073` → IC=+0.136 (n=1189)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0073 (IC base=+0.052)

- **PATRÓN** `ibs_20min` > `0.9461` → IC=+0.245 (n=873)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0883` → IC=+0.167 (n=1105)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` < 0.0883 (IC base=+0.052)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4823` → IC=+0.190 (n=395)
...
- **PATRÓN** `ballena_activa_n` < `100.0` → IC=+0.267 (n=630)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 100.0 (IC base=+0.052)

- **PATRÓN** `ibs_20min` < `0.1` → IC=+0.179 (n=1133)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3984` → IC=+0.276 (n=252)
...
- **PATRÓN** `dist_vwap_pct` > `1.1167` → IC=+0.138 (n=45)
  ...
  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 1.1167 (IC base=-0.025)

- **PATRÓN** `ballena_activa_n` < `288.0` → IC=+0.152 (n=44)

  - _Acción_: Kelly boost +0.76€ cuando `ballena_activa_n` < 288.0 (IC base=-0.025)
...
- **PATRÓN** `ibs_20min` < `0.2398` → IC=+0.385 (n=172)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2398 (IC base=+0.370)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.688` → IC=+0.372 (n=224)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.688 (IC base=+0.370)

- **PATRÓN** `volumen_pendiente_norm` > `0.1405` → IC=+0.404 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1405 (IC base=+0.370)

- **PATRÓN** `volumen_spike_ratio` < `2.9116` → IC=+0.430 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.9116 (IC base=+0.370)

- **PATRÓN** `libro_liquidez` > `1889.461` → IC=+0.396 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1889.461 (IC base=+0.370)

- **PATRÓN** `ballena_activa_n` < `74.0` → IC=+0.379 (n=56)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.156 (n=62)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=873)
...
- **PATRÓN** `ibs_20min` > `0.8929` → IC=+0.257 (n=1507)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.317` → IC=+0.255 (n=965)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.317 (IC base=+0.155)

- **PATRÓN** `volumen_regimen` > `0.6847` → IC=+0.172 (n=1405)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` > 0.6847 (IC base=+0.155)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2818` → IC=+0.153 (n=1773)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.2818 (IC base=+0.155)

- **PATRÓN** `volumen_spike_ratio` > `1.8535` → IC=+0.146 (n=1343)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.8535 (IC base=+0.155)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.167 (n=1848)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.01 (IC base=+0.155)

- **PATRÓN** `libro_liquidez` > `3801.753` → IC=+0.196 (n=754)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 3801.753 (IC base=+0.155)
...
- **PATRÓN** `sigma_h` < `0.0077` → IC=+0.230 (n=279)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0077 (IC base=+0.187)

- **PATRÓN** `drift_60min` |x|≤ `0.4992` → IC=+0.204 (n=316)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.4615` → IC=+0.244 (n=315)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `12.54` → IC=+0.344 (n=62)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.54 (IC base=+0.187)

- **PATRÓN** `volumen_regimen` > `0.6345` → IC=+0.207 (n=315)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6345 (IC base=+0.187)

- **PATRÓN** `volumen_pendiente_norm` > `0.101` → IC=+0.255 (n=137)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.101 (IC base=+0.187)

- **PATRÓN** `volumen_spike_ratio` < `1.4363` → IC=+0.211 (n=102)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4363 (IC base=+0.187)

- **PATRÓN** `volumen_spike_ratio` > `2.4478` → IC=+0.231 (n=102)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4478 (IC base=+0.187)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.189 (n=355)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.01 (IC base=+0.187)

- **PATRÓN** `libro_liquidez` > `9943.8712` → IC=+0.203 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 9943.8712 (IC base=+0.187)

- **PATRÓN** `ballena_activa_n` < `124.0` → IC=+0.190 (n=111)

  - _Acción_: Kelly boost +0.95€ cuando `ballena_activa_n` < 124.0 (IC base=+0.187)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.207 (n=1191)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.9429` → IC=+0.285 (n=1191)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9429 (IC base=+0.143)
...
- **PATRÓN** `volumen_regimen` < `0.7015` → IC=+0.154 (n=818)
...
- **PATRÓN** `libro_liquidez` > `3743.0776` → IC=+0.194 (n=876)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 3743.0776 (IC base=+0.143)

- **PATRÓN** `ballena_activa_n` < `167.0` → IC=+0.185 (n=1101)

  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 167.0 (IC base=+0.143)

- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.225 (n=1550)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0063 (IC base=+0.192)

- **PATRÓN** `drift_60min` |x|≤ `0.4427` → IC=+0.202 (n=2316)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.199 (n=1089)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.192)

- **PATRÓN** `ibs_20min` < `0.5506` → IC=+0.247 (n=2316)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5506 (IC base=+0.192)

- **PATRÓN** `dist_vwap_pct` < `0.7323` → IC=+0.177 (n=1906)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.7323 (IC base=+0.192)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.387` → IC=+0.215 (n=339)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.387 (IC base=+0.192)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.639` → IC=+0.197 (n=2169)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` < 2.639 (IC base=+0.192)

- **PATRÓN** `volumen_regimen` < `0.6203` → IC=+0.183 (n=592)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 0.6203 (IC base=+0.192)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2812` → IC=+0.260 (n=244)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2812 (IC base=+0.192)

- **PATRÓN** `volumen_spike_ratio` > `2.2674` → IC=+0.208 (n=735)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2674 (IC base=+0.192)

- **PATRÓN** `ballena_activa_n` < `195.0` → IC=+0.162 (n=1188)

  - _Acción_: Kelly boost +0.81€ cuando `ballena_activa_n` < 195.0 (IC base=+0.192)
...
- **PATRÓN** `drift_60min` |x|≤ `0.147` → IC=+0.330 (n=157)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.147 (IC base=+0.284)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.289 (n=221)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.284)

- **PATRÓN** `hora_utc` < `14.0` → IC=+0.294 (n=212)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 14.0 (IC base=+0.284)

- **PATRÓN** `ibs_20min` < `0.5819` → IC=+0.340 (n=235)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5819 (IC base=+0.284)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.095` → IC=+0.308 (n=259)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.095 (IC base=+0.284)

- **PATRÓN** `volumen_pendiente_norm` < `0.069` → IC=+0.308 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.069 (IC base=+0.284)

- **PATRÓN** `volumen_pendiente_norm` > `0.2907` → IC=+0.328 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2907 (IC base=+0.284)

- **PATRÓN** `volumen_spike_ratio` > `2.6771` → IC=+0.354 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6771 (IC base=+0.284)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.315 (n=106)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.284)

- **PATRÓN** `libro_liquidez` > `1990.966` → IC=+0.352 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1990.966 (IC base=+0.284)

- **PATRÓN** `ballena_activa_n` < `82.0` → IC=+0.284 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 82.0 (IC base=+0.284)
...
- **PATRÓN** `sigma_h` < `0.0055` → IC=+0.341 (n=105)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0055 (IC base=+0.272)

- **PATRÓN** `sigma_h` > `0.008` → IC=+0.319 (n=103)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.008 (IC base=+0.272)
...
- **PATRÓN** `ibs_20min` < `0.5597` → IC=+0.342 (n=308)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5597 (IC base=+0.272)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.123` → IC=+0.297 (n=62)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.626` → IC=+0.271 (n=338)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.626 (IC base=+0.272)

- **PATRÓN** `volumen_pendiente_norm` > `0.3755` → IC=+0.281 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3755 (IC base=+0.272)

- **PATRÓN** `volumen_spike_ratio` < `4.3096` → IC=+0.266 (n=186)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 4.3096 (IC base=+0.272)
...
- **PATRÓN** `ballena_activa_n` < `20.0` → IC=+0.237 (n=36)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 20.0 (IC base=+0.272)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5098` → IC=+0.161 (n=334)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.5098 (IC base=+0.148)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.179 (n=232)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.7546` → IC=+0.167 (n=358)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.7546 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.474` → IC=+0.254 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.474 (IC base=+0.148)
...
- **PATRÓN** `volumen_regimen` > `1.0987` → IC=+0.164 (n=111)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` > 1.0987 (IC base=+0.148)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1811` → IC=+0.183 (n=244)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 2.1811 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` > `1.557` → IC=+0.176 (n=248)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 1.557 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `9123.6009` → IC=+0.186 (n=151)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 9123.6009 (IC base=+0.148)

- **PATRÓN** `ballena_activa_n` < `141.0` → IC=+0.230 (n=113)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 141.0 (IC base=+0.148)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.149 (n=807)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3624` → IC=+0.166 (n=543)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `4.0` → IC=+0.205 (n=249)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 4.0 (IC base=+0.142)

- **PATRÓN** `ibs_20min` < `0.4087` → IC=+0.159 (n=412)
...
- **PATRÓN** `dist_vwap_pct` < `0.8904` → IC=+0.142 (n=702)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `6.009` → IC=+0.144 (n=614)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 6.009 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `1.2316` → IC=+0.159 (n=613)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.2316 (IC base=+0.142)

- **PATRÓN** `volumen_pendiente_norm` < `0.0954` → IC=+0.148 (n=547)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` < 0.0954 (IC base=+0.142)

- **PATRÓN** `volumen_pendiente_norm` > `0.0724` → IC=+0.160 (n=301)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` > 0.0724 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` < `2.5034` → IC=+0.167 (n=610)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.5034 (IC base=+0.142)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.149 (n=807)

  - _Acción_: Kelly boost +0.74€ cuando `libro_spread` < 0.01 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `8448.0568` → IC=+0.149 (n=617)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 8448.0568 (IC base=+0.142)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.170 (n=715)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.033 (n=2245)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.217 (n=701)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=2234)

- **FILTRO** `ibs_20min` > `0.2747` → IC=-0.183 (n=733)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2747
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=2202)
...
- **FILTRO** `ballena_activa_n` > `5.0` → IC=-0.148 (n=163)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.212 (n=116)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.139 (n=2142)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
...
- **FILTRO** `py_entrada` < `0.36` → IC=-0.283 (n=1753)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=5522)

- **FILTRO** `ibs_7min` < `0.7368` → IC=-0.230 (n=1815)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7368
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=5460)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.174 (n=2473)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=4802)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.230 (n=2076)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=6863)

- **FILTRO** `ibs_7min` > `0.7248` → IC=-0.168 (n=2234)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7248
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=6705)
...
- **FILTRO** `drift_7min_pct` |x|> `0.1413` → IC=-0.156 (n=515)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1413
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=1001)

- **FILTRO** `ibs_7min` > `0.8382` → IC=-0.200 (n=378)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8382
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=1138)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.156 (n=312)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
...
- **FILTRO** `py_entrada` < `0.38` → IC=-0.244 (n=353)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.004 (n=1110)
...
- **FILTRO** `ballena_activa_n` > `151.0` → IC=-0.170 (n=365)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=1098)
...
- **FILTRO** `ballena_activa_n` > `120.0` → IC=-0.177 (n=373)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.265 (n=334)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=1231)

- **FILTRO** `ibs_7min` > `0.8182` → IC=-0.182 (n=391)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8182
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=1174)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.146 (n=382)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
...
- **FILTRO** `ibs_7min` > `0.1833` → IC=-0.154 (n=417)

  - _Acción_: SKIP cuando `ibs_7min` > 0.1833
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=812)

- **FILTRO** `ballena_activa_n` > `38.0` → IC=-0.208 (n=306)

  - _Acción_: SKIP cuando `ballena_activa_n` > 38.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=923)
...
- **FILTRO** `ibs_7min` < `0.7759` → IC=-0.191 (n=341)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=1025)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.186 (n=390)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.007 (n=1245)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.292 (n=287)
...
- **FILTRO** `ibs_7min` < `0.75` → IC=-0.237 (n=287)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=906)
...
- **FILTRO** `ibs_7min` > `0.8346` → IC=-0.158 (n=375)
...
- **PATRÓN** `ibs_15` < `0.1124` → IC=+0.135 (n=403)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.67€ cuando `ibs_15` < 0.1124 (IC base=+0.028)
...
- **FILTRO** `sigma_h` > `0.0071` → IC=-0.137 (n=89)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0071
...
- **FILTRO** `drift_15min` |x|> `0.568` → IC=-0.170 (n=89)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.040 (n=270)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.036` → IC=-0.181 (n=321)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.075 (n=2253)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.0628` → IC=+0.395 (n=17)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.0628 (IC base=-0.088)

- **PATRÓN** `ibs_15` < `0.3333` → IC=+0.359 (n=97)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3333 (IC base=-0.088)

- **PATRÓN** `dist_vwap_pct` < `0.1207` → IC=+0.277 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1207 (IC base=-0.088)

- **PATRÓN** `ballena_activa_n` < `159.0` → IC=+0.318 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 159.0 (IC base=-0.088)

### UPDOWN_GBM_15M_TARDIO#BTC#15min
- **FILTRO** `sigma_h` > `0.0076` → IC=-0.238 (n=143)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `sigma_h` < `0.0046` → IC=-0.215 (n=286)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0046
  - _Potencial_: sin este filtro IC_bueno=-0.209 (n=287)

- **FILTRO** `hora_utc` > `16.0` → IC=-0.250 (n=142)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.200 (n=431)

- **FILTRO** `sigma_ewma_delta_pct` > `18.983` → IC=-0.241 (n=110)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 18.983
  - _Potencial_: sin este filtro IC_bueno=-0.205 (n=463)
...
- **FILTRO** `sigma_ewma_delta_pct` > `15.454` → IC=-0.170 (n=101)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=725)
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3333 sube el IC de -0.088 a +0.359 en UPDOWN_GBM_15M_TARDIO (n=97). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 6444 | -0.097 | -860.50€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 664 | -0.089 | -117.64€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 5780 | -0.098 | -742.86€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1049 | +0.025 | -108.49€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1049 | +0.025 | -108.49€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#XRP | 1525 | -0.190 | -338.24€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1525 | -0.190 | -338.24€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 29740 | +0.114 | -1952.40€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 5576 | +0.188 | -177.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 107 | -0.105 | -50.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 20947 | +0.094 | -1690.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3110 | +0.122 | -34.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 3516 | +0.062 | -602.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 20 | -0.045 | +2.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 3491 | +0.064 | -599.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 6188 | +0.134 | -133.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1572 | +0.198 | -81.31€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 3490 | +0.108 | -88.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1084 | +0.132 | +59.38€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 3523 | +0.077 | -467.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 20 | +0.045 | +2.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 3502 | +0.078 | -467.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 