# Hipótesis automáticas — 2026-09-04 22:26 UTC
_Generado por shadow_postmortem.py sobre 287076 resoluciones (PNL=+28193.62€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `147.45` → IC=-0.299 (n=3859)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 147.45
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=11578)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `134.01` → IC=-0.323 (n=511)
...
- **FILTRO** `restante_s_al_confirmar` < `158.23` → IC=-0.378 (n=929)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.102 (n=1888)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.190 (n=8186)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.149 (n=5780)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.34` → IC=+0.254 (n=5095)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.181 (n=2689)

  - _Acción_: Kelly boost +0.91€ cuando `py_entrada` > 0.72 (IC base=+0.151)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.228 (n=3145)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.273 (n=1128)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.190 (n=1118)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.231 (n=1536)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.75 (IC base=+0.184)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.204 (n=2582)
...
- **PATRÓN** `restante_min` > `4.94` → IC=+0.159 (n=2415)
...
- **PATRÓN** `lag_apertura_s` < `3.87` → IC=+0.158 (n=2409)
...
- **PATRÓN** `restante_min` > `4.88` → IC=+0.150 (n=1665)
...
- **PATRÓN** `lag_apertura_s` < `6.9` → IC=+0.148 (n=1579)

  - _Acción_: Kelly boost +0.74€ cuando `lag_apertura_s` < 6.9 (IC base=+0.136)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.199 (n=1287)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.127 (n=3655)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.3` → IC=+0.145 (n=4744)
...
- **PATRÓN** `ballena_activa_n` < `100.0` → IC=+0.294 (n=1811)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4728` → IC=+0.228 (n=233)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4728 (IC base=+0.026)

- **PATRÓN** `ballena_activa_n` < `37.0` → IC=+0.324 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 37.0 (IC base=+0.026)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.129 (n=1776)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5064` → IC=+0.146 (n=1344)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.5064 (IC base=+0.126)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.153 (n=499)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `6.341` → IC=+0.138 (n=1342)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1485` → IC=+0.128 (n=1344)
...
- **PATRÓN** `volumen_spike_ratio` > `1.7971` → IC=+0.133 (n=886)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 1.7971 (IC base=+0.126)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.129 (n=1776)
...
- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.283 (n=21)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.65` → IC=+0.324 (n=15)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.65 (IC base=+0.273)

- **PATRÓN** `libro_liquidez` > `2427.3092` → IC=+0.265 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2427.3092 (IC base=+0.273)

- **PATRÓN** `ballena_activa_n` < `25.0` → IC=+0.308 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 25.0 (IC base=+0.273)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.135 (n=3180)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=11761)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.282 (n=3651)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=11290)

- **FILTRO** `ibs_7min` < `0.7143` → IC=-0.241 (n=3729)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=11212)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.169 (n=5061)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=9880)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.221 (n=4623)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=14259)

- **FILTRO** `ibs_7min` > `0.7143` → IC=-0.173 (n=4706)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=14176)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.312 (n=536)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=1651)

- **FILTRO** `ibs_7min` < `0.7014` → IC=-0.255 (n=721)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7014
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=1466)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.220 (n=537)
...
- **FILTRO** `py_entrada` > `0.71` → IC=-0.237 (n=817)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=2465)

- **FILTRO** `drift_7min_pct` |x|> `0.1139` → IC=-0.130 (n=1114)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1139
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=2168)

- **FILTRO** `ibs_7min` > `0.8364` → IC=-0.197 (n=819)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.133 (n=603)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=2167)
...
- **FILTRO** `ibs_7min` < `0.7711` → IC=-0.195 (n=692)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=2078)

- **FILTRO** `ballena_activa_n` > `168.0` → IC=-0.179 (n=689)

  - _Acción_: SKIP cuando `ballena_activa_n` > 168.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=2081)

- **FILTRO** `py_entrada` > `0.64` → IC=-0.241 (n=659)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.187 (n=694)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.099 (n=1512)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.310 (n=699)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=1507)

- **FILTRO** `ibs_7min` < `0.2167` → IC=-0.294 (n=551)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2167
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=1655)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.238 (n=537)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.091 (n=1669)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.233 (n=842)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=1706)

- **FILTRO** `ibs_7min` > `0.2769` → IC=-0.177 (n=636)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2769
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=1912)

- **FILTRO** `ballena_activa_n` > `34.0` → IC=-0.186 (n=625)
...
- **FILTRO** `ibs_7min` < `0.7368` → IC=-0.205 (n=692)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=2102)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.184 (n=685)

  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=2109)
...
- **FILTRO** `py_entrada` < `0.36` → IC=-0.292 (n=599)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=1858)
...
- **FILTRO** `py_entrada` > `0.69` → IC=-0.210 (n=819)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=2489)

- **FILTRO** `ibs_7min` > `0.7674` → IC=-0.162 (n=826)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7674
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=2482)
...
### UPDOWN_GBM#SOL#5min
- **FILTRO** `pct_spot_vs_ref` |x|> `0.0264` → IC=-0.184 (n=36)
...
| ✅ BALLENAS_TARDIAS | 15437 | -0.116 | -2596.91€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 974 | -0.013 | -144.00€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 14463 | -0.122 | -2452.91€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 2047 | -0.102 | -467.31€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 2047 | -0.102 | -467.31€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 974 | -0.013 | -144.00€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 974 | -0.013 | -144.00€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 2059 | -0.174 | -573.34€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 2059 | -0.174 | -573.34€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 4117 | -0.053 | -382.61€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 4117 | -0.053 | -382.61€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 3423 | -0.128 | -271.74€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 