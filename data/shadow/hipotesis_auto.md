# Hipótesis automáticas — 2026-08-18 16:31 UTC
_Generado por shadow_postmortem.py sobre 65149 resoluciones (PNL=+6628.69€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `n_ballenas` < `5.0` → IC=-0.126 (n=2050)

  - _Acción_: SKIP cuando `n_ballenas` < 5.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=1393)

- **FILTRO** `restante_s_al_confirmar` < `149.24` → IC=-0.258 (n=860)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 149.24
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=2583)
...
- **FILTRO** `restante_s_al_confirmar` < `246.47` → IC=-0.158 (n=606)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.156 (n=3812)
...
- **PATRÓN** `hora_utc` < `10.0` → IC=+0.231 (n=225)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 10.0 (IC base=+0.232)

- **PATRÓN** `py_entrada` < `0.37` → IC=+0.308 (n=300)
...
- **PATRÓN** `libro_liquidez` > `911.3292` → IC=+0.242 (n=297)
...
- **PATRÓN** `py_entrada` < `0.73` → IC=+0.176 (n=597)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.216 (n=594)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `14.0` → IC=+0.195 (n=2232)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.298 (n=250)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.235 (n=323)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `14.0` → IC=+0.254 (n=344)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.221 (n=267)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.227 (n=583)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.150)

- **PATRÓN** `restante_min` < `3.78` → IC=+0.165 (n=521)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.157 (n=1436)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=805)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `profundidad_ratio_no` > `12.5` → IC=+0.168 (n=263)
  ...
  - _Acción_: Kelly boost +0.84€ cuando `profundidad_ratio_no` > 12.5 (IC base=+0.145)
...
- **PATRÓN** `volumen_spike_ratio` > `1.6738` → IC=+0.128 (n=619)
  ...
  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.6738 (IC base=+0.105)

- **PATRÓN** `ibs_20min` < `0.6598` → IC=+0.123 (n=2137)

  - _Acción_: Kelly boost +0.61€ cuando `ibs_20min` < 0.6598 (IC base=+0.079)

- **PATRÓN** `volumen_regimen` < `1.3132` → IC=+0.144 (n=481)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.3132 (IC base=+0.079)

- **PATRÓN** `volumen_regimen` > `0.6961` → IC=+0.143 (n=430)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 0.6961 (IC base=+0.079)

- **PATRÓN** `volumen_pendiente_norm` > `0.1266` → IC=+0.275 (n=176)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1266 (IC base=+0.079)

- **PATRÓN** `volumen_spike_ratio` < `1.7088` → IC=+0.242 (n=165)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7088 (IC base=+0.079)

- **PATRÓN** `volumen_spike_ratio` > `2.8899` → IC=+0.233 (n=170)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8899 (IC base=+0.079)

- **PATRÓN** `ballena_activa_n` < `265.0` → IC=+0.275 (n=100)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 265.0 (IC base=+0.079)
...
- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.154 (n=105)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0029 (IC base=+0.148)

- **PATRÓN** `drift_60min` |x|≤ `0.1903` → IC=+0.170 (n=204)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.1903 (IC base=+0.148)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.195 (n=165)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` > 12.0 (IC base=+0.148)

- **PATRÓN** `ibs_20min` < `0.4851` → IC=+0.205 (n=232)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4851 (IC base=+0.148)
...
- **PATRÓN** `volumen_regimen` < `1.3145` → IC=+0.167 (n=232)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.3145 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` > `0.6858` → IC=+0.165 (n=207)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 0.6858 (IC base=+0.148)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5117` → IC=+0.278 (n=43)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5117 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `11623.6154` → IC=+0.183 (n=77)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 11623.6154 (IC base=+0.148)
...
- **PATRÓN** `sigma_h` < `0.0054` → IC=+0.180 (n=23)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.0054 (IC base=+0.144)

- **PATRÓN** `drift_60min` |x|≤ `0.1198` → IC=+0.147 (n=15)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.1198 (IC base=+0.144)
...
- **PATRÓN** `ibs_20min` > `0.5714` → IC=+0.250 (n=22)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5714 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.786` → IC=+0.147 (n=15)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 6.786 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.475` → IC=+0.192 (n=24)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` < 4.475 (IC base=+0.144)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.220 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `2758.2556` → IC=+0.208 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2758.2556 (IC base=+0.144)

### GBM_LATE_15M#XRP#15min
- **PATRÓN** `sigma_ewma_delta_pct` > `8.439` → IC=+0.144 (n=57)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` > 8.439 (IC base=-0.018)

- **PATRÓN** `volumen_regimen` < `0.7863` → IC=+0.167 (n=22)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.7863 (IC base=-0.018)
...
- **PATRÓN** `ibs_20min` > `0.9448` → IC=+0.232 (n=532)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9448 (IC base=+0.054)

- **PATRÓN** `dist_vwap_pct` > `0.1183` → IC=+0.175 (n=158)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.1183 (IC base=+0.054)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.204` → IC=+0.130 (n=986)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3377` → IC=+0.192 (n=102)
  ...
  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.3377 (IC base=+0.054)

- **PATRÓN** `volumen_spike_ratio` > `2.8672` → IC=+0.168 (n=293)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 2.8672 (IC base=+0.054)
...
- **PATRÓN** `volumen_regimen` > `1.0742` → IC=+0.214 (n=197)
...
- **PATRÓN** `ballena_activa_n` < `71.0` → IC=+0.281 (n=71)
...
- **PATRÓN** `volumen_regimen` < `1.0528` → IC=+0.139 (n=34)
  ...
  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` < 1.0528 (IC base=-0.004)
...
- **FILTRO** `ibs_20min` < `0.45` → IC=-0.150 (n=141)
...
- **PATRÓN** `ibs_20min` > `0.45` → IC=+0.139 (n=142)
  ...
  - _Acción_: Kelly boost +0.69€ cuando `ibs_20min` > 0.45 (IC base=-0.005)

- **PATRÓN** `dist_vwap_pct` > `0.1775` → IC=+0.182 (n=20)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.1775 (IC base=-0.005)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.001` → IC=+0.281 (n=300)
...
- **PATRÓN** `libro_liquidez` > `2830.7244` → IC=+0.284 (n=100)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2830.7244 (IC base=+0.281)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` < `0.0047` → IC=+0.150 (n=418)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` > `0.78` → IC=+0.212 (n=1114)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.78 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` > `0.2198` → IC=+0.140 (n=276)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.2198 (IC base=+0.138)
...
- **PATRÓN** `volumen_regimen` < `1.1837` → IC=+0.136 (n=649)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 1.1837 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `0.6297` → IC=+0.137 (n=648)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 0.6297 (IC base=+0.138)
...
- **PATRÓN** `libro_spread` < `0.06` → IC=+0.144 (n=1401)
...
- **PATRÓN** `sigma_h` > `0.003` → IC=+0.280 (n=57)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.003 (IC base=+0.259)

- **PATRÓN** `drift_60min` |x|≤ `0.0943` → IC=+0.325 (n=38)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0943 (IC base=+0.259)

- **PATRÓN** `hora_utc` > `9.0` → IC=+0.265 (n=79)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.8394` → IC=+0.323 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8394 (IC base=+0.259)

- **PATRÓN** `dist_vwap_pct` < `0.3511` → IC=+0.280 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3511 (IC base=+0.259)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.418` → IC=+0.413 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.418 (IC base=+0.259)

- **PATRÓN** `volumen_regimen` > `0.6785` → IC=+0.273 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6785 (IC base=+0.259)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1479` → IC=+0.264 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1479 (IC base=+0.259)

- **PATRÓN** `volumen_spike_ratio` > `3.154` → IC=+0.293 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.154 (IC base=+0.259)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.255 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.259)

- **PATRÓN** `libro_liquidez` > `2543.553` → IC=+0.261 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2543.553 (IC base=+0.259)
...
- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.234 (n=1173)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.225)

- **PATRÓN** `sigma_h` > `0.0038` → IC=+0.231 (n=1331)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0038 (IC base=+0.225)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.254 (n=680)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 14.0 (IC base=+0.225)

- **PATRÓN** `ibs_20min` < `0.5` → IC=+0.286 (n=1338)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5 (IC base=+0.225)

- **PATRÓN** `dist_vwap_pct` < `0.1494` → IC=+0.199 (n=898)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.1494 (IC base=+0.225)
...
- **PATRÓN** `volumen_regimen` < `0.6196` → IC=+0.208 (n=299)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6196 (IC base=+0.225)

- **PATRÓN** `volumen_regimen` > `1.0749` → IC=+0.219 (n=407)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5737` → IC=+0.268 (n=566)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5737 (IC base=+0.225)
...
- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.179 (n=104)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0029 (IC base=+0.176)

- **PATRÓN** `drift_60min` |x|≤ `0.193` → IC=+0.190 (n=201)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.193 (IC base=+0.176)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.197 (n=232)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 6.0 (IC base=+0.176)

- **PATRÓN** `ibs_20min` < `0.4222` → IC=+0.227 (n=229)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4222 (IC base=+0.176)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.232` → IC=+0.267 (n=144)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.232 (IC base=+0.176)

- **PATRÓN** `volumen_regimen` < `1.3306` → IC=+0.188 (n=229)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` < 1.3306 (IC base=+0.176)

- **PATRÓN** `volumen_regimen` > `0.8593` → IC=+0.195 (n=152)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 0.8593 (IC base=+0.176)

- **PATRÓN** `volumen_pendiente_norm` < `0.2333` → IC=+0.232 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2333 (IC base=+0.176)

- **PATRÓN** `volumen_pendiente_norm` > `0.0974` → IC=+0.317 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0974 (IC base=+0.176)

- **PATRÓN** `volumen_spike_ratio` < `1.8705` → IC=+0.304 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8705 (IC base=+0.176)

- **PATRÓN** `libro_liquidez` > `4994.7986` → IC=+0.189 (n=204)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 4994.7986 (IC base=+0.176)
...
- **PATRÓN** `sigma_h` < `0.0069` → IC=+0.165 (n=228)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0069 (IC base=+0.139)

- **PATRÓN** `drift_60min` |x|≤ `0.176` → IC=+0.162 (n=152)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.176 (IC base=+0.139)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.211 (n=119)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 14.0 (IC base=+0.139)

- **PATRÓN** `ibs_20min` < `0.6` → IC=+0.230 (n=231)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` < `0.1757` → IC=+0.158 (n=179)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.1757 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.427` → IC=+0.242 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.427 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.863` → IC=+0.145 (n=212)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 2.863 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` < `0.7156` → IC=+0.196 (n=100)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` < 0.7156 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` > `1.2615` → IC=+0.167 (n=76)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 1.2615 (IC base=+0.139)

- **PATRÓN** `volumen_pendiente_norm` < `0.1742` → IC=+0.312 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1742 (IC base=+0.139)

- **PATRÓN** `volumen_spike_ratio` > `1.5635` → IC=+0.309 (n=82)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5635 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `1362.9219` → IC=+0.245 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1362.9219 (IC base=+0.139)

- **PATRÓN** `ballena_activa_n` < `67.0` → IC=+0.147 (n=15)

  - _Acción_: Kelly boost +0.74€ cuando `ballena_activa_n` < 67.0 (IC base=+0.139)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.296 (n=297)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.1818` → IC=+0.372 (n=225)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1818 (IC base=+0.258)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.348` → IC=+0.318 (n=75)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.348 (IC base=+0.258)

- **PATRÓN** `volumen_regimen` > `1.2406` → IC=+0.317 (n=113)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2406 (IC base=+0.258)

- **PATRÓN** `volumen_pendiente_norm` > `0.264` → IC=+0.354 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.264 (IC base=+0.258)
...
- **PATRÓN** `ballena_activa_n` < `55.0` → IC=+0.186 (n=49)

  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 55.0 (IC base=+0.258)
...
- **PATRÓN** `drift_20min_pct` |x|≤ `0.0531` → IC=+0.154 (n=76)
  ...
  - _Acción_: Kelly boost +0.77€ cuando `drift_20min_pct` |x|≤ 0.0531 (IC base=+0.068)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.197 (n=150)

  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.008 (n=183)

- **FILTRO** `ibs_20min` < `0.7484` → IC=-0.171 (n=83)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.128 (n=151)

  - _Acción_: Kelly boost +0.64€ cuando `py_entrada` < 0.5 (IC base=+0.021)
...
- **PATRÓN** `ibs_20min` < `0.1942` → IC=+0.134 (n=39)

  - _Acción_: Kelly boost +0.67€ cuando `ibs_20min` < 0.1942 (IC base=+0.050)
...
- **FILTRO** `libro_liquidez` < `15830.4808` → IC=-0.132 (n=36)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.200 (n=18)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.091)

- **PATRÓN** `drift_20min_pct` |x|≤ `0.1191` → IC=+0.146 (n=46)

  - _Acción_: Kelly boost +0.73€ cuando `drift_20min_pct` |x|≤ 0.1191 (IC base=+0.091)

- **PATRÓN** `libro_liquidez` > `11226.4799` → IC=+0.194 (n=47)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 11226.4799 (IC base=+0.091)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.151 (n=41)
...
- **FILTRO** `ballena_activa_n` > `28.0` → IC=-0.269 (n=37)

  - _Acción_: SKIP cuando `ballena_activa_n` > 28.0
  - _Potencial_: sin este filtro IC_bueno=-0.109 (n=113)

- **FILTRO** `libro_liquidez` < `9320.4293` → IC=-0.163 (n=99)

  - _Acción_: SKIP cuando `libro_liquidez` < 9320.4293
...
- **FILTRO** `py_entrada` > `0.5` → IC=-0.130 (n=106)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.157 (n=65)
  ...
  - _Acción_: Kelly boost +0.78€ cuando `py_entrada` < 0.5 (IC base=-0.020)
...
- **FILTRO** `py_entrada` < `0.495` → IC=-0.147 (n=32)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.036 (n=382)
...
- **PATRÓN** `ibs_15` < `0.4444` → IC=+0.126 (n=346)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.63€ cuando `ibs_15` < 0.4444 (IC base=+0.076)

- **PATRÓN** `dist_vwap_pct` > `0.5611` → IC=+0.210 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5611 (IC base=+0.076)
...
- **PATRÓN** `sigma_h` < `0.002` → IC=+0.250 (n=26)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.002 (IC base=+0.058)
...
- **PATRÓN** `sigma_h` < `0.0033` → IC=+0.189 (n=43)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0033 (IC base=+0.146)

- **PATRÓN** `sigma_h` > `0.0041` → IC=+0.212 (n=64)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0041 (IC base=+0.146)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1155` → IC=+0.159 (n=86)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.80€ cuando `delta_ratio_macro` |x|> 0.1155 (IC base=+0.146)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.173 (n=47)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 14.0 (IC base=+0.146)

- **PATRÓN** `hora_utc` < `14.0` → IC=+0.153 (n=93)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 14.0 (IC base=+0.146)

- **PATRÓN** `ibs_15` < `0.3247` → IC=+0.167 (n=97)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.83€ cuando `ibs_15` < 0.3247 (IC base=+0.146)

- **PATRÓN** `ibs_15` > `0.0292` → IC=+0.194 (n=96)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.97€ cuando `ibs_15` > 0.0292 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `0.296` → IC=+0.237 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.296 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` < `17.598` → IC=+0.211 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 17.598 (IC base=+0.146)

- **PATRÓN** `libro_liquidez` > `10785.0258` → IC=+0.152 (n=44)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 10785.0258 (IC base=+0.146)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0205` → IC=+0.123 (n=51)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.61€ cuando `pct_spot_vs_ref` |x|≤ 0.0205 (IC base=+0.090)

- **PATRÓN** `sigma_h` < `0.0025` → IC=+0.214 (n=26)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0025 (IC base=+0.090)

- **PATRÓN** `drift_15min` |x|≤ `0.1256` → IC=+0.273 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.1256 (IC base=+0.090)

- **PATRÓN** `hora_utc` < `2.0` → IC=+0.152 (n=21)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 2.0 (IC base=+0.090)

- **PATRÓN** `ibs_15` > `0.0472` → IC=+0.144 (n=57)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.72€ cuando `ibs_15` > 0.0472 (IC base=+0.090)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.77` → IC=+0.125 (n=54)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 7.77 (IC base=+0.090)

- **PATRÓN** `ballena_activa_n` < `3.0` → IC=+0.174 (n=44)

  - _Acción_: Kelly boost +0.87€ cuando `ballena_activa_n` < 3.0 (IC base=+0.090)
...
- **FILTRO** `sigma_ewma_delta_pct` > `12.903` → IC=-0.156 (n=222)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=902)
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_NO, IBS < 0.3247 sube el IC de +0.146 a +0.167 en UPDOWN_GBM#ETH#15min (n=97). Ya aplicado como kelly_boost=+0.83€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_NO, IBS > 0.0292 sube el IC de +0.146 a +0.194 en UPDOWN_GBM#ETH#15min (n=96). Ya aplicado como kelly_boost=+0.97€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.6216 sube el IC de -0.039 a +0.230 en UPDOWN_GBM_15M_TARDIO (n=113). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 3323 | -0.113 | -515.81€ | 2 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 455 | -0.021 | -16.13€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 2868 | -0.128 | -499.69€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 799 | -0.001 | -93.49€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 799 | -0.001 | -93.49€ | 1 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP | 671 | -0.219 | -116.91€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP#5min | 671 | -0.219 | -116.91€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 14257 | +0.116 | -810.42€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3370 | +0.180 | -117.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 99 | -0.104 | -47.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 8196 | +0.088 | -669.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2592 | +0.132 | +24.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 1383 | +0.034 | -298.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 13 | -0.065 | -2.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 1366 | +0.036 | -290.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 3250 | +0.141 | -2.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 920 | +0.195 | -35.72€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 41 | -0.105 | -20.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 1366 | +0.111 | -11.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 923 | +0.140 | +65.31€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 1381 | +0.057 | -236.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 9 | -0.021 | -3.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 1371 | +0.058 | -230.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 3513 | +0.127 | -46.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1244 | +0.159 | -26.51€ | 0 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 1362 | +0.106 | -15.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 895 | +0.117 | +3.13€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 3351 | +0.135 | -202.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1172 | +0.196 | -51.43€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 40 | +0.000 | -9.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 1365 | +0.085 | -97.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 774 | +0.139 | -44.18€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 1379 | +0.129 | -22.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 12 | +0.043 | +1.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 1366 | +0.130 | -23.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 3444 | +0.161 | -329.43€ | 2 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 3444 | +0.161 | -329.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 863 | +0.155 | -111.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 863 | +0.155 | -111.40€ | 0 | 2 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 764 | +0.217 | -43.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 764 | +0.217 | -43.33€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 5832 | +0.190 | -536.40€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 5832 | +0.190 | -536.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 1092 | +0.098 | -245.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 1092 | +0.098 | -245.84€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 902 | +0.249 | -6.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 902 | +0.249 | -6.97€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 
70€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 353 | -0.061 | +84.60€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 353 | -0.061 | +84.60€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 294 | -0.085 | -29.74€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 294 | -0.085 | -29.74€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 304 | -0.105 | -37.19€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 304 | -0.105 | -37.19€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 342 | -0.044 | +2.61€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 342 | -0.044 | +2.61€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 301 | -0.061 | +0.95€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 301 | -0.061 | +0.95€ | 4 | 1 |
| ✅ MOMENTUM_IBS_5M_FADE | 1758 | +0.023 | +23.91€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 1758 | +0.023 | +23.91€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB | 299 | +0.018 | +7.61€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB#5min | 299 | +0.018 | +7.61€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC | 256 | +0.039 | +4.70€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC#5min | 256 | +0.039 | +4.70€ | 1 | 1 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE | 303 | +0.021 | +3.17€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE#5min | 303 | +0.021 | +3.17€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH | 303 | +0.015 | +2.04€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH#5min | 303 | +0.015 | +2.04€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL | 307 | +0.034 | +7.87€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL#5min | 307 | +0.034 | +7.87€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP | 290 | +0.014 | -1.47€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP#5min | 290 | +0.014 | -1.47€ | 0 | 0 |
...
| ✅ STRUCT_NO_15M | 1841 | +0.015 | -5.29€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 1841 | +0.015 | -5.29€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 703 | +0.006 | -8.51€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 703 | +0.006 | -8.51€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 724 | +0.019 | +0.92€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 724 | +0.019 | +0.92€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 414 | +0.022 | +2.30€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 414 | +0.022 | +2.30€ | 2 | 0 |
| ✅ UPDOWN_GBM | 2158 | +0.021 | +118.78€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 941 | +0.085 | +161.55€ | 1 | 6 |
| ✅ UPDOWN_GBM#240min | 119 | +0.021 | +0.93€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 801 | -0.025 | -32.18€ | 4 | 0 |
...
| ✅ UPDOWN_GBM#ETH | 503 | +0.056 | +53.55€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 220 | +0.126 | +53.01€ | 1 | 14 |
| ✅ UPDOWN_GBM#ETH#240min | 36 | +0.053 | +0.92€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 131 | +0.011 | +2.10€ | 4 | 7 |
| ✅ UPDOWN_GBM#ETH#60min | 101 | +0.005 | -2.10€ | 0 | 1 |
| 🚫 UPDOWN_GBM#ETH#daily | 15 | -0.154 | -0.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 386 | -0.013 | -4.43€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 162 | +0.012 | +0.42€ | 1 | 2 |
...
| ✅ UPDOWN_GBM#XRP | 477 | +0.028 | +62.83€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 289 | +0.098 | +84.67€ | 0 | 15 |
| ✅ UPDOWN_GBM#XRP#240min | 17 | -0.112 | -2.90€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 171 | -0.072 | -18.94€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1645 | -0.048 | +172.83€ | 2 | 3 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1645 | -0.048 | +172.83€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 110 | -0.080 | -6.93€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 110 | -0.080 | -6.93€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 526 | -0.011 | +107.65€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 526 | -0.011 | +107.65€ | 1 | 1 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 510 | -0.035 | +67.09€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 510 | -0.035 | +67.09€ | 4 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.145) — sin ventaja clara. oversold(IBS<0.3): IC=-0.024 n=750 | neutral: IC=+0.006 n=729 | overbought(IBS>0.7): IC=+0.121 n=984
  - _Datos_: n=2640 IC=+0.042 PNL=+202.26€
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=162 IC=+0.012 PNL=+0.42€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.105 n=38111 | tras_1loss IC=+0.062 n=26737 | tras_2loss IC=+0.022 n=11654/40 | gap=+0.084 (umbral 0.05)
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=+0.077 n=161 | contrario_BTC IC=-0.037 n=93/40 | gap=-0.114 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1987 IC=+0.016 PNL=+87.27€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1987 IC=+0.016 PNL=+87.27€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=78 IC=+0.000 PNL=+3.47€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=78 IC=+0.000 PNL=+3.47€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=442 IC=+0.092 PNL=+106.52€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=442 IC=+0.092 PNL=+106.52€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.150 > 0.1 con n=550 PNL=+164.80€
  - _Datos_: n=550 IC=+0.150 PNL=+164.80€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1292 IC=-0.148 PNL=+59.83€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1292 IC=-0.148 PNL=+59.83€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=308 IC=+0.148 PNL=+107.33€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=308 IC=+0.148 PNL=+107.33€
...
**⏳ H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: 45/289 ops en el filtro definido (IC actual=-0.330 PNL=-17.42€)
  - _Datos_: n=45 IC=-0.330 PNL=-17.42€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=793 IC=+0.158 PNL=+421.55€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=793 IC=+0.158 PNL=+421.55€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.208 > 0.08 con n=272 PNL=+156.37€
  - _Datos_: n=272 IC=+0.208 PNL=+156.37€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.248 > 0.08 con n=640 PNL=-68.65€
  - _Datos_: n=640 IC=+0.248 PNL=-68.65€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=1092 IC=+0.098 PNL=-245.84€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=1092 IC=+0.098 PNL=-245.84€
