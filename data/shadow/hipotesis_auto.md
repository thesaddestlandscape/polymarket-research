# Hipótesis automáticas — 2026-07-19 04:34 UTC
_Generado por shadow_postmortem.py sobre 21895 resoluciones (PNL=+5306.04€)_
...
- **PATRÓN** `dist_vwap_pct` > `0.5541` → IC=+0.133 (n=205)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.5541 (IC base=+0.115)
...
- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.124 (n=402)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` < 0.0064 (IC base=+0.097)

- **PATRÓN** `dist_vwap_pct` > `0.5433` → IC=+0.200 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5433 (IC base=+0.097)
...
- **PATRÓN** `sigma_h` < `0.0121` → IC=+0.156 (n=242)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0121 (IC base=+0.153)

- **PATRÓN** `sigma_h` > `0.0154` → IC=+0.177 (n=366)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.161 (n=367)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.1135` → IC=+0.154 (n=203)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.1135 (IC base=+0.153)
...
- **PATRÓN** `sigma_h` > `0.0116` → IC=+0.239 (n=354)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.5887` → IC=+0.221 (n=188)
...
- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.131 (n=253)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `dist_vwap_pct` > `0.6146` → IC=+0.184 (n=36)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.6146 (IC base=+0.128)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.89` → IC=+0.211 (n=50)
...
- **PATRÓN** `sigma_h` < `0.017` → IC=+0.263 (n=243)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.017 (IC base=+0.257)

- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.271 (n=243)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.266 (n=250)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.257)

- **PATRÓN** `dist_vwap_pct` > `0.2787` → IC=+0.325 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2787 (IC base=+0.257)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.831` → IC=+0.394 (n=64)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` < `0.0053` → IC=+0.130 (n=325)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` < 0.0053 (IC base=+0.128)
...
- **PATRÓN** `hora_utc` < `16.0` → IC=+0.133 (n=900)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` < 16.0 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` > `0.5363` → IC=+0.172 (n=193)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.5363 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` < `0.1169` → IC=+0.152 (n=493)
...
- **PATRÓN** `sigma_h` < `0.0045` → IC=+0.158 (n=147)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0045 (IC base=+0.116)

- **PATRÓN** `sigma_h` > `0.007` → IC=+0.127 (n=73)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.63€ cuando `sigma_h` > 0.007 (IC base=+0.116)
...
- **PATRÓN** `dist_vwap_pct` > `0.5693` → IC=+0.192 (n=37)
  ...
  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.5693 (IC base=+0.116)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.72` → IC=+0.262 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.72 (IC base=+0.116)
...
- **PATRÓN** `sigma_h` < `0.0282` → IC=+0.212 (n=307)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0282 (IC base=+0.210)
...
- **PATRÓN** `dist_vwap_pct` > `0.2409` → IC=+0.263 (n=95)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2409 (IC base=+0.210)

- **PATRÓN** `dist_vwap_pct` < `0.0994` → IC=+0.211 (n=171)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0994 (IC base=+0.210)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.899` → IC=+0.292 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.899 (IC base=+0.210)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.588` → IC=-0.155 (n=27)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.128 (n=283)
...
- **PATRÓN** `ibs_15` < `0.0263` → IC=+0.120 (n=177)
  ...
  - _Acción_: Kelly boost +0.60€ cuando `ibs_15` < 0.0263 (IC base=+0.060)

- **PATRÓN** `dist_vwap_pct` > `0.8904` → IC=+0.140 (n=23)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.8904 (IC base=+0.060)

- **PATRÓN** `sigma_ewma_delta_pct` < `17.588` → IC=+0.128 (n=283)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` < 17.588 (IC base=+0.060)
...
| ✅ FAVORITO_CONFIRMADO | 3674 | +0.185 | +50.87€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 2382 | +0.218 | +91.89€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 1066 | +0.193 | +32.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 795 | +0.210 | +2.47€ | 0 | 6 |
...
| ✅ GBM_LATE_15M | 5718 | +0.106 | +1910.47€ | 0 | 9 |
| ✅ GBM_LATE_15M#15min | 5718 | +0.106 | +1910.47€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1362 | +0.077 | +245.54€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1362 | +0.077 | +245.54€ | 0 | 6 |
...
| ✅ GBM_LATE_15M#XRP | 1494 | +0.162 | +815.05€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1494 | +0.162 | +815.05€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2949 | +0.153 | +1764.77€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2949 | +0.153 | +1764.77€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 752 | +0.117 | +355.26€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 752 | +0.117 | +355.26€ | 0 | 12 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 665 | +0.278 | +703.63€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 665 | +0.278 | +703.63€ | 0 | 12 |
...
| ✅ GBM_LATE_15M_TARDIO | 2893 | +0.101 | +1011.65€ | 0 | 11 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2893 | +0.101 | +1011.65€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 695 | +0.072 | +146.05€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 695 | +0.072 | +146.05€ | 0 | 7 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 852 | +0.214 | +670.40€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 852 | +0.214 | +670.40€ | 0 | 11 |
| ✅ GBM_LATE_5M | 83 | +0.088 | +6.86€ | 0 | 0 |
| ✅ GBM_LATE_5M#5min | 83 | +0.088 | +6.86€ | 0 | 0 |
...
| ✅ GBM_LATE_5M#XRP | 50 | +0.154 | +10.22€ | 0 | 0 |
| ✅ GBM_LATE_5M#XRP#5min | 50 | +0.154 | +10.22€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 1806 | +0.021 | +131.70€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1546 | +0.046 | +169.76€ | 1 | 7 |
| 🚫 UPDOWN_GBM#240min | 11 | -0.148 | -4.31€ | 0 | 0 |
| 🚫 UPDOWN_GBM#5min | 75 | -0.162 | -22.48€ | 4 | 0 |
| ✅ UPDOWN_GBM#60min | 127 | -0.058 | -10.76€ | 7 | 0 |
| ✅ UPDOWN_GBM#BNB | 127 | +0.074 | +32.97€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 127 | +0.074 | +32.97€ | 0 | 0 |
...
| ✅ UPDOWN_GBM#DOGE | 85 | +0.063 | +11.75€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 82 | +0.071 | +12.65€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 568 | +0.186 | +259.16€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 568 | +0.186 | +259.16€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 138 | +0.221 | +96.19€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 138 | +0.221 | +96.19€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.184: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.054 n=718 | neutral: IC=-0.006 n=526 | overbought(IBS>0.7): IC=+0.179 n=664
  - _Datos_: n=2044 IC=+0.086 PNL=+426.71€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.117 n=13155 | tras_1loss IC=+0.087 n=8544 | tras_2loss IC=+0.054 n=3492/40 | gap=+0.063 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1676 IC=+0.028 PNL=+148.98€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1676 IC=+0.028 PNL=+148.98€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=727 IC=+0.045 PNL=+76.74€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=727 IC=+0.045 PNL=+76.74€

**🟡 H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  - _Hipótesis_: Feature nueva 2026-06-27: drift del precio YES en Polymarket en últimas 5 obs (~5min). Si poly_drift<0 y decidimos BUY_NO (o poly_drift>0 y BUY_YES) → confluencia. Si diverge → reducción de stake. Hipótesis: confluencia Binance+Polymarket mejora IC; divergencia empeora.
  - _Umbral_: n≥40 en confluencia vs divergencia para validar el boost ×1.1
  - _Acción_: Si IC_confluencia>IC_divergencia con n≥40 → mantener el boost. Si no → retirar.
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.179 > 0.08 con n=51 PNL=+22.36€
  - _Datos_: n=51 IC=+0.179 PNL=+22.36€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=19437 IC=+0.121 PNL=+5381.92€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=19437 IC=+0.121 PNL=+5381.92€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=196 IC=+0.015 PNL=+10.37€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=196 IC=+0.015 PNL=+10.37€
...
**🟡 H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.132 > 0.1 con n=93 PNL=+36.88€
  - _Datos_: n=93 IC=+0.132 PNL=+36.88€
...
**〰️ H-CUSTOM-GBM-04H-ASIA** — UPDOWN_GBM 04h-05h UTC — media sesión asiática, ¿mejor franja nocturna?
  ...
  - _Estado_: n=121 IC=+0.029 PNL=+17.51€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=121 IC=+0.029 PNL=+17.51€
...
**🟡 H-CUSTOM-LATE-04H-ASIA** — GBM_LATE_15M 04h-05h UTC — media sesión asiática (gemela de GBM-04H-ASIA)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.134 > 0.08 con n=468 PNL=+190.02€
  - _Datos_: n=468 IC=+0.134 PNL=+190.02€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.234 > 0.2 con n=476 PNL=+305.90€
  - _Datos_: n=476 IC=+0.234 PNL=+305.90€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.161 > 0.08 con n=272 PNL=+129.57€
  - _Datos_: n=272 IC=+0.161 PNL=+129.57€
