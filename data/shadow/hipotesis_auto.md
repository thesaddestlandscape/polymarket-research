# Hipótesis automáticas — 2026-09-21 19:43 UTC
_Generado por shadow_postmortem.py sobre 546444 resoluciones (PNL=+60992.95€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `145.51` → IC=-0.234 (n=6595)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 145.51
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=19786)
...
- **FILTRO** `restante_s_al_confirmar` < `133.44` → IC=-0.278 (n=799)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 133.44
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=2400)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `161.67` → IC=-0.229 (n=1555)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 161.67
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=4668)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `122.59` → IC=-0.365 (n=1306)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 122.59
  - _Potencial_: sin este filtro IC_bueno=-0.121 (n=3918)

### CANDIDATA9_BOT_CONSENSO
- **FILTRO** `py_entrada` < `0.47` → IC=-0.233 (n=343)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=391)

- **FILTRO** `py_entrada` > `0.54` → IC=-0.173 (n=224)
...
- **FILTRO** `py_entrada` > `0.64` → IC=-0.162 (n=69)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.176 (n=5366)

  - _Acción_: Kelly boost +0.88€ cuando `libro_spread` < 0.01 (IC base=+0.129)

- **PATRÓN** `libro_liquidez` > `7622.7563` → IC=+0.172 (n=2016)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 7622.7563 (IC base=+0.129)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.207 (n=1442)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.745` → IC=+0.350 (n=726)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.745 (IC base=+0.204)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.204 (n=1984)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.204 (n=1422)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.200 (n=2006)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.198)

- **PATRÓN** `libro_liquidez` > `15455.9709` → IC=+0.210 (n=518)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.152 (n=2647)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.332 (n=890)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.150 (n=432)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1300.2394` → IC=+0.148 (n=618)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.223 (n=670)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.203 (n=10651)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.224 (n=3640)
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.182 (n=1806)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.266 (n=323)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.187 (n=2423)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.248 (n=2280)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.239)

- **PATRÓN** `py_entrada` > `0.76` → IC=+0.320 (n=804)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.200 (n=2480)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.195 (n=1807)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.201 (n=31585)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.198)

- **PATRÓN** `py_entrada` > `0.76` → IC=+0.241 (n=11754)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.76 (IC base=+0.198)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.176 (n=6426)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.189 (n=5870)

  - _Acción_: Kelly boost +0.95€ cuando `py_entrada` > 0.71 (IC base=+0.176)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.227 (n=5655)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.233 (n=2832)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.264 (n=1987)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.75 (IC base=+0.221)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.211 (n=5228)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.195 (n=5284)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `3.15` → IC=+0.143 (n=4409)

  - _Acción_: Kelly boost +0.72€ cuando `lag_apertura_s` < 3.15 (IC base=+0.119)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.200 (n=2446)
...
- **PATRÓN** `restante_min` > `4.94` → IC=+0.140 (n=2304)
...
- **PATRÓN** `lag_apertura_s` < `3.5` → IC=+0.145 (n=2187)
...
- **PATRÓN** `restante_min` > `4.96` → IC=+0.140 (n=2364)

  - _Acción_: Kelly boost +0.70€ cuando `restante_min` > 4.96 (IC base=+0.115)

- **PATRÓN** `lag_apertura_s` < `2.35` → IC=+0.144 (n=2229)

  - _Acción_: Kelly boost +0.72€ cuando `lag_apertura_s` < 2.35 (IC base=+0.115)

### FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.321 (n=739)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1570.4643` → IC=+0.298 (n=1044)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.336 (n=352)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1466.4356` → IC=+0.316 (n=448)
...
### GBM_LATE_15M
- **PATRÓN** `drift_60min` |x|≤ `0.3603` → IC=+0.124 (n=6505)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 0.3603 (IC base=+0.104)

- **PATRÓN** `ibs_20min` > `0.9823` → IC=+0.242 (n=2464)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9823 (IC base=+0.104)

- **PATRÓN** `dist_vwap_pct` < `0.618` → IC=+0.246 (n=2056)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.618 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.935` → IC=+0.177 (n=2850)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 5.935 (IC base=+0.104)

- **PATRÓN** `volumen_regimen` < `1.2155` → IC=+0.242 (n=1976)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2155 (IC base=+0.104)

- **PATRÓN** `volumen_regimen` > `1.0538` → IC=+0.251 (n=896)
...
- **PATRÓN** `volumen_spike_ratio` > `1.9133` → IC=+0.206 (n=3350)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9133 (IC base=+0.104)

- **PATRÓN** `ibs_20min` < `0.5714` → IC=+0.131 (n=8916)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.5714 (IC base=+0.063)

- **PATRÓN** `dist_vwap_pct` > `0.5858` → IC=+0.196 (n=658)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.5858 (IC base=+0.063)

- **PATRÓN** `dist_vwap_pct` < `0.1443` → IC=+0.173 (n=2749)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.1443 (IC base=+0.063)

- **PATRÓN** `volumen_regimen` < `0.6989` → IC=+0.175 (n=1343)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 0.6989 (IC base=+0.063)

- **PATRÓN** `volumen_regimen` > `0.869` → IC=+0.174 (n=2034)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 0.869 (IC base=+0.063)

- **PATRÓN** `volumen_pendiente_norm` > `0.1673` → IC=+0.226 (n=1466)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1673 (IC base=+0.063)

- **PATRÓN** `volumen_spike_ratio` > `1.5803` → IC=+0.202 (n=4544)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5803 (IC base=+0.063)

- **PATRÓN** `ballena_activa_n` < `141.0` → IC=+0.211 (n=4866)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3446` → IC=+0.165 (n=1677)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.82€ cuando `drift_60min` |x|≤ 0.3446 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.164 (n=813)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.416` → IC=+0.252 (n=1287)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6317` → IC=+0.255 (n=370)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.228 (n=1341)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `15859.8442` → IC=+0.226 (n=579)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 15859.8442 (IC base=+0.214)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0761` → IC=+0.156 (n=443)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.0761 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.171 (n=524)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 17.0 (IC base=+0.137)

- **PATRÓN** `ibs_20min` < `0.6849` → IC=+0.167 (n=1324)

  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` < 0.6849 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.1262` → IC=+0.154 (n=1178)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.1262 (IC base=+0.137)
...
- **PATRÓN** `volumen_regimen` < `1.2085` → IC=+0.149 (n=1324)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4384` → IC=+0.147 (n=1214)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 2.4384 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `1.7701` → IC=+0.145 (n=809)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.7701 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `412.0` → IC=+0.145 (n=1130)

  - _Acción_: Kelly boost +0.72€ cuando `ballena_activa_n` < 412.0 (IC base=+0.137)

### GBM_LATE_15M#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0079` → IC=+0.207 (n=1090)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0079 (IC base=+0.187)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.189 (n=1634)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.266 (n=651)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2119` → IC=+0.191 (n=1620)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` < 0.2119 (IC base=+0.187)

- **PATRÓN** `volumen_pendiente_norm` > `0.3648` → IC=+0.194 (n=214)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.3648 (IC base=+0.187)

- **PATRÓN** `volumen_spike_ratio` > `2.9089` → IC=+0.205 (n=701)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9089 (IC base=+0.187)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.197 (n=1111)

  - _Acción_: Kelly boost +0.99€ cuando `libro_spread` < 0.02 (IC base=+0.187)

- **PATRÓN** `sigma_h` < `0.011` → IC=+0.223 (n=1387)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.011 (IC base=+0.216)

- **PATRÓN** `sigma_h` > `0.0066` → IC=+0.217 (n=1242)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0066 (IC base=+0.216)

- **PATRÓN** `drift_60min` |x|≤ `0.5471` → IC=+0.217 (n=1387)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.5471 (IC base=+0.216)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.252 (n=527)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.216)

- **PATRÓN** `ibs_20min` < `0.1951` → IC=+0.237 (n=925)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8237` → IC=+0.207 (n=554)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8237 (IC base=+0.216)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.226 (n=887)
...
- **PATRÓN** `ballena_activa_n` < `25.0` → IC=+0.221 (n=823)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.149 (n=92)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=2057)

- **PATRÓN** `ibs_20min` > `0.9414` → IC=+0.204 (n=333)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9414 (IC base=+0.021)

- **PATRÓN** `dist_vwap_pct` > `0.3551` → IC=+0.324 (n=134)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3551 (IC base=+0.021)

- **PATRÓN** `dist_vwap_pct` < `0.7574` → IC=+0.326 (n=320)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.7574 (IC base=+0.021)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.725` → IC=+0.151 (n=654)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 4.725 (IC base=+0.021)
...
- **PATRÓN** `ballena_activa_n` < `164.0` → IC=+0.328 (n=283)
...
- **FILTRO** `hora_utc` < `17.0` → IC=-0.155 (n=56)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.087 (n=296)
...
- **FILTRO** `ibs_20min` > `0.2632` → IC=-0.127 (n=2031)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2632
  - _Potencial_: sin este filtro IC_bueno=+0.125 (n=1007)

- **FILTRO** `sigma_ewma_delta_pct` > `8.666` → IC=-0.207 (n=329)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.666
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=2709)
...
- **PATRÓN** `dist_vwap_pct` > `1.6434` → IC=+0.300 (n=18)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.6434 (IC base=+0.048)

- **PATRÓN** `dist_vwap_pct` < `0.5528` → IC=+0.272 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5528 (IC base=+0.048)

- **PATRÓN** `volumen_regimen` < `0.5788` → IC=+0.257 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.5788 (IC base=+0.048)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0735` → IC=+0.308 (n=102)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0735 (IC base=+0.048)

- **PATRÓN** `volumen_spike_ratio` < `2.2264` → IC=+0.287 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.2264 (IC base=+0.048)

- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.288 (n=102)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 48.0 (IC base=+0.048)

- **PATRÓN** `ibs_20min` < `0.2632` → IC=+0.125 (n=1007)

  - _Acción_: Kelly boost +0.63€ cuando `ibs_20min` < 0.2632 (IC base=-0.043)

- **PATRÓN** `dist_vwap_pct` > `0.6573` → IC=+0.258 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6573 (IC base=-0.043)

- **PATRÓN** `volumen_regimen` < `0.6948` → IC=+0.250 (n=138)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6948 (IC base=-0.043)
...
- **PATRÓN** `volumen_spike_ratio` < `2.456` → IC=+0.262 (n=254)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.456 (IC base=-0.043)

### GBM_LATE_15M#XRP#15min
- **FILTRO** `drift_60min` |x|> `0.6331` → IC=-0.188 (n=514)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6331
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=1546)

- **FILTRO** `ibs_20min` < `0.6977` → IC=-0.157 (n=1358)

  - _Acción_: SKIP cuando `ibs_20min` < 0.6977
  - _Potencial_: sin este filtro IC_bueno=+0.088 (n=702)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.196 (n=396)

  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=1664)

- **FILTRO** `ibs_20min` > `0.7751` → IC=-0.201 (n=767)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7751
  - _Potencial_: sin este filtro IC_bueno=+0.035 (n=2303)
...
- **PATRÓN** `dist_vwap_pct` > `1.052` → IC=+0.290 (n=98)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.052 (IC base=-0.024)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1044` → IC=+0.285 (n=231)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1044 (IC base=-0.024)
...
- **PATRÓN** `sigma_h` > `0.0096` → IC=+0.195 (n=3075)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0096 (IC base=+0.096)

- **PATRÓN** `ibs_20min` > `0.4713` → IC=+0.187 (n=8234)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` > 0.4713 (IC base=+0.096)

- **PATRÓN** `dist_vwap_pct` > `1.0465` → IC=+0.293 (n=755)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0465 (IC base=+0.096)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.556` → IC=+0.155 (n=4370)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 3.556 (IC base=+0.096)

- **PATRÓN** `volumen_regimen` > `0.685` → IC=+0.244 (n=2904)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.685 (IC base=+0.096)

- **PATRÓN** `volumen_pendiente_norm` > `0.2974` → IC=+0.263 (n=777)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2974 (IC base=+0.096)

- **PATRÓN** `volumen_spike_ratio` < `1.472` → IC=+0.239 (n=1767)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.472 (IC base=+0.096)

- **PATRÓN** `volumen_spike_ratio` > `2.6984` → IC=+0.239 (n=1765)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6984 (IC base=+0.096)

- **PATRÓN** `ballena_activa_n` < `98.0` → IC=+0.267 (n=4803)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 98.0 (IC base=+0.096)

- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.144 (n=3105)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` > 0.0089 (IC base=+0.070)

- **PATRÓN** `ibs_20min` < `0.5532` → IC=+0.149 (n=8192)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` < 0.5532 (IC base=+0.070)

- **PATRÓN** `dist_vwap_pct` > `0.9749` → IC=+0.235 (n=436)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9749 (IC base=+0.070)

- **PATRÓN** `dist_vwap_pct` < `0.2409` → IC=+0.236 (n=2510)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2409 (IC base=+0.070)

- **PATRÓN** `volumen_regimen` < `0.7148` → IC=+0.236 (n=1178)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3445` → IC=+0.260 (n=1582)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3445 (IC base=+0.070)

- **PATRÓN** `ballena_activa_n` < `82.0` → IC=+0.262 (n=3364)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 82.0 (IC base=+0.070)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `sigma_ewma_delta_pct` > `4.497` → IC=-0.166 (n=498)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.497
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=1674)

- **PATRÓN** `ibs_20min` > `0.8938` → IC=+0.264 (n=633)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8938 (IC base=+0.046)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.285` → IC=+0.164 (n=872)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4395` → IC=+0.173 (n=270)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.4395 (IC base=+0.046)

- **PATRÓN** `volumen_spike_ratio` > `2.1445` → IC=+0.188 (n=367)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 2.1445 (IC base=+0.046)
...
- **FILTRO** `ibs_20min` > `0.8168` → IC=-0.150 (n=670)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8168
  - _Potencial_: sin este filtro IC_bueno=+0.049 (n=2012)

- **PATRÓN** `ibs_20min` > `0.8645` → IC=+0.162 (n=619)

  - _Acción_: Kelly boost +0.81€ cuando `ibs_20min` > 0.8645 (IC base=+0.024)

- **PATRÓN** `dist_vwap_pct` > `0.299` → IC=+0.165 (n=344)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.299 (IC base=+0.024)

- **PATRÓN** `volumen_regimen` > `0.6674` → IC=+0.159 (n=757)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 0.6674 (IC base=+0.024)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4243` → IC=+0.198 (n=276)
  ...
  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 1.4243 (IC base=+0.024)
...
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.288 (n=975)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0077 (IC base=+0.246)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.246 (n=1471)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.299 (n=778)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1386` → IC=+0.259 (n=1294)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1386 (IC base=+0.246)

- **PATRÓN** `volumen_spike_ratio` > `2.9235` → IC=+0.264 (n=624)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9235 (IC base=+0.246)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.258 (n=984)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.246)

- **PATRÓN** `libro_liquidez` > `1924.86` → IC=+0.253 (n=488)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1924.86 (IC base=+0.246)
...
- **FILTRO** `ibs_20min` < `0.2772` → IC=-0.195 (n=444)
...
- **FILTRO** `ibs_20min` > `0.7832` → IC=-0.181 (n=549)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.047 (n=1652)

- **PATRÓN** `ibs_20min` > `0.807` → IC=+0.157 (n=605)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` > 0.807 (IC base=+0.009)

- **PATRÓN** `dist_vwap_pct` > `0.4595` → IC=+0.230 (n=228)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4595 (IC base=+0.009)

- **PATRÓN** `volumen_regimen` < `0.9855` → IC=+0.233 (n=433)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9855 (IC base=+0.009)

- **PATRÓN** `volumen_regimen` > `0.6499` → IC=+0.214 (n=439)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6499 (IC base=+0.009)

- **PATRÓN** `volumen_pendiente_norm` > `0.2675` → IC=+0.274 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2675 (IC base=+0.009)

- **PATRÓN** `volumen_spike_ratio` < `2.0972` → IC=+0.246 (n=407)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0972 (IC base=+0.009)

- **PATRÓN** `ballena_activa_n` < `102.0` → IC=+0.262 (n=313)
...
- **PATRÓN** `dist_vwap_pct` < `0.6516` → IC=+0.180 (n=407)
  ...
  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.6516 (IC base=-0.010)

- **PATRÓN** `volumen_regimen` < `1.1649` → IC=+0.193 (n=360)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` < 1.1649 (IC base=-0.010)
...
- **FILTRO** `ibs_20min` < `0.7143` → IC=-0.203 (n=978)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7143
  - _Potencial_: sin este filtro IC_bueno=+0.274 (n=978)

- **FILTRO** `ibs_20min` > `0.6923` → IC=-0.235 (n=507)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6923
  - _Potencial_: sin este filtro IC_bueno=+0.094 (n=1551)

- **FILTRO** `sigma_ewma_delta_pct` > `4.689` → IC=-0.180 (n=457)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.689
  - _Potencial_: sin este filtro IC_bueno=+0.068 (n=1601)

- **PATRÓN** `ibs_20min` > `0.7143` → IC=+0.274 (n=978)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7143 (IC base=+0.035)

- **PATRÓN** `dist_vwap_pct` > `0.2978` → IC=+0.326 (n=400)
...
- **PATRÓN** `volumen_regimen` < `0.8648` → IC=+0.297 (n=477)
...
- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.309 (n=673)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 62.0 (IC base=+0.035)

- **PATRÓN** `ibs_20min` < `0.1` → IC=+0.208 (n=519)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0977` → IC=+0.210 (n=453)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5113` → IC=+0.223 (n=463)
...
- **PATRÓN** `sigma_h` > `0.0163` → IC=+0.318 (n=799)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2797` → IC=+0.314 (n=181)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2797 (IC base=+0.276)

- **PATRÓN** `volumen_spike_ratio` > `2.5322` → IC=+0.289 (n=378)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5322 (IC base=+0.276)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.280 (n=1264)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.276)

- **PATRÓN** `libro_liquidez` > `2607.8846` → IC=+0.286 (n=799)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2607.8846 (IC base=+0.276)
...
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.171 (n=2432)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.0896` → IC=+0.185 (n=2427)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.0896 (IC base=+0.168)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.179 (n=7611)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 5.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` > `0.5824` → IC=+0.217 (n=7277)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5824 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` > `0.1719` → IC=+0.195 (n=3200)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.1719 (IC base=+0.168)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.237` → IC=+0.258 (n=1491)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.237 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` < `1.2177` → IC=+0.161 (n=4813)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2177 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` > `0.6258` → IC=+0.160 (n=4812)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.6258 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.2462` → IC=+0.195 (n=1461)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2462 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` < `1.5641` → IC=+0.170 (n=3062)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` < 1.5641 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.6366` → IC=+0.177 (n=2320)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 2.6366 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `2385.1196` → IC=+0.169 (n=4851)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2385.1196 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `118.0` → IC=+0.181 (n=6194)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 118.0 (IC base=+0.168)

- **PATRÓN** `sigma_h` < `0.0065` → IC=+0.183 (n=4592)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.207 (n=2310)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` < `0.4791` → IC=+0.226 (n=6887)
...
- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.188 (n=834)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0065 (IC base=+0.183)

- **PATRÓN** `drift_60min` |x|≤ `0.3418` → IC=+0.204 (n=1248)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.249 (n=795)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.1111` → IC=+0.279 (n=587)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1111 (IC base=+0.241)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.095` → IC=+0.255 (n=950)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.188 (n=1140)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 5.0 (IC base=+0.164)

- **PATRÓN** `ibs_20min` > `0.4118` → IC=+0.229 (n=1080)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4118 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.2039` → IC=+0.214 (n=656)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2039 (IC base=+0.164)
...
- **PATRÓN** `volumen_regimen` < `1.2679` → IC=+0.169 (n=1081)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 1.2679 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` > `1.0742` → IC=+0.169 (n=490)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 1.0742 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.2334` → IC=+0.202 (n=236)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2334 (IC base=+0.164)
...
- **PATRÓN** `libro_liquidez` > `15793.9951` → IC=+0.175 (n=490)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 15793.9951 (IC base=+0.164)

- **PATRÓN** `sigma_h` < `0.0025` → IC=+0.199 (n=393)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0025 (IC base=+0.135)

- **PATRÓN** `drift_60min` |x|≤ `0.2905` → IC=+0.155 (n=1174)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.2905 (IC base=+0.135)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.170 (n=574)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 15.0 (IC base=+0.135)

- **PATRÓN** `ibs_20min` < `0.5583` → IC=+0.183 (n=1175)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `11.882` → IC=+0.211 (n=230)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.882 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` < `1.2116` → IC=+0.155 (n=1174)
...
- **PATRÓN** `volumen_spike_ratio` < `2.442` → IC=+0.143 (n=1064)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2196` → IC=+0.217 (n=814)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2196 (IC base=+0.199)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.226 (n=414)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.199)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.293 (n=651)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2104` → IC=+0.198 (n=1178)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` < 0.2104 (IC base=+0.199)

- **PATRÓN** `volumen_pendiente_norm` > `0.1338` → IC=+0.197 (n=476)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.1338 (IC base=+0.199)

- **PATRÓN** `volumen_spike_ratio` > `2.9043` → IC=+0.209 (n=523)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9043 (IC base=+0.199)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.212 (n=829)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.280 (n=352)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.223)

- **PATRÓN** `ibs_20min` < `0.25` → IC=+0.257 (n=880)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.25 (IC base=+0.223)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6281` → IC=+0.229 (n=304)
...
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.174 (n=1021)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0067 (IC base=+0.147)

- **PATRÓN** `drift_60min` |x|≤ `0.4327` → IC=+0.163 (n=1160)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.4327 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.164 (n=1160)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 6.0 (IC base=+0.147)

- **PATRÓN** `ibs_20min` > `0.3926` → IC=+0.200 (n=1160)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3926 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.157` → IC=+0.182 (n=784)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.157 (IC base=+0.147)
...
- **PATRÓN** `volumen_regimen` < `1.05` → IC=+0.153 (n=1021)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.05 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` > `0.6319` → IC=+0.152 (n=1160)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5292` → IC=+0.176 (n=378)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.5292 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `6954.443` → IC=+0.185 (n=773)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 6954.443 (IC base=+0.147)

- **PATRÓN** `ballena_activa_n` < `167.0` → IC=+0.150 (n=1097)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.135 (n=1355)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 5.0 (IC base=+0.116)

- **PATRÓN** `ibs_20min` > `0.5217` → IC=+0.200 (n=1317)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5217 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` > `0.8335` → IC=+0.212 (n=401)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8335 (IC base=+0.116)
...
- **PATRÓN** `volumen_regimen` < `1.2249` → IC=+0.127 (n=1318)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.124 (n=1373)

  - _Acción_: Kelly boost +0.62€ cuando `libro_spread` < 0.02 (IC base=+0.116)

- **PATRÓN** `libro_liquidez` > `2892.276` → IC=+0.195 (n=598)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 2892.276 (IC base=+0.116)

- **PATRÓN** `ballena_activa_n` < `50.0` → IC=+0.136 (n=1002)
...
- **PATRÓN** `sigma_h` > `0.0185` → IC=+0.210 (n=837)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0185 (IC base=+0.199)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.203 (n=1307)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_spike_ratio` < `2.1646` → IC=+0.211 (n=1065)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1646 (IC base=+0.199)

- **PATRÓN** `volumen_spike_ratio` > `1.8081` → IC=+0.207 (n=807)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8081 (IC base=+0.199)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.202 (n=1318)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.199)

- **PATRÓN** `libro_liquidez` > `2598.9768` → IC=+0.202 (n=837)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2598.9768 (IC base=+0.199)
...
- **PATRÓN** `sigma_h` > `0.009` → IC=+0.172 (n=565)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.009 (IC base=+0.147)

- **PATRÓN** `drift_60min` |x|≤ `0.1356` → IC=+0.153 (n=747)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.4` → IC=+0.181 (n=1695)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` > 0.4 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.8519` → IC=+0.191 (n=273)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.8519 (IC base=+0.147)
...
- **PATRÓN** `volumen_regimen` < `0.8735` → IC=+0.162 (n=981)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.8735 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` > `1.2084` → IC=+0.155 (n=491)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 1.2084 (IC base=+0.147)

- **PATRÓN** `volumen_pendiente_norm` > `0.1634` → IC=+0.180 (n=470)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1634 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` < `1.4373` → IC=+0.162 (n=543)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 1.4373 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` > `2.5508` → IC=+0.170 (n=543)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.5508 (IC base=+0.147)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.150 (n=1907)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.02 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `3267.1149` → IC=+0.151 (n=1130)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 3267.1149 (IC base=+0.147)

- **PATRÓN** `ballena_activa_n` < `167.0` → IC=+0.166 (n=1476)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 167.0 (IC base=+0.147)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.127 (n=1183)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.64€ cuando `sigma_h` < 0.0057 (IC base=+0.099)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.121 (n=1185)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` > 11.0 (IC base=+0.099)

- **PATRÓN** `ibs_20min` < `0.0548` → IC=+0.195 (n=591)

  - _Acción_: Kelly boost +0.97€ cuando `ibs_20min` < 0.0548 (IC base=+0.099)
...
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.210 (n=167)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.007 (IC base=+0.205)

- **PATRÓN** `drift_60min` |x|≤ `0.0953` → IC=+0.222 (n=167)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0953 (IC base=+0.205)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.251 (n=251)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.205)

- **PATRÓN** `ibs_20min` > `0.7017` → IC=+0.270 (n=333)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7017 (IC base=+0.205)

- **PATRÓN** `dist_vwap_pct` > `0.15` → IC=+0.234 (n=269)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.15 (IC base=+0.205)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.052` → IC=+0.238 (n=166)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.052 (IC base=+0.205)

- **PATRÓN** `volumen_regimen` < `0.8422` → IC=+0.216 (n=333)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8422 (IC base=+0.205)

- **PATRÓN** `volumen_regimen` > `1.1573` → IC=+0.216 (n=167)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1573 (IC base=+0.205)

- **PATRÓN** `volumen_pendiente_norm` > `0.2555` → IC=+0.340 (n=73)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2555 (IC base=+0.205)

- **PATRÓN** `volumen_spike_ratio` < `1.3751` → IC=+0.241 (n=164)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3751 (IC base=+0.205)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.208 (n=552)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.011` → IC=+0.204 (n=3079)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.011 (IC base=+0.168)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.177 (n=9662)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 5.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` > `0.4731` → IC=+0.217 (n=9233)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4731 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` > `0.9446` → IC=+0.203 (n=1315)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9446 (IC base=+0.168)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.603` → IC=+0.227 (n=4494)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.603 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` < `0.882` → IC=+0.165 (n=4117)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.882 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.2399` → IC=+0.198 (n=1727)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.2399 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.6166` → IC=+0.188 (n=2950)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 2.6166 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `2332.3562` → IC=+0.170 (n=6155)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 2332.3562 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `89.0` → IC=+0.194 (n=6936)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 89.0 (IC base=+0.168)

- **PATRÓN** `sigma_h` < `0.0091` → IC=+0.188 (n=7357)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0091 (IC base=+0.181)

- **PATRÓN** `drift_60min` |x|≤ `0.485` → IC=+0.183 (n=8359)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.485 (IC base=+0.181)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.206 (n=3190)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.181)

- **PATRÓN** `ibs_20min` < `0.5665` → IC=+0.238 (n=8359)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.93` → IC=+0.202 (n=1175)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.93 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.71` → IC=+0.182 (n=8110)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.71 (IC base=+0.181)

- **PATRÓN** `volumen_regimen` < `0.7046` → IC=+0.160 (n=2545)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.7046 (IC base=+0.181)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6455` → IC=+0.193 (n=2539)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 2.6455 (IC base=+0.181)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.190 (n=4873)
...
- **PATRÓN** `drift_60min` |x|≤ `0.349` → IC=+0.190 (n=1573)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.349 (IC base=+0.189)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.195 (n=758)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.320 (n=563)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2271` → IC=+0.247 (n=287)
...
- **PATRÓN** `sigma_h` > `0.005` → IC=+0.266 (n=1072)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.270 (n=1089)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.35` → IC=+0.293 (n=1055)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.35 (IC base=+0.260)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.466` → IC=+0.268 (n=1263)
...
- **PATRÓN** `libro_liquidez` > `1774.62` → IC=+0.275 (n=799)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0842` → IC=+0.169 (n=494)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.0842 (IC base=+0.149)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=1549)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.149)

- **PATRÓN** `ibs_20min` > `0.3136` → IC=+0.202 (n=1480)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3136 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` > `0.1256` → IC=+0.183 (n=853)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.1256 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.756` → IC=+0.167 (n=340)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 9.756 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.242` → IC=+0.154 (n=1326)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2706` → IC=+0.198 (n=213)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.2706 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` < `2.1192` → IC=+0.160 (n=1255)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.1192 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` > `1.5017` → IC=+0.153 (n=1273)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.5017 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `13722.2224` → IC=+0.152 (n=987)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 13722.2224 (IC base=+0.149)

- **PATRÓN** `ballena_activa_n` < `480.0` → IC=+0.156 (n=1356)

  - _Acción_: Kelly boost +0.78€ cuando `ballena_activa_n` < 480.0 (IC base=+0.149)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.162 (n=1276)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0057 (IC base=+0.149)

- **PATRÓN** `drift_60min` |x|≤ `0.3199` → IC=+0.159 (n=1276)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.3199 (IC base=+0.149)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.177 (n=428)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 18.0 (IC base=+0.149)

- **PATRÓN** `ibs_20min` < `0.6486` → IC=+0.191 (n=1276)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` < 0.6486 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` > `0.692` → IC=+0.159 (n=218)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.692 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` < `0.1293` → IC=+0.164 (n=1151)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.1293 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.493` → IC=+0.164 (n=218)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 11.493 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` < `1.1917` → IC=+0.161 (n=1276)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4238` → IC=+0.160 (n=1178)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.4238 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` > `1.7535` → IC=+0.160 (n=785)
...
- **PATRÓN** `sigma_h` > `0.0099` → IC=+0.245 (n=676)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0099 (IC base=+0.218)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.224 (n=1562)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.6739` → IC=+0.258 (n=1330)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2144` → IC=+0.224 (n=1460)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2144 (IC base=+0.218)

- **PATRÓN** `volumen_spike_ratio` > `2.8829` → IC=+0.241 (n=640)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8829 (IC base=+0.218)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.231 (n=1010)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.218)

- **PATRÓN** `libro_liquidez` > `1927.641` → IC=+0.223 (n=496)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1927.641 (IC base=+0.218)

- **PATRÓN** `sigma_h` < `0.0109` → IC=+0.241 (n=1377)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0109 (IC base=+0.236)

- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.240 (n=1231)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.262 (n=524)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.236)
...
- **PATRÓN** `ibs_20min` < `0.2` → IC=+0.283 (n=921)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7836` → IC=+0.238 (n=551)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7836 (IC base=+0.236)

- **PATRÓN** `volumen_spike_ratio` > `2.2178` → IC=+0.237 (n=834)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2178 (IC base=+0.236)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.249 (n=881)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4373` → IC=+0.144 (n=1576)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.4373 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.149 (n=1653)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 5.0 (IC base=+0.137)

- **PATRÓN** `ibs_20min` > `0.8801` → IC=+0.263 (n=714)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8801 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.3645` → IC=+0.169 (n=636)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.3645 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.203` → IC=+0.163 (n=656)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 4.203 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `0.8781` → IC=+0.158 (n=1050)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5194` → IC=+0.151 (n=669)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 1.5194 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `2.1374` → IC=+0.155 (n=690)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 2.1374 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `8150.9116` → IC=+0.230 (n=714)
...
- **PATRÓN** `sigma_h` > `0.008` → IC=+0.134 (n=1045)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` > 0.008 (IC base=+0.107)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.168 (n=583)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 17.0 (IC base=+0.107)

- **PATRÓN** `ibs_20min` > `0.4722` → IC=+0.188 (n=1561)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.4722 (IC base=+0.107)

- **PATRÓN** `dist_vwap_pct` > `1.0688` → IC=+0.198 (n=313)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 1.0688 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.408` → IC=+0.233 (n=590)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.408 (IC base=+0.107)

- **PATRÓN** `volumen_regimen` < `0.8904` → IC=+0.135 (n=1041)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 0.8904 (IC base=+0.107)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.120 (n=1564)

  - _Acción_: Kelly boost +0.60€ cuando `libro_spread` < 0.02 (IC base=+0.107)

- **PATRÓN** `libro_liquidez` > `2909.4755` → IC=+0.246 (n=521)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.126 (n=1590)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 5.0 (IC base=+0.112)

- **PATRÓN** `ibs_20min` < `0.6371` → IC=+0.206 (n=1534)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.446` → IC=+0.127 (n=1482)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1912` → IC=+0.126 (n=624)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_spike_ratio` > 2.1912 (IC base=+0.112)

- **PATRÓN** `libro_liquidez` > `2859.0238` → IC=+0.169 (n=511)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2859.0238 (IC base=+0.112)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` > `0.0189` → IC=+0.216 (n=1040)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0189 (IC base=+0.206)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.211 (n=1630)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.5143` → IC=+0.247 (n=1562)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5143 (IC base=+0.206)

- **PATRÓN** `dist_vwap_pct` > `0.865` → IC=+0.239 (n=438)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.865 (IC base=+0.206)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.498` → IC=+0.251 (n=744)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.498 (IC base=+0.206)

- **PATRÓN** `volumen_regimen` > `0.6356` → IC=+0.214 (n=1560)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6356 (IC base=+0.206)

- **PATRÓN** `volumen_pendiente_norm` > `0.2346` → IC=+0.244 (n=275)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2346 (IC base=+0.206)

- **PATRÓN** `volumen_spike_ratio` > `2.5223` → IC=+0.236 (n=501)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5223 (IC base=+0.206)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.213 (n=1614)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.206)

- **PATRÓN** `libro_liquidez` > `2603.9534` → IC=+0.215 (n=1040)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2603.9534 (IC base=+0.206)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.135 (n=2832)

- **PATRÓN** `sigma_h` < `0.0093` → IC=+0.159 (n=2408)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0093 (IC base=+0.150)

- **PATRÓN** `drift_60min` |x|≤ `0.5254` → IC=+0.160 (n=2734)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.5254 (IC base=+0.150)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.162 (n=1091)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9407` → IC=+0.209 (n=913)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9407 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.1905` → IC=+0.161 (n=980)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.1905 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` < `0.505` → IC=+0.141 (n=1616)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.505 (IC base=+0.150)
...
- **PATRÓN** `volumen_regimen` > `0.9028` → IC=+0.162 (n=1151)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.150 (n=1826)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.01 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `3652.7364` → IC=+0.154 (n=1823)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4815` → IC=+0.153 (n=2136)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.135 (n=2832)

  - _Acción_: Kelly boost +0.67€ cuando `libro_spread` < 0.01 (IC base=+0.133)

- **PATRÓN** `libro_liquidez` > `7164.321` → IC=+0.148 (n=1908)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 7164.321 (IC base=+0.133)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.190 (n=69)
  ...
  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` > 1.0 (IC base=+0.068)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.026` → IC=+0.200 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.026 (IC base=+0.068)

- **PATRÓN** `volumen_pendiente_norm` > `0.1583` → IC=+0.212 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1583 (IC base=+0.068)

- **PATRÓN** `libro_liquidez` > `3398.8602` → IC=+0.128 (n=162)

  - _Acción_: Kelly boost +0.64€ cuando `libro_liquidez` > 3398.8602 (IC base=+0.068)

- **PATRÓN** `sigma_h` > `0.0119` → IC=+0.204 (n=79)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0119 (IC base=+0.071)

- **PATRÓN** `drift_60min` |x|≤ `0.3786` → IC=+0.136 (n=116)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.3786 (IC base=+0.071)

- **PATRÓN** `ibs_20min` < `0.1` → IC=+0.183 (n=58)

  - _Acción_: Kelly boost +0.92€ cuando `ibs_20min` < 0.1 (IC base=+0.071)

- **PATRÓN** `dist_vwap_pct` > `0.6396` → IC=+0.167 (n=88)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.6396 (IC base=+0.071)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.123 (n=845)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 6.0 (IC base=+0.103)

- **PATRÓN** `ibs_20min` > `0.5717` → IC=+0.194 (n=721)

  - _Acción_: Kelly boost +0.97€ cuando `ibs_20min` > 0.5717 (IC base=+0.103)

- **PATRÓN** `dist_vwap_pct` > `0.3593` → IC=+0.169 (n=285)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` > 0.3593 (IC base=+0.103)
...
- **PATRÓN** `volumen_regimen` < `0.8064` → IC=+0.123 (n=481)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_regimen` < 0.8064 (IC base=+0.103)

- **PATRÓN** `volumen_regimen` > `0.9793` → IC=+0.138 (n=327)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` > 0.9793 (IC base=+0.103)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4387` → IC=+0.147 (n=612)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 2.4387 (IC base=+0.103)

- **PATRÓN** `volumen_spike_ratio` > `1.4015` → IC=+0.129 (n=612)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.4015 (IC base=+0.103)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.132 (n=585)

  - _Acción_: Kelly boost +0.66€ cuando `libro_spread` < 0.02 (IC base=+0.103)

- **PATRÓN** `libro_liquidez` > `2438.1282` → IC=+0.154 (n=316)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 2438.1282 (IC base=+0.103)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.124 (n=280)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 7.0 (IC base=+0.112)

- **PATRÓN** `ibs_20min` > `0.5656` → IC=+0.201 (n=249)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5656 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `0.1296` → IC=+0.179 (n=132)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.1296 (IC base=+0.112)

- **PATRÓN** `volumen_regimen` < `1.0554` → IC=+0.129 (n=219)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` < 1.0554 (IC base=+0.112)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0118` → IC=+0.201 (n=185)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0118 (IC base=+0.112)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.128 (n=253)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.01 (IC base=+0.112)

- **PATRÓN** `libro_liquidez` > `3972.2372` → IC=+0.133 (n=107)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 3972.2372 (IC base=+0.112)
...
- **PATRÓN** `sigma_h` < `0.005` → IC=+0.186 (n=186)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.005 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.147 (n=250)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 8.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` > `0.557` → IC=+0.216 (n=248)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.557 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` > `0.3362` → IC=+0.173 (n=102)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.3362 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.04` → IC=+0.309 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.04 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` < `0.8141` → IC=+0.143 (n=166)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 0.8141 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` > `0.6345` → IC=+0.134 (n=222)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` > 0.6345 (IC base=+0.114)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7354` → IC=+0.157 (n=132)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 1.7354 (IC base=+0.114)

- **PATRÓN** `volumen_spike_ratio` > `1.5019` → IC=+0.137 (n=177)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` > 1.5019 (IC base=+0.114)

- **PATRÓN** `libro_spread` < `0.013` → IC=+0.142 (n=163)

  - _Acción_: Kelly boost +0.71€ cuando `libro_spread` < 0.013 (IC base=+0.114)

- **PATRÓN** `libro_liquidez` > `1116.0758` → IC=+0.164 (n=218)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 1116.0758 (IC base=+0.114)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.029 (n=1576)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.176 (n=3103)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.057 (n=9803)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.174 (n=3252)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.033 (n=10083)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.45` → IC=-0.213 (n=553)

  - _Acción_: SKIP cuando `py_entrada` < 0.45
  - _Potencial_: sin este filtro IC_bueno=+0.102 (n=1670)
...
- **FILTRO** `py_entrada` < `0.45` → IC=-0.204 (n=555)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.45
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=1711)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.204 (n=589)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.061 (n=1784)

- **FILTRO** `ibs_20min` > `0.2846` → IC=-0.164 (n=593)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2846
  - _Potencial_: sin este filtro IC_bueno=+0.048 (n=1780)

- **PATRÓN** `libro_liquidez` > `1725.1772` → IC=+0.122 (n=771)
...
- **FILTRO** `py_entrada` < `0.48` → IC=-0.183 (n=543)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.48
  - _Potencial_: sin este filtro IC_bueno=+0.083 (n=1659)

- **FILTRO** `py_entrada` > `0.58` → IC=-0.193 (n=568)

  - _Acción_: SKIP cuando `py_entrada` > 0.58
  - _Potencial_: sin este filtro IC_bueno=+0.054 (n=1805)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=2794)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=2800)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=9157)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=20634)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.275 (n=7183)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=22608)

- **FILTRO** `ibs_7min` < `0.2945` → IC=-0.236 (n=7446)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2945
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=22345)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.157 (n=10080)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=19711)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.229 (n=9256)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=-0.000 (n=27938)

- **FILTRO** `ibs_7min` > `0.2941` → IC=-0.180 (n=9289)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2941
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=27905)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.308 (n=1165)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=3720)

- **FILTRO** `ibs_7min` < `0.7103` → IC=-0.253 (n=1612)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7103
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3273)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.182 (n=1151)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=3734)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.257 (n=1600)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=4814)

- **FILTRO** `drift_7min_pct` |x|> `0.1121` → IC=-0.125 (n=2179)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1121
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=4235)

- **FILTRO** `ibs_7min` > `0.7929` → IC=-0.206 (n=1603)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7929
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=4811)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.137 (n=1204)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=3970)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.249 (n=1240)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=3934)

- **FILTRO** `ibs_7min` < `0.7518` → IC=-0.189 (n=1293)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7518
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=3881)

- **FILTRO** `ballena_activa_n` > `160.0` → IC=-0.172 (n=1285)

  - _Acción_: SKIP cuando `ballena_activa_n` > 160.0
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=3889)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.260 (n=1287)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=3915)

- **FILTRO** `ibs_7min` > `0.2588` → IC=-0.174 (n=1299)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2588
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=3903)

- **FILTRO** `ballena_activa_n` > `154.0` → IC=-0.187 (n=1299)

  - _Acción_: SKIP cuando `ballena_activa_n` > 154.0
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=3903)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.171 (n=1130)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=3490)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.308 (n=1119)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=3501)

- **FILTRO** `ibs_7min` < `0.1975` → IC=-0.259 (n=1155)

  - _Acción_: SKIP cuando `ibs_7min` < 0.1975
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3465)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.213 (n=1102)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3518)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.238 (n=1592)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=5232)

- **FILTRO** `ibs_7min` > `0.759` → IC=-0.178 (n=1704)

  - _Acción_: SKIP cuando `ibs_7min` > 0.759
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=5120)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=1566)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=3334)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.243 (n=1200)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=3700)

- **FILTRO** `ibs_7min` < `0.7437` → IC=-0.181 (n=1223)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7437
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3677)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.174 (n=1196)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3704)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.262 (n=1248)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=3755)

- **FILTRO** `ibs_7min` > `0.275` → IC=-0.174 (n=1250)

  - _Acción_: SKIP cuando `ibs_7min` > 0.275
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=3753)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.190 (n=1222)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=3781)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.261 (n=1232)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=4010)

- **FILTRO** `ibs_7min` < `0.7037` → IC=-0.231 (n=1309)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7037
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=3933)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.179 (n=1697)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.019 (n=5361)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.34` → IC=-0.276 (n=1165)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=3805)

- **FILTRO** `ibs_7min` < `0.7091` → IC=-0.232 (n=1242)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7091
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=3728)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.212 (n=1191)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3779)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.121 (n=27)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.031 (n=672)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=2719)
...
### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.221 (n=644)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0083 (IC base=+0.185)

- **PATRÓN** `drift_60min` |x|≤ `0.1627` → IC=+0.192 (n=1250)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.198 (n=1005)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` > `0.6129` → IC=+0.261 (n=1419)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6129 (IC base=+0.185)

- **PATRÓN** `dist_vwap_pct` > `0.2891` → IC=+0.185 (n=510)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.2891 (IC base=+0.185)

- **PATRÓN** `dist_vwap_pct` < `0.5845` → IC=+0.173 (n=1394)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.5845 (IC base=+0.185)

- **PATRÓN** `sigma_ewma_delta_pct` > `17.106` → IC=+0.272 (n=375)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 17.106 (IC base=+0.185)

- **PATRÓN** `libro_liquidez` > `2986.5835` → IC=+0.189 (n=946)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 2986.5835 (IC base=+0.185)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.192 (n=783)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=1639)

- **FILTRO** `libro_liquidez` < `3928.9311` → IC=-0.135 (n=1598)
...
- **PATRÓN** `ibs_15` < `0.3519` → IC=+0.283 (n=1481)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3519 (IC base=-0.039)

- **PATRÓN** `dist_vwap_pct` > `0.6579` → IC=+0.285 (n=258)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6579 (IC base=-0.039)

### UPDOWN_GBM_15M_TARDIO#BTC#15min
- **FILTRO** `sigma_h` > `0.0069` → IC=-0.220 (n=370)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0069
  - _Potencial_: sin este filtro IC_bueno=-0.192 (n=1114)

- **FILTRO** `sigma_h` < `0.0033` → IC=-0.227 (n=371)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0033
  - _Potencial_: sin este filtro IC_bueno=-0.190 (n=1113)

- **FILTRO** `hora_utc` > `7.0` → IC=-0.207 (n=941)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 7.0
...
- **FILTRO** `sigma_ewma_delta_pct` > `19.796` → IC=-0.250 (n=262)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.188 (n=1222)
...
- **FILTRO** `sigma_ewma_delta_pct` > `18.012` → IC=-0.133 (n=306)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 18.012
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=2455)
...
- **FILTRO** `sigma_h` > `0.0196` → IC=-0.265 (n=364)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `libro_spread` > `0.02` → IC=-0.260 (n=181)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.174 (n=548)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1051` → IC=+0.367 (n=178)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1051 (IC base=-0.045)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6129 sube el IC de +0.185 a +0.261 en UPDOWN_GBM#15min (n=1419). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3519 sube el IC de -0.039 a +0.283 en UPDOWN_GBM_15M_TARDIO (n=1481). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#XRP#15min**: dentro de BUY_NO, IBS < 0.35 sube el IC de -0.045 a +0.316 en UPDOWN_GBM_15M_TARDIO#XRP#15min (n=412). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 23556 | -0.091 | -3089.89€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1403 | -0.047 | -214.86€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 22153 | -0.094 | -2875.03€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#ETH | 6768 | -0.029 | -608.02€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 