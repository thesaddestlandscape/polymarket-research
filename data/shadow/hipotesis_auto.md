# Hipótesis automáticas — 2026-07-12 18:50 UTC
_Generado por shadow_postmortem.py sobre 11300 resoluciones (PNL=+2132.26€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.222 (n=185)
...
- **PATRÓN** `py_entrada` > `0.585` → IC=+0.248 (n=379)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.585 (IC base=+0.216)

- **PATRÓN** `libro_liquidez` > `2838.1924` → IC=+0.232 (n=323)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2838.1924 (IC base=+0.216)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.123 (n=1369)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5436` → IC=+0.152 (n=44)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.5436 (IC base=+0.117)
...
- **PATRÓN** `sigma_h` < `0.0316` → IC=+0.177 (n=311)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0316 (IC base=+0.164)

- **PATRÓN** `sigma_h` > `0.0139` → IC=+0.179 (n=278)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` > 0.0139 (IC base=+0.164)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.193 (n=337)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0048 (IC base=+0.177)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.185 (n=255)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 13.0 (IC base=+0.177)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.192 (n=141)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 7.0 (IC base=+0.177)

- **PATRÓN** `dist_vwap_pct` > `0.5496` → IC=+0.300 (n=38)
...
- **PATRÓN** `sigma_h` < `0.0186` → IC=+0.333 (n=70)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0186 (IC base=+0.268)
...
- **PATRÓN** `dist_vwap_pct` < `0.1821` → IC=+0.324 (n=15)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1821 (IC base=+0.268)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 996 | +0.182 | +11.55€ | 0 | 9 |
...
| ✅ FAVORITO_CONFIRMADO#60min | 165 | +0.177 | +10.16€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 348 | +0.197 | +15.36€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 55 | +0.167 | -1.33€ | 0 | 2 |
| ✅ GBM_LATE_15M | 3665 | +0.113 | +1224.24€ | 0 | 10 |
| ✅ GBM_LATE_15M#15min | 3665 | +0.113 | +1224.24€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#XRP | 883 | +0.170 | +463.90€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 883 | +0.170 | +463.90€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1016 | +0.165 | +590.11€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1016 | +0.165 | +590.11€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 193 | +0.274 | +192.79€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 193 | +0.274 | +192.79€ | 0 | 5 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.092 n=6401 | tras_1loss IC=+0.047 n=4761 | tras_2loss IC=+0.014 n=2124/40 | gap=+0.078 (umbral 0.05)
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=+0.081 n=501/40 PNL=+118.88€ | H=15h UTC: IC=+0.097 n=502/40 PNL=+126.74€ | H=17h UTC: IC=+0.130 n=444/40 PNL=+146.72€ | H=19h UTC: IC=+0.063 n=385/40 PNL=+49.78€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8842 IC=+0.098 PNL=+2208.14€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8842 IC=+0.098 PNL=+2208.14€
...
**⏳ H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: 24/40 ops en el filtro definido (IC actual=+0.192 PNL=+18.73€)
  - _Datos_: n=24 IC=+0.192 PNL=+18.73€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.167 > 0.08 con n=70 PNL=-0.79€
  - _Datos_: n=70 IC=+0.167 PNL=-0.79€
