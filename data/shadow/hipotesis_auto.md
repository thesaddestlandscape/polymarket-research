# Hipótesis automáticas — 2026-07-15 08:56 UTC
_Generado por shadow_postmortem.py sobre 15066 resoluciones (PNL=+2917.00€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.201 (n=670)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.198 (n=853)

  - _Acción_: Kelly boost +0.99€ cuando `libro_spread` < 0.01 (IC base=+0.194)

- **PATRÓN** `libro_liquidez` > `4387.7143` → IC=+0.198 (n=482)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.173 (n=950)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.209 (n=139)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.248 (n=141)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.236)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.242 (n=165)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.236)

- **PATRÓN** `py_entrada` > `0.715` → IC=+0.348 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.715 (IC base=+0.236)

- **PATRÓN** `libro_liquidez` > `5171.5093` → IC=+0.257 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 5171.5093 (IC base=+0.236)
...
### GBM_LATE_15M
- **PATRÓN** `sigma_ewma_delta_pct` > `11.419` → IC=+0.234 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.419 (IC base=+0.104)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.135 (n=1157)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0091 (IC base=+0.104)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.129 (n=1264)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 12.0 (IC base=+0.104)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `dist_vwap_pct` > `0.3404` → IC=+0.136 (n=20)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 0.3404 (IC base=+0.078)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.176` → IC=+0.214 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.176 (IC base=+0.078)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.308` → IC=+0.125 (n=46)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 5.308 (IC base=+0.062)
...
- **PATRÓN** `sigma_h` < `0.0297` → IC=+0.138 (n=412)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0297 (IC base=+0.137)

- **PATRÓN** `sigma_h` > `0.0226` → IC=+0.167 (n=187)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` > 0.0226 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.144 (n=433)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.037` → IC=+0.197 (n=31)
  ...
  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 8.037 (IC base=+0.137)
...
- **PATRÓN** `sigma_h` > `0.012` → IC=+0.226 (n=213)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.168 (n=299)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 8.0 (IC base=+0.156)

- **PATRÓN** `dist_vwap_pct` > `0.7777` → IC=+0.236 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7777 (IC base=+0.156)
...
- **PATRÓN** `sigma_h` > `0.0031` → IC=+0.169 (n=140)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0031 (IC base=+0.124)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.185 (n=109)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 12.0 (IC base=+0.124)

- **PATRÓN** `dist_vwap_pct` > `0.3426` → IC=+0.184 (n=17)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.3426 (IC base=+0.124)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.074` → IC=+0.292 (n=22)
...
- **PATRÓN** `sigma_h` < `0.0179` → IC=+0.259 (n=139)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.287 (n=125)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.3705` → IC=+0.244 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3705 (IC base=+0.247)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.099` → IC=+0.318 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.099 (IC base=+0.247)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0145` → IC=+0.158 (n=194)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0145 (IC base=+0.097)
...
- **PATRÓN** `dist_vwap_pct` > `0.6918` → IC=+0.171 (n=86)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.6918 (IC base=+0.097)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.632` → IC=+0.324 (n=66)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.632 (IC base=+0.097)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.133 (n=118)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 7.0 (IC base=+0.084)

- **PATRÓN** `dist_vwap_pct` > `0.8506` → IC=+0.147 (n=15)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.8506 (IC base=+0.084)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.678` → IC=+0.237 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.678 (IC base=+0.084)
...
- **PATRÓN** `sigma_h` < `0.0324` → IC=+0.185 (n=163)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0324 (IC base=+0.180)

- **PATRÓN** `sigma_h` > `0.0173` → IC=+0.203 (n=109)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0173 (IC base=+0.180)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.250 (n=58)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.180)

- **PATRÓN** `dist_vwap_pct` > `0.1811` → IC=+0.202 (n=45)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1811 (IC base=+0.180)

- **PATRÓN** `dist_vwap_pct` < `1.4896` → IC=+0.163 (n=87)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 1.4896 (IC base=+0.180)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.7204` → IC=+0.174 (n=256)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.87€ cuando `ibs_15` > 0.7204 (IC base=+0.025)

- **PATRÓN** `dist_vwap_pct` > `0.7415` → IC=+0.197 (n=31)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.7415 (IC base=+0.025)

- **PATRÓN** `dist_vwap_pct` < `0.336` → IC=+0.149 (n=132)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.336 (IC base=+0.025)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2222` → IC=+0.123 (n=160)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 0.2222 (IC base=+0.072)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.158 (n=153)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 11.0 (IC base=+0.072)

- **PATRÓN** `ibs_15` < `0.9252` → IC=+0.135 (n=143)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.67€ cuando `ibs_15` < 0.9252 (IC base=+0.072)

- **PATRÓN** `ibs_15` > `0.6323` → IC=+0.190 (n=143)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.95€ cuando `ibs_15` > 0.6323 (IC base=+0.072)

- **PATRÓN** `dist_vwap_pct` > `0.7608` → IC=+0.324 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7608 (IC base=+0.072)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.527` → IC=+0.177 (n=29)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` < 5.527 (IC base=+0.072)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS > 0.7204 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.174 n=256). Confirma señal de reversión media → alinear con BUY_NO.
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS > 0.6323 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.190 n=143). Confirma señal de reversión media → alinear con BUY_NO.
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2071 | +0.178 | -22.89€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1340 | +0.209 | +8.16€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 604 | +0.181 | -8.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 448 | +0.198 | -20.72€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 734 | +0.181 | +16.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 442 | +0.223 | +26.51€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 727 | +0.175 | -26.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 444 | +0.208 | +6.54€ | 0 | 6 |
...
| ✅ GBM_LATE_15M | 4522 | +0.101 | +1367.89€ | 0 | 3 |
| ✅ GBM_LATE_15M#15min | 4522 | +0.101 | +1367.89€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1084 | +0.066 | +157.54€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1084 | +0.066 | +157.54€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1105 | +0.072 | +178.08€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1105 | +0.072 | +178.08€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1203 | +0.111 | +490.06€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1203 | +0.111 | +490.06€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1130 | +0.151 | +542.20€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1130 | +0.151 | +542.20€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1786 | +0.149 | +1021.74€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1786 | +0.149 | +1021.74€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 456 | +0.122 | +225.67€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 456 | +0.122 | +225.67€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 471 | +0.119 | +204.01€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 471 | +0.119 | +204.01€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 484 | +0.107 | +205.72€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 484 | +0.107 | +205.72€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 375 | +0.272 | +386.35€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 375 | +0.272 | +386.35€ | 0 | 11 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 26 | -0.143 | -2.06€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 26 | -0.143 | -2.06€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO | 1740 | +0.082 | +473.64€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1740 | +0.082 | +473.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 430 | +0.062 | +72.30€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 430 | +0.062 | +72.30€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 387 | +0.006 | +6.23€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 387 | +0.006 | +6.23€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 434 | +0.048 | +71.86€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 434 | +0.048 | +71.86€ | 2 | 5 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 489 | +0.188 | +323.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 489 | +0.188 | +323.25€ | 0 | 10 |
...
| ✅ UPDOWN_GBM | 1542 | +0.009 | +71.54€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1285 | +0.036 | +108.06€ | 0 | 7 |
...
| ✅ UPDOWN_GBM#BTC | 397 | +0.026 | +27.29€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 317 | +0.064 | +39.73€ | 1 | 10 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.123) — sin ventaja clara. oversold(IBS<0.3): IC=-0.008 n=427 | neutral: IC=-0.022 n=322 | overbought(IBS>0.7): IC=+0.102 n=342
  - _Datos_: n=1153 IC=+0.026 PNL=+97.15€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.098 n=8667 | tras_1loss IC=+0.060 n=6210 | tras_2loss IC=+0.028 n=2701/40 | gap=+0.070 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1415 IC=+0.016 PNL=+85.53€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1415 IC=+0.016 PNL=+85.53€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=687 IC=+0.025 PNL=+43.61€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=687 IC=+0.025 PNL=+43.61€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=317 IC=+0.064 PNL=+39.73€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=317 IC=+0.064 PNL=+39.73€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=675 IC=+0.073 PNL=+86.46€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=675 IC=+0.073 PNL=+86.46€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=12608 IC=+0.100 PNL=+2992.88€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=12608 IC=+0.100 PNL=+2992.88€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=835 IC=-0.005 PNL=+14.07€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=835 IC=-0.005 PNL=+14.07€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.166 > 0.1 con n=384 PNL=+136.19€
  - _Datos_: n=384 IC=+0.166 PNL=+136.19€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.149 > 0.08 con n=283 PNL=+89.85€
  - _Datos_: n=283 IC=+0.149 PNL=+89.85€
...
**🟡 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.212 > 0.08 con n=123 PNL=+56.18€
  - _Datos_: n=123 IC=+0.212 PNL=+56.18€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.257 > 0.2 con n=286 PNL=+194.64€
  - _Datos_: n=286 IC=+0.257 PNL=+194.64€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.115 > 0.08 con n=120 PNL=+32.93€
  - _Datos_: n=120 IC=+0.115 PNL=+32.93€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.127 > 0.08 con n=148 PNL=+71.70€
  - _Datos_: n=148 IC=+0.127 PNL=+71.70€
