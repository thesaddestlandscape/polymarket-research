# Hipótesis automáticas — 2026-09-26 07:17 UTC
_Generado por shadow_postmortem.py sobre 616963 resoluciones (PNL=+70112.33€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.26` → IC=-0.226 (n=7246)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.26
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=21749)
...
- **FILTRO** `restante_s_al_confirmar` < `166.14` → IC=-0.210 (n=1763)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 166.14
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=5294)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `127.5` → IC=-0.343 (n=1425)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 127.5
  - _Potencial_: sin este filtro IC_bueno=-0.110 (n=4277)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.137 (n=14428)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` < 7.0 (IC base=+0.127)

- **PATRÓN** `py_entrada` < `0.35` → IC=+0.231 (n=11223)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.35 (IC base=+0.127)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.174 (n=5785)

  - _Acción_: Kelly boost +0.87€ cuando `libro_spread` < 0.01 (IC base=+0.127)

- **PATRÓN** `libro_liquidez` > `7868.8792` → IC=+0.175 (n=2187)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 7868.8792 (IC base=+0.127)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.210 (n=1772)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.204)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.205 (n=1733)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.205 (n=2188)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.146 (n=365)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.123 (n=561)

  - _Acción_: Kelly boost +0.62€ cuando `libro_spread` < 0.01 (IC base=+0.106)

- **PATRÓN** `libro_liquidez` > `5854.7033` → IC=+0.165 (n=222)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 5854.7033 (IC base=+0.106)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.235 (n=1521)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.166 (n=600)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.437 (n=189)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.268 (n=2177)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3502` → IC=+0.166 (n=1880)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_spike_ratio` > `1.4361` → IC=+0.164 (n=1760)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.4361 (IC base=+0.161)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.176 (n=1914)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1255` → IC=+0.269 (n=630)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.0526` → IC=+0.290 (n=630)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.468` → IC=+0.245 (n=1488)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6147` → IC=+0.245 (n=437)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6147 (IC base=+0.235)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.237 (n=1582)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.235)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `sigma_h` < `0.0031` → IC=+0.229 (n=637)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0031 (IC base=+0.216)

- **PATRÓN** `drift_60min` |x|≤ `0.1138` → IC=+0.237 (n=637)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1138 (IC base=+0.216)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.231 (n=1512)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.216)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.215 (n=1467)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` > `0.2018` → IC=+0.218 (n=758)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2018 (IC base=+0.216)

- **PATRÓN** `dist_vwap_pct` < `0.5663` → IC=+0.220 (n=1515)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5663 (IC base=+0.216)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.762` → IC=+0.259 (n=247)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.762 (IC base=+0.216)

- **PATRÓN** `volumen_regimen` < `1.2501` → IC=+0.218 (n=1444)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2501 (IC base=+0.216)

- **PATRÓN** `volumen_regimen` > `1.0767` → IC=+0.226 (n=655)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3804` → IC=+0.232 (n=472)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3804 (IC base=+0.216)

- **PATRÓN** `libro_liquidez` > `11170.0615` → IC=+0.224 (n=1444)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11170.0615 (IC base=+0.216)

- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.165 (n=1010)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0039 (IC base=+0.140)

- **PATRÓN** `drift_60min` |x|≤ `0.0766` → IC=+0.169 (n=503)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.0766 (IC base=+0.140)
...
- **PATRÓN** `ibs_20min` < `0.7007` → IC=+0.171 (n=1507)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` < 0.7007 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` < `0.132` → IC=+0.156 (n=1353)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.132 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.262` → IC=+0.158 (n=241)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 11.262 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.301` → IC=+0.145 (n=1374)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1552` → IC=+0.179 (n=400)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1552 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` < `2.4301` → IC=+0.154 (n=1396)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.4301 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.4188` → IC=+0.148 (n=1396)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.4188 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `14101.4554` → IC=+0.146 (n=1004)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 14101.4554 (IC base=+0.140)

- **PATRÓN** `ballena_activa_n` < `234.0` → IC=+0.169 (n=584)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.190 (n=1950)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 5.0 (IC base=+0.184)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.188 (n=1667)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.187` → IC=+0.255 (n=390)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.187 (IC base=+0.184)

- **PATRÓN** `volumen_pendiente_norm` < `0.2091` → IC=+0.186 (n=1866)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8037` → IC=+0.202 (n=804)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8037 (IC base=+0.184)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.191 (n=1308)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.02 (IC base=+0.184)

- **PATRÓN** `sigma_h` < `0.0104` → IC=+0.221 (n=1413)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0104 (IC base=+0.210)

- **PATRÓN** `drift_60min` |x|≤ `0.6123` → IC=+0.211 (n=1606)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.6123 (IC base=+0.210)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.717` → IC=+0.233 (n=611)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.717 (IC base=+0.210)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.512` → IC=+0.210 (n=1744)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.144 (n=102)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=2279)

- **PATRÓN** `ibs_20min` > `0.9456` → IC=+0.213 (n=368)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9456 (IC base=+0.026)

- **PATRÓN** `dist_vwap_pct` > `0.1502` → IC=+0.323 (n=218)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1502 (IC base=+0.026)

- **PATRÓN** `dist_vwap_pct` < `0.5539` → IC=+0.331 (n=346)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5539 (IC base=+0.026)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.741` → IC=+0.161 (n=736)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 4.741 (IC base=+0.026)

- **PATRÓN** `volumen_regimen` < `0.8553` → IC=+0.330 (n=228)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8553 (IC base=+0.026)

- **PATRÓN** `volumen_regimen` > `1.2014` → IC=+0.336 (n=114)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2012` → IC=+0.329 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2012 (IC base=+0.026)

- **PATRÓN** `ballena_activa_n` < `158.0` → IC=+0.331 (n=330)
...
- **PATRÓN** `volumen_regimen` < `0.8471` → IC=+0.164 (n=581)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5223` → IC=+0.177 (n=731)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 1.5223 (IC base=+0.019)
...
- **FILTRO** `ibs_20min` > `0.25` → IC=-0.124 (n=2255)

  - _Acción_: SKIP cuando `ibs_20min` > 0.25
  - _Potencial_: sin este filtro IC_bueno=+0.124 (n=1119)

- **FILTRO** `sigma_ewma_delta_pct` > `8.696` → IC=-0.211 (n=362)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.696
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=3012)
...
- **PATRÓN** `ibs_20min` < `0.25` → IC=+0.124 (n=1119)
...
- **PATRÓN** `volumen_regimen` < `0.7129` → IC=+0.253 (n=164)
...
- **FILTRO** `drift_60min` |x|> `0.6568` → IC=-0.185 (n=589)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6568
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=1769)

- **FILTRO** `ibs_20min` < `0.7197` → IC=-0.153 (n=1556)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7197
  - _Potencial_: sin este filtro IC_bueno=+0.102 (n=802)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.201 (n=443)
...
- **FILTRO** `ibs_20min` > `0.77` → IC=-0.203 (n=859)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.038 (n=2601)
...
- **PATRÓN** `dist_vwap_pct` < `0.2522` → IC=+0.242 (n=2942)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2522 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` < `0.7077` → IC=+0.242 (n=1365)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7077 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` > `1.2033` → IC=+0.251 (n=1034)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2033 (IC base=+0.074)

- **PATRÓN** `volumen_pendiente_norm` > `0.2431` → IC=+0.305 (n=778)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2431 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` < `1.5983` → IC=+0.260 (n=1810)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5983 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` > `2.2963` → IC=+0.260 (n=1865)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2963 (IC base=+0.074)

- **PATRÓN** `ballena_activa_n` < `84.0` → IC=+0.267 (n=3994)
...
- **FILTRO** `ibs_20min` < `0.2583` → IC=-0.146 (n=726)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=2178)

- **FILTRO** `sigma_ewma_delta_pct` > `4.536` → IC=-0.157 (n=552)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.536
  - _Potencial_: sin este filtro IC_bueno=+0.025 (n=1852)

- **PATRÓN** `ibs_20min` > `0.8938` → IC=+0.262 (n=726)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8938 (IC base=+0.042)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.861` → IC=+0.208 (n=381)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4309` → IC=+0.171 (n=308)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` < 1.4309 (IC base=+0.042)
...
- **PATRÓN** `ballena_activa_n` < `14.0` → IC=+0.180 (n=423)
...
- **PATRÓN** `ibs_20min` > `0.8636` → IC=+0.156 (n=705)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` > 0.8636 (IC base=+0.025)

- **PATRÓN** `dist_vwap_pct` > `0.3098` → IC=+0.175 (n=383)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.3098 (IC base=+0.025)

- **PATRÓN** `volumen_regimen` > `0.6744` → IC=+0.168 (n=871)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 0.6744 (IC base=+0.025)

- **PATRÓN** `volumen_pendiente_norm` > `0.2712` → IC=+0.238 (n=124)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3925` → IC=+0.156 (n=318)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 2.3925 (IC base=+0.025)

- **PATRÓN** `ballena_activa_n` < `242.0` → IC=+0.188 (n=418)

  - _Acción_: Kelly boost +0.94€ cuando `ballena_activa_n` < 242.0 (IC base=+0.025)

- **PATRÓN** `dist_vwap_pct` < `0.16` → IC=+0.223 (n=618)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.16 (IC base=+0.006)

- **PATRÓN** `volumen_regimen` > `0.611` → IC=+0.220 (n=601)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1582` → IC=+0.240 (n=252)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1582 (IC base=+0.006)

- **PATRÓN** `ballena_activa_n` < `460.0` → IC=+0.221 (n=554)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.249 (n=1733)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.246)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.247 (n=1662)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.706` → IC=+0.283 (n=518)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.706 (IC base=+0.246)

- **PATRÓN** `volumen_pendiente_norm` < `0.1035` → IC=+0.259 (n=1400)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1035 (IC base=+0.246)

- **PATRÓN** `volumen_spike_ratio` > `3.3887` → IC=+0.263 (n=521)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.3887 (IC base=+0.246)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.252 (n=1154)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.246)

- **PATRÓN** `libro_liquidez` > `1976.32` → IC=+0.250 (n=550)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1976.32 (IC base=+0.246)

- **PATRÓN** `sigma_h` > `0.01` → IC=+0.312 (n=603)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.01 (IC base=+0.281)

- **PATRÓN** `drift_60min` |x|≤ `0.1749` → IC=+0.289 (n=586)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.808` → IC=+0.294 (n=526)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.808 (IC base=+0.281)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.747` → IC=+0.281 (n=1425)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.747 (IC base=+0.281)

- **PATRÓN** `volumen_pendiente_norm` > `0.3393` → IC=+0.303 (n=206)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3393 (IC base=+0.281)

- **PATRÓN** `volumen_spike_ratio` < `1.7379` → IC=+0.285 (n=543)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7379 (IC base=+0.281)

- **PATRÓN** `volumen_spike_ratio` > `2.7088` → IC=+0.290 (n=559)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7088 (IC base=+0.281)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.286 (n=880)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.281)

- **PATRÓN** `libro_liquidez` > `1899.5579` → IC=+0.293 (n=603)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1899.5579 (IC base=+0.281)

- **PATRÓN** `ballena_activa_n` < `28.0` → IC=+0.284 (n=814)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 28.0 (IC base=+0.281)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **FILTRO** `ibs_20min` < `0.2958` → IC=-0.179 (n=518)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2958
  - _Potencial_: sin este filtro IC_bueno=+0.077 (n=1555)

- **FILTRO** `ibs_20min` > `0.7739` → IC=-0.184 (n=619)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7739
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=1861)

- **PATRÓN** `ibs_20min` > `0.9107` → IC=+0.179 (n=519)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` > 0.9107 (IC base=+0.013)

- **PATRÓN** `dist_vwap_pct` > `0.4378` → IC=+0.220 (n=266)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4378 (IC base=+0.013)

- **PATRÓN** `dist_vwap_pct` < `0.2007` → IC=+0.218 (n=431)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2007 (IC base=+0.013)

- **PATRÓN** `volumen_regimen` < `0.9898` → IC=+0.238 (n=528)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9898 (IC base=+0.013)

- **PATRÓN** `volumen_regimen` > `0.5902` → IC=+0.216 (n=601)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.5902 (IC base=+0.013)

- **PATRÓN** `volumen_pendiente_norm` > `0.0773` → IC=+0.260 (n=219)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0773 (IC base=+0.013)

- **PATRÓN** `volumen_spike_ratio` < `2.0971` → IC=+0.245 (n=500)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0971 (IC base=+0.013)

- **PATRÓN** `ballena_activa_n` < `148.0` → IC=+0.246 (n=577)
...
- **PATRÓN** `dist_vwap_pct` < `0.35` → IC=+0.198 (n=452)
  ...
  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.35 (IC base=-0.004)
...
- **FILTRO** `ibs_20min` < `0.7308` → IC=-0.194 (n=1113)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7308
  - _Potencial_: sin este filtro IC_bueno=+0.276 (n=1116)

- **FILTRO** `ibs_20min` > `0.6857` → IC=-0.234 (n=573)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6857
  - _Potencial_: sin este filtro IC_bueno=+0.098 (n=1720)

- **FILTRO** `sigma_ewma_delta_pct` > `4.709` → IC=-0.188 (n=501)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.709
  - _Potencial_: sin este filtro IC_bueno=+0.071 (n=1792)

- **PATRÓN** `ibs_20min` > `0.7308` → IC=+0.276 (n=1116)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7308 (IC base=+0.042)

- **PATRÓN** `dist_vwap_pct` > `0.8634` → IC=+0.328 (n=265)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8634 (IC base=+0.042)
...
- **PATRÓN** `volumen_regimen` > `0.7291` → IC=+0.293 (n=738)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7291 (IC base=+0.042)

- **PATRÓN** `volumen_pendiente_norm` < `0.1013` → IC=+0.293 (n=770)
...
- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.314 (n=702)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 56.0 (IC base=+0.042)

- **PATRÓN** `ibs_20min` < `0.5833` → IC=+0.121 (n=1519)

  - _Acción_: Kelly boost +0.60€ cuando `ibs_20min` < 0.5833 (IC base=+0.015)

- **PATRÓN** `dist_vwap_pct` > `0.7475` → IC=+0.205 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7475 (IC base=+0.015)

- **PATRÓN** `dist_vwap_pct` < `0.2213` → IC=+0.224 (n=512)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2213 (IC base=+0.015)

- **PATRÓN** `volumen_regimen` < `0.7092` → IC=+0.253 (n=261)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7092 (IC base=+0.015)

- **PATRÓN** `volumen_pendiente_norm` < `0.0971` → IC=+0.215 (n=549)
...
- **PATRÓN** `volumen_spike_ratio` < `2.456` → IC=+0.224 (n=553)
...
- **PATRÓN** `ballena_activa_n` < `58.0` → IC=+0.236 (n=563)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.293 (n=642)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.278)

- **PATRÓN** `ibs_20min` > `0.7398` → IC=+0.322 (n=1213)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.609` → IC=+0.302 (n=711)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.609 (IC base=+0.278)

- **PATRÓN** `volumen_regimen` > `0.8606` → IC=+0.303 (n=906)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8606 (IC base=+0.278)

- **PATRÓN** `volumen_pendiente_norm` < `0.0782` → IC=+0.281 (n=1156)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.283 (n=1448)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.278)

- **PATRÓN** `libro_liquidez` > `2636.1235` → IC=+0.293 (n=905)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2636.1235 (IC base=+0.278)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2374` → IC=+0.340 (n=242)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2374 (IC base=+0.275)

- **PATRÓN** `volumen_spike_ratio` < `1.4234` → IC=+0.281 (n=431)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4234 (IC base=+0.275)

- **PATRÓN** `volumen_spike_ratio` > `2.1415` → IC=+0.276 (n=586)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1415 (IC base=+0.275)

- **PATRÓN** `libro_liquidez` > `2627.1278` → IC=+0.277 (n=973)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2627.1278 (IC base=+0.275)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.171 (n=2724)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0049 (IC base=+0.167)

- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.200 (n=2713)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0113 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0902` → IC=+0.184 (n=2715)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.0902 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.179 (n=8466)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 5.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` > `0.5738` → IC=+0.217 (n=8136)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5738 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.1772` → IC=+0.194 (n=3539)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.1772 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.35` → IC=+0.254 (n=1659)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.35 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.209` → IC=+0.159 (n=5389)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.209 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` > `0.6307` → IC=+0.160 (n=5390)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.6307 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.2432` → IC=+0.194 (n=1639)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2432 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.5584` → IC=+0.166 (n=3437)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.5584 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.6119` → IC=+0.175 (n=2604)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.6119 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `2409.4285` → IC=+0.168 (n=5424)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2409.4285 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `113.0` → IC=+0.180 (n=7049)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 113.0 (IC base=+0.167)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.184 (n=5196)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0066 (IC base=+0.168)

- **PATRÓN** `drift_60min` |x|≤ `0.0812` → IC=+0.209 (n=2597)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_regimen` < `1.1714` → IC=+0.152 (n=5629)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1714 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.2904` → IC=+0.218 (n=1123)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2904 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` < `1.5586` → IC=+0.165 (n=3126)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.5586 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.2465` → IC=+0.171 (n=3220)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 2.2465 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `114.0` → IC=+0.175 (n=6749)

  - _Acción_: Kelly boost +0.87€ cuando `ballena_activa_n` < 114.0 (IC base=+0.168)

### GBM_LATE_15M_MULTIHORIZONTE#BNB#15min
- **PATRÓN** `sigma_h` < `0.0052` → IC=+0.211 (n=465)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.1878` → IC=+0.289 (n=681)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.3451` → IC=+0.263 (n=1021)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3451 (IC base=+0.239)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.344` → IC=+0.250 (n=1105)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.344 (IC base=+0.239)

- **PATRÓN** `volumen_pendiente_norm` < `0.1599` → IC=+0.234 (n=966)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.239 (n=1129)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.239)

- **PATRÓN** `libro_liquidez` > `1814.8377` → IC=+0.242 (n=680)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1814.8377 (IC base=+0.239)

### GBM_LATE_15M_MULTIHORIZONTE#BTC#15min
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.231 (n=403)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0028 (IC base=+0.160)

- **PATRÓN** `drift_60min` |x|≤ `0.0732` → IC=+0.191 (n=403)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.96€ cuando `drift_60min` |x|≤ 0.0732 (IC base=+0.160)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.184 (n=1275)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 5.0 (IC base=+0.160)

- **PATRÓN** `ibs_20min` > `0.4047` → IC=+0.225 (n=1209)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4047 (IC base=+0.160)

- **PATRÓN** `dist_vwap_pct` > `0.2117` → IC=+0.214 (n=708)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2117 (IC base=+0.160)
...
- **PATRÓN** `volumen_regimen` < `0.6876` → IC=+0.165 (n=532)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.6876 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` > `1.0702` → IC=+0.169 (n=548)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 1.0702 (IC base=+0.160)
...
- **PATRÓN** `volumen_spike_ratio` < `1.503` → IC=+0.176 (n=517)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 1.503 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` > `2.4558` → IC=+0.160 (n=392)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 2.4558 (IC base=+0.160)

- **PATRÓN** `libro_liquidez` > `10644.0099` → IC=+0.168 (n=1209)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 10644.0099 (IC base=+0.160)
...
- **PATRÓN** `drift_60min` |x|≤ `0.236` → IC=+0.217 (n=913)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.236 (IC base=+0.198)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.205 (n=1414)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.435` → IC=+0.273 (n=315)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.435 (IC base=+0.198)

- **PATRÓN** `volumen_pendiente_norm` > `0.2063` → IC=+0.200 (n=405)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2063 (IC base=+0.198)
...
- **PATRÓN** `sigma_h` < `0.0102` → IC=+0.235 (n=1005)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0102 (IC base=+0.217)

- **PATRÓN** `drift_60min` |x|≤ `0.2486` → IC=+0.238 (n=761)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.3506` → IC=+0.248 (n=1141)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3506 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.729` → IC=+0.258 (n=489)
...
- **PATRÓN** `libro_liquidez` > `1904.0224` → IC=+0.217 (n=517)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.174 (n=1147)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0066 (IC base=+0.142)

- **PATRÓN** `drift_60min` |x|≤ `0.4315` → IC=+0.158 (n=1302)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.4315 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=1363)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.142)

- **PATRÓN** `ibs_20min` > `0.3713` → IC=+0.196 (n=1303)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` > 0.3713 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.1551` → IC=+0.177 (n=859)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.1551 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.995` → IC=+0.226 (n=239)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.995 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `1.0376` → IC=+0.145 (n=1146)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 1.0376 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` > `0.6251` → IC=+0.146 (n=1302)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.138 (n=1506)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 5.0 (IC base=+0.117)

- **PATRÓN** `ibs_20min` > `0.5143` → IC=+0.204 (n=1473)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5143 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` > `0.5236` → IC=+0.197 (n=638)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.5236 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.76` → IC=+0.258 (n=332)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.76 (IC base=+0.117)

- **PATRÓN** `volumen_regimen` < `1.2151` → IC=+0.129 (n=1474)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 1.2151 (IC base=+0.117)

- **PATRÓN** `volumen_pendiente_norm` > `0.0711` → IC=+0.123 (n=621)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_pendiente_norm` > 0.0711 (IC base=+0.117)

- **PATRÓN** `volumen_spike_ratio` < `1.5413` → IC=+0.139 (n=627)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 1.5413 (IC base=+0.117)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.125 (n=1539)

  - _Acción_: Kelly boost +0.62€ cuando `libro_spread` < 0.02 (IC base=+0.117)

- **PATRÓN** `libro_liquidez` > `2898.8632` → IC=+0.193 (n=668)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 2898.8632 (IC base=+0.117)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.133 (n=1122)

  - _Acción_: Kelly boost +0.66€ cuando `ballena_activa_n` < 49.0 (IC base=+0.117)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.161 (n=658)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0062 (IC base=+0.116)

- **PATRÓN** `drift_60min` |x|≤ `0.1048` → IC=+0.165 (n=499)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.82€ cuando `drift_60min` |x|≤ 0.1048 (IC base=+0.116)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.131 (n=1512)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.116)

- **PATRÓN** `ibs_20min` < `0.5714` → IC=+0.212 (n=1496)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5714 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` > `1.0221` → IC=+0.138 (n=216)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 1.0221 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` < `0.2064` → IC=+0.141 (n=1370)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.2064 (IC base=+0.116)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.551` → IC=+0.133 (n=309)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` > 7.551 (IC base=+0.116)

- **PATRÓN** `volumen_regimen` < `0.6379` → IC=+0.147 (n=499)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.6379 (IC base=+0.116)

- **PATRÓN** `volumen_pendiente_norm` > `0.2724` → IC=+0.163 (n=185)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.2724 (IC base=+0.116)

- **PATRÓN** `volumen_spike_ratio` < `1.4523` → IC=+0.132 (n=449)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` < 1.4523 (IC base=+0.116)

- **PATRÓN** `volumen_spike_ratio` > `2.4117` → IC=+0.131 (n=448)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 2.4117 (IC base=+0.116)

- **PATRÓN** `libro_liquidez` > `2761.3271` → IC=+0.160 (n=678)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 2761.3271 (IC base=+0.116)
...
- **PATRÓN** `sigma_h` < `0.0117` → IC=+0.227 (n=640)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.0915` → IC=+0.223 (n=485)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0915 (IC base=+0.204)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.222 (n=713)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.204)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.215 (n=671)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.204)

- **PATRÓN** `ibs_20min` < `0.438` → IC=+0.245 (n=1455)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.438 (IC base=+0.204)

- **PATRÓN** `dist_vwap_pct` > `1.2369` → IC=+0.221 (n=177)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2369 (IC base=+0.204)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.425` → IC=+0.238 (n=281)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.425 (IC base=+0.204)

- **PATRÓN** `volumen_regimen` > `0.6303` → IC=+0.215 (n=1455)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6303 (IC base=+0.204)

- **PATRÓN** `volumen_pendiente_norm` > `0.2814` → IC=+0.294 (n=192)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2814 (IC base=+0.204)

- **PATRÓN** `volumen_spike_ratio` < `2.1914` → IC=+0.195 (n=1154)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 2.1914 (IC base=+0.204)

- **PATRÓN** `volumen_spike_ratio` > `1.434` → IC=+0.200 (n=1311)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.434 (IC base=+0.204)

- **PATRÓN** `libro_liquidez` > `2602.0936` → IC=+0.208 (n=970)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2602.0936 (IC base=+0.204)
...
- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.176 (n=661)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0089 (IC base=+0.147)

- **PATRÓN** `drift_60min` |x|≤ `0.1371` → IC=+0.154 (n=872)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.1371 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.186 (n=992)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 15.0 (IC base=+0.147)

- **PATRÓN** `ibs_20min` > `0.75` → IC=+0.207 (n=1321)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.75 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.8726` → IC=+0.178 (n=327)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.8726 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.713` → IC=+0.175 (n=891)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 3.713 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `0.871` → IC=+0.165 (n=1165)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.871 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` > `0.623` → IC=+0.151 (n=1748)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4385` → IC=+0.163 (n=638)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 1.4385 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` > `1.8234` → IC=+0.155 (n=1275)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.8234 (IC base=+0.147)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.153 (n=2247)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.02 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `12414.6484` → IC=+0.162 (n=661)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 12414.6484 (IC base=+0.147)

- **PATRÓN** `ballena_activa_n` < `152.0` → IC=+0.167 (n=1760)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 152.0 (IC base=+0.147)

- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.138 (n=1368)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0056 (IC base=+0.111)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.124 (n=2061)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 5.0 (IC base=+0.111)

- **PATRÓN** `ibs_20min` < `0.06` → IC=+0.191 (n=685)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` < 0.06 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` < `0.2106` → IC=+0.121 (n=1829)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1656` → IC=+0.131 (n=510)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_pendiente_norm` > 0.1656 (IC base=+0.111)

- **PATRÓN** `volumen_spike_ratio` < `1.4517` → IC=+0.143 (n=659)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.4517 (IC base=+0.111)

- **PATRÓN** `libro_liquidez` > `2764.7951` → IC=+0.121 (n=1831)

  - _Acción_: Kelly boost +0.61€ cuando `libro_liquidez` > 2764.7951 (IC base=+0.111)

- **PATRÓN** `ballena_activa_n` < `29.0` → IC=+0.127 (n=843)
...
- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.256 (n=273)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.210 (n=643)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.195)

- **PATRÓN** `ibs_20min` > `0.6957` → IC=+0.242 (n=413)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6957 (IC base=+0.195)

- **PATRÓN** `dist_vwap_pct` > `0.154` → IC=+0.207 (n=319)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.154 (IC base=+0.195)

- **PATRÓN** `dist_vwap_pct` < `0.6267` → IC=+0.196 (n=683)
...
- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.167 (n=211)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.412` → IC=+0.192 (n=251)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 3.412 (IC base=+0.128)

- **PATRÓN** `volumen_regimen` < `1.069` → IC=+0.145 (n=409)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.069 (IC base=+0.128)
...
- **PATRÓN** `ibs_20min` < `0.5172` → IC=+0.151 (n=414)

  - _Acción_: Kelly boost +0.76€ cuando `ibs_20min` < 0.5172 (IC base=+0.084)

- **PATRÓN** `volumen_regimen` < `0.7082` → IC=+0.147 (n=182)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.7082 (IC base=+0.084)

- **PATRÓN** `volumen_spike_ratio` < `1.8403` → IC=+0.155 (n=259)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 1.8403 (IC base=+0.084)

- **PATRÓN** `libro_liquidez` > `2575.5064` → IC=+0.133 (n=276)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 2575.5064 (IC base=+0.084)

- **PATRÓN** `ballena_activa_n` < `42.0` → IC=+0.136 (n=361)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0114` → IC=+0.205 (n=3487)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0114 (IC base=+0.169)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.180 (n=10875)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 5.0 (IC base=+0.169)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.307 (n=3509)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` > `0.9694` → IC=+0.202 (n=1489)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9694 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.342` → IC=+0.243 (n=2624)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.342 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` < `0.88` → IC=+0.165 (n=4659)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.88 (IC base=+0.169)

- **PATRÓN** `volumen_pendiente_norm` > `0.2884` → IC=+0.202 (n=1439)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2884 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `2.5891` → IC=+0.187 (n=3350)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 2.5891 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `1794.2984` → IC=+0.171 (n=10443)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 1794.2984 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `85.0` → IC=+0.194 (n=7990)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 85.0 (IC base=+0.169)

- **PATRÓN** `sigma_h` < `0.007` → IC=+0.192 (n=6322)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.007 (IC base=+0.181)

- **PATRÓN** `drift_60min` |x|≤ `0.149` → IC=+0.190 (n=4165)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.184 (n=4417)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 7.0 (IC base=+0.181)

- **PATRÓN** `ibs_20min` < `0.5687` → IC=+0.236 (n=9466)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5687 (IC base=+0.181)

- **PATRÓN** `dist_vwap_pct` < `0.2512` → IC=+0.162 (n=5874)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2512 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.025` → IC=+0.201 (n=1325)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.025 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.731` → IC=+0.183 (n=9152)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.731 (IC base=+0.181)

- **PATRÓN** `volumen_regimen` < `0.7033` → IC=+0.163 (n=2855)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.7033 (IC base=+0.181)

- **PATRÓN** `volumen_regimen` > `1.2013` → IC=+0.155 (n=2163)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.2013 (IC base=+0.181)

- **PATRÓN** `volumen_pendiente_norm` > `0.2875` → IC=+0.241 (n=1247)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2875 (IC base=+0.181)

- **PATRÓN** `volumen_spike_ratio` > `2.614` → IC=+0.193 (n=2904)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 2.614 (IC base=+0.181)

- **PATRÓN** `ballena_activa_n` < `47.0` → IC=+0.195 (n=5578)
...
- **PATRÓN** `drift_60min` |x|≤ `0.356` → IC=+0.191 (n=1762)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.57` → IC=+0.344 (n=401)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.57 (IC base=+0.190)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5574` → IC=+0.197 (n=555)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 2.5574 (IC base=+0.190)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.209 (n=1780)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1274` → IC=+0.282 (n=613)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1274 (IC base=+0.257)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.266 (n=1257)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.257)

- **PATRÓN** `ibs_20min` < `0.3517` → IC=+0.285 (n=1227)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3517 (IC base=+0.257)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.489` → IC=+0.261 (n=1460)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.489 (IC base=+0.257)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5484` → IC=+0.255 (n=565)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.259 (n=1540)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.257)

- **PATRÓN** `libro_liquidez` > `1812.0416` → IC=+0.259 (n=929)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1812.0416 (IC base=+0.257)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0846` → IC=+0.159 (n=558)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.0846 (IC base=+0.150)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.164 (n=1746)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 5.0 (IC base=+0.150)

- **PATRÓN** `ibs_20min` > `0.3081` → IC=+0.202 (n=1673)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3081 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.3442` → IC=+0.193 (n=668)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.3442 (IC base=+0.150)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.227` → IC=+0.152 (n=1513)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` < 4.227 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` < `0.6267` → IC=+0.177 (n=558)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 0.6267 (IC base=+0.150)

- **PATRÓN** `volumen_pendiente_norm` > `0.2657` → IC=+0.203 (n=240)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2657 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` < `2.1137` → IC=+0.159 (n=1424)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.1137 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.7535` → IC=+0.158 (n=1079)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.7535 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `11341.8291` → IC=+0.160 (n=1495)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 11341.8291 (IC base=+0.150)

- **PATRÓN** `ballena_activa_n` < `471.0` → IC=+0.158 (n=1552)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.226 (n=1772)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.217)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.221 (n=1720)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.809` → IC=+0.293 (n=495)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.809 (IC base=+0.217)

- **PATRÓN** `volumen_pendiente_norm` < `0.2104` → IC=+0.220 (n=1687)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2104 (IC base=+0.217)

- **PATRÓN** `volumen_spike_ratio` > `1.784` → IC=+0.226 (n=1448)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.784 (IC base=+0.217)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.226 (n=1191)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.217)

- **PATRÓN** `sigma_h` < `0.0118` → IC=+0.237 (n=1585)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0118 (IC base=+0.230)

- **PATRÓN** `drift_60min` |x|≤ `0.1707` → IC=+0.236 (n=698)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_spike_ratio` < `1.7518` → IC=+0.229 (n=643)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7518 (IC base=+0.230)

- **PATRÓN** `volumen_spike_ratio` > `2.1787` → IC=+0.233 (n=973)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1787 (IC base=+0.230)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.239 (n=1057)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.230)

- **PATRÓN** `libro_liquidez` > `1904.2584` → IC=+0.235 (n=719)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1904.2584 (IC base=+0.230)

- **PATRÓN** `ballena_activa_n` < `53.0` → IC=+0.227 (n=1380)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 53.0 (IC base=+0.230)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0035` → IC=+0.186 (n=593)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.0035 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.4364` → IC=+0.145 (n=1779)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.4364 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.151 (n=1855)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 5.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` > `0.8748` → IC=+0.261 (n=807)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8748 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.3648` → IC=+0.163 (n=702)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.3648 (IC base=+0.136)
...
- **PATRÓN** `volumen_regimen` < `0.873` → IC=+0.155 (n=1186)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.873 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.2332` → IC=+0.213 (n=329)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2332 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` < `1.5199` → IC=+0.152 (n=759)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 1.5199 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `2.4575` → IC=+0.152 (n=575)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 2.4575 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `8015.433` → IC=+0.232 (n=807)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8015.433 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `77.0` → IC=+0.158 (n=551)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 77.0 (IC base=+0.136)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.161 (n=1274)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0066 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.4454` → IC=+0.152 (n=1445)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5824` → IC=+0.197 (n=1272)

  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` < 0.5824 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `0.5938` → IC=+0.139 (n=1600)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4415` → IC=+0.149 (n=1372)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.4415 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `7246.0232` → IC=+0.196 (n=655)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 7246.0232 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `176.0` → IC=+0.141 (n=1367)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=1817)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.190 (n=1771)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` > 0.4706 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `1.1033` → IC=+0.196 (n=376)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 1.1033 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.508` → IC=+0.238 (n=661)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.508 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` < `0.8932` → IC=+0.134 (n=1181)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 0.8932 (IC base=+0.113)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.127 (n=1783)

  - _Acción_: Kelly boost +0.63€ cuando `libro_spread` < 0.02 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `2898.8632` → IC=+0.248 (n=590)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2898.8632 (IC base=+0.113)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.130 (n=1380)

  - _Acción_: Kelly boost +0.65€ cuando `ballena_activa_n` < 54.0 (IC base=+0.113)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.177 (n=571)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0058 (IC base=+0.114)

- **PATRÓN** `drift_60min` |x|≤ `0.1336` → IC=+0.161 (n=571)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.1336 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.125 (n=1764)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 5.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` < `0.6364` → IC=+0.205 (n=1712)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6364 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` < `0.2216` → IC=+0.133 (n=1392)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.2216 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.469` → IC=+0.127 (n=1654)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 3.469 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` < `0.6488` → IC=+0.158 (n=571)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.6488 (IC base=+0.114)

- **PATRÓN** `volumen_pendiente_norm` > `0.2203` → IC=+0.167 (n=268)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.2203 (IC base=+0.114)

- **PATRÓN** `volumen_spike_ratio` < `1.442` → IC=+0.142 (n=517)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.442 (IC base=+0.114)

- **PATRÓN** `libro_liquidez` > `2815.5676` → IC=+0.172 (n=571)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 2815.5676 (IC base=+0.114)

- **PATRÓN** `ballena_activa_n` < `51.0` → IC=+0.126 (n=1343)

  - _Acción_: Kelly boost +0.63€ cuando `ballena_activa_n` < 51.0 (IC base=+0.114)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` > `0.0193` → IC=+0.217 (n=1176)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0193 (IC base=+0.209)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.214 (n=1836)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.209)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.210 (n=1577)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.209)

- **PATRÓN** `ibs_20min` > `0.5111` → IC=+0.247 (n=1764)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5111 (IC base=+0.209)

- **PATRÓN** `dist_vwap_pct` > `0.2089` → IC=+0.233 (n=1033)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2089 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.219` → IC=+0.273 (n=315)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.219 (IC base=+0.209)

- **PATRÓN** `volumen_regimen` < `1.2447` → IC=+0.211 (n=1764)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2447 (IC base=+0.209)

- **PATRÓN** `volumen_regimen` > `0.6395` → IC=+0.217 (n=1764)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6395 (IC base=+0.209)

- **PATRÓN** `volumen_pendiente_norm` > `0.2335` → IC=+0.252 (n=308)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2335 (IC base=+0.209)

- **PATRÓN** `volumen_spike_ratio` > `2.505` → IC=+0.235 (n=568)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.505 (IC base=+0.209)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.217 (n=1846)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.209)

- **PATRÓN** `libro_liquidez` > `2630.9731` → IC=+0.218 (n=1176)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2630.9731 (IC base=+0.209)

- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.227 (n=627)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.4227` → IC=+0.265 (n=1655)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4227 (IC base=+0.202)

- **PATRÓN** `dist_vwap_pct` > `1.2452` → IC=+0.207 (n=312)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2452 (IC base=+0.202)

- **PATRÓN** `dist_vwap_pct` < `0.9212` → IC=+0.205 (n=2089)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.9212 (IC base=+0.202)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.826` → IC=+0.256 (n=260)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.826 (IC base=+0.202)

- **PATRÓN** `volumen_regimen` > `1.2347` → IC=+0.239 (n=627)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2347 (IC base=+0.202)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1818` → IC=+0.198 (n=1492)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 2.1818 (IC base=+0.202)

- **PATRÓN** `volumen_spike_ratio` > `1.4297` → IC=+0.199 (n=1695)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.147 (n=3179)

- **PATRÓN** `sigma_h` < `0.0091` → IC=+0.168 (n=2691)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0091 (IC base=+0.158)

- **PATRÓN** `drift_60min` |x|≤ `0.5172` → IC=+0.167 (n=3053)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.5172 (IC base=+0.158)
...
- **PATRÓN** `ibs_20min` > `0.9411` → IC=+0.215 (n=1018)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9411 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` > `0.1848` → IC=+0.170 (n=1134)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.1848 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` < `0.4777` → IC=+0.155 (n=1913)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.4777 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.201` → IC=+0.194 (n=501)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 10.201 (IC base=+0.158)

- **PATRÓN** `volumen_regimen` > `0.8955` → IC=+0.165 (n=1361)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` > 0.8955 (IC base=+0.158)

- **PATRÓN** `volumen_pendiente_norm` > `0.1706` → IC=+0.193 (n=839)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.1706 (IC base=+0.158)

- **PATRÓN** `volumen_spike_ratio` < `1.4563` → IC=+0.166 (n=1007)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.4563 (IC base=+0.158)

- **PATRÓN** `volumen_spike_ratio` > `1.8746` → IC=+0.167 (n=2011)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.8746 (IC base=+0.158)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.159 (n=2175)

  - _Acción_: Kelly boost +0.79€ cuando `libro_spread` < 0.01 (IC base=+0.158)

- **PATRÓN** `libro_liquidez` > `8262.3226` → IC=+0.165 (n=1384)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4861` → IC=+0.163 (n=2396)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.1802` → IC=+0.175 (n=1055)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.1802 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.6764` → IC=+0.170 (n=464)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.6764 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.225` → IC=+0.157 (n=2387)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` < 6.225 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` < `1.0974` → IC=+0.157 (n=2019)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.0974 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` < `0.096` → IC=+0.149 (n=2173)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` < 0.096 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` > `0.0718` → IC=+0.147 (n=1106)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` > 0.0718 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` < `1.4246` → IC=+0.159 (n=789)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 1.4246 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` > `1.8137` → IC=+0.147 (n=1577)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.8137 (IC base=+0.145)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.147 (n=3179)

  - _Acción_: Kelly boost +0.73€ cuando `libro_spread` < 0.01 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `7196.849` → IC=+0.157 (n=2140)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 7196.849 (IC base=+0.145)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.173 (n=901)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0073 (IC base=+0.167)

- **PATRÓN** `sigma_h` > `0.0042` → IC=+0.168 (n=1024)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` > 0.0042 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.3821` → IC=+0.173 (n=900)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.3821 (IC base=+0.167)
...
- **PATRÓN** `ibs_20min` < `0.5389` → IC=+0.183 (n=682)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` < 0.5389 (IC base=+0.167)

- **PATRÓN** `ibs_20min` > `0.8847` → IC=+0.176 (n=341)

  - _Acción_: Kelly boost +0.88€ cuando `ibs_20min` > 0.8847 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` < `0.4078` → IC=+0.178 (n=957)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.4078 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.174` → IC=+0.178 (n=915)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` < 4.174 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.082` → IC=+0.169 (n=900)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.082 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` > `0.6398` → IC=+0.171 (n=1023)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` > 0.6398 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.1645` → IC=+0.180 (n=304)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1645 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `2.4732` → IC=+0.172 (n=1004)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 2.4732 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `1.5228` → IC=+0.170 (n=898)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 1.5228 (IC base=+0.167)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.172 (n=1021)
...
- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.165 (n=237)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` > 0.0069 (IC base=+0.130)

- **PATRÓN** `drift_60min` |x|≤ `0.6959` → IC=+0.147 (n=236)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.6959 (IC base=+0.130)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.130 (n=117)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 14.0 (IC base=+0.130)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.185 (n=106)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` < 6.0 (IC base=+0.130)

- **PATRÓN** `ibs_20min` < `0.1364` → IC=+0.228 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1364 (IC base=+0.130)

- **PATRÓN** `dist_vwap_pct` > `0.6223` → IC=+0.224 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6223 (IC base=+0.130)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.617` → IC=+0.136 (n=127)

  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` > 2.617 (IC base=+0.130)

- **PATRÓN** `volumen_regimen` < `0.9487` → IC=+0.150 (n=158)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 0.9487 (IC base=+0.130)

- **PATRÓN** `volumen_pendiente_norm` < `0.1103` → IC=+0.194 (n=191)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` < 0.1103 (IC base=+0.130)

- **PATRÓN** `volumen_spike_ratio` < `1.8554` → IC=+0.145 (n=153)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 1.8554 (IC base=+0.130)

- **PATRÓN** `volumen_spike_ratio` > `2.4856` → IC=+0.133 (n=77)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 2.4856 (IC base=+0.130)

- **PATRÓN** `libro_liquidez` > `3356.999` → IC=+0.164 (n=236)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 3356.999 (IC base=+0.130)

- **PATRÓN** `ballena_activa_n` < `47.0` → IC=+0.177 (n=199)

  - _Acción_: Kelly boost +0.88€ cuando `ballena_activa_n` < 47.0 (IC base=+0.130)
...
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.178 (n=426)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0041 (IC base=+0.088)

- **PATRÓN** `ibs_20min` > `0.6667` → IC=+0.194 (n=781)
...
- **PATRÓN** `volumen_spike_ratio` < `2.098` → IC=+0.122 (n=668)
...
- **PATRÓN** `sigma_h` < `0.006` → IC=+0.152 (n=331)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` < 0.006 (IC base=+0.100)

- **PATRÓN** `ibs_20min` > `0.4737` → IC=+0.182 (n=300)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` > 0.4737 (IC base=+0.100)

- **PATRÓN** `dist_vwap_pct` > `0.1343` → IC=+0.173 (n=154)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.1343 (IC base=+0.100)

- **PATRÓN** `volumen_spike_ratio` < `2.0205` → IC=+0.158 (n=229)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.0205 (IC base=+0.100)
...
- **PATRÓN** `ibs_20min` > `0.6667` → IC=+0.157 (n=249)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` > 0.6667 (IC base=+0.062)

- **PATRÓN** `dist_vwap_pct` > `0.2077` → IC=+0.127 (n=156)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` > 0.2077 (IC base=+0.062)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.497` → IC=+0.149 (n=112)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 5.497 (IC base=+0.062)

- **PATRÓN** `volumen_regimen` > `1.062` → IC=+0.174 (n=93)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 1.062 (IC base=+0.062)

- **PATRÓN** `volumen_pendiente_norm` > `0.2436` → IC=+0.173 (n=53)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.2436 (IC base=+0.062)
...
### LEADLAG_BTC_XRP_15M
- **PATRÓN** `py_entrada` > `0.5` → IC=+0.121 (n=729)

  - _Acción_: Kelly boost +0.61€ cuando `py_entrada` > 0.5 (IC base=+0.107)

- **PATRÓN** `libro_liquidez` > `2924.9554` → IC=+0.172 (n=248)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 2924.9554 (IC base=+0.107)
...
- **PATRÓN** `py_entrada` < `0.54` → IC=+0.172 (n=59)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `py_entrada` < 0.54 (IC base=+0.035)
...
- **FILTRO** `py_entrada` > `0.6` → IC=-0.231 (n=24)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=7631)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.167 (n=3643)

  - _Acción_: SKIP cuando `py_entrada` < 0.475
  - _Potencial_: sin este filtro IC_bueno=+0.057 (n=11075)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.165 (n=3775)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.035 (n=11465)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.46` → IC=-0.209 (n=627)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.100 (n=1932)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.48` → IC=-0.186 (n=651)

  - _Acción_: SKIP cuando `py_entrada` < 0.48
  - _Potencial_: sin este filtro IC_bueno=+0.103 (n=1960)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.206 (n=658)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.062 (n=2075)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=2826)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.205 (n=751)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=2273)

- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)

  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=3003)
...
- **FILTRO** `py_entrada` < `0.395` → IC=-0.230 (n=72)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=260)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.131 (n=10425)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=23144)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.274 (n=8269)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=25300)

- **FILTRO** `ibs_7min` < `0.2788` → IC=-0.235 (n=8392)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2788
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=25177)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.156 (n=11382)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=22187)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.230 (n=10354)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=31893)

- **FILTRO** `ibs_7min` > `0.2923` → IC=-0.177 (n=10557)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2923
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=31690)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.138 (n=1699)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3869)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.311 (n=1326)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4242)

- **FILTRO** `ibs_7min` < `0.7105` → IC=-0.252 (n=1837)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7105
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=3731)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.186 (n=1242)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=4326)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.259 (n=1791)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.004 (n=5474)

- **FILTRO** `drift_7min_pct` |x|> `0.1111` → IC=-0.125 (n=2469)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1111
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=4796)

- **FILTRO** `ibs_7min` > `0.7869` → IC=-0.207 (n=1815)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7869
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=5450)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.139 (n=1372)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=4417)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.250 (n=1407)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=4382)

- **FILTRO** `ibs_7min` < `0.7471` → IC=-0.193 (n=1447)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7471
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4342)

- **FILTRO** `ballena_activa_n` > `160.0` → IC=-0.177 (n=1445)

  - _Acción_: SKIP cuando `ballena_activa_n` > 160.0
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=4344)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.263 (n=1358)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=4523)

- **FILTRO** `ibs_7min` > `0.2609` → IC=-0.179 (n=1467)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2609
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=4414)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.166 (n=1505)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.082 (n=3788)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.304 (n=1312)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=3981)

- **FILTRO** `ibs_7min` < `0.7059` → IC=-0.244 (n=1739)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7059
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=3554)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.222 (n=1197)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4096)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.237 (n=1782)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=5993)

- **FILTRO** `ibs_7min` > `0.75` → IC=-0.172 (n=1938)

  - _Acción_: SKIP cuando `ibs_7min` > 0.75
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=5837)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.129 (n=1797)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=3745)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.233 (n=1643)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=3899)

- **FILTRO** `ibs_7min` < `0.74` → IC=-0.181 (n=1385)

  - _Acción_: SKIP cuando `ibs_7min` < 0.74
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4157)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.174 (n=1346)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=4196)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.265 (n=1253)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=4404)

- **FILTRO** `ibs_7min` > `0.2745` → IC=-0.178 (n=1414)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2745
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=4243)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.184 (n=1382)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4275)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.256 (n=1446)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=4387)

- **FILTRO** `ibs_7min` < `0.2955` → IC=-0.235 (n=1458)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2955
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=4375)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.178 (n=1907)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.023 (n=6137)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.254 (n=1815)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3729)

- **FILTRO** `ibs_7min` < `0.298` → IC=-0.225 (n=1385)

  - _Acción_: SKIP cuando `ibs_7min` < 0.298
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=4159)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.213 (n=1349)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=4195)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.207 (n=1801)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=5824)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=2989)
...
### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` > `0.0112` → IC=+0.224 (n=563)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0112 (IC base=+0.185)

- **PATRÓN** `drift_60min` |x|≤ `0.0507` → IC=+0.204 (n=563)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0507 (IC base=+0.185)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2159` → IC=+0.190 (n=562)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.95€ cuando `delta_ratio_macro` |x|> 0.2159 (IC base=+0.185)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1268` → IC=+0.230 (n=599)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1268 (IC base=+0.185)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.196 (n=1559)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 6.0 (IC base=+0.185)

- **PATRÓN** `ibs_15` > `0.6068` → IC=+0.266 (n=1687)
...
- **PATRÓN** `dist_vwap_pct` < `0.6172` → IC=+0.178 (n=1603)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.6172 (IC base=+0.185)

- **PATRÓN** `sigma_ewma_delta_pct` > `16.791` → IC=+0.272 (n=432)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 16.791 (IC base=+0.185)

- **PATRÓN** `libro_liquidez` > `2964.5226` → IC=+0.192 (n=1125)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 2964.5226 (IC base=+0.185)
...
- **PATRÓN** `sigma_h` < `0.0052` → IC=+0.219 (n=375)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_15min` |x|≤ `0.3843` → IC=+0.209 (n=125)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.3843 (IC base=+0.207)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2526` → IC=+0.248 (n=125)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.2526 (IC base=+0.207)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1084` → IC=+0.275 (n=100)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1084 (IC base=+0.207)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.239 (n=347)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.207)

- **PATRÓN** `ibs_15` > `0.7061` → IC=+0.275 (n=375)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7061 (IC base=+0.207)

- **PATRÓN** `dist_vwap_pct` > `0.3946` → IC=+0.264 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3946 (IC base=+0.207)
...
- **PATRÓN** `sigma_h` < `0.0065` → IC=+0.142 (n=395)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0065 (IC base=+0.133)

- **PATRÓN** `sigma_h` > `0.005` → IC=+0.149 (n=263)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.005 (IC base=+0.133)

- **PATRÓN** `drift_60min` |x|≤ `0.0672` → IC=+0.165 (n=174)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.82€ cuando `drift_60min` |x|≤ 0.0672 (IC base=+0.133)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2342` → IC=+0.172 (n=132)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.86€ cuando `delta_ratio_macro` |x|> 0.2342 (IC base=+0.133)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1188` → IC=+0.167 (n=142)

  - _Acción_: Kelly boost +0.83€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1188 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.149 (n=289)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 11.0 (IC base=+0.133)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.136 (n=413)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` < 17.0 (IC base=+0.133)

- **PATRÓN** `ibs_15` > `0.6586` → IC=+0.252 (n=353)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6586 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `0.1159` → IC=+0.151 (n=282)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.1159 (IC base=+0.133)
...
- **PATRÓN** `libro_liquidez` > `9553.1781` → IC=+0.146 (n=179)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 9553.1781 (IC base=+0.133)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1481` → IC=+0.193 (n=174)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.97€ cuando `drift_60min` |x|≤ 0.1481 (IC base=+0.168)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0713` → IC=+0.187 (n=177)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.94€ cuando `delta_ratio_macro` |x|> 0.0713 (IC base=+0.168)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2704` → IC=+0.218 (n=133)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.2704 (IC base=+0.168)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.188 (n=184)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 6.0 (IC base=+0.168)

- **PATRÓN** `ibs_15` > `0.6` → IC=+0.261 (n=199)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` > `0.1219` → IC=+0.190 (n=114)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.1219 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` < `0.5335` → IC=+0.170 (n=216)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` < 0.5335 (IC base=+0.168)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.168 (n=230)
...
- **PATRÓN** `ballena_activa_n` < `33.0` → IC=+0.216 (n=107)
...
- **FILTRO** `dist_vwap_pct` > `0.7126` → IC=-0.146 (n=97)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.049 (n=988)

### UPDOWN_GBM#SOL#60min
- **PATRÓN** `sigma_ewma_delta_pct` > `8.784` → IC=+0.167 (n=40)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 8.784 (IC base=+0.004)
...
- **FILTRO** `sigma_h` > `0.0129` → IC=-0.223 (n=679)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0129
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=2040)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.199 (n=932)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1378` → IC=+0.251 (n=211)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1378 (IC base=-0.066)

- **PATRÓN** `ibs_15` > `0.6429` → IC=+0.272 (n=648)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6429 (IC base=-0.066)

- **PATRÓN** `dist_vwap_pct` < `0.2752` → IC=+0.187 (n=519)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.2752 (IC base=-0.066)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0756` → IC=+0.246 (n=1681)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0756 (IC base=-0.029)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1784` → IC=+0.239 (n=1216)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1784 (IC base=-0.029)

- **PATRÓN** `ibs_15` < `0.35` → IC=+0.276 (n=1884)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.35 (IC base=-0.029)

- **PATRÓN** `dist_vwap_pct` > `0.6921` → IC=+0.307 (n=303)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6921 (IC base=-0.029)

### UPDOWN_GBM_15M_TARDIO#BTC#15min
- **FILTRO** `sigma_h` > `0.0068` → IC=-0.224 (n=414)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0068
  - _Potencial_: sin este filtro IC_bueno=-0.188 (n=1244)

- **FILTRO** `sigma_h` < `0.0034` → IC=-0.224 (n=414)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **FILTRO** `sigma_ewma_delta_pct` > `19.491` → IC=-0.252 (n=296)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.185 (n=1362)

- **FILTRO** `libro_liquidez` < `14444.3874` → IC=-0.207 (n=547)

  - _Acción_: SKIP cuando `libro_liquidez` < 14444.3874
  - _Potencial_: sin este filtro IC_bueno=-0.192 (n=1111)

- **PATRÓN** `sigma_h` < `0.0029` → IC=+0.156 (n=155)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0029 (IC base=+0.081)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2023` → IC=+0.295 (n=81)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.2023 (IC base=+0.081)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1079` → IC=+0.331 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1079 (IC base=+0.081)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.124 (n=317)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 12.0 (IC base=+0.081)

- **PATRÓN** `ibs_15` > `0.7497` → IC=+0.328 (n=178)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **PATRÓN** `dist_vwap_pct` < `0.3695` → IC=+0.271 (n=177)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3695 (IC base=+0.081)
...
- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.245 (n=727)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0076 (IC base=+0.231)

- **PATRÓN** `drift_60min` |x|≤ `0.4431` → IC=+0.235 (n=727)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4431 (IC base=+0.231)

- **PATRÓN** `drift_15min` |x|≤ `0.4781` → IC=+0.248 (n=320)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.4781 (IC base=+0.231)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2036` → IC=+0.260 (n=331)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **PATRÓN** `ibs_15` < `0.2692` → IC=+0.283 (n=640)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.2692 (IC base=+0.231)

- **PATRÓN** `dist_vwap_pct` > `0.7586` → IC=+0.322 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7586 (IC base=+0.231)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `12.251` → IC=+0.238 (n=770)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0764` → IC=+0.236 (n=286)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0764 (IC base=-0.040)

- **PATRÓN** `ibs_15` < `0.3529` → IC=+0.270 (n=320)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3529 (IC base=-0.040)

- **PATRÓN** `dist_vwap_pct` > `0.7783` → IC=+0.221 (n=66)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7783 (IC base=-0.040)

- **PATRÓN** `dist_vwap_pct` < `0.2005` → IC=+0.229 (n=286)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2005 (IC base=-0.040)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.134` → IC=+0.297 (n=225)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.134 (IC base=-0.041)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1051` → IC=+0.343 (n=215)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1051 (IC base=-0.041)

- **PATRÓN** `ibs_15` < `0.3429` → IC=+0.307 (n=496)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3429 (IC base=-0.041)

- **PATRÓN** `dist_vwap_pct` > `0.9133` → IC=+0.338 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9133 (IC base=-0.041)
...
### UPDOWN_GBM_IBS_ALTO
- **PATRÓN** `sigma_h` < `0.0045` → IC=+0.303 (n=455)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0045 (IC base=+0.292)

- **PATRÓN** `drift_60min` |x|≤ `0.0552` → IC=+0.330 (n=228)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0552 (IC base=+0.292)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2394` → IC=+0.308 (n=227)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.2394 (IC base=+0.292)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1084` → IC=+0.349 (n=190)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1084 (IC base=+0.292)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.314 (n=711)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.292)

- **PATRÓN** `ibs_15` > `0.8418` → IC=+0.329 (n=681)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8418 (IC base=+0.292)

- **PATRÓN** `dist_vwap_pct` > `0.2735` → IC=+0.326 (n=303)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.293 (n=831)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.292)

- **PATRÓN** `libro_liquidez` > `13084.0601` → IC=+0.301 (n=309)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13084.0601 (IC base=+0.292)

### UPDOWN_GBM_IBS_ALTO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.298 (n=250)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0039 (IC base=+0.286)

- **PATRÓN** `sigma_h` > `0.0047` → IC=+0.286 (n=171)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_15` > `0.83` → IC=+0.314 (n=375)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.83 (IC base=+0.286)

- **PATRÓN** `dist_vwap_pct` > `0.4357` → IC=+0.360 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4357 (IC base=+0.286)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6068 sube el IC de +0.185 a +0.266 en UPDOWN_GBM#15min (n=1687). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.7061 sube el IC de +0.207 a +0.275 en UPDOWN_GBM#BTC#15min (n=375). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.6586 sube el IC de +0.133 a +0.252 en UPDOWN_GBM#ETH#15min (n=353). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#SOL#15min**: dentro de BUY_YES, IBS > 0.6 sube el IC de +0.168 a +0.261 en UPDOWN_GBM#SOL#15min (n=199). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.6429 sube el IC de -0.066 a +0.272 en UPDOWN_GBM_15M_TARDIO (n=648). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.35 sube el IC de -0.029 a +0.276 en UPDOWN_GBM_15M_TARDIO (n=1884). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO#BTC#15min**: dentro de BUY_YES, IBS > 0.7497 sube el IC de +0.081 a +0.328 en UPDOWN_GBM_15M_TARDIO#BTC#15min (n=178). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.2692 sube el IC de +0.231 a +0.283 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=640). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#SOL#15min**: dentro de BUY_NO, IBS < 0.3529 sube el IC de -0.040 a +0.270 en UPDOWN_GBM_15M_TARDIO#SOL#15min (n=320). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO#XRP#15min**: dentro de BUY_NO, IBS < 0.3429 sube el IC de -0.041 a +0.307 en UPDOWN_GBM_15M_TARDIO#XRP#15min (n=496). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.