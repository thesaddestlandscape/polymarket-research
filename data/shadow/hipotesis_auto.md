# Hipótesis automáticas — 2026-07-16 22:02 UTC
_Generado por shadow_postmortem.py sobre 17631 resoluciones (PNL=+3624.74€)_
...
- **PATRÓN** `py_entrada` > `0.695` → IC=+0.291 (n=448)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.179)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.183 (n=1139)

  - _Acción_: Kelly boost +0.91€ cuando `libro_spread` < 0.01 (IC base=+0.179)

- **PATRÓN** `libro_liquidez` > `4364.0743` → IC=+0.182 (n=650)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 4364.0743 (IC base=+0.179)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.189 (n=409)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 17.0 (IC base=+0.168)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.177 (n=1219)

  - _Acción_: Kelly boost +0.89€ cuando `libro_spread` < 0.01 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `7415.7641` → IC=+0.186 (n=355)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 7415.7641 (IC base=+0.168)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.207 (n=227)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `3759.4964` → IC=+0.205 (n=225)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3759.4964 (IC base=+0.198)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.224 (n=114)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` < `0.0107` → IC=+0.140 (n=220)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0107 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.151 (n=173)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 18.0 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.515` → IC=+0.195 (n=57)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 5.515 (IC base=+0.110)
...
- **PATRÓN** `sigma_h` < `0.031` → IC=+0.147 (n=466)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.031 (IC base=+0.146)

- **PATRÓN** `sigma_h` > `0.0245` → IC=+0.171 (n=211)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0245 (IC base=+0.146)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.152 (n=417)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.6767` → IC=+0.227 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6767 (IC base=+0.146)
...
- **PATRÓN** `hora_utc` > `13.0` → IC=+0.171 (n=530)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 13.0 (IC base=+0.157)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.160 (n=810)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 18.0 (IC base=+0.157)

- **PATRÓN** `dist_vwap_pct` > `0.3769` → IC=+0.212 (n=175)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3769 (IC base=+0.157)
...
- **PATRÓN** `sigma_h` < `0.0093` → IC=+0.145 (n=74)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` < 0.0093 (IC base=+0.140)

- **PATRÓN** `sigma_h` > `0.0125` → IC=+0.216 (n=100)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0125 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.171 (n=80)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.9751` → IC=+0.150 (n=175)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.9751 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.995` → IC=+0.274 (n=82)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.995 (IC base=+0.140)
...
- **PATRÓN** `sigma_h` < `0.0221` → IC=+0.276 (n=172)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0221 (IC base=+0.252)
...
- **PATRÓN** `dist_vwap_pct` > `0.9553` → IC=+0.364 (n=20)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9553 (IC base=+0.252)
...
- **PATRÓN** `dist_vwap_pct` > `0.6576` → IC=+0.172 (n=120)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.6576 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` < `0.1434` → IC=+0.143 (n=315)
...
- **PATRÓN** `sigma_h` < `0.0333` → IC=+0.208 (n=221)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0333 (IC base=+0.203)

- **PATRÓN** `sigma_h` > `0.0138` → IC=+0.219 (n=197)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0138 (IC base=+0.203)
...
- **PATRÓN** `dist_vwap_pct` > `0.3553` → IC=+0.276 (n=56)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3553 (IC base=+0.203)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.951` → IC=+0.262 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.951 (IC base=+0.203)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2716 | +0.173 | -67.58€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 1750 | +0.204 | -7.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 115 | +0.021 | -17.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 388 | +0.100 | -59.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 463 | +0.154 | +16.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 786 | +0.176 | -18.16€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 968 | +0.173 | -8.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 580 | +0.206 | +6.42€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 953 | +0.171 | -38.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 577 | +0.210 | +10.42€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 5018 | +0.102 | +1557.18€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 5018 | +0.102 | +1557.18€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#SOL | 1344 | +0.110 | +549.40€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1344 | +0.110 | +549.40€ | 1 | 7 |
| ✅ GBM_LATE_15M#XRP | 1275 | +0.154 | +638.36€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1275 | +0.154 | +638.36€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2227 | +0.148 | +1276.28€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2227 | +0.148 | +1276.28€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 597 | +0.116 | +262.69€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 597 | +0.116 | +262.69€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 470 | +0.273 | +487.55€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 470 | +0.273 | +487.55€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 2208 | +0.092 | +671.87€ | 0 | 7 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2208 | +0.092 | +671.87€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 633 | +0.201 | +457.63€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 633 | +0.201 | +457.63€ | 0 | 10 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 171 | +0.153 | +43.63€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 171 | +0.153 | +43.63€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 54 | +0.179 | +24.09€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 54 | +0.179 | +24.09€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.150) — sin ventaja clara. oversold(IBS<0.3): IC=+0.000 n=524 | neutral: IC=-0.018 n=392 | overbought(IBS>0.7): IC=+0.132 n=430
  - _Datos_: n=1422 IC=+0.042 PNL=+136.30€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.104 n=10284 | tras_1loss IC=+0.070 n=7136 | tras_2loss IC=+0.041 n=3036/40 | gap=+0.063 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=15173 IC=+0.107 PNL=+3700.62€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=15173 IC=+0.107 PNL=+3700.62€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.152 > 0.08 con n=225 PNL=+130.05€
  - _Datos_: n=225 IC=+0.152 PNL=+130.05€
