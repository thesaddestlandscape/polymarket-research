# Hipótesis automáticas — 2026-08-19 01:17 UTC
_Generado por shadow_postmortem.py sobre 70081 resoluciones (PNL=+7023.79€)_
...
- **FILTRO** `restante_s_al_confirmar` < `119.16` → IC=-0.402 (n=100)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.118 (n=302)
...
- **FILTRO** `concentracion_yes` < `1.0` → IC=-0.121 (n=64)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.120 (n=845)

- **FILTRO** `restante_s_al_confirmar` < `247.76` → IC=-0.147 (n=681)
...
- **FILTRO** `restante_s_al_confirmar` < `156.97` → IC=-0.290 (n=184)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.202 (n=552)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.191 (n=2385)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.186 (n=495)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.204 (n=1585)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.205 (n=269)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.161 (n=1220)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.152 (n=859)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.86` → IC=+0.384 (n=67)
...
- **PATRÓN** `ibs_20min` < `0.6654` → IC=+0.123 (n=2193)
...
- **PATRÓN** `ballena_activa_n` < `244.0` → IC=+0.275 (n=127)
...
- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.220 (n=1250)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0063 (IC base=+0.215)

- **PATRÓN** `drift_60min` |x|≤ `0.2358` → IC=+0.223 (n=1099)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.375` → IC=+0.280 (n=1252)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.234` → IC=+0.216 (n=1240)
...
- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.233 (n=1229)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.225)

- **PATRÓN** `sigma_h` > `0.0038` → IC=+0.231 (n=1396)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.5` → IC=+0.287 (n=1398)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5 (IC base=+0.225)

- **PATRÓN** `dist_vwap_pct` < `0.1554` → IC=+0.199 (n=911)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.1554 (IC base=+0.225)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.485` → IC=+0.226 (n=1310)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1145` → IC=+0.249 (n=631)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2523` → IC=+0.270 (n=172)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2523 (IC base=+0.225)

- **PATRÓN** `volumen_spike_ratio` < `1.5382` → IC=+0.287 (n=237)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5382 (IC base=+0.225)

- **PATRÓN** `ballena_activa_n` < `57.0` → IC=+0.212 (n=182)
...
- **PATRÓN** `hora_utc` > `19.0` → IC=+0.222 (n=16)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 19.0 (IC base=+0.127)

- **PATRÓN** `drift_20min_pct` |x|≤ `0.0647` → IC=+0.214 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `drift_20min_pct` |x|≤ 0.0647 (IC base=+0.127)

- **PATRÓN** `ibs_20min` > `0.0947` → IC=+0.177 (n=29)

  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` > 0.0947 (IC base=+0.127)

- **PATRÓN** `libro_liquidez` > `2054.213` → IC=+0.159 (n=39)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 2054.213 (IC base=+0.127)
...
- **FILTRO** `py_entrada` < `0.4` → IC=-0.280 (n=107)

  - _Acción_: SKIP cuando `py_entrada` < 0.4
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=340)

- **FILTRO** `ibs_20min` < `0.7209` → IC=-0.243 (n=111)
...
- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.203 (n=109)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=338)

- **FILTRO** `libro_liquidez` < `2035.06` → IC=-0.136 (n=295)
...
- **FILTRO** `ibs_20min` < `0.7175` → IC=-0.289 (n=17)
...
- **FILTRO** `hora_utc` < `15.0` → IC=-0.152 (n=619)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 15.0
...
- **FILTRO** `ibs_7min` < `0.719` → IC=-0.240 (n=313)

  - _Acción_: SKIP cuando `ibs_7min` < 0.719
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=942)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.185 (n=417)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=838)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.192 (n=329)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=1067)
...
- **FILTRO** `ibs_7min` < `0.8431` → IC=-0.203 (n=99)
...
- **FILTRO** `libro_liquidez` < `4929.6301` → IC=-0.152 (n=139)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.130 (n=90)

  - _Acción_: Kelly boost +0.65€ cuando `py_entrada` < 0.5 (IC base=-0.020)
...
- **PATRÓN** `ibs_15` < `0.4585` → IC=+0.121 (n=399)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.60€ cuando `ibs_15` < 0.4585 (IC base=+0.083)

- **PATRÓN** `dist_vwap_pct` > `0.4823` → IC=+0.149 (n=72)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.4823 (IC base=+0.083)
...
| ✅ BALLENAS_TARDIAS | 3491 | -0.114 | -540.67€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 465 | -0.022 | -18.06€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 3026 | -0.128 | -522.61€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 402 | -0.191 | -97.96€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 402 | -0.191 | -97.96€ | 1 | 0 |
...
| 🚫 BALLENAS_TARDIAS#XRP | 736 | -0.225 | -135.51€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP#5min | 736 | -0.225 | -135.51€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 15027 | +0.116 | -858.92€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3467 | +0.183 | -93.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 100 | -0.098 | -46.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 8844 | +0.086 | -750.90€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2616 | +0.133 | +32.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 1493 | +0.030 | -328.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 14 | -0.044 | +0.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 1475 | +0.032 | -323.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 3392 | +0.142 | +9.31€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#DOGE | 1490 | +0.057 | -254.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 10 | +0.000 | -3.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 1479 | +0.058 | -248.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 3666 | +0.126 | -45.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1280 | +0.162 | -17.47€ | 0 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 1469 | +0.102 | -26.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 905 | +0.118 | +7.49€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL | 3498 | +0.137 | -194.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1203 | +0.198 | -49.04€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#XRP | 1488 | +0.123 | -45.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 12 | +0.043 | +1.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 1475 | +0.124 | -45.47€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 6289 | +0.188 | -593.23€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 