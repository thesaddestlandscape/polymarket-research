# Hipótesis automáticas — 2026-07-03 12:32 UTC
_Generado por shadow_postmortem.py sobre 3057 resoluciones (PNL=-38.18€)_

## Patrones causales activos

### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` < `0.0097` → IC=+0.200 (n=78)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0097 (IC base=+0.034)
...
- **PATRÓN** `ibs_15` < `0.0563` → IC=+0.155 (n=56)
  ...
  - _Acción_: Kelly boost +0.78€ cuando `ibs_15` < 0.0563 (IC base=+0.034)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1297` → IC=+0.139 (n=59)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.70€ cuando `pct_spot_vs_ref` |x|≤ 0.1297 (IC base=+0.100)

- **PATRÓN** `sigma_h` < `0.0089` → IC=+0.167 (n=40)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0089 (IC base=+0.100)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.152 (n=21)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 6.0 (IC base=+0.100)

- **PATRÓN** `ibs_15` < `0.0787` → IC=+0.258 (n=31)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0787 (IC base=+0.100)
...
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 402 | +0.059 | +60.34€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 402 | +0.059 | +60.34€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 105 | +0.023 | +5.05€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 105 | +0.023 | +5.05€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 103 | -0.024 | -0.21€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 103 | -0.024 | -0.21€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 858 | -0.042 | -53.72€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 628 | -0.021 | -25.87€ | 0 | 3 |
...
| ✅ UPDOWN_GBM#ETH | 337 | -0.013 | +4.46€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 253 | +0.002 | +10.92€ | 3 | 4 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.058) — sin ventaja clara. oversold(IBS<0.3): IC=-0.012 n=162 | neutral: IC=-0.070 n=140 | overbought(IBS>0.7): IC=-0.046 n=150
  - _Datos_: n=468 IC=-0.043 PNL=-28.48€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.009 n=1468 | tras_1loss IC=-0.028 n=1498 | tras_2loss IC=-0.045 n=772/40 | gap=+0.055 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=735 IC=-0.037 PNL=-40.16€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=735 IC=-0.037 PNL=-40.16€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=218 IC=+0.009 PNL=+9.43€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=218 IC=+0.009 PNL=+9.43€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=631 IC=+0.037 PNL=+51.98€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=631 IC=+0.037 PNL=+51.98€
