# Hipótesis automáticas — 2026-08-26 17:05 UTC
_Generado por shadow_postmortem.py sobre 164165 resoluciones (PNL=+11604.91€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `148.66` → IC=-0.280 (n=2271)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 148.66
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=6816)
...
- **FILTRO** `restante_s_al_confirmar` < `91.42` → IC=-0.436 (n=280)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.096 (n=840)

### BALLENAS_TARDIAS#ETH#5min
- **FILTRO** `restante_s_al_confirmar` < `157.96` → IC=-0.192 (n=619)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 157.96
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=1858)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `150.62` → IC=-0.273 (n=536)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 150.62
  - _Potencial_: sin este filtro IC_bueno=-0.007 (n=1610)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `154.86` → IC=-0.350 (n=518)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 154.86
  - _Potencial_: sin este filtro IC_bueno=-0.119 (n=1054)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.186 (n=5196)

  - _Acción_: Kelly boost +0.93€ cuando `py_entrada` > 0.7 (IC base=+0.096)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.171 (n=1544)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.01 (IC base=+0.096)

- **PATRÓN** `libro_liquidez` > `2382.8902` → IC=+0.173 (n=1492)
...
- **PATRÓN** `py_entrada` < `0.34` → IC=+0.261 (n=3188)
...
- **PATRÓN** `libro_liquidez` > `1844.2492` → IC=+0.182 (n=2396)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 1844.2492 (IC base=+0.141)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.221 (n=615)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.211 (n=757)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.130 (n=527)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.133 (n=1176)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.310 (n=392)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.193 (n=930)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.835` → IC=+0.440 (n=300)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.190 (n=4366)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.202 (n=2105)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.73 (IC base=+0.185)

- **PATRÓN** `libro_liquidez` > `3146.218` → IC=+0.357 (n=47)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3146.218 (IC base=+0.185)

### FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.166 (n=752)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 11.0 (IC base=+0.165)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.179 (n=964)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` < 15.0 (IC base=+0.165)

- **PATRÓN** `py_entrada` < `0.72` → IC=+0.172 (n=949)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.160 (n=1116)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.7` → IC=+0.168 (n=404)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.242 (n=993)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.315 (n=355)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.191 (n=1068)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.7` → IC=+0.194 (n=533)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.194 (n=10471)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.220 (n=8209)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.189)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.127 (n=2138)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 6.0 (IC base=+0.122)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.130 (n=1545)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` < 12.0 (IC base=+0.122)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.154 (n=1577)

  - _Acción_: Kelly boost +0.77€ cuando `py_entrada` > 0.71 (IC base=+0.122)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.243 (n=1816)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.236)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.273 (n=1343)
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.209 (n=690)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.76 (IC base=+0.160)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.237 (n=1665)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.286 (n=648)
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.247 (n=1116)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.210 (n=1403)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.184)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.201 (n=1585)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.38 (IC base=+0.134)

- **PATRÓN** `restante_min` < `3.93` → IC=+0.141 (n=1405)

  - _Acción_: Kelly boost +0.71€ cuando `restante_min` < 3.93 (IC base=+0.134)

- **PATRÓN** `restante_min` > `4.93` → IC=+0.163 (n=1417)
...
- **PATRÓN** `lag_apertura_s` < `4.47` → IC=+0.162 (n=1409)
...
- **PATRÓN** `restante_min` < `4.27` → IC=+0.145 (n=935)

  - _Acción_: Kelly boost +0.72€ cuando `restante_min` < 4.27 (IC base=+0.143)

- **PATRÓN** `restante_min` > `4.91` → IC=+0.175 (n=706)
...
- **PATRÓN** `lag_apertura_s` < `5.64` → IC=+0.175 (n=699)

  - _Acción_: Kelly boost +0.87€ cuando `lag_apertura_s` < 5.64 (IC base=+0.143)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.194 (n=792)

  - _Acción_: Kelly boost +0.97€ cuando `py_entrada` < 0.38 (IC base=+0.126)

- **PATRÓN** `restante_min` < `3.98` → IC=+0.135 (n=712)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9474` → IC=+0.206 (n=716)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9474 (IC base=+0.061)

- **PATRÓN** `dist_vwap_pct` > `0.211` → IC=+0.234 (n=288)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.211 (IC base=+0.061)

- **PATRÓN** `dist_vwap_pct` < `0.5051` → IC=+0.211 (n=407)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5051 (IC base=+0.061)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.223` → IC=+0.169 (n=874)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` > 5.223 (IC base=+0.061)

- **PATRÓN** `volumen_regimen` < `0.6304` → IC=+0.227 (n=152)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6304 (IC base=+0.061)

- **PATRÓN** `volumen_regimen` > `1.0608` → IC=+0.256 (n=207)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0608 (IC base=+0.061)

- **PATRÓN** `volumen_pendiente_norm` > `0.1711` → IC=+0.135 (n=327)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_pendiente_norm` > 0.1711 (IC base=+0.061)

- **PATRÓN** `volumen_spike_ratio` < `1.5707` → IC=+0.135 (n=502)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 1.5707 (IC base=+0.061)

- **PATRÓN** `volumen_spike_ratio` > `1.9323` → IC=+0.127 (n=760)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.9323 (IC base=+0.061)

- **PATRÓN** `ibs_20min` < `0.2129` → IC=+0.130 (n=1469)

  - _Acción_: Kelly boost +0.65€ cuando `ibs_20min` < 0.2129 (IC base=+0.031)

- **PATRÓN** `dist_vwap_pct` < `0.3023` → IC=+0.145 (n=742)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.3023 (IC base=+0.031)

- **PATRÓN** `volumen_regimen` < `0.6362` → IC=+0.154 (n=238)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.6362 (IC base=+0.031)

- **PATRÓN** `volumen_regimen` > `1.0494` → IC=+0.146 (n=323)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 1.0494 (IC base=+0.031)

- **PATRÓN** `volumen_pendiente_norm` > `0.3023` → IC=+0.264 (n=121)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3023 (IC base=+0.031)

- **PATRÓN** `volumen_spike_ratio` < `1.6146` → IC=+0.203 (n=328)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6146 (IC base=+0.031)

- **PATRÓN** `volumen_spike_ratio` > `2.9145` → IC=+0.233 (n=249)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9145 (IC base=+0.031)

- **PATRÓN** `ballena_activa_n` < `43.0` → IC=+0.245 (n=210)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.300 (n=168)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1485` → IC=+0.341 (n=168)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1485 (IC base=+0.275)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.278 (n=237)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.466` → IC=+0.321 (n=222)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.466 (IC base=+0.275)

- **PATRÓN** `sigma_ewma_delta_pct` < `1.912` → IC=+0.313 (n=266)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 1.912 (IC base=+0.275)

- **PATRÓN** `volumen_pendiente_norm` < `0.0577` → IC=+0.297 (n=126)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.23` → IC=+0.321 (n=37)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.23 (IC base=+0.275)

- **PATRÓN** `volumen_spike_ratio` < `1.7714` → IC=+0.292 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7714 (IC base=+0.275)

- **PATRÓN** `volumen_spike_ratio` > `1.5366` → IC=+0.291 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5366 (IC base=+0.275)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3368` → IC=+0.213 (n=312)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3368 (IC base=+0.207)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.232 (n=356)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.4287` → IC=+0.219 (n=354)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4287 (IC base=+0.207)

- **PATRÓN** `dist_vwap_pct` > `0.3109` → IC=+0.235 (n=209)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3109 (IC base=+0.207)

- **PATRÓN** `dist_vwap_pct` < `1.0239` → IC=+0.206 (n=376)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 1.0239 (IC base=+0.207)
...
- **PATRÓN** `volumen_regimen` < `0.7062` → IC=+0.209 (n=156)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.264` → IC=+0.261 (n=44)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.264 (IC base=+0.207)

- **PATRÓN** `volumen_spike_ratio` < `1.4455` → IC=+0.257 (n=146)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4455 (IC base=+0.207)

- **PATRÓN** `libro_liquidez` > `11189.3992` → IC=+0.215 (n=317)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11189.3992 (IC base=+0.207)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0691` → IC=+0.132 (n=142)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.66€ cuando `drift_60min` |x|≤ 0.0691 (IC base=+0.125)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.162 (n=199)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `libro_spread` < 0.01 (IC base=+0.125)

- **PATRÓN** `libro_liquidez` > `1918.9386` → IC=+0.134 (n=192)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 1918.9386 (IC base=+0.125)
...
- **FILTRO** `ibs_20min` > `0.8711` → IC=-0.163 (n=191)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.162 (n=63)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=703)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.0535` → IC=+0.269 (n=24)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0535 (IC base=-0.055)

- **PATRÓN** `volumen_spike_ratio` > `2.3406` → IC=+0.309 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3406 (IC base=-0.055)

- **PATRÓN** `ballena_activa_n` < `123.0` → IC=+0.190 (n=27)

  - _Acción_: Kelly boost +0.95€ cuando `ballena_activa_n` < 123.0 (IC base=-0.055)

### GBM_LATE_15M#SOL#15min
- **FILTRO** `drift_60min` |x|> `0.461` → IC=-0.167 (n=28)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.461
...
- **FILTRO** `ibs_20min` > `0.4842` → IC=-0.121 (n=629)

  - _Acción_: SKIP cuando `ibs_20min` > 0.4842
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=630)

- **FILTRO** `sigma_ewma_delta_pct` > `4.919` → IC=-0.154 (n=267)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.919
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=992)
...
- **PATRÓN** `sigma_h` < `0.0054` → IC=+0.177 (n=29)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0054 (IC base=+0.026)

- **PATRÓN** `drift_60min` |x|≤ `0.2664` → IC=+0.133 (n=58)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.67€ cuando `drift_60min` |x|≤ 0.2664 (IC base=+0.026)

- **PATRÓN** `ibs_20min` > `0.5263` → IC=+0.144 (n=57)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` > 0.5263 (IC base=+0.026)
...
- **FILTRO** `sigma_ewma_delta_pct` > `6.581` → IC=-0.170 (n=189)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=943)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.131 (n=1233)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` > 0.0075 (IC base=+0.051)

- **PATRÓN** `ibs_20min` > `0.9441` → IC=+0.246 (n=906)
...
- **PATRÓN** `volumen_regimen` > `1.0155` → IC=+0.190 (n=424)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_regimen` > 1.0155 (IC base=+0.051)

- **PATRÓN** `volumen_pendiente_norm` < `0.1146` → IC=+0.164 (n=1200)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` < 0.1146 (IC base=+0.051)

- **PATRÓN** `volumen_pendiente_norm` > `0.2434` → IC=+0.201 (n=239)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2434 (IC base=+0.051)

- **PATRÓN** `volumen_spike_ratio` < `1.4768` → IC=+0.186 (n=412)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 1.4768 (IC base=+0.051)

- **PATRÓN** `volumen_spike_ratio` > `2.85` → IC=+0.169 (n=412)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.85 (IC base=+0.051)

- **PATRÓN** `ballena_activa_n` < `100.0` → IC=+0.262 (n=682)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 100.0 (IC base=+0.051)

- **PATRÓN** `ibs_20min` < `0.0952` → IC=+0.182 (n=1164)
...
- **FILTRO** `ibs_20min` < `0.2441` → IC=-0.152 (n=176)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.078` → IC=+0.189 (n=104)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 5.078 (IC base=-0.017)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3584` → IC=+0.206 (n=32)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3584 (IC base=-0.017)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **PATRÓN** `volumen_regimen` > `0.9955` → IC=+0.139 (n=34)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` > 0.9955 (IC base=-0.040)

### GBM_LATE_15M_ESPACIO_ATR#DOGE#15min
- **PATRÓN** `drift_60min` |x|≤ `0.0698` → IC=+0.209 (n=139)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0698 (IC base=+0.174)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.958` → IC=+0.282 (n=122)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.958 (IC base=+0.174)

- **PATRÓN** `volumen_pendiente_norm` < `0.1458` → IC=+0.182 (n=313)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` < 0.1458 (IC base=+0.174)

- **PATRÓN** `volumen_pendiente_norm` > `0.4252` → IC=+0.180 (n=48)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.4252 (IC base=+0.174)

- **PATRÓN** `volumen_spike_ratio` < `2.0211` → IC=+0.167 (n=148)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.0211 (IC base=+0.174)

- **PATRÓN** `volumen_spike_ratio` > `3.6756` → IC=+0.182 (n=152)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 3.6756 (IC base=+0.174)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.218 (n=193)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.174)

- **PATRÓN** `libro_liquidez` > `1916.3861` → IC=+0.207 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1916.3861 (IC base=+0.174)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.157 (n=65)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=888)
...
- **FILTRO** `ibs_20min` < `0.5385` → IC=-0.154 (n=307)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.128 (n=307)
...
- **PATRÓN** `ibs_20min` > `0.8462` → IC=+0.245 (n=155)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8462 (IC base=-0.013)

- **PATRÓN** `dist_vwap_pct` > `0.3189` → IC=+0.250 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3189 (IC base=-0.013)

- **PATRÓN** `volumen_regimen` < `0.8598` → IC=+0.178 (n=116)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` < 0.8598 (IC base=-0.013)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4966` → IC=+0.161 (n=54)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 1.4966 (IC base=-0.013)

- **PATRÓN** `volumen_spike_ratio` > `2.0524` → IC=+0.171 (n=74)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 2.0524 (IC base=-0.013)

- **PATRÓN** `ballena_activa_n` < `53.0` → IC=+0.208 (n=87)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 53.0 (IC base=-0.013)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` > `0.0239` → IC=+0.320 (n=181)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0239 (IC base=+0.213)
...
- **PATRÓN** `ibs_20min` > `0.9` → IC=+0.294 (n=362)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9 (IC base=+0.213)

- **PATRÓN** `dist_vwap_pct` > `1.3472` → IC=+0.336 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.3472 (IC base=+0.213)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.128` → IC=+0.276 (n=320)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.128 (IC base=+0.213)

- **PATRÓN** `volumen_regimen` > `0.8361` → IC=+0.253 (n=362)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8361 (IC base=+0.213)

- **PATRÓN** `volumen_pendiente_norm` < `0.0804` → IC=+0.221 (n=460)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4052` → IC=+0.256 (n=162)
...
- **PATRÓN** `libro_liquidez` > `2659.8294` → IC=+0.234 (n=362)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2659.8294 (IC base=+0.213)
...
- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.186 (n=1063)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0082 (IC base=+0.151)

- **PATRÓN** `drift_60min` |x|≤ `0.3259` → IC=+0.152 (n=2060)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.3259 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.171 (n=803)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 17.0 (IC base=+0.151)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.281 (n=1160)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` > `0.7889` → IC=+0.252 (n=542)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7889 (IC base=+0.151)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.341` → IC=+0.249 (n=997)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.341 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` > `0.6824` → IC=+0.170 (n=1455)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 0.6824 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` > `0.1017` → IC=+0.177 (n=829)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.1017 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` < `2.2764` → IC=+0.149 (n=1843)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 2.2764 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` > `1.8513` → IC=+0.144 (n=1396)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.8513 (IC base=+0.151)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.165 (n=1918)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.151)

- **PATRÓN** `libro_liquidez` > `3897.8392` → IC=+0.186 (n=781)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 3897.8392 (IC base=+0.151)

- **PATRÓN** `ballena_activa_n` < `125.0` → IC=+0.169 (n=1100)

  - _Acción_: Kelly boost +0.84€ cuando `ballena_activa_n` < 125.0 (IC base=+0.151)

- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.202 (n=1852)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0073 (IC base=+0.185)

- **PATRÓN** `drift_60min` |x|≤ `0.4066` → IC=+0.200 (n=2103)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4066 (IC base=+0.185)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.216 (n=783)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.185)

- **PATRÓN** `ibs_20min` < `0.4098` → IC=+0.238 (n=2103)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4098 (IC base=+0.185)

- **PATRÓN** `dist_vwap_pct` < `0.3585` → IC=+0.177 (n=1850)
...
- **PATRÓN** `volumen_regimen` < `1.1823` → IC=+0.166 (n=1707)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.1823 (IC base=+0.185)

- **PATRÓN** `volumen_regimen` > `0.862` → IC=+0.172 (n=1138)
...
- **PATRÓN** `sigma_h` > `0.0051` → IC=+0.278 (n=183)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0051 (IC base=+0.268)

- **PATRÓN** `drift_60min` |x|≤ `0.3308` → IC=+0.317 (n=184)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4158` → IC=+0.301 (n=184)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4158 (IC base=+0.268)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.667` → IC=+0.286 (n=194)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.667 (IC base=+0.268)

- **PATRÓN** `volumen_pendiente_norm` < `0.0799` → IC=+0.276 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0799 (IC base=+0.268)

- **PATRÓN** `volumen_spike_ratio` < `1.7907` → IC=+0.319 (n=81)
...
- **PATRÓN** `libro_liquidez` > `1985.6048` → IC=+0.341 (n=61)
...
- **PATRÓN** `volumen_regimen` < `0.63` → IC=+0.205 (n=110)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.63 (IC base=+0.176)

- **PATRÓN** `volumen_regimen` > `0.8896` → IC=+0.198 (n=220)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` > 0.8896 (IC base=+0.176)

- **PATRÓN** `volumen_pendiente_norm` > `0.2116` → IC=+0.254 (n=63)
...
- **PATRÓN** `volumen_spike_ratio` < `1.3869` → IC=+0.228 (n=101)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3869 (IC base=+0.176)

- **PATRÓN** `libro_liquidez` > `11990.0766` → IC=+0.198 (n=220)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 11990.0766 (IC base=+0.176)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1433` → IC=+0.158 (n=226)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.1433 (IC base=+0.145)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.912` → IC=+0.293 (n=90)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.912 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` < `0.2295` → IC=+0.138 (n=277)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_pendiente_norm` < 0.2295 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` < `2.0039` → IC=+0.196 (n=123)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.0039 (IC base=+0.145)
...
- **PATRÓN** `sigma_h` < `0.0094` → IC=+0.215 (n=331)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0094 (IC base=+0.181)

- **PATRÓN** `drift_60min` |x|≤ `0.4977` → IC=+0.197 (n=331)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.4977 (IC base=+0.181)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.200 (n=335)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.181)

- **PATRÓN** `ibs_20min` > `0.4615` → IC=+0.238 (n=330)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4615 (IC base=+0.181)

- **PATRÓN** `dist_vwap_pct` > `1.0212` → IC=+0.229 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0212 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.407` → IC=+0.333 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.407 (IC base=+0.181)

- **PATRÓN** `volumen_regimen` > `0.6356` → IC=+0.199 (n=330)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` > 0.6356 (IC base=+0.181)

- **PATRÓN** `volumen_pendiente_norm` > `0.101` → IC=+0.238 (n=143)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4363` → IC=+0.216 (n=107)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4363 (IC base=+0.181)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.183 (n=374)

  - _Acción_: Kelly boost +0.92€ cuando `libro_spread` < 0.01 (IC base=+0.181)

- **PATRÓN** `libro_liquidez` > `8624.0494` → IC=+0.203 (n=220)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8624.0494 (IC base=+0.181)

- **PATRÓN** `ballena_activa_n` < `157.0` → IC=+0.178 (n=178)

  - _Acción_: Kelly boost +0.89€ cuando `ballena_activa_n` < 157.0 (IC base=+0.181)
...
- **PATRÓN** `sigma_h` > `0.0117` → IC=+0.201 (n=155)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0117 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.128 (n=485)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 5.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` > `0.8511` → IC=+0.231 (n=310)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8511 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` > `0.7903` → IC=+0.272 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7903 (IC base=+0.114)
...
- **PATRÓN** `volumen_regimen` > `0.6178` → IC=+0.130 (n=465)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` > 0.6178 (IC base=+0.114)

- **PATRÓN** `volumen_pendiente_norm` > `0.0962` → IC=+0.120 (n=164)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_pendiente_norm` > 0.0962 (IC base=+0.114)

- **PATRÓN** `volumen_spike_ratio` > `1.5631` → IC=+0.128 (n=388)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.5631 (IC base=+0.114)

- **PATRÓN** `libro_liquidez` > `3187.8673` → IC=+0.194 (n=155)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 3187.8673 (IC base=+0.114)
...
- **PATRÓN** `sigma_h` > `0.0225` → IC=+0.183 (n=228)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` > 0.0225 (IC base=+0.159)

- **PATRÓN** `drift_60min` |x|≤ `0.1632` → IC=+0.188 (n=222)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.1632 (IC base=+0.159)
...
- **PATRÓN** `ibs_20min` > `0.9253` → IC=+0.257 (n=335)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9253 (IC base=+0.159)

- **PATRÓN** `dist_vwap_pct` > `1.0022` → IC=+0.247 (n=164)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0022 (IC base=+0.159)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.059` → IC=+0.241 (n=272)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.059 (IC base=+0.159)

- **PATRÓN** `volumen_regimen` < `1.1907` → IC=+0.159 (n=503)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.1907 (IC base=+0.159)

- **PATRÓN** `volumen_regimen` > `0.8273` → IC=+0.191 (n=335)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` > 0.8273 (IC base=+0.159)

- **PATRÓN** `volumen_pendiente_norm` > `0.0805` → IC=+0.215 (n=191)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0805 (IC base=+0.159)

- **PATRÓN** `volumen_spike_ratio` < `2.1643` → IC=+0.175 (n=410)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.1643 (IC base=+0.159)

- **PATRÓN** `volumen_spike_ratio` > `1.8043` → IC=+0.164 (n=310)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.8043 (IC base=+0.159)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.188 (n=152)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 17.0 (IC base=+0.077)

- **PATRÓN** `ibs_20min` > `0.6242` → IC=+0.143 (n=354)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` > 0.6242 (IC base=+0.077)

- **PATRÓN** `dist_vwap_pct` > `0.7807` → IC=+0.240 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7807 (IC base=+0.077)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.14` → IC=+0.209 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.14 (IC base=+0.077)

- **PATRÓN** `volumen_pendiente_norm` > `0.1714` → IC=+0.183 (n=99)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.1714 (IC base=+0.077)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0079` → IC=+0.205 (n=1232)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0079 (IC base=+0.143)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.151 (n=2452)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 8.0 (IC base=+0.143)

- **PATRÓN** `ibs_20min` > `0.9433` → IC=+0.288 (n=1231)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9433 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` > `1.0671` → IC=+0.252 (n=433)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0671 (IC base=+0.143)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.357` → IC=+0.245 (n=1149)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.357 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` < `0.7021` → IC=+0.145 (n=846)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.7021 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` > `1.0819` → IC=+0.156 (n=871)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 1.0819 (IC base=+0.143)

- **PATRÓN** `volumen_pendiente_norm` > `0.165` → IC=+0.180 (n=685)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.165 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` < `2.3229` → IC=+0.141 (n=2111)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 2.3229 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` > `1.8767` → IC=+0.156 (n=1599)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.8767 (IC base=+0.143)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.152 (n=2219)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.01 (IC base=+0.143)

- **PATRÓN** `libro_liquidez` > `3828.4706` → IC=+0.191 (n=905)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 3828.4706 (IC base=+0.143)

- **PATRÓN** `ballena_activa_n` < `162.0` → IC=+0.185 (n=1178)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 162.0 (IC base=+0.143)

- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.227 (n=1634)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0064 (IC base=+0.193)

- **PATRÓN** `drift_60min` |x|≤ `0.4556` → IC=+0.202 (n=2446)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4556 (IC base=+0.193)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.195 (n=2239)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5536` → IC=+0.249 (n=2446)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5536 (IC base=+0.193)

- **PATRÓN** `dist_vwap_pct` < `0.7191` → IC=+0.181 (n=2005)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.7191 (IC base=+0.193)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.418` → IC=+0.226 (n=356)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.418 (IC base=+0.193)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.649` → IC=+0.197 (n=2298)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` < 2.649 (IC base=+0.193)

- **PATRÓN** `volumen_regimen` < `0.6243` → IC=+0.187 (n=618)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` < 0.6243 (IC base=+0.193)

- **PATRÓN** `volumen_regimen` > `1.1947` → IC=+0.187 (n=618)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` > 1.1947 (IC base=+0.193)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2519` → IC=+0.209 (n=793)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2519 (IC base=+0.193)

- **PATRÓN** `ballena_activa_n` < `181.0` → IC=+0.169 (n=1318)
...
- **PATRÓN** `ibs_20min` > `0.949` → IC=+0.280 (n=198)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.949 (IC base=+0.132)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.431` → IC=+0.357 (n=166)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.431 (IC base=+0.132)

- **PATRÓN** `volumen_pendiente_norm` > `0.2113` → IC=+0.176 (n=66)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.2113 (IC base=+0.132)

- **PATRÓN** `volumen_spike_ratio` > `1.8922` → IC=+0.133 (n=238)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 1.8922 (IC base=+0.132)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.168 (n=221)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.04 (IC base=+0.132)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.318 (n=174)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.277)

- **PATRÓN** `drift_60min` |x|≤ `0.1571` → IC=+0.324 (n=174)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1571 (IC base=+0.277)
...
- **PATRÓN** `ibs_20min` < `0.5876` → IC=+0.333 (n=261)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5876 (IC base=+0.277)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.979` → IC=+0.303 (n=287)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.979 (IC base=+0.277)

- **PATRÓN** `volumen_pendiente_norm` < `0.0678` → IC=+0.285 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0678 (IC base=+0.277)

- **PATRÓN** `volumen_pendiente_norm` > `0.226` → IC=+0.286 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.226 (IC base=+0.277)

- **PATRÓN** `volumen_spike_ratio` > `1.4811` → IC=+0.285 (n=161)
...
- **PATRÓN** `libro_liquidez` > `1996.9548` → IC=+0.343 (n=87)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1996.9548 (IC base=+0.277)

- **PATRÓN** `ballena_activa_n` < `72.0` → IC=+0.275 (n=87)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.721` → IC=+0.206 (n=117)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.721 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` < `0.6489` → IC=+0.164 (n=144)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.6489 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` > `1.0941` → IC=+0.185 (n=195)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` > 1.0941 (IC base=+0.153)

- **PATRÓN** `volumen_pendiente_norm` > `0.1441` → IC=+0.226 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1441 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` < `2.0884` → IC=+0.179 (n=335)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 2.0884 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` > `1.694` → IC=+0.195 (n=254)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.694 (IC base=+0.153)

- **PATRÓN** `libro_liquidez` > `11715.0209` → IC=+0.184 (n=286)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 11715.0209 (IC base=+0.153)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0753` → IC=+0.180 (n=120)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.90€ cuando `drift_60min` |x|≤ 0.0753 (IC base=+0.172)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.241 (n=164)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.172)

- **PATRÓN** `ibs_20min` > `0.7122` → IC=+0.255 (n=320)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7122 (IC base=+0.172)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.113` → IC=+0.340 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.113 (IC base=+0.172)

- **PATRÓN** `volumen_pendiente_norm` < `0.1461` → IC=+0.175 (n=263)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` < 0.1461 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` < `2.0929` → IC=+0.172 (n=126)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 2.0929 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` > `3.8419` → IC=+0.167 (n=130)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 3.8419 (IC base=+0.172)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.187 (n=244)

  - _Acción_: Kelly boost +0.93€ cuando `libro_spread` < 0.03 (IC base=+0.172)

- **PATRÓN** `libro_liquidez` > `1918.3532` → IC=+0.191 (n=163)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 1918.3532 (IC base=+0.172)

- **PATRÓN** `ballena_activa_n` < `57.0` → IC=+0.214 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 57.0 (IC base=+0.172)

- **PATRÓN** `sigma_h` < `0.0059` → IC=+0.331 (n=146)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0059 (IC base=+0.266)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.271 (n=151)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5575` → IC=+0.332 (n=332)
...
- **PATRÓN** `sigma_h` < `0.0089` → IC=+0.152 (n=432)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` > `0.3491` → IC=+0.196 (n=432)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.521` → IC=+0.191 (n=208)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 4.521 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` < `0.6307` → IC=+0.185 (n=144)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 0.6307 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` > `1.2073` → IC=+0.164 (n=144)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` > 1.2073 (IC base=+0.141)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4093` → IC=+0.194 (n=397)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 1.4093 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `4789.9771` → IC=+0.231 (n=288)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4789.9771 (IC base=+0.141)

- **PATRÓN** `ballena_activa_n` < `167.0` → IC=+0.188 (n=203)

  - _Acción_: Kelly boost +0.94€ cuando `ballena_activa_n` < 167.0 (IC base=+0.141)

- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.227 (n=119)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0028 (IC base=+0.150)

- **PATRÓN** `drift_60min` |x|≤ `0.5036` → IC=+0.160 (n=357)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.5036 (IC base=+0.150)
...
- **PATRÓN** `ibs_20min` < `0.0802` → IC=+0.242 (n=157)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0802 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.1609` → IC=+0.156 (n=181)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.1609 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` < `0.6682` → IC=+0.173 (n=383)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.6682 (IC base=+0.150)
...
- **PATRÓN** `volumen_regimen` < `0.596` → IC=+0.178 (n=119)
  ...
  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` < 0.596 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` > `1.1145` → IC=+0.169 (n=119)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 1.1145 (IC base=+0.150)

- **PATRÓN** `volumen_pendiente_norm` > `0.2304` → IC=+0.337 (n=41)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2304 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` < `2.1935` → IC=+0.185 (n=265)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 2.1935 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.557` → IC=+0.174 (n=268)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 1.557 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `10976.6914` → IC=+0.203 (n=119)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 10976.6914 (IC base=+0.150)
...
- **PATRÓN** `sigma_h` > `0.0107` → IC=+0.168 (n=221)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` > 0.0107 (IC base=+0.092)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.125 (n=329)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 12.0 (IC base=+0.092)
...
- **PATRÓN** `dist_vwap_pct` > `0.7691` → IC=+0.252 (n=131)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7691 (IC base=+0.092)
...
- **PATRÓN** `ballena_activa_n` < `68.0` → IC=+0.165 (n=258)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 68.0 (IC base=+0.092)

- **PATRÓN** `sigma_h` < `0.0059` → IC=+0.187 (n=199)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.0059 (IC base=+0.120)

- **PATRÓN** `drift_60min` |x|≤ `0.5096` → IC=+0.131 (n=453)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.65€ cuando `drift_60min` |x|≤ 0.5096 (IC base=+0.120)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.178 (n=206)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5789` → IC=+0.199 (n=453)

  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` < 0.5789 (IC base=+0.120)

- **PATRÓN** `dist_vwap_pct` < `0.4641` → IC=+0.141 (n=413)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.4641 (IC base=+0.120)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.013` → IC=+0.136 (n=443)

  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` < 3.013 (IC base=+0.120)

- **PATRÓN** `volumen_regimen` < `0.703` → IC=+0.152 (n=199)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.703 (IC base=+0.120)

- **PATRÓN** `volumen_pendiente_norm` > `0.0724` → IC=+0.177 (n=122)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.0724 (IC base=+0.120)

- **PATRÓN** `volumen_spike_ratio` > `1.7463` → IC=+0.168 (n=203)
...
- **PATRÓN** `libro_liquidez` > `2537.3478` → IC=+0.168 (n=206)
  ...
  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2537.3478 (IC base=+0.120)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` > `0.0259` → IC=+0.213 (n=193)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0259 (IC base=+0.169)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.190 (n=214)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 17.0 (IC base=+0.169)

- **PATRÓN** `ibs_20min` > `0.9684` → IC=+0.299 (n=262)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9684 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` > `1.4159` → IC=+0.291 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.4159 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.053` → IC=+0.250 (n=314)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.053 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` < `0.611` → IC=+0.177 (n=193)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 0.611 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` > `0.8477` → IC=+0.187 (n=385)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` > 0.8477 (IC base=+0.169)

- **PATRÓN** `volumen_pendiente_norm` > `0.1713` → IC=+0.246 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1713 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` < `1.5675` → IC=+0.167 (n=232)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.5675 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `1.829` → IC=+0.181 (n=352)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` > 1.829 (IC base=+0.169)
...
- **PATRÓN** `libro_liquidez` > `3108.1723` → IC=+0.182 (n=193)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 3108.1723 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.213 (n=305)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.153 (n=907)

- **PATRÓN** `sigma_h` < `0.0101` → IC=+0.127 (n=371)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.64€ cuando `sigma_h` < 0.0101 (IC base=+0.106)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.149 (n=149)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 17.0 (IC base=+0.106)

- **PATRÓN** `ibs_20min` > `0.8318` → IC=+0.132 (n=191)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` > 0.8318 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` > `1.5243` → IC=+0.200 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.5243 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` < `0.6251` → IC=+0.140 (n=123)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` < 0.6251 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` > `1.2721` → IC=+0.129 (n=122)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` > 1.2721 (IC base=+0.106)

- **PATRÓN** `volumen_pendiente_norm` > `0.1532` → IC=+0.156 (n=120)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_pendiente_norm` > 0.1532 (IC base=+0.106)

- **PATRÓN** `volumen_spike_ratio` < `2.1188` → IC=+0.123 (n=364)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_spike_ratio` < 2.1188 (IC base=+0.106)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.127 (n=403)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.01 (IC base=+0.106)

- **PATRÓN** `libro_liquidez` > `7968.984` → IC=+0.150 (n=281)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 7968.984 (IC base=+0.106)

- **PATRÓN** `sigma_h` < `0.0034` → IC=+0.204 (n=231)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0034 (IC base=+0.147)

- **PATRÓN** `drift_60min` |x|≤ `0.3523` → IC=+0.168 (n=609)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6273` → IC=+0.160 (n=609)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` < 0.6273 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.6306` → IC=+0.169 (n=155)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` > 0.6306 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` < `0.8091` → IC=+0.145 (n=787)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.8091 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.117` → IC=+0.162 (n=312)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 4.117 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.046` → IC=+0.147 (n=693)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` < 6.046 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `1.2237` → IC=+0.160 (n=680)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2237 (IC base=+0.147)

- **PATRÓN** `volumen_pendiente_norm` > `0.0707` → IC=+0.166 (n=333)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.0707 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` < `2.4869` → IC=+0.165 (n=685)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.4869 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` > `1.5125` → IC=+0.153 (n=612)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.5125 (IC base=+0.147)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.153 (n=907)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `8448.0568` → IC=+0.153 (n=692)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 8448.0568 (IC base=+0.147)
...
- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.162 (n=386)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0064 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.3481` → IC=+0.149 (n=386)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.3481 (IC base=+0.141)
...
- **PATRÓN** `ibs_20min` < `0.1485` → IC=+0.169 (n=170)

  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` < 0.1485 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` > `0.6272` → IC=+0.190 (n=56)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.6272 (IC base=+0.141)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.363` → IC=+0.151 (n=388)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` < 6.363 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` < `1.1847` → IC=+0.147 (n=386)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 1.1847 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` > `0.6241` → IC=+0.147 (n=386)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 0.6241 (IC base=+0.141)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5675` → IC=+0.156 (n=385)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.5675 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` > `1.794` → IC=+0.143 (n=256)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.794 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `11896.0265` → IC=+0.145 (n=345)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 11896.0265 (IC base=+0.141)
...
### LEADLAG_BTC_XRP_15M
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.132 (n=36)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 17.0 (IC base=+0.101)

- **PATRÓN** `libro_liquidez` > `2598.9726` → IC=+0.201 (n=95)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2598.9726 (IC base=+0.101)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.169 (n=754)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=2334)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.210 (n=733)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=2368)

- **FILTRO** `ibs_20min` > `0.2727` → IC=-0.174 (n=772)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2727
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=2329)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.182 (n=171)
...
- **FILTRO** `ibs_20min` > `0.2857` → IC=-0.232 (n=121)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=367)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.141 (n=2175)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=5438)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.288 (n=1835)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=5778)

- **FILTRO** `ibs_7min` < `0.736` → IC=-0.236 (n=1903)

  - _Acción_: SKIP cuando `ibs_7min` < 0.736
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=5710)

- **FILTRO** `ballena_activa_n` > `20.0` → IC=-0.176 (n=2533)

  - _Acción_: SKIP cuando `ballena_activa_n` > 20.0
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=5080)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.226 (n=2156)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=7227)

- **FILTRO** `ibs_7min` > `0.7169` → IC=-0.167 (n=2345)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7169
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=7038)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.161 (n=237)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.099 (n=796)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.333 (n=249)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=784)

- **FILTRO** `ibs_7min` < `0.9819` → IC=-0.201 (n=681)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.225 (n=384)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=1211)

- **FILTRO** `drift_7min_pct` |x|> `0.1411` → IC=-0.151 (n=542)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1411
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=1053)

- **FILTRO** `ibs_7min` > `0.8333` → IC=-0.188 (n=398)
...
- **FILTRO** `py_entrada` < `0.38` → IC=-0.245 (n=371)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.007 (n=1146)

- **FILTRO** `ibs_7min` < `0.8047` → IC=-0.169 (n=379)

  - _Acción_: SKIP cuando `ibs_7min` < 0.8047
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=1138)

- **FILTRO** `ballena_activa_n` > `156.0` → IC=-0.184 (n=378)

  - _Acción_: SKIP cuando `ballena_activa_n` > 156.0
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=1139)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.225 (n=373)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=1182)

- **FILTRO** `ballena_activa_n` > `129.0` → IC=-0.164 (n=388)

  - _Acción_: SKIP cuando `ballena_activa_n` > 129.0
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=1167)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.193 (n=262)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.105 (n=810)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.343 (n=252)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=820)

- **FILTRO** `ibs_7min` < `0.2097` → IC=-0.285 (n=268)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2097
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=804)

- **FILTRO** `ballena_activa_n` > `17.0` → IC=-0.282 (n=264)

  - _Acción_: SKIP cuando `ballena_activa_n` > 17.0
  - _Potencial_: sin este filtro IC_bueno=-0.075 (n=808)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.259 (n=351)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=1289)

- **FILTRO** `ibs_7min` > `0.8164` → IC=-0.184 (n=409)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8164
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=1231)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.150 (n=389)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=910)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.267 (n=303)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=996)

- **FILTRO** `ibs_7min` < `0.763` → IC=-0.190 (n=324)

  - _Acción_: SKIP cuando `ibs_7min` < 0.763
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=975)

- **FILTRO** `ballena_activa_n` > `43.0` → IC=-0.213 (n=322)

  - _Acción_: SKIP cuando `ballena_activa_n` > 43.0
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=977)

- **FILTRO** `hora_utc` > `15.0` → IC=-0.124 (n=416)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.081 (n=876)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.287 (n=312)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=980)

- **FILTRO** `ibs_7min` > `0.1818` → IC=-0.158 (n=436)

  - _Acción_: SKIP cuando `ibs_7min` > 0.1818
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=856)

- **FILTRO** `ballena_activa_n` > `38.0` → IC=-0.208 (n=320)

  - _Acción_: SKIP cuando `ballena_activa_n` > 38.0
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=972)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.4` → IC=-0.238 (n=342)

  - _Acción_: SKIP cuando `py_entrada` < 0.4
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=1091)

- **FILTRO** `ibs_7min` < `0.7714` → IC=-0.205 (n=357)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7714
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=1076)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.205 (n=351)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=1082)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.171 (n=427)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=1301)
...
- **FILTRO** `ibs_7min` < `0.75` → IC=-0.238 (n=300)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=959)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.269 (n=310)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=1263)

- **FILTRO** `ibs_7min` > `0.8272` → IC=-0.156 (n=393)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8272
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=1180)
...
- **PATRÓN** `delta_ratio` |x|> `0.449` → IC=+0.184 (n=93)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.92€ cuando `delta_ratio` |x|> 0.449 (IC base=+0.129)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.149 (n=95)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 15.0 (IC base=+0.129)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.127 (n=218)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 18.0 (IC base=+0.129)
...
- **PATRÓN** `libro_liquidez` > `3593.2243` → IC=+0.181 (n=67)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 3593.2243 (IC base=+0.129)
...
- **FILTRO** `streak_estiramiento` > `0.4763` → IC=-0.265 (n=15)

  - _Acción_: SKIP cuando `streak_estiramiento` > 0.4763
...
- **PATRÓN** `ibs_15` < `0.1136` → IC=+0.136 (n=418)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.68€ cuando `ibs_15` < 0.1136 (IC base=+0.027)
...
- **PATRÓN** `ibs_15` > `0.7117` → IC=+0.433 (n=58)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7117 (IC base=+0.335)
...
- **FILTRO** `sigma_h` > `0.0093` → IC=-0.143 (n=239)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.232` → IC=-0.180 (n=329)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 17.232
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=2370)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3346` → IC=+0.139 (n=128)

  - _Acción_: Kelly boost +0.69€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3346 (IC base=-0.057)

- **PATRÓN** `ibs_15` > `0.4486` → IC=+0.159 (n=291)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.79€ cuando `ibs_15` > 0.4486 (IC base=-0.057)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1465` → IC=+0.259 (n=81)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1465 (IC base=-0.079)
...
- **FILTRO** `sigma_h` > `0.0065` → IC=-0.230 (n=202)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0065
  - _Potencial_: sin este filtro IC_bueno=-0.193 (n=395)

- **FILTRO** `sigma_h` < `0.0027` → IC=-0.215 (n=149)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0027
  - _Potencial_: sin este filtro IC_bueno=-0.202 (n=448)

- **FILTRO** `hora_utc` > `16.0` → IC=-0.250 (n=142)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.192 (n=455)

- **FILTRO** `sigma_ewma_delta_pct` > `19.944` → IC=-0.261 (n=111)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 19.944
  - _Potencial_: sin este filtro IC_bueno=-0.193 (n=486)
...
- **FILTRO** `sigma_ewma_delta_pct` > `12.727` → IC=-0.141 (n=143)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 12.727
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=786)
...
- **FILTRO** `hora_utc` > `6.0` → IC=-0.192 (n=157)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 6.0
...
- **FILTRO** `sigma_ewma_delta_pct` > `9.208` → IC=-0.131 (n=193)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=669)

### UPDOWN_GBM_IBS_ALTO
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.280 (n=148)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0041 (IC base=+0.270)

- **PATRÓN** `sigma_h` > `0.0054` → IC=+0.294 (n=100)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0054 (IC base=+0.270)

- **PATRÓN** `drift_60min` |x|≤ `0.0537` → IC=+0.316 (n=74)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0537 (IC base=+0.270)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.382` → IC=+0.314 (n=116)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.382 (IC base=+0.270)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.303 (n=226)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.270)

- **PATRÓN** `ibs_15` > `0.8606` → IC=+0.309 (n=197)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8606 (IC base=+0.270)

- **PATRÓN** `dist_vwap_pct` > `0.2998` → IC=+0.346 (n=102)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2998 (IC base=+0.270)

- **PATRÓN** `sigma_ewma_delta_pct` > `24.679` → IC=+0.280 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 24.679 (IC base=+0.270)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.269 (n=271)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.270)

- **PATRÓN** `libro_liquidez` > `12232.3596` → IC=+0.329 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12232.3596 (IC base=+0.270)

### UPDOWN_GBM_IBS_ALTO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0025` → IC=+0.259 (n=56)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.084` → IC=+0.314 (n=57)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.084 (IC base=+0.252)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1438` → IC=+0.279 (n=84)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1438 (IC base=+0.252)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3917` → IC=+0.290 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3917 (IC base=+0.252)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.280 (n=130)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.252)

- **PATRÓN** `ibs_15` > `0.9661` → IC=+0.314 (n=57)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.9661 (IC base=+0.252)

- **PATRÓN** `dist_vwap_pct` > `0.3041` → IC=+0.345 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3041 (IC base=+0.252)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `6.915` → IC=+0.252 (n=103)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 6.915 (IC base=+0.252)

- **PATRÓN** `libro_liquidez` > `13354.6683` → IC=+0.295 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13354.6683 (IC base=+0.252)
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.4486 sube el IC de -0.057 a +0.159 en UPDOWN_GBM_15M_TARDIO (n=291). Ya aplicado como kelly_boost=+0.79€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.328 sube el IC de -0.079 a +0.345 en UPDOWN_GBM_15M_TARDIO (n=121). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.8606 sube el IC de +0.270 a +0.309 en UPDOWN_GBM_IBS_ALTO (n=197). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_IBS_ALTO#BTC#15min**: dentro de BUY_YES, IBS > 0.9661 sube el IC de +0.252 a +0.314 en UPDOWN_GBM_IBS_ALTO#BTC#15min (n=57). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min**: dentro de BUY_YES, IBS > 0.7117 sube el IC de +0.335 a +0.433 en UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min (n=58). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 6574 | -0.101 | -937.50€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 670 | -0.089 | -119.37€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 5904 | -0.103 | -818.14€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1102 | +0.009 | -145.34€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1102 | +0.009 | -145.34€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 2146 | -0.074 | -107.53€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2146 | -0.074 | -107.53€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1572 | -0.196 | -371.80€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1572 | -0.196 | -371.80€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 30633 | +0.113 | -2014.32€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 5711 | +0.186 | -191.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 107 | -0.105 | -50.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 21673 | +0.094 | -1732.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3142 | +0.121 | -39.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 3638 | +0.062 | -625.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 20 | -0.045 | +2.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 3613 | +0.064 | -622.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 6350 | +0.133 | -138.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1608 | +0.198 | -83.55€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 3607 | +0.109 | -88.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1093 | +0.129 | +55.09€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 3646 | +0.078 | -475.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 20 | +0.045 | +2.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 3625 | +0.078 | -475.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 6821 | +0.127 | -98.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2080 | +0.168 | -12.36€ | 0 | 8 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 3609 | +0.112 | -54.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1120 | +0.102 | -22.56€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#SOL | 6542 | +0.132 | -421.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1966 | +0.202 | -100.35€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 46 | +0.000 | -9.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 3601 | +0.095 | -239.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 929 | +0.133 | -72.30€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 