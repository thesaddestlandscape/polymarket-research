# Hipótesis automáticas — 2026-07-02 16:00 UTC
_Generado por shadow_postmortem.py sobre 2732 resoluciones (PNL=-53.59€)_
...
- **FILTRO** `ibs_15` > `0.3684` → IC=-0.158 (n=36)
  ...
  - _Acción_: SKIP cuando `ibs_15` > 0.3684
  - _Potencial_: sin este filtro IC_bueno=+0.061 (n=112)

- **PATRÓN** `sigma_h` < `0.0101` → IC=+0.226 (n=71)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0101 (IC base=+0.044)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.128 (n=41)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 14.0 (IC base=+0.044)

- **PATRÓN** `ibs_15` < `0.0669` → IC=+0.206 (n=49)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0669 (IC base=+0.044)
...
- **FILTRO** `sigma_h` < `0.005` → IC=-0.139 (n=34)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `13.0` → IC=+0.134 (n=39)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 13.0 (IC base=-0.029)
...
- **FILTRO** `drift_60min` |x|> `0.4142` → IC=-0.134 (n=39)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1297` → IC=+0.160 (n=48)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.80€ cuando `pct_spot_vs_ref` |x|≤ 0.1297 (IC base=+0.131)

- **PATRÓN** `sigma_h` < `0.016` → IC=+0.160 (n=48)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.016 (IC base=+0.131)

- **PATRÓN** `drift_60min` |x|≤ `0.9082` → IC=+0.134 (n=39)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.67€ cuando `drift_60min` |x|≤ 0.9082 (IC base=+0.131)

- **PATRÓN** `drift_15min` |x|≤ `0.688` → IC=+0.125 (n=30)

  - _Acción_: Kelly boost +0.62€ cuando `drift_15min` |x|≤ 0.688 (IC base=+0.131)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.237 (n=17)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.131)

- **PATRÓN** `ibs_15` < `0.0787` → IC=+0.340 (n=23)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0787 (IC base=+0.131)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS < 0.0669 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.206 n=49). Confirma señal de reversión media → alinear con BUY_YES.
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.0787 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.340 n=23). Confirma señal de reversión media → alinear con BUY_YES.
- **LIVE-CANDIDATA**: `GBM_LATE_15M#XRP#15min` — IC=+0.219 n=30. Faltan ~10 resoluciones para umbral n≥40. ETA: ~7h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M#XRP` — IC=+0.219 n=30. Faltan ~10 resoluciones para umbral n≥40. ETA: ~7h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M#BTC#15min` — IC=+0.122 n=35. Faltan ~5 resoluciones para umbral n≥40. ETA: ~4h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M#BTC` — IC=+0.122 n=35. Faltan ~5 resoluciones para umbral n≥40. ETA: ~4h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 129 | +0.134 | +35.64€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 129 | +0.134 | +35.64€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 35 | +0.122 | +8.48€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 35 | +0.122 | +8.48€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 35 | +0.041 | +1.92€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 35 | +0.041 | +1.92€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL | 29 | +0.145 | +9.27€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 29 | +0.145 | +9.27€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP | 30 | +0.219 | +15.97€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 30 | +0.219 | +15.97€ | 0 | 0 |
...
| ✅ UPDOWN_GBM#15min | 596 | -0.015 | -16.13€ | 2 | 3 |
...
| ✅ UPDOWN_GBM#BTC | 241 | -0.043 | -33.79€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 170 | -0.017 | -23.08€ | 2 | 1 |
...
| ✅ UPDOWN_GBM#ETH | 316 | -0.006 | +9.28€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 232 | +0.013 | +15.75€ | 2 | 6 |
...
| ✅ WEEKLY_PRICE | 73 | -0.020 | -16.51€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.079) — sin ventaja clara. oversold(IBS<0.3): IC=+0.003 n=145 | neutral: IC=-0.076 n=130 | overbought(IBS>0.7): IC=-0.044 n=145
  - _Datos_: n=434 IC=-0.037 PNL=-18.77€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=01h: IC=-0.200 n=28 PNL=-13.74€ → FILTRAR | H=02h: IC=-0.227 n=20 PNL=-7.89€ → FILTRAR | H=10h: IC=-0.154 n=15 PNL=-6.77€ → FILTRAR | H=16h: IC=+0.225 n=18 PNL=+7.60€ → BOOST | H=17h: IC=+0.110 n=15 PNL=+2.13€ → BOOST | H=20h: IC=-0.180 n=23 PNL=-6.41€ → FILTRAR | H=23h: IC=+0.180 n=18 PNL=+12.32€ → BOOST
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.011 n=1305 | tras_1loss IC=-0.032 n=1339 | tras_2loss IC=-0.044 n=695/40 | gap=+0.055 (umbral 0.05)
...
**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  ...
  - _Estado_: n_overlaps=24, boost estimado=+0.009. Necesita 0 más y boost>0.05
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=-0.038 n=117/40 PNL=+0.58€ | H=15h UTC: IC=+0.017 n=116/40 PNL=+5.20€ | H=17h UTC: IC=+0.221 n=41/40 PNL=+14.13€ | H=19h UTC: IC=+0.028 n=51/40 PNL=+1.21€
...
**⏳ H-WEEKLY** — Predicciones semanales de precio por par
  ...
  - _Estado_: ETH: n=25/15 IC=+0.018 PNL=-5.18€ | BTC: n=26/15 IC=-0.036 PNL=-7.89€ | SOL: n=22/15 IC=-0.042 PNL=-3.43€

**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  - _Umbral_: n≥40 en contrario_BTC y gap≥0.08 — y descartar confound con drift propio antes de actuar
  - _Acción_: Si se confirma y no es confound → boost en ETH/SOL cuando decisión contraria a drift_15min BTC
  - _Estado_: alineado_BTC IC=-0.026 n=93 | contrario_BTC IC=-0.058 n=41/40 | gap=-0.032 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-24H-GBM-BUYYES-TARDE** — GBM BUY_YES en tarde europea (15-19h UTC) — señal alcista sostenida
  ...
  - _Estado_: n=146 IC=-0.041 PNL=-11.52€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=146 IC=-0.041 PNL=-11.52€
...
**⏳ H-WEEKLY-BUYNO** — WEEKLY_PRICE BUY_NO — dirección dominante con IC muy alto
  ...
  - _Estado_: 49/50 ops en el filtro definido (IC actual=+0.108 PNL=-4.50€)
  - _Datos_: n=49 IC=+0.108 PNL=-4.50€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=701 IC=-0.033 PNL=-30.45€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=701 IC=-0.033 PNL=-30.45€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=381 IC=-0.048 PNL=-42.72€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=381 IC=-0.048 PNL=-42.72€
...
**🔴 H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.017 < 0.02 con n=170 PNL=-23.08€
  - _Datos_: n=170 IC=-0.017 PNL=-23.08€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=252 IC=-0.008 PNL=-27.06€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=252 IC=-0.008 PNL=-27.06€
...
**〰️ H-CUSTOM-ETH15-REVERSION** — ETH#15min con drift_15min < -1 — ¿mean reversion?
  ...
  - _Estado_: n=37 IC=+0.064 PNL=+8.48€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=37 IC=+0.064 PNL=+8.48€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=197 IC=+0.018 PNL=+14.92€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=197 IC=+0.018 PNL=+14.92€
...
**⏳ H-CUSTOM-CROSS-WINDOW-SPREAD-NEG** — GBM 15min/60min: spread negativo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: 3/40 ops en el filtro definido (IC actual=-0.015 PNL=-1.78€)
  - _Datos_: n=3 IC=-0.015 PNL=-1.78€
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=307 IC=+0.066 PNL=+36.07€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=307 IC=+0.066 PNL=+36.07€
...
**⏳ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: 33/40 ops en el filtro definido (IC actual=-0.014 PNL=-6.83€)
  - _Datos_: n=33 IC=-0.014 PNL=-6.83€
...
**🔴 H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.073 < -0.05 con n=523 PNL=-70.64€
  - _Datos_: n=523 IC=-0.073 PNL=-70.64€
...
**⏳ H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: 67/120 ops en el filtro definido (IC actual=+0.123 PNL=+7.77€)
  - _Datos_: n=67 IC=+0.123 PNL=+7.77€
