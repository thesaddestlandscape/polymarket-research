# Hipótesis automáticas — 2026-09-19 02:27 UTC
_Generado por shadow_postmortem.py sobre 505187 resoluciones (PNL=+55045.42€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `144.69` → IC=-0.244 (n=6192)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 144.69
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=18576)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `141.04` → IC=-0.253 (n=852)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 141.04
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=2557)
...
- **FILTRO** `restante_s_al_confirmar` < `133.42` → IC=-0.281 (n=757)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 133.42
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=2274)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `158.73` → IC=-0.235 (n=1461)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.73
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=4385)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `120.24` → IC=-0.367 (n=1206)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 120.24
  - _Potencial_: sin este filtro IC_bueno=-0.110 (n=3619)
...
- **FILTRO** `py_entrada` > `0.54` → IC=-0.158 (n=188)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=431)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.199 (n=12426)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.69 (IC base=+0.099)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.154 (n=3107)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.099)

- **PATRÓN** `libro_liquidez` > `5397.0828` → IC=+0.175 (n=1979)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.207 (n=1463)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.207 (n=1841)
...
- **PATRÓN** `py_entrada` < `0.375` → IC=+0.262 (n=1344)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.375 (IC base=+0.201)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.201 (n=1887)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.201)

- **PATRÓN** `libro_liquidez` > `13658.1592` → IC=+0.212 (n=662)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13658.1592 (IC base=+0.201)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.161 (n=308)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.141 (n=2121)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` < 15.0 (IC base=+0.140)

- **PATRÓN** `py_entrada` > `0.7` → IC=+0.330 (n=844)
...
- **PATRÓN** `libro_spread` < `0.03` → IC=+0.173 (n=1158)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.201 (n=9456)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.198)

- **PATRÓN** `py_entrada` > `0.74` → IC=+0.224 (n=3433)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.175 (n=2330)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` < 17.0 (IC base=+0.168)

- **PATRÓN** `py_entrada` < `0.74` → IC=+0.178 (n=2439)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.284 (n=202)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.185 (n=2288)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.239 (n=1819)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.239)

- **PATRÓN** `py_entrada` > `0.76` → IC=+0.325 (n=723)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.225 (n=5217)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.224)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.274 (n=1871)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.187 (n=5417)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.220 (n=1988)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.219)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.266 (n=1845)
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.254 (n=2452)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.193 (n=4867)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.915` → IC=+0.434 (n=196)
...
### GBM_LATE_15M
- **PATRÓN** `drift_60min` |x|≤ `0.3518` → IC=+0.123 (n=5965)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 0.3518 (IC base=+0.101)

- **PATRÓN** `ibs_20min` > `0.9798` → IC=+0.238 (n=2259)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9798 (IC base=+0.101)

- **PATRÓN** `dist_vwap_pct` < `0.6097` → IC=+0.246 (n=1866)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6097 (IC base=+0.101)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.862` → IC=+0.173 (n=2624)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 5.862 (IC base=+0.101)

- **PATRÓN** `volumen_regimen` < `1.2155` → IC=+0.246 (n=1763)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2155 (IC base=+0.101)

- **PATRÓN** `volumen_regimen` > `1.0578` → IC=+0.246 (n=800)
...
- **PATRÓN** `volumen_spike_ratio` > `1.911` → IC=+0.204 (n=3020)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.911 (IC base=+0.101)

- **PATRÓN** `ibs_20min` < `0.5709` → IC=+0.132 (n=8229)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.5709 (IC base=+0.059)

- **PATRÓN** `dist_vwap_pct` > `0.5729` → IC=+0.186 (n=552)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.5729 (IC base=+0.059)

- **PATRÓN** `dist_vwap_pct` < `0.1397` → IC=+0.168 (n=2510)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` < 0.1397 (IC base=+0.059)

- **PATRÓN** `volumen_regimen` < `0.7018` → IC=+0.171 (n=1212)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 0.7018 (IC base=+0.059)

- **PATRÓN** `volumen_regimen` > `0.8707` → IC=+0.173 (n=1835)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` > 0.8707 (IC base=+0.059)

- **PATRÓN** `volumen_pendiente_norm` > `0.1664` → IC=+0.223 (n=1328)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1664 (IC base=+0.059)

- **PATRÓN** `volumen_spike_ratio` > `1.5813` → IC=+0.199 (n=4085)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5813 (IC base=+0.059)

- **PATRÓN** `ballena_activa_n` < `151.0` → IC=+0.210 (n=4353)
...
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.174 (n=511)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0081 (IC base=+0.163)

- **PATRÓN** `drift_60min` |x|≤ `0.3322` → IC=+0.167 (n=1531)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.3322 (IC base=+0.163)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.168 (n=752)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 15.0 (IC base=+0.163)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.186 (n=575)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.032` → IC=+0.276 (n=668)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4339` → IC=+0.167 (n=1417)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.4339 (IC base=+0.163)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.182 (n=1471)

  - _Acción_: Kelly boost +0.91€ cuando `libro_spread` < 0.04 (IC base=+0.163)

- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.248 (n=1007)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0049 (IC base=+0.233)

- **PATRÓN** `drift_60min` |x|≤ `0.088` → IC=+0.291 (n=376)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.0583` → IC=+0.293 (n=496)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0583 (IC base=+0.233)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.373` → IC=+0.247 (n=1178)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.373 (IC base=+0.233)

- **PATRÓN** `volumen_pendiente_norm` < `0.0914` → IC=+0.228 (n=951)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.235 (n=1187)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.233)

- **PATRÓN** `libro_liquidez` > `1608.68` → IC=+0.249 (n=1006)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1608.68 (IC base=+0.233)
...
- **PATRÓN** `ibs_20min` > `0.9146` → IC=+0.260 (n=527)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9146 (IC base=+0.213)

- **PATRÓN** `dist_vwap_pct` > `0.1948` → IC=+0.215 (n=612)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1948 (IC base=+0.213)

- **PATRÓN** `dist_vwap_pct` < `0.3726` → IC=+0.217 (n=1084)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3726 (IC base=+0.213)
...
- **PATRÓN** `volumen_regimen` < `1.2524` → IC=+0.223 (n=1162)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2524 (IC base=+0.213)

- **PATRÓN** `volumen_regimen` > `0.876` → IC=+0.215 (n=774)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.876 (IC base=+0.213)

- **PATRÓN** `volumen_pendiente_norm` < `0.159` → IC=+0.214 (n=1185)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3804` → IC=+0.226 (n=378)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3804 (IC base=+0.213)

- **PATRÓN** `libro_liquidez` > `11102.6859` → IC=+0.225 (n=1161)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11102.6859 (IC base=+0.213)

- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.157 (n=824)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0039 (IC base=+0.133)

- **PATRÓN** `drift_60min` |x|≤ `0.0769` → IC=+0.151 (n=411)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.0769 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.162 (n=418)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 18.0 (IC base=+0.133)

- **PATRÓN** `ibs_20min` < `0.6789` → IC=+0.169 (n=1231)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` < 0.6789 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `0.1261` → IC=+0.150 (n=1099)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.1261 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.325` → IC=+0.162 (n=205)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 11.325 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.188` → IC=+0.133 (n=1107)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` < 4.188 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` < `1.2085` → IC=+0.142 (n=1231)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 1.2085 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` > `0.6203` → IC=+0.137 (n=1231)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 0.6203 (IC base=+0.133)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4323` → IC=+0.145 (n=1122)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 2.4323 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` > `1.7671` → IC=+0.140 (n=748)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` > 1.7671 (IC base=+0.133)

- **PATRÓN** `libro_liquidez` > `13664.2244` → IC=+0.138 (n=821)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 13664.2244 (IC base=+0.133)

- **PATRÓN** `ballena_activa_n` < `420.0` → IC=+0.142 (n=1040)

  - _Acción_: Kelly boost +0.71€ cuando `ballena_activa_n` < 420.0 (IC base=+0.133)

### GBM_LATE_15M#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0059` → IC=+0.198 (n=1496)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.196 (n=570)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` < 6.0 (IC base=+0.183)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.261 (n=588)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.183)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.505` → IC=+0.232 (n=430)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.505 (IC base=+0.183)

- **PATRÓN** `volumen_pendiente_norm` < `0.2127` → IC=+0.187 (n=1469)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` < 0.2127 (IC base=+0.183)

- **PATRÓN** `volumen_pendiente_norm` > `0.3715` → IC=+0.190 (n=195)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.3715 (IC base=+0.183)

- **PATRÓN** `volumen_spike_ratio` > `2.9317` → IC=+0.204 (n=637)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9317 (IC base=+0.183)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.196 (n=1732)

  - _Acción_: Kelly boost +0.98€ cuando `libro_spread` < 0.04 (IC base=+0.183)

- **PATRÓN** `sigma_h` < `0.0106` → IC=+0.227 (n=1269)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0106 (IC base=+0.219)

- **PATRÓN** `sigma_h` > `0.0064` → IC=+0.219 (n=1133)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.3824` → IC=+0.236 (n=1118)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3824 (IC base=+0.219)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.704` → IC=+0.237 (n=459)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.704 (IC base=+0.219)

- **PATRÓN** `volumen_pendiente_norm` > `0.3632` → IC=+0.267 (n=187)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3632 (IC base=+0.219)

- **PATRÓN** `volumen_spike_ratio` < `1.8407` → IC=+0.208 (n=502)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8407 (IC base=+0.219)

- **PATRÓN** `volumen_spike_ratio` > `2.2824` → IC=+0.227 (n=761)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2824 (IC base=+0.219)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.234 (n=738)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.219)

- **PATRÓN** `libro_liquidez` > `1905.7248` → IC=+0.243 (n=423)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1905.7248 (IC base=+0.219)

- **PATRÓN** `ballena_activa_n` < `26.0` → IC=+0.230 (n=727)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.152 (n=90)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.013 (n=1895)

- **PATRÓN** `ibs_20min` > `0.9377` → IC=+0.192 (n=310)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` > 0.9377 (IC base=+0.016)

- **PATRÓN** `dist_vwap_pct` > `0.3371` → IC=+0.341 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3371 (IC base=+0.016)

- **PATRÓN** `dist_vwap_pct` < `0.6795` → IC=+0.328 (n=282)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6795 (IC base=+0.016)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.461` → IC=+0.143 (n=595)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` > 4.461 (IC base=+0.016)

- **PATRÓN** `volumen_regimen` < `0.6001` → IC=+0.341 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6001 (IC base=+0.016)

- **PATRÓN** `volumen_regimen` > `1.1897` → IC=+0.352 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1897 (IC base=+0.016)

- **PATRÓN** `volumen_pendiente_norm` > `0.2908` → IC=+0.371 (n=68)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2908 (IC base=+0.016)

- **PATRÓN** `volumen_spike_ratio` < `1.4779` → IC=+0.347 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4779 (IC base=+0.016)

- **PATRÓN** `volumen_spike_ratio` > `2.1564` → IC=+0.344 (n=113)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1564 (IC base=+0.016)

- **PATRÓN** `ballena_activa_n` < `168.0` → IC=+0.342 (n=244)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 168.0 (IC base=+0.016)

- **PATRÓN** `dist_vwap_pct` > `0.2947` → IC=+0.168 (n=200)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` > 0.2947 (IC base=+0.006)

- **PATRÓN** `dist_vwap_pct` < `0.4682` → IC=+0.145 (n=730)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.4682 (IC base=+0.006)

- **PATRÓN** `volumen_regimen` < `0.6164` → IC=+0.161 (n=219)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.6164 (IC base=+0.006)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4994` → IC=+0.183 (n=537)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 1.4994 (IC base=+0.006)
...
- **FILTRO** `ibs_20min` > `0.2717` → IC=-0.132 (n=1882)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2717
  - _Potencial_: sin este filtro IC_bueno=+0.121 (n=927)

- **FILTRO** `sigma_ewma_delta_pct` > `8.613` → IC=-0.214 (n=313)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.613
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=2496)
...
- **PATRÓN** `ibs_20min` < `0.2717` → IC=+0.121 (n=927)

  - _Acción_: Kelly boost +0.61€ cuando `ibs_20min` < 0.2717 (IC base=-0.048)

- **PATRÓN** `dist_vwap_pct` > `0.8202` → IC=+0.300 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8202 (IC base=-0.048)

- **PATRÓN** `volumen_regimen` < `1.096` → IC=+0.218 (n=239)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.096 (IC base=-0.048)

- **PATRÓN** `volumen_pendiente_norm` < `0.1022` → IC=+0.220 (n=184)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4253` → IC=+0.257 (n=212)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.4253 (IC base=-0.048)

### GBM_LATE_15M#XRP#15min
- **FILTRO** `drift_60min` |x|> `0.6568` → IC=-0.195 (n=473)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6568
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=1421)

- **FILTRO** `ibs_20min` < `0.6824` → IC=-0.159 (n=1250)

  - _Acción_: SKIP cuando `ibs_20min` < 0.6824
  - _Potencial_: sin este filtro IC_bueno=+0.074 (n=644)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.196 (n=386)

  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=1508)

- **FILTRO** `ibs_20min` > `0.775` → IC=-0.203 (n=709)

  - _Acción_: SKIP cuando `ibs_20min` > 0.775
  - _Potencial_: sin este filtro IC_bueno=+0.033 (n=2131)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1859` → IC=+0.257 (n=430)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1859 (IC base=-0.026)

- **PATRÓN** `volumen_spike_ratio` > `1.5844` → IC=+0.245 (n=437)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5844 (IC base=-0.026)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.246 (n=499)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 35.0 (IC base=-0.026)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.192 (n=2812)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0094 (IC base=+0.093)

- **PATRÓN** `ibs_20min` > `0.4648` → IC=+0.184 (n=7535)

  - _Acción_: Kelly boost +0.92€ cuando `ibs_20min` > 0.4648 (IC base=+0.093)

- **PATRÓN** `dist_vwap_pct` > `1.0322` → IC=+0.292 (n=638)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0322 (IC base=+0.093)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.509` → IC=+0.152 (n=4001)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 3.509 (IC base=+0.093)

- **PATRÓN** `volumen_regimen` > `0.6793` → IC=+0.246 (n=2597)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6793 (IC base=+0.093)

- **PATRÓN** `volumen_pendiente_norm` > `0.2445` → IC=+0.264 (n=926)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2445 (IC base=+0.093)

- **PATRÓN** `volumen_spike_ratio` < `1.4732` → IC=+0.247 (n=1560)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4732 (IC base=+0.093)

- **PATRÓN** `volumen_spike_ratio` > `2.72` → IC=+0.245 (n=1560)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.72 (IC base=+0.093)

- **PATRÓN** `ballena_activa_n` < `101.0` → IC=+0.278 (n=4183)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 101.0 (IC base=+0.093)

- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.141 (n=2848)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` > 0.0088 (IC base=+0.069)

- **PATRÓN** `ibs_20min` < `0.5539` → IC=+0.150 (n=7513)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` < 0.5539 (IC base=+0.069)

- **PATRÓN** `dist_vwap_pct` > `0.688` → IC=+0.234 (n=483)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.688 (IC base=+0.069)

- **PATRÓN** `dist_vwap_pct` < `0.2343` → IC=+0.232 (n=2267)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2343 (IC base=+0.069)

- **PATRÓN** `volumen_regimen` < `0.7171` → IC=+0.232 (n=1058)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7171 (IC base=+0.069)

- **PATRÓN** `volumen_regimen` > `1.2001` → IC=+0.246 (n=800)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2001 (IC base=+0.069)

- **PATRÓN** `volumen_pendiente_norm` > `0.248` → IC=+0.317 (n=617)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.248 (IC base=+0.069)

- **PATRÓN** `volumen_spike_ratio` > `2.3499` → IC=+0.257 (n=1416)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3499 (IC base=+0.069)

- **PATRÓN** `ballena_activa_n` < `80.0` → IC=+0.259 (n=2988)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 80.0 (IC base=+0.069)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `sigma_ewma_delta_pct` > `4.372` → IC=-0.167 (n=440)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.372
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=1481)

- **PATRÓN** `ibs_20min` > `0.8848` → IC=+0.272 (n=577)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8848 (IC base=+0.045)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.283` → IC=+0.158 (n=790)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4451` → IC=+0.200 (n=218)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4451 (IC base=+0.045)

- **PATRÓN** `volumen_spike_ratio` > `2.5703` → IC=+0.204 (n=218)
...
- **FILTRO** `ibs_20min` > `0.82` → IC=-0.147 (n=624)

  - _Acción_: SKIP cuando `ibs_20min` > 0.82
  - _Potencial_: sin este filtro IC_bueno=+0.047 (n=1874)

- **PATRÓN** `dist_vwap_pct` > `0.3015` → IC=+0.158 (n=299)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.3015 (IC base=+0.018)

- **PATRÓN** `volumen_regimen` < `1.1952` → IC=+0.142 (n=753)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 1.1952 (IC base=+0.018)

- **PATRÓN** `volumen_regimen` > `0.6566` → IC=+0.150 (n=673)
...
- **PATRÓN** `dist_vwap_pct` < `0.155` → IC=+0.211 (n=462)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.258 (n=503)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.739` → IC=+0.285 (n=328)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.739 (IC base=+0.244)

- **PATRÓN** `volumen_pendiente_norm` < `0.1384` → IC=+0.258 (n=1178)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1384 (IC base=+0.244)

- **PATRÓN** `volumen_spike_ratio` < `1.862` → IC=+0.248 (n=553)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.862 (IC base=+0.244)

- **PATRÓN** `volumen_spike_ratio` > `2.9866` → IC=+0.257 (n=570)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9866 (IC base=+0.244)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.261 (n=1543)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.244)

- **PATRÓN** `libro_liquidez` > `1917.74` → IC=+0.256 (n=448)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1917.74 (IC base=+0.244)

- **PATRÓN** `ballena_activa_n` < `17.0` → IC=+0.267 (n=462)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 17.0 (IC base=+0.244)

- **PATRÓN** `sigma_h` > `0.0056` → IC=+0.302 (n=1061)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.3333` → IC=+0.287 (n=1061)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3333 (IC base=+0.283)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.885` → IC=+0.301 (n=401)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6234` → IC=+0.293 (n=322)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6234 (IC base=+0.283)

- **PATRÓN** `volumen_spike_ratio` > `2.8783` → IC=+0.284 (n=438)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8783 (IC base=+0.283)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.295 (n=613)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.283)

- **PATRÓN** `libro_liquidez` > `1898.72` → IC=+0.315 (n=354)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1898.72 (IC base=+0.283)

- **PATRÓN** `ballena_activa_n` < `20.0` → IC=+0.288 (n=414)
...
- **FILTRO** `ibs_20min` < `0.2556` → IC=-0.207 (n=397)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.067 (n=1193)
...
- **PATRÓN** `ibs_20min` > `0.8027` → IC=+0.159 (n=541)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.8027 (IC base=-0.001)

- **PATRÓN** `dist_vwap_pct` > `0.4586` → IC=+0.237 (n=173)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4586 (IC base=-0.001)

- **PATRÓN** `volumen_regimen` < `0.9674` → IC=+0.223 (n=352)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9674 (IC base=-0.001)

- **PATRÓN** `volumen_regimen` > `0.6394` → IC=+0.214 (n=358)
...
- **PATRÓN** `volumen_spike_ratio` < `2.066` → IC=+0.245 (n=328)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.066 (IC base=-0.001)

- **PATRÓN** `volumen_spike_ratio` > `1.372` → IC=+0.235 (n=372)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.372 (IC base=-0.001)

- **PATRÓN** `ballena_activa_n` < `160.0` → IC=+0.253 (n=375)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 160.0 (IC base=-0.001)

- **PATRÓN** `dist_vwap_pct` > `0.1348` → IC=+0.203 (n=146)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1348 (IC base=-0.011)

- **PATRÓN** `dist_vwap_pct` < `0.3099` → IC=+0.183 (n=288)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.3099 (IC base=-0.011)

- **PATRÓN** `volumen_regimen` < `1.004` → IC=+0.192 (n=258)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` < 1.004 (IC base=-0.011)

- **PATRÓN** `volumen_regimen` > `0.7292` → IC=+0.186 (n=262)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7898` → IC=+0.235 (n=168)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7898 (IC base=-0.011)

- **PATRÓN** `volumen_spike_ratio` > `1.5154` → IC=+0.252 (n=224)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5154 (IC base=-0.011)

- **PATRÓN** `ballena_activa_n` < `145.0` → IC=+0.250 (n=254)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 145.0 (IC base=-0.011)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **FILTRO** `ibs_20min` < `0.6897` → IC=-0.209 (n=905)

  - _Acción_: SKIP cuando `ibs_20min` < 0.6897
  - _Potencial_: sin este filtro IC_bueno=+0.269 (n=906)
...
- **FILTRO** `sigma_ewma_delta_pct` > `4.618` → IC=-0.177 (n=435)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.618
  - _Potencial_: sin este filtro IC_bueno=+0.059 (n=1458)

- **PATRÓN** `ibs_20min` > `0.6897` → IC=+0.269 (n=906)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6897 (IC base=+0.030)

- **PATRÓN** `dist_vwap_pct` > `0.2821` → IC=+0.324 (n=372)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2821 (IC base=+0.030)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.474` → IC=+0.154 (n=281)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 9.474 (IC base=+0.030)

- **PATRÓN** `volumen_regimen` < `0.8622` → IC=+0.292 (n=427)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8622 (IC base=+0.030)

- **PATRÓN** `volumen_regimen` > `0.636` → IC=+0.287 (n=640)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.636 (IC base=+0.030)

- **PATRÓN** `volumen_pendiente_norm` < `0.105` → IC=+0.285 (n=590)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4475` → IC=+0.318 (n=207)
...
- **PATRÓN** `ibs_20min` < `0.1071` → IC=+0.200 (n=474)
...
- **PATRÓN** `dist_vwap_pct` < `0.189` → IC=+0.198 (n=355)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.189 (IC base=+0.004)

- **PATRÓN** `volumen_regimen` < `0.7154` → IC=+0.249 (n=189)
...
- **PATRÓN** `ibs_20min` < `0.3966` → IC=+0.304 (n=1229)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5342` → IC=+0.264 (n=1065)
...
- **PATRÓN** `ibs_20min` > `0.6944` → IC=+0.234 (n=5958)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6944 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` > `0.1651` → IC=+0.196 (n=2942)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1651 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.206` → IC=+0.252 (n=1375)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.206 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` < `1.2155` → IC=+0.163 (n=4421)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.2155 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` > `0.6254` → IC=+0.161 (n=4420)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.6254 (IC base=+0.169)

- **PATRÓN** `volumen_pendiente_norm` > `0.2464` → IC=+0.195 (n=1344)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2464 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` < `1.5623` → IC=+0.177 (n=2797)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 1.5623 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `2.6489` → IC=+0.177 (n=2119)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 2.6489 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `3860.8432` → IC=+0.172 (n=2223)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 3860.8432 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `121.0` → IC=+0.183 (n=5593)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 121.0 (IC base=+0.169)

- **PATRÓN** `sigma_h` < `0.0065` → IC=+0.184 (n=4255)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0065 (IC base=+0.168)

- **PATRÓN** `drift_60min` |x|≤ `0.0792` → IC=+0.203 (n=2128)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_spike_ratio` < `1.5711` → IC=+0.167 (n=2516)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.5711 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.6382` → IC=+0.175 (n=1906)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.6382 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `122.0` → IC=+0.171 (n=5346)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3311` → IC=+0.207 (n=1138)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.204 (n=563)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.044` → IC=+0.308 (n=518)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.044 (IC base=+0.186)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4288` → IC=+0.183 (n=1040)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` > 1.4288 (IC base=+0.186)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.205 (n=1099)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.186)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.242 (n=714)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0066 (IC base=+0.237)

- **PATRÓN** `sigma_h` > `0.0047` → IC=+0.250 (n=725)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0047 (IC base=+0.237)

- **PATRÓN** `drift_60min` |x|≤ `0.1803` → IC=+0.291 (n=540)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.237 (n=854)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.237)

- **PATRÓN** `libro_liquidez` > `1610.2885` → IC=+0.256 (n=723)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0764` → IC=+0.201 (n=329)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.4289` → IC=+0.232 (n=983)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4289 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.2068` → IC=+0.214 (n=593)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2068 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.501` → IC=+0.224 (n=201)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.501 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.2525` → IC=+0.175 (n=983)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 1.2525 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` < `0.0779` → IC=+0.168 (n=820)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_pendiente_norm` < 0.0779 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.2316` → IC=+0.193 (n=216)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.2316 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.4139` → IC=+0.205 (n=317)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4139 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.4674` → IC=+0.167 (n=316)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 2.4674 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `11693.5655` → IC=+0.183 (n=878)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 11693.5655 (IC base=+0.167)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.158 (n=1091)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0058 (IC base=+0.138)

- **PATRÓN** `drift_60min` |x|≤ `0.293` → IC=+0.157 (n=1091)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5447` → IC=+0.186 (n=1091)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` < 0.5447 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.1331` → IC=+0.162 (n=1096)
...
- **PATRÓN** `volumen_regimen` < `1.2096` → IC=+0.154 (n=1091)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4315` → IC=+0.147 (n=981)
...
- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.208 (n=996)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0065 (IC base=+0.195)

- **PATRÓN** `drift_60min` |x|≤ `0.2005` → IC=+0.214 (n=742)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.290 (n=590)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.195)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.647` → IC=+0.272 (n=345)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.647 (IC base=+0.195)

- **PATRÓN** `volumen_pendiente_norm` < `0.2119` → IC=+0.194 (n=1070)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` < 0.2119 (IC base=+0.195)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2884` → IC=+0.203 (n=698)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2884 (IC base=+0.195)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.208 (n=1279)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.195)

- **PATRÓN** `libro_liquidez` > `1922.1432` → IC=+0.200 (n=371)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1922.1432 (IC base=+0.195)

- **PATRÓN** `sigma_h` < `0.0105` → IC=+0.239 (n=911)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0105 (IC base=+0.224)

- **PATRÓN** `sigma_h` > `0.0057` → IC=+0.224 (n=911)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.3486` → IC=+0.254 (n=910)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3486 (IC base=+0.224)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.718` → IC=+0.278 (n=371)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6428` → IC=+0.221 (n=278)
...
- **PATRÓN** `libro_liquidez` > `1903.9584` → IC=+0.248 (n=304)
...
- **PATRÓN** `sigma_h` < `0.0077` → IC=+0.172 (n=1055)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0077 (IC base=+0.151)

- **PATRÓN** `drift_60min` |x|≤ `0.4373` → IC=+0.165 (n=1056)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.3996` → IC=+0.204 (n=1055)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3996 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` > `0.1419` → IC=+0.190 (n=707)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.1419 (IC base=+0.151)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.01` → IC=+0.247 (n=192)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.01 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` < `1.0503` → IC=+0.157 (n=928)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.0503 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` > `1.1917` → IC=+0.161 (n=352)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4065` → IC=+0.168 (n=344)
...
- **PATRÓN** `libro_liquidez` > `7083.9363` → IC=+0.188 (n=703)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 7083.9363 (IC base=+0.151)

- **PATRÓN** `ballena_activa_n` < `172.0` → IC=+0.151 (n=992)

  - _Acción_: Kelly boost +0.75€ cuando `ballena_activa_n` < 172.0 (IC base=+0.151)

- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.161 (n=1122)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0072 (IC base=+0.127)

- **PATRÓN** `drift_60min` |x|≤ `0.3828` → IC=+0.144 (n=1122)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.3828 (IC base=+0.127)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.187 (n=381)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 18.0 (IC base=+0.127)

- **PATRÓN** `ibs_20min` < `0.6122` → IC=+0.180 (n=1122)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` < 0.6122 (IC base=+0.127)

- **PATRÓN** `dist_vwap_pct` < `0.3372` → IC=+0.142 (n=1212)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.3372 (IC base=+0.127)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.842` → IC=+0.179 (n=400)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 6.842 (IC base=+0.127)

- **PATRÓN** `volumen_regimen` < `0.8554` → IC=+0.144 (n=748)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 0.8554 (IC base=+0.127)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7858` → IC=+0.128 (n=670)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` < 1.7858 (IC base=+0.127)

- **PATRÓN** `volumen_spike_ratio` > `2.4671` → IC=+0.144 (n=335)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 2.4671 (IC base=+0.127)

- **PATRÓN** `libro_liquidez` > `9988.1936` → IC=+0.163 (n=509)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 9988.1936 (IC base=+0.127)

- **PATRÓN** `ballena_activa_n` < `173.0` → IC=+0.126 (n=934)

  - _Acción_: Kelly boost +0.63€ cuando `ballena_activa_n` < 173.0 (IC base=+0.127)

### GBM_LATE_15M_MULTIHORIZONTE#SOL#15min
- **PATRÓN** `sigma_h` > `0.01` → IC=+0.165 (n=556)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.01 (IC base=+0.117)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.133 (n=1258)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.117)

- **PATRÓN** `ibs_20min` > `0.5208` → IC=+0.203 (n=1224)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5208 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` > `1.0691` → IC=+0.218 (n=278)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0691 (IC base=+0.117)
...
- **PATRÓN** `volumen_regimen` < `1.2243` → IC=+0.126 (n=1224)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` < 1.2243 (IC base=+0.117)

- **PATRÓN** `volumen_pendiente_norm` < `0.1685` → IC=+0.130 (n=1221)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_pendiente_norm` < 0.1685 (IC base=+0.117)

- **PATRÓN** `volumen_pendiente_norm` > `0.0993` → IC=+0.123 (n=462)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_pendiente_norm` > 0.0993 (IC base=+0.117)

- **PATRÓN** `volumen_spike_ratio` < `1.8138` → IC=+0.133 (n=785)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` < 1.8138 (IC base=+0.117)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.127 (n=1271)

  - _Acción_: Kelly boost +0.63€ cuando `libro_spread` < 0.02 (IC base=+0.117)

- **PATRÓN** `libro_liquidez` > `2907.8423` → IC=+0.189 (n=555)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 2907.8423 (IC base=+0.117)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.135 (n=904)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1` → IC=+0.150 (n=410)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.1 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.173 (n=567)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 15.0 (IC base=+0.110)

- **PATRÓN** `ibs_20min` < `0.5625` → IC=+0.205 (n=1229)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5625 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` > `0.9571` → IC=+0.146 (n=162)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.9571 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.189` → IC=+0.133 (n=1099)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.189 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.375` → IC=+0.150 (n=261)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 7.375 (IC base=+0.110)

- **PATRÓN** `volumen_regimen` < `1.0417` → IC=+0.121 (n=1081)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` < 1.0417 (IC base=+0.110)
...
- **PATRÓN** `libro_liquidez` > `3078.551` → IC=+0.160 (n=410)
  ...
  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 3078.551 (IC base=+0.110)

### GBM_LATE_15M_MULTIHORIZONTE#XRP#15min
- **PATRÓN** `sigma_h` > `0.0184` → IC=+0.207 (n=773)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0184 (IC base=+0.199)

- **PATRÓN** `drift_60min` |x|≤ `0.1691` → IC=+0.210 (n=511)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.201 (n=526)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.199)

- **PATRÓN** `ibs_20min` > `0.7277` → IC=+0.256 (n=1036)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7277 (IC base=+0.199)

- **PATRÓN** `dist_vwap_pct` > `1.2344` → IC=+0.224 (n=281)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2344 (IC base=+0.199)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.371` → IC=+0.244 (n=552)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.371 (IC base=+0.199)

- **PATRÓN** `volumen_regimen` < `1.2094` → IC=+0.204 (n=1160)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2094 (IC base=+0.199)

- **PATRÓN** `volumen_regimen` > `0.6145` → IC=+0.207 (n=1161)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1696` → IC=+0.214 (n=984)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1696 (IC base=+0.199)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.201 (n=1199)
...
- **PATRÓN** `ibs_20min` < `0.6399` → IC=+0.141 (n=1688)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` < 0.6399 (IC base=+0.109)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2358` → IC=+0.128 (n=1425)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` < 2.2358 (IC base=+0.109)
...
- **FILTRO** `dist_vwap_pct` > `0.3265` → IC=-0.132 (n=36)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.099 (n=419)
...
- **PATRÓN** `ibs_20min` < `0.5` → IC=+0.147 (n=344)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` < 0.5 (IC base=+0.080)

- **PATRÓN** `volumen_regimen` < `0.7139` → IC=+0.134 (n=151)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 0.7139 (IC base=+0.080)

- **PATRÓN** `volumen_spike_ratio` < `1.604` → IC=+0.183 (n=140)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` < 1.604 (IC base=+0.080)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0109` → IC=+0.200 (n=2821)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.4728` → IC=+0.217 (n=8454)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4728 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.9202` → IC=+0.199 (n=1152)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9202 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.748` → IC=+0.235 (n=3167)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.748 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `0.8825` → IC=+0.165 (n=3779)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6231` → IC=+0.189 (n=2692)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 2.6231 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `3810.063` → IC=+0.170 (n=2818)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 3810.063 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `92.0` → IC=+0.194 (n=6255)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 92.0 (IC base=+0.167)

- **PATRÓN** `sigma_h` < `0.0069` → IC=+0.193 (n=5146)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0069 (IC base=+0.180)

- **PATRÓN** `drift_60min` |x|≤ `0.144` → IC=+0.186 (n=3395)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.144 (IC base=+0.180)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.203 (n=2971)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.180)

- **PATRÓN** `ibs_20min` < `0.5636` → IC=+0.238 (n=7715)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5636 (IC base=+0.180)

- **PATRÓN** `dist_vwap_pct` < `0.2349` → IC=+0.161 (n=4792)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2349 (IC base=+0.180)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.684` → IC=+0.181 (n=7474)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.684 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` < `0.7049` → IC=+0.155 (n=2351)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.7049 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` > `1.2035` → IC=+0.153 (n=1782)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.2035 (IC base=+0.180)

- **PATRÓN** `volumen_pendiente_norm` > `0.2878` → IC=+0.244 (n=1005)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2878 (IC base=+0.180)

- **PATRÓN** `volumen_spike_ratio` > `2.2839` → IC=+0.188 (n=3163)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 2.2839 (IC base=+0.180)

- **PATRÓN** `ballena_activa_n` < `25.0` → IC=+0.193 (n=2249)
...
- **PATRÓN** `sigma_h` > `0.0064` → IC=+0.211 (n=959)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0064 (IC base=+0.190)

- **PATRÓN** `drift_60min` |x|≤ `0.3342` → IC=+0.191 (n=1435)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.96€ cuando `drift_60min` |x|≤ 0.3342 (IC base=+0.190)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.199 (n=700)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.190)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.205 (n=548)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` > `0.0044` → IC=+0.264 (n=1106)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0044 (IC base=+0.254)

- **PATRÓN** `drift_60min` |x|≤ `0.205` → IC=+0.282 (n=736)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.3469` → IC=+0.286 (n=971)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3469 (IC base=+0.254)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.424` → IC=+0.263 (n=1164)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8722` → IC=+0.274 (n=666)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8722 (IC base=+0.254)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.254 (n=1166)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.254)

- **PATRÓN** `libro_liquidez` > `1608.0796` → IC=+0.267 (n=986)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1608.0796 (IC base=+0.254)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0842` → IC=+0.166 (n=450)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.0842 (IC base=+0.150)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.163 (n=1404)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.150)

- **PATRÓN** `ibs_20min` > `0.3149` → IC=+0.203 (n=1350)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3149 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.1246` → IC=+0.184 (n=770)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.1246 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.757` → IC=+0.161 (n=314)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 9.757 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.279` → IC=+0.154 (n=1203)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 4.279 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` < `0.6973` → IC=+0.181 (n=594)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.6973 (IC base=+0.150)

- **PATRÓN** `volumen_pendiente_norm` < `0.0736` → IC=+0.158 (n=1177)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1213` → IC=+0.158 (n=1140)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.1213 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.7597` → IC=+0.160 (n=863)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 1.7597 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `11068.9484` → IC=+0.168 (n=1206)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 11068.9484 (IC base=+0.150)

- **PATRÓN** `ballena_activa_n` < `485.0` → IC=+0.161 (n=1225)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 485.0 (IC base=+0.150)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.161 (n=1191)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0058 (IC base=+0.148)

- **PATRÓN** `drift_60min` |x|≤ `0.2576` → IC=+0.164 (n=1048)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6389` → IC=+0.196 (n=1191)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` < 0.6389 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` > `0.6982` → IC=+0.149 (n=192)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.6982 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` < `0.1288` → IC=+0.163 (n=1079)
...
- **PATRÓN** `volumen_regimen` < `1.1917` → IC=+0.157 (n=1191)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.1917 (IC base=+0.148)

- **PATRÓN** `volumen_pendiente_norm` > `0.1504` → IC=+0.195 (n=326)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.1504 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` < `2.4156` → IC=+0.158 (n=1093)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.4156 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` > `1.7499` → IC=+0.155 (n=729)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.7499 (IC base=+0.148)

- **PATRÓN** `ballena_activa_n` < `366.0` → IC=+0.161 (n=664)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 366.0 (IC base=+0.148)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0059` → IC=+0.233 (n=1356)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.220 (n=1370)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.216)

- **PATRÓN** `ibs_20min` > `0.6739` → IC=+0.256 (n=1209)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6739 (IC base=+0.216)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.559` → IC=+0.301 (n=405)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.559 (IC base=+0.216)

- **PATRÓN** `volumen_pendiente_norm` < `0.2136` → IC=+0.222 (n=1318)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2136 (IC base=+0.216)

- **PATRÓN** `volumen_spike_ratio` > `2.9269` → IC=+0.242 (n=579)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9269 (IC base=+0.216)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.230 (n=1566)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.216)

- **PATRÓN** `libro_liquidez` > `1918.8784` → IC=+0.222 (n=451)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1918.8784 (IC base=+0.216)

- **PATRÓN** `sigma_h` < `0.0105` → IC=+0.242 (n=1269)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0105 (IC base=+0.236)

- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.240 (n=847)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.3636` → IC=+0.270 (n=1117)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3636 (IC base=+0.236)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.721` → IC=+0.276 (n=444)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7858` → IC=+0.235 (n=503)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7858 (IC base=+0.236)

- **PATRÓN** `volumen_spike_ratio` > `2.2335` → IC=+0.233 (n=762)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2335 (IC base=+0.236)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.251 (n=745)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.236)

- **PATRÓN** `libro_liquidez` > `1906.6648` → IC=+0.260 (n=423)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1906.6648 (IC base=+0.236)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4384` → IC=+0.140 (n=1438)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.7002` → IC=+0.233 (n=958)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7002 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.3546` → IC=+0.174 (n=563)
...
- **PATRÓN** `volumen_regimen` < `0.8788` → IC=+0.157 (n=959)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.8788 (IC base=+0.137)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5088` → IC=+0.150 (n=609)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.5088 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `2.4447` → IC=+0.153 (n=462)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 2.4447 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `8441.3706` → IC=+0.232 (n=652)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8441.3706 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `85.0` → IC=+0.164 (n=441)

  - _Acción_: Kelly boost +0.82€ cuando `ballena_activa_n` < 85.0 (IC base=+0.137)

- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.159 (n=1169)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0076 (IC base=+0.138)

- **PATRÓN** `drift_60min` |x|≤ `0.4448` → IC=+0.156 (n=1168)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_regimen` < `0.6973` → IC=+0.147 (n=514)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.6973 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `1.1764` → IC=+0.150 (n=389)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 1.1764 (IC base=+0.138)

- **PATRÓN** `volumen_pendiente_norm` > `0.2783` → IC=+0.279 (n=138)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2783 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` > `1.4415` → IC=+0.156 (n=1097)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.4415 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `11047.6745` → IC=+0.208 (n=389)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11047.6745 (IC base=+0.138)

- **PATRÓN** `ballena_activa_n` < `188.0` → IC=+0.148 (n=1090)
...
- **FILTRO** `ibs_20min` > `0.6389` → IC=-0.175 (n=466)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.205 (n=1401)

- **PATRÓN** `sigma_h` > `0.0054` → IC=+0.121 (n=1438)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.60€ cuando `sigma_h` > 0.0054 (IC base=+0.104)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.166 (n=546)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 17.0 (IC base=+0.104)

- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.186 (n=1440)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` > 0.4706 (IC base=+0.104)

- **PATRÓN** `dist_vwap_pct` > `1.0494` → IC=+0.195 (n=277)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 1.0494 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.364` → IC=+0.230 (n=543)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.364 (IC base=+0.104)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.128 (n=995)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.01 (IC base=+0.104)

- **PATRÓN** `libro_liquidez` > `2913.9614` → IC=+0.243 (n=480)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2913.9614 (IC base=+0.104)

- **PATRÓN** `ballena_activa_n` < `53.0` → IC=+0.127 (n=1068)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1276` → IC=+0.161 (n=467)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_spike_ratio` < `1.4615` → IC=+0.143 (n=415)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.4615 (IC base=+0.110)

- **PATRÓN** `volumen_spike_ratio` > `2.2117` → IC=+0.120 (n=564)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_spike_ratio` > 2.2117 (IC base=+0.110)

- **PATRÓN** `libro_liquidez` > `2869.1995` → IC=+0.161 (n=467)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 2869.1995 (IC base=+0.110)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` > `0.0244` → IC=+0.217 (n=655)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.212 (n=1475)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.205)

- **PATRÓN** `libro_liquidez` > `2599.9752` → IC=+0.211 (n=962)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.198 (n=1673)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` < 18.0 (IC base=+0.198)

- **PATRÓN** `ibs_20min` < `0.5203` → IC=+0.251 (n=1585)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5203 (IC base=+0.198)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2192` → IC=+0.192 (n=1237)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` < 2.2192 (IC base=+0.198)

- **PATRÓN** `volumen_spike_ratio` > `1.4428` → IC=+0.197 (n=1404)
...
- **PATRÓN** `libro_liquidez` > `2569.9973` → IC=+0.198 (n=1057)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.132 (n=2671)

- **PATRÓN** `sigma_h` < `0.0113` → IC=+0.152 (n=2613)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` < 0.0113 (IC base=+0.146)

- **PATRÓN** `drift_60min` |x|≤ `0.4051` → IC=+0.153 (n=2299)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.4051 (IC base=+0.146)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.156 (n=881)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 18.0 (IC base=+0.146)

- **PATRÓN** `hora_utc` < `4.0` → IC=+0.166 (n=911)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 4.0 (IC base=+0.146)

- **PATRÓN** `ibs_20min` > `0.9401` → IC=+0.209 (n=871)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9401 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `0.1897` → IC=+0.154 (n=898)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.1897 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.174` → IC=+0.174 (n=430)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 10.174 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` > `0.9039` → IC=+0.157 (n=1075)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 0.9039 (IC base=+0.146)

- **PATRÓN** `volumen_pendiente_norm` > `0.1724` → IC=+0.176 (n=712)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.1724 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` < `1.4583` → IC=+0.159 (n=862)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 1.4583 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` > `1.8986` → IC=+0.158 (n=1721)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.8986 (IC base=+0.146)

- **PATRÓN** `libro_liquidez` > `8307.8924` → IC=+0.153 (n=1184)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 8307.8924 (IC base=+0.146)

- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.188 (n=674)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0038 (IC base=+0.130)

- **PATRÓN** `drift_60min` |x|≤ `0.4847` → IC=+0.149 (n=2015)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.4847 (IC base=+0.130)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.165 (n=756)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 17.0 (IC base=+0.130)

- **PATRÓN** `hora_utc` < `4.0` → IC=+0.156 (n=682)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 4.0 (IC base=+0.130)

- **PATRÓN** `ibs_20min` < `0.1827` → IC=+0.156 (n=887)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` < 0.1827 (IC base=+0.130)

- **PATRÓN** `ibs_20min` > `0.7351` → IC=+0.131 (n=672)

  - _Acción_: Kelly boost +0.65€ cuando `ibs_20min` > 0.7351 (IC base=+0.130)

- **PATRÓN** `dist_vwap_pct` > `0.7057` → IC=+0.134 (n=375)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` > 0.7057 (IC base=+0.130)

- **PATRÓN** `dist_vwap_pct` < `0.2438` → IC=+0.124 (n=1802)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.2438 (IC base=+0.130)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.309` → IC=+0.139 (n=1999)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` < 6.309 (IC base=+0.130)

- **PATRÓN** `volumen_regimen` < `1.2471` → IC=+0.133 (n=1915)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 1.2471 (IC base=+0.130)

- **PATRÓN** `volumen_pendiente_norm` > `0.0718` → IC=+0.147 (n=943)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_pendiente_norm` > 0.0718 (IC base=+0.130)

- **PATRÓN** `volumen_spike_ratio` < `2.5706` → IC=+0.134 (n=1995)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 2.5706 (IC base=+0.130)

- **PATRÓN** `volumen_spike_ratio` > `1.8194` → IC=+0.139 (n=1330)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 1.8194 (IC base=+0.130)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.132 (n=2671)

  - _Acción_: Kelly boost +0.66€ cuando `libro_spread` < 0.01 (IC base=+0.130)

- **PATRÓN** `libro_liquidez` > `7612.3376` → IC=+0.145 (n=1800)

  - _Acción_: Kelly boost +0.72€ cuando `libro_liquidez` > 7612.3376 (IC base=+0.130)

### GBM_LATE_5M#BTC#5min
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.171 (n=287)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0058 (IC base=+0.158)

- **PATRÓN** `sigma_h` > `0.0035` → IC=+0.172 (n=291)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0035 (IC base=+0.158)

- **PATRÓN** `drift_60min` |x|≤ `0.0921` → IC=+0.194 (n=109)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.97€ cuando `drift_60min` |x|≤ 0.0921 (IC base=+0.158)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.164 (n=331)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 5.0 (IC base=+0.158)

- **PATRÓN** `ibs_20min` < `0.5544` → IC=+0.186 (n=218)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` < 0.5544 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` > `0.2286` → IC=+0.184 (n=153)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.2286 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` < `8.282` → IC=+0.163 (n=408)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` < 8.282 (IC base=+0.158)

- **PATRÓN** `volumen_regimen` < `1.2549` → IC=+0.159 (n=326)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.2549 (IC base=+0.158)

- **PATRÓN** `volumen_regimen` > `0.8363` → IC=+0.194 (n=217)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 0.8363 (IC base=+0.158)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4485` → IC=+0.203 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4485 (IC base=+0.158)

- **PATRÓN** `volumen_spike_ratio` > `2.3963` → IC=+0.200 (n=148)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3963 (IC base=+0.158)

- **PATRÓN** `libro_liquidez` > `12645.8095` → IC=+0.203 (n=291)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3661` → IC=+0.141 (n=885)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.3661 (IC base=+0.129)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.170 (n=337)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 17.0 (IC base=+0.129)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.160 (n=324)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 5.0 (IC base=+0.129)

- **PATRÓN** `ibs_20min` < `0.1568` → IC=+0.156 (n=390)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` < 0.1568 (IC base=+0.129)

- **PATRÓN** `ibs_20min` > `0.6152` → IC=+0.140 (n=401)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` > 0.6152 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` > `0.7085` → IC=+0.143 (n=82)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.7085 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` < `0.2266` → IC=+0.130 (n=908)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.2266 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.081` → IC=+0.146 (n=967)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` < 9.081 (IC base=+0.129)

- **PATRÓN** `volumen_regimen` < `0.8847` → IC=+0.172 (n=590)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 0.8847 (IC base=+0.129)

- **PATRÓN** `volumen_pendiente_norm` > `0.0693` → IC=+0.161 (n=417)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` > 0.0693 (IC base=+0.129)

- **PATRÓN** `volumen_spike_ratio` < `2.5736` → IC=+0.137 (n=882)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 2.5736 (IC base=+0.129)

- **PATRÓN** `volumen_spike_ratio` > `1.8194` → IC=+0.141 (n=588)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` > 1.8194 (IC base=+0.129)

- **PATRÓN** `libro_liquidez` > `11358.9394` → IC=+0.144 (n=885)

  - _Acción_: Kelly boost +0.72€ cuando `libro_liquidez` > 11358.9394 (IC base=+0.129)

- **PATRÓN** `ballena_activa_n` < `714.0` → IC=+0.135 (n=840)

  - _Acción_: Kelly boost +0.68€ cuando `ballena_activa_n` < 714.0 (IC base=+0.129)
...
- **PATRÓN** `ibs_20min` < `0.5451` → IC=+0.149 (n=536)

  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` < 0.5451 (IC base=+0.147)
...
- **PATRÓN** `dist_vwap_pct` < `0.4329` → IC=+0.156 (n=746)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.4329 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.739` → IC=+0.155 (n=803)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 6.739 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `1.0972` → IC=+0.150 (n=707)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.0972 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` > `0.7206` → IC=+0.151 (n=718)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.7206 (IC base=+0.147)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5244` → IC=+0.153 (n=788)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.5244 (IC base=+0.147)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.154 (n=779)
...
- **PATRÓN** `ibs_20min` > `0.6065` → IC=+0.208 (n=641)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6065 (IC base=+0.107)

- **PATRÓN** `dist_vwap_pct` > `0.1379` → IC=+0.174 (n=357)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.1379 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.154` → IC=+0.198 (n=279)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` > 7.154 (IC base=+0.107)

- **PATRÓN** `volumen_regimen` < `0.802` → IC=+0.135 (n=428)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 0.802 (IC base=+0.107)

- **PATRÓN** `volumen_regimen` > `0.9761` → IC=+0.131 (n=291)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_regimen` > 0.9761 (IC base=+0.107)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1124` → IC=+0.168 (n=468)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 2.1124 (IC base=+0.107)

- **PATRÓN** `volumen_spike_ratio` > `1.4022` → IC=+0.139 (n=532)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 1.4022 (IC base=+0.107)
...
- **PATRÓN** `libro_liquidez` > `2468.387` → IC=+0.149 (n=280)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 2468.387 (IC base=+0.107)
...
- **PATRÓN** `py_entrada` > `0.5` → IC=+0.130 (n=544)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.333 (n=16)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.092 (n=128)

- **FILTRO** `libro_liquidez` < `10902.4411` → IC=-0.173 (n=108)

  - _Acción_: SKIP cuando `libro_liquidez` < 10902.4411
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.023 (n=1406)
...
- **FILTRO** `py_entrada` < `0.44` → IC=-0.143 (n=208)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=441)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=6941)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.177 (n=2882)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=8954)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.174 (n=2988)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=9265)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.44` → IC=-0.213 (n=497)

  - _Acción_: SKIP cuando `py_entrada` < 0.44
  - _Potencial_: sin este filtro IC_bueno=+0.098 (n=1522)
...
- **PATRÓN** `py_entrada` > `0.5` → IC=+0.121 (n=1208)

  - _Acción_: Kelly boost +0.61€ cuando `py_entrada` > 0.5 (IC base=+0.021)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.44` → IC=-0.209 (n=506)

  - _Acción_: SKIP cuando `py_entrada` < 0.44
  - _Potencial_: sin este filtro IC_bueno=+0.098 (n=1554)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.222 (n=509)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.061 (n=1660)

- **FILTRO** `ibs_20min` > `0.2857` → IC=-0.167 (n=530)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2857
  - _Potencial_: sin este filtro IC_bueno=+0.047 (n=1639)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.47` → IC=-0.198 (n=471)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.078 (n=1524)

- **FILTRO** `py_entrada` > `0.58` → IC=-0.191 (n=523)

  - _Acción_: SKIP cuando `py_entrada` > 0.58
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=1648)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=2655)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=669)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.127 (n=8383)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.082 (n=18996)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.280 (n=6538)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=20841)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.240 (n=6839)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=20540)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.161 (n=9081)

  - _Acción_: SKIP cuando `ballena_activa_n` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=18298)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.230 (n=8108)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=26324)

- **FILTRO** `ibs_7min` > `0.2957` → IC=-0.177 (n=8606)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2957
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=25826)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.311 (n=1059)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=3386)

- **FILTRO** `ibs_7min` < `0.7089` → IC=-0.257 (n=1465)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7089
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=2980)

- **FILTRO** `ballena_activa_n` > `9.0` → IC=-0.199 (n=998)

  - _Acción_: SKIP cuando `ballena_activa_n` > 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=3447)

- **FILTRO** `py_entrada` > `0.505` → IC=-0.149 (n=4019)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.103 (n=1951)

- **FILTRO** `drift_7min_pct` |x|> `0.1117` → IC=-0.125 (n=2023)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1117
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=3947)

- **FILTRO** `ibs_7min` > `0.7987` → IC=-0.205 (n=1492)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7987
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=4478)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.135 (n=1108)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=3672)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.256 (n=1139)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=3641)

- **FILTRO** `ibs_7min` < `0.7545` → IC=-0.184 (n=1195)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7545
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=3585)

- **FILTRO** `ballena_activa_n` > `162.0` → IC=-0.176 (n=1191)

  - _Acción_: SKIP cuando `ballena_activa_n` > 162.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3589)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.254 (n=1176)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=3667)

- **FILTRO** `ibs_7min` > `0.257` → IC=-0.170 (n=1210)

  - _Acción_: SKIP cuando `ibs_7min` > 0.257
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=3633)

- **FILTRO** `ballena_activa_n` > `153.0` → IC=-0.181 (n=1210)

  - _Acción_: SKIP cuando `ballena_activa_n` > 153.0
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=3633)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.173 (n=1022)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.091 (n=3145)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.313 (n=1004)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=3163)

- **FILTRO** `ibs_7min` < `0.7101` → IC=-0.245 (n=1374)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7101
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=2793)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.224 (n=950)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.077 (n=3217)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.235 (n=1481)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=4821)

- **FILTRO** `ibs_7min` > `0.7633` → IC=-0.172 (n=1575)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7633
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=4727)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=1437)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.090 (n=3079)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.239 (n=1332)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=3184)

- **FILTRO** `ibs_7min` < `0.742` → IC=-0.188 (n=1129)

  - _Acción_: SKIP cuando `ibs_7min` < 0.742
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3387)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.181 (n=1116)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.076 (n=3400)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.260 (n=1155)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=3480)

- **FILTRO** `ibs_7min` > `0.2744` → IC=-0.175 (n=1156)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2744
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=3479)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.184 (n=1136)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=3499)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.37` → IC=-0.259 (n=1202)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=3665)

- **FILTRO** `ibs_7min` < `0.7143` → IC=-0.228 (n=1213)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=3654)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.174 (n=1603)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=4917)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.34` → IC=-0.282 (n=1054)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=3550)

- **FILTRO** `ibs_7min` < `0.7167` → IC=-0.232 (n=1149)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7167
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=3455)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.211 (n=1118)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3486)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.206 (n=1456)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=+0.009 (n=4706)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=2539)

### STRUCT_NO_15M#SOL#15min
- **FILTRO** `py_entrada` < `0.495` → IC=-0.147 (n=32)

  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=1317)
...
### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` < `0.0042` → IC=+0.191 (n=416)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0042 (IC base=+0.188)

- **PATRÓN** `sigma_h` > `0.0079` → IC=+0.227 (n=566)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0079 (IC base=+0.188)

- **PATRÓN** `drift_60min` |x|≤ `0.1642` → IC=+0.191 (n=1099)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.96€ cuando `drift_60min` |x|≤ 0.1642 (IC base=+0.188)

- **PATRÓN** `delta_ratio_macro` |x|> `0.057` → IC=+0.188 (n=1248)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.94€ cuando `delta_ratio_macro` |x|> 0.057 (IC base=+0.188)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1344` → IC=+0.244 (n=408)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1344 (IC base=+0.188)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.194 (n=886)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` > 11.0 (IC base=+0.188)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.202 (n=591)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.188)

- **PATRÓN** `ibs_15` > `0.6192` → IC=+0.264 (n=1248)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6192 (IC base=+0.188)

- **PATRÓN** `dist_vwap_pct` > `0.4417` → IC=+0.184 (n=302)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.4417 (IC base=+0.188)

- **PATRÓN** `dist_vwap_pct` < `0.5648` → IC=+0.180 (n=1260)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.5648 (IC base=+0.188)
...
- **PATRÓN** `libro_liquidez` > `9175.0527` → IC=+0.196 (n=416)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 9175.0527 (IC base=+0.188)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0585` → IC=+0.279 (n=102)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0585 (IC base=+0.209)

- **PATRÓN** `drift_15min` |x|≤ `0.3787` → IC=+0.211 (n=102)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.3787 (IC base=+0.209)

- **PATRÓN** `delta_ratio_macro` |x|> `0.25` → IC=+0.211 (n=102)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.25 (IC base=+0.209)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2203` → IC=+0.271 (n=155)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.2203 (IC base=+0.209)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.257 (n=142)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.209)

- **PATRÓN** `ibs_15` > `0.7103` → IC=+0.266 (n=306)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7103 (IC base=+0.209)

- **PATRÓN** `dist_vwap_pct` > `0.4105` → IC=+0.282 (n=85)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4105 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.575` → IC=+0.257 (n=179)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.575 (IC base=+0.209)

- **PATRÓN** `libro_liquidez` > `15869.2533` → IC=+0.269 (n=102)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 15869.2533 (IC base=+0.209)
...
- **FILTRO** `dist_vwap_pct` > `0.5583` → IC=-0.136 (n=119)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.065 (n=633)
...
- **PATRÓN** `sigma_h` > `0.0164` → IC=+0.233 (n=230)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0164 (IC base=+0.186)

- **PATRÓN** `drift_60min` |x|≤ `0.0851` → IC=+0.208 (n=152)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0851 (IC base=+0.186)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0626` → IC=+0.199 (n=310)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.99€ cuando `delta_ratio_macro` |x|> 0.0626 (IC base=+0.186)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1008` → IC=+0.291 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1008 (IC base=+0.186)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.237 (n=169)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.186)

- **PATRÓN** `ibs_15` > `0.5455` → IC=+0.281 (n=345)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **PATRÓN** `dist_vwap_pct` < `0.6134` → IC=+0.195 (n=404)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` < 0.6134 (IC base=+0.186)

- **PATRÓN** `sigma_ewma_delta_pct` > `15.726` → IC=+0.235 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 15.726 (IC base=+0.186)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.186 (n=384)

  - _Acción_: Kelly boost +0.93€ cuando `libro_spread` < 0.03 (IC base=+0.186)

- **PATRÓN** `libro_liquidez` > `2863.5754` → IC=+0.261 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2863.5754 (IC base=+0.186)
...
- **FILTRO** `sigma_h` > `0.0129` → IC=-0.210 (n=570)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0129
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=1713)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.180 (n=730)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=1553)

- **FILTRO** `libro_liquidez` < `3852.5505` → IC=-0.125 (n=1506)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1166` → IC=+0.239 (n=879)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1166 (IC base=-0.043)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1818` → IC=+0.244 (n=845)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1818 (IC base=-0.043)

- **PATRÓN** `ibs_15` < `0.3605` → IC=+0.283 (n=1318)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **FILTRO** `sigma_h` > `0.007` → IC=-0.215 (n=345)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.196 (n=1039)

- **FILTRO** `sigma_h` < `0.0037` → IC=-0.229 (n=456)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0037
  - _Potencial_: sin este filtro IC_bueno=-0.187 (n=928)

- **FILTRO** `hora_utc` > `7.0` → IC=-0.209 (n=880)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.188 (n=504)

- **FILTRO** `sigma_ewma_delta_pct` > `19.895` → IC=-0.243 (n=247)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 19.895
  - _Potencial_: sin este filtro IC_bueno=-0.192 (n=1137)

- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.169 (n=125)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0028 (IC base=+0.084)
...
- **PATRÓN** `dist_vwap_pct` > `0.1073` → IC=+0.286 (n=96)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1073 (IC base=+0.084)
...
- **PATRÓN** `sigma_h` < `0.0075` → IC=+0.247 (n=544)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0075 (IC base=+0.230)

- **PATRÓN** `drift_15min` |x|≤ `0.774` → IC=+0.242 (n=478)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.774 (IC base=+0.230)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1983` → IC=+0.255 (n=247)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1983 (IC base=+0.230)

- **PATRÓN** `hora_utc` > `19.0` → IC=+0.243 (n=181)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 19.0 (IC base=+0.230)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.241 (n=365)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 12.0 (IC base=+0.230)

- **PATRÓN** `ibs_15` < `0.273` → IC=+0.292 (n=478)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.273 (IC base=+0.230)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `20.179` → IC=+0.258 (n=60)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 20.179 (IC base=+0.230)

- **PATRÓN** `sigma_ewma_delta_pct` < `12.251` → IC=+0.232 (n=583)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 12.251 (IC base=+0.230)

- **PATRÓN** `libro_liquidez` > `3551.8492` → IC=+0.230 (n=543)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3551.8492 (IC base=+0.230)

- **PATRÓN** `ballena_activa_n` < `157.0` → IC=+0.233 (n=514)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 157.0 (IC base=+0.230)
...
- **FILTRO** `sigma_ewma_delta_pct` > `18.09` → IC=-0.139 (n=286)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=2283)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1148` → IC=+0.224 (n=172)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1148 (IC base=-0.046)

- **PATRÓN** `ibs_15` < `0.3667` → IC=+0.255 (n=259)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **PATRÓN** `dist_vwap_pct` < `0.1869` → IC=+0.207 (n=223)
...
- **FILTRO** `drift_15min` |x|> `1.198` → IC=-0.240 (n=171)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 1.198
  - _Potencial_: sin este filtro IC_bueno=-0.169 (n=514)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1093` → IC=+0.348 (n=163)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1093 (IC base=-0.046)

- **PATRÓN** `ibs_15` < `0.3519` → IC=+0.310 (n=377)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
### UPDOWN_GBM_IBS_ALTO
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.294 (n=571)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0061 (IC base=+0.294)

- **PATRÓN** `sigma_h` > `0.0054` → IC=+0.301 (n=259)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1425` → IC=+0.296 (n=380)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1425 (IC base=+0.294)
...
- **PATRÓN** `ibs_15` > `0.838` → IC=+0.330 (n=570)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.296 (n=699)
...
- **PATRÓN** `drift_15min` |x|≤ `0.3892` → IC=+0.291 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.3892 (IC base=+0.286)

- **PATRÓN** `delta_ratio_macro` |x|> `0.252` → IC=+0.289 (n=107)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.252 (IC base=+0.286)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1424` → IC=+0.325 (n=112)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1424 (IC base=+0.286)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.304 (n=335)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.286)

- **PATRÓN** `ibs_15` > `0.8255` → IC=+0.308 (n=321)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8255 (IC base=+0.286)

- **PATRÓN** `dist_vwap_pct` > `0.2698` → IC=+0.350 (n=138)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2698 (IC base=+0.286)

- **PATRÓN** `sigma_ewma_delta_pct` > `23.789` → IC=+0.353 (n=73)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 23.789 (IC base=+0.286)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6192 sube el IC de +0.188 a +0.264 en UPDOWN_GBM#15min (n=1248). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.7103 sube el IC de +0.209 a +0.266 en UPDOWN_GBM#BTC#15min (n=306). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_YES, IBS > 0.5455 sube el IC de +0.186 a +0.281 en UPDOWN_GBM#XRP#15min (n=345). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.6235 sube el IC de -0.056 a +0.269 en UPDOWN_GBM_15M_TARDIO (n=565). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3605 sube el IC de -0.043 a +0.283 en UPDOWN_GBM_15M_TARDIO (n=1318). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO#BTC#15min**: dentro de BUY_YES, IBS > 0.7496 sube el IC de +0.084 a +0.336 en UPDOWN_GBM_15M_TARDIO#BTC#15min (n=138). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.273 sube el IC de +0.230 a +0.292 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=478). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#SOL#15min**: dentro de BUY_NO, IBS < 0.3667 sube el IC de -0.046 a +0.255 en UPDOWN_GBM_15M_TARDIO#SOL#15min (n=259). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO#XRP#15min**: dentro de BUY_NO, IBS < 0.3519 sube el IC de -0.046 a +0.310 en UPDOWN_GBM_15M_TARDIO#XRP#15min (n=377). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.838 sube el IC de +0.294 a +0.330 en UPDOWN_GBM_IBS_ALTO (n=570). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_IBS_ALTO#BTC#15min**: dentro de BUY_YES, IBS > 0.8255 sube el IC de +0.286 a +0.308 en UPDOWN_GBM_IBS_ALTO#BTC#15min (n=321). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 22111 | -0.094 | -3120.37€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1321 | -0.041 | -192.93€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 20790 | -0.097 | -2927.44€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3409 | -0.092 | -569.96€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3409 | -0.092 | -569.96€ | 1 | 0 |
...
| ✅ BALLENAS_TARDIAS#ETH | 6336 | -0.041 | -614.21€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 6336 | -0.041 | -614.21€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 5846 | -0.093 | -469.42€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 5846 | -0.093 | -469.42€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 4825 | -0.175 | -1112.80€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 4825 | -0.175 | -1112.80€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 14203 | -0.036 | +4185.39€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 3763 | -0.003 | +1837.28€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 10440 | -0.048 | +2348.11€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 14203 | -0.036 | +4185.39€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 3763 | -0.003 | +1837.28€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 10440 | -0.048 | +2348.11€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO | 1277 | -0.096 | -158.68€ | 2 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#15min | 127 | -0.035 | -10.33€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#5min | 1150 | -0.102 | -148.35€ | 0 | 0 |
...
| ✅ CANDIDATA9_BOT_CONSENSO#ETH | 367 | -0.126 | -62.87€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#ETH#15min | 24 | -0.077 | -5.22€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#ETH#5min | 343 | -0.129 | -57.65€ | 3 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#SOL | 119 | -0.045 | -14.09€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#SOL#5min | 119 | -0.045 | -14.09€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#XRP | 48 | -0.140 | -1.50€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#XRP#5min | 48 | -0.140 | -1.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 80441 | +0.113 | -4107.08€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 12436 | +0.185 | -341.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 311 | -0.113 | -46.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 62257 | +0.100 | -3540.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 5437 | +0.111 | -178.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 10367 | +0.096 | -949.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 41 | -0.151 | -1.09€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 10311 | +0.098 | -936.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 16270 | +0.132 | -303.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 3845 | +0.203 | -99.90€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 10316 | +0.111 | -172.91€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 2067 | +0.111 | -8.09€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 10406 | +0.088 | -1011.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 48 | -0.060 | -2.83€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#DOGE#240min | 15 | -0.243 | -11.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 10343 | +0.090 | -997.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 17192 | +0.124 | -335.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 4807 | +0.175 | -59.15€ | 1 | 7 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 10417 | +0.106 | -215.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1956 | +0.098 | -52.41€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 15821 | +0.116 | -907.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 3653 | +0.188 | -187.76€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 214 | -0.074 | +7.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 10540 | +0.093 | -609.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1414 | +0.129 | -117.78€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#XRP | 10385 | +0.103 | -600.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 42 | -0.023 | +9.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 13 | -0.022 | -0.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 10330 | +0.103 | -609.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 12723 | +0.191 | -835.12€ | 1 | 5 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 12723 | +0.191 | -835.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 3097 | +0.168 | -331.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 3097 | +0.168 | -331.34€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC | 823 | +0.197 | +10.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC#15min | 823 | +0.197 | +10.38€ | 4 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 3028 | +0.181 | -259.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 3028 | +0.181 | -259.18€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 2719 | +0.239 | -79.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 2719 | +0.239 | -79.31€ | 0 | 4 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 2977 | +0.191 | -189.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 2977 | +0.191 | -189.43€ | 0 | 4 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 43592 | +0.196 | -3536.79€ | 2 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min |