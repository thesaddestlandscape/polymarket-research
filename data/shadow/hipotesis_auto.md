# Hipótesis automáticas — 2026-07-03 09:08 UTC
_Generado por shadow_postmortem.py sobre 3012 resoluciones (PNL=-45.78€)_

## Patrones causales activos

### UPDOWN_GBM#15min
- **FILTRO** `ibs_15` > `0.3451` → IC=-0.128 (n=41)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: SKIP cuando `ibs_15` > 0.3451
  - _Potencial_: sin este filtro IC_bueno=+0.047 (n=126)

- **PATRÓN** `sigma_h` < `0.0098` → IC=+0.212 (n=78)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0098 (IC base=+0.038)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.122 (n=43)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 15.0 (IC base=+0.038)

- **PATRÓN** `ibs_15` < `0.0563` → IC=+0.155 (n=56)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.78€ cuando `ibs_15` < 0.0563 (IC base=+0.038)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1297` → IC=+0.150 (n=58)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.75€ cuando `pct_spot_vs_ref` |x|≤ 0.1297 (IC base=+0.115)

- **PATRÓN** `sigma_h` < `0.0144` → IC=+0.150 (n=58)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0144 (IC base=+0.115)

- **PATRÓN** `drift_60min` |x|≤ `1.067` → IC=+0.125 (n=54)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 1.067 (IC base=+0.115)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.141 (n=37)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` < 12.0 (IC base=+0.115)

- **PATRÓN** `ibs_15` < `0.0787` → IC=+0.281 (n=30)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0787 (IC base=+0.115)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS < 0.0563 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.155 n=56). Confirma señal de reversión media → alinear con BUY_YES.
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.0787 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.281 n=30). Confirma señal de reversión media → alinear con BUY_YES.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 359 | +0.051 | +48.65€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 359 | +0.051 | +48.65€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 94 | +0.010 | +3.38€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 94 | +0.010 | +3.38€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 92 | -0.053 | -3.85€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 92 | -0.053 | -3.85€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL | 89 | +0.115 | +20.93€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 89 | +0.115 | +20.93€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP | 84 | +0.140 | +28.19€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 84 | +0.140 | +28.19€ | 0 | 0 |
| ✅ ORDER_FLOW_5M | 1523 | +0.013 | +11.27€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#5min | 1387 | +0.008 | -1.33€ | 0 | 0 |
...
| ✅ ORDER_FLOW_5M#SOL | 297 | +0.042 | +10.39€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#SOL#5min | 297 | +0.042 | +10.39€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 856 | -0.041 | -49.64€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 626 | -0.019 | -21.79€ | 1 | 3 |
...
| ✅ UPDOWN_GBM#ETH | 335 | -0.010 | +8.54€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 251 | +0.006 | +15.00€ | 3 | 5 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.064) — sin ventaja clara. oversold(IBS<0.3): IC=-0.006 n=160 | neutral: IC=-0.070 n=140 | overbought(IBS>0.7): IC=-0.046 n=150
  - _Datos_: n=466 IC=-0.041 PNL=-24.40€
...
**🟡 H-OF-PAR** — ORDER_FLOW per-pair delta_ratio ranges
  ...
  - _Estado_: BTC: 157 ops con delta_ratio | SOL: 196 ops con delta_ratio
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.008 n=1442 | tras_1loss IC=-0.029 n=1479 | tras_2loss IC=-0.048 n=763/40 | gap=+0.055 (umbral 0.05)
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=-0.038 n=117/40 PNL=+0.58€ | H=15h UTC: IC=+0.017 n=116/40 PNL=+5.20€ | H=17h UTC: IC=+0.200 n=58/40 PNL=+21.47€ | H=19h UTC: IC=-0.013 n=72/40 PNL=-9.58€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=733 IC=-0.036 PNL=-36.08€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=733 IC=-0.036 PNL=-36.08€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=268 IC=-0.018 PNL=-34.23€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=268 IC=-0.018 PNL=-34.23€
...
**⏳ H-CUSTOM-GBM-09H** — GBM a las 09h UTC — bloqueada 2026-06-29
  ...
  - _Estado_: 17/25 ops en el filtro definido (IC actual=-0.157 PNL=-9.46€)
  - _Datos_: n=17 IC=-0.157 PNL=-9.46€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=216 IC=+0.014 PNL=+13.51€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=216 IC=+0.014 PNL=+13.51€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=586 IC=+0.032 PNL=+44.38€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=586 IC=+0.032 PNL=+44.38€
