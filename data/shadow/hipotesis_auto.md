# Hipótesis automáticas — 2026-07-12 16:19 UTC
_Generado por shadow_postmortem.py sobre 11141 resoluciones (PNL=+2062.92€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.214 (n=320)
...
- **PATRÓN** `py_entrada` > `0.585` → IC=+0.248 (n=359)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.585 (IC base=+0.214)

- **PATRÓN** `libro_liquidez` > `2419.4045` → IC=+0.223 (n=345)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2419.4045 (IC base=+0.214)

- **PATRÓN** `hora_utc` < `9.0` → IC=+0.216 (n=167)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 9.0 (IC base=+0.144)

- **PATRÓN** `py_entrada` < `0.275` → IC=+0.304 (n=131)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.275 (IC base=+0.144)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.151 (n=419)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.01 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `5937.8835` → IC=+0.159 (n=168)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 5937.8835 (IC base=+0.144)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.275 (n=78)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `4211.4102` → IC=+0.308 (n=50)
...
- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.149 (n=931)
...
- **PATRÓN** `dist_vwap_pct` < `0.1648` → IC=+0.189 (n=88)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.1648 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.069` → IC=+0.210 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.069 (IC base=+0.110)
...
- **PATRÓN** `dist_vwap_pct` > `0.0526` → IC=+0.147 (n=15)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.0526 (IC base=+0.053)
...
- **PATRÓN** `sigma_h` > `0.0043` → IC=+0.173 (n=374)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0043 (IC base=+0.150)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.187 (n=148)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` < 6.0 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` < `0.2388` → IC=+0.259 (n=106)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2388 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.075` → IC=+0.262 (n=40)
...
- **PATRÓN** `sigma_h` > `0.0025` → IC=+0.226 (n=93)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0025 (IC base=+0.159)
...
- **PATRÓN** `dist_vwap_pct` < `0.039` → IC=+0.180 (n=23)
  ...
  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.039 (IC base=+0.159)
...
- **PATRÓN** `sigma_h` > `0.005` → IC=+0.151 (n=61)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.005 (IC base=+0.093)
...
- **PATRÓN** `dist_vwap_pct` < `0.2863` → IC=+0.147 (n=15)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.2863 (IC base=+0.093)
...
- **PATRÓN** `sigma_h` > `0.0124` → IC=+0.148 (n=143)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0124 (IC base=+0.079)

- **PATRÓN** `dist_vwap_pct` < `0.2308` → IC=+0.150 (n=101)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.2308 (IC base=+0.079)
...
- **PATRÓN** `sigma_h` < `0.0054` → IC=+0.130 (n=52)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` < 0.0054 (IC base=-0.006)
...
- **PATRÓN** `sigma_h` < `0.005` → IC=+0.136 (n=31)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.005 (IC base=+0.056)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.125 (n=86)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=350)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.126 (n=241)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 13.0 (IC base=+0.020)

- **PATRÓN** `ibs_15` > `0.5859` → IC=+0.124 (n=293)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.62€ cuando `ibs_15` > 0.5859 (IC base=+0.020)

- **PATRÓN** `dist_vwap_pct` > `0.168` → IC=+0.205 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.168 (IC base=+0.020)

- **PATRÓN** `dist_vwap_pct` < `0.3813` → IC=+0.182 (n=105)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.3813 (IC base=+0.020)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2361` → IC=+0.123 (n=136)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 0.2361 (IC base=+0.061)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.172 (n=129)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 11.0 (IC base=+0.061)

- **PATRÓN** `ibs_15` < `0.9235` → IC=+0.161 (n=116)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.81€ cuando `ibs_15` < 0.9235 (IC base=+0.061)

- **PATRÓN** `ibs_15` > `0.6428` → IC=+0.170 (n=116)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.85€ cuando `ibs_15` > 0.6428 (IC base=+0.061)

- **PATRÓN** `dist_vwap_pct` > `0.2179` → IC=+0.346 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2179 (IC base=+0.061)
...
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS < 0.9235 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.161 n=116). Confirma señal de reversión media → alinear con BUY_YES.
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS > 0.6428 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.170 n=116). Confirma señal de reversión media → alinear con BUY_NO.
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 951 | +0.178 | -3.17€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 609 | +0.207 | +1.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 40 | +0.048 | +1.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 143 | +0.086 | -15.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 159 | +0.177 | +9.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 279 | +0.183 | -5.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 203 | +0.202 | -9.40€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 13 | +0.108 | +2.62€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 53 | +0.191 | +5.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 338 | +0.162 | -2.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 202 | +0.201 | +0.64€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 13 | -0.065 | -1.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 70 | +0.083 | -9.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 53 | +0.173 | +7.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL | 332 | +0.189 | +5.22€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 53 | +0.154 | -3.58€ | 0 | 2 |
| ✅ GBM_LATE_15M | 3626 | +0.114 | +1211.95€ | 0 | 9 |
| ✅ GBM_LATE_15M#15min | 3626 | +0.114 | +1211.95€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 892 | +0.069 | +116.23€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 892 | +0.069 | +116.23€ | 0 | 2 |
| ✅ GBM_LATE_15M#ETH | 917 | +0.072 | +155.06€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 917 | +0.072 | +155.06€ | 0 | 2 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR | 984 | +0.163 | +549.27€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 984 | +0.163 | +549.27€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 259 | +0.163 | +168.72€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 259 | +0.163 | +168.72€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 262 | +0.125 | +99.62€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 262 | +0.125 | +99.62€ | 0 | 7 |
...
| ✅ GBM_LATE_15M_TARDIO | 1142 | +0.080 | +280.72€ | 1 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1142 | +0.080 | +280.72€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 266 | +0.075 | +36.37€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 266 | +0.075 | +36.37€ | 0 | 1 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 283 | +0.005 | +9.03€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 283 | +0.005 | +9.03€ | 0 | 2 |
...
| ✅ GBM_LATE_60M | 312 | -0.115 | +4.15€ | 2 | 1 |
| ✅ GBM_LATE_60M#60min | 312 | -0.115 | +4.15€ | 0 | 0 |
| ✅ GBM_LATE_60M#BTC | 107 | -0.032 | +3.11€ | 0 | 0 |
| ✅ GBM_LATE_60M#BTC#60min | 107 | -0.032 | +3.11€ | 3 | 1 |
...
| ✅ UPDOWN_GBM | 1329 | -0.006 | +29.35€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1073 | +0.024 | +66.81€ | 1 | 5 |
...
| ✅ UPDOWN_GBM#BTC | 358 | +0.014 | +11.04€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 279 | +0.055 | +24.42€ | 1 | 8 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.120) — sin ventaja clara. oversold(IBS<0.3): IC=-0.023 n=344 | neutral: IC=-0.044 n=261 | overbought(IBS>0.7): IC=+0.076 n=293
  - _Datos_: n=939 IC=+0.009 PNL=+54.58€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.091 n=6299 | tras_1loss IC=+0.046 n=4704 | tras_2loss IC=+0.012 n=2107/40 | gap=+0.079 (umbral 0.05)
...
**〰️ H-24H-GBM-BUYYES-TARDE** — GBM BUY_YES en tarde europea (15-19h UTC) — señal alcista sostenida
  ...
  - _Estado_: n=210 IC=+0.009 PNL=+12.35€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=210 IC=+0.009 PNL=+12.35€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1202 IC=+0.001 PNL=+43.35€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1202 IC=+0.001 PNL=+43.35€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=611 IC=+0.020 PNL=+31.78€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=611 IC=+0.020 PNL=+31.78€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=279 IC=+0.055 PNL=+24.42€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=279 IC=+0.055 PNL=+24.42€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=554 IC=+0.054 PNL=+49.67€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=554 IC=+0.054 PNL=+49.67€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8683 IC=+0.097 PNL=+2138.80€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8683 IC=+0.097 PNL=+2138.80€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=758 IC=-0.013 PNL=+1.31€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=758 IC=-0.013 PNL=+1.31€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.176 > 0.1 con n=273 PNL=+104.97€
  - _Datos_: n=273 IC=+0.176 PNL=+104.97€
...
**〰️ H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: n=150 IC=+0.079 PNL=+14.99€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=150 IC=+0.079 PNL=+14.99€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=459 IC=+0.053 PNL=+36.82€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=459 IC=+0.053 PNL=+36.82€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.179 > 0.08 con n=207 PNL=+78.02€
  - _Datos_: n=207 IC=+0.179 PNL=+78.02€
...
**🔶 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA en BTC (IC=+0.242 n=87) pero sin cruzar ≥2 pares más — ETH: n=108 IC=+0.145 ✓; SOL: n=7 IC=-0.019; XRP: n=5 IC=+0.018
  - _Datos_: n=87 IC=+0.242 PNL=+40.79€
