# Hipótesis automáticas — 2026-08-28 02:06 UTC
_Generado por shadow_postmortem.py sobre 181944 resoluciones (PNL=+13173.89€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `147.45` → IC=-0.286 (n=2536)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 147.45
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=7611)
...
- **FILTRO** `restante_s_al_confirmar` < `104.76` → IC=-0.408 (n=324)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=974)
...
- **FILTRO** `restante_s_al_confirmar` < `145.96` → IC=-0.293 (n=592)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 145.96
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=1776)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `152.53` → IC=-0.353 (n=583)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 152.53
  - _Potencial_: sin este filtro IC_bueno=-0.090 (n=1185)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.190 (n=5625)
...
- **PATRÓN** `libro_liquidez` > `2375.6867` → IC=+0.173 (n=1565)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.156 (n=4441)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.188 (n=3028)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.02 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `1890.4124` → IC=+0.179 (n=2536)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 1890.4124 (IC base=+0.141)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.187 (n=666)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.182 (n=854)

  - _Acción_: Kelly boost +0.91€ cuando `libro_spread` < 0.01 (IC base=+0.182)

- **PATRÓN** `libro_liquidez` > `12606.5201` → IC=+0.217 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12606.5201 (IC base=+0.182)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.278 (n=489)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `2932.4561` → IC=+0.286 (n=311)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.195 (n=653)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.231 (n=581)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.223)

- **PATRÓN** `libro_liquidez` > `1890.2694` → IC=+0.238 (n=288)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1890.2694 (IC base=+0.223)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.191 (n=9105)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` < 12.0 (IC base=+0.191)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.220 (n=9264)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.130 (n=1675)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.168 (n=1556)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.228 (n=987)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.160 (n=1130)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.138 (n=1154)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.305 (n=612)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `3861.5788` → IC=+0.307 (n=268)
...
- **PATRÓN** `libro_liquidez` > `2084.3627` → IC=+0.430 (n=226)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2084.3627 (IC base=+0.414)
...
- **PATRÓN** `ibs_20min` < `0.2156` → IC=+0.131 (n=1604)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.2156 (IC base=+0.029)

- **PATRÓN** `dist_vwap_pct` < `0.2997` → IC=+0.142 (n=809)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.2997 (IC base=+0.029)

- **PATRÓN** `volumen_regimen` < `0.6267` → IC=+0.158 (n=261)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.6267 (IC base=+0.029)

- **PATRÓN** `volumen_regimen` > `1.0483` → IC=+0.141 (n=355)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 1.0483 (IC base=+0.029)

- **PATRÓN** `volumen_pendiente_norm` > `0.3033` → IC=+0.274 (n=135)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3033 (IC base=+0.029)

- **PATRÓN** `volumen_spike_ratio` < `1.9478` → IC=+0.185 (n=602)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 1.9478 (IC base=+0.029)

- **PATRÓN** `volumen_spike_ratio` > `2.9099` → IC=+0.233 (n=301)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9099 (IC base=+0.029)

- **PATRÓN** `ballena_activa_n` < `87.0` → IC=+0.220 (n=420)
...
- **PATRÓN** `sigma_h` > `0.0057` → IC=+0.139 (n=153)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0057 (IC base=+0.138)
...
- **PATRÓN** `ibs_20min` < `0.44` → IC=+0.173 (n=402)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.44 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.1366` → IC=+0.163 (n=381)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.1366 (IC base=+0.138)
...
- **PATRÓN** `volumen_regimen` > `1.0216` → IC=+0.143 (n=208)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4105` → IC=+0.157 (n=354)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.4105 (IC base=+0.138)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.139 (n=590)

  - _Acción_: Kelly boost +0.69€ cuando `libro_spread` < 0.01 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `13590.0128` → IC=+0.171 (n=208)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 13590.0128 (IC base=+0.138)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.284 (n=137)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.259)

- **PATRÓN** `ibs_20min` < `0.5322` → IC=+0.298 (n=354)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.576` → IC=+0.266 (n=387)
...
- **PATRÓN** `volumen_spike_ratio` < `3.4834` → IC=+0.228 (n=204)
...
- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.237 (n=154)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0976` → IC=+0.267 (n=28)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0976 (IC base=-0.036)
...
- **FILTRO** `sigma_ewma_delta_pct` > `4.968` → IC=-0.156 (n=283)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=1057)
...
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.139 (n=1352)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.0952` → IC=+0.185 (n=1245)
...
- **PATRÓN** `dist_vwap_pct` < `0.2116` → IC=+0.205 (n=751)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2116 (IC base=+0.039)

- **PATRÓN** `volumen_regimen` > `1.2365` → IC=+0.234 (n=261)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2365 (IC base=+0.039)

- **PATRÓN** `volumen_pendiente_norm` > `0.2561` → IC=+0.362 (n=136)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2561 (IC base=+0.039)

- **PATRÓN** `volumen_spike_ratio` > `1.6328` → IC=+0.261 (n=613)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.6328 (IC base=+0.039)

- **PATRÓN** `ballena_activa_n` < `59.0` → IC=+0.264 (n=524)
...
- **PATRÓN** `sigma_h` < `0.0082` → IC=+0.353 (n=230)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0082 (IC base=+0.340)

- **PATRÓN** `sigma_h` > `0.0093` → IC=+0.343 (n=87)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0093 (IC base=+0.340)

- **PATRÓN** `drift_60min` |x|≤ `0.2365` → IC=+0.353 (n=175)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2365 (IC base=+0.340)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.381 (n=175)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 12.0 (IC base=+0.340)

- **PATRÓN** `ibs_20min` < `0.3443` → IC=+0.355 (n=261)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3443 (IC base=+0.340)

- **PATRÓN** `ibs_20min` > `0.1368` → IC=+0.341 (n=174)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.1368 (IC base=+0.340)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.492` → IC=+0.351 (n=287)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.492 (IC base=+0.340)

- **PATRÓN** `volumen_pendiente_norm` > `0.3643` → IC=+0.426 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3643 (IC base=+0.340)

- **PATRÓN** `volumen_spike_ratio` < `3.2115` → IC=+0.333 (n=148)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.2115 (IC base=+0.340)

- **PATRÓN** `volumen_spike_ratio` > `1.9885` → IC=+0.337 (n=151)
...
- **PATRÓN** `ballena_activa_n` < `31.0` → IC=+0.321 (n=54)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 31.0 (IC base=+0.340)
...
- **PATRÓN** `drift_60min` |x|≤ `0.284` → IC=+0.275 (n=398)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.125` → IC=+0.348 (n=399)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.125 (IC base=+0.269)

- **PATRÓN** `dist_vwap_pct` < `0.9161` → IC=+0.275 (n=672)
...
- **PATRÓN** `volumen_regimen` > `1.2604` → IC=+0.306 (n=199)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2604 (IC base=+0.269)
...
- **PATRÓN** `volumen_regimen` > `0.6889` → IC=+0.173 (n=1606)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` > 0.6889 (IC base=+0.157)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2798` → IC=+0.156 (n=2060)
...
- **PATRÓN** `ballena_activa_n` < `116.0` → IC=+0.180 (n=1319)
...
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.205 (n=232)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0108 (IC base=+0.121)
...
- **PATRÓN** `ibs_20min` > `0.5714` → IC=+0.176 (n=514)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `ibs_20min` > 0.5714 (IC base=+0.121)

- **PATRÓN** `dist_vwap_pct` > `0.8776` → IC=+0.270 (n=172)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8776 (IC base=+0.121)
...
- **PATRÓN** `volumen_regimen` > `0.6201` → IC=+0.138 (n=512)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` > 0.6201 (IC base=+0.121)

- **PATRÓN** `volumen_pendiente_norm` < `0.0712` → IC=+0.125 (n=438)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_pendiente_norm` < 0.0712 (IC base=+0.121)
...
- **PATRÓN** `libro_liquidez` > `3232.1195` → IC=+0.223 (n=171)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3232.1195 (IC base=+0.121)
...
- **PATRÓN** `sigma_h` < `0.0037` → IC=+0.176 (n=143)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` < 0.0037 (IC base=+0.070)

- **PATRÓN** `drift_60min` |x|≤ `0.1224` → IC=+0.123 (n=189)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 0.1224 (IC base=+0.070)

- **PATRÓN** `ibs_20min` < `0.2975` → IC=+0.153 (n=286)

  - _Acción_: Kelly boost +0.76€ cuando `ibs_20min` < 0.2975 (IC base=+0.070)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.789` → IC=+0.177 (n=97)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 7.789 (IC base=+0.070)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.194 (n=1367)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_regimen` < `0.8775` → IC=+0.146 (n=1423)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.8775 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` > `1.0785` → IC=+0.153 (n=967)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3078` → IC=+0.149 (n=2371)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 2.3078 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` > `1.8616` → IC=+0.159 (n=1796)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 1.8616 (IC base=+0.149)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.154 (n=2441)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `2500.5916` → IC=+0.175 (n=2008)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 2500.5916 (IC base=+0.149)

- **PATRÓN** `ballena_activa_n` < `149.0` → IC=+0.193 (n=1439)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 149.0 (IC base=+0.149)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.219 (n=1809)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0066 (IC base=+0.190)

- **PATRÓN** `drift_60min` |x|≤ `0.4735` → IC=+0.202 (n=2709)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.207 (n=979)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.190)

- **PATRÓN** `ibs_20min` < `0.5625` → IC=+0.245 (n=2709)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5625 (IC base=+0.190)

- **PATRÓN** `dist_vwap_pct` < `0.724` → IC=+0.180 (n=2157)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.724 (IC base=+0.190)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.354` → IC=+0.204 (n=394)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.354 (IC base=+0.190)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.639` → IC=+0.193 (n=2539)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.177 (n=466)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` < 18.0 (IC base=+0.165)

- **PATRÓN** `ibs_20min` < `0.3843` → IC=+0.213 (n=395)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3843 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` < `0.1412` → IC=+0.181 (n=384)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.1412 (IC base=+0.165)
...
- **PATRÓN** `volumen_regimen` > `1.1954` → IC=+0.165 (n=150)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` > 1.1954 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` > `0.1495` → IC=+0.271 (n=107)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7479` → IC=+0.198 (n=236)
  ...
  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 1.7479 (IC base=+0.165)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.165 (n=580)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.165)

- **PATRÓN** `libro_liquidez` > `12259.5995` → IC=+0.180 (n=204)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 12259.5995 (IC base=+0.165)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1259` → IC=+0.251 (n=171)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1259 (IC base=+0.249)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.255 (n=267)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.249)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.291 (n=151)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.249)

- **PATRÓN** `ibs_20min` < `0.5652` → IC=+0.313 (n=388)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5652 (IC base=+0.249)
...
- **PATRÓN** `ballena_activa_n` < `65.0` → IC=+0.199 (n=184)
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.182 (n=281)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.3861` → IC=+0.158 (n=387)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.3861 (IC base=+0.152)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.842` → IC=+0.219 (n=119)
...
- **PATRÓN** `volumen_regimen` > `1.15` → IC=+0.181 (n=133)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` > 1.15 (IC base=+0.152)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8125` → IC=+0.174 (n=228)
  ...
  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.8125 (IC base=+0.152)
...
- **PATRÓN** `libro_liquidez` > `9489.0705` → IC=+0.216 (n=181)
...
- **PATRÓN** `sigma_h` > `0.0111` → IC=+0.154 (n=244)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0111 (IC base=+0.095)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.137 (n=370)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 12.0 (IC base=+0.095)

- **PATRÓN** `ibs_20min` > `0.4944` → IC=+0.175 (n=536)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` > 0.4944 (IC base=+0.095)

- **PATRÓN** `dist_vwap_pct` > `0.8569` → IC=+0.225 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8569 (IC base=+0.095)
...
- **PATRÓN** `ballena_activa_n` < `66.0` → IC=+0.161 (n=311)

  - _Acción_: Kelly boost +0.81€ cuando `ballena_activa_n` < 66.0 (IC base=+0.095)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4796` → IC=+0.227 (n=599)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_regimen` > `1.236` → IC=+0.247 (n=227)
...
- **PATRÓN** `ballena_activa_n` < `37.0` → IC=+0.179 (n=397)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.149 (n=1057)

- **PATRÓN** `sigma_h` < `0.0107` → IC=+0.135 (n=562)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.0107 (IC base=+0.117)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.173 (n=224)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 18.0 (IC base=+0.117)

- **PATRÓN** `ibs_20min` > `0.926` → IC=+0.170 (n=213)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` > 0.926 (IC base=+0.117)

- **PATRÓN** `volumen_regimen` < `0.6844` → IC=+0.127 (n=218)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` < 0.6844 (IC base=+0.117)

- **PATRÓN** `volumen_regimen` > `1.0549` → IC=+0.137 (n=224)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` > 1.0549 (IC base=+0.117)

- **PATRÓN** `volumen_pendiente_norm` > `0.2821` → IC=+0.191 (n=82)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.2821 (IC base=+0.117)

- **PATRÓN** `volumen_spike_ratio` < `1.5164` → IC=+0.137 (n=276)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 1.5164 (IC base=+0.117)

- **PATRÓN** `volumen_spike_ratio` > `1.8388` → IC=+0.125 (n=417)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_spike_ratio` > 1.8388 (IC base=+0.117)

- **PATRÓN** `libro_liquidez` > `5057.2547` → IC=+0.128 (n=425)

  - _Acción_: Kelly boost +0.64€ cuando `libro_liquidez` > 5057.2547 (IC base=+0.117)

- **PATRÓN** `sigma_h` < `0.0036` → IC=+0.190 (n=269)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.0036 (IC base=+0.143)

- **PATRÓN** `drift_60min` |x|≤ `0.3642` → IC=+0.172 (n=708)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `4.0` → IC=+0.196 (n=301)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` < 4.0 (IC base=+0.143)

- **PATRÓN** `ibs_20min` < `0.7429` → IC=+0.147 (n=805)
...
- **PATRÓN** `dist_vwap_pct` < `0.3875` → IC=+0.136 (n=767)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.3875 (IC base=+0.143)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.072` → IC=+0.153 (n=808)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 6.072 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` < `1.2072` → IC=+0.154 (n=781)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 1.2072 (IC base=+0.143)

- **PATRÓN** `volumen_pendiente_norm` < `0.0944` → IC=+0.148 (n=725)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4202` → IC=+0.152 (n=796)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.4202 (IC base=+0.143)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.149 (n=1057)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3003` → IC=+0.382 (n=15)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3003 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` < `3.2505` → IC=+0.167 (n=91)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 3.2505 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `2.2445` → IC=+0.161 (n=60)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 2.2445 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `1728.3906` → IC=+0.188 (n=91)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 1728.3906 (IC base=+0.150)
...
- **PATRÓN** `sigma_h` < `0.0088` → IC=+0.157 (n=228)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0088 (IC base=+0.139)

- **PATRÓN** `sigma_h` > `0.0062` → IC=+0.149 (n=152)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0062 (IC base=+0.139)

- **PATRÓN** `drift_60min` |x|≤ `0.5088` → IC=+0.183 (n=228)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.91€ cuando `drift_60min` |x|≤ 0.5088 (IC base=+0.139)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.145 (n=105)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 16.0 (IC base=+0.139)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.171 (n=156)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 10.0 (IC base=+0.139)

- **PATRÓN** `ibs_20min` < `0.612` → IC=+0.144 (n=200)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` < 0.612 (IC base=+0.139)

- **PATRÓN** `ibs_20min` > `0.0863` → IC=+0.174 (n=228)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` > 0.0863 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` > `0.1928` → IC=+0.145 (n=91)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.1928 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` < `0.3517` → IC=+0.152 (n=225)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.3517 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.157` → IC=+0.164 (n=114)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 3.157 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` < `0.6473` → IC=+0.244 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6473 (IC base=+0.139)

- **PATRÓN** `volumen_pendiente_norm` < `0.1309` → IC=+0.162 (n=232)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` < 0.1309 (IC base=+0.139)

- **PATRÓN** `volumen_spike_ratio` < `2.1791` → IC=+0.188 (n=197)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` < 2.1791 (IC base=+0.139)

- **PATRÓN** `volumen_spike_ratio` > `1.4479` → IC=+0.167 (n=223)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.4479 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `10283.2146` → IC=+0.167 (n=76)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 10283.2146 (IC base=+0.139)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.040 (n=520)
...
- **PATRÓN** `liq_usd_total` > `16547.41` → IC=+0.167 (n=43)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `liq_usd_total` > 16547.41 (IC base=+0.073)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.144 (n=130)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 8.0 (IC base=+0.073)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.167 (n=876)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=2703)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.217 (n=842)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=2714)

- **FILTRO** `ibs_20min` > `0.2727` → IC=-0.180 (n=887)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2727
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=2669)
...
- **FILTRO** `ballena_activa_n` > `76.0` → IC=-0.164 (n=147)
...
- **FILTRO** `drift_20min_pct` |x|> `0.2857` → IC=-0.146 (n=190)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.207 (n=145)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.052 (n=456)
...
- **FILTRO** `py_entrada` > `0.64` → IC=-0.278 (n=142)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.023 (n=428)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.135 (n=2206)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **FILTRO** `py_entrada` < `0.36` → IC=-0.282 (n=2155)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=6764)

- **FILTRO** `ibs_7min` < `0.7336` → IC=-0.231 (n=2229)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7336
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=6690)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.172 (n=3030)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=5889)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.231 (n=2497)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=8302)

- **FILTRO** `ibs_7min` > `0.7245` → IC=-0.173 (n=2699)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7245
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=8100)
...
- **FILTRO** `ibs_7min` > `0.8472` → IC=-0.187 (n=465)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8472
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=1397)
...
- **FILTRO** `py_entrada` > `0.61` → IC=-0.225 (n=413)
...
- **FILTRO** `ballena_activa_n` > `133.0` → IC=-0.174 (n=433)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=1301)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.259 (n=408)
...
- **FILTRO** `ibs_7min` > `0.8184` → IC=-0.196 (n=471)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=1416)
...
- **FILTRO** `hora_utc` > `15.0` → IC=-0.124 (n=501)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=988)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.277 (n=352)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=1137)

- **FILTRO** `ibs_7min` > `0.1868` → IC=-0.161 (n=506)
...
- **FILTRO** `ballena_activa_n` > `26.0` → IC=-0.180 (n=495)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=994)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.264 (n=380)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=1454)

- **FILTRO** `ibs_7min` > `0.8136` → IC=-0.167 (n=458)
...
- **FILTRO** `sigma_h` > `0.008` → IC=-0.156 (n=350)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.008
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=680)

- **FILTRO** `sigma_ewma_delta_pct` > `17.465` → IC=-0.169 (n=351)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 17.465
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=2600)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.133` → IC=+0.259 (n=114)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **PATRÓN** `ibs_15` < `0.3345` → IC=+0.322 (n=172)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3345 (IC base=-0.081)

- **PATRÓN** `dist_vwap_pct` < `0.2166` → IC=+0.253 (n=164)
...
- **FILTRO** `sigma_ewma_delta_pct` > `15.954` → IC=-0.141 (n=115)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=909)
...
- **FILTRO** `drift_15min` |x|> `1.1429` → IC=-0.242 (n=64)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
...
- **FILTRO** `sigma_ewma_delta_pct` > `14.492` → IC=-0.184 (n=36)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.142 (n=224)

- **FILTRO** `libro_liquidez` < `2519.5422` → IC=-0.202 (n=65)
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3345 sube el IC de -0.081 a +0.322 en UPDOWN_GBM_15M_TARDIO (n=172). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 10147 | -0.094 | -1622.29€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 726 | -0.069 | -128.13€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 9421 | -0.096 | -1494.17€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#DOGE | 1298 | -0.161 | -414.21€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 1298 | -0.161 | -414.21€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 2746 | -0.068 | -266.26€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 2746 | -0.068 | -266.26€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 2368 | -0.078 | -163.46€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2368 | -0.078 | -163.46€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1768 | -0.177 | -436.09€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1768 | -0.177 | -436.09€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 184 | -0.059 | +45.07€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 51 | -0.009 | +16.32€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 133 | -0.078 | +28.75€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 184 | -0.059 | +45.07€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 51 | -0.009 | +16.32€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 133 | -0.078 | +28.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 33570 | +0.115 | -2087.11€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 6130 | +0.186 | -228.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 109 | -0.104 | -50.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 24089 | +0.098 | -1766.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3242 | +0.120 | -41.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 4044 | +0.064 | -680.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 21 | -0.065 | +2.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 4018 | +0.066 | -676.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 6910 | +0.134 | -150.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1736 | +0.193 | -108.55€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 4009 | +0.111 | -79.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1123 | +0.130 | +59.40€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 