# Hipótesis automáticas — 2026-07-24 19:47 UTC
_Generado por shadow_postmortem.py sobre 33442 resoluciones (PNL=+7383.59€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.199 (n=2114)
...
- **PATRÓN** `py_entrada` > `0.735` → IC=+0.340 (n=712)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.735 (IC base=+0.194)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.202 (n=2388)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.238 (n=162)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.605` → IC=+0.277 (n=469)
...
- **PATRÓN** `dist_vwap_pct` > `0.2737` → IC=+0.132 (n=406)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.2737 (IC base=+0.099)
...
- **PATRÓN** `sigma_h` < `0.0095` → IC=+0.178 (n=268)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0095 (IC base=+0.153)

- **PATRÓN** `sigma_h` > `0.0268` → IC=+0.158 (n=267)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0268 (IC base=+0.153)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.177 (n=277)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.81` → IC=+0.145 (n=468)
...
- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.167 (n=1146)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` > 0.0071 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.149 (n=1208)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `1.045` → IC=+0.210 (n=98)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.045 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `0.7974` → IC=+0.135 (n=1782)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.7974 (IC base=+0.133)
...
- **PATRÓN** `sigma_h` < `0.0184` → IC=+0.249 (n=433)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0184 (IC base=+0.243)

- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.251 (n=387)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0087 (IC base=+0.243)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.265 (n=398)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.04` → IC=+0.245 (n=383)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.04 (IC base=+0.243)
...
- **PATRÓN** `sigma_h` < `0.0191` → IC=+0.193 (n=484)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0191 (IC base=+0.182)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.200 (n=388)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 12.0 (IC base=+0.182)

- **PATRÓN** `dist_vwap_pct` > `0.3707` → IC=+0.241 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3707 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.054` → IC=+0.185 (n=548)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` < 10.054 (IC base=+0.182)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.7622` → IC=+0.239 (n=393)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `23.418` → IC=+0.219 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 23.418 (IC base=+0.060)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2223` → IC=+0.142 (n=280)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.2223 (IC base=+0.105)

- **PATRÓN** `delta_ratio_macro` |x|> `0.063` → IC=+0.131 (n=280)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.66€ cuando `delta_ratio_macro` |x|> 0.063 (IC base=+0.105)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.179 (n=82)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` < 8.0 (IC base=+0.105)

- **PATRÓN** `ibs_15` > `0.724` → IC=+0.251 (n=215)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.724 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` > `0.1666` → IC=+0.240 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1666 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` > `24.96` → IC=+0.188 (n=30)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 24.96 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` < `14.847` → IC=+0.164 (n=117)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.7622 sube el IC de +0.060 a +0.239 en UPDOWN_GBM#15min (n=393). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.724 sube el IC de +0.105 a +0.251 en UPDOWN_GBM#BTC#15min (n=215). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ FAVORITO_CONFIRMADO | 5920 | +0.187 | +89.30€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#15min | 3890 | +0.213 | +75.92€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 1735 | +0.196 | +44.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1296 | +0.210 | -10.14€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 7521 | +0.097 | +2346.91€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 7521 | +0.097 | +2346.91€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#XRP | 1991 | +0.150 | +1033.12€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1991 | +0.150 | +1033.12€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 4711 | +0.124 | +2342.51€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 4711 | +0.124 | +2342.51€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 1061 | +0.239 | +975.03€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 1061 | +0.239 | +975.03€ | 0 | 10 |
...
| ✅ GBM_LATE_15M_TARDIO | 4745 | +0.075 | +1361.73€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 4745 | +0.075 | +1361.73€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 1348 | +0.181 | +915.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1348 | +0.181 | +915.60€ | 0 | 9 |
...
| ✅ UPDOWN_GBM | 2488 | +0.035 | +234.31€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 2225 | +0.055 | +272.83€ | 0 | 3 |
...
| ✅ UPDOWN_GBM#BTC | 547 | +0.054 | +68.07€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 463 | +0.089 | +82.55€ | 1 | 9 |
...
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD | 145 | +0.309 | +32.94€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#15min | 145 | +0.309 | +32.94€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC | 69 | +0.303 | +13.17€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min | 69 | +0.303 | +13.17€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH | 76 | +0.308 | +19.77€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min | 76 | +0.308 | +19.77€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO | 1591 | +0.167 | +758.19€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1591 | +0.167 | +758.19€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 146 | +0.196 | +82.51€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 146 | +0.196 | +82.51€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 178 | +0.122 | +35.73€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 178 | +0.122 | +35.73€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.181: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.070 n=1309 | neutral: IC=+0.023 n=1015 | overbought(IBS>0.7): IC=+0.204 n=1281
  - _Datos_: n=3848 IC=+0.108 PNL=+1051.33€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.116 n=20050 | tras_1loss IC=+0.083 n=13192 | tras_2loss IC=+0.052 n=5468/40 | gap=+0.063 (umbral 0.05)
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=+0.091 n=1489/40 PNL=+299.73€ | H=15h UTC: IC=+0.145 n=1431/40 PNL=+537.39€ | H=17h UTC: IC=+0.095 n=1414/40 PNL=+271.23€ | H=19h UTC: IC=+0.136 n=1323/40 PNL=+370.14€
...
**〰️ H-24H-GBM-BUYYES-TARDE** — GBM BUY_YES en tarde europea (15-19h UTC) — señal alcista sostenida
  ...
  - _Estado_: n=272 IC=-0.004 PNL=-7.60€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=272 IC=-0.004 PNL=-7.60€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2356 IC=+0.041 PNL=+249.06€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2356 IC=+0.041 PNL=+249.06€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=961 IC=+0.060 PNL=+147.27€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=961 IC=+0.060 PNL=+147.27€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=463 IC=+0.089 PNL=+82.55€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=463 IC=+0.089 PNL=+82.55€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.096 > 0.08 con n=1181 PNL=+219.85€
  - _Datos_: n=1181 IC=+0.096 PNL=+219.85€
...
**🟡 H-CUSTOM-CROSS-WINDOW-SPREAD-POS** — GBM 15min/60min: spread positivo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.263 > 0.08 con n=158 PNL=+34.49€
  - _Datos_: n=158 IC=+0.263 PNL=+34.49€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=1111 IC=+0.031 PNL=+116.72€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1111 IC=+0.031 PNL=+116.72€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.180 > 0.1 con n=782 PNL=+310.70€
  - _Datos_: n=782 IC=+0.180 PNL=+310.70€
...
**〰️ H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: n=224 IC=+0.053 PNL=-1.87€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=224 IC=+0.053 PNL=-1.87€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.148 > 0.08 con n=557 PNL=+193.51€
  - _Datos_: n=557 IC=+0.148 PNL=+193.51€
...
**🟡 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.190 > 0.08 con n=253 PNL=+103.70€
  - _Datos_: n=253 IC=+0.190 PNL=+103.70€
