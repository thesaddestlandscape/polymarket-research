# Hipótesis automáticas — 2026-07-02 16:31 UTC
_Generado por shadow_postmortem.py sobre 2748 resoluciones (PNL=-54.26€)_
...
- **PATRÓN** `sigma_h` < `0.0101` → IC=+0.216 (n=72)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0101 (IC base=+0.041)

- **PATRÓN** `ibs_15` < `0.0714` → IC=+0.211 (n=50)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0714 (IC base=+0.041)
...
- **FILTRO** `ibs_15` > `0.0792` → IC=-0.125 (n=22)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: SKIP cuando `ibs_15` > 0.0792
  - _Potencial_: sin este filtro IC_bueno=+0.346 (n=24)

- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1397` → IC=+0.167 (n=49)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.83€ cuando `pct_spot_vs_ref` |x|≤ 0.1397 (IC base=+0.121)

- **PATRÓN** `sigma_h` < `0.016` → IC=+0.147 (n=49)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.016 (IC base=+0.121)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.237 (n=17)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.121)

- **PATRÓN** `ibs_15` < `0.0792` → IC=+0.346 (n=24)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0792 (IC base=+0.121)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS < 0.0714 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.211 n=50). Confirma señal de reversión media → alinear con BUY_YES.
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.0792 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.346 n=24). Confirma señal de reversión media → alinear con BUY_YES.
- **LIVE-CANDIDATA**: `GBM_LATE_15M#XRP#15min` — IC=+0.206 n=32. Faltan ~8 resoluciones para umbral n≥40. ETA: ~6h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M#XRP` — IC=+0.206 n=32. Faltan ~8 resoluciones para umbral n≥40. ETA: ~6h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M#BTC#15min` — IC=+0.125 n=38. Faltan ~2 resoluciones para umbral n≥40. ETA: ~1h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M#BTC` — IC=+0.125 n=38. Faltan ~2 resoluciones para umbral n≥40. ETA: ~1h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M#SOL#15min` — IC=+0.167 n=31. Faltan ~9 resoluciones para umbral n≥40. ETA: ~6h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M#SOL` — IC=+0.167 n=31. Faltan ~9 resoluciones para umbral n≥40. ETA: ~6h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 137 | +0.133 | +36.82€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 137 | +0.133 | +36.82€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 38 | +0.125 | +8.93€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 38 | +0.125 | +8.93€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 36 | +0.026 | +1.41€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 36 | +0.026 | +1.41€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL | 31 | +0.167 | +12.04€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 31 | +0.167 | +12.04€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP | 32 | +0.206 | +14.45€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 32 | +0.206 | +14.45€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 826 | -0.040 | -47.36€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 598 | -0.017 | -19.48€ | 2 | 2 |
...
| ✅ UPDOWN_GBM#BTC | 242 | -0.045 | -35.11€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 171 | -0.020 | -24.39€ | 2 | 0 |
...
| ✅ UPDOWN_GBM#ETH | 317 | -0.008 | +7.24€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 233 | +0.011 | +13.71€ | 3 | 4 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.079) — sin ventaja clara. oversold(IBS<0.3): IC=+0.000 n=146 | neutral: IC=-0.079 n=131 | overbought(IBS>0.7): IC=-0.044 n=145
  - _Datos_: n=436 IC=-0.039 PNL=-22.12€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=01h: IC=-0.200 n=28 PNL=-13.74€ → FILTRAR | H=02h: IC=-0.227 n=20 PNL=-7.89€ → FILTRAR | H=10h: IC=-0.154 n=15 PNL=-6.77€ → FILTRAR | H=15h: IC=-0.109 n=21 PNL=-6.63€ → FILTRAR | H=16h: IC=+0.204 n=19 PNL=+6.29€ → BOOST | H=17h: IC=+0.110 n=15 PNL=+2.13€ → BOOST | H=20h: IC=-0.180 n=23 PNL=-6.41€ → FILTRAR | H=23h: IC=+0.180 n=18 PNL=+12.32€ → BOOST
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.011 n=1311 | tras_1loss IC=-0.033 n=1346 | tras_2loss IC=-0.045 n=697/40 | gap=+0.056 (umbral 0.05)
...
**〰️ H-24H-GBM-BUYYES-TARDE** — GBM BUY_YES en tarde europea (15-19h UTC) — señal alcista sostenida
  ...
  - _Estado_: n=147 IC=-0.044 PNL=-12.83€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=147 IC=-0.044 PNL=-12.83€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=703 IC=-0.035 PNL=-33.80€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=703 IC=-0.035 PNL=-33.80€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=382 IC=-0.050 PNL=-44.03€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=382 IC=-0.050 PNL=-44.03€
...
**🔴 H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.020 < 0.02 con n=171 PNL=-24.39€
  - _Datos_: n=171 IC=-0.020 PNL=-24.39€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=254 IC=-0.012 PNL=-30.42€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=254 IC=-0.012 PNL=-30.42€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=198 IC=+0.015 PNL=+12.88€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=198 IC=+0.015 PNL=+12.88€
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=322 IC=+0.068 PNL=+35.90€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=322 IC=+0.068 PNL=+35.90€
...
**🔴 H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.074 < -0.05 con n=524 PNL=-71.95€
  - _Datos_: n=524 IC=-0.074 PNL=-71.95€
...
**〰️ H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: n=88 IC=-0.011 PNL=-8.43€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=88 IC=-0.011 PNL=-8.43€
