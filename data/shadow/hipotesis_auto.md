# Hipótesis automáticas — 2026-07-12 20:14 UTC
_Generado por shadow_postmortem.py sobre 11375 resoluciones (PNL=+2174.15€)_
...
- **PATRÓN** `hora_utc` < `10.0` → IC=+0.219 (n=190)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 10.0 (IC base=+0.149)

- **PATRÓN** `py_entrada` < `0.275` → IC=+0.306 (n=137)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.275 (IC base=+0.149)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.156 (n=446)

  - _Acción_: Kelly boost +0.78€ cuando `libro_spread` < 0.01 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `6004.8923` → IC=+0.167 (n=178)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 6004.8923 (IC base=+0.149)
...
- **PATRÓN** `hora_utc` < `10.0` → IC=+0.232 (n=39)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 10.0 (IC base=+0.216)
...
- **PATRÓN** `libro_liquidez` > `6496.7981` → IC=+0.235 (n=81)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 6496.7981 (IC base=+0.216)
...
- **PATRÓN** `py_entrada` < `0.245` → IC=+0.274 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.245 (IC base=+0.167)

- **PATRÓN** `py_entrada` > `0.395` → IC=+0.167 (n=28)

  - _Acción_: Kelly boost +0.83€ cuando `py_entrada` > 0.395 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `3306.8336` → IC=+0.167 (n=82)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 3306.8336 (IC base=+0.167)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.233 (n=43)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.235` → IC=+0.267 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.235 (IC base=+0.208)

- **PATRÓN** `libro_liquidez` > `1598.2993` → IC=+0.233 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1598.2993 (IC base=+0.208)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.123 (n=1378)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5436` → IC=+0.153 (n=47)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.5436 (IC base=+0.117)
...
- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.148 (n=944)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.06` → IC=+0.155 (n=56)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` < 4.06 (IC base=+0.110)
...
- **PATRÓN** `dist_vwap_pct` > `0.1093` → IC=+0.278 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1093 (IC base=+0.057)
...
- **PATRÓN** `dist_vwap_pct` > `0.6593` → IC=+0.167 (n=16)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.6593 (IC base=+0.053)

- **PATRÓN** `dist_vwap_pct` < `0.2858` → IC=+0.132 (n=17)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.2858 (IC base=+0.053)

### GBM_LATE_15M#SOL#15min
- **PATRÓN** `sigma_h` < `0.0161` → IC=+0.139 (n=366)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0161 (IC base=+0.124)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.164 (n=141)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 17.0 (IC base=+0.124)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.133 (n=167)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` < 8.0 (IC base=+0.124)

- **PATRÓN** `dist_vwap_pct` > `0.312` → IC=+0.143 (n=26)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.312 (IC base=+0.124)
...
- **PATRÓN** `sigma_h` < `0.0171` → IC=+0.181 (n=236)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.0171 (IC base=+0.176)

- **PATRÓN** `sigma_h` > `0.0115` → IC=+0.187 (n=353)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0115 (IC base=+0.176)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.258 (n=130)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.176)

- **PATRÓN** `dist_vwap_pct` > `0.2834` → IC=+0.140 (n=23)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.2834 (IC base=+0.176)

- **PATRÓN** `dist_vwap_pct` < `0.1189` → IC=+0.190 (n=27)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.1189 (IC base=+0.176)
...
- **PATRÓN** `sigma_h` > `0.0042` → IC=+0.178 (n=393)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.1596` → IC=+0.260 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1596 (IC base=+0.153)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.21` → IC=+0.263 (n=57)
...
- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.250 (n=86)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.0976` → IC=+0.184 (n=17)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.0976 (IC base=+0.162)
...
- **PATRÓN** `sigma_h` > `0.0034` → IC=+0.136 (n=97)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0034 (IC base=+0.103)
...
- **PATRÓN** `dist_vwap_pct` > `0.5883` → IC=+0.220 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5883 (IC base=+0.103)

- **PATRÓN** `dist_vwap_pct` < `0.2822` → IC=+0.147 (n=15)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.2822 (IC base=+0.103)
...
- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.314 (n=68)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.0977` → IC=+0.292 (n=22)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0977 (IC base=+0.277)
...
- **PATRÓN** `sigma_h` > `0.0121` → IC=+0.141 (n=151)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` > 0.0121 (IC base=+0.075)

- **PATRÓN** `hora_utc` < `9.0` → IC=+0.120 (n=214)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` < 9.0 (IC base=+0.075)

- **PATRÓN** `dist_vwap_pct` < `0.1776` → IC=+0.156 (n=91)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.1776 (IC base=+0.075)
...
- **PATRÓN** `sigma_h` > `0.0033` → IC=+0.121 (n=93)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` > 0.0033 (IC base=+0.079)
...
- **PATRÓN** `dist_vwap_pct` > `0.6593` → IC=+0.147 (n=15)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.6593 (IC base=-0.017)
...
- **PATRÓN** `sigma_h` > `0.0303` → IC=+0.267 (n=41)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0303 (IC base=+0.201)
...
- **PATRÓN** `ibs_15` > `0.717` → IC=+0.159 (n=221)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.80€ cuando `ibs_15` > 0.717 (IC base=+0.019)

- **PATRÓN** `dist_vwap_pct` > `0.1361` → IC=+0.178 (n=88)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.1361 (IC base=+0.019)

- **PATRÓN** `dist_vwap_pct` < `0.2724` → IC=+0.185 (n=87)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` < 0.2724 (IC base=+0.019)
...
- **PATRÓN** `ibs_15` < `0.0512` → IC=+0.134 (n=132)
  ...
  - _Acción_: Kelly boost +0.67€ cuando `ibs_15` < 0.0512 (IC base=+0.030)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2308` → IC=+0.124 (n=139)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 0.2308 (IC base=+0.060)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.169 (n=119)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 12.0 (IC base=+0.060)

- **PATRÓN** `ibs_15` < `0.9252` → IC=+0.153 (n=119)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.76€ cuando `ibs_15` < 0.9252 (IC base=+0.060)

- **PATRÓN** `ibs_15` > `0.6374` → IC=+0.178 (n=119)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.89€ cuando `ibs_15` > 0.6374 (IC base=+0.060)

- **PATRÓN** `dist_vwap_pct` > `0.3483` → IC=+0.342 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3483 (IC base=+0.060)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS > 0.717 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.159 n=221). Confirma señal de reversión media → alinear con BUY_NO.
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS < 0.9252 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.153 n=119). Confirma señal de reversión media → alinear con BUY_YES.
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS > 0.6374 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.178 n=119). Confirma señal de reversión media → alinear con BUY_NO.
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 1022 | +0.185 | +20.28€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 656 | +0.214 | +16.06€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 300 | +0.189 | -1.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 219 | +0.210 | -6.37€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 363 | +0.169 | +4.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 217 | +0.208 | +6.04€ | 0 | 9 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 357 | +0.199 | +17.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 218 | +0.223 | +16.35€ | 0 | 8 |
...
| ✅ GBM_LATE_15M | 3681 | +0.113 | +1229.00€ | 0 | 9 |
| ✅ GBM_LATE_15M#15min | 3681 | +0.113 | +1229.00€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 901 | +0.072 | +128.05€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 901 | +0.072 | +128.05€ | 0 | 2 |
| ✅ GBM_LATE_15M#ETH | 931 | +0.072 | +157.18€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 931 | +0.072 | +157.18€ | 0 | 4 |
| ✅ GBM_LATE_15M#SOL | 961 | +0.138 | +478.07€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 961 | +0.138 | +478.07€ | 0 | 8 |
| ✅ GBM_LATE_15M#XRP | 888 | +0.170 | +465.70€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 888 | +0.170 | +465.70€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1032 | +0.165 | +607.38€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1032 | +0.165 | +607.38€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 270 | +0.165 | +177.56€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 270 | +0.165 | +177.56€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 275 | +0.132 | +126.20€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 275 | +0.132 | +126.20€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 289 | +0.122 | +109.04€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 289 | +0.122 | +109.04€ | 0 | 6 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 198 | +0.270 | +194.59€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 198 | +0.270 | +194.59€ | 0 | 6 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 1180 | +0.080 | +291.41€ | 1 | 8 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1180 | +0.080 | +291.41€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 272 | +0.084 | +44.69€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 272 | +0.084 | +44.69€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 295 | +0.005 | +11.16€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 295 | +0.005 | +11.16€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 308 | +0.032 | +34.92€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 308 | +0.032 | +34.92€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 305 | +0.197 | +200.63€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 305 | +0.197 | +200.63€ | 0 | 7 |
...
| ✅ STREAK_FADE_5M | 165 | -0.039 | -10.48€ | 4 | 0 |
| ✅ STREAK_FADE_5M#5min | 165 | -0.039 | -10.48€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 1337 | -0.006 | +25.86€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1081 | +0.024 | +63.33€ | 1 | 5 |
...
| ✅ UPDOWN_GBM#BTC | 362 | +0.014 | +9.19€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 283 | +0.054 | +22.57€ | 1 | 8 |
...
| ✅ UPDOWN_GBM#ETH | 661 | +0.014 | +35.10€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 568 | +0.033 | +46.16€ | 0 | 2 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.126) — sin ventaja clara. oversold(IBS<0.3): IC=-0.021 n=347 | neutral: IC=-0.047 n=263 | overbought(IBS>0.7): IC=+0.079 n=295
  - _Datos_: n=947 IC=+0.009 PNL=+51.10€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.093 n=6452 | tras_1loss IC=+0.048 n=4785 | tras_2loss IC=+0.015 n=2134/40 | gap=+0.078 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1210 IC=+0.001 PNL=+39.86€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1210 IC=+0.001 PNL=+39.86€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=617 IC=+0.019 PNL=+26.70€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=617 IC=+0.019 PNL=+26.70€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=283 IC=+0.054 PNL=+22.57€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=283 IC=+0.054 PNL=+22.57€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=561 IC=+0.052 PNL=+45.70€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=561 IC=+0.052 PNL=+45.70€
...
**〰️ H-CUSTOM-CROSS-WINDOW-SPREAD-NEG** — GBM 15min/60min: spread negativo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: n=55 IC=+0.026 PNL=+26.68€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=55 IC=+0.026 PNL=+26.68€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8917 IC=+0.099 PNL=+2250.03€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8917 IC=+0.099 PNL=+2250.03€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=764 IC=-0.014 PNL=-3.77€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=764 IC=-0.014 PNL=-3.77€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.170 > 0.1 con n=280 PNL=+100.38€
  - _Datos_: n=280 IC=+0.170 PNL=+100.38€
...
**〰️ H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: n=154 IC=+0.077 PNL=+13.13€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=154 IC=+0.077 PNL=+13.13€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=376 IC=-0.066 PNL=+67.29€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=376 IC=-0.066 PNL=+67.29€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=463 IC=+0.057 PNL=+44.82€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=463 IC=+0.057 PNL=+44.82€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.170 > 0.08 con n=213 PNL=+72.94€
  - _Datos_: n=213 IC=+0.170 PNL=+72.94€
...
**🔶 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA en BTC (IC=+0.231 n=91) pero sin cruzar ≥2 pares más — ETH: n=110 IC=+0.134 ✓; SOL: n=7 IC=-0.019; XRP: n=5 IC=+0.018
  - _Datos_: n=91 IC=+0.231 PNL=+38.93€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema, sin filtrar todavía
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.158 < -0.1 con n=317 PNL=-36.89€
  - _Datos_: n=317 IC=-0.158 PNL=-36.89€
...
**⏳ H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: 26/40 ops en el filtro definido (IC actual=+0.214 PNL=+22.65€)
  - _Datos_: n=26 IC=+0.214 PNL=+22.65€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.176 > 0.08 con n=72 PNL=+1.28€
  - _Datos_: n=72 IC=+0.176 PNL=+1.28€
...
**⏳ H-CUSTOM-ETH15-BUYNO-TARDIO** — UPDOWN_GBM ETH#15min BUY_NO tardío (T_h<0.2) -- edge fuerte no capturado por el aprendizaje causal automático
  ...
  - _Estado_: 39/40 ops en el filtro definido (IC actual=+0.256 PNL=+24.46€)
  - _Datos_: n=39 IC=+0.256 PNL=+24.46€
