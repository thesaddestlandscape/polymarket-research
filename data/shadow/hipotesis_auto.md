# Hipótesis automáticas — 2026-07-21 05:55 UTC
_Generado por shadow_postmortem.py sobre 26095 resoluciones (PNL=+6296.52€)_
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.207 (n=1649)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.204 (n=1832)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.204 (n=795)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.176 (n=1972)
...
- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.126 (n=1607)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.63€ cuando `sigma_h` > 0.0086 (IC base=+0.108)

- **PATRÓN** `dist_vwap_pct` > `0.1596` → IC=+0.128 (n=444)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` > 0.1596 (IC base=+0.108)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.306` → IC=+0.150 (n=115)
  ...
  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 4.306 (IC base=+0.069)
...
- **PATRÓN** `sigma_h` < `0.0094` → IC=+0.188 (n=216)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0094 (IC base=+0.168)

- **PATRÓN** `sigma_h` > `0.0255` → IC=+0.191 (n=215)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0255 (IC base=+0.168)
...
- **PATRÓN** `dist_vwap_pct` > `0.3076` → IC=+0.203 (n=99)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3076 (IC base=+0.168)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.7` → IC=+0.167 (n=319)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` < 3.7 (IC base=+0.168)
...
- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.189 (n=843)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.3797` → IC=+0.173 (n=1077)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.3797 (IC base=+0.157)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.113` → IC=+0.185 (n=233)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 6.113 (IC base=+0.157)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.006` → IC=+0.162 (n=859)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` < 3.006 (IC base=+0.157)
...
- **PATRÓN** `sigma_h` > `0.0031` → IC=+0.137 (n=290)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0031 (IC base=+0.120)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.157 (n=164)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 15.0 (IC base=+0.120)

- **PATRÓN** `dist_vwap_pct` < `0.3418` → IC=+0.123 (n=287)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.3418 (IC base=+0.120)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.456` → IC=+0.178 (n=116)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 4.456 (IC base=+0.120)
...
- **PATRÓN** `sigma_h` < `0.0157` → IC=+0.288 (n=314)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0157 (IC base=+0.283)

- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.307 (n=314)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0077 (IC base=+0.283)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.302 (n=317)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.283)

- **PATRÓN** `dist_vwap_pct` > `0.3142` → IC=+0.359 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3142 (IC base=+0.283)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.538` → IC=+0.292 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.538 (IC base=+0.283)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.004` → IC=+0.292 (n=263)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.004 (IC base=+0.283)
...
- **PATRÓN** `dist_vwap_pct` < `0.3689` → IC=+0.124 (n=1064)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.3689 (IC base=+0.103)
...
- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.209 (n=177)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0092 (IC base=+0.206)

- **PATRÓN** `sigma_h` > `0.0209` → IC=+0.213 (n=134)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0209 (IC base=+0.206)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.226 (n=199)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.206)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.210 (n=143)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` < `10.154` → IC=+0.215 (n=374)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 10.154 (IC base=+0.206)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.7526` → IC=+0.213 (n=336)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7526 (IC base=+0.046)

- **PATRÓN** `dist_vwap_pct` > `0.133` → IC=+0.169 (n=164)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` > 0.133 (IC base=+0.046)

- **PATRÓN** `dist_vwap_pct` < `0.4761` → IC=+0.146 (n=244)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.4761 (IC base=+0.046)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2293` → IC=+0.129 (n=246)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.65€ cuando `drift_60min` |x|≤ 0.2293 (IC base=+0.091)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0631` → IC=+0.121 (n=246)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.60€ cuando `delta_ratio_macro` |x|> 0.0631 (IC base=+0.091)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.149 (n=189)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 12.0 (IC base=+0.091)

- **PATRÓN** `ibs_15` > `0.6354` → IC=+0.208 (n=207)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6354 (IC base=+0.091)

- **PATRÓN** `dist_vwap_pct` > `0.1435` → IC=+0.221 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1435 (IC base=+0.091)

- **PATRÓN** `sigma_ewma_delta_pct` < `14.017` → IC=+0.167 (n=85)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` < 14.017 (IC base=+0.091)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS > 0.7526 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.213 n=336). Confirma señal de reversión media → alinear con BUY_NO.
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS > 0.6354 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.208 n=207). Confirma señal de reversión media → alinear con BUY_NO.
...
| ✅ FAVORITO_CONFIRMADO | 4506 | +0.185 | +56.16€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 2931 | +0.217 | +95.67€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 193 | +0.064 | -7.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 608 | +0.103 | -84.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 774 | +0.160 | +52.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 1308 | +0.199 | +54.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 976 | +0.212 | +4.91€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 64 | +0.091 | +1.32€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 258 | +0.192 | +52.00€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH | 1595 | +0.177 | +0.08€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 258 | +0.150 | +14.13€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL | 1579 | +0.183 | -0.69€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 258 | +0.135 | -13.49€ | 0 | 5 |
...
| ✅ GBM_LATE_15M | 6399 | +0.102 | +2129.77€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 6399 | +0.102 | +2129.77€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1528 | +0.075 | +290.75€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1528 | +0.075 | +290.75€ | 0 | 5 |
...
| ✅ GBM_LATE_15M#XRP | 1686 | +0.162 | +942.21€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1686 | +0.162 | +942.21€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 3595 | +0.143 | +2071.44€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 3595 | +0.143 | +2071.44€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 916 | +0.107 | +417.41€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 916 | +0.107 | +417.41€ | 0 | 9 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 827 | +0.267 | +853.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 827 | +0.267 | +853.53€ | 0 | 12 |
...
| ✅ GBM_LATE_15M_TARDIO | 3575 | +0.093 | +1224.28€ | 0 | 6 |
| ✅ GBM_LATE_15M_TARDIO#15min | 3575 | +0.093 | +1224.28€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 855 | +0.062 | +165.83€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 855 | +0.062 | +165.83€ | 0 | 5 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 1044 | +0.205 | +800.52€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1044 | +0.205 | +800.52€ | 0 | 13 |
...
| ✅ LEADLAG_BTC_XRP_15M | 243 | +0.018 | +12.15€ | 2 | 1 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 243 | +0.018 | +12.15€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 243 | +0.018 | +12.15€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 243 | +0.018 | +12.15€ | 2 | 1 |
...
| ✅ UPDOWN_GBM | 2033 | +0.026 | +158.59€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1771 | +0.049 | +197.67€ | 0 | 4 |
...
| ✅ UPDOWN_GBM#BTC | 500 | +0.040 | +41.94€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 416 | +0.077 | +56.42€ | 1 | 7 |
...
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD | 96 | +0.286 | +15.96€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#15min | 96 | +0.286 | +15.96€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC | 42 | +0.250 | +3.05€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min | 42 | +0.250 | +3.05€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH | 54 | +0.304 | +12.92€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min | 54 | +0.304 | +12.92€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO | 929 | +0.173 | +434.99€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 929 | +0.173 | +434.99€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 87 | +0.197 | +46.82€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 87 | +0.197 | +46.82€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 110 | +0.116 | +14.16€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 110 | +0.116 | +14.16€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.194: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.059 n=914 | neutral: IC=+0.001 n=689 | overbought(IBS>0.7): IC=+0.194 n=894
  - _Datos_: n=2674 IC=+0.097 PNL=+635.65€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.117 n=15714 | tras_1loss IC=+0.088 n=10179 | tras_2loss IC=+0.054 n=4146/40 | gap=+0.063 (umbral 0.05)
...
**🟡 H-24H-GBM-BUYYES-MADRUGADA** — GBM BUY_YES en madrugada europea (05-07h UTC) — señal alcista
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.091 > 0.08 con n=91 PNL=+29.54€
  - _Datos_: n=91 IC=+0.091 PNL=+29.54€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1901 IC=+0.032 PNL=+173.33€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1901 IC=+0.032 PNL=+173.33€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=846 IC=+0.046 PNL=+92.78€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=846 IC=+0.046 PNL=+92.78€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=416 IC=+0.077 PNL=+56.42€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=416 IC=+0.077 PNL=+56.42€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.081 > 0.08 con n=946 PNL=+139.57€
  - _Datos_: n=946 IC=+0.081 PNL=+139.57€
...
**🟡 H-CUSTOM-CROSS-WINDOW-SPREAD-POS** — GBM 15min/60min: spread positivo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.228 > 0.08 con n=123 PNL=+13.28€
  - _Datos_: n=123 IC=+0.228 PNL=+13.28€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=23631 IC=+0.118 PNL=+6371.37€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=23631 IC=+0.118 PNL=+6371.37€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=996 IC=+0.016 PNL=+62.23€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=996 IC=+0.016 PNL=+62.23€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.169 > 0.1 con n=615 PNL=+225.87€
  - _Datos_: n=615 IC=+0.169 PNL=+225.87€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=816 IC=+0.062 PNL=+117.92€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=816 IC=+0.062 PNL=+117.92€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.144 > 0.08 con n=442 PNL=+139.02€
  - _Datos_: n=442 IC=+0.144 PNL=+139.02€
...
**🟡 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.186 > 0.08 con n=208 PNL=+77.45€
  - _Datos_: n=208 IC=+0.186 PNL=+77.45€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.154 > 0.08 con n=443 PNL=+271.78€
  - _Datos_: n=443 IC=+0.154 PNL=+271.78€
