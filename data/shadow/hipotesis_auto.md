# Hipótesis automáticas — 2026-07-20 04:58 UTC
_Generado por shadow_postmortem.py sobre 23869 resoluciones (PNL=+6090.12€)_
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.211 (n=710)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.179 (n=1789)

  - _Acción_: Kelly boost +0.89€ cuando `libro_spread` < 0.01 (IC base=+0.173)

- **PATRÓN** `libro_liquidez` > `4167.2232` → IC=+0.177 (n=1074)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 4167.2232 (IC base=+0.173)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.272 (n=134)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.227 (n=243)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.244 (n=158)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.210)

- **PATRÓN** `py_entrada` < `0.395` → IC=+0.254 (n=339)
...
- **PATRÓN** `dist_vwap_pct` < `0.1207` → IC=+0.128 (n=687)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.1207 (IC base=+0.117)
...
- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.131 (n=1511)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` > 0.0086 (IC base=+0.116)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.136 (n=1634)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` < 12.0 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` > `0.1363` → IC=+0.138 (n=387)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 0.1363 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` < `0.8051` → IC=+0.130 (n=1202)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.8051 (IC base=+0.116)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.174` → IC=+0.147 (n=259)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 6.174 (IC base=+0.116)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.105` → IC=+0.128 (n=1082)
...
- **PATRÓN** `dist_vwap_pct` < `0.466` → IC=+0.139 (n=247)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.466 (IC base=+0.077)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.004` → IC=+0.182 (n=108)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 3.004 (IC base=+0.077)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.618` → IC=+0.214 (n=82)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.379` → IC=+0.214 (n=131)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.379 (IC base=+0.104)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.235 (n=228)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.174)

- **PATRÓN** `dist_vwap_pct` > `0.3993` → IC=+0.172 (n=59)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.3993 (IC base=+0.174)

- **PATRÓN** `dist_vwap_pct` < `0.5211` → IC=+0.175 (n=358)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` < 0.5211 (IC base=+0.174)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.515` → IC=+0.173 (n=337)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` < 9.515 (IC base=+0.174)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.207 (n=787)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.2752` → IC=+0.216 (n=336)
...
- **PATRÓN** `sigma_h` > `0.0044` → IC=+0.189 (n=1125)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.194 (n=384)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` < 5.0 (IC base=+0.175)

- **PATRÓN** `dist_vwap_pct` < `0.7734` → IC=+0.195 (n=1050)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` < 0.7734 (IC base=+0.175)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.124` → IC=+0.218 (n=200)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.124 (IC base=+0.175)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.006` → IC=+0.186 (n=731)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` < 3.006 (IC base=+0.175)
...
- **PATRÓN** `sigma_h` < `0.006` → IC=+0.146 (n=286)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` < 0.006 (IC base=+0.141)

- **PATRÓN** `sigma_h` > `0.0026` → IC=+0.159 (n=285)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.4397` → IC=+0.144 (n=259)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.4397 (IC base=+0.141)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.203` → IC=+0.207 (n=73)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.203 (IC base=+0.141)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.145 (n=260)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` < 0.0066 (IC base=+0.129)

- **PATRÓN** `sigma_h` > `0.0035` → IC=+0.135 (n=294)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0035 (IC base=+0.129)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.173 (n=151)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 15.0 (IC base=+0.129)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.149 (n=132)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` < 7.0 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` > `0.8699` → IC=+0.218 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8699 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.463` → IC=+0.234 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.463 (IC base=+0.129)
...
- **PATRÓN** `sigma_h` > `0.0118` → IC=+0.193 (n=148)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0118 (IC base=+0.131)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.172 (n=114)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 18.0 (IC base=+0.131)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.139 (n=117)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` < 6.0 (IC base=+0.131)

- **PATRÓN** `dist_vwap_pct` > `0.1909` → IC=+0.169 (n=122)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.328 (n=283)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.304 (n=253)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.299)

- **PATRÓN** `dist_vwap_pct` > `0.1637` → IC=+0.316 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1637 (IC base=+0.299)

- **PATRÓN** `dist_vwap_pct` < `0.5016` → IC=+0.307 (n=288)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5016 (IC base=+0.299)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.623` → IC=+0.315 (n=257)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.623 (IC base=+0.299)
...
- **PATRÓN** `sigma_h` > `0.0131` → IC=+0.187 (n=362)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.1063` → IC=+0.155 (n=587)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.1063 (IC base=+0.130)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.147` → IC=+0.296 (n=155)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.147 (IC base=+0.130)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.126 (n=837)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` < 12.0 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` < `0.515` → IC=+0.140 (n=1005)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.515 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.961` → IC=+0.133 (n=213)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` > 5.961 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.187` → IC=+0.132 (n=959)
...
- **PATRÓN** `sigma_h` < `0.0065` → IC=+0.126 (n=284)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.206` → IC=+0.136 (n=105)
  ...
  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` > 3.206 (IC base=+0.084)
...
- **PATRÓN** `sigma_h` > `0.0137` → IC=+0.138 (n=78)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0137 (IC base=+0.088)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.134 (n=110)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` < 8.0 (IC base=+0.088)

- **PATRÓN** `dist_vwap_pct` > `0.2877` → IC=+0.125 (n=62)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` > 0.2877 (IC base=+0.088)

- **PATRÓN** `dist_vwap_pct` < `0.1237` → IC=+0.127 (n=116)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.1237 (IC base=+0.088)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.452` → IC=+0.298 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.452 (IC base=+0.088)
...
- **PATRÓN** `sigma_h` > `0.0194` → IC=+0.228 (n=123)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.238 (n=139)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.5022` → IC=+0.215 (n=349)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5022 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.27` → IC=+0.228 (n=332)
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 392 | +0.145 | +18.29€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 392 | +0.145 | +18.29€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 4094 | +0.187 | +66.70€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 2663 | +0.218 | +97.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 175 | +0.071 | -2.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 557 | +0.105 | -75.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 699 | +0.161 | +47.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 1188 | +0.198 | +48.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 887 | +0.211 | +4.08€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 58 | +0.100 | +2.89€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 233 | +0.189 | +45.64€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH | 1457 | +0.181 | +15.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 884 | +0.221 | +45.24€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 58 | +0.000 | -9.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 282 | +0.109 | -36.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 233 | +0.155 | +16.03€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#SOL | 1431 | +0.184 | +1.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 878 | +0.224 | +51.27€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 59 | +0.107 | +3.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 261 | +0.108 | -39.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 233 | +0.134 | -14.35€ | 0 | 5 |
...
| ✅ GBM_LATE_15M | 6040 | +0.108 | +2136.53€ | 0 | 10 |
| ✅ GBM_LATE_15M#15min | 6040 | +0.108 | +2136.53€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1437 | +0.082 | +306.21€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1437 | +0.082 | +306.21€ | 0 | 8 |
| ✅ GBM_LATE_15M#ETH | 1374 | +0.079 | +274.27€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1374 | +0.079 | +274.27€ | 0 | 6 |
| ✅ GBM_LATE_15M#SOL | 1638 | +0.100 | +647.77€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1638 | +0.100 | +647.77€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1591 | +0.165 | +908.28€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1591 | +0.165 | +908.28€ | 0 | 14 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 3260 | +0.154 | +2022.65€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 3260 | +0.154 | +2022.65€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 829 | +0.121 | +432.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 829 | +0.121 | +432.64€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 855 | +0.115 | +411.63€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 855 | +0.115 | +411.63€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 826 | +0.116 | +370.05€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 826 | +0.116 | +370.05€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 750 | +0.278 | +808.33€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 750 | +0.278 | +808.33€ | 0 | 12 |
...
| ✅ GBM_LATE_15M_TARDIO | 3223 | +0.103 | +1202.94€ | 0 | 10 |
| ✅ GBM_LATE_15M_TARDIO#15min | 3223 | +0.103 | +1202.94€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 766 | +0.074 | +178.52€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 766 | +0.074 | +178.52€ | 0 | 8 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 775 | +0.041 | +93.40€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 775 | +0.041 | +93.40€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 733 | +0.057 | +171.78€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 733 | +0.057 | +171.78€ | 1 | 7 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 949 | +0.213 | +759.24€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 949 | +0.213 | +759.24€ | 0 | 13 |
| ✅ GBM_LATE_5M | 149 | +0.063 | +10.96€ | 0 | 0 |
| ✅ GBM_LATE_5M#5min | 149 | +0.063 | +10.96€ | 0 | 0 |
...
| ✅ GBM_LATE_5M#XRP | 80 | +0.134 | +15.02€ | 0 | 0 |
| ✅ GBM_LATE_5M#XRP#5min | 80 | +0.134 | +15.02€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 748 | +0.172 | +331.14€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 748 | +0.172 | +331.14€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 180 | +0.126 | +66.91€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 180 | +0.126 | +66.91€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.202: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.054 n=819 | neutral: IC=-0.013 n=604 | overbought(IBS>0.7): IC=+0.189 n=776
  - _Datos_: n=2348 IC=+0.089 PNL=+503.92€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.119 n=14435 | tras_1loss IC=+0.093 n=9239 | tras_2loss IC=+0.059 n=3727/40 | gap=+0.060 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=21411 IC=+0.123 PNL=+6166.00€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=21411 IC=+0.123 PNL=+6166.00€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=671 IC=-0.087 PNL=+114.03€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=671 IC=-0.087 PNL=+114.03€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=756 IC=+0.073 PNL=+134.18€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=756 IC=+0.073 PNL=+134.18€
...
**🟡 H-CUSTOM-LATE-04H-ASIA** — GBM_LATE_15M 04h-05h UTC — media sesión asiática (gemela de GBM-04H-ASIA)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.129 > 0.08 con n=496 PNL=+192.21€
  - _Datos_: n=496 IC=+0.129 PNL=+192.21€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema, sin filtrar todavía
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.184 < -0.1 con n=498 PNL=-62.11€
  - _Datos_: n=498 IC=-0.184 PNL=-62.11€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.136 > 0.08 con n=306 PNL=+119.37€
  - _Datos_: n=306 IC=+0.136 PNL=+119.37€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.161 > 0.08 con n=317 PNL=+166.10€
  - _Datos_: n=317 IC=+0.161 PNL=+166.10€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.166 > 0.08 con n=390 PNL=+259.31€
  - _Datos_: n=390 IC=+0.166 PNL=+259.31€
