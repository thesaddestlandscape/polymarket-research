# Hipótesis automáticas — 2026-08-26 11:30 UTC
_Generado por shadow_postmortem.py sobre 161255 resoluciones (PNL=+11532.22€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `148.79` → IC=-0.278 (n=2236)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 148.79
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=6711)
...
- **FILTRO** `restante_s_al_confirmar` < `157.37` → IC=-0.202 (n=609)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 157.37
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=1830)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `150.08` → IC=-0.273 (n=532)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 150.08
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=1597)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `156.92` → IC=-0.343 (n=509)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 156.92
  - _Potencial_: sin este filtro IC_bueno=-0.116 (n=1036)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.188 (n=5145)

  - _Acción_: Kelly boost +0.94€ cuando `py_entrada` > 0.7 (IC base=+0.098)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.172 (n=1535)

  - _Acción_: Kelly boost +0.86€ cuando `libro_spread` < 0.01 (IC base=+0.098)

- **PATRÓN** `libro_liquidez` > `2382.8902` → IC=+0.175 (n=1482)
...
- **PATRÓN** `py_entrada` < `0.34` → IC=+0.259 (n=3141)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.223 (n=608)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.213 (n=750)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.209)

- **PATRÓN** `libro_liquidez` > `12477.3281` → IC=+0.214 (n=197)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12477.3281 (IC base=+0.209)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=522)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.119)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.138 (n=451)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` < 15.0 (IC base=+0.119)

- **PATRÓN** `py_entrada` > `0.555` → IC=+0.140 (n=526)

  - _Acción_: Kelly boost +0.70€ cuando `py_entrada` > 0.555 (IC base=+0.119)

- **PATRÓN** `libro_liquidez` > `4948.9204` → IC=+0.162 (n=202)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 4948.9204 (IC base=+0.119)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.136 (n=1157)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 5.0 (IC base=+0.126)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.131 (n=991)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` < `0.335` → IC=+0.210 (n=153)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.178 (n=1068)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.165 (n=1100)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 5.0 (IC base=+0.161)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.169 (n=940)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.243 (n=979)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.232)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.232 (n=844)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.196 (n=1051)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 5.0 (IC base=+0.187)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.191 (n=909)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `restante_min` > `4.93` → IC=+0.160 (n=1376)

  - _Acción_: Kelly boost +0.80€ cuando `restante_min` > 4.93 (IC base=+0.132)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=4319)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `4.5` → IC=+0.160 (n=1379)

  - _Acción_: Kelly boost +0.80€ cuando `lag_apertura_s` < 4.5 (IC base=+0.132)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.203 (n=777)
...
- **PATRÓN** `restante_min` > `4.9` → IC=+0.169 (n=723)
...
- **PATRÓN** `lag_apertura_s` < `5.71` → IC=+0.172 (n=684)
...
- **PATRÓN** `restante_min` < `3.98` → IC=+0.136 (n=698)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.126 (n=2165)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `3.47` → IC=+0.161 (n=695)

  - _Acción_: Kelly boost +0.81€ cuando `lag_apertura_s` < 3.47 (IC base=+0.124)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9508` → IC=+0.216 (n=702)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9508 (IC base=+0.066)

- **PATRÓN** `dist_vwap_pct` > `0.2097` → IC=+0.234 (n=288)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2097 (IC base=+0.066)

- **PATRÓN** `dist_vwap_pct` < `1.0239` → IC=+0.219 (n=482)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 1.0239 (IC base=+0.066)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.215` → IC=+0.175 (n=859)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 5.215 (IC base=+0.066)

- **PATRÓN** `volumen_regimen` < `0.63` → IC=+0.230 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.63 (IC base=+0.066)

- **PATRÓN** `volumen_regimen` > `1.0608` → IC=+0.266 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0608 (IC base=+0.066)

- **PATRÓN** `volumen_pendiente_norm` > `0.3033` → IC=+0.141 (n=157)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_pendiente_norm` > 0.3033 (IC base=+0.066)

- **PATRÓN** `volumen_spike_ratio` < `1.5713` → IC=+0.144 (n=493)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 1.5713 (IC base=+0.066)

- **PATRÓN** `volumen_spike_ratio` > `1.9327` → IC=+0.132 (n=746)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 1.9327 (IC base=+0.066)

- **PATRÓN** `ibs_20min` < `0.2155` → IC=+0.128 (n=1449)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` < 0.2155 (IC base=+0.029)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1751` → IC=+0.252 (n=220)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1751 (IC base=+0.029)
...
- **PATRÓN** `volumen_spike_ratio` > `2.9243` → IC=+0.231 (n=240)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9243 (IC base=+0.029)

- **PATRÓN** `ballena_activa_n` < `40.0` → IC=+0.237 (n=196)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 40.0 (IC base=+0.029)

### GBM_LATE_15M#BNB#15min
- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.183 (n=216)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0071 (IC base=+0.117)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.294 (n=163)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0061 (IC base=+0.278)

- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.319 (n=81)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0072 (IC base=+0.278)

- **PATRÓN** `drift_60min` |x|≤ `0.1459` → IC=+0.342 (n=162)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1459 (IC base=+0.278)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.283 (n=224)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.278)

- **PATRÓN** `hora_utc` < `14.0` → IC=+0.281 (n=222)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 14.0 (IC base=+0.278)

- **PATRÓN** `ibs_20min` < `0.4667` → IC=+0.319 (n=213)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4667 (IC base=+0.278)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.511` → IC=+0.316 (n=280)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.511 (IC base=+0.278)

- **PATRÓN** `volumen_pendiente_norm` < `0.0596` → IC=+0.318 (n=119)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2302` → IC=+0.311 (n=35)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2302 (IC base=+0.278)

- **PATRÓN** `volumen_spike_ratio` < `1.7714` → IC=+0.300 (n=93)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7714 (IC base=+0.278)

- **PATRÓN** `volumen_spike_ratio` > `1.5394` → IC=+0.294 (n=124)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5394 (IC base=+0.278)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.326 (n=113)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.278)

- **PATRÓN** `libro_liquidez` > `1990.5404` → IC=+0.319 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1990.5404 (IC base=+0.278)

- **PATRÓN** `ballena_activa_n` < `59.0` → IC=+0.305 (n=75)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3375` → IC=+0.221 (n=306)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3375 (IC base=+0.214)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.239 (n=362)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.214)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.218 (n=314)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.214)

- **PATRÓN** `ibs_20min` > `0.9595` → IC=+0.244 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9595 (IC base=+0.214)

- **PATRÓN** `dist_vwap_pct` > `0.308` → IC=+0.235 (n=209)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.308 (IC base=+0.214)

- **PATRÓN** `dist_vwap_pct` < `0.5536` → IC=+0.217 (n=298)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5536 (IC base=+0.214)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.274` → IC=+0.247 (n=93)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.274 (IC base=+0.214)

- **PATRÓN** `volumen_regimen` < `0.703` → IC=+0.216 (n=153)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.703 (IC base=+0.214)

- **PATRÓN** `volumen_regimen` > `1.0851` → IC=+0.263 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0851 (IC base=+0.214)

- **PATRÓN** `volumen_pendiente_norm` < `0.0752` → IC=+0.214 (n=309)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0752 (IC base=+0.214)

- **PATRÓN** `volumen_pendiente_norm` > `0.2657` → IC=+0.261 (n=44)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2657 (IC base=+0.214)

- **PATRÓN** `volumen_spike_ratio` < `1.4455` → IC=+0.272 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4455 (IC base=+0.214)

- **PATRÓN** `libro_liquidez` > `11188.0133` → IC=+0.225 (n=311)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11188.0133 (IC base=+0.214)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0684` → IC=+0.129 (n=141)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.65€ cuando `drift_60min` |x|≤ 0.0684 (IC base=+0.130)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.924` → IC=+0.265 (n=113)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.924 (IC base=+0.130)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.165 (n=198)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.01 (IC base=+0.130)

- **PATRÓN** `libro_liquidez` > `1917.9762` → IC=+0.144 (n=189)

  - _Acción_: Kelly boost +0.72€ cuando `libro_liquidez` > 1917.9762 (IC base=+0.130)

- **PATRÓN** `ballena_activa_n` < `27.0` → IC=+0.130 (n=90)

  - _Acción_: Kelly boost +0.65€ cuando `ballena_activa_n` < 27.0 (IC base=+0.130)
...
- **FILTRO** `ibs_20min` > `0.8732` → IC=-0.174 (n=188)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.162 (n=63)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=690)
...
- **PATRÓN** `volumen_regimen` < `1.1445` → IC=+0.324 (n=32)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.1445 (IC base=-0.030)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1127` → IC=+0.309 (n=19)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1127 (IC base=-0.030)

- **PATRÓN** `volumen_spike_ratio` > `1.8068` → IC=+0.333 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8068 (IC base=-0.030)
...
- **FILTRO** `ibs_20min` > `0.4842` → IC=-0.124 (n=623)

  - _Acción_: SKIP cuando `ibs_20min` > 0.4842
...
- **FILTRO** `sigma_ewma_delta_pct` > `4.919` → IC=-0.153 (n=266)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=982)
...
- **FILTRO** `drift_60min` |x|> `0.498` → IC=-0.136 (n=215)
...
- **PATRÓN** `sigma_h` > `0.0074` → IC=+0.139 (n=1213)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0074 (IC base=+0.053)

- **PATRÓN** `ibs_20min` > `0.9464` → IC=+0.247 (n=891)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9464 (IC base=+0.053)

- **PATRÓN** `dist_vwap_pct` > `1.2509` → IC=+0.286 (n=222)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2509 (IC base=+0.053)

- **PATRÓN** `volumen_regimen` > `1.1596` → IC=+0.199 (n=307)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1596 (IC base=+0.053)

- **PATRÓN** `volumen_pendiente_norm` < `0.0883` → IC=+0.168 (n=1128)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_pendiente_norm` < 0.0883 (IC base=+0.053)

- **PATRÓN** `volumen_pendiente_norm` > `0.2437` → IC=+0.213 (n=235)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2437 (IC base=+0.053)

- **PATRÓN** `volumen_spike_ratio` < `1.4787` → IC=+0.192 (n=404)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` < 1.4787 (IC base=+0.053)

- **PATRÓN** `volumen_spike_ratio` > `2.8541` → IC=+0.172 (n=404)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 2.8541 (IC base=+0.053)

- **PATRÓN** `ballena_activa_n` < `99.0` → IC=+0.270 (n=658)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 99.0 (IC base=+0.053)

- **PATRÓN** `ibs_20min` < `0.0992` → IC=+0.176 (n=1151)

  - _Acción_: Kelly boost +0.88€ cuando `ibs_20min` < 0.0992 (IC base=+0.037)

- **PATRÓN** `dist_vwap_pct` > `0.6647` → IC=+0.211 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6647 (IC base=+0.037)

- **PATRÓN** `dist_vwap_pct` < `0.1466` → IC=+0.199 (n=689)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.1466 (IC base=+0.037)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2533` → IC=+0.362 (n=121)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2533 (IC base=+0.037)

- **PATRÓN** `volumen_spike_ratio` > `2.3783` → IC=+0.283 (n=265)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3783 (IC base=+0.037)

- **PATRÓN** `ballena_activa_n` < `58.0` → IC=+0.249 (n=421)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 58.0 (IC base=+0.037)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `sigma_ewma_delta_pct` > `5.138` → IC=-0.213 (n=120)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0684` → IC=+0.207 (n=138)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0684 (IC base=+0.179)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.027` → IC=+0.289 (n=121)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.027 (IC base=+0.179)

- **PATRÓN** `volumen_pendiente_norm` < `0.1458` → IC=+0.186 (n=304)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` < 0.1458 (IC base=+0.179)

- **PATRÓN** `volumen_pendiente_norm` > `0.4252` → IC=+0.208 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.4252 (IC base=+0.179)

- **PATRÓN** `volumen_spike_ratio` < `2.0483` → IC=+0.185 (n=144)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` < 2.0483 (IC base=+0.179)

- **PATRÓN** `volumen_spike_ratio` > `3.7364` → IC=+0.187 (n=148)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 3.7364 (IC base=+0.179)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.222 (n=192)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.179)

- **PATRÓN** `libro_liquidez` > `1915.5084` → IC=+0.217 (n=185)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1915.5084 (IC base=+0.179)

- **PATRÓN** `sigma_h` < `0.0081` → IC=+0.364 (n=211)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0081 (IC base=+0.344)

- **PATRÓN** `drift_60min` |x|≤ `0.3424` → IC=+0.350 (n=185)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3424 (IC base=+0.344)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.383 (n=143)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.344)

- **PATRÓN** `ibs_20min` < `0.3056` → IC=+0.354 (n=211)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3056 (IC base=+0.344)

- **PATRÓN** `ibs_20min` > `0.1368` → IC=+0.345 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.1368 (IC base=+0.344)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.534` → IC=+0.346 (n=245)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.534 (IC base=+0.344)

- **PATRÓN** `volumen_pendiente_norm` > `0.1402` → IC=+0.379 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1402 (IC base=+0.344)

- **PATRÓN** `volumen_spike_ratio` < `1.7842` → IC=+0.381 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7842 (IC base=+0.344)

- **PATRÓN** `volumen_spike_ratio` > `1.997` → IC=+0.333 (n=106)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.997 (IC base=+0.344)

- **PATRÓN** `libro_liquidez` > `1903.1828` → IC=+0.347 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1903.1828 (IC base=+0.344)

- **PATRÓN** `ballena_activa_n` < `19.0` → IC=+0.308 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 19.0 (IC base=+0.344)
...
- **FILTRO** `ibs_20min` < `0.5333` → IC=-0.149 (n=300)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.130 (n=301)
...
- **PATRÓN** `ibs_20min` > `0.8511` → IC=+0.245 (n=151)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8511 (IC base=-0.009)

- **PATRÓN** `dist_vwap_pct` > `0.3189` → IC=+0.250 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3189 (IC base=-0.009)

- **PATRÓN** `volumen_regimen` < `0.8622` → IC=+0.172 (n=114)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 0.8622 (IC base=-0.009)

- **PATRÓN** `volumen_regimen` > `1.1426` → IC=+0.195 (n=57)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 1.1426 (IC base=-0.009)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5794` → IC=+0.157 (n=141)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.5794 (IC base=-0.009)

- **PATRÓN** `ballena_activa_n` < `77.0` → IC=+0.198 (n=124)

  - _Acción_: Kelly boost +0.99€ cuando `ballena_activa_n` < 77.0 (IC base=-0.009)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` > `0.0236` → IC=+0.323 (n=179)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0236 (IC base=+0.216)
...
- **PATRÓN** `ibs_20min` > `0.9016` → IC=+0.292 (n=358)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9016 (IC base=+0.216)

- **PATRÓN** `dist_vwap_pct` > `0.8873` → IC=+0.327 (n=154)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8873 (IC base=+0.216)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.169` → IC=+0.277 (n=316)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.169 (IC base=+0.216)

- **PATRÓN** `volumen_regimen` > `0.8368` → IC=+0.253 (n=358)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8368 (IC base=+0.216)

- **PATRÓN** `volumen_pendiente_norm` < `0.0807` → IC=+0.222 (n=455)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0807 (IC base=+0.216)

- **PATRÓN** `volumen_pendiente_norm` > `0.2775` → IC=+0.275 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2775 (IC base=+0.216)

- **PATRÓN** `volumen_spike_ratio` < `1.4052` → IC=+0.253 (n=160)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4052 (IC base=+0.216)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.233 (n=598)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.216)

- **PATRÓN** `libro_liquidez` > `2663.0389` → IC=+0.233 (n=358)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2663.0389 (IC base=+0.216)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2774` → IC=+0.272 (n=367)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2774 (IC base=+0.265)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.275 (n=509)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.265)

- **PATRÓN** `ibs_20min` < `0.2766` → IC=+0.323 (n=484)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2766 (IC base=+0.265)

- **PATRÓN** `dist_vwap_pct` < `0.244` → IC=+0.275 (n=523)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.244 (IC base=+0.265)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.694` → IC=+0.310 (n=93)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.694 (IC base=+0.265)

- **PATRÓN** `volumen_regimen` > `1.2513` → IC=+0.301 (n=184)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1847` → IC=+0.297 (n=180)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1847 (IC base=+0.265)

- **PATRÓN** `ballena_activa_n` < `40.0` → IC=+0.247 (n=295)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 40.0 (IC base=+0.265)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` > `0.0097` → IC=+0.202 (n=770)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0097 (IC base=+0.157)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.164 (n=2403)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 5.0 (IC base=+0.157)

- **PATRÓN** `ibs_20min` > `0.8932` → IC=+0.259 (n=1536)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8932 (IC base=+0.157)

- **PATRÓN** `dist_vwap_pct` > `0.7907` → IC=+0.252 (n=539)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7907 (IC base=+0.157)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.328` → IC=+0.255 (n=982)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.328 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` < `1.2168` → IC=+0.161 (n=1601)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2168 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` > `0.686` → IC=+0.174 (n=1431)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 0.686 (IC base=+0.157)

- **PATRÓN** `volumen_pendiente_norm` > `0.102` → IC=+0.185 (n=817)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.102 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` < `2.2771` → IC=+0.156 (n=1810)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.2771 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` > `1.8546` → IC=+0.148 (n=1371)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.8546 (IC base=+0.157)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.169 (n=1893)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.01 (IC base=+0.157)

- **PATRÓN** `libro_liquidez` > `3834.3661` → IC=+0.194 (n=768)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 3834.3661 (IC base=+0.157)

- **PATRÓN** `ballena_activa_n` < `125.0` → IC=+0.180 (n=1071)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 125.0 (IC base=+0.157)

- **PATRÓN** `sigma_h` < `0.0084` → IC=+0.198 (n=2056)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0084 (IC base=+0.184)

- **PATRÓN** `drift_60min` |x|≤ `0.391` → IC=+0.200 (n=2055)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.391 (IC base=+0.184)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.217 (n=941)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.184)

- **PATRÓN** `ibs_20min` < `0.4062` → IC=+0.236 (n=2055)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4062 (IC base=+0.184)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2897` → IC=+0.245 (n=237)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2897 (IC base=+0.184)

- **PATRÓN** `volumen_spike_ratio` < `1.8709` → IC=+0.176 (n=965)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 1.8709 (IC base=+0.184)

- **PATRÓN** `volumen_spike_ratio` > `2.6218` → IC=+0.209 (n=482)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6218 (IC base=+0.184)

- **PATRÓN** `ballena_activa_n` < `204.0` → IC=+0.178 (n=1014)
...
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.236 (n=108)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0028 (IC base=+0.184)

- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.227 (n=108)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0072 (IC base=+0.184)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.221 (n=338)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.184)

- **PATRÓN** `ibs_20min` > `0.5086` → IC=+0.222 (n=322)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5086 (IC base=+0.184)

- **PATRÓN** `dist_vwap_pct` > `0.211` → IC=+0.250 (n=242)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.211 (IC base=+0.184)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.683` → IC=+0.262 (n=82)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.683 (IC base=+0.184)

- **PATRÓN** `volumen_regimen` < `0.6321` → IC=+0.209 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6321 (IC base=+0.184)

- **PATRÓN** `volumen_regimen` > `1.0851` → IC=+0.209 (n=146)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0851 (IC base=+0.184)

- **PATRÓN** `volumen_pendiente_norm` > `0.2657` → IC=+0.271 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2657 (IC base=+0.184)

- **PATRÓN** `volumen_spike_ratio` < `1.3897` → IC=+0.250 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3897 (IC base=+0.184)

- **PATRÓN** `libro_liquidez` > `11952.667` → IC=+0.210 (n=215)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11952.667 (IC base=+0.184)
...
- **PATRÓN** `sigma_h` < `0.0086` → IC=+0.301 (n=204)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0086 (IC base=+0.280)

- **PATRÓN** `drift_60min` |x|≤ `0.1873` → IC=+0.326 (n=136)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1873 (IC base=+0.280)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.332 (n=93)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.425` → IC=+0.333 (n=46)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.425 (IC base=+0.280)

- **PATRÓN** `volumen_pendiente_norm` > `0.399` → IC=+0.420 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.399 (IC base=+0.280)

- **PATRÓN** `volumen_spike_ratio` > `2.0261` → IC=+0.288 (n=116)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.0261 (IC base=+0.280)

- **PATRÓN** `ballena_activa_n` < `50.0` → IC=+0.236 (n=85)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 50.0 (IC base=+0.280)

### GBM_LATE_15M_MULTIHORIZONTE#ETH#15min
- **PATRÓN** `sigma_h` < `0.0094` → IC=+0.218 (n=324)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0094 (IC base=+0.186)

- **PATRÓN** `drift_60min` |x|≤ `0.4992` → IC=+0.205 (n=324)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4992 (IC base=+0.186)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.207 (n=326)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.186)

- **PATRÓN** `ibs_20min` > `0.4615` → IC=+0.242 (n=324)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `12.407` → IC=+0.344 (n=62)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.407 (IC base=+0.186)

- **PATRÓN** `volumen_regimen` > `0.6353` → IC=+0.203 (n=324)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6353 (IC base=+0.186)

- **PATRÓN** `volumen_pendiente_norm` > `0.101` → IC=+0.254 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.101 (IC base=+0.186)

- **PATRÓN** `volumen_spike_ratio` < `1.4363` → IC=+0.220 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4363 (IC base=+0.186)

- **PATRÓN** `volumen_spike_ratio` > `2.4478` → IC=+0.226 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4478 (IC base=+0.186)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.189 (n=365)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.01 (IC base=+0.186)

- **PATRÓN** `libro_liquidez` > `8624.0494` → IC=+0.202 (n=216)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8624.0494 (IC base=+0.186)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.186 (n=173)

  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 156.0 (IC base=+0.186)
...
- **PATRÓN** `sigma_h` > `0.0116` → IC=+0.211 (n=154)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0116 (IC base=+0.118)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.133 (n=478)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 5.0 (IC base=+0.118)

- **PATRÓN** `ibs_20min` > `0.8519` → IC=+0.235 (n=307)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8519 (IC base=+0.118)

- **PATRÓN** `dist_vwap_pct` > `0.7959` → IC=+0.269 (n=145)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7959 (IC base=+0.118)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.203` → IC=+0.294 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.203 (IC base=+0.118)

- **PATRÓN** `volumen_regimen` > `0.6783` → IC=+0.137 (n=411)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 0.6783 (IC base=+0.118)

- **PATRÓN** `volumen_pendiente_norm` > `0.097` → IC=+0.128 (n=162)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_pendiente_norm` > 0.097 (IC base=+0.118)

- **PATRÓN** `volumen_spike_ratio` > `1.5678` → IC=+0.129 (n=383)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.5678 (IC base=+0.118)

- **PATRÓN** `libro_liquidez` > `2917.9058` → IC=+0.187 (n=209)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 2917.9058 (IC base=+0.118)
...
- **PATRÓN** `sigma_h` > `0.0268` → IC=+0.202 (n=166)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0268 (IC base=+0.165)

- **PATRÓN** `drift_60min` |x|≤ `0.1619` → IC=+0.188 (n=219)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.1619 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.188 (n=171)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 17.0 (IC base=+0.165)

- **PATRÓN** `ibs_20min` > `0.7` → IC=+0.211 (n=497)
...
- **PATRÓN** `volumen_regimen` < `1.1907` → IC=+0.167 (n=496)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.1907 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` > `0.8284` → IC=+0.194 (n=331)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 0.8284 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` > `0.0804` → IC=+0.223 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0804 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` < `2.1508` → IC=+0.180 (n=404)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 2.1508 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `1.8043` → IC=+0.169 (n=306)
...
- **PATRÓN** `ibs_20min` > `0.6252` → IC=+0.150 (n=347)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` > 0.6252 (IC base=+0.084)

- **PATRÓN** `dist_vwap_pct` > `0.7807` → IC=+0.240 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7807 (IC base=+0.084)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1717` → IC=+0.190 (n=98)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.1717 (IC base=+0.084)

- **PATRÓN** `libro_liquidez` > `2940.9508` → IC=+0.124 (n=259)

  - _Acción_: Kelly boost +0.62€ cuando `libro_liquidez` > 2940.9508 (IC base=+0.084)
...
- **PATRÓN** `hora_utc` > `10.0` → IC=+0.129 (n=68)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 10.0 (IC base=+0.029)

- **PATRÓN** `ibs_20min` > `0.9306` → IC=+0.222 (n=34)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9306 (IC base=+0.029)

- **PATRÓN** `dist_vwap_pct` > `0.7947` → IC=+0.250 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7947 (IC base=+0.029)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.351` → IC=+0.139 (n=34)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` > 4.351 (IC base=+0.029)

- **PATRÓN** `libro_liquidez` > `11186.7795` → IC=+0.135 (n=50)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 11186.7795 (IC base=+0.029)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.213 (n=1214)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0078 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.156 (n=2398)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 8.0 (IC base=+0.147)

- **PATRÓN** `ibs_20min` > `0.9449` → IC=+0.288 (n=1213)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9449 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `1.0677` → IC=+0.251 (n=432)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0677 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.527` → IC=+0.229 (n=1603)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.527 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `0.6307` → IC=+0.156 (n=631)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.6307 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` > `1.081` → IC=+0.158 (n=858)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 1.081 (IC base=+0.147)

- **PATRÓN** `volumen_pendiente_norm` > `0.1651` → IC=+0.186 (n=677)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1651 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` < `2.3246` → IC=+0.144 (n=2076)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 2.3246 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` > `1.8809` → IC=+0.158 (n=1572)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.8809 (IC base=+0.147)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.155 (n=2188)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `3790.2792` → IC=+0.199 (n=892)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3790.2792 (IC base=+0.147)

- **PATRÓN** `ballena_activa_n` < `162.0` → IC=+0.193 (n=1143)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 162.0 (IC base=+0.147)

- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.225 (n=1599)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0064 (IC base=+0.191)

- **PATRÓN** `drift_60min` |x|≤ `0.4421` → IC=+0.201 (n=2396)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4421 (IC base=+0.191)
...
- **PATRÓN** `ibs_20min` < `0.5529` → IC=+0.246 (n=2396)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5529 (IC base=+0.191)

- **PATRÓN** `dist_vwap_pct` < `0.7107` → IC=+0.179 (n=1964)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.7107 (IC base=+0.191)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.418` → IC=+0.222 (n=350)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.418 (IC base=+0.191)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.649` → IC=+0.195 (n=2250)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` < 2.649 (IC base=+0.191)

- **PATRÓN** `volumen_regimen` < `0.6238` → IC=+0.187 (n=608)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` < 0.6238 (IC base=+0.191)

- **PATRÓN** `volumen_regimen` > `1.1947` → IC=+0.189 (n=608)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` > 1.1947 (IC base=+0.191)

- **PATRÓN** `volumen_pendiente_norm` > `0.282` → IC=+0.269 (n=253)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.282 (IC base=+0.191)

- **PATRÓN** `volumen_spike_ratio` > `2.2624` → IC=+0.209 (n=772)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2624 (IC base=+0.191)

- **PATRÓN** `ballena_activa_n` < `183.0` → IC=+0.161 (n=1268)
...
- **PATRÓN** `ibs_20min` > `0.8271` → IC=+0.248 (n=288)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8271 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.428` → IC=+0.355 (n=164)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.428 (IC base=+0.135)

- **PATRÓN** `volumen_pendiente_norm` > `0.2113` → IC=+0.187 (n=65)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.2113 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` > `1.8937` → IC=+0.133 (n=235)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 1.8937 (IC base=+0.135)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.313 (n=169)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.280)

- **PATRÓN** `drift_60min` |x|≤ `0.1523` → IC=+0.329 (n=168)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1523 (IC base=+0.280)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.282 (n=264)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5876` → IC=+0.338 (n=251)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5876 (IC base=+0.280)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.039` → IC=+0.309 (n=276)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.039 (IC base=+0.280)

- **PATRÓN** `volumen_pendiente_norm` < `0.069` → IC=+0.301 (n=134)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.069 (IC base=+0.280)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4811` → IC=+0.292 (n=152)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4811 (IC base=+0.280)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.306 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.280)

- **PATRÓN** `libro_liquidez` > `1997.9259` → IC=+0.337 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1997.9259 (IC base=+0.280)

- **PATRÓN** `ballena_activa_n` < `74.0` → IC=+0.278 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 74.0 (IC base=+0.280)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.182 (n=420)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 6.0 (IC base=+0.160)

- **PATRÓN** `ibs_20min` > `0.3404` → IC=+0.211 (n=420)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3404 (IC base=+0.160)

- **PATRÓN** `dist_vwap_pct` > `0.2633` → IC=+0.247 (n=247)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2633 (IC base=+0.160)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.757` → IC=+0.215 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.757 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` < `0.6538` → IC=+0.178 (n=141)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` < 0.6538 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` > `1.0896` → IC=+0.200 (n=191)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0896 (IC base=+0.160)

- **PATRÓN** `volumen_pendiente_norm` > `0.2054` → IC=+0.231 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2054 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` < `2.0884` → IC=+0.190 (n=327)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` < 2.0884 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` > `1.708` → IC=+0.196 (n=248)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.708 (IC base=+0.160)

- **PATRÓN** `libro_liquidez` > `11706.7568` → IC=+0.195 (n=280)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 11706.7568 (IC base=+0.160)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0757` → IC=+0.183 (n=118)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.0757 (IC base=+0.175)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.241 (n=164)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.175)

- **PATRÓN** `ibs_20min` > `0.7131` → IC=+0.254 (n=315)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7131 (IC base=+0.175)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.106` → IC=+0.338 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.106 (IC base=+0.175)

- **PATRÓN** `volumen_pendiente_norm` < `0.1461` → IC=+0.178 (n=256)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` < 0.1461 (IC base=+0.175)

- **PATRÓN** `volumen_spike_ratio` < `2.1217` → IC=+0.180 (n=123)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 2.1217 (IC base=+0.175)

- **PATRÓN** `volumen_spike_ratio` > `3.8461` → IC=+0.174 (n=127)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 3.8461 (IC base=+0.175)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.191 (n=241)

  - _Acción_: Kelly boost +0.96€ cuando `libro_spread` < 0.03 (IC base=+0.175)

- **PATRÓN** `libro_liquidez` > `1855.8284` → IC=+0.192 (n=235)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 1855.8284 (IC base=+0.175)

- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.227 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 61.0 (IC base=+0.175)
...
- **PATRÓN** `sigma_h` < `0.0089` → IC=+0.150 (n=424)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0089 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.168 (n=381)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 8.0 (IC base=+0.142)

- **PATRÓN** `ibs_20min` > `0.3576` → IC=+0.200 (n=424)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3576 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.9062` → IC=+0.214 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9062 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.544` → IC=+0.198 (n=203)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` > 4.544 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `0.6307` → IC=+0.194 (n=142)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` < 0.6307 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` > `1.1936` → IC=+0.167 (n=142)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 1.1936 (IC base=+0.142)
...
- **PATRÓN** `dist_vwap_pct` > `0.1609` → IC=+0.152 (n=179)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.1609 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` < `0.7008` → IC=+0.171 (n=372)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.7008 (IC base=+0.147)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1935` → IC=+0.179 (n=257)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 2.1935 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` > `1.4579` → IC=+0.172 (n=291)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.4579 (IC base=+0.147)
...
- **PATRÓN** `sigma_h` > `0.0106` → IC=+0.171 (n=217)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0106 (IC base=+0.094)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.132 (n=321)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 12.0 (IC base=+0.094)

- **PATRÓN** `ibs_20min` > `0.9091` → IC=+0.299 (n=217)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9091 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` > `0.7694` → IC=+0.250 (n=130)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7694 (IC base=+0.094)
...
- **PATRÓN** `libro_liquidez` > `2900.261` → IC=+0.276 (n=159)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2900.261 (IC base=+0.094)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.187 (n=196)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.0058 (IC base=+0.115)

- **PATRÓN** `drift_60min` |x|≤ `0.259` → IC=+0.136 (n=297)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.259 (IC base=+0.115)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.178 (n=203)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 15.0 (IC base=+0.115)

- **PATRÓN** `ibs_20min` < `0.5758` → IC=+0.196 (n=445)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` < 0.5758 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` < `0.4633` → IC=+0.135 (n=403)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.4633 (IC base=+0.115)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.994` → IC=+0.132 (n=435)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` < 2.994 (IC base=+0.115)

- **PATRÓN** `volumen_regimen` < `0.7028` → IC=+0.151 (n=196)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.7028 (IC base=+0.115)

- **PATRÓN** `volumen_pendiente_norm` > `0.072` → IC=+0.172 (n=120)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.072 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` > `1.7467` → IC=+0.167 (n=199)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.7467 (IC base=+0.115)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.124 (n=362)

  - _Acción_: Kelly boost +0.62€ cuando `libro_spread` < 0.01 (IC base=+0.115)

- **PATRÓN** `libro_liquidez` > `2537.3478` → IC=+0.162 (n=202)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 2537.3478 (IC base=+0.115)
...
- **PATRÓN** `ibs_20min` > `0.9732` → IC=+0.305 (n=259)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.048` → IC=+0.255 (n=312)
...
- **PATRÓN** `volumen_regimen` > `1.041` → IC=+0.194 (n=259)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 1.041 (IC base=+0.173)

- **PATRÓN** `volumen_pendiente_norm` > `0.1717` → IC=+0.258 (n=130)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1717 (IC base=+0.173)

- **PATRÓN** `volumen_spike_ratio` > `1.8321` → IC=+0.185 (n=347)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` > 1.8321 (IC base=+0.173)
...
- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.220 (n=301)
...
- **PATRÓN** `drift_60min` |x|≤ `0.6436` → IC=+0.222 (n=623)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.6436 (IC base=+0.211)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.222 (n=574)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.211)

- **PATRÓN** `ibs_20min` < `0.4884` → IC=+0.278 (n=623)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4884 (IC base=+0.211)

- **PATRÓN** `dist_vwap_pct` < `1.1388` → IC=+0.221 (n=725)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.021` → IC=+0.217 (n=626)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2851` → IC=+0.306 (n=70)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2851 (IC base=+0.211)

- **PATRÓN** `volumen_spike_ratio` > `2.5663` → IC=+0.230 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5663 (IC base=+0.211)

- **PATRÓN** `ballena_activa_n` < `32.0` → IC=+0.160 (n=298)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 32.0 (IC base=+0.211)

### GBM_LATE_5M
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.148 (n=880)
...
- **PATRÓN** `sigma_h` < `0.0034` → IC=+0.199 (n=224)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0034 (IC base=+0.142)

- **PATRÓN** `drift_60min` |x|≤ `0.3507` → IC=+0.163 (n=592)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.82€ cuando `drift_60min` |x|≤ 0.3507 (IC base=+0.142)
...
- **PATRÓN** `ibs_20min` < `0.6273` → IC=+0.153 (n=591)
...
- **PATRÓN** `dist_vwap_pct` < `0.8053` → IC=+0.140 (n=762)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.8053 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.126` → IC=+0.161 (n=299)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 4.126 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.046` → IC=+0.142 (n=677)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` < 6.046 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `1.2237` → IC=+0.156 (n=661)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4869` → IC=+0.161 (n=665)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.4869 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` > `1.5137` → IC=+0.149 (n=594)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.5137 (IC base=+0.142)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.148 (n=880)

  - _Acción_: Kelly boost +0.74€ cuando `libro_spread` < 0.01 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `8304.7792` → IC=+0.148 (n=672)
...
- **PATRÓN** `sigma_h` < `0.0044` → IC=+0.179 (n=247)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0044 (IC base=+0.134)

- **PATRÓN** `drift_60min` |x|≤ `0.3361` → IC=+0.143 (n=368)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.3361 (IC base=+0.134)
...
- **PATRÓN** `ibs_20min` < `0.144` → IC=+0.171 (n=162)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` < 0.144 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` > `0.6099` → IC=+0.172 (n=59)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.6099 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.369` → IC=+0.144 (n=374)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 6.369 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` < `1.1834` → IC=+0.143 (n=368)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.1834 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` > `0.6229` → IC=+0.141 (n=368)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 0.6229 (IC base=+0.134)

- **PATRÓN** `volumen_pendiente_norm` > `0.1485` → IC=+0.190 (n=114)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.1485 (IC base=+0.134)

- **PATRÓN** `volumen_spike_ratio` < `2.5675` → IC=+0.150 (n=367)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 2.5675 (IC base=+0.134)

- **PATRÓN** `volumen_spike_ratio` > `1.7958` → IC=+0.136 (n=245)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` > 1.7958 (IC base=+0.134)

- **PATRÓN** `libro_liquidez` > `11826.2881` → IC=+0.138 (n=329)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 11826.2881 (IC base=+0.134)
...
- **PATRÓN** `sigma_h` < `0.0089` → IC=+0.182 (n=209)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0089 (IC base=+0.155)

- **PATRÓN** `sigma_h` > `0.0063` → IC=+0.164 (n=138)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.0063 (IC base=+0.155)

- **PATRÓN** `drift_60min` |x|≤ `0.4064` → IC=+0.203 (n=183)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4064 (IC base=+0.155)

- **PATRÓN** `hora_utc` < `9.0` → IC=+0.183 (n=140)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 9.0 (IC base=+0.155)

- **PATRÓN** `ibs_20min` < `0.6071` → IC=+0.154 (n=183)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.6071 (IC base=+0.155)

- **PATRÓN** `ibs_20min` > `0.089` → IC=+0.189 (n=207)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.089 (IC base=+0.155)

- **PATRÓN** `dist_vwap_pct` > `0.1899` → IC=+0.159 (n=86)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.1899 (IC base=+0.155)

- **PATRÓN** `dist_vwap_pct` < `0.7772` → IC=+0.161 (n=231)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` < 0.7772 (IC base=+0.155)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.388` → IC=+0.199 (n=91)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` > 4.388 (IC base=+0.155)

- **PATRÓN** `volumen_regimen` < `0.6768` → IC=+0.264 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6768 (IC base=+0.155)

- **PATRÓN** `volumen_pendiente_norm` < `0.1284` → IC=+0.173 (n=212)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` < 0.1284 (IC base=+0.155)

- **PATRÓN** `volumen_spike_ratio` < `2.1395` → IC=+0.207 (n=179)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1395 (IC base=+0.155)

- **PATRÓN** `volumen_spike_ratio` > `1.4319` → IC=+0.183 (n=203)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 1.4319 (IC base=+0.155)

- **PATRÓN** `libro_liquidez` > `10397.5155` → IC=+0.204 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 10397.5155 (IC base=+0.155)
...
### LEADLAG_BTC_XRP_15M
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.127 (n=108)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 6.0 (IC base=+0.107)

- **PATRÓN** `libro_liquidez` > `2605.1459` → IC=+0.205 (n=93)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.169 (n=742)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=2284)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.219 (n=713)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=2286)

- **FILTRO** `ibs_20min` > `0.2734` → IC=-0.182 (n=749)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2734
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=2250)
...
- **FILTRO** `ballena_activa_n` > `79.0` → IC=-0.172 (n=126)
...
- **FILTRO** `ibs_20min` < `0.7273` → IC=-0.207 (n=114)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7273
  - _Potencial_: sin este filtro IC_bueno=+0.009 (n=342)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.201 (n=165)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.038 (n=340)

- **FILTRO** `ibs_20min` > `0.7608` → IC=-0.211 (n=126)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.192 (n=141)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.013 (n=390)

- **FILTRO** `ballena_activa_n` > `5.0` → IC=-0.151 (n=167)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.237 (n=112)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=363)

- **FILTRO** `drift_20min_pct` |x|> `0.2855` → IC=-0.144 (n=161)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.141 (n=2175)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=5249)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.285 (n=1797)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=5627)

- **FILTRO** `ibs_7min` < `0.7355` → IC=-0.233 (n=1856)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7355
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=5568)

- **FILTRO** `ballena_activa_n` > `20.0` → IC=-0.174 (n=2464)

  - _Acción_: SKIP cuando `ballena_activa_n` > 20.0
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=4960)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.230 (n=2110)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=7010)

- **FILTRO** `ibs_7min` > `0.7241` → IC=-0.170 (n=2278)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7241
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=6842)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.161 (n=237)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.094 (n=774)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.330 (n=245)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=766)

- **FILTRO** `ibs_7min` < `0.9818` → IC=-0.202 (n=666)

  - _Acción_: SKIP cuando `ibs_7min` < 0.9818
  - _Potencial_: sin este filtro IC_bueno=+0.068 (n=345)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.266 (n=250)

  - _Acción_: SKIP cuando `ballena_activa_n` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=761)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.235 (n=372)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=1175)

- **FILTRO** `drift_7min_pct` |x|> `0.1399` → IC=-0.160 (n=525)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1399
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=1022)

- **FILTRO** `ibs_7min` > `0.837` → IC=-0.204 (n=386)

  - _Acción_: SKIP cuando `ibs_7min` > 0.837
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=1161)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.156 (n=318)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=1169)

- **FILTRO** `py_entrada` < `0.38` → IC=-0.242 (n=363)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.004 (n=1124)

- **FILTRO** `ibs_7min` < `0.8058` → IC=-0.160 (n=371)

  - _Acción_: SKIP cuando `ibs_7min` < 0.8058
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=1116)

- **FILTRO** `ballena_activa_n` > `153.0` → IC=-0.167 (n=370)

  - _Acción_: SKIP cuando `ballena_activa_n` > 153.0
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=1117)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.231 (n=359)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=1161)

- **FILTRO** `ballena_activa_n` > `123.0` → IC=-0.173 (n=377)

  - _Acción_: SKIP cuando `ballena_activa_n` > 123.0
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=1143)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.208 (n=293)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.091 (n=749)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.338 (n=245)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=797)

- **FILTRO** `ibs_7min` < `0.2143` → IC=-0.279 (n=260)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2143
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=782)

- **FILTRO** `ballena_activa_n` > `17.0` → IC=-0.285 (n=254)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.266 (n=344)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=1252)

- **FILTRO** `ibs_7min` > `0.8192` → IC=-0.188 (n=398)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8192
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=1198)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.150 (n=389)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=884)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.267 (n=299)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=974)

- **FILTRO** `ibs_7min` < `0.7649` → IC=-0.188 (n=318)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7649
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=955)

- **FILTRO** `ballena_activa_n` > `43.0` → IC=-0.211 (n=316)

  - _Acción_: SKIP cuando `ballena_activa_n` > 43.0
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=957)

- **FILTRO** `hora_utc` > `15.0` → IC=-0.126 (n=410)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=842)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.285 (n=301)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=951)

- **FILTRO** `ibs_7min` > `0.1818` → IC=-0.157 (n=424)

  - _Acción_: SKIP cuando `ibs_7min` > 0.1818
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=828)

- **FILTRO** `ballena_activa_n` > `38.0` → IC=-0.209 (n=307)

  - _Acción_: SKIP cuando `ballena_activa_n` > 38.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=945)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.4` → IC=-0.230 (n=327)

  - _Acción_: SKIP cuando `py_entrada` < 0.4
  - _Potencial_: sin este filtro IC_bueno=+0.012 (n=1065)

- **FILTRO** `ibs_7min` < `0.7759` → IC=-0.194 (n=348)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7759
  - _Potencial_: sin este filtro IC_bueno=+0.005 (n=1044)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.200 (n=338)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.188 (n=395)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=1279)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.295 (n=295)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=924)
...
- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.249 (n=293)
  ...
  - _Acción_: SKIP cuando `ballena_activa_n` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.004 (n=926)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.277 (n=307)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=1224)

- **FILTRO** `ibs_7min` > `0.8346` → IC=-0.162 (n=382)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8346
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=1149)
...
- **PATRÓN** `hora_utc` < `14.0` → IC=+0.128 (n=143)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=1133)
...
- **PATRÓN** `ibs_15` < `0.1129` → IC=+0.134 (n=410)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.67€ cuando `ibs_15` < 0.1129 (IC base=+0.026)

### UPDOWN_GBM#5min
- **FILTRO** `ibs_15` < `0.2` → IC=-0.220 (n=105)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: SKIP cuando `ibs_15` < 0.2
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=317)
...
- **PATRÓN** `ibs_15` < `0.125` → IC=+0.202 (n=102)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.193` → IC=-0.179 (n=325)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 17.193
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=2315)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.887` → IC=-0.136 (n=207)
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.125 sube el IC de +0.048 a +0.202 en UPDOWN_GBM#XRP#15min (n=102). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 6488 | -0.098 | -871.13€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 669 | -0.089 | -118.30€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 5819 | -0.099 | -752.84€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1061 | +0.023 | -112.87€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1061 | +0.023 | -112.87€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 2129 | -0.072 | -101.23€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2129 | -0.072 | -101.23€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1545 | -0.191 | -345.27€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1545 | -0.191 | -345.27€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 30146 | +0.114 | -1961.66€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 5636 | +0.187 | -187.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 107 | -0.105 | -50.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 21278 | +0.095 | -1689.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3125 | +0.121 | -34.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 3571 | +0.063 | -608.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 20 | -0.045 | +2.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 3546 | +0.065 | -604.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 6260 | +0.134 | -128.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1589 | +0.197 | -86.80€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 3541 | +0.110 | -77.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1088 | +0.130 | +57.73€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 3580 | +0.078 | -472.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 20 | +0.045 | +2.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 3559 | +0.078 | -473.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 6716 | +0.130 | -67.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2049 | +0.170 | -7.30€ | 0 | 8 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 3543 | +0.115 | -34.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1112 | +0.104 | -16.77€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#SOL | 6448 | +0.131 | -427.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1941 | +0.203 | -97.39€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 46 | +0.000 | -9.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 3536 | +0.094 | -245.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 925 | +0.131 | -75.41€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 3571 | +0.104 | -256.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 17 | -0.022 | -0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 3553 | +0.105 | -254.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 5611 | +0.174 | -432.36€ | 3 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 5611 | +0.174 | -432.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 1413 | +0.166 | -151.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 1413 | +0.166 | -151.52€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 1400 | +0.161 | -165.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 1400 | +0.161 | -165.44€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 1256 | +0.232 | -32.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 1256 | +0.232 | -32.24€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 1352 | +0.187 | -99.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 1352 | +0.187 | -99.49€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 15218 | +0.192 | -1318.29€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 15218 | +0.192 | -1318.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 2794 | +0.126 | -512.67€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 2794 | +0.126 | -512.67€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 2361 | +0.239 | -41.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 2361 | +0.239 | -41.57€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 2626 | +0.163 | -338.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 2626 | +0.163 | -338.39€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 2413 | +0.233 | -62.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 2413 | +0.233 | -62.08€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 2458 | +0.219 | -112.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 2458 | +0.219 | -112.02€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 2566 | +0.185 | -251.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 2566 | +0.185 | -251.57€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 5502 | +0.132 | +175.50€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 5502 | +0.132 | +175.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 2733 | +0.140 | +125.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 2733 | +0.140 | +125.74€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 2769 | +0.124 | +49.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 2769 | +0.124 | +49.76€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0 | 206 | +0.096 | -1.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#240min | 62 | +0.094 | -0.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#60min | 144 | +0.096 | -0.87€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#ETH | 169 | +0.102 | +0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#ETH#240min | 25 | +0.130 | +1.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#ETH#60min | 144 | +0.096 | -0.87€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 5537 | +0.096 | -203.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 570 | +0.054 | -32.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 4967 | +0.101 | -170.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC | 3826 | +0.095 | -98.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#15min | 570 | +0.054 | -32.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#5min | 3256 | +0.102 | -65.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL | 1711 | +0.098 | -104.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL#5min | 1711 | +0.098 | -104.82€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 