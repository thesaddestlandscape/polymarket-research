# Hipótesis automáticas — 2026-07-15 23:35 UTC
_Generado por shadow_postmortem.py sobre 16103 resoluciones (PNL=+3275.84€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.188 (n=795)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.172 (n=1041)

  - _Acción_: Kelly boost +0.86€ cuando `libro_spread` < 0.01 (IC base=+0.162)

- **PATRÓN** `libro_liquidez` > `7415.5891` → IC=+0.191 (n=305)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 7415.5891 (IC base=+0.162)
...
- **PATRÓN** `libro_liquidez` > `7271.3027` → IC=+0.212 (n=175)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7271.3027 (IC base=+0.203)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.243 (n=68)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `4733.8122` → IC=+0.223 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4733.8122 (IC base=+0.205)
...
- **PATRÓN** `py_entrada` > `0.625` → IC=+0.282 (n=163)
...
### GBM_LATE_15M
- **PATRÓN** `dist_vwap_pct` > `0.6294` → IC=+0.127 (n=124)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` > 0.6294 (IC base=+0.109)
...
- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.133 (n=1205)
...
- **PATRÓN** `sigma_h` > `0.0234` → IC=+0.180 (n=198)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0234 (IC base=+0.148)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.161 (n=163)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.6767` → IC=+0.227 (n=31)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6767 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.138` → IC=+0.179 (n=26)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 9.138 (IC base=+0.148)
...
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.222 (n=318)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0108 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.171 (n=472)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1538` → IC=+0.197 (n=265)
  ...
  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1538 (IC base=+0.161)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.176 (n=717)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0048 (IC base=+0.160)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.164 (n=495)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.4443` → IC=+0.192 (n=488)
...
- **PATRÓN** `sigma_h` < `0.0082` → IC=+0.191 (n=66)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0082 (IC base=+0.101)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.157 (n=68)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` < 5.0 (IC base=+0.101)

- **PATRÓN** `dist_vwap_pct` < `0.1968` → IC=+0.170 (n=113)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` < 0.1968 (IC base=+0.101)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.243` → IC=+0.157 (n=106)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` < 3.243 (IC base=+0.101)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` < `0.0186` → IC=+0.276 (n=150)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0186 (IC base=+0.256)

- **PATRÓN** `sigma_h` > `0.0101` → IC=+0.257 (n=150)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.099` → IC=+0.336 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.099 (IC base=+0.256)
...
- **PATRÓN** `sigma_h` > `0.0149` → IC=+0.193 (n=216)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0149 (IC base=+0.112)
...
- **PATRÓN** `dist_vwap_pct` > `0.7118` → IC=+0.175 (n=115)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.7118 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` < `0.1434` → IC=+0.149 (n=220)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.1434 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.671` → IC=+0.328 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.671 (IC base=+0.112)
...
- **PATRÓN** `sigma_h` > `0.0114` → IC=+0.220 (n=187)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.6119` → IC=+0.269 (n=37)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6119 (IC base=+0.205)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.069` → IC=+0.208 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.069 (IC base=+0.205)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.36` → IC=+0.217 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 11.36 (IC base=+0.205)
...
- **PATRÓN** `dist_vwap_pct` > `0.9263` → IC=+0.152 (n=21)
  ...
  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.9263 (IC base=+0.049)

- **PATRÓN** `sigma_ewma_delta_pct` < `16.748` → IC=+0.138 (n=136)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2336 | +0.174 | -49.55€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1506 | +0.208 | +10.22€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 678 | +0.179 | -15.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 504 | +0.200 | -19.77€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 831 | +0.173 | +0.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 499 | +0.217 | +21.82€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 820 | +0.170 | -31.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 496 | +0.211 | +11.91€ | 0 | 5 |
...
| ✅ GBM_LATE_15M | 4721 | +0.103 | +1471.74€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 4721 | +0.103 | +1471.74€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1134 | +0.065 | +161.66€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1134 | +0.065 | +161.66€ | 0 | 3 |
| ✅ GBM_LATE_15M#ETH | 1140 | +0.076 | +200.42€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1140 | +0.076 | +200.42€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1259 | +0.111 | +509.60€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1259 | +0.111 | +509.60€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1188 | +0.156 | +600.06€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1188 | +0.156 | +600.06€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1966 | +0.151 | +1156.32€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1966 | +0.151 | +1156.32€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 531 | +0.114 | +236.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 531 | +0.114 | +236.98€ | 1 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 410 | +0.274 | +426.93€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 410 | +0.274 | +426.93€ | 0 | 11 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 51 | +0.085 | +14.31€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 51 | +0.085 | +14.31€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 26 | +0.321 | +17.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 26 | +0.321 | +17.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 1925 | +0.089 | +572.71€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1925 | +0.089 | +572.71€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 478 | +0.060 | +87.26€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 478 | +0.060 | +87.26€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 438 | +0.018 | +13.63€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 438 | +0.018 | +13.63€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 463 | +0.055 | +84.71€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 463 | +0.055 | +84.71€ | 2 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 546 | +0.199 | +387.11€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 546 | +0.199 | +387.11€ | 0 | 11 |
...
| ✅ UPDOWN_GBM | 1574 | +0.010 | +74.03€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1316 | +0.037 | +111.07€ | 0 | 7 |
| 🚫 UPDOWN_GBM#240min | 11 | -0.148 | -4.31€ | 0 | 0 |
| 🚫 UPDOWN_GBM#5min | 75 | -0.162 | -22.48€ | 4 | 0 |
| ✅ UPDOWN_GBM#60min | 127 | -0.058 | -10.76€ | 7 | 0 |
| ✅ UPDOWN_GBM#BNB | 80 | +0.073 | +18.88€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 80 | +0.073 | +18.88€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 71 | +0.199 | +22.26€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 71 | +0.199 | +22.26€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 26 | +0.143 | +7.90€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 26 | +0.143 | +7.90€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.140) — sin ventaja clara. oversold(IBS<0.3): IC=+0.005 n=459 | neutral: IC=-0.019 n=364 | overbought(IBS>0.7): IC=+0.121 n=373
  - _Datos_: n=1264 IC=+0.039 PNL=+121.49€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9338 | tras_1loss IC=+0.065 n=6546 | tras_2loss IC=+0.033 n=2809/40 | gap=+0.069 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1447 IC=+0.017 PNL=+88.03€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1447 IC=+0.017 PNL=+88.03€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=582 IC=+0.034 PNL=+42.67€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=582 IC=+0.034 PNL=+42.67€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=13645 IC=+0.105 PNL=+3351.72€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=13645 IC=+0.105 PNL=+3351.72€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.164 > 0.1 con n=400 PNL=+138.33€
  - _Datos_: n=400 IC=+0.164 PNL=+138.33€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=603 IC=+0.055 PNL=+71.25€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=603 IC=+0.055 PNL=+71.25€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.149 > 0.08 con n=146 PNL=+53.96€
  - _Datos_: n=146 IC=+0.149 PNL=+53.96€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.146 > 0.08 con n=179 PNL=+99.78€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.205 > 0.08 con n=120 PNL=+4.54€
  - _Datos_: n=120 IC=+0.205 PNL=+4.54€
