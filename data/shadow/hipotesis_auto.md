# Hipótesis automáticas — 2026-09-16 18:08 UTC
_Generado por shadow_postmortem.py sobre 470011 resoluciones (PNL=+50430.35€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `144.44` → IC=-0.252 (n=5814)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 144.44
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=17443)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `139.1` → IC=-0.279 (n=798)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 139.1
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=2395)
...
- **FILTRO** `restante_s_al_confirmar` < `157.97` → IC=-0.252 (n=1383)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 157.97
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=4151)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `121.29` → IC=-0.361 (n=1117)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 121.29
  - _Potencial_: sin este filtro IC_bueno=-0.101 (n=3352)
...
- **FILTRO** `py_entrada` > `0.57` → IC=-0.139 (n=120)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=395)

- **FILTRO** `py_entrada` < `0.48` → IC=-0.151 (n=124)
...
- **PATRÓN** `py_entrada` < `0.345` → IC=+0.246 (n=8036)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.345 (IC base=+0.135)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.172 (n=5786)

  - _Acción_: Kelly boost +0.86€ cuando `libro_spread` < 0.02 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `7222.9491` → IC=+0.177 (n=1829)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 7222.9491 (IC base=+0.135)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.210 (n=1388)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.201)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.201 (n=1375)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.202 (n=1713)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.201)

- **PATRÓN** `libro_liquidez` > `14976.1514` → IC=+0.208 (n=443)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 14976.1514 (IC base=+0.201)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.209 (n=1261)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.375` → IC=+0.264 (n=1269)
...
- **PATRÓN** `libro_liquidez` > `13192.4929` → IC=+0.214 (n=624)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13192.4929 (IC base=+0.202)
...
- **PATRÓN** `py_entrada` < `0.425` → IC=+0.166 (n=573)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.147 (n=2360)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.135 (n=641)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.144 (n=691)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.202 (n=9248)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.220 (n=3278)
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.178 (n=2289)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.319 (n=191)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.183 (n=2270)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.199 (n=2199)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.194 (n=1584)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.187 (n=5047)

  - _Acción_: Kelly boost +0.93€ cuando `py_entrada` > 0.71 (IC base=+0.170)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.225 (n=4886)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.269 (n=1738)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.75 (IC base=+0.219)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.207 (n=4529)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.255 (n=2301)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.74 (IC base=+0.203)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.192 (n=4595)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `3.3` → IC=+0.153 (n=3819)
...
- **PATRÓN** `restante_min` < `4.01` → IC=+0.135 (n=1899)

  - _Acción_: Kelly boost +0.67€ cuando `restante_min` < 4.01 (IC base=+0.130)

- **PATRÓN** `restante_min` > `4.93` → IC=+0.148 (n=2037)
...
- **PATRÓN** `lag_apertura_s` < `4.08` → IC=+0.152 (n=1893)

  - _Acción_: Kelly boost +0.76€ cuando `lag_apertura_s` < 4.08 (IC base=+0.130)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.201 (n=2073)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.38 (IC base=+0.120)

- **PATRÓN** `restante_min` < `4.48` → IC=+0.126 (n=2540)

  - _Acción_: Kelly boost +0.63€ cuando `restante_min` < 4.48 (IC base=+0.120)

- **PATRÓN** `restante_min` > `4.96` → IC=+0.147 (n=1966)

  - _Acción_: Kelly boost +0.74€ cuando `restante_min` > 4.96 (IC base=+0.120)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.124 (n=2195)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `2.5` → IC=+0.148 (n=1922)

  - _Acción_: Kelly boost +0.74€ cuando `lag_apertura_s` < 2.5 (IC base=+0.120)

### FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.300 (n=995)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.327 (n=315)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.063` → IC=+0.276 (n=615)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.227 (n=1118)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.211)

- **PATRÓN** `ibs_20min` > `0.9176` → IC=+0.251 (n=483)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9176 (IC base=+0.211)

- **PATRÓN** `dist_vwap_pct` > `0.1985` → IC=+0.221 (n=535)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1985 (IC base=+0.211)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.845` → IC=+0.234 (n=347)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.845 (IC base=+0.211)

- **PATRÓN** `volumen_regimen` < `1.2628` → IC=+0.222 (n=1065)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2628 (IC base=+0.211)

- **PATRÓN** `volumen_pendiente_norm` > `0.0991` → IC=+0.219 (n=386)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7485` → IC=+0.216 (n=692)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7485 (IC base=+0.211)

- **PATRÓN** `volumen_spike_ratio` > `2.368` → IC=+0.221 (n=346)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.368 (IC base=+0.211)

- **PATRÓN** `libro_liquidez` > `10979.8664` → IC=+0.222 (n=1065)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0749` → IC=+0.161 (n=381)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.0749 (IC base=+0.138)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.168 (n=381)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 18.0 (IC base=+0.138)

- **PATRÓN** `ibs_20min` < `0.6675` → IC=+0.174 (n=1141)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.6675 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.1278` → IC=+0.150 (n=1042)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.1278 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.49` → IC=+0.172 (n=193)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 11.49 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `1.2044` → IC=+0.146 (n=1141)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 1.2044 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `0.8472` → IC=+0.141 (n=762)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4292` → IC=+0.153 (n=1033)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.4292 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `13463.212` → IC=+0.153 (n=761)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 13463.212 (IC base=+0.138)

- **PATRÓN** `ballena_activa_n` < `423.0` → IC=+0.145 (n=948)

  - _Acción_: Kelly boost +0.73€ cuando `ballena_activa_n` < 423.0 (IC base=+0.138)
...
- **PATRÓN** `sigma_h` > `0.0058` → IC=+0.187 (n=1376)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0058 (IC base=+0.176)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.176 (n=1375)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.563` → IC=+0.211 (n=788)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.563 (IC base=+0.176)

- **PATRÓN** `volumen_pendiente_norm` < `0.1063` → IC=+0.181 (n=1169)
...
- **PATRÓN** `volumen_spike_ratio` < `1.846` → IC=+0.207 (n=466)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.846 (IC base=+0.215)

- **PATRÓN** `volumen_spike_ratio` > `2.3042` → IC=+0.223 (n=706)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3042 (IC base=+0.215)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.228 (n=633)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.215)

- **PATRÓN** `libro_liquidez` > `1884.62` → IC=+0.234 (n=396)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1884.62 (IC base=+0.215)
...
- **PATRÓN** `ibs_20min` > `0.9316` → IC=+0.168 (n=287)
...
- **PATRÓN** `dist_vwap_pct` < `0.4875` → IC=+0.326 (n=233)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4875 (IC base=+0.006)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.258` → IC=+0.134 (n=539)
...
- **PATRÓN** `volumen_regimen` > `1.1897` → IC=+0.331 (n=75)
...
- **PATRÓN** `volumen_regimen` < `0.6955` → IC=+0.152 (n=251)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.6955 (IC base=+0.004)

- **PATRÓN** `volumen_regimen` > `1.1669` → IC=+0.151 (n=190)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.1669 (IC base=+0.004)

- **PATRÓN** `volumen_pendiente_norm` > `0.2689` → IC=+0.243 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2689 (IC base=+0.004)

- **PATRÓN** `volumen_spike_ratio` > `1.5104` → IC=+0.184 (n=464)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` > 1.5104 (IC base=+0.004)
...
- **FILTRO** `ibs_20min` > `0.2692` → IC=-0.126 (n=1763)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2692
  - _Potencial_: sin este filtro IC_bueno=+0.117 (n=871)

- **FILTRO** `sigma_ewma_delta_pct` > `8.613` → IC=-0.201 (n=289)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.613
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=2345)
...
- **PATRÓN** `dist_vwap_pct` > `1.3203` → IC=+0.278 (n=16)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.3203 (IC base=+0.035)
...
- **PATRÓN** `volumen_regimen` > `1.0155` → IC=+0.328 (n=27)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0155 (IC base=+0.035)

- **PATRÓN** `volumen_spike_ratio` < `3.0245` → IC=+0.278 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.0245 (IC base=+0.035)

- **PATRÓN** `volumen_spike_ratio` > `1.5387` → IC=+0.278 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5387 (IC base=+0.035)
...
- **FILTRO** `drift_60min` |x|> `0.6568` → IC=-0.195 (n=437)
...
- **FILTRO** `ibs_20min` < `0.79` → IC=-0.140 (n=1313)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.082 (n=438)
...
- **FILTRO** `ibs_20min` > `0.7751` → IC=-0.201 (n=663)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7751
  - _Potencial_: sin este filtro IC_bueno=+0.032 (n=1992)

- **PATRÓN** `dist_vwap_pct` > `0.9754` → IC=+0.318 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9754 (IC base=-0.085)

- **PATRÓN** `dist_vwap_pct` < `0.2555` → IC=+0.300 (n=198)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2555 (IC base=-0.085)

- **PATRÓN** `volumen_regimen` > `0.6119` → IC=+0.286 (n=218)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6119 (IC base=-0.085)

- **PATRÓN** `volumen_pendiente_norm` > `0.0746` → IC=+0.283 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0746 (IC base=-0.085)

- **PATRÓN** `volumen_spike_ratio` < `2.4697` → IC=+0.277 (n=200)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.4697 (IC base=-0.085)

- **PATRÓN** `volumen_spike_ratio` > `1.8262` → IC=+0.278 (n=133)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8262 (IC base=-0.085)

- **PATRÓN** `dist_vwap_pct` > `1.0236` → IC=+0.267 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0236 (IC base=-0.026)

- **PATRÓN** `dist_vwap_pct` < `0.2677` → IC=+0.245 (n=562)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2677 (IC base=-0.026)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4427` → IC=+0.194 (n=181)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 1.4427 (IC base=+0.035)
...
- **FILTRO** `ibs_20min` > `0.8256` → IC=-0.147 (n=585)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8256
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=1759)

- **PATRÓN** `dist_vwap_pct` > `0.2992` → IC=+0.140 (n=251)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.2992 (IC base=+0.012)

- **PATRÓN** `dist_vwap_pct` < `0.1637` → IC=+0.128 (n=589)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.1637 (IC base=+0.012)

- **PATRÓN** `volumen_regimen` < `1.0507` → IC=+0.131 (n=602)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 1.0507 (IC base=+0.012)

- **PATRÓN** `volumen_regimen` > `0.6498` → IC=+0.137 (n=612)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 0.6498 (IC base=+0.012)

- **PATRÓN** `volumen_pendiente_norm` > `0.2205` → IC=+0.172 (n=123)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.2205 (IC base=+0.012)
...
- **PATRÓN** `dist_vwap_pct` < `0.1592` → IC=+0.207 (n=424)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1592 (IC base=+0.000)

- **PATRÓN** `volumen_regimen` > `1.0197` → IC=+0.219 (n=183)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0197 (IC base=+0.000)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1626` → IC=+0.221 (n=163)
...
- **PATRÓN** `sigma_h` > `0.0062` → IC=+0.266 (n=1111)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.238 (n=620)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.529` → IC=+0.269 (n=388)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.529 (IC base=+0.236)

- **PATRÓN** `volumen_pendiente_norm` < `0.1111` → IC=+0.253 (n=1043)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.253 (n=1409)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.236)

- **PATRÓN** `sigma_h` > `0.0055` → IC=+0.296 (n=993)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.3321` → IC=+0.285 (n=993)
...
- **FILTRO** `ibs_20min` < `0.2448` → IC=-0.212 (n=356)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.052 (n=1073)

- **FILTRO** `ibs_20min` > `0.8086` → IC=-0.183 (n=468)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8086
  - _Potencial_: sin este filtro IC_bueno=+0.037 (n=1406)
...
- **PATRÓN** `volumen_regimen` < `0.9565` → IC=+0.217 (n=291)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9565 (IC base=-0.014)

- **PATRÓN** `volumen_regimen` > `0.6129` → IC=+0.194 (n=295)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 0.6129 (IC base=-0.014)
...
- **PATRÓN** `ballena_activa_n` < `164.0` → IC=+0.243 (n=306)
...
- **PATRÓN** `dist_vwap_pct` < `0.2864` → IC=+0.173 (n=249)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.2864 (IC base=-0.018)

- **PATRÓN** `volumen_regimen` < `1.1494` → IC=+0.169 (n=240)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 1.1494 (IC base=-0.018)

- **PATRÓN** `volumen_regimen` > `0.7226` → IC=+0.181 (n=214)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` > 0.7226 (IC base=-0.018)
...
- **PATRÓN** `ballena_activa_n` < `151.0` → IC=+0.245 (n=202)
...
- **FILTRO** `ibs_20min` < `0.6667` → IC=-0.210 (n=836)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.254 (n=855)

- **FILTRO** `ibs_20min` > `0.7108` → IC=-0.231 (n=448)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7108
  - _Potencial_: sin este filtro IC_bueno=+0.084 (n=1346)

- **FILTRO** `sigma_ewma_delta_pct` > `4.678` → IC=-0.173 (n=417)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.678
  - _Potencial_: sin este filtro IC_bueno=+0.059 (n=1377)

- **PATRÓN** `ibs_20min` > `0.6667` → IC=+0.254 (n=855)
...
- **PATRÓN** `volumen_regimen` < `0.8616` → IC=+0.293 (n=390)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8616 (IC base=+0.025)

- **PATRÓN** `volumen_regimen` > `0.7191` → IC=+0.277 (n=522)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2759` → IC=+0.326 (n=84)
...
- **PATRÓN** `ibs_20min` < `0.1053` → IC=+0.196 (n=449)
...
- **PATRÓN** `dist_vwap_pct` < `0.1825` → IC=+0.197 (n=338)
  ...
  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.1825 (IC base=+0.005)

- **PATRÓN** `volumen_regimen` < `0.7171` → IC=+0.252 (n=167)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7171 (IC base=+0.005)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4946` → IC=+0.186 (n=345)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 1.4946 (IC base=+0.005)
...
- **PATRÓN** `ibs_20min` > `0.9024` → IC=+0.345 (n=703)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9024 (IC base=+0.270)

- **PATRÓN** `dist_vwap_pct` > `0.1852` → IC=+0.315 (n=591)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1852 (IC base=+0.270)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.398` → IC=+0.299 (n=566)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.398 (IC base=+0.270)

- **PATRÓN** `volumen_regimen` > `0.8543` → IC=+0.294 (n=703)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8543 (IC base=+0.270)

- **PATRÓN** `volumen_pendiente_norm` > `0.2365` → IC=+0.302 (n=210)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2365 (IC base=+0.270)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.438` → IC=+0.294 (n=420)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.438 (IC base=+0.269)

- **PATRÓN** `volumen_regimen` > `1.244` → IC=+0.310 (n=388)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0794` → IC=+0.203 (n=2001)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0794 (IC base=+0.172)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.205 (n=2331)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.172)

- **PATRÓN** `ibs_20min` < `0.461` → IC=+0.228 (n=6001)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.461 (IC base=+0.172)

- **PATRÓN** `dist_vwap_pct` < `0.2227` → IC=+0.162 (n=4487)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2227 (IC base=+0.172)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.238` → IC=+0.197 (n=1034)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 10.238 (IC base=+0.172)

- **PATRÓN** `volumen_regimen` < `1.1889` → IC=+0.156 (n=4387)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.1889 (IC base=+0.172)

- **PATRÓN** `volumen_regimen` > `0.6254` → IC=+0.151 (n=4387)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.6254 (IC base=+0.172)

- **PATRÓN** `volumen_pendiente_norm` > `0.2919` → IC=+0.228 (n=856)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2919 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` < `1.5751` → IC=+0.174 (n=2351)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.5751 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` > `2.6495` → IC=+0.176 (n=1781)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.6495 (IC base=+0.172)

- **PATRÓN** `ballena_activa_n` < `123.0` → IC=+0.173 (n=4960)
...
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.207 (n=349)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0078 (IC base=+0.186)

- **PATRÓN** `drift_60min` |x|≤ `0.3156` → IC=+0.203 (n=1041)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.0984` → IC=+0.275 (n=505)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0984 (IC base=+0.239)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0953` → IC=+0.235 (n=617)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.239 (n=785)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0751` → IC=+0.185 (n=300)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.0751 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.183 (n=950)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 5.0 (IC base=+0.161)

- **PATRÓN** `ibs_20min` > `0.4292` → IC=+0.221 (n=898)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `12.549` → IC=+0.223 (n=182)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.549 (IC base=+0.161)

- **PATRÓN** `volumen_regimen` < `1.2628` → IC=+0.173 (n=898)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` < 1.2628 (IC base=+0.161)
...
- **PATRÓN** `libro_liquidez` > `10070.4177` → IC=+0.177 (n=898)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 10070.4177 (IC base=+0.161)

- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.175 (n=899)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0049 (IC base=+0.147)

- **PATRÓN** `drift_60min` |x|≤ `0.0585` → IC=+0.194 (n=341)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.97€ cuando `drift_60min` |x|≤ 0.0585 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.164 (n=944)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 7.0 (IC base=+0.147)

- **PATRÓN** `ibs_20min` < `0.5309` → IC=+0.190 (n=1022)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` < 0.5309 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` < `0.1347` → IC=+0.166 (n=1042)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.1347 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.029` → IC=+0.217 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.029 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `1.2129` → IC=+0.160 (n=1022)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2129 (IC base=+0.147)

- **PATRÓN** `volumen_pendiente_norm` > `0.1586` → IC=+0.176 (n=316)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.1586 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` < `2.4267` → IC=+0.160 (n=912)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.4267 (IC base=+0.147)

- **PATRÓN** `ballena_activa_n` < `230.0` → IC=+0.154 (n=278)

  - _Acción_: Kelly boost +0.77€ cuando `ballena_activa_n` < 230.0 (IC base=+0.147)

### GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0059` → IC=+0.197 (n=1017)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0059 (IC base=+0.188)

- **PATRÓN** `drift_60min` |x|≤ `0.1945` → IC=+0.200 (n=678)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2175` → IC=+0.185 (n=975)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` < 0.2175 (IC base=+0.188)

- **PATRÓN** `volumen_pendiente_norm` > `0.1363` → IC=+0.184 (n=391)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.202 (n=1151)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4208` → IC=+0.167 (n=967)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.4208 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.163 (n=971)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 6.0 (IC base=+0.151)

- **PATRÓN** `ibs_20min` > `0.3996` → IC=+0.203 (n=967)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `12.083` → IC=+0.246 (n=179)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.083 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` < `0.8577` → IC=+0.160 (n=645)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.8577 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` > `1.1924` → IC=+0.171 (n=323)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 1.1924 (IC base=+0.151)
...
- **PATRÓN** `sigma_h` < `0.0071` → IC=+0.166 (n=1052)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0071 (IC base=+0.130)

- **PATRÓN** `drift_60min` |x|≤ `0.3789` → IC=+0.147 (n=1052)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.3789 (IC base=+0.130)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.179 (n=409)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 17.0 (IC base=+0.130)

- **PATRÓN** `ibs_20min` < `0.599` → IC=+0.180 (n=1052)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` < 0.599 (IC base=+0.130)
...
- **PATRÓN** `volumen_regimen` > `0.6094` → IC=+0.133 (n=1052)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7931` → IC=+0.139 (n=624)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` < 1.7931 (IC base=+0.130)

- **PATRÓN** `volumen_spike_ratio` > `2.4806` → IC=+0.143 (n=312)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 2.4806 (IC base=+0.130)

- **PATRÓN** `libro_liquidez` > `10039.916` → IC=+0.158 (n=477)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 10039.916 (IC base=+0.130)

- **PATRÓN** `ballena_activa_n` < `177.0` → IC=+0.128 (n=865)

  - _Acción_: Kelly boost +0.64€ cuando `ballena_activa_n` < 177.0 (IC base=+0.130)

### GBM_LATE_15M_MULTIHORIZONTE#SOL#15min
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.146 (n=766)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` > 0.0077 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.130 (n=1173)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 5.0 (IC base=+0.110)

- **PATRÓN** `ibs_20min` > `0.5` → IC=+0.194 (n=1155)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.458` → IC=+0.249 (n=257)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.172 (n=528)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 15.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` < `0.5484` → IC=+0.209 (n=1151)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5484 (IC base=+0.114)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.375` → IC=+0.161 (n=246)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 7.375 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` < `1.0388` → IC=+0.128 (n=1013)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` < 1.0388 (IC base=+0.114)

- **PATRÓN** `volumen_pendiente_norm` > `0.2785` → IC=+0.181 (n=139)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.2785 (IC base=+0.114)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1766` → IC=+0.134 (n=457)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 2.1766 (IC base=+0.114)
...
- **PATRÓN** `sigma_h` < `0.0272` → IC=+0.202 (n=1084)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0272 (IC base=+0.201)

- **PATRÓN** `sigma_h` > `0.0181` → IC=+0.211 (n=722)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.7277` → IC=+0.255 (n=968)
...
- **PATRÓN** `volumen_regimen` < `1.2068` → IC=+0.206 (n=1084)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1694` → IC=+0.256 (n=309)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1694 (IC base=+0.201)

- **PATRÓN** `volumen_spike_ratio` < `2.1881` → IC=+0.217 (n=916)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1881 (IC base=+0.201)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.203 (n=1108)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.201)

- **PATRÓN** `libro_liquidez` > `2568.2542` → IC=+0.202 (n=722)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2568.2542 (IC base=+0.201)

- **PATRÓN** `sigma_h` < `0.0078` → IC=+0.235 (n=390)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0078 (IC base=+0.205)

- **PATRÓN** `sigma_h` > `0.0222` → IC=+0.215 (n=528)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.220 (n=573)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.2688` → IC=+0.206 (n=1214)
...
- **PATRÓN** `volumen_regimen` > `0.6291` → IC=+0.218 (n=1163)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6291 (IC base=+0.205)

- **PATRÓN** `volumen_pendiente_norm` > `0.2819` → IC=+0.286 (n=157)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4611` → IC=+0.195 (n=1026)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 1.4611 (IC base=+0.205)

- **PATRÓN** `libro_liquidez` > `2541.6579` → IC=+0.216 (n=775)
...
- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.156 (n=498)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.178 (n=743)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 15.0 (IC base=+0.134)

- **PATRÓN** `ibs_20min` > `0.388` → IC=+0.165 (n=1488)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.043` → IC=+0.169 (n=518)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 6.043 (IC base=+0.134)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.139 (n=1652)

  - _Acción_: Kelly boost +0.69€ cuando `libro_spread` < 0.02 (IC base=+0.134)

- **PATRÓN** `libro_liquidez` > `8408.3868` → IC=+0.150 (n=675)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.135 (n=1056)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0057 (IC base=+0.116)

- **PATRÓN** `drift_60min` |x|≤ `0.3387` → IC=+0.129 (n=1393)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.64€ cuando `drift_60min` |x|≤ 0.3387 (IC base=+0.116)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.129 (n=1603)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 5.0 (IC base=+0.116)

- **PATRÓN** `ibs_20min` < `0.4839` → IC=+0.157 (n=1393)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` < 0.4839 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` < `0.2043` → IC=+0.123 (n=1413)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.2043 (IC base=+0.116)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.985` → IC=+0.138 (n=465)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` > 5.985 (IC base=+0.116)

- **PATRÓN** `volumen_regimen` < `1.2231` → IC=+0.122 (n=1404)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` < 1.2231 (IC base=+0.116)

- **PATRÓN** `volumen_pendiente_norm` > `0.1666` → IC=+0.143 (n=404)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_pendiente_norm` > 0.1666 (IC base=+0.116)

- **PATRÓN** `volumen_spike_ratio` < `2.2271` → IC=+0.136 (n=1332)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 2.2271 (IC base=+0.116)

- **PATRÓN** `ballena_activa_n` < `25.0` → IC=+0.125 (n=628)
...
- **PATRÓN** `drift_60min` |x|≤ `0.105` → IC=+0.153 (n=148)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.105 (IC base=+0.096)

- **PATRÓN** `hora_utc` > `10.0` → IC=+0.147 (n=304)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 10.0 (IC base=+0.096)

- **PATRÓN** `ibs_20min` > `0.2805` → IC=+0.132 (n=335)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` > 0.2805 (IC base=+0.096)

- **PATRÓN** `dist_vwap_pct` > `0.3004` → IC=+0.138 (n=103)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 0.3004 (IC base=+0.096)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.256` → IC=+0.154 (n=157)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 3.256 (IC base=+0.096)

- **PATRÓN** `volumen_regimen` < `0.6971` → IC=+0.167 (n=148)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.6971 (IC base=+0.096)
...
- **PATRÓN** `libro_liquidez` > `9834.2432` → IC=+0.129 (n=335)

  - _Acción_: Kelly boost +0.65€ cuando `libro_liquidez` > 9834.2432 (IC base=+0.096)

- **PATRÓN** `ballena_activa_n` < `151.0` → IC=+0.157 (n=103)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 151.0 (IC base=+0.096)

- **PATRÓN** `sigma_h` < `0.0026` → IC=+0.205 (n=164)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0026 (IC base=+0.126)

- **PATRÓN** `drift_60min` |x|≤ `0.3402` → IC=+0.148 (n=487)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.3402 (IC base=+0.126)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.145 (n=438)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 7.0 (IC base=+0.126)

- **PATRÓN** `ibs_20min` < `0.3419` → IC=+0.203 (n=325)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3419 (IC base=+0.126)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.384` → IC=+0.140 (n=198)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` > 4.384 (IC base=+0.126)

- **PATRÓN** `volumen_regimen` < `1.2002` → IC=+0.128 (n=487)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` < 1.2002 (IC base=+0.126)

- **PATRÓN** `volumen_regimen` > `1.0611` → IC=+0.173 (n=221)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` > 1.0611 (IC base=+0.126)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4163` → IC=+0.149 (n=477)
...
- **PATRÓN** `hora_utc` > `14.0` → IC=+0.136 (n=152)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 14.0 (IC base=+0.089)

- **PATRÓN** `ibs_20min` < `0.475` → IC=+0.160 (n=313)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` < 0.475 (IC base=+0.089)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.154` → IC=+0.124 (n=107)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 5.154 (IC base=+0.089)

- **PATRÓN** `volumen_regimen` < `0.7177` → IC=+0.150 (n=138)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 0.7177 (IC base=+0.089)

- **PATRÓN** `volumen_spike_ratio` < `2.513` → IC=+0.139 (n=289)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 2.513 (IC base=+0.089)

- **PATRÓN** `libro_liquidez` > `2922.7252` → IC=+0.139 (n=142)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 2922.7252 (IC base=+0.089)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4784` → IC=+0.184 (n=7184)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.4784 (IC base=+0.182)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.205 (n=2728)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.182)

- **PATRÓN** `ibs_20min` < `0.5586` → IC=+0.239 (n=7184)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5586 (IC base=+0.182)

- **PATRÓN** `dist_vwap_pct` < `0.2339` → IC=+0.163 (n=4576)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2339 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.842` → IC=+0.199 (n=1032)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.842 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.696` → IC=+0.183 (n=6943)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.696 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` < `0.7038` → IC=+0.159 (n=2186)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.7038 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` > `1.204` → IC=+0.161 (n=1656)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 1.204 (IC base=+0.182)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3003` → IC=+0.189 (n=2924)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` > 2.3003 (IC base=+0.182)

- **PATRÓN** `ballena_activa_n` < `124.0` → IC=+0.179 (n=6057)
...
- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.230 (n=597)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0071 (IC base=+0.192)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.198 (n=633)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.881` → IC=+0.313 (n=597)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.881 (IC base=+0.192)

- **PATRÓN** `volumen_pendiente_norm` > `0.2266` → IC=+0.239 (n=239)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2266 (IC base=+0.192)

- **PATRÓN** `volumen_spike_ratio` < `1.5561` → IC=+0.182 (n=539)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 1.5561 (IC base=+0.192)

- **PATRÓN** `volumen_spike_ratio` > `2.5691` → IC=+0.198 (n=409)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.5691 (IC base=+0.192)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.218 (n=1206)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.257 (n=1067)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.256)

- **PATRÓN** `libro_liquidez` > `1592.66` → IC=+0.270 (n=919)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.384` → IC=+0.153 (n=1103)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1542` → IC=+0.178 (n=340)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.1542 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` < `2.1158` → IC=+0.158 (n=1044)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.1158 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` > `1.7524` → IC=+0.156 (n=791)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.7524 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `10850.867` → IC=+0.166 (n=1107)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 10850.867 (IC base=+0.149)

- **PATRÓN** `ballena_activa_n` < `492.0` → IC=+0.160 (n=1117)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 492.0 (IC base=+0.149)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.169 (n=1109)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0057 (IC base=+0.154)

- **PATRÓN** `drift_60min` |x|≤ `0.2581` → IC=+0.168 (n=975)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.637` → IC=+0.206 (n=1108)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.637 (IC base=+0.154)

- **PATRÓN** `dist_vwap_pct` > `0.7002` → IC=+0.161 (n=169)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.7002 (IC base=+0.154)

- **PATRÓN** `dist_vwap_pct` < `0.1302` → IC=+0.163 (n=1025)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.1302 (IC base=+0.154)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.226` → IC=+0.167 (n=551)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` > 3.226 (IC base=+0.154)

- **PATRÓN** `volumen_regimen` < `1.1917` → IC=+0.161 (n=1108)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4046` → IC=+0.167 (n=1011)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 2.4046 (IC base=+0.154)

- **PATRÓN** `ballena_activa_n` < `371.0` → IC=+0.163 (n=610)

  - _Acción_: Kelly boost +0.82€ cuando `ballena_activa_n` < 371.0 (IC base=+0.154)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0058` → IC=+0.224 (n=1239)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0058 (IC base=+0.209)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.215 (n=1302)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_spike_ratio` < `1.7987` → IC=+0.226 (n=469)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7987 (IC base=+0.232)

- **PATRÓN** `volumen_spike_ratio` > `2.2499` → IC=+0.230 (n=710)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2499 (IC base=+0.232)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.243 (n=644)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.232)

- **PATRÓN** `libro_liquidez` > `1889.461` → IC=+0.239 (n=397)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1889.461 (IC base=+0.232)

- **PATRÓN** `ballena_activa_n` < `16.0` → IC=+0.254 (n=336)
...
- **PATRÓN** `volumen_spike_ratio` > `2.4563` → IC=+0.157 (n=424)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 2.4563 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `8770.6206` → IC=+0.230 (n=599)
...
- **PATRÓN** `sigma_h` < `0.0075` → IC=+0.160 (n=1069)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0075 (IC base=+0.140)

- **PATRÓN** `drift_60min` |x|≤ `0.4386` → IC=+0.156 (n=1069)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.4386 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.176 (n=402)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3284` → IC=+0.234 (n=713)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3284 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` < `0.2051` → IC=+0.145 (n=1006)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.2051 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.012` → IC=+0.202 (n=159)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.012 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `0.6938` → IC=+0.144 (n=470)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 0.6938 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` > `1.1877` → IC=+0.151 (n=356)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 1.1877 (IC base=+0.140)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5583` → IC=+0.146 (n=439)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 1.5583 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `2.4663` → IC=+0.169 (n=333)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 2.4663 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `10957.774` → IC=+0.196 (n=356)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 10957.774 (IC base=+0.140)

- **PATRÓN** `ballena_activa_n` < `192.0` → IC=+0.145 (n=991)

  - _Acción_: Kelly boost +0.73€ cuando `ballena_activa_n` < 192.0 (IC base=+0.140)

### GBM_LATE_15M_TARDIO#SOL#15min
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.165 (n=500)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 17.0 (IC base=+0.099)

- **PATRÓN** `ibs_20min` > `0.4667` → IC=+0.179 (n=1339)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.387` → IC=+0.219 (n=503)
...
- **PATRÓN** `drift_60min` |x|≤ `0.125` → IC=+0.153 (n=433)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.125 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.149 (n=608)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.449` → IC=+0.127 (n=1251)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5699` → IC=+0.121 (n=381)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_spike_ratio` > 2.5699 (IC base=+0.114)
...
- **PATRÓN** `sigma_h` > `0.0183` → IC=+0.211 (n=899)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0183 (IC base=+0.206)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.212 (n=1408)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_regimen` > `1.2323` → IC=+0.240 (n=498)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2324` → IC=+0.193 (n=1156)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 2.2324 (IC base=+0.200)

- **PATRÓN** `volumen_spike_ratio` > `1.443` → IC=+0.197 (n=1313)
...
- **PATRÓN** `libro_liquidez` > `2550.1344` → IC=+0.202 (n=995)
...
- **PATRÓN** `volumen_regimen` > `0.9067` → IC=+0.153 (n=1026)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.9067 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` > `0.1737` → IC=+0.173 (n=690)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.1737 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` < `1.4626` → IC=+0.163 (n=834)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 1.4626 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` > `1.9066` → IC=+0.158 (n=1668)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.9066 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `8197.4544` → IC=+0.154 (n=1147)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4871` → IC=+0.154 (n=1945)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.4871 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.169 (n=732)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `6.273` → IC=+0.145 (n=1929)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 6.273 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `1.1133` → IC=+0.144 (n=1624)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5724` → IC=+0.140 (n=1925)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.138 (n=2577)

  - _Acción_: Kelly boost +0.69€ cuando `libro_spread` < 0.01 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `7741.6466` → IC=+0.150 (n=1737)
...
- **PATRÓN** `sigma_h` > `0.0034` → IC=+0.173 (n=279)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.167 (n=322)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 5.0 (IC base=+0.157)

- **PATRÓN** `ibs_20min` < `0.5254` → IC=+0.200 (n=208)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5254 (IC base=+0.157)

- **PATRÓN** `dist_vwap_pct` > `0.2308` → IC=+0.181 (n=142)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.2308 (IC base=+0.157)

- **PATRÓN** `sigma_ewma_delta_pct` < `8.25` → IC=+0.168 (n=395)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` < 8.25 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` < `1.2372` → IC=+0.162 (n=312)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 1.2372 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` > `0.8262` → IC=+0.200 (n=208)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8262 (IC base=+0.157)

- **PATRÓN** `volumen_pendiente_norm` > `0.2301` → IC=+0.250 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2301 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` < `1.4419` → IC=+0.217 (n=104)
...
- **PATRÓN** `volumen_spike_ratio` > `2.7022` → IC=+0.198 (n=104)
  ...
  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.7022 (IC base=+0.157)

- **PATRÓN** `libro_liquidez` > `12609.7541` → IC=+0.208 (n=279)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.171 (n=335)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.6117` → IC=+0.144 (n=394)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` > 0.6117 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` > `0.6011` → IC=+0.155 (n=117)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.6011 (IC base=+0.135)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5735` → IC=+0.143 (n=866)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 2.5735 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` > `1.8205` → IC=+0.148 (n=577)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.8205 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `11358.9394` → IC=+0.150 (n=869)
...
- **PATRÓN** `ibs_20min` > `0.649` → IC=+0.219 (n=571)
...
- **PATRÓN** `volumen_regimen` < `1.0919` → IC=+0.127 (n=571)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` < 1.0919 (IC base=+0.103)
...
- **PATRÓN** `volumen_spike_ratio` < `2.456` → IC=+0.157 (n=464)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.456 (IC base=+0.103)

- **PATRÓN** `volumen_spike_ratio` > `1.4023` → IC=+0.135 (n=464)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` > 1.4023 (IC base=+0.103)
...
- **FILTRO** `ibs_20min` < `0.6774` → IC=-0.189 (n=59)
...
- **PATRÓN** `sigma_h` < `0.0059` → IC=+0.174 (n=93)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0059 (IC base=+0.075)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.122 (n=149)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 13.0 (IC base=+0.075)

- **PATRÓN** `ibs_20min` > `0.6774` → IC=+0.193 (n=177)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` > 0.6774 (IC base=+0.075)

- **PATRÓN** `dist_vwap_pct` > `0.8626` → IC=+0.156 (n=30)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.8626 (IC base=+0.075)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.655` → IC=+0.194 (n=96)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 3.655 (IC base=+0.075)

- **PATRÓN** `volumen_regimen` > `1.0632` → IC=+0.139 (n=59)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 1.0632 (IC base=+0.075)

- **PATRÓN** `volumen_pendiente_norm` > `0.0865` → IC=+0.167 (n=76)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.0865 (IC base=+0.075)

- **PATRÓN** `volumen_spike_ratio` < `2.5513` → IC=+0.152 (n=159)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 2.5513 (IC base=+0.075)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.124 (n=525)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 6.0 (IC base=+0.101)

- **PATRÓN** `libro_liquidez` > `2309.0554` → IC=+0.130 (n=552)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=6380)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.121 (n=27)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=1360)

### MOMENTUM_IBS_15M_BALLENA
- **FILTRO** `py_entrada` < `0.47` → IC=-0.179 (n=2690)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=8146)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.171 (n=2773)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=8547)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.43` → IC=-0.221 (n=439)

  - _Acción_: SKIP cuando `py_entrada` < 0.43
  - _Potencial_: sin este filtro IC_bueno=+0.092 (n=1390)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.216 (n=474)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.062 (n=1518)

- **FILTRO** `ibs_20min` > `0.2857` → IC=-0.164 (n=495)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2857
  - _Potencial_: sin este filtro IC_bueno=+0.049 (n=1497)
...
- **FILTRO** `py_entrada` > `0.58` → IC=-0.192 (n=495)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.052 (n=1499)

### MOMENTUM_IBS_15M_FADE
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=2415)

- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)

  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=2458)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=7781)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.082 (n=17700)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.284 (n=5961)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=19520)

- **FILTRO** `ibs_7min` < `0.703` → IC=-0.243 (n=6368)

  - _Acción_: SKIP cuando `ibs_7min` < 0.703
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=19113)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.160 (n=8659)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=16822)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.224 (n=7940)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=23967)

- **FILTRO** `ibs_7min` > `0.2963` → IC=-0.177 (n=7965)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2963
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=23942)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.313 (n=967)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=3131)

- **FILTRO** `ibs_7min` < `0.7089` → IC=-0.259 (n=1352)
...
- **FILTRO** `ballena_activa_n` > `9.0` → IC=-0.195 (n=965)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=3133)

- **FILTRO** `py_entrada` > `0.505` → IC=-0.149 (n=3713)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.101 (n=1834)

- **FILTRO** `drift_7min_pct` |x|> `0.136` → IC=-0.132 (n=1386)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.136
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=4161)

- **FILTRO** `ibs_7min` > `0.7987` → IC=-0.206 (n=1386)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7987
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=4161)
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.258 (n=1040)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=3412)

- **FILTRO** `ibs_7min` < `0.7555` → IC=-0.189 (n=1113)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7555
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=3339)

- **FILTRO** `ballena_activa_n` > `161.0` → IC=-0.174 (n=1110)

  - _Acción_: SKIP cuando `ballena_activa_n` > 161.0
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=3342)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.257 (n=1059)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=3446)

- **FILTRO** `ibs_7min` > `0.2531` → IC=-0.168 (n=1126)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2531
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=3379)

- **FILTRO** `ballena_activa_n` > `152.0` → IC=-0.178 (n=1123)

  - _Acción_: SKIP cuando `ballena_activa_n` > 152.0
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=3382)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.182 (n=937)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.094 (n=2904)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.323 (n=925)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=2916)

- **FILTRO** `drift_7min_pct` |x|> `0.1807` → IC=-0.131 (n=1305)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1807
  - _Potencial_: sin este filtro IC_bueno=-0.108 (n=2536)

- **FILTRO** `ibs_7min` < `0.2021` → IC=-0.274 (n=960)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2021
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=2881)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.221 (n=906)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=2935)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.235 (n=1374)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=4466)

- **FILTRO** `ibs_7min` > `0.2624` → IC=-0.156 (n=1985)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2624
  - _Potencial_: sin este filtro IC_bueno=+0.016 (n=3855)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=1324)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.089 (n=2867)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.252 (n=1019)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3172)

- **FILTRO** `ibs_7min` < `0.7427` → IC=-0.192 (n=1046)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7427
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3145)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.183 (n=1036)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3155)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.260 (n=1057)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=3230)

- **FILTRO** `ibs_7min` > `0.2744` → IC=-0.175 (n=1068)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2744
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3219)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.180 (n=1062)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=3225)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.37` → IC=-0.256 (n=1092)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=3503)

- **FILTRO** `ibs_7min` < `0.7187` → IC=-0.223 (n=1144)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7187
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=3451)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.170 (n=1454)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=4590)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.35` → IC=-0.282 (n=1047)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=3257)

- **FILTRO** `ibs_7min` < `0.7273` → IC=-0.232 (n=1076)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7273
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=3228)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.210 (n=1053)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3251)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.206 (n=1348)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=4336)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=2356)
...
| ✅ BALLENAS_TARDIAS | 20789 | -0.101 | -3016.99€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1253 | -0.046 | -182.03€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 19536 | -0.104 | -2834.96€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3193 | -0.115 | -558.09€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3193 | -0.115 | -558.09€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 1253 | -0.046 | -182.03€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 1253 | -0.046 | -182.03€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 374 | -0.136 | -161.05€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 374 | -0.136 | -161.05€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 5966 | -0.045 | -587.98€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 5966 | -0.045 | -587.98€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 5534 | -0.110 | -476.65€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 5534 | -0.110 | -476.65€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 4469 | -0.166 | -1051.20€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 4469 | -0.166 | -1051.20€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 12563 | -0.041 | +4223.49€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 3371 | -0.007 | +1850.07€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 9192 | -0.054 | +2373.42€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 12563 | -0.041 | +4223.49€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 3371 | -0.007 | +1850.07€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 9192 | -0.054 | +2373.42€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO | 1037 | -0.106 | -154.14€ | 3 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#15min | 73 | -0.087 | -14.55€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#5min | 964 | -0.108 | -139.59€ | 0 | 0 |
...
| ✅ CANDIDATA9_BOT_CONSENSO#ETH | 293 | -0.147 | -61.86€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#ETH#15min | 24 | -0.077 | -5.22€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#ETH#5min | 269 | -0.153 | -56.64€ | 2 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#SOL | 88 | -0.078 | -16.18€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#SOL#5min | 88 | -0.078 | -16.18€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#XRP | 39 | -0.159 | -4.70€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#XRP#5min | 39 | -0.159 | -4.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 75393 | +0.113 | -3886.40€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 11755 | +0.183 | -361.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 290 | -0.116 | -45.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 58160 | +0.100 | -3367.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 5188 | +0.115 | -112.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 9691 | +0.097 | -897.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 38 | -0.150 | -1.48€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 9638 | +0.098 | -884.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 15241 | +0.131 | -311.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 3603 | +0.202 | -113.78€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 9635 | +0.110 | -187.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1961 | +0.115 | +12.21€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 9730 | +0.089 | -944.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 45 | -0.053 | -1.73€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#DOGE#240min | 15 | -0.243 | -11.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 9670 | +0.091 | -932.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 16136 | +0.125 | -298.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 4537 | +0.172 | -74.87€ | 1 | 7 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 9722 | +0.108 | -168.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1865 | +0.099 | -46.74€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 14890 | +0.115 | -858.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 3494 | +0.185 | -180.19€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min |
22€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 9681 | -0.072 | +454.22€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 8478 | -0.092 | -224.29€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 8478 | -0.092 | -224.29€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 10639 | -0.048 | +332.04€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 10639 | -0.048 | +332.04€ | 3 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 9988 | -0.064 | +333.40€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 9988 | -0.064 | +333.40€ | 4 | 0 |
...
| ✅ STREAK_FADE_5M | 2431 | -0.026 | -109.97€ | 0 | 0 |
| ✅ STREAK_FADE_5M#5min | 2431 | -0.026 | -109.97€ | 0 | 0 |
...
| ✅ STREAK_FADE_5M#XRP | 913 | -0.030 | -45.21€ | 0 | 0 |
| ✅ STREAK_FADE_5M#XRP#5min | 913 | -0.030 | -45.21€ | 3 | 0 |
...
| ✅ STREAK_MOM_5M | 6502 | +0.021 | +82.26€ | 0 | 0 |
| ✅ STREAK_MOM_5M#5min | 6502 | +0.021 | +82.26€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE | 2081 | +0.021 | +20.27€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE#5min | 2081 | +0.021 | +20.27€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH | 1358 | +0.028 | +32.25€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH#5min | 1358 | +0.028 | +32.25€ | 1 | 0 |
| ✅ STREAK_MOM_5M#SOL | 1892 | +0.012 | +1.02€ | 0 | 0 |
| ✅ STREAK_MOM_5M#SOL#5min | 1892 | +0.012 | +1.02€ | 1 | 0 |
| ✅ STREAK_MOM_5M#XRP | 1171 | +0.027 | +28.71€ | 0 | 0 |
| ✅ STREAK_MOM_5M#XRP#5min | 1171 | +0.027 | +28.71€ | 2 | 0 |
| ✅ STRUCT_NO_15M | 5986 | +0.012 | -33.51€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 5986 | +0.012 | -33.51€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 2375 | +0.019 | +0.51€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 2375 | +0.019 | +0.51€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 2372 | +0.014 | -9.63€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 2372 | +0.014 | -9.63€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 1239 | -0.004 | -24.39€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 1239 | -0.004 | -24.39€ | 2 | 0 |
| ✅ UPDOWN_GBM | 25857 | +0.029 | +1426.36€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 6919 | +0.058 | +1098.60€ | 0 | 11 |
| ✅ UPDOWN_GBM#240min | 969 | +0.005 | +8.87€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 16305 | +0.023 | +320.64€ | 0 | 0 |
| ✅ UPDOWN_GBM#60min | 1558 | -0.003 | -4.40€ | 1 | 0 |
...
| ✅ UPDOWN_GBM#BTC | 4653 | +0.032 | +304.63€ | 0 | 0 |
...
| ✅ UPDOWN_GBM#BTC#60min | 689 | +0.001 | +6.38€ | 0 | 1 |
...
| ✅ UPDOWN_GBM#ETH | 5305 | +0.015 | +206.37€ | 0 | 0 |
...
| ✅ UPDOWN_GBM#ETH#60min | 557 | -0.008 | -8.19€ | 2 | 0 |
| ✅ UPDOWN_GBM#ETH#daily | 35 | -0.149 | +3.89€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 6689 | +0.017 | +158.71€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 1855 | +0.025 | +108.72€ | 1 | 11 |
| ✅ UPDOWN_GBM#SOL#240min | 256 | -0.008 | -2.45€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 4237 | +0.017 | +55.61€ | 1 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 312 | +0.000 | -2.58€ | 0 | 1 |
| ✅ UPDOWN_GBM#SOL#daily | 29 | -0.145 | -0.59€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 3903 | +0.042 | +447.11€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 1807 | +0.080 | +449.00€ | 0 | 11 |
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=1851 IC=+0.024 PNL=+107.70€
...
**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  ...
  - _Estado_: alineada_con_outcome_prev IC=+0.088 n=185/60 | contraria IC=+0.127 n=164 | gap=-0.038 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=557/40 IC=-0.008 PNL=-8.19€ | BTC#60min: n=688/40 IC=+0.000 PNL=+5.87€ | SOL#60min: n=311/40 IC=+0.002 PNL=-2.07€

**⏳ H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  - _Umbral_: n≥40 tras 2 losses y gap(IC_tras_win - IC_tras_2loss)≥0.05
  - _Acción_: Reducir stake (no desactivar) 1-2h tras 2 derrotas consecutivas en el mismo subtype
  - _Estado_: tras_win IC=+0.053 n=263285 | tras_1loss IC=+0.072 n=205888 | tras_2loss IC=+0.040 n=88139/40 | gap=+0.013 (umbral 0.05)
...
**🟡 H-24H-GBM-BUYYES-TARDE** — GBM BUY_YES en tarde europea (15-19h UTC) — señal alcista sostenida
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.191 > 0.08 con n=273 PNL=+167.17€
  - _Datos_: n=273 IC=+0.191 PNL=+167.17€
...
**〰️ H-CUSTOM-GBM-17H-BTC** — GBM BTC a las 17h UTC — ¿edge real?
  ...
  - _Estado_: n=194 IC=+0.071 PNL=+25.21€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=194 IC=+0.071 PNL=+25.21€
...
**〰️ H-CUSTOM-GBM-60MIN-BUYYES** — GBM 60min BUY_YES — ¿edge superior al BUY_NO?
  ...
  - _Estado_: n=1133 IC=-0.005 PNL=-14.63€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1133 IC=-0.005 PNL=-14.63€

**〰️ H-CUSTOM-GBM-60MIN-BUYNO** — GBM 60min BUY_NO — tracking por separado
  - _Hipótesis_: En 15min BUY_NO tiene IC=+0.119. ¿Se repite en 60min? Datos actuales: 8/14 (57%) IC=+0.044 — positivo pero débil. Puede ser que 60min requiera dirección alcista (BUY_YES) y no bajista.
  - _Umbral_: n≥30 para confirmar dirección
  - _Acción_: Si IC<0.05 con n≥30 → en 60min priorizar solo BUY_YES; si IC>0.08 → igualar al BUY_YES
  - _Estado_: n=423 IC=+0.004 PNL=+10.24€ — sin señal clara aún (umbral IC: min=0.05 max=None)
  - _Datos_: n=423 IC=+0.004 PNL=+10.24€
...
**🟡 H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.173 > 0.1 con n=1458 PNL=+822.88€
  - _Datos_: n=1458 IC=+0.173 PNL=+822.88€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=843 IC=+0.080 PNL=+189.22€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=843 IC=+0.080 PNL=+189.22€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=8392 IC=+0.049 PNL=+941.10€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=8392 IC=+0.049 PNL=+941.10€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=1389 IC=+0.049 PNL=+165.30€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1389 IC=+0.049 PNL=+165.30€
...
**🟡 H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.138 > 0.1 con n=263 PNL=+83.18€
  - _Datos_: n=263 IC=+0.138 PNL=+83.18€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=13732 IC=-0.144 PNL=+608.65€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=13732 IC=-0.144 PNL=+608.65€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=1519 IC=+0.137 PNL=+796.78€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=1519 IC=+0.137 PNL=+796.78€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=7086 IC=+0.170 PNL=-896.57€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=7086 IC=+0.170 PNL=-896.57€
