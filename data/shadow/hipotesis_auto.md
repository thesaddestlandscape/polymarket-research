# Hipótesis automáticas — 2026-07-16 02:44 UTC
_Generado por shadow_postmortem.py sobre 16321 resoluciones (PNL=+3328.40€)_
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.199 (n=333)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` < 6.0 (IC base=+0.163)

- **PATRÓN** `py_entrada` < `0.265` → IC=+0.325 (n=317)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.265 (IC base=+0.163)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.173 (n=1058)

  - _Acción_: Kelly boost +0.86€ cuando `libro_spread` < 0.01 (IC base=+0.163)

- **PATRÓN** `libro_liquidez` > `7401.6146` → IC=+0.186 (n=310)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 7401.6146 (IC base=+0.163)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.227 (n=137)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `7240.499` → IC=+0.213 (n=179)
...
- **PATRÓN** `libro_liquidez` > `3766.7239` → IC=+0.212 (n=196)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.212 (n=71)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.202)

- **PATRÓN** `py_entrada` < `0.365` → IC=+0.259 (n=172)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.169 (n=729)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 18.0 (IC base=+0.161)

- **PATRÓN** `dist_vwap_pct` > `0.3631` → IC=+0.212 (n=175)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.168 (n=197)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 18.0 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `0.3457` → IC=+0.187 (n=65)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.3457 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` < `1.1552` → IC=+0.139 (n=131)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 1.1552 (IC base=+0.146)
...
- **PATRÓN** `sigma_h` > `0.015` → IC=+0.189 (n=220)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.015 (IC base=+0.113)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.123 (n=685)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` < 18.0 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `0.6865` → IC=+0.167 (n=118)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.6865 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.1422` → IC=+0.152 (n=228)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.1422 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.296` → IC=+0.336 (n=65)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.155 (n=56)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0039 (IC base=+0.056)

- **PATRÓN** `sigma_ewma_delta_pct` > `15.38` → IC=+0.382 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 15.38 (IC base=+0.056)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2393 | +0.173 | -58.30€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1540 | +0.209 | +13.02€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 693 | +0.181 | -13.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 515 | +0.204 | -13.41€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 851 | +0.172 | -6.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 510 | +0.215 | +18.06€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 841 | +0.169 | -35.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 507 | +0.211 | +11.76€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 4758 | +0.103 | +1481.82€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 4758 | +0.103 | +1481.82€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1140 | +0.066 | +163.78€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1140 | +0.066 | +163.78€ | 0 | 3 |
| ✅ GBM_LATE_15M#ETH | 1148 | +0.076 | +202.19€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1148 | +0.076 | +202.19€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1270 | +0.110 | +516.06€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1270 | +0.110 | +516.06€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1200 | +0.155 | +599.79€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1200 | +0.155 | +599.79€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1998 | +0.151 | +1180.25€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1998 | +0.151 | +1180.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 509 | +0.113 | +248.91€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 509 | +0.113 | +248.91€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 529 | +0.131 | +257.81€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 529 | +0.131 | +257.81€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 539 | +0.112 | +236.91€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 539 | +0.112 | +236.91€ | 1 | 9 |
...
| ✅ GBM_LATE_15M_TARDIO | 1962 | +0.091 | +592.66€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1962 | +0.091 | +592.66€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 484 | +0.060 | +89.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 484 | +0.060 | +89.34€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 449 | +0.023 | +17.06€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 449 | +0.023 | +17.06€ | 0 | 2 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9478 | tras_1loss IC=+0.067 n=6633 | tras_2loss IC=+0.034 n=2841/40 | gap=+0.068 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=13863 IC=+0.105 PNL=+3404.28€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=13863 IC=+0.105 PNL=+3404.28€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema, sin filtrar todavía
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.183 < -0.1 con n=393 PNL=-84.50€
  - _Datos_: n=393 IC=-0.183 PNL=-84.50€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.120 > 0.08 con n=135 PNL=+54.36€
  - _Datos_: n=135 IC=+0.120 PNL=+54.36€
...
**🔶 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.149 n=186) pero sin cruzar ≥2 pares más — BTC: n=138 IC=+0.000; ETH: n=106 IC=+0.000; SOL: n=198 IC=+0.055 ✓
