# Hipótesis automáticas — 2026-09-05 07:08 UTC
_Generado por shadow_postmortem.py sobre 292266 resoluciones (PNL=+28607.76€)_
...
- **FILTRO** `restante_s_al_confirmar` < `146.99` → IC=-0.300 (n=3917)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=11752)
...
- **FILTRO** `restante_s_al_confirmar` > `626.81` → IC=-0.152 (n=334)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.191 (n=8264)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.152 (n=7027)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.179 (n=4057)

  - _Acción_: Kelly boost +0.90€ cuando `libro_spread` < 0.02 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `1943.6536` → IC=+0.172 (n=3468)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.129 (n=680)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 5.0 (IC base=+0.114)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.122 (n=582)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.144 (n=1706)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 5.0 (IC base=+0.133)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.138 (n=1445)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.84` → IC=+0.425 (n=425)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.197 (n=6428)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 5.0 (IC base=+0.191)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.197 (n=5465)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` < 15.0 (IC base=+0.191)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.203 (n=3080)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.73 (IC base=+0.191)

- **PATRÓN** `libro_liquidez` > `3397.72` → IC=+0.322 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3397.72 (IC base=+0.191)

### FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.179 (n=1409)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` < 15.0 (IC base=+0.166)

- **PATRÓN** `py_entrada` < `0.74` → IC=+0.179 (n=1658)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.176 (n=1636)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 5.0 (IC base=+0.172)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.179 (n=1379)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.198 (n=1577)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 5.0 (IC base=+0.186)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.190 (n=1349)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.229 (n=1210)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.166 (n=2477)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 12.0 (IC base=+0.165)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.184 (n=3202)
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.239 (n=1574)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.187 (n=2419)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `restante_min` > `4.94` → IC=+0.159 (n=2452)

  - _Acción_: Kelly boost +0.79€ cuando `restante_min` > 4.94 (IC base=+0.130)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.146 (n=3247)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 7.0 (IC base=+0.130)

- **PATRÓN** `lag_apertura_s` < `3.88` → IC=+0.159 (n=2449)

  - _Acción_: Kelly boost +0.79€ cuando `lag_apertura_s` < 3.88 (IC base=+0.130)
...
- **PATRÓN** `restante_min` > `4.95` → IC=+0.169 (n=1244)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.132 (n=1652)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` < 7.0 (IC base=+0.125)

- **PATRÓN** `lag_apertura_s` < `3.29` → IC=+0.170 (n=1239)
...
- **PATRÓN** `libro_liquidez` > `1749.9313` → IC=+0.295 (n=738)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9516` → IC=+0.212 (n=1304)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9516 (IC base=+0.086)

- **PATRÓN** `dist_vwap_pct` < `0.4693` → IC=+0.229 (n=836)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4693 (IC base=+0.086)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.677` → IC=+0.155 (n=1495)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 5.677 (IC base=+0.086)

- **PATRÓN** `volumen_regimen` < `0.627` → IC=+0.232 (n=300)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.627 (IC base=+0.086)

- **PATRÓN** `volumen_regimen` > `1.0807` → IC=+0.246 (n=407)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0807 (IC base=+0.086)

- **PATRÓN** `volumen_pendiente_norm` > `0.1711` → IC=+0.186 (n=685)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1711 (IC base=+0.086)

- **PATRÓN** `volumen_spike_ratio` < `2.8651` → IC=+0.181 (n=2371)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 2.8651 (IC base=+0.086)

- **PATRÓN** `volumen_spike_ratio` > `1.4728` → IC=+0.177 (n=2370)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 1.4728 (IC base=+0.086)

- **PATRÓN** `ibs_20min` < `0.402` → IC=+0.127 (n=3740)

  - _Acción_: Kelly boost +0.63€ cuando `ibs_20min` < 0.402 (IC base=+0.041)

- **PATRÓN** `dist_vwap_pct` < `0.3335` → IC=+0.141 (n=1407)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.3335 (IC base=+0.041)

- **PATRÓN** `volumen_regimen` < `0.8533` → IC=+0.141 (n=900)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` < 0.8533 (IC base=+0.041)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8194` → IC=+0.211 (n=691)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8194 (IC base=+0.041)

- **PATRÓN** `ballena_activa_n` < `117.0` → IC=+0.202 (n=1604)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2298` → IC=+0.235 (n=153)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2298 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` < `2.6455` → IC=+0.143 (n=776)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 2.6455 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` > `1.4427` → IC=+0.148 (n=776)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.4427 (IC base=+0.157)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.192 (n=725)
...
- **PATRÓN** `sigma_h` > `0.005` → IC=+0.270 (n=533)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.260 (n=603)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.252)

- **PATRÓN** `ibs_20min` < `0.4087` → IC=+0.278 (n=525)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4087 (IC base=+0.252)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.215` → IC=+0.268 (n=623)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.215 (IC base=+0.252)

- **PATRÓN** `volumen_pendiente_norm` < `0.0697` → IC=+0.253 (n=435)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5795` → IC=+0.277 (n=433)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5795 (IC base=+0.252)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.260 (n=601)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.252)

- **PATRÓN** `libro_liquidez` > `1718.9242` → IC=+0.273 (n=398)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1718.9242 (IC base=+0.252)

- **PATRÓN** `ballena_activa_n` < `65.0` → IC=+0.248 (n=431)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 65.0 (IC base=+0.252)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `sigma_h` < `0.003` → IC=+0.259 (n=230)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.003 (IC base=+0.217)

- **PATRÓN** `drift_60min` |x|≤ `0.21` → IC=+0.240 (n=455)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.21 (IC base=+0.217)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.238 (n=712)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.217)

- **PATRÓN** `ibs_20min` > `0.9278` → IC=+0.247 (n=310)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9278 (IC base=+0.217)

- **PATRÓN** `dist_vwap_pct` > `0.1763` → IC=+0.218 (n=424)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1763 (IC base=+0.217)

- **PATRÓN** `dist_vwap_pct` < `0.4777` → IC=+0.222 (n=627)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4777 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.225` → IC=+0.229 (n=330)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.225 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.07` → IC=+0.216 (n=682)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 7.07 (IC base=+0.217)

- **PATRÓN** `volumen_regimen` < `1.2495` → IC=+0.222 (n=682)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0983` → IC=+0.221 (n=646)
...
- **PATRÓN** `libro_liquidez` > `11027.9657` → IC=+0.238 (n=682)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11027.9657 (IC base=+0.217)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.199 (n=410)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1043` → IC=+0.162 (n=682)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` < 0.1043 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `3.1752` → IC=+0.159 (n=655)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 3.1752 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `1.6876` → IC=+0.176 (n=744)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 1.6876 (IC base=+0.167)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.190 (n=872)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.345` → IC=-0.191 (n=208)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=1622)
...
- **FILTRO** `drift_60min` |x|> `0.5121` → IC=-0.161 (n=370)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=721)
...
- **FILTRO** `ibs_20min` > `0.7881` → IC=-0.194 (n=436)
...
- **PATRÓN** `ibs_20min` > `0.3033` → IC=+0.147 (n=4825)

  - _Acción_: Kelly boost +0.73€ cuando `ibs_20min` > 0.3033 (IC base=+0.077)

- **PATRÓN** `dist_vwap_pct` > `1.2179` → IC=+0.295 (n=344)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2179 (IC base=+0.077)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.482` → IC=+0.124 (n=2554)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 2.482 (IC base=+0.077)

- **PATRÓN** `volumen_regimen` > `0.6814` → IC=+0.224 (n=1405)
...
- **FILTRO** `sigma_ewma_delta_pct` > `2.451` → IC=-0.158 (n=361)
...
- **FILTRO** `ibs_20min` > `0.8603` → IC=-0.160 (n=401)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.023 (n=1206)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2616` → IC=+0.120 (n=359)
  ...
  - _Acción_: Kelly boost +0.60€ cuando `volumen_spike_ratio` < 2.2616 (IC base=-0.008)

- **PATRÓN** `ballena_activa_n` < `291.0` → IC=+0.160 (n=151)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 291.0 (IC base=-0.008)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4356` → IC=+0.230 (n=465)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.255 (n=815)
...
- **FILTRO** `ibs_20min` > `0.8648` → IC=-0.158 (n=314)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=945)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.135 (n=83)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=1176)
...
- **FILTRO** `ibs_20min` < `0.8298` → IC=-0.134 (n=691)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.294 (n=357)
...
- **PATRÓN** `ibs_20min` > `0.8298` → IC=+0.294 (n=357)
...
- **PATRÓN** `volumen_regimen` < `0.8622` → IC=+0.259 (n=222)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4502` → IC=+0.298 (n=107)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4502 (IC base=+0.012)

- **PATRÓN** `ballena_activa_n` < `59.0` → IC=+0.309 (n=254)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.264 (n=366)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.255)

- **PATRÓN** `ibs_20min` > `0.9019` → IC=+0.328 (n=510)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9019 (IC base=+0.255)

- **PATRÓN** `dist_vwap_pct` > `0.1648` → IC=+0.315 (n=446)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5676` → IC=+0.262 (n=705)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5676 (IC base=+0.255)

- **PATRÓN** `volumen_spike_ratio` > `1.8308` → IC=+0.261 (n=470)
...
- **PATRÓN** `libro_liquidez` > `2467.2556` → IC=+0.261 (n=684)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2467.2556 (IC base=+0.255)
...
- **PATRÓN** `sigma_h` > `0.0104` → IC=+0.209 (n=1359)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0104 (IC base=+0.163)

- **PATRÓN** `drift_60min` |x|≤ `0.3373` → IC=+0.171 (n=3577)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.3373 (IC base=+0.163)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.174 (n=4275)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 5.0 (IC base=+0.163)

- **PATRÓN** `ibs_20min` > `0.7013` → IC=+0.224 (n=3630)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.091` → IC=+0.252 (n=849)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.091 (IC base=+0.163)

- **PATRÓN** `volumen_regimen` > `0.6298` → IC=+0.162 (n=2744)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.6298 (IC base=+0.163)

- **PATRÓN** `volumen_pendiente_norm` > `0.1055` → IC=+0.186 (n=1536)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1055 (IC base=+0.163)

- **PATRÓN** `volumen_spike_ratio` < `2.321` → IC=+0.164 (n=3339)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.321 (IC base=+0.163)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.164 (n=4148)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.03 (IC base=+0.163)

- **PATRÓN** `libro_liquidez` > `3926.1694` → IC=+0.174 (n=1355)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 3926.1694 (IC base=+0.163)

- **PATRÓN** `ballena_activa_n` < `143.0` → IC=+0.180 (n=2981)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.179 (n=300)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0057 (IC base=+0.177)

- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.201 (n=309)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0071 (IC base=+0.177)

- **PATRÓN** `drift_60min` |x|≤ `0.2934` → IC=+0.201 (n=681)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2934 (IC base=+0.177)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.236 (n=263)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.177)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.292 (n=320)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.177)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.551` → IC=+0.297 (n=313)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.551 (IC base=+0.177)

- **PATRÓN** `volumen_pendiente_norm` > `0.2255` → IC=+0.262 (n=124)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2255 (IC base=+0.177)

- **PATRÓN** `volumen_spike_ratio` < `2.6298` → IC=+0.168 (n=598)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 2.6298 (IC base=+0.177)

- **PATRÓN** `volumen_spike_ratio` > `1.4564` → IC=+0.171 (n=597)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.4564 (IC base=+0.177)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.210 (n=573)
...
- **PATRÓN** `libro_liquidez` > `11338.6336` → IC=+0.188 (n=536)
  ...
  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 11338.6336 (IC base=+0.165)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1321` → IC=+0.186 (n=240)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1321 (IC base=+0.174)

- **PATRÓN** `volumen_spike_ratio` < `3.1564` → IC=+0.166 (n=507)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 3.1564 (IC base=+0.174)

- **PATRÓN** `volumen_spike_ratio` > `3.8997` → IC=+0.175 (n=192)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 3.8997 (IC base=+0.174)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.197 (n=659)
...
- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.199 (n=407)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0056 (IC base=+0.154)

- **PATRÓN** `drift_60min` |x|≤ `0.3561` → IC=+0.166 (n=537)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.3561 (IC base=+0.154)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.174 (n=617)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 6.0 (IC base=+0.154)

- **PATRÓN** `ibs_20min` > `0.4339` → IC=+0.199 (n=610)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4339 (IC base=+0.154)

- **PATRÓN** `dist_vwap_pct` > `0.1446` → IC=+0.183 (n=418)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.1446 (IC base=+0.154)
...
- **PATRÓN** `volumen_regimen` < `0.887` → IC=+0.158 (n=407)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.887 (IC base=+0.154)

- **PATRÓN** `volumen_regimen` > `1.2168` → IC=+0.194 (n=204)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 1.2168 (IC base=+0.154)
...
- **PATRÓN** `libro_liquidez` > `8728.1176` → IC=+0.190 (n=407)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 8728.1176 (IC base=+0.154)

- **PATRÓN** `ballena_activa_n` < `136.0` → IC=+0.143 (n=368)
...
- **PATRÓN** `sigma_h` > `0.0103` → IC=+0.178 (n=349)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` > 0.0103 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=800)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` > `0.5294` → IC=+0.191 (n=768)
...
- **PATRÓN** `volumen_regimen` > `0.6298` → IC=+0.122 (n=768)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` > 0.6298 (IC base=+0.113)

- **PATRÓN** `volumen_pendiente_norm` < `0.1695` → IC=+0.131 (n=768)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_pendiente_norm` < 0.1695 (IC base=+0.113)

- **PATRÓN** `volumen_spike_ratio` < `1.4429` → IC=+0.148 (n=245)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.4429 (IC base=+0.113)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.121 (n=579)
...
- **PATRÓN** `ballena_activa_n` < `51.0` → IC=+0.141 (n=508)
...
- **PATRÓN** `sigma_h` > `0.0172` → IC=+0.215 (n=511)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0172 (IC base=+0.194)

- **PATRÓN** `drift_60min` |x|≤ `0.1669` → IC=+0.220 (n=337)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1669 (IC base=+0.194)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.202 (n=800)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.194)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.196 (n=347)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` < 7.0 (IC base=+0.194)

- **PATRÓN** `ibs_20min` > `0.8851` → IC=+0.274 (n=511)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8851 (IC base=+0.194)

- **PATRÓN** `dist_vwap_pct` > `0.1712` → IC=+0.226 (n=542)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1712 (IC base=+0.194)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.182` → IC=+0.243 (n=380)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.182 (IC base=+0.194)

- **PATRÓN** `volumen_regimen` > `0.6862` → IC=+0.212 (n=685)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6862 (IC base=+0.194)

- **PATRÓN** `volumen_pendiente_norm` > `0.0834` → IC=+0.252 (n=313)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0834 (IC base=+0.194)

- **PATRÓN** `volumen_spike_ratio` < `2.5276` → IC=+0.211 (n=727)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5276 (IC base=+0.194)

- **PATRÓN** `volumen_spike_ratio` > `1.5169` → IC=+0.199 (n=649)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.5169 (IC base=+0.194)
...
- **PATRÓN** `sigma_h` > `0.0095` → IC=+0.171 (n=293)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0095 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.0949` → IC=+0.147 (n=293)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.0949 (IC base=+0.141)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.169 (n=829)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 8.0 (IC base=+0.141)

- **PATRÓN** `ibs_20min` > `0.4429` → IC=+0.169 (n=878)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` > 0.4429 (IC base=+0.141)
...
- **PATRÓN** `volumen_regimen` < `0.8662` → IC=+0.165 (n=472)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1712` → IC=+0.178 (n=243)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.1712 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` < `2.2663` → IC=+0.149 (n=725)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 2.2663 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` > `1.824` → IC=+0.144 (n=549)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.824 (IC base=+0.141)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.152 (n=915)
...
- **PATRÓN** `sigma_h` < `0.0052` → IC=+0.286 (n=157)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0052 (IC base=+0.264)

- **PATRÓN** `sigma_h` > `0.0073` → IC=+0.325 (n=78)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0073 (IC base=+0.264)

- **PATRÓN** `drift_60min` |x|≤ `0.0948` → IC=+0.263 (n=78)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0948 (IC base=+0.264)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.300 (n=243)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.264)

- **PATRÓN** `ibs_20min` > `0.9229` → IC=+0.324 (n=106)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9229 (IC base=+0.264)

- **PATRÓN** `dist_vwap_pct` > `0.154` → IC=+0.289 (n=107)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.154 (IC base=+0.264)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.169` → IC=+0.302 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.169 (IC base=+0.264)

- **PATRÓN** `volumen_regimen` < `0.8223` → IC=+0.279 (n=156)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8223 (IC base=+0.264)

- **PATRÓN** `volumen_regimen` > `1.145` → IC=+0.325 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.145 (IC base=+0.264)

- **PATRÓN** `volumen_pendiente_norm` > `0.0979` → IC=+0.353 (n=100)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0979 (IC base=+0.264)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.199 (n=2236)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0086 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.174 (n=5160)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 5.0 (IC base=+0.165)

- **PATRÓN** `ibs_20min` > `0.4872` → IC=+0.210 (n=4922)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4872 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` > `1.0645` → IC=+0.219 (n=685)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0645 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.458` → IC=+0.228 (n=2470)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.458 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` < `0.8833` → IC=+0.160 (n=2266)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.8833 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` > `0.1683` → IC=+0.195 (n=1348)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.1683 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `1.8784` → IC=+0.170 (n=3062)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 1.8784 (IC base=+0.165)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.167 (n=4640)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.02 (IC base=+0.165)

- **PATRÓN** `libro_liquidez` > `3840.5914` → IC=+0.183 (n=1641)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 3840.5914 (IC base=+0.165)

- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.210 (n=2373)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 62.0 (IC base=+0.165)

- **PATRÓN** `sigma_h` < `0.0108` → IC=+0.193 (n=4519)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0108 (IC base=+0.184)

- **PATRÓN** `drift_60min` |x|≤ `0.478` → IC=+0.190 (n=4519)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_regimen` > `1.1969` → IC=+0.159 (n=1068)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 1.1969 (IC base=+0.184)
...
- **PATRÓN** `volumen_spike_ratio` > `2.305` → IC=+0.198 (n=1727)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.305 (IC base=+0.184)

- **PATRÓN** `ballena_activa_n` < `142.0` → IC=+0.176 (n=3391)
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.212 (n=543)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 11.0 (IC base=+0.188)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.320 (n=282)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.188)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.869` → IC=+0.311 (n=463)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.869 (IC base=+0.188)

- **PATRÓN** `volumen_pendiente_norm` > `0.2271` → IC=+0.279 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2271 (IC base=+0.188)

- **PATRÓN** `volumen_spike_ratio` < `1.586` → IC=+0.189 (n=320)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` < 1.586 (IC base=+0.188)

- **PATRÓN** `volumen_spike_ratio` > `1.8844` → IC=+0.177 (n=484)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 1.8844 (IC base=+0.188)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.233 (n=654)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.188)

- **PATRÓN** `ballena_activa_n` < `78.0` → IC=+0.236 (n=472)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 78.0 (IC base=+0.188)

- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.272 (n=532)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0067 (IC base=+0.265)

- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.273 (n=605)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0046 (IC base=+0.265)

- **PATRÓN** `drift_60min` |x|≤ `0.1864` → IC=+0.293 (n=404)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1864 (IC base=+0.265)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.274 (n=543)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.265)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.273 (n=615)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.265)

- **PATRÓN** `ibs_20min` < `0.4087` → IC=+0.300 (n=532)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4087 (IC base=+0.265)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.535` → IC=+0.280 (n=612)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.535 (IC base=+0.265)

- **PATRÓN** `volumen_pendiente_norm` > `0.2963` → IC=+0.321 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2963 (IC base=+0.265)

- **PATRÓN** `volumen_spike_ratio` > `1.5076` → IC=+0.280 (n=502)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5076 (IC base=+0.265)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.278 (n=291)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.265)

- **PATRÓN** `libro_liquidez` > `1489.1` → IC=+0.274 (n=541)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1489.1 (IC base=+0.265)

- **PATRÓN** `ballena_activa_n` < `71.0` → IC=+0.260 (n=393)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 71.0 (IC base=+0.265)
...
- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.173 (n=730)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.3456` → IC=+0.178 (n=730)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.171 (n=523)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.1433` → IC=+0.183 (n=636)
  ...
  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` < 0.1433 (IC base=+0.165)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3404` → IC=+0.182 (n=633)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 2.3404 (IC base=+0.165)
...
- **PATRÓN** `ballena_activa_n` < `289.0` → IC=+0.183 (n=178)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 289.0 (IC base=+0.165)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0059` → IC=+0.222 (n=718)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0059 (IC base=+0.208)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.244 (n=349)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.208)

- **PATRÓN** `ibs_20min` > `0.6809` → IC=+0.254 (n=641)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6809 (IC base=+0.208)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.261` → IC=+0.325 (n=227)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.261 (IC base=+0.208)

- **PATRÓN** `volumen_pendiente_norm` < `0.2199` → IC=+0.213 (n=653)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2199 (IC base=+0.208)

- **PATRÓN** `volumen_spike_ratio` < `3.2125` → IC=+0.206 (n=566)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.236 (n=744)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.208)

- **PATRÓN** `libro_liquidez` > `1459.7532` → IC=+0.209 (n=717)
...
- **PATRÓN** `sigma_h` < `0.007` → IC=+0.157 (n=721)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` > `0.7317` → IC=+0.240 (n=544)
...
- **PATRÓN** `volumen_regimen` < `0.8944` → IC=+0.167 (n=545)
...
- **PATRÓN** `sigma_h` > `0.0053` → IC=+0.124 (n=858)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.178 (n=859)

  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` > 0.4706 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` > `0.8753` → IC=+0.188 (n=213)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.8753 (IC base=+0.094)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.33` → IC=+0.217 (n=320)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.33 (IC base=+0.094)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.125 (n=592)
...
- **PATRÓN** `ballena_activa_n` < `63.0` → IC=+0.137 (n=631)

  - _Acción_: Kelly boost +0.68€ cuando `ballena_activa_n` < 63.0 (IC base=+0.094)
...
- **PATRÓN** `sigma_h` > `0.0278` → IC=+0.241 (n=307)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0278 (IC base=+0.202)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.213 (n=962)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.202)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.202 (n=814)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.202)

- **PATRÓN** `ibs_20min` > `0.931` → IC=+0.293 (n=418)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.931 (IC base=+0.202)

- **PATRÓN** `dist_vwap_pct` > `0.1698` → IC=+0.251 (n=551)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1698 (IC base=+0.202)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.182` → IC=+0.253 (n=459)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.182 (IC base=+0.202)

- **PATRÓN** `volumen_regimen` > `1.2214` → IC=+0.231 (n=307)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2214 (IC base=+0.202)

- **PATRÓN** `volumen_pendiente_norm` > `0.2373` → IC=+0.253 (n=156)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2373 (IC base=+0.202)

- **PATRÓN** `volumen_spike_ratio` < `2.6244` → IC=+0.201 (n=868)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.6244 (IC base=+0.202)

- **PATRÓN** `volumen_spike_ratio` > `1.5573` → IC=+0.206 (n=776)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.195 (n=254)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0057 (IC base=+0.098)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.185 (n=141)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 18.0 (IC base=+0.098)

- **PATRÓN** `ibs_20min` > `0.6404` → IC=+0.221 (n=288)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6404 (IC base=+0.098)

- **PATRÓN** `dist_vwap_pct` > `0.8642` → IC=+0.250 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8642 (IC base=+0.098)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.682` → IC=+0.231 (n=184)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.682 (IC base=+0.098)

- **PATRÓN** `volumen_regimen` < `0.8266` → IC=+0.156 (n=193)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.8266 (IC base=+0.098)

- **PATRÓN** `volumen_regimen` > `0.5803` → IC=+0.131 (n=288)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_regimen` > 0.5803 (IC base=+0.098)

- **PATRÓN** `volumen_pendiente_norm` > `0.2695` → IC=+0.289 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2695 (IC base=+0.098)

- **PATRÓN** `volumen_spike_ratio` < `1.9736` → IC=+0.173 (n=166)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.9736 (IC base=+0.098)

- **PATRÓN** `volumen_spike_ratio` > `1.4867` → IC=+0.178 (n=169)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 1.4867 (IC base=+0.098)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.164 (n=236)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.02 (IC base=+0.098)

- **PATRÓN** `libro_liquidez` > `1175.1495` → IC=+0.176 (n=236)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 1175.1495 (IC base=+0.098)
...
- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.230 (n=113)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0056 (IC base=+0.114)

- **PATRÓN** `drift_60min` |x|≤ `0.0769` → IC=+0.196 (n=21)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.0769 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.151 (n=61)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 16.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` > `0.7829` → IC=+0.235 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7829 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` > `0.3468` → IC=+0.274 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3468 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `16.35` → IC=+0.259 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 16.35 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` < `1.1039` → IC=+0.167 (n=85)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.1039 (IC base=+0.114)

- **PATRÓN** `volumen_pendiente_norm` < `0.0635` → IC=+0.217 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0635 (IC base=+0.114)

- **PATRÓN** `volumen_spike_ratio` < `2.0892` → IC=+0.226 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0892 (IC base=+0.114)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.149 (n=95)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.01 (IC base=+0.114)
...
- **PATRÓN** `libro_liquidez` > `2277.8754` → IC=+0.133 (n=262)

  - _Acción_: Kelly boost +0.66€ cuando `libro_liquidez` > 2277.8754 (IC base=+0.093)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.174 (n=1570)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.029 (n=4741)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.186 (n=1576)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=4949)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.41` → IC=-0.224 (n=241)

  - _Acción_: SKIP cuando `py_entrada` < 0.41
  - _Potencial_: sin este filtro IC_bueno=+0.059 (n=750)

- **FILTRO** `ibs_20min` < `0.7375` → IC=-0.199 (n=247)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7375
  - _Potencial_: sin este filtro IC_bueno=+0.052 (n=744)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.189 (n=339)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.057 (n=709)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.176 (n=248)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=803)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.193 (n=239)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.045 (n=745)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=969)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.196 (n=261)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.014 (n=852)
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.280 (n=3716)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=11534)

- **FILTRO** `ibs_7min` < `0.7143` → IC=-0.241 (n=3784)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=11466)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.169 (n=5137)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=10113)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.221 (n=4711)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=14573)

- **FILTRO** `ibs_7min` > `0.7143` → IC=-0.173 (n=4808)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=14476)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.309 (n=544)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=1702)
...
- **FILTRO** `ballena_activa_n` > `166.0` → IC=-0.179 (n=705)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.224 (n=820)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=2661)

- **FILTRO** `ibs_7min` > `0.2786` → IC=-0.154 (n=1183)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2786
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=2298)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.143 (n=801)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.263 (n=618)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=1958)

- **FILTRO** `ibs_7min` < `0.7549` → IC=-0.189 (n=644)
...
- **FILTRO** `ballena_activa_n` > `38.0` → IC=-0.185 (n=633)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=1943)
...
- **FILTRO** `py_entrada` < `0.39` → IC=-0.242 (n=687)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=2171)

- **FILTRO** `ibs_7min` < `0.7368` → IC=-0.206 (n=708)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7368
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=2150)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.182 (n=697)

  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=2161)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.177 (n=857)
...
- **FILTRO** `ibs_7min` > `0.7634` → IC=-0.154 (n=848)
...
- **PATRÓN** `streak_estiramiento` < `0.437` → IC=+0.197 (n=31)

  - _Acción_: Kelly boost +0.98€ cuando `streak_estiramiento` < 0.437 (IC base=+0.022)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.014 (n=1638)
...
### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` < `0.0036` → IC=+0.167 (n=196)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0036 (IC base=+0.140)

- **PATRÓN** `sigma_h` > `0.0074` → IC=+0.182 (n=196)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` > 0.0074 (IC base=+0.140)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0615` → IC=+0.142 (n=587)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.71€ cuando `delta_ratio_macro` |x|> 0.0615 (IC base=+0.140)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3285` → IC=+0.188 (n=274)

  - _Acción_: Kelly boost +0.94€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3285 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.142 (n=630)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` > `0.5909` → IC=+0.222 (n=588)
...
- **PATRÓN** `dist_vwap_pct` > `0.4301` → IC=+0.150 (n=135)
  ...
  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.4301 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` < `0.5623` → IC=+0.141 (n=627)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.5623 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `15.709` → IC=+0.228 (n=167)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 15.709 (IC base=+0.140)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.144 (n=591)

  - _Acción_: Kelly boost +0.72€ cuando `libro_spread` < 0.01 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `9958.5281` → IC=+0.177 (n=196)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 9958.5281 (IC base=+0.140)

### UPDOWN_GBM#5min
- **FILTRO** `sigma_ewma_delta_pct` > `7.84` → IC=-0.204 (n=42)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 7.84
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=1100)
...
- **FILTRO** `libro_liquidez` < `6535.7752` → IC=-0.155 (n=27)

  - _Acción_: SKIP cuando `libro_liquidez` < 6535.7752
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=81)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0571` → IC=+0.163 (n=158)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.81€ cuando `delta_ratio_macro` |x|> 0.0571 (IC base=+0.127)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3111` → IC=+0.192 (n=63)

  - _Acción_: Kelly boost +0.96€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3111 (IC base=+0.127)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.192 (n=76)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 6.0 (IC base=+0.127)

- **PATRÓN** `ibs_15` > `0.5` → IC=+0.208 (n=159)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5 (IC base=+0.127)

- **PATRÓN** `dist_vwap_pct` > `0.1119` → IC=+0.196 (n=90)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1119 (IC base=+0.127)

- **PATRÓN** `sigma_ewma_delta_pct` > `15.496` → IC=+0.214 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 15.496 (IC base=+0.127)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.133 (n=126)

  - _Acción_: Kelly boost +0.66€ cuando `libro_spread` < 0.01 (IC base=+0.127)

- **PATRÓN** `libro_liquidez` > `2479.6478` → IC=+0.171 (n=141)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 2479.6478 (IC base=+0.127)
...
- **FILTRO** `sigma_h` > `0.011` → IC=-0.195 (n=359)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=1081)

- **FILTRO** `ibs_15` < `0.5814` → IC=-0.185 (n=163)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: SKIP cuando `ibs_15` < 0.5814
  - _Potencial_: sin este filtro IC_bueno=+0.228 (n=332)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.142 (n=370)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=1070)

- **FILTRO** `sigma_ewma_delta_pct` > `18.728` → IC=-0.149 (n=523)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 18.728
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=3989)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3526` → IC=+0.178 (n=209)

  - _Acción_: Kelly boost +0.89€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.
 | 0 | 0 |
...
| ✅ STREAK_FADE_15M#XRP | 92 | +0.011 | -2.50€ | 0 | 0 |
| ✅ STREAK_FADE_15M#XRP#15min | 92 | +0.011 | -2.50€ | 0 | 0 |
...
| ✅ STRUCT_NO_15M | 4319 | +0.012 | -24.65€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 4319 | +0.012 | -24.65€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 1657 | +0.012 | -11.46€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 1657 | +0.012 | -11.46€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 1719 | +0.022 | +6.78€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 1719 | +0.022 | +6.78€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 943 | -0.006 | -19.96€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 943 | -0.006 | -19.96€ | 2 | 0 |
| ✅ UPDOWN_GBM | 10879 | +0.010 | +344.53€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 3750 | +0.040 | +399.91€ | 0 | 12 |
| ✅ UPDOWN_GBM#240min | 455 | +0.008 | +5.11€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 5935 | -0.004 | -49.17€ | 1 | 0 |
| ✅ UPDOWN_GBM#60min | 687 | -0.011 | -10.40€ | 2 | 0 |
...
| ✅ UPDOWN_GBM#BTC | 2019 | +0.015 | +111.16€ | 0 | 0 |
...
| ✅ UPDOWN_GBM#BTC#60min | 267 | -0.002 | -3.04€ | 3 | 1 |
| ✅ UPDOWN_GBM#BTC#daily | 20 | -0.136 | +1.50€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 1273 | +0.007 | +9.21€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 130 | +0.091 | +29.94€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#240min | 10 | +0.042 | +0.80€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 1133 | -0.004 | -21.53€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 2190 | -0.001 | +17.79€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 1076 | +0.029 | +47.23€ | 1 | 6 |
| ✅ UPDOWN_GBM#ETH#240min | 128 | +0.023 | +6.80€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 690 | -0.038 | -30.72€ | 4 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 279 | -0.020 | -5.56€ | 3 | 0 |
| ✅ UPDOWN_GBM#ETH#daily | 17 | -0.157 | +0.04€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 3103 | +0.003 | +13.35€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 980 | +0.007 | +21.14€ | 1 | 11 |
...
| ✅ UPDOWN_GBM#XRP | 1788 | +0.018 | +141.08€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 943 | +0.050 | +167.61€ | 0 | 9 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 5952 | -0.064 | +1312.49€ | 4 | 8 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 5952 | -0.064 | +1312.49€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 310 | -0.051 | +340.91€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 310 | -0.051 | +340.91€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 1220 | -0.147 | -54.07€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 1220 | -0.147 | -54.07€ | 3 | 5 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 1845 | -0.085 | +361.98€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 1845 | -0.085 | +361.98€ | 2 | 4 |
...
| ✅ UPDOWN_GBM_IBS_ALTO | 469 | +0.286 | +371.28€ | 0 | 12 |
| ✅ UPDOWN_GBM_IBS_ALTO#15min | 469 | +0.286 | +371.28€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.083) — sin ventaja clara. oversold(IBS<0.3): IC=+0.027 n=3855 | neutral: IC=+0.004 n=4229 | overbought(IBS>0.7): IC=+0.087 n=4239
  - _Datos_: n=12800 IC=+0.041 PNL=+1328.89€

**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  - _Umbral_: n≥40 por celda + gate riguroso completo (Wilson+shuffle+PnL bootstrap)
  - _Acción_: Añadir claves 'ESTRATEGIA#SUBTYPE#DIRECCION#HORA':1.2 a meta.hora_boost_factor, solo por celda confirmada
  - _Estado_: 215 celda(s) pasan gate riguroso completo de 1424 evaluadas (n>=40) y 2515 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.007 < 0.08 — monitorear
  - _Datos_: n=980 IC=+0.007 PNL=+21.14€
...
**⏳ H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  ...
  - _Estado_: 10817 ops, 22 horas distintas. Sin hora con n≥15 y IC extremo aún.

**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  - _Umbral_: n≥60 alineadas y gap IC≥0.08 vs contrarias — y descartar que sea proxy de drift_15min/60min
  - _Acción_: Si confirma e independiente de drift → capturar prev_window_outcome como feature en shadow_predict y boost ×1.1-1.2 en señales alineadas
  - _Estado_: alineada_con_outcome_prev IC=+0.034 n=71/60 | contraria IC=+0.138 n=45 | gap=-0.104 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=279/40 IC=-0.020 PNL=-5.56€ | BTC#60min: n=267/40 IC=-0.002 PNL=-3.04€ | SOL#60min: n=141/40 IC=-0.011 PNL=-1.80€

**⏳ H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  - _Umbral_: n≥40 tras 2 losses y gap(IC_tras_win - IC_tras_2loss)≥0.05
  - _Acción_: Reducir stake (no desactivar) 1-2h tras 2 derrotas consecutivas en el mismo subtype
  - _Estado_: tras_win IC=+0.064 n=163248 | tras_1loss IC=+0.053 n=128749 | tras_2loss IC=+0.017 n=57534/40 | gap=+0.047 (umbral 0.05)

**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  - _Umbral_: n≥40 en contrario_BTC y gap≥0.08 — y descartar confound con drift propio antes de actuar
  - _Acción_: Si se confirma y no es confound → boost en ETH/SOL cuando decisión contraria a drift_15min BTC
  - _Estado_: alineado_BTC IC=-0.003 n=971 | contrario_BTC IC=-0.009 n=870/40 | gap=-0.007 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**🟡 H-24H-GBM-BUYYES-MADRUGADA** — GBM BUY_YES en madrugada europea (05-07h UTC) — señal alcista
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.211 > 0.08 con n=119 PNL=+85.47€
  - _Datos_: n=119 IC=+0.211 PNL=+85.47€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=10569 IC=+0.008 PNL=+289.34€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=10569 IC=+0.008 PNL=+289.34€
...
**〰️ H-CUSTOM-GBM-60MIN-BUYNO** — GBM 60min BUY_NO — tracking por separado
  ...
  - _Estado_: n=151 IC=-0.049 PNL=-6.12€ — sin señal clara aún (umbral IC: min=0.05 max=None)
  - _Datos_: n=151 IC=-0.049 PNL=-6.12€
...
**🟡 H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.140 > 0.1 con n=782 PNL=+281.46€
  - _Datos_: n=782 IC=+0.140 PNL=+281.46€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=2183 IC=+0.037 PNL=+240.67€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=2183 IC=+0.037 PNL=+240.67€
...
**〰️ H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  ...
  - _Estado_: n=796 IC=+0.034 PNL=+29.56€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=796 IC=+0.034 PNL=+29.56€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=2771 IC=+0.034 PNL=+247.87€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2771 IC=+0.034 PNL=+247.87€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.169 > 0.1 con n=1175 PNL=+475.44€
  - _Datos_: n=1175 IC=+0.169 PNL=+475.44€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.142 > 0.08 con n=744 PNL=+267.09€
  - _Datos_: n=744 IC=+0.142 PNL=+267.09€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=2351 IC=+0.141 PNL=+1395.13€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=2351 IC=+0.141 PNL=+1395.13€
...
**〰️ H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: n=992 IC=+0.000 PNL=+117.89€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=992 IC=+0.000 PNL=+117.89€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.187 > 0.08 con n=908 PNL=+621.01€
  - _Datos_: n=908 IC=+0.187 PNL=+621.01€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.228 > 0.08 con n=2106 PNL=-216.84€
  - _Datos_: n=2106 IC=+0.228 PNL=-216.84€
...
**🔶 H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.093 n=349) pero sin cruzar ≥2 pares más — sin otros pares con datos
  - _Datos_: n=349 IC=+0.093 PNL=+83.28€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=4784 IC=+0.152 PNL=-733.00€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=4784 IC=+0.152 PNL=-733.00€
