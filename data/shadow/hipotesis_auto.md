# Hipótesis automáticas — 2026-07-05 11:42 UTC
_Generado por shadow_postmortem.py sobre 3851 resoluciones (PNL=+156.04€)_
...
- **PATRÓN** `sigma_h` < `0.009` → IC=+0.177 (n=94)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.009 (IC base=+0.035)

- **PATRÓN** `ibs_15` < `0.0563` → IC=+0.158 (n=71)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.79€ cuando `ibs_15` < 0.0563 (IC base=+0.035)
...
- **FILTRO** `sigma_h` < `0.0051` → IC=-0.141 (n=37)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0366` → IC=+0.140 (n=23)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.70€ cuando `pct_spot_vs_ref` |x|≤ 0.0366 (IC base=+0.032)

- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.147 (n=15)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0064 (IC base=+0.032)
...
- **FILTRO** `delta_ratio_macro` |x|≤ `0.0746` → IC=-0.153 (n=47)
  - _Por qué funciona_: flow macro débil → el mercado no ha procesado aún la presión; lag explotable
  - _Acción_: SKIP cuando `delta_ratio_macro` |x|≤ 0.0746
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.200 (n=28)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0061 (IC base=+0.086)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.186 (n=33)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` < 7.0 (IC base=+0.086)

- **PATRÓN** `ibs_15` < `0.099` → IC=+0.188 (n=46)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.94€ cuando `ibs_15` < 0.099 (IC base=+0.086)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS < 0.0563 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.158 n=71). Confirma señal de reversión media → alinear con BUY_YES.
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.099 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.188 n=46). Confirma señal de reversión media → alinear con BUY_YES.
...
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 1047 | +0.092 | +251.21€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 1047 | +0.092 | +251.21€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 271 | +0.035 | +27.78€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 271 | +0.035 | +27.78€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 271 | +0.050 | +25.38€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 271 | +0.050 | +25.38€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL | 271 | +0.141 | +109.63€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 271 | +0.141 | +109.63€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP | 234 | +0.148 | +88.42€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 234 | +0.148 | +88.42€ | 0 | 0 |
...
| ✅ ORDER_FLOW_5M | 1539 | +0.011 | +7.97€ | 1 | 0 |
| ✅ ORDER_FLOW_5M#5min | 1403 | +0.006 | -4.62€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 945 | -0.037 | -49.10€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 709 | -0.015 | -19.26€ | 1 | 2 |
...
| ✅ UPDOWN_GBM#BTC | 267 | -0.043 | -33.78€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 195 | -0.018 | -22.56€ | 3 | 2 |
...
| ✅ UPDOWN_GBM#ETH | 393 | -0.004 | +8.91€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 307 | +0.015 | +16.39€ | 3 | 3 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.055) — sin ventaja clara. oversold(IBS<0.3): IC=-0.013 n=195 | neutral: IC=-0.068 n=160 | overbought(IBS>0.7): IC=-0.044 n=178
  - _Datos_: n=555 IC=-0.035 PNL=-23.87€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=01h: IC=-0.186 n=33 PNL=-14.53€ → FILTRAR | H=02h: IC=-0.154 n=24 PNL=-5.23€ → FILTRAR | H=03h: IC=-0.113 n=19 PNL=-5.87€ → FILTRAR | H=04h: IC=+0.113 n=19 PNL=+8.64€ → BOOST | H=08h: IC=-0.199 n=15 PNL=-10.22€ → FILTRAR | H=10h: IC=-0.154 n=15 PNL=-6.77€ → FILTRAR | H=13h: IC=+0.115 n=24 PNL=+5.54€ → BOOST | H=16h: IC=+0.167 n=25 PNL=+7.36€ → BOOST | H=20h: IC=-0.147 n=32 PNL=-9.63€ → FILTRAR

**🟡 H-OF-PAR** — ORDER_FLOW per-pair delta_ratio ranges
  - _Umbral_: n≥200 por par con delta_ratio feature en shadow
  - _Acción_: Añadir DELTA_MIN/MAX por par dict en shadow_predict.py
  - _Estado_: BTC: 168 ops con delta_ratio | SOL: 201 ops con delta_ratio
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.030 n=1928 | tras_1loss IC=-0.010 n=1823 | tras_2loss IC=-0.027 n=912/40 | gap=+0.058 (umbral 0.05)
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=-0.026 n=95 | contrario_BTC IC=-0.051 n=47/40 | gap=-0.025 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-24H-GBM-BUYYES-MADRUGADA** — GBM BUY_YES en madrugada europea (05-07h UTC) — señal alcista
  ...
  - _Estado_: n=50 IC=+0.077 PNL=+7.11€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=50 IC=+0.077 PNL=+7.11€
...
**〰️ H-CUSTOM-OF-MADRUGADA** — ORDER_FLOW de madrugada (0h-6h UTC) BTC+SOL — ¿neutralizar?
  ...
  - _Estado_: n=59 IC=+0.025 PNL=+1.67€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=59 IC=+0.025 PNL=+1.67€

**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  - _Hipótesis_: Cuando la volatilidad horaria es muy alta el GBM puede sobreestimar el edge. Testear.
  - _Umbral_: n≥30 y IC<-0.05
  - _Acción_: Filtrar señales GBM cuando sigma_h > 0.002 si se confirma IC negativo
  - _Estado_: n=822 IC=-0.033 PNL=-35.55€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=822 IC=-0.033 PNL=-35.55€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=427 IC=-0.048 PNL=-46.69€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=427 IC=-0.048 PNL=-46.69€
...
**🔴 H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.018 < 0.02 con n=195 PNL=-22.56€
  - _Datos_: n=195 IC=-0.018 PNL=-22.56€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=317 IC=-0.011 PNL=-28.58€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=317 IC=-0.011 PNL=-28.58€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=260 IC=+0.008 PNL=+9.77€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=260 IC=+0.008 PNL=+9.77€
...
**⏳ H-CUSTOM-CROSS-WINDOW-SPREAD-POS** — GBM 15min/60min: spread positivo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: 10/40 ops en el filtro definido (IC actual=+0.000 PNL=+0.17€)
  - _Datos_: n=10 IC=+0.000 PNL=+0.17€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=1410 IC=+0.063 PNL=+237.18€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=1410 IC=+0.063 PNL=+237.18€
...
**🔴 H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.071 < -0.05 con n=569 PNL=-74.61€
  - _Datos_: n=569 IC=-0.071 PNL=-74.61€
...
**⏳ H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: 83/120 ops en el filtro definido (IC actual=+0.147 PNL=+17.80€)
  - _Datos_: n=83 IC=+0.147 PNL=+17.80€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=91 IC=-0.059 PNL=+12.14€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=91 IC=-0.059 PNL=+12.14€
