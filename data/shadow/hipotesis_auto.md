# Hipótesis automáticas — 2026-07-28 05:56 UTC
_Generado por shadow_postmortem.py sobre 39645 resoluciones (PNL=+8057.25€)_
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.185 (n=1192)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.173 (n=2899)
...
- **PATRÓN** `dist_vwap_pct` > `0.4564` → IC=+0.128 (n=329)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` > 0.4564 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.915` → IC=+0.175 (n=841)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 2.915 (IC base=+0.112)

- **PATRÓN** `sigma_h` > `0.0103` → IC=+0.122 (n=1520)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.139` → IC=-0.278 (n=25)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.066 (n=624)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.309` → IC=+0.224 (n=154)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.309 (IC base=+0.098)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.144 (n=414)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` < 8.0 (IC base=+0.093)

### GBM_LATE_15M#XRP#15min
- **PATRÓN** `sigma_h` < `0.0101` → IC=+0.158 (n=258)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0101 (IC base=+0.141)

- **PATRÓN** `sigma_h` > `0.0204` → IC=+0.144 (n=346)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` > 0.0204 (IC base=+0.141)
...
- **PATRÓN** `dist_vwap_pct` > `0.6566` → IC=+0.185 (n=71)
  ...
  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.6566 (IC base=+0.141)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.472` → IC=+0.250 (n=94)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.472 (IC base=+0.141)

- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.147 (n=950)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.153 (n=355)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.186 (n=1210)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0075 (IC base=+0.139)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.159 (n=1687)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 7.0 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` > `0.2767` → IC=+0.191 (n=499)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.2767 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.424` → IC=+0.230 (n=572)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.424 (IC base=+0.139)

- **PATRÓN** `sigma_h` > `0.0067` → IC=+0.153 (n=1419)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0067 (IC base=+0.120)
...
- **PATRÓN** `dist_vwap_pct` > `0.3961` → IC=+0.129 (n=340)
  ...
  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` > 0.3961 (IC base=+0.120)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.218` → IC=+0.152 (n=314)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 8.218 (IC base=+0.120)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **PATRÓN** `sigma_h` < `0.0047` → IC=+0.128 (n=398)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.64€ cuando `sigma_h` < 0.0047 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.143 (n=421)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 7.0 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` > `0.4476` → IC=+0.217 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4476 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.798` → IC=+0.187 (n=81)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 11.798 (IC base=+0.110)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.205 (n=154)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0075 (IC base=+0.119)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.146 (n=309)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 12.0 (IC base=+0.119)

- **PATRÓN** `dist_vwap_pct` > `0.672` → IC=+0.171 (n=80)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.672 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.25` → IC=+0.183 (n=121)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 7.25 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.399` → IC=+0.183 (n=102)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 9.399 (IC base=+0.089)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.139 (n=458)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` > 0.0078 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.146 (n=309)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 12.0 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` > `0.1789` → IC=+0.151 (n=164)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.1789 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.62` → IC=+0.263 (n=196)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.62 (IC base=+0.110)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` < `0.0176` → IC=+0.249 (n=420)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0176 (IC base=+0.227)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.247 (n=381)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.227)

- **PATRÓN** `dist_vwap_pct` > `0.4537` → IC=+0.287 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4537 (IC base=+0.227)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.563` → IC=+0.340 (n=117)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.563 (IC base=+0.227)

- **PATRÓN** `sigma_h` < `0.0173` → IC=+0.206 (n=546)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.43` → IC=+0.195 (n=546)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` < 5.43 (IC base=+0.204)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0104` → IC=+0.126 (n=808)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.63€ cuando `sigma_h` > 0.0104 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` > `0.4717` → IC=+0.160 (n=327)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.4717 (IC base=+0.094)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.159` → IC=+0.230 (n=283)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.159 (IC base=+0.094)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.004` → IC=+0.120 (n=264)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.60€ cuando `sigma_h` < 0.004 (IC base=+0.076)

- **PATRÓN** `dist_vwap_pct` > `0.4428` → IC=+0.178 (n=57)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.4428 (IC base=+0.076)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.673` → IC=+0.191 (n=66)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 12.673 (IC base=+0.076)
...
- **FILTRO** `sigma_ewma_delta_pct` > `7.827` → IC=-0.152 (n=21)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.069 (n=492)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.162` → IC=+0.278 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.162 (IC base=+0.050)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` < `0.0198` → IC=+0.178 (n=467)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0198 (IC base=+0.170)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.175 (n=533)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 6.0 (IC base=+0.170)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.172 (n=555)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` < 18.0 (IC base=+0.170)

- **PATRÓN** `dist_vwap_pct` > `0.2426` → IC=+0.230 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2426 (IC base=+0.170)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.951` → IC=+0.282 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.951 (IC base=+0.170)

- **PATRÓN** `sigma_h` < `0.0223` → IC=+0.156 (n=663)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0223 (IC base=+0.155)

- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.171 (n=663)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.346` → IC=+0.226 (n=111)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.346 (IC base=+0.155)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.487` → IC=+0.153 (n=683)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.6075` → IC=+0.176 (n=640)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.88€ cuando `ibs_15` > 0.6075 (IC base=+0.056)

- **PATRÓN** `dist_vwap_pct` > `0.1666` → IC=+0.165 (n=234)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.1666 (IC base=+0.056)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.107` → IC=+0.152 (n=231)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 5.107 (IC base=+0.056)

- **PATRÓN** `dist_vwap_pct` > `0.9936` → IC=+0.130 (n=52)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` > 0.9936 (IC base=+0.050)
...
- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.123 (n=322)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` < 0.0072 (IC base=+0.100)

- **PATRÓN** `drift_60min` |x|≤ `0.2204` → IC=+0.137 (n=312)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.2204 (IC base=+0.100)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0644` → IC=+0.124 (n=312)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.62€ cuando `delta_ratio_macro` |x|> 0.0644 (IC base=+0.100)

- **PATRÓN** `hora_utc` < `14.0` → IC=+0.163 (n=182)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 14.0 (IC base=+0.100)

- **PATRÓN** `ibs_15` > `0.724` → IC=+0.240 (n=244)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.724 (IC base=+0.100)

- **PATRÓN** `dist_vwap_pct` > `0.2571` → IC=+0.239 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2571 (IC base=+0.100)

- **PATRÓN** `sigma_ewma_delta_pct` > `20.773` → IC=+0.186 (n=49)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 20.773 (IC base=+0.100)
...
- **PATRÓN** `ibs_15` > `0.67` → IC=+0.197 (n=206)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.99€ cuando `ibs_15` > 0.67 (IC base=+0.046)

- **PATRÓN** `sigma_ewma_delta_pct` > `27.313` → IC=+0.179 (n=26)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 27.313 (IC base=+0.046)

- **PATRÓN** `sigma_h` < `0.0055` → IC=+0.155 (n=137)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0055 (IC base=+0.059)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6075 sube el IC de +0.056 a +0.176 en UPDOWN_GBM#15min (n=640). Ya aplicado como kelly_boost=+0.88€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.724 sube el IC de +0.100 a +0.240 en UPDOWN_GBM#BTC#15min (n=244). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.67 sube el IC de +0.046 a +0.197 en UPDOWN_GBM#ETH#15min (n=206). Ya aplicado como kelly_boost=+0.99€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **LIVE-CANDIDATA**: `GBM_LATE_15M_TARDIO#DOGE#15min` — IC=+0.125 n=30. Faltan ~10 resoluciones para umbral n≥40. ETA: ~7h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M_TARDIO#DOGE` — IC=+0.125 n=30. Faltan ~10 resoluciones para umbral n≥40. ETA: ~7h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M_ESPACIO_ATR#DOGE#15min` — IC=+0.118 n=32. Faltan ~8 resoluciones para umbral n≥40. ETA: ~6h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M_ESPACIO_ATR#DOGE` — IC=+0.118 n=32. Faltan ~8 resoluciones para umbral n≥40. ETA: ~6h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M#BNB#15min` — IC=+0.136 n=31. Faltan ~9 resoluciones para umbral n≥40. ETA: ~6h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M#BNB` — IC=+0.136 n=31. Faltan ~9 resoluciones para umbral n≥40. ETA: ~6h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M_TARDIO#BNB#15min` — IC=+0.188 n=30. Faltan ~10 resoluciones para umbral n≥40. ETA: ~7h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M_TARDIO#BNB` — IC=+0.188 n=30. Faltan ~10 resoluciones para umbral n≥40. ETA: ~7h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 1414 | +0.096 | +27.03€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 1414 | +0.096 | +27.03€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 560 | +0.089 | -2.10€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 560 | +0.089 | -2.10€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 528 | +0.117 | +10.79€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 528 | +0.117 | +10.79€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 6876 | +0.177 | -56.09€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 4533 | +0.201 | -47.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 309 | +0.056 | -21.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 819 | +0.129 | -55.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 1215 | +0.152 | +68.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 2032 | +0.182 | -12.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1513 | +0.197 | -51.77€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 2412 | +0.167 | -48.12€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 407 | +0.136 | +9.84€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#SOL | 2385 | +0.182 | -2.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1492 | +0.207 | +6.57€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 441 | +0.301 | +6.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 441 | +0.301 | +6.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 441 | +0.301 | +6.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 441 | +0.301 | +6.01€ | 0 | 0 |
| ✅ GBM_LATE_15M | 8516 | +0.093 | +2528.34€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 8516 | +0.093 | +2528.34€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB |