# Hipótesis automáticas — 2026-09-24 21:23 UTC
_Generado por shadow_postmortem.py sobre 594703 resoluciones (PNL=+66956.29€)_
...
- **FILTRO** `py_entrada` > `0.495` → IC=-0.263 (n=112)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.120 (n=488)
...
- **PATRÓN** `py_entrada` < `0.495` → IC=+0.120 (n=488)

  - _Acción_: Kelly boost +0.60€ cuando `py_entrada` < 0.495 (IC base=+0.048)

- **PATRÓN** `ballena_activa_n` < `95.0` → IC=+0.133 (n=175)
...
- **FILTRO** `py_entrada` > `0.495` → IC=-0.370 (n=52)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.111 (n=355)
...
- **PATRÓN** `ballena_activa_n` < `95.0` → IC=+0.149 (n=132)

  - _Acción_: Kelly boost +0.75€ cuando `ballena_activa_n` < 95.0 (IC base=+0.048)
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.02` → IC=-0.233 (n=7055)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.02
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=21179)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `138.26` → IC=-0.249 (n=923)
...
- **FILTRO** `restante_s_al_confirmar` < `499.92` → IC=-0.150 (n=364)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=1092)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.204 (n=1525)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.202 (n=2158)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.200)

- **PATRÓN** `libro_liquidez` > `15863.0179` → IC=+0.212 (n=558)
...
- **PATRÓN** `py_entrada` < `0.44` → IC=+0.144 (n=821)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.154 (n=2814)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.337 (n=959)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.158 (n=460)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1302.9991` → IC=+0.150 (n=656)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.163 (n=1088)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.170 (n=725)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.147 (n=310)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.202 (n=11481)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.226 (n=3792)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.167 (n=2783)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.182 (n=1941)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.248 (n=844)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.185 (n=2593)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.199 (n=2653)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.195 (n=1964)

  - _Acción_: Kelly boost +0.98€ cuando `py_entrada` < 0.71 (IC base=+0.192)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.192 (n=1025)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.190 (n=6360)

  - _Acción_: Kelly boost +0.95€ cuando `py_entrada` > 0.71 (IC base=+0.176)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.224 (n=6121)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `4097.0073` → IC=+0.311 (n=368)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4097.0073 (IC base=+0.291)

### FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#BTC#60min
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.303 (n=343)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.277)

- **PATRÓN** `py_entrada` > `0.805` → IC=+0.334 (n=179)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.805 (IC base=+0.277)

- **PATRÓN** `libro_liquidez` > `4258.3281` → IC=+0.299 (n=327)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4258.3281 (IC base=+0.277)

### FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#ETH#60min
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.335 (n=374)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1466.3152` → IC=+0.310 (n=473)
...
- **PATRÓN** `volumen_spike_ratio` > `1.9062` → IC=+0.206 (n=3678)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9062 (IC base=+0.105)

- **PATRÓN** `ibs_20min` < `0.5699` → IC=+0.134 (n=9714)

  - _Acción_: Kelly boost +0.67€ cuando `ibs_20min` < 0.5699 (IC base=+0.066)

- **PATRÓN** `dist_vwap_pct` > `0.6175` → IC=+0.196 (n=719)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.6175 (IC base=+0.066)

- **PATRÓN** `dist_vwap_pct` < `0.1545` → IC=+0.173 (n=3073)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.1545 (IC base=+0.066)

- **PATRÓN** `volumen_regimen` < `0.6971` → IC=+0.183 (n=1497)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 0.6971 (IC base=+0.066)

- **PATRÓN** `volumen_regimen` > `1.0527` → IC=+0.176 (n=1543)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 1.0527 (IC base=+0.066)

- **PATRÓN** `volumen_pendiente_norm` > `0.1674` → IC=+0.224 (n=1618)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1674 (IC base=+0.066)

- **PATRÓN** `volumen_spike_ratio` > `1.5701` → IC=+0.201 (n=5115)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5701 (IC base=+0.066)

- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.211 (n=5513)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3504` → IC=+0.166 (n=1819)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.3504 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.167 (n=880)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.269 (n=704)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4352` → IC=+0.163 (n=1701)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.4352 (IC base=+0.161)

- **PATRÓN** `sigma_h` > `0.005` → IC=+0.250 (n=1232)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.005 (IC base=+0.235)

- **PATRÓN** `drift_60min` |x|≤ `0.093` → IC=+0.280 (n=458)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.093 (IC base=+0.235)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.243 (n=1248)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.235)

- **PATRÓN** `ibs_20min` < `0.0542` → IC=+0.294 (n=605)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0542 (IC base=+0.235)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.432` → IC=+0.237 (n=211)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.432 (IC base=+0.235)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.444` → IC=+0.247 (n=1426)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.444 (IC base=+0.235)

- **PATRÓN** `volumen_pendiente_norm` < `0.0922` → IC=+0.232 (n=1181)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4285` → IC=+0.231 (n=418)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4285 (IC base=+0.235)

- **PATRÓN** `volumen_spike_ratio` > `2.6178` → IC=+0.243 (n=418)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6178 (IC base=+0.235)

- **PATRÓN** `libro_liquidez` > `1804.04` → IC=+0.235 (n=916)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1804.04 (IC base=+0.235)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `sigma_h` < `0.0032` → IC=+0.233 (n=612)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0032 (IC base=+0.216)

- **PATRÓN** `drift_60min` |x|≤ `0.0846` → IC=+0.251 (n=463)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0846 (IC base=+0.216)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.232 (n=1388)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.216)

- **PATRÓN** `ibs_20min` > `0.9865` → IC=+0.270 (n=463)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9865 (IC base=+0.216)

- **PATRÓN** `dist_vwap_pct` > `0.2068` → IC=+0.218 (n=743)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2068 (IC base=+0.216)

- **PATRÓN** `dist_vwap_pct` < `0.5843` → IC=+0.220 (n=1454)
...
- **PATRÓN** `volumen_regimen` < `1.2554` → IC=+0.219 (n=1388)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2554 (IC base=+0.216)

- **PATRÓN** `volumen_regimen` > `1.0825` → IC=+0.226 (n=629)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7475` → IC=+0.216 (n=907)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7475 (IC base=+0.216)

- **PATRÓN** `volumen_spike_ratio` > `2.3632` → IC=+0.228 (n=454)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3632 (IC base=+0.216)

- **PATRÓN** `libro_liquidez` > `16041.9297` → IC=+0.223 (n=629)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0769` → IC=+0.163 (n=482)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.82€ cuando `drift_60min` |x|≤ 0.0769 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.172 (n=571)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.6942` → IC=+0.172 (n=1447)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.282` → IC=+0.143 (n=1316)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` < 4.282 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `1.204` → IC=+0.151 (n=1446)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.204 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` > `0.1567` → IC=+0.177 (n=382)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.1567 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` < `2.4183` → IC=+0.152 (n=1336)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 2.4183 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.4164` → IC=+0.148 (n=1335)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.224` → IC=+0.249 (n=376)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.224 (IC base=+0.186)

- **PATRÓN** `volumen_pendiente_norm` < `0.2106` → IC=+0.188 (n=1786)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8308` → IC=+0.203 (n=770)
...
- **PATRÓN** `libro_liquidez` > `1889.1356` → IC=+0.224 (n=696)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1889.1356 (IC base=+0.212)

- **PATRÓN** `ballena_activa_n` < `24.0` → IC=+0.215 (n=911)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.157 (n=100)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=2217)

- **PATRÓN** `ibs_20min` > `0.9436` → IC=+0.215 (n=359)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9436 (IC base=+0.024)

- **PATRÓN** `dist_vwap_pct` > `0.1574` → IC=+0.316 (n=210)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1574 (IC base=+0.024)

- **PATRÓN** `dist_vwap_pct` < `0.7982` → IC=+0.331 (n=358)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.7982 (IC base=+0.024)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.739` → IC=+0.157 (n=712)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 4.739 (IC base=+0.024)
...
- **PATRÓN** `ballena_activa_n` < `162.0` → IC=+0.327 (n=315)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 162.0 (IC base=+0.024)

- **PATRÓN** `dist_vwap_pct` > `0.337` → IC=+0.191 (n=263)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.337 (IC base=+0.018)

- **PATRÓN** `volumen_regimen` < `0.8475` → IC=+0.166 (n=557)
...
- **FILTRO** `ibs_20min` > `0.2549` → IC=-0.125 (n=2187)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2549
  - _Potencial_: sin este filtro IC_bueno=+0.128 (n=1078)

- **FILTRO** `sigma_ewma_delta_pct` > `8.667` → IC=-0.210 (n=350)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.667
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=2915)
...
- **PATRÓN** `ibs_20min` < `0.2549` → IC=+0.128 (n=1078)
...
- **FILTRO** `drift_60min` |x|> `0.6598` → IC=-0.183 (n=566)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6598
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=1701)

- **FILTRO** `ibs_20min` < `0.7143` → IC=-0.155 (n=1496)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7143
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=771)
...
- **FILTRO** `ibs_20min` > `0.77` → IC=-0.202 (n=829)

  - _Acción_: SKIP cuando `ibs_20min` > 0.77
  - _Potencial_: sin este filtro IC_bueno=+0.036 (n=2501)

- **PATRÓN** `dist_vwap_pct` > `0.455` → IC=+0.310 (n=135)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.455 (IC base=-0.067)

- **PATRÓN** `dist_vwap_pct` < `0.2747` → IC=+0.315 (n=285)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2747 (IC base=-0.067)

- **PATRÓN** `volumen_regimen` > `0.6251` → IC=+0.301 (n=349)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6251 (IC base=-0.067)

- **PATRÓN** `volumen_pendiente_norm` < `0.1692` → IC=+0.295 (n=349)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1692 (IC base=-0.067)

- **PATRÓN** `volumen_spike_ratio` < `1.3932` → IC=+0.305 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3932 (IC base=-0.067)

- **PATRÓN** `volumen_spike_ratio` > `1.8242` → IC=+0.297 (n=220)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8242 (IC base=-0.067)

- **PATRÓN** `dist_vwap_pct` > `0.8909` → IC=+0.269 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8909 (IC base=-0.023)

- **PATRÓN** `volumen_regimen` < `0.7364` → IC=+0.252 (n=336)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7364 (IC base=-0.023)

- **PATRÓN** `volumen_regimen` > `1.0882` → IC=+0.279 (n=346)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1498` → IC=+0.257 (n=573)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1498 (IC base=-0.023)

- **PATRÓN** `volumen_spike_ratio` > `1.4322` → IC=+0.246 (n=651)
...
- **PATRÓN** `sigma_h` > `0.0098` → IC=+0.196 (n=3371)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0098 (IC base=+0.098)

- **PATRÓN** `ibs_20min` > `0.4753` → IC=+0.188 (n=9021)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.4753 (IC base=+0.098)

- **PATRÓN** `dist_vwap_pct` > `0.7746` → IC=+0.288 (n=1066)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7746 (IC base=+0.098)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.604` → IC=+0.156 (n=4753)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 3.604 (IC base=+0.098)

- **PATRÓN** `volumen_regimen` > `0.6895` → IC=+0.246 (n=3209)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2236` → IC=+0.270 (n=172)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2236 (IC base=+0.045)

- **PATRÓN** `volumen_spike_ratio` < `1.4309` → IC=+0.175 (n=293)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.4309 (IC base=+0.045)

- **PATRÓN** `volumen_spike_ratio` > `2.1457` → IC=+0.198 (n=399)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.1457 (IC base=+0.045)

- **PATRÓN** `ballena_activa_n` < `47.0` → IC=+0.173 (n=849)

  - _Acción_: Kelly boost +0.87€ cuando `ballena_activa_n` < 47.0 (IC base=+0.045)

- **PATRÓN** `volumen_pendiente_norm` < `0.1673` → IC=+0.447 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1673 (IC base=-0.017)

- **PATRÓN** `volumen_spike_ratio` < `2.6396` → IC=+0.436 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.6396 (IC base=-0.017)

- **PATRÓN** `volumen_spike_ratio` > `2.3568` → IC=+0.433 (n=28)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4243` → IC=+0.198 (n=303)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 1.4243 (IC base=+0.026)

- **PATRÓN** `ballena_activa_n` < `245.0` → IC=+0.190 (n=395)
...
- **PATRÓN** `volumen_regimen` > `1.0364` → IC=+0.228 (n=255)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0364 (IC base=+0.005)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.249 (n=1597)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1041` → IC=+0.259 (n=1349)
...
- **PATRÓN** `volumen_spike_ratio` > `2.7347` → IC=+0.287 (n=534)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7347 (IC base=+0.281)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.286 (n=864)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.281)

- **PATRÓN** `libro_liquidez` > `1877.4384` → IC=+0.298 (n=577)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1877.4384 (IC base=+0.281)

- **PATRÓN** `ballena_activa_n` < `29.0` → IC=+0.286 (n=764)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 29.0 (IC base=+0.281)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **FILTRO** `ibs_20min` < `0.2857` → IC=-0.185 (n=493)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2857
  - _Potencial_: sin este filtro IC_bueno=+0.078 (n=1482)

- **FILTRO** `ibs_20min` > `0.7745` → IC=-0.184 (n=597)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7745
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=1792)

- **PATRÓN** `ibs_20min` > `0.822` → IC=+0.155 (n=673)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` > 0.822 (IC base=+0.012)

- **PATRÓN** `dist_vwap_pct` > `0.4545` → IC=+0.222 (n=246)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4545 (IC base=+0.012)

- **PATRÓN** `dist_vwap_pct` < `0.2202` → IC=+0.210 (n=401)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2202 (IC base=+0.012)

- **PATRÓN** `volumen_regimen` < `0.9869` → IC=+0.234 (n=497)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9869 (IC base=+0.012)

- **PATRÓN** `volumen_regimen` > `0.5875` → IC=+0.209 (n=565)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.5875 (IC base=+0.012)

- **PATRÓN** `volumen_pendiente_norm` > `0.0806` → IC=+0.256 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0806 (IC base=+0.012)

- **PATRÓN** `volumen_spike_ratio` < `1.5113` → IC=+0.264 (n=235)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5113 (IC base=+0.012)

- **PATRÓN** `ballena_activa_n` < `151.0` → IC=+0.242 (n=541)
...
- **FILTRO** `ibs_20min` < `0.72` → IC=-0.198 (n=1069)

  - _Acción_: SKIP cuando `ibs_20min` < 0.72
  - _Potencial_: sin este filtro IC_bueno=+0.279 (n=1072)

- **FILTRO** `ibs_20min` > `0.6857` → IC=-0.229 (n=556)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6857
  - _Potencial_: sin este filtro IC_bueno=+0.097 (n=1672)

- **FILTRO** `sigma_ewma_delta_pct` > `4.705` → IC=-0.180 (n=485)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.705
  - _Potencial_: sin este filtro IC_bueno=+0.070 (n=1743)

- **PATRÓN** `ibs_20min` > `0.72` → IC=+0.279 (n=1072)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.72 (IC base=+0.041)

- **PATRÓN** `dist_vwap_pct` > `0.8494` → IC=+0.335 (n=246)
...
- **PATRÓN** `volumen_regimen` < `0.87` → IC=+0.301 (n=526)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.87 (IC base=+0.041)

- **PATRÓN** `volumen_regimen` > `0.6465` → IC=+0.290 (n=788)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6465 (IC base=+0.041)

- **PATRÓN** `volumen_pendiente_norm` < `0.1034` → IC=+0.292 (n=734)
...
- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.312 (n=663)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 56.0 (IC base=+0.041)

- **PATRÓN** `ibs_20min` < `0.5783` → IC=+0.120 (n=1471)

  - _Acción_: Kelly boost +0.60€ cuando `ibs_20min` < 0.5783 (IC base=+0.015)

- **PATRÓN** `dist_vwap_pct` > `0.7308` → IC=+0.199 (n=134)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.7308 (IC base=+0.015)

- **PATRÓN** `dist_vwap_pct` < `0.218` → IC=+0.219 (n=493)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.218 (IC base=+0.015)

- **PATRÓN** `volumen_regimen` < `0.7031` → IC=+0.250 (n=250)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7031 (IC base=+0.015)

- **PATRÓN** `volumen_pendiente_norm` < `0.0972` → IC=+0.209 (n=524)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0972 (IC base=+0.015)

- **PATRÓN** `volumen_pendiente_norm` > `0.0689` → IC=+0.206 (n=212)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0689 (IC base=+0.015)

- **PATRÓN** `volumen_spike_ratio` < `2.4844` → IC=+0.220 (n=530)
...
- **PATRÓN** `sigma_h` > `0.0168` → IC=+0.316 (n=873)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.7411` → IC=+0.322 (n=1170)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7411 (IC base=+0.276)

- **PATRÓN** `dist_vwap_pct` > `0.2086` → IC=+0.316 (n=780)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2086 (IC base=+0.276)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.574` → IC=+0.299 (n=691)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.574 (IC base=+0.276)

- **PATRÓN** `volumen_regimen` > `0.8611` → IC=+0.302 (n=873)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8611 (IC base=+0.276)

- **PATRÓN** `volumen_pendiente_norm` > `0.2816` → IC=+0.322 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2816 (IC base=+0.276)

- **PATRÓN** `volumen_spike_ratio` > `2.1503` → IC=+0.293 (n=564)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1503 (IC base=+0.276)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.283 (n=1403)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.276)

- **PATRÓN** `libro_liquidez` > `2630.254` → IC=+0.291 (n=873)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.282 (n=490)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3939` → IC=+0.305 (n=1414)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3939 (IC base=+0.272)

- **PATRÓN** `dist_vwap_pct` > `0.3048` → IC=+0.281 (n=542)
...
- **PATRÓN** `volumen_regimen` > `1.2435` → IC=+0.306 (n=472)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1485` → IC=+0.276 (n=566)
...
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.170 (n=2630)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0049 (IC base=+0.167)

- **PATRÓN** `sigma_h` > `0.0112` → IC=+0.201 (n=2622)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0112 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0904` → IC=+0.186 (n=2622)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.0904 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.179 (n=8206)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 5.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` > `0.5769` → IC=+0.217 (n=7865)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5769 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.1774` → IC=+0.195 (n=3435)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1774 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.347` → IC=+0.255 (n=1611)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.347 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.2119` → IC=+0.160 (n=5206)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2119 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` > `0.6307` → IC=+0.161 (n=5205)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.6307 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.2958` → IC=+0.194 (n=1178)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2958 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.5596` → IC=+0.169 (n=3321)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.5596 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.6202` → IC=+0.176 (n=2517)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.6202 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `2402.7795` → IC=+0.169 (n=5243)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2402.7795 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `116.0` → IC=+0.180 (n=6787)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 116.0 (IC base=+0.167)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.182 (n=5023)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0066 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0814` → IC=+0.208 (n=2510)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0814 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.204 (n=2911)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` < `0.4783` → IC=+0.226 (n=7521)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4783 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.9248` → IC=+0.151 (n=726)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.341` → IC=+0.195 (n=1278)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 10.341 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.174` → IC=+0.152 (n=5451)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.174 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.291` → IC=+0.221 (n=1079)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.291 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.5609` → IC=+0.166 (n=3009)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.5609 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.2527` → IC=+0.170 (n=3100)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.2527 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `116.0` → IC=+0.172 (n=6484)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3446` → IC=+0.206 (n=1341)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3446 (IC base=+0.182)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.182 (n=1413)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.301 (n=660)
...
- **PATRÓN** `ibs_20min` < `0.3437` → IC=+0.264 (n=978)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3437 (IC base=+0.239)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.286` → IC=+0.251 (n=1055)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.286 (IC base=+0.239)

- **PATRÓN** `volumen_pendiente_norm` < `0.0972` → IC=+0.235 (n=817)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4215` → IC=+0.262 (n=301)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4215 (IC base=+0.239)

- **PATRÓN** `libro_liquidez` > `1806.4548` → IC=+0.245 (n=653)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1806.4548 (IC base=+0.239)

- **PATRÓN** `ballena_activa_n` < `47.0` → IC=+0.239 (n=771)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 47.0 (IC base=+0.239)

### GBM_LATE_15M_MULTIHORIZONTE#BTC#15min
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.227 (n=393)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `volumen_regimen` < `1.2653` → IC=+0.164 (n=1171)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.2653 (IC base=+0.162)

- **PATRÓN** `volumen_regimen` > `1.0742` → IC=+0.168 (n=531)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5005` → IC=+0.181 (n=500)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 1.5005 (IC base=+0.162)

- **PATRÓN** `libro_liquidez` > `15958.35` → IC=+0.162 (n=531)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0596` → IC=+0.203 (n=426)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5649` → IC=+0.186 (n=1278)
...
- **PATRÓN** `volumen_regimen` < `1.2116` → IC=+0.155 (n=1278)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.2116 (IC base=+0.136)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4315` → IC=+0.145 (n=1166)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 2.4315 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.4185` → IC=+0.136 (n=1166)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` > 1.4185 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `215.0` → IC=+0.167 (n=364)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 215.0 (IC base=+0.136)

### GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min
- **PATRÓN** `sigma_h` > `0.01` → IC=+0.221 (n=599)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.01 (IC base=+0.197)

- **PATRÓN** `drift_60min` |x|≤ `0.2308` → IC=+0.214 (n=880)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2308 (IC base=+0.197)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.204 (n=1367)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.197)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.876` → IC=+0.273 (n=408)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.876 (IC base=+0.197)

- **PATRÓN** `volumen_pendiente_norm` > `0.2074` → IC=+0.198 (n=389)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.2074 (IC base=+0.197)

- **PATRÓN** `volumen_spike_ratio` < `1.6272` → IC=+0.200 (n=418)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6272 (IC base=+0.197)

- **PATRÓN** `volumen_spike_ratio` > `2.8416` → IC=+0.213 (n=569)
...
- **PATRÓN** `libro_liquidez` > `1961.26` → IC=+0.199 (n=440)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1961.26 (IC base=+0.197)

- **PATRÓN** `sigma_h` < `0.0114` → IC=+0.229 (n=1095)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `volumen_spike_ratio` > `2.2184` → IC=+0.226 (n=676)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2184 (IC base=+0.217)

- **PATRÓN** `libro_liquidez` > `1888.9584` → IC=+0.217 (n=497)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1888.9584 (IC base=+0.217)
...
- **PATRÓN** `volumen_regimen` < `1.0405` → IC=+0.145 (n=1107)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.0405 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` > `0.6283` → IC=+0.149 (n=1258)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 0.6283 (IC base=+0.143)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4237` → IC=+0.151 (n=411)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 1.4237 (IC base=+0.143)
...
- **PATRÓN** `libro_liquidez` > `6496.8974` → IC=+0.185 (n=839)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 6496.8974 (IC base=+0.143)

- **PATRÓN** `ballena_activa_n` < `164.0` → IC=+0.147 (n=1198)

  - _Acción_: Kelly boost +0.73€ cuando `ballena_activa_n` < 164.0 (IC base=+0.143)

- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.153 (n=1329)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` < 0.0072 (IC base=+0.123)

- **PATRÓN** `drift_60min` |x|≤ `0.3855` → IC=+0.142 (n=1329)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.3855 (IC base=+0.123)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.177 (n=519)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 17.0 (IC base=+0.123)

- **PATRÓN** `ibs_20min` < `0.6398` → IC=+0.171 (n=1329)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` < 0.6398 (IC base=+0.123)

- **PATRÓN** `dist_vwap_pct` < `0.5897` → IC=+0.132 (n=1542)
...
- **PATRÓN** `volumen_regimen` < `0.8475` → IC=+0.147 (n=886)
...
- **PATRÓN** `libro_liquidez` > `10007.3401` → IC=+0.160 (n=603)
...
- **PATRÓN** `volumen_regimen` < `1.2203` → IC=+0.131 (n=1420)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 1.2203 (IC base=+0.119)

- **PATRÓN** `volumen_regimen` > `0.6455` → IC=+0.123 (n=1419)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4411` → IC=+0.145 (n=457)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 1.4411 (IC base=+0.119)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.127 (n=1484)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.02 (IC base=+0.119)

- **PATRÓN** `libro_liquidez` > `2893.8059` → IC=+0.198 (n=644)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 2893.8059 (IC base=+0.119)

- **PATRÓN** `ballena_activa_n` < `50.0` → IC=+0.137 (n=1094)

  - _Acción_: Kelly boost +0.68€ cuando `ballena_activa_n` < 50.0 (IC base=+0.119)

- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.153 (n=635)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0061 (IC base=+0.113)

- **PATRÓN** `drift_60min` |x|≤ `0.104` → IC=+0.158 (n=481)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.104 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.131 (n=1461)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 5.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` < `0.5625` → IC=+0.210 (n=1441)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5625 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `1.0109` → IC=+0.146 (n=190)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 1.0109 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.2006` → IC=+0.138 (n=1330)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.2006 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.978` → IC=+0.138 (n=233)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` > 8.978 (IC base=+0.113)
...
- **PATRÓN** `volumen_spike_ratio` > `2.4234` → IC=+0.133 (n=431)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 2.4234 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `1463.4782` → IC=+0.136 (n=1287)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 1463.4782 (IC base=+0.113)

### GBM_LATE_15M_MULTIHORIZONTE#XRP#15min
- **PATRÓN** `sigma_h` > `0.0188` → IC=+0.213 (n=906)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0188 (IC base=+0.201)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.204 (n=1413)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.74` → IC=+0.259 (n=1214)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.74 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` > `0.5119` → IC=+0.222 (n=648)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5119 (IC base=+0.201)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.578` → IC=+0.243 (n=639)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.578 (IC base=+0.201)

- **PATRÓN** `volumen_regimen` < `1.2068` → IC=+0.203 (n=1359)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2068 (IC base=+0.201)

- **PATRÓN** `volumen_regimen` > `0.624` → IC=+0.211 (n=1359)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.624 (IC base=+0.201)

- **PATRÓN** `volumen_pendiente_norm` > `0.2335` → IC=+0.263 (n=260)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2335 (IC base=+0.201)

- **PATRÓN** `volumen_spike_ratio` < `2.1481` → IC=+0.211 (n=1156)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1481 (IC base=+0.201)

- **PATRÓN** `volumen_spike_ratio` > `1.4095` → IC=+0.208 (n=1313)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4095 (IC base=+0.201)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.205 (n=1447)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.201)

- **PATRÓN** `libro_liquidez` > `2824.2811` → IC=+0.209 (n=616)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2824.2811 (IC base=+0.201)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.157 (n=627)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.1004` → IC=+0.152 (n=627)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.1004 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.163 (n=1744)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 8.0 (IC base=+0.145)

- **PATRÓN** `ibs_20min` > `0.3875` → IC=+0.177 (n=1882)

  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` > 0.3875 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.8823` → IC=+0.183 (n=298)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.8823 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.703` → IC=+0.173 (n=858)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 3.703 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` < `0.8735` → IC=+0.160 (n=1103)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.8735 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` > `1.2084` → IC=+0.155 (n=551)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.2084 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` > `0.165` → IC=+0.170 (n=522)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` > 0.165 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` < `1.4411` → IC=+0.162 (n=604)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 1.4411 (IC base=+0.145)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.148 (n=2136)

  - _Acción_: Kelly boost +0.74€ cuando `libro_spread` < 0.02 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `12429.5841` → IC=+0.158 (n=627)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 12429.5841 (IC base=+0.145)

- **PATRÓN** `ballena_activa_n` < `161.0` → IC=+0.164 (n=1657)

  - _Acción_: Kelly boost +0.82€ cuando `ballena_activa_n` < 161.0 (IC base=+0.145)

- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.139 (n=1312)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0056 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.124 (n=1992)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 5.0 (IC base=+0.110)

- **PATRÓN** `ibs_20min` < `0.6591` → IC=+0.138 (n=1966)
...
- **PATRÓN** `volumen_regimen` < `0.6252` → IC=+0.126 (n=592)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` < 0.6252 (IC base=+0.110)

- **PATRÓN** `volumen_spike_ratio` < `1.4556` → IC=+0.138 (n=631)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` < 1.4556 (IC base=+0.110)

- **PATRÓN** `libro_liquidez` > `2750.9096` → IC=+0.122 (n=1757)

  - _Acción_: Kelly boost +0.61€ cuando `libro_liquidez` > 2750.9096 (IC base=+0.110)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3447` → IC=+0.158 (n=630)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.3447 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.150 (n=569)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 7.0 (IC base=+0.137)

- **PATRÓN** `ibs_20min` < `0.6039` → IC=+0.183 (n=554)

  - _Acción_: Kelly boost +0.92€ cuando `ibs_20min` < 0.6039 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.4881` → IC=+0.152 (n=723)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.4881 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.406` → IC=+0.157 (n=243)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 4.406 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `1.208` → IC=+0.140 (n=629)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` < 1.208 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` > `1.0598` → IC=+0.166 (n=285)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 1.0598 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` > `0.1583` → IC=+0.196 (n=169)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.1583 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` < `2.1023` → IC=+0.154 (n=545)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.1023 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `1.4203` → IC=+0.151 (n=619)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.4203 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `327.0` → IC=+0.153 (n=525)

  - _Acción_: Kelly boost +0.76€ cuando `ballena_activa_n` < 327.0 (IC base=+0.137)

### GBM_LATE_15M_PYCONFIRMADO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.251 (n=259)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0039 (IC base=+0.188)

- **PATRÓN** `sigma_h` > `0.007` → IC=+0.195 (n=195)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.007 (IC base=+0.188)

- **PATRÓN** `drift_60min` |x|≤ `0.0967` → IC=+0.211 (n=195)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0967 (IC base=+0.188)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.203 (n=610)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.188)

- **PATRÓN** `ibs_20min` > `0.9737` → IC=+0.267 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9737 (IC base=+0.188)

- **PATRÓN** `dist_vwap_pct` > `0.9336` → IC=+0.214 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9336 (IC base=+0.188)

- **PATRÓN** `dist_vwap_pct` < `0.2278` → IC=+0.189 (n=509)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.2278 (IC base=+0.188)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.015` → IC=+0.223 (n=193)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.015 (IC base=+0.188)

- **PATRÓN** `volumen_regimen` < `0.8358` → IC=+0.193 (n=389)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` < 0.8358 (IC base=+0.188)

- **PATRÓN** `volumen_regimen` > `1.156` → IC=+0.211 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.156 (IC base=+0.188)

- **PATRÓN** `volumen_pendiente_norm` > `0.2622` → IC=+0.267 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2622 (IC base=+0.188)

- **PATRÓN** `volumen_spike_ratio` < `1.3991` → IC=+0.222 (n=192)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3991 (IC base=+0.188)

- **PATRÓN** `volumen_spike_ratio` > `2.4379` → IC=+0.227 (n=192)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4379 (IC base=+0.188)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.196 (n=649)

  - _Acción_: Kelly boost +0.98€ cuando `libro_spread` < 0.01 (IC base=+0.188)

- **PATRÓN** `libro_liquidez` > `12421.138` → IC=+0.216 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12421.138 (IC base=+0.188)

- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.121 (n=484)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `volumen_regimen` < `0.6838` → IC=+0.127 (n=242)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` < 0.6838 (IC base=+0.093)
...
- **PATRÓN** `libro_liquidez` > `3827.6428` → IC=+0.121 (n=491)

  - _Acción_: Kelly boost +0.60€ cuando `libro_liquidez` > 3827.6428 (IC base=+0.093)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1156` → IC=+0.136 (n=141)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.1156 (IC base=+0.130)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.165 (n=395)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 8.0 (IC base=+0.130)

- **PATRÓN** `ibs_20min` > `0.725` → IC=+0.204 (n=376)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.725 (IC base=+0.130)

- **PATRÓN** `dist_vwap_pct` > `0.9284` → IC=+0.224 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9284 (IC base=+0.130)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.1` → IC=+0.207 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.1 (IC base=+0.130)

- **PATRÓN** `volumen_regimen` < `1.069` → IC=+0.148 (n=370)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 1.069 (IC base=+0.130)

- **PATRÓN** `volumen_pendiente_norm` > `0.287` → IC=+0.177 (n=60)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.287 (IC base=+0.130)

- **PATRÓN** `volumen_spike_ratio` > `2.2097` → IC=+0.174 (n=182)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.2097 (IC base=+0.130)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.132 (n=457)

  - _Acción_: Kelly boost +0.66€ cuando `libro_spread` < 0.02 (IC base=+0.130)

- **PATRÓN** `libro_liquidez` > `3081.1151` → IC=+0.197 (n=140)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 3081.1151 (IC base=+0.130)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5995` → IC=+0.186 (n=3223)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 2.5995 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `2345.136` → IC=+0.171 (n=6705)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 2345.136 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `87.0` → IC=+0.194 (n=7669)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 87.0 (IC base=+0.169)

- **PATRÓN** `sigma_h` < `0.007` → IC=+0.192 (n=6087)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.007 (IC base=+0.181)

- **PATRÓN** `drift_60min` |x|≤ `0.149` → IC=+0.189 (n=4018)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.149 (IC base=+0.181)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.207 (n=3472)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.181)

- **PATRÓN** `ibs_20min` < `0.5652` → IC=+0.238 (n=9127)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5652 (IC base=+0.181)

- **PATRÓN** `dist_vwap_pct` < `0.25` → IC=+0.162 (n=5646)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.25 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.029` → IC=+0.201 (n=1282)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.029 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.729` → IC=+0.182 (n=8823)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.729 (IC base=+0.181)

- **PATRÓN** `volumen_regimen` < `0.7043` → IC=+0.162 (n=2758)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.7043 (IC base=+0.181)

- **PATRÓN** `volumen_regimen` > `1.2035` → IC=+0.153 (n=2090)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.2035 (IC base=+0.181)

- **PATRÓN** `volumen_pendiente_norm` > `0.2881` → IC=+0.243 (n=1200)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2881 (IC base=+0.181)

- **PATRÓN** `volumen_spike_ratio` > `2.6271` → IC=+0.193 (n=2791)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 2.6271 (IC base=+0.181)

- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.193 (n=5391)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3575` → IC=+0.191 (n=1705)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.3575 (IC base=+0.190)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.200 (n=820)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.325 (n=614)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.190)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.067` → IC=+0.313 (n=758)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.067 (IC base=+0.190)
...
- **PATRÓN** `volumen_spike_ratio` > `2.242` → IC=+0.198 (n=730)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.242 (IC base=+0.190)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.192 (n=1030)

  - _Acción_: Kelly boost +0.96€ cuando `libro_spread` < 0.02 (IC base=+0.190)

- **PATRÓN** `sigma_h` > `0.005` → IC=+0.262 (n=1195)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.005 (IC base=+0.259)

- **PATRÓN** `drift_60min` |x|≤ `0.1298` → IC=+0.285 (n=589)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1298 (IC base=+0.259)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.269 (n=1213)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.259)

- **PATRÓN** `ibs_20min` < `0.3505` → IC=+0.290 (n=1177)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3505 (IC base=+0.259)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.519` → IC=+0.264 (n=1340)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.519 (IC base=+0.259)

- **PATRÓN** `volumen_pendiente_norm` > `0.226` → IC=+0.278 (n=246)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.226 (IC base=+0.259)

- **PATRÓN** `volumen_spike_ratio` < `1.5466` → IC=+0.258 (n=540)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5466 (IC base=+0.259)

- **PATRÓN** `volumen_spike_ratio` > `2.6339` → IC=+0.279 (n=409)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6339 (IC base=+0.259)

- **PATRÓN** `libro_liquidez` > `1803.42` → IC=+0.263 (n=892)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1803.42 (IC base=+0.259)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.190 (n=539)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.0028 (IC base=+0.149)

- **PATRÓN** `drift_60min` |x|≤ `0.0848` → IC=+0.161 (n=538)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.0848 (IC base=+0.149)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.163 (n=1688)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 5.0 (IC base=+0.149)

- **PATRÓN** `ibs_20min` > `0.3085` → IC=+0.203 (n=1612)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3085 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` > `0.3461` → IC=+0.192 (n=654)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.3461 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.74` → IC=+0.170 (n=368)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 9.74 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.235` → IC=+0.153 (n=1451)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 4.235 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` < `0.629` → IC=+0.180 (n=538)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1148` → IC=+0.162 (n=1370)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.1148 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` > `1.4033` → IC=+0.154 (n=1557)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.4033 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `15748.4784` → IC=+0.158 (n=731)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 15748.4784 (IC base=+0.149)

- **PATRÓN** `ballena_activa_n` < `475.0` → IC=+0.158 (n=1488)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 475.0 (IC base=+0.149)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.166 (n=1395)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0057 (IC base=+0.152)

- **PATRÓN** `drift_60min` |x|≤ `0.2608` → IC=+0.166 (n=1227)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.2608 (IC base=+0.152)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.179 (n=466)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 18.0 (IC base=+0.152)

- **PATRÓN** `ibs_20min` < `0.6545` → IC=+0.196 (n=1394)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` < 0.6545 (IC base=+0.152)

- **PATRÓN** `dist_vwap_pct` > `0.6965` → IC=+0.152 (n=228)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.6965 (IC base=+0.152)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.279` → IC=+0.153 (n=1262)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` < 4.279 (IC base=+0.152)

- **PATRÓN** `volumen_regimen` < `1.1894` → IC=+0.164 (n=1394)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4038` → IC=+0.160 (n=1296)
...
- **PATRÓN** `ballena_activa_n` < `421.0` → IC=+0.159 (n=1056)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.225 (n=1709)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.67` → IC=+0.256 (n=1455)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2114` → IC=+0.222 (n=1609)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2114 (IC base=+0.218)

- **PATRÓN** `volumen_spike_ratio` > `2.8331` → IC=+0.241 (n=704)
...
- **PATRÓN** `libro_liquidez` > `1887.161` → IC=+0.242 (n=689)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1887.161 (IC base=+0.232)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.229 (n=1322)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 54.0 (IC base=+0.232)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0035` → IC=+0.179 (n=571)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0035 (IC base=+0.137)

- **PATRÓN** `drift_60min` |x|≤ `0.4388` → IC=+0.145 (n=1713)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.4388 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.151 (n=1793)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 5.0 (IC base=+0.137)

- **PATRÓN** `ibs_20min` > `0.877` → IC=+0.261 (n=776)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.877 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.3755` → IC=+0.162 (n=676)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.3755 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.432` → IC=+0.168 (n=281)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` > 11.432 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `0.8764` → IC=+0.156 (n=1142)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.8764 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` > `0.2371` → IC=+0.216 (n=318)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2371 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` < `1.5223` → IC=+0.150 (n=730)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.5223 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `1.768` → IC=+0.150 (n=1105)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.768 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `8051.1298` → IC=+0.231 (n=776)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8051.1298 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `79.0` → IC=+0.146 (n=535)

  - _Acción_: Kelly boost +0.73€ cuando `ballena_activa_n` < 79.0 (IC base=+0.137)

- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.157 (n=1399)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0076 (IC base=+0.138)

- **PATRÓN** `drift_60min` |x|≤ `0.4484` → IC=+0.155 (n=1397)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.4484 (IC base=+0.138)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.175 (n=524)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.7022` → IC=+0.185 (n=1397)

  - _Acción_: Kelly boost +0.92€ cuando `ibs_20min` < 0.7022 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.6029` → IC=+0.144 (n=1549)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.6029 (IC base=+0.138)
...
- **PATRÓN** `volumen_regimen` < `0.6938` → IC=+0.151 (n=615)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.6938 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `1.1877` → IC=+0.141 (n=466)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 1.1877 (IC base=+0.138)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4416` → IC=+0.152 (n=1325)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.223` → IC=+0.171 (n=259)
...
- **PATRÓN** `libro_liquidez` > `2844.2019` → IC=+0.170 (n=553)
...
- **PATRÓN** `sigma_h` > `0.0193` → IC=+0.218 (n=1134)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0193 (IC base=+0.208)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.215 (n=1775)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.5143` → IC=+0.249 (n=1701)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5143 (IC base=+0.208)

- **PATRÓN** `dist_vwap_pct` > `0.2044` → IC=+0.234 (n=1006)
...
- **PATRÓN** `volumen_regimen` < `1.2434` → IC=+0.210 (n=1701)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2434 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` > `0.64` → IC=+0.215 (n=1701)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.64 (IC base=+0.208)

- **PATRÓN** `volumen_pendiente_norm` > `0.2358` → IC=+0.243 (n=298)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2358 (IC base=+0.208)

- **PATRÓN** `volumen_spike_ratio` > `2.5103` → IC=+0.238 (n=547)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5103 (IC base=+0.208)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.217 (n=1791)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.208)

- **PATRÓN** `libro_liquidez` > `2626.3484` → IC=+0.219 (n=1134)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2626.3484 (IC base=+0.208)

- **PATRÓN** `sigma_h` < `0.009` → IC=+0.226 (n=608)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.009 (IC base=+0.200)

- **PATRÓN** `sigma_h` > `0.0256` → IC=+0.220 (n=606)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0256 (IC base=+0.200)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.209 (n=1286)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.200)

- **PATRÓN** `ibs_20min` < `0.5172` → IC=+0.255 (n=1819)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5172 (IC base=+0.200)

- **PATRÓN** `dist_vwap_pct` > `1.2824` → IC=+0.203 (n=301)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2824 (IC base=+0.200)

- **PATRÓN** `dist_vwap_pct` < `0.9297` → IC=+0.203 (n=2022)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.9297 (IC base=+0.200)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.832` → IC=+0.259 (n=251)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.832 (IC base=+0.200)

- **PATRÓN** `volumen_regimen` > `1.2357` → IC=+0.232 (n=606)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2357 (IC base=+0.200)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2017` → IC=+0.196 (n=1439)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.2017 (IC base=+0.200)

- **PATRÓN** `volumen_spike_ratio` > `1.432` → IC=+0.196 (n=1635)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.140 (n=3049)

- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.160 (n=2552)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0092 (IC base=+0.152)

- **PATRÓN** `sigma_h` > `0.0056` → IC=+0.152 (n=2592)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0056 (IC base=+0.152)

- **PATRÓN** `drift_60min` |x|≤ `0.5227` → IC=+0.162 (n=2899)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.5227 (IC base=+0.152)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.162 (n=967)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9415` → IC=+0.212 (n=967)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9415 (IC base=+0.152)

- **PATRÓN** `dist_vwap_pct` > `0.1877` → IC=+0.164 (n=1071)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.1877 (IC base=+0.152)

- **PATRÓN** `dist_vwap_pct` < `0.4929` → IC=+0.146 (n=1774)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.4929 (IC base=+0.152)
...
- **PATRÓN** `volumen_regimen` > `0.8972` → IC=+0.161 (n=1259)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.8972 (IC base=+0.152)

- **PATRÓN** `volumen_pendiente_norm` > `0.1712` → IC=+0.185 (n=795)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1712 (IC base=+0.152)

- **PATRÓN** `volumen_spike_ratio` < `1.4575` → IC=+0.161 (n=955)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 1.4575 (IC base=+0.152)

- **PATRÓN** `volumen_spike_ratio` > `1.8835` → IC=+0.164 (n=1909)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.8835 (IC base=+0.152)

- **PATRÓN** `libro_liquidez` > `8297.6638` → IC=+0.157 (n=1315)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 8297.6638 (IC base=+0.152)

- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.196 (n=768)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0038 (IC base=+0.138)

- **PATRÓN** `drift_60min` |x|≤ `0.49` → IC=+0.156 (n=2299)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.49 (IC base=+0.138)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.173 (n=841)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.1827` → IC=+0.170 (n=1013)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` < 0.1827 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` > `0.6915` → IC=+0.155 (n=439)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.6915 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.281` → IC=+0.148 (n=2288)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` < 6.281 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `1.1012` → IC=+0.149 (n=1933)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 1.1012 (IC base=+0.138)

- **PATRÓN** `volumen_pendiente_norm` < `0.0969` → IC=+0.141 (n=2081)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4242` → IC=+0.151 (n=757)
  ...
  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.4242 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` > `1.811` → IC=+0.142 (n=1513)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 1.811 (IC base=+0.138)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.140 (n=3049)

  - _Acción_: Kelly boost +0.70€ cuando `libro_spread` < 0.01 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `7202.6889` → IC=+0.152 (n=2053)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.159 (n=810)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0073 (IC base=+0.156)

- **PATRÓN** `sigma_h` > `0.0044` → IC=+0.166 (n=919)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` > 0.0044 (IC base=+0.156)

- **PATRÓN** `drift_60min` |x|≤ `0.3886` → IC=+0.158 (n=809)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.3886 (IC base=+0.156)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.168 (n=645)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5618` → IC=+0.160 (n=613)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` < 0.5618 (IC base=+0.156)

- **PATRÓN** `ibs_20min` > `0.8888` → IC=+0.170 (n=307)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` > 0.8888 (IC base=+0.156)

- **PATRÓN** `dist_vwap_pct` > `0.9603` → IC=+0.162 (n=205)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.9603 (IC base=+0.156)

- **PATRÓN** `dist_vwap_pct` < `0.4266` → IC=+0.166 (n=859)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.4266 (IC base=+0.156)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.662` → IC=+0.165 (n=921)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` < 6.662 (IC base=+0.156)

- **PATRÓN** `volumen_regimen` < `1.0854` → IC=+0.158 (n=809)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.0854 (IC base=+0.156)

- **PATRÓN** `volumen_regimen` > `0.7182` → IC=+0.160 (n=821)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.7182 (IC base=+0.156)

- **PATRÓN** `volumen_pendiente_norm` < `0.1087` → IC=+0.157 (n=847)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_pendiente_norm` < 0.1087 (IC base=+0.156)

- **PATRÓN** `volumen_pendiente_norm` > `0.0795` → IC=+0.165 (n=398)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.0795 (IC base=+0.156)

- **PATRÓN** `volumen_spike_ratio` < `1.4406` → IC=+0.173 (n=301)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.4406 (IC base=+0.156)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.162 (n=909)

  - _Acción_: Kelly boost +0.81€ cuando `libro_spread` < 0.01 (IC base=+0.156)

- **PATRÓN** `sigma_h` < `0.0071` → IC=+0.158 (n=685)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0071 (IC base=+0.140)

- **PATRÓN** `drift_60min` |x|≤ `0.5066` → IC=+0.166 (n=777)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.7466` → IC=+0.144 (n=778)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` < 0.7466 (IC base=+0.140)

- **PATRÓN** `ibs_20min` > `0.0998` → IC=+0.148 (n=777)

  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` > 0.0998 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` > `0.6442` → IC=+0.169 (n=167)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` > 0.6442 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.387` → IC=+0.148 (n=705)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` < 4.387 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `0.6434` → IC=+0.178 (n=259)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` < 0.6434 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` > `0.0738` → IC=+0.162 (n=329)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` > 0.0738 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` < `2.1956` → IC=+0.156 (n=670)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.1956 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.7791` → IC=+0.147 (n=508)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.7791 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `7573.3328` → IC=+0.166 (n=777)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 7573.3328 (IC base=+0.140)

### GBM_LATE_5M#SOL#5min
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.234 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.086)

- **PATRÓN** `dist_vwap_pct` > `0.1887` → IC=+0.127 (n=156)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` > 0.1887 (IC base=+0.086)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.218` → IC=+0.173 (n=47)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 9.218 (IC base=+0.086)

- **PATRÓN** `volumen_pendiente_norm` > `0.1583` → IC=+0.208 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1583 (IC base=+0.086)

- **PATRÓN** `libro_liquidez` > `3427.7149` → IC=+0.139 (n=203)

  - _Acción_: Kelly boost +0.70€ cuando `libro_liquidez` > 3427.7149 (IC base=+0.086)
...
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.193 (n=405)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0041 (IC base=+0.095)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.128 (n=857)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1549` → IC=+0.156 (n=452)
...
- **PATRÓN** `sigma_h` < `0.006` → IC=+0.161 (n=314)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.006 (IC base=+0.105)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.122 (n=329)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 6.0 (IC base=+0.105)

- **PATRÓN** `ibs_20min` > `0.52` → IC=+0.184 (n=283)

  - _Acción_: Kelly boost +0.92€ cuando `ibs_20min` > 0.52 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` > `0.1379` → IC=+0.175 (n=149)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.1379 (IC base=+0.105)

- **PATRÓN** `volumen_regimen` < `1.067` → IC=+0.130 (n=249)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 1.067 (IC base=+0.105)

- **PATRÓN** `volumen_pendiente_norm` < `0.0677` → IC=+0.135 (n=217)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_pendiente_norm` < 0.0677 (IC base=+0.105)

- **PATRÓN** `volumen_spike_ratio` < `2.0118` → IC=+0.173 (n=215)
...
- **PATRÓN** `libro_liquidez` > `2919.5028` → IC=+0.175 (n=235)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 2919.5028 (IC base=+0.105)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.028 (n=1777)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.167 (n=16)
  ...
  - _Acción_: SKIP cuando `liq_imbalance_15min` |x|≤ 1.0
  - _Potencial_: sin este filtro IC_bueno=+0.043 (n=784)
...
- **FILTRO** `py_entrada` < `0.51` → IC=-0.130 (n=411)

  - _Acción_: SKIP cuando `py_entrada` < 0.51
  - _Potencial_: sin este filtro IC_bueno=+0.010 (n=145)

- **FILTRO** `restante_min` < `3.46` → IC=-0.138 (n=139)

  - _Acción_: SKIP cuando `restante_min` < 3.46
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=417)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.121 (n=151)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.070 (n=410)
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.192 (n=24)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 16.0 (IC base=+0.047)

### LIQUIDACIONES_DEPTH_FASE0#BTC#5min
- **PATRÓN** `py_entrada` < `0.59` → IC=+0.221 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.59 (IC base=+0.111)

- **PATRÓN** `restante_min` > `3.82` → IC=+0.146 (n=46)

  - _Acción_: Kelly boost +0.73€ cuando `restante_min` > 3.82 (IC base=+0.111)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.208 (n=22)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.111)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.186 (n=33)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` < 12.0 (IC base=+0.111)

- **PATRÓN** `lag_apertura_s` < `60.77` → IC=+0.154 (n=24)

  - _Acción_: Kelly boost +0.77€ cuando `lag_apertura_s` < 60.77 (IC base=+0.111)

- **PATRÓN** `profundidad_ratio` > `79.6` → IC=+0.221 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `profundidad_ratio` > 79.6 (IC base=+0.111)
...
- **FILTRO** `hora_utc` < `12.0` → IC=-0.224 (n=27)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=30)
...
- **FILTRO** `lag_apertura_s` > `90.96` → IC=-0.156 (n=30)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.211 (n=43)
...
- **PATRÓN** `py_entrada` < `0.59` → IC=+0.128 (n=41)

  - _Acción_: Kelly boost +0.64€ cuando `py_entrada` < 0.59 (IC base=+0.032)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=7515)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.165 (n=3524)

  - _Acción_: SKIP cuando `py_entrada` < 0.475
  - _Potencial_: sin este filtro IC_bueno=+0.058 (n=10620)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.167 (n=3640)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.036 (n=10990)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.46` → IC=-0.210 (n=604)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.100 (n=1853)

- **FILTRO** `py_entrada` > `0.64` → IC=-0.153 (n=644)

  - _Acción_: SKIP cuando `py_entrada` > 0.64
  - _Potencial_: sin este filtro IC_bueno=+0.064 (n=1977)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.47` → IC=-0.191 (n=616)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.201 (n=637)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.063 (n=1981)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.485` → IC=-0.173 (n=603)

  - _Acción_: SKIP cuando `py_entrada` < 0.485
  - _Potencial_: sin este filtro IC_bueno=+0.085 (n=1839)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.177 (n=638)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=1984)

### MOMENTUM_IBS_15M_FADE
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=2771)

- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)

  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=2943)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.129 (n=9940)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=22430)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.273 (n=7925)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=24445)

- **FILTRO** `ibs_7min` < `0.2857` → IC=-0.235 (n=8088)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2857
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=24282)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.157 (n=10995)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=21375)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.230 (n=9954)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=30659)

- **FILTRO** `ibs_7min` > `0.2923` → IC=-0.178 (n=10148)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2923
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=30465)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.135 (n=1628)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3731)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.309 (n=1279)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=4080)

- **FILTRO** `ibs_7min` < `0.7117` → IC=-0.248 (n=1768)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7117
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=3591)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.186 (n=1226)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=4133)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.260 (n=1717)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=5256)

- **FILTRO** `drift_7min_pct` |x|> `0.1124` → IC=-0.126 (n=2370)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1124
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=4603)

- **FILTRO** `ibs_7min` > `0.7882` → IC=-0.206 (n=1743)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7882
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=5230)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=1299)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.087 (n=4285)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.251 (n=1352)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=4232)

- **FILTRO** `ibs_7min` < `0.7471` → IC=-0.191 (n=1395)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7471
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=4189)

- **FILTRO** `ballena_activa_n` > `162.0` → IC=-0.178 (n=1387)

  - _Acción_: SKIP cuando `ballena_activa_n` > 162.0
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=4197)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.253 (n=1416)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=4263)

- **FILTRO** `ibs_7min` > `0.2604` → IC=-0.177 (n=1419)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2604
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=4260)

- **FILTRO** `ballena_activa_n` > `154.0` → IC=-0.182 (n=1415)

  - _Acción_: SKIP cuando `ballena_activa_n` > 154.0
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=4264)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.165 (n=1240)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=3845)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.306 (n=1253)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=3832)

- **FILTRO** `ibs_7min` < `0.1935` → IC=-0.258 (n=1267)

  - _Acción_: SKIP cuando `ibs_7min` < 0.1935
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=3818)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.220 (n=1177)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3908)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.237 (n=1703)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=5760)

- **FILTRO** `ibs_7min` > `0.7456` → IC=-0.174 (n=1864)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7456
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=5599)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=1705)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=3634)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.232 (n=1580)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=3759)

- **FILTRO** `ibs_7min` < `0.7407` → IC=-0.180 (n=1334)
...
- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.170 (n=1312)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=4027)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.265 (n=1206)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=4246)

- **FILTRO** `ibs_7min` > `0.275` → IC=-0.177 (n=1362)

  - _Acción_: SKIP cuando `ibs_7min` > 0.275
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=4090)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.184 (n=1348)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=4104)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.255 (n=1376)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=4265)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.238 (n=1408)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=4233)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.178 (n=1837)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=5894)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.255 (n=1731)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3631)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.228 (n=1327)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=4035)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.213 (n=1306)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=4056)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.206 (n=1747)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.010 (n=5568)
...
### ORDER_FLOW_5M
- **PATRÓN** `delta_ratio` |x|> `0.398` → IC=+0.127 (n=743)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.63€ cuando `delta_ratio` |x|> 0.398 (IC base=+0.111)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.120 (n=670)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` > 6.0 (IC base=+0.111)

- **PATRÓN** `total_vol_5m` < `471.727` → IC=+0.136 (n=248)

  - _Acción_: Kelly boost +0.68€ cuando `total_vol_5m` < 471.727 (IC base=+0.111)
...
- **PATRÓN** `delta_ratio` |x|> `0.3989` → IC=+0.172 (n=132)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.86€ cuando `delta_ratio` |x|> 0.3989 (IC base=+0.133)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.196 (n=90)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` < 11.0 (IC base=+0.133)

- **PATRÓN** `total_vol_5m` < `8082.459` → IC=+0.157 (n=132)

  - _Acción_: Kelly boost +0.78€ cuando `total_vol_5m` < 8082.459 (IC base=+0.133)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.121 (n=27)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.032 (n=756)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=2913)

### STRUCT_NO_15M#SOL#15min
- **FILTRO** `py_entrada` < `0.495` → IC=-0.147 (n=32)

  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=1478)
...
### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.218 (n=726)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0086 (IC base=+0.185)

- **PATRÓN** `drift_60min` |x|≤ `0.0509` → IC=+0.203 (n=534)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0509 (IC base=+0.185)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2136` → IC=+0.195 (n=536)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.98€ cuando `delta_ratio_macro` |x|> 0.2136 (IC base=+0.185)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1243` → IC=+0.234 (n=561)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1243 (IC base=+0.185)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.196 (n=1499)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 6.0 (IC base=+0.185)

- **PATRÓN** `ibs_15` > `0.6061` → IC=+0.265 (n=1601)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6061 (IC base=+0.185)

- **PATRÓN** `dist_vwap_pct` > `0.3037` → IC=+0.190 (n=546)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.3037 (IC base=+0.185)

- **PATRÓN** `dist_vwap_pct` < `0.6222` → IC=+0.178 (n=1532)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.6222 (IC base=+0.185)

- **PATRÓN** `sigma_ewma_delta_pct` > `16.758` → IC=+0.280 (n=416)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 16.758 (IC base=+0.185)

- **PATRÓN** `libro_liquidez` > `2972.5116` → IC=+0.195 (n=1067)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 2972.5116 (IC base=+0.185)

### UPDOWN_GBM#60min
- **FILTRO** `libro_spread` > `0.01` → IC=-0.222 (n=16)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.007 (n=611)
...
- **PATRÓN** `sigma_h` < `0.0042` → IC=+0.151 (n=164)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0042 (IC base=+0.135)

- **PATRÓN** `sigma_h` > `0.0051` → IC=+0.156 (n=248)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` > 0.0051 (IC base=+0.135)

- **PATRÓN** `drift_60min` |x|≤ `0.0674` → IC=+0.151 (n=164)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.0674 (IC base=+0.135)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2337` → IC=+0.175 (n=124)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.87€ cuando `delta_ratio_macro` |x|> 0.2337 (IC base=+0.135)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.258` → IC=+0.159 (n=265)

  - _Acción_: Kelly boost +0.80€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.258 (IC base=+0.135)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.158 (n=276)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 11.0 (IC base=+0.135)

- **PATRÓN** `ibs_15` > `0.6602` → IC=+0.252 (n=333)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6602 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` < `0.1631` → IC=+0.152 (n=294)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.1631 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.918` → IC=+0.203 (n=163)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.918 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `9553.1781` → IC=+0.155 (n=169)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 9553.1781 (IC base=+0.135)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1498` → IC=+0.204 (n=167)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1498 (IC base=+0.171)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0701` → IC=+0.198 (n=170)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.99€ cuando `delta_ratio_macro` |x|> 0.0701 (IC base=+0.171)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2673` → IC=+0.227 (n=126)
...
- **PATRÓN** `ibs_15` > `0.6111` → IC=+0.260 (n=190)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6111 (IC base=+0.171)

- **PATRÓN** `dist_vwap_pct` > `0.122` → IC=+0.196 (n=110)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.122 (IC base=+0.171)
...
- **PATRÓN** `ballena_activa_n` < `34.0` → IC=+0.221 (n=102)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.360 (n=413)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **FILTRO** `sigma_h` > `0.013` → IC=-0.218 (n=657)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.013
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=1973)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.192 (n=894)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.006 (n=1736)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1366` → IC=+0.251 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1366 (IC base=-0.061)

- **PATRÓN** `ibs_15` > `0.6364` → IC=+0.275 (n=628)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1201` → IC=+0.247 (n=1167)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1201 (IC base=-0.033)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1771` → IC=+0.234 (n=1131)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1771 (IC base=-0.033)

- **PATRÓN** `ibs_15` < `0.3462` → IC=+0.278 (n=1751)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3462 (IC base=-0.033)

- **PATRÓN** `dist_vwap_pct` > `0.6957` → IC=+0.299 (n=286)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6957 (IC base=-0.033)
...
- **FILTRO** `sigma_h` < `0.0034` → IC=-0.231 (n=400)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.187 (n=1202)

- **FILTRO** `hora_utc` > `7.0` → IC=-0.209 (n=1017)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 7.0
...
- **FILTRO** `sigma_ewma_delta_pct` > `19.796` → IC=-0.249 (n=289)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.187 (n=1313)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.122 (n=310)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.239 (n=688)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0076 (IC base=+0.228)

- **PATRÓN** `drift_60min` |x|≤ `0.4455` → IC=+0.229 (n=688)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4455 (IC base=+0.228)

- **PATRÓN** `drift_15min` |x|≤ `0.7897` → IC=+0.232 (n=605)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.7897 (IC base=+0.228)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2029` → IC=+0.261 (n=312)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.2029 (IC base=+0.228)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.235 (n=270)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` < `0.3469` → IC=+0.267 (n=688)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3469 (IC base=+0.228)

- **PATRÓN** `dist_vwap_pct` > `0.778` → IC=+0.308 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.778 (IC base=+0.228)

- **PATRÓN** `sigma_ewma_delta_pct` > `16.687` → IC=+0.252 (n=119)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 16.687 (IC base=+0.228)

- **PATRÓN** `sigma_ewma_delta_pct` < `12.189` → IC=+0.235 (n=734)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 12.189 (IC base=+0.228)

### UPDOWN_GBM_15M_TARDIO#SOL#15min
- **FILTRO** `sigma_h` > `0.0076` → IC=-0.221 (n=310)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0076
  - _Potencial_: sin este filtro IC_bueno=-0.115 (n=312)

- **FILTRO** `drift_15min` |x|> `0.888` → IC=-0.264 (n=155)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 0.888
  - _Potencial_: sin este filtro IC_bueno=-0.135 (n=467)
...
### UPDOWN_GBM_IBS_ALTO
- **PATRÓN** `sigma_h` < `0.0045` → IC=+0.298 (n=439)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1073` → IC=+0.348 (n=182)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1073 (IC base=+0.294)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.313 (n=688)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1578` → IC=+0.321 (n=384)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1578 (IC base=+0.294)

- **PATRÓN** `sigma_ewma_delta_pct` > `23.1` → IC=+0.331 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 23.1 (IC base=+0.294)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.297 (n=805)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.294)

- **PATRÓN** `libro_liquidez` > `14636.8174` → IC=+0.310 (n=219)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 14636.8174 (IC base=+0.294)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6061 sube el IC de +0.185 a +0.265 en UPDOWN_GBM#15min (n=1601). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.7061 sube el IC de +0.208 a +0.273 en UPDOWN_GBM#BTC#15min (n=363). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.6602 sube el IC de +0.135 a +0.252 en UPDOWN_GBM#ETH#15min (n=333). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#SOL#15min**: dentro de BUY_YES, IBS > 0.6111 sube el IC de +0.171 a +0.260 en UPDOWN_GBM#SOL#15min (n=190). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.6364 sube el IC de -0.061 a +0.275 en UPDOWN_GBM_15M_TARDIO (n=628). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3462 sube el IC de -0.033 a +0.278 en UPDOWN_GBM_15M_TARDIO (n=1751). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.3469 sube el IC de +0.228 a +0.267 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=688). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO#BTC#15min**: dentro de BUY_YES, IBS > 0.83 sube el IC de +0.287 a +0.315 en UPDOWN_GBM_IBS_ALTO#BTC#15min (n=365). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 1327 | +0.098 | +195.01€ | 1 | 9 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 1327 | +0.098 | +195.01€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 26 | +0.036 | -1.50€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 26 | +0.036 | -1.50€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 988 | +0.107 | +166.88€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 988 | +0.107 | +166.88€ | 2 | 8 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 253 | +0.057 | +9.47€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 253 | +0.057 | +9.47€ | 6 | 6 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 60 | +0.145 | +20.16€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 60 | +0.145 | +20.16€ | 0 | 7 |
| ✅ BALLENAS_TARDIAS | 28234 | -0.091 | -3807.12€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1456 | -0.049 | -215.76€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 26778 | -0.093 | -3591.36€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3695 | -0.092 | -590.17€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3695 | -0.092 | -590.17€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 1456 | -0.049 | -215.76€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 1456 | -0.049 | -215.76€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 3479 | -0.096 | -791.31€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 3479 | -0.096 | -791.31€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 7257 | -0.023 | -676.32€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 7257 | -0.023 | -676.32€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 6794 | -0.097 | -453.47€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 6794 | -0.097 | -453.47€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 5553 | -0.180 | -1080.10€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 5553 | -0.180 | -1080.10€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 18163 | -0.028 | +4039.05€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 4745 | +0.000 | +1825.30€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 13418 | -0.038 | +2213.74€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 18163 | -0.028 | +4039.05€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 4745 | +0.000 | +1825.30€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 13418 | -0.038 | +2213.74€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 92823 | +0.112 | -4736.70€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 14019 | +0.185 | -445.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 373 | -0.084 | -51.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 72390 | +0.100 | -4021.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 6041 | +0.107 | -218.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 12039 | +0.098 | -1018.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 45 | -0.160 | -0.32€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 11979 | +0.100 | -1006.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 18775 | +0.131 | -382.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 4414 | +0.203 | -140.94€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 12003 | +0.111 | -195.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 2316 | +0.104 | -24.06€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 12079 | +0.089 | -1123.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 52 | -0.093 | -6.98€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#DOGE#240min | 15 | -0.243 | -11.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 12012 | +0.090 | -1105.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 19754 | +0.123 | -404.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 5426 | +0.175 | -87.34€ | 1 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 12131 | +0.105 | -254.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 2185 | +0.098 | -54.66€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 18122 | +0.113 | -1090.90€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 4036 | +0.188 | -218.99€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 276 | -0.043 | +2.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 12270 | +0.090 | -734.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1540 | +0.125 | -139.96€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 12054 | +0.100 | -716.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 46 | -0.021 | +9.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 13 | -0.022 | -0.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 11995 | +0.101 | -725.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 14727 | +0.191 | -973.06€ | 1 | 5 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 14727 | +0.191 | -973.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 3510 | +0.167 | -370.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 3510 | +0.167 | -370.92€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC | 1222 | +0.195 | -11.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC#15min | 1222 | +0.195 | -11.87€ | 4 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 3454 | +0.180 | -297.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 3454 | +0.180 | -297.95€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 3072 | +0.239 | -97.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 3072 | +0.239 | -97.55€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 3390 | +0.192 | -208.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 3390 | +0.192 | -208.52€ | 0 | 4 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 50702 | +0.197 | -4007.26€ | 3 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 50702 | +0.197 | -4007.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 8776 | +0.176 | -1020.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 8776 | +0.176 | -1020.33€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 8095 | +0.222 | -311.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 8095 | +0.222 | -311.55€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 8753 | +0.172 | -1058.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 8753 | +0.172 | -1058.15€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 8196 | +0.218 | -321.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 8196 | +0.218 | -321.93€ | 1 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 8376 | +0.203 | -558.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 8376 | +0.203 | -558.30€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 8506 | +0.194 | -737.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 8506 | +0.194 | -737.00€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 19120 | +0.118 | +174.51€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 19120 | +0.118 | +174.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 9492 | +0.123 | +157.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 9492 | +0.123 | +157.42€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 9628 | +0.113 | +17.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 9628 | +0.113 | +17.09€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION | 1469 | +0.291 | -10.82€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#60min | 1469 | +0.291 | -10.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#BTC | 654 | +0.277 | -17.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#BTC#60min | 654 | +0.277 | -17.55€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#ETH | 705 | +0.295 | +5.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#ETH#60min | 705 | +0.295 | +5.31€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 35456 | +0.097 | -1125.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 2933 | +0.089 | +19.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 32523 | +0.097 | -1145.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC | 19916 | +0.101 | -335.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#15min | 2933 | +0.089 | +19.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#5min | 16983 | +0.103 | -354.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH | 6657 | +0.106 | -49.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH#5min | 6657 | +0.106 | -49.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL | 8883 | +0.079 | -741.28€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL#5min | 8883 | +0.079 | -741.28€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 25456 | +0.082 | +12060.08€ | 0 | 17 |
| ✅ GBM_LATE_15M#15min | 25456 | +0.082 | +12060.08€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 4256 | +0.193 | +3090.18€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 4256 | +0.193 | +3090.18€ | 0 | 20 |
| ✅ GBM_LATE_15M#BTC | 3777 | +0.177 | +2628.20€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 3777 | +0.177 | +2628.20€ | 0 | 27 |
| ✅ GBM_LATE_15M#DOGE | 4426 | +0.198 | +3293.67€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 4426 | +0.198 | +3293.67€ | 0 | 20 |
| ✅ GBM_LATE_15M#ETH | 3751 | +0.020 | +799.01€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 3751 | +0.020 | +799.01€ | 1 | 14 |
| ✅ GBM_LATE_15M#SOL | 3649 | -0.033 | +828.04€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 3649 | -0.033 | +828.04€ | 4 | 14 |
| ✅ GBM_LATE_15M#XRP | 5597 | -0.041 | +1421.00€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 5597 | -0.041 | +1421.00€ | 4 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 26884 | +0.086 | +14024.77€ | 0 | 19 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 26884 | +0.086 | +14024.77€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 5098 | +0.017 | +2684.47€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 5098 | +0.017 | +2684.47€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 5602 | +0.015 | +1168.05€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 5602 | +0.015 | +1168.05€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 3820 | +0.261 | +3833.00€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 3820 | +0.261 | +3833.00€ | 0 | 20 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 4364 | +0.003 | +839.71€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 4364 | +0.003 | +839.71€ | 2 | 15 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 4369 | +0.028 | +1649.52€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 4369 | +0.028 | +1649.52€ | 3 | 17 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 3631 | +0.274 | +3850.03€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 3631 | +0.274 | +3850.03€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 20513 | +0.167 | +15284.35€ | 0 | 26 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 20513 | +0.167 | +15284.35€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB | 3090 | +0.206 | +2449.21€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB#15min | 3090 | +0.206 | +2449.21€ | 0 | 21 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC | 3263 | +0.149 | +2394.96€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC#15min | 3263 | +0.149 | +2394.96€ | 0 | 22 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE | 3218 | +0.206 | +2537.32€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min | 3218 | +0.206 | +2537.32€ | 0 | 20 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH | 3448 | +0.133 | +2405.31€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH#15min | 3448 | +0.133 | +2405.31€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL | 3812 | +0.116 | +2606.27€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL#15min | 3812 | +0.116 | +2606.27€ | 0 | 24 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP | 3682 | +0.203 | +2891.27€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP#15min | 3682 | +0.203 | +2891.27€ | 0 | 26 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 5127 | +0.127 | +2145.68€ | 0 | 22 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 5127 | +0.127 | +2145.68€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BNB | 189 | +0.123 | +84.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BNB#15min | 189 | +0.123 | +84.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BTC | 1469 | +0.122 | +657.00€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BTC#15min | 1469 | +0.122 | +657.00€ | 0 | 18 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#DOGE | 373 | +0.143 | +176.78€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#DOGE#15min | 373 | +0.143 | +176.78€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH | 1509 | +0.142 | +661.78€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH#15min | 1509 | +0.142 | +661.78€ | 0 | 20 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#SOL | 1083 | +0.103 | +349.04€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#SOL#15min | 1083 | +0.103 | +349.04€ | 1 | 16 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 504 | +0.134 | +216.54€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 504 | +0.134 | +216.54€ | 0 | 27 |
| ✅ GBM_LATE_15M_TARDIO | 25574 | +0.175 | +19027.63€ | 0 | 22 |
| ✅ GBM_LATE_15M_TARDIO#15min | 25574 | +0.175 | +19027.63€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 4056 | +0.220 | +3414.70€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 4056 | +0.220 | +3414.70€ | 0 | 20 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 4006 | +0.151 | +2661.38€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 4006 | +0.151 | +2661.38€ | 0 | 26 |
| ✅ GBM_LATE_15M_TARDIO#DOGE | 4197 | +0.225 | +3601.93€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#DOGE#15min | 4197 | +0.225 | +3601.93€ | 0 | 20 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 4144 | +0.138 | +2846.26€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 4144 | +0.138 | +2846.26€ | 0 | 25 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 4480 | +0.112 | +2828.73€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 4480 | +0.112 | +2828.73€ | 0 | 20 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 4691 | +0.204 | +3674.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 4691 | +0.204 | +3674.64€ | 0 | 24 |
| ✅ GBM_LATE_5M | 6929 | +0.146 | +3921.31€ | 1 | 28 |
| ✅ GBM_LATE_5M#5min | 6929 | +0.146 | +3921.31€ | 0 | 0 |
...
| ✅ GBM_LATE_5M#ETH | 2260 | +0.149 | +1264.89€ | 0 | 0 |
| ✅ GBM_LATE_5M#ETH#5min | 2260 | +0.149 | +1264.89€ | 0 | 29 |
| ✅ GBM_LATE_5M#SOL | 593 | +0.100 | +203.20€ | 0 | 0 |
| ✅ GBM_LATE_5M#SOL#5min | 593 | +0.100 | +203.20€ | 0 | 16 |
| ✅ GBM_LATE_5M#XRP | 820 | +0.116 | +318.37€ | 0 | 0 |
| ✅ GBM_LATE_5M#XRP#5min | 820 | +0.116 | +318.37€ | 0 | 0 |
| ✅ GBM_LATE_60M | 1711 | +0.069 | +734.89€ | 2 | 12 |
| ✅ GBM_LATE_60M#60min | 1711 | +0.069 | +734.89€ | 0 | 0 |
| ✅ GBM_LATE_60M#BTC | 616 | +0.087 | +256.39€ | 0 | 0 |
| ✅ GBM_LATE_60M#BTC#60min | 616 | +0.087 | +256.39€ | 0 | 11 |
...
| ✅ LEADLAG_BTC_XRP_15M | 1930 | +0.099 | +520.66€ | 0 | 1 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 1930 | +0.099 | +520.66€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 1930 | +0.099 | +520.66€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 1930 | +0.099 | +520.66€ | 0 | 1 |
...
| ✅ LIQUIDACIONES_5M | 1975 | +0.007 | +16.46€ | 4 | 0 |
| ✅ LIQUIDACIONES_5M#5min | 1975 | +0.007 | +16.46€ | 0 | 0 |
...
| ✅ LIQUIDACIONES_5M#ETH | 831 | +0.028 | +24.67€ | 0 | 0 |
| ✅ LIQUIDACIONES_5M#ETH#5min | 831 | +0.028 | +24.67€ | 6 | 0 |
...
| ✅ LIQUIDACIONES_DEPTH_FASE0 | 1117 | -0.037 | -21.21€ | 3 | 1 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#15min | 521 | -0.047 | -23.62€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#5min | 596 | -0.028 | +2.41€ | 0 | 0 |
...
| ✅ LIQUIDACIONES_DEPTH_FASE0#BTC | 279 | +0.034 | +39.23€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#BTC#15min | 126 | +0.000 | +6.91€ | 0 | 1 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#BTC#5min | 153 | +0.061 | +32.31€ | 0 | 6 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#DOGE | 162 | -0.073 | -15.46€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#DOGE#15min | 77 | -0.057 | -6.04€ | 1 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#DOGE#5min | 85 | -0.086 | -9.42€ | 2 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#ETH | 205 | -0.085 | -25.73€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#ETH#15min | 92 | -0.096 | -12.47€ | 5 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#ETH#5min | 113 | -0.074 | -13.26€ | 5 | 2 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#SOL | 176 | -0.039 | -5.17€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#SOL#15min | 90 | -0.043 | -3.04€ | 2 | 2 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#SOL#5min | 86 | -0.034 | -2.13€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#XRP | 262 | -0.053 | -16.33€ | 0 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#XRP#15min | 117 | -0.063 | -10.62€ | 1 | 0 |
| ✅ LIQUIDACIONES_DEPTH_FASE0#XRP#5min | 145 | -0.044 | -5.71€ | 5 | 1 |
| ✅ MOMENTUM_IBS_15M | 14379 | -0.011 | -205.18€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M#15min | 14379 | -0.011 | -205.18€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB | 578 | -0.010 | -0.50€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB#15min | 578 | -0.010 | -0.50€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BTC | 3096 | -0.020 | -58.36€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BTC#15min | 3096 | -0.020 | -58.36€ | 2 | 0 |
...
| ✅ MOMENTUM_IBS_15M_BALLENA | 28774 | -0.006 | +1325.03€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#15min | 28774 | -0.006 | +1325.03€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB | 5078 | +0.017 | +617.62€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB#15min | 5078 | +0.017 | +617.62€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC | 4463 | -0.027 | -40.65€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC#15min | 4463 | -0.027 | -40.65€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE | 5116 | +0.015 | +470.41€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE#15min | 5116 | +0.015 | +470.41€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH | 4240 | -0.050 | -125.87€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH#15min | 4240 | -0.050 | -125.87€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL | 4813 | -0.010 | +185.02€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL#15min | 4813 | -0.010 | +185.02€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP | 5064 | +0.008 | +218.50€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP#15min | 5064 | +0.008 | +218.50€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE | 5815 | -0.057 | -129.29€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#15min | 5815 | -0.057 | -129.29€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_15M_FADE#SOL | 1678 | -0.077 | -31.74€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#SOL#15min | 1678 | -0.077 | -31.74€ | 2 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 72983 | -0.072 | +1737.48€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 72983 | -0.072 | +1737.48€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 12332 | -0.079 | +724.28€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 12332 | -0.079 | +724.28€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 11263 | -0.092 | -482.61€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 11263 | -0.092 | -482.61€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 12548 | -0.067 | +711.42€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 12548 | -0.067 | +711.42€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 10791 | -0.092 | -162.30€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 10791 | -0.092 | -162.30€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 13372 | -0.048 | +389.06€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 13372 | -0.048 | +389.06€ | 3 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 12677 | -0.063 | +557.62€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 12677 | -0.063 | +557.62€ | 4 | 0 |
...
| ✅ ORDER_FLOW_5M | 1125 | +0.104 | +363.20€ | 0 | 3 |
| ✅ ORDER_FLOW_5M#5min | 989 | +0.111 | +350.61€ | 0 | 0 |
...
| ✅ ORDER_FLOW_5M#SOL | 175 | +0.133 | +80.31€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#SOL#5min | 175 | +0.133 | +80.31€ | 0 | 3 |
...
| ✅ STREAK_FADE_5M | 2770 | -0.022 | -113.07€ | 0 | 0 |
| ✅ STREAK_FADE_5M#5min | 2770 | -0.022 | -113.07€ | 0 | 0 |
...
| ✅ STREAK_MOM_5M | 7761 | +0.024 | +118.92€ | 0 | 0 |
| ✅ STREAK_MOM_5M#5min | 7761 | +0.024 | +118.92€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE | 2140 | +0.021 | +21.37€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE#5min | 2140 | +0.021 | +21.37€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH | 1752 | +0.035 | +53.19€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH#5min | 1752 | +0.035 | +53.19€ | 1 | 0 |
| ✅ STREAK_MOM_5M#SOL | 2349 | +0.013 | +5.87€ | 0 | 0 |
| ✅ STREAK_MOM_5M#SOL#5min | 2349 | +0.013 | +5.87€ | 1 | 0 |
| ✅ STREAK_MOM_5M#XRP | 1520 | +0.030 | +38.49€ | 0 | 0 |
| ✅ STREAK_MOM_5M#XRP#5min | 1520 | +0.030 | +38.49€ | 2 | 0 |
| ✅ STRUCT_NO_15M | 7293 | +0.012 | -40.97€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 7293 | +0.012 | -40.97€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 2932 | +0.016 | -6.69€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 2932 | +0.016 | -6.69€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 2851 | +0.012 | -17.22€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 2851 | +0.012 | -17.22€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 1510 | +0.004 | -17.05€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 1510 | +0.004 | -17.05€ | 2 | 0 |
| ✅ UPDOWN_GBM | 37055 | +0.030 | +2168.19€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 10037 | +0.067 | +1803.44€ | 0 | 10 |
| ✅ UPDOWN_GBM#240min | 1377 | +0.005 | +7.53€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 23237 | +0.018 | +337.58€ | 0 | 0 |
| ✅ UPDOWN_GBM#60min | 2261 | +0.005 | +20.55€ | 1 | 0 |
| ✅ UPDOWN_GBM#BNB | 3812 | +0.066 | +408.80€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 670 | +0.150 | +265.30€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#240min | 32 | +0.000 | -0.19€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#5min | 3110 | +0.048 | +143.69€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 6848 | +0.035 | +472.50€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 1271 | +0.084 | +292.01€ | 0 | 9 |
| ✅ UPDOWN_GBM#BTC#240min | 372 | +0.019 | +7.64€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#5min | 4136 | +0.031 | +152.68€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#60min | 1014 | +0.003 | +19.63€ | 1 | 0 |
| ✅ UPDOWN_GBM#BTC#daily | 55 | -0.097 | +0.55€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 4348 | +0.038 | +251.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 627 | +0.136 | +214.96€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#240min | 28 | +0.000 | -1.43€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 3693 | +0.021 | +37.85€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 7855 | +0.016 | +268.51€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 2573 | +0.045 | +267.47€ | 0 | 10 |
| ✅ UPDOWN_GBM#ETH#240min | 360 | +0.005 | +6.96€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 4107 | +0.004 | -3.45€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 769 | +0.001 | -5.68€ | 2 | 0 |
| ✅ UPDOWN_GBM#ETH#daily | 46 | -0.146 | +3.20€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 8796 | +0.014 | +213.56€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 2439 | +0.028 | +170.76€ | 0 | 11 |
| ✅ UPDOWN_GBM#SOL#240min | 354 | -0.006 | -2.31€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 5485 | +0.011 | +41.34€ | 1 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 478 | +0.015 | +6.60€ | 0 | 1 |
| ✅ UPDOWN_GBM#SOL#daily | 40 | -0.167 | -2.82€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 5394 | +0.035 | +555.28€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 2457 | +0.081 | +592.95€ | 0 | 12 |
| ✅ UPDOWN_GBM#XRP#240min | 231 | -0.002 | -3.14€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 2706 | -0.003 | -34.54€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 11905 | -0.039 | +2510.49€ | 2 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 11905 | -0.039 | +2510.49€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 768 | -0.040 | +352.03€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 768 | -0.040 | +352.03€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 2201 | -0.121 | +25.56€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 2201 | -0.121 | +25.56€ | 4 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 387 | +0.179 | +256.75€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 387 | +0.179 | +256.75€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 1314 | +0.205 | +767.13€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 1314 | +0.205 | +767.13€ | 2 | 22 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 3610 | -0.063 | +548.86€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 3610 | -0.063 | +548.86€ | 2 | 5 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 3625 | -0.076 | +560.17€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 3625 | -0.076 | +560.17€ | 3 | 4 |
...
| ✅ UPDOWN_GBM_IBS_ALTO | 876 | +0.294 | +716.60€ | 0 | 11 |
| ✅ UPDOWN_GBM_IBS_ALTO#15min | 876 | +0.294 | +716.60€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#BTC | 486 | +0.287 | +373.95€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#BTC#15min | 486 | +0.287 | +373.95€ | 0 | 10 |
| ✅ UPDOWN_GBM_IBS_ALTO#ETH | 390 | +0.301 | +342.65€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#ETH#15min | 390 | +0.301 | +342.65€ | 0 | 10 |
| ✅ UPDOWN_OU_5M | 717 | -0.109 | -82.11€ | 3 | 0 |
| ✅ UPDOWN_OU_5M#5min | 717 | -0.109 | -82.11€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#BNB | 311 | -0.078 | -35.51€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#BNB#5min | 311 | -0.078 | -35.51€ | 2 | 0 |
| ✅ UPDOWN_OU_5M#BTC | 203 | -0.076 | -14.32€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#BTC#5min | 203 | -0.076 | -14.32€ | 2 | 0 |
...
**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  ...
  - _Estado_: 520 celda(s) pasan gate riguroso completo de 2235 evaluadas (n>=40) y 3219 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.028 < 0.08 — monitorear
  - _Datos_: n=2438 IC=+0.028 PNL=+170.23€
...
**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  ...
  - _Estado_: alineada_con_outcome_prev IC=+0.108 n=312/60 | contraria IC=+0.144 n=304 | gap=-0.035 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar

**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  - _Umbral_: n_overlaps≥20 y IC_overlap > IC_base + 0.05
  - _Acción_: Cambiar _aplicar_kelly_compuesto: match por activo, no market_id
  - _Estado_: n_overlaps=288, boost estimado=+0.000. Necesita 0 más y boost>0.05
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=769/40 IC=+0.001 PNL=-5.68€ | BTC#60min: n=1014/40 IC=+0.003 PNL=+19.63€ | SOL#60min: n=478/40 IC=+0.015 PNL=+6.60€

**⏳ H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  - _Umbral_: n≥40 tras 2 losses y gap(IC_tras_win - IC_tras_2loss)≥0.05
  - _Acción_: Reducir stake (no desactivar) 1-2h tras 2 derrotas consecutivas en el mismo subtype
  - _Estado_: tras_win IC=+0.050 n=334132 | tras_1loss IC=+0.078 n=260159 | tras_2loss IC=+0.047 n=109793/40 | gap=+0.003 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-60MIN-BUYYES** — GBM 60min BUY_YES — ¿edge superior al BUY_NO?
  ...
  - _Estado_: n=1596 IC=+0.013 PNL=+11.03€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1596 IC=+0.013 PNL=+11.03€

**〰️ H-CUSTOM-GBM-60MIN-BUYNO** — GBM 60min BUY_NO — tracking por separado
  - _Hipótesis_: En 15min BUY_NO tiene IC=+0.119. ¿Se repite en 60min? Datos actuales: 8/14 (57%) IC=+0.044 — positivo pero débil. Puede ser que 60min requiera dirección alcista (BUY_YES) y no bajista.
  - _Umbral_: n≥30 para confirmar dirección
  - _Acción_: Si IC<0.05 con n≥30 → en 60min priorizar solo BUY_YES; si IC>0.08 → igualar al BUY_YES
  - _Estado_: n=665 IC=-0.016 PNL=+9.52€ — sin señal clara aún (umbral IC: min=0.05 max=None)
  - _Datos_: n=665 IC=-0.016 PNL=+9.52€
...
**🟡 H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.185 > 0.1 con n=2132 PNL=+1321.96€
  - _Datos_: n=2132 IC=+0.185 PNL=+1321.96€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=1270 IC=+0.083 PNL=+291.48€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=1270 IC=+0.083 PNL=+291.48€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=12930 IC=+0.052 PNL=+1538.98€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=12930 IC=+0.052 PNL=+1538.98€
...
**🔴 H-CUSTOM-BUYNO-LONGSHOT-15MIN** — BUY_NO longshot en 15min (py_mkt≥0.55) — comprar NO barato pierde
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.157 < -0.1 con n=231 PNL=+23.10€
  - _Datos_: n=231 IC=-0.157 PNL=+23.10€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=1894 IC=+0.049 PNL=+200.25€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1894 IC=+0.049 PNL=+200.25€
...
**🟡 H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.131 > 0.1 con n=407 PNL=+124.35€
  - _Datos_: n=407 IC=+0.131 PNL=+124.35€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=17880 IC=-0.137 PNL=+1233.51€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=17880 IC=-0.137 PNL=+1233.51€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=1926 IC=+0.140 PNL=+1052.31€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=1926 IC=+0.140 PNL=+1052.31€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.238 < -0.1 con n=1741 PNL=-192.11€
  - _Datos_: n=1741 IC=-0.238 PNL=-192.11€
...
**🔶 H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.093 n=993) pero sin cruzar ≥2 pares más — sin otros pares con datos
  - _Datos_: n=993 IC=+0.093 PNL=+228.28€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=8775 IC=+0.176 PNL=-1020.73€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=8775 IC=+0.176 PNL=-1020.73€
