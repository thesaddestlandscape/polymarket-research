# Hipótesis automáticas — 2026-08-29 22:56 UTC
_Generado por shadow_postmortem.py sobre 207079 resoluciones (PNL=+15981.22€)_
...
- **FILTRO** `restante_s_al_confirmar` < `147.7` → IC=-0.291 (n=2893)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=8680)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.221 (n=708)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.212 (n=905)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.134 (n=1378)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.194 (n=5133)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.201 (n=2435)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.165 (n=1256)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.177 (n=1339)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.174 (n=1241)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.233 (n=798)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.197 (n=827)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.234 (n=893)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9504` → IC=+0.207 (n=892)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9504 (IC base=+0.070)

- **PATRÓN** `dist_vwap_pct` > `0.186` → IC=+0.223 (n=355)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.186 (IC base=+0.070)

- **PATRÓN** `dist_vwap_pct` < `0.4917` → IC=+0.210 (n=530)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4917 (IC base=+0.070)
...
- **PATRÓN** `volumen_regimen` > `1.0648` → IC=+0.240 (n=263)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0648 (IC base=+0.070)

- **PATRÓN** `volumen_pendiente_norm` > `0.2472` → IC=+0.163 (n=307)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.2472 (IC base=+0.070)

- **PATRÓN** `volumen_spike_ratio` < `2.4225` → IC=+0.157 (n=1322)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.4225 (IC base=+0.070)

- **PATRÓN** `volumen_spike_ratio` > `1.9486` → IC=+0.153 (n=1001)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.9486 (IC base=+0.070)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.286 (n=218)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.172 (n=419)

  - _Acción_: Kelly boost +0.86€ cuando `libro_spread` < 0.04 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `58.0` → IC=+0.157 (n=246)
...
- **PATRÓN** `sigma_h` < `0.0029` → IC=+0.245 (n=155)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0901` → IC=+0.244 (n=154)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0901 (IC base=+0.208)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.232 (n=471)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.3908` → IC=+0.227 (n=459)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3908 (IC base=+0.208)

- **PATRÓN** `dist_vwap_pct` > `0.212` → IC=+0.228 (n=288)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.212 (IC base=+0.208)

- **PATRÓN** `dist_vwap_pct` < `0.5452` → IC=+0.210 (n=412)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5452 (IC base=+0.208)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.354` → IC=+0.232 (n=237)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.354 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` < `0.6879` → IC=+0.216 (n=202)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6879 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` > `1.0681` → IC=+0.239 (n=209)
...
- **PATRÓN** `libro_liquidez` > `14039.8617` → IC=+0.217 (n=306)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 14039.8617 (IC base=+0.208)

- **PATRÓN** `ballena_activa_n` < `398.0` → IC=+0.207 (n=384)
...
- **FILTRO** `sigma_ewma_delta_pct` > `5.027` → IC=-0.165 (n=311)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=1135)
...
- **PATRÓN** `ibs_20min` > `0.2759` → IC=+0.121 (n=3390)
...
- **PATRÓN** `volumen_spike_ratio` > `2.85` → IC=+0.189 (n=522)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 2.85 (IC base=+0.056)

- **PATRÓN** `ballena_activa_n` < `94.0` → IC=+0.280 (n=1015)
...
- **FILTRO** `ibs_20min` < `0.2583` → IC=-0.151 (n=253)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.285 (n=263)
...
- **FILTRO** `ibs_20min` > `0.7391` → IC=-0.132 (n=362)
...
- **PATRÓN** `volumen_regimen` > `0.623` → IC=+0.173 (n=2000)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 0.623 (IC base=+0.161)

- **PATRÓN** `volumen_pendiente_norm` > `0.2403` → IC=+0.197 (n=556)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.2403 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` < `2.3122` → IC=+0.163 (n=2320)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.3122 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` > `1.4429` → IC=+0.155 (n=2637)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.4429 (IC base=+0.161)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.169 (n=2297)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.01 (IC base=+0.161)

- **PATRÓN** `libro_liquidez` > `3988.3638` → IC=+0.189 (n=965)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3988.3638 (IC base=+0.161)

- **PATRÓN** `ballena_activa_n` < `164.0` → IC=+0.180 (n=1803)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 164.0 (IC base=+0.161)

- **PATRÓN** `sigma_h` < `0.006` → IC=+0.195 (n=1753)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.197 (n=1240)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.414` → IC=+0.236 (n=2622)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4412` → IC=+0.154 (n=391)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.4412 (IC base=+0.159)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.189 (n=342)
...
- **PATRÓN** `drift_60min` |x|≤ `0.419` → IC=+0.195 (n=418)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.419 (IC base=+0.181)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.209 (n=427)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.181)

- **PATRÓN** `ibs_20min` > `0.4893` → IC=+0.221 (n=417)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4893 (IC base=+0.181)

- **PATRÓN** `dist_vwap_pct` > `0.2069` → IC=+0.236 (n=297)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2069 (IC base=+0.181)
...
- **PATRÓN** `volumen_regimen` < `1.2802` → IC=+0.183 (n=418)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 1.2802 (IC base=+0.181)

- **PATRÓN** `volumen_regimen` > `1.0613` → IC=+0.203 (n=190)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0613 (IC base=+0.181)

- **PATRÓN** `volumen_pendiente_norm` > `0.2241` → IC=+0.233 (n=84)
...
- **PATRÓN** `libro_liquidez` > `13155.0204` → IC=+0.211 (n=278)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13155.0204 (IC base=+0.181)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.543` → IC=+0.228 (n=1945)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.543 (IC base=+0.155)

- **PATRÓN** `volumen_regimen` < `0.6962` → IC=+0.157 (n=1043)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.6962 (IC base=+0.155)

- **PATRÓN** `volumen_regimen` > `1.073` → IC=+0.151 (n=1074)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.073 (IC base=+0.155)

- **PATRÓN** `volumen_pendiente_norm` > `0.165` → IC=+0.183 (n=881)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.165 (IC base=+0.155)

- **PATRÓN** `volumen_spike_ratio` < `2.3186` → IC=+0.156 (n=2681)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.3186 (IC base=+0.155)

- **PATRÓN** `volumen_spike_ratio` > `1.8738` → IC=+0.165 (n=2031)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.8738 (IC base=+0.155)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.161 (n=2679)

  - _Acción_: Kelly boost +0.80€ cuando `libro_spread` < 0.01 (IC base=+0.155)

- **PATRÓN** `libro_liquidez` > `3954.2434` → IC=+0.190 (n=1122)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 3954.2434 (IC base=+0.155)

- **PATRÓN** `ballena_activa_n` < `65.0` → IC=+0.207 (n=1326)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 65.0 (IC base=+0.155)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.210 (n=2075)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0066 (IC base=+0.187)

- **PATRÓN** `drift_60min` |x|≤ `0.4735` → IC=+0.195 (n=3109)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.97€ cuando `drift_60min` |x|≤ 0.4735 (IC base=+0.187)
...
- **PATRÓN** `dist_vwap_pct` < `0.4297` → IC=+0.179 (n=2277)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.4297 (IC base=+0.187)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.160 (n=560)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.323 (n=196)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.160)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.512` → IC=+0.308 (n=321)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.512 (IC base=+0.160)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8938` → IC=+0.162 (n=312)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.8938 (IC base=+0.160)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.213 (n=367)
...
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.189 (n=181)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0028 (IC base=+0.160)

- **PATRÓN** `sigma_h` > `0.0032` → IC=+0.172 (n=482)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0032 (IC base=+0.160)

- **PATRÓN** `drift_60min` |x|≤ `0.4039` → IC=+0.160 (n=539)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.4039 (IC base=+0.160)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.179 (n=550)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 6.0 (IC base=+0.160)

- **PATRÓN** `ibs_20min` > `0.3271` → IC=+0.213 (n=539)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3271 (IC base=+0.160)

- **PATRÓN** `dist_vwap_pct` > `0.2561` → IC=+0.230 (n=302)
...
- **PATRÓN** `volumen_regimen` > `1.0826` → IC=+0.180 (n=245)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` > 1.0826 (IC base=+0.160)

- **PATRÓN** `volumen_pendiente_norm` < `0.0714` → IC=+0.177 (n=441)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` < 0.0714 (IC base=+0.160)

- **PATRÓN** `volumen_pendiente_norm` > `0.2057` → IC=+0.189 (n=104)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.2057 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` < `2.3955` → IC=+0.189 (n=490)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` < 2.3955 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` > `1.3791` → IC=+0.177 (n=490)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 1.3791 (IC base=+0.160)

- **PATRÓN** `libro_liquidez` > `12682.0918` → IC=+0.187 (n=359)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 12682.0918 (IC base=+0.160)
...
- **PATRÓN** `ibs_20min` > `0.7078` → IC=+0.265 (n=402)
...
- **PATRÓN** `volumen_spike_ratio` > `4.2028` → IC=+0.195 (n=126)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 4.2028 (IC base=+0.193)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.223 (n=413)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.193)

- **PATRÓN** `libro_liquidez` > `1888.8686` → IC=+0.194 (n=204)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 1888.8686 (IC base=+0.193)

- **PATRÓN** `ballena_activa_n` < `59.0` → IC=+0.250 (n=162)
...
- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.160 (n=186)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0056 (IC base=+0.107)

- **PATRÓN** `drift_60min` |x|≤ `0.1236` → IC=+0.149 (n=186)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.1236 (IC base=+0.107)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.145 (n=271)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 15.0 (IC base=+0.107)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.0702` → IC=+0.148 (n=163)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` > 0.0702 (IC base=+0.107)

- **PATRÓN** `volumen_spike_ratio` < `2.4926` → IC=+0.128 (n=409)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.174 (n=1020)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.028 (n=3112)
...
- **FILTRO** `ibs_20min` < `0.7312` → IC=-0.180 (n=148)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=446)
...
- **FILTRO** `py_entrada` < `0.44` → IC=-0.206 (n=209)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.44
  - _Potencial_: sin este filtro IC_bueno=+0.029 (n=427)
...
- **FILTRO** `py_entrada` < `0.495` → IC=-0.130 (n=198)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.042 (n=515)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.138 (n=2594)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=7844)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.279 (n=2587)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=7851)

- **FILTRO** `ibs_7min` < `0.7273` → IC=-0.233 (n=2601)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7273
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=7837)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.172 (n=3536)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=6902)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.229 (n=2974)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=9875)

- **FILTRO** `ibs_7min` > `0.7254` → IC=-0.171 (n=3212)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7254
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=9637)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.3` → IC=-0.322 (n=341)

  - _Acción_: SKIP cuando `py_entrada` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=1098)

- **FILTRO** `ibs_7min` < `1.0` → IC=-0.189 (n=1004)

  - _Acción_: SKIP cuando `ibs_7min` < 1.0
  - _Potencial_: sin este filtro IC_bueno=+0.049 (n=435)

- **FILTRO** `ballena_activa_n` > `14.0` → IC=-0.242 (n=347)

  - _Acción_: SKIP cuando `ballena_activa_n` > 14.0
  - _Potencial_: sin este filtro IC_bueno=-0.077 (n=1092)
...
- **FILTRO** `ibs_7min` > `0.2976` → IC=-0.172 (n=758)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2976
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=1474)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.143 (n=429)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=1578)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.247 (n=492)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=1515)

- **FILTRO** `ibs_7min` < `0.7913` → IC=-0.192 (n=501)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7913
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=1506)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.226 (n=479)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=1521)

- **FILTRO** `ballena_activa_n` > `98.0` → IC=-0.173 (n=678)
...
- **FILTRO** `py_entrada` < `0.42` → IC=-0.232 (n=501)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=1507)

- **FILTRO** `ibs_7min` < `0.7647` → IC=-0.200 (n=502)
...
- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.184 (n=489)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=1519)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.184 (n=586)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.004 (n=1789)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `hora_utc` < `9.0` → IC=-0.129 (n=548)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=1147)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.299 (n=412)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=1283)

- **FILTRO** `ibs_7min` < `0.75` → IC=-0.233 (n=417)

  - _Acción_: SKIP cuando `ibs_7min` < 0.75
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=1278)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.225 (n=419)

  - _Acción_: SKIP cuando `ballena_activa_n` > 13.0
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=1276)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.270 (n=459)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=1764)
...
- **FILTRO** `ballena_activa_n` > `10.0` → IC=-0.131 (n=736)
  ...
  - _Acción_: SKIP cuando `ballena_activa_n` > 10.0
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=1487)
...
### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.148 (n=177)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0038 (IC base=+0.119)

- **PATRÓN** `drift_60min` |x|≤ `0.1878` → IC=+0.128 (n=401)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.64€ cuando `drift_60min` |x|≤ 0.1878 (IC base=+0.119)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2136` → IC=+0.136 (n=182)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.68€ cuando `delta_ratio_macro` |x|> 0.2136 (IC base=+0.119)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.125 (n=427)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` > `0.5319` → IC=+0.212 (n=401)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5319 (IC base=+0.119)

- **PATRÓN** `dist_vwap_pct` > `0.3926` → IC=+0.185 (n=106)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.3926 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.849` → IC=+0.240 (n=156)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.849 (IC base=+0.119)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.127 (n=421)

  - _Acción_: Kelly boost +0.63€ cuando `libro_spread` < 0.01 (IC base=+0.119)

- **PATRÓN** `libro_liquidez` > `8015.433` → IC=+0.176 (n=134)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 8015.433 (IC base=+0.119)
...
- **PATRÓN** `sigma_h` < `0.0037` → IC=+0.173 (n=105)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0037 (IC base=+0.163)

- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.191 (n=40)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0046 (IC base=+0.163)

- **PATRÓN** `drift_60min` |x|≤ `0.193` → IC=+0.186 (n=119)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.193 (IC base=+0.163)

- **PATRÓN** `drift_15min` |x|≤ `0.3897` → IC=+0.191 (n=53)

  - _Acción_: Kelly boost +0.95€ cuando `drift_15min` |x|≤ 0.3897 (IC base=+0.163)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2384` → IC=+0.167 (n=40)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.83€ cuando `delta_ratio_macro` |x|> 0.2384 (IC base=+0.163)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.188 (n=123)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 4.0 (IC base=+0.163)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.169 (n=122)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 17.0 (IC base=+0.163)

- **PATRÓN** `ibs_15` > `0.9483` → IC=+0.286 (n=54)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.9483 (IC base=+0.163)

- **PATRÓN** `dist_vwap_pct` > `0.3217` → IC=+0.227 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3217 (IC base=+0.163)

- **PATRÓN** `dist_vwap_pct` < `0.546` → IC=+0.167 (n=130)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.546 (IC base=+0.163)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.252` → IC=+0.212 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.252 (IC base=+0.163)
...
- **FILTRO** `sigma_h` > `0.01` → IC=-0.186 (n=275)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `ibs_15` < `0.7` → IC=-0.146 (n=207)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.261 (n=207)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.123 (n=290)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=811)

- **FILTRO** `sigma_ewma_delta_pct` > `17.696` → IC=-0.176 (n=378)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 17.696
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=2905)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.357` → IC=+0.147 (n=148)

  - _Acción_: Kelly boost +0.73€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.357 (IC base=-0.067)

- **PATRÓN** `ibs_15` > `0.7` → IC=+0.261 (n=207)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **FILTRO** `sigma_h` > `0.0074` → IC=-0.247 (n=176)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.201 (n=529)

- **FILTRO** `sigma_h` < `0.0035` → IC=-0.218 (n=232)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0035
  - _Potencial_: sin este filtro IC_bueno=-0.209 (n=473)
...
- **FILTRO** `hora_utc` > `17.0` → IC=-0.257 (n=150)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 17.0
...
- **FILTRO** `sigma_ewma_delta_pct` > `19.398` → IC=-0.267 (n=131)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.200 (n=574)

- **PATRÓN** `ibs_15` > `0.7496` → IC=+0.300 (n=23)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **PATRÓN** `drift_60min` |x|≤ `0.0527` → IC=+0.328 (n=91)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0527 (IC base=+0.284)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1374` → IC=+0.310 (n=182)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **PATRÓN** `hora_utc` > `4.0` → IC=+0.313 (n=287)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.284)

- **PATRÓN** `ibs_15` > `0.9673` → IC=+0.357 (n=124)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.9673 (IC base=+0.284)

- **PATRÓN** `dist_vwap_pct` > `0.3186` → IC=+0.359 (n=119)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3186 (IC base=+0.284)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.98` → IC=+0.298 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 6.98 (IC base=+0.284)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.286 (n=335)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.284)

- **PATRÓN** `libro_liquidez` > `13449.5003` → IC=+0.339 (n=91)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13449.5003 (IC base=+0.284)

### UPDOWN_GBM_IBS_ALTO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0035` → IC=+0.276 (n=105)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0035 (IC base=+0.276)

- **PATRÓN** `sigma_h` > `0.0054` → IC=+0.315 (n=52)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0054 (IC base=+0.276)

- **PATRÓN** `drift_60min` |x|≤ `0.162` → IC=+0.300 (n=138)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.162 (IC base=+0.276)

- **PATRÓN** `drift_15min` |x|≤ `0.7272` → IC=+0.281 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.7272 (IC base=+0.276)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1394` → IC=+0.302 (n=104)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1394 (IC base=+0.276)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1224` → IC=+0.318 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1224 (IC base=+0.276)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.310 (n=156)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.276)

- **PATRÓN** `ibs_15` > `0.8332` → IC=+0.297 (n=156)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8332 (IC base=+0.276)

- **PATRÓN** `dist_vwap_pct` > `0.3523` → IC=+0.361 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3523 (IC base=+0.276)

- **PATRÓN** `sigma_ewma_delta_pct` > `22.592` → IC=+0.284 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 22.592 (IC base=+0.276)

- **PATRÓN** `sigma_ewma_delta_pct` < `8.533` → IC=+0.294 (n=134)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 8.533 (IC base=+0.276)

- **PATRÓN** `libro_liquidez` > `13595.2872` → IC=+0.349 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13595.2872 (IC base=+0.276)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.5319 sube el IC de +0.119 a +0.212 en UPDOWN_GBM#15min (n=401). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.9483 sube el IC de +0.163 a +0.286 en UPDOWN_GBM#BTC#15min (n=54). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.7 sube el IC de -0.067 a +0.261 en UPDOWN_GBM_15M_TARDIO (n=207). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#BTC#15min**: dentro de BUY_YES, IBS > 0.7496 sube el IC de +0.013 a +0.300 en UPDOWN_GBM_15M_TARDIO#BTC#15min (n=23). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.9673 sube el IC de +0.284 a +0.357 en UPDOWN_GBM_IBS_ALTO (n=124). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_IBS_ALTO#BTC#15min**: dentro de BUY_YES, IBS > 0.8332 sube el IC de +0.276 a +0.297 en UPDOWN_GBM_IBS_ALTO#BTC#15min (n=156). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 11573 | -0.101 | -1940.46€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 816 | -0.033 | -130.61€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#BTC | 816 | -0.033 | -130.61€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 816 | -0.033 | -130.61€ | 2 | 0 |
...
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 1370 | -0.061 | +683.41€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 388 | -0.003 | +277.32€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 982 | -0.084 | +406.09€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 1370 | -0.061 | +683.41€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 388 | -0.003 | +277.32€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 982 | -0.084 | +406.09€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 37504 | +0.115 | -2293.44€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 6682 | +0.186 | -241.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 116 | -0.085 | -47.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 27334 | +0.098 | -1958.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3372 | +0.119 | -45.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 4583 | +0.072 | -703.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 22 | -0.083 | +0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 4556 | +0.073 | -697.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 7653 | +0.134 | -156.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1901 | +0.199 | -88.29€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 4549 | +0.110 | -99.78€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 8214 | +0.127 | -121.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2436 | +0.168 | -18.55€ | 1 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 4551 | +0.113 | -69.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1215 | +0.101 | -24.70€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 7878 | +0.132 | -496.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2283 | +0.199 | -137.13€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 54 | +0.018 | -8.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 4545 | +0.099 | -276.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 996 | +0.132 | -74.54€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#XRP | 4582 | +0.109 | -286.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 17 | -0.022 | -0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 4563 | +0.110 | -285.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 6614 | +0.175 | -514.62€ | 2 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 6614 | +0.175 | -514.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 