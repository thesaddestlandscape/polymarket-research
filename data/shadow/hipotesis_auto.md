# Hipótesis automáticas — 2026-07-27 06:56 UTC
_Generado por shadow_postmortem.py sobre 37918 resoluciones (PNL=+7889.29€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.197 (n=2535)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 5.0 (IC base=+0.190)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.194 (n=1192)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` < 8.0 (IC base=+0.190)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.331 (n=804)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.210 (n=153)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.182)

- **PATRÓN** `py_entrada` < `0.64` → IC=+0.202 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.64 (IC base=+0.182)

- **PATRÓN** `py_entrada` > `0.575` → IC=+0.182 (n=149)

  - _Acción_: Kelly boost +0.91€ cuando `py_entrada` > 0.575 (IC base=+0.182)
...
- **PATRÓN** `dist_vwap_pct` > `0.4462` → IC=+0.141 (n=332)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.4462 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.032` → IC=+0.183 (n=772)

  - _Acción_: Kelly boost +0.92€ cuando `sigma_ewma_delta_pct` > 3.032 (IC base=+0.117)

- **PATRÓN** `sigma_h` > `0.0103` → IC=+0.123 (n=1476)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.153 (n=355)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.147 (n=922)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.154 (n=348)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` < 6.0 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.3347` → IC=+0.189 (n=120)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.3347 (IC base=+0.137)
...
- **PATRÓN** `dist_vwap_pct` > `0.2713` → IC=+0.196 (n=492)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.2713 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.555` → IC=+0.242 (n=526)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.555 (IC base=+0.149)

- **PATRÓN** `sigma_h` > `0.0067` → IC=+0.154 (n=1354)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.4047` → IC=+0.125 (n=337)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` > 0.4047 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.244` → IC=+0.136 (n=300)

  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` > 8.244 (IC base=+0.119)
...
- **PATRÓN** `dist_vwap_pct` > `0.1787` → IC=+0.170 (n=180)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.1787 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.301` → IC=+0.198 (n=114)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` > 7.301 (IC base=+0.129)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.018` → IC=-0.239 (n=21)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.018
  - _Potencial_: sin este filtro IC_bueno=+0.098 (n=495)
...
- **PATRÓN** `sigma_h` < `0.0172` → IC=+0.210 (n=523)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0172 (IC base=+0.208)

- **PATRÓN** `sigma_h` > `0.007` → IC=+0.239 (n=524)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.1883` → IC=+0.319 (n=103)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1883 (IC base=+0.208)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.846` → IC=+0.206 (n=460)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.846 (IC base=+0.208)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0125` → IC=+0.147 (n=547)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` > 0.0125 (IC base=+0.107)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.123 (n=1498)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 7.0 (IC base=+0.107)

- **PATRÓN** `dist_vwap_pct` > `0.4462` → IC=+0.155 (n=334)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.4462 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.301` → IC=+0.258 (n=258)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.301 (IC base=+0.107)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.004` → IC=+0.144 (n=245)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` < 0.004 (IC base=+0.098)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `12.377` → IC=+0.200 (n=58)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.377 (IC base=+0.069)
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.177 (n=639)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.162 (n=226)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 6.0 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` > `0.34` → IC=+0.226 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.34 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.506` → IC=+0.156 (n=655)
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 1368 | +0.101 | +32.87€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 1368 | +0.101 | +32.87€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 541 | +0.089 | -1.72€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 541 | +0.089 | -1.72€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 506 | +0.128 | +15.38€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 506 | +0.128 | +15.38€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 302 | +0.076 | +19.56€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 302 | +0.076 | +19.56€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 6687 | +0.180 | -15.13€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#60min | 1182 | +0.159 | +91.04€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 2324 | +0.184 | +8.17€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 393 | +0.146 | +0.85€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 8203 | +0.095 | +2483.90€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 8203 | +0.095 | +2483.90€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1985 | +0.074 | +355.13€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1985 | +0.074 | +355.13€ | 0 | 5 |
| ✅ GBM_LATE_15M#ETH | 1807 | +0.074 | +303.80€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1807 | +0.074 | +303.80€ | 0 | 6 |
| ✅ GBM_LATE_15M#SOL | 2208 | +0.084 | +759.07€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 2208 | +0.084 | +759.07€ | 1 | 3 |
| ✅ GBM_LATE_15M#XRP | 2203 | +0.140 | +1065.89€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 2203 | +0.140 | +1065.89€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 5435 | +0.117 | +2553.49€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 5435 | +0.117 | +2553.49€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 1425 | +0.086 | +505.28€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 1425 | +0.086 | +505.28€ | 0 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1426 | +0.089 | +496.20€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1426 | +0.089 | +496.20€ | 0 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 1359 | +0.084 | +520.41€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 1359 | +0.084 | +520.41€ | 1 | 4 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 1225 | +0.221 | +1031.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 1225 | +0.221 | +1031.60€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 5422 | +0.069 | +1472.78€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 5422 | +0.069 | +1472.78€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 1295 | +0.037 | +173.63€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 1295 | +0.037 | +173.63€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 1326 | +0.027 | +114.39€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 1326 | +0.027 | +114.39€ | 0 | 2 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 1276 | +0.030 | +242.52€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 1276 | +0.030 | +242.52€ | 1 | 2 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 1525 | +0.165 | +942.24€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1525 | +0.165 | +942.24€ | 0 | 11 |
...
| ✅ UPDOWN_GBM | 2778 | +0.035 | +251.94€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 2515 | +0.052 | +290.45€ | 0 | 3 |
...
| ✅ UPDOWN_GBM#SOL | 286 | -0.042 | -1.11€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 217 | -0.016 | +5.33€ | 4 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 2021 | +0.154 | +906.94€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 2021 | +0.154 | +906.94€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 480 | +0.087 | +123.25€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 480 | +0.087 | +123.25€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 563 | +0.171 | +335.64€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 563 | +0.171 | +335.64€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.171: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.074 n=1519 | neutral: IC=+0.019 n=1241 | overbought(IBS>0.7): IC=+0.190 n=1571
  - _Datos_: n=4615 IC=+0.105 PNL=+1224.69€
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=217 IC=-0.016 PNL=+5.33€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.112 n=22583 | tras_1loss IC=+0.079 n=15126 | tras_2loss IC=+0.051 n=6329/40 | gap=+0.060 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2641 IC=+0.040 PNL=+265.74€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2641 IC=+0.040 PNL=+265.74€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.088 > 0.08 con n=1350 PNL=+229.08€
  - _Datos_: n=1350 IC=+0.088 PNL=+229.08€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=1347 IC=+0.037 PNL=+104.11€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1347 IC=+0.037 PNL=+104.11€
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=1656 IC=+0.066 PNL=+249.18€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=1656 IC=+0.066 PNL=+249.18€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=1094 IC=+0.056 PNL=+119.70€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=1094 IC=+0.056 PNL=+119.70€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.105 > 0.08 con n=537 PNL=+138.93€
  - _Datos_: n=537 IC=+0.105 PNL=+138.93€
