# Hipótesis automáticas — 2026-08-19 16:54 UTC
_Generado por shadow_postmortem.py sobre 77902 resoluciones (PNL=+8093.91€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `154.9` → IC=-0.253 (n=1043)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 154.9
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3131)
...
- **FILTRO** `restante_s_al_confirmar` < `75.85` → IC=-0.467 (n=148)
...
- **FILTRO** `restante_s_al_confirmar` < `148.37` → IC=-0.321 (n=205)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 148.37
  - _Potencial_: sin este filtro IC_bueno=-0.197 (n=618)
...
- **PATRÓN** `py_entrada` < `0.325` → IC=+0.302 (n=246)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.325 (IC base=+0.193)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.196 (n=471)
...
- **PATRÓN** `libro_liquidez` > `793.7358` → IC=+0.236 (n=365)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 793.7358 (IC base=+0.221)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.220 (n=3767)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.153 (n=433)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.216 (n=354)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.249 (n=583)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.219 (n=685)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.145)

- **PATRÓN** `restante_min` < `3.82` → IC=+0.164 (n=625)

  - _Acción_: Kelly boost +0.82€ cuando `restante_min` < 3.82 (IC base=+0.145)

- **PATRÓN** `restante_min` > `4.91` → IC=+0.207 (n=627)

  - _Acción_: Kelly boost +1.00€ cuando `restante_min` > 4.91 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.150 (n=1947)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 5.0 (IC base=+0.145)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.152 (n=1922)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 17.0 (IC base=+0.145)

- **PATRÓN** `lag_apertura_s` < `5.62` → IC=+0.208 (n=624)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.158 (n=965)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `7.22` → IC=+0.200 (n=308)

  - _Acción_: Kelly boost +1.00€ cuando `lag_apertura_s` < 7.22 (IC base=+0.152)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min
- **PATRÓN** `py_entrada` < `0.43` → IC=+0.167 (n=838)
...
- **PATRÓN** `restante_min` > `4.94` → IC=+0.200 (n=341)

  - _Acción_: Kelly boost +1.00€ cuando `restante_min` > 4.94 (IC base=+0.139)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.141 (n=982)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` > 5.0 (IC base=+0.139)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.147 (n=973)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 17.0 (IC base=+0.139)

- **PATRÓN** `lag_apertura_s` < `3.36` → IC=+0.213 (n=315)
...
- **PATRÓN** `ibs_20min` > `0.9821` → IC=+0.271 (n=457)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9821 (IC base=+0.099)
...
- **PATRÓN** `volumen_regimen` < `1.3035` → IC=+0.242 (n=184)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.3035 (IC base=+0.099)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3253` → IC=+0.146 (n=111)
  ...
  - _Acción_: Kelly boost +0.73€ cuando `volumen_pendiente_norm` > 0.3253 (IC base=+0.099)

- **PATRÓN** `volumen_spike_ratio` > `1.6514` → IC=+0.126 (n=701)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_spike_ratio` > 1.6514 (IC base=+0.099)
...
- **PATRÓN** `ibs_20min` > `0.5742` → IC=+0.125 (n=78)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` > 0.5742 (IC base=+0.044)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.253` → IC=+0.200 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.253 (IC base=+0.044)
...
- **PATRÓN** `sigma_h` < `0.0055` → IC=+0.190 (n=27)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.0055 (IC base=+0.111)

- **PATRÓN** `hora_utc` > `23.0` → IC=+0.324 (n=15)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 23.0 (IC base=+0.111)

- **PATRÓN** `ibs_20min` > `0.375` → IC=+0.210 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.375 (IC base=+0.111)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.167 (n=31)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.01 (IC base=+0.111)
...
- **PATRÓN** `ibs_20min` > `0.9448` → IC=+0.245 (n=582)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.242` → IC=+0.131 (n=1070)
...
- **FILTRO** `ibs_20min` < `0.0672` → IC=-0.160 (n=95)
...
- **PATRÓN** `dist_vwap_pct` > `0.2819` → IC=+0.237 (n=17)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2819 (IC base=-0.021)
...
- **PATRÓN** `sigma_h` > `0.0067` → IC=+0.206 (n=631)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.148 (n=665)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.917` → IC=+0.263 (n=928)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.917 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.2396` → IC=+0.190 (n=346)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.2396 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.947` → IC=+0.271 (n=678)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.947 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `1.2221` → IC=+0.146 (n=744)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 1.2221 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` > `0.6379` → IC=+0.154 (n=744)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 0.6379 (IC base=+0.147)
...
- **PATRÓN** `volumen_spike_ratio` < `2.6397` → IC=+0.127 (n=1017)
...
- **PATRÓN** `libro_spread` < `0.05` → IC=+0.155 (n=1414)

  - _Acción_: Kelly boost +0.78€ cuando `libro_spread` < 0.05 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `2823.0723` → IC=+0.191 (n=464)
...
- **PATRÓN** `dist_vwap_pct` < `0.1585` → IC=+0.188 (n=1105)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.1585 (IC base=+0.207)
...
- **PATRÓN** `sigma_h` > `0.0039` → IC=+0.213 (n=1506)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0039 (IC base=+0.211)

- **PATRÓN** `drift_60min` |x|≤ `0.2911` → IC=+0.213 (n=1505)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2911 (IC base=+0.211)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.245 (n=532)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.211)

- **PATRÓN** `ibs_20min` < `0.519` → IC=+0.277 (n=1505)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.488` → IC=+0.219 (n=1413)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.488 (IC base=+0.211)

- **PATRÓN** `volumen_regimen` < `0.6176` → IC=+0.198 (n=342)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` < 0.6176 (IC base=+0.211)

- **PATRÓN** `volumen_regimen` > `1.234` → IC=+0.209 (n=342)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.234 (IC base=+0.211)

- **PATRÓN** `volumen_pendiente_norm` < `0.114` → IC=+0.223 (n=738)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.114 (IC base=+0.211)

- **PATRÓN** `volumen_pendiente_norm` > `0.251` → IC=+0.247 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.251 (IC base=+0.211)
...
- **PATRÓN** `sigma_h` < `0.0036` → IC=+0.214 (n=138)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0036 (IC base=+0.160)

- **PATRÓN** `drift_60min` |x|≤ `0.2654` → IC=+0.171 (n=138)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.2654 (IC base=+0.160)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.196 (n=54)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 16.0 (IC base=+0.160)

- **PATRÓN** `ibs_20min` < `0.1599` → IC=+0.287 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1599 (IC base=+0.160)

- **PATRÓN** `dist_vwap_pct` > `0.1518` → IC=+0.245 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1518 (IC base=+0.160)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.492` → IC=+0.262 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.492 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` < `1.1115` → IC=+0.171 (n=138)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 1.1115 (IC base=+0.160)

- **PATRÓN** `volumen_pendiente_norm` < `0.1747` → IC=+0.274 (n=91)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1747 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` > `1.8112` → IC=+0.276 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8112 (IC base=+0.160)

- **PATRÓN** `libro_liquidez` > `5477.3482` → IC=+0.192 (n=63)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 5477.3482 (IC base=+0.160)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.141 (n=62)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 12.0 (IC base=+0.085)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.125 (n=54)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` < 17.0 (IC base=+0.085)

- **PATRÓN** `ibs_20min` < `0.9802` → IC=+0.136 (n=53)

  - _Acción_: Kelly boost +0.68€ cuando `ibs_20min` < 0.9802 (IC base=+0.085)
...
- **FILTRO** `py_entrada` < `0.4` → IC=-0.234 (n=152)

  - _Acción_: SKIP cuando `py_entrada` < 0.4
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=481)

- **FILTRO** `ibs_20min` < `0.7265` → IC=-0.188 (n=158)
...
- **PATRÓN** `libro_liquidez` > `14460.0391` → IC=+0.141 (n=37)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `libro_liquidez` > 14460.0391 (IC base=-0.054)
...
- **FILTRO** `hora_utc` > `15.0` → IC=-0.129 (n=33)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 15.0
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=272)
...
- **PATRÓN** `ibs_20min` > `0.7143` → IC=+0.147 (n=49)

  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` > 0.7143 (IC base=+0.047)

- **PATRÓN** `libro_liquidez` > `3915.5837` → IC=+0.132 (n=55)

  - _Acción_: Kelly boost +0.66€ cuando `libro_liquidez` > 3915.5837 (IC base=+0.047)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.185 (n=363)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=1324)
...
- **FILTRO** `ibs_7min` < `0.7241` → IC=-0.223 (n=421)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=1266)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.185 (n=566)

  - _Acción_: SKIP cuando `ballena_activa_n` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=1121)
...
- **FILTRO** `py_entrada` > `0.59` → IC=-0.194 (n=83)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.008 (n=738)
...
- **PATRÓN** `ibs_15` < `0.125` → IC=+0.138 (n=249)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.69€ cuando `ibs_15` < 0.125 (IC base=+0.063)
...
- **FILTRO** `sigma_ewma_delta_pct` > `12.785` → IC=-0.173 (n=258)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=1047)

- **PATRÓN** `ibs_15` > `0.4118` → IC=+0.178 (n=172)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.89€ cuando `ibs_15` > 0.4118 (IC base=-0.039)
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.4118 sube el IC de -0.039 a +0.178 en UPDOWN_GBM_15M_TARDIO (n=172). Ya aplicado como kelly_boost=+0.89€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 3748 | -0.112 | -590.58€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 474 | -0.023 | -20.57€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 3274 | -0.125 | -570.01€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 955 | +0.014 | -111.13€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 955 | +0.014 | -111.13€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP | 823 | -0.229 | -177.59€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP#5min | 823 | -0.229 | -177.59€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 16216 | +0.113 | -1043.09€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3635 | +0.182 | -127.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 104 | -0.094 | -47.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 9816 | +0.085 | -885.11€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 3609 | +0.137 | -35.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 994 | +0.195 | -44.38€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 1628 | +0.107 | -32.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 945 | +0.139 | +64.32€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 1660 | +0.058 | -286.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 10 | +0.000 | -3.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 1649 | +0.058 | -281.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 3907 | +0.128 | -42.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1342 | +0.164 | -15.10€ | 0 | 6 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 1630 | +0.107 | -21.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 923 | +0.116 | +2.81€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 3729 | +0.133 | -241.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1263 | +0.195 | -66.86€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 44 | +0.022 | -7.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 1629 | +0.084 | -117.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 793 | +0.139 | -49.42€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 1652 | +0.118 | -69.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 12 | +0.043 | +1.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 1639 | +0.119 | -69.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 3722 | +0.163 | -349.61€ | 2 | 2 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 3722 | +0.163 | -349.61€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 7022 | +0.185 | -703.57€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 7022 | +0.185 | -703.57€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 1238 | +0.152 | -184.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 1238 | +0.152 | -184.30€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 1088 | +0.242 | -19.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 1088 | +0.242 | -19.12€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 2489 | +0.145 | +125.36€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 2489 | +0.145 | +125.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 1230 | +0.152 | +74.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 1230 | +0.152 | +74.26€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 1259 | +0.139 | +51.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 
58€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 580 | -0.053 | +48.58€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE | 3211 | +0.015 | +18.37€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 3211 | +0.015 | +18.37€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_5M_FADE#DOGE | 571 | +0.013 | +1.21€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE#5min | 571 | +0.013 | +1.21€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH | 544 | +0.007 | +1.23€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH#5min | 544 | +0.007 | +1.23€ | 1 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL | 570 | +0.007 | -0.94€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL#5min | 570 | +0.007 | -0.94€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP | 554 | +0.009 | -2.72€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP#5min | 554 | +0.009 | -2.72€ | 0 | 0 |
...
| ✅ STREAK_FADE_5M | 428 | -0.016 | -18.70€ | 2 | 0 |
| ✅ STREAK_FADE_5M#5min | 428 | -0.016 | -18.70€ | 0 | 0 |
| ✅ STREAK_FADE_5M#DOGE | 130 | +0.015 | -0.04€ | 0 | 0 |
| ✅ STREAK_FADE_5M#DOGE#5min | 130 | +0.015 | -0.04€ | 0 | 0 |
...
| ✅ STREAK_MOM_5M | 715 | +0.020 | +3.32€ | 1 | 0 |
| ✅ STREAK_MOM_5M#5min | 715 | +0.020 | +3.32€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE | 231 | +0.011 | -2.26€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE#5min | 231 | +0.011 | -2.26€ | 0 | 0 |
...
| ✅ STRUCT_NO_15M | 1996 | +0.011 | -12.33€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 1996 | +0.011 | -12.33€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 757 | +0.003 | -11.54€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 757 | +0.003 | -11.54€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 779 | +0.017 | -0.42€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 779 | +0.017 | -0.42€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 460 | +0.015 | -0.36€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 460 | +0.015 | -0.36€ | 2 | 0 |
| ✅ UPDOWN_GBM | 2722 | +0.029 | +199.70€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1168 | +0.081 | +196.41€ | 1 | 8 |
...
| ✅ UPDOWN_GBM#BTC | 523 | +0.045 | +58.70€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 140 | +0.077 | +9.15€ | 2 | 12 |
...
| ✅ UPDOWN_GBM#SOL | 522 | -0.004 | -3.37€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 230 | +0.009 | -1.89€ | 1 | 2 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1900 | -0.058 | +446.10€ | 3 | 3 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1900 | -0.058 | +446.10€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 615 | -0.030 | +226.83€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 615 | -0.030 | +226.83€ | 2 | 1 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 590 | -0.056 | +106.57€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 590 | -0.056 | +106.57€ | 3 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.133) — sin ventaja clara. oversold(IBS<0.3): IC=-0.011 n=941 | neutral: IC=+0.022 n=906 | overbought(IBS>0.7): IC=+0.122 n=1203
  - _Datos_: n=3273 IC=+0.049 PNL=+334.06€
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=230 IC=+0.009 PNL=-1.89€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.098 n=45046 | tras_1loss IC=+0.056 n=32561 | tras_2loss IC=+0.015 n=14404/40 | gap=+0.083 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2486 IC=+0.022 PNL=+145.77€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2486 IC=+0.022 PNL=+145.77€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=140 IC=+0.077 PNL=+9.15€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=140 IC=+0.077 PNL=+9.15€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.080 > 0.08 con n=734 PNL=+105.77€
  - _Datos_: n=734 IC=+0.080 PNL=+105.77€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=177 IC=+0.064 PNL=+25.43€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=177 IC=+0.064 PNL=+25.43€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=623 IC=+0.073 PNL=+111.87€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=623 IC=+0.073 PNL=+111.87€
...
**⏳ H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: 33/50 ops en el filtro definido (IC actual=+0.157 PNL=+5.99€)
  - _Datos_: n=33 IC=+0.157 PNL=+5.99€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1675 IC=-0.137 PNL=+130.23€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1675 IC=-0.137 PNL=+130.23€
...
**⏳ H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: 92/289 ops en el filtro definido (IC actual=-0.277 PNL=-18.07€)
  - _Datos_: n=92 IC=-0.277 PNL=-18.07€
...
**〰️ H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: n=601 IC=+0.011 PNL=+141.57€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=601 IC=+0.011 PNL=+141.57€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.134 > 0.08 con n=170 PNL=-13.91€
  - _Datos_: n=170 IC=+0.134 PNL=-13.91€
