# Hipótesis automáticas — 2026-07-16 08:27 UTC
_Generado por shadow_postmortem.py sobre 16699 resoluciones (PNL=+3427.33€)_
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.193 (n=926)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 18.0 (IC base=+0.183)

- **PATRÓN** `py_entrada` > `0.695` → IC=+0.292 (n=417)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.183)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.190 (n=1048)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.01 (IC base=+0.183)

- **PATRÓN** `libro_liquidez` > `4364.0743` → IC=+0.187 (n=599)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.232 (n=95)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.204)

- **PATRÓN** `py_entrada` > `0.615` → IC=+0.256 (n=178)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.230 (n=202)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.225)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.235 (n=96)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.225)

- **PATRÓN** `py_entrada` > `0.62` → IC=+0.287 (n=172)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.62 (IC base=+0.225)
...
### GBM_LATE_15M
- **PATRÓN** `dist_vwap_pct` > `0.5883` → IC=+0.126 (n=129)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` > 0.5883 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.173` → IC=+0.217 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.173 (IC base=+0.107)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.135 (n=1232)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` > 0.0091 (IC base=+0.107)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.128 (n=1237)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 11.0 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.188` → IC=+0.131 (n=139)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` > 6.188 (IC base=+0.107)
...
- **PATRÓN** `dist_vwap_pct` < `0.4825` → IC=+0.126 (n=153)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` < 0.4825 (IC base=+0.064)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.176` → IC=+0.207 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.176 (IC base=+0.064)
...
- **PATRÓN** `dist_vwap_pct` > `0.7087` → IC=+0.153 (n=47)
  ...
  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.7087 (IC base=+0.068)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.518` → IC=+0.221 (n=41)
...
- **PATRÓN** `sigma_h` < `0.0108` → IC=+0.139 (n=211)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0108 (IC base=+0.111)
...
- **PATRÓN** `sigma_h` > `0.024` → IC=+0.176 (n=202)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.024 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.155 (n=471)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.118` → IC=+0.190 (n=27)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 9.118 (IC base=+0.147)
...
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.224 (n=331)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.165 (n=757)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 18.0 (IC base=+0.159)

- **PATRÓN** `dist_vwap_pct` > `0.1486` → IC=+0.196 (n=287)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1486 (IC base=+0.159)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.022` → IC=+0.302 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.022 (IC base=+0.159)

- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.180 (n=749)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.4117` → IC=+0.201 (n=526)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4117 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.188` → IC=+0.288 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.188 (IC base=+0.165)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **PATRÓN** `sigma_h` > `0.0028` → IC=+0.137 (n=177)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0028 (IC base=+0.112)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.161 (n=125)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 12.0 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `0.5737` → IC=+0.206 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5737 (IC base=+0.112)
...
- **PATRÓN** `sigma_h` > `0.0028` → IC=+0.175 (n=189)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0028 (IC base=+0.142)
...
- **PATRÓN** `dist_vwap_pct` < `0.4825` → IC=+0.136 (n=152)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.4825 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.827` → IC=+0.364 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.827 (IC base=+0.142)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.164 (n=206)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 18.0 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` > `0.699` → IC=+0.181 (n=45)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.699 (IC base=+0.143)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.551` → IC=+0.357 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.551 (IC base=+0.143)

- **PATRÓN** `sigma_h` < `0.0068` → IC=+0.147 (n=168)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0068 (IC base=+0.141)

- **PATRÓN** `sigma_h` > `0.0037` → IC=+0.158 (n=191)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0037 (IC base=+0.141)
...
- **PATRÓN** `dist_vwap_pct` > `0.7037` → IC=+0.208 (n=46)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7037 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` < `0.2574` → IC=+0.189 (n=101)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.2574 (IC base=+0.141)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.012` → IC=+0.250 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.012 (IC base=+0.141)
...
- **PATRÓN** `sigma_h` < `0.0214` → IC=+0.274 (n=157)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0214 (IC base=+0.249)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.252 (n=143)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.249)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.290 (n=141)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.249)

- **PATRÓN** `dist_vwap_pct` > `0.5381` → IC=+0.328 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5381 (IC base=+0.249)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.462` → IC=+0.338 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.462 (IC base=+0.249)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.015` → IC=+0.193 (n=226)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.015 (IC base=+0.111)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.145 (n=229)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 5.0 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` > `0.3415` → IC=+0.169 (n=176)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` > 0.3415 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` < `0.1365` → IC=+0.138 (n=244)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.1365 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.256` → IC=+0.306 (n=70)
...
- **PATRÓN** `dist_vwap_pct` < `0.2` → IC=+0.121 (n=439)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.2 (IC base=+0.089)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.969` → IC=+0.179 (n=26)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 10.969 (IC base=+0.081)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.123 (n=67)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 17.0 (IC base=+0.070)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.43` → IC=+0.136 (n=53)

  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` > 4.43 (IC base=+0.070)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.156 (n=59)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0039 (IC base=+0.057)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.997` → IC=+0.167 (n=19)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 7.997 (IC base=+0.021)
...
- **PATRÓN** `sigma_h` > `0.0117` → IC=+0.210 (n=198)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0117 (IC base=+0.199)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.198 (n=210)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.2986` → IC=+0.263 (n=57)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2986 (IC base=+0.199)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.43` → IC=+0.214 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.43 (IC base=+0.199)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.903` → IC=+0.212 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.903 (IC base=+0.199)
...
- **PATRÓN** `py_entrada` < `0.495` → IC=+0.214 (n=26)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.495 (IC base=+0.134)

- **PATRÓN** `btc_momentum` |x|≤ `0.03` → IC=+0.154 (n=50)

  - _Acción_: Kelly boost +0.77€ cuando `btc_momentum` |x|≤ 0.03 (IC base=+0.134)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `16.594` → IC=+0.122 (n=146)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` < 16.594 (IC base=+0.046)
...
- **PATRÓN** `dist_vwap_pct` > `0.9263` → IC=+0.182 (n=20)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.9263 (IC base=+0.044)
...
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_SOL_ALTACONVICCION` — IC=+0.300 n=33. Faltan ~7 resoluciones para umbral n≥40. ETA: ~5h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min` — IC=+0.300 n=33. Faltan ~7 resoluciones para umbral n≥40. ETA: ~5h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL` — IC=+0.300 n=33. Faltan ~7 resoluciones para umbral n≥40. ETA: ~5h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min` — IC=+0.300 n=33. Faltan ~7 resoluciones para umbral n≥40. ETA: ~5h.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#SOL#15min` — IC=+0.207 n=39. Faltan ~1 resoluciones para umbral n≥40. ETA: ~1h.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#SOL` — IC=+0.207 n=39. Faltan ~1 resoluciones para umbral n≥40. ETA: ~1h.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#ETH#15min` — IC=+0.203 n=35. Faltan ~5 resoluciones para umbral n≥40. ETA: ~4h.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#ETH` — IC=+0.203 n=35. Faltan ~5 resoluciones para umbral n≥40. ETA: ~4h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2490 | +0.175 | -50.07€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1599 | +0.213 | +26.44€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 721 | +0.182 | -9.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 535 | +0.208 | -7.48€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 885 | +0.174 | -3.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 529 | +0.216 | +17.35€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 876 | +0.171 | -33.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 527 | +0.216 | +19.95€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 33 | +0.300 | +2.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 33 | +0.300 | +2.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 33 | +0.300 | +2.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 33 | +0.300 | +2.11€ | 0 | 0 |
| ✅ GBM_LATE_15M | 4831 | +0.102 | +1507.22€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 4831 | +0.102 | +1507.22€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1158 | +0.063 | +161.55€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 