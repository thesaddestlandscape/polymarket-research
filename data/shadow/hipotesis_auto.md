# Hipótesis automáticas — 2026-07-16 13:38 UTC
_Generado por shadow_postmortem.py sobre 17037 resoluciones (PNL=+3458.99€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.183 (n=674)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.239 (n=136)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 12.0 (IC base=+0.222)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.229 (n=212)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `5171.5093` → IC=+0.223 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 5171.5093 (IC base=+0.222)
...
- **PATRÓN** `libro_liquidez` > `4118.7645` → IC=+0.212 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4118.7645 (IC base=+0.204)
...
- **PATRÓN** `py_entrada` < `0.375` → IC=+0.250 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.375 (IC base=+0.158)

- **PATRÓN** `libro_liquidez` > `4530.0713` → IC=+0.190 (n=56)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 4530.0713 (IC base=+0.158)
...
- **PATRÓN** `py_entrada` > `0.615` → IC=+0.275 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.615 (IC base=+0.220)

- **PATRÓN** `libro_liquidez` > `1591.9819` → IC=+0.223 (n=200)
...
### GBM_LATE_15M
- **PATRÓN** `dist_vwap_pct` > `0.8734` → IC=+0.131 (n=101)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.8734 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.059` → IC=+0.211 (n=102)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.059 (IC base=+0.107)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.135 (n=1254)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.176` → IC=+0.182 (n=61)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 3.176 (IC base=+0.060)
...
- **PATRÓN** `sigma_h` < `0.0045` → IC=+0.149 (n=129)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0045 (IC base=+0.091)

- **PATRÓN** `dist_vwap_pct` > `0.6749` → IC=+0.143 (n=26)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.6749 (IC base=+0.091)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.386` → IC=+0.240 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.386 (IC base=+0.091)

- **PATRÓN** `dist_vwap_pct` > `0.762` → IC=+0.144 (n=43)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.762 (IC base=+0.066)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.561` → IC=+0.209 (n=53)
...
- **PATRÓN** `sigma_h` > `0.0092` → IC=+0.132 (n=474)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_h` > `0.0245` → IC=+0.180 (n=204)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0245 (IC base=+0.149)
...
- **PATRÓN** `dist_vwap_pct` < `0.1341` → IC=+0.120 (n=127)

  - _Acción_: Kelly boost +0.60€ cuando `dist_vwap_pct` < 0.1341 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.178` → IC=+0.224 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.178 (IC base=+0.149)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.998` → IC=+0.300 (n=93)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.998 (IC base=+0.156)

- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.178 (n=774)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.4184` → IC=+0.191 (n=561)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.4184 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.072` → IC=+0.226 (n=104)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.159 (n=127)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 12.0 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` > `0.5034` → IC=+0.206 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5034 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.905` → IC=+0.210 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.905 (IC base=+0.110)

- **PATRÓN** `sigma_h` > `0.0028` → IC=+0.163 (n=197)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.0028 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.167 (n=76)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 16.0 (IC base=+0.133)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.132 (n=134)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` < 11.0 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `0.4662` → IC=+0.126 (n=161)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` < 0.4662 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.827` → IC=+0.300 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.827 (IC base=+0.133)
...
- **PATRÓN** `sigma_h` < `0.0069` → IC=+0.144 (n=175)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` < 0.0069 (IC base=+0.129)

- **PATRÓN** `sigma_h` > `0.0038` → IC=+0.147 (n=199)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` > 0.0038 (IC base=+0.129)
...
- **PATRÓN** `dist_vwap_pct` < `0.4151` → IC=+0.161 (n=119)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.4151 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.012` → IC=+0.202 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.012 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` < `12.305` → IC=+0.154 (n=131)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 12.305 (IC base=+0.129)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.155 (n=146)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.2378` → IC=+0.176 (n=66)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.2378 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` < `0.9819` → IC=+0.144 (n=158)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.9819 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.082` → IC=+0.264 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.082 (IC base=+0.135)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.801` → IC=+0.365 (n=35)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.288 (n=130)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.290)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.298 (n=176)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.290)

- **PATRÓN** `dist_vwap_pct` < `0.5139` → IC=+0.308 (n=149)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5139 (IC base=+0.290)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.726` → IC=+0.317 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.726 (IC base=+0.290)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0152` → IC=+0.202 (n=233)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0152 (IC base=+0.113)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.131 (n=621)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` < 15.0 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `0.3506` → IC=+0.167 (n=172)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.3506 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.1397` → IC=+0.140 (n=273)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.1397 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.023` → IC=+0.305 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.023 (IC base=+0.113)
...
- **PATRÓN** `dist_vwap_pct` > `0.4306` → IC=+0.167 (n=19)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.4306 (IC base=+0.082)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.68` → IC=+0.196 (n=21)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 13.68 (IC base=+0.082)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.120 (n=77)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` > 16.0 (IC base=+0.064)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0046` → IC=+0.134 (n=80)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.0046 (IC base=+0.054)

- **PATRÓN** `sigma_ewma_delta_pct` > `14.369` → IC=+0.289 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 14.369 (IC base=+0.054)
...
- **PATRÓN** `sigma_h` > `0.0129` → IC=+0.149 (n=72)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0129 (IC base=+0.085)
...
- **PATRÓN** `dist_vwap_pct` < `0.4086` → IC=+0.145 (n=74)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.4086 (IC base=+0.085)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.065` → IC=+0.320 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.065 (IC base=+0.085)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.951` → IC=+0.230 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.951 (IC base=+0.206)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.107` → IC=+0.222 (n=77)
...
- **PATRÓN** `ibs_15` > `0.7204` → IC=+0.179 (n=266)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.90€ cuando `ibs_15` > 0.7204 (IC base=+0.027)

- **PATRÓN** `dist_vwap_pct` > `0.7405` → IC=+0.203 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7405 (IC base=+0.027)

- **PATRÓN** `sigma_ewma_delta_pct` > `18.799` → IC=+0.265 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 18.799 (IC base=+0.027)
...
- **PATRÓN** `dist_vwap_pct` > `0.9393` → IC=+0.152 (n=21)
  ...
  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.9393 (IC base=+0.047)

- **PATRÓN** `sigma_ewma_delta_pct` < `16.557` → IC=+0.121 (n=151)

  - _Acción_: Kelly boost +0.60€ cuando `sigma_ewma_delta_pct` < 16.557 (IC base=+0.047)
...
- **PATRÓN** `drift_60min` |x|≤ `0.241` → IC=+0.129 (n=192)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.64€ cuando `drift_60min` |x|≤ 0.241 (IC base=+0.074)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.152 (n=162)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 11.0 (IC base=+0.074)

- **PATRÓN** `ibs_15` < `0.9306` → IC=+0.132 (n=153)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.66€ cuando `ibs_15` < 0.9306 (IC base=+0.074)

- **PATRÓN** `ibs_15` > `0.6323` → IC=+0.190 (n=153)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.95€ cuando `ibs_15` > 0.6323 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` > `0.7608` → IC=+0.300 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7608 (IC base=+0.074)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.485` → IC=+0.167 (n=22)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 4.485 (IC base=+0.074)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.527` → IC=+0.141 (n=37)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` < 5.527 (IC base=+0.074)
...
- **PATRÓN** `dist_vwap_pct` > `0.9766` → IC=+0.200 (n=18)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9766 (IC base=+0.046)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS > 0.7204 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.179 n=266). Confirma señal de reversión media → alinear con BUY_NO.
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS > 0.6323 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.190 n=153). Confirma señal de reversión media → alinear con BUY_NO.
...
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_SOL_ALTACONVICCION` — IC=+0.305 n=39. Faltan ~1 resoluciones para umbral n≥40. ETA: ~1h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min` — IC=+0.305 n=39. Faltan ~1 resoluciones para umbral n≥40. ETA: ~1h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL` — IC=+0.305 n=39. Faltan ~1 resoluciones para umbral n≥40. ETA: ~1h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min` — IC=+0.305 n=39. Faltan ~1 resoluciones para umbral n≥40. ETA: ~1h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2572 | +0.174 | -55.71€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1658 | +0.211 | +19.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 109 | +0.013 | -19.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 367 | +0.091 | -60.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 438 | +0.145 | +4.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 745 | +0.179 | -15.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 553 | +0.204 | -12.24€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 36 | +0.026 | -6.33€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 146 | +0.149 | +7.33€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#ETH | 916 | +0.174 | -5.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 550 | +0.214 | +15.17€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 36 | -0.079 | -12.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 184 | +0.107 | -23.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 146 | +0.169 | +15.45€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 902 | +0.172 | -33.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 546 | +0.215 | +18.79€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 37 | +0.090 | -0.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 173 | +0.094 | -33.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 146 | +0.115 | -18.23€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 9 | +0.021 | -2.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 9 | +0.021 | -2.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 39 | +0.305 | +2.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 39 | +0.305 | +2.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 39 | +0.305 | +2.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 39 | +0.305 | +2.14€ | 0 | 0 |
| ✅ GBM_LATE_15M | 4903 | +0.101 | +1512.32€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 4903 | +0.101 | +1512.32€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1176 | +0.062 | +162.61€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1176 | +0.062 | +162.61€ | 0 | 2 |
| ✅ GBM_LATE_15M#ETH | 1172 | +0.073 | +192.69€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1172 | +0.073 | +192.69€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1313 | +0.108 | +523.79€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1313 | +0.108 | +523.79€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1242 | +0.157 | +633.23€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1242 | +0.157 | +633.23€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2118 | +0.147 | +1215.63€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2118 | +0.147 | +1215.63€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 543 | +0.107 | +260.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 543 | +0.107 | +260.53€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 562 | +0.122 | +249.09€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 562 | +0.122 | +249.09€ | 0 | 14 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 570 | +0.110 | +246.83€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 570 | +0.110 | +246.83€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 443 | +0.273 | +459.18€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 443 | +0.273 | +459.18€ | 0 | 10 |
...
| ✅ GBM_LATE_15M_TARDIO | 2102 | +0.090 | +626.34€ | 0 | 6 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2102 | +0.090 | +626.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 520 | +0.054 | +84.42€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 520 | +0.054 | +84.42€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 488 | +0.018 | +7.10€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 488 | +0.018 | +7.10€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 494 | +0.062 | +98.40€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 494 | +0.062 | +98.40€ | 2 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 600 | +0.203 | +436.42€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 600 | +0.203 | +436.42€ | 0 | 10 |
...
| ✅ LEADLAG_BTC_XRP_15M | 145 | +0.071 | +21.98€ | 1 | 5 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 145 | +0.071 | +21.98€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 145 | +0.071 | +21.98€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 145 | +0.071 | +21.98€ | 1 | 5 |
...
| ✅ UPDOWN_GBM | 1602 | +0.010 | +75.05€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1344 | +0.036 | +112.08€ | 1 | 6 |
...
| ✅ UPDOWN_GBM#BTC | 414 | +0.026 | +29.55€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 333 | +0.064 | +42.50€ | 1 | 10 |
...
| ✅ UPDOWN_GBM#ETH | 770 | +0.018 | +36.43€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 677 | +0.035 | +47.49€ | 2 | 3 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 133 | +0.174 | +34.41€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 133 | +0.174 | +34.41€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 7 | -0.019 | +2.23€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 7 | -0.019 | +2.23€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 20 | +0.091 | +3.78€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 20 | +0.091 | +3.78€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 42 | +0.182 | +4.38€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 42 | +0.182 | +4.38€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.157: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.004 n=504 | neutral: IC=-0.020 n=373 | overbought(IBS>0.7): IC=+0.137 n=411
  - _Datos_: n=1362 IC=+0.044 PNL=+135.14€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9910 | tras_1loss IC=+0.069 n=6917 | tras_2loss IC=+0.039 n=2950/40 | gap=+0.063 (umbral 0.05)
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=+0.079 n=747/40 PNL=+174.63€ | H=15h UTC: IC=+0.128 n=699/40 PNL=+233.96€ | H=17h UTC: IC=+0.129 n=653/40 PNL=+194.31€ | H=19h UTC: IC=+0.106 n=620/40 PNL=+127.10€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1475 IC=+0.017 PNL=+89.05€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1475 IC=+0.017 PNL=+89.05€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=706 IC=+0.027 PNL=+48.11€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=706 IC=+0.027 PNL=+48.11€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=333 IC=+0.064 PNL=+42.50€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=333 IC=+0.064 PNL=+42.50€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=713 IC=+0.075 PNL=+96.22€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=713 IC=+0.075 PNL=+96.22€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=603 IC=+0.032 PNL=+40.13€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=603 IC=+0.032 PNL=+40.13€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=14579 IC=+0.105 PNL=+3534.87€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=14579 IC=+0.105 PNL=+3534.87€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=157 IC=+0.016 PNL=+9.96€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=157 IC=+0.016 PNL=+9.96€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=854 IC=-0.004 PNL=+18.58€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=854 IC=-0.004 PNL=+18.58€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.158 > 0.1 con n=413 PNL=+137.68€
  - _Datos_: n=413 IC=+0.158 PNL=+137.68€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=630 IC=+0.054 PNL=+69.86€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=630 IC=+0.054 PNL=+69.86€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.145 > 0.08 con n=302 PNL=+94.35€
  - _Datos_: n=302 IC=+0.145 PNL=+94.35€
...
**🟡 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.203 > 0.08 con n=136 PNL=+60.69€
  - _Datos_: n=136 IC=+0.203 PNL=+60.69€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.246 > 0.2 con n=356 PNL=+245.09€
  - _Datos_: n=356 IC=+0.246 PNL=+245.09€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.125 > 0.08 con n=166 PNL=+44.57€
  - _Datos_: n=166 IC=+0.125 PNL=+44.57€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.110 > 0.08 con n=157 PNL=+58.99€
  - _Datos_: n=157 IC=+0.110 PNL=+58.99€
...
**🔶 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.171 n=208) pero sin cruzar ≥2 pares más — BTC: n=152 IC=-0.019; ETH: n=114 IC=+0.000; SOL: n=220 IC=+0.063 ✓
  - _Datos_: n=208 IC=+0.171 PNL=+136.40€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.204 > 0.08 con n=133 PNL=+3.00€
  - _Datos_: n=133 IC=+0.204 PNL=+3.00€
