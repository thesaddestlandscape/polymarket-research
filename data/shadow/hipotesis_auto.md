# Hipótesis automáticas — 2026-09-04 03:18 UTC
_Generado por shadow_postmortem.py sobre 275255 resoluciones (PNL=+26851.12€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.62` → IC=-0.299 (n=3709)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.62
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=11127)
...
- **FILTRO** `restante_s_al_confirmar` < `142.85` → IC=-0.315 (n=823)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 142.85
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=2471)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `156.96` → IC=-0.379 (n=896)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 156.96
  - _Potencial_: sin este filtro IC_bueno=-0.102 (n=1820)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.189 (n=8029)
...
- **PATRÓN** `libro_liquidez` > `2371.8427` → IC=+0.170 (n=1944)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.152 (n=6512)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 7.0 (IC base=+0.140)

- **PATRÓN** `py_entrada` < `0.34` → IC=+0.252 (n=4912)
...
- **PATRÓN** `libro_liquidez` > `1931.6406` → IC=+0.174 (n=3300)
  ...
  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 1931.6406 (IC base=+0.140)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.140 (n=1378)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.195 (n=5210)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` < 15.0 (IC base=+0.189)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.202 (n=2946)
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.179 (n=1589)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.238 (n=1186)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.237)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.316 (n=502)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.186 (n=1288)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.207 (n=17503)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.148 (n=2431)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 12.0 (IC base=+0.147)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.176 (n=2578)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.179 (n=3022)
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.237 (n=1508)
...
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.204 (n=2466)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.38 (IC base=+0.133)

- **PATRÓN** `restante_min` < `4.01` → IC=+0.147 (n=2324)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.150 (n=3400)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.210 (n=1246)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.162 (n=1680)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.138 (n=1720)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3569` → IC=+0.127 (n=3685)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.606` → IC=+0.149 (n=1408)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 5.606 (IC base=+0.080)

- **PATRÓN** `volumen_regimen` < `0.6962` → IC=+0.223 (n=370)
...
- **PATRÓN** `volumen_spike_ratio` < `2.8793` → IC=+0.173 (n=2207)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 2.8793 (IC base=+0.080)

- **PATRÓN** `volumen_spike_ratio` > `2.4208` → IC=+0.171 (n=1001)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.4208 (IC base=+0.080)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.197 (n=1105)
...
- **PATRÓN** `volumen_spike_ratio` < `2.6736` → IC=+0.138 (n=724)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` < 2.6736 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` > `1.452` → IC=+0.145 (n=723)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.452 (IC base=+0.153)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.189 (n=657)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0974` → IC=+0.238 (n=216)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0974 (IC base=+0.210)
...
- **PATRÓN** `dist_vwap_pct` > `0.1844` → IC=+0.221 (n=413)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1844 (IC base=+0.210)

- **PATRÓN** `dist_vwap_pct` < `0.4813` → IC=+0.213 (n=580)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4813 (IC base=+0.210)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.257` → IC=+0.225 (n=311)
...
- **PATRÓN** `volumen_regimen` < `1.2608` → IC=+0.214 (n=646)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1` → IC=+0.216 (n=614)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1 (IC base=+0.210)

- **PATRÓN** `volumen_spike_ratio` < `1.4924` → IC=+0.243 (n=274)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4924 (IC base=+0.210)

- **PATRÓN** `libro_liquidez` > `11847.0948` → IC=+0.239 (n=577)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.200 (n=291)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **FILTRO** `ibs_20min` > `0.7917` → IC=-0.188 (n=412)
...
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.172 (n=1518)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0094 (IC base=+0.071)

- **PATRÓN** `ibs_20min` > `0.2908` → IC=+0.139 (n=4551)
...
- **PATRÓN** `volumen_regimen` > `0.6809` → IC=+0.217 (n=1320)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6809 (IC base=+0.071)

- **PATRÓN** `volumen_pendiente_norm` < `0.115` → IC=+0.213 (n=2151)
...
- **FILTRO** `sigma_ewma_delta_pct` > `2.449` → IC=-0.161 (n=328)
...
- **PATRÓN** `sigma_h` > `0.0132` → IC=+0.333 (n=494)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.258 (n=353)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.249)

- **PATRÓN** `ibs_20min` > `0.898` → IC=+0.323 (n=494)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.898 (IC base=+0.249)

- **PATRÓN** `dist_vwap_pct` > `0.1617` → IC=+0.313 (n=436)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1617 (IC base=+0.249)
...
- **PATRÓN** `volumen_regimen` > `0.8427` → IC=+0.290 (n=494)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8427 (IC base=+0.249)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5739` → IC=+0.257 (n=681)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5739 (IC base=+0.249)

- **PATRÓN** `volumen_spike_ratio` > `1.844` → IC=+0.254 (n=454)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.844 (IC base=+0.249)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.254 (n=864)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.249)

- **PATRÓN** `libro_liquidez` > `2467.9726` → IC=+0.253 (n=662)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2467.9726 (IC base=+0.249)
...
- **PATRÓN** `sigma_h` > `0.0106` → IC=+0.208 (n=1284)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0106 (IC base=+0.162)

- **PATRÓN** `drift_60min` |x|≤ `0.3419` → IC=+0.170 (n=3381)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.283 (n=1758)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` > `0.7901` → IC=+0.236 (n=848)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7901 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.004` → IC=+0.248 (n=808)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.004 (IC base=+0.162)

- **PATRÓN** `volumen_regimen` > `0.6285` → IC=+0.165 (n=2598)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` > 0.6285 (IC base=+0.162)

- **PATRÓN** `volumen_pendiente_norm` > `0.1059` → IC=+0.183 (n=1432)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.1059 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` < `2.324` → IC=+0.162 (n=3145)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.324 (IC base=+0.162)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.165 (n=3927)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.03 (IC base=+0.162)

- **PATRÓN** `libro_liquidez` > `3932.3605` → IC=+0.177 (n=1281)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 3932.3605 (IC base=+0.162)

- **PATRÓN** `ballena_activa_n` < `143.0` → IC=+0.179 (n=2757)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.230 (n=246)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.172)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.292 (n=301)
...
- **PATRÓN** `volumen_spike_ratio` < `2.6391` → IC=+0.161 (n=562)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.6391 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` > `1.4564` → IC=+0.167 (n=562)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.4564 (IC base=+0.172)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.204 (n=528)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.172)

- **PATRÓN** `ballena_activa_n` < `55.0` → IC=+0.185 (n=325)

  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 55.0 (IC base=+0.172)
...
- **PATRÓN** `drift_60min` |x|≤ `0.415` → IC=+0.172 (n=568)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.4744` → IC=+0.209 (n=568)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.906` → IC=+0.227 (n=159)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.906 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.405` → IC=+0.167 (n=541)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` < 7.405 (IC base=+0.167)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4814` → IC=+0.203 (n=237)
...
- **PATRÓN** `libro_liquidez` > `11309.5324` → IC=+0.190 (n=508)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2836` → IC=+0.188 (n=527)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.2836 (IC base=+0.170)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.183 (n=203)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 17.0 (IC base=+0.170)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.192 (n=209)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 5.0 (IC base=+0.170)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.303 (n=307)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1304` → IC=+0.171 (n=217)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.1304 (IC base=+0.170)

- **PATRÓN** `volumen_spike_ratio` > `1.7062` → IC=+0.162 (n=537)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.7062 (IC base=+0.170)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.194 (n=605)
...
- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.155 (n=648)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.5385` → IC=+0.194 (n=721)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.259` → IC=+0.272 (n=160)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.259 (IC base=+0.115)

- **PATRÓN** `volumen_regimen` > `0.6285` → IC=+0.125 (n=721)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` > 0.6285 (IC base=+0.115)

- **PATRÓN** `volumen_pendiente_norm` < `0.1685` → IC=+0.135 (n=718)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_pendiente_norm` < 0.1685 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` < `1.4403` → IC=+0.149 (n=229)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.4403 (IC base=+0.115)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.122 (n=725)

  - _Acción_: Kelly boost +0.61€ cuando `libro_spread` < 0.02 (IC base=+0.115)

- **PATRÓN** `libro_liquidez` > `3182.8788` → IC=+0.220 (n=241)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1722` → IC=+0.209 (n=325)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1722 (IC base=+0.190)
...
- **PATRÓN** `ibs_20min` > `0.8851` → IC=+0.271 (n=492)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8851 (IC base=+0.190)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.219` → IC=+0.238 (n=365)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.219 (IC base=+0.190)

- **PATRÓN** `volumen_regimen` > `0.6832` → IC=+0.206 (n=659)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6832 (IC base=+0.190)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5276` → IC=+0.209 (n=698)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5276 (IC base=+0.190)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.193 (n=848)
...
- **PATRÓN** `ibs_20min` > `0.4545` → IC=+0.168 (n=808)
...
- **PATRÓN** `volumen_regimen` < `0.8754` → IC=+0.157 (n=435)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.18` → IC=+0.170 (n=225)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` > 0.18 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` < `2.2724` → IC=+0.147 (n=663)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 2.2724 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `1.8529` → IC=+0.143 (n=502)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 1.8529 (IC base=+0.137)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.156 (n=605)

  - _Acción_: Kelly boost +0.78€ cuando `libro_spread` < 0.01 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `7625.2292` → IC=+0.184 (n=270)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 7625.2292 (IC base=+0.137)
...
- **PATRÓN** `sigma_h` < `0.0053` → IC=+0.268 (n=136)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` > `0.4001` → IC=+0.281 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4001 (IC base=+0.257)

- **PATRÓN** `dist_vwap_pct` > `0.1633` → IC=+0.290 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1633 (IC base=+0.257)

- **PATRÓN** `dist_vwap_pct` < `0.7834` → IC=+0.266 (n=224)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.7834 (IC base=+0.257)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.325` → IC=+0.319 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.325 (IC base=+0.257)

- **PATRÓN** `volumen_regimen` < `0.8463` → IC=+0.268 (n=136)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8463 (IC base=+0.257)

- **PATRÓN** `volumen_regimen` > `1.1605` → IC=+0.314 (n=68)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1605 (IC base=+0.257)

- **PATRÓN** `volumen_pendiente_norm` > `0.1004` → IC=+0.349 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1004 (IC base=+0.257)

- **PATRÓN** `volumen_spike_ratio` < `1.3759` → IC=+0.297 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3759 (IC base=+0.257)

- **PATRÓN** `volumen_spike_ratio` > `2.0848` → IC=+0.326 (n=90)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.200 (n=2091)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.301 (n=1579)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.2326` → IC=+0.200 (n=1725)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2326 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.639` → IC=+0.222 (n=2587)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.639 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` < `0.8842` → IC=+0.158 (n=2126)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.8842 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` > `1.2355` → IC=+0.154 (n=1063)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.2355 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.1676` → IC=+0.188 (n=1248)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.1676 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` > `1.8759` → IC=+0.172 (n=2853)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.8759 (IC base=+0.164)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.168 (n=4348)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.02 (IC base=+0.164)

- **PATRÓN** `libro_liquidez` > `3847.9652` → IC=+0.187 (n=1536)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3847.9652 (IC base=+0.164)

- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.211 (n=2167)
...
- **PATRÓN** `ibs_20min` > `0.3257` → IC=+0.205 (n=755)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.417` → IC=+0.173 (n=658)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` < 4.417 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` < `1.2673` → IC=+0.167 (n=755)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1475` → IC=+0.184 (n=207)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.1475 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` < `2.4168` → IC=+0.178 (n=706)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 2.4168 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` > `1.7262` → IC=+0.180 (n=470)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` > 1.7262 (IC base=+0.160)

- **PATRÓN** `libro_liquidez` > `10653.1142` → IC=+0.185 (n=675)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 10653.1142 (IC base=+0.160)
...
- **PATRÓN** `sigma_h` > `0.0074` → IC=+0.227 (n=441)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0074 (IC base=+0.204)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.239 (n=316)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.204)

- **PATRÓN** `ibs_20min` > `0.6815` → IC=+0.252 (n=591)
...
- **PATRÓN** `ibs_20min` > `0.7397` → IC=+0.241 (n=507)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7397 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `0.3748` → IC=+0.191 (n=309)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.3748 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.369` → IC=+0.173 (n=347)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 4.369 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` < `0.9021` → IC=+0.172 (n=508)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 0.9021 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` > `1.2073` → IC=+0.160 (n=254)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 1.2073 (IC base=+0.146)

- **PATRÓN** `volumen_pendiente_norm` > `0.2748` → IC=+0.227 (n=119)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2748 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` > `2.1479` → IC=+0.203 (n=328)
...
- **PATRÓN** `ibs_20min` > `0.48` → IC=+0.181 (n=795)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` > 0.48 (IC base=+0.095)

- **PATRÓN** `dist_vwap_pct` > `1.1408` → IC=+0.197 (n=153)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 1.1408 (IC base=+0.095)
...
- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.139 (n=574)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.128` → IC=+0.243 (n=437)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.128 (IC base=+0.198)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.207 (n=1006)
...
### LEADLAG_BTC_XRP_15M
- **PATRÓN** `libro_liquidez` > `2528.2704` → IC=+0.132 (n=169)

  - _Acción_: Kelly boost +0.66€ cuando `libro_liquidez` > 2528.2704 (IC base=+0.093)
...
- **FILTRO** `py_entrada` < `0.45` → IC=-0.183 (n=1371)

  - _Acción_: SKIP cuando `py_entrada` < 0.45
  - _Potencial_: sin este filtro IC_bueno=+0.024 (n=4455)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.182 (n=1483)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=4595)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.41` → IC=-0.219 (n=222)

  - _Acción_: SKIP cuando `py_entrada` < 0.41
  - _Potencial_: sin este filtro IC_bueno=+0.054 (n=675)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=35)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=933)
...
- **FILTRO** `py_entrada` > `0.65` → IC=-0.215 (n=240)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.023 (n=787)

- **FILTRO** `ibs_20min` > `0.7151` → IC=-0.190 (n=256)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7151
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=771)
...
- **FILTRO** `py_entrada` > `0.56` → IC=-0.167 (n=235)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=905)
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.282 (n=3510)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=10729)

- **FILTRO** `ibs_7min` < `0.7115` → IC=-0.246 (n=3559)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7115
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=10680)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.171 (n=4760)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=9479)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.220 (n=4412)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=13561)

- **FILTRO** `ibs_7min` > `0.7143` → IC=-0.173 (n=4475)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=13498)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.152 (n=616)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.100 (n=1458)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.315 (n=511)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.135 (n=589)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
...
- **FILTRO** `py_entrada` < `0.36` → IC=-0.259 (n=648)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=2011)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.183 (n=671)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
...
- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.252 (n=494)
  ...
  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=1613)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.225 (n=750)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=2473)

- **FILTRO** `ibs_7min` > `0.7964` → IC=-0.172 (n=805)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7964
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=2418)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.139 (n=731)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.259 (n=588)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=1825)

- **FILTRO** `ibs_7min` < `0.7527` → IC=-0.194 (n=603)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7527
  - _Potencial_: sin este filtro IC_bueno=-0.077 (n=1810)
...
- **FILTRO** `ibs_7min` < `0.7317` → IC=-0.215 (n=661)
...
- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.192 (n=632)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=2022)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.122 (n=758)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=1574)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.295 (n=570)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=1762)

- **FILTRO** `ibs_7min` < `0.7333` → IC=-0.244 (n=583)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7333
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=1749)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.232 (n=566)
...
### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` > `0.0053` → IC=+0.153 (n=355)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0053 (IC base=+0.127)

- **PATRÓN** `drift_60min` |x|≤ `0.1677` → IC=+0.130 (n=468)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.65€ cuando `drift_60min` |x|≤ 0.1677 (IC base=+0.127)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0596` → IC=+0.129 (n=531)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.64€ cuando `delta_ratio_macro` |x|> 0.0596 (IC base=+0.127)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.128 (n=563)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 4.0 (IC base=+0.127)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.158 (n=241)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` < 6.0 (IC base=+0.127)

- **PATRÓN** `ibs_15` > `0.5833` → IC=+0.215 (n=531)
...
- **PATRÓN** `dist_vwap_pct` < `0.0989` → IC=+0.130 (n=325)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.0989 (IC base=+0.127)

- **PATRÓN** `sigma_ewma_delta_pct` > `15.362` → IC=+0.223 (n=153)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 15.362 (IC base=+0.127)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.136 (n=545)

  - _Acción_: Kelly boost +0.68€ cuando `libro_spread` < 0.01 (IC base=+0.127)

- **PATRÓN** `libro_liquidez` > `3038.6807` → IC=+0.149 (n=354)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 3038.6807 (IC base=+0.127)

### UPDOWN_GBM#5min
- **FILTRO** `sigma_ewma_delta_pct` > `6.435` → IC=-0.205 (n=59)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 6.435
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=823)
...
- **PATRÓN** `ibs_15` < `0.125` → IC=+0.171 (n=165)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.85€ cuando `ibs_15` < 0.125 (IC base=+0.034)
...
- **FILTRO** `sigma_h` > `0.0074` → IC=-0.237 (n=226)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.188 (n=681)

- **FILTRO** `sigma_h` < `0.0038` → IC=-0.231 (n=299)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0038
  - _Potencial_: sin este filtro IC_bueno=-0.185 (n=608)

- **FILTRO** `drift_15min` |x|> `0.7355` → IC=-0.202 (n=226)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 0.7355
  - _Potencial_: sin este filtro IC_bueno=-0.200 (n=681)

- **FILTRO** `sigma_ewma_delta_pct` > `19.475` → IC=-0.250 (n=166)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 19.475
  - _Potencial_: sin este filtro IC_bueno=-0.189 (n=741)

- **FILTRO** `libro_liquidez` < `14687.2309` → IC=-0.210 (n=453)

  - _Acción_: SKIP cuando `libro_liquidez` < 14687.2309
...
### UPDOWN_GBM_IBS_ALTO
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.283 (n=334)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.0553` → IC=+0.325 (n=112)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0553 (IC base=+0.282)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1322` → IC=+0.299 (n=222)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **PATRÓN** `ibs_15` > `0.8343` → IC=+0.318 (n=333)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **PATRÓN** `sigma_ewma_delta_pct` < `7.231` → IC=+0.284 (n=267)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 7.231 (IC base=+0.282)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.288 (n=408)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.5833 sube el IC de +0.127 a +0.215 en UPDOWN_GBM#15min (n=531). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.125 sube el IC de +0.034 a +0.171 en UPDOWN_GBM#XRP#15min (n=165). Ya aplicado como kelly_boost=+0.85€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.8343 sube el IC de +0.282 a +0.318 en UPDOWN_GBM_IBS_ALTO (n=333). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 14836 | -0.112 | -2514.39€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 957 | -0.010 | -140.51€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 13879 | -0.119 | -2373.88€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#DOGE | 1980 | -0.170 | -542.16€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 1980 | -0.170 | -542.16€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 3962 | -0.050 | -369.46€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 3962 | -0.050 | -369.46€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 3294 | -0.128 | -289.24€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 3294 | -0.128 | -289.24€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 2716 | -0.194 | -749.06€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 2716 | -0.194 | -749.06€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 4120 | -0.072 | +1805.27€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 1172 | -0.009 | +852.26€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 2948 | -0.096 | +953.02€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 4120 | -0.072 | +1805.27€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 1172 | -0.009 | +852.26€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 2948 | -0.096 | +953.02€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 47785 | +0.113 | -3037.17€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 7985 | +0.184 | -289.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 162 | -0.110 | -54.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 35888 | +0.098 | -2619.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 
 | 0 | 0 |
...
| ✅ UPDOWN_GBM | 9769 | +0.009 | +274.95€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 3445 | +0.038 | +329.61€ | 0 | 11 |
| ✅ UPDOWN_GBM#240min | 413 | +0.008 | +6.41€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 5246 | -0.007 | -59.85€ | 1 | 0 |
...
| ✅ UPDOWN_GBM#BTC | 1894 | +0.015 | +96.13€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 390 | +0.066 | +68.57€ | 2 | 12 |
...
| ✅ UPDOWN_GBM#SOL | 2825 | +0.005 | +19.26€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 901 | +0.006 | +15.64€ | 1 | 7 |
| ✅ UPDOWN_GBM#SOL#240min | 109 | -0.004 | -2.83€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 1675 | +0.007 | +4.53€ | 3 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 128 | +0.000 | +2.05€ | 1 | 2 |
| ✅ UPDOWN_GBM#SOL#daily | 12 | -0.129 | -0.13€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 1575 | +0.015 | +109.76€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 844 | +0.051 | +140.27€ | 0 | 9 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 5622 | -0.065 | +1300.50€ | 3 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 5622 | -0.065 | +1300.50€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 307 | -0.050 | +341.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 307 | -0.050 | +341.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 1162 | -0.153 | -57.32€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 1162 | -0.153 | -57.32€ | 6 | 4 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 1830 | -0.061 | +380.76€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 1830 | -0.061 | +380.76€ | 4 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 1720 | -0.084 | +372.64€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 1720 | -0.084 | +372.64€ | 4 | 4 |
...
| ✅ UPDOWN_GBM_IBS_ALTO | 444 | +0.282 | +345.28€ | 0 | 11 |
| ✅ UPDOWN_GBM_IBS_ALTO#15min | 444 | +0.282 | +345.28€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.086) — sin ventaja clara. oversold(IBS<0.3): IC=+0.025 n=3487 | neutral: IC=+0.003 n=3802 | overbought(IBS>0.7): IC=+0.088 n=3803
  - _Datos_: n=11534 IC=+0.040 PNL=+1167.92€

**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  - _Umbral_: n≥40 por celda + gate riguroso completo (Wilson+shuffle+PnL bootstrap)
  - _Acción_: Añadir claves 'ESTRATEGIA#SUBTYPE#DIRECCION#HORA':1.2 a meta.hora_boost_factor, solo por celda confirmada
  - _Estado_: (cache 626s) 177 celda(s) GATE OK de 2462 trackeadas

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.006 < 0.08 — monitorear
  - _Datos_: n=901 IC=+0.006 PNL=+15.64€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.066 n=153870 | tras_1loss IC=+0.051 n=121103 | tras_2loss IC=+0.015 n=54323/40 | gap=+0.051 (umbral 0.05)
...
**⏳ H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  ...
  - _Estado_: 9707 ops, 22 horas distintas. Sin hora con n≥15 y IC extremo aún.
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=9492 IC=+0.006 PNL=+221.33€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=9492 IC=+0.006 PNL=+221.33€
...
**🟡 H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.127 > 0.1 con n=708 PNL=+216.20€
  - _Datos_: n=708 IC=+0.127 PNL=+216.20€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=390 IC=+0.066 PNL=+68.57€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=390 IC=+0.066 PNL=+68.57€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=2009 IC=+0.031 PNL=+174.64€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=2009 IC=+0.031 PNL=+174.64€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=2603 IC=-0.018 PNL=-46.08€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=2603 IC=-0.018 PNL=-46.08€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=2552 IC=+0.014 PNL=+106.59€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=2552 IC=+0.014 PNL=+106.59€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=1350 IC=+0.027 PNL=+72.14€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1350 IC=+0.027 PNL=+72.14€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=2375 IC=+0.030 PNL=+186.86€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2375 IC=+0.030 PNL=+186.86€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.163 > 0.1 con n=1049 PNL=+396.67€
  - _Datos_: n=1049 IC=+0.163 PNL=+396.67€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=657 IC=+0.034 PNL=+71.81€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=657 IC=+0.034 PNL=+71.81€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.128 > 0.08 con n=670 PNL=+201.83€
  - _Datos_: n=670 IC=+0.128 PNL=+201.83€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=2198 IC=+0.138 PNL=+1269.20€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=2198 IC=+0.138 PNL=+1269.20€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.182 > 0.08 con n=850 PNL=+560.69€
  - _Datos_: n=850 IC=+0.182 PNL=+560.69€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=4544 IC=+0.147 PNL=-727.92€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=4544 IC=+0.147 PNL=-727.92€
