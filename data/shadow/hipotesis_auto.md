# Hipótesis automáticas — 2026-09-03 17:26 UTC
_Generado por shadow_postmortem.py sobre 270002 resoluciones (PNL=+25268.13€)_
...
- **FILTRO** `py_entrada` > `0.5` → IC=-0.392 (n=81)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.124 (n=275)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.124 (n=275)

  - _Acción_: Kelly boost +0.62€ cuando `py_entrada` < 0.5 (IC base=+0.006)

### BALLENAS_CONFIRMADAS_15M#ETH#15min
- **FILTRO** `py_entrada` > `0.795` → IC=-0.385 (n=50)
...
- **PATRÓN** `py_entrada` < `0.475` → IC=+0.131 (n=82)
  ...
  - _Acción_: Kelly boost +0.65€ cuando `py_entrada` < 0.475 (IC base=-0.015)
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.53` → IC=-0.297 (n=3654)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.53
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=10963)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `142.06` → IC=-0.281 (n=473)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 142.06
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=1420)
...
- **FILTRO** `restante_s_al_confirmar` < `113.97` → IC=-0.401 (n=483)
...
- **FILTRO** `restante_s_al_confirmar` < `141.74` → IC=-0.314 (n=811)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=2433)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `156.16` → IC=-0.378 (n=883)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 156.16
  - _Potencial_: sin este filtro IC_bueno=-0.102 (n=1793)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.192 (n=7797)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.147 (n=5464)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.34` → IC=+0.254 (n=4851)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.34 (IC base=+0.141)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.185 (n=3813)

  - _Acción_: Kelly boost +0.93€ cuando `libro_spread` < 0.02 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `1928.3531` → IC=+0.177 (n=3255)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 1928.3531 (IC base=+0.141)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.199 (n=822)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.365` → IC=+0.262 (n=791)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.130 (n=657)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.595` → IC=+0.166 (n=291)
...
- **PATRÓN** `libro_liquidez` > `3855.969` → IC=+0.148 (n=347)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 3855.969 (IC base=+0.129)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.144 (n=1621)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.267 (n=615)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.2` → IC=+0.411 (n=224)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.2 (IC base=+0.260)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.261 (n=698)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.260)

- **PATRÓN** `libro_liquidez` > `1939.4936` → IC=+0.270 (n=666)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1939.4936 (IC base=+0.260)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.126 (n=532)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.197 (n=6080)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.204 (n=2891)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.176 (n=1508)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.177 (n=1471)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 6.0 (IC base=+0.172)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.177 (n=1307)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` < 15.0 (IC base=+0.172)

- **PATRÓN** `py_entrada` < `0.74` → IC=+0.176 (n=1579)

  - _Acción_: Kelly boost +0.88€ cuando `py_entrada` < 0.74 (IC base=+0.172)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.182 (n=1049)

  - _Acción_: Kelly boost +0.91€ cuando `py_entrada` > 0.72 (IC base=+0.172)

### FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.247 (n=1384)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.197 (n=1500)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.199 (n=7073)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 17.0 (IC base=+0.189)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.208 (n=17084)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.71 (IC base=+0.189)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.152 (n=3508)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 5.0 (IC base=+0.147)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.147 (n=2400)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 12.0 (IC base=+0.147)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.177 (n=2495)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.260 (n=2187)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.182 (n=2943)

  - _Acción_: Kelly boost +0.91€ cuando `py_entrada` > 0.71 (IC base=+0.163)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.236 (n=1489)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.274 (n=1062)
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.242 (n=1457)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.74 (IC base=+0.200)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.193 (n=1194)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `4.16` → IC=+0.155 (n=2282)

  - _Acción_: Kelly boost +0.77€ cuando `lag_apertura_s` < 4.16 (IC base=+0.132)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.211 (n=1230)
...
- **PATRÓN** `restante_min` < `4.43` → IC=+0.133 (n=1517)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.131 (n=3451)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3559` → IC=+0.129 (n=3590)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` > 0.3559 (IC base=+0.080)

- **PATRÓN** `dist_vwap_pct` > `0.1712` → IC=+0.223 (n=500)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1712 (IC base=+0.080)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.556` → IC=+0.152 (n=1374)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 5.556 (IC base=+0.080)

- **PATRÓN** `volumen_regimen` < `0.6313` → IC=+0.221 (n=270)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6313 (IC base=+0.080)

- **PATRÓN** `volumen_regimen` > `1.0907` → IC=+0.248 (n=367)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0907 (IC base=+0.080)

- **PATRÓN** `volumen_pendiente_norm` > `0.1085` → IC=+0.181 (n=790)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1085 (IC base=+0.080)

- **PATRÓN** `volumen_spike_ratio` < `2.8852` → IC=+0.176 (n=2133)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 2.8852 (IC base=+0.080)

- **PATRÓN** `volumen_spike_ratio` > `1.4763` → IC=+0.172 (n=2133)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.4763 (IC base=+0.080)

- **PATRÓN** `ballena_activa_n` < `55.0` → IC=+0.202 (n=1060)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 55.0 (IC base=+0.080)

- **PATRÓN** `ibs_20min` < `0.4071` → IC=+0.127 (n=3467)

  - _Acción_: Kelly boost +0.63€ cuando `ibs_20min` < 0.4071 (IC base=+0.040)

- **PATRÓN** `dist_vwap_pct` < `0.3354` → IC=+0.144 (n=1262)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.3354 (IC base=+0.040)

- **PATRÓN** `volumen_regimen` < `0.686` → IC=+0.152 (n=535)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.686 (IC base=+0.040)

- **PATRÓN** `volumen_pendiente_norm` > `0.3036` → IC=+0.245 (n=269)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3036 (IC base=+0.040)

- **PATRÓN** `volumen_spike_ratio` > `2.8566` → IC=+0.214 (n=603)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8566 (IC base=+0.040)

- **PATRÓN** `ballena_activa_n` < `129.0` → IC=+0.213 (n=1366)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 129.0 (IC base=+0.040)

### GBM_LATE_15M#BNB#15min
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.184 (n=267)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0076 (IC base=+0.156)

- **PATRÓN** `drift_60min` |x|≤ `0.2784` → IC=+0.158 (n=799)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.2784 (IC base=+0.156)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.199 (n=294)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.156)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.295 (n=286)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.156)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.581` → IC=+0.304 (n=253)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.581 (IC base=+0.156)

- **PATRÓN** `volumen_pendiente_norm` > `0.2271` → IC=+0.220 (n=141)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2271 (IC base=+0.156)

- **PATRÓN** `volumen_spike_ratio` < `2.6728` → IC=+0.140 (n=701)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 2.6728 (IC base=+0.156)

- **PATRÓN** `volumen_spike_ratio` > `1.4443` → IC=+0.146 (n=701)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.4443 (IC base=+0.156)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.192 (n=634)

  - _Acción_: Kelly boost +0.96€ cuando `libro_spread` < 0.04 (IC base=+0.156)

- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.192 (n=449)

  - _Acción_: Kelly boost +0.96€ cuando `ballena_activa_n` < 62.0 (IC base=+0.156)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.265 (n=356)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0058 (IC base=+0.263)

- **PATRÓN** `sigma_h` > `0.0073` → IC=+0.293 (n=177)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0073 (IC base=+0.263)

- **PATRÓN** `drift_60min` |x|≤ `0.1091` → IC=+0.335 (n=234)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1091 (IC base=+0.263)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.272 (n=484)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.263)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.271 (n=539)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.263)

- **PATRÓN** `ibs_20min` < `0.4048` → IC=+0.289 (n=467)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4048 (IC base=+0.263)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.332` → IC=+0.263 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.332 (IC base=+0.263)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.175` → IC=+0.280 (n=566)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.175 (IC base=+0.263)

- **PATRÓN** `volumen_pendiente_norm` < `0.0697` → IC=+0.272 (n=380)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0697 (IC base=+0.263)

- **PATRÓN** `volumen_pendiente_norm` > `0.2883` → IC=+0.336 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2883 (IC base=+0.263)

- **PATRÓN** `volumen_spike_ratio` > `1.5811` → IC=+0.288 (n=376)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5811 (IC base=+0.263)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.271 (n=521)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.263)

- **PATRÓN** `libro_liquidez` > `1703.72` → IC=+0.284 (n=354)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1703.72 (IC base=+0.263)

- **PATRÓN** `ballena_activa_n` < `63.0` → IC=+0.266 (n=365)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 63.0 (IC base=+0.263)
...
- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.225 (n=209)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0068 (IC base=+0.213)

- **PATRÓN** `drift_60min` |x|≤ `0.0966` → IC=+0.236 (n=210)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0966 (IC base=+0.213)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.236 (n=653)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.213)

- **PATRÓN** `ibs_20min` > `0.9292` → IC=+0.252 (n=284)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9292 (IC base=+0.213)

- **PATRÓN** `dist_vwap_pct` > `0.1813` → IC=+0.222 (n=397)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1813 (IC base=+0.213)

- **PATRÓN** `dist_vwap_pct` < `0.4685` → IC=+0.216 (n=569)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4685 (IC base=+0.213)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.257` → IC=+0.229 (n=304)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.257 (IC base=+0.213)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.068` → IC=+0.216 (n=628)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 7.068 (IC base=+0.213)

- **PATRÓN** `volumen_regimen` < `1.2608` → IC=+0.218 (n=626)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2608 (IC base=+0.213)

- **PATRÓN** `volumen_regimen` > `1.1032` → IC=+0.227 (n=284)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1032 (IC base=+0.213)

- **PATRÓN** `volumen_pendiente_norm` < `0.1007` → IC=+0.217 (n=595)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1007 (IC base=+0.213)

- **PATRÓN** `volumen_spike_ratio` < `2.1084` → IC=+0.229 (n=530)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1084 (IC base=+0.213)

- **PATRÓN** `libro_liquidez` > `11871.8913` → IC=+0.240 (n=559)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11871.8913 (IC base=+0.213)

- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.147 (n=690)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0061 (IC base=+0.138)

- **PATRÓN** `drift_60min` |x|≤ `0.1766` → IC=+0.151 (n=460)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.1766 (IC base=+0.138)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.140 (n=621)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 8.0 (IC base=+0.138)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.140 (n=490)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` < 12.0 (IC base=+0.138)

- **PATRÓN** `ibs_20min` < `0.6573` → IC=+0.166 (n=689)

  - _Acción_: Kelly boost +0.83€ cuando `ibs_20min` < 0.6573 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.1407` → IC=+0.163 (n=582)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.1407 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.553` → IC=+0.218 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.553 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `0.6809` → IC=+0.169 (n=303)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 0.6809 (IC base=+0.138)

- **PATRÓN** `volumen_pendiente_norm` > `0.1506` → IC=+0.211 (n=171)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1506 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` < `2.3943` → IC=+0.149 (n=582)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 2.3943 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` > `1.4058` → IC=+0.154 (n=582)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.4058 (IC base=+0.138)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.139 (n=893)

  - _Acción_: Kelly boost +0.70€ cuando `libro_spread` < 0.01 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `12529.5931` → IC=+0.155 (n=459)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 12529.5931 (IC base=+0.138)

- **PATRÓN** `ballena_activa_n` < `290.0` → IC=+0.155 (n=331)

  - _Acción_: Kelly boost +0.77€ cuando `ballena_activa_n` < 290.0 (IC base=+0.138)

### GBM_LATE_15M#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.219 (n=251)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0094 (IC base=+0.165)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.202 (n=283)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.165)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.266 (n=280)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.299` → IC=+0.267 (n=170)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.299 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` < `0.1321` → IC=+0.167 (n=643)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` < 0.1321 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` < `3.9033` → IC=+0.159 (n=670)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 3.9033 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `1.6798` → IC=+0.174 (n=669)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 1.6798 (IC base=+0.165)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.191 (n=771)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.04 (IC base=+0.165)

- **PATRÓN** `ballena_activa_n` < `17.0` → IC=+0.217 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 17.0 (IC base=+0.165)

- **PATRÓN** `sigma_h` < `0.0102` → IC=+0.250 (n=607)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0102 (IC base=+0.236)

- **PATRÓN** `drift_60min` |x|≤ `0.3699` → IC=+0.242 (n=534)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3699 (IC base=+0.236)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.236 (n=293)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.0417` → IC=+0.295 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0417 (IC base=+0.236)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.545` → IC=+0.300 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.545 (IC base=+0.236)

- **PATRÓN** `volumen_pendiente_norm` > `0.3773` → IC=+0.307 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3773 (IC base=+0.236)

- **PATRÓN** `volumen_spike_ratio` < `1.7088` → IC=+0.256 (n=162)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7088 (IC base=+0.236)

- **PATRÓN** `volumen_spike_ratio` > `2.447` → IC=+0.217 (n=323)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.447 (IC base=+0.236)

- **PATRÓN** `ballena_activa_n` < `52.0` → IC=+0.220 (n=412)
...
- **PATRÓN** `dist_vwap_pct` > `0.3442` → IC=+0.300 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3442 (IC base=-0.039)

- **PATRÓN** `dist_vwap_pct` < `0.1477` → IC=+0.274 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1477 (IC base=-0.039)

- **PATRÓN** `volumen_regimen` < `0.6313` → IC=+0.315 (n=25)
...
- **PATRÓN** `volumen_spike_ratio` > `1.9251` → IC=+0.292 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9251 (IC base=-0.039)

- **PATRÓN** `ballena_activa_n` < `176.0` → IC=+0.262 (n=61)
...
- **FILTRO** `ibs_20min` < `0.2791` → IC=-0.150 (n=58)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.313` → IC=-0.200 (n=201)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=1541)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.172 (n=59)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0066 (IC base=+0.039)

- **PATRÓN** `ibs_20min` > `0.2791` → IC=+0.133 (n=118)

  - _Acción_: Kelly boost +0.67€ cuando `ibs_20min` > 0.2791 (IC base=+0.039)

- **PATRÓN** `dist_vwap_pct` < `0.6887` → IC=+0.300 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6887 (IC base=+0.039)

- **PATRÓN** `volumen_regimen` > `0.7776` → IC=+0.375 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7776 (IC base=+0.039)

- **PATRÓN** `volumen_spike_ratio` < `1.6091` → IC=+0.265 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6091 (IC base=+0.039)

- **PATRÓN** `volumen_pendiente_norm` > `0.1129` → IC=+0.184 (n=17)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.1129 (IC base=-0.059)

### GBM_LATE_15M#XRP#15min
- **FILTRO** `drift_60min` |x|> `0.5305` → IC=-0.161 (n=343)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.5305
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=666)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.181 (n=249)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.077 (n=760)

- **FILTRO** `sigma_h` > `0.0248` → IC=-0.145 (n=404)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0248
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=1214)

- **FILTRO** `ibs_20min` > `0.7965` → IC=-0.190 (n=404)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.885` → IC=-0.144 (n=189)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=1429)
...
- **PATRÓN** `dist_vwap_pct` < `0.2617` → IC=+0.217 (n=210)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2617 (IC base=-0.051)

- **PATRÓN** `volumen_regimen` > `1.0971` → IC=+0.255 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0971 (IC base=-0.051)

- **PATRÓN** `volumen_pendiente_norm` > `0.3286` → IC=+0.222 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3286 (IC base=-0.051)

- **PATRÓN** `volumen_spike_ratio` < `2.339` → IC=+0.182 (n=86)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 2.339 (IC base=-0.051)

- **PATRÓN** `ballena_activa_n` < `20.0` → IC=+0.152 (n=67)

  - _Acción_: Kelly boost +0.76€ cuando `ballena_activa_n` < 20.0 (IC base=-0.051)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.173 (n=1485)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0094 (IC base=+0.070)

- **PATRÓN** `ibs_20min` > `0.2857` → IC=+0.139 (n=4446)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` > 0.2857 (IC base=+0.070)

- **PATRÓN** `dist_vwap_pct` > `1.1758` → IC=+0.294 (n=324)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1758 (IC base=+0.070)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.472` → IC=+0.121 (n=2349)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` > 2.472 (IC base=+0.070)

- **PATRÓN** `volumen_regimen` > `0.6815` → IC=+0.219 (n=1287)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6815 (IC base=+0.070)

- **PATRÓN** `volumen_pendiente_norm` < `0.0874` → IC=+0.215 (n=1989)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0874 (IC base=+0.070)

- **PATRÓN** `volumen_pendiente_norm` > `0.2532` → IC=+0.234 (n=419)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2532 (IC base=+0.070)

- **PATRÓN** `volumen_spike_ratio` < `1.4965` → IC=+0.237 (n=714)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4965 (IC base=+0.070)

- **PATRÓN** `volumen_spike_ratio` > `2.837` → IC=+0.228 (n=714)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.837 (IC base=+0.070)

- **PATRÓN** `ballena_activa_n` < `96.0` → IC=+0.303 (n=1595)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 96.0 (IC base=+0.070)

- **PATRÓN** `ibs_20min` < `0.5848` → IC=+0.131 (n=4314)

  - _Acción_: Kelly boost +0.65€ cuando `ibs_20min` < 0.5848 (IC base=+0.051)

- **PATRÓN** `dist_vwap_pct` > `0.754` → IC=+0.265 (n=202)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.754 (IC base=+0.051)

- **PATRÓN** `dist_vwap_pct` < `0.2278` → IC=+0.222 (n=1055)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2278 (IC base=+0.051)

- **PATRÓN** `volumen_regimen` < `0.7154` → IC=+0.232 (n=486)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7154 (IC base=+0.051)

- **PATRÓN** `volumen_regimen` > `1.236` → IC=+0.246 (n=368)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.236 (IC base=+0.051)

- **PATRÓN** `volumen_pendiente_norm` > `0.2584` → IC=+0.348 (n=255)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2584 (IC base=+0.051)

- **PATRÓN** `volumen_spike_ratio` > `2.433` → IC=+0.273 (n=567)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.433 (IC base=+0.051)

- **PATRÓN** `ballena_activa_n` < `66.0` → IC=+0.270 (n=1094)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 66.0 (IC base=+0.051)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `ibs_20min` < `0.2829` → IC=-0.156 (n=367)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2829
  - _Potencial_: sin este filtro IC_bueno=+0.107 (n=748)

- **FILTRO** `sigma_ewma_delta_pct` > `2.395` → IC=-0.165 (n=317)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 2.395
  - _Potencial_: sin este filtro IC_bueno=+0.029 (n=694)

- **PATRÓN** `ibs_20min` > `0.8162` → IC=+0.230 (n=279)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8162 (IC base=+0.020)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.112` → IC=+0.129 (n=418)
...
- **PATRÓN** `volumen_spike_ratio` < `1.9482` → IC=+0.244 (n=119)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9482 (IC base=+0.020)

- **PATRÓN** `volumen_spike_ratio` > `2.6489` → IC=+0.290 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6489 (IC base=+0.020)

- **PATRÓN** `ballena_activa_n` < `74.0` → IC=+0.353 (n=134)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 74.0 (IC base=+0.020)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **FILTRO** `ibs_20min` > `0.8642` → IC=-0.165 (n=377)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8642
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=1133)

- **PATRÓN** `volumen_spike_ratio` < `1.6989` → IC=+0.129 (n=211)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` < 1.6989 (IC base=-0.017)

- **PATRÓN** `ballena_activa_n` < `290.0` → IC=+0.147 (n=134)

  - _Acción_: Kelly boost +0.74€ cuando `ballena_activa_n` < 290.0 (IC base=-0.017)

- **PATRÓN** `dist_vwap_pct` > `0.531` → IC=+0.143 (n=26)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.531 (IC base=-0.030)

- **PATRÓN** `dist_vwap_pct` < `0.1132` → IC=+0.163 (n=161)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.1132 (IC base=-0.030)

- **PATRÓN** `volumen_regimen` < `0.5824` → IC=+0.212 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.5824 (IC base=-0.030)

- **PATRÓN** `volumen_regimen` > `1.1253` → IC=+0.161 (n=57)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 1.1253 (IC base=-0.030)

- **PATRÓN** `volumen_pendiente_norm` < `0.0647` → IC=+0.151 (n=124)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_pendiente_norm` < 0.0647 (IC base=-0.030)

- **PATRÓN** `volumen_spike_ratio` < `1.7706` → IC=+0.233 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7706 (IC base=-0.030)

- **PATRÓN** `volumen_spike_ratio` > `1.3934` → IC=+0.148 (n=126)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.3934 (IC base=-0.030)

- **PATRÓN** `ballena_activa_n` < `260.0` → IC=+0.174 (n=41)

  - _Acción_: Kelly boost +0.87€ cuando `ballena_activa_n` < 260.0 (IC base=-0.030)

### GBM_LATE_15M_ESPACIO_ATR#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0095` → IC=+0.297 (n=239)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0095 (IC base=+0.223)

- **PATRÓN** `drift_60min` |x|≤ `0.1252` → IC=+0.223 (n=316)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1252 (IC base=+0.223)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.255 (n=267)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.223)

- **PATRÓN** `ibs_20min` > `0.712` → IC=+0.259 (n=640)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.712 (IC base=+0.223)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.299` → IC=+0.297 (n=175)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.299 (IC base=+0.223)

- **PATRÓN** `volumen_pendiente_norm` < `0.1395` → IC=+0.239 (n=604)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1395 (IC base=+0.223)

- **PATRÓN** `volumen_spike_ratio` < `2.4905` → IC=+0.222 (n=422)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.4905 (IC base=+0.223)

- **PATRÓN** `volumen_spike_ratio` > `3.2691` → IC=+0.240 (n=287)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.2691 (IC base=+0.223)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.253 (n=730)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.223)

- **PATRÓN** `sigma_h` > `0.0061` → IC=+0.321 (n=444)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0061 (IC base=+0.308)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.323 (n=353)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.308)

- **PATRÓN** `ibs_20min` < `0.3333` → IC=+0.319 (n=496)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3333 (IC base=+0.308)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.707` → IC=+0.333 (n=154)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.707 (IC base=+0.308)

- **PATRÓN** `volumen_pendiente_norm` > `0.3624` → IC=+0.355 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3624 (IC base=+0.308)

- **PATRÓN** `volumen_spike_ratio` < `3.4973` → IC=+0.307 (n=403)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.4973 (IC base=+0.308)

- **PATRÓN** `volumen_spike_ratio` > `2.4058` → IC=+0.319 (n=269)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4058 (IC base=+0.308)

- **PATRÓN** `libro_liquidez` > `1845.461` → IC=+0.320 (n=165)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1845.461 (IC base=+0.308)

- **PATRÓN** `ballena_activa_n` < `30.0` → IC=+0.287 (n=162)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.155 (n=227)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=489)

- **FILTRO** `ibs_20min` < `0.7622` → IC=-0.146 (n=472)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7622
  - _Potencial_: sin este filtro IC_bueno=+0.089 (n=244)

- **FILTRO** `ibs_20min` > `0.7367` → IC=-0.150 (n=409)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7367
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=794)
...
- **PATRÓN** `dist_vwap_pct` > `0.2815` → IC=+0.190 (n=56)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.2815 (IC base=-0.066)

- **PATRÓN** `volumen_regimen` < `0.9683` → IC=+0.128 (n=84)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` < 0.9683 (IC base=-0.066)

- **PATRÓN** `volumen_pendiente_norm` > `0.1914` → IC=+0.278 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1914 (IC base=-0.066)

- **PATRÓN** `volumen_spike_ratio` < `2.091` → IC=+0.233 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.091 (IC base=-0.066)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **FILTRO** `ibs_20min` < `0.8036` → IC=-0.144 (n=635)

  - _Acción_: SKIP cuando `ibs_20min` < 0.8036
  - _Potencial_: sin este filtro IC_bueno=+0.285 (n=328)

- **FILTRO** `ibs_20min` > `0.7568` → IC=-0.227 (n=276)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7568
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=831)

- **FILTRO** `sigma_ewma_delta_pct` > `4.714` → IC=-0.154 (n=281)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.714
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=826)

- **PATRÓN** `ibs_20min` > `0.9231` → IC=+0.327 (n=241)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9231 (IC base=+0.003)

- **PATRÓN** `dist_vwap_pct` > `0.5648` → IC=+0.318 (n=130)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5648 (IC base=+0.003)

- **PATRÓN** `volumen_regimen` < `0.8696` → IC=+0.240 (n=198)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8696 (IC base=+0.003)

- **PATRÓN** `volumen_regimen` > `1.1593` → IC=+0.292 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1593 (IC base=+0.003)

- **PATRÓN** `volumen_pendiente_norm` < `0.1176` → IC=+0.250 (n=282)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1176 (IC base=+0.003)

- **PATRÓN** `volumen_pendiente_norm` > `0.2328` → IC=+0.263 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2328 (IC base=+0.003)

- **PATRÓN** `volumen_spike_ratio` < `1.4502` → IC=+0.294 (n=95)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4502 (IC base=+0.003)

- **PATRÓN** `ballena_activa_n` < `58.0` → IC=+0.298 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 58.0 (IC base=+0.003)
...
- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.214 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 61.0 (IC base=-0.031)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` > `0.0129` → IC=+0.334 (n=485)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0129 (IC base=+0.252)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.261 (n=642)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.252)

- **PATRÓN** `ibs_20min` > `0.898` → IC=+0.326 (n=485)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.898 (IC base=+0.252)

- **PATRÓN** `dist_vwap_pct` > `0.1579` → IC=+0.319 (n=424)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1579 (IC base=+0.252)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.182` → IC=+0.296 (n=396)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.182 (IC base=+0.252)

- **PATRÓN** `volumen_regimen` > `0.6815` → IC=+0.276 (n=650)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6815 (IC base=+0.252)

- **PATRÓN** `volumen_pendiente_norm` > `0.2413` → IC=+0.296 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2413 (IC base=+0.252)

- **PATRÓN** `volumen_spike_ratio` < `2.5766` → IC=+0.261 (n=667)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5766 (IC base=+0.252)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.254 (n=853)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.252)

- **PATRÓN** `libro_liquidez` > `2472.0173` → IC=+0.256 (n=650)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2472.0173 (IC base=+0.252)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.282 (n=264)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.277)

- **PATRÓN** `sigma_h` > `0.0239` → IC=+0.295 (n=261)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0239 (IC base=+0.277)

- **PATRÓN** `drift_60min` |x|≤ `0.3178` → IC=+0.279 (n=523)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3178 (IC base=+0.277)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.291 (n=376)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.277)

- **PATRÓN** `ibs_20min` < `0.375` → IC=+0.318 (n=784)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.375 (IC base=+0.277)

- **PATRÓN** `dist_vwap_pct` > `0.521` → IC=+0.279 (n=220)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.521 (IC base=+0.277)

- **PATRÓN** `dist_vwap_pct` < `0.2774` → IC=+0.282 (n=745)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2774 (IC base=+0.277)
...
- **PATRÓN** `volumen_regimen` < `0.7185` → IC=+0.282 (n=346)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7185 (IC base=+0.277)

- **PATRÓN** `volumen_regimen` > `1.0971` → IC=+0.301 (n=355)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0971 (IC base=+0.277)

- **PATRÓN** `volumen_pendiente_norm` > `0.2891` → IC=+0.383 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2891 (IC base=+0.277)

- **PATRÓN** `volumen_spike_ratio` > `2.1971` → IC=+0.294 (n=285)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1971 (IC base=+0.277)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` > `0.0106` → IC=+0.209 (n=1253)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0106 (IC base=+0.164)

- **PATRÓN** `drift_60min` |x|≤ `0.3439` → IC=+0.170 (n=3303)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.3439 (IC base=+0.164)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.186 (n=1320)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 17.0 (IC base=+0.164)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.290 (n=1711)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.7694` → IC=+0.242 (n=812)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7694 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.699` → IC=+0.237 (n=1528)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.699 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` > `0.6316` → IC=+0.167 (n=2538)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 0.6316 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.1057` → IC=+0.188 (n=1400)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.1057 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` < `2.3263` → IC=+0.163 (n=3068)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.3263 (IC base=+0.164)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.167 (n=2836)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.01 (IC base=+0.164)

- **PATRÓN** `libro_liquidez` > `3939.7883` → IC=+0.178 (n=1251)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 3939.7883 (IC base=+0.164)

- **PATRÓN** `ballena_activa_n` < `144.0` → IC=+0.183 (n=2667)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 144.0 (IC base=+0.164)

- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.192 (n=3075)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0083 (IC base=+0.180)

- **PATRÓN** `drift_60min` |x|≤ `0.0794` → IC=+0.212 (n=1166)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0794 (IC base=+0.180)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.199 (n=1680)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.4387` → IC=+0.232 (n=3493)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4387 (IC base=+0.180)

- **PATRÓN** `dist_vwap_pct` < `0.228` → IC=+0.174 (n=2669)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.228 (IC base=+0.180)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.957` → IC=+0.215 (n=620)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.957 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` < `1.1795` → IC=+0.170 (n=2649)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 1.1795 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` > `0.6241` → IC=+0.162 (n=2649)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.6241 (IC base=+0.180)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5767` → IC=+0.175 (n=1262)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.5767 (IC base=+0.180)

- **PATRÓN** `volumen_spike_ratio` > `2.6609` → IC=+0.202 (n=956)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6609 (IC base=+0.180)

- **PATRÓN** `ballena_activa_n` < `157.0` → IC=+0.181 (n=2455)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 157.0 (IC base=+0.180)

### GBM_LATE_15M_MULTIHORIZONTE#BNB#15min
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.185 (n=277)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0058 (IC base=+0.175)

- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.198 (n=286)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0072 (IC base=+0.175)

- **PATRÓN** `drift_60min` |x|≤ `0.2824` → IC=+0.197 (n=629)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.2824 (IC base=+0.175)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.225 (n=278)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.175)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.313 (n=287)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.175)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.738` → IC=+0.314 (n=202)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.738 (IC base=+0.175)

- **PATRÓN** `volumen_pendiente_norm` > `0.2249` → IC=+0.252 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2249 (IC base=+0.175)

- **PATRÓN** `volumen_spike_ratio` < `2.6391` → IC=+0.166 (n=546)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.6391 (IC base=+0.175)

- **PATRÓN** `volumen_spike_ratio` > `1.4555` → IC=+0.168 (n=546)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 1.4555 (IC base=+0.175)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.206 (n=511)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.175)

- **PATRÓN** `ballena_activa_n` < `66.0` → IC=+0.191 (n=360)

  - _Acción_: Kelly boost +0.95€ cuando `ballena_activa_n` < 66.0 (IC base=+0.175)

- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.266 (n=352)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0048 (IC base=+0.247)

- **PATRÓN** `drift_60min` |x|≤ `0.159` → IC=+0.311 (n=262)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.159 (IC base=+0.247)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.247 (n=397)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.247)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.263 (n=395)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.247)

- **PATRÓN** `ibs_20min` < `0.2712` → IC=+0.273 (n=346)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2712 (IC base=+0.247)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.664` → IC=+0.260 (n=436)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.664 (IC base=+0.247)

- **PATRÓN** `volumen_pendiente_norm` < `0.0655` → IC=+0.243 (n=278)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0655 (IC base=+0.247)

- **PATRÓN** `volumen_pendiente_norm` > `0.2422` → IC=+0.294 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2422 (IC base=+0.247)

- **PATRÓN** `volumen_spike_ratio` < `1.9249` → IC=+0.249 (n=217)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9249 (IC base=+0.247)

- **PATRÓN** `volumen_spike_ratio` > `2.7651` → IC=+0.264 (n=108)
...
- **PATRÓN** `libro_liquidez` > `1698.3448` → IC=+0.281 (n=263)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1698.3448 (IC base=+0.247)

- **PATRÓN** `ballena_activa_n` < `63.0` → IC=+0.250 (n=254)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 63.0 (IC base=+0.247)

### GBM_LATE_15M_MULTIHORIZONTE#BTC#15min
- **PATRÓN** `sigma_h` < `0.006` → IC=+0.193 (n=486)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.006 (IC base=+0.168)

- **PATRÓN** `drift_60min` |x|≤ `0.4166` → IC=+0.176 (n=553)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.4166 (IC base=+0.168)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.196 (n=563)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 6.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` > `0.4827` → IC=+0.215 (n=552)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4827 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` > `0.1993` → IC=+0.228 (n=398)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1993 (IC base=+0.168)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.906` → IC=+0.234 (n=156)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.906 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` < `0.6419` → IC=+0.179 (n=185)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` < 0.6419 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` > `0.8993` → IC=+0.178 (n=368)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` > 0.8993 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.2311` → IC=+0.209 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2311 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` < `1.4857` → IC=+0.208 (n=231)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4857 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.4598` → IC=+0.167 (n=175)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 2.4598 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `11341.8291` → IC=+0.190 (n=494)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 11341.8291 (IC base=+0.168)
...
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.227 (n=196)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0094 (IC base=+0.173)

- **PATRÓN** `drift_60min` |x|≤ `0.2851` → IC=+0.188 (n=517)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.2851 (IC base=+0.173)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.188 (n=235)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 16.0 (IC base=+0.173)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.193 (n=203)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 5.0 (IC base=+0.173)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.311 (n=299)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.173)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.546` → IC=+0.319 (n=136)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.546 (IC base=+0.173)

- **PATRÓN** `volumen_pendiente_norm` > `0.1304` → IC=+0.173 (n=212)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.1304 (IC base=+0.173)

- **PATRÓN** `volumen_spike_ratio` < `1.9217` → IC=+0.170 (n=231)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` < 1.9217 (IC base=+0.173)

- **PATRÓN** `volumen_spike_ratio` > `2.4409` → IC=+0.165 (n=350)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 2.4409 (IC base=+0.173)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.199 (n=589)

  - _Acción_: Kelly boost +0.99€ cuando `libro_spread` < 0.04 (IC base=+0.173)

- **PATRÓN** `ballena_activa_n` < `18.0` → IC=+0.197 (n=120)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 18.0 (IC base=+0.173)
...
- **PATRÓN** `sigma_h` < `0.0074` → IC=+0.194 (n=491)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0074 (IC base=+0.160)

- **PATRÓN** `drift_60min` |x|≤ `0.1299` → IC=+0.181 (n=246)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.91€ cuando `drift_60min` |x|≤ 0.1299 (IC base=+0.160)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.176 (n=569)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 6.0 (IC base=+0.160)

- **PATRÓN** `ibs_20min` > `0.4502` → IC=+0.204 (n=558)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4502 (IC base=+0.160)

- **PATRÓN** `dist_vwap_pct` > `0.8779` → IC=+0.227 (n=130)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8779 (IC base=+0.160)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.391` → IC=+0.266 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.391 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` > `1.2168` → IC=+0.218 (n=186)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2168 (IC base=+0.160)

- **PATRÓN** `volumen_pendiente_norm` > `0.2487` → IC=+0.221 (n=127)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2487 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` > `2.6294` → IC=+0.203 (n=180)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6294 (IC base=+0.160)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.162 (n=626)

  - _Acción_: Kelly boost +0.81€ cuando `libro_spread` < 0.01 (IC base=+0.160)

- **PATRÓN** `libro_liquidez` > `8750.1763` → IC=+0.195 (n=372)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 8750.1763 (IC base=+0.160)

- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.149 (n=329)

  - _Acción_: Kelly boost +0.75€ cuando `ballena_activa_n` < 135.0 (IC base=+0.160)

- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.162 (n=631)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0076 (IC base=+0.134)

- **PATRÓN** `drift_60min` |x|≤ `0.3806` → IC=+0.152 (n=631)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.3806 (IC base=+0.134)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.171 (n=211)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 18.0 (IC base=+0.134)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.146 (n=286)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 7.0 (IC base=+0.134)

- **PATRÓN** `ibs_20min` < `0.3935` → IC=+0.199 (n=556)

  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` < 0.3935 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` < `0.3473` → IC=+0.146 (n=698)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.3473 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.378` → IC=+0.204 (n=133)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.378 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` < `1.1652` → IC=+0.140 (n=631)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` < 1.1652 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` > `0.6146` → IC=+0.140 (n=631)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 0.6146 (IC base=+0.134)

- **PATRÓN** `volumen_pendiente_norm` > `0.1052` → IC=+0.174 (n=213)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.1052 (IC base=+0.134)

- **PATRÓN** `volumen_spike_ratio` < `1.8624` → IC=+0.152 (n=349)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 1.8624 (IC base=+0.134)

- **PATRÓN** `volumen_spike_ratio` > `2.554` → IC=+0.165 (n=174)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 2.554 (IC base=+0.134)

- **PATRÓN** `libro_liquidez` > `5025.7991` → IC=+0.148 (n=421)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 5025.7991 (IC base=+0.134)

- **PATRÓN** `ballena_activa_n` < `180.0` → IC=+0.129 (n=391)

  - _Acción_: Kelly boost +0.64€ cuando `ballena_activa_n` < 180.0 (IC base=+0.134)

### GBM_LATE_15M_MULTIHORIZONTE#SOL#15min
- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.154 (n=631)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0065 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.131 (n=741)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` > `0.5405` → IC=+0.195 (n=706)

  - _Acción_: Kelly boost +0.97€ cuando `ibs_20min` > 0.5405 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `1.1423` → IC=+0.267 (n=161)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1423 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.259` → IC=+0.272 (n=156)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.259 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` > `0.7092` → IC=+0.130 (n=631)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` > 0.7092 (IC base=+0.113)

- **PATRÓN** `volumen_pendiente_norm` < `0.1685` → IC=+0.132 (n=701)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_pendiente_norm` < 0.1685 (IC base=+0.113)

- **PATRÓN** `volumen_spike_ratio` < `1.4397` → IC=+0.146 (n=224)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 1.4397 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `3197.545` → IC=+0.218 (n=236)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3197.545 (IC base=+0.113)

- **PATRÓN** `ballena_activa_n` < `57.0` → IC=+0.146 (n=512)

  - _Acción_: Kelly boost +0.73€ cuando `ballena_activa_n` < 57.0 (IC base=+0.113)

- **PATRÓN** `sigma_h` < `0.0054` → IC=+0.139 (n=214)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0054 (IC base=+0.135)

- **PATRÓN** `sigma_h` > `0.0095` → IC=+0.154 (n=290)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0095 (IC base=+0.135)

- **PATRÓN** `drift_60min` |x|≤ `0.5288` → IC=+0.140 (n=640)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.70€ cuando `drift_60min` |x|≤ 0.5288 (IC base=+0.135)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.194 (n=292)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` > 15.0 (IC base=+0.135)

- **PATRÓN** `ibs_20min` < `0.4667` → IC=+0.232 (n=640)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4667 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` > `1.0043` → IC=+0.149 (n=72)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 1.0043 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` < `0.2491` → IC=+0.148 (n=612)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.2491 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.218` → IC=+0.173 (n=249)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 3.218 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` < `1.2004` → IC=+0.151 (n=640)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.2004 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` > `0.8584` → IC=+0.152 (n=426)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.8584 (IC base=+0.135)

- **PATRÓN** `volumen_pendiente_norm` > `0.2708` → IC=+0.243 (n=68)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2708 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` > `2.3673` → IC=+0.202 (n=169)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3673 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `2170.6408` → IC=+0.164 (n=426)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 2170.6408 (IC base=+0.135)

### GBM_LATE_15M_MULTIHORIZONTE#XRP#15min
- **PATRÓN** `sigma_h` > `0.0177` → IC=+0.219 (n=482)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0177 (IC base=+0.193)

- **PATRÓN** `drift_60min` |x|≤ `0.1739` → IC=+0.213 (n=319)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1739 (IC base=+0.193)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.202 (n=757)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.193)

- **PATRÓN** `ibs_20min` > `0.8875` → IC=+0.273 (n=482)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8875 (IC base=+0.193)

- **PATRÓN** `dist_vwap_pct` > `0.1663` → IC=+0.227 (n=515)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1663 (IC base=+0.193)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.219` → IC=+0.246 (n=360)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.219 (IC base=+0.193)

- **PATRÓN** `volumen_regimen` > `0.686` → IC=+0.214 (n=646)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.686 (IC base=+0.193)

- **PATRÓN** `volumen_pendiente_norm` > `0.2397` → IC=+0.267 (n=127)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2397 (IC base=+0.193)

- **PATRÓN** `volumen_spike_ratio` < `2.5262` → IC=+0.211 (n=684)
...
- **PATRÓN** `drift_60min` |x|≤ `0.6418` → IC=+0.223 (n=732)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.6418 (IC base=+0.213)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.217 (n=365)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.4102` → IC=+0.259 (n=732)
...
- **PATRÓN** `volumen_regimen` > `0.6961` → IC=+0.235 (n=654)
...
- **PATRÓN** `sigma_h` > `0.0097` → IC=+0.172 (n=260)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0097 (IC base=+0.135)

- **PATRÓN** `drift_60min` |x|≤ `0.0973` → IC=+0.136 (n=259)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.0973 (IC base=+0.135)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.164 (n=737)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 8.0 (IC base=+0.135)

- **PATRÓN** `ibs_20min` > `0.4762` → IC=+0.168 (n=775)

  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` > 0.4762 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` > `0.8774` → IC=+0.217 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8774 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.261` → IC=+0.183 (n=197)

  - _Acción_: Kelly boost +0.92€ cuando `sigma_ewma_delta_pct` > 8.261 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` < `0.8791` → IC=+0.154 (n=417)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.8791 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` > `1.1833` → IC=+0.152 (n=208)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.1833 (IC base=+0.135)

- **PATRÓN** `volumen_pendiente_norm` > `0.1782` → IC=+0.185 (n=214)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1782 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` < `2.2818` → IC=+0.145 (n=634)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 2.2818 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` > `1.8649` → IC=+0.143 (n=480)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.8649 (IC base=+0.135)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.152 (n=582)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.01 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `6450.6304` → IC=+0.174 (n=259)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 6450.6304 (IC base=+0.135)

- **PATRÓN** `ballena_activa_n` < `13.0` → IC=+0.182 (n=193)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 13.0 (IC base=+0.135)

- **PATRÓN** `ibs_20min` < `0.6346` → IC=+0.120 (n=793)

  - _Acción_: Kelly boost +0.60€ cuando `ibs_20min` < 0.6346 (IC base=+0.070)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.58` → IC=+0.127 (n=116)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 9.58 (IC base=+0.070)

- **PATRÓN** `ballena_activa_n` < `20.0` → IC=+0.155 (n=285)

  - _Acción_: Kelly boost +0.78€ cuando `ballena_activa_n` < 20.0 (IC base=+0.070)
...
- **PATRÓN** `hora_utc` > `10.0` → IC=+0.159 (n=127)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 10.0 (IC base=+0.086)

- **PATRÓN** `ibs_20min` > `0.6242` → IC=+0.154 (n=108)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` > 0.6242 (IC base=+0.086)

- **PATRÓN** `dist_vwap_pct` > `0.8281` → IC=+0.190 (n=27)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.8281 (IC base=+0.086)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.734` → IC=+0.178 (n=57)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 3.734 (IC base=+0.086)

- **PATRÓN** `volumen_regimen` < `0.6949` → IC=+0.154 (n=53)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.6949 (IC base=+0.086)

- **PATRÓN** `libro_liquidez` > `11005.6828` → IC=+0.145 (n=108)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 11005.6828 (IC base=+0.086)

- **PATRÓN** `ibs_20min` < `0.6193` → IC=+0.150 (n=232)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` < 0.6193 (IC base=+0.064)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.093` → IC=+0.154 (n=50)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 9.093 (IC base=+0.064)

- **PATRÓN** `volumen_pendiente_norm` > `0.1669` → IC=+0.225 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1669 (IC base=+0.064)

- **PATRÓN** `ballena_activa_n` < `148.0` → IC=+0.175 (n=78)

  - _Acción_: Kelly boost +0.88€ cuando `ballena_activa_n` < 148.0 (IC base=+0.064)

### GBM_LATE_15M_PYCONFIRMADO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0052` → IC=+0.266 (n=126)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0052 (IC base=+0.263)

- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.315 (n=63)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0076 (IC base=+0.263)

- **PATRÓN** `drift_60min` |x|≤ `0.0967` → IC=+0.269 (n=63)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0967 (IC base=+0.263)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.300 (n=198)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.263)

- **PATRÓN** `ibs_20min` > `0.7776` → IC=+0.320 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7776 (IC base=+0.263)

- **PATRÓN** `dist_vwap_pct` > `0.1633` → IC=+0.295 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1633 (IC base=+0.263)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.256` → IC=+0.324 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.256 (IC base=+0.263)

- **PATRÓN** `volumen_regimen` < `0.8565` → IC=+0.273 (n=126)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.0968` → IC=+0.364 (n=79)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0968 (IC base=+0.263)

- **PATRÓN** `volumen_spike_ratio` < `1.3845` → IC=+0.297 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3845 (IC base=+0.263)

- **PATRÓN** `volumen_spike_ratio` > `2.0848` → IC=+0.326 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.0848 (IC base=+0.263)

- **PATRÓN** `libro_liquidez` > `2746.5037` → IC=+0.264 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2746.5037 (IC base=+0.263)

- **PATRÓN** `drift_60min` |x|≤ `0.1024` → IC=+0.188 (n=46)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.1024 (IC base=+0.049)

- **PATRÓN** `ibs_20min` < `0.3132` → IC=+0.149 (n=92)

  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` < 0.3132 (IC base=+0.049)

- **PATRÓN** `libro_liquidez` > `10077.5355` → IC=+0.188 (n=62)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 10077.5355 (IC base=+0.049)
...
- **PATRÓN** `sigma_h` > `0.0146` → IC=+0.201 (n=95)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0146 (IC base=+0.109)

- **PATRÓN** `ibs_20min` < `0.1538` → IC=+0.131 (n=63)

  - _Acción_: Kelly boost +0.65€ cuando `ibs_20min` < 0.1538 (IC base=+0.109)

- **PATRÓN** `dist_vwap_pct` > `0.9973` → IC=+0.342 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9973 (IC base=+0.109)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.481` → IC=+0.227 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.481 (IC base=+0.109)

- **PATRÓN** `volumen_regimen` > `0.6197` → IC=+0.135 (n=143)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` > 0.6197 (IC base=+0.109)

- **PATRÓN** `volumen_pendiente_norm` > `0.2474` → IC=+0.233 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2474 (IC base=+0.109)

- **PATRÓN** `volumen_spike_ratio` > `2.8751` → IC=+0.174 (n=44)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.8751 (IC base=+0.109)

- **PATRÓN** `ballena_activa_n` < `15.0` → IC=+0.145 (n=105)

  - _Acción_: Kelly boost +0.72€ cuando `ballena_activa_n` < 15.0 (IC base=+0.109)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.205 (n=2036)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0087 (IC base=+0.166)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.173 (n=4498)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 6.0 (IC base=+0.166)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.308 (n=1543)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.166)

- **PATRÓN** `dist_vwap_pct` > `1.0343` → IC=+0.229 (n=641)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0343 (IC base=+0.166)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.629` → IC=+0.227 (n=2536)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.629 (IC base=+0.166)

- **PATRÓN** `volumen_regimen` < `0.8889` → IC=+0.159 (n=2075)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.8889 (IC base=+0.166)

- **PATRÓN** `volumen_regimen` > `1.0894` → IC=+0.155 (n=1411)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 1.0894 (IC base=+0.166)

- **PATRÓN** `volumen_pendiente_norm` > `0.1673` → IC=+0.197 (n=1217)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.1673 (IC base=+0.166)

- **PATRÓN** `volumen_spike_ratio` > `1.8759` → IC=+0.175 (n=2776)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 1.8759 (IC base=+0.166)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.167 (n=4260)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.02 (IC base=+0.166)

- **PATRÓN** `libro_liquidez` > `3861.5494` → IC=+0.189 (n=1497)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3861.5494 (IC base=+0.166)

- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.216 (n=2077)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 61.0 (IC base=+0.166)

- **PATRÓN** `sigma_h` < `0.0108` → IC=+0.198 (n=4156)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0108 (IC base=+0.188)

- **PATRÓN** `drift_60min` |x|≤ `0.4819` → IC=+0.197 (n=4156)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.4819 (IC base=+0.188)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.206 (n=1866)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.188)

- **PATRÓN** `ibs_20min` < `0.5625` → IC=+0.242 (n=4156)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5625 (IC base=+0.188)

- **PATRÓN** `dist_vwap_pct` < `0.4398` → IC=+0.173 (n=2963)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.4398 (IC base=+0.188)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.667` → IC=+0.214 (n=581)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.667 (IC base=+0.188)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.581` → IC=+0.190 (n=4097)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` < 3.581 (IC base=+0.188)

- **PATRÓN** `volumen_regimen` < `0.6233` → IC=+0.173 (n=989)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` < 0.6233 (IC base=+0.188)

- **PATRÓN** `volumen_pendiente_norm` > `0.2871` → IC=+0.253 (n=500)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2871 (IC base=+0.188)

- **PATRÓN** `volumen_spike_ratio` > `2.3089` → IC=+0.204 (n=1563)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3089 (IC base=+0.188)

- **PATRÓN** `ballena_activa_n` < `142.0` → IC=+0.181 (n=3029)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 142.0 (IC base=+0.188)

### GBM_LATE_15M_TARDIO#BNB#15min
- **PATRÓN** `sigma_h` < `0.0053` → IC=+0.194 (n=246)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0053 (IC base=+0.188)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.341 (n=255)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.188)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.788` → IC=+0.310 (n=435)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.788 (IC base=+0.188)

- **PATRÓN** `volumen_pendiente_norm` > `0.218` → IC=+0.266 (n=135)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.218 (IC base=+0.188)

- **PATRÓN** `volumen_spike_ratio` < `1.469` → IC=+0.191 (n=218)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` < 1.469 (IC base=+0.188)

- **PATRÓN** `volumen_spike_ratio` > `1.886` → IC=+0.180 (n=436)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` > 1.886 (IC base=+0.188)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.238 (n=566)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.188)

- **PATRÓN** `ballena_activa_n` < `50.0` → IC=+0.243 (n=267)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 50.0 (IC base=+0.188)

- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.280 (n=479)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0067 (IC base=+0.275)

- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.281 (n=542)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0046 (IC base=+0.275)

- **PATRÓN** `drift_60min` |x|≤ `0.1153` → IC=+0.309 (n=239)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1153 (IC base=+0.275)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.286 (n=493)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.275)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.281 (n=554)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.275)

- **PATRÓN** `ibs_20min` < `0.4113` → IC=+0.312 (n=477)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4113 (IC base=+0.275)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.337` → IC=+0.286 (n=583)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.337 (IC base=+0.275)

- **PATRÓN** `volumen_pendiente_norm` > `0.2958` → IC=+0.326 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2958 (IC base=+0.275)

- **PATRÓN** `volumen_spike_ratio` > `1.5024` → IC=+0.295 (n=438)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5024 (IC base=+0.275)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.280 (n=248)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.275)

- **PATRÓN** `libro_liquidez` > `1459.068` → IC=+0.280 (n=484)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1459.068 (IC base=+0.275)

- **PATRÓN** `ballena_activa_n` < `69.0` → IC=+0.279 (n=338)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 69.0 (IC base=+0.275)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0029` → IC=+0.188 (n=245)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0029 (IC base=+0.161)

- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.165 (n=246)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` > 0.0068 (IC base=+0.161)

- **PATRÓN** `drift_60min` |x|≤ `0.0956` → IC=+0.169 (n=246)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.0956 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.179 (n=771)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 5.0 (IC base=+0.161)

- **PATRÓN** `ibs_20min` > `0.3271` → IC=+0.209 (n=734)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3271 (IC base=+0.161)

- **PATRÓN** `dist_vwap_pct` > `0.2322` → IC=+0.223 (n=420)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2322 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.757` → IC=+0.191 (n=179)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 9.757 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.464` → IC=+0.170 (n=638)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` < 4.464 (IC base=+0.161)

- **PATRÓN** `volumen_regimen` < `1.2687` → IC=+0.167 (n=734)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.2687 (IC base=+0.161)

- **PATRÓN** `volumen_regimen` > `1.1073` → IC=+0.172 (n=333)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` > 1.1073 (IC base=+0.161)

- **PATRÓN** `volumen_pendiente_norm` < `0.0714` → IC=+0.174 (n=612)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` < 0.0714 (IC base=+0.161)

- **PATRÓN** `volumen_pendiente_norm` > `0.2078` → IC=+0.202 (n=149)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2078 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` < `2.4168` → IC=+0.180 (n=685)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 2.4168 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` > `1.7246` → IC=+0.181 (n=456)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 1.7246 (IC base=+0.161)

- **PATRÓN** `libro_liquidez` > `10653.1142` → IC=+0.187 (n=656)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 10653.1142 (IC base=+0.161)

- **PATRÓN** `ballena_activa_n` < `509.0` → IC=+0.173 (n=610)

  - _Acción_: Kelly boost +0.87€ cuando `ballena_activa_n` < 509.0 (IC base=+0.161)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.170 (n=677)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0062 (IC base=+0.160)

- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.160 (n=604)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` > 0.0029 (IC base=+0.160)

- **PATRÓN** `drift_60min` |x|≤ `0.2725` → IC=+0.177 (n=595)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.2725 (IC base=+0.160)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.166 (n=677)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 17.0 (IC base=+0.160)

- **PATRÓN** `ibs_20min` < `0.6106` → IC=+0.201 (n=676)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6106 (IC base=+0.160)

- **PATRÓN** `dist_vwap_pct` < `0.3362` → IC=+0.174 (n=672)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.3362 (IC base=+0.160)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.77` → IC=+0.213 (n=120)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.77 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` < `0.6183` → IC=+0.228 (n=226)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6183 (IC base=+0.160)

- **PATRÓN** `volumen_pendiente_norm` > `0.1411` → IC=+0.232 (n=177)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1411 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` < `1.3983` → IC=+0.189 (n=194)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` < 1.3983 (IC base=+0.160)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.161 (n=877)
  ...
  - _Acción_: Kelly boost +0.80€ cuando `libro_spread` < 0.01 (IC base=+0.160)

- **PATRÓN** `ballena_activa_n` < `290.0` → IC=+0.171 (n=162)

  - _Acción_: Kelly boost +0.85€ cuando `ballena_activa_n` < 290.0 (IC base=+0.160)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.252 (n=292)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0088 (IC base=+0.209)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.242 (n=308)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.209)

- **PATRÓN** `ibs_20min` > `0.6842` → IC=+0.259 (n=575)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6842 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.58` → IC=+0.347 (n=155)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.58 (IC base=+0.209)

- **PATRÓN** `volumen_pendiente_norm` < `0.2195` → IC=+0.216 (n=583)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2195 (IC base=+0.209)

- **PATRÓN** `volumen_spike_ratio` < `3.9629` → IC=+0.206 (n=570)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.9629 (IC base=+0.209)

- **PATRÓN** `volumen_spike_ratio` > `2.5092` → IC=+0.207 (n=380)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5092 (IC base=+0.209)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.241 (n=646)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.209)

- **PATRÓN** `libro_liquidez` > `1451.2984` → IC=+0.212 (n=643)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1451.2984 (IC base=+0.209)

- **PATRÓN** `ballena_activa_n` < `64.0` → IC=+0.270 (n=359)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 64.0 (IC base=+0.209)

- **PATRÓN** `sigma_h` < `0.0059` → IC=+0.294 (n=216)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0059 (IC base=+0.240)

- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.240 (n=294)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0087 (IC base=+0.240)

- **PATRÓN** `drift_60min` |x|≤ `0.4678` → IC=+0.240 (n=648)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4678 (IC base=+0.240)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.269 (n=301)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.240)

- **PATRÓN** `ibs_20min` < `0.4074` → IC=+0.295 (n=570)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4074 (IC base=+0.240)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.481` → IC=+0.294 (n=197)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.481 (IC base=+0.240)
...
- **PATRÓN** `volumen_spike_ratio` < `3.0513` → IC=+0.243 (n=462)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.0513 (IC base=+0.240)

- **PATRÓN** `volumen_spike_ratio` > `1.9102` → IC=+0.224 (n=469)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9102 (IC base=+0.240)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.239 (n=351)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.240)

- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.214 (n=442)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 62.0 (IC base=+0.240)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0071` → IC=+0.157 (n=653)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0071 (IC base=+0.143)

- **PATRÓN** `drift_60min` |x|≤ `0.1427` → IC=+0.147 (n=327)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.1427 (IC base=+0.143)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.158 (n=668)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 8.0 (IC base=+0.143)

- **PATRÓN** `ibs_20min` > `0.74` → IC=+0.240 (n=494)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.74 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` > `0.3645` → IC=+0.192 (n=293)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.3645 (IC base=+0.143)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.935` → IC=+0.190 (n=127)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 11.935 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` < `0.9068` → IC=+0.166 (n=495)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.9068 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` > `1.2094` → IC=+0.163 (n=247)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 1.2094 (IC base=+0.143)

- **PATRÓN** `volumen_pendiente_norm` > `0.2765` → IC=+0.235 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2765 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` > `2.1629` → IC=+0.207 (n=319)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1629 (IC base=+0.143)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.146 (n=811)

  - _Acción_: Kelly boost +0.73€ cuando `libro_spread` < 0.01 (IC base=+0.143)

- **PATRÓN** `libro_liquidez` > `9479.813` → IC=+0.243 (n=336)
...
- **PATRÓN** `sigma_h` > `0.0085` → IC=+0.147 (n=520)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0085 (IC base=+0.093)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.136 (n=523)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 12.0 (IC base=+0.093)

- **PATRÓN** `ibs_20min` > `0.4787` → IC=+0.181 (n=779)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` > 0.4787 (IC base=+0.093)

- **PATRÓN** `dist_vwap_pct` > `1.1138` → IC=+0.203 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1138 (IC base=+0.093)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.451` → IC=+0.225 (n=296)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.451 (IC base=+0.093)

- **PATRÓN** `libro_liquidez` > `2949.3334` → IC=+0.267 (n=260)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2949.3334 (IC base=+0.093)

- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.137 (n=557)

  - _Acción_: Kelly boost +0.68€ cuando `ballena_activa_n` < 62.0 (IC base=+0.093)

- **PATRÓN** `sigma_h` < `0.0059` → IC=+0.171 (n=250)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0059 (IC base=+0.115)

- **PATRÓN** `drift_60min` |x|≤ `0.1332` → IC=+0.167 (n=250)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.1332 (IC base=+0.115)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.141 (n=357)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 15.0 (IC base=+0.115)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.138 (n=285)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` < 6.0 (IC base=+0.115)

- **PATRÓN** `ibs_20min` < `0.6` → IC=+0.200 (n=751)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` < `0.5096` → IC=+0.139 (n=726)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.5096 (IC base=+0.115)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.195` → IC=+0.138 (n=742)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` < 3.195 (IC base=+0.115)

- **PATRÓN** `volumen_regimen` < `1.202` → IC=+0.124 (n=749)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_regimen` < 1.202 (IC base=+0.115)

- **PATRÓN** `volumen_pendiente_norm` > `0.0725` → IC=+0.156 (n=248)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_pendiente_norm` > 0.0725 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` < `1.4553` → IC=+0.144 (n=200)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 1.4553 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` > `2.1911` → IC=+0.150 (n=272)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 2.1911 (IC base=+0.115)

- **PATRÓN** `libro_liquidez` > `2648.8968` → IC=+0.172 (n=340)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 2648.8968 (IC base=+0.115)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` > `0.0282` → IC=+0.259 (n=288)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0282 (IC base=+0.201)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.212 (n=902)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.201)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.304 (n=324)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` > `0.1653` → IC=+0.256 (n=515)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1653 (IC base=+0.201)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.169` → IC=+0.251 (n=428)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.169 (IC base=+0.201)

- **PATRÓN** `volumen_regimen` > `1.2351` → IC=+0.234 (n=287)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2351 (IC base=+0.201)

- **PATRÓN** `volumen_pendiente_norm` > `0.1007` → IC=+0.246 (n=293)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1007 (IC base=+0.201)

- **PATRÓN** `volumen_spike_ratio` > `1.4407` → IC=+0.204 (n=810)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4407 (IC base=+0.201)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.206 (n=993)
...
- **PATRÓN** `sigma_h` > `0.0254` → IC=+0.220 (n=316)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0254 (IC base=+0.213)

- **PATRÓN** `drift_60min` |x|≤ `0.5323` → IC=+0.217 (n=833)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.5323 (IC base=+0.213)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.216 (n=456)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.213)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.217 (n=1003)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.213)

- **PATRÓN** `ibs_20min` < `0.5` → IC=+0.266 (n=958)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5 (IC base=+0.213)

- **PATRÓN** `dist_vwap_pct` < `0.2661` → IC=+0.223 (n=864)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2661 (IC base=+0.213)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.222` → IC=+0.285 (n=133)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.222 (IC base=+0.213)

- **PATRÓN** `volumen_regimen` > `0.7057` → IC=+0.224 (n=846)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7057 (IC base=+0.213)

- **PATRÓN** `volumen_pendiente_norm` > `0.2831` → IC=+0.310 (n=119)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2831 (IC base=+0.213)

- **PATRÓN** `volumen_spike_ratio` > `1.8573` → IC=+0.212 (n=515)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8573 (IC base=+0.213)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.214 (n=1111)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.213)

- **PATRÓN** `ballena_activa_n` < `34.0` → IC=+0.187 (n=666)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.129 (n=1702)

- **PATRÓN** `sigma_h` < `0.01` → IC=+0.132 (n=1299)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` < 0.01 (IC base=+0.120)

- **PATRÓN** `drift_60min` |x|≤ `0.5556` → IC=+0.130 (n=1475)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.65€ cuando `drift_60min` |x|≤ 0.5556 (IC base=+0.120)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `6.022` → IC=+0.123 (n=1500)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 6.022 (IC base=+0.120)

- **PATRÓN** `volumen_regimen` > `0.9007` → IC=+0.122 (n=655)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` > 0.9007 (IC base=+0.120)

- **PATRÓN** `volumen_pendiente_norm` > `0.1747` → IC=+0.149 (n=400)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_pendiente_norm` > 0.1747 (IC base=+0.120)

- **PATRÓN** `volumen_spike_ratio` < `1.4629` → IC=+0.148 (n=487)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.4629 (IC base=+0.120)

- **PATRÓN** `volumen_spike_ratio` > `1.9026` → IC=+0.140 (n=973)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` > 1.9026 (IC base=+0.120)

- **PATRÓN** `libro_liquidez` > `8940.1386` → IC=+0.133 (n=669)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 8940.1386 (IC base=+0.120)

- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.176 (n=430)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` < 0.0039 (IC base=+0.125)

- **PATRÓN** `drift_60min` |x|≤ `0.4014` → IC=+0.153 (n=1134)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.4014 (IC base=+0.125)
...
- **PATRÓN** `ibs_20min` < `0.1963` → IC=+0.155 (n=567)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` < 0.1963 (IC base=+0.125)

- **PATRÓN** `dist_vwap_pct` > `0.4781` → IC=+0.126 (n=346)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` > 0.4781 (IC base=+0.125)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.372` → IC=+0.138 (n=1286)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` < 6.372 (IC base=+0.125)

- **PATRÓN** `volumen_regimen` < `1.2372` → IC=+0.135 (n=1257)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 1.2372 (IC base=+0.125)

- **PATRÓN** `volumen_pendiente_norm` < `0.1476` → IC=+0.126 (n=1284)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_pendiente_norm` < 0.1476 (IC base=+0.125)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4757` → IC=+0.135 (n=1275)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 2.4757 (IC base=+0.125)

- **PATRÓN** `volumen_spike_ratio` > `1.7938` → IC=+0.128 (n=850)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.7938 (IC base=+0.125)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.129 (n=1702)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.01 (IC base=+0.125)

- **PATRÓN** `libro_liquidez` > `8304.7792` → IC=+0.134 (n=1151)
...
- **PATRÓN** `sigma_h` < `0.0052` → IC=+0.160 (n=236)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0052 (IC base=+0.133)

- **PATRÓN** `drift_60min` |x|≤ `0.4818` → IC=+0.150 (n=533)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.4818 (IC base=+0.133)
...
- **PATRÓN** `ibs_20min` > `0.1834` → IC=+0.145 (n=534)

  - _Acción_: Kelly boost +0.73€ cuando `ibs_20min` > 0.1834 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` > `0.9899` → IC=+0.161 (n=113)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.9899 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `0.4329` → IC=+0.145 (n=511)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.4329 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.061` → IC=+0.146 (n=537)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` < 7.061 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` < `1.2826` → IC=+0.143 (n=533)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 1.2826 (IC base=+0.133)

- **PATRÓN** `volumen_pendiente_norm` < `0.1175` → IC=+0.138 (n=492)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_pendiente_norm` < 0.1175 (IC base=+0.133)

- **PATRÓN** `volumen_pendiente_norm` > `0.1782` → IC=+0.156 (n=158)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_pendiente_norm` > 0.1782 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` < `1.4475` → IC=+0.172 (n=175)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.4475 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` > `1.8447` → IC=+0.141 (n=349)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 1.8447 (IC base=+0.133)

- **PATRÓN** `libro_liquidez` > `8936.6811` → IC=+0.144 (n=476)

  - _Acción_: Kelly boost +0.72€ cuando `libro_liquidez` > 8936.6811 (IC base=+0.133)

- **PATRÓN** `sigma_h` < `0.0088` → IC=+0.163 (n=399)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0088 (IC base=+0.143)

- **PATRÓN** `drift_60min` |x|≤ `0.425` → IC=+0.203 (n=351)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.1055` → IC=+0.159 (n=400)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.1055 (IC base=+0.143)
...
- **PATRÓN** `volumen_regimen` < `1.2252` → IC=+0.168 (n=399)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.2252 (IC base=+0.143)

- **PATRÓN** `volumen_pendiente_norm` < `0.1489` → IC=+0.151 (n=405)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_pendiente_norm` < 0.1489 (IC base=+0.143)

- **PATRÓN** `volumen_pendiente_norm` > `0.0728` → IC=+0.156 (n=178)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_pendiente_norm` > 0.0728 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` < `2.1612` → IC=+0.171 (n=344)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` < 2.1612 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` > `1.4487` → IC=+0.158 (n=390)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.4487 (IC base=+0.143)

- **PATRÓN** `libro_liquidez` > `7907.9702` → IC=+0.158 (n=399)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.208 (n=217)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0057 (IC base=+0.107)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.175 (n=115)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 18.0 (IC base=+0.107)

- **PATRÓN** `ibs_20min` > `0.6839` → IC=+0.250 (n=234)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6839 (IC base=+0.107)

- **PATRÓN** `dist_vwap_pct` > `0.3445` → IC=+0.216 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3445 (IC base=+0.107)

- **PATRÓN** `dist_vwap_pct` < `0.2075` → IC=+0.161 (n=184)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2075 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.907` → IC=+0.283 (n=113)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.907 (IC base=+0.107)

- **PATRÓN** `volumen_regimen` < `0.6313` → IC=+0.167 (n=103)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.6313 (IC base=+0.107)

- **PATRÓN** `volumen_regimen` > `1.1122` → IC=+0.175 (n=78)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 1.1122 (IC base=+0.107)

- **PATRÓN** `volumen_pendiente_norm` < `0.1507` → IC=+0.214 (n=131)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1507 (IC base=+0.107)
...
- **PATRÓN** `volumen_spike_ratio` > `1.7679` → IC=+0.261 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7679 (IC base=+0.107)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.195 (n=201)

  - _Acción_: Kelly boost +0.97€ cuando `libro_spread` < 0.02 (IC base=+0.107)

- **PATRÓN** `libro_liquidez` > `1222.6873` → IC=+0.189 (n=210)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 1222.6873 (IC base=+0.107)
...
- **PATRÓN** `sigma_h` < `0.0054` → IC=+0.230 (n=98)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0054 (IC base=+0.117)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.136 (n=42)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 17.0 (IC base=+0.117)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.154 (n=53)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` < 10.0 (IC base=+0.117)

- **PATRÓN** `ibs_20min` > `0.7937` → IC=+0.232 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7937 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` > `0.1642` → IC=+0.174 (n=44)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.1642 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` < `0.4923` → IC=+0.138 (n=78)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.4923 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `17.525` → IC=+0.208 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 17.525 (IC base=+0.117)

- **PATRÓN** `volumen_regimen` < `0.6265` → IC=+0.203 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6265 (IC base=+0.117)

- **PATRÓN** `volumen_pendiente_norm` < `0.0651` → IC=+0.265 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0651 (IC base=+0.117)

- **PATRÓN** `volumen_spike_ratio` < `1.9095` → IC=+0.250 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9095 (IC base=+0.117)

- **PATRÓN** `volumen_spike_ratio` > `1.681` → IC=+0.242 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.681 (IC base=+0.117)
...
- **PATRÓN** `sigma_h` < `0.007` → IC=+0.177 (n=131)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` < 0.007 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.150 (n=121)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 7.0 (IC base=+0.140)

- **PATRÓN** `ibs_20min` > `0.5632` → IC=+0.292 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5632 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` > `0.4502` → IC=+0.250 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4502 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` < `0.2791` → IC=+0.208 (n=94)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2791 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.249` → IC=+0.329 (n=68)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.249 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `0.791` → IC=+0.196 (n=67)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` < 0.791 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` > `0.9231` → IC=+0.223 (n=45)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.9231 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` < `0.158` → IC=+0.265 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.158 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `2.6009` → IC=+0.400 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6009 (IC base=+0.140)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.212 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `2468.387` → IC=+0.265 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2468.387 (IC base=+0.140)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.198 (n=41)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 18.0 (IC base=+0.039)

- **PATRÓN** `ibs_20min` > `0.6842` → IC=+0.172 (n=56)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` > 0.6842 (IC base=+0.039)

- **PATRÓN** `dist_vwap_pct` > `0.3068` → IC=+0.167 (n=28)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.3068 (IC base=+0.039)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.36` → IC=+0.241 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.36 (IC base=+0.039)

- **PATRÓN** `volumen_regimen` > `0.5999` → IC=+0.121 (n=56)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_regimen` > 0.5999 (IC base=+0.039)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.196 (n=21)

  - _Acción_: Kelly boost +0.98€ cuando `libro_spread` < 0.04 (IC base=+0.039)

- **PATRÓN** `libro_liquidez` > `1222.6873` → IC=+0.132 (n=17)

  - _Acción_: Kelly boost +0.66€ cuando `libro_liquidez` > 1222.6873 (IC base=+0.039)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.121 (n=217)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 6.0 (IC base=+0.108)

- **PATRÓN** `libro_liquidez` > `2469.9192` → IC=+0.138 (n=194)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 2469.9192 (IC base=+0.108)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=1040)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.167 (n=16)
  ...
  - _Acción_: SKIP cuando `liq_imbalance_15min` |x|≤ 1.0
  - _Potencial_: sin este filtro IC_bueno=+0.041 (n=342)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.176 (n=1428)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=4297)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.187 (n=1459)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.008 (n=4489)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.4` → IC=-0.243 (n=204)

  - _Acción_: SKIP cuando `py_entrada` < 0.4
  - _Potencial_: sin este filtro IC_bueno=+0.054 (n=669)
...
- **FILTRO** `py_entrada` > `0.66` → IC=-0.163 (n=259)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=779)
...
- **FILTRO** `py_entrada` < `0.45` → IC=-0.200 (n=301)
...
- **FILTRO** `py_entrada` > `0.65` → IC=-0.226 (n=235)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=769)

- **FILTRO** `ibs_20min` > `0.7172` → IC=-0.198 (n=250)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7172
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=754)
...
- **FILTRO** `py_entrada` < `0.495` → IC=-0.144 (n=259)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=895)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.201 (n=249)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=749)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=2991)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=11026)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.283 (n=3451)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=10566)

- **FILTRO** `ibs_7min` < `0.7116` → IC=-0.248 (n=3504)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7116
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=10513)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.169 (n=4692)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=9325)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.219 (n=4339)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=13295)

- **FILTRO** `ibs_7min` > `0.7143` → IC=-0.172 (n=4400)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=13234)
...
- **FILTRO** `py_entrada` > `0.52` → IC=-0.140 (n=2041)

  - _Acción_: SKIP cuando `py_entrada` > 0.52
  - _Potencial_: sin este filtro IC_bueno=+0.076 (n=1026)

- **FILTRO** `drift_7min_pct` |x|> `0.1142` → IC=-0.129 (n=1042)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1142
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=2025)

- **FILTRO** `ibs_7min` > `0.8333` → IC=-0.190 (n=765)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8333
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=2302)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=572)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=2048)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.259 (n=636)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=1984)

- **FILTRO** `ibs_7min` < `0.7711` → IC=-0.194 (n=655)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7711
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=1965)

- **FILTRO** `ballena_activa_n` > `168.0` → IC=-0.178 (n=651)

  - _Acción_: SKIP cuando `ballena_activa_n` > 168.0
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=1969)

- **FILTRO** `py_entrada` > `0.64` → IC=-0.245 (n=602)

  - _Acción_: SKIP cuando `py_entrada` > 0.64
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=2027)

- **FILTRO** `ballena_activa_n` > `106.0` → IC=-0.176 (n=893)

  - _Acción_: SKIP cuando `ballena_activa_n` > 106.0
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=1736)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.224 (n=738)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=2423)

- **FILTRO** `ibs_7min` > `0.799` → IC=-0.172 (n=790)

  - _Acción_: SKIP cuando `ibs_7min` > 0.799
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=2371)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.138 (n=717)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.091 (n=1659)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.259 (n=582)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=1794)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.237 (n=775)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=1612)

- **FILTRO** `ibs_7min` > `0.2755` → IC=-0.177 (n=596)
...
- **FILTRO** `ballena_activa_n` > `34.0` → IC=-0.182 (n=587)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=1800)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.39` → IC=-0.249 (n=635)

  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=1978)

- **FILTRO** `ibs_7min` < `0.7333` → IC=-0.213 (n=653)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7333
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=1960)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.191 (n=623)

  - _Acción_: SKIP cuando `ballena_activa_n` > 13.0
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=1990)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.179 (n=798)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.005 (n=2515)
...
- **FILTRO** `py_entrada` < `0.36` → IC=-0.298 (n=563)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=1732)

- **FILTRO** `ibs_7min` < `0.7333` → IC=-0.249 (n=571)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7333
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=1724)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.230 (n=554)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.254 (n=607)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=2470)
...
- **PATRÓN** `streak_estiramiento` < `0.3697` → IC=+0.182 (n=61)

  - _Acción_: Kelly boost +0.91€ cuando `streak_estiramiento` < 0.3697 (IC base=+0.041)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.121 (n=27)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=255)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.013 (n=1568)

### STRUCT_NO_15M#SOL#15min
- **FILTRO** `py_entrada` < `0.495` → IC=-0.147 (n=32)

  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=884)
...
### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` < `0.0034` → IC=+0.169 (n=170)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0034 (IC base=+0.131)

- **PATRÓN** `sigma_h` > `0.0052` → IC=+0.157 (n=339)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` > 0.0052 (IC base=+0.131)

- **PATRÓN** `drift_60min` |x|≤ `0.1678` → IC=+0.131 (n=448)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.66€ cuando `drift_60min` |x|≤ 0.1678 (IC base=+0.131)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0597` → IC=+0.136 (n=509)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.68€ cuando `delta_ratio_macro` |x|> 0.0597 (IC base=+0.131)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.136 (n=548)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 4.0 (IC base=+0.131)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.153 (n=226)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` < 6.0 (IC base=+0.131)

- **PATRÓN** `ibs_15` > `0.5728` → IC=+0.222 (n=509)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5728 (IC base=+0.131)

- **PATRÓN** `dist_vwap_pct` > `0.424` → IC=+0.188 (n=126)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.424 (IC base=+0.131)

- **PATRÓN** `dist_vwap_pct` < `0.0973` → IC=+0.125 (n=313)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` < 0.0973 (IC base=+0.131)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.661` → IC=+0.218 (n=200)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.661 (IC base=+0.131)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.135 (n=532)

  - _Acción_: Kelly boost +0.67€ cuando `libro_spread` < 0.01 (IC base=+0.131)

- **PATRÓN** `libro_liquidez` > `9958.5281` → IC=+0.163 (n=170)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 9958.5281 (IC base=+0.131)

- **PATRÓN** `ballena_activa_n` < `37.0` → IC=+0.173 (n=99)

  - _Acción_: Kelly boost +0.87€ cuando `ballena_activa_n` < 37.0 (IC base=+0.131)

### UPDOWN_GBM#5min
- **FILTRO** `sigma_ewma_delta_pct` > `6.475` → IC=-0.205 (n=59)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 6.475
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=763)
...
- **PATRÓN** `sigma_h` < `0.003` → IC=+0.190 (n=98)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.003 (IC base=+0.170)

- **PATRÓN** `sigma_h` > `0.0051` → IC=+0.186 (n=49)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0051 (IC base=+0.170)

- **PATRÓN** `drift_60min` |x|≤ `0.1907` → IC=+0.191 (n=147)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.96€ cuando `drift_60min` |x|≤ 0.1907 (IC base=+0.170)

- **PATRÓN** `drift_15min` |x|≤ `0.5909` → IC=+0.172 (n=129)

  - _Acción_: Kelly boost +0.86€ cuando `drift_15min` |x|≤ 0.5909 (IC base=+0.170)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.199 (n=154)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 4.0 (IC base=+0.170)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.169 (n=152)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 17.0 (IC base=+0.170)

- **PATRÓN** `ibs_15` > `0.8766` → IC=+0.290 (n=98)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8766 (IC base=+0.170)

- **PATRÓN** `dist_vwap_pct` > `0.3054` → IC=+0.212 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3054 (IC base=+0.170)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.252` → IC=+0.219 (n=87)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.252 (IC base=+0.170)

- **PATRÓN** `sigma_ewma_delta_pct` < `18.708` → IC=+0.171 (n=147)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` < 18.708 (IC base=+0.170)

- **PATRÓN** `libro_liquidez` > `14003.3235` → IC=+0.245 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 14003.3235 (IC base=+0.170)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `25.968` → IC=+0.182 (n=20)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 25.968 (IC base=+0.003)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.2614` → IC=+0.167 (n=43)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.83€ cuando `delta_ratio_macro` |x|> 0.2614 (IC base=+0.090)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1659` → IC=+0.128 (n=41)

  - _Acción_: Kelly boost +0.64€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1659 (IC base=+0.090)

- **PATRÓN** `ibs_15` > `0.6298` → IC=+0.195 (n=129)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.97€ cuando `ibs_15` > 0.6298 (IC base=+0.090)

- **PATRÓN** `dist_vwap_pct` < `0.1449` → IC=+0.125 (n=102)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.1449 (IC base=+0.090)

- **PATRÓN** `sigma_ewma_delta_pct` > `15.362` → IC=+0.133 (n=47)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` > 15.362 (IC base=+0.090)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0501` → IC=+0.164 (n=132)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.82€ cuando `delta_ratio_macro` |x|> 0.0501 (IC base=+0.118)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.146 (n=63)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 15.0 (IC base=+0.118)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.161 (n=60)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 6.0 (IC base=+0.118)

- **PATRÓN** `ibs_15` > `0.5676` → IC=+0.208 (n=118)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5676 (IC base=+0.118)

- **PATRÓN** `dist_vwap_pct` > `0.4287` → IC=+0.278 (n=43)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4287 (IC base=+0.118)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.352` → IC=+0.281 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.352 (IC base=+0.118)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.128 (n=119)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.01 (IC base=+0.118)

- **PATRÓN** `libro_liquidez` > `2503.3208` → IC=+0.167 (n=118)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 2503.3208 (IC base=+0.118)

- **PATRÓN** `ballena_activa_n` < `17.0` → IC=+0.237 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 17.0 (IC base=+0.118)
...
- **FILTRO** `sigma_h` > `0.0107` → IC=-0.206 (n=331)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `ibs_15` < `0.4875` → IC=-0.229 (n=116)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.186 (n=348)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.137 (n=340)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=987)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.299` → IC=+0.175 (n=164)

  - _Acción_: Kelly boost +0.87€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.299 (IC base=-0.069)

- **PATRÓN** `ibs_15` > `0.4875` → IC=+0.186 (n=348)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.93€ cuando `ibs_15` > 0.4875 (IC base=-0.069)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1241` → IC=+0.246 (n=289)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **PATRÓN** `ibs_15` < `0.3714` → IC=+0.275 (n=433)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3714 (IC base=-0.066)

- **PATRÓN** `dist_vwap_pct` > `0.7546` → IC=+0.227 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7546 (IC base=-0.066)

- **PATRÓN** `dist_vwap_pct` < `0.2569` → IC=+0.218 (n=423)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2569 (IC base=-0.066)

- **PATRÓN** `ballena_activa_n` < `52.0` → IC=+0.229 (n=264)
...
- **FILTRO** `sigma_h` > `0.0074` → IC=-0.240 (n=221)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0074
  - _Potencial_: sin este filtro IC_bueno=-0.194 (n=665)

- **FILTRO** `sigma_h` < `0.0038` → IC=-0.228 (n=292)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0038
  - _Potencial_: sin este filtro IC_bueno=-0.195 (n=594)

- **FILTRO** `drift_15min` |x|> `0.7406` → IC=-0.208 (n=221)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 0.7406
  - _Potencial_: sin este filtro IC_bueno=-0.205 (n=665)

- **FILTRO** `sigma_ewma_delta_pct` > `19.708` → IC=-0.250 (n=162)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 19.708
  - _Potencial_: sin este filtro IC_bueno=-0.196 (n=724)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1731` → IC=+0.152 (n=21)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.76€ cuando `delta_ratio_macro` |x|> 0.1731 (IC base=+0.020)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1267` → IC=+0.184 (n=17)

  - _Acción_: Kelly boost +0.92€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1267 (IC base=+0.020)

- **PATRÓN** `ibs_15` > `0.6868` → IC=+0.291 (n=41)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6868 (IC base=+0.020)

- **PATRÓN** `dist_vwap_pct` > `0.1982` → IC=+0.149 (n=35)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.1982 (IC base=+0.020)

- **PATRÓN** `dist_vwap_pct` < `0.2619` → IC=+0.167 (n=28)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.2619 (IC base=+0.020)

- **PATRÓN** `ballena_activa_n` < `431.0` → IC=+0.149 (n=35)

  - _Acción_: Kelly boost +0.74€ cuando `ballena_activa_n` < 431.0 (IC base=+0.020)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2968` → IC=+0.233 (n=88)
...
- **PATRÓN** `sigma_h` < `0.0079` → IC=+0.251 (n=219)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0079 (IC base=+0.217)

- **PATRÓN** `sigma_h` > `0.0045` → IC=+0.220 (n=219)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0045 (IC base=+0.217)

- **PATRÓN** `drift_60min` |x|≤ `0.4492` → IC=+0.229 (n=219)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4492 (IC base=+0.217)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0918` → IC=+0.236 (n=195)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0918 (IC base=+0.217)
...
- **PATRÓN** `ibs_15` < `0.3879` → IC=+0.283 (n=219)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3879 (IC base=+0.217)

- **PATRÓN** `dist_vwap_pct` > `0.7799` → IC=+0.278 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7799 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.224` → IC=+0.232 (n=233)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 11.224 (IC base=+0.217)

- **PATRÓN** `libro_liquidez` > `4457.1025` → IC=+0.216 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4457.1025 (IC base=+0.217)

- **PATRÓN** `ballena_activa_n` < `162.0` → IC=+0.214 (n=166)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 162.0 (IC base=+0.217)

### UPDOWN_GBM_15M_TARDIO#SOL#15min
- **FILTRO** `sigma_h` > `0.0077` → IC=-0.209 (n=163)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0077
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=165)

- **FILTRO** `drift_15min` |x|> `0.8686` → IC=-0.259 (n=81)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1211` → IC=+0.172 (n=62)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.86€ cuando `delta_ratio_macro` |x|> 0.1211 (IC base=-0.048)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3219` → IC=+0.178 (n=88)

  - _Acción_: Kelly boost +0.89€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3219 (IC base=-0.048)
...
- **PATRÓN** `dist_vwap_pct` < `0.2474` → IC=+0.185 (n=87)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` < 0.2474 (IC base=-0.048)

- **PATRÓN** `ballena_activa_n` < `42.0` → IC=+0.159 (n=83)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 42.0 (IC base=-0.048)

### UPDOWN_GBM_15M_TARDIO#XRP#15min
- **FILTRO** `sigma_h` > `0.0227` → IC=-0.258 (n=122)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0227
  - _Potencial_: sin este filtro IC_bueno=-0.131 (n=239)

- **FILTRO** `drift_60min` |x|> `0.183` → IC=-0.217 (n=90)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.183
  - _Potencial_: sin este filtro IC_bueno=-0.159 (n=271)

- **FILTRO** `drift_15min` |x|> `1.198` → IC=-0.250 (n=90)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 1.198
  - _Potencial_: sin este filtro IC_bueno=-0.148 (n=271)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.256 (n=39)

  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.164 (n=322)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.5728 sube el IC de +0.131 a +0.222 en UPDOWN_GBM#15min (n=509). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.8766 sube el IC de +0.170 a +0.290 en UPDOWN_GBM#BTC#15min (n=98). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.6298 sube el IC de +0.090 a +0.195 en UPDOWN_GBM#ETH#15min (n=129). Ya aplicado como kelly_boost=+0.97€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_YES, IBS > 0.5676 sube el IC de +0.118 a +0.208 en UPDOWN_GBM#XRP#15min (n=118). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.4875 sube el IC de -0.069 a +0.186 en UPDOWN_GBM_15M_TARDIO (n=348). Ya aplicado como kelly_boost=+0.93€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3714 sube el IC de -0.066 a +0.275 en UPDOWN_GBM_15M_TARDIO (n=433). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO#BTC#15min**: dentro de BUY_YES, IBS > 0.6868 sube el IC de +0.020 a +0.291 en UPDOWN_GBM_15M_TARDIO#BTC#15min (n=41). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.3879 sube el IC de +0.217 a +0.283 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=219). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#SOL#15min**: dentro de BUY_NO, IBS < 0.3814 sube el IC de -0.048 a +0.219 en UPDOWN_GBM_15M_TARDIO#SOL#15min (n=94). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 791 | +0.088 | +44.80€ | 2 | 10 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min |