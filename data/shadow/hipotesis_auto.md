# Hipótesis automáticas — 2026-08-18 15:17 UTC
_Generado por shadow_postmortem.py sobre 64378 resoluciones (PNL=+6593.02€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `n_ballenas` < `5.0` → IC=-0.128 (n=2030)
...
- **FILTRO** `restante_s_al_confirmar` < `148.64` → IC=-0.262 (n=851)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=2554)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.200 (n=2206)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.159 (n=3772)
...
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.301 (n=239)
...
- **PATRÓN** `py_entrada` < `0.73` → IC=+0.176 (n=593)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.217 (n=589)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.72` → IC=+0.188 (n=492)
...
- **PATRÓN** `hora_utc` > `14.0` → IC=+0.199 (n=2156)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `restante_min` < `3.79` → IC=+0.162 (n=518)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.159 (n=1410)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 7.0 (IC base=+0.152)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.159 (n=1610)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.164 (n=793)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.153 (n=710)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 7.0 (IC base=+0.146)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.156 (n=815)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` > `0.5682` → IC=+0.273 (n=20)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5682 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.784` → IC=+0.238 (n=590)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.784 (IC base=+0.104)

- **PATRÓN** `volumen_regimen` < `1.3288` → IC=+0.239 (n=151)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.3288 (IC base=+0.104)

- **PATRÓN** `volumen_regimen` > `0.7602` → IC=+0.243 (n=134)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7602 (IC base=+0.104)

- **PATRÓN** `volumen_pendiente_norm` > `0.327` → IC=+0.153 (n=96)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_pendiente_norm` > 0.327 (IC base=+0.104)

- **PATRÓN** `volumen_spike_ratio` > `1.6776` → IC=+0.127 (n=609)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_spike_ratio` > 1.6776 (IC base=+0.104)

- **PATRÓN** `ibs_20min` < `0.6585` → IC=+0.124 (n=2128)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` < 0.6585 (IC base=+0.080)

- **PATRÓN** `volumen_regimen` < `1.3145` → IC=+0.141 (n=480)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 1.3145 (IC base=+0.080)

- **PATRÓN** `volumen_regimen` > `0.6966` → IC=+0.143 (n=429)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 0.6966 (IC base=+0.080)
...
- **PATRÓN** `volumen_spike_ratio` > `2.9057` → IC=+0.237 (n=169)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9057 (IC base=+0.080)
...
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.172 (n=178)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.007 (IC base=+0.123)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.577` → IC=+0.342 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.577 (IC base=+0.123)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.163 (n=286)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.06 (IC base=+0.123)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.21` → IC=+0.131 (n=978)
  ...
  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` > 2.21 (IC base=+0.053)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8755` → IC=+0.172 (n=288)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 2.8755 (IC base=+0.053)
...
- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.191 (n=558)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.9211` → IC=+0.254 (n=819)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9211 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.2168` → IC=+0.132 (n=267)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.2168 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.418` → IC=+0.241 (n=817)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.418 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` > `0.6287` → IC=+0.134 (n=637)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` > 0.6287 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.114` → IC=+0.158 (n=404)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_pendiente_norm` > 0.114 (IC base=+0.136)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.143 (n=1378)

  - _Acción_: Kelly boost +0.71€ cuando `libro_spread` < 0.06 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `2663.0389` → IC=+0.165 (n=410)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 2663.0389 (IC base=+0.136)
...
- **PATRÓN** `ibs_20min` > `0.8872` → IC=+0.286 (n=204)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8872 (IC base=+0.151)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.482` → IC=+0.352 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.482 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` > `0.0922` → IC=+0.142 (n=93)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_pendiente_norm` > 0.0922 (IC base=+0.151)

- **PATRÓN** `libro_spread` < `0.08` → IC=+0.171 (n=332)
...
- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.272 (n=55)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0029 (IC base=+0.259)

- **PATRÓN** `drift_60min` |x|≤ `0.181` → IC=+0.310 (n=56)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.181 (IC base=+0.259)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.273 (n=86)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.259)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.263 (n=78)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.259)

- **PATRÓN** `ibs_20min` > `0.7638` → IC=+0.312 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7638 (IC base=+0.259)

- **PATRÓN** `dist_vwap_pct` < `0.3233` → IC=+0.284 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3233 (IC base=+0.259)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.946` → IC=+0.431 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.946 (IC base=+0.259)

- **PATRÓN** `volumen_regimen` > `0.6785` → IC=+0.265 (n=83)
...
- **PATRÓN** `volumen_spike_ratio` < `1.63` → IC=+0.286 (n=26)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.63 (IC base=+0.259)

- **PATRÓN** `volumen_spike_ratio` > `3.1675` → IC=+0.286 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.1675 (IC base=+0.259)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.256 (n=88)
...
- **PATRÓN** `sigma_h` > `0.0062` → IC=+0.245 (n=108)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0062 (IC base=+0.118)

- **PATRÓN** `drift_60min` |x|≤ `0.2124` → IC=+0.132 (n=210)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.66€ cuando `drift_60min` |x|≤ 0.2124 (IC base=+0.118)
...
- **PATRÓN** `dist_vwap_pct` > `0.2855` → IC=+0.171 (n=68)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.2855 (IC base=+0.118)

- **PATRÓN** `dist_vwap_pct` < `0.0656` → IC=+0.141 (n=168)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.0656 (IC base=+0.118)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.38` → IC=+0.224 (n=154)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.38 (IC base=+0.118)

- **PATRÓN** `volumen_regimen` < `1.1092` → IC=+0.133 (n=238)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 1.1092 (IC base=+0.118)

- **PATRÓN** `volumen_pendiente_norm` > `0.327` → IC=+0.190 (n=27)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.327 (IC base=+0.118)

- **PATRÓN** `volumen_spike_ratio` < `1.5866` → IC=+0.143 (n=68)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.5866 (IC base=+0.118)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.124 (n=243)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.194 (n=501)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0068 (IC base=+0.116)
...
- **PATRÓN** `dist_vwap_pct` > `0.3273` → IC=+0.165 (n=153)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.3273 (IC base=+0.116)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.29` → IC=+0.230 (n=1013)
...
- **PATRÓN** `libro_liquidez` > `2709.5617` → IC=+0.146 (n=501)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 2709.5617 (IC base=+0.116)

- **PATRÓN** `ballena_activa_n` < `152.0` → IC=+0.180 (n=126)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 152.0 (IC base=+0.116)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.365` → IC=+0.349 (n=144)
...
- **PATRÓN** `libro_spread` < `0.06` → IC=+0.170 (n=268)
...
- **PATRÓN** `sigma_h` > `0.0062` → IC=+0.201 (n=135)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0062 (IC base=+0.127)

- **PATRÓN** `drift_60min` |x|≤ `0.0717` → IC=+0.183 (n=99)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.0717 (IC base=+0.127)
...
- **PATRÓN** `dist_vwap_pct` > `0.3174` → IC=+0.263 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3174 (IC base=+0.127)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.293` → IC=+0.212 (n=196)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.293 (IC base=+0.127)

- **PATRÓN** `volumen_regimen` > `0.6753` → IC=+0.140 (n=265)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 0.6753 (IC base=+0.127)

- **PATRÓN** `volumen_pendiente_norm` > `0.327` → IC=+0.177 (n=29)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.327 (IC base=+0.127)
...
- **PATRÓN** `drift_20min_pct` |x|≤ `0.0531` → IC=+0.180 (n=73)
  ...
  - _Acción_: Kelly boost +0.90€ cuando `drift_20min_pct` |x|≤ 0.0531 (IC base=+0.077)
...
- **PATRÓN** `drift_20min_pct` |x|≤ `0.0655` → IC=+0.214 (n=19)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `drift_20min_pct` |x|≤ 0.0655 (IC base=+0.144)

- **PATRÓN** `ibs_20min` < `0.0407` → IC=+0.177 (n=29)

  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` < 0.0407 (IC base=+0.144)

- **PATRÓN** `ibs_20min` > `0.1574` → IC=+0.147 (n=15)

  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` > 0.1574 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `15301.6756` → IC=+0.147 (n=15)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 15301.6756 (IC base=+0.144)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.138 (n=147)

  - _Acción_: Kelly boost +0.69€ cuando `py_entrada` < 0.5 (IC base=+0.041)
...
- **PATRÓN** `ibs_20min` < `0.1477` → IC=+0.184 (n=36)

  - _Acción_: Kelly boost +0.92€ cuando `ibs_20min` < 0.1477 (IC base=+0.089)
...
- **PATRÓN** `drift_20min_pct` |x|≤ `0.1502` → IC=+0.154 (n=50)

  - _Acción_: Kelly boost +0.77€ cuando `drift_20min_pct` |x|≤ 0.1502 (IC base=+0.103)

- **PATRÓN** `ballena_activa_n` < `3.0` → IC=+0.160 (n=51)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 3.0 (IC base=+0.103)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.208 (n=22)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.5 (IC base=+0.050)

- **PATRÓN** `ballena_activa_n` < `1.0` → IC=+0.154 (n=24)

  - _Acción_: Kelly boost +0.77€ cuando `ballena_activa_n` < 1.0 (IC base=+0.050)

- **PATRÓN** `libro_liquidez` > `2706.3446` → IC=+0.136 (n=20)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 2706.3446 (IC base=+0.050)
...
- **FILTRO** `ibs_7min` > `0.243` → IC=-0.197 (n=31)
...
- **PATRÓN** `py_entrada` < `0.53` → IC=+0.130 (n=44)
  ...
  - _Acción_: Kelly boost +0.65€ cuando `py_entrada` < 0.53 (IC base=-0.028)
...
- **FILTRO** `drift_7min_pct` |x|> `0.0631` → IC=-0.140 (n=48)
...
- **FILTRO** `py_entrada` > `0.6` → IC=-0.268 (n=67)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.157 (n=65)
  ...
  - _Acción_: Kelly boost +0.78€ cuando `py_entrada` < 0.5 (IC base=-0.028)
...
- **PATRÓN** `ibs_15` < `0.5778` → IC=+0.130 (n=384)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.65€ cuando `ibs_15` < 0.5778 (IC base=+0.083)

- **PATRÓN** `dist_vwap_pct` > `0.4544` → IC=+0.278 (n=34)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4544 (IC base=+0.083)
...
- **PATRÓN** `sigma_h` > `0.0062` → IC=+0.154 (n=79)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0062 (IC base=+0.113)

- **PATRÓN** `drift_60min` |x|≤ `0.14` → IC=+0.191 (n=40)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.14 (IC base=+0.113)

- **PATRÓN** `drift_15min` |x|≤ `0.7498` → IC=+0.128 (n=119)

  - _Acción_: Kelly boost +0.64€ cuando `drift_15min` |x|≤ 0.7498 (IC base=+0.113)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0789` → IC=+0.161 (n=119)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.81€ cuando `delta_ratio_macro` |x|> 0.0789 (IC base=+0.113)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.154 (n=108)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` < 15.0 (IC base=+0.113)

- **PATRÓN** `ibs_15` < `0.234` → IC=+0.220 (n=80)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.234 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `0.1168` → IC=+0.339 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1168 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.148` → IC=+0.163 (n=99)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` < 5.148 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `2536.0733` → IC=+0.161 (n=119)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 2536.0733 (IC base=+0.113)
...
- **FILTRO** `sigma_ewma_delta_pct` > `12.796` → IC=-0.156 (n=222)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=888)
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.234 sube el IC de +0.113 a +0.220 en UPDOWN_GBM#XRP#15min (n=80). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 3304 | -0.114 | -514.05€ | 2 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 455 | -0.021 | -16.13€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 2849 | -0.129 | -497.93€ | 0 | 0 |
...
| 🚫 BALLENAS_TARDIAS#XRP | 664 | -0.219 | -115.79€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP#5min | 664 | -0.219 | -115.79€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 14144 | +0.117 | -781.85€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3354 | +0.180 | -113.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 99 | -0.104 | -47.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 8103 | +0.089 | -643.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2588 | +0.132 | +22.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 1369 | +0.033 | -297.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 13 | -0.065 | -2.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 1352 | +0.036 | -289.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 3229 | +0.142 | +5.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 916 | +0.196 | -33.36€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 41 | -0.105 | -20.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 1350 | +0.114 | -4.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 922 | +0.140 | +64.54€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 1365 | +0.057 | -234.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 9 | -0.021 | -3.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 1355 | +0.058 | -228.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 3490 | +0.127 | -44.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1238 | +0.159 | -27.10€ | 0 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 1347 | +0.107 | -12.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min |
 | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#15min | 245 | -0.095 | -26.38€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_15M_FADE#XRP | 43 | -0.033 | -1.78€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#XRP#15min | 43 | -0.033 | -1.78€ | 1 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 1770 | -0.076 | -6.48€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 1770 | -0.076 | -6.48€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 265 | -0.081 | +8.40€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 265 | -0.081 | +8.40€ | 5 | 1 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 336 | -0.065 | +79.75€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 336 | -0.065 | +79.75€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 279 | -0.101 | -42.01€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 279 | -0.101 | -42.01€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 284 | -0.101 | -40.40€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 284 | -0.101 | -40.40€ | 8 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 320 | -0.043 | +0.52€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 320 | -0.043 | +0.52€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 286 | -0.069 | -12.74€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 286 | -0.069 | -12.74€ | 5 | 1 |
| ✅ MOMENTUM_IBS_5M_FADE | 1673 | +0.018 | +12.39€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 1673 | +0.018 | +12.39€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB | 285 | +0.026 | +9.25€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB#5min | 285 | +0.026 | +9.25€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC | 241 | +0.039 | +3.79€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC#5min | 241 | +0.039 | +3.79€ | 2 | 1 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE | 288 | +0.010 | -0.19€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE#5min | 288 | +0.010 | -0.19€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_5M_FADE#XRP | 276 | +0.007 | -3.39€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP#5min | 276 | +0.007 | -3.39€ | 0 | 0 |
...
| ✅ STRUCT_NO_15M | 1829 | +0.016 | -4.12€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 1829 | +0.016 | -4.12€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 699 | +0.006 | -8.45€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 699 | +0.006 | -8.45€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 719 | +0.020 | +1.49€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 719 | +0.020 | +1.49€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 411 | +0.023 | +2.84€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 411 | +0.023 | +2.84€ | 2 | 0 |
| ✅ UPDOWN_GBM | 2126 | +0.022 | +125.62€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 926 | +0.089 | +170.78€ | 1 | 6 |
...
| ✅ UPDOWN_GBM#XRP | 471 | +0.029 | +62.97€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 285 | +0.099 | +84.76€ | 0 | 16 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1627 | -0.044 | +187.24€ | 2 | 2 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1627 | -0.044 | +187.24€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 505 | -0.031 | +72.45€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 505 | -0.031 | +72.45€ | 3 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.141) — sin ventaja clara. oversold(IBS<0.3): IC=-0.022 n=745 | neutral: IC=+0.007 n=716 | overbought(IBS>0.7): IC=+0.119 n=964
  - _Datos_: n=2599 IC=+0.042 PNL=+207.02€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=14h: IC=+0.120 n=106 PNL=+29.73€ → BOOST | H=23h: IC=-0.149 n=112 PNL=-29.64€ → FILTRAR
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.106 n=37693 | tras_1loss IC=+0.062 n=26384 | tras_2loss IC=+0.021 n=11499/40 | gap=+0.085 (umbral 0.05)
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  ...
  - _Estado_: 0 celda(s) pasan gate riguroso completo de 30 evaluadas (n>=40) y 259 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1955 IC=+0.017 PNL=+94.11€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1955 IC=+0.017 PNL=+94.11€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.084 > 0.08 con n=577 PNL=+79.47€
  - _Datos_: n=577 IC=+0.084 PNL=+79.47€
...
**🟡 H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.101 > 0.1 con n=429 PNL=+115.78€
  - _Datos_: n=429 IC=+0.101 PNL=+115.78€
...
**🟡 H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.113 > 0.1 con n=158 PNL=+52.81€
  - _Datos_: n=158 IC=+0.113 PNL=+52.81€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1269 IC=-0.154 PNL=+37.86€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1269 IC=-0.154 PNL=+37.86€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=781 IC=+0.158 PNL=+414.15€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=781 IC=+0.158 PNL=+414.15€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=1078 IC=+0.099 PNL=-240.75€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=1078 IC=+0.099 PNL=-240.75€
