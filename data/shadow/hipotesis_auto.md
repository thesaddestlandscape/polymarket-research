# Hipótesis automáticas — 2026-08-24 07:37 UTC
_Generado por shadow_postmortem.py sobre 135643 resoluciones (PNL=+9409.66€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `157.35` → IC=-0.256 (n=1893)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 157.35
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=5680)
...
- **FILTRO** `restante_s_al_confirmar` < `109.47` → IC=-0.389 (n=215)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 109.47
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=647)

### BALLENAS_TARDIAS#ETH#5min
- **FILTRO** `restante_s_al_confirmar` < `169.3` → IC=-0.147 (n=500)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 169.3
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=1501)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `148.13` → IC=-0.272 (n=468)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 148.13
  - _Potencial_: sin este filtro IC_bueno=+0.006 (n=1410)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `170.8` → IC=-0.299 (n=451)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 170.8
  - _Potencial_: sin este filtro IC_bueno=-0.108 (n=917)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.150 (n=3560)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` < 7.0 (IC base=+0.135)

- **PATRÓN** `py_entrada` < `0.335` → IC=+0.262 (n=2559)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.335 (IC base=+0.135)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.184 (n=2644)

  - _Acción_: Kelly boost +0.92€ cuando `libro_spread` < 0.02 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `1774.5918` → IC=+0.173 (n=2183)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 1774.5918 (IC base=+0.135)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.289 (n=400)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.284)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.292 (n=398)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.284)

- **PATRÓN** `py_entrada` < `0.375` → IC=+0.324 (n=385)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.375 (IC base=+0.284)

- **PATRÓN** `libro_liquidez` > `2729.7839` → IC=+0.301 (n=254)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2729.7839 (IC base=+0.284)
...
- **PATRÓN** `py_entrada` < `0.305` → IC=+0.301 (n=349)
...
- **PATRÓN** `libro_liquidez` > `803.4863` → IC=+0.231 (n=519)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 803.4863 (IC base=+0.213)
...
- **PATRÓN** `restante_min` > `4.86` → IC=+0.161 (n=779)

  - _Acción_: Kelly boost +0.80€ cuando `restante_min` > 4.86 (IC base=+0.141)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.155 (n=1505)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` < 15.0 (IC base=+0.141)

- **PATRÓN** `lag_apertura_s` < `6.36` → IC=+0.166 (n=569)

  - _Acción_: Kelly boost +0.83€ cuando `lag_apertura_s` < 6.36 (IC base=+0.141)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.017` → IC=+0.191 (n=774)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 5.017 (IC base=+0.070)
...
- **PATRÓN** `sigma_h` > `0.008` → IC=+0.146 (n=780)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.339` → IC=+0.123 (n=1331)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1157` → IC=+0.159 (n=997)
...
- **PATRÓN** `volumen_spike_ratio` > `2.9207` → IC=+0.171 (n=344)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.9207 (IC base=+0.052)
...
- **PATRÓN** `volumen_regimen` > `0.63` → IC=+0.201 (n=649)
...
- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.247 (n=299)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3868` → IC=+0.270 (n=420)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.3654` → IC=+0.306 (n=478)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3654 (IC base=+0.260)
...
- **PATRÓN** `volumen_regimen` > `0.715` → IC=+0.278 (n=426)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1844` → IC=+0.279 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1844 (IC base=+0.260)

- **PATRÓN** `ballena_activa_n` < `39.0` → IC=+0.233 (n=223)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3449` → IC=+0.195 (n=1743)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.3985` → IC=+0.244 (n=1743)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.471` → IC=+0.186 (n=1762)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` < 5.471 (IC base=+0.185)

- **PATRÓN** `volumen_regimen` < `1.1795` → IC=+0.162 (n=1443)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.206 (n=769)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_regimen` < `0.7083` → IC=+0.148 (n=695)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.7083 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` > `1.0839` → IC=+0.142 (n=716)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 1.0839 (IC base=+0.139)
...
- **PATRÓN** `volumen_spike_ratio` > `1.9188` → IC=+0.150 (n=1326)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.9188 (IC base=+0.139)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.145 (n=1826)
...
- **PATRÓN** `ballena_activa_n` < `166.0` → IC=+0.187 (n=818)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3962` → IC=+0.200 (n=2062)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5417` → IC=+0.241 (n=2062)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5417 (IC base=+0.185)

- **PATRÓN** `dist_vwap_pct` < `0.6747` → IC=+0.175 (n=1667)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.61` → IC=+0.192 (n=1935)
...
- **PATRÓN** `volumen_regimen` > `1.1867` → IC=+0.179 (n=527)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` > 1.1867 (IC base=+0.185)

- **PATRÓN** `volumen_pendiente_norm` > `0.2334` → IC=+0.236 (n=282)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2334 (IC base=+0.185)

- **PATRÓN** `volumen_spike_ratio` > `2.2671` → IC=+0.195 (n=621)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 2.2671 (IC base=+0.185)

- **PATRÓN** `ballena_activa_n` < `57.0` → IC=+0.138 (n=631)
...
- **PATRÓN** `sigma_h` < `0.0087` → IC=+0.142 (n=342)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0087 (IC base=+0.128)
...
- **PATRÓN** `ibs_20min` > `0.3557` → IC=+0.195 (n=342)
  ...
  - _Acción_: Kelly boost +0.97€ cuando `ibs_20min` > 0.3557 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` > `0.9583` → IC=+0.200 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9583 (IC base=+0.128)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.499` → IC=+0.182 (n=168)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 4.499 (IC base=+0.128)
...
- **PATRÓN** `volumen_regimen` > `1.2091` → IC=+0.155 (n=114)
  ...
  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 1.2091 (IC base=+0.128)
...
- **PATRÓN** `libro_liquidez` > `4224.1911` → IC=+0.196 (n=228)
  ...
  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 4224.1911 (IC base=+0.128)

- **PATRÓN** `ballena_activa_n` < `232.0` → IC=+0.167 (n=214)
...
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.163 (n=182)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.0087 (IC base=+0.078)
...
- **PATRÓN** `dist_vwap_pct` > `0.9263` → IC=+0.244 (n=84)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9263 (IC base=+0.078)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3903` → IC=+0.233 (n=478)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3903 (IC base=+0.208)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.224 (n=498)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.208)

- **PATRÓN** `ibs_20min` < `0.4884` → IC=+0.280 (n=544)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4884 (IC base=+0.208)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.936` → IC=+0.216 (n=548)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.936 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` > `1.233` → IC=+0.249 (n=181)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6664` → IC=+0.220 (n=123)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6664 (IC base=+0.208)

- **PATRÓN** `ballena_activa_n` < `36.0` → IC=+0.147 (n=256)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.130 (n=90)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` < 18.0 (IC base=+0.102)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.163 (n=81)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=+0.039 (n=265)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.153 (n=572)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.042 (n=1777)
...
- **FILTRO** `ibs_20min` > `0.2778` → IC=-0.204 (n=569)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2778
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=1714)
...
- **FILTRO** `py_entrada` < `0.495` → IC=-0.139 (n=106)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.030 (n=315)
...
- **FILTRO** `py_entrada` < `0.36` → IC=-0.278 (n=1369)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=4344)

- **FILTRO** `ibs_7min` < `0.7411` → IC=-0.227 (n=1428)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7411
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=4285)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.180 (n=1907)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=3806)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.219 (n=1707)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=5302)
...
- **FILTRO** `py_entrada` > `0.71` → IC=-0.243 (n=274)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=861)

- **FILTRO** `drift_7min_pct` |x|> `0.1413` → IC=-0.159 (n=385)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1413
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=750)

- **FILTRO** `ibs_7min` > `0.2976` → IC=-0.187 (n=385)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.160 (n=248)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=944)
...
- **FILTRO** `ballena_activa_n` > `131.0` → IC=-0.176 (n=297)
...
- **FILTRO** `ibs_7min` > `0.2771` → IC=-0.161 (n=411)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.185 (n=211)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=758)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.266 (n=229)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=740)

- **FILTRO** `ibs_7min` < `0.7797` → IC=-0.201 (n=242)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7797
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=727)

- **FILTRO** `ballena_activa_n` > `43.0` → IC=-0.218 (n=236)
...
- **FILTRO** `py_entrada` < `0.38` → IC=-0.270 (n=228)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=+0.030 (n=700)

- **FILTRO** `ibs_7min` < `0.75` → IC=-0.219 (n=222)
...
- **FILTRO** `sigma_h` > `0.0089` → IC=-0.148 (n=749)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0089
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=1456)
...
- **FILTRO** `drift_60min` |x|> `0.5025` → IC=-0.134 (n=244)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.5025
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=476)

- **FILTRO** `drift_15min` |x|> `1.3957` → IC=-0.158 (n=179)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 1.3957
...
- **FILTRO** `sigma_ewma_delta_pct` > `15.17` → IC=-0.214 (n=89)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=631)
...
| ✅ BALLENAS_TARDIAS | 5794 | -0.099 | -674.33€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 619 | -0.083 | -108.36€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 5175 | -0.101 | -565.97€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 1878 | -0.064 | -54.46€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 1878 | -0.064 | -54.46€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1368 | -0.172 | -216.30€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1368 | -0.172 | -216.30€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 25870 | +0.113 | -1789.73€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 5046 | +0.185 | -185.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 106 | -0.102 | -48.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 17734 | +0.091 | -1539.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2984 | +0.124 | -16.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 2982 | +0.058 | -544.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 18 | -0.045 | +2.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 2959 | +0.060 | -540.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 5444 | +0.134 | -123.56€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1409 | +0.193 | -91.34€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 2950 | +0.109 | -72.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1043 | +0.134 | +62.46€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 2985 | +0.074 | -421.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 18 | +0.045 | -1.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 2966 | +0.075 | -418.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 5856 | +0.129 | -62.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1840 | +0.169 | -4.72€ | 0 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 2951 | +0.113 | -39.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1053 | +0.108 | -9.30€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 5621 | +0.127 | -425.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1745 | +0.202 | -90.04€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 45 | +0.011 | -8.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 2943 | +0.083 | -258.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 888 | +0.132 | -69.63€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 2982 | +0.106 | -211.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 16 | +0.000 | +0.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 2965 | +0.107 | -210.34€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 4565 | +0.130 | +111.65€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 4565 | +0.130 | +111.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 2266 | +0.141 | +98.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 2266 | +0.141 | +98.31€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 2299 | +0.120 | +13.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 2299 | +0.120 | +13.34€ | 0 | 5 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 3906 | +0.098 | -153.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 402 | +0.047 | -28.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 3504 | +0.103 | -125.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC | 2694 | +0.096 | -78.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#15min | 402 | +0.047 | -28.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#5min | 2292 | +0.105 | -50.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL | 1212 | +0.101 | -74.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL#5min | 1212 | +0.101 | -74.87€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 6369 | +0.046 | +1939.17€ | 0 | 18 |
| ✅ GBM_LATE_15M#15min | 6369 | +0.046 | +1939.17€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#ETH | 1005 | -0.057 | -13.09€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1005 | -0.057 | -13.09€ | 4 | 3 |
| ✅ GBM_LATE_15M#SOL | 1231 | -0.037 | +146.68€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1231 | -0.037 | +146.68€ | 3 | 3 |
| ✅ GBM_LATE_15M#XRP | 1575 | -0.039 | +221.21€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1575 | -0.039 | +221.21€ | 4 | 4 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 7329 | +0.042 | +2744.81€ | 0 | 17 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 7329 | +0.042 | +2744.81€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 1126 | -0.028 | +510.46€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 1126 | -0.028 | +510.46€ | 1 | 3 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 1595 | -0.037 | +155.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 1595 | -0.037 | +155.25€ | 1 | 2 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 1344 | -0.013 | +278.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 1344 | -0.013 | +278.25€ | 3 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 1264 | +0.233 | +1129.93€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 1264 | +0.233 | +1129.93€ | 0 | 22 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 4994 | +0.170 | +3413.11€ | 0 | 25 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 4994 | +0.170 | +3413.11€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP | 1108 | +0.196 | +856.21€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP#15min | 1108 | +0.196 | +856.21€ | 0 | 25 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 817 | +0.065 | +126.07€ | 0 | 10 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 817 | +0.065 | +126.07€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BTC | 208 | +0.095 | +56.07€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BTC#15min | 208 | +0.095 | +56.07€ | 4 | 15 |
...
| ✅ GBM_LATE_15M_TARDIO | 5817 | +0.161 | +3665.01€ | 0 | 25 |
| ✅ GBM_LATE_15M_TARDIO#15min | 