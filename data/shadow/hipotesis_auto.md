# Hipótesis automáticas — 2026-07-20 01:21 UTC
_Generado por shadow_postmortem.py sobre 23560 resoluciones (PNL=+6036.30€)_
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.236 (n=327)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.233 (n=403)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.254 (n=116)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.124` → IC=+0.131 (n=673)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.124 (IC base=+0.118)
...
- **PATRÓN** `dist_vwap_pct` < `0.819` → IC=+0.127 (n=1187)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `11.058` → IC=+0.124 (n=1066)
...
- **PATRÓN** `dist_vwap_pct` < `0.4662` → IC=+0.137 (n=243)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.4662 (IC base=+0.076)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.004` → IC=+0.188 (n=107)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 3.004 (IC base=+0.076)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.382` → IC=+0.222 (n=95)
...
- **PATRÓN** `sigma_h` < `0.0268` → IC=+0.158 (n=580)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0268 (IC base=+0.158)

- **PATRÓN** `sigma_h` > `0.0202` → IC=+0.179 (n=263)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0202 (IC base=+0.158)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.163 (n=520)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 8.0 (IC base=+0.158)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.160 (n=515)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 15.0 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` > `0.5541` → IC=+0.222 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5541 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` < `0.1104` → IC=+0.164 (n=227)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.1104 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.792` → IC=+0.283 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.792 (IC base=+0.158)
...
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.206 (n=771)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0081 (IC base=+0.166)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.190 (n=771)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 13.0 (IC base=+0.166)

- **PATRÓN** `dist_vwap_pct` > `0.5164` → IC=+0.221 (n=213)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5164 (IC base=+0.166)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.147` → IC=+0.298 (n=176)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.147 (IC base=+0.166)
...
- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.137 (n=271)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0062 (IC base=+0.134)

- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.156 (n=242)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` > 0.0029 (IC base=+0.134)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.179 (n=185)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 13.0 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` > `0.5248` → IC=+0.207 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5248 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.371` → IC=+0.223 (n=45)
...
- **PATRÓN** `sigma_h` > `0.008` → IC=+0.250 (n=98)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.149 (n=303)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` < 18.0 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `0.4217` → IC=+0.180 (n=73)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.4217 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` < `0.9779` → IC=+0.143 (n=256)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.9779 (IC base=+0.146)
...
- **PATRÓN** `sigma_h` > `0.0118` → IC=+0.189 (n=146)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0118 (IC base=+0.129)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.172 (n=114)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 18.0 (IC base=+0.129)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.132 (n=112)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.574` → IC=+0.295 (n=125)
...
- **PATRÓN** `sigma_h` < `0.0164` → IC=+0.261 (n=274)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0164 (IC base=+0.260)

- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.275 (n=273)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0084 (IC base=+0.260)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.263 (n=259)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.260)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.262 (n=280)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.260)

- **PATRÓN** `dist_vwap_pct` > `0.5559` → IC=+0.346 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5559 (IC base=+0.260)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.293` → IC=+0.365 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.293 (IC base=+0.260)
...
- **PATRÓN** `sigma_h` < `0.0052` → IC=+0.137 (n=356)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0052 (IC base=+0.133)

- **PATRÓN** `sigma_h` > `0.0131` → IC=+0.187 (n=356)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0131 (IC base=+0.133)
...
- **PATRÓN** `dist_vwap_pct` > `0.4799` → IC=+0.176 (n=217)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.4799 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `0.1124` → IC=+0.163 (n=571)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.1124 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.182` → IC=+0.296 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.182 (IC base=+0.133)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.120 (n=820)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` < 12.0 (IC base=+0.108)

- **PATRÓN** `dist_vwap_pct` < `0.7126` → IC=+0.130 (n=1043)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.7126 (IC base=+0.108)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `11.13` → IC=+0.126 (n=942)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 11.13 (IC base=+0.108)
...
- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.123 (n=282)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` < 0.0064 (IC base=+0.081)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.140 (n=98)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 17.0 (IC base=+0.081)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.206` → IC=+0.136 (n=105)

  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` > 3.206 (IC base=+0.081)
...
- **PATRÓN** `dist_vwap_pct` < `0.2441` → IC=+0.128 (n=146)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.2441 (IC base=+0.077)
...
- **PATRÓN** `sigma_h` > `0.0136` → IC=+0.133 (n=77)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0136 (IC base=+0.088)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.136 (n=105)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` < 8.0 (IC base=+0.088)

- **PATRÓN** `dist_vwap_pct` > `0.289` → IC=+0.129 (n=60)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` > 0.289 (IC base=+0.088)

- **PATRÓN** `dist_vwap_pct` < `0.1211` → IC=+0.135 (n=113)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.1211 (IC base=+0.088)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.452` → IC=+0.291 (n=89)
...
- **PATRÓN** `sigma_h` < `0.0255` → IC=+0.211 (n=337)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0255 (IC base=+0.212)

- **PATRÓN** `sigma_h` > `0.0103` → IC=+0.223 (n=301)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.5537` → IC=+0.268 (n=54)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5537 (IC base=+0.212)

- **PATRÓN** `dist_vwap_pct` < `0.0986` → IC=+0.221 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0986 (IC base=+0.212)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.155` → IC=+0.302 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.155 (IC base=+0.212)
...
- **PATRÓN** `ibs_15` < `0.0303` → IC=+0.132 (n=188)
  ...
  - _Acción_: Kelly boost +0.66€ cuando `ibs_15` < 0.0303 (IC base=+0.055)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.492` → IC=+0.124 (n=296)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 10.492 (IC base=+0.055)
...
- **PATRÓN** `dist_vwap_pct` > `0.9422` → IC=+0.182 (n=20)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.9422 (IC base=+0.048)

- **PATRÓN** `sigma_ewma_delta_pct` < `8.241` → IC=+0.123 (n=104)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` < 8.241 (IC base=+0.048)
...
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min` — IC=+0.235 n=32. Faltan ~8 resoluciones para umbral n≥40. ETA: ~6h.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC` — IC=+0.235 n=32. Faltan ~8 resoluciones para umbral n≥40. ETA: ~6h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 377 | +0.136 | +12.51€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 377 | +0.136 | +12.51€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 4027 | +0.186 | +63.23€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 2623 | +0.218 | +96.46€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 