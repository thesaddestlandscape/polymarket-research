# Hipótesis automáticas — 2026-07-16 11:33 UTC
_Generado por shadow_postmortem.py sobre 16885 resoluciones (PNL=+3438.85€)_
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.196 (n=436)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` < 7.0 (IC base=+0.168)

- **PATRÓN** `py_entrada` < `0.265` → IC=+0.333 (n=345)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.265 (IC base=+0.168)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.178 (n=1127)

  - _Acción_: Kelly boost +0.89€ cuando `libro_spread` < 0.01 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `7351.4655` → IC=+0.182 (n=328)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 7351.4655 (IC base=+0.168)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.224 (n=154)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 12.0 (IC base=+0.206)

- **PATRÓN** `py_entrada` < `0.235` → IC=+0.390 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.235 (IC base=+0.206)

- **PATRÓN** `libro_liquidez` > `6674.7419` → IC=+0.207 (n=213)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 6674.7419 (IC base=+0.206)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.128 (n=606)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.1391` → IC=+0.135 (n=258)
...
- **PATRÓN** `dist_vwap_pct` > `0.4395` → IC=+0.167 (n=19)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.4395 (IC base=+0.081)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.755` → IC=+0.182 (n=20)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 13.755 (IC base=+0.081)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2538 | +0.173 | -71.72€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 1635 | +0.209 | +6.53€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 736 | +0.179 | -18.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 547 | +0.203 | -16.47€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 4872 | +0.102 | +1517.99€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 4872 | +0.102 | +1517.99€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1167 | +0.063 | +162.51€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1167 | +0.063 | +162.51€ | 0 | 4 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2094 | +0.148 | +1211.04€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2094 | +0.148 | +1211.04€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 535 | +0.109 | +260.13€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 535 | +0.109 | +260.13€ | 0 | 8 |
...
| ✅ GBM_LATE_15M_TARDIO | 2071 | +0.091 | +620.25€ | 0 | 7 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2071 | +0.091 | +620.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 511 | +0.056 | +83.00€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 511 | +0.056 | +83.00€ | 0 | 3 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9823 | tras_1loss IC=+0.069 n=6853 | tras_2loss IC=+0.038 n=2927/40 | gap=+0.064 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=14427 IC=+0.105 PNL=+3514.74€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=14427 IC=+0.105 PNL=+3514.74€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema, sin filtrar todavía
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.183 < -0.1 con n=402 PNL=-86.07€
  - _Datos_: n=402 IC=-0.183 PNL=-86.07€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.106 > 0.08 con n=153 PNL=+56.33€
  - _Datos_: n=153 IC=+0.106 PNL=+56.33€
