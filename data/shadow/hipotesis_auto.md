# Hipótesis automáticas — 2026-08-26 21:09 UTC
_Generado por shadow_postmortem.py sobre 166279 resoluciones (PNL=+11915.50€)_
...
- **FILTRO** `restante_s_al_confirmar` < `148.19` → IC=-0.283 (n=2297)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=6894)
...
- **FILTRO** `restante_s_al_confirmar` < `158.28` → IC=-0.193 (n=626)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=1881)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.186 (n=5242)

  - _Acción_: Kelly boost +0.93€ cuando `py_entrada` > 0.7 (IC base=+0.096)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.172 (n=1555)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.149 (n=3525)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.190 (n=2884)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.02 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `1849.7055` → IC=+0.183 (n=2412)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 1849.7055 (IC base=+0.141)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.198 (n=561)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.187 (n=797)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.01 (IC base=+0.187)

- **PATRÓN** `libro_liquidez` > `12142.401` → IC=+0.216 (n=206)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12142.401 (IC base=+0.187)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.133 (n=1190)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.195 (n=938)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.190 (n=4415)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.161 (n=1129)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.239 (n=1007)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.194 (n=10690)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.219 (n=8344)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.242 (n=1850)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.175 (n=782)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `restante_min` < `3.91` → IC=+0.142 (n=713)

  - _Acción_: Kelly boost +0.71€ cuando `restante_min` < 3.91 (IC base=+0.142)
...
- **PATRÓN** `lag_apertura_s` < `5.61` → IC=+0.176 (n=709)

  - _Acción_: Kelly boost +0.88€ cuando `lag_apertura_s` < 5.61 (IC base=+0.142)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.303 (n=282)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.03` → IC=+0.301 (n=339)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.03 (IC base=+0.285)

- **PATRÓN** `libro_liquidez` > `1563.731` → IC=+0.340 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1563.731 (IC base=+0.285)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9479` → IC=+0.206 (n=726)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.219` → IC=+0.170 (n=891)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 5.219 (IC base=+0.063)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3067` → IC=+0.135 (n=165)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_pendiente_norm` > 0.3067 (IC base=+0.063)

- **PATRÓN** `volumen_spike_ratio` < `2.4442` → IC=+0.139 (n=1024)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 2.4442 (IC base=+0.063)

- **PATRÓN** `ibs_20min` < `0.2131` → IC=+0.131 (n=1485)

  - _Acción_: Kelly boost +0.65€ cuando `ibs_20min` < 0.2131 (IC base=+0.031)

- **PATRÓN** `dist_vwap_pct` < `0.3016` → IC=+0.146 (n=750)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.3016 (IC base=+0.031)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6146` → IC=+0.201 (n=336)
...
- **PATRÓN** `ballena_activa_n` < `44.0` → IC=+0.243 (n=220)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 44.0 (IC base=+0.031)
...
- **PATRÓN** `sigma_h` < `0.0021` → IC=+0.195 (n=139)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0021 (IC base=+0.142)

- **PATRÓN** `drift_60min` |x|≤ `0.1677` → IC=+0.161 (n=278)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.1677 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.147 (n=372)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 8.0 (IC base=+0.142)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.151 (n=437)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 18.0 (IC base=+0.142)

- **PATRÓN** `ibs_20min` < `0.5876` → IC=+0.168 (n=416)

  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` < 0.5876 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` < `0.1429` → IC=+0.171 (n=350)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` < 0.1429 (IC base=+0.142)
...
- **PATRÓN** `volumen_regimen` < `0.6331` → IC=+0.188 (n=139)
  ...
  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` < 0.6331 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` > `1.0206` → IC=+0.144 (n=189)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 1.0206 (IC base=+0.142)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7427` → IC=+0.182 (n=209)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 1.7427 (IC base=+0.142)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.142 (n=537)

  - _Acción_: Kelly boost +0.71€ cuando `libro_spread` < 0.01 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `14087.0235` → IC=+0.181 (n=139)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 14087.0235 (IC base=+0.142)

- **PATRÓN** `ballena_activa_n` < `229.0` → IC=+0.188 (n=75)

  - _Acción_: Kelly boost +0.94€ cuando `ballena_activa_n` < 229.0 (IC base=+0.142)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.271 (n=181)
...
- **PATRÓN** `dist_vwap_pct` > `0.1404` → IC=+0.326 (n=21)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1404 (IC base=-0.028)

- **PATRÓN** `dist_vwap_pct` < `0.278` → IC=+0.306 (n=34)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.278 (IC base=-0.028)

- **PATRÓN** `volumen_regimen` < `0.8497` → IC=+0.321 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8497 (IC base=-0.028)

- **PATRÓN** `volumen_regimen` > `0.7485` → IC=+0.306 (n=34)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7485 (IC base=-0.028)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.132 (n=1250)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0075 (IC base=+0.052)

- **PATRÓN** `ibs_20min` > `0.9429` → IC=+0.246 (n=918)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9429 (IC base=+0.052)

- **PATRÓN** `dist_vwap_pct` > `1.2538` → IC=+0.287 (n=219)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2538 (IC base=+0.052)

- **PATRÓN** `volumen_regimen` > `1.1556` → IC=+0.195 (n=316)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 1.1556 (IC base=+0.052)

- **PATRÓN** `volumen_pendiente_norm` < `0.1149` → IC=+0.165 (n=1221)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` < 0.1149 (IC base=+0.052)
...
- **PATRÓN** `volumen_spike_ratio` > `2.85` → IC=+0.171 (n=418)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 2.85 (IC base=+0.052)

- **PATRÓN** `ballena_activa_n` < `100.0` → IC=+0.264 (n=701)
...
- **PATRÓN** `drift_60min` |x|≤ `0.099` → IC=+0.209 (n=187)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.099 (IC base=+0.178)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0195` → IC=+0.173 (n=151)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.0195 (IC base=+0.178)

- **PATRÓN** `volumen_spike_ratio` > `3.6591` → IC=+0.177 (n=156)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 3.6591 (IC base=+0.178)
...
- **PATRÓN** `ballena_activa_n` < `58.0` → IC=+0.189 (n=117)

  - _Acción_: Kelly boost +0.95€ cuando `ballena_activa_n` < 58.0 (IC base=+0.178)
...
- **PATRÓN** `ibs_20min` > `0.8511` → IC=+0.252 (n=155)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8511 (IC base=-0.013)

- **PATRÓN** `dist_vwap_pct` > `0.3163` → IC=+0.253 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3163 (IC base=-0.013)

- **PATRÓN** `volumen_regimen` < `0.8598` → IC=+0.183 (n=118)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 0.8598 (IC base=-0.013)
...
- **PATRÓN** `volumen_spike_ratio` > `2.0538` → IC=+0.175 (n=75)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.0538 (IC base=-0.013)

- **PATRÓN** `ballena_activa_n` < `53.0` → IC=+0.214 (n=89)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3255` → IC=+0.155 (n=2089)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.3255 (IC base=+0.153)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.175 (n=843)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.282 (n=1175)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` > `0.7837` → IC=+0.251 (n=545)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7837 (IC base=+0.153)
...
- **PATRÓN** `volumen_regimen` > `0.6824` → IC=+0.169 (n=1474)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2818` → IC=+0.151 (n=1872)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 2.2818 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` > `1.8513` → IC=+0.146 (n=1418)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.8513 (IC base=+0.153)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.165 (n=1944)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.153)

- **PATRÓN** `libro_liquidez` > `3936.5622` → IC=+0.188 (n=792)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3936.5622 (IC base=+0.153)

- **PATRÓN** `ballena_activa_n` < `125.0` → IC=+0.172 (n=1131)

  - _Acción_: Kelly boost +0.86€ cuando `ballena_activa_n` < 125.0 (IC base=+0.153)

- **PATRÓN** `sigma_h` < `0.0074` → IC=+0.201 (n=1872)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0074 (IC base=+0.185)

- **PATRÓN** `drift_60min` |x|≤ `0.4068` → IC=+0.199 (n=2125)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4068 (IC base=+0.185)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.215 (n=812)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.185)

- **PATRÓN** `ibs_20min` < `0.4097` → IC=+0.238 (n=2125)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4097 (IC base=+0.185)

- **PATRÓN** `dist_vwap_pct` < `0.3585` → IC=+0.177 (n=1866)
...
- **PATRÓN** `volumen_regimen` < `1.1777` → IC=+0.166 (n=1720)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.1777 (IC base=+0.185)

- **PATRÓN** `volumen_regimen` > `0.8615` → IC=+0.173 (n=1147)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` > 0.8615 (IC base=+0.185)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8765` → IC=+0.178 (n=1010)
...
- **PATRÓN** `ballena_activa_n` < `202.0` → IC=+0.183 (n=1085)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7385` → IC=+0.174 (n=216)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.7385 (IC base=+0.151)

- **PATRÓN** `libro_liquidez` > `14231.3167` → IC=+0.158 (n=144)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 14231.3167 (IC base=+0.151)

- **PATRÓN** `ballena_activa_n` < `239.0` → IC=+0.179 (n=79)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 239.0 (IC base=+0.151)

### GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min
- **PATRÓN** `drift_60min` |x|≤ `0.144` → IC=+0.158 (n=229)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.144 (IC base=+0.148)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.157 (n=135)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.289 (n=192)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2288` → IC=+0.144 (n=282)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_pendiente_norm` < 0.2288 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` < `1.9992` → IC=+0.201 (n=125)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9992 (IC base=+0.148)
...
- **PATRÓN** `sigma_h` < `0.0093` → IC=+0.218 (n=338)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0093 (IC base=+0.181)

- **PATRÓN** `drift_60min` |x|≤ `0.493` → IC=+0.197 (n=338)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.493 (IC base=+0.181)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.209 (n=304)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.181)

- **PATRÓN** `ibs_20min` > `0.4502` → IC=+0.238 (n=338)
...
- **PATRÓN** `volumen_regimen` > `0.6356` → IC=+0.197 (n=338)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` > 0.6356 (IC base=+0.181)
...
- **PATRÓN** `volumen_spike_ratio` > `2.4478` → IC=+0.221 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4478 (IC base=+0.181)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.184 (n=384)
...
- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.177 (n=184)

  - _Acción_: Kelly boost +0.89€ cuando `ballena_activa_n` < 156.0 (IC base=+0.181)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.130 (n=492)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 5.0 (IC base=+0.115)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.262 (n=233)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` > `0.7814` → IC=+0.275 (n=149)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7814 (IC base=+0.115)
...
- **PATRÓN** `volumen_regimen` > `0.6164` → IC=+0.130 (n=471)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` > 0.6164 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` > `1.5631` → IC=+0.129 (n=392)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` > 1.5631 (IC base=+0.115)

- **PATRÓN** `libro_liquidez` > `3182.3782` → IC=+0.198 (n=157)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 3182.3782 (IC base=+0.115)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.193 (n=164)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 17.0 (IC base=+0.082)

- **PATRÓN** `ibs_20min` > `0.6252` → IC=+0.147 (n=363)

  - _Acción_: Kelly boost +0.73€ cuando `ibs_20min` > 0.6252 (IC base=+0.082)

- **PATRÓN** `dist_vwap_pct` > `0.7767` → IC=+0.240 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7767 (IC base=+0.082)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.287` → IC=+0.195 (n=116)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 8.287 (IC base=+0.082)

- **PATRÓN** `volumen_pendiente_norm` > `0.1717` → IC=+0.180 (n=101)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1717 (IC base=+0.082)

- **PATRÓN** `libro_liquidez` > `2902.7974` → IC=+0.126 (n=271)

  - _Acción_: Kelly boost +0.63€ cuando `libro_liquidez` > 2902.7974 (IC base=+0.082)

- **PATRÓN** `sigma_h` < `0.0034` → IC=+0.177 (n=125)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0034 (IC base=+0.076)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.145 (n=139)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 16.0 (IC base=+0.076)

- **PATRÓN** `ibs_20min` < `0.6219` → IC=+0.124 (n=370)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` < 0.6219 (IC base=+0.076)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.192` → IC=+0.218 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.192 (IC base=+0.076)

- **PATRÓN** `volumen_regimen` < `0.7418` → IC=+0.139 (n=153)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` < 0.7418 (IC base=+0.076)

- **PATRÓN** `volumen_spike_ratio` < `1.8445` → IC=+0.137 (n=210)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 1.8445 (IC base=+0.076)

- **PATRÓN** `libro_liquidez` > `3963.5811` → IC=+0.131 (n=247)

  - _Acción_: Kelly boost +0.65€ cuando `libro_liquidez` > 3963.5811 (IC base=+0.076)
...
- **PATRÓN** `sigma_h` > `0.0052` → IC=+0.204 (n=69)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0052 (IC base=+0.165)

- **PATRÓN** `drift_60min` |x|≤ `0.2887` → IC=+0.204 (n=133)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2887 (IC base=+0.165)
...
- **PATRÓN** `ibs_20min` < `0.6664` → IC=+0.193 (n=151)
  ...
  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` < 0.6664 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` > `0.493` → IC=+0.167 (n=34)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.493 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` < `0.1967` → IC=+0.181 (n=136)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.1967 (IC base=+0.165)
...
- **PATRÓN** `volumen_regimen` < `1.178` → IC=+0.193 (n=151)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` < 1.178 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` > `0.0873` → IC=+0.330 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0873 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` < `1.5164` → IC=+0.223 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5164 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `2.2716` → IC=+0.194 (n=47)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 2.2716 (IC base=+0.165)

- **PATRÓN** `ballena_activa_n` < `137.0` → IC=+0.173 (n=53)

  - _Acción_: Kelly boost +0.86€ cuando `ballena_activa_n` < 137.0 (IC base=+0.165)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0079` → IC=+0.206 (n=1250)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0079 (IC base=+0.146)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.154 (n=2501)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 8.0 (IC base=+0.146)

- **PATRÓN** `ibs_20min` > `0.9416` → IC=+0.290 (n=1248)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9416 (IC base=+0.146)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.352` → IC=+0.247 (n=1166)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.352 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` < `0.7018` → IC=+0.151 (n=857)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1649` → IC=+0.182 (n=693)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.1649 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` > `1.8738` → IC=+0.157 (n=1624)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.8738 (IC base=+0.146)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.153 (n=2248)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.146)

- **PATRÓN** `libro_liquidez` > `3887.2465` → IC=+0.194 (n=918)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 3887.2465 (IC base=+0.146)

- **PATRÓN** `ballena_activa_n` < `160.0` → IC=+0.189 (n=1210)

  - _Acción_: Kelly boost +0.94€ cuando `ballena_activa_n` < 160.0 (IC base=+0.146)

- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.228 (n=1653)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0064 (IC base=+0.195)

- **PATRÓN** `drift_60min` |x|≤ `0.4581` → IC=+0.203 (n=2476)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4581 (IC base=+0.195)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.203 (n=920)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5551` → IC=+0.251 (n=2476)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5551 (IC base=+0.195)

- **PATRÓN** `dist_vwap_pct` < `0.7191` → IC=+0.183 (n=2029)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.7191 (IC base=+0.195)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.646` → IC=+0.198 (n=2328)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` < 2.646 (IC base=+0.195)

- **PATRÓN** `volumen_regimen` < `0.6257` → IC=+0.185 (n=626)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 0.6257 (IC base=+0.195)
...
- **PATRÓN** `sigma_h` < `0.0022` → IC=+0.204 (n=140)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0022 (IC base=+0.169)

- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.174 (n=191)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0046 (IC base=+0.169)

- **PATRÓN** `drift_60min` |x|≤ `0.2654` → IC=+0.181 (n=368)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.91€ cuando `drift_60min` |x|≤ 0.2654 (IC base=+0.169)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.175 (n=395)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 7.0 (IC base=+0.169)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.180 (n=432)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` < 18.0 (IC base=+0.169)

- **PATRÓN** `ibs_20min` < `0.3756` → IC=+0.218 (n=367)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3756 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` < `0.7539` → IC=+0.177 (n=463)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.7539 (IC base=+0.169)
...
- **PATRÓN** `volumen_regimen` < `0.6267` → IC=+0.239 (n=140)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6267 (IC base=+0.169)

- **PATRÓN** `volumen_pendiente_norm` > `0.1496` → IC=+0.288 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1496 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` < `2.4765` → IC=+0.194 (n=322)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 2.4765 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `1.506` → IC=+0.189 (n=287)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 1.506 (IC base=+0.169)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.169 (n=539)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.01 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `13529.305` → IC=+0.174 (n=139)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 13529.305 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `313.0` → IC=+0.188 (n=75)

  - _Acción_: Kelly boost +0.94€ cuando `ballena_activa_n` < 313.0 (IC base=+0.169)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `drift_60min` |x|≤ `0.0753` → IC=+0.185 (n=122)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.0753 (IC base=+0.177)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.241 (n=164)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.177)

- **PATRÓN** `ibs_20min` > `0.7131` → IC=+0.258 (n=325)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7131 (IC base=+0.177)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.113` → IC=+0.342 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.113 (IC base=+0.177)

- **PATRÓN** `volumen_pendiente_norm` < `0.2301` → IC=+0.181 (n=296)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` < 0.2301 (IC base=+0.177)

- **PATRÓN** `volumen_spike_ratio` < `2.0727` → IC=+0.185 (n=128)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` < 2.0727 (IC base=+0.177)

- **PATRÓN** `volumen_spike_ratio` > `2.8061` → IC=+0.168 (n=194)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 2.8061 (IC base=+0.177)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.197 (n=176)

  - _Acción_: Kelly boost +0.98€ cuando `libro_spread` < 0.01 (IC base=+0.177)

- **PATRÓN** `libro_liquidez` > `1919.501` → IC=+0.195 (n=165)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 1919.501 (IC base=+0.177)

- **PATRÓN** `ballena_activa_n` < `58.0` → IC=+0.235 (n=66)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 58.0 (IC base=+0.177)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2691` → IC=+0.278 (n=70)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2691 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` > `1.3991` → IC=+0.195 (n=405)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.3991 (IC base=+0.141)
...
- **PATRÓN** `sigma_h` > `0.0106` → IC=+0.170 (n=222)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0106 (IC base=+0.096)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.132 (n=335)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 12.0 (IC base=+0.096)

- **PATRÓN** `ibs_20min` > `0.9048` → IC=+0.300 (n=223)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9048 (IC base=+0.096)

- **PATRÓN** `dist_vwap_pct` > `0.7678` → IC=+0.254 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7678 (IC base=+0.096)
...
- **PATRÓN** `libro_liquidez` > `2900.261` → IC=+0.282 (n=163)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2900.261 (IC base=+0.096)

- **PATRÓN** `ballena_activa_n` < `68.0` → IC=+0.173 (n=264)

  - _Acción_: Kelly boost +0.86€ cuando `ballena_activa_n` < 68.0 (IC base=+0.096)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.147 (n=15)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=398)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.170 (n=770)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=2378)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.210 (n=743)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=2414)

- **FILTRO** `ibs_20min` > `0.2723` → IC=-0.175 (n=788)
...
- **FILTRO** `ibs_20min` > `0.7608` → IC=-0.202 (n=132)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7608
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=400)

### MOMENTUM_IBS_15M_BALLENA#ETH#15min
- **FILTRO** `py_entrada` < `0.495` → IC=-0.133 (n=137)

  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=435)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.136 (n=1901)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=5893)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.286 (n=1865)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=5929)

- **FILTRO** `ibs_7min` < `0.7368` → IC=-0.235 (n=1946)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7368
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=5848)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.176 (n=2649)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=5145)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.223 (n=2385)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=7167)

- **FILTRO** `ibs_7min` > `0.7174` → IC=-0.168 (n=2387)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7174
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=7165)
...
- **FILTRO** `py_entrada` > `0.6` → IC=-0.228 (n=380)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=1199)

- **FILTRO** `ballena_activa_n` > `130.0` → IC=-0.167 (n=394)

  - _Acción_: SKIP cuando `ballena_activa_n` > 130.0
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=1185)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.193 (n=262)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.103 (n=844)
...
- **FILTRO** `ibs_7min` < `0.2097` → IC=-0.284 (n=276)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=830)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.270 (n=276)

  - _Acción_: SKIP cuando `ballena_activa_n` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.076 (n=830)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.262 (n=355)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=1311)

- **FILTRO** `ibs_7min` > `0.8182` → IC=-0.182 (n=416)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8182
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=1250)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.150 (n=389)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=935)
...
- **FILTRO** `ibs_7min` < `0.765` → IC=-0.185 (n=331)
...
- **FILTRO** `ballena_activa_n` > `43.0` → IC=-0.216 (n=326)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=998)

- **FILTRO** `hora_utc` > `15.0` → IC=-0.129 (n=440)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.081 (n=876)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.287 (n=317)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=999)

- **FILTRO** `ibs_7min` > `0.2707` → IC=-0.170 (n=328)
...
- **FILTRO** `ballena_activa_n` > `38.0` → IC=-0.208 (n=323)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=993)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.4` → IC=-0.241 (n=346)

  - _Acción_: SKIP cuando `py_entrada` < 0.4
  - _Potencial_: sin este filtro IC_bueno=+0.012 (n=1131)

- **FILTRO** `ibs_7min` < `0.7719` → IC=-0.206 (n=369)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7719
  - _Potencial_: sin este filtro IC_bueno=+0.005 (n=1108)

- **FILTRO** `ballena_activa_n` > `14.0` → IC=-0.195 (n=369)

  - _Acción_: SKIP cuando `ballena_activa_n` > 14.0
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=1108)
...
- **PATRÓN** `streak_len` < `3.0` → IC=+0.135 (n=143)
  ...
  - _Acción_: Kelly boost +0.67€ cuando `streak_len` < 3.0 (IC base=+0.066)
...
- **PATRÓN** `sigma_h` < `0.0053` → IC=+0.137 (n=169)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0053 (IC base=+0.017)
...
- **PATRÓN** `sigma_h` > `0.0057` → IC=+0.400 (n=48)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0057 (IC base=+0.306)

- **PATRÓN** `drift_60min` |x|≤ `0.058` → IC=+0.340 (n=48)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.058 (IC base=+0.306)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1528` → IC=+0.325 (n=95)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1528 (IC base=+0.306)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3943` → IC=+0.377 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3943 (IC base=+0.306)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.321 (n=154)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.306)

- **PATRÓN** `ibs_15` > `0.7314` → IC=+0.375 (n=142)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7314 (IC base=+0.306)

- **PATRÓN** `dist_vwap_pct` > `0.4531` → IC=+0.360 (n=41)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4531 (IC base=+0.306)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.489` → IC=+0.335 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.489 (IC base=+0.306)

- **PATRÓN** `libro_liquidez` > `7885.1862` → IC=+0.360 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7885.1862 (IC base=+0.306)

- **PATRÓN** `ballena_activa_n` < `540.0` → IC=+0.375 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 540.0 (IC base=+0.306)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.333 (n=28)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0039 (IC base=+0.333)

- **PATRÓN** `sigma_h` > `0.0059` → IC=+0.400 (n=28)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0059 (IC base=+0.333)

- **PATRÓN** `drift_60min` |x|≤ `0.0796` → IC=+0.367 (n=28)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0796 (IC base=+0.333)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1501` → IC=+0.384 (n=41)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1501 (IC base=+0.333)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2773` → IC=+0.348 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.2773 (IC base=+0.333)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.341 (n=42)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` > `0.7053` → IC=+0.422 (n=62)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7053 (IC base=+0.333)

- **PATRÓN** `dist_vwap_pct` > `0.4682` → IC=+0.350 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4682 (IC base=+0.333)

- **PATRÓN** `dist_vwap_pct` < `0.156` → IC=+0.333 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.156 (IC base=+0.333)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.664` → IC=+0.400 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.664 (IC base=+0.333)

- **PATRÓN** `libro_liquidez` > `3271.2182` → IC=+0.360 (n=41)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.232` → IC=-0.176 (n=334)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=2397)
...
- **FILTRO** `sigma_ewma_delta_pct` > `12.718` → IC=-0.140 (n=145)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 12.718
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=794)
...
### UPDOWN_GBM_IBS_ALTO
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.276 (n=150)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.0537` → IC=+0.318 (n=75)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0537 (IC base=+0.273)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1881` → IC=+0.319 (n=103)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1881 (IC base=+0.273)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.382` → IC=+0.318 (n=119)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.382 (IC base=+0.273)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.306 (n=230)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.273)

- **PATRÓN** `ibs_15` > `0.8606` → IC=+0.312 (n=200)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8606 (IC base=+0.273)

- **PATRÓN** `dist_vwap_pct` > `0.3` → IC=+0.349 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3 (IC base=+0.273)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `9.292` → IC=+0.272 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 9.292 (IC base=+0.273)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.273 (n=275)
...
- **PATRÓN** `sigma_h` < `0.0051` → IC=+0.309 (n=66)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0051 (IC base=+0.295)

- **PATRÓN** `sigma_h` > `0.0062` → IC=+0.308 (n=45)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0062 (IC base=+0.295)

- **PATRÓN** `drift_60min` |x|≤ `0.0603` → IC=+0.322 (n=43)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0603 (IC base=+0.295)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1879` → IC=+0.372 (n=45)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1879 (IC base=+0.295)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3048` → IC=+0.352 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3048 (IC base=+0.295)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.371 (n=68)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.295)

- **PATRÓN** `ibs_15` > `0.863` → IC=+0.344 (n=88)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.863 (IC base=+0.295)

- **PATRÓN** `dist_vwap_pct` > `0.6362` → IC=+0.413 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6362 (IC base=+0.295)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.952` → IC=+0.300 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.952 (IC base=+0.295)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.564` → IC=+0.293 (n=85)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 9.564 (IC base=+0.295)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.303 (n=115)
...
- **PATRÓN** `ballena_activa_n` < `263.0` → IC=+0.309 (n=66)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 263.0 (IC base=+0.295)
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.8606 sube el IC de +0.273 a +0.312 en UPDOWN_GBM_IBS_ALTO (n=200). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO#ETH#15min**: dentro de BUY_YES, IBS > 0.863 sube el IC de +0.295 a +0.344 en UPDOWN_GBM_IBS_ALTO#ETH#15min (n=88). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD**: dentro de BUY_YES, IBS > 0.7314 sube el IC de +0.306 a +0.375 en UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD (n=142). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min**: dentro de BUY_YES, IBS > 0.7053 sube el IC de +0.333 a +0.422 en UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min (n=62). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ FAVORITO_CONFIRMADO | 30986 | +0.114 | -1991.21€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 5761 | +0.187 | -184.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 107 | -0.105 | -50.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 21965 | +0.095 | -1720.38€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 6418 | +0.134 | -126.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1624 | +0.199 | -78.42€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 3655 | +0.110 | -81.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 