# Hipótesis automáticas — 2026-09-15 17:09 UTC
_Generado por shadow_postmortem.py sobre 454403 resoluciones (PNL=+48843.47€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `145.95` → IC=-0.256 (n=5664)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 145.95
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=16995)
...
- **FILTRO** `restante_s_al_confirmar` < `141.15` → IC=-0.156 (n=1459)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 141.15
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=4380)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `158.32` → IC=-0.254 (n=1337)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.32
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=4013)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `156.96` → IC=-0.331 (n=1416)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 156.96
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=2876)

### CANDIDATA9_BOT_CONSENSO
- **FILTRO** `py_entrada` < `0.38` → IC=-0.265 (n=147)
...
- **FILTRO** `py_entrada` < `0.43` → IC=-0.297 (n=57)

  - _Acción_: SKIP cuando `py_entrada` < 0.43
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=185)

- **FILTRO** `py_entrada` > `0.54` → IC=-0.130 (n=44)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.195 (n=11540)

  - _Acción_: Kelly boost +0.98€ cuando `py_entrada` > 0.69 (IC base=+0.097)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.151 (n=2863)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.01 (IC base=+0.097)

- **PATRÓN** `libro_liquidez` > `5240.5962` → IC=+0.168 (n=1827)
...
- **PATRÓN** `libro_liquidez` > `10989.0993` → IC=+0.203 (n=859)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 10989.0993 (IC base=+0.201)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.209 (n=1227)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.202)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.207 (n=1352)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.203 (n=1729)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.202)

- **PATRÓN** `libro_liquidez` > `12949.669` → IC=+0.213 (n=607)
...
- **PATRÓN** `libro_liquidez` > `4643.6334` → IC=+0.149 (n=229)
  ...
  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 4643.6334 (IC base=+0.102)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.145 (n=2308)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.327 (n=749)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.7 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.253 (n=533)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.127 (n=561)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 5.0 (IC base=+0.124)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.131 (n=540)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `1950.0111` → IC=+0.157 (n=357)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 1950.0111 (IC base=+0.124)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.183 (n=960)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.03` → IC=+0.182 (n=1107)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.178 (n=2153)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` < 17.0 (IC base=+0.169)

- **PATRÓN** `py_entrada` < `0.74` → IC=+0.180 (n=2231)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.336 (n=163)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.200 (n=2144)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.193 (n=1539)

  - _Acción_: Kelly boost +0.97€ cuando `py_entrada` < 0.71 (IC base=+0.191)

### FAVORITO_CONFIRMADO_15MIN_EXTREMO
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.438 (n=370)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.915` → IC=+0.444 (n=425)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.915 (IC base=+0.433)

- **PATRÓN** `libro_liquidez` > `2048.1399` → IC=+0.443 (n=404)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.192 (n=4458)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `3.33` → IC=+0.150 (n=3697)

  - _Acción_: Kelly boost +0.75€ cuando `lag_apertura_s` < 3.33 (IC base=+0.125)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.206 (n=2039)
...
- **PATRÓN** `restante_min` > `4.93` → IC=+0.146 (n=1949)
...
- **PATRÓN** `lag_apertura_s` < `4.2` → IC=+0.149 (n=1827)

  - _Acción_: Kelly boost +0.74€ cuando `lag_apertura_s` < 4.2 (IC base=+0.130)
...
- **PATRÓN** `restante_min` < `4.48` → IC=+0.126 (n=2465)

  - _Acción_: Kelly boost +0.63€ cuando `restante_min` < 4.48 (IC base=+0.119)

- **PATRÓN** `restante_min` > `4.96` → IC=+0.147 (n=1877)
...
- **PATRÓN** `lag_apertura_s` < `4.4` → IC=+0.145 (n=2451)

  - _Acción_: Kelly boost +0.72€ cuando `lag_apertura_s` < 4.4 (IC base=+0.119)

### FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.317 (n=654)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.288)

- **PATRÓN** `py_entrada` > `0.815` → IC=+0.382 (n=336)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.815 (IC base=+0.288)

- **PATRÓN** `libro_liquidez` > `1604.9934` → IC=+0.297 (n=921)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.326 (n=309)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.291)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.290 (n=442)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `1492.929` → IC=+0.313 (n=393)
...
- **PATRÓN** `py_entrada` > `0.915` → IC=+0.431 (n=187)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9773` → IC=+0.230 (n=2007)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9773 (IC base=+0.096)

- **PATRÓN** `dist_vwap_pct` < `0.6067` → IC=+0.243 (n=1611)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6067 (IC base=+0.096)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.854` → IC=+0.166 (n=2324)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 5.854 (IC base=+0.096)

- **PATRÓN** `volumen_regimen` < `0.612` → IC=+0.251 (n=504)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.612 (IC base=+0.096)

- **PATRÓN** `volumen_regimen` > `1.0696` → IC=+0.247 (n=685)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0696 (IC base=+0.096)

- **PATRÓN** `volumen_pendiente_norm` < `0.175` → IC=+0.192 (n=4077)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` < 0.175 (IC base=+0.096)

- **PATRÓN** `volumen_pendiente_norm` > `0.3088` → IC=+0.203 (n=560)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3088 (IC base=+0.096)

- **PATRÓN** `volumen_spike_ratio` > `1.9304` → IC=+0.199 (n=2622)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.9304 (IC base=+0.096)

- **PATRÓN** `ibs_20min` < `0.5687` → IC=+0.130 (n=7410)

  - _Acción_: Kelly boost +0.65€ cuando `ibs_20min` < 0.5687 (IC base=+0.059)

- **PATRÓN** `dist_vwap_pct` > `0.5753` → IC=+0.181 (n=450)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.5753 (IC base=+0.059)

- **PATRÓN** `dist_vwap_pct` < `0.3466` → IC=+0.168 (n=2527)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` < 0.3466 (IC base=+0.059)

- **PATRÓN** `volumen_regimen` < `1.2069` → IC=+0.168 (n=2392)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.2069 (IC base=+0.059)

- **PATRÓN** `volumen_regimen` > `0.8706` → IC=+0.174 (n=1595)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 0.8706 (IC base=+0.059)

- **PATRÓN** `volumen_pendiente_norm` > `0.3008` → IC=+0.223 (n=605)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3008 (IC base=+0.059)

- **PATRÓN** `volumen_spike_ratio` > `1.4641` → IC=+0.198 (n=3981)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.4641 (IC base=+0.059)

- **PATRÓN** `ballena_activa_n` < `158.0` → IC=+0.207 (n=3753)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.265 (n=530)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.077` → IC=+0.282 (n=590)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.077 (IC base=+0.167)
...
- **PATRÓN** `volumen_spike_ratio` < `2.6284` → IC=+0.155 (n=1244)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.6284 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `1.4361` → IC=+0.167 (n=1244)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.4361 (IC base=+0.167)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.191 (n=1233)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.213` → IC=+0.243 (n=1132)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.213 (IC base=+0.234)

- **PATRÓN** `volumen_pendiente_norm` < `0.0688` → IC=+0.233 (n=803)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0688 (IC base=+0.234)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3839` → IC=+0.219 (n=332)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3839 (IC base=+0.213)

- **PATRÓN** `libro_liquidez` > `11927.4558` → IC=+0.226 (n=913)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11927.4558 (IC base=+0.213)

- **PATRÓN** `sigma_h` < `0.0048` → IC=+0.161 (n=969)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0048 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.0766` → IC=+0.169 (n=367)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.0766 (IC base=+0.141)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.170 (n=437)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 17.0 (IC base=+0.141)

- **PATRÓN** `ibs_20min` < `0.6717` → IC=+0.178 (n=1099)

  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` < 0.6717 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` < `0.1332` → IC=+0.157 (n=991)
...
- **PATRÓN** `volumen_regimen` < `1.2046` → IC=+0.151 (n=1099)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.2046 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` > `0.6862` → IC=+0.143 (n=982)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 0.6862 (IC base=+0.141)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4315` → IC=+0.155 (n=991)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.4315 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `13110.5319` → IC=+0.157 (n=733)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 13110.5319 (IC base=+0.141)

- **PATRÓN** `ballena_activa_n` < `216.0` → IC=+0.165 (n=302)

  - _Acción_: Kelly boost +0.82€ cuando `ballena_activa_n` < 216.0 (IC base=+0.141)

### GBM_LATE_15M#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0057` → IC=+0.187 (n=1322)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0057 (IC base=+0.177)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.178 (n=648)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.47` → IC=+0.232 (n=375)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.47 (IC base=+0.177)

- **PATRÓN** `volumen_pendiente_norm` < `0.1064` → IC=+0.184 (n=1117)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` < 0.1064 (IC base=+0.177)

- **PATRÓN** `volumen_pendiente_norm` > `0.377` → IC=+0.182 (n=171)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.377 (IC base=+0.177)

- **PATRÓN** `volumen_spike_ratio` > `3.0416` → IC=+0.197 (n=559)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 3.0416 (IC base=+0.177)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.190 (n=1505)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.04 (IC base=+0.177)

- **PATRÓN** `sigma_h` < `0.0091` → IC=+0.216 (n=1010)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.3822` → IC=+0.232 (n=1009)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `7.574` → IC=+0.214 (n=1350)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 7.574 (IC base=+0.214)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2934` → IC=+0.223 (n=680)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2934 (IC base=+0.214)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.227 (n=583)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.214)

- **PATRÓN** `libro_liquidez` > `1889.461` → IC=+0.232 (n=382)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1889.461 (IC base=+0.214)

- **PATRÓN** `ballena_activa_n` < `18.0` → IC=+0.217 (n=425)
...
- **PATRÓN** `ibs_20min` > `0.9299` → IC=+0.167 (n=283)
...
- **PATRÓN** `dist_vwap_pct` < `0.491` → IC=+0.331 (n=223)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.491 (IC base=+0.007)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.258` → IC=+0.135 (n=526)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` > 4.258 (IC base=+0.007)

- **PATRÓN** `volumen_regimen` < `0.6604` → IC=+0.376 (n=95)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2836` → IC=+0.362 (n=56)
...
- **PATRÓN** `ballena_activa_n` < `167.0` → IC=+0.345 (n=204)
...
- **PATRÓN** `dist_vwap_pct` < `0.4612` → IC=+0.147 (n=607)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.4612 (IC base=+0.001)

- **PATRÓN** `volumen_regimen` < `0.6944` → IC=+0.152 (n=234)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.6944 (IC base=+0.001)

- **PATRÓN** `volumen_regimen` > `1.1639` → IC=+0.144 (n=178)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5036` → IC=+0.181 (n=430)
...
- **FILTRO** `ibs_20min` < `0.3553` → IC=-0.174 (n=93)

  - _Acción_: SKIP cuando `ibs_20min` < 0.3553
  - _Potencial_: sin este filtro IC_bueno=+0.137 (n=191)

- **FILTRO** `ibs_20min` > `0.2727` → IC=-0.128 (n=1709)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2727
  - _Potencial_: sin este filtro IC_bueno=+0.117 (n=850)

- **FILTRO** `sigma_ewma_delta_pct` > `8.613` → IC=-0.198 (n=279)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.613
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=2280)

- **PATRÓN** `ibs_20min` > `0.75` → IC=+0.207 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.75 (IC base=+0.035)

- **PATRÓN** `dist_vwap_pct` > `1.1883` → IC=+0.300 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1883 (IC base=+0.035)

- **PATRÓN** `dist_vwap_pct` < `0.4558` → IC=+0.297 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4558 (IC base=+0.035)

- **PATRÓN** `volumen_regimen` < `0.5788` → IC=+0.278 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.5788 (IC base=+0.035)
...
- **PATRÓN** `volumen_spike_ratio` < `2.9536` → IC=+0.273 (n=73)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.9536 (IC base=+0.035)

- **PATRÓN** `volumen_spike_ratio` > `1.4528` → IC=+0.273 (n=73)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4528 (IC base=+0.035)
...
- **FILTRO** `libro_spread` > `0.02` → IC=-0.198 (n=376)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=1317)

- **FILTRO** `ibs_20min` > `0.7733` → IC=-0.201 (n=640)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7733
  - _Potencial_: sin este filtro IC_bueno=+0.030 (n=1923)

- **PATRÓN** `dist_vwap_pct` > `0.9664` → IC=+0.329 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9664 (IC base=-0.086)

- **PATRÓN** `dist_vwap_pct` < `0.2585` → IC=+0.293 (n=177)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2585 (IC base=-0.086)

- **PATRÓN** `volumen_regimen` > `0.616` → IC=+0.288 (n=201)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4208` → IC=+0.278 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4208 (IC base=-0.086)

- **PATRÓN** `volumen_spike_ratio` > `1.8015` → IC=+0.274 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8015 (IC base=-0.086)

- **PATRÓN** `dist_vwap_pct` > `1.0042` → IC=+0.263 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0042 (IC base=-0.028)

- **PATRÓN** `dist_vwap_pct` < `0.2617` → IC=+0.241 (n=535)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2617 (IC base=-0.028)

- **PATRÓN** `volumen_regimen` < `0.7344` → IC=+0.242 (n=231)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1065` → IC=+0.260 (n=177)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1065 (IC base=-0.028)

- **PATRÓN** `volumen_spike_ratio` < `2.24` → IC=+0.258 (n=366)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.24 (IC base=-0.028)

- **PATRÓN** `volumen_spike_ratio` > `1.4747` → IC=+0.237 (n=416)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4747 (IC base=-0.028)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.242 (n=416)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 35.0 (IC base=-0.028)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.009` → IC=+0.175 (n=2515)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.009 (IC base=+0.086)

- **PATRÓN** `ibs_20min` > `0.9833` → IC=+0.288 (n=2513)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9833 (IC base=+0.086)

- **PATRÓN** `dist_vwap_pct` > `1.0147` → IC=+0.285 (n=542)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0147 (IC base=+0.086)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.508` → IC=+0.142 (n=3562)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 3.508 (IC base=+0.086)

- **PATRÓN** `volumen_regimen` > `0.6782` → IC=+0.234 (n=2281)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6782 (IC base=+0.086)

- **PATRÓN** `volumen_pendiente_norm` < `0.1131` → IC=+0.224 (n=3862)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1131 (IC base=+0.086)

- **PATRÓN** `volumen_pendiente_norm` > `0.2482` → IC=+0.257 (n=809)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2482 (IC base=+0.086)

- **PATRÓN** `volumen_spike_ratio` < `1.4779` → IC=+0.239 (n=1355)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4779 (IC base=+0.086)

- **PATRÓN** `volumen_spike_ratio` > `2.7774` → IC=+0.235 (n=1354)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7774 (IC base=+0.086)

- **PATRÓN** `ballena_activa_n` < `102.0` → IC=+0.277 (n=3544)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 102.0 (IC base=+0.086)

- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.140 (n=2573)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` > 0.0084 (IC base=+0.068)

- **PATRÓN** `ibs_20min` < `0.5556` → IC=+0.150 (n=6778)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` < 0.5556 (IC base=+0.068)

- **PATRÓN** `dist_vwap_pct` > `0.6839` → IC=+0.245 (n=387)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6839 (IC base=+0.068)

- **PATRÓN** `dist_vwap_pct` < `0.2328` → IC=+0.230 (n=1997)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2328 (IC base=+0.068)

- **PATRÓN** `volumen_regimen` < `0.7144` → IC=+0.229 (n=918)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.251` → IC=+0.325 (n=545)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.251 (IC base=+0.068)

- **PATRÓN** `volumen_spike_ratio` < `1.6229` → IC=+0.254 (n=1191)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6229 (IC base=+0.068)

- **PATRÓN** `volumen_spike_ratio` > `2.3642` → IC=+0.255 (n=1227)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3642 (IC base=+0.068)

- **PATRÓN** `ballena_activa_n` < `59.0` → IC=+0.257 (n=2269)
...
- **PATRÓN** `ibs_20min` > `0.8641` → IC=+0.249 (n=513)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8641 (IC base=+0.038)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.283` → IC=+0.149 (n=691)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 3.283 (IC base=+0.038)

- **PATRÓN** `volumen_pendiente_norm` > `0.2248` → IC=+0.296 (n=106)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2248 (IC base=+0.038)

- **PATRÓN** `volumen_spike_ratio` < `1.8491` → IC=+0.197 (n=351)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 1.8491 (IC base=+0.038)
...
- **FILTRO** `ibs_20min` > `0.827` → IC=-0.147 (n=567)

  - _Acción_: SKIP cuando `ibs_20min` > 0.827
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=1704)

- **PATRÓN** `dist_vwap_pct` > `0.3061` → IC=+0.137 (n=246)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 0.3061 (IC base=+0.008)

- **PATRÓN** `volumen_regimen` > `0.6518` → IC=+0.134 (n=582)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` > 0.6518 (IC base=+0.008)

- **PATRÓN** `volumen_pendiente_norm` > `0.2213` → IC=+0.167 (n=118)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.2213 (IC base=+0.008)

- **PATRÓN** `volumen_spike_ratio` < `1.4206` → IC=+0.162 (n=211)
...
- **PATRÓN** `dist_vwap_pct` < `0.1603` → IC=+0.216 (n=396)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7866` → IC=+0.230 (n=224)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7866 (IC base=+0.001)

- **PATRÓN** `volumen_spike_ratio` > `2.1554` → IC=+0.234 (n=152)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1554 (IC base=+0.001)

- **PATRÓN** `ballena_activa_n` < `503.0` → IC=+0.213 (n=333)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 503.0 (IC base=+0.001)

### GBM_LATE_15M_ESPACIO_ATR#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0061` → IC=+0.267 (n=1073)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.241 (n=596)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.526` → IC=+0.271 (n=374)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.526 (IC base=+0.237)

- **PATRÓN** `volumen_pendiente_norm` < `0.1101` → IC=+0.256 (n=1000)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1101 (IC base=+0.237)

- **PATRÓN** `volumen_spike_ratio` < `2.3446` → IC=+0.239 (n=741)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.3446 (IC base=+0.237)

- **PATRÓN** `volumen_spike_ratio` > `3.6325` → IC=+0.251 (n=371)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.6325 (IC base=+0.237)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.254 (n=1352)
...
- **PATRÓN** `ibs_20min` < `0.3333` → IC=+0.286 (n=958)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3333 (IC base=+0.278)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.884` → IC=+0.297 (n=368)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.884 (IC base=+0.278)

- **PATRÓN** `volumen_pendiente_norm` > `0.3457` → IC=+0.312 (n=147)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2184` → IC=+0.283 (n=575)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2184 (IC base=+0.278)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.287 (n=482)
...
- **PATRÓN** `ballena_activa_n` < `22.0` → IC=+0.278 (n=367)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 22.0 (IC base=+0.278)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **FILTRO** `ibs_20min` < `0.2435` → IC=-0.214 (n=341)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2435
  - _Potencial_: sin este filtro IC_bueno=+0.048 (n=1024)

- **FILTRO** `ibs_20min` > `0.8142` → IC=-0.183 (n=449)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8142
  - _Potencial_: sin este filtro IC_bueno=+0.036 (n=1350)

- **PATRÓN** `ibs_20min` > `0.8024` → IC=+0.142 (n=465)

  - _Acción_: Kelly boost +0.71€ cuando `ibs_20min` > 0.8024 (IC base=-0.018)

- **PATRÓN** `dist_vwap_pct` > `0.3052` → IC=+0.226 (n=155)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3052 (IC base=-0.018)

- **PATRÓN** `volumen_regimen` < `0.9565` → IC=+0.206 (n=270)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9565 (IC base=-0.018)

- **PATRÓN** `volumen_regimen` > `0.6162` → IC=+0.188 (n=274)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` > 0.6162 (IC base=-0.018)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4842` → IC=+0.256 (n=125)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4842 (IC base=-0.018)

- **PATRÓN** `volumen_spike_ratio` > `1.7422` → IC=+0.216 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7422 (IC base=-0.018)

- **PATRÓN** `ballena_activa_n` < `118.0` → IC=+0.253 (n=188)
...
- **PATRÓN** `dist_vwap_pct` < `0.4617` → IC=+0.161 (n=246)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.4617 (IC base=-0.019)

- **PATRÓN** `volumen_regimen` < `0.979` → IC=+0.168 (n=194)
...
- **PATRÓN** `ballena_activa_n` < `151.0` → IC=+0.236 (n=180)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 151.0 (IC base=-0.019)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **FILTRO** `ibs_20min` < `0.6667` → IC=-0.207 (n=816)

  - _Acción_: SKIP cuando `ibs_20min` < 0.6667
  - _Potencial_: sin este filtro IC_bueno=+0.252 (n=827)
...
- **FILTRO** `sigma_ewma_delta_pct` > `4.689` → IC=-0.169 (n=406)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.689
  - _Potencial_: sin este filtro IC_bueno=+0.055 (n=1334)

- **PATRÓN** `ibs_20min` > `0.6667` → IC=+0.252 (n=827)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6667 (IC base=+0.024)

- **PATRÓN** `dist_vwap_pct` > `0.7667` → IC=+0.340 (n=160)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7667 (IC base=+0.024)

- **PATRÓN** `volumen_regimen` < `0.8622` → IC=+0.286 (n=377)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1092` → IC=+0.276 (n=520)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4475` → IC=+0.311 (n=183)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4475 (IC base=+0.024)

- **PATRÓN** `ballena_activa_n` < `55.0` → IC=+0.321 (n=462)
...
- **PATRÓN** `dist_vwap_pct` > `0.5842` → IC=+0.214 (n=75)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5842 (IC base=+0.003)

- **PATRÓN** `dist_vwap_pct` < `0.3916` → IC=+0.182 (n=369)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.3916 (IC base=+0.003)
...
- **PATRÓN** `sigma_h` > `0.0151` → IC=+0.323 (n=688)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.383` → IC=+0.296 (n=551)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.383 (IC base=+0.268)

- **PATRÓN** `volumen_regimen` > `0.6862` → IC=+0.287 (n=922)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6862 (IC base=+0.268)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5505` → IC=+0.275 (n=425)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5505 (IC base=+0.268)

- **PATRÓN** `volumen_spike_ratio` > `2.2094` → IC=+0.275 (n=438)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2094 (IC base=+0.268)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.271 (n=1065)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.268)

- **PATRÓN** `libro_liquidez` > `2584.1244` → IC=+0.275 (n=688)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2584.1244 (IC base=+0.268)

- **PATRÓN** `sigma_h` < `0.0071` → IC=+0.278 (n=376)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0071 (IC base=+0.268)

- **PATRÓN** `sigma_h` > `0.0141` → IC=+0.291 (n=750)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0141 (IC base=+0.268)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.283 (n=557)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_spike_ratio` < `2.5387` → IC=+0.265 (n=965)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5387 (IC base=+0.268)

- **PATRÓN** `volumen_spike_ratio` > `2.17` → IC=+0.264 (n=438)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.17 (IC base=+0.268)

- **PATRÓN** `libro_liquidez` > `2563.4979` → IC=+0.274 (n=750)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2563.4979 (IC base=+0.268)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` < `0.0047` → IC=+0.178 (n=1987)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0047 (IC base=+0.167)

- **PATRÓN** `sigma_h` > `0.0103` → IC=+0.201 (n=1989)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0103 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.3327` → IC=+0.173 (n=5246)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.3327 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.177 (n=6228)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 5.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` > `0.6952` → IC=+0.229 (n=5324)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6952 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.1646` → IC=+0.199 (n=2590)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.1646 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.225` → IC=+0.250 (n=1233)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.225 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.2168` → IC=+0.162 (n=3976)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 1.2168 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` > `0.6237` → IC=+0.159 (n=3976)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.6237 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.1068` → IC=+0.186 (n=2335)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1068 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.5667` → IC=+0.171 (n=2491)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.5667 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.6691` → IC=+0.167 (n=1887)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 2.6691 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `3822.0558` → IC=+0.172 (n=1987)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 3822.0558 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `123.0` → IC=+0.184 (n=4871)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 123.0 (IC base=+0.167)

- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.188 (n=3859)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.203 (n=2760)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.172)

- **PATRÓN** `ibs_20min` < `0.459` → IC=+0.227 (n=5786)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.459 (IC base=+0.172)

- **PATRÓN** `dist_vwap_pct` < `0.2249` → IC=+0.163 (n=4292)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2249 (IC base=+0.172)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.271` → IC=+0.198 (n=1003)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` > 10.271 (IC base=+0.172)

- **PATRÓN** `volumen_regimen` < `1.1885` → IC=+0.157 (n=4227)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.1885 (IC base=+0.172)

- **PATRÓN** `volumen_regimen` > `0.6264` → IC=+0.152 (n=4227)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.6264 (IC base=+0.172)

- **PATRÓN** `volumen_pendiente_norm` > `0.2919` → IC=+0.231 (n=820)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2919 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` < `1.5745` → IC=+0.173 (n=2258)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.5745 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` > `2.6497` → IC=+0.179 (n=1711)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 2.6497 (IC base=+0.172)

- **PATRÓN** `ballena_activa_n` < `126.0` → IC=+0.172 (n=4751)

  - _Acción_: Kelly boost +0.86€ cuando `ballena_activa_n` < 126.0 (IC base=+0.172)

### GBM_LATE_15M_MULTIHORIZONTE#BNB#15min
- **PATRÓN** `sigma_h` < `0.0051` → IC=+0.218 (n=335)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0051 (IC base=+0.189)

- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.205 (n=455)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0071 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.3121` → IC=+0.203 (n=1003)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.300 (n=493)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.189)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.105` → IC=+0.309 (n=459)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5529` → IC=+0.183 (n=911)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` < 2.5529 (IC base=+0.189)

- **PATRÓN** `volumen_spike_ratio` > `1.5413` → IC=+0.185 (n=814)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 1.5413 (IC base=+0.189)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.211 (n=923)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2648` → IC=+0.174 (n=761)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.2648 (IC base=+0.164)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.201 (n=596)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 12.0 (IC base=+0.164)

- **PATRÓN** `ibs_20min` > `0.4304` → IC=+0.222 (n=865)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4304 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.2184` → IC=+0.214 (n=515)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2184 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.681` → IC=+0.225 (n=234)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.681 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` < `1.268` → IC=+0.176 (n=865)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 1.268 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.2352` → IC=+0.190 (n=188)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.2352 (IC base=+0.164)
...
- **PATRÓN** `libro_liquidez` > `9834.2432` → IC=+0.182 (n=865)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 9834.2432 (IC base=+0.164)

- **PATRÓN** `ballena_activa_n` < `405.0` → IC=+0.161 (n=692)

  - _Acción_: Kelly boost +0.81€ cuando `ballena_activa_n` < 405.0 (IC base=+0.164)
...
- **PATRÓN** `sigma_h` > `0.0058` → IC=+0.198 (n=983)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0058 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.1945` → IC=+0.197 (n=655)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.284 (n=522)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.189)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.584` → IC=+0.266 (n=301)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.584 (IC base=+0.189)

- **PATRÓN** `volumen_pendiente_norm` < `0.1073` → IC=+0.186 (n=805)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` < 0.1073 (IC base=+0.189)

- **PATRÓN** `volumen_pendiente_norm` > `0.2187` → IC=+0.184 (n=283)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.203 (n=1104)
...
- **PATRÓN** `sigma_h` > `0.0062` → IC=+0.224 (n=743)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0062 (IC base=+0.223)

- **PATRÓN** `drift_60min` |x|≤ `0.091` → IC=+0.250 (n=278)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.3448` → IC=+0.251 (n=832)
...
- **PATRÓN** `libro_liquidez` > `1889.461` → IC=+0.238 (n=277)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1889.461 (IC base=+0.223)

- **PATRÓN** `ballena_activa_n` < `14.0` → IC=+0.211 (n=247)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 14.0 (IC base=+0.223)

### GBM_LATE_15M_MULTIHORIZONTE#ETH#15min
- **PATRÓN** `sigma_h` < `0.0065` → IC=+0.181 (n=828)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.0065 (IC base=+0.153)

- **PATRÓN** `drift_60min` |x|≤ `0.4219` → IC=+0.168 (n=940)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.4219 (IC base=+0.153)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.164 (n=947)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 6.0 (IC base=+0.153)

- **PATRÓN** `ibs_20min` > `0.4079` → IC=+0.202 (n=940)
...
- **PATRÓN** `volumen_regimen` < `0.8596` → IC=+0.160 (n=627)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.8596 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` > `1.1956` → IC=+0.177 (n=314)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` > 1.1956 (IC base=+0.153)

- **PATRÓN** `volumen_pendiente_norm` > `0.2884` → IC=+0.218 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2884 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` < `1.4014` → IC=+0.169 (n=306)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.4014 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` > `2.5308` → IC=+0.185 (n=306)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 2.5308 (IC base=+0.153)

- **PATRÓN** `libro_liquidez` > `7283.8545` → IC=+0.192 (n=627)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 7283.8545 (IC base=+0.153)

- **PATRÓN** `ballena_activa_n` < `154.0` → IC=+0.161 (n=774)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3779` → IC=+0.144 (n=1014)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6028` → IC=+0.180 (n=1014)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` < 0.6028 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` < `0.3378` → IC=+0.140 (n=1111)
...
- **PATRÓN** `volumen_regimen` < `0.8618` → IC=+0.142 (n=676)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 0.8618 (IC base=+0.128)

- **PATRÓN** `volumen_regimen` > `0.6121` → IC=+0.130 (n=1015)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7931` → IC=+0.134 (n=599)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.47` → IC=+0.254 (n=250)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.47 (IC base=+0.111)

- **PATRÓN** `volumen_regimen` < `1.2203` → IC=+0.121 (n=1111)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_regimen` < 1.2203 (IC base=+0.111)

- **PATRÓN** `volumen_spike_ratio` < `1.7979` → IC=+0.125 (n=713)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_spike_ratio` < 1.7979 (IC base=+0.111)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.123 (n=1143)

  - _Acción_: Kelly boost +0.61€ cuando `libro_spread` < 0.02 (IC base=+0.111)

- **PATRÓN** `libro_liquidez` > `2897.5388` → IC=+0.192 (n=504)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 2897.5388 (IC base=+0.111)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.135 (n=808)

  - _Acción_: Kelly boost +0.67€ cuando `ballena_activa_n` < 49.0 (IC base=+0.111)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.152 (n=487)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` < 0.0058 (IC base=+0.115)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.175 (n=506)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 15.0 (IC base=+0.115)

- **PATRÓN** `ibs_20min` < `0.5435` → IC=+0.208 (n=1107)
...
- **PATRÓN** `dist_vwap_pct` < `0.1923` → IC=+0.137 (n=1024)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.1923 (IC base=+0.115)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.33` → IC=+0.158 (n=235)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 7.33 (IC base=+0.115)

- **PATRÓN** `volumen_regimen` < `1.0407` → IC=+0.131 (n=974)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_regimen` < 1.0407 (IC base=+0.115)

- **PATRÓN** `volumen_pendiente_norm` > `0.2729` → IC=+0.184 (n=131)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.2729 (IC base=+0.115)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1727` → IC=+0.138 (n=437)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 2.1727 (IC base=+0.115)

- **PATRÓN** `libro_liquidez` > `3082.2041` → IC=+0.160 (n=369)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 3082.2041 (IC base=+0.115)

### GBM_LATE_15M_MULTIHORIZONTE#XRP#15min
- **PATRÓN** `sigma_h` < `0.0271` → IC=+0.201 (n=1061)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0271 (IC base=+0.200)

- **PATRÓN** `sigma_h` > `0.0179` → IC=+0.209 (n=707)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0179 (IC base=+0.200)

- **PATRÓN** `drift_60min` |x|≤ `0.1651` → IC=+0.221 (n=467)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1651 (IC base=+0.200)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.204 (n=1102)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.7209` → IC=+0.254 (n=950)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.217 (n=553)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `1.1415` → IC=+0.227 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1415 (IC base=+0.204)

- **PATRÓN** `dist_vwap_pct` < `0.2677` → IC=+0.204 (n=1167)
...
- **PATRÓN** `volumen_regimen` > `0.6291` → IC=+0.218 (n=1124)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6291 (IC base=+0.204)

- **PATRÓN** `volumen_pendiente_norm` > `0.2819` → IC=+0.284 (n=151)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4624` → IC=+0.193 (n=988)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 1.4624 (IC base=+0.204)

- **PATRÓN** `libro_liquidez` > `2527.1799` → IC=+0.214 (n=749)
...
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.155 (n=476)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0087 (IC base=+0.137)

- **PATRÓN** `drift_60min` |x|≤ `0.0958` → IC=+0.151 (n=476)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.0958 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.183 (n=708)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 15.0 (IC base=+0.137)

- **PATRÓN** `ibs_20min` > `0.3944` → IC=+0.168 (n=1427)

  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` > 0.3944 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.8238` → IC=+0.167 (n=202)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.8238 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.699` → IC=+0.167 (n=649)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 3.699 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `0.8662` → IC=+0.155 (n=818)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.8662 (IC base=+0.137)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4332` → IC=+0.154 (n=455)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3378` → IC=+0.127 (n=1321)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.63€ cuando `drift_60min` |x|≤ 0.3378 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.125 (n=1517)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 5.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` < `0.4835` → IC=+0.157 (n=1321)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` < 0.4835 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` < `0.2153` → IC=+0.123 (n=1318)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.2153 (IC base=+0.114)
...
- **PATRÓN** `volumen_regimen` < `1.227` → IC=+0.122 (n=1322)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` < 1.227 (IC base=+0.114)

- **PATRÓN** `volumen_pendiente_norm` > `0.1668` → IC=+0.140 (n=387)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_pendiente_norm` > 0.1668 (IC base=+0.114)

- **PATRÓN** `volumen_spike_ratio` < `2.2349` → IC=+0.133 (n=1261)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 2.2349 (IC base=+0.114)

- **PATRÓN** `ballena_activa_n` < `27.0` → IC=+0.127 (n=599)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.135 (n=382)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0066 (IC base=+0.106)

- **PATRÓN** `drift_60min` |x|≤ `0.098` → IC=+0.154 (n=128)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.098 (IC base=+0.106)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.137 (n=260)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 11.0 (IC base=+0.106)

- **PATRÓN** `ibs_20min` < `0.298` → IC=+0.162 (n=255)

  - _Acción_: Kelly boost +0.81€ cuando `ibs_20min` < 0.298 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.48` → IC=+0.160 (n=104)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 6.48 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` < `1.2331` → IC=+0.122 (n=382)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` < 1.2331 (IC base=+0.106)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5372` → IC=+0.128 (n=323)
  ...
  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.5372 (IC base=+0.106)

### GBM_LATE_15M_PYCONFIRMADO#SOL#15min
- **FILTRO** `dist_vwap_pct` > `0.3844` → IC=-0.222 (n=16)

  - _Acción_: SKIP cuando `dist_vwap_pct` > 0.3844
  - _Potencial_: sin este filtro IC_bueno=+0.098 (n=349)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.142 (n=202)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` > 11.0 (IC base=+0.075)

- **PATRÓN** `ibs_20min` > `0.9048` → IC=+0.205 (n=198)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9048 (IC base=+0.075)

- **PATRÓN** `dist_vwap_pct` > `0.7285` → IC=+0.130 (n=44)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` > 0.7285 (IC base=+0.075)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.328` → IC=+0.154 (n=131)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 5.328 (IC base=+0.075)

- **PATRÓN** `libro_liquidez` > `2963.0907` → IC=+0.183 (n=99)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 2963.0907 (IC base=+0.075)

- **PATRÓN** `ibs_20min` < `0.4524` → IC=+0.167 (n=274)

  - _Acción_: Kelly boost +0.83€ cuando `ibs_20min` < 0.4524 (IC base=+0.083)

- **PATRÓN** `volumen_regimen` < `0.7183` → IC=+0.142 (n=121)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 0.7183 (IC base=+0.083)

- **PATRÓN** `volumen_spike_ratio` < `2.4929` → IC=+0.130 (n=252)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` < 2.4929 (IC base=+0.083)

- **PATRÓN** `ballena_activa_n` < `44.0` → IC=+0.132 (n=218)

  - _Acción_: Kelly boost +0.66€ cuando `ballena_activa_n` < 44.0 (IC base=+0.083)
...
- **PATRÓN** `ibs_20min` < `0.5567` → IC=+0.239 (n=6937)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5567 (IC base=+0.182)

- **PATRÓN** `dist_vwap_pct` < `0.2346` → IC=+0.164 (n=4365)
...
- **PATRÓN** `volumen_regimen` < `0.6282` → IC=+0.160 (n=1599)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.6282 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` > `1.2024` → IC=+0.161 (n=1597)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 1.2024 (IC base=+0.182)

- **PATRÓN** `volumen_pendiente_norm` > `0.2897` → IC=+0.254 (n=891)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2897 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` > `2.3005` → IC=+0.189 (n=2813)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` > 2.3005 (IC base=+0.182)

- **PATRÓN** `ballena_activa_n` < `127.0` → IC=+0.178 (n=5818)
...
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.231 (n=575)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.007 (IC base=+0.194)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.200 (n=604)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.317 (n=452)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.194)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.379` → IC=+0.292 (n=778)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.379 (IC base=+0.194)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5578` → IC=+0.186 (n=518)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 1.5578 (IC base=+0.194)

- **PATRÓN** `volumen_spike_ratio` > `1.8681` → IC=+0.193 (n=784)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 1.8681 (IC base=+0.194)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.223 (n=1140)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.194)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.260 (n=876)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0066 (IC base=+0.255)

- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.266 (n=891)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0048 (IC base=+0.255)

- **PATRÓN** `drift_60min` |x|≤ `0.1993` → IC=+0.280 (n=665)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1993 (IC base=+0.255)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.266 (n=904)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.255)

- **PATRÓN** `ibs_20min` < `0.3409` → IC=+0.285 (n=876)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3409 (IC base=+0.255)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.611` → IC=+0.264 (n=1001)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.611 (IC base=+0.255)

- **PATRÓN** `volumen_pendiente_norm` > `0.2258` → IC=+0.296 (n=179)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.255 (n=1023)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.255)

- **PATRÓN** `libro_liquidez` > `1908.78` → IC=+0.270 (n=332)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1908.78 (IC base=+0.255)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1823` → IC=+0.156 (n=795)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.1823 (IC base=+0.148)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=1251)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.148)

- **PATRÓN** `ibs_20min` > `0.6903` → IC=+0.243 (n=795)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.803` → IC=+0.160 (n=280)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 9.803 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.367` → IC=+0.153 (n=1055)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` < 4.367 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` < `0.6939` → IC=+0.183 (n=525)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4203` → IC=+0.156 (n=1139)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.4203 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` > `1.7596` → IC=+0.158 (n=759)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.7596 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `10716.3301` → IC=+0.169 (n=1065)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 10716.3301 (IC base=+0.148)

- **PATRÓN** `ballena_activa_n` < `495.0` → IC=+0.163 (n=1072)

  - _Acción_: Kelly boost +0.81€ cuando `ballena_activa_n` < 495.0 (IC base=+0.148)

- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.171 (n=941)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0049 (IC base=+0.157)

- **PATRÓN** `drift_60min` |x|≤ `0.2576` → IC=+0.172 (n=940)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.2576 (IC base=+0.157)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.182 (n=356)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 18.0 (IC base=+0.157)

- **PATRÓN** `ibs_20min` < `0.6377` → IC=+0.207 (n=1069)
...
- **PATRÓN** `dist_vwap_pct` < `0.1327` → IC=+0.168 (n=977)
...
- **PATRÓN** `volumen_regimen` < `1.1954` → IC=+0.167 (n=1069)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.1954 (IC base=+0.157)

- **PATRÓN** `volumen_pendiente_norm` > `0.217` → IC=+0.214 (n=201)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.217 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` < `2.4156` → IC=+0.172 (n=971)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 2.4156 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` > `1.5156` → IC=+0.163 (n=867)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.5156 (IC base=+0.157)

- **PATRÓN** `ballena_activa_n` < `376.0` → IC=+0.166 (n=584)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 376.0 (IC base=+0.157)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0058` → IC=+0.223 (n=1189)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0058 (IC base=+0.211)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.216 (n=1248)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3664` → IC=+0.268 (n=1013)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2457` → IC=+0.226 (n=684)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2457 (IC base=+0.231)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.242 (n=596)
...
- **PATRÓN** `ballena_activa_n` < `24.0` → IC=+0.244 (n=420)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4318` → IC=+0.136 (n=1277)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.4318 (IC base=+0.134)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.146 (n=1341)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 5.0 (IC base=+0.134)

- **PATRÓN** `ibs_20min` > `0.7095` → IC=+0.233 (n=851)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7095 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` > `0.3538` → IC=+0.182 (n=483)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.3538 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.292` → IC=+0.165 (n=545)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 4.292 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` < `0.8805` → IC=+0.158 (n=852)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5044` → IC=+0.148 (n=540)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.5044 (IC base=+0.134)

- **PATRÓN** `volumen_spike_ratio` > `2.4694` → IC=+0.159 (n=409)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 2.4694 (IC base=+0.134)

- **PATRÓN** `libro_liquidez` > `8770.6206` → IC=+0.230 (n=579)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8770.6206 (IC base=+0.134)

- **PATRÓN** `ballena_activa_n` < `90.0` → IC=+0.158 (n=384)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4329` → IC=+0.153 (n=1028)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6883` → IC=+0.191 (n=1028)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` < 0.6883 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.2048` → IC=+0.144 (n=957)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.245` → IC=+0.138 (n=961)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` < 4.245 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `0.8608` → IC=+0.142 (n=686)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5565` → IC=+0.151 (n=422)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.5565 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` > `2.4671` → IC=+0.171 (n=320)
...
- **PATRÓN** `ballena_activa_n` < `195.0` → IC=+0.144 (n=951)

  - _Acción_: Kelly boost +0.72€ cuando `ballena_activa_n` < 195.0 (IC base=+0.138)

### GBM_LATE_15M_TARDIO#SOL#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.120 (n=1336)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` > 5.0 (IC base=+0.098)

- **PATRÓN** `ibs_20min` > `0.4667` → IC=+0.178 (n=1301)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.447` → IC=+0.220 (n=488)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.447 (IC base=+0.098)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.124 (n=906)
...
- **PATRÓN** `ballena_activa_n` < `53.0` → IC=+0.127 (n=956)
...
- **PATRÓN** `sigma_h` > `0.0239` → IC=+0.218 (n=594)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0239 (IC base=+0.205)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.212 (n=1368)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.557` → IC=+0.243 (n=714)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.557 (IC base=+0.205)

- **PATRÓN** `volumen_regimen` < `1.2439` → IC=+0.205 (n=1309)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2439 (IC base=+0.205)

- **PATRÓN** `volumen_regimen` > `0.627` → IC=+0.209 (n=1309)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.627 (IC base=+0.205)

- **PATRÓN** `volumen_pendiente_norm` > `0.2373` → IC=+0.235 (n=236)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2373 (IC base=+0.205)

- **PATRÓN** `volumen_spike_ratio` > `2.5891` → IC=+0.236 (n=419)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5891 (IC base=+0.205)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.212 (n=1314)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.205)

- **PATRÓN** `libro_liquidez` > `2565.5344` → IC=+0.211 (n=873)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.211 (n=697)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.200)

- **PATRÓN** `ibs_20min` < `0.5043` → IC=+0.255 (n=1446)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5043 (IC base=+0.200)

- **PATRÓN** `dist_vwap_pct` < `0.8605` → IC=+0.204 (n=1625)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.8605 (IC base=+0.200)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.017` → IC=+0.259 (n=272)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.017 (IC base=+0.200)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2259` → IC=+0.194 (n=1114)
...
- **PATRÓN** `libro_liquidez` > `2541.4798` → IC=+0.202 (n=964)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2541.4798 (IC base=+0.200)

### GBM_LATE_5M
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.140 (n=2559)

- **PATRÓN** `sigma_h` < `0.0094` → IC=+0.155 (n=2219)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0094 (IC base=+0.144)

- **PATRÓN** `drift_60min` |x|≤ `0.5286` → IC=+0.155 (n=2521)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.5286 (IC base=+0.144)
...
- **PATRÓN** `ibs_20min` > `0.9374` → IC=+0.211 (n=841)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9374 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `0.1891` → IC=+0.154 (n=820)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.1891 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.736` → IC=+0.166 (n=810)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4861` → IC=+0.156 (n=1931)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.1769` → IC=+0.161 (n=850)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` < 0.1769 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.7057` → IC=+0.147 (n=344)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.7057 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.265` → IC=+0.146 (n=1914)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` < 6.265 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `1.1144` → IC=+0.146 (n=1612)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 1.1144 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` > `0.0723` → IC=+0.152 (n=911)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_pendiente_norm` > 0.0723 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` < `2.5735` → IC=+0.142 (n=1912)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 2.5735 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `1.8251` → IC=+0.149 (n=1274)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.8251 (IC base=+0.137)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.140 (n=2559)

  - _Acción_: Kelly boost +0.70€ cuando `libro_spread` < 0.01 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `7756.2728` → IC=+0.150 (n=1725)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 7756.2728 (IC base=+0.137)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3655` → IC=+0.146 (n=862)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.3655 (IC base=+0.133)
...
- **PATRÓN** `ibs_20min` < `0.1518` → IC=+0.162 (n=380)

  - _Acción_: Kelly boost +0.81€ cuando `ibs_20min` < 0.1518 (IC base=+0.133)

- **PATRÓN** `ibs_20min` > `0.6078` → IC=+0.144 (n=391)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` > 0.6078 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` > `0.6015` → IC=+0.155 (n=117)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.6015 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `0.2289` → IC=+0.134 (n=889)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.2289 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.22` → IC=+0.158 (n=842)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` < 6.22 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` < `0.8847` → IC=+0.178 (n=575)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` < 0.8847 (IC base=+0.133)

- **PATRÓN** `volumen_pendiente_norm` > `0.0693` → IC=+0.162 (n=409)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` > 0.0693 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` < `2.5735` → IC=+0.142 (n=859)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 2.5735 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` > `1.8205` → IC=+0.147 (n=573)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.8205 (IC base=+0.133)

- **PATRÓN** `libro_liquidez` > `11358.0518` → IC=+0.149 (n=862)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 11358.0518 (IC base=+0.133)
...
- **FILTRO** `ibs_20min` < `0.6407` → IC=-0.176 (n=183)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.215 (n=549)
...
- **PATRÓN** `ibs_20min` > `0.6407` → IC=+0.215 (n=549)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6407 (IC base=+0.101)

- **PATRÓN** `dist_vwap_pct` > `0.1343` → IC=+0.166 (n=285)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.1343 (IC base=+0.101)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.105` → IC=+0.217 (n=136)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.105 (IC base=+0.101)

- **PATRÓN** `volumen_regimen` < `0.7924` → IC=+0.123 (n=367)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.135 (n=445)
...
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.173 (n=148)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0049 (IC base=+0.116)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.140 (n=198)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 8.0 (IC base=+0.116)

- **PATRÓN** `ibs_20min` > `0.6407` → IC=+0.241 (n=191)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6407 (IC base=+0.116)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `11.057` → IC=+0.309 (n=66)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.057 (IC base=+0.116)

- **PATRÓN** `volumen_regimen` < `0.789` → IC=+0.162 (n=128)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.789 (IC base=+0.116)

- **PATRÓN** `volumen_regimen` > `0.6214` → IC=+0.142 (n=171)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 0.6214 (IC base=+0.116)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7434` → IC=+0.160 (n=95)
  ...
  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 1.7434 (IC base=+0.116)

- **PATRÓN** `volumen_spike_ratio` > `1.3826` → IC=+0.143 (n=141)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.3826 (IC base=+0.116)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.148 (n=200)

  - _Acción_: Kelly boost +0.74€ cuando `libro_spread` < 0.02 (IC base=+0.116)

- **PATRÓN** `libro_liquidez` > `1072.6991` → IC=+0.186 (n=167)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 1072.6991 (IC base=+0.116)
...
- **FILTRO** `ibs_20min` > `0.2374` → IC=-0.134 (n=99)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2374
  - _Potencial_: sin este filtro IC_bueno=+0.138 (n=194)
...
- **PATRÓN** `ibs_20min` < `0.2374` → IC=+0.138 (n=194)

  - _Acción_: Kelly boost +0.69€ cuando `ibs_20min` < 0.2374 (IC base=+0.046)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.126` → IC=+0.143 (n=96)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 6.126 (IC base=+0.046)

- **PATRÓN** `libro_liquidez` > `3771.3449` → IC=+0.157 (n=100)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 3771.3449 (IC base=+0.046)
...
### LEADLAG_BTC_XRP_15M
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.128 (n=482)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 5.0 (IC base=+0.111)

- **PATRÓN** `py_entrada` > `0.5` → IC=+0.124 (n=447)

  - _Acción_: Kelly boost +0.62€ cuando `py_entrada` > 0.5 (IC base=+0.111)

- **PATRÓN** `libro_liquidez` > `2816.9896` → IC=+0.171 (n=159)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 2816.9896 (IC base=+0.111)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.121 (n=505)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 6.0 (IC base=+0.098)

- **PATRÓN** `libro_liquidez` > `2303.417` → IC=+0.126 (n=530)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=1313)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.125 (n=1004)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=5397)

- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=6144)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.179 (n=2582)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=7852)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.171 (n=2671)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.033 (n=8209)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.43` → IC=-0.216 (n=421)

  - _Acción_: SKIP cuando `py_entrada` < 0.43
  - _Potencial_: sin este filtro IC_bueno=+0.091 (n=1331)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.43` → IC=-0.197 (n=444)

  - _Acción_: SKIP cuando `py_entrada` < 0.43
  - _Potencial_: sin este filtro IC_bueno=+0.080 (n=1359)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.216 (n=456)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.060 (n=1454)

- **FILTRO** `ibs_20min` > `0.2865` → IC=-0.168 (n=477)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2865
  - _Potencial_: sin este filtro IC_bueno=+0.048 (n=1433)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.47` → IC=-0.203 (n=422)
...
- **FILTRO** `py_entrada` > `0.59` → IC=-0.194 (n=465)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.046 (n=1451)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=2366)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=2372)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=7497)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=17072)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.282 (n=5685)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=18884)

- **FILTRO** `ibs_7min` < `0.7062` → IC=-0.238 (n=6141)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7062
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=18428)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.160 (n=8159)

  - _Acción_: SKIP cuando `ballena_activa_n` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=16410)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.224 (n=7657)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=23111)

- **FILTRO** `ibs_7min` > `0.297` → IC=-0.177 (n=7691)

  - _Acción_: SKIP cuando `ibs_7min` > 0.297
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=23077)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.312 (n=925)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=2999)

- **FILTRO** `ibs_7min` < `0.7099` → IC=-0.258 (n=1294)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7099
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=2630)

- **FILTRO** `ballena_activa_n` > `9.0` → IC=-0.194 (n=951)

  - _Acción_: SKIP cuando `ballena_activa_n` > 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=2973)

- **FILTRO** `py_entrada` > `0.505` → IC=-0.147 (n=3600)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.098 (n=1758)

- **FILTRO** `ibs_7min` > `0.7987` → IC=-0.206 (n=1339)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7987
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=4019)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.139 (n=988)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.081 (n=3314)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.255 (n=998)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=3304)

- **FILTRO** `ibs_7min` < `0.7596` → IC=-0.186 (n=1075)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7596
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=3227)

- **FILTRO** `ballena_activa_n` > `160.0` → IC=-0.170 (n=1074)

  - _Acción_: SKIP cuando `ballena_activa_n` > 160.0
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=3228)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.255 (n=1018)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=3336)

- **FILTRO** `ibs_7min` > `0.2504` → IC=-0.167 (n=1088)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2504
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=3266)

- **FILTRO** `ballena_activa_n` > `114.0` → IC=-0.164 (n=1468)

  - _Acción_: SKIP cuando `ballena_activa_n` > 114.0
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=2886)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.181 (n=898)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.095 (n=2791)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.320 (n=883)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=2806)

- **FILTRO** `drift_7min_pct` |x|> `0.181` → IC=-0.131 (n=1254)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.181
  - _Potencial_: sin este filtro IC_bueno=-0.108 (n=2435)

- **FILTRO** `ibs_7min` < `0.2034` → IC=-0.274 (n=922)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2034
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=2767)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.218 (n=892)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=2797)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.235 (n=1321)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=4306)

- **FILTRO** `ibs_7min` > `0.2647` → IC=-0.158 (n=1909)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2647
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=3718)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.127 (n=1277)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.089 (n=2760)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.251 (n=974)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=3063)

- **FILTRO** `ibs_7min` < `0.7455` → IC=-0.191 (n=1009)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7455
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3028)

- **FILTRO** `ballena_activa_n` > `33.0` → IC=-0.184 (n=989)

  - _Acción_: SKIP cuando `ballena_activa_n` > 33.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3048)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.261 (n=1012)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=3128)

- **FILTRO** `ibs_7min` > `0.2748` → IC=-0.176 (n=1034)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2748
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3106)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.180 (n=1029)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=3111)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.240 (n=1102)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=3359)

- **FILTRO** `ibs_7min` < `0.7241` → IC=-0.211 (n=1112)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7241
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=3349)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.177 (n=1402)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.019 (n=4414)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.124 (n=1246)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=2910)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.279 (n=995)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=3161)

- **FILTRO** `ibs_7min` < `0.7333` → IC=-0.228 (n=1039)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7333
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=3117)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.205 (n=1021)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=3135)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.205 (n=1307)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=4166)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.128 (n=41)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.013 (n=913)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=2296)
...
### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.188 (n=350)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0039 (IC base=+0.169)

- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.190 (n=476)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0071 (IC base=+0.169)

- **PATRÓN** `drift_60min` |x|≤ `0.0787` → IC=+0.171 (n=463)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.0787 (IC base=+0.169)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0589` → IC=+0.173 (n=1049)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.86€ cuando `delta_ratio_macro` |x|> 0.0589 (IC base=+0.169)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3429` → IC=+0.204 (n=729)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3429 (IC base=+0.169)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.173 (n=744)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 11.0 (IC base=+0.169)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.182 (n=501)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` < 6.0 (IC base=+0.169)

- **PATRÓN** `ibs_15` > `0.617` → IC=+0.239 (n=1049)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.617 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` > `0.1504` → IC=+0.165 (n=517)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.1504 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` < `0.1089` → IC=+0.168 (n=694)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` < 0.1089 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.868` → IC=+0.241 (n=381)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.868 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `5104.4221` → IC=+0.180 (n=476)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 5104.4221 (IC base=+0.169)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `20.029` → IC=+0.141 (n=62)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` > 20.029 (IC base=-0.005)

### UPDOWN_GBM#ETH#15min
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.146 (n=252)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` < 0.0061 (IC base=+0.129)

- **PATRÓN** `drift_60min` |x|≤ `0.0736` → IC=+0.140 (n=112)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.70€ cuando `drift_60min` |x|≤ 0.0736 (IC base=+0.129)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1428` → IC=+0.159 (n=168)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.79€ cuando `delta_ratio_macro` |x|> 0.1428 (IC base=+0.129)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.266` → IC=+0.161 (n=160)

  - _Acción_: Kelly boost +0.80€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.266 (IC base=+0.129)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.135 (n=187)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 11.0 (IC base=+0.129)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.138 (n=252)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` < 16.0 (IC base=+0.129)

- **PATRÓN** `ibs_15` > `0.6489` → IC=+0.221 (n=252)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6489 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` < `0.4172` → IC=+0.141 (n=263)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.4172 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.912` → IC=+0.205 (n=110)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.912 (IC base=+0.129)

- **PATRÓN** `libro_liquidez` > `10110.2635` → IC=+0.129 (n=114)

  - _Acción_: Kelly boost +0.65€ cuando `libro_liquidez` > 10110.2635 (IC base=+0.129)
...
- **FILTRO** `dist_vwap_pct` > `0.5885` → IC=-0.147 (n=114)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.063 (n=581)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0851` → IC=+0.202 (n=132)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0851 (IC base=+0.176)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0668` → IC=+0.189 (n=268)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.94€ cuando `delta_ratio_macro` |x|> 0.0668 (IC base=+0.176)
...
- **PATRÓN** `ibs_15` > `0.5354` → IC=+0.272 (n=300)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5354 (IC base=+0.176)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `7.408` → IC=+0.177 (n=267)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` < 7.408 (IC base=+0.176)

- **PATRÓN** `libro_liquidez` > `2714.4944` → IC=+0.225 (n=136)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2714.4944 (IC base=+0.176)
...
- **FILTRO** `sigma_h` > `0.0125` → IC=-0.201 (n=526)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `ibs_15` < `0.6046` → IC=-0.184 (n=175)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.249 (n=527)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2866` → IC=+0.218 (n=317)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.2866 (IC base=-0.058)

- **PATRÓN** `ibs_15` > `0.6046` → IC=+0.249 (n=527)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6046 (IC base=-0.058)

- **PATRÓN** `dist_vwap_pct` < `0.2672` → IC=+0.168 (n=411)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` < 0.2672 (IC base=-0.058)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1199` → IC=+0.239 (n=730)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1199 (IC base=-0.046)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1819` → IC=+0.237 (n=697)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1819 (IC base=-0.046)

- **PATRÓN** `ibs_15` < `0.3542` → IC=+0.278 (n=1096)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3542 (IC base=-0.046)

- **PATRÓN** `dist_vwap_pct` > `0.6551` → IC=+0.265 (n=168)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6551 (IC base=-0.046)

### UPDOWN_GBM_15M_TARDIO#BTC#15min
- **FILTRO** `sigma_h` > `0.007` → IC=-0.208 (n=313)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.007
  - _Potencial_: sin este filtro IC_bueno=-0.196 (n=940)

- **FILTRO** `sigma_h` < `0.0032` → IC=-0.233 (n=313)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0032
  - _Potencial_: sin este filtro IC_bueno=-0.188 (n=940)

- **FILTRO** `hora_utc` > `7.0` → IC=-0.213 (n=799)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.175 (n=454)

- **FILTRO** `sigma_ewma_delta_pct` > `19.79` → IC=-0.246 (n=226)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 19.79
  - _Potencial_: sin este filtro IC_bueno=-0.189 (n=1027)
...
- **FILTRO** `ibs_15` < `0.65` → IC=-0.236 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.249 (n=257)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.132 (n=17)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.142 (n=325)

- **PATRÓN** `sigma_h` > `0.0039` → IC=+0.151 (n=230)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0039 (IC base=+0.128)

- **PATRÓN** `drift_60min` |x|≤ `0.0768` → IC=+0.213 (n=113)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0768 (IC base=+0.128)

- **PATRÓN** `drift_15min` |x|≤ `0.4178` → IC=+0.159 (n=86)

  - _Acción_: Kelly boost +0.80€ cuando `drift_15min` |x|≤ 0.4178 (IC base=+0.128)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0897` → IC=+0.129 (n=230)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.65€ cuando `delta_ratio_macro` |x|> 0.0897 (IC base=+0.128)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2968` → IC=+0.225 (n=169)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.2968 (IC base=+0.128)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.176 (n=109)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 16.0 (IC base=+0.128)

- **PATRÓN** `ibs_15` > `0.65` → IC=+0.249 (n=257)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.65 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` < `0.1025` → IC=+0.172 (n=184)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.1025 (IC base=+0.128)

- **PATRÓN** `sigma_ewma_delta_pct` < `18.822` → IC=+0.135 (n=275)

  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` < 18.822 (IC base=+0.128)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.142 (n=325)

  - _Acción_: Kelly boost +0.71€ cuando `libro_spread` < 0.01 (IC base=+0.128)

- **PATRÓN** `libro_liquidez` > `10550.3134` → IC=+0.189 (n=117)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 10550.3134 (IC base=+0.128)

- **PATRÓN** `sigma_h` < `0.0074` → IC=+0.248 (n=459)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0074 (IC base=+0.224)

- **PATRÓN** `drift_60min` |x|≤ `0.4379` → IC=+0.229 (n=459)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4379 (IC base=+0.224)

- **PATRÓN** `drift_15min` |x|≤ `0.7642` → IC=+0.227 (n=404)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.7642 (IC base=+0.224)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2074` → IC=+0.257 (n=208)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.2074 (IC base=+0.224)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.226 (n=213)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` < `0.361` → IC=+0.270 (n=459)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.361 (IC base=+0.224)

- **PATRÓN** `dist_vwap_pct` > `0.3779` → IC=+0.248 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3779 (IC base=+0.224)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.175` → IC=+0.233 (n=174)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.175 (IC base=+0.224)

- **PATRÓN** `sigma_ewma_delta_pct` < `12.033` → IC=+0.228 (n=486)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 12.033 (IC base=+0.224)
...
- **FILTRO** `sigma_h` > `0.019` → IC=-0.252 (n=317)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.019
  - _Potencial_: sin este filtro IC_bueno=-0.106 (n=318)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.261 (n=174)

  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.148 (n=461)
...
- **PATRÓN** `ibs_15` < `0.3333` → IC=+0.298 (n=325)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
### UPDOWN_GBM_IBS_ALTO
- **PATRÓN** `sigma_h` < `0.0043` → IC=+0.294 (n=343)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0043 (IC base=+0.283)

- **PATRÓN** `sigma_h` > `0.0028` → IC=+0.283 (n=514)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_h` < `0.0059` → IC=+0.303 (n=201)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0059 (IC base=+0.291)

- **PATRÓN** `sigma_h` > `0.0034` → IC=+0.296 (n=228)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1481` → IC=+0.312 (n=152)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1481 (IC base=+0.291)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2925` → IC=+0.333 (n=166)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.2925 (IC base=+0.291)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.318 (n=218)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.291)

- **PATRÓN** `ibs_15` > `0.8527` → IC=+0.335 (n=228)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **PATRÓN** `dist_vwap_pct` < `0.4471` → IC=+0.295 (n=232)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4471 (IC base=+0.291)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.72` → IC=+0.314 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.72 (IC base=+0.291)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.301 (n=260)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.291)

- **PATRÓN** `ballena_activa_n` < `159.0` → IC=+0.294 (n=173)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 159.0 (IC base=+0.291)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.617 sube el IC de +0.169 a +0.239 en UPDOWN_GBM#15min (n=1049). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.6489 sube el IC de +0.129 a +0.221 en UPDOWN_GBM#ETH#15min (n=252). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_YES, IBS > 0.5354 sube el IC de +0.176 a +0.272 en UPDOWN_GBM#XRP#15min (n=300). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1053 sube el IC de +0.050 a +0.178 en UPDOWN_GBM#XRP#15min (n=333). Ya aplicado como kelly_boost=+0.89€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.6046 sube el IC de -0.058 a +0.249 en UPDOWN_GBM_15M_TARDIO (n=527). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3542 sube el IC de -0.046 a +0.278 en UPDOWN_GBM_15M_TARDIO (n=1096). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_YES, IBS > 0.65 sube el IC de +0.128 a +0.249 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=257). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.361 sube el IC de +0.224 a +0.270 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=459). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#XRP#15min**: dentro de BUY_NO, IBS < 0.3333 sube el IC de -0.046 a +0.298 en UPDOWN_GBM_15M_TARDIO#XRP#15min (n=325). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.8357 sube el IC de +0.283 a +0.324 en UPDOWN_GBM_IBS_ALTO (n=514). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO#ETH#15min**: dentro de BUY_YES, IBS > 0.8527 sube el IC de +0.291 a +0.335 en UPDOWN_GBM_IBS_ALTO#ETH#15min (n=228). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS |
71€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#240min | 19 | -0.023 | -0.57€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#5min | 1853 | +0.065 | +122.51€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 4389 | +0.031 | +287.98€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 784 | +0.083 | +184.81€ | 0 | 13 |
| ✅ UPDOWN_GBM#BTC#240min | 263 | +0.025 | +7.08€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#5min | 2657 | +0.025 | +92.08€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#60min | 646 | +0.000 | +2.33€ | 0 | 1 |
| ✅ UPDOWN_GBM#BTC#daily | 39 | -0.110 | +1.69€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 2937 | +0.033 | +94.66€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 229 | +0.106 | +55.02€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#240min | 16 | +0.000 | -0.34€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 2692 | +0.026 | +39.99€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 4962 | +0.015 | +195.07€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 1810 | +0.043 | +189.87€ | 0 | 10 |
| ✅ UPDOWN_GBM#ETH#240min | 250 | +0.008 | +8.40€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 2344 | +0.003 | +0.29€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 524 | -0.009 | -7.89€ | 2 | 0 |
| ✅ UPDOWN_GBM#ETH#daily | 34 | -0.139 | +4.40€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 6458 | +0.016 | +150.44€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 1770 | +0.023 | +102.02€ | 1 | 12 |
| ✅ UPDOWN_GBM#SOL#240min | 244 | -0.008 | -2.13€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 4128 | +0.018 | +56.16€ | 1 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 288 | -0.007 | -5.53€ | 0 | 1 |
| ✅ UPDOWN_GBM#SOL#daily | 28 | -0.133 | -0.08€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 3745 | +0.040 | +414.34€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 1719 | +0.079 | +421.72€ | 0 | 11 |
| ✅ UPDOWN_GBM#XRP#240min | 132 | -0.015 | -3.58€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 1894 | +0.008 | -3.81€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 8828 | -0.049 | +1841.39€ | 3 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 8828 | -0.049 | +1841.39€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 403 | -0.053 | +344.78€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 403 | -0.053 | +344.78€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 1685 | -0.130 | -12.13€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 1685 | -0.130 | -12.13€ | 4 | 6 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 139 | +0.117 | +54.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 139 | +0.117 | +54.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 953 | +0.190 | +533.90€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 953 | +0.190 | +533.90€ | 2 | 21 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 2837 | -0.061 | +448.22€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 2837 | -0.061 | +448.22€ | 4 | 5 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 2811 | -0.076 | +472.08€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 2811 | -0.076 | +472.08€ | 2 | 3 |
...
| ✅ UPDOWN_GBM_IBS_ALTO | 685 | +0.283 | +546.17€ | 0 | 11 |
| ✅ UPDOWN_GBM_IBS_ALTO#15min | 685 | +0.283 | +546.17€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#BTC | 381 | +0.276 | +288.87€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#BTC#15min | 381 | +0.276 | +288.87€ | 0 | 11 |
| ✅ UPDOWN_GBM_IBS_ALTO#ETH | 304 | +0.291 | +257.30€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#ETH#15min | 304 | +0.291 | +257.30€ | 0 | 12 |
...
**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  ...
  - _Estado_: 449 celda(s) pasan gate riguroso completo de 2037 evaluadas (n>=40) y 2992 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.023 < 0.08 — monitorear
  - _Datos_: n=1769 IC=+0.023 PNL=+101.53€
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=523/40 IC=-0.009 PNL=-7.38€ | BTC#60min: n=646/40 IC=+0.000 PNL=+2.33€ | SOL#60min: n=288/40 IC=-0.007 PNL=-5.53€

**⏳ H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  - _Umbral_: n≥40 tras 2 losses y gap(IC_tras_win - IC_tras_2loss)≥0.05
  - _Acción_: Reducir stake (no desactivar) 1-2h tras 2 derrotas consecutivas en el mismo subtype
  - _Estado_: tras_win IC=+0.054 n=254628 | tras_1loss IC=+0.071 n=199207 | tras_2loss IC=+0.039 n=85486/40 | gap=+0.015 (umbral 0.05)
...
**🟡 H-24H-GBM-BUYYES-TARDE** — GBM BUY_YES en tarde europea (15-19h UTC) — señal alcista sostenida
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.193 > 0.08 con n=255 PNL=+160.73€
  - _Datos_: n=255 IC=+0.193 PNL=+160.73€
...
**〰️ H-CUSTOM-GBM-60MIN-BUYYES** — GBM 60min BUY_YES — ¿edge superior al BUY_NO?
  ...
  - _Estado_: n=1071 IC=-0.005 PNL=-15.76€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1071 IC=-0.005 PNL=-15.76€

**〰️ H-CUSTOM-GBM-60MIN-BUYNO** — GBM 60min BUY_NO — tracking por separado
  - _Hipótesis_: En 15min BUY_NO tiene IC=+0.119. ¿Se repite en 60min? Datos actuales: 8/14 (57%) IC=+0.044 — positivo pero débil. Puede ser que 60min requiera dirección alcista (BUY_YES) y no bajista.
  - _Umbral_: n≥30 para confirmar dirección
  - _Acción_: Si IC<0.05 con n≥30 → en 60min priorizar solo BUY_YES; si IC>0.08 → igualar al BUY_YES
  - _Estado_: n=386 IC=-0.003 PNL=+5.18€ — sin señal clara aún (umbral IC: min=0.05 max=None)
  - _Datos_: n=386 IC=-0.003 PNL=+5.18€
...
**🟡 H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.168 > 0.1 con n=1395 PNL=+771.65€
  - _Datos_: n=1395 IC=+0.168 PNL=+771.65€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=782 IC=+0.082 PNL=+183.04€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=782 IC=+0.082 PNL=+183.04€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=7928 IC=+0.048 PNL=+885.34€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=7928 IC=+0.048 PNL=+885.34€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=1318 IC=+0.050 PNL=+161.90€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1318 IC=+0.050 PNL=+161.90€
...
**🟡 H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.133 > 0.1 con n=243 PNL=+76.46€
  - _Datos_: n=243 IC=+0.133 PNL=+76.46€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=13206 IC=-0.142 PNL=+665.00€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=13206 IC=-0.142 PNL=+665.00€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=1464 IC=+0.141 PNL=+793.96€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=1464 IC=+0.141 PNL=+793.96€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.241 < -0.1 con n=1346 PNL=-196.76€
  - _Datos_: n=1346 IC=-0.241 PNL=-196.76€
...
**🔶 H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.097 n=705) pero sin cruzar ≥2 pares más — sin otros pares con datos
  - _Datos_: n=705 IC=+0.097 PNL=+180.20€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=6884 IC=+0.169 PNL=-884.47€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=6884 IC=+0.169 PNL=-884.47€
