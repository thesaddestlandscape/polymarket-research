# Hipótesis automáticas — 2026-07-30 02:11 UTC
_Generado por shadow_postmortem.py sobre 42718 resoluciones (PNL=+9337.85€)_
...
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.188 (n=1408)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0078 (IC base=+0.146)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.167 (n=1903)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 8.0 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `0.1144` → IC=+0.172 (n=863)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.1144 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.189` → IC=+0.242 (n=712)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.189 (IC base=+0.146)

- **PATRÓN** `sigma_h` > `0.0043` → IC=+0.140 (n=2289)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` > 0.0043 (IC base=+0.125)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.137 (n=1567)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 12.0 (IC base=+0.125)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.126 (n=845)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` < 6.0 (IC base=+0.125)

- **PATRÓN** `dist_vwap_pct` > `0.6713` → IC=+0.136 (n=248)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 0.6713 (IC base=+0.125)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.792` → IC=+0.140 (n=501)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` > 5.792 (IC base=+0.125)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.131 (n=477)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 6.0 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` > `0.4541` → IC=+0.162 (n=63)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.4541 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.57` → IC=+0.159 (n=86)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 11.57 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.309` → IC=+0.145 (n=212)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` > 4.309 (IC base=+0.089)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **PATRÓN** `sigma_h` > `0.008` → IC=+0.180 (n=167)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.008 (IC base=+0.115)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.135 (n=332)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 12.0 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` > `0.1183` → IC=+0.142 (n=224)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.1183 (IC base=+0.115)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.315` → IC=+0.183 (n=137)

  - _Acción_: Kelly boost +0.92€ cuando `sigma_ewma_delta_pct` > 7.315 (IC base=+0.115)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.389` → IC=+0.173 (n=111)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 9.389 (IC base=+0.082)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.154 (n=512)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0078 (IC base=+0.121)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.151 (n=462)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 8.0 (IC base=+0.121)

- **PATRÓN** `dist_vwap_pct` > `0.1896` → IC=+0.163 (n=191)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.1896 (IC base=+0.121)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.397` → IC=+0.285 (n=226)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.397 (IC base=+0.121)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.122 (n=464)

  - _Acción_: Kelly boost +0.61€ cuando `libro_spread` < 0.01 (IC base=+0.121)

- **PATRÓN** `libro_liquidez` > `1730.8259` → IC=+0.154 (n=426)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 1730.8259 (IC base=+0.121)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.3` → IC=+0.130 (n=447)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` < 2.3 (IC base=+0.090)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` < `0.0166` → IC=+0.233 (n=484)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0166 (IC base=+0.223)

- **PATRÓN** `sigma_h` > `0.0111` → IC=+0.223 (n=323)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0111 (IC base=+0.223)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.236 (n=498)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.223)

- **PATRÓN** `dist_vwap_pct` > `0.5156` → IC=+0.281 (n=94)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5156 (IC base=+0.223)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.462` → IC=+0.348 (n=136)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.462 (IC base=+0.223)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.217 (n=536)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.223)

- **PATRÓN** `sigma_h` < `0.0164` → IC=+0.214 (n=578)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0164 (IC base=+0.210)

- **PATRÓN** `sigma_h` > `0.007` → IC=+0.232 (n=581)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.007 (IC base=+0.210)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.248 (n=391)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 12.0 (IC base=+0.210)

- **PATRÓN** `dist_vwap_pct` > `0.1929` → IC=+0.322 (n=116)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1929 (IC base=+0.210)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.29` → IC=+0.210 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.29 (IC base=+0.210)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.101` → IC=+0.205 (n=638)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 9.101 (IC base=+0.210)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.222 (n=477)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.210)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0103` → IC=+0.126 (n=954)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.63€ cuando `sigma_h` > 0.0103 (IC base=+0.095)

- **PATRÓN** `dist_vwap_pct` > `0.4892` → IC=+0.140 (n=376)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.4892 (IC base=+0.095)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.757` → IC=+0.227 (n=346)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.757 (IC base=+0.095)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `dist_vwap_pct` > `0.3646` → IC=+0.134 (n=80)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` > 0.3646 (IC base=+0.067)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.355` → IC=+0.184 (n=74)

  - _Acción_: Kelly boost +0.92€ cuando `sigma_ewma_delta_pct` > 12.355 (IC base=+0.067)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.126` → IC=+0.228 (n=167)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.126 (IC base=+0.047)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` < `0.009` → IC=+0.175 (n=201)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.009 (IC base=+0.166)

- **PATRÓN** `sigma_h` > `0.01` → IC=+0.170 (n=537)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.01 (IC base=+0.166)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.181 (n=312)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 15.0 (IC base=+0.166)

- **PATRÓN** `dist_vwap_pct` > `0.5004` → IC=+0.234 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5004 (IC base=+0.166)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.791` → IC=+0.288 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.791 (IC base=+0.166)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.165 (n=463)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.166)

- **PATRÓN** `sigma_h` < `0.0217` → IC=+0.164 (n=688)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0217 (IC base=+0.159)

- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.171 (n=688)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0076 (IC base=+0.159)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.183 (n=468)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 12.0 (IC base=+0.159)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.160 (n=242)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 6.0 (IC base=+0.159)

- **PATRÓN** `dist_vwap_pct` > `0.1407` → IC=+0.233 (n=174)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1407 (IC base=+0.159)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.405` → IC=+0.159 (n=713)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` < 9.405 (IC base=+0.159)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.6` → IC=+0.175 (n=672)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.88€ cuando `ibs_15` > 0.6 (IC base=+0.056)

- **PATRÓN** `dist_vwap_pct` > `0.1663` → IC=+0.153 (n=249)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.1663 (IC base=+0.056)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.069` → IC=+0.152 (n=231)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 7.069 (IC base=+0.056)

- **PATRÓN** `libro_liquidez` > `10112.2619` → IC=+0.135 (n=195)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 10112.2619 (IC base=+0.056)

- **PATRÓN** `dist_vwap_pct` > `0.9424` → IC=+0.171 (n=68)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.9424 (IC base=+0.055)
...
- **PATRÓN** `sigma_h` < `0.0055` → IC=+0.208 (n=22)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0055 (IC base=+0.012)
...
- **PATRÓN** `ibs_15` > `0.6676` → IC=+0.193 (n=216)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.96€ cuando `ibs_15` > 0.6676 (IC base=+0.046)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.038` → IC=+0.167 (n=70)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 7.038 (IC base=+0.046)

- **PATRÓN** `sigma_h` < `0.006` → IC=+0.130 (n=190)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` < 0.006 (IC base=+0.063)

- **PATRÓN** `dist_vwap_pct` > `0.9263` → IC=+0.218 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9263 (IC base=+0.063)
...
- **PATRÓN** `drift_15min` |x|≤ `0.4608` → IC=+0.150 (n=78)

  - _Acción_: Kelly boost +0.75€ cuando `drift_15min` |x|≤ 0.4608 (IC base=+0.041)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.141 (n=179)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 12.0 (IC base=+0.088)

- **PATRÓN** `ibs_15` < `0.0448` → IC=+0.151 (n=81)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.75€ cuando `ibs_15` < 0.0448 (IC base=+0.088)

- **PATRÓN** `libro_liquidez` > `2982.5235` → IC=+0.160 (n=95)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 2982.5235 (IC base=+0.088)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6 sube el IC de +0.056 a +0.175 en UPDOWN_GBM#15min (n=672). Ya aplicado como kelly_boost=+0.88€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.6676 sube el IC de +0.046 a +0.193 en UPDOWN_GBM#ETH#15min (n=216). Ya aplicado como kelly_boost=+0.96€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.0448 sube el IC de +0.088 a +0.151 en UPDOWN_GBM#XRP#15min (n=81). Ya aplicado como kelly_boost=+0.75€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 1459 | +0.093 | +25.19€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 1459 | +0.093 | +25.19€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 590 | +0.088 | +1.27€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 590 | +0.088 | +1.27€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 543 | +0.109 | +5.59€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 543 | +0.109 | +5.59€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 307 | +0.073 | +18.69€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 307 | +0.073 | +18.69€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS | 89 | +0.291 | +5.22€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 78 | +0.312 | +1.27€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 11 | +0.064 | +3.95€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 78 | +0.312 | +1.27€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 78 | +0.312 | +1.27€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 6 | +0.075 | +0.57€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 6 | +0.075 | +0.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 7029 | +0.175 | -65.84€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 4622 | +0.198 | -65.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 324 | +0.052 | -28.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 821 | +0.128 | -56.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 1262 | +0.153 | +84.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 5 | +0.018 | -0.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 5 | +0.018 | -0.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 2102 | +0.180 | -9.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1554 | +0.195 | -53.79€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 108 | +0.036 | -16.85€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 430 | +0.174 | +64.88€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 8 | +0.120 | +5.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 7 | +0.097 | +4.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 2463 | +0.166 | -47.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1514 | +0.197 | -11.25€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 107 | -0.014 | -23.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 416 | +0.127 | -31.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 426 | +0.140 | +18.11€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#SOL | 2416 | +0.180 | -15.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1513 | +0.204 | -2.00€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 82 | +0.333 | -1.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 82 | +0.333 | -1.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC | 29 | +0.274 | -2.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC#15min | 29 | +0.274 | -2.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 28 | +0.333 | +0.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 28 | +0.333 | +0.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 19 | +0.294 | -0.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 19 | +0.294 | -0.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_EXTREMO | 9 | +0.184 | +0.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_EXTREMO#15min | 9 | +0.184 | +0.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION | 43 | +0.389 | +8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#60min | 43 | +0.389 | +8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#BTC | 18 | +0.315 | +4.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#BTC#60min | 18 | +0.315 | +4.42€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 460 | +0.303 | +6.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 460 | +0.303 | +6.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 460 | +0.303 | +6.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 460 | +0.303 | +6.65€ | 0 | 0 |
| ✅ GBM_LATE_15M | 9219 | +0.094 | +2791.61€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 9219 | +0.094 | +2791.61€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 175 | +0.189 | +109.64€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 175 | +0.189 | +109.64€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 2146 | +0.067 | +344.58€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 2146 | +0.067 | +344.58€ | 0 | 6 |
| ✅ GBM_LATE_15M#DOGE | 173 | +0.129 | +58.50€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 173 | +0.129 | +58.50€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 1920 | +0.067 | +292.76€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1920 | +0.067 | +292.76€ | 0 | 7 |
| ✅ GBM_LATE_15M#SOL | 2371 | +0.085 | +804.76€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 2371 | +0.085 | +804.76€ | 1 | 3 |
| ✅ GBM_LATE_15M#XRP | 2434 | +0.140 | +1181.36€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 2434 | +0.140 | +1181.36€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 6396 | +0.121 | +3152.61€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 6396 | +0.121 | +3152.61€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 130 | +0.295 | +145.57€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 130 | +0.295 | +145.57€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 1583 | +0.077 | +519.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 1583 | +0.077 | +519.98€ | 0 | 4 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 154 | +0.199 | +111.96€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 154 | +0.199 | +111.96€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1575 | +0.083 | +552.70€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1575 | +0.083 | +552.70€ | 0 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 1537 | +0.093 | +655.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 1537 | +0.093 | +655.08€ | 0 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 1417 | +0.216 | +1167.32€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 1417 | +0.216 | +1167.32€ | 0 | 13 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 321 | +0.048 | +96.99€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 321 | +0.048 | +96.99€ | 0 | 0 |
| 🚫 GBM_LATE_15M_PYCONFIRMADO#BTC | 9 | -0.102 | -1.94€ | 0 | 0 |
| 🚫 GBM_LATE_15M_PYCONFIRMADO#BTC#15min | 9 | -0.102 | -1.94€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH | 140 | -0.085 | -2.41€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH#15min | 140 | -0.085 | -2.41€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO | 6404 | +0.066 | +1800.54€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#15min | 6404 | +0.066 | +1800.54€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 147 | +0.225 | +112.16€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 147 | +0.225 | +112.16€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 1455 | +0.019 | +168.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 1455 | +0.019 | +168.08€ | 0 | 2 |
| ✅ GBM_LATE_15M_TARDIO#DOGE | 157 | +0.167 | +79.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#DOGE#15min | 157 | +0.167 | +79.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 1485 | +0.014 | +113.89€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 1485 | +0.014 | +113.89€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 1440 | +0.026 | +285.41€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 1440 | +0.026 | +285.41€ | 1 | 1 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 1720 | +0.162 | +1041.66€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1720 | +0.162 | +1041.66€ | 0 | 13 |
| ✅ GBM_LATE_5M | 1014 | -0.029 | +2.63€ | 0 | 0 |
| ✅ GBM_LATE_5M#5min | 1014 | -0.029 | +2.63€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC | 240 | -0.008 | -0.16€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC#5min | 240 | -0.008 | -0.16€ | 0 | 0 |
| 🚫 GBM_LATE_5M#ETH | 82 | -0.202 | -15.93€ | 0 | 0 |
| 🚫 GBM_LATE_5M#ETH#5min | 82 | -0.202 | -15.93€ | 0 | 0 |
| ✅ GBM_LATE_5M#SOL | 310 | -0.058 | +9.32€ | 0 | 0 |
| ✅ GBM_LATE_5M#SOL#5min | 310 | -0.058 | +9.32€ | 0 | 0 |
| ✅ GBM_LATE_5M#XRP | 382 | +0.021 | +9.41€ | 0 | 0 |
| ✅ GBM_LATE_5M#XRP#5min | 382 | +0.021 | +9.41€ | 0 | 0 |
...
| ✅ LATE_WINDOW_5MIN | 386 | -0.064 | -7.07€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN#5min | 386 | -0.064 | -7.07€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN#BTC | 386 | -0.064 | -7.07€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN#BTC#5min | 386 | -0.064 | -7.07€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M | 310 | +0.010 | +9.65€ | 1 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 310 | +0.010 | +9.65€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 310 | +0.010 | +9.65€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 310 | +0.010 | +9.65€ | 1 | 0 |
| ✅ ORDER_FLOW_5M | 1689 | +0.011 | +10.49€ | 1 | 0 |
| ✅ ORDER_FLOW_5M#5min | 1553 | +0.007 | -2.11€ | 0 | 0 |
...
| ✅ ORDER_FLOW_5M#XRP | 215 | -0.011 | -6.52€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#XRP#5min | 215 | -0.011 | -6.52€ | 1 | 0 |
...
| ✅ UPDOWN_GBM | 3139 | +0.039 | +306.80€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 2875 | +0.055 | +345.82€ | 0 | 5 |
| 🚫 UPDOWN_GBM#240min | 11 | -0.148 | -4.31€ | 0 | 0 |
| 🚫 UPDOWN_GBM#5min | 75 | -0.162 | -22.48€ | 4 | 0 |
| ✅ UPDOWN_GBM#60min | 131 | -0.064 | -11.72€ | 6 | 0 |
| ✅ UPDOWN_GBM#BNB | 343 | +0.054 | +61.72€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 343 | +0.054 | +61.72€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 606 | +0.056 | +78.84€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 522 | +0.088 | +93.32€ | 1 | 9 |
| 🚫 UPDOWN_GBM#BTC#5min | 19 | -0.158 | -7.19€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#60min | 43 | -0.078 | -6.67€ | 1 | 0 |
| ✅ UPDOWN_GBM#BTC#daily | 18 | -0.135 | +1.83€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 281 | +0.016 | +6.58€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 278 | +0.018 | +7.48€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 1163 | +0.043 | +85.73€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 1069 | +0.056 | +97.30€ | 0 | 4 |
| ✅ UPDOWN_GBM#ETH#240min | 5 | -0.018 | -0.44€ | 0 | 0 |
| 🚫 UPDOWN_GBM#ETH#5min | 19 | -0.204 | -7.68€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 55 | -0.026 | -3.07€ | 0 | 0 |
| 🚫 UPDOWN_GBM#ETH#daily | 15 | -0.154 | -0.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 363 | -0.015 | +13.43€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 293 | +0.012 | +20.37€ | 4 | 1 |
| 🚫 UPDOWN_GBM#SOL#5min | 23 | -0.060 | -3.42€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 33 | -0.100 | -1.97€ | 0 | 0 |
| 🚫 UPDOWN_GBM#SOL#daily | 12 | -0.129 | -0.13€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 381 | +0.061 | +62.33€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 370 | +0.070 | +65.62€ | 4 | 3 |
| 🚫 UPDOWN_GBM#XRP#5min | 11 | -0.106 | -3.29€ | 0 | 0 |
| 🚫 UPDOWN_GBM#daily | 45 | -0.202 | +1.33€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD | 209 | +0.310 | +49.56€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#15min | 209 | +0.310 | +49.56€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC | 99 | +0.312 | +23.98€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min | 99 | +0.312 | +23.98€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH | 110 | +0.304 | +25.58€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min | 110 | +0.304 | +25.58€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO | 2439 | +0.151 | +1081.34€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 2439 | +0.151 | +1081.34€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 243 | +0.218 | +170.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 243 | +0.218 | +170.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 249 | +0.110 | +42.92€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 249 | +0.110 | +42.92€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 225 | +0.200 | +146.74€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 225 | +0.200 | +146.74€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 463 | +0.165 | +171.17€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 463 | +0.165 | +171.17€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 572 | +0.084 | +137.85€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 572 | +0.084 | +137.85€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 687 | +0.172 | +412.10€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 687 | +0.172 | +412.10€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_IBS_ALTO | 12 | +0.171 | +2.51€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#15min | 12 | +0.171 | +2.51€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#ETH | 8 | +0.120 | +1.80€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#ETH#15min | 8 | +0.120 | +1.80€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.163: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.081 n=1887 | neutral: IC=+0.025 n=1457 | overbought(IBS>0.7): IC=+0.188 n=1756
  - _Datos_: n=5434 IC=+0.106 PNL=+1464.20€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=05h: IC=+0.155 n=111 PNL=+50.33€ → BOOST | H=10h: IC=-0.154 n=15 PNL=-6.77€ → FILTRAR | H=14h: IC=+0.135 n=135 PNL=+46.12€ → BOOST | H=22h: IC=+0.173 n=148 PNL=+43.39€ → BOOST
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=293 IC=+0.012 PNL=+20.37€

**🟡 H-WEEKLY** — Predicciones semanales de precio por par
  - _Umbral_: n≥15 por par con IC≥+0.05
  - _Acción_: Si confirma IC≥+0.10 n≥15 en SOL → considerar live semanal
  - _Estado_: ETH: n=130/15 IC=+0.174 PNL=-3.87€ | BTC: n=128/15 IC=+0.154 PNL=-11.62€ | SOL: n=199/15 IC=+0.321 PNL=+147.53€

**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  - _Umbral_: n≥40 tras 2 losses y gap(IC_tras_win - IC_tras_2loss)≥0.05
  - _Acción_: Reducir stake (no desactivar) 1-2h tras 2 derrotas consecutivas en el mismo subtype
  - _Estado_: tras_win IC=+0.112 n=25431 | tras_1loss IC=+0.078 n=17014 | tras_2loss IC=+0.051 n=7126/40 | gap=+0.061 (umbral 0.05)
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=-0.006 n=170 | contrario_BTC IC=-0.031 n=79/40 | gap=-0.025 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**🟡 H-24H-GBM-BUYYES-MADRUGADA** — GBM BUY_YES en madrugada europea (05-07h UTC) — señal alcista
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.100 > 0.08 con n=148 PNL=+51.21€
  - _Datos_: n=148 IC=+0.100 PNL=+51.21€
...
**〰️ H-24H-GBM-BUYYES-TARDE** — GBM BUY_YES en tarde europea (15-19h UTC) — señal alcista sostenida
  ...
  - _Estado_: n=314 IC=+0.003 PNL=-2.60€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=314 IC=+0.003 PNL=-2.60€
...
**🟡 H-WEEKLY-BUYNO** — WEEKLY_PRICE BUY_NO — dirección dominante con IC muy alto
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.315 > 0.1 con n=387 PNL=+146.57€
  - _Datos_: n=387 IC=+0.315 PNL=+146.57€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=3002 IC=+0.044 PNL=+320.59€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=3002 IC=+0.044 PNL=+320.59€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=1177 IC=+0.056 PNL=+158.21€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1177 IC=+0.056 PNL=+158.21€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=522 IC=+0.088 PNL=+93.32€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=522 IC=+0.088 PNL=+93.32€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.087 > 0.08 con n=1502 PNL=+252.85€
  - _Datos_: n=1502 IC=+0.087 PNL=+252.85€

**⏳ H-CUSTOM-LONGSHOT-BIAS** — Longshot bias — ¿mejor IC cuando py_mkt < 0.20 o > 0.80?
  - _Hipótesis_: Jon-Becker repo documenta formalmente: contratos a 1-20 cents tienen win_rate < precio implícito (compradores pierden sistemáticamente en longshots). En nuestro sistema: cuando py_mkt<0.20 el GBM predice BUY_NO con edge estructural adicional al del modelo. ¿Se confirma en nuestros datos? Buscar en feature pct_spot_vs_ref si los mercados extremos tienen mejor IC en BUY_NO.
  - _Umbral_: 30
  - _Acción_: Si IC>0.10 con n≥30 en mercados extremos → boost ×1.2 en BUY_NO cuando py_mkt<0.20
  - _Estado_: 26/30 ops en el filtro definido (IC actual=-0.143 PNL=-0.22€)
  - _Datos_: n=26 IC=-0.143 PNL=-0.22€

**〰️ H-CUSTOM-ETH15-REVERSION** — ETH#15min con drift_15min < -1 — ¿mean reversion?
  - _Hipótesis_: ETH y BTC tienen patrones opuestos: BTC funciona con momentum (drift>0.3). ETH funciona con reversión (drift<-1): 9/14 (64%) IC=+0.087. La hipótesis es que ETH tiene más mean-reversion que BTC en 15min.
  - _Umbral_: n≥20 y IC>+0.08
  - _Acción_: Si ETH drift<-1 confirma IC>0.08 con n≥20 → boost ×1.1 en ETH#15min cuando drift_15min<-1
  - _Estado_: n=120 IC=+0.025 PNL=+6.85€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=120 IC=+0.025 PNL=+6.85€

**〰️ H-CUSTOM-GBM-09H** — GBM a las 09h UTC — bloqueada 2026-06-29
  - _Hipótesis_: IC=-0.158 n=19 PNL=-11.62€. Bloqueada manualmente el 2026-06-29 añadiendo hora 9 a meta.gbm_blacklist_hours_auto. Esta hipótesis monitorea que el IC siga siendo negativo para justificar el bloqueo.
  - _Umbral_: n≥25 para confirmar el bloqueo es necesario
  - _Acción_: Si IC sube a >-0.05 con n≥30 → evaluar desbloquear. Si se mantiene <-0.10 → confirmar bloqueo permanente.
  - _Estado_: n=43 IC=-0.100 PNL=-8.87€ — sin señal clara aún (umbral IC: min=None max=-0.1)
  - _Datos_: n=43 IC=-0.100 PNL=-8.87€
...
**〰️ H-LATE-WINDOW-5MIN** — Late-window BTC 5min — arbitraje timing vs Polymarket
  ...
  - _Estado_: n=386 IC=-0.064 PNL=-7.07€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=386 IC=-0.064 PNL=-7.07€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=1629 IC=+0.046 PNL=+152.38€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1629 IC=+0.046 PNL=+152.38€

**〰️ H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  - _Hipótesis_: Feature nueva 2026-06-27: drift del precio YES en Polymarket en últimas 5 obs (~5min). Si poly_drift<0 y decidimos BUY_NO (o poly_drift>0 y BUY_YES) → confluencia. Si diverge → reducción de stake. Hipótesis: confluencia Binance+Polymarket mejora IC; divergencia empeora.
  - _Umbral_: n≥40 en confluencia vs divergencia para validar el boost ×1.1
  - _Acción_: Si IC_confluencia>IC_divergencia con n≥40 → mantener el boost. Si no → retirar.
  - _Estado_: n=484 IC=+0.074 PNL=+77.87€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=484 IC=+0.074 PNL=+77.87€
...
**〰️ H-CUSTOM-OF-VOLUMEN-ALTO** — ORDER_FLOW_5M con total_vol_5m alto — ¿volumen extremo mejora el IC?
  ...
  - _Estado_: n=383 IC=-0.014 PNL=-11.16€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=383 IC=-0.014 PNL=-11.16€
...
**🟡 H-CUSTOM-CROSS-WINDOW-SPREAD-POS** — GBM 15min/60min: spread positivo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.239 > 0.08 con n=220 PNL=+53.77€
  - _Datos_: n=220 IC=+0.239 PNL=+53.77€
...
**〰️ H-CUSTOM-CROSS-WINDOW-SPREAD-NEG** — GBM 15min/60min: spread negativo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: n=197 IC=+0.048 PNL=+57.01€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=197 IC=+0.048 PNL=+57.01€
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=6364 IC=+0.089 PNL=+1657.05€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=6364 IC=+0.089 PNL=+1657.05€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=472 IC=+0.038 PNL=+33.93€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=472 IC=+0.038 PNL=+33.93€
...
**🟡 H-CUSTOM-WEEKLY-PCTDIST-BUYNO** — WEEKLY_PRICE BUY_NO con pct_dist alto — cuanto más lejos del rango, más seguro
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.412 > 0.1 con n=260 PNL=+168.61€
  - _Datos_: n=260 IC=+0.412 PNL=+168.61€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=1327 IC=+0.032 PNL=+127.66€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1327 IC=+0.032 PNL=+127.66€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.160 > 0.1 con n=1115 PNL=+371.05€
  - _Datos_: n=1115 IC=+0.160 PNL=+371.05€
...
**〰️ H-CUSTOM-BUYNO-LONGSHOT-15MIN** — BUY_NO longshot en 15min (py_mkt≥0.55) — comprar NO barato pierde
  ...
  - _Estado_: n=72 IC=-0.013 PNL=+15.52€ — sin señal clara aún (umbral IC: min=None max=-0.1)
  - _Datos_: n=72 IC=-0.013 PNL=+15.52€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=333 IC=+0.088 PNL=+68.95€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=333 IC=+0.088 PNL=+68.95€
...
**〰️ H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: n=244 IC=+0.057 PNL=+3.44€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=244 IC=+0.057 PNL=+3.44€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1101 IC=-0.068 PNL=+307.77€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1101 IC=-0.068 PNL=+307.77€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=1186 IC=+0.048 PNL=+111.85€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=1186 IC=+0.048 PNL=+111.85€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.117 > 0.08 con n=773 PNL=+204.46€
  - _Datos_: n=773 IC=+0.117 PNL=+204.46€
...
**🟡 H-CUSTOM-GBM-04H-ASIA** — UPDOWN_GBM 04h-05h UTC — media sesión asiática, ¿mejor franja nocturna?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.086 > 0.08 con n=242 PNL=+46.40€
  - _Datos_: n=242 IC=+0.086 PNL=+46.40€
...
**🟡 H-CUSTOM-LATE-04H-ASIA** — GBM_LATE_15M 04h-05h UTC — media sesión asiática (gemela de GBM-04H-ASIA)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.098 > 0.08 con n=746 PNL=+225.98€
  - _Datos_: n=746 IC=+0.098 PNL=+225.98€
...
**🟡 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.172 > 0.08 con n=306 PNL=+113.37€
  - _Datos_: n=306 IC=+0.172 PNL=+113.37€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.203 > 0.2 con n=898 PNL=+531.89€
  - _Datos_: n=898 IC=+0.203 PNL=+531.89€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.087 > 0.08 con n=610 PNL=+130.54€
  - _Datos_: n=610 IC=+0.087 PNL=+130.54€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.091 > 0.08 con n=663 PNL=+178.07€
  - _Datos_: n=663 IC=+0.091 PNL=+178.07€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.124 > 0.08 con n=831 PNL=+395.92€
  - _Datos_: n=831 IC=+0.124 PNL=+395.92€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.270 > 0.08 con n=515 PNL=-3.54€
  - _Datos_: n=515 IC=+0.270 PNL=-3.54€
...
**〰️ H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Estado_: n=134 IC=+0.051 PNL=+13.95€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=134 IC=+0.051 PNL=+13.95€
...
**🟡 H-CUSTOM-ETH15-BUYNO-TARDIO** — UPDOWN_GBM ETH#15min BUY_NO tardío (T_h<0.2) -- edge fuerte no capturado por el aprendizaje causal automático
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.305 > 0.08 con n=121 PNL=+64.12€
  - _Datos_: n=121 IC=+0.305 PNL=+64.12€
