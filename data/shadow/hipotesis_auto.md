# Hipótesis automáticas — 2026-09-24 18:17 UTC
_Generado por shadow_postmortem.py sobre 592664 resoluciones (PNL=+66520.07€)_
...
- **FILTRO** `py_entrada` > `0.495` → IC=-0.263 (n=112)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.121 (n=483)
...
- **PATRÓN** `py_entrada` < `0.495` → IC=+0.121 (n=483)

  - _Acción_: Kelly boost +0.60€ cuando `py_entrada` < 0.495 (IC base=+0.048)

- **PATRÓN** `ballena_activa_n` < `95.0` → IC=+0.130 (n=171)

  - _Acción_: Kelly boost +0.65€ cuando `ballena_activa_n` < 95.0 (IC base=+0.048)
...
- **PATRÓN** `ballena_activa_n` < `96.0` → IC=+0.142 (n=135)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `ballena_activa_n` < 96.0 (IC base=+0.049)
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.05` → IC=-0.233 (n=7041)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.05
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=21123)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `138.26` → IC=-0.249 (n=922)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 138.26
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=2767)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.206 (n=2114)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.205 (n=1516)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.375` → IC=+0.264 (n=1531)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.375 (IC base=+0.201)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.203 (n=2149)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.163 (n=1082)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.147 (n=307)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.203 (n=11430)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.168 (n=2772)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.182 (n=1938)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.251 (n=833)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.185 (n=2583)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.185 (n=2241)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.200 (n=2643)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.196 (n=1957)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.190 (n=6340)

  - _Acción_: Kelly boost +0.95€ cuando `py_entrada` > 0.71 (IC base=+0.175)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.224 (n=6097)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `2.83` → IC=+0.141 (n=4767)

  - _Acción_: Kelly boost +0.70€ cuando `lag_apertura_s` < 2.83 (IC base=+0.118)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.197 (n=2602)
...
- **PATRÓN** `restante_min` > `4.94` → IC=+0.140 (n=2566)
...
### GBM_LATE_15M
- **PATRÓN** `drift_60min` |x|≤ `0.4859` → IC=+0.122 (n=8021)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.61€ cuando `drift_60min` |x|≤ 0.4859 (IC base=+0.106)

- **PATRÓN** `ibs_20min` > `0.9829` → IC=+0.244 (n=2673)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9829 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` > `0.8443` → IC=+0.251 (n=476)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8443 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` < `0.6343` → IC=+0.249 (n=2261)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6343 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.352` → IC=+0.186 (n=2127)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 8.352 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` < `1.2119` → IC=+0.247 (n=2164)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2119 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` > `1.0481` → IC=+0.259 (n=982)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0481 (IC base=+0.106)

- **PATRÓN** `volumen_pendiente_norm` > `0.3047` → IC=+0.219 (n=803)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3047 (IC base=+0.106)

- **PATRÓN** `volumen_spike_ratio` > `1.4638` → IC=+0.202 (n=5487)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4638 (IC base=+0.106)

- **PATRÓN** `ibs_20min` < `0.57` → IC=+0.134 (n=9681)

  - _Acción_: Kelly boost +0.67€ cuando `ibs_20min` < 0.57 (IC base=+0.065)

- **PATRÓN** `dist_vwap_pct` > `0.6152` → IC=+0.195 (n=707)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.6152 (IC base=+0.065)

- **PATRÓN** `dist_vwap_pct` < `0.1534` → IC=+0.174 (n=3069)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.1534 (IC base=+0.065)

- **PATRÓN** `volumen_regimen` < `0.6983` → IC=+0.182 (n=1489)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.6983 (IC base=+0.065)

- **PATRÓN** `volumen_regimen` > `1.0548` → IC=+0.176 (n=1535)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3537` → IC=+0.165 (n=1814)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.3537 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.168 (n=871)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.444` → IC=+0.246 (n=1418)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.444 (IC base=+0.235)

- **PATRÓN** `volumen_pendiente_norm` < `0.0923` → IC=+0.231 (n=1175)
...
- **PATRÓN** `volumen_regimen` < `1.2554` → IC=+0.219 (n=1381)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7478` → IC=+0.217 (n=903)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0769` → IC=+0.162 (n=480)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6928` → IC=+0.172 (n=1439)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.6928 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` < `0.1332` → IC=+0.156 (n=1284)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.1332 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.236` → IC=+0.160 (n=230)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 11.236 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.261` → IC=+0.142 (n=1311)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` < 4.261 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `1.2046` → IC=+0.150 (n=1439)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2114` → IC=+0.189 (n=1775)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` < 0.2114 (IC base=+0.186)

- **PATRÓN** `volumen_pendiente_norm` > `0.3605` → IC=+0.199 (n=234)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3605 (IC base=+0.186)
...
- **PATRÓN** `drift_60min` |x|≤ `0.6013` → IC=+0.215 (n=1528)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.6013 (IC base=+0.212)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.250 (n=574)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.0636` → IC=+0.250 (n=674)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0636 (IC base=+0.212)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.654` → IC=+0.241 (n=523)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.654 (IC base=+0.212)

- **PATRÓN** `volumen_pendiente_norm` > `0.3561` → IC=+0.269 (n=219)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3561 (IC base=+0.212)

- **PATRÓN** `volumen_spike_ratio` < `1.7743` → IC=+0.204 (n=616)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7743 (IC base=+0.212)

- **PATRÓN** `volumen_spike_ratio` > `2.2107` → IC=+0.221 (n=932)
...
- **PATRÓN** `libro_liquidez` > `1887.358` → IC=+0.224 (n=693)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1887.358 (IC base=+0.212)

- **PATRÓN** `ballena_activa_n` < `24.0` → IC=+0.215 (n=905)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.153 (n=99)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.025 (n=2211)
...
- **PATRÓN** `dist_vwap_pct` > `0.1965` → IC=+0.183 (n=339)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.1965 (IC base=+0.018)

- **PATRÓN** `volumen_regimen` < `0.8497` → IC=+0.164 (n=555)
...
- **PATRÓN** `volumen_spike_ratio` > `1.518` → IC=+0.176 (n=695)
...
- **FILTRO** `ibs_20min` > `0.2558` → IC=-0.125 (n=2179)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2558
  - _Potencial_: sin este filtro IC_bueno=+0.126 (n=1074)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1566` → IC=+0.256 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1566 (IC base=-0.042)

- **PATRÓN** `volumen_spike_ratio` < `2.43` → IC=+0.269 (n=296)
...
- **FILTRO** `drift_60min` |x|> `0.6622` → IC=-0.185 (n=563)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=1692)

- **FILTRO** `ibs_20min` < `0.7126` → IC=-0.155 (n=1488)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7126
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=767)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.201 (n=413)

  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=1842)

- **FILTRO** `ibs_20min` > `0.77` → IC=-0.202 (n=827)

  - _Acción_: SKIP cuando `ibs_20min` > 0.77
  - _Potencial_: sin este filtro IC_bueno=+0.036 (n=2491)

- **PATRÓN** `dist_vwap_pct` > `0.7916` → IC=+0.324 (n=89)
...
- **PATRÓN** `volumen_regimen` < `0.9855` → IC=+0.291 (n=304)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9855 (IC base=-0.067)

- **PATRÓN** `volumen_regimen` > `0.6251` → IC=+0.304 (n=345)
...
- **PATRÓN** `volumen_spike_ratio` > `1.7999` → IC=+0.295 (n=218)
...
- **PATRÓN** `sigma_h` > `0.0098` → IC=+0.196 (n=3359)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0098 (IC base=+0.098)

- **PATRÓN** `ibs_20min` > `0.4754` → IC=+0.189 (n=8983)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.4754 (IC base=+0.098)

- **PATRÓN** `dist_vwap_pct` > `0.7694` → IC=+0.290 (n=1054)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7694 (IC base=+0.098)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.605` → IC=+0.156 (n=4740)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 3.605 (IC base=+0.098)

- **PATRÓN** `volumen_regimen` > `0.6895` → IC=+0.247 (n=3193)
...
- **FILTRO** `sigma_ewma_delta_pct` > `4.525` → IC=-0.158 (n=530)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.025 (n=1781)
...
- **PATRÓN** `ballena_activa_n` < `47.0` → IC=+0.174 (n=844)
...
- **PATRÓN** `ibs_20min` > `0.8587` → IC=+0.157 (n=672)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` > 0.8587 (IC base=+0.026)

- **PATRÓN** `dist_vwap_pct` > `0.3119` → IC=+0.178 (n=371)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.3119 (IC base=+0.026)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4239` → IC=+0.196 (n=301)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 1.4239 (IC base=+0.026)

- **PATRÓN** `ballena_activa_n` < `245.0` → IC=+0.191 (n=393)
...
- **PATRÓN** `volumen_regimen` > `1.0364` → IC=+0.227 (n=254)
...
- **PATRÓN** `libro_liquidez` > `1959.3148` → IC=+0.254 (n=530)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1959.3148 (IC base=+0.245)

- **PATRÓN** `sigma_h` > `0.0098` → IC=+0.315 (n=575)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0098 (IC base=+0.281)

- **PATRÓN** `drift_60min` |x|≤ `0.6083` → IC=+0.282 (n=1267)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.2222` → IC=+0.290 (n=1115)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2222 (IC base=+0.281)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.816` → IC=+0.296 (n=502)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5947` → IC=+0.284 (n=391)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5947 (IC base=+0.281)

- **PATRÓN** `volumen_spike_ratio` > `2.7408` → IC=+0.286 (n=531)
...
- **PATRÓN** `libro_liquidez` > `1875.8832` → IC=+0.299 (n=575)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1875.8832 (IC base=+0.281)

- **PATRÓN** `ballena_activa_n` < `29.0` → IC=+0.285 (n=758)
...
- **FILTRO** `ibs_20min` < `0.2855` → IC=-0.184 (n=491)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.077 (n=1474)

- **FILTRO** `ibs_20min` > `0.7745` → IC=-0.185 (n=594)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7745
  - _Potencial_: sin este filtro IC_bueno=+0.055 (n=1786)

- **PATRÓN** `ibs_20min` > `0.9101` → IC=+0.178 (n=492)

  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` > 0.9101 (IC base=+0.012)
...
- **FILTRO** `ibs_20min` > `0.6875` → IC=-0.231 (n=549)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.096 (n=1669)

- **FILTRO** `sigma_ewma_delta_pct` > `4.705` → IC=-0.180 (n=485)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.705
  - _Potencial_: sin este filtro IC_bueno=+0.069 (n=1733)
...
- **PATRÓN** `ibs_20min` > `0.9118` → IC=+0.344 (n=871)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9118 (IC base=+0.277)

- **PATRÓN** `dist_vwap_pct` > `0.2079` → IC=+0.316 (n=775)
...
- **PATRÓN** `volumen_regimen` > `0.8619` → IC=+0.302 (n=870)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1554` → IC=+0.292 (n=561)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1554 (IC base=+0.277)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.283 (n=1397)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.277)

- **PATRÓN** `libro_liquidez` > `2630.254` → IC=+0.291 (n=870)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2630.254 (IC base=+0.277)

- **PATRÓN** `sigma_h` > `0.0152` → IC=+0.297 (n=940)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0152 (IC base=+0.271)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.279 (n=708)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3945` → IC=+0.304 (n=1411)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3945 (IC base=+0.271)

- **PATRÓN** `dist_vwap_pct` > `0.3041` → IC=+0.280 (n=539)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3041 (IC base=+0.271)

- **PATRÓN** `dist_vwap_pct` < `0.9772` → IC=+0.272 (n=1591)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.9772 (IC base=+0.271)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8208` → IC=+0.269 (n=829)
...
- **PATRÓN** `libro_liquidez` > `2622.7148` → IC=+0.275 (n=940)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2622.7148 (IC base=+0.271)
...
- **PATRÓN** `ibs_20min` < `0.4783` → IC=+0.227 (n=7491)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4783 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.9207` → IC=+0.145 (n=716)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.9207 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` < `0.2332` → IC=+0.158 (n=5453)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.2332 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.347` → IC=+0.195 (n=1272)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 10.347 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.1753` → IC=+0.152 (n=5429)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1753 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.2915` → IC=+0.220 (n=1074)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2915 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.5615` → IC=+0.167 (n=2998)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.5615 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.2537` → IC=+0.170 (n=3087)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.2537 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `116.0` → IC=+0.172 (n=6455)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.250 (n=869)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.244 (n=877)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3443` → IC=+0.264 (n=973)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3443 (IC base=+0.238)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.286` → IC=+0.251 (n=1049)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.286 (IC base=+0.238)

- **PATRÓN** `volumen_pendiente_norm` < `0.1599` → IC=+0.234 (n=918)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4233` → IC=+0.264 (n=299)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4233 (IC base=+0.238)

- **PATRÓN** `libro_liquidez` > `1806.4548` → IC=+0.245 (n=648)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.157 (n=1276)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0057 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.2932` → IC=+0.158 (n=1273)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.2932 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.183 (n=494)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 17.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` < `0.5649` → IC=+0.186 (n=1273)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` < 0.5649 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `0.1339` → IC=+0.160 (n=1263)
...
- **PATRÓN** `volumen_regimen` < `1.2129` → IC=+0.156 (n=1273)
...
- **PATRÓN** `sigma_h` < `0.0114` → IC=+0.229 (n=1090)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0114 (IC base=+0.217)

- **PATRÓN** `drift_60min` |x|≤ `0.0968` → IC=+0.247 (n=365)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.2417` → IC=+0.256 (n=959)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2417 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.748` → IC=+0.265 (n=465)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7801` → IC=+0.211 (n=445)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7801 (IC base=+0.217)

- **PATRÓN** `volumen_spike_ratio` > `2.221` → IC=+0.224 (n=673)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.221 (IC base=+0.217)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4237` → IC=+0.150 (n=409)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.4237 (IC base=+0.143)
...
- **PATRÓN** `volumen_regimen` < `0.8476` → IC=+0.147 (n=883)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.8476 (IC base=+0.123)

- **PATRÓN** `volumen_pendiente_norm` > `0.2919` → IC=+0.196 (n=192)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.2919 (IC base=+0.123)

- **PATRÓN** `volumen_spike_ratio` < `1.7891` → IC=+0.137 (n=802)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 1.7891 (IC base=+0.123)

- **PATRÓN** `libro_liquidez` > `10007.3401` → IC=+0.160 (n=600)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.139 (n=1447)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 5.0 (IC base=+0.119)

- **PATRÓN** `ibs_20min` > `0.5172` → IC=+0.204 (n=1414)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5172 (IC base=+0.119)

- **PATRÓN** `dist_vwap_pct` > `1.0956` → IC=+0.227 (n=324)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0956 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.731` → IC=+0.252 (n=320)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.731 (IC base=+0.119)

- **PATRÓN** `volumen_regimen` < `1.2208` → IC=+0.131 (n=1414)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 1.2208 (IC base=+0.119)

- **PATRÓN** `volumen_regimen` > `0.6454` → IC=+0.124 (n=1414)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_regimen` > 0.6454 (IC base=+0.119)

- **PATRÓN** `volumen_pendiente_norm` < `0.1637` → IC=+0.129 (n=1420)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5464` → IC=+0.140 (n=601)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 1.5464 (IC base=+0.119)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.128 (n=1480)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1034` → IC=+0.157 (n=479)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.1034 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.131 (n=1455)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 5.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` < `0.5652` → IC=+0.210 (n=1437)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5652 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `1.0043` → IC=+0.142 (n=188)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 1.0043 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.2002` → IC=+0.138 (n=1328)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.2002 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.978` → IC=+0.141 (n=232)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 8.978 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` < `0.6359` → IC=+0.136 (n=479)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 0.6359 (IC base=+0.113)

- **PATRÓN** `volumen_pendiente_norm` > `0.2757` → IC=+0.167 (n=175)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.2757 (IC base=+0.113)

- **PATRÓN** `volumen_spike_ratio` < `1.4538` → IC=+0.131 (n=429)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` < 1.4538 (IC base=+0.113)

- **PATRÓN** `volumen_spike_ratio` > `2.4249` → IC=+0.133 (n=429)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 2.4249 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `1463.3817` → IC=+0.136 (n=1283)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 1463.3817 (IC base=+0.113)

### GBM_LATE_15M_MULTIHORIZONTE#XRP#15min
- **PATRÓN** `sigma_h` > `0.0187` → IC=+0.213 (n=902)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0187 (IC base=+0.202)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.204 (n=1406)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.74` → IC=+0.260 (n=1209)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.74 (IC base=+0.202)

- **PATRÓN** `dist_vwap_pct` > `0.5094` → IC=+0.223 (n=644)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5094 (IC base=+0.202)
...
- **PATRÓN** `sigma_h` > `0.0172` → IC=+0.206 (n=932)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.221 (n=683)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.4375` → IC=+0.246 (n=1398)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4375 (IC base=+0.203)

- **PATRÓN** `dist_vwap_pct` > `1.2478` → IC=+0.225 (n=165)
...
- **PATRÓN** `volumen_regimen` > `0.6302` → IC=+0.215 (n=1398)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.187 (n=936)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 15.0 (IC base=+0.146)

- **PATRÓN** `ibs_20min` > `0.388` → IC=+0.180 (n=1869)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` > 0.388 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `0.8726` → IC=+0.187 (n=292)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.8726 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.713` → IC=+0.174 (n=854)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 3.713 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` < `0.8749` → IC=+0.161 (n=1096)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.8749 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` > `1.2097` → IC=+0.154 (n=548)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.2097 (IC base=+0.146)

- **PATRÓN** `volumen_pendiente_norm` > `0.1644` → IC=+0.174 (n=520)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.1644 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` < `1.4411` → IC=+0.163 (n=601)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.4411 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` > `1.824` → IC=+0.152 (n=1201)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.824 (IC base=+0.146)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.150 (n=2123)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.02 (IC base=+0.146)

- **PATRÓN** `libro_liquidez` > `12439.6953` → IC=+0.159 (n=623)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 12439.6953 (IC base=+0.146)

- **PATRÓN** `ballena_activa_n` < `162.0` → IC=+0.166 (n=1651)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 162.0 (IC base=+0.146)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.137 (n=1310)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0057 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.124 (n=1981)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 5.0 (IC base=+0.110)

- **PATRÓN** `ibs_20min` < `0.6579` → IC=+0.138 (n=1958)

  - _Acción_: Kelly boost +0.69€ cuando `ibs_20min` < 0.6579 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.2135` → IC=+0.122 (n=1742)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.2135 (IC base=+0.110)

- **PATRÓN** `volumen_regimen` < `0.6986` → IC=+0.122 (n=776)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` < 0.6986 (IC base=+0.110)

- **PATRÓN** `volumen_spike_ratio` < `2.2245` → IC=+0.125 (n=1658)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_spike_ratio` < 2.2245 (IC base=+0.110)

- **PATRÓN** `libro_liquidez` > `2737.8875` → IC=+0.122 (n=1749)

  - _Acción_: Kelly boost +0.61€ cuando `libro_liquidez` > 2737.8875 (IC base=+0.110)

- **PATRÓN** `ballena_activa_n` < `20.0` → IC=+0.123 (n=606)

  - _Acción_: Kelly boost +0.62€ cuando `ballena_activa_n` < 20.0 (IC base=+0.110)

### GBM_LATE_15M_PYCONFIRMADO#BTC#15min
- **PATRÓN** `drift_60min` |x|≤ `0.3485` → IC=+0.122 (n=471)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.61€ cuando `drift_60min` |x|≤ 0.3485 (IC base=+0.104)

- **PATRÓN** `hora_utc` > `9.0` → IC=+0.150 (n=421)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 9.0 (IC base=+0.104)

- **PATRÓN** `ibs_20min` > `0.2513` → IC=+0.143 (n=471)

  - _Acción_: Kelly boost +0.71€ cuando `ibs_20min` > 0.2513 (IC base=+0.104)

- **PATRÓN** `dist_vwap_pct` > `0.3086` → IC=+0.161 (n=163)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.3086 (IC base=+0.104)

- **PATRÓN** `volumen_regimen` < `0.6136` → IC=+0.148 (n=157)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.6136 (IC base=+0.104)

- **PATRÓN** `libro_liquidez` > `15008.644` → IC=+0.142 (n=314)

  - _Acción_: Kelly boost +0.71€ cuando `libro_liquidez` > 15008.644 (IC base=+0.104)

- **PATRÓN** `ballena_activa_n` < `147.0` → IC=+0.142 (n=149)

  - _Acción_: Kelly boost +0.71€ cuando `ballena_activa_n` < 147.0 (IC base=+0.104)

- **PATRÓN** `sigma_h` < `0.0027` → IC=+0.212 (n=210)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0027 (IC base=+0.137)

- **PATRÓN** `drift_60min` |x|≤ `0.3402` → IC=+0.155 (n=625)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.3402 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.150 (n=563)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.4881` → IC=+0.152 (n=719)
...
- **PATRÓN** `volumen_regimen` < `1.2142` → IC=+0.141 (n=625)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0967` → IC=+0.209 (n=194)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0967 (IC base=+0.190)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.206 (n=606)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.190)

- **PATRÓN** `ibs_20min` > `0.9719` → IC=+0.270 (n=194)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9719 (IC base=+0.190)
...
- **PATRÓN** `volumen_regimen` < `0.836` → IC=+0.197 (n=387)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` < 0.836 (IC base=+0.190)
...
- **PATRÓN** `volumen_spike_ratio` < `1.3991` → IC=+0.225 (n=191)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.199 (n=645)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.120 (n=480)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.60€ cuando `sigma_h` < 0.0061 (IC base=+0.093)

- **PATRÓN** `ibs_20min` < `0.0816` → IC=+0.163 (n=182)

  - _Acción_: Kelly boost +0.82€ cuando `ibs_20min` < 0.0816 (IC base=+0.093)

- **PATRÓN** `volumen_regimen` < `0.6847` → IC=+0.124 (n=240)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_regimen` < 0.6847 (IC base=+0.093)

- **PATRÓN** `volumen_pendiente_norm` > `0.1667` → IC=+0.128 (n=127)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_pendiente_norm` > 0.1667 (IC base=+0.093)

- **PATRÓN** `libro_liquidez` > `3825.7125` → IC=+0.122 (n=488)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5317` → IC=+0.136 (n=416)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.5317 (IC base=+0.131)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.168 (n=389)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 8.0 (IC base=+0.131)

- **PATRÓN** `ibs_20min` > `0.7308` → IC=+0.203 (n=372)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7308 (IC base=+0.131)

- **PATRÓN** `dist_vwap_pct` > `0.8958` → IC=+0.226 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8958 (IC base=+0.131)
...
- **PATRÓN** `volumen_regimen` < `1.0697` → IC=+0.149 (n=366)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.0697 (IC base=+0.131)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.200 (n=812)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.539` → IC=+0.345 (n=385)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.251` → IC=+0.154 (n=1443)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 4.251 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` < `0.63` → IC=+0.180 (n=535)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1158` → IC=+0.163 (n=1363)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.1158 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` > `1.4033` → IC=+0.154 (n=1549)
...
- **PATRÓN** `ibs_20min` < `0.653` → IC=+0.196 (n=1388)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` < 0.653 (IC base=+0.152)

- **PATRÓN** `dist_vwap_pct` < `0.1347` → IC=+0.166 (n=1253)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.274` → IC=+0.152 (n=1258)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` < 4.274 (IC base=+0.152)

- **PATRÓN** `volumen_regimen` < `1.1917` → IC=+0.163 (n=1388)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4003` → IC=+0.161 (n=1290)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.4003 (IC base=+0.152)

- **PATRÓN** `volumen_spike_ratio` > `2.0873` → IC=+0.164 (n=585)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 2.0873 (IC base=+0.152)

- **PATRÓN** `ballena_activa_n` < `421.0` → IC=+0.159 (n=1050)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.226 (n=1698)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.6697` → IC=+0.257 (n=1448)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6697 (IC base=+0.218)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.81` → IC=+0.295 (n=476)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.81 (IC base=+0.218)

- **PATRÓN** `volumen_pendiente_norm` < `0.2118` → IC=+0.222 (n=1602)
...
- **PATRÓN** `sigma_h` < `0.0116` → IC=+0.238 (n=1513)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0116 (IC base=+0.232)

- **PATRÓN** `drift_60min` |x|≤ `0.4535` → IC=+0.234 (n=1331)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4535 (IC base=+0.232)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.263 (n=568)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.232)
...
- **PATRÓN** `ibs_20min` < `0.0138` → IC=+0.311 (n=505)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0138 (IC base=+0.232)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.767` → IC=+0.278 (n=561)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7564` → IC=+0.231 (n=612)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7564 (IC base=+0.232)

- **PATRÓN** `volumen_spike_ratio` > `2.1919` → IC=+0.238 (n=925)
...
- **PATRÓN** `libro_liquidez` > `1884.62` → IC=+0.244 (n=686)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1884.62 (IC base=+0.232)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.230 (n=1313)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4388` → IC=+0.146 (n=1706)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.4388 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.150 (n=1783)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_regimen` < `0.8764` → IC=+0.155 (n=1137)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.8764 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.2375` → IC=+0.214 (n=316)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2375 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` < `1.5223` → IC=+0.150 (n=726)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.5223 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.767` → IC=+0.150 (n=1100)
...
- **PATRÓN** `ballena_activa_n` < `79.0` → IC=+0.148 (n=529)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `ballena_activa_n` < 79.0 (IC base=+0.136)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.164 (n=1227)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0066 (IC base=+0.139)

- **PATRÓN** `drift_60min` |x|≤ `0.4484` → IC=+0.154 (n=1393)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.4484 (IC base=+0.139)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.176 (n=519)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.7021` → IC=+0.185 (n=1393)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` < 0.7021 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` < `0.6016` → IC=+0.145 (n=1546)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.6016 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.137` → IC=+0.183 (n=206)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 11.137 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` < `0.8628` → IC=+0.148 (n=929)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.8628 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` > `1.1921` → IC=+0.142 (n=465)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 1.1921 (IC base=+0.139)

- **PATRÓN** `volumen_pendiente_norm` > `0.2894` → IC=+0.256 (n=166)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2894 (IC base=+0.139)

- **PATRÓN** `volumen_spike_ratio` > `1.4421` → IC=+0.152 (n=1321)
...
- **PATRÓN** `ballena_activa_n` < `179.0` → IC=+0.147 (n=1323)

  - _Acción_: Kelly boost +0.73€ cuando `ballena_activa_n` < 179.0 (IC base=+0.139)

### GBM_LATE_15M_TARDIO#SOL#15min
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.139 (n=1130)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0081 (IC base=+0.112)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.131 (n=1742)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.112)

- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.190 (n=1697)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` > 0.4706 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `1.081` → IC=+0.205 (n=337)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.081 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.478` → IC=+0.234 (n=636)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.478 (IC base=+0.112)

- **PATRÓN** `volumen_regimen` < `0.8932` → IC=+0.134 (n=1130)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 0.8932 (IC base=+0.112)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.126 (n=1703)
...
- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.131 (n=1305)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1326` → IC=+0.160 (n=551)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.1326 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.124 (n=1709)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 5.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` < `0.6364` → IC=+0.204 (n=1654)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.486` → IC=+0.124 (n=1598)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 3.486 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` < `0.7169` → IC=+0.153 (n=727)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.7169 (IC base=+0.113)

- **PATRÓN** `volumen_pendiente_norm` > `0.2229` → IC=+0.167 (n=256)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.2229 (IC base=+0.113)

- **PATRÓN** `volumen_spike_ratio` < `1.4442` → IC=+0.143 (n=497)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 1.4442 (IC base=+0.113)

- **PATRÓN** `volumen_spike_ratio` > `2.5014` → IC=+0.127 (n=497)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 2.5014 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `2848.7898` → IC=+0.169 (n=551)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 2848.7898 (IC base=+0.113)

- **PATRÓN** `ballena_activa_n` < `51.0` → IC=+0.120 (n=1285)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.215 (n=1766)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.2024` → IC=+0.234 (n=1000)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2024 (IC base=+0.208)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.298` → IC=+0.273 (n=302)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.298 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` < `1.0709` → IC=+0.211 (n=1491)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.0709 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` > `0.6395` → IC=+0.215 (n=1694)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6395 (IC base=+0.208)

- **PATRÓN** `volumen_pendiente_norm` > `0.2351` → IC=+0.249 (n=297)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2351 (IC base=+0.208)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.218 (n=1783)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.208)

- **PATRÓN** `libro_liquidez` > `2626.3484` → IC=+0.220 (n=1129)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.208 (n=1277)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.200)

- **PATRÓN** `ibs_20min` < `0.5185` → IC=+0.255 (n=1812)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5185 (IC base=+0.200)

- **PATRÓN** `dist_vwap_pct` > `1.2595` → IC=+0.201 (n=292)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2595 (IC base=+0.200)

- **PATRÓN** `dist_vwap_pct` < `0.9244` → IC=+0.203 (n=2020)
...
- **PATRÓN** `volumen_regimen` > `1.2365` → IC=+0.233 (n=604)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2014` → IC=+0.196 (n=1433)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.2014 (IC base=+0.200)

- **PATRÓN** `volumen_spike_ratio` > `1.4315` → IC=+0.196 (n=1628)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.139 (n=3036)

- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.158 (n=2541)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0092 (IC base=+0.151)

- **PATRÓN** `sigma_h` > `0.0056` → IC=+0.152 (n=2583)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0056 (IC base=+0.151)

- **PATRÓN** `drift_60min` |x|≤ `0.5232` → IC=+0.160 (n=2887)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.5232 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.163 (n=1139)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9422` → IC=+0.212 (n=962)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9422 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` > `0.1866` → IC=+0.159 (n=1055)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.1866 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` < `0.4904` → IC=+0.145 (n=1769)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.4904 (IC base=+0.151)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.187` → IC=+0.176 (n=474)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 10.187 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` > `0.8982` → IC=+0.161 (n=1250)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.8982 (IC base=+0.151)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8854` → IC=+0.163 (n=1901)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.8854 (IC base=+0.151)

- **PATRÓN** `libro_liquidez` > `8275.452` → IC=+0.154 (n=1309)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 8275.452 (IC base=+0.151)

- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.196 (n=764)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0038 (IC base=+0.137)

- **PATRÓN** `drift_60min` |x|≤ `0.4933` → IC=+0.156 (n=2290)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.4933 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.170 (n=828)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.0837` → IC=+0.173 (n=763)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.0837 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.6896` → IC=+0.151 (n=433)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.6896 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.4305` → IC=+0.131 (n=2270)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.4305 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.274` → IC=+0.148 (n=2281)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` < 6.274 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `0.9009` → IC=+0.156 (n=1458)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.9009 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` < `0.0969` → IC=+0.141 (n=2072)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_pendiente_norm` < 0.0969 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` > `0.2204` → IC=+0.143 (n=488)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_pendiente_norm` > 0.2204 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` < `1.425` → IC=+0.150 (n=755)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.425 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `1.8123` → IC=+0.141 (n=1507)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 1.8123 (IC base=+0.137)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.139 (n=3036)

  - _Acción_: Kelly boost +0.70€ cuando `libro_spread` < 0.01 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `7196.849` → IC=+0.151 (n=2045)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 7196.849 (IC base=+0.137)

### GBM_LATE_5M#BTC#5min
- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.174 (n=339)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0056 (IC base=+0.158)

- **PATRÓN** `sigma_h` > `0.0034` → IC=+0.168 (n=344)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` > 0.0034 (IC base=+0.158)

- **PATRÓN** `drift_60min` |x|≤ `0.0902` → IC=+0.195 (n=129)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.97€ cuando `drift_60min` |x|≤ 0.0902 (IC base=+0.158)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.167 (n=388)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 5.0 (IC base=+0.158)

- **PATRÓN** `ibs_20min` < `0.5463` → IC=+0.187 (n=257)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` < 0.5463 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` > `0.212` → IC=+0.161 (n=184)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.212 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` < `0.3825` → IC=+0.161 (n=373)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.3825 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.819` → IC=+0.161 (n=60)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 5.819 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.415` → IC=+0.166 (n=408)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` < 2.415 (IC base=+0.158)

- **PATRÓN** `volumen_regimen` < `1.2549` → IC=+0.164 (n=385)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.2549 (IC base=+0.158)

- **PATRÓN** `volumen_regimen` > `0.8476` → IC=+0.190 (n=256)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_regimen` > 0.8476 (IC base=+0.158)

- **PATRÓN** `volumen_pendiente_norm` > `0.3075` → IC=+0.295 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3075 (IC base=+0.158)

- **PATRÓN** `volumen_spike_ratio` < `1.4485` → IC=+0.195 (n=129)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 1.4485 (IC base=+0.158)

- **PATRÓN** `volumen_spike_ratio` > `2.6734` → IC=+0.208 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6734 (IC base=+0.158)

- **PATRÓN** `libro_liquidez` > `12505.1299` → IC=+0.199 (n=344)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12505.1299 (IC base=+0.158)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.179 (n=359)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.6078` → IC=+0.144 (n=425)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `9.107` → IC=+0.153 (n=1025)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 9.107 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `0.8795` → IC=+0.186 (n=626)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` < 0.8795 (IC base=+0.138)

- **PATRÓN** `volumen_pendiente_norm` > `0.0691` → IC=+0.162 (n=439)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8194` → IC=+0.148 (n=623)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.8194 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `11402.2097` → IC=+0.152 (n=937)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 11402.2097 (IC base=+0.138)

- **PATRÓN** `ballena_activa_n` < `706.0` → IC=+0.144 (n=892)

  - _Acción_: Kelly boost +0.72€ cuando `ballena_activa_n` < 706.0 (IC base=+0.138)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.155 (n=803)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0073 (IC base=+0.153)

- **PATRÓN** `sigma_h` > `0.0044` → IC=+0.163 (n=912)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.0044 (IC base=+0.153)

- **PATRÓN** `drift_60min` |x|≤ `0.3886` → IC=+0.156 (n=804)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.3886 (IC base=+0.153)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.163 (n=636)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5623` → IC=+0.158 (n=609)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` < 0.5623 (IC base=+0.153)

- **PATRÓN** `ibs_20min` > `0.7889` → IC=+0.166 (n=414)

  - _Acción_: Kelly boost +0.83€ cuando `ibs_20min` > 0.7889 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` > `0.9629` → IC=+0.159 (n=203)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.9629 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` < `0.4255` → IC=+0.164 (n=856)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.4255 (IC base=+0.153)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.676` → IC=+0.162 (n=913)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` < 6.676 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` < `1.0856` → IC=+0.156 (n=803)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.0856 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` > `0.7179` → IC=+0.157 (n=815)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 0.7179 (IC base=+0.153)

- **PATRÓN** `volumen_pendiente_norm` < `0.1099` → IC=+0.155 (n=842)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_pendiente_norm` < 0.1099 (IC base=+0.153)

- **PATRÓN** `volumen_pendiente_norm` > `0.0788` → IC=+0.162 (n=394)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` > 0.0788 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` < `1.4409` → IC=+0.167 (n=298)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.4409 (IC base=+0.153)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.159 (n=902)
...
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.194 (n=403)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0041 (IC base=+0.096)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.129 (n=850)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1543` → IC=+0.160 (n=445)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.1543 (IC base=+0.096)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.442` → IC=+0.208 (n=193)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0892` → IC=+0.136 (n=624)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 2.0892 (IC base=+0.096)
...
- **PATRÓN** `hora_utc` > `13.0` → IC=+0.130 (n=206)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 13.0 (IC base=+0.074)

- **PATRÓN** `ibs_20min` > `0.6719` → IC=+0.183 (n=235)

  - _Acción_: Kelly boost +0.92€ cuando `ibs_20min` > 0.6719 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` > `0.2066` → IC=+0.153 (n=148)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.2066 (IC base=+0.074)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.497` → IC=+0.170 (n=104)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 5.497 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` > `1.0643` → IC=+0.178 (n=88)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` > 1.0643 (IC base=+0.074)

- **PATRÓN** `volumen_pendiente_norm` > `0.2448` → IC=+0.173 (n=50)
...
- **PATRÓN** `sigma_h` > `0.0058` → IC=+0.155 (n=117)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` > 0.0058 (IC base=+0.093)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.139 (n=120)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 15.0 (IC base=+0.093)

- **PATRÓN** `ibs_20min` > `0.6466` → IC=+0.163 (n=253)

  - _Acción_: Kelly boost +0.81€ cuando `ibs_20min` > 0.6466 (IC base=+0.093)

- **PATRÓN** `dist_vwap_pct` > `0.4967` → IC=+0.189 (n=59)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.4967 (IC base=+0.093)

- **PATRÓN** `volumen_pendiente_norm` < `0.1654` → IC=+0.124 (n=147)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_pendiente_norm` < 0.1654 (IC base=+0.093)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=1766)
...
- **FILTRO** `py_entrada` < `0.42` → IC=-0.175 (n=250)
...
- **FILTRO** `restante_min` < `3.45` → IC=-0.144 (n=133)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=405)

- **FILTRO** `hora_utc` < `12.0` → IC=-0.130 (n=252)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=286)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.133 (n=145)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.072 (n=393)

- **PATRÓN** `py_entrada` < `0.47` → IC=+0.186 (n=138)

  - _Acción_: Kelly boost +0.93€ cuando `py_entrada` < 0.47 (IC base=+0.017)
...
- **FILTRO** `lag_apertura_s` > `76.24` → IC=-0.321 (n=26)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=7502)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.166 (n=3505)

  - _Acción_: SKIP cuando `py_entrada` < 0.475
  - _Potencial_: sin este filtro IC_bueno=+0.058 (n=10581)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.167 (n=3633)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.035 (n=10943)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.46` → IC=-0.209 (n=600)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.189 (n=613)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=1875)
...
- **FILTRO** `py_entrada` < `0.485` → IC=-0.172 (n=599)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.085 (n=1832)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.175 (n=636)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=1974)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=2938)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.129 (n=9940)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=22304)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.273 (n=7882)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=24362)

- **FILTRO** `ibs_7min` < `0.2857` → IC=-0.235 (n=8056)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2857
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=24188)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.157 (n=10957)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=21287)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.229 (n=9929)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=30545)

- **FILTRO** `ibs_7min` > `0.2924` → IC=-0.178 (n=10117)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2924
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=30357)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.135 (n=1628)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3708)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.308 (n=1274)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=4062)

- **FILTRO** `ibs_7min` < `0.7113` → IC=-0.249 (n=1760)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7113
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=3576)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.186 (n=1226)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=4110)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.261 (n=1714)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=5237)

- **FILTRO** `drift_7min_pct` |x|> `0.1123` → IC=-0.125 (n=2363)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1123
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=4588)

- **FILTRO** `ibs_7min` > `0.7889` → IC=-0.206 (n=1737)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7889
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=5214)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=1299)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.087 (n=4266)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.252 (n=1347)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=4218)

- **FILTRO** `ibs_7min` < `0.7473` → IC=-0.194 (n=1391)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7473
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=4174)

- **FILTRO** `ballena_activa_n` > `161.0` → IC=-0.175 (n=1391)

  - _Acción_: SKIP cuando `ballena_activa_n` > 161.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=4174)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.253 (n=1411)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=4249)

- **FILTRO** `ibs_7min` > `0.2599` → IC=-0.176 (n=1414)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2599
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=4246)

- **FILTRO** `ballena_activa_n` > `154.0` → IC=-0.182 (n=1411)

  - _Acción_: SKIP cuando `ballena_activa_n` > 154.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=4249)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.165 (n=1240)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=3825)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.306 (n=1243)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=3822)

- **FILTRO** `ibs_7min` < `0.1935` → IC=-0.259 (n=1261)

  - _Acción_: SKIP cuando `ibs_7min` < 0.1935
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=3804)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.219 (n=1174)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3891)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.237 (n=1698)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=5738)

- **FILTRO** `ibs_7min` > `0.7456` → IC=-0.174 (n=1857)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7456
  - _Potencial_: sin este filtro IC_bueno=+0.005 (n=5579)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=1705)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=3611)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.232 (n=1572)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=3744)

- **FILTRO** `ibs_7min` < `0.7409` → IC=-0.181 (n=1329)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7409
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=3987)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.170 (n=1309)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=4007)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.265 (n=1204)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=4233)

- **FILTRO** `ibs_7min` > `0.2745` → IC=-0.176 (n=1359)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2745
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4078)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.183 (n=1346)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=4091)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.257 (n=1366)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=4255)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.238 (n=1401)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=4220)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.178 (n=1835)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.023 (n=5862)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.254 (n=1723)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3618)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.228 (n=1323)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=4018)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.214 (n=1301)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=4040)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.205 (n=1742)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=5551)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=2905)
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 1322 | +0.098 | +194.40€ | 1 | 9 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 1322 | +0.098 | +194.40€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 26 | +0.036 | -1.50€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 26 | +0.036 | -1.50€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 985 | +0.108 | +168.34€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 985 | +0.108 | +168.34€ | 2 | 8 |
...
| ✅ BALLENAS_TARDIAS | 28164 | -0.091 | -3801.49€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1451 | -0.049 | -220.26€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 26713 | -0.094 | -3581.23€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3689 | -0.092 | -587.63€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3689 | -0.092 | -587.63€ | 1 | 0 |
...
| ✅ BALLENAS_TARDIAS#ETH | 7243 | -0.024 | -679.40€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 7243 | -0.024 | -679.40€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 6769 | -0.097 | -453.41€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 6769 | -0.097 | -453.41€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 5546 | -0.180 | -1079.65€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 5546 | -0.180 | -1079.65€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 18074 | -0.028 | +4042.55€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 4722 | +0.000 | +1823.45€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 13352 | -0.038 | +2219.10€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 18074 | -0.028 | +4042.55€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 4722 | +0.000 | +1823.45€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 13352 | -0.038 | +2219.10€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 92532 | +0.112 | -4734.69€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 13979 | +0.185 | -438.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 371 | -0.085 | -51.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 72157 | +0.099 | -4027.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 6025 | +0.107 | -217.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 12000 | +0.098 | -1023.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 45 | -0.160 | -0.32€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 11940 | +0.099 | -1011.37€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 18716 | +0.131 | -376.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 4399 | +0.203 | -135.96€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 11965 | +0.111 | -196.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 