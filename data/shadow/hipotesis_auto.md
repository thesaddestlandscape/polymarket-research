# Hipótesis automáticas — 2026-07-16 08:08 UTC
_Generado por shadow_postmortem.py sobre 16683 resoluciones (PNL=+3419.67€)_
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.196 (n=436)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` < 7.0 (IC base=+0.166)

- **PATRÓN** `py_entrada` < `0.265` → IC=+0.331 (n=336)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.265 (IC base=+0.166)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.176 (n=1107)

  - _Acción_: Kelly boost +0.88€ cuando `libro_spread` < 0.01 (IC base=+0.166)

- **PATRÓN** `libro_liquidez` > `7360.9474` → IC=+0.186 (n=323)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 7360.9474 (IC base=+0.166)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.229 (n=149)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `8047.2094` → IC=+0.211 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8047.2094 (IC base=+0.208)
...
### GBM_LATE_15M
- **PATRÓN** `dist_vwap_pct` > `0.5857` → IC=+0.129 (n=130)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` > 0.5857 (IC base=+0.108)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.066` → IC=+0.227 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.066 (IC base=+0.108)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.134 (n=1231)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` > 0.0091 (IC base=+0.107)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.127 (n=1235)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 11.0 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.188` → IC=+0.129 (n=138)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 6.188 (IC base=+0.107)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `dist_vwap_pct` > `0.2778` → IC=+0.121 (n=27)

  - _Acción_: Kelly boost +0.60€ cuando `dist_vwap_pct` > 0.2778 (IC base=+0.073)

- **PATRÓN** `sigma_ewma_delta_pct` > `15.305` → IC=+0.237 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 15.305 (IC base=+0.073)
...
- **PATRÓN** `sigma_h` < `0.0115` → IC=+0.146 (n=312)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` < 0.0115 (IC base=+0.130)

- **PATRÓN** `sigma_h` > `0.0092` → IC=+0.133 (n=466)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0092 (IC base=+0.130)
...
- **PATRÓN** `sigma_h` > `0.0275` → IC=+0.211 (n=157)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.5138` → IC=+0.141 (n=182)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.5138 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.696` → IC=+0.139 (n=164)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` < 7.696 (IC base=+0.164)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.223 (n=330)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.167 (n=755)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 18.0 (IC base=+0.160)

- **PATRÓN** `dist_vwap_pct` > `0.1473` → IC=+0.197 (n=288)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1473 (IC base=+0.160)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.007` → IC=+0.313 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.007 (IC base=+0.160)

- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.180 (n=748)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.4117` → IC=+0.200 (n=525)
...
- **PATRÓN** `sigma_h` > `0.0028` → IC=+0.140 (n=176)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` > 0.0028 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.161 (n=125)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 12.0 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` > `0.5737` → IC=+0.206 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5737 (IC base=+0.114)
...
- **PATRÓN** `sigma_h` < `0.0165` → IC=+0.292 (n=166)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0165 (IC base=+0.289)

- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.321 (n=166)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.298 (n=166)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.289)

- **PATRÓN** `dist_vwap_pct` > `0.1789` → IC=+0.300 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1789 (IC base=+0.289)

- **PATRÓN** `dist_vwap_pct` < `0.5244` → IC=+0.308 (n=139)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5244 (IC base=+0.289)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.062` → IC=+0.309 (n=113)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.062 (IC base=+0.289)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0151` → IC=+0.196 (n=225)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0151 (IC base=+0.113)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.122 (n=707)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` < 18.0 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `0.3409` → IC=+0.165 (n=177)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.3409 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.1365` → IC=+0.143 (n=242)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.1365 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.209` → IC=+0.319 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.209 (IC base=+0.113)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.126 (n=303)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` < 6.0 (IC base=+0.089)

- **PATRÓN** `dist_vwap_pct` < `0.2` → IC=+0.120 (n=438)

  - _Acción_: Kelly boost +0.60€ cuando `dist_vwap_pct` < 0.2 (IC base=+0.089)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `dist_vwap_pct` > `0.4395` → IC=+0.167 (n=19)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.4395 (IC base=+0.084)

- **PATRÓN** `dist_vwap_pct` < `0.1111` → IC=+0.123 (n=59)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.1111 (IC base=+0.084)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.969` → IC=+0.179 (n=26)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 10.969 (IC base=+0.084)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.156 (n=59)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0039 (IC base=+0.060)

- **PATRÓN** `dist_vwap_pct` < `0.2519` → IC=+0.128 (n=49)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.2519 (IC base=+0.060)

- **PATRÓN** `sigma_ewma_delta_pct` > `14.551` → IC=+0.333 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 14.551 (IC base=+0.060)
...
- **PATRÓN** `sigma_h` > `0.0265` → IC=+0.241 (n=79)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0265 (IC base=+0.194)
...
- **PATRÓN** `dist_vwap_pct` > `0.3031` → IC=+0.179 (n=51)
  ...
  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.3031 (IC base=+0.194)

- **PATRÓN** `dist_vwap_pct` < `0.4878` → IC=+0.181 (n=180)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.4878 (IC base=+0.194)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.517` → IC=+0.197 (n=140)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` < 2.517 (IC base=+0.194)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.167 (n=52)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 17.0 (IC base=+0.129)

- **PATRÓN** `py_entrada` < `0.495` → IC=+0.214 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.495 (IC base=+0.129)

- **PATRÓN** `btc_momentum` |x|≤ `0.03` → IC=+0.147 (n=49)

  - _Acción_: Kelly boost +0.74€ cuando `btc_momentum` |x|≤ 0.03 (IC base=+0.129)
...
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#SOL#15min` — IC=+0.200 n=38. Faltan ~2 resoluciones para umbral n≥40. ETA: ~1h.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#SOL` — IC=+0.200 n=38. Faltan ~2 resoluciones para umbral n≥40. ETA: ~1h.
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2487 | +0.175 | -51.87€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1598 | +0.212 | +25.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 106 | +0.009 | -19.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 360 | +0.088 | -59.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 423 | +0.145 | +2.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 720 | +0.181 | -10.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 534 | +0.207 | -8.69€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 4827 | +0.103 | +1505.66€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 4827 | +0.103 | +1505.66€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1157 | +0.063 | +162.28€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1157 | +0.063 | +162.28€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1157 | +0.077 | +204.02€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1157 | +0.077 | +204.02€ | 1 | 6 |
| ✅ GBM_LATE_15M#SOL | 1292 | +0.110 | +522.58€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1292 | +0.110 | +522.58€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1221 | +0.156 | +616.77€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1221 | +0.156 | +616.77€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2056 | +0.151 | +1209.89€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2056 | +0.151 | +1209.89€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 525 | +0.111 | +258.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 525 | +0.111 | +258.64€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 545 | +0.133 | +264.57€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 545 | +0.133 | +264.57€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 556 | +0.113 | +244.71€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 556 | +0.113 | +244.71€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 430 | +0.271 | +441.97€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 430 | +0.271 | +441.97€ | 0 | 12 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 71 | +0.075 | +17.56€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 71 | +0.075 | +17.56€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 43 | +0.233 | +20.20€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 43 | +0.233 | +20.20€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 2027 | +0.090 | +607.09€ | 0 | 7 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2027 | +0.090 | +607.09€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 501 | +0.057 | +86.10€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 501 | +0.057 | +86.10€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 468 | +0.025 | +17.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 468 | +0.025 | +17.64€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 479 | +0.059 | +95.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 479 | +0.059 | +95.64€ | 3 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 579 | +0.197 | +407.70€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 579 | +0.197 | +407.70€ | 0 | 11 |
...
| ✅ LEADLAG_BTC_XRP_15M | 139 | +0.089 | +29.42€ | 0 | 7 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 139 | +0.089 | +29.42€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 139 | +0.089 | +29.42€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 139 | +0.089 | +29.42€ | 0 | 7 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 118 | +0.192 | +35.73€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 118 | +0.192 | +35.73€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 38 | +0.200 | +17.11€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 38 | +0.200 | +17.11€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.154: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.006 n=488 | neutral: IC=-0.020 n=373 | overbought(IBS>0.7): IC=+0.134 n=402
  - _Datos_: n=1337 IC=+0.044 PNL=+136.93€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9709 | tras_1loss IC=+0.069 n=6765 | tras_2loss IC=+0.037 n=2886/40 | gap=+0.065 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=14225 IC=+0.106 PNL=+3495.55€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=14225 IC=+0.106 PNL=+3495.55€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.248 > 0.2 con n=347 PNL=+240.53€
  - _Datos_: n=347 IC=+0.248 PNL=+240.53€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.154 > 0.08 con n=157 PNL=+57.02€
  - _Datos_: n=157 IC=+0.154 PNL=+57.02€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.117 > 0.08 con n=147 PNL=+59.05€
  - _Datos_: n=147 IC=+0.117 PNL=+59.05€
...
**🔶 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.163 n=197) pero sin cruzar ≥2 pares más — BTC: n=143 IC=-0.017; ETH: n=108 IC=+0.000; SOL: n=209 IC=+0.059 ✓
  - _Datos_: n=197 IC=+0.163 PNL=+122.90€
...
**🔶 H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.129 n=68) pero sin cruzar ≥2 pares más — sin otros pares con datos
  - _Datos_: n=68 IC=+0.129 PNL=+24.97€
