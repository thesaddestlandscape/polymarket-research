# Hipótesis automáticas — 2026-08-19 01:00 UTC
_Generado por shadow_postmortem.py sobre 69897 resoluciones (PNL=+6977.87€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `152.54` → IC=-0.248 (n=931)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 152.54
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=2794)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `118.97` → IC=-0.401 (n=99)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.192 (n=2381)
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.153 (n=2897)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.195 (n=1178)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` < `0.7` → IC=+0.190 (n=230)
  ...
  - _Acción_: Kelly boost +0.95€ cuando `py_entrada` < 0.7 (IC base=+0.156)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.196 (n=278)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.169 (n=373)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.162 (n=1214)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.169 (n=600)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` < `0.43` → IC=+0.172 (n=763)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.149 (n=907)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.144 (n=447)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `ibs_20min` < `0.6654` → IC=+0.122 (n=2191)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.21` → IC=+0.127 (n=1024)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2067` → IC=+0.154 (n=449)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 2.2067 (IC base=+0.050)

- **PATRÓN** `ballena_activa_n` < `38.0` → IC=+0.260 (n=77)
...
- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.220 (n=1246)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.214)

- **PATRÓN** `drift_60min` |x|≤ `0.2353` → IC=+0.222 (n=1096)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.375` → IC=+0.279 (n=1249)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.255` → IC=+0.215 (n=1236)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.255 (IC base=+0.214)

- **PATRÓN** `volumen_regimen` < `1.252` → IC=+0.191 (n=967)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` < 1.252 (IC base=+0.214)

- **PATRÓN** `volumen_regimen` > `0.8747` → IC=+0.198 (n=644)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` > 0.8747 (IC base=+0.214)
...
- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.277 (n=137)
...
- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.283 (n=233)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0064 (IC base=+0.266)

- **PATRÓN** `sigma_h` > `0.005` → IC=+0.274 (n=237)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.292 (n=99)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.266)

- **PATRÓN** `ibs_20min` < `0.2292` → IC=+0.321 (n=233)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2292 (IC base=+0.266)

- **PATRÓN** `dist_vwap_pct` > `0.3967` → IC=+0.333 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3967 (IC base=+0.266)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.724` → IC=+0.304 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.724 (IC base=+0.266)

- **PATRÓN** `volumen_regimen` > `1.0811` → IC=+0.321 (n=121)
...
- **PATRÓN** `volumen_spike_ratio` > `1.659` → IC=+0.257 (n=138)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.659 (IC base=+0.266)

- **PATRÓN** `libro_liquidez` > `2667.3568` → IC=+0.272 (n=121)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2667.3568 (IC base=+0.266)

- **PATRÓN** `ballena_activa_n` < `40.0` → IC=+0.291 (n=41)
...
- **PATRÓN** `ibs_20min` < `0.3601` → IC=+0.129 (n=165)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` < 0.3601 (IC base=+0.058)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.874` → IC=+0.229 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.874 (IC base=+0.058)

- **PATRÓN** `volumen_spike_ratio` < `2.7248` → IC=+0.130 (n=117)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` < 2.7248 (IC base=+0.058)
...
- **PATRÓN** `ibs_20min` < `0.0588` → IC=+0.265 (n=15)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0588 (IC base=+0.056)
...
- **PATRÓN** `sigma_h` > `0.0038` → IC=+0.231 (n=1393)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.5` → IC=+0.287 (n=1396)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5 (IC base=+0.224)

- **PATRÓN** `dist_vwap_pct` < `0.1561` → IC=+0.199 (n=910)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.1561 (IC base=+0.224)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.498` → IC=+0.226 (n=1307)
...
- **PATRÓN** `volumen_regimen` > `1.0639` → IC=+0.214 (n=428)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0639 (IC base=+0.224)

- **PATRÓN** `volumen_pendiente_norm` < `0.1153` → IC=+0.247 (n=627)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1153 (IC base=+0.224)
...
- **PATRÓN** `ballena_activa_n` < `57.0` → IC=+0.207 (n=179)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.286 (n=353)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `volumen_regimen` > `0.883` → IC=+0.297 (n=235)
...
- **PATRÓN** `libro_liquidez` > `2841.183` → IC=+0.267 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2841.183 (IC base=+0.261)

- **PATRÓN** `ballena_activa_n` < `53.0` → IC=+0.231 (n=65)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.167 (n=19)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 5.0 (IC base=+0.062)

- **PATRÓN** `drift_20min_pct` |x|≤ `0.0481` → IC=+0.192 (n=24)

  - _Acción_: Kelly boost +0.96€ cuando `drift_20min_pct` |x|≤ 0.0481 (IC base=+0.062)

- **PATRÓN** `ibs_20min` > `0.0648` → IC=+0.167 (n=25)

  - _Acción_: Kelly boost +0.83€ cuando `ibs_20min` > 0.0648 (IC base=+0.062)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.177 (n=29)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 17.0 (IC base=+0.068)

- **PATRÓN** `py_entrada` < `0.69` → IC=+0.136 (n=64)

  - _Acción_: Kelly boost +0.68€ cuando `py_entrada` < 0.69 (IC base=+0.068)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.129 (n=33)

  - _Acción_: Kelly boost +0.64€ cuando `py_entrada` < 0.5 (IC base=+0.018)

- **PATRÓN** `libro_liquidez` > `2636.658` → IC=+0.167 (n=28)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 2636.658 (IC base=+0.018)
...
- **FILTRO** `hora_utc` < `15.0` → IC=-0.152 (n=607)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=636)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.283 (n=289)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=954)

- **FILTRO** `ibs_7min` < `0.7187` → IC=-0.237 (n=310)
...
- **FILTRO** `py_entrada` > `0.65` → IC=-0.191 (n=328)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=1048)
...
- **FILTRO** `drift_7min_pct` |x|> `0.0988` → IC=-0.151 (n=64)
...
- **FILTRO** `hora_utc` < `15.0` → IC=-0.187 (n=81)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 15.0
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.140 (n=87)

  - _Acción_: Kelly boost +0.70€ cuando `py_entrada` < 0.5 (IC base=-0.021)
...
- **PATRÓN** `ibs_15` < `0.246` → IC=+0.137 (n=301)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.68€ cuando `ibs_15` < 0.246 (IC base=+0.080)

- **PATRÓN** `dist_vwap_pct` > `0.4863` → IC=+0.149 (n=72)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.4863 (IC base=+0.080)
...
- **PATRÓN** `sigma_h` > `0.0063` → IC=+0.153 (n=93)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0063 (IC base=+0.106)

- **PATRÓN** `drift_60min` |x|≤ `0.1563` → IC=+0.203 (n=62)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1563 (IC base=+0.106)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0979` → IC=+0.161 (n=125)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.81€ cuando `delta_ratio_macro` |x|> 0.0979 (IC base=+0.106)

- **PATRÓN** `hora_utc` < `19.0` → IC=+0.134 (n=140)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` < 19.0 (IC base=+0.106)

- **PATRÓN** `ibs_15` < `0.1163` → IC=+0.203 (n=62)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.1163 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` > `0.127` → IC=+0.204 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.127 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.148` → IC=+0.150 (n=118)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` < 5.148 (IC base=+0.106)

- **PATRÓN** `libro_liquidez` > `2603.9534` → IC=+0.153 (n=125)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 2603.9534 (IC base=+0.106)
...
- **FILTRO** `sigma_ewma_delta_pct` > `12.832` → IC=-0.154 (n=232)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=944)
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1163 sube el IC de +0.106 a +0.203 en UPDOWN_GBM#XRP#15min (n=62). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 3484 | -0.113 | -539.98€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 465 | -0.022 | -18.06€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 3019 | -0.127 | -521.92€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 399 | -0.188 | -94.75€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 399 | -0.188 | -94.75€ | 1 | 0 |
...
| 🚫 BALLENAS_TARDIAS#XRP | 732 | -0.226 | -138.04€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP#5min | 732 | -0.226 | -138.04€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 14999 | +0.116 | -857.30€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3464 | +0.183 | -92.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 100 | -0.098 | -46.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 8821 | +0.086 | -748.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2614 | +0.133 | +30.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 1489 | +0.030 | -326.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 14 | -0.044 | +0.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 1471 | +0.033 | -320.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 3387 | +0.142 | +9.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 947 | +0.200 | -26.28€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 41 | -0.105 | -20.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 1469 | +0.114 | -7.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 930 | +0.140 | +64.35€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 1486 | +0.057 | -252.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 10 | +0.000 | -3.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 1475 | +0.058 | -247.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 3661 | +0.126 | -44.22€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 3492 | +0.137 | -197.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1202 | +0.199 | -47.97€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 41 | +0.012 | -8.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 1469 | +0.086 | -100.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 780 | +0.142 | -40.21€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 1484 | +0.122 | -46.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 12 | +0.043 | +1.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 1471 | +0.123 | -46.37€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 3534 | +0.159 | -346.95€ | 2 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 3534 | +0.159 | -346.95€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 876 | +0.156 | -113.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 876 | +0.156 | -113.15€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 829 | +0.172 | -82.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 829 | +0.172 | -82.95€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 6275 | +0.188 | -593.78€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 6275 | +0.188 | -593.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 1175 | +0.096 | -266.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 1175 | +0.096 | -266.22€ | 0 | 2 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 1101 | +0.155 | -156.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 1101 | +0.155 | -156.12€ | 0 | 2 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 976 | +0.246 | -11.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 976 | +0.246 | -11.36€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 1048 | +0.183 | -108.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 1048 | +0.183 | -108.98€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 2257 | +0.147 | +117.64€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 2257 | +0.147 | +117.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 1115 | +0.153 | +68.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 1115 | +0.153 | +68.54€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 1142 | +0.142 | +49.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 1142 | +0.142 | +49.10€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 4641 | +0.085 | +1639.34€ | 0 | 14 |
| ✅ GBM_LATE_15M#15min | 4641 | +0.085 | +1639.34€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#DOGE | 816 | +0.188 | +550.88€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 816 | +0.188 | +550.88€ | 0 | 16 |
...
| ✅ GBM_LATE_15M#XRP | 1070 | +0.013 | +225.27€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1070 | +0.013 | +225.27€ | 0 | 4 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 5662 | +0.049 | +1690.77€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 5662 | +0.049 | +1690.77€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 3387 | +0.176 | +2299.85€ | 0 | 25 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 3387 | +0.176 | +2299.85€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP | 681 | +0.198 | +532.80€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP#15min | 681 | +0.198 | +532.80€ | 0 | 26 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 567 | +0.050 | +50.06€ | 0 | 7 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 567 | +0.050 | +50.06€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 118 | +0.058 | +7.21€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 118 | +0.058 | +7.21€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO | 3947 | +0.168 | +2544.48€ | 0 | 23 |
| ✅ GBM_LATE_15M_TARDIO#15min | 3947 | +0.168 | +2544.48€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 878 | +0.201 | +673.61€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 878 | +0.201 | +673.61€ | 0 | 25 |
...
| ✅ LIQUIDACIONES_5M | 90 | -0.120 | -12.22€ | 4 | 0 |
| ✅ LIQUIDACIONES_5M#5min | 90 | -0.120 | -12.22€ | 0 | 0 |
| ✅ LIQUIDACIONES_5M#BTC | 30 | +0.000 | -0.45€ | 0 | 0 |
| ✅ LIQUIDACIONES_5M#BTC#5min | 30 | +0.000 | -0.45€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_15M | 685 | +0.028 | +6.64€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#15min | 685 | +0.028 | +6.64€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB | 104 | +0.057 | +11.72€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB#15min | 104 | +0.057 | +11.72€ | 1 | 5 |
| ✅ MOMENTUM_IBS_15M#BTC | 121 | +0.069 | +13.26€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BTC#15min | 121 | +0.069 | +13.26€ | 0 | 3 |
| ✅ MOMENTUM_IBS_15M#DOGE | 108 | +0.009 | -11.23€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#DOGE#15min | 108 | +0.009 | -11.23€ | 2 | 0 |
...
| ✅ MOMENTUM_IBS_15M#XRP | 118 | +0.000 | -7.17€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#XRP#15min | 118 | +0.000 | -7.17€ | 0 | 1 |
| ✅ MOMENTUM_IBS_15M_BALLENA | 943 | -0.041 | +9.15€ | 4 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#15min | 943 | -0.041 | +9.15€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE | 153 | +0.003 | +34.31€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE#15min | 153 | +0.003 | +34.31€ | 3 | 2 |
...
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP | 160 | -0.043 | -7.84€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP#15min | 160 | -0.043 | -7.84€ | 5 | 2 |
| ✅ MOMENTUM_IBS_15M_FADE | 338 | -0.056 | -22.44€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#15min | 338 | -0.056 | -22.44€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 2619 | -0.066 | +80.82€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 2619 | -0.066 | +80.82€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 387 | -0.096 | +26.00€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 387 | -0.096 | +26.00€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 505 | -0.058 | +74.78€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 505 | -0.058 | +74.78€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 400 | -0.065 | -5.50€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 400 | -0.065 | -5.50€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 444 | -0.081 | -30.11€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 444 | -0.081 | -30.11€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 477 | -0.043 | -2.39€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 477 | -0.043 | -2.39€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 406 | -0.059 | +18.03€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 406 | -0.059 | +18.03€ | 5 | 1 |
| ✅ MOMENTUM_IBS_5M_FADE | 2343 | +0.016 | +15.47€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 2343 | +0.016 | +15.47€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB | 394 | +0.015 | +7.07€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB#5min | 394 | +0.015 | +7.07€ | 1 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC | 334 | +0.033 | +4.87€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC#5min | 334 | +0.033 | +4.87€ | 2 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE | 405 | +0.009 | -0.96€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE#5min | 405 | +0.009 | -0.96€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH | 410 | +0.007 | +1.49€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH#5min | 410 | +0.007 | +1.49€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL | 411 | +0.013 | +1.88€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL#5min | 411 | +0.013 | +1.88€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP | 389 | +0.019 | +1.13€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP#5min | 389 | +0.019 | +1.13€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 2366 | +0.027 | +175.35€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1024 | +0.090 | +195.39€ | 1 | 8 |
| ✅ UPDOWN_GBM#240min | 124 | +0.016 | -0.04€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 916 | -0.016 | -10.32€ | 3 | 0 |
...
| ✅ UPDOWN_GBM#SOL | 435 | -0.006 | +2.54€ | 0 | 0 |
...
| ✅ UPDOWN_GBM#XRP | 530 | +0.024 | +69.66€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 314 | +0.098 | +94.89€ | 0 | 15 |
| ✅ UPDOWN_GBM#XRP#240min | 17 | -0.112 | -2.90€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 199 | -0.077 | -22.33€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1728 | -0.050 | +217.75€ | 2 | 3 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1728 | -0.050 | +217.75€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 538 | -0.039 | +72.54€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 538 | -0.039 | +72.54€ | 4 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.134) — sin ventaja clara. oversold(IBS<0.3): IC=-0.011 n=831 | neutral: IC=+0.010 n=794 | overbought(IBS>0.7): IC=+0.122 n=1049
  - _Datos_: n=2869 IC=+0.046 PNL=+273.53€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=40673 | tras_1loss IC=+0.060 n=28925 | tras_2loss IC=+0.019 n=12683/40 | gap=+0.083 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2141 IC=+0.018 PNL=+120.45€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2141 IC=+0.018 PNL=+120.45€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.090 > 0.08 con n=637 PNL=+106.74€
  - _Datos_: n=637 IC=+0.090 PNL=+106.74€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=125 IC=+0.028 PNL=+12.75€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=125 IC=+0.028 PNL=+12.75€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=506 IC=+0.095 PNL=+128.92€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=506 IC=+0.095 PNL=+128.92€

**〰️ H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  - _Hipótesis_: Feature nueva 2026-06-27: drift del precio YES en Polymarket en últimas 5 obs (~5min). Si poly_drift<0 y decidimos BUY_NO (o poly_drift>0 y BUY_YES) → confluencia. Si diverge → reducción de stake. Hipótesis: confluencia Binance+Polymarket mejora IC; divergencia empeora.
  - _Umbral_: n≥40 en confluencia vs divergencia para validar el boost ×1.1
  - _Acción_: Si IC_confluencia>IC_divergencia con n≥40 → mantener el boost. Si no → retirar.
  - _Estado_: n=309 IC=+0.037 PNL=-0.91€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=309 IC=+0.037 PNL=-0.91€
...
**🟡 H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.106 > 0.1 con n=186 PNL=+60.87€
  - _Datos_: n=186 IC=+0.106 PNL=+60.87€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1449 IC=-0.145 PNL=+71.75€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1449 IC=-0.145 PNL=+71.75€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.205 > 0.08 con n=286 PNL=+164.46€
  - _Datos_: n=286 IC=+0.205 PNL=+164.46€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=1175 IC=+0.096 PNL=-266.22€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=1175 IC=+0.096 PNL=-266.22€
