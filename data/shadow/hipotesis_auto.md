# Hipótesis automáticas — 2026-09-26 06:14 UTC
_Generado por shadow_postmortem.py sobre 616297 resoluciones (PNL=+70014.31€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.26` → IC=-0.226 (n=7241)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.26
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=21732)
...
- **FILTRO** `restante_s_al_confirmar` < `166.21` → IC=-0.210 (n=1762)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 166.21
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=5286)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `127.5` → IC=-0.344 (n=1424)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 127.5
  - _Potencial_: sin este filtro IC_bueno=-0.110 (n=4275)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.205 (n=14178)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.69 (IC base=+0.101)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.153 (n=3582)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.101)

- **PATRÓN** `libro_liquidez` > `5676.6756` → IC=+0.178 (n=2279)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.205 (n=2186)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.204)

- **PATRÓN** `libro_liquidez` > `15975.8246` → IC=+0.240 (n=564)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 15975.8246 (IC base=+0.204)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.205 (n=1562)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.202)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.208 (n=1735)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.204 (n=2219)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.145 (n=364)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.155 (n=2876)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 5.0 (IC base=+0.145)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.146 (n=2451)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` < `0.225` → IC=+0.364 (n=507)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.165 (n=598)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.242 (n=904)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.184 (n=2812)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 5.0 (IC base=+0.179)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.183 (n=2687)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.248 (n=2477)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.189 (n=6419)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.221 (n=2409)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.220)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.268 (n=2175)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.254 (n=2339)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.75 (IC base=+0.203)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.195 (n=6931)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
### GBM_LATE_15M
- **PATRÓN** `drift_60min` |x|≤ `0.4865` → IC=+0.122 (n=8347)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.61€ cuando `drift_60min` |x|≤ 0.4865 (IC base=+0.106)

- **PATRÓN** `ibs_20min` > `0.9823` → IC=+0.243 (n=2782)
...
- **PATRÓN** `volumen_regimen` < `0.6967` → IC=+0.185 (n=1563)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 0.6967 (IC base=+0.066)

- **PATRÓN** `volumen_regimen` > `1.0483` → IC=+0.176 (n=1610)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 1.0483 (IC base=+0.066)

- **PATRÓN** `volumen_pendiente_norm` > `0.1671` → IC=+0.228 (n=1693)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1671 (IC base=+0.066)

- **PATRÓN** `volumen_spike_ratio` > `1.5705` → IC=+0.202 (n=5354)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5705 (IC base=+0.066)

- **PATRÓN** `ballena_activa_n` < `134.0` → IC=+0.211 (n=5789)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 134.0 (IC base=+0.066)

### GBM_LATE_15M#BNB#15min
- **PATRÓN** `sigma_h` < `0.005` → IC=+0.181 (n=626)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3502` → IC=+0.166 (n=1878)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.166 (n=1261)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.136` → IC=+0.266 (n=805)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4363` → IC=+0.165 (n=1758)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.4363 (IC base=+0.161)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.176 (n=1912)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.235 (n=692)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.235)

- **PATRÓN** `ibs_20min` < `0.0538` → IC=+0.290 (n=630)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.468` → IC=+0.245 (n=1487)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4285` → IC=+0.231 (n=437)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.237 (n=1580)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.235)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `sigma_h` < `0.0031` → IC=+0.230 (n=636)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0031 (IC base=+0.216)

- **PATRÓN** `drift_60min` |x|≤ `0.0842` → IC=+0.242 (n=482)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0842 (IC base=+0.216)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.232 (n=1511)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.216)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.216 (n=1466)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.5663` → IC=+0.220 (n=1514)
...
- **PATRÓN** `volumen_regimen` < `1.2505` → IC=+0.218 (n=1444)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2505 (IC base=+0.216)

- **PATRÓN** `volumen_regimen` > `1.0754` → IC=+0.228 (n=655)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0754 (IC base=+0.216)
...
- **PATRÓN** `volumen_regimen` < `1.1913` → IC=+0.150 (n=1506)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.1913 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` > `0.8481` → IC=+0.140 (n=1004)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 0.8481 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` > `0.1556` → IC=+0.178 (n=399)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.1556 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` < `2.4311` → IC=+0.154 (n=1396)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.4311 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.42` → IC=+0.148 (n=1395)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.191 (n=1946)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 5.0 (IC base=+0.184)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.188 (n=1663)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` < `0.0104` → IC=+0.221 (n=1414)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0104 (IC base=+0.210)

- **PATRÓN** `drift_60min` |x|≤ `0.6123` → IC=+0.211 (n=1603)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.219 (n=752)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.210)

- **PATRÓN** `ibs_20min` < `0.0643` → IC=+0.247 (n=705)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0643 (IC base=+0.210)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.714` → IC=+0.235 (n=609)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.714 (IC base=+0.210)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.506` → IC=+0.210 (n=1742)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7621` → IC=+0.203 (n=648)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7621 (IC base=+0.210)

- **PATRÓN** `volumen_spike_ratio` > `2.1874` → IC=+0.218 (n=982)
...
- **PATRÓN** `libro_liquidez` > `1905.7248` → IC=+0.216 (n=727)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1905.7248 (IC base=+0.210)

- **PATRÓN** `ballena_activa_n` < `23.0` → IC=+0.215 (n=950)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.144 (n=102)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=2276)

- **PATRÓN** `ibs_20min` > `0.9456` → IC=+0.213 (n=368)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9456 (IC base=+0.026)

- **PATRÓN** `dist_vwap_pct` > `0.1549` → IC=+0.323 (n=218)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1549 (IC base=+0.026)

- **PATRÓN** `dist_vwap_pct` < `0.5539` → IC=+0.330 (n=345)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5539 (IC base=+0.026)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.74` → IC=+0.161 (n=735)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 4.74 (IC base=+0.026)

- **PATRÓN** `volumen_regimen` < `0.8561` → IC=+0.330 (n=228)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2083` → IC=+0.328 (n=149)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2083 (IC base=+0.026)

- **PATRÓN** `ballena_activa_n` < `158.0` → IC=+0.331 (n=329)
...
- **PATRÓN** `volumen_regimen` < `0.8439` → IC=+0.163 (n=580)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.8439 (IC base=+0.019)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5219` → IC=+0.176 (n=729)
...
- **FILTRO** `ibs_20min` > `0.25` → IC=-0.124 (n=2253)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.692` → IC=-0.211 (n=361)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=3011)
...
- **FILTRO** `drift_60min` |x|> `0.6576` → IC=-0.185 (n=588)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6576
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=1768)

- **FILTRO** `ibs_20min` < `0.7197` → IC=-0.154 (n=1554)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7197
  - _Potencial_: sin este filtro IC_bueno=+0.102 (n=802)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.202 (n=441)
...
- **FILTRO** `ibs_20min` > `0.77` → IC=-0.203 (n=859)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.039 (n=2597)
...
- **PATRÓN** `ibs_20min` > `0.8935` → IC=+0.264 (n=727)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8935 (IC base=+0.042)
...
- **PATRÓN** `volumen_spike_ratio` < `1.433` → IC=+0.171 (n=308)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` < 1.433 (IC base=+0.042)
...
- **PATRÓN** `ibs_20min` > `0.8641` → IC=+0.157 (n=704)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` > 0.8641 (IC base=+0.025)

- **PATRÓN** `dist_vwap_pct` > `0.3115` → IC=+0.175 (n=383)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.3115 (IC base=+0.025)

- **PATRÓN** `volumen_regimen` > `0.6736` → IC=+0.169 (n=870)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4239` → IC=+0.191 (n=318)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` < 1.4239 (IC base=+0.025)

- **PATRÓN** `volumen_spike_ratio` > `2.3919` → IC=+0.156 (n=318)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 2.3919 (IC base=+0.025)

- **PATRÓN** `ballena_activa_n` < `242.0` → IC=+0.190 (n=417)

  - _Acción_: Kelly boost +0.95€ cuando `ballena_activa_n` < 242.0 (IC base=+0.025)

- **PATRÓN** `dist_vwap_pct` < `0.1603` → IC=+0.223 (n=616)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1603 (IC base=+0.006)

- **PATRÓN** `volumen_regimen` > `0.6101` → IC=+0.221 (n=599)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6101 (IC base=+0.006)

- **PATRÓN** `volumen_pendiente_norm` > `0.2697` → IC=+0.324 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2697 (IC base=+0.006)

- **PATRÓN** `volumen_spike_ratio` < `1.4386` → IC=+0.217 (n=185)
...
- **PATRÓN** `ballena_activa_n` < `460.0` → IC=+0.222 (n=552)
...
- **PATRÓN** `sigma_h` > `0.0115` → IC=+0.288 (n=551)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0115 (IC base=+0.245)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.249 (n=1731)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.245)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.247 (n=1660)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1035` → IC=+0.259 (n=1399)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1035 (IC base=+0.245)

- **PATRÓN** `volumen_spike_ratio` > `3.3887` → IC=+0.263 (n=521)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.3887 (IC base=+0.245)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.252 (n=1154)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.245)

- **PATRÓN** `libro_liquidez` > `1976.1558` → IC=+0.250 (n=550)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1976.1558 (IC base=+0.245)

- **PATRÓN** `sigma_h` > `0.01` → IC=+0.313 (n=602)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.01 (IC base=+0.281)

- **PATRÓN** `drift_60min` |x|≤ `0.1749` → IC=+0.289 (n=585)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.0909` → IC=+0.288 (n=887)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0909 (IC base=+0.281)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.795` → IC=+0.295 (n=526)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.795 (IC base=+0.281)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.754` → IC=+0.281 (n=1422)
...
- **PATRÓN** `volumen_spike_ratio` < `1.739` → IC=+0.285 (n=542)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.739 (IC base=+0.281)

- **PATRÓN** `volumen_spike_ratio` > `2.7106` → IC=+0.289 (n=558)
...
- **PATRÓN** `libro_liquidez` > `1898.5935` → IC=+0.293 (n=602)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1898.5935 (IC base=+0.281)

- **PATRÓN** `ballena_activa_n` < `28.0` → IC=+0.285 (n=811)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 28.0 (IC base=+0.281)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **FILTRO** `ibs_20min` < `0.2956` → IC=-0.180 (n=517)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2956
  - _Potencial_: sin este filtro IC_bueno=+0.078 (n=1552)

- **FILTRO** `ibs_20min` > `0.7744` → IC=-0.185 (n=618)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7744
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=1858)

- **PATRÓN** `ibs_20min` > `0.9107` → IC=+0.179 (n=518)

  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` > 0.9107 (IC base=+0.013)

- **PATRÓN** `dist_vwap_pct` > `0.4389` → IC=+0.219 (n=265)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4389 (IC base=+0.013)

- **PATRÓN** `dist_vwap_pct` < `0.687` → IC=+0.215 (n=613)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.687 (IC base=+0.013)

- **PATRÓN** `volumen_regimen` < `0.9969` → IC=+0.237 (n=527)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9969 (IC base=+0.013)

- **PATRÓN** `volumen_regimen` > `0.5902` → IC=+0.215 (n=599)
...
- **PATRÓN** `volumen_spike_ratio` < `1.3994` → IC=+0.264 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3994 (IC base=+0.013)

- **PATRÓN** `ballena_activa_n` < `148.0` → IC=+0.245 (n=575)
...
- **PATRÓN** `dist_vwap_pct` < `0.351` → IC=+0.198 (n=451)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.351 (IC base=-0.004)

- **PATRÓN** `volumen_regimen` < `1.1663` → IC=+0.209 (n=448)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8268` → IC=+0.261 (n=270)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1389` → IC=+0.235 (n=183)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1389 (IC base=-0.004)

- **PATRÓN** `ballena_activa_n` < `138.0` → IC=+0.253 (n=406)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 138.0 (IC base=-0.004)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **FILTRO** `ibs_20min` < `0.7317` → IC=-0.193 (n=1113)
...
- **PATRÓN** `ibs_20min` > `0.7317` → IC=+0.277 (n=1113)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.635` → IC=+0.171 (n=350)
...
- **PATRÓN** `volumen_regimen` > `0.7291` → IC=+0.293 (n=737)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7291 (IC base=+0.042)

- **PATRÓN** `volumen_pendiente_norm` < `0.1016` → IC=+0.292 (n=769)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.285 (n=1423)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.277)

- **PATRÓN** `ibs_20min` > `0.74` → IC=+0.322 (n=1212)
...
- **PATRÓN** `volumen_regimen` > `0.8606` → IC=+0.303 (n=905)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2814` → IC=+0.328 (n=202)
...
- **PATRÓN** `libro_liquidez` > `2636.2554` → IC=+0.294 (n=904)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2636.2554 (IC base=+0.277)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.279 (n=727)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.275)

- **PATRÓN** `ibs_20min` < `0.3902` → IC=+0.306 (n=1459)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3902 (IC base=+0.275)

- **PATRÓN** `dist_vwap_pct` > `0.3125` → IC=+0.281 (n=551)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3125 (IC base=+0.275)

- **PATRÓN** `dist_vwap_pct` < `0.2281` → IC=+0.275 (n=1324)
...
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.171 (n=2721)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0049 (IC base=+0.167)

- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.200 (n=2711)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0113 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0902` → IC=+0.184 (n=2711)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.0902 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.179 (n=8460)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 5.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` > `0.5737` → IC=+0.217 (n=8132)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5737 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.1772` → IC=+0.194 (n=3539)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.1772 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.335` → IC=+0.255 (n=1660)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.335 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.2091` → IC=+0.159 (n=5386)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.2091 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` > `0.6307` → IC=+0.160 (n=5385)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5582` → IC=+0.166 (n=3434)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.5582 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.6119` → IC=+0.175 (n=2603)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.6119 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `2409.2447` → IC=+0.168 (n=5421)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2409.2447 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `113.0` → IC=+0.180 (n=7043)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 113.0 (IC base=+0.167)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.183 (n=5189)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0066 (IC base=+0.168)

- **PATRÓN** `drift_60min` |x|≤ `0.0812` → IC=+0.210 (n=2595)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.170 (n=3691)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 7.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` < `0.4818` → IC=+0.226 (n=7780)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4818 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` < `0.2351` → IC=+0.159 (n=5640)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` < 0.2351 (IC base=+0.168)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.32` → IC=+0.192 (n=1318)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 10.32 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` < `1.1707` → IC=+0.152 (n=5624)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1707 (IC base=+0.168)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6147` → IC=+0.172 (n=2365)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 2.6147 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `115.0` → IC=+0.174 (n=6757)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.240 (n=899)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.1871` → IC=+0.289 (n=680)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.252 (n=506)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.238)

- **PATRÓN** `ibs_20min` < `0.3451` → IC=+0.263 (n=1019)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3451 (IC base=+0.238)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.34` → IC=+0.249 (n=1104)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.34 (IC base=+0.238)

- **PATRÓN** `volumen_pendiente_norm` < `0.1592` → IC=+0.234 (n=965)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4206` → IC=+0.263 (n=314)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4206 (IC base=+0.238)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.239 (n=1127)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.238)

- **PATRÓN** `libro_liquidez` > `1814.5` → IC=+0.242 (n=679)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1814.5 (IC base=+0.238)

### GBM_LATE_15M_MULTIHORIZONTE#BTC#15min
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.229 (n=408)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0028 (IC base=+0.161)

- **PATRÓN** `drift_60min` |x|≤ `0.0733` → IC=+0.191 (n=403)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.96€ cuando `drift_60min` |x|≤ 0.0733 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.184 (n=1274)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 5.0 (IC base=+0.161)

- **PATRÓN** `ibs_20min` > `0.4047` → IC=+0.226 (n=1208)
...
- **PATRÓN** `volumen_regimen` < `0.6876` → IC=+0.165 (n=532)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.6876 (IC base=+0.161)
...
- **PATRÓN** `libro_liquidez` > `10644.0099` → IC=+0.169 (n=1208)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 10644.0099 (IC base=+0.161)
...
- **PATRÓN** `sigma_h` < `0.0102` → IC=+0.236 (n=1001)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.35` → IC=+0.248 (n=1138)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.35 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.718` → IC=+0.261 (n=488)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7646` → IC=+0.211 (n=465)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7646 (IC base=+0.217)

- **PATRÓN** `volumen_spike_ratio` > `2.1999` → IC=+0.233 (n=705)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1999 (IC base=+0.217)

- **PATRÓN** `libro_liquidez` > `1903.2584` → IC=+0.220 (n=516)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1903.2584 (IC base=+0.217)

- **PATRÓN** `ballena_activa_n` < `11.0` → IC=+0.211 (n=358)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 11.0 (IC base=+0.217)

### GBM_LATE_15M_MULTIHORIZONTE#ETH#15min
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.173 (n=1145)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0066 (IC base=+0.142)

- **PATRÓN** `drift_60min` |x|≤ `0.4315` → IC=+0.158 (n=1300)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.4315 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.161 (n=1361)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.142)

- **PATRÓN** `ibs_20min` > `0.3713` → IC=+0.195 (n=1301)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` > 0.3713 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.1555` → IC=+0.177 (n=859)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.1555 (IC base=+0.142)
...
- **PATRÓN** `volumen_regimen` > `0.6251` → IC=+0.146 (n=1300)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5049` → IC=+0.167 (n=424)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 2.5049 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `5983.7933` → IC=+0.181 (n=867)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 5983.7933 (IC base=+0.142)

- **PATRÓN** `ballena_activa_n` < `162.0` → IC=+0.147 (n=1241)

  - _Acción_: Kelly boost +0.73€ cuando `ballena_activa_n` < 162.0 (IC base=+0.142)

- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.153 (n=1367)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0072 (IC base=+0.123)

- **PATRÓN** `drift_60min` |x|≤ `0.3834` → IC=+0.144 (n=1366)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6462` → IC=+0.171 (n=1366)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.6462 (IC base=+0.123)

- **PATRÓN** `dist_vwap_pct` < `0.364` → IC=+0.135 (n=1476)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.364 (IC base=+0.123)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.963` → IC=+0.164 (n=483)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7994` → IC=+0.139 (n=830)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 1.7994 (IC base=+0.123)

- **PATRÓN** `libro_liquidez` > `10007.3401` → IC=+0.159 (n=620)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.138 (n=1504)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 5.0 (IC base=+0.117)

- **PATRÓN** `ibs_20min` > `0.5143` → IC=+0.203 (n=1472)
...
- **PATRÓN** `volumen_regimen` < `1.2145` → IC=+0.129 (n=1472)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5387` → IC=+0.139 (n=626)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` < 1.5387 (IC base=+0.117)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.125 (n=1537)

  - _Acción_: Kelly boost +0.63€ cuando `libro_spread` < 0.02 (IC base=+0.117)

- **PATRÓN** `libro_liquidez` > `2898.7698` → IC=+0.193 (n=668)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 2898.7698 (IC base=+0.117)

- **PATRÓN** `ballena_activa_n` < `50.0` → IC=+0.133 (n=1145)

  - _Acción_: Kelly boost +0.66€ cuando `ballena_activa_n` < 50.0 (IC base=+0.117)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.159 (n=658)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0062 (IC base=+0.115)

- **PATRÓN** `drift_60min` |x|≤ `0.1048` → IC=+0.164 (n=498)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.82€ cuando `drift_60min` |x|≤ 0.1048 (IC base=+0.115)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.159 (n=678)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 15.0 (IC base=+0.115)

- **PATRÓN** `ibs_20min` < `0.5714` → IC=+0.212 (n=1493)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5714 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` > `1.0231` → IC=+0.136 (n=215)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 1.0231 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` < `0.2057` → IC=+0.140 (n=1367)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.2057 (IC base=+0.115)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.542` → IC=+0.133 (n=309)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` > 7.542 (IC base=+0.115)

- **PATRÓN** `volumen_regimen` < `0.6379` → IC=+0.146 (n=498)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4523` → IC=+0.131 (n=448)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` < 1.4523 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` > `2.4117` → IC=+0.131 (n=448)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 2.4117 (IC base=+0.115)

- **PATRÓN** `libro_liquidez` > `2760.8425` → IC=+0.160 (n=677)
...
- **PATRÓN** `sigma_h` < `0.0117` → IC=+0.226 (n=640)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0117 (IC base=+0.204)

- **PATRÓN** `sigma_h` > `0.0224` → IC=+0.208 (n=659)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.215 (n=669)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.204)

- **PATRÓN** `ibs_20min` < `0.438` → IC=+0.246 (n=1453)
...
- **PATRÓN** `volumen_regimen` > `0.6303` → IC=+0.215 (n=1453)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1921` → IC=+0.195 (n=1153)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.1921 (IC base=+0.204)

- **PATRÓN** `volumen_spike_ratio` > `1.4336` → IC=+0.200 (n=1310)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4336 (IC base=+0.204)

- **PATRÓN** `libro_liquidez` > `2602.132` → IC=+0.207 (n=969)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2602.132 (IC base=+0.204)

### GBM_LATE_15M_PYCONFIRMADO
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.167 (n=665)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0039 (IC base=+0.147)

- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.175 (n=660)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0089 (IC base=+0.147)

- **PATRÓN** `drift_60min` |x|≤ `0.3553` → IC=+0.150 (n=1743)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.3832` → IC=+0.179 (n=1980)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` > 0.3832 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.8737` → IC=+0.178 (n=327)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.8737 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.719` → IC=+0.175 (n=890)
...
- **PATRÓN** `volumen_regimen` > `0.623` → IC=+0.151 (n=1746)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4385` → IC=+0.162 (n=637)
...
- **PATRÓN** `ballena_activa_n` < `152.0` → IC=+0.167 (n=1758)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 152.0 (IC base=+0.147)

- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.138 (n=1364)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.0598` → IC=+0.190 (n=682)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` < 0.0598 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.2116` → IC=+0.121 (n=1825)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.2116 (IC base=+0.110)

- **PATRÓN** `volumen_regimen` < `0.6989` → IC=+0.130 (n=812)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 0.6989 (IC base=+0.110)

- **PATRÓN** `volumen_pendiente_norm` > `0.166` → IC=+0.132 (n=509)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_pendiente_norm` > 0.166 (IC base=+0.110)

- **PATRÓN** `volumen_spike_ratio` < `1.4512` → IC=+0.144 (n=658)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 1.4512 (IC base=+0.110)

- **PATRÓN** `libro_liquidez` > `2761.3271` → IC=+0.121 (n=1828)

  - _Acción_: Kelly boost +0.60€ cuando `libro_liquidez` > 2761.3271 (IC base=+0.110)

- **PATRÓN** `ballena_activa_n` < `29.0` → IC=+0.126 (n=840)
...
- **PATRÓN** `ibs_20min` < `0.0837` → IC=+0.160 (n=195)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` < 0.0837 (IC base=+0.093)

- **PATRÓN** `volumen_regimen` < `0.6864` → IC=+0.139 (n=258)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` < 0.6864 (IC base=+0.093)

- **PATRÓN** `volumen_pendiente_norm` > `0.1666` → IC=+0.136 (n=138)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_pendiente_norm` > 0.1666 (IC base=+0.093)

### GBM_LATE_15M_PYCONFIRMADO#SOL#15min
- **FILTRO** `dist_vwap_pct` > `0.3429` → IC=-0.149 (n=35)

  - _Acción_: SKIP cuando `dist_vwap_pct` > 0.3429
  - _Potencial_: sin este filtro IC_bueno=+0.099 (n=514)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.167 (n=211)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` > 0.0091 (IC base=+0.127)
...
- **PATRÓN** `dist_vwap_pct` > `1.0124` → IC=+0.223 (n=92)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0124 (IC base=+0.127)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.234` → IC=+0.198 (n=200)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` > 5.234 (IC base=+0.127)

- **PATRÓN** `volumen_regimen` < `1.069` → IC=+0.144 (n=408)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.069 (IC base=+0.127)

- **PATRÓN** `volumen_pendiente_norm` > `0.181` → IC=+0.172 (n=132)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.181 (IC base=+0.127)

- **PATRÓN** `volumen_spike_ratio` > `2.1991` → IC=+0.180 (n=201)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` > 2.1991 (IC base=+0.127)
...
- **PATRÓN** `libro_liquidez` > `3131.3797` → IC=+0.188 (n=155)
  ...
  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3131.3797 (IC base=+0.127)

- **PATRÓN** `ibs_20min` < `0.5172` → IC=+0.150 (n=412)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` < 0.5172 (IC base=+0.083)

- **PATRÓN** `volumen_regimen` < `0.7083` → IC=+0.141 (n=182)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 0.7083 (IC base=+0.083)

- **PATRÓN** `volumen_spike_ratio` < `1.5789` → IC=+0.180 (n=170)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 1.5789 (IC base=+0.083)

- **PATRÓN** `libro_liquidez` > `2571.3473` → IC=+0.128 (n=275)

  - _Acción_: Kelly boost +0.64€ cuando `libro_liquidez` > 2571.3473 (IC base=+0.083)

- **PATRÓN** `ballena_activa_n` < `42.0` → IC=+0.134 (n=359)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0114` → IC=+0.205 (n=3478)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0114 (IC base=+0.169)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.180 (n=10862)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `drift_60min` |x|≤ `0.356` → IC=+0.191 (n=1760)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.194 (n=1190)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.564` → IC=+0.344 (n=401)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.210 (n=1778)
...
- **PATRÓN** `ibs_20min` < `0.3517` → IC=+0.285 (n=1225)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3517 (IC base=+0.257)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.489` → IC=+0.261 (n=1459)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5478` → IC=+0.256 (n=564)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5478 (IC base=+0.257)

- **PATRÓN** `volumen_spike_ratio` > `2.635` → IC=+0.279 (n=427)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.635 (IC base=+0.257)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.258 (n=1538)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.257)

- **PATRÓN** `libro_liquidez` > `1811.4172` → IC=+0.259 (n=928)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1811.4172 (IC base=+0.257)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.165 (n=1453)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0057 (IC base=+0.152)

- **PATRÓN** `drift_60min` |x|≤ `0.26` → IC=+0.167 (n=1279)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.157 (n=659)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 7.0 (IC base=+0.152)

- **PATRÓN** `ibs_20min` < `0.2761` → IC=+0.236 (n=968)
...
- **PATRÓN** `dist_vwap_pct` < `0.1331` → IC=+0.165 (n=1319)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.1331 (IC base=+0.152)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.471` → IC=+0.160 (n=242)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 11.471 (IC base=+0.152)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.226 (n=1769)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.217)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.221 (n=1717)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.298` → IC=+0.297 (n=362)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.298 (IC base=+0.217)

- **PATRÓN** `volumen_pendiente_norm` < `0.2104` → IC=+0.220 (n=1685)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2104 (IC base=+0.217)

- **PATRÓN** `volumen_spike_ratio` > `1.7837` → IC=+0.227 (n=1446)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7837 (IC base=+0.217)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.227 (n=1190)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.217)

- **PATRÓN** `sigma_h` < `0.0118` → IC=+0.236 (n=1584)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0118 (IC base=+0.230)

- **PATRÓN** `drift_60min` |x|≤ `0.1709` → IC=+0.236 (n=698)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1709 (IC base=+0.230)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.256 (n=601)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.230)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.240 (n=749)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.230)

- **PATRÓN** `ibs_20min` < `0.0152` → IC=+0.307 (n=528)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0152 (IC base=+0.230)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.722` → IC=+0.273 (n=588)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.722 (IC base=+0.230)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7518` → IC=+0.228 (n=642)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7518 (IC base=+0.230)

- **PATRÓN** `volumen_spike_ratio` > `2.1787` → IC=+0.233 (n=972)
...
- **PATRÓN** `libro_liquidez` > `1903.9584` → IC=+0.236 (n=719)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1903.9584 (IC base=+0.230)

- **PATRÓN** `ballena_activa_n` < `53.0` → IC=+0.227 (n=1377)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 53.0 (IC base=+0.230)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0035` → IC=+0.181 (n=597)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0035 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.4364` → IC=+0.145 (n=1777)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.4364 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.151 (n=1853)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 5.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` > `0.8748` → IC=+0.261 (n=806)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8748 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.3653` → IC=+0.163 (n=701)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.3653 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.44` → IC=+0.167 (n=289)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 11.44 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `0.873` → IC=+0.155 (n=1185)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5194` → IC=+0.151 (n=758)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 1.5194 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `2.4575` → IC=+0.151 (n=574)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 2.4575 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `8015.433` → IC=+0.231 (n=806)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.161 (n=1271)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0066 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.4461` → IC=+0.152 (n=1443)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5812` → IC=+0.197 (n=1270)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` < 0.5812 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `0.5941` → IC=+0.139 (n=1597)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.5941 (IC base=+0.136)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2891` → IC=+0.243 (n=173)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2891 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.4412` → IC=+0.149 (n=1370)
...
- **PATRÓN** `ballena_activa_n` < `176.0` → IC=+0.141 (n=1364)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.133 (n=1814)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.191 (n=1769)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.493` → IC=+0.238 (n=661)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.493 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` < `0.8929` → IC=+0.134 (n=1179)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 0.8929 (IC base=+0.113)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.127 (n=1781)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.02 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `2898.7698` → IC=+0.248 (n=590)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2898.7698 (IC base=+0.113)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.130 (n=1377)
...
- **PATRÓN** `volumen_regimen` < `1.2447` → IC=+0.210 (n=1762)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2447 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` > `0.6398` → IC=+0.217 (n=1761)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6398 (IC base=+0.208)

- **PATRÓN** `volumen_pendiente_norm` > `0.2335` → IC=+0.251 (n=307)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.217 (n=1845)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.208)

- **PATRÓN** `libro_liquidez` > `2631.3944` → IC=+0.218 (n=1174)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2631.3944 (IC base=+0.208)

- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.225 (n=627)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.4227` → IC=+0.266 (n=1654)
...
- **PATRÓN** `dist_vwap_pct` < `0.9233` → IC=+0.205 (n=2088)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1818` → IC=+0.199 (n=1491)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 2.1818 (IC base=+0.202)

- **PATRÓN** `volumen_spike_ratio` > `1.4288` → IC=+0.199 (n=1694)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.4288 (IC base=+0.202)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.147 (n=3176)

- **PATRÓN** `sigma_h` < `0.0091` → IC=+0.168 (n=2687)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0091 (IC base=+0.158)

- **PATRÓN** `drift_60min` |x|≤ `0.5175` → IC=+0.167 (n=3050)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.5175 (IC base=+0.158)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.172 (n=1212)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 17.0 (IC base=+0.158)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.169 (n=1371)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` > `0.1855` → IC=+0.171 (n=1133)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.1855 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` < `0.4782` → IC=+0.154 (n=1911)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.4782 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.2` → IC=+0.194 (n=501)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 10.2 (IC base=+0.158)

- **PATRÓN** `volumen_regimen` > `0.8955` → IC=+0.165 (n=1359)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` > 0.8955 (IC base=+0.158)

- **PATRÓN** `volumen_pendiente_norm` > `0.1707` → IC=+0.193 (n=838)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.1707 (IC base=+0.158)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8746` → IC=+0.167 (n=2009)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.8746 (IC base=+0.158)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.159 (n=2173)

  - _Acción_: Kelly boost +0.79€ cuando `libro_spread` < 0.01 (IC base=+0.158)

- **PATRÓN** `libro_liquidez` > `8263.9499` → IC=+0.164 (n=1383)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 8263.9499 (IC base=+0.158)

- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.200 (n=798)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0038 (IC base=+0.145)

- **PATRÓN** `drift_60min` |x|≤ `0.4865` → IC=+0.163 (n=2394)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.166 (n=1066)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 6.0 (IC base=+0.145)

- **PATRÓN** `ibs_20min` < `0.1802` → IC=+0.175 (n=1054)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `6.223` → IC=+0.157 (n=2386)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` < 6.223 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` < `1.0973` → IC=+0.157 (n=2017)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.0973 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` < `0.0959` → IC=+0.149 (n=2173)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` < 0.0959 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` > `0.0718` → IC=+0.147 (n=1103)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4242` → IC=+0.158 (n=788)
  ...
  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 1.4242 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` > `1.8137` → IC=+0.147 (n=1576)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.8137 (IC base=+0.145)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.147 (n=3176)

  - _Acción_: Kelly boost +0.73€ cuando `libro_spread` < 0.01 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `7207.0296` → IC=+0.157 (n=2138)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 7207.0296 (IC base=+0.145)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.173 (n=897)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.383` → IC=+0.173 (n=897)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.383 (IC base=+0.167)
...
- **PATRÓN** `ibs_20min` < `0.5389` → IC=+0.182 (n=680)
...
- **PATRÓN** `dist_vwap_pct` < `0.4084` → IC=+0.177 (n=953)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.4084 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.174` → IC=+0.178 (n=913)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` < 4.174 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.082` → IC=+0.167 (n=897)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.082 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` > `0.6393` → IC=+0.170 (n=1020)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 0.6393 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.1655` → IC=+0.182 (n=303)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.1655 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `2.4732` → IC=+0.171 (n=1001)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` < 2.4732 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `1.5228` → IC=+0.170 (n=895)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 1.5228 (IC base=+0.167)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.172 (n=1019)

  - _Acción_: Kelly boost +0.86€ cuando `libro_spread` < 0.01 (IC base=+0.167)

- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.192 (n=280)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0041 (IC base=+0.153)

- **PATRÓN** `drift_60min` |x|≤ `0.3872` → IC=+0.184 (n=739)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.3872 (IC base=+0.153)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.173 (n=289)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 17.0 (IC base=+0.153)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.159 (n=596)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` < 11.0 (IC base=+0.153)

- **PATRÓN** `ibs_20min` < `0.7429` → IC=+0.158 (n=840)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` < 0.7429 (IC base=+0.153)

- **PATRÓN** `ibs_20min` > `0.0954` → IC=+0.162 (n=840)

  - _Acción_: Kelly boost +0.81€ cuando `ibs_20min` > 0.0954 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` > `0.1618` → IC=+0.171 (n=387)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.1618 (IC base=+0.153)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.36` → IC=+0.164 (n=762)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` < 4.36 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` < `0.6452` → IC=+0.193 (n=281)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` < 0.6452 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` > `0.7214` → IC=+0.154 (n=750)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 0.7214 (IC base=+0.153)
...
- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.162 (n=235)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` > 0.0069 (IC base=+0.132)

- **PATRÓN** `drift_60min` |x|≤ `0.6959` → IC=+0.150 (n=235)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.1429` → IC=+0.244 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1429 (IC base=+0.132)

- **PATRÓN** `dist_vwap_pct` > `0.6189` → IC=+0.224 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6189 (IC base=+0.132)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.617` → IC=+0.141 (n=126)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` > 2.617 (IC base=+0.132)

- **PATRÓN** `volumen_regimen` < `0.9473` → IC=+0.148 (n=157)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.9473 (IC base=+0.132)

- **PATRÓN** `volumen_pendiente_norm` < `0.1103` → IC=+0.194 (n=191)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` < 0.1103 (IC base=+0.132)

- **PATRÓN** `volumen_spike_ratio` < `1.867` → IC=+0.145 (n=153)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 1.867 (IC base=+0.132)

- **PATRÓN** `volumen_spike_ratio` > `2.1617` → IC=+0.132 (n=104)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 2.1617 (IC base=+0.132)

- **PATRÓN** `libro_liquidez` > `3356.999` → IC=+0.167 (n=235)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 3356.999 (IC base=+0.132)

- **PATRÓN** `ballena_activa_n` < `47.0` → IC=+0.180 (n=198)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 47.0 (IC base=+0.132)

### GBM_LATE_60M
- **FILTRO** `sigma_h` > `0.0066` → IC=-0.205 (n=127)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0066
  - _Potencial_: sin este filtro IC_bueno=+0.082 (n=383)

- **FILTRO** `dist_vwap_pct` > `0.182` → IC=-0.179 (n=26)

  - _Acción_: SKIP cuando `dist_vwap_pct` > 0.182
  - _Potencial_: sin este filtro IC_bueno=+0.119 (n=342)

- **PATRÓN** `sigma_h` < `0.004` → IC=+0.178 (n=424)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.004 (IC base=+0.089)

- **PATRÓN** `ibs_20min` > `0.6667` → IC=+0.194 (n=780)

  - _Acción_: Kelly boost +0.97€ cuando `ibs_20min` > 0.6667 (IC base=+0.089)

- **PATRÓN** `dist_vwap_pct` > `0.1546` → IC=+0.151 (n=468)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.1546 (IC base=+0.089)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0922` → IC=+0.123 (n=666)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_spike_ratio` < 2.0922 (IC base=+0.089)

- **PATRÓN** `ibs_20min` < `0.2693` → IC=+0.225 (n=205)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2693 (IC base=+0.010)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.191` → IC=+0.123 (n=120)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` > 3.191 (IC base=+0.010)

- **PATRÓN** `volumen_pendiente_norm` > `0.07` → IC=+0.204 (n=106)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.07 (IC base=+0.010)

- **PATRÓN** `volumen_spike_ratio` < `2.5091` → IC=+0.131 (n=247)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` < 2.5091 (IC base=+0.010)
...
- **PATRÓN** `sigma_h` < `0.005` → IC=+0.150 (n=218)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.005 (IC base=+0.101)
...
- **PATRÓN** `dist_vwap_pct` > `0.1272` → IC=+0.171 (n=159)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.1272 (IC base=+0.101)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7335` → IC=+0.165 (n=162)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.7335 (IC base=+0.101)

- **PATRÓN** `libro_liquidez` > `1133.3296` → IC=+0.153 (n=260)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 1133.3296 (IC base=+0.101)

- **PATRÓN** `ibs_20min` < `0.1674` → IC=+0.278 (n=43)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1674 (IC base=-0.015)

- **PATRÓN** `dist_vwap_pct` < `0.1197` → IC=+0.121 (n=101)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.1197 (IC base=-0.015)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1353` → IC=+0.227 (n=20)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1353 (IC base=-0.015)

- **PATRÓN** `volumen_spike_ratio` > `2.268` → IC=+0.167 (n=37)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 2.268 (IC base=-0.015)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.148 (n=86)

  - _Acción_: Kelly boost +0.74€ cuando `libro_spread` < 0.02 (IC base=-0.015)
...
- **PATRÓN** `ibs_20min` > `0.6667` → IC=+0.160 (n=248)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.6667 (IC base=+0.063)

- **PATRÓN** `dist_vwap_pct` > `0.2077` → IC=+0.127 (n=156)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` > 0.2077 (IC base=+0.063)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.521` → IC=+0.149 (n=112)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 5.521 (IC base=+0.063)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2443` → IC=+0.173 (n=53)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.2443 (IC base=+0.063)
...
### LEADLAG_BTC_XRP_15M
- **PATRÓN** `py_entrada` > `0.5` → IC=+0.120 (n=727)

  - _Acción_: Kelly boost +0.60€ cuando `py_entrada` > 0.5 (IC base=+0.107)

- **PATRÓN** `libro_liquidez` > `2925.1106` → IC=+0.171 (n=247)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 2925.1106 (IC base=+0.107)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.120 (n=517)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.037 (n=279)

- **FILTRO** `restante_min` < `3.82` → IC=-0.121 (n=259)
...
- **FILTRO** `py_entrada` < `0.49` → IC=-0.176 (n=66)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=7629)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.167 (n=3642)

  - _Acción_: SKIP cuando `py_entrada` < 0.475
  - _Potencial_: sin este filtro IC_bueno=+0.057 (n=11062)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.165 (n=3774)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.035 (n=11446)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.46` → IC=-0.209 (n=627)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.100 (n=1930)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.48` → IC=-0.186 (n=651)

  - _Acción_: SKIP cuando `py_entrada` < 0.48
  - _Potencial_: sin este filtro IC_bueno=+0.104 (n=1957)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.206 (n=658)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.063 (n=2072)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.485` → IC=-0.177 (n=617)

  - _Acción_: SKIP cuando `py_entrada` < 0.485
  - _Potencial_: sin este filtro IC_bueno=+0.083 (n=1930)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.175 (n=660)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=2075)

### MOMENTUM_IBS_15M_FADE
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=2825)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.205 (n=751)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=2272)

- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)

  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=3002)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.130 (n=10401)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=23144)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.274 (n=8255)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=25290)

- **FILTRO** `ibs_7min` < `0.279` → IC=-0.235 (n=8386)

  - _Acción_: SKIP cuando `ibs_7min` < 0.279
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=25159)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.156 (n=11375)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=22170)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.230 (n=10330)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=31863)

- **FILTRO** `ibs_7min` > `0.2922` → IC=-0.177 (n=10546)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2922
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=31647)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.138 (n=1694)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3869)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.311 (n=1323)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4240)

- **FILTRO** `ibs_7min` < `0.7105` → IC=-0.251 (n=1835)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7105
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=3728)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.186 (n=1241)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=4322)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.258 (n=1787)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.004 (n=5470)

- **FILTRO** `drift_7min_pct` |x|> `0.1112` → IC=-0.125 (n=2467)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1112
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=4790)

- **FILTRO** `ibs_7min` > `0.7869` → IC=-0.207 (n=1814)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7869
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=5443)
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.250 (n=1406)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=4380)

- **FILTRO** `ibs_7min` < `0.7468` → IC=-0.193 (n=1444)
...
- **FILTRO** `py_entrada` > `0.66` → IC=-0.264 (n=1350)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=4522)

- **FILTRO** `ibs_7min` > `0.2609` → IC=-0.179 (n=1464)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2609
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=4408)

- **FILTRO** `ballena_activa_n` > `153.0` → IC=-0.182 (n=1466)

  - _Acción_: SKIP cuando `ballena_activa_n` > 153.0
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4406)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.166 (n=1499)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.082 (n=3788)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.304 (n=1308)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=3979)

- **FILTRO** `ibs_7min` < `0.7059` → IC=-0.244 (n=1736)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7059
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=3551)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.222 (n=1197)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4090)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.237 (n=1777)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=5989)

- **FILTRO** `ibs_7min` > `0.7474` → IC=-0.173 (n=1941)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7474
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=5825)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=1793)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=3745)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.232 (n=1642)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=3896)

- **FILTRO** `ibs_7min` < `0.7399` → IC=-0.182 (n=1384)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7399
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=4154)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.174 (n=1346)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=4192)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.259 (n=1410)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=4239)

- **FILTRO** `ibs_7min` > `0.2745` → IC=-0.178 (n=1411)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2745
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=4238)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.184 (n=1380)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4269)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.257 (n=1444)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=4387)

- **FILTRO** `ibs_7min` < `0.2955` → IC=-0.235 (n=1457)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2955
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=4374)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.178 (n=1903)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.023 (n=6130)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.254 (n=1812)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3728)

- **FILTRO** `ibs_7min` < `0.2982` → IC=-0.225 (n=1385)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2982
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=4155)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.214 (n=1346)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=4194)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.207 (n=1799)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=5817)
...
### ORDER_FLOW_5M
- **PATRÓN** `delta_ratio` |x|> `0.3979` → IC=+0.128 (n=765)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.64€ cuando `delta_ratio` |x|> 0.3979 (IC base=+0.112)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.121 (n=689)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` > 6.0 (IC base=+0.112)

- **PATRÓN** `total_vol_5m` < `469.512` → IC=+0.143 (n=256)
...
- **PATRÓN** `delta_ratio` |x|> `0.4133` → IC=+0.188 (n=107)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.94€ cuando `delta_ratio` |x|> 0.4133 (IC base=+0.099)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.120 (n=164)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` > 4.0 (IC base=+0.099)

- **PATRÓN** `total_vol_5m` < `391.8444` → IC=+0.194 (n=70)

  - _Acción_: Kelly boost +0.97€ cuando `total_vol_5m` < 391.8444 (IC base=+0.099)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.019 (n=2986)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.199 (n=931)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0755` → IC=+0.247 (n=1676)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0755 (IC base=-0.029)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1783` → IC=+0.238 (n=1213)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1783 (IC base=-0.029)

- **PATRÓN** `ibs_15` < `0.35` → IC=+0.277 (n=1878)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **FILTRO** `sigma_h` > `0.0068` → IC=-0.224 (n=414)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.188 (n=1243)
...
- **FILTRO** `sigma_ewma_delta_pct` > `19.491` → IC=-0.252 (n=296)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.185 (n=1361)

- **FILTRO** `libro_liquidez` < `14433.2567` → IC=-0.206 (n=546)

  - _Acción_: SKIP cuando `libro_liquidez` < 14433.2567
...
- **PATRÓN** `ibs_15` < `0.2693` → IC=+0.285 (n=639)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **PATRÓN** `sigma_ewma_delta_pct` < `12.254` → IC=+0.239 (n=769)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0764` → IC=+0.235 (n=285)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0764 (IC base=-0.041)

- **PATRÓN** `ibs_15` < `0.3529` → IC=+0.270 (n=319)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **PATRÓN** `dist_vwap_pct` < `0.2005` → IC=+0.228 (n=285)
...
- **FILTRO** `drift_15min` |x|> `1.2607` → IC=-0.270 (n=198)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.175 (n=595)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1334` → IC=+0.298 (n=226)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1334 (IC base=-0.040)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1035` → IC=+0.347 (n=214)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1035 (IC base=-0.040)

- **PATRÓN** `ibs_15` < `0.3437` → IC=+0.309 (n=495)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3437 (IC base=-0.040)
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.35 sube el IC de -0.029 a +0.277 en UPDOWN_GBM_15M_TARDIO (n=1878). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.2693 sube el IC de +0.232 a +0.285 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=639). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#SOL#15min**: dentro de BUY_NO, IBS < 0.3529 sube el IC de -0.041 a +0.270 en UPDOWN_GBM_15M_TARDIO#SOL#15min (n=319). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO#XRP#15min**: dentro de BUY_NO, IBS < 0.3437 sube el IC de -0.040 a +0.309 en UPDOWN_GBM_15M_TARDIO#XRP#15min (n=495). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 28973 | -0.087 | -3879.19€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1513 | -0.041 | -225.18€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 27460 | -0.089 | -3654.01€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3800 | -0.099 | -625.61€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3800 | -0.099 | -625.61€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 1513 | -0.041 | -225.18€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 1513 | -0.041 | -225.18€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 3483 | -0.097 | -789.03€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 3483 | -0.097 | -789.03€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 7430 | -0.020 | -702.28€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 7430 | -0.020 | -702.28€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 7048 | -0.090 | -446.66€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 7048 | -0.090 | -446.66€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 5699 | -0.168 | -1090.43€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 5699 | -0.168 | -1090.43€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 19099 | -0.027 | +3965.23€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 4975 | +0.000 | +1816.92€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 14124 | -0.037 | +2148.31€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 19099 | -0.027 | +3965.23€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 4975 | +0.000 | +1816.92€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 14124 | -0.037 | +2148.31€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 95719 | +0.112 | -4789.30€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 14362 | +0.185 | -436.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 385 | -0.076 | -49.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 74791 | +0.100 | -4081.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 6181 | +0.107 | -221.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 12437 | +0.098 | -1036.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 46 | -0.167 | -1.52€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 12376 | +0.100 | -1023.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 19365 | +0.132 | -377.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 4546 | +0.203 | -141.13€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 12400 | +0.112 | -191.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 2377 | +0.102 | -22.46€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 12475 | +0.090 | -1139.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 53 | -0.100 | -7.49€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#DOGE#240min | 15 | -0.243 | -11.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 12407 | +0.091 | -1120.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 20337 | +0.123 | -392.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 5553 | +0.175 | -78.85€ | 1 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 12534 | +0.105 | -247.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 2238 | +0.098 | -57.62€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 18656 | +0.113 | -1095.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 4117 | +0.189 | -215.19€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 288 | -0.035 | +4.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 12685 | +0.091 | -743.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1566 | +0.126 | -141.79€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 12449 | +0.100 | -747.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 47 | -0.031 | +8.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 13 | -0.022 | -0.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 12389 | +0.101 | -755.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 15179 | +0.191 | -1015.36€ | 1 | 5 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 15179 | +0.191 | -1015.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 3604 | +0.167 | -381.28€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 3604 | +0.167 | -381.28€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC | 1314 | +0.194 | -19.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC#15min | 1314 | +0.194 | -19.32€ | 4 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 3558 | +0.179 | -310.28€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 3558 | +0.179 | -310.28€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 3141 | +0.239 | -104.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 3141 | +0.239 | -104.48€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 3483 | +0.192 | -213.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 3483 | +0.192 | -213.76€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 52399 | +0.197 | -4115.16€ | 3 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 52399 | +0.197 | -4115.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 