# Hipótesis automáticas — 2026-07-18 23:58 UTC
_Generado por shadow_postmortem.py sobre 21531 resoluciones (PNL=+5161.50€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.195 (n=1206)
...
- **PATRÓN** `py_entrada` < `0.265` → IC=+0.343 (n=481)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.265 (IC base=+0.174)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.180 (n=1589)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.206 (n=107)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` < `9.0` → IC=+0.291 (n=41)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 9.0 (IC base=+0.143)

- **PATRÓN** `py_entrada` < `0.395` → IC=+0.233 (n=43)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.395 (IC base=+0.143)

- **PATRÓN** `libro_liquidez` > `6436.3724` → IC=+0.159 (n=83)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 6436.3724 (IC base=+0.143)

### FAVORITO_CONFIRMADO#ETH#15min
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.235 (n=266)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.715` → IC=+0.352 (n=133)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.715 (IC base=+0.224)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.225 (n=362)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.224)

- **PATRÓN** `libro_liquidez` > `4419.7043` → IC=+0.233 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4419.7043 (IC base=+0.224)
...
- **PATRÓN** `py_entrada` < `0.405` → IC=+0.187 (n=65)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `py_entrada` < 0.405 (IC base=+0.134)

- **PATRÓN** `libro_liquidez` > `3899.2794` → IC=+0.171 (n=83)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 3899.2794 (IC base=+0.134)

### FAVORITO_CONFIRMADO#SOL#15min
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.252 (n=103)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.665` → IC=+0.297 (n=200)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.665 (IC base=+0.232)

- **PATRÓN** `libro_liquidez` > `1984.229` → IC=+0.238 (n=128)
...
- **PATRÓN** `py_entrada` < `0.395` → IC=+0.267 (n=298)
...
- **PATRÓN** `dist_vwap_pct` > `0.7467` → IC=+0.138 (n=139)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 0.7467 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.024` → IC=+0.221 (n=163)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.024 (IC base=+0.114)

- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.132 (n=1429)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0089 (IC base=+0.112)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.133 (n=1533)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` < 12.0 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `0.6698` → IC=+0.134 (n=159)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` > 0.6698 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` < `0.9151` → IC=+0.122 (n=1061)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.9151 (IC base=+0.112)
...
- **PATRÓN** `dist_vwap_pct` > `0.136` → IC=+0.146 (n=63)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.136 (IC base=+0.073)

- **PATRÓN** `dist_vwap_pct` < `0.5188` → IC=+0.128 (n=216)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.5188 (IC base=+0.073)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.845` → IC=+0.191 (n=95)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 2.845 (IC base=+0.073)

### GBM_LATE_15M#ETH#15min
- **PATRÓN** `sigma_h` < `0.0045` → IC=+0.167 (n=136)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0045 (IC base=+0.097)

- **PATRÓN** `dist_vwap_pct` > `0.8499` → IC=+0.150 (n=18)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.8499 (IC base=+0.097)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.301` → IC=+0.208 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.301 (IC base=+0.097)

- **PATRÓN** `dist_vwap_pct` > `0.6921` → IC=+0.172 (n=56)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.6921 (IC base=+0.077)
...
- **PATRÓN** `sigma_h` < `0.0101` → IC=+0.122 (n=252)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` < 0.0101 (IC base=+0.103)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.145 (n=201)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 18.0 (IC base=+0.103)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.326` → IC=+0.206 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.326 (IC base=+0.103)

- **PATRÓN** `sigma_h` < `0.013` → IC=+0.141 (n=469)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.013 (IC base=+0.124)

- **PATRÓN** `sigma_h` > `0.009` → IC=+0.127 (n=531)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.63€ cuando `sigma_h` > 0.009 (IC base=+0.124)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.170 (n=386)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 12.0 (IC base=+0.124)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.755` → IC=+0.121 (n=196)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` < 2.755 (IC base=+0.124)

### GBM_LATE_15M#XRP#15min
- **PATRÓN** `sigma_h` < `0.0286` → IC=+0.153 (n=542)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` < 0.0286 (IC base=+0.152)

- **PATRÓN** `sigma_h` > `0.0217` → IC=+0.185 (n=246)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0217 (IC base=+0.152)
...
- **PATRÓN** `dist_vwap_pct` < `0.1211` → IC=+0.142 (n=199)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.1211 (IC base=+0.152)
...
- **PATRÓN** `sigma_h` < `0.0099` → IC=+0.196 (n=189)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0099 (IC base=+0.169)

- **PATRÓN** `sigma_h` > `0.0257` → IC=+0.202 (n=189)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0257 (IC base=+0.169)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.226 (n=206)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` > `0.3105` → IC=+0.179 (n=82)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.3105 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` < `0.5914` → IC=+0.161 (n=308)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.5914 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.696` → IC=+0.166 (n=276)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` < 7.696 (IC base=+0.169)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0117` → IC=+0.238 (n=346)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.1758` → IC=+0.209 (n=383)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1758 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.022` → IC=+0.300 (n=153)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.022 (IC base=+0.165)

- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.188 (n=1018)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.8834` → IC=+0.187 (n=928)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.8834 (IC base=+0.170)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.048` → IC=+0.181 (n=635)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.048 (IC base=+0.170)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.139 (n=167)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0041 (IC base=+0.131)

- **PATRÓN** `sigma_h` > `0.003` → IC=+0.147 (n=222)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.003 (IC base=+0.131)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.180 (n=170)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 13.0 (IC base=+0.131)

- **PATRÓN** `dist_vwap_pct` > `0.3666` → IC=+0.188 (n=46)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.3666 (IC base=+0.131)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.142` → IC=+0.250 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.142 (IC base=+0.131)

- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.136 (n=259)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0061 (IC base=+0.136)

- **PATRÓN** `sigma_h` > `0.0027` → IC=+0.162 (n=258)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` > 0.0027 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.177 (n=131)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 15.0 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.1203` → IC=+0.147 (n=66)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.1203 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `0.7143` → IC=+0.130 (n=228)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.7143 (IC base=+0.136)
...
- **PATRÓN** `dist_vwap_pct` > `0.5531` → IC=+0.192 (n=63)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.5531 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` < `0.8871` → IC=+0.146 (n=210)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.8871 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.551` → IC=+0.274 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.551 (IC base=+0.148)

- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.140 (n=234)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0067 (IC base=+0.128)

- **PATRÓN** `sigma_h` > `0.0036` → IC=+0.137 (n=265)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0036 (IC base=+0.128)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.163 (n=93)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.6406` → IC=+0.224 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6406 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` < `1.1468` → IC=+0.144 (n=248)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 1.1468 (IC base=+0.128)
...
- **PATRÓN** `hora_utc` > `13.0` → IC=+0.153 (n=191)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 13.0 (IC base=+0.133)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.143 (n=96)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` < 6.0 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` > `0.6216` → IC=+0.184 (n=55)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.6216 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.566` → IC=+0.277 (n=110)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.566 (IC base=+0.133)

- **PATRÓN** `sigma_h` < `0.0122` → IC=+0.155 (n=247)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0122 (IC base=+0.119)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.123 (n=120)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 15.0 (IC base=+0.119)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.171 (n=83)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 5.0 (IC base=+0.119)

- **PATRÓN** `dist_vwap_pct` < `0.7969` → IC=+0.162 (n=205)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.7969 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.606` → IC=+0.185 (n=163)

  - _Acción_: Kelly boost +0.92€ cuando `sigma_ewma_delta_pct` < 2.606 (IC base=+0.119)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` < `0.0172` → IC=+0.266 (n=237)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0172 (IC base=+0.257)
...
- **PATRÓN** `dist_vwap_pct` > `0.3142` → IC=+0.333 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3142 (IC base=+0.299)

- **PATRÓN** `dist_vwap_pct` < `0.5721` → IC=+0.310 (n=246)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5721 (IC base=+0.299)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.458` → IC=+0.317 (n=217)
...
- **PATRÓN** `sigma_h` > `0.0137` → IC=+0.198 (n=319)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0137 (IC base=+0.128)
...
- **PATRÓN** `dist_vwap_pct` > `0.7668` → IC=+0.179 (n=138)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.7668 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` < `0.1236` → IC=+0.154 (n=483)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.1236 (IC base=+0.128)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.343` → IC=+0.286 (n=129)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.343 (IC base=+0.128)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.127 (n=387)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 6.0 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` < `0.7895` → IC=+0.126 (n=921)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` < 0.7895 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.488` → IC=+0.127 (n=239)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` > 3.488 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.972` → IC=+0.121 (n=819)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.134 (n=91)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 17.0 (IC base=+0.080)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.888` → IC=+0.153 (n=96)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 2.888 (IC base=+0.080)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.821` → IC=+0.198 (n=51)
  ...
  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` > 7.821 (IC base=+0.044)
...
- **PATRÓN** `sigma_h` > `0.0123` → IC=+0.126 (n=89)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.63€ cuando `sigma_h` > 0.0123 (IC base=+0.082)

- **PATRÓN** `hora_utc` < `9.0` → IC=+0.143 (n=96)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` < 9.0 (IC base=+0.082)

- **PATRÓN** `dist_vwap_pct` < `0.1555` → IC=+0.136 (n=86)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.1555 (IC base=+0.082)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.184` → IC=+0.269 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.184 (IC base=+0.082)

- **PATRÓN** `sigma_h` < `0.009` → IC=+0.124 (n=115)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` < 0.009 (IC base=+0.042)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.121 (n=93)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` < 6.0 (IC base=+0.042)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` < `0.0287` → IC=+0.212 (n=300)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0287 (IC base=+0.208)

- **PATRÓN** `sigma_h` > `0.0151` → IC=+0.247 (n=200)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0151 (IC base=+0.208)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.234 (n=107)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.208)

- **PATRÓN** `dist_vwap_pct` > `0.2498` → IC=+0.260 (n=94)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2498 (IC base=+0.208)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.884` → IC=+0.283 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.884 (IC base=+0.208)

- **PATRÓN** `sigma_h` < `0.0095` → IC=+0.233 (n=144)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0095 (IC base=+0.216)

- **PATRÓN** `sigma_h` > `0.0172` → IC=+0.240 (n=148)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0172 (IC base=+0.216)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.237 (n=116)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.216)

- **PATRÓN** `dist_vwap_pct` > `0.2943` → IC=+0.225 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2943 (IC base=+0.216)

- **PATRÓN** `dist_vwap_pct` < `0.5513` → IC=+0.215 (n=296)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5513 (IC base=+0.216)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.154` → IC=+0.224 (n=277)
...
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#DOGE#15min` — IC=+0.271 n=33. Faltan ~7 resoluciones para umbral n≥40. ETA: ~5h.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#DOGE` — IC=+0.271 n=33. Faltan ~7 resoluciones para umbral n≥40. ETA: ~5h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 243 | +0.133 | +10.61€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min |