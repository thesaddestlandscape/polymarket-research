# Hipótesis automáticas — 2026-09-03 21:58 UTC
_Generado por shadow_postmortem.py sobre 271764 resoluciones (PNL=+26490.00€)_
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.221 (n=886)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.210 (n=1082)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.145 (n=1627)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.313 (n=538)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.196 (n=6118)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.202 (n=2920)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.248 (n=1389)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.315 (n=496)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.196 (n=1506)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.194 (n=7295)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.206 (n=17294)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.71 (IC base=+0.188)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.149 (n=3369)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.175 (n=2539)

  - _Acción_: Kelly boost +0.87€ cuando `py_entrada` > 0.72 (IC base=+0.146)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.234 (n=1119)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.260 (n=2202)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.226)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.175 (n=1092)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.178 (n=2981)

  - _Acción_: Kelly boost +0.89€ cuando `py_entrada` > 0.71 (IC base=+0.161)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.231 (n=1523)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.191 (n=1236)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.131 (n=3465)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3577` → IC=+0.128 (n=3636)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` > 0.3577 (IC base=+0.081)

- **PATRÓN** `dist_vwap_pct` > `0.269` → IC=+0.226 (n=468)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.269 (IC base=+0.081)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.577` → IC=+0.151 (n=1392)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 5.577 (IC base=+0.081)

- **PATRÓN** `volumen_regimen` < `0.627` → IC=+0.223 (n=276)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.627 (IC base=+0.081)

- **PATRÓN** `volumen_regimen` > `1.0858` → IC=+0.248 (n=375)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0858 (IC base=+0.081)

- **PATRÓN** `volumen_pendiente_norm` > `0.1083` → IC=+0.174 (n=803)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.1083 (IC base=+0.081)

- **PATRÓN** `volumen_spike_ratio` < `2.8819` → IC=+0.175 (n=2170)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.8819 (IC base=+0.081)

- **PATRÓN** `volumen_spike_ratio` > `1.4772` → IC=+0.170 (n=2170)
...
- **PATRÓN** `ibs_20min` < `0.4059` → IC=+0.127 (n=3498)
...
- **PATRÓN** `ballena_activa_n` < `125.0` → IC=+0.212 (n=1392)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 125.0 (IC base=+0.041)
...
- **PATRÓN** `ibs_20min` < `0.4038` → IC=+0.284 (n=474)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0672` → IC=+0.262 (n=385)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0672 (IC base=+0.260)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5873` → IC=+0.284 (n=383)
...
- **PATRÓN** `libro_liquidez` > `1702.02` → IC=+0.281 (n=359)
...
- **PATRÓN** `ibs_20min` < `0.0385` → IC=+0.298 (n=206)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0385 (IC base=+0.239)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7033` → IC=+0.267 (n=165)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7033 (IC base=+0.239)

- **PATRÓN** `volumen_spike_ratio` > `2.4461` → IC=+0.220 (n=330)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.147 (n=168)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=538)
...
- **PATRÓN** `dist_vwap_pct` > `0.3632` → IC=+0.288 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3632 (IC base=-0.035)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.11` → IC=+0.278 (n=43)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.11 (IC base=-0.035)

- **PATRÓN** `volumen_spike_ratio` < `1.4384` → IC=+0.352 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4384 (IC base=-0.035)

- **PATRÓN** `volumen_spike_ratio` > `1.9251` → IC=+0.280 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9251 (IC base=-0.035)

- **PATRÓN** `ballena_activa_n` < `168.0` → IC=+0.273 (n=64)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.313` → IC=-0.193 (n=203)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=1552)
...
- **PATRÓN** `dist_vwap_pct` < `0.2666` → IC=+0.214 (n=211)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2666 (IC base=-0.049)

- **PATRÓN** `volumen_regimen` > `1.3414` → IC=+0.257 (n=68)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.3414 (IC base=-0.049)

- **PATRÓN** `volumen_pendiente_norm` > `0.0976` → IC=+0.204 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0976 (IC base=-0.049)

- **PATRÓN** `volumen_spike_ratio` < `2.339` → IC=+0.167 (n=88)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.339 (IC base=-0.049)

- **PATRÓN** `volumen_spike_ratio` > `1.9623` → IC=+0.147 (n=66)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.9623 (IC base=-0.049)
...
- **PATRÓN** `ibs_20min` > `0.2901` → IC=+0.140 (n=4494)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` > 0.2901 (IC base=+0.071)

- **PATRÓN** `dist_vwap_pct` > `1.2175` → IC=+0.298 (n=335)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2175 (IC base=+0.071)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.469` → IC=+0.122 (n=2371)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` > 2.469 (IC base=+0.071)

- **PATRÓN** `volumen_regimen` > `0.6814` → IC=+0.219 (n=1302)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6814 (IC base=+0.071)

- **PATRÓN** `volumen_pendiente_norm` < `0.0871` → IC=+0.215 (n=2024)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0871 (IC base=+0.071)

- **PATRÓN** `volumen_pendiente_norm` > `0.2532` → IC=+0.231 (n=425)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2532 (IC base=+0.071)

- **PATRÓN** `volumen_spike_ratio` < `1.4927` → IC=+0.236 (n=726)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4927 (IC base=+0.071)

- **PATRÓN** `volumen_spike_ratio` > `2.837` → IC=+0.227 (n=726)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.837 (IC base=+0.071)

- **PATRÓN** `ballena_activa_n` < `94.0` → IC=+0.299 (n=1632)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 94.0 (IC base=+0.071)

- **PATRÓN** `ibs_20min` < `0.5833` → IC=+0.131 (n=4357)
...
- **PATRÓN** `volumen_spike_ratio` > `2.4256` → IC=+0.272 (n=576)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4256 (IC base=+0.052)
...
- **FILTRO** `ibs_20min` < `0.2908` → IC=-0.152 (n=372)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=757)
...
- **PATRÓN** `ibs_20min` > `0.8238` → IC=+0.223 (n=283)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8238 (IC base=+0.020)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.053` → IC=+0.127 (n=422)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 2.053 (IC base=+0.020)
...
- **PATRÓN** `volumen_spike_ratio` < `1.9482` → IC=+0.227 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9482 (IC base=+0.020)

- **PATRÓN** `volumen_spike_ratio` > `2.7084` → IC=+0.285 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7084 (IC base=+0.020)
...
- **PATRÓN** `volumen_spike_ratio` < `3.4804` → IC=+0.312 (n=413)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.4804 (IC base=+0.310)

- **PATRÓN** `volumen_spike_ratio` > `2.3984` → IC=+0.320 (n=275)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3984 (IC base=+0.310)

- **PATRÓN** `libro_liquidez` > `1843.4629` → IC=+0.325 (n=169)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1843.4629 (IC base=+0.310)
...
- **FILTRO** `ibs_20min` < `0.7725` → IC=-0.140 (n=478)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.254 (n=282)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.8982` → IC=+0.325 (n=488)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8982 (IC base=+0.253)

- **PATRÓN** `dist_vwap_pct` > `1.3483` → IC=+0.353 (n=161)
...
- **PATRÓN** `volumen_regimen` > `0.6814` → IC=+0.276 (n=654)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6814 (IC base=+0.253)

- **PATRÓN** `volumen_pendiente_norm` > `0.2413` → IC=+0.292 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2413 (IC base=+0.253)

- **PATRÓN** `volumen_spike_ratio` < `2.5754` → IC=+0.261 (n=672)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5754 (IC base=+0.253)
...
- **PATRÓN** `libro_liquidez` > `2468.9826` → IC=+0.253 (n=654)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2468.9826 (IC base=+0.253)
...
- **PATRÓN** `sigma_h` > `0.0106` → IC=+0.209 (n=1268)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0106 (IC base=+0.163)

- **PATRÓN** `drift_60min` |x|≤ `0.3429` → IC=+0.169 (n=3347)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.3429 (IC base=+0.163)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.169 (n=3827)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.7898` → IC=+0.240 (n=836)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7898 (IC base=+0.163)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.679` → IC=+0.236 (n=1545)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.679 (IC base=+0.163)

- **PATRÓN** `volumen_regimen` > `0.6283` → IC=+0.166 (n=2572)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 0.6283 (IC base=+0.163)

- **PATRÓN** `volumen_pendiente_norm` > `0.1061` → IC=+0.184 (n=1422)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.1061 (IC base=+0.163)

- **PATRÓN** `volumen_spike_ratio` < `2.3254` → IC=+0.163 (n=3112)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.3254 (IC base=+0.163)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.166 (n=3891)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.03 (IC base=+0.163)

- **PATRÓN** `libro_liquidez` > `3932.3168` → IC=+0.179 (n=1268)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 3932.3168 (IC base=+0.163)

- **PATRÓN** `ballena_activa_n` < `142.0` → IC=+0.180 (n=2711)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 142.0 (IC base=+0.163)

- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.190 (n=3098)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.0083 (IC base=+0.179)

- **PATRÓN** `drift_60min` |x|≤ `0.0792` → IC=+0.212 (n=1174)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0792 (IC base=+0.179)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.197 (n=1715)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.4387` → IC=+0.231 (n=3520)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4159` → IC=+0.175 (n=562)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.4159 (IC base=+0.168)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.196 (n=576)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 6.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` > `0.4793` → IC=+0.211 (n=562)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4793 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` > `0.2046` → IC=+0.225 (n=409)
...
- **PATRÓN** `volumen_regimen` > `1.0731` → IC=+0.185 (n=255)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` > 1.0731 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.2311` → IC=+0.200 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2311 (IC base=+0.168)
...
- **PATRÓN** `volumen_spike_ratio` > `2.4598` → IC=+0.167 (n=178)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 2.4598 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `11309.5324` → IC=+0.191 (n=502)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 11309.5324 (IC base=+0.168)
...
- **PATRÓN** `sigma_h` < `0.0102` → IC=+0.255 (n=460)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0102 (IC base=+0.235)

- **PATRÓN** `drift_60min` |x|≤ `0.2231` → IC=+0.257 (n=307)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2231 (IC base=+0.235)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.253 (n=318)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.235)

- **PATRÓN** `ibs_20min` < `0.3798` → IC=+0.268 (n=460)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6637` → IC=+0.233 (n=129)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6637 (IC base=+0.235)

- **PATRÓN** `volumen_spike_ratio` > `2.364` → IC=+0.222 (n=257)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1283` → IC=+0.177 (n=249)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.89€ cuando `drift_60min` |x|≤ 0.1283 (IC base=+0.159)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.173 (n=580)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 6.0 (IC base=+0.159)

- **PATRÓN** `ibs_20min` > `0.4382` → IC=+0.203 (n=566)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4382 (IC base=+0.159)

- **PATRÓN** `dist_vwap_pct` > `0.923` → IC=+0.213 (n=134)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4303` → IC=+0.154 (n=183)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 1.4303 (IC base=+0.159)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.160 (n=630)

  - _Acción_: Kelly boost +0.80€ cuando `libro_spread` < 0.01 (IC base=+0.159)

- **PATRÓN** `libro_liquidez` > `8738.117` → IC=+0.191 (n=377)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 8738.117 (IC base=+0.159)

- **PATRÓN** `ballena_activa_n` < `134.0` → IC=+0.147 (n=335)

  - _Acción_: Kelly boost +0.73€ cuando `ballena_activa_n` < 134.0 (IC base=+0.159)
...
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.151 (n=293)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0094 (IC base=+0.133)

- **PATRÓN** `drift_60min` |x|≤ `0.5231` → IC=+0.137 (n=646)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.69€ cuando `drift_60min` |x|≤ 0.5231 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.189 (n=300)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.2523` → IC=+0.148 (n=612)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.2523 (IC base=+0.133)
...
- **PATRÓN** `volumen_regimen` < `1.1954` → IC=+0.151 (n=646)
  ...
  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1954 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` > `0.8568` → IC=+0.150 (n=430)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 0.8568 (IC base=+0.133)

- **PATRÓN** `volumen_pendiente_norm` > `0.2708` → IC=+0.232 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2708 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` > `2.3492` → IC=+0.205 (n=171)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3492 (IC base=+0.133)

- **PATRÓN** `libro_liquidez` > `3185.27` → IC=+0.177 (n=215)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 3185.27 (IC base=+0.133)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0896` → IC=+0.243 (n=247)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0896 (IC base=+0.213)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.223 (n=334)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.4102` → IC=+0.259 (n=736)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4102 (IC base=+0.213)

- **PATRÓN** `dist_vwap_pct` < `0.2661` → IC=+0.219 (n=756)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2661 (IC base=+0.213)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.814` → IC=+0.272 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.814 (IC base=+0.213)

- **PATRÓN** `volumen_regimen` > `0.6917` → IC=+0.236 (n=657)
...
- **PATRÓN** `hora_utc` > `9.0` → IC=+0.169 (n=705)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 9.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` > `0.476` → IC=+0.169 (n=790)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` > 0.476 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.933` → IC=+0.224 (n=103)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.933 (IC base=+0.136)
...
- **PATRÓN** `volumen_regimen` < `0.8754` → IC=+0.158 (n=425)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.8754 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` > `1.1816` → IC=+0.156 (n=213)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 1.1816 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.18` → IC=+0.179 (n=219)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.18 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` < `2.2818` → IC=+0.148 (n=646)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 2.2818 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.8649` → IC=+0.141 (n=489)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 1.8649 (IC base=+0.136)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.154 (n=590)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `6959.6494` → IC=+0.175 (n=263)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 6959.6494 (IC base=+0.136)
...
- **PATRÓN** `sigma_h` < `0.0052` → IC=+0.265 (n=130)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0052 (IC base=+0.263)

- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.306 (n=65)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0076 (IC base=+0.263)

- **PATRÓN** `drift_60min` |x|≤ `0.0967` → IC=+0.261 (n=65)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0967 (IC base=+0.263)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.292 (n=195)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.263)

- **PATRÓN** `ibs_20min` > `0.7759` → IC=+0.311 (n=130)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7759 (IC base=+0.263)

- **PATRÓN** `dist_vwap_pct` > `0.1787` → IC=+0.287 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1787 (IC base=+0.263)

- **PATRÓN** `dist_vwap_pct` < `0.4447` → IC=+0.261 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4447 (IC base=+0.263)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.256` → IC=+0.326 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.256 (IC base=+0.263)

- **PATRÓN** `volumen_regimen` < `0.8463` → IC=+0.273 (n=130)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8463 (IC base=+0.263)

- **PATRÓN** `volumen_regimen` > `1.1833` → IC=+0.306 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1833 (IC base=+0.263)

- **PATRÓN** `volumen_pendiente_norm` > `0.0976` → IC=+0.355 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0976 (IC base=+0.263)

- **PATRÓN** `volumen_spike_ratio` < `1.3845` → IC=+0.303 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3845 (IC base=+0.263)

- **PATRÓN** `volumen_spike_ratio` > `2.0848` → IC=+0.320 (n=87)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.0848 (IC base=+0.263)

- **PATRÓN** `libro_liquidez` > `2746.5037` → IC=+0.261 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2746.5037 (IC base=+0.263)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.202 (n=2061)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0087 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.172 (n=4573)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `1.0626` → IC=+0.223 (n=665)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0626 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.635` → IC=+0.224 (n=2562)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.635 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` < `0.8867` → IC=+0.159 (n=2100)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.8867 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` > `1.0875` → IC=+0.154 (n=1428)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.0875 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` > `0.1675` → IC=+0.190 (n=1234)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.1675 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `1.8748` → IC=+0.173 (n=2814)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.8748 (IC base=+0.165)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.167 (n=4303)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.02 (IC base=+0.165)

- **PATRÓN** `libro_liquidez` > `3840.5914` → IC=+0.188 (n=1516)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3840.5914 (IC base=+0.165)

- **PATRÓN** `ballena_activa_n` < `60.0` → IC=+0.212 (n=2117)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 60.0 (IC base=+0.165)

- **PATRÓN** `sigma_h` < `0.0108` → IC=+0.198 (n=4197)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0108 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.4815` → IC=+0.197 (n=4195)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.4815 (IC base=+0.189)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.206 (n=1866)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.189)

- **PATRÓN** `ibs_20min` < `0.5628` → IC=+0.242 (n=4195)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.576` → IC=+0.190 (n=4132)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` < 3.576 (IC base=+0.189)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3087` → IC=+0.204 (n=1581)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3087 (IC base=+0.189)

- **PATRÓN** `ballena_activa_n` < `139.0` → IC=+0.183 (n=3067)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 139.0 (IC base=+0.189)
...
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.276 (n=485)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0067 (IC base=+0.271)

- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.279 (n=549)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0046 (IC base=+0.271)

- **PATRÓN** `drift_60min` |x|≤ `0.1152` → IC=+0.300 (n=243)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1152 (IC base=+0.271)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.280 (n=503)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.4087` → IC=+0.307 (n=484)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5076` → IC=+0.292 (n=446)
...
- **PATRÓN** `libro_liquidez` > `1456.1771` → IC=+0.279 (n=491)
...
- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.164 (n=248)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.0069 (IC base=+0.161)

- **PATRÓN** `drift_60min` |x|≤ `0.0966` → IC=+0.169 (n=249)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.0966 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.184 (n=666)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 8.0 (IC base=+0.161)

- **PATRÓN** `ibs_20min` > `0.3271` → IC=+0.206 (n=744)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3271 (IC base=+0.161)

- **PATRÓN** `dist_vwap_pct` > `0.2359` → IC=+0.222 (n=429)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2359 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.699` → IC=+0.190 (n=182)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 9.699 (IC base=+0.161)
...
- **PATRÓN** `volumen_regimen` < `1.2687` → IC=+0.167 (n=745)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.2687 (IC base=+0.161)

- **PATRÓN** `volumen_regimen` > `1.1034` → IC=+0.171 (n=338)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 1.1034 (IC base=+0.161)

- **PATRÓN** `volumen_pendiente_norm` < `0.0723` → IC=+0.175 (n=622)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` < 0.0723 (IC base=+0.161)

- **PATRÓN** `volumen_pendiente_norm` > `0.1475` → IC=+0.191 (n=205)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.1475 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` < `2.413` → IC=+0.179 (n=695)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 2.413 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` > `1.7228` → IC=+0.182 (n=463)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 1.7228 (IC base=+0.161)

- **PATRÓN** `libro_liquidez` > `10634.2635` → IC=+0.187 (n=665)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 10634.2635 (IC base=+0.161)
...
- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.247 (n=298)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0088 (IC base=+0.243)

- **PATRÓN** `drift_60min` |x|≤ `0.4734` → IC=+0.242 (n=658)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4074` → IC=+0.297 (n=579)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4074 (IC base=+0.243)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.453` → IC=+0.298 (n=201)
...
- **PATRÓN** `volumen_spike_ratio` < `3.0442` → IC=+0.248 (n=470)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.158 (n=680)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 8.0 (IC base=+0.144)

- **PATRÓN** `ibs_20min` > `0.7425` → IC=+0.239 (n=500)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7425 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `0.377` → IC=+0.189 (n=303)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.377 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.376` → IC=+0.173 (n=344)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 4.376 (IC base=+0.144)
...
- **PATRÓN** `volumen_regimen` > `1.2091` → IC=+0.163 (n=250)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 1.2091 (IC base=+0.144)

- **PATRÓN** `volumen_pendiente_norm` > `0.2765` → IC=+0.231 (n=117)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2765 (IC base=+0.144)

- **PATRÓN** `volumen_spike_ratio` > `2.1579` → IC=+0.205 (n=323)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1579 (IC base=+0.144)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.145 (n=815)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.137 (n=535)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 12.0 (IC base=+0.094)

- **PATRÓN** `ibs_20min` > `0.48` → IC=+0.181 (n=788)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` > 0.48 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` > `0.8753` → IC=+0.192 (n=209)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.8753 (IC base=+0.094)
...
- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.139 (n=568)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.211 (n=913)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1688` → IC=+0.253 (n=525)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.176 (n=1437)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=4317)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.185 (n=1466)
...
- **FILTRO** `ibs_20min` < `0.7325` → IC=-0.210 (n=219)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7325
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=660)
...
- **FILTRO** `ballena_activa_n` > `60.0` → IC=-0.150 (n=238)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=717)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=35)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=920)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.45` → IC=-0.202 (n=303)

  - _Acción_: SKIP cuando `py_entrada` < 0.45
  - _Potencial_: sin este filtro IC_bueno=+0.041 (n=632)
...
- **FILTRO** `py_entrada` < `0.495` → IC=-0.143 (n=261)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.024 (n=708)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.201 (n=249)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=2991)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=11072)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.282 (n=3463)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=10600)

- **FILTRO** `ibs_7min` < `0.7117` → IC=-0.247 (n=3514)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7117
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=10549)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.170 (n=4709)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=9354)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.219 (n=4360)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=13343)

- **FILTRO** `ibs_7min` > `0.7143` → IC=-0.172 (n=4418)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=13285)
...
- **FILTRO** `py_entrada` > `0.52` → IC=-0.141 (n=2051)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.076 (n=1028)
...
- **FILTRO** `ibs_7min` > `0.8349` → IC=-0.191 (n=769)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8349
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=2310)
...
- **FILTRO** `ballena_activa_n` > `107.0` → IC=-0.176 (n=887)

  - _Acción_: SKIP cuando `ballena_activa_n` > 107.0
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=1751)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.225 (n=739)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=2437)

- **FILTRO** `ibs_7min` > `0.799` → IC=-0.170 (n=793)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.138 (n=717)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.091 (n=1667)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.258 (n=584)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=1800)

- **FILTRO** `ibs_7min` < `0.7509` → IC=-0.196 (n=596)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7509
  - _Potencial_: sin este filtro IC_bueno=-0.075 (n=1788)
...
- **FILTRO** `ibs_7min` > `0.2759` → IC=-0.178 (n=598)
...
- **FILTRO** `ballena_activa_n` > `34.0` → IC=-0.182 (n=589)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=1807)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.178 (n=799)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=2526)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.254 (n=608)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=2481)

- **FILTRO** `ibs_7min` > `0.775` → IC=-0.163 (n=772)

  - _Acción_: SKIP cuando `ibs_7min` > 0.775
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=2317)
...
- **FILTRO** `sigma_h` < `0.0079` → IC=-0.349 (n=104)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.1678` → IC=+0.129 (n=454)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.65€ cuando `drift_60min` |x|≤ 0.1678 (IC base=+0.128)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.133 (n=516)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` > `0.5787` → IC=+0.218 (n=516)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5787 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` > `0.4298` → IC=+0.174 (n=130)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.4298 (IC base=+0.128)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.945` → IC=+0.204 (n=258)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.945 (IC base=+0.128)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.134 (n=536)

  - _Acción_: Kelly boost +0.67€ cuando `libro_spread` < 0.01 (IC base=+0.128)

- **PATRÓN** `libro_liquidez` > `3051.8123` → IC=+0.153 (n=344)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 3051.8123 (IC base=+0.128)
...
- **FILTRO** `sigma_h` < `0.005` → IC=-0.174 (n=44)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **FILTRO** `libro_liquidez` < `14339.8207` → IC=-0.196 (n=44)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1911` → IC=+0.156 (n=59)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.78€ cuando `delta_ratio_macro` |x|> 0.1911 (IC base=+0.089)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1659` → IC=+0.136 (n=42)

  - _Acción_: Kelly boost +0.68€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1659 (IC base=+0.089)

- **PATRÓN** `ibs_15` > `0.6314` → IC=+0.197 (n=130)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.98€ cuando `ibs_15` > 0.6314 (IC base=+0.089)

- **PATRÓN** `sigma_ewma_delta_pct` > `15.362` → IC=+0.133 (n=47)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` > 15.362 (IC base=+0.089)
...
- **FILTRO** `sigma_h` > `0.0107` → IC=-0.204 (n=333)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=1002)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.138 (n=343)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=992)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.299` → IC=+0.173 (n=166)

  - _Acción_: Kelly boost +0.86€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.299 (IC base=-0.068)

- **PATRÓN** `ibs_15` > `0.4875` → IC=+0.186 (n=351)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.93€ cuando `ibs_15` > 0.4875 (IC base=-0.068)
...
- **PATRÓN** `sigma_h` < `0.0032` → IC=+0.123 (n=83)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` < 0.0032 (IC base=+0.018)
...
- **FILTRO** `ibs_15` < `0.5457` → IC=-0.307 (n=55)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.213 (n=165)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0608` → IC=+0.224 (n=56)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0608 (IC base=+0.081)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2968` → IC=+0.228 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.2968 (IC base=+0.081)

- **PATRÓN** `ibs_15` > `0.5457` → IC=+0.213 (n=165)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5457 (IC base=+0.081)

- **PATRÓN** `libro_liquidez` > `10575.7678` → IC=+0.201 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 10575.7678 (IC base=+0.081)
...
### UPDOWN_GBM_IBS_ALTO
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.291 (n=329)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0067 (IC base=+0.289)

- **PATRÓN** `sigma_h` > `0.0057` → IC=+0.301 (n=149)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0057 (IC base=+0.289)
...
- **PATRÓN** `hora_utc` > `4.0` → IC=+0.316 (n=345)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.289)

- **PATRÓN** `ibs_15` > `0.8348` → IC=+0.322 (n=329)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8348 (IC base=+0.289)

- **PATRÓN** `dist_vwap_pct` > `0.306` → IC=+0.334 (n=149)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.306 (IC base=+0.289)

- **PATRÓN** `sigma_ewma_delta_pct` < `18.963` → IC=+0.292 (n=344)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 18.963 (IC base=+0.289)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.291 (n=404)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.289)

- **PATRÓN** `libro_liquidez` > `12693.279` → IC=+0.328 (n=149)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12693.279 (IC base=+0.289)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.5787 sube el IC de +0.128 a +0.218 en UPDOWN_GBM#15min (n=516). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.6314 sube el IC de +0.089 a +0.197 en UPDOWN_GBM#ETH#15min (n=130). Ya aplicado como kelly_boost=+0.98€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.4875 sube el IC de -0.068 a +0.186 en UPDOWN_GBM_15M_TARDIO (n=351). Ya aplicado como kelly_boost=+0.93€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_YES, IBS > 0.5457 sube el IC de +0.081 a +0.213 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=165). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.8348 sube el IC de +0.289 a +0.322 en UPDOWN_GBM_IBS_ALTO (n=329). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 794 | +0.087 | +43.44€ | 2 | 10 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 794 | +0.087 | +43.44€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS | 14666 | -0.111 | -2471.82€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 952 | -0.009 | -141.10€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 13714 | -0.118 | -2330.72€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1900 | -0.088 | -410.43€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1900 | -0.088 | -410.43€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 952 | -0.009 | -141.10€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 952 | -0.009 | -141.10€ | 2 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 1945 | -0.166 | -531.57€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 1945 | -0.166 | -531.57€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 3931 | -0.050 | -364.74€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 3931 | -0.050 | -364.74€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 3254 | -0.127 | -280.61€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 3254 | -0.127 | -280.61€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 2684 | -0.194 | -743.37€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 2684 | -0.194 | -743.37€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 3986 | -0.070 | +1828.05€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 1133 | -0.009 | +853.21€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 2853 | -0.094 | +974.84€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 3986 | -0.070 | +1828.05€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 1133 | -0.009 | +853.21€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 2853 | -0.094 | +974.84€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 47288 | +0.113 | -3002.66€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 7915 | +0.184 | -277.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 160 | -0.111 | -53.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 35484 | +0.098 | -2596.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3729 | +0.117 | -74.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 5961 | +0.081 | -805.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 26 | -0.107 | +5.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 14 | -0.219 | -10.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 5921 | +0.083 | -799.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 9485 | +0.132 | -230.56€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 2328 | +0.197 | -113.73€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 5860 | +0.109 | -143.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1255 | +0.126 | +48.94€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 5978 | +0.081 | -764.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 28 | +0.033 | +3.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#240min | 9 | -0.143 | -7.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 5941 | +0.082 | -760.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 10230 | +0.126 | -161.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2947 | +0.172 | -13.85€ | 1 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 5898 | +0.111 | -99.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1373 | +0.096 | -39.71€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 9675 | +0.125 | -631.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2566 | +0.192 | -160.91€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 78 | -0.013 | -3.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 5930 | +0.096 | -383.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1101 | +0.131 | -83.35€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 5959 | +0.105 | -409.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 20 | +0.000 | +1.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 5 | -0.018 | -1.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 5934 | +0.106 | -410.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 7874 | +0.177 | -610.38€ | 2 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 7874 | +0.177 | -610.38€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 1758 | +0.237 | -38.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 1758 | +0.237 | -38.12€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 1919 | +0.183 | -151.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 1919 | +0.183 | -151.63€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 25208 | +0.188 | -2399.34€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 25208 | +0.188 | -2399.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 4491 | +0.146 | -726.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 4491 | +0.146 | -726.12€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 3931 | +0.226 | -133.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 3931 | +0.226 | -133.64€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 4366 | +0.161 | -604.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 4366 | +0.161 | -604.31€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 4004 | +0.219 | -165.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 4004 | +0.219 | -165.18€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 4249 | +0.183 | -451.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 4249 | +0.183 | -451.85€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 9154 | +0.132 | +338.23€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 9154 | +0.132 | +338.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 4549 | +0.137 | +207.91€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 4549 | +0.137 | +207.91€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 4605 | +0.127 | +130.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 4605 | +0.127 | +130.32€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 13688 | +0.094 | -538.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 1234 | +0.071 | -33.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 12454 | +0.096 | -504.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC | 8260 | +0.097 | -197.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#15min | 1234 | +0.071 | -33.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#5min | 7026 | +0.101 | -164.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH | 1672 | +0.119 | +38.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH#5min | 1672 | +0.119 | +38.10€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 11842 | +0.057 | +4993.71€ | 0 | 16 |
| ✅ GBM_LATE_15M#15min | 11842 | +0.057 | +4993.71€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 1797 | +0.196 | +1316.27€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 1797 | +0.196 | +1316.27€ | 0 | 24 |
| ✅ GBM_LATE_15M#BTC | 1774 | +0.174 | +1180.00€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1774 | +0.174 | +1180.00€ | 0 | 28 |
| ✅ GBM_LATE_15M#DOGE | 1839 | +0.196 | +1343.64€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 1839 | +0.196 | +1343.64€ | 0 | 20 |
| ✅ GBM_LATE_15M#ETH | 1848 | -0.039 | +90.71€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1848 | -0.039 | +90.71€ | 3 | 11 |
| ✅ GBM_LATE_15M#SOL | 1937 | -0.048 | +463.72€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1937 | -0.048 | +463.72€ | 5 | 5 |
| ✅ GBM_LATE_15M#XRP | 2647 | -0.069 | +599.35€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 2647 | -0.069 | +599.35€ | 5 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 12588 | +0.061 | +6593.51€ | 0 | 18 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 12588 | +0.061 | +6593.51€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 2158 | -0.004 | +1689.81€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 2158 | -0.004 | +1689.81€ | 2 | 6 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 2742 | -0.022 | +406.32€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 2742 | -0.022 | +406.32€ | 1 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 1637 | +0.258 | +1622.81€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 1637 | +0.258 | +1622.81€ | 0 | 19 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1935 | -0.049 | +50.57€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1935 | -0.049 | +50.57€ | 8 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 2089 | -0.014 | +745.87€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 2089 | -0.014 | +745.87€ | 3 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 2027 | +0.266 | +2078.13€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 2027 | +0.266 | +2078.13€ | 0 | 24 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 9763 | +0.171 | +6945.70€ | 0 | 25 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 9763 | +0.171 | +6945.70€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB | 1379 | +0.199 | +1052.54€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB#15min | 1379 | +0.199 | +1052.54€ | 0 | 22 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC | 1614 | +0.160 | +1151.97€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC#15min | 1614 | +0.160 | +1151.97€ | 0 | 24 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE | 1404 | +0.199 | +1073.77€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min | 1404 | +0.199 | +1073.77€ | 0 | 20 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH | 1600 | +0.145 | +985.33€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH#15min | 1600 | +0.145 | +985.33€ | 0 | 27 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL | 1812 | +0.123 | +1132.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL#15min | 1812 | +0.123 | +1132.64€ | 0 | 21 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP | 1954 | +0.203 | +1549.45€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP#15min | 1954 | +0.203 | +1549.45€ | 0 | 20 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 2123 | +0.102 | +684.79€ | 0 | 17 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 2123 | +0.102 | +684.79€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH | 450 | +0.170 | +192.09€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH#15min | 450 | +0.170 | +192.09€ | 0 | 17 |
...
| ✅ GBM_LATE_15M_TARDIO | 11654 | +0.176 | +8461.59€ | 0 | 23 |
| ✅ GBM_LATE_15M_TARDIO#15min | 11654 | +0.176 | +8461.59€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 1726 | +0.222 | +1463.78€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 1726 | +0.222 | +1463.78€ | 0 | 22 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 1901 | +0.161 | +1355.77€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 1901 | +0.161 | +1355.77€ | 0 | 27 |
| ✅ GBM_LATE_15M_TARDIO#DOGE | 1745 | +0.224 | +1493.20€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#DOGE#15min | 1745 | +0.224 | +1493.20€ | 0 | 21 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 1797 | +0.140 | +1103.11€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 1797 | +0.140 | +1103.11€ | 0 | 26 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 2058 | +0.105 | +1122.67€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 2058 | +0.105 | +1122.67€ | 0 | 19 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 2427 | +0.207 | +1923.07€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 2427 | +0.207 | +1923.07€ | 0 | 22 |
...
| ✅ LEADLAG_BTC_XRP_15M | 541 | +0.102 | +134.07€ | 0 | 2 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 541 | +0.102 | +134.07€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 541 | +0.102 | +134.07€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 541 | +0.102 | +134.07€ | 0 | 2 |
...
| ✅ MOMENTUM_IBS_15M | 7277 | -0.003 | -92.01€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#15min | 7277 | -0.003 | -92.01€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_15M#DOGE | 1711 | +0.007 | -17.14€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#DOGE#15min | 1711 | +0.007 | -17.14€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH | 1585 | +0.001 | +1.75€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH#15min | 1585 | +0.001 | +1.75€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M#SOL | 1297 | -0.013 | -37.62€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#SOL#15min | 1297 | -0.013 | -37.62€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#XRP | 1475 | -0.005 | -31.52€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#XRP#15min | 1475 | -0.005 | -31.52€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA | 11729 | -0.032 | +611.16€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#15min | 11729 | -0.032 | +611.16€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB | 1922 | -0.021 | +320.05€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB#15min | 1922 | -0.021 | +320.05€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC | 2031 | -0.030 | -9.32€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC#15min | 2031 | -0.030 | -9.32€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE | 1944 | -0.036 | +174.04€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE#15min | 1944 | -0.036 | +174.04€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH | 1949 | -0.040 | -20.22€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH#15min | 1949 | -0.040 | -20.22€ | 1 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL | 1967 | -0.037 | +74.64€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL#15min | 1967 | -0.037 | +74.64€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP | 1916 | -0.028 | +71.96€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP#15min | 1916 | -0.028 | +71.96€ | 3 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 31766 | -0.076 | +674.08€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 31766 | -0.076 | +674.08€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 5125 | -0.088 | +443.23€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 5125 | -0.088 | +443.23€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 5267 | -0.077 | -98.97€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 5267 | -0.077 | -98.97€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 5256 | -0.080 | +173.18€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 5256 | -0.080 | +173.18€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 4780 | -0.097 | -220.28€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 4780 | -0.097 | -220.28€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 5948 | -0.053 | +111.06€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 5948 | -0.053 | +111.06€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 5390 | -0.067 | +265.86€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 5390 | -0.067 | +265.86€ | 6 | 0 |
...
| ✅ STRUCT_NO_15M | 4163 | +0.010 | -31.74€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 4163 | +0.010 | -31.74€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 1594 | +0.010 | -13.73€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 1594 | +0.010 | -13.73€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 1652 | +0.018 | -0.43€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 1652 | +0.018 | -0.43€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 917 | -0.004 | -17.59€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 917 | -0.004 | -17.59€ | 2 | 0 |
| ✅ UPDOWN_GBM | 9507 | +0.009 | +271.14€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 3358 | +0.037 | +319.33€ | 0 | 11 |
...
| ✅ UPDOWN_GBM#BTC | 1867 | +0.015 | +96.82€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 376 | +0.066 | +68.94€ | 3 | 10 |
...
| ✅ UPDOWN_GBM#ETH | 2035 | -0.000 | +14.72€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 983 | +0.022 | +35.38€ | 1 | 4 |
...
| ✅ UPDOWN_GBM#XRP | 1516 | +0.015 | +110.50€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 816 | +0.051 | +140.31€ | 0 | 9 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 5544 | -0.066 | +1267.72€ | 3 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 5544 | -0.066 | +1267.72€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 307 | -0.050 | +341.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 307 | -0.050 | +341.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 1144 | -0.155 | -59.79€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 1144 | -0.155 | -59.79€ | 5 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 80 | +0.049 | +9.82€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 80 | +0.049 | +9.82€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 514 | +0.155 | +247.63€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 514 | +0.155 | +247.63€ | 2 | 16 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 1804 | -0.063 | +366.05€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 1804 | -0.063 | +366.05€ | 4 | 6 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 1695 | -0.085 | +362.46€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 1695 | -0.085 | +362.46€ | 4 | 5 |
...
| ✅ UPDOWN_GBM_IBS_ALTO | 438 | +0.289 | +350.36€ | 0 | 11 |
| ✅ UPDOWN_GBM_IBS_ALTO#15min | 438 | +0.289 | +350.36€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.089) — sin ventaja clara. oversold(IBS<0.3): IC=+0.026 n=3384 | neutral: IC=+0.002 n=3699 | overbought(IBS>0.7): IC=+0.090 n=3719
  - _Datos_: n=11236 IC=+0.040 PNL=+1151.28€

**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  - _Umbral_: n≥40 por celda + gate riguroso completo (Wilson+shuffle+PnL bootstrap)
  - _Acción_: Añadir claves 'ESTRATEGIA#SUBTYPE#DIRECCION#HORA':1.2 a meta.hora_boost_factor, solo por celda confirmada
  - _Estado_: (cache 363s) 171 celda(s) GATE OK de 2458 trackeadas
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.067 n=151998 | tras_1loss IC=+0.051 n=119484 | tras_2loss IC=+0.015 n=53610/40 | gap=+0.052 (umbral 0.05)
...
**⏳ H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  ...
  - _Estado_: 9445 ops, 22 horas distintas. Sin hora con n≥15 y IC extremo aún.
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=9230 IC=+0.006 PNL=+217.53€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=9230 IC=+0.006 PNL=+217.53€
...
**🟡 H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.128 > 0.1 con n=687 PNL=+214.55€
  - _Datos_: n=687 IC=+0.128 PNL=+214.55€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=376 IC=+0.066 PNL=+68.94€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=376 IC=+0.066 PNL=+68.94€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=1961 IC=+0.028 PNL=+164.68€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1961 IC=+0.028 PNL=+164.68€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=2505 IC=-0.019 PNL=-49.27€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=2505 IC=-0.019 PNL=-49.27€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=2486 IC=+0.013 PNL=+97.95€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=2486 IC=+0.013 PNL=+97.95€
...
**🟡 H-CUSTOM-CROSS-WINDOW-SPREAD-POS** — GBM 15min/60min: spread positivo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.108 > 0.08 con n=164 PNL=+12.01€
  - _Datos_: n=164 IC=+0.108 PNL=+12.01€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=2303 IC=+0.029 PNL=+182.12€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2303 IC=+0.029 PNL=+182.12€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.165 > 0.1 con n=1014 PNL=+390.15€
  - _Datos_: n=1014 IC=+0.165 PNL=+390.15€
...
**🟡 H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.106 > 0.1 con n=125 PNL=+28.58€
  - _Datos_: n=125 IC=+0.106 PNL=+28.58€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=7553 IC=-0.142 PNL=+376.54€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=7553 IC=-0.142 PNL=+376.54€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.130 > 0.08 con n=649 PNL=+200.19€
  - _Datos_: n=649 IC=+0.130 PNL=+200.19€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=2161 IC=+0.138 PNL=+1251.11€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=2161 IC=+0.138 PNL=+1251.11€
...
**〰️ H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: n=939 IC=-0.010 PNL=+85.60€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=939 IC=-0.010 PNL=+85.60€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.185 > 0.08 con n=839 PNL=+559.97€
  - _Datos_: n=839 IC=+0.185 PNL=+559.97€
...
**〰️ H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: n=1399 IC=-0.060 PNL=+311.11€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1399 IC=-0.060 PNL=+311.11€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=4491 IC=+0.146 PNL=-726.12€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=4491 IC=+0.146 PNL=-726.12€
