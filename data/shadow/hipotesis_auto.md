# Hipótesis automáticas — 2026-07-16 11:29 UTC
_Generado por shadow_postmortem.py sobre 16872 resoluciones (PNL=+3439.68€)_
...
- **PATRÓN** `py_entrada` < `0.265` → IC=+0.332 (n=343)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.265 (IC base=+0.167)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.178 (n=1125)
...
- **PATRÓN** `py_entrada` < `0.265` → IC=+0.362 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.265 (IC base=+0.203)

- **PATRÓN** `libro_liquidez` > `3727.5827` → IC=+0.214 (n=208)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3727.5827 (IC base=+0.203)
...
- **PATRÓN** `py_entrada` < `0.365` → IC=+0.269 (n=184)
...
- **PATRÓN** `sigma_h` < `0.0108` → IC=+0.137 (n=213)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0108 (IC base=+0.109)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.141 (n=218)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 16.0 (IC base=+0.109)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.515` → IC=+0.236 (n=51)
...
- **PATRÓN** `dist_vwap_pct` > `0.3154` → IC=+0.143 (n=54)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.3154 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` < `0.5109` → IC=+0.142 (n=191)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.5109 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.494` → IC=+0.138 (n=172)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` < 7.494 (IC base=+0.164)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.162 (n=767)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 18.0 (IC base=+0.156)

- **PATRÓN** `dist_vwap_pct` > `0.1497` → IC=+0.197 (n=285)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1497 (IC base=+0.156)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.178 (n=763)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` > 0.0048 (IC base=+0.162)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.164 (n=549)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.4184` → IC=+0.195 (n=546)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` < 0.4184 (IC base=+0.162)
...
- **PATRÓN** `sigma_h` > `0.0127` → IC=+0.223 (n=92)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0127 (IC base=+0.136)
...
- **PATRÓN** `dist_vwap_pct` > `0.2328` → IC=+0.181 (n=67)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.2328 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `1.0007` → IC=+0.147 (n=154)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 1.0007 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.061` → IC=+0.283 (n=67)
...
- **PATRÓN** `sigma_h` < `0.0172` → IC=+0.298 (n=171)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0172 (IC base=+0.290)

- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.321 (n=171)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0086 (IC base=+0.290)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.289 (n=126)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.290)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.299 (n=172)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.290)

- **PATRÓN** `dist_vwap_pct` > `0.1883` → IC=+0.300 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1883 (IC base=+0.290)

- **PATRÓN** `dist_vwap_pct` < `0.5145` → IC=+0.309 (n=145)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5145 (IC base=+0.290)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.130 (n=604)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` < 15.0 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `0.3418` → IC=+0.172 (n=175)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.3418 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` < `0.138` → IC=+0.140 (n=256)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.138 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.296` → IC=+0.294 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.296 (IC base=+0.112)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.126 (n=303)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` < 6.0 (IC base=+0.091)

- **PATRÓN** `dist_vwap_pct` < `0.3641` → IC=+0.122 (n=517)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.3641 (IC base=+0.091)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.156 (n=59)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0039 (IC base=+0.053)

- **PATRÓN** `sigma_ewma_delta_pct` > `14.551` → IC=+0.333 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 14.551 (IC base=+0.053)
...
- **PATRÓN** `dist_vwap_pct` < `0.2565` → IC=+0.150 (n=58)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.2565 (IC base=+0.086)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.311` → IC=+0.351 (n=45)
...
- **PATRÓN** `sigma_h` < `0.0114` → IC=+0.197 (n=107)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0114 (IC base=+0.198)
...
- **PATRÓN** `dist_vwap_pct` < `0.4836` → IC=+0.186 (n=186)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` < 0.4836 (IC base=+0.198)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.517` → IC=+0.201 (n=145)
...
- **PATRÓN** `dist_vwap_pct` > `0.8904` → IC=+0.140 (n=23)
  ...
  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.8904 (IC base=+0.046)
...
- **PATRÓN** `dist_vwap_pct` > `0.9454` → IC=+0.182 (n=20)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.9454 (IC base=+0.044)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2536 | +0.173 | -72.71€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 1633 | +0.209 | +5.55€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 903 | +0.171 | -13.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 541 | +0.211 | +10.03€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 889 | +0.170 | -37.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 537 | +0.214 | +15.37€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 4869 | +0.102 | +1519.97€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 4869 | +0.102 | +1519.97€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1167 | +0.063 | +162.51€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1167 | +0.063 | +162.51€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1165 | +0.075 | +198.16€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1165 | +0.075 | +198.16€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1304 | +0.110 | +531.06€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1304 | +0.110 | +531.06€ | 1 | 7 |
| ✅ GBM_LATE_15M#XRP | 1233 | +0.157 | +628.24€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1233 | +0.157 | +628.24€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2091 | +0.148 | +1212.25€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2091 | +0.148 | +1212.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 535 | +0.109 | +260.13€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 535 | +0.109 | +260.13€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 554 | +0.126 | +254.89€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 554 | +0.126 | +254.89€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 564 | +0.110 | +243.56€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 564 | +0.110 | +243.56€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 438 | +0.273 | +453.67€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 438 | +0.273 | +453.67€ | 0 | 12 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 79 | +0.105 | +28.47€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 79 | +0.105 | +28.47€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 51 | +0.255 | +31.12€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 51 | +0.255 | +31.12€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 2068 | +0.091 | +621.37€ | 0 | 7 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2068 | +0.091 | +621.37€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 511 | +0.056 | +83.00€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 511 | +0.056 | +83.00€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 479 | +0.020 | +10.14€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 479 | +0.020 | +10.14€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 487 | +0.064 | +100.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 487 | +0.064 | +100.98€ | 2 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 591 | +0.202 | +427.26€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 591 | +0.202 | +427.26€ | 0 | 11 |
...
| ✅ UPDOWN_GBM | 1595 | +0.010 | +76.14€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1337 | +0.036 | +113.18€ | 1 | 6 |
...
| ✅ UPDOWN_GBM#ETH | 766 | +0.017 | +35.51€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 673 | +0.033 | +46.57€ | 2 | 3 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.155: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.005 n=493 | neutral: IC=-0.020 n=373 | overbought(IBS>0.7): IC=+0.135 n=403
  - _Datos_: n=1343 IC=+0.043 PNL=+136.85€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9810 | tras_1loss IC=+0.069 n=6853 | tras_2loss IC=+0.038 n=2927/40 | gap=+0.064 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1468 IC=+0.016 PNL=+90.14€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1468 IC=+0.016 PNL=+90.14€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=599 IC=+0.031 PNL=+39.21€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=599 IC=+0.031 PNL=+39.21€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=14414 IC=+0.106 PNL=+3515.56€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=14414 IC=+0.106 PNL=+3515.56€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema, sin filtrar todavía
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.182 < -0.1 con n=401 PNL=-84.13€
  - _Datos_: n=401 IC=-0.182 PNL=-84.13€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.138 > 0.08 con n=161 PNL=+50.76€
  - _Datos_: n=161 IC=+0.138 PNL=+50.76€
