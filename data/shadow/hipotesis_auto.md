# Hipótesis automáticas — 2026-07-27 07:23 UTC
_Generado por shadow_postmortem.py sobre 37940 resoluciones (PNL=+7897.77€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.197 (n=2536)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 5.0 (IC base=+0.190)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.195 (n=1193)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.195 (n=2689)
...
- **PATRÓN** `dist_vwap_pct` > `0.4463` → IC=+0.138 (n=332)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 0.4463 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.032` → IC=+0.183 (n=773)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 3.032 (IC base=+0.117)
...
- **PATRÓN** `sigma_h` < `0.0044` → IC=+0.153 (n=410)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` < 0.0044 (IC base=+0.111)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.374` → IC=+0.231 (n=143)
...
- **PATRÓN** `sigma_h` > `0.0207` → IC=+0.150 (n=332)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0207 (IC base=+0.144)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.152 (n=737)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 6.0 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `0.609` → IC=+0.184 (n=74)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.609 (IC base=+0.144)
...
- **PATRÓN** `dist_vwap_pct` > `0.2713` → IC=+0.195 (n=493)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.2713 (IC base=+0.149)
...
- **PATRÓN** `sigma_h` > `0.0067` → IC=+0.155 (n=1353)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.4007` → IC=+0.127 (n=338)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` > 0.4007 (IC base=+0.120)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.238` → IC=+0.137 (n=301)

  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` > 8.238 (IC base=+0.120)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.447` → IC=+0.139 (n=189)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` > 4.447 (IC base=+0.092)
...
- **PATRÓN** `dist_vwap_pct` > `0.1787` → IC=+0.167 (n=181)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.1787 (IC base=+0.128)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.315` → IC=+0.198 (n=114)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` > 7.315 (IC base=+0.128)
...
- **FILTRO** `sigma_ewma_delta_pct` > `7.949` → IC=-0.250 (n=22)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.100 (n=495)
...
- **PATRÓN** `sigma_h` < `0.0172` → IC=+0.211 (n=524)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0172 (IC base=+0.209)

- **PATRÓN** `sigma_h` > `0.007` → IC=+0.240 (n=525)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.846` → IC=+0.206 (n=460)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.846 (IC base=+0.209)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0124` → IC=+0.145 (n=548)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.4462` → IC=+0.153 (n=335)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.4462 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.307` → IC=+0.255 (n=259)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.307 (IC base=+0.106)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0026` → IC=+0.164 (n=123)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0026 (IC base=+0.097)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.969` → IC=+0.218 (n=76)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.969 (IC base=+0.097)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `12.377` → IC=+0.200 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.377 (IC base=+0.068)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.386` → IC=+0.242 (n=149)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.386 (IC base=+0.060)
...
- **PATRÓN** `sigma_h` < `0.0202` → IC=+0.183 (n=443)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0202 (IC base=+0.176)

- **PATRÓN** `sigma_h` > `0.0105` → IC=+0.176 (n=449)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0105 (IC base=+0.176)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.181 (n=506)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.4429` → IC=+0.239 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4429 (IC base=+0.176)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.065` → IC=+0.287 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.065 (IC base=+0.176)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.609` → IC=+0.176 (n=621)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.88€ cuando `ibs_15` > 0.609 (IC base=+0.058)

- **PATRÓN** `dist_vwap_pct` > `0.1663` → IC=+0.170 (n=228)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.1663 (IC base=+0.058)
...
- **PATRÓN** `ibs_15` > `0.67` → IC=+0.193 (n=200)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.97€ cuando `ibs_15` > 0.67 (IC base=+0.045)

- **PATRÓN** `dist_vwap_pct` > `0.1283` → IC=+0.127 (n=116)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` > 0.1283 (IC base=+0.045)

- **PATRÓN** `sigma_ewma_delta_pct` > `27.624` → IC=+0.167 (n=25)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 27.624 (IC base=+0.045)
...
- **PATRÓN** `ibs_15` < `0.025` → IC=+0.135 (n=61)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.67€ cuando `ibs_15` < 0.025 (IC base=+0.076)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.609 sube el IC de +0.058 a +0.176 en UPDOWN_GBM#15min (n=621). Ya aplicado como kelly_boost=+0.88€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.67 sube el IC de +0.045 a +0.193 en UPDOWN_GBM#ETH#15min (n=200). Ya aplicado como kelly_boost=+0.97€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 1371 | +0.102 | +34.69€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 1371 | +0.102 | +34.69€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 542 | +0.090 | -1.24€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 542 | +0.090 | -1.24€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 507 | +0.129 | +16.02€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 507 | +0.129 | +16.02€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 303 | +0.077 | +20.27€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 303 | +0.077 | +20.27€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 6688 | +0.180 | -13.78€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 4404 | +0.203 | -24.40€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 2350 | +0.168 | -42.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1450 | +0.199 | -10.10€ | 0 | 5 |
...
| ✅ GBM_LATE_15M | 8207 | +0.095 | +2486.77€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 8207 | +0.095 | +2486.77€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1986 | +0.074 | +355.69€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1986 | +0.074 | +355.69€ | 0 | 5 |
| ✅ GBM_LATE_15M#ETH | 1808 | +0.073 | +302.66€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1808 | +0.073 | +302.66€ | 0 | 6 |
| ✅ GBM_LATE_15M#SOL | 