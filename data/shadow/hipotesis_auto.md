# Hipótesis automáticas — 2026-07-03 14:24 UTC
_Generado por shadow_postmortem.py sobre 3088 resoluciones (PNL=-39.36€)_

## Patrones causales activos

### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` < `0.0096` → IC=+0.204 (n=79)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0096 (IC base=+0.037)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.122 (n=43)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 15.0 (IC base=+0.037)

- **PATRÓN** `ibs_15` < `0.0613` → IC=+0.161 (n=57)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.81€ cuando `ibs_15` < 0.0613 (IC base=+0.037)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1397` → IC=+0.151 (n=61)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.75€ cuando `pct_spot_vs_ref` |x|≤ 0.1397 (IC base=+0.110)

- **PATRÓN** `sigma_h` < `0.0087` → IC=+0.174 (n=41)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0087 (IC base=+0.110)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.152 (n=21)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 6.0 (IC base=+0.110)

- **PATRÓN** `ibs_15` < `0.0792` → IC=+0.265 (n=32)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0792 (IC base=+0.110)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS < 0.0613 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.161 n=57). Confirma señal de reversión media → alinear con BUY_YES.
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.0792 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.265 n=32). Confirma señal de reversión media → alinear con BUY_YES.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 428 | +0.051 | +55.89€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 428 | +0.051 | +55.89€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 110 | +0.009 | +2.95€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 110 | +0.009 | +2.95€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 111 | -0.022 | -0.67€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 111 | -0.022 | -0.67€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL | 107 | +0.115 | +29.75€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 107 | +0.115 | +29.75€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP | 100 | +0.108 | +23.87€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 100 | +0.108 | +23.87€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 860 | -0.041 | -49.88€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 630 | -0.019 | -22.03€ | 0 | 3 |
...
| ✅ UPDOWN_GBM#ETH | 339 | -0.010 | +8.30€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 255 | +0.006 | +14.76€ | 3 | 4 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.064) — sin ventaja clara. oversold(IBS<0.3): IC=-0.006 n=164 | neutral: IC=-0.070 n=140 | overbought(IBS>0.7): IC=-0.046 n=150
  - _Datos_: n=470 IC=-0.040 PNL=-24.64€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.008 n=1483 | tras_1loss IC=-0.027 n=1513 | tras_2loss IC=-0.044 n=778/40 | gap=+0.052 (umbral 0.05)
...
**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  ...
  - _Estado_: n_overlaps=25, boost estimado=+0.013. Necesita 0 más y boost>0.05
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=-0.052 n=132/40 PNL=-4.05€ | H=15h UTC: IC=+0.017 n=116/40 PNL=+5.20€ | H=17h UTC: IC=+0.200 n=58/40 PNL=+21.47€ | H=19h UTC: IC=-0.013 n=72/40 PNL=-9.58€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=737 IC=-0.036 PNL=-36.32€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=737 IC=-0.036 PNL=-36.32€
...
**〰️ H-CUSTOM-ETH15-REVERSION** — ETH#15min con drift_15min < -1 — ¿mean reversion?
  ...
  - _Estado_: n=43 IC=+0.078 PNL=+12.46€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=43 IC=+0.078 PNL=+12.46€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=220 IC=+0.013 PNL=+13.27€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=220 IC=+0.013 PNL=+13.27€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=662 IC=+0.033 PNL=+50.81€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=662 IC=+0.033 PNL=+50.81€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=46 IC=-0.021 PNL=-8.75€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=46 IC=-0.021 PNL=-8.75€
