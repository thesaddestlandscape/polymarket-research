# Hipótesis automáticas — 2026-07-20 03:28 UTC
_Generado por shadow_postmortem.py sobre 23731 resoluciones (PNL=+6069.61€)_
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.209 (n=1495)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.204 (n=1666)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.214 (n=593)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.178 (n=1776)

  - _Acción_: Kelly boost +0.89€ cuando `libro_spread` < 0.01 (IC base=+0.173)

- **PATRÓN** `libro_liquidez` > `4183.8127` → IC=+0.175 (n=1063)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 4183.8127 (IC base=+0.173)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.236 (n=331)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.234 (n=407)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.229)

- **PATRÓN** `libro_liquidez` > `4298.9488` → IC=+0.235 (n=213)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.260 (n=119)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.1226` → IC=+0.130 (n=679)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.136 (n=1626)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` < 12.0 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` > `0.1364` → IC=+0.138 (n=385)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 0.1364 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` < `0.8085` → IC=+0.129 (n=1195)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `11.097` → IC=+0.127 (n=1076)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` < 11.097 (IC base=+0.116)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.127 (n=421)
...
- **PATRÓN** `dist_vwap_pct` > `0.4801` → IC=+0.198 (n=41)
  ...
  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.4801 (IC base=+0.103)

- **PATRÓN** `dist_vwap_pct` < `0.1071` → IC=+0.151 (n=144)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.236 (n=225)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.5244` → IC=+0.175 (n=355)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` < 0.5244 (IC base=+0.174)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.52` → IC=+0.173 (n=334)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` < 9.52 (IC base=+0.174)
...
- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.157 (n=246)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.148 (n=308)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 18.0 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `0.4153` → IC=+0.184 (n=74)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.4153 (IC base=+0.146)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.324 (n=282)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.306 (n=250)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.5023` → IC=+0.308 (n=285)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5023 (IC base=+0.300)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.68` → IC=+0.313 (n=255)
...
- **PATRÓN** `dist_vwap_pct` > `0.4717` → IC=+0.180 (n=220)
  ...
  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.4717 (IC base=+0.132)

- **PATRÓN** `dist_vwap_pct` < `0.1111` → IC=+0.159 (n=579)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` < 0.1111 (IC base=+0.132)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.175` → IC=+0.299 (n=152)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.175 (IC base=+0.132)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.124 (n=831)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` < 12.0 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.5188` → IC=+0.138 (n=1000)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.5188 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.961` → IC=+0.136 (n=212)

  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` > 5.961 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.187` → IC=+0.130 (n=954)
...
- **PATRÓN** `sigma_h` < `0.0043` → IC=+0.173 (n=160)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0043 (IC base=+0.122)
...
- **PATRÓN** `dist_vwap_pct` > `0.501` → IC=+0.221 (n=41)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.501 (IC base=+0.122)

- **PATRÓN** `dist_vwap_pct` < `0.1391` → IC=+0.147 (n=148)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.1391 (IC base=+0.122)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.145 (n=91)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` < 0.0039 (IC base=+0.078)

- **PATRÓN** `dist_vwap_pct` < `0.2277` → IC=+0.127 (n=148)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` < 0.2277 (IC base=+0.078)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.565` → IC=+0.262 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.565 (IC base=+0.078)
...
- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.135 (n=94)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0083 (IC base=+0.048)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.239 (n=136)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.5022` → IC=+0.215 (n=346)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5022 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.223` → IC=+0.226 (n=330)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.593` → IC=-0.155 (n=27)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.098 (n=336)
...
| ✅ FAVORITO_CONFIRMADO | 4061 | +0.186 | +61.54€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 2645 | +0.218 | +96.09€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 