# Hipótesis automáticas — 2026-07-16 09:10 UTC
_Generado por shadow_postmortem.py sobre 16746 resoluciones (PNL=+3431.44€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.182 (n=810)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 8.0 (IC base=+0.182)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.192 (n=932)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.188 (n=1053)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.01 (IC base=+0.182)

- **PATRÓN** `libro_liquidez` > `4371.9525` → IC=+0.189 (n=602)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 4371.9525 (IC base=+0.182)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.232 (n=177)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `5259.0173` → IC=+0.222 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 5259.0173 (IC base=+0.223)
...
- **PATRÓN** `py_entrada` > `0.615` → IC=+0.275 (n=185)
...
- **PATRÓN** `dist_vwap_pct` > `0.9112` → IC=+0.130 (n=98)
  ...
  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` > 0.9112 (IC base=+0.108)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.173` → IC=+0.220 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.173 (IC base=+0.108)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.134 (n=1234)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` > 0.0091 (IC base=+0.107)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.127 (n=1240)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 11.0 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.2` → IC=+0.125 (n=142)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 6.2 (IC base=+0.107)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.697` → IC=+0.204 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.697 (IC base=+0.067)
...
- **PATRÓN** `sigma_h` < `0.016` → IC=+0.121 (n=478)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.60€ cuando `sigma_h` < 0.016 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.141 (n=218)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 16.0 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.477` → IC=+0.245 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.477 (IC base=+0.110)
...
- **PATRÓN** `dist_vwap_pct` < `0.5138` → IC=+0.145 (n=184)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.5138 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.696` → IC=+0.141 (n=165)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` < 7.696 (IC base=+0.165)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.166 (n=761)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `11.023` → IC=+0.302 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.023 (IC base=+0.159)

- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.179 (n=752)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.4123` → IC=+0.199 (n=529)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.4123 (IC base=+0.164)
...
- **PATRÓN** `sigma_h` > `0.0127` → IC=+0.220 (n=91)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0127 (IC base=+0.142)
...
- **PATRÓN** `dist_vwap_pct` > `0.3907` → IC=+0.191 (n=53)
  ...
  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.3907 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` < `1.0397` → IC=+0.154 (n=151)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 1.0397 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.082` → IC=+0.291 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.082 (IC base=+0.142)
...
- **PATRÓN** `sigma_h` < `0.0166` → IC=+0.293 (n=167)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0166 (IC base=+0.290)

- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.323 (n=167)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.299 (n=167)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.5285` → IC=+0.310 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5285 (IC base=+0.290)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.053` → IC=+0.317 (n=113)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.053 (IC base=+0.290)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0151` → IC=+0.194 (n=227)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0151 (IC base=+0.112)
...
- **PATRÓN** `dist_vwap_pct` < `0.138` → IC=+0.140 (n=248)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.138 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.298` → IC=+0.303 (n=69)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.997` → IC=+0.136 (n=20)
  ...
  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` > 7.997 (IC base=+0.019)
...
- **PATRÓN** `sigma_h` < `0.0095` → IC=+0.195 (n=80)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0095 (IC base=+0.196)

- **PATRÓN** `sigma_h` > `0.0265` → IC=+0.244 (n=80)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0265 (IC base=+0.196)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.253 (n=87)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.196)

- **PATRÓN** `dist_vwap_pct` < `0.4878` → IC=+0.185 (n=182)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` < 0.4878 (IC base=+0.196)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.549` → IC=+0.199 (n=141)
...
- **PATRÓN** `dist_vwap_pct` > `0.6858` → IC=+0.122 (n=35)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` > 0.6858 (IC base=+0.045)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.294` → IC=+0.123 (n=128)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 6.294 (IC base=+0.045)
...
- **PATRÓN** `dist_vwap_pct` > `0.9263` → IC=+0.182 (n=20)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.9263 (IC base=+0.043)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2502 | +0.175 | -50.59€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1607 | +0.212 | +25.06€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 725 | +0.184 | -6.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 538 | +0.209 | -5.18€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 890 | +0.174 | -3.91€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 532 | +0.215 | +17.74€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 879 | +0.170 | -37.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 529 | +0.214 | +15.87€ | 0 | 6 |
...
| ✅ GBM_LATE_15M | 4840 | +0.103 | +1511.88€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 4840 | +0.103 | +1511.88€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1161 | +0.063 | +160.76€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1161 | +0.063 | +160.76€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1159 | +0.076 | +200.38€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1159 | +0.076 | +200.38€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1295 | +0.110 | +526.18€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1295 | +0.110 | +526.18€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1225 | +0.157 | +624.56€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1225 | +0.157 | +624.56€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2068 | +0.150 | +1209.35€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2068 | +0.150 | +1209.35€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 558 | +0.113 | +246.24€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 558 | +0.113 | +246.24€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 433 | +0.272 | +447.83€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 433 | +0.272 | +447.83€ | 0 | 12 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 74 | +0.092 | +22.95€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 74 | +0.092 | +22.95€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 46 | +0.250 | +25.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 46 | +0.250 | +25.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 2039 | +0.090 | +609.63€ | 0 | 6 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2039 | +0.090 | +609.63€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 505 | +0.056 | +84.57€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 505 | +0.056 | +84.57€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 471 | +0.022 | +13.23€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 471 | +0.022 | +13.23€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 480 | +0.060 | +96.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 480 | +0.060 | +96.34€ | 3 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 583 | +0.199 | +415.48€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 583 | +0.199 | +415.48€ | 0 | 11 |
...
| ✅ UPDOWN_GBM | 1594 | +0.009 | +75.02€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1336 | +0.036 | +112.06€ | 1 | 6 |
...
| ✅ UPDOWN_GBM#ETH | 765 | +0.016 | +34.39€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 672 | +0.033 | +45.45€ | 2 | 3 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.155: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.004 n=492 | neutral: IC=-0.020 n=373 | overbought(IBS>0.7): IC=+0.135 n=403
  - _Datos_: n=1342 IC=+0.043 PNL=+135.73€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9746 | tras_1loss IC=+0.069 n=6791 | tras_2loss IC=+0.036 n=2898/40 | gap=+0.066 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1467 IC=+0.016 PNL=+89.02€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1467 IC=+0.016 PNL=+89.02€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=598 IC=+0.030 PNL=+38.09€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=598 IC=+0.030 PNL=+38.09€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=14288 IC=+0.106 PNL=+3507.33€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=14288 IC=+0.106 PNL=+3507.33€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=155 IC=+0.010 PNL=+7.48€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=155 IC=+0.010 PNL=+7.48€
...
**🔶 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.167 n=199) pero sin cruzar ≥2 pares más — BTC: n=144 IC=-0.014; ETH: n=108 IC=+0.000; SOL: n=210 IC=+0.061 ✓
