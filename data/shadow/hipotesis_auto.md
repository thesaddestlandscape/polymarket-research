# Hipótesis automáticas — 2026-07-15 15:32 UTC
_Generado por shadow_postmortem.py sobre 15506 resoluciones (PNL=+3089.60€)_
...
- **PATRÓN** `py_entrada` > `0.695` → IC=+0.309 (n=364)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.193)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.198 (n=915)
...
### GBM_LATE_15M
- **PATRÓN** `dist_vwap_pct` > `0.6733` → IC=+0.124 (n=115)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` > 0.6733 (IC base=+0.108)
...
- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.136 (n=1179)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.004` → IC=+0.186 (n=49)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 3.004 (IC base=+0.058)
...
- **PATRÓN** `dist_vwap_pct` > `0.7215` → IC=+0.176 (n=35)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.7215 (IC base=+0.065)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.073` → IC=+0.135 (n=50)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` > 5.073 (IC base=+0.065)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.126` → IC=+0.227 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.126 (IC base=+0.111)
...
- **PATRÓN** `sigma_h` > `0.023` → IC=+0.186 (n=192)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.023 (IC base=+0.144)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.152 (n=446)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.7064` → IC=+0.233 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7064 (IC base=+0.144)
...
- **PATRÓN** `sigma_h` > `0.0047` → IC=+0.182 (n=687)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` > 0.0047 (IC base=+0.162)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.169 (n=324)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.4443` → IC=+0.196 (n=461)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` < 0.4443 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.927` → IC=+0.224 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.927 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.13` → IC=+0.176 (n=344)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` < 3.13 (IC base=+0.162)
...
- **PATRÓN** `sigma_h` > `0.0027` → IC=+0.182 (n=174)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.106` → IC=+0.300 (n=18)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.106 (IC base=+0.140)
...
- **PATRÓN** `sigma_h` > `0.0131` → IC=+0.214 (n=82)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0131 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.151 (n=130)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 12.0 (IC base=+0.136)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.146 (n=159)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 15.0 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.4625` → IC=+0.186 (n=49)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.4625 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `1.0918` → IC=+0.151 (n=130)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 1.0918 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.441` → IC=+0.259 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.441 (IC base=+0.136)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.128 (n=552)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 15.0 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` > `0.7118` → IC=+0.183 (n=99)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.7118 (IC base=+0.106)
...
- **PATRÓN** `sigma_h` < `0.0326` → IC=+0.197 (n=173)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0326 (IC base=+0.194)

- **PATRÓN** `sigma_h` > `0.0178` → IC=+0.227 (n=115)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0178 (IC base=+0.194)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.225 (n=158)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.194)

- **PATRÓN** `dist_vwap_pct` > `0.1906` → IC=+0.241 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1906 (IC base=+0.194)

- **PATRÓN** `dist_vwap_pct` < `1.4499` → IC=+0.187 (n=97)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` < 1.4499 (IC base=+0.194)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.253` → IC=+0.214 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.253 (IC base=+0.194)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.983` → IC=+0.186 (n=49)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` < 4.983 (IC base=+0.194)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2196 | +0.174 | -44.83€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 1414 | +0.208 | +4.33€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 780 | +0.174 | +2.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 467 | +0.216 | +18.14€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 773 | +0.170 | -29.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 468 | +0.211 | +12.15€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 9 | +0.143 | +0.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 9 | +0.143 | +0.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 9 | +0.143 | +0.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 9 | +0.143 | +0.29€ | 0 | 0 |
| ✅ GBM_LATE_15M | 4611 | +0.103 | +1420.64€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 4611 | +0.103 | +1420.64€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1107 | +0.065 | +156.51€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1107 | +0.065 | +156.51€ | 0 | 3 |
| ✅ GBM_LATE_15M#ETH | 1122 | +0.076 | +197.21€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1122 | +0.076 | +197.21€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1226 | +0.112 | +497.64€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1226 | +0.112 | +497.64€ | 0 | 6 |
| ✅ GBM_LATE_15M#XRP | 1156 | +0.154 | +569.28€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1156 | +0.154 | +569.28€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1865 | +0.154 | +1103.93€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1865 | +0.154 | +1103.93€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 477 | +0.122 | +244.09€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 477 | +0.122 | +244.09€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 492 | +0.130 | +230.89€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 492 | +0.130 | +230.89€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 503 | +0.112 | +219.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 503 | +0.112 | +219.25€ | 1 | 10 |
...
| ✅ GBM_LATE_15M_TARDIO | 1818 | +0.086 | +521.51€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1818 | +0.086 | +521.51€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 452 | +0.062 | +84.48€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 452 | +0.062 | +84.48€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 409 | +0.018 | +11.91€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 409 | +0.018 | +11.91€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 442 | +0.050 | +74.80€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 442 | +0.050 | +74.80€ | 2 | 5 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 515 | +0.193 | +350.33€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 515 | +0.193 | +350.33€ | 0 | 13 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 24 | +0.346 | +12.17€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 24 | +0.346 | +12.17€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 8 | +0.120 | +4.22€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 8 | +0.120 | +4.22€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.128) — sin ventaja clara. oversold(IBS<0.3): IC=+0.001 n=435 | neutral: IC=-0.016 n=337 | overbought(IBS>0.7): IC=+0.112 n=351
  - _Datos_: n=1188 IC=+0.035 PNL=+112.31€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.100 n=8931 | tras_1loss IC=+0.062 n=6341 | tras_2loss IC=+0.030 n=2747/40 | gap=+0.070 (umbral 0.05)
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=+0.075 n=692/40 PNL=+165.38€ | H=15h UTC: IC=+0.123 n=659/40 PNL=+209.86€ | H=17h UTC: IC=+0.127 n=575/40 PNL=+170.31€ | H=19h UTC: IC=+0.082 n=556/40 PNL=+84.58€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=13048 IC=+0.103 PNL=+3165.48€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=13048 IC=+0.103 PNL=+3165.48€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=590 IC=+0.054 PNL=+64.81€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=590 IC=+0.054 PNL=+64.81€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.254 > 0.2 con n=307 PNL=+208.04€
  - _Datos_: n=307 IC=+0.254 PNL=+208.04€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.142 > 0.08 con n=135 PNL=+49.11€
  - _Datos_: n=135 IC=+0.142 PNL=+49.11€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.139 > 0.08 con n=164 PNL=+86.84€
  - _Datos_: n=164 IC=+0.139 PNL=+86.84€
