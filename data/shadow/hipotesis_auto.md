# Hipótesis automáticas — 2026-07-28 20:44 UTC
_Generado por shadow_postmortem.py sobre 40753 resoluciones (PNL=+8509.35€)_
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.194 (n=1293)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0076 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.169 (n=1747)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 8.0 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.2771` → IC=+0.177 (n=537)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.2771 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.373` → IC=+0.236 (n=635)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.373 (IC base=+0.145)
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.196 (n=159)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0076 (IC base=+0.117)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.142 (n=319)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` > 12.0 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` > `0.672` → IC=+0.148 (n=86)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.672 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.264` → IC=+0.185 (n=128)

  - _Acción_: Kelly boost +0.92€ cuando `sigma_ewma_delta_pct` > 7.264 (IC base=+0.117)
...
- **PATRÓN** `sigma_h` < `0.0171` → IC=+0.242 (n=448)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0171 (IC base=+0.228)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.246 (n=415)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.656` → IC=+0.280 (n=57)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.222 (n=488)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.126 (n=1279)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.63€ cuando `sigma_h` > 0.0083 (IC base=+0.098)

- **PATRÓN** `dist_vwap_pct` > `0.4721` → IC=+0.138 (n=354)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 0.4721 (IC base=+0.098)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.946` → IC=+0.236 (n=312)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.946 (IC base=+0.098)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.467` → IC=+0.286 (n=68)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.467 (IC base=+0.052)
...
- **PATRÓN** `sigma_h` > `0.0102` → IC=+0.172 (n=501)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0102 (IC base=+0.170)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.175 (n=571)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 6.0 (IC base=+0.170)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.169 (n=497)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 15.0 (IC base=+0.170)

- **PATRÓN** `dist_vwap_pct` > `0.2459` → IC=+0.221 (n=159)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2459 (IC base=+0.170)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.167 (n=427)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.01 (IC base=+0.170)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.604` → IC=+0.173 (n=650)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.87€ cuando `ibs_15` > 0.604 (IC base=+0.054)

- **PATRÓN** `dist_vwap_pct` > `0.1672` → IC=+0.163 (n=241)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.1672 (IC base=+0.054)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.107` → IC=+0.146 (n=238)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` > 5.107 (IC base=+0.054)

- **PATRÓN** `libro_liquidez` > `10361.4771` → IC=+0.134 (n=184)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 10361.4771 (IC base=+0.054)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.120 (n=327)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.60€ cuando `sigma_h` < 0.0073 (IC base=+0.100)

- **PATRÓN** `drift_60min` |x|≤ `0.2192` → IC=+0.136 (n=317)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.2192 (IC base=+0.100)

- **PATRÓN** `delta_ratio_macro` |x|> `0.063` → IC=+0.128 (n=318)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.64€ cuando `delta_ratio_macro` |x|> 0.063 (IC base=+0.100)

- **PATRÓN** `hora_utc` < `14.0` → IC=+0.165 (n=186)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 14.0 (IC base=+0.100)

- **PATRÓN** `ibs_15` > `0.6354` → IC=+0.214 (n=278)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6354 (IC base=+0.100)

- **PATRÓN** `dist_vwap_pct` > `0.2571` → IC=+0.226 (n=93)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2571 (IC base=+0.100)

- **PATRÓN** `sigma_ewma_delta_pct` > `24.96` → IC=+0.191 (n=40)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 24.96 (IC base=+0.100)

- **PATRÓN** `libro_liquidez` > `11693.755` → IC=+0.161 (n=110)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 11693.755 (IC base=+0.100)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.069` → IC=+0.151 (n=64)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 7.069 (IC base=+0.044)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.604 sube el IC de +0.054 a +0.173 en UPDOWN_GBM#15min (n=650). Ya aplicado como kelly_boost=+0.87€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.6354 sube el IC de +0.100 a +0.214 en UPDOWN_GBM#BTC#15min (n=278). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.6674 sube el IC de +0.044 a +0.187 en UPDOWN_GBM#ETH#15min (n=209). Ya aplicado como kelly_boost=+0.94€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ FAVORITO_CONFIRMADO | 6942 | +0.177 | -55.36€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 4568 | +0.200 | -47.77€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 2060 | +0.182 | -3.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1529 | +0.198 | -44.99€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 8784 | +0.093 | +2620.36€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 8784 | +0.093 | +2620.36€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 85 | +0.190 | +45.43€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 85 | +0.190 | +45.43€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 2083 | +0.069 | +340.23€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 2083 | +0.069 | +340.23€ | 0 | 6 |
| ✅ GBM_LATE_15M#DOGE | 84 | +0.093 | +14.12€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 84 | +0.093 | +14.12€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 1870 | +0.070 | +303.59€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1870 | +0.070 | +303.59€ | 0 | 7 |
| ✅ GBM_LATE_15M#SOL | 2321 | +0.084 | +793.24€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 2321 | +0.084 | +793.24€ | 1 | 3 |
| ✅ GBM_LATE_15M#XRP | 2341 | +0.139 | +1123.75€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 2341 | +0.139 | +1123.75€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 6007 | +0.118 | +2846.36€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 6007 | +0.118 | +2846.36€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 64 | +0.348 | +82.47€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 64 | +0.348 | +82.47€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 