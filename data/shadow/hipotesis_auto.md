# Hipótesis automáticas — 2026-07-16 01:43 UTC
_Generado por shadow_postmortem.py sobre 16255 resoluciones (PNL=+3289.53€)_
...
- **PATRÓN** `py_entrada` > `0.615` → IC=+0.253 (n=172)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.615 (IC base=+0.199)

- **PATRÓN** `libro_liquidez` > `11062.4554` → IC=+0.203 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11062.4554 (IC base=+0.199)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.232 (n=192)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `3669.7607` → IC=+0.223 (n=186)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3669.7607 (IC base=+0.224)
...
- **PATRÓN** `dist_vwap_pct` > `0.1536` → IC=+0.197 (n=265)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1536 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.007` → IC=+0.302 (n=84)
...
- **PATRÓN** `sigma_h` < `0.0186` → IC=+0.278 (n=151)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0186 (IC base=+0.254)

- **PATRÓN** `sigma_h` > `0.0101` → IC=+0.252 (n=151)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0101 (IC base=+0.254)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.300 (n=133)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.254)

- **PATRÓN** `dist_vwap_pct` > `0.8165` → IC=+0.364 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8165 (IC base=+0.254)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.099` → IC=+0.323 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.099 (IC base=+0.254)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0149` → IC=+0.186 (n=218)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0149 (IC base=+0.111)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.120 (n=677)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` < 18.0 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` > `0.3666` → IC=+0.167 (n=163)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.3666 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` < `0.1427` → IC=+0.146 (n=224)
...
- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.219 (n=190)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0113 (IC base=+0.201)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.246 (n=69)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` > `0.5779` → IC=+0.275 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5779 (IC base=+0.201)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.951` → IC=+0.206 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.951 (IC base=+0.201)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.161` → IC=+0.212 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.161 (IC base=+0.201)
...
- **PATRÓN** `py_entrada` < `0.485` → IC=+0.150 (n=18)
  ...
  - _Acción_: Kelly boost +0.75€ cuando `py_entrada` < 0.485 (IC base=+0.049)
...
### UPDOWN_GBM#15min
- **FILTRO** `sigma_ewma_delta_pct` > `16.748` → IC=-0.130 (n=25)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `16.748` → IC=+0.134 (n=140)
...
- **PATRÓN** `dist_vwap_pct` > `0.974` → IC=+0.214 (n=19)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.974 (IC base=+0.046)

- **PATRÓN** `sigma_ewma_delta_pct` < `15.261` → IC=+0.121 (n=64)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` < 15.261 (IC base=+0.046)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2377 | +0.173 | -55.95€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1531 | +0.210 | +16.78€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 689 | +0.180 | -15.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 512 | +0.204 | -13.78€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 845 | +0.172 | -5.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 507 | +0.215 | +18.60€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 4747 | +0.103 | +1476.72€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 4747 | +0.103 | +1476.72€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#XRP | 1196 | +0.155 | +599.74€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1196 | +0.155 | +599.74€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1987 | +0.151 | +1170.29€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1987 | +0.151 | +1170.29€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 417 | +0.273 | +432.61€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 417 | +0.273 | +432.61€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 1950 | +0.089 | +573.50€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1950 | +0.089 | +573.50€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 554 | +0.196 | +386.71€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 554 | +0.196 | +386.71€ | 0 | 10 |
...
| ✅ LEADLAG_BTC_XRP_15M | 132 | +0.090 | +26.89€ | 0 | 4 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 132 | +0.090 | +26.89€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 132 | +0.090 | +26.89€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 132 | +0.090 | +26.89€ | 0 | 4 |
...
| ✅ UPDOWN_GBM | 1580 | +0.009 | +71.56€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1322 | +0.036 | +108.60€ | 1 | 6 |
...
| ✅ UPDOWN_GBM#DOGE | 56 | +0.017 | -0.06€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 53 | +0.027 | +0.84€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 759 | +0.018 | +36.37€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 666 | +0.034 | +47.42€ | 1 | 4 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 83 | +0.206 | +25.66€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 83 | +0.206 | +25.66€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 22 | +0.208 | +2.99€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 22 | +0.208 | +2.99€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 31 | +0.167 | +10.20€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 31 | +0.167 | +10.20€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 11 | +0.148 | +3.63€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 11 | +0.148 | +3.63€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.145) — sin ventaja clara. oversold(IBS<0.3): IC=+0.011 n=472 | neutral: IC=-0.023 n=367 | overbought(IBS>0.7): IC=+0.122 n=374
  - _Datos_: n=1282 IC=+0.040 PNL=+122.42€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9436 | tras_1loss IC=+0.066 n=6612 | tras_2loss IC=+0.033 n=2834/40 | gap=+0.069 (umbral 0.05)
...
**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  ...
  - _Estado_: n_overlaps=34, boost estimado=-0.040. Necesita 0 más y boost>0.05
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1453 IC=+0.016 PNL=+85.56€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1453 IC=+0.016 PNL=+85.56€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=698 IC=+0.074 PNL=+89.94€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=698 IC=+0.074 PNL=+89.94€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=588 IC=+0.032 PNL=+40.20€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=588 IC=+0.032 PNL=+40.20€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=13797 IC=+0.105 PNL=+3365.42€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=13797 IC=+0.105 PNL=+3365.42€
...
**🔶 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.150 n=184) pero sin cruzar ≥2 pares más — BTC: n=137 IC=-0.004; ETH: n=105 IC=-0.005; SOL: n=195 IC=+0.058 ✓
  - _Datos_: n=184 IC=+0.150 PNL=+105.59€
