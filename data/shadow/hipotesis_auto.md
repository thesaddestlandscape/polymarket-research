# Hipótesis automáticas — 2026-08-18 07:32 UTC
_Generado por shadow_postmortem.py sobre 60269 resoluciones (PNL=+6299.54€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `143.98` → IC=-0.267 (n=791)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 143.98
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=2373)
...
- **FILTRO** `restante_s_al_confirmar` > `638.84` → IC=-0.266 (n=109)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.081 (n=330)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.228 (n=307)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.192 (n=417)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.305 (n=254)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.287)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.291 (n=256)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.303 (n=292)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.228)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.251 (n=287)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.228)

- **PATRÓN** `libro_liquidez` > `909.0749` → IC=+0.236 (n=290)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.199 (n=1161)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=646)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.161)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.190 (n=288)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.232 (n=2856)
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.291 (n=314)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.148 (n=441)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 6.0 (IC base=+0.110)

- **PATRÓN** `ibs_20min` > `0.995` → IC=+0.276 (n=391)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.995 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` > `0.5823` → IC=+0.309 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5823 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.85` → IC=+0.241 (n=574)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.85 (IC base=+0.110)

- **PATRÓN** `volumen_regimen` < `1.3288` → IC=+0.224 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.3288 (IC base=+0.110)

- **PATRÓN** `volumen_regimen` > `0.7541` → IC=+0.231 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7541 (IC base=+0.110)

- **PATRÓN** `volumen_pendiente_norm` > `0.3247` → IC=+0.174 (n=90)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.3247 (IC base=+0.110)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.123 (n=1246)

  - _Acción_: Kelly boost +0.62€ cuando `libro_spread` < 0.06 (IC base=+0.110)

- **PATRÓN** `ibs_20min` < `0.6594` → IC=+0.123 (n=2066)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7094` → IC=+0.230 (n=150)
...
- **PATRÓN** `ballena_activa_n` < `265.0` → IC=+0.261 (n=65)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1384` → IC=+0.144 (n=88)
  ...
  - _Acción_: Kelly boost +0.72€ cuando `volumen_pendiente_norm` > 0.1384 (IC base=+0.126)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.175 (n=272)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.181 (n=114)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` > 0.0075 (IC base=+0.135)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.182 (n=171)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` < 8.0 (IC base=+0.135)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.123` → IC=+0.292 (n=94)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.123 (IC base=+0.135)

- **PATRÓN** `volumen_pendiente_norm` > `0.4225` → IC=+0.175 (n=38)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.4225 (IC base=+0.135)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.158 (n=372)

  - _Acción_: Kelly boost +0.79€ cuando `libro_spread` < 0.06 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `1915.1184` → IC=+0.162 (n=155)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 1915.1184 (IC base=+0.135)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.97` → IC=+0.125 (n=94)
  ...
  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 4.97 (IC base=+0.006)
...
- **PATRÓN** `ibs_20min` > `0.9465` → IC=+0.232 (n=509)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9465 (IC base=+0.058)

- **PATRÓN** `dist_vwap_pct` > `0.1183` → IC=+0.169 (n=146)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` > 0.1183 (IC base=+0.058)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.287` → IC=+0.130 (n=953)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` > 2.287 (IC base=+0.058)
...
- **PATRÓN** `dist_vwap_pct` > `0.5126` → IC=+0.321 (n=26)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5126 (IC base=+0.048)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3619` → IC=+0.405 (n=40)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3619 (IC base=+0.048)

- **PATRÓN** `volumen_spike_ratio` > `3.805` → IC=+0.308 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.805 (IC base=+0.048)

- **PATRÓN** `ballena_activa_n` < `46.0` → IC=+0.183 (n=39)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 46.0 (IC base=+0.048)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.422` → IC=+0.164 (n=114)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 3.422 (IC base=-0.012)
...
- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.194 (n=544)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.160 (n=545)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 7.0 (IC base=+0.138)

- **PATRÓN** `ibs_20min` > `0.9206` → IC=+0.255 (n=794)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9206 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` > `0.217` → IC=+0.147 (n=256)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.217 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.425` → IC=+0.248 (n=793)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.425 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `0.6285` → IC=+0.134 (n=618)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` > 0.6285 (IC base=+0.138)

- **PATRÓN** `volumen_pendiente_norm` > `0.113` → IC=+0.160 (n=389)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` > 0.113 (IC base=+0.138)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.146 (n=1328)

  - _Acción_: Kelly boost +0.73€ cuando `libro_spread` < 0.06 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `2646.8744` → IC=+0.162 (n=397)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 2646.8744 (IC base=+0.138)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.221 (n=1143)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.215)

- **PATRÓN** `drift_60min` |x|≤ `0.3035` → IC=+0.222 (n=1143)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.3654` → IC=+0.286 (n=1142)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3654 (IC base=+0.215)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.745` → IC=+0.240 (n=248)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.745 (IC base=+0.215)

- **PATRÓN** `volumen_regimen` < `1.2618` → IC=+0.191 (n=886)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` < 1.2618 (IC base=+0.215)

- **PATRÓN** `volumen_regimen` > `0.8864` → IC=+0.208 (n=591)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8864 (IC base=+0.215)

- **PATRÓN** `volumen_pendiente_norm` > `0.3245` → IC=+0.293 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3245 (IC base=+0.215)

- **PATRÓN** `volumen_spike_ratio` < `2.0729` → IC=+0.235 (n=361)
...
- **PATRÓN** `volumen_spike_ratio` > `3.2928` → IC=+0.249 (n=181)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.2928 (IC base=+0.215)

- **PATRÓN** `ballena_activa_n` < `155.0` → IC=+0.277 (n=101)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 155.0 (IC base=+0.215)

### GBM_LATE_15M_MULTIHORIZONTE#BNB#15min
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.157 (n=100)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0058 (IC base=+0.154)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.511` → IC=+0.355 (n=143)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.511 (IC base=+0.154)

- **PATRÓN** `volumen_pendiente_norm` > `0.1485` → IC=+0.167 (n=67)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.1485 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` > `1.4363` → IC=+0.120 (n=222)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_spike_ratio` > 1.4363 (IC base=+0.154)

- **PATRÓN** `libro_spread` < `0.08` → IC=+0.176 (n=322)
...
- **PATRÓN** `sigma_h` < `0.0018` → IC=+0.199 (n=71)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0018 (IC base=+0.188)

- **PATRÓN** `sigma_h` > `0.002` → IC=+0.190 (n=188)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.2889` → IC=+0.237 (n=211)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2889 (IC base=+0.188)
...
- **PATRÓN** `volumen_regimen` < `0.6412` → IC=+0.240 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6412 (IC base=+0.188)

- **PATRÓN** `volumen_regimen` > `1.092` → IC=+0.194 (n=96)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 1.092 (IC base=+0.188)

- **PATRÓN** `volumen_pendiente_norm` < `0.2719` → IC=+0.212 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2719 (IC base=+0.188)
...
- **PATRÓN** `libro_liquidez` > `10996.8422` → IC=+0.194 (n=70)
  ...
  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 10996.8422 (IC base=+0.188)

### GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.192 (n=92)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0075 (IC base=+0.155)

- **PATRÓN** `drift_60min` |x|≤ `0.1483` → IC=+0.167 (n=184)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.1483 (IC base=+0.155)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.179 (n=104)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 16.0 (IC base=+0.155)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.868` → IC=+0.333 (n=76)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.868 (IC base=+0.155)

- **PATRÓN** `volumen_pendiente_norm` < `0.2322` → IC=+0.139 (n=214)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_pendiente_norm` < 0.2322 (IC base=+0.155)

- **PATRÓN** `volumen_spike_ratio` < `2.0469` → IC=+0.211 (n=95)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0469 (IC base=+0.155)

- **PATRÓN** `volumen_spike_ratio` > `4.0243` → IC=+0.150 (n=98)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 4.0243 (IC base=+0.155)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.208 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.155)

- **PATRÓN** `libro_liquidez` > `1956.8086` → IC=+0.202 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1956.8086 (IC base=+0.155)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0067` → IC=+0.198 (n=492)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.298` → IC=+0.230 (n=991)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.298 (IC base=+0.116)

- **PATRÓN** `volumen_pendiente_norm` > `0.1804` → IC=+0.137 (n=337)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_pendiente_norm` > 0.1804 (IC base=+0.116)

- **PATRÓN** `libro_liquidez` > `2693.0092` → IC=+0.136 (n=490)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 2693.0092 (IC base=+0.116)

- **PATRÓN** `ballena_activa_n` < `107.0` → IC=+0.211 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 107.0 (IC base=+0.116)

- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.233 (n=1103)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0061 (IC base=+0.228)

- **PATRÓN** `sigma_h` > `0.0038` → IC=+0.236 (n=1254)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0038 (IC base=+0.228)

- **PATRÓN** `drift_60min` |x|≤ `0.2373` → IC=+0.229 (n=1103)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2373 (IC base=+0.228)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.238 (n=1333)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.228)

- **PATRÓN** `ibs_20min` < `0.4891` → IC=+0.289 (n=1254)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4891 (IC base=+0.228)

- **PATRÓN** `dist_vwap_pct` < `0.1542` → IC=+0.202 (n=853)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1542 (IC base=+0.228)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.153` → IC=+0.258 (n=246)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.153 (IC base=+0.228)

- **PATRÓN** `volumen_regimen` < `0.6189` → IC=+0.207 (n=281)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6189 (IC base=+0.228)

- **PATRÓN** `volumen_regimen` > `1.0794` → IC=+0.223 (n=380)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0794 (IC base=+0.228)

- **PATRÓN** `volumen_pendiente_norm` > `0.1855` → IC=+0.297 (n=200)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1855 (IC base=+0.228)

- **PATRÓN** `volumen_spike_ratio` < `1.6694` → IC=+0.290 (n=250)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6694 (IC base=+0.228)

### GBM_LATE_15M_TARDIO#BNB#15min
- **PATRÓN** `sigma_h` > `0.0063` → IC=+0.192 (n=245)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2094` → IC=+0.148 (n=52)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` > 0.2094 (IC base=+0.134)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.181 (n=255)

  - _Acción_: Kelly boost +0.90€ cuando `libro_spread` < 0.06 (IC base=+0.134)
...
- **PATRÓN** `drift_60min` |x|≤ `0.05` → IC=+0.206 (n=32)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.05 (IC base=+0.146)
...
- **PATRÓN** `ibs_20min` < `0.1374` → IC=+0.300 (n=63)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1374 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `0.1453` → IC=+0.239 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1453 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.753` → IC=+0.261 (n=44)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.753 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` < `0.8856` → IC=+0.218 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8856 (IC base=+0.146)

- **PATRÓN** `volumen_pendiente_norm` < `0.0641` → IC=+0.382 (n=32)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8037` → IC=+0.362 (n=27)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8037 (IC base=+0.146)
...
- **PATRÓN** `sigma_h` < `0.0071` → IC=+0.282 (n=319)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0071 (IC base=+0.258)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.278 (n=340)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.258)

- **PATRÓN** `ibs_20min` < `0.1731` → IC=+0.374 (n=213)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1731 (IC base=+0.258)

- **PATRÓN** `dist_vwap_pct` > `0.4043` → IC=+0.372 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4043 (IC base=+0.258)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.941` → IC=+0.350 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.941 (IC base=+0.258)

- **PATRÓN** `volumen_regimen` > `0.8907` → IC=+0.304 (n=212)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8907 (IC base=+0.258)

- **PATRÓN** `volumen_pendiente_norm` > `0.2776` → IC=+0.386 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2776 (IC base=+0.258)

- **PATRÓN** `volumen_spike_ratio` < `1.5061` → IC=+0.245 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5061 (IC base=+0.258)

- **PATRÓN** `volumen_spike_ratio` > `2.6958` → IC=+0.265 (n=66)
...
- **PATRÓN** `drift_20min_pct` |x|≤ `0.0526` → IC=+0.217 (n=51)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `drift_20min_pct` |x|≤ 0.0526 (IC base=+0.101)

- **PATRÓN** `libro_liquidez` > `14257.7047` → IC=+0.162 (n=69)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 14257.7047 (IC base=+0.101)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.191 (n=53)
...
- **PATRÓN** `hora_utc` < `2.0` → IC=+0.243 (n=72)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 2.0 (IC base=+0.032)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `libro_liquidez` < `2127.3472` → IC=-0.179 (n=26)

  - _Acción_: SKIP cuando `libro_liquidez` < 2127.3472
  - _Potencial_: sin este filtro IC_bueno=+0.136 (n=9)

### MOMENTUM_IBS_15M_BALLENA#BTC#15min
- **FILTRO** `libro_liquidez` < `14831.7206` → IC=-0.167 (n=22)
...
- **PATRÓN** `ibs_20min` > `0.0944` → IC=+0.206 (n=15)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.0944 (IC base=+0.144)

### MOMENTUM_IBS_15M_BALLENA#SOL#15min
- **FILTRO** `py_entrada` < `0.505` → IC=-0.167 (n=28)
...
- **FILTRO** `ballena_activa_n` > `58.0` → IC=-0.189 (n=43)
...
- **FILTRO** `ballena_activa_n` > `5.0` → IC=-0.161 (n=54)
...
- **PATRÓN** `libro_liquidez` > `3923.0651` → IC=+0.147 (n=32)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 3923.0651 (IC base=+0.011)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.156 (n=30)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 18.0 (IC base=+0.052)
...
- **PATRÓN** `libro_liquidez` > `9403.8686` → IC=+0.174 (n=44)
  ...
  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 9403.8686 (IC base=+0.028)
...
- **PATRÓN** `ibs_15` < `0.5385` → IC=+0.127 (n=330)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.63€ cuando `ibs_15` < 0.5385 (IC base=+0.081)

- **PATRÓN** `dist_vwap_pct` > `0.5736` → IC=+0.237 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5736 (IC base=+0.081)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1514` → IC=+0.136 (n=31)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.1514 (IC base=+0.040)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.866` → IC=+0.239 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.866 (IC base=+0.040)
...
- **PATRÓN** `hora_utc` < `2.0` → IC=+0.152 (n=21)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 2.0 (IC base=+0.071)

- **PATRÓN** `ibs_15` > `0.0732` → IC=+0.141 (n=37)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.71€ cuando `ibs_15` > 0.0732 (IC base=+0.071)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.724` → IC=+0.147 (n=32)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` < 3.724 (IC base=+0.071)

- **PATRÓN** `ballena_activa_n` < `1.0` → IC=+0.206 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 1.0 (IC base=+0.071)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1495` → IC=+0.210 (n=36)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1495 (IC base=+0.099)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0943` → IC=+0.156 (n=94)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.78€ cuando `delta_ratio_macro` |x|> 0.0943 (IC base=+0.099)

- **PATRÓN** `hora_utc` < `19.0` → IC=+0.139 (n=106)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` < 19.0 (IC base=+0.099)

- **PATRÓN** `ibs_15` < `0.2` → IC=+0.212 (n=71)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.2 (IC base=+0.099)

- **PATRÓN** `dist_vwap_pct` > `0.127` → IC=+0.250 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.127 (IC base=+0.099)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.148` → IC=+0.141 (n=90)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` < 5.148 (IC base=+0.099)

- **PATRÓN** `libro_liquidez` > `2522.0608` → IC=+0.136 (n=105)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 2522.0608 (IC base=+0.099)
...
- **FILTRO** `ibs_15` < `0.5714` → IC=-0.271 (n=107)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.252 (n=107)
...
- **PATRÓN** `ibs_15` > `0.5714` → IC=+0.252 (n=107)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5714 (IC base=-0.030)
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.2 sube el IC de +0.099 a +0.212 en UPDOWN_GBM#XRP#15min (n=71). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.5714 sube el IC de -0.030 a +0.252 en UPDOWN_GBM_15M_TARDIO (n=107). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 3164 | -0.116 | -474.43€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 439 | -0.006 | -4.24€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 2725 | -0.134 | -470.20€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 353 | -0.199 | -105.65€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 353 | -0.199 | -105.65€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 439 | -0.006 | -4.24€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 439 | -0.006 | -4.24€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 298 | -0.167 | -147.44€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 298 | -0.167 | -147.44€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 715 | -0.136 | -32.97€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 715 | -0.136 | -32.97€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 747 | -0.018 | -88.94€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 747 | -0.018 | -88.94€ | 1 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP | 612 | -0.217 | -95.19€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP#5min | 612 | -0.217 | -95.19€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 13466 | +0.116 | -791.95€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3264 | +0.179 | -122.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 98 | -0.100 | -46.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 7538 | +0.086 | -639.19€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 3101 | +0.140 | -7.56€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 889 | +0.191 | -41.53€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 41 | -0.105 | -20.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 1255 | +0.111 | -9.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 916 | +0.139 | +64.22€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 1271 | +0.053 | -225.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 9 | -0.021 | -3.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 1261 | +0.054 | -220.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 3354 | +0.125 | -54.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1204 | +0.158 | -28.40€ | 0 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 1253 | +0.103 | -17.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 885 | +0.116 | +0.01€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 3196 | +0.137 | -194.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1137 | +0.196 | -48.45€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 39 | +0.012 | -7.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 1255 | +0.086 | -89.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 765 | +0.138 | -48.04€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 1269 | +0.124 | -32.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 12 | +0.043 | +1.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 1256 | +0.125 | -32.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 3334 | +0.164 | -302.96€ | 2 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 3334 | +0.164 | -302.96€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 828 | +0.161 | -100.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 828 | +0.161 | -100.76€ | 0 | 4 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 5381 | +0.194 | -470.23€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 5381 | +0.194 | -470.23€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 828 | +0.254 | -0.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 828 | +0.254 | -0.97€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 1911 | +0.151 | +107.73€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 1911 | +0.151 | +107.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 950 | +0.154 | +60.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 950 | +0.154 | +60.93€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 961 | +0.147 | +46.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 961 | +0.147 | +46.80€ | 0 | 6 |
...
| ✅ GBM_LATE_15M | 4317 | +0.090 | +1546.76€ | 0 | 15 |
| ✅ GBM_LATE_15M#15min | 4317 | +0.090 | +1546.76€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 749 | +0.176 | +471.51€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 749 | +0.176 | +471.51€ | 0 | 18 |
| ✅ GBM_LATE_15M#BTC | 409 | +0.172 | +186.64€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 409 | +0.172 | +186.64€ | 0 | 22 |
| ✅ GBM_LATE_15M#DOGE | 751 | +0.195 | +522.51€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 751 | +0.195 | +522.51€ | 0 | 19 |
| ✅ GBM_LATE_15M#ETH | 584 | +0.003 | +40.32€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 584 | +0.003 | +40.32€ | 0 | 3 |
| ✅ GBM_LATE_15M#SOL | 819 | +0.009 | +81.90€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 819 | +0.009 | +81.90€ | 3 | 3 |
| ✅ GBM_LATE_15M#XRP | 1005 | +0.031 | +243.88€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1005 | +0.031 | +243.88€ | 0 | 4 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 5333 | +0.052 | +1616.08€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 5333 | +0.052 | +1616.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 1024 | -0.026 | +190.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 1024 | -0.026 | +190.64€ | 1 | 2 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 1009 | +0.001 | +126.87€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 1009 | +0.001 | +126.87€ | 0 | 2 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 640 | +0.240 | +584.11€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 640 | +0.240 | +584.11€ | 0 | 19 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 930 | -0.009 | +38.09€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 930 | -0.009 | +38.09€ | 4 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 959 | +0.005 | +92.21€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 959 | +0.005 | +92.21€ | 0 | 1 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 771 | +0.200 | +584.16€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 771 | +0.200 | +584.16€ | 0 | 20 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 3108 | +0.176 | +2115.70€ | 0 | 22 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 3108 | +0.176 | +2115.70€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB | 554 | +0.191 | +398.52€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB#15min | 554 | +0.191 | +398.52€ | 0 | 17 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC | 404 | +0.195 | +269.17€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC#15min | 404 | +0.195 | +269.17€ | 0 | 24 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE | 551 | +0.207 | +439.27€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min | 551 | +0.207 | +439.27€ | 0 | 21 |
...
| ✅ GBM_LATE_15M_TARDIO | 3630 | +0.167 | +2330.93€ | 0 | 19 |
| ✅ GBM_LATE_15M_TARDIO#15min | 3630 | +0.167 | +2330.93€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 720 | +0.187 | +504.47€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 720 | +0.187 | +504.47€ | 0 | 17 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 476 | +0.151 | +246.86€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 476 | +0.151 | +246.86€ | 1 | 22 |
| ✅ GBM_LATE_15M_TARDIO#DOGE | 719 | +0.224 | +612.12€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#DOGE#15min | 719 | +0.224 | +612.12€ | 0 | 15 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 315 | +0.109 | +119.62€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 315 | +0.109 | +119.62€ | 1 | 19 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 588 | +0.078 | +241.03€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 588 | +0.078 | +241.03€ | 1 | 14 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 812 | +0.195 | +606.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 812 | +0.195 | +606.84€ | 0 | 20 |
...
| ✅ MOMENTUM_IBS_15M | 369 | +0.047 | +15.68€ | 1 | 2 |
| ✅ MOMENTUM_IBS_15M#15min | 369 | +0.047 | +15.68€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_15M#ETH | 64 | +0.136 | +24.08€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH#15min | 64 | +0.136 | +24.08€ | 0 | 1 |
...
| ✅ MOMENTUM_IBS_15M_BALLENA | 446 | -0.004 | +19.87€ | 2 | 1 |
| ✅ MOMENTUM_IBS_15M_BALLENA#15min | 446 | -0.004 | +19.87€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB | 70 | -0.028 | +15.92€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB#15min | 70 | -0.028 | +15.92€ | 1 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC | 75 | -0.045 | -6.06€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC#15min | 75 | -0.045 | -6.06€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE | 75 | +0.019 | +10.21€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE#15min | 75 | +0.019 | +10.21€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH | 75 | +0.058 | +6.59€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH#15min | 75 | +0.058 | +6.59€ | 0 | 2 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL | 73 | -0.060 | -10.57€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL#15min | 73 | -0.060 | -10.57€ | 1 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP | 78 | +0.025 | +3.77€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP#15min | 78 | +0.025 | +3.77€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 1119 | -0.067 | -92.20€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 1119 | -0.067 | -92.20€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 171 | -0.049 | +14.74€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 171 | -0.049 | +14.74€ | 3 | 1 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 207 | -0.074 | -22.96€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 207 | -0.074 | -22.96€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 186 | -0.080 | -32.97€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 186 | -0.080 | -32.97€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 167 | -0.104 | -20.66€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 167 | -0.104 | -20.66€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 197 | -0.048 | -12.90€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 197 | -0.048 | -12.90€ | 4 | 2 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 191 | -0.049 | -17.44€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 191 | -0.049 | -17.44€ | 6 | 2 |
| ✅ MOMENTUM_IBS_5M_FADE | 1146 | +0.016 | -0.25€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 1146 | +0.016 | -0.25€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB | 193 | +0.038 | +9.29€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB#5min | 193 | +0.038 | +9.29€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC | 169 | +0.032 | -1.21€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC#5min | 169 | +0.032 | -1.21€ | 2 | 1 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE | 196 | +0.005 | -1.11€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE#5min | 196 | +0.005 | -1.11€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH | 200 | -0.005 | -5.97€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH#5min | 200 | -0.005 | -5.97€ | 1 | 1 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL | 200 | -0.005 | -2.68€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL#5min | 200 | -0.005 | -2.68€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP | 188 | +0.032 | +1.43€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP#5min | 188 | +0.032 | +1.43€ | 1 | 0 |
...
| ✅ STRUCT_NO_15M | 1781 | +0.010 | -13.19€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 1781 | +0.010 | -13.19€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 679 | +0.005 | -9.12€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 679 | +0.005 | -9.12€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 706 | +0.014 | -2.68€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 706 | +0.014 | -2.68€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 396 | +0.013 | -1.39€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 396 | +0.013 | -1.39€ | 2 | 0 |
| ✅ UPDOWN_GBM | 1958 | +0.020 | +100.90€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 851 | +0.088 | +148.46€ | 1 | 7 |
| ✅ UPDOWN_GBM#240min | 112 | +0.026 | +0.60€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 702 | -0.033 | -37.01€ | 4 | 0 |
...
| ✅ UPDOWN_GBM#BTC | 346 | -0.006 | -11.31€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 118 | +0.050 | -4.61€ | 2 | 6 |
| ✅ UPDOWN_GBM#BTC#240min | 33 | +0.071 | +2.23€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#5min | 92 | -0.021 | -1.04€ | 2 | 2 |
| ✅ UPDOWN_GBM#BTC#60min | 85 | -0.063 | -9.73€ | 1 | 0 |
| ✅ UPDOWN_GBM#BTC#daily | 18 | -0.135 | +1.83€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 290 | +0.003 | +2.72€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 54 | +0.143 | +18.37€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 236 | -0.029 | -15.65€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 457 | +0.053 | +44.91€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 202 | +0.132 | +51.63€ | 1 | 15 |
| ✅ UPDOWN_GBM#ETH#240min | 34 | +0.056 | +0.63€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 109 | -0.013 | -5.25€ | 3 | 4 |
| ✅ UPDOWN_GBM#ETH#60min | 97 | +0.005 | -1.72€ | 0 | 0 |
| 🚫 UPDOWN_GBM#ETH#daily | 15 | -0.154 | -0.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 343 | -0.001 | +6.09€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 137 | +0.018 | +1.07€ | 2 | 2 |
...
| ✅ UPDOWN_GBM#XRP | 440 | +0.025 | +49.63€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 267 | +0.091 | +69.68€ | 0 | 14 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1561 | -0.043 | +174.66€ | 2 | 2 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1561 | -0.043 | +174.66€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 101 | -0.073 | -3.12€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 101 | -0.073 | -3.12€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 311 | -0.126 | -1.23€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 311 | -0.126 | -1.23€ | 3 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 38 | +0.000 | +2.64€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 38 | +0.000 | +2.64€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 127 | -0.012 | +16.14€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 127 | -0.012 | +16.14€ | 2 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 498 | -0.006 | +95.42€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 498 | -0.006 | +95.42€ | 1 | 1 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 486 | -0.033 | +64.81€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 486 | -0.033 | +64.81€ | 3 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.156: overbought→boost, oversold→filtro | oversold(IBS<0.3): IC=-0.035 n=664 | neutral: IC=+0.004 n=672 | overbought(IBS>0.7): IC=+0.121 n=917
  - _Datos_: n=2412 IC=+0.040 PNL=+171.08€
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=137 IC=+0.018 PNL=+1.07€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.109 n=35445 | tras_1loss IC=+0.065 n=24529 | tras_2loss IC=+0.022 n=10624/40 | gap=+0.087 (umbral 0.05)
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=+0.056 n=131 | contrario_BTC IC=-0.020 n=73/40 | gap=-0.076 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1821 IC=+0.019 PNL=+99.92€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1821 IC=+0.019 PNL=+99.92€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.083 > 0.08 con n=535 PNL=+67.00€
  - _Datos_: n=535 IC=+0.083 PNL=+67.00€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=371 IC=+0.095 PNL=+92.08€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=371 IC=+0.095 PNL=+92.08€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=949 IC=+0.018 PNL=+19.01€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=949 IC=+0.018 PNL=+19.01€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.146 > 0.1 con n=535 PNL=+153.26€
  - _Datos_: n=535 IC=+0.146 PNL=+153.26€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=140 IC=+0.099 PNL=+37.73€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=140 IC=+0.099 PNL=+37.73€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1139 IC=-0.132 PNL=+119.11€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1139 IC=-0.132 PNL=+119.11€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=752 IC=+0.159 PNL=+400.54€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=752 IC=+0.159 PNL=+400.54€
...
**〰️ H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: n=539 IC=+0.040 PNL=+160.24€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=539 IC=+0.040 PNL=+160.24€
