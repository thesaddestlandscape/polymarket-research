# Hipótesis automáticas — 2026-07-15 18:51 UTC
_Generado por shadow_postmortem.py sobre 15778 resoluciones (PNL=+3165.57€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.214 (n=309)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.198 (n=938)

  - _Acción_: Kelly boost +0.99€ cuando `libro_spread` < 0.01 (IC base=+0.194)

- **PATRÓN** `libro_liquidez` > `4414.67` → IC=+0.194 (n=534)
...
### GBM_LATE_15M
- **PATRÓN** `dist_vwap_pct` > `0.6583` → IC=+0.134 (n=121)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` > 0.6583 (IC base=+0.109)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.281` → IC=+0.222 (n=106)
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.140 (n=212)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.223 (n=308)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0108 (IC base=+0.163)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.178 (n=489)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.3705` → IC=+0.217 (n=164)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3705 (IC base=+0.163)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.074` → IC=+0.297 (n=77)
...
- **PATRÓN** `sigma_h` > `0.0028` → IC=+0.145 (n=170)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` > 0.0028 (IC base=+0.121)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.172 (n=120)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 12.0 (IC base=+0.121)

- **PATRÓN** `dist_vwap_pct` > `0.3404` → IC=+0.167 (n=22)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.3404 (IC base=+0.121)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.412` → IC=+0.214 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.412 (IC base=+0.121)
...
- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.229 (n=83)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0069 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.169 (n=128)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.3418` → IC=+0.205 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3418 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` < `1.1747` → IC=+0.145 (n=122)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 1.1747 (IC base=+0.147)
...
- **PATRÓN** `sigma_h` > `0.0133` → IC=+0.198 (n=84)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0133 (IC base=+0.137)
...
- **PATRÓN** `dist_vwap_pct` > `0.3907` → IC=+0.192 (n=50)
  ...
  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.3907 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `1.0756` → IC=+0.150 (n=135)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 1.0756 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.236` → IC=+0.263 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.236 (IC base=+0.137)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0148` → IC=+0.181 (n=208)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0148 (IC base=+0.109)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.130 (n=557)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` < 15.0 (IC base=+0.109)

- **PATRÓN** `dist_vwap_pct` > `0.7311` → IC=+0.194 (n=106)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.7311 (IC base=+0.109)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.175` → IC=+0.310 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.175 (IC base=+0.109)
...
- **PATRÓN** `dist_vwap_pct` > `0.3604` → IC=+0.140 (n=23)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.3604 (IC base=+0.087)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.72` → IC=+0.208 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.72 (IC base=+0.087)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.149 (n=55)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0039 (IC base=+0.056)

- **PATRÓN** `dist_vwap_pct` > `0.6728` → IC=+0.159 (n=42)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.6728 (IC base=+0.056)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.728` → IC=+0.357 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.728 (IC base=+0.056)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2255 | +0.175 | -34.90€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1453 | +0.207 | +7.91€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 802 | +0.177 | +10.91€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 481 | +0.216 | +21.68€ | 0 | 6 |
...
| ✅ GBM_LATE_15M | 4661 | +0.102 | +1433.15€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 4661 | +0.102 | +1433.15€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1121 | +0.064 | +156.38€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1121 | +0.064 | +156.38€ | 0 | 2 |
| ✅ GBM_LATE_15M#ETH | 1130 | +0.076 | +196.51€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1130 | +0.076 | +196.51€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1240 | +0.110 | +495.76€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1240 | +0.110 | +495.76€ | 0 | 6 |
| ✅ GBM_LATE_15M#XRP | 1170 | +0.155 | +584.50€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1170 | +0.155 | +584.50€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1911 | +0.151 | +1116.28€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1911 | +0.151 | +1116.28€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 490 | +0.116 | +241.44€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 490 | +0.116 | +241.44€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 505 | +0.129 | +234.51€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 505 | +0.129 | +234.51€ | 0 | 14 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 515 | +0.109 | +223.07€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 515 | +0.109 | +223.07€ | 1 | 10 |
...
| ✅ GBM_LATE_15M_TARDIO | 1864 | +0.087 | +547.00€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1864 | +0.087 | +547.00€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 466 | +0.058 | +84.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 466 | +0.058 | +84.34€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 422 | +0.019 | +16.04€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 422 | +0.019 | +16.04€ | 0 | 3 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.101 n=9134 | tras_1loss IC=+0.064 n=6435 | tras_2loss IC=+0.031 n=2772/40 | gap=+0.070 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=13320 IC=+0.103 PNL=+3241.45€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=13320 IC=+0.103 PNL=+3241.45€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.257 > 0.2 con n=319 PNL=+223.51€
  - _Datos_: n=319 IC=+0.257 PNL=+223.51€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.109 > 0.08 con n=126 PNL=+50.00€
  - _Datos_: n=126 IC=+0.109 PNL=+50.00€
...
**🔶 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.142 n=174) pero sin cruzar ≥2 pares más — BTC: n=128 IC=-0.008; ETH: n=98 IC=+0.000; SOL: n=180 IC=+0.060 ✓
