# Hipótesis automáticas — 2026-07-15 18:36 UTC
_Generado por shadow_postmortem.py sobre 15765 resoluciones (PNL=+3171.02€)_
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.180 (n=336)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.275` → IC=+0.320 (n=321)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.275 (IC base=+0.157)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.168 (n=1016)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.01 (IC base=+0.157)

- **PATRÓN** `libro_liquidez` > `7405.3527` → IC=+0.182 (n=297)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 7405.3527 (IC base=+0.157)
...
- **PATRÓN** `libro_liquidez` > `7271.3027` → IC=+0.203 (n=170)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.225 (n=96)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.365` → IC=+0.255 (n=165)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.120 (n=1478)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` > 8.0 (IC base=+0.109)

- **PATRÓN** `dist_vwap_pct` > `0.6609` → IC=+0.134 (n=121)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` > 0.6609 (IC base=+0.109)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.281` → IC=+0.229 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.281 (IC base=+0.109)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.133 (n=1192)
...
- **PATRÓN** `dist_vwap_pct` > `0.2225` → IC=+0.122 (n=35)
  ...
  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` > 0.2225 (IC base=+0.078)
...
- **PATRÓN** `dist_vwap_pct` < `0.5451` → IC=+0.131 (n=158)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.5451 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.722` → IC=+0.127 (n=140)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 7.722 (IC base=+0.162)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0123` → IC=+0.234 (n=227)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0123 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.182 (n=486)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 12.0 (IC base=+0.165)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.170 (n=717)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 18.0 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` > `0.3709` → IC=+0.221 (n=163)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3709 (IC base=+0.165)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.175 (n=705)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0048 (IC base=+0.158)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.160 (n=478)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.4441` → IC=+0.190 (n=476)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.4441 (IC base=+0.158)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.074` → IC=+0.250 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.074 (IC base=+0.158)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **PATRÓN** `sigma_h` > `0.0028` → IC=+0.149 (n=169)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0028 (IC base=+0.124)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.178 (n=119)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 12.0 (IC base=+0.124)

- **PATRÓN** `dist_vwap_pct` > `0.3404` → IC=+0.167 (n=22)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.3404 (IC base=+0.124)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.412` → IC=+0.241 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.412 (IC base=+0.124)
...
- **PATRÓN** `sigma_h` > `0.0132` → IC=+0.209 (n=84)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0132 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.156 (n=62)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `1.0756` → IC=+0.154 (n=134)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 1.0756 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.082` → IC=+0.263 (n=57)
...
- **PATRÓN** `sigma_h` < `0.0159` → IC=+0.286 (n=157)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0159 (IC base=+0.286)

- **PATRÓN** `sigma_h` > `0.0085` → IC=+0.323 (n=156)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0085 (IC base=+0.286)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.291 (n=108)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.5869` → IC=+0.306 (n=127)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5869 (IC base=+0.286)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.933` → IC=+0.296 (n=106)
...
- **PATRÓN** `dist_vwap_pct` > `0.7123` → IC=+0.204 (n=106)
...
- **PATRÓN** `dist_vwap_pct` > `0.3646` → IC=+0.167 (n=22)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.3646 (IC base=+0.090)
...
- **PATRÓN** `sigma_h` > `0.023` → IC=+0.233 (n=73)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.023 (IC base=+0.191)

- **PATRÓN** `hora_utc` < `9.0` → IC=+0.220 (n=105)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 9.0 (IC base=+0.191)

- **PATRÓN** `dist_vwap_pct` > `0.3325` → IC=+0.181 (n=45)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.3325 (IC base=+0.191)

- **PATRÓN** `dist_vwap_pct` < `0.5109` → IC=+0.175 (n=158)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` < 0.5109 (IC base=+0.191)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.652` → IC=+0.182 (n=124)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 2.652 (IC base=+0.191)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2252 | +0.175 | -34.06€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1452 | +0.208 | +9.95€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 653 | +0.179 | -14.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 485 | +0.198 | -21.68€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 800 | +0.177 | +12.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 480 | +0.218 | +23.72€ | 0 | 5 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 792 | +0.170 | -28.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 480 | +0.209 | +11.64€ | 0 | 5 |
...
| ✅ GBM_LATE_15M | 4658 | +0.102 | +1434.10€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 4658 | +0.102 | +1434.10€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1120 | +0.064 | +158.20€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1120 | +0.064 | +158.20€ | 0 | 3 |
| ✅ GBM_LATE_15M#ETH | 1130 | +0.076 | +196.51€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1130 | +0.076 | +196.51€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1239 | +0.110 | +496.81€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1239 | +0.110 | +496.81€ | 0 | 6 |
| ✅ GBM_LATE_15M#XRP | 1169 | +0.155 | +582.58€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1169 | +0.155 | +582.58€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1907 | +0.152 | +1118.84€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1907 | +0.152 | +1118.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 489 | +0.117 | +242.62€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 489 | +0.117 | +242.62€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 504 | +0.130 | +236.55€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 504 | +0.130 | +236.55€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 514 | +0.111 | +224.32€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 514 | +0.111 | +224.32€ | 1 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 400 | +0.274 | +415.35€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 400 | +0.274 | +415.35€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 1861 | +0.087 | +548.11€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1861 | +0.087 | +548.11€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 465 | +0.059 | +85.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 465 | +0.059 | +85.84€ | 0 | 3 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 528 | +0.196 | +367.59€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 528 | +0.196 | +367.59€ | 0 | 10 |
...
| ✅ STREAK_FADE_15M | 198 | +0.090 | +21.94€ | 0 | 9 |
| ✅ STREAK_FADE_15M#15min | 198 | +0.090 | +21.94€ | 0 | 0 |
| ✅ STREAK_FADE_15M#ETH | 42 | +0.068 | -2.03€ | 0 | 0 |
| ✅ STREAK_FADE_15M#ETH#15min | 42 | +0.068 | -2.03€ | 0 | 0 |
| ✅ STREAK_FADE_15M#SOL | 69 | +0.176 | +27.03€ | 0 | 0 |
| ✅ STREAK_FADE_15M#SOL#15min | 69 | +0.176 | +27.03€ | 0 | 2 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.101 n=9126 | tras_1loss IC=+0.064 n=6430 | tras_2loss IC=+0.031 n=2771/40 | gap=+0.070 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=13307 IC=+0.104 PNL=+3246.90€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=13307 IC=+0.104 PNL=+3246.90€
...
**🔶 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.140 n=173) pero sin cruzar ≥2 pares más — BTC: n=128 IC=-0.008; ETH: n=98 IC=+0.000; SOL: n=179 IC=+0.064 ✓
  - _Datos_: n=173 IC=+0.140 PNL=+92.29€
