# Hipótesis automáticas — 2026-07-12 20:41 UTC
_Generado por shadow_postmortem.py sobre 11401 resoluciones (PNL=+2198.75€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.228 (n=377)
...
- **PATRÓN** `py_entrada` > `0.585` → IC=+0.256 (n=395)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.585 (IC base=+0.224)

- **PATRÓN** `libro_liquidez` > `2902.992` → IC=+0.243 (n=336)
...
- **PATRÓN** `py_entrada` < `0.665` → IC=+0.254 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.665 (IC base=+0.245)

- **PATRÓN** `py_entrada` > `0.715` → IC=+0.305 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.715 (IC base=+0.245)

- **PATRÓN** `libro_liquidez` > `4211.4102` → IC=+0.304 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4211.4102 (IC base=+0.245)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.250 (n=58)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.625` → IC=+0.292 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.625 (IC base=+0.234)

- **PATRÓN** `libro_liquidez` > `1888.7041` → IC=+0.293 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1888.7041 (IC base=+0.234)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.123 (n=1380)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5618` → IC=+0.140 (n=48)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.5618 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` < `0.134` → IC=+0.204 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.134 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.067` → IC=+0.375 (n=22)
...
- **PATRÓN** `dist_vwap_pct` < `0.1905` → IC=+0.190 (n=98)
  ...
  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.1905 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.234` → IC=+0.161 (n=60)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` < 4.234 (IC base=+0.111)
...
- **PATRÓN** `sigma_h` < `0.0044` → IC=+0.170 (n=113)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0044 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` > `0.6749` → IC=+0.222 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6749 (IC base=+0.094)
...
- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.178 (n=119)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0092 (IC base=+0.152)

- **PATRÓN** `sigma_h` > `0.0148` → IC=+0.169 (n=119)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0148 (IC base=+0.152)
...
- **PATRÓN** `dist_vwap_pct` < `0.187` → IC=+0.224 (n=27)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.187 (IC base=+0.152)
...
- **PATRÓN** `sigma_h` > `0.0042` → IC=+0.180 (n=395)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.1606` → IC=+0.260 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1606 (IC base=+0.155)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.21` → IC=+0.274 (n=60)
...
- **PATRÓN** `sigma_h` > `0.0034` → IC=+0.140 (n=98)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` > 0.0034 (IC base=+0.106)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.139 (n=34)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5883` → IC=+0.231 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5883 (IC base=+0.106)
...
- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.241 (n=56)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0083 (IC base=+0.105)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.148 (n=52)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 6.0 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` < `0.152` → IC=+0.294 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.152 (IC base=+0.105)
...
- **FILTRO** `sigma_ewma_delta_pct` < `7.347` → IC=-0.150 (n=18)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.348 (n=31)

- **PATRÓN** `sigma_h` > `0.0135` → IC=+0.133 (n=145)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0135 (IC base=+0.085)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.125 (n=214)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` < 8.0 (IC base=+0.085)

- **PATRÓN** `dist_vwap_pct` > `0.4603` → IC=+0.141 (n=37)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.4603 (IC base=+0.085)

- **PATRÓN** `dist_vwap_pct` < `0.1391` → IC=+0.136 (n=75)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.1391 (IC base=+0.085)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.347` → IC=+0.348 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.347 (IC base=+0.085)

- **PATRÓN** `sigma_h` > `0.012` → IC=+0.136 (n=152)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.012 (IC base=+0.078)

- **PATRÓN** `hora_utc` < `9.0` → IC=+0.120 (n=214)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` < 9.0 (IC base=+0.078)

- **PATRÓN** `dist_vwap_pct` < `0.1825` → IC=+0.160 (n=92)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` < 0.1825 (IC base=+0.078)
...
- **PATRÓN** `dist_vwap_pct` > `0.6728` → IC=+0.206 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6728 (IC base=+0.026)
...
- **PATRÓN** `sigma_h` < `0.0081` → IC=+0.214 (n=40)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0081 (IC base=+0.025)

- **PATRÓN** `hora_utc` < `9.0` → IC=+0.161 (n=57)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 9.0 (IC base=+0.025)

- **PATRÓN** `dist_vwap_pct` < `0.1776` → IC=+0.204 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1776 (IC base=+0.025)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.125 (n=86)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=+0.099 (n=357)

- **PATRÓN** `ibs_15` > `0.7168` → IC=+0.161 (n=222)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.80€ cuando `ibs_15` > 0.7168 (IC base=+0.018)

- **PATRÓN** `dist_vwap_pct` > `0.1374` → IC=+0.178 (n=88)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.1374 (IC base=+0.018)
...
- **PATRÓN** `ibs_15` > `0.7617` → IC=+0.204 (n=86)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7617 (IC base=+0.029)

- **PATRÓN** `dist_vwap_pct` < `0.5787` → IC=+0.162 (n=66)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.5787 (IC base=+0.029)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 1029 | +0.187 | +24.52€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 662 | +0.215 | +18.93€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 365 | +0.168 | +2.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 219 | +0.206 | +4.10€ | 0 | 9 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 360 | +0.202 | +20.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 220 | +0.225 | +18.34€ | 0 | 8 |
...
| ✅ GBM_LATE_15M | 3688 | +0.114 | +1236.96€ | 0 | 9 |
| ✅ GBM_LATE_15M#15min | 3688 | +0.114 | +1236.96€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 902 | +0.073 | +129.09€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 902 | +0.073 | +129.09€ | 0 | 2 |
| ✅ GBM_LATE_15M#ETH | 933 | +0.073 | +160.21€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 933 | +0.073 | +160.21€ | 0 | 4 |
| ✅ GBM_LATE_15M#SOL | 963 | +0.137 | +478.03€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 963 | +0.137 | +478.03€ | 0 | 8 |
| ✅ GBM_LATE_15M#XRP | 890 | +0.170 | +469.62€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 890 | +0.170 | +469.62€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1036 | +0.167 | +614.47€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1036 | +0.167 | +614.47€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 271 | +0.167 | +178.88€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 271 | +0.167 | +178.88€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 277 | +0.134 | +130.04€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 277 | +0.134 | +130.04€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 289 | +0.122 | +109.04€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 289 | +0.122 | +109.04€ | 0 | 6 |
...
| ✅ GBM_LATE_15M_TARDIO | 1184 | +0.082 | +297.39€ | 1 | 8 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1184 | +0.082 | +297.39€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 273 | +0.086 | +45.62€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 273 | +0.086 | +45.62€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 296 | +0.007 | +12.30€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 296 | +0.007 | +12.30€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 308 | +0.032 | +34.92€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 308 | +0.032 | +34.92€ | 0 | 4 |
...
| ✅ UPDOWN_GBM | 1339 | -0.006 | +25.25€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1083 | +0.024 | +62.72€ | 1 | 5 |
...
| ✅ UPDOWN_GBM#ETH | 662 | +0.015 | +36.12€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 569 | +0.034 | +47.18€ | 0 | 2 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.128) — sin ventaja clara. oversold(IBS<0.3): IC=-0.020 n=348 | neutral: IC=-0.049 n=264 | overbought(IBS>0.7): IC=+0.079 n=295
  - _Datos_: n=949 IC=+0.009 PNL=+50.49€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.094 n=6474 | tras_1loss IC=+0.049 n=4789 | tras_2loss IC=+0.015 n=2135/40 | gap=+0.078 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1212 IC=+0.001 PNL=+39.25€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1212 IC=+0.001 PNL=+39.25€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=618 IC=+0.018 PNL=+25.07€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=618 IC=+0.018 PNL=+25.07€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=563 IC=+0.052 PNL=+45.09€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=563 IC=+0.052 PNL=+45.09€
...
**〰️ H-CUSTOM-CROSS-WINDOW-SPREAD-NEG** — GBM 15min/60min: spread negativo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: n=56 IC=+0.017 PNL=+25.05€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=56 IC=+0.017 PNL=+25.05€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8943 IC=+0.100 PNL=+2274.63€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8943 IC=+0.100 PNL=+2274.63€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=765 IC=-0.015 PNL=-5.40€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=765 IC=-0.015 PNL=-5.40€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.169 > 0.1 con n=282 PNL=+99.77€
  - _Datos_: n=282 IC=+0.169 PNL=+99.77€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.167 > 0.08 con n=214 PNL=+71.31€
  - _Datos_: n=214 IC=+0.167 PNL=+71.31€
...
**🔶 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA en BTC (IC=+0.223 n=92) pero sin cruzar ≥2 pares más — ETH: n=110 IC=+0.134 ✓; SOL: n=7 IC=-0.019; XRP: n=5 IC=+0.018
...
**⏳ H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: 22/40 ops en el filtro definido (IC actual=+0.167 PNL=+16.43€)
  - _Datos_: n=22 IC=+0.167 PNL=+16.43€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.312 > 0.08 con n=78 PNL=+8.50€
  - _Datos_: n=78 IC=+0.312 PNL=+8.50€
