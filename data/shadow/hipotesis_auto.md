# Hipótesis automáticas — 2026-08-18 16:45 UTC
_Generado por shadow_postmortem.py sobre 65283 resoluciones (PNL=+6636.32€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `n_ballenas` < `5.0` → IC=-0.127 (n=2055)

  - _Acción_: SKIP cuando `n_ballenas` < 5.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=1397)

- **FILTRO** `restante_s_al_confirmar` < `149.38` → IC=-0.257 (n=863)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 149.38
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=2589)
...
- **FILTRO** `restante_s_al_confirmar` < `247.16` → IC=-0.160 (n=609)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.197 (n=2240)

  - _Acción_: Kelly boost +0.99€ cuando `py_entrada` > 0.7 (IC base=+0.094)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.163 (n=1159)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.094)

- **PATRÓN** `libro_liquidez` > `2373.5135` → IC=+0.165 (n=1121)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 2373.5135 (IC base=+0.094)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.156 (n=3817)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.205 (n=361)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.200 (n=452)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.177 (n=571)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.73` → IC=+0.175 (n=598)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.215 (n=595)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.72` → IC=+0.187 (n=497)
...
- **PATRÓN** `hora_utc` > `14.0` → IC=+0.193 (n=2246)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.252 (n=1505)
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.137 (n=400)

  - _Acción_: Kelly boost +0.68€ cuando `py_entrada` > 0.73 (IC base=+0.098)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.291 (n=252)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.217 (n=270)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.227 (n=584)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.150)

- **PATRÓN** `restante_min` < `3.78` → IC=+0.167 (n=523)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.157 (n=1440)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `5.5` → IC=+0.214 (n=522)

  - _Acción_: Kelly boost +1.00€ cuando `lag_apertura_s` < 5.5 (IC base=+0.150)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.241 (n=291)
...
- **PATRÓN** `restante_min` > `4.88` → IC=+0.191 (n=273)

  - _Acción_: Kelly boost +0.95€ cuando `restante_min` > 4.88 (IC base=+0.155)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=806)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `7.36` → IC=+0.192 (n=258)
...
- **PATRÓN** `restante_min` < `3.88` → IC=+0.150 (n=264)

  - _Acción_: Kelly boost +0.75€ cuando `restante_min` < 3.88 (IC base=+0.145)

- **PATRÓN** `restante_min` > `4.95` → IC=+0.223 (n=283)

  - _Acción_: Kelly boost +1.00€ cuando `restante_min` > 4.95 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.152 (n=727)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 7.0 (IC base=+0.145)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.157 (n=793)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 16.0 (IC base=+0.145)

- **PATRÓN** `lag_apertura_s` < `3.23` → IC=+0.222 (n=264)

  - _Acción_: Kelly boost +1.00€ cuando `lag_apertura_s` < 3.23 (IC base=+0.145)

- **PATRÓN** `profundidad_ratio_no` > `12.5` → IC=+0.169 (n=264)

  - _Acción_: Kelly boost +0.85€ cuando `profundidad_ratio_no` > 12.5 (IC base=+0.145)
...
- **PATRÓN** `hora_utc` > `10.0` → IC=+0.275 (n=136)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1401.7964` → IC=+0.312 (n=62)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1401.7964 (IC base=+0.256)
...
- **PATRÓN** `ibs_20min` > `0.9923` → IC=+0.268 (n=412)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9923 (IC base=+0.104)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7088` → IC=+0.244 (n=166)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7088 (IC base=+0.079)

- **PATRÓN** `volumen_spike_ratio` > `2.8774` → IC=+0.228 (n=171)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8774 (IC base=+0.079)

- **PATRÓN** `ballena_activa_n` < `254.0` → IC=+0.288 (n=102)
...
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.178 (n=181)
  ...
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` > 0.007 (IC base=+0.124)
...
- **PATRÓN** `libro_spread` < `0.06` → IC=+0.163 (n=289)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.06 (IC base=+0.124)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1881` → IC=+0.170 (n=204)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.1881 (IC base=+0.149)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.196 (n=166)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 12.0 (IC base=+0.149)

- **PATRÓN** `ibs_20min` < `0.4787` → IC=+0.209 (n=232)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4787 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.472` → IC=+0.253 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.472 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` < `1.3019` → IC=+0.171 (n=232)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 1.3019 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` > `0.6842` → IC=+0.162 (n=208)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.6842 (IC base=+0.149)

- **PATRÓN** `volumen_pendiente_norm` > `0.1294` → IC=+0.274 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1294 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` < `1.5117` → IC=+0.278 (n=43)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5117 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `11623.6154` → IC=+0.188 (n=78)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 11623.6154 (IC base=+0.149)

- **PATRÓN** `ballena_activa_n` < `245.0` → IC=+0.289 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 245.0 (IC base=+0.149)
...
- **PATRÓN** `sigma_h` < `0.0054` → IC=+0.180 (n=23)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.0054 (IC base=+0.130)

- **PATRÓN** `hora_utc` > `19.0` → IC=+0.208 (n=22)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 19.0 (IC base=+0.130)

- **PATRÓN** `ibs_20min` > `0.5714` → IC=+0.250 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5714 (IC base=+0.130)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.475` → IC=+0.167 (n=25)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` < 4.475 (IC base=+0.130)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.192 (n=24)

  - _Acción_: Kelly boost +0.96€ cuando `libro_spread` < 0.01 (IC base=+0.130)
...
- **PATRÓN** `ibs_20min` > `0.9444` → IC=+0.233 (n=533)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9444 (IC base=+0.053)

- **PATRÓN** `dist_vwap_pct` > `0.1183` → IC=+0.175 (n=158)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.1183 (IC base=+0.053)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.204` → IC=+0.128 (n=988)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 2.204 (IC base=+0.053)

- **PATRÓN** `volumen_pendiente_norm` > `0.3377` → IC=+0.192 (n=102)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.3377 (IC base=+0.053)
...
- **PATRÓN** `ballena_activa_n` < `71.0` → IC=+0.284 (n=72)
...
- **PATRÓN** `volumen_regimen` < `0.5591` → IC=+0.184 (n=17)
  ...
  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 0.5591 (IC base=-0.005)
...
- **PATRÓN** `sigma_h` < `0.0047` → IC=+0.153 (n=416)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` > `0.9185` → IC=+0.251 (n=832)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9185 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.2198` → IC=+0.138 (n=277)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 0.2198 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.948` → IC=+0.263 (n=613)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.948 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` > `0.6287` → IC=+0.138 (n=649)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` > 0.6287 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` > `0.1127` → IC=+0.157 (n=412)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_pendiente_norm` > 0.1127 (IC base=+0.137)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.144 (n=1402)

  - _Acción_: Kelly boost +0.72€ cuando `libro_spread` < 0.06 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `2674.5492` → IC=+0.170 (n=416)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 2674.5492 (IC base=+0.137)

- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.223 (n=1198)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0063 (IC base=+0.215)

- **PATRÓN** `drift_60min` |x|≤ `0.2986` → IC=+0.221 (n=1197)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2986 (IC base=+0.215)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.272 (n=600)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 14.0 (IC base=+0.215)

- **PATRÓN** `ibs_20min` < `0.3729` → IC=+0.282 (n=1197)
...
- **PATRÓN** `volumen_regimen` < `1.2604` → IC=+0.193 (n=928)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` < 1.2604 (IC base=+0.215)

- **PATRÓN** `volumen_regimen` > `0.8805` → IC=+0.202 (n=619)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8805 (IC base=+0.215)

- **PATRÓN** `volumen_pendiente_norm` > `0.2721` → IC=+0.277 (n=155)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2721 (IC base=+0.215)

- **PATRÓN** `volumen_spike_ratio` < `2.0574` → IC=+0.235 (n=398)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0574 (IC base=+0.215)

- **PATRÓN** `volumen_spike_ratio` > `3.1581` → IC=+0.241 (n=199)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.1581 (IC base=+0.215)

- **PATRÓN** `ballena_activa_n` < `92.0` → IC=+0.263 (n=137)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 92.0 (IC base=+0.215)

### GBM_LATE_15M_MULTIHORIZONTE#BNB#15min
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.154 (n=105)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0058 (IC base=+0.153)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.0902` → IC=+0.146 (n=94)
  ...
  - _Acción_: Kelly boost +0.73€ cuando `volumen_pendiente_norm` > 0.0902 (IC base=+0.153)

- **PATRÓN** `libro_spread` < `0.08` → IC=+0.172 (n=336)
...
- **PATRÓN** `sigma_h` > `0.002` → IC=+0.190 (n=198)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.002 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.1928` → IC=+0.206 (n=195)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1928 (IC base=+0.189)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.252 (n=155)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.189)

- **PATRÓN** `ibs_20min` < `0.2977` → IC=+0.244 (n=221)
...
- **PATRÓN** `volumen_regimen` < `0.6392` → IC=+0.263 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6392 (IC base=+0.189)

- **PATRÓN** `volumen_pendiente_norm` > `0.1352` → IC=+0.269 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1352 (IC base=+0.189)

- **PATRÓN** `volumen_spike_ratio` < `2.7073` → IC=+0.267 (n=101)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.7073 (IC base=+0.189)

- **PATRÓN** `libro_liquidez` > `11146.3152` → IC=+0.210 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11146.3152 (IC base=+0.189)
...
- **FILTRO** `ibs_20min` > `0.5625` → IC=-0.250 (n=78)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.225 (n=238)
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.242 (n=87)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 16.0 (IC base=+0.107)

- **PATRÓN** `ibs_20min` < `0.5625` → IC=+0.225 (n=238)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5625 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.987` → IC=+0.250 (n=34)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.987 (IC base=+0.107)

- **PATRÓN** `volumen_regimen` > `0.871` → IC=+0.150 (n=158)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 0.871 (IC base=+0.107)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5471` → IC=+0.140 (n=109)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` > 1.5471 (IC base=+0.107)

- **PATRÓN** `libro_liquidez` > `2378.282` → IC=+0.228 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2378.282 (IC base=+0.107)
...
- **PATRÓN** `ibs_20min` > `0.96` → IC=+0.280 (n=689)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.96 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` > `0.3331` → IC=+0.167 (n=160)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.3331 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.268` → IC=+0.228 (n=1019)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.268 (IC base=+0.117)

- **PATRÓN** `volumen_pendiente_norm` > `0.1804` → IC=+0.133 (n=352)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_pendiente_norm` > 0.1804 (IC base=+0.117)

- **PATRÓN** `volumen_spike_ratio` > `1.6913` → IC=+0.121 (n=1084)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_spike_ratio` > 1.6913 (IC base=+0.117)

- **PATRÓN** `libro_liquidez` > `2716.8736` → IC=+0.150 (n=506)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 2716.8736 (IC base=+0.117)

- **PATRÓN** `ballena_activa_n` < `155.0` → IC=+0.185 (n=141)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.135 (n=382)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.349` → IC=+0.349 (n=144)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.349 (IC base=+0.133)

- **PATRÓN** `volumen_pendiente_norm` > `0.2097` → IC=+0.138 (n=56)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_pendiente_norm` > 0.2097 (IC base=+0.133)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.170 (n=271)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2333` → IC=+0.233 (n=148)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2333 (IC base=+0.177)
...
- **PATRÓN** `drift_20min_pct` |x|≤ `0.0531` → IC=+0.154 (n=76)
  ...
  - _Acción_: Kelly boost +0.77€ cuando `drift_20min_pct` |x|≤ 0.0531 (IC base=+0.069)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.201 (n=152)

  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.005 (n=184)

- **FILTRO** `ibs_20min` < `0.748` → IC=-0.174 (n=84)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.137 (n=155)

  - _Acción_: Kelly boost +0.68€ cuando `py_entrada` < 0.5 (IC base=+0.026)
...
- **PATRÓN** `ibs_20min` < `0.1608` → IC=+0.159 (n=39)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` < 0.1608 (IC base=+0.057)
...
- **FILTRO** `py_entrada` > `0.5` → IC=-0.159 (n=39)

  - _Acción_: SKIP cuando `py_entrada` > 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.145 (n=29)

- **FILTRO** `ibs_20min` > `0.2282` → IC=-0.265 (n=15)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.145 (n=29)
  ...
  - _Acción_: Kelly boost +0.73€ cuando `py_entrada` < 0.5 (IC base=-0.029)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.200 (n=18)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.097)

- **PATRÓN** `drift_20min_pct` |x|≤ `0.1191` → IC=+0.153 (n=47)

  - _Acción_: Kelly boost +0.77€ cuando `drift_20min_pct` |x|≤ 0.1191 (IC base=+0.097)

- **PATRÓN** `libro_liquidez` > `11243.3198` → IC=+0.194 (n=47)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 11243.3198 (IC base=+0.097)
...
- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.167 (n=19)
...
- **FILTRO** `ibs_20min` > `0.255` → IC=-0.206 (n=15)
...
- **PATRÓN** `py_entrada` < `0.51` → IC=+0.128 (n=41)
  ...
  - _Acción_: Kelly boost +0.64€ cuando `py_entrada` < 0.51 (IC base=+0.024)
...
- **FILTRO** `py_entrada` < `0.36` → IC=-0.312 (n=222)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=719)

- **FILTRO** `ibs_7min` < `0.7439` → IC=-0.238 (n=235)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7439
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=706)

- **FILTRO** `ballena_activa_n` > `23.0` → IC=-0.222 (n=228)

  - _Acción_: SKIP cuando `ballena_activa_n` > 23.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=713)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.185 (n=319)
...
- **FILTRO** `py_entrada` > `0.66` → IC=-0.167 (n=43)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=+0.042 (n=94)

- **FILTRO** `drift_7min_pct` |x|> `0.0862` → IC=-0.123 (n=67)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.0862
  - _Potencial_: sin este filtro IC_bueno=+0.069 (n=70)
...
- **FILTRO** `py_entrada` < `0.39` → IC=-0.231 (n=50)

  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=153)

- **FILTRO** `ballena_activa_n` > `96.0` → IC=-0.192 (n=50)
...
- **FILTRO** `ibs_7min` < `0.7295` → IC=-0.295 (n=42)
...
- **FILTRO** `py_entrada` > `0.5` → IC=-0.127 (n=108)
...
- **FILTRO** `drift_7min_pct` |x|> `0.1045` → IC=-0.143 (n=26)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1045
...
- **FILTRO** `ibs_15` < `0.5714` → IC=-0.155 (n=137)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.220 (n=280)

- **PATRÓN** `sigma_h` > `0.0047` → IC=+0.126 (n=209)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.63€ cuando `sigma_h` > 0.0047 (IC base=+0.097)

- **PATRÓN** `ibs_15` > `0.5714` → IC=+0.220 (n=280)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5714 (IC base=+0.097)

- **PATRÓN** `dist_vwap_pct` > `0.4246` → IC=+0.172 (n=62)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.4246 (IC base=+0.097)
...
- **PATRÓN** `ibs_15` < `0.5833` → IC=+0.122 (n=397)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.61€ cuando `ibs_15` < 0.5833 (IC base=+0.079)

- **PATRÓN** `dist_vwap_pct` > `0.5611` → IC=+0.219 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5611 (IC base=+0.079)
...
- **PATRÓN** `sigma_h` < `0.0078` → IC=+0.129 (n=122)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` < 0.0078 (IC base=+0.110)

- **PATRÓN** `drift_60min` |x|≤ `0.14` → IC=+0.198 (n=41)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.14 (IC base=+0.110)

- **PATRÓN** `drift_15min` |x|≤ `0.4729` → IC=+0.143 (n=54)

  - _Acción_: Kelly boost +0.71€ cuando `drift_15min` |x|≤ 0.4729 (IC base=+0.110)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0966` → IC=+0.176 (n=109)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.88€ cuando `delta_ratio_macro` |x|> 0.0966 (IC base=+0.110)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.138 (n=125)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` < 17.0 (IC base=+0.110)

- **PATRÓN** `ibs_15` < `0.234` → IC=+0.214 (n=82)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.234 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` > `0.1184` → IC=+0.294 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1184 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.148` → IC=+0.157 (n=103)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` < 5.148 (IC base=+0.110)

- **PATRÓN** `libro_liquidez` > `2541.6579` → IC=+0.153 (n=122)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 2541.6579 (IC base=+0.110)
...
- **FILTRO** `ibs_15` < `0.6216` → IC=-0.254 (n=112)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.230 (n=113)

- **FILTRO** `sigma_ewma_delta_pct` > `12.903` → IC=-0.156 (n=222)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 12.903
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=904)

- **PATRÓN** `ibs_15` > `0.6216` → IC=+0.230 (n=113)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.253 (n=79)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.165 (n=171)

- **FILTRO** `hora_utc` > `16.0` → IC=-0.209 (n=53)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.188 (n=197)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.5714 sube el IC de +0.097 a +0.220 en UPDOWN_GBM#15min (n=280). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.234 sube el IC de +0.110 a +0.214 en UPDOWN_GBM#XRP#15min (n=82). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.6216 sube el IC de -0.040 a +0.230 en UPDOWN_GBM_15M_TARDIO (n=113). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 3327 | -0.113 | -520.10€ | 2 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 456 | -0.022 | -17.20€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 2871 | -0.128 | -502.90€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 800 | -0.001 | -94.56€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 800 | -0.001 | -94.56€ | 1 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP | 673 | -0.220 | -119.05€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP#5min | 673 | -0.220 | -119.05€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 14276 | +0.116 | -820.50€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3373 | +0.180 | -117.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 99 | -0.104 | -47.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 8212 | +0.087 | -679.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2592 | +0.132 | +24.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 1386 | +0.033 | -300.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 13 | -0.065 | -2.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 1369 | +0.036 | -292.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 3253 | +0.140 | -3.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 921 | +0.196 | -34.22€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 41 | -0.105 | -20.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 1368 | +0.110 | -13.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 923 | +0.140 | +65.31€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 1383 | +0.057 | -236.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 9 | -0.021 | -3.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 1373 | +0.058 | -231.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 3517 | +0.126 | -47.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1245 | +0.158 | -27.58€ | 0 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 1365 | +0.106 | -14.90€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 895 | +0.117 | +3.13€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 3355 | +0.135 | -205.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1173 | +0.196 | -51.13€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 40 | +0.000 | -9.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 1368 | +0.083 | -101.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 774 | +0.139 | -44.18€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 1382 | +0.128 | -26.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 12 | +0.043 | +1.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 1369 | +0.129 | -26.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 3446 | +0.160 | -331.57€ | 2 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 3446 | +0.160 | -331.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 864 | +0.155 | -112.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 864 | +0.155 | -112.47€ | 0 | 2 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 765 | +0.216 | -44.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min |
08€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 304 | -0.059 | +4.02€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 304 | -0.059 | +4.02€ | 4 | 1 |
| ✅ MOMENTUM_IBS_5M_FADE | 1773 | +0.021 | +20.26€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 1773 | +0.021 | +20.26€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB | 302 | +0.016 | +7.09€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB#5min | 302 | +0.016 | +7.09€ | 1 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC | 257 | +0.037 | +4.19€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC#5min | 257 | +0.037 | +4.19€ | 1 | 1 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE | 305 | +0.018 | +2.15€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE#5min | 305 | +0.018 | +2.15€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH | 306 | +0.016 | +2.53€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH#5min | 306 | +0.016 | +2.53€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL | 310 | +0.029 | +6.34€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL#5min | 310 | +0.029 | +6.34€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP | 293 | +0.012 | -2.03€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP#5min | 293 | +0.012 | -2.03€ | 0 | 0 |
...
| ✅ STRUCT_NO_15M | 1844 | +0.016 | -3.84€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 1844 | +0.016 | -3.84€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 704 | +0.007 | -8.03€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 704 | +0.007 | -8.03€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 725 | +0.020 | +1.40€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 725 | +0.020 | +1.40€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 415 | +0.023 | +2.78€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 415 | +0.023 | +2.78€ | 2 | 0 |
| ✅ UPDOWN_GBM | 2168 | +0.023 | +127.37€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 945 | +0.087 | +168.27€ | 1 | 6 |
| ✅ UPDOWN_GBM#240min | 119 | +0.021 | +0.93€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 807 | -0.023 | -30.30€ | 4 | 0 |
| ✅ UPDOWN_GBM#60min | 250 | -0.024 | -11.02€ | 3 | 0 |
| ✅ UPDOWN_GBM#BNB | 95 | +0.098 | +17.78€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 88 | +0.122 | +19.40€ | 0 | 0 |
...
| ✅ UPDOWN_GBM#SOL | 389 | -0.009 | -0.88€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 163 | +0.015 | +1.89€ | 1 | 2 |
...
| ✅ UPDOWN_GBM#XRP | 480 | +0.031 | +65.82€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 290 | +0.099 | +86.59€ | 0 | 16 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1648 | -0.048 | +174.85€ | 2 | 3 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1648 | -0.048 | +174.85€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 111 | -0.075 | -4.38€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 111 | -0.075 | -4.38€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 325 | -0.139 | -14.73€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 325 | -0.139 | -14.73€ | 3 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 527 | -0.012 | +106.58€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 527 | -0.012 | +106.58€ | 1 | 1 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 511 | -0.034 | +67.64€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 511 | -0.034 | +67.64€ | 4 | 0 |
...
| ✅ UPDOWN_OU_5M | 344 | -0.061 | -26.73€ | 5 | 0 |
| ✅ UPDOWN_OU_5M#5min | 344 | -0.061 | -26.73€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#BNB | 221 | -0.002 | -10.04€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#BNB#5min | 221 | -0.002 | -10.04€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.140) — sin ventaja clara. oversold(IBS<0.3): IC=-0.020 n=758 | neutral: IC=+0.007 n=730 | overbought(IBS>0.7): IC=+0.121 n=984
  - _Datos_: n=2650 IC=+0.043 PNL=+210.86€
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=163 IC=+0.015 PNL=+1.89€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.105 n=38181 | tras_1loss IC=+0.062 n=26801 | tras_2loss IC=+0.021 n=11691/40 | gap=+0.084 (umbral 0.05)
...
**🟡 H-24H-GBM-BUYYES-TARDE** — GBM BUY_YES en tarde europea (15-19h UTC) — señal alcista sostenida
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.115 > 0.08 con n=76 PNL=+16.48€
  - _Datos_: n=76 IC=+0.115 PNL=+16.48€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1997 IC=+0.018 PNL=+95.87€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1997 IC=+0.018 PNL=+95.87€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=417 IC=+0.097 PNL=+54.72€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=417 IC=+0.097 PNL=+54.72€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=590 IC=+0.078 PNL=+70.21€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=590 IC=+0.078 PNL=+70.21€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=81 IC=+0.018 PNL=+7.02€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=81 IC=+0.018 PNL=+7.02€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=446 IC=+0.096 PNL=+113.24€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=446 IC=+0.096 PNL=+113.24€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=966 IC=+0.021 PNL=+21.96€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=966 IC=+0.021 PNL=+21.96€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.152 > 0.1 con n=553 PNL=+169.60€
  - _Datos_: n=553 IC=+0.152 PNL=+169.60€
...
**🟡 H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.110 > 0.1 con n=162 PNL=+52.57€
  - _Datos_: n=162 IC=+0.110 PNL=+52.57€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1296 IC=-0.149 PNL=+55.55€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1296 IC=-0.149 PNL=+55.55€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=309 IC=+0.149 PNL=+109.33€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=309 IC=+0.149 PNL=+109.33€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.097 > 0.08 con n=417 PNL=+54.72€
  - _Datos_: n=417 IC=+0.097 PNL=+54.72€
...
**⏳ H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: 47/289 ops en el filtro definido (IC actual=-0.337 PNL=-19.56€)
  - _Datos_: n=47 IC=-0.337 PNL=-19.56€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=795 IC=+0.156 PNL=+419.40€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=795 IC=+0.156 PNL=+419.40€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.246 > 0.08 con n=641 PNL=-69.73€
  - _Datos_: n=641 IC=+0.246 PNL=-69.73€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=1094 IC=+0.098 PNL=-246.48€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=1094 IC=+0.098 PNL=-246.48€
