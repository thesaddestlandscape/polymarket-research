# Hipótesis automáticas — 2026-08-28 18:16 UTC
_Generado por shadow_postmortem.py sobre 191294 resoluciones (PNL=+14243.75€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `147.38` → IC=-0.288 (n=2674)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 147.38
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=8025)
...
- **FILTRO** `restante_s_al_confirmar` < `140.27` → IC=-0.311 (n=621)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=1863)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.146 (n=3961)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.34` → IC=+0.261 (n=3649)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.34 (IC base=+0.143)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.191 (n=3100)
...
- **PATRÓN** `restante_min` > `4.93` → IC=+0.160 (n=1745)
...
- **PATRÓN** `lag_apertura_s` < `4.25` → IC=+0.161 (n=1645)
...
- **PATRÓN** `ibs_20min` < `0.2109` → IC=+0.134 (n=1674)

  - _Acción_: Kelly boost +0.67€ cuando `ibs_20min` < 0.2109 (IC base=+0.031)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.261 (n=304)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.4231` → IC=+0.296 (n=282)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4231 (IC base=+0.259)

- **PATRÓN** `sigma_ewma_delta_pct` < `1.939` → IC=+0.288 (n=333)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8611` → IC=+0.259 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8611 (IC base=+0.259)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.172 (n=65)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=813)
...
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.137 (n=1412)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0078 (IC base=+0.055)

- **PATRÓN** `ibs_20min` > `0.2753` → IC=+0.120 (n=3114)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.115` → IC=+0.179 (n=1408)
  ...
  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` < 0.115 (IC base=+0.055)
...
- **PATRÓN** `ibs_20min` < `0.093` → IC=+0.191 (n=1280)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` < 0.093 (IC base=+0.040)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3783` → IC=+0.285 (n=333)
...
- **FILTRO** `ibs_20min` < `0.2469` → IC=-0.159 (n=215)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2469
  - _Potencial_: sin este filtro IC_bueno=+0.076 (n=437)
...
- **PATRÓN** `sigma_h` < `0.0084` → IC=+0.341 (n=249)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0084 (IC base=+0.333)

- **PATRÓN** `drift_60min` |x|≤ `0.252` → IC=+0.343 (n=189)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.252 (IC base=+0.333)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.379 (n=139)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.333)

- **PATRÓN** `ibs_20min` < `0.3333` → IC=+0.349 (n=283)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3333 (IC base=+0.333)

- **PATRÓN** `ibs_20min` > `0.1333` → IC=+0.337 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.1333 (IC base=+0.333)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.492` → IC=+0.343 (n=310)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.492 (IC base=+0.333)

- **PATRÓN** `volumen_pendiente_norm` > `0.3592` → IC=+0.439 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3592 (IC base=+0.333)

- **PATRÓN** `volumen_spike_ratio` < `1.7067` → IC=+0.333 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7067 (IC base=+0.333)

- **PATRÓN** `volumen_spike_ratio` > `1.9441` → IC=+0.338 (n=171)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9441 (IC base=+0.333)

- **PATRÓN** `libro_liquidez` > `1874.4798` → IC=+0.375 (n=94)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1874.4798 (IC base=+0.333)

- **PATRÓN** `ballena_activa_n` < `30.0` → IC=+0.315 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 30.0 (IC base=+0.333)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0819` → IC=+0.213 (n=811)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0819 (IC base=+0.187)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.215 (n=1224)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 14.0 (IC base=+0.187)

- **PATRÓN** `ibs_20min` < `0.4105` → IC=+0.241 (n=2433)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4105 (IC base=+0.187)

- **PATRÓN** `dist_vwap_pct` < `0.2206` → IC=+0.183 (n=1977)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.2206 (IC base=+0.187)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.921` → IC=+0.218 (n=459)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.921 (IC base=+0.187)

- **PATRÓN** `volumen_regimen` < `1.1764` → IC=+0.171 (n=1927)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 1.1764 (IC base=+0.187)

- **PATRÓN** `volumen_regimen` > `0.6238` → IC=+0.170 (n=1927)
...
- **PATRÓN** `ballena_activa_n` < `123.0` → IC=+0.197 (n=1225)
...
- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.269 (n=232)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0049 (IC base=+0.256)

- **PATRÓN** `drift_60min` |x|≤ `0.2618` → IC=+0.296 (n=204)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.3248` → IC=+0.296 (n=204)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3248 (IC base=+0.256)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.436` → IC=+0.273 (n=258)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.219 (n=151)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.138)

- **PATRÓN** `ibs_20min` < `0.4085` → IC=+0.236 (n=441)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4085 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` > `0.7284` → IC=+0.139 (n=95)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.7284 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.2288` → IC=+0.154 (n=420)
...
- **PATRÓN** `volumen_regimen` < `1.1607` → IC=+0.150 (n=441)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.1607 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `0.8507` → IC=+0.172 (n=294)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` > 0.8507 (IC base=+0.138)
...
- **PATRÓN** `volumen_spike_ratio` > `2.273` → IC=+0.224 (n=103)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.273 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `2105.0629` → IC=+0.193 (n=294)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 2105.0629 (IC base=+0.138)
...
- **PATRÓN** `dist_vwap_pct` < `0.2572` → IC=+0.233 (n=578)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2572 (IC base=+0.224)
...
- **PATRÓN** `sigma_h` > `0.0059` → IC=+0.136 (n=344)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0059 (IC base=+0.099)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.196 (n=202)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 17.0 (IC base=+0.099)

- **PATRÓN** `ibs_20min` > `0.617` → IC=+0.164 (n=459)

  - _Acción_: Kelly boost +0.82€ cuando `ibs_20min` > 0.617 (IC base=+0.099)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.306` → IC=+0.181 (n=142)
  ...
  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 8.306 (IC base=+0.099)

- **PATRÓN** `volumen_pendiente_norm` > `0.18` → IC=+0.157 (n=141)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_pendiente_norm` > 0.18 (IC base=+0.099)

- **PATRÓN** `libro_liquidez` > `2664.3504` → IC=+0.137 (n=342)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 2664.3504 (IC base=+0.099)
...
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.217 (n=1906)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0067 (IC base=+0.191)

- **PATRÓN** `drift_60min` |x|≤ `0.4838` → IC=+0.202 (n=2858)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4838 (IC base=+0.191)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.197 (n=1031)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5617` → IC=+0.247 (n=2858)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5617 (IC base=+0.191)

- **PATRÓN** `dist_vwap_pct` < `0.4264` → IC=+0.183 (n=2104)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.4264 (IC base=+0.191)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.417` → IC=+0.206 (n=420)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.417 (IC base=+0.191)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.654` → IC=+0.195 (n=2678)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` < 2.654 (IC base=+0.191)

- **PATRÓN** `volumen_regimen` < `0.6228` → IC=+0.185 (n=707)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` < 0.6228 (IC base=+0.191)

- **PATRÓN** `volumen_regimen` > `1.2088` → IC=+0.177 (n=706)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 1.2088 (IC base=+0.191)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3099` → IC=+0.210 (n=979)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3099 (IC base=+0.191)

- **PATRÓN** `ballena_activa_n` < `175.0` → IC=+0.173 (n=1730)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1115` → IC=+0.309 (n=145)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1115 (IC base=+0.270)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.274 (n=312)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.562` → IC=+0.313 (n=329)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.562 (IC base=+0.270)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.139` → IC=+0.290 (n=355)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.139 (IC base=+0.270)
...
- **PATRÓN** `libro_liquidez` > `1963.3048` → IC=+0.304 (n=110)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1963.3048 (IC base=+0.270)
...
- **PATRÓN** `drift_60min` |x|≤ `0.208` → IC=+0.251 (n=275)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.208 (IC base=+0.246)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.251 (n=279)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5541` → IC=+0.312 (n=412)
...
- **PATRÓN** `ballena_activa_n` < `59.0` → IC=+0.198 (n=180)

  - _Acción_: Kelly boost +0.99€ cuando `ballena_activa_n` < 59.0 (IC base=+0.246)
...
- **PATRÓN** `sigma_h` < `0.0032` → IC=+0.197 (n=143)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0032 (IC base=+0.145)

- **PATRÓN** `drift_60min` |x|≤ `0.5115` → IC=+0.163 (n=428)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.5115 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.146 (n=145)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.6499` → IC=+0.181 (n=428)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` < 0.6499 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.1716` → IC=+0.152 (n=202)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.1716 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` < `0.3861` → IC=+0.149 (n=425)
...
- **PATRÓN** `volumen_regimen` < `0.5848` → IC=+0.155 (n=143)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.5848 (IC base=+0.145)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8173` → IC=+0.163 (n=247)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 1.8173 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` > `2.4862` → IC=+0.191 (n=124)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` > 2.4862 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `9771.3836` → IC=+0.204 (n=194)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 9771.3836 (IC base=+0.145)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2814` → IC=+0.143 (n=343)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.2814 (IC base=+0.117)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.171 (n=238)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 15.0 (IC base=+0.117)

- **PATRÓN** `ibs_20min` < `0.5918` → IC=+0.203 (n=514)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5918 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` < `0.4894` → IC=+0.144 (n=481)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.4894 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.091` → IC=+0.141 (n=502)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` < 3.091 (IC base=+0.117)

- **PATRÓN** `volumen_regimen` < `0.7141` → IC=+0.146 (n=227)
...
- **PATRÓN** `volumen_spike_ratio` < `2.108` → IC=+0.145 (n=322)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 2.108 (IC base=+0.117)

- **PATRÓN** `volumen_spike_ratio` > `1.7794` → IC=+0.146 (n=244)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.7794 (IC base=+0.117)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.125 (n=537)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.145 (n=1215)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.149 (n=297)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 17.0 (IC base=+0.104)

- **PATRÓN** `ibs_20min` > `0.9059` → IC=+0.163 (n=268)

  - _Acción_: Kelly boost +0.81€ cuando `ibs_20min` > 0.9059 (IC base=+0.104)

- **PATRÓN** `volumen_pendiente_norm` > `0.245` → IC=+0.171 (n=150)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.245 (IC base=+0.104)

- **PATRÓN** `volumen_spike_ratio` > `2.6002` → IC=+0.132 (n=264)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 2.6002 (IC base=+0.104)

- **PATRÓN** `sigma_h` < `0.0037` → IC=+0.191 (n=309)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0037 (IC base=+0.140)

- **PATRÓN** `drift_60min` |x|≤ `0.3821` → IC=+0.163 (n=812)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.82€ cuando `drift_60min` |x|≤ 0.3821 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.157 (n=330)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.1931` → IC=+0.164 (n=406)

  - _Acción_: Kelly boost +0.82€ cuando `ibs_20min` < 0.1931 (IC base=+0.140)

- **PATRÓN** `ibs_20min` > `0.089` → IC=+0.143 (n=923)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` > 0.089 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` > `0.1916` → IC=+0.152 (n=357)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.1916 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.104` → IC=+0.150 (n=924)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` < 6.104 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `1.2228` → IC=+0.150 (n=899)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.2228 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` < `0.0933` → IC=+0.149 (n=829)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` < 0.0933 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` < `2.5479` → IC=+0.159 (n=915)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.5479 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.423` → IC=+0.150 (n=915)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.423 (IC base=+0.140)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.145 (n=1215)

  - _Acción_: Kelly boost +0.73€ cuando `libro_spread` < 0.01 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `10638.1798` → IC=+0.148 (n=615)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 10638.1798 (IC base=+0.140)
...
- **PATRÓN** `sigma_h` < `0.0036` → IC=+0.193 (n=223)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0036 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.3904` → IC=+0.150 (n=507)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.3904 (IC base=+0.141)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.188 (n=184)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.1762` → IC=+0.176 (n=223)

  - _Acción_: Kelly boost +0.88€ cuando `ibs_20min` < 0.1762 (IC base=+0.141)

- **PATRÓN** `ibs_20min` > `0.3872` → IC=+0.144 (n=338)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` > 0.3872 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` > `0.6648` → IC=+0.187 (n=65)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.6648 (IC base=+0.141)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.318` → IC=+0.160 (n=507)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` < 6.318 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` < `1.1906` → IC=+0.158 (n=507)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.1906 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` > `0.6188` → IC=+0.144 (n=507)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 0.6188 (IC base=+0.141)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5555` → IC=+0.157 (n=505)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.5555 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` > `1.8118` → IC=+0.148 (n=336)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.8118 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `11974.8913` → IC=+0.153 (n=453)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 11974.8913 (IC base=+0.141)
...
- **PATRÓN** `sigma_h` < `0.0089` → IC=+0.145 (n=271)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` < 0.0089 (IC base=+0.132)

- **PATRÓN** `sigma_h` > `0.0062` → IC=+0.137 (n=180)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0062 (IC base=+0.132)

- **PATRÓN** `drift_60min` |x|≤ `0.4118` → IC=+0.186 (n=237)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.4118 (IC base=+0.132)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.161 (n=181)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 10.0 (IC base=+0.132)

- **PATRÓN** `ibs_20min` < `0.6071` → IC=+0.140 (n=237)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` < 0.6071 (IC base=+0.132)

- **PATRÓN** `ibs_20min` > `0.1055` → IC=+0.154 (n=270)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` > 0.1055 (IC base=+0.132)

- **PATRÓN** `dist_vwap_pct` > `0.1508` → IC=+0.157 (n=103)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.1508 (IC base=+0.132)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.138` → IC=+0.186 (n=49)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 11.138 (IC base=+0.132)

- **PATRÓN** `volumen_regimen` < `1.1943` → IC=+0.154 (n=270)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 1.1943 (IC base=+0.132)

- **PATRÓN** `volumen_pendiente_norm` < `0.135` → IC=+0.158 (n=276)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_pendiente_norm` < 0.135 (IC base=+0.132)

- **PATRÓN** `volumen_spike_ratio` < `2.1996` → IC=+0.172 (n=233)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 2.1996 (IC base=+0.132)

- **PATRÓN** `volumen_spike_ratio` > `1.4499` → IC=+0.159 (n=265)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 1.4499 (IC base=+0.132)

- **PATRÓN** `libro_liquidez` > `10385.425` → IC=+0.141 (n=90)

  - _Acción_: Kelly boost +0.71€ cuando `libro_liquidez` > 10385.425 (IC base=+0.132)
...
- **FILTRO** `ibs_20min` > `0.942` → IC=-0.173 (n=99)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.173 (n=926)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.025 (n=2821)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.208 (n=951)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.009 (n=2877)
...
- **FILTRO** `ibs_20min` > `0.7435` → IC=-0.163 (n=164)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.009 (n=495)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.143 (n=2723)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=6748)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.281 (n=2312)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=7159)

- **FILTRO** `ibs_7min` < `0.73` → IC=-0.234 (n=2366)

  - _Acción_: SKIP cuando `ibs_7min` < 0.73
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=7105)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.173 (n=3204)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=6267)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.221 (n=2867)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=8674)

- **FILTRO** `ibs_7min` > `0.7169` → IC=-0.172 (n=2885)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7169
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=8656)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.316 (n=313)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=974)

- **FILTRO** `ibs_7min` < `1.0` → IC=-0.189 (n=900)

  - _Acción_: SKIP cuando `ibs_7min` < 1.0
  - _Potencial_: sin este filtro IC_bueno=+0.058 (n=387)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.244 (n=310)
...
- **FILTRO** `py_entrada` > `0.61` → IC=-0.215 (n=440)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=1388)

- **FILTRO** `ballena_activa_n` > `97.0` → IC=-0.160 (n=619)
...
- **FILTRO** `hora_utc` > `17.0` → IC=-0.126 (n=372)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 17.0
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=1217)
...
- **FILTRO** `ibs_7min` > `0.1846` → IC=-0.164 (n=540)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=1049)

- **FILTRO** `ballena_activa_n` > `25.0` → IC=-0.173 (n=537)

  - _Acción_: SKIP cuando `ballena_activa_n` > 25.0
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=1052)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.43` → IC=-0.229 (n=445)

  - _Acción_: SKIP cuando `py_entrada` < 0.43
  - _Potencial_: sin este filtro IC_bueno=+0.006 (n=1380)

- **FILTRO** `ibs_7min` < `0.7705` → IC=-0.199 (n=456)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7705
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=1369)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.185 (n=513)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=1623)
...
- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.122 (n=646)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=1328)
...
| ✅ BALLENAS_TARDIAS | 10699 | -0.096 | -1762.87€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 759 | -0.060 | -135.01€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 9940 | -0.098 | -1627.86€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1344 | -0.030 | -258.86€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1344 | -0.030 | -258.86€ | 1 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 2484 | -0.092 | -198.03€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2484 | -0.092 | -198.03€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1859 | -0.166 | -452.65€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1859 | -0.166 | -452.65€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 603 | -0.054 | +284.15€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 172 | -0.006 | +32.97€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 431 | -0.073 | +251.18€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 603 | -0.054 | +284.15€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 172 | -0.006 | +32.97€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 431 | -0.073 | +251.18€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 35001 | +0.114 | -2219.44€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 6328 | +0.186 | -220.81€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 111 | -0.093 | -48.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 25271 | +0.096 | -1898.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3291 | +0.119 | -52.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 4239 | +0.067 | -689.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 21 | -0.065 | +2.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 4213 | +0.068 | -685.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 7177 | +0.133 | -156.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1794 | +0.198 | -89.83€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 4205 | +0.109 | -98.34€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 7701 | +0.126 | -134.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2300 | +0.166 | -20.57€ | 0 | 8 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 4207 | +0.112 | -76.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1182 | +0.100 | -28.74€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#SOL | 7397 | +0.133 | -453.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2174 | +0.201 | -115.56€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 50 | +0.019 | -7.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 4200 | +0.099 | -252.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 973 | +0.132 | -77.44€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#XRP | 4237 | +0.107 | -273.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 17 | -0.022 | -0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 4219 | +0.108 | -270.84€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 6566 | +0.134 | +253.02€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 6566 | +0.134 | +253.02€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 3295 | +0.128 | +90.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 3295 | +0.128 | +90.27€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 7755 | +0.096 | -262.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 759 | +0.070 | -19.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 6996 | +0.098 | -243.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC | 5118 | +0.096 | -109.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#15min | 759 | +0.070 | -19.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#5min | 4359 | +0.101 | -90.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH | 346 | +0.132 | +14.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH#5min | 346 | +0.132 | +14.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL | 2291 | +0.088 | -168.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL#5min | 2291 | +0.088 | -168.03€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 8393 | +0.044 | +2717.36€ | 0 | 15 |
| ✅ GBM_LATE_15M#15min | 8393 | +0.044 | +2717.36€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 1167 | +0.177 | +750.61€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 1167 | +0.177 | +750.61€ | 0 | 20 |
| ✅ GBM_LATE_15M#BTC | 1201 | +0.171 | +695.50€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1201 | +0.171 | +695.50€ | 0 | 29 |
| ✅ GBM_LATE_15M#DOGE | 1171 | +0.195 | +834.48€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 1171 | +0.195 | +834.48€ | 0 | 20 |
| ✅ GBM_LATE_15M#ETH | 1374 | -0.049 | +27.74€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1374 | -0.049 | +27.74€ | 4 | 12 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR | 9268 | +0.047 | +3672.50€ | 0 | 16 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 9268 | +0.047 | +3672.50€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 1464 | -0.016 | +676.47€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 1464 | -0.016 | +676.47€ | 2 | 4 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 2015 | -0.039 | +160.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 2015 | -0.039 | +160.84€ | 1 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 1035 | +0.243 | +959.72€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 1035 | +0.243 | +959.72€ | 0 | 21 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1519 | -0.052 | -16.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1519 | -0.052 | -16.60€ | 9 | 2 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 1616 | -0.026 | +334.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 1616 | -0.026 | +334.34€ | 7 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 1619 | +0.250 | +1557.73€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 1619 | +0.250 | +1557.73€ | 0 | 22 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6822 | +0.171 | +4749.34€ | 0 | 24 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6822 | +0.171 | +4749.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB | 892 | +0.189 | +641.92€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB#15min | 892 | +0.189 | +641.92€ | 0 | 20 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC | 1159 | +0.160 | +763.16€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC#15min | 1159 | +0.160 | +763.16€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE | 881 | +0.208 | +702.51€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min | 881 | +0.208 | +702.51€ | 0 | 17 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH | 1132 | +0.155 | +711.17€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH#15min | 1132 | +0.155 | +711.17€ | 0 | 26 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL | 1290 | +0.129 | +801.72€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL#15min | 1290 | +0.129 | +801.72€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP | 1468 | +0.197 | +1128.86€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP#15min | 1468 | +0.197 | +1128.86€ | 0 | 21 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 1326 | +0.080 | +288.85€ | 0 | 9 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 1326 | +0.080 | +288.85€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#DOGE | 176 | +0.124 | +66.79€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#DOGE#15min | 176 | +0.124 | +66.79€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO | 8001 | +0.169 | +5366.60€ | 0 | 26 |
| ✅ GBM_LATE_15M_TARDIO#15min | 8001 | +0.169 | +5366.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 1120 | +0.199 | +840.97€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 1120 | +0.199 | +840.97€ | 0 | 19 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 1302 | +0.160 | +823.59€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 1302 | +0.160 | +823.59€ | 0 | 28 |
| ✅ GBM_LATE_15M_TARDIO#DOGE | 1103 | +0.216 | +903.62€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#DOGE#15min | 1103 | +0.216 | +903.62€ | 0 | 20 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 1242 | +0.141 | +707.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 1242 | +0.141 | +707.25€ | 0 | 27 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 1427 | +0.104 | +716.02€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 1427 | +0.104 | +716.02€ | 0 | 19 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 1807 | +0.199 | +1375.14€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1807 | +0.199 | +1375.14€ | 0 | 21 |
| ✅ GBM_LATE_5M | 2301 | +0.123 | +1007.16€ | 1 | 18 |
| ✅ GBM_LATE_5M#5min | 2301 | +0.123 | +1007.16€ | 0 | 0 |
| ✅ GBM_LATE_5M#BNB | 95 | +0.222 | +72.24€ | 0 | 0 |
| ✅ GBM_LATE_5M#BNB#5min | 95 | +0.222 | +72.24€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC | 811 | +0.126 | +408.57€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC#5min | 811 | +0.126 | +408.57€ | 2 | 16 |
| ✅ GBM_LATE_5M#DOGE | 207 | +0.136 | +92.73€ | 0 | 0 |
| ✅ GBM_LATE_5M#DOGE#5min | 207 | +0.136 | +92.73€ | 0 | 7 |
| ✅ GBM_LATE_5M#ETH | 753 | +0.129 | +314.69€ | 0 | 0 |
| ✅ GBM_LATE_5M#ETH#5min | 753 | +0.129 | +314.69€ | 0 | 28 |
| ✅ GBM_LATE_5M#SOL | 125 | -0.020 | +0.12€ | 0 | 0 |
| ✅ GBM_LATE_5M#SOL#5min | 125 | -0.020 | +0.12€ | 3 | 0 |
| ✅ GBM_LATE_5M#XRP | 310 | +0.119 | +118.83€ | 0 | 0 |
| ✅ GBM_LATE_5M#XRP#5min | 310 | +0.119 | +118.83€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_15M | 4848 | -0.002 | -74.77€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#15min | 4848 | -0.002 | -74.77€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_15M#DOGE | 887 | +0.001 | -20.77€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#DOGE#15min | 887 | +0.001 | -20.77€ | 1 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH | 1038 | +0.011 | +15.77€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH#15min | 1038 | +0.011 | +15.77€ | 1 | 0 |
...
| ✅ MOMENTUM_IBS_15M_BALLENA | 7575 | -0.035 | +188.23€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#15min | 7575 | -0.035 | +188.23€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB | 1203 | -0.030 | +133.08€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB#15min | 1203 | -0.030 | +133.08€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC | 1323 | -0.031 | -12.21€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC#15min | 1323 | -0.031 | -12.21€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE | 1215 | -0.046 | +80.09€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE#15min | 1215 | -0.046 | +80.09€ | 5 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH | 1339 | -0.032 | -35.84€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH#15min | 1339 | -0.032 | -35.84€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL | 1251 | -0.039 | +35.00€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL#15min | 1251 | -0.039 | +35.00€ | 4 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 21012 | -0.075 | +372.18€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 21012 | -0.075 | +372.18€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 3274 | -0.088 | +406.53€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 3274 | -0.088 | +406.53€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 3664 | -0.066 | -35.96€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 3664 | -0.066 | -35.96€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 3394 | -0.084 | +33.78€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 3394 | -0.084 | +33.78€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 3181 | -0.098 | -169.90€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 3181 | -0.098 | -169.90€ | 8 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 3961 | -0.050 | +7.87€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 3961 | -0.050 | +7.87€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 3538 | -0.074 | +129.86€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 3538 | -0.074 | +129.86€ | 6 | 0 |
...
| ✅ STREAK_MOM_5M | 2472 | +0.028 | +47.12€ | 0 | 0 |
| ✅ STREAK_MOM_5M#5min | 2472 | +0.028 | +47.12€ | 0 | 0 |
...
| ✅ STREAK_MOM_5M#SOL | 729 | +0.025 | +7.35€ | 0 | 0 |
| ✅ STREAK_MOM_5M#SOL#5min | 729 | +0.025 | +7.35€ | 1 | 0 |
...
| ✅ UPDOWN_GBM | 6627 | +0.004 | +146.95€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 2403 | +0.044 | +233.29€ | 0 | 8 |
...
| ✅ UPDOWN_GBM#ETH | 1579 | -0.003 | -6.15€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 711 | +0.022 | +18.38€ | 1 | 5 |
...
| ✅ UPDOWN_GBM#SOL | 1617 | -0.007 | -18.11€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 566 | +0.005 | -0.25€ | 1 | 2 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.083) — sin ventaja clara. oversold(IBS<0.3): IC=+0.016 n=2369 | neutral: IC=+0.001 n=2537 | overbought(IBS>0.7): IC=+0.085 n=2588
  - _Datos_: n=7819 IC=+0.035 PNL=+694.10€

**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  - _Umbral_: n≥40 por celda + gate riguroso completo (Wilson+shuffle+PnL bootstrap)
  - _Acción_: Añadir claves 'ESTRATEGIA#SUBTYPE#DIRECCION#HORA':1.2 a meta.hora_boost_factor, solo por celda confirmada
  - _Estado_: (cache 847s) 28 celda(s) GATE OK de 2160 trackeadas

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.005 < 0.08 — monitorear
  - _Datos_: n=566 IC=+0.005 PNL=-0.25€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.074 n=107295 | tras_1loss IC=+0.046 n=83693 | tras_2loss IC=+0.009 n=37958/40 | gap=+0.065 (umbral 0.05)
...
**⏳ H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  ...
  - _Estado_: 6565 ops, 22 horas distintas. Sin hora con n≥15 y IC extremo aún.
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=-0.013 n=721 | contrario_BTC IC=-0.011 n=540/40 | gap=+0.002 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=6380 IC=+0.001 PNL=+97.27€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=6380 IC=+0.001 PNL=+97.27€
...
**🔴 H-CUSTOM-GBM-18H** — GBM a las 18h UTC — ¿blacklist necesario?
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.086 < -0.08 con n=114 PNL=-10.42€
  - _Datos_: n=114 IC=-0.086 PNL=-10.42€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=1390 IC=+0.027 PNL=+84.81€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1390 IC=+0.027 PNL=+84.81€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=1758 IC=+0.024 PNL=+103.58€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1758 IC=+0.024 PNL=+103.58€

**〰️ H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  - _Hipótesis_: Feature nueva 2026-06-27: drift del precio YES en Polymarket en últimas 5 obs (~5min). Si poly_drift<0 y decidimos BUY_NO (o poly_drift>0 y BUY_YES) → confluencia. Si diverge → reducción de stake. Hipótesis: confluencia Binance+Polymarket mejora IC; divergencia empeora.
  - _Umbral_: n≥40 en confluencia vs divergencia para validar el boost ×1.1
  - _Acción_: Si IC_confluencia>IC_divergencia con n≥40 → mantener el boost. Si no → retirar.
  - _Estado_: n=460 IC=+0.035 PNL=+1.42€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=460 IC=+0.035 PNL=+1.42€
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=17194 IC=+0.101 PNL=+5094.25€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=17194 IC=+0.101 PNL=+5094.25€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=4989 IC=-0.147 PNL=+142.71€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=4989 IC=-0.147 PNL=+142.71€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=1319 IC=+0.136 PNL=+680.19€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=1319 IC=+0.136 PNL=+680.19€
