# Hipótesis automáticas — 2026-07-16 08:40 UTC
_Generado por shadow_postmortem.py sobre 16714 resoluciones (PNL=+3422.05€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.184 (n=806)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.190 (n=1049)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.01 (IC base=+0.184)

- **PATRÓN** `libro_liquidez` > `4371.9525` → IC=+0.189 (n=600)
...
- **PATRÓN** `py_entrada` > `0.615` → IC=+0.257 (n=179)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `11.066` → IC=+0.220 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.066 (IC base=+0.108)
...
- **PATRÓN** `sigma_h` < `0.0307` → IC=+0.147 (n=446)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0307 (IC base=+0.148)

- **PATRÓN** `sigma_h` > `0.0244` → IC=+0.176 (n=202)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0244 (IC base=+0.148)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.156 (n=472)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 5.0 (IC base=+0.148)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.154 (n=304)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.166 (n=758)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `11.007` → IC=+0.304 (n=90)
...
- **PATRÓN** `dist_vwap_pct` > `0.3415` → IC=+0.169 (n=176)
  ...
  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` > 0.3415 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` < `0.1365` → IC=+0.140 (n=245)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.1365 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.296` → IC=+0.303 (n=69)
...
- **PATRÓN** `sigma_h` > `0.0117` → IC=+0.211 (n=199)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0117 (IC base=+0.200)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.200 (n=211)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.2986` → IC=+0.263 (n=57)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2986 (IC base=+0.200)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.863` → IC=+0.214 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.863 (IC base=+0.200)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.903` → IC=+0.216 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.903 (IC base=+0.200)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2492 | +0.175 | -47.90€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1601 | +0.213 | +28.61€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 722 | +0.182 | -9.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 536 | +0.208 | -6.90€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 4834 | +0.102 | +1505.85€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 4834 | +0.102 | +1505.85€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1159 | +0.062 | +159.96€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1159 | +0.062 | +159.96€ | 0 | 4 |
...
| ✅ GBM_LATE_15M#XRP | 1223 | +0.156 | +620.87€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1223 | +0.156 | +620.87€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2062 | +0.150 | +1206.86€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2062 | +0.150 | +1206.86€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 527 | +0.109 | +255.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 527 | +0.109 | +255.60€ | 0 | 9 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 432 | +0.272 | +446.06€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 432 | +0.272 | +446.06€ | 0 | 12 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 73 | +0.087 | +21.18€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 73 | +0.087 | +21.18€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 45 | +0.245 | +23.83€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 45 | +0.245 | +23.83€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 2033 | +0.090 | +605.70€ | 0 | 6 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2033 | +0.090 | +605.70€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 503 | +0.054 | +83.50€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 503 | +0.054 | +83.50€ | 0 | 4 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 581 | +0.198 | +411.79€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 581 | +0.198 | +411.79€ | 0 | 11 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9725 | tras_1loss IC=+0.068 n=6780 | tras_2loss IC=+0.036 n=2893/40 | gap=+0.066 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=14256 IC=+0.106 PNL=+3497.93€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=14256 IC=+0.106 PNL=+3497.93€
...
**🔶 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.167 n=199) pero sin cruzar ≥2 pares más — BTC: n=143 IC=-0.017; ETH: n=108 IC=+0.000; SOL: n=209 IC=+0.059 ✓
