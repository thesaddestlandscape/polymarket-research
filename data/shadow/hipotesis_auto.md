# Hipótesis automáticas — 2026-07-16 09:45 UTC
_Generado por shadow_postmortem.py sobre 16773 resoluciones (PNL=+3444.27€)_
...
- **PATRÓN** `py_entrada` > `0.695` → IC=+0.293 (n=418)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.179)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.184 (n=1059)

  - _Acción_: Kelly boost +0.92€ cuando `libro_spread` < 0.01 (IC base=+0.179)

- **PATRÓN** `libro_liquidez` > `4371.9525` → IC=+0.185 (n=605)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 4371.9525 (IC base=+0.179)
...
- **PATRÓN** `py_entrada` < `0.265` → IC=+0.332 (n=338)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.265 (IC base=+0.168)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.179 (n=1116)

  - _Acción_: Kelly boost +0.89€ cuando `libro_spread` < 0.01 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `7360.9474` → IC=+0.185 (n=325)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 7360.9474 (IC base=+0.168)
...
- **PATRÓN** `py_entrada` > `0.615` → IC=+0.254 (n=181)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.223 (n=204)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.218)

- **PATRÓN** `py_entrada` > `0.715` → IC=+0.353 (n=93)
...
- **PATRÓN** `py_entrada` > `0.615` → IC=+0.271 (n=186)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.615 (IC base=+0.216)

- **PATRÓN** `libro_liquidez` > `1608.5814` → IC=+0.216 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1608.5814 (IC base=+0.216)
...
- **PATRÓN** `py_entrada` < `0.365` → IC=+0.265 (n=181)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `11.066` → IC=+0.223 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.066 (IC base=+0.107)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.135 (n=1235)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0091 (IC base=+0.108)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.128 (n=1243)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 11.0 (IC base=+0.108)

- **PATRÓN** `dist_vwap_pct` < `0.3941` → IC=+0.120 (n=617)

  - _Acción_: Kelly boost +0.60€ cuando `dist_vwap_pct` < 0.3941 (IC base=+0.108)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.188` → IC=+0.125 (n=142)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 6.188 (IC base=+0.108)
...
- **PATRÓN** `dist_vwap_pct` < `0.4825` → IC=+0.122 (n=154)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.4825 (IC base=+0.063)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.075` → IC=+0.241 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.075 (IC base=+0.063)
...
- **PATRÓN** `dist_vwap_pct` > `0.7151` → IC=+0.146 (n=46)
  ...
  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.7151 (IC base=+0.068)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.518` → IC=+0.204 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.518 (IC base=+0.068)
...
- **PATRÓN** `sigma_h` < `0.0108` → IC=+0.136 (n=212)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0108 (IC base=+0.108)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.141 (n=218)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 16.0 (IC base=+0.108)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.515` → IC=+0.231 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.515 (IC base=+0.108)

- **PATRÓN** `sigma_h` < `0.0115` → IC=+0.148 (n=313)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0115 (IC base=+0.130)

- **PATRÓN** `sigma_h` > `0.0092` → IC=+0.133 (n=467)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_h` > `0.0279` → IC=+0.219 (n=158)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.3154` → IC=+0.143 (n=54)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.3154 (IC base=+0.166)

- **PATRÓN** `dist_vwap_pct` < `0.5109` → IC=+0.149 (n=186)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.5109 (IC base=+0.166)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.494` → IC=+0.145 (n=167)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 7.494 (IC base=+0.166)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.222 (n=332)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.165 (n=762)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 18.0 (IC base=+0.159)

- **PATRÓN** `dist_vwap_pct` > `0.1487` → IC=+0.198 (n=286)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.1487 (IC base=+0.159)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.022` → IC=+0.304 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.022 (IC base=+0.159)

- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.181 (n=753)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0048 (IC base=+0.165)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.190 (n=259)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` < 5.0 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` < `0.4123` → IC=+0.200 (n=532)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4123 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.188` → IC=+0.274 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.188 (IC base=+0.165)
...
- **PATRÓN** `sigma_h` > `0.0028` → IC=+0.172 (n=190)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0028 (IC base=+0.139)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.163 (n=90)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 15.0 (IC base=+0.139)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.139 (n=128)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` < 11.0 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` < `0.4825` → IC=+0.132 (n=153)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.4825 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.827` → IC=+0.364 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.827 (IC base=+0.139)
...
- **PATRÓN** `sigma_h` < `0.0068` → IC=+0.149 (n=169)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0068 (IC base=+0.140)

- **PATRÓN** `sigma_h` > `0.0038` → IC=+0.160 (n=192)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` > 0.0038 (IC base=+0.140)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.191 (n=66)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 5.0 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` > `0.7087` → IC=+0.202 (n=45)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7087 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` < `0.3807` → IC=+0.185 (n=109)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` < 0.3807 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.403` → IC=+0.236 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.403 (IC base=+0.140)
...
- **PATRÓN** `sigma_h` > `0.0127` → IC=+0.220 (n=91)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0127 (IC base=+0.139)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.153 (n=188)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 8.0 (IC base=+0.139)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.153 (n=70)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 6.0 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` > `0.3907` → IC=+0.191 (n=53)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.3907 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` < `1.0007` → IC=+0.154 (n=151)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 1.0007 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.083` → IC=+0.279 (n=66)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.083 (IC base=+0.139)
...
- **PATRÓN** `sigma_h` < `0.0169` → IC=+0.295 (n=169)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0169 (IC base=+0.292)

- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.324 (n=168)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0086 (IC base=+0.292)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.301 (n=169)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.5244` → IC=+0.312 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5244 (IC base=+0.292)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.048` → IC=+0.319 (n=114)
...
- **PATRÓN** `dist_vwap_pct` < `0.3568` → IC=+0.121 (n=500)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.3568 (IC base=+0.090)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.462` → IC=+0.123 (n=59)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` > 3.462 (IC base=+0.070)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.821` → IC=+0.152 (n=21)
  ...
  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 7.821 (IC base=+0.024)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.120 (n=77)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` < 6.0 (IC base=+0.034)
...
- **PATRÓN** `sigma_h` > `0.0269` → IC=+0.244 (n=80)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.4841` → IC=+0.186 (n=183)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` < 0.4841 (IC base=+0.198)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.517` → IC=+0.201 (n=142)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.128 (n=49)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.495` → IC=+0.214 (n=26)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.495 (IC base=+0.125)

- **PATRÓN** `btc_momentum` |x|≤ `0.03` → IC=+0.141 (n=51)

  - _Acción_: Kelly boost +0.71€ cuando `btc_momentum` |x|≤ 0.03 (IC base=+0.125)
...
- **PATRÓN** `hora_utc` > `13.0` → IC=+0.191 (n=40)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 13.0 (IC base=+0.104)

- **PATRÓN** `py_entrada` < `0.505` → IC=+0.138 (n=45)

  - _Acción_: Kelly boost +0.69€ cuando `py_entrada` < 0.505 (IC base=+0.104)

- **PATRÓN** `streak_len` < `4.0` → IC=+0.172 (n=59)

  - _Acción_: Kelly boost +0.86€ cuando `streak_len` < 4.0 (IC base=+0.104)

- **PATRÓN** `volumen_racha` < `270220.1` → IC=+0.210 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_racha` < 270220.1 (IC base=+0.104)

- **PATRÓN** `libro_liquidez` > `2048.5069` → IC=+0.189 (n=43)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 2048.5069 (IC base=+0.104)
...
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_SOL_ALTACONVICCION` — IC=+0.278 n=34. Faltan ~6 resoluciones para umbral n≥40. ETA: ~4h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min` — IC=+0.278 n=34. Faltan ~6 resoluciones para umbral n≥40. ETA: ~4h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL` — IC=+0.278 n=34. Faltan ~6 resoluciones para umbral n≥40. ETA: ~4h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min` — IC=+0.278 n=34. Faltan ~6 resoluciones para umbral n≥40. ETA: ~4h.
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2509 | +0.173 | -61.62€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 1613 | +0.210 | +16.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 106 | +0.009 | -19.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 364 | +0.087 | -63.37€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 426 | +0.147 | +4.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 727 | +0.182 | -10.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 540 | +0.207 | -9.26€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 35 | +0.041 | -5.63€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 142 | +0.153 | +8.37€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#ETH | 893 | +0.172 | -10.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 534 | +0.213 | +13.66€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 35 | -0.095 | -12.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 182 | +0.103 | -25.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 142 | +0.167 | +13.95€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL | 881 | +0.169 | -37.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 531 | +0.213 | +15.04€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 36 | +0.079 | -1.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 172 | +0.092 | -34.37€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 142 | +0.118 | -17.37€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 8 | +0.000 | -3.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 8 | +0.000 | -3.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 34 | +0.278 | +0.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 34 | +0.278 | +0.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 34 | +0.278 | +0.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 34 | +0.278 | +0.07€ | 0 | 0 |
| ✅ GBM_LATE_15M | 4846 | +0.103 | +1516.93€ | 0 | 6 |
| ✅ GBM_LATE_15M#15min | 4846 | +0.103 | +1516.93€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1162 | +0.064 | +163.12€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1162 | +0.064 | +163.12€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1160 | +0.077 | +202.75€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1160 | +0.077 | +202.75€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1297 | +0.109 | +522.50€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1297 | +0.109 | +522.50€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1227 | +0.157 | +628.56€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1227 | +0.157 | +628.56€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2073 | +0.151 | +1218.60€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2073 | +0.151 | +1218.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 530 | +0.111 | +260.77€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 530 | +0.111 | +260.77€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 549 | +0.130 | +261.01€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 549 | +0.130 | +261.01€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 559 | +0.111 | +245.00€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 559 | +0.111 | +245.00€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 435 | +0.274 | +451.83€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 435 | +0.274 | +451.83€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 2045 | +0.091 | +614.26€ | 0 | 7 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2045 | +0.091 | +614.26€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 506 | +0.057 | +85.55€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 506 | +0.057 | +85.55€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 473 | +0.024 | +14.77€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 473 | +0.024 | +14.77€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 481 | +0.059 | +94.45€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 481 | +0.059 | +94.45€ | 2 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 585 | +0.200 | +419.49€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 585 | +0.200 | +419.49€ | 0 | 10 |
...
| ✅ LEADLAG_BTC_XRP_15M | 141 | +0.087 | +29.55€ | 0 | 7 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 141 | +0.087 | +29.55€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 141 | +0.087 | +29.55€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 141 | +0.087 | +29.55€ | 0 | 7 |
...
| ✅ STREAK_FADE_15M | 203 | +0.095 | +27.35€ | 0 | 9 |
| ✅ STREAK_FADE_15M#15min | 203 | +0.095 | +27.35€ | 0 | 0 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.103 n=9763 | tras_1loss IC=+0.068 n=6801 | tras_2loss IC=+0.037 n=2904/40 | gap=+0.066 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=14315 IC=+0.106 PNL=+3520.16€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=14315 IC=+0.106 PNL=+3520.16€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=620 IC=+0.056 PNL=+71.78€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=620 IC=+0.056 PNL=+71.78€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.243 > 0.2 con n=352 PNL=+238.92€
  - _Datos_: n=352 IC=+0.243 PNL=+238.92€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.146 > 0.08 con n=159 PNL=+53.38€
  - _Datos_: n=159 IC=+0.146 PNL=+53.38€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.112 > 0.08 con n=150 PNL=+57.19€
  - _Datos_: n=150 IC=+0.112 PNL=+57.19€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.168 > 0.08 con n=200 PNL=+128.99€
  - _Datos_: n=200 IC=+0.168 PNL=+128.99€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.249 > 0.08 con n=193 PNL=-9.39€
  - _Datos_: n=193 IC=+0.249 PNL=-9.39€
...
**🔶 H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.125 n=70) pero sin cruzar ≥2 pares más — sin otros pares con datos
  - _Datos_: n=70 IC=+0.125 PNL=+25.10€
