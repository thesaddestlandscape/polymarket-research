# Hipótesis automáticas — 2026-07-19 08:13 UTC
_Generado por shadow_postmortem.py sobre 22177 resoluciones (PNL=+5386.14€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.209 (n=1387)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.197)

- **PATRÓN** `py_entrada` > `0.695` → IC=+0.307 (n=615)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.197)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.200 (n=1555)
...
- **PATRÓN** `py_entrada` < `0.265` → IC=+0.342 (n=499)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.265 (IC base=+0.174)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.181 (n=1648)

  - _Acción_: Kelly boost +0.90€ cuando `libro_spread` < 0.01 (IC base=+0.174)

- **PATRÓN** `libro_liquidez` > `4284.1482` → IC=+0.176 (n=973)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 4284.1482 (IC base=+0.174)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.226 (n=308)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.215)

- **PATRÓN** `py_entrada` > `0.615` → IC=+0.274 (n=272)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.615 (IC base=+0.215)

- **PATRÓN** `libro_liquidez` > `11249.9072` → IC=+0.220 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11249.9072 (IC base=+0.215)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.278 (n=124)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.206)

- **PATRÓN** `py_entrada` < `0.265` → IC=+0.361 (n=142)
...
- **PATRÓN** `py_entrada` < `0.41` → IC=+0.221 (n=59)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.41 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `6334.6659` → IC=+0.156 (n=88)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 6334.6659 (IC base=+0.147)
...
- **PATRÓN** `py_entrada` < `0.275` → IC=+0.366 (n=147)
...
- **PATRÓN** `py_entrada` < `0.405` → IC=+0.181 (n=67)

  - _Acción_: Kelly boost +0.91€ cuando `py_entrada` < 0.405 (IC base=+0.130)

- **PATRÓN** `py_entrada` > `0.39` → IC=+0.130 (n=79)

  - _Acción_: Kelly boost +0.65€ cuando `py_entrada` > 0.39 (IC base=+0.130)

- **PATRÓN** `libro_liquidez` > `3897.8672` → IC=+0.178 (n=88)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 3897.8672 (IC base=+0.130)
...
- **PATRÓN** `py_entrada` > `0.665` → IC=+0.304 (n=207)
...
- **PATRÓN** `py_entrada` < `0.395` → IC=+0.264 (n=312)
...
- **PATRÓN** `dist_vwap_pct` > `0.5436` → IC=+0.132 (n=207)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.5436 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.998` → IC=+0.227 (n=170)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.998 (IC base=+0.117)

- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.131 (n=1454)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0088 (IC base=+0.114)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.135 (n=1577)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` < 12.0 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` > `0.1518` → IC=+0.130 (n=371)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` > 0.1518 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` < `0.8879` → IC=+0.124 (n=1104)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.8879 (IC base=+0.114)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `11.03` → IC=+0.121 (n=982)

  - _Acción_: Kelly boost +0.60€ cuando `sigma_ewma_delta_pct` < 11.03 (IC base=+0.114)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.126 (n=404)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.63€ cuando `sigma_h` < 0.0064 (IC base=+0.099)

- **PATRÓN** `dist_vwap_pct` > `0.5433` → IC=+0.200 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5433 (IC base=+0.099)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.466` → IC=+0.240 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.466 (IC base=+0.099)

- **PATRÓN** `dist_vwap_pct` > `0.1355` → IC=+0.136 (n=64)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 0.1355 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` < `0.5143` → IC=+0.133 (n=224)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.5143 (IC base=+0.074)
...
- **PATRÓN** `dist_vwap_pct` > `0.9491` → IC=+0.157 (n=33)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.9491 (IC base=+0.077)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.125` → IC=-0.278 (n=16)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.102 (n=252)

- **PATRÓN** `sigma_h` < `0.0101` → IC=+0.121 (n=259)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.60€ cuando `sigma_h` < 0.0101 (IC base=+0.106)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.326` → IC=+0.216 (n=86)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.326 (IC base=+0.106)

- **PATRÓN** `sigma_h` < `0.013` → IC=+0.139 (n=474)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.013 (IC base=+0.124)

- **PATRÓN** `sigma_h` > `0.009` → IC=+0.128 (n=538)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.63` → IC=+0.123 (n=205)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 2.63 (IC base=+0.124)

### GBM_LATE_15M#XRP#15min
- **PATRÓN** `sigma_h` < `0.0105` → IC=+0.163 (n=185)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0105 (IC base=+0.155)

- **PATRÓN** `sigma_h` > `0.0152` → IC=+0.179 (n=369)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0152 (IC base=+0.155)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.158 (n=554)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 6.0 (IC base=+0.155)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.165 (n=374)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 11.0 (IC base=+0.155)

- **PATRÓN** `dist_vwap_pct` > `0.5779` → IC=+0.255 (n=47)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5779 (IC base=+0.155)

- **PATRÓN** `dist_vwap_pct` < `0.113` → IC=+0.159 (n=206)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.113 (IC base=+0.155)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.547` → IC=+0.282 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.547 (IC base=+0.155)

- **PATRÓN** `sigma_h` < `0.0097` → IC=+0.192 (n=193)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0097 (IC base=+0.172)
...
- **PATRÓN** `dist_vwap_pct` > `0.4345` → IC=+0.173 (n=50)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.4345 (IC base=+0.172)

- **PATRÓN** `dist_vwap_pct` < `0.5463` → IC=+0.170 (n=322)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` < 0.5463 (IC base=+0.172)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.536` → IC=+0.172 (n=291)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` < 7.536 (IC base=+0.172)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0115` → IC=+0.233 (n=358)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.1626` → IC=+0.207 (n=404)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1626 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.007` → IC=+0.299 (n=162)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.007 (IC base=+0.165)

- **PATRÓN** `sigma_h` > `0.0045` → IC=+0.189 (n=1054)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0045 (IC base=+0.170)
...
- **PATRÓN** `dist_vwap_pct` < `0.8513` → IC=+0.187 (n=967)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.8513 (IC base=+0.170)
...
- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.136 (n=256)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0063 (IC base=+0.132)

- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.152 (n=228)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.3426` → IC=+0.186 (n=49)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.3426 (IC base=+0.132)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.89` → IC=+0.217 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.89 (IC base=+0.132)

- **PATRÓN** `sigma_h` < `0.006` → IC=+0.141 (n=268)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.006 (IC base=+0.137)

- **PATRÓN** `sigma_h` > `0.0026` → IC=+0.163 (n=268)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` > 0.0026 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.172 (n=132)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 15.0 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.116` → IC=+0.143 (n=68)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.116 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.466` → IC=+0.137 (n=235)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.466 (IC base=+0.137)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.172 (n=190)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 12.0 (IC base=+0.146)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.154 (n=290)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` < 18.0 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `0.1309` → IC=+0.179 (n=110)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.1309 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` < `0.8639` → IC=+0.143 (n=222)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.8639 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.778` → IC=+0.281 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.778 (IC base=+0.146)
...
- **PATRÓN** `hora_utc` < `16.0` → IC=+0.134 (n=277)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` < 16.0 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` > `0.1498` → IC=+0.178 (n=119)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.1498 (IC base=+0.133)
...
- **PATRÓN** `sigma_h` < `0.0122` → IC=+0.151 (n=250)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `dist_vwap_pct` < `0.7922` → IC=+0.164 (n=209)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.7922 (IC base=+0.121)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.606` → IC=+0.186 (n=167)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` < 2.606 (IC base=+0.121)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` < `0.0169` → IC=+0.263 (n=247)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0169 (IC base=+0.258)

- **PATRÓN** `sigma_h` > `0.0085` → IC=+0.275 (n=247)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.276 (n=217)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.258)

- **PATRÓN** `dist_vwap_pct` > `0.2671` → IC=+0.329 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2671 (IC base=+0.258)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.801` → IC=+0.397 (n=66)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.801 (IC base=+0.258)

- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.333 (n=261)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0076 (IC base=+0.296)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.304 (n=268)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.296)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.297 (n=235)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.296)

- **PATRÓN** `dist_vwap_pct` > `0.1747` → IC=+0.322 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1747 (IC base=+0.296)

- **PATRÓN** `dist_vwap_pct` < `0.5342` → IC=+0.305 (n=260)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5342 (IC base=+0.296)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.458` → IC=+0.311 (n=231)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.458 (IC base=+0.296)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` < `0.0053` → IC=+0.136 (n=330)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0053 (IC base=+0.130)

- **PATRÓN** `sigma_h` > `0.0134` → IC=+0.190 (n=330)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0134 (IC base=+0.130)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.134 (n=887)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 8.0 (IC base=+0.130)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.136 (n=920)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` < 16.0 (IC base=+0.130)

- **PATRÓN** `dist_vwap_pct` > `0.5254` → IC=+0.175 (n=195)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.5254 (IC base=+0.130)

- **PATRÓN** `dist_vwap_pct` < `0.1158` → IC=+0.157 (n=499)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.1158 (IC base=+0.130)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.182` → IC=+0.287 (n=134)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.182 (IC base=+0.130)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.129 (n=418)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 6.0 (IC base=+0.104)

- **PATRÓN** `dist_vwap_pct` < `0.7685` → IC=+0.127 (n=956)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.7685 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.506` → IC=+0.132 (n=248)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` > 3.506 (IC base=+0.104)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0044` → IC=+0.162 (n=149)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0044 (IC base=+0.119)

- **PATRÓN** `sigma_h` > `0.0028` → IC=+0.120 (n=222)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.60€ cuando `sigma_h` > 0.0028 (IC base=+0.119)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.155 (n=201)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 8.0 (IC base=+0.119)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.121 (n=230)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` < 18.0 (IC base=+0.119)

- **PATRÓN** `dist_vwap_pct` > `0.5433` → IC=+0.200 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5433 (IC base=+0.119)

- **PATRÓN** `dist_vwap_pct` < `0.1341` → IC=+0.141 (n=126)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.1341 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.025` → IC=+0.267 (n=41)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.025 (IC base=+0.119)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.888` → IC=+0.160 (n=98)
  ...
  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 2.888 (IC base=+0.079)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.171 (n=83)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0039 (IC base=+0.080)

- **PATRÓN** `dist_vwap_pct` < `0.2613` → IC=+0.134 (n=129)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.2613 (IC base=+0.080)

- **PATRÓN** `sigma_ewma_delta_pct` > `14.068` → IC=+0.267 (n=28)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.125 (n=70)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` < 5.0 (IC base=+0.081)

- **PATRÓN** `dist_vwap_pct` < `0.1406` → IC=+0.129 (n=95)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.1406 (IC base=+0.081)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.399` → IC=+0.285 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.399 (IC base=+0.081)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` < `0.027` → IC=+0.209 (n=311)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.027 (IC base=+0.209)

- **PATRÓN** `sigma_h` > `0.0095` → IC=+0.225 (n=311)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.2297` → IC=+0.265 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2297 (IC base=+0.209)

- **PATRÓN** `dist_vwap_pct` < `0.0993` → IC=+0.214 (n=173)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0993 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.976` → IC=+0.292 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.976 (IC base=+0.209)

- **PATRÓN** `sigma_h` < `0.0094` → IC=+0.214 (n=152)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0094 (IC base=+0.214)

- **PATRÓN** `sigma_h` > `0.0166` → IC=+0.231 (n=154)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0166 (IC base=+0.214)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.235 (n=130)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.214)

- **PATRÓN** `dist_vwap_pct` < `0.5335` → IC=+0.212 (n=311)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5335 (IC base=+0.214)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.003` → IC=+0.222 (n=293)
...
- **PATRÓN** `dist_vwap_pct` > `0.133` → IC=+0.142 (n=146)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.133 (IC base=+0.033)

- **PATRÓN** `dist_vwap_pct` < `0.5084` → IC=+0.139 (n=211)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.5084 (IC base=+0.033)
...
- **PATRÓN** `ibs_15` > `0.7721` → IC=+0.200 (n=98)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7721 (IC base=+0.028)

- **PATRÓN** `dist_vwap_pct` < `0.5087` → IC=+0.142 (n=93)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.5087 (IC base=+0.028)

- **PATRÓN** `sigma_ewma_delta_pct` > `22.389` → IC=+0.222 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 22.389 (IC base=+0.028)
...
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#DOGE#15min` — IC=+0.250 n=38. Faltan ~2 resoluciones para umbral n≥40. ETA: ~1h.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#DOGE` — IC=+0.250 n=38. Faltan ~2 resoluciones para umbral n≥40. ETA: ~1h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 286 | +0.142 | +16.84€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 286 | +0.142 | +16.84€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 118 | +0.175 | +11.03€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 118 | +0.175 | +11.03€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 107 | +0.133 | +2.92€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 107 | +0.133 | +2.92€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 61 | +0.087 | +2.89€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 61 | +0.087 | +2.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 3733 | +0.185 | +52.97€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 2424 | +0.219 | +100.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 160 | +0.074 | +2.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 510 | +0.102 | -74.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 639 | +0.149 | +23.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 1084 | +0.193 | +34.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 808 | +0.211 | +4.80€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 53 | +0.100 | +4.04€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 213 | +0.170 | +29.24€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH | 1324 | +0.180 | +20.90€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 802 | +0.224 | +52.31€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 53 | -0.009 | -8.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 256 | +0.108 | -33.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 213 | +0.146 | +10.56€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#SOL | 1309 | +0.183 | -4.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 802 | +0.224 | +46.40€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 54 | +0.125 | +7.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 240 | +0.103 | -41.90€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 213 | +0.128 | -15.88€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#XRP | 16 | +0.133 | +1.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 12 | +0.043 | -2.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 143 | +0.314 | +7.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 143 | +0.314 | +7.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 143 | +0.314 | +7.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 143 | +0.314 | +7.34€ | 0 | 0 |
| ✅ GBM_LATE_15M | 5763 | +0.107 | +1944.28€ | 0 | 9 |
| ✅ GBM_LATE_15M#15min | 5763 | +0.107 | +1944.28€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1370 | +0.078 | +252.27€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1370 | +0.078 | +252.27€ | 0 | 6 |
| ✅ GBM_LATE_15M#ETH | 1325 | +0.079 | +248.59€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1325 | +0.079 | +248.59€ | 0 | 6 |
| ✅ GBM_LATE_15M#SOL | 1560 | +0.101 | +612.72€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1560 | +0.101 | +612.72€ | 1 | 7 |
| ✅ GBM_LATE_15M#XRP | 1508 | +0.164 | +830.70€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1508 | +0.164 | +830.70€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2994 | +0.153 | +1779.53€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2994 | +0.153 | +1779.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 762 | +0.118 | +361.19€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 762 | +0.118 | +361.19€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 789 | +0.117 | +365.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 789 | +0.117 | +365.53€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 768 | +0.114 | +341.59€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 768 | +0.114 | +341.59€ | 1 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 675 | +0.277 | +711.22€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 675 | +0.277 | +711.22€ | 0 | 12 |
...
| ✅ GBM_LATE_15M_TARDIO | 2938 | +0.100 | +1019.34€ | 0 | 10 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2938 | +0.100 | +1019.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 703 | +0.072 | +146.20€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 703 | +0.072 | +146.20€ | 0 | 9 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 708 | +0.039 | +67.47€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 708 | +0.039 | +67.47€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 661 | +0.049 | +131.72€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 661 | +0.049 | +131.72€ | 1 | 3 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 866 | +0.212 | +673.96€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 866 | +0.212 | +673.96€ | 0 | 11 |
| ✅ GBM_LATE_5M | 90 | +0.076 | +4.82€ | 0 | 0 |
| ✅ GBM_LATE_5M#5min | 90 | +0.076 | +4.82€ | 0 | 0 |
...
| ✅ LEADLAG_BTC_XRP_15M | 190 | +0.042 | +15.05€ | 1 | 2 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 190 | +0.042 | +15.05€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 190 | +0.042 | +15.05€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 190 | +0.042 | +15.05€ | 1 | 2 |
...
| ✅ UPDOWN_GBM | 1818 | +0.020 | +127.55€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1558 | +0.045 | +165.61€ | 1 | 6 |
...
| ✅ UPDOWN_GBM#DOGE | 88 | +0.044 | +8.46€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 85 | +0.052 | +9.36€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 834 | +0.026 | +46.15€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 741 | +0.042 | +57.20€ | 2 | 6 |
...
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD | 60 | +0.274 | +10.45€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#15min | 60 | +0.274 | +10.45€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC | 20 | +0.182 | -1.31€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min | 20 | +0.182 | -1.31€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH | 40 | +0.309 | +11.75€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min | 40 | +0.309 | +11.75€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO | 599 | +0.185 | +279.38€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 599 | +0.185 | +279.38€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 38 | +0.250 | +20.28€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 38 | +0.250 | +20.28€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 147 | +0.191 | +55.30€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 147 | +0.191 | +55.30€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 150 | +0.151 | +65.44€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 150 | +0.151 | +65.44€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 148 | +0.233 | +111.34€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 148 | +0.233 | +111.34€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.182: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.056 n=727 | neutral: IC=-0.005 n=544 | overbought(IBS>0.7): IC=+0.176 n=684
  - _Datos_: n=2092 IC=+0.086 PNL=+442.93€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.117 n=13340 | tras_1loss IC=+0.089 n=8637 | tras_2loss IC=+0.054 n=3519/40 | gap=+0.063 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1687 IC=+0.026 PNL=+143.16€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1687 IC=+0.026 PNL=+143.16€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=783 IC=+0.033 PNL=+62.35€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=783 IC=+0.033 PNL=+62.35€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=823 IC=+0.078 PNL=+118.89€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=823 IC=+0.078 PNL=+118.89€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=19719 IC=+0.121 PNL=+5462.02€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=19719 IC=+0.121 PNL=+5462.02€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=197 IC=+0.013 PNL=+9.03€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=197 IC=+0.013 PNL=+9.03€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=933 IC=+0.003 PNL=+31.80€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=933 IC=+0.003 PNL=+31.80€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.159 > 0.1 con n=520 PNL=+178.50€
  - _Datos_: n=520 IC=+0.159 PNL=+178.50€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=628 IC=-0.087 PNL=+100.38€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=628 IC=-0.087 PNL=+100.38€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=723 IC=+0.068 PNL=+105.47€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=723 IC=+0.068 PNL=+105.47€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.133 > 0.08 con n=379 PNL=+108.60€
  - _Datos_: n=379 IC=+0.133 PNL=+108.60€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema, sin filtrar todavía
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.185 < -0.1 con n=465 PNL=-85.74€
  - _Datos_: n=465 IC=-0.185 PNL=-85.74€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.139 > 0.08 con n=275 PNL=+102.95€
  - _Datos_: n=275 IC=+0.139 PNL=+102.95€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.167 > 0.08 con n=277 PNL=+137.76€
  - _Datos_: n=277 IC=+0.167 PNL=+137.76€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.172 > 0.08 con n=349 PNL=+228.11€
  - _Datos_: n=349 IC=+0.172 PNL=+228.11€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.195 > 0.08 con n=201 PNL=+4.01€
  - _Datos_: n=201 IC=+0.195 PNL=+4.01€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.263 > 0.08 con n=285 PNL=-4.09€
  - _Datos_: n=285 IC=+0.263 PNL=-4.09€
