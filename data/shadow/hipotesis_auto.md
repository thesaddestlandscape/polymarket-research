# Hipótesis automáticas — 2026-09-25 16:28 UTC
_Generado por shadow_postmortem.py sobre 607272 resoluciones (PNL=+68679.40€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.3` → IC=-0.228 (n=7170)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.3
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=21515)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `137.87` → IC=-0.249 (n=942)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 137.87
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=2827)

### BALLENAS_TARDIAS#BTC#15min
- **FILTRO** `restante_s_al_confirmar` < `500.19` → IC=-0.145 (n=370)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 500.19
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=1113)
...
- **FILTRO** `restante_s_al_confirmar` < `165.62` → IC=-0.214 (n=1738)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 165.62
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=5217)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.204 (n=14024)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.69 (IC base=+0.101)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.155 (n=3535)

  - _Acción_: Kelly boost +0.78€ cuando `libro_spread` < 0.01 (IC base=+0.101)

- **PATRÓN** `libro_liquidez` > `5652.3116` → IC=+0.178 (n=2252)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.155 (n=2853)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.715` → IC=+0.344 (n=907)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.235 (n=1502)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.245 (n=882)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.241)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.245 (n=883)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.199 (n=2697)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.192)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.194 (n=2604)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.433 (n=538)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.915` → IC=+0.452 (n=226)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.915 (IC base=+0.438)

- **PATRÓN** `libro_liquidez` > `14347.2058` → IC=+0.442 (n=137)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 14347.2058 (IC base=+0.438)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.231 (n=3112)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.266 (n=2141)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.75 (IC base=+0.220)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.210 (n=5760)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.204)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.256 (n=2308)
...
- **PATRÓN** `lag_apertura_s` < `2.75` → IC=+0.140 (n=4876)

  - _Acción_: Kelly boost +0.70€ cuando `lag_apertura_s` < 2.75 (IC base=+0.117)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.197 (n=2660)
...
- **PATRÓN** `restante_min` > `4.94` → IC=+0.140 (n=2651)
...
- **PATRÓN** `dist_vwap_pct` < `0.6335` → IC=+0.250 (n=2334)
...
- **PATRÓN** `volumen_regimen` < `1.2089` → IC=+0.248 (n=2237)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2089 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` > `1.0457` → IC=+0.261 (n=1014)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4628` → IC=+0.203 (n=5653)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4628 (IC base=+0.106)

- **PATRÓN** `ibs_20min` < `0.5714` → IC=+0.133 (n=9937)

  - _Acción_: Kelly boost +0.67€ cuando `ibs_20min` < 0.5714 (IC base=+0.066)

- **PATRÓN** `dist_vwap_pct` > `0.6162` → IC=+0.202 (n=737)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6162 (IC base=+0.066)

- **PATRÓN** `volumen_regimen` < `0.6986` → IC=+0.185 (n=1536)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 0.6986 (IC base=+0.066)

- **PATRÓN** `volumen_regimen` > `1.0529` → IC=+0.176 (n=1580)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 1.0529 (IC base=+0.066)

- **PATRÓN** `volumen_pendiente_norm` > `0.1671` → IC=+0.227 (n=1664)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1671 (IC base=+0.066)

- **PATRÓN** `volumen_spike_ratio` > `1.5681` → IC=+0.201 (n=5253)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5681 (IC base=+0.066)

- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.210 (n=5671)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4352` → IC=+0.164 (n=1735)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.4352 (IC base=+0.160)

- **PATRÓN** `sigma_h` > `0.005` → IC=+0.247 (n=1265)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.005 (IC base=+0.234)

- **PATRÓN** `drift_60min` |x|≤ `0.0931` → IC=+0.271 (n=470)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0931 (IC base=+0.234)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.246 (n=963)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.464` → IC=+0.245 (n=1461)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6131` → IC=+0.240 (n=429)
...
- **PATRÓN** `dist_vwap_pct` < `0.5738` → IC=+0.222 (n=1492)
...
- **PATRÓN** `volumen_regimen` < `1.2554` → IC=+0.219 (n=1422)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2554 (IC base=+0.217)

- **PATRÓN** `volumen_regimen` > `0.8736` → IC=+0.225 (n=948)
...
- **PATRÓN** `libro_liquidez` > `12544.4566` → IC=+0.221 (n=1270)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12544.4566 (IC base=+0.217)

- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.168 (n=991)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0039 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.0773` → IC=+0.167 (n=494)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.7006` → IC=+0.173 (n=1482)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.7006 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` < `0.1332` → IC=+0.157 (n=1321)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.274` → IC=+0.144 (n=1351)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 4.274 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` < `1.204` → IC=+0.152 (n=1482)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.190 (n=1923)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.199` → IC=+0.251 (n=384)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.199 (IC base=+0.185)

- **PATRÓN** `volumen_pendiente_norm` < `0.2094` → IC=+0.186 (n=1833)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` < 0.2094 (IC base=+0.185)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8194` → IC=+0.204 (n=789)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8194 (IC base=+0.185)
...
- **PATRÓN** `drift_60min` |x|≤ `0.6119` → IC=+0.211 (n=1575)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.0643` → IC=+0.247 (n=693)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0643 (IC base=+0.210)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.615` → IC=+0.236 (n=540)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.615 (IC base=+0.210)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.534` → IC=+0.210 (n=1709)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7621` → IC=+0.204 (n=636)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7621 (IC base=+0.210)

- **PATRÓN** `volumen_spike_ratio` > `2.1929` → IC=+0.217 (n=963)
...
- **PATRÓN** `libro_liquidez` > `1902.4584` → IC=+0.215 (n=714)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1902.4584 (IC base=+0.210)

- **PATRÓN** `ballena_activa_n` < `23.0` → IC=+0.214 (n=917)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.144 (n=102)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=2255)
...
- **PATRÓN** `volumen_regimen` < `0.8497` → IC=+0.165 (n=571)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.8497 (IC base=+0.018)

- **PATRÓN** `volumen_regimen` > `1.1607` → IC=+0.152 (n=285)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.1607 (IC base=+0.018)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5204` → IC=+0.177 (n=717)
...
- **FILTRO** `ibs_20min` > `0.25` → IC=-0.124 (n=2227)

  - _Acción_: SKIP cuando `ibs_20min` > 0.25
  - _Potencial_: sin este filtro IC_bueno=+0.125 (n=1099)

- **FILTRO** `sigma_ewma_delta_pct` > `8.692` → IC=-0.209 (n=355)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.692
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=2971)
...
- **FILTRO** `ibs_20min` < `0.7174` → IC=-0.153 (n=1529)
...
- **FILTRO** `ibs_20min` > `0.77` → IC=-0.203 (n=850)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.037 (n=2553)
...
- **PATRÓN** `dist_vwap_pct` < `0.253` → IC=+0.240 (n=2870)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.253 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` < `0.7099` → IC=+0.243 (n=1337)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7099 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` > `1.2058` → IC=+0.248 (n=1013)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2058 (IC base=+0.074)

- **PATRÓN** `volumen_pendiente_norm` > `0.2436` → IC=+0.304 (n=758)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2436 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` < `1.6005` → IC=+0.261 (n=1769)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6005 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` > `2.3041` → IC=+0.258 (n=1822)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3041 (IC base=+0.074)

- **PATRÓN** `ballena_activa_n` < `84.0` → IC=+0.265 (n=3896)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 84.0 (IC base=+0.074)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `sigma_ewma_delta_pct` > `4.541` → IC=-0.155 (n=543)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.541
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=1827)
...
- **PATRÓN** `volumen_spike_ratio` < `1.433` → IC=+0.168 (n=302)
...
- **PATRÓN** `ibs_20min` > `0.8641` → IC=+0.160 (n=692)
...
- **PATRÓN** `volumen_regimen` > `0.6746` → IC=+0.170 (n=853)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 0.6746 (IC base=+0.025)

- **PATRÓN** `volumen_pendiente_norm` > `0.2732` → IC=+0.232 (n=121)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2732 (IC base=+0.025)

- **PATRÓN** `volumen_spike_ratio` < `1.4243` → IC=+0.194 (n=312)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 1.4243 (IC base=+0.025)

- **PATRÓN** `volumen_spike_ratio` > `2.3875` → IC=+0.155 (n=311)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 2.3875 (IC base=+0.025)

- **PATRÓN** `ballena_activa_n` < `243.0` → IC=+0.195 (n=408)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 243.0 (IC base=+0.025)

- **PATRÓN** `dist_vwap_pct` < `0.1622` → IC=+0.224 (n=595)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1622 (IC base=+0.005)

- **PATRÓN** `volumen_regimen` > `0.8539` → IC=+0.229 (n=389)
...
- **PATRÓN** `ballena_activa_n` < `462.0` → IC=+0.219 (n=536)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.249 (n=1629)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1039` → IC=+0.258 (n=1382)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3403` → IC=+0.309 (n=202)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3403 (IC base=+0.282)

- **PATRÓN** `volumen_spike_ratio` < `1.7379` → IC=+0.284 (n=531)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7379 (IC base=+0.282)

- **PATRÓN** `volumen_spike_ratio` > `2.7242` → IC=+0.287 (n=548)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7242 (IC base=+0.282)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.286 (n=871)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.282)

- **PATRÓN** `libro_liquidez` > `1892.2584` → IC=+0.296 (n=591)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1892.2584 (IC base=+0.282)

- **PATRÓN** `ballena_activa_n` < `28.0` → IC=+0.285 (n=781)
...
- **FILTRO** `ibs_20min` < `0.2956` → IC=-0.185 (n=506)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.080 (n=1521)

- **FILTRO** `ibs_20min` > `0.7745` → IC=-0.187 (n=609)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7745
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=1828)

- **PATRÓN** `ibs_20min` > `0.8224` → IC=+0.160 (n=691)
...
- **PATRÓN** `ballena_activa_n` < `149.0` → IC=+0.246 (n=564)
...
- **PATRÓN** `dist_vwap_pct` < `0.6853` → IC=+0.202 (n=504)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6853 (IC base=-0.005)

- **PATRÓN** `volumen_regimen` < `1.1677` → IC=+0.213 (n=437)
...
- **PATRÓN** `ballena_activa_n` < `139.0` → IC=+0.251 (n=399)
...
- **FILTRO** `ibs_20min` < `0.7273` → IC=-0.193 (n=1093)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.277 (n=1100)

- **FILTRO** `ibs_20min` > `0.6875` → IC=-0.236 (n=562)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6875
  - _Potencial_: sin este filtro IC_bueno=+0.097 (n=1700)

- **FILTRO** `sigma_ewma_delta_pct` > `4.71` → IC=-0.185 (n=493)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.71
  - _Potencial_: sin este filtro IC_bueno=+0.070 (n=1769)

- **PATRÓN** `ibs_20min` > `0.7273` → IC=+0.277 (n=1100)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7273 (IC base=+0.043)

- **PATRÓN** `dist_vwap_pct` > `0.2157` → IC=+0.314 (n=499)
...
- **PATRÓN** `volumen_regimen` > `0.7306` → IC=+0.296 (n=724)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7306 (IC base=+0.043)

- **PATRÓN** `volumen_pendiente_norm` < `0.1016` → IC=+0.292 (n=756)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4185` → IC=+0.326 (n=262)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4185 (IC base=+0.043)

- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.314 (n=687)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 56.0 (IC base=+0.043)

- **PATRÓN** `ibs_20min` < `0.5806` → IC=+0.120 (n=1493)

  - _Acción_: Kelly boost +0.60€ cuando `ibs_20min` < 0.5806 (IC base=+0.014)

- **PATRÓN** `dist_vwap_pct` > `0.7294` → IC=+0.200 (n=138)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7294 (IC base=+0.014)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0972` → IC=+0.213 (n=534)
...
- **PATRÓN** `ibs_20min` > `0.7411` → IC=+0.322 (n=1198)
...
- **PATRÓN** `volumen_regimen` > `0.8606` → IC=+0.302 (n=895)
...
- **PATRÓN** `libro_liquidez` > `2636.9304` → IC=+0.293 (n=894)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2636.9304 (IC base=+0.276)

- **PATRÓN** `sigma_h` > `0.0154` → IC=+0.301 (n=959)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.3913` → IC=+0.306 (n=1439)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1415` → IC=+0.272 (n=577)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1415 (IC base=+0.273)

- **PATRÓN** `libro_liquidez` > `2629.6954` → IC=+0.276 (n=959)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2629.6954 (IC base=+0.273)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.170 (n=2687)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0049 (IC base=+0.167)

- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.200 (n=2681)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_regimen` < `1.212` → IC=+0.160 (n=5322)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.212 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` > `0.6316` → IC=+0.160 (n=5322)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.6316 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.2952` → IC=+0.196 (n=1202)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.2952 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.5586` → IC=+0.167 (n=3392)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.5586 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.6162` → IC=+0.176 (n=2570)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.6162 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `2407.6479` → IC=+0.169 (n=5356)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2407.6479 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `114.0` → IC=+0.180 (n=6943)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 114.0 (IC base=+0.167)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.182 (n=5111)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0066 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0814` → IC=+0.208 (n=2556)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.48` → IC=+0.225 (n=7670)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.48 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.9207` → IC=+0.144 (n=749)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.9207 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` < `0.2355` → IC=+0.157 (n=5556)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.2355 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.344` → IC=+0.193 (n=1300)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 10.344 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.1737` → IC=+0.151 (n=5548)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1737 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.2907` → IC=+0.216 (n=1104)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2907 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.5585` → IC=+0.164 (n=3071)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.5585 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.2496` → IC=+0.170 (n=3164)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.2496 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `115.0` → IC=+0.172 (n=6626)
...
- **PATRÓN** `ibs_20min` < `0.3427` → IC=+0.265 (n=1000)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3427 (IC base=+0.238)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.331` → IC=+0.251 (n=1080)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.331 (IC base=+0.238)

- **PATRÓN** `volumen_pendiente_norm` < `0.1592` → IC=+0.234 (n=945)
...
- **PATRÓN** `libro_spread` < `0.03` → IC=+0.238 (n=708)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.03 (IC base=+0.238)

- **PATRÓN** `libro_liquidez` > `1812.929` → IC=+0.243 (n=667)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1812.929 (IC base=+0.238)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.185 (n=1260)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 5.0 (IC base=+0.161)

- **PATRÓN** `ibs_20min` > `0.4039` → IC=+0.228 (n=1194)
...
- **PATRÓN** `libro_liquidez` > `11875.7279` → IC=+0.169 (n=1067)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 11875.7279 (IC base=+0.161)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.157 (n=1299)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.565` → IC=+0.187 (n=1299)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` < 0.565 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.1346` → IC=+0.160 (n=1288)
...
- **PATRÓN** `volumen_regimen` < `1.2096` → IC=+0.158 (n=1299)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4411` → IC=+0.145 (n=1187)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 2.4411 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `1.4211` → IC=+0.137 (n=1187)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6263` → IC=+0.200 (n=428)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.153 (n=1349)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` < 0.0073 (IC base=+0.122)

- **PATRÓN** `drift_60min` |x|≤ `0.3855` → IC=+0.143 (n=1348)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4638` → IC=+0.189 (n=1187)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` < 0.4638 (IC base=+0.122)

- **PATRÓN** `dist_vwap_pct` < `0.2152` → IC=+0.137 (n=1363)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.2152 (IC base=+0.122)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.957` → IC=+0.167 (n=475)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 6.957 (IC base=+0.122)

- **PATRÓN** `volumen_regimen` < `0.8485` → IC=+0.149 (n=899)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 0.8485 (IC base=+0.122)
...
- **PATRÓN** `libro_liquidez` > `10077.5355` → IC=+0.156 (n=611)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 10077.5355 (IC base=+0.122)

### GBM_LATE_15M_MULTIHORIZONTE#SOL#15min
- **PATRÓN** `sigma_h` > `0.0101` → IC=+0.154 (n=660)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0101 (IC base=+0.118)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.138 (n=1487)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 5.0 (IC base=+0.118)

- **PATRÓN** `ibs_20min` > `0.5143` → IC=+0.204 (n=1453)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5143 (IC base=+0.118)

- **PATRÓN** `dist_vwap_pct` > `0.5191` → IC=+0.197 (n=628)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.5191 (IC base=+0.118)
...
- **PATRÓN** `volumen_regimen` < `1.2173` → IC=+0.130 (n=1453)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 1.2173 (IC base=+0.118)

- **PATRÓN** `volumen_regimen` > `0.6466` → IC=+0.123 (n=1453)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4397` → IC=+0.143 (n=468)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.4397 (IC base=+0.118)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.127 (n=1516)

  - _Acción_: Kelly boost +0.63€ cuando `libro_spread` < 0.02 (IC base=+0.118)

- **PATRÓN** `libro_liquidez` > `2898.7698` → IC=+0.194 (n=659)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 2898.7698 (IC base=+0.118)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.136 (n=1104)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.130 (n=1490)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 5.0 (IC base=+0.112)

- **PATRÓN** `ibs_20min` < `0.5676` → IC=+0.209 (n=1471)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5676 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `1.013` → IC=+0.140 (n=201)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 1.013 (IC base=+0.112)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2745` → IC=+0.156 (n=181)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_pendiente_norm` > 0.2745 (IC base=+0.112)

- **PATRÓN** `volumen_spike_ratio` < `1.4547` → IC=+0.131 (n=440)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` < 1.4547 (IC base=+0.112)

- **PATRÓN** `libro_liquidez` > `3085.1851` → IC=+0.161 (n=490)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 3085.1851 (IC base=+0.112)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2006` → IC=+0.193 (n=1134)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` < 2.2006 (IC base=+0.202)

- **PATRÓN** `volumen_spike_ratio` > `1.4336` → IC=+0.197 (n=1288)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.4336 (IC base=+0.202)

- **PATRÓN** `libro_liquidez` > `2604.596` → IC=+0.209 (n=954)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2604.596 (IC base=+0.202)
...
- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.174 (n=648)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0089 (IC base=+0.147)

- **PATRÓN** `drift_60min` |x|≤ `0.356` → IC=+0.150 (n=1710)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.356 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.185 (n=966)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 15.0 (IC base=+0.147)

- **PATRÓN** `ibs_20min` > `0.5385` → IC=+0.188 (n=1737)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.5385 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.8719` → IC=+0.177 (n=314)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.8719 (IC base=+0.147)
...
- **PATRÓN** `volumen_regimen` < `0.8753` → IC=+0.164 (n=1139)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.8753 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` > `0.6251` → IC=+0.150 (n=1708)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 0.6251 (IC base=+0.147)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4413` → IC=+0.162 (n=625)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 1.4413 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` > `2.2101` → IC=+0.153 (n=849)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 2.2101 (IC base=+0.147)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.152 (n=2203)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.02 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `12451.6072` → IC=+0.160 (n=648)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 12451.6072 (IC base=+0.147)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.167 (n=1723)
...
- **PATRÓN** `libro_liquidez` > `2755.5763` → IC=+0.122 (n=1801)

  - _Acción_: Kelly boost +0.61€ cuando `libro_liquidez` > 2755.5763 (IC base=+0.110)

- **PATRÓN** `ballena_activa_n` < `29.0` → IC=+0.121 (n=821)

  - _Acción_: Kelly boost +0.60€ cuando `ballena_activa_n` < 29.0 (IC base=+0.110)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.210 (n=632)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.2332` → IC=+0.195 (n=533)
...
- **PATRÓN** `volumen_regimen` < `0.8389` → IC=+0.200 (n=404)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.203 (n=676)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.195)

- **PATRÓN** `libro_liquidez` > `12459.2276` → IC=+0.221 (n=202)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12459.2276 (IC base=+0.195)

- **PATRÓN** `ibs_20min` < `0.084` → IC=+0.158 (n=191)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` < 0.084 (IC base=+0.092)

- **PATRÓN** `volumen_regimen` < `0.6864` → IC=+0.134 (n=252)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 0.6864 (IC base=+0.092)

- **PATRÓN** `volumen_pendiente_norm` > `0.1664` → IC=+0.130 (n=133)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_pendiente_norm` > 0.1664 (IC base=+0.092)
...
- **PATRÓN** `sigma_h` > `0.0073` → IC=+0.157 (n=295)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` > 0.0073 (IC base=+0.130)

- **PATRÓN** `drift_60min` |x|≤ `0.1179` → IC=+0.147 (n=148)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.1179 (IC base=+0.130)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.164 (n=415)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 8.0 (IC base=+0.130)

- **PATRÓN** `ibs_20min` > `0.8906` → IC=+0.241 (n=295)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8906 (IC base=+0.130)

- **PATRÓN** `dist_vwap_pct` > `0.9532` → IC=+0.226 (n=82)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9532 (IC base=+0.130)
...
- **PATRÓN** `volumen_regimen` < `1.0738` → IC=+0.148 (n=390)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 1.0738 (IC base=+0.130)

- **PATRÓN** `volumen_pendiente_norm` > `0.1831` → IC=+0.172 (n=126)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.1831 (IC base=+0.130)

- **PATRÓN** `volumen_spike_ratio` > `2.2017` → IC=+0.170 (n=192)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.2017 (IC base=+0.130)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.132 (n=482)

  - _Acción_: Kelly boost +0.66€ cuando `libro_spread` < 0.02 (IC base=+0.130)

- **PATRÓN** `libro_liquidez` > `3131.3797` → IC=+0.187 (n=148)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 3131.3797 (IC base=+0.130)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5941` → IC=+0.186 (n=3300)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 2.5941 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `2352.5852` → IC=+0.172 (n=6857)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 2352.5852 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `86.0` → IC=+0.194 (n=7854)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 86.0 (IC base=+0.169)

- **PATRÓN** `sigma_h` < `0.007` → IC=+0.192 (n=6223)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.007 (IC base=+0.181)

- **PATRÓN** `drift_60min` |x|≤ `0.1495` → IC=+0.188 (n=4100)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5667` → IC=+0.237 (n=9316)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5667 (IC base=+0.181)

- **PATRÓN** `dist_vwap_pct` < `0.2514` → IC=+0.161 (n=5766)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2514 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.037` → IC=+0.200 (n=1305)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.037 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.732` → IC=+0.182 (n=9008)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.732 (IC base=+0.181)

- **PATRÓN** `volumen_regimen` < `0.705` → IC=+0.162 (n=2812)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.705 (IC base=+0.181)

- **PATRÓN** `volumen_regimen` > `1.2037` → IC=+0.153 (n=2131)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.2037 (IC base=+0.181)

- **PATRÓN** `volumen_pendiente_norm` > `0.2876` → IC=+0.240 (n=1228)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2876 (IC base=+0.181)

- **PATRÓN** `volumen_spike_ratio` > `2.6192` → IC=+0.193 (n=2854)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 2.6192 (IC base=+0.181)

- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.193 (n=5538)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.564` → IC=+0.341 (n=394)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.564 (IC base=+0.189)
...
- **PATRÓN** `sigma_h` > `0.0045` → IC=+0.260 (n=1370)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0045 (IC base=+0.257)

- **PATRÓN** `drift_60min` |x|≤ `0.1294` → IC=+0.280 (n=603)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1294 (IC base=+0.257)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.266 (n=1242)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.257)

- **PATRÓN** `ibs_20min` < `0.3517` → IC=+0.287 (n=1207)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3517 (IC base=+0.257)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.721` → IC=+0.261 (n=1539)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.721 (IC base=+0.257)

- **PATRÓN** `volumen_pendiente_norm` > `0.2256` → IC=+0.279 (n=256)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2256 (IC base=+0.257)

- **PATRÓN** `volumen_spike_ratio` < `1.5484` → IC=+0.256 (n=555)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5484 (IC base=+0.257)

- **PATRÓN** `volumen_spike_ratio` > `2.6339` → IC=+0.277 (n=420)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6339 (IC base=+0.257)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.257 (n=965)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.03 (IC base=+0.257)

- **PATRÓN** `libro_liquidez` > `1996.8992` → IC=+0.265 (n=457)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1996.8992 (IC base=+0.257)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0853` → IC=+0.159 (n=550)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.0853 (IC base=+0.150)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.164 (n=1726)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 5.0 (IC base=+0.150)

- **PATRÓN** `ibs_20min` > `0.3081` → IC=+0.203 (n=1650)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3081 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.1333` → IC=+0.184 (n=933)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.1333 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.73` → IC=+0.169 (n=376)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 9.73 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.235` → IC=+0.154 (n=1488)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 4.235 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` < `0.63` → IC=+0.181 (n=550)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.63 (IC base=+0.150)

- **PATRÓN** `volumen_pendiente_norm` > `0.2665` → IC=+0.202 (n=236)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2665 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` < `2.1148` → IC=+0.160 (n=1404)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.1148 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.7559` → IC=+0.157 (n=1063)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.7559 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `15757.8757` → IC=+0.159 (n=748)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 15757.8757 (IC base=+0.150)

- **PATRÓN** `ballena_activa_n` < `472.0` → IC=+0.158 (n=1528)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 472.0 (IC base=+0.150)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.166 (n=1429)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.2708` → IC=+0.239 (n=953)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2708 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` > `0.6916` → IC=+0.158 (n=232)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.6916 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` < `0.135` → IC=+0.166 (n=1289)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.279` → IC=+0.154 (n=1295)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 4.279 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` < `1.1894` → IC=+0.165 (n=1429)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.1894 (IC base=+0.153)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4003` → IC=+0.161 (n=1330)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.4003 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` > `1.7532` → IC=+0.165 (n=887)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.7532 (IC base=+0.153)

- **PATRÓN** `ballena_activa_n` < `420.0` → IC=+0.158 (n=1088)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 420.0 (IC base=+0.153)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0121` → IC=+0.245 (n=556)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0121 (IC base=+0.218)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.226 (n=1747)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.218)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.223 (n=1696)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2104` → IC=+0.221 (n=1654)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8216` → IC=+0.241 (n=721)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8216 (IC base=+0.218)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.228 (n=1180)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.218)

- **PATRÓN** `libro_liquidez` > `1803.3584` → IC=+0.218 (n=1111)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1803.3584 (IC base=+0.218)

- **PATRÓN** `sigma_h` < `0.0117` → IC=+0.237 (n=1555)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0117 (IC base=+0.231)

- **PATRÓN** `drift_60min` |x|≤ `0.1686` → IC=+0.238 (n=685)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.36` → IC=+0.266 (n=1369)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.36 (IC base=+0.231)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.751` → IC=+0.272 (n=578)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.751 (IC base=+0.231)

- **PATRÓN** `volumen_pendiente_norm` > `0.3433` → IC=+0.306 (n=230)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3433 (IC base=+0.231)

- **PATRÓN** `volumen_spike_ratio` < `1.7484` → IC=+0.227 (n=629)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7484 (IC base=+0.231)

- **PATRÓN** `volumen_spike_ratio` > `2.1845` → IC=+0.233 (n=953)
...
- **PATRÓN** `libro_liquidez` > `1958.1784` → IC=+0.239 (n=519)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1958.1784 (IC base=+0.231)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.229 (n=1361)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4388` → IC=+0.146 (n=1752)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.4388 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.152 (n=1832)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 5.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` > `0.8762` → IC=+0.261 (n=794)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8762 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.374` → IC=+0.163 (n=695)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.374 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.195` → IC=+0.159 (n=725)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 4.195 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `0.8748` → IC=+0.156 (n=1168)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.8748 (IC base=+0.136)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5214` → IC=+0.149 (n=747)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.5214 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.7664` → IC=+0.150 (n=1131)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.7664 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `8024.5486` → IC=+0.234 (n=794)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8024.5486 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `91.0` → IC=+0.149 (n=724)

  - _Acción_: Kelly boost +0.74€ cuando `ballena_activa_n` < 91.0 (IC base=+0.136)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.160 (n=1255)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0066 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.4486` → IC=+0.153 (n=1426)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.4486 (IC base=+0.136)
...
- **PATRÓN** `ibs_20min` < `0.7033` → IC=+0.181 (n=1426)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` < 0.7033 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `0.5983` → IC=+0.140 (n=1577)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.5983 (IC base=+0.136)
...
- **PATRÓN** `volumen_regimen` < `0.6958` → IC=+0.152 (n=628)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.6958 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` > `1.1938` → IC=+0.144 (n=475)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 1.1938 (IC base=+0.136)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4415` → IC=+0.150 (n=1353)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.4415 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `7280.6728` → IC=+0.196 (n=646)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 7280.6728 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `177.0` → IC=+0.143 (n=1347)

  - _Acción_: Kelly boost +0.71€ cuando `ballena_activa_n` < 177.0 (IC base=+0.136)

### GBM_LATE_15M_TARDIO#SOL#15min
- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.141 (n=1161)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` > 0.0082 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=1792)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.191 (n=1744)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` > 0.4706 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `1.0902` → IC=+0.197 (n=361)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 1.0902 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.503` → IC=+0.235 (n=652)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.503 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` < `0.8936` → IC=+0.135 (n=1161)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 0.8936 (IC base=+0.113)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.127 (n=1750)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.02 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `2897.8254` → IC=+0.250 (n=581)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2897.8254 (IC base=+0.113)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.132 (n=1350)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1332` → IC=+0.161 (n=562)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.1332 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.125 (n=1741)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 5.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` < `0.6364` → IC=+0.205 (n=1685)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.488` → IC=+0.127 (n=1628)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 3.488 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` < `0.6488` → IC=+0.156 (n=562)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.6488 (IC base=+0.114)

- **PATRÓN** `volumen_pendiente_norm` > `0.2224` → IC=+0.164 (n=263)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.2224 (IC base=+0.114)

- **PATRÓN** `volumen_spike_ratio` < `1.4462` → IC=+0.143 (n=508)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 1.4462 (IC base=+0.114)

- **PATRÓN** `libro_liquidez` > `2844.2019` → IC=+0.171 (n=561)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 2844.2019 (IC base=+0.114)

- **PATRÓN** `ballena_activa_n` < `51.0` → IC=+0.121 (n=1316)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.214 (n=1815)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.5135` → IC=+0.249 (n=1740)
...
- **PATRÓN** `volumen_regimen` < `1.2478` → IC=+0.211 (n=1741)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2478 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` > `0.64` → IC=+0.215 (n=1740)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5002` → IC=+0.235 (n=561)
...
- **PATRÓN** `libro_liquidez` > `2633.3465` → IC=+0.219 (n=1160)
...
- **PATRÓN** `dist_vwap_pct` < `0.9274` → IC=+0.202 (n=2059)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.9274 (IC base=+0.200)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1818` → IC=+0.197 (n=1470)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 2.1818 (IC base=+0.200)

- **PATRÓN** `volumen_spike_ratio` > `1.5433` → IC=+0.198 (n=1493)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.144 (n=3123)

- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.167 (n=2637)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0092 (IC base=+0.156)

- **PATRÓN** `drift_60min` |x|≤ `0.5194` → IC=+0.165 (n=2996)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.5194 (IC base=+0.156)
...
- **PATRÓN** `ibs_20min` > `0.9412` → IC=+0.214 (n=999)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9412 (IC base=+0.156)

- **PATRÓN** `dist_vwap_pct` > `0.1894` → IC=+0.169 (n=1110)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.1894 (IC base=+0.156)

- **PATRÓN** `dist_vwap_pct` < `0.4848` → IC=+0.152 (n=1862)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.4848 (IC base=+0.156)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.188` → IC=+0.188 (n=491)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 10.188 (IC base=+0.156)

- **PATRÓN** `volumen_regimen` > `0.8972` → IC=+0.169 (n=1323)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 0.8972 (IC base=+0.156)

- **PATRÓN** `volumen_pendiente_norm` > `0.1706` → IC=+0.191 (n=822)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.1706 (IC base=+0.156)

- **PATRÓN** `volumen_spike_ratio` < `1.4563` → IC=+0.164 (n=987)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.4563 (IC base=+0.156)

- **PATRÓN** `volumen_spike_ratio` > `1.8741` → IC=+0.165 (n=1973)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.8741 (IC base=+0.156)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.158 (n=2113)

  - _Acción_: Kelly boost +0.79€ cuando `libro_spread` < 0.01 (IC base=+0.156)

- **PATRÓN** `libro_liquidez` > `3902.3621` → IC=+0.164 (n=1997)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 3902.3621 (IC base=+0.156)

- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.200 (n=788)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0039 (IC base=+0.142)

- **PATRÓN** `drift_60min` |x|≤ `0.4873` → IC=+0.160 (n=2354)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.1819` → IC=+0.173 (n=1036)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.1819 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.6828` → IC=+0.164 (n=453)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.6828 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.273` → IC=+0.153 (n=2343)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` < 6.273 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `1.2449` → IC=+0.148 (n=2252)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 1.2449 (IC base=+0.142)

- **PATRÓN** `volumen_pendiente_norm` < `0.0968` → IC=+0.145 (n=2132)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_pendiente_norm` < 0.0968 (IC base=+0.142)

- **PATRÓN** `volumen_pendiente_norm` > `0.0724` → IC=+0.145 (n=1088)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_pendiente_norm` > 0.0724 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` < `1.4246` → IC=+0.155 (n=775)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 1.4246 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` > `1.8143` → IC=+0.145 (n=1549)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.8143 (IC base=+0.142)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.144 (n=3123)

  - _Acción_: Kelly boost +0.72€ cuando `libro_spread` < 0.01 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `7267.9636` → IC=+0.155 (n=2103)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 7267.9636 (IC base=+0.142)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.169 (n=866)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0073 (IC base=+0.163)

- **PATRÓN** `sigma_h` > `0.0043` → IC=+0.169 (n=984)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0043 (IC base=+0.163)

- **PATRÓN** `drift_60min` |x|≤ `0.3878` → IC=+0.165 (n=866)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.82€ cuando `drift_60min` |x|≤ 0.3878 (IC base=+0.163)
...
- **PATRÓN** `ibs_20min` < `0.545` → IC=+0.172 (n=656)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.545 (IC base=+0.163)

- **PATRÓN** `ibs_20min` > `0.886` → IC=+0.173 (n=328)
...
- **PATRÓN** `dist_vwap_pct` < `0.424` → IC=+0.171 (n=914)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.424 (IC base=+0.163)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.134` → IC=+0.174 (n=882)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` < 4.134 (IC base=+0.163)

- **PATRÓN** `volumen_regimen` < `1.0856` → IC=+0.164 (n=866)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.0856 (IC base=+0.163)

- **PATRÓN** `volumen_regimen` > `0.6431` → IC=+0.167 (n=985)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 0.6431 (IC base=+0.163)

- **PATRÓN** `volumen_pendiente_norm` < `0.1051` → IC=+0.165 (n=905)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` < 0.1051 (IC base=+0.163)

- **PATRÓN** `volumen_pendiente_norm` > `0.0781` → IC=+0.171 (n=429)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` > 0.0781 (IC base=+0.163)

- **PATRÓN** `volumen_spike_ratio` < `1.4358` → IC=+0.176 (n=322)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 1.4358 (IC base=+0.163)

- **PATRÓN** `volumen_spike_ratio` > `1.5228` → IC=+0.165 (n=863)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.5228 (IC base=+0.163)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.169 (n=979)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4985` → IC=+0.176 (n=814)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.7466` → IC=+0.153 (n=814)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.7466 (IC base=+0.150)

- **PATRÓN** `ibs_20min` > `0.0993` → IC=+0.158 (n=814)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` > 0.0993 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.6243` → IC=+0.182 (n=177)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.6243 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.388` → IC=+0.160 (n=738)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` < 4.388 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` < `0.6472` → IC=+0.186 (n=272)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` < 0.6472 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` > `0.7262` → IC=+0.153 (n=727)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.7262 (IC base=+0.150)

- **PATRÓN** `volumen_pendiente_norm` > `0.0738` → IC=+0.167 (n=343)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.0738 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` < `2.2051` → IC=+0.162 (n=702)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.2051 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.7807` → IC=+0.154 (n=532)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.7807 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `7573.3328` → IC=+0.175 (n=814)
...
- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.149 (n=223)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0069 (IC base=+0.114)

- **PATRÓN** `drift_60min` |x|≤ `0.3998` → IC=+0.156 (n=149)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.3998 (IC base=+0.114)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.170 (n=98)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 6.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` < `0.1538` → IC=+0.214 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1538 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` > `0.5986` → IC=+0.194 (n=106)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.5986 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.684` → IC=+0.129 (n=122)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` > 2.684 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` < `0.9487` → IC=+0.129 (n=149)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 0.9487 (IC base=+0.114)

- **PATRÓN** `volumen_pendiente_norm` < `0.089` → IC=+0.174 (n=170)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` < 0.089 (IC base=+0.114)

- **PATRÓN** `volumen_spike_ratio` < `1.8554` → IC=+0.137 (n=144)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 1.8554 (IC base=+0.114)

- **PATRÓN** `libro_liquidez` > `3291.3893` → IC=+0.147 (n=222)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 3291.3893 (IC base=+0.114)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.156 (n=187)

  - _Acción_: Kelly boost +0.78€ cuando `ballena_activa_n` < 49.0 (IC base=+0.114)
...
### LEADLAG_BTC_XRP_15M
- **PATRÓN** `libro_liquidez` > `2843.9766` → IC=+0.161 (n=328)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 2843.9766 (IC base=+0.108)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=1840)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.133 (n=469)
...
- **FILTRO** `restante_min` < `3.82` → IC=-0.125 (n=238)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=485)

- **PATRÓN** `py_entrada` < `0.47` → IC=+0.165 (n=192)

  - _Acción_: Kelly boost +0.82€ cuando `py_entrada` < 0.47 (IC base=+0.020)
...
- **PATRÓN** `py_entrada` < `0.56` → IC=+0.167 (n=58)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `py_entrada` < 0.56 (IC base=+0.069)

- **PATRÓN** `profundidad_ratio` > `79.6` → IC=+0.120 (n=77)

  - _Acción_: Kelly boost +0.60€ cuando `profundidad_ratio` > 79.6 (IC base=+0.069)

### LIQUIDACIONES_DEPTH_FASE0#DOGE#15min
- **FILTRO** `restante_min` > `13.27` → IC=-0.278 (n=16)
...
- **FILTRO** `profundidad_ratio` < `114.6` → IC=-0.158 (n=36)
...
- **PATRÓN** `py_entrada` < `0.6` → IC=+0.167 (n=46)

  - _Acción_: Kelly boost +0.83€ cuando `py_entrada` < 0.6 (IC base=+0.051)

- **PATRÓN** `restante_min` > `13.49` → IC=+0.220 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `restante_min` > 13.49 (IC base=+0.051)

- **PATRÓN** `lag_apertura_s` < `90.81` → IC=+0.220 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `lag_apertura_s` < 90.81 (IC base=+0.051)

- **PATRÓN** `profundidad_ratio` > `9.3` → IC=+0.140 (n=23)

  - _Acción_: Kelly boost +0.70€ cuando `profundidad_ratio` > 9.3 (IC base=+0.051)
...
- **FILTRO** `py_entrada` < `0.4` → IC=-0.283 (n=21)

  - _Acción_: SKIP cuando `py_entrada` < 0.4
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=69)

- **FILTRO** `restante_min` > `10.3` → IC=-0.194 (n=60)

  - _Acción_: SKIP cuando `restante_min` > 10.3
  - _Potencial_: sin este filtro IC_bueno=+0.094 (n=30)

### LIQUIDACIONES_DEPTH_FASE0#XRP#5min
- **FILTRO** `restante_min` < `2.85` → IC=-0.190 (n=27)
...
- **FILTRO** `lag_apertura_s` > `129.03` → IC=-0.190 (n=27)

  - _Acción_: SKIP cuando `lag_apertura_s` > 129.03
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=82)

- **PATRÓN** `restante_min` > `3.37` → IC=+0.123 (n=51)

  - _Acción_: Kelly boost +0.61€ cuando `restante_min` > 3.37 (IC base=+0.026)

- **PATRÓN** `lag_apertura_s` < `97.74` → IC=+0.123 (n=51)

  - _Acción_: Kelly boost +0.61€ cuando `lag_apertura_s` < 97.74 (IC base=+0.026)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=7577)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.166 (n=3596)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.058 (n=10865)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.166 (n=3725)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.035 (n=11250)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.46` → IC=-0.208 (n=617)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.100 (n=1896)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.47` → IC=-0.193 (n=629)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=1934)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.202 (n=649)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.062 (n=2032)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.485` → IC=-0.175 (n=610)

  - _Acción_: SKIP cuando `py_entrada` < 0.485
  - _Potencial_: sin este filtro IC_bueno=+0.085 (n=1893)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.176 (n=653)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=2033)

### MOMENTUM_IBS_15M_FADE
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=2798)

- **FILTRO** `py_entrada` > `0.595` → IC=-0.212 (n=709)

  - _Acción_: SKIP cuando `py_entrada` > 0.595
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=2284)

- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)

  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=2972)

### MOMENTUM_IBS_15M_FADE#BTC#15min
- **FILTRO** `hora_utc` < `15.0` → IC=-0.175 (n=121)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=392)

- **FILTRO** `ibs_20min` > `0.1668` → IC=-0.146 (n=128)

  - _Acción_: SKIP cuando `ibs_20min` > 0.1668
  - _Potencial_: sin este filtro IC_bueno=-0.076 (n=385)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.129 (n=10193)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=22860)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.273 (n=8112)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=24941)

- **FILTRO** `ibs_7min` < `0.2812` → IC=-0.234 (n=8263)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2812
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=24790)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.156 (n=11229)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=21824)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.231 (n=10172)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=31348)

- **FILTRO** `ibs_7min` > `0.2929` → IC=-0.178 (n=10376)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2929
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=31144)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.136 (n=1664)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3817)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.310 (n=1303)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4178)

- **FILTRO** `ibs_7min` < `0.7111` → IC=-0.250 (n=1807)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7111
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=3674)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.185 (n=1235)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4246)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.260 (n=1753)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=5383)

- **FILTRO** `drift_7min_pct` |x|> `0.1121` → IC=-0.126 (n=2424)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1121
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=4712)

- **FILTRO** `ibs_7min` > `0.7889` → IC=-0.207 (n=1783)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7889
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=5353)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.137 (n=1334)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=4367)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.250 (n=1381)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=4320)

- **FILTRO** `ibs_7min` < `0.7468` → IC=-0.192 (n=1424)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7468
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4277)

- **FILTRO** `ballena_activa_n` > `161.0` → IC=-0.176 (n=1420)

  - _Acción_: SKIP cuando `ballena_activa_n` > 161.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4281)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.256 (n=1446)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=4345)

- **FILTRO** `ibs_7min` > `0.2609` → IC=-0.180 (n=1446)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2609
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=4345)

- **FILTRO** `ballena_activa_n` > `154.0` → IC=-0.183 (n=1444)

  - _Acción_: SKIP cuando `ballena_activa_n` > 154.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=4347)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.165 (n=1470)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.082 (n=3732)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.305 (n=1286)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=3916)

- **FILTRO** `ibs_7min` < `0.7063` → IC=-0.242 (n=1716)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7063
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=3486)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.222 (n=1190)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4012)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.237 (n=1746)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=5892)

- **FILTRO** `ibs_7min` > `0.7474` → IC=-0.172 (n=1909)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7474
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=5729)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=1755)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=3702)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.234 (n=1613)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=3844)

- **FILTRO** `ibs_7min` < `0.7403` → IC=-0.180 (n=1364)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7403
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4093)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.172 (n=1335)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=4122)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.267 (n=1230)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=4332)

- **FILTRO** `ibs_7min` > `0.2745` → IC=-0.177 (n=1390)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2745
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=4172)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.183 (n=1369)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4193)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.253 (n=1416)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=4339)

- **FILTRO** `ibs_7min` < `0.3` → IC=-0.233 (n=1434)

  - _Acción_: SKIP cuando `ibs_7min` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4321)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.179 (n=1873)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=6027)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.254 (n=1778)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3679)

- **FILTRO** `ibs_7min` < `0.3` → IC=-0.227 (n=1360)

  - _Acción_: SKIP cuando `ibs_7min` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=4097)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.214 (n=1328)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=4129)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.206 (n=1783)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.010 (n=5710)
...
- **FILTRO** `sigma_h` < `0.008` → IC=-0.129 (n=103)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=2959)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.222 (n=16)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=641)
...
- **PATRÓN** `ibs_15` < `0.1176` → IC=+0.159 (n=490)
...
- **FILTRO** `sigma_h` > `0.0129` → IC=-0.221 (n=669)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=2010)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.195 (n=910)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1193` → IC=+0.249 (n=1217)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1193 (IC base=-0.031)

- **PATRÓN** `ibs_15` < `0.3455` → IC=+0.277 (n=1825)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **FILTRO** `sigma_h` > `0.0068` → IC=-0.225 (n=409)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `sigma_h` < `0.0034` → IC=-0.232 (n=409)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.188 (n=1228)

- **FILTRO** `hora_utc` > `7.0` → IC=-0.211 (n=1035)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 7.0
...
- **FILTRO** `sigma_ewma_delta_pct` > `19.574` → IC=-0.250 (n=294)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.188 (n=1343)
...
- **PATRÓN** `ibs_15` < `0.3453` → IC=+0.270 (n=712)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **PATRÓN** `sigma_ewma_delta_pct` < `12.221` → IC=+0.238 (n=755)
...
- **FILTRO** `drift_15min` |x|> `1.2607` → IC=-0.267 (n=195)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 1.2607
  - _Potencial_: sin este filtro IC_bueno=-0.174 (n=587)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.103` → IC=+0.344 (n=209)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.103 (IC base=-0.042)

- **PATRÓN** `ibs_15` < `0.34` → IC=+0.305 (n=479)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1176 sube el IC de +0.051 a +0.159 en UPDOWN_GBM#XRP#15min (n=490). Ya aplicado como kelly_boost=+0.79€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3455 sube el IC de -0.031 a +0.277 en UPDOWN_GBM_15M_TARDIO (n=1825). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.3453 sube el IC de +0.231 a +0.270 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=712). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#XRP#15min**: dentro de BUY_NO, IBS < 0.34 sube el IC de -0.042 a +0.305 en UPDOWN_GBM_15M_TARDIO#XRP#15min (n=479). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 28685 | -0.088 | -3842.85€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1483 | -0.046 | -221.79€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 27202 | -0.091 | -3621.06€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3769 | -0.098 | -618.41€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3769 | -0.098 | -618.41€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 1483 | -0.046 | -221.79€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 1483 | -0.046 | -221.79€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 3482 | -0.096 | -787.96€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 3482 | -0.096 | -787.96€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 7351 | -0.021 | -689.72€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 7351 | -0.021 | -689.72€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 6955 | -0.090 | -435.54€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 6955 | -0.090 | -435.54€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 5645 | -0.173 | -1089.43€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 5645 | -0.173 | -1089.43€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 18698 | -0.027 | +3995.79€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 4874 | +0.000 | +1813.81€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 13824 | -0.037 | +2181.98€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 18698 | -0.027 | +3995.79€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 4874 | +0.000 | +1813.81€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 13824 | -0.037 | +2181.98€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 94503 | +0.112 | -4772.00€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 14215 | +0.185 | -436.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 381 | -0.080 | -50.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 73785 | +0.100 | -4068.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 6122 | +0.107 | -216.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 12270 | +0.097 | -1042.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 45 | -0.160 | -0.32€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 12210 | +0.099 | -1030.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 19114 | +0.131 | -375.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 4489 | +0.204 | -135.27€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 12233 | +0.111 | -202.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 2350 | +0.103 | -15.99€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 12309 | +0.090 | -1121.90€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 52 | -0.093 | -6.98€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#DOGE#240min | 15 | -0.243 | -11.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 12242 | +0.091 | -1103.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 20093 | +0.123 | -396.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 5499 | +0.175 | -81.57€ | 1 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 12365 | +0.105 | -249.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 2217 | +0.098 | -56.85€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 18433 | +0.113 | -1104.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 4083 | +0.188 | -220.42€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 284 | -0.038 | +3.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 12511 | +0.091 | -743.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1555 | +0.125 | -144.10€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 12284 | +0.100 | -730.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 47 | -0.031 | +8.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 13 | -0.022 | -0.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 12224 | +0.101 | -738.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 14996 | +0.191 | -996.95€ | 1 | 5 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 14996 | +0.191 | -996.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 3565 | +0.166 | -379.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 3565 | +0.166 | -379.92€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC | 1274 | +0.195 | -14.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC#15min | 1274 | +0.195 | -14.30€ | 4 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 3515 | +0.179 | -304.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 3515 | +0.179 | -304.27€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 3115 | +0.239 | -100.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 3115 | +0.239 | -100.48€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 3448 | +0.192 | -211.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 3448 | +0.192 | -211.73€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_EXTREMO |