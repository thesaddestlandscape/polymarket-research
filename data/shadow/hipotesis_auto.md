# Hipótesis automáticas — 2026-08-18 16:55 UTC
_Generado por shadow_postmortem.py sobre 65392 resoluciones (PNL=+6637.93€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `n_ballenas` < `5.0` → IC=-0.126 (n=2057)

  - _Acción_: SKIP cuando `n_ballenas` < 5.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=1400)

- **FILTRO** `restante_s_al_confirmar` < `149.51` → IC=-0.258 (n=864)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 149.51
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=2593)
...
- **FILTRO** `restante_s_al_confirmar` < `246.47` → IC=-0.157 (n=610)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 246.47
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=204)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `144.04` → IC=-0.188 (n=200)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.197 (n=2242)

  - _Acción_: Kelly boost +0.99€ cuando `py_entrada` > 0.7 (IC base=+0.095)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.164 (n=1160)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.095)

- **PATRÓN** `libro_liquidez` > `2373.5135` → IC=+0.166 (n=1122)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 2373.5135 (IC base=+0.095)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.155 (n=3821)
...
- **PATRÓN** `py_entrada` < `0.335` → IC=+0.286 (n=1381)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.206 (n=362)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.201 (n=453)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.144 (n=391)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.6` → IC=+0.172 (n=181)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `py_entrada` > 0.6 (IC base=+0.134)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.178 (n=572)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.73` → IC=+0.175 (n=599)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=670)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.7` → IC=+0.192 (n=225)

  - _Acción_: Kelly boost +0.96€ cuando `py_entrada` < 0.7 (IC base=+0.162)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.163 (n=366)

  - _Acción_: Kelly boost +0.82€ cuando `py_entrada` > 0.73 (IC base=+0.162)

### FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.216 (n=596)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.72` → IC=+0.188 (n=498)
...
- **PATRÓN** `hora_utc` > `14.0` → IC=+0.195 (n=2260)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` > 14.0 (IC base=+0.190)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.203 (n=1528)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.190)

- **PATRÓN** `py_entrada` > `0.76` → IC=+0.252 (n=1509)
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.130 (n=576)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` < 11.0 (IC base=+0.099)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.138 (n=401)

  - _Acción_: Kelly boost +0.69€ cuando `py_entrada` > 0.73 (IC base=+0.099)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.293 (n=254)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.288 (n=343)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.74 (IC base=+0.247)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.215 (n=272)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `restante_min` < `3.71` → IC=+0.171 (n=259)

  - _Acción_: Kelly boost +0.85€ cuando `restante_min` < 3.71 (IC base=+0.155)

- **PATRÓN** `restante_min` > `4.88` → IC=+0.192 (n=274)

  - _Acción_: Kelly boost +0.96€ cuando `restante_min` > 4.88 (IC base=+0.155)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=808)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `7.36` → IC=+0.194 (n=259)

  - _Acción_: Kelly boost +0.97€ cuando `lag_apertura_s` < 7.36 (IC base=+0.155)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min
- **PATRÓN** `py_entrada` < `0.43` → IC=+0.180 (n=705)

  - _Acción_: Kelly boost +0.90€ cuando `py_entrada` < 0.43 (IC base=+0.145)

- **PATRÓN** `restante_min` < `3.88` → IC=+0.152 (n=265)

  - _Acción_: Kelly boost +0.76€ cuando `restante_min` < 3.88 (IC base=+0.145)

- **PATRÓN** `restante_min` > `4.95` → IC=+0.223 (n=283)

  - _Acción_: Kelly boost +1.00€ cuando `restante_min` > 4.95 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.151 (n=729)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 7.0 (IC base=+0.145)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.156 (n=795)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 16.0 (IC base=+0.145)

- **PATRÓN** `lag_apertura_s` < `3.24` → IC=+0.222 (n=268)

  - _Acción_: Kelly boost +1.00€ cuando `lag_apertura_s` < 3.24 (IC base=+0.145)

- **PATRÓN** `profundidad_ratio_no` > `12.4` → IC=+0.168 (n=266)

  - _Acción_: Kelly boost +0.84€ cuando `profundidad_ratio_no` > 12.4 (IC base=+0.145)
...
- **PATRÓN** `hora_utc` > `10.0` → IC=+0.277 (n=137)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1397.8324` → IC=+0.300 (n=63)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.74` → IC=+0.236 (n=593)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.74 (IC base=+0.104)
...
- **PATRÓN** `volumen_spike_ratio` > `1.6738` → IC=+0.129 (n=620)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.6738 (IC base=+0.104)

- **PATRÓN** `ibs_20min` < `0.6594` → IC=+0.123 (n=2140)

  - _Acción_: Kelly boost +0.61€ cuando `ibs_20min` < 0.6594 (IC base=+0.079)

- **PATRÓN** `volumen_regimen` < `1.3132` → IC=+0.143 (n=483)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.3132 (IC base=+0.079)

- **PATRÓN** `volumen_regimen` > `0.6951` → IC=+0.142 (n=431)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 0.6951 (IC base=+0.079)

- **PATRÓN** `volumen_pendiente_norm` > `0.0856` → IC=+0.273 (n=187)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0856 (IC base=+0.079)

- **PATRÓN** `volumen_spike_ratio` < `1.7075` → IC=+0.244 (n=166)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7075 (IC base=+0.079)

- **PATRÓN** `volumen_spike_ratio` > `2.8678` → IC=+0.224 (n=172)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8678 (IC base=+0.079)

- **PATRÓN** `ballena_activa_n` < `254.0` → IC=+0.281 (n=103)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.314 (n=127)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1349` → IC=+0.345 (n=127)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1349 (IC base=+0.284)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.288 (n=201)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.284)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.303 (n=130)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 10.0 (IC base=+0.284)

- **PATRÓN** `ibs_20min` < `0.578` → IC=+0.333 (n=190)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.578 (IC base=+0.284)

- **PATRÓN** `volumen_pendiente_norm` < `0.0707` → IC=+0.322 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0707 (IC base=+0.284)

- **PATRÓN** `volumen_pendiente_norm` > `0.2422` → IC=+0.321 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2422 (IC base=+0.284)

- **PATRÓN** `volumen_spike_ratio` < `1.8119` → IC=+0.336 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8119 (IC base=+0.284)

- **PATRÓN** `volumen_spike_ratio` > `2.298` → IC=+0.309 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.298 (IC base=+0.284)

- **PATRÓN** `libro_spread` < `0.05` → IC=+0.330 (n=133)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.05 (IC base=+0.284)

- **PATRÓN** `libro_liquidez` > `1915.4982` → IC=+0.320 (n=87)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1915.4982 (IC base=+0.284)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1903` → IC=+0.172 (n=205)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.1903 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.192 (n=167)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 12.0 (IC base=+0.147)

- **PATRÓN** `ibs_20min` < `0.4787` → IC=+0.206 (n=233)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4787 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.472` → IC=+0.253 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.472 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `1.3019` → IC=+0.168 (n=233)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.3019 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` > `0.6842` → IC=+0.162 (n=208)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.6842 (IC base=+0.147)

- **PATRÓN** `volumen_pendiente_norm` > `0.1294` → IC=+0.262 (n=61)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5138` → IC=+0.283 (n=44)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5138 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `11633.3746` → IC=+0.188 (n=78)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 11633.3746 (IC base=+0.147)

- **PATRÓN** `ballena_activa_n` < `245.0` → IC=+0.250 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 245.0 (IC base=+0.147)

### GBM_LATE_15M#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.181 (n=164)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.059` → IC=+0.302 (n=99)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.059 (IC base=+0.133)

- **PATRÓN** `volumen_pendiente_norm` > `0.4168` → IC=+0.174 (n=41)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.4168 (IC base=+0.133)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.154 (n=394)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.06 (IC base=+0.133)

- **PATRÓN** `libro_liquidez` > `1917.9762` → IC=+0.148 (n=163)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 1917.9762 (IC base=+0.133)

- **PATRÓN** `ballena_activa_n` < `13.0` → IC=+0.222 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 13.0 (IC base=+0.133)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.204` → IC=+0.129 (n=989)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 2.204 (IC base=+0.053)

- **PATRÓN** `volumen_pendiente_norm` < `0.1186` → IC=+0.122 (n=617)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_pendiente_norm` < 0.1186 (IC base=+0.053)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8672` → IC=+0.168 (n=293)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 2.8672 (IC base=+0.053)

- **PATRÓN** `ballena_activa_n` < `41.0` → IC=+0.290 (n=60)
...
- **PATRÓN** `dist_vwap_pct` > `0.5057` → IC=+0.288 (n=31)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0643` → IC=+0.192 (n=115)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.96€ cuando `drift_60min` |x|≤ 0.0643 (IC base=+0.179)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.071` → IC=+0.315 (n=106)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.071 (IC base=+0.179)

- **PATRÓN** `volumen_pendiente_norm` < `0.1388` → IC=+0.188 (n=245)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` < 0.1388 (IC base=+0.179)
...
- **PATRÓN** `volumen_spike_ratio` < `4.766` → IC=+0.163 (n=262)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 4.766 (IC base=+0.179)

- **PATRÓN** `volumen_spike_ratio` > `3.8419` → IC=+0.203 (n=119)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.8419 (IC base=+0.179)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.196 (n=370)
...
- **PATRÓN** `ballena_activa_n` < `31.0` → IC=+0.250 (n=30)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.152 (n=44)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.006 (n=688)
...
- **PATRÓN** `sigma_h` > `0.0066` → IC=+0.191 (n=567)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.78` → IC=+0.213 (n=1115)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.78 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` > `0.2198` → IC=+0.138 (n=277)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 0.2198 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.948` → IC=+0.263 (n=613)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.948 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `0.6287` → IC=+0.138 (n=649)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` > 0.6287 (IC base=+0.138)

- **PATRÓN** `volumen_pendiente_norm` > `0.113` → IC=+0.157 (n=412)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_pendiente_norm` > 0.113 (IC base=+0.138)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.144 (n=1403)

  - _Acción_: Kelly boost +0.72€ cuando `libro_spread` < 0.06 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `2674.5492` → IC=+0.170 (n=416)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 2674.5492 (IC base=+0.138)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.222 (n=1198)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.214)

- **PATRÓN** `drift_60min` |x|≤ `0.2979` → IC=+0.221 (n=1198)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2979 (IC base=+0.214)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.270 (n=602)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 14.0 (IC base=+0.214)

- **PATRÓN** `ibs_20min` < `0.3724` → IC=+0.282 (n=1198)
...
- **PATRÓN** `volumen_regimen` < `1.2604` → IC=+0.191 (n=930)
...
- **PATRÓN** `sigma_h` > `0.002` → IC=+0.187 (n=199)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.002 (IC base=+0.187)

- **PATRÓN** `drift_60min` |x|≤ `0.1928` → IC=+0.206 (n=195)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1928 (IC base=+0.187)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.247 (n=156)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.187)

- **PATRÓN** `ibs_20min` < `0.2977` → IC=+0.241 (n=222)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2977 (IC base=+0.187)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.413` → IC=+0.281 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.413 (IC base=+0.187)

- **PATRÓN** `volumen_regimen` < `0.6385` → IC=+0.250 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6385 (IC base=+0.187)

- **PATRÓN** `volumen_pendiente_norm` < `0.2596` → IC=+0.211 (n=119)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2596 (IC base=+0.187)

- **PATRÓN** `volumen_pendiente_norm` > `0.1352` → IC=+0.258 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1352 (IC base=+0.187)

- **PATRÓN** `volumen_spike_ratio` < `2.6037` → IC=+0.257 (n=101)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.6037 (IC base=+0.187)

- **PATRÓN** `libro_liquidez` > `11192.8304` → IC=+0.210 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11192.8304 (IC base=+0.187)

### GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.187 (n=97)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0076 (IC base=+0.149)

- **PATRÓN** `drift_60min` |x|≤ `0.1483` → IC=+0.153 (n=194)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.1483 (IC base=+0.149)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.804` → IC=+0.317 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.804 (IC base=+0.149)

- **PATRÓN** `volumen_pendiente_norm` < `0.2311` → IC=+0.128 (n=229)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_pendiente_norm` < 0.2311 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` < `2.0146` → IC=+0.199 (n=101)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0146 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` > `3.8997` → IC=+0.141 (n=104)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 3.8997 (IC base=+0.149)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.180 (n=217)
...
- **PATRÓN** `ibs_20min` > `0.9601` → IC=+0.160 (n=104)
  ...
  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.9601 (IC base=+0.036)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.863` → IC=+0.240 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.863 (IC base=+0.036)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.286 (n=639)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` > `0.3348` → IC=+0.173 (n=160)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.3348 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.265` → IC=+0.228 (n=1020)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.265 (IC base=+0.117)

- **PATRÓN** `volumen_pendiente_norm` > `0.1802` → IC=+0.134 (n=353)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_pendiente_norm` > 0.1802 (IC base=+0.117)

- **PATRÓN** `volumen_spike_ratio` > `1.6913` → IC=+0.121 (n=1085)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_spike_ratio` > 1.6913 (IC base=+0.117)

- **PATRÓN** `libro_liquidez` > `2716.8736` → IC=+0.150 (n=507)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 2716.8736 (IC base=+0.117)

- **PATRÓN** `ballena_activa_n` < `152.0` → IC=+0.188 (n=142)

  - _Acción_: Kelly boost +0.94€ cuando `ballena_activa_n` < 152.0 (IC base=+0.117)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.235 (n=1175)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.226)

- **PATRÓN** `sigma_h` > `0.0038` → IC=+0.232 (n=1334)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0038 (IC base=+0.226)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.255 (n=683)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.1499` → IC=+0.199 (n=898)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.1499 (IC base=+0.226)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.046` → IC=+0.260 (n=265)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.046 (IC base=+0.226)

- **PATRÓN** `volumen_regimen` < `0.6189` → IC=+0.212 (n=300)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6189 (IC base=+0.226)

- **PATRÓN** `volumen_regimen` > `1.0742` → IC=+0.220 (n=408)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0742 (IC base=+0.226)

- **PATRÓN** `volumen_pendiente_norm` > `0.1846` → IC=+0.267 (n=225)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1846 (IC base=+0.226)

- **PATRÓN** `volumen_spike_ratio` < `2.5737` → IC=+0.268 (n=568)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5737 (IC base=+0.226)
...
- **PATRÓN** `sigma_h` > `0.0033` → IC=+0.183 (n=77)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0033 (IC base=+0.177)

- **PATRÓN** `drift_60min` |x|≤ `0.193` → IC=+0.191 (n=202)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.96€ cuando `drift_60min` |x|≤ 0.193 (IC base=+0.177)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.198 (n=233)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 6.0 (IC base=+0.177)

- **PATRÓN** `ibs_20min` < `0.4191` → IC=+0.227 (n=229)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4191 (IC base=+0.177)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.22` → IC=+0.262 (n=145)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.22 (IC base=+0.177)

- **PATRÓN** `volumen_regimen` < `1.3065` → IC=+0.188 (n=229)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` < 1.3065 (IC base=+0.177)

- **PATRÓN** `volumen_regimen` > `1.1302` → IC=+0.198 (n=104)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` > 1.1302 (IC base=+0.177)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8539` → IC=+0.304 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8539 (IC base=+0.177)

- **PATRÓN** `libro_liquidez` > `4994.7986` → IC=+0.191 (n=205)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 4994.7986 (IC base=+0.177)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.317 (n=151)
...
- **PATRÓN** `volumen_spike_ratio` > `4.9893` → IC=+0.167 (n=79)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 4.9893 (IC base=+0.169)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.188 (n=334)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.06 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `1854.4986` → IC=+0.188 (n=206)
...
- **PATRÓN** `sigma_h` < `0.0033` → IC=+0.220 (n=105)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0033 (IC base=+0.145)

- **PATRÓN** `drift_60min` |x|≤ `0.2641` → IC=+0.154 (n=105)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.2641 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.192 (n=37)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 16.0 (IC base=+0.145)

- **PATRÓN** `ibs_20min` < `0.3` → IC=+0.245 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.1302` → IC=+0.260 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1302 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.578` → IC=+0.269 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.578 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` < `0.8934` → IC=+0.202 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8934 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` < `0.141` → IC=+0.336 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.141 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` < `2.2674` → IC=+0.296 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.2674 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` > `1.7447` → IC=+0.306 (n=34)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7447 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `6213.1923` → IC=+0.230 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 6213.1923 (IC base=+0.145)
...
- **PATRÓN** `dist_vwap_pct` > `0.326` → IC=+0.250 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.326 (IC base=+0.128)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.29` → IC=+0.215 (n=198)
...
- **PATRÓN** `volumen_spike_ratio` > `1.7671` → IC=+0.128 (n=224)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.7671 (IC base=+0.128)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.148 (n=305)

  - _Acción_: Kelly boost +0.74€ cuando `libro_spread` < 0.01 (IC base=+0.128)

- **PATRÓN** `libro_liquidez` > `2469.502` → IC=+0.130 (n=268)

  - _Acción_: Kelly boost +0.65€ cuando `libro_liquidez` > 2469.502 (IC base=+0.128)
...
- **FILTRO** `ibs_20min` < `0.7083` → IC=-0.167 (n=52)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.236 (n=108)
...
- **PATRÓN** `sigma_h` < `0.0054` → IC=+0.181 (n=142)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.0054 (IC base=+0.060)

- **PATRÓN** `drift_60min` |x|≤ `0.1223` → IC=+0.175 (n=38)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.1223 (IC base=+0.060)

- **PATRÓN** `ibs_20min` > `0.7083` → IC=+0.236 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7083 (IC base=+0.060)

- **PATRÓN** `dist_vwap_pct` > `0.1247` → IC=+0.151 (n=64)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.1247 (IC base=+0.060)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.379` → IC=+0.233 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.379 (IC base=+0.060)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.123 (n=75)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 7.0 (IC base=+0.098)
...
- **PATRÓN** `dist_vwap_pct` < `0.1209` → IC=+0.204 (n=42)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1209 (IC base=+0.098)
...
- **PATRÓN** `volumen_regimen` < `0.8241` → IC=+0.176 (n=35)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 0.8241 (IC base=+0.098)
...
- **PATRÓN** `libro_liquidez` > `2351.5975` → IC=+0.235 (n=32)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2351.5975 (IC base=+0.098)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.173 (n=47)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 15.0 (IC base=+0.019)
...
- **PATRÓN** `drift_20min_pct` |x|≤ `0.0531` → IC=+0.154 (n=76)

  - _Acción_: Kelly boost +0.77€ cuando `drift_20min_pct` |x|≤ 0.0531 (IC base=+0.067)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.197 (n=153)

  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.005 (n=184)

- **FILTRO** `ibs_20min` < `0.7468` → IC=-0.163 (n=84)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7468
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=253)

- **FILTRO** `ballena_activa_n` > `14.0` → IC=-0.175 (n=81)

  - _Acción_: SKIP cuando `ballena_activa_n` > 14.0
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=256)

- **PATRÓN** `py_entrada` < `0.5` → IC=+0.133 (n=156)

  - _Acción_: Kelly boost +0.66€ cuando `py_entrada` < 0.5 (IC base=+0.021)
...
- **FILTRO** `py_entrada` > `0.5` → IC=-0.167 (n=40)

  - _Acción_: SKIP cuando `py_entrada` > 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.145 (n=29)

- **FILTRO** `ibs_20min` > `0.1707` → IC=-0.180 (n=23)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.145 (n=29)
  ...
  - _Acción_: Kelly boost +0.73€ cuando `py_entrada` < 0.5 (IC base=-0.035)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.250 (n=22)

  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.081 (n=29)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.289 (n=17)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.200 (n=18)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.089)

- **PATRÓN** `drift_20min_pct` |x|≤ `0.1191` → IC=+0.153 (n=47)

  - _Acción_: Kelly boost +0.77€ cuando `drift_20min_pct` |x|≤ 0.1191 (IC base=+0.089)

- **PATRÓN** `libro_liquidez` > `11243.3198` → IC=+0.180 (n=48)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 11243.3198 (IC base=+0.089)
...
- **FILTRO** `ibs_20min` > `0.8571` → IC=-0.200 (n=28)
...
- **FILTRO** `py_entrada` < `0.37` → IC=-0.311 (n=236)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=711)

- **FILTRO** `ibs_7min` < `0.7399` → IC=-0.235 (n=236)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7399
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=711)

- **FILTRO** `ballena_activa_n` > `23.0` → IC=-0.219 (n=229)

  - _Acción_: SKIP cuando `ballena_activa_n` > 23.0
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=718)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.189 (n=326)
...
- **FILTRO** `libro_liquidez` < `9212.3177` → IC=-0.157 (n=100)
...
- **FILTRO** `py_entrada` > `0.5` → IC=-0.122 (n=109)
...
- **PATRÓN** `ibs_15` < `0.4397` → IC=+0.125 (n=350)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.62€ cuando `ibs_15` < 0.4397 (IC base=+0.077)

- **PATRÓN** `dist_vwap_pct` > `0.5616` → IC=+0.197 (n=31)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.5616 (IC base=+0.077)
...
- **PATRÓN** `sigma_h` < `0.002` → IC=+0.250 (n=26)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.002 (IC base=+0.062)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.758` → IC=+0.250 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.758 (IC base=+0.062)
...
- **PATRÓN** `sigma_h` < `0.005` → IC=+0.171 (n=86)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.005 (IC base=+0.149)

- **PATRÓN** `sigma_h` > `0.0041` → IC=+0.216 (n=65)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0041 (IC base=+0.149)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1155` → IC=+0.163 (n=87)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.81€ cuando `delta_ratio_macro` |x|> 0.1155 (IC base=+0.149)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.180 (n=48)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 14.0 (IC base=+0.149)

- **PATRÓN** `hora_utc` < `14.0` → IC=+0.153 (n=93)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 14.0 (IC base=+0.149)

- **PATRÓN** `ibs_15` < `0.3241` → IC=+0.177 (n=97)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.88€ cuando `ibs_15` < 0.3241 (IC base=+0.149)

- **PATRÓN** `ibs_15` > `0.0292` → IC=+0.197 (n=97)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.98€ cuando `ibs_15` > 0.0292 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` > `0.296` → IC=+0.250 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.296 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` < `17.598` → IC=+0.211 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 17.598 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `11058.1954` → IC=+0.157 (n=33)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 11058.1954 (IC base=+0.149)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0094` → IC=+0.143 (n=26)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `pct_spot_vs_ref` |x|≤ 0.0094 (IC base=+0.082)

- **PATRÓN** `sigma_h` < `0.0025` → IC=+0.214 (n=26)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0025 (IC base=+0.082)

- **PATRÓN** `drift_15min` |x|≤ `0.513` → IC=+0.150 (n=58)

  - _Acción_: Kelly boost +0.75€ cuando `drift_15min` |x|≤ 0.513 (IC base=+0.082)

- **PATRÓN** `hora_utc` < `2.0` → IC=+0.152 (n=21)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 2.0 (IC base=+0.082)

- **PATRÓN** `ibs_15` > `0.0472` → IC=+0.133 (n=58)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.67€ cuando `ibs_15` > 0.0472 (IC base=+0.082)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.992` → IC=+0.125 (n=54)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 6.992 (IC base=+0.082)

- **PATRÓN** `ballena_activa_n` < `3.0` → IC=+0.160 (n=45)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 3.0 (IC base=+0.082)
...
- **PATRÓN** `sigma_h` < `0.0078` → IC=+0.124 (n=123)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` < 0.0078 (IC base=+0.106)

- **PATRÓN** `drift_60min` |x|≤ `0.14` → IC=+0.198 (n=41)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.14 (IC base=+0.106)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0966` → IC=+0.170 (n=110)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.85€ cuando `delta_ratio_macro` |x|> 0.0966 (IC base=+0.106)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.146 (n=111)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 15.0 (IC base=+0.106)

- **PATRÓN** `ibs_15` < `0.2258` → IC=+0.202 (n=82)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.2258 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` > `0.1184` → IC=+0.271 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1184 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.148` → IC=+0.151 (n=104)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` < 5.148 (IC base=+0.106)

- **PATRÓN** `libro_liquidez` > `2541.6579` → IC=+0.148 (n=123)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 2541.6579 (IC base=+0.106)
...
- **FILTRO** `sigma_ewma_delta_pct` > `13.09` → IC=-0.158 (n=223)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.253 (n=79)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.167 (n=172)
...
- **FILTRO** `sigma_ewma_delta_pct` > `13.982` → IC=-0.194 (n=60)
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_NO, IBS < 0.3241 sube el IC de +0.149 a +0.177 en UPDOWN_GBM#ETH#15min (n=97). Ya aplicado como kelly_boost=+0.88€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_NO, IBS > 0.0292 sube el IC de +0.149 a +0.197 en UPDOWN_GBM#ETH#15min (n=97). Ya aplicado como kelly_boost=+0.98€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.2258 sube el IC de +0.106 a +0.202 en UPDOWN_GBM#XRP#15min (n=82). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 3329 | -0.113 | -519.21€ | 2 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 456 | -0.022 | -17.20€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 2873 | -0.128 | -502.01€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 802 | +0.000 | -93.67€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 802 | +0.000 | -93.67€ | 1 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP | 673 | -0.220 | -119.05€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP#5min | 673 | -0.220 | -119.05€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 14294 | +0.116 | -819.28€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3376 | +0.180 | -114.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 99 | -0.104 | -47.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 8227 | +0.087 | -681.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2592 | +0.132 | +24.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 1389 | +0.034 | -298.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 13 | -0.065 | -2.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 1372 | +0.037 | -290.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 3256 | +0.140 | -2.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 922 | +0.196 | -32.65€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 41 | -0.105 | -20.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 1370 | +0.110 | -14.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 923 | +0.140 | +65.31€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 1386 | +0.056 | -238.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 9 | -0.021 | -3.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 1376 | +0.057 | -233.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 3521 | +0.126 | -48.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1246 | +0.159 | -26.84€ | 0 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 1368 | +0.106 | -16.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 895 | +0.117 | +3.13€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 3358 | +0.135 | -204.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1174 | +0.196 | -50.80€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 40 | +0.000 | -9.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 1370 | +0.084 | -99.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 774 | +0.139 | -44.18€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 1384 | +0.128 | -26.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 12 | +0.043 | +1.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 1371 | +0.129 | -26.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 3449 | +0.160 | -330.30€ | 2 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 3449 | +0.160 | -330.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 865 | +0.155 | -112.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 865 | +0.155 | -112.06€ | 0 | 2 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 852 | +0.162 | -103.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 852 | +0.162 | -103.33€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 766 | +0.216 | -43.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 766 | +0.216 | -43.98€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 809 | +0.173 | -78.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 809 | +0.173 | -78.97€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 5860 | +0.190 | -540.75€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 5860 | +0.190 | -540.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 1097 | +0.099 | -245.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 1097 | +0.099 | -245.45€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 906 | +0.248 | -8.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 906 | +0.248 | -8.38€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 1032 | +0.154 | -148.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 1032 | +0.154 | -148.20€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 946 | +0.222 | -39.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 946 | +0.222 | -39.31€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 911 | +0.247 | -10.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 911 | +0.247 | -10.11€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 968 | +0.192 | -89.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 968 | +0.192 | -89.30€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 2090 | +0.150 | +118.94€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 2090 | +0.150 | +118.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 1033 | +0.155 | +68.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 1033 | +0.155 | +68.00€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 1057 | +0.145 | +50.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 1057 | +0.145 | +50.94€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 249 | +0.257 | -30.83€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 249 | +0.257 | -30.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 249 | +0.257 | -30.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 249 | +0.257 | -30.83€ | 0 | 4 |
| ✅ GBM_LATE_15M | 4502 | +0.088 | +1605.40€ | 0 | 14 |
| ✅ GBM_LATE_15M#15min | 4502 | +0.088 | +1605.40€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 782 | +0.177 | +495.75€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 782 | +0.177 | +495.75€ | 0 | 18 |
| ✅ GBM_LATE_15M#BTC | 441 | +0.182 | +225.96€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 441 | +0.182 | +225.96€ | 0 | 23 |
| ✅ GBM_LATE_15M#DOGE | 785 | +0.193 | +540.81€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 785 | +0.193 | +540.81€ | 0 | 18 |
...
| ✅ GBM_LATE_15M#XRP | 1045 | +0.021 | +228.96€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1045 | +0.021 | +228.96€ | 0 | 4 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 5520 | +0.051 | +1652.14€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 5520 | +0.051 | +1652.14€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 1059 | -0.028 | +190.00€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 1059 | -0.028 | +190.00€ | 1 | 2 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 1046 | -0.006 | +111.25€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 1046 | -0.006 | +111.25€ | 0 | 3 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 665 | +0.239 | +604.56€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 665 | +0.239 | +604.56€ | 0 | 20 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 966 | -0.015 | +20.29€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 966 | -0.015 | +20.29€ | 6 | 0 |
...
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 3261 | +0.175 | +2207.10€ | 0 | 21 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 3261 | +0.175 | +2207.10€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB | 581 | +0.193 | +423.67€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB#15min | 581 | +0.193 | +423.67€ | 0 | 16 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC | 429 | +0.196 | +285.97€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC#15min | 429 | +0.196 | +285.97€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE | 576 | +0.202 | +448.28€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min | 576 | +0.202 | +448.28€ | 0 | 22 |
...
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP | 659 | +0.194 | +505.83€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP#15min | 659 | +0.194 | +505.83€ | 0 | 25 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 534 | +0.041 | +38.06€ | 0 | 7 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 534 | +0.041 | +38.06€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 100 | +0.020 | -5.16€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 100 | +0.020 | -5.16€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 3802 | +0.168 | +2455.81€ | 0 | 19 |
| ✅ GBM_LATE_15M_TARDIO#15min | 3802 | +0.168 | +2455.81€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 752 | +0.188 | +531.46€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 752 | +0.188 | +531.46€ | 0 | 18 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 501 | +0.158 | +273.18€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 501 | +0.158 | +273.18€ | 1 | 22 |
| ✅ GBM_LATE_15M_TARDIO#DOGE | 744 | +0.217 | +612.87€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#DOGE#15min | 744 | +0.217 | +612.87€ | 0 | 16 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 341 | +0.115 | +133.46€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 341 | +0.115 | +133.46€ | 1 | 20 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 615 | +0.077 | +261.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 615 | +0.077 | +261.84€ | 1 | 15 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 849 | +0.198 | +642.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 849 | +0.198 | +642.98€ | 0 | 22 |
...
| ✅ GBM_LATE_60M | 490 | -0.047 | +67.58€ | 4 | 6 |
| ✅ GBM_LATE_60M#60min | 490 | -0.047 | +67.58€ | 0 | 0 |
| ✅ GBM_LATE_60M#BTC | 169 | -0.003 | +6.56€ | 0 | 0 |
| ✅ GBM_LATE_60M#BTC#60min | 169 | -0.003 | +6.56€ | 3 | 3 |
| ✅ GBM_LATE_60M#ETH | 173 | -0.026 | +35.52€ | 0 | 0 |
| ✅ GBM_LATE_60M#ETH#60min | 173 | -0.026 | +35.52€ | 1 | 8 |
...
| ✅ GBM_LATE_60M_PYCONFIRMADO | 304 | +0.043 | +6.51€ | 0 | 2 |
| ✅ GBM_LATE_60M_PYCONFIRMADO#60min | 304 | +0.043 | +6.51€ | 0 | 0 |
...
| ✅ LEADLAG_BTC_XRP_15M | 53 | +0.136 | +13.41€ | 0 | 1 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 53 | +0.136 | +13.41€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 53 | +0.136 | +13.41€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 53 | +0.136 | +13.41€ | 0 | 1 |
...
| ✅ MOMENTUM_IBS_15M | 501 | +0.037 | +3.22€ | 1 | 1 |
| ✅ MOMENTUM_IBS_15M#15min | 501 | +0.037 | +3.22€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB | 81 | +0.054 | +5.15€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB#15min | 81 | +0.054 | +5.15€ | 0 | 1 |
| ✅ MOMENTUM_IBS_15M#BTC | 86 | +0.045 | +5.84€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BTC#15min | 86 | +0.045 | +5.84€ | 0 | 1 |
| ✅ MOMENTUM_IBS_15M#DOGE | 78 | +0.000 | -10.06€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#DOGE#15min | 78 | +0.000 | -10.06€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH | 88 | +0.089 | +18.39€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH#15min | 88 | +0.089 | +18.39€ | 0 | 5 |
| ✅ MOMENTUM_IBS_15M#SOL | 84 | -0.023 | -14.39€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#SOL#15min | 84 | -0.023 | -14.39€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#XRP | 84 | +0.046 | -1.71€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#XRP#15min | 84 | +0.046 | -1.71€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA | 719 | -0.030 | +4.57€ | 3 | 1 |
| ✅ MOMENTUM_IBS_15M_BALLENA#15min | 719 | -0.030 | +4.57€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB | 115 | -0.038 | +16.46€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB#15min | 115 | -0.038 | +16.46€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC | 125 | -0.051 | -9.77€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC#15min | 125 | -0.051 | -9.77€ | 5 | 1 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE | 117 | -0.029 | +3.78€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE#15min | 117 | -0.029 | +3.78€ | 1 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH | 122 | +0.024 | +3.66€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH#15min | 122 | +0.024 | +3.66€ | 2 | 3 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL | 117 | -0.055 | -2.65€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL#15min | 117 | -0.055 | -2.65€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP | 123 | -0.028 | -6.90€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP#15min | 123 | -0.028 | -6.90€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE | 258 | -0.088 | -25.93€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#15min | 258 | -0.088 | -25.93€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#BNB | 35 | -0.068 | -3.26€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#BNB#15min | 35 | -0.068 | -3.26€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_15M_FADE#SOL | 53 | -0.045 | -2.98€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#SOL#15min | 53 | -0.045 | -2.98€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#XRP | 48 | -0.020 | -1.30€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#XRP#15min | 48 | -0.020 | -1.30€ | 1 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 1908 | -0.070 | +43.54€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 1908 | -0.070 | +43.54€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 286 | -0.080 | +13.56€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 286 | -0.080 | +13.56€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 358 | -0.058 | +86.27€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 358 | -0.058 | +86.27€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 299 | -0.088 | -32.53€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 299 | -0.088 | -32.53€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 310 | -0.103 | -34.51€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 310 | -0.103 | -34.51€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 349 | -0.038 | +4.18€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 349 | -0.038 | +4.18€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 306 | -0.055 | +6.57€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 306 | -0.055 | +6.57€ | 4 | 1 |
| ✅ MOMENTUM_IBS_5M_FADE | 1786 | +0.021 | +20.55€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 1786 | +0.021 | +20.55€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB | 305 | +0.018 | +7.49€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB#5min | 305 | +0.018 | +7.49€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC | 259 | +0.040 | +5.16€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC#5min | 259 | +0.040 | +5.16€ | 1 | 1 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE | 308 | +0.019 | +2.63€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE#5min | 308 | +0.019 | +2.63€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH | 307 | +0.018 | +3.01€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH#5min | 307 | +0.018 | +3.01€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL | 312 | +0.025 | +5.32€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL#5min | 312 | +0.025 | +5.32€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP | 295 | +0.008 | -3.05€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP#5min | 295 | +0.008 | -3.05€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 2171 | +0.023 | +125.77€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 948 | +0.086 | +166.67€ | 1 | 6 |
| ✅ UPDOWN_GBM#240min | 119 | +0.021 | +0.93€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 807 | -0.023 | -30.30€ | 4 | 0 |
...
| ✅ UPDOWN_GBM#ETH | 505 | +0.056 | +53.66€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 221 | +0.128 | +54.93€ | 1 | 14 |
| ✅ UPDOWN_GBM#ETH#240min | 36 | +0.053 | +0.92€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 132 | +0.007 | +0.30€ | 4 | 7 |
| ✅ UPDOWN_GBM#ETH#60min | 101 | +0.005 | -2.10€ | 0 | 1 |
| 🚫 UPDOWN_GBM#ETH#daily | 15 | -0.154 | -0.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 390 | -0.010 | -2.44€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 164 | +0.012 | +0.33€ | 1 | 2 |
...
| ✅ UPDOWN_GBM#XRP | 481 | +0.030 | +63.78€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 291 | +0.097 | +84.55€ | 0 | 15 |
| ✅ UPDOWN_GBM#XRP#240min | 17 | -0.112 | -2.90€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 173 | -0.066 | -17.87€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1652 | -0.048 | +173.29€ | 2 | 3 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1652 | -0.048 | +173.29€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 112 | -0.070 | -2.73€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 112 | -0.070 | -2.73€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 326 | -0.140 | -15.81€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 326 | -0.140 | -15.81€ | 3 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 528 | -0.013 | +105.50€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 528 | -0.013 | +105.50€ | 1 | 1 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 512 | -0.035 | +66.57€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 512 | -0.035 | +66.57€ | 4 | 0 |
...
| ✅ UPDOWN_OU_5M | 345 | -0.062 | -27.24€ | 5 | 0 |
| ✅ UPDOWN_OU_5M#5min | 345 | -0.062 | -27.24€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#BNB | 222 | -0.004 | -10.55€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#BNB#5min | 222 | -0.004 | -10.55€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.142) — sin ventaja clara. oversold(IBS<0.3): IC=-0.020 n=759 | neutral: IC=+0.007 n=730 | overbought(IBS>0.7): IC=+0.121 n=986
  - _Datos_: n=2654 IC=+0.043 PNL=+210.91€
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=164 IC=+0.012 PNL=+0.33€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.105 n=38232 | tras_1loss IC=+0.062 n=26859 | tras_2loss IC=+0.022 n=11719/40 | gap=+0.084 (umbral 0.05)
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=+0.081 n=165 | contrario_BTC IC=-0.037 n=93/40 | gap=-0.118 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2000 IC=+0.018 PNL=+94.27€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2000 IC=+0.018 PNL=+94.27€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=591 IC=+0.078 PNL=+72.21€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=591 IC=+0.078 PNL=+72.21€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=82 IC=+0.012 PNL=+5.46€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=82 IC=+0.012 PNL=+5.46€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=448 IC=+0.093 PNL=+109.64€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=448 IC=+0.093 PNL=+109.64€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=967 IC=+0.021 PNL=+23.96€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=967 IC=+0.021 PNL=+23.96€
...
**🟡 H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.106 > 0.1 con n=163 PNL=+50.53€
  - _Datos_: n=163 IC=+0.106 PNL=+50.53€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1300 IC=-0.147 PNL=+62.32€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1300 IC=-0.147 PNL=+62.32€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=310 IC=+0.147 PNL=+107.29€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=310 IC=+0.147 PNL=+107.29€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.209 > 0.08 con n=273 PNL=+158.37€
  - _Datos_: n=273 IC=+0.209 PNL=+158.37€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.247 > 0.08 con n=643 PNL=-68.92€
  - _Datos_: n=643 IC=+0.247 PNL=-68.92€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=1097 IC=+0.099 PNL=-245.45€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=1097 IC=+0.099 PNL=-245.45€
