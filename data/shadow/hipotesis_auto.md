# Hipótesis automáticas — 2026-08-20 15:48 UTC
_Generado por shadow_postmortem.py sobre 90735 resoluciones (PNL=+8233.70€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `153.65` → IC=-0.259 (n=1221)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 153.65
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=3664)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `n_ballenas` < `4.0` → IC=-0.187 (n=314)
...
- **PATRÓN** `py_entrada` < `0.335` → IC=+0.271 (n=1756)
...
- **PATRÓN** `libro_liquidez` > `4174.357` → IC=+0.177 (n=911)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 4174.357 (IC base=+0.139)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.211 (n=420)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.198)

- **PATRÓN** `py_entrada` > `0.785` → IC=+0.367 (n=141)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.785 (IC base=+0.198)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.203 (n=523)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.149 (n=425)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 5.0 (IC base=+0.133)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.157 (n=371)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 15.0 (IC base=+0.133)

- **PATRÓN** `py_entrada` > `0.555` → IC=+0.147 (n=449)

  - _Acción_: Kelly boost +0.74€ cuando `py_entrada` > 0.555 (IC base=+0.133)
...
- **PATRÓN** `libro_liquidez` > `2117.8922` → IC=+0.170 (n=283)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 2117.8922 (IC base=+0.146)
...
- **PATRÓN** `py_entrada` > `0.83` → IC=+0.407 (n=214)
...
- **PATRÓN** `libro_liquidez` > `748.7377` → IC=+0.239 (n=396)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 748.7377 (IC base=+0.216)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.160 (n=787)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 5.0 (IC base=+0.155)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.160 (n=687)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` < `0.72` → IC=+0.202 (n=585)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.216 (n=4518)
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.295 (n=480)
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.195 (n=427)

  - _Acción_: Kelly boost +0.97€ cuando `py_entrada` > 0.76 (IC base=+0.148)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.241 (n=993)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.295 (n=364)
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.263 (n=491)
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.226 (n=421)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.76 (IC base=+0.189)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.208 (n=776)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.136)

- **PATRÓN** `restante_min` < `3.87` → IC=+0.152 (n=736)

  - _Acción_: Kelly boost +0.76€ cuando `restante_min` < 3.87 (IC base=+0.136)

- **PATRÓN** `restante_min` > `4.91` → IC=+0.179 (n=775)

  - _Acción_: Kelly boost +0.90€ cuando `restante_min` > 4.91 (IC base=+0.136)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.146 (n=2248)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 17.0 (IC base=+0.136)

- **PATRÓN** `lag_apertura_s` < `5.33` → IC=+0.182 (n=733)

  - _Acción_: Kelly boost +0.91€ cuando `lag_apertura_s` < 5.33 (IC base=+0.136)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.225 (n=383)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.148)

- **PATRÓN** `restante_min` < `3.79` → IC=+0.162 (n=365)

  - _Acción_: Kelly boost +0.81€ cuando `restante_min` < 3.79 (IC base=+0.148)

- **PATRÓN** `restante_min` > `4.88` → IC=+0.188 (n=402)

  - _Acción_: Kelly boost +0.94€ cuando `restante_min` > 4.88 (IC base=+0.148)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.149 (n=1139)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 5.0 (IC base=+0.148)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.161 (n=988)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 15.0 (IC base=+0.148)

- **PATRÓN** `lag_apertura_s` < `6.9` → IC=+0.190 (n=363)

  - _Acción_: Kelly boost +0.95€ cuando `lag_apertura_s` < 6.9 (IC base=+0.148)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.191 (n=393)

  - _Acción_: Kelly boost +0.96€ cuando `py_entrada` < 0.37 (IC base=+0.124)

- **PATRÓN** `restante_min` < `3.94` → IC=+0.139 (n=372)

  - _Acción_: Kelly boost +0.70€ cuando `restante_min` < 3.94 (IC base=+0.124)

- **PATRÓN** `restante_min` > `4.95` → IC=+0.177 (n=385)

  - _Acción_: Kelly boost +0.89€ cuando `restante_min` > 4.95 (IC base=+0.124)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.136 (n=1137)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` < 17.0 (IC base=+0.124)

- **PATRÓN** `lag_apertura_s` < `3.26` → IC=+0.183 (n=373)

  - _Acción_: Kelly boost +0.91€ cuando `lag_apertura_s` < 3.26 (IC base=+0.124)

### FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.316 (n=401)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.302)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.310 (n=408)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.296 (n=170)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.277)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.281 (n=176)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.315 (n=219)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.303)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.316 (n=215)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `1852.2015` → IC=+0.314 (n=208)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9714` → IC=+0.273 (n=486)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9714 (IC base=+0.095)

- **PATRÓN** `dist_vwap_pct` > `0.112` → IC=+0.262 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.112 (IC base=+0.095)

- **PATRÓN** `dist_vwap_pct` < `0.6914` → IC=+0.233 (n=238)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6914 (IC base=+0.095)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.835` → IC=+0.227 (n=662)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.835 (IC base=+0.095)

- **PATRÓN** `volumen_regimen` < `0.7215` → IC=+0.235 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7215 (IC base=+0.095)

- **PATRÓN** `volumen_regimen` > `0.646` → IC=+0.236 (n=218)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.646 (IC base=+0.095)

- **PATRÓN** `volumen_pendiente_norm` > `0.3218` → IC=+0.150 (n=115)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_pendiente_norm` > 0.3218 (IC base=+0.095)

- **PATRÓN** `volumen_spike_ratio` > `1.5121` → IC=+0.128 (n=818)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.5121 (IC base=+0.095)

- **PATRÓN** `ibs_20min` < `0.3919` → IC=+0.144 (n=1629)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` < 0.3919 (IC base=+0.059)
...
- **PATRÓN** `sigma_h` < `0.0019` → IC=+0.336 (n=53)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0019 (IC base=+0.252)

- **PATRÓN** `drift_60min` |x|≤ `0.3288` → IC=+0.263 (n=154)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3288 (IC base=+0.252)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.300 (n=153)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.252)

- **PATRÓN** `ibs_20min` > `0.5283` → IC=+0.274 (n=153)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5283 (IC base=+0.252)

- **PATRÓN** `dist_vwap_pct` > `0.3432` → IC=+0.312 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3432 (IC base=+0.252)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.619` → IC=+0.314 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.619 (IC base=+0.252)

- **PATRÓN** `volumen_regimen` < `0.6705` → IC=+0.278 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6705 (IC base=+0.252)

- **PATRÓN** `volumen_regimen` > `0.7505` → IC=+0.255 (n=137)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7505 (IC base=+0.252)

- **PATRÓN** `volumen_pendiente_norm` < `0.1141` → IC=+0.287 (n=120)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1141 (IC base=+0.252)

- **PATRÓN** `volumen_pendiente_norm` > `0.3178` → IC=+0.300 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3178 (IC base=+0.252)

- **PATRÓN** `volumen_spike_ratio` < `1.4256` → IC=+0.304 (n=44)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4256 (IC base=+0.252)

- **PATRÓN** `volumen_spike_ratio` > `1.7306` → IC=+0.261 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7306 (IC base=+0.252)

- **PATRÓN** `libro_liquidez` > `13582.0229` → IC=+0.278 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13582.0229 (IC base=+0.252)

- **PATRÓN** `ballena_activa_n` < `431.0` → IC=+0.300 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 431.0 (IC base=+0.252)
...
- **FILTRO** `ibs_20min` > `0.8786` → IC=-0.204 (n=133)
...
- **PATRÓN** `ibs_20min` > `0.772` → IC=+0.147 (n=66)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` > 0.772 (IC base=+0.023)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.253` → IC=+0.152 (n=44)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 10.253 (IC base=+0.023)

### GBM_LATE_15M#SOL#15min
- **FILTRO** `sigma_ewma_delta_pct` > `4.604` → IC=-0.133 (n=208)
...
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.157 (n=621)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` > 0.007 (IC base=+0.051)

- **PATRÓN** `ibs_20min` > `0.9425` → IC=+0.250 (n=617)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9425 (IC base=+0.051)

- **PATRÓN** `dist_vwap_pct` > `0.4318` → IC=+0.246 (n=136)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4318 (IC base=+0.051)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.298` → IC=+0.127 (n=1117)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 2.298 (IC base=+0.051)

- **PATRÓN** `volumen_regimen` > `1.1704` → IC=+0.150 (n=175)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 1.1704 (IC base=+0.051)
...
- **PATRÓN** `volumen_spike_ratio` > `2.7899` → IC=+0.173 (n=350)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.7899 (IC base=+0.051)

- **PATRÓN** `ballena_activa_n` < `86.0` → IC=+0.273 (n=214)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 86.0 (IC base=+0.051)

- **PATRÓN** `ibs_20min` < `0.1047` → IC=+0.166 (n=930)

  - _Acción_: Kelly boost +0.83€ cuando `ibs_20min` < 0.1047 (IC base=+0.043)
...
- **FILTRO** `libro_liquidez` < `8791.013` → IC=-0.181 (n=70)
...
- **FILTRO** `ibs_20min` < `0.4444` → IC=-0.155 (n=172)
...
- **PATRÓN** `ibs_20min` > `0.6` → IC=+0.192 (n=118)
  ...
  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` > 0.6 (IC base=-0.007)
...
- **PATRÓN** `dist_vwap_pct` > `0.4988` → IC=+0.297 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4988 (IC base=+0.170)
...
- **PATRÓN** `volumen_regimen` > `0.8259` → IC=+0.206 (n=229)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8259 (IC base=+0.170)

- **PATRÓN** `volumen_pendiente_norm` > `0.3097` → IC=+0.261 (n=44)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3097 (IC base=+0.170)

- **PATRÓN** `volumen_spike_ratio` < `1.6893` → IC=+0.198 (n=127)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 1.6893 (IC base=+0.170)

- **PATRÓN** `volumen_spike_ratio` > `2.5676` → IC=+0.169 (n=131)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.5676 (IC base=+0.170)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.184 (n=353)

  - _Acción_: Kelly boost +0.92€ cuando `libro_spread` < 0.01 (IC base=+0.170)

- **PATRÓN** `libro_liquidez` > `2503.3208` → IC=+0.193 (n=307)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 2503.3208 (IC base=+0.170)

- **PATRÓN** `ballena_activa_n` < `57.0` → IC=+0.381 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 57.0 (IC base=+0.170)
...
- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.209 (n=693)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.9123` → IC=+0.261 (n=1017)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9123 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.5914` → IC=+0.241 (n=230)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5914 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.016` → IC=+0.267 (n=723)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.016 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` > `0.7012` → IC=+0.164 (n=784)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` > 0.7012 (IC base=+0.150)

- **PATRÓN** `volumen_pendiente_norm` > `0.107` → IC=+0.177 (n=530)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.107 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` < `3.043` → IC=+0.135 (n=1287)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 3.043 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.5005` → IC=+0.136 (n=1287)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` > 1.5005 (IC base=+0.150)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.161 (n=1542)

  - _Acción_: Kelly boost +0.81€ cuando `libro_spread` < 0.04 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `3098.356` → IC=+0.199 (n=509)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 3098.356 (IC base=+0.150)

- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.217 (n=1413)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0067 (IC base=+0.199)

- **PATRÓN** `drift_60min` |x|≤ `0.3005` → IC=+0.207 (n=1411)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3005 (IC base=+0.199)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.248 (n=713)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 14.0 (IC base=+0.199)

- **PATRÓN** `ibs_20min` < `0.3878` → IC=+0.264 (n=1411)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3878 (IC base=+0.199)

- **PATRÓN** `dist_vwap_pct` < `0.2576` → IC=+0.187 (n=1208)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` < 0.2576 (IC base=+0.199)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.607` → IC=+0.220 (n=284)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.607 (IC base=+0.199)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.268` → IC=+0.199 (n=1417)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.268 (IC base=+0.199)

- **PATRÓN** `volumen_regimen` < `1.2288` → IC=+0.173 (n=1111)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 1.2288 (IC base=+0.199)

- **PATRÓN** `volumen_regimen` > `0.856` → IC=+0.185 (n=740)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` > 0.856 (IC base=+0.199)
...
- **PATRÓN** `volumen_spike_ratio` < `1.9841` → IC=+0.197 (n=539)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 1.9841 (IC base=+0.199)
...
- **PATRÓN** `ballena_activa_n` < `173.0` → IC=+0.178 (n=371)

  - _Acción_: Kelly boost +0.89€ cuando `ballena_activa_n` < 173.0 (IC base=+0.199)
...
- **PATRÓN** `sigma_h` > `0.0041` → IC=+0.257 (n=72)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0041 (IC base=+0.214)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.281 (n=144)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.214)

- **PATRÓN** `ibs_20min` > `0.5545` → IC=+0.283 (n=159)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5545 (IC base=+0.214)

- **PATRÓN** `dist_vwap_pct` > `0.1685` → IC=+0.296 (n=106)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1685 (IC base=+0.214)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.435` → IC=+0.303 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.435 (IC base=+0.214)

- **PATRÓN** `volumen_regimen` > `0.9205` → IC=+0.259 (n=106)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.9205 (IC base=+0.214)

- **PATRÓN** `volumen_pendiente_norm` < `0.1173` → IC=+0.215 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1173 (IC base=+0.214)

- **PATRÓN** `volumen_pendiente_norm` > `0.3178` → IC=+0.326 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3178 (IC base=+0.214)

- **PATRÓN** `volumen_spike_ratio` < `1.4539` → IC=+0.283 (n=44)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4539 (IC base=+0.214)

- **PATRÓN** `volumen_spike_ratio` > `1.9012` → IC=+0.239 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9012 (IC base=+0.214)

- **PATRÓN** `libro_liquidez` > `7219.8643` → IC=+0.243 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7219.8643 (IC base=+0.214)
...
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.253 (n=148)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0067 (IC base=+0.237)

- **PATRÓN** `drift_60min` |x|≤ `0.1163` → IC=+0.321 (n=65)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1163 (IC base=+0.237)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.261 (n=136)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.237)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.246 (n=140)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.237)

- **PATRÓN** `ibs_20min` > `0.5341` → IC=+0.305 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5341 (IC base=+0.237)

- **PATRÓN** `dist_vwap_pct` > `0.1612` → IC=+0.262 (n=103)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1612 (IC base=+0.237)

- **PATRÓN** `dist_vwap_pct` < `0.3064` → IC=+0.246 (n=116)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3064 (IC base=+0.237)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.307` → IC=+0.386 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.307 (IC base=+0.237)

- **PATRÓN** `volumen_regimen` > `0.7127` → IC=+0.261 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7127 (IC base=+0.237)

- **PATRÓN** `volumen_pendiente_norm` > `0.1047` → IC=+0.303 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1047 (IC base=+0.237)

- **PATRÓN** `volumen_spike_ratio` > `2.3505` → IC=+0.269 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3505 (IC base=+0.237)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.241 (n=160)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.237)

- **PATRÓN** `libro_liquidez` > `2706.764` → IC=+0.238 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2706.764 (IC base=+0.237)

- **PATRÓN** `ballena_activa_n` < `233.0` → IC=+0.218 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 233.0 (IC base=+0.237)
...
- **FILTRO** `ibs_20min` > `0.5` → IC=-0.242 (n=91)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.209 (n=280)

- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.172 (n=123)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0065 (IC base=+0.065)

- **PATRÓN** `ibs_20min` > `0.8667` → IC=+0.203 (n=180)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8667 (IC base=+0.065)

- **PATRÓN** `dist_vwap_pct` > `0.5786` → IC=+0.167 (n=61)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.5786 (IC base=+0.065)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.386` → IC=+0.214 (n=82)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.386 (IC base=+0.065)

- **PATRÓN** `libro_liquidez` > `2650.5037` → IC=+0.132 (n=123)

  - _Acción_: Kelly boost +0.66€ cuando `libro_liquidez` > 2650.5037 (IC base=+0.065)

- **PATRÓN** `ballena_activa_n` < `32.0` → IC=+0.192 (n=24)

  - _Acción_: Kelly boost +0.96€ cuando `ballena_activa_n` < 32.0 (IC base=+0.065)

- **PATRÓN** `sigma_h` < `0.0046` → IC=+0.226 (n=93)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0046 (IC base=+0.098)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.213 (n=127)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 14.0 (IC base=+0.098)

- **PATRÓN** `ibs_20min` < `0.5` → IC=+0.209 (n=280)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5 (IC base=+0.098)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.974` → IC=+0.183 (n=39)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 6.974 (IC base=+0.098)

- **PATRÓN** `volumen_regimen` > `0.8536` → IC=+0.138 (n=186)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` > 0.8536 (IC base=+0.098)

- **PATRÓN** `volumen_pendiente_norm` > `0.0979` → IC=+0.158 (n=71)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_pendiente_norm` > 0.0979 (IC base=+0.098)

- **PATRÓN** `volumen_spike_ratio` > `2.4742` → IC=+0.173 (n=50)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.4742 (IC base=+0.098)

- **PATRÓN** `libro_liquidez` > `2361.288` → IC=+0.167 (n=127)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 2361.288 (IC base=+0.098)

### GBM_LATE_15M_MULTIHORIZONTE#XRP#15min
- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.267 (n=101)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0086 (IC base=+0.147)

- **PATRÓN** `drift_60min` |x|≤ `0.0938` → IC=+0.170 (n=101)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.0938 (IC base=+0.147)
...
- **PATRÓN** `dist_vwap_pct` > `0.4122` → IC=+0.202 (n=102)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4122 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.39` → IC=+0.240 (n=194)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.39 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `1.2221` → IC=+0.156 (n=303)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.2221 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` > `0.8345` → IC=+0.162 (n=202)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.8345 (IC base=+0.147)

- **PATRÓN** `volumen_pendiente_norm` > `0.1883` → IC=+0.243 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1883 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` < `2.0007` → IC=+0.178 (n=178)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 2.0007 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` > `1.4889` → IC=+0.147 (n=267)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.4889 (IC base=+0.147)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.146 (n=312)

  - _Acción_: Kelly boost +0.73€ cuando `libro_spread` < 0.01 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `2507.6219` → IC=+0.159 (n=271)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 2507.6219 (IC base=+0.147)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.223 (n=586)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0071 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.158 (n=860)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9625` → IC=+0.286 (n=794)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9625 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` > `0.5419` → IC=+0.250 (n=214)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5419 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.984` → IC=+0.261 (n=819)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.984 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` < `0.7149` → IC=+0.137 (n=453)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` < 0.7149 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` > `1.2623` → IC=+0.135 (n=343)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` > 1.2623 (IC base=+0.133)

- **PATRÓN** `volumen_pendiente_norm` > `0.175` → IC=+0.161 (n=429)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` > 0.175 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` < `3.0052` → IC=+0.130 (n=1443)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` < 3.0052 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` > `1.6592` → IC=+0.134 (n=1288)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 1.6592 (IC base=+0.133)

- **PATRÓN** `libro_spread` < `0.05` → IC=+0.141 (n=1793)

  - _Acción_: Kelly boost +0.71€ cuando `libro_spread` < 0.05 (IC base=+0.133)

- **PATRÓN** `libro_liquidez` > `3024.8615` → IC=+0.189 (n=584)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 3024.8615 (IC base=+0.133)

- **PATRÓN** `sigma_h` < `0.0074` → IC=+0.224 (n=1642)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0074 (IC base=+0.199)

- **PATRÓN** `drift_60min` |x|≤ `0.3194` → IC=+0.207 (n=1639)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3194 (IC base=+0.199)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.226 (n=593)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.199)

- **PATRÓN** `ibs_20min` < `0.5301` → IC=+0.267 (n=1639)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5301 (IC base=+0.199)

- **PATRÓN** `dist_vwap_pct` < `0.4343` → IC=+0.183 (n=1249)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.4343 (IC base=+0.199)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.504` → IC=+0.204 (n=1537)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.504 (IC base=+0.199)

- **PATRÓN** `volumen_regimen` < `0.6078` → IC=+0.186 (n=387)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` < 0.6078 (IC base=+0.199)

- **PATRÓN** `volumen_regimen` > `1.2185` → IC=+0.180 (n=386)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` > 1.2185 (IC base=+0.199)

- **PATRÓN** `volumen_pendiente_norm` > `0.2425` → IC=+0.242 (n=211)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2425 (IC base=+0.199)

- **PATRÓN** `volumen_spike_ratio` < `1.9367` → IC=+0.204 (n=633)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9367 (IC base=+0.199)

- **PATRÓN** `volumen_spike_ratio` > `2.8317` → IC=+0.218 (n=317)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8317 (IC base=+0.199)
...
- **PATRÓN** `sigma_h` > `0.0047` → IC=+0.232 (n=69)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0047 (IC base=+0.163)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.198 (n=213)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 5.0 (IC base=+0.163)

- **PATRÓN** `ibs_20min` > `0.3404` → IC=+0.260 (n=206)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3404 (IC base=+0.163)

- **PATRÓN** `dist_vwap_pct` > `0.2862` → IC=+0.307 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2862 (IC base=+0.163)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.184` → IC=+0.267 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.184 (IC base=+0.163)

- **PATRÓN** `volumen_regimen` < `0.6834` → IC=+0.190 (n=69)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_regimen` < 0.6834 (IC base=+0.163)

- **PATRÓN** `volumen_regimen` > `1.4432` → IC=+0.176 (n=69)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 1.4432 (IC base=+0.163)

- **PATRÓN** `volumen_pendiente_norm` < `0.1049` → IC=+0.222 (n=149)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1049 (IC base=+0.163)

- **PATRÓN** `volumen_spike_ratio` < `2.4203` → IC=+0.254 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.4203 (IC base=+0.163)

- **PATRÓN** `volumen_spike_ratio` > `1.3899` → IC=+0.208 (n=159)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.3899 (IC base=+0.163)

- **PATRÓN** `libro_liquidez` > `8789.4973` → IC=+0.263 (n=137)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8789.4973 (IC base=+0.163)
...
- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.180 (n=95)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0046 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.0613` → IC=+0.167 (n=70)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.0613 (IC base=+0.141)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.167 (n=193)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 8.0 (IC base=+0.141)

- **PATRÓN** `ibs_20min` > `0.3787` → IC=+0.231 (n=210)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3787 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` > `0.5076` → IC=+0.245 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5076 (IC base=+0.141)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.513` → IC=+0.193 (n=125)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 3.513 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` < `0.7651` → IC=+0.226 (n=93)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7651 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` > `1.1314` → IC=+0.149 (n=95)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 1.1314 (IC base=+0.141)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4384` → IC=+0.239 (n=178)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4384 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `3639.2085` → IC=+0.239 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3639.2085 (IC base=+0.141)

- **PATRÓN** `ballena_activa_n` < `231.0` → IC=+0.265 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 231.0 (IC base=+0.141)

- **PATRÓN** `sigma_h` < `0.0023` → IC=+0.266 (n=75)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0023 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.3476` → IC=+0.151 (n=170)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.3476 (IC base=+0.136)
...
- **PATRÓN** `ibs_20min` < `0.2192` → IC=+0.250 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2192 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.1302` → IC=+0.143 (n=82)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.1302 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `0.3499` → IC=+0.142 (n=177)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.3499 (IC base=+0.136)
...
- **PATRÓN** `volumen_regimen` < `0.5365` → IC=+0.195 (n=57)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` < 0.5365 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` < `0.0855` → IC=+0.198 (n=117)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` < 0.0855 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.2263` → IC=+0.237 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2263 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` < `2.1429` → IC=+0.202 (n=102)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1429 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.797` → IC=+0.222 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.797 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `10039.916` → IC=+0.144 (n=57)

  - _Acción_: Kelly boost +0.72€ cuando `libro_liquidez` > 10039.916 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `122.0` → IC=+0.235 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 122.0 (IC base=+0.136)

### GBM_LATE_15M_TARDIO#SOL#15min
- **FILTRO** `ibs_20min` > `0.6` → IC=-0.177 (n=94)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.200 (n=285)

- **PATRÓN** `ibs_20min` > `0.9412` → IC=+0.235 (n=119)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9412 (IC base=+0.039)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.935` → IC=+0.253 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.935 (IC base=+0.039)

- **PATRÓN** `libro_liquidez` > `2530.9904` → IC=+0.185 (n=87)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 2530.9904 (IC base=+0.039)
...
- **PATRÓN** `sigma_h` < `0.0069` → IC=+0.160 (n=251)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0069 (IC base=+0.106)

- **PATRÓN** `drift_60min` |x|≤ `0.199` → IC=+0.141 (n=190)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.70€ cuando `drift_60min` |x|≤ 0.199 (IC base=+0.106)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.184 (n=131)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 15.0 (IC base=+0.106)

- **PATRÓN** `ibs_20min` < `0.6` → IC=+0.200 (n=285)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` < `0.7224` → IC=+0.127 (n=301)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.7224 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.307` → IC=+0.167 (n=37)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 7.307 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.391` → IC=+0.121 (n=296)

  - _Acción_: Kelly boost +0.60€ cuando `sigma_ewma_delta_pct` < 4.391 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` < `0.6176` → IC=+0.149 (n=95)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 0.6176 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` > `1.0706` → IC=+0.141 (n=129)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 1.0706 (IC base=+0.106)

- **PATRÓN** `volumen_pendiente_norm` < `0.149` → IC=+0.176 (n=146)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` < 0.149 (IC base=+0.106)

- **PATRÓN** `volumen_pendiente_norm` > `0.0719` → IC=+0.177 (n=60)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.0719 (IC base=+0.106)

- **PATRÓN** `volumen_spike_ratio` > `1.5635` → IC=+0.191 (n=124)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` > 1.5635 (IC base=+0.106)

- **PATRÓN** `libro_liquidez` > `1178.5077` → IC=+0.146 (n=190)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 1178.5077 (IC base=+0.106)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.242 (n=118)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0078 (IC base=+0.149)

- **PATRÓN** `drift_60min` |x|≤ `0.0796` → IC=+0.203 (n=119)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0796 (IC base=+0.149)
...
- **PATRÓN** `ibs_20min` > `0.8507` → IC=+0.248 (n=236)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8507 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` > `0.2198` → IC=+0.246 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2198 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.293` → IC=+0.226 (n=228)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.293 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` < `0.5997` → IC=+0.150 (n=118)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 0.5997 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` > `0.6753` → IC=+0.157 (n=316)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 0.6753 (IC base=+0.149)

- **PATRÓN** `volumen_pendiente_norm` > `0.3162` → IC=+0.250 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3162 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` < `2.6244` → IC=+0.144 (n=268)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 2.6244 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` > `1.675` → IC=+0.158 (n=273)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.675 (IC base=+0.149)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.163 (n=363)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `2664.0574` → IC=+0.160 (n=236)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 2664.0574 (IC base=+0.149)

- **PATRÓN** `ballena_activa_n` < `42.0` → IC=+0.243 (n=72)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3502` → IC=+0.228 (n=421)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3502 (IC base=+0.214)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.271 (n=208)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 14.0 (IC base=+0.214)

- **PATRÓN** `ibs_20min` < `0.4611` → IC=+0.287 (n=421)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4611 (IC base=+0.214)

- **PATRÓN** `dist_vwap_pct` < `0.1176` → IC=+0.232 (n=378)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1176 (IC base=+0.214)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.597` → IC=+0.282 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.597 (IC base=+0.214)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.787` → IC=+0.221 (n=431)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.787 (IC base=+0.214)

- **PATRÓN** `volumen_regimen` > `0.883` → IC=+0.235 (n=281)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.883 (IC base=+0.214)

- **PATRÓN** `volumen_pendiente_norm` > `0.2442` → IC=+0.293 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2442 (IC base=+0.214)
...
- **PATRÓN** `sigma_h` > `0.0051` → IC=+0.128 (n=92)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.64€ cuando `sigma_h` > 0.0051 (IC base=+0.058)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.183 (n=39)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 16.0 (IC base=+0.058)

- **PATRÓN** `dist_vwap_pct` > `0.257` → IC=+0.187 (n=65)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.257 (IC base=+0.058)
...
- **PATRÓN** `libro_liquidez` > `19901.2449` → IC=+0.184 (n=36)
  ...
  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 19901.2449 (IC base=+0.028)
...
- **PATRÓN** `hora_utc` < `3.0` → IC=+0.174 (n=41)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` < 3.0 (IC base=+0.026)

- **PATRÓN** `drift_20min_pct` |x|≤ `0.0658` → IC=+0.160 (n=51)

  - _Acción_: Kelly boost +0.80€ cuando `drift_20min_pct` |x|≤ 0.0658 (IC base=+0.026)

### MOMENTUM_IBS_15M_BALLENA
- **FILTRO** `py_entrada` < `0.42` → IC=-0.201 (n=232)

  - _Acción_: SKIP cuando `py_entrada` < 0.42
  - _Potencial_: sin este filtro IC_bueno=+0.044 (n=729)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.218 (n=253)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.016 (n=789)

- **FILTRO** `ibs_20min` > `0.2687` → IC=-0.199 (n=260)
...
- **FILTRO** `ibs_20min` < `0.7588` → IC=-0.202 (n=45)
...
- **FILTRO** `drift_20min_pct` |x|> `0.2921` → IC=-0.214 (n=40)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.169 (n=548)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=2042)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.266 (n=642)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.007 (n=1948)

- **FILTRO** `ibs_7min` < `0.7347` → IC=-0.202 (n=646)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7347
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=1944)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.177 (n=866)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=1724)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.207 (n=740)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=2437)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.3` → IC=-0.274 (n=91)

  - _Acción_: SKIP cuando `py_entrada` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=285)

- **FILTRO** `ibs_7min` < `0.9405` → IC=-0.168 (n=248)

  - _Acción_: SKIP cuando `ibs_7min` < 0.9405
  - _Potencial_: sin este filtro IC_bueno=+0.008 (n=128)

- **FILTRO** `ballena_activa_n` > `4.0` → IC=-0.176 (n=183)

  - _Acción_: SKIP cuando `ballena_activa_n` > 4.0
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=193)

- **FILTRO** `hora_utc` > `14.0` → IC=-0.127 (n=148)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 14.0
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=327)

- **FILTRO** `py_entrada` > `0.64` → IC=-0.185 (n=230)

  - _Acción_: SKIP cuando `py_entrada` > 0.64
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=245)

- **FILTRO** `drift_7min_pct` |x|> `0.1194` → IC=-0.175 (n=161)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1194
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=314)

- **FILTRO** `ibs_7min` > `0.8302` → IC=-0.183 (n=118)
...
- **FILTRO** `ibs_7min` > `0.8592` → IC=-0.142 (n=132)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8592
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=400)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.218 (n=115)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=360)

- **FILTRO** `ibs_7min` < `0.837` → IC=-0.158 (n=118)

  - _Acción_: SKIP cuando `ibs_7min` < 0.837
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=357)

- **FILTRO** `ballena_activa_n` > `31.0` → IC=-0.167 (n=115)

  - _Acción_: SKIP cuando `ballena_activa_n` > 31.0
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=360)

- **FILTRO** `py_entrada` > `0.56` → IC=-0.174 (n=127)

  - _Acción_: SKIP cuando `py_entrada` > 0.56
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=382)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.185 (n=125)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=384)

- **FILTRO** `libro_liquidez` < `9359.2765` → IC=-0.120 (n=335)

  - _Acción_: SKIP cuando `libro_liquidez` < 9359.2765
...
- **FILTRO** `ibs_7min` < `0.7727` → IC=-0.186 (n=119)
...
- **FILTRO** `py_entrada` > `0.52` → IC=-0.146 (n=190)

  - _Acción_: SKIP cuando `py_entrada` > 0.52
  - _Potencial_: sin este filtro IC_bueno=+0.031 (n=373)
...
- **FILTRO** `py_entrada` < `0.41` → IC=-0.260 (n=123)

  - _Acción_: SKIP cuando `py_entrada` < 0.41
  - _Potencial_: sin este filtro IC_bueno=+0.064 (n=255)

- **FILTRO** `ibs_7min` < `0.7348` → IC=-0.302 (n=94)
...
- **PATRÓN** `hora_utc` > `10.0` → IC=+0.129 (n=60)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 10.0 (IC base=+0.043)

- **PATRÓN** `py_entrada` < `0.495` → IC=+0.167 (n=28)

  - _Acción_: Kelly boost +0.83€ cuando `py_entrada` < 0.495 (IC base=+0.043)
...
- **PATRÓN** `ibs_15` < `0.0835` → IC=+0.153 (n=223)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.77€ cuando `ibs_15` < 0.0835 (IC base=+0.042)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `17.236` → IC=+0.154 (n=24)
  ...
  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 17.236 (IC base=+0.031)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.152 (n=110)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.03 (IC base=+0.031)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1006` → IC=+0.208 (n=22)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +1.00€ cuando `pct_spot_vs_ref` |x|≤ 0.1006 (IC base=+0.017)

- **PATRÓN** `ibs_15` > `0.9253` → IC=+0.192 (n=24)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.96€ cuando `ibs_15` > 0.9253 (IC base=+0.017)
...
- **PATRÓN** `ibs_15` < `0.3424` → IC=+0.153 (n=142)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.76€ cuando `ibs_15` < 0.3424 (IC base=+0.085)

- **PATRÓN** `dist_vwap_pct` > `0.46` → IC=+0.200 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.46 (IC base=+0.085)

- **PATRÓN** `sigma_ewma_delta_pct` < `16.415` → IC=+0.131 (n=147)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` < 16.415 (IC base=+0.085)
...
- **PATRÓN** `sigma_h` < `0.0023` → IC=+0.189 (n=43)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0023 (IC base=+0.015)

### UPDOWN_GBM#ETH#60min
- **PATRÓN** `libro_spread` < `0.03` → IC=+0.189 (n=43)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.03 (IC base=+0.050)
...
- **PATRÓN** `ibs_15` < `0.1034` → IC=+0.132 (n=55)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.66€ cuando `ibs_15` < 0.1034 (IC base=+0.046)
...
- **FILTRO** `sigma_h` > `0.0077` → IC=-0.170 (n=377)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0077
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=1136)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3237` → IC=+0.176 (n=35)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3237 (IC base=-0.039)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.232 (n=106)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.197 (n=229)

- **FILTRO** `hora_utc` > `16.0` → IC=-0.233 (n=73)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.201 (n=262)
...
- **FILTRO** `libro_liquidez` < `12395.997` → IC=-0.216 (n=167)

  - _Acción_: SKIP cuando `libro_liquidez` < 12395.997
  - _Potencial_: sin este filtro IC_bueno=-0.200 (n=168)
...
- **FILTRO** `sigma_h` > `0.0078` → IC=-0.169 (n=134)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `drift_15min` |x|> `0.7892` → IC=-0.140 (n=123)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 0.7892
...
- **PATRÓN** `hora_utc` > `3.0` → IC=+0.316 (n=145)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 3.0 (IC base=+0.288)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_NO, IBS < 0.0835 sube el IC de +0.042 a +0.153 en UPDOWN_GBM#15min (n=223). Ya aplicado como kelly_boost=+0.77€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_NO, IBS < 0.3424 sube el IC de +0.085 a +0.153 en UPDOWN_GBM#ETH#15min (n=142). Ya aplicado como kelly_boost=+0.76€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM
50€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 849 | +0.015 | -2.50€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 503 | +0.009 | -3.48€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 503 | +0.009 | -3.48€ | 2 | 0 |
| ✅ UPDOWN_GBM | 3192 | +0.017 | +166.87€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1334 | +0.067 | +189.59€ | 1 | 10 |
| ✅ UPDOWN_GBM#240min | 141 | +0.004 | -3.11€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 1387 | -0.018 | -24.94€ | 4 | 0 |
| ✅ UPDOWN_GBM#60min | 283 | +0.002 | +5.84€ | 3 | 2 |
| ✅ UPDOWN_GBM#BNB | 140 | +0.085 | +27.36€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 129 | +0.111 | +30.02€ | 0 | 0 |
| 🚫 UPDOWN_GBM#BNB#5min | 9 | -0.102 | -2.69€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 628 | +0.030 | +49.98€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 151 | +0.088 | +17.70€ | 3 | 13 |
| ✅ UPDOWN_GBM#BTC#240min | 44 | +0.065 | +3.36€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#5min | 319 | +0.023 | +28.60€ | 1 | 3 |
| ✅ UPDOWN_GBM#BTC#60min | 96 | -0.020 | -1.51€ | 2 | 2 |
...
| ✅ UPDOWN_GBM#ETH | 720 | +0.042 | +57.07€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 314 | +0.098 | +57.61€ | 1 | 11 |
| ✅ UPDOWN_GBM#ETH#240min | 43 | +0.033 | -0.34€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 234 | -0.013 | -6.92€ | 3 | 1 |
| ✅ UPDOWN_GBM#ETH#60min | 114 | +0.035 | +7.10€ | 0 | 1 |
| 🚫 UPDOWN_GBM#ETH#daily | 15 | -0.154 | -0.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 663 | -0.005 | -5.35€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 282 | -0.007 | -6.60€ | 1 | 2 |
| ✅ UPDOWN_GBM#SOL#240min | 33 | -0.043 | -3.23€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 263 | +0.017 | +4.36€ | 2 | 1 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 2142 | -0.069 | +436.04€ | 4 | 4 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 2142 | -0.069 | +436.04€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 165 | -0.063 | +105.63€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 165 | -0.063 | +105.63€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 423 | -0.152 | -17.71€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 423 | -0.152 | -17.71€ | 4 | 1 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 689 | -0.044 | +206.90€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 689 | -0.044 | +206.90€ | 3 | 1 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 663 | -0.068 | +106.89€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 663 | -0.068 | +106.89€ | 5 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.097) — sin ventaja clara. oversold(IBS<0.3): IC=+0.001 n=1105 | neutral: IC=+0.013 n=1080 | overbought(IBS>0.7): IC=+0.098 n=1367
  - _Datos_: n=3787 IC=+0.041 PNL=+343.84€
...
**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  ...
  - _Estado_: 2 celda(s) pasan gate riguroso completo de 80 evaluadas (n>=40) y 376 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=-0.007 < 0.08 — monitorear
  - _Datos_: n=282 IC=-0.007 PNL=-6.60€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.091 n=51905 | tras_1loss IC=+0.050 n=38526 | tras_2loss IC=+0.011 n=17276/40 | gap=+0.081 (umbral 0.05)
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=114/40 IC=+0.035 PNL=+7.10€ | BTC#60min: n=96/40 IC=-0.020 PNL=-1.51€ | SOL#60min: n=73/40 IC=-0.020 PNL=+0.25€

**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  - _Umbral_: n≥40 en contrario_BTC y gap≥0.08 — y descartar confound con drift propio antes de actuar
  - _Acción_: Si se confirma y no es confound → boost en ETH/SOL cuando decisión contraria a drift_15min BTC
  - _Estado_: alineado_BTC IC=+0.034 n=277 | contrario_BTC IC=-0.031 n=194/40 | gap=-0.065 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2956 IC=+0.011 PNL=+112.94€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2956 IC=+0.011 PNL=+112.94€
...
**〰️ H-CUSTOM-GBM-60MIN-BUYYES** — GBM 60min BUY_YES — ¿edge superior al BUY_NO?
  ...
  - _Estado_: n=224 IC=+0.031 PNL=+13.31€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=224 IC=+0.031 PNL=+13.31€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=828 IC=+0.059 PNL=+85.55€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=828 IC=+0.059 PNL=+85.55€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=501 IC=-0.029 PNL=-25.15€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=501 IC=-0.029 PNL=-25.15€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=780 IC=+0.046 PNL=+92.12€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=780 IC=+0.046 PNL=+92.12€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=445 IC=+0.035 PNL=+35.48€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=445 IC=+0.035 PNL=+35.48€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=1087 IC=+0.029 PNL=+68.19€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1087 IC=+0.029 PNL=+68.19€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1979 IC=-0.123 PNL=+249.34€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1979 IC=-0.123 PNL=+249.34€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=383 IC=+0.144 PNL=+144.75€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=383 IC=+0.144 PNL=+144.75€
...
**〰️ H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: n=395 IC=-0.024 PNL=+14.65€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=395 IC=-0.024 PNL=+14.65€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.198 > 0.08 con n=342 PNL=+198.10€
  - _Datos_: n=342 IC=+0.198 PNL=+198.10€
...
**〰️ H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: n=644 IC=+0.003 PNL=+145.13€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=644 IC=+0.003 PNL=+145.13€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.241 > 0.08 con n=797 PNL=-84.94€
  - _Datos_: n=797 IC=+0.241 PNL=-84.94€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=1550 IC=+0.100 PNL=-346.27€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=1550 IC=+0.100 PNL=-346.27€
