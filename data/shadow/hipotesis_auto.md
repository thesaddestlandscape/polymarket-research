# Hipótesis automáticas — 2026-07-15 14:25 UTC
_Generado por shadow_postmortem.py sobre 15424 resoluciones (PNL=+3049.27€)_
...
- **PATRÓN** `py_entrada` > `0.695` → IC=+0.310 (n=361)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.192)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.197 (n=908)

  - _Acción_: Kelly boost +0.98€ cuando `libro_spread` < 0.01 (IC base=+0.192)

- **PATRÓN** `libro_liquidez` > `4402.2111` → IC=+0.194 (n=514)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 4402.2111 (IC base=+0.192)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.215 (n=121)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.615` → IC=+0.253 (n=152)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.248 (n=153)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.237)

- **PATRÓN** `py_entrada` > `0.725` → IC=+0.346 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.725 (IC base=+0.237)

- **PATRÓN** `libro_liquidez` > `5171.5093` → IC=+0.244 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 5171.5093 (IC base=+0.237)
...
- **PATRÓN** `libro_liquidez` > `1636.4687` → IC=+0.226 (n=166)
...
### GBM_LATE_15M
- **PATRÓN** `sigma_ewma_delta_pct` > `11.419` → IC=+0.233 (n=84)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `16.029` → IC=+0.206 (n=15)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 16.029 (IC base=+0.078)
...
- **PATRÓN** `sigma_h` < `0.0297` → IC=+0.143 (n=421)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` < 0.0297 (IC base=+0.142)

- **PATRÓN** `sigma_h` > `0.0228` → IC=+0.179 (n=191)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` > 0.0228 (IC base=+0.142)
...
- **PATRÓN** `dist_vwap_pct` > `0.5361` → IC=+0.194 (n=34)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.5361 (IC base=+0.142)
...
- **PATRÓN** `sigma_h` > `0.0123` → IC=+0.231 (n=221)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0123 (IC base=+0.162)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.178 (n=467)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 12.0 (IC base=+0.162)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.167 (n=698)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 18.0 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` > `1.0625` → IC=+0.231 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0625 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.225` → IC=+0.308 (n=76)
...
- **PATRÓN** `sigma_h` > `0.0027` → IC=+0.153 (n=165)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0027 (IC base=+0.124)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.181 (n=114)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 12.0 (IC base=+0.124)

- **PATRÓN** `dist_vwap_pct` > `0.132` → IC=+0.160 (n=48)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.132 (IC base=+0.124)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.466` → IC=+0.241 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.466 (IC base=+0.124)
...
- **PATRÓN** `sigma_h` < `0.0179` → IC=+0.266 (n=143)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0179 (IC base=+0.255)

- **PATRÓN** `sigma_h` > `0.0101` → IC=+0.259 (n=143)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.297 (n=131)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.255)

- **PATRÓN** `dist_vwap_pct` > `0.9145` → IC=+0.357 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9145 (IC base=+0.255)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.099` → IC=+0.325 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.099 (IC base=+0.255)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0148` → IC=+0.170 (n=201)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0148 (IC base=+0.104)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.125 (n=545)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` < 15.0 (IC base=+0.104)

- **PATRÓN** `dist_vwap_pct` > `0.6918` → IC=+0.173 (n=96)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.6918 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.39` → IC=+0.327 (n=73)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.39 (IC base=+0.104)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.133 (n=126)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 7.0 (IC base=+0.086)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.702` → IC=+0.227 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.702 (IC base=+0.086)
...
- **PATRÓN** `sigma_h` < `0.0324` → IC=+0.195 (n=172)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0324 (IC base=+0.191)

- **PATRÓN** `sigma_h` > `0.0178` → IC=+0.224 (n=114)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0178 (IC base=+0.191)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.222 (n=156)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.191)

- **PATRÓN** `dist_vwap_pct` > `0.189` → IC=+0.232 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.189 (IC base=+0.191)

- **PATRÓN** `dist_vwap_pct` < `1.4833` → IC=+0.184 (n=96)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` < 1.4833 (IC base=+0.191)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.253` → IC=+0.214 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.253 (IC base=+0.191)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.983` → IC=+0.180 (n=48)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` < 4.983 (IC base=+0.191)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2175 | +0.175 | -41.68€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1402 | +0.209 | +8.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 91 | +0.016 | -14.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 313 | +0.094 | -46.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 369 | +0.152 | +10.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 632 | +0.180 | -15.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 469 | +0.199 | -22.21€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 772 | +0.177 | +7.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 463 | +0.220 | +23.54€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 30 | -0.125 | -12.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 156 | +0.108 | -18.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 123 | +0.172 | +14.84€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL | 765 | +0.170 | -29.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 464 | +0.210 | +11.15€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 31 | +0.076 | -1.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 147 | +0.104 | -24.37€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 4596 | +0.102 | +1406.61€ | 0 | 3 |
| ✅ GBM_LATE_15M#15min | 4596 | +0.102 | +1406.61€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1103 | +0.067 | +159.67€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1103 | +0.067 | +159.67€ | 0 | 3 |
| ✅ GBM_LATE_15M#ETH | 1119 | +0.075 | +190.85€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1119 | +0.075 | +190.85€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1222 | +0.111 | +494.46€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1222 | +0.111 | +494.46€ | 0 | 6 |
| ✅ GBM_LATE_15M#XRP | 1152 | +0.152 | +561.63€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1152 | +0.152 | +561.63€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1851 | +0.154 | +1097.34€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1851 | +0.154 | +1097.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 473 | +0.125 | +249.85€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 473 | +0.125 | +249.85€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 488 | +0.129 | +224.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 488 | +0.129 | +224.84€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 499 | +0.111 | +216.66€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 499 | +0.111 | +216.66€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 391 | +0.274 | +405.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 391 | +0.274 | +405.98€ | 0 | 11 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 30 | -0.094 | -1.11€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 30 | -0.094 | -1.11€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 6 | +0.075 | +1.95€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 6 | +0.075 | +1.95€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 1804 | +0.085 | +501.14€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1804 | +0.085 | +501.14€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 448 | +0.062 | +75.49€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 448 | +0.062 | +75.49€ | 0 | 2 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 405 | +0.016 | +10.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 405 | +0.016 | +10.98€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 440 | +0.048 | +71.99€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 440 | +0.048 | +71.99€ | 2 | 5 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 511 | +0.190 | +342.68€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 511 | +0.190 | +342.68€ | 0 | 12 |
...
| ✅ LEADLAG_BTC_XRP_15M | 121 | +0.085 | +28.76€ | 0 | 4 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 121 | +0.085 | +28.76€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 121 | +0.085 | +28.76€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 121 | +0.085 | +28.76€ | 0 | 4 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.100 n=8881 | tras_1loss IC=+0.061 n=6322 | tras_2loss IC=+0.029 n=2741/40 | gap=+0.071 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=12966 IC=+0.102 PNL=+3125.16€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=12966 IC=+0.102 PNL=+3125.16€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.252 > 0.2 con n=305 PNL=+205.89€
  - _Datos_: n=305 IC=+0.252 PNL=+205.89€
