# Hipótesis automáticas — 2026-07-03 14:44 UTC
_Generado por shadow_postmortem.py sobre 3093 resoluciones (PNL=-37.94€)_
...
- **PATRÓN** `sigma_h` < `0.0096` → IC=+0.204 (n=79)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0096 (IC base=+0.039)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.122 (n=43)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 15.0 (IC base=+0.039)

- **PATRÓN** `ibs_15` < `0.0613` → IC=+0.161 (n=57)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.81€ cuando `ibs_15` < 0.0613 (IC base=+0.039)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1297` → IC=+0.151 (n=61)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.75€ cuando `pct_spot_vs_ref` |x|≤ 0.1297 (IC base=+0.115)

- **PATRÓN** `sigma_h` < `0.0139` → IC=+0.151 (n=61)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0139 (IC base=+0.115)
...
- **PATRÓN** `ibs_15` < `0.0792` → IC=+0.265 (n=32)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0792 (IC base=+0.115)
...
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 432 | +0.051 | +55.52€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 432 | +0.051 | +55.52€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#SOL | 108 | +0.118 | +30.92€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 108 | +0.118 | +30.92€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 861 | -0.040 | -48.09€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 631 | -0.018 | -20.25€ | 0 | 3 |
...
| ✅ UPDOWN_GBM#ETH | 340 | -0.009 | +10.08€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 256 | +0.008 | +16.54€ | 3 | 4 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.060) — sin ventaja clara. oversold(IBS<0.3): IC=-0.006 n=164 | neutral: IC=-0.066 n=141 | overbought(IBS>0.7): IC=-0.046 n=150
  - _Datos_: n=471 IC=-0.039 PNL=-22.86€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.009 n=1484 | tras_1loss IC=-0.027 n=1517 | tras_2loss IC=-0.043 n=780/40 | gap=+0.052 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=738 IC=-0.035 PNL=-34.54€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=738 IC=-0.035 PNL=-34.54€
...
**🟡 H-CUSTOM-ETH15-REVERSION** — ETH#15min con drift_15min < -1 — ¿mean reversion?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.087 > 0.08 con n=44 PNL=+14.24€
  - _Datos_: n=44 IC=+0.087 PNL=+14.24€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=221 IC=+0.016 PNL=+15.05€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=221 IC=+0.016 PNL=+15.05€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=667 IC=+0.034 PNL=+52.22€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=667 IC=+0.034 PNL=+52.22€
