# Hipótesis automáticas — 2026-08-06 08:33 UTC
_Generado por shadow_postmortem.py sobre 78898 resoluciones (PNL=+13075.78€)_
...
- **PATRÓN** `sigma_h` > `0.0066` → IC=+0.186 (n=2240)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0066 (IC base=+0.144)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.155 (n=3129)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.891` → IC=+0.242 (n=1248)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.891 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `2054.4766` → IC=+0.152 (n=3000)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 2054.4766 (IC base=+0.144)

- **PATRÓN** `sigma_h` > `0.0039` → IC=+0.160 (n=2941)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` > 0.0039 (IC base=+0.144)

- **PATRÓN** `drift_60min` |x|≤ `0.1228` → IC=+0.184 (n=17)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.1228 (IC base=+0.144)
...
- **PATRÓN** `dist_vwap_pct` > `0.62` → IC=+0.143 (n=312)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.62 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` < `0.1627` → IC=+0.124 (n=2532)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.1627 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.189` → IC=+0.172 (n=486)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 8.189 (IC base=+0.144)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.452` → IC=+0.179 (n=160)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 9.452 (IC base=+0.080)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.132 (n=267)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` < 7.0 (IC base=+0.078)
...
- **PATRÓN** `sigma_h` > `0.0085` → IC=+0.246 (n=487)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.7734` → IC=+0.357 (n=33)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7734 (IC base=+0.221)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.29` → IC=+0.253 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.29 (IC base=+0.221)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.236 (n=639)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1306` → IC=+0.267 (n=28)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1306 (IC base=+0.181)

- **PATRÓN** `hora_utc` > `21.0` → IC=+0.260 (n=23)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 21.0 (IC base=+0.181)

- **PATRÓN** `hora_utc` < `2.0` → IC=+0.300 (n=28)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 2.0 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.589` → IC=+0.254 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.589 (IC base=+0.181)

- **PATRÓN** `libro_liquidez` > `1998.99` → IC=+0.208 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1998.99 (IC base=+0.181)

### GBM_LATE_15M_PYCONFIRMADO
- **FILTRO** `sigma_h` < `0.0069` → IC=-0.283 (n=113)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.340 (n=48)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0038 (IC base=+0.286)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.331 (n=128)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.286)

- **PATRÓN** `dist_vwap_pct` > `0.5537` → IC=+0.316 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5537 (IC base=+0.286)

- **PATRÓN** `dist_vwap_pct` < `0.0981` → IC=+0.290 (n=103)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0981 (IC base=+0.286)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.925` → IC=+0.439 (n=47)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.925 (IC base=+0.286)
...
- **PATRÓN** `libro_liquidez` > `3120.1883` → IC=+0.320 (n=48)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3120.1883 (IC base=+0.286)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.124 (n=2241)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` > 0.007 (IC base=+0.096)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.871` → IC=+0.210 (n=1176)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.871 (IC base=+0.096)

- **PATRÓN** `drift_60min` |x|≤ `0.1815` → IC=+0.214 (n=40)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1815 (IC base=+0.106)
...
- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.167 (n=855)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` > 0.0068 (IC base=+0.154)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=897)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.43` → IC=+0.281 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.43 (IC base=+0.154)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.159 (n=732)

  - _Acción_: Kelly boost +0.80€ cuando `libro_spread` < 0.01 (IC base=+0.154)

- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.183 (n=869)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.7353` → IC=+0.268 (n=54)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7353 (IC base=+0.172)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.352` → IC=+0.175 (n=782)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` < 3.352 (IC base=+0.172)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.565` → IC=+0.204 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.565 (IC base=+0.017)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1888` → IC=+0.147 (n=284)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.1888 (IC base=+0.104)
...
- **PATRÓN** `drift_15min` |x|≤ `0.4618` → IC=+0.139 (n=81)
  ...
  - _Acción_: Kelly boost +0.69€ cuando `drift_15min` |x|≤ 0.4618 (IC base=+0.076)
...
- **PATRÓN** `sigma_h` < `0.0074` → IC=+0.135 (n=154)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.0074 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.141 (n=179)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 12.0 (IC base=+0.114)

- **PATRÓN** `ibs_15` < `0.0448` → IC=+0.143 (n=82)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.71€ cuando `ibs_15` < 0.0448 (IC base=+0.114)

- **PATRÓN** `libro_liquidez` > `3035.0793` → IC=+0.158 (n=71)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 3035.0793 (IC base=+0.114)
...
- **PATRÓN** `sigma_h` > `0.0056` → IC=+0.159 (n=1009)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.206 (n=457)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `dist_vwap_pct` < `0.3791` → IC=+0.157 (n=782)
  ...
  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.3791 (IC base=+0.179)
...
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.218 (n=133)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0067 (IC base=+0.174)
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 1675 | +0.052 | -30.48€ | 0 | 4 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 1675 | +0.052 | -30.48€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 729 | +0.025 | -39.47€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 729 | +0.025 | -39.47€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS | 1295 | -0.053 | -132.18€ | 0 | 1 |
| ✅ BALLENAS_TARDIAS#15min | 317 | +0.265 | -16.08€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 978 | -0.156 | -116.10€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#BNB | 250 | -0.302 | -40.95€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#BNB#5min | 250 | -0.302 | -40.95€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 317 | +0.265 | -16.08€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 317 | +0.265 | -16.08€ | 0 | 1 |
| ✅ BALLENAS_TARDIAS#DOGE | 221 | -0.137 | -38.07€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 221 | -0.137 | -38.07€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 183 | -0.197 | +4.90€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 183 | -0.197 | +4.90€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 184 | -0.145 | -27.26€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 184 | -0.145 | -27.26€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 140 | +0.120 | -14.70€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 140 | +0.120 | -14.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 20214 | +0.122 | -469.28€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 5537 | +0.194 | -110.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 425 | +0.053 | -35.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 12580 | +0.090 | -386.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 1672 | +0.144 | +63.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 1973 | +0.083 | -103.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 9 | -0.021 | -4.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 1961 | +0.084 | -102.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 4518 | +0.133 | -79.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 
47€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 354 | +0.073 | +63.77€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 354 | +0.073 | +63.77€ | 1 | 12 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 346 | +0.201 | +235.74€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 346 | +0.201 | +235.74€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 993 | +0.194 | +684.31€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 993 | +0.194 | +684.31€ | 0 | 20 |
...
| ✅ UPDOWN_GBM_IBS_ALTO | 101 | +0.286 | +66.37€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#15min | 101 | +0.286 | +66.37€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#BTC | 53 | +0.264 | +27.14€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#BTC#15min | 53 | +0.264 | +27.14€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#ETH | 48 | +0.300 | +39.23€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#ETH#15min | 48 | +0.300 | +39.23€ | 0 | 0 |
| 🚫 UPDOWN_OU_5M | 100 | -0.206 | -22.13€ | 0 | 0 |
| 🚫 UPDOWN_OU_5M#5min | 100 | -0.206 | -22.13€ | 0 | 0 |
| 🚫 UPDOWN_OU_5M#BNB | 19 | -0.294 | -7.17€ | 0 | 0 |
| 🚫 UPDOWN_OU_5M#BNB#5min | 19 | -0.294 | -7.17€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#BTC | 15 | -0.110 | -2.49€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#BTC#5min | 15 | -0.110 | -2.49€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#DOGE | 17 | -0.157 | -3.72€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#DOGE#5min | 17 | -0.157 | -3.72€ | 0 | 0 |
| 🚫 UPDOWN_OU_5M#ETH | 18 | -0.225 | -4.88€ | 0 | 0 |
| 🚫 UPDOWN_OU_5M#ETH#5min | 18 | -0.225 | -4.88€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#SOL | 16 | -0.089 | -2.19€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#SOL#5min | 16 | -0.089 | -2.19€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#XRP | 15 | -0.066 | -1.69€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#XRP#5min | 15 | -0.066 | -1.69€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.152: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.111 n=2430 | neutral: IC=+0.050 n=1896 | overbought(IBS>0.7): IC=+0.202 n=2360
  - _Datos_: n=7136 IC=+0.130 PNL=+2427.84€
...
**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  ...
  - _Estado_: 33 celda(s) pasan gate riguroso completo de 140 evaluadas (n>=40) y 290 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.050 < 0.08 — monitorear
  - _Datos_: n=398 IC=+0.050 PNL=+54.37€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.128 n=48202 | tras_1loss IC=+0.090 n=30385 | tras_2loss IC=+0.064 n=12421/40 | gap=+0.064 (umbral 0.05)
...
**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  ...
  - _Estado_: alineada_con_outcome_prev IC=+0.033 n=178/60 | contraria IC=+0.114 n=68 | gap=-0.081 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=55/40 IC=-0.026 PNL=-3.07€ | BTC#60min: n=43/40 IC=-0.078 PNL=-6.67€ | SOL#60min: n=35/40 IC=-0.095 PNL=-1.92€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=3684 IC=+0.063 PNL=+541.82€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=3684 IC=+0.063 PNL=+541.82€
...
**〰️ H-CUSTOM-GBM-60MIN-BUYNO** — GBM 60min BUY_NO — tracking por separado
  ...
  - _Estado_: n=44 IC=-0.065 PNL=-4.07€ — sin señal clara aún (umbral IC: min=0.05 max=None)
  - _Datos_: n=44 IC=-0.065 PNL=-4.07€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=1451 IC=+0.063 PNL=+214.40€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1451 IC=+0.063 PNL=+214.40€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=595 IC=+0.091 PNL=+101.40€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=595 IC=+0.091 PNL=+101.40€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.095 > 0.08 con n=1890 PNL=+359.04€
  - _Datos_: n=1890 IC=+0.095 PNL=+359.04€

**⏳ H-CUSTOM-LONGSHOT-BIAS** — Longshot bias — ¿mejor IC cuando py_mkt < 0.20 o > 0.80?
  - _Hipótesis_: Jon-Becker repo documenta formalmente: contratos a 1-20 cents tienen win_rate < precio implícito (compradores pierden sistemáticamente en longshots). En nuestro sistema: cuando py_mkt<0.20 el GBM predice BUY_NO con edge estructural adicional al del modelo. ¿Se confirma en nuestros datos? Buscar en feature pct_spot_vs_ref si los mercados extremos tienen mejor IC en BUY_NO.
  - _Umbral_: 30
  - _Acción_: Si IC>0.10 con n≥30 en mercados extremos → boost ×1.2 en BUY_NO cuando py_mkt<0.20
  - _Estado_: 27/30 ops en el filtro definido (IC actual=-0.155 PNL=-0.78€)
  - _Datos_: n=27 IC=-0.155 PNL=-0.78€
...
**〰️ H-CUSTOM-GBM-09H** — GBM a las 09h UTC — bloqueada 2026-06-29
  ...
  - _Estado_: n=49 IC=-0.029 PNL=-2.44€ — sin señal clara aún (umbral IC: min=None max=-0.1)
  - _Datos_: n=49 IC=-0.029 PNL=-2.44€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=2030 IC=+0.072 PNL=+305.99€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=2030 IC=+0.072 PNL=+305.99€

**〰️ H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  - _Hipótesis_: Feature nueva 2026-06-27: drift del precio YES en Polymarket en últimas 5 obs (~5min). Si poly_drift<0 y decidimos BUY_NO (o poly_drift>0 y BUY_YES) → confluencia. Si diverge → reducción de stake. Hipótesis: confluencia Binance+Polymarket mejora IC; divergencia empeora.
  - _Umbral_: n≥40 en confluencia vs divergencia para validar el boost ×1.1
  - _Acción_: Si IC_confluencia>IC_divergencia con n≥40 → mantener el boost. Si no → retirar.
  - _Estado_: n=651 IC=+0.076 PNL=+83.61€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=651 IC=+0.076 PNL=+83.61€
...
**🟡 H-CUSTOM-CROSS-WINDOW-SPREAD-POS** — GBM 15min/60min: spread positivo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.240 > 0.08 con n=286 PNL=+65.20€
  - _Datos_: n=286 IC=+0.240 PNL=+65.20€
...
**〰️ H-CUSTOM-CROSS-WINDOW-SPREAD-NEG** — GBM 15min/60min: spread negativo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: n=263 IC=+0.028 PNL=+59.57€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=263 IC=+0.028 PNL=+59.57€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=558 IC=+0.057 PNL=+67.22€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=558 IC=+0.057 PNL=+67.22€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=1601 IC=+0.043 PNL=+183.85€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1601 IC=+0.043 PNL=+183.85€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.167 > 0.1 con n=1548 PNL=+540.32€
  - _Datos_: n=1548 IC=+0.167 PNL=+540.32€
...
**〰️ H-CUSTOM-BUYNO-LONGSHOT-15MIN** — BUY_NO longshot en 15min (py_mkt≥0.55) — comprar NO barato pierde
  ...
  - _Estado_: n=82 IC=-0.012 PNL=+20.65€ — sin señal clara aún (umbral IC: min=None max=-0.1)
  - _Datos_: n=82 IC=-0.012 PNL=+20.65€
...
**🟡 H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.114 > 0.1 con n=464 PNL=+144.82€
  - _Datos_: n=464 IC=+0.114 PNL=+144.82€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1556 IC=-0.081 PNL=+363.59€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1556 IC=-0.081 PNL=+363.59€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=1414 IC=+0.064 PNL=+151.38€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=1414 IC=+0.064 PNL=+151.38€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.111 > 0.08 con n=1047 PNL=+260.65€
  - _Datos_: n=1047 IC=+0.111 PNL=+260.65€
...
**🟡 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.161 > 0.08 con n=376 PNL=+118.95€
  - _Datos_: n=376 IC=+0.161 PNL=+118.95€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=1657 IC=+0.187 PNL=+934.14€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=1657 IC=+0.187 PNL=+934.14€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.085 > 0.08 con n=822 PNL=+159.21€
  - _Datos_: n=822 IC=+0.085 PNL=+159.21€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.114 > 0.08 con n=887 PNL=+282.93€
  - _Datos_: n=887 IC=+0.114 PNL=+282.93€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.125 > 0.08 con n=1205 PNL=+545.30€
  - _Datos_: n=1205 IC=+0.125 PNL=+545.30€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.204 > 0.08 con n=468 PNL=+25.82€
  - _Datos_: n=468 IC=+0.204 PNL=+25.82€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.260 > 0.08 con n=705 PNL=-26.03€
  - _Datos_: n=705 IC=+0.260 PNL=-26.03€
...
**〰️ H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Estado_: n=141 IC=+0.059 PNL=+16.35€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=141 IC=+0.059 PNL=+16.35€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=1449 IC=+0.158 PNL=-190.26€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=1449 IC=+0.158 PNL=-190.26€

**🟡 H-CUSTOM-GBMLATE15M-SOL-RESCATE-PRECIO** — GBM_LATE_15M#SOL#15min#BUY_YES (pausada 05-Ago) -- posible rescate con filtro py en [0.45,0.55)
  - _Hipótesis_: 06-Ago: hallazgo al barrer gate_bucket_propio.json. GBM_LATE_15M#SOL#15min#BUY_YES fue PAUSADA el 05-Ago por veto sigma_ewma_delta_pct (ver project_veto_sigma_ewma_gbmlate_05ago). Desagregando por precio: bucket [0.50,0.55) tiene n=411, pnl/trade +0.498, gate riguroso COMPLETO (bueno_confirmado, split-half consistente ambas mitades [0.305,0.273]). El bucket vecino [0.45,0.50) (n=356, sin_concluir todavia) tambien da pnl positivo +0.323. Juntos (0.45-0.55) suman n=767, la mayoria del volumen de la tupla. En cambio [0.20,0.25) (n=20) da pnl=-0.866, malo_confirmado -- el problema parece concentrado en precio bajo, no en toda la tupla. HIPOTESIS: restringir la reactivacion a un filtro de precio py en [0.45,0.55) en vez de mantener la pausa total podria rescatar la mayor parte del edge sin el drenaje que motivo la pausa -- pero el veto sigma_ewma que causo la pausa es una dimension DISTINTA (volatilidad reciente, no precio), asi que ambos filtros podrian ser complementarios, no sustitutos. NO proponer reactivacion sin cruzar este hallazgo con el analisis original de sigma_ewma que motivo la pausa.
  - _Umbral_: Cruzar con el analisis de sigma_ewma_delta_pct que motivo la pausa 05-Ago antes de proponer nada -- ver si el filtro de precio [0.45,0.55) ya excluye la mayoria de los casos que sigma_ewma vetaria, o si son independientes
  - _Acción_: Investigacion pendiente: cruzar bucket de precio con el estado de sigma_ewma_delta_pct en las mismas filas. Si son independientes, un filtro combinado (precio Y sigma_ewma) podria ser mas preciso que cualquiera de los dos solo.
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.153 > 0.15 con n=767 PNL=+319.63€
