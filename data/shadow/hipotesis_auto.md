# Hipótesis automáticas — 2026-07-28 10:47 UTC
_Generado por shadow_postmortem.py sobre 39941 resoluciones (PNL=+8172.49€)_
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.188 (n=1232)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0075 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.161 (n=1717)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 7.0 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.2733` → IC=+0.188 (n=504)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.2733 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.398` → IC=+0.230 (n=591)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.398 (IC base=+0.142)

- **PATRÓN** `sigma_h` > `0.0067` → IC=+0.156 (n=1431)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.3922` → IC=+0.129 (n=340)
  ...
  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` > 0.3922 (IC base=+0.121)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.139` → IC=+0.151 (n=316)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 8.139 (IC base=+0.121)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.142 (n=425)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.140 (n=462)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` > 0.0078 (IC base=+0.112)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.566` → IC=+0.266 (n=199)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.566 (IC base=+0.112)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` < `0.0175` → IC=+0.251 (n=427)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0175 (IC base=+0.230)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.250 (n=386)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.230)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.229 (n=382)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.230)

- **PATRÓN** `dist_vwap_pct` > `0.4483` → IC=+0.293 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4483 (IC base=+0.230)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.55` → IC=+0.344 (n=120)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.55 (IC base=+0.230)

- **PATRÓN** `sigma_h` < `0.0172` → IC=+0.209 (n=551)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0172 (IC base=+0.205)

- **PATRÓN** `sigma_h` > `0.007` → IC=+0.232 (n=551)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` < `9.119` → IC=+0.197 (n=605)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` < 9.119 (IC base=+0.205)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0122` → IC=+0.128 (n=606)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.64€ cuando `sigma_h` > 0.0122 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` > `0.4595` → IC=+0.155 (n=331)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.4595 (IC base=+0.094)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.033` → IC=+0.229 (n=293)
...
- **PATRÓN** `sigma_h` < `0.0192` → IC=+0.176 (n=474)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` < 0.0192 (IC base=+0.169)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.175 (n=543)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 6.0 (IC base=+0.169)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.169 (n=482)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` < `0.0221` → IC=+0.155 (n=665)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0221 (IC base=+0.155)

- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.170 (n=665)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` < `9.452` → IC=+0.153 (n=686)
...
- **LIVE-CANDIDATA**: `GBM_LATE_15M_ESPACIO_ATR#BNB#15min` — IC=+0.284 n=35. Faltan ~5 resoluciones para umbral n≥40. ETA: ~4h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M_ESPACIO_ATR#BNB` — IC=+0.284 n=35. Faltan ~5 resoluciones para umbral n≥40. ETA: ~4h.
...
| ✅ FAVORITO_CONFIRMADO | 6893 | +0.177 | -55.81€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 4540 | +0.201 | -44.90€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 2390 | +0.182 | -3.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1494 | +0.207 | +5.74€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 442 | +0.302 | +6.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 442 | +0.302 | +6.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 442 | +0.302 | +6.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 442 | +0.302 | +6.23€ | 0 | 0 |
| ✅ GBM_LATE_15M | 8593 | +0.093 | +2546.74€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 8593 | +0.093 | +2546.74€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 47 | +0.173 | +15.88€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 47 | +0.173 | +15.88€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 2056 | +0.072 | +350.32€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 2056 | +0.072 | +350.32€ | 0 | 5 |
| ✅ GBM_LATE_15M#DOGE | 46 | +0.000 | -1.96€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 46 | +0.000 | -1.96€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 1851 | +0.073 | +312.99€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1851 | +0.073 | +312.99€ | 0 | 6 |
| ✅ GBM_LATE_15M#SOL | 2291 | +0.082 | +772.20€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 2291 | +0.082 | +772.20€ | 1 | 3 |
| ✅ GBM_LATE_15M#XRP | 2302 | +0.139 | +1097.32€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 2302 | +0.139 | +1097.32€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 5828 | +0.116 | +2713.46€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 5828 | +0.116 | +2713.46€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 35 | +0.284 | +33.89€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 35 | +0.284 | +33.89€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 1505 | +0.084 | +508.49€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 1505 | +0.084 | +508.49€ | 0 | 4 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 46 | +0.146 | +19.41€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 46 | +0.146 | +19.41€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1497 | +0.088 | +527.49€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1497 | +0.088 | +527.49€ | 0 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 1441 | +0.083 | +551.94€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 1441 | +0.083 | +551.94€ | 0 | 4 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 1304 | +0.216 | +1072.23€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 1304 | +0.216 | +1072.23€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 5815 | +0.064 | +1493.89€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#15min | 5815 | +0.064 | +1493.89€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 42 | +0.227 | +20.59€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 42 | +0.227 | +20.59€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 1371 | +0.027 | +147.32€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 1371 | +0.027 | +147.32€ | 0 | 2 |
| ✅ GBM_LATE_15M_TARDIO#DOGE | 42 | +0.114 | +7.27€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#DOGE#15min | 42 | +0.114 | +7.27€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 1397 | +0.022 | +111.93€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 1397 | +0.022 | +111.93€ | 0 | 1 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 1356 | +0.022 | +240.86€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 1356 | +0.022 | +240.86€ | 0 | 1 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 1607 | +0.161 | +965.94€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1607 | +0.161 | +965.94€ | 0 | 11 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.109 n=23707 | tras_1loss IC=+0.078 n=16011 | tras_2loss IC=+0.053 n=6723/40 | gap=+0.056 (umbral 0.05)
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=3659 IC=+0.060 PNL=+517.06€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=3659 IC=+0.060 PNL=+517.06€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.201 > 0.2 con n=711 PNL=+408.42€
  - _Datos_: n=711 IC=+0.201 PNL=+408.42€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.106 > 0.08 con n=612 PNL=+182.83€
  - _Datos_: n=612 IC=+0.106 PNL=+182.83€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.206 > 0.08 con n=437 PNL=+27.46€
  - _Datos_: n=437 IC=+0.206 PNL=+27.46€
