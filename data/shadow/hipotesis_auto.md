# Hipótesis automáticas — 2026-08-29 00:43 UTC
_Generado por shadow_postmortem.py sobre 194968 resoluciones (PNL=+14630.08€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `147.93` → IC=-0.286 (n=2724)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 147.93
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=8172)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `133.51` → IC=-0.275 (n=344)
...
- **FILTRO** `restante_s_al_confirmar` < `115.59` → IC=-0.386 (n=350)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=1055)

### BALLENAS_TARDIAS#ETH#5min
- **FILTRO** `restante_s_al_confirmar` < `161.14` → IC=-0.157 (n=732)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 161.14
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=2198)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `141.52` → IC=-0.309 (n=632)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 141.52
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=1897)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.159 (n=4681)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` < 7.0 (IC base=+0.143)

- **PATRÓN** `py_entrada` < `0.34` → IC=+0.259 (n=3713)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.219 (n=9939)
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.282 (n=1221)
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.205 (n=1122)
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.249 (n=1018)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.74 (IC base=+0.213)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.192 (n=1626)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.150 (n=2436)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.163 (n=1207)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.137 (n=1229)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9499` → IC=+0.209 (n=844)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.262` → IC=+0.157 (n=1008)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 5.262 (IC base=+0.064)
...
- **PATRÓN** `volumen_spike_ratio` > `2.924` → IC=+0.235 (n=345)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.924 (IC base=+0.030)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.232 (n=259)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.260 (n=223)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0752` → IC=+0.314 (n=111)
...
- **PATRÓN** `ibs_20min` < `0.4216` → IC=+0.290 (n=293)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4216 (IC base=+0.253)

- **PATRÓN** `sigma_ewma_delta_pct` < `1.934` → IC=+0.280 (n=348)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 1.934 (IC base=+0.253)

- **PATRÓN** `volumen_pendiente_norm` < `0.0647` → IC=+0.243 (n=204)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0647 (IC base=+0.253)

- **PATRÓN** `volumen_pendiente_norm` > `0.3054` → IC=+0.311 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3054 (IC base=+0.253)

- **PATRÓN** `volumen_spike_ratio` < `1.8721` → IC=+0.258 (n=151)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8721 (IC base=+0.253)

- **PATRÓN** `volumen_spike_ratio` > `2.7667` → IC=+0.308 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7667 (IC base=+0.253)
...
- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.253 (n=75)
...
- **FILTRO** `drift_60min` |x|> `0.5338` → IC=-0.156 (n=251)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.5338
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=489)
...
- **PATRÓN** `ibs_20min` > `0.2759` → IC=+0.120 (n=3177)

  - _Acción_: Kelly boost +0.60€ cuando `ibs_20min` > 0.2759 (IC base=+0.054)
...
- **PATRÓN** `dist_vwap_pct` < `0.2222` → IC=+0.214 (n=809)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.349` → IC=+0.177 (n=128)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 5.349 (IC base=-0.002)
...
- **PATRÓN** `sigma_h` > `0.0237` → IC=+0.278 (n=210)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.2788` → IC=+0.329 (n=553)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2788 (IC base=+0.272)

- **PATRÓN** `dist_vwap_pct` < `0.2733` → IC=+0.282 (n=613)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2904` → IC=+0.386 (n=68)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2904 (IC base=+0.272)

- **PATRÓN** `volumen_spike_ratio` > `2.1674` → IC=+0.288 (n=215)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1674 (IC base=+0.272)
...
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.214 (n=1957)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0067 (IC base=+0.188)

- **PATRÓN** `drift_60min` |x|≤ `0.4835` → IC=+0.197 (n=2934)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.4835 (IC base=+0.188)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.206 (n=1033)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.188)

- **PATRÓN** `ibs_20min` < `0.5628` → IC=+0.243 (n=2934)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5628 (IC base=+0.188)

- **PATRÓN** `dist_vwap_pct` < `0.4433` → IC=+0.180 (n=2187)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.4433 (IC base=+0.188)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.418` → IC=+0.206 (n=426)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.418 (IC base=+0.188)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.654` → IC=+0.192 (n=2753)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` < 2.654 (IC base=+0.188)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2881` → IC=+0.271 (n=325)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2881 (IC base=+0.188)

- **PATRÓN** `volumen_spike_ratio` > `2.3029` → IC=+0.206 (n=1013)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3029 (IC base=+0.188)

- **PATRÓN** `ballena_activa_n` < `172.0` → IC=+0.167 (n=1806)
...
- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.283 (n=229)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.263)

- **PATRÓN** `drift_60min` |x|≤ `0.3357` → IC=+0.273 (n=341)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3357 (IC base=+0.263)
...
- **PATRÓN** `ibs_20min` < `0.562` → IC=+0.302 (n=342)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.562 (IC base=+0.263)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.116` → IC=+0.283 (n=371)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3023` → IC=+0.325 (n=38)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3023 (IC base=+0.263)
...
- **PATRÓN** `libro_liquidez` > `1960.7048` → IC=+0.310 (n=114)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1960.7048 (IC base=+0.263)

- **PATRÓN** `ballena_activa_n` < `74.0` → IC=+0.239 (n=159)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 74.0 (IC base=+0.263)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4903` → IC=+0.225 (n=630)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.221 (n=760)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.216)

- **PATRÓN** `ibs_20min` < `0.4964` → IC=+0.280 (n=716)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4964 (IC base=+0.216)

- **PATRÓN** `dist_vwap_pct` < `0.4796` → IC=+0.225 (n=759)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4796 (IC base=+0.216)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.602` → IC=+0.296 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.602 (IC base=+0.216)
...
- **PATRÓN** `ballena_activa_n` < `37.0` → IC=+0.183 (n=437)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.145 (n=1259)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3812` → IC=+0.166 (n=841)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `4.0` → IC=+0.195 (n=332)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` > `0.0062` → IC=+0.138 (n=183)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0062 (IC base=+0.135)

- **PATRÓN** `drift_60min` |x|≤ `0.5277` → IC=+0.177 (n=274)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.89€ cuando `drift_60min` |x|≤ 0.5277 (IC base=+0.135)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.165 (n=186)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `ibs_20min` > `0.1102` → IC=+0.156 (n=274)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` > 0.1102 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` > `0.1525` → IC=+0.157 (n=103)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.1525 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.438` → IC=+0.180 (n=48)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 11.438 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` < `1.1919` → IC=+0.156 (n=274)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.1919 (IC base=+0.135)

- **PATRÓN** `volumen_pendiente_norm` < `0.135` → IC=+0.162 (n=279)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` < 0.135 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` < `2.183` → IC=+0.178 (n=237)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 2.183 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` > `1.4487` → IC=+0.158 (n=270)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.4487 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `10375.0696` → IC=+0.138 (n=92)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 10375.0696 (IC base=+0.135)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.172 (n=954)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=2881)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.134 (n=2397)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=7307)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.282 (n=2379)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=7325)

- **FILTRO** `ibs_7min` < `0.7297` → IC=-0.237 (n=2424)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7297
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=7280)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.173 (n=3263)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=6441)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.223 (n=2954)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=8880)

- **FILTRO** `ibs_7min` > `0.7174` → IC=-0.174 (n=2957)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7174
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=8877)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.316 (n=324)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=998)

- **FILTRO** `ibs_7min` < `1.0` → IC=-0.190 (n=926)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.249 (n=449)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=1631)

- **FILTRO** `ibs_7min` > `0.812` → IC=-0.189 (n=519)

  - _Acción_: SKIP cuando `ibs_7min` > 0.812
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=1561)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.149 (n=485)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
...
- **FILTRO** `py_entrada` < `0.44` → IC=-0.215 (n=784)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.44
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=848)

- **FILTRO** `ibs_7min` < `0.7576` → IC=-0.193 (n=408)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7576
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=1224)

- **FILTRO** `ballena_activa_n` > `42.0` → IC=-0.201 (n=402)

  - _Acción_: SKIP cuando `ballena_activa_n` > 42.0
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=1230)
...
- **FILTRO** `ballena_activa_n` > `25.0` → IC=-0.172 (n=550)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=1076)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.43` → IC=-0.229 (n=456)

  - _Acción_: SKIP cuando `py_entrada` < 0.43
  - _Potencial_: sin este filtro IC_bueno=+0.005 (n=1416)

- **FILTRO** `ibs_7min` < `0.7692` → IC=-0.202 (n=457)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.187 (n=532)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=1660)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.37` → IC=-0.299 (n=387)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=1209)
...
- **FILTRO** `delta_ratio_macro` |x|≤ `0.1001` → IC=-0.128 (n=181)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=370)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.701` → IC=-0.175 (n=364)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=2768)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0639` → IC=+0.244 (n=213)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0639 (IC base=-0.076)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2976` → IC=+0.263 (n=167)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.2976 (IC base=-0.076)

- **PATRÓN** `ibs_15` < `0.3707` → IC=+0.309 (n=213)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3707 (IC base=-0.076)

- **PATRÓN** `dist_vwap_pct` < `0.2526` → IC=+0.245 (n=222)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2526 (IC base=-0.076)

- **PATRÓN** `ballena_activa_n` < `67.0` → IC=+0.262 (n=120)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 67.0 (IC base=-0.076)
...
### UPDOWN_OU_5M
- **FILTRO** `pct_spot_vs_ref` |x|> `0.1011` → IC=-0.256 (n=43)
  - _Por qué funciona_: precio spot lejos de la referencia → señal GBM sobreextiende; riesgo de reversión
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3707 sube el IC de -0.076 a +0.309 en UPDOWN_GBM_15M_TARDIO (n=213). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 10896 | -0.095 | -1798.47€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 767 | -0.054 | -132.22€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 10129 | -0.098 | -1666.25€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1379 | -0.039 | -276.21€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1379 | -0.039 | -276.21€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 767 | -0.054 | -132.22€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 767 | -0.054 | -132.22€ | 3 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 1405 | -0.143 | -424.80€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 1405 | -0.143 | -424.80€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 2930 | -0.064 | -287.29€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 2930 | -0.064 | -287.29€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 2529 | -0.096 | -223.01€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2529 | -0.096 | -223.01€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1886 | -0.163 | -454.94€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1886 | -0.163 | -454.94€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 771 | -0.063 | +348.44€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 222 | +0.000 | +92.03€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 549 | -0.088 | +256.41€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 771 | -0.063 | +348.44€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 222 | +0.000 | +92.03€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 549 | -0.088 | +256.41€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 18309 | +0.191 | -1595.37€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 18309 | +0.191 | -1595.37€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 2864 | +0.234 | -62.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 2864 | +0.234 | -62.92€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 3136 | +0.168 | -379.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 3136 | +0.168 | -379.07€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 2913 | +0.227 | -98.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 