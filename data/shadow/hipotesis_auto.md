# Hipótesis automáticas — 2026-08-18 20:00 UTC
_Generado por shadow_postmortem.py sobre 67034 resoluciones (PNL=+6635.93€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `150.46` → IC=-0.254 (n=888)
...
- **FILTRO** `restante_s_al_confirmar` < `117.22` → IC=-0.404 (n=92)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.111 (n=278)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.157 (n=3912)
...
- **PATRÓN** `py_entrada` < `0.34` → IC=+0.285 (n=1485)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.34 (IC base=+0.151)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.189 (n=2012)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.02 (IC base=+0.151)

- **PATRÓN** `libro_liquidez` > `4043.1829` → IC=+0.180 (n=815)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 4043.1829 (IC base=+0.151)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.230 (n=309)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.199 (n=437)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.312 (n=269)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.296 (n=282)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.293)

- **PATRÓN** `libro_liquidez` > `3334.5877` → IC=+0.352 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3334.5877 (IC base=+0.293)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.278 (n=115)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.235)

- **PATRÓN** `py_entrada` < `0.31` → IC=+0.351 (n=226)
...
- **PATRÓN** `libro_liquidez` > `913.9984` → IC=+0.245 (n=300)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 913.9984 (IC base=+0.235)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.200 (n=1564)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.228 (n=3180)
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.293 (n=274)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.289 (n=353)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.74 (IC base=+0.246)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.235 (n=262)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.254 (n=283)
...
- **PATRÓN** `lag_apertura_s` < `5.56` → IC=+0.206 (n=539)
...
- **PATRÓN** `lag_apertura_s` < `7.33` → IC=+0.191 (n=267)

  - _Acción_: Kelly boost +0.96€ cuando `lag_apertura_s` < 7.33 (IC base=+0.155)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.146 (n=761)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `profundidad_ratio_no` > `12.3` → IC=+0.166 (n=273)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `profundidad_ratio_no` > 12.3 (IC base=+0.142)
...
- **PATRÓN** `ibs_20min` > `0.9866` → IC=+0.263 (n=420)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9866 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` > `0.5913` → IC=+0.308 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5913 (IC base=+0.102)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.749` → IC=+0.237 (n=602)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.749 (IC base=+0.102)
...
- **PATRÓN** `volumen_regimen` > `0.6419` → IC=+0.240 (n=156)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6419 (IC base=+0.102)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.508` → IC=+0.345 (n=153)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.508 (IC base=+0.123)
...
- **PATRÓN** `libro_spread` < `0.06` → IC=+0.149 (n=405)
...
- **PATRÓN** `ballena_activa_n` < `27.0` → IC=+0.148 (n=52)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `ballena_activa_n` < 27.0 (IC base=+0.129)
...
- **PATRÓN** `ibs_20min` > `0.9429` → IC=+0.229 (n=541)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.2` → IC=+0.127 (n=1001)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 2.2 (IC base=+0.051)

- **PATRÓN** `volumen_pendiente_norm` > `0.3377` → IC=+0.195 (n=103)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.3377 (IC base=+0.051)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.361` → IC=+0.172 (n=123)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 3.361 (IC base=-0.015)
...
- **PATRÓN** `volumen_regimen` < `1.0528` → IC=+0.139 (n=34)
  ...
  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` < 1.0528 (IC base=-0.012)
...
- **PATRÓN** `sigma_h` > `0.0066` → IC=+0.192 (n=575)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0066 (IC base=+0.138)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.145 (n=424)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9206` → IC=+0.248 (n=845)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.947` → IC=+0.264 (n=622)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1112` → IC=+0.156 (n=420)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_pendiente_norm` > 0.1112 (IC base=+0.138)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.144 (n=1428)

  - _Acción_: Kelly boost +0.72€ cuando `libro_spread` < 0.06 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `2682.2064` → IC=+0.175 (n=423)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 2682.2064 (IC base=+0.138)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.220 (n=1213)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.213)

- **PATRÓN** `drift_60min` |x|≤ `0.2382` → IC=+0.222 (n=1068)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2382 (IC base=+0.213)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.257 (n=553)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_spike_ratio` < `2.0479` → IC=+0.232 (n=408)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0479 (IC base=+0.213)

- **PATRÓN** `volumen_spike_ratio` > `3.1117` → IC=+0.238 (n=204)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.1117 (IC base=+0.213)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.272 (n=77)
...
- **PATRÓN** `ibs_20min` > `0.8817` → IC=+0.283 (n=210)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8817 (IC base=+0.151)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.61` → IC=+0.369 (n=120)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.61 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` > `0.1505` → IC=+0.143 (n=68)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_pendiente_norm` > 0.1505 (IC base=+0.151)

- **PATRÓN** `libro_spread` < `0.08` → IC=+0.168 (n=341)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.08 (IC base=+0.151)
...
- **PATRÓN** `sigma_h` > `0.002` → IC=+0.188 (n=203)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.002 (IC base=+0.183)

- **PATRÓN** `drift_60min` |x|≤ `0.1925` → IC=+0.202 (n=199)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1925 (IC base=+0.183)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.218 (n=214)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.413` → IC=+0.274 (n=82)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.413 (IC base=+0.183)

- **PATRÓN** `volumen_regimen` < `0.6357` → IC=+0.218 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6357 (IC base=+0.183)

- **PATRÓN** `volumen_regimen` > `0.6916` → IC=+0.186 (n=202)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` > 0.6916 (IC base=+0.183)

- **PATRÓN** `volumen_pendiente_norm` < `0.1932` → IC=+0.205 (n=110)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1932 (IC base=+0.183)

- **PATRÓN** `volumen_pendiente_norm` > `0.136` → IC=+0.261 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.136 (IC base=+0.183)

- **PATRÓN** `volumen_spike_ratio` < `2.5404` → IC=+0.248 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5404 (IC base=+0.183)

- **PATRÓN** `libro_liquidez` > `11393.2913` → IC=+0.218 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11393.2913 (IC base=+0.183)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1417` → IC=+0.198 (n=147)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.1417 (IC base=+0.191)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.214 (n=222)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.191)

- **PATRÓN** `ibs_20min` < `0.3172` → IC=+0.257 (n=220)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3172 (IC base=+0.191)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.088` → IC=+0.250 (n=82)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.088 (IC base=+0.191)

- **PATRÓN** `volumen_regimen` < `1.2412` → IC=+0.221 (n=220)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2412 (IC base=+0.191)

- **PATRÓN** `volumen_pendiente_norm` < `0.0843` → IC=+0.226 (n=93)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0843 (IC base=+0.191)

- **PATRÓN** `volumen_spike_ratio` < `1.9357` → IC=+0.287 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9357 (IC base=+0.191)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.191 (n=254)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.251` → IC=+0.230 (n=1032)
...
- **PATRÓN** `volumen_spike_ratio` > `1.6912` → IC=+0.121 (n=1101)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_spike_ratio` > 1.6912 (IC base=+0.118)

- **PATRÓN** `libro_liquidez` > `2726.373` → IC=+0.152 (n=513)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 2726.373 (IC base=+0.118)

- **PATRÓN** `ballena_activa_n` < `147.0` → IC=+0.193 (n=161)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 147.0 (IC base=+0.118)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.234 (n=1192)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.225)

- **PATRÓN** `sigma_h` > `0.0038` → IC=+0.231 (n=1353)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0038 (IC base=+0.225)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.236 (n=1232)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.044` → IC=+0.257 (n=269)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.044 (IC base=+0.225)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.483` → IC=+0.226 (n=1266)
...
- **PATRÓN** `volumen_regimen` > `1.0607` → IC=+0.219 (n=415)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0607 (IC base=+0.225)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0108` → IC=+0.274 (n=444)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0108 (IC base=+0.225)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.133 (n=390)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.208` → IC=+0.351 (n=146)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.208 (IC base=+0.131)

- **PATRÓN** `volumen_pendiente_norm` > `0.2113` → IC=+0.155 (n=56)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_pendiente_norm` > 0.2113 (IC base=+0.131)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.163 (n=277)
...
- **PATRÓN** `dist_vwap_pct` < `0.135` → IC=+0.188 (n=254)
  ...
  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.135 (IC base=+0.173)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.314 (n=154)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1458` → IC=+0.170 (n=222)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` < 0.1458 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` < `2.1497` → IC=+0.157 (n=106)
...
- **PATRÓN** `volumen_spike_ratio` > `4.0128` → IC=+0.167 (n=109)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 4.0128 (IC base=+0.171)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.189 (n=339)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.06 (IC base=+0.171)

- **PATRÓN** `libro_liquidez` > `1854.121` → IC=+0.192 (n=209)
...
- **PATRÓN** `drift_60min` |x|≤ `0.056` → IC=+0.167 (n=37)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.056 (IC base=+0.146)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.158 (n=115)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1931` → IC=+0.262 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1931 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.578` → IC=+0.259 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.578 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` < `0.8857` → IC=+0.204 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8857 (IC base=+0.146)

- **PATRÓN** `volumen_pendiente_norm` < `0.1407` → IC=+0.333 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1407 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` < `2.2709` → IC=+0.289 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.2709 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` > `1.7529` → IC=+0.295 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7529 (IC base=+0.146)

- **PATRÓN** `libro_liquidez` > `6213.1923` → IC=+0.218 (n=37)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.167 (n=16)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 5.0 (IC base=+0.050)

- **PATRÓN** `drift_20min_pct` |x|≤ `0.0549` → IC=+0.227 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `drift_20min_pct` |x|≤ 0.0549 (IC base=+0.050)

- **PATRÓN** `ibs_20min` > `0.1071` → IC=+0.206 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.1071 (IC base=+0.050)

- **PATRÓN** `libro_liquidez` > `19918.5721` → IC=+0.136 (n=20)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 19918.5721 (IC base=+0.050)

### MOMENTUM_IBS_15M#DOGE#15min
- **FILTRO** `ibs_20min` > `0.8701` → IC=-0.182 (n=20)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.204 (n=25)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.109)

- **PATRÓN** `drift_20min_pct` |x|≤ `0.0536` → IC=+0.237 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `drift_20min_pct` |x|≤ 0.0536 (IC base=+0.109)

- **PATRÓN** `libro_liquidez` > `15301.6756` → IC=+0.184 (n=17)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 15301.6756 (IC base=+0.109)

### MOMENTUM_IBS_15M_BALLENA
- **FILTRO** `py_entrada` < `0.5` → IC=-0.201 (n=175)

  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=208)

- **FILTRO** `ibs_20min` < `0.7405` → IC=-0.180 (n=95)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7405
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=288)

- **FILTRO** `ballena_activa_n` > `14.0` → IC=-0.188 (n=94)

  - _Acción_: SKIP cuando `ballena_activa_n` > 14.0
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=289)
...
- **FILTRO** `ibs_20min` < `0.7593` → IC=-0.227 (n=20)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.214 (n=26)
...
- **FILTRO** `libro_liquidez` < `15830.4808` → IC=-0.136 (n=42)
...
- **FILTRO** `py_entrada` < `0.42` → IC=-0.265 (n=15)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.206 (n=32)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.5 (IC base=+0.066)

- **PATRÓN** `drift_20min_pct` |x|≤ `0.1182` → IC=+0.125 (n=54)

  - _Acción_: Kelly boost +0.62€ cuando `drift_20min_pct` |x|≤ 0.1182 (IC base=+0.066)
...
- **FILTRO** `py_entrada` < `0.495` → IC=-0.241 (n=25)

  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=38)

- **FILTRO** `ibs_20min` < `0.7358` → IC=-0.265 (n=15)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7358
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=48)

- **FILTRO** `ballena_activa_n` > `14.0` → IC=-0.265 (n=15)

  - _Acción_: SKIP cuando `ballena_activa_n` > 14.0
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=48)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.505` → IC=-0.173 (n=47)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.174 (n=225)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=835)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.310 (n=251)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=809)

- **FILTRO** `ibs_7min` < `0.7368` → IC=-0.238 (n=265)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7368
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=795)

- **FILTRO** `ballena_activa_n` > `23.0` → IC=-0.212 (n=262)

  - _Acción_: SKIP cuando `ballena_activa_n` > 23.0
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=798)

- **FILTRO** `libro_liquidez` < `2827.2406` → IC=-0.139 (n=699)

  - _Acción_: SKIP cuando `libro_liquidez` < 2827.2406
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=361)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.219 (n=261)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=852)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.32` → IC=-0.225 (n=38)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.105 (n=127)

- **FILTRO** `ibs_7min` < `0.8571` → IC=-0.187 (n=81)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.192 (n=50)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=151)

- **FILTRO** `ibs_7min` < `0.8889` → IC=-0.191 (n=66)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.289 (n=69)
...
- **FILTRO** `py_entrada` > `0.5` → IC=-0.149 (n=132)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.181 (n=70)
  ...
  - _Acción_: Kelly boost +0.90€ cuando `py_entrada` < 0.5 (IC base=-0.034)
...
- **PATRÓN** `drift_7min_pct` |x|≤ `0.0404` → IC=+0.147 (n=32)

  - _Acción_: Kelly boost +0.74€ cuando `drift_7min_pct` |x|≤ 0.0404 (IC base=+0.061)

- **PATRÓN** `libro_liquidez` > `10993.5371` → IC=+0.135 (n=72)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 10993.5371 (IC base=+0.061)
...
- **FILTRO** `py_entrada` < `0.495` → IC=-0.147 (n=32)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.038 (n=392)
...
- **PATRÓN** `ibs_15` < `0.4545` → IC=+0.126 (n=364)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.63€ cuando `ibs_15` < 0.4545 (IC base=+0.078)

- **PATRÓN** `dist_vwap_pct` > `0.5709` → IC=+0.175 (n=38)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.5709 (IC base=+0.078)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0193` → IC=+0.129 (n=60)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.65€ cuando `pct_spot_vs_ref` |x|≤ 0.0193 (IC base=+0.087)

- **PATRÓN** `sigma_h` < `0.0019` → IC=+0.281 (n=30)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0019 (IC base=+0.087)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.001` → IC=+0.241 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.001 (IC base=+0.087)

- **PATRÓN** `libro_liquidez` > `11535.0306` → IC=+0.145 (n=60)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 11535.0306 (IC base=+0.087)
...
- **PATRÓN** `sigma_h` < `0.0033` → IC=+0.174 (n=44)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0033 (IC base=+0.144)

- **PATRÓN** `sigma_h` > `0.004` → IC=+0.210 (n=67)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.004 (IC base=+0.144)

- **PATRÓN** `drift_15min` |x|≤ `0.7408` → IC=+0.144 (n=88)

  - _Acción_: Kelly boost +0.72€ cuando `drift_15min` |x|≤ 0.7408 (IC base=+0.144)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1155` → IC=+0.156 (n=91)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **PATRÓN** `ibs_15` < `0.3247` → IC=+0.167 (n=100)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.83€ cuando `ibs_15` < 0.3247 (IC base=+0.144)

- **PATRÓN** `ibs_15` > `0.0304` → IC=+0.186 (n=100)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.93€ cuando `ibs_15` > 0.0304 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `0.4732` → IC=+0.265 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4732 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` < `23.05` → IC=+0.203 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 23.05 (IC base=+0.144)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0331` → IC=+0.135 (n=61)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.67€ cuando `pct_spot_vs_ref` |x|≤ 0.0331 (IC base=+0.102)

- **PATRÓN** `sigma_h` < `0.0021` → IC=+0.326 (n=21)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0021 (IC base=+0.102)

- **PATRÓN** `drift_60min` |x|≤ `0.3533` → IC=+0.135 (n=61)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.67€ cuando `drift_60min` |x|≤ 0.3533 (IC base=+0.102)

- **PATRÓN** `drift_15min` |x|≤ `0.513` → IC=+0.167 (n=61)

  - _Acción_: Kelly boost +0.83€ cuando `drift_15min` |x|≤ 0.513 (IC base=+0.102)

- **PATRÓN** `hora_utc` < `2.0` → IC=+0.152 (n=21)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 2.0 (IC base=+0.102)

- **PATRÓN** `ibs_15` > `0.0472` → IC=+0.151 (n=61)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.75€ cuando `ibs_15` > 0.0472 (IC base=+0.102)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.131` → IC=+0.127 (n=65)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 10.131 (IC base=+0.102)

- **PATRÓN** `ballena_activa_n` < `3.0` → IC=+0.186 (n=49)

  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 3.0 (IC base=+0.102)
...
- **PATRÓN** `sigma_h` < `0.005` → IC=+0.155 (n=27)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.005 (IC base=+0.049)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.130 (n=44)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` < 8.0 (IC base=+0.049)

- **PATRÓN** `ibs_15` < `0.1154` → IC=+0.293 (n=27)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.1154 (IC base=+0.049)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1698` → IC=+0.172 (n=56)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.1698 (IC base=+0.102)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0789` → IC=+0.143 (n=127)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.72€ cuando `delta_ratio_macro` |x|> 0.0789 (IC base=+0.102)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.137 (n=122)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` < 16.0 (IC base=+0.102)

- **PATRÓN** `ibs_15` < `0.1154` → IC=+0.224 (n=56)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.1154 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` > `0.1269` → IC=+0.225 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1269 (IC base=+0.102)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.393` → IC=+0.157 (n=106)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` < 5.393 (IC base=+0.102)

- **PATRÓN** `libro_liquidez` > `2603.9534` → IC=+0.147 (n=114)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 2603.9534 (IC base=+0.102)
...
- **PATRÓN** `ibs_15` > `0.3664` → IC=+0.152 (n=153)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.76€ cuando `ibs_15` > 0.3664 (IC base=-0.041)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.371` → IC=+0.208 (n=22)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.371 (IC base=-0.004)
...
- **H-IBS-UPDOWN_GBM#ETH#5min**: dentro de BUY_NO, IBS > 0.0472 sube el IC de +0.102 a +0.151 en UPDOWN_GBM#ETH#5min (n=61). Ya aplicado como kelly_boost=+0.75€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#SOL#5min**: dentro de BUY_NO, IBS < 0.1154 sube el IC de +0.049 a +0.293 en UPDOWN_GBM#SOL#5min (n=27). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_NO, IBS < 0.3247 sube el IC de +0.144 a +0.167 en UPDOWN_GBM#ETH#15min (n=100). Ya aplicado como kelly_boost=+0.83€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_NO, IBS > 0.0304 sube el IC de +0.144 a +0.186 en UPDOWN_GBM#ETH#15min (n=100). Ya aplicado como kelly_boost=+0.93€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1154 sube el IC de +0.102 a +0.224 en UPDOWN_GBM#XRP#15min (n=56). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.3664 sube el IC de -0.041 a +0.152 en UPDOWN_GBM_15M_TARDIO (n=153). Ya aplicado como kelly_boost=+0.76€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 3380 | -0.113 | -533.06€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 460 | -0.024 | -17.86€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 2920 | -0.127 | -515.20€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 370 | -0.185 | -102.97€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min |
79€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#BNB#15min | 42 | -0.045 | -2.79€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#BTC | 45 | -0.160 | -7.95€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#BTC#15min | 45 | -0.160 | -7.95€ | 1 | 0 |
...
| ✅ MOMENTUM_IBS_15M_FADE#SOL | 65 | -0.037 | -3.04€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#SOL#15min | 65 | -0.037 | -3.04€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 2173 | -0.071 | +23.23€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 2173 | -0.071 | +23.23€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 324 | -0.095 | +3.41€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 324 | -0.095 | +3.41€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 412 | -0.063 | +76.81€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 412 | -0.063 | +76.81€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 336 | -0.077 | -19.97€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 336 | -0.077 | -19.97€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 358 | -0.092 | -38.12€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 358 | -0.092 | -38.12€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 399 | -0.034 | +7.74€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 399 | -0.034 | +7.74€ | 3 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 344 | -0.069 | -6.65€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 344 | -0.069 | -6.65€ | 5 | 1 |
| ✅ MOMENTUM_IBS_5M_FADE | 1999 | +0.019 | +22.02€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 1999 | +0.019 | +22.02€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB | 341 | +0.004 | +3.16€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB#5min | 341 | +0.004 | +3.16€ | 1 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC | 287 | +0.043 | +9.39€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC#5min | 287 | +0.043 | +9.39€ | 1 | 2 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE | 345 | +0.022 | +3.66€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE#5min | 345 | +0.022 | +3.66€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH | 345 | +0.016 | +4.26€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH#5min | 345 | +0.016 | +4.26€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL | 349 | +0.024 | +5.45€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL#5min | 349 | +0.024 | +5.45€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP | 332 | +0.006 | -3.89€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP#5min | 332 | +0.006 | -3.89€ | 0 | 0 |
...
| ✅ STRUCT_NO_15M | 1871 | +0.016 | -2.76€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 1871 | +0.016 | -2.76€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 713 | +0.008 | -7.67€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 713 | +0.008 | -7.67€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 734 | +0.020 | +1.76€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 734 | +0.020 | +1.76€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 424 | +0.024 | +3.14€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 424 | +0.024 | +3.14€ | 2 | 0 |
| ✅ UPDOWN_GBM | 2221 | +0.025 | +152.40€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 968 | +0.087 | +170.61€ | 1 | 6 |
| ✅ UPDOWN_GBM#240min | 119 | +0.021 | +0.93€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 836 | -0.017 | -8.82€ | 4 | 0 |
...
| ✅ UPDOWN_GBM#ETH | 514 | +0.060 | +60.31€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 225 | +0.126 | +53.25€ | 1 | 14 |
| ✅ UPDOWN_GBM#ETH#240min | 36 | +0.053 | +0.92€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 136 | +0.022 | +7.41€ | 4 | 8 |
| ✅ UPDOWN_GBM#ETH#60min | 102 | +0.010 | -0.89€ | 0 | 2 |
| 🚫 UPDOWN_GBM#ETH#daily | 15 | -0.154 | -0.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 399 | -0.006 | +3.23€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 170 | +0.017 | +2.94€ | 1 | 2 |
| ✅ UPDOWN_GBM#SOL#240min | 29 | -0.016 | -0.92€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 124 | -0.008 | +0.52€ | 3 | 3 |
| ✅ UPDOWN_GBM#SOL#60min | 64 | -0.015 | +0.81€ | 2 | 0 |
| 🚫 UPDOWN_GBM#SOL#daily | 12 | -0.129 | -0.13€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 492 | +0.026 | +62.28€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 297 | +0.095 | +84.66€ | 0 | 14 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1673 | -0.048 | +173.09€ | 2 | 3 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1673 | -0.048 | +173.09€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 115 | -0.081 | -5.95€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 115 | -0.081 | -5.95€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 538 | -0.011 | +106.68€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 538 | -0.011 | +106.68€ | 1 | 1 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 519 | -0.034 | +69.48€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 519 | -0.034 | +69.48€ | 3 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.137) — sin ventaja clara. oversold(IBS<0.3): IC=-0.017 n=775 | neutral: IC=+0.011 n=747 | overbought(IBS>0.7): IC=+0.119 n=999
  - _Datos_: n=2706 IC=+0.045 PNL=+237.78€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=14h: IC=+0.109 n=108 PNL=+26.16€ → BOOST | H=19h: IC=+0.121 n=114 PNL=+35.45€ → BOOST | H=23h: IC=-0.149 n=112 PNL=-29.64€ → FILTRAR
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Estado_: SOL#15min: n≥40 pero IC=+0.017 < 0.08 — monitorear
  - _Datos_: n=170 IC=+0.017 PNL=+2.94€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.104 n=39113 | tras_1loss IC=+0.060 n=27620 | tras_2loss IC=+0.020 n=12090/40 | gap=+0.084 (umbral 0.05)
...
**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  ...
  - _Estado_: n_overlaps=15, boost estimado=+0.028. Necesita 5 más y boost>0.05
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=+0.081 n=172 | contrario_BTC IC=-0.031 n=96/40 | gap=-0.111 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2037 IC=+0.019 PNL=+110.21€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2037 IC=+0.019 PNL=+110.21€
...
**〰️ H-CUSTOM-GBM-SIGMA-BAJO** — GBM con sigma_h muy bajo (<0.0018/h, p1 real) — ¿mercado dormido = más predecible?
  ...
  - _Estado_: n=124 IC=+0.079 PNL=+27.48€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=124 IC=+0.079 PNL=+27.48€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=605 IC=+0.080 PNL=+78.35€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=605 IC=+0.080 PNL=+78.35€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=91 IC=+0.027 PNL=+11.13€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=91 IC=+0.027 PNL=+11.13€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=464 IC=+0.092 PNL=+111.80€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=464 IC=+0.092 PNL=+111.80€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.150 > 0.1 con n=561 PNL=+168.88€
  - _Datos_: n=561 IC=+0.150 PNL=+168.88€
...
**🟡 H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.102 > 0.1 con n=169 PNL=+50.64€
  - _Datos_: n=169 IC=+0.102 PNL=+50.64€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1359 IC=-0.148 PNL=+57.56€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1359 IC=-0.148 PNL=+57.56€
...
**🟡 H-CUSTOM-ETH15-BUYNO-TARDIO** — UPDOWN_GBM ETH#15min BUY_NO tardío (T_h<0.2) -- edge fuerte no capturado por el aprendizaje causal automático
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.372 > 0.08 con n=45 PNL=+38.69€
  - _Datos_: n=45 IC=+0.372 PNL=+38.69€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=1127 IC=+0.097 PNL=-254.87€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=1127 IC=+0.097 PNL=-254.87€
