# Hipótesis automáticas — 2026-07-12 12:10 UTC
_Generado por shadow_postmortem.py sobre 10884 resoluciones (PNL=+1912.69€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.223 (n=290)
...
- **PATRÓN** `py_entrada` > `0.585` → IC=+0.252 (n=337)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.585 (IC base=+0.219)

- **PATRÓN** `libro_liquidez` > `2363.0481` → IC=+0.235 (n=322)
...
### GBM_LATE_15M
- **PATRÓN** `sigma_h` > `0.0151` → IC=+0.136 (n=435)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0151 (IC base=+0.115)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.121 (n=1324)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` > 6.0 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` < `0.1365` → IC=+0.206 (n=66)
...
- **PATRÓN** `dist_vwap_pct` < `0.1111` → IC=+0.167 (n=22)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.1111 (IC base=+0.085)
...
- **PATRÓN** `dist_vwap_pct` < `0.1427` → IC=+0.209 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1427 (IC base=+0.167)

- **PATRÓN** `sigma_h` > `0.0045` → IC=+0.174 (n=354)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.184 (n=93)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 7.0 (IC base=+0.162)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.167 (n=100)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 18.0 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` < `0.1111` → IC=+0.182 (n=20)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.1111 (IC base=+0.162)
...
- **PATRÓN** `sigma_h` < `0.008` → IC=+0.250 (n=38)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.008 (IC base=+0.105)

- **PATRÓN** `sigma_h` > `0.0116` → IC=+0.125 (n=38)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` > 0.0116 (IC base=+0.105)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.174 (n=41)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` < 5.0 (IC base=+0.105)

- **PATRÓN** `dist_vwap_pct` < `0.1136` → IC=+0.274 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1136 (IC base=+0.105)
...
- **PATRÓN** `sigma_h` > `0.0127` → IC=+0.155 (n=137)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0127 (IC base=+0.083)

- **PATRÓN** `dist_vwap_pct` < `0.503` → IC=+0.138 (n=114)
...
- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.184 (n=36)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0083 (IC base=+0.042)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.122 (n=72)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` < 11.0 (IC base=+0.042)

- **PATRÓN** `dist_vwap_pct` < `0.1589` → IC=+0.239 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1589 (IC base=+0.042)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.125 (n=86)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=+0.103 (n=341)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.124 (n=235)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 13.0 (IC base=+0.018)

- **PATRÓN** `ibs_15` > `0.7177` → IC=+0.153 (n=214)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.76€ cuando `ibs_15` > 0.7177 (IC base=+0.018)

- **PATRÓN** `dist_vwap_pct` > `0.1439` → IC=+0.204 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1439 (IC base=+0.018)

- **PATRÓN** `dist_vwap_pct` < `0.3774` → IC=+0.193 (n=99)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` < 0.3774 (IC base=+0.018)
...
- **PATRÓN** `drift_60min` |x|≤ `0.237` → IC=+0.125 (n=134)
  ...
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 0.237 (IC base=+0.058)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.169 (n=125)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 11.0 (IC base=+0.058)

- **PATRÓN** `ibs_15` < `0.9136` → IC=+0.152 (n=113)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.76€ cuando `ibs_15` < 0.9136 (IC base=+0.058)

- **PATRÓN** `ibs_15` > `0.6428` → IC=+0.170 (n=113)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.85€ cuando `ibs_15` > 0.6428 (IC base=+0.058)

- **PATRÓN** `dist_vwap_pct` > `0.2179` → IC=+0.333 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2179 (IC base=+0.058)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS > 0.7177 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.153 n=214). Confirma señal de reversión media → alinear con BUY_NO.
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS < 0.9136 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.152 n=113). Confirma señal de reversión media → alinear con BUY_YES.
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS > 0.6428 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.170 n=113). Confirma señal de reversión media → alinear con BUY_NO.
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 878 | +0.189 | +22.32€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 563 | +0.215 | +13.64€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 259 | +0.197 | +5.67€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 188 | +0.216 | -1.88€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 307 | +0.193 | +8.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 187 | +0.214 | +9.08€ | 0 | 8 |
...
| ✅ GBM_LATE_15M | 3570 | +0.112 | +1146.93€ | 0 | 6 |
| ✅ GBM_LATE_15M#15min | 3570 | +0.112 | +1146.93€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 880 | +0.069 | +106.96€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 880 | +0.069 | +106.96€ | 0 | 1 |
| ✅ GBM_LATE_15M#ETH | 905 | +0.071 | +133.80€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 905 | +0.071 | +133.80€ | 0 | 2 |
| ✅ GBM_LATE_15M#SOL | 929 | +0.137 | +451.12€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 929 | +0.137 | +451.12€ | 0 | 8 |
| ✅ GBM_LATE_15M#XRP | 856 | +0.172 | +455.05€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 856 | +0.172 | +455.05€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 932 | +0.161 | +480.62€ | 0 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 932 | +0.161 | +480.62€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 245 | +0.168 | +154.29€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 245 | +0.168 | +154.29€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 251 | +0.121 | +78.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 251 | +0.121 | +78.64€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 262 | +0.114 | +76.03€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 262 | +0.114 | +76.03€ | 0 | 6 |
...
| ✅ GBM_LATE_15M_TARDIO | 1105 | +0.079 | +252.67€ | 1 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1105 | +0.079 | +252.67€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 256 | +0.074 | +26.55€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 256 | +0.074 | +26.55€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 276 | +0.000 | +1.81€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 276 | +0.000 | +1.81€ | 0 | 1 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 287 | +0.036 | +32.78€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 287 | +0.036 | +32.78€ | 0 | 3 |
...
| ✅ UPDOWN_GBM | 1317 | -0.006 | +22.43€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1061 | +0.023 | +59.89€ | 1 | 5 |
...
| ✅ UPDOWN_GBM#BTC | 354 | +0.011 | +4.08€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 275 | +0.052 | +17.46€ | 1 | 8 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.122) — sin ventaja clara. oversold(IBS<0.3): IC=-0.021 n=340 | neutral: IC=-0.048 n=257 | overbought(IBS>0.7): IC=+0.074 n=289
  - _Datos_: n=927 IC=+0.008 PNL=+47.66€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.090 n=6134 | tras_1loss IC=+0.044 n=4613 | tras_2loss IC=+0.010 n=2076/40 | gap=+0.080 (umbral 0.05)
...
**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  ...
  - _Estado_: alineada_con_outcome_prev IC=+0.125 n=70/60 | contraria IC=+0.045 n=20 | gap=+0.080 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1190 IC=+0.000 PNL=+36.43€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1190 IC=+0.000 PNL=+36.43€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=602 IC=+0.018 PNL=+24.34€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=602 IC=+0.018 PNL=+24.34€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=275 IC=+0.052 PNL=+17.46€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=275 IC=+0.052 PNL=+17.46€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=545 IC=+0.052 PNL=+41.21€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=545 IC=+0.052 PNL=+41.21€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8426 IC=+0.096 PNL=+1988.57€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8426 IC=+0.096 PNL=+1988.57€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=749 IC=-0.015 PNL=-6.13€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=749 IC=-0.015 PNL=-6.13€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.177 > 0.1 con n=264 PNL=+97.53€
  - _Datos_: n=264 IC=+0.177 PNL=+97.53€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.180 > 0.08 con n=198 PNL=+70.58€
  - _Datos_: n=198 IC=+0.180 PNL=+70.58€
...
**🔶 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA en BTC (IC=+0.241 n=83) pero sin cruzar ≥2 pares más — ETH: n=103 IC=+0.148 ✓; SOL: n=7 IC=-0.019; XRP: n=5 IC=+0.018
  - _Datos_: n=83 IC=+0.241 PNL=+33.83€
