# Hipótesis automáticas — 2026-07-02 13:09 UTC
_Generado por shadow_postmortem.py sobre 2662 resoluciones (PNL=-63.63€)_
...
- **PATRÓN** `sigma_h` < `0.0101` → IC=+0.222 (n=70)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0101 (IC base=+0.047)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.150 (n=38)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 14.0 (IC base=+0.047)

- **PATRÓN** `ibs_15` < `0.0669` → IC=+0.200 (n=48)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0669 (IC base=+0.047)
...
- **PATRÓN** `hora_utc` > `13.0` → IC=+0.158 (n=36)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 13.0 (IC base=-0.030)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1132` → IC=+0.167 (n=46)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.83€ cuando `pct_spot_vs_ref` |x|≤ 0.1132 (IC base=+0.135)

- **PATRÓN** `sigma_h` < `0.0144` → IC=+0.167 (n=46)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0144 (IC base=+0.135)

- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.136 (n=31)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0082 (IC base=+0.135)

- **PATRÓN** `drift_60min` |x|≤ `0.8591` → IC=+0.141 (n=37)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.8591 (IC base=+0.135)

- **PATRÓN** `drift_15min` |x|≤ `0.688` → IC=+0.145 (n=29)

  - _Acción_: Kelly boost +0.73€ cuando `drift_15min` |x|≤ 0.688 (IC base=+0.135)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.237 (n=17)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.135)

- **PATRÓN** `ibs_15` < `0.0787` → IC=+0.333 (n=22)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0787 (IC base=+0.135)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS < 0.0669 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.200 n=48). Confirma señal de reversión media → alinear con BUY_YES.
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.0787 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.333 n=22). Confirma señal de reversión media → alinear con BUY_YES.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 90 | +0.152 | +26.28€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 90 | +0.152 | +26.28€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 24 | +0.077 | +2.63€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 24 | +0.077 | +2.63€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 24 | +0.077 | +2.86€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 24 | +0.077 | +2.86€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL | 20 | +0.227 | +10.02€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 20 | +0.227 | +10.02€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP | 22 | +0.208 | +10.77€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 22 | +0.208 | +10.77€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 810 | -0.036 | -38.83€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 585 | -0.013 | -12.49€ | 2 | 3 |
| 🚫 UPDOWN_GBM#240min | 11 | -0.148 | -4.31€ | 0 | 0 |
| 🚫 UPDOWN_GBM#5min | 75 | -0.162 | -22.48€ | 4 | 0 |
| ✅ UPDOWN_GBM#60min | 122 | -0.057 | -10.32€ | 5 | 0 |
| ✅ UPDOWN_GBM#BNB | 23 | -0.140 | -6.76€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 23 | -0.140 | -6.76€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 235 | -0.040 | -30.93€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 165 | -0.015 | -20.73€ | 2 | 1 |
| 🚫 UPDOWN_GBM#BTC#5min | 19 | -0.158 | -7.19€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#60min | 42 | -0.068 | -6.16€ | 2 | 0 |
| ✅ UPDOWN_GBM#BTC#daily | 5 | +0.054 | +5.61€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 26 | -0.107 | -5.20€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 23 | -0.100 | -4.31€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 312 | -0.003 | +10.61€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 229 | +0.015 | +16.57€ | 2 | 7 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.081) — sin ventaja clara. oversold(IBS<0.3): IC=+0.014 n=140 | neutral: IC=-0.067 n=125 | overbought(IBS>0.7): IC=-0.045 n=141
  - _Datos_: n=420 IC=-0.031 PNL=-13.59€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.011 n=1273 | tras_1loss IC=-0.034 n=1301 | tras_2loss IC=-0.047 n=673/40 | gap=+0.057 (umbral 0.05)
...
**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  ...
  - _Estado_: n_overlaps=23, boost estimado=+0.009. Necesita 0 más y boost>0.05
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=-0.057 n=104/40 PNL=-6.08€ | H=15h UTC: IC=+0.000 n=98/40 PNL=+1.41€ | H=17h UTC: IC=+0.221 n=41/40 PNL=+14.13€ | H=19h UTC: IC=+0.028 n=51/40 PNL=+1.21€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=687 IC=-0.030 PNL=-25.28€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=687 IC=-0.030 PNL=-25.28€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=373 IC=-0.047 PNL=-39.78€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=373 IC=-0.047 PNL=-39.78€
...
**🔴 H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.015 < 0.02 con n=165 PNL=-20.73€
  - _Datos_: n=165 IC=-0.015 PNL=-20.73€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=245 IC=-0.006 PNL=-24.75€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=245 IC=-0.006 PNL=-24.75€
...
**〰️ H-CUSTOM-ETH15-REVERSION** — ETH#15min con drift_15min < -1 — ¿mean reversion?
  ...
  - _Estado_: n=35 IC=+0.068 PNL=+7.26€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=35 IC=+0.068 PNL=+7.26€
...
**⏳ H-LATE-WINDOW-5MIN** — Late-window BTC 5min — arbitraje timing vs Polymarket
  ...
  - _Estado_: 2/30 ops en el filtro definido (IC actual=-0.025 PNL=-1.02€)
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=194 IC=+0.020 PNL=+15.63€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=194 IC=+0.020 PNL=+15.63€
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=245 IC=+0.063 PNL=+29.29€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=245 IC=+0.063 PNL=+29.29€
...
**⏳ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: 29/40 ops en el filtro definido (IC actual=+0.016 PNL=-4.62€)
  - _Datos_: n=29 IC=+0.016 PNL=-4.62€
...
**🔴 H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.072 < -0.05 con n=515 PNL=-67.70€
  - _Datos_: n=515 IC=-0.072 PNL=-67.70€
