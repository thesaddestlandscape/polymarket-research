# Hipótesis automáticas — 2026-06-29 20:31 UTC
_Generado por shadow_postmortem.py sobre 1313 resoluciones (PNL=-31.02€)_

## Patrones causales activos

### UPDOWN_GBM#BTC#15min
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0921` → IC=+0.182 (n=20)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.91€ cuando `pct_spot_vs_ref` |x|≤ 0.0921 (IC base=+0.057)

- **PATRÓN** `sigma_h` < `0.0125` → IC=+0.134 (n=39)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.0125 (IC base=+0.057)

### UPDOWN_GBM#ETH#15min
- **PATRÓN** `sigma_h` < `0.0155` → IC=+0.127 (n=57)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.64€ cuando `sigma_h` < 0.0155 (IC base=+0.080)

- **PATRÓN** `drift_60min` |x|≤ `0.4113` → IC=+0.125 (n=38)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 0.4113 (IC base=+0.080)

- **PATRÓN** `drift_15min` |x|≤ `0.5582` → IC=+0.130 (n=25)

  - _Acción_: Kelly boost +0.65€ cuando `drift_15min` |x|≤ 0.5582 (IC base=+0.080)
...
| ✅ UPDOWN_GBM | 344 | +0.017 | +10.42€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 201 | +0.062 | +14.37€ | 0 | 0 |
| 🚫 UPDOWN_GBM#240min | 12 | -0.171 | -4.82€ | 0 | 0 |
| 🚫 UPDOWN_GBM#5min | 56 | -0.155 | -16.64€ | 0 | 0 |
| ✅ UPDOWN_GBM#60min | 62 | +0.109 | +11.25€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 107 | +0.023 | +1.65€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 59 | +0.057 | +1.83€ | 0 | 2 |
| ✅ UPDOWN_GBM#BTC#240min | 5 | -0.089 | -2.96€ | 0 | 0 |
| 🚫 UPDOWN_GBM#BTC#5min | 16 | -0.133 | -6.30€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#60min | 23 | +0.140 | +4.50€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 132 | +0.060 | +9.58€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 86 | +0.080 | +6.39€ | 0 | 3 |
| ✅ UPDOWN_GBM#ETH#240min | 5 | -0.018 | -0.44€ | 0 | 0 |
| 🚫 UPDOWN_GBM#ETH#5min | 12 | -0.086 | -3.67€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 25 | +0.130 | +5.43€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 74 | -0.040 | +0.68€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 38 | +0.025 | +3.96€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.156: neutral→boost, oversold→filtro | oversold(IBS<0.3): IC=+0.022 n=15 | neutral: IC=+0.178 n=16 | overbought(IBS>0.7): IC=+0.083 n=22
  - _Datos_: n=55 IC=+0.114 PNL=+9.29€
...
**⏳ H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  ...
  - _Estado_: 55 ops, 10 horas distintas. Sin hora con n≥15 y IC extremo aún.
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Estado_: ETH#60min: n=25/40 IC=+0.130 PNL=+5.43€ | BTC#60min: n=23/40 IC=+0.140 PNL=+4.50€ | SOL#60min: n=14/40 IC=+0.000 PNL=+1.32€

**⏳ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n=38/40 IC=+0.025 PNL=+3.96€ (ETA: 2 ops)
  - _Datos_: n=38 IC=+0.025 PNL=+3.96€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=245 IC=+0.043 PNL=+16.06€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=245 IC=+0.043 PNL=+16.06€
...
**🟡 H-CUSTOM-GBM-60MIN-BUYYES** — GBM 60min BUY_YES — ¿edge superior al BUY_NO?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.112 > 0.08 con n=47 PNL=+10.29€
  - _Datos_: n=47 IC=+0.112 PNL=+10.29€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=150 IC=+0.040 PNL=+1.20€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=150 IC=+0.040 PNL=+1.20€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=59 IC=+0.057 PNL=+1.83€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=59 IC=+0.057 PNL=+1.83€

**⏳ H-CUSTOM-DRIFT15-ZONA-MUERTA** — GBM#15min drift_15min ∈ [-0.3,+0.3] — zona muerta de señal
  - _Hipótesis_: Análisis n=127 GBM#15min: cuando drift_15min está entre -0.3 y +0.3 (mercado sin dirección clara) el IC es negativo (-0.043). Cuando drift>0.3 IC=+0.100 (n=28). Cuando drift<-1 IC=+0.048 (reversión). La señal requiere mercado con dirección clara.
  - _Umbral_: 50
  - _Acción_: Filtrar señales GBM#15min cuando drift_15min ∈ [-0.3, +0.3] — validar con n≥50 antes de implementar
  - _Estado_: 26/50 ops en el filtro definido (IC actual=+0.036 PNL=+0.40€)
  - _Datos_: n=26 IC=+0.036 PNL=+0.40€

**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  - _Hipótesis_: Cuando drift_15min > 0.3%/h el GBM captura bien la dirección: IC=+0.100 n=28 en todos GBM#15min; IC=+0.152 n=13 solo BTC. El mercado tiene dirección clara y el GBM la sigue. Hipótesis: este rango es donde la señal es real.
  - _Umbral_: n≥40 y IC>+0.08
  - _Acción_: Si se confirma IC>0.10 con n≥40 → boost ×1.2 en GBM#15min cuando drift_15min>0.3
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.145 > 0.08 con n=74 PNL=+12.16€
  - _Datos_: n=74 IC=+0.145 PNL=+12.16€
