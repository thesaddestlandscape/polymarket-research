# Hipótesis automáticas — 2026-07-12 20:47 UTC
_Generado por shadow_postmortem.py sobre 11410 resoluciones (PNL=+2199.99€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.230 (n=379)
...
- **PATRÓN** `py_entrada` > `0.585` → IC=+0.257 (n=397)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.585 (IC base=+0.225)

- **PATRÓN** `libro_liquidez` > `2891.3531` → IC=+0.244 (n=338)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2891.3531 (IC base=+0.225)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.234 (n=77)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.605` → IC=+0.250 (n=86)
...
- **PATRÓN** `sigma_h` > `0.009` → IC=+0.148 (n=947)
...
- **PATRÓN** `dist_vwap_pct` > `0.5914` → IC=+0.138 (n=45)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 0.5914 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.1905` → IC=+0.190 (n=98)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.1905 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.067` → IC=+0.151 (n=61)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` < 4.067 (IC base=+0.110)
...
- **PATRÓN** `sigma_h` < `0.0171` → IC=+0.183 (n=238)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0171 (IC base=+0.177)

- **PATRÓN** `sigma_h` > `0.0114` → IC=+0.188 (n=354)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0114 (IC base=+0.177)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.258 (n=130)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.177)

- **PATRÓN** `dist_vwap_pct` > `0.2854` → IC=+0.154 (n=24)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.2854 (IC base=+0.177)
...
- **PATRÓN** `sigma_h` > `0.0042` → IC=+0.178 (n=396)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.1625` → IC=+0.262 (n=99)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1625 (IC base=+0.153)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.075` → IC=+0.246 (n=61)
...
- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.253 (n=87)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0029 (IC base=+0.164)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.196 (n=44)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 15.0 (IC base=+0.164)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.171 (n=68)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` < 12.0 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.0976` → IC=+0.200 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.0976 (IC base=+0.164)
...
- **PATRÓN** `sigma_h` > `0.0088` → IC=+0.317 (n=69)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.0977` → IC=+0.292 (n=22)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0977 (IC base=+0.279)
...
- **PATRÓN** `sigma_h` < `0.0194` → IC=+0.203 (n=109)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0194 (IC base=+0.205)

- **PATRÓN** `sigma_h` > `0.0303` → IC=+0.267 (n=41)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0303 (IC base=+0.205)
...
- **PATRÓN** `dist_vwap_pct` > `0.2971` → IC=+0.180 (n=23)
  ...
  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.2971 (IC base=+0.205)

- **PATRÓN** `dist_vwap_pct` < `0.186` → IC=+0.188 (n=30)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.186 (IC base=+0.205)
...
- **PATRÓN** `ibs_15` > `0.7168` → IC=+0.161 (n=222)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.80€ cuando `ibs_15` > 0.7168 (IC base=+0.018)

- **PATRÓN** `dist_vwap_pct` > `0.1374` → IC=+0.181 (n=89)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.1374 (IC base=+0.018)

- **PATRÓN** `dist_vwap_pct` < `0.2737` → IC=+0.189 (n=88)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.2737 (IC base=+0.018)

- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.122 (n=117)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` < 0.0063 (IC base=+0.031)

- **PATRÓN** `ibs_15` < `0.0512` → IC=+0.134 (n=132)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.67€ cuando `ibs_15` < 0.0512 (IC base=+0.031)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS > 0.7168 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.161 n=222). Confirma señal de reversión media → alinear con BUY_NO.
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 1031 | +0.187 | +26.42€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 664 | +0.216 | +20.82€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 302 | +0.191 | +1.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 221 | +0.213 | -3.55€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 3690 | +0.114 | +1237.73€ | 0 | 10 |
| ✅ GBM_LATE_15M#15min | 3690 | +0.114 | +1237.73€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#XRP | 890 | +0.170 | +469.62€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 890 | +0.170 | +469.62€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1038 | +0.165 | +610.39€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1038 | +0.165 | +610.39€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 271 | +0.167 | +178.88€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 271 | +0.167 | +178.88€ | 0 | 8 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 199 | +0.271 | +196.51€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 199 | +0.271 | +196.51€ | 0 | 6 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 1186 | +0.082 | +298.04€ | 1 | 8 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1186 | +0.082 | +298.04€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 307 | +0.199 | +204.55€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 307 | +0.199 | +204.55€ | 0 | 9 |
...
| ✅ STREAK_FADE_5M | 167 | -0.038 | -10.54€ | 4 | 0 |
| ✅ STREAK_FADE_5M#5min | 167 | -0.038 | -10.54€ | 0 | 0 |
...
| ✅ STREAK_FADE_5M#XRP | 62 | +0.031 | +0.39€ | 0 | 0 |
| ✅ STREAK_FADE_5M#XRP#5min | 62 | +0.031 | +0.39€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 1340 | -0.005 | +27.27€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1084 | +0.024 | +64.74€ | 1 | 5 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.126) — sin ventaja clara. oversold(IBS<0.3): IC=-0.020 n=348 | neutral: IC=-0.047 n=265 | overbought(IBS>0.7): IC=+0.079 n=295
  - _Datos_: n=950 IC=+0.009 PNL=+52.50€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.093 n=6482 | tras_1loss IC=+0.049 n=4790 | tras_2loss IC=+0.015 n=2135/40 | gap=+0.078 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1213 IC=+0.001 PNL=+41.27€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1213 IC=+0.001 PNL=+41.27€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=619 IC=+0.018 PNL=+27.08€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=619 IC=+0.018 PNL=+27.08€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=564 IC=+0.053 PNL=+47.11€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=564 IC=+0.053 PNL=+47.11€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=434 IC=+0.014 PNL=+17.32€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=434 IC=+0.014 PNL=+17.32€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8952 IC=+0.100 PNL=+2275.87€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8952 IC=+0.100 PNL=+2275.87€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=766 IC=-0.014 PNL=-3.39€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=766 IC=-0.014 PNL=-3.39€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.170 > 0.1 con n=283 PNL=+101.78€
  - _Datos_: n=283 IC=+0.170 PNL=+101.78€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.168 > 0.08 con n=215 PNL=+73.33€
  - _Datos_: n=215 IC=+0.168 PNL=+73.33€
...
**🔶 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA en BTC (IC=+0.223 n=92) pero sin cruzar ≥2 pares más — ETH: n=111 IC=+0.137 ✓; SOL: n=7 IC=-0.019; XRP: n=5 IC=+0.018
...
**🔶 H-CUSTOM-ETH15-BUYNO-TARDIO** — UPDOWN_GBM ETH#15min BUY_NO tardío (T_h<0.2) -- edge fuerte no capturado por el aprendizaje causal automático
  ...
  - _Estado_: SEÑAL POSITIVA en ETH (IC=+0.262 n=40) pero sin cruzar ≥2 pares más — BNB: n=1 IC=-0.008; BTC: n=12 IC=+0.043; DOGE: n=2 IC=+0.000; SOL: n=8 IC=+0.000; XRP: n=5 IC=+0.018
