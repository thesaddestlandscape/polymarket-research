# Hipótesis automáticas — 2026-07-16 11:46 UTC
_Generado por shadow_postmortem.py sobre 16889 resoluciones (PNL=+3446.52€)_
...
- **PATRÓN** `py_entrada` > `0.695` → IC=+0.296 (n=424)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.178)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.184 (n=1075)

  - _Acción_: Kelly boost +0.92€ cuando `libro_spread` < 0.01 (IC base=+0.178)

- **PATRÓN** `libro_liquidez` > `4358.7773` → IC=+0.185 (n=614)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 4358.7773 (IC base=+0.178)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.197 (n=368)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 17.0 (IC base=+0.167)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.196 (n=436)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` < 7.0 (IC base=+0.167)

- **PATRÓN** `py_entrada` < `0.265` → IC=+0.331 (n=346)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.265 (IC base=+0.167)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.178 (n=1128)

  - _Acción_: Kelly boost +0.89€ cuando `libro_spread` < 0.01 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `7351.4655` → IC=+0.182 (n=328)
...
- **PATRÓN** `py_entrada` < `0.425` → IC=+0.237 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.425 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `7105.7782` → IC=+0.198 (n=51)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 7105.7782 (IC base=+0.167)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.224 (n=208)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `5233.5693` → IC=+0.217 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 5233.5693 (IC base=+0.218)
...
- **PATRÓN** `libro_liquidez` > `4118.7645` → IC=+0.214 (n=187)
...
- **PATRÓN** `libro_liquidez` > `5703.6318` → IC=+0.210 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 5703.6318 (IC base=+0.149)
...
- **PATRÓN** `libro_liquidez` > `1592.1422` → IC=+0.219 (n=197)
...
- **PATRÓN** `py_entrada` < `0.365` → IC=+0.270 (n=185)
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.125 (n=22)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 16.0 (IC base=+0.095)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `11.066` → IC=+0.226 (n=100)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.066 (IC base=+0.107)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.135 (n=1246)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0091 (IC base=+0.107)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.127 (n=1262)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` < 11.0 (IC base=+0.107)
...
- **PATRÓN** `dist_vwap_pct` < `0.4763` → IC=+0.123 (n=157)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.4763 (IC base=+0.062)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.206` → IC=+0.189 (n=59)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 3.206 (IC base=+0.062)
...
- **PATRÓN** `dist_vwap_pct` > `0.7215` → IC=+0.152 (n=44)
  ...
  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.7215 (IC base=+0.066)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.447` → IC=+0.189 (n=43)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 7.447 (IC base=+0.066)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.515` → IC=+0.222 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.515 (IC base=+0.108)

- **PATRÓN** `sigma_h` < `0.0114` → IC=+0.151 (n=313)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0114 (IC base=+0.132)

- **PATRÓN** `sigma_h` > `0.0092` → IC=+0.136 (n=470)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0092 (IC base=+0.132)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.168 (n=320)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 11.0 (IC base=+0.132)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.875` → IC=+0.128 (n=127)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` < 2.875 (IC base=+0.132)

### GBM_LATE_15M#XRP#15min
- **PATRÓN** `sigma_h` < `0.0311` → IC=+0.149 (n=448)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0311 (IC base=+0.149)

- **PATRÓN** `sigma_h` > `0.0244` → IC=+0.178 (n=203)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` > 0.0244 (IC base=+0.149)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.157 (n=164)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 17.0 (IC base=+0.149)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.156 (n=306)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` > `0.3145` → IC=+0.143 (n=54)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.3145 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` < `0.5085` → IC=+0.144 (n=192)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.5085 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.46` → IC=+0.140 (n=173)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` < 7.46 (IC base=+0.164)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.220 (n=334)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0108 (IC base=+0.155)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.173 (n=518)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 12.0 (IC base=+0.155)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.161 (n=768)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `11.007` → IC=+0.306 (n=91)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.007 (IC base=+0.155)

- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.179 (n=764)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` > 0.0048 (IC base=+0.163)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.165 (n=550)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.4154` → IC=+0.196 (n=547)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` < 0.4154 (IC base=+0.163)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.089` → IC=+0.260 (n=73)
...
- **PATRÓN** `sigma_h` > `0.0028` → IC=+0.172 (n=193)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0028 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.167 (n=76)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 16.0 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.4763` → IC=+0.133 (n=156)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.4763 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.139` → IC=+0.309 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.139 (IC base=+0.137)
...
- **PATRÓN** `sigma_h` < `0.0068` → IC=+0.144 (n=172)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` < 0.0068 (IC base=+0.134)

- **PATRÓN** `sigma_h` > `0.0037` → IC=+0.150 (n=195)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.7422` → IC=+0.189 (n=43)
  ...
  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.7422 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` < `0.3828` → IC=+0.172 (n=114)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.3828 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.997` → IC=+0.232 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.997 (IC base=+0.134)
...
- **PATRÓN** `dist_vwap_pct` < `0.9959` → IC=+0.143 (n=155)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.9959 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.061` → IC=+0.271 (n=68)
...
- **PATRÓN** `sigma_h` < `0.0172` → IC=+0.293 (n=172)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0172 (IC base=+0.291)

- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.321 (n=171)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0087 (IC base=+0.291)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.291 (n=127)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.5145` → IC=+0.311 (n=146)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5145 (IC base=+0.291)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.683` → IC=+0.314 (n=116)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.683 (IC base=+0.291)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0151` → IC=+0.198 (n=230)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0151 (IC base=+0.111)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.129 (n=607)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.1391` → IC=+0.136 (n=259)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.1391 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.256` → IC=+0.297 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.256 (IC base=+0.111)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.126 (n=303)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` < 6.0 (IC base=+0.091)

- **PATRÓN** `dist_vwap_pct` < `0.3632` → IC=+0.123 (n=518)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.3632 (IC base=+0.091)
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.120 (n=77)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` > 16.0 (IC base=+0.069)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.626` → IC=+0.152 (n=21)
  ...
  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 7.626 (IC base=+0.018)
...
- **PATRÓN** `sigma_h` > `0.0129` → IC=+0.144 (n=71)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` > 0.0129 (IC base=+0.084)
...
- **PATRÓN** `dist_vwap_pct` < `0.3862` → IC=+0.139 (n=70)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.3862 (IC base=+0.084)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.311` → IC=+0.333 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.311 (IC base=+0.084)
...
- **PATRÓN** `sigma_h` > `0.0117` → IC=+0.221 (n=202)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.229 (n=179)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.107` → IC=+0.220 (n=73)
...
- **PATRÓN** `dist_vwap_pct` < `0.4836` → IC=+0.188 (n=187)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.4836 (IC base=+0.199)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.923` → IC=+0.194 (n=168)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` < 7.923 (IC base=+0.199)
...
- **PATRÓN** `py_entrada` < `0.505` → IC=+0.172 (n=56)

  - _Acción_: Kelly boost +0.86€ cuando `py_entrada` < 0.505 (IC base=+0.116)

- **PATRÓN** `btc_momentum` |x|≤ `0.03` → IC=+0.130 (n=52)

  - _Acción_: Kelly boost +0.65€ cuando `btc_momentum` |x|≤ 0.03 (IC base=+0.116)
...
- **PATRÓN** `hora_utc` > `13.0` → IC=+0.191 (n=40)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 13.0 (IC base=+0.108)

- **PATRÓN** `py_entrada` < `0.505` → IC=+0.138 (n=45)

  - _Acción_: Kelly boost +0.69€ cuando `py_entrada` < 0.505 (IC base=+0.108)

- **PATRÓN** `py_entrada` > `0.515` → IC=+0.123 (n=51)

  - _Acción_: Kelly boost +0.61€ cuando `py_entrada` > 0.515 (IC base=+0.108)

- **PATRÓN** `streak_len` < `4.0` → IC=+0.177 (n=60)

  - _Acción_: Kelly boost +0.89€ cuando `streak_len` < 4.0 (IC base=+0.108)

- **PATRÓN** `regimen_ma_toques` > `3.0` → IC=+0.122 (n=35)

  - _Acción_: Kelly boost +0.61€ cuando `regimen_ma_toques` > 3.0 (IC base=+0.108)

- **PATRÓN** `volumen_racha` < `270220.1` → IC=+0.218 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_racha` < 270220.1 (IC base=+0.108)

- **PATRÓN** `libro_liquidez` > `1959.3298` → IC=+0.180 (n=48)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 1959.3298 (IC base=+0.108)
...
- **PATRÓN** `ibs_15` < `0.0266` → IC=+0.121 (n=143)
  ...
  - _Acción_: Kelly boost +0.60€ cuando `ibs_15` < 0.0266 (IC base=+0.046)

- **PATRÓN** `dist_vwap_pct` > `0.921` → IC=+0.125 (n=22)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` > 0.921 (IC base=+0.046)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.294` → IC=+0.126 (n=129)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 6.294 (IC base=+0.046)
...
- **PATRÓN** `dist_vwap_pct` > `0.9454` → IC=+0.182 (n=20)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.9454 (IC base=+0.045)
...
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#ETH#15min` — IC=+0.192 n=37. Faltan ~3 resoluciones para umbral n≥40. ETA: ~2h.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#ETH` — IC=+0.192 n=37. Faltan ~3 resoluciones para umbral n≥40. ETA: ~2h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2539 | +0.173 | -73.76€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 1636 | +0.209 | +4.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 106 | +0.009 | -19.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 365 | +0.089 | -62.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 432 | +0.145 | +3.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 737 | +0.178 | -20.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 548 | +0.202 | -18.51€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 35 | +0.041 | -5.63€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 144 | +0.151 | +7.83€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#ETH | 904 | +0.171 | -12.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 542 | +0.211 | +10.47€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 35 | -0.095 | -12.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 183 | +0.105 | -24.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 144 | +0.164 | +13.41€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL | 890 | +0.170 | -37.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 538 | +0.215 | +15.91€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 36 | +0.079 | -1.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 172 | +0.092 | -34.37€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 144 | +0.116 | -17.76€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 4873 | +0.102 | +1520.29€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 4873 | +0.102 | +1520.29€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1168 | +0.063 | +164.81€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1168 | +0.063 | +164.81€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1166 | +0.074 | +196.22€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1166 | +0.074 | +196.22€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1305 | +0.110 | +529.02€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1305 | +0.110 | +529.02€ | 1 | 7 |
| ✅ GBM_LATE_15M#XRP | 1234 | +0.157 | +630.24€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1234 | +0.157 | +630.24€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2095 | +0.148 | +1214.17€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2095 | +0.148 | +1214.17€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 536 | +0.110 | +263.27€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 536 | +0.110 | +263.27€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 555 | +0.125 | +252.85€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 555 | +0.125 | +252.85€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 565 | +0.108 | +242.39€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 565 | +0.108 | +242.39€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 439 | +0.273 | +455.67€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 439 | +0.273 | +455.67€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 2072 | +0.091 | +624.52€ | 0 | 7 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2072 | +0.091 | +624.52€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 512 | +0.056 | +87.27€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 512 | +0.056 | +87.27€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 480 | +0.019 | +8.92€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 480 | +0.019 | +8.92€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 488 | +0.063 | +99.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 488 | +0.063 | +99.08€ | 2 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 592 | +0.202 | +429.26€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 592 | +0.202 | +429.26€ | 0 | 10 |
...
| ✅ LEADLAG_BTC_XRP_15M | 142 | +0.083 | +27.51€ | 0 | 6 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 142 | +0.083 | +27.51€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 142 | +0.083 | +27.51€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 142 | +0.083 | +27.51€ | 0 | 6 |
...
| ✅ STREAK_FADE_15M | 205 | +0.099 | +29.61€ | 0 | 11 |
| ✅ STREAK_FADE_15M#15min | 205 | +0.099 | +29.61€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 1596 | +0.010 | +76.64€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1338 | +0.037 | +113.68€ | 1 | 7 |
...
| ✅ UPDOWN_GBM#ETH | 767 | +0.018 | +36.01€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 674 | +0.034 | +47.07€ | 2 | 3 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 122 | +0.185 | +35.66€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 122 | +0.185 | +35.66€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 37 | +0.192 | +5.01€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 37 | +0.192 | +5.01€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 40 | +0.191 | +17.14€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 40 | +0.191 | +17.14€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.155: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.006 n=494 | neutral: IC=-0.020 n=373 | overbought(IBS>0.7): IC=+0.135 n=403
  - _Datos_: n=1344 IC=+0.044 PNL=+137.35€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9827 | tras_1loss IC=+0.069 n=6853 | tras_2loss IC=+0.038 n=2927/40 | gap=+0.064 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1469 IC=+0.017 PNL=+90.64€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1469 IC=+0.017 PNL=+90.64€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=703 IC=+0.028 PNL=+50.12€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=703 IC=+0.028 PNL=+50.12€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=330 IC=+0.066 PNL=+44.52€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=330 IC=+0.066 PNL=+44.52€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=708 IC=+0.075 PNL=+96.68€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=708 IC=+0.075 PNL=+96.68€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=600 IC=+0.032 PNL=+39.71€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=600 IC=+0.032 PNL=+39.71€
...
**〰️ H-CUSTOM-CROSS-WINDOW-SPREAD-NEG** — GBM 15min/60min: spread negativo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: n=77 IC=+0.032 PNL=+34.62€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=77 IC=+0.032 PNL=+34.62€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=14431 IC=+0.106 PNL=+3522.40€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=14431 IC=+0.106 PNL=+3522.40€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=851 IC=-0.003 PNL=+20.59€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=851 IC=-0.003 PNL=+20.59€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.159 > 0.1 con n=409 PNL=+139.35€
  - _Datos_: n=409 IC=+0.159 PNL=+139.35€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=624 IC=+0.056 PNL=+71.91€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=624 IC=+0.056 PNL=+71.91€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.148 > 0.08 con n=299 PNL=+96.37€
  - _Datos_: n=299 IC=+0.148 PNL=+96.37€
...
**🟡 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.211 > 0.08 con n=133 PNL=+62.71€
  - _Datos_: n=133 IC=+0.211 PNL=+62.71€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema, sin filtrar todavía
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.181 < -0.1 con n=403 PNL=-83.77€
  - _Datos_: n=403 IC=-0.181 PNL=-83.77€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.134 > 0.08 con n=162 PNL=+48.82€
  - _Datos_: n=162 IC=+0.134 PNL=+48.82€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.109 > 0.08 con n=154 PNL=+58.63€
  - _Datos_: n=154 IC=+0.109 PNL=+58.63€
...
**🔶 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.170 n=204) pero sin cruzar ≥2 pares más — BTC: n=147 IC=-0.010; ETH: n=112 IC=+0.000; SOL: n=215 IC=+0.067 ✓
  - _Datos_: n=204 IC=+0.170 PNL=+132.75€
...
**🔶 H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Datos_: n=71 IC=+0.116 PNL=+23.06€
...
**🔶 H-CUSTOM-ETH15-BUYNO-TARDIO** — UPDOWN_GBM ETH#15min BUY_NO tardío (T_h<0.2) -- edge fuerte no capturado por el aprendizaje causal automático
  ...
  - _Estado_: SEÑAL POSITIVA en ETH (IC=+0.242 n=60) pero sin cruzar ≥2 pares más — BNB: n=12 IC=+0.043; BTC: n=12 IC=+0.043; DOGE: n=9 IC=+0.102; SOL: n=8 IC=+0.000; XRP: n=9 IC=+0.061
  - _Datos_: n=60 IC=+0.242 PNL=+31.19€
