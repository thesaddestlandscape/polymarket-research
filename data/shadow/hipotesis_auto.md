# Hipótesis automáticas — 2026-08-30 01:07 UTC
_Generado por shadow_postmortem.py sobre 208256 resoluciones (PNL=+16033.23€)_
...
- **FILTRO** `restante_s_al_confirmar` < `121.17` → IC=-0.381 (n=375)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 121.17
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=1128)

### BALLENAS_TARDIAS#ETH#5min
- **FILTRO** `restante_s_al_confirmar` < `158.31` → IC=-0.165 (n=778)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.31
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=2335)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `139.94` → IC=-0.322 (n=673)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 139.94
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=2019)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `156.95` → IC=-0.357 (n=663)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 156.95
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=1349)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.193 (n=6233)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.158 (n=5003)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` < 7.0 (IC base=+0.142)

- **PATRÓN** `py_entrada` < `0.34` → IC=+0.256 (n=3939)
...
- **PATRÓN** `libro_liquidez` > `1906.1368` → IC=+0.181 (n=2725)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.278 (n=543)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `2174.0854` → IC=+0.281 (n=464)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2174.0854 (IC base=+0.277)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.192 (n=4330)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 15.0 (IC base=+0.188)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.202 (n=2451)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.176 (n=1114)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.218 (n=10794)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.163 (n=1834)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.285 (n=831)
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.243 (n=1108)
...
- **PATRÓN** `restante_min` < `4.41` → IC=+0.128 (n=1191)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.139 (n=1324)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.735` → IC=+0.346 (n=335)
...
- **PATRÓN** `libro_liquidez` > `2113.4428` → IC=+0.323 (n=122)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9517` → IC=+0.211 (n=898)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9517 (IC base=+0.071)

- **PATRÓN** `dist_vwap_pct` > `0.186` → IC=+0.223 (n=363)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.186 (IC base=+0.071)

- **PATRÓN** `dist_vwap_pct` < `0.7242` → IC=+0.215 (n=601)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.7242 (IC base=+0.071)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4264` → IC=+0.158 (n=1336)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.4264 (IC base=+0.071)

- **PATRÓN** `volumen_spike_ratio` > `1.4604` → IC=+0.153 (n=1518)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.4604 (IC base=+0.071)

- **PATRÓN** `ibs_20min` < `0.1167` → IC=+0.151 (n=1358)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` < 0.1167 (IC base=+0.029)

- **PATRÓN** `dist_vwap_pct` < `0.1787` → IC=+0.142 (n=881)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.187 (n=215)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_spike_ratio` > `1.4303` → IC=+0.125 (n=507)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_spike_ratio` > 1.4303 (IC base=+0.138)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.175 (n=423)

  - _Acción_: Kelly boost +0.88€ cuando `libro_spread` < 0.04 (IC base=+0.138)

- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.168 (n=218)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.226 (n=195)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1042` → IC=+0.150 (n=429)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_pendiente_norm` < 0.1042 (IC base=+0.155)
...
- **PATRÓN** `volumen_spike_ratio` < `3.3381` → IC=+0.143 (n=413)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 3.3381 (IC base=+0.155)

- **PATRÓN** `volumen_spike_ratio` > `1.6927` → IC=+0.158 (n=469)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.6927 (IC base=+0.155)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.184 (n=523)

  - _Acción_: Kelly boost +0.92€ cuando `libro_spread` < 0.04 (IC base=+0.155)

- **PATRÓN** `ballena_activa_n` < `43.0` → IC=+0.191 (n=241)
...
- **FILTRO** `ibs_20min` > `0.8599` → IC=-0.174 (n=231)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.007 (n=696)
...
- **PATRÓN** `dist_vwap_pct` > `0.2759` → IC=+0.219 (n=30)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2759 (IC base=-0.049)

- **PATRÓN** `volumen_pendiente_norm` > `0.0872` → IC=+0.158 (n=36)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_pendiente_norm` > 0.0872 (IC base=-0.049)

- **PATRÓN** `volumen_spike_ratio` > `1.4619` → IC=+0.125 (n=118)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_spike_ratio` > 1.4619 (IC base=-0.049)
...
- **FILTRO** `sigma_h` > `0.0105` → IC=-0.135 (n=362)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=1090)
...
- **PATRÓN** `ibs_20min` < `0.0909` → IC=+0.194 (n=1339)
...
- **FILTRO** `ibs_20min` < `0.2606` → IC=-0.151 (n=256)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2606
  - _Potencial_: sin este filtro IC_bueno=+0.075 (n=520)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.99` → IC=+0.135 (n=195)
  ...
  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` > 3.99 (IC base=+0.000)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.176 (n=69)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=1001)
...
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.162 (n=977)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.0852` → IC=+0.168 (n=975)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `dist_vwap_pct` > `1.1449` → IC=+0.257 (n=496)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1449 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.462` → IC=+0.245 (n=1203)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.462 (IC base=+0.161)

- **PATRÓN** `volumen_regimen` > `0.6232` → IC=+0.172 (n=2019)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3185` → IC=+0.163 (n=2345)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.3185 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` > `1.8684` → IC=+0.156 (n=1776)
...
- **PATRÓN** `volumen_spike_ratio` < `1.9113` → IC=+0.177 (n=1341)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 1.9113 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` > `2.7073` → IC=+0.206 (n=671)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7073 (IC base=+0.182)

- **PATRÓN** `ballena_activa_n` < `110.0` → IC=+0.184 (n=1399)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 110.0 (IC base=+0.182)

### GBM_LATE_15M_MULTIHORIZONTE#BNB#15min
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.168 (n=209)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0058 (IC base=+0.160)

- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.206 (n=216)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0071 (IC base=+0.160)

- **PATRÓN** `drift_60min` |x|≤ `0.2631` → IC=+0.166 (n=474)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.2631 (IC base=+0.160)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.496` → IC=+0.286 (n=274)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4412` → IC=+0.156 (n=393)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.4412 (IC base=+0.160)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.192 (n=345)
...
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.167 (n=512)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0058 (IC base=+0.152)

- **PATRÓN** `drift_60min` |x|≤ `0.2258` → IC=+0.178 (n=451)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4297` → IC=+0.195 (n=512)

  - _Acción_: Kelly boost +0.97€ cuando `ibs_20min` < 0.4297 (IC base=+0.152)

- **PATRÓN** `dist_vwap_pct` < `0.1586` → IC=+0.178 (n=510)
...
- **PATRÓN** `volumen_regimen` < `0.6337` → IC=+0.223 (n=171)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5679` → IC=+0.168 (n=404)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 2.5679 (IC base=+0.152)

- **PATRÓN** `libro_liquidez` > `15006.2527` → IC=+0.182 (n=171)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 15006.2527 (IC base=+0.152)
...
- **PATRÓN** `sigma_h` < `0.0098` → IC=+0.263 (n=310)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0098 (IC base=+0.247)

- **PATRÓN** `drift_60min` |x|≤ `0.217` → IC=+0.285 (n=207)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.217 (IC base=+0.247)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.274 (n=215)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.247)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.246 (n=282)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 16.0 (IC base=+0.247)

- **PATRÓN** `ibs_20min` < `0.3889` → IC=+0.285 (n=310)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3889 (IC base=+0.247)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.973` → IC=+0.280 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.973 (IC base=+0.247)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.211` → IC=+0.249 (n=345)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3746` → IC=+0.318 (n=42)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3746 (IC base=+0.247)

- **PATRÓN** `volumen_spike_ratio` > `3.1717` → IC=+0.262 (n=107)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.1717 (IC base=+0.247)

- **PATRÓN** `libro_liquidez` > `1880.0598` → IC=+0.264 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1880.0598 (IC base=+0.247)

- **PATRÓN** `ballena_activa_n` < `59.0` → IC=+0.208 (n=193)
...
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.172 (n=382)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0081 (IC base=+0.120)
...
- **PATRÓN** `dist_vwap_pct` > `0.924` → IC=+0.266 (n=182)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.924 (IC base=+0.120)
...
- **PATRÓN** `volumen_regimen` > `0.6193` → IC=+0.138 (n=573)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` > 0.6193 (IC base=+0.120)

- **PATRÓN** `volumen_pendiente_norm` < `0.1612` → IC=+0.136 (n=564)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_pendiente_norm` < 0.1612 (IC base=+0.120)

- **PATRÓN** `volumen_spike_ratio` < `1.4411` → IC=+0.132 (n=180)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` < 1.4411 (IC base=+0.120)

- **PATRÓN** `volumen_spike_ratio` > `1.5596` → IC=+0.128 (n=482)
...
- **PATRÓN** `ibs_20min` < `0.3182` → IC=+0.128 (n=374)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.536` → IC=+0.192 (n=3060)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` < 3.536 (IC base=+0.187)

- **PATRÓN** `volumen_regimen` < `0.6183` → IC=+0.181 (n=762)
...
- **PATRÓN** `volumen_spike_ratio` > `2.34` → IC=+0.199 (n=1096)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.34 (IC base=+0.187)

- **PATRÓN** `ballena_activa_n` < `37.0` → IC=+0.174 (n=884)

  - _Acción_: Kelly boost +0.87€ cuando `ballena_activa_n` < 37.0 (IC base=+0.187)

### GBM_LATE_15M_TARDIO#BNB#15min
- **PATRÓN** `sigma_h` < `0.0053` → IC=+0.161 (n=184)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0053 (IC base=+0.162)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.193 (n=203)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.532` → IC=+0.311 (n=321)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8937` → IC=+0.165 (n=314)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.8937 (IC base=+0.162)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.216 (n=371)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.250 (n=206)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2205` → IC=+0.197 (n=391)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` < 0.2205 (IC base=+0.192)

- **PATRÓN** `volumen_spike_ratio` < `1.9531` → IC=+0.194 (n=168)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 1.9531 (IC base=+0.192)

- **PATRÓN** `volumen_spike_ratio` > `4.1999` → IC=+0.200 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 4.1999 (IC base=+0.192)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.224 (n=418)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.192)

- **PATRÓN** `libro_liquidez` > `1803.9032` → IC=+0.195 (n=303)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 1803.9032 (IC base=+0.192)

- **PATRÓN** `ballena_activa_n` < `51.0` → IC=+0.250 (n=146)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4736` → IC=+0.162 (n=472)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.182 (n=218)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_regimen` > `1.1518` → IC=+0.173 (n=157)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5177` → IC=+0.186 (n=138)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 2.5177 (IC base=+0.146)

- **PATRÓN** `libro_liquidez` > `9899.6333` → IC=+0.194 (n=214)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 9899.6333 (IC base=+0.146)

- **PATRÓN** `ballena_activa_n` < `193.0` → IC=+0.171 (n=348)
...
- **PATRÓN** `libro_liquidez` > `3014.1599` → IC=+0.253 (n=200)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3014.1599 (IC base=+0.095)

- **PATRÓN** `ballena_activa_n` < `66.0` → IC=+0.154 (n=377)

  - _Acción_: Kelly boost +0.77€ cuando `ballena_activa_n` < 66.0 (IC base=+0.095)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.146 (n=1327)
...
- **PATRÓN** `sigma_h` < `0.0036` → IC=+0.196 (n=337)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0036 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.3655` → IC=+0.166 (n=886)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_h` < `0.0035` → IC=+0.197 (n=242)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0035 (IC base=+0.138)

- **PATRÓN** `drift_60min` |x|≤ `0.0856` → IC=+0.170 (n=183)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.0856 (IC base=+0.138)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.171 (n=211)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 17.0 (IC base=+0.138)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.183 (n=200)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` > `0.6649` → IC=+0.187 (n=65)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.6649 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.363` → IC=+0.162 (n=546)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` < 6.363 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `1.1906` → IC=+0.158 (n=547)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5675` → IC=+0.153 (n=545)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 2.5675 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` > `1.8093` → IC=+0.144 (n=363)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.8093 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `12040.0659` → IC=+0.140 (n=489)

  - _Acción_: Kelly boost +0.70€ cuando `libro_liquidez` > 12040.0659 (IC base=+0.138)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5066` → IC=+0.187 (n=295)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.5066 (IC base=+0.142)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.168 (n=197)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 10.0 (IC base=+0.142)

- **PATRÓN** `ibs_20min` > `0.1271` → IC=+0.169 (n=294)

  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` > 0.1271 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.1407` → IC=+0.161 (n=116)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.1407 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.138` → IC=+0.185 (n=52)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 11.138 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.476` → IC=+0.144 (n=301)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 6.476 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `1.1943` → IC=+0.170 (n=295)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 1.1943 (IC base=+0.142)

- **PATRÓN** `volumen_pendiente_norm` < `0.1393` → IC=+0.163 (n=298)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` < 0.1393 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` < `2.1791` → IC=+0.177 (n=255)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 2.1791 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` > `1.4556` → IC=+0.164 (n=290)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.4556 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `8820.3762` → IC=+0.145 (n=263)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 8820.3762 (IC base=+0.142)
...
- **PATRÓN** `ibs_20min` > `0.6842` → IC=+0.253 (n=164)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6842 (IC base=+0.092)

- **PATRÓN** `dist_vwap_pct` > `0.1239` → IC=+0.190 (n=85)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.1239 (IC base=+0.092)
...
- **PATRÓN** `volumen_regimen` < `0.6319` → IC=+0.162 (n=72)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.6319 (IC base=+0.092)

- **PATRÓN** `volumen_regimen` > `1.1984` → IC=+0.202 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1984 (IC base=+0.092)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2582` → IC=+0.350 (n=18)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2582 (IC base=+0.092)

- **PATRÓN** `volumen_spike_ratio` > `1.4015` → IC=+0.336 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4015 (IC base=+0.092)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.189 (n=149)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.02 (IC base=+0.092)

- **PATRÓN** `libro_liquidez` > `1593.6494` → IC=+0.174 (n=139)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 1593.6494 (IC base=+0.092)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.215 (n=1008)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=3283)

- **FILTRO** `ibs_20min` > `0.2738` → IC=-0.173 (n=1072)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2738
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=3219)
...
- **FILTRO** `ibs_20min` > `0.1823` → IC=-0.126 (n=375)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.048 (n=376)

### MOMENTUM_IBS_15M_BALLENA#BTC#15min
- **FILTRO** `py_entrada` > `0.57` → IC=-0.216 (n=167)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=528)

- **FILTRO** `ballena_activa_n` > `39.0` → IC=-0.135 (n=236)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.216 (n=167)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=521)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.140 (n=2620)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=7894)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.278 (n=2614)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=7900)

- **FILTRO** `ibs_7min` < `0.7273` → IC=-0.231 (n=2626)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7273
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=7888)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.172 (n=3555)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=6959)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.229 (n=2993)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=9947)

- **FILTRO** `ibs_7min` > `0.7273` → IC=-0.169 (n=3227)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7273
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=9713)
...
- **FILTRO** `py_entrada` > `0.71` → IC=-0.229 (n=540)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=1711)
...
- **FILTRO** `py_entrada` < `0.37` → IC=-0.307 (n=486)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=1059)

- **FILTRO** `ibs_7min` < `0.2222` → IC=-0.282 (n=383)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2222
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=1162)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.241 (n=380)

  - _Acción_: SKIP cuando `ballena_activa_n` > 13.0
  - _Potencial_: sin este filtro IC_bueno=-0.076 (n=1165)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.245 (n=500)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=1766)

- **FILTRO** `ibs_7min` > `0.8113` → IC=-0.177 (n=565)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8113
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=1701)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.149 (n=531)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.081 (n=1241)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.258 (n=428)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=1344)

- **FILTRO** `ibs_7min` < `0.7608` → IC=-0.183 (n=443)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7608
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=1329)

- **FILTRO** `ballena_activa_n` > `39.0` → IC=-0.192 (n=439)

  - _Acción_: SKIP cuando `ballena_activa_n` > 39.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=1333)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.261 (n=437)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=1342)

- **FILTRO** `ibs_7min` > `0.192` → IC=-0.171 (n=603)

  - _Acción_: SKIP cuando `ibs_7min` > 0.192
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=1176)

- **FILTRO** `ballena_activa_n` > `35.0` → IC=-0.187 (n=436)

  - _Acción_: SKIP cuando `ballena_activa_n` > 35.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=1343)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.182 (n=592)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=1800)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `hora_utc` < `9.0` → IC=-0.129 (n=551)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
...
- **FILTRO** `ibs_7min` < `0.75` → IC=-0.230 (n=420)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.270 (n=463)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=1777)
...
- **FILTRO** `ballena_activa_n` > `10.0` → IC=-0.132 (n=739)
  ...
  - _Acción_: SKIP cuando `ballena_activa_n` > 10.0
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=1501)
...
- **PATRÓN** `delta_ratio` |x|> `0.3982` → IC=+0.137 (n=384)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.125 (n=331)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` < 18.0 (IC base=+0.122)

- **PATRÓN** `total_vol_5m` < `451.5692` → IC=+0.183 (n=121)

  - _Acción_: Kelly boost +0.91€ cuando `total_vol_5m` < 451.5692 (IC base=+0.122)

- **PATRÓN** `libro_liquidez` > `3250.4222` → IC=+0.157 (n=141)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 3250.4222 (IC base=+0.122)
...
- **FILTRO** `sigma_h` > `0.0101` → IC=-0.187 (n=276)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=831)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.125 (n=291)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=816)

- **FILTRO** `sigma_ewma_delta_pct` > `17.671` → IC=-0.176 (n=381)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 17.671
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=2919)
...
- **PATRÓN** `dist_vwap_pct` < `0.3035` → IC=+0.148 (n=52)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.3035 (IC base=-0.061)
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3676 sube el IC de -0.078 a +0.312 en UPDOWN_GBM_15M_TARDIO (n=238). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#SOL#15min**: dentro de BUY_NO, IBS < 0.38 sube el IC de -0.061 a +0.167 en UPDOWN_GBM_15M_TARDIO#SOL#15min (n=52). Ya aplicado como kelly_boost=+0.83€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 11643 | -0.101 | -1960.80€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 819 | -0.032 | -130.91€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 10824 | -0.106 | -1829.90€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1504 | -0.056 | -303.57€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1504 | -0.056 | -303.57€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 819 | -0.032 | -130.91€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 819 | -0.032 | -130.91€ | 2 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 1503 | -0.140 | -448.46€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 1503 | -0.140 | -448.46€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 3113 | -0.070 | -318.26€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 3113 | -0.070 | -318.26€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 2692 | -0.109 | -271.99€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2692 | -0.109 | -271.99€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 2012 | -0.172 | -487.60€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 2012 | -0.172 | -487.60€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 1429 | -0.060 | +697.05€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 403 | -0.001 | +277.53€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 1026 | -0.083 | +419.52€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 1429 | -0.060 | +697.05€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 403 | -0.001 | +277.53€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 1026 | -0.083 | +419.52€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 37689 | +0.115 | -2342.01€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 6706 | +0.186 | -247.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 117 | -0.088 | -49.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 27487 | +0.098 | -1998.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3379 | +0.119 | -45.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 4609 | +0.071 | -711.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 22 | -0.083 | +0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 4582 | +0.072 | -705.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 7689 | +0.134 | -161.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1910 | +0.198 | -94.58€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 4574 | +0.110 | -100.56€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1163 | +0.128 | +55.63€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 4621 | +0.084 | -536.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 24 | +0.077 | +2.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 4596 | +0.084 | -537.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 8251 | +0.126 | -133.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2444 | +0.168 | -18.93€ | 1 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 4577 | +0.112 | -79.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1218 | +0.100 | -26.35€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 7912 | +0.131 | -505.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2289 | +0.199 | -137.24€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 55 | +0.009 | -9.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 4570 | +0.098 | -283.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 998 | +0.132 | -75.00€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#XRP | 4607 | +0.108 | -293.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 17 | -0.022 | -0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 4588 | +0.109 | -292.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 6645 | +0.175 | -516.00€ | 2 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 6645 | +0.175 | -516.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 1680 | +0.164 | -187.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 1680 | +0.164 | -187.30€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 19602 | +0.191 | -1718.16€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 19602 | +0.191 | -1718.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 3545 | +0.134 | -614.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 3545 | +0.134 | -614.47€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 3074 | +0.233 | -75.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 3074 | +0.233 | -75.84€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 3348 | +0.170 | -397.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 3348 | +0.170 | -397.75€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 3117 | +0.227 | -105.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 3117 | +0.227 | -105.15€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 3212 | +0.209 | -191.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 3212 | +0.209 | -191.46€ | 0 | 2 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 7148 | +0.132 | +251.20€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 7148 | +0.132 | +251.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 3561 | +0.138 | +156.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 3561 | +0.138 | +156.30€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 3587 | +0.127 | +94.90€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 3587 | +0.127 | +94.90€ | 0 | 5 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 9087 | +0.096 | -322.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 865 | +0.068 | -28.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 8222 | +0.099 | -294.92€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC | 5824 | +0.097 | -125.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#15min | 865 | +0.068 | -28.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#5min | 4959 | +0.102 | -97.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH | 646 | +0.117 | +9.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH#5min | 646 | +0.117 | +9.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL | 2617 | +0.086 | -206.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL#5min | 