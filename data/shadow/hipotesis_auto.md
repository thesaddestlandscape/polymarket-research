# Hipótesis automáticas — 2026-07-12 18:29 UTC
_Generado por shadow_postmortem.py sobre 11270 resoluciones (PNL=+2138.09€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.214 (n=337)
...
- **PATRÓN** `py_entrada` > `0.585` → IC=+0.245 (n=374)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.585 (IC base=+0.213)

- **PATRÓN** `libro_liquidez` > `2838.1924` → IC=+0.229 (n=319)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2838.1924 (IC base=+0.213)
...
- **PATRÓN** `libro_liquidez` > `6004.8923` → IC=+0.172 (n=175)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 6004.8923 (IC base=+0.154)
...
- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.150 (n=938)
...
- **PATRÓN** `dist_vwap_pct` < `0.4249` → IC=+0.171 (n=141)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.4249 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.234` → IC=+0.200 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.234 (IC base=+0.111)
...
- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.193 (n=333)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0049 (IC base=+0.173)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.179 (n=250)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 13.0 (IC base=+0.173)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.180 (n=383)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` < 17.0 (IC base=+0.173)

- **PATRÓN** `dist_vwap_pct` > `0.5381` → IC=+0.295 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5381 (IC base=+0.173)

- **PATRÓN** `dist_vwap_pct` < `0.1312` → IC=+0.202 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1312 (IC base=+0.173)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.579` → IC=+0.326 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.579 (IC base=+0.173)
...
- **PATRÓN** `sigma_h` > `0.0067` → IC=+0.186 (n=49)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0067 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.175 (n=78)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 12.0 (IC base=+0.151)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.176 (n=103)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` < 16.0 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` > `0.4503` → IC=+0.239 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4503 (IC base=+0.151)
...
- **PATRÓN** `dist_vwap_pct` < `0.1754` → IC=+0.163 (n=87)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.1754 (IC base=+0.083)
...
- **PATRÓN** `dist_vwap_pct` > `0.568` → IC=+0.136 (n=20)
  ...
  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 0.568 (IC base=-0.007)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 988 | +0.183 | +11.89€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 636 | +0.212 | +11.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 40 | +0.048 | +1.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 147 | +0.097 | -10.91€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 351 | +0.166 | +1.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 211 | +0.204 | +2.94€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 13 | -0.065 | -1.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 72 | +0.095 | -7.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 55 | +0.167 | +7.09€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO#SOL | 345 | +0.197 | +14.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 211 | +0.223 | +16.10€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 14 | +0.044 | +0.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 65 | +0.157 | +0.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 55 | +0.167 | -1.33€ | 0 | 2 |
| ✅ GBM_LATE_15M | 3658 | +0.114 | +1230.70€ | 0 | 10 |
| ✅ GBM_LATE_15M#15min | 3658 | +0.114 | +1230.70€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 897 | +0.071 | +123.10€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 897 | +0.071 | +123.10€ | 0 | 2 |
| ✅ GBM_LATE_15M#ETH | 926 | +0.073 | +158.77€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 926 | +0.073 | +158.77€ | 0 | 3 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1009 | +0.165 | +581.30€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1009 | +0.165 | +581.30€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 265 | +0.159 | +167.86€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 265 | +0.159 | +167.86€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 269 | +0.131 | +116.56€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 269 | +0.131 | +116.56€ | 0 | 8 |
...
| ✅ GBM_LATE_15M_TARDIO | 1164 | +0.082 | +292.31€ | 1 | 7 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1164 | +0.082 | +292.31€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 270 | +0.081 | +42.70€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 270 | +0.081 | +42.70€ | 0 | 2 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 291 | +0.009 | +12.06€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 291 | +0.009 | +12.06€ | 0 | 2 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.092 n=6385 | tras_1loss IC=+0.048 n=4747 | tras_2loss IC=+0.013 n=2120/40 | gap=+0.079 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8812 IC=+0.099 PNL=+2213.97€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8812 IC=+0.099 PNL=+2213.97€
