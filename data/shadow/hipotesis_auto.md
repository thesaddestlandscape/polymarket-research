# Hipótesis automáticas — 2026-08-28 03:33 UTC
_Generado por shadow_postmortem.py sobre 182741 resoluciones (PNL=+13273.26€)_
...
- **FILTRO** `restante_s_al_confirmar` < `147.36` → IC=-0.287 (n=2548)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=7646)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.157 (n=4488)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.128 (n=1065)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.192 (n=656)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.192 (n=3973)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.179 (n=1020)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` < 15.0 (IC base=+0.166)

- **PATRÓN** `py_entrada` < `0.72` → IC=+0.175 (n=1020)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.169 (n=1002)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.230 (n=904)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.185 (n=981)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 15.0 (IC base=+0.183)

- **PATRÓN** `py_entrada` < `0.7` → IC=+0.202 (n=578)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.219 (n=9305)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.288 (n=720)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.190 (n=1545)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` < 12.0 (IC base=+0.185)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.210 (n=1603)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.149 (n=2312)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` < 8.0 (IC base=+0.133)

- **PATRÓN** `lag_apertura_s` < `4.32` → IC=+0.161 (n=1573)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.136 (n=1041)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9504` → IC=+0.210 (n=794)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3076` → IC=+0.160 (n=186)
  ...
  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` > 0.3076 (IC base=+0.066)

- **PATRÓN** `volumen_spike_ratio` < `2.4442` → IC=+0.149 (n=1142)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.169 (n=255)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1422` → IC=+0.151 (n=127)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_pendiente_norm` > 0.1422 (IC base=+0.125)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.154 (n=336)
...
- **FILTRO** `ibs_20min` > `0.5` → IC=-0.138 (n=645)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=698)

- **FILTRO** `sigma_ewma_delta_pct` > `4.966` → IC=-0.156 (n=283)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.966
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=1060)
...
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.138 (n=1354)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_regimen` > `1.1729` → IC=+0.199 (n=340)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` > 1.1729 (IC base=+0.056)

- **PATRÓN** `volumen_pendiente_norm` < `0.115` → IC=+0.176 (n=1345)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` < 0.115 (IC base=+0.056)

- **PATRÓN** `volumen_pendiente_norm` > `0.2492` → IC=+0.206 (n=263)
...
- **PATRÓN** `ballena_activa_n` < `100.0` → IC=+0.278 (n=834)
...
- **FILTRO** `ibs_20min` < `0.2584` → IC=-0.160 (n=198)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.075 (n=403)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.229` → IC=+0.175 (n=115)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 5.229 (IC base=-0.003)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.257 (n=167)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1444` → IC=+0.201 (n=366)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.220 (n=426)
...
- **FILTRO** `ibs_20min` < `0.7381` → IC=-0.132 (n=444)
...
- **PATRÓN** `ibs_20min` > `0.8529` → IC=+0.272 (n=169)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8529 (IC base=-0.010)

- **PATRÓN** `dist_vwap_pct` > `0.3412` → IC=+0.273 (n=117)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3412 (IC base=-0.010)

- **PATRÓN** `volumen_regimen` < `0.8566` → IC=+0.192 (n=128)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` < 0.8566 (IC base=-0.010)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0905` → IC=+0.174 (n=173)
  ...
  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` < 0.0905 (IC base=-0.010)
...
- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.238 (n=128)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 62.0 (IC base=-0.010)
...
- **PATRÓN** `volumen_regimen` > `0.8369` → IC=+0.255 (n=386)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8369 (IC base=+0.222)

- **PATRÓN** `volumen_pendiente_norm` < `0.0804` → IC=+0.229 (n=492)
...
- **PATRÓN** `libro_liquidez` > `2451.9374` → IC=+0.232 (n=579)
...
- **PATRÓN** `sigma_h` > `0.0102` → IC=+0.191 (n=866)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0102 (IC base=+0.156)

- **PATRÓN** `drift_60min` |x|≤ `0.3392` → IC=+0.158 (n=2284)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_regimen` > `0.6895` → IC=+0.170 (n=1609)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2798` → IC=+0.155 (n=2062)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.2798 (IC base=+0.156)

- **PATRÓN** `volumen_spike_ratio` > `1.8493` → IC=+0.150 (n=1562)
...
- **PATRÓN** `libro_liquidez` > `3201.5915` → IC=+0.180 (n=1177)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 3201.5915 (IC base=+0.156)

- **PATRÓN** `ballena_activa_n` < `116.0` → IC=+0.178 (n=1322)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.192 (n=1369)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.505` → IC=+0.227 (n=1769)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.505 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` < `0.8781` → IC=+0.146 (n=1426)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.8781 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` > `1.0787` → IC=+0.151 (n=969)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 1.0787 (IC base=+0.148)

- **PATRÓN** `volumen_pendiente_norm` > `0.1653` → IC=+0.179 (n=781)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1653 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` < `2.3078` → IC=+0.148 (n=2377)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 2.3078 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` > `1.8616` → IC=+0.158 (n=1800)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.8616 (IC base=+0.148)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.154 (n=2446)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `2498.0518` → IC=+0.175 (n=2013)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 2498.0518 (IC base=+0.148)

- **PATRÓN** `ballena_activa_n` < `149.0` → IC=+0.191 (n=1447)
...
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.214 (n=222)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.341` → IC=+0.306 (n=292)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.197 (n=288)
...
- **PATRÓN** `sigma_h` < `0.0089` → IC=+0.153 (n=488)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` > `0.3436` → IC=+0.192 (n=488)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` > 0.3436 (IC base=+0.138)
...
- **PATRÓN** `volumen_regimen` < `0.6197` → IC=+0.173 (n=163)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 0.6197 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `1.1962` → IC=+0.154 (n=163)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.1962 (IC base=+0.138)
...
- **PATRÓN** `volumen_spike_ratio` > `2.4262` → IC=+0.232 (n=151)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4262 (IC base=+0.138)
...
- **PATRÓN** `ibs_20min` > `0.4872` → IC=+0.175 (n=539)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` > 0.4872 (IC base=+0.092)

- **PATRÓN** `dist_vwap_pct` > `0.8687` → IC=+0.225 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8687 (IC base=+0.092)
...
- **PATRÓN** `libro_liquidez` > `2951.497` → IC=+0.280 (n=180)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8157` → IC=+0.188 (n=386)
...
- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.226 (n=359)
...
- **PATRÓN** `sigma_h` < `0.0108` → IC=+0.132 (n=571)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` > `0.9219` → IC=+0.176 (n=217)

  - _Acción_: Kelly boost +0.88€ cuando `ibs_20min` > 0.9219 (IC base=+0.116)

- **PATRÓN** `volumen_regimen` < `0.6872` → IC=+0.126 (n=220)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` < 0.6872 (IC base=+0.116)
...
- **PATRÓN** `libro_liquidez` > `9431.7078` → IC=+0.123 (n=295)

  - _Acción_: Kelly boost +0.61€ cuando `libro_liquidez` > 9431.7078 (IC base=+0.116)
...
- **PATRÓN** `sigma_h` < `0.0094` → IC=+0.171 (n=259)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0094 (IC base=+0.135)

- **PATRÓN** `drift_60min` |x|≤ `0.4426` → IC=+0.155 (n=227)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.4426 (IC base=+0.135)

- **PATRÓN** `hora_utc` > `19.0` → IC=+0.203 (n=89)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 19.0 (IC base=+0.135)

- **PATRÓN** `ibs_20min` < `0.893` → IC=+0.135 (n=258)

  - _Acción_: Kelly boost +0.67€ cuando `ibs_20min` < 0.893 (IC base=+0.135)

- **PATRÓN** `ibs_20min` > `0.8027` → IC=+0.181 (n=117)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` > 0.8027 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` > `0.9533` → IC=+0.181 (n=70)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.9533 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` < `0.397` → IC=+0.141 (n=229)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.397 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.158` → IC=+0.159 (n=39)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 11.158 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.589` → IC=+0.139 (n=258)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` < 6.589 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` < `0.6457` → IC=+0.193 (n=86)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` < 0.6457 (IC base=+0.135)

- **PATRÓN** `volumen_pendiente_norm` < `0.1178` → IC=+0.137 (n=235)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_pendiente_norm` < 0.1178 (IC base=+0.135)

- **PATRÓN** `volumen_pendiente_norm` > `0.1741` → IC=+0.154 (n=79)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_pendiente_norm` > 0.1741 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` < `1.4166` → IC=+0.198 (n=84)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 1.4166 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `8959.8986` → IC=+0.160 (n=230)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.170 (n=883)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=2706)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.217 (n=842)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=2741)

- **FILTRO** `ibs_20min` > `0.2723` → IC=-0.181 (n=894)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2723
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=2689)
...
- **FILTRO** `ibs_20min` > `0.7407` → IC=-0.210 (n=150)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=452)
...
- **FILTRO** `ibs_20min` > `0.28` → IC=-0.238 (n=143)

  - _Acción_: SKIP cuando `ibs_20min` > 0.28
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=432)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.140 (n=2560)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
...
- **FILTRO** `py_entrada` < `0.36` → IC=-0.282 (n=2165)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=6788)

- **FILTRO** `ibs_7min` < `0.7333` → IC=-0.232 (n=2234)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7333
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=6719)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.172 (n=3041)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=5912)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.228 (n=2512)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=8366)

- **FILTRO** `ibs_7min` > `0.7215` → IC=-0.174 (n=2719)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7215
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=8159)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.140 (n=376)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
...
- **FILTRO** `py_entrada` < `0.37` → IC=-0.252 (n=409)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=1333)

- **FILTRO** `ibs_7min` < `0.7974` → IC=-0.177 (n=435)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7974
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=1307)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.201 (n=312)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **FILTRO** `py_entrada` < `0.37` → IC=-0.317 (n=413)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.030 (n=877)

- **FILTRO** `ibs_7min` < `0.2143` → IC=-0.287 (n=322)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2143
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=968)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.278 (n=322)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=968)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.150 (n=450)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
...
- **FILTRO** `ibs_7min` < `0.7565` → IC=-0.184 (n=375)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=1127)

- **FILTRO** `ballena_activa_n` > `43.0` → IC=-0.211 (n=365)

  - _Acción_: SKIP cuando `ballena_activa_n` > 43.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=1137)

- **FILTRO** `hora_utc` > `15.0` → IC=-0.124 (n=501)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=1000)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.269 (n=357)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=1144)

- **FILTRO** `ibs_7min` > `0.1849` → IC=-0.160 (n=510)

  - _Acción_: SKIP cuando `ibs_7min` > 0.1849
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=991)

- **FILTRO** `ballena_activa_n` > `26.0` → IC=-0.177 (n=499)

  - _Acción_: SKIP cuando `ballena_activa_n` > 26.0
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=1002)
...
- **FILTRO** `ibs_7min` < `0.7719` → IC=-0.197 (n=430)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=1291)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.186 (n=419)

  - _Acción_: SKIP cuando `ballena_activa_n` > 13.0
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=1302)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.121 (n=484)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=1007)

- **FILTRO** `py_entrada` < `0.38` → IC=-0.290 (n=369)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=1122)

- **FILTRO** `ibs_7min` < `0.7531` → IC=-0.211 (n=372)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7531
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=1119)

- **FILTRO** `ballena_activa_n` > `14.0` → IC=-0.225 (n=369)
...
- **PATRÓN** `ibs_15` < `0.1154` → IC=+0.141 (n=455)
...
- **FILTRO** `drift_15min` |x|> `0.5274` → IC=-0.161 (n=116)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.023 (n=350)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.439` → IC=-0.170 (n=352)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 17.439
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=2614)
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3357 sube el IC de -0.079 a +0.328 en UPDOWN_GBM_15M_TARDIO (n=178). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 10194 | -0.095 | -1649.34€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 727 | -0.068 | -127.76€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 9467 | -0.097 | -1521.58€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#ETH | 2759 | -0.069 | -270.86€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 2759 | -0.069 | -270.86€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 2379 | -0.080 | -175.24€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2379 | -0.080 | -175.24€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1781 | -0.177 | -442.82€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1781 | -0.177 | -442.82€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 223 | -0.060 | +71.26€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 61 | -0.008 | +37.39€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 162 | -0.079 | +33.87€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 223 | -0.060 | +71.26€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 61 | -0.008 | +37.39€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 162 | -0.079 | +33.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 33693 | +0.115 | -2104.61€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 6148 | +0.186 | -221.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 109 | -0.104 | -50.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 24190 | +0.097 | -1792.59€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 6933 | +0.134 | -151.56€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1741 | +0.194 | -105.53€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 4026 | +0.111 | -82.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1124 | +0.130 | +58.33€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 4068 | +0.082 | -491.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 22 | +0.083 | +3.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 4045 | +0.082 | -493.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 7437 | +0.127 | -112.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2234 | +0.167 | -18.62€ | 0 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 4027 | +0.114 | -58.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1164 | +0.100 | -26.89€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 7138 | +0.135 | -412.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2113 | +0.204 | -102.59€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 48 | +0.000 | -9.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 4019 | +0.101 | -228.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 958 | +0.133 | -71.31€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 4056 | +0.109 | -253.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 17 | -0.022 | -0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 4038 | +0.109 | -251.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 6076 | +0.174 | -474.74€ | 3 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 6076 | +0.174 | -474.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 1533 | +0.166 | -163.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 1533 | +0.166 | -163.66€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 1511 | +0.164 | -170.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 1511 | +0.164 | -170.85€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 1363 | +0.230 | -39.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 1363 | +0.230 | -39.29€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 1473 | +0.183 | -114.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 1473 | +0.183 | -114.77€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 17280 | +0.190 | -1531.39€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 17280 | +0.190 | -1531.39€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 2746 | +0.227 | -91.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 2746 | +0.227 | -91.53€ | 0 | 2 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 2908 | +0.185 | -288.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 2908 | +0.185 | -288.22€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 6269 | +0.133 | +232.46€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 6269 | +0.133 | +232.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 3121 | +0.140 | +151.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 3121 | +0.140 | +151.54€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 