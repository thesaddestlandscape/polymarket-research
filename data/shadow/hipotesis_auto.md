# Hipótesis automáticas — 2026-07-05 01:39 UTC
_Generado por shadow_postmortem.py sobre 3705 resoluciones (PNL=+119.11€)_
...
- **PATRÓN** `sigma_h` < `0.009` → IC=+0.160 (n=92)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.009 (IC base=+0.029)

- **PATRÓN** `ibs_15` < `0.0642` → IC=+0.139 (n=70)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.69€ cuando `ibs_15` < 0.0642 (IC base=+0.029)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.155 (n=27)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0061 (IC base=+0.078)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.167 (n=31)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 7.0 (IC base=+0.078)

- **PATRÓN** `ibs_15` < `0.1053` → IC=+0.160 (n=45)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.80€ cuando `ibs_15` < 0.1053 (IC base=+0.078)
...
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 913 | +0.096 | +218.90€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 913 | +0.096 | +218.90€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 237 | +0.048 | +28.36€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 237 | +0.048 | +28.36€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 237 | +0.040 | +16.80€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 237 | +0.040 | +16.80€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL | 234 | +0.144 | +92.10€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 234 | +0.144 | +92.10€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP | 205 | +0.157 | +81.64€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 205 | +0.157 | +81.64€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 936 | -0.039 | -54.24€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 700 | -0.017 | -24.40€ | 1 | 2 |
...
| ✅ UPDOWN_GBM#ETH | 387 | -0.006 | +6.18€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 301 | +0.012 | +13.67€ | 3 | 3 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.045) — sin ventaja clara. oversold(IBS<0.3): IC=-0.018 n=193 | neutral: IC=-0.062 n=158 | overbought(IBS>0.7): IC=-0.048 n=175
  - _Datos_: n=546 IC=-0.038 PNL=-29.00€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=01h: IC=-0.206 n=32 PNL=-16.14€ → FILTRAR | H=02h: IC=-0.154 n=24 PNL=-5.23€ → FILTRAR | H=03h: IC=-0.113 n=19 PNL=-5.87€ → FILTRAR | H=10h: IC=-0.154 n=15 PNL=-6.77€ → FILTRAR | H=13h: IC=+0.115 n=24 PNL=+5.54€ → BOOST | H=16h: IC=+0.167 n=25 PNL=+7.36€ → BOOST | H=20h: IC=-0.147 n=32 PNL=-9.63€ → FILTRAR
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.028 n=1846 | tras_1loss IC=-0.012 n=1759 | tras_2loss IC=-0.031 n=880/40 | gap=+0.058 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=813 IC=-0.035 PNL=-40.69€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=813 IC=-0.035 PNL=-40.69€
...
**〰️ H-CUSTOM-ETH15-REVERSION** — ETH#15min con drift_15min < -1 — ¿mean reversion?
  ...
  - _Estado_: n=52 IC=+0.037 PNL=+7.14€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=52 IC=+0.037 PNL=+7.14€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=257 IC=+0.002 PNL=+4.93€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=257 IC=+0.002 PNL=+4.93€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=1264 IC=+0.062 PNL=+200.25€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=1264 IC=+0.062 PNL=+200.25€
