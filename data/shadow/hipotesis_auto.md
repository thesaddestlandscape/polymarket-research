# Hipótesis automáticas — 2026-07-23 06:34 UTC
_Generado por shadow_postmortem.py sobre 30372 resoluciones (PNL=+6835.80€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.207 (n=1953)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.200)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.202 (n=1674)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.200)

- **PATRÓN** `py_entrada` > `0.735` → IC=+0.332 (n=648)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.735 (IC base=+0.200)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.205 (n=2152)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.179 (n=2340)

  - _Acción_: Kelly boost +0.89€ cuando `libro_spread` < 0.01 (IC base=+0.175)

- **PATRÓN** `libro_liquidez` > `6055.3694` → IC=+0.176 (n=952)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 6055.3694 (IC base=+0.175)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.218 (n=370)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.212)

- **PATRÓN** `py_entrada` > `0.605` → IC=+0.270 (n=419)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.272 (n=90)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `8877.3756` → IC=+0.222 (n=34)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8877.3756 (IC base=+0.213)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.228 (n=200)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `1873.3412` → IC=+0.251 (n=279)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1873.3412 (IC base=+0.235)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.212 (n=220)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.211)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.223 (n=452)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.228 (n=112)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` > `0.0133` → IC=+0.131 (n=908)
...
- **FILTRO** `sigma_ewma_delta_pct` > `9.271` → IC=-0.300 (n=18)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.080 (n=417)
...
- **PATRÓN** `sigma_h` < `0.0106` → IC=+0.120 (n=443)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.60€ cuando `sigma_h` < 0.0106 (IC base=+0.108)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.145 (n=480)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` > `0.0267` → IC=+0.171 (n=247)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.202 (n=273)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.004` → IC=+0.131 (n=147)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` > 3.004 (IC base=+0.152)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.771` → IC=+0.139 (n=405)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` < 3.771 (IC base=+0.152)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.143 (n=592)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.7309` → IC=+0.135 (n=1550)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.7309 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.174` → IC=+0.157 (n=307)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 6.174 (IC base=+0.133)
...
- **PATRÓN** `dist_vwap_pct` > `0.6179` → IC=+0.163 (n=81)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.6179 (IC base=+0.092)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.127` → IC=+0.194 (n=106)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 7.127 (IC base=+0.092)
...
- **PATRÓN** `sigma_h` < `0.0082` → IC=+0.183 (n=162)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0082 (IC base=+0.104)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.148 (n=140)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 6.0 (IC base=+0.104)

- **PATRÓN** `dist_vwap_pct` < `0.7304` → IC=+0.124 (n=344)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.7304 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.477` → IC=+0.124 (n=336)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 7.477 (IC base=+0.104)
...
- **PATRÓN** `dist_vwap_pct` < `0.1071` → IC=+0.133 (n=794)
...
- **FILTRO** `sigma_ewma_delta_pct` > `6.945` → IC=-0.152 (n=21)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.088 (n=338)
...
- **PATRÓN** `sigma_h` < `0.0109` → IC=+0.125 (n=326)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` < 0.0109 (IC base=+0.063)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.152 (n=133)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` < `0.0262` → IC=+0.179 (n=499)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0262 (IC base=+0.177)

- **PATRÓN** `sigma_h` > `0.0085` → IC=+0.179 (n=500)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.200 (n=181)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.177)

- **PATRÓN** `dist_vwap_pct` > `0.1933` → IC=+0.236 (n=123)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1933 (IC base=+0.177)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.27` → IC=+0.176 (n=486)
...
- **PATRÓN** `dist_vwap_pct` > `0.4501` → IC=+0.162 (n=63)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.4501 (IC base=+0.051)
...
| ✅ FAVORITO_CONFIRMADO | 5304 | +0.187 | +88.13€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 3477 | +0.215 | +86.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 229 | +0.054 | -14.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 677 | +0.119 | -65.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 921 | +0.165 | +82.28€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 1554 | +0.193 | +32.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1161 | +0.208 | -11.89€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 76 | +0.051 | -7.27€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 307 | +0.186 | +55.55€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#ETH | 1873 | +0.179 | +20.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1144 | +0.215 | +41.99€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 76 | -0.026 | -17.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 346 | +0.126 | -26.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 307 | +0.154 | +22.50€ | 0 | 11 |
| ✅ FAVORITO_CONFIRMADO#SOL | 1848 | +0.190 | +30.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1147 | +0.223 | +55.96€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 77 | +0.133 | +9.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 317 | +0.118 | -39.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 307 | +0.154 | +4.22€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 7052 | +0.097 | +2218.98€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 7052 | +0.097 | +2218.98€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 