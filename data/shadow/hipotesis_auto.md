# Hipótesis automáticas — 2026-07-15 22:15 UTC
_Generado por shadow_postmortem.py sobre 15996 resoluciones (PNL=+3251.66€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.194 (n=773)
...
- **PATRÓN** `py_entrada` > `0.695` → IC=+0.295 (n=384)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.191)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.194 (n=965)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.208 (n=166)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.615` → IC=+0.259 (n=164)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.246 (n=179)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `5330.0456` → IC=+0.235 (n=81)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.273 (n=64)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1656.6048` → IC=+0.226 (n=177)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1656.6048 (IC base=+0.227)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.121 (n=1498)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` > `0.0232` → IC=+0.192 (n=196)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0232 (IC base=+0.150)
...
- **PATRÓN** `dist_vwap_pct` > `0.6854` → IC=+0.227 (n=31)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6854 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.281` → IC=+0.192 (n=24)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 9.281 (IC base=+0.150)
...
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.223 (n=316)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0108 (IC base=+0.162)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.174 (n=464)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 13.0 (IC base=+0.162)
...
- **PATRÓN** `dist_vwap_pct` > `0.3709` → IC=+0.209 (n=170)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3709 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.431` → IC=+0.288 (n=102)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.431 (IC base=+0.162)
...
- **PATRÓN** `sigma_h` < `0.0179` → IC=+0.273 (n=148)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0179 (IC base=+0.259)

- **PATRÓN** `sigma_h` > `0.0101` → IC=+0.260 (n=148)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.9145` → IC=+0.364 (n=20)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9145 (IC base=+0.259)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0149` → IC=+0.193 (n=213)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0149 (IC base=+0.112)
...
- **PATRÓN** `dist_vwap_pct` > `0.7123` → IC=+0.184 (n=112)
  ...
  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.7123 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` < `0.1434` → IC=+0.151 (n=213)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.1434 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.717` → IC=+0.323 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.717 (IC base=+0.112)
...
- **FILTRO** `sigma_ewma_delta_pct` < `7.065` → IC=-0.150 (n=18)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.357 (n=40)
...
- **PATRÓN** `sigma_h` > `0.0134` → IC=+0.143 (n=68)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` > 0.0134 (IC base=+0.080)
...
- **PATRÓN** `dist_vwap_pct` > `0.6196` → IC=+0.150 (n=18)
  ...
  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.6196 (IC base=+0.080)

- **PATRÓN** `dist_vwap_pct` < `0.6196` → IC=+0.134 (n=69)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.6196 (IC base=+0.080)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.065` → IC=+0.357 (n=40)
...
- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.227 (n=185)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.1821` → IC=+0.258 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1821 (IC base=+0.208)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.304` → IC=+0.217 (n=44)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.304 (IC base=+0.208)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.065` → IC=+0.224 (n=85)
...
- **PATRÓN** `hora_utc` > `14.0` → IC=+0.121 (n=27)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` > 14.0 (IC base=+0.044)

- **PATRÓN** `py_entrada` > `0.505` → IC=+0.121 (n=27)

  - _Acción_: Kelly boost +0.60€ cuando `py_entrada` > 0.505 (IC base=+0.044)
...
- **PATRÓN** `dist_vwap_pct` > `0.7448` → IC=+0.186 (n=33)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.7448 (IC base=+0.028)

- **PATRÓN** `dist_vwap_pct` < `0.5087` → IC=+0.151 (n=164)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.5087 (IC base=+0.028)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.541` → IC=+0.151 (n=41)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2311 | +0.176 | -35.12€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 1488 | +0.211 | +20.33€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 671 | +0.182 | -9.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 498 | +0.202 | -15.85€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 822 | +0.176 | +6.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 493 | +0.219 | +24.81€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 811 | +0.172 | -28.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 490 | +0.213 | +15.10€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 20 | +0.318 | +2.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 20 | +0.318 | +2.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 20 | +0.318 | +2.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 20 | +0.318 | +2.11€ | 0 | 0 |
| ✅ GBM_LATE_15M | 4701 | +0.103 | +1460.59€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 4701 | +0.103 | +1460.59€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1129 | +0.065 | +160.74€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1129 | +0.065 | +160.74€ | 0 | 3 |
| ✅ GBM_LATE_15M#ETH | 1136 | +0.076 | +195.11€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1136 | +0.076 | +195.11€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1253 | +0.110 | +502.90€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1253 | +0.110 | +502.90€ | 0 | 6 |
| ✅ GBM_LATE_15M#XRP | 1183 | +0.157 | +601.84€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1183 | +0.157 | +601.84€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1948 | +0.151 | +1130.92€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1948 | +0.151 | +1130.92€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 499 | +0.113 | +240.68€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 499 | +0.113 | +240.68€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 516 | +0.127 | +233.68€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 516 | +0.127 | +233.68€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 525 | +0.113 | +229.76€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 525 | +0.113 | +229.76€ | 1 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 408 | +0.276 | +426.80€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 408 | +0.276 | +426.80€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 1907 | +0.089 | +568.04€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1907 | +0.089 | +568.04€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 474 | +0.061 | +87.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 474 | +0.061 | +87.60€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 433 | +0.015 | +10.99€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 433 | +0.015 | +10.99€ | 0 | 2 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 458 | +0.054 | +82.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 458 | +0.054 | +82.60€ | 2 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 542 | +0.200 | +386.85€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 542 | +0.200 | +386.85€ | 0 | 11 |
...
| ✅ LEADLAG_BTC_XRP_15M | 129 | +0.088 | +25.86€ | 0 | 5 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 129 | +0.088 | +25.86€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 129 | +0.088 | +25.86€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 129 | +0.088 | +25.86€ | 0 | 5 |
...
| ✅ UPDOWN_GBM | 1567 | +0.011 | +74.74€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1309 | +0.038 | +111.78€ | 0 | 5 |
| 🚫 UPDOWN_GBM#240min | 11 | -0.148 | -4.31€ | 0 | 0 |
| 🚫 UPDOWN_GBM#5min | 75 | -0.162 | -22.48€ | 4 | 0 |
| ✅ UPDOWN_GBM#60min | 127 | -0.058 | -10.76€ | 7 | 0 |
| ✅ UPDOWN_GBM#BNB | 78 | +0.087 | +19.90€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 78 | +0.087 | +19.90€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 61 | +0.246 | +23.77€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 61 | +0.246 | +23.77€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.135) — sin ventaja clara. oversold(IBS<0.3): IC=+0.007 n=456 | neutral: IC=-0.014 n=350 | overbought(IBS>0.7): IC=+0.120 n=372
  - _Datos_: n=1245 IC=+0.041 PNL=+124.15€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9276 | tras_1loss IC=+0.066 n=6506 | tras_2loss IC=+0.032 n=2795/40 | gap=+0.070 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1440 IC=+0.017 PNL=+88.74€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1440 IC=+0.017 PNL=+88.74€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=695 IC=+0.028 PNL=+47.12€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=695 IC=+0.028 PNL=+47.12€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=691 IC=+0.074 PNL=+89.06€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=691 IC=+0.074 PNL=+89.06€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=13538 IC=+0.105 PNL=+3327.55€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=13538 IC=+0.105 PNL=+3327.55€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=843 IC=-0.003 PNL=+17.59€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=843 IC=-0.003 PNL=+17.59€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.170 > 0.1 con n=395 PNL=+142.11€
  - _Datos_: n=395 IC=+0.170 PNL=+142.11€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.152 > 0.08 con n=291 PNL=+93.37€
  - _Datos_: n=291 IC=+0.152 PNL=+93.37€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.258 > 0.2 con n=328 PNL=+230.23€
  - _Datos_: n=328 IC=+0.258 PNL=+230.23€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.148 > 0.08 con n=143 PNL=+52.63€
  - _Datos_: n=143 IC=+0.148 PNL=+52.63€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.203 > 0.08 con n=116 PNL=+2.99€
  - _Datos_: n=116 IC=+0.203 PNL=+2.99€
