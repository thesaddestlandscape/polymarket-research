# Hipótesis automáticas — 2026-07-03 02:52 UTC
_Generado por shadow_postmortem.py sobre 2915 resoluciones (PNL=-49.47€)_
...
- **PATRÓN** `sigma_h` < `0.0101` → IC=+0.227 (n=75)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0101 (IC base=+0.046)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.122 (n=43)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 15.0 (IC base=+0.046)

- **PATRÓN** `ibs_15` < `0.0613` → IC=+0.173 (n=53)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.86€ cuando `ibs_15` < 0.0613 (IC base=+0.046)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1297` → IC=+0.161 (n=54)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.80€ cuando `pct_spot_vs_ref` |x|≤ 0.1297 (IC base=+0.130)

- **PATRÓN** `sigma_h` < `0.0144` → IC=+0.161 (n=54)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0144 (IC base=+0.130)

- **PATRÓN** `drift_60min` |x|≤ `1.124` → IC=+0.154 (n=50)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 1.124 (IC base=+0.130)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.262 (n=19)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.130)

- **PATRÓN** `ibs_15` < `0.0787` → IC=+0.293 (n=27)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0787 (IC base=+0.130)
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.0787 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.293 n=27). Confirma señal de reversión media → alinear con BUY_YES.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 277 | +0.070 | +38.99€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 277 | +0.070 | +38.99€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 73 | +0.033 | +4.16€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 73 | +0.033 | +4.16€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 73 | -0.007 | +0.76€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 73 | -0.007 | +0.76€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL | 67 | +0.094 | +11.08€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 67 | +0.094 | +11.08€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP | 64 | +0.167 | +23.00€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 64 | +0.167 | +23.00€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 845 | -0.038 | -43.60€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 616 | -0.016 | -16.27€ | 1 | 3 |
...
| ✅ UPDOWN_GBM#ETH | 329 | -0.008 | +11.20€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 245 | +0.010 | +17.67€ | 3 | 5 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.061) — sin ventaja clara. oversold(IBS<0.3): IC=-0.003 n=153 | neutral: IC=-0.064 n=138 | overbought(IBS>0.7): IC=-0.043 n=149
  - _Datos_: n=455 IC=-0.036 PNL=-18.37€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=01h: IC=-0.200 n=28 PNL=-13.74€ → FILTRAR | H=02h: IC=-0.167 n=22 PNL=-4.05€ → FILTRAR | H=10h: IC=-0.154 n=15 PNL=-6.77€ → FILTRAR | H=15h: IC=-0.109 n=21 PNL=-6.63€ → FILTRAR | H=16h: IC=+0.140 n=23 PNL=+3.65€ → BOOST | H=20h: IC=-0.143 n=26 PNL=-5.82€ → FILTRAR | H=23h: IC=+0.180 n=18 PNL=+12.32€ → BOOST
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.007 n=1393 | tras_1loss IC=-0.028 n=1431 | tras_2loss IC=-0.045 n=736/40 | gap=+0.052 (umbral 0.05)
...
**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  ...
  - _Estado_: n_overlaps=25, boost estimado=+0.007. Necesita 0 más y boost>0.05
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=722 IC=-0.033 PNL=-30.05€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=722 IC=-0.033 PNL=-30.05€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=261 IC=-0.013 PNL=-30.80€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=261 IC=-0.013 PNL=-30.80€
...
**🟡 H-CUSTOM-ETH15-REVERSION** — ETH#15min con drift_15min < -1 — ¿mean reversion?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.081 > 0.08 con n=41 PNL=+12.58€
  - _Datos_: n=41 IC=+0.081 PNL=+12.58€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=207 IC=+0.021 PNL=+18.52€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=207 IC=+0.021 PNL=+18.52€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=489 IC=+0.046 PNL=+40.69€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=489 IC=+0.046 PNL=+40.69€
...
**⏳ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: 39/40 ops en el filtro definido (IC actual=+0.012 PNL=-3.70€)
  - _Datos_: n=39 IC=+0.012 PNL=-3.70€
