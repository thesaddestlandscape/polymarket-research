# Hipótesis automáticas — 2026-08-25 16:44 UTC
_Generado por shadow_postmortem.py sobre 152745 resoluciones (PNL=+10134.72€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `148.79` → IC=-0.278 (n=2143)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 148.79
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=6431)
...
- **FILTRO** `restante_s_al_confirmar` < `85.16` → IC=-0.450 (n=256)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 85.16
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=768)

### BALLENAS_TARDIAS#ETH#5min
- **FILTRO** `restante_s_al_confirmar` < `158.25` → IC=-0.198 (n=578)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.25
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=1737)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `150.33` → IC=-0.281 (n=523)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 150.33
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=1570)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.190 (n=4938)

  - _Acción_: Kelly boost +0.95€ cuando `py_entrada` > 0.7 (IC base=+0.099)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.175 (n=1495)

  - _Acción_: Kelly boost +0.87€ cuando `libro_spread` < 0.01 (IC base=+0.099)

- **PATRÓN** `libro_liquidez` > `2382.8902` → IC=+0.177 (n=1443)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 2382.8902 (IC base=+0.099)
...
- **PATRÓN** `py_entrada` < `0.34` → IC=+0.258 (n=3007)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.34 (IC base=+0.138)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.187 (n=2769)

  - _Acción_: Kelly boost +0.93€ cuando `libro_spread` < 0.02 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `1813.4701` → IC=+0.177 (n=2310)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 1813.4701 (IC base=+0.138)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.227 (n=584)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.216 (n=723)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.211)

- **PATRÓN** `libro_liquidez` > `12031.2986` → IC=+0.213 (n=190)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12031.2986 (IC base=+0.211)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.192 (n=524)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.325` → IC=+0.284 (n=397)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.325 (IC base=+0.182)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.183 (n=751)

  - _Acción_: Kelly boost +0.91€ cuando `libro_spread` < 0.01 (IC base=+0.182)

- **PATRÓN** `libro_liquidez` > `11733.6925` → IC=+0.189 (n=194)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 11733.6925 (IC base=+0.182)

### FAVORITO_CONFIRMADO#BTC#60min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.136 (n=512)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.136 (n=1123)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.304 (n=376)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.198 (n=892)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `841.0684` → IC=+0.232 (n=561)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 841.0684 (IC base=+0.216)
...
- **PATRÓN** `py_entrada` < `0.335` → IC=+0.210 (n=150)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.195 (n=4178)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.202 (n=2035)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.167 (n=1067)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.197 (n=1019)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.7` → IC=+0.199 (n=503)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.7 (IC base=+0.190)

### FAVORITO_CONFIRMADO_15MIN_EXTREMO
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.453 (n=191)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `3299.2146` → IC=+0.464 (n=137)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.197 (n=10865)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.222 (n=7650)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.192)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.129 (n=2096)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.214 (n=656)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.76 (IC base=+0.163)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.244 (n=1533)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.288 (n=608)
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.249 (n=1044)
...
- **PATRÓN** `lag_apertura_s` < `4.54` → IC=+0.156 (n=1306)

  - _Acción_: Kelly boost +0.78€ cuando `lag_apertura_s` < 4.54 (IC base=+0.131)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.204 (n=749)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.38 (IC base=+0.138)

- **PATRÓN** `restante_min` < `3.88` → IC=+0.145 (n=649)

  - _Acción_: Kelly boost +0.73€ cuando `restante_min` < 3.88 (IC base=+0.138)

- **PATRÓN** `restante_min` > `4.9` → IC=+0.171 (n=676)
...
- **PATRÓN** `lag_apertura_s` < `5.77` → IC=+0.175 (n=647)

  - _Acción_: Kelly boost +0.87€ cuando `lag_apertura_s` < 5.77 (IC base=+0.138)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.189 (n=658)

  - _Acción_: Kelly boost +0.95€ cuando `py_entrada` < 0.37 (IC base=+0.123)

- **PATRÓN** `restante_min` < `3.95` → IC=+0.138 (n=659)

  - _Acción_: Kelly boost +0.69€ cuando `restante_min` < 3.95 (IC base=+0.123)

- **PATRÓN** `restante_min` > `4.94` → IC=+0.153 (n=707)

  - _Acción_: Kelly boost +0.77€ cuando `restante_min` > 4.94 (IC base=+0.123)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.124 (n=2057)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `3.47` → IC=+0.155 (n=656)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.297 (n=269)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `821.9118` → IC=+0.295 (n=301)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 821.9118 (IC base=+0.282)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9462` → IC=+0.214 (n=663)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9462 (IC base=+0.069)

- **PATRÓN** `dist_vwap_pct` > `0.1985` → IC=+0.238 (n=280)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1985 (IC base=+0.069)

- **PATRÓN** `dist_vwap_pct` < `0.992` → IC=+0.221 (n=438)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.992 (IC base=+0.069)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.077` → IC=+0.181 (n=822)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 5.077 (IC base=+0.069)

- **PATRÓN** `volumen_regimen` < `0.6304` → IC=+0.231 (n=139)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6304 (IC base=+0.069)

- **PATRÓN** `volumen_regimen` > `1.0807` → IC=+0.264 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0807 (IC base=+0.069)

- **PATRÓN** `volumen_pendiente_norm` < `0.107` → IC=+0.134 (n=993)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_pendiente_norm` < 0.107 (IC base=+0.069)

- **PATRÓN** `volumen_pendiente_norm` > `0.3056` → IC=+0.140 (n=145)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_pendiente_norm` > 0.3056 (IC base=+0.069)

- **PATRÓN** `volumen_spike_ratio` < `1.5818` → IC=+0.146 (n=458)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 1.5818 (IC base=+0.069)

- **PATRÓN** `volumen_spike_ratio` > `1.9516` → IC=+0.133 (n=693)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 1.9516 (IC base=+0.069)

- **PATRÓN** `ibs_20min` < `0.2116` → IC=+0.124 (n=1376)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` < 0.2116 (IC base=+0.028)

- **PATRÓN** `dist_vwap_pct` < `0.1787` → IC=+0.151 (n=665)
...
- **PATRÓN** `ibs_20min` < `0.4739` → IC=+0.336 (n=187)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4739 (IC base=+0.286)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7987` → IC=+0.342 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7987 (IC base=+0.286)

- **PATRÓN** `volumen_spike_ratio` > `1.4634` → IC=+0.295 (n=110)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4634 (IC base=+0.286)
...
- **PATRÓN** `ballena_activa_n` < `39.0` → IC=+0.379 (n=31)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 39.0 (IC base=+0.286)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3368` → IC=+0.220 (n=284)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3368 (IC base=+0.219)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.248 (n=323)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9595` → IC=+0.243 (n=146)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9595 (IC base=+0.219)

- **PATRÓN** `dist_vwap_pct` > `0.2345` → IC=+0.237 (n=226)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2345 (IC base=+0.219)

- **PATRÓN** `dist_vwap_pct` < `1.0067` → IC=+0.222 (n=332)
...
- **PATRÓN** `volumen_regimen` < `1.2894` → IC=+0.219 (n=322)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2894 (IC base=+0.219)

- **PATRÓN** `volumen_regimen` > `1.1032` → IC=+0.250 (n=146)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1032 (IC base=+0.219)

- **PATRÓN** `volumen_pendiente_norm` < `0.0769` → IC=+0.229 (n=286)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4524` → IC=+0.261 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4524 (IC base=+0.219)

- **PATRÓN** `libro_liquidez` > `11109.7462` → IC=+0.234 (n=288)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11109.7462 (IC base=+0.219)

- **PATRÓN** `ballena_activa_n` < `371.0` → IC=+0.222 (n=217)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 371.0 (IC base=+0.219)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7479` → IC=+0.182 (n=196)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 1.7479 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` > `1.4122` → IC=+0.159 (n=294)
...
- **PATRÓN** `libro_liquidez` > `12702.6869` → IC=+0.170 (n=180)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 12702.6869 (IC base=+0.138)
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.187 (n=132)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.924` → IC=+0.282 (n=108)
...
- **PATRÓN** `libro_liquidez` > `1916.3861` → IC=+0.146 (n=179)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 1916.3861 (IC base=+0.129)

- **PATRÓN** `ballena_activa_n` < `28.0` → IC=+0.142 (n=79)

  - _Acción_: Kelly boost +0.71€ cuando `ballena_activa_n` < 28.0 (IC base=+0.129)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0885` → IC=+0.304 (n=90)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0885 (IC base=+0.275)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.281 (n=181)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5146` → IC=+0.309 (n=270)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5146 (IC base=+0.275)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.013` → IC=+0.292 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.013 (IC base=+0.275)

- **PATRÓN** `volumen_pendiente_norm` > `0.4178` → IC=+0.417 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.4178 (IC base=+0.275)

- **PATRÓN** `volumen_spike_ratio` < `2.0427` → IC=+0.261 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0427 (IC base=+0.275)

- **PATRÓN** `volumen_spike_ratio` > `2.929` → IC=+0.260 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.929 (IC base=+0.275)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.297 (n=121)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.275)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.243 (n=68)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 35.0 (IC base=+0.275)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.145 (n=60)
...
- **PATRÓN** `dist_vwap_pct` < `0.0784` → IC=+0.346 (n=24)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0784 (IC base=-0.029)
...
- **FILTRO** `ibs_20min` > `0.48` → IC=-0.120 (n=601)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.025 (n=605)
...
- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.194 (n=34)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0056 (IC base=+0.063)

- **PATRÓN** `drift_60min` |x|≤ `0.3737` → IC=+0.138 (n=67)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.69€ cuando `drift_60min` |x|≤ 0.3737 (IC base=+0.063)

- **PATRÓN** `hora_utc` > `23.0` → IC=+0.250 (n=30)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 23.0 (IC base=+0.063)

- **PATRÓN** `ibs_20min` > `0.5263` → IC=+0.160 (n=51)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.5263 (IC base=+0.063)
...
- **FILTRO** `ibs_20min` < `0.4466` → IC=-0.152 (n=303)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.121 (n=93)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=513)

- **FILTRO** `ibs_20min` > `0.77` → IC=-0.172 (n=269)
...
- **PATRÓN** `sigma_h` > `0.0073` → IC=+0.133 (n=1156)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0073 (IC base=+0.054)

- **PATRÓN** `ibs_20min` > `0.9461` → IC=+0.245 (n=846)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.09` → IC=+0.122 (n=1008)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` > 5.09 (IC base=+0.054)

- **PATRÓN** `volumen_regimen` > `1.164` → IC=+0.196 (n=294)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` > 1.164 (IC base=+0.054)

- **PATRÓN** `volumen_pendiente_norm` < `0.1152` → IC=+0.166 (n=1108)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` < 0.1152 (IC base=+0.054)

- **PATRÓN** `volumen_pendiente_norm` > `0.2458` → IC=+0.212 (n=220)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2458 (IC base=+0.054)

- **PATRÓN** `volumen_spike_ratio` < `1.4848` → IC=+0.189 (n=380)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` < 1.4848 (IC base=+0.054)

- **PATRÓN** `volumen_spike_ratio` > `2.8753` → IC=+0.173 (n=380)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 2.8753 (IC base=+0.054)

- **PATRÓN** `ballena_activa_n` < `102.0` → IC=+0.270 (n=586)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 102.0 (IC base=+0.054)

- **PATRÓN** `ibs_20min` < `0.097` → IC=+0.174 (n=1098)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.097 (IC base=+0.036)

- **PATRÓN** `dist_vwap_pct` > `0.642` → IC=+0.219 (n=144)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.642 (IC base=+0.036)

- **PATRÓN** `dist_vwap_pct` < `0.1458` → IC=+0.194 (n=652)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` < 0.1458 (IC base=+0.036)

- **PATRÓN** `volumen_regimen` > `0.6311` → IC=+0.207 (n=697)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6311 (IC base=+0.036)

- **PATRÓN** `volumen_pendiente_norm` > `0.2476` → IC=+0.341 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2476 (IC base=+0.036)

- **PATRÓN** `volumen_spike_ratio` > `2.9244` → IC=+0.278 (n=174)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9244 (IC base=+0.036)

- **PATRÓN** `ballena_activa_n` < `58.0` → IC=+0.251 (n=360)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.447` → IC=+0.155 (n=137)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 3.447 (IC base=-0.015)
...
- **PATRÓN** `volumen_regimen` > `0.9983` → IC=+0.129 (n=33)
  ...
  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` > 0.9983 (IC base=-0.040)

### GBM_LATE_15M_ESPACIO_ATR#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.286 (n=129)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0076 (IC base=+0.180)

- **PATRÓN** `drift_60min` |x|≤ `0.0626` → IC=+0.215 (n=128)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0626 (IC base=+0.180)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.263 (n=137)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.180)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.294 (n=192)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.180)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.94` → IC=+0.297 (n=116)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.94 (IC base=+0.180)

- **PATRÓN** `volumen_pendiente_norm` < `0.1458` → IC=+0.188 (n=280)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` < 0.1458 (IC base=+0.180)

- **PATRÓN** `volumen_pendiente_norm` > `0.4225` → IC=+0.233 (n=43)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.4225 (IC base=+0.180)

- **PATRÓN** `volumen_spike_ratio` < `2.0838` → IC=+0.174 (n=133)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.0838 (IC base=+0.180)

- **PATRÓN** `volumen_spike_ratio` > `3.8943` → IC=+0.212 (n=137)
...
- **PATRÓN** `libro_liquidez` > `1912.1779` → IC=+0.204 (n=174)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1912.1779 (IC base=+0.180)

- **PATRÓN** `ballena_activa_n` < `37.0` → IC=+0.225 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 37.0 (IC base=+0.180)

- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.405 (n=82)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0069 (IC base=+0.367)

- **PATRÓN** `drift_60min` |x|≤ `0.1856` → IC=+0.377 (n=120)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1856 (IC base=+0.367)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.388 (n=168)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.367)

- **PATRÓN** `ibs_20min` < `0.2398` → IC=+0.381 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2398 (IC base=+0.367)

- **PATRÓN** `volumen_pendiente_norm` < `0.1045` → IC=+0.381 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1045 (IC base=+0.367)
...
- **PATRÓN** `volumen_spike_ratio` < `2.9594` → IC=+0.418 (n=59)
...
- **PATRÓN** `ballena_activa_n` < `43.0` → IC=+0.365 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 43.0 (IC base=+0.367)
...
- **FILTRO** `ibs_20min` < `0.5294` → IC=-0.146 (n=283)

  - _Acción_: SKIP cuando `ibs_20min` < 0.5294
  - _Potencial_: sin este filtro IC_bueno=+0.136 (n=289)

- **FILTRO** `ibs_20min` > `0.7333` → IC=-0.153 (n=214)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7333
  - _Potencial_: sin este filtro IC_bueno=+0.024 (n=643)
...
- **PATRÓN** `ibs_20min` > `0.85` → IC=+0.245 (n=143)
...
- **PATRÓN** `volumen_regimen` < `0.8696` → IC=+0.167 (n=109)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.8696 (IC base=-0.004)

- **PATRÓN** `volumen_regimen` > `1.1437` → IC=+0.179 (n=54)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` > 1.1437 (IC base=-0.004)

- **PATRÓN** `volumen_pendiente_norm` > `0.2642` → IC=+0.292 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2642 (IC base=-0.004)

- **PATRÓN** `volumen_spike_ratio` < `1.4987` → IC=+0.179 (n=51)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 1.4987 (IC base=-0.004)

- **PATRÓN** `volumen_spike_ratio` > `1.586` → IC=+0.154 (n=134)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.586 (IC base=-0.004)

- **PATRÓN** `ballena_activa_n` < `77.0` → IC=+0.203 (n=116)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 77.0 (IC base=-0.004)
...
- **PATRÓN** `ibs_20min` > `0.9048` → IC=+0.287 (n=345)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9048 (IC base=+0.209)

- **PATRÓN** `dist_vwap_pct` > `1.3033` → IC=+0.332 (n=129)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.3033 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.12` → IC=+0.270 (n=306)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.001` → IC=+0.261 (n=520)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.001 (IC base=+0.260)
...
- **PATRÓN** `sigma_h` > `0.0066` → IC=+0.182 (n=1459)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` > 0.0066 (IC base=+0.156)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.163 (n=2291)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.156)

- **PATRÓN** `ibs_20min` > `0.893` → IC=+0.258 (n=1459)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.893 (IC base=+0.156)

- **PATRÓN** `dist_vwap_pct` > `0.7903` → IC=+0.252 (n=539)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7903 (IC base=+0.156)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.235` → IC=+0.256 (n=937)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.235 (IC base=+0.156)

- **PATRÓN** `volumen_regimen` < `1.23` → IC=+0.162 (n=1524)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 1.23 (IC base=+0.156)

- **PATRÓN** `volumen_regimen` > `0.691` → IC=+0.173 (n=1361)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` > 0.691 (IC base=+0.156)

- **PATRÓN** `volumen_pendiente_norm` > `0.104` → IC=+0.185 (n=778)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.104 (IC base=+0.156)

- **PATRÓN** `volumen_spike_ratio` < `2.2957` → IC=+0.158 (n=1710)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.2957 (IC base=+0.156)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.168 (n=1785)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.01 (IC base=+0.156)

- **PATRÓN** `libro_liquidez` > `3740.9246` → IC=+0.197 (n=730)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 3740.9246 (IC base=+0.156)

- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.180 (n=969)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 135.0 (IC base=+0.156)

- **PATRÓN** `sigma_h` < `0.0081` → IC=+0.198 (n=1907)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0081 (IC base=+0.182)

- **PATRÓN** `drift_60min` |x|≤ `0.3742` → IC=+0.198 (n=1901)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.3742 (IC base=+0.182)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.204 (n=862)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.4` → IC=+0.239 (n=1901)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4 (IC base=+0.182)

- **PATRÓN** `dist_vwap_pct` < `0.2104` → IC=+0.178 (n=1566)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.2104 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.806` → IC=+0.206 (n=366)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.806 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` < `1.1737` → IC=+0.163 (n=1579)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 1.1737 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` > `0.854` → IC=+0.175 (n=1052)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 0.854 (IC base=+0.182)

- **PATRÓN** `volumen_pendiente_norm` > `0.2897` → IC=+0.230 (n=213)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2897 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` < `1.5799` → IC=+0.182 (n=570)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 1.5799 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` > `2.6536` → IC=+0.196 (n=432)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 2.6536 (IC base=+0.182)

- **PATRÓN** `ballena_activa_n` < `218.0` → IC=+0.169 (n=860)

  - _Acción_: Kelly boost +0.85€ cuando `ballena_activa_n` < 218.0 (IC base=+0.182)

### GBM_LATE_15M_MULTIHORIZONTE#BNB#15min
- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.225 (n=158)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.321 (n=160)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.572` → IC=+0.355 (n=129)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1402` → IC=+0.159 (n=80)
  ...
  - _Acción_: Kelly boost +0.79€ cuando `volumen_pendiente_norm` > 0.1402 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` > `1.4192` → IC=+0.132 (n=270)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 1.4192 (IC base=+0.149)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.187 (n=177)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.04 (IC base=+0.149)

- **PATRÓN** `sigma_h` < `0.006` → IC=+0.296 (n=101)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.2245` → IC=+0.321 (n=132)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2245 (IC base=+0.286)

- **PATRÓN** `hora_utc` < `9.0` → IC=+0.343 (n=100)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 9.0 (IC base=+0.286)

- **PATRÓN** `ibs_20min` < `0.0253` → IC=+0.385 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0253 (IC base=+0.286)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.639` → IC=+0.297 (n=161)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.639 (IC base=+0.286)

- **PATRÓN** `volumen_pendiente_norm` < `0.0882` → IC=+0.306 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0882 (IC base=+0.286)

- **PATRÓN** `volumen_spike_ratio` < `1.8747` → IC=+0.385 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8747 (IC base=+0.286)

- **PATRÓN** `libro_spread` < `0.05` → IC=+0.355 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.05 (IC base=+0.286)

- **PATRÓN** `libro_liquidez` > `1982.2745` → IC=+0.365 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1982.2745 (IC base=+0.286)
...
- **PATRÓN** `volumen_regimen` < `0.6383` → IC=+0.202 (n=102)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6383 (IC base=+0.187)

- **PATRÓN** `volumen_regimen` > `0.9006` → IC=+0.211 (n=202)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.9006 (IC base=+0.187)
...
- **PATRÓN** `libro_liquidez` > `11847.0948` → IC=+0.226 (n=202)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11847.0948 (IC base=+0.187)

- **PATRÓN** `ballena_activa_n` < `372.0` → IC=+0.191 (n=150)

  - _Acción_: Kelly boost +0.95€ cuando `ballena_activa_n` < 372.0 (IC base=+0.187)

- **PATRÓN** `sigma_h` < `0.0044` → IC=+0.190 (n=349)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.0044 (IC base=+0.151)

- **PATRÓN** `drift_60min` |x|≤ `0.2835` → IC=+0.180 (n=395)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.90€ cuando `drift_60min` |x|≤ 0.2835 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.171 (n=363)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 7.0 (IC base=+0.151)

- **PATRÓN** `ibs_20min` < `0.3843` → IC=+0.190 (n=395)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` < 0.3843 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` < `0.1603` → IC=+0.181 (n=381)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.1603 (IC base=+0.151)
...
- **PATRÓN** `volumen_regimen` < `0.6357` → IC=+0.216 (n=132)
...
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.185 (n=106)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0077 (IC base=+0.145)

- **PATRÓN** `drift_60min` |x|≤ `0.144` → IC=+0.156 (n=213)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.144 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.153 (n=122)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 16.0 (IC base=+0.145)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.904` → IC=+0.305 (n=85)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.904 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` < `0.2311` → IC=+0.132 (n=256)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_pendiente_norm` < 0.2311 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` < `2.0183` → IC=+0.198 (n=114)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 2.0183 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` > `3.8668` → IC=+0.147 (n=117)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 3.8668 (IC base=+0.145)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.171 (n=253)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.04 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `1961.8084` → IC=+0.167 (n=106)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 1961.8084 (IC base=+0.145)

- **PATRÓN** `sigma_h` < `0.0077` → IC=+0.312 (n=174)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0077 (IC base=+0.294)

- **PATRÓN** `drift_60min` |x|≤ `0.1721` → IC=+0.339 (n=116)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1721 (IC base=+0.294)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.315 (n=117)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 12.0 (IC base=+0.294)

- **PATRÓN** `ibs_20min` < `0.3636` → IC=+0.324 (n=174)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3636 (IC base=+0.294)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.108` → IC=+0.329 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.108 (IC base=+0.294)

- **PATRÓN** `volumen_pendiente_norm` > `0.404` → IC=+0.447 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.404 (IC base=+0.294)

- **PATRÓN** `volumen_spike_ratio` < `4.276` → IC=+0.289 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 4.276 (IC base=+0.294)

- **PATRÓN** `volumen_spike_ratio` > `1.936` → IC=+0.292 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.936 (IC base=+0.294)

### GBM_LATE_15M_MULTIHORIZONTE#ETH#15min
- **PATRÓN** `sigma_h` < `0.0078` → IC=+0.234 (n=269)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0078 (IC base=+0.188)

- **PATRÓN** `drift_60min` |x|≤ `0.4992` → IC=+0.203 (n=304)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4992 (IC base=+0.188)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.211 (n=309)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.188)

- **PATRÓN** `ibs_20min` > `0.4502` → IC=+0.248 (n=304)
...
- **PATRÓN** `volumen_regimen` > `0.6382` → IC=+0.209 (n=304)
...
- **PATRÓN** `volumen_spike_ratio` > `2.4571` → IC=+0.230 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4571 (IC base=+0.188)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.189 (n=342)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.01 (IC base=+0.188)

- **PATRÓN** `libro_liquidez` > `8422.3422` → IC=+0.198 (n=203)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 8422.3422 (IC base=+0.188)

- **PATRÓN** `ballena_activa_n` < `111.0` → IC=+0.207 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 111.0 (IC base=+0.188)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.133 (n=456)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 5.0 (IC base=+0.117)

- **PATRÓN** `ibs_20min` > `0.8511` → IC=+0.229 (n=293)
...
- **PATRÓN** `volumen_regimen` > `0.6182` → IC=+0.130 (n=439)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` > 0.6182 (IC base=+0.117)

- **PATRÓN** `volumen_pendiente_norm` > `0.0999` → IC=+0.139 (n=153)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_pendiente_norm` > 0.0999 (IC base=+0.117)

- **PATRÓN** `libro_liquidez` > `2853.2159` → IC=+0.187 (n=199)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 2853.2159 (IC base=+0.117)

- **PATRÓN** `ballena_activa_n` < `44.0` → IC=+0.187 (n=161)
...
- **PATRÓN** `sigma_h` > `0.0219` → IC=+0.199 (n=217)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0219 (IC base=+0.164)

- **PATRÓN** `drift_60min` |x|≤ `0.1598` → IC=+0.176 (n=211)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.1598 (IC base=+0.164)
...
- **PATRÓN** `ibs_20min` > `0.7013` → IC=+0.208 (n=478)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7013 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.9631` → IC=+0.241 (n=168)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9631 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.037` → IC=+0.243 (n=259)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.037 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` < `1.2177` → IC=+0.167 (n=478)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.2177 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` > `0.8308` → IC=+0.189 (n=319)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` > 0.8308 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.0804` → IC=+0.220 (n=184)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0804 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` < `2.1696` → IC=+0.179 (n=388)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 2.1696 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` > `1.8108` → IC=+0.166 (n=294)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.8108 (IC base=+0.164)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.167 (n=536)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.01 (IC base=+0.164)

- **PATRÓN** `sigma_h` < `0.0074` → IC=+0.257 (n=294)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0074 (IC base=+0.219)

- **PATRÓN** `drift_60min` |x|≤ `0.3761` → IC=+0.230 (n=387)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3761 (IC base=+0.219)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.258 (n=209)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.219)

- **PATRÓN** `ibs_20min` < `0.1341` → IC=+0.307 (n=294)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1341 (IC base=+0.219)

- **PATRÓN** `dist_vwap_pct` < `1.0811` → IC=+0.229 (n=525)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 1.0811 (IC base=+0.219)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.822` → IC=+0.267 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.822 (IC base=+0.219)

- **PATRÓN** `volumen_regimen` > `0.7158` → IC=+0.244 (n=393)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7158 (IC base=+0.219)

- **PATRÓN** `volumen_pendiente_norm` > `0.2432` → IC=+0.319 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2432 (IC base=+0.219)

- **PATRÓN** `volumen_spike_ratio` > `2.7756` → IC=+0.264 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7756 (IC base=+0.219)

- **PATRÓN** `ballena_activa_n` < `30.0` → IC=+0.184 (n=223)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 30.0 (IC base=+0.219)

### GBM_LATE_15M_PYCONFIRMADO
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.202 (n=176)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.085)

- **PATRÓN** `ibs_20min` > `0.6176` → IC=+0.155 (n=323)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` > 0.6176 (IC base=+0.085)

- **PATRÓN** `dist_vwap_pct` > `0.7744` → IC=+0.240 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7744 (IC base=+0.085)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.14` → IC=+0.214 (n=110)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.14 (IC base=+0.085)

- **PATRÓN** `volumen_pendiente_norm` > `0.1824` → IC=+0.225 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1824 (IC base=+0.085)

- **PATRÓN** `libro_liquidez` > `2964.2504` → IC=+0.126 (n=241)

  - _Acción_: Kelly boost +0.63€ cuando `libro_liquidez` > 2964.2504 (IC base=+0.085)

- **PATRÓN** `sigma_h` < `0.0033` → IC=+0.230 (n=109)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0033 (IC base=+0.059)

- **PATRÓN** `ibs_20min` < `0.2708` → IC=+0.139 (n=217)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` < 0.2708 (IC base=+0.059)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.783` → IC=+0.211 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.783 (IC base=+0.059)

- **PATRÓN** `volumen_regimen` < `0.727` → IC=+0.122 (n=141)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` < 0.727 (IC base=+0.059)

- **PATRÓN** `volumen_spike_ratio` < `1.6449` → IC=+0.128 (n=119)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` < 1.6449 (IC base=+0.059)

- **PATRÓN** `libro_liquidez` > `3871.0971` → IC=+0.126 (n=217)

  - _Acción_: Kelly boost +0.63€ cuando `libro_liquidez` > 3871.0971 (IC base=+0.059)
...
- **PATRÓN** `sigma_h` < `0.0026` → IC=+0.229 (n=57)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0026 (IC base=+0.163)

- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.183 (n=58)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0049 (IC base=+0.163)

- **PATRÓN** `drift_60min` |x|≤ `0.2802` → IC=+0.204 (n=113)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2802 (IC base=+0.163)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.167 (n=46)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.6346` → IC=+0.192 (n=128)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` < 0.6346 (IC base=+0.163)

- **PATRÓN** `ibs_20min` > `0.0393` → IC=+0.164 (n=114)

  - _Acción_: Kelly boost +0.82€ cuando `ibs_20min` > 0.0393 (IC base=+0.163)

- **PATRÓN** `dist_vwap_pct` > `0.5182` → IC=+0.176 (n=32)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.5182 (IC base=+0.163)

- **PATRÓN** `dist_vwap_pct` < `0.2398` → IC=+0.186 (n=116)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` < 0.2398 (IC base=+0.163)
...
- **PATRÓN** `volumen_regimen` < `1.2361` → IC=+0.192 (n=128)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7551` → IC=+0.220 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7551 (IC base=+0.163)

- **PATRÓN** `volumen_spike_ratio` > `2.0515` → IC=+0.196 (n=54)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 2.0515 (IC base=+0.163)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.204 (n=1150)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0076 (IC base=+0.144)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.168 (n=1264)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 15.0 (IC base=+0.144)

- **PATRÓN** `ibs_20min` > `0.9434` → IC=+0.287 (n=1150)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9434 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `1.0536` → IC=+0.256 (n=441)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0536 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.484` → IC=+0.228 (n=1522)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.484 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` < `0.7048` → IC=+0.155 (n=787)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.7048 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` > `1.0851` → IC=+0.153 (n=811)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.0851 (IC base=+0.144)

- **PATRÓN** `volumen_pendiente_norm` > `0.1658` → IC=+0.179 (n=634)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1658 (IC base=+0.144)

- **PATRÓN** `volumen_spike_ratio` > `1.8972` → IC=+0.153 (n=1477)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.8972 (IC base=+0.144)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.153 (n=2056)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `3102.2831` → IC=+0.190 (n=1148)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 3102.2831 (IC base=+0.144)

- **PATRÓN** `ballena_activa_n` < `172.0` → IC=+0.186 (n=1015)

  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 172.0 (IC base=+0.144)

- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.223 (n=1495)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0063 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.4332` → IC=+0.200 (n=2239)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4332 (IC base=+0.189)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.196 (n=1023)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5455` → IC=+0.244 (n=2241)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5455 (IC base=+0.189)

- **PATRÓN** `dist_vwap_pct` < `0.7107` → IC=+0.177 (n=1844)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.7107 (IC base=+0.189)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.472` → IC=+0.202 (n=407)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.472 (IC base=+0.189)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.61` → IC=+0.195 (n=2094)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` < 2.61 (IC base=+0.189)

- **PATRÓN** `volumen_regimen` < `0.6224` → IC=+0.176 (n=578)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 0.6224 (IC base=+0.189)

- **PATRÓN** `volumen_regimen` > `1.193` → IC=+0.188 (n=578)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` > 1.193 (IC base=+0.189)

- **PATRÓN** `volumen_pendiente_norm` > `0.2333` → IC=+0.252 (n=312)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2333 (IC base=+0.189)

- **PATRÓN** `volumen_spike_ratio` > `2.256` → IC=+0.198 (n=701)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.256 (IC base=+0.189)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.158 (n=492)
...
- **PATRÓN** `ibs_20min` > `0.9489` → IC=+0.284 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9489 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.2` → IC=+0.353 (n=154)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.2 (IC base=+0.129)

- **PATRÓN** `volumen_pendiente_norm` > `0.088` → IC=+0.140 (n=134)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_pendiente_norm` > 0.088 (IC base=+0.129)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.167 (n=190)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.04 (IC base=+0.129)

- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.315 (n=144)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0063 (IC base=+0.288)

- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.290 (n=98)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0069 (IC base=+0.288)

- **PATRÓN** `drift_60min` |x|≤ `0.2147` → IC=+0.327 (n=189)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2147 (IC base=+0.288)
...
- **PATRÓN** `ibs_20min` < `0.5765` → IC=+0.339 (n=215)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5765 (IC base=+0.288)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.049` → IC=+0.317 (n=233)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.049 (IC base=+0.288)

- **PATRÓN** `volumen_pendiente_norm` < `0.0647` → IC=+0.314 (n=95)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0647 (IC base=+0.288)

- **PATRÓN** `volumen_pendiente_norm` > `0.2284` → IC=+0.300 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2284 (IC base=+0.288)

- **PATRÓN** `volumen_spike_ratio` < `1.8682` → IC=+0.287 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8682 (IC base=+0.288)

- **PATRÓN** `volumen_spike_ratio` > `1.5851` → IC=+0.302 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5851 (IC base=+0.288)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.327 (n=108)
...
- **PATRÓN** `ballena_activa_n` < `72.0` → IC=+0.296 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 72.0 (IC base=+0.288)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0027` → IC=+0.189 (n=133)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0027 (IC base=+0.164)

- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.209 (n=132)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0069 (IC base=+0.164)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.188 (n=398)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 6.0 (IC base=+0.164)

- **PATRÓN** `ibs_20min` > `0.3308` → IC=+0.216 (n=396)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3308 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.2615` → IC=+0.247 (n=243)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2615 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.757` → IC=+0.227 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.757 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` < `0.6563` → IC=+0.187 (n=132)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` < 0.6563 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` > `1.1006` → IC=+0.192 (n=180)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` > 1.1006 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.1444` → IC=+0.227 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1444 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` < `2.53` → IC=+0.196 (n=347)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.53 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` > `1.4672` → IC=+0.202 (n=310)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4672 (IC base=+0.164)

- **PATRÓN** `libro_liquidez` > `11526.545` → IC=+0.203 (n=264)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11526.545 (IC base=+0.164)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2543` → IC=+0.180 (n=348)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.90€ cuando `drift_60min` |x|≤ 0.2543 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.175 (n=376)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 7.0 (IC base=+0.167)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.174 (n=412)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` < 18.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` < `0.3628` → IC=+0.209 (n=348)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3628 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` < `0.7434` → IC=+0.177 (n=434)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` < 0.7434 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.639` → IC=+0.229 (n=216)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4122` → IC=+0.189 (n=300)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 1.4122 (IC base=+0.167)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.169 (n=511)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.01 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `13125.5618` → IC=+0.179 (n=132)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 13125.5618 (IC base=+0.167)
...
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.268 (n=110)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0077 (IC base=+0.173)

- **PATRÓN** `drift_60min` |x|≤ `0.0714` → IC=+0.175 (n=112)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.0714 (IC base=+0.173)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.237 (n=154)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.173)

- **PATRÓN** `ibs_20min` > `0.7131` → IC=+0.247 (n=295)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7131 (IC base=+0.173)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.912` → IC=+0.344 (n=94)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.912 (IC base=+0.173)

- **PATRÓN** `volumen_pendiente_norm` < `0.2327` → IC=+0.173 (n=261)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` < 0.2327 (IC base=+0.173)

- **PATRÓN** `volumen_spike_ratio` < `2.1406` → IC=+0.161 (n=113)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.1406 (IC base=+0.173)

- **PATRÓN** `volumen_spike_ratio` > `3.8756` → IC=+0.172 (n=117)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 3.8756 (IC base=+0.173)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.210 (n=253)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.173)

- **PATRÓN** `libro_liquidez` > `1915.9532` → IC=+0.191 (n=150)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 1915.9532 (IC base=+0.173)

- **PATRÓN** `sigma_h` < `0.0054` → IC=+0.340 (n=98)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0054 (IC base=+0.267)

- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.275 (n=100)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0078 (IC base=+0.267)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.270 (n=133)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.267)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.274 (n=295)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.267)

- **PATRÓN** `ibs_20min` < `0.5597` → IC=+0.337 (n=292)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5597 (IC base=+0.267)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.02` → IC=+0.272 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.02 (IC base=+0.267)

- **PATRÓN** `volumen_pendiente_norm` < `0.1592` → IC=+0.224 (n=154)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1592 (IC base=+0.267)

- **PATRÓN** `volumen_pendiente_norm` > `0.378` → IC=+0.267 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.378 (IC base=+0.267)

- **PATRÓN** `volumen_spike_ratio` < `3.6434` → IC=+0.263 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.6434 (IC base=+0.267)
...
- **PATRÓN** `ballena_activa_n` < `19.0` → IC=+0.219 (n=30)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 19.0 (IC base=+0.267)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.009` → IC=+0.150 (n=395)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.009 (IC base=+0.138)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.169 (n=358)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 8.0 (IC base=+0.138)

- **PATRÓN** `ibs_20min` > `0.3491` → IC=+0.195 (n=395)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` > 0.3491 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` > `0.8989` → IC=+0.214 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8989 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.499` → IC=+0.194 (n=191)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 4.499 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `0.6268` → IC=+0.187 (n=132)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` < 0.6268 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `1.2094` → IC=+0.164 (n=132)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4206` → IC=+0.200 (n=361)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4206 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `3082.6348` → IC=+0.190 (n=353)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 3082.6348 (IC base=+0.138)

- **PATRÓN** `ballena_activa_n` < `171.0` → IC=+0.185 (n=179)

  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 171.0 (IC base=+0.138)

- **PATRÓN** `sigma_h` < `0.0027` → IC=+0.227 (n=108)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0027 (IC base=+0.143)

- **PATRÓN** `drift_60min` |x|≤ `0.5023` → IC=+0.154 (n=322)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.0801` → IC=+0.243 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0801 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` < `0.727` → IC=+0.167 (n=340)
...
- **PATRÓN** `volumen_regimen` < `0.5857` → IC=+0.164 (n=108)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.5857 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` > `0.6387` → IC=+0.152 (n=288)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.6387 (IC base=+0.143)

- **PATRÓN** `volumen_pendiente_norm` > `0.2603` → IC=+0.339 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2603 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` < `1.8043` → IC=+0.183 (n=178)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` < 1.8043 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` > `1.443` → IC=+0.172 (n=266)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.443 (IC base=+0.143)

- **PATRÓN** `libro_liquidez` > `4274.4143` → IC=+0.168 (n=215)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 4274.4143 (IC base=+0.143)

- **PATRÓN** `ballena_activa_n` < `141.0` → IC=+0.221 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 141.0 (IC base=+0.143)

### GBM_LATE_15M_TARDIO#SOL#15min
- **PATRÓN** `sigma_h` > `0.0105` → IC=+0.157 (n=205)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0105 (IC base=+0.088)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.130 (n=309)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 12.0 (IC base=+0.088)

- **PATRÓN** `ibs_20min` > `0.5` → IC=+0.160 (n=457)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.5 (IC base=+0.088)

- **PATRÓN** `dist_vwap_pct` > `1.0341` → IC=+0.263 (n=95)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0341 (IC base=+0.088)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.503` → IC=+0.226 (n=177)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.503 (IC base=+0.088)

- **PATRÓN** `libro_liquidez` > `2815.6056` → IC=+0.265 (n=151)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2815.6056 (IC base=+0.088)

- **PATRÓN** `ballena_activa_n` < `69.0` → IC=+0.167 (n=226)
...
- **PATRÓN** `sigma_h` > `0.0255` → IC=+0.219 (n=183)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0255 (IC base=+0.170)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.191 (n=273)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 15.0 (IC base=+0.170)

- **PATRÓN** `ibs_20min` > `0.9684` → IC=+0.296 (n=248)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9684 (IC base=+0.170)

- **PATRÓN** `dist_vwap_pct` > `1.3375` → IC=+0.280 (n=139)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.3375 (IC base=+0.170)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.048` → IC=+0.251 (n=299)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.048 (IC base=+0.170)

- **PATRÓN** `volumen_regimen` < `0.6149` → IC=+0.192 (n=183)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` < 0.6149 (IC base=+0.170)

- **PATRÓN** `volumen_regimen` > `1.0463` → IC=+0.192 (n=248)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` > 1.0463 (IC base=+0.170)

- **PATRÓN** `volumen_pendiente_norm` > `0.2391` → IC=+0.253 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2391 (IC base=+0.170)

- **PATRÓN** `volumen_spike_ratio` < `1.5621` → IC=+0.179 (n=219)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 1.5621 (IC base=+0.170)

- **PATRÓN** `volumen_spike_ratio` > `1.8445` → IC=+0.177 (n=332)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 1.8445 (IC base=+0.170)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.183 (n=611)

  - _Acción_: Kelly boost +0.92€ cuando `libro_spread` < 0.01 (IC base=+0.170)

- **PATRÓN** `libro_liquidez` > `3111.5385` → IC=+0.181 (n=183)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 3111.5385 (IC base=+0.170)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.219 (n=276)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4354` → IC=+0.227 (n=519)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4354 (IC base=+0.209)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.220 (n=548)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.209)

- **PATRÓN** `ibs_20min` < `0.4887` → IC=+0.275 (n=590)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4887 (IC base=+0.209)

- **PATRÓN** `dist_vwap_pct` < `1.1224` → IC=+0.218 (n=679)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 1.1224 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.209` → IC=+0.268 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.209 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.977` → IC=+0.216 (n=596)
...
- **PATRÓN** `ballena_activa_n` < `37.0` → IC=+0.169 (n=309)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.154 (n=721)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.140 (n=137)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 16.0 (IC base=+0.079)

- **PATRÓN** `dist_vwap_pct` > `1.7332` → IC=+0.200 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.7332 (IC base=+0.079)

- **PATRÓN** `libro_liquidez` > `8385.7236` → IC=+0.136 (n=215)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 8385.7236 (IC base=+0.079)

- **PATRÓN** `sigma_h` < `0.0037` → IC=+0.210 (n=243)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0037 (IC base=+0.146)

- **PATRÓN** `drift_60min` |x|≤ `0.3378` → IC=+0.174 (n=486)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.3378 (IC base=+0.146)
...
- **PATRÓN** `ibs_20min` < `0.6273` → IC=+0.160 (n=486)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` < 0.6273 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `0.6099` → IC=+0.154 (n=160)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.6099 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` < `0.7583` → IC=+0.149 (n=610)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.7583 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.913` → IC=+0.164 (n=278)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 2.913 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` < `1.2467` → IC=+0.165 (n=553)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.2467 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` > `0.7234` → IC=+0.147 (n=494)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` > 0.7234 (IC base=+0.146)

- **PATRÓN** `volumen_pendiente_norm` > `0.0733` → IC=+0.172 (n=269)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.0733 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` < `2.5268` → IC=+0.170 (n=547)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` < 2.5268 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` > `1.4162` → IC=+0.150 (n=546)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.4162 (IC base=+0.146)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.154 (n=721)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.146)

- **PATRÓN** `libro_liquidez` > `8260.3607` → IC=+0.161 (n=552)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 8260.3607 (IC base=+0.146)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.195 (n=201)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0039 (IC base=+0.147)

- **PATRÓN** `drift_60min` |x|≤ `0.3098` → IC=+0.167 (n=301)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.3098 (IC base=+0.147)
...
- **PATRÓN** `ibs_20min` < `0.1373` → IC=+0.211 (n=133)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1373 (IC base=+0.147)

- **PATRÓN** `ibs_20min` > `0.7391` → IC=+0.147 (n=100)

  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` > 0.7391 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.6733` → IC=+0.189 (n=43)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.6733 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` < `0.2288` → IC=+0.148 (n=282)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.2288 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.149` → IC=+0.147 (n=148)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` > 3.149 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.171` → IC=+0.152 (n=268)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` < 4.171 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `1.2377` → IC=+0.157 (n=301)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.2377 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` > `0.7034` → IC=+0.156 (n=268)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 0.7034 (IC base=+0.147)

- **PATRÓN** `volumen_pendiente_norm` > `0.0733` → IC=+0.201 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0733 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` < `2.6056` → IC=+0.175 (n=300)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 2.6056 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `11671.1061` → IC=+0.167 (n=268)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 11671.1061 (IC base=+0.147)

### GBM_LATE_5M#ETH#5min
- **PATRÓN** `sigma_h` < `0.0094` → IC=+0.146 (n=142)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` < 0.0094 (IC base=+0.104)

- **PATRÓN** `drift_60min` |x|≤ `0.3021` → IC=+0.127 (n=108)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.64€ cuando `drift_60min` |x|≤ 0.3021 (IC base=+0.104)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.223 (n=63)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.104)

- **PATRÓN** `ibs_20min` < `0.1965` → IC=+0.125 (n=54)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` < 0.1965 (IC base=+0.104)

- **PATRÓN** `ibs_20min` > `0.7734` → IC=+0.145 (n=74)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` > 0.7734 (IC base=+0.104)

- **PATRÓN** `dist_vwap_pct` > `1.1654` → IC=+0.222 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1654 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.377` → IC=+0.175 (n=38)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 10.377 (IC base=+0.104)

- **PATRÓN** `volumen_regimen` < `0.7281` → IC=+0.130 (n=71)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 0.7281 (IC base=+0.104)

- **PATRÓN** `volumen_spike_ratio` < `1.4017` → IC=+0.191 (n=53)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` < 1.4017 (IC base=+0.104)

- **PATRÓN** `libro_liquidez` > `7710.7968` → IC=+0.152 (n=162)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 7710.7968 (IC base=+0.104)

- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.254 (n=59)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0039 (IC base=+0.174)

- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.181 (n=117)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0065 (IC base=+0.174)
...
- **PATRÓN** `ibs_20min` < `0.5833` → IC=+0.181 (n=155)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` < 0.5833 (IC base=+0.174)

- **PATRÓN** `ibs_20min` > `0.1096` → IC=+0.208 (n=176)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.1096 (IC base=+0.174)

- **PATRÓN** `dist_vwap_pct` < `0.6774` → IC=+0.184 (n=185)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` < 0.6774 (IC base=+0.174)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.561` → IC=+0.268 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.561 (IC base=+0.174)

- **PATRÓN** `volumen_regimen` < `1.1687` → IC=+0.219 (n=176)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.1687 (IC base=+0.174)

- **PATRÓN** `volumen_pendiente_norm` < `0.0898` → IC=+0.199 (n=161)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4319` → IC=+0.195 (n=172)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.4319 (IC base=+0.174)

- **PATRÓN** `libro_liquidez` > `9437.7622` → IC=+0.181 (n=117)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 9437.7622 (IC base=+0.174)
...
- **FILTRO** `ibs_20min` < `0.75` → IC=-0.155 (n=56)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.209 (n=115)
...
- **PATRÓN** `sigma_h` < `0.0054` → IC=+0.178 (n=147)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0054 (IC base=+0.052)

- **PATRÓN** `drift_60min` |x|≤ `0.1168` → IC=+0.144 (n=43)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.1168 (IC base=+0.052)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.273 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.052)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.328` → IC=+0.216 (n=93)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.328 (IC base=+0.052)

- **PATRÓN** `volumen_pendiente_norm` < `0.0847` → IC=+0.260 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0847 (IC base=+0.052)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.135 (n=113)

  - _Acción_: Kelly boost +0.67€ cuando `libro_spread` < 0.02 (IC base=+0.052)

- **PATRÓN** `libro_liquidez` > `1977.9398` → IC=+0.135 (n=94)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 1977.9398 (IC base=+0.052)
...
- **PATRÓN** `sigma_h` < `0.0029` → IC=+0.242 (n=29)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0029 (IC base=+0.098)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.122 (n=80)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 7.0 (IC base=+0.098)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.346 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.098)

- **PATRÓN** `dist_vwap_pct` < `0.2294` → IC=+0.222 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2294 (IC base=+0.098)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.41` → IC=+0.304 (n=44)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.41 (IC base=+0.098)

- **PATRÓN** `volumen_regimen` < `0.8241` → IC=+0.200 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8241 (IC base=+0.098)

- **PATRÓN** `volumen_regimen` > `1.0919` → IC=+0.167 (n=19)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 1.0919 (IC base=+0.098)

- **PATRÓN** `libro_liquidez` > `1923.3872` → IC=+0.198 (n=51)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 1923.3872 (IC base=+0.098)
...
- **PATRÓN** `ibs_20min` > `0.6061` → IC=+0.150 (n=135)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` > 0.6061 (IC base=+0.055)
...
- **PATRÓN** `sigma_h` < `0.0025` → IC=+0.281 (n=30)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0025 (IC base=+0.138)

- **PATRÓN** `drift_60min` |x|≤ `0.1051` → IC=+0.220 (n=23)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1051 (IC base=+0.138)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.333 (n=16)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 14.0 (IC base=+0.138)

- **PATRÓN** `ibs_20min` > `0.9404` → IC=+0.260 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9404 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.212` → IC=+0.239 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.212 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `0.6735` → IC=+0.206 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6735 (IC base=+0.138)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.242 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `2399.5952` → IC=+0.222 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2399.5952 (IC base=+0.138)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.165 (n=688)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.035 (n=2143)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.225 (n=674)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.007 (n=2091)

- **FILTRO** `ibs_20min` > `0.2759` → IC=-0.198 (n=690)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2759
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=2075)
...
- **FILTRO** `ibs_20min` < `0.7265` → IC=-0.195 (n=103)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=312)
...
- **FILTRO** `ballena_activa_n` > `3.0` → IC=-0.168 (n=194)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.192 (n=118)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.067 (n=354)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.142 (n=2031)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=4885)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.285 (n=1665)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=5251)

- **FILTRO** `ibs_7min` < `0.7368` → IC=-0.233 (n=1725)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7368
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=5191)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.175 (n=2340)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=4576)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.228 (n=1985)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=6478)

- **FILTRO** `ibs_7min` > `0.7215` → IC=-0.170 (n=2115)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7215
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=6348)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.154 (n=218)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.092 (n=723)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.326 (n=228)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=713)

- **FILTRO** `ibs_7min` < `0.9839` → IC=-0.200 (n=621)

  - _Acción_: SKIP cuando `ibs_7min` < 0.9839
  - _Potencial_: sin este filtro IC_bueno=+0.074 (n=320)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.263 (n=234)

  - _Acción_: SKIP cuando `ballena_activa_n` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=707)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.241 (n=353)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=1073)

- **FILTRO** `drift_7min_pct` |x|> `0.1053` → IC=-0.136 (n=712)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1053
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=714)

- **FILTRO** `ibs_7min` > `0.837` → IC=-0.201 (n=356)

  - _Acción_: SKIP cuando `ibs_7min` > 0.837
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=1070)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.161 (n=290)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=1110)

- **FILTRO** `py_entrada` < `0.39` → IC=-0.247 (n=350)

  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=1050)

- **FILTRO** `ballena_activa_n` > `148.0` → IC=-0.172 (n=349)

  - _Acción_: SKIP cuando `ballena_activa_n` > 148.0
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=1051)

- **FILTRO** `py_entrada` > `0.58` → IC=-0.219 (n=347)

  - _Acción_: SKIP cuando `py_entrada` > 0.58
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=1081)

- **FILTRO** `ballena_activa_n` > `119.0` → IC=-0.172 (n=355)

  - _Acción_: SKIP cuando `ballena_activa_n` > 119.0
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.213 (n=273)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.077 (n=693)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.330 (n=228)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=738)

- **FILTRO** `ibs_7min` < `0.2174` → IC=-0.278 (n=241)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2174
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=725)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.272 (n=239)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.261 (n=320)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=1160)

- **FILTRO** `ibs_7min` > `0.8192` → IC=-0.187 (n=369)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8192
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=1111)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.157 (n=359)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=820)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.265 (n=291)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=888)

- **FILTRO** `ibs_7min` < `0.7674` → IC=-0.189 (n=294)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7674
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=885)

- **FILTRO** `ballena_activa_n` > `44.0` → IC=-0.214 (n=292)

  - _Acción_: SKIP cuando `ballena_activa_n` > 44.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=887)

- **FILTRO** `hora_utc` > `15.0` → IC=-0.137 (n=364)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=803)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.284 (n=276)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=891)

- **FILTRO** `ibs_7min` > `0.1818` → IC=-0.155 (n=395)

  - _Acción_: SKIP cuando `ibs_7min` > 0.1818
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=772)

- **FILTRO** `ballena_activa_n` > `39.0` → IC=-0.213 (n=287)

  - _Acción_: SKIP cuando `ballena_activa_n` > 39.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=880)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.4` → IC=-0.226 (n=312)

  - _Acción_: SKIP cuando `py_entrada` < 0.4
  - _Potencial_: sin este filtro IC_bueno=+0.010 (n=982)

- **FILTRO** `ibs_7min` < `0.7727` → IC=-0.197 (n=321)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7727
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=973)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.195 (n=323)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=971)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.182 (n=366)
...
- **FILTRO** `py_entrada` < `0.38` → IC=-0.290 (n=274)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.016 (n=862)

- **FILTRO** `ibs_7min` < `0.75` → IC=-0.240 (n=275)

  - _Acción_: SKIP cuando `ibs_7min` < 0.75
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=861)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.244 (n=283)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=853)
...
- **FILTRO** `ibs_7min` > `0.8387` → IC=-0.148 (n=356)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8387
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=1069)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.124 (n=479)

  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=946)
...
- **PATRÓN** `delta_ratio` |x|> `0.3994` → IC=+0.143 (n=256)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.72€ cuando `delta_ratio` |x|> 0.3994 (IC base=+0.116)

- **PATRÓN** `total_vol_5m` < `451.687` → IC=+0.188 (n=78)

  - _Acción_: Kelly boost +0.94€ cuando `total_vol_5m` < 451.687 (IC base=+0.116)

- **PATRÓN** `libro_liquidez` > `3571.4392` → IC=+0.151 (n=61)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 3571.4392 (IC base=+0.116)
...
- **PATRÓN** `streak_len` < `3.0` → IC=+0.140 (n=123)

  - _Acción_: Kelly boost +0.70€ cuando `streak_len` < 3.0 (IC base=+0.059)
...
- **PATRÓN** `sigma_h` < `0.0029` → IC=+0.146 (n=125)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` < 0.0029 (IC base=+0.107)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2162` → IC=+0.132 (n=169)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.66€ cuando `delta_ratio_macro` |x|> 0.2162 (IC base=+0.107)

- **PATRÓN** `ibs_15` > `0.5166` → IC=+0.193 (n=373)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.97€ cuando `ibs_15` > 0.5166 (IC base=+0.107)

- **PATRÓN** `dist_vwap_pct` > `0.3848` → IC=+0.173 (n=102)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.3848 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.614` → IC=+0.238 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.614 (IC base=+0.107)

- **PATRÓN** `libro_liquidez` > `2958.1829` → IC=+0.134 (n=249)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 2958.1829 (IC base=+0.107)

- **PATRÓN** `ballena_activa_n` < `73.0` → IC=+0.167 (n=31)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 73.0 (IC base=+0.107)
...
- **FILTRO** `sigma_h` > `0.0061` → IC=-0.127 (n=116)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_h` > `0.0054` → IC=+0.394 (n=45)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0054 (IC base=+0.300)

- **PATRÓN** `drift_60min` |x|≤ `0.0615` → IC=+0.351 (n=45)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0615 (IC base=+0.300)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1549` → IC=+0.313 (n=89)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1549 (IC base=+0.300)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.4217` → IC=+0.377 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.4217 (IC base=+0.300)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.323 (n=122)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.300)

- **PATRÓN** `ibs_15` > `0.8044` → IC=+0.361 (n=120)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8044 (IC base=+0.300)

- **PATRÓN** `dist_vwap_pct` > `0.447` → IC=+0.378 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.447 (IC base=+0.300)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.489` → IC=+0.329 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.489 (IC base=+0.300)

- **PATRÓN** `libro_liquidez` > `3382.7783` → IC=+0.311 (n=120)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3382.7783 (IC base=+0.300)

- **PATRÓN** `ballena_activa_n` < `540.0` → IC=+0.373 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 540.0 (IC base=+0.300)

### UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1986` → IC=+0.286 (n=68)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +1.00€ cuando `pct_spot_vs_ref` |x|≤ 0.1986 (IC base=+0.269)
...
- **PATRÓN** `drift_60min` |x|≤ `0.16` → IC=+0.300 (n=68)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.16 (IC base=+0.269)

- **PATRÓN** `drift_15min` |x|≤ `0.4366` → IC=+0.306 (n=34)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.4366 (IC base=+0.269)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.308 (n=71)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.269)

- **PATRÓN** `hora_utc` < `13.0` → IC=+0.275 (n=69)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 13.0 (IC base=+0.269)

- **PATRÓN** `ibs_15` > `0.8246` → IC=+0.317 (n=69)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8246 (IC base=+0.269)

- **PATRÓN** `dist_vwap_pct` > `0.4311` → IC=+0.380 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4311 (IC base=+0.269)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.871` → IC=+0.287 (n=45)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.871 (IC base=+0.269)

- **PATRÓN** `sigma_ewma_delta_pct` < `14.018` → IC=+0.268 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 14.018 (IC base=+0.269)

- **PATRÓN** `libro_liquidez` > `7959.8654` → IC=+0.338 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7959.8654 (IC base=+0.269)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.036` → IC=-0.188 (n=312)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 17.036
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=2156)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.0853` → IC=+0.350 (n=18)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.0853 (IC base=-0.094)

- **PATRÓN** `ibs_15` < `0.3357` → IC=+0.339 (n=85)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3357 (IC base=-0.094)

- **PATRÓN** `dist_vwap_pct` < `0.1943` → IC=+0.233 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1943 (IC base=-0.094)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.278 (n=43)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 156.0 (IC base=-0.094)

### UPDOWN_GBM_15M_TARDIO#BTC#15min
- **FILTRO** `sigma_h` < `0.0045` → IC=-0.219 (n=276)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0045
  - _Potencial_: sin este filtro IC_bueno=-0.209 (n=276)

- **FILTRO** `hora_utc` > `16.0` → IC=-0.241 (n=133)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.205 (n=419)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1584` → IC=+0.236 (n=51)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +1.00€ cuando `pct_spot_vs_ref` |x|≤ 0.1584 (IC base=+0.218)

- **PATRÓN** `sigma_h` > `0.0056` → IC=+0.275 (n=38)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0056 (IC base=+0.218)

- **PATRÓN** `drift_15min` |x|≤ `0.4463` → IC=+0.273 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.4463 (IC base=+0.218)

- **PATRÓN** `delta_ratio_macro` |x|> `0.111` → IC=+0.217 (n=51)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.111 (IC base=+0.218)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.261 (n=44)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 12.0 (IC base=+0.218)

- **PATRÓN** `hora_utc` < `14.0` → IC=+0.236 (n=51)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 14.0 (IC base=+0.218)

- **PATRÓN** `ibs_15` < `0.428` → IC=+0.350 (n=58)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.428 (IC base=+0.218)

- **PATRÓN** `dist_vwap_pct` < `0.1085` → IC=+0.238 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1085 (IC base=+0.218)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.051` → IC=+0.219 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.051 (IC base=+0.218)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.486` → IC=+0.259 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 10.486 (IC base=+0.218)

- **PATRÓN** `libro_liquidez` > `7056.133` → IC=+0.321 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7056.133 (IC base=+0.218)
...
- **FILTRO** `sigma_ewma_delta_pct` > `15.4` → IC=-0.153 (n=96)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 15.4
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=763)
...
- **FILTRO** `drift_15min` |x|> `1.5354` → IC=-0.140 (n=198)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 1.5354
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=595)

- **FILTRO** `sigma_ewma_delta_pct` > `15.454` → IC=-0.170 (n=98)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 15.454
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=695)

### UPDOWN_GBM_IBS_ALTO
- **PATRÓN** `sigma_h` > `0.0052` → IC=+0.294 (n=95)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1898` → IC=+0.325 (n=95)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1898 (IC base=+0.271)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3917` → IC=+0.311 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.3917 (IC base=+0.271)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.309 (n=213)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.271)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.271 (n=212)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.271)

- **PATRÓN** `ibs_15` > `0.8604` → IC=+0.315 (n=187)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8604 (IC base=+0.271)

- **PATRÓN** `dist_vwap_pct` > `0.2899` → IC=+0.361 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2899 (IC base=+0.271)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `13.713` → IC=+0.270 (n=202)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 13.713 (IC base=+0.271)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.270 (n=255)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.271)

- **PATRÓN** `libro_liquidez` > `11800.2375` → IC=+0.333 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11800.2375 (IC base=+0.271)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1596` → IC=+0.290 (n=103)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1596 (IC base=+0.253)

- **PATRÓN** `drift_15min` |x|≤ `0.6592` → IC=+0.271 (n=103)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.6592 (IC base=+0.253)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1976` → IC=+0.286 (n=54)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1976 (IC base=+0.253)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1411` → IC=+0.300 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1411 (IC base=+0.253)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.273 (n=108)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.2911` → IC=+0.357 (n=54)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2911 (IC base=+0.253)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `7.438` → IC=+0.258 (n=93)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 7.438 (IC base=+0.253)

- **PATRÓN** `libro_liquidez` > `13108.0743` → IC=+0.281 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13108.0743 (IC base=+0.253)

- **PATRÓN** `ballena_activa_n` < `581.0` → IC=+0.274 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 581.0 (IC base=+0.253)
...
- **FILTRO** `pct_spot_vs_ref` |x|> `0.1233` → IC=-0.193 (n=86)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=259)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.5166 sube el IC de +0.107 a +0.193 en UPDOWN_GBM#15min (n=373). Ya aplicado como kelly_boost=+0.97€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3357 sube el IC de -0.094 a +0.339 en UPDOWN_GBM_15M_TARDIO (n=85). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.428 sube el IC de +0.218 a +0.350 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=58). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.8604 sube el IC de +0.271 a +0.315 en UPDOWN_GBM_IBS_ALTO (n=187). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_IBS_ALTO#BTC#15min**: dentro de BUY_YES, IBS > 0.9661 sube el IC de +0.253 a +0.339 en UPDOWN_GBM_IBS_ALTO#BTC#15min (n=54). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD**: dentro de BUY_YES, IBS > 0.8044 sube el IC de +0.300 a +0.361 en UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD (n=120). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min**: dentro de BUY_YES, IBS > 0.8246 sube el IC de +0.269 a +0.317 en UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min (n=69). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 6319 | -0.099 | -822.37€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 657 | -0.086 | -112.53€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 5662 | -0.100 | -709.84€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 997 | +0.019 | -106.99€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 997 | +0.019 | -106.99€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 2093 | -0.076 | -97.29€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2093 | -0.076 | -97.29€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1488 | -0.186 | -312.09€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1488 | -0.186 | -312.09€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 28797 | +0.114 | -1884.77€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 5455 | +0.187 | -183.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 107 | -0.105 | -50.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 20157 | +0.094 | -1622.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3078 | +0.122 | -29.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 3383 | +0.063 | -582.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 19 | -0.068 | +0.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 3359 | +0.064 | -577.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 6005 | +0.135 | -121.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1531 | +0.197 | -85.34€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 3358 | +0.110 | -73.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1074 | +0.132 | +59.80€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 3391 | +0.076 | -464.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 19 | +0.023 | -2.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 3371 | +0.077 | -459.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 6442 | +0.130 | -50.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1983 | +0.171 | +1.37€ | 0 | 8 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 3356 | +0.116 | -27.56€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1091 | +0.105 | -15.49€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 6193 | +0.131 | -418.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1886 | +0.202 | -96.52€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 46 | +0.000 | -9.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 3348 | +0.092 | -239.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 913 | +0.132 | -73.32€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 3383 | +0.104 | -247.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 17 | -0.022 | -0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 3365 | +0.105 | -245.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 5429 | +0.176 | -399.48€ | 3 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 5429 | +0.176 | -399.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 1365 | +0.172 | -136.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 1365 | +0.172 | -136.62€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 1356 | +0.162 | -157.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 1356 | +0.162 | -157.60€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 1216 | +0.232 | -30.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 1216 | +0.232 | -30.61€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 1306 | +0.190 | -89.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 1306 | +0.190 | -89.98€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_EXTREMO | 273 | +0.442 | +0.83€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_EXTREMO#15min | 273 | +0.442 | +0.83€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 14440 | +0.192 | -1245.28€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 14440 | +0.192 | -1245.28€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 2661 | +0.125 | -491.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 2661 | +0.125 | -491.77€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 2241 | +0.237 | -47.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 2241 | +0.237 | -47.85€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 2500 | +0.163 | -320.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 2500 | +0.163 | -320.68€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 2285 | +0.234 | -54.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 2285 | +0.234 | -54.35€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 2322 | +0.220 | -102.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 2322 | +0.220 | -102.68€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 2431 | +0.189 | -227.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 2431 | +0.189 | -227.95€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 5205 | +0.131 | +143.72€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 5205 | +0.131 | +143.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 2584 | +0.138 | +108.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 2584 | +0.138 | +108.08€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 2621 | +0.123 | +35.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 2621 | +0.123 | +35.64€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0 | 186 | +0.101 | +0.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#240min | 57 | +0.110 | +0.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#60min | 129 | +0.095 | -0.02€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#ETH | 152 | +0.104 | +1.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#ETH#240min | 23 | +0.140 | +1.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#ETH#60min | 129 | +0.095 | -0.02€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 5018 | +0.096 | -183.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 516 | +0.046 | -37.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 4502 | +0.102 | -146.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC | 3467 | +0.096 | -91.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#15min | 516 | +0.046 | -37.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#5min | 2951 | +0.104 | -54.67€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL | 1551 | +0.098 | -92.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL#5min | 1551 | +0.098 | -92.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 401 | +0.282 | -27.38€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 401 | +0.282 | -27.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 401 | +0.282 | -27.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 401 | +0.282 | -27.38€ | 0 | 4 |
| ✅ GBM_LATE_15M | 6814 | +0.044 | +2071.62€ | 0 | 18 |
| ✅ GBM_LATE_15M#15min | 6814 | +0.044 | +2071.62€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 875 | +0.174 | +549.55€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 875 | +0.174 | +549.55€ | 0 | 18 |
| ✅ GBM_LATE_15M#BTC | 958 | +0.175 | +554.07€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 958 | +0.175 | +554.07€ | 0 | 30 |
| ✅ GBM_LATE_15M#DOGE | 884 | +0.189 | +600.28€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 884 | +0.189 | +600.28€ | 0 | 18 |
| ✅ GBM_LATE_15M#ETH | 1106 | -0.049 | +20.83€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1106 | -0.049 | +20.83€ | 2 | 6 |
| ✅ GBM_LATE_15M#SOL | 1307 | -0.039 | +151.51€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1307 | -0.039 | +151.51€ | 4 | 4 |
| ✅ GBM_LATE_15M#XRP | 1684 | -0.050 | +195.38€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1684 | -0.050 | +195.38€ | 5 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 7771 | +0.044 | +2917.91€ | 0 | 17 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 7771 | +0.044 | +2917.91€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 1157 | -0.028 | +503.61€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 1157 | -0.028 | +503.61€ | 1 | 3 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 1716 | -0.032 | +174.90€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 1716 | -0.032 | +174.90€ | 0 | 1 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 749 | +0.240 | +685.22€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 749 | +0.240 | +685.22€ | 0 | 21 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1339 | -0.042 | +14.33€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1339 | -0.042 | +14.33€ | 8 | 3 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 1429 | -0.014 | +297.04€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 1429 | -0.014 | +297.04€ | 5 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 1381 | +0.235 | +1242.81€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 1381 | +0.235 | +1242.81€ | 0 | 23 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 5451 | +0.168 | +3689.65€ | 0 | 25 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 5451 | +0.168 | +3689.65€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB | 661 | +0.191 | +479.32€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BNB#15min | 661 | +0.191 | +479.32€ | 0 | 18 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC | 930 | +0.167 | +624.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#BTC#15min | 930 | +0.167 | +624.64€ | 0 | 22 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE | 655 | +0.199 | +499.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min | 655 | +0.199 | +499.64€ | 0 | 19 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH | 910 | +0.163 | +574.44€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#ETH#15min | 910 | +0.163 | +574.44€ | 0 | 25 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL | 1072 | +0.115 | +597.55€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#SOL#15min | 1072 | +0.115 | +597.55€ | 1 | 19 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP | 1223 | +0.191 | +914.06€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#XRP#15min | 1223 | +0.191 | +914.06€ | 0 | 23 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 915 | +0.072 | +160.76€ | 0 | 12 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 915 | +0.072 | +160.76€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BNB | 20 | +0.045 | +2.06€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BNB#15min | 20 | +0.045 | +2.06€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BTC | 263 | +0.111 | +86.52€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#BTC#15min | 263 | +0.111 | +86.52€ | 4 | 18 |
...
| ✅ GBM_LATE_15M_TARDIO | 6359 | +0.165 | +4113.98€ | 0 | 25 |
| ✅ GBM_LATE_15M_TARDIO#15min | 6359 | +0.165 | +4113.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 837 | +0.184 | +580.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 837 | +0.184 | +580.84€ | 0 | 20 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 1054 | +0.166 | +675.10€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 1054 | +0.166 | +675.10€ | 0 | 28 |
| ✅ GBM_LATE_15M_TARDIO#DOGE | 828 | +0.218 | +686.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#DOGE#15min | 828 | +0.218 | +686.53€ | 0 | 21 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 955 | +0.141 | +511.16€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 955 | +0.141 | +511.16€ | 0 | 24 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 1170 | +0.099 | +557.33€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 1170 | +0.099 | +557.33€ | 0 | 18 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 1515 | +0.191 | +1103.02€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1515 | +0.191 | +1103.02€ | 0 | 24 |
| ✅ GBM_LATE_5M | 1166 | +0.122 | +500.79€ | 1 | 18 |
| ✅ GBM_LATE_5M#5min | 1166 | +0.122 | +500.79€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC | 466 | +0.135 | +259.43€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC#5min | 466 | +0.135 | +259.43€ | 2 | 22 |
| ✅ GBM_LATE_5M#DOGE | 13 | -0.108 | -4.42€ | 0 | 0 |
| ✅ GBM_LATE_5M#DOGE#5min | 13 | -0.108 | -4.42€ | 0 | 0 |
| ✅ GBM_LATE_5M#ETH | 449 | +0.141 | +187.93€ | 0 | 0 |
| ✅ GBM_LATE_5M#ETH#5min | 449 | +0.141 | +187.93€ | 0 | 23 |
...
| ✅ GBM_LATE_60M | 505 | -0.046 | +74.01€ | 5 | 7 |
| ✅ GBM_LATE_60M#60min | 505 | -0.046 | +74.01€ | 0 | 0 |
| ✅ GBM_LATE_60M#BTC | 174 | -0.006 | +4.02€ | 0 | 0 |
| ✅ GBM_LATE_60M#BTC#60min | 174 | -0.006 | +4.02€ | 4 | 3 |
| ✅ GBM_LATE_60M#ETH | 179 | -0.019 | +47.22€ | 0 | 0 |
| ✅ GBM_LATE_60M#ETH#60min | 179 | -0.019 | +47.22€ | 4 | 8 |
| ✅ GBM_LATE_60M#SOL | 152 | -0.123 | +22.77€ | 0 | 0 |
| ✅ GBM_LATE_60M#SOL#60min | 152 | -0.123 | +22.77€ | 5 | 1 |
...
| ✅ GBM_LATE_60M_PYCONFIRMADO | 317 | +0.042 | +9.22€ | 1 | 2 |
| ✅ GBM_LATE_60M_PYCONFIRMADO#60min | 317 | +0.042 | +9.22€ | 0 | 0 |
| ✅ GBM_LATE_60M_PYCONFIRMADO#BTC | 123 | +0.020 | +5.30€ | 0 | 0 |
| ✅ GBM_LATE_60M_PYCONFIRMADO#BTC#60min | 123 | +0.020 | +5.30€ | 2 | 7 |
| ✅ GBM_LATE_60M_PYCONFIRMADO#ETH | 78 | +0.087 | +7.23€ | 0 | 0 |
| ✅ GBM_LATE_60M_PYCONFIRMADO#ETH#60min | 78 | +0.087 | +7.23€ | 0 | 8 |
...
| ✅ LIQUIDACIONES_60M | 407 | -0.011 | -6.28€ | 2 | 0 |
| ✅ LIQUIDACIONES_60M#60min | 407 | -0.011 | -6.28€ | 0 | 0 |
...
| ✅ LIQUIDACIONES_60M#SOL | 150 | +0.020 | +6.18€ | 0 | 0 |
| ✅ LIQUIDACIONES_60M#SOL#60min | 150 | +0.020 | +6.18€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M | 3806 | +0.002 | -46.86€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#15min | 3806 | +0.002 | -46.86€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB | 388 | +0.005 | +7.86€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BNB#15min | 388 | +0.005 | +7.86€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BTC | 598 | +0.005 | -8.60€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#BTC#15min | 598 | +0.005 | -8.60€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#DOGE | 630 | +0.000 | -19.00€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#DOGE#15min | 630 | +0.000 | -19.00€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH | 746 | +0.011 | +15.34€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#ETH#15min | 746 | +0.011 | +15.34€ | 1 | 0 |
| ✅ MOMENTUM_IBS_15M#SOL | 690 | -0.003 | -23.73€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#SOL#15min | 690 | -0.003 | -23.73€ | 1 | 0 |
| ✅ MOMENTUM_IBS_15M#XRP | 754 | -0.003 | -18.73€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M#XRP#15min | 754 | -0.003 | -18.73€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA | 5596 | -0.037 | +156.56€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#15min | 5596 | -0.037 | +156.56€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB | 887 | -0.028 | +126.84€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BNB#15min | 887 | -0.028 | +126.84€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC | 1002 | -0.032 | -21.20€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC#15min | 1002 | -0.032 | -21.20€ | 4 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE | 894 | -0.043 | +92.74€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE#15min | 894 | -0.043 | +92.74€ | 4 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH | 1001 | -0.036 | -33.23€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH#15min | 1001 | -0.036 | -33.23€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL | 908 | -0.050 | +0.18€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL#15min | 908 | -0.050 | +0.18€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP | 904 | -0.032 | -8.77€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP#15min | 904 | -0.032 | -8.77€ | 5 | 0 |
...
| ✅ MOMENTUM_IBS_5M | 3007 | +0.005 | -1.28€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#5min | 3007 | +0.005 | -1.28€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_5M#ETH | 997 | +0.011 | +11.66€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#ETH#5min | 997 | +0.011 | +11.66€ | 1 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 15379 | -0.073 | +263.84€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 15379 | -0.073 | +263.84€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 2367 | -0.094 | +250.03€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 2367 | -0.094 | +250.03€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 2828 | -0.057 | +3.41€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 2828 | -0.057 | +3.41€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 2446 | -0.085 | +16.13€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 2446 | -0.085 | +16.13€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 2346 | -0.097 | -169.29€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 2346 | -0.097 | -169.29€ | 8 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 2831 | -0.049 | +28.65€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 2831 | -0.049 | +28.65€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 2561 | -0.067 | +134.89€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 2561 | -0.067 | +134.89€ | 6 | 0 |
...
| ✅ ORDER_FLOW_5M | 377 | +0.086 | +75.36€ | 1 | 3 |
| ✅ ORDER_FLOW_5M#5min | 241 | +0.101 | +62.76€ | 0 | 0 |
...
| ✅ STREAK_FADE_5M | 932 | -0.006 | -31.01€ | 1 | 0 |
| ✅ STREAK_FADE_5M#5min | 932 | -0.006 | -31.01€ | 0 | 0 |
| ✅ STREAK_FADE_5M#DOGE | 319 | +0.002 | -4.97€ | 0 | 0 |
| ✅ STREAK_FADE_5M#DOGE#5min | 319 | +0.002 | -4.97€ | 0 | 0 |
| ✅ STREAK_FADE_5M#ETH | 311 | +0.011 | -3.73€ | 0 | 0 |
| ✅ STREAK_FADE_5M#ETH#5min | 311 | +0.011 | -3.73€ | 1 | 0 |
...
| ✅ STREAK_MOM_5M | 1815 | +0.022 | +21.55€ | 0 | 0 |
| ✅ STREAK_MOM_5M#5min | 1815 | +0.022 | +21.55€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE | 605 | +0.022 | +4.90€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE#5min | 605 | +0.022 | +4.90€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH | 320 | +0.009 | +0.58€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH#5min | 320 | +0.009 | +0.58€ | 1 | 0 |
| ✅ STREAK_MOM_5M#SOL | 547 | +0.025 | +5.76€ | 0 | 0 |
| ✅ STREAK_MOM_5M#SOL#5min | 547 | +0.025 | +5.76€ | 2 | 1 |
| ✅ STREAK_MOM_5M#XRP | 343 | +0.028 | +10.31€ | 0 | 0 |
| ✅ STREAK_MOM_5M#XRP#5min | 343 | +0.028 | +10.31€ | 2 | 2 |
| ✅ STRUCT_NO_15M | 2912 | +0.006 | -33.57€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 2912 | +0.006 | -33.57€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 1124 | +0.006 | -13.56€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 1124 | +0.006 | -13.56€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 1121 | +0.014 | -4.18€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 1121 | +0.014 | -4.18€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 667 | -0.008 | -15.84€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 667 | -0.008 | -15.84€ | 2 | 0 |
| ✅ UPDOWN_GBM | 5292 | +0.007 | +139.74€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 2047 | +0.044 | +187.28€ | 0 | 7 |
| ✅ UPDOWN_GBM#240min | 228 | +0.030 | +7.00€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 2624 | -0.017 | -52.00€ | 2 | 0 |
...
| ✅ UPDOWN_GBM#BTC | 1138 | +0.017 | +53.95€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 214 | +0.065 | +22.92€ | 3 | 11 |
| ✅ UPDOWN_GBM#BTC#240min | 67 | +0.094 | +9.35€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#5min | 725 | +0.009 | +26.42€ | 3 | 0 |
| ✅ UPDOWN_GBM#BTC#60min | 114 | -0.043 | -6.57€ | 1 | 0 |
| ✅ UPDOWN_GBM#BTC#daily | 18 | -0.135 | +1.83€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 670 | -0.003 | +0.67€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 116 | +0.093 | +25.08€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#240min | 8 | +0.000 | -0.09€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 546 | -0.024 | -24.32€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 1209 | +0.010 | +17.64€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 559 | +0.031 | +23.95€ | 1 | 5 |
| ✅ UPDOWN_GBM#ETH#240min | 66 | +0.088 | +6.12€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 423 | -0.027 | -17.38€ | 4 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 146 | +0.027 | +5.32€ | 0 | 2 |
| 🚫 UPDOWN_GBM#ETH#daily | 15 | -0.154 | -0.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 1256 | -0.006 | -13.11€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 462 | +0.000 | -4.46€ | 1 | 2 |
| ✅ UPDOWN_GBM#SOL#240min | 51 | -0.028 | -3.83€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 645 | -0.002 | -3.92€ | 3 | 0 |
...
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD | 178 | +0.300 | +25.70€ | 0 | 10 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#15min | 178 | +0.300 | +25.70€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC | 102 | +0.269 | +2.75€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min | 102 | +0.269 | +2.75€ | 0 | 12 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 3377 | -0.083 | +390.50€ | 2 | 5 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 3377 | -0.083 | +390.50€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 278 | -0.061 | +129.91€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 278 | -0.061 | +129.91€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 698 | -0.169 | -85.22€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 698 | -0.169 | -85.22€ | 3 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 73 | +0.047 | +8.80€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 73 | +0.047 | +8.80€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 232 | +0.090 | +83.56€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 232 | +0.090 | +83.56€ | 2 | 15 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 1076 | -0.072 | +159.47€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 1076 | -0.072 | +159.47€ | 2 | 1 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 1020 | -0.091 | +93.99€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 1020 | -0.091 | +93.99€ | 5 | 0 |
...
| ✅ UPDOWN_GBM_IBS_ALTO | 278 | +0.271 | +193.29€ | 0 | 12 |
| ✅ UPDOWN_GBM_IBS_ALTO#15min | 278 | +0.271 | +193.29€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#BTC | 156 | +0.253 | +94.53€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#BTC#15min | 156 | +0.253 | +94.53€ | 0 | 14 |
| ✅ UPDOWN_GBM_IBS_ALTO#ETH | 122 | +0.290 | +98.76€ | 0 | 0 |
| ✅ UPDOWN_GBM_IBS_ALTO#ETH#15min | 122 | +0.290 | +98.76€ | 0 | 12 |
| ✅ UPDOWN_OU_5M | 390 | -0.069 | -32.74€ | 4 | 0 |
| ✅ UPDOWN_OU_5M#5min | 390 | -0.069 | -32.74€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#BNB | 237 | -0.011 | -12.18€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#BNB#5min | 237 | -0.011 | -12.18€ | 0 | 0 |
...
| 🚫 UPDOWN_OU_5M#SOL#5min | 33 | -0.214 | -6.24€ | 1 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.087) — sin ventaja clara. oversold(IBS<0.3): IC=+0.003 n=1830 | neutral: IC=+0.004 n=1988 | overbought(IBS>0.7): IC=+0.089 n=2097
  - _Datos_: n=6198 IC=+0.033 PNL=+474.04€

**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  - _Umbral_: n≥40 por celda + gate riguroso completo (Wilson+shuffle+PnL bootstrap)
  - _Acción_: Añadir claves 'ESTRATEGIA#SUBTYPE#DIRECCION#HORA':1.2 a meta.hora_boost_factor, solo por celda confirmada
  - _Estado_: 5 celda(s) pasan gate riguroso completo de 199 evaluadas (n>=40) y 566 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.000 < 0.08 — monitorear
  - _Datos_: n=462 IC=+0.000 PNL=-4.46€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.076 n=85728 | tras_1loss IC=+0.044 n=66706 | tras_2loss IC=+0.008 n=30317/40 | gap=+0.068 (umbral 0.05)
...
**⏳ H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  ...
  - _Estado_: 5230 ops, 22 horas distintas. Sin hora con n≥15 y IC extremo aún.
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=-0.002 n=528 | contrario_BTC IC=-0.010 n=402/40 | gap=-0.008 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**🟡 H-24H-GBM-BUYYES-TARDE** — GBM BUY_YES en tarde europea (15-19h UTC) — señal alcista sostenida
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.115 > 0.08 con n=89 PNL=+16.78€
  - _Datos_: n=89 IC=+0.115 PNL=+16.78€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=5051 IC=+0.004 PNL=+89.02€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=5051 IC=+0.004 PNL=+89.02€
...
**🟡 H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.107 > 0.1 con n=497 PNL=+97.53€
  - _Datos_: n=497 IC=+0.107 PNL=+97.53€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=1200 IC=+0.036 PNL=+75.66€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1200 IC=+0.036 PNL=+75.66€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=1951 IC=-0.014 PNL=-41.83€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1951 IC=-0.014 PNL=-41.83€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=1433 IC=+0.024 PNL=+85.14€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1433 IC=+0.024 PNL=+85.14€
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=2974 IC=+0.094 PNL=+531.03€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=2974 IC=+0.094 PNL=+531.03€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=1362 IC=+0.025 PNL=+70.79€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1362 IC=+0.025 PNL=+70.79€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.158 > 0.1 con n=737 PNL=+250.74€
  - _Datos_: n=737 IC=+0.158 PNL=+250.74€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=3733 IC=-0.131 PNL=+302.80€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=3733 IC=-0.131 PNL=+302.80€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=529 IC=+0.138 PNL=+198.33€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=529 IC=+0.138 PNL=+198.33€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.107 > 0.08 con n=497 PNL=+97.53€
  - _Datos_: n=497 IC=+0.107 PNL=+97.53€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.226 < -0.1 con n=396 PNL=-26.82€
  - _Datos_: n=396 IC=-0.226 PNL=-26.82€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=915 IC=+0.150 PNL=+494.07€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=915 IC=+0.150 PNL=+494.07€
...
**〰️ H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: n=574 IC=-0.026 PNL=+33.07€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=574 IC=-0.026 PNL=+33.07€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.183 > 0.08 con n=497 PNL=+291.79€
  - _Datos_: n=497 IC=+0.183 PNL=+291.79€
...
**〰️ H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: n=885 IC=-0.042 PNL=+113.80€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=885 IC=-0.042 PNL=+113.80€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.242 > 0.08 con n=1263 PNL=-110.84€
  - _Datos_: n=1263 IC=+0.242 PNL=-110.84€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=2661 IC=+0.125 PNL=-491.77€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=2661 IC=+0.125 PNL=-491.77€
...
**⏳ H-CUSTOM-GBMLATE15M-SOL-RESCATE-PRECIO** — GBM_LATE_15M#SOL#15min#BUY_YES (pausada 05-Ago) -- posible rescate con filtro py en [0.45,0.55)
  ...
  - _Estado_: 37/40 ops en el filtro definido (IC actual=+0.141 PNL=+15.90€)
  - _Datos_: n=37 IC=+0.141 PNL=+15.90€
