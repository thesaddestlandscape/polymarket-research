# Hipótesis automáticas — 2026-07-12 09:00 UTC
_Generado por shadow_postmortem.py sobre 10686 resoluciones (PNL=+1853.05€)_
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.183 (n=219)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 12.0 (IC base=+0.148)

- **PATRÓN** `py_entrada` < `0.275` → IC=+0.318 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.275 (IC base=+0.148)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.150 (n=355)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.01 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `5897.7268` → IC=+0.162 (n=143)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 5897.7268 (IC base=+0.148)
...
- **PATRÓN** `libro_liquidez` > `6497.9556` → IC=+0.246 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 6497.9556 (IC base=+0.216)
...
- **PATRÓN** `py_entrada` < `0.245` → IC=+0.269 (n=24)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.245 (IC base=+0.148)
...
- **PATRÓN** `libro_liquidez` > `1555.2801` → IC=+0.221 (n=66)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1555.2801 (IC base=+0.200)
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.140 (n=918)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` < 11.0 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.3862` → IC=+0.180 (n=98)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.3862 (IC base=+0.110)
...
- **PATRÓN** `sigma_h` < `0.0102` → IC=+0.165 (n=150)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0102 (IC base=+0.154)

- **PATRÓN** `sigma_h` > `0.015` → IC=+0.187 (n=113)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.015 (IC base=+0.154)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.215 (n=230)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.205 (n=161)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.4922` → IC=+0.180 (n=23)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.4922 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` < `0.1221` → IC=+0.221 (n=41)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1221 (IC base=+0.167)

- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.170 (n=337)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0046 (IC base=+0.147)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.184 (n=169)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 7.0 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` < `0.2529` → IC=+0.268 (n=80)
...
- **PATRÓN** `sigma_h` < `0.0055` → IC=+0.167 (n=82)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0055 (IC base=+0.167)

- **PATRÓN** `sigma_h` > `0.0025` → IC=+0.238 (n=82)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `16.0` → IC=+0.171 (n=83)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 16.0 (IC base=+0.167)
...
- **PATRÓN** `sigma_h` > `0.0038` → IC=+0.136 (n=75)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0038 (IC base=+0.093)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.133 (n=28)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 18.0 (IC base=+0.093)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.167 (n=40)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 7.0 (IC base=+0.093)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.179 (n=54)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` > 0.0113 (IC base=+0.115)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.145 (n=29)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 7.0 (IC base=+0.115)
...
- **PATRÓN** `dist_vwap_pct` < `0.1752` → IC=+0.202 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1752 (IC base=+0.082)
...
- **PATRÓN** `sigma_h` > `0.0031` → IC=+0.124 (n=91)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` > 0.0031 (IC base=+0.077)
...
- **PATRÓN** `sigma_h` < `0.0085` → IC=+0.203 (n=35)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0085 (IC base=+0.040)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.135 (n=50)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` < 8.0 (IC base=+0.040)

- **PATRÓN** `dist_vwap_pct` < `0.1478` → IC=+0.250 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1478 (IC base=+0.040)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 817 | +0.185 | +12.18€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 524 | +0.215 | +8.53€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 242 | +0.193 | +0.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 175 | +0.217 | -2.33€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 288 | +0.169 | +3.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 173 | +0.209 | +2.51€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 285 | +0.193 | +8.60€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 174 | +0.216 | +8.30€ | 0 | 8 |
...
| ✅ GBM_LATE_15M | 3527 | +0.113 | +1146.11€ | 0 | 7 |
| ✅ GBM_LATE_15M#15min | 3527 | +0.113 | +1146.11€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 871 | +0.068 | +104.19€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 871 | +0.068 | +104.19€ | 0 | 1 |
| ✅ GBM_LATE_15M#ETH | 896 | +0.072 | +137.16€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 896 | +0.072 | +137.16€ | 0 | 1 |
| ✅ GBM_LATE_15M#SOL | 916 | +0.139 | +453.08€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 916 | +0.139 | +453.08€ | 0 | 8 |
| ✅ GBM_LATE_15M#XRP | 844 | +0.174 | +451.69€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 844 | +0.174 | +451.69€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 891 | +0.157 | +437.41€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 891 | +0.157 | +437.41€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 235 | +0.167 | +150.03€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 235 | +0.167 | +150.03€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 241 | +0.126 | +78.30€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 241 | +0.126 | +78.30€ | 0 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 250 | +0.103 | +50.87€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 250 | +0.103 | +50.87€ | 0 | 5 |
...
| ✅ GBM_LATE_15M_TARDIO | 1069 | +0.081 | +245.61€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1069 | +0.081 | +245.61€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 247 | +0.074 | +27.71€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 247 | +0.074 | +27.71€ | 0 | 2 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 268 | +0.004 | +5.54€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 268 | +0.004 | +5.54€ | 0 | 1 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 278 | +0.036 | +24.24€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 278 | +0.036 | +24.24€ | 0 | 3 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.089 n=6012 | tras_1loss IC=+0.042 n=4537 | tras_2loss IC=+0.008 n=2049/40 | gap=+0.082 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8228 IC=+0.095 PNL=+1928.94€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8228 IC=+0.095 PNL=+1928.94€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=445 IC=+0.053 PNL=+34.92€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=445 IC=+0.053 PNL=+34.92€
...
**⏳ H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: 2/40 ops en el filtro definido (IC actual=+0.000 PNL=+0.56€)
  - _Datos_: n=2 IC=+0.000 PNL=+0.56€
