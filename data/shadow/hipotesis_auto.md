# Hipótesis automáticas — 2026-07-12 11:37 UTC
_Generado por shadow_postmortem.py sobre 10851 resoluciones (PNL=+1880.62€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.219 (n=226)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.215)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.222 (n=325)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.215)

- **PATRÓN** `py_entrada` > `0.695` → IC=+0.323 (n=145)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.215)

- **PATRÓN** `libro_liquidez` > `2729.0017` → IC=+0.231 (n=281)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2729.0017 (IC base=+0.215)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.240 (n=48)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.605` → IC=+0.230 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.605 (IC base=+0.191)

- **PATRÓN** `libro_liquidez` > `8874.3159` → IC=+0.192 (n=24)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 8874.3159 (IC base=+0.191)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.300 (n=63)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.253)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.253 (n=71)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `2982.5636` → IC=+0.289 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2982.5636 (IC base=+0.253)
...
- **PATRÓN** `py_entrada` > `0.715` → IC=+0.409 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.715 (IC base=+0.220)

- **PATRÓN** `libro_liquidez` > `1870.3552` → IC=+0.260 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1870.3552 (IC base=+0.220)
...
### GBM_LATE_15M
- **PATRÓN** `sigma_h` > `0.0151` → IC=+0.137 (n=433)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0151 (IC base=+0.115)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.121 (n=1317)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` > 6.0 (IC base=+0.115)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.120 (n=872)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` < 11.0 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` < `0.1312` → IC=+0.197 (n=64)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` < 0.1312 (IC base=+0.115)

- **PATRÓN** `sigma_h` > `0.0092` → IC=+0.149 (n=915)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0092 (IC base=+0.110)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.141 (n=937)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` < 11.0 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.5163` → IC=+0.178 (n=119)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.5163 (IC base=+0.110)
...
- **PATRÓN** `dist_vwap_pct` < `0.1287` → IC=+0.132 (n=17)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.1287 (IC base=+0.121)
...
- **PATRÓN** `sigma_h` < `0.0322` → IC=+0.182 (n=300)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0322 (IC base=+0.166)

- **PATRÓN** `sigma_h` > `0.0144` → IC=+0.178 (n=268)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` > 0.0144 (IC base=+0.166)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.169 (n=303)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 6.0 (IC base=+0.166)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.184 (n=207)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 11.0 (IC base=+0.166)
...
- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.182 (n=306)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.3729` → IC=+0.143 (n=26)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.3729 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` < `0.1427` → IC=+0.198 (n=51)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.1427 (IC base=+0.164)
...
- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.208 (n=87)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.160 (n=98)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 18.0 (IC base=+0.157)

- **PATRÓN** `dist_vwap_pct` < `0.1221` → IC=+0.136 (n=20)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.1221 (IC base=+0.157)
...
- **PATRÓN** `sigma_h` < `0.0033` → IC=+0.167 (n=34)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0033 (IC base=+0.140)

- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.167 (n=67)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` > 0.0049 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.162 (n=69)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 12.0 (IC base=+0.140)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.157 (n=106)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` < 18.0 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` > `0.557` → IC=+0.278 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.557 (IC base=+0.140)
...
- **PATRÓN** `sigma_h` > `0.0112` → IC=+0.190 (n=56)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0112 (IC base=+0.119)

- **PATRÓN** `hora_utc` < `9.0` → IC=+0.151 (n=41)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 9.0 (IC base=+0.119)
...
- **PATRÓN** `sigma_h` < `0.0214` → IC=+0.312 (n=62)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0214 (IC base=+0.262)

- **PATRÓN** `hora_utc` < `13.0` → IC=+0.304 (n=44)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` > `0.0135` → IC=+0.127 (n=140)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.63€ cuando `sigma_h` > 0.0135 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` < `0.1095` → IC=+0.138 (n=67)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.1095 (IC base=+0.074)

- **PATRÓN** `sigma_h` > `0.0127` → IC=+0.152 (n=136)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0127 (IC base=+0.085)

- **PATRÓN** `dist_vwap_pct` < `0.503` → IC=+0.149 (n=112)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.503 (IC base=+0.085)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` > `0.003` → IC=+0.129 (n=95)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.64€ cuando `sigma_h` > 0.003 (IC base=+0.086)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.122 (n=88)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` < `0.0035` → IC=+0.141 (n=37)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0035 (IC base=+0.010)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.125 (n=86)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=+0.100 (n=338)

- **PATRÓN** `ibs_15` > `0.717` → IC=+0.149 (n=212)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.75€ cuando `ibs_15` > 0.717 (IC base=+0.016)

- **PATRÓN** `dist_vwap_pct` > `0.1647` → IC=+0.210 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1647 (IC base=+0.016)

- **PATRÓN** `dist_vwap_pct` < `0.3809` → IC=+0.177 (n=97)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` < 0.3809 (IC base=+0.016)
...
- **PATRÓN** `drift_60min` |x|≤ `0.237` → IC=+0.122 (n=133)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.61€ cuando `drift_60min` |x|≤ 0.237 (IC base=+0.053)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.173 (n=111)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 12.0 (IC base=+0.053)

- **PATRÓN** `ibs_15` < `0.9136` → IC=+0.149 (n=112)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.75€ cuando `ibs_15` < 0.9136 (IC base=+0.053)

- **PATRÓN** `ibs_15` > `0.6428` → IC=+0.164 (n=111)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.82€ cuando `ibs_15` > 0.6428 (IC base=+0.053)

- **PATRÓN** `dist_vwap_pct` > `0.2347` → IC=+0.326 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2347 (IC base=+0.053)
...
- **PATRÓN** `ibs_15` > `0.7622` → IC=+0.206 (n=83)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7622 (IC base=+0.029)

- **PATRÓN** `dist_vwap_pct` < `0.2737` → IC=+0.189 (n=43)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.2737 (IC base=+0.029)
...
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS > 0.6428 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.164 n=111). Confirma señal de reversión media → alinear con BUY_NO.
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS > 0.7622 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.206 n=83). Confirma señal de reversión media → alinear con BUY_NO.
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 866 | +0.187 | +15.44€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 557 | +0.214 | +10.39€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 255 | +0.193 | +1.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 186 | +0.213 | -3.83€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 306 | +0.172 | +4.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 184 | +0.210 | +4.23€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 303 | +0.195 | +10.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 185 | +0.217 | +9.96€ | 0 | 8 |
...
| ✅ GBM_LATE_15M | 3562 | +0.113 | +1148.19€ | 0 | 7 |
| ✅ GBM_LATE_15M#15min | 3562 | +0.113 | +1148.19€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 878 | +0.069 | +106.60€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 878 | +0.069 | +106.60€ | 0 | 1 |
| ✅ GBM_LATE_15M#ETH | 903 | +0.071 | +134.52€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 903 | +0.071 | +134.52€ | 0 | 1 |
| ✅ GBM_LATE_15M#SOL | 927 | +0.137 | +451.97€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 927 | +0.137 | +451.97€ | 0 | 8 |
| ✅ GBM_LATE_15M#XRP | 854 | +0.173 | +455.09€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 854 | +0.173 | +455.09€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 926 | +0.158 | +461.34€ | 0 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 926 | +0.158 | +461.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 243 | +0.165 | +151.41€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 243 | +0.165 | +151.41€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 250 | +0.119 | +77.54€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 250 | +0.119 | +77.54€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 260 | +0.111 | +62.73€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 260 | +0.111 | +62.73€ | 0 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 173 | +0.271 | +169.66€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 173 | +0.271 | +169.66€ | 0 | 5 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 1101 | +0.079 | +247.77€ | 1 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1101 | +0.079 | +247.77€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 254 | +0.078 | +29.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 254 | +0.078 | +29.98€ | 0 | 2 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 275 | -0.002 | +1.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 275 | -0.002 | +1.53€ | 0 | 1 |
...
| ✅ UPDOWN_GBM | 1314 | -0.008 | +20.17€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1058 | +0.022 | +57.64€ | 1 | 4 |
...
| ✅ UPDOWN_GBM#BTC | 352 | +0.009 | +2.66€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 273 | +0.049 | +16.04€ | 1 | 8 |
...
| ✅ UPDOWN_GBM#ETH | 648 | +0.014 | +35.94€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 555 | +0.033 | +46.99€ | 0 | 2 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.118) — sin ventaja clara. oversold(IBS<0.3): IC=-0.021 n=340 | neutral: IC=-0.048 n=257 | overbought(IBS>0.7): IC=+0.069 n=286
  - _Datos_: n=924 IC=+0.006 PNL=+45.41€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.090 n=6110 | tras_1loss IC=+0.043 n=4604 | tras_2loss IC=+0.009 n=2069/40 | gap=+0.081 (umbral 0.05)
...
**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  ...
  - _Estado_: alineada_con_outcome_prev IC=+0.114 n=68/60 | contraria IC=+0.045 n=20 | gap=+0.069 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1187 IC=-0.001 PNL=+34.17€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1187 IC=-0.001 PNL=+34.17€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=599 IC=+0.016 PNL=+22.08€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=599 IC=+0.016 PNL=+22.08€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=273 IC=+0.049 PNL=+16.04€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=273 IC=+0.049 PNL=+16.04€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=542 IC=+0.050 PNL=+38.95€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=542 IC=+0.050 PNL=+38.95€
...
**🟡 H-CUSTOM-CROSS-WINDOW-SPREAD-POS** — GBM 15min/60min: spread positivo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.229 > 0.08 con n=57 PNL=+13.03€
  - _Datos_: n=57 IC=+0.229 PNL=+13.03€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8393 IC=+0.095 PNL=+1956.50€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8393 IC=+0.095 PNL=+1956.50€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=746 IC=-0.017 PNL=-8.39€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=746 IC=-0.017 PNL=-8.39€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.173 > 0.1 con n=261 PNL=+95.28€
  - _Datos_: n=261 IC=+0.173 PNL=+95.28€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=450 IC=+0.055 PNL=+37.66€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=450 IC=+0.055 PNL=+37.66€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.175 > 0.08 con n=195 PNL=+68.33€
  - _Datos_: n=195 IC=+0.175 PNL=+68.33€
...
**🔶 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA en BTC (IC=+0.235 n=81) pero sin cruzar ≥2 pares más — ETH: n=102 IC=+0.144 ✓; SOL: n=7 IC=-0.019; XRP: n=5 IC=+0.018
  - _Datos_: n=81 IC=+0.235 PNL=+32.40€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.304 > 0.2 con n=131 PNL=+88.45€
  - _Datos_: n=131 IC=+0.304 PNL=+88.45€
...
**⏳ H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: 6/40 ops en el filtro definido (IC actual=+0.000 PNL=-0.80€)
  - _Datos_: n=6 IC=+0.000 PNL=-0.80€
