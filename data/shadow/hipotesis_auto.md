# Hipótesis automáticas — 2026-07-15 14:07 UTC
_Generado por shadow_postmortem.py sobre 15406 resoluciones (PNL=+3030.71€)_
...
- **PATRÓN** `py_entrada` > `0.695` → IC=+0.309 (n=359)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.190)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.195 (n=904)

  - _Acción_: Kelly boost +0.98€ cuando `libro_spread` < 0.01 (IC base=+0.190)

- **PATRÓN** `libro_liquidez` > `4402.2111` → IC=+0.192 (n=511)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 4402.2111 (IC base=+0.190)
...
- **PATRÓN** `py_entrada` > `0.615` → IC=+0.252 (n=151)
...
- **PATRÓN** `py_entrada` > `0.715` → IC=+0.355 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.715 (IC base=+0.235)

- **PATRÓN** `libro_liquidez` > `5149.7333` → IC=+0.244 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 5149.7333 (IC base=+0.235)
...
- **PATRÓN** `libro_liquidez` > `1636.4687` → IC=+0.225 (n=165)
...
- **PATRÓN** `py_entrada` < `0.375` → IC=+0.241 (n=168)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.375 (IC base=+0.194)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.194 (n=220)
...
### GBM_LATE_15M
- **PATRÓN** `sigma_ewma_delta_pct` > `9.332` → IC=+0.224 (n=103)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.332 (IC base=+0.107)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.133 (n=1176)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `16.029` → IC=+0.206 (n=15)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 16.029 (IC base=+0.077)

- **PATRÓN** `dist_vwap_pct` > `0.144` → IC=+0.129 (n=33)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` > 0.144 (IC base=+0.060)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.941` → IC=+0.200 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.941 (IC base=+0.060)
...
- **PATRÓN** `dist_vwap_pct` > `0.7151` → IC=+0.167 (n=34)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.7151 (IC base=+0.064)
...
- **PATRÓN** `sigma_h` < `0.0108` → IC=+0.145 (n=201)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` < 0.0108 (IC base=+0.110)
...
- **PATRÓN** `sigma_h` < `0.0135` → IC=+0.137 (n=389)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0135 (IC base=+0.129)

- **PATRÓN** `sigma_h` > `0.0093` → IC=+0.137 (n=442)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0093 (IC base=+0.129)
...
- **PATRÓN** `sigma_h` < `0.0297` → IC=+0.142 (n=420)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0297 (IC base=+0.142)

- **PATRÓN** `sigma_h` > `0.0226` → IC=+0.174 (n=191)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0226 (IC base=+0.142)
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.153 (n=292)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` < 11.0 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.6767` → IC=+0.179 (n=26)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.6767 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.431` → IC=+0.180 (n=23)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 9.431 (IC base=+0.142)

- **PATRÓN** `sigma_h` > `0.0262` → IC=+0.200 (n=148)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.5396` → IC=+0.130 (n=152)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.5396 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.082` → IC=+0.125 (n=126)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 6.082 (IC base=+0.162)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0122` → IC=+0.230 (n=220)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0122 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.175 (n=463)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 12.0 (IC base=+0.161)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.165 (n=694)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 18.0 (IC base=+0.161)

- **PATRÓN** `dist_vwap_pct` > `1.0564` → IC=+0.227 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0564 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.179` → IC=+0.308 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.179 (IC base=+0.161)

- **PATRÓN** `sigma_h` > `0.0047` → IC=+0.182 (n=684)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.4443` → IC=+0.196 (n=458)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` < 0.4443 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.097` → IC=+0.224 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.097 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.098` → IC=+0.174 (n=339)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` < 3.098 (IC base=+0.162)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **PATRÓN** `sigma_h` > `0.0027` → IC=+0.151 (n=164)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0027 (IC base=+0.123)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.178 (n=113)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 12.0 (IC base=+0.123)

- **PATRÓN** `dist_vwap_pct` > `0.132` → IC=+0.153 (n=47)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.132 (IC base=+0.123)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.412` → IC=+0.241 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.412 (IC base=+0.123)
...
- **PATRÓN** `sigma_h` > `0.0037` → IC=+0.174 (n=176)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0037 (IC base=+0.144)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.164 (n=120)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 12.0 (IC base=+0.144)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.161 (n=187)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 18.0 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `0.659` → IC=+0.227 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.659 (IC base=+0.144)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.136 (n=152)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0066 (IC base=+0.129)

- **PATRÓN** `sigma_h` > `0.0037` → IC=+0.146 (n=173)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.172 (n=59)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` < 5.0 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` > `0.7151` → IC=+0.194 (n=34)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.7151 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` < `1.1646` → IC=+0.162 (n=128)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 1.1646 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.0` → IC=+0.191 (n=40)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 5.0 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.175` → IC=+0.160 (n=98)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` < 10.175 (IC base=+0.129)
...
- **PATRÓN** `sigma_h` > `0.013` → IC=+0.211 (n=81)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.013 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.147 (n=171)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 8.0 (IC base=+0.133)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.137 (n=122)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` < 12.0 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` > `0.4625` → IC=+0.180 (n=48)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.4625 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `1.1149` → IC=+0.141 (n=129)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 1.1149 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.441` → IC=+0.259 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.441 (IC base=+0.133)

- **PATRÓN** `sigma_h` < `0.0082` → IC=+0.177 (n=63)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` < 0.0082 (IC base=+0.104)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.157 (n=68)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` < 5.0 (IC base=+0.104)

- **PATRÓN** `dist_vwap_pct` < `0.2673` → IC=+0.176 (n=103)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` < 0.2673 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.165` → IC=+0.174 (n=93)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` < 3.165 (IC base=+0.104)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` < `0.0179` → IC=+0.264 (n=142)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0179 (IC base=+0.254)

- **PATRÓN** `sigma_h` > `0.0101` → IC=+0.257 (n=142)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.295 (n=130)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.254)

- **PATRÓN** `dist_vwap_pct` > `0.9145` → IC=+0.350 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9145 (IC base=+0.254)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.136` → IC=+0.321 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.136 (IC base=+0.254)

- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.325 (n=152)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0084 (IC base=+0.288)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.296 (n=101)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 12.0 (IC base=+0.288)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.301 (n=139)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.288)

- **PATRÓN** `dist_vwap_pct` < `0.5686` → IC=+0.306 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5686 (IC base=+0.288)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.352` → IC=+0.310 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.352 (IC base=+0.288)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0147` → IC=+0.163 (n=200)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.0147 (IC base=+0.102)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.123 (n=542)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` < 15.0 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` > `0.6865` → IC=+0.167 (n=94)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.6865 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` < `0.1434` → IC=+0.125 (n=190)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.1434 (IC base=+0.102)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.366` → IC=+0.329 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.366 (IC base=+0.102)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.130 (n=125)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 7.0 (IC base=+0.083)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.632` → IC=+0.196 (n=21)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 12.632 (IC base=+0.083)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0043` → IC=+0.138 (n=67)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0043 (IC base=+0.049)

- **PATRÓN** `dist_vwap_pct` > `0.8371` → IC=+0.130 (n=25)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` > 0.8371 (IC base=+0.049)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.728` → IC=+0.350 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.728 (IC base=+0.049)
...
- **FILTRO** `sigma_ewma_delta_pct` > `3.219` → IC=-0.147 (n=15)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 3.219
...
- **PATRÓN** `sigma_h` < `0.0324` → IC=+0.194 (n=171)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0324 (IC base=+0.190)

- **PATRÓN** `sigma_h` > `0.0176` → IC=+0.215 (n=114)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0176 (IC base=+0.190)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.220 (n=155)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.190)

- **PATRÓN** `dist_vwap_pct` > `0.189` → IC=+0.227 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.189 (IC base=+0.190)

- **PATRÓN** `dist_vwap_pct` < `1.4833` → IC=+0.180 (n=95)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 1.4833 (IC base=+0.190)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.253` → IC=+0.214 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.253 (IC base=+0.190)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.141` → IC=+0.176 (n=72)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` < 11.141 (IC base=+0.190)

- **PATRÓN** `sigma_h` < `0.0135` → IC=+0.188 (n=142)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0135 (IC base=+0.188)

- **PATRÓN** `sigma_h` > `0.0174` → IC=+0.207 (n=97)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0174 (IC base=+0.188)
...
- **PATRÓN** `dist_vwap_pct` < `0.51` → IC=+0.171 (n=153)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` < 0.51 (IC base=+0.188)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.549` → IC=+0.186 (n=116)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` < 2.549 (IC base=+0.188)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.7204` → IC=+0.177 (n=258)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.88€ cuando `ibs_15` > 0.7204 (IC base=+0.027)

- **PATRÓN** `dist_vwap_pct` > `0.7405` → IC=+0.206 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7405 (IC base=+0.027)

- **PATRÓN** `dist_vwap_pct` < `0.3288` → IC=+0.150 (n=135)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.3288 (IC base=+0.027)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2429` → IC=+0.126 (n=185)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.63€ cuando `drift_60min` |x|≤ 0.2429 (IC base=+0.077)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.162 (n=155)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 11.0 (IC base=+0.077)

- **PATRÓN** `ibs_15` < `0.9252` → IC=+0.140 (n=145)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.70€ cuando `ibs_15` < 0.9252 (IC base=+0.077)

- **PATRÓN** `ibs_15` > `0.6323` → IC=+0.194 (n=145)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.97€ cuando `ibs_15` > 0.6323 (IC base=+0.077)

- **PATRÓN** `dist_vwap_pct` > `0.7448` → IC=+0.324 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7448 (IC base=+0.077)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.986` → IC=+0.197 (n=31)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` < 5.986 (IC base=+0.077)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS > 0.7204 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.177 n=258). Confirma señal de reversión media → alinear con BUY_NO.
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS > 0.6323 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.194 n=145). Confirma señal de reversión media → alinear con BUY_NO.
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2170 | +0.174 | -46.07€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1399 | +0.208 | +6.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 91 | +0.016 | -14.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 311 | +0.091 | -48.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 369 | +0.152 | +10.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 631 | +0.179 | -15.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 468 | +0.198 | -22.53€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 770 | +0.176 | +5.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 462 | +0.220 | +23.06€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 30 | -0.125 | -12.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 155 | +0.105 | -19.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 123 | +0.172 | +14.84€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL | 763 | +0.169 | -32.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 463 | +0.210 | +9.65€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 31 | +0.076 | -1.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 146 | +0.101 | -25.08€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 7 | +0.097 | -0.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 7 | +0.097 | -0.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 7 | +0.097 | -0.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 7 | +0.097 | -0.29€ | 0 | 0 |
| ✅ GBM_LATE_15M | 4593 | +0.102 | +1403.31€ | 0 | 3 |
| ✅ GBM_LATE_15M#15min | 4593 | +0.102 | +1403.31€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1102 | +0.066 | +159.02€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1102 | +0.066 | +159.02€ | 0 | 3 |
| ✅ GBM_LATE_15M#ETH | 1119 | +0.075 | +190.85€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1119 | +0.075 | +190.85€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1221 | +0.111 | +493.72€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1221 | +0.111 | +493.72€ | 0 | 6 |
| ✅ GBM_LATE_15M#XRP | 1151 | +0.152 | +559.71€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1151 | +0.152 | +559.71€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1847 | +0.153 | +1091.96€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1847 | +0.153 | +1091.96€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 472 | +0.124 | +248.88€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 472 | +0.124 | +248.88€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 487 | +0.128 | +223.22€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 487 | +0.128 | +223.22€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 498 | +0.110 | +215.81€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 498 | +0.110 | +215.81€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 390 | +0.273 | +404.06€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 390 | +0.273 | +404.06€ | 0 | 11 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 29 | -0.113 | -1.59€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 29 | -0.113 | -1.59€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 5 | +0.054 | +1.47€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 5 | +0.054 | +1.47€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 1801 | +0.084 | +496.81€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1801 | +0.084 | +496.81€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 447 | +0.061 | +74.17€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 447 | +0.061 | +74.17€ | 0 | 2 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 404 | +0.015 | +9.89€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 404 | +0.015 | +9.89€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 440 | +0.048 | +71.99€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 440 | +0.048 | +71.99€ | 2 | 5 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 510 | +0.190 | +340.76€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 510 | +0.190 | +340.76€ | 0 | 12 |
...
| ✅ LEADLAG_BTC_XRP_15M | 120 | +0.082 | +28.28€ | 0 | 4 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 120 | +0.082 | +28.28€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 120 | +0.082 | +28.28€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 120 | +0.082 | +28.28€ | 0 | 4 |
...
| ✅ UPDOWN_GBM | 1547 | +0.010 | +74.99€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1290 | +0.037 | +111.52€ | 0 | 7 |
...
| ✅ UPDOWN_GBM#BTC | 400 | +0.030 | +31.61€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 320 | +0.068 | +44.05€ | 1 | 10 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 16 | +0.311 | +10.11€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 16 | +0.311 | +10.11€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 6 | +0.113 | +5.68€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 6 | +0.113 | +5.68€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 5 | +0.054 | +0.92€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 5 | +0.054 | +0.92€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.121) — sin ventaja clara. oversold(IBS<0.3): IC=-0.002 n=432 | neutral: IC=-0.012 n=332 | overbought(IBS>0.7): IC=+0.109 n=348
  - _Datos_: n=1175 IC=+0.034 PNL=+110.77€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=02h: IC=-0.105 n=41 PNL=-7.10€ → FILTRAR | H=07h: IC=+0.135 n=50 PNL=+24.03€ → BOOST | H=10h: IC=-0.154 n=15 PNL=-6.77€ → FILTRAR | H=14h: IC=+0.133 n=47 PNL=+18.07€ → BOOST | H=16h: IC=+0.111 n=52 PNL=+10.12€ → BOOST
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.099 n=8869 | tras_1loss IC=+0.061 n=6318 | tras_2loss IC=+0.029 n=2740/40 | gap=+0.070 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1420 IC=+0.017 PNL=+88.99€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1420 IC=+0.017 PNL=+88.99€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=691 IC=+0.027 PNL=+46.48€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=691 IC=+0.027 PNL=+46.48€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=320 IC=+0.068 PNL=+44.05€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=320 IC=+0.068 PNL=+44.05€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=679 IC=+0.074 PNL=+89.33€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=679 IC=+0.074 PNL=+89.33€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=12948 IC=+0.102 PNL=+3106.59€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=12948 IC=+0.102 PNL=+3106.59€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=147 IC=+0.010 PNL=+4.21€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=147 IC=+0.010 PNL=+4.21€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=839 IC=-0.004 PNL=+16.95€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=839 IC=-0.004 PNL=+16.95€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.168 > 0.1 con n=389 PNL=+139.65€
  - _Datos_: n=389 IC=+0.168 PNL=+139.65€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=587 IC=+0.057 PNL=+68.21€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=587 IC=+0.057 PNL=+68.21€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.150 > 0.08 con n=287 PNL=+92.72€
  - _Datos_: n=287 IC=+0.150 PNL=+92.72€
...
**🟡 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.219 > 0.08 con n=126 PNL=+60.51€
  - _Datos_: n=126 IC=+0.219 PNL=+60.51€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.250 > 0.2 con n=302 PNL=+202.59€
  - _Datos_: n=302 IC=+0.250 PNL=+202.59€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.132 > 0.08 con n=161 PNL=+81.11€
  - _Datos_: n=161 IC=+0.132 PNL=+81.11€
