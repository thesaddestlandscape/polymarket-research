# Hipótesis automáticas — 2026-07-16 16:34 UTC
_Generado por shadow_postmortem.py sobre 17270 resoluciones (PNL=+3507.81€)_
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.196 (n=515)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.180 (n=1177)

  - _Acción_: Kelly boost +0.90€ cuando `libro_spread` < 0.01 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `7377.4888` → IC=+0.189 (n=342)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 7377.4888 (IC base=+0.169)
...
- **PATRÓN** `libro_liquidez` > `8047.2094` → IC=+0.218 (n=147)
...
- **PATRÓN** `py_entrada` < `0.425` → IC=+0.224 (n=56)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.425 (IC base=+0.154)

- **PATRÓN** `libro_liquidez` > `7105.7782` → IC=+0.173 (n=53)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 7105.7782 (IC base=+0.154)
...
- **PATRÓN** `libro_liquidez` > `3727.5827` → IC=+0.217 (n=217)
...
- **PATRÓN** `py_entrada` < `0.375` → IC=+0.260 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.375 (IC base=+0.154)

- **PATRÓN** `libro_liquidez` > `4553.9223` → IC=+0.178 (n=57)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 4553.9223 (IC base=+0.154)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.241 (n=106)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.365` → IC=+0.264 (n=193)
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.125 (n=22)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 16.0 (IC base=+0.085)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.145 (n=29)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 7.0 (IC base=+0.085)

- **PATRÓN** `py_entrada` < `0.335` → IC=+0.283 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.335 (IC base=+0.085)
...
- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.134 (n=1263)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `12.702` → IC=+0.155 (n=27)
  ...
  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 12.702 (IC base=+0.071)

- **PATRÓN** `dist_vwap_pct` > `0.1365` → IC=+0.136 (n=42)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 0.1365 (IC base=+0.061)
...
- **PATRÓN** `dist_vwap_pct` > `1.0425` → IC=+0.143 (n=26)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 1.0425 (IC base=+0.066)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.518` → IC=+0.181 (n=45)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 7.518 (IC base=+0.066)
...
- **PATRÓN** `sigma_h` < `0.0107` → IC=+0.141 (n=215)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.561` → IC=+0.202 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.561 (IC base=+0.108)
...
- **PATRÓN** `sigma_h` < `0.0311` → IC=+0.151 (n=456)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0311 (IC base=+0.149)

- **PATRÓN** `sigma_h` > `0.0248` → IC=+0.179 (n=207)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0248 (IC base=+0.149)
...
- **PATRÓN** `dist_vwap_pct` > `0.602` → IC=+0.235 (n=32)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.602 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` < `0.1341` → IC=+0.121 (n=130)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.1341 (IC base=+0.149)
...
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.222 (n=343)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0108 (IC base=+0.156)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.172 (n=541)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 12.0 (IC base=+0.156)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.162 (n=793)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 18.0 (IC base=+0.156)

- **PATRÓN** `dist_vwap_pct` > `0.1497` → IC=+0.198 (n=289)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.1497 (IC base=+0.156)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.007` → IC=+0.294 (n=95)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.007 (IC base=+0.156)

- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.181 (n=788)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` > 0.0048 (IC base=+0.163)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.171 (n=375)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.4123` → IC=+0.193 (n=577)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` < 0.4123 (IC base=+0.163)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.037` → IC=+0.227 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.037 (IC base=+0.163)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **PATRÓN** `sigma_h` > `0.0028` → IC=+0.132 (n=183)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0028 (IC base=+0.108)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.154 (n=131)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 12.0 (IC base=+0.108)

- **PATRÓN** `dist_vwap_pct` > `0.5034` → IC=+0.167 (n=16)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.5034 (IC base=+0.108)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.905` → IC=+0.167 (n=31)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 10.905 (IC base=+0.108)

- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.163 (n=200)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.0029 (IC base=+0.134)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.170 (n=92)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 15.0 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` > `0.1365` → IC=+0.143 (n=40)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.1365 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` < `0.466` → IC=+0.129 (n=165)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.466 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.397` → IC=+0.275 (n=38)
...
- **PATRÓN** `sigma_h` < `0.007` → IC=+0.144 (n=178)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` < 0.007 (IC base=+0.131)

- **PATRÓN** `sigma_h` > `0.0038` → IC=+0.152 (n=202)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0038 (IC base=+0.131)
...
- **PATRÓN** `dist_vwap_pct` > `0.8068` → IC=+0.183 (n=39)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.8068 (IC base=+0.131)

- **PATRÓN** `dist_vwap_pct` < `0.4154` → IC=+0.167 (n=124)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.4154 (IC base=+0.131)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.997` → IC=+0.211 (n=43)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.997 (IC base=+0.131)

- **PATRÓN** `sigma_ewma_delta_pct` < `12.305` → IC=+0.157 (n=135)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` < 12.305 (IC base=+0.131)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.160 (n=151)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 12.0 (IC base=+0.138)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.142 (n=185)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` < 15.0 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` > `0.2346` → IC=+0.186 (n=68)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.2346 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.9751` → IC=+0.148 (n=163)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.9751 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.082` → IC=+0.270 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.082 (IC base=+0.138)
...
- **PATRÓN** `sigma_h` > `0.0153` → IC=+0.196 (n=238)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0153 (IC base=+0.109)
...
- **PATRÓN** `dist_vwap_pct` < `0.1397` → IC=+0.131 (n=285)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.1397 (IC base=+0.109)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.023` → IC=+0.302 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.023 (IC base=+0.109)
...
- **PATRÓN** `dist_vwap_pct` > `0.4799` → IC=+0.125 (n=22)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` > 0.4799 (IC base=+0.077)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.68` → IC=+0.167 (n=22)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 13.68 (IC base=+0.077)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.626` → IC=+0.140 (n=23)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` > 7.626 (IC base=+0.018)
...
- **PATRÓN** `sigma_h` > `0.0274` → IC=+0.265 (n=83)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0274 (IC base=+0.198)
...
- **PATRÓN** `dist_vwap_pct` < `0.5914` → IC=+0.186 (n=202)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` < 0.5914 (IC base=+0.198)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.517` → IC=+0.208 (n=152)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.517 (IC base=+0.198)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `8.037` → IC=+0.120 (n=135)

  - _Acción_: Kelly boost +0.60€ cuando `sigma_ewma_delta_pct` < 8.037 (IC base=+0.044)
...
- **PATRÓN** `dist_vwap_pct` > `0.9766` → IC=+0.200 (n=18)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9766 (IC base=+0.041)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2626 | +0.176 | -46.67€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1690 | +0.213 | +32.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min |