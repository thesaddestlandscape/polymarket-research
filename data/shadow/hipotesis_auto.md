# Hipótesis automáticas — 2026-09-05 00:13 UTC
_Generado por shadow_postmortem.py sobre 288102 resoluciones (PNL=+28229.63€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `147.36` → IC=-0.300 (n=3868)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 147.36
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=11612)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `133.51` → IC=-0.325 (n=513)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 133.51
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=1541)
...
- **FILTRO** `restante_s_al_confirmar` < `144.69` → IC=-0.312 (n=858)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 144.69
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=2576)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `158.35` → IC=-0.378 (n=931)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.35
  - _Potencial_: sin este filtro IC_bueno=-0.101 (n=1894)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.190 (n=8204)
...
- **PATRÓN** `libro_liquidez` > `2369.6362` → IC=+0.169 (n=2002)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.192 (n=1212)
...
- **PATRÓN** `libro_liquidez` > `933.731` → IC=+0.206 (n=801)
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.179 (n=1643)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.209 (n=18215)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.182 (n=2699)
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.272 (n=1781)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.230 (n=1541)
...
- **PATRÓN** `restante_min` > `4.95` → IC=+0.169 (n=1230)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.135 (n=1783)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` < 8.0 (IC base=+0.125)

- **PATRÓN** `lag_apertura_s` < `3.28` → IC=+0.169 (n=1219)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3616` → IC=+0.130 (n=3859)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.651` → IC=+0.151 (n=1472)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1707` → IC=+0.184 (n=668)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.1707 (IC base=+0.084)

- **PATRÓN** `volumen_spike_ratio` < `2.8467` → IC=+0.179 (n=2337)
...
- **PATRÓN** `ibs_20min` < `0.4016` → IC=+0.128 (n=3695)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` < 0.4016 (IC base=+0.042)

- **PATRÓN** `dist_vwap_pct` < `0.3424` → IC=+0.147 (n=1382)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.3424 (IC base=+0.042)

- **PATRÓN** `volumen_regimen` < `0.6858` → IC=+0.149 (n=582)
...
- **PATRÓN** `sigma_h` < `0.004` → IC=+0.153 (n=491)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.004 (IC base=+0.141)

- **PATRÓN** `sigma_h` > `0.003` → IC=+0.146 (n=659)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` > 0.003 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.1757` → IC=+0.149 (n=491)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.1757 (IC base=+0.141)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.145 (n=665)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 8.0 (IC base=+0.141)

- **PATRÓN** `ibs_20min` < `0.6603` → IC=+0.168 (n=736)

  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` < 0.6603 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` < `0.342` → IC=+0.161 (n=720)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` < 0.342 (IC base=+0.141)
...
- **PATRÓN** `volumen_regimen` < `0.615` → IC=+0.173 (n=246)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` < 0.615 (IC base=+0.141)
...
- **PATRÓN** `volumen_spike_ratio` > `1.3961` → IC=+0.158 (n=627)
...
- **PATRÓN** `ibs_20min` > `0.3007` → IC=+0.146 (n=4756)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.479` → IC=+0.123 (n=2518)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.251` → IC=+0.245 (n=461)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8023` → IC=+0.218 (n=788)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8023 (IC base=+0.076)

- **PATRÓN** `ballena_activa_n` < `100.0` → IC=+0.294 (n=1825)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 100.0 (IC base=+0.076)

- **PATRÓN** `ibs_20min` < `0.5834` → IC=+0.132 (n=4592)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.5834 (IC base=+0.053)
...
- **FILTRO** `ibs_20min` > `0.8608` → IC=-0.158 (n=396)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=1191)
...
- **FILTRO** `ibs_20min` > `0.8648` → IC=-0.159 (n=312)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3416` → IC=+0.171 (n=3525)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `dist_vwap_pct` > `0.8014` → IC=+0.233 (n=841)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8014 (IC base=+0.163)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.728` → IC=+0.236 (n=1618)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.728 (IC base=+0.163)

- **PATRÓN** `volumen_regimen` > `0.6285` → IC=+0.163 (n=2706)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.6285 (IC base=+0.163)

- **PATRÓN** `volumen_pendiente_norm` > `0.1052` → IC=+0.187 (n=1504)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1052 (IC base=+0.163)

- **PATRÓN** `volumen_spike_ratio` < `2.3154` → IC=+0.165 (n=3287)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.3154 (IC base=+0.163)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.165 (n=4098)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.03 (IC base=+0.163)

- **PATRÓN** `libro_liquidez` > `3951.6306` → IC=+0.174 (n=1335)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 3951.6306 (IC base=+0.163)

- **PATRÓN** `ballena_activa_n` < `90.0` → IC=+0.183 (n=2572)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 90.0 (IC base=+0.163)

- **PATRÓN** `sigma_h` < `0.0084` → IC=+0.188 (n=3282)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0084 (IC base=+0.177)

- **PATRÓN** `drift_60min` |x|≤ `0.0795` → IC=+0.211 (n=1243)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4387` → IC=+0.227 (n=3729)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4387 (IC base=+0.177)

- **PATRÓN** `dist_vwap_pct` < `0.2273` → IC=+0.170 (n=2834)
...
- **PATRÓN** `volumen_regimen` < `1.1693` → IC=+0.164 (n=2812)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.1693 (IC base=+0.177)

- **PATRÓN** `volumen_regimen` > `0.6213` → IC=+0.159 (n=2812)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 0.6213 (IC base=+0.177)
...
- **PATRÓN** `volumen_spike_ratio` > `2.653` → IC=+0.202 (n=1034)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.653 (IC base=+0.177)

- **PATRÓN** `ballena_activa_n` < `157.0` → IC=+0.176 (n=2690)
...
- **PATRÓN** `ibs_20min` < `0.4851` → IC=+0.192 (n=682)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` < 0.4851 (IC base=+0.155)

- **PATRÓN** `dist_vwap_pct` < `0.2004` → IC=+0.173 (n=690)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.2004 (IC base=+0.155)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.232` → IC=+0.243 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.232 (IC base=+0.155)

- **PATRÓN** `volumen_regimen` < `0.6986` → IC=+0.222 (n=300)
...
- **PATRÓN** `volumen_spike_ratio` > `1.3943` → IC=+0.165 (n=574)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1779` → IC=+0.201 (n=419)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_spike_ratio` < `3.1555` → IC=+0.167 (n=497)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.198 (n=643)
...
- **PATRÓN** `sigma_h` > `0.008` → IC=+0.172 (n=394)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.008 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.0967` → IC=+0.147 (n=290)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.0967 (IC base=+0.141)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.185 (n=607)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 12.0 (IC base=+0.141)

- **PATRÓN** `ibs_20min` > `0.4481` → IC=+0.172 (n=867)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` > 0.4481 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` > `0.9747` → IC=+0.229 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9747 (IC base=+0.141)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.607` → IC=+0.177 (n=398)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 3.607 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` < `0.8601` → IC=+0.160 (n=466)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.8601 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` > `1.1634` → IC=+0.147 (n=233)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 1.1634 (IC base=+0.141)

- **PATRÓN** `volumen_pendiente_norm` > `0.1712` → IC=+0.176 (n=239)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.1712 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` < `1.4532` → IC=+0.159 (n=271)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 1.4532 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` > `1.8216` → IC=+0.145 (n=542)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.8216 (IC base=+0.141)
...
- **PATRÓN** `libro_liquidez` > `9457.747` → IC=+0.191 (n=289)
  ...
  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 9457.747 (IC base=+0.141)
...
- **PATRÓN** `ibs_20min` > `0.4884` → IC=+0.209 (n=4842)
...
- **PATRÓN** `dist_vwap_pct` < `0.4491` → IC=+0.171 (n=3167)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` < 0.4491 (IC base=+0.187)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.7` → IC=+0.188 (n=4164)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` < 2.7 (IC base=+0.187)

- **PATRÓN** `volumen_regimen` < `0.6213` → IC=+0.166 (n=1053)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.6213 (IC base=+0.187)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2973` → IC=+0.201 (n=1696)
...
- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.173 (n=720)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.2716` → IC=+0.182 (n=633)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.91€ cuando `drift_60min` |x|≤ 0.2716 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.165 (n=648)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 8.0 (IC base=+0.165)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.170 (n=510)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 12.0 (IC base=+0.165)

- **PATRÓN** `ibs_20min` < `0.6154` → IC=+0.205 (n=720)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6154 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` < `0.1479` → IC=+0.181 (n=626)
...
- **PATRÓN** `volumen_regimen` < `0.6166` → IC=+0.231 (n=240)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6166 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` > `0.1417` → IC=+0.237 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1417 (IC base=+0.165)
...
- **PATRÓN** `volumen_spike_ratio` > `2.0524` → IC=+0.195 (n=283)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 2.0524 (IC base=+0.165)
...
- **PATRÓN** `ibs_20min` > `0.6809` → IC=+0.254 (n=628)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2205` → IC=+0.211 (n=641)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2205 (IC base=+0.207)

- **PATRÓN** `volumen_spike_ratio` < `3.9028` → IC=+0.205 (n=629)
...
- **PATRÓN** `libro_liquidez` > `1458.7184` → IC=+0.209 (n=703)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1458.7184 (IC base=+0.207)

- **PATRÓN** `ballena_activa_n` < `65.0` → IC=+0.255 (n=418)
...
- **PATRÓN** `ibs_20min` > `0.4737` → IC=+0.179 (n=839)
...
- **PATRÓN** `ballena_activa_n` < `63.0` → IC=+0.138 (n=619)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.130 (n=1781)

- **PATRÓN** `sigma_h` < `0.01` → IC=+0.137 (n=1379)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.01 (IC base=+0.121)

- **PATRÓN** `drift_60min` |x|≤ `0.4317` → IC=+0.134 (n=1379)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.9278` → IC=+0.191 (n=522)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` > 0.9278 (IC base=+0.121)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.786` → IC=+0.122 (n=1429)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` < 3.786 (IC base=+0.121)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5059` → IC=+0.147 (n=1348)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_ewma_delta_pct` < `6.329` → IC=+0.140 (n=1346)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1485` → IC=+0.129 (n=1348)
...
- **PATRÓN** `volumen_spike_ratio` < `2.489` → IC=+0.136 (n=1333)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 2.489 (IC base=+0.127)

- **PATRÓN** `volumen_spike_ratio` > `1.7971` → IC=+0.134 (n=889)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 1.7971 (IC base=+0.127)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.130 (n=1781)
...
- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.196 (n=123)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0064 (IC base=+0.145)

- **PATRÓN** `sigma_h` > `0.0104` → IC=+0.182 (n=168)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` > 0.0104 (IC base=+0.145)

- **PATRÓN** `drift_60min` |x|≤ `0.4317` → IC=+0.152 (n=323)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.4317 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.188 (n=335)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 8.0 (IC base=+0.145)

- **PATRÓN** `ibs_20min` > `0.9804` → IC=+0.234 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9804 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.299` → IC=+0.223 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.299 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` > `0.0966` → IC=+0.165 (n=150)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.0966 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` < `3.5716` → IC=+0.148 (n=365)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 3.5716 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` > `2.4093` → IC=+0.157 (n=243)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 2.4093 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `1770.637` → IC=+0.149 (n=366)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 1770.637 (IC base=+0.145)

- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.333 (n=16)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0073 (IC base=+0.292)

- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.300 (n=23)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0086 (IC base=+0.292)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.328 (n=27)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.292)

- **PATRÓN** `ibs_20min` > `0.65` → IC=+0.333 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.65 (IC base=+0.292)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.158` → IC=+0.333 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.158 (IC base=+0.292)

- **PATRÓN** `volumen_spike_ratio` < `2.6654` → IC=+0.312 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.6654 (IC base=+0.292)

- **PATRÓN** `ballena_activa_n` < `29.0` → IC=+0.348 (n=31)
...
- **FILTRO** `py_entrada` < `0.41` → IC=-0.224 (n=237)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=728)
...
- **FILTRO** `py_entrada` > `0.61` → IC=-0.162 (n=264)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=811)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.195 (n=260)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=829)
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.282 (n=3658)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=11354)

- **FILTRO** `ibs_7min` < `0.7143` → IC=-0.241 (n=3735)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=11277)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.170 (n=5080)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=9932)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.221 (n=4637)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=14318)

- **FILTRO** `ibs_7min` > `0.7143` → IC=-0.173 (n=4724)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=14231)
...
- **FILTRO** `ibs_7min` > `0.837` → IC=-0.197 (n=823)

  - _Acción_: SKIP cuando `ibs_7min` > 0.837
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=2473)
...
- **FILTRO** `ibs_7min` < `0.7701` → IC=-0.194 (n=695)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7701
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=2087)

- **FILTRO** `ballena_activa_n` > `168.0` → IC=-0.178 (n=691)

  - _Acción_: SKIP cuando `ballena_activa_n` > 168.0
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=2091)

- **FILTRO** `py_entrada` > `0.64` → IC=-0.242 (n=664)

  - _Acción_: SKIP cuando `py_entrada` > 0.64
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=2135)
...
- **FILTRO** `ibs_7min` < `0.2174` → IC=-0.295 (n=553)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=1663)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.238 (n=537)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.091 (n=1679)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.224 (n=800)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=2617)

- **FILTRO** `ibs_7min` > `0.797` → IC=-0.169 (n=854)
...
- **FILTRO** `ballena_activa_n` > `34.0` → IC=-0.186 (n=626)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=1932)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.176 (n=847)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=2717)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.291 (n=602)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=1871)

- **FILTRO** `ibs_7min` < `0.7368` → IC=-0.235 (n=618)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7368
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=1855)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.225 (n=606)
...
- **FILTRO** `py_entrada` > `0.69` → IC=-0.210 (n=820)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=2501)

- **FILTRO** `ibs_7min` > `0.7647` → IC=-0.162 (n=830)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7647
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=2491)
...
- **PATRÓN** `delta_ratio` |x|> `0.3986` → IC=+0.147 (n=525)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.74€ cuando `delta_ratio` |x|> 0.3986 (IC base=+0.132)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.154 (n=215)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 15.0 (IC base=+0.132)

- **PATRÓN** `total_vol_5m` < `456.268` → IC=+0.165 (n=168)

  - _Acción_: Kelly boost +0.82€ cuando `total_vol_5m` < 456.268 (IC base=+0.132)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.132 (n=245)

  - _Acción_: Kelly boost +0.66€ cuando `libro_spread` < 0.01 (IC base=+0.132)

- **PATRÓN** `libro_liquidez` > `3338.8044` → IC=+0.157 (n=205)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 3338.8044 (IC base=+0.132)

- **PATRÓN** `ballena_activa_n` < `64.0` → IC=+0.133 (n=369)

  - _Acción_: Kelly boost +0.67€ cuando `ballena_activa_n` < 64.0 (IC base=+0.132)
...
- **PATRÓN** `ballena_activa_n` < `57.0` → IC=+0.176 (n=72)

  - _Acción_: Kelly boost +0.88€ cuando `ballena_activa_n` < 57.0 (IC base=+0.131)
...
- **PATRÓN** `ibs_15` < `0.1171` → IC=+0.180 (n=179)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.90€ cuando `ibs_15` < 0.1171 (IC base=+0.032)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1246` → IC=+0.229 (n=334)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **PATRÓN** `dist_vwap_pct` < `0.4624` → IC=+0.224 (n=534)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4624 (IC base=-0.062)

- **PATRÓN** `ballena_activa_n` < `128.0` → IC=+0.228 (n=464)
...
- **PATRÓN** `dist_vwap_pct` > `0.1609` → IC=+0.337 (n=41)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1609 (IC base=-0.059)
...
- **FILTRO** `drift_15min` |x|> `0.6616` → IC=-0.154 (n=53)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 0.6616
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1171 sube el IC de +0.032 a +0.180 en UPDOWN_GBM#XRP#15min (n=179). Ya aplicado como kelly_boost=+0.90€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 15480 | -0.116 | -2609.60€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 975 | -0.014 | -145.07€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 14505 | -0.123 | -2464.53€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 2054 | -0.103 | -468.97€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 2054 | -0.103 | -468.97€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 975 | -0.014 | -145.07€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 975 | -0.014 | -145.07€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 2065 | -0.174 | -572.88€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 2065 | -0.174 | -572.88€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 4127 | -0.054 | -393.32€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 4127 | -0.054 | -393.32€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 3434 | -0.129 | -274.38€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 3434 | -0.129 | -274.38€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 2825 | -0.193 | -754.97€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 2825 | -0.193 | -754.97€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 4618 | -0.078 | +1868.34€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 1319 | -0.013 | +969.67€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 3299 | -0.104 | +898.67€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 4618 | -0.078 | +1868.34€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 1319 | -0.013 | +969.67€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 3299 | -0.104 | +898.67€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 49692 | +0.114 | -3010.20€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 8274 | +0.183 | -276.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 174 | -0.108 | -55.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 37423 | +0.099 | -2597.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3821 | +0.116 | -80.53€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 6281 | +0.085 | -795.90€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 27 | -0.121 | +3.75€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 6239 | +0.087 | -787.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 9942 | +0.132 | -217.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 2439 | +0.199 | -111.05€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 6182 | +0.109 | -130.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1279 | +0.124 | +46.35€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 6300 | +0.084 | -762.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 29 | +0.016 | +3.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#240min | 13 | -0.195 | -9.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 6258 | +0.085 | -756.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 10742 | +0.127 | -157.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 3088 | +0.172 | -18.89€ | 1 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 6224 | +0.112 | -84.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1418 | +0.095 | -45.09€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 10147 | +0.123 | -657.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2670 | +0.190 | -154.94€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 85 | -0.017 | -4.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 6268 | +0.095 | -416.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1124 | +0.131 | -81.80€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 6280 | +0.105 | -419.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 21 | -0.022 | +1.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 7 | +0.019 | +0.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 6252 | +0.106 | -421.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 8192 | +0.178 | -632.65€ | 2 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 8192 | +0.178 | -632.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 2092 | +0.165 | -235.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 2092 | +0.165 | -235.15€ | 0 | 2 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 1831 | +0.236 | -43.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 1831 | +0.236 | -43.06€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 1997 | +0.184 | -155.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 1997 | +0.184 | -155.11€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 26577 | +0.189 | -2451.90€ | 2 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 26577 | +0.189 | -2451.90€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 4723 | +0.151 | -726.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 4723 | +0.151 | -726.84€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 4149 | +0.225 | -143.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 4149 | +0.225 | -143.15€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 4596 | +0.164 | -617.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 4596 | +0.164 | -617.50€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 4237 | +0.219 | -173.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 4237 | +0.219 | -173.12€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 4397 | +0.199 | -326.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 4397 | +0.199 | -326.24€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 4475 | +0.184 | -465.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 4475 | +0.184 | -465.05€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 9664 | +0.131 | +325.19€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 9664 | +0.131 | +325.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 4793 | +0.136 | +206.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 