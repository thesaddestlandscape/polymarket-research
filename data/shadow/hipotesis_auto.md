# Hipótesis automáticas — 2026-08-25 07:01 UTC
_Generado por shadow_postmortem.py sobre 147646 resoluciones (PNL=+9990.86€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `151.68` → IC=-0.263 (n=2065)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 151.68
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=6198)
...
- **FILTRO** `restante_s_al_confirmar` < `85.16` → IC=-0.450 (n=240)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 85.16
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=720)

### BALLENAS_TARDIAS#ETH#5min
- **FILTRO** `restante_s_al_confirmar` < `159.14` → IC=-0.183 (n=556)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 159.14
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=1669)
...
- **FILTRO** `restante_s_al_confirmar` < `164.39` → IC=-0.311 (n=475)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 164.39
  - _Potencial_: sin este filtro IC_bueno=-0.108 (n=965)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.190 (n=4851)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.288 (n=423)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.281)

- **PATRÓN** `py_entrada` < `0.375` → IC=+0.326 (n=405)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.375 (IC base=+0.281)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.283 (n=435)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.281)

- **PATRÓN** `libro_liquidez` > `2826.4245` → IC=+0.293 (n=269)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2826.4245 (IC base=+0.281)
...
- **PATRÓN** `py_entrada` < `0.305` → IC=+0.303 (n=373)
...
- **PATRÓN** `libro_liquidez` > `830.4526` → IC=+0.233 (n=548)
...
- **PATRÓN** `py_entrada` < `0.34` → IC=+0.213 (n=155)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.34 (IC base=+0.098)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.183 (n=1009)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 17.0 (IC base=+0.173)

- **PATRÓN** `py_entrada` < `0.73` → IC=+0.192 (n=990)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.167 (n=1040)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 5.0 (IC base=+0.162)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.169 (n=890)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.166 (n=534)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.148 (n=1732)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.207 (n=725)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.38 (IC base=+0.139)

- **PATRÓN** `restante_min` < `3.87` → IC=+0.152 (n=627)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.161 (n=854)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.189 (n=642)

  - _Acción_: Kelly boost +0.95€ cuando `py_entrada` < 0.37 (IC base=+0.122)

- **PATRÓN** `restante_min` < `3.95` → IC=+0.137 (n=635)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.123 (n=1960)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 5.0 (IC base=+0.122)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.134 (n=878)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1062` → IC=+0.136 (n=950)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4826` → IC=+0.138 (n=879)
...
- **PATRÓN** `ibs_20min` < `0.2133` → IC=+0.123 (n=1345)

  - _Acción_: Kelly boost +0.61€ cuando `ibs_20min` < 0.2133 (IC base=+0.028)

- **PATRÓN** `dist_vwap_pct` < `0.1781` → IC=+0.149 (n=653)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6146` → IC=+0.219 (n=265)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6146 (IC base=+0.028)

- **PATRÓN** `volumen_spike_ratio` > `2.9495` → IC=+0.228 (n=200)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.147 (n=409)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 18.0 (IC base=+0.137)

- **PATRÓN** `ibs_20min` < `0.4244` → IC=+0.169 (n=345)

  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` < 0.4244 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.191` → IC=+0.168 (n=335)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` < 0.191 (IC base=+0.137)
...
- **PATRÓN** `volumen_regimen` < `0.6279` → IC=+0.177 (n=131)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 0.6279 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` > `1.025` → IC=+0.144 (n=178)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 1.025 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` > `0.0963` → IC=+0.222 (n=113)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0963 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` < `1.7604` → IC=+0.182 (n=193)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 1.7604 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `1.4188` → IC=+0.155 (n=288)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.4188 (IC base=+0.137)
...
- **PATRÓN** `libro_liquidez` > `12624.5033` → IC=+0.161 (n=178)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 12624.5033 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `235.0` → IC=+0.171 (n=68)

  - _Acción_: Kelly boost +0.86€ cuando `ballena_activa_n` < 235.0 (IC base=+0.137)
...
- **FILTRO** `sigma_h` > `0.0065` → IC=-0.144 (n=172)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=523)
...
- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.134 (n=1111)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` > 0.0072 (IC base=+0.055)

- **PATRÓN** `ibs_20min` > `0.9474` → IC=+0.254 (n=818)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.353` → IC=+0.122 (n=1394)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` > 2.353 (IC base=+0.055)

- **PATRÓN** `volumen_regimen` > `1.1694` → IC=+0.196 (n=287)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` > 1.1694 (IC base=+0.055)

- **PATRÓN** `volumen_pendiente_norm` < `0.1152` → IC=+0.163 (n=1072)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` < 0.1152 (IC base=+0.055)

- **PATRÓN** `volumen_pendiente_norm` > `0.2417` → IC=+0.212 (n=217)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2417 (IC base=+0.055)

- **PATRÓN** `volumen_spike_ratio` < `1.4824` → IC=+0.184 (n=368)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` < 1.4824 (IC base=+0.055)

- **PATRÓN** `volumen_spike_ratio` > `2.8774` → IC=+0.176 (n=368)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.8774 (IC base=+0.055)

- **PATRÓN** `ballena_activa_n` < `103.0` → IC=+0.272 (n=550)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 103.0 (IC base=+0.055)
...
- **PATRÓN** `dist_vwap_pct` < `0.1366` → IC=+0.192 (n=632)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` < 0.1366 (IC base=+0.034)

- **PATRÓN** `volumen_regimen` > `0.6318` → IC=+0.204 (n=684)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6318 (IC base=+0.034)

- **PATRÓN** `volumen_pendiente_norm` > `0.2475` → IC=+0.332 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2475 (IC base=+0.034)

- **PATRÓN** `volumen_spike_ratio` > `2.8551` → IC=+0.286 (n=166)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8551 (IC base=+0.034)

- **PATRÓN** `ballena_activa_n` < `53.0` → IC=+0.255 (n=332)
...
- **PATRÓN** `ibs_20min` > `0.8511` → IC=+0.250 (n=138)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2556` → IC=+0.318 (n=20)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5739` → IC=+0.151 (n=130)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.5739 (IC base=-0.002)

- **PATRÓN** `ballena_activa_n` < `79.0` → IC=+0.210 (n=112)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.12` → IC=+0.276 (n=301)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.12 (IC base=+0.207)

- **PATRÓN** `volumen_regimen` > `0.8342` → IC=+0.243 (n=337)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8342 (IC base=+0.207)

- **PATRÓN** `volumen_pendiente_norm` < `0.0807` → IC=+0.208 (n=426)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0807 (IC base=+0.207)

- **PATRÓN** `volumen_pendiente_norm` > `0.283` → IC=+0.276 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.283 (IC base=+0.207)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.225 (n=562)
...
- **PATRÓN** `sigma_h` < `0.0074` → IC=+0.274 (n=339)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0074 (IC base=+0.257)

- **PATRÓN** `drift_60min` |x|≤ `0.422` → IC=+0.266 (n=447)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.422 (IC base=+0.257)
...
- **PATRÓN** `ibs_20min` < `0.2727` → IC=+0.315 (n=447)
...
- **PATRÓN** `dist_vwap_pct` < `0.2367` → IC=+0.266 (n=467)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2367 (IC base=+0.257)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.486` → IC=+0.277 (n=128)
...
- **PATRÓN** `volumen_regimen` > `1.248` → IC=+0.296 (n=170)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.194` → IC=+0.262 (n=923)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.194 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` < `1.2434` → IC=+0.168 (n=1486)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.2434 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` > `0.6942` → IC=+0.175 (n=1329)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 0.6942 (IC base=+0.160)

- **PATRÓN** `volumen_pendiente_norm` > `0.1037` → IC=+0.191 (n=758)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.1037 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` < `2.313` → IC=+0.160 (n=1662)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.313 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` > `1.8755` → IC=+0.149 (n=1259)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.8755 (IC base=+0.160)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.172 (n=1743)

  - _Acción_: Kelly boost +0.86€ cuando `libro_spread` < 0.01 (IC base=+0.160)

- **PATRÓN** `libro_liquidez` > `3704.4761` → IC=+0.202 (n=712)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3704.4761 (IC base=+0.160)

- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.191 (n=918)

  - _Acción_: Kelly boost +0.96€ cuando `ballena_activa_n` < 135.0 (IC base=+0.160)

- **PATRÓN** `sigma_h` < `0.008` → IC=+0.197 (n=1835)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.008 (IC base=+0.181)

- **PATRÓN** `drift_60min` |x|≤ `0.3614` → IC=+0.198 (n=1833)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.3614 (IC base=+0.181)
...
- **PATRÓN** `ibs_20min` < `0.4` → IC=+0.240 (n=1833)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4 (IC base=+0.181)

- **PATRÓN** `dist_vwap_pct` < `0.3373` → IC=+0.173 (n=1608)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.3373 (IC base=+0.181)
...
- **PATRÓN** `volumen_regimen` > `0.8552` → IC=+0.172 (n=1022)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` > 0.8552 (IC base=+0.181)

- **PATRÓN** `volumen_pendiente_norm` > `0.2916` → IC=+0.232 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2916 (IC base=+0.181)

- **PATRÓN** `volumen_spike_ratio` < `1.5762` → IC=+0.175 (n=540)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 1.5762 (IC base=+0.181)

- **PATRÓN** `volumen_spike_ratio` > `2.6561` → IC=+0.198 (n=409)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.6561 (IC base=+0.181)

- **PATRÓN** `ballena_activa_n` < `223.0` → IC=+0.164 (n=792)
...
- **PATRÓN** `sigma_h` < `0.0042` → IC=+0.195 (n=336)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0042 (IC base=+0.148)

- **PATRÓN** `drift_60min` |x|≤ `0.2121` → IC=+0.186 (n=336)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.2121 (IC base=+0.148)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.165 (n=347)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 7.0 (IC base=+0.148)

- **PATRÓN** `ibs_20min` < `0.3782` → IC=+0.188 (n=382)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` < 0.3782 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` < `0.1998` → IC=+0.177 (n=376)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.1998 (IC base=+0.148)
...
- **PATRÓN** `volumen_spike_ratio` < `1.732` → IC=+0.183 (n=184)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 1.732 (IC base=+0.148)
...
- **PATRÓN** `sigma_h` < `0.0077` → IC=+0.239 (n=259)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0077 (IC base=+0.190)

- **PATRÓN** `drift_60min` |x|≤ `0.5056` → IC=+0.204 (n=295)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.5056 (IC base=+0.190)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.215 (n=296)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.190)

- **PATRÓN** `ibs_20min` > `0.4687` → IC=+0.247 (n=294)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4687 (IC base=+0.190)

- **PATRÓN** `dist_vwap_pct` > `1.0628` → IC=+0.244 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0628 (IC base=+0.190)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.788` → IC=+0.339 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.788 (IC base=+0.190)

- **PATRÓN** `volumen_regimen` > `0.6356` → IC=+0.213 (n=294)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6356 (IC base=+0.190)

- **PATRÓN** `volumen_pendiente_norm` > `0.1005` → IC=+0.273 (n=130)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.190 (n=330)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.01 (IC base=+0.190)

- **PATRÓN** `libro_liquidez` > `9646.877` → IC=+0.206 (n=134)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 9646.877 (IC base=+0.190)

- **PATRÓN** `ballena_activa_n` < `111.0` → IC=+0.196 (n=77)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 111.0 (IC base=+0.190)

- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.195 (n=323)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0056 (IC base=+0.143)

- **PATRÓN** `drift_60min` |x|≤ `0.3631` → IC=+0.167 (n=367)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.3631 (IC base=+0.143)
...
- **PATRÓN** `ibs_20min` < `0.4496` → IC=+0.199 (n=367)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4496 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` < `0.5211` → IC=+0.161 (n=423)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.5211 (IC base=+0.143)
...
- **PATRÓN** `volumen_regimen` < `0.849` → IC=+0.148 (n=245)
  ...
  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.849 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` > `0.6146` → IC=+0.158 (n=366)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 0.6146 (IC base=+0.143)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8877` → IC=+0.193 (n=174)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 1.8877 (IC base=+0.143)

### GBM_LATE_15M_MULTIHORIZONTE#SOL#15min
- **FILTRO** `ibs_20min` > `0.4362` → IC=-0.235 (n=119)

  - _Acción_: SKIP cuando `ibs_20min` > 0.4362
  - _Potencial_: sin este filtro IC_bueno=+0.223 (n=359)
...
- **PATRÓN** `ibs_20min` < `0.4362` → IC=+0.223 (n=359)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4362 (IC base=+0.108)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.673` → IC=+0.161 (n=57)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` > 7.673 (IC base=+0.108)

- **PATRÓN** `volumen_regimen` > `0.853` → IC=+0.143 (n=239)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 0.853 (IC base=+0.108)

- **PATRÓN** `volumen_pendiente_norm` > `0.2675` → IC=+0.233 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2675 (IC base=+0.108)

- **PATRÓN** `volumen_spike_ratio` > `2.0661` → IC=+0.192 (n=105)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 2.0661 (IC base=+0.108)

- **PATRÓN** `libro_liquidez` > `1900.4885` → IC=+0.156 (n=239)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5626` → IC=+0.230 (n=427)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.5626 (IC base=+0.219)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.225 (n=209)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.219)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.254 (n=205)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.7231` → IC=+0.230 (n=491)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.7231 (IC base=+0.219)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.892` → IC=+0.265 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.892 (IC base=+0.219)

- **PATRÓN** `volumen_regimen` > `0.7127` → IC=+0.241 (n=381)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7127 (IC base=+0.219)
...
- **PATRÓN** `ballena_activa_n` < `30.0` → IC=+0.189 (n=210)

  - _Acción_: Kelly boost +0.94€ cuando `ballena_activa_n` < 30.0 (IC base=+0.219)
...
- **PATRÓN** `ibs_20min` < `0.275` → IC=+0.127 (n=210)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` < 0.275 (IC base=+0.050)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.608` → IC=+0.217 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.608 (IC base=+0.050)

- **PATRÓN** `volumen_regimen` < `0.7334` → IC=+0.121 (n=138)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` < 0.7334 (IC base=+0.050)
...
- **PATRÓN** `sigma_h` > `0.0059` → IC=+0.167 (n=40)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` > 0.0059 (IC base=+0.154)

- **PATRÓN** `drift_60min` |x|≤ `0.3378` → IC=+0.191 (n=121)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.96€ cuando `drift_60min` |x|≤ 0.3378 (IC base=+0.154)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.159 (n=42)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 17.0 (IC base=+0.154)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.198 (n=61)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` < 6.0 (IC base=+0.154)

- **PATRÓN** `ibs_20min` < `0.6561` → IC=+0.183 (n=121)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` < 0.6561 (IC base=+0.154)

- **PATRÓN** `ibs_20min` > `0.0393` → IC=+0.154 (n=108)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` > 0.0393 (IC base=+0.154)

- **PATRÓN** `dist_vwap_pct` > `0.493` → IC=+0.167 (n=34)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.493 (IC base=+0.154)

- **PATRÓN** `dist_vwap_pct` < `0.2353` → IC=+0.179 (n=107)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.2353 (IC base=+0.154)
...
- **PATRÓN** `volumen_regimen` < `0.9041` → IC=+0.211 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9041 (IC base=+0.154)

- **PATRÓN** `volumen_pendiente_norm` > `0.1721` → IC=+0.338 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1721 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` < `1.5649` → IC=+0.245 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5649 (IC base=+0.154)

- **PATRÓN** `ballena_activa_n` < `138.0` → IC=+0.159 (n=39)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 138.0 (IC base=+0.154)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.153 (n=2464)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 6.0 (IC base=+0.144)

- **PATRÓN** `ibs_20min` > `0.9449` → IC=+0.291 (n=1113)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.466` → IC=+0.233 (n=1483)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.466 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` < `0.7062` → IC=+0.157 (n=762)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.7062 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` > `1.0858` → IC=+0.149 (n=785)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 1.0858 (IC base=+0.144)

- **PATRÓN** `volumen_pendiente_norm` > `0.1658` → IC=+0.183 (n=620)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.1658 (IC base=+0.144)

- **PATRÓN** `volumen_spike_ratio` > `1.9022` → IC=+0.156 (n=1428)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.9022 (IC base=+0.144)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.155 (n=1987)

  - _Acción_: Kelly boost +0.78€ cuando `libro_spread` < 0.01 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `3075.6186` → IC=+0.194 (n=1113)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 3075.6186 (IC base=+0.144)

- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.214 (n=1912)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0076 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.4222` → IC=+0.201 (n=2170)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_spike_ratio` > `2.6339` → IC=+0.202 (n=492)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6339 (IC base=+0.189)

- **PATRÓN** `ballena_activa_n` < `34.0` → IC=+0.147 (n=463)
...
- **PATRÓN** `sigma_h` > `0.0032` → IC=+0.172 (n=260)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0032 (IC base=+0.170)

- **PATRÓN** `drift_60min` |x|≤ `0.3199` → IC=+0.179 (n=388)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.90€ cuando `drift_60min` |x|≤ 0.3199 (IC base=+0.170)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.177 (n=366)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 7.0 (IC base=+0.170)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.177 (n=401)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` < 18.0 (IC base=+0.170)

- **PATRÓN** `ibs_20min` < `0.3504` → IC=+0.208 (n=341)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3504 (IC base=+0.170)

- **PATRÓN** `dist_vwap_pct` < `0.7387` → IC=+0.179 (n=422)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.7387 (IC base=+0.170)
...
- **PATRÓN** `volumen_regimen` < `0.6237` → IC=+0.242 (n=130)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6237 (IC base=+0.170)

- **PATRÓN** `volumen_regimen` > `1.2285` → IC=+0.179 (n=129)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` > 1.2285 (IC base=+0.170)

- **PATRÓN** `volumen_pendiente_norm` > `0.1498` → IC=+0.283 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1498 (IC base=+0.170)

- **PATRÓN** `volumen_spike_ratio` < `1.7416` → IC=+0.216 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7416 (IC base=+0.170)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.172 (n=501)

  - _Acción_: Kelly boost +0.86€ cuando `libro_spread` < 0.01 (IC base=+0.170)

- **PATRÓN** `libro_liquidez` > `12924.6533` → IC=+0.187 (n=129)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 12924.6533 (IC base=+0.170)
...
- **PATRÓN** `sigma_h` < `0.0089` → IC=+0.155 (n=381)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` > `0.3491` → IC=+0.195 (n=381)

  - _Acción_: Kelly boost +0.97€ cuando `ibs_20min` > 0.3491 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` > `0.919` → IC=+0.214 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.919 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.544` → IC=+0.197 (n=186)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 4.544 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` < `0.6268` → IC=+0.182 (n=127)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.6268 (IC base=+0.139)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4166` → IC=+0.207 (n=349)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4166 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `3078.5215` → IC=+0.190 (n=340)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 3078.5215 (IC base=+0.139)

- **PATRÓN** `ballena_activa_n` < `170.0` → IC=+0.186 (n=170)

  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 170.0 (IC base=+0.139)

- **PATRÓN** `sigma_h` < `0.0026` → IC=+0.229 (n=105)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0026 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.2341` → IC=+0.159 (n=206)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.2341 (IC base=+0.141)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.177 (n=215)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.7008` → IC=+0.167 (n=319)
...
- **PATRÓN** `volumen_regimen` > `1.0934` → IC=+0.157 (n=103)
  ...
  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 1.0934 (IC base=+0.141)

- **PATRÓN** `volumen_pendiente_norm` > `0.2303` → IC=+0.338 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2303 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` < `2.1233` → IC=+0.179 (n=222)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 2.1233 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` > `1.4416` → IC=+0.177 (n=252)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 1.4416 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `4274.4143` → IC=+0.152 (n=205)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 4274.4143 (IC base=+0.141)

- **PATRÓN** `ballena_activa_n` < `173.0` → IC=+0.188 (n=155)

  - _Acción_: Kelly boost +0.94€ cuando `ballena_activa_n` < 173.0 (IC base=+0.141)
...
- **PATRÓN** `ibs_20min` > `0.5` → IC=+0.160 (n=445)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.503` → IC=+0.230 (n=172)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.503 (IC base=+0.089)

- **PATRÓN** `libro_liquidez` > `2764.5849` → IC=+0.285 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2764.5849 (IC base=+0.089)
...
- **PATRÓN** `sigma_h` > `0.0257` → IC=+0.226 (n=177)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.9722` → IC=+0.298 (n=241)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.037` → IC=+0.260 (n=290)
...
- **PATRÓN** `volumen_regimen` > `1.2439` → IC=+0.187 (n=177)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` > 1.2439 (IC base=+0.172)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5702` → IC=+0.178 (n=212)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 1.5702 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` > `1.8461` → IC=+0.178 (n=321)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 1.8461 (IC base=+0.172)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.185 (n=592)

  - _Acción_: Kelly boost +0.93€ cuando `libro_spread` < 0.01 (IC base=+0.172)

- **PATRÓN** `libro_liquidez` > `3097.4289` → IC=+0.193 (n=177)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 3097.4289 (IC base=+0.172)

- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.310 (n=193)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0056 (IC base=+0.208)

- **PATRÓN** `drift_60min` |x|≤ `0.4224` → IC=+0.233 (n=507)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.158 (n=658)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.151 (n=107)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 17.0 (IC base=+0.077)

- **PATRÓN** `dist_vwap_pct` > `1.74` → IC=+0.200 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.74 (IC base=+0.077)

- **PATRÓN** `libro_liquidez` > `8331.3362` → IC=+0.128 (n=213)

  - _Acción_: Kelly boost +0.64€ cuando `libro_liquidez` > 8331.3362 (IC base=+0.077)

- **PATRÓN** `sigma_h` < `0.0035` → IC=+0.220 (n=223)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0035 (IC base=+0.149)

- **PATRÓN** `drift_60min` |x|≤ `0.3383` → IC=+0.178 (n=445)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.89€ cuando `drift_60min` |x|≤ 0.3383 (IC base=+0.149)
...
- **PATRÓN** `ibs_20min` < `0.6323` → IC=+0.164 (n=445)

  - _Acción_: Kelly boost +0.82€ cuando `ibs_20min` < 0.6323 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` > `0.6916` → IC=+0.162 (n=137)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.6916 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` < `0.3541` → IC=+0.155 (n=453)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.3541 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.747` → IC=+0.178 (n=234)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 3.747 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` < `1.2758` → IC=+0.169 (n=505)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.2758 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` > `0.7218` → IC=+0.149 (n=451)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 0.7218 (IC base=+0.149)

- **PATRÓN** `volumen_pendiente_norm` < `0.0965` → IC=+0.154 (n=449)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_pendiente_norm` < 0.0965 (IC base=+0.149)

- **PATRÓN** `volumen_pendiente_norm` > `0.0752` → IC=+0.164 (n=245)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.0752 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` < `2.5651` → IC=+0.177 (n=500)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 2.5651 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` > `1.4169` → IC=+0.151 (n=500)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.4169 (IC base=+0.149)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.158 (n=658)

  - _Acción_: Kelly boost +0.79€ cuando `libro_spread` < 0.01 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `7890.3247` → IC=+0.163 (n=505)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2987` → IC=+0.172 (n=260)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.2987 (IC base=+0.149)
...
- **PATRÓN** `ibs_20min` < `0.1372` → IC=+0.218 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1372 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` > `0.6118` → IC=+0.172 (n=59)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.6118 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` < `0.3075` → IC=+0.157 (n=243)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.3075 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.005` → IC=+0.149 (n=132)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 3.005 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.104` → IC=+0.157 (n=260)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` < 6.104 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` < `1.2781` → IC=+0.164 (n=260)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.2781 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` > `0.6677` → IC=+0.162 (n=232)
...
- **PATRÓN** `volumen_spike_ratio` < `2.6509` → IC=+0.176 (n=260)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 2.6509 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `10414.568` → IC=+0.164 (n=260)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 10414.568 (IC base=+0.149)

### GBM_LATE_5M#ETH#5min
- **PATRÓN** `sigma_h` < `0.0077` → IC=+0.161 (n=107)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0077 (IC base=+0.101)

- **PATRÓN** `drift_60min` |x|≤ `0.2098` → IC=+0.139 (n=70)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.69€ cuando `drift_60min` |x|≤ 0.2098 (IC base=+0.101)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.227 (n=53)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.101)

- **PATRÓN** `dist_vwap_pct` > `1.1861` → IC=+0.217 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1861 (IC base=+0.101)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.506` → IC=+0.167 (n=37)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 10.506 (IC base=+0.101)

- **PATRÓN** `volumen_spike_ratio` < `1.4096` → IC=+0.167 (n=52)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.4096 (IC base=+0.101)

- **PATRÓN** `libro_liquidez` > `7679.2554` → IC=+0.146 (n=159)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 7679.2554 (IC base=+0.101)

- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.246 (n=57)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0039 (IC base=+0.172)

- **PATRÓN** `sigma_h` > `0.0064` → IC=+0.181 (n=114)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` > 0.0064 (IC base=+0.172)

- **PATRÓN** `drift_60min` |x|≤ `0.4286` → IC=+0.210 (n=150)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4286 (IC base=+0.172)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.203 (n=116)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 10.0 (IC base=+0.172)

- **PATRÓN** `ibs_20min` < `0.5833` → IC=+0.180 (n=151)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` < 0.5833 (IC base=+0.172)

- **PATRÓN** `ibs_20min` > `0.1096` → IC=+0.205 (n=171)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.1096 (IC base=+0.172)

- **PATRÓN** `dist_vwap_pct` < `0.7043` → IC=+0.183 (n=178)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` < 0.7043 (IC base=+0.172)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.54` → IC=+0.264 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.54 (IC base=+0.172)

- **PATRÓN** `volumen_regimen` < `1.1886` → IC=+0.211 (n=171)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.1886 (IC base=+0.172)

- **PATRÓN** `volumen_pendiente_norm` < `0.0946` → IC=+0.206 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0946 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` < `2.1543` → IC=+0.238 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1543 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` > `1.4319` → IC=+0.186 (n=167)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 1.4319 (IC base=+0.172)

- **PATRÓN** `libro_liquidez` > `7907.9702` → IC=+0.176 (n=171)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 7907.9702 (IC base=+0.172)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.03` → IC=+0.182 (n=20)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 3.03 (IC base=-0.013)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.128 (n=100)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 18.0 (IC base=+0.103)

- **PATRÓN** `py_entrada` < `0.495` → IC=+0.143 (n=40)

  - _Acción_: Kelly boost +0.71€ cuando `py_entrada` < 0.495 (IC base=+0.103)

- **PATRÓN** `libro_liquidez` > `2548.3818` → IC=+0.140 (n=87)

  - _Acción_: Kelly boost +0.70€ cuando `libro_liquidez` > 2548.3818 (IC base=+0.103)
...
- **FILTRO** `libro_liquidez` < `2162.4529` → IC=-0.197 (n=31)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.147 (n=15)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.040 (n=322)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.156 (n=646)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.042 (n=2059)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.229 (n=641)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=1971)

- **FILTRO** `ibs_20min` > `0.279` → IC=-0.202 (n=652)

  - _Acción_: SKIP cuando `ibs_20min` > 0.279
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=1960)
...
- **FILTRO** `py_entrada` > `0.56` → IC=-0.241 (n=106)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=343)
...
- **FILTRO** `ibs_20min` > `0.7891` → IC=-0.205 (n=110)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=332)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.203 (n=119)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=344)

- **FILTRO** `ballena_activa_n` > `10.0` → IC=-0.175 (n=112)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.266 (n=92)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=318)

- **FILTRO** `drift_20min_pct` |x|> `0.3697` → IC=-0.173 (n=102)

  - _Acción_: SKIP cuando `drift_20min_pct` |x|> 0.3697
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=308)

- **FILTRO** `ibs_20min` > `0.2778` → IC=-0.209 (n=101)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2778
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=309)

- **FILTRO** `ballena_activa_n` > `1.0` → IC=-0.148 (n=262)

  - _Acción_: SKIP cuando `ballena_activa_n` > 1.0
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=148)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.48` → IC=-0.172 (n=114)

  - _Acción_: SKIP cuando `py_entrada` < 0.48
  - _Potencial_: sin este filtro IC_bueno=+0.073 (n=342)
...
- **FILTRO** `ibs_7min` < `1.0` → IC=-0.125 (n=30)
  ...
  - _Acción_: SKIP cuando `ibs_7min` < 1.0
  - _Potencial_: sin este filtro IC_bueno=+0.010 (n=312)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.138 (n=1997)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=4564)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.286 (n=1586)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=4975)

- **FILTRO** `ibs_7min` < `0.737` → IC=-0.234 (n=1640)

  - _Acción_: SKIP cuando `ibs_7min` < 0.737
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4921)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.179 (n=2205)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=4356)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.227 (n=1898)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=6126)

- **FILTRO** `ibs_7min` > `0.72` → IC=-0.167 (n=2005)

  - _Acción_: SKIP cuando `ibs_7min` > 0.72
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=6019)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.154 (n=218)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=686)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.323 (n=218)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=686)

- **FILTRO** `ibs_7min` < `0.9841` → IC=-0.194 (n=596)

  - _Acción_: SKIP cuando `ibs_7min` < 0.9841
  - _Potencial_: sin este filtro IC_bueno=+0.074 (n=308)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.252 (n=220)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=684)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.242 (n=320)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=1015)

- **FILTRO** `drift_7min_pct` |x|> `0.0747` → IC=-0.123 (n=893)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.0747
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=442)

- **FILTRO** `ibs_7min` > `0.8333` → IC=-0.202 (n=333)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8333
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=1002)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.161 (n=290)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=1046)

- **FILTRO** `py_entrada` < `0.39` → IC=-0.243 (n=329)

  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=1007)

- **FILTRO** `ibs_7min` < `0.8125` → IC=-0.165 (n=332)

  - _Acción_: SKIP cuando `ibs_7min` < 0.8125
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=1004)

- **FILTRO** `ballena_activa_n` > `145.0` → IC=-0.165 (n=332)

  - _Acción_: SKIP cuando `ballena_activa_n` > 145.0
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=1004)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.218 (n=335)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=1038)

- **FILTRO** `ballena_activa_n` > `114.0` → IC=-0.170 (n=343)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.262 (n=309)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=1094)

- **FILTRO** `ibs_7min` > `0.8222` → IC=-0.182 (n=350)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8222
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=1053)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.164 (n=257)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.076 (n=863)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.271 (n=277)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=843)

- **FILTRO** `ibs_7min` < `0.7716` → IC=-0.188 (n=280)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7716
...
- **FILTRO** `ballena_activa_n` > `43.0` → IC=-0.225 (n=278)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=842)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.286 (n=265)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=843)

- **FILTRO** `ibs_7min` > `0.1778` → IC=-0.156 (n=376)

  - _Acción_: SKIP cuando `ibs_7min` > 0.1778
  - _Potencial_: sin este filtro IC_bueno=-0.075 (n=732)

- **FILTRO** `ballena_activa_n` > `40.0` → IC=-0.219 (n=272)

  - _Acción_: SKIP cuando `ballena_activa_n` > 40.0
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=836)
...
- **FILTRO** `py_entrada` < `0.38` → IC=-0.292 (n=262)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=809)

- **FILTRO** `ibs_7min` < `0.75` → IC=-0.241 (n=257)

  - _Acción_: SKIP cuando `ibs_7min` < 0.75
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=814)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.249 (n=257)

  - _Acción_: SKIP cuando `ballena_activa_n` > 16.0
  - _Potencial_: sin este filtro IC_bueno=+0.006 (n=814)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.281 (n=276)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=1078)

- **FILTRO** `ibs_7min` > `0.8136` → IC=-0.141 (n=338)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8136
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=1016)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.129 (n=453)

  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=901)
...
- **PATRÓN** `delta_ratio` |x|> `0.3997` → IC=+0.145 (n=249)
...
- **PATRÓN** `total_vol_5m` < `447.889` → IC=+0.192 (n=76)
  ...
  - _Acción_: Kelly boost +0.96€ cuando `total_vol_5m` < 447.889 (IC base=+0.117)

- **PATRÓN** `libro_liquidez` > `3566.692` → IC=+0.156 (n=59)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 3566.692 (IC base=+0.117)
...
- **FILTRO** `hora_utc` > `8.0` → IC=-0.220 (n=23)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 8.0
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=1087)
...
- **FILTRO** `drift_15min` |x|> `0.589` → IC=-0.175 (n=81)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=245)
...
- **PATRÓN** `ibs_15` < `0.1304` → IC=+0.177 (n=97)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.88€ cuando `ibs_15` < 0.1304 (IC base=+0.038)
...
- **FILTRO** `sigma_ewma_delta_pct` > `16.823` → IC=-0.183 (n=307)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 16.823
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=2089)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.887` → IC=-0.146 (n=190)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.887
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=644)
...
- **FILTRO** `drift_60min` |x|> `0.7952` → IC=-0.149 (n=192)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.7952
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=580)

- **FILTRO** `drift_15min` |x|> `1.5354` → IC=-0.149 (n=192)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
...
- **FILTRO** `sigma_ewma_delta_pct` > `15.454` → IC=-0.184 (n=96)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=676)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0569` → IC=+0.288 (n=201)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0569 (IC base=+0.270)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.4165` → IC=+0.318 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.4165 (IC base=+0.270)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.310 (n=203)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.270)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.270 (n=202)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.270)

- **PATRÓN** `ibs_15` > `0.83` → IC=+0.303 (n=201)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.83 (IC base=+0.270)

- **PATRÓN** `dist_vwap_pct` > `0.2797` → IC=+0.353 (n=100)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2797 (IC base=+0.270)

- **PATRÓN** `sigma_ewma_delta_pct` > `23.866` → IC=+0.271 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 23.866 (IC base=+0.270)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.269 (n=245)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.270)

- **PATRÓN** `libro_liquidez` > `10408.2229` → IC=+0.298 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 10408.2229 (IC base=+0.270)
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1304 sube el IC de +0.038 a +0.177 en UPDOWN_GBM#XRP#15min (n=97). Ya aplicado como kelly_boost=+0.88€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.4444 sube el IC de -0.052 a +0.151 en UPDOWN_GBM_15M_TARDIO (n=276). Ya aplicado como kelly_boost=+0.76€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.83 sube el IC de +0.270 a +0.303 en UPDOWN_GBM_IBS_ALTO (n=201). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 6162 | -0.096 | -752.01€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 648 | -0.080 | -102.89€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 5514 | -0.098 | -649.12€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 950 | +0.007 | -107.02€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 950 | +0.007 | -107.02€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#XRP | 1440 | -0.175 | -260.68€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1440 | -0.175 | -260.68€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 27929 | +0.114 | -1847.10€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 5338 | +0.187 | -181.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 107 | -0.105 | -50.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 19435 | +0.094 | -1591.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3049 | +0.123 | -23.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 3265 | +0.062 | -571.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 19 | -0.068 | +0.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 3241 | +0.064 | -565.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 5839 | +0.135 | -116.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1496 | +0.196 | -86.18€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 3236 | +0.111 | -70.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1065 | +0.134 | +62.73€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 3270 | +0.076 | -452.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 19 | +0.023 | -2.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 3250 | +0.076 | -447.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 6269 | +0.130 | -56.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1941 | +0.170 | -2.55€ | 0 | 8 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 3236 | +0.115 | -31.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1080 | +0.105 | -14.00€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 6022 | +0.131 | -407.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1846 | +0.203 | -89.83€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 46 | +0.000 | -9.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 3226 | +0.091 | -235.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 904 | +0.131 | -72.53€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 3264 | +0.104 | -242.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 17 | -0.022 | -0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 3246 | +0.105 | -240.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 5319 | +0.177 | -385.67€ | 3 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 5319 | +0.177 | -385.67€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 1337 | +0.173 | -131.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 1337 | +0.173 | -131.59€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 1329 | +0.162 | -155.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 1329 | +0.162 | -155.06€ | 0 | 4 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 1278 | +0.192 | -85.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 1278 | +0.192 | -85.17€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 13950 | +0.192 | -1219.88€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 13950 | +0.192 | -1219.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 2576 | +0.125 | -477.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 