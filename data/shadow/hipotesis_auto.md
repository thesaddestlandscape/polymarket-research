# Hipótesis automáticas — 2026-07-20 03:38 UTC
_Generado por shadow_postmortem.py sobre 23748 resoluciones (PNL=+6072.82€)_
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.222 (n=282)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.211)

- **PATRÓN** `py_entrada` > `0.605` → IC=+0.259 (n=318)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.254 (n=120)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.1226` → IC=+0.129 (n=680)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.1226 (IC base=+0.118)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.041` → IC=+0.243 (n=189)
...
- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.123 (n=420)
...
- **PATRÓN** `dist_vwap_pct` > `0.4801` → IC=+0.198 (n=41)
  ...
  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.4801 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` < `0.1071` → IC=+0.146 (n=145)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.1071 (IC base=+0.102)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.396` → IC=+0.227 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.396 (IC base=+0.104)
...
- **PATRÓN** `sigma_h` < `0.0267` → IC=+0.158 (n=583)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.157 (n=520)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.1078` → IC=+0.162 (n=229)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.1078 (IC base=+0.156)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.676` → IC=+0.290 (n=60)
...
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.207 (n=781)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.334` → IC=+0.250 (n=498)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.334 (IC base=+0.165)
...
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.134 (n=184)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.0041 (IC base=+0.131)

- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.153 (n=246)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.5034` → IC=+0.207 (n=39)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5034 (IC base=+0.131)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.233` → IC=+0.208 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.233 (IC base=+0.131)
...
- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.253 (n=99)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.146 (n=309)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 18.0 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `0.1929` → IC=+0.173 (n=108)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.1929 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.722` → IC=+0.284 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.722 (IC base=+0.144)
...
- **PATRÓN** `sigma_h` < `0.0165` → IC=+0.260 (n=277)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0165 (IC base=+0.257)

- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.273 (n=276)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.259 (n=284)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` < `0.0052` → IC=+0.138 (n=360)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0052 (IC base=+0.131)
...
- **PATRÓN** `dist_vwap_pct` > `0.4717` → IC=+0.180 (n=220)
  ...
  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.4717 (IC base=+0.131)

- **PATRÓN** `dist_vwap_pct` < `0.1095` → IC=+0.157 (n=581)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.1095 (IC base=+0.131)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.147` → IC=+0.293 (n=153)
...
- **PATRÓN** `sigma_h` < `0.0255` → IC=+0.208 (n=341)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0255 (IC base=+0.208)

- **PATRÓN** `sigma_h` > `0.0092` → IC=+0.217 (n=341)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.223 (n=128)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.208)

- **PATRÓN** `dist_vwap_pct` > `0.5454` → IC=+0.268 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5454 (IC base=+0.208)

- **PATRÓN** `dist_vwap_pct` < `0.0983` → IC=+0.214 (n=197)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0983 (IC base=+0.208)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.809` → IC=+0.306 (n=60)
...
- **PATRÓN** `ibs_15` < `0.0308` → IC=+0.134 (n=189)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.67€ cuando `ibs_15` < 0.0308 (IC base=+0.052)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `8.229` → IC=+0.120 (n=106)

  - _Acción_: Kelly boost +0.60€ cuando `sigma_ewma_delta_pct` < 8.229 (IC base=+0.046)
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 388 | +0.144 | +17.92€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 388 | +0.144 | +17.92€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 153 | +0.171 | +9.57€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 153 | +0.171 | +9.57€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 154 | +0.128 | -0.42€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 154 | +0.128 | -0.42€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 81 | +0.115 | +8.77€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 81 | +0.115 | +8.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 4064 | +0.186 | +57.55€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 2648 | +0.218 | +92.11€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 1181 | +0.197 | +47.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 882 | +0.210 | +1.96€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 1446 | +0.180 | +10.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 879 | +0.221 | +43.87€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 1419 | +0.183 | -0.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 873 | +0.223 | +49.40€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 169 | +0.319 | +9.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 169 | +0.319 | +9.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 169 | +0.319 | +9.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 169 | +0.319 | +9.80€ | 0 | 0 |
| ✅ GBM_LATE_15M | 6025 | +0.108 | +2134.41€ | 0 | 10 |
| ✅ GBM_LATE_15M#15min |