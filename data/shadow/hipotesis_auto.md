# Hipótesis automáticas — 2026-09-17 19:24 UTC
_Generado por shadow_postmortem.py sobre 485728 resoluciones (PNL=+52449.53€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `144.1` → IC=-0.249 (n=5983)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 144.1
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=17949)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `141.1` → IC=-0.265 (n=826)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 141.1
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=2479)
...
- **FILTRO** `restante_s_al_confirmar` < `131.87` → IC=-0.293 (n=724)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 131.87
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=2175)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `158.73` → IC=-0.243 (n=1421)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.73
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=4263)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `117.43` → IC=-0.367 (n=1160)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 117.43
  - _Potencial_: sin este filtro IC_bueno=-0.108 (n=3480)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.197 (n=12067)

  - _Acción_: Kelly boost +0.98€ cuando `py_entrada` > 0.69 (IC base=+0.097)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.151 (n=3008)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.01 (IC base=+0.097)

- **PATRÓN** `libro_liquidez` > `5334.3634` → IC=+0.170 (n=1919)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 5334.3634 (IC base=+0.097)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.146 (n=9382)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `15261.6142` → IC=+0.223 (n=456)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 15261.6142 (IC base=+0.202)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.209 (n=1301)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.205 (n=1832)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.204)

- **PATRÓN** `libro_liquidez` > `13363.4017` → IC=+0.226 (n=643)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13363.4017 (IC base=+0.204)
...
- **PATRÓN** `py_entrada` < `0.435` → IC=+0.154 (n=681)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.146 (n=2418)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.179 (n=985)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.03` → IC=+0.178 (n=1140)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.200 (n=9532)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.222 (n=3342)
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.176 (n=2353)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.302 (n=220)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.183 (n=2203)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.186 (n=1981)

  - _Acción_: Kelly boost +0.93€ cuando `py_entrada` > 0.71 (IC base=+0.178)

### FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.244 (n=2080)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.322 (n=696)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.76 (IC base=+0.235)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.199 (n=2253)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.195 (n=1623)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.187 (n=5227)

  - _Acción_: Kelly boost +0.93€ cuando `py_entrada` > 0.71 (IC base=+0.171)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.226 (n=5043)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `3.28` → IC=+0.152 (n=3949)

  - _Acción_: Kelly boost +0.76€ cuando `lag_apertura_s` < 3.28 (IC base=+0.124)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.207 (n=2175)
...
- **PATRÓN** `restante_min` > `4.93` → IC=+0.146 (n=2123)
...
- **PATRÓN** `lag_apertura_s` < `2.47` → IC=+0.150 (n=1990)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.4986` → IC=+0.155 (n=5783)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` > 0.4986 (IC base=+0.096)

- **PATRÓN** `dist_vwap_pct` < `0.1442` → IC=+0.246 (n=1289)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1442 (IC base=+0.096)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.846` → IC=+0.168 (n=2507)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` > 5.846 (IC base=+0.096)

- **PATRÓN** `volumen_regimen` < `1.2223` → IC=+0.243 (n=1663)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2223 (IC base=+0.096)

- **PATRÓN** `volumen_regimen` > `1.0676` → IC=+0.241 (n=754)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0676 (IC base=+0.096)

- **PATRÓN** `volumen_pendiente_norm` > `0.3071` → IC=+0.203 (n=621)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3071 (IC base=+0.096)

- **PATRÓN** `volumen_spike_ratio` > `1.9163` → IC=+0.199 (n=2864)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.9163 (IC base=+0.096)

- **PATRÓN** `ibs_20min` < `0.57` → IC=+0.133 (n=7925)

  - _Acción_: Kelly boost +0.67€ cuando `ibs_20min` < 0.57 (IC base=+0.062)

- **PATRÓN** `dist_vwap_pct` > `0.5579` → IC=+0.185 (n=481)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.5579 (IC base=+0.062)

- **PATRÓN** `dist_vwap_pct` < `0.337` → IC=+0.171 (n=2776)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.337 (IC base=+0.062)

- **PATRÓN** `volumen_regimen` < `0.7012` → IC=+0.175 (n=1151)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` < 0.7012 (IC base=+0.062)

- **PATRÓN** `volumen_regimen` > `0.8717` → IC=+0.178 (n=1744)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` > 0.8717 (IC base=+0.062)

- **PATRÓN** `volumen_pendiente_norm` > `0.2439` → IC=+0.226 (n=881)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2439 (IC base=+0.062)

- **PATRÓN** `volumen_spike_ratio` > `1.5869` → IC=+0.201 (n=3886)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5869 (IC base=+0.062)

- **PATRÓN** `ballena_activa_n` < `153.0` → IC=+0.212 (n=4123)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 153.0 (IC base=+0.062)
...
- **PATRÓN** `sigma_h` > `0.0079` → IC=+0.169 (n=488)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0079 (IC base=+0.161)

- **PATRÓN** `drift_60min` |x|≤ `0.3249` → IC=+0.162 (n=1460)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.3249 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.162 (n=713)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 15.0 (IC base=+0.161)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.185 (n=547)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 6.0 (IC base=+0.161)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.266 (n=561)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.999` → IC=+0.274 (n=640)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4295` → IC=+0.162 (n=1347)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.4295 (IC base=+0.161)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.180 (n=1377)

  - _Acción_: Kelly boost +0.90€ cuando `libro_spread` < 0.04 (IC base=+0.161)

- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.251 (n=969)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2799` → IC=+0.268 (n=140)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2799 (IC base=+0.235)

- **PATRÓN** `volumen_spike_ratio` > `2.6473` → IC=+0.259 (n=322)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6473 (IC base=+0.235)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.237 (n=1128)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.235)

- **PATRÓN** `libro_liquidez` > `1746.14` → IC=+0.255 (n=721)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1746.14 (IC base=+0.235)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `sigma_h` < `0.0052` → IC=+0.223 (n=973)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0052 (IC base=+0.211)

- **PATRÓN** `drift_60min` |x|≤ `0.1152` → IC=+0.240 (n=487)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1152 (IC base=+0.211)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.226 (n=1161)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1866` → IC=+0.215 (n=559)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1866 (IC base=+0.211)

- **PATRÓN** `dist_vwap_pct` < `0.1291` → IC=+0.213 (n=853)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1291 (IC base=+0.211)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.811` → IC=+0.231 (n=359)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.811 (IC base=+0.211)

- **PATRÓN** `volumen_regimen` < `1.258` → IC=+0.220 (n=1106)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.258 (IC base=+0.211)

- **PATRÓN** `volumen_regimen` > `0.8755` → IC=+0.212 (n=737)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8755 (IC base=+0.211)

- **PATRÓN** `volumen_pendiente_norm` > `0.2277` → IC=+0.213 (n=214)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2277 (IC base=+0.211)

- **PATRÓN** `volumen_spike_ratio` < `1.4014` → IC=+0.221 (n=360)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4014 (IC base=+0.211)

- **PATRÓN** `volumen_spike_ratio` > `2.368` → IC=+0.224 (n=360)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.368 (IC base=+0.211)

- **PATRÓN** `libro_liquidez` > `11003.4334` → IC=+0.221 (n=1106)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11003.4334 (IC base=+0.211)

- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.158 (n=1041)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0049 (IC base=+0.140)

- **PATRÓN** `drift_60min` |x|≤ `0.0755` → IC=+0.160 (n=395)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.0755 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.165 (n=395)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 18.0 (IC base=+0.140)

- **PATRÓN** `ibs_20min` < `0.6744` → IC=+0.176 (n=1183)

  - _Acción_: Kelly boost +0.88€ cuando `ibs_20min` < 0.6744 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` < `0.1248` → IC=+0.152 (n=1080)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.1248 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.577` → IC=+0.165 (n=386)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 6.577 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `1.2116` → IC=+0.147 (n=1183)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 1.2116 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` > `0.6201` → IC=+0.141 (n=1183)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 0.6201 (IC base=+0.140)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4323` → IC=+0.153 (n=1074)
...
- **PATRÓN** `libro_liquidez` > `13570.903` → IC=+0.154 (n=789)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 13570.903 (IC base=+0.140)

- **PATRÓN** `ballena_activa_n` < `214.0` → IC=+0.163 (n=330)

  - _Acción_: Kelly boost +0.81€ cuando `ballena_activa_n` < 214.0 (IC base=+0.140)

### GBM_LATE_15M#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0058` → IC=+0.189 (n=1429)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0058 (IC base=+0.177)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.181 (n=704)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.255 (n=556)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.177)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.505` → IC=+0.229 (n=408)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.505 (IC base=+0.177)

- **PATRÓN** `volumen_pendiente_norm` < `0.1059` → IC=+0.183 (n=1215)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` < 0.1059 (IC base=+0.177)

- **PATRÓN** `volumen_pendiente_norm` > `0.3748` → IC=+0.184 (n=185)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.3748 (IC base=+0.177)

- **PATRÓN** `volumen_spike_ratio` > `2.9982` → IC=+0.200 (n=608)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9982 (IC base=+0.177)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.189 (n=1647)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.04 (IC base=+0.177)

- **PATRÓN** `sigma_h` < `0.0105` → IC=+0.227 (n=1226)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0105 (IC base=+0.218)

- **PATRÓN** `sigma_h` > `0.0063` → IC=+0.217 (n=1095)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0063 (IC base=+0.218)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.252 (n=462)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.218)

- **PATRÓN** `ibs_20min` < `0.3808` → IC=+0.234 (n=1079)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3808 (IC base=+0.218)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.323` → IC=+0.219 (n=1344)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3649` → IC=+0.269 (n=180)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3649 (IC base=+0.218)

- **PATRÓN** `volumen_spike_ratio` < `1.8483` → IC=+0.208 (n=484)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8483 (IC base=+0.218)

- **PATRÓN** `volumen_spike_ratio` > `2.3042` → IC=+0.225 (n=732)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3042 (IC base=+0.218)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.232 (n=684)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.218)

- **PATRÓN** `libro_liquidez` > `1896.6084` → IC=+0.237 (n=409)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1896.6084 (IC base=+0.218)

- **PATRÓN** `ballena_activa_n` < `27.0` → IC=+0.226 (n=701)
...
- **PATRÓN** `dist_vwap_pct` > `0.17` → IC=+0.191 (n=218)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.17 (IC base=+0.008)

- **PATRÓN** `volumen_regimen` < `1.0374` → IC=+0.157 (n=534)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.0374 (IC base=+0.008)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5104` → IC=+0.191 (n=497)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 1.5104 (IC base=+0.008)
...
- **FILTRO** `ibs_20min` > `0.2692` → IC=-0.127 (n=1814)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2692
  - _Potencial_: sin este filtro IC_bueno=+0.122 (n=895)

- **FILTRO** `sigma_ewma_delta_pct` > `8.635` → IC=-0.207 (n=298)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.635
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=2411)
...
- **PATRÓN** `ibs_20min` < `0.2692` → IC=+0.122 (n=895)
...
- **FILTRO** `ibs_20min` > `0.775` → IC=-0.198 (n=684)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.033 (n=2058)
...
- **PATRÓN** `dist_vwap_pct` < `0.163` → IC=+0.234 (n=2123)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.163 (IC base=+0.070)

- **PATRÓN** `volumen_regimen` < `0.7148` → IC=+0.236 (n=1005)
...
- **FILTRO** `ibs_20min` < `0.2391` → IC=-0.145 (n=553)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.101 (n=1659)

- **FILTRO** `sigma_ewma_delta_pct` > `4.316` → IC=-0.168 (n=416)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.316
  - _Potencial_: sin este filtro IC_bueno=+0.025 (n=1382)

- **PATRÓN** `ibs_20min` > `0.871` → IC=+0.257 (n=553)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.871 (IC base=+0.040)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.283` → IC=+0.152 (n=751)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4409` → IC=+0.211 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4409 (IC base=+0.040)

- **PATRÓN** `volumen_spike_ratio` > `2.5899` → IC=+0.190 (n=195)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` > 2.5899 (IC base=+0.040)
...
- **PATRÓN** `dist_vwap_pct` < `0.1506` → IC=+0.213 (n=454)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1506 (IC base=+0.003)

- **PATRÓN** `volumen_regimen` > `1.0467` → IC=+0.216 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0467 (IC base=+0.003)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8143` → IC=+0.213 (n=256)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.295 (n=660)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.239)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.52` → IC=+0.276 (n=405)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.52 (IC base=+0.239)

- **PATRÓN** `volumen_pendiente_norm` < `0.1395` → IC=+0.253 (n=1126)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1395 (IC base=+0.239)

- **PATRÓN** `volumen_spike_ratio` < `1.8762` → IC=+0.241 (n=528)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8762 (IC base=+0.239)

- **PATRÓN** `volumen_spike_ratio` > `3.0626` → IC=+0.252 (n=543)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.0626 (IC base=+0.239)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.255 (n=1466)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.239)

- **PATRÓN** `libro_liquidez` > `1910.6984` → IC=+0.242 (n=428)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1910.6984 (IC base=+0.239)

- **PATRÓN** `sigma_h` > `0.0055` → IC=+0.299 (n=1025)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0055 (IC base=+0.281)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.317 (n=348)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.281)

- **PATRÓN** `ibs_20min` < `0.3333` → IC=+0.287 (n=1025)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.575` → IC=+0.281 (n=1105)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.291 (n=567)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.281)

- **PATRÓN** `libro_liquidez` > `1886.4395` → IC=+0.305 (n=341)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1886.4395 (IC base=+0.281)

- **PATRÓN** `ballena_activa_n` < `21.0` → IC=+0.279 (n=405)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 21.0 (IC base=+0.281)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **FILTRO** `ibs_20min` < `0.2479` → IC=-0.212 (n=373)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2479
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=1125)

- **FILTRO** `ibs_20min` > `0.8018` → IC=-0.185 (n=484)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8018
  - _Potencial_: sin este filtro IC_bueno=+0.045 (n=1456)

- **PATRÓN** `ibs_20min` > `0.8048` → IC=+0.152 (n=510)

  - _Acción_: Kelly boost +0.76€ cuando `ibs_20min` > 0.8048 (IC base=-0.011)

- **PATRÓN** `dist_vwap_pct` > `0.4461` → IC=+0.219 (n=144)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4461 (IC base=-0.011)

- **PATRÓN** `dist_vwap_pct` < `0.1908` → IC=+0.190 (n=275)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.1908 (IC base=-0.011)

- **PATRÓN** `volumen_regimen` < `0.9565` → IC=+0.216 (n=315)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9565 (IC base=-0.011)

- **PATRÓN** `volumen_regimen` > `0.6225` → IC=+0.193 (n=320)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` > 0.6225 (IC base=-0.011)
...
- **PATRÓN** `volumen_regimen` < `1.1418` → IC=+0.185 (n=265)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` < 1.1418 (IC base=-0.012)

- **PATRÓN** `volumen_regimen` > `0.7255` → IC=+0.189 (n=236)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_regimen` > 0.7255 (IC base=-0.012)
...
- **FILTRO** `ibs_20min` < `0.6774` → IC=-0.212 (n=873)

  - _Acción_: SKIP cuando `ibs_20min` < 0.6774
  - _Potencial_: sin este filtro IC_bueno=+0.261 (n=874)
...
- **PATRÓN** `ibs_20min` > `0.6774` → IC=+0.261 (n=874)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6774 (IC base=+0.025)

- **PATRÓN** `dist_vwap_pct` > `0.1845` → IC=+0.309 (n=375)
...
- **PATRÓN** `volumen_regimen` < `0.8648` → IC=+0.290 (n=407)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8648 (IC base=+0.025)

- **PATRÓN** `volumen_regimen` > `0.7203` → IC=+0.277 (n=545)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7203 (IC base=+0.025)

- **PATRÓN** `volumen_pendiente_norm` < `0.1055` → IC=+0.277 (n=563)
...
- **PATRÓN** `ballena_activa_n` < `55.0` → IC=+0.318 (n=504)
...
- **PATRÓN** `sigma_h` > `0.0157` → IC=+0.323 (n=719)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.383` → IC=+0.300 (n=579)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.383 (IC base=+0.273)

- **PATRÓN** `volumen_regimen` > `0.6862` → IC=+0.289 (n=965)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.277 (n=1123)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.273)

- **PATRÓN** `libro_liquidez` > `2431.1711` → IC=+0.283 (n=964)
...
- **PATRÓN** `sigma_h` < `0.0048` → IC=+0.175 (n=2133)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.3377` → IC=+0.173 (n=5609)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.3377 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.175 (n=6654)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 5.0 (IC base=+0.165)

- **PATRÓN** `ibs_20min` > `0.5808` → IC=+0.213 (n=6372)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5808 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` > `0.9321` → IC=+0.216 (n=948)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9321 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.217` → IC=+0.247 (n=1313)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.217 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` < `1.2135` → IC=+0.161 (n=4232)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2135 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` > `0.623` → IC=+0.157 (n=4232)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5654` → IC=+0.172 (n=2668)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.5654 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `2.6619` → IC=+0.169 (n=2021)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.6619 (IC base=+0.165)

- **PATRÓN** `libro_liquidez` > `3832.0952` → IC=+0.168 (n=2124)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 3832.0952 (IC base=+0.165)

- **PATRÓN** `ballena_activa_n` < `121.0` → IC=+0.179 (n=5291)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 121.0 (IC base=+0.165)

- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.188 (n=4126)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0064 (IC base=+0.171)

- **PATRÓN** `drift_60min` |x|≤ `0.0792` → IC=+0.205 (n=2064)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0792 (IC base=+0.171)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.208 (n=2065)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.171)

- **PATRÓN** `ibs_20min` < `0.4698` → IC=+0.229 (n=6185)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4698 (IC base=+0.171)

- **PATRÓN** `dist_vwap_pct` < `0.2208` → IC=+0.162 (n=4605)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2208 (IC base=+0.171)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.18` → IC=+0.195 (n=1063)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 10.18 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` < `1.1813` → IC=+0.156 (n=4520)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.1813 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` > `0.625` → IC=+0.150 (n=4520)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 0.625 (IC base=+0.171)

- **PATRÓN** `volumen_pendiente_norm` > `0.2924` → IC=+0.230 (n=884)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2924 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` < `1.576` → IC=+0.174 (n=2432)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.576 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` > `2.2831` → IC=+0.175 (n=2505)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.2831 (IC base=+0.171)

- **PATRÓN** `ballena_activa_n` < `122.0` → IC=+0.173 (n=5146)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3224` → IC=+0.201 (n=1083)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.302 (n=524)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.032` → IC=+0.307 (n=495)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5417` → IC=+0.172 (n=987)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 2.5417 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` > `1.4242` → IC=+0.178 (n=988)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 1.4242 (IC base=+0.182)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.201 (n=1027)
...
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.167 (n=1059)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0058 (IC base=+0.146)

- **PATRÓN** `drift_60min` |x|≤ `0.0593` → IC=+0.202 (n=354)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0593 (IC base=+0.146)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.163 (n=982)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 7.0 (IC base=+0.146)

- **PATRÓN** `ibs_20min` < `0.5429` → IC=+0.191 (n=1059)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` < 0.5429 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` < `0.1327` → IC=+0.166 (n=1079)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.1327 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.937` → IC=+0.212 (n=210)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.937 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` < `1.2116` → IC=+0.162 (n=1059)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 1.2116 (IC base=+0.146)

- **PATRÓN** `volumen_pendiente_norm` > `0.158` → IC=+0.176 (n=325)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.158 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` < `2.4315` → IC=+0.157 (n=949)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.4315 (IC base=+0.146)

- **PATRÓN** `ballena_activa_n` < `223.0` → IC=+0.166 (n=288)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 223.0 (IC base=+0.146)
...
- **PATRÓN** `sigma_h` < `0.0105` → IC=+0.241 (n=886)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0105 (IC base=+0.224)

- **PATRÓN** `sigma_h` > `0.0057` → IC=+0.226 (n=887)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0057 (IC base=+0.224)

- **PATRÓN** `drift_60min` |x|≤ `0.0891` → IC=+0.245 (n=296)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0891 (IC base=+0.224)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.272 (n=314)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.224)

- **PATRÓN** `ibs_20min` < `0.3478` → IC=+0.255 (n=886)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8483` → IC=+0.218 (n=356)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8483 (IC base=+0.224)
...
- **PATRÓN** `ballena_activa_n` < `18.0` → IC=+0.215 (n=342)
...
- **PATRÓN** `sigma_h` < `0.0071` → IC=+0.169 (n=1078)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0071 (IC base=+0.133)

- **PATRÓN** `drift_60min` |x|≤ `0.381` → IC=+0.147 (n=1078)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.381 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.184 (n=422)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 17.0 (IC base=+0.133)

- **PATRÓN** `ibs_20min` < `0.6046` → IC=+0.184 (n=1078)

  - _Acción_: Kelly boost +0.92€ cuando `ibs_20min` < 0.6046 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `0.1489` → IC=+0.148 (n=1078)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.1489 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.872` → IC=+0.191 (n=208)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 11.872 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` < `0.8541` → IC=+0.145 (n=719)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 0.8541 (IC base=+0.133)

- **PATRÓN** `volumen_regimen` > `0.609` → IC=+0.133 (n=1077)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_regimen` > 0.609 (IC base=+0.133)

- **PATRÓN** `volumen_pendiente_norm` > `0.2894` → IC=+0.205 (n=154)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2894 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` < `1.8021` → IC=+0.142 (n=641)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.8021 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` > `2.4806` → IC=+0.144 (n=321)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 2.4806 (IC base=+0.133)

- **PATRÓN** `libro_liquidez` > `10084.8377` → IC=+0.162 (n=489)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 10084.8377 (IC base=+0.133)

- **PATRÓN** `ballena_activa_n` < `175.0` → IC=+0.131 (n=892)

  - _Acción_: Kelly boost +0.65€ cuando `ballena_activa_n` < 175.0 (IC base=+0.133)

### GBM_LATE_15M_MULTIHORIZONTE#SOL#15min
- **PATRÓN** `sigma_h` > `0.0079` → IC=+0.145 (n=786)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` > 0.0079 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.128 (n=1211)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 5.0 (IC base=+0.110)

- **PATRÓN** `ibs_20min` > `0.5` → IC=+0.195 (n=1191)
...
- **PATRÓN** `volumen_regimen` < `1.2208` → IC=+0.121 (n=1179)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_regimen` < 1.2208 (IC base=+0.110)

- **PATRÓN** `volumen_spike_ratio` < `1.4467` → IC=+0.134 (n=378)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 1.4467 (IC base=+0.110)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.120 (n=1218)

  - _Acción_: Kelly boost +0.60€ cuando `libro_spread` < 0.02 (IC base=+0.110)

- **PATRÓN** `libro_liquidez` > `2898.8632` → IC=+0.187 (n=535)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 2898.8632 (IC base=+0.110)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.129 (n=875)

  - _Acción_: Kelly boost +0.65€ cuando `ballena_activa_n` < 49.0 (IC base=+0.110)

- **PATRÓN** `sigma_h` < `0.006` → IC=+0.148 (n=526)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.006 (IC base=+0.112)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.172 (n=547)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 15.0 (IC base=+0.112)

- **PATRÓN** `ibs_20min` < `0.5581` → IC=+0.204 (n=1191)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5581 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `0.6845` → IC=+0.135 (n=220)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 0.6845 (IC base=+0.112)
...
- **PATRÓN** `volumen_regimen` < `1.1914` → IC=+0.120 (n=1191)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_regimen` < 1.1914 (IC base=+0.112)

- **PATRÓN** `volumen_pendiente_norm` > `0.2797` → IC=+0.181 (n=142)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.2797 (IC base=+0.112)

- **PATRÓN** `volumen_spike_ratio` < `1.4638` → IC=+0.138 (n=349)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` < 1.4638 (IC base=+0.112)

- **PATRÓN** `volumen_spike_ratio` > `2.1784` → IC=+0.128 (n=474)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 2.1784 (IC base=+0.112)

- **PATRÓN** `libro_liquidez` > `3085.1851` → IC=+0.164 (n=397)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 3085.1851 (IC base=+0.112)
...
- **PATRÓN** `sigma_h` > `0.0223` → IC=+0.216 (n=543)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.221 (n=589)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_regimen` > `0.6271` → IC=+0.217 (n=1194)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4611` → IC=+0.196 (n=1056)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.4611 (IC base=+0.204)

- **PATRÓN** `libro_liquidez` > `2549.3704` → IC=+0.215 (n=796)
...
- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.162 (n=518)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` > 0.0089 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.3432` → IC=+0.141 (n=1367)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.3432 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.175 (n=780)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 15.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` > `0.3929` → IC=+0.168 (n=1553)

  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` > 0.3929 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.817` → IC=+0.162 (n=217)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.817 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.678` → IC=+0.167 (n=716)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` > 3.678 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `0.8692` → IC=+0.154 (n=894)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.8692 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` > `0.6933` → IC=+0.136 (n=1197)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 0.6933 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.1633` → IC=+0.167 (n=428)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_pendiente_norm` > 0.1633 (IC base=+0.136)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.141 (n=1734)

  - _Acción_: Kelly boost +0.70€ cuando `libro_spread` < 0.02 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `2476.6183` → IC=+0.142 (n=1387)

  - _Acción_: Kelly boost +0.71€ cuando `libro_liquidez` > 2476.6183 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `158.0` → IC=+0.156 (n=1330)
...
- **PATRÓN** `drift_60min` |x|≤ `0.34` → IC=+0.133 (n=1450)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.66€ cuando `drift_60min` |x|≤ 0.34 (IC base=+0.117)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.131 (n=1665)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 5.0 (IC base=+0.117)

- **PATRÓN** `ibs_20min` < `0.4878` → IC=+0.158 (n=1450)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` < 0.4878 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` < `0.3354` → IC=+0.120 (n=1593)

  - _Acción_: Kelly boost +0.60€ cuando `dist_vwap_pct` < 0.3354 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.847` → IC=+0.132 (n=640)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` > 3.847 (IC base=+0.117)

- **PATRÓN** `volumen_regimen` < `1.2216` → IC=+0.124 (n=1462)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2373` → IC=+0.136 (n=1389)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 2.2373 (IC base=+0.117)

- **PATRÓN** `libro_liquidez` > `2625.3161` → IC=+0.123 (n=1472)
...
- **PATRÓN** `sigma_h` < `0.0027` → IC=+0.214 (n=173)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0027 (IC base=+0.137)

- **PATRÓN** `drift_60min` |x|≤ `0.3387` → IC=+0.157 (n=511)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.3387 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.154 (n=460)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 7.0 (IC base=+0.137)

- **PATRÓN** `ibs_20min` < `0.3422` → IC=+0.208 (n=341)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3422 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.41` → IC=+0.149 (n=203)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 4.41 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.142` → IC=+0.138 (n=581)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` < 9.142 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `1.208` → IC=+0.139 (n=511)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` < 1.208 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` > `1.0622` → IC=+0.175 (n=232)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 1.0622 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` > `0.1596` → IC=+0.217 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1596 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` < `2.4413` → IC=+0.157 (n=502)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.4413 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `1.4209` → IC=+0.144 (n=501)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.4209 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `154.0` → IC=+0.179 (n=160)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 154.0 (IC base=+0.137)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.126 (n=316)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 8.0 (IC base=+0.089)

- **PATRÓN** `ibs_20min` > `0.8906` → IC=+0.209 (n=228)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8906 (IC base=+0.089)

- **PATRÓN** `dist_vwap_pct` > `0.5918` → IC=+0.129 (n=68)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` > 0.5918 (IC base=+0.089)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.234` → IC=+0.173 (n=157)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 5.234 (IC base=+0.089)
...
- **PATRÓN** `libro_liquidez` > `3078.7088` → IC=+0.207 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3078.7088 (IC base=+0.089)

- **PATRÓN** `ballena_activa_n` < `21.0` → IC=+0.152 (n=113)

  - _Acción_: Kelly boost +0.76€ cuando `ballena_activa_n` < 21.0 (IC base=+0.089)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.192 (n=3675)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0087 (IC base=+0.164)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.173 (n=8462)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 5.0 (IC base=+0.164)

- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.212 (n=8095)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4706 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.8871` → IC=+0.195 (n=1046)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.8871 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.75` → IC=+0.231 (n=3028)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.75 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` < `0.8826` → IC=+0.165 (n=3625)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.8826 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.2393` → IC=+0.190 (n=1522)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.2393 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` > `2.6304` → IC=+0.182 (n=2573)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 2.6304 (IC base=+0.164)

- **PATRÓN** `libro_liquidez` > `3811.2917` → IC=+0.168 (n=2698)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 3811.2917 (IC base=+0.164)

- **PATRÓN** `ballena_activa_n` < `92.0` → IC=+0.192 (n=5923)

  - _Acción_: Kelly boost +0.96€ cuando `ballena_activa_n` < 92.0 (IC base=+0.164)

- **PATRÓN** `sigma_h` < `0.0068` → IC=+0.196 (n=4948)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0068 (IC base=+0.183)

- **PATRÓN** `drift_60min` |x|≤ `0.4818` → IC=+0.185 (n=7421)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.4818 (IC base=+0.183)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.206 (n=2819)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.183)

- **PATRÓN** `ibs_20min` < `0.5608` → IC=+0.240 (n=7421)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5608 (IC base=+0.183)

- **PATRÓN** `dist_vwap_pct` < `0.2311` → IC=+0.164 (n=4704)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.2311 (IC base=+0.183)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.842` → IC=+0.196 (n=1064)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 9.842 (IC base=+0.183)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.683` → IC=+0.184 (n=7177)

  - _Acción_: Kelly boost +0.92€ cuando `sigma_ewma_delta_pct` < 3.683 (IC base=+0.183)

- **PATRÓN** `volumen_regimen` < `0.7044` → IC=+0.160 (n=2260)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.7044 (IC base=+0.183)

- **PATRÓN** `volumen_regimen` > `1.2035` → IC=+0.160 (n=1712)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 1.2035 (IC base=+0.183)

- **PATRÓN** `volumen_pendiente_norm` > `0.2891` → IC=+0.248 (n=967)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2891 (IC base=+0.183)

- **PATRÓN** `volumen_spike_ratio` > `2.2995` → IC=+0.192 (n=3031)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 2.2995 (IC base=+0.183)

- **PATRÓN** `ballena_activa_n` < `25.0` → IC=+0.194 (n=2129)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 25.0 (IC base=+0.183)
...
- **PATRÓN** `sigma_h` > `0.0063` → IC=+0.206 (n=914)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0063 (IC base=+0.188)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.192 (n=664)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.319 (n=484)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.188)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.299` → IC=+0.344 (n=312)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.299 (IC base=+0.188)

- **PATRÓN** `volumen_pendiente_norm` > `0.2233` → IC=+0.239 (n=247)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2233 (IC base=+0.188)

- **PATRÓN** `volumen_spike_ratio` > `2.2361` → IC=+0.194 (n=579)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 2.2361 (IC base=+0.188)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.213 (n=1275)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.188)

- **PATRÓN** `sigma_h` < `0.0075` → IC=+0.257 (n=1060)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0075 (IC base=+0.256)

- **PATRÓN** `sigma_h` > `0.0044` → IC=+0.265 (n=1060)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_pendiente_norm` > `0.224` → IC=+0.305 (n=193)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.224 (IC base=+0.256)

- **PATRÓN** `volumen_spike_ratio` > `1.88` → IC=+0.279 (n=637)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.88 (IC base=+0.256)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.256 (n=1109)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.256)

- **PATRÓN** `libro_liquidez` > `1747.06` → IC=+0.276 (n=707)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1747.06 (IC base=+0.256)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.195 (n=434)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0028 (IC base=+0.148)

- **PATRÓN** `drift_60min` |x|≤ `0.1844` → IC=+0.153 (n=858)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.1844 (IC base=+0.148)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=1345)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.148)

- **PATRÓN** `ibs_20min` > `0.3159` → IC=+0.199 (n=1287)

  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` > 0.3159 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` > `0.3242` → IC=+0.195 (n=489)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.3242 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.902` → IC=+0.153 (n=436)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 6.902 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.296` → IC=+0.154 (n=1144)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 4.296 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` < `0.6973` → IC=+0.181 (n=566)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.6973 (IC base=+0.148)

- **PATRÓN** `volumen_pendiente_norm` > `0.1544` → IC=+0.175 (n=352)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.1544 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` < `2.1166` → IC=+0.156 (n=1085)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.1166 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` > `1.7608` → IC=+0.158 (n=822)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.7608 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `10925.7104` → IC=+0.165 (n=1150)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 10925.7104 (IC base=+0.148)

- **PATRÓN** `ballena_activa_n` < `487.0` → IC=+0.159 (n=1162)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 487.0 (IC base=+0.148)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.171 (n=1148)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0058 (IC base=+0.155)

- **PATRÓN** `drift_60min` |x|≤ `0.2618` → IC=+0.169 (n=1009)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.2618 (IC base=+0.155)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.174 (n=385)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 18.0 (IC base=+0.155)

- **PATRÓN** `ibs_20min` < `0.6377` → IC=+0.207 (n=1147)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6377 (IC base=+0.155)

- **PATRÓN** `dist_vwap_pct` > `0.6888` → IC=+0.165 (n=171)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.6888 (IC base=+0.155)

- **PATRÓN** `dist_vwap_pct` < `0.1267` → IC=+0.166 (n=1061)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.1267 (IC base=+0.155)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.199` → IC=+0.166 (n=567)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 3.199 (IC base=+0.155)

- **PATRÓN** `volumen_regimen` < `1.2029` → IC=+0.163 (n=1147)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.2029 (IC base=+0.155)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4232` → IC=+0.169 (n=1049)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 2.4232 (IC base=+0.155)

- **PATRÓN** `libro_liquidez` > `12561.7266` → IC=+0.157 (n=764)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 12561.7266 (IC base=+0.155)

- **PATRÓN** `ballena_activa_n` < `366.0` → IC=+0.169 (n=635)

  - _Acción_: Kelly boost +0.84€ cuando `ballena_activa_n` < 366.0 (IC base=+0.155)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0059` → IC=+0.226 (n=1292)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0059 (IC base=+0.210)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.216 (n=1356)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_spike_ratio` < `1.7946` → IC=+0.229 (n=485)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7946 (IC base=+0.234)

- **PATRÓN** `volumen_spike_ratio` > `2.2483` → IC=+0.234 (n=735)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2483 (IC base=+0.234)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.248 (n=693)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.234)

- **PATRÓN** `libro_liquidez` > `1899.5579` → IC=+0.244 (n=409)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1899.5579 (IC base=+0.234)

- **PATRÓN** `ballena_activa_n` < `15.0` → IC=+0.262 (n=355)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 15.0 (IC base=+0.234)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0034` → IC=+0.175 (n=463)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` < 0.0034 (IC base=+0.134)

- **PATRÓN** `drift_60min` |x|≤ `0.4371` → IC=+0.138 (n=1375)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.69€ cuando `drift_60min` |x|≤ 0.4371 (IC base=+0.134)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.144 (n=1440)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 5.0 (IC base=+0.134)

- **PATRÓN** `ibs_20min` > `0.6988` → IC=+0.229 (n=916)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6988 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` > `0.5571` → IC=+0.174 (n=369)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.5571 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.201` → IC=+0.160 (n=578)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 4.201 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` < `0.8772` → IC=+0.158 (n=917)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5119` → IC=+0.146 (n=583)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 1.5119 (IC base=+0.134)

- **PATRÓN** `volumen_spike_ratio` > `2.4575` → IC=+0.157 (n=441)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 2.4575 (IC base=+0.134)

- **PATRÓN** `libro_liquidez` > `8736.9866` → IC=+0.226 (n=623)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8736.9866 (IC base=+0.134)

- **PATRÓN** `ballena_activa_n` < `85.0` → IC=+0.165 (n=416)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 85.0 (IC base=+0.134)

- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.164 (n=1116)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0076 (IC base=+0.143)

- **PATRÓN** `drift_60min` |x|≤ `0.4454` → IC=+0.161 (n=1114)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.4454 (IC base=+0.143)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.177 (n=419)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.6856` → IC=+0.197 (n=1114)

  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` < 0.6856 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` > `0.1523` → IC=+0.145 (n=496)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.1523 (IC base=+0.143)

- **PATRÓN** `dist_vwap_pct` < `0.3486` → IC=+0.145 (n=1142)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.3486 (IC base=+0.143)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.012` → IC=+0.192 (n=167)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 11.012 (IC base=+0.143)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.194` → IC=+0.144 (n=1040)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 4.194 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` < `0.695` → IC=+0.150 (n=490)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 0.695 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` > `1.1774` → IC=+0.154 (n=371)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.1774 (IC base=+0.143)

- **PATRÓN** `volumen_pendiente_norm` > `0.2787` → IC=+0.276 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2787 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` > `2.4751` → IC=+0.174 (n=348)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.4751 (IC base=+0.143)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.162 (n=518)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 17.0 (IC base=+0.098)

- **PATRÓN** `ibs_20min` > `0.4694` → IC=+0.181 (n=1384)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1292` → IC=+0.155 (n=447)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.1292 (IC base=+0.113)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.148 (n=629)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 15.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` < `0.6371` → IC=+0.208 (n=1339)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6371 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` < `0.4687` → IC=+0.129 (n=1312)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.4687 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.439` → IC=+0.127 (n=1288)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` < 3.439 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` < `0.72` → IC=+0.150 (n=590)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 0.72 (IC base=+0.113)

- **PATRÓN** `volumen_pendiente_norm` > `0.2227` → IC=+0.168 (n=206)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_pendiente_norm` > 0.2227 (IC base=+0.113)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2313` → IC=+0.123 (n=536)
...
- **PATRÓN** `sigma_h` > `0.0242` → IC=+0.216 (n=632)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0242 (IC base=+0.205)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.210 (n=1455)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.205)

- **PATRÓN** `ibs_20min` > `0.5116` → IC=+0.245 (n=1392)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.212 (n=1416)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.205)

- **PATRÓN** `libro_liquidez` > `2587.0012` → IC=+0.209 (n=928)
...
- **PATRÓN** `sigma_h` > `0.0256` → IC=+0.226 (n=513)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0256 (IC base=+0.201)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.216 (n=745)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.201)

- **PATRÓN** `ibs_20min` < `0.5129` → IC=+0.254 (n=1537)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5129 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` < `0.2661` → IC=+0.206 (n=1436)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2661 (IC base=+0.201)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.723` → IC=+0.262 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.723 (IC base=+0.201)

- **PATRÓN** `volumen_regimen` > `1.2298` → IC=+0.234 (n=513)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2257` → IC=+0.196 (n=1194)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.2257 (IC base=+0.201)

- **PATRÓN** `volumen_spike_ratio` > `1.4453` → IC=+0.200 (n=1357)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4453 (IC base=+0.201)
...
- **PATRÓN** `dist_vwap_pct` > `0.1877` → IC=+0.155 (n=830)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.1877 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.737` → IC=+0.167 (n=818)
...
- **PATRÓN** `libro_liquidez` > `8268.5946` → IC=+0.154 (n=1150)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 8268.5946 (IC base=+0.145)
...
- **PATRÓN** `sigma_h` < `0.0088` → IC=+0.152 (n=753)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.5046` → IC=+0.153 (n=753)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.5046 (IC base=+0.144)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.164 (n=293)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.185` → IC=+0.154 (n=753)
...
- **PATRÓN** `dist_vwap_pct` < `0.4235` → IC=+0.151 (n=717)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.4235 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.678` → IC=+0.152 (n=752)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` < 6.678 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` < `1.1122` → IC=+0.146 (n=662)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.151 (n=715)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.167 (n=232)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 18.0 (IC base=+0.106)

- **PATRÓN** `ibs_20min` > `0.6355` → IC=+0.214 (n=600)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6355 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` > `0.1301` → IC=+0.172 (n=315)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.1301 (IC base=+0.106)
...
- **PATRÓN** `volumen_regimen` < `0.8014` → IC=+0.135 (n=401)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 0.8014 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` > `0.977` → IC=+0.128 (n=272)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` > 0.977 (IC base=+0.106)
...
### LEADLAG_BTC_XRP_15M
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.153 (n=188)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 16.0 (IC base=+0.113)

- **PATRÓN** `py_entrada` > `0.5` → IC=+0.127 (n=505)

  - _Acción_: Kelly boost +0.64€ cuando `py_entrada` > 0.5 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `2843.9296` → IC=+0.182 (n=174)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 2843.9296 (IC base=+0.113)

- **PATRÓN** `libro_liquidez` > `2318.4277` → IC=+0.123 (n=579)

  - _Acción_: Kelly boost +0.62€ cuando `libro_liquidez` > 2318.4277 (IC base=+0.100)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=6669)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.181 (n=2781)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=8495)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.172 (n=2871)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.036 (n=8885)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.44` → IC=-0.215 (n=475)

  - _Acción_: SKIP cuando `py_entrada` < 0.44
  - _Potencial_: sin este filtro IC_bueno=+0.094 (n=1440)
...
- **FILTRO** `py_entrada` < `0.43` → IC=-0.206 (n=474)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.086 (n=1483)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.221 (n=489)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.062 (n=1587)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.202 (n=458)
...
- **FILTRO** `py_entrada` > `0.58` → IC=-0.194 (n=508)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.053 (n=1565)

### MOMENTUM_IBS_15M_FADE
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=2469)

- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)

  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=2536)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=2542)

### MOMENTUM_IBS_15M_FADE#BTC#15min
- **FILTRO** `hora_utc` < `15.0` → IC=-0.147 (n=83)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=308)

- **FILTRO** `ibs_20min` > `0.1427` → IC=-0.127 (n=132)

  - _Acción_: SKIP cuando `ibs_20min` > 0.1427
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=259)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.130 (n=8036)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=18245)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.283 (n=6213)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=20068)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.245 (n=6516)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=19765)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.161 (n=8928)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=17353)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.225 (n=8255)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=24833)

- **FILTRO** `ibs_7min` > `0.2955` → IC=-0.176 (n=8271)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2955
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=24817)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.133 (n=1302)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.087 (n=2946)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.309 (n=1006)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=3242)

- **FILTRO** `ibs_7min` < `0.7071` → IC=-0.261 (n=1401)
...
- **FILTRO** `ballena_activa_n` > `9.0` → IC=-0.197 (n=976)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=3272)

- **FILTRO** `py_entrada` > `0.505` → IC=-0.150 (n=3850)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=1896)

- **FILTRO** `drift_7min_pct` |x|> `0.1107` → IC=-0.122 (n=1953)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1107
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=3793)

- **FILTRO** `ibs_7min` > `0.7976` → IC=-0.206 (n=1435)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7976
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=4311)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=1056)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=3529)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.258 (n=1084)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=3501)

- **FILTRO** `ibs_7min` < `0.7547` → IC=-0.189 (n=1146)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7547
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=3439)

- **FILTRO** `ballena_activa_n` > `162.0` → IC=-0.179 (n=1140)

  - _Acción_: SKIP cuando `ballena_activa_n` > 162.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3445)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.258 (n=1112)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=3559)

- **FILTRO** `ibs_7min` > `0.2547` → IC=-0.170 (n=1167)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2547
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=3504)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.181 (n=970)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.092 (n=3002)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.318 (n=957)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=3015)

- **FILTRO** `drift_7min_pct` |x|> `0.1797` → IC=-0.132 (n=1350)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1797
  - _Potencial_: sin este filtro IC_bueno=-0.105 (n=2622)

- **FILTRO** `ibs_7min` < `0.2` → IC=-0.272 (n=984)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=2988)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.221 (n=920)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.082 (n=3052)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.235 (n=1418)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=4642)

- **FILTRO** `ibs_7min` > `0.2609` → IC=-0.155 (n=2057)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2609
  - _Potencial_: sin este filtro IC_bueno=+0.016 (n=4003)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.129 (n=1374)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.092 (n=2958)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.253 (n=1061)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=3271)

- **FILTRO** `ibs_7min` < `0.7428` → IC=-0.191 (n=1083)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7428
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3249)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.186 (n=1068)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.076 (n=3264)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.260 (n=1097)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=3353)

- **FILTRO** `ibs_7min` > `0.2743` → IC=-0.174 (n=1112)
...
- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.182 (n=1091)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=3359)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.37` → IC=-0.258 (n=1135)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=3571)

- **FILTRO** `ibs_7min` < `0.7143` → IC=-0.233 (n=1151)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=3555)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.169 (n=1527)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=4738)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.126 (n=1346)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.079 (n=3092)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.282 (n=1100)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=3338)

- **FILTRO** `ibs_7min` < `0.72` → IC=-0.236 (n=1106)

  - _Acción_: SKIP cuando `ibs_7min` < 0.72
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=3332)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.213 (n=1085)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=3353)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.206 (n=1396)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=+0.010 (n=4500)
...
- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.125 (n=550)

  - _Acción_: Kelly boost +0.62€ cuando `ballena_activa_n` < 61.0 (IC base=+0.121)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.190 (n=27)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=601)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=2432)
...
### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.193 (n=386)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0041 (IC base=+0.178)

- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.221 (n=524)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0078 (IC base=+0.178)

- **PATRÓN** `drift_60min` |x|≤ `0.0771` → IC=+0.183 (n=509)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.91€ cuando `drift_60min` |x|≤ 0.0771 (IC base=+0.178)

- **PATRÓN** `delta_ratio_macro` |x|> `0.058` → IC=+0.180 (n=1156)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.90€ cuando `delta_ratio_macro` |x|> 0.058 (IC base=+0.178)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1356` → IC=+0.232 (n=368)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1356 (IC base=+0.178)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.187 (n=816)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` > `0.62` → IC=+0.253 (n=1155)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.62 (IC base=+0.178)
...
- **PATRÓN** `ibs_15` < `0.113` → IC=+0.169 (n=367)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.85€ cuando `ibs_15` < 0.113 (IC base=+0.046)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0717` → IC=+0.207 (n=220)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0717 (IC base=-0.041)
...
- **PATRÓN** `dist_vwap_pct` < `0.1586` → IC=+0.210 (n=219)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1586 (IC base=-0.041)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.62 sube el IC de +0.178 a +0.253 en UPDOWN_GBM#15min (n=1155). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.113 sube el IC de +0.046 a +0.169 en UPDOWN_GBM#XRP#15min (n=367). Ya aplicado como kelly_boost=+0.85€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#SOL#15min**: dentro de BUY_NO, IBS < 0.3667 sube el IC de -0.041 a +0.254 en UPDOWN_GBM_15M_TARDIO#SOL#15min (n=246). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 21407 | -0.097 | -3152.27€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1272 | -0.050 | -194.63€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 20135 | -0.100 | -2957.63€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3305 | -0.103 | -567.30€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3305 | -0.103 | -567.30€ | 1 | 0 |
...
| ✅ BALLENAS_TARDIAS#ETH | 6132 | -0.041 | -609.18€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 6132 | -0.041 | -609.18€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 5684 | -0.100 | -482.18€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 5684 | -0.100 | -482.18€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 4640 | -0.173 | -1137.93€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 4640 | -0.173 | -1137.93€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 13302 | -0.039 | +4197.20€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 3551 | -0.005 | +1835.36€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 9751 | -0.051 | +2361.84€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC |