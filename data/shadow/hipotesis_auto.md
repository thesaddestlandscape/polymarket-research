# Hipótesis automáticas — 2026-09-02 14:58 UTC
_Generado por shadow_postmortem.py sobre 253584 resoluciones (PNL=+22694.33€)_
...
- **PATRÓN** `py_entrada` < `0.805` → IC=+0.138 (n=241)

  - _Acción_: Kelly boost +0.69€ cuando `py_entrada` < 0.805 (IC base=+0.006)

- **PATRÓN** `ballena_activa_n` < `116.0` → IC=+0.129 (n=33)

  - _Acción_: Kelly boost +0.64€ cuando `ballena_activa_n` < 116.0 (IC base=+0.006)
...
- **PATRÓN** `py_entrada` < `0.495` → IC=+0.122 (n=125)

  - _Acción_: Kelly boost +0.61€ cuando `py_entrada` < 0.495 (IC base=-0.025)
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `147.44` → IC=-0.292 (n=3460)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 147.44
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=10382)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `153.08` → IC=-0.240 (n=448)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.138 (n=1331)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 8.0 (IC base=+0.132)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.138 (n=1314)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.174 (n=1444)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` < 17.0 (IC base=+0.164)

- **PATRÓN** `py_entrada` < `0.74` → IC=+0.178 (n=1507)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.245 (n=1331)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.235)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.236 (n=1130)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.206 (n=16127)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.71 (IC base=+0.188)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.146 (n=3319)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.171 (n=2331)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.258 (n=2082)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.179 (n=2765)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.277 (n=1006)
...
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.209 (n=2335)
...
- **PATRÓN** `restante_min` > `4.93` → IC=+0.154 (n=2295)
...
- **PATRÓN** `lag_apertura_s` < `4.24` → IC=+0.157 (n=2156)
...
- **PATRÓN** `lag_apertura_s` < `3.36` → IC=+0.169 (n=1085)

  - _Acción_: Kelly boost +0.84€ cuando `lag_apertura_s` < 3.36 (IC base=+0.131)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3511` → IC=+0.125 (n=3347)

  - _Acción_: Kelly boost +0.63€ cuando `ibs_20min` > 0.3511 (IC base=+0.075)

- **PATRÓN** `dist_vwap_pct` > `0.2623` → IC=+0.224 (n=407)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2623 (IC base=+0.075)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.525` → IC=+0.145 (n=1285)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` > 5.525 (IC base=+0.075)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1705` → IC=+0.171 (n=561)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.1705 (IC base=+0.075)

- **PATRÓN** `volumen_spike_ratio` < `1.6042` → IC=+0.172 (n=863)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.6042 (IC base=+0.075)

- **PATRÓN** `volumen_spike_ratio` > `1.4772` → IC=+0.168 (n=1962)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 1.4772 (IC base=+0.075)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.203 (n=941)
...
- **PATRÓN** `drift_60min` |x|≤ `0.271` → IC=+0.151 (n=740)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.271 (IC base=+0.150)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4596` → IC=+0.146 (n=643)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.4596 (IC base=+0.150)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.191 (n=580)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.04 (IC base=+0.150)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.227 (n=607)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1813` → IC=+0.219 (n=354)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `7.071` → IC=+0.213 (n=583)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 7.071 (IC base=+0.207)

- **PATRÓN** `volumen_regimen` < `1.2608` → IC=+0.209 (n=583)
...
- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.197 (n=464)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.258` → IC=+0.255 (n=161)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.258 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` < `0.1318` → IC=+0.170 (n=588)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` < 0.1318 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` < `3.2528` → IC=+0.157 (n=538)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 3.2528 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `1.6757` → IC=+0.173 (n=612)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.6757 (IC base=+0.165)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.191 (n=698)
...
- **FILTRO** `ibs_20min` < `0.2905` → IC=-0.141 (n=210)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=429)
...
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.149 (n=1887)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0077 (IC base=+0.062)

- **PATRÓN** `ibs_20min` > `0.2727` → IC=+0.131 (n=4155)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` > 0.2727 (IC base=+0.062)

- **PATRÓN** `dist_vwap_pct` > `1.1893` → IC=+0.287 (n=289)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1893 (IC base=+0.062)

- **PATRÓN** `volumen_regimen` > `0.6802` → IC=+0.210 (n=1190)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6802 (IC base=+0.062)

- **PATRÓN** `volumen_pendiente_norm` < `0.1142` → IC=+0.209 (n=1892)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1142 (IC base=+0.062)

- **PATRÓN** `volumen_pendiente_norm` > `0.2526` → IC=+0.217 (n=369)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2526 (IC base=+0.062)

- **PATRÓN** `volumen_spike_ratio` < `1.4927` → IC=+0.229 (n=648)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4927 (IC base=+0.062)

- **PATRÓN** `volumen_spike_ratio` > `2.8513` → IC=+0.215 (n=648)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8513 (IC base=+0.062)

- **PATRÓN** `ballena_activa_n` < `91.0` → IC=+0.298 (n=1398)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 91.0 (IC base=+0.062)

- **PATRÓN** `ibs_20min` < `0.5866` → IC=+0.127 (n=4097)
...
- **FILTRO** `sigma_ewma_delta_pct` > `2.262` → IC=-0.162 (n=297)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=644)

- **PATRÓN** `ibs_20min` > `0.7933` → IC=+0.200 (n=258)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7933 (IC base=+0.009)

- **PATRÓN** `volumen_pendiente_norm` > `0.2317` → IC=+0.362 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2317 (IC base=+0.009)

- **PATRÓN** `volumen_spike_ratio` > `2.7645` → IC=+0.283 (n=44)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7645 (IC base=+0.009)

- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.374 (n=85)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 61.0 (IC base=+0.009)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **FILTRO** `ibs_20min` > `0.8684` → IC=-0.162 (n=359)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8684
  - _Potencial_: sin este filtro IC_bueno=+0.016 (n=1080)

- **PATRÓN** `ballena_activa_n` < `254.0` → IC=+0.137 (n=89)

  - _Acción_: Kelly boost +0.69€ cuando `ballena_activa_n` < 254.0 (IC base=-0.026)
...
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.277 (n=307)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.7091` → IC=+0.253 (n=601)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1064` → IC=+0.236 (n=548)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1064 (IC base=+0.217)

- **PATRÓN** `volumen_spike_ratio` < `2.4715` → IC=+0.222 (n=393)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.246 (n=675)
...
- **FILTRO** `ibs_20min` < `0.7307` → IC=-0.151 (n=431)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.114` → IC=+0.148 (n=52)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` < 0.114 (IC base=-0.078)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4124` → IC=+0.220 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4124 (IC base=-0.078)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **FILTRO** `ibs_20min` < `0.7826` → IC=-0.149 (n=596)
...
- **PATRÓN** `ibs_20min` > `0.7826` → IC=+0.268 (n=308)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7826 (IC base=-0.007)
...
- **PATRÓN** `sigma_h` > `0.0214` → IC=+0.323 (n=315)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0214 (IC base=+0.247)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.246 (n=731)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.247)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.257 (n=614)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.247)

- **PATRÓN** `ibs_20min` > `0.8982` → IC=+0.317 (n=463)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8982 (IC base=+0.247)

- **PATRÓN** `dist_vwap_pct` > `0.1484` → IC=+0.311 (n=395)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1484 (IC base=+0.247)
...
- **PATRÓN** `volumen_regimen` > `0.8427` → IC=+0.285 (n=463)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8427 (IC base=+0.247)

- **PATRÓN** `volumen_pendiente_norm` > `0.2397` → IC=+0.288 (n=130)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2397 (IC base=+0.247)

- **PATRÓN** `volumen_spike_ratio` < `2.5769` → IC=+0.254 (n=635)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2293` → IC=+0.252 (n=288)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2293 (IC base=+0.247)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.256 (n=700)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.247)

- **PATRÓN** `libro_liquidez` > `2469.6086` → IC=+0.251 (n=621)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2469.6086 (IC base=+0.247)
...
- **PATRÓN** `dist_vwap_pct` > `1.111` → IC=+0.244 (n=557)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.111 (IC base=+0.163)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.585` → IC=+0.233 (n=1420)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.585 (IC base=+0.163)

- **PATRÓN** `volumen_regimen` > `0.6285` → IC=+0.168 (n=2371)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3275` → IC=+0.166 (n=2836)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.3275 (IC base=+0.163)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.167 (n=3614)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.03 (IC base=+0.163)

- **PATRÓN** `libro_liquidez` > `3946.0946` → IC=+0.180 (n=1162)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 3946.0946 (IC base=+0.163)

- **PATRÓN** `ballena_activa_n` < `90.0` → IC=+0.183 (n=2112)
...
- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.193 (n=262)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.627` → IC=+0.315 (n=187)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.627 (IC base=+0.171)
...
- **PATRÓN** `volumen_spike_ratio` < `2.718` → IC=+0.154 (n=495)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.718 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` > `1.4596` → IC=+0.166 (n=495)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.4596 (IC base=+0.171)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.205 (n=463)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4092` → IC=+0.181 (n=509)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.91€ cuando `drift_60min` |x|≤ 0.4092 (IC base=+0.171)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.195 (n=516)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` > 6.0 (IC base=+0.171)

- **PATRÓN** `ibs_20min` > `0.4579` → IC=+0.222 (n=509)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4579 (IC base=+0.171)

- **PATRÓN** `dist_vwap_pct` > `0.1867` → IC=+0.232 (n=352)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `7.52` → IC=+0.174 (n=486)
...
- **PATRÓN** `volumen_regimen` > `1.0648` → IC=+0.182 (n=231)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` > 1.0648 (IC base=+0.171)

- **PATRÓN** `volumen_pendiente_norm` > `0.1606` → IC=+0.195 (n=152)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.1606 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` < `2.1261` → IC=+0.191 (n=422)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` < 2.1261 (IC base=+0.171)

- **PATRÓN** `libro_liquidez` > `11354.3179` → IC=+0.192 (n=455)
...
- **PATRÓN** `sigma_h` > `0.009` → IC=+0.236 (n=180)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.009 (IC base=+0.172)

- **PATRÓN** `drift_60min` |x|≤ `0.2711` → IC=+0.189 (n=474)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.2711 (IC base=+0.172)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.376` → IC=+0.297 (n=126)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.376 (IC base=+0.172)

- **PATRÓN** `volumen_pendiente_norm` < `0.1052` → IC=+0.167 (n=427)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` < 0.1052 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` < `1.7273` → IC=+0.183 (n=159)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` < 1.7273 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` > `3.2528` → IC=+0.165 (n=216)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 3.2528 (IC base=+0.172)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.197 (n=529)

  - _Acción_: Kelly boost +0.98€ cuando `libro_spread` < 0.04 (IC base=+0.172)

- **PATRÓN** `libro_liquidez` > `1910.8884` → IC=+0.176 (n=180)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 1910.8884 (IC base=+0.172)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4658` → IC=+0.179 (n=512)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.89€ cuando `drift_60min` |x|≤ 0.4658 (IC base=+0.168)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.183 (n=521)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 6.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` > `0.4362` → IC=+0.212 (n=512)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4362 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` > `0.1502` → IC=+0.204 (n=349)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1502 (IC base=+0.168)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.194` → IC=+0.286 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.194 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` > `0.6402` → IC=+0.181 (n=512)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` > 0.6402 (IC base=+0.168)
...
- **PATRÓN** `ballena_activa_n` < `171.0` → IC=+0.158 (n=396)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 171.0 (IC base=+0.168)
...
- **PATRÓN** `sigma_h` > `0.0106` → IC=+0.181 (n=302)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0106 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=697)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` > `0.5417` → IC=+0.190 (n=666)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.144` → IC=+0.259 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.144 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` > `0.6282` → IC=+0.127 (n=666)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` > 0.6282 (IC base=+0.114)

- **PATRÓN** `volumen_pendiente_norm` < `0.1669` → IC=+0.136 (n=660)
...
- **PATRÓN** `libro_liquidez` > `3211.4832` → IC=+0.228 (n=222)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3211.4832 (IC base=+0.114)

- **PATRÓN** `ballena_activa_n` < `51.0` → IC=+0.150 (n=412)
...
- **PATRÓN** `sigma_h` > `0.0097` → IC=+0.175 (n=244)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0097 (IC base=+0.134)

- **PATRÓN** `drift_60min` |x|≤ `0.3354` → IC=+0.137 (n=642)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.3354 (IC base=+0.134)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.162 (n=696)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_regimen` > `0.6967` → IC=+0.145 (n=519)
...
- **PATRÓN** `volumen_spike_ratio` < `2.307` → IC=+0.144 (n=594)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.140 (n=770)

  - _Acción_: Kelly boost +0.70€ cuando `libro_spread` < 0.02 (IC base=+0.134)

- **PATRÓN** `libro_liquidez` > `5190.7142` → IC=+0.161 (n=243)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.407` → IC=+0.228 (n=2129)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.407 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` < `0.882` → IC=+0.158 (n=1929)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.882 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` > `1.0851` → IC=+0.152 (n=1312)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.0851 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.1659` → IC=+0.191 (n=1119)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.1659 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` < `2.3154` → IC=+0.165 (n=3376)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.3154 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` > `1.8809` → IC=+0.171 (n=2558)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.8809 (IC base=+0.164)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.165 (n=4311)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.03 (IC base=+0.164)

- **PATRÓN** `libro_liquidez` > `3889.6241` → IC=+0.187 (n=1387)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 3889.6241 (IC base=+0.164)

- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.217 (n=1857)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.184 (n=681)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2211` → IC=+0.258 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2211 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` < `1.499` → IC=+0.173 (n=200)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.499 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` > `1.9052` → IC=+0.180 (n=398)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` > 1.9052 (IC base=+0.182)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.238 (n=514)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.34` → IC=+0.168 (n=588)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` < 4.34 (IC base=+0.159)

- **PATRÓN** `volumen_regimen` < `1.2687` → IC=+0.165 (n=678)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.2687 (IC base=+0.159)

- **PATRÓN** `volumen_regimen` > `1.1002` → IC=+0.168 (n=308)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 1.1002 (IC base=+0.159)

- **PATRÓN** `volumen_pendiente_norm` < `0.0709` → IC=+0.173 (n=567)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` < 0.0709 (IC base=+0.159)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1045` → IC=+0.182 (n=554)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 2.1045 (IC base=+0.159)
...
- **PATRÓN** `libro_liquidez` > `10716.3301` → IC=+0.186 (n=606)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 10716.3301 (IC base=+0.159)

- **PATRÓN** `ballena_activa_n` < `502.0` → IC=+0.176 (n=553)
...
- **PATRÓN** `ibs_20min` > `0.6842` → IC=+0.261 (n=529)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6842 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.487` → IC=+0.337 (n=145)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.487 (IC base=+0.209)

- **PATRÓN** `volumen_pendiente_norm` < `0.1388` → IC=+0.220 (n=484)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1388 (IC base=+0.209)

- **PATRÓN** `volumen_spike_ratio` < `1.9068` → IC=+0.225 (n=227)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9068 (IC base=+0.209)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.240 (n=579)
...
- **PATRÓN** `sigma_h` < `0.0071` → IC=+0.155 (n=601)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0071 (IC base=+0.143)

- **PATRÓN** `sigma_h` > `0.0041` → IC=+0.145 (n=609)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.158 (n=614)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 8.0 (IC base=+0.143)

- **PATRÓN** `ibs_20min` > `0.7445` → IC=+0.237 (n=454)
...
- **PATRÓN** `volumen_regimen` < `0.9021` → IC=+0.170 (n=455)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4124` → IC=+0.176 (n=644)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 1.4124 (IC base=+0.143)
...
- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.147 (n=482)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` > 0.0084 (IC base=+0.096)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.136 (n=484)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 12.0 (IC base=+0.096)

- **PATRÓN** `ibs_20min` > `0.4868` → IC=+0.187 (n=723)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` > 0.4868 (IC base=+0.096)
...
- **PATRÓN** `ballena_activa_n` < `64.0` → IC=+0.143 (n=502)

  - _Acción_: Kelly boost +0.71€ cuando `ballena_activa_n` < 64.0 (IC base=+0.096)
...
- **PATRÓN** `volumen_regimen` > `0.6863` → IC=+0.209 (n=726)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6863 (IC base=+0.197)

- **PATRÓN** `volumen_pendiente_norm` > `0.2864` → IC=+0.268 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2864 (IC base=+0.197)

- **PATRÓN** `volumen_spike_ratio` > `1.4438` → IC=+0.199 (n=763)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4438 (IC base=+0.197)
...
- **PATRÓN** `sigma_h` < `0.0101` → IC=+0.128 (n=1214)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.64€ cuando `sigma_h` < 0.0101 (IC base=+0.119)

- **PATRÓN** `drift_60min` |x|≤ `0.4393` → IC=+0.133 (n=1214)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.67€ cuando `drift_60min` |x|≤ 0.4393 (IC base=+0.119)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.593` → IC=+0.147 (n=216)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` > 10.593 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.071` → IC=+0.123 (n=1401)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` < 6.071 (IC base=+0.119)

- **PATRÓN** `volumen_regimen` > `0.8938` → IC=+0.122 (n=628)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` > 0.8938 (IC base=+0.119)

- **PATRÓN** `volumen_pendiente_norm` > `0.175` → IC=+0.153 (n=376)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3911` → IC=+0.147 (n=619)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 2.3911 (IC base=+0.119)
...
- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.211 (n=102)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0064 (IC base=+0.143)

- **PATRÓN** `sigma_h` > `0.0104` → IC=+0.181 (n=139)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0104 (IC base=+0.143)

- **PATRÓN** `drift_60min` |x|≤ `0.5573` → IC=+0.149 (n=306)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.5573 (IC base=+0.143)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.248` → IC=+0.231 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.248 (IC base=+0.143)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.415` → IC=+0.144 (n=276)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 3.415 (IC base=+0.143)

- **PATRÓN** `volumen_pendiente_norm` < `0.0931` → IC=+0.147 (n=284)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_pendiente_norm` < 0.0931 (IC base=+0.143)

- **PATRÓN** `volumen_pendiente_norm` > `0.215` → IC=+0.154 (n=76)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_pendiente_norm` > 0.215 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` < `1.6694` → IC=+0.183 (n=102)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 1.6694 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` > `2.4202` → IC=+0.163 (n=203)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 2.4202 (IC base=+0.143)

- **PATRÓN** `libro_liquidez` > `1880.7833` → IC=+0.147 (n=273)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 1880.7833 (IC base=+0.143)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.183 (n=1306)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.023 (n=3968)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.187 (n=1379)
...
- **FILTRO** `ibs_20min` < `0.7222` → IC=-0.182 (n=209)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=628)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.190 (n=214)
...
- **FILTRO** `py_entrada` > `0.64` → IC=-0.223 (n=211)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.147 (n=3747)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=9248)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.283 (n=3229)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=9766)

- **FILTRO** `ibs_7min` < `0.7095` → IC=-0.246 (n=3248)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7095
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=9747)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.172 (n=4362)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=8633)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.223 (n=3804)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=12618)

- **FILTRO** `ibs_7min` > `0.72` → IC=-0.170 (n=4103)

  - _Acción_: SKIP cuando `ibs_7min` > 0.72
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=12319)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.170 (n=559)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.107 (n=1314)

- **FILTRO** `py_entrada` < `0.3` → IC=-0.327 (n=454)

  - _Acción_: SKIP cuando `py_entrada` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=1419)

- **FILTRO** `ibs_7min` < `0.283` → IC=-0.267 (n=617)

  - _Acción_: SKIP cuando `ibs_7min` < 0.283
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=1256)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.240 (n=452)
...
- **FILTRO** `py_entrada` > `0.52` → IC=-0.138 (n=1917)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.079 (n=946)

- **FILTRO** `drift_7min_pct` |x|> `0.1143` → IC=-0.128 (n=973)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1143
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=1890)

- **FILTRO** `ibs_7min` > `0.8361` → IC=-0.186 (n=715)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8361
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=2148)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.147 (n=533)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=1914)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.264 (n=588)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=1859)

- **FILTRO** `ibs_7min` < `0.7699` → IC=-0.192 (n=611)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7699
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=1836)

- **FILTRO** `ballena_activa_n` > `166.0` → IC=-0.192 (n=606)
...
- **FILTRO** `ibs_7min` > `0.8049` → IC=-0.172 (n=729)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=2191)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.144 (n=670)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.090 (n=1547)
...
- **FILTRO** `ibs_7min` < `0.75` → IC=-0.194 (n=554)

  - _Acción_: SKIP cuando `ibs_7min` < 0.75
  - _Potencial_: sin este filtro IC_bueno=-0.077 (n=1663)

- **FILTRO** `ballena_activa_n` > `27.0` → IC=-0.185 (n=734)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.236 (n=728)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=1500)

- **FILTRO** `ibs_7min` > `0.2766` → IC=-0.181 (n=556)
...
- **FILTRO** `ballena_activa_n` > `34.0` → IC=-0.182 (n=555)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=1673)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.39` → IC=-0.244 (n=587)

  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=1837)

- **FILTRO** `ibs_7min` < `0.7333` → IC=-0.212 (n=606)
...
- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.194 (n=586)
  ...
  - _Acción_: SKIP cuando `ballena_activa_n` > 13.0
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=1838)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.127 (n=686)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=1437)
...
- **FILTRO** `ibs_7min` < `0.73` → IC=-0.248 (n=529)

  - _Acción_: SKIP cuando `ibs_7min` < 0.73
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=1594)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.226 (n=517)

  - _Acción_: SKIP cuando `ballena_activa_n` > 13.0
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=1606)
...
### UPDOWN_GBM#5min
- **FILTRO** `ibs_15` < `0.3077` → IC=-0.155 (n=166)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: SKIP cuando `ibs_15` < 0.3077
...
- **PATRÓN** `libro_liquidez` > `14354.3614` → IC=+0.283 (n=44)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 14354.3614 (IC base=+0.172)
...
- **FILTRO** `sigma_h` > `0.0074` → IC=-0.235 (n=209)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0074
  - _Potencial_: sin este filtro IC_bueno=-0.190 (n=630)

- **FILTRO** `sigma_h` < `0.0037` → IC=-0.227 (n=276)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0037
  - _Potencial_: sin este filtro IC_bueno=-0.189 (n=563)

- **FILTRO** `drift_15min` |x|> `0.7406` → IC=-0.201 (n=209)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 0.7406
  - _Potencial_: sin este filtro IC_bueno=-0.201 (n=630)

- **FILTRO** `hora_utc` > `17.0` → IC=-0.225 (n=176)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 17.0
  - _Potencial_: sin este filtro IC_bueno=-0.195 (n=663)
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 735 | +0.093 | +42.70€ | 2 | 12 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 735 | +0.093 | +42.70€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 12 | +0.129 | +7.24€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 12 | +0.129 | +7.24€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 469 | +0.112 | +27.21€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 469 | +0.112 | +27.21€ | 1 | 10 |
...
| ✅ BALLENAS_TARDIAS | 13842 | -0.107 | -2336.44€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 914 | -0.005 | -134.65€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 12928 | -0.115 | -2201.79€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1794 | -0.077 | -350.53€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1794 | -0.077 | -350.53€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 914 | -0.005 | -134.65€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 914 | -0.005 | -134.65€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 1792 | -0.157 | -533.70€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 1792 | -0.157 | -533.70€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 3772 | -0.058 | -358.87€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 3772 | -0.058 | -358.87€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 3068 | -0.122 | -291.41€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 3068 | -0.122 | -291.41€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 2502 | -0.186 | -667.27€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 2502 | -0.186 | -667.27€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 3343 | -0.064 | +1551.43€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 949 | -0.005 | +678.34€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 2394 | -0.088 | +873.09€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 3343 | -0.064 | +1551.43€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 949 | -0.005 | +678.34€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 2394 | -0.088 | +873.09€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 44590 | +0.113 | -2918.40€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 7573 | +0.186 | -249.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 140 | -0.099 | -57.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 33253 | +0.096 | -2550.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3624 | +0.118 | -61.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 5585 | +0.079 | -788.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 24 | -0.115 | -1.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 9 | -0.143 | -7.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 5552 | +0.080 | -778.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 8981 | +0.130 | -241.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 2204 | +0.199 | -91.16€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 5506 | +0.105 | -176.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1229 | +0.126 | +48.12€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 5594 | +0.081 | -722.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 26 | +0.036 | +0.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#240min | 6 | -0.075 | -4.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 5562 | +0.082 | -718.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 9665 | +0.126 | -145.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2796 | +0.172 | -5.43€ | 1 | 7 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 5534 | +0.110 | -97.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1323 | +0.097 | -34.28€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 9188 | +0.127 | -617.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2504 | +0.195 | -150.67€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 68 | +0.000 | -12.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 