# Hipótesis automáticas — 2026-09-03 23:58 UTC
_Generado por shadow_postmortem.py sobre 273027 resoluciones (PNL=+26576.78€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.68` → IC=-0.297 (n=3681)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.68
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=11046)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `140.79` → IC=-0.290 (n=478)
...
- **PATRÓN** `libro_liquidez` > `1930.2375` → IC=+0.176 (n=3276)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 1930.2375 (IC base=+0.141)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.217 (n=851)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.210 (n=1088)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.207)

- **PATRÓN** `libro_liquidez` > `13031.9573` → IC=+0.222 (n=386)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13031.9573 (IC base=+0.207)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.145 (n=1549)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.214 (n=389)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.195 (n=6145)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.172 (n=1488)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.174 (n=1587)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.247 (n=1323)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.228 (n=3007)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.217 (n=1051)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.190 (n=1075)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.204 (n=2905)
...
- **PATRÓN** `lag_apertura_s` < `4.19` → IC=+0.156 (n=2299)
...
- **PATRÓN** `restante_min` > `4.88` → IC=+0.152 (n=1577)
...
- **PATRÓN** `lag_apertura_s` < `7.01` → IC=+0.151 (n=1507)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.131 (n=3486)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1083` → IC=+0.174 (n=807)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.1083 (IC base=+0.081)

- **PATRÓN** `volumen_spike_ratio` < `2.8806` → IC=+0.175 (n=2185)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.8806 (IC base=+0.081)

- **PATRÓN** `volumen_spike_ratio` > `1.4772` → IC=+0.169 (n=2185)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `7.068` → IC=+0.215 (n=641)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 7.068 (IC base=+0.212)

- **PATRÓN** `volumen_regimen` < `1.2628` → IC=+0.215 (n=640)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1` → IC=+0.218 (n=607)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1 (IC base=+0.212)

- **PATRÓN** `volumen_spike_ratio` < `2.1133` → IC=+0.228 (n=542)
...
- **PATRÓN** `dist_vwap_pct` > `0.377` → IC=+0.271 (n=33)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.377 (IC base=-0.033)

- **PATRÓN** `dist_vwap_pct` < `0.2033` → IC=+0.278 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2033 (IC base=-0.033)

- **PATRÓN** `volumen_regimen` < `0.6119` → IC=+0.362 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6119 (IC base=-0.033)

- **PATRÓN** `volumen_regimen` > `1.1956` → IC=+0.286 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1956 (IC base=-0.033)

- **PATRÓN** `volumen_pendiente_norm` < `0.1203` → IC=+0.259 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1203 (IC base=-0.033)

- **PATRÓN** `volumen_pendiente_norm` > `0.2279` → IC=+0.286 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2279 (IC base=-0.033)
...
- **PATRÓN** `volumen_spike_ratio` > `1.924` → IC=+0.269 (n=50)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.924 (IC base=-0.033)

- **PATRÓN** `ballena_activa_n` < `175.0` → IC=+0.268 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 175.0 (IC base=-0.033)
...
- **FILTRO** `ibs_20min` < `0.2946` → IC=-0.149 (n=374)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.102 (n=762)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.072` → IC=+0.126 (n=426)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` > 2.072 (IC base=+0.019)

- **PATRÓN** `volumen_pendiente_norm` > `0.2248` → IC=+0.337 (n=41)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2248 (IC base=+0.019)

- **PATRÓN** `volumen_spike_ratio` < `1.6273` → IC=+0.227 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6273 (IC base=+0.019)

- **PATRÓN** `volumen_spike_ratio` > `2.7084` → IC=+0.261 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7084 (IC base=+0.019)

- **PATRÓN** `ballena_activa_n` < `64.0` → IC=+0.325 (n=135)
...
- **FILTRO** `ibs_20min` < `0.7741` → IC=-0.136 (n=481)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0792` → IC=+0.213 (n=1181)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0792 (IC base=+0.180)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.197 (n=1746)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_regimen` < `1.1753` → IC=+0.169 (n=2680)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.1753 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` > `0.6234` → IC=+0.162 (n=2680)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5767` → IC=+0.173 (n=1284)
...
- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.180 (n=2504)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4159` → IC=+0.174 (n=565)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.4159 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.194 (n=579)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` > 6.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` > `0.4793` → IC=+0.208 (n=564)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4793 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.2059` → IC=+0.222 (n=412)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2059 (IC base=+0.167)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `7.439` → IC=+0.167 (n=536)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` < 7.439 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `0.6383` → IC=+0.181 (n=189)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` < 0.6383 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` > `1.0769` → IC=+0.182 (n=256)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` > 1.0769 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.2332` → IC=+0.200 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2332 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.4872` → IC=+0.206 (n=236)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4872 (IC base=+0.167)
...
- **PATRÓN** `libro_liquidez` > `11289.7522` → IC=+0.192 (n=504)
  ...
  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 11289.7522 (IC base=+0.167)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5245` → IC=+0.139 (n=651)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.69€ cuando `drift_60min` |x|≤ 0.5245 (IC base=+0.134)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.189 (n=307)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 15.0 (IC base=+0.134)

- **PATRÓN** `ibs_20min` < `0.4642` → IC=+0.229 (n=651)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4642 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` > `1.0399` → IC=+0.151 (n=81)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 1.0399 (IC base=+0.134)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.22` → IC=+0.176 (n=254)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 3.22 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` < `1.1973` → IC=+0.152 (n=651)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1973 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` > `0.8565` → IC=+0.151 (n=434)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.8565 (IC base=+0.134)
...
- **PATRÓN** `libro_liquidez` > `3176.6468` → IC=+0.176 (n=217)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 3176.6468 (IC base=+0.134)
...
- **PATRÓN** `hora_utc` > `9.0` → IC=+0.170 (n=714)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 9.0 (IC base=+0.138)

- **PATRÓN** `ibs_20min` > `0.4762` → IC=+0.169 (n=795)

  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` > 0.4762 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` > `0.9582` → IC=+0.227 (n=108)
...
- **PATRÓN** `volumen_regimen` < `0.8754` → IC=+0.161 (n=428)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.8754 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `1.1816` → IC=+0.157 (n=214)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 1.1816 (IC base=+0.138)

- **PATRÓN** `volumen_pendiente_norm` > `0.18` → IC=+0.173 (n=221)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.18 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` < `2.2754` → IC=+0.150 (n=652)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 2.2754 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` > `1.8612` → IC=+0.143 (n=494)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.8612 (IC base=+0.138)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.157 (n=595)

  - _Acción_: Kelly boost +0.78€ cuando `libro_spread` < 0.01 (IC base=+0.138)

- **PATRÓN** `libro_liquidez` > `7237.8784` → IC=+0.178 (n=265)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 7237.8784 (IC base=+0.138)

- **PATRÓN** `ballena_activa_n` < `12.0` → IC=+0.184 (n=194)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 12.0 (IC base=+0.138)

- **PATRÓN** `ibs_20min` < `0.3074` → IC=+0.136 (n=542)

  - _Acción_: Kelly boost +0.68€ cuando `ibs_20min` < 0.3074 (IC base=+0.070)

- **PATRÓN** `ballena_activa_n` < `20.0` → IC=+0.149 (n=297)

  - _Acción_: Kelly boost +0.74€ cuando `ballena_activa_n` < 20.0 (IC base=+0.070)
...
- **PATRÓN** `ibs_20min` < `0.6123` → IC=+0.146 (n=238)

  - _Acción_: Kelly boost +0.73€ cuando `ibs_20min` < 0.6123 (IC base=+0.064)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.021` → IC=+0.167 (n=52)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 9.021 (IC base=+0.064)
...
- **PATRÓN** `ballena_activa_n` < `146.0` → IC=+0.163 (n=81)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `ballena_activa_n` < 146.0 (IC base=+0.064)
...
- **PATRÓN** `dist_vwap_pct` < `0.4496` → IC=+0.174 (n=2973)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.4496 (IC base=+0.189)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.669` → IC=+0.212 (n=595)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.669 (IC base=+0.189)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.585` → IC=+0.190 (n=4151)
...
- **PATRÓN** `ballena_activa_n` < `140.0` → IC=+0.183 (n=3091)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.417` → IC=+0.174 (n=651)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` < 4.417 (IC base=+0.161)

- **PATRÓN** `volumen_regimen` < `1.2687` → IC=+0.167 (n=748)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0714` → IC=+0.175 (n=626)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4168` → IC=+0.179 (n=699)
...
- **PATRÓN** `ballena_activa_n` < `509.0` → IC=+0.172 (n=624)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.159 (n=685)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 8.0 (IC base=+0.144)

- **PATRÓN** `ibs_20min` > `0.74` → IC=+0.241 (n=503)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.74 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `0.3814` → IC=+0.189 (n=307)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.3814 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.371` → IC=+0.172 (n=345)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 4.371 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` < `0.9021` → IC=+0.171 (n=503)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 0.9021 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` > `1.2078` → IC=+0.158 (n=252)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 1.2078 (IC base=+0.144)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.146 (n=819)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.145 (n=375)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.205` → IC=+0.137 (n=753)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` < 3.205 (IC base=+0.117)

- **PATRÓN** `volumen_regimen` < `1.0655` → IC=+0.130 (n=671)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5612` → IC=+0.143 (n=270)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 1.5612 (IC base=+0.117)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.183 (n=1476)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=4540)
...
- **FILTRO** `py_entrada` > `0.61` → IC=-0.163 (n=250)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=757)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.134 (n=3521)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=10614)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.282 (n=3478)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=10657)

- **FILTRO** `ibs_7min` < `0.712` → IC=-0.246 (n=3533)

  - _Acción_: SKIP cuando `ibs_7min` < 0.712
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=10602)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.171 (n=4731)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=9404)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.220 (n=4378)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=13416)

- **FILTRO** `ibs_7min` > `0.7143` → IC=-0.172 (n=4434)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=13360)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.155 (n=604)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.100 (n=1455)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.316 (n=509)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=1550)
...
- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.228 (n=512)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.079 (n=1547)
...
- **FILTRO** `drift_7min_pct` |x|> `0.1142` → IC=-0.128 (n=1050)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=2044)

- **FILTRO** `ibs_7min` > `0.8333` → IC=-0.189 (n=773)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8333
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=2321)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=572)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=2069)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.259 (n=642)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=1999)
...
- **FILTRO** `ballena_activa_n` > `168.0` → IC=-0.179 (n=656)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=1985)
...
- **FILTRO** `ballena_activa_n` > `107.0` → IC=-0.175 (n=893)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=1757)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `9.0` → IC=-0.184 (n=656)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.097 (n=1434)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.312 (n=664)
...
- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.252 (n=493)
  ...
  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=1597)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.138 (n=717)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.092 (n=1680)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.259 (n=586)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=1811)

- **FILTRO** `ibs_7min` < `0.7509` → IC=-0.196 (n=599)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7509
  - _Potencial_: sin este filtro IC_bueno=-0.076 (n=1798)
...
- **FILTRO** `ibs_7min` > `0.2755` → IC=-0.180 (n=601)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2755
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=1807)

- **FILTRO** `ballena_activa_n` > `34.0` → IC=-0.183 (n=591)

  - _Acción_: SKIP cuando `ballena_activa_n` > 34.0
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=1817)
...
- **FILTRO** `ibs_7min` < `0.7353` → IC=-0.213 (n=659)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7353
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=1979)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.192 (n=625)

  - _Acción_: SKIP cuando `ballena_activa_n` > 13.0
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=2013)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.123 (n=739)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=1571)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.297 (n=565)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=1745)

- **FILTRO** `ibs_7min` < `0.7333` → IC=-0.250 (n=573)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7333
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=1737)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.254 (n=611)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=2498)

- **FILTRO** `ibs_7min` > `0.7752` → IC=-0.161 (n=777)
...
- **FILTRO** `sigma_h` > `0.0108` → IC=-0.205 (n=334)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `sigma_h` > `0.0075` → IC=-0.239 (n=224)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.190 (n=675)
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3696 sube el IC de -0.064 a +0.269 en UPDOWN_GBM_15M_TARDIO (n=440). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 800 | +0.087 | +45.42€ | 2 | 10 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 800 | +0.087 | +45.42€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 23 | +0.060 | +0.96€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 23 | +0.060 | +0.96€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 522 | +0.107 | +37.89€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 522 | +0.107 | +37.89€ | 2 | 11 |
...
| ✅ BALLENAS_TARDIAS | 14727 | -0.111 | -2490.70€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 954 | -0.009 | -140.48€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 13773 | -0.118 | -2350.23€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1912 | -0.089 | -416.47€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1912 | -0.089 | -416.47€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 954 | -0.009 | -140.48€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 954 | -0.009 | -140.48€ | 2 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 1957 | -0.167 | -535.28€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 1957 | -0.167 | -535.28€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 3940 | -0.049 | -362.57€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 3940 | -0.049 | -362.57€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 3271 | -0.128 | -288.23€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 3271 | -0.128 | -288.23€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 2693 | -0.194 | -747.68€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 2693 | -0.194 | -747.68€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 4029 | -0.072 | +1813.87€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 1147 | -0.009 | +853.92€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 2882 | -0.096 | +959.95€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 4029 | -0.072 | +1813.87€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 1147 | -0.009 | +853.92€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 2882 | -0.096 | +959.95€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 47472 | +0.113 | -3014.58€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 7943 | +0.184 | -278.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 160 | -0.111 | -53.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 35632 | +0.098 | -2609.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3737 | +0.117 | -73.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 5985 | +0.081 | -809.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 26 | -0.107 | +5.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 14 | -0.219 | -10.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 5945 | +0.083 | -804.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 9519 | +0.132 | -231.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 2337 | +0.197 | -114.76€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 5884 | +0.109 | -144.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1256 | +0.126 | +49.74€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 6001 | +0.081 | -766.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 28 | +0.033 | +3.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#240min | 9 | -0.143 | -7.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 5964 | +0.082 | -763.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 10272 | +0.126 | -161.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2960 | +0.172 | -14.22€ | 1 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 5923 | +0.112 | -97.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1377 | +0.095 | -40.42€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 9711 | +0.125 | -629.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2572 | +0.192 | -160.03€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 78 | -0.013 | -3.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 5957 | +0.097 | -383.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1104 | +0.131 | -82.41€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 5984 | +0.105 | -416.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 20 | +0.000 | +1.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 5 | -0.018 | -1.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 5959 | +0.105 | -416.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 7901 | +0.177 | -617.59€ | 2 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 7901 | +0.177 | -617.59€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 1970 | +0.169 | -209.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 1970 | +0.169 | -209.70€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 1765 | +0.237 | -38.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 1765 | +0.237 | -38.43€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 25304 | +0.188 | -2401.62€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 25304 | +0.188 | -2401.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 4509 | +0.146 | -726.81€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 4509 | +0.146 | -726.81€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 3948 | +0.226 | -138.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 3948 | +0.226 | -138.64€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 4381 | +0.161 | -603.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 4381 | +0.161 | -603.59€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 4019 | +0.220 | -162.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 4019 | +0.220 | -162.39€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 