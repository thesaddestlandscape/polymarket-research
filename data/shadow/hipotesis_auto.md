# Hipótesis automáticas — 2026-07-03 14:11 UTC
_Generado por shadow_postmortem.py sobre 3082 resoluciones (PNL=-36.73€)_

## Patrones causales activos

### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` < `0.0097` → IC=+0.204 (n=79)
...
- **PATRÓN** `ibs_15` < `0.0613` → IC=+0.161 (n=57)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `ibs_15` < 0.0613 (IC base=+0.036)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1297` → IC=+0.145 (n=60)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.73€ cuando `pct_spot_vs_ref` |x|≤ 0.1297 (IC base=+0.105)

- **PATRÓN** `sigma_h` < `0.0087` → IC=+0.167 (n=40)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0087 (IC base=+0.105)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.152 (n=21)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 6.0 (IC base=+0.105)

- **PATRÓN** `ibs_15` < `0.0787` → IC=+0.258 (n=31)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0787 (IC base=+0.105)
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.0787 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.258 n=31). Confirma señal de reversión media → alinear con BUY_YES.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 424 | +0.056 | +59.93€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 424 | +0.056 | +59.93€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 109 | +0.013 | +3.65€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 109 | +0.013 | +3.65€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 110 | -0.018 | -0.16€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 110 | -0.018 | -0.16€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL | 106 | +0.120 | +31.04€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 106 | +0.120 | +31.04€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP | 99 | +0.114 | +25.40€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 99 | +0.114 | +25.40€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 859 | -0.041 | -51.80€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 629 | -0.020 | -23.95€ | 0 | 3 |
...
| ✅ UPDOWN_GBM#ETH | 338 | -0.012 | +6.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 254 | +0.004 | +12.84€ | 3 | 4 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.061) — sin ventaja clara. oversold(IBS<0.3): IC=-0.009 n=163 | neutral: IC=-0.070 n=140 | overbought(IBS>0.7): IC=-0.046 n=150
  - _Datos_: n=469 IC=-0.041 PNL=-26.56€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.009 n=1480 | tras_1loss IC=-0.027 n=1511 | tras_2loss IC=-0.042 n=776/40 | gap=+0.051 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=736 IC=-0.037 PNL=-38.24€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=736 IC=-0.037 PNL=-38.24€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=270 IC=-0.018 PNL=-34.35€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=270 IC=-0.018 PNL=-34.35€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=219 IC=+0.011 PNL=+11.35€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=219 IC=+0.011 PNL=+11.35€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=656 IC=+0.036 PNL=+53.44€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=656 IC=+0.036 PNL=+53.44€
