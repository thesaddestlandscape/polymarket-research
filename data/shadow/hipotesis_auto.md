# Hipótesis automáticas — 2026-08-28 07:46 UTC
_Generado por shadow_postmortem.py sobre 185158 resoluciones (PNL=+13467.45€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.37` → IC=-0.288 (n=2580)
...
- **FILTRO** `restante_s_al_confirmar` < `161.22` → IC=-0.170 (n=697)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.159 (n=4639)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 7.0 (IC base=+0.143)

- **PATRÓN** `py_entrada` < `0.34` → IC=+0.260 (n=3554)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.34 (IC base=+0.143)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.189 (n=3054)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.02 (IC base=+0.143)

- **PATRÓN** `libro_liquidez` > `1893.3172` → IC=+0.181 (n=2560)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 1893.3172 (IC base=+0.143)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.280 (n=499)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.277 (n=509)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.278)

- **PATRÓN** `libro_liquidez` > `2911.6657` → IC=+0.280 (n=316)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2911.6657 (IC base=+0.278)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.222 (n=243)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `1894.2457` → IC=+0.238 (n=292)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1894.2457 (IC base=+0.223)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.194 (n=13111)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9504` → IC=+0.209 (n=805)
...
- **PATRÓN** `dist_vwap_pct` < `0.4965` → IC=+0.210 (n=460)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4965 (IC base=+0.066)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.233` → IC=+0.160 (n=971)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4423` → IC=+0.151 (n=1162)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 2.4423 (IC base=+0.066)

- **PATRÓN** `ibs_20min` < `0.2131` → IC=+0.128 (n=1629)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.273 (n=205)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5756` → IC=+0.272 (n=178)
...
- **PATRÓN** `sigma_h` < `0.0046` → IC=+0.228 (n=274)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.0927` → IC=+0.239 (n=136)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0927 (IC base=+0.206)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.235 (n=409)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.206)

- **PATRÓN** `ibs_20min` > `0.4244` → IC=+0.222 (n=408)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4244 (IC base=+0.206)

- **PATRÓN** `dist_vwap_pct` > `0.2185` → IC=+0.230 (n=268)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2185 (IC base=+0.206)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.332` → IC=+0.224 (n=212)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.332 (IC base=+0.206)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.071` → IC=+0.207 (n=397)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 7.071 (IC base=+0.206)

- **PATRÓN** `volumen_regimen` < `1.2629` → IC=+0.215 (n=408)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2629 (IC base=+0.206)

- **PATRÓN** `volumen_regimen` > `1.0844` → IC=+0.233 (n=185)
...
- **PATRÓN** `libro_liquidez` > `11533.5682` → IC=+0.219 (n=364)
...
- **PATRÓN** `ibs_20min` < `0.5294` → IC=+0.294 (n=363)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.576` → IC=+0.267 (n=397)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6195` → IC=+0.247 (n=160)
...
- **PATRÓN** `ballena_activa_n` < `51.0` → IC=+0.245 (n=163)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.161 (n=113)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.004 (n=367)
...
- **FILTRO** `sigma_h` > `0.0104` → IC=-0.138 (n=338)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=1017)

- **FILTRO** `sigma_ewma_delta_pct` > `4.968` → IC=-0.153 (n=286)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.182 (n=146)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.077 (n=561)
...
- **PATRÓN** `ibs_20min` > `0.9465` → IC=+0.246 (n=1009)
...
- **PATRÓN** `ibs_20min` < `0.0939` → IC=+0.183 (n=1257)
...
- **PATRÓN** `dist_vwap_pct` < `0.1497` → IC=+0.207 (n=742)
...
- **FILTRO** `ibs_20min` < `0.2606` → IC=-0.157 (n=202)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.074 (n=411)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.238` → IC=+0.167 (n=118)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 5.238 (IC base=-0.002)
...
- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.341 (n=237)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.3388` → IC=+0.349 (n=269)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.484` → IC=+0.347 (n=298)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.484 (IC base=+0.336)

- **PATRÓN** `volumen_pendiente_norm` > `0.3592` → IC=+0.429 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3592 (IC base=+0.336)

- **PATRÓN** `volumen_spike_ratio` < `1.732` → IC=+0.336 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.732 (IC base=+0.336)

- **PATRÓN** `volumen_spike_ratio` > `1.9567` → IC=+0.344 (n=158)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2849` → IC=+0.277 (n=402)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2849 (IC base=+0.270)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.279 (n=558)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3675` → IC=+0.309 (n=604)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3675 (IC base=+0.270)

- **PATRÓN** `dist_vwap_pct` < `0.2661` → IC=+0.281 (n=574)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3387` → IC=+0.158 (n=2310)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.356` → IC=+0.245 (n=1095)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.356 (IC base=+0.156)

- **PATRÓN** `volumen_regimen` > `0.6876` → IC=+0.171 (n=1627)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 0.6876 (IC base=+0.156)

- **PATRÓN** `volumen_pendiente_norm` > `0.2364` → IC=+0.187 (n=497)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.2364 (IC base=+0.156)

- **PATRÓN** `volumen_spike_ratio` < `2.2822` → IC=+0.157 (n=2088)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.2822 (IC base=+0.156)

- **PATRÓN** `volumen_spike_ratio` > `1.8507` → IC=+0.149 (n=1582)
...
- **PATRÓN** `libro_liquidez` > `3204.5735` → IC=+0.181 (n=1190)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 3204.5735 (IC base=+0.156)

- **PATRÓN** `ballena_activa_n` < `114.0` → IC=+0.179 (n=1349)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 114.0 (IC base=+0.156)

- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.191 (n=2346)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0092 (IC base=+0.184)

- **PATRÓN** `drift_60min` |x|≤ `0.422` → IC=+0.195 (n=2345)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4107` → IC=+0.237 (n=2345)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4107 (IC base=+0.184)

- **PATRÓN** `dist_vwap_pct` < `0.3697` → IC=+0.176 (n=2024)
...
- **PATRÓN** `volumen_regimen` < `1.1724` → IC=+0.167 (n=1870)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.1724 (IC base=+0.184)

- **PATRÓN** `volumen_regimen` > `0.6221` → IC=+0.168 (n=1869)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6536` → IC=+0.209 (n=579)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6536 (IC base=+0.184)

- **PATRÓN** `ballena_activa_n` < `127.0` → IC=+0.192 (n=1150)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2479` → IC=+0.303 (n=191)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2479 (IC base=+0.255)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.535` → IC=+0.280 (n=243)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.311` → IC=+0.241 (n=25)
...
- **PATRÓN** `libro_liquidez` > `1962.999` → IC=+0.338 (n=72)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1962.999 (IC base=+0.255)

- **PATRÓN** `ballena_activa_n` < `66.0` → IC=+0.255 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 66.0 (IC base=+0.255)

### GBM_LATE_15M_MULTIHORIZONTE#BTC#15min
- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.198 (n=332)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.3375` → IC=+0.176 (n=331)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.3375 (IC base=+0.174)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.202 (n=377)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_regimen` < `1.2894` → IC=+0.180 (n=376)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` < 1.2894 (IC base=+0.174)

- **PATRÓN** `volumen_regimen` > `1.0855` → IC=+0.194 (n=171)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 1.0855 (IC base=+0.174)

- **PATRÓN** `volumen_pendiente_norm` > `0.2178` → IC=+0.244 (n=76)
...
- **PATRÓN** `libro_liquidez` > `12448.5931` → IC=+0.188 (n=251)
  ...
  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 12448.5931 (IC base=+0.174)
...
- **PATRÓN** `sigma_h` < `0.0098` → IC=+0.286 (n=260)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.3077` → IC=+0.318 (n=229)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3077 (IC base=+0.267)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.973` → IC=+0.329 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.973 (IC base=+0.267)
...
- **PATRÓN** `volumen_spike_ratio` > `1.671` → IC=+0.261 (n=186)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.671 (IC base=+0.267)

- **PATRÓN** `ballena_activa_n` < `60.0` → IC=+0.229 (n=142)
...
- **PATRÓN** `sigma_h` > `0.0104` → IC=+0.201 (n=142)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` < `0.4098` → IC=+0.232 (n=427)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4098 (IC base=+0.132)

- **PATRÓN** `dist_vwap_pct` > `0.6893` → IC=+0.144 (n=99)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.6893 (IC base=+0.132)

- **PATRÓN** `dist_vwap_pct` < `0.2206` → IC=+0.147 (n=398)
...
- **PATRÓN** `volumen_regimen` < `1.1404` → IC=+0.143 (n=427)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.1404 (IC base=+0.132)

- **PATRÓN** `volumen_regimen` > `0.8419` → IC=+0.161 (n=284)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2667` → IC=+0.223 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2667 (IC base=+0.132)

- **PATRÓN** `libro_liquidez` > `2097.9237` → IC=+0.189 (n=284)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 2097.9237 (IC base=+0.132)
...
- **PATRÓN** `drift_60min` |x|≤ `0.6625` → IC=+0.235 (n=527)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.253 (n=257)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.2471` → IC=+0.232 (n=554)
...
- **PATRÓN** `volumen_regimen` > `0.6963` → IC=+0.244 (n=471)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6963 (IC base=+0.220)
...
- **PATRÓN** `volumen_spike_ratio` > `2.7449` → IC=+0.270 (n=133)
...
- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.151 (n=150)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` < 0.0038 (IC base=+0.063)

- **PATRÓN** `ibs_20min` < `0.298` → IC=+0.145 (n=297)

  - _Acción_: Kelly boost +0.73€ cuando `ibs_20min` < 0.298 (IC base=+0.063)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.772` → IC=+0.164 (n=102)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 7.772 (IC base=+0.063)
...
- **PATRÓN** `ibs_20min` < `0.0588` → IC=+0.250 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0588 (IC base=+0.034)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.192 (n=1386)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0082 (IC base=+0.149)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.158 (n=3078)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 6.0 (IC base=+0.149)

- **PATRÓN** `ibs_20min` > `0.9399` → IC=+0.288 (n=1386)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9399 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` > `0.7721` → IC=+0.232 (n=646)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7721 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.349` → IC=+0.246 (n=1260)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.349 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` < `0.6965` → IC=+0.152 (n=951)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.6965 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` > `1.0761` → IC=+0.149 (n=980)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` > 1.0761 (IC base=+0.149)

- **PATRÓN** `volumen_pendiente_norm` > `0.165` → IC=+0.178 (n=791)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.165 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` < `2.304` → IC=+0.149 (n=2408)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 2.304 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` > `1.8616` → IC=+0.158 (n=1824)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.8616 (IC base=+0.149)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.154 (n=2471)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `2493.5758` → IC=+0.174 (n=2037)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 2493.5758 (IC base=+0.149)

- **PATRÓN** `ballena_activa_n` < `147.0` → IC=+0.190 (n=1480)

  - _Acción_: Kelly boost +0.95€ cuando `ballena_activa_n` < 147.0 (IC base=+0.149)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.219 (n=1843)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0066 (IC base=+0.190)

- **PATRÓN** `drift_60min` |x|≤ `0.4749` → IC=+0.201 (n=2761)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5632` → IC=+0.244 (n=2761)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5632 (IC base=+0.190)

- **PATRÓN** `dist_vwap_pct` < `0.7324` → IC=+0.179 (n=2207)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.638` → IC=+0.194 (n=2591)
...
- **PATRÓN** `volumen_regimen` > `1.2013` → IC=+0.182 (n=683)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` > 1.2013 (IC base=+0.190)

- **PATRÓN** `volumen_pendiente_norm` > `0.2853` → IC=+0.262 (n=296)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2853 (IC base=+0.190)

- **PATRÓN** `volumen_spike_ratio` > `2.3025` → IC=+0.209 (n=935)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3025 (IC base=+0.190)

- **PATRÓN** `ballena_activa_n` < `176.0` → IC=+0.169 (n=1633)
...
- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.302 (n=210)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `volumen_pendiente_norm` > `0.299` → IC=+0.300 (n=33)
...
- **PATRÓN** `ballena_activa_n` < `71.0` → IC=+0.248 (n=133)
...
- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.173 (n=163)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0071 (IC base=+0.151)

- **PATRÓN** `drift_60min` |x|≤ `0.4191` → IC=+0.152 (n=490)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.4191 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.172 (n=492)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 6.0 (IC base=+0.151)

- **PATRÓN** `ibs_20min` > `0.3271` → IC=+0.205 (n=489)
...
- **PATRÓN** `volumen_regimen` > `1.0896` → IC=+0.170 (n=222)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 1.0896 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` < `0.0734` → IC=+0.167 (n=391)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` < 0.0734 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` > `0.2053` → IC=+0.214 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2053 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` < `2.0651` → IC=+0.177 (n=388)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 2.0651 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` > `1.3655` → IC=+0.175 (n=441)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 1.3655 (IC base=+0.151)

- **PATRÓN** `libro_liquidez` > `12055.0066` → IC=+0.168 (n=326)
...
- **PATRÓN** `ibs_20min` < `0.5588` → IC=+0.309 (n=397)
...
- **PATRÓN** `volumen_spike_ratio` > `2.408` → IC=+0.219 (n=183)
...
- **PATRÓN** `ballena_activa_n` < `57.0` → IC=+0.206 (n=168)
...
- **PATRÓN** `dist_vwap_pct` < `1.2245` → IC=+0.223 (n=797)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 1.2245 (IC base=+0.216)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1835` → IC=+0.228 (n=233)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1835 (IC base=+0.216)

- **PATRÓN** `ballena_activa_n` < `37.0` → IC=+0.182 (n=404)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.148 (n=1097)
...
- **PATRÓN** `sigma_h` < `0.0037` → IC=+0.192 (n=280)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0037 (IC base=+0.143)

- **PATRÓN** `drift_60min` |x|≤ `0.3661` → IC=+0.169 (n=734)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.1903` → IC=+0.153 (n=367)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.1903 (IC base=+0.143)

- **PATRÓN** `ibs_20min` > `0.0862` → IC=+0.149 (n=834)
...
- **PATRÓN** `dist_vwap_pct` < `0.4157` → IC=+0.137 (n=815)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.173 (n=898)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.024 (n=2742)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.217 (n=857)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=2790)

- **FILTRO** `ibs_20min` > `0.2723` → IC=-0.181 (n=911)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2723
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=2736)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.134 (n=173)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
...
- **FILTRO** `ibs_20min` < `0.7271` → IC=-0.193 (n=138)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=416)
...
- **FILTRO** `ibs_20min` > `0.7354` → IC=-0.216 (n=153)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.014 (n=461)
...
- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.171 (n=138)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=439)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.142 (n=2708)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=6393)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.282 (n=2212)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=6889)

- **FILTRO** `ibs_7min` < `0.731` → IC=-0.233 (n=2275)

  - _Acción_: SKIP cuando `ibs_7min` < 0.731
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=6826)

- **FILTRO** `ballena_activa_n` > `20.0` → IC=-0.171 (n=3019)

  - _Acción_: SKIP cuando `ballena_activa_n` > 20.0
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=