# Hipótesis automáticas — 2026-07-12 12:22 UTC
_Generado por shadow_postmortem.py sobre 10901 resoluciones (PNL=+1939.20€)_
...
- **PATRÓN** `py_entrada` < `0.275` → IC=+0.271 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.275 (IC base=+0.201)

- **PATRÓN** `libro_liquidez` > `1648.9469` → IC=+0.227 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1648.9469 (IC base=+0.201)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.121 (n=1327)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 6.0 (IC base=+0.115)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.120 (n=1323)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` > `0.0092` → IC=+0.148 (n=916)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.505` → IC=+0.172 (n=120)
...
- **PATRÓN** `sigma_h` < `0.0162` → IC=+0.136 (n=355)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0162 (IC base=+0.120)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.163 (n=161)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 16.0 (IC base=+0.120)
...
- **PATRÓN** `dist_vwap_pct` < `0.1403` → IC=+0.150 (n=18)
  ...
  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.1403 (IC base=+0.120)
...
- **PATRÓN** `sigma_h` < `0.032` → IC=+0.181 (n=302)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.032 (IC base=+0.166)

- **PATRÓN** `sigma_h` > `0.0142` → IC=+0.180 (n=270)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0142 (IC base=+0.166)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.169 (n=306)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.1277` → IC=+0.278 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1277 (IC base=+0.166)
...
- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.187 (n=311)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.1427` → IC=+0.214 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1427 (IC base=+0.170)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.442` → IC=+0.324 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.442 (IC base=+0.170)

- **PATRÓN** `sigma_h` > `0.0045` → IC=+0.171 (n=354)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.2052` → IC=+0.268 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2052 (IC base=+0.152)
...
- **PATRÓN** `sigma_h` < `0.0055` → IC=+0.182 (n=86)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0055 (IC base=+0.172)

- **PATRÓN** `sigma_h` > `0.0024` → IC=+0.227 (n=86)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0024 (IC base=+0.172)
...
- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.190 (n=56)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0113 (IC base=+0.123)

- **PATRÓN** `hora_utc` < `9.0` → IC=+0.151 (n=41)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 9.0 (IC base=+0.123)
...
- **PATRÓN** `sigma_h` < `0.0214` → IC=+0.318 (n=64)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0214 (IC base=+0.267)

- **PATRÓN** `hora_utc` < `13.0` → IC=+0.312 (n=46)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **FILTRO** `sigma_ewma_delta_pct` < `6.483` → IC=-0.222 (n=16)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.324 (n=15)

- **PATRÓN** `sigma_h` > `0.0135` → IC=+0.129 (n=141)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` > 0.0135 (IC base=+0.075)

- **PATRÓN** `dist_vwap_pct` < `0.1011` → IC=+0.138 (n=67)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.1011 (IC base=+0.075)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.483` → IC=+0.324 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.483 (IC base=+0.075)

- **PATRÓN** `sigma_h` > `0.0127` → IC=+0.147 (n=137)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0127 (IC base=+0.082)

- **PATRÓN** `dist_vwap_pct` < `0.1536` → IC=+0.179 (n=76)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.1536 (IC base=+0.082)
...
- **PATRÓN** `sigma_h` < `0.0333` → IC=+0.210 (n=105)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0333 (IC base=+0.188)

- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.201 (n=105)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0113 (IC base=+0.188)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 882 | +0.187 | +17.70€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 566 | +0.213 | +11.06€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 308 | +0.190 | +6.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 188 | +0.210 | +7.04€ | 0 | 8 |
...
| ✅ GBM_LATE_15M | 3574 | +0.112 | +1156.51€ | 0 | 7 |
| ✅ GBM_LATE_15M#15min | 3574 | +0.112 | +1156.51€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 881 | +0.070 | +114.52€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 881 | +0.070 | +114.52€ | 0 | 1 |
| ✅ GBM_LATE_15M#ETH | 906 | +0.072 | +135.86€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 906 | +0.072 | +135.86€ | 0 | 2 |
| ✅ GBM_LATE_15M#SOL | 930 | +0.136 | +449.08€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 930 | +0.136 | +449.08€ | 0 | 8 |
| ✅ GBM_LATE_15M#XRP | 857 | +0.173 | +457.05€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 857 | +0.173 | +457.05€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 936 | +0.161 | +491.46€ | 0 | 7 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 936 | +0.161 | +491.46€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 246 | +0.169 | +162.50€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 246 | +0.169 | +162.50€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 252 | +0.122 | +81.20€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 252 | +0.122 | +81.20€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 263 | +0.111 | +74.09€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 263 | +0.111 | +74.09€ | 0 | 6 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 175 | +0.274 | +173.66€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 175 | +0.274 | +173.66€ | 0 | 5 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 1107 | +0.079 | +258.33€ | 1 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1107 | +0.079 | +258.33€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 257 | +0.075 | +33.49€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 257 | +0.075 | +33.49€ | 0 | 1 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 276 | +0.000 | +1.81€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 276 | +0.000 | +1.81€ | 0 | 1 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 288 | +0.035 | +31.50€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 288 | +0.035 | +31.50€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 286 | +0.201 | +191.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 286 | +0.201 | +191.53€ | 0 | 7 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.090 n=6146 | tras_1loss IC=+0.044 n=4618 | tras_2loss IC=+0.010 n=2077/40 | gap=+0.080 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8443 IC=+0.096 PNL=+2015.08€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8443 IC=+0.096 PNL=+2015.08€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=451 IC=+0.054 PNL=+36.19€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=451 IC=+0.054 PNL=+36.19€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.303 > 0.2 con n=135 PNL=+90.34€
  - _Datos_: n=135 IC=+0.303 PNL=+90.34€
...
**⏳ H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: 6/40 ops en el filtro definido (IC actual=+0.037 PNL=+8.45€)
  - _Datos_: n=6 IC=+0.037 PNL=+8.45€
