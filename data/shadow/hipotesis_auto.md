# Hipótesis automáticas — 2026-08-24 19:28 UTC
_Generado por shadow_postmortem.py sobre 141789 resoluciones (PNL=+9714.28€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `154.55` → IC=-0.259 (n=1983)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 154.55
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=5951)
...
- **FILTRO** `restante_s_al_confirmar` < `163.44` → IC=-0.160 (n=530)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 163.44
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=1592)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.146 (n=2971)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.198 (n=8971)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 8.0 (IC base=+0.193)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.224 (n=7087)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.193)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=1950)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.165 (n=1328)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.172 (n=651)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.230 (n=599)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `restante_min` > `4.92` → IC=+0.156 (n=1281)
...
- **PATRÓN** `lag_apertura_s` < `6.73` → IC=+0.151 (n=1585)

  - _Acción_: Kelly boost +0.75€ cuando `lag_apertura_s` < 6.73 (IC base=+0.130)
...
- **PATRÓN** `restante_min` > `4.9` → IC=+0.169 (n=603)
...
- **PATRÓN** `lag_apertura_s` < `6.15` → IC=+0.168 (n=597)

  - _Acción_: Kelly boost +0.84€ cuando `lag_apertura_s` < 6.15 (IC base=+0.139)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.150 (n=569)
...
- **PATRÓN** `sigma_h` < `0.0033` → IC=+0.213 (n=193)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0033 (IC base=+0.140)

- **PATRÓN** `drift_60min` |x|≤ `0.3378` → IC=+0.162 (n=386)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.3378 (IC base=+0.140)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.178 (n=147)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.612` → IC=+0.162 (n=386)

  - _Acción_: Kelly boost +0.81€ cuando `ibs_20min` < 0.612 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` > `0.7556` → IC=+0.140 (n=123)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.7556 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` < `0.2251` → IC=+0.152 (n=374)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.2251 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.522` → IC=+0.183 (n=206)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 3.522 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `1.3112` → IC=+0.164 (n=439)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.3112 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` > `0.6452` → IC=+0.141 (n=438)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 0.6452 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` < `0.0976` → IC=+0.142 (n=383)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_pendiente_norm` < 0.0976 (IC base=+0.140)

- **PATRÓN** `volumen_pendiente_norm` > `0.0766` → IC=+0.158 (n=220)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_pendiente_norm` > 0.0766 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` < `2.6123` → IC=+0.167 (n=434)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 2.6123 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.4336` → IC=+0.144 (n=434)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.4336 (IC base=+0.140)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.150 (n=569)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.01 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `7614.1842` → IC=+0.159 (n=438)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 7614.1842 (IC base=+0.140)
...
- **PATRÓN** `sigma_h` < `0.0084` → IC=+0.219 (n=126)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0084 (IC base=+0.177)

- **PATRÓN** `drift_60min` |x|≤ `0.3192` → IC=+0.235 (n=96)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3192 (IC base=+0.177)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.196 (n=54)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 16.0 (IC base=+0.177)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.220 (n=98)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 10.0 (IC base=+0.177)

- **PATRÓN** `ibs_20min` < `0.6855` → IC=+0.176 (n=143)

  - _Acción_: Kelly boost +0.88€ cuando `ibs_20min` < 0.6855 (IC base=+0.177)

- **PATRÓN** `ibs_20min` > `0.1532` → IC=+0.231 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.1532 (IC base=+0.177)

- **PATRÓN** `dist_vwap_pct` < `0.8894` → IC=+0.198 (n=147)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.8894 (IC base=+0.177)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.54` → IC=+0.296 (n=47)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.54 (IC base=+0.177)

- **PATRÓN** `volumen_regimen` < `1.1151` → IC=+0.242 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.1151 (IC base=+0.177)

- **PATRÓN** `volumen_pendiente_norm` < `0.1261` → IC=+0.197 (n=143)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` < 0.1261 (IC base=+0.177)

- **PATRÓN** `volumen_spike_ratio` < `2.2019` → IC=+0.244 (n=123)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.2019 (IC base=+0.177)

- **PATRÓN** `volumen_spike_ratio` > `1.4925` → IC=+0.185 (n=125)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 1.4925 (IC base=+0.177)

- **PATRÓN** `libro_liquidez` > `9140.0556` → IC=+0.211 (n=95)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 9140.0556 (IC base=+0.177)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.137 (n=1750)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=4410)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.279 (n=1475)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=4685)

- **FILTRO** `ibs_7min` < `0.7391` → IC=-0.230 (n=1538)
...
- **FILTRO** `hora_utc` > `17.0` → IC=-0.132 (n=275)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 17.0
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.267 (n=290)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.004 (n=1017)
...
- **FILTRO** `ibs_7min` < `0.7774` → IC=-0.202 (n=260)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7774
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=781)

- **FILTRO** `ballena_activa_n` > `43.0` → IC=-0.219 (n=254)

  - _Acción_: SKIP cuando `ballena_activa_n` > 43.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=787)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.174 (n=335)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=1026)
...
| ✅ BALLENAS_TARDIAS | 5991 | -0.098 | -712.07€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 632 | -0.080 | -104.46€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 5359 | -0.100 | -607.61€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 896 | -0.004 | -101.03€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 896 | -0.004 | -101.03€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#DOGE | 3131 | +0.074 | -441.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 18 | +0.045 | -1.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 3112 | +0.075 | -437.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 6068 | +0.129 | -60.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1891 | +0.171 | +0.75€ | 0 | 8 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 3097 | +0.113 | -40.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1068 | +0.106 | -12.10€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 5827 | +0.129 | -424.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1797 | +0.203 | -92.58€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 45 | +0.011 | -8.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 3088 | +0.086 | -254.89€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 13351 | +0.193 | -1161.00€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 13351 | +0.193 | -1161.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 2470 | +0.126 | -455.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 2470 | +0.126 | -455.01€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 2319 | +0.160 | -310.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 2319 | +0.160 | -310.27€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 2141 | +0.222 | -92.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 2141 | +0.222 | -92.73€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 4800 | +0.130 | +120.20€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 4800 | +0.130 | +120.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 2385 | +0.139 | +96.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 2385 | +0.139 | +96.83€ | 0 | 5 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 4303 | +0.097 | -163.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 443 | +0.039 | -36.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 3860 | +0.104 | -127.22€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL | 1332 | +0.102 | -73.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL#5min | 1332 | +0.102 | -73.83€ | 0 | 0 |
...
| ✅ GBM_LATE_5M | 937 | +0.110 | +364.26€ | 1 | 19 |
| ✅ GBM_LATE_5M#5min | 937 | +0.110 | +364.26€ | 0 | 0 |
...
| ✅ GBM_LATE_5M#ETH | 355 | +0.133 | +132.53€ | 0 | 0 |
| ✅ GBM_LATE_5M#ETH#5min | 355 | +0.133 | +132.53€ | 0 | 17 |
...
| ✅ LIQUIDACIONES_5M | 514 | -0.054 | -29.70€ | 3 | 0 |
| ✅ LIQUIDACIONES_5M#5min | 514 | -0.054 | -29.70€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_5M | 2623 | +0.004 | -2.51€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#5min | 2623 | +0.004 | -2.51€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_5M#SOL | 1215 | +0.003 | -5.22€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#SOL#5min | 1215 | +0.003 | -5.22€ | 1 | 0 |
| ✅ MOMENTUM_IBS_5M#XRP | 178 | -0.006 | -5.46€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#XRP#5min | 178 | -0.006 | -5.46€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA | 13696 | -0.070 | +333.12€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 13696 | -0.070 | +333.12€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 2095 | -0.090 | +239.05€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 2095 | -0.090 | +239.05€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 2570 | -0.051 | +43.46€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 2570 | -0.051 | +43.46€ | 3 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 2163 | -0.083 | +34.21€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 2163 | -0.083 | +34.21€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 2091 | -0.097 | -170.64€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 2091 | -0.097 | -170.64€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 2497 | -0.046 | +37.06€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 2497 | -0.046 | +37.06€ | 4 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.090) — sin ventaja clara. oversold(IBS<0.3): IC=-0.004 n=1673 | neutral: IC=+0.006 n=1795 | overbought(IBS>0.7): IC=+0.085 n=1965
  - _Datos_: n=5702 IC=+0.030 PNL=+386.54€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.078 n=79719 | tras_1loss IC=+0.045 n=61769 | tras_2loss IC=+0.009 n=28069/40 | gap=+0.069 (umbral 0.05)
...
**⏳ H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  ...
  - _Estado_: 4824 ops, 22 horas distintas. Sin hora con n≥15 y IC extremo aún.
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=4645 IC=+0.004 PNL=+80.45€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=4645 IC=+0.004 PNL=+80.45€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=1729 IC=-0.018 PNL=-48.81€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1729 IC=-0.018 PNL=-48.81€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=3409 IC=-0.122 PNL=+379.78€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=3409 IC=-0.122 PNL=+379.78€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=2470 IC=+0.126 PNL=-455.01€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=2470 IC=+0.126 PNL=-455.01€
