# Hipótesis automáticas — 2026-09-03 21:22 UTC
_Generado por shadow_postmortem.py sobre 271457 resoluciones (PNL=+26456.15€)_
...
- **FILTRO** `restante_s_al_confirmar` < `146.53` → IC=-0.297 (n=3662)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=10987)
...
- **FILTRO** `restante_s_al_confirmar` < `156.16` → IC=-0.378 (n=885)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.102 (n=1797)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.188 (n=7956)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.147 (n=5503)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.198 (n=826)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.145 (n=1625)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.196 (n=6111)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.202 (n=2917)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.174 (n=1478)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.179 (n=1055)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.196 (n=1505)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.194 (n=7271)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.206 (n=17275)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.71 (IC base=+0.188)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.150 (n=3549)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.175 (n=2535)

  - _Acción_: Kelly boost +0.87€ cuando `py_entrada` > 0.72 (IC base=+0.146)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.235 (n=1115)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.260 (n=2199)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.232 (n=1518)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.268 (n=1084)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.130 (n=3461)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3571` → IC=+0.128 (n=3629)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` > 0.3571 (IC base=+0.081)

- **PATRÓN** `dist_vwap_pct` > `0.2678` → IC=+0.224 (n=466)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2678 (IC base=+0.081)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.562` → IC=+0.152 (n=1385)
...
- **PATRÓN** `volumen_spike_ratio` < `2.8852` → IC=+0.175 (n=2164)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 2.8852 (IC base=+0.081)

- **PATRÓN** `volumen_spike_ratio` > `1.4754` → IC=+0.171 (n=2163)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.4754 (IC base=+0.081)

- **PATRÓN** `ballena_activa_n` < `53.0` → IC=+0.200 (n=1068)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 53.0 (IC base=+0.081)

- **PATRÓN** `ibs_20min` < `0.4059` → IC=+0.127 (n=3493)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` < 0.4059 (IC base=+0.041)

- **PATRÓN** `dist_vwap_pct` < `0.3391` → IC=+0.144 (n=1266)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.3391 (IC base=+0.041)

- **PATRÓN** `volumen_regimen` < `0.6858` → IC=+0.154 (n=539)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.6858 (IC base=+0.041)

- **PATRÓN** `volumen_pendiente_norm` > `0.3044` → IC=+0.253 (n=273)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3044 (IC base=+0.041)

- **PATRÓN** `volumen_spike_ratio` > `2.8613` → IC=+0.216 (n=611)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8613 (IC base=+0.041)

- **PATRÓN** `ballena_activa_n` < `125.0` → IC=+0.213 (n=1387)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2784` → IC=+0.157 (n=808)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.2784 (IC base=+0.154)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.581` → IC=+0.302 (n=256)
...
- **PATRÓN** `volumen_spike_ratio` < `2.6507` → IC=+0.139 (n=710)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 2.6507 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` > `1.4431` → IC=+0.146 (n=710)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.233 (n=635)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1838` → IC=+0.221 (n=406)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1838 (IC base=+0.212)

- **PATRÓN** `dist_vwap_pct` < `0.4806` → IC=+0.216 (n=572)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4806 (IC base=+0.212)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `7.011` → IC=+0.213 (n=636)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 7.011 (IC base=+0.212)

- **PATRÓN** `volumen_regimen` < `1.2593` → IC=+0.215 (n=634)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1007` → IC=+0.217 (n=603)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1007 (IC base=+0.212)

- **PATRÓN** `volumen_spike_ratio` < `2.109` → IC=+0.227 (n=537)
...
- **PATRÓN** `ibs_20min` < `0.6603` → IC=+0.164 (n=694)

  - _Acción_: Kelly boost +0.82€ cuando `ibs_20min` < 0.6603 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` < `0.1424` → IC=+0.163 (n=582)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.1424 (IC base=+0.140)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1504` → IC=+0.213 (n=172)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1504 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` < `1.7299` → IC=+0.155 (n=392)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 1.7299 (IC base=+0.140)

- **PATRÓN** `volumen_spike_ratio` > `1.4046` → IC=+0.154 (n=587)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.4046 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `12510.8748` → IC=+0.154 (n=463)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 12510.8748 (IC base=+0.140)

- **PATRÓN** `ballena_activa_n` < `287.0` → IC=+0.158 (n=334)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 287.0 (IC base=+0.140)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.287` → IC=+0.269 (n=171)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.287 (IC base=+0.162)

- **PATRÓN** `volumen_pendiente_norm` < `0.1321` → IC=+0.164 (n=650)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` < 0.1321 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` < `3.9092` → IC=+0.157 (n=677)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 3.9092 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` > `1.6798` → IC=+0.170 (n=677)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 1.6798 (IC base=+0.162)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.187 (n=781)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3735` → IC=+0.244 (n=542)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3735 (IC base=+0.240)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.245 (n=304)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.0415` → IC=+0.303 (n=206)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0415 (IC base=+0.240)
...
- **PATRÓN** `volumen_spike_ratio` > `2.4461` → IC=+0.222 (n=329)
...
- **PATRÓN** `ballena_activa_n` < `52.0` → IC=+0.226 (n=422)
...
- **FILTRO** `drift_60min` |x|> `0.5305` → IC=-0.157 (n=345)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=670)
...
- **FILTRO** `sigma_h` > `0.0248` → IC=-0.143 (n=407)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=1222)

- **FILTRO** `ibs_20min` > `0.7937` → IC=-0.190 (n=407)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7937
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=1222)

- **FILTRO** `sigma_ewma_delta_pct` > `8.908` → IC=-0.141 (n=190)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.908
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=1439)
...
- **PATRÓN** `volumen_regimen` < `0.742` → IC=+0.196 (n=90)
  ...
  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` < 0.742 (IC base=-0.049)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3306` → IC=+0.222 (n=16)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3306 (IC base=-0.049)

- **PATRÓN** `volumen_spike_ratio` < `2.339` → IC=+0.174 (n=87)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.339 (IC base=-0.049)
...
- **PATRÓN** `ibs_20min` > `0.8224` → IC=+0.222 (n=282)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8224 (IC base=+0.020)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.053` → IC=+0.127 (n=421)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` > 2.053 (IC base=+0.020)
...
- **PATRÓN** `volumen_spike_ratio` < `1.9482` → IC=+0.232 (n=125)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9482 (IC base=+0.020)

- **PATRÓN** `volumen_spike_ratio` > `2.6489` → IC=+0.285 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6489 (IC base=+0.020)

- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.336 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 62.0 (IC base=+0.020)
...
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.287 (n=327)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0083 (IC base=+0.223)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.297` → IC=+0.298 (n=176)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.297 (IC base=+0.223)

- **PATRÓN** `volumen_pendiente_norm` < `0.1395` → IC=+0.239 (n=608)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3984` → IC=+0.323 (n=274)
...
- **FILTRO** `ibs_20min` > `0.7551` → IC=-0.225 (n=278)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=837)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0923` → IC=+0.253 (n=277)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0923 (IC base=+0.006)

- **PATRÓN** `volumen_pendiente_norm` > `0.2328` → IC=+0.263 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2328 (IC base=+0.006)

- **PATRÓN** `volumen_spike_ratio` < `1.4502` → IC=+0.296 (n=96)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4502 (IC base=+0.006)

- **PATRÓN** `ballena_activa_n` < `57.0` → IC=+0.308 (n=222)
...
- **PATRÓN** `ibs_20min` < `0.4387` → IC=+0.231 (n=3514)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4387 (IC base=+0.179)

- **PATRÓN** `dist_vwap_pct` < `0.2292` → IC=+0.174 (n=2671)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.2292 (IC base=+0.179)
...
- **PATRÓN** `volumen_regimen` < `1.176` → IC=+0.169 (n=2660)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.176 (IC base=+0.179)

- **PATRÓN** `volumen_regimen` > `0.6221` → IC=+0.162 (n=2661)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5767` → IC=+0.173 (n=1271)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.5767 (IC base=+0.179)

- **PATRÓN** `volumen_spike_ratio` > `2.6613` → IC=+0.200 (n=963)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6613 (IC base=+0.179)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.180 (n=2477)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 156.0 (IC base=+0.179)

### GBM_LATE_15M_MULTIHORIZONTE#BNB#15min
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.184 (n=280)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0057 (IC base=+0.173)

- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.201 (n=292)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0071 (IC base=+0.173)

- **PATRÓN** `drift_60min` |x|≤ `0.283` → IC=+0.196 (n=637)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.283 (IC base=+0.173)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.225 (n=242)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.173)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.300 (n=293)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.173)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.527` → IC=+0.293 (n=293)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2255` → IC=+0.259 (n=114)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2255 (IC base=+0.173)

- **PATRÓN** `volumen_spike_ratio` < `2.6388` → IC=+0.163 (n=553)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.6388 (IC base=+0.173)

- **PATRÓN** `volumen_spike_ratio` > `1.4556` → IC=+0.165 (n=553)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.4556 (IC base=+0.173)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.203 (n=519)
...
- **PATRÓN** `sigma_h` < `0.003` → IC=+0.220 (n=187)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.003 (IC base=+0.168)

- **PATRÓN** `drift_60min` |x|≤ `0.4166` → IC=+0.175 (n=561)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.4166 (IC base=+0.168)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.194 (n=574)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.204` → IC=+0.224 (n=408)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.204 (IC base=+0.168)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.986` → IC=+0.240 (n=156)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.986 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` < `0.6381` → IC=+0.188 (n=187)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` < 0.6381 (IC base=+0.168)
...
- **PATRÓN** `libro_liquidez` > `11309.5324` → IC=+0.190 (n=501)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2988` → IC=+0.173 (n=649)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.2988 (IC base=+0.154)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.172 (n=604)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 7.0 (IC base=+0.154)

- **PATRÓN** `ibs_20min` < `0.4851` → IC=+0.191 (n=649)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` < 0.4851 (IC base=+0.154)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4443` → IC=+0.163 (n=541)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.4443 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` > `1.4046` → IC=+0.159 (n=540)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.4046 (IC base=+0.154)

- **PATRÓN** `libro_liquidez` > `15557.7065` → IC=+0.156 (n=216)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 15557.7065 (IC base=+0.154)
...
- **PATRÓN** `sigma_h` < `0.0103` → IC=+0.257 (n=459)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0103 (IC base=+0.236)

- **PATRÓN** `drift_60min` |x|≤ `0.2231` → IC=+0.257 (n=306)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2231 (IC base=+0.236)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.255 (n=316)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.236)

- **PATRÓN** `ibs_20min` < `0.3827` → IC=+0.268 (n=459)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3827 (IC base=+0.236)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.609` → IC=+0.325 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.609 (IC base=+0.236)

- **PATRÓN** `volumen_pendiente_norm` > `0.3666` → IC=+0.287 (n=73)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3666 (IC base=+0.236)
...
- **PATRÓN** `volumen_spike_ratio` > `2.364` → IC=+0.225 (n=256)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.364 (IC base=+0.236)
...
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.205 (n=378)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.172 (n=578)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.9226` → IC=+0.211 (n=133)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2487` → IC=+0.215 (n=128)
...
- **PATRÓN** `volumen_spike_ratio` < `1.435` → IC=+0.154 (n=183)
  ...
  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 1.435 (IC base=+0.158)
...
- **PATRÓN** `libro_liquidez` > `8750.1763` → IC=+0.191 (n=376)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=751)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `1.1582` → IC=+0.259 (n=164)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1582 (IC base=+0.114)
...
- **PATRÓN** `volumen_regimen` > `0.6282` → IC=+0.126 (n=714)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` > 0.6282 (IC base=+0.114)

- **PATRÓN** `volumen_pendiente_norm` < `0.1685` → IC=+0.133 (n=710)
...
- **PATRÓN** `ballena_activa_n` < `57.0` → IC=+0.147 (n=522)
...
- **PATRÓN** `libro_liquidez` > `3185.27` → IC=+0.177 (n=215)
  ...
  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 3185.27 (IC base=+0.134)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1746` → IC=+0.213 (n=322)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1746 (IC base=+0.192)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.202 (n=766)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.192)

- **PATRÓN** `ibs_20min` > `0.89` → IC=+0.271 (n=487)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.89 (IC base=+0.192)

- **PATRÓN** `dist_vwap_pct` > `0.1675` → IC=+0.226 (n=524)
...
- **PATRÓN** `volumen_regimen` > `0.6857` → IC=+0.211 (n=652)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6857 (IC base=+0.192)

- **PATRÓN** `volumen_pendiente_norm` > `0.2397` → IC=+0.262 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2397 (IC base=+0.192)

- **PATRÓN** `volumen_spike_ratio` < `2.5276` → IC=+0.210 (n=691)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.165 (n=751)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.93` → IC=+0.218 (n=101)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.93 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.176` → IC=+0.182 (n=199)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 8.176 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `0.8772` → IC=+0.157 (n=424)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.18` → IC=+0.176 (n=217)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.18 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` < `2.2818` → IC=+0.148 (n=643)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 2.2818 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.8691` → IC=+0.140 (n=487)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` > 1.8691 (IC base=+0.136)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.152 (n=587)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.01 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `6774.8969` → IC=+0.174 (n=262)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 6774.8969 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `12.0` → IC=+0.184 (n=194)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 12.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` < `0.3103` → IC=+0.139 (n=536)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` < 0.3103 (IC base=+0.069)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1773` → IC=+0.152 (n=199)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_pendiente_norm` > 0.1773 (IC base=+0.069)
...
- **PATRÓN** `ibs_20min` < `0.6147` → IC=+0.146 (n=235)

  - _Acción_: Kelly boost +0.73€ cuando `ibs_20min` < 0.6147 (IC base=+0.063)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.06` → IC=+0.160 (n=51)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 9.06 (IC base=+0.063)

- **PATRÓN** `volumen_pendiente_norm` > `0.1669` → IC=+0.225 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1669 (IC base=+0.063)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.203 (n=2059)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0087 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.172 (n=4562)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 6.0 (IC base=+0.165)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.303 (n=1565)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` > `1.0594` → IC=+0.222 (n=663)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0594 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.635` → IC=+0.224 (n=2556)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.635 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` < `0.8868` → IC=+0.159 (n=2097)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.8868 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` > `1.088` → IC=+0.154 (n=1426)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.088 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` > `0.1675` → IC=+0.191 (n=1230)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.1675 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `1.8746` → IC=+0.173 (n=2808)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.8746 (IC base=+0.165)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.167 (n=4299)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.02 (IC base=+0.165)

- **PATRÓN** `libro_liquidez` > `3031.8812` → IC=+0.185 (n=2057)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 3031.8812 (IC base=+0.165)

- **PATRÓN** `ballena_activa_n` < `60.0` → IC=+0.213 (n=2114)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 60.0 (IC base=+0.165)

- **PATRÓN** `sigma_h` < `0.0108` → IC=+0.198 (n=4191)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0108 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.4818` → IC=+0.197 (n=4189)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5625` → IC=+0.242 (n=4189)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3087` → IC=+0.205 (n=1579)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3087 (IC base=+0.189)

- **PATRÓN** `ballena_activa_n` < `139.0` → IC=+0.183 (n=3061)
...
- **PATRÓN** `sigma_h` < `0.0053` → IC=+0.192 (n=248)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0053 (IC base=+0.186)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.331 (n=259)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.186)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.788` → IC=+0.309 (n=438)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8895` → IC=+0.179 (n=441)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` > 1.8895 (IC base=+0.186)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.233 (n=575)
...
- **PATRÓN** `sigma_h` < `0.0029` → IC=+0.184 (n=248)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0029 (IC base=+0.160)

- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.164 (n=248)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.0069 (IC base=+0.160)

- **PATRÓN** `drift_60min` |x|≤ `0.0956` → IC=+0.168 (n=248)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.0956 (IC base=+0.160)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.183 (n=664)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 8.0 (IC base=+0.160)

- **PATRÓN** `ibs_20min` > `0.3271` → IC=+0.206 (n=743)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3271 (IC base=+0.160)

- **PATRÓN** `dist_vwap_pct` > `0.2354` → IC=+0.219 (n=429)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2354 (IC base=+0.160)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.707` → IC=+0.189 (n=181)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 9.707 (IC base=+0.160)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.439` → IC=+0.170 (n=647)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` < 4.439 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` < `1.2687` → IC=+0.166 (n=743)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.2687 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` > `1.1052` → IC=+0.170 (n=337)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 1.1052 (IC base=+0.160)

- **PATRÓN** `volumen_pendiente_norm` < `0.0726` → IC=+0.174 (n=621)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` < 0.0726 (IC base=+0.160)

- **PATRÓN** `volumen_pendiente_norm` > `0.1475` → IC=+0.189 (n=204)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.1475 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` < `2.413` → IC=+0.178 (n=694)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 2.413 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` > `1.7227` → IC=+0.181 (n=462)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 1.7227 (IC base=+0.160)
...
- **PATRÓN** `ibs_20min` > `0.6825` → IC=+0.255 (n=581)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6825 (IC base=+0.206)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.58` → IC=+0.348 (n=156)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.58 (IC base=+0.206)

- **PATRÓN** `volumen_pendiente_norm` < `0.2205` → IC=+0.213 (n=590)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2205 (IC base=+0.206)

- **PATRÓN** `volumen_spike_ratio` < `3.9813` → IC=+0.205 (n=577)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.9813 (IC base=+0.206)

- **PATRÓN** `volumen_spike_ratio` > `2.4996` → IC=+0.202 (n=384)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4996 (IC base=+0.206)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.237 (n=655)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.206)

- **PATRÓN** `libro_liquidez` > `1452.72` → IC=+0.207 (n=651)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1452.72 (IC base=+0.206)
...
- **PATRÓN** `ibs_20min` < `0.4064` → IC=+0.298 (n=577)
...
- **PATRÓN** `volumen_spike_ratio` < `3.0442` → IC=+0.247 (n=469)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.0442 (IC base=+0.243)

- **PATRÓN** `volumen_spike_ratio` > `1.9076` → IC=+0.224 (n=476)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9076 (IC base=+0.243)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.244 (n=357)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.243)

- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.220 (n=452)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.136 (n=534)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 12.0 (IC base=+0.094)

- **PATRÓN** `ibs_20min` > `0.4805` → IC=+0.182 (n=787)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` > 0.4805 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` > `0.8747` → IC=+0.192 (n=209)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.8747 (IC base=+0.094)
...
- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.138 (n=567)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.211 (n=912)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.167` → IC=+0.253 (n=524)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5323` → IC=+0.218 (n=838)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.5323 (IC base=+0.214)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.218 (n=463)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5` → IC=+0.266 (n=963)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5 (IC base=+0.214)

- **PATRÓN** `dist_vwap_pct` < `0.2664` → IC=+0.223 (n=864)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2664 (IC base=+0.214)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.228` → IC=+0.285 (n=133)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.228 (IC base=+0.214)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8668` → IC=+0.213 (n=518)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8668 (IC base=+0.214)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.214 (n=1115)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.214)

- **PATRÓN** `ballena_activa_n` < `34.0` → IC=+0.189 (n=673)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5544` → IC=+0.130 (n=1483)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.65€ cuando `drift_60min` |x|≤ 0.5544 (IC base=+0.120)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.167 (n=497)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.981` → IC=+0.123 (n=1506)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` < 5.981 (IC base=+0.120)

- **PATRÓN** `volumen_regimen` > `0.8968` → IC=+0.122 (n=660)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4612` → IC=+0.150 (n=489)
  ...
  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.4612 (IC base=+0.120)

- **PATRÓN** `volumen_spike_ratio` > `2.3856` → IC=+0.145 (n=665)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 2.3856 (IC base=+0.120)

- **PATRÓN** `libro_liquidez` > `8958.3045` → IC=+0.134 (n=672)
...
- **PATRÓN** `sigma_h` < `0.009` → IC=+0.150 (n=541)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.009 (IC base=+0.134)

- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.135 (n=540)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.190 (n=185)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 18.0 (IC base=+0.134)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.159 (n=218)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 5.0 (IC base=+0.134)

- **PATRÓN** `ibs_20min` > `0.186` → IC=+0.148 (n=540)

  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` > 0.186 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` > `1.0394` → IC=+0.164 (n=120)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 1.0394 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` < `0.439` → IC=+0.148 (n=515)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.439 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.008` → IC=+0.146 (n=544)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` < 7.008 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` < `1.2816` → IC=+0.143 (n=541)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8309` → IC=+0.138 (n=354)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 1.8309 (IC base=+0.134)

- **PATRÓN** `libro_liquidez` > `8940.1386` → IC=+0.145 (n=483)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 8940.1386 (IC base=+0.134)
...
- **PATRÓN** `libro_liquidez` > `2344.4754` → IC=+0.130 (n=222)
  ...
  - _Acción_: Kelly boost +0.65€ cuando `libro_liquidez` > 2344.4754 (IC base=+0.106)
...
- **FILTRO** `py_entrada` < `0.45` → IC=-0.183 (n=1352)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.185 (n=1464)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=4503)
...
- **FILTRO** `ibs_20min` < `0.7325` → IC=-0.210 (n=219)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=658)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=2991)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=11050)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.282 (n=3459)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=10582)

- **FILTRO** `ibs_7min` < `0.7115` → IC=-0.247 (n=3508)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7115
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=10533)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.169 (n=4700)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=9341)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.219 (n=4353)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=13332)

- **FILTRO** `ibs_7min` > `0.7143` → IC=-0.172 (n=4411)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=13274)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.155 (n=604)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.101 (n=1438)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.317 (n=507)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=1535)

- **FILTRO** `ibs_7min` < `0.2945` → IC=-0.263 (n=673)
...
- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.227 (n=507)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=1535)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.237 (n=777)
...
- **FILTRO** `ibs_7min` > `0.2755` → IC=-0.178 (n=598)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=1795)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.254 (n=607)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=2478)

- **FILTRO** `ibs_7min` > `0.775` → IC=-0.162 (n=771)

  - _Acción_: SKIP cuando `ibs_7min` > 0.775
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=2314)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.125 (n=62)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 6.0 (IC base=+0.036)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.013 (n=1573)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1242` → IC=+0.242 (n=289)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1242 (IC base=-0.065)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1028` → IC=+0.244 (n=170)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1028 (IC base=-0.065)

- **PATRÓN** `ibs_15` < `0.3707` → IC=+0.271 (n=434)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3707 (IC base=-0.065)

- **PATRÓN** `dist_vwap_pct` > `0.7574` → IC=+0.216 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7574 (IC base=-0.065)
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3707 sube el IC de -0.065 a +0.271 en UPDOWN_GBM_15M_TARDIO (n=434). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 14649 | -0.111 | -2470.46€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 951 | -0.010 | -142.47€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 13698 | -0.118 | -2328.00€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#XRP | 2682 | -0.193 | -741.23€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 2682 | -0.193 | -741.23€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 3975 | -0.070 | +1832.54€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 1130 | -0.010 | +850.94€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 2845 | -0.094 | +981.60€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 3975 | -0.070 | +1832.54€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 1130 | -0.010 | +850.94€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 2845 | -0.094 | +981.60€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 47250 | +0.113 | -2998.96€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 7910 | +0.185 | -272.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 160 | -0.111 | -53.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 35451 | +0.098 | -2598.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3729 | +0.117 | -74.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 5956 | +0.081 | -802.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 26 | -0.107 | +5.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 14 | -0.219 | -10.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 5916 | +0.083 | -797.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 9477 | +0.132 | -230.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 2326 | +0.198 | -109.65€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 5854 | +0.108 | -147.80€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 10221 | +0.126 | -160.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2944 | +0.172 | -13.17€ | 1 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 5892 | +0.111 | -99.26€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#XRP | 5954 | +0.105 | -411.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 20 | +0.000 | +1.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 5 | -0.018 | -1.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 5929 | +0.106 | -411.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 