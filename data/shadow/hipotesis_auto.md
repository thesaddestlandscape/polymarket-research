# Hipótesis automáticas — 2026-07-18 19:59 UTC
_Generado por shadow_postmortem.py sobre 21168 resoluciones (PNL=+5047.86€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.190 (n=1177)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 8.0 (IC base=+0.189)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.205 (n=1325)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.189)

- **PATRÓN** `py_entrada` > `0.695` → IC=+0.297 (n=579)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.189)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.194 (n=1477)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.183 (n=1555)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.236 (n=259)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.226 (n=355)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.224)

- **PATRÓN** `libro_liquidez` > `4455.1294` → IC=+0.229 (n=186)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4455.1294 (IC base=+0.224)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.243 (n=99)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` < `16.0` → IC=+0.172 (n=62)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` < 16.0 (IC base=+0.174)

- **PATRÓN** `py_entrada` < `0.66` → IC=+0.199 (n=71)

  - _Acción_: Kelly boost +0.99€ cuando `py_entrada` < 0.66 (IC base=+0.174)

- **PATRÓN** `py_entrada` > `0.575` → IC=+0.171 (n=71)

  - _Acción_: Kelly boost +0.86€ cuando `py_entrada` > 0.575 (IC base=+0.174)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.126 (n=1771)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 8.0 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `0.7393` → IC=+0.139 (n=131)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.7393 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.041` → IC=+0.212 (n=158)
...
- **PATRÓN** `sigma_h` < `0.0103` → IC=+0.126 (n=249)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.63€ cuando `sigma_h` < 0.0103 (IC base=+0.103)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.148 (n=191)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 18.0 (IC base=+0.103)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.326` → IC=+0.195 (n=80)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 5.326 (IC base=+0.103)
...
- **PATRÓN** `sigma_h` < `0.0287` → IC=+0.152 (n=536)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` < 0.0287 (IC base=+0.151)

- **PATRÓN** `sigma_h` > `0.022` → IC=+0.186 (n=243)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.022 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.159 (n=479)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.183 (n=737)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5878` → IC=+0.219 (n=169)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5878 (IC base=+0.164)
...
- **PATRÓN** `sigma_h` < `0.0173` → IC=+0.263 (n=234)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0173 (IC base=+0.253)

- **PATRÓN** `sigma_h` > `0.0092` → IC=+0.266 (n=233)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0092 (IC base=+0.253)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.257 (n=220)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.6709` → IC=+0.328 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6709 (IC base=+0.253)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.135 (n=856)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5248` → IC=+0.188 (n=168)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.5248 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` < `0.1178` → IC=+0.153 (n=476)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.1178 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.39` → IC=+0.284 (n=123)
...
- **PATRÓN** `dist_vwap_pct` < `0.7882` → IC=+0.128 (n=903)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.7882 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.506` → IC=+0.124 (n=235)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 3.506 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.031` → IC=+0.122 (n=794)
...
- **PATRÓN** `sigma_h` < `0.009` → IC=+0.129 (n=114)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` < 0.009 (IC base=+0.045)
...
- **PATRÓN** `sigma_h` < `0.0289` → IC=+0.209 (n=294)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0289 (IC base=+0.207)

- **PATRÓN** `sigma_h` > `0.0112` → IC=+0.235 (n=262)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.2339` → IC=+0.264 (n=87)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2339 (IC base=+0.207)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.899` → IC=+0.276 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.899 (IC base=+0.207)
...
- **PATRÓN** `ibs_15` > `0.734` → IC=+0.190 (n=295)
  ...
  - _Acción_: Kelly boost +0.95€ cuando `ibs_15` > 0.734 (IC base=+0.032)

- **PATRÓN** `dist_vwap_pct` > `0.133` → IC=+0.154 (n=134)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.133 (IC base=+0.032)

- **PATRÓN** `dist_vwap_pct` < `0.2752` → IC=+0.143 (n=169)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.2752 (IC base=+0.032)

- **PATRÓN** `sigma_ewma_delta_pct` > `17.819` → IC=+0.150 (n=38)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 17.819 (IC base=+0.032)
...
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min` — IC=+0.333 n=34. Faltan ~6 resoluciones para umbral n≥40. ETA: ~4h.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH` — IC=+0.333 n=34. Faltan ~6 resoluciones para umbral n≥40. ETA: ~4h.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 214 | +0.148 | +13.34€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 214 | +0.148 | +13.34€ | 0 | 0 |
...
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 44 | +0.087 | +2.75€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 44 | +0.087 | +2.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 3514 | +0.183 | +26.46€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 2286 | +0.217 | +81.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 148 | +0.080 | +4.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 480 | +0.091 | -82.56€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 600 | +0.151 | +23.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 1021 | +0.193 | +30.22€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 200 | +0.173 | +28.24€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH | 1250 | +0.176 | +3.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 757 | +0.219 | +38.23€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 49 | -0.010 | -8.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 244 | +0.102 | -35.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 200 | +0.148 | +9.38€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL | 1231 | +0.182 | -5.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 756 | +0.224 | +45.64€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 50 | +0.135 | +8.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 225 | +0.095 | -44.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 200 | +0.129 | -14.53€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 5611 | +0.105 | +1850.46€ | 0 | 8 |
| ✅ GBM_LATE_15M#15min | 5611 | +0.105 | +1850.46€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#SOL | 1514 | +0.101 | +592.32€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1514 | +0.101 | +592.32€ | 1 | 7 |
| ✅ GBM_LATE_15M#XRP | 1459 | +0.161 | +782.81€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1459 | +0.161 | +782.81€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2837 | +0.151 | +1675.95€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2837 | +0.151 | +1675.95€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 724 | +0.116 | +339.68€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 724 | +0.116 | +339.68€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 746 | +0.116 | +352.68€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 746 | +0.116 | +352.68€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 735 | +0.111 | +315.78€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 735 | +0.111 | +315.78€ | 1 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 632 | +0.278 | +667.81€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 632 | +0.278 | +667.81€ | 0 | 12 |
...
| ✅ GBM_LATE_15M_TARDIO | 2779 | +0.102 | +973.98€ | 0 | 10 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2779 | +0.102 | +973.98€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 675 | +0.069 | +135.29€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 675 | +0.069 | +135.29€ | 0 | 8 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 669 | +0.038 | +66.90€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 669 | +0.038 | +66.90€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 618 | +0.058 | +133.47€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 618 | +0.058 | +133.47€ | 2 | 7 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 817 | +0.213 | +638.33€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 817 | +0.213 | +638.33€ | 0 | 12 |
...
| ✅ UPDOWN_GBM | 1777 | +0.021 | +131.10€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1517 | +0.046 | +169.15€ | 1 | 7 |
...
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD | 52 | +0.278 | +10.00€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#15min | 52 | +0.278 | +10.00€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC | 18 | +0.135 | -2.16€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min | 18 | +0.135 | -2.16€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH | 34 | +0.333 | +12.16€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min | 34 | +0.333 | +12.16€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO | 492 | +0.184 | +216.67€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 492 | +0.184 | +216.67€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 123 | +0.212 | +50.20€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 123 | +0.212 | +50.20€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 128 | +0.131 | +44.73€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 128 | +0.131 | +44.73€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 115 | +0.227 | +79.75€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 115 | +0.227 | +79.75€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.183: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.051 n=689 | neutral: IC=-0.010 n=502 | overbought(IBS>0.7): IC=+0.173 n=619
  - _Datos_: n=1934 IC=+0.082 PNL=+383.37€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.116 n=12670 | tras_1loss IC=+0.085 n=8294 | tras_2loss IC=+0.052 n=3409/40 | gap=+0.065 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1648 IC=+0.028 PNL=+148.48€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1648 IC=+0.028 PNL=+148.48€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=765 IC=+0.032 PNL=+58.51€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=765 IC=+0.032 PNL=+58.51€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=799 IC=+0.079 PNL=+116.52€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=799 IC=+0.079 PNL=+116.52€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=18710 IC=+0.119 PNL=+5123.75€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=18710 IC=+0.119 PNL=+5123.75€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=915 IC=+0.002 PNL=+27.96€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=915 IC=+0.002 PNL=+27.96€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.162 > 0.1 con n=498 PNL=+172.57€
  - _Datos_: n=498 IC=+0.162 PNL=+172.57€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.136 > 0.08 con n=361 PNL=+104.76€
  - _Datos_: n=361 IC=+0.136 PNL=+104.76€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.207 > 0.08 con n=186 PNL=+10.72€
  - _Datos_: n=186 IC=+0.207 PNL=+10.72€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.253 > 0.08 con n=269 PNL=-11.91€
  - _Datos_: n=269 IC=+0.253 PNL=-11.91€
