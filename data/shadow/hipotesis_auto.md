# Hipótesis automáticas — 2026-07-16 01:31 UTC
_Generado por shadow_postmortem.py sobre 16235 resoluciones (PNL=+3271.38€)_
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.203 (n=324)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.174 (n=1050)

  - _Acción_: Kelly boost +0.87€ cuando `libro_spread` < 0.01 (IC base=+0.163)

- **PATRÓN** `libro_liquidez` > `7407.0697` → IC=+0.186 (n=307)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 7407.0697 (IC base=+0.163)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.228 (n=134)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `7250.1498` → IC=+0.215 (n=177)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7250.1498 (IC base=+0.207)
...
- **PATRÓN** `libro_liquidez` > `3766.7239` → IC=+0.214 (n=194)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.214 (n=68)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.203)

- **PATRÓN** `py_entrada` < `0.365` → IC=+0.262 (n=170)
...
- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.132 (n=1212)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0091 (IC base=+0.104)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.127 (n=1306)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` < 12.0 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.245` → IC=+0.121 (n=130)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` > 6.245 (IC base=+0.104)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.206` → IC=+0.191 (n=53)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 3.206 (IC base=+0.059)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.518` → IC=+0.200 (n=38)
...
- **PATRÓN** `sigma_h` > `0.0093` → IC=+0.133 (n=456)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` > 0.0093 (IC base=+0.128)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.173 (n=334)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.5306` → IC=+0.133 (n=167)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.5306 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.722` → IC=+0.125 (n=150)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.177 (n=728)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.188 (n=245)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` < 5.0 (IC base=+0.160)

- **PATRÓN** `dist_vwap_pct` < `0.4336` → IC=+0.193 (n=500)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` < 0.4336 (IC base=+0.160)
...
- **PATRÓN** `sigma_h` < `0.0161` → IC=+0.293 (n=162)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0161 (IC base=+0.288)

- **PATRÓN** `sigma_h` > `0.0085` → IC=+0.323 (n=162)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0085 (IC base=+0.288)
...
- **PATRÓN** `dist_vwap_pct` < `0.5285` → IC=+0.307 (n=133)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5285 (IC base=+0.288)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.933` → IC=+0.305 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.933 (IC base=+0.288)
...
- **PATRÓN** `sigma_h` > `0.0238` → IC=+0.231 (n=76)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0238 (IC base=+0.190)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.244 (n=76)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.190)

- **PATRÓN** `dist_vwap_pct` > `0.3121` → IC=+0.180 (n=48)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.3121 (IC base=+0.190)

- **PATRÓN** `dist_vwap_pct` < `0.5022` → IC=+0.175 (n=167)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.5022 (IC base=+0.190)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.549` → IC=+0.189 (n=130)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` < 2.549 (IC base=+0.190)
...
- **PATRÓN** `ibs_15` < `0.0266` → IC=+0.124 (n=139)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.62€ cuando `ibs_15` < 0.0266 (IC base=+0.048)

- **PATRÓN** `dist_vwap_pct` > `0.921` → IC=+0.125 (n=22)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` > 0.921 (IC base=+0.048)

- **PATRÓN** `sigma_ewma_delta_pct` < `16.748` → IC=+0.136 (n=138)

  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` < 16.748 (IC base=+0.048)
...
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#SOL#15min` — IC=+0.156 n=30. Faltan ~10 resoluciones para umbral n≥40. ETA: ~7h.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_TARDIO#SOL` — IC=+0.156 n=30. Faltan ~10 resoluciones para umbral n≥40. ETA: ~7h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2374 | +0.173 | -56.23€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1528 | +0.210 | +16.50€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 688 | +0.180 | -15.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 511 | +0.204 | -14.53€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 844 | +0.173 | -3.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 506 | +0.216 | +20.64€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 834 | +0.170 | -33.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 503 | +0.213 | +13.77€ | 0 | 6 |
...
| ✅ GBM_LATE_15M | 4743 | +0.102 | +1472.95€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 4743 | +0.102 | +1472.95€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1137 | +0.065 | +159.59€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1137 | +0.065 | +159.59€ | 0 | 3 |
| ✅ GBM_LATE_15M#ETH | 1146 | +0.077 | +202.43€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1146 | +0.077 | +202.43€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1265 | +0.110 | +513.11€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1265 | +0.110 | +513.11€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1195 | +0.155 | +597.82€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1195 | +0.155 | +597.82€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1984 | +0.151 | +1157.43€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1984 | +0.151 | +1157.43€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 507 | +0.111 | +244.51€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 507 | +0.111 | +244.51€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 526 | +0.129 | +244.80€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 526 | +0.129 | +244.80€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 535 | +0.113 | +237.42€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 535 | +0.113 | +237.42€ | 1 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 416 | +0.273 | +430.69€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 416 | +0.273 | +430.69€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 1946 | +0.088 | +570.65€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1946 | +0.088 | +570.65€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 481 | +0.059 | +86.40€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 481 | +0.059 | +86.40€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 445 | +0.019 | +12.77€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 445 | +0.019 | +12.77€ | 0 | 2 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 467 | +0.057 | +86.70€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 467 | +0.057 | +86.70€ | 2 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 553 | +0.196 | +384.79€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 553 | +0.196 | +384.79€ | 0 | 10 |
...
| ✅ UPDOWN_GBM | 1577 | +0.010 | +73.08€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1319 | +0.037 | +110.11€ | 0 | 7 |
...
| ✅ UPDOWN_GBM#ETH | 758 | +0.018 | +36.93€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 665 | +0.035 | +47.98€ | 1 | 3 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 80 | +0.207 | +25.76€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 80 | +0.207 | +25.76€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 21 | +0.239 | +4.87€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 21 | +0.239 | +4.87€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 30 | +0.156 | +8.95€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 30 | +0.156 | +8.95€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 10 | +0.125 | +3.11€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 10 | +0.125 | +3.11€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.142) — sin ventaja clara. oversold(IBS<0.3): IC=+0.011 n=470 | neutral: IC=-0.020 n=365 | overbought(IBS>0.7): IC=+0.121 n=373
  - _Datos_: n=1276 IC=+0.040 PNL=+124.03€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9425 | tras_1loss IC=+0.066 n=6603 | tras_2loss IC=+0.033 n=2833/40 | gap=+0.069 (umbral 0.05)
...
**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  ...
  - _Estado_: n_overlaps=33, boost estimado=-0.040. Necesita 0 más y boost>0.05
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1450 IC=+0.017 PNL=+87.07€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1450 IC=+0.017 PNL=+87.07€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=697 IC=+0.074 PNL=+88.86€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=697 IC=+0.074 PNL=+88.86€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=585 IC=+0.033 PNL=+41.72€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=585 IC=+0.033 PNL=+41.72€

**⏳ H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  - _Hipótesis_: Feature nueva 2026-06-27: drift del precio YES en Polymarket en últimas 5 obs (~5min). Si poly_drift<0 y decidimos BUY_NO (o poly_drift>0 y BUY_YES) → confluencia. Si diverge → reducción de stake. Hipótesis: confluencia Binance+Polymarket mejora IC; divergencia empeora.
  - _Umbral_: 40
  - _Acción_: Si IC_confluencia>IC_divergencia con n≥40 → mantener el boost. Si no → retirar.
  - _Estado_: 4/40 ops en el filtro definido (IC actual=+0.000 PNL=-1.61€)
  - _Datos_: n=4 IC=+0.000 PNL=-1.61€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=13777 IC=+0.104 PNL=+3347.26€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=13777 IC=+0.104 PNL=+3347.26€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.163 > 0.1 con n=401 PNL=+136.60€
  - _Datos_: n=401 IC=+0.163 PNL=+136.60€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=606 IC=+0.054 PNL=+69.18€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=606 IC=+0.054 PNL=+69.18€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.156 > 0.08 con n=149 PNL=+56.39€
  - _Datos_: n=149 IC=+0.156 PNL=+56.39€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.115 > 0.08 con n=133 PNL=+51.10€
  - _Datos_: n=133 IC=+0.115 PNL=+51.10€
...
**🔶 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.149 n=183) pero sin cruzar ≥2 pares más — BTC: n=137 IC=-0.004; ETH: n=105 IC=-0.005; SOL: n=194 IC=+0.056 ✓
  - _Datos_: n=183 IC=+0.149 PNL=+103.67€
...
**🔶 H-CUSTOM-ETH15-BUYNO-TARDIO** — UPDOWN_GBM ETH#15min BUY_NO tardío (T_h<0.2) -- edge fuerte no capturado por el aprendizaje causal automático
  ...
  - _Estado_: SEÑAL POSITIVA en ETH (IC=+0.267 n=58) pero sin cruzar ≥2 pares más — BNB: n=11 IC=+0.064; BTC: n=12 IC=+0.043; DOGE: n=8 IC=+0.080; SOL: n=8 IC=+0.000; XRP: n=9 IC=+0.061
