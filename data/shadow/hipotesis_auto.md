# Hipótesis automáticas — 2026-08-24 05:52 UTC
_Generado por shadow_postmortem.py sobre 134743 resoluciones (PNL=+9388.21€)_
...
- **FILTRO** `restante_s_al_confirmar` < `158.15` → IC=-0.252 (n=1877)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=5632)
...
- **FILTRO** `restante_s_al_confirmar` < `148.13` → IC=-0.271 (n=465)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=1397)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.152 (n=3474)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 7.0 (IC base=+0.136)

- **PATRÓN** `py_entrada` < `0.335` → IC=+0.263 (n=2536)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.335 (IC base=+0.136)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.184 (n=2631)

  - _Acción_: Kelly boost +0.92€ cuando `libro_spread` < 0.02 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `1771.7285` → IC=+0.174 (n=2171)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 1771.7285 (IC base=+0.136)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.181 (n=955)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` < 17.0 (IC base=+0.173)

- **PATRÓN** `py_entrada` < `0.74` → IC=+0.192 (n=993)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.238 (n=876)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.230)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.232 (n=752)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.246 (n=1527)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.188 (n=1296)
...
- **PATRÓN** `restante_min` > `4.92` → IC=+0.150 (n=1197)

  - _Acción_: Kelly boost +0.75€ cuando `restante_min` > 4.92 (IC base=+0.130)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.149 (n=1552)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 7.0 (IC base=+0.130)

- **PATRÓN** `lag_apertura_s` < `4.63` → IC=+0.147 (n=1133)

  - _Acción_: Kelly boost +0.73€ cuando `lag_apertura_s` < 4.63 (IC base=+0.130)
...
- **PATRÓN** `restante_min` > `4.86` → IC=+0.157 (n=770)

  - _Acción_: Kelly boost +0.78€ cuando `restante_min` > 4.86 (IC base=+0.140)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.155 (n=1486)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` < 15.0 (IC base=+0.140)

- **PATRÓN** `lag_apertura_s` < `6.37` → IC=+0.158 (n=565)
...
- **PATRÓN** `restante_min` > `4.94` → IC=+0.151 (n=625)

  - _Acción_: Kelly boost +0.75€ cuando `restante_min` > 4.94 (IC base=+0.120)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.137 (n=786)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` < 7.0 (IC base=+0.120)

- **PATRÓN** `lag_apertura_s` < `3.42` → IC=+0.158 (n=571)

  - _Acción_: Kelly boost +0.79€ cuando `lag_apertura_s` < 3.42 (IC base=+0.120)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9497` → IC=+0.223 (n=607)
...
- **FILTRO** `ibs_20min` < `0.4414` → IC=-0.140 (n=273)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.331` → IC=+0.124 (n=1327)
...
- **PATRÓN** `dist_vwap_pct` < `0.1312` → IC=+0.193 (n=597)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` < 0.1312 (IC base=+0.035)

- **PATRÓN** `volumen_regimen` > `0.63` → IC=+0.200 (n=645)
...
- **PATRÓN** `volumen_spike_ratio` > `2.9418` → IC=+0.282 (n=154)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9418 (IC base=+0.035)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.251 (n=295)
...
- **PATRÓN** `dist_vwap_pct` > `0.659` → IC=+0.283 (n=21)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.659 (IC base=-0.067)

- **PATRÓN** `volumen_spike_ratio` > `1.7262` → IC=+0.152 (n=21)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.7262 (IC base=-0.067)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **FILTRO** `ibs_20min` < `0.5094` → IC=-0.142 (n=255)

  - _Acción_: SKIP cuando `ibs_20min` < 0.5094
  - _Potencial_: sin este filtro IC_bueno=+0.124 (n=256)
...
- **PATRÓN** `dist_vwap_pct` > `0.2099` → IC=+0.216 (n=86)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2099 (IC base=-0.009)

- **PATRÓN** `volumen_regimen` > `1.1463` → IC=+0.153 (n=47)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.1463 (IC base=-0.009)

- **PATRÓN** `volumen_pendiente_norm` > `0.2585` → IC=+0.278 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2585 (IC base=-0.009)

- **PATRÓN** `volumen_spike_ratio` < `1.4704` → IC=+0.167 (n=43)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.4704 (IC base=-0.009)

- **PATRÓN** `ballena_activa_n` < `80.0` → IC=+0.184 (n=93)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 80.0 (IC base=-0.009)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3868` → IC=+0.273 (n=417)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3868 (IC base=+0.262)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.294 (n=217)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.262)

- **PATRÓN** `ibs_20min` < `0.2766` → IC=+0.323 (n=417)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2766 (IC base=+0.262)

- **PATRÓN** `dist_vwap_pct` < `0.2282` → IC=+0.273 (n=434)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2282 (IC base=+0.262)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.487` → IC=+0.284 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.487 (IC base=+0.262)

- **PATRÓN** `volumen_regimen` > `0.715` → IC=+0.279 (n=423)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.715 (IC base=+0.262)
...
- **PATRÓN** `ballena_activa_n` < `39.0` → IC=+0.238 (n=219)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.161 (n=2070)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.155)

- **PATRÓN** `ibs_20min` > `0.9` → IC=+0.256 (n=1332)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9 (IC base=+0.155)

- **PATRÓN** `dist_vwap_pct` > `0.744` → IC=+0.243 (n=484)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.744 (IC base=+0.155)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.107` → IC=+0.257 (n=878)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.107 (IC base=+0.155)

- **PATRÓN** `volumen_regimen` > `0.6988` → IC=+0.173 (n=1202)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3331` → IC=+0.150 (n=1539)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 2.3331 (IC base=+0.155)

- **PATRÓN** `volumen_spike_ratio` > `1.8902` → IC=+0.143 (n=1165)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.8902 (IC base=+0.155)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.166 (n=1583)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.01 (IC base=+0.155)

- **PATRÓN** `libro_liquidez` > `2446.4744` → IC=+0.177 (n=1329)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 2446.4744 (IC base=+0.155)

- **PATRÓN** `ballena_activa_n` < `132.0` → IC=+0.188 (n=794)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3449` → IC=+0.196 (n=1739)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.3974` → IC=+0.244 (n=1739)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3974 (IC base=+0.186)

- **PATRÓN** `dist_vwap_pct` < `0.3292` → IC=+0.178 (n=1516)
...
- **PATRÓN** `volumen_regimen` < `1.1795` → IC=+0.163 (n=1439)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.1795 (IC base=+0.186)

- **PATRÓN** `volumen_regimen` > `0.8556` → IC=+0.178 (n=959)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` > 0.8556 (IC base=+0.186)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8969` → IC=+0.182 (n=756)
...
- **PATRÓN** `ballena_activa_n` < `60.0` → IC=+0.197 (n=466)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 60.0 (IC base=+0.186)
...
- **PATRÓN** `sigma_h` < `0.0026` → IC=+0.258 (n=89)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0026 (IC base=+0.176)

- **PATRÓN** `drift_60min` |x|≤ `0.419` → IC=+0.175 (n=263)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.419 (IC base=+0.176)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.214 (n=274)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.176)

- **PATRÓN** `ibs_20min` > `0.655` → IC=+0.234 (n=235)
...
- **PATRÓN** `volumen_regimen` < `0.6445` → IC=+0.178 (n=88)
  ...
  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` < 0.6445 (IC base=+0.176)

- **PATRÓN** `volumen_regimen` > `0.8987` → IC=+0.212 (n=175)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8987 (IC base=+0.176)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4001` → IC=+0.225 (n=78)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4001 (IC base=+0.176)

- **PATRÓN** `volumen_spike_ratio` > `2.1749` → IC=+0.176 (n=106)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.1749 (IC base=+0.176)

- **PATRÓN** `libro_liquidez` > `11526.545` → IC=+0.206 (n=175)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11526.545 (IC base=+0.176)
...
- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.237 (n=230)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0076 (IC base=+0.191)

- **PATRÓN** `drift_60min` |x|≤ `0.133` → IC=+0.209 (n=115)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.133 (IC base=+0.191)
...
- **PATRÓN** `ibs_20min` > `0.4362` → IC=+0.257 (n=261)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4362 (IC base=+0.191)

- **PATRÓN** `dist_vwap_pct` > `0.1644` → IC=+0.224 (n=190)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1644 (IC base=+0.191)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.902` → IC=+0.339 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.902 (IC base=+0.191)

- **PATRÓN** `volumen_regimen` > `0.6382` → IC=+0.219 (n=261)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6382 (IC base=+0.191)

- **PATRÓN** `volumen_pendiente_norm` > `0.101` → IC=+0.261 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.101 (IC base=+0.191)

- **PATRÓN** `volumen_spike_ratio` < `1.4657` → IC=+0.209 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4657 (IC base=+0.191)
...
- **PATRÓN** `libro_liquidez` > `9534.4594` → IC=+0.211 (n=119)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 9534.4594 (IC base=+0.191)

- **PATRÓN** `ballena_activa_n` < `195.0` → IC=+0.169 (n=173)

  - _Acción_: Kelly boost +0.84€ cuando `ballena_activa_n` < 195.0 (IC base=+0.191)
...
- **PATRÓN** `sigma_h` > `0.0092` → IC=+0.183 (n=178)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0092 (IC base=+0.111)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.127 (n=405)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.6618` → IC=+0.262 (n=128)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6618 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.885` → IC=+0.269 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.885 (IC base=+0.111)

- **PATRÓN** `volumen_regimen` > `0.6824` → IC=+0.134 (n=350)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` > 0.6824 (IC base=+0.111)

- **PATRÓN** `volumen_pendiente_norm` > `0.1` → IC=+0.137 (n=133)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_pendiente_norm` > 0.1 (IC base=+0.111)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.268 (n=175)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0057 (IC base=+0.228)

- **PATRÓN** `drift_60min` |x|≤ `0.4895` → IC=+0.234 (n=397)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4895 (IC base=+0.228)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.241 (n=191)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.228)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.253 (n=192)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.228)

- **PATRÓN** `ibs_20min` < `0.1272` → IC=+0.320 (n=265)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1272 (IC base=+0.228)

- **PATRÓN** `dist_vwap_pct` < `0.6823` → IC=+0.241 (n=454)
...
- **PATRÓN** `volumen_regimen` > `0.8739` → IC=+0.271 (n=264)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8739 (IC base=+0.228)
...
- **PATRÓN** `libro_liquidez` > `2540.0452` → IC=+0.229 (n=264)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2540.0452 (IC base=+0.228)

- **PATRÓN** `ballena_activa_n` < `30.0` → IC=+0.194 (n=178)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.209 (n=765)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.9482` → IC=+0.285 (n=1037)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9482 (IC base=+0.140)

- **PATRÓN** `dist_vwap_pct` > `1.0218` → IC=+0.242 (n=397)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0218 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.406` → IC=+0.231 (n=1402)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.406 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` < `0.7083` → IC=+0.148 (n=689)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.7083 (IC base=+0.140)

- **PATRÓN** `volumen_regimen` > `1.2452` → IC=+0.147 (n=522)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 1.2452 (IC base=+0.140)
...
- **PATRÓN** `volumen_spike_ratio` > `1.9176` → IC=+0.150 (n=1317)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.9176 (IC base=+0.140)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.146 (n=1812)

  - _Acción_: Kelly boost +0.73€ cuando `libro_spread` < 0.01 (IC base=+0.140)

- **PATRÓN** `libro_liquidez` > `3461.0358` → IC=+0.182 (n=763)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 3461.0358 (IC base=+0.140)

- **PATRÓN** `ballena_activa_n` < `166.0` → IC=+0.190 (n=805)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3962` → IC=+0.201 (n=2056)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.188 (n=997)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` < 7.0 (IC base=+0.186)

- **PATRÓN** `ibs_20min` < `0.5413` → IC=+0.242 (n=2056)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5413 (IC base=+0.186)

- **PATRÓN** `dist_vwap_pct` < `0.676` → IC=+0.176 (n=1661)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.603` → IC=+0.193 (n=1929)
...
- **PATRÓN** `volumen_regimen` > `1.1867` → IC=+0.177 (n=525)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` > 1.1867 (IC base=+0.186)

- **PATRÓN** `volumen_pendiente_norm` > `0.1667` → IC=+0.234 (n=400)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1667 (IC base=+0.186)

- **PATRÓN** `volumen_spike_ratio` > `2.2679` → IC=+0.195 (n=618)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 2.2679 (IC base=+0.186)

- **PATRÓN** `ballena_activa_n` < `34.0` → IC=+0.144 (n=419)
...
- **PATRÓN** `sigma_h` < `0.0029` → IC=+0.171 (n=150)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0029 (IC base=+0.156)

- **PATRÓN** `sigma_h` > `0.0054` → IC=+0.186 (n=154)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0054 (IC base=+0.156)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.182 (n=353)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 5.0 (IC base=+0.156)

- **PATRÓN** `ibs_20min` > `0.3232` → IC=+0.218 (n=339)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3232 (IC base=+0.156)

- **PATRÓN** `dist_vwap_pct` > `0.2301` → IC=+0.241 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2301 (IC base=+0.156)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.973` → IC=+0.233 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.973 (IC base=+0.156)

- **PATRÓN** `volumen_regimen` < `0.6608` → IC=+0.170 (n=113)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 0.6608 (IC base=+0.156)

- **PATRÓN** `volumen_regimen` > `1.1138` → IC=+0.179 (n=154)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` > 1.1138 (IC base=+0.156)

- **PATRÓN** `volumen_pendiente_norm` > `0.2551` → IC=+0.229 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2551 (IC base=+0.156)

- **PATRÓN** `volumen_spike_ratio` < `2.5664` → IC=+0.193 (n=291)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` < 2.5664 (IC base=+0.156)

- **PATRÓN** `volumen_spike_ratio` > `1.3679` → IC=+0.196 (n=291)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.3679 (IC base=+0.156)

- **PATRÓN** `libro_liquidez` > `11088.6716` → IC=+0.197 (n=226)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 11088.6716 (IC base=+0.156)
...
- **PATRÓN** `sigma_h` < `0.0086` → IC=+0.144 (n=338)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` < 0.0086 (IC base=+0.131)

- **PATRÓN** `drift_60min` |x|≤ `0.1293` → IC=+0.136 (n=149)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.1293 (IC base=+0.131)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.157 (n=307)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 8.0 (IC base=+0.131)

- **PATRÓN** `ibs_20min` > `0.3491` → IC=+0.197 (n=338)

  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` > 0.3491 (IC base=+0.131)

- **PATRÓN** `dist_vwap_pct` > `0.9611` → IC=+0.196 (n=77)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.9611 (IC base=+0.131)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.499` → IC=+0.191 (n=166)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 4.499 (IC base=+0.131)

- **PATRÓN** `volumen_regimen` < `0.914` → IC=+0.162 (n=226)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.914 (IC base=+0.131)

- **PATRÓN** `volumen_regimen` > `1.2094` → IC=+0.152 (n=113)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.2094 (IC base=+0.131)

- **PATRÓN** `volumen_pendiente_norm` > `0.2719` → IC=+0.300 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2719 (IC base=+0.131)

- **PATRÓN** `volumen_spike_ratio` > `1.4212` → IC=+0.207 (n=305)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4212 (IC base=+0.131)

- **PATRÓN** `libro_liquidez` > `4238.7781` → IC=+0.196 (n=225)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 4238.7781 (IC base=+0.131)

- **PATRÓN** `ballena_activa_n` < `233.0` → IC=+0.178 (n=212)

  - _Acción_: Kelly boost +0.89€ cuando `ballena_activa_n` < 233.0 (IC base=+0.131)
...
- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.170 (n=180)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0086 (IC base=+0.080)
...
- **PATRÓN** `ibs_20min` > `0.9231` → IC=+0.266 (n=182)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9231 (IC base=+0.080)

- **PATRÓN** `dist_vwap_pct` > `0.6975` → IC=+0.236 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6975 (IC base=+0.080)
...
- **PATRÓN** `libro_liquidez` > `2747.518` → IC=+0.270 (n=133)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2747.518 (IC base=+0.080)

- **PATRÓN** `ballena_activa_n` < `69.0` → IC=+0.174 (n=173)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.274 (n=361)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0073 (IC base=+0.209)

- **PATRÓN** `drift_60min` |x|≤ `0.3903` → IC=+0.234 (n=476)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4875` → IC=+0.281 (n=541)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4875 (IC base=+0.209)

- **PATRÓN** `dist_vwap_pct` < `0.6914` → IC=+0.224 (n=593)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.936` → IC=+0.217 (n=545)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.936 (IC base=+0.209)

- **PATRÓN** `volumen_regimen` > `0.8844` → IC=+0.232 (n=360)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8844 (IC base=+0.209)
...
- **PATRÓN** `ballena_activa_n` < `37.0` → IC=+0.152 (n=257)
...
- **FILTRO** `ibs_20min` > `0.962` → IC=-0.189 (n=59)

  - _Acción_: SKIP cuando `ibs_20min` > 0.962
  - _Potencial_: sin este filtro IC_bueno=+0.067 (n=178)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.155 (n=564)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.044 (n=1750)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.237 (n=553)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.004 (n=1714)

- **FILTRO** `ibs_20min` > `0.2774` → IC=-0.206 (n=566)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2774
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=1701)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.5` → IC=-0.160 (n=148)

  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.106 (n=191)

- **FILTRO** `ibs_20min` < `0.7263` → IC=-0.186 (n=84)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.178 (n=113)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.049 (n=231)

- **FILTRO** `ibs_20min` < `0.7273` → IC=-0.193 (n=86)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.195 (n=93)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=378)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.150 (n=1245)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
...
- **FILTRO** `py_entrada` < `0.36` → IC=-0.280 (n=1351)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=4282)

- **FILTRO** `ibs_7min` < `0.7412` → IC=-0.228 (n=1408)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7412
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=4225)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.180 (n=1875)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=3758)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.218 (n=1698)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=5237)
...
- **FILTRO** `py_entrada` > `0.52` → IC=-0.181 (n=302)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=914)
...
- **FILTRO** `ibs_7min` < `0.7727` → IC=-0.194 (n=256)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.284 (n=234)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=942)

- **FILTRO** `ibs_7min` > `0.8039` → IC=-0.131 (n=293)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8039
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=883)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.154 (n=24)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=609)
...
- **FILTRO** `sigma_h` > `0.0089` → IC=-0.148 (n=745)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `sigma_ewma_delta_pct` > `16.69` → IC=-0.200 (n=295)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 16.69
  - _Potencial_: sin este filtro IC_bueno=-0.081 (n=1899)
...
- **FILTRO** `sigma_ewma_delta_pct` > `12.226` → IC=-0.180 (n=123)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=646)
...
- **FILTRO** `drift_60min` |x|> `0.775` → IC=-0.141 (n=179)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.775
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=538)

- **FILTRO** `drift_15min` |x|> `1.3646` → IC=-0.158 (n=179)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 1.3646
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n