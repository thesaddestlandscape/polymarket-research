# Hipótesis automáticas — 2026-07-15 12:49 UTC
_Generado por shadow_postmortem.py sobre 15324 resoluciones (PNL=+3002.06€)_
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.197 (n=899)

  - _Acción_: Kelly boost +0.99€ cuando `libro_spread` < 0.01 (IC base=+0.193)

- **PATRÓN** `libro_liquidez` > `4390.8559` → IC=+0.194 (n=508)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 4390.8559 (IC base=+0.193)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.170 (n=974)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.01 (IC base=+0.161)

- **PATRÓN** `libro_liquidez` > `7322.1644` → IC=+0.184 (n=283)

  - _Acción_: Kelly boost +0.92€ cuando `libro_liquidez` > 7322.1644 (IC base=+0.161)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.214 (n=117)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.212 (n=182)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `4675.0354` → IC=+0.217 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4675.0354 (IC base=+0.206)
...
### GBM_LATE_15M
- **PATRÓN** `sigma_ewma_delta_pct` > `11.432` → IC=+0.238 (n=82)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.432 (IC base=+0.107)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.133 (n=1171)
...
- **PATRÓN** `dist_vwap_pct` > `0.1537` → IC=+0.129 (n=33)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` > 0.1537 (IC base=+0.060)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.941` → IC=+0.194 (n=47)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 2.941 (IC base=+0.060)

### GBM_LATE_15M#ETH#15min
- **FILTRO** `sigma_ewma_delta_pct` < `6.416` → IC=-0.122 (n=35)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` < 6.416
  - _Potencial_: sin este filtro IC_bueno=+0.262 (n=40)

- **PATRÓN** `sigma_h` < `0.0045` → IC=+0.156 (n=126)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0045 (IC base=+0.092)

- **PATRÓN** `dist_vwap_pct` > `0.6609` → IC=+0.140 (n=23)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.6609 (IC base=+0.092)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.416` → IC=+0.262 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.416 (IC base=+0.092)
...
- **PATRÓN** `sigma_h` < `0.0296` → IC=+0.140 (n=417)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0296 (IC base=+0.139)

- **PATRÓN** `sigma_h` > `0.0226` → IC=+0.170 (n=189)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0226 (IC base=+0.139)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.146 (n=439)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.6583` → IC=+0.140 (n=23)
  ...
  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.6583 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.893` → IC=+0.176 (n=32)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 7.893 (IC base=+0.139)
...
- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.124 (n=163)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` < 0.0064 (IC base=+0.124)

- **PATRÓN** `sigma_h` > `0.0027` → IC=+0.152 (n=162)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0027 (IC base=+0.124)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.181 (n=111)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 12.0 (IC base=+0.124)

- **PATRÓN** `dist_vwap_pct` > `0.1312` → IC=+0.167 (n=46)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.1312 (IC base=+0.124)
...
- **PATRÓN** `sigma_h` > `0.0067` → IC=+0.228 (n=79)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0067 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.167 (n=118)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 12.0 (IC base=+0.145)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.163 (n=185)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 18.0 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.637` → IC=+0.250 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.637 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.752` → IC=+0.395 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.752 (IC base=+0.145)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.149 (n=169)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 8.0 (IC base=+0.135)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.134 (n=121)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` < 12.0 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` > `0.4625` → IC=+0.194 (n=47)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.4625 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` < `1.0918` → IC=+0.143 (n=127)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 1.0918 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.459` → IC=+0.250 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.459 (IC base=+0.135)
...
- **PATRÓN** `sigma_h` < `0.0179` → IC=+0.262 (n=141)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.292 (n=128)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.3705` → IC=+0.250 (n=90)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3705 (IC base=+0.251)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.099` → IC=+0.318 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.099 (IC base=+0.251)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0145` → IC=+0.160 (n=198)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` > 0.0145 (IC base=+0.102)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.123 (n=534)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` < 15.0 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` > `0.6286` → IC=+0.174 (n=90)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.6286 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` < `0.1427` → IC=+0.125 (n=190)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.1427 (IC base=+0.102)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.366` → IC=+0.324 (n=72)
...
- **PATRÓN** `sigma_h` < `0.0043` → IC=+0.132 (n=66)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` < 0.0043 (IC base=+0.052)

- **PATRÓN** `dist_vwap_pct` > `0.7964` → IC=+0.180 (n=23)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.7964 (IC base=+0.052)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.728` → IC=+0.342 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.728 (IC base=+0.052)
...
- **PATRÓN** `sigma_h` < `0.0323` → IC=+0.194 (n=168)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0323 (IC base=+0.184)

- **PATRÓN** `sigma_h` > `0.0112` → IC=+0.194 (n=168)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0112 (IC base=+0.184)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.237 (n=112)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 11.0 (IC base=+0.184)

- **PATRÓN** `dist_vwap_pct` > `0.1821` → IC=+0.211 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1821 (IC base=+0.184)

- **PATRÓN** `dist_vwap_pct` < `1.4833` → IC=+0.170 (n=92)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` < 1.4833 (IC base=+0.184)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.491` → IC=+0.192 (n=37)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 8.491 (IC base=+0.184)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.141` → IC=+0.162 (n=69)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` < 11.141 (IC base=+0.184)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.7197` → IC=+0.173 (n=258)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.87€ cuando `ibs_15` > 0.7197 (IC base=+0.026)

- **PATRÓN** `dist_vwap_pct` > `0.7405` → IC=+0.206 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7405 (IC base=+0.026)

- **PATRÓN** `dist_vwap_pct` < `0.3343` → IC=+0.147 (n=134)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.3343 (IC base=+0.026)

- **PATRÓN** `sigma_ewma_delta_pct` > `15.613` → IC=+0.167 (n=19)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 15.613 (IC base=+0.026)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2429` → IC=+0.124 (n=184)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 0.2429 (IC base=+0.075)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.160 (n=154)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 11.0 (IC base=+0.075)

- **PATRÓN** `ibs_15` < `0.9252` → IC=+0.140 (n=145)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.70€ cuando `ibs_15` < 0.9252 (IC base=+0.075)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.986` → IC=+0.197 (n=31)
  ...
  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` < 5.986 (IC base=+0.075)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS > 0.7197 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.173 n=258). Confirma señal de reversión media → alinear con BUY_NO.
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2147 | +0.177 | -32.05€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1386 | +0.209 | +7.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 91 | +0.016 | -14.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 307 | +0.092 | -45.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 363 | +0.163 | +20.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 624 | +0.180 | -13.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 463 | +0.197 | -24.14€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 30 | +0.094 | -0.56€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 121 | +0.175 | +15.25€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#ETH | 761 | +0.179 | +11.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 457 | +0.221 | +23.92€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 30 | -0.125 | -12.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 153 | +0.106 | -17.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 121 | +0.183 | +18.21€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL | 756 | +0.173 | -26.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 460 | +0.212 | +12.13€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 31 | +0.076 | -1.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 144 | +0.103 | -24.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min |