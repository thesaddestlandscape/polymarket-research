# Hipótesis automáticas — 2026-09-23 06:52 UTC
_Generado por shadow_postmortem.py sobre 569544 resoluciones (PNL=+63818.73€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.3` → IC=-0.231 (n=6832)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.3
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=20498)
...
- **FILTRO** `restante_s_al_confirmar` < `133.5` → IC=-0.284 (n=844)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 133.5
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=2533)
...
- **FILTRO** `restante_s_al_confirmar` < `125.17` → IC=-0.357 (n=1366)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 125.17
  - _Potencial_: sin este filtro IC_bueno=-0.124 (n=4099)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.202 (n=13503)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.69 (IC base=+0.102)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.154 (n=3388)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.102)

- **PATRÓN** `libro_liquidez` > `5600.4068` → IC=+0.175 (n=2159)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.204 (n=1624)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.200)

- **PATRÓN** `py_entrada` < `0.375` → IC=+0.263 (n=1477)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.375 (IC base=+0.200)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.201 (n=2080)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.200)

- **PATRÓN** `libro_liquidez` > `13975.0525` → IC=+0.208 (n=731)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13975.0525 (IC base=+0.200)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.145 (n=342)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.154 (n=2720)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 5.0 (IC base=+0.144)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.145 (n=2326)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.203 (n=11018)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.199)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.202 (n=10562)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.199)

- **PATRÓN** `py_entrada` > `0.74` → IC=+0.224 (n=3714)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.175 (n=2554)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` < 17.0 (IC base=+0.168)

- **PATRÓN** `py_entrada` < `0.71` → IC=+0.184 (n=1866)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.254 (n=761)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.191 (n=6004)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.264 (n=2054)
...
- **PATRÓN** `restante_min` > `4.96` → IC=+0.136 (n=2496)

  - _Acción_: Kelly boost +0.68€ cuando `restante_min` > 4.96 (IC base=+0.113)

- **PATRÓN** `lag_apertura_s` < `2.31` → IC=+0.139 (n=2313)
...
- **PATRÓN** `dist_vwap_pct` < `0.6283` → IC=+0.247 (n=2139)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6283 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.932` → IC=+0.177 (n=2971)
...
- **PATRÓN** `volumen_spike_ratio` > `1.9077` → IC=+0.206 (n=3510)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9077 (IC base=+0.105)

- **PATRÓN** `ibs_20min` < `0.5703` → IC=+0.132 (n=9291)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.5703 (IC base=+0.063)

- **PATRÓN** `dist_vwap_pct` > `0.5913` → IC=+0.193 (n=718)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.5913 (IC base=+0.063)

- **PATRÓN** `dist_vwap_pct` < `0.1487` → IC=+0.172 (n=2870)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.1487 (IC base=+0.063)

- **PATRÓN** `volumen_regimen` < `0.6999` → IC=+0.177 (n=1418)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` < 0.6999 (IC base=+0.063)

- **PATRÓN** `volumen_regimen` > `0.8691` → IC=+0.174 (n=2147)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 0.8691 (IC base=+0.063)

- **PATRÓN** `volumen_pendiente_norm` > `0.1682` → IC=+0.223 (n=1537)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1682 (IC base=+0.063)

- **PATRÓN** `volumen_spike_ratio` > `1.5762` → IC=+0.201 (n=4812)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5762 (IC base=+0.063)

- **PATRÓN** `ballena_activa_n` < `137.0` → IC=+0.210 (n=5168)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 137.0 (IC base=+0.063)

### GBM_LATE_15M#BNB#15min
- **PATRÓN** `sigma_h` < `0.005` → IC=+0.181 (n=588)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3473` → IC=+0.166 (n=1747)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.3473 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.166 (n=847)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 15.0 (IC base=+0.161)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.173 (n=1171)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.107` → IC=+0.271 (n=752)
...
- **PATRÓN** `sigma_h` > `0.005` → IC=+0.251 (n=1160)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.005 (IC base=+0.235)

- **PATRÓN** `drift_60min` |x|≤ `0.089` → IC=+0.286 (n=433)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.0603` → IC=+0.296 (n=571)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0603 (IC base=+0.235)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.403` → IC=+0.249 (n=1351)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.403 (IC base=+0.235)

- **PATRÓN** `volumen_pendiente_norm` < `0.092` → IC=+0.231 (n=1107)
...
- **PATRÓN** `ibs_20min` < `0.6918` → IC=+0.170 (n=1381)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` < 0.6918 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.1316` → IC=+0.157 (n=1216)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.195 (n=1203)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5843` → IC=+0.215 (n=1451)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_spike_ratio` > `2.2222` → IC=+0.223 (n=881)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2222 (IC base=+0.215)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.223 (n=963)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.215)

- **PATRÓN** `libro_liquidez` > `1869.9762` → IC=+0.229 (n=658)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1869.9762 (IC base=+0.215)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.163 (n=96)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=2134)
...
- **PATRÓN** `volumen_regimen` < `0.8511` → IC=+0.158 (n=527)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.8511 (IC base=+0.014)
...
- **PATRÓN** `volumen_spike_ratio` > `1.518` → IC=+0.176 (n=658)
...
- **FILTRO** `ibs_20min` > `0.2576` → IC=-0.126 (n=2109)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2576
  - _Potencial_: sin este filtro IC_bueno=+0.125 (n=1040)

- **FILTRO** `sigma_ewma_delta_pct` > `8.651` → IC=-0.204 (n=339)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.651
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=2810)
...
- **PATRÓN** `ibs_20min` < `0.2576` → IC=+0.125 (n=1040)
...
- **FILTRO** `drift_60min` |x|> `0.6469` → IC=-0.184 (n=539)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6469
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=1618)

- **FILTRO** `ibs_20min` < `0.705` → IC=-0.158 (n=1422)

  - _Acción_: SKIP cuando `ibs_20min` < 0.705
  - _Potencial_: sin este filtro IC_bueno=+0.108 (n=735)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.200 (n=401)

  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=1756)

- **FILTRO** `ibs_20min` > `0.7734` → IC=-0.204 (n=798)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7734
  - _Potencial_: sin este filtro IC_bueno=+0.033 (n=2396)
...
- **PATRÓN** `volumen_regimen` < `0.7389` → IC=+0.248 (n=315)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4409` → IC=+0.176 (n=279)
...
- **PATRÓN** `ballena_activa_n` < `15.0` → IC=+0.176 (n=369)
...
- **FILTRO** `ibs_20min` > `0.8145` → IC=-0.148 (n=695)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8145
  - _Potencial_: sin este filtro IC_bueno=+0.052 (n=2089)
...
- **PATRÓN** `dist_vwap_pct` < `0.16` → IC=+0.217 (n=532)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.16 (IC base=+0.002)

- **PATRÓN** `volumen_regimen` > `0.6093` → IC=+0.212 (n=526)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1382` → IC=+0.257 (n=1356)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.256 (n=1064)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.246)

- **PATRÓN** `libro_liquidez` > `1944.5584` → IC=+0.257 (n=509)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1944.5584 (IC base=+0.246)

- **PATRÓN** `sigma_h` > `0.0096` → IC=+0.315 (n=549)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0096 (IC base=+0.283)

- **PATRÓN** `drift_60min` |x|≤ `0.597` → IC=+0.283 (n=1207)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.0909` → IC=+0.289 (n=808)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0909 (IC base=+0.283)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.812` → IC=+0.300 (n=472)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6063` → IC=+0.282 (n=370)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6063 (IC base=+0.283)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.291 (n=794)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.283)

- **PATRÓN** `libro_liquidez` > `1925.1448` → IC=+0.299 (n=402)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1925.1448 (IC base=+0.283)
...
- **FILTRO** `ibs_20min` < `0.2817` → IC=-0.192 (n=469)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.076 (n=1408)

- **FILTRO** `ibs_20min` > `0.7758` → IC=-0.184 (n=571)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7758
  - _Potencial_: sin este filtro IC_bueno=+0.052 (n=1714)

- **PATRÓN** `ibs_20min` > `0.8146` → IC=+0.160 (n=639)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.8146 (IC base=+0.009)

- **PATRÓN** `dist_vwap_pct` > `0.4471` → IC=+0.226 (n=246)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4471 (IC base=+0.009)

- **PATRÓN** `volumen_regimen` < `0.9969` → IC=+0.236 (n=464)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.0773` → IC=+0.253 (n=192)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0773 (IC base=+0.009)

- **PATRÓN** `volumen_spike_ratio` < `1.5053` → IC=+0.260 (n=219)
...
- **PATRÓN** `dist_vwap_pct` < `0.635` → IC=+0.194 (n=442)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` < 0.635 (IC base=-0.007)

- **PATRÓN** `volumen_regimen` < `1.1607` → IC=+0.206 (n=392)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.1607 (IC base=-0.007)

- **PATRÓN** `volumen_regimen` > `0.7197` → IC=+0.193 (n=350)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 0.7197 (IC base=-0.007)

- **PATRÓN** `volumen_pendiente_norm` > `0.1669` → IC=+0.269 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1669 (IC base=-0.007)

- **PATRÓN** `volumen_spike_ratio` < `1.8338` → IC=+0.254 (n=234)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8338 (IC base=-0.007)

- **PATRÓN** `volumen_spike_ratio` > `2.1743` → IC=+0.252 (n=159)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1743 (IC base=-0.007)

- **PATRÓN** `ballena_activa_n` < `138.0` → IC=+0.248 (n=351)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 138.0 (IC base=-0.007)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **FILTRO** `ibs_20min` < `0.7188` → IC=-0.201 (n=1023)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7188
  - _Potencial_: sin este filtro IC_bueno=+0.279 (n=1024)

- **FILTRO** `ibs_20min` > `0.6875` → IC=-0.230 (n=535)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6875
  - _Potencial_: sin este filtro IC_bueno=+0.095 (n=1610)

- **FILTRO** `sigma_ewma_delta_pct` > `4.689` → IC=-0.179 (n=468)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.689
  - _Potencial_: sin este filtro IC_bueno=+0.068 (n=1677)

- **PATRÓN** `ibs_20min` > `0.7188` → IC=+0.279 (n=1024)
...
- **PATRÓN** `volumen_regimen` < `0.8646` → IC=+0.300 (n=502)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8646 (IC base=+0.039)

- **PATRÓN** `volumen_regimen` > `0.6398` → IC=+0.289 (n=753)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6398 (IC base=+0.039)

- **PATRÓN** `volumen_pendiente_norm` < `0.1024` → IC=+0.293 (n=699)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1024 (IC base=+0.039)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4432` → IC=+0.321 (n=244)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4432 (IC base=+0.039)

- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.312 (n=632)
...
- **PATRÓN** `dist_vwap_pct` < `0.2117` → IC=+0.222 (n=462)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1552` → IC=+0.204 (n=542)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5113` → IC=+0.220 (n=501)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.286 (n=1341)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.280)

- **PATRÓN** `libro_liquidez` > `2618.6162` → IC=+0.295 (n=839)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2618.6162 (IC base=+0.280)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.271 (n=680)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.270)

- **PATRÓN** `ibs_20min` < `0.3976` → IC=+0.303 (n=1360)
...
- **PATRÓN** `dist_vwap_pct` < `0.9422` → IC=+0.270 (n=1522)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4342` → IC=+0.268 (n=398)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5628` → IC=+0.172 (n=3194)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.5628 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.6282` → IC=+0.176 (n=2420)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.6282 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `2399.5624` → IC=+0.169 (n=5051)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 2399.5624 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `116.0` → IC=+0.182 (n=6486)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 116.0 (IC base=+0.168)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.182 (n=4787)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0066 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0799` → IC=+0.209 (n=2393)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4779` → IC=+0.225 (n=7169)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4779 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` < `0.2278` → IC=+0.159 (n=5166)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.2278 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.264` → IC=+0.193 (n=1221)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 10.264 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.1748` → IC=+0.151 (n=5210)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.239 (n=815)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `volumen_spike_ratio` < `1.507` → IC=+0.258 (n=374)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.507 (IC base=+0.238)

- **PATRÓN** `libro_liquidez` > `1960.45` → IC=+0.259 (n=309)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1960.45 (IC base=+0.238)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.155 (n=1226)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0057 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.2905` → IC=+0.158 (n=1225)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5636` → IC=+0.184 (n=1225)

  - _Acción_: Kelly boost +0.92€ cuando `ibs_20min` < 0.5636 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `0.1331` → IC=+0.161 (n=1202)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.1331 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.756` → IC=+0.207 (n=240)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.756 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `1.2096` → IC=+0.155 (n=1225)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4292` → IC=+0.143 (n=1114)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 2.4292 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.7551` → IC=+0.134 (n=742)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 1.7551 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `216.0` → IC=+0.168 (n=347)
...
- **PATRÓN** `ibs_20min` < `0.2417` → IC=+0.256 (n=911)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2417 (IC base=+0.221)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.729` → IC=+0.271 (n=438)
...
- **PATRÓN** `volumen_spike_ratio` > `2.221` → IC=+0.229 (n=637)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.221 (IC base=+0.221)

- **PATRÓN** `libro_liquidez` > `1874.461` → IC=+0.230 (n=469)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1874.461 (IC base=+0.221)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.155 (n=1266)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0073 (IC base=+0.123)

- **PATRÓN** `drift_60min` |x|≤ `0.3831` → IC=+0.142 (n=1265)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.3831 (IC base=+0.123)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.178 (n=498)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 17.0 (IC base=+0.123)

- **PATRÓN** `ibs_20min` < `0.6259` → IC=+0.169 (n=1265)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` < 0.6259 (IC base=+0.123)

- **PATRÓN** `dist_vwap_pct` < `0.1565` → IC=+0.143 (n=1215)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.1565 (IC base=+0.123)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.878` → IC=+0.173 (n=445)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 6.878 (IC base=+0.123)

- **PATRÓN** `volumen_regimen` < `0.8506` → IC=+0.143 (n=844)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 0.8506 (IC base=+0.123)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7931` → IC=+0.130 (n=765)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` < 1.7931 (IC base=+0.123)

- **PATRÓN** `volumen_spike_ratio` > `2.4874` → IC=+0.135 (n=382)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` > 2.4874 (IC base=+0.123)

- **PATRÓN** `libro_liquidez` > `9950.2818` → IC=+0.160 (n=574)
...
- **PATRÓN** `dist_vwap_pct` < `0.1956` → IC=+0.137 (n=1253)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.1956 (IC base=+0.112)
...
- **PATRÓN** `volumen_regimen` < `1.1848` → IC=+0.120 (n=1378)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_regimen` < 1.1848 (IC base=+0.112)

- **PATRÓN** `volumen_pendiente_norm` > `0.277` → IC=+0.175 (n=167)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.277 (IC base=+0.112)
...
- **PATRÓN** `libro_liquidez` > `3082.5359` → IC=+0.157 (n=459)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 3082.5359 (IC base=+0.112)

### GBM_LATE_15M_MULTIHORIZONTE#XRP#15min
- **PATRÓN** `sigma_h` > `0.0189` → IC=+0.216 (n=874)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_spike_ratio` < `2.1554` → IC=+0.217 (n=1114)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.207 (n=1390)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.203)

- **PATRÓN** `libro_liquidez` > `2801.5065` → IC=+0.207 (n=595)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2801.5065 (IC base=+0.203)

- **PATRÓN** `sigma_h` < `0.0085` → IC=+0.236 (n=448)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0085 (IC base=+0.204)

- **PATRÓN** `sigma_h` > `0.0224` → IC=+0.212 (n=609)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.212 (n=623)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.204)

- **PATRÓN** `ibs_20min` < `0.44` → IC=+0.246 (n=1346)
...
- **PATRÓN** `dist_vwap_pct` < `0.8659` → IC=+0.204 (n=1558)
...
- **PATRÓN** `volumen_regimen` > `0.6302` → IC=+0.216 (n=1344)
...
- **PATRÓN** `volumen_spike_ratio` < `2.224` → IC=+0.193 (n=1058)
...
- **PATRÓN** `libro_liquidez` > `2575.1761` → IC=+0.209 (n=896)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2575.1761 (IC base=+0.204)

### GBM_LATE_15M_PYCONFIRMADO
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.162 (n=590)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.0995` → IC=+0.154 (n=590)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.5556` → IC=+0.189 (n=1581)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.5556 (IC base=+0.146)

- **PATRÓN** `dist_vwap_pct` > `0.8458` → IC=+0.187 (n=292)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.8458 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.696` → IC=+0.177 (n=818)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 3.696 (IC base=+0.146)

- **PATRÓN** `volumen_regimen` < `0.8749` → IC=+0.164 (n=1030)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1641` → IC=+0.176 (n=492)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.1641 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` < `1.4377` → IC=+0.167 (n=568)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.4377 (IC base=+0.146)

- **PATRÓN** `volumen_spike_ratio` > `2.5508` → IC=+0.160 (n=568)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 2.5508 (IC base=+0.146)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.150 (n=2000)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.02 (IC base=+0.146)

- **PATRÓN** `libro_liquidez` > `12247.1343` → IC=+0.150 (n=590)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 12247.1343 (IC base=+0.146)

- **PATRÓN** `ballena_activa_n` < `164.0` → IC=+0.166 (n=1547)
...
- **PATRÓN** `ibs_20min` < `0.5217` → IC=+0.140 (n=1629)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` < 0.5217 (IC base=+0.102)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3378` → IC=+0.148 (n=592)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.74€ cuando `drift_60min` |x|≤ 0.3378 (IC base=+0.129)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.145 (n=530)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 7.0 (IC base=+0.129)

- **PATRÓN** `ibs_20min` < `0.6039` → IC=+0.177 (n=521)

  - _Acción_: Kelly boost +0.88€ cuando `ibs_20min` < 0.6039 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` < `0.4755` → IC=+0.146 (n=671)
...
- **PATRÓN** `volumen_regimen` > `0.7202` → IC=+0.147 (n=528)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` > 0.7202 (IC base=+0.129)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.209 (n=558)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.149` → IC=+0.207 (n=302)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.149 (IC base=+0.194)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.874` → IC=+0.224 (n=237)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.874 (IC base=+0.194)

- **PATRÓN** `volumen_regimen` < `0.8419` → IC=+0.206 (n=358)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8419 (IC base=+0.194)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.201 (n=594)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.194)

- **PATRÓN** `libro_liquidez` > `12298.0783` → IC=+0.202 (n=179)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12298.0783 (IC base=+0.194)

- **PATRÓN** `ibs_20min` < `0.0811` → IC=+0.145 (n=167)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` < 0.0811 (IC base=+0.079)

### GBM_LATE_15M_PYCONFIRMADO#SOL#15min
- **FILTRO** `ibs_20min` > `0.4167` → IC=-0.125 (n=166)

  - _Acción_: SKIP cuando `ibs_20min` > 0.4167
  - _Potencial_: sin este filtro IC_bueno=+0.158 (n=325)
...
- **PATRÓN** `ibs_20min` < `0.4167` → IC=+0.158 (n=325)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` < 0.4167 (IC base=+0.062)

- **PATRÓN** `volumen_spike_ratio` < `1.604` → IC=+0.162 (n=152)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 1.604 (IC base=+0.062)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0112` → IC=+0.207 (n=3217)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0112 (IC base=+0.169)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.179 (n=10044)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 5.0 (IC base=+0.169)

- **PATRÓN** `ibs_20min` > `0.4717` → IC=+0.216 (n=9644)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4717 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` > `0.9485` → IC=+0.207 (n=1409)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9485 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.812` → IC=+0.236 (n=3560)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.812 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` < `0.8816` → IC=+0.165 (n=4299)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.8816 (IC base=+0.169)

- **PATRÓN** `volumen_pendiente_norm` > `0.2399` → IC=+0.201 (n=1803)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2399 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `2.6068` → IC=+0.189 (n=3086)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 2.6068 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `2342.6107` → IC=+0.172 (n=6429)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 2342.6107 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `87.0` → IC=+0.195 (n=7277)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 87.0 (IC base=+0.169)

- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.187 (n=7679)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.0092 (IC base=+0.180)

- **PATRÓN** `drift_60min` |x|≤ `0.1463` → IC=+0.189 (n=3836)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5667` → IC=+0.237 (n=8723)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5667 (IC base=+0.180)

- **PATRÓN** `dist_vwap_pct` < `0.2428` → IC=+0.160 (n=5351)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` < 0.2428 (IC base=+0.180)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.952` → IC=+0.199 (n=1227)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` > 9.952 (IC base=+0.180)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.72` → IC=+0.182 (n=8450)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.72 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` < `0.7053` → IC=+0.158 (n=2646)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.7053 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` > `1.2037` → IC=+0.153 (n=2005)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.2037 (IC base=+0.180)

- **PATRÓN** `volumen_pendiente_norm` > `0.288` → IC=+0.242 (n=1142)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.288 (IC base=+0.180)

- **PATRÓN** `volumen_spike_ratio` > `2.6329` → IC=+0.193 (n=2658)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 2.6329 (IC base=+0.180)

- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.191 (n=5073)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3528` → IC=+0.189 (n=1640)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.3528 (IC base=+0.189)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.197 (n=790)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 15.0 (IC base=+0.189)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.200 (n=1107)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.987` → IC=+0.311 (n=730)
...
- **PATRÓN** `sigma_h` < `0.0078` → IC=+0.260 (n=1266)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.1282` → IC=+0.289 (n=557)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.163 (n=1608)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.149)

- **PATRÓN** `ibs_20min` > `0.3136` → IC=+0.202 (n=1543)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.74` → IC=+0.175 (n=352)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 9.74 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.23` → IC=+0.152 (n=1388)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1158` → IC=+0.162 (n=1310)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.1158 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` > `1.7583` → IC=+0.156 (n=992)
...
- **PATRÓN** `ballena_activa_n` < `477.0` → IC=+0.158 (n=1420)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 477.0 (IC base=+0.149)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.163 (n=1333)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0057 (IC base=+0.150)

- **PATRÓN** `drift_60min` |x|≤ `0.3196` → IC=+0.160 (n=1333)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.3196 (IC base=+0.150)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.175 (n=451)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 18.0 (IC base=+0.150)

- **PATRÓN** `ibs_20min` < `0.6522` → IC=+0.191 (n=1333)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` < 0.6522 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.6855` → IC=+0.152 (n=231)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.6855 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` < `0.1331` → IC=+0.166 (n=1189)
...
- **PATRÓN** `volumen_regimen` < `1.186` → IC=+0.161 (n=1333)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.186 (IC base=+0.150)

- **PATRÓN** `volumen_pendiente_norm` > `0.1514` → IC=+0.198 (n=362)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.1514 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` < `1.4159` → IC=+0.167 (n=412)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.4159 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `2.0974` → IC=+0.164 (n=560)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 2.0974 (IC base=+0.150)

- **PATRÓN** `ballena_activa_n` < `423.0` → IC=+0.156 (n=1001)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.224 (n=1628)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.218)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.224 (n=1584)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.218)

- **PATRÓN** `ibs_20min` > `0.6721` → IC=+0.256 (n=1391)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6721 (IC base=+0.218)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.811` → IC=+0.294 (n=460)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.811 (IC base=+0.218)

- **PATRÓN** `volumen_pendiente_norm` < `0.2117` → IC=+0.220 (n=1532)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.229 (n=1095)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.218)

- **PATRÓN** `libro_liquidez` > `1952.3598` → IC=+0.218 (n=519)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1952.3598 (IC base=+0.218)

- **PATRÓN** `sigma_h` < `0.0114` → IC=+0.240 (n=1441)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `volumen_spike_ratio` < `1.766` → IC=+0.233 (n=579)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.766 (IC base=+0.234)

- **PATRÓN** `volumen_spike_ratio` > `2.1985` → IC=+0.238 (n=876)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1985 (IC base=+0.234)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.245 (n=956)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.234)

- **PATRÓN** `libro_liquidez` > `1871.1484` → IC=+0.251 (n=653)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1871.1484 (IC base=+0.234)

- **PATRÓN** `ballena_activa_n` < `55.0` → IC=+0.232 (n=1239)
...
- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.154 (n=1338)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0076 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.4454` → IC=+0.153 (n=1338)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.142 (n=616)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` < 7.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` < `0.6988` → IC=+0.184 (n=1338)

  - _Acción_: Kelly boost +0.92€ cuando `ibs_20min` < 0.6988 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `0.788` → IC=+0.141 (n=1546)
...
- **PATRÓN** `volumen_regimen` < `0.6971` → IC=+0.146 (n=589)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.6971 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` > `1.1852` → IC=+0.145 (n=446)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 1.1852 (IC base=+0.136)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4426` → IC=+0.151 (n=1266)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.4426 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `10957.774` → IC=+0.205 (n=446)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 10957.774 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `180.0` → IC=+0.143 (n=1263)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.132 (n=1670)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.111)

- **PATRÓN** `ibs_20min` > `0.4725` → IC=+0.190 (n=1629)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` > 0.4725 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` > `1.0688` → IC=+0.204 (n=333)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0688 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.451` → IC=+0.234 (n=611)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.125 (n=1645)

  - _Acción_: Kelly boost +0.62€ cuando `libro_spread` < 0.02 (IC base=+0.111)

- **PATRÓN** `libro_liquidez` > `2913.9614` → IC=+0.249 (n=543)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2913.9614 (IC base=+0.111)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.129 (n=1250)

  - _Acción_: Kelly boost +0.65€ cuando `ballena_activa_n` < 54.0 (IC base=+0.111)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.172 (n=534)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0058 (IC base=+0.110)

- **PATRÓN** `drift_60min` |x|≤ `0.1315` → IC=+0.160 (n=533)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.1315 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.123 (n=1647)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 5.0 (IC base=+0.110)

- **PATRÓN** `ibs_20min` < `0.6364` → IC=+0.202 (n=1600)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6364 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.217` → IC=+0.128 (n=1287)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.217 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.46` → IC=+0.124 (n=1547)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 3.46 (IC base=+0.110)

- **PATRÓN** `volumen_regimen` < `0.72` → IC=+0.150 (n=704)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5228` → IC=+0.131 (n=480)
  ...
  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` > 2.5228 (IC base=+0.110)
...
- **PATRÓN** `volumen_regimen` < `1.0716` → IC=+0.210 (n=1438)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.0716 (IC base=+0.210)

- **PATRÓN** `volumen_regimen` > `0.6363` → IC=+0.218 (n=1634)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5107` → IC=+0.238 (n=525)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5107 (IC base=+0.210)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.219 (n=1710)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.210)

- **PATRÓN** `libro_liquidez` > `2616.3022` → IC=+0.224 (n=1089)
...
- **PATRÓN** `ibs_20min` < `0.5208` → IC=+0.254 (n=1747)
...
- **PATRÓN** `dist_vwap_pct` < `0.8782` → IC=+0.203 (n=1924)
...
- **PATRÓN** `ibs_20min` > `0.9428` → IC=+0.213 (n=934)
...
- **PATRÓN** `dist_vwap_pct` < `0.4929` → IC=+0.143 (n=1667)
...
- **PATRÓN** `volumen_regimen` > `0.902` → IC=+0.161 (n=1194)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4564` → IC=+0.156 (n=923)
...
- **PATRÓN** `libro_liquidez` > `8154.7046` → IC=+0.151 (n=1271)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 8154.7046 (IC base=+0.150)
...
- **FILTRO** `dist_vwap_pct` > `0.1781` → IC=-0.155 (n=27)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.112 (n=292)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.0685` → IC=+0.199 (n=91)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.0685 (IC base=-0.005)

- **PATRÓN** `volumen_spike_ratio` < `2.5683` → IC=+0.129 (n=211)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` < 2.5683 (IC base=-0.005)
...
- **PATRÓN** `ibs_20min` < `0.7006` → IC=+0.129 (n=87)

  - _Acción_: Kelly boost +0.65€ cuando `ibs_20min` < 0.7006 (IC base=-0.021)

- **PATRÓN** `dist_vwap_pct` < `0.1197` → IC=+0.129 (n=87)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.1197 (IC base=-0.021)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.924` → IC=+0.141 (n=37)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 2.924 (IC base=-0.021)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6357` → IC=+0.192 (n=24)
  ...
  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 2.6357 (IC base=-0.021)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.133 (n=77)

  - _Acción_: Kelly boost +0.66€ cuando `libro_spread` < 0.02 (IC base=-0.021)
...
### LEADLAG_BTC_XRP_15M
- **PATRÓN** `py_entrada` > `0.5` → IC=+0.123 (n=661)

  - _Acción_: Kelly boost +0.61€ cuando `py_entrada` > 0.5 (IC base=+0.106)

- **PATRÓN** `libro_liquidez` > `2900.177` → IC=+0.171 (n=223)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.029 (n=1695)
...
- **PATRÓN** `liq_usd_total` > `80505.33` → IC=+0.182 (n=86)

  - _Acción_: Kelly boost +0.91€ cuando `liq_usd_total` > 80505.33 (IC base=+0.032)

- **PATRÓN** `py_entrada` < `0.495` → IC=+0.151 (n=84)

  - _Acción_: Kelly boost +0.76€ cuando `py_entrada` < 0.495 (IC base=+0.032)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=7383)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.172 (n=3209)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=10306)

- **FILTRO** `py_entrada` > `0.595` → IC=-0.171 (n=3464)

  - _Acción_: SKIP cuando `py_entrada` > 0.595
  - _Potencial_: sin este filtro IC_bueno=+0.033 (n=10478)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.45` → IC=-0.210 (n=568)

  - _Acción_: SKIP cuando `py_entrada` < 0.45
  - _Potencial_: sin este filtro IC_bueno=+0.098 (n=1765)

- **FILTRO** `py_entrada` > `0.64` → IC=-0.154 (n=622)

  - _Acción_: SKIP cuando `py_entrada` > 0.64
  - _Potencial_: sin este filtro IC_bueno=+0.063 (n=1875)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.46` → IC=-0.203 (n=583)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.106 (n=1799)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.206 (n=611)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.062 (n=1878)

- **FILTRO** `ibs_20min` > `0.282` → IC=-0.164 (n=622)

  - _Acción_: SKIP cuando `ibs_20min` > 0.282
  - _Potencial_: sin este filtro IC_bueno=+0.049 (n=1867)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.48` → IC=-0.174 (n=556)

  - _Acción_: SKIP cuando `py_entrada` < 0.48
  - _Potencial_: sin este filtro IC_bueno=+0.084 (n=1763)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.186 (n=615)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.049 (n=1876)

### MOMENTUM_IBS_15M_FADE
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=2705)

- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)

  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=2870)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=2876)
...
- **FILTRO** `ibs_20min` > `0.1725` → IC=-0.136 (n=116)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=356)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=9641)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.077 (n=21395)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.272 (n=7549)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=23487)

- **FILTRO** `ibs_7min` < `0.289` → IC=-0.234 (n=7759)

  - _Acción_: SKIP cuando `ibs_7min` < 0.289
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=23277)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.156 (n=10518)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=20518)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.229 (n=9592)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=29196)

- **FILTRO** `ibs_7min` > `0.2941` → IC=-0.180 (n=9679)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2941
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=29109)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.304 (n=1220)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=3899)

- **FILTRO** `ibs_7min` < `0.7097` → IC=-0.249 (n=1687)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7097
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3432)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.182 (n=1185)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=3934)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.260 (n=1653)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=5022)

- **FILTRO** `drift_7min_pct` |x|> `0.1117` → IC=-0.125 (n=2265)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1117
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=4410)

- **FILTRO** `ibs_7min` > `0.7937` → IC=-0.208 (n=1668)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7937
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=5007)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.137 (n=1265)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=4103)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.250 (n=1288)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=4080)

- **FILTRO** `ibs_7min` < `0.7501` → IC=-0.191 (n=1342)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7501
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4026)

- **FILTRO** `ballena_activa_n` > `160.0` → IC=-0.171 (n=1335)

  - _Acción_: SKIP cuando `ballena_activa_n` > 160.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=4033)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.253 (n=1348)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=4081)

- **FILTRO** `ibs_7min` > `0.2605` → IC=-0.178 (n=1357)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2605
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=4072)

- **FILTRO** `ballena_activa_n` > `154.0` → IC=-0.186 (n=1349)

  - _Acción_: SKIP cuando `ballena_activa_n` > 154.0
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=4080)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.165 (n=1388)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.079 (n=3457)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.305 (n=1187)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=3658)

- **FILTRO** `ibs_7min` < `0.1951` → IC=-0.257 (n=1211)

  - _Acción_: SKIP cuando `ibs_7min` < 0.1951
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=3634)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.215 (n=1137)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=3708)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.239 (n=1648)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.016 (n=5472)

- **FILTRO** `ibs_7min` > `0.7545` → IC=-0.179 (n=1779)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7545
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=5341)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `py_entrada` < `0.37` → IC=-0.233 (n=1513)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=3608)

- **FILTRO** `ibs_7min` < `0.7425` → IC=-0.181 (n=1279)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7425
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=3842)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.168 (n=1256)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3865)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.266 (n=1154)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=4051)

- **FILTRO** `ibs_7min` > `0.2753` → IC=-0.174 (n=1301)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2753
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=3904)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.190 (n=1280)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=3925)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.262 (n=1299)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=4129)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.240 (n=1341)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=4087)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.178 (n=1774)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=5599)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.34` → IC=-0.271 (n=1221)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=3934)

- **FILTRO** `ibs_7min` < `0.7037` → IC=-0.227 (n=1287)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7037
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=3868)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.212 (n=1239)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=3916)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.204 (n=1700)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.010 (n=5286)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.190 (n=27)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=723)
...
- **FILTRO** `streak_len` > `3.0` → IC=-0.147 (n=15)
  ...
  - _Acción_: SKIP cuando `streak_len` > 3.0
  - _Potencial_: sin este filtro IC_bueno=+0.037 (n=692)

### STRUCT_NO_15M#BTC#15min
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=2817)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `16.915` → IC=+0.274 (n=397)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 16.915 (IC base=+0.186)

- **PATRÓN** `libro_liquidez` > `2980.0928` → IC=+0.193 (n=1009)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 2980.0928 (IC base=+0.186)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.207 (n=828)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `15.662` → IC=+0.220 (n=23)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 15.662 (IC base=+0.009)
...
- **PATRÓN** `ibs_15` < `0.1176` → IC=+0.158 (n=451)
...
- **PATRÓN** `drift_15min` |x|≤ `0.7826` → IC=+0.230 (n=568)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.7826 (IC base=+0.225)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2011` → IC=+0.253 (n=293)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.239 (n=293)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.225)

- **PATRÓN** `ibs_15` < `0.2693` → IC=+0.277 (n=568)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
...
- **PATRÓN** `sigma_ewma_delta_pct` < `12.189` → IC=+0.229 (n=688)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 12.189 (IC base=+0.225)

- **PATRÓN** `libro_liquidez` > `3552.4266` → IC=+0.225 (n=645)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3552.4266 (IC base=+0.225)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6079 sube el IC de +0.186 a +0.263 en UPDOWN_GBM#15min (n=1513). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1176 sube el IC de +0.045 a +0.158 en UPDOWN_GBM#XRP#15min (n=451). Ya aplicado como kelly_boost=+0.79€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.35 sube el IC de -0.038 a +0.273 en UPDOWN_GBM_15M_TARDIO (n=1622). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.2693 sube el IC de +0.225 a +0.277 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=568). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 24327 | -0.090 | -3062.06€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1431 | -0.047 | -221.52€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min |
02€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 21937 | +0.021 | +377.13€ | 0 | 0 |
| ✅ UPDOWN_GBM#60min | 2111 | +0.008 | +23.34€ | 1 | 0 |
| ✅ UPDOWN_GBM#BNB | 3470 | +0.068 | +371.96€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 574 | +0.149 | +225.21€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#240min | 29 | -0.016 | -0.66€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#5min | 2867 | +0.052 | +147.41€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 6465 | +0.035 | +431.10€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 1194 | +0.080 | +257.70€ | 0 | 10 |
| ✅ UPDOWN_GBM#BTC#240min | 352 | +0.020 | +7.24€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#5min | 3919 | +0.032 | +148.21€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#60min | 949 | +0.005 | +17.13€ | 1 | 0 |
| ✅ UPDOWN_GBM#BTC#daily | 51 | -0.104 | +0.80€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 4067 | +0.040 | +240.42€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 534 | +0.140 | +193.05€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#240min | 25 | -0.018 | -1.67€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 3508 | +0.025 | +49.03€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 7429 | +0.018 | +281.96€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 2446 | +0.045 | +267.35€ | 0 | 11 |
| ✅ UPDOWN_GBM#ETH#240min | 340 | +0.006 | +7.22€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 3878 | +0.006 | +5.73€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 722 | +0.005 | -1.91€ | 1 | 0 |
| ✅ UPDOWN_GBM#ETH#daily | 43 | -0.144 | +3.56€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 8392 | +0.016 | +219.10€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 2330 | +0.024 | +159.86€ | 0 | 11 |
| ✅ UPDOWN_GBM#SOL#240min | 334 | -0.006 | -3.18€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 5251 | +0.014 | +57.00€ | 1 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 440 | +0.020 | +8.11€ | 0 | 1 |
| ✅ UPDOWN_GBM#SOL#daily | 37 | -0.167 | -2.70€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 5055 | +0.035 | +527.54€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 2330 | +0.078 | +560.73€ | 0 | 11 |
| ✅ UPDOWN_GBM#XRP#240min | 211 | -0.002 | -2.94€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 2514 | -0.002 | -30.26€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 11349 | -0.043 | +2308.45€ | 2 | 8 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 11349 | -0.043 | +2308.45€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 687 | -0.043 | +329.33€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 687 | -0.043 | +329.33€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 2125 | -0.123 | +20.46€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 2125 | -0.123 | +20.46€ | 4 | 6 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 328 | +0.173 | +208.77€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 328 | +0.173 | +208.77€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 1250 | +0.201 | +717.90€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 1250 | +0.201 | +717.90€ | 2 | 23 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 3472 | -0.065 | +527.13€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 3472 | -0.065 | +527.13€ | 3 | 6 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 3487 | -0.080 | +504.86€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 3487 | -0.080 | +504.86€ | 3 | 4 |
...
**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  ...
  - _Estado_: 522 celda(s) pasan gate riguroso completo de 2186 evaluadas (n>=40) y 3183 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.025 < 0.08 — monitorear
  - _Datos_: n=2327 IC=+0.025 PNL=+161.44€
...
**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  ...
  - _Estado_: alineada_con_outcome_prev IC=+0.098 n=274/60 | contraria IC=+0.150 n=272 | gap=-0.052 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=721/40 IC=+0.005 PNL=-2.39€ | BTC#60min: n=948/40 IC=+0.004 PNL=+16.69€ | SOL#60min: n=439/40 IC=+0.019 PNL=+7.72€

**⏳ H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  - _Umbral_: n≥40 tras 2 losses y gap(IC_tras_win - IC_tras_2loss)≥0.05
  - _Acción_: Reducir stake (no desactivar) 1-2h tras 2 derrotas consecutivas en el mismo subtype
  - _Estado_: tras_win IC=+0.051 n=319801 | tras_1loss IC=+0.077 n=248932 | tras_2loss IC=+0.046 n=105316/40 | gap=+0.005 (umbral 0.05)
...
**🟡 H-24H-GBM-BUYYES-MADRUGADA** — GBM BUY_YES en madrugada europea (05-07h UTC) — señal alcista
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.174 > 0.08 con n=308 PNL=+191.85€
  - _Datos_: n=308 IC=+0.174 PNL=+191.85€
...
**〰️ H-CUSTOM-GBM-60MIN-BUYYES** — GBM 60min BUY_YES — ¿edge superior al BUY_NO?
  ...
  - _Estado_: n=1510 IC=+0.018 PNL=+18.16€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1510 IC=+0.018 PNL=+18.16€

**〰️ H-CUSTOM-GBM-60MIN-BUYNO** — GBM 60min BUY_NO — tracking por separado
  - _Hipótesis_: En 15min BUY_NO tiene IC=+0.119. ¿Se repite en 60min? Datos actuales: 8/14 (57%) IC=+0.044 — positivo pero débil. Puede ser que 60min requiera dirección alcista (BUY_YES) y no bajista.
  - _Umbral_: n≥30 para confirmar dirección
  - _Acción_: Si IC<0.05 con n≥30 → en 60min priorizar solo BUY_YES; si IC>0.08 → igualar al BUY_YES
  - _Estado_: n=598 IC=-0.020 PNL=+3.86€ — sin señal clara aún (umbral IC: min=0.05 max=None)
  - _Datos_: n=598 IC=-0.020 PNL=+3.86€
...
**🟡 H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.186 > 0.1 con n=2015 PNL=+1260.99€
  - _Datos_: n=2015 IC=+0.186 PNL=+1260.99€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=1193 IC=+0.080 PNL=+257.20€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=1193 IC=+0.080 PNL=+257.20€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=12097 IC=+0.057 PNL=+1519.22€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=12097 IC=+0.057 PNL=+1519.22€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=1788 IC=+0.046 PNL=+185.32€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1788 IC=+0.046 PNL=+185.32€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=16964 IC=-0.137 PNL=+1183.64€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=16964 IC=-0.137 PNL=+1183.64€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=1838 IC=+0.138 PNL=+995.92€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=1838 IC=+0.138 PNL=+995.92€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.238 < -0.1 con n=1668 PNL=-186.53€
  - _Datos_: n=1668 IC=-0.238 PNL=-186.53€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=8435 IC=+0.176 PNL=-981.99€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=8435 IC=+0.176 PNL=-981.99€
