# Hipótesis automáticas — 2026-07-12 14:48 UTC
_Generado por shadow_postmortem.py sobre 11038 resoluciones (PNL=+2015.66€)_
...
- **PATRÓN** `py_entrada` > `0.585` → IC=+0.244 (n=350)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.585 (IC base=+0.210)

- **PATRÓN** `libro_liquidez` > `2379.73` → IC=+0.219 (n=336)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2379.73 (IC base=+0.210)
...
### GBM_LATE_15M
- **PATRÓN** `sigma_h` > `0.0151` → IC=+0.138 (n=440)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0151 (IC base=+0.116)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.123 (n=1342)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 6.0 (IC base=+0.116)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.122 (n=1338)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` < 17.0 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` > `0.4454` → IC=+0.150 (n=38)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.4454 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` < `0.1403` → IC=+0.199 (n=71)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` < 0.1403 (IC base=+0.116)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.267` → IC=+0.278 (n=16)
...
- **PATRÓN** `dist_vwap_pct` < `0.505` → IC=+0.172 (n=132)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.505 (IC base=+0.111)
...
- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.194 (n=321)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0049 (IC base=+0.173)
...
- **PATRÓN** `dist_vwap_pct` > `0.4503` → IC=+0.206 (n=32)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4503 (IC base=+0.173)

- **PATRÓN** `dist_vwap_pct` < `0.1221` → IC=+0.204 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1221 (IC base=+0.173)
...
- **PATRÓN** `sigma_h` > `0.0124` → IC=+0.150 (n=141)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0124 (IC base=+0.083)

- **PATRÓN** `dist_vwap_pct` < `0.3497` → IC=+0.136 (n=119)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.3497 (IC base=+0.083)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 921 | +0.179 | +1.30€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 593 | +0.208 | +5.96€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 3605 | +0.114 | +1190.13€ | 0 | 9 |
| ✅ GBM_LATE_15M#15min | 3605 | +0.114 | +1190.13€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR | 966 | +0.164 | +530.48€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 966 | +0.164 | +530.48€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO | 1130 | +0.081 | +274.87€ | 1 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1130 | +0.081 | +274.87€ | 0 | 0 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.091 n=6238 | tras_1loss IC=+0.045 n=4663 | tras_2loss IC=+0.011 n=2092/40 | gap=+0.080 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8580 IC=+0.097 PNL=+2091.54€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8580 IC=+0.097 PNL=+2091.54€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.303 > 0.2 con n=140 PNL=+94.25€
  - _Datos_: n=140 IC=+0.303 PNL=+94.25€
