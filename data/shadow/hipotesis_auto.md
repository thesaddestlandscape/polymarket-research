# Hipótesis automáticas — 2026-07-23 19:18 UTC
_Generado por shadow_postmortem.py sobre 31490 resoluciones (PNL=+6883.77€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.204 (n=2064)
...
- **PATRÓN** `py_entrada` > `0.605` → IC=+0.272 (n=441)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.605 (IC base=+0.215)

- **PATRÓN** `libro_liquidez` > `11735.8708` → IC=+0.216 (n=146)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11735.8708 (IC base=+0.215)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.223 (n=398)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.715` → IC=+0.350 (n=205)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.715 (IC base=+0.214)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.215 (n=552)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.214)

- **PATRÓN** `libro_liquidez` > `4253.2534` → IC=+0.216 (n=290)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4253.2534 (IC base=+0.214)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.170 (n=113)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.595` → IC=+0.162 (n=72)

  - _Acción_: Kelly boost +0.81€ cuando `py_entrada` < 0.595 (IC base=+0.148)

- **PATRÓN** `py_entrada` > `0.595` → IC=+0.167 (n=88)

  - _Acción_: Kelly boost +0.83€ cuando `py_entrada` > 0.595 (IC base=+0.148)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.260 (n=152)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.675` → IC=+0.336 (n=291)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.675 (IC base=+0.238)

- **PATRÓN** `libro_liquidez` > `2367.8144` → IC=+0.255 (n=145)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2367.8144 (IC base=+0.238)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.216 (n=227)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1613.1563` → IC=+0.215 (n=462)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1613.1563 (IC base=+0.212)

### FAVORITO_CONFIRMADO#SOL#60min
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.239 (n=44)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.605` → IC=+0.196 (n=77)

  - _Acción_: Kelly boost +0.98€ cuando `py_entrada` > 0.605 (IC base=+0.184)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.129 (n=2099)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 8.0 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `0.4622` → IC=+0.127 (n=298)
...
- **PATRÓN** `sigma_h` > `0.0133` → IC=+0.129 (n=926)
...
- **FILTRO** `sigma_ewma_delta_pct` > `9.228` → IC=-0.309 (n=19)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.075 (n=433)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.136 (n=262)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 17.0 (IC base=+0.100)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.154 (n=709)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.1129` → IC=+0.136 (n=306)
...
- **PATRÓN** `sigma_h` > `0.0272` → IC=+0.164 (n=254)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.0272 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.165 (n=267)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.771` → IC=+0.141 (n=421)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` < 3.771 (IC base=+0.151)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0117` → IC=+0.216 (n=477)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0117 (IC base=+0.150)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.172 (n=1299)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 8.0 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.2754` → IC=+0.204 (n=407)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2754 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.735` → IC=+0.254 (n=441)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.735 (IC base=+0.150)

- **PATRÓN** `sigma_h` > `0.0043` → IC=+0.147 (n=1601)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0043 (IC base=+0.130)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.144 (n=1109)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.9884` → IC=+0.164 (n=117)
  ...
  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.9884 (IC base=+0.130)

- **PATRÓN** `dist_vwap_pct` < `0.7417` → IC=+0.131 (n=1611)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.138 (n=346)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 6.0 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` > `0.4457` → IC=+0.211 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4457 (IC base=+0.110)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.151 (n=356)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.781` → IC=+0.291 (n=156)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.781 (IC base=+0.127)

- **PATRÓN** `sigma_h` < `0.0081` → IC=+0.147 (n=168)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.241 (n=350)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0086 (IC base=+0.237)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.255 (n=316)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.089` → IC=+0.362 (n=92)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0135` → IC=+0.162 (n=456)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` > 0.0135 (IC base=+0.112)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.127 (n=1219)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 8.0 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `0.4476` → IC=+0.155 (n=282)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.4476 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.72` → IC=+0.286 (n=208)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.72 (IC base=+0.112)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.140 (n=270)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 8.0 (IC base=+0.090)

- **PATRÓN** `dist_vwap_pct` > `0.439` → IC=+0.173 (n=50)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.439 (IC base=+0.090)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.244` → IC=+0.223 (n=63)
...
- **PATRÓN** `sigma_h` < `0.0046` → IC=+0.122 (n=146)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` < 0.0046 (IC base=+0.065)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.699` → IC=-0.167 (n=16)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.082 (n=357)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.238` → IC=+0.262 (n=120)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.238 (IC base=+0.075)
...
- **PATRÓN** `sigma_h` < `0.0212` → IC=+0.194 (n=387)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0212 (IC base=+0.187)

- **PATRÓN** `sigma_h` > `0.0107` → IC=+0.194 (n=393)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0107 (IC base=+0.187)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.192 (n=465)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 5.0 (IC base=+0.187)

- **PATRÓN** `dist_vwap_pct` > `0.5947` → IC=+0.260 (n=73)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5947 (IC base=+0.187)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.119` → IC=+0.315 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.119 (IC base=+0.187)

- **PATRÓN** `sigma_h` < `0.0193` → IC=+0.183 (n=453)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0193 (IC base=+0.176)

- **PATRÓN** `sigma_h` > `0.0134` → IC=+0.176 (n=344)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0134 (IC base=+0.176)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.194 (n=178)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` > 18.0 (IC base=+0.176)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `10.154` → IC=+0.176 (n=505)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` < 10.154 (IC base=+0.176)
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 878 | +0.128 | +43.04€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 878 | +0.128 | +43.04€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 10 | +0.083 | +2.04€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 10 | +0.083 | +2.04€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 333 | +0.133 | +10.77€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 333 | +0.133 | +10.77€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 336 | +0.145 | +14.99€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min |