# Hipótesis automáticas — 2026-09-04 03:01 UTC
_Generado por shadow_postmortem.py sobre 275096 resoluciones (PNL=+26799.88€)_
...
- **FILTRO** `restante_s_al_confirmar` < `146.54` → IC=-0.299 (n=3707)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=11126)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.262 (n=701)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.261 (n=708)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.258)

- **PATRÓN** `libro_liquidez` > `1943.5814` → IC=+0.266 (n=677)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.195 (n=5206)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` < 15.0 (IC base=+0.189)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.202 (n=2945)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.176 (n=1315)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.238 (n=1185)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.186 (n=1287)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.206 (n=17491)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.148 (n=2427)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3569` → IC=+0.127 (n=3682)
...
- **PATRÓN** `ballena_activa_n` < `126.0` → IC=+0.214 (n=1430)
...
- **FILTRO** `libro_spread` > `0.02` → IC=-0.198 (n=114)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.090 (n=915)
...
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.172 (n=1517)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0094 (IC base=+0.071)

- **PATRÓN** `ibs_20min` > `0.2908` → IC=+0.139 (n=4547)
...
- **PATRÓN** `volumen_regimen` > `0.6814` → IC=+0.218 (n=1319)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6814 (IC base=+0.071)

- **PATRÓN** `volumen_pendiente_norm` < `0.115` → IC=+0.213 (n=2148)
...
- **PATRÓN** `ballena_activa_n` < `254.0` → IC=+0.154 (n=105)
  ...
  - _Acción_: Kelly boost +0.77€ cuando `ballena_activa_n` < 254.0 (IC base=-0.015)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1246` → IC=+0.222 (n=322)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1246 (IC base=+0.220)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.253 (n=273)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.220)

- **PATRÓN** `ibs_20min` > `0.7091` → IC=+0.253 (n=653)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7091 (IC base=+0.220)
...
- **FILTRO** `ibs_20min` < `0.8095` → IC=-0.141 (n=648)

  - _Acción_: SKIP cuando `ibs_20min` < 0.8095
  - _Potencial_: sin este filtro IC_bueno=+0.289 (n=334)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.257 (n=352)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.249)

- **PATRÓN** `ibs_20min` > `0.8915` → IC=+0.323 (n=494)
...
- **PATRÓN** `sigma_h` > `0.0106` → IC=+0.208 (n=1283)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0106 (IC base=+0.162)

- **PATRÓN** `drift_60min` |x|≤ `0.3422` → IC=+0.169 (n=3378)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `dist_vwap_pct` > `0.7903` → IC=+0.237 (n=847)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1057` → IC=+0.183 (n=1432)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.1057 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` < `2.324` → IC=+0.162 (n=3142)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.324 (IC base=+0.162)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.165 (n=3923)
...
- **PATRÓN** `ballena_activa_n` < `143.0` → IC=+0.178 (n=2754)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.187 (n=1641)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.2292` → IC=+0.176 (n=2699)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` < 0.2292 (IC base=+0.181)
...
- **PATRÓN** `volumen_regimen` < `1.1748` → IC=+0.170 (n=2703)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 1.1748 (IC base=+0.181)

- **PATRÓN** `volumen_regimen` > `0.6222` → IC=+0.163 (n=2703)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.6222 (IC base=+0.181)
...
- **PATRÓN** `volumen_spike_ratio` < `1.575` → IC=+0.174 (n=1298)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.575 (IC base=+0.181)

- **PATRÓN** `volumen_spike_ratio` > `2.658` → IC=+0.201 (n=983)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.658 (IC base=+0.181)

- **PATRÓN** `ballena_activa_n` < `157.0` → IC=+0.182 (n=2537)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5207` → IC=+0.138 (n=658)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4667` → IC=+0.229 (n=658)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4667 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` < `0.2523` → IC=+0.149 (n=620)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.2523 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.22` → IC=+0.179 (n=257)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 3.22 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` < `1.1973` → IC=+0.151 (n=658)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3492` → IC=+0.206 (n=175)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3492 (IC base=+0.134)

- **PATRÓN** `libro_liquidez` > `1427.0271` → IC=+0.152 (n=587)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 1427.0271 (IC base=+0.134)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.193 (n=847)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.200 (n=2090)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.234` → IC=+0.201 (n=1724)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.234 (IC base=+0.164)
...
- **PATRÓN** `volumen_regimen` < `0.8847` → IC=+0.158 (n=2124)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.8847 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` > `1.2367` → IC=+0.154 (n=1062)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.2367 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.1676` → IC=+0.188 (n=1247)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.1676 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` > `1.8759` → IC=+0.172 (n=2850)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.8759 (IC base=+0.164)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.168 (n=4343)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.02 (IC base=+0.164)

- **PATRÓN** `libro_liquidez` > `3848.1598` → IC=+0.187 (n=1534)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 3848.1598 (IC base=+0.164)

- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.211 (n=2166)
...
- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.163 (n=253)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` > 0.0068 (IC base=+0.159)
...
- **PATRÓN** `ibs_20min` > `0.3266` → IC=+0.205 (n=754)
...
- **PATRÓN** `volumen_regimen` < `1.2673` → IC=+0.167 (n=754)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0723` → IC=+0.174 (n=633)
...
- **PATRÓN** `sigma_h` > `0.0073` → IC=+0.225 (n=441)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0073 (IC base=+0.203)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.238 (n=315)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.203)

- **PATRÓN** `ibs_20min` > `0.6809` → IC=+0.250 (n=591)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6809 (IC base=+0.203)
...
- **PATRÓN** `sigma_h` < `0.0071` → IC=+0.160 (n=669)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0071 (IC base=+0.145)

- **PATRÓN** `drift_60min` |x|≤ `0.1445` → IC=+0.147 (n=335)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.1445 (IC base=+0.145)
...
- **PATRÓN** `dist_vwap_pct` > `0.3748` → IC=+0.191 (n=309)
  ...
  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.3748 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.369` → IC=+0.173 (n=347)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 4.369 (IC base=+0.145)
...
- **PATRÓN** `volumen_regimen` > `1.2073` → IC=+0.160 (n=254)
  ...
  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 1.2073 (IC base=+0.145)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1486` → IC=+0.202 (n=327)
...
- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.143 (n=502)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `ballena_activa_n` < 54.0 (IC base=+0.095)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1685` → IC=+0.235 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1685 (IC base=+0.198)

- **PATRÓN** `volumen_spike_ratio` > `1.828` → IC=+0.205 (n=551)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.828 (IC base=+0.198)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.207 (n=1005)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4756` → IC=+0.145 (n=558)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.4756 (IC base=+0.132)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.179 (n=1452)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=4368)
...
- **FILTRO** `py_entrada` > `0.68` → IC=-0.220 (n=4405)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=13552)

- **FILTRO** `ibs_7min` > `0.7134` → IC=-0.172 (n=4489)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7134
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=13468)
...
- **FILTRO** `py_entrada` > `0.64` → IC=-0.247 (n=618)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=2052)

- **FILTRO** `ballena_activa_n` > `106.0` → IC=-0.174 (n=906)

  - _Acción_: SKIP cuando `ballena_activa_n` > 106.0
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=1764)
...
- **FILTRO** `ibs_7min` > `0.7742` → IC=-0.163 (n=784)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=2354)
...
### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.165 (n=177)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.007 (IC base=+0.126)

- **PATRÓN** `drift_60min` |x|≤ `0.1675` → IC=+0.128 (n=466)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.64€ cuando `drift_60min` |x|≤ 0.1675 (IC base=+0.126)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0596` → IC=+0.128 (n=530)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.64€ cuando `delta_ratio_macro` |x|> 0.0596 (IC base=+0.126)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.128 (n=563)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 4.0 (IC base=+0.126)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.156 (n=239)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 6.0 (IC base=+0.126)

- **PATRÓN** `ibs_15` > `0.5794` → IC=+0.214 (n=530)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5794 (IC base=+0.126)

- **PATRÓN** `dist_vwap_pct` > `0.4298` → IC=+0.152 (n=136)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.4298 (IC base=+0.126)

- **PATRÓN** `dist_vwap_pct` < `0.5472` → IC=+0.122 (n=549)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.5472 (IC base=+0.126)

- **PATRÓN** `sigma_ewma_delta_pct` > `15.311` → IC=+0.223 (n=153)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 15.311 (IC base=+0.126)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.136 (n=544)

  - _Acción_: Kelly boost +0.68€ cuando `libro_spread` < 0.01 (IC base=+0.126)

- **PATRÓN** `libro_liquidez` > `3042.6038` → IC=+0.151 (n=353)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 3042.6038 (IC base=+0.126)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0615` → IC=+0.135 (n=61)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.67€ cuando `delta_ratio_macro` |x|> 0.0615 (IC base=+0.115)

- **PATRÓN** `hora_utc` < `4.0` → IC=+0.239 (n=21)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 4.0 (IC base=+0.115)

- **PATRÓN** `ibs_15` > `0.5385` → IC=+0.272 (n=55)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5385 (IC base=+0.115)
...
- **PATRÓN** `libro_liquidez` > `3008.8466` → IC=+0.283 (n=21)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3008.8466 (IC base=+0.115)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0487` → IC=+0.148 (n=140)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.74€ cuando `delta_ratio_macro` |x|> 0.0487 (IC base=+0.106)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.157 (n=65)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 6.0 (IC base=+0.106)

- **PATRÓN** `ibs_15` > `0.5676` → IC=+0.201 (n=125)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5676 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` > `0.4401` → IC=+0.214 (n=47)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4401 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.798` → IC=+0.274 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.798 (IC base=+0.106)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.131 (n=120)

  - _Acción_: Kelly boost +0.66€ cuando `libro_spread` < 0.01 (IC base=+0.106)

- **PATRÓN** `libro_liquidez` > `2492.3796` → IC=+0.161 (n=125)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 2492.3796 (IC base=+0.106)

- **PATRÓN** `ballena_activa_n` < `21.0` → IC=+0.197 (n=31)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 21.0 (IC base=+0.106)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.5794 sube el IC de +0.126 a +0.214 en UPDOWN_GBM#15min (n=530). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#SOL#15min**: dentro de BUY_YES, IBS > 0.5385 sube el IC de +0.115 a +0.272 en UPDOWN_GBM#SOL#15min (n=55). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_YES, IBS > 0.5676 sube el IC de +0.106 a +0.201 en UPDOWN_GBM#XRP#15min (n=125). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 14833 | -0.112 | -2517.66€ | 1 | 0 |
...
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 4112 | -0.071 | +1808.01€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 1170 | -0.009 | +852.34€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 2942 | -0.096 | +955.67€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 4112 | -0.071 | +1808.01€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 1170 | -0.009 | +852.34€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 2942 | -0.096 | +955.67€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 47761 | +0.113 | -3035.36€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 7981 | +0.184 | -286.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 162 | -0.110 | -54.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 35870 | +0.098 | -2620.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3748 | +0.116 | -73.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 6025 | +0.082 | -806.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 27 | -0.121 | +3.75€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 