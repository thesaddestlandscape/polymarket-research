# Hipótesis automáticas — 2026-08-27 11:18 UTC
_Generado por shadow_postmortem.py sobre 173710 resoluciones (PNL=+12602.48€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `147.36` → IC=-0.285 (n=2415)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 147.36
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=7247)
...
- **FILTRO** `restante_s_al_confirmar` < `97.92` → IC=-0.427 (n=301)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 97.92
  - _Potencial_: sin este filtro IC_bueno=-0.089 (n=904)

### BALLENAS_TARDIAS#ETH#5min
- **FILTRO** `restante_s_al_confirmar` < `160.13` → IC=-0.181 (n=654)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 160.13
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=1964)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `154.96` → IC=-0.257 (n=565)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 154.96
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=1695)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `117.99` → IC=-0.381 (n=419)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 117.99
  - _Potencial_: sin este filtro IC_bueno=-0.133 (n=1257)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.189 (n=5446)

  - _Acción_: Kelly boost +0.94€ cuando `py_entrada` > 0.7 (IC base=+0.098)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.170 (n=1585)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.01 (IC base=+0.098)

- **PATRÓN** `libro_liquidez` > `2385.1288` → IC=+0.174 (n=1529)
...
- **PATRÓN** `py_entrada` < `0.34` → IC=+0.258 (n=3351)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.209 (n=788)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.133 (n=1220)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 5.0 (IC base=+0.122)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.125 (n=1043)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.197 (n=644)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.192 (n=4526)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 5.0 (IC base=+0.186)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.190 (n=3882)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.193 (n=1109)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` > 5.0 (IC base=+0.182)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.182 (n=958)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.167 (n=1522)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.245 (n=1200)
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.193 (n=1388)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 11.0 (IC base=+0.185)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.210 (n=1517)
...
- **PATRÓN** `restante_min` < `3.96` → IC=+0.136 (n=1504)

  - _Acción_: Kelly boost +0.68€ cuando `restante_min` < 3.96 (IC base=+0.133)

- **PATRÓN** `restante_min` > `4.93` → IC=+0.162 (n=1517)
...
- **PATRÓN** `lag_apertura_s` < `4.43` → IC=+0.160 (n=1491)
...
- **PATRÓN** `restante_min` < `4.02` → IC=+0.127 (n=754)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.127 (n=2345)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9475` → IC=+0.210 (n=759)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9475 (IC base=+0.064)

- **PATRÓN** `dist_vwap_pct` > `0.1852` → IC=+0.230 (n=305)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1852 (IC base=+0.064)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.206` → IC=+0.166 (n=923)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 5.206 (IC base=+0.064)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4495` → IC=+0.141 (n=1080)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 2.4495 (IC base=+0.064)

- **PATRÓN** `ibs_20min` < `0.2143` → IC=+0.134 (n=1543)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6269` → IC=+0.196 (n=366)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 1.6269 (IC base=+0.031)

- **PATRÓN** `volumen_spike_ratio` > `2.924` → IC=+0.232 (n=278)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.924 (IC base=+0.031)

- **PATRÓN** `ballena_activa_n` < `221.0` → IC=+0.206 (n=492)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.281 (n=185)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.273 (n=258)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.266)

- **PATRÓN** `hora_utc` < `14.0` → IC=+0.275 (n=251)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 14.0 (IC base=+0.266)

- **PATRÓN** `ibs_20min` < `0.4341` → IC=+0.303 (n=242)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4341 (IC base=+0.266)

- **PATRÓN** `sigma_ewma_delta_pct` < `1.934` → IC=+0.299 (n=292)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 1.934 (IC base=+0.266)

- **PATRÓN** `volumen_pendiente_norm` < `0.0577` → IC=+0.276 (n=150)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2336` → IC=+0.275 (n=38)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2336 (IC base=+0.266)

- **PATRÓN** `volumen_spike_ratio` < `1.8288` → IC=+0.293 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8288 (IC base=+0.266)

- **PATRÓN** `volumen_spike_ratio` > `2.6473` → IC=+0.297 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6473 (IC base=+0.266)
...
- **PATRÓN** `libro_liquidez` > `1980.3` → IC=+0.308 (n=92)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1980.3 (IC base=+0.266)

- **PATRÓN** `ballena_activa_n` < `66.0` → IC=+0.273 (n=108)
...
- **PATRÓN** `ibs_20min` < `0.5294` → IC=+0.294 (n=333)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5294 (IC base=+0.257)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.961` → IC=+0.318 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.961 (IC base=+0.257)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.571` → IC=+0.260 (n=368)
...
- **PATRÓN** `volumen_spike_ratio` < `3.5547` → IC=+0.222 (n=185)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.5547 (IC base=+0.257)

- **PATRÓN** `volumen_spike_ratio` > `2.743` → IC=+0.239 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.743 (IC base=+0.257)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.283 (n=141)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.257)

- **PATRÓN** `ballena_activa_n` < `47.0` → IC=+0.221 (n=134)
...
- **PATRÓN** `volumen_regimen` > `0.6186` → IC=+0.126 (n=113)
  ...
  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` > 0.6186 (IC base=-0.048)

- **PATRÓN** `volumen_pendiente_norm` > `0.0567` → IC=+0.250 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0567 (IC base=-0.048)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2519` → IC=+0.197 (n=31)
  ...
  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 2.2519 (IC base=-0.048)
...
- **FILTRO** `ibs_20min` > `0.5` → IC=-0.134 (n=621)
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.134 (n=1294)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` > 0.0076 (IC base=+0.053)

- **PATRÓN** `ibs_20min` > `0.9465` → IC=+0.247 (n=951)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.088` → IC=+0.169 (n=1223)
...
- **PATRÓN** `dist_vwap_pct` < `0.1474` → IC=+0.206 (n=726)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2486` → IC=+0.358 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2486 (IC base=+0.039)

- **PATRÓN** `volumen_spike_ratio` > `2.3812` → IC=+0.290 (n=294)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3812 (IC base=+0.039)

- **PATRÓN** `ballena_activa_n` < `59.0` → IC=+0.261 (n=483)
...
- **FILTRO** `sigma_ewma_delta_pct` > `2.09` → IC=-0.167 (n=238)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2301` → IC=+0.359 (n=161)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2301 (IC base=+0.339)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.502` → IC=+0.346 (n=270)
...
- **PATRÓN** `volumen_spike_ratio` > `1.9918` → IC=+0.344 (n=133)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9918 (IC base=+0.339)

- **PATRÓN** `libro_liquidez` > `1894.0152` → IC=+0.367 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1894.0152 (IC base=+0.339)
...
- **FILTRO** `ibs_20min` < `0.7273` → IC=-0.121 (n=423)
...
- **PATRÓN** `ibs_20min` > `0.8983` → IC=+0.299 (n=376)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0802` → IC=+0.224 (n=479)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4115` → IC=+0.266 (n=169)
...
- **PATRÓN** `libro_liquidez` > `2453.0908` → IC=+0.230 (n=564)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3351` → IC=+0.156 (n=2182)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `dist_vwap_pct` > `0.7898` → IC=+0.249 (n=575)
...
- **PATRÓN** `volumen_regimen` > `0.6875` → IC=+0.174 (n=1536)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2847` → IC=+0.152 (n=1963)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 2.2847 (IC base=+0.155)

- **PATRÓN** `volumen_spike_ratio` > `1.8521` → IC=+0.150 (n=1487)
...
- **PATRÓN** `ballena_activa_n` < `121.0` → IC=+0.174 (n=1222)

  - _Acción_: Kelly boost +0.87€ cuando `ballena_activa_n` < 121.0 (IC base=+0.155)

- **PATRÓN** `sigma_h` < `0.0075` → IC=+0.197 (n=1944)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0075 (IC base=+0.184)

- **PATRÓN** `drift_60min` |x|≤ `0.4141` → IC=+0.197 (n=2206)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.4141 (IC base=+0.184)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.213 (n=1115)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 14.0 (IC base=+0.184)

- **PATRÓN** `ibs_20min` < `0.4088` → IC=+0.237 (n=2206)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4088 (IC base=+0.184)

- **PATRÓN** `dist_vwap_pct` < `0.3545` → IC=+0.176 (n=1927)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` < 0.3545 (IC base=+0.184)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.745` → IC=+0.213 (n=416)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.745 (IC base=+0.184)

- **PATRÓN** `volumen_regimen` < `1.174` → IC=+0.166 (n=1777)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2892` → IC=+0.242 (n=258)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2892 (IC base=+0.184)

- **PATRÓN** `volumen_spike_ratio` < `1.5763` → IC=+0.179 (n=703)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 1.5763 (IC base=+0.184)

- **PATRÓN** `volumen_spike_ratio` > `2.6551` → IC=+0.212 (n=532)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6551 (IC base=+0.184)

- **PATRÓN** `ballena_activa_n` < `197.0` → IC=+0.181 (n=1165)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.261 (n=174)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0066 (IC base=+0.261)

- **PATRÓN** `sigma_h` > `0.005` → IC=+0.279 (n=197)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.005 (IC base=+0.261)

- **PATRÓN** `drift_60min` |x|≤ `0.3308` → IC=+0.299 (n=197)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3308 (IC base=+0.261)

- **PATRÓN** `hora_utc` < `13.0` → IC=+0.278 (n=174)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 13.0 (IC base=+0.261)

- **PATRÓN** `ibs_20min` < `0.4057` → IC=+0.284 (n=197)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4057 (IC base=+0.261)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.436` → IC=+0.287 (n=223)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.436 (IC base=+0.261)

- **PATRÓN** `volumen_spike_ratio` < `1.8801` → IC=+0.304 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8801 (IC base=+0.261)

- **PATRÓN** `volumen_spike_ratio` > `2.8455` → IC=+0.330 (n=45)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8455 (IC base=+0.261)
...
- **PATRÓN** `libro_liquidez` > `1973.46` → IC=+0.338 (n=66)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1973.46 (IC base=+0.261)

- **PATRÓN** `ballena_activa_n` < `72.0` → IC=+0.268 (n=80)
...
- **PATRÓN** `sigma_h` < `0.0094` → IC=+0.293 (n=235)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0094 (IC base=+0.277)

- **PATRÓN** `drift_60min` |x|≤ `0.2149` → IC=+0.324 (n=157)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2149 (IC base=+0.277)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.847` → IC=+0.312 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.847 (IC base=+0.277)
...
- **PATRÓN** `volumen_spike_ratio` > `2.0029` → IC=+0.272 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.0029 (IC base=+0.277)

- **PATRÓN** `ballena_activa_n` < `59.0` → IC=+0.246 (n=116)
...
- **PATRÓN** `ibs_20min` < `0.3411` → IC=+0.217 (n=373)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3411 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` < `0.5253` → IC=+0.163 (n=497)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.5253 (IC base=+0.149)
...
- **PATRÓN** `volumen_regimen` > `0.6199` → IC=+0.160 (n=424)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.6199 (IC base=+0.149)

- **PATRÓN** `volumen_pendiente_norm` > `0.1018` → IC=+0.167 (n=127)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.1018 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` < `1.8946` → IC=+0.192 (n=212)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` < 1.8946 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `10039.916` → IC=+0.153 (n=142)
...
- **PATRÓN** `sigma_h` > `0.0103` → IC=+0.201 (n=222)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.8538` → IC=+0.235 (n=326)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8538 (IC base=+0.118)

- **PATRÓN** `dist_vwap_pct` > `0.7969` → IC=+0.269 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7969 (IC base=+0.118)
...
- **PATRÓN** `volumen_regimen` > `0.6201` → IC=+0.135 (n=489)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 0.6201 (IC base=+0.118)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5596` → IC=+0.133 (n=409)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 1.5596 (IC base=+0.118)

- **PATRÓN** `libro_liquidez` > `3206.3834` → IC=+0.221 (n=163)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1165` → IC=+0.124 (n=176)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 0.1165 (IC base=+0.076)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.146 (n=145)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 16.0 (IC base=+0.076)

- **PATRÓN** `ibs_20min` < `0.6296` → IC=+0.128 (n=399)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` < 0.6296 (IC base=+0.076)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.192` → IC=+0.192 (n=131)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 5.192 (IC base=+0.076)

- **PATRÓN** `volumen_regimen` < `0.6508` → IC=+0.147 (n=120)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 0.6508 (IC base=+0.076)

- **PATRÓN** `volumen_spike_ratio` < `1.8705` → IC=+0.136 (n=229)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 1.8705 (IC base=+0.076)

- **PATRÓN** `libro_liquidez` > `9264.2772` → IC=+0.139 (n=181)

  - _Acción_: Kelly boost +0.70€ cuando `libro_liquidez` > 9264.2772 (IC base=+0.076)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.008` → IC=+0.202 (n=1309)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `drift_60min` |x|≤ `0.4679` → IC=+0.204 (n=2583)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4679 (IC base=+0.194)
...
- **PATRÓN** `ibs_20min` < `0.5584` → IC=+0.250 (n=2583)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5584 (IC base=+0.194)

- **PATRÓN** `dist_vwap_pct` < `0.7104` → IC=+0.185 (n=2097)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.639` → IC=+0.198 (n=2428)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` < 2.639 (IC base=+0.194)

- **PATRÓN** `volumen_regimen` < `0.6245` → IC=+0.186 (n=647)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2825` → IC=+0.273 (n=275)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2825 (IC base=+0.194)

- **PATRÓN** `volumen_spike_ratio` > `2.2842` → IC=+0.215 (n=855)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2842 (IC base=+0.194)

- **PATRÓN** `ballena_activa_n` < `179.0` → IC=+0.174 (n=1455)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1052` → IC=+0.327 (n=125)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1052 (IC base=+0.274)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.279 (n=265)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5819` → IC=+0.325 (n=284)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5819 (IC base=+0.274)

- **PATRÓN** `volumen_pendiente_norm` < `0.0689` → IC=+0.271 (n=168)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0689 (IC base=+0.274)

- **PATRÓN** `volumen_pendiente_norm` > `0.2935` → IC=+0.339 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2935 (IC base=+0.274)

- **PATRÓN** `volumen_spike_ratio` > `2.7158` → IC=+0.328 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7158 (IC base=+0.274)
...
- **PATRÓN** `ballena_activa_n` < `71.0` → IC=+0.259 (n=106)
...
- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.328 (n=120)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0056 (IC base=+0.261)

- **PATRÓN** `hora_utc` > `10.0` → IC=+0.264 (n=252)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5683` → IC=+0.326 (n=360)
...
- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.201 (n=152)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 61.0 (IC base=+0.261)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1447` → IC=+0.142 (n=202)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.1447 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.166 (n=414)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.8943` → IC=+0.203 (n=99)
...
- **PATRÓN** `volumen_regimen` < `0.6268` → IC=+0.173 (n=154)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` < 0.6268 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` > `1.1962` → IC=+0.171 (n=153)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 1.1962 (IC base=+0.142)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5115` → IC=+0.172 (n=382)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.5115 (IC base=+0.157)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.192 (n=186)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 7.0 (IC base=+0.157)
...
- **PATRÓN** `dist_vwap_pct` < `0.6568` → IC=+0.178 (n=405)
...
- **PATRÓN** `volumen_regimen` < `0.5978` → IC=+0.177 (n=128)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 0.5978 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` > `1.15` → IC=+0.198 (n=127)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` > 1.15 (IC base=+0.157)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8122` → IC=+0.183 (n=216)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` < 1.8122 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` > `2.4643` → IC=+0.191 (n=108)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` > 2.4643 (IC base=+0.157)

- **PATRÓN** `libro_liquidez` > `9483.0008` → IC=+0.209 (n=173)
...
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.170 (n=234)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.908` → IC=+0.313 (n=233)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.908 (IC base=+0.099)

- **PATRÓN** `dist_vwap_pct` > `1.0609` → IC=+0.266 (n=105)
...
- **PATRÓN** `libro_liquidez` > `2940.9508` → IC=+0.286 (n=171)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2940.9508 (IC base=+0.099)

- **PATRÓN** `ballena_activa_n` < `67.0` → IC=+0.175 (n=287)
...
- **PATRÓN** `volumen_regimen` < `0.611` → IC=+0.178 (n=203)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` < 0.611 (IC base=+0.173)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1708` → IC=+0.246 (n=140)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1708 (IC base=+0.173)
...
- **PATRÓN** `ballena_activa_n` < `36.0` → IC=+0.213 (n=336)
...
- **PATRÓN** `sigma_h` < `0.0099` → IC=+0.134 (n=440)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.0099 (IC base=+0.117)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.160 (n=186)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 17.0 (IC base=+0.117)

- **PATRÓN** `ibs_20min` > `0.9375` → IC=+0.157 (n=167)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` > 0.9375 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` > `1.3755` → IC=+0.194 (n=122)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 1.3755 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.266` → IC=+0.183 (n=80)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 10.266 (IC base=+0.117)

- **PATRÓN** `volumen_regimen` < `0.6128` → IC=+0.152 (n=139)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.6128 (IC base=+0.117)

- **PATRÓN** `volumen_regimen` > `1.0865` → IC=+0.142 (n=188)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 1.0865 (IC base=+0.117)

- **PATRÓN** `volumen_pendiente_norm` > `0.2718` → IC=+0.194 (n=70)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2718 (IC base=+0.117)

- **PATRÓN** `volumen_spike_ratio` < `1.4017` → IC=+0.151 (n=164)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.4017 (IC base=+0.117)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.127 (n=454)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.01 (IC base=+0.117)

- **PATRÓN** `libro_liquidez` > `7554.6686` → IC=+0.154 (n=333)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 7554.6686 (IC base=+0.117)
...
- **FILTRO** `hora_utc` < `12.0` → IC=-0.145 (n=29)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 12.0
  - _Potencial_: sin este filtro IC_bueno=+0.156 (n=62)

- **FILTRO** `sigma_ewma_delta_pct` > `3.871` → IC=-0.214 (n=19)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 3.871
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.156 (n=62)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 12.0 (IC base=+0.059)

- **PATRÓN** `ibs_20min` < `0.6079` → IC=+0.125 (n=46)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` < 0.6079 (IC base=+0.059)

- **PATRÓN** `dist_vwap_pct` > `0.5746` → IC=+0.141 (n=51)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.5746 (IC base=+0.059)

- **PATRÓN** `volumen_regimen` > `0.7707` → IC=+0.146 (n=46)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 0.7707 (IC base=+0.059)

- **PATRÓN** `volumen_spike_ratio` < `1.7837` → IC=+0.125 (n=46)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_spike_ratio` < 1.7837 (IC base=+0.059)

- **PATRÓN** `libro_liquidez` > `12272.8392` → IC=+0.134 (n=69)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 12272.8392 (IC base=+0.059)
...
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.143 (n=26)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` > 0.0081 (IC base=+0.074)

- **PATRÓN** `ibs_20min` > `0.8876` → IC=+0.200 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8876 (IC base=+0.074)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.168 (n=818)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.028 (n=2547)
...
- **FILTRO** `ibs_20min` > `0.2723` → IC=-0.175 (n=839)
...
- **FILTRO** `ballena_activa_n` > `10.0` → IC=-0.155 (n=143)
...
- **FILTRO** `drift_20min_pct` |x|> `0.2824` → IC=-0.146 (n=179)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.143 (n=2451)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=5904)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.283 (n=2016)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=6339)

- **FILTRO** `ibs_7min` < `0.7333` → IC=-0.232 (n=2084)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7333
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=6271)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.173 (n=2834)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=5521)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.230 (n=2351)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=7805)

- **FILTRO** `ibs_7min` > `0.7197` → IC=-0.176 (n=2538)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7197
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=7618)
...
- **FILTRO** `ibs_7min` < `0.9787` → IC=-0.203 (n=741)
...
- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.255 (n=280)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=845)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.229 (n=411)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=1328)

- **FILTRO** `drift_7min_pct` |x|> `0.1381` → IC=-0.150 (n=590)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1381
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=1149)

- **FILTRO** `ibs_7min` > `0.84` → IC=-0.190 (n=434)

  - _Acción_: SKIP cuando `ibs_7min` > 0.84
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=1305)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.155 (n=355)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=1283)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.256 (n=383)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=1255)

- **FILTRO** `ibs_7min` < `0.797` → IC=-0.171 (n=408)

  - _Acción_: SKIP cuando `ibs_7min` < 0.797
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=1230)

- **FILTRO** `ballena_activa_n` > `157.0` → IC=-0.183 (n=408)

  - _Acción_: SKIP cuando `ballena_activa_n` > 157.0
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=1230)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.221 (n=410)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=1243)

- **FILTRO** `ballena_activa_n` > `131.0` → IC=-0.172 (n=412)

  - _Acción_: SKIP cuando `ballena_activa_n` > 131.0
  - _Potencial_: sin este filtro IC_bueno=-0.018 (n=1241)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.201 (n=296)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.098 (n=900)
...
- **FILTRO** `ibs_7min` < `0.2097` → IC=-0.287 (n=299)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2097
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=897)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.280 (n=294)

  - _Acción_: SKIP cuando `ballena_activa_n` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=902)
...
- **FILTRO** `ibs_7min` > `0.8192` → IC=-0.195 (n=441)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=1325)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.151 (n=434)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.076 (n=974)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.258 (n=332)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=1076)

- **FILTRO** `ibs_7min` < `0.7565` → IC=-0.184 (n=352)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7565
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=1056)

- **FILTRO** `ballena_activa_n` > `43.0` → IC=-0.217 (n=341)

  - _Acción_: SKIP cuando `ballena_activa_n` > 43.0
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=1067)

- **FILTRO** `hora_utc` > `15.0` → IC=-0.133 (n=456)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=944)
...
- **FILTRO** `ibs_7min` > `0.1866` → IC=-0.167 (n=475)

  - _Acción_: SKIP cuando `ibs_7min` > 0.1866
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=925)

- **FILTRO** `ballena_activa_n` > `26.0` → IC=-0.193 (n=471)

  - _Acción_: SKIP cuando `ballena_activa_n` > 26.0
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=929)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.42` → IC=-0.226 (n=399)

  - _Acción_: SKIP cuando `py_entrada` < 0.42
  - _Potencial_: sin este filtro IC_bueno=+0.019 (n=1199)

- **FILTRO** `ibs_7min` < `0.7759` → IC=-0.191 (n=399)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7759
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=1199)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.180 (n=398)

  - _Acción_: SKIP cuando `ballena_activa_n` > 13.0
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=1200)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.188 (n=459)
...
- **FILTRO** `ibs_7min` < `0.7534` → IC=-0.205 (n=347)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7534
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=1043)

- **FILTRO** `ballena_activa_n` > `14.0` → IC=-0.222 (n=343)

  - _Acción_: SKIP cuando `ballena_activa_n` > 14.0
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=1047)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.269 (n=353)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=1364)

- **FILTRO** `ibs_7min` > `0.8333` → IC=-0.166 (n=429)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8333
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=1288)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.121 (n=565)
...
- **PATRÓN** `delta_ratio` |x|> `0.4498` → IC=+0.173 (n=96)
  - _Por qué funciona_: delta_ratio alto → flow informado visible; edge real en el desequilibrio
  - _Acción_: Kelly boost +0.87€ cuando `delta_ratio` |x|> 0.4498 (IC base=+0.121)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.157 (n=97)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 15.0 (IC base=+0.121)

- **PATRÓN** `total_vol_5m` < `422.506` → IC=+0.222 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `total_vol_5m` < 422.506 (IC base=+0.121)

- **PATRÓN** `libro_liquidez` > `3582.6278` → IC=+0.158 (n=71)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 3582.6278 (IC base=+0.121)
...
- **PATRÓN** `libro_liquidez` > `2147.801` → IC=+0.139 (n=34)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 2147.801 (IC base=+0.077)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.392` → IC=-0.177 (n=342)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=2486)
...
- **FILTRO** `sigma_h` > `0.0074` → IC=-0.239 (n=155)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.199 (n=467)
...
- **FILTRO** `hora_utc` > `16.0` → IC=-0.247 (n=152)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.197 (n=470)
...
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH#5min` — IC=+0.125 n=30. Faltan ~10 resoluciones para umbral n≥40. ETA: ~7h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_DEPTH_FASE0#ETH` — IC=+0.125 n=30. Faltan ~10 resoluciones para umbral n≥40. ETA: ~7h.
...
| ✅ BALLENAS_TARDIAS | 9662 | -0.099 | -1525.04€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 696 | -0.085 | -128.45€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 8966 | -0.100 | -1396.59€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1207 | -0.009 | -199.94€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1207 | -0.009 | -199.94€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 696 | -0.085 | -128.45€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 696 | -0.085 | -128.45€ | 3 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 1205 | -0.174 | -405.80€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 1205 | -0.174 | -405.80€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 2618 | -0.072 | -254.03€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 2618 | -0.072 | -254.03€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 2260 | -0.071 | -108.01€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2260 | -0.071 | -108.01€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1676 | -0.196 | -428.80€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1676 | -0.196 | -428.80€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 32226 | +0.115 | -2019.80€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 5937 | +0.186 | -214.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 107 | -0.105 | -50.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 22988 | +0.097 | -1715.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3194 | +0.120 | -39.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 3858 | +0.065 | -649.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 21 | -0.065 | +2.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 3832 | +0.066 | -645.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 6655 | +0.134 | -146.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1677 | +0.196 | -95.77€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 3826 | +0.110 | -85.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1110 | +0.130 | +57.05€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 3867 | +0.082 | -471.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 21 | +0.065 | +3.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 3845 | +0.082 | -472.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 7142 | +0.129 | -86.28€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2161 | +0.166 | -20.69€ | 0 | 8 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 3827 | +0.116 | -33.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 