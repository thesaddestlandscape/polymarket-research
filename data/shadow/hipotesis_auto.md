# Hipótesis automáticas — 2026-08-25 17:58 UTC
_Generado por shadow_postmortem.py sobre 153413 resoluciones (PNL=+10190.21€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `148.69` → IC=-0.278 (n=2152)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 148.69
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=6459)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.225 (n=588)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.215 (n=727)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.210)

- **PATRÓN** `libro_liquidez` > `12122.7` → IC=+0.215 (n=191)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.135 (n=1128)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.195 (n=4192)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.167 (n=1070)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.242 (n=955)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.197 (n=1023)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.197 (n=10931)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.222 (n=7681)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.192)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.129 (n=2108)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.248 (n=1691)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.175 (n=699)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.244 (n=1544)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.230 (n=647)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.205 (n=673)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `restante_min` > `4.92` → IC=+0.158 (n=1406)
...
- **PATRÓN** `lag_apertura_s` < `4.54` → IC=+0.157 (n=1311)
...
- **PATRÓN** `restante_min` < `3.89` → IC=+0.144 (n=655)

  - _Acción_: Kelly boost +0.72€ cuando `restante_min` < 3.89 (IC base=+0.139)
...
- **PATRÓN** `restante_min` > `4.94` → IC=+0.153 (n=709)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9462` → IC=+0.214 (n=667)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9462 (IC base=+0.068)

- **PATRÓN** `dist_vwap_pct` > `0.2069` → IC=+0.237 (n=279)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2069 (IC base=+0.068)

- **PATRÓN** `dist_vwap_pct` < `1.0161` → IC=+0.224 (n=444)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 1.0161 (IC base=+0.068)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.084` → IC=+0.181 (n=828)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 5.084 (IC base=+0.068)

- **PATRÓN** `volumen_regimen` < `0.6304` → IC=+0.234 (n=141)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1063` → IC=+0.134 (n=1001)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3023` → IC=+0.142 (n=146)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `volumen_pendiente_norm` > 0.3023 (IC base=+0.068)

- **PATRÓN** `volumen_spike_ratio` < `2.443` → IC=+0.140 (n=923)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 2.443 (IC base=+0.068)

- **PATRÓN** `volumen_spike_ratio` > `1.9482` → IC=+0.133 (n=699)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 1.9482 (IC base=+0.068)

- **PATRÓN** `ibs_20min` < `0.2121` → IC=+0.125 (n=1380)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.195 (n=149)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0075 (IC base=+0.119)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.482` → IC=+0.336 (n=163)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.482 (IC base=+0.119)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.152 (n=222)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.249 (n=325)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.4287` → IC=+0.227 (n=324)
...
- **PATRÓN** `dist_vwap_pct` < `1.0239` → IC=+0.224 (n=335)
...
- **PATRÓN** `volumen_regimen` < `1.2894` → IC=+0.221 (n=324)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2894 (IC base=+0.221)

- **PATRÓN** `volumen_regimen` > `1.099` → IC=+0.252 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.099 (IC base=+0.221)
...
- **FILTRO** `sigma_ewma_delta_pct` > `4.888` → IC=-0.147 (n=259)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=949)
...
- **FILTRO** `ibs_20min` > `0.7744` → IC=-0.169 (n=270)
...
- **PATRÓN** `sigma_h` > `0.0073` → IC=+0.135 (n=1156)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `1.2509` → IC=+0.286 (n=222)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2509 (IC base=+0.054)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.085` → IC=+0.122 (n=1012)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1152` → IC=+0.166 (n=1114)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4852` → IC=+0.186 (n=384)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 1.4852 (IC base=+0.054)

- **PATRÓN** `volumen_spike_ratio` > `2.8724` → IC=+0.174 (n=382)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.8724 (IC base=+0.054)

- **PATRÓN** `ballena_activa_n` < `102.0` → IC=+0.270 (n=594)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 102.0 (IC base=+0.054)

- **PATRÓN** `ibs_20min` < `0.0967` → IC=+0.175 (n=1101)
...
- **PATRÓN** `volumen_spike_ratio` > `3.8943` → IC=+0.214 (n=138)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.215 (n=191)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.4231` → IC=+0.313 (n=212)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4231 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.12` → IC=+0.270 (n=307)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.227 (n=577)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.209)

- **PATRÓN** `libro_liquidez` > `3101.1618` → IC=+0.243 (n=173)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3101.1618 (IC base=+0.209)
...
- **PATRÓN** `sigma_h` > `0.0062` → IC=+0.131 (n=166)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` > 0.0062 (IC base=+0.084)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.198 (n=180)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 15.0 (IC base=+0.084)

- **PATRÓN** `ibs_20min` > `0.617` → IC=+0.152 (n=326)

  - _Acción_: Kelly boost +0.76€ cuando `ibs_20min` > 0.617 (IC base=+0.084)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.181` → IC=+0.228 (n=90)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.181 (IC base=+0.084)

- **PATRÓN** `libro_liquidez` > `2962.0874` → IC=+0.124 (n=243)

  - _Acción_: Kelly boost +0.62€ cuando `libro_liquidez` > 2962.0874 (IC base=+0.084)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.204 (n=1158)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0077 (IC base=+0.144)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.167 (n=1282)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 15.0 (IC base=+0.144)

- **PATRÓN** `ibs_20min` > `0.9434` → IC=+0.288 (n=1155)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9434 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `1.0588` → IC=+0.254 (n=437)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0588 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.489` → IC=+0.229 (n=1532)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.489 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` < `0.6307` → IC=+0.157 (n=599)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.6307 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` > `1.0839` → IC=+0.152 (n=815)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8959` → IC=+0.153 (n=1487)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.8959 (IC base=+0.144)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.153 (n=2066)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `3103.6736` → IC=+0.190 (n=1154)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 3103.6736 (IC base=+0.144)

- **PATRÓN** `ballena_activa_n` < `169.0` → IC=+0.187 (n=1027)

  - _Acción_: Kelly boost +0.94€ cuando `ballena_activa_n` < 169.0 (IC base=+0.144)
...
- **PATRÓN** `ibs_20min` > `0.3404` → IC=+0.217 (n=397)
...
- **PATRÓN** `volumen_regimen` > `1.1006` → IC=+0.192 (n=180)
  ...
  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` > 1.1006 (IC base=+0.165)
...
- **PATRÓN** `volumen_spike_ratio` < `2.53` → IC=+0.198 (n=349)
...
- **PATRÓN** `libro_liquidez` > `11533.5682` → IC=+0.204 (n=265)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11533.5682 (IC base=+0.165)
...
- **PATRÓN** `ibs_20min` > `0.7131` → IC=+0.249 (n=297)
...
- **PATRÓN** `volumen_spike_ratio` > `3.9028` → IC=+0.175 (n=118)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 3.9028 (IC base=+0.173)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.209 (n=256)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.173)

- **PATRÓN** `libro_liquidez` > `1854.1544` → IC=+0.186 (n=221)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 1854.1544 (IC base=+0.173)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.169 (n=360)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 8.0 (IC base=+0.138)

- **PATRÓN** `ibs_20min` > `0.3557` → IC=+0.196 (n=396)
...
- **PATRÓN** `libro_liquidez` > `3082.6348` → IC=+0.191 (n=354)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 3082.6348 (IC base=+0.138)

- **PATRÓN** `ballena_activa_n` < `170.0` → IC=+0.185 (n=179)
...
- **PATRÓN** `sigma_h` > `0.0106` → IC=+0.164 (n=206)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.0106 (IC base=+0.090)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.134 (n=312)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 12.0 (IC base=+0.090)

- **PATRÓN** `ibs_20min` > `0.9091` → IC=+0.288 (n=206)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9091 (IC base=+0.090)
...
- **PATRÓN** `ballena_activa_n` < `69.0` → IC=+0.171 (n=229)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.188 (n=277)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 15.0 (IC base=+0.169)

- **PATRÓN** `ibs_20min` > `0.9655` → IC=+0.294 (n=250)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.037` → IC=+0.249 (n=301)
...
- **PATRÓN** `volumen_regimen` > `1.041` → IC=+0.191 (n=250)
  ...
  - _Acción_: Kelly boost +0.95€ cuando `volumen_regimen` > 1.041 (IC base=+0.169)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5675` → IC=+0.173 (n=221)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.5675 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `1.8445` → IC=+0.176 (n=334)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 1.8445 (IC base=+0.169)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.182 (n=615)

  - _Acción_: Kelly boost +0.91€ cuando `libro_spread` < 0.01 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `3115.2842` → IC=+0.177 (n=184)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 3115.2842 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.215 (n=279)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.144 (n=116)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 17.0 (IC base=+0.078)

- **PATRÓN** `dist_vwap_pct` > `1.7325` → IC=+0.203 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.7325 (IC base=+0.078)

- **PATRÓN** `libro_liquidez` > `8352.5828` → IC=+0.131 (n=220)
...
- **PATRÓN** `sigma_h` < `0.0094` → IC=+0.144 (n=147)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` < 0.0094 (IC base=+0.104)
...
- **PATRÓN** `ibs_20min` < `0.2849` → IC=+0.132 (n=74)
  ...
  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.2849 (IC base=+0.104)
...
- **PATRÓN** `volumen_spike_ratio` < `1.3905` → IC=+0.196 (n=54)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 1.3905 (IC base=+0.104)

- **PATRÓN** `libro_liquidez` > `7751.0166` → IC=+0.147 (n=165)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 7751.0166 (IC base=+0.104)
...
### LEADLAG_BTC_XRP_15M
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.130 (n=98)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 6.0 (IC base=+0.108)

- **PATRÓN** `libro_liquidez` > `2605.1459` → IC=+0.216 (n=86)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2605.1459 (IC base=+0.108)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.224 (n=675)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=2113)

- **FILTRO** `ibs_20min` > `0.2746` → IC=-0.195 (n=696)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.142 (n=2031)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=4928)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.285 (n=1671)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=5288)

- **FILTRO** `ibs_7min` < `0.7371` → IC=-0.231 (n=1739)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7371
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=5220)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.174 (n=2353)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=4606)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.229 (n=1992)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=6523)

- **FILTRO** `ibs_7min` > `0.7209` → IC=-0.171 (n=2128)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7209
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=6387)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.154 (n=218)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.090 (n=730)
...
- **FILTRO** `ibs_7min` < `0.9833` → IC=-0.199 (n=625)
...
- **FILTRO** `drift_7min_pct` |x|> `0.1048` → IC=-0.134 (n=717)
...
- **FILTRO** `py_entrada` < `0.39` → IC=-0.246 (n=352)

  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=1056)

- **FILTRO** `ballena_activa_n` > `149.0` → IC=-0.171 (n=351)

  - _Acción_: SKIP cuando `ballena_activa_n` > 149.0
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=1057)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.262 (n=321)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=1168)

- **FILTRO** `ibs_7min` > `0.8184` → IC=-0.187 (n=372)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8184
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=1117)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.157 (n=359)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=826)
...
- **FILTRO** `ibs_7min` < `0.7689` → IC=-0.185 (n=296)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=889)

- **FILTRO** `ballena_activa_n` > `43.0` → IC=-0.210 (n=295)

  - _Acción_: SKIP cuando `ballena_activa_n` > 43.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=890)

- **FILTRO** `hora_utc` > `15.0` → IC=-0.135 (n=373)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **FILTRO** `py_entrada` < `0.38` → IC=-0.290 (n=274)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=869)
...
- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.245 (n=284)
  ...
  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=+0.005 (n=859)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.276 (n=292)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=1141)

- **FILTRO** `ibs_7min` > `0.8387` → IC=-0.150 (n=358)
...
- **FILTRO** `sigma_ewma_delta_pct` > `16.993` → IC=-0.186 (n=313)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 16.993
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=2169)
...
- **FILTRO** `hora_utc` > `16.0` → IC=-0.243 (n=134)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 16.0
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.923` → IC=-0.143 (n=197)
...
- **FILTRO** `drift_15min` |x|> `1.5229` → IC=-0.142 (n=199)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
...
| ✅ BALLENAS_TARDIAS | 6333 | -0.098 | -827.99€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 657 | -0.086 | -112.53€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 5676 | -0.100 | -715.46€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 2101 | -0.076 | -102.21€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 2101 | -0.076 | -102.21€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1488 | -0.186 | -312.09€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1488 | -0.186 | -312.09€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 28909 | +0.114 | -1887.87€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 5469 | +0.187 | -186.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 107 | -0.105 | -50.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 20253 | +0.094 | -1623.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 