# Hipótesis automáticas — 2026-07-12 17:10 UTC
_Generado por shadow_postmortem.py sobre 11189 resoluciones (PNL=+2085.84€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.228 (n=171)
...
- **PATRÓN** `py_entrada` > `0.585` → IC=+0.253 (n=366)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.585 (IC base=+0.219)

- **PATRÓN** `libro_liquidez` > `2838.1924` → IC=+0.233 (n=313)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2838.1924 (IC base=+0.219)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.153 (n=427)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.01 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `6004.8923` → IC=+0.163 (n=170)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 6004.8923 (IC base=+0.145)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.123 (n=1364)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 6.0 (IC base=+0.117)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.122 (n=1360)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` < 17.0 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` > `0.5361` → IC=+0.167 (n=43)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.5361 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` < `0.1286` → IC=+0.206 (n=66)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1286 (IC base=+0.117)
...
- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.197 (n=331)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0049 (IC base=+0.177)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.176 (n=273)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 12.0 (IC base=+0.177)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.185 (n=379)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` < 17.0 (IC base=+0.177)

- **PATRÓN** `dist_vwap_pct` > `0.5381` → IC=+0.316 (n=36)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.442` → IC=+0.353 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.442 (IC base=+0.177)
...
- **PATRÓN** `dist_vwap_pct` > `0.293` → IC=+0.289 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.293 (IC base=+0.142)
...
- **PATRÓN** `sigma_h` > `0.0124` → IC=+0.144 (n=144)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` > 0.0124 (IC base=+0.077)

- **PATRÓN** `dist_vwap_pct` < `0.2531` → IC=+0.157 (n=103)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.2531 (IC base=+0.077)
...
- **FILTRO** `dist_vwap_pct` > `0.2265` → IC=-0.150 (n=18)
...
- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.207 (n=39)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0083 (IC base=+0.030)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.143 (n=40)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` < 6.0 (IC base=+0.030)

- **PATRÓN** `dist_vwap_pct` < `0.2265` → IC=+0.188 (n=30)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.2265 (IC base=+0.030)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 967 | +0.181 | +6.98€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 621 | +0.213 | +13.27€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 284 | +0.185 | -3.98€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 338 | +0.194 | +12.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 206 | +0.221 | +15.21€ | 0 | 8 |
...
| ✅ GBM_LATE_15M | 3640 | +0.113 | +1211.14€ | 0 | 9 |
| ✅ GBM_LATE_15M#15min | 3640 | +0.113 | +1211.14€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#SOL | 949 | +0.138 | +470.04€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 949 | +0.138 | +470.04€ | 0 | 8 |
| ✅ GBM_LATE_15M#XRP | 876 | +0.172 | +466.30€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 876 | +0.172 | +466.30€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 994 | +0.164 | +560.44€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 994 | +0.164 | +560.44€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 278 | +0.121 | +99.66€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 278 | +0.121 | +99.66€ | 0 | 6 |
...
| ✅ GBM_LATE_15M_TARDIO | 1148 | +0.080 | +281.52€ | 1 | 7 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1148 | +0.080 | +281.52€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#SOL | 298 | +0.033 | +35.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 298 | +0.033 | +35.64€ | 1 | 4 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.091 n=6331 | tras_1loss IC=+0.046 n=4720 | tras_2loss IC=+0.013 n=2113/40 | gap=+0.079 (umbral 0.05)
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=+0.081 n=501/40 PNL=+118.88€ | H=15h UTC: IC=+0.097 n=502/40 PNL=+126.74€ | H=17h UTC: IC=+0.124 n=397/40 PNL=+133.15€ | H=19h UTC: IC=+0.063 n=385/40 PNL=+49.78€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8731 IC=+0.098 PNL=+2161.73€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8731 IC=+0.098 PNL=+2161.73€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.167 > 0.08 con n=64 PNL=-0.65€
  - _Datos_: n=64 IC=+0.167 PNL=-0.65€
