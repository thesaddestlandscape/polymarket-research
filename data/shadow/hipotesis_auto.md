# Hipótesis automáticas — 2026-09-15 23:39 UTC
_Generado por shadow_postmortem.py sobre 458316 resoluciones (PNL=+49362.60€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.05` → IC=-0.253 (n=5702)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.05
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=17106)
...
- **FILTRO** `restante_s_al_confirmar` < `136.09` → IC=-0.279 (n=709)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=2129)

### BALLENAS_TARDIAS#ETH#5min
- **FILTRO** `restante_s_al_confirmar` < `141.36` → IC=-0.152 (n=1468)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 141.36
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=4407)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `158.58` → IC=-0.252 (n=1348)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.58
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=4044)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `157.06` → IC=-0.328 (n=1428)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 157.06
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=2900)

### CANDIDATA9_BOT_CONSENSO
- **FILTRO** `py_entrada` < `0.39` → IC=-0.277 (n=155)

  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=315)

- **FILTRO** `py_entrada` < `0.47` → IC=-0.167 (n=97)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=350)

### CANDIDATA9_BOT_CONSENSO#BTC#5min
- **FILTRO** `py_entrada` < `0.43` → IC=-0.312 (n=62)
...
- **PATRÓN** `py_entrada` > `0.745` → IC=+0.348 (n=610)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.745 (IC base=+0.200)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.200 (n=1672)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.200)

- **PATRÓN** `libro_liquidez` > `11005.6828` → IC=+0.201 (n=864)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11005.6828 (IC base=+0.200)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.208 (n=1239)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.133 (n=377)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1940.1614` → IC=+0.159 (n=359)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 1940.1614 (IC base=+0.126)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.176 (n=1097)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.202 (n=9067)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.219 (n=3213)
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.181 (n=2246)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.345 (n=346)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.288)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.182 (n=1902)
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.321 (n=651)
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.194 (n=1550)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.187 (n=4921)

  - _Acción_: Kelly boost +0.93€ cuando `py_entrada` > 0.71 (IC base=+0.169)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.225 (n=4782)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `3.32` → IC=+0.150 (n=3731)

  - _Acción_: Kelly boost +0.75€ cuando `lag_apertura_s` < 3.32 (IC base=+0.125)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.207 (n=2055)
...
- **PATRÓN** `restante_min` > `4.93` → IC=+0.146 (n=1978)
...
- **PATRÓN** `lag_apertura_s` < `4.14` → IC=+0.149 (n=1843)

  - _Acción_: Kelly boost +0.75€ cuando `lag_apertura_s` < 4.14 (IC base=+0.130)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min
- **PATRÓN** `py_entrada` < `0.38` → IC=+0.199 (n=2022)

  - _Acción_: Kelly boost +0.99€ cuando `py_entrada` < 0.38 (IC base=+0.119)

- **PATRÓN** `restante_min` < `4.48` → IC=+0.125 (n=2483)

  - _Acción_: Kelly boost +0.63€ cuando `restante_min` < 4.48 (IC base=+0.119)

- **PATRÓN** `restante_min` > `4.96` → IC=+0.146 (n=1900)
...
- **PATRÓN** `lag_apertura_s` < `4.38` → IC=+0.145 (n=2471)

  - _Acción_: Kelly boost +0.72€ cuando `lag_apertura_s` < 4.38 (IC base=+0.119)

### FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.318 (n=657)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1493.0973` → IC=+0.313 (n=394)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9785` → IC=+0.231 (n=2027)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9785 (IC base=+0.096)

- **PATRÓN** `dist_vwap_pct` > `0.3939` → IC=+0.239 (n=565)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3939 (IC base=+0.096)

- **PATRÓN** `dist_vwap_pct` < `0.1548` → IC=+0.242 (n=1189)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1548 (IC base=+0.096)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.862` → IC=+0.166 (n=2347)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 5.862 (IC base=+0.096)

- **PATRÓN** `volumen_regimen` < `0.6095` → IC=+0.256 (n=511)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6095 (IC base=+0.096)

- **PATRÓN** `volumen_regimen` > `1.0676` → IC=+0.243 (n=695)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0676 (IC base=+0.096)

- **PATRÓN** `volumen_pendiente_norm` < `0.1757` → IC=+0.192 (n=4138)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` < 0.1757 (IC base=+0.096)

- **PATRÓN** `volumen_pendiente_norm` > `0.3097` → IC=+0.201 (n=567)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3097 (IC base=+0.096)

- **PATRÓN** `volumen_spike_ratio` > `1.9342` → IC=+0.198 (n=2655)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.9342 (IC base=+0.096)

- **PATRÓN** `ibs_20min` < `0.5684` → IC=+0.130 (n=7476)
...
- **PATRÓN** `dist_vwap_pct` < `0.3525` → IC=+0.168 (n=2564)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` < 0.3525 (IC base=+0.059)

- **PATRÓN** `volumen_regimen` < `0.6973` → IC=+0.172 (n=1064)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 0.6973 (IC base=+0.059)

- **PATRÓN** `volumen_regimen` > `0.8691` → IC=+0.172 (n=1611)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` > 0.8691 (IC base=+0.059)

- **PATRÓN** `volumen_pendiente_norm` > `0.2458` → IC=+0.222 (n=815)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2458 (IC base=+0.059)

- **PATRÓN** `volumen_spike_ratio` > `1.4653` → IC=+0.198 (n=4026)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.4653 (IC base=+0.059)

- **PATRÓN** `ballena_activa_n` < `158.0` → IC=+0.207 (n=3800)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2317` → IC=+0.194 (n=243)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2317 (IC base=+0.166)

- **PATRÓN** `volumen_spike_ratio` < `2.634` → IC=+0.156 (n=1258)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.634 (IC base=+0.166)

- **PATRÓN** `volumen_spike_ratio` > `1.44` → IC=+0.167 (n=1257)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 1.44 (IC base=+0.166)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.190 (n=1251)
...
- **PATRÓN** `volumen_spike_ratio` > `2.7352` → IC=+0.264 (n=303)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7352 (IC base=+0.235)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.238 (n=1050)
...
- **PATRÓN** `sigma_h` < `0.0052` → IC=+0.223 (n=911)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` > `0.9238` → IC=+0.253 (n=468)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9238 (IC base=+0.211)

- **PATRÓN** `dist_vwap_pct` > `0.2178` → IC=+0.219 (n=524)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2178 (IC base=+0.211)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.862` → IC=+0.234 (n=336)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.862 (IC base=+0.211)

- **PATRÓN** `volumen_regimen` < `1.2629` → IC=+0.221 (n=1033)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2629 (IC base=+0.211)

- **PATRÓN** `volumen_regimen` > `0.8746` → IC=+0.213 (n=688)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.168 (n=380)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 18.0 (IC base=+0.141)

- **PATRÓN** `ibs_20min` < `0.6737` → IC=+0.176 (n=1111)

  - _Acción_: Kelly boost +0.88€ cuando `ibs_20min` < 0.6737 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` < `0.1334` → IC=+0.156 (n=1006)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.1334 (IC base=+0.141)
...
- **PATRÓN** `volumen_regimen` < `1.2046` → IC=+0.151 (n=1111)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.2046 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` > `0.613` → IC=+0.141 (n=1110)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 0.613 (IC base=+0.141)

- **PATRÓN** `volumen_pendiente_norm` > `0.1561` → IC=+0.185 (n=300)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1561 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` < `2.4323` → IC=+0.154 (n=1002)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.4323 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `13227.2478` → IC=+0.156 (n=740)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 13227.2478 (IC base=+0.141)

- **PATRÓN** `ballena_activa_n` < `216.0` → IC=+0.166 (n=309)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 216.0 (IC base=+0.141)

### GBM_LATE_15M#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0057` → IC=+0.188 (n=1335)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.471` → IC=+0.226 (n=378)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.471 (IC base=+0.176)

- **PATRÓN** `volumen_pendiente_norm` < `0.1062` → IC=+0.182 (n=1133)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` < 0.1062 (IC base=+0.176)

- **PATRÓN** `volumen_pendiente_norm` > `0.377` → IC=+0.180 (n=173)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.377 (IC base=+0.176)

- **PATRÓN** `volumen_spike_ratio` > `3.0439` → IC=+0.197 (n=566)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 3.0439 (IC base=+0.176)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.188 (n=1525)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.04 (IC base=+0.176)

- **PATRÓN** `sigma_h` < `0.0104` → IC=+0.222 (n=1156)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.247 (n=390)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.213)

- **PATRÓN** `ibs_20min` < `0.3808` → IC=+0.230 (n=1018)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `7.606` → IC=+0.215 (n=1361)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8407` → IC=+0.203 (n=453)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8407 (IC base=+0.213)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.226 (n=596)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.213)

- **PATRÓN** `libro_liquidez` > `1887.358` → IC=+0.229 (n=386)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1887.358 (IC base=+0.213)

- **PATRÓN** `ballena_activa_n` < `28.0` → IC=+0.212 (n=645)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 28.0 (IC base=+0.213)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.258` → IC=+0.134 (n=531)
...
- **PATRÓN** `dist_vwap_pct` > `0.1669` → IC=+0.190 (n=185)
  ...
  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.1669 (IC base=+0.001)

- **PATRÓN** `volumen_regimen` < `0.8572` → IC=+0.154 (n=359)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.8572 (IC base=+0.001)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5036` → IC=+0.180 (n=435)
...
- **FILTRO** `ibs_20min` < `0.3714` → IC=-0.173 (n=96)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.141 (n=196)

- **FILTRO** `ibs_20min` > `0.2727` → IC=-0.128 (n=1721)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2727
  - _Potencial_: sin este filtro IC_bueno=+0.117 (n=858)
...
- **PATRÓN** `ibs_20min` > `0.75` → IC=+0.209 (n=101)
...
- **PATRÓN** `dist_vwap_pct` < `0.5093` → IC=+0.288 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5093 (IC base=+0.037)
...
- **PATRÓN** `volumen_spike_ratio` < `3.0245` → IC=+0.275 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.0245 (IC base=+0.037)

- **PATRÓN** `volumen_spike_ratio` > `1.5387` → IC=+0.278 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5387 (IC base=+0.037)

- **PATRÓN** `ballena_activa_n` < `47.0` → IC=+0.321 (n=76)
...
- **FILTRO** `drift_60min` |x|> `0.6568` → IC=-0.189 (n=426)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6568
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=1280)

- **FILTRO** `ibs_20min` < `0.6552` → IC=-0.158 (n=1125)
...
- **FILTRO** `libro_spread` > `0.02` → IC=-0.199 (n=377)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=1329)

- **FILTRO** `ibs_20min` > `0.7736` → IC=-0.201 (n=646)
...
- **PATRÓN** `dist_vwap_pct` < `0.2662` → IC=+0.297 (n=185)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2662 (IC base=-0.084)

- **PATRÓN** `volumen_regimen` < `0.9997` → IC=+0.271 (n=181)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9997 (IC base=-0.084)

- **PATRÓN** `volumen_regimen` > `0.6107` → IC=+0.288 (n=206)
...
- **PATRÓN** `volumen_spike_ratio` > `1.7999` → IC=+0.280 (n=125)
...
- **PATRÓN** `dist_vwap_pct` < `0.2666` → IC=+0.239 (n=543)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.512` → IC=+0.142 (n=3594)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 3.512 (IC base=+0.086)

- **PATRÓN** `volumen_regimen` > `0.6753` → IC=+0.234 (n=2305)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6753 (IC base=+0.086)

- **PATRÓN** `volumen_pendiente_norm` < `0.1135` → IC=+0.225 (n=3912)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4784` → IC=+0.238 (n=1371)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4784 (IC base=+0.086)

- **PATRÓN** `volumen_spike_ratio` > `2.7839` → IC=+0.236 (n=1369)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7839 (IC base=+0.086)

- **PATRÓN** `ballena_activa_n` < `102.0` → IC=+0.277 (n=3593)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 102.0 (IC base=+0.086)

- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.140 (n=2589)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` > 0.0084 (IC base=+0.067)

- **PATRÓN** `ibs_20min` < `0.5563` → IC=+0.150 (n=6832)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` < 0.5563 (IC base=+0.067)

- **PATRÓN** `dist_vwap_pct` > `0.6975` → IC=+0.243 (n=380)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6975 (IC base=+0.067)

- **PATRÓN** `dist_vwap_pct` < `0.1692` → IC=+0.229 (n=1955)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1692 (IC base=+0.067)

- **PATRÓN** `volumen_regimen` < `0.7106` → IC=+0.230 (n=928)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7106 (IC base=+0.067)

- **PATRÓN** `volumen_regimen` > `1.201` → IC=+0.250 (n=703)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.201 (IC base=+0.067)

- **PATRÓN** `volumen_pendiente_norm` > `0.2528` → IC=+0.320 (n=559)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2528 (IC base=+0.067)

- **PATRÓN** `volumen_spike_ratio` < `1.6251` → IC=+0.253 (n=1206)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6251 (IC base=+0.067)

- **PATRÓN** `volumen_spike_ratio` > `2.3737` → IC=+0.255 (n=1243)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3737 (IC base=+0.067)

- **PATRÓN** `ballena_activa_n` < `78.0` → IC=+0.254 (n=2595)
...
- **PATRÓN** `ibs_20min` > `0.8645` → IC=+0.252 (n=519)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8645 (IC base=+0.038)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.293` → IC=+0.147 (n=700)
...
- **FILTRO** `ibs_20min` > `0.8269` → IC=-0.150 (n=572)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8269
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=1719)

- **PATRÓN** `dist_vwap_pct` > `0.3098` → IC=+0.140 (n=245)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.3098 (IC base=+0.009)

- **PATRÓN** `volumen_regimen` > `0.6498` → IC=+0.134 (n=588)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` > 0.6498 (IC base=+0.009)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4208` → IC=+0.161 (n=213)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 1.4208 (IC base=+0.009)

- **PATRÓN** `ballena_activa_n` < `235.0` → IC=+0.182 (n=209)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 235.0 (IC base=+0.009)

- **PATRÓN** `dist_vwap_pct` < `0.1624` → IC=+0.213 (n=406)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1624 (IC base=+0.001)

- **PATRÓN** `volumen_regimen` > `1.1352` → IC=+0.218 (n=129)
...
- **PATRÓN** `sigma_h` > `0.0062` → IC=+0.266 (n=1082)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0062 (IC base=+0.236)

- **PATRÓN** `drift_60min` |x|≤ `0.0975` → IC=+0.246 (n=404)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.526` → IC=+0.270 (n=376)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.526 (IC base=+0.236)

- **PATRÓN** `volumen_pendiente_norm` < `0.1101` → IC=+0.254 (n=1012)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.627` → IC=+0.277 (n=1040)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.285 (n=495)
...
- **PATRÓN** `ballena_activa_n` < `22.0` → IC=+0.275 (n=372)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 22.0 (IC base=+0.278)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **FILTRO** `ibs_20min` < `0.2414` → IC=-0.212 (n=345)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2414
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=1038)

- **FILTRO** `ibs_20min` > `0.8097` → IC=-0.184 (n=454)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8097
  - _Potencial_: sin este filtro IC_bueno=+0.036 (n=1363)

- **PATRÓN** `ibs_20min` > `0.8027` → IC=+0.145 (n=471)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` > 0.8027 (IC base=-0.015)

- **PATRÓN** `dist_vwap_pct` > `0.3182` → IC=+0.223 (n=153)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3182 (IC base=-0.015)

- **PATRÓN** `volumen_regimen` < `0.9496` → IC=+0.215 (n=275)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9496 (IC base=-0.015)

- **PATRÓN** `volumen_regimen` > `0.5518` → IC=+0.191 (n=312)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` > 0.5518 (IC base=-0.015)

- **PATRÓN** `volumen_pendiente_norm` < `0.108` → IC=+0.217 (n=288)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2693` → IC=+0.316 (n=36)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2693 (IC base=-0.015)

- **PATRÓN** `volumen_spike_ratio` < `1.4909` → IC=+0.260 (n=127)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4909 (IC base=-0.015)

- **PATRÓN** `volumen_spike_ratio` > `1.7478` → IC=+0.222 (n=192)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7478 (IC base=-0.015)

- **PATRÓN** `ballena_activa_n` < `115.0` → IC=+0.258 (n=192)
...
- **FILTRO** `ibs_20min` < `0.6667` → IC=-0.207 (n=824)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.254 (n=834)

- **FILTRO** `ibs_20min` > `0.7143` → IC=-0.236 (n=438)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7143
  - _Potencial_: sin este filtro IC_bueno=+0.082 (n=1315)

- **FILTRO** `sigma_ewma_delta_pct` > `4.704` → IC=-0.170 (n=410)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.704
  - _Potencial_: sin este filtro IC_bueno=+0.055 (n=1343)

- **PATRÓN** `ibs_20min` > `0.6667` → IC=+0.254 (n=834)
...
- **PATRÓN** `volumen_regimen` < `0.8605` → IC=+0.293 (n=380)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8605 (IC base=+0.025)

- **PATRÓN** `volumen_regimen` > `0.7191` → IC=+0.275 (n=509)
...
- **PATRÓN** `sigma_h` > `0.0152` → IC=+0.323 (n=694)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.9048` → IC=+0.345 (n=694)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.383` → IC=+0.297 (n=554)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.383 (IC base=+0.269)

- **PATRÓN** `volumen_regimen` > `0.6815` → IC=+0.285 (n=930)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5494` → IC=+0.275 (n=429)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5494 (IC base=+0.269)

- **PATRÓN** `volumen_spike_ratio` > `2.207` → IC=+0.273 (n=442)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.207 (IC base=+0.269)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.273 (n=1075)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.269)

- **PATRÓN** `libro_liquidez` > `2421.677` → IC=+0.273 (n=930)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.429` → IC=+0.289 (n=411)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5474` → IC=+0.265 (n=975)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5474 (IC base=+0.268)

- **PATRÓN** `volumen_spike_ratio` > `2.1719` → IC=+0.266 (n=442)
...
- **PATRÓN** `libro_liquidez` > `2331.9466` → IC=+0.274 (n=1015)
...
- **PATRÓN** `sigma_h` > `0.0103` → IC=+0.202 (n=2009)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0103 (IC base=+0.168)

- **PATRÓN** `drift_60min` |x|≤ `0.3349` → IC=+0.173 (n=5287)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.3349 (IC base=+0.168)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.177 (n=6292)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 5.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.289 (n=2778)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.225` → IC=+0.249 (n=1239)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.225 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` < `1.2161` → IC=+0.163 (n=4006)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.2161 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` > `0.6208` → IC=+0.160 (n=4006)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.6208 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.1069` → IC=+0.186 (n=2352)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1069 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` < `1.5664` → IC=+0.172 (n=2512)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.5664 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.6687` → IC=+0.168 (n=1903)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 2.6687 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `3822.0558` → IC=+0.172 (n=2003)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 3822.0558 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `123.0` → IC=+0.184 (n=4922)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 123.0 (IC base=+0.168)

- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.188 (n=3898)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0063 (IC base=+0.171)

- **PATRÓN** `drift_60min` |x|≤ `0.0795` → IC=+0.205 (n=1949)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0795 (IC base=+0.171)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.202 (n=2831)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.171)

- **PATRÓN** `ibs_20min` < `0.4593` → IC=+0.226 (n=5839)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4593 (IC base=+0.171)

- **PATRÓN** `dist_vwap_pct` < `0.2265` → IC=+0.162 (n=4344)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2265 (IC base=+0.171)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.265` → IC=+0.195 (n=1013)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 10.265 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` < `1.1909` → IC=+0.157 (n=4264)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.1909 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` > `0.6238` → IC=+0.151 (n=4264)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.6238 (IC base=+0.171)

- **PATRÓN** `volumen_pendiente_norm` > `0.2927` → IC=+0.229 (n=835)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2927 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` < `1.575` → IC=+0.172 (n=2281)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.575 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` > `2.6529` → IC=+0.179 (n=1728)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` > 2.6529 (IC base=+0.171)

- **PATRÓN** `ballena_activa_n` < `125.0` → IC=+0.172 (n=4798)
...
- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.202 (n=461)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0071 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.314` → IC=+0.204 (n=1013)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.237 (n=759)
...
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.181 (n=876)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0049 (IC base=+0.150)

- **PATRÓN** `drift_60min` |x|≤ `0.2905` → IC=+0.166 (n=994)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.2905 (IC base=+0.150)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.165 (n=919)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 7.0 (IC base=+0.150)

- **PATRÓN** `ibs_20min` < `0.5289` → IC=+0.192 (n=994)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` < 0.5289 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` < `0.1407` → IC=+0.167 (n=1008)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` < 0.1407 (IC base=+0.150)
...
- **PATRÓN** `volumen_regimen` < `1.2113` → IC=+0.165 (n=994)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.2113 (IC base=+0.150)

- **PATRÓN** `volumen_pendiente_norm` > `0.1594` → IC=+0.181 (n=308)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1594 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` < `2.4411` → IC=+0.162 (n=885)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.4411 (IC base=+0.150)

- **PATRÓN** `ballena_activa_n` < `230.0` → IC=+0.159 (n=268)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 230.0 (IC base=+0.150)

### GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0058` → IC=+0.198 (n=990)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0058 (IC base=+0.188)

- **PATRÓN** `drift_60min` |x|≤ `0.1947` → IC=+0.196 (n=660)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.1947 (IC base=+0.188)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.218 (n=342)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1072` → IC=+0.185 (n=813)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` < 0.1072 (IC base=+0.188)

- **PATRÓN** `volumen_pendiente_norm` > `0.218` → IC=+0.185 (n=284)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.218 (IC base=+0.188)

- **PATRÓN** `volumen_spike_ratio` < `1.6803` → IC=+0.187 (n=308)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` < 1.6803 (IC base=+0.188)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.202 (n=1115)
...
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.149 (n=750)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0077 (IC base=+0.111)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.131 (n=1153)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_regimen` < `1.2203` → IC=+0.122 (n=1120)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.123 (n=1153)
...
- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.136 (n=817)

  - _Acción_: Kelly boost +0.68€ cuando `ballena_activa_n` < 49.0 (IC base=+0.111)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.150 (n=493)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0058 (IC base=+0.115)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.174 (n=519)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 15.0 (IC base=+0.115)

- **PATRÓN** `ibs_20min` < `0.5435` → IC=+0.208 (n=1117)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5435 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` > `0.7144` → IC=+0.145 (n=198)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.7144 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` < `0.1935` → IC=+0.138 (n=1039)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.1935 (IC base=+0.115)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.336` → IC=+0.158 (n=238)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 7.336 (IC base=+0.115)

- **PATRÓN** `volumen_regimen` < `1.0417` → IC=+0.132 (n=983)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_regimen` < 1.0417 (IC base=+0.115)

- **PATRÓN** `volumen_pendiente_norm` > `0.276` → IC=+0.179 (n=135)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.276 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` < `1.465` → IC=+0.133 (n=325)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 1.465 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` > `2.1743` → IC=+0.141 (n=441)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 2.1743 (IC base=+0.115)

- **PATRÓN** `libro_liquidez` > `3082.2041` → IC=+0.163 (n=372)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 3082.2041 (IC base=+0.115)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.217 (n=564)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.2688` → IC=+0.204 (n=1178)
...
- **PATRÓN** `volumen_regimen` > `0.6282` → IC=+0.219 (n=1132)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2447` → IC=+0.197 (n=876)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 2.2447 (IC base=+0.204)

- **PATRÓN** `volumen_spike_ratio` > `1.4611` → IC=+0.192 (n=996)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.696` → IC=+0.167 (n=656)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` > 3.696 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `0.8626` → IC=+0.154 (n=828)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.142 (n=1595)

  - _Acción_: Kelly boost +0.71€ cuando `libro_spread` < 0.02 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `8101.5764` → IC=+0.158 (n=655)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 8101.5764 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.159 (n=1225)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 156.0 (IC base=+0.136)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.139 (n=1014)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0057 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.125 (n=1541)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 5.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` < `0.4799` → IC=+0.156 (n=1336)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` < 0.4799 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` < `0.2174` → IC=+0.123 (n=1343)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.2174 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.996` → IC=+0.133 (n=448)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` > 5.996 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` < `1.2259` → IC=+0.123 (n=1339)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2374` → IC=+0.134 (n=1276)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 2.2374 (IC base=+0.114)

- **PATRÓN** `ballena_activa_n` < `27.0` → IC=+0.126 (n=613)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3449` → IC=+0.149 (n=471)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.3449 (IC base=+0.125)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.143 (n=424)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 7.0 (IC base=+0.125)

- **PATRÓN** `ibs_20min` < `0.3451` → IC=+0.206 (n=314)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3451 (IC base=+0.125)

- **PATRÓN** `dist_vwap_pct` < `0.1864` → IC=+0.144 (n=473)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.1864 (IC base=+0.125)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.384` → IC=+0.131 (n=193)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` > 4.384 (IC base=+0.125)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.142` → IC=+0.126 (n=532)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 9.142 (IC base=+0.125)

- **PATRÓN** `volumen_regimen` < `1.1917` → IC=+0.130 (n=471)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 1.1917 (IC base=+0.125)

- **PATRÓN** `volumen_regimen` > `1.0598` → IC=+0.157 (n=214)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 1.0598 (IC base=+0.125)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1165` → IC=+0.154 (n=406)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.1165 (IC base=+0.125)
...
- **PATRÓN** `sigma_h` < `0.0037` → IC=+0.259 (n=197)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.0954` → IC=+0.211 (n=147)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0954 (IC base=+0.192)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.229 (n=300)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 12.0 (IC base=+0.192)

- **PATRÓN** `ibs_20min` > `0.2679` → IC=+0.228 (n=439)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.2679 (IC base=+0.192)

- **PATRÓN** `dist_vwap_pct` > `0.15` → IC=+0.219 (n=222)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.15 (IC base=+0.192)
...
- **PATRÓN** `volumen_regimen` < `0.8331` → IC=+0.199 (n=294)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.252` → IC=+0.312 (n=62)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.252 (IC base=+0.192)
...
- **FILTRO** `dist_vwap_pct` > `0.3993` → IC=-0.206 (n=15)

  - _Acción_: SKIP cuando `dist_vwap_pct` > 0.3993
  - _Potencial_: sin este filtro IC_bueno=+0.097 (n=363)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.180 (n=145)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 15.0 (IC base=+0.078)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.328` → IC=+0.154 (n=134)
  ...
  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 5.328 (IC base=+0.078)

- **PATRÓN** `libro_liquidez` > `2971.9929` → IC=+0.164 (n=102)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 2971.9929 (IC base=+0.078)

- **PATRÓN** `ibs_20min` < `0.4545` → IC=+0.166 (n=285)

  - _Acción_: Kelly boost +0.83€ cuando `ibs_20min` < 0.4545 (IC base=+0.084)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.921` → IC=+0.141 (n=51)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 8.921 (IC base=+0.084)

- **PATRÓN** `volumen_regimen` < `0.7139` → IC=+0.153 (n=125)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.7139 (IC base=+0.084)

- **PATRÓN** `volumen_pendiente_norm` < `0.1024` → IC=+0.121 (n=262)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_pendiente_norm` < 0.1024 (IC base=+0.084)

- **PATRÓN** `volumen_spike_ratio` < `2.513` → IC=+0.126 (n=260)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_spike_ratio` < 2.513 (IC base=+0.084)

- **PATRÓN** `ballena_activa_n` < `44.0` → IC=+0.129 (n=230)

  - _Acción_: Kelly boost +0.65€ cuando `ballena_activa_n` < 44.0 (IC base=+0.084)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.193 (n=3445)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0086 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.175 (n=7973)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 5.0 (IC base=+0.165)

- **PATRÓN** `ibs_20min` > `0.4722` → IC=+0.212 (n=7599)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4722 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` > `0.9198` → IC=+0.205 (n=981)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9198 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.577` → IC=+0.223 (n=3723)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.577 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` < `0.882` → IC=+0.166 (n=3414)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.882 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` > `0.2402` → IC=+0.186 (n=1440)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.2402 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `2.6402` → IC=+0.183 (n=2412)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` > 2.6402 (IC base=+0.165)

- **PATRÓN** `libro_liquidez` > `3774.9265` → IC=+0.171 (n=2533)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 3774.9265 (IC base=+0.165)

- **PATRÓN** `ballena_activa_n` < `94.0` → IC=+0.195 (n=5478)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 94.0 (IC base=+0.165)

- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.196 (n=4668)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0067 (IC base=+0.182)

- **PATRÓN** `drift_60min` |x|≤ `0.4797` → IC=+0.184 (n=6998)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.4797 (IC base=+0.182)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.203 (n=2706)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.182)

- **PATRÓN** `ibs_20min` < `0.5556` → IC=+0.239 (n=7000)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5556 (IC base=+0.182)

- **PATRÓN** `dist_vwap_pct` < `0.2362` → IC=+0.164 (n=4429)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.2362 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.852` → IC=+0.201 (n=1004)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.852 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` < `0.6257` → IC=+0.161 (n=1612)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.6257 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` > `1.2035` → IC=+0.161 (n=1611)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 1.2035 (IC base=+0.182)

- **PATRÓN** `volumen_pendiente_norm` > `0.2899` → IC=+0.250 (n=906)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2899 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` > `2.3011` → IC=+0.190 (n=2840)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` > 2.3011 (IC base=+0.182)

- **PATRÓN** `ballena_activa_n` < `26.0` → IC=+0.188 (n=2009)
...
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.230 (n=582)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.007 (IC base=+0.194)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.199 (n=622)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.865` → IC=+0.313 (n=576)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.865 (IC base=+0.194)

- **PATRÓN** `volumen_pendiente_norm` > `0.2778` → IC=+0.238 (n=166)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2778 (IC base=+0.194)

- **PATRÓN** `volumen_spike_ratio` < `1.4609` → IC=+0.187 (n=397)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 1.4609 (IC base=+0.194)

- **PATRÓN** `volumen_spike_ratio` > `1.873` → IC=+0.194 (n=794)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 1.873 (IC base=+0.194)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.222 (n=1157)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.194)

- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.262 (n=673)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.1832` → IC=+0.153 (n=803)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_spike_ratio` < `2.132` → IC=+0.159 (n=1012)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.132 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` > `1.7608` → IC=+0.158 (n=767)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 1.7608 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `10760.7327` → IC=+0.169 (n=1075)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 10760.7327 (IC base=+0.148)

- **PATRÓN** `ballena_activa_n` < `495.0` → IC=+0.163 (n=1086)

  - _Acción_: Kelly boost +0.81€ cuando `ballena_activa_n` < 495.0 (IC base=+0.148)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.169 (n=1080)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0057 (IC base=+0.157)

- **PATRÓN** `drift_60min` |x|≤ `0.263` → IC=+0.171 (n=950)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.263 (IC base=+0.157)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.177 (n=370)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 18.0 (IC base=+0.157)

- **PATRÓN** `ibs_20min` < `0.6376` → IC=+0.206 (n=1080)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.255 (n=443)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.230)

- **PATRÓN** `ibs_20min` < `0.3653` → IC=+0.267 (n=1022)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.274` → IC=+0.164 (n=549)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 4.274 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `0.8789` → IC=+0.160 (n=859)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5053` → IC=+0.147 (n=545)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.5053 (IC base=+0.136)
...
- **PATRÓN** `libro_liquidez` > `8750.1763` → IC=+0.229 (n=584)
...
- **PATRÓN** `ibs_20min` < `0.3284` → IC=+0.230 (n=691)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3284 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.2052` → IC=+0.145 (n=967)
...
- **PATRÓN** `volumen_regimen` < `0.8599` → IC=+0.144 (n=691)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.432` → IC=+0.220 (n=491)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.432 (IC base=+0.099)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.125 (n=913)

  - _Acción_: Kelly boost +0.63€ cuando `libro_spread` < 0.01 (IC base=+0.099)

- **PATRÓN** `libro_liquidez` > `2907.4324` → IC=+0.242 (n=436)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2907.4324 (IC base=+0.099)

- **PATRÓN** `ballena_activa_n` < `53.0` → IC=+0.129 (n=966)

  - _Acción_: Kelly boost +0.65€ cuando `ballena_activa_n` < 53.0 (IC base=+0.099)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.151 (n=600)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 15.0 (IC base=+0.115)

- **PATRÓN** `ibs_20min` < `0.625` → IC=+0.209 (n=1264)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.625 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` < `0.4776` → IC=+0.129 (n=1247)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.4776 (IC base=+0.115)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.437` → IC=+0.125 (n=1218)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 3.437 (IC base=+0.115)

- **PATRÓN** `volumen_regimen` < `0.7163` → IC=+0.156 (n=556)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.7163 (IC base=+0.115)

- **PATRÓN** `volumen_pendiente_norm` > `0.2186` → IC=+0.172 (n=193)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.2186 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` < `1.4623` → IC=+0.141 (n=369)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.4623 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` > `2.222` → IC=+0.126 (n=501)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_spike_ratio` > 2.222 (IC base=+0.115)

- **PATRÓN** `libro_liquidez` > `2917.7847` → IC=+0.169 (n=421)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 2917.7847 (IC base=+0.115)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` > `0.0181` → IC=+0.213 (n=881)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.1867` → IC=+0.235 (n=749)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1867 (IC base=+0.205)
...
- **PATRÓN** `volumen_regimen` < `1.2439` → IC=+0.207 (n=1321)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2439 (IC base=+0.205)

- **PATRÓN** `volumen_regimen` > `0.6262` → IC=+0.208 (n=1321)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6262 (IC base=+0.205)

- **PATRÓN** `volumen_pendiente_norm` > `0.0799` → IC=+0.227 (n=533)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.213 (n=1327)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.205)

- **PATRÓN** `libro_liquidez` > `2565.5344` → IC=+0.209 (n=881)
...
- **PATRÓN** `sigma_h` > `0.0254` → IC=+0.225 (n=486)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0254 (IC base=+0.201)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.212 (n=711)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5039` → IC=+0.255 (n=1456)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5039 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` < `0.2745` → IC=+0.205 (n=1352)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2745 (IC base=+0.201)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.01` → IC=+0.263 (n=276)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2257` → IC=+0.196 (n=1123)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.2257 (IC base=+0.201)

- **PATRÓN** `volumen_spike_ratio` > `1.4427` → IC=+0.196 (n=1276)
...
- **PATRÓN** `libro_liquidez` > `2541.6579` → IC=+0.203 (n=971)
...
- **FILTRO** `sigma_ewma_delta_pct` > `3.354` → IC=-0.260 (n=23)
...
- **PATRÓN** `py_entrada` > `0.5` → IC=+0.124 (n=456)

  - _Acción_: Kelly boost +0.62€ cuando `py_entrada` > 0.5 (IC base=+0.112)

- **PATRÓN** `libro_liquidez` > `2822.2308` → IC=+0.169 (n=161)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2822.2308 (IC base=+0.112)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.125 (n=1014)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=5422)

- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=6196)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.179 (n=2608)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=7930)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.171 (n=2689)
...
- **FILTRO** `py_entrada` < `0.43` → IC=-0.218 (n=427)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.43
  - _Potencial_: sin este filtro IC_bueno=+0.092 (n=1345)

- **FILTRO** `ibs_20min` < `0.748` → IC=-0.178 (n=442)

  - _Acción_: SKIP cuando `ibs_20min` < 0.748
  - _Potencial_: sin este filtro IC_bueno=+0.082 (n=1330)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.162 (n=450)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=+0.052 (n=1508)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.43` → IC=-0.199 (n=447)

  - _Acción_: SKIP cuando `py_entrada` < 0.43
  - _Potencial_: sin este filtro IC_bueno=+0.080 (n=1375)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.214 (n=459)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.060 (n=1473)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.201 (n=429)
...
- **FILTRO** `py_entrada` > `0.58` → IC=-0.196 (n=481)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=1454)

### MOMENTUM_IBS_15M_FADE
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=2383)

- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)

  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=2388)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=7497)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.081 (n=17301)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.283 (n=5750)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=19048)

- **FILTRO** `ibs_7min` < `0.7059` → IC=-0.242 (n=6184)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7059
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=18614)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.161 (n=8229)

  - _Acción_: SKIP cuando `ballena_activa_n` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=16569)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.224 (n=7716)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=23358)

- **FILTRO** `ibs_7min` > `0.2963` → IC=-0.178 (n=7764)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2963
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=23310)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.312 (n=935)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=3037)

- **FILTRO** `ibs_7min` < `0.7097` → IC=-0.260 (n=1309)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7097
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=2663)

- **FILTRO** `ballena_activa_n` > `9.0` → IC=-0.194 (n=954)

  - _Acción_: SKIP cuando `ballena_activa_n` > 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=3018)

- **FILTRO** `py_entrada` > `0.505` → IC=-0.148 (n=3625)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.099 (n=1786)

- **FILTRO** `ibs_7min` > `0.7976` → IC=-0.206 (n=1352)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.139 (n=988)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.082 (n=3349)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.257 (n=1006)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=3331)

- **FILTRO** `ibs_7min` < `0.7595` → IC=-0.188 (n=1084)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7595
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=3253)

- **FILTRO** `ballena_activa_n` > `161.0` → IC=-0.173 (n=1077)

  - _Acción_: SKIP cuando `ballena_activa_n` > 161.0
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=3260)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.254 (n=1027)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=3371)

- **FILTRO** `ibs_7min` > `0.2509` → IC=-0.167 (n=1099)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2509
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=3299)

- **FILTRO** `ballena_activa_n` > `114.0` → IC=-0.163 (n=1489)

  - _Acción_: SKIP cuando `ballena_activa_n` > 114.0
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=2909)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.181 (n=898)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.095 (n=2828)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.322 (n=890)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=2836)

- **FILTRO** `drift_7min_pct` |x|> `0.1817` → IC=-0.129 (n=1266)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1817
  - _Potencial_: sin este filtro IC_bueno=-0.108 (n=2460)

- **FILTRO** `ibs_7min` < `0.2034` → IC=-0.275 (n=930)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2034
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=2796)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.219 (n=896)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=2830)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.235 (n=1333)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=4347)

- **FILTRO** `ibs_7min` > `0.2633` → IC=-0.158 (n=1931)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2633
  - _Potencial_: sin este filtro IC_bueno=+0.016 (n=3749)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.127 (n=1277)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.090 (n=2799)
...
- **FILTRO** `ibs_7min` < `0.7454` → IC=-0.193 (n=1019)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7454
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3057)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.183 (n=1017)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3059)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.261 (n=1021)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=3159)

- **FILTRO** `ibs_7min` > `0.2745` → IC=-0.175 (n=1044)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2745
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3136)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.179 (n=1039)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=3141)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.243 (n=1120)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=3377)

- **FILTRO** `ibs_7min` < `0.7234` → IC=-0.215 (n=1123)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7234
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=3374)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.176 (n=1413)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.124 (n=1246)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.075 (n=2944)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.280 (n=1008)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=3182)

- **FILTRO** `ibs_7min` < `0.7315` → IC=-0.231 (n=1047)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7315
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=3143)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.207 (n=1035)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=3155)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.205 (n=1313)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=4214)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=2310)

### STRUCT_NO_15M#SOL#15min
- **FILTRO** `py_entrada` < `0.495` → IC=-0.147 (n=32)

  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=1188)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0587` → IC=+0.175 (n=1057)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.88€ cuando `delta_ratio_macro` |x|> 0.0587 (IC base=+0.171)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.093` → IC=+0.226 (n=246)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.093 (IC base=+0.171)
...
- **PATRÓN** `sigma_h` > `0.0224` → IC=+0.267 (n=101)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0224 (IC base=+0.180)

- **PATRÓN** `drift_60min` |x|≤ `0.0857` → IC=+0.199 (n=134)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.0857 (IC base=+0.180)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0668` → IC=+0.192 (n=271)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.96€ cuando `delta_ratio_macro` |x|> 0.0668 (IC base=+0.180)
...
- **PATRÓN** `ibs_15` > `0.5385` → IC=+0.274 (n=303)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5385 (IC base=+0.180)

- **PATRÓN** `dist_vwap_pct` > `0.1742` → IC=+0.199 (n=161)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1742 (IC base=+0.180)

- **PATRÓN** `dist_vwap_pct` < `0.3093` → IC=+0.180 (n=289)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.3093 (IC base=+0.180)

- **PATRÓN** `sigma_ewma_delta_pct` > `15.55` → IC=+0.232 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 15.55 (IC base=+0.180)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.489` → IC=+0.179 (n=269)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` < 7.489 (IC base=+0.180)

- **PATRÓN** `libro_liquidez` > `2714.4944` → IC=+0.229 (n=138)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2714.4944 (IC base=+0.180)
...
- **FILTRO** `sigma_h` > `0.007` → IC=-0.201 (n=316)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.197 (n=949)

- **FILTRO** `sigma_h` < `0.0032` → IC=-0.232 (n=315)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0032
  - _Potencial_: sin este filtro IC_bueno=-0.187 (n=950)

- **FILTRO** `hora_utc` > `7.0` → IC=-0.211 (n=811)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.175 (n=454)

- **FILTRO** `sigma_ewma_delta_pct` > `19.944` → IC=-0.247 (n=227)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 19.944
  - _Potencial_: sin este filtro IC_bueno=-0.188 (n=1038)
...
- **PATRÓN** `sigma_h` < `0.0075` → IC=+0.247 (n=465)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0075 (IC base=+0.223)

- **PATRÓN** `drift_60min` |x|≤ `0.4382` → IC=+0.228 (n=465)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4382 (IC base=+0.223)

- **PATRÓN** `drift_15min` |x|≤ `0.7675` → IC=+0.228 (n=409)
...
- **PATRÓN** `hora_utc` > `19.0` → IC=+0.244 (n=158)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` < `0.361` → IC=+0.269 (n=465)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.361 (IC base=+0.223)

- **PATRÓN** `dist_vwap_pct` > `0.3857` → IC=+0.245 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3857 (IC base=+0.223)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.263` → IC=+0.233 (n=174)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.263 (IC base=+0.223)

- **PATRÓN** `sigma_ewma_delta_pct` < `12.221` → IC=+0.226 (n=494)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 12.221 (IC base=+0.223)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6176 sube el IC de +0.171 a +0.242 en UPDOWN_GBM#15min (n=1057). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_YES, IBS > 0.5385 sube el IC de +0.180 a +0.274 en UPDOWN_GBM#XRP#15min (n=303). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1053 sube el IC de +0.049 a +0.179 en UPDOWN_GBM#XRP#15min (n=334). Ya aplicado como kelly_boost=+0.89€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3542 sube el IC de -0.046 a +0.278 en UPDOWN_GBM_15M_TARDIO (n=1105). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.361 sube el IC de +0.223 a +0.269 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=465). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 20344 | -0.103 | -2881.16€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1234 | -0.044 | -178.84€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 19110 | -0.107 | -2702.32€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3141 | -0.122 | -556.49€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3141 | -0.122 | -556.49€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 1234 | -0.044 | -178.84€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 1234 | -0.044 | -178.84€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 374 | -0.136 | -161.05€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 374 | -0.136 | -161.05€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 5875 | -0.050 | -582.29€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 5875 | -0.050 | -582.29€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 5392 | -0.114 | -430.56€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 5392 | -0.114 | -430.56€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 4328 | -0.162 | -971.92€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 4328 | -0.162 | -971.92€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 12008 | -0.043 | +4247.34€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 3231 | -0.008 | +1849.67€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 8777 | -0.055 | +2397.67€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 12008 | -0.043 | +4247.34€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 3231 | -0.008 | +1849.67€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 8777 | -0.055 | +2397.67€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO | 917 | -0.104 | -139.23€ | 2 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#15min | 58 | -0.083 | -10.97€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#5min | 859 | -0.105 | -128.26€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#BNB | 22 | -0.083 | +4.56€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#BNB#5min | 22 | -0.083 | +4.56€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#BTC | 525 | -0.081 | -72.75€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#BTC#15min | 40 | -0.071 | -5.95€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#BTC#5min | 485 | -0.081 | -66.80€ | 2 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#ETH | 256 | -0.159 | -57.56€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#ETH#15min | 18 | -0.090 | -5.02€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#ETH#5min | 238 | -0.163 | -52.54€ | 2 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 73764 | +0.113 | -3818.74€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 11557 | +0.182 | -367.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 284 | -0.115 | -45.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 56810 | +0.100 | -3299.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 5113 | +0.116 | -105.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 9468 | +0.097 | -882.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 37 | -0.141 | -0.23€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 9416 | +0.098 | -870.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 14909 | +0.131 | -316.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 3525 | +0.201 | -120.48€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 9413 | +0.109 | -189.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1929 | +0.116 | +15.97€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 9507 | +0.089 | -936.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 44 | -0.065 | -4.30€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#DOGE#240min | 15 | -0.243 | -11.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 9448 | +0.090 | -920.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 15800 | +0.125 | -298.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 4457 | +0.172 | -75.26€ | 1 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 9493 | +0.108 | -167.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1838 | +0.100 | -47.86€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 14598 | +0.116 | -821.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 3457 | +0.185 | -177.35€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 187 | -0.071 | +8.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 9608 | +0.092 | -578.64€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1346 | +0.138 | -73.34€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#XRP | 9482 | +0.103 | -563.84€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 37 | -0.013 | +9.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 13 | -0.022 | -0.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 9432 | +0.103 | -573.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 11660 | +0.189 | -800.77€ | 3 | 5 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 11660 | +0.189 | -800.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 2867 | +0.169 | -305.63€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 2867 | +0.169 | -305.63€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC | 609 | +0.182 | +8.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC#15min | 609 | +0.182 | +8.01€ | 4 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 2815 | +0.176 | -265.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 2815 | +0.176 | -265.05€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 2532 | +0.236 | -77.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 2532 | +0.236 | -77.43€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 