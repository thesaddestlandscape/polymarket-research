# Hipótesis automáticas — 2026-08-18 05:19 UTC
_Generado por shadow_postmortem.py sobre 59052 resoluciones (PNL=+6254.13€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `143.14` → IC=-0.269 (n=773)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 143.14
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=2320)
...
- **FILTRO** `restante_s_al_confirmar` > `638.33` → IC=-0.264 (n=108)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` > 638.33
  - _Potencial_: sin este filtro IC_bueno=+0.082 (n=326)

### BALLENAS_TARDIAS#DOGE#5min
- **FILTRO** `restante_s_al_confirmar` < `111.95` → IC=-0.452 (n=145)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.204 (n=2064)
...
- **PATRÓN** `py_entrada` < `0.275` → IC=+0.328 (n=155)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.275 (IC base=+0.191)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.192 (n=413)
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.142 (n=439)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` < `0.315` → IC=+0.339 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.315 (IC base=+0.227)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.251 (n=287)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.227)

- **PATRÓN** `libro_liquidez` > `911.1943` → IC=+0.237 (n=287)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 911.1943 (IC base=+0.227)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.196 (n=1137)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.203 (n=876)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.159 (n=641)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 5.0 (IC base=+0.159)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.184 (n=283)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.218 (n=561)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.218)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.230 (n=494)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.203 (n=1493)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.192)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.230 (n=2781)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.165 (n=267)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 5.0 (IC base=+0.148)

- **PATRÓN** `py_entrada` > `0.76` → IC=+0.216 (n=255)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.157 (n=1411)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 5.0 (IC base=+0.151)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.159 (n=1424)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.147 (n=423)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` > `0.327` → IC=+0.170 (n=89)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` > 0.327 (IC base=+0.109)

- **PATRÓN** `volumen_spike_ratio` > `1.6939` → IC=+0.131 (n=573)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 1.6939 (IC base=+0.109)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.122 (n=1225)

  - _Acción_: Kelly boost +0.61€ cuando `libro_spread` < 0.06 (IC base=+0.109)

- **PATRÓN** `ibs_20min` < `0.6577` → IC=+0.123 (n=2053)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` < 0.6577 (IC base=+0.080)

- **PATRÓN** `volumen_regimen` < `0.6291` → IC=+0.145 (n=153)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.6291 (IC base=+0.080)

- **PATRÓN** `volumen_regimen` > `0.6951` → IC=+0.140 (n=409)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 0.6951 (IC base=+0.080)

- **PATRÓN** `volumen_pendiente_norm` > `0.1255` → IC=+0.276 (n=159)
...
- **PATRÓN** `volumen_spike_ratio` > `2.9145` → IC=+0.237 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9145 (IC base=+0.080)

- **PATRÓN** `ballena_activa_n` < `166.0` → IC=+0.300 (n=48)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 166.0 (IC base=+0.080)
...
- **PATRÓN** `sigma_h` < `0.0018` → IC=+0.162 (n=72)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0018 (IC base=+0.141)

- **PATRÓN** `sigma_h` > `0.0024` → IC=+0.155 (n=143)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` > 0.0024 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.1903` → IC=+0.165 (n=189)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.82€ cuando `drift_60min` |x|≤ 0.1903 (IC base=+0.141)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.190 (n=156)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 12.0 (IC base=+0.141)

- **PATRÓN** `ibs_20min` < `0.455` → IC=+0.213 (n=214)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.455 (IC base=+0.141)
...
- **PATRÓN** `volumen_regimen` < `1.2823` → IC=+0.148 (n=214)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 1.2823 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` > `1.1352` → IC=+0.187 (n=97)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` > 1.1352 (IC base=+0.141)
...
- **PATRÓN** `volumen_spike_ratio` > `2.625` → IC=+0.192 (n=50)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 2.625 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `5224.4606` → IC=+0.142 (n=191)

  - _Acción_: Kelly boost +0.71€ cuando `libro_liquidez` > 5224.4606 (IC base=+0.141)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.190 (n=114)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` < 5.0 (IC base=+0.131)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8278` → IC=+0.123 (n=258)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_spike_ratio` > 1.8278 (IC base=+0.131)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.154 (n=368)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.06 (IC base=+0.131)

- **PATRÓN** `libro_liquidez` > `1914.6584` → IC=+0.147 (n=154)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 1914.6584 (IC base=+0.131)
...
- **PATRÓN** `ibs_20min` > `0.4899` → IC=+0.161 (n=60)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `ibs_20min` > 0.4899 (IC base=+0.066)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.253` → IC=+0.210 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.253 (IC base=+0.066)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.29` → IC=+0.132 (n=935)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` > 2.29 (IC base=+0.059)
...
- **PATRÓN** `sigma_h` > `0.0074` → IC=+0.277 (n=110)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0074 (IC base=+0.181)

- **PATRÓN** `drift_60min` |x|≤ `0.066` → IC=+0.188 (n=110)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.066 (IC base=+0.181)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.247 (n=156)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1466` → IC=+0.194 (n=230)
  ...
  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` < 0.1466 (IC base=+0.181)

- **PATRÓN** `volumen_pendiente_norm` > `0.4263` → IC=+0.192 (n=37)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.4263 (IC base=+0.181)

- **PATRÓN** `volumen_spike_ratio` < `4.8756` → IC=+0.167 (n=247)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 4.8756 (IC base=+0.181)

- **PATRÓN** `volumen_spike_ratio` > `2.7873` → IC=+0.195 (n=165)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 2.7873 (IC base=+0.181)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.200 (n=351)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.06 (IC base=+0.181)

- **PATRÓN** `libro_liquidez` > `1905.341` → IC=+0.215 (n=149)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1905.341 (IC base=+0.181)
...
- **FILTRO** `ibs_20min` < `0.6102` → IC=-0.126 (n=137)
...
- **PATRÓN** `ibs_20min` > `0.4444` → IC=+0.132 (n=134)
...
- **PATRÓN** `sigma_h` < `0.0046` → IC=+0.154 (n=394)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0046 (IC base=+0.136)

- **PATRÓN** `sigma_h` > `0.007` → IC=+0.200 (n=394)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.154 (n=533)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` < 7.0 (IC base=+0.136)

- **PATRÓN** `ibs_20min` > `0.9206` → IC=+0.253 (n=788)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9206 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.2183` → IC=+0.144 (n=254)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.2183 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.44` → IC=+0.248 (n=787)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.44 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` > `0.6285` → IC=+0.132 (n=614)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_regimen` > 0.6285 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.1138` → IC=+0.160 (n=383)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` > 0.1138 (IC base=+0.136)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.144 (n=1317)

  - _Acción_: Kelly boost +0.72€ cuando `libro_spread` < 0.06 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `2638.9696` → IC=+0.154 (n=394)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1483` → IC=+0.165 (n=183)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.82€ cuando `drift_60min` |x|≤ 0.1483 (IC base=+0.152)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.868` → IC=+0.331 (n=75)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.868 (IC base=+0.152)

- **PATRÓN** `volumen_pendiente_norm` < `0.2322` → IC=+0.134 (n=211)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_pendiente_norm` < 0.2322 (IC base=+0.152)

- **PATRÓN** `volumen_spike_ratio` < `2.0554` → IC=+0.198 (n=94)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 2.0554 (IC base=+0.152)

- **PATRÓN** `volumen_spike_ratio` > `4.0428` → IC=+0.146 (n=97)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 4.0428 (IC base=+0.152)
...
- **PATRÓN** `libro_liquidez` > `1956.263` → IC=+0.199 (n=91)
  ...
  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 1956.263 (IC base=+0.152)
...
- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.292 (n=51)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0029 (IC base=+0.267)

- **PATRÓN** `drift_60min` |x|≤ `0.0943` → IC=+0.306 (n=34)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0943 (IC base=+0.267)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.285 (n=77)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.267)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.275 (n=69)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.267)

- **PATRÓN** `ibs_20min` > `0.7754` → IC=+0.321 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7754 (IC base=+0.267)

- **PATRÓN** `dist_vwap_pct` > `0.2179` → IC=+0.271 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2179 (IC base=+0.267)

- **PATRÓN** `dist_vwap_pct` < `0.3343` → IC=+0.281 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3343 (IC base=+0.267)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.979` → IC=+0.426 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.979 (IC base=+0.267)

- **PATRÓN** `volumen_regimen` > `1.1837` → IC=+0.321 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1837 (IC base=+0.267)

- **PATRÓN** `volumen_pendiente_norm` > `0.1159` → IC=+0.314 (n=41)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1159 (IC base=+0.267)

- **PATRÓN** `volumen_spike_ratio` < `1.6353` → IC=+0.308 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6353 (IC base=+0.267)

- **PATRÓN** `volumen_spike_ratio` > `2.6482` → IC=+0.294 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6482 (IC base=+0.267)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.238 (n=1088)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0061 (IC base=+0.232)

- **PATRÓN** `sigma_h` > `0.0038` → IC=+0.239 (n=1235)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0038 (IC base=+0.232)

- **PATRÓN** `drift_60min` |x|≤ `0.1571` → IC=+0.232 (n=823)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1571 (IC base=+0.232)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.243 (n=1306)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.232)

- **PATRÓN** `ibs_20min` < `0.4787` → IC=+0.294 (n=1234)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4787 (IC base=+0.232)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.181` → IC=+0.269 (n=240)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.181 (IC base=+0.232)

- **PATRÓN** `volumen_regimen` < `0.6183` → IC=+0.209 (n=276)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6183 (IC base=+0.232)

- **PATRÓN** `volumen_regimen` > `1.0811` → IC=+0.224 (n=375)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0811 (IC base=+0.232)

- **PATRÓN** `volumen_pendiente_norm` > `0.1869` → IC=+0.317 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1869 (IC base=+0.232)

- **PATRÓN** `volumen_spike_ratio` < `1.677` → IC=+0.307 (n=241)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.677 (IC base=+0.232)
...
- **PATRÓN** `sigma_h` < `0.0018` → IC=+0.203 (n=72)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0018 (IC base=+0.172)

- **PATRÓN** `sigma_h` > `0.0024` → IC=+0.183 (n=143)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` > 0.0024 (IC base=+0.172)

- **PATRÓN** `drift_60min` |x|≤ `0.1928` → IC=+0.191 (n=189)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.96€ cuando `drift_60min` |x|≤ 0.1928 (IC base=+0.172)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.195 (n=195)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 8.0 (IC base=+0.172)

- **PATRÓN** `ibs_20min` < `0.402` → IC=+0.222 (n=214)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.402 (IC base=+0.172)
...
- **PATRÓN** `volumen_regimen` < `0.6202` → IC=+0.189 (n=72)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_regimen` < 0.6202 (IC base=+0.172)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5501` → IC=+0.300 (n=53)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5501 (IC base=+0.172)

- **PATRÓN** `libro_liquidez` > `4779.9919` → IC=+0.189 (n=191)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 4779.9919 (IC base=+0.172)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.302 (n=245)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.293)

- **PATRÓN** `ibs_20min` < `0.5294` → IC=+0.344 (n=235)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5294 (IC base=+0.293)

- **PATRÓN** `volumen_pendiente_norm` > `0.3976` → IC=+0.340 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3976 (IC base=+0.293)

- **PATRÓN** `volumen_spike_ratio` < `3.7526` → IC=+0.304 (n=100)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.7526 (IC base=+0.293)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.293 (n=274)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.06 (IC base=+0.293)
...
- **PATRÓN** `drift_60min` |x|≤ `0.05` → IC=+0.227 (n=31)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.05 (IC base=+0.159)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.177 (n=91)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 4.0 (IC base=+0.159)

- **PATRÓN** `ibs_20min` < `0.0955` → IC=+0.309 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0955 (IC base=+0.159)

- **PATRÓN** `dist_vwap_pct` > `0.0972` → IC=+0.260 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.0972 (IC base=+0.159)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.829` → IC=+0.295 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.829 (IC base=+0.159)

- **PATRÓN** `volumen_regimen` < `0.8847` → IC=+0.220 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8847 (IC base=+0.159)

- **PATRÓN** `volumen_pendiente_norm` < `0.0562` → IC=+0.435 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0562 (IC base=+0.159)

- **PATRÓN** `volumen_spike_ratio` > `1.5396` → IC=+0.361 (n=34)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5396 (IC base=+0.159)

- **PATRÓN** `libro_liquidez` > `4542.625` → IC=+0.227 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4542.625 (IC base=+0.159)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.157 (n=68)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 18.0 (IC base=+0.107)

- **PATRÓN** `drift_20min_pct` |x|≤ `0.0526` → IC=+0.250 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `drift_20min_pct` |x|≤ 0.0526 (IC base=+0.107)

- **PATRÓN** `libro_liquidez` > `14343.9197` → IC=+0.172 (n=62)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 14343.9197 (IC base=+0.107)
...
- **FILTRO** `py_entrada` < `0.485` → IC=-0.174 (n=44)
...
- **FILTRO** `ibs_20min` < `1.0` → IC=-0.184 (n=17)
...
- **FILTRO** `py_entrada` > `0.53` → IC=-0.149 (n=35)

  - _Acción_: SKIP cuando `py_entrada` > 0.53
  - _Potencial_: sin este filtro IC_bueno=+0.182 (n=20)

- **PATRÓN** `py_entrada` < `0.53` → IC=+0.182 (n=20)

  - _Acción_: Kelly boost +0.91€ cuando `py_entrada` < 0.53 (IC base=-0.026)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0804` → IC=+0.124 (n=312)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.62€ cuando `delta_ratio_macro` |x|> 0.0804 (IC base=+0.091)

- **PATRÓN** `ibs_15` < `0.2213` → IC=+0.178 (n=209)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.89€ cuando `ibs_15` < 0.2213 (IC base=+0.091)

- **PATRÓN** `dist_vwap_pct` > `0.4942` → IC=+0.208 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4942 (IC base=+0.091)

- **PATRÓN** `libro_liquidez` > `3522.6985` → IC=+0.132 (n=104)

  - _Acción_: Kelly boost +0.66€ cuando `libro_liquidez` > 3522.6985 (IC base=+0.091)
...
- **PATRÓN** `sigma_h` > `0.0043` → IC=+0.196 (n=54)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0043 (IC base=+0.173)

- **PATRÓN** `drift_60min` |x|≤ `0.4838` → IC=+0.191 (n=82)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.4838 (IC base=+0.173)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1143` → IC=+0.193 (n=73)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.97€ cuando `delta_ratio_macro` |x|> 0.1143 (IC base=+0.173)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.197 (n=74)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 6.0 (IC base=+0.173)

- **PATRÓN** `hora_utc` < `14.0` → IC=+0.188 (n=75)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` < 14.0 (IC base=+0.173)

- **PATRÓN** `ibs_15` < `0.2213` → IC=+0.203 (n=72)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.2213 (IC base=+0.173)

- **PATRÓN** `ibs_15` > `0.032` → IC=+0.223 (n=81)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.032 (IC base=+0.173)

- **PATRÓN** `dist_vwap_pct` > `0.1727` → IC=+0.273 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1727 (IC base=+0.173)

- **PATRÓN** `sigma_ewma_delta_pct` < `17.886` → IC=+0.224 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 17.886 (IC base=+0.173)

- **PATRÓN** `libro_liquidez` > `4741.1738` → IC=+0.232 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4741.1738 (IC base=+0.173)
...
- **FILTRO** `sigma_h` < `0.0042` → IC=-0.155 (n=265)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_NO, IBS < 0.2213 sube el IC de +0.091 a +0.178 en UPDOWN_GBM#15min (n=209). Ya aplicado como kelly_boost=+0.89€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_NO, IBS < 0.2213 sube el IC de +0.173 a +0.203 en UPDOWN_GBM#ETH#15min (n=72). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_NO, IBS > 0.032 sube el IC de +0.173 a +0.223 en UPDOWN_GBM#ETH#15min (n=81). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 3093 | -0.119 | -489.57€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 434 | -0.005 | -6.67€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 2659 | -0.137 | -482.90€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 346 | -0.195 | -106.23€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 346 | -0.195 | -106.23€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC |
 | 2 | 2 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL | 64 | -0.030 | -4.59€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL#15min | 64 | -0.030 | -4.59€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 935 | -0.064 | -73.65€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 935 | -0.064 | -73.65€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 145 | -0.051 | +9.82€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 145 | -0.051 | +9.82€ | 2 | 1 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 171 | -0.084 | -22.58€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 171 | -0.084 | -22.58€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 157 | -0.053 | -20.30€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 157 | -0.053 | -20.30€ | 6 | 1 |
...
| ✅ MOMENTUM_IBS_5M_FADE | 1003 | +0.017 | +5.40€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 1003 | +0.017 | +5.40€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB | 167 | +0.062 | +13.10€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB#5min | 167 | +0.062 | +13.10€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC | 155 | +0.041 | +0.06€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC#5min | 155 | +0.041 | +0.06€ | 2 | 2 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE | 170 | -0.006 | -2.84€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE#5min | 170 | -0.006 | -2.84€ | 0 | 0 |
...
| ✅ STRUCT_NO_15M | 1761 | +0.011 | -11.86€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 1761 | +0.011 | -11.86€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 671 | +0.007 | -8.01€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 671 | +0.007 | -8.01€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 702 | +0.014 | -2.58€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 702 | +0.014 | -2.58€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 388 | +0.013 | -1.27€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 388 | +0.013 | -1.27€ | 2 | 0 |
| ✅ UPDOWN_GBM | 1887 | +0.021 | +108.09€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 828 | +0.094 | +159.26€ | 1 | 9 |
| ✅ UPDOWN_GBM#240min | 112 | +0.026 | +0.60€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 654 | -0.038 | -40.62€ | 5 | 0 |
...
| ✅ UPDOWN_GBM#DOGE | 273 | +0.005 | +4.48€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 53 | +0.154 | +19.98€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 220 | -0.032 | -15.50€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 442 | +0.054 | +48.78€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 197 | +0.138 | +54.92€ | 1 | 15 |
...
| ✅ UPDOWN_GBM#SOL | 333 | -0.004 | +2.74€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 131 | +0.019 | +0.89€ | 1 | 2 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1537 | -0.041 | +182.23€ | 3 | 2 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1537 | -0.041 | +182.23€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 99 | -0.074 | -2.65€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 99 | -0.074 | -2.65€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 306 | -0.130 | -0.41€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 306 | -0.130 | -0.41€ | 3 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 491 | +0.001 | +103.53€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 491 | +0.001 | +103.53€ | 1 | 1 |
...
| ✅ UPDOWN_OU_5M | 316 | -0.063 | -25.47€ | 3 | 0 |
| ✅ UPDOWN_OU_5M#5min | 316 | -0.063 | -25.47€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#BNB | 204 | -0.015 | -12.38€ | 0 | 0 |
| ✅ UPDOWN_OU_5M#BNB#5min | 204 | -0.015 | -12.38€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.157: overbought→boost, oversold→filtro | oversold(IBS<0.3): IC=-0.033 n=641 | neutral: IC=+0.003 n=656 | overbought(IBS>0.7): IC=+0.124 n=886
  - _Datos_: n=2333 IC=+0.041 PNL=+176.61€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=05h: IC=-0.123 n=67 PNL=-9.40€ → FILTRAR | H=14h: IC=+0.120 n=90 PNL=+25.02€ → BOOST | H=23h: IC=-0.149 n=112 PNL=-29.64€ → FILTRAR

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.019 < 0.08 — monitorear
  - _Datos_: n=131 IC=+0.019 PNL=+0.89€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.110 n=34770 | tras_1loss IC=+0.066 n=23987 | tras_2loss IC=+0.023 n=10368/40 | gap=+0.086 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1752 IC=+0.021 PNL=+109.52€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1752 IC=+0.021 PNL=+109.52€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.089 > 0.08 con n=523 PNL=+73.42€
  - _Datos_: n=523 IC=+0.089 PNL=+73.42€
...
**🟡 H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.101 > 0.1 con n=354 PNL=+97.02€
  - _Datos_: n=354 IC=+0.101 PNL=+97.02€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1107 IC=-0.131 PNL=+119.29€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1107 IC=-0.131 PNL=+119.29€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=285 IC=+0.141 PNL=+84.92€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=285 IC=+0.141 PNL=+84.92€
...
**〰️ H-CUSTOM-GBM-04H-ASIA** — UPDOWN_GBM 04h-05h UTC — media sesión asiática, ¿mejor franja nocturna?
  ...
  - _Estado_: n=197 IC=-0.033 PNL=-11.73€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=197 IC=-0.033 PNL=-11.73€
...
**🟡 H-CUSTOM-LATE-04H-ASIA** — GBM_LATE_15M 04h-05h UTC — media sesión asiática (gemela de GBM-04H-ASIA)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.128 > 0.08 con n=364 PNL=+189.01€
  - _Datos_: n=364 IC=+0.128 PNL=+189.01€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=736 IC=+0.159 PNL=+388.74€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=736 IC=+0.159 PNL=+388.74€
...
**〰️ H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: n=319 IC=+0.002 PNL=+23.06€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=319 IC=+0.002 PNL=+23.06€
