# Hipótesis automáticas — 2026-07-18 16:34 UTC
_Generado por shadow_postmortem.py sobre 20913 resoluciones (PNL=+4993.68€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.203 (n=1305)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.188)

- **PATRÓN** `py_entrada` > `0.69` → IC=+0.296 (n=568)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.69 (IC base=+0.188)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.193 (n=1455)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.185 (n=1533)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.231 (n=251)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.221)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.236 (n=286)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.223 (n=348)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.221)

- **PATRÓN** `libro_liquidez` > `3157.2453` → IC=+0.224 (n=273)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3157.2453 (IC base=+0.221)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.231 (n=139)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.395` → IC=+0.266 (n=288)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.126 (n=1743)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `11.059` → IC=+0.231 (n=154)
...
- **PATRÓN** `dist_vwap_pct` > `0.1641` → IC=+0.124 (n=320)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` > 0.1641 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` < `0.9224` → IC=+0.121 (n=1030)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.9224 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.913` → IC=+0.141 (n=154)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 8.913 (IC base=+0.111)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `14.154` → IC=+0.265 (n=32)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.476` → IC=+0.210 (n=67)
...
- **PATRÓN** `dist_vwap_pct` < `0.6049` → IC=+0.159 (n=297)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.185 (n=706)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 12.0 (IC base=+0.164)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.166 (n=1048)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 18.0 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.5883` → IC=+0.222 (n=160)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5883 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.023` → IC=+0.314 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.023 (IC base=+0.164)

- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.186 (n=990)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0046 (IC base=+0.168)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.183 (n=477)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.8942` → IC=+0.186 (n=897)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` < 0.8942 (IC base=+0.168)
...
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.138 (n=158)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0041 (IC base=+0.130)

- **PATRÓN** `sigma_h` > `0.0031` → IC=+0.148 (n=211)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0031 (IC base=+0.130)
...
- **PATRÓN** `dist_vwap_pct` > `0.4762` → IC=+0.224 (n=27)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4762 (IC base=+0.130)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.007` → IC=+0.261 (n=44)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.007 (IC base=+0.130)
...
- **PATRÓN** `dist_vwap_pct` > `0.5316` → IC=+0.190 (n=56)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.5316 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.778` → IC=+0.267 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.778 (IC base=+0.148)
...
- **PATRÓN** `sigma_h` < `0.0123` → IC=+0.149 (n=243)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0123 (IC base=+0.115)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.171 (n=83)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 5.0 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` < `0.8313` → IC=+0.160 (n=201)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` < 0.8313 (IC base=+0.115)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.666` → IC=+0.175 (n=158)
...
- **PATRÓN** `sigma_h` > `0.008` → IC=+0.318 (n=240)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.1789` → IC=+0.331 (n=57)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1789 (IC base=+0.300)

- **PATRÓN** `dist_vwap_pct` < `0.6163` → IC=+0.315 (n=236)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6163 (IC base=+0.300)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.458` → IC=+0.317 (n=206)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.136 (n=825)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5248` → IC=+0.200 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5248 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.39` → IC=+0.303 (n=120)
...
- **PATRÓN** `dist_vwap_pct` < `0.7968` → IC=+0.130 (n=889)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.7968 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.536` → IC=+0.121 (n=230)

  - _Acción_: Kelly boost +0.60€ cuando `sigma_ewma_delta_pct` > 3.536 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.916` → IC=+0.125 (n=779)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 10.916 (IC base=+0.106)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0046` → IC=+0.150 (n=138)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0046 (IC base=+0.114)

- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.134 (n=69)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` > 0.0072 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.160 (n=186)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 8.0 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` > `0.4306` → IC=+0.219 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4306 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.119` → IC=+0.257 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.119 (IC base=+0.114)
...
- **PATRÓN** `sigma_h` < `0.0099` → IC=+0.232 (n=140)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.216 (n=287)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.5545` → IC=+0.219 (n=286)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5545 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.003` → IC=+0.226 (n=264)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.588` → IC=-0.155 (n=27)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.138 (n=266)
...
- **PATRÓN** `dist_vwap_pct` > `0.133` → IC=+0.159 (n=133)
  ...
  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.133 (IC base=+0.032)

- **PATRÓN** `dist_vwap_pct` < `0.2752` → IC=+0.141 (n=168)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.2752 (IC base=+0.032)

- **PATRÓN** `sigma_ewma_delta_pct` > `23.418` → IC=+0.155 (n=27)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 23.418 (IC base=+0.032)

- **PATRÓN** `ibs_15` < `0.0263` → IC=+0.129 (n=173)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.64€ cuando `ibs_15` < 0.0263 (IC base=+0.062)

- **PATRÓN** `dist_vwap_pct` > `0.9263` → IC=+0.152 (n=21)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.9263 (IC base=+0.062)

- **PATRÓN** `sigma_ewma_delta_pct` < `17.588` → IC=+0.138 (n=266)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` < 17.588 (IC base=+0.062)
...
- **FILTRO** `sigma_ewma_delta_pct` < `21.876` → IC=-0.130 (n=25)
...
- **PATRÓN** `dist_vwap_pct` < `0.5043` → IC=+0.137 (n=89)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.5043 (IC base=+0.030)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.258` → IC=+0.214 (n=19)
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 186 | +0.149 | +14.59€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 186 | +0.149 | +14.59€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 85 | +0.144 | +2.18€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 85 | +0.144 | +2.18€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 63 | +0.192 | +10.72€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 63 | +0.192 | +10.72€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 38 | +0.075 | +1.69€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 38 | +0.075 | +1.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 3462 | +0.183 | +21.22€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#15min | 2248 | +0.216 | +69.18€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 1005 | +0.194 | +30.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 749 | +0.208 | -4.79€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 1233 | +0.176 | +1.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 745 | +0.217 | +33.85€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 1212 | +0.182 | -8.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 743 | +0.224 | +43.09€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 5567 | +0.105 | +1838.91€ | 0 | 8 |
| ✅ GBM_LATE_15M#15min | 