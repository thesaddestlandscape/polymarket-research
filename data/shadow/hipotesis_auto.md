# Hipótesis automáticas — 2026-07-28 02:08 UTC
_Generado por shadow_postmortem.py sobre 39341 resoluciones (PNL=+8005.94€)_
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.187 (n=1223)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.190 (n=2777)
...
- **PATRÓN** `py_entrada` < `0.575` → IC=+0.154 (n=53)
  ...
  - _Acción_: Kelly boost +0.77€ cuando `py_entrada` < 0.575 (IC base=+0.139)

- **PATRÓN** `py_entrada` > `0.575` → IC=+0.147 (n=148)

  - _Acción_: Kelly boost +0.73€ cuando `py_entrada` > 0.575 (IC base=+0.139)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.169 (n=146)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.01 (IC base=+0.139)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.130 (n=2354)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 8.0 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `0.4679` → IC=+0.125 (n=326)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` > 0.4679 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.988` → IC=+0.207 (n=401)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.988 (IC base=+0.113)
...
- **PATRÓN** `dist_vwap_pct` > `0.6697` → IC=+0.181 (n=70)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.6697 (IC base=+0.141)
...
- **PATRÓN** `sigma_h` > `0.0074` → IC=+0.185 (n=1190)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0074 (IC base=+0.139)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.163 (n=1599)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 8.0 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` > `0.279` → IC=+0.189 (n=493)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.279 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.0` → IC=+0.241 (n=388)
...
- **PATRÓN** `sigma_h` < `0.0177` → IC=+0.248 (n=414)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0177 (IC base=+0.229)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.247 (n=381)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.229)

- **PATRÓN** `dist_vwap_pct` > `0.2602` → IC=+0.278 (n=124)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2602 (IC base=+0.229)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.797` → IC=+0.353 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.797 (IC base=+0.229)
...
- **PATRÓN** `dist_vwap_pct` > `0.4756` → IC=+0.157 (n=322)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.4756 (IC base=+0.094)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.21` → IC=+0.228 (n=281)
...
- **PATRÓN** `sigma_h` < `0.02` → IC=+0.180 (n=463)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.02 (IC base=+0.170)

- **PATRÓN** `sigma_h` > `0.0138` → IC=+0.173 (n=350)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0138 (IC base=+0.170)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.177 (n=472)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 8.0 (IC base=+0.170)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.171 (n=548)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 18.0 (IC base=+0.170)

- **PATRÓN** `dist_vwap_pct` > `0.4727` → IC=+0.240 (n=102)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4727 (IC base=+0.170)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.958` → IC=+0.282 (n=99)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.958 (IC base=+0.170)
...
| ✅ FAVORITO_CONFIRMADO | 6851 | +0.178 | -41.82€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#60min | 1209 | +0.153 | +73.61€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 2402 | +0.169 | -37.87€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 403 | +0.140 | +14.52€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#SOL | 2377 | +0.183 | +0.32€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 8452 | +0.094 | +2529.16€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 8452 | +0.094 | +2529.16€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 16 | +0.133 | +2.30€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 16 | +0.133 | +2.30€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 2041 | +0.072 | +349.97€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 2041 | +0.072 | +349.97€ | 0 | 5 |
| ✅ GBM_LATE_15M#DOGE | 17 | -0.022 | -0.12€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 17 | -0.022 | -0.12€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#XRP | 2270 | +0.140 | +1093.26€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 2270 | +0.140 | +1093.26€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 5688 | +0.114 | +2591.79€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 5688 | +0.114 | +2591.79€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 18 | +0.090 | +4.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 18 | +0.090 | +4.60€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 1276 | +0.215 | +1046.85€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 1276 | +0.215 | +1046.85€ | 0 | 10 |
...
| ✅ GBM_LATE_15M_TARDIO | 5681 | +0.065 | +1468.42€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#15min | 5681 | +0.065 | +1468.42€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB | 15 | +0.199 | +7.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BNB#15min | 15 | +0.199 | +7.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 1353 | +0.029 | +152.78€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 1353 | +0.029 | +152.78€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#DOGE | 17 | +0.112 | +2.69€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#DOGE#15min | 17 | +0.112 | +2.69€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 1582 | +0.162 | +953.89€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1582 | +0.162 | +953.89€ | 0 | 12 |
...
| ✅ UPDOWN_GBM | 2921 | +0.036 | +265.67€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 2657 | +0.053 | +304.69€ | 0 | 3 |
...
| ✅ UPDOWN_GBM#DOGE | 259 | +0.013 | +9.31€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 256 | +0.015 | +10.21€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 2182 | +0.151 | +963.50€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 2182 | +0.151 | +963.50€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 211 | +0.214 | +141.09€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 211 | +0.214 | +141.09€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.162: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.075 n=1670 | neutral: IC=+0.023 n=1324 | overbought(IBS>0.7): IC=+0.185 n=1630
  - _Datos_: n=4928 IC=+0.104 PNL=+1293.11€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.110 n=23366 | tras_1loss IC=+0.077 n=15760 | tras_2loss IC=+0.051 n=6625/40 | gap=+0.059 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2784 IC=+0.041 PNL=+279.47€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2784 IC=+0.041 PNL=+279.47€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.091 > 0.08 con n=1404 PNL=+240.79€
  - _Datos_: n=1404 IC=+0.091 PNL=+240.79€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=1469 IC=+0.043 PNL=+123.72€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1469 IC=+0.043 PNL=+123.72€

**〰️ H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  - _Hipótesis_: Feature nueva 2026-06-27: drift del precio YES en Polymarket en últimas 5 obs (~5min). Si poly_drift<0 y decidimos BUY_NO (o poly_drift>0 y BUY_YES) → confluencia. Si diverge → reducción de stake. Hipótesis: confluencia Binance+Polymarket mejora IC; divergencia empeora.
  - _Umbral_: n≥40 en confluencia vs divergencia para validar el boost ×1.1
  - _Acción_: Si IC_confluencia>IC_divergencia con n≥40 → mantener el boost. Si no → retirar.
  - _Estado_: n=422 IC=+0.057 PNL=+50.58€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=422 IC=+0.057 PNL=+50.58€
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=3059 IC=+0.054 PNL=+350.50€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=3059 IC=+0.054 PNL=+350.50€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=434 IC=+0.018 PNL=+13.01€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=434 IC=+0.018 PNL=+13.01€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.205 > 0.2 con n=672 PNL=+386.16€
  - _Datos_: n=672 IC=+0.205 PNL=+386.16€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.126 > 0.08 con n=750 PNL=+365.64€
  - _Datos_: n=750 IC=+0.126 PNL=+365.64€
