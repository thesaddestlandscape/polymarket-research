# Hipótesis automáticas — 2026-09-15 19:17 UTC
_Generado por shadow_postmortem.py sobre 455667 resoluciones (PNL=+49106.60€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `145.98` → IC=-0.255 (n=5674)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 145.98
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=17027)
...
- **FILTRO** `restante_s_al_confirmar` < `135.32` → IC=-0.282 (n=705)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 135.32
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=2117)

### BALLENAS_TARDIAS#ETH#5min
- **FILTRO** `restante_s_al_confirmar` < `141.37` → IC=-0.156 (n=1462)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 141.37
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=4386)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `158.58` → IC=-0.253 (n=1340)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.58
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=4021)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `156.97` → IC=-0.329 (n=1420)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 156.97
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=2884)

### CANDIDATA9_BOT_CONSENSO
- **FILTRO** `py_entrada` < `0.38` → IC=-0.267 (n=148)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.163 (n=96)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=333)

### CANDIDATA9_BOT_CONSENSO#BTC#5min
- **FILTRO** `py_entrada` < `0.43` → IC=-0.303 (n=59)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.195 (n=11552)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.146 (n=8796)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.345` → IC=+0.245 (n=7821)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.146 (n=2312)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.143 (n=678)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.203 (n=9003)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.220 (n=3203)
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.180 (n=2236)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.341 (n=168)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.348 (n=341)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.294)

### FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.182 (n=2221)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.200 (n=2149)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.186 (n=4896)

  - _Acción_: Kelly boost +0.93€ cuando `py_entrada` > 0.71 (IC base=+0.169)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.225 (n=4749)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `4.16` → IC=+0.149 (n=1832)

  - _Acción_: Kelly boost +0.75€ cuando `lag_apertura_s` < 4.16 (IC base=+0.130)
...
- **PATRÓN** `restante_min` < `4.48` → IC=+0.125 (n=2470)

  - _Acción_: Kelly boost +0.63€ cuando `restante_min` < 4.48 (IC base=+0.119)

- **PATRÓN** `restante_min` > `4.96` → IC=+0.147 (n=1888)
...
- **PATRÓN** `lag_apertura_s` < `4.39` → IC=+0.145 (n=2460)

  - _Acción_: Kelly boost +0.73€ cuando `lag_apertura_s` < 4.39 (IC base=+0.119)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9773` → IC=+0.231 (n=2014)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9773 (IC base=+0.097)

- **PATRÓN** `dist_vwap_pct` < `0.6094` → IC=+0.244 (n=1621)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6094 (IC base=+0.097)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.857` → IC=+0.167 (n=2336)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 5.857 (IC base=+0.097)

- **PATRÓN** `volumen_regimen` < `0.6118` → IC=+0.254 (n=507)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6118 (IC base=+0.097)

- **PATRÓN** `volumen_regimen` > `1.0681` → IC=+0.247 (n=689)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0681 (IC base=+0.097)

- **PATRÓN** `volumen_pendiente_norm` < `0.0811` → IC=+0.193 (n=3559)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` < 0.0811 (IC base=+0.097)

- **PATRÓN** `volumen_pendiente_norm` > `0.3093` → IC=+0.206 (n=566)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3093 (IC base=+0.097)

- **PATRÓN** `volumen_spike_ratio` > `1.9323` → IC=+0.200 (n=2633)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9323 (IC base=+0.097)

- **PATRÓN** `ibs_20min` < `0.5684` → IC=+0.130 (n=7430)

  - _Acción_: Kelly boost +0.65€ cuando `ibs_20min` < 0.5684 (IC base=+0.059)

- **PATRÓN** `dist_vwap_pct` > `0.5804` → IC=+0.181 (n=447)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.5804 (IC base=+0.059)

- **PATRÓN** `dist_vwap_pct` < `0.3493` → IC=+0.168 (n=2543)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` < 0.3493 (IC base=+0.059)

- **PATRÓN** `volumen_regimen` < `0.6996` → IC=+0.171 (n=1058)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 0.6996 (IC base=+0.059)

- **PATRÓN** `volumen_regimen` > `0.8712` → IC=+0.174 (n=1602)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 0.8712 (IC base=+0.059)

- **PATRÓN** `volumen_pendiente_norm` > `0.2462` → IC=+0.224 (n=808)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2462 (IC base=+0.059)

- **PATRÓN** `volumen_spike_ratio` > `1.4644` → IC=+0.198 (n=4000)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.4644 (IC base=+0.059)

- **PATRÓN** `ballena_activa_n` < `157.0` → IC=+0.207 (n=3767)
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.189 (n=452)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0076 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.3228` → IC=+0.167 (n=1355)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.3228 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.169 (n=656)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `drift_60min` |x|≤ `0.1117` → IC=+0.237 (n=451)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1117 (IC base=+0.213)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.228 (n=1027)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.2132` → IC=+0.219 (n=525)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2132 (IC base=+0.213)
...
- **PATRÓN** `volumen_regimen` < `1.2629` → IC=+0.223 (n=1025)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2629 (IC base=+0.213)

- **PATRÓN** `volumen_regimen` > `0.8751` → IC=+0.214 (n=683)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8751 (IC base=+0.213)

- **PATRÓN** `volumen_pendiente_norm` > `0.0993` → IC=+0.220 (n=373)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3852` → IC=+0.219 (n=333)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3852 (IC base=+0.213)

- **PATRÓN** `libro_liquidez` > `11907.94` → IC=+0.226 (n=916)
...
- **PATRÓN** `volumen_regimen` < `1.2085` → IC=+0.151 (n=1103)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.2085 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` > `0.6867` → IC=+0.143 (n=985)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4315` → IC=+0.155 (n=994)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.4315 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `13156.4982` → IC=+0.157 (n=735)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 13156.4982 (IC base=+0.141)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.178 (n=654)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.471` → IC=+0.230 (n=376)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.471 (IC base=+0.177)

- **PATRÓN** `volumen_pendiente_norm` < `0.1063` → IC=+0.183 (n=1122)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` < 0.1063 (IC base=+0.177)

- **PATRÓN** `volumen_pendiente_norm` > `0.377` → IC=+0.184 (n=172)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.377 (IC base=+0.177)

- **PATRÓN** `volumen_spike_ratio` > `3.043` → IC=+0.198 (n=561)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 3.043 (IC base=+0.177)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.190 (n=1511)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.04 (IC base=+0.177)

- **PATRÓN** `sigma_h` < `0.0104` → IC=+0.221 (n=1150)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0104 (IC base=+0.213)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.248 (n=435)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.213)

- **PATRÓN** `ibs_20min` < `0.3796` → IC=+0.232 (n=1012)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3796 (IC base=+0.213)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `7.606` → IC=+0.215 (n=1353)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 7.606 (IC base=+0.213)

- **PATRÓN** `volumen_pendiente_norm` > `0.365` → IC=+0.274 (n=166)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.365 (IC base=+0.213)

- **PATRÓN** `volumen_spike_ratio` < `1.8406` → IC=+0.204 (n=451)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8406 (IC base=+0.213)

- **PATRÓN** `volumen_spike_ratio` > `2.2947` → IC=+0.222 (n=682)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2947 (IC base=+0.213)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.225 (n=588)
...
- **PATRÓN** `ballena_activa_n` < `18.0` → IC=+0.216 (n=427)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.152 (n=90)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.009 (n=1677)
...
- **FILTRO** `ibs_20min` > `0.2727` → IC=-0.128 (n=1712)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2727
  - _Potencial_: sin este filtro IC_bueno=+0.118 (n=851)

- **FILTRO** `sigma_ewma_delta_pct` > `8.618` → IC=-0.201 (n=279)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.618
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=2284)
...
- **PATRÓN** `volumen_regimen` > `0.9112` → IC=+0.183 (n=159)
  ...
  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` > 0.9112 (IC base=-0.046)

- **PATRÓN** `volumen_pendiente_norm` < `0.203` → IC=+0.205 (n=198)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.203 (IC base=-0.046)
...
- **FILTRO** `drift_60min` |x|> `0.6568` → IC=-0.193 (n=424)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6568
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=1276)
...
- **FILTRO** `ibs_20min` > `0.7708` → IC=-0.202 (n=642)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7708
  - _Potencial_: sin este filtro IC_bueno=+0.030 (n=1929)
...
- **PATRÓN** `volumen_spike_ratio` > `2.7847` → IC=+0.237 (n=1360)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7847 (IC base=+0.086)

- **PATRÓN** `ballena_activa_n` < `102.0` → IC=+0.277 (n=3563)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 102.0 (IC base=+0.086)

- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.139 (n=2580)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0084 (IC base=+0.067)

- **PATRÓN** `ibs_20min` < `0.5556` → IC=+0.150 (n=6797)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` < 0.5556 (IC base=+0.067)

- **PATRÓN** `dist_vwap_pct` > `0.686` → IC=+0.244 (n=385)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.686 (IC base=+0.067)

- **PATRÓN** `dist_vwap_pct` < `0.2338` → IC=+0.230 (n=2012)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2338 (IC base=+0.067)

- **PATRÓN** `volumen_regimen` < `0.7144` → IC=+0.230 (n=921)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7144 (IC base=+0.067)

- **PATRÓN** `volumen_regimen` > `1.2011` → IC=+0.251 (n=698)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2011 (IC base=+0.067)

- **PATRÓN** `volumen_pendiente_norm` > `0.2525` → IC=+0.326 (n=554)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2525 (IC base=+0.067)

- **PATRÓN** `volumen_spike_ratio` < `1.6249` → IC=+0.254 (n=1197)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6249 (IC base=+0.067)

- **PATRÓN** `volumen_spike_ratio` > `2.37` → IC=+0.256 (n=1233)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.37 (IC base=+0.067)

- **PATRÓN** `ballena_activa_n` < `79.0` → IC=+0.255 (n=2582)
...
- **FILTRO** `sigma_ewma_delta_pct` > `4.412` → IC=-0.164 (n=388)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=1297)

- **PATRÓN** `ibs_20min` > `0.8645` → IC=+0.252 (n=515)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8645 (IC base=+0.039)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.29` → IC=+0.150 (n=696)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 3.29 (IC base=+0.039)

- **PATRÓN** `volumen_pendiente_norm` > `0.2255` → IC=+0.291 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2255 (IC base=+0.039)

- **PATRÓN** `volumen_spike_ratio` < `1.8494` → IC=+0.197 (n=354)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 1.8494 (IC base=+0.039)

- **PATRÓN** `volumen_spike_ratio` > `2.6662` → IC=+0.198 (n=177)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 2.6662 (IC base=+0.039)
...
- **FILTRO** `ibs_20min` > `0.8269` → IC=-0.148 (n=569)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8269
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=1709)
...
- **PATRÓN** `volumen_regimen` < `0.843` → IC=+0.202 (n=256)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.843 (IC base=+0.000)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1101` → IC=+0.255 (n=1003)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1101 (IC base=+0.237)

- **PATRÓN** `volumen_spike_ratio` < `1.8762` → IC=+0.238 (n=491)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8762 (IC base=+0.237)

- **PATRÓN** `volumen_spike_ratio` > `3.6325` → IC=+0.251 (n=372)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.6325 (IC base=+0.237)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.254 (n=1356)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.237)

- **PATRÓN** `sigma_h` > `0.0055` → IC=+0.294 (n=962)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0055 (IC base=+0.278)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.318 (n=327)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.278)

- **PATRÓN** `ibs_20min` < `0.3333` → IC=+0.285 (n=963)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3333 (IC base=+0.278)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.885` → IC=+0.293 (n=370)
...
- **FILTRO** `ibs_20min` > `0.8119` → IC=-0.184 (n=451)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8119
  - _Potencial_: sin este filtro IC_bueno=+0.037 (n=1354)

- **PATRÓN** `ibs_20min` > `0.8024` → IC=+0.144 (n=467)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` > 0.8024 (IC base=-0.016)

- **PATRÓN** `dist_vwap_pct` > `0.3121` → IC=+0.226 (n=155)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3121 (IC base=-0.016)
...
- **PATRÓN** `dist_vwap_pct` > `0.1692` → IC=+0.181 (n=92)
  ...
  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.1692 (IC base=-0.018)

- **PATRÓN** `dist_vwap_pct` < `0.2845` → IC=+0.164 (n=230)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.2845 (IC base=-0.018)

- **PATRÓN** `volumen_regimen` < `0.979` → IC=+0.168 (n=197)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 0.979 (IC base=-0.018)
...
- **FILTRO** `ibs_20min` > `0.7143` → IC=-0.237 (n=435)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.082 (n=1308)

- **FILTRO** `sigma_ewma_delta_pct` > `4.689` → IC=-0.170 (n=407)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.689
  - _Potencial_: sin este filtro IC_bueno=+0.055 (n=1336)
...
- **PATRÓN** `sigma_h` > `0.0151` → IC=+0.322 (n=690)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.2672` → IC=+0.316 (n=530)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2672 (IC base=+0.268)
...
- **PATRÓN** `volumen_regimen` > `0.6862` → IC=+0.286 (n=924)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6862 (IC base=+0.268)

- **PATRÓN** `volumen_pendiente_norm` > `0.2383` → IC=+0.304 (n=207)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2383 (IC base=+0.268)

- **PATRÓN** `volumen_spike_ratio` < `2.5657` → IC=+0.271 (n=969)
...
- **PATRÓN** `volumen_regimen` > `1.248` → IC=+0.310 (n=377)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.248 (IC base=+0.268)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5387` → IC=+0.265 (n=968)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5387 (IC base=+0.268)

- **PATRÓN** `volumen_spike_ratio` > `2.1719` → IC=+0.264 (n=439)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1719 (IC base=+0.268)

- **PATRÓN** `libro_liquidez` > `2331.157` → IC=+0.273 (n=1009)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2331.157 (IC base=+0.268)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.203 (n=2781)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.171)

- **PATRÓN** `ibs_20min` < `0.459` → IC=+0.227 (n=5802)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.459 (IC base=+0.171)

- **PATRÓN** `dist_vwap_pct` < `0.2252` → IC=+0.162 (n=4308)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2252 (IC base=+0.171)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.27` → IC=+0.195 (n=1008)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 10.27 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` < `1.1918` → IC=+0.157 (n=4239)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 1.1918 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` > `0.6264` → IC=+0.152 (n=4239)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5751` → IC=+0.173 (n=2265)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.5751 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` > `2.6529` → IC=+0.179 (n=1716)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` > 2.6529 (IC base=+0.171)

- **PATRÓN** `ballena_activa_n` < `126.0` → IC=+0.172 (n=4766)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.105` → IC=+0.309 (n=460)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.105 (IC base=+0.190)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5605` → IC=+0.183 (n=914)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` < 2.5605 (IC base=+0.190)

- **PATRÓN** `volumen_spike_ratio` > `1.5423` → IC=+0.186 (n=817)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 1.5423 (IC base=+0.190)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.212 (n=926)
...
- **PATRÓN** `ibs_20min` < `0.3333` → IC=+0.257 (n=730)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3333 (IC base=+0.236)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.049` → IC=+0.251 (n=790)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0963` → IC=+0.235 (n=597)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0963 (IC base=+0.236)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8747` → IC=+0.248 (n=439)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8747 (IC base=+0.236)

- **PATRÓN** `volumen_spike_ratio` > `2.679` → IC=+0.243 (n=220)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.679 (IC base=+0.236)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.237 (n=750)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.236)

- **PATRÓN** `libro_liquidez` > `1593.12` → IC=+0.251 (n=652)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1593.12 (IC base=+0.236)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1692` → IC=+0.176 (n=579)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.1692 (IC base=+0.164)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.201 (n=600)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.686` → IC=+0.229 (n=234)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.686 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` < `1.2757` → IC=+0.177 (n=868)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` < 1.2757 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.2352` → IC=+0.191 (n=189)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.2352 (IC base=+0.164)
...
- **PATRÓN** `libro_liquidez` > `9838.614` → IC=+0.182 (n=868)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 9838.614 (IC base=+0.164)

- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.182 (n=868)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `volumen_regimen` < `1.2145` → IC=+0.165 (n=987)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4386` → IC=+0.161 (n=877)
...
- **PATRÓN** `sigma_h` > `0.0058` → IC=+0.197 (n=986)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.220 (n=334)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.618` → IC=+0.266 (n=302)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.618 (IC base=+0.189)

- **PATRÓN** `volumen_pendiente_norm` < `0.1072` → IC=+0.186 (n=806)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` < 0.1072 (IC base=+0.189)

- **PATRÓN** `volumen_pendiente_norm` > `0.2187` → IC=+0.185 (n=284)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.2187 (IC base=+0.189)

- **PATRÓN** `volumen_spike_ratio` < `1.6803` → IC=+0.185 (n=306)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 1.6803 (IC base=+0.189)
...
- **PATRÓN** `ibs_20min` < `0.3443` → IC=+0.250 (n=834)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3443 (IC base=+0.222)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.671` → IC=+0.270 (n=345)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.164 (n=950)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1357` → IC=+0.195 (n=618)
  ...
  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1357 (IC base=+0.153)
...
- **PATRÓN** `volumen_regimen` < `0.8629` → IC=+0.161 (n=629)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.8629 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` > `1.198` → IC=+0.174 (n=314)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 1.198 (IC base=+0.153)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4032` → IC=+0.170 (n=307)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` < 1.4032 (IC base=+0.153)
...
- **PATRÓN** `libro_liquidez` > `7289.8412` → IC=+0.192 (n=628)
  ...
  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 7289.8412 (IC base=+0.153)

- **PATRÓN** `ballena_activa_n` < `153.0` → IC=+0.159 (n=773)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 153.0 (IC base=+0.153)

- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.161 (n=1017)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` < 0.0072 (IC base=+0.128)

- **PATRÓN** `drift_60min` |x|≤ `0.3789` → IC=+0.145 (n=1018)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.3789 (IC base=+0.128)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.177 (n=398)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 17.0 (IC base=+0.128)

- **PATRÓN** `ibs_20min` < `0.6046` → IC=+0.180 (n=1017)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` < 0.6046 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` < `0.3396` → IC=+0.140 (n=1116)
...
- **PATRÓN** `volumen_regimen` < `0.8624` → IC=+0.143 (n=679)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 0.8624 (IC base=+0.128)

- **PATRÓN** `volumen_regimen` > `0.6121` → IC=+0.129 (n=1018)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` > 0.6121 (IC base=+0.128)

- **PATRÓN** `volumen_pendiente_norm` > `0.2904` → IC=+0.200 (n=148)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2904 (IC base=+0.128)

- **PATRÓN** `volumen_spike_ratio` < `1.7931` → IC=+0.134 (n=601)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 1.7931 (IC base=+0.128)

- **PATRÓN** `volumen_spike_ratio` > `2.4834` → IC=+0.144 (n=301)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 2.4834 (IC base=+0.128)

- **PATRÓN** `libro_liquidez` > `10007.3401` → IC=+0.152 (n=461)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 10007.3401 (IC base=+0.128)

### GBM_LATE_15M_MULTIHORIZONTE#SOL#15min
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.147 (n=746)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0077 (IC base=+0.111)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.130 (n=1145)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 5.0 (IC base=+0.111)

- **PATRÓN** `ibs_20min` > `0.5098` → IC=+0.195 (n=1114)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.176 (n=510)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 15.0 (IC base=+0.115)

- **PATRÓN** `ibs_20min` < `0.5435` → IC=+0.209 (n=1110)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5435 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` > `0.7056` → IC=+0.144 (n=200)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.7056 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` < `0.1924` → IC=+0.138 (n=1029)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.1924 (IC base=+0.115)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.33` → IC=+0.160 (n=236)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 7.33 (IC base=+0.115)

- **PATRÓN** `volumen_regimen` < `1.0417` → IC=+0.131 (n=977)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_regimen` < 1.0417 (IC base=+0.115)

- **PATRÓN** `volumen_pendiente_norm` > `0.2741` → IC=+0.191 (n=134)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.2741 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` < `1.465` → IC=+0.134 (n=323)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 1.465 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` > `2.1742` → IC=+0.141 (n=438)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` > 2.1742 (IC base=+0.115)

- **PATRÓN** `libro_liquidez` > `3082.5359` → IC=+0.161 (n=370)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 3082.5359 (IC base=+0.115)
...
- **PATRÓN** `sigma_h` < `0.0077` → IC=+0.235 (n=376)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.0909` → IC=+0.220 (n=376)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0909 (IC base=+0.204)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.217 (n=557)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.0244` → IC=+0.306 (n=497)
...
- **PATRÓN** `dist_vwap_pct` < `0.2681` → IC=+0.204 (n=1171)
...
- **PATRÓN** `volumen_regimen` > `0.6291` → IC=+0.218 (n=1127)
...
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.154 (n=478)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.183 (n=714)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.8295` → IC=+0.165 (n=201)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.8295 (IC base=+0.137)
...
- **PATRÓN** `volumen_regimen` < `0.866` → IC=+0.155 (n=821)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.866 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` > `0.1644` → IC=+0.165 (n=395)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.1644 (IC base=+0.137)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5764` → IC=+0.157 (n=456)
  ...
  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` > 2.5764 (IC base=+0.137)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.143 (n=1580)

  - _Acción_: Kelly boost +0.71€ cuando `libro_spread` < 0.02 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `8135.1658` → IC=+0.159 (n=649)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 8135.1658 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.160 (n=1211)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3387` → IC=+0.128 (n=1325)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.64€ cuando `drift_60min` |x|≤ 0.3387 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.125 (n=1524)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 5.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` < `0.4833` → IC=+0.156 (n=1325)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` < 0.4833 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` < `0.2153` → IC=+0.123 (n=1325)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.2153 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.996` → IC=+0.135 (n=444)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` > 5.996 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` < `1.2261` → IC=+0.122 (n=1327)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_regimen` < 1.2261 (IC base=+0.114)

- **PATRÓN** `volumen_pendiente_norm` > `0.1668` → IC=+0.143 (n=390)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_pendiente_norm` > 0.1668 (IC base=+0.114)

- **PATRÓN** `volumen_spike_ratio` < `2.2373` → IC=+0.134 (n=1266)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 2.2373 (IC base=+0.114)

- **PATRÓN** `ballena_activa_n` < `27.0` → IC=+0.127 (n=601)

  - _Acción_: Kelly boost +0.63€ cuando `ballena_activa_n` < 27.0 (IC base=+0.114)
...
- **FILTRO** `dist_vwap_pct` > `0.3877` → IC=-0.222 (n=16)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.101 (n=351)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.181 (n=136)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 15.0 (IC base=+0.076)

- **PATRÓN** `ibs_20min` > `0.9048` → IC=+0.208 (n=200)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9048 (IC base=+0.076)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.329` → IC=+0.154 (n=134)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` > 5.329 (IC base=+0.076)

- **PATRÓN** `libro_liquidez` > `2970.4361` → IC=+0.186 (n=100)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 2970.4361 (IC base=+0.076)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.127 (n=132)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 14.0 (IC base=+0.085)

- **PATRÓN** `ibs_20min` < `0.4524` → IC=+0.169 (n=276)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` < 0.4524 (IC base=+0.085)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.921` → IC=+0.147 (n=49)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 8.921 (IC base=+0.085)

- **PATRÓN** `volumen_regimen` < `0.7183` → IC=+0.145 (n=122)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.7183 (IC base=+0.085)

- **PATRÓN** `volumen_spike_ratio` < `2.4929` → IC=+0.131 (n=253)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` < 2.4929 (IC base=+0.085)

- **PATRÓN** `libro_liquidez` > `2879.8937` → IC=+0.146 (n=125)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 2879.8937 (IC base=+0.085)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0085` → IC=+0.193 (n=3423)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0085 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.175 (n=7909)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 5.0 (IC base=+0.165)

- **PATRÓN** `ibs_20min` > `0.4733` → IC=+0.213 (n=7549)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4733 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` > `0.9028` → IC=+0.202 (n=997)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9028 (IC base=+0.165)
...
- **PATRÓN** `volumen_regimen` < `0.8839` → IC=+0.165 (n=3394)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.8839 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` > `0.24` → IC=+0.188 (n=1438)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.24 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `2.6421` → IC=+0.184 (n=2396)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` > 2.6421 (IC base=+0.165)

- **PATRÓN** `libro_liquidez` > `3784.5011` → IC=+0.171 (n=2517)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 3784.5011 (IC base=+0.165)

- **PATRÓN** `ballena_activa_n` < `95.0` → IC=+0.195 (n=5450)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 95.0 (IC base=+0.165)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.196 (n=4643)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0066 (IC base=+0.182)

- **PATRÓN** `drift_60min` |x|≤ `0.4773` → IC=+0.185 (n=6957)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.4773 (IC base=+0.182)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.203 (n=2651)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.182)

- **PATRÓN** `ibs_20min` < `0.556` → IC=+0.239 (n=6957)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.556 (IC base=+0.182)

- **PATRÓN** `dist_vwap_pct` < `0.2351` → IC=+0.165 (n=4383)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.2351 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.857` → IC=+0.200 (n=998)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.857 (IC base=+0.182)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.699` → IC=+0.182 (n=6728)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.699 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` < `0.7031` → IC=+0.161 (n=2116)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.7031 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` > `1.2037` → IC=+0.161 (n=1602)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 1.2037 (IC base=+0.182)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3011` → IC=+0.189 (n=2822)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` > 2.3011 (IC base=+0.182)

- **PATRÓN** `ballena_activa_n` < `127.0` → IC=+0.178 (n=5838)
...
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.232 (n=580)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.007 (IC base=+0.194)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.201 (n=610)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2271` → IC=+0.241 (n=234)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2271 (IC base=+0.194)

- **PATRÓN** `volumen_spike_ratio` < `1.4574` → IC=+0.187 (n=394)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 1.4574 (IC base=+0.194)

- **PATRÓN** `volumen_spike_ratio` > `1.8707` → IC=+0.195 (n=787)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 1.8707 (IC base=+0.194)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.224 (n=1145)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.267 (n=893)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.266 (n=908)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.255)

- **PATRÓN** `ibs_20min` < `0.3409` → IC=+0.284 (n=879)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3409 (IC base=+0.255)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.466` → IC=+0.263 (n=1048)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.466 (IC base=+0.255)

- **PATRÓN** `volumen_pendiente_norm` > `0.2279` → IC=+0.291 (n=180)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2279 (IC base=+0.255)

- **PATRÓN** `volumen_spike_ratio` > `1.9027` → IC=+0.279 (n=596)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9027 (IC base=+0.255)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.255 (n=1027)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.255)

- **PATRÓN** `libro_liquidez` > `1584.11` → IC=+0.267 (n=892)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1584.11 (IC base=+0.255)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.805` → IC=+0.160 (n=280)
  ...
  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 9.805 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.374` → IC=+0.153 (n=1060)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 4.374 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` < `0.6939` → IC=+0.184 (n=527)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4231` → IC=+0.157 (n=1143)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.4231 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` > `1.7596` → IC=+0.160 (n=762)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 1.7596 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `10721.5149` → IC=+0.169 (n=1068)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 10721.5149 (IC base=+0.148)

- **PATRÓN** `ballena_activa_n` < `495.0` → IC=+0.163 (n=1076)

  - _Acción_: Kelly boost +0.82€ cuando `ballena_activa_n` < 495.0 (IC base=+0.148)

- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.170 (n=944)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0049 (IC base=+0.157)

- **PATRÓN** `drift_60min` |x|≤ `0.3258` → IC=+0.168 (n=1072)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.3258 (IC base=+0.157)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.180 (n=360)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 18.0 (IC base=+0.157)

- **PATRÓN** `ibs_20min` < `0.6372` → IC=+0.207 (n=1072)
...
- **PATRÓN** `dist_vwap_pct` < `0.1327` → IC=+0.168 (n=982)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` < 0.1327 (IC base=+0.157)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.288` → IC=+0.175 (n=528)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 3.288 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` < `1.1954` → IC=+0.167 (n=1072)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4236` → IC=+0.172 (n=975)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 2.4236 (IC base=+0.157)

- **PATRÓN** `libro_liquidez` > `12129.7221` → IC=+0.157 (n=715)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 12129.7221 (IC base=+0.157)
...
- **PATRÓN** `sigma_h` > `0.0058` → IC=+0.224 (n=1191)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0058 (IC base=+0.211)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.216 (n=1254)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.6749` → IC=+0.249 (n=1064)
...
- **PATRÓN** `ibs_20min` < `0.3655` → IC=+0.266 (n=1016)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3655 (IC base=+0.230)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.708` → IC=+0.275 (n=411)
...
- **PATRÓN** `volumen_spike_ratio` < `1.791` → IC=+0.226 (n=454)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.241 (n=601)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4327` → IC=+0.137 (n=1281)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.4327 (IC base=+0.135)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.146 (n=1346)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 5.0 (IC base=+0.135)

- **PATRÓN** `ibs_20min` > `0.7089` → IC=+0.234 (n=854)
...
- **PATRÓN** `volumen_regimen` < `0.8805` → IC=+0.159 (n=854)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.8805 (IC base=+0.135)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4364` → IC=+0.155 (n=1032)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.4364 (IC base=+0.138)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.171 (n=393)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.6884` → IC=+0.191 (n=1032)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` < 0.6884 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.2048` → IC=+0.144 (n=962)
...
- **PATRÓN** `volumen_regimen` < `0.8599` → IC=+0.142 (n=688)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.125 (n=909)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1235` → IC=+0.150 (n=418)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.1235 (IC base=+0.115)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.154 (n=587)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 15.0 (IC base=+0.115)

- **PATRÓN** `ibs_20min` < `0.625` → IC=+0.210 (n=1255)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.625 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` < `0.4735` → IC=+0.131 (n=1232)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.4735 (IC base=+0.115)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.43` → IC=+0.125 (n=1207)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4623` → IC=+0.141 (n=366)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.4623 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` > `2.5613` → IC=+0.128 (n=366)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 2.5613 (IC base=+0.115)

- **PATRÓN** `libro_liquidez` > `2921.4363` → IC=+0.167 (n=418)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 2921.4363 (IC base=+0.115)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` > `0.0181` → IC=+0.213 (n=876)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0181 (IC base=+0.204)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.211 (n=1373)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1858` → IC=+0.235 (n=752)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1858 (IC base=+0.204)
...
- **PATRÓN** `volumen_regimen` < `1.2457` → IC=+0.206 (n=1313)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2457 (IC base=+0.204)

- **PATRÓN** `volumen_regimen` > `0.627` → IC=+0.209 (n=1313)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5891` → IC=+0.237 (n=420)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5891 (IC base=+0.204)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.212 (n=1318)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.204)

- **PATRÓN** `libro_liquidez` > `2567.2944` → IC=+0.210 (n=875)
...
- **PATRÓN** `sigma_h` > `0.0253` → IC=+0.223 (n=485)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0253 (IC base=+0.201)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.212 (n=702)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_spike_ratio` < `2.2257` → IC=+0.196 (n=1117)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.2257 (IC base=+0.201)

- **PATRÓN** `volumen_spike_ratio` > `1.4427` → IC=+0.196 (n=1269)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 1.4427 (IC base=+0.201)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.208 (n=982)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.201)

- **PATRÓN** `libro_liquidez` > `2542.2704` → IC=+0.201 (n=966)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=6161)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.179 (n=2594)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=7874)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.171 (n=2678)

  - _Acción_: SKIP cuando `py_entrada` > 0.6
  - _Potencial_: sin este filtro IC_bueno=+0.033 (n=8238)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.43` → IC=-0.216 (n=424)

  - _Acción_: SKIP cuando `py_entrada` < 0.43
  - _Potencial_: sin este filtro IC_bueno=+0.092 (n=1336)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.160 (n=445)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=1497)
...
- **FILTRO** `ibs_20min` > `0.2857` → IC=-0.167 (n=479)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2857
  - _Potencial_: sin este filtro IC_bueno=+0.047 (n=1438)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.47` → IC=-0.203 (n=425)
...
- **FILTRO** `py_entrada` > `0.58` → IC=-0.195 (n=480)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.049 (n=1442)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=7497)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.081 (n=17156)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.282 (n=5709)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=18944)

- **FILTRO** `ibs_7min` < `0.7059` → IC=-0.241 (n=6143)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7059
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=18510)

- **FILTRO** `ballena_activa_n` > `16.0` → IC=-0.160 (n=8190)

  - _Acción_: SKIP cuando `ballena_activa_n` > 16.0
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=16463)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.224 (n=7672)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=23195)

- **FILTRO** `ibs_7min` > `0.2967` → IC=-0.178 (n=7714)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2967
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=23153)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.312 (n=927)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=3014)

- **FILTRO** `ibs_7min` < `0.7099` → IC=-0.258 (n=1300)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7099
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=2641)

- **FILTRO** `ballena_activa_n` > `9.0` → IC=-0.194 (n=952)

  - _Acción_: SKIP cuando `ballena_activa_n` > 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=2989)

- **FILTRO** `py_entrada` > `0.505` → IC=-0.147 (n=3607)

  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.098 (n=1768)

- **FILTRO** `ibs_7min` > `0.7987` → IC=-0.206 (n=1343)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7987
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=4032)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.139 (n=988)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.081 (n=3328)
...
- **FILTRO** `ibs_7min` < `0.7595` → IC=-0.187 (n=1079)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7595
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=3237)

- **FILTRO** `ballena_activa_n` > `160.0` → IC=-0.171 (n=1077)

  - _Acción_: SKIP cuando `ballena_activa_n` > 160.0
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=3239)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.253 (n=1023)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=3344)

- **FILTRO** `ibs_7min` > `0.2506` → IC=-0.166 (n=1091)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2506
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=3276)

- **FILTRO** `ballena_activa_n` > `114.0` → IC=-0.164 (n=1475)

  - _Acción_: SKIP cuando `ballena_activa_n` > 114.0
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=2892)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.181 (n=898)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.095 (n=2805)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.321 (n=885)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=2818)

- **FILTRO** `drift_7min_pct` |x|> `0.1814` → IC=-0.131 (n=1259)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1814
  - _Potencial_: sin este filtro IC_bueno=-0.108 (n=2444)

- **FILTRO** `ibs_7min` < `0.2031` → IC=-0.274 (n=925)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2031
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=2778)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.218 (n=893)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=2810)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.235 (n=1325)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=4318)

- **FILTRO** `ibs_7min` > `0.2642` → IC=-0.157 (n=1917)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2642
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=3726)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.127 (n=1277)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.090 (n=2774)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.252 (n=978)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3073)

- **FILTRO** `ibs_7min` < `0.7454` → IC=-0.192 (n=1012)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7454
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=3039)

- **FILTRO** `ballena_activa_n` > `33.0` → IC=-0.185 (n=992)

  - _Acción_: SKIP cuando `ballena_activa_n` > 33.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3059)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.261 (n=1013)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=3140)

- **FILTRO** `ibs_7min` > `0.2748` → IC=-0.174 (n=1038)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2748
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3115)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.179 (n=1032)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=3121)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.241 (n=1108)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=3365)

- **FILTRO** `ibs_7min` < `0.7241` → IC=-0.212 (n=1117)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7241
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=3356)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.176 (n=1406)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.124 (n=1246)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=2923)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.279 (n=1003)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=3166)

- **FILTRO** `ibs_7min` < `0.7321` → IC=-0.230 (n=1042)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7321
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=3127)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.206 (n=1028)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=3141)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.205 (n=1307)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=4186)
...
- **PATRÓN** `libro_liquidez` > `3709.6192` → IC=+0.123 (n=282)

  - _Acción_: Kelly boost +0.62€ cuando `libro_liquidez` > 3709.6192 (IC base=+0.121)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.190 (n=27)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=509)
...
- **FILTRO** `streak_len` > `3.0` → IC=-0.155 (n=27)
  ...
  - _Acción_: SKIP cuando `streak_len` > 3.0
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=453)

### STREAK_MOM_5M#SOL#5min
- **FILTRO** `py_entrada` < `0.5` → IC=-0.128 (n=41)

  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.012 (n=918)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.021 (n=2303)

### STRUCT_NO_15M#SOL#15min
- **FILTRO** `py_entrada` < `0.495` → IC=-0.147 (n=32)

  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=1184)
...
| ✅ BALLENAS_TARDIAS | 20253 | -0.104 | -2885.33€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1226 | -0.043 | -184.45€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 