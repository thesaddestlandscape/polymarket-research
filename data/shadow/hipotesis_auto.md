# Hipótesis automáticas — 2026-09-03 16:38 UTC
_Generado por shadow_postmortem.py sobre 269510 resoluciones (PNL=+25126.67€)_
...
- **FILTRO** `py_entrada` > `0.5` → IC=-0.392 (n=81)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.126 (n=273)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.126 (n=273)

  - _Acción_: Kelly boost +0.63€ cuando `py_entrada` < 0.5 (IC base=+0.006)
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.54` → IC=-0.297 (n=3647)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.54
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=10948)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `142.45` → IC=-0.281 (n=472)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 142.45
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=1418)

### BALLENAS_TARDIAS#BTC#15min
- **FILTRO** `restante_s_al_confirmar` > `649.17` → IC=-0.154 (n=235)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` > 649.17
  - _Potencial_: sin este filtro IC_bueno=+0.038 (n=713)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.199 (n=821)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 7.0 (IC base=+0.189)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.198 (n=906)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.190 (n=1158)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.01 (IC base=+0.189)

- **PATRÓN** `libro_liquidez` > `12428.1933` → IC=+0.201 (n=406)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.210 (n=357)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.206)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.206 (n=783)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.03` → IC=+0.213 (n=799)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.03 (IC base=+0.206)

- **PATRÓN** `libro_liquidez` > `918.1633` → IC=+0.216 (n=768)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.197 (n=6068)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.176 (n=1468)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.175 (n=1576)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.197 (n=1496)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.7` → IC=+0.189 (n=782)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.208 (n=17062)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.71 (IC base=+0.189)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.152 (n=3501)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.177 (n=2491)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.181 (n=2939)

  - _Acción_: Kelly boost +0.91€ cuando `py_entrada` > 0.71 (IC base=+0.163)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.237 (n=1483)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `restante_min` > `4.93` → IC=+0.151 (n=2453)
...
- **PATRÓN** `lag_apertura_s` < `4.17` → IC=+0.154 (n=2281)
...
- **PATRÓN** `restante_min` > `4.88` → IC=+0.150 (n=1563)
...
- **PATRÓN** `lag_apertura_s` < `7.04` → IC=+0.150 (n=1496)
...
- **PATRÓN** `restante_min` > `4.94` → IC=+0.158 (n=1288)

  - _Acción_: Kelly boost +0.79€ cuando `restante_min` > 4.94 (IC base=+0.127)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.131 (n=3443)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3569` → IC=+0.128 (n=3581)
...
- **PATRÓN** `ibs_20min` < `0.4071` → IC=+0.127 (n=3462)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` < 0.4071 (IC base=+0.040)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.165` → IC=+0.279 (n=564)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.165 (IC base=+0.263)

- **PATRÓN** `volumen_pendiente_norm` < `0.0688` → IC=+0.271 (n=378)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0688 (IC base=+0.263)

- **PATRÓN** `volumen_pendiente_norm` > `0.287` → IC=+0.336 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.287 (IC base=+0.263)

- **PATRÓN** `volumen_spike_ratio` > `1.5811` → IC=+0.288 (n=375)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5811 (IC base=+0.263)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.270 (n=520)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.263)

- **PATRÓN** `libro_liquidez` > `1703.72` → IC=+0.283 (n=353)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3699` → IC=+0.242 (n=534)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3699 (IC base=+0.237)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.238 (n=292)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.237)
...
- **PATRÓN** `volumen_spike_ratio` > `2.4466` → IC=+0.220 (n=323)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4466 (IC base=+0.237)

- **PATRÓN** `ballena_activa_n` < `52.0` → IC=+0.222 (n=411)
...
- **PATRÓN** `dist_vwap_pct` > `0.3391` → IC=+0.333 (n=28)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3391 (IC base=-0.038)
...
- **PATRÓN** `volumen_spike_ratio` > `1.9887` → IC=+0.287 (n=45)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.9887 (IC base=-0.038)
...
- **PATRÓN** `ibs_20min` > `0.2858` → IC=+0.138 (n=4434)

  - _Acción_: Kelly boost +0.69€ cuando `ibs_20min` > 0.2858 (IC base=+0.070)

- **PATRÓN** `dist_vwap_pct` > `0.8562` → IC=+0.287 (n=421)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8562 (IC base=+0.070)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.472` → IC=+0.121 (n=2345)

  - _Acción_: Kelly boost +0.60€ cuando `sigma_ewma_delta_pct` > 2.472 (IC base=+0.070)

- **PATRÓN** `volumen_regimen` > `0.6831` → IC=+0.218 (n=1280)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6831 (IC base=+0.070)

- **PATRÓN** `volumen_pendiente_norm` < `0.0869` → IC=+0.214 (n=1978)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0869 (IC base=+0.070)

- **PATRÓN** `volumen_pendiente_norm` > `0.253` → IC=+0.233 (n=418)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4966` → IC=+0.238 (n=711)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4966 (IC base=+0.070)

- **PATRÓN** `volumen_spike_ratio` > `2.8416` → IC=+0.228 (n=711)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8416 (IC base=+0.070)

- **PATRÓN** `ballena_activa_n` < `96.0` → IC=+0.302 (n=1587)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 96.0 (IC base=+0.070)

- **PATRÓN** `ibs_20min` < `0.5842` → IC=+0.131 (n=4305)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.5842 (IC base=+0.051)

- **PATRÓN** `dist_vwap_pct` > `0.7528` → IC=+0.267 (n=200)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7528 (IC base=+0.051)
...
- **PATRÓN** `volumen_regimen` > `1.2365` → IC=+0.245 (n=367)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2365 (IC base=+0.051)
...
- **PATRÓN** `ibs_20min` > `0.8162` → IC=+0.229 (n=278)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8162 (IC base=+0.020)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.116` → IC=+0.128 (n=417)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 2.116 (IC base=+0.020)
...
- **PATRÓN** `volumen_spike_ratio` < `1.9518` → IC=+0.233 (n=118)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9518 (IC base=+0.020)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3631` → IC=+0.353 (n=66)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3631 (IC base=+0.309)
...
- **PATRÓN** `volumen_spike_ratio` > `2.4058` → IC=+0.322 (n=268)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4058 (IC base=+0.309)

- **PATRÓN** `libro_liquidez` > `1845.461` → IC=+0.320 (n=165)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1845.461 (IC base=+0.309)
...
- **FILTRO** `ibs_20min` < `0.7621` → IC=-0.144 (n=470)
...
- **PATRÓN** `dist_vwap_pct` > `0.8443` → IC=+0.250 (n=30)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8443 (IC base=-0.065)

- **PATRÓN** `volumen_regimen` < `0.9683` → IC=+0.131 (n=82)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 0.9683 (IC base=-0.065)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0972` → IC=+0.246 (n=57)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0972 (IC base=-0.065)

- **PATRÓN** `ballena_activa_n` < `212.0` → IC=+0.250 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 212.0 (IC base=-0.065)
...
- **FILTRO** `ibs_20min` > `0.7568` → IC=-0.226 (n=275)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=829)
...
- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.210 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 61.0 (IC base=-0.031)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` > `0.0129` → IC=+0.332 (n=485)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.1554` → IC=+0.318 (n=422)
...
- **PATRÓN** `libro_liquidez` > `2472.0173` → IC=+0.255 (n=648)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2472.0173 (IC base=+0.251)
...
- **PATRÓN** `sigma_h` > `0.0105` → IC=+0.208 (n=1251)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0105 (IC base=+0.163)

- **PATRÓN** `drift_60min` |x|≤ `0.3432` → IC=+0.169 (n=3294)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.290 (n=1708)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.163)

- **PATRÓN** `dist_vwap_pct` > `0.7674` → IC=+0.243 (n=806)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7674 (IC base=+0.163)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.678` → IC=+0.237 (n=1526)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.678 (IC base=+0.163)

- **PATRÓN** `volumen_regimen` > `0.6325` → IC=+0.167 (n=2532)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 0.6325 (IC base=+0.163)

- **PATRÓN** `volumen_pendiente_norm` > `0.1054` → IC=+0.187 (n=1399)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.1054 (IC base=+0.163)

- **PATRÓN** `volumen_spike_ratio` < `2.3263` → IC=+0.162 (n=3058)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.3263 (IC base=+0.163)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.165 (n=3840)
...
- **PATRÓN** `ballena_activa_n` < `143.0` → IC=+0.182 (n=2650)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.199 (n=1675)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.4385` → IC=+0.232 (n=3490)
...
- **PATRÓN** `volumen_regimen` < `1.1802` → IC=+0.169 (n=2646)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 1.1802 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` > `0.6248` → IC=+0.162 (n=2646)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.310 (n=298)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.172)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.526` → IC=+0.319 (n=136)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.526 (IC base=+0.172)

- **PATRÓN** `volumen_pendiente_norm` > `0.1302` → IC=+0.173 (n=212)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.1302 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` < `1.9213` → IC=+0.168 (n=230)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.9213 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` > `4.0169` → IC=+0.165 (n=174)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 4.0169 (IC base=+0.172)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.197 (n=586)

  - _Acción_: Kelly boost +0.99€ cuando `libro_spread` < 0.04 (IC base=+0.172)

- **PATRÓN** `ballena_activa_n` < `18.0` → IC=+0.197 (n=120)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 18.0 (IC base=+0.172)
...
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.154 (n=290)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0094 (IC base=+0.134)

- **PATRÓN** `drift_60min` |x|≤ `0.5288` → IC=+0.140 (n=639)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.70€ cuando `drift_60min` |x|≤ 0.5288 (IC base=+0.134)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.193 (n=291)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 15.0 (IC base=+0.134)

- **PATRÓN** `ibs_20min` < `0.4667` → IC=+0.232 (n=639)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4667 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` > `0.9932` → IC=+0.144 (n=71)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.9932 (IC base=+0.134)
...
- **PATRÓN** `volumen_regimen` < `1.2004` → IC=+0.150 (n=639)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.2004 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` > `0.8568` → IC=+0.149 (n=426)
...
- **PATRÓN** `libro_liquidez` > `2170.2079` → IC=+0.164 (n=426)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 2170.2079 (IC base=+0.134)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.202 (n=754)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1656` → IC=+0.228 (n=513)
...
- **PATRÓN** `volumen_regimen` > `0.6862` → IC=+0.215 (n=644)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.205 (n=2033)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0087 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.172 (n=4482)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `1.0304` → IC=+0.227 (n=636)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0304 (IC base=+0.165)
...
- **PATRÓN** `volumen_regimen` < `0.89` → IC=+0.158 (n=2070)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.89 (IC base=+0.165)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8759` → IC=+0.174 (n=2768)
...
- **PATRÓN** `ibs_20min` < `0.5624` → IC=+0.242 (n=4150)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.581` → IC=+0.190 (n=4090)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3087` → IC=+0.204 (n=1561)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3087 (IC base=+0.189)

- **PATRÓN** `ballena_activa_n` < `142.0` → IC=+0.181 (n=3024)
...
- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.281 (n=541)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0046 (IC base=+0.275)

- **PATRÓN** `drift_60min` |x|≤ `0.1152` → IC=+0.308 (n=238)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1152 (IC base=+0.275)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.285 (n=492)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.275)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.280 (n=553)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.275)

- **PATRÓN** `ibs_20min` < `0.4113` → IC=+0.312 (n=476)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4113 (IC base=+0.275)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.327` → IC=+0.286 (n=582)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.327 (IC base=+0.275)

- **PATRÓN** `volumen_pendiente_norm` > `0.2944` → IC=+0.326 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2944 (IC base=+0.275)

- **PATRÓN** `volumen_spike_ratio` > `1.5014` → IC=+0.295 (n=438)
...
- **PATRÓN** `libro_liquidez` > `1456.1771` → IC=+0.280 (n=484)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1456.1771 (IC base=+0.275)

- **PATRÓN** `ballena_activa_n` < `69.0` → IC=+0.279 (n=337)
...
- **PATRÓN** `ibs_20min` < `0.4074` → IC=+0.297 (n=569)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.140 (n=356)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.6` → IC=+0.200 (n=750)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` < `0.5076` → IC=+0.140 (n=725)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.5076 (IC base=+0.115)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.195` → IC=+0.138 (n=741)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` < 3.195 (IC base=+0.115)

- **PATRÓN** `volumen_regimen` < `0.8807` → IC=+0.135 (n=499)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 0.8807 (IC base=+0.115)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4564` → IC=+0.144 (n=200)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 1.4564 (IC base=+0.115)

- **PATRÓN** `volumen_spike_ratio` > `2.1929` → IC=+0.148 (n=271)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 2.1929 (IC base=+0.115)
...
- **PATRÓN** `sigma_h` > `0.0282` → IC=+0.258 (n=287)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0282 (IC base=+0.201)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.212 (n=899)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1627` → IC=+0.257 (n=512)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1627 (IC base=+0.201)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.141` → IC=+0.251 (n=427)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4407` → IC=+0.205 (n=807)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.177 (n=1426)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=4286)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.187 (n=1456)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.008 (n=4477)
...
- **FILTRO** `py_entrada` > `0.67` → IC=-0.167 (n=244)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.019 (n=792)
...
- **FILTRO** `py_entrada` > `0.625` → IC=-0.382 (n=15)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=2991)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=10998)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.282 (n=3446)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=10543)

- **FILTRO** `ibs_7min` < `0.7112` → IC=-0.248 (n=3497)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7112
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=10492)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.169 (n=4683)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=9306)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.219 (n=4335)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=13259)

- **FILTRO** `ibs_7min` > `0.7143` → IC=-0.173 (n=4393)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=13201)
...
- **FILTRO** `py_entrada` > `0.52` → IC=-0.141 (n=2037)

  - _Acción_: SKIP cuando `py_entrada` > 0.52
  - _Potencial_: sin este filtro IC_bueno=+0.076 (n=1023)

- **FILTRO** `drift_7min_pct` |x|> `0.1143` → IC=-0.130 (n=1038)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1143
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=2022)

- **FILTRO** `ibs_7min` > `0.8349` → IC=-0.191 (n=764)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8349
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=2296)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=572)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=2044)
...
- **FILTRO** `ibs_7min` < `0.7711` → IC=-0.195 (n=654)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=1962)

- **FILTRO** `ballena_activa_n` > `168.0` → IC=-0.177 (n=649)
...
- **FILTRO** `ibs_7min` < `0.2105` → IC=-0.293 (n=515)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=1552)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.249 (n=489)

  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=1578)
...
- **FILTRO** `ibs_7min` > `0.8` → IC=-0.171 (n=786)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=2367)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.180 (n=795)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.123 (n=739)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=1550)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.297 (n=560)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=1729)

- **FILTRO** `ibs_7min` < `0.7333` → IC=-0.248 (n=569)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7333
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=1720)
...
- **FILTRO** `sigma_ewma_delta_pct` > `6.428` → IC=-0.205 (n=59)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=755)
...
- **FILTRO** `sigma_h` > `0.0107` → IC=-0.211 (n=330)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.141 (n=338)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1036` → IC=+0.248 (n=169)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1036 (IC base=-0.065)

- **PATRÓN** `ibs_15` < `0.3714` → IC=+0.274 (n=432)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3714 (IC base=-0.065)

- **PATRÓN** `dist_vwap_pct` > `0.7529` → IC=+0.223 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7529 (IC base=-0.065)
...
- **PATRÓN** `sigma_h` < `0.0079` → IC=+0.250 (n=218)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0079 (IC base=+0.216)

- **PATRÓN** `sigma_h` > `0.0045` → IC=+0.218 (n=218)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0045 (IC base=+0.216)

- **PATRÓN** `drift_60min` |x|≤ `0.4455` → IC=+0.227 (n=218)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4455 (IC base=+0.216)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0918` → IC=+0.236 (n=195)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0918 (IC base=+0.216)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.287 (n=73)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.216)

- **PATRÓN** `ibs_15` < `0.3879` → IC=+0.282 (n=218)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3879 (IC base=+0.216)

- **PATRÓN** `dist_vwap_pct` > `0.7797` → IC=+0.269 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7797 (IC base=+0.216)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.326` → IC=+0.231 (n=232)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 11.326 (IC base=+0.216)

- **PATRÓN** `libro_liquidez` > `4425.7357` → IC=+0.216 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4425.7357 (IC base=+0.216)

- **PATRÓN** `ballena_activa_n` < `162.0` → IC=+0.214 (n=166)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 162.0 (IC base=+0.216)
...
- **FILTRO** `sigma_h` > `0.0226` → IC=-0.266 (n=122)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3714 sube el IC de -0.065 a +0.274 en UPDOWN_GBM_15M_TARDIO (n=432). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.3879 sube el IC de +0.216 a +0.282 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=218). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 789 | +0.088 | +44.47€ | 2 | 10 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 789 | +0.088 | +44.47€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 20 | +0.091 | +1.87€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 20 | +0.091 | +1.87€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 514 | +0.107 | +36.03€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 514 | +0.107 | +36.03€ | 2 | 10 |
...
| ✅ BALLENAS_TARDIAS | 14595 | -0.111 | -2439.77€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 948 | -0.009 | -142.09€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 13647 | -0.118 | -2297.68€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1890 | -0.087 | -403.76€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1890 | -0.087 | -403.76€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 948 | -0.009 | -142.09€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 948 | -0.009 | -142.09€ | 2 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 1929 | -0.164 | -522.36€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 1929 | -0.164 | -522.36€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 3917 | -0.050 | -360.21€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 3917 | -0.050 | -360.21€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 3240 | -0.127 | -279.67€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 3240 | -0.127 | -279.67€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 