# Hipótesis automáticas — 2026-07-19 23:53 UTC
_Generado por shadow_postmortem.py sobre 23423 resoluciones (PNL=+5998.06€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.202 (n=1327)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.204 (n=1645)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.222 (n=210)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.615` → IC=+0.274 (n=294)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.233 (n=402)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.230)

- **PATRÓN** `libro_liquidez` > `4303.632` → IC=+0.236 (n=210)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4303.632 (IC base=+0.230)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.130 (n=1888)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.124` → IC=+0.130 (n=671)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.124 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.066` → IC=+0.239 (n=186)
...
- **PATRÓN** `dist_vwap_pct` < `0.1117` → IC=+0.155 (n=140)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.1117 (IC base=+0.104)
...
- **PATRÓN** `sigma_h` < `0.0268` → IC=+0.157 (n=578)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0268 (IC base=+0.157)

- **PATRÓN** `sigma_h` > `0.0148` → IC=+0.174 (n=385)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0148 (IC base=+0.157)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.162 (n=519)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5541` → IC=+0.222 (n=52)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5541 (IC base=+0.157)

- **PATRÓN** `dist_vwap_pct` < `0.1113` → IC=+0.162 (n=226)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.1113 (IC base=+0.157)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.792` → IC=+0.283 (n=58)
...
- **PATRÓN** `sigma_h` > `0.0114` → IC=+0.228 (n=384)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0114 (IC base=+0.166)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.189 (n=770)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 13.0 (IC base=+0.166)

- **PATRÓN** `dist_vwap_pct` > `0.5248` → IC=+0.226 (n=210)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5248 (IC base=+0.166)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.775` → IC=+0.268 (n=330)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.775 (IC base=+0.166)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.140 (n=270)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0061 (IC base=+0.134)

- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.158 (n=241)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0029 (IC base=+0.134)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.158 (n=270)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 7.0 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` > `0.5248` → IC=+0.207 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5248 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.368` → IC=+0.217 (n=44)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.178 (n=206)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 12.0 (IC base=+0.147)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.150 (n=301)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` < 18.0 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.4274` → IC=+0.189 (n=72)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.4274 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` < `0.9779` → IC=+0.144 (n=254)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.9779 (IC base=+0.147)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.293` → IC=+0.365 (n=72)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` < `0.0051` → IC=+0.138 (n=354)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0051 (IC base=+0.133)

- **PATRÓN** `sigma_h` > `0.0131` → IC=+0.191 (n=354)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0131 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.138 (n=985)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 8.0 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` > `0.4821` → IC=+0.181 (n=214)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.4821 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `0.1129` → IC=+0.162 (n=566)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.1129 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.232` → IC=+0.296 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.232 (IC base=+0.133)
...
- **PATRÓN** `sigma_h` < `0.0043` → IC=+0.169 (n=158)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0043 (IC base=+0.127)
...
- **PATRÓN** `dist_vwap_pct` > `0.5163` → IC=+0.214 (n=40)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5163 (IC base=+0.127)

- **PATRÓN** `dist_vwap_pct` < `0.1434` → IC=+0.153 (n=145)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.1434 (IC base=+0.127)
...
- **PATRÓN** `dist_vwap_pct` < `0.2441` → IC=+0.130 (n=144)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.2441 (IC base=+0.080)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.605` → IC=+0.256 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.605 (IC base=+0.080)
...
- **PATRÓN** `sigma_h` < `0.0255` → IC=+0.209 (n=335)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0255 (IC base=+0.210)

- **PATRÓN** `sigma_h` > `0.0136` → IC=+0.229 (n=223)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0136 (IC base=+0.210)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.211 (n=119)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.177` → IC=+0.300 (n=78)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.26` → IC=-0.167 (n=28)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.111 (n=322)

- **PATRÓN** `ibs_15` > `0.7428` → IC=+0.202 (n=320)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `18.799` → IC=+0.143 (n=54)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 18.799 (IC base=+0.038)

- **PATRÓN** `ibs_15` < `0.029` → IC=+0.128 (n=186)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.64€ cuando `ibs_15` < 0.029 (IC base=+0.056)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.492` → IC=+0.128 (n=288)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` < 10.492 (IC base=+0.056)
...
- **PATRÓN** `sigma_h` < `0.0055` → IC=+0.250 (n=18)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0055 (IC base=+0.000)
...
- **PATRÓN** `sigma_h` < `0.0126` → IC=+0.140 (n=73)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0126 (IC base=+0.125)

- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.147 (n=83)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0082 (IC base=+0.125)

- **PATRÓN** `drift_60min` |x|≤ `0.1824` → IC=+0.176 (n=35)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.1824 (IC base=+0.125)

- **PATRÓN** `drift_15min` |x|≤ `0.9862` → IC=+0.130 (n=79)

  - _Acción_: Kelly boost +0.65€ cuando `drift_15min` |x|≤ 0.9862 (IC base=+0.125)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1408` → IC=+0.167 (n=52)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.83€ cuando `delta_ratio_macro` |x|> 0.1408 (IC base=+0.125)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.141 (n=76)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` > 5.0 (IC base=+0.125)

- **PATRÓN** `ibs_15` < `0.1467` → IC=+0.158 (n=74)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.79€ cuando `ibs_15` < 0.1467 (IC base=+0.125)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS > 0.7428 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.202 n=320). Confirma señal de reversión media → alinear con BUY_NO.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: IBS < 0.1467 correlaciona con éxito en UPDOWN_GBM#XRP#15min (IC=+0.158 n=74). Confirma señal de reversión media → alinear con BUY_YES.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min` — IC=+0.227 n=31. Faltan ~9 resoluciones para umbral n≥40. ETA: ~6h.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC` — IC=+0.227 n=31. Faltan ~9 resoluciones para umbral n≥40. ETA: ~6h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 367 | +0.137 | +11.55€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 367 | +0.137 | +11.55€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 149 | +0.162 | +7.28€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 149 | +0.162 | +7.28€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 3999 | +0.187 | +74.28€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 2604 | +0.219 | +104.27€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 1163 | +0.198 | +49.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 869 | +0.212 | +5.46€ | 0 | 5 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 1422 | +0.182 | +23.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 864 | +0.224 | +53.68€ | 0 | 8 |
...
| ✅ GBM_LATE_15M | 5975 | +0.109 | +2104.67€ | 0 | 10 |
| ✅ GBM_LATE_15M#15min | 5975 | +0.109 | +2104.67€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1420 | +0.083 | +309.24€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1420 | +0.083 | +309.24€ | 0 | 8 |
...
| ✅ GBM_LATE_15M#XRP | 1571 | +0.165 | +879.07€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1571 | +0.165 | +879.07€ | 0 | 14 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 3199 | +0.155 | +1980.47€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 3199 | +0.155 | +1980.47€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 815 | +0.122 | +424.59€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 815 | +0.122 | +424.59€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 838 | +0.119 | +421.39€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 838 | +0.119 | +421.39€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 815 | +0.113 | +361.55€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 815 | +0.113 | +361.55€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 731 | +0.278 | +772.95€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 731 | +0.278 | +772.95€ | 0 | 12 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 177 | +0.159 | +98.20€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 177 | +0.159 | +98.20€ | 0 | 0 |
| 🚫 GBM_LATE_15M_PYCONFIRMADO#BTC | 9 | -0.102 | -1.94€ | 0 | 0 |
| 🚫 GBM_LATE_15M_PYCONFIRMADO#BTC#15min | 9 | -0.102 | -1.94€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH | 36 | +0.079 | +4.11€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH#15min | 36 | +0.079 | +4.11€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO | 3153 | +0.104 | +1173.16€ | 0 | 11 |
| ✅ GBM_LATE_15M_TARDIO#15min | 3153 | +0.104 | +1173.16€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 751 | +0.079 | +188.78€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 751 | +0.079 | +188.78€ | 0 | 8 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 756 | +0.043 | +94.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 756 | +0.043 | +94.84€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 717 | +0.053 | +159.53€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 717 | +0.053 | +159.53€ | 1 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 929 | +0.213 | +730.02€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 929 | +0.213 | +730.02€ | 0 | 14 |
...
| ✅ LEADLAG_BTC_XRP_15M | 206 | +0.038 | +14.82€ | 1 | 2 |
| ✅ LEADLAG_BTC_XRP_15M#15min | 206 | +0.038 | +14.82€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP | 206 | +0.038 | +14.82€ | 0 | 0 |
| ✅ LEADLAG_BTC_XRP_15M#XRP#15min | 206 | +0.038 | +14.82€ | 1 | 2 |
...
| ✅ UPDOWN_GBM | 1882 | +0.022 | +142.72€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1622 | +0.047 | +180.78€ | 1 | 6 |
| 🚫 UPDOWN_GBM#240min | 11 | -0.148 | -4.31€ | 0 | 0 |
| 🚫 UPDOWN_GBM#5min | 75 | -0.162 | -22.48€ | 4 | 0 |
| ✅ UPDOWN_GBM#60min | 127 | -0.058 | -10.76€ | 7 | 0 |
| ✅ UPDOWN_GBM#BNB | 141 | +0.059 | +33.66€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 141 | +0.059 | +33.66€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 475 | +0.028 | +29.22€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 392 | +0.064 | +43.20€ | 1 | 6 |
...
| ✅ UPDOWN_GBM#XRP | 150 | +0.053 | +36.02€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 139 | +0.074 | +39.31€ | 3 | 7 |
| 🚫 UPDOWN_GBM#XRP#5min | 11 | -0.106 | -3.29€ | 0 | 0 |
| 🚫 UPDOWN_GBM#daily | 45 | -0.202 | +1.33€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD | 77 | +0.285 | +14.87€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#15min | 77 | +0.285 | +14.87€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC | 31 | +0.227 | +1.65€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min | 31 | +0.227 | +1.65€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH | 46 | +0.312 | +13.22€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min | 46 | +0.312 | +13.22€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO | 700 | +0.179 | +324.39€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 700 | +0.179 | +324.39€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 61 | +0.167 | +22.28€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 61 | +0.167 | +22.28€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 83 | +0.088 | +7.68€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 83 | +0.088 | +7.68€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 47 | +0.255 | +29.05€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 47 | +0.255 | +29.05€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 165 | +0.189 | +65.36€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 165 | +0.189 | +65.36€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.192: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.056 n=793 | neutral: IC=-0.005 n=582 | overbought(IBS>0.7): IC=+0.187 n=755
  - _Datos_: n=2274 IC=+0.091 PNL=+507.54€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.120 n=14159 | tras_1loss IC=+0.093 n=9071 | tras_2loss IC=+0.059 n=3664/40 | gap=+0.061 (umbral 0.05)
...
**⏳ H-WINDOW-MOMENTUM** — Momentum de outcome entre ventanas 15min contiguas
  ...
  - _Estado_: alineada_con_outcome_prev IC=+0.068 n=130/60 | contraria IC=+0.081 n=41 | gap=-0.013 (umbral 0.08) — verificar independencia de drift_15min/60min antes de actuar
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1750 IC=+0.029 PNL=+157.47€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1750 IC=+0.029 PNL=+157.47€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=814 IC=+0.038 PNL=+75.63€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=814 IC=+0.038 PNL=+75.63€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=392 IC=+0.064 PNL=+43.19€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=392 IC=+0.064 PNL=+43.19€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=864 IC=+0.079 PNL=+124.76€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=864 IC=+0.079 PNL=+124.76€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=766 IC=+0.040 PNL=+73.78€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=766 IC=+0.040 PNL=+73.78€

**🟡 H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  - _Hipótesis_: Feature nueva 2026-06-27: drift del precio YES en Polymarket en últimas 5 obs (~5min). Si poly_drift<0 y decidimos BUY_NO (o poly_drift>0 y BUY_YES) → confluencia. Si diverge → reducción de stake. Hipótesis: confluencia Binance+Polymarket mejora IC; divergencia empeora.
  - _Umbral_: n≥40 en confluencia vs divergencia para validar el boost ×1.1
  - _Acción_: Si IC_confluencia>IC_divergencia con n≥40 → mantener el boost. Si no → retirar.
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.155 > 0.08 con n=82 PNL=+28.62€
  - _Datos_: n=82 IC=+0.155 PNL=+28.62€
...
**🟡 H-CUSTOM-CROSS-WINDOW-SPREAD-POS** — GBM 15min/60min: spread positivo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.214 > 0.08 con n=110 PNL=+9.04€
  - _Datos_: n=110 IC=+0.214 PNL=+9.04€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=20965 IC=+0.124 PNL=+6073.94€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=20965 IC=+0.124 PNL=+6073.94€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=205 IC=+0.007 PNL=+5.98€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=205 IC=+0.007 PNL=+5.98€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=964 IC=+0.008 PNL=+45.08€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=964 IC=+0.008 PNL=+45.08€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.162 > 0.1 con n=563 PNL=+201.57€
  - _Datos_: n=563 IC=+0.162 PNL=+201.57€
...
**🟡 H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.125 > 0.1 con n=110 PNL=+42.67€
  - _Datos_: n=110 IC=+0.125 PNL=+42.67€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.136 > 0.08 con n=410 PNL=+121.88€
  - _Datos_: n=410 IC=+0.136 PNL=+121.88€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.240 > 0.2 con n=514 PNL=+339.92€
  - _Datos_: n=514 IC=+0.240 PNL=+339.92€
...
**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.168 > 0.08 con n=308 PNL=+168.23€
  - _Datos_: n=308 IC=+0.168 PNL=+168.23€
