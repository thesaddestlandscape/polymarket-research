# Hipótesis automáticas — 2026-07-23 22:45 UTC
_Generado por shadow_postmortem.py sobre 31769 resoluciones (PNL=+6952.37€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.204 (n=1828)
...
- **PATRÓN** `py_entrada` > `0.735` → IC=+0.337 (n=680)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.735 (IC base=+0.196)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.202 (n=2266)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.234 (n=156)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.605` → IC=+0.272 (n=445)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.221 (n=407)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.715` → IC=+0.344 (n=209)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.715 (IC base=+0.214)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.214 (n=561)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.214)

- **PATRÓN** `libro_liquidez` > `4248.6395` → IC=+0.216 (n=294)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4248.6395 (IC base=+0.214)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.241 (n=450)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.236)

- **PATRÓN** `py_entrada` > `0.67` → IC=+0.335 (n=295)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.130 (n=2121)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.604` → IC=+0.222 (n=455)
...
- **FILTRO** `sigma_ewma_delta_pct` > `9.123` → IC=-0.309 (n=19)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.075 (n=440)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.153 (n=679)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.1132` → IC=+0.135 (n=316)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.1132 (IC base=+0.146)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.791` → IC=+0.289 (n=74)
...
- **PATRÓN** `sigma_h` > `0.0117` → IC=+0.215 (n=483)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0117 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.172 (n=1325)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` > `0.0043` → IC=+0.147 (n=1612)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0043 (IC base=+0.131)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.145 (n=1124)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.7524` → IC=+0.133 (n=1629)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.7524 (IC base=+0.131)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.153` → IC=+0.154 (n=322)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.896` → IC=+0.142 (n=107)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 6.896 (IC base=+0.090)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.155 (n=247)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 12.0 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` > `0.7076` → IC=+0.190 (n=56)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.7076 (IC base=+0.128)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0135` → IC=+0.159 (n=461)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0135 (IC base=+0.112)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.126 (n=1244)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 8.0 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `0.4709` → IC=+0.167 (n=274)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.4709 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.76` → IC=+0.280 (n=212)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.76 (IC base=+0.112)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `13.968` → IC=+0.223 (n=45)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.968 (IC base=+0.065)
...
- **PATRÓN** `sigma_h` < `0.0215` → IC=+0.189 (n=394)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.0215 (IC base=+0.184)

- **PATRÓN** `sigma_h` > `0.0287` → IC=+0.195 (n=149)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0287 (IC base=+0.184)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.192 (n=400)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.6216` → IC=+0.183 (n=512)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `12.482` → IC=+0.179 (n=107)
  ...
  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 12.482 (IC base=+0.056)
...
- **PATRÓN** `sigma_h` < `0.0078` → IC=+0.121 (n=283)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` < 0.0078 (IC base=+0.101)

- **PATRÓN** `drift_60min` |x|≤ `0.2235` → IC=+0.136 (n=273)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.2235 (IC base=+0.101)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0647` → IC=+0.126 (n=273)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.63€ cuando `delta_ratio_macro` |x|> 0.0647 (IC base=+0.101)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.175 (n=78)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` < 8.0 (IC base=+0.101)

- **PATRÓN** `ibs_15` > `0.6374` → IC=+0.216 (n=234)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.986` → IC=+0.158 (n=77)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 5.986 (IC base=+0.101)

- **PATRÓN** `sigma_ewma_delta_pct` < `14.299` → IC=+0.167 (n=112)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` < 14.299 (IC base=+0.101)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6216 sube el IC de +0.056 a +0.183 en UPDOWN_GBM#15min (n=512). Ya aplicado como kelly_boost=+0.91€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.6374 sube el IC de +0.101 a +0.216 en UPDOWN_GBM#BTC#15min (n=234). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 893 | +0.128 | +41.51€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 893 | +0.128 | +41.51€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 10 | +0.083 | +2.04€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 10 | +0.083 | +2.04€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 339 | +0.136 | +11.72€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 339 | +0.136 | +11.72€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 341 | +0.141 | +12.73€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 341 | +0.141 | +12.73€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 203 | +0.085 | +15.01€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 203 | +0.085 | +15.01€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 5566 | +0.187 | +96.08€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3652 | +0.215 | +95.76€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 1628 | +0.196 | +45.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1215 | +0.210 | -6.40€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 1967 | +0.176 | +3.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1204 | +0.212 | +37.54€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 1941 | +0.192 | +44.31€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1207 | +0.224 | +66.15€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 302 | +0.319 | +18.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 302 | +0.319 | +18.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 302 | +0.319 | +18.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 302 | +0.319 | +18.12€ | 0 | 0 |
| ✅ GBM_LATE_15M | 7248 | +0.096 | +2254.80€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 7248 | +0.096 | +2254.80€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1755 | +0.065 | +285.27€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1755 | +0.065 | +285.27€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1623 | +0.075 | +287.25€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1623 | +0.075 | +287.25€ | 0 | 6 |
| ✅ GBM_LATE_15M#SOL | 1953 | +0.089 | +696.98€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1953 | +0.089 | +696.98€ | 1 | 3 |
| ✅ GBM_LATE_15M#XRP | 1917 | +0.149 | +985.31€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1917 | +0.149 | +985.31€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 4442 | +0.125 | +2210.07€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 4442 | +0.125 | +2210.07€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 1148 | +0.082 | +394.45€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 1148 | +0.082 | +394.45€ | 0 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1159 | +0.092 | +423.31€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1159 | +0.092 | +423.31€ | 0 | 6 |
...
| ✅ GBM_LATE_15M_TARDIO | 4469 | +0.077 | +1292.11€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#15min | 4469 | +0.077 | +1292.11€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 1077 | +0.034 | +134.74€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 1077 | +0.034 | +134.74€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 1076 | +0.028 | +80.06€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 1076 | +0.028 | +80.06€ | 0 | 1 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 1032 | +0.043 | +213.75€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 1032 | +0.043 | +213.75€ | 1 | 3 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 1284 | +0.180 | +863.56€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1284 | +0.180 | +863.56€ | 0 | 10 |
| ✅ GBM_LATE_5M | 504 | -0.026 | -2.57€ | 0 | 0 |
| ✅ GBM_LATE_5M#5min | 504 | -0.026 | -2.57€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC | 125 | -0.035 | -13.53€ | 0 | 0 |
| ✅ GBM_LATE_5M#BTC#5min | 125 | -0.035 | -13.53€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 2390 | +0.032 | +206.41€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 2127 | +0.052 | +244.93€ | 0 | 3 |
...
| ✅ UPDOWN_GBM#BTC | 538 | +0.050 | +59.02€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 454 | +0.086 | +73.50€ | 1 | 9 |
...
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD | 135 | +0.296 | +26.25€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#15min | 135 | +0.296 | +26.25€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC | 64 | +0.288 | +10.33€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min | 64 | +0.288 | +10.33€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH | 71 | +0.294 | +15.92€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min | 71 | +0.294 | +15.92€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO | 1433 | +0.163 | +662.07€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1433 | +0.163 | +662.07€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 141 | +0.206 | +84.41€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 141 | +0.206 | +84.41€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 163 | +0.124 | +30.63€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 163 | +0.124 | +30.63€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 347 | +0.102 | +95.67€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 347 | +0.102 | +95.67€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 361 | +0.203 | +254.09€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 361 | +0.203 | +254.09€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.187: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.062 n=1215 | neutral: IC=+0.013 n=940 | overbought(IBS>0.7): IC=+0.200 n=1204
  - _Datos_: n=3581 IC=+0.103 PNL=+920.17€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=05h: IC=+0.128 n=84 PNL=+35.91€ → BOOST | H=10h: IC=-0.154 n=15 PNL=-6.77€ → FILTRAR | H=14h: IC=+0.167 n=94 PNL=+41.63€ → BOOST | H=16h: IC=+0.128 n=92 PNL=+18.23€ → BOOST | H=21h: IC=+0.113 n=91 PNL=+14.40€ → BOOST | H=22h: IC=+0.107 n=110 PNL=+17.43€ → BOOST
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.114 n=18998 | tras_1loss IC=+0.082 n=12575 | tras_2loss IC=+0.050 n=5225/40 | gap=+0.064 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2258 IC=+0.037 PNL=+221.16€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2258 IC=+0.037 PNL=+221.16€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=939 IC=+0.056 PNL=+129.89€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=939 IC=+0.056 PNL=+129.89€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=454 IC=+0.086 PNL=+73.50€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=454 IC=+0.086 PNL=+73.50€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.092 > 0.08 con n=1133 PNL=+195.49€
  - _Datos_: n=1133 IC=+0.092 PNL=+195.49€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=1089 IC=+0.027 PNL=+99.34€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1089 IC=+0.027 PNL=+99.34€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.176 > 0.1 con n=746 PNL=+284.65€
  - _Datos_: n=746 IC=+0.176 PNL=+284.65€
...
**〰️ H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: n=221 IC=+0.052 PNL=-3.27€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=221 IC=+0.052 PNL=-3.27€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=960 IC=+0.050 PNL=+109.95€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=960 IC=+0.050 PNL=+109.95€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.144 > 0.08 con n=535 PNL=+176.14€
  - _Datos_: n=535 IC=+0.144 PNL=+176.14€
...
**🟡 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.187 > 0.08 con n=244 PNL=+94.65€
  - _Datos_: n=244 IC=+0.187 PNL=+94.65€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.280 > 0.08 con n=417 PNL=+7.64€
  - _Datos_: n=417 IC=+0.280 PNL=+7.64€
