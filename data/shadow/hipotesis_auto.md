# Hipótesis automáticas — 2026-07-02 00:45 UTC
_Generado por shadow_postmortem.py sobre 2497 resoluciones (PNL=-66.77€)_
...
- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.268 (n=54)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0092 (IC base=+0.118)

- **PATRÓN** `drift_60min` |x|≤ `0.7816` → IC=+0.146 (n=94)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.7816 (IC base=+0.118)

- **PATRÓN** `drift_15min` |x|≤ `0.7351` → IC=+0.153 (n=70)

  - _Acción_: Kelly boost +0.76€ cuando `drift_15min` |x|≤ 0.7351 (IC base=+0.118)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2386` → IC=+0.122 (n=35)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.61€ cuando `delta_ratio_macro` |x|> 0.2386 (IC base=+0.118)
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.186 (n=33)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` < 11.0 (IC base=+0.118)

- **PATRÓN** `ibs_15` < `0.0634` → IC=+0.265 (n=32)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0634 (IC base=+0.118)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS < 0.0634 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.265 n=32). Confirma señal de reversión media → alinear con BUY_YES.
...
| ✅ UPDOWN_GBM | 746 | -0.027 | -16.24€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 528 | +0.002 | +9.71€ | 1 | 7 |
...
| ✅ UPDOWN_GBM#SOL | 128 | -0.069 | -4.88€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 76 | -0.038 | -0.63€ | 4 | 0 |
| 🚫 UPDOWN_GBM#SOL#5min | 23 | -0.060 | -3.42€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 23 | -0.140 | -2.19€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 59 | +0.025 | +8.69€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 48 | +0.080 | +11.98€ | 1 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.081) — sin ventaja clara. oversold(IBS<0.3): IC=+0.036 n=110 | neutral: IC=-0.045 n=108 | overbought(IBS>0.7): IC=-0.027 n=129
  - _Datos_: n=356 IC=-0.011 PNL=+8.99€
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=76 IC=-0.038 PNL=-0.63€

**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  - _Umbral_: n≥40 tras 2 losses y gap(IC_tras_win - IC_tras_2loss)≥0.05
  - _Acción_: Reducir stake (no desactivar) 1-2h tras 2 derrotas consecutivas en el mismo subtype
  - _Estado_: tras_win IC=+0.004 n=1179 | tras_1loss IC=-0.033 n=1236 | tras_2loss IC=-0.047 n=643/40 | gap=+0.051 (umbral 0.05)
...
**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  ...
  - _Estado_: n_overlaps=21, boost estimado=+0.005. Necesita 0 más y boost>0.05
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=623 IC=-0.018 PNL=-2.69€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=623 IC=-0.018 PNL=-2.69€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=145 IC=+0.092 PNL=+37.57€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=145 IC=+0.092 PNL=+37.57€
...
**⏳ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: 80/200 ops en el filtro definido (IC actual=+0.134 PNL=+26.15€)
  - _Datos_: n=80 IC=+0.134 PNL=+26.15€
...
**⏳ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: 80/100 ops en el filtro definido (IC actual=+0.134 PNL=+26.15€)
  - _Datos_: n=80 IC=+0.134 PNL=+26.15€
