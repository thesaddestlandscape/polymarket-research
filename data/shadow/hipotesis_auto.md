# Hipótesis automáticas — 2026-07-12 21:15 UTC
_Generado por shadow_postmortem.py sobre 11437 resoluciones (PNL=+2195.65€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.230 (n=380)
...
- **PATRÓN** `py_entrada` > `0.585` → IC=+0.258 (n=398)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.585 (IC base=+0.225)

- **PATRÓN** `libro_liquidez` > `2902.992` → IC=+0.244 (n=338)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2902.992 (IC base=+0.225)
...
- **PATRÓN** `py_entrada` < `0.275` → IC=+0.308 (n=139)
...
- **PATRÓN** `libro_liquidez` > `5946.4255` → IC=+0.163 (n=182)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 5946.4255 (IC base=+0.155)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.237 (n=78)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.605` → IC=+0.253 (n=87)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.605 (IC base=+0.207)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.238 (n=82)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `6442.1918` → IC=+0.238 (n=82)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 6442.1918 (IC base=+0.221)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.275 (n=87)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.248)

- **PATRÓN** `py_entrada` < `0.665` → IC=+0.259 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.665 (IC base=+0.248)

- **PATRÓN** `py_entrada` > `0.715` → IC=+0.305 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.715 (IC base=+0.248)

- **PATRÓN** `libro_liquidez` > `4211.4102` → IC=+0.307 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4211.4102 (IC base=+0.248)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.204 (n=42)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `3727.5827` → IC=+0.167 (n=76)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 3727.5827 (IC base=+0.167)
...
- **PATRÓN** `hora_utc` > `13.0` → IC=+0.268 (n=54)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.625` → IC=+0.295 (n=76)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.625 (IC base=+0.236)

- **PATRÓN** `libro_liquidez` > `1892.1539` → IC=+0.293 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1892.1539 (IC base=+0.236)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.256 (n=39)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.245` → IC=+0.281 (n=30)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.245 (IC base=+0.215)

- **PATRÓN** `libro_liquidez` > `1672.5274` → IC=+0.247 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1672.5274 (IC base=+0.215)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.122 (n=1383)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 6.0 (IC base=+0.116)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.121 (n=1362)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` < 17.0 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` > `0.5688` → IC=+0.147 (n=49)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.5688 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` < `0.1341` → IC=+0.204 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1341 (IC base=+0.116)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.067` → IC=+0.380 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.067 (IC base=+0.116)

- **PATRÓN** `sigma_h` > `0.009` → IC=+0.149 (n=949)
...
- **PATRÓN** `dist_vwap_pct` > `0.5914` → IC=+0.146 (n=46)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.5914 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.1918` → IC=+0.190 (n=98)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.1918 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.06` → IC=+0.162 (n=63)

  - _Acción_: Kelly boost +0.81€ cuando `sigma_ewma_delta_pct` < 4.06 (IC base=+0.110)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `dist_vwap_pct` < `0.1391` → IC=+0.177 (n=29)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.1391 (IC base=+0.089)
...
- **PATRÓN** `sigma_h` < `0.0044` → IC=+0.172 (n=114)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0044 (IC base=+0.095)

- **PATRÓN** `dist_vwap_pct` > `0.6749` → IC=+0.237 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6749 (IC base=+0.095)

- **PATRÓN** `dist_vwap_pct` > `0.6921` → IC=+0.237 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6921 (IC base=+0.055)

- **PATRÓN** `dist_vwap_pct` < `0.2858` → IC=+0.132 (n=17)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.2858 (IC base=+0.055)

### GBM_LATE_15M#SOL#15min
- **PATRÓN** `sigma_h` < `0.016` → IC=+0.137 (n=367)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.016 (IC base=+0.121)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.156 (n=123)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.172 (n=120)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0092 (IC base=+0.151)

- **PATRÓN** `sigma_h` > `0.0148` → IC=+0.169 (n=119)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0148 (IC base=+0.151)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.219 (n=261)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 12.0 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` < `0.187` → IC=+0.224 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.187 (IC base=+0.151)

### GBM_LATE_15M#XRP#15min
- **PATRÓN** `sigma_h` < `0.0313` → IC=+0.172 (n=315)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0313 (IC base=+0.160)

- **PATRÓN** `sigma_h` > `0.0137` → IC=+0.175 (n=281)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0137 (IC base=+0.160)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.162 (n=323)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` < `0.017` → IC=+0.178 (n=237)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.017 (IC base=+0.176)

- **PATRÓN** `sigma_h` > `0.0114` → IC=+0.189 (n=355)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0114 (IC base=+0.176)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.258 (n=130)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.176)

- **PATRÓN** `dist_vwap_pct` < `0.1627` → IC=+0.190 (n=27)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` < 0.1627 (IC base=+0.176)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.188 (n=344)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0048 (IC base=+0.175)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.182 (n=265)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 13.0 (IC base=+0.175)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.184 (n=406)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 18.0 (IC base=+0.175)

- **PATRÓN** `dist_vwap_pct` > `0.557` → IC=+0.273 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.557 (IC base=+0.175)

- **PATRÓN** `dist_vwap_pct` < `0.1391` → IC=+0.212 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1391 (IC base=+0.175)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.404` → IC=+0.346 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.404 (IC base=+0.175)

- **PATRÓN** `sigma_h` > `0.0042` → IC=+0.176 (n=399)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0042 (IC base=+0.152)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.187 (n=148)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` < 6.0 (IC base=+0.152)

- **PATRÓN** `dist_vwap_pct` < `0.1648` → IC=+0.255 (n=100)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1648 (IC base=+0.152)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.075` → IC=+0.242 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.075 (IC base=+0.152)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **PATRÓN** `sigma_h` > `0.0023` → IC=+0.206 (n=107)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.167 (n=109)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 18.0 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` < `0.1193` → IC=+0.179 (n=26)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.1193 (IC base=+0.167)

- **PATRÓN** `sigma_h` > `0.0028` → IC=+0.256 (n=88)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.171 (n=68)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` < 12.0 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` > `0.0484` → IC=+0.167 (n=22)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.0484 (IC base=+0.162)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **PATRÓN** `sigma_h` > `0.0049` → IC=+0.184 (n=74)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0049 (IC base=+0.158)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.187 (n=81)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 12.0 (IC base=+0.158)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.175 (n=118)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` < 18.0 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` > `0.5183` → IC=+0.292 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5183 (IC base=+0.158)

- **PATRÓN** `sigma_h` > `0.0034` → IC=+0.144 (n=99)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` > 0.0034 (IC base=+0.104)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.132 (n=36)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 18.0 (IC base=+0.104)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.176 (n=35)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` < 6.0 (IC base=+0.104)

- **PATRÓN** `dist_vwap_pct` > `0.6003` → IC=+0.241 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6003 (IC base=+0.104)

- **PATRÓN** `dist_vwap_pct` < `0.3828` → IC=+0.150 (n=18)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` < 0.3828 (IC base=+0.104)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **PATRÓN** `sigma_h` > `0.0109` → IC=+0.208 (n=63)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0109 (IC base=+0.138)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.184 (n=36)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 8.0 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` > `0.312` → IC=+0.227 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.312 (IC base=+0.138)

- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.254 (n=55)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0083 (IC base=+0.101)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.148 (n=52)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 6.0 (IC base=+0.101)

- **PATRÓN** `dist_vwap_pct` < `0.1589` → IC=+0.300 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1589 (IC base=+0.101)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` < `0.018` → IC=+0.303 (n=74)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.018 (IC base=+0.250)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.350 (n=38)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 11.0 (IC base=+0.250)

- **PATRÓN** `dist_vwap_pct` < `0.2767` → IC=+0.364 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2767 (IC base=+0.250)

- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.319 (n=70)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0087 (IC base=+0.271)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.300 (n=53)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 12.0 (IC base=+0.271)

- **PATRÓN** `dist_vwap_pct` < `0.0977` → IC=+0.292 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0977 (IC base=+0.271)

### GBM_LATE_15M_TARDIO
- **FILTRO** `sigma_ewma_delta_pct` < `7.347` → IC=-0.150 (n=18)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` < 7.347
  - _Potencial_: sin este filtro IC_bueno=+0.353 (n=32)

- **PATRÓN** `sigma_h` > `0.0135` → IC=+0.133 (n=145)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0135 (IC base=+0.086)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.125 (n=214)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` < 8.0 (IC base=+0.086)

- **PATRÓN** `dist_vwap_pct` > `0.4603` → IC=+0.150 (n=38)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.4603 (IC base=+0.086)

- **PATRÓN** `dist_vwap_pct` < `0.1427` → IC=+0.136 (n=75)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.1427 (IC base=+0.086)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.347` → IC=+0.353 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.347 (IC base=+0.086)

- **PATRÓN** `sigma_h` > `0.012` → IC=+0.139 (n=153)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.012 (IC base=+0.078)

- **PATRÓN** `hora_utc` < `9.0` → IC=+0.120 (n=214)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.60€ cuando `hora_utc` < 9.0 (IC base=+0.078)

- **PATRÓN** `dist_vwap_pct` < `0.1854` → IC=+0.163 (n=93)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.1854 (IC base=+0.078)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.121 (n=93)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 7.0 (IC base=+0.088)

- **PATRÓN** `dist_vwap_pct` < `0.1111` → IC=+0.136 (n=20)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.1111 (IC base=+0.088)

- **PATRÓN** `sigma_h` > `0.0033` → IC=+0.125 (n=94)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` > 0.0033 (IC base=+0.085)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.122 (n=35)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 17.0 (IC base=+0.085)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.141 (n=51)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0039 (IC base=+0.029)

- **PATRÓN** `dist_vwap_pct` > `0.7076` → IC=+0.206 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7076 (IC base=+0.029)
...
- **PATRÓN** `sigma_h` > `0.0127` → IC=+0.123 (n=51)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` > 0.0127 (IC base=+0.040)

- **PATRÓN** `sigma_h` < `0.008` → IC=+0.214 (n=40)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.008 (IC base=+0.022)

- **PATRÓN** `hora_utc` < `9.0` → IC=+0.161 (n=57)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 9.0 (IC base=+0.022)

- **PATRÓN** `dist_vwap_pct` < `0.2` → IC=+0.214 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2 (IC base=+0.022)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` < `0.033` → IC=+0.209 (n=108)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.033 (IC base=+0.190)

- **PATRÓN** `sigma_h` > `0.0111` → IC=+0.191 (n=108)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0111 (IC base=+0.190)
...
- **PATRÓN** `dist_vwap_pct` < `0.1011` → IC=+0.184 (n=17)
  ...
  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` < 0.1011 (IC base=+0.190)

- **PATRÓN** `sigma_h` > `0.0302` → IC=+0.273 (n=42)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0302 (IC base=+0.201)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.250 (n=62)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 10.0 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` > `0.2925` → IC=+0.154 (n=24)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.2925 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` < `0.1648` → IC=+0.179 (n=26)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.1648 (IC base=+0.201)
...
- **PATRÓN** `hora_utc` > `14.0` → IC=+0.167 (n=19)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 14.0 (IC base=+0.100)
...
- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.122 (n=117)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` < 0.0063 (IC base=+0.031)

- **PATRÓN** `ibs_15` < `0.0512` → IC=+0.130 (n=133)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.65€ cuando `ibs_15` < 0.0512 (IC base=+0.031)

- **PATRÓN** `dist_vwap_pct` > `0.3298` → IC=+0.130 (n=25)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` > 0.3298 (IC base=+0.031)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 1039 | +0.190 | +34.47€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 668 | +0.218 | +24.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 43 | +0.056 | +2.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 154 | +0.109 | -6.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 174 | +0.182 | +13.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 305 | +0.194 | +5.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 223 | +0.216 | -1.43€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 14 | +0.087 | +2.11€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 58 | +0.200 | +8.13€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO#ETH | 369 | +0.171 | +5.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 221 | +0.208 | +6.08€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 14 | -0.044 | -0.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 76 | +0.103 | -6.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 58 | +0.167 | +6.86€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO#SOL | 363 | +0.204 | +23.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 222 | +0.228 | +20.05€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 15 | +0.066 | +1.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 68 | +0.171 | +3.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 58 | +0.167 | -1.24€ | 0 | 2 |
| ✅ GBM_LATE_15M | 3695 | +0.113 | +1232.82€ | 0 | 10 |
| ✅ GBM_LATE_15M#15min | 3695 | +0.113 | +1232.82€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 903 | +0.073 | +129.93€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 903 | +0.073 | +129.93€ | 0 | 2 |
| ✅ GBM_LATE_15M#ETH | 935 | +0.074 | +163.40€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 935 | +0.074 | +163.40€ | 0 | 4 |
| ✅ GBM_LATE_15M#SOL | 965 | +0.136 | +473.95€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 965 | +0.136 | +473.95€ | 0 | 7 |
| ✅ GBM_LATE_15M#XRP | 892 | +0.169 | +465.54€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 892 | +0.169 | +465.54€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1044 | +0.164 | +603.73€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1044 | +0.164 | +603.73€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 273 | +0.166 | +177.90€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 273 | +0.166 | +177.90€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 279 | +0.134 | +128.44€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 279 | +0.134 | +128.44€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 291 | +0.118 | +104.96€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 291 | +0.118 | +104.96€ | 0 | 6 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 201 | +0.264 | +192.43€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 201 | +0.264 | +192.43€ | 0 | 6 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 1189 | +0.082 | +297.33€ | 1 | 8 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1189 | +0.082 | +297.33€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 274 | +0.087 | +46.58€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 274 | +0.087 | +46.58€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 298 | +0.010 | +14.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 298 | +0.010 | +14.84€ | 1 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 309 | +0.030 | +33.39€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 309 | +0.030 | +33.39€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 308 | +0.197 | +202.51€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 308 | +0.197 | +202.51€ | 0 | 8 |
...
| ✅ STREAK_FADE_5M | 168 | -0.035 | -9.66€ | 4 | 0 |
| ✅ STREAK_FADE_5M#5min | 168 | -0.035 | -9.66€ | 0 | 0 |
| ✅ STREAK_FADE_5M#ETH | 45 | -0.138 | -7.05€ | 0 | 0 |
| ✅ STREAK_FADE_5M#ETH#5min | 45 | -0.138 | -7.05€ | 5 | 0 |
| ✅ STREAK_FADE_5M#SOL | 60 | -0.032 | -3.88€ | 0 | 0 |
| ✅ STREAK_FADE_5M#SOL#5min | 60 | -0.032 | -3.88€ | 3 | 0 |
| ✅ STREAK_FADE_5M#XRP | 63 | +0.038 | +1.27€ | 0 | 0 |
| ✅ STREAK_FADE_5M#XRP#5min | 63 | +0.038 | +1.27€ | 0 | 1 |
...
| ✅ UPDOWN_GBM | 1343 | -0.005 | +27.59€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1087 | +0.024 | +65.05€ | 1 | 6 |
| 🚫 UPDOWN_GBM#240min | 11 | -0.148 | -4.31€ | 0 | 0 |
| 🚫 UPDOWN_GBM#5min | 75 | -0.162 | -22.48€ | 4 | 0 |
| ✅ UPDOWN_GBM#60min | 127 | -0.058 | -10.76€ | 7 | 0 |
| ✅ UPDOWN_GBM#BNB | 25 | -0.130 | -6.02€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 25 | -0.130 | -6.02€ | 0 | 0 |
...
| ✅ UPDOWN_GBM#ETH | 664 | +0.017 | +39.25€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 571 | +0.036 | +50.30€ | 0 | 2 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Datos_: n=953 IC=+0.010 PNL=+52.82€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.094 n=6498 | tras_1loss IC=+0.049 n=4801 | tras_2loss IC=+0.015 n=2136/40 | gap=+0.078 (umbral 0.05)
...
**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  ...
  - _Estado_: n_overlaps=31, boost estimado=-0.028. Necesita 0 más y boost>0.05
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1216 IC=+0.002 PNL=+41.59€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1216 IC=+0.002 PNL=+41.59€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=435 IC=+0.013 PNL=+15.28€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=435 IC=+0.013 PNL=+15.28€
...
**〰️ H-CUSTOM-CROSS-WINDOW-SPREAD-NEG** — GBM 15min/60min: spread negativo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: n=57 IC=+0.025 PNL=+27.06€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=57 IC=+0.025 PNL=+27.06€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8979 IC=+0.100 PNL=+2271.53€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8979 IC=+0.100 PNL=+2271.53€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.169 > 0.1 con n=285 PNL=+100.99€
  - _Datos_: n=285 IC=+0.169 PNL=+100.99€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=465 IC=+0.059 PNL=+46.70€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=465 IC=+0.059 PNL=+46.70€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema, sin filtrar todavía
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.159 < -0.1 con n=318 PNL=-38.93€
  - _Datos_: n=318 IC=-0.159 PNL=-38.93€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.312 > 0.2 con n=147 PNL=+107.62€
  - _Datos_: n=147 IC=+0.312 PNL=+107.62€
...
**⏳ H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: 23/40 ops en el filtro definido (IC actual=+0.180 PNL=+19.24€)
  - _Datos_: n=23 IC=+0.180 PNL=+19.24€

**⏳ H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: 40
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: 19/40 ops en el filtro definido (IC actual=+0.249 PNL=+22.41€)
  - _Datos_: n=19 IC=+0.249 PNL=+22.41€
...
**⏳ H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: 27/40 ops en el filtro definido (IC actual=+0.224 PNL=+24.57€)
  - _Datos_: n=27 IC=+0.224 PNL=+24.57€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.315 > 0.08 con n=79 PNL=+9.38€
  - _Datos_: n=79 IC=+0.315 PNL=+9.38€
...
**🔶 H-CUSTOM-ETH15-BUYNO-TARDIO** — UPDOWN_GBM ETH#15min BUY_NO tardío (T_h<0.2) -- edge fuerte no capturado por el aprendizaje causal automático
  ...
  - _Estado_: SEÑAL POSITIVA en ETH (IC=+0.262 n=40) pero sin cruzar ≥2 pares más — BNB: n=1 IC=-0.008; BTC: n=12 IC=+0.043; DOGE: n=2 IC=+0.000; SOL: n=8 IC=+0.000; XRP: n=6 IC=+0.000
  - _Datos_: n=40 IC=+0.262 PNL=+25.49€
