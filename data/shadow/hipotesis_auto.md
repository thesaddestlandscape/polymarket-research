# Hipótesis automáticas — 2026-07-04 20:28 UTC
_Generado por shadow_postmortem.py sobre 3620 resoluciones (PNL=+108.92€)_
...
- **PATRÓN** `sigma_h` < `0.0091` → IC=+0.167 (n=91)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0091 (IC base=+0.027)

- **PATRÓN** `ibs_15` < `0.0613` → IC=+0.143 (n=68)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.71€ cuando `ibs_15` < 0.0613 (IC base=+0.027)
...
- **PATRÓN** `hora_utc` > `19.0` → IC=+0.136 (n=20)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 19.0 (IC base=-0.024)

- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0366` → IC=+0.125 (n=22)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.62€ cuando `pct_spot_vs_ref` |x|≤ 0.0366 (IC base=+0.033)
...
- **PATRÓN** `sigma_h` < `0.0087` → IC=+0.130 (n=52)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` < 0.0087 (IC base=+0.071)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.145 (n=29)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 7.0 (IC base=+0.071)

- **PATRÓN** `ibs_15` < `0.0949` → IC=+0.167 (n=43)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.83€ cuando `ibs_15` < 0.0949 (IC base=+0.071)
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.0949 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.167 n=43). Confirma señal de reversión media → alinear con BUY_YES.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 840 | +0.096 | +208.74€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 840 | +0.096 | +208.74€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 218 | +0.045 | +27.50€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 218 | +0.045 | +27.50€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 218 | +0.041 | +17.22€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 218 | +0.041 | +17.22€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL | 215 | +0.150 | +89.66€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 215 | +0.150 | +89.66€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP | 189 | +0.154 | +74.37€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 189 | +0.154 | +74.37€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 925 | -0.039 | -54.82€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 689 | -0.017 | -24.98€ | 1 | 2 |
...
| ✅ UPDOWN_GBM#BTC | 258 | -0.038 | -32.15€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 186 | -0.011 | -20.93€ | 2 | 2 |
...
| ✅ UPDOWN_GBM#ETH | 383 | -0.009 | +3.16€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 297 | +0.008 | +10.65€ | 3 | 3 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.045) — sin ventaja clara. oversold(IBS<0.3): IC=-0.021 n=190 | neutral: IC=-0.066 n=157 | overbought(IBS>0.7): IC=-0.041 n=168
  - _Datos_: n=535 IC=-0.038 PNL=-29.58€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=01h: IC=-0.227 n=31 PNL=-17.74€ → FILTRAR | H=02h: IC=-0.154 n=24 PNL=-5.23€ → FILTRAR | H=03h: IC=-0.113 n=19 PNL=-5.87€ → FILTRAR | H=10h: IC=-0.154 n=15 PNL=-6.77€ → FILTRAR | H=13h: IC=+0.115 n=24 PNL=+5.54€ → BOOST | H=16h: IC=+0.167 n=25 PNL=+7.36€ → BOOST | H=20h: IC=-0.145 n=29 PNL=-9.09€ → FILTRAR
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.027 n=1792 | tras_1loss IC=-0.015 n=1720 | tras_2loss IC=-0.033 n=866/40 | gap=+0.060 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=802 IC=-0.035 PNL=-41.27€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=802 IC=-0.035 PNL=-41.27€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=416 IC=-0.045 PNL=-44.59€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=416 IC=-0.045 PNL=-44.59€
...
**🔴 H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.011 < 0.02 con n=186 PNL=-20.93€
  - _Datos_: n=186 IC=-0.011 PNL=-20.93€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=305 IC=-0.008 PNL=-28.13€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=305 IC=-0.008 PNL=-28.13€
...
**〰️ H-CUSTOM-ETH15-REVERSION** — ETH#15min con drift_15min < -1 — ¿mean reversion?
  ...
  - _Estado_: n=50 IC=+0.038 PNL=+7.16€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=50 IC=+0.038 PNL=+7.16€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=252 IC=+0.000 PNL=+3.27€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=252 IC=+0.000 PNL=+3.27€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=1179 IC=+0.062 PNL=+190.06€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=1179 IC=+0.062 PNL=+190.06€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=62 IC=-0.016 PNL=-8.51€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=62 IC=-0.016 PNL=-8.51€
...
**🔴 H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.070 < -0.05 con n=558 PNL=-72.51€
  - _Datos_: n=558 IC=-0.070 PNL=-72.51€
...
**⏳ H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: 77/120 ops en el filtro definido (IC actual=+0.146 PNL=+15.06€)
  - _Datos_: n=77 IC=+0.146 PNL=+15.06€
...
**〰️ H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: n=96 IC=+0.010 PNL=-6.26€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=96 IC=+0.010 PNL=-6.26€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=84 IC=-0.058 PNL=+11.17€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=84 IC=-0.058 PNL=+11.17€
