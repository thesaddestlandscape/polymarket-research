# Hipótesis automáticas — 2026-07-18 16:01 UTC
_Generado por shadow_postmortem.py sobre 20863 resoluciones (PNL=+4968.05€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.188 (n=1144)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 8.0 (IC base=+0.188)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.203 (n=1300)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.188)

- **PATRÓN** `py_entrada` > `0.695` → IC=+0.299 (n=565)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.188)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.193 (n=1452)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.187 (n=1527)

  - _Acción_: Kelly boost +0.93€ cuando `libro_spread` < 0.01 (IC base=+0.180)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.214 (n=197)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `11728.9824` → IC=+0.210 (n=91)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11728.9824 (IC base=+0.209)
...
- **PATRÓN** `py_entrada` < `0.265` → IC=+0.373 (n=132)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.233 (n=249)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.222)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.238 (n=284)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.224 (n=346)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.222)

- **PATRÓN** `libro_liquidez` > `4496.8029` → IC=+0.227 (n=181)
...
- **PATRÓN** `py_entrada` > `0.665` → IC=+0.300 (n=193)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.665 (IC base=+0.229)

- **PATRÓN** `libro_liquidez` > `1584.5899` → IC=+0.231 (n=273)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1584.5899 (IC base=+0.229)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.126 (n=1738)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 8.0 (IC base=+0.113)

- **PATRÓN** `dist_vwap_pct` > `0.7567` → IC=+0.156 (n=126)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.7567 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.041` → IC=+0.229 (n=153)
...
- **PATRÓN** `dist_vwap_pct` > `0.1646` → IC=+0.124 (n=320)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` > 0.1646 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` < `0.9224` → IC=+0.121 (n=1028)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.9224 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.834` → IC=+0.143 (n=155)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` > 8.834 (IC base=+0.112)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `dist_vwap_pct` > `0.3946` → IC=+0.200 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3946 (IC base=+0.095)

- **PATRÓN** `dist_vwap_pct` < `0.1002` → IC=+0.122 (n=117)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.1002 (IC base=+0.095)

- **PATRÓN** `sigma_ewma_delta_pct` > `14.154` → IC=+0.258 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 14.154 (IC base=+0.095)
...
- **PATRÓN** `dist_vwap_pct` > `0.6784` → IC=+0.160 (n=51)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.6784 (IC base=+0.075)
...
- **PATRÓN** `sigma_h` < `0.0103` → IC=+0.127 (n=247)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.63€ cuando `sigma_h` < 0.0103 (IC base=+0.104)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.151 (n=187)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 18.0 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.264` → IC=+0.204 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.264 (IC base=+0.104)

- **PATRÓN** `sigma_h` < `0.013` → IC=+0.143 (n=463)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` < 0.013 (IC base=+0.125)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.127 (n=526)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.755` → IC=+0.123 (n=189)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 2.755 (IC base=+0.125)

### GBM_LATE_15M#XRP#15min
- **PATRÓN** `sigma_h` < `0.029` → IC=+0.150 (n=530)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.029 (IC base=+0.149)

- **PATRÓN** `sigma_h` > `0.0223` → IC=+0.182 (n=240)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` > 0.0223 (IC base=+0.149)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.152 (n=532)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` > 6.0 (IC base=+0.149)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.156 (n=481)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.1141` → IC=+0.148 (n=194)
...
- **PATRÓN** `sigma_h` < `0.0101` → IC=+0.195 (n=185)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0101 (IC base=+0.169)
...
- **PATRÓN** `dist_vwap_pct` < `0.6129` → IC=+0.160 (n=295)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` < 0.6129 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.536` → IC=+0.168 (n=263)

  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` < 7.536 (IC base=+0.169)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.185 (n=700)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 12.0 (IC base=+0.164)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.166 (n=1042)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 18.0 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.5919` → IC=+0.227 (n=159)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5919 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.022` → IC=+0.314 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.022 (IC base=+0.164)

- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.187 (n=987)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.8965` → IC=+0.188 (n=894)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.8965 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.996` → IC=+0.232 (n=121)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.996 (IC base=+0.169)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.135 (n=157)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0041 (IC base=+0.129)

- **PATRÓN** `sigma_h` > `0.0031` → IC=+0.148 (n=211)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0031 (IC base=+0.129)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.169 (n=167)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 12.0 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` > `0.4762` → IC=+0.224 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4762 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.007` → IC=+0.256 (n=43)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.007 (IC base=+0.129)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.142 (n=252)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0062 (IC base=+0.138)

- **PATRÓN** `sigma_h` > `0.0031` → IC=+0.170 (n=225)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0031 (IC base=+0.138)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.185 (n=122)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 15.0 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` > `0.1165` → IC=+0.150 (n=58)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.1165 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.452` → IC=+0.136 (n=223)
...
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.136 (n=226)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0067 (IC base=+0.125)

- **PATRÓN** `sigma_h` > `0.0036` → IC=+0.135 (n=258)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` > 0.0036 (IC base=+0.125)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.167 (n=97)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 6.0 (IC base=+0.125)

- **PATRÓN** `dist_vwap_pct` > `0.6297` → IC=+0.198 (n=51)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.6297 (IC base=+0.125)

- **PATRÓN** `dist_vwap_pct` < `0.245` → IC=+0.140 (n=176)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.245 (IC base=+0.125)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.518` → IC=+0.254 (n=63)
...
- **PATRÓN** `sigma_h` > `0.012` → IC=+0.203 (n=126)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.3907` → IC=+0.190 (n=69)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.3907 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.495` → IC=+0.292 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.495 (IC base=+0.136)
...
- **PATRÓN** `sigma_h` > `0.008` → IC=+0.321 (n=238)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.008 (IC base=+0.302)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.312 (n=248)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1789` → IC=+0.331 (n=57)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1789 (IC base=+0.302)

- **PATRÓN** `dist_vwap_pct` < `0.6204` → IC=+0.318 (n=234)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6204 (IC base=+0.302)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.458` → IC=+0.320 (n=204)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.458 (IC base=+0.302)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0143` → IC=+0.204 (n=302)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0143 (IC base=+0.128)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.135 (n=820)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 8.0 (IC base=+0.128)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.135 (n=819)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` < 15.0 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` > `0.5262` → IC=+0.198 (n=157)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.5262 (IC base=+0.128)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.39` → IC=+0.302 (n=119)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.39 (IC base=+0.128)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.127 (n=387)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 6.0 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` < `0.7998` → IC=+0.130 (n=887)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.7998 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.527` → IC=+0.121 (n=230)

  - _Acción_: Kelly boost +0.60€ cuando `sigma_ewma_delta_pct` > 3.527 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.847` → IC=+0.126 (n=776)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 10.847 (IC base=+0.106)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0046` → IC=+0.143 (n=138)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0046 (IC base=+0.112)

- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.134 (n=69)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` > 0.0072 (IC base=+0.112)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.158 (n=185)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 8.0 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `0.4306` → IC=+0.219 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4306 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.045` → IC=+0.257 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.045 (IC base=+0.112)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.997` → IC=+0.194 (n=47)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 7.997 (IC base=+0.042)
...
- **PATRÓN** `sigma_h` > `0.0126` → IC=+0.147 (n=83)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0126 (IC base=+0.088)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.148 (n=86)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 8.0 (IC base=+0.088)

- **PATRÓN** `dist_vwap_pct` > `0.5254` → IC=+0.180 (n=23)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.5254 (IC base=+0.088)

- **PATRÓN** `dist_vwap_pct` < `0.1434` → IC=+0.135 (n=83)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.1434 (IC base=+0.088)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.399` → IC=+0.294 (n=71)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.399 (IC base=+0.088)

- **PATRÓN** `sigma_h` < `0.009` → IC=+0.132 (n=112)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` < 0.009 (IC base=+0.047)
...
- **PATRÓN** `sigma_h` < `0.029` → IC=+0.208 (n=286)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.029 (IC base=+0.205)

- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.238 (n=258)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0113 (IC base=+0.205)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.228 (n=255)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.205)

- **PATRÓN** `dist_vwap_pct` > `0.4935` → IC=+0.259 (n=56)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4935 (IC base=+0.205)

- **PATRÓN** `dist_vwap_pct` < `0.1011` → IC=+0.207 (n=162)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1011 (IC base=+0.205)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.899` → IC=+0.281 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.899 (IC base=+0.205)
...
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH#15min` — IC=+0.353 n=32. Faltan ~8 resoluciones para umbral n≥40. ETA: ~6h.
- **LIVE-CANDIDATA**: `UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#ETH` — IC=+0.353 n=32. Faltan ~8 resoluciones para umbral n≥40. ETA: ~6h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 182 | +0.152 | +15.82€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 182 | +0.152 | +15.82€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 83 | +0.147 | +3.04€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 83 | +0.147 | +3.04€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 61 | +0.198 | +11.09€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min |