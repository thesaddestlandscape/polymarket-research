# Hipótesis automáticas — 2026-09-22 11:08 UTC
_Generado por shadow_postmortem.py sobre 556267 resoluciones (PNL=+62012.39€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.05` → IC=-0.232 (n=6701)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.05
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=20104)
...
- **FILTRO** `restante_s_al_confirmar` < `134.5` → IC=-0.280 (n=822)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 134.5
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=2467)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `162.9` → IC=-0.228 (n=1586)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 162.9
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=4761)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `123.96` → IC=-0.362 (n=1333)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 123.96
  - _Potencial_: sin este filtro IC_bueno=-0.124 (n=3999)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.201 (n=13307)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.69 (IC base=+0.101)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.153 (n=3334)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.01 (IC base=+0.101)

- **PATRÓN** `libro_liquidez` > `9537.2598` → IC=+0.189 (n=1445)
...
- **PATRÓN** `py_entrada` < `0.35` → IC=+0.234 (n=10302)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.35 (IC base=+0.128)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.175 (n=5438)

  - _Acción_: Kelly boost +0.87€ cuando `libro_spread` < 0.01 (IC base=+0.128)

- **PATRÓN** `libro_liquidez` > `7654.6247` → IC=+0.172 (n=2044)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 7654.6247 (IC base=+0.128)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.211 (n=1626)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.203)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.205 (n=1600)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.204 (n=2010)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.203 (n=1440)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.201 (n=2043)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.153 (n=2676)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 5.0 (IC base=+0.144)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.144 (n=2292)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.240 (n=1423)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.204 (n=10801)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.199)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.202 (n=10359)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.199)

- **PATRÓN** `py_entrada` > `0.74` → IC=+0.224 (n=3674)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.176 (n=2512)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` < 17.0 (IC base=+0.169)

- **PATRÓN** `py_entrada` < `0.71` → IC=+0.184 (n=1832)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.254 (n=720)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.186 (n=2591)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 5.0 (IC base=+0.182)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.187 (n=2478)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.248 (n=2310)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.238)

- **PATRÓN** `py_entrada` > `0.76` → IC=+0.319 (n=819)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.190 (n=5866)
...
- **PATRÓN** `restante_min` < `4.13` → IC=+0.129 (n=4492)

  - _Acción_: Kelly boost +0.64€ cuando `restante_min` < 4.13 (IC base=+0.118)

- **PATRÓN** `restante_min` > `4.95` → IC=+0.141 (n=4793)
...
- **PATRÓN** `lag_apertura_s` < `3.11` → IC=+0.143 (n=4490)
...
- **PATRÓN** `restante_min` < `4.08` → IC=+0.130 (n=2228)

  - _Acción_: Kelly boost +0.65€ cuando `restante_min` < 4.08 (IC base=+0.122)

- **PATRÓN** `restante_min` > `4.94` → IC=+0.140 (n=2370)
...
- **PATRÓN** `lag_apertura_s` < `3.47` → IC=+0.146 (n=2228)
...
- **PATRÓN** `libro_liquidez` > `1782.98` → IC=+0.243 (n=840)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1782.98 (IC base=+0.235)
...
- **PATRÓN** `drift_60min` |x|≤ `0.086` → IC=+0.250 (n=434)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.086 (IC base=+0.214)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.230 (n=1366)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1948` → IC=+0.221 (n=697)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1948 (IC base=+0.214)

- **PATRÓN** `dist_vwap_pct` < `0.5785` → IC=+0.216 (n=1351)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5785 (IC base=+0.214)
...
- **PATRÓN** `volumen_regimen` < `1.2525` → IC=+0.218 (n=1301)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2525 (IC base=+0.214)

- **PATRÓN** `volumen_regimen` > `1.0826` → IC=+0.223 (n=590)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0826 (IC base=+0.214)

- **PATRÓN** `volumen_pendiente_norm` > `0.0739` → IC=+0.230 (n=535)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0739 (IC base=+0.214)

- **PATRÓN** `volumen_spike_ratio` < `1.4011` → IC=+0.221 (n=425)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4011 (IC base=+0.214)

- **PATRÓN** `volumen_spike_ratio` > `2.3632` → IC=+0.221 (n=425)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3632 (IC base=+0.214)

- **PATRÓN** `libro_liquidez` > `16510.0092` → IC=+0.225 (n=434)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 16510.0092 (IC base=+0.214)
...
- **PATRÓN** `ibs_20min` < `0.6839` → IC=+0.169 (n=1351)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.206` → IC=+0.141 (n=1228)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` < 4.206 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `1.2046` → IC=+0.148 (n=1351)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.0948` → IC=+0.173 (n=487)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.0948 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` < `2.4315` → IC=+0.149 (n=1241)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 2.4315 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` > `1.7706` → IC=+0.148 (n=827)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8852` → IC=+0.205 (n=713)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8852 (IC base=+0.188)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.197 (n=1147)

  - _Acción_: Kelly boost +0.99€ cuando `libro_spread` < 0.02 (IC base=+0.188)

- **PATRÓN** `sigma_h` < `0.0112` → IC=+0.223 (n=1418)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0112 (IC base=+0.215)

- **PATRÓN** `drift_60min` |x|≤ `0.5723` → IC=+0.216 (n=1417)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.5723 (IC base=+0.215)
...
- **PATRÓN** `ibs_20min` < `0.0636` → IC=+0.243 (n=624)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0636 (IC base=+0.215)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.688` → IC=+0.238 (n=525)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6224` → IC=+0.206 (n=430)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6224 (IC base=+0.215)

- **PATRÓN** `volumen_spike_ratio` > `2.8784` → IC=+0.227 (n=584)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8784 (IC base=+0.215)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.224 (n=922)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.215)

- **PATRÓN** `libro_liquidez` > `1862.1984` → IC=+0.229 (n=643)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1862.1984 (IC base=+0.215)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.218 (n=1070)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.153 (n=93)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=2091)
...
- **PATRÓN** `volumen_regimen` < `0.8511` → IC=+0.158 (n=510)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5138` → IC=+0.175 (n=636)
...
- **FILTRO** `ibs_20min` > `0.2581` → IC=-0.125 (n=2069)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2581
  - _Potencial_: sin este filtro IC_bueno=+0.124 (n=1021)

- **FILTRO** `sigma_ewma_delta_pct` > `8.654` → IC=-0.207 (n=332)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.654
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=2758)
...
- **PATRÓN** `ibs_20min` < `0.2581` → IC=+0.124 (n=1021)
...
- **FILTRO** `drift_60min` |x|> `0.6434` → IC=-0.182 (n=523)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6434
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=1577)

- **FILTRO** `ibs_20min` < `0.699` → IC=-0.156 (n=1386)

  - _Acción_: SKIP cuando `ibs_20min` < 0.699
  - _Potencial_: sin este filtro IC_bueno=+0.095 (n=714)
...
- **FILTRO** `ibs_20min` > `0.7736` → IC=-0.203 (n=780)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7736
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=2344)

- **PATRÓN** `dist_vwap_pct` > `0.7157` → IC=+0.302 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7157 (IC base=-0.070)

- **PATRÓN** `dist_vwap_pct` < `0.258` → IC=+0.318 (n=256)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.258 (IC base=-0.070)

- **PATRÓN** `volumen_regimen` > `0.6166` → IC=+0.303 (n=307)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6166 (IC base=-0.070)

- **PATRÓN** `volumen_pendiente_norm` > `0.0746` → IC=+0.300 (n=123)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0746 (IC base=-0.070)

- **PATRÓN** `volumen_spike_ratio` < `1.5298` → IC=+0.298 (n=127)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5298 (IC base=-0.070)

- **PATRÓN** `volumen_spike_ratio` > `2.1203` → IC=+0.289 (n=131)
...
- **PATRÓN** `volumen_regimen` < `0.7389` → IC=+0.243 (n=305)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1041` → IC=+0.275 (n=238)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1041 (IC base=-0.026)

- **PATRÓN** `volumen_spike_ratio` < `2.4934` → IC=+0.245 (n=583)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.4934 (IC base=-0.026)

- **PATRÓN** `volumen_spike_ratio` > `1.4512` → IC=+0.245 (n=583)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4512 (IC base=-0.026)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0097` → IC=+0.196 (n=3144)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0097 (IC base=+0.096)

- **PATRÓN** `ibs_20min` > `0.4702` → IC=+0.187 (n=8414)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.4702 (IC base=+0.096)

- **PATRÓN** `dist_vwap_pct` > `0.7447` → IC=+0.287 (n=1014)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7447 (IC base=+0.096)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.561` → IC=+0.156 (n=4455)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 3.561 (IC base=+0.096)

- **PATRÓN** `volumen_regimen` > `0.685` → IC=+0.244 (n=2969)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.685 (IC base=+0.096)

- **PATRÓN** `volumen_pendiente_norm` > `0.2975` → IC=+0.264 (n=797)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2975 (IC base=+0.096)

- **PATRÓN** `volumen_spike_ratio` < `1.4704` → IC=+0.238 (n=1803)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4704 (IC base=+0.096)

- **PATRÓN** `volumen_spike_ratio` > `2.6893` → IC=+0.236 (n=1803)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6893 (IC base=+0.096)

- **PATRÓN** `ballena_activa_n` < `98.0` → IC=+0.267 (n=4919)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 98.0 (IC base=+0.096)

- **PATRÓN** `sigma_h` > `0.009` → IC=+0.146 (n=3155)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` > 0.009 (IC base=+0.071)

- **PATRÓN** `ibs_20min` < `0.5504` → IC=+0.151 (n=8324)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` < 0.5504 (IC base=+0.071)

- **PATRÓN** `dist_vwap_pct` < `0.2434` → IC=+0.239 (n=2588)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2434 (IC base=+0.071)

- **PATRÓN** `volumen_regimen` < `0.7142` → IC=+0.238 (n=1212)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7142 (IC base=+0.071)

- **PATRÓN** `volumen_regimen` > `1.2033` → IC=+0.245 (n=917)
...
- **PATRÓN** `ibs_20min` > `0.8932` → IC=+0.263 (n=648)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8932 (IC base=+0.044)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.294` → IC=+0.164 (n=891)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 3.294 (IC base=+0.044)
...
- **FILTRO** `ibs_20min` > `0.8136` → IC=-0.150 (n=681)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8136
  - _Potencial_: sin este filtro IC_bueno=+0.052 (n=2046)

- **PATRÓN** `ibs_20min` > `0.862` → IC=+0.159 (n=632)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.862 (IC base=+0.023)

- **PATRÓN** `dist_vwap_pct` > `0.3002` → IC=+0.168 (n=353)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` > 0.3002 (IC base=+0.023)

- **PATRÓN** `volumen_regimen` < `1.0443` → IC=+0.144 (n=760)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.0443 (IC base=+0.023)

- **PATRÓN** `volumen_regimen` > `0.6622` → IC=+0.158 (n=772)
...
- **PATRÓN** `dist_vwap_pct` < `0.1574` → IC=+0.219 (n=528)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1574 (IC base=+0.002)

- **PATRÓN** `volumen_regimen` > `0.6093` → IC=+0.211 (n=517)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1657` → IC=+0.232 (n=214)
...
- **PATRÓN** `sigma_h` > `0.0095` → IC=+0.286 (n=675)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0095 (IC base=+0.245)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.246 (n=1492)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.299 (n=794)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1384` → IC=+0.257 (n=1319)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1384 (IC base=+0.245)

- **PATRÓN** `volumen_spike_ratio` > `2.9076` → IC=+0.259 (n=636)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9076 (IC base=+0.245)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.256 (n=1018)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.245)

- **PATRÓN** `libro_liquidez` > `1930.1086` → IC=+0.252 (n=497)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1930.1086 (IC base=+0.245)

- **PATRÓN** `sigma_h` > `0.0095` → IC=+0.319 (n=535)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0095 (IC base=+0.283)

- **PATRÓN** `drift_60min` |x|≤ `0.1657` → IC=+0.285 (n=520)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.2248` → IC=+0.290 (n=1039)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2248 (IC base=+0.283)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.812` → IC=+0.299 (n=456)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6079` → IC=+0.288 (n=362)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6079 (IC base=+0.283)

- **PATRÓN** `volumen_spike_ratio` > `2.7895` → IC=+0.287 (n=491)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7895 (IC base=+0.283)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.292 (n=762)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.283)

- **PATRÓN** `libro_liquidez` > `1914.9372` → IC=+0.308 (n=394)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1914.9372 (IC base=+0.283)

- **PATRÓN** `ballena_activa_n` < `19.0` → IC=+0.291 (n=476)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 19.0 (IC base=+0.283)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **FILTRO** `ibs_20min` < `0.2783` → IC=-0.192 (n=456)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2783
  - _Potencial_: sin este filtro IC_bueno=+0.075 (n=1370)

- **FILTRO** `ibs_20min` > `0.7792` → IC=-0.180 (n=558)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7792
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=1678)

- **PATRÓN** `ibs_20min` > `0.9035` → IC=+0.178 (n=457)

  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` > 0.9035 (IC base=+0.008)

- **PATRÓN** `dist_vwap_pct` > `0.4618` → IC=+0.220 (n=234)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4618 (IC base=+0.008)

- **PATRÓN** `volumen_regimen` < `0.9869` → IC=+0.233 (n=447)
...
- **PATRÓN** `ballena_activa_n` < `102.0` → IC=+0.257 (n=327)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 102.0 (IC base=+0.008)

- **PATRÓN** `dist_vwap_pct` > `0.1552` → IC=+0.197 (n=196)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1552 (IC base=-0.007)

- **PATRÓN** `dist_vwap_pct` < `0.6717` → IC=+0.187 (n=423)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.6717 (IC base=-0.007)

- **PATRÓN** `volumen_regimen` < `1.1615` → IC=+0.198 (n=376)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` < 1.1615 (IC base=-0.007)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1389` → IC=+0.245 (n=151)
...
- **FILTRO** `ibs_20min` < `0.7143` → IC=-0.203 (n=998)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.276 (n=998)

- **FILTRO** `ibs_20min` > `0.6901` → IC=-0.230 (n=524)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6901
  - _Potencial_: sin este filtro IC_bueno=+0.096 (n=1575)

- **FILTRO** `sigma_ewma_delta_pct` > `4.656` → IC=-0.180 (n=461)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.656
  - _Potencial_: sin este filtro IC_bueno=+0.070 (n=1638)

- **PATRÓN** `ibs_20min` > `0.7143` → IC=+0.276 (n=998)
...
- **PATRÓN** `volumen_regimen` < `0.8657` → IC=+0.299 (n=486)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8657 (IC base=+0.036)

- **PATRÓN** `volumen_regimen` > `0.6388` → IC=+0.289 (n=728)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6388 (IC base=+0.036)

- **PATRÓN** `volumen_pendiente_norm` < `0.1034` → IC=+0.292 (n=671)
...
- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.310 (n=684)
...
- **PATRÓN** `ibs_20min` > `0.63` → IC=+0.312 (n=1225)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.63 (IC base=+0.278)

- **PATRÓN** `dist_vwap_pct` > `0.5256` → IC=+0.329 (n=500)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5256 (IC base=+0.278)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.485` → IC=+0.303 (n=648)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.485 (IC base=+0.278)

- **PATRÓN** `volumen_regimen` > `0.8627` → IC=+0.303 (n=817)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8627 (IC base=+0.278)

- **PATRÓN** `volumen_pendiente_norm` > `0.2809` → IC=+0.320 (n=187)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2809 (IC base=+0.278)

- **PATRÓN** `volumen_spike_ratio` > `2.5204` → IC=+0.289 (n=386)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5204 (IC base=+0.278)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.283 (n=1298)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.278)

- **PATRÓN** `libro_liquidez` > `2611.4379` → IC=+0.289 (n=817)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2611.4379 (IC base=+0.278)
...
- **PATRÓN** `ibs_20min` < `0.3962` → IC=+0.302 (n=1340)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3962 (IC base=+0.269)

- **PATRÓN** `dist_vwap_pct` > `0.2873` → IC=+0.277 (n=513)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2873 (IC base=+0.269)

- **PATRÓN** `dist_vwap_pct` < `0.9274` → IC=+0.269 (n=1506)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5625` → IC=+0.171 (n=3117)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` < 1.5625 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.6294` → IC=+0.176 (n=2362)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.6294 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `2393.8414` → IC=+0.168 (n=4935)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2393.8414 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `117.0` → IC=+0.182 (n=6310)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 117.0 (IC base=+0.168)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.182 (n=4694)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0066 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0799` → IC=+0.205 (n=2349)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4762` → IC=+0.226 (n=7036)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4762 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` < `0.2283` → IC=+0.159 (n=5106)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.2283 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.234` → IC=+0.192 (n=1194)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 10.234 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.1748` → IC=+0.151 (n=5121)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.1748 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.2909` → IC=+0.221 (n=1009)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2909 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.5701` → IC=+0.164 (n=2801)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.5701 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.2644` → IC=+0.170 (n=2885)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.2644 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `118.0` → IC=+0.171 (n=5997)
...
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.186 (n=422)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0083 (IC base=+0.183)

- **PATRÓN** `drift_60min` |x|≤ `0.3418` → IC=+0.206 (n=1265)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3418 (IC base=+0.183)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.197 (n=846)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` < 11.0 (IC base=+0.183)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.302 (n=623)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.183)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.117` → IC=+0.312 (n=573)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4352` → IC=+0.183 (n=1165)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 1.4352 (IC base=+0.183)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.239 (n=794)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.1842` → IC=+0.295 (n=602)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1842 (IC base=+0.239)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.247 (n=809)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3427` → IC=+0.266 (n=903)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3427 (IC base=+0.239)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.095` → IC=+0.253 (n=980)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.095 (IC base=+0.239)

- **PATRÓN** `volumen_pendiente_norm` < `0.0953` → IC=+0.238 (n=749)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.238 (n=541)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.239)

- **PATRÓN** `libro_liquidez` > `1946.06` → IC=+0.257 (n=302)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1946.06 (IC base=+0.239)
...
- **PATRÓN** `volumen_regimen` < `1.2653` → IC=+0.163 (n=1103)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2324` → IC=+0.201 (n=242)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2324 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` < `1.4127` → IC=+0.194 (n=357)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 1.4127 (IC base=+0.160)

- **PATRÓN** `libro_liquidez` > `15821.8878` → IC=+0.169 (n=500)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 15821.8878 (IC base=+0.160)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.156 (n=1198)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0057 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.29` → IC=+0.157 (n=1198)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5573` → IC=+0.184 (n=1198)
...
- **PATRÓN** `volumen_regimen` < `1.2116` → IC=+0.153 (n=1198)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4315` → IC=+0.144 (n=1087)
...
- **PATRÓN** `sigma_h` < `0.0111` → IC=+0.236 (n=1016)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0111 (IC base=+0.222)

- **PATRÓN** `drift_60min` |x|≤ `0.0946` → IC=+0.259 (n=338)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.2453` → IC=+0.258 (n=892)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2453 (IC base=+0.222)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.695` → IC=+0.270 (n=424)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.154 (n=1241)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0073 (IC base=+0.122)

- **PATRÓN** `drift_60min` |x|≤ `0.3834` → IC=+0.142 (n=1240)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6196` → IC=+0.168 (n=1240)

  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` < 0.6196 (IC base=+0.122)

- **PATRÓN** `dist_vwap_pct` < `0.1593` → IC=+0.142 (n=1196)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.1593 (IC base=+0.122)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.879` → IC=+0.173 (n=436)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 6.879 (IC base=+0.122)

- **PATRÓN** `volumen_regimen` < `0.8485` → IC=+0.145 (n=827)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.8485 (IC base=+0.122)

- **PATRÓN** `volumen_pendiente_norm` > `0.2894` → IC=+0.200 (n=178)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2894 (IC base=+0.122)

- **PATRÓN** `volumen_spike_ratio` < `1.7931` → IC=+0.129 (n=748)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` < 1.7931 (IC base=+0.122)

- **PATRÓN** `libro_liquidez` > `10007.3401` → IC=+0.160 (n=562)
...
- **FILTRO** `ibs_20min` > `0.5676` → IC=-0.183 (n=452)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.208 (n=1357)
...
- **PATRÓN** `ibs_20min` < `0.5676` → IC=+0.208 (n=1357)
...
- **PATRÓN** `dist_vwap_pct` < `0.196` → IC=+0.136 (n=1238)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.196 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.438` → IC=+0.144 (n=282)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` > 7.438 (IC base=+0.110)

- **PATRÓN** `volumen_regimen` < `1.0382` → IC=+0.122 (n=1194)
...
- **PATRÓN** `ibs_20min` < `0.44` → IC=+0.246 (n=1327)
...
- **PATRÓN** `dist_vwap_pct` < `0.2669` → IC=+0.203 (n=1369)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2669 (IC base=+0.203)
...
- **PATRÓN** `volumen_regimen` > `0.632` → IC=+0.216 (n=1327)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2328` → IC=+0.191 (n=1045)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` < 2.2328 (IC base=+0.203)

- **PATRÓN** `volumen_spike_ratio` > `1.451` → IC=+0.198 (n=1187)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.451 (IC base=+0.203)

- **PATRÓN** `libro_liquidez` > `2570.316` → IC=+0.207 (n=885)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2570.316 (IC base=+0.203)
...
- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.176 (n=575)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.5522` → IC=+0.188 (n=1541)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.5522 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.8623` → IC=+0.193 (n=285)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.8623 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.703` → IC=+0.176 (n=801)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` > 3.703 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` < `0.8753` → IC=+0.162 (n=1002)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.8753 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` > `1.21` → IC=+0.154 (n=501)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.21 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` > `0.1634` → IC=+0.178 (n=476)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` > 0.1634 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` < `1.4364` → IC=+0.165 (n=553)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.4364 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` > `2.5517` → IC=+0.165 (n=553)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 2.5517 (IC base=+0.145)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.149 (n=1945)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.02 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `2503.9142` → IC=+0.147 (n=1541)

  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` > 2503.9142 (IC base=+0.145)

- **PATRÓN** `ballena_activa_n` < `21.0` → IC=+0.169 (n=514)

  - _Acción_: Kelly boost +0.84€ cuando `ballena_activa_n` < 21.0 (IC base=+0.145)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.128 (n=1206)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.64€ cuando `sigma_h` < 0.0057 (IC base=+0.101)

- **PATRÓN** `ibs_20min` < `0.6561` → IC=+0.131 (n=1806)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.6561 (IC base=+0.101)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3387` → IC=+0.146 (n=575)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.3387 (IC base=+0.129)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.143 (n=555)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` > 6.0 (IC base=+0.129)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.130 (n=593)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` < 17.0 (IC base=+0.129)

- **PATRÓN** `ibs_20min` < `0.5957` → IC=+0.175 (n=506)

  - _Acción_: Kelly boost +0.88€ cuando `ibs_20min` < 0.5957 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` < `0.1739` → IC=+0.153 (n=571)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.1739 (IC base=+0.129)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.41` → IC=+0.149 (n=223)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 4.41 (IC base=+0.129)

- **PATRÓN** `volumen_regimen` > `0.7202` → IC=+0.145 (n=514)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1142` → IC=+0.149 (n=497)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 2.1142 (IC base=+0.129)

- **PATRÓN** `volumen_spike_ratio` > `1.42` → IC=+0.138 (n=565)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 1.42 (IC base=+0.129)

- **PATRÓN** `ballena_activa_n` < `390.0` → IC=+0.139 (n=543)

  - _Acción_: Kelly boost +0.69€ cuando `ballena_activa_n` < 390.0 (IC base=+0.129)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0949` → IC=+0.228 (n=171)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.7017` → IC=+0.259 (n=342)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7017 (IC base=+0.201)

- **PATRÓN** `dist_vwap_pct` > `0.3919` → IC=+0.233 (n=185)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3919 (IC base=+0.201)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.996` → IC=+0.241 (n=172)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.996 (IC base=+0.201)

- **PATRÓN** `volumen_regimen` < `0.8432` → IC=+0.215 (n=342)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8432 (IC base=+0.201)

- **PATRÓN** `volumen_regimen` > `1.156` → IC=+0.217 (n=171)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.156 (IC base=+0.201)
...
- **PATRÓN** `volumen_spike_ratio` < `1.3845` → IC=+0.243 (n=169)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3845 (IC base=+0.201)

- **PATRÓN** `volumen_spike_ratio` > `2.405` → IC=+0.248 (n=169)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.405 (IC base=+0.201)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.205 (n=567)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0111` → IC=+0.206 (n=3142)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0111 (IC base=+0.168)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.178 (n=9831)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 5.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.215 (n=9431)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4706 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` > `0.9505` → IC=+0.208 (n=1361)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9505 (IC base=+0.168)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.61` → IC=+0.228 (n=4579)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.61 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` < `0.8826` → IC=+0.165 (n=4202)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.8826 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.2903` → IC=+0.202 (n=1301)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2903 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.6131` → IC=+0.189 (n=3012)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 2.6131 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `2338.0825` → IC=+0.171 (n=6280)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 2338.0825 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `89.0` → IC=+0.195 (n=7106)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 89.0 (IC base=+0.168)

- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.187 (n=7513)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0092 (IC base=+0.180)

- **PATRÓN** `drift_60min` |x|≤ `0.4895` → IC=+0.183 (n=8530)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5641` → IC=+0.237 (n=8530)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5641 (IC base=+0.180)

- **PATRÓN** `dist_vwap_pct` < `0.2426` → IC=+0.161 (n=5282)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2426 (IC base=+0.180)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.918` → IC=+0.198 (n=1200)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` > 9.918 (IC base=+0.180)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.713` → IC=+0.182 (n=8271)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.713 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` < `0.7044` → IC=+0.160 (n=2594)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.7044 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` > `1.2035` → IC=+0.153 (n=1966)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8671` → IC=+0.275 (n=748)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8671 (IC base=+0.258)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.258 (n=743)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.258)

- **PATRÓN** `libro_liquidez` > `1782.62` → IC=+0.270 (n=820)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1782.62 (IC base=+0.258)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0846` → IC=+0.163 (n=505)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.0846 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.161 (n=1581)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.147)

- **PATRÓN** `ibs_20min` > `0.3136` → IC=+0.200 (n=1513)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3136 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` > `0.1284` → IC=+0.183 (n=865)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.1284 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.755` → IC=+0.171 (n=347)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` > 9.755 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.234` → IC=+0.150 (n=1357)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` < 4.234 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `0.6272` → IC=+0.175 (n=505)
...
- **PATRÓN** `ibs_20min` < `0.6466` → IC=+0.192 (n=1304)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` < 0.6466 (IC base=+0.150)
...
- **PATRÓN** `volumen_regimen` < `1.1894` → IC=+0.162 (n=1304)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4232` → IC=+0.160 (n=1206)
...
- **PATRÓN** `ballena_activa_n` < `361.0` → IC=+0.161 (n=741)

  - _Acción_: Kelly boost +0.80€ cuando `ballena_activa_n` < 361.0 (IC base=+0.150)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0116` → IC=+0.242 (n=506)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0116 (IC base=+0.218)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.224 (n=1587)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.218)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.223 (n=1542)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.218)

- **PATRÓN** `ibs_20min` > `0.6721` → IC=+0.258 (n=1355)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6721 (IC base=+0.218)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.779` → IC=+0.295 (n=451)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.779 (IC base=+0.218)

- **PATRÓN** `volumen_pendiente_norm` < `0.212` → IC=+0.223 (n=1486)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.212 (IC base=+0.218)

- **PATRÓN** `volumen_spike_ratio` > `2.8789` → IC=+0.242 (n=652)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8789 (IC base=+0.218)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.230 (n=1044)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.218)

- **PATRÓN** `libro_liquidez` > `1933.3484` → IC=+0.220 (n=505)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1933.3484 (IC base=+0.218)

- **PATRÓN** `ballena_activa_n` < `53.0` → IC=+0.234 (n=1240)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 53.0 (IC base=+0.218)

- **PATRÓN** `sigma_h` < `0.0111` → IC=+0.241 (n=1407)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0111 (IC base=+0.235)

- **PATRÓN** `sigma_h` > `0.008` → IC=+0.237 (n=941)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.008 (IC base=+0.235)

- **PATRÓN** `drift_60min` |x|≤ `0.1615` → IC=+0.244 (n=619)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1615 (IC base=+0.235)
...
- **PATRÓN** `ibs_20min` < `0.3623` → IC=+0.271 (n=1238)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3623 (IC base=+0.235)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.741` → IC=+0.279 (n=505)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7668` → IC=+0.235 (n=565)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7668 (IC base=+0.235)

- **PATRÓN** `volumen_spike_ratio` > `2.2018` → IC=+0.237 (n=854)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2018 (IC base=+0.235)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.246 (n=916)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.235)

- **PATRÓN** `libro_liquidez` > `1863.8584` → IC=+0.248 (n=638)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1863.8584 (IC base=+0.235)

- **PATRÓN** `ballena_activa_n` < `14.0` → IC=+0.252 (n=409)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 14.0 (IC base=+0.235)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0035` → IC=+0.185 (n=537)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0035 (IC base=+0.137)

- **PATRÓN** `drift_60min` |x|≤ `0.4369` → IC=+0.144 (n=1607)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.4369 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.151 (n=1682)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 5.0 (IC base=+0.137)

- **PATRÓN** `ibs_20min` > `0.8773` → IC=+0.262 (n=729)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8773 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.3697` → IC=+0.169 (n=647)

  - _Acción_: Kelly boost +0.84€ cuando `dist_vwap_pct` > 0.3697 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.201` → IC=+0.163 (n=669)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 4.201 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `0.8782` → IC=+0.159 (n=1072)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 0.8782 (IC base=+0.137)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5181` → IC=+0.149 (n=684)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.5181 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `2.4694` → IC=+0.156 (n=518)
...
- **PATRÓN** `ibs_20min` < `0.693` → IC=+0.186 (n=1306)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` < 0.693 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.8101` → IC=+0.141 (n=1505)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.8101 (IC base=+0.137)
...
- **PATRÓN** `volumen_regimen` < `0.8594` → IC=+0.145 (n=871)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4426` → IC=+0.150 (n=1234)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.4426 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `10997.1346` → IC=+0.212 (n=435)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 10997.1346 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `181.0` → IC=+0.142 (n=1229)

  - _Acción_: Kelly boost +0.71€ cuando `ballena_activa_n` < 181.0 (IC base=+0.137)

### GBM_LATE_15M_TARDIO#SOL#15min
- **PATRÓN** `sigma_h` > `0.0081` → IC=+0.135 (n=1062)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0081 (IC base=+0.108)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.129 (n=1636)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 5.0 (IC base=+0.108)

- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.188 (n=1597)
...
- **PATRÓN** `volumen_regimen` < `0.8928` → IC=+0.134 (n=1062)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 0.8928 (IC base=+0.108)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.121 (n=1601)
...
- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.128 (n=1394)

  - _Acción_: Kelly boost +0.64€ cuando `ballena_activa_n` < 62.0 (IC base=+0.108)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.175 (n=524)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0057 (IC base=+0.112)

- **PATRÓN** `drift_60min` |x|≤ `0.1303` → IC=+0.157 (n=523)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.1303 (IC base=+0.112)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.146 (n=724)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` > 15.0 (IC base=+0.112)

- **PATRÓN** `ibs_20min` < `0.6364` → IC=+0.205 (n=1569)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6364 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` < `0.2182` → IC=+0.129 (n=1265)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.2182 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.451` → IC=+0.125 (n=1514)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 3.451 (IC base=+0.112)

- **PATRÓN** `volumen_regimen` < `0.7204` → IC=+0.155 (n=690)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.7204 (IC base=+0.112)

- **PATRÓN** `volumen_pendiente_norm` > `0.2255` → IC=+0.169 (n=240)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` > 0.2255 (IC base=+0.112)
...
- **PATRÓN** `volumen_spike_ratio` > `2.527` → IC=+0.129 (n=470)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` > 2.527 (IC base=+0.112)

- **PATRÓN** `libro_liquidez` > `2861.2843` → IC=+0.168 (n=522)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2861.2843 (IC base=+0.112)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` > `0.0192` → IC=+0.219 (n=1062)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0192 (IC base=+0.208)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.214 (n=1660)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.208)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.209 (n=1427)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.208)

- **PATRÓN** `ibs_20min` > `0.5143` → IC=+0.249 (n=1593)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5143 (IC base=+0.208)

- **PATRÓN** `dist_vwap_pct` > `0.8711` → IC=+0.246 (n=454)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8711 (IC base=+0.208)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.505` → IC=+0.252 (n=756)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.505 (IC base=+0.208)

- **PATRÓN** `volumen_regimen` > `0.6363` → IC=+0.216 (n=1591)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6363 (IC base=+0.208)

- **PATRÓN** `volumen_pendiente_norm` > `0.235` → IC=+0.247 (n=283)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.235 (IC base=+0.208)

- **PATRÓN** `volumen_spike_ratio` > `2.5191` → IC=+0.241 (n=511)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5191 (IC base=+0.208)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.217 (n=1654)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.208)

- **PATRÓN** `libro_liquidez` > `2607.7466` → IC=+0.218 (n=1061)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2607.7466 (IC base=+0.208)
...
- **PATRÓN** `ibs_20min` < `0.5207` → IC=+0.253 (n=1720)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5207 (IC base=+0.198)

- **PATRÓN** `dist_vwap_pct` < `0.8705` → IC=+0.202 (n=1905)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2118` → IC=+0.191 (n=1353)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` < 2.2118 (IC base=+0.198)

- **PATRÓN** `volumen_spike_ratio` > `1.4369` → IC=+0.197 (n=1537)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.137 (n=2866)

- **PATRÓN** `sigma_h` < `0.0093` → IC=+0.158 (n=2425)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0093 (IC base=+0.149)

- **PATRÓN** `sigma_h` > `0.0056` → IC=+0.149 (n=2466)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0056 (IC base=+0.149)

- **PATRÓN** `drift_60min` |x|≤ `0.5254` → IC=+0.159 (n=2755)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.5254 (IC base=+0.149)
...
- **PATRÓN** `ibs_20min` > `0.9408` → IC=+0.209 (n=920)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9408 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` > `0.1908` → IC=+0.158 (n=988)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.1908 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` < `1.1468` → IC=+0.140 (n=1948)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 1.1468 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.152` → IC=+0.181 (n=450)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 10.152 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` > `0.9026` → IC=+0.161 (n=1163)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.9026 (IC base=+0.149)

- **PATRÓN** `volumen_pendiente_norm` > `0.1728` → IC=+0.182 (n=754)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.1728 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` < `1.4578` → IC=+0.157 (n=908)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 1.4578 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` > `1.8898` → IC=+0.161 (n=1814)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 1.8898 (IC base=+0.149)

- **PATRÓN** `libro_liquidez` > `3664.0514` → IC=+0.152 (n=1837)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 3664.0514 (IC base=+0.149)

- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.193 (n=725)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0038 (IC base=+0.135)

- **PATRÓN** `drift_60min` |x|≤ `0.4865` → IC=+0.153 (n=2161)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.1833` → IC=+0.163 (n=951)
...
- **PATRÓN** `dist_vwap_pct` < `0.2474` → IC=+0.128 (n=1927)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.2474 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.214` → IC=+0.143 (n=2151)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 6.214 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` < `0.9017` → IC=+0.150 (n=1374)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4285` → IC=+0.144 (n=712)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 1.4285 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` > `1.8137` → IC=+0.141 (n=1424)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` > 1.8137 (IC base=+0.135)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.137 (n=2866)

  - _Acción_: Kelly boost +0.69€ cuando `libro_spread` < 0.01 (IC base=+0.135)

- **PATRÓN** `libro_liquidez` > `7059.7056` → IC=+0.150 (n=1931)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 7059.7056 (IC base=+0.135)

### GBM_LATE_5M#BTC#5min
- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.165 (n=317)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0056 (IC base=+0.153)

- **PATRÓN** `sigma_h` > `0.0033` → IC=+0.164 (n=322)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` > 0.0033 (IC base=+0.153)

- **PATRÓN** `drift_60min` |x|≤ `0.0902` → IC=+0.180 (n=120)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.90€ cuando `drift_60min` |x|≤ 0.0902 (IC base=+0.153)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=368)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.153)

- **PATRÓN** `ibs_20min` < `0.5194` → IC=+0.190 (n=240)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` < 0.5194 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` > `0.2197` → IC=+0.171 (n=168)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.2197 (IC base=+0.153)

- **PATRÓN** `dist_vwap_pct` < `0.4017` → IC=+0.161 (n=352)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.4017 (IC base=+0.153)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.366` → IC=+0.165 (n=389)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` < 2.366 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` > `0.8487` → IC=+0.186 (n=240)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` > 0.8487 (IC base=+0.153)

- **PATRÓN** `volumen_pendiente_norm` > `0.3101` → IC=+0.295 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3101 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` < `1.454` → IC=+0.189 (n=120)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` < 1.454 (IC base=+0.153)
...
- **PATRÓN** `libro_liquidez` > `12537.6672` → IC=+0.194 (n=321)
  ...
  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 12537.6672 (IC base=+0.153)
...
- **PATRÓN** `sigma_h` < `0.0087` → IC=+0.156 (n=842)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0087 (IC base=+0.151)

- **PATRÓN** `sigma_h` > `0.0044` → IC=+0.160 (n=842)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` > 0.0044 (IC base=+0.151)

- **PATRÓN** `drift_60min` |x|≤ `0.1256` → IC=+0.154 (n=281)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.1256 (IC base=+0.151)
...
- **PATRÓN** `hora_utc` < `4.0` → IC=+0.172 (n=297)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` < 4.0 (IC base=+0.151)

- **PATRÓN** `ibs_20min` < `0.5529` → IC=+0.156 (n=562)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` < 0.5529 (IC base=+0.151)

- **PATRÓN** `ibs_20min` > `0.8892` → IC=+0.168 (n=281)

  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` > 0.8892 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` > `0.9899` → IC=+0.163 (n=197)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.9899 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` < `0.424` → IC=+0.162 (n=777)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.424 (IC base=+0.151)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.126` → IC=+0.152 (n=139)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 11.126 (IC base=+0.151)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.684` → IC=+0.160 (n=844)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` < 6.684 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` > `0.7206` → IC=+0.154 (n=752)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 0.7206 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` < `0.1126` → IC=+0.152 (n=775)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_pendiente_norm` < 0.1126 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` > `0.1724` → IC=+0.164 (n=248)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.1724 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` < `2.2097` → IC=+0.157 (n=726)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.2097 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` > `1.5168` → IC=+0.152 (n=737)
...
- **PATRÓN** `sigma_h` < `0.0084` → IC=+0.154 (n=707)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0084 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.395` → IC=+0.172 (n=622)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.735` → IC=+0.140 (n=707)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` < 0.735 (IC base=+0.141)

- **PATRÓN** `ibs_20min` > `0.1015` → IC=+0.152 (n=706)

  - _Acción_: Kelly boost +0.76€ cuando `ibs_20min` > 0.1015 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` > `0.6243` → IC=+0.171 (n=165)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.6243 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` < `0.3863` → IC=+0.144 (n=708)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.3863 (IC base=+0.141)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.985` → IC=+0.150 (n=155)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 8.985 (IC base=+0.141)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.281` → IC=+0.142 (n=641)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` < 4.281 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` < `1.0954` → IC=+0.152 (n=622)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.0954 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` > `0.7253` → IC=+0.144 (n=631)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 0.7253 (IC base=+0.141)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1931` → IC=+0.155 (n=610)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.1931 (IC base=+0.141)
...
- **PATRÓN** `libro_liquidez` > `7600.4466` → IC=+0.165 (n=706)
...
- **PATRÓN** `sigma_h` > `0.0119` → IC=+0.224 (n=85)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0119 (IC base=+0.092)

- **PATRÓN** `drift_60min` |x|≤ `0.5393` → IC=+0.127 (n=164)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.63€ cuando `drift_60min` |x|≤ 0.5393 (IC base=+0.092)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.144 (n=130)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` < 10.0 (IC base=+0.092)

- **PATRÓN** `ibs_20min` < `0.1538` → IC=+0.192 (n=63)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` < 0.1538 (IC base=+0.092)

- **PATRÓN** `dist_vwap_pct` > `0.6396` → IC=+0.167 (n=94)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.6396 (IC base=+0.092)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.774` → IC=+0.124 (n=107)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 2.774 (IC base=+0.092)

- **PATRÓN** `volumen_pendiente_norm` < `0.0881` → IC=+0.148 (n=143)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` < 0.0881 (IC base=+0.092)

- **PATRÓN** `libro_liquidez` > `3218.3001` → IC=+0.133 (n=186)

  - _Acción_: Kelly boost +0.66€ cuando `libro_liquidez` > 3218.3001 (IC base=+0.092)

- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.150 (n=155)

  - _Acción_: Kelly boost +0.75€ cuando `ballena_activa_n` < 56.0 (IC base=+0.092)
...
- **PATRÓN** `ibs_20min` < `0.0674` → IC=+0.256 (n=117)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0674 (IC base=-0.004)

- **PATRÓN** `volumen_pendiente_norm` > `0.1383` → IC=+0.203 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1383 (IC base=-0.004)

- **PATRÓN** `volumen_spike_ratio` < `2.5975` → IC=+0.135 (n=206)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` < 2.5975 (IC base=-0.004)
...
- **PATRÓN** `ibs_20min` < `0.0889` → IC=+0.255 (n=51)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0889 (IC base=+0.048)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.732` → IC=+0.163 (n=96)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` < 4.732 (IC base=+0.048)

- **PATRÓN** `volumen_regimen` < `0.5944` → IC=+0.183 (n=39)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.5944 (IC base=+0.048)

- **PATRÓN** `volumen_pendiente_norm` > `0.0664` → IC=+0.227 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0664 (IC base=+0.048)

- **PATRÓN** `volumen_spike_ratio` < `2.3987` → IC=+0.170 (n=92)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` < 2.3987 (IC base=+0.048)

- **PATRÓN** `libro_liquidez` > `3342.6566` → IC=+0.134 (n=69)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 3342.6566 (IC base=+0.048)
...
- **PATRÓN** `py_entrada` > `0.5` → IC=+0.123 (n=642)

  - _Acción_: Kelly boost +0.61€ cuando `py_entrada` > 0.5 (IC base=+0.107)

- **PATRÓN** `libro_liquidez` > `2874.277` → IC=+0.162 (n=217)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 2874.277 (IC base=+0.107)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=1627)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=7315)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.175 (n=3152)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=10003)

- **FILTRO** `py_entrada` > `0.595` → IC=-0.171 (n=3400)

  - _Acción_: SKIP cuando `py_entrada` > 0.595
  - _Potencial_: sin este filtro IC_bueno=+0.034 (n=10218)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.45` → IC=-0.216 (n=561)

  - _Acción_: SKIP cuando `py_entrada` < 0.45
  - _Potencial_: sin este filtro IC_bueno=+0.101 (n=1709)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.202 (n=576)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.107 (n=1735)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.207 (n=598)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.063 (n=1830)

- **FILTRO** `ibs_20min` > `0.2846` → IC=-0.164 (n=605)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2846
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=1823)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.48` → IC=-0.177 (n=550)

  - _Acción_: SKIP cuando `py_entrada` < 0.48
  - _Potencial_: sin este filtro IC_bueno=+0.083 (n=1702)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.182 (n=601)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=1824)

### MOMENTUM_IBS_15M_FADE
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=2667)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.130 (n=9434)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.077 (n=20909)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.274 (n=7348)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=22995)

- **FILTRO** `ibs_7min` < `0.2907` → IC=-0.236 (n=7585)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2907
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=22758)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.156 (n=10286)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=20057)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.230 (n=9397)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=28495)

- **FILTRO** `ibs_7min` > `0.2941` → IC=-0.180 (n=9432)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2941
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=28460)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.135 (n=1544)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.076 (n=3438)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.306 (n=1188)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=3794)

- **FILTRO** `ibs_7min` < `0.7099` → IC=-0.251 (n=1643)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7099
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=3339)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.184 (n=1162)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=3820)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.260 (n=1624)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.004 (n=4901)

- **FILTRO** `drift_7min_pct` |x|> `0.1121` → IC=-0.124 (n=2215)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1121
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=4310)

- **FILTRO** `ibs_7min` > `0.7925` → IC=-0.205 (n=1631)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7925
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=4894)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=1237)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=4025)
...
- **FILTRO** `ibs_7min` < `0.7501` → IC=-0.193 (n=1315)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7501
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=3947)

- **FILTRO** `ballena_activa_n` > `160.0` → IC=-0.172 (n=1312)

  - _Acción_: SKIP cuando `ballena_activa_n` > 160.0
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=3950)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.257 (n=1308)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.034 (n=3989)

- **FILTRO** `ibs_7min` > `0.2588` → IC=-0.176 (n=1324)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2588
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=3973)

- **FILTRO** `ballena_activa_n` > `153.0` → IC=-0.187 (n=1324)

  - _Acción_: SKIP cuando `ballena_activa_n` > 153.0
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=3973)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.169 (n=1352)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=3368)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.309 (n=1152)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=3568)

- **FILTRO** `ibs_7min` < `0.1966` → IC=-0.260 (n=1180)

  - _Acción_: SKIP cuando `ibs_7min` < 0.1966
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=3540)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.214 (n=1116)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=3604)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.239 (n=1613)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=5346)

- **FILTRO** `ibs_7min` > `0.7547` → IC=-0.178 (n=1739)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7547
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=5220)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `py_entrada` < `0.35` → IC=-0.240 (n=1227)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=3774)

- **FILTRO** `ibs_7min` < `0.7428` → IC=-0.180 (n=1250)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7428
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=3751)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.171 (n=1225)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3776)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.265 (n=1128)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=3958)

- **FILTRO** `ibs_7min` > `0.2747` → IC=-0.173 (n=1271)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2747
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=3815)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.191 (n=1244)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=3842)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.262 (n=1262)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=4060)

- **FILTRO** `ibs_7min` < `0.7` → IC=-0.240 (n=1304)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=4018)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.179 (n=1725)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=5474)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.34` → IC=-0.274 (n=1193)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=3863)

- **FILTRO** `ibs_7min` < `0.7037` → IC=-0.229 (n=1262)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7037
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=3794)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.213 (n=1214)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=3842)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.203 (n=1666)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=5160)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.015 (n=2762)
...
- **FILTRO** `libro_liquidez` < `16223.9385` → IC=-0.202 (n=996)

  - _Acción_: SKIP cuando `libro_liquidez` < 16223.9385
...
| ✅ BALLENAS_TARDIAS | 23890 | -0.091 | -3085.10€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1415 | -0.046 | -217.33€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 22475 | -0.094 | -2867.77€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#ETH | 6857 | -0.026 | -604.25€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 6857 | -0.026 | -604.25€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 6347 | -0.095 | -417.47€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 6347 | -0.095 | -417.47€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 5332 | -0.183 | -1111.11€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 5332 | -0.183 | -1111.11€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 16471 | -0.030 | +4117.88€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 4323 | -0.000 | +1832.92€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 12148 | -0.041 | +2284.96€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 16471 | -0.030 | +4117.88€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 4323 | -0.000 | +1832.92€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 12148 | -0.041 | +2284.96€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO | 1445 | -0.103 | -188.89€ | 3 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#15min | 168 | -0.053 | -21.36€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#5min | 1277 | -0.109 | -167.54€ | 0 | 0 |
...
| ✅ CANDIDATA9_BOT_CONSENSO#ETH | 467 | -0.129 | -77.38€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#ETH#15min | 24 | -0.077 | -5.22€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#ETH#5min | 443 | -0.132 | -72.16€ | 3 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#SOL | 119 | -0.045 | -14.09€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#SOL#5min | 119 | -0.045 | -14.09€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#XRP | 57 | -0.161 | -4.36€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#XRP#5min | 57 | -0.161 | -4.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 87587 | +0.113 | -4465.36€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 