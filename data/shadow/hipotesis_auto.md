# Hipótesis automáticas — 2026-09-25 19:40 UTC
_Generado por shadow_postmortem.py sobre 609293 resoluciones (PNL=+68917.83€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.33` → IC=-0.228 (n=7189)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.33
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=21568)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `137.45` → IC=-0.250 (n=946)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 137.45
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=2839)

### BALLENAS_TARDIAS#BTC#15min
- **FILTRO** `restante_s_al_confirmar` < `500.19` → IC=-0.142 (n=372)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 500.19
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=1118)
...
- **FILTRO** `restante_s_al_confirmar` < `165.95` → IC=-0.214 (n=1744)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 165.95
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=5235)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `127.7` → IC=-0.351 (n=1413)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 127.7
  - _Potencial_: sin este filtro IC_bueno=-0.112 (n=4242)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.207 (n=2160)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.205 (n=1557)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.204 (n=2203)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.202)

- **PATRÓN** `libro_liquidez` > `15892.8673` → IC=+0.216 (n=569)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.155 (n=2861)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.715` → IC=+0.343 (n=909)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.715 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.249 (n=535)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.235 (n=1507)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.232)

### FAVORITO_CONFIRMADO#ETH#60min
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.158 (n=469)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1313.0714` → IC=+0.151 (n=671)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.231 (n=715)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.161 (n=1103)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.202 (n=11730)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.226 (n=3853)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.168 (n=2832)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.245 (n=890)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.186 (n=2645)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.199 (n=2705)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.196 (n=2010)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.201 (n=34920)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.198)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.235 (n=15501)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.75 (IC base=+0.198)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.177 (n=6029)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.190 (n=6512)

  - _Acción_: Kelly boost +0.95€ cuando `py_entrada` > 0.71 (IC base=+0.176)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.225 (n=6264)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.232 (n=3139)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `4103.5959` → IC=+0.306 (n=375)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4103.5959 (IC base=+0.289)

### FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#BTC#60min
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.298 (n=349)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `4263.4949` → IC=+0.298 (n=334)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.305 (n=213)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.275 (n=473)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.257)

- **PATRÓN** `libro_liquidez` > `1375.0881` → IC=+0.286 (n=376)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1375.0881 (IC base=+0.257)
...
### GBM_LATE_15M
- **PATRÓN** `drift_60min` |x|≤ `0.4868` → IC=+0.123 (n=8256)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 0.4868 (IC base=+0.106)

- **PATRÓN** `ibs_20min` > `0.9821` → IC=+0.243 (n=2752)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9821 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` > `0.8405` → IC=+0.252 (n=501)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8405 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` < `0.3997` → IC=+0.251 (n=2077)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3997 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.342` → IC=+0.188 (n=2182)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 8.342 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` < `1.2074` → IC=+0.248 (n=2248)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2074 (IC base=+0.106)

- **PATRÓN** `volumen_regimen` > `1.0442` → IC=+0.262 (n=1019)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0442 (IC base=+0.106)

- **PATRÓN** `volumen_pendiente_norm` > `0.3033` → IC=+0.224 (n=828)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3033 (IC base=+0.106)

- **PATRÓN** `volumen_spike_ratio` > `1.4617` → IC=+0.203 (n=5674)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4617 (IC base=+0.106)

- **PATRÓN** `ibs_20min` < `0.5714` → IC=+0.133 (n=9972)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.5714 (IC base=+0.065)

- **PATRÓN** `dist_vwap_pct` > `0.6152` → IC=+0.203 (n=739)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6152 (IC base=+0.065)

- **PATRÓN** `volumen_regimen` < `0.697` → IC=+0.184 (n=1541)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 0.697 (IC base=+0.065)

- **PATRÓN** `volumen_regimen` > `1.0502` → IC=+0.176 (n=1588)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 1.0502 (IC base=+0.065)

- **PATRÓN** `volumen_pendiente_norm` > `0.167` → IC=+0.226 (n=1675)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.167 (IC base=+0.065)

- **PATRÓN** `volumen_spike_ratio` > `1.5694` → IC=+0.200 (n=5279)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5694 (IC base=+0.065)

- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.210 (n=5703)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.479` → IC=+0.244 (n=1468)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.236 (n=1559)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.234 (n=1427)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.5724` → IC=+0.222 (n=1499)
...
- **PATRÓN** `volumen_regimen` < `1.2524` → IC=+0.220 (n=1427)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2524 (IC base=+0.217)

- **PATRÓN** `volumen_regimen` > `1.0786` → IC=+0.229 (n=647)
...
- **PATRÓN** `libro_liquidez` > `12554.3019` → IC=+0.221 (n=1275)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12554.3019 (IC base=+0.217)

- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.167 (n=996)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0039 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.0775` → IC=+0.169 (n=497)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.0775 (IC base=+0.141)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.171 (n=582)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.7006` → IC=+0.173 (n=1489)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.7006 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` < `0.1336` → IC=+0.157 (n=1330)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.274` → IC=+0.145 (n=1357)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 4.274 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` < `1.2025` → IC=+0.152 (n=1489)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.2025 (IC base=+0.141)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4183` → IC=+0.154 (n=1378)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.4183 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` > `1.4185` → IC=+0.149 (n=1378)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.4185 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `14134.8887` → IC=+0.145 (n=992)

  - _Acción_: Kelly boost +0.72€ cuando `libro_liquidez` > 14134.8887 (IC base=+0.141)

- **PATRÓN** `ballena_activa_n` < `235.0` → IC=+0.167 (n=575)

  - _Acción_: Kelly boost +0.84€ cuando `ballena_activa_n` < 235.0 (IC base=+0.141)

### GBM_LATE_15M#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0117` → IC=+0.205 (n=615)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0117 (IC base=+0.185)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.191 (n=1931)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.199` → IC=+0.252 (n=385)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.199 (IC base=+0.185)

- **PATRÓN** `volumen_pendiente_norm` < `0.2092` → IC=+0.187 (n=1839)
...
- **PATRÓN** `volumen_spike_ratio` > `2.816` → IC=+0.205 (n=792)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3528` → IC=+0.273 (n=227)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3528 (IC base=+0.209)

- **PATRÓN** `volumen_spike_ratio` < `1.7646` → IC=+0.202 (n=639)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7646 (IC base=+0.209)

- **PATRÓN** `volumen_spike_ratio` > `2.1929` → IC=+0.216 (n=968)
...
- **PATRÓN** `libro_liquidez` > `1903.9584` → IC=+0.214 (n=719)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1903.9584 (IC base=+0.209)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.144 (n=102)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=2263)
...
- **PATRÓN** `volumen_regimen` < `0.8471` → IC=+0.166 (n=573)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.8471 (IC base=+0.018)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5223` → IC=+0.177 (n=720)
...
- **FILTRO** `hora_utc` < `17.0` → IC=-0.177 (n=60)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.088 (n=333)

- **FILTRO** `ibs_20min` < `0.2821` → IC=-0.200 (n=98)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2821
  - _Potencial_: sin este filtro IC_bueno=+0.130 (n=295)

- **FILTRO** `ibs_20min` > `0.25` → IC=-0.124 (n=2235)

  - _Acción_: SKIP cuando `ibs_20min` > 0.25
  - _Potencial_: sin este filtro IC_bueno=+0.124 (n=1102)

- **FILTRO** `sigma_ewma_delta_pct` > `8.692` → IC=-0.207 (n=357)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.692
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=2980)

- **PATRÓN** `ibs_20min` > `0.6` → IC=+0.180 (n=198)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` > 0.6 (IC base=+0.047)

- **PATRÓN** `dist_vwap_pct` > `1.8814` → IC=+0.346 (n=24)
...
- **PATRÓN** `sigma_h` > `0.0098` → IC=+0.197 (n=3465)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0098 (IC base=+0.098)

- **PATRÓN** `ibs_20min` > `0.4762` → IC=+0.187 (n=9277)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` > 0.4762 (IC base=+0.098)

- **PATRÓN** `dist_vwap_pct` > `0.7669` → IC=+0.285 (n=1108)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7669 (IC base=+0.098)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.609` → IC=+0.157 (n=4873)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 3.609 (IC base=+0.098)

- **PATRÓN** `volumen_regimen` > `0.6894` → IC=+0.249 (n=3310)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6894 (IC base=+0.098)

- **PATRÓN** `volumen_pendiente_norm` > `0.2942` → IC=+0.271 (n=878)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2942 (IC base=+0.098)

- **PATRÓN** `volumen_spike_ratio` < `1.4646` → IC=+0.237 (n=2004)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4646 (IC base=+0.098)

- **PATRÓN** `volumen_spike_ratio` > `2.6541` → IC=+0.244 (n=2004)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6541 (IC base=+0.098)

- **PATRÓN** `ballena_activa_n` < `97.0` → IC=+0.267 (n=5534)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 97.0 (IC base=+0.098)

- **PATRÓN** `sigma_h` > `0.0092` → IC=+0.159 (n=3441)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0092 (IC base=+0.074)

- **PATRÓN** `ibs_20min` < `0.5499` → IC=+0.154 (n=9076)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.5499 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` > `0.7294` → IC=+0.241 (n=651)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7294 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` < `0.2538` → IC=+0.240 (n=2891)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2538 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` < `0.7075` → IC=+0.241 (n=1344)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7075 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` > `1.2044` → IC=+0.249 (n=1018)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2044 (IC base=+0.074)

- **PATRÓN** `volumen_pendiente_norm` > `0.2435` → IC=+0.303 (n=761)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2435 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` < `1.5994` → IC=+0.259 (n=1779)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5994 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` > `2.3006` → IC=+0.259 (n=1833)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3006 (IC base=+0.074)

- **PATRÓN** `ballena_activa_n` < `84.0` → IC=+0.264 (n=3918)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 84.0 (IC base=+0.074)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `ibs_20min` < `0.2584` → IC=-0.145 (n=714)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2584
  - _Potencial_: sin este filtro IC_bueno=+0.106 (n=2147)

- **FILTRO** `sigma_ewma_delta_pct` > `4.548` → IC=-0.157 (n=543)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.548
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=1834)

- **PATRÓN** `ibs_20min` > `0.8935` → IC=+0.266 (n=716)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4309` → IC=+0.169 (n=303)
...
- **PATRÓN** `dist_vwap_pct` < `0.1624` → IC=+0.223 (n=601)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1624 (IC base=+0.006)

- **PATRÓN** `volumen_regimen` > `0.8501` → IC=+0.228 (n=391)
...
- **PATRÓN** `libro_liquidez` > `1803.4984` → IC=+0.248 (n=1087)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1803.4984 (IC base=+0.246)
...
- **FILTRO** `ibs_20min` < `0.2958` → IC=-0.181 (n=509)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2958
  - _Potencial_: sin este filtro IC_bueno=+0.079 (n=1529)

- **FILTRO** `ibs_20min` > `0.7745` → IC=-0.187 (n=611)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7745
  - _Potencial_: sin este filtro IC_bueno=+0.055 (n=1835)

- **PATRÓN** `ibs_20min` > `0.8233` → IC=+0.159 (n=693)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` > 0.8233 (IC base=+0.014)
...
- **FILTRO** `ibs_20min` < `0.7273` → IC=-0.193 (n=1094)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.277 (n=1104)

- **FILTRO** `ibs_20min` > `0.6875` → IC=-0.234 (n=563)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6875
  - _Potencial_: sin este filtro IC_bueno=+0.096 (n=1708)

- **FILTRO** `sigma_ewma_delta_pct` > `4.71` → IC=-0.184 (n=495)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.71
  - _Potencial_: sin este filtro IC_bueno=+0.069 (n=1776)

- **PATRÓN** `ibs_20min` > `0.7273` → IC=+0.277 (n=1104)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7273 (IC base=+0.043)

- **PATRÓN** `dist_vwap_pct` > `0.2161` → IC=+0.314 (n=502)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2161 (IC base=+0.043)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.635` → IC=+0.172 (n=345)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 9.635 (IC base=+0.043)

- **PATRÓN** `volumen_regimen` < `0.8684` → IC=+0.304 (n=543)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8684 (IC base=+0.043)

- **PATRÓN** `volumen_regimen` > `0.7299` → IC=+0.294 (n=727)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7299 (IC base=+0.043)

- **PATRÓN** `volumen_pendiente_norm` < `0.1016` → IC=+0.292 (n=758)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1016 (IC base=+0.043)

- **PATRÓN** `volumen_pendiente_norm` > `0.2244` → IC=+0.299 (n=152)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2244 (IC base=+0.043)

- **PATRÓN** `volumen_spike_ratio` < `1.4185` → IC=+0.326 (n=263)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4185 (IC base=+0.043)

- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.315 (n=689)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 56.0 (IC base=+0.043)

- **PATRÓN** `ibs_20min` < `0.1923` → IC=+0.191 (n=752)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` < 0.1923 (IC base=+0.014)
...
- **PATRÓN** `ibs_20min` > `0.74` → IC=+0.322 (n=1203)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.606` → IC=+0.301 (n=708)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.606 (IC base=+0.277)

- **PATRÓN** `volumen_regimen` > `0.8595` → IC=+0.302 (n=897)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8595 (IC base=+0.277)

- **PATRÓN** `volumen_pendiente_norm` < `0.0784` → IC=+0.280 (n=1146)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0784 (IC base=+0.277)

- **PATRÓN** `volumen_pendiente_norm` > `0.2809` → IC=+0.327 (n=200)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2809 (IC base=+0.277)

- **PATRÓN** `volumen_spike_ratio` > `2.1479` → IC=+0.297 (n=579)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1479 (IC base=+0.277)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.282 (n=1440)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.277)

- **PATRÓN** `libro_liquidez` > `2638.0182` → IC=+0.293 (n=897)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2638.0182 (IC base=+0.277)

- **PATRÓN** `sigma_h` > `0.0154` → IC=+0.301 (n=963)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0154 (IC base=+0.273)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.283 (n=496)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3925` → IC=+0.304 (n=1445)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4234` → IC=+0.278 (n=426)
...
- **PATRÓN** `libro_liquidez` > `2632.213` → IC=+0.277 (n=963)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2632.213 (IC base=+0.273)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.171 (n=2693)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0049 (IC base=+0.167)

- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.199 (n=2687)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0113 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0908` → IC=+0.187 (n=2686)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.0908 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.179 (n=8404)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 5.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` > `0.5744` → IC=+0.218 (n=8058)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5744 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.1784` → IC=+0.194 (n=3518)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.1784 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.333` → IC=+0.255 (n=1645)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.333 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.211` → IC=+0.159 (n=5339)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.211 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` > `0.6297` → IC=+0.160 (n=5341)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.6297 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.2436` → IC=+0.194 (n=1624)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2436 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.5584` → IC=+0.168 (n=3402)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.5584 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.6142` → IC=+0.175 (n=2577)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.6142 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `2409.4285` → IC=+0.168 (n=5372)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2409.4285 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `114.0` → IC=+0.180 (n=6968)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 114.0 (IC base=+0.167)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.182 (n=5128)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0066 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0816` → IC=+0.207 (n=2568)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0816 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.205 (n=2957)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` < `0.4815` → IC=+0.225 (n=7693)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4815 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.9221` → IC=+0.144 (n=756)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.9221 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` < `0.2357` → IC=+0.158 (n=5573)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.2357 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.343` → IC=+0.192 (n=1301)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 10.343 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.1733` → IC=+0.151 (n=5565)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.1733 (IC base=+0.167)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5583` → IC=+0.163 (n=3082)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.5583 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.2479` → IC=+0.170 (n=3175)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.2479 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `115.0` → IC=+0.172 (n=6652)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3425` → IC=+0.206 (n=1367)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3425 (IC base=+0.183)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.184 (n=495)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_spike_ratio` > `1.4343` → IC=+0.182 (n=1266)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 1.4343 (IC base=+0.183)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.197 (n=1392)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1884` → IC=+0.287 (n=670)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.3451` → IC=+0.263 (n=1004)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3451 (IC base=+0.237)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.344` → IC=+0.249 (n=1086)
...
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.230 (n=402)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.184 (n=1266)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 5.0 (IC base=+0.161)

- **PATRÓN** `ibs_20min` > `0.4035` → IC=+0.228 (n=1199)
...
- **PATRÓN** `volumen_regimen` < `0.6882` → IC=+0.166 (n=528)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.6882 (IC base=+0.161)
...
- **PATRÓN** `volumen_regimen` < `1.2089` → IC=+0.159 (n=1302)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4411` → IC=+0.146 (n=1190)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 2.4411 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `1.4211` → IC=+0.137 (n=1190)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` > 1.4211 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `214.0` → IC=+0.169 (n=373)

  - _Acción_: Kelly boost +0.85€ cuando `ballena_activa_n` < 214.0 (IC base=+0.137)

### GBM_LATE_15M_MULTIHORIZONTE#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0102` → IC=+0.217 (n=614)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0102 (IC base=+0.199)

- **PATRÓN** `drift_60min` |x|≤ `0.2355` → IC=+0.217 (n=902)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2355 (IC base=+0.199)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.205 (n=1403)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.422` → IC=+0.274 (n=312)
...
- **PATRÓN** `volumen_spike_ratio` > `2.833` → IC=+0.215 (n=584)
...
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.175 (n=1135)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0067 (IC base=+0.142)

- **PATRÓN** `drift_60min` |x|≤ `0.4335` → IC=+0.157 (n=1288)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.4335 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.161 (n=1353)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 5.0 (IC base=+0.142)

- **PATRÓN** `ibs_20min` > `0.3743` → IC=+0.196 (n=1288)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` > 0.3743 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.1608` → IC=+0.179 (n=854)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.1608 (IC base=+0.142)
...
- **PATRÓN** `volumen_regimen` < `1.0384` → IC=+0.144 (n=1134)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.0384 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` > `0.6251` → IC=+0.146 (n=1288)
...
- **PATRÓN** `ibs_20min` < `0.644` → IC=+0.171 (n=1351)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.644 (IC base=+0.122)

- **PATRÓN** `dist_vwap_pct` < `0.368` → IC=+0.135 (n=1460)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.368 (IC base=+0.122)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.957` → IC=+0.165 (n=476)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 6.957 (IC base=+0.122)

- **PATRÓN** `volumen_regimen` < `0.8475` → IC=+0.149 (n=901)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7906` → IC=+0.136 (n=820)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 1.7906 (IC base=+0.122)

- **PATRÓN** `libro_liquidez` > `10039.916` → IC=+0.157 (n=613)
...
- **PATRÓN** `ibs_20min` > `0.5156` → IC=+0.205 (n=1458)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5156 (IC base=+0.118)

- **PATRÓN** `dist_vwap_pct` > `0.5215` → IC=+0.198 (n=631)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.5215 (IC base=+0.118)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.729` → IC=+0.257 (n=327)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.729 (IC base=+0.118)

- **PATRÓN** `volumen_regimen` < `1.2173` → IC=+0.131 (n=1459)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_regimen` < 1.2173 (IC base=+0.118)

- **PATRÓN** `volumen_pendiente_norm` > `0.0711` → IC=+0.125 (n=611)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_pendiente_norm` > 0.0711 (IC base=+0.118)

- **PATRÓN** `volumen_spike_ratio` < `1.5431` → IC=+0.140 (n=620)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 1.5431 (IC base=+0.118)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.127 (n=1523)

  - _Acción_: Kelly boost +0.63€ cuando `libro_spread` < 0.02 (IC base=+0.118)

- **PATRÓN** `libro_liquidez` > `2898.8632` → IC=+0.194 (n=661)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 2898.8632 (IC base=+0.118)

- **PATRÓN** `ballena_activa_n` < `50.0` → IC=+0.136 (n=1132)

  - _Acción_: Kelly boost +0.68€ cuando `ballena_activa_n` < 50.0 (IC base=+0.118)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.155 (n=651)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0062 (IC base=+0.112)

- **PATRÓN** `drift_60min` |x|≤ `0.1048` → IC=+0.161 (n=494)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.1048 (IC base=+0.112)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.129 (n=1498)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 5.0 (IC base=+0.112)

- **PATRÓN** `ibs_20min` < `0.5714` → IC=+0.209 (n=1479)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5714 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `1.0217` → IC=+0.133 (n=208)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` > 1.0217 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` < `0.2057` → IC=+0.136 (n=1353)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.2057 (IC base=+0.112)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.542` → IC=+0.130 (n=306)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` > 7.542 (IC base=+0.112)

- **PATRÓN** `volumen_regimen` < `0.6381` → IC=+0.143 (n=494)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 0.6381 (IC base=+0.112)
...
- **PATRÓN** `ibs_20min` > `0.7391` → IC=+0.257 (n=1246)
...
- **PATRÓN** `sigma_h` > `0.0224` → IC=+0.205 (n=652)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0224 (IC base=+0.202)

- **PATRÓN** `drift_60min` |x|≤ `0.0902` → IC=+0.219 (n=479)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0902 (IC base=+0.202)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.220 (n=704)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.4385` → IC=+0.243 (n=1437)
...
- **PATRÓN** `volumen_regimen` > `0.6295` → IC=+0.214 (n=1437)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1933` → IC=+0.192 (n=1138)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` < 2.1933 (IC base=+0.202)

- **PATRÓN** `volumen_spike_ratio` > `1.434` → IC=+0.197 (n=1293)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` > 1.434 (IC base=+0.202)

- **PATRÓN** `libro_liquidez` > `2605.1416` → IC=+0.209 (n=958)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2605.1416 (IC base=+0.202)

### GBM_LATE_15M_PYCONFIRMADO
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.166 (n=651)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0039 (IC base=+0.147)

- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.174 (n=652)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_spike_ratio` > `1.824` → IC=+0.153 (n=1253)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.824 (IC base=+0.147)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.152 (n=2211)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.02 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `12453.6384` → IC=+0.160 (n=650)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 12453.6384 (IC base=+0.147)

- **PATRÓN** `ballena_activa_n` < `156.0` → IC=+0.168 (n=1731)

  - _Acción_: Kelly boost +0.84€ cuando `ballena_activa_n` < 156.0 (IC base=+0.147)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.137 (n=1349)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0057 (IC base=+0.110)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.123 (n=2043)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 5.0 (IC base=+0.110)

- **PATRÓN** `ibs_20min` < `0.6667` → IC=+0.138 (n=2024)

  - _Acción_: Kelly boost +0.69€ cuando `ibs_20min` < 0.6667 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.2153` → IC=+0.121 (n=1795)
...
- **PATRÓN** `libro_liquidez` > `2760.3927` → IC=+0.122 (n=1805)
...
- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.259 (n=268)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.210 (n=636)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.6428` → IC=+0.196 (n=672)
...
- **PATRÓN** `volumen_regimen` < `0.836` → IC=+0.201 (n=406)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.836 (IC base=+0.195)
...
- **PATRÓN** `sigma_h` > `0.0073` → IC=+0.159 (n=297)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.165 (n=419)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 8.0 (IC base=+0.131)

- **PATRÓN** `ibs_20min` > `0.8906` → IC=+0.242 (n=297)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8906 (IC base=+0.131)

- **PATRÓN** `dist_vwap_pct` > `0.9868` → IC=+0.233 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9868 (IC base=+0.131)
...
- **PATRÓN** `volumen_regimen` < `1.0738` → IC=+0.148 (n=393)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 1.0738 (IC base=+0.131)

- **PATRÓN** `volumen_pendiente_norm` > `0.1826` → IC=+0.174 (n=127)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.1826 (IC base=+0.131)

- **PATRÓN** `volumen_spike_ratio` > `2.2017` → IC=+0.172 (n=193)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 2.2017 (IC base=+0.131)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.135 (n=485)

  - _Acción_: Kelly boost +0.67€ cuando `libro_spread` < 0.02 (IC base=+0.131)

- **PATRÓN** `libro_liquidez` > `3132.1873` → IC=+0.189 (n=149)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3132.1873 (IC base=+0.131)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0114` → IC=+0.204 (n=3440)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0114 (IC base=+0.169)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.180 (n=10787)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 5.0 (IC base=+0.169)

- **PATRÓN** `ibs_20min` > `0.4688` → IC=+0.216 (n=10322)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4688 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` > `0.9764` → IC=+0.202 (n=1472)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9764 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.857` → IC=+0.234 (n=3793)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.857 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` < `0.88` → IC=+0.166 (n=4605)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.88 (IC base=+0.169)

- **PATRÓN** `volumen_pendiente_norm` > `0.2885` → IC=+0.202 (n=1422)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2885 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `2.5931` → IC=+0.187 (n=3310)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 2.5931 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `2354.8161` → IC=+0.172 (n=6880)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 2354.8161 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `86.0` → IC=+0.195 (n=7887)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 86.0 (IC base=+0.169)

- **PATRÓN** `sigma_h` < `0.007` → IC=+0.192 (n=6240)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.007 (IC base=+0.180)

- **PATRÓN** `drift_60min` |x|≤ `0.1496` → IC=+0.189 (n=4117)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.1496 (IC base=+0.180)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.206 (n=3536)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5684` → IC=+0.236 (n=9355)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5684 (IC base=+0.180)

- **PATRÓN** `dist_vwap_pct` < `0.2524` → IC=+0.161 (n=5792)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` < 0.2524 (IC base=+0.180)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.034` → IC=+0.200 (n=1307)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.034 (IC base=+0.180)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.732` → IC=+0.182 (n=9044)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 3.732 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` < `0.7039` → IC=+0.162 (n=2825)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.7039 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` > `1.2022` → IC=+0.154 (n=2140)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.2022 (IC base=+0.180)

- **PATRÓN** `volumen_pendiente_norm` > `0.2876` → IC=+0.239 (n=1230)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2876 (IC base=+0.180)

- **PATRÓN** `volumen_spike_ratio` > `2.6149` → IC=+0.192 (n=2867)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 2.6149 (IC base=+0.180)

- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.193 (n=5568)
...
- **PATRÓN** `drift_60min` |x|≤ `0.356` → IC=+0.191 (n=1742)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.356 (IC base=+0.190)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.202 (n=834)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_spike_ratio` < `1.5488` → IC=+0.255 (n=557)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.709` → IC=+0.168 (n=377)
  ...
  - _Acción_: Kelly boost +0.84€ cuando `sigma_ewma_delta_pct` > 9.709 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.227` → IC=+0.153 (n=1495)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 4.227 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` < `0.6277` → IC=+0.179 (n=553)
...
- **PATRÓN** `ibs_20min` < `0.6603` → IC=+0.196 (n=1435)
...
- **PATRÓN** `dist_vwap_pct` < `0.1371` → IC=+0.166 (n=1299)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `6.511` → IC=+0.154 (n=1454)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 6.511 (IC base=+0.153)

- **PATRÓN** `volumen_regimen` < `1.186` → IC=+0.165 (n=1435)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3998` → IC=+0.162 (n=1337)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.3998 (IC base=+0.153)

- **PATRÓN** `volumen_spike_ratio` > `1.7535` → IC=+0.165 (n=891)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.7535 (IC base=+0.153)

- **PATRÓN** `ballena_activa_n` < `355.0` → IC=+0.162 (n=830)

  - _Acción_: Kelly boost +0.81€ cuando `ballena_activa_n` < 355.0 (IC base=+0.153)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0121` → IC=+0.249 (n=559)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0121 (IC base=+0.218)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.226 (n=1755)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2104` → IC=+0.221 (n=1661)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2104 (IC base=+0.218)

- **PATRÓN** `volumen_spike_ratio` > `2.8194` → IC=+0.241 (n=724)
...
- **PATRÓN** `sigma_h` < `0.0118` → IC=+0.236 (n=1564)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.257 (n=591)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.0159` → IC=+0.307 (n=521)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3437` → IC=+0.302 (n=230)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3437 (IC base=+0.230)

- **PATRÓN** `volumen_spike_ratio` < `1.7521` → IC=+0.228 (n=633)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7521 (IC base=+0.230)

- **PATRÓN** `volumen_spike_ratio` > `2.184` → IC=+0.232 (n=958)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.184 (IC base=+0.230)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.238 (n=1050)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.230)

- **PATRÓN** `libro_liquidez` > `1900.976` → IC=+0.234 (n=709)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1900.976 (IC base=+0.230)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.227 (n=1370)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 54.0 (IC base=+0.230)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.179 (n=775)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.0041 (IC base=+0.136)

- **PATRÓN** `drift_60min` |x|≤ `0.4387` → IC=+0.145 (n=1759)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.4387 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.151 (n=1841)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.3735` → IC=+0.163 (n=696)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.3735 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.181` → IC=+0.158 (n=727)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` > 4.181 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `0.8733` → IC=+0.154 (n=1173)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.8733 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.2803` → IC=+0.220 (n=244)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2803 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` < `1.5208` → IC=+0.150 (n=750)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.5208 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `2.138` → IC=+0.150 (n=772)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 2.138 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `8031.121` → IC=+0.233 (n=797)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8031.121 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `78.0` → IC=+0.156 (n=544)

  - _Acción_: Kelly boost +0.78€ cuando `ballena_activa_n` < 78.0 (IC base=+0.136)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.160 (n=1259)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0066 (IC base=+0.135)

- **PATRÓN** `drift_60min` |x|≤ `0.4471` → IC=+0.151 (n=1431)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.4471 (IC base=+0.135)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.173 (n=533)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.7057` → IC=+0.180 (n=1431)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` < 0.7057 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` < `0.5969` → IC=+0.139 (n=1583)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.5969 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.146` → IC=+0.178 (n=212)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 11.146 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` < `0.8616` → IC=+0.145 (n=954)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 0.8616 (IC base=+0.135)
...
- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.133 (n=1355)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.488` → IC=+0.127 (n=1633)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 3.488 (IC base=+0.113)

- **PATRÓN** `volumen_regimen` < `0.6498` → IC=+0.155 (n=564)
...
- **PATRÓN** `ibs_20min` > `0.5116` → IC=+0.248 (n=1746)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.208 (n=1314)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.200)

- **PATRÓN** `ibs_20min` < `0.4231` → IC=+0.264 (n=1640)
...
- **PATRÓN** `dist_vwap_pct` < `0.9254` → IC=+0.202 (n=2067)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.9254 (IC base=+0.200)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.144 (n=3139)

- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.167 (n=2655)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0092 (IC base=+0.157)

- **PATRÓN** `drift_60min` |x|≤ `0.52` → IC=+0.166 (n=3017)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.52 (IC base=+0.157)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.172 (n=1192)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9414` → IC=+0.215 (n=1005)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9414 (IC base=+0.157)

- **PATRÓN** `dist_vwap_pct` > `0.1897` → IC=+0.171 (n=1121)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.1897 (IC base=+0.157)

- **PATRÓN** `dist_vwap_pct` < `0.4848` → IC=+0.154 (n=1877)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.4848 (IC base=+0.157)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.192` → IC=+0.192 (n=494)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 10.192 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` > `0.8959` → IC=+0.166 (n=1336)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4563` → IC=+0.165 (n=994)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.4563 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` > `1.8762` → IC=+0.166 (n=1986)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.8762 (IC base=+0.157)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.159 (n=2135)

  - _Acción_: Kelly boost +0.80€ cuando `libro_spread` < 0.01 (IC base=+0.157)

- **PATRÓN** `libro_liquidez` > `3922.395` → IC=+0.166 (n=2010)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 3922.395 (IC base=+0.157)

- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.201 (n=790)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0039 (IC base=+0.142)

- **PATRÓN** `drift_60min` |x|≤ `0.4873` → IC=+0.160 (n=2366)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.4873 (IC base=+0.142)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.174 (n=856)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.1824` → IC=+0.170 (n=1041)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` < 0.1824 (IC base=+0.142)

- **PATRÓN** `dist_vwap_pct` > `0.6795` → IC=+0.166 (n=456)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.6795 (IC base=+0.142)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.265` → IC=+0.153 (n=2356)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 6.265 (IC base=+0.142)

- **PATRÓN** `volumen_regimen` < `1.0974` → IC=+0.152 (n=1992)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.0974 (IC base=+0.142)

- **PATRÓN** `volumen_pendiente_norm` < `0.0966` → IC=+0.146 (n=2146)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4237` → IC=+0.157 (n=779)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 1.4237 (IC base=+0.142)

- **PATRÓN** `volumen_spike_ratio` > `1.8136` → IC=+0.145 (n=1557)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.8136 (IC base=+0.142)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.144 (n=3139)

  - _Acción_: Kelly boost +0.72€ cuando `libro_spread` < 0.01 (IC base=+0.142)

- **PATRÓN** `libro_liquidez` > `7268.8224` → IC=+0.155 (n=2114)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 7268.8224 (IC base=+0.142)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.168 (n=875)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0073 (IC base=+0.165)

- **PATRÓN** `sigma_h` > `0.0043` → IC=+0.172 (n=994)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0043 (IC base=+0.165)

- **PATRÓN** `drift_60min` |x|≤ `0.3869` → IC=+0.167 (n=875)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.3869 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.184 (n=378)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5403` → IC=+0.174 (n=663)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` < 0.5403 (IC base=+0.165)

- **PATRÓN** `ibs_20min` > `0.8859` → IC=+0.177 (n=332)

  - _Acción_: Kelly boost +0.88€ cuando `ibs_20min` > 0.8859 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` < `0.4234` → IC=+0.173 (n=926)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.4234 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.219` → IC=+0.167 (n=166)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 11.219 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.134` → IC=+0.176 (n=892)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` < 4.134 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` < `1.0828` → IC=+0.166 (n=875)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.0828 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` > `0.6393` → IC=+0.168 (n=994)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 0.6393 (IC base=+0.165)

- **PATRÓN** `volumen_pendiente_norm` < `0.1051` → IC=+0.167 (n=916)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4349` → IC=+0.174 (n=326)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.4349 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `1.5194` → IC=+0.168 (n=871)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 1.5194 (IC base=+0.165)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.171 (n=991)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.01 (IC base=+0.165)

- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.190 (n=275)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.0041 (IC base=+0.150)

- **PATRÓN** `drift_60min` |x|≤ `0.497` → IC=+0.175 (n=824)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.497 (IC base=+0.150)
...
- **PATRÓN** `ibs_20min` < `0.748` → IC=+0.154 (n=824)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.748 (IC base=+0.150)

- **PATRÓN** `ibs_20min` > `0.0998` → IC=+0.160 (n=824)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.0998 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.6137` → IC=+0.180 (n=179)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.6137 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.387` → IC=+0.161 (n=747)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` < 4.387 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` < `0.6452` → IC=+0.187 (n=276)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` < 0.6452 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` > `0.721` → IC=+0.153 (n=736)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 0.721 (IC base=+0.150)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1958` → IC=+0.165 (n=711)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.1958 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.7791` → IC=+0.152 (n=538)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.7791 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `7578.204` → IC=+0.175 (n=824)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 7578.204 (IC base=+0.150)

### GBM_LATE_5M#SOL#5min
- **PATRÓN** `ibs_20min` > `0.5455` → IC=+0.153 (n=243)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` > 0.5455 (IC base=+0.104)

- **PATRÓN** `dist_vwap_pct` > `0.2115` → IC=+0.148 (n=174)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.2115 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.177` → IC=+0.192 (n=50)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 9.177 (IC base=+0.104)

- **PATRÓN** `volumen_pendiente_norm` > `0.1597` → IC=+0.210 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1597 (IC base=+0.104)

- **PATRÓN** `volumen_spike_ratio` > `1.4296` → IC=+0.133 (n=235)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` > 1.4296 (IC base=+0.104)

- **PATRÓN** `libro_liquidez` > `3430.0272` → IC=+0.150 (n=218)

  - _Acción_: Kelly boost +0.75€ cuando `libro_liquidez` > 3430.0272 (IC base=+0.104)

- **PATRÓN** `ballena_activa_n` < `59.0` → IC=+0.125 (n=198)

  - _Acción_: Kelly boost +0.62€ cuando `ballena_activa_n` < 59.0 (IC base=+0.104)

- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.153 (n=226)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0069 (IC base=+0.118)

- **PATRÓN** `drift_60min` |x|≤ `0.3998` → IC=+0.158 (n=150)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.3998 (IC base=+0.118)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.154 (n=151)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` < 10.0 (IC base=+0.118)

- **PATRÓN** `ibs_20min` < `0.1538` → IC=+0.214 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1538 (IC base=+0.118)

- **PATRÓN** `dist_vwap_pct` > `0.6027` → IC=+0.209 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6027 (IC base=+0.118)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.67` → IC=+0.129 (n=122)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` > 2.67 (IC base=+0.118)

- **PATRÓN** `volumen_regimen` < `0.9473` → IC=+0.132 (n=150)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_regimen` < 0.9473 (IC base=+0.118)

- **PATRÓN** `volumen_pendiente_norm` < `0.089` → IC=+0.180 (n=173)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` < 0.089 (IC base=+0.118)

- **PATRÓN** `volumen_spike_ratio` < `1.867` → IC=+0.135 (n=146)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 1.867 (IC base=+0.118)

- **PATRÓN** `libro_liquidez` > `3291.3893` → IC=+0.152 (n=225)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 3291.3893 (IC base=+0.118)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.158 (n=188)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 49.0 (IC base=+0.118)
...
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.186 (n=419)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.0041 (IC base=+0.094)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.127 (n=874)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 8.0 (IC base=+0.094)

- **PATRÓN** `ibs_20min` > `0.6667` → IC=+0.201 (n=766)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6667 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` > `0.1571` → IC=+0.160 (n=462)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.1571 (IC base=+0.094)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.45` → IC=+0.190 (n=201)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 11.45 (IC base=+0.094)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0834` → IC=+0.131 (n=654)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` < 2.0834 (IC base=+0.094)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.120 (n=509)

  - _Acción_: Kelly boost +0.60€ cuando `libro_spread` < 0.01 (IC base=+0.094)

- **PATRÓN** `libro_liquidez` > `2424.4386` → IC=+0.137 (n=378)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 2424.4386 (IC base=+0.094)
...
- **PATRÓN** `sigma_h` < `0.006` → IC=+0.160 (n=324)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.006 (IC base=+0.103)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.121 (n=336)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 6.0 (IC base=+0.103)

- **PATRÓN** `ibs_20min` > `0.4823` → IC=+0.181 (n=293)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` > 0.4823 (IC base=+0.103)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1573` → IC=+0.132 (n=270)
  ...
  - _Acción_: Kelly boost +0.66€ cuando `volumen_pendiente_norm` < 0.1573 (IC base=+0.103)

- **PATRÓN** `volumen_spike_ratio` < `2.0118` → IC=+0.164 (n=224)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.0118 (IC base=+0.103)
...
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.188 (n=142)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0041 (IC base=+0.107)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.138 (n=302)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 7.0 (IC base=+0.107)

- **PATRÓN** `ibs_20min` > `0.6632` → IC=+0.233 (n=260)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6632 (IC base=+0.107)

- **PATRÓN** `dist_vwap_pct` > `0.3477` → IC=+0.181 (n=117)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.3477 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.742` → IC=+0.312 (n=67)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.742 (IC base=+0.107)

- **PATRÓN** `volumen_regimen` > `0.9325` → IC=+0.134 (n=132)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` > 0.9325 (IC base=+0.107)

- **PATRÓN** `volumen_pendiente_norm` > `0.2706` → IC=+0.218 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2706 (IC base=+0.107)

- **PATRÓN** `volumen_spike_ratio` < `1.7281` → IC=+0.177 (n=159)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 1.7281 (IC base=+0.107)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.128 (n=194)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.01 (IC base=+0.107)

- **PATRÓN** `libro_liquidez` > `1133.3296` → IC=+0.159 (n=256)
...
- **PATRÓN** `libro_liquidez` > `2928.7278` → IC=+0.168 (n=242)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2928.7278 (IC base=+0.108)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=1851)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.125 (n=62)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=+0.049 (n=765)
...
- **FILTRO** `py_entrada` < `0.47` → IC=-0.122 (n=487)
...
- **PATRÓN** `py_entrada` < `0.47` → IC=+0.165 (n=204)
...
- **PATRÓN** `py_entrada` < `0.6` → IC=+0.160 (n=48)

  - _Acción_: Kelly boost +0.80€ cuando `py_entrada` < 0.6 (IC base=+0.041)

- **PATRÓN** `restante_min` > `13.48` → IC=+0.250 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `restante_min` > 13.48 (IC base=+0.041)

- **PATRÓN** `lag_apertura_s` < `90.81` → IC=+0.231 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `lag_apertura_s` < 90.81 (IC base=+0.041)

- **PATRÓN** `profundidad_ratio` > `9.2` → IC=+0.167 (n=25)

  - _Acción_: Kelly boost +0.83€ cuando `profundidad_ratio` > 9.2 (IC base=+0.041)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=7589)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.166 (n=3610)

  - _Acción_: SKIP cuando `py_entrada` < 0.475
  - _Potencial_: sin este filtro IC_bueno=+0.058 (n=10910)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.165 (n=3740)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.035 (n=11292)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.46` → IC=-0.208 (n=621)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.100 (n=1903)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.47` → IC=-0.193 (n=630)

  - _Acción_: SKIP cuando `py_entrada` < 0.47
  - _Potencial_: sin este filtro IC_bueno=+0.104 (n=1945)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.203 (n=650)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.062 (n=2043)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.485` → IC=-0.176 (n=612)

  - _Acción_: SKIP cuando `py_entrada` < 0.485
  - _Potencial_: sin este filtro IC_bueno=+0.085 (n=1901)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=80)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=2808)

- **FILTRO** `py_entrada` > `0.595` → IC=-0.212 (n=709)

  - _Acción_: SKIP cuando `py_entrada` > 0.595
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=2289)

- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)

  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=2977)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.129 (n=10193)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.079 (n=22978)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.273 (n=8151)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=25020)

- **FILTRO** `ibs_7min` < `0.2801` → IC=-0.233 (n=8291)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2801
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=24880)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.156 (n=11269)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=21902)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.231 (n=10217)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=31454)

- **FILTRO** `ibs_7min` > `0.2929` → IC=-0.178 (n=10416)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2929
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=31255)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.136 (n=1664)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3839)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.309 (n=1308)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4195)

- **FILTRO** `ibs_7min` < `0.7109` → IC=-0.249 (n=1815)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7109
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=3688)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.185 (n=1235)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4268)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.259 (n=1766)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=5396)

- **FILTRO** `drift_7min_pct` |x|> `0.112` → IC=-0.126 (n=2435)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.112
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=4727)

- **FILTRO** `ibs_7min` > `0.7882` → IC=-0.207 (n=1790)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7882
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=5372)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.137 (n=1334)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=4386)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.250 (n=1386)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=4334)

- **FILTRO** `ibs_7min` < `0.7468` → IC=-0.191 (n=1430)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7468
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=4290)

- **FILTRO** `ballena_activa_n` > `161.0` → IC=-0.177 (n=1425)

  - _Acción_: SKIP cuando `ballena_activa_n` > 161.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4295)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.263 (n=1336)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=4475)

- **FILTRO** `ibs_7min` > `0.2609` → IC=-0.179 (n=1452)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2609
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4359)

- **FILTRO** `ballena_activa_n` > `154.0` → IC=-0.182 (n=1449)

  - _Acción_: SKIP cuando `ballena_activa_n` > 154.0
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=4362)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.166 (n=1278)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=3947)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.304 (n=1292)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=3933)

- **FILTRO** `ibs_7min` < `0.7063` → IC=-0.242 (n=1724)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7063
  - _Potencial_: sin este filtro IC_bueno=-0.038 (n=3501)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.222 (n=1195)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4030)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.237 (n=1753)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=5912)

- **FILTRO** `ibs_7min` > `0.7474` → IC=-0.172 (n=1916)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7474
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=5749)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.128 (n=1755)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=3721)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.233 (n=1622)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=3854)

- **FILTRO** `ibs_7min` < `0.7403` → IC=-0.180 (n=1369)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7403
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4107)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.172 (n=1338)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=4138)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.260 (n=1394)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=4187)

- **FILTRO** `ibs_7min` > `0.2747` → IC=-0.179 (n=1395)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2747
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=4186)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.184 (n=1373)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=4208)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.255 (n=1425)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=4348)

- **FILTRO** `ibs_7min` < `0.3` → IC=-0.234 (n=1439)

  - _Acción_: SKIP cuando `ibs_7min` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4334)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.180 (n=1879)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=6049)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.254 (n=1785)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3689)

- **FILTRO** `ibs_7min` < `0.3` → IC=-0.226 (n=1368)

  - _Acción_: SKIP cuando `ibs_7min` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=4106)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.215 (n=1335)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=4139)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.207 (n=1788)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=5736)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.324 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=1099)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.020 (n=2964)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.195 (n=1537)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 6.0 (IC base=+0.185)

- **PATRÓN** `ibs_15` > `0.6053` → IC=+0.268 (n=1654)
...
- **PATRÓN** `dist_vwap_pct` < `0.6244` → IC=+0.179 (n=1571)
...
- **PATRÓN** `libro_liquidez` > `8801.8341` → IC=+0.199 (n=552)
...
- **PATRÓN** `sigma_h` < `0.0065` → IC=+0.141 (n=385)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0065 (IC base=+0.133)

- **PATRÓN** `sigma_h` > `0.0051` → IC=+0.149 (n=257)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` > 0.0051 (IC base=+0.133)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.258` → IC=+0.158 (n=276)

  - _Acción_: Kelly boost +0.79€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.258 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.152 (n=285)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` > `0.659` → IC=+0.251 (n=344)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.659 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `0.1643` → IC=+0.152 (n=303)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.1643 (IC base=+0.133)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.307` → IC=+0.207 (n=165)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.307 (IC base=+0.133)

- **PATRÓN** `libro_liquidez` > `3432.3892` → IC=+0.142 (n=344)

  - _Acción_: Kelly boost +0.71€ cuando `libro_liquidez` > 3432.3892 (IC base=+0.133)
...
- **FILTRO** `sigma_h` > `0.013` → IC=-0.221 (n=671)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.013
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=2016)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.196 (n=916)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=1771)
...
- **FILTRO** `sigma_h` < `0.0034` → IC=-0.231 (n=410)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.188 (n=1231)

- **FILTRO** `hora_utc` > `7.0` → IC=-0.210 (n=1039)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 7.0
...
- **FILTRO** `sigma_ewma_delta_pct` > `19.563` → IC=-0.250 (n=294)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.187 (n=1347)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.238 (n=277)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `12.221` → IC=+0.239 (n=758)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6053 sube el IC de +0.185 a +0.268 en UPDOWN_GBM#15min (n=1654). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.659 sube el IC de +0.133 a +0.251 en UPDOWN_GBM#ETH#15min (n=344). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3482 sube el IC de -0.031 a +0.277 en UPDOWN_GBM_15M_TARDIO (n=1838). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 28757 | -0.088 | -3851.81€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1490 | -0.044 | -221.05€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 27267 | -0.090 | -3630.76€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3785 | -0.099 | -621.37€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3785 | -0.099 | -621.37€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 1490 | -0.044 | -221.05€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 1490 | -0.044 | -221.05€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 3482 | -0.096 | -787.96€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 3482 | -0.096 | -787.96€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 7366 | -0.020 | -689.16€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 7366 | -0.020 | -689.16€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 6979 | -0.090 | -442.00€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 6979 | -0.090 | -442.00€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 5655 | -0.172 | -1090.26€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 5655 | -0.172 | -1090.26€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 18790 | -0.027 | +3984.21€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 4899 | -0.000 | +1810.08€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 13891 | -0.037 | +2174.13€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 18790 | -0.027 | +3984.21€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 4899 | -0.000 | +1810.08€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 13891 | -0.037 | +2174.13€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 94785 | +0.112 | -4790.75€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 14252 | +0.185 | -430.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 381 | -0.080 | -50.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 74018 | +0.100 | -4088.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 6134 | +0.107 | -221.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 12308 | +0.097 | -1046.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 45 | -0.160 | -0.32€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 12248 | +0.099 | -1034.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 19173 | +0.132 | -366.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 4503 | +0.204 | -128.10€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 12272 | +0.111 | -197.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 2356 | +0.103 | -17.83€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 12347 | +0.089 | -1129.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 52 | -0.093 | -6.98€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#DOGE#240min | 15 | -0.243 | -11.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 12280 | +0.091 | -1111.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 20150 | +0.123 | -397.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 5512 | +0.175 | -81.65€ | 1 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 12404 | +0.105 | -249.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 2222 | +0.098 | -57.49€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 18484 | +0.113 | -1116.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 4093 | +0.188 | -221.63€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 284 | -0.038 | +3.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 12551 | +0.090 | -752.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1556 | +0.124 | -146.14€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 12323 | +0.100 | -735.16€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 47 | -0.031 | +8.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 13 | -0.022 | -0.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 12263 | +0.101 | -743.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 15038 | +0.191 | -994.08€ | 1 | 5 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 15038 | +0.191 | -994.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 3573 | +0.167 | -377.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 3573 | +0.167 | -377.09€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC | 1282 | +0.196 | -14.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC#15min | 1282 | +0.196 | -14.03€ | 4 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 3525 | +0.179 | -304.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 3525 | +0.179 | -304.75€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 3123 | +0.239 | -100.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 3123 | +0.239 | -100.45€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 3456 | +0.192 | -211.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 3456 | +0.192 | -211.51€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 51870 | +0.197 | -4063.68€ | 3 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 51870 | +0.197 | -4063.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 8968 | +0.176 | -1039.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 8968 | +0.176 | -1039.59€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 8287 | +0.223 | -305.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 8287 | +0.223 | -305.29€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 