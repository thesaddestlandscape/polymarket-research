# Hipótesis automáticas — 2026-07-02 06:24 UTC
_Generado por shadow_postmortem.py sobre 2543 resoluciones (PNL=-82.69€)_
...
- **PATRÓN** `sigma_h` < `0.0101` → IC=+0.235 (n=66)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0101 (IC base=+0.068)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.167 (n=34)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 15.0 (IC base=+0.068)

- **PATRÓN** `ibs_15` < `0.0714` → IC=+0.233 (n=43)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0714 (IC base=+0.068)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0813` → IC=+0.175 (n=38)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.88€ cuando `pct_spot_vs_ref` |x|≤ 0.0813 (IC base=+0.144)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.206 (n=15)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0058 (IC base=+0.144)

- **PATRÓN** `drift_60min` |x|≤ `1.0585` → IC=+0.176 (n=35)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 1.0585 (IC base=+0.144)

- **PATRÓN** `drift_15min` |x|≤ `0.688` → IC=+0.155 (n=27)

  - _Acción_: Kelly boost +0.78€ cuando `drift_15min` |x|≤ 0.688 (IC base=+0.144)

- **PATRÓN** `ibs_15` < `0.0787` → IC=+0.364 (n=20)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0787 (IC base=+0.144)
...
- **PATRÓN** `sigma_h` < `0.0136` → IC=+0.214 (n=19)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **H-IBS-UPDOWN_GBM#15min**: IBS < 0.0714 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.233 n=43). Confirma señal de reversión media → alinear con BUY_YES.
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.0787 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.364 n=20). Confirma señal de reversión media → alinear con BUY_YES.
...
| ✅ UPDOWN_GBM | 785 | -0.034 | -33.81€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 562 | -0.009 | -7.41€ | 2 | 3 |
...
| ✅ UPDOWN_GBM#ETH | 305 | -0.005 | +9.73€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 222 | +0.013 | +15.69€ | 4 | 5 |
...
| ✅ UPDOWN_GBM#XRP | 68 | +0.014 | +6.36€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 57 | +0.059 | +9.65€ | 1 | 1 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.081) — sin ventaja clara. oversold(IBS<0.3): IC=+0.023 n=128 | neutral: IC=-0.058 n=118 | overbought(IBS>0.7): IC=-0.051 n=136
  - _Datos_: n=395 IC=-0.026 PNL=-8.58€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.003 n=1201 | tras_1loss IC=-0.036 n=1260 | tras_2loss IC=-0.050 n=658/40 | gap=+0.053 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=662 IC=-0.027 PNL=-20.26€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=662 IC=-0.027 PNL=-20.26€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=179 IC=+0.041 PNL=+20.45€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=179 IC=+0.041 PNL=+20.45€
...
**⏳ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: 126/200 ops en el filtro definido (IC actual=+0.047 PNL=+10.23€)
  - _Datos_: n=126 IC=+0.047 PNL=+10.23€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=126 IC=+0.047 PNL=+10.23€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=126 IC=+0.047 PNL=+10.23€
...
**⏳ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: 20/40 ops en el filtro definido (IC actual=+0.045 PNL=+0.14€)
  - _Datos_: n=20 IC=+0.045 PNL=+0.14€
...
**⏳ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: 36/50 ops en el filtro definido (IC actual=+0.132 PNL=+10.66€)
  - _Datos_: n=36 IC=+0.132 PNL=+10.66€
