# Hipótesis automáticas — 2026-09-05 11:13 UTC
_Generado por shadow_postmortem.py sobre 294667 resoluciones (PNL=+28793.43€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `147.29` → IC=-0.301 (n=3948)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 147.29
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=11844)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `129.69` → IC=-0.334 (n=529)
...
- **FILTRO** `restante_s_al_confirmar` < `145.36` → IC=-0.311 (n=878)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 145.36
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=2634)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `159.25` → IC=-0.375 (n=953)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 159.25
  - _Potencial_: sin este filtro IC_bueno=-0.101 (n=1936)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.191 (n=8303)

  - _Acción_: Kelly boost +0.96€ cuando `py_entrada` > 0.7 (IC base=+0.097)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.164 (n=2125)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.097)

- **PATRÓN** `libro_liquidez` > `2361.0666` → IC=+0.166 (n=2038)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 2361.0666 (IC base=+0.097)
...
- **PATRÓN** `py_entrada` > `0.595` → IC=+0.162 (n=300)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.145 (n=1716)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.72€ cuando `hora_utc` > 5.0 (IC base=+0.133)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.139 (n=1455)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.264 (n=662)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.255)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.256 (n=730)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.259 (n=769)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.255)

- **PATRÓN** `libro_liquidez` > `1933.9137` → IC=+0.269 (n=729)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.197 (n=741)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 7.0 (IC base=+0.195)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.196 (n=838)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `939.0669` → IC=+0.197 (n=819)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 939.0669 (IC base=+0.195)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.127 (n=550)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.198 (n=6480)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 5.0 (IC base=+0.192)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.198 (n=5517)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` < 15.0 (IC base=+0.192)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.205 (n=3110)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.73 (IC base=+0.192)

- **PATRÓN** `libro_liquidez` > `5642.183` → IC=+0.308 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 5642.183 (IC base=+0.192)

### FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.179 (n=1597)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` < 17.0 (IC base=+0.167)

- **PATRÓN** `py_entrada` < `0.74` → IC=+0.180 (n=1668)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.193 (n=18204)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` > 8.0 (IC base=+0.191)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.210 (n=18610)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.71 (IC base=+0.191)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.158 (n=3797)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 5.0 (IC base=+0.153)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.155 (n=2613)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` < 12.0 (IC base=+0.153)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.183 (n=2768)
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.273 (n=1823)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.166 (n=2516)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 12.0 (IC base=+0.165)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.184 (n=3232)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.275 (n=1160)
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.240 (n=1584)
...
- **PATRÓN** `lag_apertura_s` < `3.28` → IC=+0.168 (n=1248)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9518` → IC=+0.212 (n=1313)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9518 (IC base=+0.086)

- **PATRÓN** `dist_vwap_pct` < `0.46` → IC=+0.227 (n=841)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.46 (IC base=+0.086)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.681` → IC=+0.157 (n=1506)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 5.681 (IC base=+0.086)

- **PATRÓN** `volumen_regimen` < `0.6973` → IC=+0.230 (n=398)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6973 (IC base=+0.086)

- **PATRÓN** `volumen_regimen` > `1.0799` → IC=+0.245 (n=410)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0799 (IC base=+0.086)

- **PATRÓN** `volumen_pendiente_norm` > `0.1712` → IC=+0.188 (n=690)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.1712 (IC base=+0.086)

- **PATRÓN** `volumen_spike_ratio` < `2.8651` → IC=+0.182 (n=2393)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 2.8651 (IC base=+0.086)

- **PATRÓN** `volumen_spike_ratio` > `1.4734` → IC=+0.178 (n=2393)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 1.4734 (IC base=+0.086)

- **PATRÓN** `ibs_20min` < `0.4021` → IC=+0.125 (n=3765)

  - _Acción_: Kelly boost +0.63€ cuando `ibs_20min` < 0.4021 (IC base=+0.040)

- **PATRÓN** `dist_vwap_pct` < `0.3318` → IC=+0.141 (n=1415)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.3318 (IC base=+0.040)

- **PATRÓN** `volumen_regimen` < `0.8549` → IC=+0.141 (n=906)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` < 0.8549 (IC base=+0.040)

- **PATRÓN** `volumen_regimen` > `1.0483` → IC=+0.142 (n=616)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 1.0483 (IC base=+0.040)

- **PATRÓN** `volumen_pendiente_norm` > `0.2487` → IC=+0.233 (n=418)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2487 (IC base=+0.040)

- **PATRÓN** `volumen_spike_ratio` > `2.8253` → IC=+0.209 (n=699)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8253 (IC base=+0.040)

- **PATRÓN** `ballena_activa_n` < `37.0` → IC=+0.217 (n=830)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 37.0 (IC base=+0.040)

### GBM_LATE_15M#BNB#15min
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.191 (n=296)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0076 (IC base=+0.159)

- **PATRÓN** `drift_60min` |x|≤ `0.2917` → IC=+0.163 (n=884)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.2917 (IC base=+0.159)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.201 (n=430)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.159)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.271 (n=325)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.159)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.72` → IC=+0.298 (n=270)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.72 (IC base=+0.159)
...
- **PATRÓN** `volumen_spike_ratio` < `2.6507` → IC=+0.146 (n=787)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 2.6507 (IC base=+0.159)

- **PATRÓN** `volumen_spike_ratio` > `1.4431` → IC=+0.152 (n=786)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.4431 (IC base=+0.159)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.196 (n=737)

  - _Acción_: Kelly boost +0.98€ cuando `libro_spread` < 0.04 (IC base=+0.159)

- **PATRÓN** `ballena_activa_n` < `43.0` → IC=+0.193 (n=363)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 43.0 (IC base=+0.159)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.253 (n=403)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0057 (IC base=+0.247)

- **PATRÓN** `sigma_h` > `0.005` → IC=+0.263 (n=539)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.005 (IC base=+0.247)

- **PATRÓN** `drift_60min` |x|≤ `0.1777` → IC=+0.290 (n=403)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1777 (IC base=+0.247)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.253 (n=545)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.247)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.254 (n=612)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.247)

- **PATRÓN** `ibs_20min` < `0.4088` → IC=+0.277 (n=531)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4088 (IC base=+0.247)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.215` → IC=+0.264 (n=629)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.215 (IC base=+0.247)

- **PATRÓN** `volumen_pendiente_norm` < `0.069` → IC=+0.245 (n=442)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.069 (IC base=+0.247)

- **PATRÓN** `volumen_pendiente_norm` > `0.2889` → IC=+0.333 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2889 (IC base=+0.247)

- **PATRÓN** `volumen_spike_ratio` > `1.5811` → IC=+0.271 (n=439)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5811 (IC base=+0.247)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.268 (n=282)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.247)

- **PATRÓN** `libro_liquidez` > `1719.82` → IC=+0.265 (n=402)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1719.82 (IC base=+0.247)

- **PATRÓN** `ballena_activa_n` < `45.0` → IC=+0.259 (n=292)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 45.0 (IC base=+0.247)

### GBM_LATE_15M#BTC#15min
- **PATRÓN** `sigma_h` < `0.0029` → IC=+0.259 (n=230)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0029 (IC base=+0.215)

- **PATRÓN** `drift_60min` |x|≤ `0.2092` → IC=+0.236 (n=457)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2092 (IC base=+0.215)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.235 (n=685)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1712` → IC=+0.218 (n=427)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1712 (IC base=+0.215)

- **PATRÓN** `dist_vwap_pct` < `0.4693` → IC=+0.220 (n=630)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4693 (IC base=+0.215)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.217` → IC=+0.230 (n=331)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.217 (IC base=+0.215)

- **PATRÓN** `volumen_regimen` < `1.2495` → IC=+0.219 (n=685)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2495 (IC base=+0.215)

- **PATRÓN** `volumen_regimen` > `1.0826` → IC=+0.232 (n=311)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0826 (IC base=+0.215)

- **PATRÓN** `volumen_pendiente_norm` < `0.0965` → IC=+0.218 (n=650)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0965 (IC base=+0.215)

- **PATRÓN** `volumen_spike_ratio` < `1.4779` → IC=+0.247 (n=291)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4779 (IC base=+0.215)

- **PATRÓN** `libro_liquidez` > `11907.94` → IC=+0.244 (n=612)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11907.94 (IC base=+0.215)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.338` → IC=+0.270 (n=185)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.338 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` < `0.1042` → IC=+0.165 (n=688)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` < 0.1042 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` < `3.8668` → IC=+0.161 (n=751)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 3.8668 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `1.6876` → IC=+0.177 (n=750)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 1.6876 (IC base=+0.168)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.192 (n=880)

  - _Acción_: Kelly boost +0.96€ cuando `libro_spread` < 0.04 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `47.0` → IC=+0.199 (n=522)

  - _Acción_: Kelly boost +0.99€ cuando `ballena_activa_n` < 47.0 (IC base=+0.168)

- **PATRÓN** `sigma_h` < `0.0102` → IC=+0.233 (n=686)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0102 (IC base=+0.227)

- **PATRÓN** `sigma_h` > `0.0064` → IC=+0.230 (n=613)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0064 (IC base=+0.227)

- **PATRÓN** `drift_60min` |x|≤ `0.4907` → IC=+0.228 (n=686)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4907 (IC base=+0.227)
...
- **PATRÓN** `ibs_20min` < `0.03` → IC=+0.275 (n=229)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.03 (IC base=+0.227)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6985` → IC=+0.221 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6985 (IC base=+0.227)

- **PATRÓN** `volumen_spike_ratio` > `3.1631` → IC=+0.220 (n=255)
...
- **PATRÓN** `ballena_activa_n` < `33.0` → IC=+0.215 (n=328)
...
- **PATRÓN** `dist_vwap_pct` > `0.1461` → IC=+0.159 (n=80)
  ...
  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.1461 (IC base=-0.040)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8133` → IC=+0.145 (n=150)
  ...
  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.8133 (IC base=-0.040)
...
- **FILTRO** `drift_60min` |x|> `0.511` → IC=-0.161 (n=373)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=726)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.195 (n=139)

  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.081 (n=960)

- **FILTRO** `ibs_20min` > `0.7885` → IC=-0.196 (n=438)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7885
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=1317)

- **FILTRO** `sigma_ewma_delta_pct` > `8.99` → IC=-0.134 (n=200)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.99
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=1555)
...
- **PATRÓN** `sigma_h` > `0.0093` → IC=+0.172 (n=1628)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0093 (IC base=+0.077)

- **PATRÓN** `ibs_20min` > `0.3039` → IC=+0.147 (n=4870)

  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` > 0.3039 (IC base=+0.077)

- **PATRÓN** `dist_vwap_pct` > `1.2123` → IC=+0.297 (n=347)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.2123 (IC base=+0.077)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.482` → IC=+0.123 (n=2578)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 2.482 (IC base=+0.077)

- **PATRÓN** `volumen_regimen` > `0.682` → IC=+0.222 (n=1418)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.682 (IC base=+0.077)

- **PATRÓN** `volumen_pendiente_norm` > `0.2498` → IC=+0.246 (n=479)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2498 (IC base=+0.077)

- **PATRÓN** `volumen_spike_ratio` < `1.4865` → IC=+0.235 (n=812)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4865 (IC base=+0.077)

- **PATRÓN** `volumen_spike_ratio` > `2.8009` → IC=+0.219 (n=812)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8009 (IC base=+0.077)

- **PATRÓN** `ballena_activa_n` < `100.0` → IC=+0.292 (n=1896)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 100.0 (IC base=+0.077)

- **PATRÓN** `ibs_20min` < `0.5853` → IC=+0.128 (n=4687)
...
- **PATRÓN** `volumen_regimen` < `0.7096` → IC=+0.217 (n=536)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7096 (IC base=+0.051)

- **PATRÓN** `volumen_regimen` > `1.2256` → IC=+0.243 (n=406)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2256 (IC base=+0.051)
...
- **PATRÓN** `ibs_20min` > `0.8402` → IC=+0.237 (n=310)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8402 (IC base=+0.033)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.665` → IC=+0.167 (n=229)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 6.665 (IC base=+0.033)
...
- **FILTRO** `ibs_20min` > `0.8603` → IC=-0.162 (n=403)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.024 (n=1213)

- **PATRÓN** `dist_vwap_pct` > `0.873` → IC=+0.153 (n=93)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.873 (IC base=-0.010)

- **PATRÓN** `volumen_spike_ratio` < `1.6899` → IC=+0.142 (n=241)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.6899 (IC base=-0.010)

- **PATRÓN** `ballena_activa_n` < `290.0` → IC=+0.149 (n=152)

  - _Acción_: Kelly boost +0.75€ cuando `ballena_activa_n` < 290.0 (IC base=-0.010)
...
- **PATRÓN** `sigma_h` > `0.0062` → IC=+0.256 (n=704)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0062 (IC base=+0.228)

- **PATRÓN** `drift_60min` |x|≤ `0.0852` → IC=+0.236 (n=263)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.709` → IC=+0.259 (n=703)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.709 (IC base=+0.228)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.36` → IC=+0.300 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.36 (IC base=+0.228)

- **PATRÓN** `volumen_pendiente_norm` < `0.22` → IC=+0.234 (n=732)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.22 (IC base=+0.228)

- **PATRÓN** `volumen_spike_ratio` < `2.4267` → IC=+0.230 (n=469)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.4267 (IC base=+0.228)

- **PATRÓN** `volumen_spike_ratio` > `1.7205` → IC=+0.233 (n=702)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7205 (IC base=+0.228)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.256 (n=823)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.228)

- **PATRÓN** `sigma_h` > `0.0061` → IC=+0.319 (n=507)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0061 (IC base=+0.295)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.319 (n=395)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.295)

- **PATRÓN** `ibs_20min` < `0.35` → IC=+0.308 (n=567)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6624` → IC=+0.287 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6624 (IC base=+0.295)

- **PATRÓN** `volumen_spike_ratio` > `2.3535` → IC=+0.305 (n=316)
...
- **PATRÓN** `libro_liquidez` > `1846.261` → IC=+0.296 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1846.261 (IC base=+0.295)

- **PATRÓN** `ballena_activa_n` < `30.0` → IC=+0.276 (n=194)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.143 (n=250)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=542)

- **FILTRO** `ibs_20min` < `0.5712` → IC=-0.166 (n=396)
...
- **PATRÓN** `volumen_regimen` < `0.9565` → IC=+0.157 (n=103)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.9565 (IC base=-0.053)

- **PATRÓN** `volumen_pendiente_norm` > `0.1662` → IC=+0.309 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1662 (IC base=-0.053)

- **PATRÓN** `volumen_spike_ratio` < `2.091` → IC=+0.237 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.091 (IC base=-0.053)

- **PATRÓN** `volumen_spike_ratio` > `1.3689` → IC=+0.209 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.3689 (IC base=-0.053)

- **PATRÓN** `ballena_activa_n` < `187.0` → IC=+0.246 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 187.0 (IC base=-0.053)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **FILTRO** `ibs_20min` < `0.8333` → IC=-0.131 (n=698)
...
- **FILTRO** `ibs_20min` > `0.7561` → IC=-0.237 (n=299)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.038 (n=901)

- **FILTRO** `sigma_ewma_delta_pct` > `4.678` → IC=-0.150 (n=301)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.678
  - _Potencial_: sin este filtro IC_bueno=+0.009 (n=899)

- **PATRÓN** `ibs_20min` > `0.8333` → IC=+0.296 (n=360)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8333 (IC base=+0.014)

- **PATRÓN** `dist_vwap_pct` > `1.1776` → IC=+0.348 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1776 (IC base=+0.014)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1117` → IC=+0.260 (n=315)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1117 (IC base=+0.014)

- **PATRÓN** `volumen_pendiente_norm` > `0.2243` → IC=+0.281 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2243 (IC base=+0.014)
...
- **PATRÓN** `volumen_regimen` < `1.2143` → IC=+0.120 (n=127)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_regimen` < 1.2143 (IC base=-0.031)

- **PATRÓN** `volumen_pendiente_norm` > `0.2973` → IC=+0.289 (n=17)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2973 (IC base=-0.031)

- **PATRÓN** `ballena_activa_n` < `65.0` → IC=+0.170 (n=92)

  - _Acción_: Kelly boost +0.85€ cuando `ballena_activa_n` < 65.0 (IC base=-0.031)
...
- **PATRÓN** `sigma_h` > `0.0129` → IC=+0.283 (n=557)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0129 (IC base=+0.270)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.278 (n=790)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.521` → IC=+0.283 (n=233)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.521 (IC base=+0.270)

- **PATRÓN** `dist_vwap_pct` < `0.2762` → IC=+0.272 (n=793)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2762 (IC base=+0.270)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.402` → IC=+0.296 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.402 (IC base=+0.270)

- **PATRÓN** `volumen_regimen` > `1.2604` → IC=+0.304 (n=279)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2604 (IC base=+0.270)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5884` → IC=+0.259 (n=679)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5884 (IC base=+0.270)

- **PATRÓN** `volumen_spike_ratio` > `2.1963` → IC=+0.281 (n=308)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1963 (IC base=+0.270)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.270 (n=959)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.270)

- **PATRÓN** `libro_liquidez` > `2547.9574` → IC=+0.271 (n=557)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2547.9574 (IC base=+0.270)

- **PATRÓN** `ballena_activa_n` < `41.0` → IC=+0.265 (n=584)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 41.0 (IC base=+0.270)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` > `0.0104` → IC=+0.208 (n=1366)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0104 (IC base=+0.164)

- **PATRÓN** `drift_60min` |x|≤ `0.3382` → IC=+0.171 (n=3605)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.3382 (IC base=+0.164)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.175 (n=4319)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 5.0 (IC base=+0.164)

- **PATRÓN** `ibs_20min` > `0.7013` → IC=+0.224 (n=3659)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.092` → IC=+0.253 (n=857)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.092 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` > `0.6308` → IC=+0.162 (n=2762)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.6308 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.1055` → IC=+0.187 (n=1549)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.1055 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` < `2.3183` → IC=+0.166 (n=3367)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.3183 (IC base=+0.164)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.164 (n=4172)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.03 (IC base=+0.164)

- **PATRÓN** `libro_liquidez` > `3903.2039` → IC=+0.174 (n=1366)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 3903.2039 (IC base=+0.164)

- **PATRÓN** `ballena_activa_n` < `140.0` → IC=+0.182 (n=3004)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 140.0 (IC base=+0.164)

- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.180 (n=3365)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.0083 (IC base=+0.171)

- **PATRÓN** `drift_60min` |x|≤ `0.0781` → IC=+0.203 (n=1275)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.4409` → IC=+0.222 (n=3824)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4409 (IC base=+0.171)

- **PATRÓN** `dist_vwap_pct` < `0.2201` → IC=+0.164 (n=2893)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.2201 (IC base=+0.171)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.075` → IC=+0.205 (n=682)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.075 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` < `1.1737` → IC=+0.160 (n=2875)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.1737 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` > `0.623` → IC=+0.154 (n=2875)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 0.623 (IC base=+0.171)

- **PATRÓN** `volumen_pendiente_norm` > `0.2945` → IC=+0.243 (n=520)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2945 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` > `2.6561` → IC=+0.194 (n=1065)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 2.6561 (IC base=+0.171)

- **PATRÓN** `ballena_activa_n` < `155.0` → IC=+0.166 (n=2785)

  - _Acción_: Kelly boost +0.83€ cuando `ballena_activa_n` < 155.0 (IC base=+0.171)

### GBM_LATE_15M_MULTIHORIZONTE#BNB#15min
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.184 (n=308)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2273` → IC=+0.266 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2273 (IC base=+0.180)

- **PATRÓN** `volumen_spike_ratio` < `2.6388` → IC=+0.170 (n=607)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` < 2.6388 (IC base=+0.180)

- **PATRÓN** `volumen_spike_ratio` > `1.459` → IC=+0.174 (n=606)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 1.459 (IC base=+0.180)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.214 (n=583)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.261 (n=392)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0048 (IC base=+0.234)

- **PATRÓN** `drift_60min` |x|≤ `0.1661` → IC=+0.302 (n=291)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1661 (IC base=+0.234)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.234 (n=392)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.234)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.252 (n=441)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.234)

- **PATRÓN** `ibs_20min` < `0.2712` → IC=+0.259 (n=384)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2712 (IC base=+0.234)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.702` → IC=+0.249 (n=476)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 4.702 (IC base=+0.234)

- **PATRÓN** `volumen_pendiente_norm` < `0.0683` → IC=+0.227 (n=317)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0683 (IC base=+0.234)

- **PATRÓN** `volumen_pendiente_norm` > `0.2958` → IC=+0.297 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2958 (IC base=+0.234)

- **PATRÓN** `volumen_spike_ratio` < `1.9106` → IC=+0.225 (n=245)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9106 (IC base=+0.234)

- **PATRÓN** `volumen_spike_ratio` > `2.7352` → IC=+0.252 (n=123)
...
- **PATRÓN** `libro_liquidez` > `1703.72` → IC=+0.268 (n=291)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1703.72 (IC base=+0.234)

- **PATRÓN** `ballena_activa_n` < `64.0` → IC=+0.230 (n=291)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 64.0 (IC base=+0.234)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1777` → IC=+0.198 (n=431)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.1777 (IC base=+0.176)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.546` → IC=+0.308 (n=149)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.546 (IC base=+0.176)

- **PATRÓN** `volumen_pendiente_norm` > `0.1321` → IC=+0.189 (n=242)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.1321 (IC base=+0.176)

- **PATRÓN** `volumen_spike_ratio` < `3.1497` → IC=+0.171 (n=512)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 3.1497 (IC base=+0.176)

- **PATRÓN** `volumen_spike_ratio` > `3.8745` → IC=+0.173 (n=194)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 3.8745 (IC base=+0.176)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.200 (n=667)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.176)

- **PATRÓN** `ballena_activa_n` < `18.0` → IC=+0.194 (n=132)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 18.0 (IC base=+0.176)

- **PATRÓN** `sigma_h` < `0.0103` → IC=+0.234 (n=514)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0103 (IC base=+0.220)

- **PATRÓN** `drift_60min` |x|≤ `0.217` → IC=+0.228 (n=343)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.217 (IC base=+0.220)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.250 (n=342)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.220)

- **PATRÓN** `ibs_20min` < `0.3724` → IC=+0.254 (n=514)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3724 (IC base=+0.220)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.703` → IC=+0.261 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.703 (IC base=+0.220)

- **PATRÓN** `volumen_pendiente_norm` > `0.3709` → IC=+0.277 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3709 (IC base=+0.220)

- **PATRÓN** `volumen_spike_ratio` < `1.9277` → IC=+0.203 (n=193)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9277 (IC base=+0.220)

- **PATRÓN** `volumen_spike_ratio` > `3.101` → IC=+0.226 (n=199)
...
- **PATRÓN** `sigma_h` < `0.0076` → IC=+0.151 (n=682)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0076 (IC base=+0.125)

- **PATRÓN** `drift_60min` |x|≤ `0.3788` → IC=+0.143 (n=682)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.3788 (IC base=+0.125)
...
- **PATRÓN** `ibs_20min` < `0.5432` → IC=+0.175 (n=682)

  - _Acción_: Kelly boost +0.88€ cuando `ibs_20min` < 0.5432 (IC base=+0.125)

- **PATRÓN** `dist_vwap_pct` < `0.6019` → IC=+0.139 (n=799)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.6019 (IC base=+0.125)
...
- **PATRÓN** `volumen_regimen` < `1.1639` → IC=+0.133 (n=682)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 1.1639 (IC base=+0.125)

- **PATRÓN** `volumen_regimen` > `0.6135` → IC=+0.131 (n=681)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_regimen` > 0.6135 (IC base=+0.125)

- **PATRÓN** `volumen_pendiente_norm` > `0.1023` → IC=+0.168 (n=230)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_pendiente_norm` > 0.1023 (IC base=+0.125)
...
- **PATRÓN** `sigma_h` > `0.0103` → IC=+0.181 (n=352)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0103 (IC base=+0.115)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.134 (n=809)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 5.0 (IC base=+0.115)

- **PATRÓN** `ibs_20min` > `0.5294` → IC=+0.192 (n=775)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` > 0.5294 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` > `1.1251` → IC=+0.253 (n=176)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1251 (IC base=+0.115)
...
- **PATRÓN** `volumen_regimen` > `0.6308` → IC=+0.124 (n=775)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_regimen` > 0.6308 (IC base=+0.115)

- **PATRÓN** `volumen_pendiente_norm` < `0.1695` → IC=+0.133 (n=775)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4421` → IC=+0.143 (n=247)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.4421 (IC base=+0.115)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.120 (n=780)
...
- **PATRÓN** `sigma_h` > `0.0106` → IC=+0.154 (n=235)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0106 (IC base=+0.121)

- **PATRÓN** `drift_60min` |x|≤ `0.0982` → IC=+0.139 (n=236)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.69€ cuando `drift_60min` |x|≤ 0.0982 (IC base=+0.121)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.169 (n=358)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 14.0 (IC base=+0.121)

- **PATRÓN** `ibs_20min` < `0.4737` → IC=+0.213 (n=706)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4737 (IC base=+0.121)

- **PATRÓN** `dist_vwap_pct` < `0.1758` → IC=+0.136 (n=640)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.1758 (IC base=+0.121)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.287` → IC=+0.148 (n=282)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 3.287 (IC base=+0.121)

- **PATRÓN** `volumen_regimen` < `1.18` → IC=+0.136 (n=704)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 1.18 (IC base=+0.121)

- **PATRÓN** `volumen_regimen` > `0.8584` → IC=+0.129 (n=469)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_regimen` > 0.8584 (IC base=+0.121)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3851` → IC=+0.182 (n=190)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 2.3851 (IC base=+0.121)

- **PATRÓN** `libro_liquidez` > `3184.3475` → IC=+0.158 (n=235)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 3184.3475 (IC base=+0.121)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0897` → IC=+0.223 (n=265)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0897 (IC base=+0.199)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.225 (n=373)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.199)

- **PATRÓN** `ibs_20min` < `0.4224` → IC=+0.245 (n=795)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4224 (IC base=+0.199)

- **PATRÓN** `dist_vwap_pct` < `0.2511` → IC=+0.205 (n=819)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2511 (IC base=+0.199)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.959` → IC=+0.245 (n=159)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.959 (IC base=+0.199)

- **PATRÓN** `volumen_regimen` > `0.6915` → IC=+0.225 (n=710)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6915 (IC base=+0.199)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6744` → IC=+0.204 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6744 (IC base=+0.199)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.202 (n=725)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.199)

- **PATRÓN** `libro_liquidez` > `2514.0359` → IC=+0.207 (n=530)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2514.0359 (IC base=+0.199)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.170 (n=834)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 8.0 (IC base=+0.141)

- **PATRÓN** `ibs_20min` > `0.4429` → IC=+0.169 (n=885)

  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` > 0.4429 (IC base=+0.141)
...
- **PATRÓN** `volumen_regimen` > `1.1694` → IC=+0.146 (n=238)
  ...
  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 1.1694 (IC base=+0.141)
...
- **PATRÓN** `volumen_spike_ratio` < `2.2547` → IC=+0.150 (n=730)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 2.2547 (IC base=+0.141)
...
- **PATRÓN** `ibs_20min` < `0.3111` → IC=+0.123 (n=601)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1733` → IC=+0.164 (n=224)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.1733 (IC base=+0.075)
...
- **PATRÓN** `sigma_h` < `0.0051` → IC=+0.281 (n=158)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0051 (IC base=+0.263)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0948` → IC=+0.265 (n=79)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0948 (IC base=+0.263)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.298 (n=246)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.263)

- **PATRÓN** `ibs_20min` > `0.9229` → IC=+0.317 (n=107)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9229 (IC base=+0.263)

- **PATRÓN** `dist_vwap_pct` > `0.15` → IC=+0.284 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.15 (IC base=+0.263)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.169` → IC=+0.304 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.169 (IC base=+0.263)

- **PATRÓN** `volumen_regimen` < `0.8258` → IC=+0.281 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8258 (IC base=+0.263)

- **PATRÓN** `volumen_regimen` > `1.1544` → IC=+0.315 (n=79)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1544 (IC base=+0.263)

- **PATRÓN** `volumen_pendiente_norm` > `0.0968` → IC=+0.353 (n=100)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0968 (IC base=+0.263)

- **PATRÓN** `volumen_spike_ratio` < `1.368` → IC=+0.287 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.368 (IC base=+0.263)

- **PATRÓN** `volumen_spike_ratio` > `2.0629` → IC=+0.332 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.0629 (IC base=+0.263)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5851` → IC=+0.196 (n=324)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 1.5851 (IC base=+0.190)

- **PATRÓN** `volumen_spike_ratio` > `1.886` → IC=+0.177 (n=490)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 1.886 (IC base=+0.190)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.236 (n=666)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.190)

- **PATRÓN** `ballena_activa_n` < `80.0` → IC=+0.241 (n=484)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1882` → IC=+0.295 (n=408)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1882 (IC base=+0.260)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.267 (n=552)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.260)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.267 (n=624)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.260)

- **PATRÓN** `ibs_20min` < `0.4113` → IC=+0.298 (n=538)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4113 (IC base=+0.260)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.535` → IC=+0.276 (n=618)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.535 (IC base=+0.260)

- **PATRÓN** `volumen_pendiente_norm` > `0.2958` → IC=+0.312 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2958 (IC base=+0.260)

- **PATRÓN** `volumen_spike_ratio` > `1.5076` → IC=+0.275 (n=508)
...
- **PATRÓN** `libro_liquidez` > `1730.44` → IC=+0.271 (n=408)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1730.44 (IC base=+0.260)

- **PATRÓN** `ballena_activa_n` < `71.0` → IC=+0.259 (n=396)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 71.0 (IC base=+0.260)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.003` → IC=+0.176 (n=270)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` < 0.003 (IC base=+0.157)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.174 (n=848)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 5.0 (IC base=+0.157)

- **PATRÓN** `ibs_20min` > `0.3271` → IC=+0.203 (n=810)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3271 (IC base=+0.157)

- **PATRÓN** `dist_vwap_pct` > `0.2276` → IC=+0.215 (n=450)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2276 (IC base=+0.157)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.33` → IC=+0.164 (n=707)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` < 4.33 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` < `1.261` → IC=+0.165 (n=809)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.261 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` > `1.0975` → IC=+0.159 (n=367)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 1.0975 (IC base=+0.157)

- **PATRÓN** `volumen_pendiente_norm` < `0.0705` → IC=+0.166 (n=687)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` < 0.0705 (IC base=+0.157)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3909` → IC=+0.173 (n=760)
...
- **PATRÓN** `libro_liquidez` > `10679.1014` → IC=+0.183 (n=723)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 10679.1014 (IC base=+0.157)
...
- **PATRÓN** `sigma_h` > `0.0059` → IC=+0.223 (n=726)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.6809` → IC=+0.255 (n=646)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6809 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.274` → IC=+0.322 (n=228)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.274 (IC base=+0.209)

- **PATRÓN** `volumen_pendiente_norm` < `0.2198` → IC=+0.214 (n=659)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2198 (IC base=+0.209)

- **PATRÓN** `volumen_spike_ratio` < `3.2048` → IC=+0.209 (n=571)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.2048 (IC base=+0.209)

- **PATRÓN** `volumen_spike_ratio` > `2.4495` → IC=+0.203 (n=433)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4495 (IC base=+0.209)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.237 (n=752)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.209)

- **PATRÓN** `libro_liquidez` > `1467.8784` → IC=+0.212 (n=723)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1467.8784 (IC base=+0.209)

- **PATRÓN** `ballena_activa_n` < `65.0` → IC=+0.255 (n=442)
...
- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.248 (n=327)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0089 (IC base=+0.235)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.237 (n=492)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.4` → IC=+0.280 (n=638)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3603` → IC=+0.286 (n=96)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3603 (IC base=+0.235)

- **PATRÓN** `volumen_spike_ratio` < `3.0645` → IC=+0.231 (n=526)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.0645 (IC base=+0.235)

- **PATRÓN** `volumen_spike_ratio` > `2.3399` → IC=+0.225 (n=398)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3399 (IC base=+0.235)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.239 (n=393)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.235)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.213 (n=455)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 54.0 (IC base=+0.235)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0069` → IC=+0.158 (n=724)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` < 0.0069 (IC base=+0.141)

- **PATRÓN** `drift_60min` |x|≤ `0.2325` → IC=+0.144 (n=549)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.313` → IC=+0.172 (n=370)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 4.313 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` < `0.8962` → IC=+0.170 (n=549)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 0.8962 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` > `1.2022` → IC=+0.145 (n=274)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 1.2022 (IC base=+0.141)
...
- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.179 (n=870)

  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` > 0.4706 (IC base=+0.095)

- **PATRÓN** `dist_vwap_pct` > `0.8687` → IC=+0.180 (n=217)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` > 0.8687 (IC base=+0.095)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.33` → IC=+0.218 (n=324)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.33 (IC base=+0.095)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.125 (n=601)

  - _Acción_: Kelly boost +0.63€ cuando `libro_spread` < 0.01 (IC base=+0.095)

- **PATRÓN** `libro_liquidez` > `2951.9012` → IC=+0.253 (n=290)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2951.9012 (IC base=+0.095)

- **PATRÓN** `ballena_activa_n` < `33.0` → IC=+0.159 (n=215)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 33.0 (IC base=+0.095)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.172 (n=272)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.0057 (IC base=+0.114)

- **PATRÓN** `drift_60min` |x|≤ `0.1276` → IC=+0.172 (n=272)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.1276 (IC base=+0.114)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.147 (n=389)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` > 15.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` < `0.5882` → IC=+0.193 (n=816)

  - _Acción_: Kelly boost +0.97€ cuando `ibs_20min` < 0.5882 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` < `0.4927` → IC=+0.137 (n=789)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.4927 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.218` → IC=+0.131 (n=795)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` < 3.218 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` < `1.0627` → IC=+0.129 (n=718)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 1.0627 (IC base=+0.114)

- **PATRÓN** `volumen_pendiente_norm` > `0.0737` → IC=+0.167 (n=277)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.0737 (IC base=+0.114)

- **PATRÓN** `volumen_spike_ratio` < `1.5628` → IC=+0.137 (n=293)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` < 1.5628 (IC base=+0.114)

- **PATRÓN** `volumen_spike_ratio` > `2.1831` → IC=+0.135 (n=302)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 2.1831 (IC base=+0.114)

- **PATRÓN** `libro_liquidez` > `2654.6046` → IC=+0.153 (n=370)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 2654.6046 (IC base=+0.114)
...
- **PATRÓN** `sigma_h` > `0.0253` → IC=+0.218 (n=342)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0253 (IC base=+0.203)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.206 (n=962)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.203)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.208 (n=1082)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.203)

- **PATRÓN** `ibs_20min` < `0.5` → IC=+0.254 (n=1036)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5 (IC base=+0.203)

- **PATRÓN** `dist_vwap_pct` < `0.1777` → IC=+0.209 (n=897)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1777 (IC base=+0.203)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.376` → IC=+0.274 (n=144)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.376 (IC base=+0.203)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4567` → IC=+0.192 (n=849)
...
- **PATRÓN** `libro_liquidez` > `2531.064` → IC=+0.203 (n=682)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2531.064 (IC base=+0.203)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.181 (n=745)
...
- **PATRÓN** `sigma_h` < `0.01` → IC=+0.135 (n=1397)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.01 (IC base=+0.121)

- **PATRÓN** `drift_60min` |x|≤ `0.2771` → IC=+0.140 (n=1057)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.9259` → IC=+0.189 (n=529)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8992` → IC=+0.141 (n=1046)
...
- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.195 (n=257)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0056 (IC base=+0.099)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.185 (n=141)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` > 18.0 (IC base=+0.099)

- **PATRÓN** `ibs_20min` > `0.6471` → IC=+0.222 (n=293)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6471 (IC base=+0.099)

- **PATRÓN** `dist_vwap_pct` > `0.8416` → IC=+0.262 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8416 (IC base=+0.099)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.672` → IC=+0.232 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.672 (IC base=+0.099)

- **PATRÓN** `volumen_regimen` < `0.8279` → IC=+0.162 (n=196)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.8279 (IC base=+0.099)

- **PATRÓN** `volumen_regimen` > `0.5821` → IC=+0.131 (n=293)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` > 0.5821 (IC base=+0.099)

- **PATRÓN** `volumen_pendiente_norm` > `0.2695` → IC=+0.295 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2695 (IC base=+0.099)

- **PATRÓN** `volumen_spike_ratio` < `1.9736` → IC=+0.169 (n=170)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.9736 (IC base=+0.099)

- **PATRÓN** `volumen_spike_ratio` > `1.4893` → IC=+0.186 (n=173)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` > 1.4893 (IC base=+0.099)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.164 (n=239)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.02 (IC base=+0.099)

- **PATRÓN** `libro_liquidez` > `1136.0742` → IC=+0.169 (n=240)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 1136.0742 (IC base=+0.099)

- **PATRÓN** `ibs_20min` < `0.0952` → IC=+0.204 (n=42)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0952 (IC base=-0.116)

- **PATRÓN** `volumen_pendiente_norm` > `0.1363` → IC=+0.167 (n=16)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.1363 (IC base=-0.116)
...
- **PATRÓN** `sigma_h` < `0.005` → IC=+0.186 (n=100)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.005 (IC base=+0.117)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.171 (n=77)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 15.0 (IC base=+0.117)

- **PATRÓN** `ibs_20min` > `0.6839` → IC=+0.278 (n=106)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6839 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` > `0.49` → IC=+0.278 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.49 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` < `0.1427` → IC=+0.157 (n=106)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.1427 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.185` → IC=+0.285 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.185 (IC base=+0.117)

- **PATRÓN** `volumen_regimen` < `0.8058` → IC=+0.171 (n=80)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 0.8058 (IC base=+0.117)

- **PATRÓN** `volumen_pendiente_norm` < `0.1419` → IC=+0.186 (n=68)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` < 0.1419 (IC base=+0.117)

- **PATRÓN** `volumen_pendiente_norm` > `0.2254` → IC=+0.196 (n=21)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.2254 (IC base=+0.117)

- **PATRÓN** `volumen_spike_ratio` < `1.7889` → IC=+0.160 (n=48)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 1.7889 (IC base=+0.117)

- **PATRÓN** `volumen_spike_ratio` > `1.4512` → IC=+0.189 (n=72)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` > 1.4512 (IC base=+0.117)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.177 (n=125)

  - _Acción_: Kelly boost +0.89€ cuando `libro_spread` < 0.02 (IC base=+0.117)

- **PATRÓN** `libro_liquidez` > `1101.9191` → IC=+0.207 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1101.9191 (IC base=+0.117)
...
### LEADLAG_BTC_XRP_15M
- **PATRÓN** `libro_liquidez` > `2766.4535` → IC=+0.189 (n=101)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 2766.4535 (IC base=+0.097)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.174 (n=1583)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.031 (n=4796)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.185 (n=1584)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.006 (n=5007)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.41` → IC=-0.228 (n=244)

  - _Acción_: SKIP cuando `py_entrada` < 0.41
  - _Potencial_: sin este filtro IC_bueno=+0.065 (n=763)

- **FILTRO** `ibs_20min` < `0.7372` → IC=-0.198 (n=250)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7372
  - _Potencial_: sin este filtro IC_bueno=+0.057 (n=757)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.167 (n=283)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=871)

### MOMENTUM_IBS_15M_BALLENA#BTC#15min
- **FILTRO** `py_entrada` > `0.56` → IC=-0.180 (n=254)

  - _Acción_: SKIP cuando `py_entrada` > 0.56
  - _Potencial_: sin este filtro IC_bueno=+0.005 (n=780)
...
- **FILTRO** `py_entrada` > `0.64` → IC=-0.218 (n=278)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.028 (n=848)
...
- **FILTRO** `py_entrada` < `0.495` → IC=-0.141 (n=285)
...
- **FILTRO** `py_entrada` > `0.61` → IC=-0.157 (n=269)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=837)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.46` → IC=-0.194 (n=240)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.046 (n=753)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=978)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.086 (n=530)
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.280 (n=3761)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=11658)

- **FILTRO** `ibs_7min` < `0.7143` → IC=-0.240 (n=3822)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=11597)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.169 (n=5187)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=10232)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.222 (n=4748)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=14715)

- **FILTRO** `ibs_7min` > `0.7143` → IC=-0.172 (n=4851)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=14612)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.308 (n=550)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=1723)

- **FILTRO** `ibs_7min` < `0.7037` → IC=-0.254 (n=749)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7037
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=1524)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.218 (n=545)

  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=1728)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.240 (n=840)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=2538)

- **FILTRO** `drift_7min_pct` |x|> `0.1134` → IC=-0.129 (n=1147)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1134
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=2231)

- **FILTRO** `ibs_7min` > `0.837` → IC=-0.197 (n=843)

  - _Acción_: SKIP cuando `ibs_7min` > 0.837
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=2535)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.137 (n=635)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=2213)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.260 (n=689)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=2159)

- **FILTRO** `ibs_7min` < `0.7722` → IC=-0.193 (n=712)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7722
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=2136)

- **FILTRO** `ballena_activa_n` > `167.0` → IC=-0.177 (n=710)

  - _Acción_: SKIP cuando `ballena_activa_n` > 167.0
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=2138)

- **FILTRO** `py_entrada` > `0.64` → IC=-0.246 (n=687)

  - _Acción_: SKIP cuando `py_entrada` > 0.64
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=2179)

- **FILTRO** `ballena_activa_n` > `108.0` → IC=-0.173 (n=967)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.183 (n=740)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.100 (n=1535)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.305 (n=721)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=1554)

- **FILTRO** `ibs_7min` < `0.2167` → IC=-0.289 (n=568)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2167
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=1707)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.238 (n=544)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.092 (n=1731)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.223 (n=827)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=2692)

- **FILTRO** `ibs_7min` > `0.2785` → IC=-0.154 (n=1196)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2785
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=2323)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.141 (n=808)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.093 (n=1796)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.264 (n=625)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=1979)

- **FILTRO** `ibs_7min` < `0.7555` → IC=-0.192 (n=651)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7555
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=1953)

- **FILTRO** `ballena_activa_n` > `37.0` → IC=-0.185 (n=648)

  - _Acción_: SKIP cuando `ballena_activa_n` > 37.0
  - _Potencial_: sin este filtro IC_bueno=-0.082 (n=1956)
...
- **FILTRO** `ibs_7min` > `0.2764` → IC=-0.179 (n=655)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2764
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=1969)

- **FILTRO** `ballena_activa_n` > `33.0` → IC=-0.185 (n=655)

  - _Acción_: SKIP cuando `ballena_activa_n` > 33.0
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=1969)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.39` → IC=-0.244 (n=701)

  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=2189)
...
- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.182 (n=709)

  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=2181)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.178 (n=865)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=2786)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.133 (n=719)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=1810)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.293 (n=617)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=1912)

- **FILTRO** `ibs_7min` < `0.7391` → IC=-0.232 (n=632)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7391
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=1897)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.227 (n=616)

  - _Acción_: SKIP cuando `ballena_activa_n` > 13.0
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=1913)
...
- **FILTRO** `ibs_7min` > `0.7634` → IC=-0.155 (n=856)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7634
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=2569)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.141 (n=430)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` > 6.0 (IC base=+0.131)

- **PATRÓN** `total_vol_5m` < `459.6089` → IC=+0.169 (n=173)

  - _Acción_: Kelly boost +0.84€ cuando `total_vol_5m` < 459.6089 (IC base=+0.131)
...
- **PATRÓN** `libro_liquidez` > `3304.2559` → IC=+0.154 (n=212)
  ...
  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 3304.2559 (IC base=+0.131)

- **PATRÓN** `ballena_activa_n` < `63.0` → IC=+0
04€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL | 2180 | -0.034 | +86.82€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL#15min | 2180 | -0.034 | +86.82€ | 2 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP | 2120 | -0.025 | +81.82€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#XRP#15min | 2120 | -0.025 | +81.82€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE | 768 | -0.087 | -48.08€ | 4 | 0 |
| ✅ MOMENTUM_IBS_15M_FADE#15min | 768 | -0.087 | -48.08€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_5M_BALLENA | 34882 | -0.077 | +588.83€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 34882 | -0.077 | +588.83€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 5651 | -0.088 | +445.49€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 5651 | -0.088 | +445.49€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 5714 | -0.080 | -157.30€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 5714 | -0.080 | -157.30€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 5794 | -0.083 | +195.66€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 5794 | -0.083 | +195.66€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 5228 | -0.101 | -306.90€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 5228 | -0.101 | -306.90€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 6541 | -0.053 | +117.07€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 6541 | -0.053 | +117.07€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 5954 | -0.064 | +294.82€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 5954 | -0.064 | +294.82€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE | 6283 | -0.014 | -112.29€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 6283 | -0.014 | -112.29€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_5M_FADE#ETH | 1364 | -0.004 | -9.08€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH#5min | 1364 | -0.004 | -9.08€ | 1 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL | 855 | -0.020 | -12.75€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL#5min | 855 | -0.020 | -12.75€ | 1 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP | 736 | -0.020 | -23.66€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP#5min | 736 | -0.020 | -23.66€ | 0 | 0 |
| ✅ ORDER_FLOW_5M | 757 | +0.115 | +260.07€ | 1 | 6 |
| ✅ ORDER_FLOW_5M#5min | 621 | +0.128 | +247.48€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#BNB | 145 | +0.119 | +60.26€ | 0 | 0 |
| ✅ ORDER_FLOW_5M#BNB#5min | 145 | +0.119 | +60.26€ | 0 | 1 |
...
| ✅ STREAK_MOM_5M | 4083 | +0.018 | +48.06€ | 0 | 0 |
| ✅ STREAK_MOM_5M#5min | 4083 | +0.018 | +48.06€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE | 1329 | +0.019 | +9.55€ | 0 | 0 |
| ✅ STREAK_MOM_5M#DOGE#5min | 1329 | +0.019 | +9.55€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH | 829 | +0.037 | +32.32€ | 0 | 0 |
| ✅ STREAK_MOM_5M#ETH#5min | 829 | +0.037 | +32.32€ | 1 | 1 |
| ✅ STREAK_MOM_5M#SOL | 1205 | +0.006 | -7.65€ | 0 | 0 |
| ✅ STREAK_MOM_5M#SOL#5min | 1205 | +0.006 | -7.65€ | 1 | 0 |
| ✅ STREAK_MOM_5M#XRP | 720 | +0.017 | +13.83€ | 0 | 0 |
| ✅ STREAK_MOM_5M#XRP#5min | 720 | +0.017 | +13.83€ | 2 | 0 |
| ✅ STRUCT_NO_15M | 4349 | +0.012 | -24.28€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 4349 | +0.012 | -24.28€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 1668 | +0.012 | -11.23€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 1668 | +0.012 | -11.23€ | 1 | 0 |
| ✅ STRUCT_NO_15M#ETH | 1732 | +0.021 | +6.01€ | 0 | 0 |
| ✅ STRUCT_NO_15M#ETH#15min | 1732 | +0.021 | +6.01€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL | 949 | -0.005 | -19.06€ | 0 | 0 |
| ✅ STRUCT_NO_15M#SOL#15min | 949 | -0.005 | -19.06€ | 2 | 0 |
| ✅ UPDOWN_GBM | 10996 | +0.011 | +371.03€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 3771 | +0.040 | +409.77€ | 0 | 13 |
| ✅ UPDOWN_GBM#240min | 461 | +0.008 | +6.77€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 6020 | -0.002 | -34.62€ | 1 | 0 |
| ✅ UPDOWN_GBM#60min | 692 | -0.010 | -9.97€ | 2 | 0 |
| ✅ UPDOWN_GBM#BNB | 520 | +0.073 | +59.13€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 186 | +0.117 | +44.98€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#240min | 11 | -0.021 | -0.49€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#5min | 323 | +0.051 | +14.64€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.082) — sin ventaja clara. oversold(IBS<0.3): IC=+0.027 n=3887 | neutral: IC=+0.005 n=4276 | overbought(IBS>0.7): IC=+0.088 n=4282
  - _Datos_: n=12927 IC=+0.042 PNL=+1358.08€

**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  - _Umbral_: n≥40 por celda + gate riguroso completo (Wilson+shuffle+PnL bootstrap)
  - _Acción_: Añadir claves 'ESTRATEGIA#SUBTYPE#DIRECCION#HORA':1.2 a meta.hora_boost_factor, solo por celda confirmada
  - _Estado_: 223 celda(s) pasan gate riguroso completo de 1461 evaluadas (n>=40) y 2529 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json
...
**⏳ H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  ...
  - _Estado_: 10934 ops, 22 horas distintas. Sin hora con n≥15 y IC extremo aún.
...
**⏳ H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.064 n=164568 | tras_1loss IC=+0.053 n=129829 | tras_2loss IC=+0.018 n=57994/40 | gap=+0.046 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=10679 IC=+0.009 PNL=+316.42€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=10679 IC=+0.009 PNL=+316.42€
...
**〰️ H-CUSTOM-GBM-SIGMA-BAJO** — GBM con sigma_h muy bajo (<0.0018/h, p1 real) — ¿mercado dormido = más predecible?
  ...
  - _Estado_: n=216 IC=+0.060 PNL=+38.66€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=216 IC=+0.060 PNL=+38.66€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=1507 IC=+0.031 PNL=+80.26€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1507 IC=+0.031 PNL=+80.26€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=2830 IC=+0.038 PNL=+274.00€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2830 IC=+0.038 PNL=+274.00€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=8406 IC=-0.139 PNL=+494.16€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=8406 IC=-0.139 PNL=+494.16€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.234 < -0.1 con n=912 PNL=-99.40€
  - _Datos_: n=912 IC=-0.234 PNL=-99.40€
...
**〰️ H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: n=1501 IC=-0.058 PNL=+322.24€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1501 IC=-0.058 PNL=+322.24€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.228 > 0.08 con n=2114 PNL=-217.77€
  - _Datos_: n=2114 IC=+0.228 PNL=-217.77€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=4825 IC=+0.153 PNL=-731.41€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=4825 IC=+0.153 PNL=-731.41€
