# Hipótesis automáticas — 2026-09-03 21:44 UTC
_Generado por shadow_postmortem.py sobre 271693 resoluciones (PNL=+26504.93€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.4` → IC=-0.297 (n=3665)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.4
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=10995)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `141.98` → IC=-0.281 (n=474)
...
- **FILTRO** `restante_s_al_confirmar` < `141.92` → IC=-0.315 (n=813)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=2440)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `156.16` → IC=-0.378 (n=885)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 156.16
  - _Potencial_: sin este filtro IC_bueno=-0.102 (n=1798)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.188 (n=7960)
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.146 (n=4739)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.265 (n=619)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.2` → IC=+0.408 (n=226)
...
- **PATRÓN** `libro_liquidez` > `1936.5461` → IC=+0.269 (n=669)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1936.5461 (IC base=+0.259)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.196 (n=6116)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.202 (n=2919)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.174 (n=1480)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.175 (n=1581)
...
- **PATRÓN** `libro_liquidez` > `2004.8341` → IC=+0.453 (n=296)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2004.8341 (IC base=+0.447)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.194 (n=7293)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.206 (n=17292)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.164 (n=2944)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 8.0 (IC base=+0.161)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.161 (n=2307)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 12.0 (IC base=+0.161)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.178 (n=2980)

  - _Acción_: Kelly boost +0.89€ cuando `py_entrada` > 0.71 (IC base=+0.161)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.231 (n=1522)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.268 (n=1087)
...
- **PATRÓN** `restante_min` > `4.93` → IC=+0.152 (n=2460)
...
- **PATRÓN** `lag_apertura_s` < `4.19` → IC=+0.155 (n=2288)

  - _Acción_: Kelly boost +0.77€ cuando `lag_apertura_s` < 4.19 (IC base=+0.132)
...
- **PATRÓN** `restante_min` > `4.88` → IC=+0.151 (n=1568)
...
- **PATRÓN** `lag_apertura_s` < `7.04` → IC=+0.151 (n=1501)
...
- **PATRÓN** `restante_min` > `4.94` → IC=+0.159 (n=1290)

  - _Acción_: Kelly boost +0.79€ cuando `restante_min` > 4.94 (IC base=+0.127)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.130 (n=3464)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3577` → IC=+0.128 (n=3634)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.572` → IC=+0.152 (n=1391)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 5.572 (IC base=+0.081)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1083` → IC=+0.175 (n=802)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_pendiente_norm` > 0.1083 (IC base=+0.081)

- **PATRÓN** `volumen_spike_ratio` < `2.8819` → IC=+0.175 (n=2169)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 2.8819 (IC base=+0.081)

- **PATRÓN** `volumen_spike_ratio` > `1.4763` → IC=+0.171 (n=2169)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.4763 (IC base=+0.081)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.200 (n=1084)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 54.0 (IC base=+0.081)

- **PATRÓN** `ibs_20min` < `0.4059` → IC=+0.127 (n=3497)

  - _Acción_: Kelly boost +0.63€ cuando `ibs_20min` < 0.4059 (IC base=+0.041)

- **PATRÓN** `dist_vwap_pct` < `0.3396` → IC=+0.144 (n=1266)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.3396 (IC base=+0.041)

- **PATRÓN** `volumen_regimen` < `0.6858` → IC=+0.153 (n=540)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.6858 (IC base=+0.041)

- **PATRÓN** `volumen_regimen` > `1.0473` → IC=+0.136 (n=556)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 1.0473 (IC base=+0.041)

- **PATRÓN** `volumen_pendiente_norm` > `0.3044` → IC=+0.254 (n=274)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3044 (IC base=+0.041)

- **PATRÓN** `volumen_spike_ratio` > `2.8617` → IC=+0.215 (n=612)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8617 (IC base=+0.041)

- **PATRÓN** `ballena_activa_n` < `123.0` → IC=+0.212 (n=1390)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 123.0 (IC base=+0.041)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2784` → IC=+0.157 (n=809)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.2784 (IC base=+0.154)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.595` → IC=+0.299 (n=257)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.595 (IC base=+0.154)

- **PATRÓN** `volumen_pendiente_norm` > `0.2273` → IC=+0.218 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2273 (IC base=+0.154)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4434` → IC=+0.145 (n=711)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.4434 (IC base=+0.154)
...
- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.185 (n=462)
  ...
  - _Acción_: Kelly boost +0.93€ cuando `ballena_activa_n` < 62.0 (IC base=+0.154)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.259 (n=359)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0058 (IC base=+0.259)

- **PATRÓN** `sigma_h` > `0.005` → IC=+0.274 (n=481)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.266 (n=494)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.259)
...
- **PATRÓN** `ibs_20min` < `0.4048` → IC=+0.284 (n=474)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.204` → IC=+0.277 (n=555)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.204 (IC base=+0.259)

- **PATRÓN** `volumen_pendiente_norm` < `0.0675` → IC=+0.262 (n=385)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5873` → IC=+0.284 (n=382)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5873 (IC base=+0.259)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.269 (n=527)
...
- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.144 (n=621)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.142 (n=629)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.6603` → IC=+0.163 (n=695)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3943` → IC=+0.151 (n=588)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 2.3943 (IC base=+0.139)

- **PATRÓN** `volumen_spike_ratio` > `1.4046` → IC=+0.152 (n=588)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.4046 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `12527.0336` → IC=+0.152 (n=463)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 12527.0336 (IC base=+0.139)
...
- **PATRÓN** `sigma_h` > `0.0095` → IC=+0.212 (n=255)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0095 (IC base=+0.162)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.202 (n=283)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.162)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.261 (n=283)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.297` → IC=+0.270 (n=172)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.297 (IC base=+0.162)

- **PATRÓN** `volumen_pendiente_norm` < `0.1321` → IC=+0.164 (n=652)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` < 0.1321 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` < `3.9092` → IC=+0.156 (n=679)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 3.9092 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` > `1.6798` → IC=+0.171 (n=678)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 1.6798 (IC base=+0.162)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.187 (n=783)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.149 (n=72)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=1069)
...
- **PATRÓN** `dist_vwap_pct` > `0.293` → IC=+0.195 (n=57)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.293 (IC base=-0.041)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5804` → IC=+0.142 (n=177)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 1.5804 (IC base=-0.041)
...
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.174 (n=1500)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0094 (IC base=+0.071)

- **PATRÓN** `ibs_20min` > `0.2901` → IC=+0.140 (n=4492)
...
- **PATRÓN** `dist_vwap_pct` > `0.768` → IC=+0.258 (n=209)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2588` → IC=+0.351 (n=259)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2588 (IC base=+0.052)

- **PATRÓN** `volumen_spike_ratio` > `2.4256` → IC=+0.273 (n=575)
...
- **FILTRO** `ibs_20min` < `0.2908` → IC=-0.152 (n=372)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=756)

- **FILTRO** `sigma_ewma_delta_pct` > `2.419` → IC=-0.166 (n=321)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 2.419
  - _Potencial_: sin este filtro IC_bueno=+0.030 (n=707)

- **PATRÓN** `ibs_20min` > `0.8238` → IC=+0.225 (n=282)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8238 (IC base=+0.020)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.072` → IC=+0.127 (n=422)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 2.072 (IC base=+0.020)

- **PATRÓN** `volumen_pendiente_norm` > `0.2229` → IC=+0.357 (n=40)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2229 (IC base=+0.020)

- **PATRÓN** `volumen_spike_ratio` < `1.6273` → IC=+0.229 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6273 (IC base=+0.020)

- **PATRÓN** `volumen_spike_ratio` > `2.7084` → IC=+0.285 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.7084 (IC base=+0.020)

- **PATRÓN** `ballena_activa_n` < `73.0` → IC=+0.328 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 73.0 (IC base=+0.020)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **FILTRO** `ibs_20min` > `0.8638` → IC=-0.161 (n=379)

  - _Acción_: SKIP cuando `ibs_20min` > 0.8638
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=1138)
...
- **PATRÓN** `dist_vwap_pct` > `0.531` → IC=+0.143 (n=26)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.531 (IC base=-0.028)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0647` → IC=+0.151 (n=124)
  ...
  - _Acción_: Kelly boost +0.75€ cuando `volumen_pendiente_norm` < 0.0647 (IC base=-0.028)
...
- **PATRÓN** `ballena_activa_n` < `260.0` → IC=+0.174 (n=41)
  ...
  - _Acción_: Kelly boost +0.87€ cuando `ballena_activa_n` < 260.0 (IC base=-0.028)

### GBM_LATE_15M_ESPACIO_ATR#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.285 (n=329)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0083 (IC base=+0.222)

- **PATRÓN** `drift_60min` |x|≤ `0.1245` → IC=+0.222 (n=318)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.287 (n=350)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.222)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.298` → IC=+0.298 (n=176)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.298 (IC base=+0.222)

- **PATRÓN** `volumen_pendiente_norm` < `0.107` → IC=+0.240 (n=591)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.107 (IC base=+0.222)

- **PATRÓN** `volumen_spike_ratio` < `2.4905` → IC=+0.222 (n=426)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.251 (n=738)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.678` → IC=+0.237 (n=1544)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.106` → IC=+0.184 (n=1421)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` > 0.106 (IC base=+0.163)

- **PATRÓN** `volumen_spike_ratio` < `2.3241` → IC=+0.164 (n=3110)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.3241 (IC base=+0.163)
...
- **PATRÓN** `ballena_activa_n` < `143.0` → IC=+0.181 (n=2719)
...
- **PATRÓN** `drift_60min` |x|≤ `0.283` → IC=+0.197 (n=638)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.283 (IC base=+0.172)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.225 (n=242)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.172)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.298 (n=295)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.172)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.72` → IC=+0.312 (n=206)
...
- **PATRÓN** `volumen_spike_ratio` < `2.6391` → IC=+0.163 (n=555)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.6391 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` > `1.4556` → IC=+0.164 (n=555)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.4556 (IC base=+0.172)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2845` → IC=+0.187 (n=522)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.2845 (IC base=+0.172)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.190 (n=201)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` > 17.0 (IC base=+0.172)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.193 (n=203)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 5.0 (IC base=+0.172)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.307 (n=303)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1304` → IC=+0.168 (n=215)
  ...
  - _Acción_: Kelly boost +0.84€ cuando `volumen_pendiente_norm` > 0.1304 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` < `1.9217` → IC=+0.170 (n=234)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` < 1.9217 (IC base=+0.172)

- **PATRÓN** `volumen_spike_ratio` > `2.4409` → IC=+0.163 (n=354)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 2.4409 (IC base=+0.172)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.196 (n=597)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.203 (n=2064)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0087 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.172 (n=4571)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.635` → IC=+0.224 (n=2560)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1675` → IC=+0.191 (n=1233)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_pendiente_norm` > 0.1675 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `1.8748` → IC=+0.173 (n=2813)
...
- **PATRÓN** `libro_liquidez` > `3845.8163` → IC=+0.188 (n=1515)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3845.8163 (IC base=+0.165)

- **PATRÓN** `ballena_activa_n` < `60.0` → IC=+0.212 (n=2116)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 60.0 (IC base=+0.165)

- **PATRÓN** `sigma_h` < `0.0108` → IC=+0.198 (n=4194)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0108 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.4818` → IC=+0.197 (n=4193)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.4818 (IC base=+0.189)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.206 (n=1866)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.189)

- **PATRÓN** `ibs_20min` < `0.5625` → IC=+0.242 (n=4193)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5625 (IC base=+0.189)

- **PATRÓN** `dist_vwap_pct` < `0.4473` → IC=+0.174 (n=2968)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.4473 (IC base=+0.189)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.654` → IC=+0.214 (n=589)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.654 (IC base=+0.189)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.578` → IC=+0.190 (n=4130)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` < 3.578 (IC base=+0.189)

- **PATRÓN** `volumen_regimen` < `0.6225` → IC=+0.174 (n=996)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3087` → IC=+0.205 (n=1580)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3087 (IC base=+0.189)

- **PATRÓN** `ballena_activa_n` < `139.0` → IC=+0.182 (n=3066)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.788` → IC=+0.308 (n=440)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.788 (IC base=+0.185)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8897` → IC=+0.178 (n=442)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 1.8897 (IC base=+0.185)
...
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.276 (n=484)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.1757` → IC=+0.293 (n=366)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1757 (IC base=+0.271)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.280 (n=502)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.4087` → IC=+0.306 (n=483)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4087 (IC base=+0.271)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.453` → IC=+0.285 (n=566)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.453 (IC base=+0.271)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5076` → IC=+0.292 (n=445)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5076 (IC base=+0.271)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.275 (n=544)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.271)

- **PATRÓN** `libro_liquidez` > `1456.1771` → IC=+0.279 (n=490)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1456.1771 (IC base=+0.271)

- **PATRÓN** `ballena_activa_n` < `68.0` → IC=+0.271 (n=339)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 68.0 (IC base=+0.271)
...
- **PATRÓN** `ibs_20min` < `0.6154` → IC=+0.199 (n=682)
  ...
  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` < 0.6154 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` < `0.1465` → IC=+0.179 (n=586)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.1465 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.763` → IC=+0.207 (n=121)
...
- **PATRÓN** `volumen_spike_ratio` < `1.395` → IC=+0.192 (n=196)
  ...
  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` < 1.395 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` > `2.062` → IC=+0.183 (n=266)
...
- **PATRÓN** `sigma_h` > `0.01` → IC=+0.249 (n=217)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.6842` → IC=+0.255 (n=582)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6842 (IC base=+0.206)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.607` → IC=+0.348 (n=156)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.607 (IC base=+0.206)

- **PATRÓN** `volumen_pendiente_norm` < `0.2205` → IC=+0.213 (n=591)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2205 (IC base=+0.206)

- **PATRÓN** `volumen_spike_ratio` < `3.9629` → IC=+0.203 (n=578)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.9629 (IC base=+0.206)

- **PATRÓN** `volumen_spike_ratio` > `2.4981` → IC=+0.203 (n=385)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4981 (IC base=+0.206)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.236 (n=657)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.206)

- **PATRÓN** `libro_liquidez` > `1854.121` → IC=+0.208 (n=296)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1854.121 (IC base=+0.206)

- **PATRÓN** `ballena_activa_n` < `63.0` → IC=+0.265 (n=364)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 63.0 (IC base=+0.206)
...
- **PATRÓN** `ibs_20min` < `0.6744` → IC=+0.182 (n=598)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.291` → IC=+0.139 (n=566)

  - _Acción_: Kelly boost +0.70€ cuando `sigma_ewma_delta_pct` < 4.291 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `0.825` → IC=+0.138 (n=399)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` < 0.825 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` > `1.1408` → IC=+0.167 (n=199)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` > 1.1408 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` > `0.2797` → IC=+0.278 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2797 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `2.5341` → IC=+0.169 (n=179)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 2.5341 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `11643.1172` → IC=+0.211 (n=199)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11643.1172 (IC base=+0.137)
...
- **PATRÓN** `sigma_h` < `0.01` → IC=+0.133 (n=1307)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.01 (IC base=+0.120)

- **PATRÓN** `drift_60min` |x|≤ `0.5538` → IC=+0.130 (n=1485)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.65€ cuando `drift_60min` |x|≤ 0.5538 (IC base=+0.120)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.165 (n=500)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 18.0 (IC base=+0.120)

- **PATRÓN** `ibs_20min` > `0.921` → IC=+0.190 (n=495)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` > 0.921 (IC base=+0.120)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.976` → IC=+0.122 (n=1507)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` < 5.976 (IC base=+0.120)

- **PATRÓN** `volumen_regimen` > `0.8967` → IC=+0.120 (n=662)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_regimen` > 0.8967 (IC base=+0.120)

- **PATRÓN** `volumen_pendiente_norm` > `0.1747` → IC=+0.150 (n=404)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_pendiente_norm` > 0.1747 (IC base=+0.120)

- **PATRÓN** `volumen_spike_ratio` < `1.4612` → IC=+0.148 (n=490)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.4612 (IC base=+0.120)

- **PATRÓN** `volumen_spike_ratio` > `1.8992` → IC=+0.139 (n=979)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` > 1.8992 (IC base=+0.120)

- **PATRÓN** `libro_liquidez` > `8959.8986` → IC=+0.134 (n=673)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 8959.8986 (IC base=+0.120)
...
- **PATRÓN** `sigma_h` < `0.0089` → IC=+0.151 (n=542)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0089 (IC base=+0.134)

- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.134 (n=542)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` > 0.0046 (IC base=+0.134)

- **PATRÓN** `drift_60min` |x|≤ `0.4007` → IC=+0.143 (n=477)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.4007 (IC base=+0.134)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.188 (n=187)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.185` → IC=+0.145 (n=542)

  - _Acción_: Kelly boost +0.73€ cuando `ibs_20min` > 0.185 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` > `1.0443` → IC=+0.167 (n=121)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 1.0443 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` < `0.4409` → IC=+0.148 (n=515)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.4409 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.989` → IC=+0.147 (n=545)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` < 6.989 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` < `0.7283` → IC=+0.156 (n=239)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.7283 (IC base=+0.134)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1782` → IC=+0.159 (n=162)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_pendiente_norm` > 0.1782 (IC base=+0.134)

- **PATRÓN** `volumen_spike_ratio` < `1.4471` → IC=+0.183 (n=178)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` < 1.4471 (IC base=+0.134)

- **PATRÓN** `volumen_spike_ratio` > `1.825` → IC=+0.139 (n=355)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 1.825 (IC base=+0.134)

- **PATRÓN** `libro_liquidez` > `8943.6658` → IC=+0.146 (n=484)
...
- **FILTRO** `ibs_20min` > `0.1176` → IC=-0.294 (n=32)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.185 (n=1464)
...
- **FILTRO** `py_entrada` > `0.67` → IC=-0.165 (n=246)
...
- **FILTRO** `py_entrada` > `0.65` → IC=-0.223 (n=236)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=772)

- **FILTRO** `ibs_20min` > `0.7172` → IC=-0.196 (n=251)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7172
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=757)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=2991)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=11064)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.282 (n=3459)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=10596)

- **FILTRO** `ibs_7min` < `0.7117` → IC=-0.246 (n=3513)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7117
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=10542)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.169 (n=4703)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=9352)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.219 (n=4357)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=13340)

- **FILTRO** `ibs_7min` > `0.7143` → IC=-0.172 (n=4415)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=13282)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.155 (n=604)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.100 (n=1441)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.317 (n=507)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=1538)

- **FILTRO** `ibs_7min` < `0.3` → IC=-0.262 (n=674)

  - _Acción_: SKIP cuando `ibs_7min` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=1371)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.228 (n=508)

  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=1537)

- **FILTRO** `py_entrada` > `0.52` → IC=-0.140 (n=2050)
...
- **FILTRO** `drift_7min_pct` |x|> `0.1142` → IC=-0.130 (n=1046)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=2032)

- **FILTRO** `ibs_7min` > `0.8333` → IC=-0.190 (n=769)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8333
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=2309)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=572)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=2055)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.258 (n=638)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=1989)

- **FILTRO** `ibs_7min` < `0.7699` → IC=-0.193 (n=656)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7699
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=1971)

- **FILTRO** `ballena_activa_n` > `168.0` → IC=-0.177 (n=652)

  - _Acción_: SKIP cuando `ballena_activa_n` > 168.0
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=1975)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.184 (n=656)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.096 (n=1422)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.313 (n=657)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=1421)

- **FILTRO** `ibs_7min` < `0.2127` → IC=-0.293 (n=519)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2127
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=1559)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.250 (n=490)

  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=1588)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.224 (n=738)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=2435)

- **FILTRO** `ibs_7min` > `0.7981` → IC=-0.170 (n=793)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7981
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=2380)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.138 (n=717)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.090 (n=1666)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.257 (n=583)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=1800)

- **FILTRO** `ibs_7min` < `0.7509` → IC=-0.195 (n=595)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.237 (n=778)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=1617)

- **FILTRO** `ibs_7min` > `0.2755` → IC=-0.178 (n=598)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2755
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=1797)

- **FILTRO** `ballena_activa_n` > `34.0` → IC=-0.181 (n=588)
...
- **FILTRO** `ibs_7min` < `0.7333` → IC=-0.213 (n=654)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=1968)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.178 (n=799)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=2525)
...
- **FILTRO** `py_entrada` < `0.36` → IC=-0.297 (n=564)
...
- **FILTRO** `ibs_7min` < `0.7333` → IC=-0.249 (n=572)
  ...
  - _Acción_: SKIP cuando `ibs_7min` < 0.7333
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=1728)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.232 (n=557)
...
- **PATRÓN** `sigma_h` > `0.0053` → IC=+0.149 (n=343)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0053 (IC base=+0.128)

- **PATRÓN** `drift_60min` |x|≤ `0.1678` → IC=+0.129 (n=453)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.64€ cuando `drift_60min` |x|≤ 0.1678 (IC base=+0.128)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0599` → IC=+0.133 (n=515)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.66€ cuando `delta_ratio_macro` |x|> 0.0599 (IC base=+0.128)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.133 (n=515)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_15` > `0.5787` → IC=+0.218 (n=515)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5787 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` > `0.4298` → IC=+0.172 (n=129)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.4298 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` < `0.1005` → IC=+0.125 (n=315)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.1005 (IC base=+0.128)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.945` → IC=+0.203 (n=257)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.945 (IC base=+0.128)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.133 (n=535)

  - _Acción_: Kelly boost +0.67€ cuando `libro_spread` < 0.01 (IC base=+0.128)

- **PATRÓN** `libro_liquidez` > `3062.2042` → IC=+0.152 (n=343)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 3062.2042 (IC base=+0.128)
...
- **FILTRO** `sigma_h` > `0.0107` → IC=-0.204 (n=333)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=1001)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.138 (n=343)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=991)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.297` → IC=+0.171 (n=165)

  - _Acción_: Kelly boost +0.85€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.297 (IC base=-0.069)

- **PATRÓN** `ibs_15` > `0.4875` → IC=+0.185 (n=350)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.92€ cuando `ibs_15` > 0.4875 (IC base=-0.069)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1242` → IC=+0.240 (n=290)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **PATRÓN** `ibs_15` < `0.3707` → IC=+0.269 (n=435)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3707 (IC base=-0.065)

- **PATRÓN** `dist_vwap_pct` < `0.4752` → IC=+0.215 (n=461)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4752 (IC base=-0.065)
...
- **PATRÓN** `ibs_15` < `0.3811` → IC=+0.275 (n=220)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3811 (IC base=+0.212)

- **PATRÓN** `dist_vwap_pct` > `0.1907` → IC=+0.234 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1907 (IC base=+0.212)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.326` → IC=+0.229 (n=234)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 11.326 (IC base=+0.212)

- **PATRÓN** `libro_liquidez` > `13702.0305` → IC=+0.210 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13702.0305 (IC base=+0.212)
...
- **PATRÓN** `ibs_15` > `0.0714` → IC=+0.132 (n=85)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.66€ cuando `ibs_15` > 0.0714 (IC base=-0.049)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.5787 sube el IC de +0.128 a +0.218 en UPDOWN_GBM#15min (n=515). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.4875 sube el IC de -0.069 a +0.185 en UPDOWN_GBM_15M_TARDIO (n=350). Ya aplicado como kelly_boost=+0.92€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3707 sube el IC de -0.065 a +0.269 en UPDOWN_GBM_15M_TARDIO (n=435). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.3811 sube el IC de +0.212 a +0.275 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=220). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 14660 | -0.111 | -2466.67€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 952 | -0.009 | -141.10€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 13708 | -0.118 | -2325.57€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1899 | -0.088 | -409.36€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1899 | -0.088 | -409.36€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC |