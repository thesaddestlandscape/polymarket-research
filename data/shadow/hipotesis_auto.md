# Hipótesis automáticas — 2026-07-29 13:38 UTC
_Generado por shadow_postmortem.py sobre 41819 resoluciones (PNL=+9003.08€)_
...
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.191 (n=1352)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0077 (IC base=+0.145)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.164 (n=1906)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 7.0 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.1132` → IC=+0.170 (n=858)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.1132 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.881` → IC=+0.223 (n=976)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.881 (IC base=+0.145)

- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.157 (n=1502)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` > 0.0068 (IC base=+0.125)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.137 (n=1525)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.797` → IC=+0.146 (n=489)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` > 5.797 (IC base=+0.125)

### GBM_LATE_15M_ESPACIO_ATR#BTC#15min
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.136 (n=438)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 7.0 (IC base=+0.107)

- **PATRÓN** `dist_vwap_pct` > `0.448` → IC=+0.167 (n=64)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.448 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.76` → IC=+0.167 (n=85)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 11.76 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.36` → IC=+0.146 (n=207)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` > 4.36 (IC base=+0.091)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.189 (n=162)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` > 0.0078 (IC base=+0.118)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.138 (n=346)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 11.0 (IC base=+0.118)

- **PATRÓN** `dist_vwap_pct` > `0.1175` → IC=+0.140 (n=223)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` > 0.1175 (IC base=+0.118)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.301` → IC=+0.187 (n=132)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 7.301 (IC base=+0.118)

- **PATRÓN** `libro_liquidez` > `8292.9308` → IC=+0.137 (n=254)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 8292.9308 (IC base=+0.118)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.399` → IC=+0.185 (n=109)

  - _Acción_: Kelly boost +0.92€ cuando `sigma_ewma_delta_pct` > 9.399 (IC base=+0.087)

### GBM_LATE_15M_ESPACIO_ATR#SOL#15min
- **PATRÓN** `sigma_h` > `0.011` → IC=+0.202 (n=226)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.011 (IC base=+0.117)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.150 (n=332)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 12.0 (IC base=+0.117)

- **PATRÓN** `dist_vwap_pct` > `0.1896` → IC=+0.162 (n=190)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.1896 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.397` → IC=+0.281 (n=217)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.397 (IC base=+0.117)

- **PATRÓN** `libro_liquidez` > `3261.4804` → IC=+0.204 (n=187)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3261.4804 (IC base=+0.117)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.296` → IC=+0.131 (n=445)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` < 2.296 (IC base=+0.090)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` < `0.0169` → IC=+0.239 (n=466)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0169 (IC base=+0.227)

- **PATRÓN** `sigma_h` > `0.0084` → IC=+0.227 (n=466)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0084 (IC base=+0.227)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.240 (n=478)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.227)

- **PATRÓN** `dist_vwap_pct` > `0.1558` → IC=+0.256 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1558 (IC base=+0.227)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.462` → IC=+0.351 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.462 (IC base=+0.227)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.222 (n=512)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.227)

- **PATRÓN** `sigma_h` < `0.0166` → IC=+0.212 (n=571)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0166 (IC base=+0.208)

- **PATRÓN** `sigma_h` > `0.007` → IC=+0.232 (n=572)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.007 (IC base=+0.208)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.232 (n=513)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.208)

- **PATRÓN** `dist_vwap_pct` > `0.1917` → IC=+0.322 (n=116)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1917 (IC base=+0.208)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.347` → IC=+0.221 (n=66)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.347 (IC base=+0.208)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.119` → IC=+0.202 (n=628)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 9.119 (IC base=+0.208)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.221 (n=468)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.208)

### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.121 (n=1347)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.61€ cuando `sigma_h` > 0.0083 (IC base=+0.096)

- **PATRÓN** `dist_vwap_pct` > `0.482` → IC=+0.133 (n=379)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.482 (IC base=+0.096)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.764` → IC=+0.237 (n=333)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.764 (IC base=+0.096)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `dist_vwap_pct` > `0.4756` → IC=+0.136 (n=64)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 0.4756 (IC base=+0.072)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.471` → IC=+0.189 (n=72)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 12.471 (IC base=+0.072)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_ewma_delta_pct` > `12.266` → IC=+0.171 (n=68)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 12.266 (IC base=+0.051)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.134` → IC=+0.229 (n=164)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.134 (IC base=+0.046)

### GBM_LATE_15M_TARDIO#XRP#15min
- **PATRÓN** `sigma_h` < `0.0182` → IC=+0.174 (n=511)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` < 0.0182 (IC base=+0.171)

- **PATRÓN** `sigma_h` > `0.0244` → IC=+0.173 (n=194)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0244 (IC base=+0.171)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.177 (n=589)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 6.0 (IC base=+0.171)

- **PATRÓN** `dist_vwap_pct` > `0.486` → IC=+0.224 (n=125)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.486 (IC base=+0.171)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.665` → IC=+0.291 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.665 (IC base=+0.171)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.170 (n=446)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.01 (IC base=+0.171)

- **PATRÓN** `sigma_h` < `0.0217` → IC=+0.163 (n=681)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0217 (IC base=+0.159)

- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.173 (n=681)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0076 (IC base=+0.159)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.182 (n=460)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1404` → IC=+0.234 (n=175)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1404 (IC base=+0.159)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.452` → IC=+0.158 (n=705)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.6011` → IC=+0.172 (n=663)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.86€ cuando `ibs_15` > 0.6011 (IC base=+0.055)

- **PATRÓN** `dist_vwap_pct` > `0.1676` → IC=+0.157 (n=246)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.1676 (IC base=+0.055)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.864` → IC=+0.148 (n=245)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 4.864 (IC base=+0.055)

- **PATRÓN** `libro_liquidez` > `10159.6272` → IC=+0.142 (n=191)

  - _Acción_: Kelly boost +0.71€ cuando `libro_liquidez` > 10159.6272 (IC base=+0.055)

- **PATRÓN** `dist_vwap_pct` > `0.9454` → IC=+0.167 (n=67)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.9454 (IC base=+0.051)
...
- **PATRÓN** `ibs_15` > `0.6674` → IC=+0.187 (n=212)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.93€ cuando `ibs_15` > 0.6674 (IC base=+0.043)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.038` → IC=+0.142 (n=65)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 7.038 (IC base=+0.043)

- **PATRÓN** `sigma_h` < `0.0055` → IC=+0.139 (n=142)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0055 (IC base=+0.060)

- **PATRÓN** `dist_vwap_pct` > `0.9263` → IC=+0.218 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9263 (IC base=+0.060)
...
- **PATRÓN** `drift_15min` |x|≤ `0.4588` → IC=+0.162 (n=75)

  - _Acción_: Kelly boost +0.81€ cuando `drift_15min` |x|≤ 0.4588 (IC base=+0.045)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.123 (n=168)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 12.0 (IC base=+0.078)

- **PATRÓN** `ibs_15` < `0.04` → IC=+0.154 (n=79)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.77€ cuando `ibs_15` < 0.04 (IC base=+0.078)

- **PATRÓN** `libro_liquidez` > `2969.8419` → IC=+0.124 (n=91)

  - _Acción_: Kelly boost +0.62€ cuando `libro_liquidez` > 2969.8419 (IC base=+0.078)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6011 sube el IC de +0.055 a +0.172 en UPDOWN_GBM#15min (n=663). Ya aplicado como kelly_boost=+0.86€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.6674 sube el IC de +0.043 a +0.187 en UPDOWN_GBM#ETH#15min (n=212). Ya aplicado como kelly_boost=+0.93€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.04 sube el IC de +0.078 a +0.154 en UPDOWN_GBM#XRP#15min (n=79). Ya aplicado como kelly_boost=+0.77€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO#XRP` — IC=+0.149 n=35. Faltan ~5 resoluciones para umbral n≥40. ETA: ~4h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION` — IC=+0.382 n=32. Faltan ~8 resoluciones para umbral n≥40. ETA: ~6h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#60min` — IC=+0.382 n=32. Faltan ~8 resoluciones para umbral n≥40. ETA: ~6h.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 1448 | +0.095 | +27.94€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 1448 | +0.095 | +27.94€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 19 | +0.068 | -0.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 583 | +0.091 | +5.50€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 583 | +0.091 | +5.50€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 539 | +0.110 | +4.11€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS | 66 | +0.309 | +6.46€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 62 | +0.328 | +2.01€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 62 | +0.328 | +2.01€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 62 | +0.328 | +2.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 6981 | +0.177 | -45.50€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 4590 | +0.200 | -49.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 321 | +0.057 | -23.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 821 | +0.128 | -56.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 1249 | +0.154 | +84.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 2082 | +0.183 | +4.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1541 | +0.197 | -44.25€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 107 | +0.041 | -15.06€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BTC#5min | 10 | -0.125 | -3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 424 | +0.176 | +67.37€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 8 | +0.120 | +5.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 7 | +0.097 | +4.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 2446 | +0.167 | -44.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1505 | +0.198 | -6.71€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 106 | -0.009 | -21.67€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 416 | +0.127 | -31.30€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 419 | +0.139 | +14.99€ | 0 | 10 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 47 | +0.357 | +2.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 47 | +0.357 | +2.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC | 16 | +0.222 | -0.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BTC#15min | 16 | +0.222 | -0.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 18 | +0.315 | +1.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 18 | +0.315 | +1.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 10 | +0.167 | +0.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 10 | +0.167 | +0.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION | 32 | +0.382 | +4.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#60min | 32 | +0.382 | +4.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#BTC | 14 | +0.219 | +2.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#BTC#60min | 14 | +0.219 | +2.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#ETH | 14 | +0.263 | +2.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_60MIN_ALTACONVICCION#ETH#60min | 14 | +0.263 | +2.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 451 | +0.303 | +8.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 451 | +0.303 | +8.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 451 | +0.303 | +8.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 451 | +0.303 | +8.45€ | 0 | 0 |
| ✅ GBM_LATE_15M | 9032 | +0.095 | +2729.53€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 9032 | +0.095 | +2729.53€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 135 | +0.201 | +87.31€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min |
overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.078 n=1825 | neutral: IC=+0.025 n=1426 | overbought(IBS>0.7): IC=+0.184 n=1719
  - _Datos_: n=5304 IC=+0.104 PNL=+1395.38€
...
**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  ...
  - _Estado_: 15 celda(s) pasan gate riguroso completo de 40 evaluadas (n>=40) y 157 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=+0.014 < 0.08 — monitorear
  - _Datos_: n=282 IC=+0.014 PNL=+19.38€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.112 n=24900 | tras_1loss IC=+0.078 n=16679 | tras_2loss IC=+0.052 n=7000/40 | gap=+0.059 (umbral 0.05)
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=-0.006 n=170 | contrario_BTC IC=-0.033 n=75/40 | gap=-0.027 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2946 IC=+0.041 PNL=+299.31€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2946 IC=+0.041 PNL=+299.31€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=1164 IC=+0.055 PNL=+157.12€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1164 IC=+0.055 PNL=+157.12€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=520 IC=+0.088 PNL=+93.33€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=520 IC=+0.088 PNL=+93.33€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.087 > 0.08 con n=1476 PNL=+246.83€
  - _Datos_: n=1476 IC=+0.087 PNL=+246.83€
...
**〰️ H-CUSTOM-ETH15-REVERSION** — ETH#15min con drift_15min < -1 — ¿mean reversion?
  ...
  - _Estado_: n=117 IC=+0.021 PNL=+6.13€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=117 IC=+0.021 PNL=+6.13€
...
**〰️ H-LATE-WINDOW-5MIN** — Late-window BTC 5min — arbitraje timing vs Polymarket
  ...
  - _Estado_: n=382 IC=-0.065 PNL=-7.71€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=382 IC=-0.065 PNL=-7.71€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=1586 IC=+0.042 PNL=+132.19€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1586 IC=+0.042 PNL=+132.19€

**〰️ H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  - _Hipótesis_: Feature nueva 2026-06-27: drift del precio YES en Polymarket en últimas 5 obs (~5min). Si poly_drift<0 y decidimos BUY_NO (o poly_drift>0 y BUY_YES) → confluencia. Si diverge → reducción de stake. Hipótesis: confluencia Binance+Polymarket mejora IC; divergencia empeora.
  - _Umbral_: n≥40 en confluencia vs divergencia para validar el boost ×1.1
  - _Acción_: Si IC_confluencia>IC_divergencia con n≥40 → mantener el boost. Si no → retirar.
  - _Estado_: n=455 IC=+0.060 PNL=+58.90€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=455 IC=+0.060 PNL=+58.90€
...
**🟡 H-CUSTOM-CROSS-WINDOW-SPREAD-POS** — GBM 15min/60min: spread positivo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.235 > 0.08 con n=209 PNL=+45.59€
  - _Datos_: n=209 IC=+0.235 PNL=+45.59€
...
**〰️ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: n=5502 IC=+0.085 PNL=+1333.47€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=5502 IC=+0.085 PNL=+1333.47€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=462 IC=+0.030 PNL=+23.29€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=462 IC=+0.030 PNL=+23.29€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=1314 IC=+0.031 PNL=+126.57€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1314 IC=+0.031 PNL=+126.57€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.158 > 0.1 con n=1087 PNL=+358.55€
  - _Datos_: n=1087 IC=+0.158 PNL=+358.55€
...
**〰️ H-CUSTOM-BUYNO-LONGSHOT-15MIN** — BUY_NO longshot en 15min (py_mkt≥0.55) — comprar NO barato pierde
  ...
  - _Estado_: n=66 IC=-0.029 PNL=+11.38€ — sin señal clara aún (umbral IC: min=None max=-0.1)
  - _Datos_: n=66 IC=-0.029 PNL=+11.38€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=320 IC=+0.078 PNL=+54.88€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=320 IC=+0.078 PNL=+54.88€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1082 IC=-0.069 PNL=+297.08€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1082 IC=-0.069 PNL=+297.08€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=1167 IC=+0.049 PNL=+111.56€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=1167 IC=+0.049 PNL=+111.56€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.117 > 0.08 con n=760 PNL=+203.37€
  - _Datos_: n=760 IC=+0.117 PNL=+203.37€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.209 > 0.2 con n=833 PNL=+506.53€
  - _Datos_: n=833 IC=+0.209 PNL=+506.53€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.095 > 0.08 con n=595 PNL=+138.53€
  - _Datos_: n=595 IC=+0.095 PNL=+138.53€

**🟡 H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  - _Hipótesis_: 12-Jul: mismo mecanismo que ETH (ver H-CUSTOM-ETH15-SIGMA-ACCEL). Verificado ad-hoc n=35: hit sube de 63.6% (agregado BTC) a 68.6%, ic_bayes=+0.176.
  - _Umbral_: n>=40 y IC>+0.08
  - _Acción_: Si confirma con n>=40 -> proponer kelly_boost condicionado a sigma_ewma_delta_pct>=0 en BTC#15min
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.093 > 0.08 con n=645 PNL=+176.25€
  - _Datos_: n=645 IC=+0.093 PNL=+176.25€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.125 > 0.08 con n=812 PNL=+388.92€
  - _Datos_: n=812 IC=+0.125 PNL=+388.92€
...
**〰️ H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Estado_: n=132 IC=+0.045 PNL=+12.87€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=132 IC=+0.045 PNL=+12.87€
...
**🟡 H-CUSTOM-ETH15-BUYNO-TARDIO** — UPDOWN_GBM ETH#15min BUY_NO tardío (T_h<0.2) -- edge fuerte no capturado por el aprendizaje causal automático
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.302 > 0.08 con n=119 PNL=+63.13€
  - _Datos_: n=119 IC=+0.302 PNL=+63.13€
