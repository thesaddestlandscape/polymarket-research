# Hipótesis automáticas — 2026-08-18 22:18 UTC
_Generado por shadow_postmortem.py sobre 68289 resoluciones (PNL=+6730.56€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `n_ballenas` < `5.0` → IC=-0.126 (n=2157)

  - _Acción_: SKIP cuando `n_ballenas` < 5.0
  - _Potencial_: sin este filtro IC_bueno=-0.083 (n=1470)

- **FILTRO** `restante_s_al_confirmar` < `150.62` → IC=-0.252 (n=906)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 150.62
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=2721)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `117.22` → IC=-0.406 (n=94)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 117.22
  - _Potencial_: sin este filtro IC_bueno=-0.113 (n=285)
...
- **FILTRO** `concentracion_yes` < `1.0` → IC=-0.135 (n=61)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.126 (n=814)

- **FILTRO** `restante_s_al_confirmar` < `247.25` → IC=-0.155 (n=656)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 247.25
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=219)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `156.96` → IC=-0.282 (n=177)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 156.96
  - _Potencial_: sin este filtro IC_bueno=-0.205 (n=531)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.193 (n=2330)

  - _Acción_: Kelly boost +0.96€ cuando `py_entrada` > 0.7 (IC base=+0.092)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.164 (n=1169)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.092)

- **PATRÓN** `libro_liquidez` > `2371.8427` → IC=+0.167 (n=1130)

  - _Acción_: Kelly boost +0.83€ cuando `libro_liquidez` > 2371.8427 (IC base=+0.092)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.158 (n=3992)
...
- **PATRÓN** `py_entrada` < `0.34` → IC=+0.286 (n=1500)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.208 (n=368)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.128 (n=619)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 15.0 (IC base=+0.113)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.279 (n=120)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `915.2245` → IC=+0.244 (n=303)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.219 (n=607)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.194 (n=1679)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.227 (n=3242)
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.138 (n=423)

  - _Acción_: Kelly boost +0.69€ cuando `py_entrada` > 0.73 (IC base=+0.096)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.298 (n=245)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.323 (n=241)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.232 (n=368)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.298 (n=261)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.75 (IC base=+0.221)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.251 (n=263)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.284 (n=360)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.74 (IC base=+0.245)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.222 (n=282)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.156 (n=1553)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.239 (n=304)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.157)

- **PATRÓN** `restante_min` < `3.73` → IC=+0.173 (n=276)

  - _Acción_: Kelly boost +0.86€ cuando `restante_min` < 3.73 (IC base=+0.157)

- **PATRÓN** `restante_min` > `4.88` → IC=+0.195 (n=293)

  - _Acción_: Kelly boost +0.97€ cuando `restante_min` > 4.88 (IC base=+0.157)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.164 (n=861)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `7.32` → IC=+0.197 (n=272)

  - _Acción_: Kelly boost +0.99€ cuando `lag_apertura_s` < 7.32 (IC base=+0.157)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.212 (n=304)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.147 (n=785)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `profundidad_ratio_no` > `11.6` → IC=+0.167 (n=280)
...
- **PATRÓN** `ibs_20min` > `0.9834` → IC=+0.265 (n=424)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.749` → IC=+0.235 (n=606)
...
- **PATRÓN** `ibs_20min` < `0.6597` → IC=+0.123 (n=2173)

  - _Acción_: Kelly boost +0.61€ cuando `ibs_20min` < 0.6597 (IC base=+0.078)

- **PATRÓN** `dist_vwap_pct` < `0.1465` → IC=+0.142 (n=548)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.1465 (IC base=+0.078)

- **PATRÓN** `volumen_regimen` < `1.3049` → IC=+0.142 (n=492)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 1.3049 (IC base=+0.078)

- **PATRÓN** `volumen_regimen` > `0.6936` → IC=+0.142 (n=439)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.292 (n=166)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5027` → IC=+0.314 (n=235)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5027 (IC base=+0.287)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.108` → IC=+0.316 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.108 (IC base=+0.287)

- **PATRÓN** `volumen_pendiente_norm` > `0.3446` → IC=+0.397 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3446 (IC base=+0.287)

- **PATRÓN** `volumen_spike_ratio` < `4.8135` → IC=+0.274 (n=113)
...
- **PATRÓN** `libro_spread` < `0.06` → IC=+0.292 (n=277)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.204` → IC=+0.126 (n=1008)
...
- **PATRÓN** `ibs_20min` < `0.1` → IC=+0.165 (n=864)
...
- **PATRÓN** `ballena_activa_n` < `74.0` → IC=+0.316 (n=85)
...
- **PATRÓN** `volumen_regimen` < `0.88` → IC=+0.155 (n=27)
  ...
  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.88 (IC base=-0.013)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.152 (n=44)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=697)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.001` → IC=+0.287 (n=308)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.2891` → IC=+0.379 (n=31)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2891 (IC base=+0.284)

- **PATRÓN** `volumen_spike_ratio` > `3.5418` → IC=+0.315 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.5418 (IC base=+0.284)
...
- **PATRÓN** `sigma_h` < `0.0047` → IC=+0.149 (n=426)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0047 (IC base=+0.137)

- **PATRÓN** `sigma_h` > `0.0066` → IC=+0.192 (n=579)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0066 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.144 (n=431)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.2265` → IC=+0.145 (n=288)
  ...
  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.2265 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.94` → IC=+0.266 (n=625)
...
- **PATRÓN** `volumen_regimen` > `0.6237` → IC=+0.139 (n=660)
  ...
  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` > 0.6237 (IC base=+0.137)
...
- **PATRÓN** `libro_spread` < `0.06` → IC=+0.144 (n=1434)
...
- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.220 (n=1232)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.215)

- **PATRÓN** `drift_60min` |x|≤ `0.2365` → IC=+0.224 (n=1084)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2365 (IC base=+0.215)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.259 (n=578)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.278` → IC=+0.216 (n=1219)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.278 (IC base=+0.215)

- **PATRÓN** `volumen_regimen` < `1.2477` → IC=+0.192 (n=958)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` < 1.2477 (IC base=+0.215)

- **PATRÓN** `volumen_regimen` > `0.8706` → IC=+0.202 (n=638)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0432` → IC=+0.230 (n=420)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0432 (IC base=+0.215)

- **PATRÓN** `volumen_spike_ratio` > `3.1082` → IC=+0.245 (n=210)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.1082 (IC base=+0.215)

- **PATRÓN** `ballena_activa_n` < `34.0` → IC=+0.291 (n=84)
...
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.324 (n=129)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0067 (IC base=+0.312)

- **PATRÓN** `sigma_h` > `0.0053` → IC=+0.312 (n=131)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0053 (IC base=+0.312)

- **PATRÓN** `drift_60min` |x|≤ `0.1716` → IC=+0.370 (n=98)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1716 (IC base=+0.312)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.333 (n=100)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 13.0 (IC base=+0.312)

- **PATRÓN** `ibs_20min` < `0.2453` → IC=+0.340 (n=129)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2453 (IC base=+0.312)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.781` → IC=+0.353 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.781 (IC base=+0.312)

- **PATRÓN** `volumen_pendiente_norm` > `0.3446` → IC=+0.364 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3446 (IC base=+0.312)

- **PATRÓN** `volumen_spike_ratio` < `3.4561` → IC=+0.324 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.4561 (IC base=+0.312)

- **PATRÓN** `volumen_spike_ratio` > `2.1707` → IC=+0.306 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1707 (IC base=+0.312)

- **PATRÓN** `libro_liquidez` > `1915.1084` → IC=+0.324 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1915.1084 (IC base=+0.312)
...
- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.284 (n=230)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0064 (IC base=+0.268)

- **PATRÓN** `sigma_h` > `0.005` → IC=+0.276 (n=235)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.005 (IC base=+0.268)

- **PATRÓN** `drift_60min` |x|≤ `0.0795` → IC=+0.286 (n=87)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0795 (IC base=+0.268)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.286 (n=124)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.2333` → IC=+0.323 (n=230)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2333 (IC base=+0.268)

- **PATRÓN** `dist_vwap_pct` > `0.1301` → IC=+0.336 (n=53)
...
- **PATRÓN** `volumen_regimen` > `1.0811` → IC=+0.325 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0811 (IC base=+0.268)

- **PATRÓN** `volumen_pendiente_norm` > `0.2721` → IC=+0.365 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2721 (IC base=+0.268)

- **PATRÓN** `volumen_spike_ratio` > `1.7731` → IC=+0.269 (n=119)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7731 (IC base=+0.268)

- **PATRÓN** `libro_liquidez` > `2655.4157` → IC=+0.283 (n=118)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2655.4157 (IC base=+0.268)

- **PATRÓN** `ballena_activa_n` < `30.0` → IC=+0.312 (n=30)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 30.0 (IC base=+0.268)
...
- **PATRÓN** `ibs_20min` < `0.3601` → IC=+0.123 (n=160)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` < 0.3601 (IC base=+0.057)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.874` → IC=+0.223 (n=45)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.874 (IC base=+0.057)

- **PATRÓN** `volumen_spike_ratio` < `2.6304` → IC=+0.132 (n=112)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` < 2.6304 (IC base=+0.057)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.145 (n=576)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.251` → IC=+0.230 (n=1038)
...
- **PATRÓN** `volumen_spike_ratio` > `1.6912` → IC=+0.120 (n=1109)

  - _Acción_: Kelly boost +0.60€ cuando `volumen_spike_ratio` > 1.6912 (IC base=+0.117)

- **PATRÓN** `libro_liquidez` > `2728.0734` → IC=+0.154 (n=516)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 2728.0734 (IC base=+0.117)

- **PATRÓN** `ballena_activa_n` < `147.0` → IC=+0.182 (n=171)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 147.0 (IC base=+0.117)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.233 (n=1209)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0062 (IC base=+0.225)

- **PATRÓN** `sigma_h` > `0.0038` → IC=+0.231 (n=1375)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0038 (IC base=+0.225)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.235 (n=1260)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.225)

- **PATRÓN** `ibs_20min` < `0.5` → IC=+0.287 (n=1382)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5 (IC base=+0.225)

- **PATRÓN** `dist_vwap_pct` < `0.1561` → IC=+0.199 (n=903)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1561 (IC base=+0.225)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6646` → IC=+0.280 (n=302)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6646 (IC base=+0.225)
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.283 (n=178)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5455` → IC=+0.337 (n=256)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5455 (IC base=+0.278)

- **PATRÓN** `volumen_pendiente_norm` < `0.2348` → IC=+0.233 (n=129)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.378` → IC=+0.321 (n=26)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.378 (IC base=+0.278)

- **PATRÓN** `volumen_spike_ratio` < `2.3561` → IC=+0.272 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.3561 (IC base=+0.278)
...
- **PATRÓN** `sigma_h` < `0.0033` → IC=+0.224 (n=114)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0033 (IC base=+0.147)

- **PATRÓN** `drift_60min` |x|≤ `0.0556` → IC=+0.200 (n=38)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0556 (IC base=+0.147)
...
- **PATRÓN** `dist_vwap_pct` > `0.1953` → IC=+0.241 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1953 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.578` → IC=+0.264 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.578 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `0.8856` → IC=+0.216 (n=100)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.8856 (IC base=+0.147)

- **PATRÓN** `volumen_pendiente_norm` < `0.1193` → IC=+0.315 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1193 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` < `2.2674` → IC=+0.290 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.2674 (IC base=+0.147)

- **PATRÓN** `volumen_spike_ratio` > `1.4319` → IC=+0.274 (n=60)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1743` → IC=+0.163 (n=158)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.1743 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.198 (n=114)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.1817` → IC=+0.161 (n=181)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.1817 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.414` → IC=+0.227 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.414 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.931` → IC=+0.145 (n=218)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` < 2.931 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `0.703` → IC=+0.198 (n=104)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` < 0.703 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` > `1.2477` → IC=+0.154 (n=79)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.2477 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` < `0.1084` → IC=+0.298 (n=82)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1084 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` < `1.9701` → IC=+0.278 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9701 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.7052` → IC=+0.283 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7052 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `1397.8324` → IC=+0.243 (n=107)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1397.8324 (IC base=+0.136)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.288 (n=347)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0073 (IC base=+0.261)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.281 (n=318)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.261)

- **PATRÓN** `ibs_20min` < `0.186` → IC=+0.363 (n=232)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.186 (IC base=+0.261)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.326` → IC=+0.367 (n=28)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.326 (IC base=+0.261)

- **PATRÓN** `volumen_spike_ratio` < `1.5056` → IC=+0.283 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5056 (IC base=+0.261)

- **PATRÓN** `volumen_spike_ratio` > `3.1663` → IC=+0.283 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.1663 (IC base=+0.261)

- **PATRÓN** `libro_liquidez` > `2678.9499` → IC=+0.263 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2678.9499 (IC base=+0.261)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.238 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 54.0 (IC base=+0.261)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.150 (n=184)

- **PATRÓN** `drift_60min` |x|≤ `0.0923` → IC=+0.152 (n=21)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.0923 (IC base=-0.008)

- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.178 (n=150)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0039 (IC base=+0.122)

- **PATRÓN** `drift_60min` |x|≤ `0.0785` → IC=+0.250 (n=50)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5` → IC=+0.145 (n=150)

  - _Acción_: Kelly boost +0.72€ cuando `ibs_20min` < 0.5 (IC base=+0.122)

- **PATRÓN** `dist_vwap_pct` < `0.1765` → IC=+0.136 (n=160)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.1765 (IC base=+0.122)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.52` → IC=+0.149 (n=35)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 11.52 (IC base=+0.122)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.046` → IC=+0.130 (n=133)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` < 6.046 (IC base=+0.122)

- **PATRÓN** `volumen_regimen` < `1.4038` → IC=+0.158 (n=150)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.4038 (IC base=+0.122)

- **PATRÓN** `volumen_pendiente_norm` < `0.1509` → IC=+0.138 (n=139)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_pendiente_norm` < 0.1509 (IC base=+0.122)

- **PATRÓN** `volumen_spike_ratio` < `2.817` → IC=+0.167 (n=148)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.817 (IC base=+0.122)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.150 (n=184)

  - _Acción_: Kelly boost +0.75€ cuando `libro_spread` < 0.01 (IC base=+0.122)

- **PATRÓN** `libro_liquidez` > `7518.5292` → IC=+0.145 (n=150)

  - _Acción_: Kelly boost +0.72€ cuando `libro_liquidez` > 7518.5292 (IC base=+0.122)

### GBM_LATE_5M#BTC#5min
- **PATRÓN** `sigma_h` < `0.0032` → IC=+0.149 (n=92)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0032 (IC base=+0.113)

- **PATRÓN** `drift_60min` |x|≤ `0.0685` → IC=+0.258 (n=31)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0685 (IC base=+0.113)

- **PATRÓN** `hora_utc` < `9.0` → IC=+0.129 (n=68)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 9.0 (IC base=+0.113)

- **PATRÓN** `ibs_20min` < `0.5387` → IC=+0.149 (n=92)

  - _Acción_: Kelly boost +0.74€ cuando `ibs_20min` < 0.5387 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.278` → IC=+0.125 (n=94)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 10.278 (IC base=+0.113)

- **PATRÓN** `volumen_pendiente_norm` > `0.1563` → IC=+0.203 (n=35)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1563 (IC base=+0.113)

- **PATRÓN** `volumen_spike_ratio` < `2.7111` → IC=+0.149 (n=92)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 2.7111 (IC base=+0.113)
...
- **FILTRO** `libro_liquidez` < `2239.29` → IC=-0.220 (n=23)

  - _Acción_: SKIP cuando `libro_liquidez` < 2239.29
  - _Potencial_: sin este filtro IC_bueno=+0.154 (n=24)

- **PATRÓN** `libro_liquidez` > `2239.29` → IC=+0.154 (n=24)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 2239.29 (IC base=-0.031)
...
- **PATRÓN** `ibs_20min` < `0.1477` → IC=+0.135 (n=50)

  - _Acción_: Kelly boost +0.67€ cuando `ibs_20min` < 0.1477 (IC base=+0.058)
...
- **FILTRO** `hora_utc` > `9.0` → IC=-0.161 (n=54)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 9.0
  - _Potencial_: sin este filtro IC_bueno=+0.086 (n=27)
...
- **FILTRO** `drift_7min_pct` |x|> `0.1097` → IC=-0.167 (n=43)
...
- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.262 (n=40)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=122)

- **FILTRO** `py_entrada` > `0.6` → IC=-0.207 (n=97)
...
- **PATRÓN** `py_entrada` < `0.52` → IC=+0.123 (n=67)
  ...
  - _Acción_: Kelly boost +0.62€ cuando `py_entrada` < 0.52 (IC base=-0.051)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.281 (n=80)
...
- **FILTRO** `py_entrada` > `0.66` → IC=-0.247 (n=73)
...
- **PATRÓN** `py_entrada` < `0.5` → IC=+0.158 (n=77)
  ...
  - _Acción_: Kelly boost +0.79€ cuando `py_entrada` < 0.5 (IC base=-0.030)
...
- **PATRÓN** `ibs_15` < `0.4545` → IC=+0.127 (n=379)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.64€ cuando `ibs_15` < 0.4545 (IC base=+0.080)

- **PATRÓN** `dist_vwap_pct` > `0.4823` → IC=+0.156 (n=62)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.4823 (IC base=+0.080)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0172` → IC=+0.123 (n=67)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.62€ cuando `pct_spot_vs_ref` |x|≤ 0.0172 (IC base=+0.078)

- **PATRÓN** `sigma_h` < `0.003` → IC=+0.133 (n=88)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.003 (IC base=+0.078)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.758` → IC=+0.267 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.758 (IC base=+0.078)
...
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.141 (n=90)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0049 (IC base=+0.135)

- **PATRÓN** `sigma_h` > `0.004` → IC=+0.200 (n=68)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.004 (IC base=+0.135)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1157` → IC=+0.134 (n=91)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.67€ cuando `delta_ratio_macro` |x|> 0.1157 (IC base=+0.135)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.142 (n=93)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` > 7.0 (IC base=+0.135)

- **PATRÓN** `hora_utc` < `14.0` → IC=+0.153 (n=93)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.76€ cuando `hora_utc` < 14.0 (IC base=+0.135)

- **PATRÓN** `ibs_15` < `0.3424` → IC=+0.164 (n=102)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.82€ cuando `ibs_15` < 0.3424 (IC base=+0.135)

- **PATRÓN** `ibs_15` > `0.0304` → IC=+0.173 (n=102)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.87€ cuando `ibs_15` > 0.0304 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` > `0.4732` → IC=+0.265 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4732 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` < `23.05` → IC=+0.189 (n=101)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` < 23.05 (IC base=+0.135)
...
- **PATRÓN** `drift_15min` |x|≤ `0.4028` → IC=+0.131 (n=63)

  - _Acción_: Kelly boost +0.65€ cuando `drift_15min` |x|≤ 0.4028 (IC base=+0.073)

- **PATRÓN** `ballena_activa_n` < `3.0` → IC=+0.125 (n=62)

  - _Acción_: Kelly boost +0.62€ cuando `ballena_activa_n` < 3.0 (IC base=+0.073)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1098` → IC=+0.223 (n=45)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1098 (IC base=+0.109)

- **PATRÓN** `drift_15min` |x|≤ `0.7446` → IC=+0.122 (n=133)

  - _Acción_: Kelly boost +0.61€ cuando `drift_15min` |x|≤ 0.7446 (IC base=+0.109)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0793` → IC=+0.152 (n=133)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.76€ cuando `delta_ratio_macro` |x|> 0.0793 (IC base=+0.109)
...
- **PATRÓN** `ibs_15` < `0.2353` → IC=+0.196 (n=90)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.98€ cuando `ibs_15` < 0.2353 (IC base=+0.109)

- **PATRÓN** `dist_vwap_pct` > `0.1272` → IC=+0.229 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1272 (IC base=+0.109)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.393` → IC=+0.158 (n=112)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` < 5.393 (IC base=+0.109)

- **PATRÓN** `libro_liquidez` > `2607.8903` → IC=+0.161 (n=119)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 2607.8903 (IC base=+0.109)
...
- **FILTRO** `sigma_ewma_delta_pct` > `12.849` → IC=-0.155 (n=227)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=925)

- **PATRÓN** `ibs_15` > `0.6371` → IC=+0.235 (n=115)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6371 (IC base=-0.045)
...
- **FILTRO** `hora_utc` > `7.0` → IC=-0.153 (n=93)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 7.0
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_NO, IBS < 0.3424 sube el IC de +0.135 a +0.164 en UPDOWN_GBM#ETH#15min (n=102). Ya aplicado como kelly_boost=+0.82€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_NO, IBS > 0.0304 sube el IC de +0.135 a +0.173 en UPDOWN_GBM#ETH#15min (n=102). Ya aplicado como kelly_boost=+0.87€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.2353 sube el IC de +0.109 a +0.196 en UPDOWN_GBM#XRP#15min (n=90). Ya aplicado como kelly_boost=+0.98€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.6371 sube el IC de -0.045 a +0.235 en UPDOWN_GBM_15M_TARDIO (n=115). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 3424 | -0.114 | -547.54€ | 2 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 463 | -0.023 | -17.53€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 2961 | -0.128 | -530.01€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 379 | -0.188 | -103.48€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 379 | -0.188 | -103.48€ | 1 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 838 | +0.004 | -100.12€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 838 | +0.004 | -100.12€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP | 708 | -0.225 | -136.86€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP#5min | 708 | -0.225 | -136.86€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 14764 | +0.115 | -854.58€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3432 | +0.183 | -95.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 99 | -0.104 | -47.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 8624 | +0.086 | -737.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2609 | +0.132 | +26.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 1455 | +0.030 | -321.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 13 | -0.065 | -2.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 1438 | +0.033 | -313.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 3343 | +0.141 | +3.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 937 | +0.198 | -28.23€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 41 | -0.105 | -20.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 1437 | +0.112 | -10.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 928 | +0.139 | +63.26€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 1452 | +0.054 | -256.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 9 | -0.021 | -3.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 1442 | +0.055 | -250.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 3614 | +0.127 | -39.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1267 | +0.163 | -15.36€ | 0 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 1433 | +0.104 | -19.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 902 | +0.117 | +4.10€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL | 3449 | +0.136 | -202.14€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1194 | +0.198 | -47.81€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 40 | +0.000 | -9.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 1436 | +0.084 | -104.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 779 | +0.141 | -40.57€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 1451 | +0.124 | -39.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 12 | +0.043 | +1.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 1438 | +0.125 | -39.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 3501 | +0.159 | -341.98€ | 2 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 3501 | +0.159 | -341.98€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 868 | +0.158 | -110.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 868 | +0.158 | -110.43€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 777 | +0.219 | -41.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 777 | +0.219 | -41.42€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 6132 | +0.188 | -585.88€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 6132 | +0.188 | -585.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 1148 | +0.096 | -261.56€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 1148 | +0.096 | -261.56€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 942 | +0.249 | -7.91€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 942 | +0.249 | -7.91€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 1078 | +0.151 | -158.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 1078 | +0.151 | -158.66€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 986 | +0.221 | -42.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 986 | +0.221 | -42.75€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 956 | +0.245 | -12.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 956 | +0.245 | -12.48€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 1022 | +0.185 | -102.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 1022 | +0.185 | -102.52€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 2199 | +0.150 | +125.75€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 2199 | +0.150 | +125.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 1086 | +0.157 | +76.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 1086 | +0.157 | +76.20€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 1113 | +0.142 | +49.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 1113 | +0.142 | +49.55€ | 0 | 6 |
...
| ✅ GBM_LATE_15M | 4591 | +0.086 | +1625.77€ | 0 | 14 |
| ✅ GBM_LATE_15M#15min | 4591 | +0.086 | +1625.77€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#DOGE | 805 | +0.189 | +544.60€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 805 | +0.189 | +544.60€ | 0 | 17 |
| ✅ GBM_LATE_15M#ETH |
93€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB#5min | 365 | +0.018 | +7.93€ | 1 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC | 308 | +0.042 | +9.87€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC#5min | 308 | +0.042 | +9.87€ | 1 | 2 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE | 372 | +0.029 | +6.87€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#DOGE#5min | 372 | +0.029 | +6.87€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH | 372 | +0.003 | -0.27€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#ETH#5min | 372 | +0.003 | -0.27€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL | 376 | +0.024 | +5.73€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#SOL#5min | 376 | +0.024 | +5.73€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP | 359 | +0.015 | -0.60€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#XRP#5min | 359 | +0.015 | -0.60€ | 0 | 0 |
...
| ✅ STREAK_MOM_5M | 608 | +0.021 | +2.35€ | 1 | 0 |
| ✅ STREAK_MOM_5M#5min | 608 | +0.021 | +2.35€ | 0 | 0 |
...
| ✅ STRUCT_NO_15M | 1880 | +0.017 | -1.50€ | 0 | 0 |
| ✅ STRUCT_NO_15M#15min | 1880 | +0.017 | -1.50€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC | 717 | +0.009 | -6.83€ | 0 | 0 |
| ✅ STRUCT_NO_15M#BTC#15min | 717 | +0.009 | -6.83€ | 1 | 0 |
...
| ✅ UPDOWN_GBM | 2288 | +0.024 | +148.09€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 990 | +0.088 | +180.44€ | 1 | 6 |
| ✅ UPDOWN_GBM#240min | 121 | +0.012 | -0.86€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 877 | -0.020 | -19.49€ | 4 | 0 |
| ✅ UPDOWN_GBM#60min | 253 | -0.025 | -11.49€ | 4 | 1 |
| ✅ UPDOWN_GBM#BNB | 100 | +0.108 | +22.42€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 93 | +0.132 | +24.05€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#5min | 6 | -0.075 | -2.16€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 428 | +0.012 | +8.05€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 124 | +0.040 | -6.62€ | 2 | 6 |
| ✅ UPDOWN_GBM#BTC#240min | 37 | +0.064 | +2.33€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#5min | 164 | +0.036 | +20.24€ | 2 | 3 |
| ✅ UPDOWN_GBM#BTC#60min | 85 | -0.063 | -9.73€ | 1 | 0 |
| ✅ UPDOWN_GBM#BTC#daily | 18 | -0.135 | +1.83€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 304 | +0.003 | +1.14€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 62 | +0.125 | +16.82€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 242 | -0.029 | -15.68€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 530 | +0.053 | +49.29€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 227 | +0.120 | +49.17€ | 1 | 13 |
| ✅ UPDOWN_GBM#ETH#240min | 37 | +0.038 | +0.09€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 149 | +0.010 | +1.31€ | 4 | 3 |
| ✅ UPDOWN_GBM#ETH#60min | 102 | +0.010 | -0.89€ | 0 | 2 |
| 🚫 UPDOWN_GBM#ETH#daily | 15 | -0.154 | -0.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 417 | -0.008 | +0.30€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 179 | +0.019 | +4.31€ | 1 | 2 |
...
| ✅ UPDOWN_GBM#XRP | 507 | +0.026 | +68.72€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 305 | +0.099 | +92.72€ | 0 | 15 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 1693 | -0.049 | +192.29€ | 2 | 3 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 1693 | -0.049 | +192.29€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 547 | -0.012 | +112.37€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 547 | -0.012 | +112.37€ | 1 | 1 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 527 | -0.035 | +77.68€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 527 | -0.035 | +77.68€ | 4 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.134) — sin ventaja clara. oversold(IBS<0.3): IC=-0.016 n=804 | neutral: IC=+0.006 n=770 | overbought(IBS>0.7): IC=+0.118 n=1013
  - _Datos_: n=2775 IC=+0.043 PNL=+233.59€
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=179 IC=+0.019 PNL=+4.31€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.103 n=39766 | tras_1loss IC=+0.060 n=28221 | tras_2loss IC=+0.018 n=12374/40 | gap=+0.084 (umbral 0.05)
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=+0.067 n=176 | contrario_BTC IC=-0.029 n=102/40 | gap=-0.096 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=2080 IC=+0.018 PNL=+110.57€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=2080 IC=+0.018 PNL=+110.57€
...
**〰️ H-CUSTOM-GBM-SIGMA-BAJO** — GBM con sigma_h muy bajo (<0.0018/h, p1 real) — ¿mercado dormido = más predecible?
  ...
  - _Estado_: n=147 IC=+0.064 PNL=+24.85€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=147 IC=+0.064 PNL=+24.85€
...
**🟡 H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.083 > 0.08 con n=616 PNL=+86.82€
  - _Datos_: n=616 IC=+0.083 PNL=+86.82€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=108 IC=+0.018 PNL=+9.38€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=108 IC=+0.018 PNL=+9.38€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=484 IC=+0.097 PNL=+125.70€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=484 IC=+0.097 PNL=+125.70€

**〰️ H-CUSTOM-POLY-DRIFT-CONFIRM** — poly_drift_5obs: ¿el precio YES interno de Polymarket confirma nuestra señal?
  - _Hipótesis_: Feature nueva 2026-06-27: drift del precio YES en Polymarket en últimas 5 obs (~5min). Si poly_drift<0 y decidimos BUY_NO (o poly_drift>0 y BUY_YES) → confluencia. Si diverge → reducción de stake. Hipótesis: confluencia Binance+Polymarket mejora IC; divergencia empeora.
  - _Umbral_: n≥40 en confluencia vs divergencia para validar el boost ×1.1
  - _Acción_: Si IC_confluencia>IC_divergencia con n≥40 → mantener el boost. Si no → retirar.
  - _Estado_: n=303 IC=+0.031 PNL=-5.81€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=303 IC=+0.031 PNL=-5.81€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=334 IC=+0.024 PNL=+5.09€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=334 IC=+0.024 PNL=+5.09€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.150 > 0.1 con n=564 PNL=+170.39€
  - _Datos_: n=564 IC=+0.150 PNL=+170.39€
...
**🟡 H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.109 > 0.1 con n=177 PNL=+58.70€
  - _Datos_: n=177 IC=+0.109 PNL=+58.70€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=1403 IC=-0.149 PNL=+53.60€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1403 IC=-0.149 PNL=+53.60€
...
**〰️ H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: n=811 IC=+0.156 PNL=+430.15€ — sin señal clara aún (umbral IC: min=0.2 max=None)
  - _Datos_: n=811 IC=+0.156 PNL=+430.15€
...
**〰️ H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: n=562 IC=+0.025 PNL=+146.74€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=562 IC=+0.025 PNL=+146.74€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.246 > 0.08 con n=663 PNL=-70.56€
  - _Datos_: n=663 IC=+0.246 PNL=-70.56€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=1148 IC=+0.096 PNL=-261.56€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=1148 IC=+0.096 PNL=-261.56€
