# Hipótesis automáticas — 2026-07-12 19:10 UTC
_Generado por shadow_postmortem.py sobre 11312 resoluciones (PNL=+2142.01€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.231 (n=191)
...
- **PATRÓN** `py_entrada` > `0.585` → IC=+0.252 (n=385)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.585 (IC base=+0.220)

- **PATRÓN** `libro_liquidez` > `2848.6588` → IC=+0.239 (n=327)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2848.6588 (IC base=+0.220)
...
- **PATRÓN** `py_entrada` > `0.605` → IC=+0.241 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.605 (IC base=+0.194)

- **PATRÓN** `libro_liquidez` > `6091.7099` → IC=+0.191 (n=82)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 6091.7099 (IC base=+0.194)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.123 (n=1371)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.067` → IC=+0.357 (n=19)
...
- **PATRÓN** `dist_vwap_pct` < `0.134` → IC=+0.177 (n=29)
  ...
  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.134 (IC base=+0.087)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.194 (n=338)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` > 0.0048 (IC base=+0.177)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.186 (n=256)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.5381` → IC=+0.305 (n=39)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5381 (IC base=+0.177)

- **PATRÓN** `dist_vwap_pct` < `0.132` → IC=+0.212 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.132 (IC base=+0.177)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.417` → IC=+0.321 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.417 (IC base=+0.177)
...
- **PATRÓN** `sigma_h` > `0.0028` → IC=+0.219 (n=94)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.1528` → IC=+0.167 (n=31)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.1528 (IC base=+0.162)
...
- **PATRÓN** `sigma_h` > `0.0135` → IC=+0.130 (n=144)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` > 0.0135 (IC base=+0.082)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.125 (n=214)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` < 8.0 (IC base=+0.082)

- **PATRÓN** `dist_vwap_pct` > `0.4603` → IC=+0.132 (n=36)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.4603 (IC base=+0.082)

- **PATRÓN** `dist_vwap_pct` < `0.115` → IC=+0.143 (n=68)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.115 (IC base=+0.082)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.064` → IC=+0.333 (n=28)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.125 (n=86)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=+0.098 (n=354)

- **PATRÓN** `ibs_15` > `0.7168` → IC=+0.158 (n=220)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.79€ cuando `ibs_15` > 0.7168 (IC base=+0.017)

- **PATRÓN** `dist_vwap_pct` > `0.1361` → IC=+0.171 (n=86)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.1361 (IC base=+0.017)

- **PATRÓN** `dist_vwap_pct` < `0.272` → IC=+0.185 (n=87)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` < 0.272 (IC base=+0.017)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 1003 | +0.184 | +15.52€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 645 | +0.211 | +11.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 40 | +0.048 | +1.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 150 | +0.099 | -10.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 168 | +0.182 | +13.34€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 294 | +0.186 | -2.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 215 | +0.205 | -7.48€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 350 | +0.199 | +16.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 214 | +0.222 | +15.82€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 14 | +0.044 | +0.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 66 | +0.162 | +1.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 56 | +0.172 | -0.34€ | 0 | 2 |
| ✅ GBM_LATE_15M | 3667 | +0.113 | +1228.25€ | 0 | 10 |
| ✅ GBM_LATE_15M#15min | 3667 | +0.113 | +1228.25€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 898 | +0.071 | +124.68€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 898 | +0.071 | +124.68€ | 0 | 2 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1017 | +0.165 | +592.11€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1017 | +0.165 | +592.11€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 267 | +0.162 | +172.01€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 267 | +0.162 | +172.01€ | 0 | 9 |
...
| ✅ GBM_LATE_15M_TARDIO | 1171 | +0.080 | +285.73€ | 1 | 8 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1171 | +0.080 | +285.73€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 270 | +0.081 | +42.70€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 270 | +0.081 | +42.70€ | 0 | 2 |
...
| ✅ UPDOWN_GBM | 1334 | -0.007 | +23.65€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1078 | +0.022 | +61.12€ | 1 | 4 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Datos_: n=944 IC=+0.007 PNL=+48.88€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.092 n=6409 | tras_1loss IC=+0.048 n=4765 | tras_2loss IC=+0.014 n=2127/40 | gap=+0.078 (umbral 0.05)
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=+0.081 n=501/40 PNL=+118.88€ | H=15h UTC: IC=+0.097 n=502/40 PNL=+126.74€ | H=17h UTC: IC=+0.130 n=444/40 PNL=+146.72€ | H=19h UTC: IC=+0.071 n=397/40 PNL=+59.53€
...
**〰️ H-24H-GBM-BUYYES-TARDE** — GBM BUY_YES en tarde europea (15-19h UTC) — señal alcista sostenida
  ...
  - _Estado_: n=214 IC=+0.000 PNL=+5.54€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=214 IC=+0.000 PNL=+5.54€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1207 IC=-0.000 PNL=+37.65€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1207 IC=-0.000 PNL=+37.65€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=615 IC=+0.017 PNL=+24.96€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=615 IC=+0.017 PNL=+24.96€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=559 IC=+0.051 PNL=+43.97€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=559 IC=+0.051 PNL=+43.97€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8854 IC=+0.098 PNL=+2217.89€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8854 IC=+0.098 PNL=+2217.89€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=762 IC=-0.016 PNL=-5.51€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=762 IC=-0.016 PNL=-5.51€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.167 > 0.1 con n=277 PNL=+98.16€
  - _Datos_: n=277 IC=+0.167 PNL=+98.16€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.167 > 0.08 con n=211 PNL=+71.21€
  - _Datos_: n=211 IC=+0.167 PNL=+71.21€
...
**🔶 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA en BTC (IC=+0.225 n=89) pero sin cruzar ≥2 pares más — ETH: n=110 IC=+0.134 ✓; SOL: n=7 IC=-0.019; XRP: n=5 IC=+0.018
...
**⏳ H-CUSTOM-BTC15-SIGMA-ACCEL** — GBM_LATE_15M BTC — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: 16/40 ops en el filtro definido (IC actual=+0.178 PNL=+19.03€)
  - _Datos_: n=16 IC=+0.178 PNL=+19.03€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.321 > 0.08 con n=76 PNL=+9.66€
  - _Datos_: n=76 IC=+0.321 PNL=+9.66€
