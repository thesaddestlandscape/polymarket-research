# Hipótesis automáticas — 2026-07-12 16:52 UTC
_Generado por shadow_postmortem.py sobre 11174 resoluciones (PNL=+2076.66€)_
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.156 (n=425)

  - _Acción_: Kelly boost +0.78€ cuando `libro_spread` < 0.01 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `5946.4255` → IC=+0.163 (n=170)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 5946.4255 (IC base=+0.148)
...
- **PATRÓN** `hora_utc` > `9.0` → IC=+0.230 (n=72)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `7710.8175` → IC=+0.241 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7710.8175 (IC base=+0.214)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.225 (n=38)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1590.0504` → IC=+0.232 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1590.0504 (IC base=+0.204)
...
### GBM_LATE_15M
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.122 (n=1361)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 6.0 (IC base=+0.116)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.121 (n=1357)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` < 17.0 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` > `0.5169` → IC=+0.136 (n=42)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 0.5169 (IC base=+0.116)

- **PATRÓN** `dist_vwap_pct` < `0.1286` → IC=+0.206 (n=66)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1286 (IC base=+0.116)
...
- **PATRÓN** `dist_vwap_pct` > `0.6749` → IC=+0.206 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6749 (IC base=+0.092)

### GBM_LATE_15M#SOL#15min
- **PATRÓN** `sigma_h` < `0.0161` → IC=+0.138 (n=363)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `dist_vwap_pct` > `0.536` → IC=+0.132 (n=17)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.536 (IC base=+0.123)
...
- **PATRÓN** `sigma_h` < `0.0318` → IC=+0.178 (n=309)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.89€ cuando `sigma_h` < 0.0318 (IC base=+0.163)

- **PATRÓN** `sigma_h` > `0.014` → IC=+0.176 (n=276)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.014 (IC base=+0.163)
...
- **PATRÓN** `dist_vwap_pct` < `0.1341` → IC=+0.278 (n=16)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1341 (IC base=+0.163)
...
- **PATRÓN** `dist_vwap_pct` > `0.5148` → IC=+0.263 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5148 (IC base=+0.175)

- **PATRÓN** `dist_vwap_pct` < `0.1264` → IC=+0.196 (n=54)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` < 0.1264 (IC base=+0.175)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.442` → IC=+0.348 (n=31)
...
- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.213 (n=92)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0029 (IC base=+0.162)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.198 (n=51)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` > 16.0 (IC base=+0.162)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.167 (n=106)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.125 (n=86)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=+0.103 (n=351)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.123 (n=242)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.61€ cuando `hora_utc` > 13.0 (IC base=+0.019)

- **PATRÓN** `ibs_15` > `0.5859` → IC=+0.124 (n=293)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.62€ cuando `ibs_15` > 0.5859 (IC base=+0.019)

- **PATRÓN** `dist_vwap_pct` > `0.1741` → IC=+0.196 (n=77)

  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` > 0.1741 (IC base=+0.019)

- **PATRÓN** `dist_vwap_pct` < `0.2677` → IC=+0.182 (n=86)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.2677 (IC base=+0.019)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1638` → IC=+0.132 (n=104)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.66€ cuando `drift_60min` |x|≤ 0.1638 (IC base=+0.059)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.167 (n=130)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 11.0 (IC base=+0.059)

- **PATRÓN** `ibs_15` < `0.9235` → IC=+0.155 (n=117)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.78€ cuando `ibs_15` < 0.9235 (IC base=+0.059)

- **PATRÓN** `ibs_15` > `0.6374` → IC=+0.172 (n=117)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.86€ cuando `ibs_15` > 0.6374 (IC base=+0.059)

- **PATRÓN** `dist_vwap_pct` > `0.2179` → IC=+0.315 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2179 (IC base=+0.059)
...
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS < 0.9235 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.155 n=117). Confirma señal de reversión media → alinear con BUY_YES.
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 961 | +0.181 | +7.60€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 618 | +0.211 | +11.33€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 282 | +0.187 | -2.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 206 | +0.207 | -6.61€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 341 | +0.165 | -0.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 205 | +0.205 | +3.23€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 336 | +0.192 | +10.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 205 | +0.220 | +14.67€ | 0 | 8 |
...
| ✅ GBM_LATE_15M | 3637 | +0.113 | +1205.46€ | 0 | 9 |
| ✅ GBM_LATE_15M#15min | 3637 | +0.113 | +1205.46€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 894 | +0.069 | +116.47€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 894 | +0.069 | +116.47€ | 0 | 2 |
| ✅ GBM_LATE_15M#ETH | 920 | +0.073 | +156.48€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 920 | +0.073 | +156.48€ | 0 | 2 |
| ✅ GBM_LATE_15M#SOL | 948 | +0.138 | +468.12€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 948 | +0.138 | +468.12€ | 0 | 8 |
| ✅ GBM_LATE_15M#XRP | 875 | +0.172 | +464.38€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 875 | +0.172 | +464.38€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 991 | +0.164 | +558.72€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 991 | +0.164 | +558.72€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 261 | +0.162 | +168.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 261 | +0.162 | +168.60€ | 0 | 8 |
...
| ✅ UPDOWN_GBM | 1330 | -0.006 | +27.54€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1074 | +0.023 | +65.01€ | 1 | 5 |
...
| ✅ UPDOWN_GBM#BTC | 359 | +0.013 | +9.24€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 280 | +0.053 | +22.62€ | 1 | 8 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.120) — sin ventaja clara. oversold(IBS<0.3): IC=-0.025 n=345 | neutral: IC=-0.044 n=261 | overbought(IBS>0.7): IC=+0.076 n=293
  - _Datos_: n=940 IC=+0.009 PNL=+52.78€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.091 n=6322 | tras_1loss IC=+0.046 n=4714 | tras_2loss IC=+0.013 n=2111/40 | gap=+0.079 (umbral 0.05)
...
**〰️ H-24H-GBM-BUYYES-TARDE** — GBM BUY_YES en tarde europea (15-19h UTC) — señal alcista sostenida
  ...
  - _Estado_: n=211 IC=+0.007 PNL=+10.55€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=211 IC=+0.007 PNL=+10.55€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1203 IC=+0.000 PNL=+41.54€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1203 IC=+0.000 PNL=+41.54€
...
**〰️ H-CUSTOM-BUYYES-15MIN-POSTFILTRO** — BUY_YES #15min con filtro drift_60min activo — ¿funciona en forward?
  ...
  - _Estado_: n=612 IC=+0.019 PNL=+29.97€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=612 IC=+0.019 PNL=+29.97€
...
**〰️ H-CUSTOM-BTC15-TENDENCIA** — BTC#15min — ¿el edge está decayendo?
  ...
  - _Estado_: n=280 IC=+0.053 PNL=+22.62€ — sin señal clara aún (umbral IC: min=None max=0.02)
  - _Datos_: n=280 IC=+0.053 PNL=+22.62€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=555 IC=+0.053 PNL=+47.86€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=555 IC=+0.053 PNL=+47.86€
...
**〰️ H-CUSTOM-CROSS-WINDOW-SPREAD-NEG** — GBM 15min/60min: spread negativo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: n=52 IC=+0.056 PNL=+31.69€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=52 IC=+0.056 PNL=+31.69€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=8716 IC=+0.097 PNL=+2152.55€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=8716 IC=+0.097 PNL=+2152.55€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=759 IC=-0.014 PNL=-0.50€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=759 IC=-0.014 PNL=-0.50€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.174 > 0.1 con n=274 PNL=+103.17€
  - _Datos_: n=274 IC=+0.174 PNL=+103.17€
...
**〰️ H-CUSTOM-BTC15-TARDE** — BTC#15min en tarde UTC (hora>=16) — el bolsillo rentable dentro de un subtipo mediocre
  ...
  - _Estado_: n=151 IC=+0.075 PNL=+13.18€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=151 IC=+0.075 PNL=+13.18€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=373 IC=-0.065 PNL=+66.47€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=373 IC=-0.065 PNL=+66.47€
...
**🟡 H-CUSTOM-BUYYES15-SOLO-TARDIO** — UPDOWN_GBM BUY_YES #15min solo tardío (T_h<0.2) — gate forward hacia live
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.176 > 0.08 con n=208 PNL=+76.22€
  - _Datos_: n=208 IC=+0.176 PNL=+76.22€
...
**🔶 H-CUSTOM-UPDOWNGBM-BTC15-TARDIO** — UPDOWN_GBM BTC#15min BUY_YES tardío (T_h<0.2) — lane nueva, no cubierta por GBM_LATE_15M
  ...
  - _Estado_: SEÑAL POSITIVA en BTC (IC=+0.233 n=88) pero sin cruzar ≥2 pares más — ETH: n=108 IC=+0.145 ✓; SOL: n=7 IC=-0.019; XRP: n=5 IC=+0.018
  - _Datos_: n=88 IC=+0.233 PNL=+38.98€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.162 > 0.08 con n=63 PNL=-1.62€
  - _Datos_: n=63 IC=+0.162 PNL=-1.62€
