# Hipótesis automáticas — 2026-08-25 16:55 UTC
_Generado por shadow_postmortem.py sobre 152853 resoluciones (PNL=+10157.04€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `148.86` → IC=-0.278 (n=2144)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 148.86
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=6433)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.190 (n=4941)

  - _Acción_: Kelly boost +0.95€ cuando `py_entrada` > 0.7 (IC base=+0.099)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.175 (n=1496)

  - _Acción_: Kelly boost +0.87€ cuando `libro_spread` < 0.01 (IC base=+0.099)

- **PATRÓN** `libro_liquidez` > `2382.8902` → IC=+0.177 (n=1445)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 2382.8902 (IC base=+0.099)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.227 (n=585)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.216 (n=724)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.212)

- **PATRÓN** `libro_liquidez` > `12055.0066` → IC=+0.219 (n=190)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12055.0066 (IC base=+0.212)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.137 (n=1124)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.198 (n=893)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.195 (n=4182)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.203 (n=2038)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.167 (n=1068)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.198 (n=1020)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.7` → IC=+0.199 (n=503)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.7 (IC base=+0.191)

### FAVORITO_CONFIRMADO_15MIN_EXTREMO
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.454 (n=192)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1582.3645` → IC=+0.444 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1582.3645 (IC base=+0.439)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.197 (n=10872)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.223 (n=7654)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.192)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.129 (n=2097)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.161 (n=1445)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.172 (n=920)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.249 (n=1046)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.123 (n=2059)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.298 (n=270)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.282)

- **PATRÓN** `py_entrada` > `0.865` → IC=+0.435 (n=105)
...
- **PATRÓN** `libro_liquidez` > `821.9118` → IC=+0.296 (n=302)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9463` → IC=+0.217 (n=665)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9463 (IC base=+0.069)

- **PATRÓN** `dist_vwap_pct` > `0.2046` → IC=+0.237 (n=279)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2046 (IC base=+0.069)

- **PATRÓN** `dist_vwap_pct` < `0.4965` → IC=+0.224 (n=360)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4965 (IC base=+0.069)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.077` → IC=+0.182 (n=824)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 5.077 (IC base=+0.069)

- **PATRÓN** `volumen_regimen` < `0.63` → IC=+0.232 (n=140)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.63 (IC base=+0.069)

- **PATRÓN** `volumen_regimen` > `1.0803` → IC=+0.260 (n=190)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0803 (IC base=+0.069)

- **PATRÓN** `volumen_pendiente_norm` < `0.1067` → IC=+0.135 (n=996)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_pendiente_norm` < 0.1067 (IC base=+0.069)

- **PATRÓN** `volumen_pendiente_norm` > `0.3033` → IC=+0.140 (n=145)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_pendiente_norm` > 0.3033 (IC base=+0.069)

- **PATRÓN** `volumen_spike_ratio` < `2.443` → IC=+0.140 (n=918)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 2.443 (IC base=+0.069)

- **PATRÓN** `volumen_spike_ratio` > `1.9513` → IC=+0.134 (n=695)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_spike_ratio` > 1.9513 (IC base=+0.069)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0927` → IC=+0.227 (n=108)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0927 (IC base=+0.220)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.248 (n=324)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.220)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.223 (n=323)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 16.0 (IC base=+0.220)

- **PATRÓN** `ibs_20min` > `0.9585` → IC=+0.238 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9585 (IC base=+0.220)

- **PATRÓN** `dist_vwap_pct` > `0.2345` → IC=+0.237 (n=226)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2345 (IC base=+0.220)

- **PATRÓN** `dist_vwap_pct` < `1.0067` → IC=+0.222 (n=333)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 1.0067 (IC base=+0.220)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.375` → IC=+0.267 (n=88)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.375 (IC base=+0.220)

- **PATRÓN** `volumen_regimen` < `0.714` → IC=+0.222 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.714 (IC base=+0.220)

- **PATRÓN** `volumen_regimen` > `1.099` → IC=+0.252 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.099 (IC base=+0.220)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4485` → IC=+0.261 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4485 (IC base=+0.220)

- **PATRÓN** `libro_liquidez` > `11109.7462` → IC=+0.235 (n=289)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11109.7462 (IC base=+0.220)

- **PATRÓN** `ballena_activa_n` < `371.0` → IC=+0.223 (n=218)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 371.0 (IC base=+0.220)
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.189 (n=133)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0076 (IC base=+0.130)
...
- **PATRÓN** `libro_liquidez` > `1916.3861` → IC=+0.148 (n=180)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 1916.3861 (IC base=+0.130)

- **PATRÓN** `ballena_activa_n` < `30.0` → IC=+0.134 (n=80)

  - _Acción_: Kelly boost +0.67€ cuando `ballena_activa_n` < 30.0 (IC base=+0.130)
...
- **FILTRO** `ibs_20min` > `0.48` → IC=-0.121 (n=602)
...
- **FILTRO** `sigma_ewma_delta_pct` > `4.888` → IC=-0.147 (n=259)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=948)
...
- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.194 (n=34)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0056 (IC base=+0.067)

- **PATRÓN** `drift_60min` |x|≤ `0.3737` → IC=+0.143 (n=68)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.3737 (IC base=+0.067)

- **PATRÓN** `hora_utc` > `23.0` → IC=+0.250 (n=30)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 23.0 (IC base=+0.067)

- **PATRÓN** `ibs_20min` > `0.5294` → IC=+0.160 (n=51)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.5294 (IC base=+0.067)
...
- **FILTRO** `ibs_20min` > `0.7733` → IC=-0.172 (n=269)
...
- **FILTRO** `sigma_ewma_delta_pct` > `6.547` → IC=-0.183 (n=178)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=901)
...
- **PATRÓN** `sigma_h` > `0.0073` → IC=+0.135 (n=1152)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0073 (IC base=+0.055)

- **PATRÓN** `ibs_20min` > `0.9463` → IC=+0.247 (n=847)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `5.085` → IC=+0.121 (n=1010)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` > 5.085 (IC base=+0.055)

- **PATRÓN** `volumen_regimen` > `1.1596` → IC=+0.194 (n=295)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 1.1596 (IC base=+0.055)

- **PATRÓN** `volumen_pendiente_norm` < `0.1151` → IC=+0.167 (n=1112)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` < 0.1151 (IC base=+0.055)

- **PATRÓN** `volumen_pendiente_norm` > `0.2434` → IC=+0.213 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2434 (IC base=+0.055)

- **PATRÓN** `volumen_spike_ratio` < `1.4848` → IC=+0.190 (n=382)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` < 1.4848 (IC base=+0.055)

- **PATRÓN** `volumen_spike_ratio` > `2.8724` → IC=+0.174 (n=381)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 2.8724 (IC base=+0.055)

- **PATRÓN** `ballena_activa_n` < `102.0` → IC=+0.272 (n=590)
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.288 (n=130)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1444` → IC=+0.189 (n=281)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0611` → IC=+0.174 (n=133)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.0611 (IC base=+0.180)

- **PATRÓN** `volumen_spike_ratio` > `3.8668` → IC=+0.214 (n=138)
...
- **PATRÓN** `libro_liquidez` > `1914.9184` → IC=+0.204 (n=174)
...
- **FILTRO** `ibs_20min` < `0.5294` → IC=-0.146 (n=283)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.137 (n=290)

- **FILTRO** `ibs_20min` > `0.7391` → IC=-0.161 (n=213)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7391
  - _Potencial_: sin este filtro IC_bueno=+0.025 (n=645)
...
- **PATRÓN** `ibs_20min` > `0.85` → IC=+0.247 (n=144)
...
- **PATRÓN** `volumen_regimen` < `0.8622` → IC=+0.176 (n=109)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 0.8622 (IC base=-0.003)
...
- **PATRÓN** `volumen_spike_ratio` > `1.586` → IC=+0.157 (n=135)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.586 (IC base=-0.003)

- **PATRÓN** `ballena_activa_n` < `77.0` → IC=+0.206 (n=117)
...
- **PATRÓN** `volumen_regimen` > `0.8342` → IC=+0.246 (n=345)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.8342 (IC base=+0.209)

- **PATRÓN** `volumen_pendiente_norm` < `0.0805` → IC=+0.212 (n=436)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4115` → IC=+0.248 (n=153)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4115 (IC base=+0.209)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.227 (n=576)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.209)

- **PATRÓN** `libro_liquidez` > `3099.7084` → IC=+0.243 (n=173)
...
- **PATRÓN** `sigma_h` > `0.0066` → IC=+0.183 (n=1462)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0066 (IC base=+0.157)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.163 (n=2296)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 5.0 (IC base=+0.157)

- **PATRÓN** `ibs_20min` > `0.8935` → IC=+0.259 (n=1461)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8935 (IC base=+0.157)

- **PATRÓN** `dist_vwap_pct` > `0.7907` → IC=+0.252 (n=538)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7907 (IC base=+0.157)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.235` → IC=+0.256 (n=938)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.235 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` < `1.2297` → IC=+0.162 (n=1526)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 1.2297 (IC base=+0.157)

- **PATRÓN** `volumen_regimen` > `0.6896` → IC=+0.172 (n=1363)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` > 0.6896 (IC base=+0.157)

- **PATRÓN** `volumen_pendiente_norm` > `0.1037` → IC=+0.185 (n=779)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1037 (IC base=+0.157)

- **PATRÓN** `volumen_spike_ratio` < `2.2948` → IC=+0.158 (n=1712)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 2.2948 (IC base=+0.157)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.168 (n=1788)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.01 (IC base=+0.157)

- **PATRÓN** `libro_liquidez` > `3744.3772` → IC=+0.197 (n=731)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 3744.3772 (IC base=+0.157)

- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.181 (n=973)
...
- **PATRÓN** `ibs_20min` > `0.7013` → IC=+0.209 (n=479)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.205 (n=1149)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0077 (IC base=+0.144)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.169 (n=1269)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` > 15.0 (IC base=+0.144)

- **PATRÓN** `ibs_20min` > `0.9435` → IC=+0.288 (n=1149)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9435 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `1.0541` → IC=+0.256 (n=440)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0541 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.481` → IC=+0.229 (n=1524)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.481 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` < `0.6312` → IC=+0.158 (n=597)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.6312 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` > `1.085` → IC=+0.152 (n=812)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.085 (IC base=+0.144)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8963` → IC=+0.153 (n=1480)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 1.8963 (IC base=+0.144)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.154 (n=2059)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `3103.6736` → IC=+0.191 (n=1149)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 3103.6736 (IC base=+0.144)

- **PATRÓN** `ballena_activa_n` < `171.0` → IC=+0.188 (n=1018)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4332` → IC=+0.199 (n=2240)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4332 (IC base=+0.189)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.195 (n=1024)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.209 (n=132)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.007 (IC base=+0.164)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.188 (n=399)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` > 6.0 (IC base=+0.164)

- **PATRÓN** `ibs_20min` > `0.3404` → IC=+0.216 (n=396)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3404 (IC base=+0.164)

- **PATRÓN** `dist_vwap_pct` > `0.2633` → IC=+0.246 (n=242)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2633 (IC base=+0.164)
...
- **PATRÓN** `volumen_regimen` < `0.6563` → IC=+0.189 (n=133)
  ...
  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` < 0.6563 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` > `1.1006` → IC=+0.192 (n=180)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` > 1.1006 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.1441` → IC=+0.227 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1441 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` < `2.53` → IC=+0.197 (n=348)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 2.53 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` > `1.4672` → IC=+0.203 (n=311)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4672 (IC base=+0.164)

- **PATRÓN** `libro_liquidez` > `11533.5682` → IC=+0.203 (n=264)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11533.5682 (IC base=+0.164)
...
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.277 (n=110)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.717` → IC=+0.247 (n=295)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.717 (IC base=+0.174)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.923` → IC=+0.344 (n=94)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.923 (IC base=+0.174)

- **PATRÓN** `volumen_pendiente_norm` < `0.2322` → IC=+0.174 (n=262)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` < 0.2322 (IC base=+0.174)

- **PATRÓN** `volumen_spike_ratio` < `2.1497` → IC=+0.164 (n=114)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.1497 (IC base=+0.174)

- **PATRÓN** `volumen_spike_ratio` > `3.9028` → IC=+0.172 (n=117)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 3.9028 (IC base=+0.174)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.211 (n=254)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.174)

- **PATRÓN** `libro_liquidez` > `1854.121` → IC=+0.189 (n=220)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 1854.121 (IC base=+0.174)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5023` → IC=+0.152 (n=323)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `dist_vwap_pct` < `0.7176` → IC=+0.164 (n=340)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.7176 (IC base=+0.141)
...
- **PATRÓN** `volumen_regimen` < `0.5848` → IC=+0.164 (n=108)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.5848 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` > `0.6383` → IC=+0.149 (n=289)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` > 0.6383 (IC base=+0.141)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1688` → IC=+0.179 (n=235)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` < 2.1688 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` > `1.443` → IC=+0.169 (n=267)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 1.443 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `8980.0561` → IC=+0.171 (n=147)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 8980.0561 (IC base=+0.141)
...
- **PATRÓN** `sigma_h` > `0.0105` → IC=+0.159 (n=206)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0105 (IC base=+0.088)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.131 (n=310)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` > 12.0 (IC base=+0.088)

- **PATRÓN** `ibs_20min` > `0.5` → IC=+0.161 (n=458)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.5 (IC base=+0.088)

- **PATRÓN** `dist_vwap_pct` > `1.0378` → IC=+0.263 (n=95)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0378 (IC base=+0.088)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.478` → IC=+0.228 (n=178)
...
- **PATRÓN** `ballena_activa_n` < `69.0` → IC=+0.168 (n=227)

  - _Acción_: Kelly boost +0.84€ cuando `ballena_activa_n` < 69.0 (IC base=+0.088)
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.192 (n=274)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 15.0 (IC base=+0.171)

- **PATRÓN** `ibs_20min` > `0.9655` → IC=+0.293 (n=249)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9655 (IC base=+0.171)

- **PATRÓN** `dist_vwap_pct` > `1.3407` → IC=+0.286 (n=138)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.3407 (IC base=+0.171)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.037` → IC=+0.251 (n=299)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.037 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` < `0.6141` → IC=+0.197 (n=183)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_regimen` < 0.6141 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` > `1.0445` → IC=+0.193 (n=249)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_regimen` > 1.0445 (IC base=+0.171)

- **PATRÓN** `volumen_pendiente_norm` > `0.1718` → IC=+0.248 (n=121)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1718 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` < `1.5675` → IC=+0.176 (n=220)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 1.5675 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` > `1.8456` → IC=+0.180 (n=332)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` > 1.8456 (IC base=+0.171)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.184 (n=612)

  - _Acción_: Kelly boost +0.92€ cuando `libro_spread` < 0.01 (IC base=+0.171)

- **PATRÓN** `libro_liquidez` > `3112.8962` → IC=+0.181 (n=183)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 3112.8962 (IC base=+0.171)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.153 (n=722)
...
- **PATRÓN** `sigma_h` < `0.0037` → IC=+0.208 (n=245)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0037 (IC base=+0.145)

- **PATRÓN** `drift_60min` |x|≤ `0.3378` → IC=+0.173 (n=487)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.3378 (IC base=+0.145)
...
- **PATRÓN** `ibs_20min` < `0.6296` → IC=+0.159 (n=487)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` < 0.6296 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.6108` → IC=+0.154 (n=160)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.6108 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` < `0.7583` → IC=+0.148 (n=611)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.7583 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.913` → IC=+0.164 (n=278)

  - _Acción_: Kelly boost +0.82€ cuando `sigma_ewma_delta_pct` > 2.913 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` < `1.2459` → IC=+0.163 (n=553)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.2459 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` > `0.7234` → IC=+0.147 (n=494)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` > 0.7234 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` > `0.0724` → IC=+0.172 (n=269)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.0724 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` < `2.5254` → IC=+0.170 (n=547)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` < 2.5254 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` > `1.4141` → IC=+0.148 (n=547)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.4141 (IC base=+0.145)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.153 (n=722)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `8260.3607` → IC=+0.160 (n=553)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 8260.3607 (IC base=+0.145)
...
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.195 (n=201)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0039 (IC base=+0.145)

- **PATRÓN** `drift_60min` |x|≤ `0.3097` → IC=+0.163 (n=301)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.82€ cuando `drift_60min` |x|≤ 0.3097 (IC base=+0.145)
...
- **PATRÓN** `ibs_20min` < `0.1373` → IC=+0.211 (n=133)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1373 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` > `0.6733` → IC=+0.189 (n=43)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.6733 (IC base=+0.145)

- **PATRÓN** `dist_vwap_pct` < `0.2288` → IC=+0.146 (n=283)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.2288 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.149` → IC=+0.147 (n=148)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` > 3.149 (IC base=+0.145)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.369` → IC=+0.156 (n=303)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` < 6.369 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` < `1.1988` → IC=+0.153 (n=301)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 1.1988 (IC base=+0.145)

- **PATRÓN** `volumen_regimen` > `0.6971` → IC=+0.157 (n=269)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 0.6971 (IC base=+0.145)

- **PATRÓN** `volumen_pendiente_norm` > `0.0733` → IC=+0.201 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0733 (IC base=+0.145)

- **PATRÓN** `volumen_spike_ratio` < `2.6056` → IC=+0.173 (n=301)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.6056 (IC base=+0.145)

- **PATRÓN** `libro_liquidez` > `11671.1061` → IC=+0.164 (n=269)

  - _Acción_: Kelly boost +0.82€ cuando `libro_liquidez` > 11671.1061 (IC base=+0.145)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.225 (n=674)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=2097)

- **FILTRO** `ibs_20min` > `0.2754` → IC=-0.197 (n=692)
...
- **FILTRO** `hora_utc` > `11.0` → IC=-0.127 (n=218)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 11.0
...
- **FILTRO** `ballena_activa_n` > `78.0` → IC=-0.167 (n=118)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=356)
...
- **FILTRO** `drift_20min_pct` |x|> `0.2773` → IC=-0.128 (n=216)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.142 (n=2031)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=4894)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.285 (n=1666)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.011 (n=5259)

- **FILTRO** `ibs_7min` < `0.7368` → IC=-0.232 (n=1727)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7368
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=5198)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.175 (n=2342)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=4583)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.228 (n=1985)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=6482)

- **FILTRO** `ibs_7min` > `0.7222` → IC=-0.170 (n=2115)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7222
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=6352)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.154 (n=218)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.091 (n=725)
...
- **FILTRO** `ibs_7min` < `0.9833` → IC=-0.200 (n=622)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.161 (n=290)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=1112)

- **FILTRO** `py_entrada` < `0.39` → IC=-0.247 (n=350)

  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=1052)

- **FILTRO** `ballena_activa_n` > `148.0` → IC=-0.171 (n=350)

  - _Acción_: SKIP cuando `ballena_activa_n` > 148.0
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=1052)
...
- **FILTRO** `ibs_7min` > `0.8192` → IC=-0.188 (n=370)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8192
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=1112)
...
- **FILTRO** `py_entrada` < `0.4` → IC=-0.226 (n=312)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=984)

- **FILTRO** `ibs_7min` < `0.7759` → IC=-0.193 (n=324)
...
- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.195 (n=323)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=973)
...
- **FILTRO** `py_entrada` < `0.38` → IC=-0.290 (n=274)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=864)

- **FILTRO** `ibs_7min` < `0.75` → IC=-0.240 (n=275)

  - _Acción_: SKIP cuando `ibs_7min` < 0.75
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=863)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.036` → IC=-0.188 (n=312)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.081 (n=2159)
...
- **FILTRO** `sigma_h` < `0.0045` → IC=-0.219 (n=276)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.210 (n=277)
...
- **FILTRO** `drift_15min` |x|> `1.5354` → IC=-0.140 (n=198)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=596)

- **FILTRO** `sigma_ewma_delta_pct` > `15.464` → IC=-0.167 (n=97)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 15.464
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=697)
...
| ✅ BALLENAS_TARDIAS | 6320 | -0.098 | -822.15€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 657 | -0.086 | -112.53€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 5663 | -0.100 | -709.61€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 998 | +0.020 | -106.77€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 998 | +0.020 | -106.77€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 28811 | +0.114 | -1884.86€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 5458 | +0.187 | -182.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 107 | -0.105 | -50.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 20168 | +0.094 | -1623.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3078 | +0.122 | -29.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 3385 | +0.063 | -582.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 19 | -0.068 | +0.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 3361 | +0.064 | -577.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 6008 | +0.135 | -123.02€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1532 | +0.197 | -84.89€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 3360 | +0.110 | -75.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1074 | +0.132 | +59.80€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 3393 | +0.076 | -463.00€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 19 | +0.023 | -2.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 3373 | +0.077 | -458.20€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 6444 | +0.131 | -49.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1984 | +0.171 | +1.65€ | 0 | 8 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 3357 | +0.116 | -26.91€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1091 | +0.105 | -15.49€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 6196 | +0.131 | -420.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1887 | +0.202 | -96.38€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 46 | +0.000 | -9.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 3350 | +0.092 | -241.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 913 | +0.132 | -73.32€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 3385 | +0.104 | -245.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 17 | -0.022 | -0.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 3367 | +0.105 | -243.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 5433 | +0.176 | -398.23€ | 3 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 5433 | +0.176 | -398.23€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 1357 | +0.162 | -157.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 1357 | +0.162 | -157.29€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 1217 | +0.233 | -30.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 1217 | +0.233 | -30.45€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 1307 | +0.191 | -89.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 1307 | +0.191 | -89.61€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_EXTREMO | 274 | +0.442 | +0.97€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_EXTREMO#15min | 274 | +0.442 | +0.97€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_EXTREMO#SOL | 64 | +0.439 | +1.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_EXTREMO#SOL#15