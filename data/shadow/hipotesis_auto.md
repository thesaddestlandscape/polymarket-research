# Hipótesis automáticas — 2026-07-15 20:12 UTC
_Generado por shadow_postmortem.py sobre 15868 resoluciones (PNL=+3210.66€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.195 (n=756)
...
- **PATRÓN** `py_entrada` > `0.695` → IC=+0.297 (n=378)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.191)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.195 (n=949)

  - _Acción_: Kelly boost +0.98€ cuando `libro_spread` < 0.01 (IC base=+0.191)

- **PATRÓN** `libro_liquidez` > `4419.7043` → IC=+0.194 (n=540)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 4419.7043 (IC base=+0.191)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.191 (n=348)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.171 (n=1025)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.204 (n=174)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `7271.6387` → IC=+0.213 (n=172)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7271.6387 (IC base=+0.198)
...
- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.134 (n=1196)
...
- **PATRÓN** `dist_vwap_pct` > `0.3066` → IC=+0.130 (n=25)
  ...
  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` > 0.3066 (IC base=+0.077)
...
- **PATRÓN** `sigma_h` < `0.0115` → IC=+0.142 (n=300)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0115 (IC base=+0.129)

- **PATRÓN** `sigma_h` > `0.0093` → IC=+0.135 (n=450)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.3501` → IC=+0.135 (n=50)
  ...
  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` > 0.3501 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` < `0.5367` → IC=+0.140 (n=162)

  - _Acción_: Kelly boost +0.70€ cuando `dist_vwap_pct` < 0.5367 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.722` → IC=+0.135 (n=143)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` < 7.722 (IC base=+0.165)

### GBM_LATE_15M_ESPACIO_ATR
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.224 (n=313)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0108 (IC base=+0.162)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.177 (n=496)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.4426` → IC=+0.194 (n=481)
...
- **PATRÓN** `sigma_h` > `0.0028` → IC=+0.142 (n=171)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` > 0.0028 (IC base=+0.118)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.167 (n=121)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` > 12.0 (IC base=+0.118)

- **PATRÓN** `dist_vwap_pct` > `0.132` → IC=+0.136 (n=53)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 0.132 (IC base=+0.118)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.412` → IC=+0.214 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.412 (IC base=+0.118)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.169 (n=131)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 12.0 (IC base=+0.148)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.165 (n=195)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 18.0 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` > `0.3457` → IC=+0.210 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3457 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` < `1.1747` → IC=+0.146 (n=125)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 1.1747 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.752` → IC=+0.400 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.752 (IC base=+0.148)
...
- **FILTRO** `sigma_ewma_delta_pct` > `3.165` → IC=-0.167 (n=16)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.164 (n=102)
...
- **PATRÓN** `dist_vwap_pct` < `0.2663` → IC=+0.179 (n=110)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.2663 (IC base=+0.102)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.165` → IC=+0.164 (n=102)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0148` → IC=+0.184 (n=210)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0148 (IC base=+0.110)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.130 (n=557)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` < 15.0 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` > `0.7123` → IC=+0.194 (n=109)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.7123 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.853` → IC=+0.312 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.853 (IC base=+0.110)
...
- **PATRÓN** `dist_vwap_pct` < `0.1312` → IC=+0.121 (n=56)

  - _Acción_: Kelly boost +0.60€ cuando `dist_vwap_pct` < 0.1312 (IC base=+0.088)

- **PATRÓN** `sigma_ewma_delta_pct` > `15.305` → IC=+0.206 (n=15)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 15.305 (IC base=+0.088)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0045` → IC=+0.144 (n=71)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` < 0.0045 (IC base=+0.056)

- **PATRÓN** `dist_vwap_pct` > `0.6865` → IC=+0.144 (n=43)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` > 0.6865 (IC base=+0.056)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.366` → IC=+0.364 (n=20)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.366 (IC base=+0.056)
...
- **PATRÓN** `sigma_h` > `0.0237` → IC=+0.250 (n=74)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0237 (IC base=+0.195)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.196 (n=77)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 18.0 (IC base=+0.195)
...
- **PATRÓN** `dist_vwap_pct` > `0.3325` → IC=+0.188 (n=46)
  ...
  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` > 0.3325 (IC base=+0.195)

- **PATRÓN** `dist_vwap_pct` < `0.5085` → IC=+0.183 (n=162)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.5085 (IC base=+0.195)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.517` → IC=+0.195 (n=126)
...
- **PATRÓN** `dist_vwap_pct` > `0.6858` → IC=+0.125 (n=30)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` > 0.6858 (IC base=+0.047)

- **PATRÓN** `sigma_ewma_delta_pct` < `17.186` → IC=+0.134 (n=129)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` < 17.186 (IC base=+0.047)
...
- **PATRÓN** `dist_vwap_pct` > `0.9454` → IC=+0.132 (n=17)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.9454 (IC base=+0.047)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2281 | +0.175 | -36.12€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1468 | +0.209 | +14.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 97 | +0.015 | -15.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 329 | +0.092 | -48.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 387 | +0.155 | +13.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 661 | +0.183 | -8.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 490 | +0.201 | -18.29€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#ETH |