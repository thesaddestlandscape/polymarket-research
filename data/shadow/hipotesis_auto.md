# Hipótesis automáticas — 2026-08-19 18:54 UTC
_Generado por shadow_postmortem.py sobre 79090 resoluciones (PNL=+8244.23€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `154.9` → IC=-0.255 (n=1061)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 154.9
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=3186)
...
- **FILTRO** `n_ballenas` < `4.0` → IC=-0.133 (n=246)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.095 (n=802)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.184 (n=2729)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.157 (n=1731)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.188 (n=2112)

  - _Acción_: Kelly boost +0.94€ cuando `libro_spread` < 0.02 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `4102.9784` → IC=+0.179 (n=866)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.181 (n=2875)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `11.0` → IC=+0.158 (n=486)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.75` → IC=+0.173 (n=739)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.193 (n=1878)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.217 (n=3852)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.184)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.295 (n=276)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.230 (n=867)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.247 (n=294)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.261 (n=555)
...
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.221 (n=692)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.146)

- **PATRÓN** `restante_min` < `3.81` → IC=+0.166 (n=632)

  - _Acción_: Kelly boost +0.83€ cuando `restante_min` < 3.81 (IC base=+0.146)

- **PATRÓN** `restante_min` > `4.91` → IC=+0.205 (n=636)

  - _Acción_: Kelly boost +1.00€ cuando `restante_min` > 4.91 (IC base=+0.146)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.150 (n=1980)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.199 (n=344)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.139)

- **PATRÓN** `restante_min` < `3.89` → IC=+0.151 (n=319)

  - _Acción_: Kelly boost +0.76€ cuando `restante_min` < 3.89 (IC base=+0.139)

- **PATRÓN** `restante_min` > `4.95` → IC=+0.207 (n=319)

  - _Acción_: Kelly boost +1.00€ cuando `restante_min` > 4.95 (IC base=+0.139)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.141 (n=998)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `lag_apertura_s` < `3.3` → IC=+0.207 (n=319)

  - _Acción_: Kelly boost +1.00€ cuando `lag_apertura_s` < 3.3 (IC base=+0.139)
...
- **PATRÓN** `ibs_20min` < `0.5556` → IC=+0.131 (n=2033)
...
- **PATRÓN** `volumen_regimen` < `1.295` → IC=+0.139 (n=519)
  ...
  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` < 1.295 (IC base=+0.070)

- **PATRÓN** `volumen_regimen` > `0.6895` → IC=+0.137 (n=463)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 0.6895 (IC base=+0.070)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5637` → IC=+0.257 (n=150)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.5637 (IC base=+0.070)
...
- **PATRÓN** `ballena_activa_n` < `140.0` → IC=+0.268 (n=153)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 140.0 (IC base=+0.070)
...
- **PATRÓN** `volumen_regimen` < `0.5591` → IC=+0.182 (n=20)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.5591 (IC base=-0.015)
...
- **PATRÓN** `dist_vwap_pct` > `0.2448` → IC=+0.200 (n=351)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2448 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.966` → IC=+0.271 (n=679)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.966 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` < `1.2184` → IC=+0.149 (n=751)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.2184 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` > `0.6307` → IC=+0.156 (n=750)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 0.6307 (IC base=+0.148)
...
- **PATRÓN** `volumen_spike_ratio` < `2.6369` → IC=+0.129 (n=1023)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` < 2.6369 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` > `1.5378` → IC=+0.128 (n=1162)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.5378 (IC base=+0.148)

- **PATRÓN** `libro_spread` < `0.05` → IC=+0.157 (n=1423)

  - _Acción_: Kelly boost +0.78€ cuando `libro_spread` < 0.05 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `2505.7164` → IC=+0.187 (n=634)
...
- **PATRÓN** `sigma_h` < `0.0018` → IC=+0.278 (n=43)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0018 (IC base=+0.219)

- **PATRÓN** `sigma_h` > `0.0035` → IC=+0.344 (n=43)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0035 (IC base=+0.219)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.269 (n=132)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.219)

- **PATRÓN** `ibs_20min` > `0.655` → IC=+0.291 (n=127)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.655 (IC base=+0.219)

- **PATRÓN** `dist_vwap_pct` > `0.1487` → IC=+0.305 (n=75)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1487 (IC base=+0.219)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.807` → IC=+0.319 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.807 (IC base=+0.219)

- **PATRÓN** `volumen_regimen` < `1.4274` → IC=+0.229 (n=127)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3392` → IC=+0.333 (n=16)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3392 (IC base=+0.219)

- **PATRÓN** `volumen_spike_ratio` < `1.4774` → IC=+0.300 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4774 (IC base=+0.219)

- **PATRÓN** `libro_liquidez` > `11170.0615` → IC=+0.317 (n=58)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11170.0615 (IC base=+0.219)
...
- **PATRÓN** `sigma_h` < `0.0064` → IC=+0.231 (n=1334)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.247 (n=544)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.212)

- **PATRÓN** `ibs_20min` < `0.519` → IC=+0.278 (n=1516)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.485` → IC=+0.220 (n=1423)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.485 (IC base=+0.212)

- **PATRÓN** `volumen_regimen` < `0.6112` → IC=+0.206 (n=345)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1143` → IC=+0.223 (n=745)
...
- **PATRÓN** `volumen_spike_ratio` < `1.6485` → IC=+0.246 (n=364)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6485 (IC base=+0.212)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2187` → IC=+0.187 (n=196)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.93€ cuando `drift_60min` |x|≤ 0.2187 (IC base=+0.148)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.179 (n=76)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 15.0 (IC base=+0.148)

- **PATRÓN** `hora_utc` < `3.0` → IC=+0.244 (n=76)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 3.0 (IC base=+0.148)

- **PATRÓN** `ibs_20min` < `0.5008` → IC=+0.177 (n=196)

  - _Acción_: Kelly boost +0.88€ cuando `ibs_20min` < 0.5008 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` > `0.4828` → IC=+0.160 (n=45)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` > 0.4828 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` < `0.3474` → IC=+0.158 (n=238)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.3474 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.468` → IC=+0.147 (n=120)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 2.468 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.477` → IC=+0.154 (n=212)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 5.477 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` < `1.1717` → IC=+0.187 (n=196)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` < 1.1717 (IC base=+0.148)

- **PATRÓN** `volumen_pendiente_norm` < `0.152` → IC=+0.161 (n=213)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` < 0.152 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` < `1.9176` → IC=+0.225 (n=147)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9176 (IC base=+0.148)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.168 (n=281)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.01 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `7518.5292` → IC=+0.196 (n=222)

  - _Acción_: Kelly boost +0.98€ cuando `libro_liquidez` > 7518.5292 (IC base=+0.148)
...
- **FILTRO** `libro_liquidez` < `15381.0964` → IC=-0.167 (n=19)
...
- **FILTRO** `py_entrada` > `0.58` → IC=-0.174 (n=179)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.160 (n=565)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=1198)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.271 (n=438)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=1325)

- **FILTRO** `ibs_7min` < `0.7273` → IC=-0.214 (n=439)
...
- **FILTRO** `py_entrada` > `0.65` → IC=-0.192 (n=527)
...
- **FILTRO** `py_entrada` < `0.41` → IC=-0.206 (n=134)

  - _Acción_: SKIP cuando `py_entrada` < 0.41
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=137)

- **FILTRO** `ibs_7min` < `0.9268` → IC=-0.172 (n=178)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.260 (n=73)
...
- **FILTRO** `py_entrada` > `0.59` → IC=-0.177 (n=91)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.149 (n=72)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=172)
...
- **FILTRO** `ibs_7min` > `0.8` → IC=-0.163 (n=90)
...
| ✅ FAVORITO_CONFIRMADO | 16394 | +0.113 | -1070.27€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3659 | +0.183 | -124.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 104 | -0.094 | -47.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 9964 | +0.084 | -912.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2667 | +0.131 | +14.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 1684 | +0.031 | -375.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 14 | -0.044 | +0.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 1666 | +0.034 | -369.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 3642 | +0.137 | -38.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1000 | +0.195 | -46.25€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 1654 | +0.107 | -33.37€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 946 | +0.138 | +63.25€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 1683 | +0.057 | -291.90€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 10 | +0.000 | -3.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 1672 | +0.058 | -286.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 3943 | +0.128 | -44.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1352 | +0.165 | -12.86€ | 0 | 6 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 1654 | +0.106 | -25.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 925 | +0.116 | +2.33€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 3766 | +0.132 | -247.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1271 | +0.196 | -63.87€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 44 | +0.022 | -7.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 1655 | +0.083 | -125.07€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 796 | +0.138 | -50.88€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 1676 | +0.118 | -72.43€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 12 | +0.043 | +1.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 1663 | +0.119 | -72.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 3743 | +0.163 | -350.47€ | 2 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 3743 | +0.163 | -350.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB | 942 | +0.154 | -125.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#BNB#15min | 942 | +0.154 | -125.47€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 7130 | +0.184 | -721.45€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 7130 | +0.184 | -721.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 1336 | +0.094 | -307.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 1336 | +0.094 | -307.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 1089 | +0.242 | -18.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 1089 | +0.242 | -18.44€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 1256 | +0.150 | -189.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 1256 | +0.150 | -189.39€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 1142 | +0.224 | -46.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 1142 | +0.224 | -46.74€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 1108 | +0.240 | -23.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 1108 | +0.240 | -23.71€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 1199 | +0.179 | -135.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 1199 | +0.179 | -135.49€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 2522 | +0.146 | +128.54€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 2522 | +0.146 | +128.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 1247 | +0.152 | +77.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 1247 | +0.152 | +77.38€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 1275 | +0.139 | +51.15€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 1275 | +0.139 | +51.15€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 268 | +0.115 | -2.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#15min | 24 | +0.000 | -3.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#5min | 244 | +0.126 | +0.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC | 178 | +0.100 | -3.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#15min | 24 | +0.000 | -3.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#BTC#5min | 154 | +0.115 | -0.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL | 90 | +0.141 | +1.28€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0#SOL#5min |