# Hipótesis automáticas — 2026-09-03 16:55 UTC
_Generado por shadow_postmortem.py sobre 269671 resoluciones (PNL=+25210.70€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.54` → IC=-0.297 (n=3650)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.54
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=10956)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `restante_s_al_confirmar` < `142.17` → IC=-0.281 (n=472)
...
- **FILTRO** `restante_s_al_confirmar` < `141.58` → IC=-0.314 (n=810)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=2432)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `156.29` → IC=-0.378 (n=882)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 156.29
  - _Potencial_: sin este filtro IC_bueno=-0.101 (n=1791)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.192 (n=7792)

  - _Acción_: Kelly boost +0.96€ cuando `py_entrada` > 0.7 (IC base=+0.095)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.169 (n=2006)

  - _Acción_: Kelly boost +0.84€ cuando `libro_spread` < 0.01 (IC base=+0.095)

- **PATRÓN** `libro_liquidez` > `2371.8427` → IC=+0.171 (n=1923)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 2371.8427 (IC base=+0.095)
...
- **PATRÓN** `py_entrada` < `0.34` → IC=+0.254 (n=4850)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.144 (n=1619)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.314 (n=535)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.197 (n=6070)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.204 (n=2889)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.175 (n=1505)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` < 17.0 (IC base=+0.166)

- **PATRÓN** `py_entrada` < `0.74` → IC=+0.179 (n=1567)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.176 (n=1469)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.176 (n=1577)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.248 (n=1382)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.197 (n=1497)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.208 (n=17070)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.71 (IC base=+0.189)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.152 (n=3504)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.177 (n=2492)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.182 (n=2941)

  - _Acción_: Kelly boost +0.91€ cuando `py_entrada` > 0.71 (IC base=+0.163)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.236 (n=1485)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.241 (n=1456)
...
- **PATRÓN** `lag_apertura_s` < `4.15` → IC=+0.154 (n=2280)

  - _Acción_: Kelly boost +0.77€ cuando `lag_apertura_s` < 4.15 (IC base=+0.132)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.131 (n=3446)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3559` → IC=+0.129 (n=3583)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` > 0.3559 (IC base=+0.080)

- **PATRÓN** `dist_vwap_pct` > `0.8436` → IC=+0.226 (n=177)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8436 (IC base=+0.080)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.544` → IC=+0.152 (n=1370)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 5.544 (IC base=+0.080)

- **PATRÓN** `volumen_regimen` > `1.0966` → IC=+0.247 (n=366)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0966 (IC base=+0.080)

- **PATRÓN** `volumen_pendiente_norm` > `0.1725` → IC=+0.180 (n=611)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1725 (IC base=+0.080)

- **PATRÓN** `volumen_spike_ratio` < `2.8928` → IC=+0.175 (n=2127)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 2.8928 (IC base=+0.080)

- **PATRÓN** `volumen_spike_ratio` > `1.4763` → IC=+0.172 (n=2127)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.4763 (IC base=+0.080)

- **PATRÓN** `ballena_activa_n` < `55.0` → IC=+0.201 (n=1057)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 55.0 (IC base=+0.080)

- **PATRÓN** `ibs_20min` < `0.407` → IC=+0.127 (n=3463)
...
- **PATRÓN** `volumen_regimen` < `0.6868` → IC=+0.152 (n=535)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.6868 (IC base=+0.040)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8602` → IC=+0.215 (n=602)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2778` → IC=+0.157 (n=796)
...
- **PATRÓN** `volumen_spike_ratio` < `2.6742` → IC=+0.138 (n=699)
...
- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.190 (n=447)
...
- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.220 (n=209)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.235 (n=651)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.1813` → IC=+0.220 (n=395)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.257` → IC=+0.228 (n=303)
...
- **PATRÓN** `drift_60min` |x|≤ `0.177` → IC=+0.153 (n=459)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.76€ cuando `drift_60min` |x|≤ 0.177 (IC base=+0.139)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.142 (n=619)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` > 8.0 (IC base=+0.139)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.141 (n=688)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `11.666` → IC=+0.218 (n=115)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.666 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` < `0.6207` → IC=+0.178 (n=231)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` < 0.6207 (IC base=+0.139)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3943` → IC=+0.151 (n=580)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 2.3943 (IC base=+0.139)

- **PATRÓN** `volumen_spike_ratio` > `1.41` → IC=+0.156 (n=580)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.41 (IC base=+0.139)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.141 (n=891)

  - _Acción_: Kelly boost +0.70€ cuando `libro_spread` < 0.01 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `12527.0336` → IC=+0.157 (n=458)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.287` → IC=+0.267 (n=170)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.287 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` < `0.1318` → IC=+0.166 (n=642)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` < 0.1318 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` < `3.9033` → IC=+0.158 (n=668)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 3.9033 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` > `1.8972` → IC=+0.178 (n=597)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 1.8972 (IC base=+0.164)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.190 (n=769)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.329` → IC=-0.200 (n=201)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=1539)
...
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.172 (n=1481)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0094 (IC base=+0.069)

- **PATRÓN** `ibs_20min` > `0.2852` → IC=+0.139 (n=4438)

  - _Acción_: Kelly boost +0.69€ cuando `ibs_20min` > 0.2852 (IC base=+0.069)

- **PATRÓN** `dist_vwap_pct` > `0.8569` → IC=+0.287 (n=421)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8569 (IC base=+0.069)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.472` → IC=+0.121 (n=2345)

  - _Acción_: Kelly boost +0.60€ cuando `sigma_ewma_delta_pct` > 2.472 (IC base=+0.069)

- **PATRÓN** `volumen_regimen` > `0.6831` → IC=+0.219 (n=1281)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6831 (IC base=+0.069)

- **PATRÓN** `volumen_pendiente_norm` < `0.0869` → IC=+0.214 (n=1979)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0869 (IC base=+0.069)

- **PATRÓN** `volumen_pendiente_norm` > `0.2532` → IC=+0.233 (n=417)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2532 (IC base=+0.069)

- **PATRÓN** `volumen_spike_ratio` < `1.4966` → IC=+0.238 (n=711)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4966 (IC base=+0.069)

- **PATRÓN** `volumen_spike_ratio` > `2.8423` → IC=+0.228 (n=711)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8423 (IC base=+0.069)

- **PATRÓN** `ballena_activa_n` < `96.0` → IC=+0.302 (n=1588)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 96.0 (IC base=+0.069)

- **PATRÓN** `ibs_20min` < `0.5834` → IC=+0.131 (n=4307)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.5834 (IC base=+0.051)
...
- **FILTRO** `ibs_20min` < `0.2829` → IC=-0.156 (n=367)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.106 (n=746)
...
- **PATRÓN** `ibs_20min` > `0.8155` → IC=+0.230 (n=279)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8155 (IC base=+0.019)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.116` → IC=+0.128 (n=417)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 2.116 (IC base=+0.019)

- **PATRÓN** `volumen_pendiente_norm` > `0.2236` → IC=+0.354 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2236 (IC base=+0.019)

- **PATRÓN** `volumen_spike_ratio` < `1.9518` → IC=+0.233 (n=118)
...
- **PATRÓN** `ballena_activa_n` < `74.0` → IC=+0.351 (n=132)
...
- **FILTRO** `ibs_20min` > `0.8644` → IC=-0.164 (n=376)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.016 (n=1131)
...
- **PATRÓN** `sigma_h` > `0.0095` → IC=+0.292 (n=239)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.712` → IC=+0.258 (n=639)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.712 (IC base=+0.223)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.297` → IC=+0.297 (n=175)
...
- **FILTRO** `ibs_20min` < `0.7621` → IC=-0.145 (n=471)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.088 (n=243)
...
- **PATRÓN** `dist_vwap_pct` > `0.8443` → IC=+0.250 (n=30)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8443 (IC base=-0.066)

- **PATRÓN** `volumen_regimen` < `0.9683` → IC=+0.131 (n=82)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 0.9683 (IC base=-0.066)

- **PATRÓN** `volumen_pendiente_norm` > `0.1716` → IC=+0.278 (n=16)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1716 (IC base=-0.066)

- **PATRÓN** `volumen_spike_ratio` < `2.0972` → IC=+0.246 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0972 (IC base=-0.066)
...
- **FILTRO** `ibs_20min` < `0.8` → IC=-0.147 (n=629)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.281 (n=332)

- **FILTRO** `ibs_20min` > `0.7561` → IC=-0.227 (n=276)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7561
  - _Potencial_: sin este filtro IC_bueno=+0.035 (n=829)
...
- **PATRÓN** `dist_vwap_pct` > `0.3189` → IC=+0.299 (n=167)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3189 (IC base=+0.002)

- **PATRÓN** `volumen_regimen` < `0.87` → IC=+0.239 (n=197)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.87 (IC base=+0.002)

- **PATRÓN** `volumen_regimen` > `1.1627` → IC=+0.300 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1627 (IC base=+0.002)

- **PATRÓN** `volumen_pendiente_norm` < `0.1176` → IC=+0.248 (n=280)
...
- **PATRÓN** `ballena_activa_n` < `58.0` → IC=+0.296 (n=219)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 58.0 (IC base=+0.002)

- **PATRÓN** `volumen_regimen` < `0.7519` → IC=+0.182 (n=42)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.7519 (IC base=-0.030)

- **PATRÓN** `volumen_pendiente_norm` > `0.2464` → IC=+0.250 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2464 (IC base=-0.030)

- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.214 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 61.0 (IC base=-0.030)
...
- **PATRÓN** `sigma_h` > `0.0106` → IC=+0.209 (n=1248)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0106 (IC base=+0.163)

- **PATRÓN** `drift_60min` |x|≤ `0.3434` → IC=+0.169 (n=3295)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `dist_vwap_pct` > `0.7674` → IC=+0.243 (n=807)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7674 (IC base=+0.163)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.679` → IC=+0.237 (n=1525)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.679 (IC base=+0.163)

- **PATRÓN** `volumen_regimen` > `0.6325` → IC=+0.167 (n=2533)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 0.6325 (IC base=+0.163)

- **PATRÓN** `volumen_pendiente_norm` > `0.1055` → IC=+0.187 (n=1399)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.1055 (IC base=+0.163)

- **PATRÓN** `volumen_spike_ratio` < `2.3275` → IC=+0.162 (n=3060)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.3275 (IC base=+0.163)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.165 (n=3841)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.03 (IC base=+0.163)

- **PATRÓN** `libro_liquidez` > `3937.758` → IC=+0.178 (n=1248)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 3937.758 (IC base=+0.163)

- **PATRÓN** `ballena_activa_n` < `144.0` → IC=+0.182 (n=2658)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0794` → IC=+0.212 (n=1165)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0794 (IC base=+0.180)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.199 (n=1677)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.4384` → IC=+0.232 (n=3491)
...
- **PATRÓN** `volumen_regimen` < `1.1802` → IC=+0.169 (n=2647)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 1.1802 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` > `0.6252` → IC=+0.162 (n=2646)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 0.6252 (IC base=+0.180)
...
- **PATRÓN** `drift_60min` |x|≤ `0.283` → IC=+0.196 (n=627)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_spike_ratio` < `2.6425` → IC=+0.165 (n=544)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.6425 (IC base=+0.174)

- **PATRÓN** `volumen_spike_ratio` > `1.4556` → IC=+0.166 (n=543)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.4556 (IC base=+0.174)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.204 (n=509)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.174)

- **PATRÓN** `ballena_activa_n` < `66.0` → IC=+0.188 (n=357)
...
- **PATRÓN** `sigma_h` < `0.006` → IC=+0.192 (n=485)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.006 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.4166` → IC=+0.174 (n=551)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.4166 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.195 (n=561)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` > 6.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` > `0.4793` → IC=+0.212 (n=551)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4793 (IC base=+0.167)

- **PATRÓN** `dist_vwap_pct` > `0.1985` → IC=+0.227 (n=397)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1985 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.802` → IC=+0.233 (n=155)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.802 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.2773` → IC=+0.169 (n=551)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 1.2773 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` > `1.0799` → IC=+0.182 (n=250)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` > 1.0799 (IC base=+0.167)

- **PATRÓN** `volumen_pendiente_norm` > `0.2311` → IC=+0.209 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2311 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `1.4857` → IC=+0.207 (n=230)
...
- **PATRÓN** `volumen_spike_ratio` > `2.4674` → IC=+0.171 (n=174)
  ...
  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.4674 (IC base=+0.167)

- **PATRÓN** `libro_liquidez` > `11341.8291` → IC=+0.188 (n=492)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 11341.8291 (IC base=+0.167)
...
- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.221 (n=195)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0094 (IC base=+0.173)

- **PATRÓN** `drift_60min` |x|≤ `0.2845` → IC=+0.189 (n=515)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.539` → IC=+0.319 (n=136)
...
- **PATRÓN** `volumen_spike_ratio` > `3.1817` → IC=+0.165 (n=237)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 3.1817 (IC base=+0.173)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.198 (n=587)
...
- **PATRÓN** `sigma_h` < `0.0074` → IC=+0.193 (n=490)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0074 (IC base=+0.161)

- **PATRÓN** `drift_60min` |x|≤ `0.129` → IC=+0.184 (n=245)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.129 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.177 (n=567)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 6.0 (IC base=+0.161)

- **PATRÓN** `ibs_20min` > `0.4502` → IC=+0.204 (n=556)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4502 (IC base=+0.161)

- **PATRÓN** `dist_vwap_pct` > `0.8741` → IC=+0.231 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8741 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.391` → IC=+0.266 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.391 (IC base=+0.161)

- **PATRÓN** `volumen_regimen` > `1.2168` → IC=+0.218 (n=186)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2168 (IC base=+0.161)

- **PATRÓN** `volumen_pendiente_norm` > `0.3006` → IC=+0.222 (n=95)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3006 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` > `2.6294` → IC=+0.203 (n=180)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6294 (IC base=+0.161)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.163 (n=624)

  - _Acción_: Kelly boost +0.81€ cuando `libro_spread` < 0.01 (IC base=+0.161)

- **PATRÓN** `libro_liquidez` > `8738.117` → IC=+0.197 (n=371)

  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 8738.117 (IC base=+0.161)

- **PATRÓN** `ballena_activa_n` < `135.0` → IC=+0.151 (n=328)

  - _Acción_: Kelly boost +0.76€ cuando `ballena_activa_n` < 135.0 (IC base=+0.161)
...
- **PATRÓN** `sigma_h` > `0.0097` → IC=+0.169 (n=258)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0097 (IC base=+0.135)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.164 (n=734)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 8.0 (IC base=+0.135)

- **PATRÓN** `ibs_20min` > `0.4762` → IC=+0.167 (n=773)

  - _Acción_: Kelly boost +0.84€ cuando `ibs_20min` > 0.4762 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` > `0.8562` → IC=+0.219 (n=87)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8562 (IC base=+0.135)
...
- **PATRÓN** `volumen_regimen` < `0.8827` → IC=+0.155 (n=415)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 0.8827 (IC base=+0.135)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1778` → IC=+0.182 (n=215)
  ...
  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.1778 (IC base=+0.135)

- **PATRÓN** `volumen_spike_ratio` < `2.283` → IC=+0.143 (n=632)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 2.283 (IC base=+0.135)
...
- **PATRÓN** `libro_liquidez` > `6332.2628` → IC=+0.177 (n=258)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 6332.2628 (IC base=+0.135)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0087` → IC=+0.205 (n=2031)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0087 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.172 (n=4484)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 6.0 (IC base=+0.165)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.309 (n=1541)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` > `1.0305` → IC=+0.227 (n=636)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0305 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.626` → IC=+0.227 (n=2534)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.626 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` < `0.8902` → IC=+0.159 (n=2071)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8761` → IC=+0.174 (n=2769)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 1.8761 (IC base=+0.165)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.167 (n=4253)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.02 (IC base=+0.165)

- **PATRÓN** `libro_liquidez` > `3855.5121` → IC=+0.188 (n=1493)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3855.5121 (IC base=+0.165)

- **PATRÓN** `ballena_activa_n` < `61.0` → IC=+0.215 (n=2070)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 61.0 (IC base=+0.165)

- **PATRÓN** `sigma_h` < `0.0108` → IC=+0.198 (n=4153)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` < 0.0108 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.4819` → IC=+0.197 (n=4153)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5624` → IC=+0.242 (n=4153)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5624 (IC base=+0.189)

- **PATRÓN** `dist_vwap_pct` < `0.4391` → IC=+0.174 (n=2961)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.4391 (IC base=+0.189)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.585` → IC=+0.190 (n=4093)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` < 3.585 (IC base=+0.189)

- **PATRÓN** `volumen_regimen` < `0.6234` → IC=+0.174 (n=988)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` < 0.6234 (IC base=+0.189)

- **PATRÓN** `volumen_regimen` > `1.2018` → IC=+0.162 (n=988)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 1.2018 (IC base=+0.189)
...
- **PATRÓN** `volumen_spike_ratio` > `2.3089` → IC=+0.204 (n=1562)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.3089 (IC base=+0.189)

- **PATRÓN** `ballena_activa_n` < `142.0` → IC=+0.181 (n=3027)
...
- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.235 (n=334)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_pendiente_norm` > `0.218` → IC=+0.266 (n=135)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.218 (IC base=+0.187)

- **PATRÓN** `volumen_spike_ratio` < `1.5795` → IC=+0.185 (n=287)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 1.5795 (IC base=+0.187)

- **PATRÓN** `volumen_spike_ratio` > `1.8862` → IC=+0.179 (n=434)
...
- **PATRÓN** `ballena_activa_n` < `50.0` → IC=+0.242 (n=266)
...
- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.163 (n=244)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_spike_ratio` < `2.4168` → IC=+0.178 (n=682)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 2.4168 (IC base=+0.160)
...
- **PATRÓN** `libro_liquidez` > `10640.5814` → IC=+0.186 (n=654)

  - _Acción_: Kelly boost +0.93€ cuando `libro_liquidez` > 10640.5814 (IC base=+0.160)

- **PATRÓN** `ballena_activa_n` < `509.0` → IC=+0.172 (n=607)

  - _Acción_: Kelly boost +0.86€ cuando `ballena_activa_n` < 509.0 (IC base=+0.160)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.170 (n=677)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0062 (IC base=+0.161)

- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.161 (n=603)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.81€ cuando `sigma_h` > 0.0029 (IC base=+0.161)

- **PATRÓN** `drift_60min` |x|≤ `0.2728` → IC=+0.178 (n=595)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.89€ cuando `drift_60min` |x|≤ 0.2728 (IC base=+0.161)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.167 (n=676)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.83€ cuando `hora_utc` < 17.0 (IC base=+0.161)

- **PATRÓN** `ibs_20min` < `0.6106` → IC=+0.201 (n=676)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6106 (IC base=+0.161)

- **PATRÓN** `dist_vwap_pct` < `0.3362` → IC=+0.174 (n=672)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.3362 (IC base=+0.161)
...
- **PATRÓN** `volumen_regimen` < `0.6202` → IC=+0.225 (n=227)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6202 (IC base=+0.161)

- **PATRÓN** `volumen_pendiente_norm` > `0.1414` → IC=+0.232 (n=177)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1414 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` < `1.4014` → IC=+0.194 (n=194)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 1.4014 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` > `2.0753` → IC=+0.183 (n=263)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` > 2.0753 (IC base=+0.161)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.162 (n=876)
...
- **PATRÓN** `ibs_20min` > `0.6842` → IC=+0.259 (n=574)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2192` → IC=+0.216 (n=582)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2192 (IC base=+0.208)

- **PATRÓN** `volumen_spike_ratio` < `3.9629` → IC=+0.205 (n=568)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5185` → IC=+0.206 (n=379)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.5185 (IC base=+0.208)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.240 (n=644)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.208)

- **PATRÓN** `libro_liquidez` > `1451.1486` → IC=+0.211 (n=642)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1451.1486 (IC base=+0.208)

- **PATRÓN** `ballena_activa_n` < `63.0` → IC=+0.275 (n=353)
...
- **PATRÓN** `sigma_h` < `0.0071` → IC=+0.157 (n=651)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.78€ cuando `sigma_h` < 0.0071 (IC base=+0.144)

- **PATRÓN** `drift_60min` |x|≤ `0.1427` → IC=+0.149 (n=326)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.1427 (IC base=+0.144)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.159 (n=666)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` > 8.0 (IC base=+0.144)

- **PATRÓN** `ibs_20min` > `0.7425` → IC=+0.239 (n=493)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7425 (IC base=+0.144)

- **PATRÓN** `dist_vwap_pct` > `0.364` → IC=+0.193 (n=291)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` > 0.364 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.935` → IC=+0.190 (n=127)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 11.935 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` < `0.9069` → IC=+0.167 (n=494)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 0.9069 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` > `1.2094` → IC=+0.163 (n=247)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` > 1.2094 (IC base=+0.144)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1724` → IC=+0.206 (n=318)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1724 (IC base=+0.144)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.146 (n=809)

  - _Acción_: Kelly boost +0.73€ cuando `libro_spread` < 0.01 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `9441.8802` → IC=+0.246 (n=336)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 9441.8802 (IC base=+0.144)
...
- **PATRÓN** `sigma_h` > `0.0085` → IC=+0.146 (n=518)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` > 0.0085 (IC base=+0.092)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.135 (n=521)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 12.0 (IC base=+0.092)

- **PATRÓN** `ibs_20min` > `0.4787` → IC=+0.180 (n=777)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` > 0.4787 (IC base=+0.092)

- **PATRÓN** `dist_vwap_pct` > `1.1136` → IC=+0.194 (n=142)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 1.1136 (IC base=+0.092)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.478` → IC=+0.227 (n=295)
...
- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.136 (n=555)

  - _Acción_: Kelly boost +0.68€ cuando `ballena_activa_n` < 62.0 (IC base=+0.092)
...
- **PATRÓN** `drift_60min` |x|≤ `0.5323` → IC=+0.217 (n=832)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.5323 (IC base=+0.213)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.217 (n=454)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.213)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.218 (n=1001)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_regimen` > `0.6405` → IC=+0.222 (n=945)
...
- **PATRÓN** `ballena_activa_n` < `34.0` → IC=+0.188 (n=665)
...
- **PATRÓN** `sigma_h` < `0.01` → IC=+0.132 (n=1297)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` < 0.01 (IC base=+0.120)

- **PATRÓN** `drift_60min` |x|≤ `0.556` → IC=+0.130 (n=1474)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.65€ cuando `drift_60min` |x|≤ 0.556 (IC base=+0.120)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `6.022` → IC=+0.123 (n=1498)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` < 6.022 (IC base=+0.120)

- **PATRÓN** `volumen_regimen` > `0.9016` → IC=+0.123 (n=654)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_regimen` > 0.9016 (IC base=+0.120)

- **PATRÓN** `volumen_pendiente_norm` > `0.1742` → IC=+0.150 (n=401)
...
- **PATRÓN** `libro_liquidez` > `8938.0512` → IC=+0.134 (n=668)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4825` → IC=+0.150 (n=532)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.1834` → IC=+0.146 (n=532)

  - _Acción_: Kelly boost +0.73€ cuando `ibs_20min` > 0.1834 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` > `0.9872` → IC=+0.167 (n=112)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 0.9872 (IC base=+0.134)

- **PATRÓN** `dist_vwap_pct` < `0.4319` → IC=+0.145 (n=511)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.4319 (IC base=+0.134)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.061` → IC=+0.146 (n=535)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` < 7.061 (IC base=+0.134)

- **PATRÓN** `volumen_regimen` < `1.283` → IC=+0.144 (n=532)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.283 (IC base=+0.134)

- **PATRÓN** `volumen_pendiente_norm` < `0.1172` → IC=+0.138 (n=490)
...
- **PATRÓN** `volumen_spike_ratio` < `1.451` → IC=+0.167 (n=175)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.451 (IC base=+0.134)

- **PATRÓN** `volumen_spike_ratio` > `1.8484` → IC=+0.143 (n=348)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 1.8484 (IC base=+0.134)

- **PATRÓN** `libro_liquidez` > `8926.6094` → IC=+0.146 (n=475)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.177 (n=1427)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=4287)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.187 (n=1456)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.008 (n=4483)
...
- **FILTRO** `ballena_activa_n` > `60.0` → IC=-0.153 (n=237)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=715)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.122 (n=35)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=917)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.45` → IC=-0.200 (n=301)

  - _Acción_: SKIP cuando `py_entrada` < 0.45
  - _Potencial_: sin este filtro IC_bueno=+0.042 (n=627)
...
- **FILTRO** `ibs_20min` > `0.7156` → IC=-0.194 (n=250)
...
- **FILTRO** `py_entrada` < `0.495` → IC=-0.144 (n=259)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.023 (n=705)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.181 (n=224)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=764)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=2991)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=11007)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.283 (n=3448)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=10550)

- **FILTRO** `ibs_7min` < `0.7115` → IC=-0.248 (n=3499)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7115
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=10499)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.169 (n=4685)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=9313)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.220 (n=4337)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=13274)

- **FILTRO** `ibs_7min` > `0.7143` → IC=-0.172 (n=4397)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=13214)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.155 (n=604)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.102 (n=1432)
...
- **FILTRO** `ibs_7min` < `0.2945` → IC=-0.265 (n=671)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2945
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=1365)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.226 (n=506)

  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.082 (n=1530)

- **FILTRO** `py_entrada` > `0.52` → IC=-0.140 (n=2039)
...
- **FILTRO** `drift_7min_pct` |x|> `0.1143` → IC=-0.129 (n=1039)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=2024)

- **FILTRO** `ibs_7min` > `0.8333` → IC=-0.190 (n=765)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8333
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=2298)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=572)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=2045)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.259 (n=636)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=1981)

- **FILTRO** `ibs_7min` < `0.7711` → IC=-0.195 (n=654)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7711
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=1963)

- **FILTRO** `ballena_activa_n` > `168.0` → IC=-0.178 (n=650)

  - _Acción_: SKIP cuando `ballena_activa_n` > 168.0
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=1967)

- **FILTRO** `py_entrada` > `0.64` → IC=-0.245 (n=602)

  - _Acción_: SKIP cuando `py_entrada` > 0.64
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=2024)

- **FILTRO** `ballena_activa_n` > `106.0` → IC=-0.176 (n=891)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.224 (n=738)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=2418)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.138 (n=717)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.090 (n=1657)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.258 (n=581)
...
- **FILTRO** `ibs_7min` < `0.75` → IC=-0.198 (n=593)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=1781)

- **FILTRO** `ballena_activa_n` > `26.0` → IC=-0.175 (n=802)

  - _Acción_: SKIP cuando `ballena_activa_n` > 26.0
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=1572)
...
- **FILTRO** `ibs_7min` > `0.2755` → IC=-0.178 (n=595)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=1788)

- **FILTRO** `ballena_activa_n` > `34.0` → IC=-0.182 (n=586)

  - _Acción_: SKIP cuando `ballena_activa_n` > 34.0
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=1797)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.39` → IC=-0.249 (n=635)

  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=1975)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.179 (n=796)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.123 (n=739)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=1552)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.297 (n=561)
...
- **FILTRO** `ibs_7min` < `0.7333` → IC=-0.248 (n=570)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=1721)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.230 (n=550)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.014 (n=1565)
...
- **FILTRO** `sigma_h` > `0.0074` → IC=-0.239 (n=220)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.193 (n=663)

- **FILTRO** `sigma_h` < `0.0038` → IC=-0.230 (n=291)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: SKIP cuando `sigma_h` < 0.0038
  - _Potencial_: sin este filtro IC_bueno=-0.192 (n=592)

- **FILTRO** `sigma_ewma_delta_pct` > `19.574` → IC=-0.250 (n=162)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 19.574
  - _Potencial_: sin este filtro IC_bueno=-0.194 (n=721)

- **FILTRO** `libro_liquidez` < `16085.2315` → IC=-0.214 (n=582)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.069` → IC=+0.305 (n=126)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.069 (IC base=+0.294)
...
| ✅ BALLENAS_TARDIAS | 14606 | -0.111 | -2442.71€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 949 | -0.010 | -143.16€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 13657 | -0.118 | -2299.54€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 1891 | -0.087 | -404.83€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 1891 | -0.087 | -404.83€ | 1 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 3242 | -0.127 | -278.53€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 3242 | -0.127 | -278.53€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 2673 | -0.193 | -733.82€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 2673 | -0.193 | -733.82€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 3939 | -0.069 | +1789.52€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 1117 | -0.010 | +852.38€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 2822 | -0.093 | +937.15€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 3939 | -0.069 | +1789.52€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 1117 | -0.010 | +852.38€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 2822 | -0.093 | +937.15€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO | 85 | -0.075 | -15.40€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#5min | 85 | -0.075 | -15.40€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#BTC | 85 | -0.075 | -15.40€ | 0 | 0 |
| ✅ CANDIDATA9_BOT_CONSENSO#BTC#5min | 85 | -0.075 | -15.40€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO | 46974 | +0.113 | -2926.30€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 7892 | +0.185 | -266.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 158 | -0.113 | -54.08€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 35202 | +0.098 | -2530.56€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3722 | +0.116 | -74.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 5907 | +0.082 | -787.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 26 | -0.107 | +5.05€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 13 | -0.238 | -12.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 5868 | +0.084 | -780.12€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 9446 | +0.132 | -228.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 2318 | +0.199 | -104.29€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 5832 | +0.108 | -151.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1254 | +0.126 | +48.94€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 5924 | +0.082 | -740.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 28 | +0.033 | +3.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#240min | 8 | -0.120 | -5.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 5888 | +0.083 | -738.58€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 10171 | +0.127 | -148.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2934 | +0.172 | -12.35€ | 1 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 5855 | +0.112 | -87.56€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1370 | +0.096 | -39.98€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 9621 | +0.125 | -619.98€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2566 | +0.192 | -160.91€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 78 | -0.013 | -3.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 5879 | +0.097 | -371.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1098 | +0.131 | -83.83€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 5905 | +0.105 | -401.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 20 | +0.000 | +1.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 5 | -0.018 | -1.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 5880 | +0.106 | -401.38€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 