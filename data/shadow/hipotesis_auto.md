# Hipótesis automáticas — 2026-08-29 23:14 UTC
_Generado por shadow_postmortem.py sobre 207233 resoluciones (PNL=+16012.11€)_
...
- **FILTRO** `restante_s_al_confirmar` < `121.17` → IC=-0.380 (n=374)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=1124)

### BALLENAS_TARDIAS#ETH#5min
- **FILTRO** `restante_s_al_confirmar` < `158.28` → IC=-0.168 (n=774)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.28
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=2322)

### BALLENAS_TARDIAS#SOL#5min
- **FILTRO** `restante_s_al_confirmar` < `139.94` → IC=-0.323 (n=669)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 139.94
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=2007)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `158.05` → IC=-0.356 (n=659)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 158.05
  - _Potencial_: sin este filtro IC_bueno=-0.079 (n=1339)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.193 (n=6208)

  - _Acción_: Kelly boost +0.96€ cuando `py_entrada` > 0.7 (IC base=+0.098)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.174 (n=1741)

  - _Acción_: Kelly boost +0.87€ cuando `libro_spread` < 0.01 (IC base=+0.098)

- **PATRÓN** `libro_liquidez` > `2375.6867` → IC=+0.175 (n=1674)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 2375.6867 (IC base=+0.098)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.146 (n=3752)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `1904.0199` → IC=+0.181 (n=2720)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 1904.0199 (IC base=+0.142)

### FAVORITO_CONFIRMADO#BTC#15min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.222 (n=709)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.213 (n=906)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.131 (n=584)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.555` → IC=+0.135 (n=573)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.134 (n=1379)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.322 (n=437)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.194 (n=5137)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.202 (n=2437)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.166 (n=1257)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.177 (n=1340)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.174 (n=1242)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.306 (n=427)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.196 (n=1262)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.7` → IC=+0.197 (n=642)
...
- **PATRÓN** `restante_min` < `3.96` → IC=+0.136 (n=1793)

  - _Acción_: Kelly boost +0.68€ cuando `restante_min` < 3.96 (IC base=+0.132)

- **PATRÓN** `restante_min` > `4.93` → IC=+0.156 (n=1909)
...
- **PATRÓN** `lag_apertura_s` < `7.01` → IC=+0.155 (n=1169)
...
- **PATRÓN** `restante_min` < `4.41` → IC=+0.127 (n=1183)

  - _Acción_: Kelly boost +0.64€ cuando `restante_min` < 4.41 (IC base=+0.127)

- **PATRÓN** `restante_min` > `4.94` → IC=+0.163 (n=987)

  - _Acción_: Kelly boost +0.82€ cuando `restante_min` > 4.94 (IC base=+0.127)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.129 (n=2680)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.951` → IC=+0.209 (n=893)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.951 (IC base=+0.070)

- **PATRÓN** `dist_vwap_pct` > `0.186` → IC=+0.224 (n=357)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.186 (IC base=+0.070)

- **PATRÓN** `dist_vwap_pct` < `0.493` → IC=+0.211 (n=531)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.493 (IC base=+0.070)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.341` → IC=+0.160 (n=1053)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 5.341 (IC base=+0.070)

- **PATRÓN** `volumen_regimen` < `0.6297` → IC=+0.219 (n=194)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6297 (IC base=+0.070)

- **PATRÓN** `volumen_regimen` > `1.0648` → IC=+0.241 (n=264)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.963` → IC=+0.289 (n=263)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.963 (IC base=+0.137)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4339` → IC=+0.122 (n=504)

  - _Acción_: Kelly boost +0.61€ cuando `volumen_spike_ratio` > 1.4339 (IC base=+0.137)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.173 (n=420)

  - _Acción_: Kelly boost +0.86€ cuando `libro_spread` < 0.04 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `58.0` → IC=+0.159 (n=247)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 58.0 (IC base=+0.137)
...
- **PATRÓN** `sigma_h` > `0.0068` → IC=+0.211 (n=154)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0068 (IC base=+0.209)

- **PATRÓN** `drift_60min` |x|≤ `0.0895` → IC=+0.244 (n=154)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0895 (IC base=+0.209)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.232 (n=472)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.3908` → IC=+0.227 (n=460)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3908 (IC base=+0.209)

- **PATRÓN** `dist_vwap_pct` > `0.212` → IC=+0.229 (n=289)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.212 (IC base=+0.209)

- **PATRÓN** `dist_vwap_pct` < `0.5452` → IC=+0.210 (n=412)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5452 (IC base=+0.209)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.354` → IC=+0.233 (n=238)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.354 (IC base=+0.209)

- **PATRÓN** `volumen_regimen` < `0.6892` → IC=+0.217 (n=203)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6892 (IC base=+0.209)

- **PATRÓN** `volumen_regimen` > `1.0799` → IC=+0.239 (n=209)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0799 (IC base=+0.209)

- **PATRÓN** `volumen_pendiente_norm` < `0.0994` → IC=+0.213 (n=430)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0994 (IC base=+0.209)

- **PATRÓN** `volumen_spike_ratio` < `1.4558` → IC=+0.258 (n=192)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4558 (IC base=+0.209)

- **PATRÓN** `libro_liquidez` > `14039.8617` → IC=+0.218 (n=307)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 14039.8617 (IC base=+0.209)

- **PATRÓN** `ballena_activa_n` < `398.0` → IC=+0.208 (n=385)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1342` → IC=+0.147 (n=443)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` < 0.1342 (IC base=+0.155)

- **PATRÓN** `volumen_pendiente_norm` > `0.4089` → IC=+0.152 (n=67)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_pendiente_norm` > 0.4089 (IC base=+0.155)
...
- **FILTRO** `sigma_ewma_delta_pct` > `5.027` → IC=-0.165 (n=311)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=1136)
...
- **FILTRO** `ibs_20min` > `0.7937` → IC=-0.181 (n=327)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7937
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=985)

- **FILTRO** `sigma_ewma_delta_pct` > `6.782` → IC=-0.153 (n=214)
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.137 (n=1540)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` > 0.0076 (IC base=+0.056)

- **PATRÓN** `ibs_20min` > `0.2759` → IC=+0.122 (n=3393)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1152` → IC=+0.185 (n=1524)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_pendiente_norm` < 0.1152 (IC base=+0.056)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8513` → IC=+0.187 (n=522)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 2.8513 (IC base=+0.056)

- **PATRÓN** `ballena_activa_n` < `94.0` → IC=+0.279 (n=1017)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0753` → IC=+0.225 (n=180)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1431` → IC=+0.211 (n=437)
...
- **PATRÓN** `volumen_spike_ratio` > `3.3191` → IC=+0.214 (n=208)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.3191 (IC base=+0.200)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.227 (n=515)
...
- **FILTRO** `sigma_h` > `0.0065` → IC=-0.144 (n=133)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=403)

- **FILTRO** `hora_utc` < `9.0` → IC=-0.172 (n=172)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=364)
...
- **FILTRO** `ibs_20min` > `0.7647` → IC=-0.222 (n=235)
...
- **PATRÓN** `sigma_h` > `0.0202` → IC=+0.318 (n=284)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0813` → IC=+0.232 (n=528)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8219` → IC=+0.244 (n=377)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8219 (IC base=+0.229)
...
- **PATRÓN** `sigma_h` > `0.0102` → IC=+0.198 (n=967)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0102 (IC base=+0.162)

- **PATRÓN** `drift_60min` |x|≤ `0.0856` → IC=+0.170 (n=967)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.0856 (IC base=+0.162)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.170 (n=2944)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 6.0 (IC base=+0.162)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.282 (n=1392)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` > `0.7898` → IC=+0.250 (n=647)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7898 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.477` → IC=+0.245 (n=1197)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.477 (IC base=+0.162)

- **PATRÓN** `volumen_regimen` > `0.6228` → IC=+0.173 (n=2003)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 0.6228 (IC base=+0.162)

- **PATRÓN** `volumen_pendiente_norm` > `0.2403` → IC=+0.198 (n=557)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` > 0.2403 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` < `2.3122` → IC=+0.163 (n=2324)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 2.3122 (IC base=+0.162)

- **PATRÓN** `volumen_spike_ratio` > `1.8624` → IC=+0.156 (n=1761)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.8624 (IC base=+0.162)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.169 (n=2299)

  - _Acción_: Kelly boost +0.85€ cuando `libro_spread` < 0.01 (IC base=+0.162)

- **PATRÓN** `libro_liquidez` > `3974.1116` → IC=+0.189 (n=967)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 3974.1116 (IC base=+0.162)

- **PATRÓN** `ballena_activa_n` < `164.0` → IC=+0.180 (n=1809)
...
- **PATRÓN** `sigma_h` < `0.006` → IC=+0.211 (n=368)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.006 (IC base=+0.182)

- **PATRÓN** `drift_60min` |x|≤ `0.4174` → IC=+0.195 (n=418)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.4174 (IC base=+0.182)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.209 (n=428)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.182)

- **PATRÓN** `ibs_20min` > `0.4893` → IC=+0.221 (n=418)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4893 (IC base=+0.182)

- **PATRÓN** `dist_vwap_pct` > `0.2069` → IC=+0.237 (n=298)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2069 (IC base=+0.182)
...
- **PATRÓN** `volumen_regimen` < `1.2802` → IC=+0.183 (n=418)
  ...
  - _Acción_: Kelly boost +0.92€ cuando `volumen_regimen` < 1.2802 (IC base=+0.182)

- **PATRÓN** `volumen_regimen` > `1.0648` → IC=+0.203 (n=190)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0648 (IC base=+0.182)

- **PATRÓN** `volumen_pendiente_norm` > `0.2241` → IC=+0.233 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2241 (IC base=+0.182)

- **PATRÓN** `volumen_spike_ratio` < `1.3602` → IC=+0.242 (n=130)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.3602 (IC base=+0.182)

- **PATRÓN** `libro_liquidez` > `13155.0204` → IC=+0.212 (n=279)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13155.0204 (IC base=+0.182)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1541` → IC=+0.178 (n=284)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1342` → IC=+0.154 (n=154)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_pendiente_norm` > 0.1342 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` < `2.5703` → IC=+0.182 (n=243)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` < 2.5703 (IC base=+0.167)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.194 (n=393)
...
- **PATRÓN** `volumen_spike_ratio` > `2.563` → IC=+0.223 (n=135)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.563 (IC base=+0.171)
...
- **PATRÓN** `ballena_activa_n` < `143.0` → IC=+0.163 (n=241)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.137 (n=599)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` > 5.0 (IC base=+0.119)

- **PATRÓN** `ibs_20min` > `0.5455` → IC=+0.186 (n=569)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` > 0.5455 (IC base=+0.119)

- **PATRÓN** `dist_vwap_pct` > `0.9064` → IC=+0.267 (n=178)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.9064 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.191` → IC=+0.275 (n=127)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.191 (IC base=+0.119)

- **PATRÓN** `volumen_regimen` > `0.6193` → IC=+0.139 (n=568)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` > 0.6193 (IC base=+0.119)

- **PATRÓN** `volumen_pendiente_norm` < `0.1617` → IC=+0.135 (n=560)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_pendiente_norm` < 0.1617 (IC base=+0.119)

- **PATRÓN** `volumen_spike_ratio` < `1.4411` → IC=+0.130 (n=179)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` < 1.4411 (IC base=+0.119)

- **PATRÓN** `volumen_spike_ratio` > `1.5596` → IC=+0.127 (n=478)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.5596 (IC base=+0.119)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.123 (n=449)

  - _Acción_: Kelly boost +0.62€ cuando `libro_spread` < 0.01 (IC base=+0.119)

- **PATRÓN** `libro_liquidez` > `3261.6828` → IC=+0.208 (n=190)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3261.6828 (IC base=+0.119)
...
- **PATRÓN** `ibs_20min` > `0.8984` → IC=+0.263 (n=399)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8984 (IC base=+0.176)

- **PATRÓN** `dist_vwap_pct` > `1.0022` → IC=+0.243 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0022 (IC base=+0.176)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.169` → IC=+0.238 (n=311)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.169 (IC base=+0.176)

- **PATRÓN** `volumen_regimen` > `0.6052` → IC=+0.189 (n=599)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.180 (n=716)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.157 (n=546)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 8.0 (IC base=+0.120)

- **PATRÓN** `ibs_20min` > `0.6207` → IC=+0.177 (n=509)

  - _Acción_: Kelly boost +0.89€ cuando `ibs_20min` > 0.6207 (IC base=+0.120)

- **PATRÓN** `dist_vwap_pct` > `0.8459` → IC=+0.272 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.8459 (IC base=+0.120)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.157` → IC=+0.188 (n=155)

  - _Acción_: Kelly boost +0.94€ cuando `sigma_ewma_delta_pct` > 8.157 (IC base=+0.120)

- **PATRÓN** `volumen_regimen` < `0.8863` → IC=+0.136 (n=303)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 0.8863 (IC base=+0.120)

- **PATRÓN** `volumen_regimen` > `1.1727` → IC=+0.141 (n=151)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` > 1.1727 (IC base=+0.120)
...
- **PATRÓN** `volumen_spike_ratio` < `2.3406` → IC=+0.132 (n=455)
  ...
  - _Acción_: Kelly boost +0.66€ cuando `volumen_spike_ratio` < 2.3406 (IC base=+0.120)

- **PATRÓN** `volumen_spike_ratio` > `1.5952` → IC=+0.129 (n=461)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` > 1.5952 (IC base=+0.120)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.128 (n=586)

  - _Acción_: Kelly boost +0.64€ cuando `libro_spread` < 0.02 (IC base=+0.120)

- **PATRÓN** `libro_liquidez` > `3047.4604` → IC=+0.136 (n=259)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 3047.4604 (IC base=+0.120)
...
- **FILTRO** `ibs_20min` < `0.5714` → IC=-0.179 (n=51)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.112 (n=155)
...
- **PATRÓN** `hora_utc` > `14.0` → IC=+0.154 (n=79)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.77€ cuando `hora_utc` > 14.0 (IC base=+0.038)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.210 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.038)

- **PATRÓN** `dist_vwap_pct` > `0.4993` → IC=+0.200 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4993 (IC base=+0.038)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.935` → IC=+0.149 (n=55)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 6.935 (IC base=+0.038)

- **PATRÓN** `volumen_pendiente_norm` > `0.2903` → IC=+0.182 (n=20)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.2903 (IC base=+0.038)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.197 (n=1530)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0082 (IC base=+0.156)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.163 (n=3446)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 6.0 (IC base=+0.156)

- **PATRÓN** `ibs_20min` > `0.9375` → IC=+0.284 (n=1528)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9375 (IC base=+0.156)

- **PATRÓN** `dist_vwap_pct` > `1.0724` → IC=+0.236 (n=509)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0724 (IC base=+0.156)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.544` → IC=+0.228 (n=1949)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.544 (IC base=+0.156)

- **PATRÓN** `volumen_regimen` < `0.6942` → IC=+0.158 (n=1044)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.6942 (IC base=+0.156)

- **PATRÓN** `volumen_regimen` > `1.2249` → IC=+0.152 (n=792)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.2249 (IC base=+0.156)

- **PATRÓN** `volumen_pendiente_norm` > `0.165` → IC=+0.182 (n=883)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.165 (IC base=+0.156)

- **PATRÓN** `volumen_spike_ratio` < `2.319` → IC=+0.157 (n=2685)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` < 2.319 (IC base=+0.156)

- **PATRÓN** `volumen_spike_ratio` > `1.8746` → IC=+0.165 (n=2034)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.8746 (IC base=+0.156)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.161 (n=2682)

  - _Acción_: Kelly boost +0.80€ cuando `libro_spread` < 0.01 (IC base=+0.156)

- **PATRÓN** `libro_liquidez` > `3954.2434` → IC=+0.191 (n=1124)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 3954.2434 (IC base=+0.156)

- **PATRÓN** `ballena_activa_n` < `65.0` → IC=+0.207 (n=1331)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.161 (n=561)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 6.0 (IC base=+0.160)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.190 (n=201)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` < 6.0 (IC base=+0.160)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.213 (n=368)
...
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.198 (n=180)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `volumen_regimen` > `1.0851` → IC=+0.180 (n=245)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` > 1.0851 (IC base=+0.160)

- **PATRÓN** `volumen_pendiente_norm` < `0.0714` → IC=+0.178 (n=442)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_pendiente_norm` < 0.0714 (IC base=+0.160)

- **PATRÓN** `volumen_pendiente_norm` > `0.2066` → IC=+0.189 (n=104)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.2066 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` < `2.3977` → IC=+0.190 (n=491)

  - _Acción_: Kelly boost +0.95€ cuando `volumen_spike_ratio` < 2.3977 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` > `1.3791` → IC=+0.177 (n=491)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 1.3791 (IC base=+0.160)

- **PATRÓN** `libro_liquidez` > `12682.0918` → IC=+0.188 (n=360)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 12682.0918 (IC base=+0.160)
...
- **PATRÓN** `ibs_20min` > `0.7078` → IC=+0.265 (n=403)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7078 (IC base=+0.193)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.275` → IC=+0.339 (n=116)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.275 (IC base=+0.193)

- **PATRÓN** `volumen_pendiente_norm` < `0.2205` → IC=+0.197 (n=388)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_pendiente_norm` < 0.2205 (IC base=+0.193)

- **PATRÓN** `volumen_spike_ratio` < `1.9553` → IC=+0.198 (n=167)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 1.9553 (IC base=+0.193)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.224 (n=414)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.193)

- **PATRÓN** `libro_liquidez` > `1806.0504` → IC=+0.193 (n=301)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 1806.0504 (IC base=+0.193)

- **PATRÓN** `ballena_activa_n` < `59.0` → IC=+0.252 (n=163)
...
- **PATRÓN** `sigma_h` < `0.0086` → IC=+0.152 (n=541)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.2375` → IC=+0.142 (n=361)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.71€ cuando `drift_60min` |x|≤ 0.2375 (IC base=+0.141)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.163 (n=494)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 8.0 (IC base=+0.141)

- **PATRÓN** `ibs_20min` > `0.3389` → IC=+0.194 (n=540)

  - _Acción_: Kelly boost +0.97€ cuando `ibs_20min` > 0.3389 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` > `0.7991` → IC=+0.214 (n=110)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7991 (IC base=+0.141)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.376` → IC=+0.191 (n=260)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 4.376 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` < `0.8962` → IC=+0.169 (n=361)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` < 0.8962 (IC base=+0.141)
...
- **PATRÓN** `sigma_h` > `0.008` → IC=+0.145 (n=398)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` > 0.008 (IC base=+0.095)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.123 (n=534)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 8.0 (IC base=+0.095)

- **PATRÓN** `ibs_20min` > `0.4805` → IC=+0.179 (n=597)

  - _Acción_: Kelly boost +0.90€ cuando `ibs_20min` > 0.4805 (IC base=+0.095)

- **PATRÓN** `dist_vwap_pct` > `0.8895` → IC=+0.216 (n=160)
...
- **PATRÓN** `libro_liquidez` > `3011.5404` → IC=+0.251 (n=199)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3011.5404 (IC base=+0.095)

- **PATRÓN** `ballena_activa_n` < `66.0` → IC=+0.155 (n=372)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.196 (n=702)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.98€ cuando `hora_utc` > 6.0 (IC base=+0.186)

- **PATRÓN** `ibs_20min` > `0.951` → IC=+0.289 (n=316)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.118` → IC=+0.248 (n=363)
...
- **PATRÓN** `sigma_h` < `0.0053` → IC=+0.208 (n=169)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0053 (IC base=+0.090)

- **PATRÓN** `ibs_20min` > `0.6842` → IC=+0.250 (n=162)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.6842 (IC base=+0.090)

- **PATRÓN** `dist_vwap_pct` > `0.1239` → IC=+0.186 (n=84)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.1239 (IC base=+0.090)

- **PATRÓN** `sigma_ewma_delta_pct` > `14.002` → IC=+0.297 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 14.002 (IC base=+0.090)

- **PATRÓN** `volumen_regimen` < `0.6313` → IC=+0.171 (n=71)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 0.6313 (IC base=+0.090)

- **PATRÓN** `volumen_regimen` > `1.1731` → IC=+0.179 (n=54)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` > 1.1731 (IC base=+0.090)

- **PATRÓN** `volumen_pendiente_norm` < `0.0651` → IC=+0.276 (n=47)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.157 (n=33)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` < 7.0 (IC base=+0.098)

- **PATRÓN** `ibs_20min` > `0.8206` → IC=+0.264 (n=53)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8206 (IC base=+0.098)

- **PATRÓN** `dist_vwap_pct` > `0.1487` → IC=+0.214 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1487 (IC base=+0.098)

- **PATRÓN** `sigma_ewma_delta_pct` > `15.354` → IC=+0.196 (n=21)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 15.354 (IC base=+0.098)

- **PATRÓN** `volumen_regimen` < `0.9418` → IC=+0.143 (n=40)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` < 0.9418 (IC base=+0.098)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.174 (n=1020)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.029 (n=3116)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.214 (n=997)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=3269)

- **FILTRO** `ibs_20min` > `0.2734` → IC=-0.172 (n=1065)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2734
  - _Potencial_: sin este filtro IC_bueno=-0.010 (n=3201)
...
- **FILTRO** `py_entrada` > `0.67` → IC=-0.156 (n=184)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=563)

- **FILTRO** `ibs_20min` > `0.1842` → IC=-0.127 (n=373)
...
- **FILTRO** `py_entrada` < `0.44` → IC=-0.206 (n=209)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.44
  - _Potencial_: sin este filtro IC_bueno=+0.030 (n=428)
...
- **FILTRO** `py_entrada` < `0.495` → IC=-0.130 (n=198)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.043 (n=516)
...
- **FILTRO** `ibs_20min` > `0.2724` → IC=-0.191 (n=173)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.138 (n=2594)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=7855)

- **FILTRO** `py_entrada` < `0.36` → IC=-0.279 (n=2588)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.027 (n=7861)

- **FILTRO** `ibs_7min` < `0.7273` → IC=-0.233 (n=2602)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7273
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=7847)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.172 (n=3538)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=6911)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.230 (n=2978)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=9883)

- **FILTRO** `ibs_7min` > `0.7272` → IC=-0.171 (n=3215)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7272
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=9646)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.3` → IC=-0.323 (n=342)

  - _Acción_: SKIP cuando `py_entrada` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=1100)

- **FILTRO** `ibs_7min` < `1.0` → IC=-0.189 (n=1006)
...
- **FILTRO** `ballena_activa_n` > `14.0` → IC=-0.243 (n=348)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.076 (n=1094)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.227 (n=481)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=1521)

- **FILTRO** `ballena_activa_n` > `98.0` → IC=-0.174 (n=679)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.195 (n=362)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.092 (n=1171)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.306 (n=482)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=1051)

- **FILTRO** `ibs_7min` < `0.2222` → IC=-0.282 (n=379)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2222
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=1154)
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.261 (n=424)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.051 (n=1336)

- **FILTRO** `ibs_7min` < `0.7599` → IC=-0.181 (n=440)
...
- **FILTRO** `ballena_activa_n` > `39.0` → IC=-0.190 (n=437)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=1323)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.260 (n=436)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=1333)

- **FILTRO** `ibs_7min` > `0.1909` → IC=-0.172 (n=601)

  - _Acción_: SKIP cuando `ibs_7min` > 0.1909
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=1168)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.157 (n=870)
...
- **FILTRO** `py_entrada` < `0.42` → IC=-0.232 (n=501)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=1509)

- **FILTRO** `ibs_7min` < `0.7647` → IC=-0.200 (n=502)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7647
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=1508)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.184 (n=587)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.129 (n=548)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=1148)
...
- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.225 (n=419)
  ...
  - _Acción_: SKIP cuando `ballena_activa_n` > 13.0
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=1277)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.271 (n=460)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=1766)

- **FILTRO** `ibs_7min` > `0.8` → IC=-0.168 (n=552)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=1674)

- **FILTRO** `ballena_activa_n` > `10.0` → IC=-0.131 (n=737)

  - _Acción_: SKIP cuando `ballena_activa_n` > 10.0
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=1489)
...
- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.148 (n=177)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0038 (IC base=+0.118)

- **PATRÓN** `drift_60min` |x|≤ `0.1878` → IC=+0.126 (n=402)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.63€ cuando `drift_60min` |x|≤ 0.1878 (IC base=+0.118)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0679` → IC=+0.126 (n=402)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.63€ cuando `delta_ratio_macro` |x|> 0.0679 (IC base=+0.118)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.123 (n=428)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 4.0 (IC base=+0.118)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.158 (n=182)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` < 6.0 (IC base=+0.118)

- **PATRÓN** `ibs_15` > `0.5319` → IC=+0.210 (n=402)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5319 (IC base=+0.118)

- **PATRÓN** `dist_vwap_pct` > `0.3946` → IC=+0.179 (n=107)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.3946 (IC base=+0.118)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.849` → IC=+0.236 (n=157)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.849 (IC base=+0.118)
...
- **PATRÓN** `libro_liquidez` > `8015.433` → IC=+0.176 (n=134)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 8015.433 (IC base=+0.118)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0597` → IC=+0.136 (n=105)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.68€ cuando `delta_ratio_macro` |x|> 0.0597 (IC base=+0.099)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.160 (n=48)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 6.0 (IC base=+0.099)

- **PATRÓN** `ibs_15` > `0.5` → IC=+0.164 (n=105)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.82€ cuando `ibs_15` > 0.5 (IC base=+0.099)

- **PATRÓN** `dist_vwap_pct` > `0.3338` → IC=+0.265 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3338 (IC base=+0.099)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.773` → IC=+0.226 (n=49)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.773 (IC base=+0.099)
...
- **PATRÓN** `libro_liquidez` > `2490.4522` → IC=+0.146 (n=94)
  ...
  - _Acción_: Kelly boost +0.73€ cuando `libro_liquidez` >