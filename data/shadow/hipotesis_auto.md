# Hipótesis automáticas — 2026-07-19 04:21 UTC
_Generado por shadow_postmortem.py sobre 21880 resoluciones (PNL=+5301.84€)_
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.222 (n=631)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.182 (n=1619)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.239 (n=213)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.248 (n=141)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.212)

- **PATRÓN** `py_entrada` < `0.395` → IC=+0.263 (n=306)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.135 (n=1553)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.9035` → IC=+0.125 (n=1084)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` < 0.9035 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.174` → IC=+0.143 (n=228)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` > 6.174 (IC base=+0.113)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.016` → IC=+0.122 (n=960)
...
- **PATRÓN** `dist_vwap_pct` < `0.5173` → IC=+0.135 (n=220)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.5173 (IC base=+0.075)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.887` → IC=+0.197 (n=97)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 2.887 (IC base=+0.075)
...
- **PATRÓN** `sigma_h` < `0.0098` → IC=+0.200 (n=191)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0098 (IC base=+0.172)

- **PATRÓN** `sigma_h` > `0.0256` → IC=+0.200 (n=191)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0256 (IC base=+0.172)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.232 (n=214)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.172)

- **PATRÓN** `dist_vwap_pct` > `0.3076` → IC=+0.174 (n=87)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` > 0.3076 (IC base=+0.172)

- **PATRÓN** `dist_vwap_pct` < `0.5554` → IC=+0.167 (n=316)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.5554 (IC base=+0.172)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.536` → IC=+0.172 (n=285)
...
- **PATRÓN** `dist_vwap_pct` > `0.1687` → IC=+0.209 (n=393)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1687 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.061` → IC=+0.292 (n=157)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.061 (IC base=+0.165)

- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.189 (n=1039)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.143 (n=264)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` < 0.0061 (IC base=+0.137)

- **PATRÓN** `sigma_h` > `0.0026` → IC=+0.168 (n=266)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` > 0.0026 (IC base=+0.137)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.172 (n=132)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 15.0 (IC base=+0.137)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.141 (n=90)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` < 5.0 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.1165` → IC=+0.138 (n=67)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` > 0.1165 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.4763` → IC=+0.138 (n=230)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.4763 (IC base=+0.137)
...
- **PATRÓN** `dist_vwap_pct` > `0.1369` → IC=+0.182 (n=108)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` > 0.1369 (IC base=+0.148)

- **PATRÓN** `dist_vwap_pct` < `0.8807` → IC=+0.147 (n=216)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.145 (n=105)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.330 (n=256)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.302 (n=230)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.5452` → IC=+0.310 (n=256)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5452 (IC base=+0.299)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.458` → IC=+0.316 (n=226)
...
- **PATRÓN** `sigma_h` > `0.0136` → IC=+0.188 (n=325)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` > 0.0136 (IC base=+0.128)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.134 (n=887)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 8.0 (IC base=+0.128)

- **PATRÓN** `hora_utc` < `16.0` → IC=+0.132 (n=898)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` < 16.0 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` > `0.5395` → IC=+0.172 (n=193)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.5395 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` < `0.1169` → IC=+0.151 (n=491)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` < 0.1169 (IC base=+0.128)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.232` → IC=+0.284 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.232 (IC base=+0.128)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.135 (n=406)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` < 6.0 (IC base=+0.106)

- **PATRÓN** `dist_vwap_pct` < `0.7822` → IC=+0.129 (n=941)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.7822 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.589` → IC=+0.130 (n=244)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` > 3.589 (IC base=+0.106)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.031` → IC=+0.124 (n=838)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.888` → IC=+0.160 (n=98)
  ...
  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 2.888 (IC base=+0.082)

### GBM_LATE_15M_TARDIO#ETH#15min
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.171 (n=83)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0039 (IC base=+0.077)

- **PATRÓN** `dist_vwap_pct` < `0.2773` → IC=+0.136 (n=127)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.2773 (IC base=+0.077)

- **PATRÓN** `sigma_ewma_delta_pct` > `14.068` → IC=+0.259 (n=27)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 14.068 (IC base=+0.077)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.129 (n=95)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 8.0 (IC base=+0.077)

- **PATRÓN** `dist_vwap_pct` > `0.7129` → IC=+0.130 (n=25)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` > 0.7129 (IC base=+0.077)

- **PATRÓN** `dist_vwap_pct` < `0.1427` → IC=+0.128 (n=92)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.1427 (IC base=+0.077)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.593` → IC=-0.143 (n=26)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.133 (n=281)
...
- **PATRÓN** `ibs_15` < `0.0263` → IC=+0.124 (n=176)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.62€ cuando `ibs_15` < 0.0263 (IC base=+0.062)

- **PATRÓN** `dist_vwap_pct` > `0.4502` → IC=+0.123 (n=51)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` > 0.4502 (IC base=+0.062)

- **PATRÓN** `sigma_ewma_delta_pct` < `17.593` → IC=+0.133 (n=281)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` < 17.593 (IC base=+0.062)
...
- **PATRÓN** `sigma_h` < `0.0094` → IC=+0.227 (n=31)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0094 (IC base=+0.138)

- **PATRÓN** `drift_60min` |x|≤ `0.2192` → IC=+0.145 (n=29)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.73€ cuando `drift_60min` |x|≤ 0.2192 (IC base=+0.138)

- **PATRÓN** `drift_15min` |x|≤ `1.1387` → IC=+0.157 (n=65)

  - _Acción_: Kelly boost +0.78€ cuando `drift_15min` |x|≤ 1.1387 (IC base=+0.138)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1253` → IC=+0.167 (n=43)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.83€ cuando `delta_ratio_macro` |x|> 0.1253 (IC base=+0.138)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.172 (n=65)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 4.0 (IC base=+0.138)

- **PATRÓN** `ibs_15` < `0.0669` → IC=+0.173 (n=53)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.86€ cuando `ibs_15` < 0.0669 (IC base=+0.138)
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: IBS < 0.0669 correlaciona con éxito en UPDOWN_GBM#XRP#15min (IC=+0.173 n=53). Confirma señal de reversión media → alinear con BUY_YES.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 270 | +0.143 | +16.58€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 270 | +0.143 | +16.58€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 115 | +0.167 | +9.30€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 115 | +0.167 | +9.30€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 3673 | +0.185 | +50.23€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 2381 | +0.218 | +91.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 157 | +0.079 | +5.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 508 | +0.100 | -75.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 627 | +0.153 | +28.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 1065 | +0.193 | +31.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 794 | +0.210 | +1.83€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 1304 | +0.180 | +20.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 788 | +0.223 | +49.90€ | 0 | 8 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 1288 | +0.183 | -3.49€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 787 | +0.222 | +42.09€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 53 | +0.136 | +9.39€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 239 | +0.102 | -42.49€ | 0 | 0 |
...
| ✅ GBM_LATE_15M | 5716 | +0.106 | +1908.15€ | 0 | 9 |
| ✅ GBM_LATE_15M#15min | 5716 | +0.106 | +1908.15€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1361 | +0.077 | +245.07€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1361 | +0.077 | +245.07€ | 0 | 6 |
| ✅ GBM_LATE_15M#ETH | 