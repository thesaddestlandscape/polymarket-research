# Hipótesis automáticas — 2026-09-04 00:45 UTC
_Generado por shadow_postmortem.py sobre 273551 resoluciones (PNL=+26572.13€)_

## Patrones causales activos

### BALLENAS_CONFIRMADAS_15M
- **FILTRO** `py_entrada` < `0.515` → IC=-0.179 (n=110)
...
- **PATRÓN** `py_entrada` > `0.515` → IC=+0.263 (n=331)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.515 (IC base=+0.152)
...
- **PATRÓN** `banda_hit_calibrado` > `0.6274` → IC=+0.257 (n=331)

  - _Acción_: Kelly boost +1.00€ cuando `banda_hit_calibrado` > 0.6274 (IC base=+0.152)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.158 (n=238)
...
- **PATRÓN** `libro_liquidez` > `3094.168` → IC=+0.197 (n=150)
  ...
  - _Acción_: Kelly boost +0.99€ cuando `libro_liquidez` > 3094.168 (IC base=+0.152)
...
- **PATRÓN** `banda_hit_calibrado` > `0.8036` → IC=+0.267 (n=238)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `banda_hit_calibrado` > 0.8036 (IC base=+0.183)

- **PATRÓN** `banda_z` > `11.818` → IC=+0.273 (n=108)

  - _Acción_: Kelly boost +1.00€ cuando `banda_z` > 11.818 (IC base=+0.183)

- **PATRÓN** `ballenas_wallet_edge_medio` > `3.098` → IC=+0.188 (n=78)

  - _Acción_: Kelly boost +0.94€ cuando `ballenas_wallet_edge_medio` > 3.098 (IC base=+0.183)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.192 (n=222)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 7.0 (IC base=+0.183)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.191 (n=166)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `2918.5053` → IC=+0.202 (n=159)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2918.5053 (IC base=+0.183)
...
- **FILTRO** `restante_s_al_confirmar` < `146.66` → IC=-0.298 (n=3688)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=11064)
...
- **FILTRO** `restante_s_al_confirmar` < `142.53` → IC=-0.316 (n=818)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.153 (n=6388)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.210 (n=870)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.210 (n=1090)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.207)

- **PATRÓN** `libro_liquidez` > `13044.1569` → IC=+0.224 (n=386)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.140 (n=1369)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.195 (n=5181)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` < 15.0 (IC base=+0.189)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.201 (n=2935)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.176 (n=1309)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.178 (n=1061)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.237 (n=1179)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.186 (n=1281)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.206 (n=17398)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.147 (n=2405)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 12.0 (IC base=+0.146)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.176 (n=2558)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.228 (n=1105)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.226)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.178 (n=3000)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.228 (n=1122)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.236 (n=1500)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.186 (n=2255)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` < 12.0 (IC base=+0.182)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.227 (n=1491)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.137 (n=1697)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3577` → IC=+0.127 (n=3662)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` > 0.3577 (IC base=+0.080)

- **PATRÓN** `dist_vwap_pct` > `0.2741` → IC=+0.224 (n=476)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2741 (IC base=+0.080)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.603` → IC=+0.150 (n=1401)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 5.603 (IC base=+0.080)

- **PATRÓN** `volumen_regimen` < `0.6936` → IC=+0.224 (n=367)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6936 (IC base=+0.080)

- **PATRÓN** `volumen_regimen` > `1.0855` → IC=+0.243 (n=380)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0855 (IC base=+0.080)

- **PATRÓN** `volumen_pendiente_norm` > `0.1732` → IC=+0.175 (n=625)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.1732 (IC base=+0.080)

- **PATRÓN** `volumen_spike_ratio` < `2.88` → IC=+0.174 (n=2190)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 2.88 (IC base=+0.080)

- **PATRÓN** `volumen_spike_ratio` > `1.4754` → IC=+0.170 (n=2190)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 1.4754 (IC base=+0.080)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.197 (n=1094)

  - _Acción_: Kelly boost +0.99€ cuando `ballena_activa_n` < 54.0 (IC base=+0.080)

- **PATRÓN** `ibs_20min` < `0.4038` → IC=+0.128 (n=3523)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` < 0.4038 (IC base=+0.042)

- **PATRÓN** `dist_vwap_pct` < `0.3424` → IC=+0.146 (n=1274)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.3424 (IC base=+0.042)

- **PATRÓN** `volumen_regimen` < `0.6868` → IC=+0.150 (n=546)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 0.6868 (IC base=+0.042)

- **PATRÓN** `volumen_regimen` > `1.047` → IC=+0.138 (n=562)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` > 1.047 (IC base=+0.042)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0967` → IC=+0.236 (n=214)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0967 (IC base=+0.211)
...
- **PATRÓN** `dist_vwap_pct` > `0.1844` → IC=+0.221 (n=413)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1844 (IC base=+0.211)

- **PATRÓN** `dist_vwap_pct` < `0.49` → IC=+0.217 (n=577)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.49 (IC base=+0.211)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.225` → IC=+0.224 (n=310)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.225 (IC base=+0.211)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.068` → IC=+0.215 (n=643)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 7.068 (IC base=+0.211)

- **PATRÓN** `volumen_regimen` < `1.2608` → IC=+0.215 (n=641)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1` → IC=+0.217 (n=609)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1 (IC base=+0.211)

- **PATRÓN** `volumen_spike_ratio` < `2.109` → IC=+0.227 (n=543)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.109 (IC base=+0.211)

- **PATRÓN** `libro_liquidez` > `11005.6828` → IC=+0.234 (n=641)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11005.6828 (IC base=+0.211)
...
- **PATRÓN** `sigma_h` > `0.0096` → IC=+0.207 (n=257)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0096 (IC base=+0.160)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.199 (n=284)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1321` → IC=+0.160 (n=659)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.3915` → IC=+0.153 (n=96)
  ...
  - _Acción_: Kelly boost +0.77€ cuando `volumen_pendiente_norm` > 0.3915 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` < `3.9006` → IC=+0.154 (n=685)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 3.9006 (IC base=+0.160)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.183 (n=791)
...
- **FILTRO** `ibs_20min` > `0.8252` → IC=-0.189 (n=287)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.010 (n=863)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.149 (n=72)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=1078)
...
- **FILTRO** `ibs_20min` > `0.7921` → IC=-0.187 (n=410)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=1232)

- **FILTRO** `sigma_ewma_delta_pct` > `8.908` → IC=-0.137 (n=191)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.908
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=1451)
...
- **PATRÓN** `ibs_20min` > `0.2901` → IC=+0.138 (n=4527)
...
- **PATRÓN** `volumen_regimen` > `1.032` → IC=+0.235 (n=666)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.032 (IC base=+0.070)

- **PATRÓN** `volumen_pendiente_norm` < `0.115` → IC=+0.212 (n=2129)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.115 (IC base=+0.070)

- **PATRÓN** `volumen_pendiente_norm` > `0.2532` → IC=+0.232 (n=430)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2532 (IC base=+0.070)

- **PATRÓN** `volumen_spike_ratio` < `1.4917` → IC=+0.231 (n=734)
...
- **PATRÓN** `ballena_activa_n` < `95.0` → IC=+0.294 (n=1655)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 95.0 (IC base=+0.070)

- **PATRÓN** `ibs_20min` < `0.582` → IC=+0.132 (n=4384)
...
- **FILTRO** `ibs_20min` > `0.8618` → IC=-0.163 (n=381)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=1145)
...
- **PATRÓN** `sigma_h` > `0.0083` → IC=+0.280 (n=330)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0083 (IC base=+0.219)

- **PATRÓN** `drift_60min` |x|≤ `0.1252` → IC=+0.221 (n=321)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1252 (IC base=+0.219)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.249 (n=269)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.219)

- **PATRÓN** `ibs_20min` > `0.712` → IC=+0.253 (n=650)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.712 (IC base=+0.219)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.299` → IC=+0.290 (n=179)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.299 (IC base=+0.219)
...
- **PATRÓN** `volumen_spike_ratio` > `1.7205` → IC=+0.225 (n=644)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7205 (IC base=+0.219)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.247 (n=745)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.219)

- **PATRÓN** `ballena_activa_n` < `21.0` → IC=+0.252 (n=143)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 21.0 (IC base=+0.219)
...
- **FILTRO** `ibs_20min` > `0.75` → IC=-0.224 (n=281)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.037 (n=844)

- **FILTRO** `sigma_ewma_delta_pct` > `4.705` → IC=-0.149 (n=286)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.257 (n=348)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.191` → IC=+0.290 (n=402)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.191 (IC base=+0.249)

- **PATRÓN** `volumen_regimen` > `0.8448` → IC=+0.289 (n=492)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5754` → IC=+0.257 (n=677)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5754 (IC base=+0.249)

- **PATRÓN** `volumen_spike_ratio` > `1.8444` → IC=+0.253 (n=451)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8444 (IC base=+0.249)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.254 (n=861)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.249)

- **PATRÓN** `libro_liquidez` > `2467.2556` → IC=+0.253 (n=659)
...
- **PATRÓN** `sigma_h` > `0.0106` → IC=+0.207 (n=1274)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0106 (IC base=+0.161)

- **PATRÓN** `drift_60min` |x|≤ `0.3424` → IC=+0.168 (n=3365)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.283 (n=1750)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.161)

- **PATRÓN** `dist_vwap_pct` > `0.7959` → IC=+0.236 (n=843)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7959 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.033` → IC=+0.248 (n=804)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.033 (IC base=+0.161)

- **PATRÓN** `volumen_regimen` > `0.6285` → IC=+0.164 (n=2584)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` > 0.6285 (IC base=+0.161)

- **PATRÓN** `volumen_pendiente_norm` > `0.106` → IC=+0.183 (n=1427)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_pendiente_norm` > 0.106 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` < `2.324` → IC=+0.161 (n=3128)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.324 (IC base=+0.161)

- **PATRÓN** `libro_spread` < `0.03` → IC=+0.165 (n=3905)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.03 (IC base=+0.161)

- **PATRÓN** `libro_liquidez` > `3936.5622` → IC=+0.177 (n=1274)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 3936.5622 (IC base=+0.161)

- **PATRÓN** `ballena_activa_n` < `143.0` → IC=+0.178 (n=2738)

  - _Acción_: Kelly boost +0.89€ cuando `ballena_activa_n` < 143.0 (IC base=+0.161)

- **PATRÓN** `sigma_h` < `0.0083` → IC=+0.190 (n=3125)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `volumen_regimen` < `1.1749` → IC=+0.169 (n=2686)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` < 1.1749 (IC base=+0.180)

- **PATRÓN** `volumen_regimen` > `0.6234` → IC=+0.162 (n=2686)
...
- **PATRÓN** `volumen_spike_ratio` < `1.576` → IC=+0.174 (n=1287)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4155` → IC=+0.172 (n=566)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.86€ cuando `drift_60min` |x|≤ 0.4155 (IC base=+0.167)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.194 (n=580)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` > 6.0 (IC base=+0.167)

- **PATRÓN** `ibs_20min` > `0.4793` → IC=+0.208 (n=566)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `7.439` → IC=+0.167 (n=538)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` < 7.439 (IC base=+0.167)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4814` → IC=+0.202 (n=236)
...
- **PATRÓN** `libro_liquidez` > `11289.7522` → IC=+0.191 (n=506)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 11289.7522 (IC base=+0.167)
...
- **PATRÓN** `sigma_h` > `0.0095` → IC=+0.218 (n=200)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0095 (IC base=+0.168)

- **PATRÓN** `drift_60min` |x|≤ `0.2845` → IC=+0.183 (n=525)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.92€ cuando `drift_60min` |x|≤ 0.2845 (IC base=+0.168)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.183 (n=203)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 17.0 (IC base=+0.168)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.186 (n=205)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.546` → IC=+0.314 (n=138)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.546 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.1302` → IC=+0.170 (n=216)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` > 0.1302 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `1.7062` → IC=+0.160 (n=534)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 1.7062 (IC base=+0.168)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.192 (n=601)
...
- **PATRÓN** `sigma_h` > `0.0177` → IC=+0.215 (n=489)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0177 (IC base=+0.190)

- **PATRÓN** `drift_60min` |x|≤ `0.1724` → IC=+0.208 (n=323)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.8875` → IC=+0.272 (n=489)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8875 (IC base=+0.190)

- **PATRÓN** `dist_vwap_pct` > `0.167` → IC=+0.224 (n=527)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.167 (IC base=+0.190)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.219` → IC=+0.240 (n=363)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.219 (IC base=+0.190)

- **PATRÓN** `volumen_regimen` > `0.686` → IC=+0.208 (n=656)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.686 (IC base=+0.190)
...
- **PATRÓN** `volumen_spike_ratio` < `2.5276` → IC=+0.208 (n=694)
...
- **PATRÓN** `ibs_20min` > `0.476` → IC=+0.169 (n=799)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.157` → IC=+0.181 (n=202)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 8.157 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` < `0.8709` → IC=+0.159 (n=429)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.8709 (IC base=+0.138)

- **PATRÓN** `volumen_regimen` > `1.1751` → IC=+0.154 (n=215)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 1.1751 (IC base=+0.138)

- **PATRÓN** `volumen_pendiente_norm` > `0.18` → IC=+0.174 (n=222)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.18 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` < `2.2754` → IC=+0.149 (n=654)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 2.2754 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` > `1.8612` → IC=+0.144 (n=495)
...
- **PATRÓN** `ballena_activa_n` < `140.0` → IC=+0.182 (n=3099)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0966` → IC=+0.167 (n=250)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.3271` → IC=+0.205 (n=750)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.417` → IC=+0.174 (n=652)
  ...
  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` < 4.417 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` < `0.7145` → IC=+0.178 (n=330)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` < 0.7145 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` > `1.1034` → IC=+0.170 (n=340)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_regimen` > 1.1034 (IC base=+0.160)

- **PATRÓN** `volumen_pendiente_norm` < `0.0723` → IC=+0.175 (n=628)
...
- **PATRÓN** `volumen_spike_ratio` < `2.413` → IC=+0.178 (n=700)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 2.413 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` > `1.7228` → IC=+0.180 (n=467)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_spike_ratio` > 1.7228 (IC base=+0.160)

- **PATRÓN** `libro_liquidez` > `10634.3743` → IC=+0.188 (n=670)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 10634.3743 (IC base=+0.160)

- **PATRÓN** `ballena_activa_n` < `510.0` → IC=+0.172 (n=625)

  - _Acción_: Kelly boost +0.86€ cuando `ballena_activa_n` < 510.0 (IC base=+0.160)
...
- **PATRÓN** `volumen_spike_ratio` < `3.9401` → IC=+0.199 (n=583)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.9401 (IC base=+0.203)

- **PATRÓN** `volumen_spike_ratio` > `2.4711` → IC=+0.203 (n=389)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4711 (IC base=+0.203)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.233 (n=664)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.203)

- **PATRÓN** `libro_liquidez` > `1451.1486` → IC=+0.209 (n=657)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1451.1486 (IC base=+0.203)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.172` → IC=+0.244 (n=435)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.172 (IC base=+0.198)

- **PATRÓN** `volumen_regimen` > `1.2349` → IC=+0.230 (n=291)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2349 (IC base=+0.198)

- **PATRÓN** `volumen_pendiente_norm` > `0.1676` → IC=+0.240 (n=221)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1676 (IC base=+0.198)

- **PATRÓN** `volumen_spike_ratio` > `1.4407` → IC=+0.202 (n=822)
...
- **PATRÓN** `sigma_h` < `0.01` → IC=+0.134 (n=1313)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.01 (IC base=+0.120)

- **PATRÓN** `drift_60min` |x|≤ `0.5519` → IC=+0.131 (n=1492)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.66€ cuando `drift_60min` |x|≤ 0.5519 (IC base=+0.120)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.976` → IC=+0.123 (n=1514)

  - _Acción_: Kelly boost +0.61€ cuando `sigma_ewma_delta_pct` < 5.976 (IC base=+0.120)

- **PATRÓN** `volumen_regimen` > `0.8967` → IC=+0.121 (n=666)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4616` → IC=+0.148 (n=492)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 1.4616 (IC base=+0.120)

- **PATRÓN** `volumen_spike_ratio` > `1.8992` → IC=+0.139 (n=984)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_spike_ratio` > 1.8992 (IC base=+0.120)

- **PATRÓN** `libro_liquidez` > `8994.0735` → IC=+0.136 (n=676)

  - _Acción_: Kelly boost +0.68€ cuando `libro_liquidez` > 8994.0735 (IC base=+0.120)
...
- **PATRÓN** `sigma_h` < `0.009` → IC=+0.151 (n=548)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.4818` → IC=+0.149 (n=548)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.4818 (IC base=+0.135)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.194 (n=194)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` > 18.0 (IC base=+0.135)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.156 (n=219)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 5.0 (IC base=+0.135)

- **PATRÓN** `ibs_20min` > `0.186` → IC=+0.148 (n=549)
...
- **PATRÓN** `dist_vwap_pct` < `0.4443` → IC=+0.145 (n=517)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` < 0.4443 (IC base=+0.135)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.953` → IC=+0.150 (n=550)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` < 6.953 (IC base=+0.135)

- **PATRÓN** `volumen_regimen` < `1.2728` → IC=+0.145 (n=548)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 1.2728 (IC base=+0.135)

- **PATRÓN** `volumen_pendiente_norm` < `0.1175` → IC=+0.138 (n=504)
...
- **PATRÓN** `libro_liquidez` > `8953.8054` → IC=+0.146 (n=490)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.183 (n=1479)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=4550)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.147 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=468)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.135 (n=3534)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=10627)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.283 (n=3485)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=10676)

- **FILTRO** `ibs_7min` < `0.7118` → IC=-0.247 (n=3540)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7118
  - _Potencial_: sin este filtro IC_bueno=-0.047 (n=10621)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.171 (n=4736)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=9425)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.220 (n=4390)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=13447)

- **FILTRO** `ibs_7min` > `0.7143` → IC=-0.172 (n=4445)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7143
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=13392)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.155 (n=606)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
...
- **FILTRO** `py_entrada` < `0.31` → IC=-0.316 (n=510)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=1554)

- **FILTRO** `ibs_7min` < `0.3` → IC=-0.261 (n=679)
...
- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.226 (n=513)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.080 (n=1551)
...
- **FILTRO** `drift_7min_pct` |x|> `0.1139` → IC=-0.127 (n=1054)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1139
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=2048)

- **FILTRO** `ibs_7min` > `0.8349` → IC=-0.190 (n=775)

  - _Acción_: SKIP cuando `ibs_7min` > 0.8349
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=2327)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.138 (n=575)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
...
- **FILTRO** `py_entrada` < `0.36` → IC=-0.260 (n=643)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=2002)
...
- **FILTRO** `ballena_activa_n` > `168.0` → IC=-0.180 (n=657)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=1988)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.185 (n=658)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.097 (n=1436)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.311 (n=666)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=1428)

- **FILTRO** `ibs_7min` < `0.2127` → IC=-0.292 (n=523)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2127
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=1571)

- **FILTRO** `ballena_activa_n` > `12.0` → IC=-0.252 (n=493)

  - _Acción_: SKIP cuando `ballena_activa_n` > 12.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=1601)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.224 (n=744)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=2454)

- **FILTRO** `ibs_7min` > `0.797` → IC=-0.170 (n=799)

  - _Acción_: SKIP cuando `ibs_7min` > 0.797
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=2399)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.139 (n=720)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.259 (n=587)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=1815)

- **FILTRO** `ibs_7min` < `0.7509` → IC=-0.196 (n=600)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7509
  - _Potencial_: sin este filtro IC_bueno=-0.076 (n=1802)
...
- **FILTRO** `ibs_7min` > `0.2755` → IC=-0.179 (n=602)
...
- **FILTRO** `ballena_activa_n` > `34.0` → IC=-0.182 (n=592)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=1820)
...
- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.193 (n=626)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=2015)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.178 (n=800)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=2552)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.253 (n=614)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=2503)

- **FILTRO** `ibs_7min` > `0.7767` → IC=-0.162 (n=779)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7767
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=2338)
...
- **PATRÓN** `sigma_h` < `0.0035` → IC=+0.165 (n=174)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.82€ cuando `sigma_h` < 0.0035 (IC base=+0.123)

- **PATRÓN** `sigma_h` > `0.0053` → IC=+0.143 (n=348)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.71€ cuando `sigma_h` > 0.0053 (IC base=+0.123)

- **PATRÓN** `drift_60min` |x|≤ `0.168` → IC=+0.123 (n=459)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.61€ cuando `drift_60min` |x|≤ 0.168 (IC base=+0.123)
...
- **PATRÓN** `hora_utc` < `19.0` → IC=+0.135 (n=560)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` < 19.0 (IC base=+0.123)

- **PATRÓN** `ibs_15` > `0.5728` → IC=+0.214 (n=522)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.5728 (IC base=+0.123)

- **PATRÓN** `dist_vwap_pct` > `0.4346` → IC=+0.149 (n=132)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.4346 (IC base=+0.123)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.132 (n=539)

  - _Acción_: Kelly boost +0.66€ cuando `libro_spread` < 0.01 (IC base=+0.123)

- **PATRÓN** `libro_liquidez` > `3043.835` → IC=+0.149 (n=348)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 3043.835 (IC base=+0.123)
...
- **PATRÓN** `sigma_h` < `0.0031` → IC=+0.186 (n=100)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.0031 (IC base=+0.162)

- **PATRÓN** `sigma_h` > `0.0045` → IC=+0.171 (n=68)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` > 0.0045 (IC base=+0.162)

- **PATRÓN** `drift_60min` |x|≤ `0.1916` → IC=+0.191 (n=150)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.1916 (IC base=+0.162)

- **PATRÓN** `drift_15min` |x|≤ `0.4863` → IC=+0.167 (n=100)

  - _Acción_: Kelly boost +0.83€ cuando `drift_15min` |x|≤ 0.4863 (IC base=+0.162)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.192 (n=157)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 4.0 (IC base=+0.162)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.160 (n=154)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 17.0 (IC base=+0.162)

- **PATRÓN** `ibs_15` > `0.8766` → IC=+0.284 (n=100)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8766 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` > `0.306` → IC=+0.206 (n=66)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.306 (IC base=+0.162)

- **PATRÓN** `dist_vwap_pct` < `0.5508` → IC=+0.165 (n=165)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` < 0.5508 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.367` → IC=+0.214 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.367 (IC base=+0.162)

- **PATRÓN** `sigma_ewma_delta_pct` < `18.708` → IC=+0.160 (n=151)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` < 18.708 (IC base=+0.162)

- **PATRÓN** `libro_liquidez` > `14003.3235` → IC=+0.231 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 14003.3235 (IC base=+0.162)
...
- **PATRÓN** `ibs_15` < `0.1304` → IC=+0.175 (n=164)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.87€ cuando `ibs_15` < 0.1304 (IC base=+0.034)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1153` → IC=+0.348 (n=130)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.324 (n=237)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.324)

- **PATRÓN** `libro_liquidez` > `8237.5947` → IC=+0.335 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8237.5947 (IC base=+0.324)

- **PATRÓN** `ballena_activa_n` < `522.0` → IC=+0.379 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 522.0 (IC base=+0.324)

### UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1189` → IC=+0.325 (n=38)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +1.00€ cuando `pct_spot_vs_ref` |x|≤ 0.1189 (IC base=+0.310)
...
- **PATRÓN** `drift_60min` |x|≤ `0.16` → IC=+0.333 (n=100)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.16 (IC base=+0.310)

- **PATRÓN** `drift_15min` |x|≤ `0.411` → IC=+0.327 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.411 (IC base=+0.310)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0911` → IC=+0.308 (n=102)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0911 (IC base=+0.310)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1224` → IC=+0.397 (n=27)
...
- **PATRÓN** `ibs_15` > `0.9198` → IC=+0.385 (n=76)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.9198 (IC base=+0.310)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `18.716` → IC=+0.309 (n=40)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 18.716 (IC base=+0.310)

- **PATRÓN** `sigma_ewma_delta_pct` < `8.509` → IC=+0.332 (n=87)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 8.509 (IC base=+0.310)

- **PATRÓN** `libro_liquidez` > `11333.8932` → IC=+0.352 (n=52)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11333.8932 (IC base=+0.310)

- **PATRÓN** `ballena_activa_n` < `625.0` → IC=+0.425 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 625.0 (IC base=+0.310)
...
### UPDOWN_GBM_IBS_ALTO
- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.284 (n=331)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.3595` → IC=+0.307 (n=226)
...
- **PATRÓN** `ibs_15` > `0.8341` → IC=+0.317 (n=331)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.8341 (IC base=+0.283)

- **PATRÓN** `dist_vwap_pct` > `0.306` → IC=+0.324 (n=151)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.306 (IC base=+0.283)

- **PATRÓN** `sigma_ewma_delta_pct` < `18.92` → IC=+0.284 (n=346)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 18.92 (IC base=+0.283)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.287 (n=406)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.283)

- **PATRÓN** `libro_liquidez` > `12693.279` → IC=+0.322 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12693.279 (IC base=+0.283)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1567` → IC=+0.292 (n=166)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1567 (IC base=+0.279)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1286` → IC=+0.312 (n=126)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1286 (IC base=+0.279)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.311 (n=199)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.279)

- **PATRÓN** `ibs_15` > `0.968` → IC=+0.343 (n=87)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.968 (IC base=+0.279)

- **PATRÓN** `dist_vwap_pct` > `0.3186` → IC=+0.354 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3186 (IC base=+0.279)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `8.606` → IC=+0.285 (n=161)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 8.606 (IC base=+0.279)

- **PATRÓN** `libro_liquidez` > `15285.027` → IC=+0.346 (n=63)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 15285.027 (IC base=+0.279)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.5728 sube el IC de +0.123 a +0.214 en UPDOWN_GBM#15min (n=522). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.8766 sube el IC de +0.162 a +0.284 en UPDOWN_GBM#BTC#15min (n=100). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1304 sube el IC de +0.034 a +0.175 en UPDOWN_GBM#XRP#15min (n=164). Ya aplicado como kelly_boost=+0.87€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.8341 sube el IC de +0.283 a +0.317 en UPDOWN_GBM_IBS_ALTO (n=331). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_IBS_ALTO#BTC#15min**: dentro de BUY_YES, IBS > 0.968 sube el IC de +0.279 a +0.343 en UPDOWN_GBM_IBS_ALTO#BTC#15min (n=87). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD**: dentro de BUY_YES, IBS > 0.7853 sube el IC de +0.324 a +0.373 en UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD (n=195). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min**: dentro de BUY_YES, IBS > 0.9198 sube el IC de +0.310 a +0.385 en UPDOWN_GBM_15M_CROSS_WINDOW_SPREAD#BTC#15min (n=76). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 803 | +0.086 | +43.35€ | 2 | 10 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 803 | +0.086 | +43.35€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE | 23 | +0.060 | +0.96€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#DOGE#15min | 23 | +0.060 | +0.96€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 525 | +0.105 | +35.81€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 525 | +0.105 | +35.81€ | 2 | 11 |
...
| ✅ BALLENAS_TARDIAS | 14752 | -0.112 | -2495.41€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 955 | -0.010 | -141.55€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 13797 | -0.118 | -2353.86€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#ETH | 3947 | -0.049 | -364.21€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 3947 | -0.049 | -364.21€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 3275 | -0.128 | -289.42€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 3275 | -0.128 | -289.42€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 2698 | -0.194 | -748.66€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 2698 | -0.194 | -748.66€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 4047 | -0.072 | +1803.41€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 1152 | -0.009 | +852.32€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 2895 | -0.097 | +951.09€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 4047 | -0.072 | +1803.41€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 1152 | -0.009 | +852.32€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 2895 | -0.097 | +951.09€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 47548 | +0.113 | -3017.79€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 7953 | +0.184 | -279.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 162 | -0.110 | -54.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 35693 | +0.098 | -2613.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 3740 | +0.117 | -70.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 5997 | +0.081 | -810.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 27 | -0.121 | +3.75€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 5955 | +0.083 | -802.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 9534 | +0.132 | -234.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 2340 | +0.197 | -115.35€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 5895 | +0.109 | -147.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1257 | +0.127 | +50.51€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 6012 | +0.081 | -771.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 29 | +0.016 | +3.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#240min | 9 | -0.143 | -7.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 5974 | +0.082 | -767.71€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 10286 | +0.127 | -157.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 2963 | +0.172 | -13.61€ | 1 | 8 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 5933 | +0.112 | -95.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 1378 | +0.096 | -39.04€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 9725 | +0.125 | -624.17€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 2573 | +0.192 | -159.26€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 79 | -0.006 | -2.61€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 5968 | +0.097 | -380.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1105 | +0.131 | -81.51€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 5994 | +0.104 | -419.72€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 21 | -0.022 | +1.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 5 | -0.018 | -1.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 5968 | +0.105 | -419.55€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 7912 | +0.177 | -620.68€ | 2 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 7912 | +0.177 | -620.68€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 1973 | +0.168 | -211.41€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 1973 | +0.168 | -211.41€ | 0 | 4 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 1767 | +0.237 | -39.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 1767 | +0.237 | -39.10€ | 0 | 3 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL | 79 | -0.204 | +13.76€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#SOL#15min | 79 | -0.204 | +13.76€ | 3 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP | 1928 | +0.182 | -155.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#XRP#15min | 1928 | +0.182 | -155.50€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 25350 | +0.188 | -2408.09€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 25350 | +0.188 | -2408.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 4518 | +0.146 | -727.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 4518 | +0.146 | -727.09€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 3954 | +0.226 | -137.70€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 3954 | +0.226 | -137.70€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 4390 | +0.161 | -607.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 4390 | +0.161 | -607.52€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH | 4026 | +0.220 | -163.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#ETH#5min | 4026 | +0.220 | -163.50€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 4190 | +0.199 | -316.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 4190 | +0.199 | -316.76€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 4272 | +0.182 | -455.52€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 4272 | +0.182 | -455.52€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 9209 | +0.132 | +343.75€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 9209 | +0.132 | +343.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE | 4575 | +0.138 | +209.96€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min | 4575 | +0.138 | +209.96€ | 0 | 5 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 4634 | +0.127 | +133.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 4634 | +0.127 | +133.80€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO_DEPTH_FASE0 | 