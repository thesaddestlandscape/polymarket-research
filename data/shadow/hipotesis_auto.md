# Hipótesis automáticas — 2026-07-21 06:33 UTC
_Generado por shadow_postmortem.py sobre 26157 resoluciones (PNL=+6281.41€)_
...
- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.125 (n=1611)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` > 0.0086 (IC base=+0.107)

- **PATRÓN** `dist_vwap_pct` > `0.1605` → IC=+0.128 (n=444)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` > 0.1605 (IC base=+0.107)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `9.238` → IC=+0.157 (n=389)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` < 9.238 (IC base=+0.167)
...
- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.187 (n=847)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` > 0.0072 (IC base=+0.155)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.181 (n=628)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 15.0 (IC base=+0.155)

- **PATRÓN** `dist_vwap_pct` < `0.3854` → IC=+0.169 (n=1087)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` < 0.3854 (IC base=+0.155)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.122` → IC=+0.186 (n=234)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 6.122 (IC base=+0.155)
...
- **PATRÓN** `sigma_h` < `0.0157` → IC=+0.286 (n=316)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0157 (IC base=+0.282)

- **PATRÓN** `sigma_h` > `0.0077` → IC=+0.304 (n=315)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0077 (IC base=+0.282)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.301 (n=319)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.282)

- **PATRÓN** `dist_vwap_pct` > `0.3145` → IC=+0.359 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3145 (IC base=+0.282)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.588` → IC=+0.300 (n=23)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.588 (IC base=+0.282)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.004` → IC=+0.289 (n=264)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.004 (IC base=+0.282)
...
- **PATRÓN** `sigma_h` > `0.0208` → IC=+0.215 (n=135)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0208 (IC base=+0.206)
...
- **PATRÓN** `dist_vwap_pct` > `0.4269` → IC=+0.254 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4269 (IC base=+0.206)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.122` → IC=+0.213 (n=375)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 10.122 (IC base=+0.206)
...
| ✅ GBM_LATE_15M | 6410 | +0.102 | +2126.27€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 6410 | +0.102 | +2126.27€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#XRP | 1688 | +0.162 | +942.01€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1688 | +0.162 | +942.01€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 3606 | +0.142 | +2064.98€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 3606 | +0.142 | +2064.98€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 829 | +0.267 | +853.33€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 829 | +0.267 | +853.33€ | 0 | 12 |
...
| ✅ GBM_LATE_15M_TARDIO | 3585 | +0.093 | +1221.26€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 3585 | +0.093 | +1221.26€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 1046 | +0.204 | +800.33€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 1046 | +0.204 | +800.33€ | 0 | 13 |
...
| ✅ LATE_WINDOW_5MIN | 128 | +0.085 | +19.70€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN#5min | 128 | +0.085 | +19.70€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN#BTC | 128 | +0.085 | +19.70€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN#BTC#5min | 128 | +0.085 | +19.70€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 935 | +0.172 | +433.54€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 935 | +0.172 | +433.54€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 231 | +0.234 | +184.08€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 231 | +0.234 | +184.08€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.194: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.058 n=915 | neutral: IC=+0.001 n=689 | overbought(IBS>0.7): IC=+0.195 n=903
  - _Datos_: n=2684 IC=+0.097 PNL=+638.59€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.117 n=15740 | tras_1loss IC=+0.088 n=10215 | tras_2loss IC=+0.053 n=4170/40 | gap=+0.064 (umbral 0.05)
...
**🔶 H-LATE-WINDOW-5MIN** — Late-window BTC 5min — arbitraje timing vs Polymarket
  ...
  - _Estado_: SEÑAL POSITIVA en BTC (IC=+0.085 n=128) pero sin cruzar ≥2 pares más — sin otros pares con datos
  - _Datos_: n=128 IC=+0.085 PNL=+19.70€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=23693 IC=+0.118 PNL=+6356.25€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=23693 IC=+0.118 PNL=+6356.25€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.152 > 0.08 con n=444 PNL=+269.74€
  - _Datos_: n=444 IC=+0.152 PNL=+269.74€
