# Hipótesis automáticas — 2026-09-26 02:04 UTC
_Generado por shadow_postmortem.py sobre 613511 resoluciones (PNL=+69482.39€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.36` → IC=-0.227 (n=7223)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.36
  - _Potencial_: sin este filtro IC_bueno=-0.041 (n=21671)
...
- **FILTRO** `restante_s_al_confirmar` < `499.92` → IC=-0.140 (n=376)
...
- **FILTRO** `restante_s_al_confirmar` < `166.11` → IC=-0.211 (n=1755)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 166.11
  - _Potencial_: sin este filtro IC_bueno=-0.049 (n=5267)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `127.7` → IC=-0.347 (n=1420)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 127.7
  - _Potencial_: sin este filtro IC_bueno=-0.111 (n=4263)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.205 (n=14136)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.136 (n=14204)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` < 7.0 (IC base=+0.127)

- **PATRÓN** `py_entrada` < `0.35` → IC=+0.231 (n=11152)
...
- **PATRÓN** `libro_liquidez` > `7868.8792` → IC=+0.175 (n=2178)

  - _Acción_: Kelly boost +0.87€ cuando `libro_liquidez` > 7868.8792 (IC base=+0.127)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.146 (n=2442)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.200 (n=11249)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.197)

- **PATRÓN** `py_entrada` > `0.74` → IC=+0.225 (n=3869)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.173 (n=2699)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` < 17.0 (IC base=+0.167)

- **PATRÓN** `py_entrada` < `0.73` → IC=+0.176 (n=2700)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.247 (n=942)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.241)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.348 (n=426)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.184 (n=2675)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 17.0 (IC base=+0.179)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.185 (n=2294)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.194 (n=2612)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` < 17.0 (IC base=+0.192)

- **PATRÓN** `py_entrada` < `0.71` → IC=+0.197 (n=2022)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.235 (n=15595)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.180 (n=4851)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` < 12.0 (IC base=+0.176)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.190 (n=6559)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.224 (n=6268)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.223)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.263 (n=3566)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.189 (n=6388)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.221 (n=2377)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.220)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.267 (n=2161)
...
- **PATRÓN** `py_entrada` > `0.75` → IC=+0.255 (n=2330)
...
### GBM_LATE_15M
- **PATRÓN** `drift_60min` |x|≤ `0.4867` → IC=+0.123 (n=8316)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 0.4867 (IC base=+0.106)

- **PATRÓN** `ibs_20min` > `0.9825` → IC=+0.244 (n=2772)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3503` → IC=+0.167 (n=1872)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.3503 (IC base=+0.162)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.172 (n=907)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 15.0 (IC base=+0.162)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.166 (n=1254)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_spike_ratio` > `1.4361` → IC=+0.165 (n=1753)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` > 1.4361 (IC base=+0.162)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.177 (n=1908)
...
- **PATRÓN** `sigma_h` < `0.0032` → IC=+0.233 (n=635)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0032 (IC base=+0.217)

- **PATRÓN** `drift_60min` |x|≤ `0.1142` → IC=+0.240 (n=632)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.217 (n=1457)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.217)

- **PATRÓN** `ibs_20min` > `0.9034` → IC=+0.263 (n=652)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9034 (IC base=+0.217)

- **PATRÓN** `dist_vwap_pct` > `0.2078` → IC=+0.217 (n=755)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2078 (IC base=+0.217)

- **PATRÓN** `dist_vwap_pct` < `0.3703` → IC=+0.220 (n=1333)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3703 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.779` → IC=+0.261 (n=245)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.779 (IC base=+0.217)

- **PATRÓN** `volumen_regimen` < `1.2505` → IC=+0.219 (n=1437)
...
- **PATRÓN** `libro_liquidez` > `11187.4445` → IC=+0.226 (n=1437)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11187.4445 (IC base=+0.217)

- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.167 (n=1000)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.3` → IC=+0.144 (n=1367)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` < 4.3 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` < `1.1967` → IC=+0.151 (n=1497)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.190 (n=1650)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `9.199` → IC=+0.253 (n=387)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.199 (IC base=+0.186)

- **PATRÓN** `volumen_pendiente_norm` < `0.2077` → IC=+0.187 (n=1853)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` < 0.2077 (IC base=+0.186)

- **PATRÓN** `volumen_pendiente_norm` > `0.3576` → IC=+0.196 (n=248)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.3576 (IC base=+0.186)

- **PATRÓN** `volumen_spike_ratio` > `2.8061` → IC=+0.205 (n=798)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8061 (IC base=+0.186)
...
- **PATRÓN** `drift_60min` |x|≤ `0.6144` → IC=+0.211 (n=1593)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.6144 (IC base=+0.210)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `5.512` → IC=+0.210 (n=1731)
...
- **PATRÓN** `libro_liquidez` > `1905.4784` → IC=+0.215 (n=722)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.144 (n=102)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.025 (n=2272)
...
- **PATRÓN** `volumen_regimen` < `0.8471` → IC=+0.162 (n=578)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_regimen` < 0.8471 (IC base=+0.018)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5219` → IC=+0.174 (n=726)
...
- **FILTRO** `ibs_20min` > `0.25` → IC=-0.125 (n=2245)

  - _Acción_: SKIP cuando `ibs_20min` > 0.25
  - _Potencial_: sin este filtro IC_bueno=+0.125 (n=1113)

- **FILTRO** `sigma_ewma_delta_pct` > `8.692` → IC=-0.209 (n=359)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.692
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=2999)
...
- **PATRÓN** `ibs_20min` < `0.25` → IC=+0.125 (n=1113)
...
- **FILTRO** `drift_60min` |x|> `0.6576` → IC=-0.184 (n=586)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6576
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=1761)

- **FILTRO** `ibs_20min` < `0.72` → IC=-0.153 (n=1548)

  - _Acción_: SKIP cuando `ibs_20min` < 0.72
  - _Potencial_: sin este filtro IC_bueno=+0.104 (n=799)
...
- **FILTRO** `ibs_20min` > `0.77` → IC=-0.203 (n=857)

  - _Acción_: SKIP cuando `ibs_20min` > 0.77
  - _Potencial_: sin este filtro IC_bueno=+0.038 (n=2582)

- **PATRÓN** `dist_vwap_pct` > `0.4566` → IC=+0.312 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4566 (IC base=-0.065)
...
- **PATRÓN** `volumen_regimen` > `0.6163` → IC=+0.305 (n=372)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6163 (IC base=-0.065)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4047` → IC=+0.291 (n=353)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4047 (IC base=-0.065)
...
- **PATRÓN** `sigma_h` > `0.0098` → IC=+0.198 (n=3489)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.99€ cuando `sigma_h` > 0.0098 (IC base=+0.097)

- **PATRÓN** `ibs_20min` > `0.4766` → IC=+0.187 (n=9346)

  - _Acción_: Kelly boost +0.93€ cuando `ibs_20min` > 0.4766 (IC base=+0.097)

- **PATRÓN** `dist_vwap_pct` > `0.7595` → IC=+0.285 (n=1117)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7595 (IC base=+0.097)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.615` → IC=+0.156 (n=4905)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 3.615 (IC base=+0.097)

- **PATRÓN** `volumen_regimen` > `0.6889` → IC=+0.250 (n=3338)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4646` → IC=+0.237 (n=2022)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4646 (IC base=+0.097)

- **PATRÓN** `volumen_spike_ratio` > `2.6512` → IC=+0.244 (n=2021)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6512 (IC base=+0.097)

- **PATRÓN** `ballena_activa_n` < `96.0` → IC=+0.267 (n=5580)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 96.0 (IC base=+0.097)

- **PATRÓN** `sigma_h` > `0.0092` → IC=+0.160 (n=3463)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` > 0.0092 (IC base=+0.074)

- **PATRÓN** `ibs_20min` < `0.5499` → IC=+0.155 (n=9138)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.5499 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` > `0.7244` → IC=+0.241 (n=659)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7244 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` < `0.2545` → IC=+0.241 (n=2917)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2545 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` < `0.7077` → IC=+0.243 (n=1354)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7077 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` > `1.2033` → IC=+0.249 (n=1026)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2033 (IC base=+0.074)

- **PATRÓN** `volumen_pendiente_norm` > `0.2435` → IC=+0.305 (n=772)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2435 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` < `1.6004` → IC=+0.260 (n=1794)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6004 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` > `2.2988` → IC=+0.260 (n=1848)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2988 (IC base=+0.074)

- **PATRÓN** `ballena_activa_n` < `84.0` → IC=+0.265 (n=3954)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 84.0 (IC base=+0.074)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `ibs_20min` < `0.2587` → IC=-0.146 (n=721)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2587
  - _Potencial_: sin este filtro IC_bueno=+0.107 (n=2164)

- **FILTRO** `sigma_ewma_delta_pct` > `4.536` → IC=-0.157 (n=549)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.536
  - _Potencial_: sin este filtro IC_bueno=+0.025 (n=1849)

- **PATRÓN** `ibs_20min` > `0.8938` → IC=+0.265 (n=722)
...
- **PATRÓN** `ibs_20min` > `0.8636` → IC=+0.158 (n=700)
...
- **PATRÓN** `volumen_regimen` > `0.6718` → IC=+0.170 (n=864)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1553` → IC=+0.241 (n=249)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1553 (IC base=+0.006)

- **PATRÓN** `ballena_activa_n` < `462.0` → IC=+0.221 (n=547)
...
- **PATRÓN** `sigma_h` > `0.01` → IC=+0.315 (n=598)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.01 (IC base=+0.281)

- **PATRÓN** `drift_60min` |x|≤ `0.1748` → IC=+0.287 (n=581)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.816` → IC=+0.294 (n=523)
...
- **PATRÓN** `libro_liquidez` > `1896.567` → IC=+0.295 (n=598)
...
- **FILTRO** `ibs_20min` < `0.2956` → IC=-0.183 (n=513)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.080 (n=1542)

- **FILTRO** `ibs_20min` > `0.7745` → IC=-0.187 (n=615)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7745
  - _Potencial_: sin este filtro IC_bueno=+0.055 (n=1849)

- **PATRÓN** `ibs_20min` > `0.8232` → IC=+0.159 (n=699)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.8232 (IC base=+0.014)
...
- **PATRÓN** `dist_vwap_pct` < `0.3522` → IC=+0.196 (n=448)
  ...
  - _Acción_: Kelly boost +0.98€ cuando `dist_vwap_pct` < 0.3522 (IC base=-0.006)
...
- **FILTRO** `ibs_20min` < `0.7308` → IC=-0.194 (n=1108)
...
- **FILTRO** `ibs_20min` > `0.6857` → IC=-0.233 (n=571)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.096 (n=1714)

- **FILTRO** `sigma_ewma_delta_pct` > `4.71` → IC=-0.186 (n=498)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.71
  - _Potencial_: sin este filtro IC_bueno=+0.070 (n=1787)
...
- **PATRÓN** `volumen_spike_ratio` < `1.8115` → IC=+0.226 (n=366)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.8115 (IC base=+0.014)
...
- **PATRÓN** `ibs_20min` > `0.5738` → IC=+0.218 (n=8114)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5738 (IC base=+0.168)

- **PATRÓN** `dist_vwap_pct` > `0.1779` → IC=+0.194 (n=3532)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 0.1779 (IC base=+0.168)
...
- **PATRÓN** `volumen_regimen` < `1.2094` → IC=+0.160 (n=5371)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2094 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` > `0.63` → IC=+0.161 (n=5371)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 0.63 (IC base=+0.168)

- **PATRÓN** `volumen_pendiente_norm` > `0.2433` → IC=+0.194 (n=1636)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.2433 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` < `1.5584` → IC=+0.167 (n=3426)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.5584 (IC base=+0.168)

- **PATRÓN** `volumen_spike_ratio` > `2.613` → IC=+0.175 (n=2596)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.613 (IC base=+0.168)

- **PATRÓN** `libro_liquidez` > `2408.2501` → IC=+0.169 (n=5408)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2408.2501 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `113.0` → IC=+0.180 (n=7019)

  - _Acción_: Kelly boost +0.90€ cuando `ballena_activa_n` < 113.0 (IC base=+0.168)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.182 (n=5161)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0066 (IC base=+0.167)

- **PATRÓN** `drift_60min` |x|≤ `0.0815` → IC=+0.209 (n=2579)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.168 (n=3632)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.2357` → IC=+0.158 (n=5610)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.2357 (IC base=+0.167)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.323` → IC=+0.192 (n=1309)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 10.323 (IC base=+0.167)

- **PATRÓN** `volumen_regimen` < `1.1725` → IC=+0.151 (n=5599)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1725 (IC base=+0.167)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5584` → IC=+0.165 (n=3102)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_spike_ratio` < 1.5584 (IC base=+0.167)

- **PATRÓN** `volumen_spike_ratio` > `2.2466` → IC=+0.170 (n=3196)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 2.2466 (IC base=+0.167)

- **PATRÓN** `ballena_activa_n` < `115.0` → IC=+0.173 (n=6704)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3426` → IC=+0.206 (n=1378)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.191 (n=921)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 11.0 (IC base=+0.183)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.304 (n=681)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4352` → IC=+0.182 (n=1276)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 1.4352 (IC base=+0.183)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.198 (n=1405)
...
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.234 (n=404)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` > `0.4039` → IC=+0.227 (n=1205)
...
- **PATRÓN** `volumen_regimen` < `0.6876` → IC=+0.165 (n=530)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.6876 (IC base=+0.162)

- **PATRÓN** `volumen_regimen` > `0.8738` → IC=+0.168 (n=803)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 0.8738 (IC base=+0.162)

- **PATRÓN** `volumen_pendiente_norm` > `0.281` → IC=+0.208 (n=197)
...
- **PATRÓN** `libro_liquidez` > `11875.7279` → IC=+0.168 (n=1077)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 11875.7279 (IC base=+0.162)
...
- **PATRÓN** `drift_60min` |x|≤ `0.236` → IC=+0.217 (n=910)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `drift_60min` |x|≤ `0.4324` → IC=+0.158 (n=1296)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `dist_vwap_pct` > `0.157` → IC=+0.177 (n=857)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.157 (IC base=+0.143)
...
- **PATRÓN** `volumen_regimen` < `1.0376` → IC=+0.145 (n=1140)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` < 1.0376 (IC base=+0.143)

- **PATRÓN** `volumen_regimen` > `0.6214` → IC=+0.147 (n=1296)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4265` → IC=+0.152 (n=423)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 1.4265 (IC base=+0.143)

- **PATRÓN** `volumen_spike_ratio` > `2.5013` → IC=+0.166 (n=423)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 2.5013 (IC base=+0.143)

- **PATRÓN** `libro_liquidez` > `6113.3127` → IC=+0.182 (n=864)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 6113.3127 (IC base=+0.143)

- **PATRÓN** `ballena_activa_n` < `162.0` → IC=+0.147 (n=1235)

  - _Acción_: Kelly boost +0.74€ cuando `ballena_activa_n` < 162.0 (IC base=+0.143)

- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.153 (n=1361)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` < 0.0072 (IC base=+0.122)

- **PATRÓN** `drift_60min` |x|≤ `0.3842` → IC=+0.143 (n=1361)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.6462` → IC=+0.171 (n=1361)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.6462 (IC base=+0.122)

- **PATRÓN** `dist_vwap_pct` < `0.3677` → IC=+0.135 (n=1471)
...
- **PATRÓN** `sigma_h` > `0.0101` → IC=+0.153 (n=666)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `volumen_regimen` < `1.2155` → IC=+0.130 (n=1468)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 1.2155 (IC base=+0.117)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5413` → IC=+0.139 (n=624)
...
- **PATRÓN** `libro_liquidez` > `2898.7698` → IC=+0.193 (n=666)

  - _Acción_: Kelly boost +0.97€ cuando `libro_liquidez` > 2898.7698 (IC base=+0.117)

- **PATRÓN** `ballena_activa_n` < `50.0` → IC=+0.134 (n=1139)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1048` → IC=+0.163 (n=496)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `dist_vwap_pct` < `0.2057` → IC=+0.137 (n=1357)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.2057 (IC base=+0.113)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4523` → IC=+0.129 (n=445)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` < 1.4523 (IC base=+0.113)
...
- **PATRÓN** `volumen_regimen` > `0.8579` → IC=+0.222 (n=936)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1485` → IC=+0.210 (n=1194)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1485 (IC base=+0.201)

- **PATRÓN** `volumen_spike_ratio` > `1.4095` → IC=+0.210 (n=1357)
...
- **PATRÓN** `sigma_h` > `0.0089` → IC=+0.175 (n=659)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0089 (IC base=+0.148)

- **PATRÓN** `drift_60min` |x|≤ `0.356` → IC=+0.151 (n=1733)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `dist_vwap_pct` > `0.8833` → IC=+0.179 (n=325)
  ...
  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.8833 (IC base=+0.148)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.703` → IC=+0.174 (n=888)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 3.703 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` < `0.871` → IC=+0.166 (n=1156)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.871 (IC base=+0.148)

- **PATRÓN** `volumen_regimen` > `0.6229` → IC=+0.152 (n=1734)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.6229 (IC base=+0.148)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4413` → IC=+0.163 (n=633)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.153 (n=2233)

  - _Acción_: Kelly boost +0.76€ cuando `libro_spread` < 0.02 (IC base=+0.148)

- **PATRÓN** `libro_liquidez` > `12429.5841` → IC=+0.163 (n=656)

  - _Acción_: Kelly boost +0.81€ cuando `libro_liquidez` > 12429.5841 (IC base=+0.148)

- **PATRÓN** `ballena_activa_n` < `153.0` → IC=+0.168 (n=1746)

  - _Acción_: Kelly boost +0.84€ cuando `ballena_activa_n` < 153.0 (IC base=+0.148)

- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.135 (n=1358)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_20min` < `0.6667` → IC=+0.137 (n=2035)

  - _Acción_: Kelly boost +0.69€ cuando `ibs_20min` < 0.6667 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.2135` → IC=+0.121 (n=1807)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.2135 (IC base=+0.110)

- **PATRÓN** `volumen_regimen` < `0.6989` → IC=+0.130 (n=805)
...
- **PATRÓN** `volumen_regimen` < `0.6865` → IC=+0.140 (n=256)
  ...
  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` < 0.6865 (IC base=+0.091)

- **PATRÓN** `volumen_pendiente_norm` > `0.1664` → IC=+0.133 (n=137)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_pendiente_norm` > 0.1664 (IC base=+0.091)

### GBM_LATE_15M_PYCONFIRMADO#SOL#15min
- **FILTRO** `dist_vwap_pct` > `0.5928` → IC=-0.167 (n=16)

  - _Acción_: SKIP cuando `dist_vwap_pct` > 0.5928
  - _Potencial_: sin este filtro IC_bueno=+0.089 (n=523)

- **PATRÓN** `sigma_h` > `0.0092` → IC=+0.167 (n=208)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` > 0.0092 (IC base=+0.130)

- **PATRÓN** `drift_60min` |x|≤ `0.5425` → IC=+0.132 (n=457)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `dist_vwap_pct` > `1.0202` → IC=+0.223 (n=92)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0202 (IC base=+0.130)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.234` → IC=+0.197 (n=199)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 5.234 (IC base=+0.130)

- **PATRÓN** `volumen_regimen` < `1.0697` → IC=+0.148 (n=402)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 1.0697 (IC base=+0.130)

- **PATRÓN** `volumen_pendiente_norm` > `0.181` → IC=+0.174 (n=130)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_pendiente_norm` > 0.181 (IC base=+0.130)

- **PATRÓN** `volumen_spike_ratio` > `2.1991` → IC=+0.175 (n=198)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` > 2.1991 (IC base=+0.130)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.133 (n=497)

  - _Acción_: Kelly boost +0.67€ cuando `libro_spread` < 0.02 (IC base=+0.130)

- **PATRÓN** `libro_liquidez` > `2944.1737` → IC=+0.181 (n=208)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 2944.1737 (IC base=+0.130)

- **PATRÓN** `ibs_20min` < `0.5172` → IC=+0.151 (n=405)

  - _Acción_: Kelly boost +0.76€ cuando `ibs_20min` < 0.5172 (IC base=+0.080)
...
- **PATRÓN** `libro_liquidez` > `2561.7273` → IC=+0.125 (n=270)

  - _Acción_: Kelly boost +0.62€ cuando `libro_liquidez` > 2561.7273 (IC base=+0.080)

- **PATRÓN** `ballena_activa_n` < `42.0` → IC=+0.133 (n=349)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0114` → IC=+0.205 (n=3464)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.308 (n=3499)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` > `0.9761` → IC=+0.202 (n=1485)
...
- **PATRÓN** `volumen_regimen` < `1.2309` → IC=+0.159 (n=6951)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.2309 (IC base=+0.169)

- **PATRÓN** `volumen_pendiente_norm` > `0.2885` → IC=+0.202 (n=1432)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2885 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `2.5899` → IC=+0.187 (n=3333)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 2.5899 (IC base=+0.169)

- **PATRÓN** `libro_liquidez` > `1794.2984` → IC=+0.171 (n=10390)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 1794.2984 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `85.0` → IC=+0.195 (n=7931)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 85.0 (IC base=+0.169)

- **PATRÓN** `sigma_h` < `0.007` → IC=+0.192 (n=6280)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.007 (IC base=+0.181)

- **PATRÓN** `drift_60min` |x|≤ `0.1498` → IC=+0.189 (n=4141)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.1498 (IC base=+0.181)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.206 (n=3588)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.181)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.183 (n=4343)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` < 7.0 (IC base=+0.181)

- **PATRÓN** `ibs_20min` < `0.5686` → IC=+0.236 (n=9411)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5686 (IC base=+0.181)

- **PATRÓN** `dist_vwap_pct` < `0.2524` → IC=+0.161 (n=5832)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` < 0.2524 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.027` → IC=+0.201 (n=1317)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.027 (IC base=+0.181)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.731` → IC=+0.182 (n=9098)
...
- **PATRÓN** `volumen_regimen` > `1.2017` → IC=+0.155 (n=2152)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` > 1.2017 (IC base=+0.181)

- **PATRÓN** `volumen_pendiente_norm` > `0.2876` → IC=+0.241 (n=1240)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2876 (IC base=+0.181)

- **PATRÓN** `volumen_spike_ratio` > `2.6131` → IC=+0.192 (n=2886)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_spike_ratio` > 2.6131 (IC base=+0.181)

- **PATRÓN** `ballena_activa_n` < `47.0` → IC=+0.194 (n=5523)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3586` → IC=+0.191 (n=1755)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.3586 (IC base=+0.191)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.326 (n=635)
...
- **PATRÓN** `ibs_20min` < `0.3522` → IC=+0.285 (n=1218)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3522 (IC base=+0.257)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.7` → IC=+0.261 (n=1556)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.7 (IC base=+0.257)

- **PATRÓN** `volumen_pendiente_norm` > `0.2808` → IC=+0.287 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2808 (IC base=+0.257)

- **PATRÓN** `volumen_spike_ratio` < `1.549` → IC=+0.255 (n=561)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.549 (IC base=+0.257)

- **PATRÓN** `volumen_spike_ratio` > `2.635` → IC=+0.278 (n=425)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.635 (IC base=+0.257)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.259 (n=1528)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.257)

- **PATRÓN** `libro_liquidez` > `1810.0992` → IC=+0.260 (n=922)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1810.0992 (IC base=+0.257)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.193 (n=558)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0028 (IC base=+0.150)

- **PATRÓN** `drift_60min` |x|≤ `0.085` → IC=+0.160 (n=556)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.3085` → IC=+0.203 (n=1666)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.157 (n=648)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 7.0 (IC base=+0.152)

- **PATRÓN** `ibs_20min` < `0.6603` → IC=+0.195 (n=1444)
...
- **PATRÓN** `dist_vwap_pct` < `0.1372` → IC=+0.165 (n=1310)
...
- **PATRÓN** `volumen_regimen` < `1.1856` → IC=+0.165 (n=1444)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.1856 (IC base=+0.152)

- **PATRÓN** `volumen_pendiente_norm` > `0.1509` → IC=+0.196 (n=386)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` > 0.1509 (IC base=+0.152)

- **PATRÓN** `volumen_spike_ratio` < `2.4003` → IC=+0.162 (n=1345)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 2.4003 (IC base=+0.152)

- **PATRÓN** `volumen_spike_ratio` > `1.7547` → IC=+0.163 (n=897)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.7547 (IC base=+0.152)

- **PATRÓN** `ballena_activa_n` < `354.0` → IC=+0.162 (n=834)

  - _Acción_: Kelly boost +0.81€ cuando `ballena_activa_n` < 354.0 (IC base=+0.152)

### GBM_LATE_15M_TARDIO#DOGE#15min
- **PATRÓN** `sigma_h` > `0.0122` → IC=+0.252 (n=562)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.223 (n=1705)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.811` → IC=+0.291 (n=491)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.811 (IC base=+0.218)

- **PATRÓN** `volumen_pendiente_norm` < `0.2104` → IC=+0.221 (n=1674)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2104 (IC base=+0.218)

- **PATRÓN** `volumen_spike_ratio` > `1.6285` → IC=+0.224 (n=1609)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1707` → IC=+0.235 (n=693)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.240 (n=736)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.748` → IC=+0.272 (n=586)
...
- **PATRÓN** `libro_liquidez` > `1903.24` → IC=+0.236 (n=713)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1903.24 (IC base=+0.230)

- **PATRÓN** `ballena_activa_n` < `54.0` → IC=+0.228 (n=1384)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4373` → IC=+0.146 (n=1770)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` > `0.8757` → IC=+0.261 (n=802)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8757 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.3695` → IC=+0.163 (n=698)

  - _Acción_: Kelly boost +0.81€ cuando `dist_vwap_pct` > 0.3695 (IC base=+0.136)
...
- **PATRÓN** `volumen_regimen` < `0.873` → IC=+0.156 (n=1180)
...
- **PATRÓN** `volumen_spike_ratio` < `1.5194` → IC=+0.151 (n=754)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.5194 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `2.1357` → IC=+0.151 (n=777)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 2.1357 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `8031.121` → IC=+0.233 (n=802)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8031.121 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `78.0` → IC=+0.157 (n=555)

  - _Acción_: Kelly boost +0.79€ cuando `ballena_activa_n` < 78.0 (IC base=+0.136)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.159 (n=1266)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` < 0.0066 (IC base=+0.135)

- **PATRÓN** `drift_60min` |x|≤ `0.4464` → IC=+0.151 (n=1439)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.4464 (IC base=+0.135)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.172 (n=540)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` > 17.0 (IC base=+0.135)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.139 (n=658)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` < 7.0 (IC base=+0.135)

- **PATRÓN** `ibs_20min` < `0.5803` → IC=+0.196 (n=1266)

  - _Acción_: Kelly boost +0.98€ cuando `ibs_20min` < 0.5803 (IC base=+0.135)

- **PATRÓN** `dist_vwap_pct` < `0.2264` → IC=+0.138 (n=1306)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.2264 (IC base=+0.135)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4415` → IC=+0.148 (n=1366)
...
- **PATRÓN** `ballena_activa_n` < `177.0` → IC=+0.141 (n=1365)

  - _Acción_: Kelly boost +0.70€ cuando `ballena_activa_n` < 177.0 (IC base=+0.135)

### GBM_LATE_15M_TARDIO#SOL#15min
- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.142 (n=1173)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.192 (n=1762)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` > 0.4706 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` > `1.1055` → IC=+0.194 (n=374)

  - _Acción_: Kelly boost +0.97€ cuando `dist_vwap_pct` > 1.1055 (IC base=+0.114)
...
- **PATRÓN** `ibs_20min` < `0.6364` → IC=+0.205 (n=1705)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.6364 (IC base=+0.114)

- **PATRÓN** `dist_vwap_pct` < `0.2212` → IC=+0.132 (n=1380)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` < 0.2212 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.486` → IC=+0.127 (n=1646)
...
- **PATRÓN** `ballena_activa_n` < `51.0` → IC=+0.124 (n=1332)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.146 (n=3160)

- **PATRÓN** `sigma_h` < `0.0091` → IC=+0.168 (n=2675)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.84€ cuando `sigma_h` < 0.0091 (IC base=+0.158)

- **PATRÓN** `drift_60min` |x|≤ `0.5186` → IC=+0.167 (n=3040)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.168 (n=1358)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` > `0.1877` → IC=+0.171 (n=1129)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.1877 (IC base=+0.158)

- **PATRÓN** `dist_vwap_pct` < `0.4819` → IC=+0.154 (n=1902)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.4819 (IC base=+0.158)
...
- **PATRÓN** `volumen_regimen` > `0.8944` → IC=+0.165 (n=1354)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8727` → IC=+0.166 (n=2003)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.8727 (IC base=+0.158)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.159 (n=2162)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4873` → IC=+0.161 (n=2383)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.81€ cuando `drift_60min` |x|≤ 0.4873 (IC base=+0.144)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.178 (n=871)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 17.0 (IC base=+0.144)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.164 (n=1050)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` > `0.6792` → IC=+0.170 (n=462)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.6792 (IC base=+0.144)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.238` → IC=+0.155 (n=2372)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` < 6.238 (IC base=+0.144)

- **PATRÓN** `volumen_regimen` < `1.098` → IC=+0.154 (n=2006)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 1.098 (IC base=+0.144)

- **PATRÓN** `volumen_pendiente_norm` < `0.0965` → IC=+0.148 (n=2161)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` < 0.0965 (IC base=+0.144)

- **PATRÓN** `volumen_pendiente_norm` > `0.0721` → IC=+0.146 (n=1097)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_pendiente_norm` > 0.0721 (IC base=+0.144)

- **PATRÓN** `volumen_spike_ratio` < `1.4237` → IC=+0.159 (n=784)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 1.4237 (IC base=+0.144)

- **PATRÓN** `volumen_spike_ratio` > `1.813` → IC=+0.146 (n=1568)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` > 1.813 (IC base=+0.144)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.146 (n=3160)

  - _Acción_: Kelly boost +0.73€ cuando `libro_spread` < 0.01 (IC base=+0.144)

- **PATRÓN** `libro_liquidez` > `7245.2494` → IC=+0.156 (n=2128)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 7245.2494 (IC base=+0.144)

### GBM_LATE_5M#BTC#5min
- **PATRÓN** `sigma_h` < `0.003` → IC=+0.181 (n=136)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.003 (IC base=+0.170)
...
- **PATRÓN** `sigma_h` < `0.0073` → IC=+0.170 (n=893)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` < 0.0073 (IC base=+0.165)

- **PATRÓN** `sigma_h` > `0.0043` → IC=+0.169 (n=1014)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0043 (IC base=+0.165)

- **PATRÓN** `drift_60min` |x|≤ `0.3855` → IC=+0.170 (n=891)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.3855 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.183 (n=393)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.92€ cuando `hora_utc` > 17.0 (IC base=+0.165)

- **PATRÓN** `hora_utc` < `4.0` → IC=+0.172 (n=346)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.86€ cuando `hora_utc` < 4.0 (IC base=+0.165)

- **PATRÓN** `ibs_20min` < `0.545` → IC=+0.176 (n=675)

  - _Acción_: Kelly boost +0.88€ cuando `ibs_20min` < 0.545 (IC base=+0.165)

- **PATRÓN** `ibs_20min` > `0.886` → IC=+0.173 (n=338)

  - _Acción_: Kelly boost +0.87€ cuando `ibs_20min` > 0.886 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` < `0.4146` → IC=+0.176 (n=946)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` < 0.4146 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.174` → IC=+0.177 (n=907)

  - _Acción_: Kelly boost +0.88€ cuando `sigma_ewma_delta_pct` < 4.174 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` < `1.0828` → IC=+0.166 (n=891)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 1.0828 (IC base=+0.165)

- **PATRÓN** `volumen_regimen` > `0.6413` → IC=+0.169 (n=1012)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4338` → IC=+0.174 (n=332)
  ...
  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` < 1.4338 (IC base=+0.165)

- **PATRÓN** `volumen_spike_ratio` > `1.5168` → IC=+0.169 (n=888)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 1.5168 (IC base=+0.165)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3889` → IC=+0.183 (n=733)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.157 (n=587)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.79€ cuando `hora_utc` < 11.0 (IC base=+0.152)

- **PATRÓN** `ibs_20min` < `0.7466` → IC=+0.155 (n=834)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` < 0.7466 (IC base=+0.152)

- **PATRÓN** `ibs_20min` > `0.0993` → IC=+0.160 (n=833)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` > 0.0993 (IC base=+0.152)

- **PATRÓN** `dist_vwap_pct` > `0.1713` → IC=+0.171 (n=384)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.1713 (IC base=+0.152)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.615` → IC=+0.160 (n=856)
...
- **PATRÓN** `volumen_regimen` > `0.7218` → IC=+0.154 (n=744)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 0.7218 (IC base=+0.152)

- **PATRÓN** `volumen_pendiente_norm` < `0.1511` → IC=+0.155 (n=863)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1958` → IC=+0.167 (n=719)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 2.1958 (IC base=+0.152)
...
- **PATRÓN** `libro_liquidez` > `7558.89` → IC=+0.175 (n=833)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 7558.89 (IC base=+0.152)

### GBM_LATE_5M#SOL#5min
- **PATRÓN** `ibs_20min` > `0.5469` → IC=+0.153 (n=249)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` > 0.5469 (IC base=+0.102)

- **PATRÓN** `dist_vwap_pct` > `0.2132` → IC=+0.146 (n=179)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.2132 (IC base=+0.102)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.218` → IC=+0.192 (n=50)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 9.218 (IC base=+0.102)

- **PATRÓN** `volumen_pendiente_norm` > `0.1616` → IC=+0.192 (n=76)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.1616 (IC base=+0.102)

- **PATRÓN** `volumen_spike_ratio` > `1.4296` → IC=+0.130 (n=241)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` > 1.4296 (IC base=+0.102)

- **PATRÓN** `libro_liquidez` > `3434.0223` → IC=+0.149 (n=223)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 3434.0223 (IC base=+0.102)

- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.161 (n=231)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` > 0.0069 (IC base=+0.128)

- **PATRÓN** `drift_60min` |x|≤ `0.4026` → IC=+0.167 (n=154)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.4026 (IC base=+0.128)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.130 (n=117)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` > 14.0 (IC base=+0.128)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.145 (n=167)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` > `0.6223` → IC=+0.224 (n=114)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6223 (IC base=+0.128)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.617` → IC=+0.138 (n=125)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` > 2.617 (IC base=+0.128)

- **PATRÓN** `volumen_regimen` < `1.1643` → IC=+0.139 (n=203)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_regimen` < 1.1643 (IC base=+0.128)

- **PATRÓN** `volumen_pendiente_norm` < `0.085` → IC=+0.196 (n=179)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_pendiente_norm` < 0.085 (IC base=+0.128)

- **PATRÓN** `volumen_spike_ratio` < `1.8554` → IC=+0.151 (n=150)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` < 1.8554 (IC base=+0.128)

- **PATRÓN** `libro_liquidez` > `3331.2707` → IC=+0.161 (n=231)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 3331.2707 (IC base=+0.128)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.170 (n=195)

  - _Acción_: Kelly boost +0.85€ cuando `ballena_activa_n` < 49.0 (IC base=+0.128)
...
- **PATRÓN** `ibs_20min` > `0.6682` → IC=+0.198 (n=772)

  - _Acción_: Kelly boost +0.99€ cuando `ibs_20min` > 0.6682 (IC base=+0.091)

- **PATRÓN** `dist_vwap_pct` > `0.1562` → IC=+0.154 (n=466)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` > 0.1562 (IC base=+0.091)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.443` → IC=+0.189 (n=204)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 11.443 (IC base=+0.091)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0918` → IC=+0.128 (n=660)

  - _Acción_: Kelly boost +0.64€ cuando `volumen_spike_ratio` < 2.0918 (IC base=+0.091)

- **PATRÓN** `libro_liquidez` > `2435.8886` → IC=+0.137 (n=381)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 2435.8886 (IC base=+0.091)
...
- **PATRÓN** `libro_liquidez` > `2925.8106` → IC=+0.168 (n=245)
  ...
  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2925.8106 (IC base=+0.107)
...
- **FILTRO** `liq_imbalance_15min` |x|≤ `1.0` → IC=-0.121 (n=85)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.026 (n=1865)
...
- **PATRÓN** `py_entrada` < `0.495` → IC=+0.139 (n=70)

  - _Acción_: Kelly boost +0.69€ cuando `py_entrada` < 0.495 (IC base=+0.007)

- **PATRÓN** `libro_liquidez` > `3983.3207` → IC=+0.173 (n=53)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 3983.3207 (IC base=+0.007)
...
- **PATRÓN** `py_entrada` < `0.47` → IC=+0.167 (n=208)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=7612)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.167 (n=3622)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.058 (n=11013)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.166 (n=3761)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.035 (n=11379)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.46` → IC=-0.209 (n=623)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.101 (n=1922)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.48` → IC=-0.186 (n=648)

  - _Acción_: SKIP cuando `py_entrada` < 0.48
  - _Potencial_: sin este filtro IC_bueno=+0.105 (n=1947)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.206 (n=657)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.062 (n=2057)

- **FILTRO** `ibs_20min` > `0.2812` → IC=-0.163 (n=678)

  - _Acción_: SKIP cuando `ibs_20min` > 0.2812
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=2036)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.485` → IC=-0.176 (n=616)

  - _Acción_: SKIP cuando `py_entrada` < 0.485
  - _Potencial_: sin este filtro IC_bueno=+0.085 (n=1919)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.177 (n=658)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.050 (n=2062)
...
- **FILTRO** `py_entrada` > `0.59` → IC=-0.205 (n=751)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=-0.012 (n=2261)

- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)

  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=2991)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.130 (n=10251)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=23144)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.273 (n=8201)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=25194)

- **FILTRO** `ibs_7min` < `0.2805` → IC=-0.234 (n=8347)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2805
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=25048)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.156 (n=11332)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.062 (n=22063)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.230 (n=10293)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=31691)

- **FILTRO** `ibs_7min` > `0.2927` → IC=-0.177 (n=10495)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2927
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=31489)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.137 (n=1670)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=3869)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.310 (n=1316)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=4223)

- **FILTRO** `ibs_7min` < `0.7113` → IC=-0.250 (n=1827)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7113
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=3712)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.186 (n=1239)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4300)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.258 (n=1780)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.005 (n=5441)

- **FILTRO** `drift_7min_pct` |x|> `0.1116` → IC=-0.125 (n=2454)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1116
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=4767)

- **FILTRO** `ibs_7min` > `0.7881` → IC=-0.207 (n=1805)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7881
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=5416)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.139 (n=1345)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=4414)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.251 (n=1398)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=4361)

- **FILTRO** `ibs_7min` < `0.7468` → IC=-0.192 (n=1437)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7468
  - _Potencial_: sin este filtro IC_bueno=-0.066 (n=4322)

- **FILTRO** `ballena_activa_n` > `161.0` → IC=-0.177 (n=1433)

  - _Acción_: SKIP cuando `ballena_activa_n` > 161.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4326)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.255 (n=1460)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=4388)

- **FILTRO** `ibs_7min` > `0.2609` → IC=-0.179 (n=1459)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2609
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4389)

- **FILTRO** `ballena_activa_n` > `154.0` → IC=-0.182 (n=1453)

  - _Acción_: SKIP cuando `ballena_activa_n` > 154.0
  - _Potencial_: sin este filtro IC_bueno=-0.056 (n=4395)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.166 (n=1286)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=3980)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.303 (n=1300)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.040 (n=3966)

- **FILTRO** `ibs_7min` < `0.1931` → IC=-0.259 (n=1316)

  - _Acción_: SKIP cuando `ibs_7min` < 0.1931
  - _Potencial_: sin este filtro IC_bueno=-0.054 (n=3950)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.222 (n=1196)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4070)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.237 (n=1770)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=5957)

- **FILTRO** `ibs_7min` > `0.7468` → IC=-0.172 (n=1931)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7468
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=5796)

### MOMENTUM_IBS_5M_BALLENA#ETH#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.129 (n=1767)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.084 (n=3745)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.233 (n=1633)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.042 (n=3879)

- **FILTRO** `ibs_7min` < `0.7398` → IC=-0.183 (n=1378)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7398
  - _Potencial_: sin este filtro IC_bueno=-0.070 (n=4134)

- **FILTRO** `ballena_activa_n` > `32.0` → IC=-0.173 (n=1343)

  - _Acción_: SKIP cuando `ballena_activa_n` > 32.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=4169)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.259 (n=1404)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.031 (n=4218)

- **FILTRO** `ibs_7min` > `0.2748` → IC=-0.177 (n=1405)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2748
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=4217)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.185 (n=1378)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4244)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.256 (n=1434)

  - _Acción_: SKIP cuando `py_entrada` < 0.36
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=4372)

- **FILTRO** `ibs_7min` < `0.3` → IC=-0.234 (n=1448)

  - _Acción_: SKIP cuando `ibs_7min` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=4358)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.179 (n=1896)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=6094)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.254 (n=1803)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=3710)

- **FILTRO** `ibs_7min` < `0.3` → IC=-0.225 (n=1376)

  - _Acción_: SKIP cuando `ibs_7min` < 0.3
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=4137)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.215 (n=1340)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=4173)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.207 (n=1797)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.011 (n=5779)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.324 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=1102)
...
- **PATRÓN** `streak_estiramiento` < `0.4086` → IC=+0.173 (n=47)

  - _Acción_: Kelly boost +0.87€ cuando `streak_estiramiento` < 0.4086 (IC base=+0.040)
...
- **PATRÓN** `volumen_racha` < `990711.2` → IC=+0.167 (n=28)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_racha` < 990711.2 (IC base=+0.009)

- **PATRÓN** `streak_estiramiento` < `0.5243` → IC=+0.157 (n=33)

  - _Acción_: Kelly boost +0.79€ cuando `streak_estiramiento` < 0.5243 (IC base=+0.009)

- **PATRÓN** `ballena_activa_n` < `51.0` → IC=+0.176 (n=35)

  - _Acción_: Kelly boost +0.88€ cuando `ballena_activa_n` < 51.0 (IC base=+0.009)
...
### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` < `0.0044` → IC=+0.186 (n=559)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.93€ cuando `sigma_h` < 0.0044 (IC base=+0.186)

- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.229 (n=559)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0113 (IC base=+0.186)

- **PATRÓN** `drift_60min` |x|≤ `0.1607` → IC=+0.194 (n=1474)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.97€ cuando `drift_60min` |x|≤ 0.1607 (IC base=+0.186)

- **PATRÓN** `delta_ratio_macro` |x|> `0.2158` → IC=+0.189 (n=558)
...
- **PATRÓN** `ibs_15` > `0.6071` → IC=+0.268 (n=1675)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6071 (IC base=+0.186)

- **PATRÓN** `dist_vwap_pct` > `0.1187` → IC=+0.185 (n=850)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.1187 (IC base=+0.186)

- **PATRÓN** `dist_vwap_pct` < `0.6222` → IC=+0.180 (n=1588)

  - _Acción_: Kelly boost +0.90€ cuando `dist_vwap_pct` < 0.6222 (IC base=+0.186)

- **PATRÓN** `sigma_ewma_delta_pct` > `16.822` → IC=+0.273 (n=429)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 16.822 (IC base=+0.186)

- **PATRÓN** `libro_liquidez` > `8780.1789` → IC=+0.201 (n=559)
...
- **PATRÓN** `sigma_h` < `0.0065` → IC=+0.140 (n=390)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.70€ cuando `sigma_h` < 0.0065 (IC base=+0.133)

- **PATRÓN** `sigma_h` > `0.0051` → IC=+0.153 (n=260)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` > 0.0051 (IC base=+0.133)

- **PATRÓN** `drift_60min` |x|≤ `0.0674` → IC=+0.155 (n=172)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.0674 (IC base=+0.133)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2579` → IC=+0.156 (n=280)

  - _Acción_: Kelly boost +0.78€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.2579 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.149 (n=289)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 11.0 (IC base=+0.133)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.137 (n=406)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` < 17.0 (IC base=+0.133)

- **PATRÓN** `ibs_15` > `0.659` → IC=+0.251 (n=348)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.659 (IC base=+0.133)

- **PATRÓN** `dist_vwap_pct` < `0.1604` → IC=+0.150 (n=307)
...
- **PATRÓN** `libro_liquidez` > `9475.4181` → IC=+0.154 (n=177)

  - _Acción_: Kelly boost +0.77€ cuando `libro_liquidez` > 9475.4181 (IC base=+0.133)
...
### UPDOWN_GBM#SOL#5min
- **FILTRO** `dist_vwap_pct` > `0.7186` → IC=-0.139 (n=95)

  - _Acción_: SKIP cuando `dist_vwap_pct` > 0.7186
  - _Potencial_: sin este filtro IC_bueno=+0.045 (n=959)
...
- **PATRÓN** `ibs_15` < `0.1176` → IC=+0.157 (n=496)
...
- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1366` → IC=+0.249 (n=209)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1366 (IC base=-0.065)

- **PATRÓN** `ibs_15` > `0.6423` → IC=+0.274 (n=645)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6423 (IC base=-0.065)

- **PATRÓN** `dist_vwap_pct` < `0.5931` → IC=+0.184 (n=637)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` < 0.5931 (IC base=-0.065)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.132 (n=17)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.163 (n=390)

- **PATRÓN** `sigma_h` < `0.0068` → IC=+0.153 (n=306)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.76€ cuando `sigma_h` < 0.0068 (IC base=+0.150)

- **PATRÓN** `sigma_h` > `0.0051` → IC=+0.180 (n=204)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0051 (IC base=+0.150)

- **PATRÓN** `drift_60min` |x|≤ `0.0756` → IC=+0.215 (n=135)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0756 (IC base=+0.150)

- **PATRÓN** `drift_15min` |x|≤ `0.4178` → IC=+0.164 (n=102)

  - _Acción_: Kelly boost +0.82€ cuando `drift_15min` |x|≤ 0.4178 (IC base=+0.150)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0589` → IC=+0.156 (n=306)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.78€ cuando `delta_ratio_macro` |x|> 0.0589 (IC base=+0.150)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2883` → IC=+0.231 (n=210)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.2883 (IC base=+0.150)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.205 (n=144)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.150)

- **PATRÓN** `ibs_15` > `0.6686` → IC=+0.263 (n=306)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6686 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.4713` → IC=+0.152 (n=87)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 0.4713 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` < `0.1135` → IC=+0.186 (n=218)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` < 0.1135 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` < `9.031` → IC=+0.158 (n=261)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` < 9.031 (IC base=+0.150)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.163 (n=390)

  - _Acción_: Kelly boost +0.82€ cuando `libro_spread` < 0.01 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `11031.3332` → IC=+0.188 (n=139)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 11031.3332 (IC base=+0.150)
...
### UPDOWN_GBM_IBS_ALTO
- **PATRÓN** `sigma_h` < `0.0045` → IC=+0.303 (n=451)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `ibs_15` > `0.8411` → IC=+0.329 (n=675)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.295 (n=824)
...
- **PATRÓN** `sigma_h` < `0.0068` → IC=+0.314 (n=304)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0068 (IC base=+0.298)

- **PATRÓN** `drift_60min` |x|≤ `0.069` → IC=+0.301 (n=134)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.069 (IC base=+0.298)

- **PATRÓN** `delta_ratio_macro` |x|> `0.1481` → IC=+0.310 (n=203)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.1481 (IC base=+0.298)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.2866` → IC=+0.332 (n=230)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.2866 (IC base=+0.298)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.329 (n=272)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 6.0 (IC base=+0.298)

- **PATRÓN** `ibs_15` > `0.8527` → IC=+0.340 (n=304)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6071 sube el IC de +0.186 a +0.268 en UPDOWN_GBM#15min (n=1675). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.659 sube el IC de +0.133 a +0.251 en UPDOWN_GBM#ETH#15min (n=348). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.1176 sube el IC de +0.051 a +0.157 en UPDOWN_GBM#XRP#15min (n=496). Ya aplicado como kelly_boost=+0.78€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.6423 sube el IC de -0.065 a +0.274 en UPDOWN_GBM_15M_TARDIO (n=645). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_YES, IBS > 0.6686 sube el IC de +0.150 a +0.263 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=306). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.8411 sube el IC de +0.294 a +0.329 en UPDOWN_GBM_IBS_ALTO (n=675). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO#ETH#15min**: dentro de BUY_YES, IBS > 0.8527 sube el IC de +0.298 a +0.340 en UPDOWN_GBM_IBS_ALTO#ETH#15min (n=304). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 28894 | -0.087 | -3878.07€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1505 | -0.042 | -223.62€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 27389 | -0.090 | -3654.45€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3799 | -0.099 | -626.41€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3799 | -0.099 | -626.41€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 1505 | -0.042 | -223.62€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 1505 | -0.042 | -223.62€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 3483 | -0.097 | -789.03€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 3483 | -0.097 | -789.03€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 7402 | -0.020 | -697.34€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 7402 | -0.020 | -697.34€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 7022 | -0.090 | -449.91€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 7022 | -0.090 | -449.91€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 5683 | -0.170 | -1091.76€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 5683 | -0.170 | -1091.76€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 18981 | -0.027 | +3977.06€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 4946 | +0.000 | +1817.00€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 14035 | -0.037 | +2160.07€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 18981 | -0.027 | +3977.06€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 