# Hipótesis automáticas — 2026-07-17 06:18 UTC
_Generado por shadow_postmortem.py sobre 18231 resoluciones (PNL=+3925.49€)_

## Patrones causales activos

### FAVORITO_CONFIRMADO
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.191 (n=1052)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.95€ cuando `hora_utc` < 18.0 (IC base=+0.176)

- **PATRÓN** `py_entrada` > `0.695` → IC=+0.296 (n=469)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.695 (IC base=+0.176)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.180 (n=1200)

  - _Acción_: Kelly boost +0.90€ cuando `libro_spread` < 0.01 (IC base=+0.176)

- **PATRÓN** `libro_liquidez` > `4397.2353` → IC=+0.181 (n=685)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 4397.2353 (IC base=+0.176)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.187 (n=1279)
...
- **PATRÓN** `libro_liquidez` > `7188.7301` → IC=+0.222 (n=217)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.224 (n=230)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.203)

- **PATRÓN** `py_entrada` > `0.715` → IC=+0.357 (n=103)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.715 (IC base=+0.203)

- **PATRÓN** `libro_liquidez` > `4604.1148` → IC=+0.215 (n=149)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 4604.1148 (IC base=+0.203)
...
- **PATRÓN** `py_entrada` > `0.62` → IC=+0.285 (n=198)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.62 (IC base=+0.227)

- **PATRÓN** `libro_liquidez` > `1592.1422` → IC=+0.232 (n=222)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1592.1422 (IC base=+0.227)
...
### GBM_LATE_15M
- **PATRÓN** `dist_vwap_pct` > `0.8688` → IC=+0.131 (n=101)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.8688 (IC base=+0.108)
...
- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.132 (n=1312)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.66€ cuando `sigma_h` > 0.0091 (IC base=+0.109)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.132 (n=1431)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.66€ cuando `hora_utc` < 12.0 (IC base=+0.109)

- **PATRÓN** `dist_vwap_pct` < `0.4129` → IC=+0.121 (n=768)

  - _Acción_: Kelly boost +0.60€ cuando `dist_vwap_pct` < 0.4129 (IC base=+0.109)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.447` → IC=+0.185 (n=52)

  - _Acción_: Kelly boost +0.93€ cuando `sigma_ewma_delta_pct` > 7.447 (IC base=+0.069)
...
- **PATRÓN** `sigma_h` < `0.0106` → IC=+0.136 (n=223)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0106 (IC base=+0.108)
...
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.229 (n=370)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.159 (n=837)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `11.022` → IC=+0.309 (n=108)
...
- **PATRÓN** `sigma_h` < `0.0093` → IC=+0.146 (n=77)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.73€ cuando `sigma_h` < 0.0093 (IC base=+0.137)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.150 (n=78)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` < 6.0 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.2378` → IC=+0.186 (n=68)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` > 0.2378 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` < `0.9751` → IC=+0.145 (n=187)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.122 (n=676)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.1434` → IC=+0.137 (n=348)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.134 (n=345)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` < 6.0 (IC base=+0.100)

- **PATRÓN** `dist_vwap_pct` < `0.3731` → IC=+0.131 (n=640)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.3731 (IC base=+0.100)
...
- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.120 (n=77)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.60€ cuando `sigma_h` < 0.0038 (IC base=+0.083)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.127 (n=164)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 7.0 (IC base=+0.083)

- **PATRÓN** `dist_vwap_pct` > `0.3946` → IC=+0.125 (n=22)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` > 0.3946 (IC base=+0.083)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.632` → IC=+0.190 (n=27)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` > 12.632 (IC base=+0.083)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.997` → IC=+0.167 (n=31)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 7.997 (IC base=+0.029)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2862 | +0.177 | -34.34€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 1847 | +0.212 | +32.16€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 829 | +0.186 | +4.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 616 | +0.207 | -6.66€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 1021 | +0.172 | -13.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 611 | +0.206 | +6.62€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 1002 | +0.176 | -23.99€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 610 | +0.222 | +33.64€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION | 65 | +0.321 | +3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#15min | 65 | +0.321 | +3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL | 65 | +0.321 | +3.77€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_SOL_ALTACONVICCION#SOL#15min | 65 | +0.321 | +3.77€ | 0 | 0 |
| ✅ GBM_LATE_15M | 5131 | +0.102 | +1610.10€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 5131 | +0.102 | +1610.10€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1236 | +0.067 | +187.20€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1236 | +0.067 | +187.20€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1212 | +0.075 | +204.65€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1212 | +0.075 | +204.65€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1374 | +0.106 | +545.77€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1374 | +0.106 | +545.77€ | 1 | 7 |
| ✅ GBM_LATE_15M#XRP | 1309 | +0.157 | +672.48€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1309 | +0.157 | +672.48€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2334 | +0.150 | +1347.87€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2334 | +0.150 | +1347.87€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 600 | +0.110 | +288.46€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 600 | +0.110 | +288.46€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 617 | +0.117 | +258.46€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 617 | +0.117 | +258.46€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 618 | +0.118 | +274.14€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 618 | +0.118 | +274.14€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 2316 | +0.096 | +733.91€ | 0 | 9 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2316 | +0.096 | +733.91€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 579 | +0.063 | +106.76€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 579 | +0.063 | +106.76€ | 0 | 6 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 546 | +0.024 | +19.26€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 546 | +0.024 | +19.26€ | 0 | 3 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 224 | +0.186 | +77.57€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 224 | +0.186 | +77.57€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 9 | +0.021 | +3.32€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 9 | +0.021 | +3.32€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 30 | +0.062 | +3.86€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 30 | +0.062 | +3.86€ | 0 | 0 |
...
**🟡 H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread=0.160: overbought→boost, neutral→filtro | oversold(IBS<0.3): IC=+0.021 n=559 | neutral: IC=-0.012 n=404 | overbought(IBS>0.7): IC=+0.148 n=455
  - _Datos_: n=1502 IC=+0.057 PNL=+190.29€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.108 n=10715 | tras_1loss IC=+0.074 n=7315 | tras_2loss IC=+0.043 n=3087/40 | gap=+0.065 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=15773 IC=+0.111 PNL=+4001.37€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=15773 IC=+0.111 PNL=+4001.37€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema, sin filtrar todavía
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.183 < -0.1 con n=427 PNL=-83.40€
  - _Datos_: n=427 IC=-0.183 PNL=-83.40€
...
**🟡 H-CUSTOM-GBMLATE-ANCHURA-MERCADO** — GBM_LATE_15M BUY_YES — anchura de mercado (retorno concurrente de los otros 3 majors) como modificador secundario
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.242 > 0.2 con n=397 PNL=+271.41€
  - _Datos_: n=397 IC=+0.242 PNL=+271.41€
...
**🟡 H-CUSTOM-ETH15-SIGMA-ACCEL** — GBM_LATE_15M ETH — vol acelerando (EWMA10>flat) mejora la señal
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.119 > 0.08 con n=195 PNL=+52.60€
  - _Datos_: n=195 IC=+0.119 PNL=+52.60€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.257 > 0.08 con n=216 PNL=-7.82€
  - _Datos_: n=216 IC=+0.257 PNL=-7.82€
