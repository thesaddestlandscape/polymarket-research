# Hipótesis automáticas — 2026-07-15 21:51 UTC
_Generado por shadow_postmortem.py sobre 15975 resoluciones (PNL=+3237.59€)_
...
- **PATRÓN** `sigma_h` > `0.0231` → IC=+0.192 (n=196)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0231 (IC base=+0.149)
...
- **PATRÓN** `dist_vwap_pct` > `0.6854` → IC=+0.227 (n=31)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6854 (IC base=+0.149)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.281` → IC=+0.192 (n=24)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 9.281 (IC base=+0.149)
...
- **PATRÓN** `sigma_h` > `0.0108` → IC=+0.222 (n=315)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0108 (IC base=+0.162)

- **PATRÓN** `hora_utc` > `13.0` → IC=+0.172 (n=462)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_h` > `0.0101` → IC=+0.258 (n=147)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0148` → IC=+0.184 (n=213)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` > 0.0148 (IC base=+0.111)
...
- **PATRÓN** `dist_vwap_pct` > `0.7123` → IC=+0.184 (n=112)
  ...
  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.7123 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` < `0.1438` → IC=+0.143 (n=211)

  - _Acción_: Kelly boost +0.72€ cuando `dist_vwap_pct` < 0.1438 (IC base=+0.111)
...
- **PATRÓN** `sigma_h` > `0.0113` → IC=+0.224 (n=183)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.1836` → IC=+0.254 (n=59)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1836 (IC base=+0.206)

- **PATRÓN** `dist_vwap_pct` < `1.2749` → IC=+0.206 (n=107)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 1.2749 (IC base=+0.206)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.471` → IC=+0.211 (n=43)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.471 (IC base=+0.206)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.308` → IC=+0.218 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 10.308 (IC base=+0.206)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `17.186` → IC=+0.134 (n=132)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` < 17.186 (IC base=+0.048)
...
| ✅ GBM_LATE_15M | 4698 | +0.103 | +1455.84€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 4698 | +0.103 | +1455.84€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#XRP | 1181 | +0.157 | +597.92€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1181 | +0.157 | +597.92€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 1946 | +0.151 | +1127.46€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 1946 | +0.151 | +1127.46€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 407 | +0.275 | +424.80€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 407 | +0.275 | +424.80€ | 0 | 11 |
...
| ✅ GBM_LATE_15M_TARDIO | 1904 | +0.088 | +562.69€ | 0 | 5 |
| ✅ GBM_LATE_15M_TARDIO#15min | 1904 | +0.088 | +562.69€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 540 | +0.199 | +382.92€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 540 | +0.199 | +382.92€ | 0 | 12 |
...
| ✅ UPDOWN_GBM | 1565 | +0.010 | +72.43€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1307 | +0.037 | +109.47€ | 0 | 5 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 58 | +0.233 | +19.55€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 58 | +0.233 | +19.55€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 8 | +0.080 | +2.13€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 8 | +0.080 | +2.13€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.133) — sin ventaja clara. oversold(IBS<0.3): IC=+0.003 n=453 | neutral: IC=-0.014 n=350 | overbought(IBS>0.7): IC=+0.118 n=370
  - _Datos_: n=1240 IC=+0.039 PNL=+117.61€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9260 | tras_1loss IC=+0.066 n=6504 | tras_2loss IC=+0.032 n=2795/40 | gap=+0.070 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=1438 IC=+0.017 PNL=+86.43€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1438 IC=+0.017 PNL=+86.43€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=578 IC=+0.033 PNL=+39.05€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=578 IC=+0.033 PNL=+39.05€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=13517 IC=+0.105 PNL=+3313.47€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=13517 IC=+0.105 PNL=+3313.47€
