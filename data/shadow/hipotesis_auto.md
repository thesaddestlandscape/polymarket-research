# Hipótesis automáticas — 2026-07-21 03:08 UTC
_Generado por shadow_postmortem.py sobre 25812 resoluciones (PNL=+6368.84€)_
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.221 (n=669)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.177 (n=1955)

  - _Acción_: Kelly boost +0.89€ cuando `libro_spread` < 0.01 (IC base=+0.172)

- **PATRÓN** `libro_liquidez` > `4207.6295` → IC=+0.176 (n=1179)

  - _Acción_: Kelly boost +0.88€ cuando `libro_liquidez` > 4207.6295 (IC base=+0.172)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.277 (n=146)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `19.0` → IC=+0.271 (n=94)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.635` → IC=+0.258 (n=31)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.635 (IC base=+0.218)

- **PATRÓN** `libro_liquidez` > `8909.1861` → IC=+0.242 (n=29)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 8909.1861 (IC base=+0.218)
...
- **PATRÓN** `hora_utc` < `16.0` → IC=+0.218 (n=76)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.655` → IC=+0.177 (n=29)
  ...
  - _Acción_: Kelly boost +0.89€ cuando `py_entrada` > 0.655 (IC base=+0.172)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.191 (n=95)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.01 (IC base=+0.172)

- **PATRÓN** `libro_liquidez` > `3384.7911` → IC=+0.171 (n=86)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 3384.7911 (IC base=+0.172)
...
- **PATRÓN** `py_entrada` < `0.575` → IC=+0.203 (n=35)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.575 (IC base=+0.178)

- **PATRÓN** `py_entrada` > `0.635` → IC=+0.188 (n=46)

  - _Acción_: Kelly boost +0.94€ cuando `py_entrada` > 0.635 (IC base=+0.178)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.178 (n=88)

  - _Acción_: Kelly boost +0.89€ cuando `libro_spread` < 0.02 (IC base=+0.178)
...
- **PATRÓN** `sigma_h` > `0.0133` → IC=+0.139 (n=795)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` > 0.0133 (IC base=+0.111)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.142 (n=808)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.71€ cuando `hora_utc` < 5.0 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` > `0.1542` → IC=+0.132 (n=449)

  - _Acción_: Kelly boost +0.66€ cuando `dist_vwap_pct` > 0.1542 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.138` → IC=+0.133 (n=287)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` > 6.138 (IC base=+0.111)
...
- **PATRÓN** `dist_vwap_pct` < `0.3525` → IC=+0.123 (n=266)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.3525 (IC base=+0.073)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.285` → IC=+0.173 (n=111)

  - _Acción_: Kelly boost +0.86€ cuando `sigma_ewma_delta_pct` > 4.285 (IC base=+0.073)
...
- **PATRÓN** `dist_vwap_pct` > `1.0412` → IC=+0.167 (n=31)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` > 1.0412 (IC base=+0.075)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.497` → IC=+0.196 (n=90)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 7.497 (IC base=+0.075)
...
- **PATRÓN** `sigma_h` < `0.0111` → IC=+0.139 (n=388)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0111 (IC base=+0.117)

- **PATRÓN** `hora_utc` < `12.0` → IC=+0.158 (n=422)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` < `0.0093` → IC=+0.184 (n=213)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0093 (IC base=+0.173)

- **PATRÓN** `sigma_h` > `0.0251` → IC=+0.206 (n=212)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0251 (IC base=+0.173)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `9.238` → IC=+0.169 (n=376)

  - _Acción_: Kelly boost +0.85€ cuando `sigma_ewma_delta_pct` < 9.238 (IC base=+0.173)
...
- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.196 (n=823)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0072 (IC base=+0.165)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.181 (n=628)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 15.0 (IC base=+0.165)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.186 (n=422)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.93€ cuando `hora_utc` < 5.0 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` < `0.6268` → IC=+0.184 (n=1102)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` < 0.6268 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.113` → IC=+0.205 (n=225)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.113 (IC base=+0.165)
...
- **PATRÓN** `sigma_h` < `0.006` → IC=+0.128 (n=318)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.64€ cuando `sigma_h` < 0.006 (IC base=+0.128)

- **PATRÓN** `sigma_h` > `0.0031` → IC=+0.150 (n=284)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` > 0.0031 (IC base=+0.128)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.151 (n=144)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 16.0 (IC base=+0.128)

- **PATRÓN** `dist_vwap_pct` < `0.5188` → IC=+0.134 (n=282)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.5188 (IC base=+0.128)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.43` → IC=+0.196 (n=113)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 4.43 (IC base=+0.128)
...
- **PATRÓN** `sigma_h` < `0.0052` → IC=+0.136 (n=218)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0052 (IC base=+0.115)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.149 (n=112)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 18.0 (IC base=+0.115)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.140 (n=112)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` < 5.0 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` > `0.5578` → IC=+0.136 (n=86)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` > 0.5578 (IC base=+0.115)

- **PATRÓN** `dist_vwap_pct` < `0.9235` → IC=+0.124 (n=301)

  - _Acción_: Kelly boost +0.62€ cuando `dist_vwap_pct` < 0.9235 (IC base=+0.115)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.533` → IC=+0.198 (n=117)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` > 4.533 (IC base=+0.115)
...
- **PATRÓN** `sigma_h` < `0.0121` → IC=+0.144 (n=290)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.72€ cuando `sigma_h` < 0.0121 (IC base=+0.119)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.149 (n=112)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` < 6.0 (IC base=+0.119)

- **PATRÓN** `dist_vwap_pct` < `0.8834` → IC=+0.159 (n=247)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` < 0.8834 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.3` → IC=+0.178 (n=209)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` < 2.3 (IC base=+0.119)
...
- **PATRÓN** `sigma_h` > `0.0076` → IC=+0.328 (n=306)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.302 (n=109)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.297)

- **PATRÓN** `dist_vwap_pct` > `0.1747` → IC=+0.343 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1747 (IC base=+0.297)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.981` → IC=+0.305 (n=255)
...
- **PATRÓN** `dist_vwap_pct` < `0.8071` → IC=+0.127 (n=1161)

  - _Acción_: Kelly boost +0.63€ cuando `dist_vwap_pct` < 0.8071 (IC base=+0.108)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.222` → IC=+0.127 (n=1088)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 11.222 (IC base=+0.108)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.44` → IC=+0.132 (n=112)

  - _Acción_: Kelly boost +0.66€ cuando `sigma_ewma_delta_pct` > 4.44 (IC base=+0.078)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.518` → IC=+0.176 (n=69)
...
- **PATRÓN** `sigma_h` > `0.0204` → IC=+0.239 (n=132)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0204 (IC base=+0.215)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.231 (n=132)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.215)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.246 (n=132)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.215)

- **PATRÓN** `dist_vwap_pct` > `0.4091` → IC=+0.242 (n=64)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4091 (IC base=+0.215)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.662` → IC=+0.219 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.662 (IC base=+0.215)

- **PATRÓN** `sigma_ewma_delta_pct` < `10.122` → IC=+0.226 (n=363)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.7526` → IC=+0.210 (n=332)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.7526 (IC base=+0.043)

- **PATRÓN** `dist_vwap_pct` > `0.7268` → IC=+0.183 (n=58)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.7268 (IC base=+0.043)

- **PATRÓN** `dist_vwap_pct` < `0.4866` → IC=+0.142 (n=241)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` < 0.4866 (IC base=+0.043)
...
- **PATRÓN** `drift_60min` |x|≤ `0.2293` → IC=+0.124 (n=243)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.62€ cuando `drift_60min` |x|≤ 0.2293 (IC base=+0.086)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.149 (n=189)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 12.0 (IC base=+0.086)

- **PATRÓN** `ibs_15` > `0.6354` → IC=+0.204 (n=204)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.6354 (IC base=+0.086)

- **PATRÓN** `dist_vwap_pct` > `0.7736` → IC=+0.237 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7736 (IC base=+0.086)

- **PATRÓN** `sigma_ewma_delta_pct` < `14.017` → IC=+0.155 (n=82)

  - _Acción_: Kelly boost +0.77€ cuando `sigma_ewma_delta_pct` < 14.017 (IC base=+0.086)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS > 0.7526 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.210 n=332). Confirma señal de reversión media → alinear con BUY_NO.
- **H-IBS-UPDOWN_GBM#BTC#15min**: IBS > 0.6354 correlaciona con éxito en UPDOWN_GBM#BTC#15min (IC=+0.204 n=204). Confirma señal de reversión media → alinear con BUY_NO.
...
|---|---|---|---|---|---|
| ✅ BALLENAS_CONFIRMADAS_15M | 529 | +0.131 | +19.56€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#15min | 529 | +0.131 | +19.56€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH | 196 | +0.146 | +5.62€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#ETH#15min | 196 | +0.146 | +5.62€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL | 212 | +0.126 | +0.03€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#SOL#15min | 212 | +0.126 | +0.03€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP | 121 | +0.110 | +13.91€ | 0 | 0 |
| ✅ BALLENAS_CONFIRMADAS_15M#XRP#15min | 121 | +0.110 | +13.91€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 4466 | +0.186 | +62.45€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 2904 | +0.218 | +101.37€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 190 | +0.068 | -5.65€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min |