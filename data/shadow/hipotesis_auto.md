# Hipótesis automáticas — 2026-07-16 09:23 UTC
_Generado por shadow_postmortem.py sobre 16759 resoluciones (PNL=+3432.35€)_
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.186 (n=1056)

  - _Acción_: Kelly boost +0.93€ cuando `libro_spread` < 0.01 (IC base=+0.180)

- **PATRÓN** `libro_liquidez` > `4371.9525` → IC=+0.188 (n=603)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 4371.9525 (IC base=+0.180)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.179 (n=1115)
...
- **PATRÓN** `py_entrada` > `0.615` → IC=+0.258 (n=180)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.227 (n=203)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_h` < `0.0108` → IC=+0.136 (n=212)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0108 (IC base=+0.109)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.141 (n=218)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.70€ cuando `hora_utc` > 16.0 (IC base=+0.109)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.477` → IC=+0.231 (n=50)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.477 (IC base=+0.109)
...
- **PATRÓN** `sigma_h` > `0.0275` → IC=+0.212 (n=158)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.3154` → IC=+0.143 (n=54)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.3154 (IC base=+0.165)

- **PATRÓN** `dist_vwap_pct` < `0.5138` → IC=+0.147 (n=185)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` < 0.5138 (IC base=+0.165)

- **PATRÓN** `sigma_ewma_delta_pct` < `7.696` → IC=+0.143 (n=166)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` < 7.696 (IC base=+0.165)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.179 (n=753)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` < `0.4117` → IC=+0.199 (n=530)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.4117 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.192` → IC=+0.274 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.192 (IC base=+0.164)
...
- **PATRÓN** `sigma_h` < `0.0068` → IC=+0.149 (n=169)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0068 (IC base=+0.138)

- **PATRÓN** `sigma_h` > `0.0037` → IC=+0.155 (n=192)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` > 0.0037 (IC base=+0.138)
...
- **PATRÓN** `dist_vwap_pct` > `0.7087` → IC=+0.202 (n=45)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7087 (IC base=+0.138)

- **PATRÓN** `dist_vwap_pct` < `0.2665` → IC=+0.183 (n=102)

  - _Acción_: Kelly boost +0.91€ cuando `dist_vwap_pct` < 0.2665 (IC base=+0.138)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.012` → IC=+0.236 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.012 (IC base=+0.138)
...
- **PATRÓN** `sigma_h` < `0.0168` → IC=+0.294 (n=168)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0168 (IC base=+0.291)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.300 (n=168)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` < `0.5244` → IC=+0.311 (n=141)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5244 (IC base=+0.291)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.048` → IC=+0.319 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.048 (IC base=+0.291)
...
- **PATRÓN** `dist_vwap_pct` < `0.2015` → IC=+0.122 (n=445)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.2015 (IC base=+0.089)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `4.44` → IC=+0.125 (n=54)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 4.44 (IC base=+0.068)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.997` → IC=+0.136 (n=20)

  - _Acción_: Kelly boost +0.68€ cuando `sigma_ewma_delta_pct` > 7.997 (IC base=+0.021)
...
- **PATRÓN** `sigma_h` > `0.0265` → IC=+0.244 (n=80)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0265 (IC base=+0.197)

- **PATRÓN** `hora_utc` < `6.0` → IC=+0.253 (n=87)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 6.0 (IC base=+0.197)

- **PATRÓN** `dist_vwap_pct` < `0.4878` → IC=+0.186 (n=183)

  - _Acción_: Kelly boost +0.93€ cuando `dist_vwap_pct` < 0.4878 (IC base=+0.197)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2505 | +0.174 | -56.71€ | 0 | 9 |
| ✅ FAVORITO_CONFIRMADO#15min | 1610 | +0.211 | +18.94€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 726 | +0.183 | -8.25€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 539 | +0.208 | -7.22€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 891 | +0.173 | -5.95€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 533 | +0.214 | +15.70€ | 0 | 6 |
...
| ✅ FAVORITO_CONFIRMADO#SOL | 880 | +0.169 | -39.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 530 | +0.212 | +13.83€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 4843 | +0.103 | +1514.21€ | 0 | 5 |
| ✅ GBM_LATE_15M#15min | 4843 | +0.103 | +1514.21€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1161 | +0.063 | +160.76€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1161 | +0.063 | +160.76€ | 0 | 4 |
| ✅ GBM_LATE_15M#ETH | 1160 | +0.077 | +202.75€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1160 | +0.077 | +202.75€ | 1 | 5 |
| ✅ GBM_LATE_15M#SOL | 1296 | +0.109 | +524.14€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 1296 | +0.109 | +524.14€ | 1 | 6 |
| ✅ GBM_LATE_15M#XRP | 1226 | +0.157 | +626.56€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1226 | +0.157 | +626.56€ | 0 | 11 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2070 | +0.150 | +1210.12€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2070 | +0.150 | +1210.12€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 529 | +0.110 | +256.84€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 529 | +0.110 | +256.84€ | 0 | 9 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 548 | +0.129 | +258.45€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 548 | +0.129 | +258.45€ | 0 | 13 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 559 | +0.111 | +245.00€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 559 | +0.111 | +245.00€ | 1 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 434 | +0.273 | +449.83€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 434 | +0.273 | +449.83€ | 0 | 12 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 75 | +0.097 | +24.75€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 75 | +0.097 | +24.75€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 47 | +0.255 | +27.40€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 47 | +0.255 | +27.40€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 2042 | +0.090 | +610.64€ | 0 | 7 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2042 | +0.090 | +610.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 505 | +0.056 | +84.57€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 505 | +0.056 | +84.57€ | 0 | 4 |
| ✅ GBM_LATE_15M_TARDIO#ETH | 472 | +0.023 | +14.13€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#ETH#15min | 472 | +0.023 | +14.13€ | 0 | 3 |
| ✅ GBM_LATE_15M_TARDIO#SOL | 481 | +0.059 | +94.45€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#SOL#15min | 481 | +0.059 | +94.45€ | 2 | 6 |
| ✅ GBM_LATE_15M_TARDIO#XRP | 584 | +0.200 | +417.48€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 584 | +0.200 | +417.48€ | 0 | 10 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.102 n=9755 | tras_1loss IC=+0.069 n=6795 | tras_2loss IC=+0.036 n=2902/40 | gap=+0.066 (umbral 0.05)
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=14301 IC=+0.106 PNL=+3508.24€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=14301 IC=+0.106 PNL=+3508.24€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=619 IC=+0.056 PNL=+69.42€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=619 IC=+0.056 PNL=+69.42€
...
**🟡 H-CUSTOM-XRP15-SIGMA-DECEL** — GBM_LATE_15M XRP — vol DESacelerando (EWMA10<=flat) mejora la señal (signo opuesto a ETH/BTC)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.167 > 0.08 con n=199 PNL=+126.99€
  - _Datos_: n=199 IC=+0.167 PNL=+126.99€
