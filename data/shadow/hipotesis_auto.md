# Hipótesis automáticas — 2026-07-16 17:19 UTC
_Generado por shadow_postmortem.py sobre 17329 resoluciones (PNL=+3518.49€)_
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.197 (n=523)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.181 (n=1185)

  - _Acción_: Kelly boost +0.90€ cuando `libro_spread` < 0.01 (IC base=+0.170)

- **PATRÓN** `libro_liquidez` > `7392.3738` → IC=+0.188 (n=344)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 7392.3738 (IC base=+0.170)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.244 (n=84)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `7163.0167` → IC=+0.211 (n=199)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7163.0167 (IC base=+0.208)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.138 (n=733)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.69€ cuando `hora_utc` < 6.0 (IC base=+0.106)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.004` → IC=+0.202 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.004 (IC base=+0.062)
...
- **PATRÓN** `sigma_h` < `0.0311` → IC=+0.149 (n=457)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0311 (IC base=+0.147)

- **PATRÓN** `sigma_h` > `0.0248` → IC=+0.179 (n=207)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0248 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.157 (n=164)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.602` → IC=+0.235 (n=32)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.602 (IC base=+0.147)

- **PATRÓN** `dist_vwap_pct` < `0.1341` → IC=+0.121 (n=130)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.1341 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.389` → IC=+0.242 (n=29)
...
- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.180 (n=792)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` > 0.0048 (IC base=+0.163)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.171 (n=381)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.4117` → IC=+0.192 (n=583)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` < 0.4117 (IC base=+0.163)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.037` → IC=+0.234 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.037 (IC base=+0.163)
...
- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.165 (n=201)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` > 0.0029 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.174 (n=93)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 15.0 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.136` → IC=+0.143 (n=40)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.136 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `0.466` → IC=+0.131 (n=166)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.466 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.659` → IC=+0.275 (n=38)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.659 (IC base=+0.136)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0153` → IC=+0.196 (n=238)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.0153 (IC base=+0.110)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.126 (n=640)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` < 15.0 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` > `0.3506` → IC=+0.165 (n=177)

  - _Acción_: Kelly boost +0.82€ cuando `dist_vwap_pct` > 0.3506 (IC base=+0.110)

- **PATRÓN** `dist_vwap_pct` < `0.1391` → IC=+0.133 (n=287)

  - _Acción_: Kelly boost +0.67€ cuando `dist_vwap_pct` < 0.1391 (IC base=+0.110)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.007` → IC=+0.293 (n=80)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.007 (IC base=+0.110)
...
- **PATRÓN** `sigma_h` < `0.0339` → IC=+0.209 (n=211)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0339 (IC base=+0.206)

- **PATRÓN** `sigma_h` > `0.0119` → IC=+0.222 (n=210)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `sigma_ewma_delta_pct` > `10.065` → IC=+0.244 (n=37)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 10.065 (IC base=+0.206)
...
|---|---|---|---|---|---|
| ✅ FAVORITO_CONFIRMADO | 2642 | +0.176 | -46.48€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#15min | 1701 | +0.212 | +28.45€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 764 | +0.179 | -14.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 567 | +0.205 | -11.98€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 4955 | +0.101 | +1518.52€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 4955 | +0.101 | +1518.52€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 1190 | +0.063 | +165.62€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 1190 | +0.063 | +165.62€ | 0 | 3 |
...
| ✅ GBM_LATE_15M#XRP | 1257 | +0.155 | +634.87€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 1257 | +0.155 | +634.87€ | 0 | 12 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 2166 | +0.147 | +1237.68€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 2166 | +0.147 | +1237.68€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 556 | +0.106 | +264.27€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 556 | +0.106 | +264.27€ | 0 | 9 |
...
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 454 | +0.274 | +472.37€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 454 | +0.274 | +472.37€ | 0 | 10 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 90 | +0.120 | +37.60€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 90 | +0.120 | +37.60€ | 0 | 0 |
...
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP | 62 | +0.250 | +40.24€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#XRP#15min | 62 | +0.250 | +40.24€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO | 2151 | +0.089 | +636.65€ | 0 | 6 |
| ✅ GBM_LATE_15M_TARDIO#15min | 2151 | +0.089 | +636.65€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC | 534 | +0.054 | +89.06€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#BTC#15min | 534 | +0.054 | +89.06€ | 0 | 3 |
...
| ✅ GBM_LATE_15M_TARDIO#XRP | 615 | +0.202 | +446.14€ | 0 | 0 |
| ✅ GBM_LATE_15M_TARDIO#XRP#15min | 615 | +0.202 | +446.14€ | 0 | 10 |
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.103 n=10086 | tras_1loss IC=+0.069 n=7033 | tras_2loss IC=+0.039 n=3003/40 | gap=+0.064 (umbral 0.05)
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=+0.066 n=769/40 PNL=+144.61€ | H=15h UTC: IC=+0.138 n=772/40 PNL=+274.99€ | H=17h UTC: IC=+0.136 n=688/40 PNL=+207.41€ | H=19h UTC: IC=+0.106 n=620/40 PNL=+127.10€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=14871 IC=+0.105 PNL=+3594.37€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=14871 IC=+0.105 PNL=+3594.37€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=635 IC=+0.056 PNL=+73.07€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=635 IC=+0.056 PNL=+73.07€
