# Hipótesis automáticas — 2026-08-21 07:51 UTC
_Generado por shadow_postmortem.py sobre 98896 resoluciones (PNL=+8469.18€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `155.99` → IC=-0.261 (n=1378)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 155.99
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=4134)
...
- **PATRÓN** `libro_liquidez` > `2443.5113` → IC=+0.307 (n=205)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2443.5113 (IC base=+0.286)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.177 (n=252)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `2112.3487` → IC=+0.169 (n=285)
  ...
  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 2112.3487 (IC base=+0.148)
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.233 (n=410)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.213)

- **PATRÓN** `libro_liquidez` > `737.0716` → IC=+0.238 (n=411)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.162 (n=824)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.229 (n=729)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.227)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.236 (n=638)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.189 (n=804)
...
- **PATRÓN** `hora_utc` < `14.0` → IC=+0.434 (n=150)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `3939.3842` → IC=+0.447 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3939.3842 (IC base=+0.427)
...
- **PATRÓN** `hora_utc` > `4.0` → IC=+0.425 (n=65)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.408)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.400 (n=58)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 11.0 (IC base=+0.408)

- **PATRÓN** `py_entrada` < `0.915` → IC=+0.414 (n=33)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.915 (IC base=+0.408)

- **PATRÓN** `py_entrada` > `0.939` → IC=+0.417 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.939 (IC base=+0.408)

- **PATRÓN** `libro_liquidez` > `2077.5174` → IC=+0.432 (n=57)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2077.5174 (IC base=+0.408)
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.194 (n=4576)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` < 11.0 (IC base=+0.193)

- **PATRÓN** `py_entrada` > `0.72` → IC=+0.225 (n=5014)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.72 (IC base=+0.193)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.130 (n=860)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.65€ cuando `hora_utc` < 11.0 (IC base=+0.111)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.158 (n=909)

  - _Acción_: Kelly boost +0.79€ cuando `py_entrada` > 0.71 (IC base=+0.111)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.251 (n=1083)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` < `11.0` → IC=+0.159 (n=798)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.275 (n=549)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.197 (n=391)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.99€ cuando `hora_utc` < 5.0 (IC base=+0.197)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.239 (n=554)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.75 (IC base=+0.197)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.216 (n=885)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.135)

- **PATRÓN** `restante_min` < `3.84` → IC=+0.167 (n=806)

  - _Acción_: Kelly boost +0.84€ cuando `restante_min` < 3.84 (IC base=+0.135)

- **PATRÓN** `restante_min` > `4.91` → IC=+0.165 (n=852)

  - _Acción_: Kelly boost +0.83€ cuando `restante_min` > 4.91 (IC base=+0.135)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.148 (n=1627)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 11.0 (IC base=+0.135)

- **PATRÓN** `lag_apertura_s` < `5.32` → IC=+0.167 (n=805)

  - _Acción_: Kelly boost +0.83€ cuando `lag_apertura_s` < 5.32 (IC base=+0.135)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#DOGE#5min
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.236 (n=437)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.149)

- **PATRÓN** `restante_min` < `3.74` → IC=+0.188 (n=399)

  - _Acción_: Kelly boost +0.94€ cuando `restante_min` < 3.74 (IC base=+0.149)

- **PATRÓN** `restante_min` > `4.89` → IC=+0.173 (n=402)

  - _Acción_: Kelly boost +0.87€ cuando `restante_min` > 4.89 (IC base=+0.149)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.161 (n=1054)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` < 15.0 (IC base=+0.149)

- **PATRÓN** `lag_apertura_s` < `6.85` → IC=+0.177 (n=398)

  - _Acción_: Kelly boost +0.89€ cuando `lag_apertura_s` < 6.85 (IC base=+0.149)

### FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.196 (n=448)

  - _Acción_: Kelly boost +0.98€ cuando `py_entrada` < 0.37 (IC base=+0.121)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.128 (n=1225)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.418 (n=206)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.415)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.434 (n=196)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.415)

- **PATRÓN** `py_entrada` < `0.935` → IC=+0.422 (n=204)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.935 (IC base=+0.415)

- **PATRÓN** `py_entrada` > `0.915` → IC=+0.431 (n=202)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.915 (IC base=+0.415)

- **PATRÓN** `libro_liquidez` > `1864.6918` → IC=+0.429 (n=194)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1864.6918 (IC base=+0.415)

### FAVORITO_CONFIRMADO_60MIN_EXTREMO#BTC#60min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.410 (n=87)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.413)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.444 (n=87)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.915` → IC=+0.425 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.915 (IC base=+0.413)

- **PATRÓN** `libro_liquidez` > `5399.1782` → IC=+0.451 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 5399.1782 (IC base=+0.413)
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.9743` → IC=+0.266 (n=506)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9743 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` > `0.212` → IC=+0.245 (n=159)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.212 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` < `0.8223` → IC=+0.233 (n=264)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.8223 (IC base=+0.094)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.899` → IC=+0.222 (n=684)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.899 (IC base=+0.094)

- **PATRÓN** `volumen_regimen` < `0.7207` → IC=+0.243 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7207 (IC base=+0.094)

- **PATRÓN** `volumen_regimen` > `1.0991` → IC=+0.244 (n=115)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0991 (IC base=+0.094)

- **PATRÓN** `volumen_pendiente_norm` > `0.3201` → IC=+0.158 (n=118)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_pendiente_norm` > 0.3201 (IC base=+0.094)

- **PATRÓN** `volumen_spike_ratio` > `1.5038` → IC=+0.130 (n=852)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` > 1.5038 (IC base=+0.094)

- **PATRÓN** `ibs_20min` < `0.3979` → IC=+0.136 (n=1707)

  - _Acción_: Kelly boost +0.68€ cuando `ibs_20min` < 0.3979 (IC base=+0.051)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.276 (n=181)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0061 (IC base=+0.237)

- **PATRÓN** `drift_60min` |x|≤ `0.0745` → IC=+0.262 (n=61)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0745 (IC base=+0.237)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.272 (n=182)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.237)

- **PATRÓN** `ibs_20min` > `0.4975` → IC=+0.254 (n=181)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.4975 (IC base=+0.237)

- **PATRÓN** `dist_vwap_pct` > `0.1602` → IC=+0.254 (n=128)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1602 (IC base=+0.237)

- **PATRÓN** `dist_vwap_pct` < `0.6939` → IC=+0.246 (n=171)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6939 (IC base=+0.237)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.779` → IC=+0.328 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.779 (IC base=+0.237)

- **PATRÓN** `volumen_regimen` < `0.6608` → IC=+0.262 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6608 (IC base=+0.237)

- **PATRÓN** `volumen_regimen` > `1.1438` → IC=+0.250 (n=82)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.1438 (IC base=+0.237)

- **PATRÓN** `volumen_pendiente_norm` < `0.105` → IC=+0.258 (n=151)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.105 (IC base=+0.237)

- **PATRÓN** `volumen_pendiente_norm` > `0.2983` → IC=+0.333 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2983 (IC base=+0.237)

- **PATRÓN** `volumen_spike_ratio` < `1.4811` → IC=+0.289 (n=69)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4811 (IC base=+0.237)

- **PATRÓN** `volumen_spike_ratio` > `1.721` → IC=+0.257 (n=105)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.721 (IC base=+0.237)

- **PATRÓN** `libro_liquidez` > `11754.5395` → IC=+0.289 (n=121)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11754.5395 (IC base=+0.237)

- **PATRÓN** `ballena_activa_n` < `414.0` → IC=+0.258 (n=93)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 414.0 (IC base=+0.237)
...
- **FILTRO** `sigma_ewma_delta_pct` > `4.618` → IC=-0.148 (n=214)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.618
  - _Potencial_: sin este filtro IC_bueno=+0.006 (n=747)
...
- **FILTRO** `sigma_ewma_delta_pct` > `7.996` → IC=-0.164 (n=102)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.025 (n=763)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.745` → IC=+0.130 (n=79)

  - _Acción_: Kelly boost +0.65€ cuando `sigma_ewma_delta_pct` > 6.745 (IC base=-0.035)
...
- **PATRÓN** `dist_vwap_pct` < `0.3335` → IC=+0.221 (n=152)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3335 (IC base=+0.003)
...
- **PATRÓN** `sigma_h` > `0.0072` → IC=+0.160 (n=647)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` > 0.0072 (IC base=+0.054)

- **PATRÓN** `ibs_20min` > `0.9461` → IC=+0.252 (n=646)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9461 (IC base=+0.054)

- **PATRÓN** `dist_vwap_pct` > `0.3033` → IC=+0.254 (n=246)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3033 (IC base=+0.054)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.305` → IC=+0.127 (n=1160)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` > 2.305 (IC base=+0.054)

- **PATRÓN** `volumen_regimen` > `1.1729` → IC=+0.165 (n=192)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` > 1.1729 (IC base=+0.054)
...
- **PATRÓN** `volumen_spike_ratio` > `2.7129` → IC=+0.167 (n=373)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 2.7129 (IC base=+0.054)

- **PATRÓN** `ballena_activa_n` < `85.0` → IC=+0.280 (n=266)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 85.0 (IC base=+0.054)

- **PATRÓN** `ibs_20min` < `0.1077` → IC=+0.166 (n=953)

  - _Acción_: Kelly boost +0.83€ cuando `ibs_20min` < 0.1077 (IC base=+0.040)

- **PATRÓN** `dist_vwap_pct` > `0.5961` → IC=+0.209 (n=84)
...
- **PATRÓN** `volumen_regimen` > `0.6271` → IC=+0.188 (n=540)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_regimen` > 0.6271 (IC base=+0.040)

- **PATRÓN** `volumen_pendiente_norm` > `0.2666` → IC=+0.347 (n=83)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2666 (IC base=+0.040)
...
- **PATRÓN** `dist_vwap_pct` > `0.4164` → IC=+0.141 (n=37)

  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 0.4164 (IC base=-0.024)
...
- **FILTRO** `ibs_20min` < `0.4783` → IC=-0.147 (n=185)
...
- **PATRÓN** `ibs_20min` > `0.7778` → IC=+0.240 (n=94)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7778 (IC base=-0.001)

- **PATRÓN** `dist_vwap_pct` > `0.1775` → IC=+0.221 (n=59)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1775 (IC base=-0.001)

- **PATRÓN** `volumen_regimen` > `1.0275` → IC=+0.159 (n=42)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` > 1.0275 (IC base=-0.001)

- **PATRÓN** `volumen_pendiente_norm` > `0.231` → IC=+0.214 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.231 (IC base=-0.001)

- **PATRÓN** `volumen_spike_ratio` > `2.0894` → IC=+0.141 (n=37)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 2.0894 (IC base=-0.001)

- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.197 (n=31)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 62.0 (IC base=-0.001)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.325 (n=169)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0071 (IC base=+0.179)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.206 (n=124)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.179)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.218 (n=179)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.179)

- **PATRÓN** `ibs_20min` > `0.9048` → IC=+0.256 (n=248)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9048 (IC base=+0.179)

- **PATRÓN** `dist_vwap_pct` > `0.4386` → IC=+0.302 (n=109)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4386 (IC base=+0.179)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.118` → IC=+0.249 (n=237)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.118 (IC base=+0.179)

- **PATRÓN** `volumen_regimen` > `0.8308` → IC=+0.212 (n=248)
...
- **PATRÓN** `volumen_spike_ratio` < `2.0117` → IC=+0.195 (n=211)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 2.0117 (IC base=+0.179)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.193 (n=389)

  - _Acción_: Kelly boost +0.97€ cuando `libro_spread` < 0.01 (IC base=+0.179)

- **PATRÓN** `libro_liquidez` > `2680.8058` → IC=+0.208 (n=248)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2680.8058 (IC base=+0.179)

- **PATRÓN** `ballena_activa_n` < `46.0` → IC=+0.354 (n=94)
...
- **PATRÓN** `sigma_h` > `0.0052` → IC=+0.263 (n=339)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0052 (IC base=+0.261)

- **PATRÓN** `drift_60min` |x|≤ `0.3668` → IC=+0.269 (n=379)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `dist_vwap_pct` > `0.478` → IC=+0.287 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.478 (IC base=+0.261)

- **PATRÓN** `dist_vwap_pct` < `0.1268` → IC=+0.262 (n=346)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1268 (IC base=+0.261)
...
- **PATRÓN** `volumen_regimen` > `1.2602` → IC=+0.306 (n=127)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2602 (IC base=+0.261)
...
- **PATRÓN** `volumen_spike_ratio` > `2.9156` → IC=+0.269 (n=76)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.9156 (IC base=+0.261)

- **PATRÓN** `ballena_activa_n` < `48.0` → IC=+0.209 (n=125)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 48.0 (IC base=+0.261)

### GBM_LATE_15M_MULTIHORIZONTE
- **PATRÓN** `sigma_h` > `0.0071` → IC=+0.206 (n=747)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.157 (n=747)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` < 7.0 (IC base=+0.154)

- **PATRÓN** `ibs_20min` > `0.908` → IC=+0.258 (n=1091)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.908 (IC base=+0.154)

- **PATRÓN** `dist_vwap_pct` > `0.7203` → IC=+0.242 (n=289)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7203 (IC base=+0.154)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.276` → IC=+0.289 (n=533)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.276 (IC base=+0.154)

- **PATRÓN** `volumen_regimen` > `0.7019` → IC=+0.167 (n=882)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_regimen` > 0.7019 (IC base=+0.154)

- **PATRÓN** `volumen_pendiente_norm` > `0.1061` → IC=+0.179 (n=572)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1061 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` < `2.9578` → IC=+0.140 (n=1394)

  - _Acción_: Kelly boost +0.70€ cuando `volumen_spike_ratio` < 2.9578 (IC base=+0.154)

- **PATRÓN** `volumen_spike_ratio` > `1.4943` → IC=+0.143 (n=1394)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.4943 (IC base=+0.154)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.166 (n=1681)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.04 (IC base=+0.154)

- **PATRÓN** `libro_liquidez` > `3206.2208` → IC=+0.206 (n=545)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 3206.2208 (IC base=+0.154)
...
- **PATRÓN** `sigma_h` < `0.0026` → IC=+0.250 (n=82)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0026 (IC base=+0.206)

- **PATRÓN** `sigma_h` > `0.0033` → IC=+0.212 (n=123)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0033 (IC base=+0.206)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.251 (n=187)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.206)

- **PATRÓN** `ibs_20min` > `0.5283` → IC=+0.249 (n=185)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.5283 (IC base=+0.206)

- **PATRÓN** `dist_vwap_pct` > `0.2163` → IC=+0.275 (n=136)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2163 (IC base=+0.206)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.376` → IC=+0.306 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.376 (IC base=+0.206)

- **PATRÓN** `volumen_regimen` > `0.9281` → IC=+0.252 (n=123)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.9281 (IC base=+0.206)

- **PATRÓN** `volumen_pendiente_norm` > `0.2983` → IC=+0.321 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2983 (IC base=+0.206)

- **PATRÓN** `volumen_spike_ratio` < `1.4452` → IC=+0.278 (n=52)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8245` → IC=+0.217 (n=104)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.8245 (IC base=+0.206)

- **PATRÓN** `libro_liquidez` > `13337.4554` → IC=+0.297 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 13337.4554 (IC base=+0.206)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0849` → IC=+0.314 (n=57)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0849 (IC base=+0.234)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.250 (n=174)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.234)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.244 (n=162)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 15.0 (IC base=+0.234)

- **PATRÓN** `ibs_20min` > `0.481` → IC=+0.292 (n=171)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.481 (IC base=+0.234)

- **PATRÓN** `dist_vwap_pct` > `0.1656` → IC=+0.262 (n=124)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1656 (IC base=+0.234)

- **PATRÓN** `dist_vwap_pct` < `0.7151` → IC=+0.237 (n=173)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.7151 (IC base=+0.234)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.902` → IC=+0.402 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.902 (IC base=+0.234)

- **PATRÓN** `volumen_regimen` > `0.9074` → IC=+0.284 (n=114)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.9074 (IC base=+0.234)

- **PATRÓN** `volumen_pendiente_norm` > `0.1047` → IC=+0.291 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.1047 (IC base=+0.234)

- **PATRÓN** `volumen_spike_ratio` < `1.4788` → IC=+0.250 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4788 (IC base=+0.234)

- **PATRÓN** `volumen_spike_ratio` > `2.2882` → IC=+0.263 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2882 (IC base=+0.234)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.233 (n=189)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.234)

- **PATRÓN** `ballena_activa_n` < `219.0` → IC=+0.219 (n=94)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 219.0 (IC base=+0.234)
...
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.203 (n=136)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.007 (IC base=+0.087)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.127 (n=156)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 15.0 (IC base=+0.087)

- **PATRÓN** `ibs_20min` > `0.8621` → IC=+0.211 (n=199)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.8621 (IC base=+0.087)

- **PATRÓN** `dist_vwap_pct` > `0.6447` → IC=+0.247 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6447 (IC base=+0.087)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.464` → IC=+0.240 (n=94)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.464 (IC base=+0.087)

- **PATRÓN** `volumen_pendiente_norm` > `0.0973` → IC=+0.124 (n=107)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_pendiente_norm` > 0.0973 (IC base=+0.087)

- **PATRÓN** `libro_liquidez` > `2664.3504` → IC=+0.152 (n=136)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 2664.3504 (IC base=+0.087)

- **PATRÓN** `ballena_activa_n` < `36.0` → IC=+0.196 (n=44)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 36.0 (IC base=+0.087)
...
- **PATRÓN** `sigma_h` > `0.0064` → IC=+0.237 (n=222)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0064 (IC base=+0.150)

- **PATRÓN** `drift_60min` |x|≤ `0.3036` → IC=+0.155 (n=294)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.3036 (IC base=+0.150)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.178 (n=116)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` > 17.0 (IC base=+0.150)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.162 (n=152)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` < 7.0 (IC base=+0.150)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.239 (n=209)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 1.0 (IC base=+0.150)

- **PATRÓN** `dist_vwap_pct` > `0.2855` → IC=+0.213 (n=179)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2855 (IC base=+0.150)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.38` → IC=+0.232 (n=207)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.38 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` < `1.2221` → IC=+0.158 (n=334)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 1.2221 (IC base=+0.150)

- **PATRÓN** `volumen_regimen` > `0.8361` → IC=+0.153 (n=223)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` > 0.8361 (IC base=+0.150)

- **PATRÓN** `volumen_pendiente_norm` > `0.2537` → IC=+0.250 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2537 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` < `2.7128` → IC=+0.167 (n=298)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.7128 (IC base=+0.150)

- **PATRÓN** `volumen_spike_ratio` > `1.4643` → IC=+0.152 (n=297)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_spike_ratio` > 1.4643 (IC base=+0.150)

- **PATRÓN** `libro_liquidez` > `2460.3759` → IC=+0.160 (n=333)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 2460.3759 (IC base=+0.150)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0073` → IC=+0.222 (n=623)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0073 (IC base=+0.136)
...
- **PATRÓN** `dist_vwap_pct` > `0.6468` → IC=+0.237 (n=287)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.6468 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.049` → IC=+0.262 (n=856)
...
- **PATRÓN** `volumen_regimen` > `1.0941` → IC=+0.137 (n=519)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` > 1.0941 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.1732` → IC=+0.163 (n=455)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` > 0.1732 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` < `2.9487` → IC=+0.131 (n=1558)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_spike_ratio` < 2.9487 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.6453` → IC=+0.141 (n=1391)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` > 1.6453 (IC base=+0.136)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.146 (n=1880)

  - _Acción_: Kelly boost +0.73€ cuando `libro_spread` < 0.04 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `3108.1723` → IC=+0.193 (n=623)

  - _Acción_: Kelly boost +0.96€ cuando `libro_liquidez` > 3108.1723 (IC base=+0.136)

- **PATRÓN** `sigma_h` < `0.0075` → IC=+0.216 (n=1702)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0075 (IC base=+0.193)

- **PATRÓN** `drift_60min` |x|≤ `0.331` → IC=+0.203 (n=1702)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.331 (IC base=+0.193)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.213 (n=789)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 15.0 (IC base=+0.193)

- **PATRÓN** `ibs_20min` < `0.5364` → IC=+0.258 (n=1702)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5364 (IC base=+0.193)

- **PATRÓN** `dist_vwap_pct` < `0.676` → IC=+0.178 (n=1354)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.676 (IC base=+0.193)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.101` → IC=+0.226 (n=326)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.101 (IC base=+0.193)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.513` → IC=+0.197 (n=1601)
...
- **PATRÓN** `volumen_regimen` > `1.2037` → IC=+0.180 (n=407)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` > 1.2037 (IC base=+0.193)

- **PATRÓN** `volumen_pendiente_norm` > `0.2396` → IC=+0.244 (n=221)
...
- **PATRÓN** `sigma_h` > `0.0032` → IC=+0.192 (n=157)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` > 0.0032 (IC base=+0.160)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.191 (n=241)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` > 5.0 (IC base=+0.160)

- **PATRÓN** `ibs_20min` > `0.3308` → IC=+0.243 (n=235)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3308 (IC base=+0.160)

- **PATRÓN** `dist_vwap_pct` > `0.1871` → IC=+0.257 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1871 (IC base=+0.160)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.184` → IC=+0.272 (n=77)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.184 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` < `0.6819` → IC=+0.191 (n=79)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_regimen` < 0.6819 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` > `0.9222` → IC=+0.179 (n=157)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_regimen` > 0.9222 (IC base=+0.160)

- **PATRÓN** `volumen_pendiente_norm` < `0.1013` → IC=+0.202 (n=176)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1013 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` < `2.7165` → IC=+0.226 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.7165 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` > `1.3899` → IC=+0.200 (n=188)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.3899 (IC base=+0.160)

- **PATRÓN** `libro_liquidez` > `11800.2375` → IC=+0.271 (n=107)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11800.2375 (IC base=+0.160)

- **PATRÓN** `ballena_activa_n` < `534.0` → IC=+0.226 (n=111)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 534.0 (IC base=+0.160)

- **PATRÓN** `sigma_h` < `0.0021` → IC=+0.204 (n=133)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0021 (IC base=+0.163)

- **PATRÓN** `drift_60min` |x|≤ `0.2896` → IC=+0.174 (n=302)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.87€ cuando `drift_60min` |x|≤ 0.2896 (IC base=+0.163)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.174 (n=286)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.87€ cuando `hora_utc` > 7.0 (IC base=+0.163)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.170 (n=310)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 18.0 (IC base=+0.163)

- **PATRÓN** `ibs_20min` < `0.4957` → IC=+0.210 (n=302)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4957 (IC base=+0.163)

- **PATRÓN** `dist_vwap_pct` < `0.2613` → IC=+0.188 (n=306)

  - _Acción_: Kelly boost +0.94€ cuando `dist_vwap_pct` < 0.2613 (IC base=+0.163)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.552` → IC=+0.249 (n=173)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.552 (IC base=+0.163)

- **PATRÓN** `volumen_regimen` < `0.6183` → IC=+0.209 (n=101)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6183 (IC base=+0.163)

- **PATRÓN** `volumen_regimen` > `0.8408` → IC=+0.190 (n=201)
...
- **PATRÓN** `volumen_spike_ratio` < `1.7532` → IC=+0.221 (n=138)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7532 (IC base=+0.163)

- **PATRÓN** `libro_liquidez` > `12123.9122` → IC=+0.189 (n=101)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 12123.9122 (IC base=+0.163)
...
- **PATRÓN** `sigma_h` < `0.0024` → IC=+0.253 (n=83)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0024 (IC base=+0.124)

- **PATRÓN** `drift_60min` |x|≤ `0.3625` → IC=+0.149 (n=189)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.3625 (IC base=+0.124)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.127 (n=73)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 17.0 (IC base=+0.124)

- **PATRÓN** `hora_utc` < `11.0` → IC=+0.147 (n=131)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.73€ cuando `hora_utc` < 11.0 (IC base=+0.124)

- **PATRÓN** `ibs_20min` < `0.2508` → IC=+0.234 (n=126)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2508 (IC base=+0.124)

- **PATRÓN** `dist_vwap_pct` < `0.5736` → IC=+0.153 (n=200)

  - _Acción_: Kelly boost +0.77€ cuando `dist_vwap_pct` < 0.5736 (IC base=+0.124)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.945` → IC=+0.256 (n=39)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.945 (IC base=+0.124)

- **PATRÓN** `volumen_regimen` < `0.5375` → IC=+0.177 (n=63)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` < 0.5375 (IC base=+0.124)

- **PATRÓN** `volumen_pendiente_norm` < `0.0641` → IC=+0.169 (n=131)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` < 0.0641 (IC base=+0.124)

- **PATRÓN** `volumen_pendiente_norm` > `0.2214` → IC=+0.250 (n=18)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2214 (IC base=+0.124)

- **PATRÓN** `volumen_spike_ratio` < `2.4048` → IC=+0.172 (n=135)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 2.4048 (IC base=+0.124)

- **PATRÓN** `volumen_spike_ratio` > `1.4416` → IC=+0.193 (n=135)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` > 1.4416 (IC base=+0.124)

- **PATRÓN** `libro_liquidez` > `9831.5669` → IC=+0.131 (n=63)

  - _Acción_: Kelly boost +0.65€ cuando `libro_liquidez` > 9831.5669 (IC base=+0.124)
...
- **PATRÓN** `sigma_h` > `0.0062` → IC=+0.209 (n=259)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0062 (IC base=+0.151)

- **PATRÓN** `drift_60min` |x|≤ `0.0851` → IC=+0.197 (n=130)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.98€ cuando `drift_60min` |x|≤ 0.0851 (IC base=+0.151)
...
- **PATRÓN** `dist_vwap_pct` > `0.27` → IC=+0.243 (n=169)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.27 (IC base=+0.151)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.842` → IC=+0.241 (n=222)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.842 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` < `0.5999` → IC=+0.174 (n=130)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` < 0.5999 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` > `1.0445` → IC=+0.157 (n=176)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` > 1.0445 (IC base=+0.151)

- **PATRÓN** `volumen_pendiente_norm` > `0.2518` → IC=+0.246 (n=61)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2518 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` < `2.9818` → IC=+0.143 (n=340)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` < 2.9818 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` > `1.5011` → IC=+0.160 (n=339)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 1.5011 (IC base=+0.151)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.163 (n=407)

  - _Acción_: Kelly boost +0.81€ cuando `libro_spread` < 0.01 (IC base=+0.151)

- **PATRÓN** `libro_liquidez` > `2455.4649` → IC=+0.156 (n=388)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 2455.4649 (IC base=+0.151)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.221 (n=102)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 35.0 (IC base=+0.151)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3675` → IC=+0.228 (n=436)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.3675 (IC base=+0.213)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.270 (n=220)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 14.0 (IC base=+0.213)

- **PATRÓN** `ibs_20min` < `0.4645` → IC=+0.285 (n=436)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.839` → IC=+0.250 (n=54)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.839 (IC base=+0.213)

- **PATRÓN** `sigma_ewma_delta_pct` < `2.839` → IC=+0.224 (n=444)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.839 (IC base=+0.213)

- **PATRÓN** `volumen_regimen` > `0.8813` → IC=+0.243 (n=290)
...
- **PATRÓN** `volumen_spike_ratio` > `2.8857` → IC=+0.242 (n=87)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8857 (IC base=+0.213)
...
- **PATRÓN** `dist_vwap_pct` > `0.2134` → IC=+0.146 (n=77)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.2134 (IC base=+0.047)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.125 (n=54)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` < 18.0 (IC base=+0.111)

- **PATRÓN** `py_entrada` > `0.505` → IC=+0.167 (n=43)

  - _Acción_: Kelly boost +0.83€ cuando `py_entrada` > 0.505 (IC base=+0.111)
...
- **PATRÓN** `ibs_20min` > `0.9979` → IC=+0.136 (n=53)

  - _Acción_: Kelly boost +0.68€ cuando `ibs_20min` > 0.9979 (IC base=+0.038)

### MOMENTUM_IBS_15M#SOL#15min
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.127 (n=57)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 7.0 (IC base=+0.081)

- **PATRÓN** `drift_20min_pct` |x|≤ `0.0896` → IC=+0.152 (n=44)

  - _Acción_: Kelly boost +0.76€ cuando `drift_20min_pct` |x|≤ 0.0896 (IC base=+0.081)

### MOMENTUM_IBS_15M_BALLENA
- **FILTRO** `py_entrada` < `0.44` → IC=-0.179 (n=294)

  - _Acción_: SKIP cuando `py_entrada` < 0.44
  - _Potencial_: sin este filtro IC_bueno=+0.067 (n=923)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.233 (n=320)

  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=968)

- **FILTRO** `ibs_20min` > `0.2762` → IC=-0.221 (n=320)
...
- **FILTRO** `ibs_20min` < `0.7263` → IC=-0.196 (n=44)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7263
  - _Potencial_: sin este filtro IC_bueno=+0.077 (n=135)

- **FILTRO** `ibs_20min` > `0.7327` → IC=-0.209 (n=53)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7327
  - _Potencial_: sin este filtro IC_bueno=+0.033 (n=163)

- **PATRÓN** `py_entrada` > `0.505` → IC=+0.131 (n=63)

  - _Acción_: Kelly boost +0.65€ cuando `py_entrada` > 0.505 (IC base=+0.008)

- **PATRÓN** `ibs_20min` > `0.8286` → IC=+0.141 (n=90)

  - _Acción_: Kelly boost +0.71€ cuando `ibs_20min` > 0.8286 (IC base=+0.008)

### MOMENTUM_IBS_15M_BALLENA#BTC#15min
- **FILTRO** `py_entrada` > `0.56` → IC=-0.342 (n=55)
...
- **FILTRO** `ballena_activa_n` > `43.0` → IC=-0.224 (n=74)
...
- **PATRÓN** `ibs_20min` > `0.9522` → IC=+0.141 (n=62)

  - _Acción_: Kelly boost +0.70€ cuando `ibs_20min` > 0.9522 (IC base=-0.019)
...
- **FILTRO** `libro_liquidez` < `9946.6004` → IC=-0.154 (n=76)

  - _Acción_: SKIP cuando `libro_liquidez` < 9946.6004
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=155)

### MOMENTUM_IBS_15M_BALLENA#SOL#15min
- **FILTRO** `py_entrada` > `0.57` → IC=-0.269 (n=50)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=152)

- **FILTRO** `drift_20min_pct` |x|> `0.2499` → IC=-0.192 (n=50)

  - _Acción_: SKIP cuando `drift_20min_pct` |x|> 0.2499
  - _Potencial_: sin este filtro IC_bueno=-0.078 (n=152)

- **FILTRO** `ballena_activa_n` > `2.0` → IC=-0.216 (n=79)

  - _Acción_: SKIP cuando `ballena_activa_n` > 2.0
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=123)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.45` → IC=-0.236 (n=51)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.304 (n=49)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.007 (n=148)

- **FILTRO** `drift_20min_pct` |x|> `0.3942` → IC=-0.186 (n=49)
...
- **PATRÓN** `py_entrada` > `0.505` → IC=+0.132 (n=85)

  - _Acción_: Kelly boost +0.66€ cuando `py_entrada` > 0.505 (IC base=+0.019)

- **PATRÓN** `libro_liquidez` > `2588.4382` → IC=+0.148 (n=52)

  - _Acción_: Kelly boost +0.74€ cuando `libro_liquidez` > 2588.4382 (IC base=+0.019)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.175 (n=660)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=2406)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.276 (n=742)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.006 (n=2324)

- **FILTRO** `ibs_7min` < `0.7323` → IC=-0.216 (n=766)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7323
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=2300)

- **FILTRO** `ballena_activa_n` > `18.0` → IC=-0.179 (n=1029)

  - _Acción_: SKIP cuando `ballena_activa_n` > 18.0
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=2037)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.218 (n=892)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=-0.017 (n=2851)
...
- **FILTRO** `ballena_activa_n` > `2.0` → IC=-0.120 (n=377)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.197 (n=130)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
...
- **FILTRO** `ibs_7min` < `0.2857` → IC=-0.250 (n=134)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2857
  - _Potencial_: sin este filtro IC_bueno=-0.025 (n=274)

- **FILTRO** `ballena_activa_n` > `20.0` → IC=-0.258 (n=97)

  - _Acción_: SKIP cuando `ballena_activa_n` > 20.0
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=311)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.250 (n=142)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.010 (n=496)

- **FILTRO** `ibs_7min` > `0.85` → IC=-0.158 (n=159)
...
- **FILTRO** `ibs_7min` < `0.7333` → IC=-0.289 (n=112)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.154 (n=24)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.016 (n=512)
...
- **PATRÓN** `ibs_15` < `0.1858` → IC=+0.125 (n=126)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.62€ cuando `ibs_15` < 0.1858 (IC base=+0.056)
...
- **FILTRO** `ibs_15` < `0.455` → IC=-0.289 (n=93)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: SKIP cuando `ibs_15` < 0.455
  - _Potencial_: sin este filtro IC_bueno=+0.179 (n=191)

- **FILTRO** `sigma_h` > `0.0082` → IC=-0.179 (n=416)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0082
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=1249)
...
- **FILTRO** `sigma_ewma_delta_pct` > `13.352` → IC=-0.194 (n=305)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 13.352
  - _Potencial_: sin este filtro IC_bueno=-0.065 (n=1360)

- **PATRÓN** `ibs_15` > `0.455` → IC=+0.179 (n=191)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.89€ cuando `ibs_15` > 0.455 (IC base=-0.033)
...
- **FILTRO** `sigma_h` > `0.0056` → IC=-0.247 (n=93)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.188 (n=280)

- **FILTRO** `drift_15min` |x|> `0.7351` → IC=-0.205 (n=93)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 0.7351
  - _Potencial_: sin este filtro IC_bueno=-0.202 (n=280)
...
- **FILTRO** `sigma_ewma_delta_pct` > `20.971` → IC=-0.232 (n=80)
...
- **FILTRO** `sigma_h` > `0.0075` → IC=-0.152 (n=199)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: SKIP cuando `sigma_h` > 0.0075
  - _Potencial_: sin este filtro IC_bueno=-0.009 (n=389)

- **FILTRO** `sigma_ewma_delta_pct` > `14.276` → IC=-0.197 (n=74)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 14.276
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=514)
...
- **FILTRO** `sigma_h` > `0.0079` → IC=-0.183 (n=184)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_YES, IBS > 0.455 sube el IC de -0.033 a +0.179 en UPDOWN_GBM_15M_TARDIO (n=191). Ya aplicado como kelly_boost=+0.89€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#ETH#60min` — IC=+0.167 n=37. Faltan ~3 resoluciones para umbral n≥40. ETA: ~2h.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO_60_240MIN_DEPTH_FASE0#60min` — IC=+0.167 n=37. Faltan ~3 resoluciones para umbral n≥40. ETA: ~2h.
...
| ✅ BALLENAS_TARDIAS | 4557 | -0.103 | -616.79€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 538 | -0.067 | -78.82€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 4019 | -0.108 | -537.97€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 544 | -0.137 | -81.97€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 544 | -0.137 | -81.97€ | 2 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 538 | -0.067 | -78.82€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 538 | -0.067 | -78.82€ | 2 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 356 | -0.137 | -154.64€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 356 | -0.137 | -154.64€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 725 | -0.137 | -38.80€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 725 | -0.137 | -38.80€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 1301 | -0.003 | -57.65€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 1301 | -0.003 | -57.65€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1093 | -0.188 | -204.91€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1093 | -0.188 | -204.91€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 19600 | +0.114 | -1351.09€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 4111 | +0.185 | -150.73€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 106 | -0.102 | -48.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 12608 | +0.089 | -1153.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2775 | +0.129 | +1.80€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 2125 | +0.045 | -444.48€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 16 | -0.089 | -1.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 2104 | +0.048 | -436.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 4255 | +0.139 | -57.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1137 | +0.194 | -65.19€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 2096 | +0.112 | -42.76€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 980 | +0.141 | +72.40€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 2129 | +0.065 | -345.46€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 15 | +0.022 | -2.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 2113 | +0.066 | -340.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 4591 | +0.128 | -72.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1515 | +0.170 | -7.71€ | 0 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 2097 | +0.107 | -48.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min |
06€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 140 | +0.099 | +27.23€ | 0 | 0 |
| 🚫 UPDOWN_GBM#BNB#5min | 9 | -0.102 | -2.69€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 688 | +0.026 | +44.78€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 161 | +0.064 | +11.27€ | 4 | 13 |
| ✅ UPDOWN_GBM#BTC#240min | 47 | +0.071 | +4.62€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#5min | 363 | +0.023 | +27.25€ | 1 | 0 |
| ✅ UPDOWN_GBM#BTC#60min | 99 | -0.015 | -0.19€ | 2 | 1 |
| ✅ UPDOWN_GBM#BTC#daily | 18 | -0.135 | +1.83€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 411 | +0.013 | +7.35€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 85 | +0.098 | +19.89€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 325 | -0.011 | -13.04€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 788 | +0.029 | +39.09€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 354 | +0.070 | +40.64€ | 1 | 8 |
| ✅ UPDOWN_GBM#ETH#240min | 47 | +0.051 | +1.04€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 251 | -0.022 | -9.53€ | 5 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 121 | +0.037 | +7.33€ | 1 | 1 |
| 🚫 UPDOWN_GBM#ETH#daily | 15 | -0.154 | -0.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 746 | -0.017 | -18.41€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 305 | -0.011 | -9.86€ | 1 | 2 |
| ✅ UPDOWN_GBM#SOL#240min | 37 | -0.038 | -3.14€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 317 | -0.011 | -4.70€ | 2 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 75 | -0.019 | -0.59€ | 1 | 1 |
| 🚫 UPDOWN_GBM#SOL#daily | 12 | -0.129 | -0.13€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 706 | -0.006 | +36.28€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 409 | +0.052 | +70.35€ | 0 | 8 |
| ✅ UPDOWN_GBM#XRP#240min | 21 | -0.152 | -3.95€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 2333 | -0.073 | +438.34€ | 4 | 3 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 2333 | -0.073 | +438.34€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB | 186 | -0.080 | +100.96€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BNB#15min | 186 | -0.080 | +100.96€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC | 470 | -0.150 | -26.47€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#BTC#15min | 470 | -0.150 | -26.47€ | 4 | 1 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE | 57 | +0.009 | +2.95€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#DOGE#15min | 57 | +0.009 | +2.95€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH | 153 | +0.029 | +34.20€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#ETH#15min | 153 | +0.029 | +34.20€ | 1 | 12 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 748 | -0.053 | +201.48€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 748 | -0.053 | +201.48€ | 3 | 1 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 719 | -0.069 | +125.22€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 719 | -0.069 | +125.22€ | 5 | 0 |
| ✅ UPDOWN_GBM_ETH_15M_HORA7 | 14 | -0.044 | -1.67€ | 0 | 0 |
| ✅ UPDOWN_GBM_ETH_15M_HORA7#15min | 14 | -0.044 | -1.67€ | 0 | 0 |
| ✅ UPDOWN_GBM_ETH_15M_HORA7#ETH | 14 | -0.044 | -1.67€ | 0 | 0 |
| ✅ UPDOWN_GBM_ETH_15M_HORA7#ETH#15min | 14 | -0.044 | -1.67€ | 0 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.099) — sin ventaja clara. oversold(IBS<0.3): IC=-0.005 n=1187 | neutral: IC=+0.005 n=1215 | overbought(IBS>0.7): IC=+0.093 n=1478
  - _Datos_: n=4123 IC=+0.035 PNL=+318.55€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.089 n=56405 | tras_1loss IC=+0.049 n=42177 | tras_2loss IC=+0.011 n=18953/40 | gap=+0.079 (umbral 0.05)
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=+0.005 n=307 | contrario_BTC IC=+0.002 n=235/40 | gap=-0.003 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=3256 IC=+0.005 PNL=+78.36€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=3256 IC=+0.005 PNL=+78.36€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=899 IC=+0.044 PNL=+65.59€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=899 IC=+0.044 PNL=+65.59€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=670 IC=-0.051 PNL=-52.14€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=670 IC=-0.051 PNL=-52.14€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=892 IC=+0.029 PNL=+72.24€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=892 IC=+0.029 PNL=+72.24€
...
**🟡 H-CUSTOM-CROSS-WINDOW-SPREAD-NEG** — GBM 15min/60min: spread negativo alto de precio_yes contra la ventana relacionada
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.109 > 0.08 con n=90 PNL=+22.04€
  - _Datos_: n=90 IC=+0.109 PNL=+22.04€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=1125 IC=+0.030 PNL=+63.84€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=1125 IC=+0.030 PNL=+63.84€
...
**🟡 H-CUSTOM-LATE-ENTRY-15MIN** — Entrada tardía en ventanas 15min (T_h<0.2) — el edge vive al final de la ventana
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.154 > 0.1 con n=637 PNL=+215.85€
  - _Datos_: n=637 IC=+0.154 PNL=+215.85€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=2178 IC=-0.121 PNL=+289.13€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=2178 IC=-0.121 PNL=+289.13€
...
**⏳ H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: 161/289 ops en el filtro definido (IC actual=-0.212 PNL=+1.99€)
  - _Datos_: n=161 IC=-0.212 PNL=+1.99€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.247 > 0.08 con n=868 PNL=-86.49€
  - _Datos_: n=868 IC=+0.247 PNL=-86.49€
...
**🔶 H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.111 n=70) pero sin cruzar ≥2 pares más — sin otros pares con datos
  - _Datos_: n=70 IC=+0.111 PNL=+13.97€
...
**🟡 H-CUSTOM-ETH15-BUYNO-TARDIO** — UPDOWN_GBM ETH#15min BUY_NO tardío (T_h<0.2) -- edge fuerte no capturado por el aprendizaje causal automático
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.357 > 0.08 con n=54 PNL=+45.05€
  - _Datos_: n=54 IC=+0.357 PNL=+45.05€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=1686 IC=+0.111 PNL=-357.56€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=1686 IC=+0.111 PNL=-357.56€
