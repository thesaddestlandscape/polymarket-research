# Hipótesis automáticas — 2026-07-03 21:17 UTC
_Generado por shadow_postmortem.py sobre 3217 resoluciones (PNL=-24.65€)_
...
### UPDOWN_GBM#15min
- **FILTRO** `ibs_15` > `0.3451` → IC=-0.138 (n=45)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: SKIP cuando `ibs_15` > 0.3451
  - _Potencial_: sin este filtro IC_bueno=+0.047 (n=137)

- **PATRÓN** `sigma_h` < `0.0097` → IC=+0.194 (n=83)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.97€ cuando `sigma_h` < 0.0097 (IC base=+0.034)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.128 (n=49)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 15.0 (IC base=+0.034)

- **PATRÓN** `ibs_15` < `0.0642` → IC=+0.151 (n=61)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.75€ cuando `ibs_15` < 0.0642 (IC base=+0.034)
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.1297` → IC=+0.123 (n=67)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.62€ cuando `pct_spot_vs_ref` |x|≤ 0.1297 (IC base=+0.093)

- **PATRÓN** `sigma_h` < `0.009` → IC=+0.181 (n=45)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.90€ cuando `sigma_h` < 0.009 (IC base=+0.093)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.192 (n=24)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 7.0 (IC base=+0.093)

- **PATRÓN** `ibs_15` < `0.0949` → IC=+0.210 (n=36)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0949 (IC base=+0.093)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS < 0.0642 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.151 n=61). Confirma señal de reversión media → alinear con BUY_YES.
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.0949 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.210 n=36). Confirma señal de reversión media → alinear con BUY_YES.

## Estado de aprendizaje por estrategia

| Estrategia | n | IC | PNL | Filtros | Patrones |
|---|---|---|---|---|---|
| ✅ GBM_LATE_15M | 515 | +0.063 | +73.62€ | 0 | 0 |
| ✅ GBM_LATE_15M#15min | 515 | +0.063 | +73.62€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 136 | +0.036 | +8.18€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 136 | +0.036 | +8.18€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 134 | +0.007 | +2.92€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 134 | +0.007 | +2.92€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL | 128 | +0.100 | +31.42€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 128 | +0.100 | +31.42€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP | 117 | +0.113 | +31.10€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 117 | +0.113 | +31.10€ | 0 | 0 |
| 🚫 GBM_LATE_60M | 8 | -0.120 | -3.18€ | 0 | 0 |
| 🚫 GBM_LATE_60M#60min | 8 | -0.120 | -3.18€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN | 6 | +0.000 | -0.27€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN#5min | 6 | +0.000 | -0.27€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN#BTC | 6 | +0.000 | -0.27€ | 0 | 0 |
| ✅ LATE_WINDOW_5MIN#BTC#5min | 6 | +0.000 | -0.27€ | 0 | 0 |
...
| ✅ UPDOWN_GBM | 874 | -0.042 | -52.48€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 643 | -0.021 | -24.13€ | 1 | 3 |
...
| ✅ UPDOWN_GBM#ETH | 350 | -0.014 | +5.10€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 265 | +0.002 | +12.08€ | 3 | 4 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.066) — sin ventaja clara. oversold(IBS<0.3): IC=-0.009 n=169 | neutral: IC=-0.074 n=146 | overbought(IBS>0.7): IC=-0.052 n=152
  - _Datos_: n=484 IC=-0.043 PNL=-27.25€

**🟡 H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  - _Umbral_: n≥20 forward con hora_utc + alguna hora con n≥15 IC<-0.10 o >+0.10
  - _Acción_: El sistema lo aplica automáticamente vía FEATURE_RULES. Verificar en strategy_params.json.
  - _Estado_: H=01h: IC=-0.200 n=28 PNL=-13.74€ → FILTRAR | H=02h: IC=-0.180 n=23 PNL=-6.09€ → FILTRAR | H=03h: IC=-0.110 n=15 PNL=-5.87€ → FILTRAR | H=10h: IC=-0.154 n=15 PNL=-6.77€ → FILTRAR | H=16h: IC=+0.167 n=25 PNL=+7.36€ → BOOST | H=20h: IC=-0.155 n=27 PNL=-7.55€ → FILTRAR | H=23h: IC=+0.158 n=19 PNL=+11.81€ → BOOST
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.012 n=1547 | tras_1loss IC=-0.026 n=1568 | tras_2loss IC=-0.044 n=805/40 | gap=+0.056 (umbral 0.05)
...
**⏳ H-KELLY-HORA** — Kelly boost ×1.2 en horas top (15/17/19h UTC)
  ...
  - _Estado_: H=13h UTC: IC=-0.052 n=132/40 PNL=-4.05€ | H=15h UTC: IC=-0.004 n=135/40 PNL=-0.84€ | H=17h UTC: IC=+0.153 n=73/40 PNL=+21.60€ | H=19h UTC: IC=-0.005 n=89/40 PNL=-7.46€
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=751 IC=-0.038 PNL=-38.93€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=751 IC=-0.038 PNL=-38.93€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=277 IC=-0.023 PNL=-35.76€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=277 IC=-0.023 PNL=-35.76€
...
**⏳ H-LATE-WINDOW-5MIN** — Late-window BTC 5min — arbitraje timing vs Polymarket
  ...
  - _Estado_: 6/30 ops en el filtro definido (IC actual=+0.000 PNL=-0.27€)
  - _Datos_: n=6 IC=+0.000 PNL=-0.27€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=231 IC=+0.011 PNL=+12.29€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=231 IC=+0.011 PNL=+12.29€
...
**〰️ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: n=784 IC=+0.034 PNL=+60.91€ — sin señal clara aún (umbral IC: min=None max=-0.03)
  - _Datos_: n=784 IC=+0.034 PNL=+60.91€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=59 IC=-0.090 PNL=-0.51€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=59 IC=-0.090 PNL=-0.51€
