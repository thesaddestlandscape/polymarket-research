# Hipótesis automáticas — 2026-07-01 22:50 UTC
_Generado por shadow_postmortem.py sobre 2476 resoluciones (PNL=-87.74€)_
...
- **FILTRO** `ibs_15` > `0.2802` → IC=-0.143 (n=26)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: SKIP cuando `ibs_15` > 0.2802
  - _Potencial_: sin este filtro IC_bueno=+0.136 (n=53)

- **PATRÓN** `sigma_h` < `0.009` → IC=+0.265 (n=49)
...
- **PATRÓN** `hora_utc` > `16.0` → IC=+0.125 (n=22)
  ...
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 16.0 (IC base=+0.081)

- **PATRÓN** `ibs_15` < `0.0512` → IC=+0.224 (n=27)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0512 (IC base=+0.081)
...
- **FILTRO** `pct_spot_vs_ref` |x|> `0.017` → IC=-0.167 (n=16)
  - _Por qué funciona_: precio spot lejos de la referencia → señal GBM sobreextiende; riesgo de reversión
  - _Acción_: SKIP cuando `pct_spot_vs_ref` |x|> 0.017
  - _Potencial_: sin este filtro IC_bueno=+0.136 (n=9)

- **FILTRO** `drift_15min` |x|> `0.5828` → IC=-0.132 (n=17)
  - _Por qué funciona_: drift fuerte en 15min → momentum reciente ya en el precio Polymarket
  - _Acción_: SKIP cuando `drift_15min` |x|> 0.5828
...
- **PATRÓN** `pct_spot_vs_ref` |x|≤ `0.0943` → IC=+0.149 (n=35)
  - _Por qué funciona_: precio spot cerca de la referencia → señal GBM más calibrada
  - _Acción_: Kelly boost +0.74€ cuando `pct_spot_vs_ref` |x|≤ 0.0943 (IC base=+0.130)

- **PATRÓN** `sigma_h` < `0.0089` → IC=+0.190 (n=27)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.0089 (IC base=+0.130)

- **PATRÓN** `drift_60min` |x|≤ `1.067` → IC=+0.176 (n=32)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 1.067 (IC base=+0.130)

- **PATRÓN** `ibs_15` < `0.0792` → IC=+0.400 (n=18)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.0792 (IC base=+0.130)
...
- **H-IBS-UPDOWN_GBM#15min**: IBS < 0.0512 correlaciona con éxito en UPDOWN_GBM#15min (IC=+0.224 n=27). Confirma señal de reversión media → alinear con BUY_YES.
- **H-IBS-UPDOWN_GBM#ETH#15min**: IBS < 0.0792 correlaciona con éxito en UPDOWN_GBM#ETH#15min (IC=+0.400 n=18). Confirma señal de reversión media → alinear con BUY_YES.
...
| ✅ UPDOWN_GBM | 728 | -0.038 | -36.62€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 511 | -0.013 | -10.21€ | 2 | 3 |
| 🚫 UPDOWN_GBM#240min | 11 | -0.148 | -4.31€ | 0 | 0 |
| 🚫 UPDOWN_GBM#5min | 75 | -0.162 | -22.48€ | 4 | 0 |
| ✅ UPDOWN_GBM#60min | 114 | -0.060 | -10.38€ | 6 | 0 |
...
| ✅ UPDOWN_GBM#ETH | 297 | -0.012 | +2.95€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 217 | +0.007 | +9.39€ | 5 | 4 |
| ✅ UPDOWN_GBM#ETH#240min | 5 | -0.018 | -0.44€ | 0 | 0 |
| 🚫 UPDOWN_GBM#ETH#5min | 19 | -0.204 | -7.68€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 50 | -0.019 | -2.54€ | 3 | 0 |
| ✅ UPDOWN_GBM#ETH#daily | 6 | +0.037 | +4.21€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 122 | -0.089 | -9.50€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 70 | -0.069 | -5.25€ | 4 | 0 |
| 🚫 UPDOWN_GBM#SOL#5min | 23 | -0.060 | -3.42€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 23 | -0.140 | -2.19€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 55 | -0.009 | +0.94€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 44 | +0.043 | +4.23€ | 1 | 0 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.120) — sin ventaja clara. oversold(IBS<0.3): IC=+0.014 n=103 | neutral: IC=-0.106 n=97 | overbought(IBS>0.7): IC=-0.027 n=129
  - _Datos_: n=338 IC=-0.035 PNL=-11.38€
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Estado_: SOL#15min: n≥40 pero IC=-0.069 < 0.08 — monitorear
  - _Datos_: n=70 IC=-0.069 PNL=-5.25€
...
**⏳ H-CROSS-ASSET** — Cross-asset confirmation GBM+OF BUY_NO
  ...
  - _Estado_: n_overlaps=20, boost estimado=+0.006. Necesita 0 más y boost>0.05
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=50/40 IC=-0.019 PNL=-2.54€ | BTC#60min: n=41/40 IC=-0.058 PNL=-5.65€ | SOL#60min: n=23/40 IC=-0.140 PNL=-2.19€
...
**⏳ H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=-0.002 n=1164 | tras_1loss IC=-0.035 n=1230 | tras_2loss IC=-0.050 n=640/40 | gap=+0.048 (umbral 0.05)
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=605 IC=-0.032 PNL=-23.06€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=605 IC=-0.032 PNL=-23.06€
...
**⏳ H-CUSTOM-GBM-60MIN-BUYNO** — GBM 60min BUY_NO — tracking por separado
  ...
  - _Estado_: 25/30 ops en el filtro definido (IC actual=-0.056 PNL=-2.78€)
  - _Datos_: n=25 IC=-0.056 PNL=-2.78€
...
**〰️ H-CUSTOM-DRIFT15-MOMENTUM** — GBM#15min drift_15min > 0.3 — zona de momentum (señal fuerte)
  ...
  - _Estado_: n=218 IC=+0.018 PNL=-4.62€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=218 IC=+0.018 PNL=-4.62€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=128 IC=+0.046 PNL=+17.65€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=128 IC=+0.046 PNL=+17.65€
...
**⏳ H-CUSTOM-MOON-LLENA** — Fase lunar: ¿rendimiento peor cerca de luna llena?
  ...
  - _Estado_: 59/200 ops en el filtro definido (IC actual=+0.041 PNL=+5.19€)
  - _Datos_: n=59 IC=+0.041 PNL=+5.19€
...
**⏳ H-CUSTOM-MERCURY-RETROGRADO** — Mercurio retrógrado: ¿rendimiento peor durante la ventana?
  ...
  - _Estado_: 59/100 ops en el filtro definido (IC actual=+0.041 PNL=+5.19€)
  - _Datos_: n=59 IC=+0.041 PNL=+5.19€
...
**⏳ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: 3/40 ops en el filtro definido (IC actual=-0.015 PNL=-0.59€)
  - _Datos_: n=3 IC=-0.015 PNL=-0.59€
