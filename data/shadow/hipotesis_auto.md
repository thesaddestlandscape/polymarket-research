# Hipótesis automáticas — 2026-07-28 07:13 UTC
_Generado por shadow_postmortem.py sobre 39716 resoluciones (PNL=+8064.47€)_
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.185 (n=1193)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.173 (n=2900)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.186 (n=1217)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.688` → IC=+0.197 (n=226)

  - _Acción_: Kelly boost +0.99€ cuando `dist_vwap_pct` > 0.688 (IC base=+0.140)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.424` → IC=+0.233 (n=578)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.424 (IC base=+0.140)
...
- **PATRÓN** `sigma_h` < `0.0047` → IC=+0.128 (n=399)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `dist_vwap_pct` > `0.4457` → IC=+0.217 (n=58)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4457 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.697` → IC=+0.179 (n=82)

  - _Acción_: Kelly boost +0.89€ cuando `sigma_ewma_delta_pct` > 11.697 (IC base=+0.111)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.246` → IC=+0.183 (n=121)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` > 7.246 (IC base=+0.119)
...
- **PATRÓN** `sigma_h` > `0.0078` → IC=+0.140 (n=459)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.312` → IC=+0.156 (n=126)
  ...
  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` > 0.312 (IC base=+0.111)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.574` → IC=+0.265 (n=198)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.574 (IC base=+0.111)

### GBM_LATE_15M_ESPACIO_ATR#XRP#15min
- **PATRÓN** `sigma_h` < `0.0176` → IC=+0.248 (n=422)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `dist_vwap_pct` > `0.4523` → IC=+0.293 (n=80)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4523 (IC base=+0.227)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.585` → IC=+0.350 (n=118)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0104` → IC=+0.125 (n=811)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` > 0.0104 (IC base=+0.094)

- **PATRÓN** `dist_vwap_pct` > `0.4709` → IC=+0.158 (n=328)

  - _Acción_: Kelly boost +0.79€ cuando `dist_vwap_pct` > 0.4709 (IC base=+0.094)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.126` → IC=+0.233 (n=286)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.126 (IC base=+0.094)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `dist_vwap_pct` > `0.4428` → IC=+0.178 (n=57)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` > 0.4428 (IC base=+0.075)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.655` → IC=+0.196 (n=67)

  - _Acción_: Kelly boost +0.98€ cuando `sigma_ewma_delta_pct` > 12.655 (IC base=+0.075)
...
- **FILTRO** `sigma_ewma_delta_pct` > `7.86` → IC=-0.152 (n=21)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.069 (n=494)
...
- **PATRÓN** `sigma_h` < `0.0092` → IC=+0.189 (n=178)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.94€ cuando `sigma_h` < 0.0092 (IC base=+0.170)

- **PATRÓN** `hora_utc` > `6.0` → IC=+0.176 (n=535)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` > 6.0 (IC base=+0.170)

- **PATRÓN** `hora_utc` < `15.0` → IC=+0.170 (n=474)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` < 15.0 (IC base=+0.170)

- **PATRÓN** `dist_vwap_pct` > `0.2416` → IC=+0.230 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2416 (IC base=+0.170)
...
### UPDOWN_GBM#15min
- **PATRÓN** `ibs_15` > `0.6079` → IC=+0.176 (n=640)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.88€ cuando `ibs_15` > 0.6079 (IC base=+0.057)

- **PATRÓN** `dist_vwap_pct` > `0.1672` → IC=+0.170 (n=234)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.1672 (IC base=+0.057)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.107` → IC=+0.152 (n=231)

  - _Acción_: Kelly boost +0.76€ cuando `sigma_ewma_delta_pct` > 5.107 (IC base=+0.057)
...
- **PATRÓN** `sigma_h` < `0.0071` → IC=+0.123 (n=322)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.62€ cuando `sigma_h` < 0.0071 (IC base=+0.101)

- **PATRÓN** `drift_60min` |x|≤ `0.2204` → IC=+0.138 (n=313)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.69€ cuando `drift_60min` |x|≤ 0.2204 (IC base=+0.101)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0644` → IC=+0.125 (n=313)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.63€ cuando `delta_ratio_macro` |x|> 0.0644 (IC base=+0.101)

- **PATRÓN** `hora_utc` < `14.0` → IC=+0.165 (n=183)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` < 14.0 (IC base=+0.101)

- **PATRÓN** `ibs_15` > `0.724` → IC=+0.240 (n=244)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.724 (IC base=+0.101)

- **PATRÓN** `dist_vwap_pct` > `0.2571` → IC=+0.239 (n=90)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.2571 (IC base=+0.101)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.121` → IC=+0.173 (n=96)

  - _Acción_: Kelly boost +0.87€ cuando `sigma_ewma_delta_pct` > 8.121 (IC base=+0.101)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.127 (n=156)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 12.0 (IC base=+0.077)

- **PATRÓN** `ibs_15` < `0.0357` → IC=+0.153 (n=70)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +0.76€ cuando `ibs_15` < 0.0357 (IC base=+0.077)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.6079 sube el IC de +0.057 a +0.176 en UPDOWN_GBM#15min (n=640). Ya aplicado como kelly_boost=+0.88€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **H-IBS-UPDOWN_GBM#BTC#15min**: dentro de BUY_YES, IBS > 0.724 sube el IC de +0.101 a +0.240 en UPDOWN_GBM#BTC#15min (n=244). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#XRP#15min**: dentro de BUY_NO, IBS < 0.0357 sube el IC de +0.077 a +0.153 en UPDOWN_GBM#XRP#15min (n=70). Ya aplicado como kelly_boost=+0.76€ automático (shadow) — no es señal de reversión a la dirección contraria.
- **LIVE-CANDIDATA**: `FAVORITO_CONFIRMADO#XRP` — IC=+0.149 n=35. Faltan ~5 resoluciones para umbral n≥40. ETA: ~4h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M_TARDIO#DOGE#15min` — IC=+0.129 n=33. Faltan ~7 resoluciones para umbral n≥40. ETA: ~5h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M_TARDIO#DOGE` — IC=+0.129 n=33. Faltan ~7 resoluciones para umbral n≥40. ETA: ~5h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M_ESPACIO_ATR#DOGE#15min` — IC=+0.122 n=35. Faltan ~5 resoluciones para umbral n≥40. ETA: ~4h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M_ESPACIO_ATR#DOGE` — IC=+0.122 n=35. Faltan ~5 resoluciones para umbral n≥40. ETA: ~4h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M#BNB#15min` — IC=+0.139 n=34. Faltan ~6 resoluciones para umbral n≥40. ETA: ~4h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M#BNB` — IC=+0.139 n=34. Faltan ~6 resoluciones para umbral n≥40. ETA: ~4h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M_TARDIO#BNB#15min` — IC=+0.186 n=33. Faltan ~7 resoluciones para umbral n≥40. ETA: ~5h.
- **LIVE-CANDIDATA**: `GBM_LATE_15M_TARDIO#BNB` — IC=+0.186 n=33. Faltan ~7 resoluciones para umbral n≥40. ETA: ~5h.
...
| ✅ FAVORITO_CONFIRMADO | 6879 | +0.177 | -58.68€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 4535 | +0.201 | -48.24€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO#BTC | 2033 | +0.182 | -14.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1514 | +0.197 | -53.81€ | 0 | 7 |
...
| ✅ FAVORITO_CONFIRMADO#ETH | 2414 | +0.167 | -48.67€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1489 | +0.199 | -4.39€ | 0 | 5 |
...
| ✅ GBM_LATE_15M | 8535 | +0.093 | +2534.06€ | 0 | 4 |
| ✅ GBM_LATE_15M#15min | 8535 | +0.093 | +2534.06€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB | 34 | +0.139 | +5.50€ | 0 | 0 |
| ✅ GBM_LATE_15M#BNB#15min | 34 | +0.139 | +5.50€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC | 2050 | +0.072 | +352.15€ | 0 | 0 |
| ✅ GBM_LATE_15M#BTC#15min | 2050 | +0.072 | +352.15€ | 0 | 5 |
| ✅ GBM_LATE_15M#DOGE | 33 | -0.014 | -0.57€ | 0 | 0 |
| ✅ GBM_LATE_15M#DOGE#15min | 33 | -0.014 | -0.57€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH | 1849 | +0.074 | +314.20€ | 0 | 0 |
| ✅ GBM_LATE_15M#ETH#15min | 1849 | +0.074 | +314.20€ | 0 | 6 |
| ✅ GBM_LATE_15M#SOL | 2281 | +0.082 | +771.15€ | 0 | 0 |
| ✅ GBM_LATE_15M#SOL#15min | 2281 | +0.082 | +771.15€ | 1 | 3 |
| ✅ GBM_LATE_15M#XRP | 2288 | +0.138 | +1091.63€ | 0 | 0 |
| ✅ GBM_LATE_15M#XRP#15min | 2288 | +0.138 | +1091.63€ | 0 | 10 |
| ✅ GBM_LATE_15M_ESPACIO_ATR | 5773 | +0.114 | +2638.94€ | 0 | 8 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#15min | 5773 | +0.114 | +2638.94€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB | 25 | +0.204 | +15.24€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BNB#15min | 25 | +0.204 | +15.24€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC | 1497 | +0.084 | +501.61€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#BTC#15min | 1497 | +0.084 | +501.61€ | 0 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE | 35 | +0.122 | +11.57€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#DOGE#15min | 35 | +0.122 | +11.57€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH | 1491 | +0.088 | +519.34€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#ETH#15min | 1491 | +0.088 | +519.34€ | 0 | 5 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL | 1433 | +0.082 | +541.55€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#SOL#15min | 1433 | +0.082 | +541.55€ | 0 | 4 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP | 1292 | +0.213 | +1049.64€ | 0 | 0 |
| ✅ GBM_LATE_15M_ESPACIO_ATR#XRP#15min | 1292 | +0.213 | +1049.64€ | 0 | 10 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_MULTIHORIZONTE#15min | 6 | +0.000 | -0.08€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO | 290 | +0.062 | +98.48€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#15min | 290 | +0.062 | +98.48€ | 0 | 0 |
| 🚫 GBM_LATE_15M_PYCONFIRMADO#BTC | 9 | -0.102 | -1.94€ | 0 | 0 |
| 🚫 GBM_LATE_15M_PYCONFIRMADO#BTC#15min | 9 | -0.102 | -1.94€ | 0 | 0 |
| ✅ GBM_LATE_15M_PYCONFIRMADO#ETH |