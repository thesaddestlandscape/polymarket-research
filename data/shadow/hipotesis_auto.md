# Hipótesis automáticas — 2026-08-18 23:53 UTC
_Generado por shadow_postmortem.py sobre 69191 resoluciones (PNL=+6896.76€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `151.44` → IC=-0.251 (n=920)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 151.44
  - _Potencial_: sin este filtro IC_bueno=-0.060 (n=2760)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.157 (n=4036)
...
- **PATRÓN** `py_entrada` < `0.34` → IC=+0.286 (n=1514)
...
- **PATRÓN** `libro_liquidez` > `4052.555` → IC=+0.181 (n=822)
  ...
  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 4052.555 (IC base=+0.151)
...
- **PATRÓN** `py_entrada` < `0.7` → IC=+0.190 (n=230)
  ...
  - _Acción_: Kelly boost +0.95€ cuando `py_entrada` < 0.7 (IC base=+0.158)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.195 (n=1769)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.249 (n=1617)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.76 (IC base=+0.188)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `py_entrada` > `0.73` → IC=+0.137 (n=430)

  - _Acción_: Kelly boost +0.68€ cuando `py_entrada` > 0.73 (IC base=+0.097)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.301 (n=260)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.76` → IC=+0.328 (n=248)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.178 (n=318)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.89€ cuando `hora_utc` < 6.0 (IC base=+0.154)
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.286 (n=367)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.74 (IC base=+0.246)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.211 (n=299)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.152 (n=1674)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.37` → IC=+0.209 (n=311)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.37 (IC base=+0.142)

- **PATRÓN** `restante_min` < `3.9` → IC=+0.145 (n=285)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.146 (n=848)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `profundidad_ratio_no` > `11.6` → IC=+0.164 (n=284)
...
- **PATRÓN** `ibs_20min` > `0.9866` → IC=+0.265 (n=428)
...
- **PATRÓN** `sigma_h` > `0.0075` → IC=+0.167 (n=124)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` > 0.0075 (IC base=+0.127)

- **PATRÓN** `drift_60min` |x|≤ `0.0628` → IC=+0.133 (n=126)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.66€ cuando `drift_60min` |x|≤ 0.0628 (IC base=+0.127)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.4168` → IC=+0.167 (n=43)
  ...
  - _Acción_: Kelly boost +0.83€ cuando `volumen_pendiente_norm` > 0.4168 (IC base=+0.127)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.146 (n=413)

  - _Acción_: Kelly boost +0.73€ cuando `libro_spread` < 0.06 (IC base=+0.127)

- **PATRÓN** `ballena_activa_n` < `27.0` → IC=+0.123 (n=59)

  - _Acción_: Kelly boost +0.61€ cuando `ballena_activa_n` < 27.0 (IC base=+0.127)
...
- **PATRÓN** `volumen_spike_ratio` > `2.2038` → IC=+0.155 (n=447)
...
- **PATRÓN** `ibs_20min` > `0.9211` → IC=+0.253 (n=859)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9211 (IC base=+0.139)

- **PATRÓN** `dist_vwap_pct` > `0.1207` → IC=+0.150 (n=378)

  - _Acción_: Kelly boost +0.75€ cuando `dist_vwap_pct` > 0.1207 (IC base=+0.139)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.934` → IC=+0.267 (n=632)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.934 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` < `1.1814` → IC=+0.143 (n=668)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 1.1814 (IC base=+0.139)

- **PATRÓN** `volumen_regimen` > `0.6258` → IC=+0.143 (n=668)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` > 0.6258 (IC base=+0.139)

- **PATRÓN** `volumen_pendiente_norm` > `0.1106` → IC=+0.156 (n=431)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_pendiente_norm` > 0.1106 (IC base=+0.139)

- **PATRÓN** `libro_spread` < `0.06` → IC=+0.146 (n=1452)

  - _Acción_: Kelly boost +0.73€ cuando `libro_spread` < 0.06 (IC base=+0.139)

- **PATRÓN** `libro_liquidez` > `2698.2354` → IC=+0.178 (n=430)
...
- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.147 (n=100)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.74€ cuando `sigma_h` < 0.0056 (IC base=+0.146)

- **PATRÓN** `sigma_h` > `0.0065` → IC=+0.170 (n=201)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.85€ cuando `sigma_h` > 0.0065 (IC base=+0.146)

- **PATRÓN** `drift_60min` |x|≤ `0.1453` → IC=+0.153 (n=200)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.77€ cuando `drift_60min` |x|≤ 0.1453 (IC base=+0.146)

- **PATRÓN** `hora_utc` > `16.0` → IC=+0.161 (n=119)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 16.0 (IC base=+0.146)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2311` → IC=+0.125 (n=238)
  ...
  - _Acción_: Kelly boost +0.62€ cuando `volumen_pendiente_norm` < 0.2311 (IC base=+0.146)
...
- **PATRÓN** `volumen_spike_ratio` > `3.8997` → IC=+0.149 (n=109)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 3.8997 (IC base=+0.146)
...
- **PATRÓN** `libro_liquidez` > `1970.881` → IC=+0.157 (n=100)
  ...
  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 1970.881 (IC base=+0.146)
...
- **PATRÓN** `sigma_h` < `0.0021` → IC=+0.318 (n=31)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0021 (IC base=+0.256)

- **PATRÓN** `sigma_h` > `0.0029` → IC=+0.262 (n=61)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0029 (IC base=+0.256)

- **PATRÓN** `drift_60min` |x|≤ `0.1718` → IC=+0.309 (n=61)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1718 (IC base=+0.256)

- **PATRÓN** `hora_utc` > `9.0` → IC=+0.261 (n=86)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.7638` → IC=+0.306 (n=91)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7638 (IC base=+0.256)

- **PATRÓN** `dist_vwap_pct` < `0.3636` → IC=+0.287 (n=92)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.3636 (IC base=+0.256)

- **PATRÓN** `sigma_ewma_delta_pct` > `13.328` → IC=+0.413 (n=21)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 13.328 (IC base=+0.256)

- **PATRÓN** `volumen_regimen` > `0.6636` → IC=+0.285 (n=91)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6636 (IC base=+0.256)

- **PATRÓN** `volumen_pendiente_norm` > `0.107` → IC=+0.296 (n=47)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.107 (IC base=+0.256)

- **PATRÓN** `volumen_spike_ratio` < `1.688` → IC=+0.295 (n=37)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.688 (IC base=+0.256)

- **PATRÓN** `volumen_spike_ratio` > `3.1444` → IC=+0.267 (n=28)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 3.1444 (IC base=+0.256)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.258 (n=97)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.01 (IC base=+0.256)

- **PATRÓN** `libro_liquidez` > `2543.553` → IC=+0.263 (n=91)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2543.553 (IC base=+0.256)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.150 (n=595)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 17.0 (IC base=+0.119)

- **PATRÓN** `ibs_20min` > `0.9626` → IC=+0.280 (n=708)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9626 (IC base=+0.119)

- **PATRÓN** `dist_vwap_pct` > `0.3549` → IC=+0.189 (n=175)

  - _Acción_: Kelly boost +0.95€ cuando `dist_vwap_pct` > 0.3549 (IC base=+0.119)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.252` → IC=+0.231 (n=1049)
...
- **PATRÓN** `libro_spread` < `0.06` → IC=+0.125 (n=1748)

  - _Acción_: Kelly boost +0.63€ cuando `libro_spread` < 0.06 (IC base=+0.119)

- **PATRÓN** `libro_liquidez` > `2738.706` → IC=+0.158 (n=521)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 2738.706 (IC base=+0.119)

- **PATRÓN** `ballena_activa_n` < `142.0` → IC=+0.183 (n=184)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.235 (n=1273)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` < `0.1568` → IC=+0.199 (n=905)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1568 (IC base=+0.224)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.488` → IC=+0.226 (n=1295)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 2.488 (IC base=+0.224)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.113` → IC=+0.249 (n=619)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.113 (IC base=+0.224)

- **PATRÓN** `volumen_pendiente_norm` > `0.2516` → IC=+0.265 (n=168)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2516 (IC base=+0.224)

- **PATRÓN** `volumen_spike_ratio` < `2.0228` → IC=+0.273 (n=464)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.0228 (IC base=+0.224)
...
- **PATRÓN** `ibs_20min` < `0.5455` → IC=+0.335 (n=259)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5455 (IC base=+0.275)

- **PATRÓN** `sigma_ewma_delta_pct` > `4.674` → IC=+0.278 (n=34)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 4.674 (IC base=+0.275)

- **PATRÓN** `volumen_pendiente_norm` < `0.2348` → IC=+0.231 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2348 (IC base=+0.275)

- **PATRÓN** `volumen_pendiente_norm` > `0.3816` → IC=+0.286 (n=26)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3816 (IC base=+0.275)

- **PATRÓN** `volumen_spike_ratio` < `3.6802` → IC=+0.264 (n=121)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.6802 (IC base=+0.275)
...
- **PATRÓN** `ballena_activa_n` < `20.0` → IC=+0.184 (n=17)
  ...
  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 20.0 (IC base=+0.275)

### GBM_LATE_15M_TARDIO#ETH#15min
- **FILTRO** `ibs_20min` < `0.3557` → IC=-0.222 (n=52)

  - _Acción_: SKIP cuando `ibs_20min` < 0.3557
  - _Potencial_: sin este filtro IC_bueno=+0.212 (n=158)

- **PATRÓN** `sigma_h` < `0.0019` → IC=+0.209 (n=53)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0019 (IC base=+0.104)

- **PATRÓN** `drift_60min` |x|≤ `0.0533` → IC=+0.136 (n=53)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.0533 (IC base=+0.104)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.182 (n=61)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 17.0 (IC base=+0.104)

- **PATRÓN** `ibs_20min` > `0.3557` → IC=+0.212 (n=158)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3557 (IC base=+0.104)

- **PATRÓN** `dist_vwap_pct` > `0.435` → IC=+0.204 (n=25)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.435 (IC base=+0.104)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.292` → IC=+0.207 (n=73)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.292 (IC base=+0.104)

- **PATRÓN** `volumen_regimen` < `0.7843` → IC=+0.153 (n=70)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.7843 (IC base=+0.104)

- **PATRÓN** `volumen_regimen` > `1.0903` → IC=+0.135 (n=72)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` > 1.0903 (IC base=+0.104)

- **PATRÓN** `volumen_pendiente_norm` > `0.2893` → IC=+0.292 (n=22)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2893 (IC base=+0.104)

- **PATRÓN** `volumen_spike_ratio` > `1.4389` → IC=+0.221 (n=127)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4389 (IC base=+0.104)

- **PATRÓN** `libro_liquidez` > `7277.355` → IC=+0.257 (n=72)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 7277.355 (IC base=+0.104)
...
- **PATRÓN** `hora_utc` > `19.0` → IC=+0.136 (n=20)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.68€ cuando `hora_utc` > 19.0 (IC base=+0.103)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.227 (n=20)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.103)

- **PATRÓN** `drift_20min_pct` |x|≤ `0.0669` → IC=+0.179 (n=26)

  - _Acción_: Kelly boost +0.89€ cuando `drift_20min_pct` |x|≤ 0.0669 (IC base=+0.103)

- **PATRÓN** `libro_liquidez` > `15323.3068` → IC=+0.214 (n=19)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 15323.3068 (IC base=+0.103)
...
- **FILTRO** `py_entrada` < `0.41` → IC=-0.273 (n=108)

  - _Acción_: SKIP cuando `py_entrada` < 0.41
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=325)

- **FILTRO** `ibs_20min` < `0.7235` → IC=-0.245 (n=108)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7235
  - _Potencial_: sin este filtro IC_bueno=-0.053 (n=325)

- **FILTRO** `ballena_activa_n` > `14.0` → IC=-0.200 (n=108)

  - _Acción_: SKIP cuando `ballena_activa_n` > 14.0
  - _Potencial_: sin este filtro IC_bueno=-0.069 (n=325)

- **FILTRO** `libro_liquidez` < `2645.8085` → IC=-0.126 (n=324)

  - _Acción_: SKIP cuando `libro_liquidez` < 2645.8085
...
- **FILTRO** `libro_liquidez` < `11699.3594` → IC=-0.146 (n=46)
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.278 (n=291)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.045 (n=912)

- **FILTRO** `ibs_7min` < `0.7242` → IC=-0.235 (n=300)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7242
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=903)

- **FILTRO** `ballena_activa_n` > `25.0` → IC=-0.190 (n=298)

  - _Acción_: SKIP cuando `ballena_activa_n` > 25.0
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=905)

- **FILTRO** `py_entrada` > `0.65` → IC=-0.196 (n=320)

  - _Acción_: SKIP cuando `py_entrada` > 0.65
  - _Potencial_: sin este filtro IC_bueno=+0.016 (n=991)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.31` → IC=-0.235 (n=47)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.107 (n=143)

- **FILTRO** `ibs_7min` < `0.8533` → IC=-0.211 (n=95)
...
- **FILTRO** `ibs_7min` < `0.8176` → IC=-0.212 (n=64)
...
- **PATRÓN** `sigma_h` > `0.0033` → IC=+0.292 (n=51)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0033 (IC base=+0.253)

- **PATRÓN** `drift_60min` |x|≤ `0.1826` → IC=+0.263 (n=112)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1826 (IC base=+0.253)

- **PATRÓN** `drift_15min` |x|≤ `0.5909` → IC=+0.260 (n=98)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.5909 (IC base=+0.253)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0673` → IC=+0.270 (n=111)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0673 (IC base=+0.253)

- **PATRÓN** `hora_utc` > `4.0` → IC=+0.300 (n=103)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 4.0 (IC base=+0.253)

- **PATRÓN** `ibs_15` > `0.9358` → IC=+0.329 (n=74)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` > 0.9358 (IC base=+0.253)

- **PATRÓN** `dist_vwap_pct` > `0.3763` → IC=+0.316 (n=36)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3763 (IC base=+0.253)

- **PATRÓN** `dist_vwap_pct` < `0.0845` → IC=+0.297 (n=62)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.0845 (IC base=+0.253)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.952` → IC=+0.267 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 6.952 (IC base=+0.253)

- **PATRÓN** `libro_liquidez` > `5397.7239` → IC=+0.263 (n=74)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 5397.7239 (IC base=+0.253)
...
- **H-IBS-UPDOWN_GBM_IBS_ALTO**: dentro de BUY_YES, IBS > 0.9358 sube el IC de +0.253 a +0.329 en UPDOWN_GBM_IBS_ALTO (n=74). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 3457 | -0.114 | -553.60€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 464 | -0.021 | -16.99€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 2993 | -0.128 | -536.61€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 847 | +0.008 | -98.64€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 847 | +0.008 | -98.64€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP | 720 | -0.227 | -142.53€ | 0 | 0 |
| 🚫 BALLENAS_TARDIAS#XRP#5min | 720 | -0.227 | -142.53€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 14900 | +0.115 | -858.06€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 3450 | +0.183 | -95.10€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 99 | -0.104 | -47.87€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 8739 | +0.086 | -743.97€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2612 | +0.133 | +28.88€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 1475 | +0.030 | -326.13€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 13 | -0.065 | -2.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 1458 | +0.033 | -318.09€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 3369 | +0.142 | +11.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 943 | +0.200 | -25.12€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 41 | -0.105 | -20.74€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 1456 | +0.114 | -7.04€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 929 | +0.139 | +63.94€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 1471 | +0.055 | -256.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 9 | -0.021 | -3.54€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 1461 | +0.056 | -251.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 3641 | +0.126 | -41.94€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1275 | +0.163 | -15.16€ | 0 | 7 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 1451 | +0.103 | -23.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 903 | +0.118 | +5.16€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL | 3474 | +0.136 | -201.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 1198 | +0.198 | -50.68€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 40 | +0.000 | -9.29€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 1456 | +0.086 | -101.24€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 780 | +0.142 | -40.21€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#XRP | 1470 | +0.124 | -42.67€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 12 | +0.043 | +1.69€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 1457 | +0.124 | -42.89€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 3522 | +0.160 | -342.89€ | 2 | 3 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 3522 | +0.160 | -342.89€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE | 874 | +0.158 | -111.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#DOGE#15min | 874 | +0.158 | -111.01€ | 0 | 2 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH | 784 | +0.218 | -43.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#ETH#15min | 784 | +0.218 | -43.36€ | 0 | 3 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION | 6222 | +0.188 | -590.50€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#5min | 6222 | +0.188 | -590.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB | 1163 | +0.097 | -263.42€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min | 1163 | +0.097 | -263.42€ | 0 | 1 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC | 957 | +0.251 | -5.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min | 957 | +0.251 | -5.75€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE | 1092 | +0.154 | -156.47€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#DOGE#5min | 1092 | +0.154 | -156.47€ | 0 | 2 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL | 970 | +0.246 | -12.27€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#SOL#5min | 970 | +0.246 | -12.27€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP | 1039 | +0.183 | -107.86€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#XRP#5min | 1039 | +0.183 | -107.86€ | 0 | 3 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA | 2230 | +0.148 | +121.04€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#5min | 2230 | +0.148 | +121.04€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP | 1129 | +0.142 | +49.32€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_5MIN_BAJALATENCIA#XRP#5min | 1129 | +0.142 | +49.32€ | 0 | 7 |
...
| ✅ GBM_LATE_15M | 4618 | +0.086 | +1635.89€ | 0 | 14 |
| ✅ GBM_LATE_15M#15min | 4618 | +0.086 | +1635.89€ | 0 | 0 |
...
| ✅ GBM_LATE_15M#DOGE |