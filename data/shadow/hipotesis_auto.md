# Hipótesis automáticas — 2026-08-22 14:02 UTC
_Generado por shadow_postmortem.py sobre 113987 resoluciones (PNL=+9013.35€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `157.2` → IC=-0.261 (n=1597)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 157.2
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4794)

### BALLENAS_TARDIAS#BNB#5min
- **FILTRO** `n_ballenas` < `4.0` → IC=-0.126 (n=396)

  - _Acción_: SKIP cuando `n_ballenas` < 4.0
  - _Potencial_: sin este filtro IC_bueno=-0.061 (n=228)

- **FILTRO** `restante_s_al_confirmar` < `169.32` → IC=-0.263 (n=205)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 169.32
  - _Potencial_: sin este filtro IC_bueno=-0.023 (n=419)
...
- **FILTRO** `restante_s_al_confirmar` < `145.28` → IC=-0.253 (n=390)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.056 (n=1171)

### BALLENAS_TARDIAS#XRP#5min
- **FILTRO** `restante_s_al_confirmar` < `253.99` → IC=-0.243 (n=933)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 253.99
  - _Potencial_: sin este filtro IC_bueno=-0.021 (n=311)

### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.192 (n=3817)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.191 (n=429)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.180 (n=610)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.126 (n=934)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_liquidez` > `762.1544` → IC=+0.224 (n=454)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 762.1544 (IC base=+0.203)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.230 (n=5777)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.253 (n=1144)
...
- **PATRÓN** `restante_min` > `4.92` → IC=+0.147 (n=978)
...
- **PATRÓN** `lag_apertura_s` < `4.7` → IC=+0.144 (n=943)

  - _Acción_: Kelly boost +0.72€ cuando `lag_apertura_s` < 4.7 (IC base=+0.130)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.125 (n=1443)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `ibs_20min` < `0.4048` → IC=+0.124 (n=1826)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` < 0.4048 (IC base=+0.041)
...
- **FILTRO** `sigma_ewma_delta_pct` > `2.933` → IC=-0.132 (n=305)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 2.933
  - _Potencial_: sin este filtro IC_bueno=+0.001 (n=729)
...
- **FILTRO** `libro_liquidez` < `8738.1618` → IC=-0.182 (n=83)
...
- **PATRÓN** `sigma_h` < `0.0063` → IC=+0.212 (n=1381)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0063 (IC base=+0.190)

- **PATRÓN** `drift_60min` |x|≤ `0.3226` → IC=+0.199 (n=1567)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.3226 (IC base=+0.190)

- **PATRÓN** `hora_utc` > `14.0` → IC=+0.233 (n=791)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 14.0 (IC base=+0.190)

- **PATRÓN** `ibs_20min` < `0.3937` → IC=+0.251 (n=1567)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3937 (IC base=+0.190)

- **PATRÓN** `dist_vwap_pct` < `0.4879` → IC=+0.173 (n=1433)

  - _Acción_: Kelly boost +0.87€ cuando `dist_vwap_pct` < 0.4879 (IC base=+0.190)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.677` → IC=+0.198 (n=313)

  - _Acción_: Kelly boost +0.99€ cuando `sigma_ewma_delta_pct` > 9.677 (IC base=+0.190)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.398` → IC=+0.191 (n=1577)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` < 5.398 (IC base=+0.190)

- **PATRÓN** `volumen_regimen` < `1.174` → IC=+0.164 (n=1267)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 1.174 (IC base=+0.190)

- **PATRÓN** `volumen_regimen` > `1.0285` → IC=+0.176 (n=575)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_regimen` > 1.0285 (IC base=+0.190)

- **PATRÓN** `volumen_pendiente_norm` > `0.2511` → IC=+0.244 (n=236)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2511 (IC base=+0.190)

- **PATRÓN** `volumen_spike_ratio` < `1.6125` → IC=+0.185 (n=424)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 1.6125 (IC base=+0.190)

- **PATRÓN** `volumen_spike_ratio` > `2.8167` → IC=+0.225 (n=321)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.8167 (IC base=+0.190)

- **PATRÓN** `ballena_activa_n` < `58.0` → IC=+0.195 (n=355)

  - _Acción_: Kelly boost +0.97€ cuando `ballena_activa_n` < 58.0 (IC base=+0.190)
...
- **PATRÓN** `sigma_h` < `0.0037` → IC=+0.199 (n=274)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0037 (IC base=+0.160)

- **PATRÓN** `drift_60min` |x|≤ `0.2043` → IC=+0.203 (n=274)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2043 (IC base=+0.160)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.183 (n=288)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 7.0 (IC base=+0.160)

- **PATRÓN** `ibs_20min` < `0.3397` → IC=+0.210 (n=312)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3397 (IC base=+0.160)

- **PATRÓN** `dist_vwap_pct` < `0.1366` → IC=+0.192 (n=313)

  - _Acción_: Kelly boost +0.96€ cuando `dist_vwap_pct` < 0.1366 (IC base=+0.160)

- **PATRÓN** `sigma_ewma_delta_pct` > `12.625` → IC=+0.250 (n=78)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 12.625 (IC base=+0.160)

- **PATRÓN** `volumen_regimen` < `0.6283` → IC=+0.236 (n=104)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.6283 (IC base=+0.160)

- **PATRÓN** `volumen_pendiente_norm` > `0.0712` → IC=+0.216 (n=107)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.0712 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` < `1.7551` → IC=+0.198 (n=137)

  - _Acción_: Kelly boost +0.99€ cuando `volumen_spike_ratio` < 1.7551 (IC base=+0.160)

- **PATRÓN** `volumen_spike_ratio` > `1.4427` → IC=+0.165 (n=204)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.4427 (IC base=+0.160)

- **PATRÓN** `ballena_activa_n` < `230.0` → IC=+0.183 (n=39)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 230.0 (IC base=+0.160)
...
- **PATRÓN** `sigma_h` < `0.0057` → IC=+0.193 (n=307)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.0057 (IC base=+0.149)

- **PATRÓN** `drift_60min` |x|≤ `0.344` → IC=+0.170 (n=307)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.344 (IC base=+0.149)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.160 (n=313)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.80€ cuando `hora_utc` > 5.0 (IC base=+0.149)

- **PATRÓN** `ibs_20min` < `0.272` → IC=+0.232 (n=270)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.272 (IC base=+0.149)

- **PATRÓN** `dist_vwap_pct` < `0.4406` → IC=+0.172 (n=358)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.4406 (IC base=+0.149)
...
- **PATRÓN** `volumen_regimen` < `0.8485` → IC=+0.167 (n=205)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_regimen` < 0.8485 (IC base=+0.149)

- **PATRÓN** `volumen_regimen` > `0.6089` → IC=+0.153 (n=306)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 0.6089 (IC base=+0.149)

- **PATRÓN** `volumen_pendiente_norm` > `0.24` → IC=+0.160 (n=48)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_pendiente_norm` > 0.24 (IC base=+0.149)

- **PATRÓN** `volumen_spike_ratio` < `1.9018` → IC=+0.193 (n=135)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_spike_ratio` < 1.9018 (IC base=+0.149)

### GBM_LATE_15M_MULTIHORIZONTE#SOL#15min
- **FILTRO** `ibs_20min` > `0.4737` → IC=-0.226 (n=100)

  - _Acción_: SKIP cuando `ibs_20min` > 0.4737
  - _Potencial_: sin este filtro IC_bueno=+0.213 (n=308)
...
- **PATRÓN** `ibs_20min` < `0.4737` → IC=+0.213 (n=308)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4737 (IC base=+0.105)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.178` → IC=+0.167 (n=43)

  - _Acción_: Kelly boost +0.83€ cuando `sigma_ewma_delta_pct` > 7.178 (IC base=+0.105)

- **PATRÓN** `volumen_regimen` > `0.853` → IC=+0.146 (n=204)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 0.853 (IC base=+0.105)

- **PATRÓN** `volumen_pendiente_norm` > `0.094` → IC=+0.171 (n=80)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_pendiente_norm` > 0.094 (IC base=+0.105)

- **PATRÓN** `volumen_spike_ratio` > `2.4132` → IC=+0.189 (n=59)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 2.4132 (IC base=+0.105)

- **PATRÓN** `libro_liquidez` > `2410.3616` → IC=+0.181 (n=139)

  - _Acción_: Kelly boost +0.90€ cuando `libro_liquidez` > 2410.3616 (IC base=+0.105)

- **PATRÓN** `ballena_activa_n` < `32.0` → IC=+0.153 (n=47)

  - _Acción_: Kelly boost +0.77€ cuando `ballena_activa_n` < 32.0 (IC base=+0.105)
...
- **PATRÓN** `sigma_h` < `0.0071` → IC=+0.217 (n=1641)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0071 (IC base=+0.189)

- **PATRÓN** `drift_60min` |x|≤ `0.3646` → IC=+0.201 (n=1865)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `ibs_20min` < `0.5385` → IC=+0.248 (n=1865)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.5385 (IC base=+0.189)

- **PATRÓN** `dist_vwap_pct` < `0.5827` → IC=+0.177 (n=1436)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.5827 (IC base=+0.189)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `2.566` → IC=+0.193 (n=1747)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` < 2.566 (IC base=+0.189)

- **PATRÓN** `volumen_regimen` < `0.6111` → IC=+0.181 (n=462)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` < 0.6111 (IC base=+0.189)

- **PATRÓN** `volumen_regimen` > `1.1823` → IC=+0.187 (n=461)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_regimen` > 1.1823 (IC base=+0.189)

- **PATRÓN** `volumen_pendiente_norm` > `0.2365` → IC=+0.238 (n=250)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2365 (IC base=+0.189)

- **PATRÓN** `volumen_spike_ratio` > `2.2942` → IC=+0.206 (n=532)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2942 (IC base=+0.189)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.143 (n=329)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3993` → IC=+0.155 (n=230)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.3993 (IC base=+0.136)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.146 (n=207)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.2541` → IC=+0.211 (n=154)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.2541 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` > `0.1508` → IC=+0.146 (n=125)

  - _Acción_: Kelly boost +0.73€ cuando `dist_vwap_pct` > 0.1508 (IC base=+0.136)

- **PATRÓN** `dist_vwap_pct` < `0.5211` → IC=+0.157 (n=234)

  - _Acción_: Kelly boost +0.78€ cuando `dist_vwap_pct` < 0.5211 (IC base=+0.136)

- **PATRÓN** `sigma_ewma_delta_pct` > `11.864` → IC=+0.250 (n=46)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 11.864 (IC base=+0.136)

- **PATRÓN** `volumen_regimen` < `0.5484` → IC=+0.158 (n=77)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.5484 (IC base=+0.136)

- **PATRÓN** `volumen_pendiente_norm` > `0.2304` → IC=+0.308 (n=24)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2304 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` < `2.1206` → IC=+0.186 (n=154)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_spike_ratio` < 2.1206 (IC base=+0.136)

- **PATRÓN** `volumen_spike_ratio` > `1.4547` → IC=+0.182 (n=174)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_spike_ratio` > 1.4547 (IC base=+0.136)

- **PATRÓN** `libro_liquidez` > `4090.2043` → IC=+0.152 (n=153)

  - _Acción_: Kelly boost +0.76€ cuando `libro_liquidez` > 4090.2043 (IC base=+0.136)

- **PATRÓN** `ballena_activa_n` < `125.0` → IC=+0.255 (n=51)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 125.0 (IC base=+0.136)
...
- **PATRÓN** `sigma_h` < `0.0049` → IC=+0.196 (n=113)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` < 0.0049 (IC base=+0.097)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.162 (n=155)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.81€ cuando `hora_utc` > 15.0 (IC base=+0.097)

- **PATRÓN** `ibs_20min` < `0.5789` → IC=+0.182 (n=338)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` < 0.5789 (IC base=+0.097)

- **PATRÓN** `dist_vwap_pct` < `0.4003` → IC=+0.129 (n=289)

  - _Acción_: Kelly boost +0.64€ cuando `dist_vwap_pct` < 0.4003 (IC base=+0.097)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.558` → IC=+0.150 (n=58)

  - _Acción_: Kelly boost +0.75€ cuando `sigma_ewma_delta_pct` > 6.558 (IC base=+0.097)

- **PATRÓN** `volumen_regimen` < `0.6196` → IC=+0.135 (n=113)

  - _Acción_: Kelly boost +0.67€ cuando `volumen_regimen` < 0.6196 (IC base=+0.097)

- **PATRÓN** `volumen_regimen` > `1.0473` → IC=+0.126 (n=153)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` > 1.0473 (IC base=+0.097)

- **PATRÓN** `volumen_pendiente_norm` < `0.1471` → IC=+0.132 (n=202)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_pendiente_norm` < 0.1471 (IC base=+0.097)

- **PATRÓN** `volumen_pendiente_norm` > `0.072` → IC=+0.147 (n=83)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_pendiente_norm` > 0.072 (IC base=+0.097)

- **PATRÓN** `volumen_spike_ratio` > `1.5283` → IC=+0.171 (n=171)

  - _Acción_: Kelly boost +0.85€ cuando `volumen_spike_ratio` > 1.5283 (IC base=+0.097)

- **PATRÓN** `libro_liquidez` > `1285.46` → IC=+0.139 (n=225)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 1285.46 (IC base=+0.097)
...
- **FILTRO** `py_entrada` < `0.45` → IC=-0.167 (n=424)

  - _Acción_: SKIP cuando `py_entrada` < 0.45
  - _Potencial_: sin este filtro IC_bueno=+0.063 (n=1304)
...
- **FILTRO** `py_entrada` < `0.5` → IC=-0.152 (n=110)

  - _Acción_: SKIP cuando `py_entrada` < 0.5
  - _Potencial_: sin este filtro IC_bueno=+0.081 (n=146)

- **FILTRO** `ibs_20min` < `0.7317` → IC=-0.167 (n=64)
...
- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.143 (n=68)

  - _Acción_: Kelly boost +0.71€ cuando `ibs_20min` > 1.0 (IC base=-0.019)
...
- **PATRÓN** `ballena_activa_n` < `1.0` → IC=+0.122 (n=133)
  ...
  - _Acción_: Kelly boost +0.61€ cuando `ballena_activa_n` < 1.0 (IC base=+0.027)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.154 (n=867)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.046 (n=3311)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.282 (n=1014)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=3164)

- **FILTRO** `ibs_7min` < `0.7321` → IC=-0.229 (n=1043)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7321
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3135)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.171 (n=1420)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=2758)

- **FILTRO** `py_entrada` > `0.68` → IC=-0.229 (n=1200)

  - _Acción_: SKIP cuando `py_entrada` > 0.68
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=3740)

- **FILTRO** `ibs_7min` > `0.7133` → IC=-0.170 (n=1234)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7133
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=3706)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `py_entrada` < `0.29` → IC=-0.343 (n=138)

  - _Acción_: SKIP cuando `py_entrada` < 0.29
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=461)

- **FILTRO** `ibs_7min` < `0.9545` → IC=-0.190 (n=395)
...
- **FILTRO** `hora_utc` > `17.0` → IC=-0.137 (n=166)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: SKIP cuando `hora_utc` > 17.0
  - _Potencial_: sin este filtro IC_bueno=-0.096 (n=602)
...
- **FILTRO** `drift_7min_pct` |x|> `0.1398` → IC=-0.181 (n=261)
...
- **FILTRO** `ibs_7min` > `0.8296` → IC=-0.205 (n=191)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=577)

- **FILTRO** `ballena_activa_n` > `14.0` → IC=-0.152 (n=185)

  - _Acción_: SKIP cuando `ballena_activa_n` > 14.0
  - _Potencial_: sin este filtro IC_bueno=-0.090 (n=583)
...
- **FILTRO** `ibs_7min` < `0.7976` → IC=-0.165 (n=210)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=633)

- **FILTRO** `ballena_activa_n` > `145.0` → IC=-0.151 (n=210)

  - _Acción_: SKIP cuando `ballena_activa_n` > 145.0
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=633)

- **FILTRO** `py_entrada` > `0.54` → IC=-0.220 (n=212)

  - _Acción_: SKIP cuando `py_entrada` > 0.54
  - _Potencial_: sin este filtro IC_bueno=+0.006 (n=644)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.208 (n=169)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=403)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.272 (n=195)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.004 (n=658)

- **FILTRO** `ibs_7min` > `0.8261` → IC=-0.174 (n=213)
...
- **FILTRO** `py_entrada` > `0.63` → IC=-0.279 (n=179)
...
- **FILTRO** `ballena_activa_n` > `36.0` → IC=-0.206 (n=178)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=544)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.39` → IC=-0.197 (n=183)

  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=+0.012 (n=592)
...
- **FILTRO** `py_entrada` < `0.37` → IC=-0.265 (n=160)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.052 (n=498)
...
- **FILTRO** `ibs_7min` > `0.2913` → IC=-0.171 (n=281)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.755` → IC=+0.145 (n=74)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` > 8.755 (IC base=+0.026)
...
- **FILTRO** `drift_15min` |x|> `0.568` → IC=-0.173 (n=50)
...
- **FILTRO** `sigma_h` > `0.0078` → IC=-0.175 (n=639)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **FILTRO** `drift_15min` |x|> `0.8473` → IC=-0.133 (n=469)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=1411)

- **FILTRO** `sigma_ewma_delta_pct` > `13.46` → IC=-0.191 (n=338)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 13.46
  - _Potencial_: sin este filtro IC_bueno=-0.076 (n=1542)
...
- **PATRÓN** `dist_vwap_pct` < `0.1453` → IC=+0.173 (n=53)
  ...
  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` < 0.1453 (IC base=-0.097)
...
- **FILTRO** `sigma_h` > `0.0083` → IC=-0.155 (n=224)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.53 sube el IC de -0.097 a +0.268 en UPDOWN_GBM_15M_TARDIO (n=54). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 5097 | -0.104 | -556.50€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 584 | -0.085 | -110.57€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 4513 | -0.106 | -445.92€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 624 | -0.102 | -80.07€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 624 | -0.102 | -80.07€ | 2 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 584 | -0.085 | -110.57€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 584 | -0.085 | -110.57€ | 2 | 0 |
...
| ✅ BALLENAS_TARDIAS#SOL | 1561 | -0.021 | +29.15€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 1561 | -0.021 | +29.15€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 1244 | -0.188 | -201.54€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 1244 | -0.188 | -201.54€ | 1 | 0 |
| ✅ FAVORITO_CONFIRMADO | 22223 | +0.114 | -1548.67€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 4506 | +0.184 | -183.81€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 106 | -0.102 | -48.79€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 14741 | +0.091 | -1298.22€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 2870 | +0.125 | -17.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 2482 | +0.060 | -448.68€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 17 | -0.067 | -0.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#240min | 5 | -0.089 | -6.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 2460 | +0.063 | -442.36€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 4751 | +0.134 | -108.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 1249 | +0.191 | -85.34€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 2451 | +0.109 | -65.59€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 1009 | +0.136 | +64.50€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 2485 | +0.068 | -385.21€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 16 | +0.044 | -1.11€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 2468 | +0.068 | -382.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 5122 | +0.129 | -63.03€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 1652 | +0.169 | -4.93€ | 0 | 8 |
| 🚫 FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 2451 | +0.113 | -34.82€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 
60€ | 5 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC | 621 | -0.026 | -15.33€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#BTC#15min | 621 | -0.026 | -15.33€ | 4 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE | 542 | -0.031 | +73.23€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#DOGE#15min | 542 | -0.031 | +73.23€ | 4 | 1 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH | 614 | -0.023 | -6.88€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#ETH#15min | 614 | -0.023 | -6.88€ | 3 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL | 551 | -0.048 | +4.29€ | 0 | 0 |
| ✅ MOMENTUM_IBS_15M_BALLENA#SOL#15min | 551 | -0.048 | +4.29€ | 5 | 1 |
...
| ✅ MOMENTUM_IBS_5M | 1458 | +0.009 | +11.07€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#5min | 1458 | +0.009 | +11.07€ | 0 | 0 |
...
| ✅ MOMENTUM_IBS_5M#ETH | 276 | +0.029 | +14.69€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#ETH#5min | 276 | +0.029 | +14.69€ | 1 | 0 |
| ✅ MOMENTUM_IBS_5M#SOL | 557 | +0.017 | +7.30€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#SOL#5min | 557 | +0.017 | +7.30€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#XRP | 178 | -0.006 | -5.46€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M#XRP#5min | 178 | -0.006 | -5.46€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA | 9118 | -0.072 | +179.48€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#5min | 9118 | -0.072 | +179.48€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB | 1367 | -0.106 | +97.81€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BNB#5min | 1367 | -0.106 | +97.81€ | 8 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC | 1699 | -0.055 | +66.19€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#BTC#5min | 1699 | -0.055 | +66.19€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE | 1425 | -0.080 | +35.42€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#DOGE#5min | 1425 | -0.080 | +35.42€ | 6 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH | 1453 | -0.093 | -108.59€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#ETH#5min | 1453 | -0.093 | -108.59€ | 7 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL | 1689 | -0.049 | +26.10€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#SOL#5min | 1689 | -0.049 | +26.10€ | 4 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP | 1485 | -0.058 | +62.55€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_BALLENA#XRP#5min | 1485 | -0.058 | +62.55€ | 5 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE | 5852 | -0.008 | -102.20€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#5min | 5852 | -0.008 | -102.20€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB | 992 | -0.018 | -21.41€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BNB#5min | 992 | -0.018 | -21.41€ | 3 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC | 1045 | +0.015 | +1.90€ | 0 | 0 |
| ✅ MOMENTUM_IBS_5M_FADE#BTC#5min | 1045 | +0.015 | +1.90€ | 1 | 0 |
...
| ✅ STREAK_MOM_5M | 1165 | +0.018 | +4.68€ | 0 | 0 |
| ✅ STREAK_MOM_5M#5min | 1165 | +0.018 | +4.68€ | 0 | 0 |
...
| ✅ STREAK_MOM_5M#SOL | 347 | +0.010 | -2.33€ | 0 | 0 |
| ✅ STREAK_MOM_5M#SOL#5min | 347 | +0.010 | -2.33€ | 0 | 1 |
...
| ✅ UPDOWN_GBM | 3881 | +0.012 | +146.94€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 1605 | +0.050 | +168.97€ | 0 | 8 |
| ✅ UPDOWN_GBM#240min | 176 | +0.028 | +4.68€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 1750 | -0.018 | -34.64€ | 4 | 0 |
...
| ✅ UPDOWN_GBM#ETH | 868 | +0.028 | +39.27€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 405 | +0.055 | +34.76€ | 1 | 6 |
| ✅ UPDOWN_GBM#ETH#240min | 53 | +0.082 | +4.06€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 268 | -0.018 | -9.15€ | 5 | 1 |
| ✅ UPDOWN_GBM#ETH#60min | 127 | +0.043 | +9.98€ | 0 | 4 |
| 🚫 UPDOWN_GBM#ETH#daily | 15 | -0.154 | -0.38€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 862 | -0.018 | -22.62€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 346 | -0.011 | -11.06€ | 1 | 2 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 2604 | -0.081 | +412.29€ | 4 | 3 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 2604 | -0.081 | +412.29€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 834 | -0.062 | +188.93€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 834 | -0.062 | +188.93€ | 3 | 1 |
...
**〰️ H-IBS-15** — IBS-15 como señal de mean-reversion
  ...
  - _Estado_: Spread bajo (0.101) — sin ventaja clara. oversold(IBS<0.3): IC=-0.008 n=1316 | neutral: IC=+0.010 n=1391 | overbought(IBS>0.7): IC=+0.093 n=1611
  - _Datos_: n=4565 IC=+0.035 PNL=+348.62€

**🟡 H-KELLY-HORA** — Kelly boost ×1.2 por celda (estrategia#subtype#dirección#hora)
  - _Umbral_: n≥40 por celda + gate riguroso completo (Wilson+shuffle+PnL bootstrap)
  - _Acción_: Añadir claves 'ESTRATEGIA#SUBTYPE#DIRECCION#HORA':1.2 a meta.hora_boost_factor, solo por celda confirmada
  - _Estado_: 3 celda(s) pasan gate riguroso completo de 112 evaluadas (n>=40) y 440 trackeadas (n>=15). Detalle: kelly_hora_segmentado.json

**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  - _Umbral_: IC≥0.08 y n≥40
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: SOL#15min: n≥40 pero IC=-0.011 < 0.08 — monitorear
  - _Datos_: n=346 IC=-0.011 PNL=-11.06€
...
**🟡 H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  ...
  - _Estado_: tras_win IC=+0.086 n=64659 | tras_1loss IC=+0.046 n=49015 | tras_2loss IC=+0.008 n=22206/40 | gap=+0.078 (umbral 0.05)
...
**⏳ H-HORA-GBM** — hora_utc causal automático en GBM (forward)
  ...
  - _Estado_: 3819 ops, 22 horas distintas. Sin hora con n≥15 y IC extremo aún.
...
**⏳ H-BTC-LEADS-ETH** — ETH/SOL GBM contrario al drift_15min de BTC del mismo ciclo
  ...
  - _Estado_: alineado_BTC IC=-0.006 n=346 | contrario_BTC IC=+0.002 n=265/40 | gap=+0.008 (umbral 0.08) — SIN CONFIRMAR independencia de filtros propios de ETH
...
**〰️ H-CUSTOM-GBM-SIGMA-ALTO** — GBM con sigma_h alto (>0.002/h) — ¿destruye edge?
  ...
  - _Estado_: n=3644 IC=+0.007 PNL=+93.99€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=3644 IC=+0.007 PNL=+93.99€
...
**〰️ H-FUNDING-HIGH-BUYNO** — Funding rate alto (>p90 real ≈0.009%/8h) → BUY_NO tiene más edge
  ...
  - _Estado_: n=961 IC=-0.037 PNL=-52.52€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=961 IC=-0.037 PNL=-52.52€
...
**〰️ H-DVOL-SPIKE-BUYNO** — DVOL spike (sigma_h alto) → BUY_NO tiene más edge (panic regime)
  ...
  - _Estado_: n=1033 IC=+0.029 PNL=+75.71€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=1033 IC=+0.029 PNL=+75.71€
...
**〰️ H-CUSTOM-SMART-MONEY-CONSENSUS** — Consenso de wallets 'smart money' — ¿confirma nuestra dirección?
  ...
  - _Estado_: n=539 IC=+0.025 PNL=+33.23€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=539 IC=+0.025 PNL=+33.23€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=2678 IC=-0.114 PNL=+396.13€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=2678 IC=-0.114 PNL=+396.13€
...
**⏳ H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: 228/289 ops en el filtro definido (IC actual=-0.222 PNL=-11.88€)
  - _Datos_: n=228 IC=-0.222 PNL=-11.88€
...
**🟡 H-CUSTOM-SMARTMONEY-FAVORITO-SOL** — FAVORITO_CONFIRMADO SOL — alineado con smart_money_consensus bate ir en contra (REABRE hallazgo cerrado 08-Jul)
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.120 > 0.08 con n=198 PNL=-26.18€
  - _Datos_: n=198 IC=+0.120 PNL=-26.18€
...
**🟡 H-CUSTOM-FAVORITO-SOL-ALTACONVICCION** — FAVORITO_CONFIRMADO SOL BUY_YES alta conviccion (py_entrada alto) — UNICO caso positivo en fill-ability de hoy
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.246 > 0.08 con n=993 PNL=-93.68€
  - _Datos_: n=993 IC=+0.246 PNL=-93.68€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=1945 IC=+0.127 PNL=-363.65€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=1945 IC=+0.127 PNL=-363.65€
