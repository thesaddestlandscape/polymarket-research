# Hipótesis automáticas — 2026-09-26 19:04 UTC
_Generado por shadow_postmortem.py sobre 624211 resoluciones (PNL=+71413.38€)_
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.69` → IC=+0.206 (n=14314)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.69 (IC base=+0.102)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.153 (n=3600)

  - _Acción_: Kelly boost +0.77€ cuando `libro_spread` < 0.01 (IC base=+0.102)

- **PATRÓN** `libro_liquidez` > `5663.2952` → IC=+0.177 (n=2294)

  - _Acción_: Kelly boost +0.89€ cuando `libro_liquidez` > 5663.2952 (IC base=+0.102)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.138 (n=11906)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.35` → IC=+0.231 (n=11319)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` < 0.35 (IC base=+0.127)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.173 (n=5816)

  - _Acción_: Kelly boost +0.87€ cuando `libro_spread` < 0.01 (IC base=+0.127)

- **PATRÓN** `libro_liquidez` > `7848.8603` → IC=+0.176 (n=2202)
...
- **PATRÓN** `hora_utc` > `7.0` → IC=+0.206 (n=1571)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.205 (n=2229)
...
- **PATRÓN** `libro_liquidez` > `4614.6151` → IC=+0.142 (n=238)

  - _Acción_: Kelly boost +0.71€ cuando `libro_liquidez` > 4614.6151 (IC base=+0.099)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.155 (n=2911)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.250 (n=546)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.235 (n=1532)
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.233 (n=725)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.208)

- **PATRÓN** `py_entrada` > `0.86` → IC=+0.432 (n=658)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.202 (n=11977)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.225 (n=3924)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.168 (n=2886)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.73` → IC=+0.175 (n=2736)
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.246 (n=940)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.187 (n=2694)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.249 (n=2506)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.239)

- **PATRÓN** `py_entrada` > `0.77` → IC=+0.325 (n=798)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.199 (n=2754)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` < `0.71` → IC=+0.197 (n=2049)
...
- **PATRÓN** `hora_utc` > `8.0` → IC=+0.201 (n=35663)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 8.0 (IC base=+0.198)

- **PATRÓN** `py_entrada` > `0.75` → IC=+0.236 (n=15888)

  - _Acción_: Kelly boost +1.00€ cuando `py_entrada` > 0.75 (IC base=+0.198)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.177 (n=7238)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.191 (n=6666)

  - _Acción_: Kelly boost +0.95€ cuando `py_entrada` > 0.71 (IC base=+0.176)

### FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BTC#5min
- **PATRÓN** `hora_utc` > `6.0` → IC=+0.225 (n=6410)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `15.0` → IC=+0.230 (n=3212)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `hora_utc` > `17.0` → IC=+0.305 (n=219)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.274 (n=485)
...
- **PATRÓN** `sigma_h` > `0.0082` → IC=+0.175 (n=631)
  ...
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0082 (IC base=+0.163)

- **PATRÓN** `drift_60min` |x|≤ `0.3482` → IC=+0.168 (n=1893)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.3482 (IC base=+0.163)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.173 (n=910)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_spike_ratio` > `1.4361` → IC=+0.166 (n=1774)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` > 1.4361 (IC base=+0.163)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.178 (n=1932)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0896` → IC=+0.276 (n=484)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0896 (IC base=+0.237)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.248 (n=996)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.237)

- **PATRÓN** `ibs_20min` < `0.0517` → IC=+0.294 (n=638)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.0517 (IC base=+0.237)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.479` → IC=+0.246 (n=1508)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.240 (n=1602)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.237)

- **PATRÓN** `libro_liquidez` > `1814.8377` → IC=+0.237 (n=967)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1814.8377 (IC base=+0.237)
...
- **PATRÓN** `drift_60min` |x|≤ `0.0836` → IC=+0.240 (n=487)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0836 (IC base=+0.217)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.232 (n=1534)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9864` → IC=+0.269 (n=487)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9864 (IC base=+0.217)

- **PATRÓN** `dist_vwap_pct` > `0.1942` → IC=+0.219 (n=766)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1942 (IC base=+0.217)

- **PATRÓN** `dist_vwap_pct` < `0.5623` → IC=+0.221 (n=1531)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5623 (IC base=+0.217)
...
- **PATRÓN** `volumen_regimen` < `1.2505` → IC=+0.219 (n=1461)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.2505 (IC base=+0.217)

- **PATRÓN** `volumen_regimen` > `1.0786` → IC=+0.227 (n=662)
...
- **PATRÓN** `ibs_20min` < `0.7028` → IC=+0.170 (n=1518)

  - _Acción_: Kelly boost +0.85€ cuando `ibs_20min` < 0.7028 (IC base=+0.141)

- **PATRÓN** `dist_vwap_pct` < `0.1283` → IC=+0.156 (n=1360)
...
- **PATRÓN** `volumen_regimen` < `1.1954` → IC=+0.151 (n=1518)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 1.1954 (IC base=+0.141)

- **PATRÓN** `volumen_regimen` > `0.8505` → IC=+0.141 (n=1012)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_regimen` > 0.8505 (IC base=+0.141)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4301` → IC=+0.154 (n=1408)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.4301 (IC base=+0.141)

- **PATRÓN** `volumen_spike_ratio` > `1.4233` → IC=+0.148 (n=1407)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` > 1.4233 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `14075.3664` → IC=+0.145 (n=1012)

  - _Acción_: Kelly boost +0.72€ cuando `libro_liquidez` > 14075.3664 (IC base=+0.141)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.192 (n=1979)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `volumen_pendiente_norm` < `0.1008` → IC=+0.189 (n=1634)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` < 0.1008 (IC base=+0.185)

- **PATRÓN** `volumen_pendiente_norm` > `0.3561` → IC=+0.194 (n=253)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.3561 (IC base=+0.185)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.144 (n=102)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.027 (n=2293)
...
- **PATRÓN** `volumen_regimen` < `0.8475` → IC=+0.163 (n=588)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5242` → IC=+0.174 (n=740)
...
- **FILTRO** `hora_utc` < `17.0` → IC=-0.172 (n=62)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.086 (n=343)
...
- **FILTRO** `ibs_20min` > `0.25` → IC=-0.125 (n=2277)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.719` → IC=-0.210 (n=364)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.022 (n=3048)

- **PATRÓN** `ibs_20min` > `0.7647` → IC=+0.229 (n=138)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.7647 (IC base=+0.045)

- **PATRÓN** `dist_vwap_pct` > `1.0762` → IC=+0.289 (n=55)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.0762 (IC base=+0.045)

- **PATRÓN** `dist_vwap_pct` < `0.5952` → IC=+0.279 (n=84)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.5952 (IC base=+0.045)

- **PATRÓN** `volumen_regimen` > `0.7668` → IC=+0.298 (n=82)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.7668 (IC base=+0.045)

- **PATRÓN** `volumen_spike_ratio` < `2.5444` → IC=+0.282 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.5444 (IC base=+0.045)

- **PATRÓN** `volumen_spike_ratio` > `1.4528` → IC=+0.266 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4528 (IC base=+0.045)

- **PATRÓN** `ballena_activa_n` < `49.0` → IC=+0.297 (n=121)
...
- **FILTRO** `drift_60min` |x|> `0.6536` → IC=-0.185 (n=594)
  - _Por qué funciona_: drift fuerte en 1h → el movimiento ya está priceado en Polymarket; edge agotado
  - _Acción_: SKIP cuando `drift_60min` |x|> 0.6536
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=1795)

- **FILTRO** `ibs_20min` < `0.7191` → IC=-0.155 (n=1576)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7191
  - _Potencial_: sin este filtro IC_bueno=+0.101 (n=813)

- **FILTRO** `libro_spread` > `0.02` → IC=-0.200 (n=461)

  - _Acción_: SKIP cuando `libro_spread` > 0.02
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=1928)

- **FILTRO** `ibs_20min` > `0.7692` → IC=-0.205 (n=875)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7692
  - _Potencial_: sin este filtro IC_bueno=+0.041 (n=2629)
...
- **PATRÓN** `volumen_regimen` < `0.7344` → IC=+0.256 (n=362)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7344 (IC base=-0.021)

- **PATRÓN** `volumen_regimen` > `1.2485` → IC=+0.286 (n=274)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2485 (IC base=-0.021)

- **PATRÓN** `volumen_pendiente_norm` > `0.2422` → IC=+0.285 (n=142)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2422 (IC base=-0.021)

- **PATRÓN** `volumen_spike_ratio` < `2.1392` → IC=+0.261 (n=625)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1392 (IC base=-0.021)

- **PATRÓN** `volumen_spike_ratio` > `1.5236` → IC=+0.250 (n=634)
...
- **PATRÓN** `ibs_20min` > `0.4754` → IC=+0.187 (n=9530)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.616` → IC=+0.156 (n=4989)

  - _Acción_: Kelly boost +0.78€ cuando `sigma_ewma_delta_pct` > 3.616 (IC base=+0.097)

- **PATRÓN** `volumen_regimen` > `0.6901` → IC=+0.250 (n=3401)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4634` → IC=+0.236 (n=2059)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4634 (IC base=+0.097)

- **PATRÓN** `volumen_spike_ratio` > `2.6456` → IC=+0.247 (n=2059)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6456 (IC base=+0.097)

- **PATRÓN** `ballena_activa_n` < `95.0` → IC=+0.269 (n=5693)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 95.0 (IC base=+0.097)

- **PATRÓN** `sigma_h` > `0.0091` → IC=+0.159 (n=3526)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.79€ cuando `sigma_h` > 0.0091 (IC base=+0.074)

- **PATRÓN** `ibs_20min` < `0.5493` → IC=+0.155 (n=9293)

  - _Acción_: Kelly boost +0.77€ cuando `ibs_20min` < 0.5493 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` > `0.7124` → IC=+0.242 (n=664)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.7124 (IC base=+0.074)

- **PATRÓN** `dist_vwap_pct` < `0.2496` → IC=+0.242 (n=2987)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.2496 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` < `0.7084` → IC=+0.242 (n=1383)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.7084 (IC base=+0.074)

- **PATRÓN** `volumen_regimen` > `1.2049` → IC=+0.250 (n=1047)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.2049 (IC base=+0.074)

- **PATRÓN** `volumen_pendiente_norm` > `0.2422` → IC=+0.304 (n=791)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2422 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` < `1.6001` → IC=+0.261 (n=1838)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.6001 (IC base=+0.074)

- **PATRÓN** `volumen_spike_ratio` > `2.2951` → IC=+0.261 (n=1894)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.2951 (IC base=+0.074)

- **PATRÓN** `ballena_activa_n` < `84.0` → IC=+0.268 (n=4058)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 84.0 (IC base=+0.074)

### GBM_LATE_15M_ESPACIO_ATR#BNB#15min
- **FILTRO** `ibs_20min` < `0.2548` → IC=-0.153 (n=735)

  - _Acción_: SKIP cuando `ibs_20min` < 0.2548
  - _Potencial_: sin este filtro IC_bueno=+0.106 (n=2206)

- **FILTRO** `sigma_ewma_delta_pct` > `4.548` → IC=-0.160 (n=560)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.548
  - _Potencial_: sin este filtro IC_bueno=+0.024 (n=1874)

- **PATRÓN** `ibs_20min` > `0.8961` → IC=+0.266 (n=736)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4344` → IC=+0.177 (n=314)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` < 1.4344 (IC base=+0.042)

- **PATRÓN** `volumen_spike_ratio` > `2.1445` → IC=+0.205 (n=425)
...
- **PATRÓN** `ibs_20min` > `0.8658` → IC=+0.163 (n=714)

  - _Acción_: Kelly boost +0.82€ cuando `ibs_20min` > 0.8658 (IC base=+0.026)
...
- **PATRÓN** `sigma_h` > `0.01` → IC=+0.313 (n=612)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.01 (IC base=+0.282)

- **PATRÓN** `drift_60min` |x|≤ `0.613` → IC=+0.285 (n=1349)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.613 (IC base=+0.282)

- **PATRÓN** `hora_utc` > `18.0` → IC=+0.324 (n=453)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 18.0 (IC base=+0.282)

- **PATRÓN** `ibs_20min` < `0.228` → IC=+0.287 (n=1187)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.228 (IC base=+0.282)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.816` → IC=+0.296 (n=532)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.816 (IC base=+0.282)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.75` → IC=+0.282 (n=1447)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.75 (IC base=+0.282)

- **PATRÓN** `volumen_pendiente_norm` > `0.3358` → IC=+0.304 (n=207)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3358 (IC base=+0.282)

- **PATRÓN** `volumen_spike_ratio` < `1.7408` → IC=+0.287 (n=551)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7408 (IC base=+0.282)

- **PATRÓN** `volumen_spike_ratio` > `2.6975` → IC=+0.288 (n=568)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.6975 (IC base=+0.282)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.286 (n=887)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.02 (IC base=+0.282)

- **PATRÓN** `libro_liquidez` > `1902.41` → IC=+0.295 (n=612)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1902.41 (IC base=+0.282)

- **PATRÓN** `ballena_activa_n` < `27.0` → IC=+0.287 (n=809)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 27.0 (IC base=+0.282)

### GBM_LATE_15M_ESPACIO_ATR#ETH#15min
- **FILTRO** `ibs_20min` < `0.297` → IC=-0.174 (n=528)

  - _Acción_: SKIP cuando `ibs_20min` < 0.297
  - _Potencial_: sin este filtro IC_bueno=+0.077 (n=1586)

- **FILTRO** `ibs_20min` > `0.7735` → IC=-0.185 (n=627)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7735
  - _Potencial_: sin este filtro IC_bueno=+0.055 (n=1883)

- **PATRÓN** `ibs_20min` > `0.9105` → IC=+0.182 (n=529)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` > 0.9105 (IC base=+0.014)

- **PATRÓN** `dist_vwap_pct` > `0.4352` → IC=+0.221 (n=270)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4352 (IC base=+0.014)

- **PATRÓN** `dist_vwap_pct` < `0.6708` → IC=+0.218 (n=629)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.6708 (IC base=+0.014)

- **PATRÓN** `volumen_regimen` < `0.9865` → IC=+0.243 (n=539)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.9865 (IC base=+0.014)

- **PATRÓN** `volumen_regimen` > `0.5902` → IC=+0.218 (n=612)
...
- **PATRÓN** `volumen_spike_ratio` > `1.7472` → IC=+0.233 (n=387)
...
- **FILTRO** `ibs_20min` < `0.7333` → IC=-0.194 (n=1129)

  - _Acción_: SKIP cuando `ibs_20min` < 0.7333
  - _Potencial_: sin este filtro IC_bueno=+0.280 (n=1129)

- **FILTRO** `ibs_20min` > `0.6857` → IC=-0.235 (n=579)

  - _Acción_: SKIP cuando `ibs_20min` > 0.6857
  - _Potencial_: sin este filtro IC_bueno=+0.097 (n=1739)

- **FILTRO** `sigma_ewma_delta_pct` > `4.714` → IC=-0.188 (n=507)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 4.714
  - _Potencial_: sin este filtro IC_bueno=+0.071 (n=1811)

- **PATRÓN** `ibs_20min` > `0.7333` → IC=+0.280 (n=1129)
...
- **PATRÓN** `ballena_activa_n` < `56.0` → IC=+0.315 (n=717)
...
- **PATRÓN** `libro_liquidez` > `2631.2954` → IC=+0.290 (n=914)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2631.2954 (IC base=+0.278)

- **PATRÓN** `sigma_h` > `0.0154` → IC=+0.303 (n=983)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` > `18.0` → IC=+0.287 (n=504)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.14` → IC=+0.334 (n=985)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.14 (IC base=+0.276)

- **PATRÓN** `dist_vwap_pct` > `1.3409` → IC=+0.284 (n=234)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.3409 (IC base=+0.276)

- **PATRÓN** `dist_vwap_pct` < `0.2288` → IC=+0.277 (n=1344)
...
- **PATRÓN** `volumen_regimen` > `1.244` → IC=+0.309 (n=491)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1415` → IC=+0.276 (n=592)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.1415 (IC base=+0.276)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.181 (n=8569)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 5.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` > `0.5745` → IC=+0.218 (n=8214)
...
- **PATRÓN** `volumen_regimen` < `1.2076` → IC=+0.160 (n=5443)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_regimen` < 1.2076 (IC base=+0.168)

- **PATRÓN** `volumen_regimen` > `0.6301` → IC=+0.161 (n=5444)
...
- **PATRÓN** `libro_liquidez` > `1952.5984` → IC=+0.169 (n=7338)

  - _Acción_: Kelly boost +0.84€ cuando `libro_liquidez` > 1952.5984 (IC base=+0.168)

- **PATRÓN** `ballena_activa_n` < `112.0` → IC=+0.181 (n=7125)

  - _Acción_: Kelly boost +0.91€ cuando `ballena_activa_n` < 112.0 (IC base=+0.168)

- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.184 (n=5258)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.92€ cuando `sigma_h` < 0.0066 (IC base=+0.169)

- **PATRÓN** `drift_60min` |x|≤ `0.0811` → IC=+0.211 (n=2624)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0811 (IC base=+0.169)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.207 (n=3031)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.4814` → IC=+0.227 (n=7871)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4814 (IC base=+0.169)

- **PATRÓN** `dist_vwap_pct` < `0.2343` → IC=+0.160 (n=5706)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` < 0.2343 (IC base=+0.169)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.341` → IC=+0.193 (n=1330)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 10.341 (IC base=+0.169)

- **PATRÓN** `volumen_regimen` < `1.1727` → IC=+0.152 (n=5680)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 1.1727 (IC base=+0.169)

- **PATRÓN** `volumen_pendiente_norm` > `0.2904` → IC=+0.215 (n=1139)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2904 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` < `1.5588` → IC=+0.167 (n=3161)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 1.5588 (IC base=+0.169)

- **PATRÓN** `volumen_spike_ratio` > `2.248` → IC=+0.171 (n=3257)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 2.248 (IC base=+0.169)

- **PATRÓN** `ballena_activa_n` < `113.0` → IC=+0.175 (n=6836)
...
- **PATRÓN** `sigma_h` < `0.0066` → IC=+0.246 (n=911)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.1848` → IC=+0.292 (n=690)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1848 (IC base=+0.240)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.244 (n=933)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.3427` → IC=+0.265 (n=1035)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3427 (IC base=+0.240)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.34` → IC=+0.251 (n=1123)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.34 (IC base=+0.240)

- **PATRÓN** `volumen_pendiente_norm` < `0.096` → IC=+0.236 (n=866)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.096 (IC base=+0.240)

- **PATRÓN** `volumen_pendiente_norm` > `0.2828` → IC=+0.250 (n=150)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2828 (IC base=+0.240)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.242 (n=1144)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.240)

- **PATRÓN** `libro_liquidez` > `1817.84` → IC=+0.246 (n=690)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1817.84 (IC base=+0.240)
...
- **PATRÓN** `volumen_regimen` < `1.195` → IC=+0.159 (n=1318)
...
- **PATRÓN** `volumen_spike_ratio` < `2.442` → IC=+0.148 (n=1207)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_spike_ratio` < 2.442 (IC base=+0.138)

- **PATRÓN** `volumen_spike_ratio` > `1.4236` → IC=+0.137 (n=1206)
...
- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.155 (n=1379)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.77€ cuando `sigma_h` < 0.0072 (IC base=+0.123)

- **PATRÓN** `drift_60min` |x|≤ `0.3825` → IC=+0.144 (n=1379)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.72€ cuando `drift_60min` |x|≤ 0.3825 (IC base=+0.123)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.181 (n=537)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.90€ cuando `hora_utc` > 17.0 (IC base=+0.123)

- **PATRÓN** `ibs_20min` < `0.6462` → IC=+0.173 (n=1379)

  - _Acción_: Kelly boost +0.86€ cuando `ibs_20min` < 0.6462 (IC base=+0.123)

- **PATRÓN** `dist_vwap_pct` < `0.3606` → IC=+0.135 (n=1492)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.3606 (IC base=+0.123)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.965` → IC=+0.161 (n=490)

  - _Acción_: Kelly boost +0.80€ cuando `sigma_ewma_delta_pct` > 6.965 (IC base=+0.123)

- **PATRÓN** `volumen_regimen` < `0.849` → IC=+0.150 (n=920)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 0.849 (IC base=+0.123)

- **PATRÓN** `volumen_pendiente_norm` > `0.2919` → IC=+0.188 (n=203)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_pendiente_norm` > 0.2919 (IC base=+0.123)

- **PATRÓN** `volumen_spike_ratio` < `1.8007` → IC=+0.139 (n=838)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.156 (n=666)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.129 (n=1534)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` > 5.0 (IC base=+0.114)

- **PATRÓN** `ibs_20min` < `0.5745` → IC=+0.211 (n=1512)
...
- **PATRÓN** `dist_vwap_pct` < `0.2057` → IC=+0.139 (n=1383)

  - _Acción_: Kelly boost +0.69€ cuando `dist_vwap_pct` < 0.2057 (IC base=+0.114)

- **PATRÓN** `sigma_ewma_delta_pct` > `7.609` → IC=+0.135 (n=313)

  - _Acción_: Kelly boost +0.67€ cuando `sigma_ewma_delta_pct` > 7.609 (IC base=+0.114)

- **PATRÓN** `volumen_regimen` < `0.6365` → IC=+0.144 (n=504)
...
- **PATRÓN** `sigma_h` > `0.0173` → IC=+0.210 (n=984)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0173 (IC base=+0.206)

- **PATRÓN** `drift_60min` |x|≤ `0.0922` → IC=+0.222 (n=491)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.0922 (IC base=+0.206)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.224 (n=723)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.438` → IC=+0.247 (n=1473)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.442` → IC=+0.246 (n=285)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.442 (IC base=+0.206)

- **PATRÓN** `volumen_regimen` > `0.6331` → IC=+0.216 (n=1473)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6331 (IC base=+0.206)

- **PATRÓN** `volumen_pendiente_norm` > `0.2814` → IC=+0.293 (n=196)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2814 (IC base=+0.206)

- **PATRÓN** `volumen_spike_ratio` < `2.1921` → IC=+0.196 (n=1170)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` < 2.1921 (IC base=+0.206)

- **PATRÓN** `volumen_spike_ratio` > `1.4272` → IC=+0.201 (n=1329)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4272 (IC base=+0.206)

- **PATRÓN** `libro_liquidez` > `2594.5798` → IC=+0.208 (n=982)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2594.5798 (IC base=+0.206)

### GBM_LATE_15M_PYCONFIRMADO
- **PATRÓN** `sigma_h` < `0.0038` → IC=+0.175 (n=671)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.3506` → IC=+0.155 (n=1771)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.78€ cuando `drift_60min` |x|≤ 0.3506 (IC base=+0.151)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.169 (n=1851)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.85€ cuando `hora_utc` > 8.0 (IC base=+0.151)

- **PATRÓN** `ibs_20min` > `0.5385` → IC=+0.190 (n=1799)

  - _Acción_: Kelly boost +0.95€ cuando `ibs_20min` > 0.5385 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` > `0.8519` → IC=+0.176 (n=331)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 0.8519 (IC base=+0.151)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.726` → IC=+0.180 (n=905)

  - _Acción_: Kelly boost +0.90€ cuando `sigma_ewma_delta_pct` > 3.726 (IC base=+0.151)

- **PATRÓN** `volumen_regimen` < `0.871` → IC=+0.170 (n=1186)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1641` → IC=+0.180 (n=555)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` > 0.1641 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` < `1.4413` → IC=+0.162 (n=648)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` < 1.4413 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` > `1.8245` → IC=+0.159 (n=1295)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 1.8245 (IC base=+0.151)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.156 (n=2279)

  - _Acción_: Kelly boost +0.78€ cuando `libro_spread` < 0.02 (IC base=+0.151)

- **PATRÓN** `libro_liquidez` > `12367.2302` → IC=+0.157 (n=671)

  - _Acción_: Kelly boost +0.78€ cuando `libro_liquidez` > 12367.2302 (IC base=+0.151)

- **PATRÓN** `ballena_activa_n` < `151.0` → IC=+0.171 (n=1790)

  - _Acción_: Kelly boost +0.86€ cuando `ballena_activa_n` < 151.0 (IC base=+0.151)

- **PATRÓN** `sigma_h` < `0.0056` → IC=+0.138 (n=1378)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.69€ cuando `sigma_h` < 0.0056 (IC base=+0.111)

- **PATRÓN** `drift_60min` |x|≤ `0.3435` → IC=+0.127 (n=1817)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.63€ cuando `drift_60min` |x|≤ 0.3435 (IC base=+0.111)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.125 (n=2082)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.62€ cuando `hora_utc` > 5.0 (IC base=+0.111)

- **PATRÓN** `ibs_20min` < `0.6667` → IC=+0.138 (n=2067)

  - _Acción_: Kelly boost +0.69€ cuando `ibs_20min` < 0.6667 (IC base=+0.111)

- **PATRÓN** `dist_vwap_pct` < `0.2096` → IC=+0.121 (n=1847)

  - _Acción_: Kelly boost +0.61€ cuando `dist_vwap_pct` < 0.2096 (IC base=+0.111)

- **PATRÓN** `volumen_regimen` < `0.6989` → IC=+0.129 (n=821)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_regimen` < 0.6989 (IC base=+0.111)

- **PATRÓN** `volumen_pendiente_norm` > `0.1654` → IC=+0.130 (n=517)

  - _Acción_: Kelly boost +0.65€ cuando `volumen_pendiente_norm` > 0.1654 (IC base=+0.111)

- **PATRÓN** `volumen_spike_ratio` < `1.4499` → IC=+0.143 (n=664)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.4499 (IC base=+0.111)

- **PATRÓN** `libro_liquidez` > `3915.7226` → IC=+0.127 (n=1377)

  - _Acción_: Kelly boost +0.64€ cuando `libro_liquidez` > 3915.7226 (IC base=+0.111)

- **PATRÓN** `ballena_activa_n` < `29.0` → IC=+0.126 (n=854)

  - _Acción_: Kelly boost +0.63€ cuando `ballena_activa_n` < 29.0 (IC base=+0.111)

### GBM_LATE_15M_PYCONFIRMADO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0039` → IC=+0.131 (n=331)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.65€ cuando `sigma_h` < 0.0039 (IC base=+0.112)

- **PATRÓN** `drift_60min` |x|≤ `0.3377` → IC=+0.131 (n=494)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.66€ cuando `drift_60min` |x|≤ 0.3377 (IC base=+0.112)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.156 (n=463)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.78€ cuando `hora_utc` > 8.0 (IC base=+0.112)

- **PATRÓN** `ibs_20min` > `0.2572` → IC=+0.155 (n=494)

  - _Acción_: Kelly boost +0.78€ cuando `ibs_20min` > 0.2572 (IC base=+0.112)

- **PATRÓN** `dist_vwap_pct` > `0.2999` → IC=+0.148 (n=174)

  - _Acción_: Kelly boost +0.74€ cuando `dist_vwap_pct` > 0.2999 (IC base=+0.112)

- **PATRÓN** `volumen_regimen` < `0.6182` → IC=+0.159 (n=165)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_regimen` < 0.6182 (IC base=+0.112)

- **PATRÓN** `volumen_pendiente_norm` > `0.0904` → IC=+0.131 (n=177)

  - _Acción_: Kelly boost +0.66€ cuando `volumen_pendiente_norm` > 0.0904 (IC base=+0.112)

- **PATRÓN** `libro_liquidez` > `10925.7104` → IC=+0.133 (n=494)

  - _Acción_: Kelly boost +0.67€ cuando `libro_liquidez` > 10925.7104 (IC base=+0.112)

- **PATRÓN** `ballena_activa_n` < `147.0` → IC=+0.141 (n=157)

  - _Acción_: Kelly boost +0.71€ cuando `ballena_activa_n` < 147.0 (IC base=+0.112)
...
- **PATRÓN** `sigma_h` < `0.0037` → IC=+0.265 (n=279)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.2111` → IC=+0.214 (n=421)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2111 (IC base=+0.200)

- **PATRÓN** `hora_utc` > `5.0` → IC=+0.215 (n=658)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.200)

- **PATRÓN** `ibs_20min` > `0.9681` → IC=+0.264 (n=210)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9681 (IC base=+0.200)

- **PATRÓN** `dist_vwap_pct` > `0.1455` → IC=+0.205 (n=324)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1455 (IC base=+0.200)

- **PATRÓN** `dist_vwap_pct` < `0.218` → IC=+0.201 (n=556)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.218 (IC base=+0.200)
...
- **PATRÓN** `volumen_regimen` < `1.0063` → IC=+0.204 (n=555)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 1.0063 (IC base=+0.200)

- **PATRÓN** `volumen_regimen` > `0.6082` → IC=+0.206 (n=630)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 0.6082 (IC base=+0.200)

- **PATRÓN** `volumen_pendiente_norm` > `0.2624` → IC=+0.263 (n=91)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2624 (IC base=+0.200)

- **PATRÓN** `volumen_spike_ratio` < `1.4003` → IC=+0.224 (n=208)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.4003 (IC base=+0.200)

- **PATRÓN** `volumen_spike_ratio` > `1.7512` → IC=+0.227 (n=415)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.7512 (IC base=+0.200)
...
- **PATRÓN** `libro_liquidez` > `12451.6072` → IC=+0.222 (n=210)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 12451.6072 (IC base=+0.200)

- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.121 (n=523)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.60€ cuando `sigma_h` < 0.0062 (IC base=+0.095)

- **PATRÓN** `drift_60min` |x|≤ `0.3517` → IC=+0.121 (n=521)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.61€ cuando `drift_60min` |x|≤ 0.3517 (IC base=+0.095)

- **PATRÓN** `ibs_20min` < `0.0837` → IC=+0.160 (n=198)

  - _Acción_: Kelly boost +0.80€ cuando `ibs_20min` < 0.0837 (IC base=+0.095)

- **PATRÓN** `volumen_regimen` < `0.6868` → IC=+0.139 (n=261)

  - _Acción_: Kelly boost +0.69€ cuando `volumen_regimen` < 0.6868 (IC base=+0.095)

- **PATRÓN** `volumen_pendiente_norm` > `0.1664` → IC=+0.136 (n=141)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_pendiente_norm` > 0.1664 (IC base=+0.095)

### GBM_LATE_15M_PYCONFIRMADO#SOL#15min
- **FILTRO** `dist_vwap_pct` > `0.6009` → IC=-0.147 (n=15)

  - _Acción_: SKIP cuando `dist_vwap_pct` > 0.6009
...
- **PATRÓN** `drift_60min` |x|≤ `0.5425` → IC=+0.135 (n=477)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.67€ cuando `drift_60min` |x|≤ 0.5425 (IC base=+0.133)

- **PATRÓN** `hora_utc` > `8.0` → IC=+0.170 (n=443)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `sigma_ewma_delta_pct` > `3.412` → IC=+0.198 (n=256)
...
- **PATRÓN** `volumen_regimen` > `0.7281` → IC=+0.147 (n=426)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4824` → IC=+0.143 (n=152)

  - _Acción_: Kelly boost +0.71€ cuando `volumen_spike_ratio` < 1.4824 (IC base=+0.133)

- **PATRÓN** `volumen_spike_ratio` > `2.2017` → IC=+0.184 (n=207)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` > 2.2017 (IC base=+0.133)
...
- **PATRÓN** `ibs_20min` < `0.5172` → IC=+0.151 (n=422)

  - _Acción_: Kelly boost +0.75€ cuando `ibs_20min` < 0.5172 (IC base=+0.083)

- **PATRÓN** `volumen_regimen` < `0.708` → IC=+0.144 (n=186)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_regimen` < 0.708 (IC base=+0.083)

- **PATRÓN** `volumen_spike_ratio` < `1.5787` → IC=+0.172 (n=175)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` < 1.5787 (IC base=+0.083)

- **PATRÓN** `libro_liquidez` > `2590.446` → IC=+0.129 (n=281)

  - _Acción_: Kelly boost +0.64€ cuando `libro_liquidez` > 2590.446 (IC base=+0.083)

- **PATRÓN** `ballena_activa_n` < `42.0` → IC=+0.132 (n=370)

  - _Acción_: Kelly boost +0.66€ cuando `ballena_activa_n` < 42.0 (IC base=+0.083)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.181 (n=11019)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` > 5.0 (IC base=+0.170)

- **PATRÓN** `ibs_20min` > `1.0` → IC=+0.308 (n=3548)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.345` → IC=+0.244 (n=2652)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.345 (IC base=+0.170)

- **PATRÓN** `volumen_regimen` < `0.88` → IC=+0.166 (n=4706)
...
- **PATRÓN** `volumen_spike_ratio` > `2.5872` → IC=+0.189 (n=3385)

  - _Acción_: Kelly boost +0.94€ cuando `volumen_spike_ratio` > 2.5872 (IC base=+0.170)

- **PATRÓN** `libro_liquidez` > `1794.786` → IC=+0.172 (n=10551)

  - _Acción_: Kelly boost +0.86€ cuando `libro_liquidez` > 1794.786 (IC base=+0.170)

- **PATRÓN** `ballena_activa_n` < `85.0` → IC=+0.196 (n=8103)

  - _Acción_: Kelly boost +0.98€ cuando `ballena_activa_n` < 85.0 (IC base=+0.170)

- **PATRÓN** `sigma_h` < `0.007` → IC=+0.193 (n=6384)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.96€ cuando `sigma_h` < 0.007 (IC base=+0.182)

- **PATRÓN** `drift_60min` |x|≤ `0.1487` → IC=+0.190 (n=4211)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.95€ cuando `drift_60min` |x|≤ 0.1487 (IC base=+0.182)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.208 (n=3616)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `drift_60min` |x|≤ `0.3528` → IC=+0.191 (n=1777)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.211 (n=1798)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.191)

- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.261 (n=945)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `drift_60min` |x|≤ `0.1263` → IC=+0.283 (n=621)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1263 (IC base=+0.259)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.268 (n=1280)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.259)

- **PATRÓN** `ibs_20min` < `0.3509` → IC=+0.286 (n=1241)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3509 (IC base=+0.259)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.483` → IC=+0.263 (n=1479)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.262 (n=1559)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.259)

- **PATRÓN** `libro_liquidez` > `1813.3286` → IC=+0.264 (n=940)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1813.3286 (IC base=+0.259)

### GBM_LATE_15M_TARDIO#BTC#15min
- **PATRÓN** `sigma_h` < `0.0028` → IC=+0.198 (n=568)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.165 (n=1768)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.82€ cuando `hora_utc` > 5.0 (IC base=+0.151)

- **PATRÓN** `ibs_20min` > `0.3081` → IC=+0.203 (n=1689)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.3081 (IC base=+0.151)

- **PATRÓN** `dist_vwap_pct` > `0.1282` → IC=+0.184 (n=948)

  - _Acción_: Kelly boost +0.92€ cuando `dist_vwap_pct` > 0.1282 (IC base=+0.151)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `4.2` → IC=+0.153 (n=1526)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1137` → IC=+0.160 (n=1438)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` < 2.1137 (IC base=+0.151)

- **PATRÓN** `volumen_spike_ratio` > `1.4033` → IC=+0.157 (n=1634)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_spike_ratio` > 1.4033 (IC base=+0.151)

- **PATRÓN** `libro_liquidez` > `11295.2956` → IC=+0.161 (n=1509)
...
- **PATRÓN** `sigma_h` < `0.0026` → IC=+0.189 (n=490)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.95€ cuando `sigma_h` < 0.0026 (IC base=+0.152)

- **PATRÓN** `drift_60min` |x|≤ `0.2565` → IC=+0.167 (n=1288)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.84€ cuando `drift_60min` |x|≤ 0.2565 (IC base=+0.152)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.185 (n=570)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.2772` → IC=+0.234 (n=976)
...
- **PATRÓN** `dist_vwap_pct` < `0.1318` → IC=+0.166 (n=1329)

  - _Acción_: Kelly boost +0.83€ cuando `dist_vwap_pct` < 0.1318 (IC base=+0.152)
...
- **PATRÓN** `volumen_regimen` < `1.1847` → IC=+0.164 (n=1463)
...
- **PATRÓN** `volumen_spike_ratio` < `2.4038` → IC=+0.161 (n=1365)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.227 (n=1801)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 5.0 (IC base=+0.217)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `7.811` → IC=+0.295 (n=500)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 7.811 (IC base=+0.217)

- **PATRÓN** `volumen_pendiente_norm` < `0.1345` → IC=+0.219 (n=1564)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1345 (IC base=+0.217)

- **PATRÓN** `volumen_spike_ratio` > `1.7707` → IC=+0.228 (n=1466)
...
- **PATRÓN** `libro_liquidez` > `1914.6432` → IC=+0.220 (n=780)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 1914.6432 (IC base=+0.217)

- **PATRÓN** `sigma_h` < `0.0118` → IC=+0.239 (n=1604)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0118 (IC base=+0.232)

- **PATRÓN** `drift_60min` |x|≤ `0.1712` → IC=+0.237 (n=706)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1712 (IC base=+0.232)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.259 (n=608)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 17.0 (IC base=+0.232)
...
- **PATRÓN** `hora_utc` > `5.0` → IC=+0.134 (n=1845)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 5.0 (IC base=+0.115)

- **PATRÓN** `ibs_20min` > `0.4706` → IC=+0.191 (n=1792)
...
- **PATRÓN** `dist_vwap_pct` < `0.9236` → IC=+0.207 (n=2119)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.9236 (IC base=+0.204)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.853` → IC=+0.261 (n=262)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.853 (IC base=+0.204)

- **PATRÓN** `volumen_regimen` > `1.2349` → IC=+0.241 (n=634)
...
- **PATRÓN** `volumen_spike_ratio` < `2.1798` → IC=+0.200 (n=1511)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 2.1798 (IC base=+0.204)

- **PATRÓN** `volumen_spike_ratio` > `1.4288` → IC=+0.201 (n=1716)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.206 (n=15)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.149 (n=3207)

- **PATRÓN** `sigma_h` < `0.009` → IC=+0.171 (n=2715)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.86€ cuando `sigma_h` < 0.009 (IC base=+0.161)

- **PATRÓN** `drift_60min` |x|≤ `0.5152` → IC=+0.171 (n=3083)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.85€ cuando `drift_60min` |x|≤ 0.5152 (IC base=+0.161)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.173 (n=1218)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` > `0.9412` → IC=+0.218 (n=1029)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` > 0.9412 (IC base=+0.161)

- **PATRÓN** `dist_vwap_pct` > `0.1795` → IC=+0.172 (n=1151)

  - _Acción_: Kelly boost +0.86€ cuando `dist_vwap_pct` > 0.1795 (IC base=+0.161)

- **PATRÓN** `dist_vwap_pct` < `0.4689` → IC=+0.161 (n=1942)

  - _Acción_: Kelly boost +0.80€ cuando `dist_vwap_pct` < 0.4689 (IC base=+0.161)

- **PATRÓN** `sigma_ewma_delta_pct` > `10.2` → IC=+0.195 (n=506)

  - _Acción_: Kelly boost +0.97€ cuando `sigma_ewma_delta_pct` > 10.2 (IC base=+0.161)

- **PATRÓN** `volumen_regimen` > `0.6307` → IC=+0.163 (n=2072)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4563` → IC=+0.169 (n=1016)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 1.4563 (IC base=+0.161)

- **PATRÓN** `volumen_spike_ratio` > `1.8746` → IC=+0.169 (n=2030)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 1.8746 (IC base=+0.161)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.161 (n=3436)

  - _Acción_: Kelly boost +0.81€ cuando `libro_spread` < 0.02 (IC base=+0.161)

- **PATRÓN** `libro_liquidez` > `3963.7863` → IC=+0.170 (n=2055)
...
- **PATRÓN** `drift_60min` |x|≤ `0.4845` → IC=+0.166 (n=2417)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.83€ cuando `drift_60min` |x|≤ 0.4845 (IC base=+0.147)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.180 (n=879)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `dist_vwap_pct` > `0.6719` → IC=+0.171 (n=469)

  - _Acción_: Kelly boost +0.85€ cuando `dist_vwap_pct` > 0.6719 (IC base=+0.147)

- **PATRÓN** `sigma_ewma_delta_pct` < `6.241` → IC=+0.158 (n=2410)

  - _Acción_: Kelly boost +0.79€ cuando `sigma_ewma_delta_pct` < 6.241 (IC base=+0.147)

- **PATRÓN** `volumen_regimen` < `1.2441` → IC=+0.153 (n=2314)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_regimen` < 1.2441 (IC base=+0.147)

- **PATRÓN** `volumen_pendiente_norm` < `0.0966` → IC=+0.151 (n=2191)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8181` → IC=+0.150 (n=1591)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.8181 (IC base=+0.147)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.149 (n=3207)

  - _Acción_: Kelly boost +0.74€ cuando `libro_spread` < 0.01 (IC base=+0.147)

- **PATRÓN** `libro_liquidez` > `7064.3454` → IC=+0.158 (n=2159)

  - _Acción_: Kelly boost +0.79€ cuando `libro_liquidez` > 7064.3454 (IC base=+0.147)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3801` → IC=+0.177 (n=914)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.88€ cuando `drift_60min` |x|≤ 0.3801 (IC base=+0.171)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.186 (n=396)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
...
- **PATRÓN** `ibs_20min` < `0.5342` → IC=+0.187 (n=692)

  - _Acción_: Kelly boost +0.94€ cuando `ibs_20min` < 0.5342 (IC base=+0.171)

- **PATRÓN** `ibs_20min` > `0.8839` → IC=+0.181 (n=346)

  - _Acción_: Kelly boost +0.91€ cuando `ibs_20min` > 0.8839 (IC base=+0.171)

- **PATRÓN** `dist_vwap_pct` < `0.2173` → IC=+0.179 (n=857)

  - _Acción_: Kelly boost +0.89€ cuando `dist_vwap_pct` < 0.2173 (IC base=+0.171)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.178` → IC=+0.182 (n=928)

  - _Acción_: Kelly boost +0.91€ cuando `sigma_ewma_delta_pct` < 4.178 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` < `1.0845` → IC=+0.172 (n=914)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_regimen` < 1.0845 (IC base=+0.171)

- **PATRÓN** `volumen_regimen` > `0.6413` → IC=+0.175 (n=1037)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 0.6413 (IC base=+0.171)

- **PATRÓN** `volumen_pendiente_norm` > `0.1655` → IC=+0.191 (n=312)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.1655 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` < `2.4706` → IC=+0.175 (n=1018)

  - _Acción_: Kelly boost +0.88€ cuando `volumen_spike_ratio` < 2.4706 (IC base=+0.171)

- **PATRÓN** `volumen_spike_ratio` > `1.5228` → IC=+0.174 (n=911)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_spike_ratio` > 1.5228 (IC base=+0.171)
...
- **PATRÓN** `drift_60min` |x|≤ `0.6706` → IC=+0.157 (n=243)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.6706 (IC base=+0.137)

- **PATRÓN** `hora_utc` < `10.0` → IC=+0.169 (n=164)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.84€ cuando `hora_utc` < 10.0 (IC base=+0.137)

- **PATRÓN** `ibs_20min` < `0.1364` → IC=+0.235 (n=81)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1364 (IC base=+0.137)

- **PATRÓN** `dist_vwap_pct` > `0.5982` → IC=+0.220 (n=116)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.5982 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.222` → IC=+0.142 (n=118)

  - _Acción_: Kelly boost +0.71€ cuando `sigma_ewma_delta_pct` > 3.222 (IC base=+0.137)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.18` → IC=+0.147 (n=233)

  - _Acción_: Kelly boost +0.73€ cuando `sigma_ewma_delta_pct` < 5.18 (IC base=+0.137)

- **PATRÓN** `volumen_regimen` < `0.9493` → IC=+0.152 (n=162)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` < 0.9493 (IC base=+0.137)

- **PATRÓN** `volumen_pendiente_norm` < `0.1129` → IC=+0.201 (n=195)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.1129 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` < `1.611` → IC=+0.151 (n=104)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` < 1.611 (IC base=+0.137)

- **PATRÓN** `volumen_spike_ratio` > `2.1901` → IC=+0.161 (n=107)

  - _Acción_: Kelly boost +0.80€ cuando `volumen_spike_ratio` > 2.1901 (IC base=+0.137)

- **PATRÓN** `libro_liquidez` > `3331.2707` → IC=+0.169 (n=243)

  - _Acción_: Kelly boost +0.85€ cuando `libro_liquidez` > 3331.2707 (IC base=+0.137)

- **PATRÓN** `ballena_activa_n` < `47.0` → IC=+0.188 (n=206)

  - _Acción_: Kelly boost +0.94€ cuando `ballena_activa_n` < 47.0 (IC base=+0.137)
...
- **FILTRO** `ibs_20min` > `0.1754` → IC=-0.125 (n=126)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.159 (n=247)

- **FILTRO** `dist_vwap_pct` > `0.6296` → IC=-0.179 (n=26)

  - _Acción_: SKIP cuando `dist_vwap_pct` > 0.6296
  - _Potencial_: sin este filtro IC_bueno=+0.082 (n=347)
...
- **PATRÓN** `ibs_20min` < `0.1754` → IC=+0.159 (n=247)

  - _Acción_: Kelly boost +0.79€ cuando `ibs_20min` < 0.1754 (IC base=+0.063)

- **PATRÓN** `sigma_ewma_delta_pct` > `6.007` → IC=+0.143 (n=113)

  - _Acción_: Kelly boost +0.72€ cuando `sigma_ewma_delta_pct` > 6.007 (IC base=+0.063)
...
### LEADLAG_BTC_XRP_15M
- **PATRÓN** `py_entrada` > `0.5` → IC=+0.122 (n=746)
...
- **FILTRO** `py_entrada` < `0.42` → IC=-0.136 (n=383)

  - _Acción_: SKIP cuando `py_entrada` < 0.42
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=459)

- **FILTRO** `restante_min` < `3.83` → IC=-0.123 (n=274)

  - _Acción_: SKIP cuando `restante_min` < 3.83
  - _Potencial_: sin este filtro IC_bueno=-0.033 (n=568)
...
- **FILTRO** `restante_min` < `13.34` → IC=-0.151 (n=41)

  - _Acción_: SKIP cuando `restante_min` < 13.34
  - _Potencial_: sin este filtro IC_bueno=+0.077 (n=24)
...
- **FILTRO** `lag_apertura_s` > `91.93` → IC=-0.122 (n=43)

  - _Acción_: SKIP cuando `lag_apertura_s` > 91.93
  - _Potencial_: sin este filtro IC_bueno=+0.042 (n=22)

- **FILTRO** `profundidad_ratio` < `44.7` → IC=-0.239 (n=21)
...
- **FILTRO** `py_entrada` > `0.505` → IC=-0.126 (n=241)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=7670)
...
- **FILTRO** `py_entrada` < `0.475` → IC=-0.168 (n=3672)

  - _Acción_: SKIP cuando `py_entrada` < 0.475
  - _Potencial_: sin este filtro IC_bueno=+0.058 (n=11242)

- **FILTRO** `py_entrada` > `0.59` → IC=-0.165 (n=3816)

  - _Acción_: SKIP cuando `py_entrada` > 0.59
  - _Potencial_: sin este filtro IC_bueno=+0.036 (n=11629)

### MOMENTUM_IBS_15M_BALLENA#BNB#15min
- **FILTRO** `py_entrada` < `0.46` → IC=-0.208 (n=632)

  - _Acción_: SKIP cuando `py_entrada` < 0.46
  - _Potencial_: sin este filtro IC_bueno=+0.100 (n=1962)

### MOMENTUM_IBS_15M_BALLENA#DOGE#15min
- **FILTRO** `py_entrada` < `0.48` → IC=-0.188 (n=658)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.207 (n=670)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.62
  - _Potencial_: sin este filtro IC_bueno=+0.064 (n=2102)

### MOMENTUM_IBS_15M_BALLENA#XRP#15min
- **FILTRO** `py_entrada` < `0.49` → IC=-0.171 (n=642)

  - _Acción_: SKIP cuando `py_entrada` < 0.49
  - _Potencial_: sin este filtro IC_bueno=+0.083 (n=1948)

- **FILTRO** `py_entrada` > `0.57` → IC=-0.175 (n=665)

  - _Acción_: SKIP cuando `py_entrada` > 0.57
  - _Potencial_: sin este filtro IC_bueno=+0.051 (n=2115)
...
- **FILTRO** `py_entrada` < `0.505` → IC=-0.239 (n=21)
  ...
  - _Acción_: SKIP cuando `py_entrada` < 0.505
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=3020)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.131 (n=10450)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.079 (n=23539)

- **FILTRO** `py_entrada` < `0.34` → IC=-0.274 (n=8366)

  - _Acción_: SKIP cuando `py_entrada` < 0.34
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=25623)

- **FILTRO** `ibs_7min` < `0.2791` → IC=-0.235 (n=8497)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2791
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=25492)

- **FILTRO** `ballena_activa_n` > `15.0` → IC=-0.157 (n=11495)

  - _Acción_: SKIP cuando `ballena_activa_n` > 15.0
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=22494)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.230 (n=10483)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.002 (n=32338)

- **FILTRO** `ibs_7min` > `0.2917` → IC=-0.177 (n=10700)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2917
  - _Potencial_: sin este filtro IC_bueno=-0.014 (n=32121)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.137 (n=1702)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.075 (n=3949)

- **FILTRO** `py_entrada` < `0.31` → IC=-0.312 (n=1342)

  - _Acción_: SKIP cuando `py_entrada` < 0.31
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=4309)

- **FILTRO** `ibs_7min` < `0.7113` → IC=-0.252 (n=1864)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7113
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=3787)

- **FILTRO** `ballena_activa_n` > `8.0` → IC=-0.188 (n=1249)

  - _Acción_: SKIP cuando `ballena_activa_n` > 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.067 (n=4402)

- **FILTRO** `py_entrada` > `0.71` → IC=-0.258 (n=1815)

  - _Acción_: SKIP cuando `py_entrada` > 0.71
  - _Potencial_: sin este filtro IC_bueno=-0.003 (n=5549)

- **FILTRO** `drift_7min_pct` |x|> `0.1104` → IC=-0.122 (n=2501)

  - _Acción_: SKIP cuando `drift_7min_pct` |x|> 0.1104
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=4863)

- **FILTRO** `ibs_7min` > `0.7869` → IC=-0.208 (n=1839)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7869
  - _Potencial_: sin este filtro IC_bueno=-0.019 (n=5525)

### MOMENTUM_IBS_5M_BALLENA#BTC#5min
- **FILTRO** `hora_utc` < `6.0` → IC=-0.139 (n=1372)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
  - _Potencial_: sin este filtro IC_bueno=-0.087 (n=4488)

- **FILTRO** `py_entrada` < `0.35` → IC=-0.252 (n=1424)

  - _Acción_: SKIP cuando `py_entrada` < 0.35
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=4436)

- **FILTRO** `ibs_7min` < `0.7471` → IC=-0.193 (n=1464)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7471
  - _Potencial_: sin este filtro IC_bueno=-0.068 (n=4396)

- **FILTRO** `ballena_activa_n` > `160.0` → IC=-0.177 (n=1457)

  - _Acción_: SKIP cuando `ballena_activa_n` > 160.0
  - _Potencial_: sin este filtro IC_bueno=-0.073 (n=4403)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.263 (n=1373)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=4582)

- **FILTRO** `ibs_7min` > `0.2609` → IC=-0.181 (n=1485)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2609
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=4470)

- **FILTRO** `ballena_activa_n` > `153.0` → IC=-0.183 (n=1477)

  - _Acción_: SKIP cuando `ballena_activa_n` > 153.0
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=4478)

### MOMENTUM_IBS_5M_BALLENA#DOGE#5min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.166 (n=1313)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=4068)

- **FILTRO** `py_entrada` < `0.32` → IC=-0.303 (n=1335)

  - _Acción_: SKIP cuando `py_entrada` < 0.32
  - _Potencial_: sin este filtro IC_bueno=-0.039 (n=4046)

- **FILTRO** `ibs_7min` < `0.7061` → IC=-0.243 (n=1775)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7061
  - _Potencial_: sin este filtro IC_bueno=-0.037 (n=3606)

- **FILTRO** `ballena_activa_n` > `7.0` → IC=-0.222 (n=1202)

  - _Acción_: SKIP cuando `ballena_activa_n` > 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4179)

- **FILTRO** `py_entrada` > `0.7` → IC=-0.239 (n=1810)

  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=+0.018 (n=6066)
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.129 (n=1802)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
  - _Potencial_: sin este filtro IC_bueno=-0.085 (n=3803)

- **FILTRO** `py_entrada` < `0.37` → IC=-0.234 (n=1657)

  - _Acción_: SKIP cuando `py_entrada` < 0.37
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=3948)

- **FILTRO** `ibs_7min` < `0.7399` → IC=-0.183 (n=1401)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7399
  - _Potencial_: sin este filtro IC_bueno=-0.071 (n=4204)

- **FILTRO** `ballena_activa_n` > `31.0` → IC=-0.174 (n=1398)

  - _Acción_: SKIP cuando `ballena_activa_n` > 31.0
  - _Potencial_: sin este filtro IC_bueno=-0.074 (n=4207)

- **FILTRO** `py_entrada` > `0.66` → IC=-0.257 (n=1432)

  - _Acción_: SKIP cuando `py_entrada` > 0.66
  - _Potencial_: sin este filtro IC_bueno=-0.032 (n=4303)

- **FILTRO** `ibs_7min` > `0.2744` → IC=-0.177 (n=1433)

  - _Acción_: SKIP cuando `ibs_7min` > 0.2744
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=4302)

- **FILTRO** `ballena_activa_n` > `30.0` → IC=-0.183 (n=1397)

  - _Acción_: SKIP cuando `ballena_activa_n` > 30.0
  - _Potencial_: sin este filtro IC_bueno=-0.058 (n=4338)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.36` → IC=-0.256 (n=1467)
...
- **FILTRO** `ibs_7min` < `0.2941` → IC=-0.233 (n=1469)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=4423)

- **FILTRO** `py_entrada` > `0.61` → IC=-0.179 (n=1930)

  - _Acción_: SKIP cuando `py_entrada` > 0.61
  - _Potencial_: sin este filtro IC_bueno=+0.022 (n=6226)

### MOMENTUM_IBS_5M_BALLENA#XRP#5min
- **FILTRO** `py_entrada` < `0.38` → IC=-0.254 (n=1832)

  - _Acción_: SKIP cuando `py_entrada` < 0.38
  - _Potencial_: sin este filtro IC_bueno=-0.016 (n=3768)

- **FILTRO** `ibs_7min` < `0.299` → IC=-0.224 (n=1400)

  - _Acción_: SKIP cuando `ibs_7min` < 0.299
  - _Potencial_: sin este filtro IC_bueno=-0.050 (n=4200)

- **FILTRO** `ballena_activa_n` > `11.0` → IC=-0.214 (n=1358)

  - _Acción_: SKIP cuando `ballena_activa_n` > 11.0
  - _Potencial_: sin este filtro IC_bueno=-0.055 (n=4242)

- **FILTRO** `py_entrada` > `0.67` → IC=-0.208 (n=1819)

  - _Acción_: SKIP cuando `py_entrada` > 0.67
  - _Potencial_: sin este filtro IC_bueno=+0.013 (n=5916)
...
### UPDOWN_GBM#SOL#5min
- **FILTRO** `dist_vwap_pct` > `0.6969` → IC=-0.153 (n=99)

  - _Acción_: SKIP cuando `dist_vwap_pct` > 0.6969
  - _Potencial_: sin este filtro IC_bueno=+0.052 (n=1030)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.0764` → IC=+0.245 (n=1705)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.0764 (IC base=-0.029)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.1783` → IC=+0.238 (n=1236)

  - _Acción_: Kelly boost +1.00€ cuando `divergencia_cvd_spot_perp` |x|≤ 0.1783 (IC base=-0.029)

- **PATRÓN** `ibs_15` < `0.3469` → IC=+0.276 (n=1909)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3469 (IC base=-0.029)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1365` → IC=+0.301 (n=229)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3469 sube el IC de -0.029 a +0.276 en UPDOWN_GBM_15M_TARDIO (n=1909). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 29183 | -0.085 | -3889.67€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 1541 | -0.036 | -226.47€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 27642 | -0.088 | -3663.20€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB | 3805 | -0.099 | -627.94€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BNB#5min | 3805 | -0.099 | -627.94€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#BTC | 1541 | -0.036 | -226.47€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#BTC#15min | 1541 | -0.036 | -226.47€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE | 3484 | -0.096 | -788.92€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 3484 | -0.096 | -788.92€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 7496 | -0.019 | -705.50€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 7496 | -0.019 | -705.50€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL | 7114 | -0.089 | -450.93€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#SOL#5min | 7114 | -0.089 | -450.93€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#XRP | 5743 | -0.164 | -1089.90€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#XRP#5min | 5743 | -0.164 | -1089.90€ | 1 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 19460 | -0.026 | +3956.29€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 5060 | +0.001 | +1814.85€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 14400 | -0.036 | +2141.44€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 19460 | -0.026 | +3956.29€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 5060 | +0.001 | +1814.85€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 14400 | -0.036 | +2141.44€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 96821 | +0.113 | -4755.17€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 14471 | +0.185 | -430.93€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 391 | -0.075 | -50.62€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 75725 | +0.100 | -4060.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#60min | 6234 | +0.107 | -212.85€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB | 12590 | +0.099 | -1018.06€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#15min | 46 | -0.167 | -1.52€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#BNB#240min | 15 | -0.243 | -11.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BNB#5min | 12529 | +0.101 | -1004.75€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC | 19564 | +0.132 | -359.33€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#15min | 4567 | +0.203 | -129.42€ | 0 | 10 |
| ✅ FAVORITO_CONFIRMADO#BTC#240min | 42 | -0.114 | -22.23€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#5min | 12554 | +0.112 | -185.83€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#BTC#60min | 2401 | +0.102 | -21.85€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#DOGE | 12629 | +0.090 | -1139.45€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#15min | 53 | -0.100 | -7.49€ | 0 | 0 |
| 🚫 FAVORITO_CONFIRMADO#DOGE#240min | 15 | -0.243 | -11.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#DOGE#5min | 12561 | +0.091 | -1120.78€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH | 20568 | +0.124 | -379.81€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#15min | 5607 | +0.175 | -78.34€ | 1 | 6 |
| ✅ FAVORITO_CONFIRMADO#ETH#240min | 12 | -0.129 | -8.57€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#5min | 12693 | +0.106 | -237.66€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#ETH#60min | 2256 | +0.099 | -55.24€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#SOL | 18868 | +0.114 | -1089.01€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#15min | 4151 | +0.188 | -222.34€ | 0 | 7 |
| ✅ FAVORITO_CONFIRMADO#SOL#240min | 294 | -0.034 | +3.35€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#5min | 12846 | +0.092 | -734.26€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#SOL#60min | 1577 | +0.127 | -135.76€ | 0 | 6 |
| ✅ FAVORITO_CONFIRMADO#XRP | 12602 | +0.099 | -769.51€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#15min | 47 | -0.031 | +8.18€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#240min | 13 | -0.022 | -0.19€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#XRP#5min | 12542 | +0.100 | -777.50€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION | 15350 | +0.191 | -1016.52€ | 1 | 5 |
| ✅ FAVORITO_CONFIRMADO_15MIN_ALTACONVICCION#15min | 
25€ | 0 | 0 |
| ✅ UPDOWN_GBM#15min | 10737 | +0.070 | +2001.16€ | 0 | 11 |
| ✅ UPDOWN_GBM#240min | 1454 | +0.005 | +7.13€ | 0 | 0 |
| ✅ UPDOWN_GBM#5min | 25435 | +0.022 | +466.63€ | 0 | 0 |
| ✅ UPDOWN_GBM#60min | 2435 | +0.000 | +13.75€ | 1 | 0 |
| ✅ UPDOWN_GBM#BNB | 4186 | +0.070 | +479.77€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#15min | 757 | +0.155 | +310.68€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#240min | 33 | -0.014 | -0.70€ | 0 | 0 |
| ✅ UPDOWN_GBM#BNB#5min | 3396 | +0.052 | +169.79€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC | 7480 | +0.038 | +523.59€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#15min | 1351 | +0.085 | +303.23€ | 0 | 9 |
| ✅ UPDOWN_GBM#BTC#240min | 390 | +0.018 | +7.21€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#5min | 4584 | +0.036 | +190.46€ | 0 | 0 |
| ✅ UPDOWN_GBM#BTC#60min | 1096 | +0.001 | +21.64€ | 1 | 0 |
| ✅ UPDOWN_GBM#BTC#daily | 59 | -0.090 | +1.05€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE | 4756 | +0.042 | +306.76€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#15min | 710 | +0.140 | +253.46€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#240min | 28 | +0.000 | -1.43€ | 0 | 0 |
| ✅ UPDOWN_GBM#DOGE#5min | 4018 | +0.025 | +54.73€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH | 8576 | +0.020 | +335.49€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#15min | 2718 | +0.049 | +308.37€ | 0 | 11 |
| ✅ UPDOWN_GBM#ETH#240min | 381 | +0.006 | +7.30€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#5min | 4607 | +0.009 | +26.12€ | 0 | 0 |
| ✅ UPDOWN_GBM#ETH#60min | 820 | -0.004 | -9.53€ | 2 | 0 |
| ✅ UPDOWN_GBM#ETH#daily | 50 | -0.135 | +3.23€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL | 9343 | +0.015 | +229.58€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#15min | 2580 | +0.026 | +175.89€ | 0 | 12 |
| ✅ UPDOWN_GBM#SOL#240min | 373 | -0.004 | -1.99€ | 0 | 0 |
| ✅ UPDOWN_GBM#SOL#5min | 5829 | +0.013 | +57.89€ | 1 | 0 |
| ✅ UPDOWN_GBM#SOL#60min | 519 | +0.005 | +1.63€ | 0 | 1 |
| ✅ UPDOWN_GBM#SOL#daily | 42 | -0.182 | -3.84€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP | 5871 | +0.037 | +613.90€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#15min | 2621 | +0.085 | +649.52€ | 0 | 12 |
| ✅ UPDOWN_GBM#XRP#240min | 249 | -0.002 | -3.25€ | 0 | 0 |
| ✅ UPDOWN_GBM#XRP#5min | 3001 | -0.002 | -32.36€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO | 12568 | -0.037 | +2720.21€ | 3 | 7 |
| ✅ UPDOWN_GBM_15M_TARDIO#15min | 12568 | -0.037 | +2720.21€ | 0 | 0 |
...
| ✅ UPDOWN_GBM_15M_TARDIO#SOL | 3774 | -0.065 | +565.40€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#SOL#15min | 3774 | -0.065 | +565.40€ | 3 | 5 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP | 3810 | -0.074 | +624.53€ | 0 | 0 |
| ✅ UPDOWN_GBM_15M_TARDIO#XRP#15min | 3810 | -0.074 | +624.53€ | 3 | 4 |
...
**✅ H-GBM-18H** — Bloquear hora 18h UTC en GBM
  ...
  - _Estado_: IC=+0.016 n=525 — no justifica filtro, seguir monitorizando
  - _Datos_: n=525 IC=+0.016 PNL=+21.34€
...
**⚠️ H-SOL-15MIN** — SOL#15min → umbral live (IC≥0.08 n≥40)
  ...
  - _Datos_: n=2579 IC=+0.026 PNL=+175.40€
...
**⏳ H-60MIN-LIVE** — Estrategias 60min → umbral live (IC≥0.08 n≥40)
  ...
  - _Acción_: Activar live cuando haya credenciales Polymarket API
  - _Estado_: ETH#60min: n=819/40 IC=-0.003 PNL=-9.02€ | BTC#60min: n=1093/40 IC=+0.001 PNL=+22.21€ | SOL#60min: n=519/40 IC=+0.005 PNL=+1.63€

**⏳ H-STREAK-COOLDOWN** — Cooldown tras 2 derrotas consecutivas (mismo subtype)
  - _Umbral_: n≥40 tras 2 losses y gap(IC_tras_win - IC_tras_2loss)≥0.05
  - _Acción_: Reducir stake (no desactivar) 1-2h tras 2 derrotas consecutivas en el mismo subtype
  - _Estado_: tras_win IC=+0.050 n=351089 | tras_1loss IC=+0.080 n=272393 | tras_2loss IC=+0.050 n=114367/40 | gap=+0.000 (umbral 0.05)
...
**🟡 H-24H-OF-18H** — ORDER_FLOW BUY_NO a las 18h UTC — GBM bloqueado pero OF funciona
  ...
  - _Estado_: SEÑAL POSITIVA confirmada: IC=+0.250 > 0.08 con n=42 PNL=+33.43€
  - _Datos_: n=42 IC=+0.250 PNL=+33.43€
...
**〰️ H-CUSTOM-GBM-60MIN-BUYYES** — GBM 60min BUY_YES — ¿edge superior al BUY_NO?
  ...
  - _Estado_: n=1705 IC=+0.007 PNL=+1.78€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=1705 IC=+0.007 PNL=+1.78€
...
**〰️ H-CUSTOM-GBM-18H** — GBM a las 18h UTC — ¿blacklist necesario?
  ...
  - _Estado_: n=525 IC=+0.016 PNL=+21.34€ — sin señal clara aún (umbral IC: min=None max=-0.08)
  - _Datos_: n=525 IC=+0.016 PNL=+21.34€
...
**〰️ H-CUSTOM-GBM-BUYYES-GLOBAL-MALO** — UPDOWN_GBM BUY_YES global — ¿estructuralmente peor que BUY_NO en todas las estrategias activas?
  ...
  - _Estado_: n=14281 IC=+0.054 PNL=+1715.68€ — sin señal clara aún (umbral IC: min=None max=-0.05)
  - _Datos_: n=14281 IC=+0.054 PNL=+1715.68€
...
**〰️ H-CUSTOM-XRP15-BUYNO-LIVE** — XRP#15min BUY_NO — candidato live nº2 (detrás de ETH#15min)
  ...
  - _Estado_: n=2020 IC=+0.053 PNL=+221.87€ — sin señal clara aún (umbral IC: min=0.1 max=None)
  - _Datos_: n=2020 IC=+0.053 PNL=+221.87€
...
**〰️ H-PRECIO-YES-BARATO** — BUY_YES con precio de mercado 0.30-0.40 — mercado infravalora YES
  ...
  - _Estado_: n=18891 IC=-0.138 PNL=+1175.94€ — sin señal clara aún (umbral IC: min=0.08 max=None)
  - _Datos_: n=18891 IC=-0.138 PNL=+1175.94€
...
**〰️ H-CUSTOM-LATE15-BTC-BUYNO-COINFLIP** — GBM_LATE_15M BTC#BUY_NO es moneda al aire — candidata a quitar del motor estrella
  ...
  - _Estado_: n=2022 IC=+0.140 PNL=+1135.04€ — sin señal clara aún (umbral IC: min=None max=0.03)
  - _Datos_: n=2022 IC=+0.140 PNL=+1135.04€
...
**🔴 H-CUSTOM-GBMLATE-PYBAJO-LONGSHOT** — GBM_LATE_15M BUY_YES con prob_yes_modelo<0.53 — mismo sesgo favorito-longshot que el resto del sistema. IMPLEMENTADO 21-Jul
  ...
  - _Estado_: SEÑAL NEGATIVA confirmada: IC=-0.238 < -0.1 con n=1852 PNL=-215.75€
  - _Datos_: n=1852 IC=-0.238 PNL=-215.75€
...
**⏳ H-CUSTOM-GBM18H-XRP-EXCEPCION** — UPDOWN_GBM XRP a las 18h UTC -- puede estar mal incluida en el blacklist horario global
  ...
  - _Estado_: 38/40 ops en el filtro definido (IC actual=+0.000 PNL=+4.52€)
  - _Datos_: n=38 IC=+0.000 PNL=+4.52€
...
**🔶 H-CUSTOM-LEADLAG-XRP-BUYNO** — LEADLAG_BTC_XRP_15M -- la señal se concentra en BUY_NO, BUY_YES está plano
  ...
  - _Estado_: SEÑAL POSITIVA en XRP (IC=+0.103 n=1059) pero sin cruzar ≥2 pares más — sin otros pares con datos
  - _Datos_: n=1059 IC=+0.103 PNL=+259.86€
...
**〰️ H-CUSTOM-FAVALTACONV-BNB5M-PAYOUT-NEGATIVO** — ALERTA -- FAVORITO_CONFIRMADO_5MIN_ALTACONVICCION#BNB#5min#BUY_YES pierde dinero en TODOS los buckets de precio pese a IC positivo
  ...
  - _Estado_: n=9151 IC=+0.176 PNL=-1056.77€ — sin señal clara aún (umbral IC: min=999 max=None)
  - _Datos_: n=9151 IC=+0.176 PNL=-1056.77€
