# Hipótesis automáticas — 2026-09-02 00:25 UTC
_Generado por shadow_postmortem.py sobre 245800 resoluciones (PNL=+21391.83€)_
...
### BALLENAS_TARDIAS
- **FILTRO** `restante_s_al_confirmar` < `146.16` → IC=-0.294 (n=3387)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 146.16
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=10161)
...
- **FILTRO** `restante_s_al_confirmar` < `110.65` → IC=-0.411 (n=435)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 110.65
  - _Potencial_: sin este filtro IC_bueno=-0.072 (n=1308)

### BALLENAS_TARDIAS#ETH#5min
- **FILTRO** `restante_s_al_confirmar` < `157.05` → IC=-0.157 (n=926)

  - _Acción_: SKIP cuando `restante_s_al_confirmar` < 157.05
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=2778)
...
### FAVORITO_CONFIRMADO
- **PATRÓN** `py_entrada` > `0.7` → IC=+0.191 (n=7242)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.155 (n=5725)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.02` → IC=+0.191 (n=3553)

  - _Acción_: Kelly boost +0.95€ cuando `libro_spread` < 0.02 (IC base=+0.141)

- **PATRÓN** `libro_liquidez` > `1919.9988` → IC=+0.183 (n=3019)
...
- **PATRÓN** `hora_utc` < `17.0` → IC=+0.204 (n=829)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.196 (n=1069)

  - _Acción_: Kelly boost +0.98€ cuando `libro_spread` < 0.01 (IC base=+0.195)

- **PATRÓN** `libro_liquidez` > `12484.3864` → IC=+0.216 (n=375)
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.273 (n=618)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `libro_liquidez` > `2838.1003` → IC=+0.273 (n=398)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 2838.1003 (IC base=+0.269)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.194 (n=4831)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` < 15.0 (IC base=+0.189)

- **PATRÓN** `py_entrada` > `0.73` → IC=+0.203 (n=2744)
...
- **PATRÓN** `py_entrada` < `0.74` → IC=+0.180 (n=1487)
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.237 (n=1103)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `15.0` → IC=+0.187 (n=1193)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.94€ cuando `hora_utc` < 15.0 (IC base=+0.184)

- **PATRÓN** `py_entrada` < `0.7` → IC=+0.191 (n=722)
...
- **PATRÓN** `py_entrada` > `0.71` → IC=+0.208 (n=15621)
...
- **PATRÓN** `py_entrada` > `0.72` → IC=+0.171 (n=2237)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.223 (n=1315)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `py_entrada` > `0.74` → IC=+0.243 (n=1323)
...
- **PATRÓN** `hora_utc` < `12.0` → IC=+0.191 (n=2026)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.96€ cuando `hora_utc` < 12.0 (IC base=+0.183)

- **PATRÓN** `py_entrada` > `0.71` → IC=+0.204 (n=2594)
...
- **PATRÓN** `restante_min` < `3.98` → IC=+0.145 (n=2113)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.149 (n=3055)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.162 (n=1511)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.137 (n=1544)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
### GBM_LATE_15M
- **PATRÓN** `ibs_20min` > `0.3503` → IC=+0.124 (n=3234)

  - _Acción_: Kelly boost +0.62€ cuando `ibs_20min` > 0.3503 (IC base=+0.075)

- **PATRÓN** `dist_vwap_pct` > `0.1813` → IC=+0.222 (n=433)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.1813 (IC base=+0.075)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.436` → IC=+0.147 (n=1240)

  - _Acción_: Kelly boost +0.74€ cuando `sigma_ewma_delta_pct` > 5.436 (IC base=+0.075)

- **PATRÓN** `volumen_regimen` > `1.0855` → IC=+0.239 (n=328)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` > 1.0855 (IC base=+0.075)

- **PATRÓN** `volumen_pendiente_norm` > `0.1087` → IC=+0.173 (n=692)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_pendiente_norm` > 0.1087 (IC base=+0.075)

- **PATRÓN** `volumen_spike_ratio` < `2.9301` → IC=+0.169 (n=1879)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 2.9301 (IC base=+0.075)

- **PATRÓN** `volumen_spike_ratio` > `1.4784` → IC=+0.167 (n=1878)
...
- **PATRÓN** `ibs_20min` < `0.4091` → IC=+0.123 (n=3172)

  - _Acción_: Kelly boost +0.61€ cuando `ibs_20min` < 0.4091 (IC base=+0.036)

- **PATRÓN** `volumen_regimen` < `0.6747` → IC=+0.163 (n=476)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_regimen` < 0.6747 (IC base=+0.036)

- **PATRÓN** `volumen_pendiente_norm` > `0.3071` → IC=+0.273 (n=231)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3071 (IC base=+0.036)

- **PATRÓN** `volumen_spike_ratio` > `2.8929` → IC=+0.215 (n=513)
...
- **PATRÓN** `sigma_h` > `0.0069` → IC=+0.177 (n=326)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.88€ cuando `sigma_h` > 0.0069 (IC base=+0.148)

- **PATRÓN** `drift_60min` |x|≤ `0.2679` → IC=+0.150 (n=716)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.75€ cuando `drift_60min` |x|≤ 0.2679 (IC base=+0.148)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.183 (n=342)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.412` → IC=+0.294 (n=226)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.412 (IC base=+0.148)

- **PATRÓN** `volumen_pendiente_norm` > `0.2291` → IC=+0.218 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2291 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` < `2.6958` → IC=+0.125 (n=620)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_spike_ratio` < 2.6958 (IC base=+0.148)

- **PATRÓN** `volumen_spike_ratio` > `1.4532` → IC=+0.143 (n=620)

  - _Acción_: Kelly boost +0.72€ cuando `volumen_spike_ratio` > 1.4532 (IC base=+0.148)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.193 (n=551)
...
- **PATRÓN** `libro_liquidez` > `1698.3448` → IC=+0.269 (n=314)
...
- **PATRÓN** `sigma_h` < `0.0061` → IC=+0.150 (n=627)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.75€ cuando `sigma_h` < 0.0061 (IC base=+0.146)

- **PATRÓN** `drift_60min` |x|≤ `0.0804` → IC=+0.159 (n=209)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.79€ cuando `drift_60min` |x|≤ 0.0804 (IC base=+0.146)

- **PATRÓN** `hora_utc` > `12.0` → IC=+0.149 (n=434)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.75€ cuando `hora_utc` > 12.0 (IC base=+0.146)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.148 (n=655)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.74€ cuando `hora_utc` < 18.0 (IC base=+0.146)

- **PATRÓN** `ibs_20min` < `0.6376` → IC=+0.172 (n=625)
...
- **PATRÓN** `volumen_regimen` < `0.613` → IC=+0.197 (n=209)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_regimen` < 0.613 (IC base=+0.146)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4857` → IC=+0.162 (n=462)

  - _Acción_: Kelly boost +0.81€ cuando `volumen_spike_ratio` > 1.4857 (IC base=+0.146)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.147 (n=808)
...
- **PATRÓN** `ballena_activa_n` < `234.0` → IC=+0.162 (n=190)

  - _Acción_: Kelly boost +0.81€ cuando `ballena_activa_n` < 234.0 (IC base=+0.146)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.213 (n=242)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.171` → IC=+0.256 (n=154)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.171 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` < `0.1324` → IC=+0.163 (n=562)

  - _Acción_: Kelly boost +0.82€ cuando `volumen_pendiente_norm` < 0.1324 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` < `3.2867` → IC=+0.158 (n=515)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 3.2867 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` > `1.6882` → IC=+0.175 (n=585)
...
- **PATRÓN** `sigma_h` < `0.01` → IC=+0.251 (n=531)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.01 (IC base=+0.236)

- **PATRÓN** `drift_60min` |x|≤ `0.4614` → IC=+0.245 (n=531)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4614 (IC base=+0.236)

- **PATRÓN** `hora_utc` > `11.0` → IC=+0.242 (n=378)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 11.0 (IC base=+0.236)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.261 (n=241)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.236)

- **PATRÓN** `ibs_20min` < `0.3964` → IC=+0.268 (n=467)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.385` → IC=+0.333 (n=70)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.385 (IC base=+0.236)

- **PATRÓN** `volumen_spike_ratio` < `3.8848` → IC=+0.218 (n=409)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.8848 (IC base=+0.236)

- **PATRÓN** `ballena_activa_n` < `50.0` → IC=+0.226 (n=330)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 50.0 (IC base=+0.236)

### GBM_LATE_15M#ETH#15min
- **FILTRO** `hora_utc` < `7.0` → IC=-0.154 (n=151)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
  - _Potencial_: sin este filtro IC_bueno=-0.007 (n=467)

- **FILTRO** `ibs_20min` < `0.2901` → IC=-0.139 (n=203)
...
- **PATRÓN** `dist_vwap_pct` > `0.3391` → IC=+0.308 (n=24)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.3391 (IC base=-0.043)
...
- **FILTRO** `sigma_ewma_delta_pct` > `8.287` → IC=-0.179 (n=185)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 8.287
  - _Potencial_: sin este filtro IC_bueno=-0.044 (n=1445)
...
- **FILTRO** `sigma_h` > `0.0245` → IC=-0.141 (n=369)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.026 (n=1111)
...
- **FILTRO** `sigma_ewma_delta_pct` > `6.892` → IC=-0.155 (n=236)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.035 (n=1244)
...
- **PATRÓN** `ballena_activa_n` < `18.0` → IC=+0.175 (n=38)
  ...
  - _Acción_: Kelly boost +0.88€ cuando `ballena_activa_n` < 18.0 (IC base=-0.055)
...
- **PATRÓN** `ibs_20min` > `0.2691` → IC=+0.127 (n=4040)

  - _Acción_: Kelly boost +0.64€ cuando `ibs_20min` > 0.2691 (IC base=+0.059)
...
- **PATRÓN** `ibs_20min` < `0.5882` → IC=+0.126 (n=3967)
...
- **PATRÓN** `volumen_regimen` > `1.2272` → IC=+0.245 (n=328)
...
- **PATRÓN** `volumen_spike_ratio` > `2.4273` → IC=+0.279 (n=482)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.4273 (IC base=+0.048)

- **PATRÓN** `ballena_activa_n` < `51.0` → IC=+0.278 (n=805)
...
- **FILTRO** `sigma_ewma_delta_pct` > `2.177` → IC=-0.163 (n=277)

  - _Acción_: SKIP cuando `sigma_ewma_delta_pct` > 2.177
  - _Potencial_: sin este filtro IC_bueno=+0.032 (n=607)
...
- **FILTRO** `ibs_20min` > `0.7651` → IC=-0.139 (n=475)

  - _Acción_: SKIP cuando `ibs_20min` > 0.7651
...
- **PATRÓN** `volumen_regimen` < `0.5727` → IC=+0.255 (n=47)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_regimen` < 0.5727 (IC base=-0.031)

- **PATRÓN** `volumen_regimen` > `1.1181` → IC=+0.173 (n=47)

  - _Acción_: Kelly boost +0.87€ cuando `volumen_regimen` > 1.1181 (IC base=-0.031)

- **PATRÓN** `volumen_pendiente_norm` < `0.1467` → IC=+0.179 (n=104)

  - _Acción_: Kelly boost +0.90€ cuando `volumen_pendiente_norm` < 0.1467 (IC base=-0.031)

- **PATRÓN** `volumen_spike_ratio` < `1.7866` → IC=+0.246 (n=65)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.7866 (IC base=-0.031)
...
- **PATRÓN** `ibs_20min` < `0.3262` → IC=+0.328 (n=433)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.3262 (IC base=+0.310)

- **PATRÓN** `sigma_ewma_delta_pct` > `2.729` → IC=+0.313 (n=132)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.729 (IC base=+0.310)

- **PATRÓN** `sigma_ewma_delta_pct` < `5.533` → IC=+0.313 (n=490)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 5.533 (IC base=+0.310)
...
- **PATRÓN** `volumen_spike_ratio` < `3.5266` → IC=+0.304 (n=340)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 3.5266 (IC base=+0.310)

- **PATRÓN** `volumen_spike_ratio` > `2.3984` → IC=+0.325 (n=227)
...
- **FILTRO** `hora_utc` < `9.0` → IC=-0.162 (n=205)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 9.0
  - _Potencial_: sin este filtro IC_bueno=-0.043 (n=427)

- **FILTRO** `ibs_20min` < `0.7268` → IC=-0.156 (n=417)
...
- **PATRÓN** `drift_60min` |x|≤ `0.3304` → IC=+0.171 (n=2986)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `dist_vwap_pct` > `1.1185` → IC=+0.244 (n=552)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 1.1185 (IC base=+0.164)

- **PATRÓN** `sigma_ewma_delta_pct` > `5.522` → IC=+0.237 (n=1383)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 5.522 (IC base=+0.164)

- **PATRÓN** `volumen_regimen` > `0.8622` → IC=+0.182 (n=1543)

  - _Acción_: Kelly boost +0.91€ cuando `volumen_regimen` > 0.8622 (IC base=+0.164)

- **PATRÓN** `volumen_pendiente_norm` > `0.1056` → IC=+0.187 (n=1246)

  - _Acción_: Kelly boost +0.93€ cuando `volumen_pendiente_norm` > 0.1056 (IC base=+0.164)

- **PATRÓN** `volumen_spike_ratio` < `2.3284` → IC=+0.166 (n=2755)

  - _Acción_: Kelly boost +0.83€ cuando `volumen_spike_ratio` < 2.3284 (IC base=+0.164)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.173 (n=2631)

  - _Acción_: Kelly boost +0.86€ cuando `libro_spread` < 0.01 (IC base=+0.164)

- **PATRÓN** `libro_liquidez` > `3954.1559` → IC=+0.182 (n=1131)

  - _Acción_: Kelly boost +0.91€ cuando `libro_liquidez` > 3954.1559 (IC base=+0.164)

- **PATRÓN** `ballena_activa_n` < `91.0` → IC=+0.184 (n=2029)
...
- **PATRÓN** `hora_utc` < `7.0` → IC=+0.183 (n=1445)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.91€ cuando `hora_utc` < 7.0 (IC base=+0.179)

- **PATRÓN** `ibs_20min` < `0.4216` → IC=+0.231 (n=3197)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4216 (IC base=+0.179)

- **PATRÓN** `sigma_ewma_delta_pct` > `9.938` → IC=+0.220 (n=583)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 9.938 (IC base=+0.179)

- **PATRÓN** `volumen_regimen` < `1.1693` → IC=+0.169 (n=2436)
...
- **PATRÓN** `volumen_spike_ratio` > `2.6744` → IC=+0.197 (n=858)

  - _Acción_: Kelly boost +0.98€ cuando `volumen_spike_ratio` > 2.6744 (IC base=+0.179)

- **PATRÓN** `ballena_activa_n` < `161.0` → IC=+0.176 (n=2155)
...
- **PATRÓN** `sigma_h` > `0.007` → IC=+0.197 (n=255)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.98€ cuando `sigma_h` > 0.007 (IC base=+0.170)

- **PATRÓN** `drift_60min` |x|≤ `0.272` → IC=+0.187 (n=561)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.94€ cuando `drift_60min` |x|≤ 0.272 (IC base=+0.170)

- **PATRÓN** `hora_utc` < `8.0` → IC=+0.210 (n=274)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `6.534` → IC=+0.320 (n=181)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 6.534 (IC base=+0.170)
...
- **PATRÓN** `volumen_spike_ratio` < `2.6777` → IC=+0.155 (n=479)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` < 2.6777 (IC base=+0.170)

- **PATRÓN** `volumen_spike_ratio` > `1.4564` → IC=+0.167 (n=479)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` > 1.4564 (IC base=+0.170)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.208 (n=444)

  - _Acción_: Kelly boost +1.00€ cuando `libro_spread` < 0.04 (IC base=+0.170)

- **PATRÓN** `ballena_activa_n` < `66.0` → IC=+0.185 (n=290)

  - _Acción_: Kelly boost +0.92€ cuando `ballena_activa_n` < 66.0 (IC base=+0.170)

- **PATRÓN** `sigma_h` > `0.0048` → IC=+0.271 (n=317)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0048 (IC base=+0.247)

- **PATRÓN** `drift_60min` |x|≤ `0.2562` → IC=+0.287 (n=312)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.2562 (IC base=+0.247)

- **PATRÓN** `hora_utc` < `18.0` → IC=+0.267 (n=375)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.247)

- **PATRÓN** `ibs_20min` < `0.1524` → IC=+0.282 (n=237)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.1524 (IC base=+0.247)

- **PATRÓN** `sigma_ewma_delta_pct` < `4.691` → IC=+0.261 (n=392)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.0887` → IC=+0.242 (n=254)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.0887 (IC base=+0.247)

- **PATRÓN** `volumen_pendiente_norm` > `0.3016` → IC=+0.304 (n=44)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.3016 (IC base=+0.247)

- **PATRÓN** `volumen_spike_ratio` < `1.9106` → IC=+0.262 (n=191)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9106 (IC base=+0.247)

- **PATRÓN** `volumen_spike_ratio` > `2.7651` → IC=+0.265 (n=96)
...
- **PATRÓN** `libro_liquidez` > `11364.2884` → IC=+0.191 (n=441)

  - _Acción_: Kelly boost +0.95€ cuando `libro_liquidez` > 11364.2884 (IC base=+0.172)
...
- **PATRÓN** `drift_60min` |x|≤ `0.1633` → IC=+0.198 (n=346)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.99€ cuando `drift_60min` |x|≤ 0.1633 (IC base=+0.170)

- **PATRÓN** `hora_utc` > `17.0` → IC=+0.194 (n=178)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.97€ cuando `hora_utc` > 17.0 (IC base=+0.170)

- **PATRÓN** `hora_utc` < `5.0` → IC=+0.193 (n=174)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `volumen_pendiente_norm` < `0.226` → IC=+0.160 (n=463)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1328` → IC=+0.162 (n=190)
  ...
  - _Acción_: Kelly boost +0.81€ cuando `volumen_pendiente_norm` > 0.1328 (IC base=+0.170)

- **PATRÓN** `volumen_spike_ratio` < `2.5043` → IC=+0.168 (n=305)

  - _Acción_: Kelly boost +0.84€ cuando `volumen_spike_ratio` < 2.5043 (IC base=+0.170)
...
- **PATRÓN** `sigma_h` < `0.0074` → IC=+0.210 (n=436)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0074 (IC base=+0.172)

- **PATRÓN** `drift_60min` |x|≤ `0.3548` → IC=+0.192 (n=436)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `volumen_regimen` > `0.6414` → IC=+0.186 (n=495)
...
- **PATRÓN** `libro_spread` < `0.01` → IC=+0.174 (n=557)
...
- **PATRÓN** `sigma_h` < `0.0059` → IC=+0.137 (n=254)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.68€ cuando `sigma_h` < 0.0059 (IC base=+0.132)

- **PATRÓN** `sigma_h` > `0.0094` → IC=+0.173 (n=261)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.87€ cuando `sigma_h` > 0.0094 (IC base=+0.132)

- **PATRÓN** `drift_60min` |x|≤ `0.1` → IC=+0.134 (n=192)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.67€ cuando `drift_60min` |x|≤ 0.1 (IC base=+0.132)
...
- **PATRÓN** `ibs_20min` < `0.4362` → IC=+0.234 (n=574)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.4362 (IC base=+0.132)

- **PATRÓN** `dist_vwap_pct` > `1.0269` → IC=+0.151 (n=64)

  - _Acción_: Kelly boost +0.76€ cuando `dist_vwap_pct` > 1.0269 (IC base=+0.132)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.26` → IC=+0.214 (n=89)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 8.26 (IC base=+0.132)

- **PATRÓN** `volumen_regimen` < `1.1545` → IC=+0.148 (n=574)

  - _Acción_: Kelly boost +0.74€ cuando `volumen_regimen` < 1.1545 (IC base=+0.132)

- **PATRÓN** `volumen_regimen` > `0.8419` → IC=+0.147 (n=383)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_regimen` > 0.8419 (IC base=+0.132)

- **PATRÓN** `volumen_pendiente_norm` > `0.2675` → IC=+0.226 (n=60)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` > 0.2675 (IC base=+0.132)

- **PATRÓN** `volumen_spike_ratio` < `1.4487` → IC=+0.147 (n=148)

  - _Acción_: Kelly boost +0.73€ cuando `volumen_spike_ratio` < 1.4487 (IC base=+0.132)

- **PATRÓN** `volumen_spike_ratio` > `2.3264` → IC=+0.185 (n=147)

  - _Acción_: Kelly boost +0.92€ cuando `volumen_spike_ratio` > 2.3264 (IC base=+0.132)

- **PATRÓN** `libro_liquidez` > `1430.1495` → IC=+0.160 (n=513)

  - _Acción_: Kelly boost +0.80€ cuando `libro_liquidez` > 1430.1495 (IC base=+0.132)
...
- **PATRÓN** `sigma_h` > `0.0064` → IC=+0.160 (n=469)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` > 0.0064 (IC base=+0.133)

- **PATRÓN** `drift_60min` |x|≤ `0.3287` → IC=+0.135 (n=620)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.68€ cuando `drift_60min` |x|≤ 0.3287 (IC base=+0.133)
...
- **PATRÓN** `sigma_ewma_delta_pct` > `8.314` → IC=+0.192 (n=186)

  - _Acción_: Kelly boost +0.96€ cuando `sigma_ewma_delta_pct` > 8.314 (IC base=+0.133)
...
- **PATRÓN** `volumen_pendiente_norm` > `0.1826` → IC=+0.194 (n=191)

  - _Acción_: Kelly boost +0.97€ cuando `volumen_pendiente_norm` > 0.1826 (IC base=+0.133)
...
- **PATRÓN** `volumen_spike_ratio` > `1.9051` → IC=+0.137 (n=433)
...
- **PATRÓN** `ibs_20min` < `0.3111` → IC=+0.132 (n=474)

  - _Acción_: Kelly boost +0.66€ cuando `ibs_20min` < 0.3111 (IC base=+0.061)
...
### GBM_LATE_15M_TARDIO
- **PATRÓN** `sigma_h` > `0.0066` → IC=+0.192 (n=2686)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `dist_vwap_pct` > `0.232` → IC=+0.202 (n=1457)
  ...
  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.232 (IC base=+0.163)

- **PATRÓN** `sigma_ewma_delta_pct` > `3.396` → IC=+0.231 (n=2053)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 3.396 (IC base=+0.163)

- **PATRÓN** `volumen_regimen` < `0.8814` → IC=+0.156 (n=1871)

  - _Acción_: Kelly boost +0.78€ cuando `volumen_regimen` < 0.8814 (IC base=+0.163)

- **PATRÓN** `volumen_regimen` > `1.0858` → IC=+0.152 (n=1272)

  - _Acción_: Kelly boost +0.76€ cuando `volumen_regimen` > 1.0858 (IC base=+0.163)

- **PATRÓN** `volumen_pendiente_norm` > `0.1671` → IC=+0.191 (n=1080)

  - _Acción_: Kelly boost +0.96€ cuando `volumen_pendiente_norm` > 0.1671 (IC base=+0.163)

- **PATRÓN** `volumen_spike_ratio` > `1.8815` → IC=+0.172 (n=2468)

  - _Acción_: Kelly boost +0.86€ cuando `volumen_spike_ratio` > 1.8815 (IC base=+0.163)

- **PATRÓN** `libro_spread` < `0.01` → IC=+0.167 (n=3145)

  - _Acción_: Kelly boost +0.83€ cuando `libro_spread` < 0.01 (IC base=+0.163)

- **PATRÓN** `libro_liquidez` > `3900.1145` → IC=+0.189 (n=1342)

  - _Acción_: Kelly boost +0.94€ cuando `libro_liquidez` > 3900.1145 (IC base=+0.163)

- **PATRÓN** `ballena_activa_n` < `62.0` → IC=+0.213 (n=1780)

  - _Acción_: Kelly boost +1.00€ cuando `ballena_activa_n` < 62.0 (IC base=+0.163)

- **PATRÓN** `sigma_h` < `0.0067` → IC=+0.206 (n=2517)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0067 (IC base=+0.188)

- **PATRÓN** `drift_60min` |x|≤ `0.4735` → IC=+0.195 (n=3766)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.97€ cuando `drift_60min` |x|≤ 0.4735 (IC base=+0.188)

- **PATRÓN** `hora_utc` < `7.0` → IC=+0.205 (n=1671)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 7.0 (IC base=+0.188)

- **PATRÓN** `ibs_20min` < `0.56` → IC=+0.241 (n=3768)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.56 (IC base=+0.188)

- **PATRÓN** `dist_vwap_pct` < `0.2431` → IC=+0.173 (n=2411)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `3.561` → IC=+0.190 (n=3689)

  - _Acción_: Kelly boost +0.95€ cuando `sigma_ewma_delta_pct` < 3.561 (IC base=+0.188)

- **PATRÓN** `volumen_regimen` < `0.6196` → IC=+0.178 (n=904)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_regimen` < 0.6196 (IC base=+0.188)
...
- **PATRÓN** `volumen_spike_ratio` > `2.328` → IC=+0.199 (n=1388)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 2.328 (IC base=+0.188)

- **PATRÓN** `ballena_activa_n` < `147.0` → IC=+0.174 (n=2638)
...
- **PATRÓN** `hora_utc` < `6.0` → IC=+0.202 (n=246)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `sigma_ewma_delta_pct` > `2.701` → IC=+0.307 (n=381)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` > 2.701 (IC base=+0.180)

- **PATRÓN** `volumen_pendiente_norm` > `0.2176` → IC=+0.263 (n=116)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8922` → IC=+0.179 (n=384)

  - _Acción_: Kelly boost +0.89€ cuando `volumen_spike_ratio` > 1.8922 (IC base=+0.180)

- **PATRÓN** `libro_spread` < `0.04` → IC=+0.239 (n=488)
...
- **PATRÓN** `sigma_h` < `0.0072` → IC=+0.280 (n=484)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0072 (IC base=+0.274)

- **PATRÓN** `sigma_h` > `0.0046` → IC=+0.279 (n=483)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` > 0.0046 (IC base=+0.274)

- **PATRÓN** `drift_60min` |x|≤ `0.1095` → IC=+0.309 (n=213)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.1095 (IC base=+0.274)

- **PATRÓN** `hora_utc` > `7.0` → IC=+0.280 (n=443)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` > 7.0 (IC base=+0.274)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.280 (n=484)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 17.0 (IC base=+0.274)

- **PATRÓN** `ibs_20min` < `0.405` → IC=+0.308 (n=425)

  - _Acción_: Kelly boost +1.00€ cuando `ibs_20min` < 0.405 (IC base=+0.274)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.289` → IC=+0.290 (n=516)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 3.289 (IC base=+0.274)
...
- **PATRÓN** `volumen_spike_ratio` > `1.5014` → IC=+0.295 (n=379)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.5014 (IC base=+0.274)

- **PATRÓN** `libro_spread` < `0.02` → IC=+0.288 (n=224)
...
- **PATRÓN** `sigma_h` < `0.0062` → IC=+0.172 (n=616)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.176 (n=637)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.88€ cuando `hora_utc` < 18.0 (IC base=+0.168)

- **PATRÓN** `ibs_20min` < `0.4361` → IC=+0.215 (n=542)
...
- **PATRÓN** `hora_utc` < `8.0` → IC=+0.252 (n=260)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 8.0 (IC base=+0.207)

- **PATRÓN** `ibs_20min` > `0.6923` → IC=+0.268 (n=503)
...
- **PATRÓN** `volumen_pendiente_norm` < `0.2205` → IC=+0.215 (n=500)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_pendiente_norm` < 0.2205 (IC base=+0.207)

- **PATRÓN** `volumen_spike_ratio` < `1.9299` → IC=+0.220 (n=216)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` < 1.9299 (IC base=+0.207)
...
- **PATRÓN** `libro_spread` < `0.04` → IC=+0.237 (n=550)
...
- **PATRÓN** `sigma_h` > `0.0086` → IC=+0.238 (n=261)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
...
- **PATRÓN** `hora_utc` < `18.0` → IC=+0.247 (n=598)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 18.0 (IC base=+0.238)

- **PATRÓN** `ibs_20min` < `0.42` → IC=+0.301 (n=505)
...
- **PATRÓN** `ibs_20min` > `0.7546` → IC=+0.238 (n=437)
...
- **PATRÓN** `volumen_regimen` < `0.9067` → IC=+0.172 (n=437)
...
- **PATRÓN** `volumen_spike_ratio` > `2.1819` → IC=+0.207 (n=281)
...
- **PATRÓN** `sigma_h` < `0.0058` → IC=+0.181 (n=224)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.91€ cuando `sigma_h` < 0.0058 (IC base=+0.107)

- **PATRÓN** `drift_60min` |x|≤ `0.1259` → IC=+0.159 (n=224)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.80€ cuando `drift_60min` |x|≤ 0.1259 (IC base=+0.107)

- **PATRÓN** `hora_utc` > `15.0` → IC=+0.135 (n=332)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.67€ cuando `hora_utc` > 15.0 (IC base=+0.107)

- **PATRÓN** `ibs_20min` < `0.6` → IC=+0.191 (n=672)

  - _Acción_: Kelly boost +0.96€ cuando `ibs_20min` < 0.6 (IC base=+0.107)

- **PATRÓN** `dist_vwap_pct` < `0.2907` → IC=+0.130 (n=555)

  - _Acción_: Kelly boost +0.65€ cuando `dist_vwap_pct` < 0.2907 (IC base=+0.107)

- **PATRÓN** `sigma_ewma_delta_pct` < `3.195` → IC=+0.129 (n=655)

  - _Acción_: Kelly boost +0.64€ cuando `sigma_ewma_delta_pct` < 3.195 (IC base=+0.107)

- **PATRÓN** `volumen_regimen` < `0.7076` → IC=+0.136 (n=295)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_regimen` < 0.7076 (IC base=+0.107)

- **PATRÓN** `volumen_pendiente_norm` > `0.072` → IC=+0.149 (n=212)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_pendiente_norm` > 0.072 (IC base=+0.107)

- **PATRÓN** `volumen_spike_ratio` < `1.4441` → IC=+0.136 (n=174)

  - _Acción_: Kelly boost +0.68€ cuando `volumen_spike_ratio` < 1.4441 (IC base=+0.107)

- **PATRÓN** `volumen_spike_ratio` > `1.7892` → IC=+0.125 (n=347)

  - _Acción_: Kelly boost +0.62€ cuando `volumen_spike_ratio` > 1.7892 (IC base=+0.107)

- **PATRÓN** `libro_liquidez` > `1284.2338` → IC=+0.138 (n=598)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 1284.2338 (IC base=+0.107)
...
- **PATRÓN** `volumen_spike_ratio` > `1.4651` → IC=+0.210 (n=695)

  - _Acción_: Kelly boost +1.00€ cuando `volumen_spike_ratio` > 1.4651 (IC base=+0.217)

- **PATRÓN** `ballena_activa_n` < `35.0` → IC=+0.195 (n=591)
...
- **PATRÓN** `sigma_h` < `0.0102` → IC=+0.135 (n=1110)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.67€ cuando `sigma_h` < 0.0102 (IC base=+0.124)

- **PATRÓN** `drift_60min` |x|≤ `0.4394` → IC=+0.140 (n=1109)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.135 (n=485)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
...
- **PATRÓN** `dist_vwap_pct` > `1.3382` → IC=+0.141 (n=168)
  ...
  - _Acción_: Kelly boost +0.71€ cuando `dist_vwap_pct` > 1.3382 (IC base=+0.124)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `6.064` → IC=+0.127 (n=1286)

  - _Acción_: Kelly boost +0.63€ cuando `sigma_ewma_delta_pct` < 6.064 (IC base=+0.124)

- **PATRÓN** `volumen_regimen` > `0.8859` → IC=+0.125 (n=593)

  - _Acción_: Kelly boost +0.63€ cuando `volumen_regimen` > 0.8859 (IC base=+0.124)
...
- **PATRÓN** `volumen_spike_ratio` < `1.4629` → IC=+0.158 (n=416)

  - _Acción_: Kelly boost +0.79€ cuando `volumen_spike_ratio` < 1.4629 (IC base=+0.124)

- **PATRÓN** `volumen_spike_ratio` > `2.6893` → IC=+0.153 (n=416)

  - _Acción_: Kelly boost +0.77€ cuando `volumen_spike_ratio` > 2.6893 (IC base=+0.124)
...
- **PATRÓN** `ibs_20min` > `0.7912` → IC=+0.162 (n=211)

  - _Acción_: Kelly boost +0.81€ cuando `ibs_20min` > 0.7912 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` > `1.0341` → IC=+0.176 (n=100)

  - _Acción_: Kelly boost +0.88€ cuando `dist_vwap_pct` > 1.0341 (IC base=+0.129)

- **PATRÓN** `dist_vwap_pct` < `0.2556` → IC=+0.136 (n=394)

  - _Acción_: Kelly boost +0.68€ cuando `dist_vwap_pct` < 0.2556 (IC base=+0.129)
...
- **PATRÓN** `sigma_ewma_delta_pct` < `7.07` → IC=+0.138 (n=467)

  - _Acción_: Kelly boost +0.69€ cuando `sigma_ewma_delta_pct` < 7.07 (IC base=+0.129)

- **PATRÓN** `volumen_regimen` < `0.6468` → IC=+0.150 (n=155)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_regimen` < 0.6468 (IC base=+0.129)
...
- **PATRÓN** `volumen_spike_ratio` > `1.8484` → IC=+0.150 (n=304)

  - _Acción_: Kelly boost +0.75€ cuando `volumen_spike_ratio` > 1.8484 (IC base=+0.129)

- **PATRÓN** `libro_liquidez` > `8919.2028` → IC=+0.138 (n=415)

  - _Acción_: Kelly boost +0.69€ cuando `libro_liquidez` > 8919.2028 (IC base=+0.129)
...
- **FILTRO** `py_entrada` < `0.46` → IC=-0.184 (n=1269)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.024 (n=3834)

- **FILTRO** `py_entrada` > `0.63` → IC=-0.200 (n=1233)

  - _Acción_: SKIP cuando `py_entrada` > 0.63
  - _Potencial_: sin este filtro IC_bueno=+0.003 (n=4104)
...
- **FILTRO** `py_entrada` > `0.65` → IC=-0.231 (n=221)
  ...
  - _Potencial_: sin este filtro IC_bueno=+0.024 (n=666)
...
- **FILTRO** `py_entrada` > `0.55` → IC=-0.183 (n=219)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.002 (n=669)
...
- **FILTRO** `py_entrada` > `0.62` → IC=-0.191 (n=208)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=674)
...
- **FILTRO** `hora_utc` < `6.0` → IC=-0.142 (n=2702)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 6.0
...
- **FILTRO** `py_entrada` < `0.35` → IC=-0.285 (n=3101)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.036 (n=9494)

- **FILTRO** `ibs_7min` < `0.712` → IC=-0.246 (n=3147)

  - _Acción_: SKIP cuando `ibs_7min` < 0.712
  - _Potencial_: sin este filtro IC_bueno=-0.048 (n=9448)

- **FILTRO** `ballena_activa_n` > `19.0` → IC=-0.173 (n=4202)

  - _Acción_: SKIP cuando `ballena_activa_n` > 19.0
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=8393)

- **FILTRO** `py_entrada` > `0.69` → IC=-0.222 (n=3708)

  - _Acción_: SKIP cuando `py_entrada` > 0.69
  - _Potencial_: sin este filtro IC_bueno=-0.015 (n=12187)

- **FILTRO** `ibs_7min` > `0.7273` → IC=-0.169 (n=3972)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7273
  - _Potencial_: sin este filtro IC_bueno=-0.029 (n=11923)

### MOMENTUM_IBS_5M_BALLENA#BNB#5min
- **FILTRO** `hora_utc` < `8.0` → IC=-0.166 (n=534)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
...
- **FILTRO** `py_entrada` < `0.3` → IC=-0.328 (n=439)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.059 (n=1368)

- **FILTRO** `ibs_7min` < `0.2846` → IC=-0.271 (n=596)

  - _Acción_: SKIP cuando `ibs_7min` < 0.2846
  - _Potencial_: sin este filtro IC_bueno=-0.052 (n=1211)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.238 (n=437)
...
- **FILTRO** `hora_utc` < `7.0` → IC=-0.192 (n=456)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 7.0
...
- **FILTRO** `hora_utc` < `8.0` → IC=-0.144 (n=641)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: SKIP cuando `hora_utc` < 8.0
...
- **FILTRO** `ibs_7min` > `0.2769` → IC=-0.184 (n=540)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.063 (n=1624)

- **FILTRO** `ballena_activa_n` > `34.0` → IC=-0.183 (n=535)

  - _Acción_: SKIP cuando `ballena_activa_n` > 34.0
  - _Potencial_: sin este filtro IC_bueno=-0.064 (n=1629)

### MOMENTUM_IBS_5M_BALLENA#SOL#5min
- **FILTRO** `py_entrada` < `0.39` → IC=-0.241 (n=562)

  - _Acción_: SKIP cuando `py_entrada` < 0.39
  - _Potencial_: sin este filtro IC_bueno=-0.013 (n=1787)

- **FILTRO** `ibs_7min` < `0.7368` → IC=-0.209 (n=586)

  - _Acción_: SKIP cuando `ibs_7min` < 0.7368
  - _Potencial_: sin este filtro IC_bueno=-0.020 (n=1763)

- **FILTRO** `ballena_activa_n` > `13.0` → IC=-0.194 (n=556)

  - _Acción_: SKIP cuando `ballena_activa_n` > 13.0
  - _Potencial_: sin este filtro IC_bueno=-0.028 (n=1793)

- **FILTRO** `py_entrada` > `0.62` → IC=-0.177 (n=739)
...
- **FILTRO** `py_entrada` > `0.7` → IC=-0.249 (n=564)
  ...
  - _Acción_: SKIP cuando `py_entrada` > 0.7
  - _Potencial_: sin este filtro IC_bueno=-0.008 (n=2199)

- **FILTRO** `ibs_7min` > `0.7937` → IC=-0.157 (n=689)

  - _Acción_: SKIP cuando `ibs_7min` > 0.7937
  - _Potencial_: sin este filtro IC_bueno=-0.024 (n=2074)
...
- **FILTRO** `streak_len` > `3.0` → IC=-0.155 (n=27)
  ...
  - _Acción_: SKIP cuando `streak_len` > 3.0
  - _Potencial_: sin este filtro IC_bueno=+0.066 (n=233)
...
- **FILTRO** `libro_spread` > `0.01` → IC=-0.167 (n=19)
  ...
  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=+0.017 (n=1455)

### STRUCT_NO_15M#SOL#15min
- **FILTRO** `py_entrada` < `0.495` → IC=-0.147 (n=32)

  - _Acción_: SKIP cuando `py_entrada` < 0.495
  - _Potencial_: sin este filtro IC_bueno=+0.000 (n=826)

- **FILTRO** `libro_spread` > `0.01` → IC=-0.154 (n=24)

  - _Acción_: SKIP cuando `libro_spread` > 0.01
  - _Potencial_: sin este filtro IC_bueno=-0.001 (n=834)

### UPDOWN_GBM#15min
- **PATRÓN** `sigma_h` < `0.0041` → IC=+0.167 (n=196)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +0.83€ cuando `sigma_h` < 0.0041 (IC base=+0.124)

- **PATRÓN** `sigma_h` > `0.0064` → IC=+0.160 (n=148)
  - _Por qué funciona_: alta volatilidad → el modelo GBM sobreestima la señal; el mercado es más aleatorio
  - _Acción_: Kelly boost +0.80€ cuando `sigma_h` > 0.0064 (IC base=+0.124)

- **PATRÓN** `drift_60min` |x|≤ `0.1648` → IC=+0.129 (n=391)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +0.64€ cuando `drift_60min` |x|≤ 0.1648 (IC base=+0.124)

- **PATRÓN** `delta_ratio_macro` |x|> `0.0646` → IC=+0.135 (n=444)
...
- **PATRÓN** `hora_utc` > `12.0` → IC=+0.127 (n=306)
  - _Por qué funciona_: hora tardía/noche → sesión US cerrada, menos participantes informados; señales más ruidosas
  - _Acción_: Kelly boost +0.63€ cuando `hora_utc` > 12.0 (IC base=+0.124)

- **PATRÓN** `hora_utc` < `17.0` → IC=+0.128 (n=444)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +0.64€ cuando `hora_utc` < 17.0 (IC base=+0.124)

- **PATRÓN** `ibs_15` > `0.5581` → IC=+0.211 (n=444)
...
- **PATRÓN** `delta_ratio_macro` |x|> `0.1911` → IC=+0.153 (n=47)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +0.77€ cuando `delta_ratio_macro` |x|> 0.1911 (IC base=+0.075)

- **PATRÓN** `ibs_15` > `0.5659` → IC=+0.186 (n=103)
  - _Por qué funciona_: IBS alto (precio cerca del máximo) → sobrecompra de corto plazo; BUY_YES menos fiable
  - _Acción_: Kelly boost +0.93€ cuando `ibs_15` > 0.5659 (IC base=+0.075)

- **PATRÓN** `sigma_ewma_delta_pct` > `8.937` → IC=+0.125 (n=46)

  - _Acción_: Kelly boost +0.62€ cuando `sigma_ewma_delta_pct` > 8.937 (IC base=+0.075)
...
- **FILTRO** `sigma_ewma_delta_pct` > `17.977` → IC=-0.155 (n=442)
  ...
  - _Potencial_: sin este filtro IC_bueno=-0.057 (n=3426)
...
- **PATRÓN** `ballena_activa_n` < `128.0` → IC=+0.221 (n=320)
...
- **PATRÓN** `sigma_h` < `0.0079` → IC=+0.261 (n=182)
  - _Por qué funciona_: baja volatilidad → señal GBM más fiable; el spread de Polymarket cubre mejor el edge
  - _Acción_: Kelly boost +1.00€ cuando `sigma_h` < 0.0079 (IC base=+0.217)

- **PATRÓN** `drift_60min` |x|≤ `0.4521` → IC=+0.228 (n=182)
  - _Por qué funciona_: drift moderado → precio aún no ha reaccionado del todo; lag explotable
  - _Acción_: Kelly boost +1.00€ cuando `drift_60min` |x|≤ 0.4521 (IC base=+0.217)

- **PATRÓN** `drift_15min` |x|≤ `0.6005` → IC=+0.218 (n=122)

  - _Acción_: Kelly boost +1.00€ cuando `drift_15min` |x|≤ 0.6005 (IC base=+0.217)

- **PATRÓN** `delta_ratio_macro` |x|> `0.099` → IC=+0.233 (n=163)
  - _Por qué funciona_: flow macro dominante → el lado comprador/vendedor ya fijó el precio en Polymarket
  - _Acción_: Kelly boost +1.00€ cuando `delta_ratio_macro` |x|> 0.099 (IC base=+0.217)

- **PATRÓN** `divergencia_cvd_spot_perp` |x|≤ `0.0928` → IC=+0.231 (n=65)
...
- **PATRÓN** `hora_utc` < `5.0` → IC=+0.285 (n=63)
  - _Por qué funciona_: hora temprana → mercados cripto menos líquidos, spreads más amplios; edge real menor
  - _Acción_: Kelly boost +1.00€ cuando `hora_utc` < 5.0 (IC base=+0.217)

- **PATRÓN** `ibs_15` < `0.3764` → IC=+0.283 (n=182)
  - _Por qué funciona_: IBS bajo (precio cerca del mínimo) → sobreventa de corto plazo; BUY_NO menos fiable
  - _Acción_: Kelly boost +1.00€ cuando `ibs_15` < 0.3764 (IC base=+0.217)

- **PATRÓN** `dist_vwap_pct` > `0.4192` → IC=+0.267 (n=41)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` > 0.4192 (IC base=+0.217)

- **PATRÓN** `dist_vwap_pct` < `0.1628` → IC=+0.217 (n=182)

  - _Acción_: Kelly boost +1.00€ cuando `dist_vwap_pct` < 0.1628 (IC base=+0.217)

- **PATRÓN** `sigma_ewma_delta_pct` < `11.224` → IC=+0.244 (n=197)

  - _Acción_: Kelly boost +1.00€ cuando `sigma_ewma_delta_pct` < 11.224 (IC base=+0.217)

- **PATRÓN** `libro_liquidez` > `11577.7714` → IC=+0.232 (n=121)

  - _Acción_: Kelly boost +1.00€ cuando `libro_liquidez` > 11577.7714 (IC base=+0.217)

- **PATRÓN** `ballena_activa_n` < `184.0` → IC=+0.223 (n=153)
...
- **H-IBS-UPDOWN_GBM#15min**: dentro de BUY_YES, IBS > 0.5581 sube el IC de +0.124 a +0.211 en UPDOWN_GBM#15min (n=444). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM#ETH#15min**: dentro de BUY_YES, IBS > 0.5659 sube el IC de +0.075 a +0.186 en UPDOWN_GBM#ETH#15min (n=103). Ya aplicado como kelly_boost=+0.93€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO**: dentro de BUY_NO, IBS < 0.3542 sube el IC de -0.068 a +0.282 en UPDOWN_GBM_15M_TARDIO (n=356). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
- **H-IBS-UPDOWN_GBM_15M_TARDIO#ETH#15min**: dentro de BUY_NO, IBS < 0.3764 sube el IC de +0.217 a +0.283 en UPDOWN_GBM_15M_TARDIO#ETH#15min (n=182). Ya aplicado como kelly_boost=+1.00€ automático (shadow) — no es señal de reversión a la dirección contraria.
...
| ✅ BALLENAS_TARDIAS | 13548 | -0.107 | -2296.84€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#15min | 908 | -0.003 | -129.86€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#5min | 12640 | -0.114 | -2166.98€ | 0 | 0 |
...
| ✅ BALLENAS_TARDIAS#DOGE | 1743 | -0.157 | -530.56€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#DOGE#5min | 1743 | -0.157 | -530.56€ | 1 | 0 |
| ✅ BALLENAS_TARDIAS#ETH | 3704 | -0.061 | -350.82€ | 0 | 0 |
| ✅ BALLENAS_TARDIAS#ETH#5min | 3704 | -0.061 | -350.82€ | 1 | 0 |
...
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA | 3057 | -0.062 | +1338.36€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#15min | 865 | -0.006 | +442.18€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#5min | 2192 | -0.084 | +896.18€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC | 3057 | -0.062 | +1338.36€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#15min | 865 | -0.006 | +442.18€ | 0 | 0 |
| ✅ CANDIDATA10_CONFIRMACION_CRUZADA#BTC#5min | 2192 | -0.084 | +896.18€ | 0 | 0 |
...
| ✅ FAVORITO_CONFIRMADO | 43425 | +0.113 | -2809.42€ | 0 | 8 |
| ✅ FAVORITO_CONFIRMADO#15min | 7434 | +0.186 | -242.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#240min | 136 | -0.101 | -56.44€ | 0 | 0 |
| ✅ FAVORITO_CONFIRMADO#5min | 