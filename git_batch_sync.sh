...
if ! git diff --cached --quiet 2>/dev/null; then
    ...
    if [ "$(git branch --show-current)" = "main" ]; then
        ...
        _t_push0=$(now_ms)
        # 08-Sep (barrido de salud, aviso Javi via Telegram "pipeline
        # lento"): 60s dejó de bastar bajo carga alta -- logs/fast.log
        # mostró CADA push fallando exactamente a 60189/60244/60191ms
        # etc. durante más de 1 hora seguida (09:25-10:31 UTC), un bucle
        # que se retroalimenta solo: push agotado -> backlog no se
        # limpia -> el siguiente intento tiene AÚN MÁS que subir -> sigue
        # agotándose. git_ops.lock llegó a tardar 90s solo en adquirirse.
        # Mismo patrón de timeout fijo insuficiente ya visto y corregido
        # esta sesión en otros vigías (120s->400s) según crecen los datos
        # -- aquí sube 60s->180s. origin/main quedó ~3h y ~26 commits por
        # detrás hasta un push manual.
        if ! timeout -k 10 180s git push origin main >> "$LOG" 2>&1; then
            _t_push1=$(now_ms)
            ...
            if [ "$DIVERGIO" -gt 0 ] 2>/dev/null; then
                ...
                if timeout -k 10 180s git rebase -X ours origin/main >> "$LOG" 2>&1; then
                    ...
                    timeout -k 10 180s git push origin main >> "$LOG" 2>&1 \
