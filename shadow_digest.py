...
def enviar_telegram_critico(texto: str, log_fn=None) -> bool:
    """Para las alertas donde un fallo silencioso de Telegram significa
    que NADIE se entera de un incidente con dinero real (circuit
    breaker, reconciliación con mismatch, posiciones anómalas) —
    `enviar_telegram` devuelve False sin lanzar excepción (token/chat_id
    mal configurados, API caída) y ese False se ha ignorado en silencio
    en varios sitios del proyecto (auditoría 14-Jul, ver
    idea_auditoria_telegram_criticos_14jul). Reintenta 1 vez tras 2s
    antes de darse por vencido, y deja constancia por `log_fn` (o
    print si no se pasa) si ambos intentos fallan — para que quien mire
    el log sepa que hubo un aviso crítico que no llegó, en vez de asumir
    silenciosamente que si no hubo Telegram es porque no pasó nada.
    Patrón extraído de `nested_arb_trade._alertar_criticamente` (primer
    caso que lo necesitó) para poder reutilizarlo en el resto del
    proyecto sin duplicarlo de nuevo."""
    if enviar_telegram(texto):
        return True
    time.sleep(2)
    if enviar_telegram(texto):
        return True
    aviso = ("🚨 ALERTA TELEGRAM FALLÓ 2 VECES para un mensaje crítico — "
             "nadie ha sido avisado por Telegram. Revisar manualmente.")
    (log_fn or print)(aviso)
    return False
