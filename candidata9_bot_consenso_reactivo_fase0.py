...
latencia dedicado para CANDIDATA9_BOT_CONSENSO, petición explícita Javi
tras confirmar con rigor completo (n=151 fillable, 4/4 días positivos,
45 wallets, sin concentración) que el consenso mayoritario de bot
wallets en BTC#5min tiene edge real en la zona [0.50,0.55).

08-Sep: generalizado a TODOS los activos/marcos (antes hardcodeado a
BTC#5min únicamente) -- hallazgo real: `candidata9_10_gate_bucket.json`
confirmó ETH#15min[0.50,0.55) (n=366, shuffle_p=0.013, CI90% bootstrap
no cruza cero) y fill-ability real verificada aparte (n=256 mercados,
75% fillable, edge+7.2pp sobre ask), pero este observador solo vigilaba
BTC#5min -- la señal existía en el gate y en el fill-ability, pero NADA
la habría detectado en tiempo real. Mismo patrón que ya se cazó en
weather el mismo día (ver memoria feedback_nunca_dejar_candidatas_
confirmadas_sin_conectar_08sep).

Petición explícita Javi, 08-Sep, textual: "amplía el observer reactivo
de bot consenso a todos los marcos y monedas" -- en vez de una lista de
zonas confirmadas mantenida a mano (que exigiría volver a tocar este
fichero cada vez que el gate confirme algo nuevo), el observador ahora
CONSULTA EN CALIENTE `candidata9_10_gate_bucket.json` (mismo patrón
autoaprendiente que `gate_bucket_propio.py::evaluar()` en cripto) para
CUALQUIER activo/marco/bucket que ya esté `bueno_confirmado` -- vigila
TODO el firehose sin filtro de activo/marco, y solo registra predicción
cuando el precio de decisión cae en un bucket que el gate confirma
AHORA MISMO. Zonas nuevas que el gate confirme mañana se detectan solas,
sin tocar código. Fail-closed: sin gate cargado o bucket no confirmado
-> no registra, solo audita (mismo criterio que el resto del proyecto).
...
bucket_fase0.py):
  1. Trackea votos por condition_id de TODOS los activos/marcos (sin
     filtro, 08-Sep) de las bot wallets ya validadas
     (bot_wallets_universo_25ago.json).
  ...
  3. Consulta EN CALIENTE `_gate_confirma(activo, marco, precio)` contra
     candidata9_10_gate_bucket.json (08-Sep) -- si el (activo,marco) y
     el micro-bucket de precio de decisión están `bueno_confirmado`
     AHORA MISMO, Y sigue fillable en el instante de decisión, registra
     la predicción en predictions_YYYY-MM-DD.csv con
     STRATEGY="CANDIDATA9_BOT_CONSENSO" -- fuera de zona confirmada,
     solo se audita (no se registra como predicción, evita contaminar
     el aprendizaje causal con zonas no confirmadas, mismo criterio que
     el resto de fase0 del proyecto). Zonas que el gate confirme mañana
     se detectan solas, sin tocar este fichero.
...
from wallet_mirror_tracker import _archivos_activity, _fillability_mirror, leer_activity_incremental  # noqa: E402
...
ACTIVITY_CHECKPOINT_PATH = DIR_SHADOW / "candidata9_bot_consenso_reactivo_fase0_activity_checkpoint.json"
...
GATE_PATH = DIR_SHADOW / "candidata9_10_gate_bucket.json"
GATE_STEP = 0.05
...
from candidata9_gate_bucket import _gate_veredicto_dict, evaluar_para_recheck  # noqa: E402,F401


def _gate_confirma(activo: str, marco: str, precio: float) -> bool:
...
def _registrar_prediccion(condition_id: str, market_slug: str, lado_mayoria: str,
                           py: float, restante_s: float | None,
                           activo: str, marco: str) -> None:
...
def _procesar_condition(condition_id: str, votos: list[dict], vistos_trigger: set,
                         activo: str, marco: str) -> None:
    ...
    # /code-review 08-Sep: si ambas consultas de fillability fallan
    # (ask_det=ask_dec=None, ej. _fillability_mirror no resuelve el
    # token cerca del cierre del mercado), el fallback anterior usaba
    # trigger["precio"] SIN convertir -- viene crudo del CSV (str), y
    # _gate_confirma() hace `precio / GATE_STEP` -> TypeError, dejando el
    # condition_id marcado como visto sin auditoria ni log claro.
    # Corregido: castea explícitamente, y si ni eso es válido, no hay
    # precio de referencia -> no confirma zona (fail-closed).
    ...
    if en_zona and sigue_fillable:
        _registrar_prediccion(condition_id, market_slug, lado_mayoria, py_ref, None, activo, marco)
        registrada = True
...
def main() -> None:
    ...
    _log(f"arrancado -- {len(bots)} bot wallets, vigilando TODOS los activos/marcos "
         f"(zonas confirmadas consultadas en caliente vía {GATE_PATH.name})")
    ...
    # 07-Sep: usa el mismo checkpoint incremental que el bucle principal --
    # este backlog inicial consume el fichero hasta el final actual, así
    # que la primera pasada del `while True` de abajo ya solo ve filas
    # GENUINAMENTE nuevas (ver leer_activity_incremental(), arreglo real
    # de la sobrecarga crónica de CPU: 4 consumidores releían 1,5GB/4M
    # filas completos cada 5-15s).
        ...
        try:
            for row in leer_activity_incremental(ACTIVITY_CHECKPOINT_PATH):
                ...
                if not activo or not marco:
                    continue  # sin activo/marco clasificado, no se puede consultar el gate
                try:
            ...
            for condition_id, votos in list(votos_por_condition.items()):
                if condition_id in vistos_trigger:
                    continue
                activo, marco = votos[0]["activo"], votos[0]["marco"]
                _procesar_condition(condition_id, votos, vistos_trigger, activo, marco)
