...
def evaluar_tupla(filas, seed_key: str):
    """filas: [(ts, py, pnl), ...]. Devuelve dict con la ventana ganadora
    y su rigor, o {} si no hay suficiente n o ninguna ventana válida.

    `seed_key` (07-Sep, hallazgo real de Javi: el mismo bucket daba
    p-valores ligeramente distintos entre dos corridas del mismo día,
    minutos aparte, con el mismo n) -- antes había un `_rng` GLOBAL a
    nivel de módulo compartido entre TODAS las tuplas de una misma
    corrida: su estado avanza con cada llamada (bootstrap + 2000
    permutaciones), así que el resultado de la tupla N depende de
    cuántos números aleatorios consumieron las tuplas 1..N-1 antes --
    si el conjunto/orden de tuplas cambia entre corridas (una tupla nueva
    cruza N_MIN_TUPLA, una deja de tener datos, etc.) el mismo bucket
    recibe una secuencia de permutaciones distinta y su p-valor jitterea.
    Mismo patrón de no-determinismo ya fijado el 20-Ago en
    analisis_wallet_mirror_gate_bucket_10ago.py -- generador propio
    seedeado por hash estable de `seed_key` (nunca compartido entre
    llamadas), reproducible sin importar el resto de la corrida."""
    _rng = np.random.default_rng(zlib.crc32(seed_key.encode("utf-8")))
    n_total = len(filas)
