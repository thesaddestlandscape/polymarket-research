...
from shadow_digest import enviar_telegram, enviar_telegram_critico
...
def _resolver_tokens_desde_payload(data: dict, etiqueta: str) -> tuple[str, str]:
    """(yes_token_id, no_token_id) a partir de un payload YA obtenido de
    Gamma (por id o por slug, da igual el origen). Valida el orden real
    contra `outcomes` en vez de asumir ciegamente clobTokenIds[0]=YES/UP
    — si algún mercado trajera el orden invertido, asumirlo a ciegas
    compraría el token contrario con dinero real sin ningún aviso. Si
    `outcomes` no trae las etiquetas esperadas, falla fuerte en vez de
    arriesgar una dirección adivinada. `etiqueta` es solo para el mensaje
    de error (market_id o slug, lo que identifique el origen al caller)."""
    raw = data.get("clobTokenIds", [])
    tokens = json.loads(raw) if isinstance(raw, str) else raw
    if len(tokens) < 2:
        raise ValueError(f"clobTokenIds incompleto para {etiqueta}: {tokens}")

    raw_outcomes = data.get("outcomes", [])
    outcomes = json.loads(raw_outcomes) if isinstance(raw_outcomes, str) else raw_outcomes
    if len(outcomes) < 2:
        raise ValueError(f"outcomes incompleto para {etiqueta}: {outcomes}")
    ...
    raise ValueError(
        f"outcomes inesperados para {etiqueta}: {outcomes} — "
        f"no se puede mapear YES/NO con seguridad"
    )


def _get_token_ids(market_id: str) -> tuple[str, str]:
    """Devuelve (yes_token_id, no_token_id) desde Gamma API por market_id
    numérico. Ver `_resolver_tokens_desde_payload` para la validación de
    orden (se captura arriba en _ejecutar_orden_polymarket)."""
    resp = requests.get(
    ...
    data = resp.json()
    return _resolver_tokens_desde_payload(data, f"market {market_id}")
...
def _avisar_override_superado(key: str, n_real: int, min_n_global: int):
    ...
    log(f"  ℹ️  Override {key} superó min_n global ({n_real}≥{min_n_global}) — "
        f"{'avisado' if ok else 'INTENTO DE AVISO FALLIDO'} por Telegram")
...
def main():
    ...
    if disparado:
        ...
        # Alerta CRÍTICA con reintento (auditoría Telegram 14-Jul): un
        # circuit breaker disparado ya para el trading por sí solo (el
        # `return` de abajo), pero si la notificación falla en silencio
        # nadie se entera de inmediato de que el bot está parado.
    ...
    for pred in predicciones:
        ...
        # Auditoría Telegram 14-Jul: el trade ya está en trades.csv
        # (_registrar_trade arriba) independientemente de si este aviso
        # llega — un fallo de Telegram aquí retrasa la notificación, no
        # pierde el dato, así que basta con comprobar y dejar constancia
        # en el log (no hace falta el reintento de enviar_telegram_critico,
        # reservado para incidentes cuya ÚNICA constancia sería Telegram).
        if resultado["ok"]:
        ...
        if not ok_tg:
            log(f"  ⚠️ aviso Telegram de orden {mid} no llegó (trade ya registrado en trades.csv)")
    ...
    if ejecutados > 0:
        ...
        if not enviar_telegram(
            ...
            log("  ⚠️ aviso Telegram de resumen de ciclo no llegó (ver alertas por-orden arriba)")
